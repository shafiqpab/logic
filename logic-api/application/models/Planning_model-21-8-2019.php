...
class Planning_model extends CI_Model {
    ...
    public function get_menu_by_privilege($user_id)
     {
        ...
        if(!$all_comp) $all_comp=0;
        if(!$all_loc)  $all_loc=0;
        ...
        $complexity_level_data[4]['target'] = 1100;*/
    ...
    function po_details_info($company_id,$po_id) 
    {
       ...
       foreach($sql_data as $row)
       {
        ...
        foreach( $result as $row ) 
        {
            ...
            else if ($row->PRODUCTION_TYPE==3) $tsktype=85;
        ...
        $sql = "SELECT b.PO_BREAKDOWN_ID,b.ENTRY_FORM, min(a.receive_date)  as MINDATE, max(a.receive_date) as  MAXDATE, sum(quantity) as PROD_QNTRY 
        FROM inv_receive_master a,  order_wise_pro_details b, pro_grey_prod_entry_dtls c where  c.mst_id=a.id and c.id=b.dtls_id and b.entry_form in ( 2 ) and b.po_breakdown_id in ($po_id) group by b.po_breakdown_id,b.entry_form order by b.po_breakdown_id";
        $data_array=sql_select($sql) ;
        foreach($data_array as $row)
        {
        ...
        $sql = "SELECT b.PO_BREAKDOWN_ID, min(a.transaction_date) as MINDATE, max(a.transaction_date) as  MAXDATE, sum(quantity) as PROD_QNTRY,d.TRIM_TYPE 
        FROM inv_transaction a,  order_wise_pro_details b, product_details_master c , lib_item_group d
        where a.prod_id=c.id and b.trans_id=a.id and c.item_group_id=d.id and d.trim_type in (1,2) and b.entry_form in ( 24 ) and b.po_breakdown_id in ($po_id) group by b.po_breakdown_id,d.trim_type order by b.po_breakdown_id";
       // return $sql;
        ...
        $days_required=0;
        if($daily_production) $days_required=ceil($balance/$daily_production);
        $to_be_end=add_date(date("Y-m-d",time()),$days_required,1); 
        if(!isset($podtls['pub_shipment_date']))$podtls['pub_shipment_date']=null;
       ...
       // return $tna_task_update_data;
        ...
        {
            if(isset($tnadata[60]['start'])) $data_array[$i]["KNITTING_START"]=$tnadata[60]['start'];
            else $data_array[$i]["KNITTING_START"]=null;
            if(isset($tnadata[60]['end'])) $data_array[$i]["KNITTING_END"]=$tnadata[60]['end'];
            else $data_array[$i]["KNITTING_END"]=null;
            ...
            else  $data_array[$i]["KNITTING_QTY_REQUIRED"]=null;
            ...
            else $data_array[$i]["KNITTING_QTY_AVAILABLE"]=null;
            ...
            else $data_array[$i]["FIN_FAB_QTY_REQUIRED"]=null;
            ...
            else $data_array[$i]["FIN_FAB_QTY_AVAILABLE"]=null;
            if(isset($tnadata[12]['start'])) $data_array[$i]["PP_START"]=$tnadata[12]['start'];
            else  $data_array[$i]["PP_START"]=null;
            if(isset($tnadata[12]['end'])) $data_array[$i]["PP_END"]=$tnadata[12]['end'];
            else $data_array[$i]["PP_END"]=null;
            if(isset($tnadata[70]['start']))   $data_array[$i]["SEW_TRIM_REV_START"]=$tnadata[70]['start'];
            else    $data_array[$i]["SEW_TRIM_REV_START"]=null;
            if(isset($tnadata[70]['end'])) $data_array[$i]["SEW_TRIM_REV_END"]=$tnadata[70]['end'];
            else  $data_array[$i]["SEW_TRIM_REV_END"]=null;
            if(isset($tna_task_update_data[70]['reqqnty'])) $data_array[$i]["SEW_TRIM_REV_REQUIRED"]=number_format($tna_task_update_data[70]['reqqnty'],0,'.','');
            else $data_array[$i]["SEW_TRIM_REV_REQUIRED"]=null;
            if(isset($tnadata[84]['start'])) $data_array[$i]["CUT_START"]=$tnadata[84]['start'];
            else  $data_array[$i]["CUT_START"]=null;
            if(isset($tnadata[84]['end'])) $data_array[$i]["CUT_END"]=$tnadata[84]['end'];
            else  $data_array[$i]["CUT_END"]=null;
            if(isset($tna_task_update_data[84]['reqqnty']))  $data_array[$i]["CUT_REQUIRED"]=number_format($tna_task_update_data[84]['reqqnty'],0,'.','');
            else   $data_array[$i]["CUT_REQUIRED"]=null;
            if(isset($tna_task_update_data[84]['doneqnty']))   $data_array[$i]["CUT_AVAILABLE"]=number_format($tna_task_update_data[84]['doneqnty'],0,'.','');
            else $data_array[$i]["CUT_AVAILABLE"]=null;

            if(isset($tnadata[86]['start']))  $data_array[$i]["SEWING_START"]=$tnadata[86]['start'];
            else   $data_array[$i]["SEWING_START"]=null;
            if(isset($tnadata[86]['end']))   $data_array[$i]["SEWING_END"]=$tnadata[86]['end'];
            else    $data_array[$i]["SEWING_END"]=null;
            if(isset($tna_task_update_data[86]['reqqnty']))  $data_array[$i]["SEWING_REQUIRED"]=number_format($tna_task_update_data[86]['reqqnty'],0,'.','');
            else   $data_array[$i]["SEWING_REQUIRED"]=null;
            if(isset($tna_task_update_data[86]['doneqnty']))  $data_array[$i]["SEWING_AVAILABLE"]=number_format($tna_task_update_data[86]['doneqnty'],0,'.','');
            else   $data_array[$i]["SEWING_AVAILABLE"]=null;
            ...
            else  $data_array[$i]["location_name"]=null;
    ...
    function get_item_wise_line_efficiency_info($company_id,$item_id) 
    {
       ...
       // $search_prod_date=change_date_format(str_replace("'","",$txt_date),'yyyy-mm-dd');
        ...
        foreach($resource_data as $vals)
        {
            ...
            foreach($all_line_arr as $l_id)
            {
                if(isset($line_array[$l_id]))
                {
                    ...
                    if($l_name=="") $l_name.=$line_array[$l_id];
                    else $l_name.=','.$line_array[$l_id];   
                }   
        ...
        if($this->db->dbdriver=='mysqli') $prod_start_cond=" prod_start_time";
        else   $prod_start_cond=" TO_CHAR(prod_start_time,'HH24:MI')";
        ...
        foreach($line_data as $vals)
        {
           ...
           if($efficiency_min)
           {
                ...
                {
                    if(isset($efficiency_min))  $po_item_wise_eff_arr[$vals->SEWING_LINE] +=number_format( ($smv_qnty*100/$efficiency_min)/count($po_days_run_array[$vals->PO_BREAK_DOWN_ID]),2);
                }
                else
                {
    ...
    {
        if($this->db->dbdriver=='mysqli')$db_type=0;else $db_type=2;
        ...
        foreach($sql_machine_idle as $idleRow)
        {
            ...
            for($k=0; $k<$datediff_n; $k++)
            {
                ...
                else $idle_machine_array[$newdate_n] =$idleRow->MACHINE_ENTRY_TBL_ID;
            }
        }
    ...
    if($location) $location_cond_effi= "and location=$location "; else $location_cond_effi="";
    if($floor) $floor_con= "and floor_id=$floor "; else $floor_con="";
    $effi_data_arr=array();
    $po_id_arr=array();
    $sewProdArr=array();
  ...
  where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.production_type=5 and b.production_type=5 $companyCon $location_cond_effi $floor_con and a.prod_reso_allo=1 and a.production_date between '$firstDate' and '$lastDate' group by a.production_date, a.sewing_line, a.prod_reso_allo, a.po_break_down_id, a.item_number_id";
    ...
    if($location) $location_cond_subcon= "and a.location_id=$location "; else $location_cond_subcon="";
    if($pro_company){$companyCon="and a.company_id=$pro_company";}else{$companyCon="and a.company_id=$company";}
    if($floor) $floor_con= "and a.floor_id=$floor "; else $floor_con="";
    ...
    foreach($subconProdData as $row)
    {
         ...
         else $sewProdArr[$production_date] =$row->PRODUCTION_QNTY;
         if(isset($subAchvSmv_arr[$production_date]))
         $subAchvSmv_arr[$production_date]+=$row->PRODUCTION_QNTY*$row->SMV;
         else $subAchvSmv_arr[$production_date] =$row->PRODUCTION_QNTY*$row->SMV;
         if(isset($effi_data_arr[$production_date]))
         $effi_data_arr[$production_date].=$row->SEWING_LINE."**".$row->PRODUCTION_QNTY."**".$row->ORDER_ID."**".$row->ITEM_NUMBER_ID."**".$row->PROD_RESO_ALLO.",";
        else $effi_data_arr[$production_date] =$row->SEWING_LINE."**".$row->PRODUCTION_QNTY."**".$row->ORDER_ID."**".$row->ITEM_NUMBER_ID."**".$row->PROD_RESO_ALLO.",";
    ...
    if($location) $location_cond_item= "and a.location_name=$location "; else $location_cond_item="";
    ...
    else
    {
        ...
        if($pro_company){
            ...
            {
                if($p==1){$whereCon .=" and ( b.id in(".implode(',',$po_process).")";} 
    ...
    foreach($tpd_data_arr as $row)
    {
        ...
        if($row->SMV_ADJUST_TYPE==1) 
        {
            ...
            else $tsmvArr[$production_date][$row->ID]+=$row->SMV_ADJUST;
        }
    ...
    for($j=0;$j<$datediff;$j++)
    {
        ...
        $achv_smv=0;
        $today_smv=0;
        if(isset($effi_data_arr[$prod_date]))  $effi_data=explode(",",substr($effi_data_arr[$prod_date],0,-1));
        else $effi_data=array();
        ...
        if(isset($subAchvSmv_arr[$prod_date])) $achv_smv+=$subAchvSmv_arr[$prod_date];
        if(isset($today_smv_arr[$prod_date]))  $today_smv=array_sum($today_smv_arr[$prod_date]); 
        else  $today_smv=0;  
        if($today_smv)    
    ...
    {
        $data_array=array();
        $sqls="SELECT  ID, PLAN_ID, PO_BREAK_DOWN_ID,  PLAN_QNTY, PO_DTLS, ITEM_NUMBER_ID,    COUNTRY_ID, COLOR_NUMBER_ID, SIZE_NUMBER_ID, JOB_NO, PO_ID,SMV FROM ppl_sewing_plan_board_powise where PLAN_ID='$plan_id'";
        $data_sql=sql_select($sqls);
        $color_sql=sql_select("SELECT ID,COLOR_NAME from lib_color where status_active=1");
        $color_arr=array();
        $color_arr[0]="";
        foreach($color_sql as $v) $color_arr[$v->ID]=$v->COLOR_NAME;
        ...
        foreach($data_sql as $v)
        {
            ...
            if(isset($color_arr[$v->COLOR_NUMBER_ID]))   $data_array[$k]["COLOR_NAME"]=$color_arr[$v->COLOR_NUMBER_ID] ;
            else $data_array[$k]["COLOR_NAME"]="";
    ...
    function get_job_data_info($cbo_company_mst, $cbo_buyer_name = "0", $chk_job_wo_po = "0", $txt_date_from, $txt_date_to, $garments_nature = "", $txt_job_prifix = "", $cbo_year_selection, $cbo_string_search_type = "", $txt_order_search = "", $cbo_date_type,$plan_level="0", $txt_style_ref="", $ignore_tna="0", $order_status="0",$po_break_down_id=0, $color_size_id=0) 
    {
        ...
        $is_confirmed_cond="";
        if($order_status) $is_confirmed_cond.=" and b.is_confirmed='$order_status'";
		...
		if($order_status) $gsd_were_con.=" and e.is_confirmed='$order_status'";
        ...
        {
            if($search_types==1) $gsd_were_con .= " and d.style_ref_no = '$txt_style_ref'  ";
            else if($search_types==2) $gsd_were_con .= " and d.style_ref_no like '%$txt_style_ref%'  ";
            else if($search_types==3) $gsd_were_con .= " and d.style_ref_no like '%$txt_style_ref'  ";
            else if($search_types==4) $gsd_were_con .= " and d.style_ref_no like '%$txt_style_ref%'  ";
        }
		...
		{
			if($search_types==1) $gsd_were_con .= " and e.po_number = '$txt_order_search'  ";
			else if($search_types==2) $gsd_were_con .= " and e.po_number like '%$txt_order_search%'  ";
			else if($search_types==3) $gsd_were_con .= " and e.po_number like '%$txt_order_search'  ";
			else if($search_types==4) $gsd_were_con .= " and e.po_number like '%$txt_order_search%'  ";
        ...
        if (trim($txt_order_search))
        {
            if($search_types==1) $order_cond = " and b.po_number = '$txt_order_search'  ";
            else if($search_types==2) $order_cond = " and b.po_number like '%$txt_order_search%'  ";
            else if($search_types==3) $order_cond = " and b.po_number like '%$txt_order_search'  ";
            else if($search_types==4) $order_cond = " and b.po_number like '%$txt_order_search%'  ";
        ...
        if (trim($txt_style_ref))
        {
            if($search_types==1) $style_cond = " and a.style_ref_no = '$txt_style_ref'  ";
            else if($search_types==2) $style_cond = " and a.style_ref_no like '%$txt_style_ref%'  ";
            else if($search_types==3) $style_cond = " and a.style_ref_no like '%$txt_style_ref'  ";
            else if($search_types==4) $style_cond = " and a.style_ref_no like '%$txt_style_ref%'  ";
        ...
        $conds="";
        if($txt_style_ref) $conds.=" and  style_ref_no like '%$txt_style_ref%'";
        if($txt_order_search) $conds.=" and  po_number like '%$txt_order_search%'";
        ...
        if($all_jobs_by_style_st)$all_jobs_by_style_cond.=" and job_no in($all_jobs_by_style_st)";
        if(count($all_jobs_by_style_arr)>999)
        {
            ...
            {
                $jobs="'".implode("','", $v)."'";
                if($all_jobs_by_style_cond=="")$all_jobs_by_style_cond.=" and (job_no in($jobs)";
                else $all_jobs_by_style_cond.=" or  job_no in($jobs)";
            }
            $all_jobs_by_style_cond.=")";

        }
        //return $all_jobs_by_style_cond;
        ...
        $sql = "SELECT b.PLAN_QNTY,b.PO_BREAK_DOWN_ID,b.ITEM_NUMBER_ID,b.COLOR_NUMBER_ID from ppl_sewing_plan_board a, ppl_sewing_plan_board_powise b where a.plan_id=b.plan_id   ";
        ...
        if ($sql->num_rows() > 0) 
        {
            ...
            foreach ($sql->result() as $srows)
            {
                if(!$srows->COLOR_NUMBER_ID)$srows->COLOR_NUMBER_ID=0;
        ...
        if($plan_level != 1){
            $sql .= " group by c.item_number_id, b.is_confirmed,a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name, a.style_ref_no,a.job_quantity, b.id,b.po_number,b.shipment_date,c.country_ship_date,a.garments_nature,a.insert_date,b.id, set_break_down,total_set_qnty,set_smv $group_by";
        }
        ...
        foreach ($sql_exe as $rows) {
            ...
            foreach ($set as $setdtls) {
                ...
                else $data_array[$i]["TNA_START_DATE"] ="01-01-1970";
                ...
                else $data_array[$i]["TNA_FINISH_DATE"] ="01-01-1970";
                ...
                $color_index=0;
                if($plan_level==2)$color_index=$rows->COLOR_NUMBER_ID;;
                ...
                if(!$order_qty_id)$order_qty_id=0;
                if(!$gsd_smv_val)$gsd_smv_val=0;
...
function get_plan_data_info($company_id,$location_id,$floor_id,$txt_date_from, $user_id,$auto_balancing=1) 
{
    ...
    $loc_cond="";
    if($location_id) $loc_cond.=" and location_name='$location_id' ";
     
    if($user_arr_type[$user_id]==1)
    {
         
         $sql_line="SELECT a.BOARD_STATUS,a.USER_ID ,b.IS_PLANNER from planning_board_status a,user_passwd b  where b.id=a.user_id and b.IS_PLANNER=1 and  a.company_name='$company_id' $loc_cond $floor_cond_line_sts   order by a.BOARD_STATUS asc";
		  //return $sql_line;
   ...
   a.next_increment, a.next_terget,a.increment_qty,a.terget,a.day_wise_plan,a.company_id,a.location_id,a.item_number_id, a.off_day_plan,a.order_complexity,a.ship_date, extra_param,a.plan_level,a.first_day_capacity,a.last_day_capacity,a.seq_no,a.po_company_id,a.use_learning_curve,a.current_production_date,a.production_percent, a.top_border_color,a.bottom_border_color,a.left_color,a.right_color , a.MERGE_COMMENTS,a.MERGE_TYPE,a.INSERT_DATE ,a.NOTES,a.CLOSING_STATUS,a.CLOSED_BY,a.CLOSING_DATE,a.CLOSING_NOTE,a.RE_OPEN_DATE,a.RE_OPENED_BY,a.RE_OPEN_NOTE order by a.id desc  ";
    ...
    if(!empty($line_id_arr)){
        $production_sql="select a.PO_BREAK_DOWN_ID, a.SEWING_LINE,sum(b.PRODUCTION_QNTY) PRODUCTION_QNTY from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id   and a.production_type=5 $sql3  and a.status_active=1 and a.is_deleted=0 group by a.po_break_down_id, a.SEWING_LINE";
    ...
    foreach ($plan_data as $rows) 
    {
        ...
        if(!empty($production_arr) && ($rows->PO_BREAK_DOWN_ID>0))
        {
            foreach(array_unique(explode(",", $rows->PO_BREAK_DOWN_ID )) as $po_val)
            {
                ...
                if(isset($production_arr[$po_val][$rows->LINE_ID]["PRODUCTION_QNTY"])){
        ...
        if(1==1){
            ...
            if( $multi_po==0 )
            {
                ...
                foreach($npo_arr as $v)
                {
                    if($all_buyer)
                    {
                        if(  isset( $wo_po_details[$v]['buyer_name'] ))
                        {
                            $all_buyer.=','. $buyer_arr[$wo_po_details[$v]['buyer_name']];
                        }
                    ...
                    {  
                      if(isset($wo_po_details[$v]['buyer_name']))   
                      {
                         if(isset($buyer_arr[$wo_po_details[$v]['buyer_name']]))
                             $all_buyer.=  $buyer_arr[$wo_po_details[$v]['buyer_name']];
                      }
                     
                    }
                }
                 $data_array[$i]["BUYER_NAME"] =$all_buyer;
            }
            else
            {
                if(isset( $wo_po_details[$rows->PO_BREAK_DOWN_ID]['buyer_name'] ) && isset($buyer_arr[$wo_po_details[$rows->PO_BREAK_DOWN_ID]['buyer_name']]))
                $data_array[$i]["BUYER_NAME"] =$buyer_arr[$wo_po_details[$rows->PO_BREAK_DOWN_ID]['buyer_name']];
                else  
                $data_array[$i]["BUYER_NAME"] = "";
            ...
            if( $multi_po==0 )
            {
                ...
                foreach($npo_arr as $v)
                {
                    if($all_jobs)
                    {
                        if(isset($wo_po_details[$v]['job_no']))
                        {
                            $all_jobs.=','. $wo_po_details[$v]['job_no'];
                        }
                   ...
                   /* if(!$all_smv)
                    {
                        if(isset($job_smv_arr[$wo_po_details[$v]['job_no']][$items]) )
                        {
                           $all_smv.=','.$job_smv_arr[$wo_po_details[$v]['job_no']][$items];
                        }
                 ...
                 $data_array[$i]["JOB_NO"] =$all_jobs;
                 //$data_array[$i]["SMV"] = implode("," ,array_unique( explode(",",chop($all_smv,","))));
            }

            else
            {
                 if(isset($wo_po_details[$rows->PO_BREAK_DOWN_ID]['job_no']))
                 $data_array[$i]["JOB_NO"] = $wo_po_details[$rows->PO_BREAK_DOWN_ID]['job_no'];
                else $data_array[$i]["JOB_NO"] ="";
                /*if(isset($wo_po_details[$rows->PO_BREAK_DOWN_ID]['job_no']))
                {
                    if(isset($job_smv_arr[$wo_po_details[$rows->PO_BREAK_DOWN_ID]['job_no']][$rows->ITEM_NUMBER_ID])) $data_array[$i]["SMV"] = implode(",", array_unique( explode(",",$job_smv_arr[$wo_po_details[$rows->PO_BREAK_DOWN_ID]['job_no']][$rows->ITEM_NUMBER_ID])));
                    else
                         $data_array[$i]["SMV"] =0;

                }
                
                 else  $data_array[$i]["SMV"] =0;*/
            ...
            if( $multi_po==0 )
            {
                ...
                foreach($npo_arr as $v)
                {
                    if($all_style)
                    {
                        if(isset($wo_po_details[$v]['style_ref']))
                        {
                            $all_style.=','. $wo_po_details[$v]['style_ref'];
                        }
                 ...
                 $data_array[$i]["STYLE_REF_NO"] =$all_style;
            }
            ...
            else
            {
                ...
                if(isset($wo_po_details[$rows->PO_BREAK_DOWN_ID]['style_ref']))
                $data_array[$i]["STYLE_REF_NO"] = $wo_po_details[$rows->PO_BREAK_DOWN_ID]['style_ref'];
                else $data_array[$i]["STYLE_REF_NO"] ="";
            }
            ...
            if( $multi_po==0 )
            {
                ...
                foreach($npo_arr as $v)
                {
                    if(isset($wo_po_details[$v]['po_quantity']))
            ...
            else  $data_array[$i]["location_id"] =0;
            ...
            else $data_array[$i]["location_name"] ="";
           ...
           if(isset( $rows->ITEM_NUMBER_ID))
            ...
            foreach($npo_arr as $v)
            {
                if(isset( $tna_task_data[$v]['task_start_date']))
                {
                    ...
                    {
                        if($task_start_date<$st_date)$early_date=$tna_task_data[$v]['task_start_date'];
                    }
                    ...
                    if(isset( $tna_task_data[$v]['task_finish_date']))
                    {
                        ...
                        {
                            if($task_finish_date>$end_date)$delay_date=$tna_task_data[$v]['task_finish_date'];
                        }
...
function po_vs_plan_info_data($company_id,$txt_date_from="", $txt_date_to="")
{
    ...
    foreach ($color_size_data as $rows) 
    {
        ...
        if(isset($plan_arr[$rows->ID]))   $data_array[$i]["plan_qnty"]=$plan_arr[$rows->ID];
        else $data_array[$i]["plan_qnty"]=0;
        $i++;
    }
    $null_arr=array();
...
function get_line_po_wise_plan_data_info($company_id,$po_id="",$line_id,$plan_id)
{
    ...
    $actual_link=$url_arr[0]."/".$url_arr[1];
    $multi_po=0;
    if (strpos($po_id, ',') !== false)$multi_po=1;

     
    $prod_reso_allo= get_resource_allocation_variable($company_id);

    //$production_sql="select PO_BREAK_DOWN_ID,SUM(PRODUCTION_QUANTITY) AS PRODUCTION_QUANTITY,PRODUCTION_DATE,SEWING_LINE,COMPANY_ID from   pro_garments_production_mst where production_type=5 and company_id=$company_id and po_break_down_id=$po_id and sewing_line=$line_id and status_active=1 and is_deleted=0 group by production_date,po_break_down_id,sewing_line,company_id order by sewing_line,po_break_down_id,production_date";
    if($prod_reso_allo==1)
    {
    ...
    if(!empty($production_data)){       
		...
		/*   if($multi_po==0)
            $production_arr[$pr_row->PO_BREAK_DOWN_ID][$pr_row->LINE_NUMBER][$pr_row->PRODUCTION_DATE] = $pr_row->PRODUCTION_QUANTITY;
                ...
                $production_arr[0][$pr_row->LINE_NUMBER][$pr_row->PRODUCTION_DATE] = $pr_row->PRODUCTION_QUANTITY;
        */
    ...
    foreach ($daywise_plan as $rows) 
    {
        ...
        if(!empty($production_arr))
        {
            ...
            {
                 if($multi_po==0)$production_line_qnty=$production_arr[$rows->PO_BREAK_DOWN_ID][$rows->LINE_ID][$rows->PLAN_DATE];
                 else $production_line_qnty=$production_arr[0][$rows->LINE_ID][$rows->PLAN_DATE];
            }
        }
...
function get_line_list_info($company_id,$location_id="0",$floor_id="0")
{
    ...
    if(($floor_id*1) !="0")$floor_cond_sewing = " and a.floor_name='$floor_id'"; else {$floor_cond_sewing = "";}
...
function get_week_list_info($company_id,$txt_date_from="", $txt_date_to="")
{
    ...
    if($txt_date_to=='') $txt_date_to=add_date($bk_from,120,1);
...
function create_plan($plan_obj)
{
    ...
    if($response_obj->Status == true)
    {
        ...
        foreach ($response_obj->SewingPlanBoard as $sewing_plan_row) 
        {
            ...
            if($this->db->dbdriver=='mysqli')
            {
                $ppl_sewing_plan_board_data = array(
					...
					'RE_OPEN_NOTE'		=> $sewing_plan_row->RE_OPEN_NOTE,					
					
					'NOTES'             => $sewing_plan_row->NOTEFORSTRIP,
        ...
        foreach ($response_obj->SewingPlanBoardDtls as $sewing_plan_dtls_row) 
        {
             ...
             {
                    if($dtls_plan_delete)$dtls_plan_delete.=','.$sewing_plan_dtls_row->PLAN_ID;
                    else $dtls_plan_delete.=$sewing_plan_dtls_row->PLAN_ID;
             }
        ...
        foreach ($response_obj->SewingPlanBoardDtls as $sewing_plan_dtls_row) 
        {
            ...
            {
                /*if($sewing_plan_dtls_row->RowState == "add" || $sewing_plan_dtls_row->RowState == "update")
                {

                    $plan_id = ($sewing_plan_dtls_row->RowState == "add") ? $plan_ids[$sewing_plan_dtls_row->PLAN_ID] : $sewing_plan_dtls_row->PLAN_ID;

                    $this->db->query("delete from  ppl_sewing_plan_board_dtls where plan_id in($plan_id)");
...
public function get_archive_info($company=0,$location=0,$floor=0,$generate_date="")
{
    ...
    $all_cond="";
    if($company)$all_cond.=" and company_id ='$company'";
    if($location)$all_cond.=" and location_id ='$location'";
...
function get_capacity_data_info($fromDate,$toDate,$company,$location,$type)
{
    ...
    if(count($data)>0)
    {
        foreach($data as $key=>$val)
        {
            ...
            if(isset($data_smv[$key])) $data_arr[$k]["smv"]=$data_smv[$key] ;
            else $data_arr[$k]["smv"]=0;
            if(isset($data_qty[$key]))      $data_arr[$k]["qty"]=$data_qty[$key]    ;
            else
                $data_arr[$k]["qty"]=0 ;
...
function get_line_capacity_info($fromDate='',$toDate='')
{
	if($this->db->dbdriver=='mysqli')
	{ 
		$dateFormat="Y-m-d";
