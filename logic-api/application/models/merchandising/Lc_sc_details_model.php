...
class Lc_sc_details_model extends CI_Model
{
    ...
    function lc_sc_details()
    {
        ...
        $query_delivery_chalan_no = "SELECT ID,SYS_NUMBER FROM PRO_EX_FACTORY_DELIVERY_MST WHERE STATUS_ACTIVE = 1 AND IS_DELETED = 0";
        $table_delivery_chalan_no = $this->db->query($query_delivery_chalan_no)->result();
        $delivery_chalan_no = array();
        foreach ($table_delivery_chalan_no as $row) {
            $delivery_chalan_no[$row->ID] = $row->SYS_NUMBER;
        }
        ...
        $query = "SELECT i.EX_FACTORY_QNTY,i.INVOICE_NO,g.JOB_NO, b.PO_BREAKDOWN_ID, h.PO_NUMBER, h.PO_RECEIVED_DATE, h.PUB_SHIPMENT_DATE, g.BUYER_NAME, i.ITEM_NUMBER_ID, a.INVOICE_NO, a.LC_SC_ID, a.INVOICE_NO, c.EXPORT_LC_NO, c.INTERNAL_FILE_NO_BK, e.CONTRACT_NO, e.BANK_FILE_NO, e.INTERNAL_FILE_NO, e.CONVERTIBLE_TO_LC, g.LOCATION_NAME, g.PRODUCT_DEPT, h.PO_TOTAL_PRICE, h.PO_QUANTITY, h.UNIT_PRICE, g.ORDER_UOM, e.CONTRACT_NO,i.DELIVERY_MST_ID FROM COM_EXPORT_INVOICE_SHIP_MST a, COM_EXPORT_INVOICE_SHIP_DTLS b, COM_EXPORT_LC c, COM_EXPORT_LC_ORDER_INFO d, COM_SALES_CONTRACT e, COM_SALES_CONTRACT_ORDER_INFO  f, WO_PO_DETAILS_MASTER g, WO_PO_BREAK_DOWN h, PRO_EX_FACTORY_MST i WHERE a.id = b.MST_ID AND c.ID = d.COM_EXPORT_LC_ID AND b.PO_BREAKDOWN_ID = d.WO_PO_BREAK_DOWN_ID AND e.ID = f.COM_SALES_CONTRACT_ID AND f.WO_PO_BREAK_DOWN_ID = b.PO_BREAKDOWN_ID AND g.ID = h.JOB_ID AND b.PO_BREAKDOWN_ID = h.ID AND i.PO_BREAK_DOWN_ID = b.PO_BREAKDOWN_ID AND a.STATUS_ACTIVE = 1 AND a.IS_DELETED = 0 AND b.STATUS_ACTIVE = 1 AND b.IS_DELETED = 0 AND c.STATUS_ACTIVE = 1 AND c.IS_DELETED = 0 AND d.STATUS_ACTIVE = 1 AND d.IS_DELETED = 0 AND e.STATUS_ACTIVE = 1 AND e.IS_DELETED = 0 AND f.STATUS_ACTIVE = 1 AND f.IS_DELETED = 0 AND g.STATUS_ACTIVE = 1 AND g.IS_DELETED = 0 AND h.STATUS_ACTIVE = 1 AND h.IS_DELETED = 0 AND i.STATUS_ACTIVE = 1 AND i.IS_DELETED = 0";
        $table = $this->db->query($query)->result();
        //print_r($table);die;
        $ship_quantity_by_po_chalan = Array();
        $po_quantity_by_po_chalan = Array();
        $price_by_po_chalan = Array();
        $return_data = array();
        foreach ($table as $row) {

            $ship_quantity_by_po_chalan[$row->PO_BREAKDOWN_ID][$delivery_chalan_no[$row->DELIVERY_MST_ID]] += $row->EX_FACTORY_QNTY;
            $po_quantity_by_po_chalan[$row->PO_BREAKDOWN_ID][$delivery_chalan_no[$row->DELIVERY_MST_ID]] += $row->PO_QUANTITY;
            $price_by_po_chalan[$row->PO_BREAKDOWN_ID][$delivery_chalan_no[$row->DELIVERY_MST_ID]] += $row->PO_TOTAL_PRICE;

            $return_data[$row->PO_BREAKDOWN_ID][$delivery_chalan_no[$row->DELIVERY_MST_ID]] = [
                "job_no" => $row->JOB_NO,
                ...
                "delivery_chalan_no" =>  $delivery_chalan_no[$row->DELIVERY_MST_ID],
                //"item_category" =>  $lib_pro_dept[$row->PRODUCT_DEPT],
                "item_category" =>  $row->ITEM_NUMBER_ID,
                "price_list" =>$price_by_po_chalan[$row->PO_BREAKDOWN_ID][$delivery_chalan_no[$row->DELIVERY_MST_ID]],
                "order_qty" =>  $row->PO_QUANTITY,
                "ship_qty" => $ship_quantity_by_po_chalan[$row->PO_BREAKDOWN_ID][$delivery_chalan_no[$row->DELIVERY_MST_ID]],
