...
class Fabric_inventory_receipt_model extends CI_Model
{
	...
	function Fabric_inventory_receipt($start_date,$end_date,$booking_no)
	{
		//print_r(5);die;
		//UOM lib
		...
		$query_job_details = "SELECT a.TRANS_ID,b.STYLE_REF_NO,b.JOB_NO FROM ORDER_WISE_PRO_DETAILS a, WO_PO_DETAILS_MASTER b, WO_PO_BREAK_DOWN c WHERE a.PO_BREAKDOWN_ID = c.ID and b.ID = c.JOB_ID and a.STATUS_ACTIVE = 1 and a.IS_DELETED = 0 and b.STATUS_ACTIVE = 1 and b.IS_DELETED = 0 and c.STATUS_ACTIVE = 1 and c.IS_DELETED = 0 ";
		$table_job_details = $this->db->query($query_job_details)->result();
		$job_info_by_trans_id = array();
		foreach ($table_job_details as $row) {
			$job_info_by_trans_id[$row->TRANS_ID]["STYLE_REF_NO"] = $row->STYLE_REF_NO;
			$job_info_by_trans_id[$row->TRANS_ID]["JOB_NO"] = $row->JOB_NO;
		}
		...
		if ($start_date && $end_date) {
			$where_con = "AND a.RECEIVE_DATE between TO_DATE('$start_date', 'DD/MM/YYYY') and TO_DATE('$end_date', 'DD/MM/YYYY')";
		...
		$query_receive = "SELECT a.RECV_NUMBER, a.RECEIVE_DATE, a.BOOKING_ID, a.BOOKING_NO, a.CURRENCY_ID, b.COMPANY_ID, b.SUPPLIER_ID, b.PROD_ID, b.ORDER_UOM, b.ORDER_RATE, b.ORDER_QNTY, b.ORDER_AMOUNT,b.ID as TRANS_ID FROM INV_RECEIVE_MASTER a, INV_TRANSACTION b WHERE a.ID = b.MST_ID AND b.TRANSACTION_TYPE = 1 AND b.ITEM_CATEGORY = 3 AND a.ITEM_CATEGORY = 3 $where_con and  a.STATUS_ACTIVE = 1 AND a.IS_DELETED = 0 AND b.STATUS_ACTIVE = 1 AND b.IS_DELETED = 0";
		$table_receive = $this->db->query($query_receive)->result();
		//print_r($query_receive);die;
		$return_array = array();
		foreach ($table_receive as $row) {
			$return_array[$row->RECV_NUMBER][$row->PROD_ID] = [
                "receipt_no" => $row->RECV_NUMBER,
                "booking_no" => $row->BOOKING_NO,
                "job_no" => $job_info_by_trans_id[$row->TRANS_ID]["JOB_NO"],
                "style_ref_no" => $job_info_by_trans_id[$row->TRANS_ID]["STYLE_REF_NO"],
