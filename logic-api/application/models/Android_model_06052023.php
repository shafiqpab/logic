...
class Android_model extends CI_Model {
	...
	public function login($user_id, $password, $device_id="") {
		$empty_arrr = [];
	...
	public function finish_prod_dtls_list_view_data($batch_id = 0,$entry_form_no = 0) { //Finish start
...
100 => 'Others');
	...
	foreach($yarn_lot_data as $rows)
	{
		...
		$YARN_COUNT=$rows->YARN_COUNT;
		$BRAND_ID=$rows->BRAND_ID;
		$PO_BREAKDOWN_ID=$rows->PO_BREAKDOWN_ID;
		$YARN_LOT=$rows->YARN_LOT;
		$YARN_LOTS=explode(",",$YARN_LOT);
		$BRAND_ID=explode(",",$BRAND_ID);
		$yarn_lot_arr[$PROD_ID][$PO_BREAKDOWN_ID]['lot']=implode(", ",array_unique($YARN_LOTS));
		$yarn_lot_arr[$PROD_ID][$PO_BREAKDOWN_ID]['yarn_count']=$YARN_COUNT;
		$yarn_lot_arr[$PROD_ID][$PO_BREAKDOWN_ID]['brand_id']=implode(", ",array_unique($BRAND_ID));

		$yarn_lot_arr_prodwise[$PROD_ID]['lot']=implode(", ",array_unique($YARN_LOTS));
		$yarn_lot_arr_prodwise[$PROD_ID]['yarn_count']=$YARN_LOT;
		$yarn_lot_arr_prodwise[$PROD_ID]['brand_id']=implode(", ",array_unique($BRAND_ID));
...
a.batch_weight,a.process_id,a.color_id,a.color_range_id,a.booking_without_order,a.booking_no";
			  $select_list=" LISTAGG(B.PO_ID,',') WITHIN GROUP (ORDER BY B.PO_ID) AS PO_ID";
			
			$sql_re="SELECT MAX(A.ID) AS ID,A.BATCH_NO,A.IS_SALES,A.ENTRY_FORM,A.BATCH_DATE,A.COMPANY_ID,A.TOTAL_TRIMS_WEIGHT, A.BATCH_WEIGHT,MAX(A.EXTENTION_NO) AS EXTENTION_NO,A.PROCESS_ID AS PROCESS_ID_BATCH,A.COLOR_ID,A.COLOR_RANGE_ID,MAX(A.INSERT_DATE) AS INSERT_DATE, A.BOOKING_WITHOUT_ORDER,A.BOOKING_NO, SUM(B.BATCH_QNTY) AS BATCH_QNTY, $select_list from pro_batch_create_mst a,pro_batch_create_dtls b where a.id='$batch_id' and a.entry_form in(0,36)  and a.id=b.mst_id  and b.status_active=1 $pop_batch    ";
 ...
 $select_listagg = "listagg(cast(A.PO_NUMBER as varchar(500)),',') within group (order by A.PO_NUMBER) as PO_NO,listagg(cast(A.FILE_NO as varchar(500)),',') within group (order by A.FILE_NO) as FILE_NO,listagg(cast(A.GROUPING as varchar(500)),',') within group (order by A.GROUPING) as REF_NO";
  $select_listagg_subcon="listagg(cast(a.order_no as varchar2(4000)),',') within group (order by a.order_no) as po_no";
		...
		$within_group=$sales_job_arr[$booking_no]["within_group"];
		if ($salesOrder == 1) {
			if($within_group == 1){
				  	$buyer_name=$company_arr[$sales_job_arr[$booking_no]["buyer_id"]];
				    $job_no=$sales_job_arr[$booking_no]["job_no_mst"];
					$sales_order_no=$sales_job_arr[$booking_no]["sales_order_no"];
			 }
			 else
			 {
				 $buyer_name=$buyer_arr[$sales_job_arr[$booking_no]["buyer_id"]];
				 $job_no='';
				 $sales_order_no=$sales_job_arr[$booking_no]["sales_order_no"];
			 }
  ...
  //  print_r($save_obj);die;
   ...
   //====Response Status End=
	...
	public function norsel_print_response($response_obj)
	{
		...
		if(!empty($log_data)){
			$this->insertData($log_data, 'AKHERP.PRO_GREY_PROD_NORSEL_LOG');
		}
	...
	public function finish_roll_rcv_save($issue_obj)
	{
		...
		foreach ($response_obj->resultset->DtlsPart as $dtls_row)
		{
			...
			if(!empty($recv_trans_data)){
				$this->insertData($recv_trans_data, 'AKHERP.INV_TRANSACTION');
			}

			if(!empty($recv_dtls_data)){
				$this->insertData($recv_dtls_data, 'AKHERP.PRO_FINISH_FABRIC_RCV_DTLS');
			}

			if(!empty($recv_roll_data)){
				$this->insertData($recv_roll_data, 'AKHERP.PRO_ROLL_DETAILS');
			}

			if(!empty($recv_prop_data)){
				$this->insertData($recv_prop_data, 'AKHERP.ORDER_WISE_PRO_DETAILS');
			}
	...
	public function finish_roll_issue_save($issue_obj)
	{
		...
		foreach ($response_obj->resultset->DtlsPart as $dtls_row)
		{
			...
			if(!empty($issue_trans_data)){
				$this->insertData($issue_trans_data, 'AKHERP.INV_TRANSACTION');
			}
			if(!empty($issue_dtls_data)){
				$this->insertData($issue_dtls_data, 'AKHERP.INV_FINISH_FABRIC_ISSUE_DTLS');
			}
			if(!empty($issue_roll_data)){
				$this->insertData($issue_roll_data, 'AKHERP.PRO_ROLL_DETAILS');
			}
			if(!empty($issue_prop_data)){
				$this->insertData($issue_prop_data, 'AKHERP.ORDER_WISE_PRO_DETAILS');
			}
	...
	public function fin_fab_barcode_result_entry_store($response_obj)
	{
		...
		if($mst_row)
		{
			...
			if(!empty($result_mst_data)){
				$this->insertData($result_mst_data, 'AKHERP.PRO_QC_RESULT_MST');
			}
			...
			foreach ($response_json->resultset->DtlsPart as $dtls_row)
			{
				...
				if(!empty($dtls_data)){
					$this->insertData($dtls_data, 'AKHERP.PRO_QC_RESULT_DTLS');
				}
			}
	...
	public function finish_fabric_production_store($plan_obj)
	{
		...
		foreach ($response_obj->resultset->DtlsPart as $dtls_row)
		{
			...
			$id_roll = return_next_id_by_sequence("PRO_ROLL_DTLS_PK_SEQ", "pro_roll_details");
			$id_prop = return_next_id_by_sequence("ORDER_WISE_PROP_PK_SEQ", "order_wise_pro_details");
			$id_prod = return_next_id_by_sequence("PRODUCT_DETAILS_MASTER_PK_SEQ", "product_details_master");
			...
			if(!empty($receive_dtls_data)){
				$this->insertData($receive_dtls_data, 'AKHERP.PRO_FINISH_FABRIC_RCV_DTLS');
			}
			if(!empty($receive_roll_data)){
				$this->insertData($receive_roll_data, 'AKHERP.PRO_ROLL_DETAILS');
			}
			if(!empty($receive_prop_data)){
				$this->insertData($receive_prop_data, 'AKHERP.ORDER_WISE_PRO_DETAILS');
			}
			if(!empty($prod_data)){
				$this->insertData($prod_data, 'AKHERP.PRODUCT_DETAILS_MASTER');
			}
		}
	...
	public function check_if_barcode_already_qc($barcode_no){
		...
		where c.id=a.mst_id and a.id=b.dtls_id and b.entry_form=66 and b.status_active=1 and b.is_deleted=0 and b.barcode_no='$barcode_no'";
	...
	public function print_emb_sp_insert_receive($qc_obj)
	{
		...
		if(!empty($delivery_mst)){
			$this->insertData($delivery_mst, 'AKHERP.PRO_GMTS_DELIVERY_MST');
		}
			...
			foreach ($orderData as $gmtsItemId => $gmtsItemIdData) {
				foreach ($gmtsItemIdData as $countryId => $qty) {
					...
					if(!empty($production_mst)){
						$this->insertData($production_mst, 'AKHERP.PRO_GARMENTS_PRODUCTION_MST');
					}
				}
			}
		}
		...
		foreach ($dtlsArr as $bundle_no => $qty) {
			...
			if(!empty($data_array_dtls)){
				$this->insertData($data_array_dtls, 'AKHERP.PRO_GARMENTS_PRODUCTION_DTLS');
			}
	...
	public function qc_bundle_insert($qc_obj)
	{
		...
		// die;
		/*if(!empty($qc_mst)){
			$this->insertData($qc_mst, 'AKHERP.PRO_GMTS_CUTTING_QC_MST');
		}*/
		...
		foreach ($dtls_part_data as $dtls_data)
		{
			...
			foreach ($bundle_data as $b_row) {
				...
				if(!empty($production_dtls)){
					$this->insertData($production_dtls, 'AKHERP.PRO_GARMENTS_PRODUCTION_DTLS');
				}
				...
				//print_r($qc_dtls);
				if(!empty($qc_dtls)){
					$this->insertData($qc_dtls, 'AKHERP.PRO_GMTS_CUTTING_QC_DTLS');
				}
				...
				if(trim($b_row->DEFECT_STR) !="")
				{
					...
					foreach ($defect_data as $def_row) {
						...
						//print_r($qc_dtls);
						if(!empty($qc_defect)){
							$this->insertData($qc_defect, 'AKHERP.PRO_GMTS_PROD_DFT');
						}
					}
				}
				...
				foreach($pos as $pid)
				{
					if( $duplicate_seq[$pid]=='')
					{
						...
						if(!empty($qc_sequence)){
							$this->insertData($qc_sequence, 'AKHERP.PRO_PRODUCTION_SEQUENCE');
						}
					}
				}
			}
				...
				if($dtlsAlterStrtArr[$bundle_no] !="")
				{
					...
					foreach ($defect_data as $def_row) {
						...
						if(!empty($alter_defect)){
							$this->insertData($alter_defect, 'AKHERP.PRO_GMTS_PROD_DFT');
						}
					}
				}
				if($dtlsSpotStrtArr[$bundle_no] !="")
				{
					...
					foreach ($defect_data as $def_row) {
						...
						if(!empty($spot_defect)){
							$this->insertData($spot_defect, 'AKHERP.PRO_GMTS_PROD_DFT');
						}
					}
				}
			}
	...
	public function home_data($company = 0, $location = 0, $floor = 0, $line = 0) 
	{
		...
		foreach ($po_item_wise_prod_qty_arr as $po_id => $po_data) 
		{
			foreach ($po_data as $item_id => $qty) 
			{
				...
				// echo $qty."*".$item_smv_array[$po_id][$item_id]."<br>";
			}
		}
		// echo "(".$produce_min." / (".$sql[0]->TARGET_PER_HOUR."*".$diff_hour."))*100";
		...
		$data_array['home_data']['efficiency'] = ($day_total_qty / ($sql[0]->TARGET_PER_HOUR*$diff_hour))*100;
		$data_array['home_data']['efficiency_min'] = ($produce_min / ($sql[0]->MAN_POWER*$diff_hour*60))*100;
	...
	public function barcode_details_data($bar_code, $type = 0) {
		...
		if ($type == 2) {
			...
			$sqls = "SELECT  c.REJECT_QNTY,a.COMPANY_ID,a.BUYER_ID,a.ROLL_MAINTAINED, c.BARCODE_NO,c.MST_ID,c.DTLS_ID ,  b.PROD_ID, b.BODY_PART_ID, b.FEBRIC_DESCRIPTION_ID, b.MACHINE_NO_ID, b.GSM, b.WIDTH, b.COLOR_ID,b.YARN_PROD_ID, b.YARN_PROD_ID,b.YARN_LOT, b.YARN_COUNT, c.id as ROLL_ID,c.ROLL_NO,c.QNTY, e.SYS_NUMBER
			FROM INV_RECEIVE_MASTER a, pro_grey_prod_entry_dtls b, pro_roll_details c left join pro_roll_details d on c.barcode_no=d.barcode_no and d.entry_form=56 and d.status_active=1 and d.is_deleted=0 left join pro_grey_prod_delivery_mst e on d.mst_id=e.id and e.entry_form=56 and e.status_active=1
	...
	function tabwise_sewingline_style_item_data($company_id, $location_id, $floor_id, $line_id, $job_no="", $po_number="", $internal_ref="", $style_ref="") {
		...
		where a.id=b.mst_id and a.production_type in(4,5) and a.SERVING_COMPANY=$company_id and a.LOCATION=$location_id and a.FLOOR_ID=$floor_id and SEWING_LINE=$line_id
		...
		{
			$sewing_output_arr[$row->PO_BREAK_DOWN_ID][$row->COUNTRY_ID][$row->ITEM_NUMBER_ID]+=$row->OUTPUT_QNTY;
			$sewing_input_arr[$row->PO_BREAK_DOWN_ID][$row->COUNTRY_ID][$row->ITEM_NUMBER_ID]+=$row->INPUT_QNTY;
		}

		$sqls = sql_select("SELECT a.ID as JOB_ID,A.JOB_NO, A.STYLE_REF_NO,B.GROUPING, null as PO_ID B.PO_NUMBER, C.ITEM_NUMBER_ID, E.ITEM_NAME, C.COUNTRY_ID, F.COUNTRY_NAME
		...
		GROUP BY a.ID,A.JOB_NO, A.STYLE_REF_NO,B.GROUPING, B.PO_NUMBER, C.ITEM_NUMBER_ID, C.COUNTRY_ID, E.ITEM_NAME, C.COUNTRY_ID, F.COUNTRY_NAME");//and d.production_date='$production_date'
		//, B.ID AS PO_ID
		//, B.ID
		$i = 0;
		foreach ($sqls as $v) {
			$input_qnty = $sewing_input_arr[$v->PO_ID][$v->COUNTRY_ID][$v->ITEM_NUMBER_ID];
			$output_qnty = $sewing_output_arr[$v->PO_ID][$v->COUNTRY_ID][$v->ITEM_NUMBER_ID];
			...
			if($output_qnty < $input_qnty)
			{
				...
				$data_array[$i]["PO_NUMBER"] = $v->PO_NUMBER;
				//$data_array[$i]["PO_ID"] = $v->PO_ID;
 ...
 ////====End=======////=====
		...
		if ($response_obj->status == true) 
		{
			if ($response_obj->mode == "save") 
			{
				...
				else 
				{
					...
					$pc_date_time = date("d-M-Y h:i:s A", time());
					$production_date = date("d-M-Y");
					$txt_reporting_hour = str_replace("'", "", $production_date) . " " . date('H');
					$txt_reporting_hour = "to_date('$txt_reporting_hour','DD MONTH YYYY HH24:MI:SS')";
				...
				foreach ($receive_result as $row) 
				{
					...
					{
						$prod_data_array[$row->COLOR_SIZE_BREAK_DOWN_ID][$row->PRODUCTION_TYPE]["in"] += $row->PRODUCTION_QNTY+$row->REPLACE_QTY;
					}

					if($row->PRODUCTION_TYPE == 5 && $row->COLOR_SIZE_BREAK_DOWN_ID == $color_size_id)
					{
						$prod_data_array[$row->COLOR_SIZE_BREAK_DOWN_ID][$row->PRODUCTION_TYPE]["out"] += $row->PRODUCTION_QNTY+$row->REJECT_QTY+$row->ALTER_QTY+$row->SPOT_QTY;
					}
					...
					if($sewing_production_date == strtotime($current_date) && ($row->PRODUCTION_TYPE == 5) && ($row->INSERTED_BY == $user_id))
					{
						if($row->COLOR_SIZE_BREAK_DOWN_ID == $color_size_id)
						{
							...
							$prev_qty_array[$row->PRODUCTION_TYPE]['rectified'] += $row->REPLACE_QTY;
						}
						$color_wise_production_array[$row->COLOR_NUMBER_ID] += $row->PRODUCTION_QNTY+$row->REJECT_QTY+$row->ALTER_QTY+$row->SPOT_QTY;
					}
				}

				if(($prod_data_array[$color_size_id][5]["out"]+1) > $prod_data_array[$color_size_id][4]["in"])
				{
				...
				foreach ($dtls_data as $val) 
				{
					...
					$mst_part_data = array(
						...
						'PRODUCTION_DATE' => $production_date,
						'PRODUCTION_TYPE' => $production_types,
						'PRODUCTION_QUANTITY' => ($val->good==1)?$val->qnty:0,
					...
					$dtls_part_data = array(
						...
						'REPLACE_QTY' => ($val->rectified==1)?$val->qnty:0,
						'ENTRY_FORM' => $entry_forms,
					...
					$dtls_tbl_id = $this->insertData($dtls_part_data, "PRO_GARMENTS_PRODUCTION_DTLS");
					if ($dtls_tbl_id && $rectified_from_dtls_id!="") {
						$this->db->query("update PRO_GARMENTS_PRODUCTION_DTLS set IS_RECTIFIED=1 where id=$rectified_from_dtls_id");
					}
					...
					if ($this->db->trans_status() == TRUE) 
					{
						if ($mst_tbl_id && $dtls_tbl_id) 
						{
							...
							$resultset["total_out_qnty"] = ($prod_data_array[$colorSizeId][5]["out"]+1)*1;
							$resultset["output_qnty"] = ($prod_data_output_array[$colorSizeId][5]+1)*1;
							$resultset["color_wise_chk_qnty"] = ($color_wise_production_array[$color_id]+1)*1;
							$resultset["ok"] = (($val->good==1)?($prev_qty_array[5]['ok']+1):$prev_qty_array[5]['ok']) *1;
							$resultset["rej"] = (($val->reject==1)?($prev_qty_array[5]['rej']+1):$prev_qty_array[5]['rej']) *1;
							$resultset["alter"] = (($val->alter==1)?($prev_qty_array[5]['alter']+1):$prev_qty_array[5]['alter']) *1;
							$resultset["spot"] = (($val->spot==1)?($prev_qty_array[5]['spot']+1):$prev_qty_array[5]['spot']) *1;
							$resultset["rectified"] = (($val->rectified==1)?($prev_qty_array[5]['rectified']+1):$prev_qty_array[5]['rectified']) *1;
							return $resultset;
						} 
						else 
						{
			...
			elseif ($response_obj->mode == "delete") 
			{
				...
				if($update_id>0)
				{
					$mst_status = $this->db->query("update PRO_GARMENTS_PRODUCTION_MST set status_active=0,is_deleted=1 where id ='$update_id'");
					$dtls_status = $this->db->query("update PRO_GARMENTS_PRODUCTION_DTLS set status_active=0,is_deleted=1 where mst_id ='$update_id'");
					$dft_status = $this->db->query("update PRO_GMTS_PROD_DFT set status_active=0,is_deleted=1 where mst_id ='$update_id'");
					...
					if ($this->db->trans_status() == TRUE) 
					{
						if ($mst_status && $dtls_status) 
						{
							...
							$current_date = date("d-M-Y");
							$prev_qty_sql = "SELECT B.COLOR_SIZE_BREAK_DOWN_ID,B.PRODUCTION_QNTY,B.REJECT_QTY,B.PRODUCTION_TYPE,B.REPLACE_QTY,B.REJECT_QTY, B.ALTER_QTY, B.SPOT_QTY,C.COLOR_NUMBER_ID,a.PRODUCTION_DATE  from pro_garments_production_mst a, pro_garments_production_dtls b,wo_po_color_size_breakdown c where a.id=b.mst_id and b.color_size_break_down_id=c.id and b.status_active=1 and b.is_deleted=0 and b.production_type in(4,5) and c.status_active=1 and a.sewing_line=$sewing_line and a.po_break_down_id = $order_id AND a.item_number_id = $item_id AND a.country_id = $country_id";// and b.color_size_break_down_id=$color_size_id
							...
							foreach ($receive_result as $row) 
							{
								if($row->PRODUCTION_TYPE==4)
								{
									$total_input_out_qnty_arr[$row->COLOR_SIZE_BREAK_DOWN_ID][$row->PRODUCTION_TYPE]["in"] += $row->PRODUCTION_QNTY+$row->REPLACE_QTY;
								}
							...
							$resultset["input_qnty"] = $total_input_out_qnty_arr[$color_size_id][4]*1;
							$resultset["total_out_qnty"] = $total_input_out_qnty_arr[$color_size_id][5]*1;
	...
	public function knitting_production_data($job_id) {
		...
		$knitting_pdo=$row->KNITTING_PDO;
			...
			if($row->JOB_NO && $row->PROGRAM_NO && $row->RECEIVE_DATE && $row->PROD_ID && $row->PRODUCT_NAME_DETAILS && $row->MACHINE_NO && $row->QUANTITY && $row->UOM && $row->FEBRIC_DESCRIPTION_ID && $row->GSM && $row->WIDTH && $color_names)
			{
	...
	public function dyeing_production_data($job_id) {
		...
		group by g.id, a.batch_no, f.batch_id, a.extention_no, g.batch_qty, g.production_qty, c.job_no_mst, f.production_date, f.process_end_date, f.end_hours, f.end_minutes, f.machine_id, f.entry_form, f.result,f.process_id ,f.load_unload_id, a.double_dyeing
		order by f.batch_id, f.process_id,f.load_unload_id");
	  ...
	  $dyeing_result = array(1 => 'Shade Matched', 2 => 'Re-Dyeing Needed', 3 => 'Fabric Damaged', 4 => 'Incomplete/Running', 5 => 'Under Trial', 6 => 'Re-Wash Needed',11 => 'Complete',12 => 'Next process Stentering',13 => 'Next process Dryer',14 => 'Next process Compacting',15 => 'Next process Brush',16 => 'Next process Peach',17 => 'Waiting for Fastness',18 => 'Waiting for Shrinkage',19 => 'Waiting for Decision',100 => 'Others');
		...
		foreach ($other_process_sql as $row) 
		{
			 if($dye_dupli_chk[$row->PROCESS_DTLS_ID] =="")
			 {
				  ...
				  $dye_string = $row->PROCESS_ID."*".$row->JOB_NO."*".$row->BATCH_ID."*".$row->BATCH_NO."*".$row->EXTENTION_NO."*".$row->DOUBLE_DYEING;
	...
	public function bundle_wise_sewing_barcode_data( $barcode_no )
	{
		$data_arr = array();
		...
		$input_sql = "SELECT BARCODE_NO,BUNDLE_NO from PRO_GARMENTS_PRODUCTION_OPERATION_TRACKING where status_active=1 and barcode_no='" . trim($barcode_no) . "'";
		$input_exist_data = sql_select($input_sql);
		if (count($input_exist_data) > 0) 
		{
			return array(
	...
	public function bundle_wise_sewing_barcode($input_data)
	{
		...
		$pc_date_time = date("d-M-Y h:i:s A",time());
		$user_id 	  = 1;
		//$mst_id = return_next_id_by_sequence(  "PRO_GARMENTS_PRODUCTION_OPERATION_TRACKING_SEQ", "PRO_GARMENTS_PRODUCTION_OPERATION_TRACKING" );
		$mst_id = return_next_id("id", "PRO_GARMENTS_PRODUCTION_OPERATION_TRACKING", "", "", $db_type);
		...
		{
			$flag = $this->insertData($operation_tracking, 'PRO_GARMENTS_PRODUCTION_OPERATION_TRACKING');
		}
	...
	$variable_inventory_arr = return_library_array("SELECT allocation, company_name from variable_settings_inventory where variable_list=40 and status_active=1 and is_deleted=0", "company_name", "allocation");
		...
		FROM inv_receive_master a, pro_grey_prod_entry_dtls b, pro_roll_details c
		WHERE a.id=b.mst_id and b.id=c.dtls_id and a.entry_form=2 and c.entry_form=2 and c.status_active=1 and c.is_deleted=0 and c.barcode_no=$barcode_no";
		$production_data = $this->db->query($production_sql)->result();$barcode_no_info=array();
		foreach($production_data as $row)
		{
			if($row->IS_SALES !=0)
			{
		...
		foreach($receive_data as $row)
		{
			...
			$roll_info["DtlsPart"]['ROLL_ID'] = $row->ROLL_ID;
			...
			$roll_info["DtlsPart"]['QNTY'] = number_format($row->QNTY, 2, '.', '');
	...
	function style_wise_operation_list_model($JOB_NO_MST,$PO_BREAK_DOWN_ID,$ITEM_NUMBER_ID,$COUNTRY_ID,$LINE_ID,$STYLE_NUMBER,$USER_ID)
	{
		//Color Query start
		...
		$lib_operation_arr=array();
		$query_LIB_SEWING_OPERATION_ENTRY = "SELECT ID,OPERATION_NAME FROM LIB_SEWING_OPERATION_ENTRY where STATUS_ACTIVE = 1 AND IS_DELETED = 0 AND GMT_ITEM_ID=$ITEM_NUMBER_ID AND IS_QC=1";
		$table_LIB_SEWING_OPERATION_ENTRY = $this->db->query($query_LIB_SEWING_OPERATION_ENTRY)->result();
		foreach($table_LIB_SEWING_OPERATION_ENTRY as $row){
			$lib_operation[] = ['ID'=>$row->ID,'NAME'=>$row->OPERATION_NAME];
		}
		//SEWING_OPERATION Query end
		$current_date = date("d-M-Y");
		$prod_sql = "select B.PRODUCTION_QNTY,B.REPLACE_QTY,B.REJECT_QTY, B.ALTER_QTY, B.SPOT_QTY, B.COLOR_SIZE_BREAK_DOWN_ID AS COL_SIZE_ID,B.PRODUCTION_TYPE,C.COLOR_NUMBER_ID,a.PRODUCTION_DATE,a.INSERTED_BY from pro_garments_production_mst a, pro_garments_production_dtls b,wo_po_color_size_breakdown c
		where a.id=b.mst_id and b.color_size_break_down_id=c.id and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and b.production_type in(4,5) and c.status_active=1 and a.po_break_down_id = $PO_BREAK_DOWN_ID AND a.item_number_id = $ITEM_NUMBER_ID AND a.country_id = $COUNTRY_ID and a.sewing_line=$LINE_ID";// and a.PRODUCTION_DATE='$current_date'
		...
		foreach($prod_res as $v)
		{
			...
			{
				$prod_data_array[$v->COL_SIZE_ID][$v->PRODUCTION_TYPE] += $v->PRODUCTION_QNTY+$v->REPLACE_QTY;
			}
			...
			if($production_date == strtotime($current_date) && $v->PRODUCTION_TYPE == 5 && $v->INSERTED_BY == $USER_ID)
			{
				...
				$event_wise_prod_data_array[$v->COL_SIZE_ID]["rectified"] += $v->REPLACE_QTY;
				...
				$color_wise_production_array[$v->COLOR_NUMBER_ID]["rectified"] += $v->REPLACE_QTY;
			}
		}
		// ===================================

		$wo_po_color_size_breakdown_query = "select a.ID,a.JOB_NO_MST,a.PO_BREAK_DOWN_ID,a.COLOR_NUMBER_ID,a.SIZE_NUMBER_ID from wo_po_color_size_breakdown a WHERE a.item_number_id = $ITEM_NUMBER_ID AND a.country_id = $COUNTRY_ID AND a.JOB_NO_MST ='$JOB_NO_MST' AND a.status_active = 1 AND a.is_deleted = 0 ORDER BY a.size_order ASC";//a.po_break_down_id = $PO_BREAK_DOWN_ID AND 
		...
		foreach($wo_po_color_size_breakdown_table as $row){
			...
			$responseArr['po_break_down_id'] = $row->PO_BREAK_DOWN_ID;

			$total_input_qnty += $prod_data_array[$row->ID][4]*1;
			$total_output_qnty += $prod_data_output_array[$row->ID][5]*1;
			
			$datarArr['color'][$row->COLOR_NUMBER_ID]=[
				'colour_id' => $row->COLOR_NUMBER_ID,
				'colour_name' =>$lib_color [$row->COLOR_NUMBER_ID]["COLOR_NAME"],
				'color_output_qnty' => ($color_wise_production_array[$row->COLOR_NUMBER_ID]["good"]+$color_wise_production_array[$row->COLOR_NUMBER_ID]["reject"]+$color_wise_production_array[$row->COLOR_NUMBER_ID]["alter"]+$color_wise_production_array[$row->COLOR_NUMBER_ID]["spot"])*1,
			];
		...
		$responseArr['total_input_qnty'] += $total_input_qnty*1;
		$responseArr['total_output_qnty'] += $total_output_qnty*1;

		$colorArr=array();
		foreach($datarArr['color'] as $rows){
			$colorArr[]=$rows;
		}
		$responseArr['color'] = $colorArr;

		$sizeArr=array();
		foreach($datarArr['size'] as $rows){
			$sizeArr[]=$rows;
		}
	...
	function style_wise_operation_list_model_backup($JOB_NO_MST,$PO_BREAK_DOWN_ID,$ITEM_NUMBER_ID,$COUNTRY_ID,$LINE_ID,$STYLE_NUMBER,$USER_ID)
	{
		...
		$table_LIB_SEWING_OPERATION_ENTRY = $this->db->query($query_LIB_SEWING_OPERATION_ENTRY)->result();
		foreach($table_LIB_SEWING_OPERATION_ENTRY as $row){
			//$lib_operation_arr[$row->ID] =$row->OPERATION_NAME;
			$lib_operation[] = ['ID'=>$row->ID,'NAME'=>$row->OPERATION_NAME];
		}

		//print_r($lib_operation_arr);die;

		/*$query_LIB_SEWING_OPERATION_ENTRY = "select b.LIB_SEWING_ID FROM PPL_GSD_ENTRY_MST a, PPL_GSD_ENTRY_dtls b where a.id=b.mst_id and a.STATUS_ACTIVE = 1 AND  a.IS_DELETED = 0 and b.STATUS_ACTIVE = 1 AND  b.IS_DELETED = 0 AND  a.GMTS_ITEM_ID  = $ITEM_NUMBER_ID and a.STYLE_REF = '$STYLE_NUMBER' and a.BULLETIN_TYPE=4 group by b.LIB_SEWING_ID";
		
		//echo $query_LIB_SEWING_OPERATION_ENTRY;die;
		$table_LIB_SEWING_OPERATION_ENTRY = $this->db->query($query_LIB_SEWING_OPERATION_ENTRY)->result();
		foreach($table_LIB_SEWING_OPERATION_ENTRY as $row){
			$lib_operation[] = ['ID'=>$row->LIB_SEWING_ID,'NAME'=>$lib_operation_arr[$row->LIB_SEWING_ID]];
		}*/
		//asort($lib_operation);
		//SEWING_OPERATION Query end
		$current_date = date("d-M-Y");
		...
		$wo_po_color_size_breakdown_query = "select a.ID,a.JOB_NO_MST,a.PO_BREAK_DOWN_ID,a.COLOR_NUMBER_ID,a.SIZE_NUMBER_ID from wo_po_color_size_breakdown a WHERE a.po_break_down_id = $PO_BREAK_DOWN_ID AND a.item_number_id = $ITEM_NUMBER_ID AND a.country_id = $COUNTRY_ID AND a.JOB_NO_MST ='$JOB_NO_MST' AND a.status_active = 1 AND a.is_deleted = 0 ORDER BY a.size_order ASC";
	...
	function active_qc_session_list()
	{
		...
		$config_data = $this->db->query("SELECT a.ID,F.FLOOR_NAME, d.LINE_NUMBER, A.DEVICE_ID,B.ITEM_NAME, A.PO_NUMBER,A.JOB_NO,A.INTERNAL_REF
			from tabwise_sewing_line a,lib_garment_item b, prod_resource_mst d,lib_prod_floor f
			where a.status_active=1 and a.item_number_id=b.id and a.sewing_line=d.id and a.floor_id=f.id")->result();
		...
		foreach ($receive_result as $row) 
		{
			...
			if($production_date == strtotime($current_date) && $row->PRODUCTION_TYPE == 5 && $row->INSERTED_BY == $USER_ID)
			{
				if($row->COLOR_SIZE_BREAK_DOWN_ID == $COLOR_SIZE_ID)
				{
					...
					$event_wise_prod_data_array["rectified"] += $row->REPLACE_QTY;
				}
			}
			$i++;
		}
	...
	{
		$current_date = date("d-M-Y");
		ECHO $prev_qty_sql = "SELECT B.COLOR_SIZE_BREAK_DOWN_ID,B.PRODUCTION_QNTY,B.REJECT_QTY,B.PRODUCTION_TYPE,B.REPLACE_QTY,B.REJECT_QTY, B.ALTER_QTY, B.SPOT_QTY,C.COLOR_NUMBER_ID,c.SIZE_NUMBER_ID,a.PRODUCTION_DATE,a.INSERTED_BY from pro_garments_production_mst a, pro_garments_production_dtls b,wo_po_color_size_breakdown c where a.id=b.mst_id and b.color_size_break_down_id=c.id and b.status_active=1 and b.is_deleted=0 and b.production_type in(4,5) and c.status_active=1 and a.sewing_line=$sewing_line and a.po_break_down_id = $PO_BREAK_DOWN_ID AND a.item_number_id = $ITEM_NUMBER_ID AND C.COLOR_NUMBER_ID=$COLOR_ID AND a.country_id = $COUNTRY_ID";// and b.color_size_break_down_id=$color_size_id
	...
	function defected_details_list_to_rectify($color_size_id, $sewing_line)
	{
		$sew_fin_reject_type_for_arr = array(1=>"Fabric", 2=>"Sewing", 3=>"Measurement", 4=>"Spot", 5=>"Shade", 6=>"Hole", 7=>"Cutting", 8=>"Wash", 9=>"Print", 10=>"Twisting", 11=>"Conta",12 => "Color Spot",13 => "Crease Mark",14 => "Dirty Spot",15 => "Distinguish",16 => "Dusted",17 => "Dyeline",18 => "Emb Rejection ",19 => "Embroidery",20 => "Fabric (Z) Hole",21 => "HTS Problem Cutting",22 => "HTS Problem Finishing",23 => "Iron Spot",24 => "Knot",25 => "M/C Knife  Cut",26 => "Measurement (+-)",27 => "Needle Cut",28 => "Oil Spot",29 => "Part Mistake",30 => "Part Shade",31 => "Patta",32 => "Pleat",33 => "Print Reject",34 => "Runing Shade",35 => "Scissor Cut",36 => "Slub",37 => "Softner Mark",38 => "Tag Gun Rej",39 => "Twist",40 => "Uneven Dyeing",41 => "Yarn Missing",42 => "Yarn Contamination",43=>"DIRTY MARK",44=>"FAB FAULT",45=>"SEW IN COMPLETE",46=>"SHADING",47=>"REP DAMAGE",48=>"OTHERS-1",49=>"OTHERS-2",100=>"Others");
		...
		$defect_dtls_sql = "SELECT B.MST_ID,a.PRODUCTION_DATE,B.REPLACE_QTY,B.REJECT_QTY, B.ALTER_QTY, B.SPOT_QTY, B.COLOR_SIZE_BREAK_DOWN_ID AS COL_SIZE_ID,B.ID, b.OPERATION_ID, c.OPERATION_NAME  
		from pro_garments_production_mst a, pro_garments_production_dtls b 
		left join lib_sewing_operation_entry c on b.operation_id=c.id
		where a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and b.production_type=5 and b.COLOR_SIZE_BREAK_DOWN_ID = $color_size_id and a.status_active=1 
		and b.status_active=1 and B.PRODUCTION_QNTY=0 and B.REJECT_QTY=0 and B.REPLACE_QTY=0 and (B.ALTER_QTY>0 or B.SPOT_QTY>0) and (b.IS_RECTIFIED is null or b.IS_RECTIFIED=0) and a.sewing_line=$sewing_line";
		$defect_dtls_data = $this->db->query($defect_dtls_sql)->result();

		foreach($defect_dtls_data as $v)
		{
			$dtls_id_arr[$v->ID] = $v->ID;
			$color_size_id_arr[$v->COL_SIZE_ID] = $v->COL_SIZE_ID;
		}
		$dtls_ids = implode(",", $dtls_id_arr);
		$defect_info_sql = "SELECT DTLS_ID,DEFECT_TYPE_ID,DEFECT_POINT_ID,DEFECT_QTY from PRO_GMTS_PROD_DFT WHERE dtls_id in($dtls_ids)";
		...
		foreach($defect_dtls_data as $v)
		{
			...
			$responseArr[$i]["OPERATION_NAME"] = $v->OPERATION_NAME;
