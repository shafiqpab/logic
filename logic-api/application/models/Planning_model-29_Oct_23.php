...
class Planning_model extends CI_Model
{
	...
	function create_plan($plan_obj)
	{
		...
		if ($is_delete != 'delete' && count($temp_plan_id_arr) != 0) {
			...
			$plan_sql = "SELECT a.PO_COMPANY_ID,a.BALANCE_QTY,a.LINE_ID,b.PLAN_ID,b.PO_BREAK_DOWN_ID,b.COLOR_NUMBER_ID 
			FROM PPL_SEWING_PLAN_BOARD a, PPL_SEWING_PLAN_BOARD_POWISE b where a.id=b.plan_id and b.PLAN_ID in(" . implode(',', $temp_plan_id_arr) . ")";
			// echo $plan_sql;die;
		...
		if ($response_obj->Status == 1) {
			...
			foreach ($response_obj->SewingPlanBoard as $sewing_plan_row) {
				...
				$ppl_sewing_plan_board_data = array(
					...
					'PUB_SHIP_DATES_H' => $sewing_plan_row->PUB_SHIP_DATES_H,
			...
			foreach ($response_obj->SewingPlanBoardPOWise as $sewing_plan_po_row) {
				...
				if ($sewing_plan_po_row->RowState != "delete" && $sewing_plan_po_row->RowState  && $plan_wise_mage_type_arr[$planning_id] != 'Merged') {
					...
					//echo ($plan_qty_arr[$planning_id].','.$plan_po_qty_arr[$planning_id].','.$po_qty_arr[$sewing_plan_po_row->PO_BREAK_DOWN_ID * 1][$sewing_plan_po_row->COLOR_NUMBER_ID * 1]); die;
					// if($sewing_plan_po_row->PLAN_QNTY < 1){
					// 	$this->db->trans_rollback();
					// 	return $resultset["status"] = "Failed (Quantity < 1)";
					// }
	...
	function po_tracking($po_id = "",$order_type)
	{
		...
		if($order_type==1){
			...
			WHERE a.ID = b.MST_ID AND b.MST_ID = c.MST_ID and b.id = c.ORDER_ID AND a.ENTRY_FORM = 238 AND b.MAIN_PROCESS_ID ='5'
	...
	function powise_backup($plan_id_from, $plan_id_to)
	{
		...
		$query_sewing_plan_board_powise = "SELECT PLAN_ID,ITEM_NUMBER_ID,COLOR_NUMBER_ID,PO_BREAK_DOWN_ID,JOB_NO,PLAN_QNTY FROM PPL_SEWING_PLAN_BOARD_POWISE where PLAN_ID BETWEEN $plan_id_from AND $plan_id_to";
		$table_sewing_plan_board_powise = $this->db->query($query_sewing_plan_board_powise)->result();
