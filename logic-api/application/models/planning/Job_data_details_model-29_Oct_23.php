...
class Job_data_details_model extends CI_Model
{
	...
	{   //print_r(5);die; 
		...
		if($order_type == 0 || $order_type == 2){
			...
			$query_subcon_production = "SELECT a.ORDER_ID, a.GMTS_ITEM_ID , b.PROD_QNTY,  c.COLOR_ID, c.QNTY, d.DELIVERY_DATE,d.ID as PO_BREAKDOWN_ID 
			FROM subcon_gmts_prod_dtls a,SUBCON_GMTS_PROD_COL_SZ b, subcon_ord_breakdown c, SUBCON_ORD_DTLS d 
			WHERE d.id = c.ORDER_ID and a.ORDER_ID = d.ID AND  b.ORD_COLOR_SIZE_ID = c.ID AND b.DTLS_ID = a.id
			AND a.status_active = 1 AND a.is_deleted = 0 and a.PRODUCTION_TYPE=2 AND b.PRODUCTION_TYPE = 2 $subcon_com_con $subcon_deli_date_2";
		...
		$sql = "SELECT b.PLAN_ID,b.PLAN_QNTY,b.PO_BREAK_DOWN_ID,b.ITEM_NUMBER_ID,b.COLOR_NUMBER_ID,a.ORDER_TYPE
