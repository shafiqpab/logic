...
class Plan_info_model extends CI_Model
{
	...
	function plan_info($company_id, $location_id, $floor_id, $txt_date_from, $user_id)
	{
		...
		//test
				// $time = time();
				// $date_time = date("j/n/Y g:i:s A",$time);
				// $temp_table = "TEMP-Table-$user_id-$time";
				//$this->db->query("INSERT INTO PPL_BOARD_TEMP_TABLE_NAME (USER_ID, DATE_TIME, TABLE_NAME) VALUES ($user_id, TO_DATE('$date_time', 'DD/MM/YYYY HH:MI:SS AM'), '$temp_table')");
				//echo($temp_table);die;
				//print_r($user_id);die;
				//
		...
		$query_LIB_SEWING_LINE = "SELECT ID,USER_IDS,LOCATION_NAME,FLOOR_NAME,LINE_NAME from LIB_SEWING_LINE where STATUS_ACTIVE=1 and IS_DELETED=0 $comCon3 $loc_cond $floor_cond_line";
		...
		if ($user_arr_type[$user_id] == 1) {
			...
			$sql_line = "SELECT a.BOARD_STATUS,a.USER_ID ,b.IS_PLANNER from PLANNING_BOARD_STATUS a,USER_PASSWD b  where b.id=a.user_id and b.IS_PLANNER=1 $comCon $loc_cond_lock $floor_cond_line_sts";
		...
		// $table_pro_variable = $this->db->query("SELECT WORK_STUDY_INTEGRATED,BULLETIN_TYPE FROM variable_settings_production WHERE company_name=$company_id and variable_list in (9,12)")->result();
		//print_r($pro_varibale[0]->WORK_STUDY_INTEGRATED);die;
		// $pro_variable_arr = array();
		// foreach ($table_pro_variable as $row) {
		// 	if ($row->WORK_STUDY_INTEGRATED) {
		// 		$pro_variable_arr[$row->VARIABLE_LIST] = $row->WORK_STUDY_INTEGRATED;
		// 	} elseif ($row->BULLETIN_TYPE) {
		// 		$pro_variable_arr[$row->VARIABLE_LIST] = $row->BULLETIN_TYPE;
		// 	}
		// }
		if ($is_integrated == 1) {
		...
		if (count($allPlanIdArr)) {
			...
			// $this->db->query("INSERT INTO PPL_BOARD_TEMP_ENGINE (ID, REF_VAL, REF_FROM,REF_STRING,ENTRY_FORM,USER_ID) VALUES (1,1,1,1,1,1)");
			// $tttt = $this->db->query("SELECT * FROM PPL_BOARD_TEMP_ENGINE")->result();
			//print_r($tttt);die;
			//$this->db->insert_batch("GBL_TEMP_ENGINE", $insert_plan_temp_data);
		}
		//print_r(5);die;
		...
		$com_res = $this->db->query("SELECT ID,COMPANY_NAME from lib_company where status_active =1 and is_deleted=0 and CORE_BUSINESS=1")->result();
		foreach ($com_res as $value) {
			$comp[$value->ID] = $value->COMPANY_NAME;
		}

		$location_res = $this->db->query("SELECT ID,LOCATION_NAME from lib_location where status_active =1 and is_deleted=0")->result();
		foreach ($location_res as $value) {
			$location_arr[$value->ID] = $value->LOCATION_NAME;
		}

		$garment_res = $this->db->query("SELECT ID,ITEM_NAME from  lib_garment_item where status_active=1 and is_deleted=0")->result();
		foreach ($garment_res as $value) {
			$garments_item[$value->ID] = $value->ITEM_NAME;
		}

		$buyer_res = $this->db->query("SELECT ID,BUYER_NAME from  lib_buyer where status_active=1 and is_deleted=0")->result();
		...
		if (count($po_arr)) {
			$empty_arr = array();
			//$this->db->insert_batch("GBL_TEMP_ENGINE", $insert_plan_temp_data);
			fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 5001, 56, $po_arr, $empty_arr);
			//print_r($insert_plan_temp_data);die;
		}
		//die;
		//print_r($query_powise);die;
		$query_booking = "SELECT b.PO_BREAK_DOWN_ID, a.ITEM_CATEGORY, b.GMTS_COLOR_ID, b.FIN_FAB_QNTY AS QUANTITY, a.FABRIC_SOURCE, b.COLOR_SIZE_TABLE_ID, b.FABRIC_COLOR_ID, a.DELIVERY_DATE, b.COLOR_TYPE,b.WO_QNTY,b.TRIM_GROUP FROM GBL_TEMP_ENGINE gbl, WO_BOOKING_MST a, WO_BOOKING_DTLS b WHERE a.ID = b.BOOKING_MST_ID AND a.COMPANY_ID = $company_id AND a.IS_DELETED = 0 AND a.STATUS_ACTIVE = 1 AND b.IS_DELETED = 0 AND b.STATUS_ACTIVE = 1 and  gbl.entry_form = 5001 and gbl.ref_from = 56 and gbl.user_id = $user_id  and gbl.ref_val=b.PO_BREAK_DOWN_ID";
		...
		$query_production_quantity = "SELECT d.PO_BREAKDOWN_ID,c.ITEM_CATEGORY,a.COLOR_ID,a.RECEIVE_QNTY,c.RECEIVE_BASIS,c.TRANSACTION_TYPE FROM PRO_FINISH_FABRIC_RCV_DTLS a, PRO_BATCH_CREATE_MST b, INV_TRANSACTION c,ORDER_WISE_PRO_DETAILS d,PRODUCT_DETAILS_MASTER e, gbl_temp_engine gbl WHERE a.BATCH_ID = b.ID AND a.TRANS_ID = c.ID AND d.TRANS_ID = c.ID AND e.ID = d.PROD_ID AND a.IS_DELETED = 0 AND a.STATUS_ACTIVE = 1 AND b.IS_DELETED = 0 AND b.STATUS_ACTIVE = 1 AND c.IS_DELETED = 0 AND c.STATUS_ACTIVE = 1 AND d.IS_DELETED = 0 AND d.STATUS_ACTIVE = 1 AND e.IS_DELETED = 0 AND e.STATUS_ACTIVE = 1 and d.PO_BREAKDOWN_ID = gbl.ref_val and gbl.REF_FROM=56 and gbl.entry_form=5001 and gbl.user_id=$user_id";
			...
			if ($row->ITEM_CATEGORY == 2) {
				...
				if ($lib_production_quantity[$row->PO_BREAK_DOWN_ID][$row->ITEM_CATEGORY][$row->GMTS_COLOR_ID] > 0) {
					if ($lib_booking_quantity[$row->PO_BREAK_DOWN_ID][$row->ITEM_CATEGORY][$row->GMTS_COLOR_ID] > $lib_production_quantity[$row->PO_BREAK_DOWN_ID][$row->ITEM_CATEGORY][$row->GMTS_COLOR_ID]) {
						if ($inv_status[$row->PO_BREAK_DOWN_ID] == null || $inv_status[$row->PO_BREAK_DOWN_ID] == 0 || $inv_status[$row->PO_BREAK_DOWN_ID] == 2) {
						...
						if ($inv_status[$row->PO_BREAK_DOWN_ID] == null || $inv_status[$row->PO_BREAK_DOWN_ID] != 0 || $inv_status[$row->PO_BREAK_DOWN_ID] != 1) {
					...
					}else if($inv_status[$row->PO_BREAK_DOWN_ID] ==2){
						$inv_status[$row->PO_BREAK_DOWN_ID] = 1;
					}
					// if($row->PO_BREAK_DOWN_ID == 84890){
		...
		$images_sql = "SELECT a.ID,b.IMAGE_LOCATION from WO_PO_BREAK_DOWN a, COMMON_PHOTO_LIBRARY b, gbl_temp_engine gbl where a.job_no_mst=b.MASTER_TBLE_ID  and a.is_deleted=0 and a.id= gbl.ref_val and gbl.REF_FROM=56 and gbl.entry_form=5001 and gbl.user_id=$user_id and b.file_type=1";
		...
		if (!empty($line_id_arr)) {
			...
			$production_sql = "SELECT a.PRODUCTION_DATE, a.PO_BREAK_DOWN_ID, a.SEWING_LINE, a.ITEM_NUMBER_ID, c.COLOR_NUMBER_ID, sum(b.PRODUCTION_QNTY) as PRODUCTION_QNTY, 1 as ORDER_TYPE FROM GBL_TEMP_ENGINE gbl, WO_PO_COLOR_SIZE_BREAKDOWN c , PRO_GARMENTS_PRODUCTION_DTLS b, PRO_GARMENTS_PRODUCTION_MST a WHERE gbl.entry_form = 5001 and gbl.ref_from = 56 and gbl.user_id = $user_id  and gbl.ref_val=c.po_break_down_id and c.id=b.color_size_break_down_id and b.mst_id=a.id and c.status_active = 1 and c.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and b.production_type = 5 and a.status_active = 1 and a.production_type = 5 and a.is_deleted = 0 group by a.PRODUCTION_DATE, a.PO_BREAK_DOWN_ID, a.SEWING_LINE, a.ITEM_NUMBER_ID, c.COLOR_NUMBER_ID UNION ALL SELECT a.PRODUCTION_DATE,c.ID as PO_BREAK_DOWN_ID ,a.LINE_ID as SEWING_LINE,a.GMTS_ITEM_ID as ITEM_NUMBER_ID,b.COLOR_ID as COLOR_NUMBER_ID,sum(d.PROD_QNTY) as PRODUCTION_QNTY , 2 as ORDER_TYPE FROM GBL_TEMP_ENGINE gbl, SUBCON_ORD_DTLS c, subcon_gmts_prod_dtls a, subcon_ord_breakdown b,SUBCON_GMTS_PROD_COL_SZ d WHERE gbl.entry_form = 5001 AND gbl.REF_FROM = 56 AND gbl.user_id = $user_id AND gbl.ref_val=c.ID AND c.ID = b.ORDER_ID AND a.ORDER_ID = c.ID and d.ORD_COLOR_SIZE_ID = b.ID AND d.DTLS_ID = a.id AND a.status_active = 1 AND a.is_deleted = 0 and a.PRODUCTION_TYPE=2 AND d.PRODUCTION_TYPE=2 group by a.PRODUCTION_DATE,c.ID  ,a.LINE_ID ,a.GMTS_ITEM_ID ,b.COLOR_ID";
		...
		$daywise_sql = "SELECT c.PUB_SHIPMENT_DATE as SHIP_DATE,b.IS_OFFDAY, b.WORKING_HOUR, c.PO_BREAK_DOWN_ID,a.LINE_ID,a.START_DATE,a.END_DATE, a.BALANCE_QTY,a.FIRST_DAY_OUTPUT,a.INCREMENT_QTY,a.TERGET ,a.COMPANY_ID, a.LOCATION_ID,c.ITEM_NUMBER_ID ,a.OFF_DAY_PLAN,a.ORDER_COMPLEXITY,b.PLAN_DATE,b.PLAN_QNTY,b.PLAN_ID ,C.JOB_NO,c.COLOR_NUMBER_ID,c.PLAN_QNTY as PLAN_COLOR_QTY,a.ORDER_TYPE from ppl_sewing_plan_board a,ppl_sewing_plan_board_dtls b,PPL_SEWING_PLAN_BOARD_POWISE c, gbl_temp_engine gbl where a.plan_id=c.plan_id and b.plan_id=c.plan_id and a.plan_id=b.plan_id and a.plan_id=gbl.ref_val and gbl.entry_form=5000 and gbl.REF_FROM=53 and gbl.user_id=$user_id and a.is_deleted=0 and a.status_active=1";
		...
		//print_r($blance_pro_res_arr);die;
		...
		//print_r($blance_prod_data_arr);die;
		//.................................blance data
					...
					foreach ($lineArr as $line => $colorArr) {
						foreach ($colorArr as $color => $rolorRow) {
							foreach ($rolorRow as $date) {
								...
								//print_r($blance_color_line_prod_data_arr);die;
		...
		//------------------
		//print_r($po_line_color_date_production_arr);die;
		...
		//print_r("SELECT min(a.task_start_date) as TASK_START_DATE,max(a.task_finish_date) as TASK_FINISH_DATE,a.PO_NUMBER_ID from tna_process_mst a, gbl_temp_engine gbl where a.is_deleted=0 and a.status_active=1  and a.task_number in(86,190,191) and a.PO_NUMBER_ID=gbl.ref_val and gbl.entry_form=5001 and gbl.REF_FROM=56 and gbl.user_id=$user_id  group by a.po_number_id");
		//die;
		...
		foreach ($plan_data as $rows) {
			...
			$ROWS_PO_BREAK_DOWN_ID_ARR3 = $po_arr_by_plan[$rows->PLAN_ID];
			...
			//print_r($blance_pro_qty_qnty[$rows->PLAN_ID]);die;
			...
			//print_r($plan_production_qnty_rest);die;
			...
			foreach ($ROWS_COLOR_NUMBER_ID_ARR as $cid) {
				...
				$color_name_arr[] = $lib_color_arr[$cid];
			}
			//print_r($color_name_arr);die;
			$size_name_arr = array();
			foreach ($ROWS_COLOR_NUMBER_ID_ARR as $cid) {
				//$size_name_arr[$cid] = $lib_size_arr[$cid];
				$size_name_arr[] = $lib_size_arr[$cid];
			}
			...
			if ($multi_po == 0) {
				$all_buyer = array();
				foreach ($ROWS_PO_BREAK_DOWN_ID_ARR3 as $v) {
			...
			$data_array[$i]["PRODUCT_CATE"] = ($rows->PRODUCT_CATE) ? $rows->PRODUCT_CATE : 0;
			$data_array[$i]["PRODUCT_CATE_H"] = ($rows->PRODUCT_CATE_H) ? $rows->PRODUCT_CATE_H : "";
			$groupingArr = array();
			foreach ($ROWS_PO_BREAK_DOWN_ID_ARR3 as $v) {
			...
			//print_r($po_line_color_production_arr[1]);die;
			...
			$data_array[$i]["PUB_SHIP_DATES"] = $rows->PUB_SHIP_DATES_H;
			$data_array[$i]["PLAN_CUT_QTY_H"] = ($rows->PLAN_CUT_QTY_H) ? $rows->PLAN_CUT_QTY_H : 0;
			...
			foreach (explode(',', $rows->PO_BREAK_DOWN_ID_H) as $pid) {
				foreach (explode(',', $rows->COLOR_NUMBER_ID_H) as $Cid) {
					...
					//$tmpArr[] = $po_wise_colour_production_arr[$rows->PLAN_ID][$pid][$Cid] * 1;
				}
			}
			// print_r($po_wise_colour_production_arr);
			// die;
