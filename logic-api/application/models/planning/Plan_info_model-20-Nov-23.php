...
class Plan_info_model extends CI_Model
{
	...
	function plan_info($company_id, $location_id, $floor_id, $txt_date_from, $user_id)
	{
		...
		//print_r($po_arr_by_plan);die;		


		$images_sql = "SELECT a.ID,b.IMAGE_LOCATION from WO_PO_BREAK_DOWN a, COMMON_PHOTO_LIBRARY b, gbl_temp_engine gbl where a.job_no_mst=b.MASTER_TBLE_ID  and a.is_deleted=0 and a.id= gbl.ref_val and gbl.REF_FROM=53 and gbl.entry_form=5000 and gbl.user_id=$user_id and b.file_type=1";
		...
		foreach ($plan_data as $rows) {
			...
			if (strtotime($today) < strtotime($first_booking_delivery_date[$rows->PLAN_ID])) {
				...
				$data_array[$i]["TOP_BORDER_COLOR"] = "#660000";
			}
