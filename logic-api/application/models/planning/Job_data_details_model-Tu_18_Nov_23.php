...
class Job_data_details_model extends CI_Model
{
	...
	{   //print_r(5);die; 
		...
		$gsd_sql = "SELECT a.WORKING_HOUR, a.TOTAL_SMV, a.GMTS_ITEM_ID, a.STYLE_REF, b.RESOURCE_GSD, c.EFFICIENCY, c.ALLOCATED_MP, c.TARGET, a.BULLETIN_TYPE FROM ppl_gsd_entry_mst a, ppl_gsd_entry_dtls b LEFT JOIN ppl_balancing_mst_entry c ON     c.GSD_MST_ID = b.mst_id AND c.STATUS_ACTIVE = 1 AND c.IS_DELETED = 0 AND c.ALLOCATED_MP > 0 WHERE a.ID = b.MST_ID AND a.STATUS_ACTIVE = 1 AND a.IS_DELETED = 0 AND b.STATUS_ACTIVE = 1 AND a.BULLETIN_TYPE = 0 AND a.STYLE_REF IN (SELECT STYLE_REF_NO FROM wo_po_details_master c, GBL_TEMP_ENGINE d WHERE     IS_DELETED = 0 AND STATUS_ACTIVE = 1 AND c.ID = d.REF_VAL AND d.ENTRY_FORM = 5002 AND d.REF_FROM = 56 AND d.USER_ID = $user_id ) ORDER BY a.ID ASC";
		//print_r($gsd_sql);die;
		...
		$query_booking = "SELECT b.PO_BREAK_DOWN_ID, MAX (a.DELIVERY_DATE) AS DELIVERY_DATE, b.TRIM_GROUP, b.BOOKING_TYPE FROM WO_BOOKING_MST a, WO_BOOKING_DTLS b, GBL_TEMP_ENGINE c WHERE     a.BOOKING_NO = b.BOOKING_NO AND a.STATUS_ACTIVE = 1 AND a.IS_DELETED = 0 AND b.STATUS_ACTIVE = 1 AND b.IS_DELETED = 0 AND b.PO_BREAK_DOWN_ID = c.REF_VAL AND c.ENTRY_FORM = 5002 AND c.REF_FROM = 55 AND c.USER_ID = $user_id GROUP BY b.PO_BREAK_DOWN_ID, b.TRIM_GROUP, b.BOOKING_TYPE";
		// print_r($query_booking);
		// die;
		$table_booking = $this->db->query($query_booking)->result();
		$lib_delivery_date = array();
		$temp_delivery_date = array();
		// print_r($all_po_by_style_arr);
		// die;
		...
		$subcon_arr = [];
		if (($order_type == 0 || $order_type == 2) && $product_cate == 0 && empty($internal_ref) && empty($department) && empty($sub_department) && empty($brand)&& empty($season) ) {
			...
			foreach ($table_subcon as $row) {
				...
				$job_qnty[$row->ID] += $row->ORDER_QUANTITY;

				// if($row->SUBCON_JOB == 'RpC-23-00054'){
				// 	//print_r($planned_qnty[$row->PO_BREAKDOWN_ID]); die;
				// 	//print_r($row->PO_BREAKDOWN_ID); die;
				// }
				//print_r($subcon_production_qnty[10455]);die;
				//print_r($row->PO_BREAKDOWN_ID);die;
				//print_r($table_subcon);die;
				// if($row->PO_BREAKDOWN_ID == 10495){
				// 	print_r($planned_qnty[2]);die;
				// }
				...
				$subcon_arr[] = [
					...
					"YEAR" => 0,
					"STYLE_REF_NO" => $row->CUST_STYLE_REF,
					"INTERNAL_REF" => "",
					"JOB_QUANTITY" => ($job_qnty[$row->ID]) ? $job_qnty[$row->ID] : 0,
