...
class Job_data_details_model extends CI_Model
{
	...
	{ //print_r(5);die;
		...
		$production_dtls_data_arr = $this->get_production_qnty_by_po_item(implode(',', $poArr), '', '', $plan_level, $user_id);
		...
		foreach ($sql_exe as $rows) {
			...
			//Full Plan Sorting 
			if ($rows->PLAN_CUT_QNTY > $planned_qnty[$rows->PO_BREAK_DOWN_ID][$rows->GMTS_ITEM_ID][$rows->COLOR_NUMBER_ID] && $plan_status == 3) {
				continue;
			}
	...
	function get_production_qnty_by_po_item($po_ids, $lineid = '', $daterange = '', $array_type = 1, $user_id)
	{
