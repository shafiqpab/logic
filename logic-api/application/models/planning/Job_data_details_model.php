...
class Job_data_details_model extends CI_Model
{
	...
	function Job_data_details($cbo_company_mst, $cbo_buyer_name = "0", $chk_job_wo_po = "0", $txt_date_from, $txt_date_to, $garments_nature = "", $txt_job_prifix = "", $cbo_year_selection, $cbo_string_search_type = "", $txt_order_search = "", $cbo_date_type, $plan_level = "0", $txt_style_ref = "", $ignore_tna = "0", $order_status = "0", $po_break_down_id = 0, $set_dtls_id = 0, $color_size_id = 0, $plan_full = 0, $smv_range = '', $item_name = '', $internal_ref = '', $department = 0, $sub_department = 0, $brand = 0, $season = 0, $season_year = 0, $ignore_full_prod = 0, $smv_type, $plan_status, $user_id, $order_type, $product_cate, $emb_filter)
	{
		...
		if (count($gbl_insert_po_array) || count($gbl_insertjob_array)) {
			...
			$this->db->insert_batch("PPL_BOARD_LOAD_LIST_TEMP", $gbl_insert_po_array);
			$this->db->insert_batch("PPL_BOARD_LOAD_LIST_TEMP", $gbl_insertjob_array);
		}

		//$data = $this->db->query("SELECT * FROM PPL_BOARD_LOAD_LIST_TEMP WHERE REF_FROM = 56")->result();


		$query_emb = "SELECT a.id,a.JOB_ID, a.job_no, a.EMB_NAME from wo_pre_cost_embe_cost_dtls a,PPL_BOARD_LOAD_LIST_TEMP gbl where a.JOB_ID=gbl.REF_VAL and gbl.ENTRY_FORM = 5002 and gbl.USER_ID =$user_id and gbl.REF_FROM = 56 and a.is_deleted=0";
		...
		$tna_query = "SELECT MIN (TASK_START_DATE) AS TASK_START_DATE, MAX (TASK_FINISH_DATE) AS TASK_FINISH_DATE, PO_NUMBER_ID, JOB_NO, TASK_NUMBER FROM tna_process_mst a,PPL_BOARD_LOAD_LIST_TEMP b WHERE IS_DELETED = 0 AND STATUS_ACTIVE = 1 AND a.PO_NUMBER_ID = b.REF_VAL AND b.ENTRY_FORM = 5002 AND b.REF_FROM = 55 AND b.user_id = $user_id GROUP BY PO_NUMBER_ID, JOB_NO, TASK_NUMBER";
		...
		$sql = "SELECT PO_BREAK_DOWN_ID, SUM (PRODUCTION_QUANTITY) AS PRODUCTION_QUANTITY FROM pro_garments_production_mst a, PPL_BOARD_LOAD_LIST_TEMP b WHERE a.PO_BREAK_DOWN_ID = b.REF_VAL AND b.ENTRY_FORM = 5002 AND b.REF_FROM = 55 AND b.USER_ID = $user_id AND PRODUCTION_TYPE = 5 AND STATUS_ACTIVE = 1 AND IS_DELETED = 0 GROUP BY PO_BREAK_DOWN_ID";
		//print_r($sql);die;
		...
		$query_booking = "SELECT a.ID,a.BOOKING_NO_PREFIX_NUM,a.BOOKING_NO,b.PO_BREAK_DOWN_ID, MAX (a.DELIVERY_DATE) AS DELIVERY_DATE, b.TRIM_GROUP, b.BOOKING_TYPE FROM WO_BOOKING_MST a, WO_BOOKING_DTLS b, PPL_BOARD_LOAD_LIST_TEMP c WHERE a.ID = b.BOOKING_MST_ID AND a.STATUS_ACTIVE = 1 AND a.IS_DELETED = 0 AND b.STATUS_ACTIVE = 1 AND b.IS_DELETED = 0 AND b.PO_BREAK_DOWN_ID = c.REF_VAL AND c.ENTRY_FORM = 5002 AND c.REF_FROM = 55 AND c.USER_ID = $user_id GROUP BY b.PO_BREAK_DOWN_ID, b.TRIM_GROUP, b.BOOKING_TYPE,a.ID,a.BOOKING_NO,a.BOOKING_NO_PREFIX_NUM";
		...
		foreach ($table_booking as $row) {
			...
			{
				$booking_no_Po_wise[$row->PO_BREAK_DOWN_ID][$row->ID] = $row->BOOKING_NO_PREFIX_NUM;
			}
		...
		$query_efficiency_slab = "SELECT a.COMPANY_ID,a.GMTS_ITEM_ID,a.BUYER_ID,b.SMV_UPPER_LIMIT,b.SMV_LOWER_LIMIT,b.LEVEL_TYPE from EFFICIENCY_PERCENTAGE_SLAB_MST a, EFFICIENCY_PERCENTAGE_SLAB_DTL b where a.id=b.mst_id and a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0 and b.LEVEL_TYPE is not null ";
		// print_r($query_booking);
		// die;
		$table_efficiency_slab = $this->db->query($query_efficiency_slab)->result();

		$complex_level_by_smv_range = array();
		foreach ($table_efficiency_slab as $row) {
			$complex_level_by_smv_range[$row->COMPANY_ID][$row->GMTS_ITEM_ID][$row->BUYER_ID][] =
				[
					"upper_smv_lmt" => $row->SMV_UPPER_LIMIT,
					"lower_smv_lmt" => $row->SMV_LOWER_LIMIT,
					"complexity" => $row->LEVEL_TYPE,
				];
		}

		//print_r($complex_level_by_smv_range);die;
		...
		foreach ($sql_exe as $rows) {
			...
			$complexity_level = 1;
			if ($by_pass == 1) {
				$smv = $gsd_array[$rows->STYLE_REF_NO][$rows->GMTS_ITEM_ID]["SMV"];

				foreach ($complex_level_by_smv_range[$rows->COMPANY_NAME][$rows->GMTS_ITEM_ID][$rows->BUYER_NAME] as $set) {
					$set = (object)$set;
					//print_r($set);die;					
					if ($set->lower_smv_lmt <= $smv && $set->upper_smv_lmt >= $smv) {
						$complexity_level = $set->complexity;
						break;
					}
				}
			} else {
				$complexity_level = $job_set_data[$rows->JOB_NO][$rows->ITEM_NUMBER_ID]['COMPLEXITY'];
			}
			//print_r();die;
			$data_array[$i]["ITEM_COMPLEXITY_LEVEL"] = ($complexity_level) ? $complexity_level : 1;
