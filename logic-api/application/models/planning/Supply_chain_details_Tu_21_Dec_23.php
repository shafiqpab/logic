	...
	function supply_chain_details($po_breakdown_ids, $job_no)
	{
		...
		foreach ($sqlfabRes as $row) {
			...
			if ($itemRatio != "") {
				...
				$reqQtyAmtArr[$row->BODY_PART_ID][$row->FAB_NATURE_ID][$row->LIB_YARN_COUNT_DETER_ID][$row->COLOR_NUMBER_ID]['prodfin_qty'] += $finReq;
			}
		}
		...
		foreach ($sqlfabRes as $row) {
			...
			$mat_inv[$row->POID][$row->FAB_NATURE_ID][$row->COLOR_NUMBER_ID][] = [
				...
				"UOM" => $unit_of_measurement[$row->UOM],
				"REQ_QNTY" => number_format($reqQtyAmtArr[$row->BODY_PART_ID][$row->FAB_NATURE_ID][$row->LIB_YARN_COUNT_DETER_ID][$row->COLOR_NUMBER_ID]["prodfin_qty"],2)." ",
				...
				"BALANCE_QNTY" => number_format($lib_booking_quantity[$row->BODY_PART_ID][$row->FAB_NATURE_ID][$row->LIB_YARN_COUNT_DETER_ID][$row->COLOR_NUMBER_ID] - $lib_production_quantity[$row->BODY_PART_ID][$row->POID][$row->FAB_NATURE_ID][$row->COLOR_NUMBER_ID],2),
				"INHOUSE_PERCENT" => round($inhouse_percent)." ",
...
where 1=1 and a.id=b.wo_pre_cost_trim_cost_dtls_id and b.cons>0 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $jobCond $jobidCond";
		...
		// foreach ($table_trim_book_qnty as $row) {
		// 	if($trims_book_qnty_arr[$row->TRIMS_ID]){
		// 		$in_house_percent =  ($trims_quantity[$row->TRIM_TYPE][$row->TRIMS_ID]*100)/$trims_book_qnty_arr[$row->TRIMS_ID];
		// 	}else{
		// 		$in_house_percent = 0;
		// 	}
		...
		// 			"REQUIRED_QUANTITY" => number_format($arr_req_qnty[$row->TRIMS_ID],2),
		// 			"BOOKING_QUANTITY" => number_format($trims_book_qnty_arr[$row->TRIMS_ID],2),
		// 			"IN_HOUSE_QNTY" => number_format($trims_quantity[$row->TRIM_TYPE][$row->TRIMS_ID],2),
		// 			"BALANCE_QNTY" => number_format($trims_book_qnty_arr[$row->TRIMS_ID] - $trims_quantity[$row->TRIM_TYPE][$row->TRIMS_ID],2),
		// 			"INHOUSE_PERCENT" => round($in_house_percent[$row->TRIMS_ID]),
		...
		foreach ($table_req_qnty as $row) {
			...
			if ($row->TRIM_TYPE == 1) {
				$sewing_trims[$row->TRIM_GROUP] = [
					...
					"BALANCE_QNTY" => number_format($trims_book_qnty_arr[$row->TRIM_GROUP] - $trims_quantity[$row->TRIM_TYPE][$row->TRIM_GROUP],2),
					"INHOUSE_PERCENT" => round($in_house_percent[$row->TRIM_GROUP])." ",
