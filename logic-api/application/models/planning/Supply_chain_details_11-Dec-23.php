...
class Supply_chain_details extends CI_Model
{
	...
	function supply_chain_details($po_breakdown_ids, $job_no)
	{
		...
		//FABRIC PART:
		//====================================
		$sqlfab = "SELECT a.BODY_PART_ID,a.FAB_NATURE_ID,a.LIB_YARN_COUNT_DETER_ID,a.BUDGET_ON,a.job_id AS JOB_ID, a.id AS ID, a.item_number_id AS ITEM_NUMBER_ID, a.fab_nature_id AS FAB_NATURE_ID, a.color_type_id AS COLOR_TYPE_ID, a.fabric_source as FABRIC_SOURCE, a.color_size_sensitive AS COLOR_SIZE_SENSITIVE, a.construction AS CONSTRUCTION, a.gsm_weight AS GSM_WEIGHT, a.uom AS UOM, b.po_break_down_id AS POID, b.color_number_id AS COLOR_NUMBER_ID, b.gmts_sizes AS SIZE_NUMBER_ID, b.cons AS CONS, b.requirment AS REQUIRMENT, b.rate as RATE
		...
		foreach ($sqlfabRes as $row) {
			...
			if ($itemRatio != "") {
				...
				$reqQtyAmtArr[$row->BODY_PART_ID][$row->FAB_NATURE_ID][$row->LIB_YARN_COUNT_DETER_ID]['prodfin_qty'] += $finReq;
			}
		}
		...
		//$query_booking = "SELECT a.ID,b.PO_BREAK_DOWN_ID, a.ITEM_CATEGORY, b.GMTS_COLOR_ID,b.CONSTRUCTION,b.COPMPOSITION,b.DESCRIPTION,b.UOM, b.FIN_FAB_QNTY AS QUANTITY, a.FABRIC_SOURCE, b.COLOR_SIZE_TABLE_ID, b.FABRIC_COLOR_ID, a.DELIVERY_DATE, b.COLOR_TYPE,b.WO_QNTY,b.TRIM_GROUP FROM WO_BOOKING_MST a, WO_BOOKING_DTLS b WHERE a.ID = b.BOOKING_MST_ID AND a.IS_DELETED = 0 AND a.STATUS_ACTIVE = 1 AND b.IS_DELETED = 0 AND b.STATUS_ACTIVE = 1 and b.PO_BREAK_DOWN_ID in( $po_breakdown_ids)";
		...
		foreach ($table_booking as $row) {
			...
			$lib_booking_quantity[$row->BODY_PART_ID][$row->ITEM_CATEGORY][$row->LIB_YARN_COUNT_DETER_ID][$row->FABRIC_COLOR_ID] += $row->QUANTITY;
		...
		foreach ($table_production_quantity as $row) {
			$lib_production_quantity[$row->BODY_PART_ID][$row->PO_BREAKDOWN_ID][$row->ITEM_CATEGORY][$row->COLOR_ID][$row->BOOKING_ID] += $row->RECEIVE_QNTY;
		}
		...
		foreach ($table_production_quantity as $row) {
			//print_r($row);die;
			$inhouse_percent = ($row->RECEIVE_QNTY * 100) / $lib_booking_quantity[$row->BODY_PART_ID][$row->ITEM_CATEGORY][$row->FABRIC_DESCRIPTION_ID][$row->COLOR_ID];


			$mat_inv[$row->PO_BREAKDOWN_ID][$row->ITEM_CATEGORY][$row->COLOR_ID][] = [
				...
				"DESCRIPTION" => $row->PRODUCT_NAME_DETAILS,
				"GMTS_COLOR_ID" => $lib_color[$row->COLOR_ID],
				"UOM" => $unit_of_measurement[$row->UOM],
				"REQ_QNTY" => $reqQtyAmtArr[$row->BODY_PART_ID][$row->ITEM_CATEGORY][$row->FABRIC_DESCRIPTION_ID]["prodfin_qty"],
				"BOOKING_QNTY" => $lib_booking_quantity[$row->BODY_PART_ID][$row->ITEM_CATEGORY][$row->FABRIC_DESCRIPTION_ID][$row->COLOR_ID],
				"INV_QNTY" => $row->RECEIVE_QNTY,
				//"INV_QNTY" => $lib_production_quantity[$row->BODY_PART_ID][$row->PO_BREAKDOWN_ID][$row->ITEM_CATEGORY][$row->COLOR_ID][$row->BOOKING_ID],
				"BALANCE_QNTY" => round($lib_booking_quantity[$row->BODY_PART_ID][$row->ITEM_CATEGORY][$row->FABRIC_DESCRIPTION_ID][$row->COLOR_ID] - $row->RECEIVE_QNTY),
				"INHOUSE_PERCENT" => ($inhouse_percent) ? $inhouse_percent : 0,
				//"DELIVERY_DATE" => $lib_booking_delivery_date[$row->PO_BREAKDOWN_ID],
				"DELIVERY_DATE" => $row->DELIVERY_DATE,
			];
		}
		//print_r($table_production_quantity);die;
		$mat_inv_2 = array();
		...
		$query_trim_book_qnty = "SELECT a.ITEM_CATEGORY,a.FABRIC_SOURCE,sum(b.WO_QNTY) as BOOKING_QNTY,b.TRIM_GROUP,c.ITEM_NAME,a.DELIVERY_DATE
		FROM WO_BOOKING_MST a, WO_BOOKING_DTLS b, LIB_ITEM_GROUP c WHERE a.ID = b.BOOKING_MST_ID and b.TRIM_GROUP = c.ID and a.ITEM_CATEGORY =4
		AND a.IS_DELETED = 0 AND a.STATUS_ACTIVE = 1 AND b.IS_DELETED = 0 AND b.STATUS_ACTIVE = 1 and b.PO_BREAK_DOWN_ID in( $po_breakdown_ids)
		GROUP BY  a.ITEM_CATEGORY,a.FABRIC_SOURCE,b.TRIM_GROUP,c.ITEM_NAME,a.DELIVERY_DATE";
		...
		foreach ($table_trim_book_qnty as $row) {
			$trims_book_qnty_arr[$row->TRIM_GROUP] = $row->BOOKING_QNTY;

			if(empty($trims_delivery_date[$row->TRIM_GROUP])||$trims_delivery_date[$row->TRIM_GROUP] < $row->DELIVERY_DATE){
				$trims_delivery_date[$row->TRIM_GROUP] = $row->DELIVERY_DATE;
			}
		...
		foreach ($table_trims as $row) {
			...
			if ($row->TRIM_TYPE == 1) {
				$sewing_trims[$row->TRIMS_ID] = [
					"ITEM_NAME" => $row->ITEM_NAME,
					"ITEM_DESCRIPTION" => $array_trims_type[$row->TRIM_TYPE],
					"PO_NUMBER" => $row->PO_NUMBER,
					"UOM" =>  $unit_of_measurement[$row->ORDER_UOM],
					"REQUIRED_QUANTITY" => $arr_req_qnty[$row->TRIMS_ID],
					"BOOKING_QUANTITY" => $trims_book_qnty_arr[$row->TRIMS_ID],
					"IN_HOUSE_QNTY" => $trims_quantity[$row->TRIM_TYPE][$row->TRIMS_ID],
					"BALANCE_QNTY" => round($trims_book_qnty_arr[$row->TRIMS_ID] - $trims_quantity[$row->TRIM_TYPE][$row->TRIMS_ID]),
					"DELIVERY_DATE" => $trims_delivery_date[$row->TRIMS_ID],
				];
			} else if ($row->TRIM_TYPE == 2) {
				$finish_trims[$row->TRIMS_ID] = [
					"ITEM_NAME" => $row->ITEM_NAME,
					"ITEM_DESCRIPTION" => $array_trims_type[$row->TRIM_TYPE],
					"PO_NUMBER" => $row->PO_NUMBER,
					"UOM" =>  $unit_of_measurement[$row->ORDER_UOM],
