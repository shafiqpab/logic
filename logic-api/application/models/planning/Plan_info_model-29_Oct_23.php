...
class Plan_info_model extends CI_Model
{
    ...
    function plan_info($company_id, $location_id, $floor_id, $txt_date_from, $user_id)
	{
		...
		if (!empty($line_id_arr)) {
			...
			$production_sql = "SELECT a.PRODUCTION_DATE, a.PO_BREAK_DOWN_ID, a.SEWING_LINE, a.ITEM_NUMBER_ID, c.COLOR_NUMBER_ID, SUM (b.PRODUCTION_QNTY)     PRODUCTION_QNTY, 1 as ORDER_TYPE  FROM PRO_GARMENTS_PRODUCTION_MST a, GBL_TEMP_ENGINE gbl,  PRO_GARMENTS_PRODUCTION_DTLS b, WO_PO_COLOR_SIZE_BREAKDOWN  c WHERE   a.PO_BREAK_DOWN_ID = gbl.ref_val AND a.id = b.mst_id AND b.COLOR_SIZE_BREAK_DOWN_ID = c.id AND a.status_active = 1 AND a.production_type = 5 AND a.is_deleted = 0 AND gbl.entry_form = 5001 AND gbl.REF_FROM = 56 AND gbl.user_id = $user_id AND b.status_active = 1 AND b.is_deleted = 0 AND b.production_type = 5 AND c.status_active = 1 AND c.is_deleted = 0  GROUP BY a.PRODUCTION_DATE, a.po_break_down_id, a.SEWING_LINE, a.ITEM_NUMBER_ID, c.COLOR_NUMBER_ID
			UNION ALL 
			SELECT a.PRODUCTION_DATE,c.ID as PO_BREAK_DOWN_ID ,a.LINE_ID as SEWING_LINE,a.GMTS_ITEM_ID as ITEM_NUMBER_ID,b.COLOR_ID as COLOR_NUMBER_ID,SUM(d.PROD_QNTY) as PRODUCTION_QNTY , 2 as ORDER_TYPE
			...
			GROUP BY  a.PRODUCTION_DATE,c.ID  ,a.LINE_ID ,a.GMTS_ITEM_ID ,b.COLOR_ID";
		...
		$query_subcon = "SELECT b.ID AS PO_BREAKDOWN_ID , a.PARTY_ID, a.SUBCON_JOB,b.CUST_STYLE_REF, b.ORDER_NO, b.ORDER_QUANTITY,b.DELIVERY_DATE,b.MATERIAL_RECV_DATE FROM SUBCON_ORD_MST a, SUBCON_ORD_DTLS b, SUBCON_ORD_BREAKDOWN c WHERE a.ID = b.MST_ID AND a.ID = c.MST_ID AND a.ENTRY_FORM = 238 AND b.MAIN_PROCESS_ID = '5' AND a.STATUS_ACTIVE = 1 AND a.IS_DELETED = 0 AND b.STATUS_ACTIVE = 1 AND b.IS_DELETED = 0 AND c.STATUS_ACTIVE = 1 AND c.IS_DELETED = 0";
		...
		foreach ($plan_data as $rows) {
			...
			// if($rows->PLAN_ID==2033){
			// 	print_r($data_array[$i]["JOB_NO"]);die;
			// }
