...
class Planning_model extends CI_Model {
  ...
  where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.production_type=5 and b.production_type=5 $companyCon $location_cond_effi $floor_con and a.prod_reso_allo=1 and a.production_date between '$firstDate' and '$lastDate' group by a.production_date, a.sewing_line, a.prod_reso_allo, a.po_break_down_id, a.item_number_id";
	...
	function get_job_data_info($cbo_company_mst, $cbo_buyer_name = "0", $chk_job_wo_po = "0", $txt_date_from, $txt_date_to, $garments_nature = "", $txt_job_prifix = "", $cbo_year_selection, $cbo_string_search_type = "", $txt_order_search = "", $cbo_date_type, $plan_level = "0", $txt_style_ref = "", $ignore_tna = "0", $order_status = "0", $po_break_down_id = 0, $set_dtls_id = 0, $color_size_id = 0,$plan_full=0,$smv_range='',$item_name='',$internal_ref='',$department=0,$sub_department=0,$brand=0,$season=0,$season_year=0) {
   ...
   a.next_increment, a.next_terget,a.increment_qty,a.terget,a.day_wise_plan,a.company_id,a.location_id,a.item_number_id, a.off_day_plan,a.order_complexity,a.ship_date, extra_param,a.plan_level,a.first_day_capacity,a.last_day_capacity,a.seq_no,a.po_company_id,a.use_learning_curve,a.current_production_date,a.production_percent, a.top_border_color,a.bottom_border_color,a.left_color,a.right_color , a.MERGE_COMMENTS,a.MERGE_TYPE,a.INSERT_DATE ,a.NOTES,a.CLOSING_STATUS,a.CLOSED_BY,a.CLOSING_DATE,a.CLOSING_NOTE,a.RE_OPEN_DATE,a.RE_OPENED_BY,a.RE_OPEN_NOTE,a.set_dtls_id,a.color_size_id,a.ALLOCATED_MP,a.BYPASS_MP,a.remaining_work_hour,a.AUTO_TARGET order by a.id desc  ";
		...
		$daywise_sql = "SELECT b.IS_OFFDAY, b.WORKING_HOUR, c.PO_BREAK_DOWN_ID,a.LINE_ID,a.START_DATE,a.END_DATE, a.BALANCE_QTY,a.FIRST_DAY_OUTPUT,a.INCREMENT_QTY,a.TERGET ,a.COMPANY_ID, a.LOCATION_ID,a.ITEM_NUMBER_ID ,a.OFF_DAY_PLAN,a.ORDER_COMPLEXITY,b.PLAN_DATE,b.PLAN_QNTY,b.PLAN_ID ,C.JOB_NO from ppl_sewing_plan_board a,ppl_sewing_plan_board_dtls b,PPL_SEWING_PLAN_BOARD_POWISE c where a.plan_id=c.plan_id and  b.plan_id=c.plan_id and a.plan_id=b.plan_id and a.is_deleted=0 and a.status_active=1 ";
 ...
 //var_dump($po_line_production_arr);die;
//var_dump($plan_po_id_arr);
//var_dump($plan_production_qnty);
//die;
// echo $plan_production_qnty2[10].'='.$plan_wise_balance_qty_arr[10];die;
		...
		foreach ($plan_data as $rows) {
			...
			if (1 == 1) {
				...
				$plan_production_qnty_rest[$rows->PLAN_ID]=($plan_production_qnty[$rows->PLAN_ID]-$plan_wise_balance_qty_arr[$rows->PLAN_ID]) * 1;
				
				$data_array[$i]["IS_PRODUCTION"] = ($plan_production_qnty_rest[$rows->PLAN_ID] <= 0) ? false : true;
				//$data_array[$i]["THIS_PRODUCTION"] = ($plan_production_qnty_rest[$rows->PLAN_ID]>0)?$plan_production_qnty_rest[$rows->PLAN_ID]:0;
				...
				//$data_array[$i]["THIS_PRODUCTION"] =$plan_production_qnty[$rows->PLAN_ID];
				...
				if ($multi_po == 0) {
					...
					$npo_arr1 = explode(',', $rows->PO_BREAK_DOWN_ID);
					foreach ($npo_arr1 as $v) {
						$groupingArr[] = $wo_po_details[$v]['GROUPING'];
					}
			...
			foreach($dateRows as $plan_date=>$lineRows){
				...
				foreach($lineRows as $lid=>$bal){
					...
					if($data_array[$serial]["start_date"]==$dateKey){
						...
						//$data_array[$serial]["PRODUCTION_QNTY"] = $data_array[$serial]["PRODUCTION_QNTY"]+$adjustQty;
 ...
 */
	...
	function create_plan($plan_obj) {
		...
		if ($response_obj->Status == 1) {
			...
			$plan_qty_arr = array();
			foreach ($response_obj->SewingPlanBoardDtls as $sewing_plan_dtls_row) {
				if ($sewing_plan_dtls_row->RowState != "delete" && $sewing_plan_dtls_row->RowState) {
					if ($this->db->dbdriver == 'mysqli') {
		...
		//print_r($plan_qty_arr);die;	
			...
			foreach ($response_obj->SewingPlanBoardPOWise as $sewing_plan_po_row) {
				if ($sewing_plan_po_row->RowState != "delete" && $sewing_plan_po_row->RowState) {
					$planning_id = ($sewing_plan_po_row->RowState == "add") ? $plan_ids[$sewing_plan_po_row->PLAN_ID] : $sewing_plan_po_row->PLAN_ID;
					
					$sewing_plan_po_row->PLAN_QNTY=round(($plan_qty_arr[$planning_id]/$plan_po_qty_arr[$planning_id])*$po_qty_arr[$sewing_plan_po_row->PO_BREAK_DOWN_ID]);
