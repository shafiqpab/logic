...
class Planning_model extends CI_Model {
	...
	public function get_menu_by_privilege($user_id) {
		...
		if ($all_comp) {
			$comp_sql = "SELECT ID,COMPANY_NAME from lib_company   where status_active =1 and is_deleted=0  and id in($all_comp) and CORE_BUSINESS=1 order by company_name";
		} else {
		...
		$erp_com = $this->db->query("SELECT count(ID) as ERP_COM from lib_company   where status_active =1 and is_deleted=0  and CORE_BUSINESS=1")->result();
		...
		if ($all_loc) {
			$loc_sql = "SELECT b.ID,b.LOCATION_NAME,b.COMPANY_ID from lib_location  b, lib_company a  where a.id=b.company_id and b.status_active=1 and b.is_deleted=0 and b.id in($all_loc) and  a.status_active =1 and a.is_deleted=0 and a.CORE_BUSINESS=1 order by b.location_name";
		} else {
			$loc_sql = "SELECT b.ID,b.LOCATION_NAME,b.COMPANY_ID from lib_location  b, lib_company a  where a.id=b.company_id and b.status_active=1 and b.is_deleted=0  and  a.status_active =1 and a.is_deleted=0 and a.CORE_BUSINESS=1 order by b.location_name";
		}
  ...
  where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.production_type=5 and b.production_type=5 $companyCon $location_cond_effi $floor_con and a.prod_reso_allo=1 and a.production_date between '$firstDate' and '$lastDate' group by a.production_date, a.sewing_line, a.prod_reso_allo, a.po_break_down_id, a.item_number_id";
	...
	function get_job_data_info($cbo_company_mst, $cbo_buyer_name = "0", $chk_job_wo_po = "0", $txt_date_from, $txt_date_to, $garments_nature = "", $txt_job_prifix = "", $cbo_year_selection, $cbo_string_search_type = "", $txt_order_search = "", $cbo_date_type, $plan_level = "0", $txt_style_ref = "", $ignore_tna = "0", $order_status = "0", $po_break_down_id = 0, $set_dtls_id = 0, $color_size_id = 0,$plan_full=0,$smv_range='',$item_name='',$internal_ref='') {
		...
		if ($item_name) {
			...
			if ($search_types == 1) {
				$item_were_con = " and ITEM_NAME = '$item_name'  ";
			} else if ($search_types == 2) {
				$item_were_con = " and ITEM_NAME like '$item_name%' ";
			...
			$sql_item="SELECT ID FROM LIB_GARMENT_ITEM WHERE 1=1 $item_were_con ";
			foreach (sql_select($sql_item) as $vals) {
				$item_id_arr[$vals->ID] = $vals->ID;
			}
			
			if(count($item_id_arr)){
				//$txt_date_from='';$txt_date_to='';
				$item_id_str=implode(',',$item_id_arr);
				$item_name_cond = " and GMTS_ITEM_ID in($item_id_str)";
				$item_name_cond_d = " and d.GMTS_ITEM_ID in($item_id_str)";
				$item_name_cond_c = " and c.GMTS_ITEM_ID in($item_id_str)";
			}
		}
	...
	$gsd_sql = "SELECT a.WORKING_HOUR, a.TOTAL_SMV, a.GMTS_ITEM_ID, a.STYLE_REF ,g.RESOURCE_GSD,b.EFFICIENCY,b.ALLOCATED_MP,b.TARGET,c.JOB_NO from ppl_gsd_entry_mst a ,ppl_balancing_mst_entry b,PPL_GSD_ENTRY_DTLS g,wo_po_details_mas_set_details c,wo_po_details_master d ,wo_po_break_down e,wo_po_color_size_breakdown f  where c.JOB_NO=d.job_no and c.JOB_NO=e.job_no_mst and c.job_id=d.id and a.id= b.gsd_mst_id and a.id=g.mst_id and b.gsd_mst_id=g.mst_id and  a.gmts_item_id = c.gmts_item_id and a.style_ref=d.style_ref_no and d.job_no=e.job_no_mst  and e.id=f.po_break_down_id $gsd_were_con and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0  and e.status_active=1 and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 $smv_cond_c $item_name_cond_c $yearCon_d  order by a.id desc ";	
		...
		//$sql="SELECT a.JOB_NO from wo_po_details_master a,wo_po_break_down b where a.job_no=b.job_no_mst and  a.status_active=1  $conds group by a.job_no ";
		
		$sql = "SELECT a.JOB_NO from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c,wo_po_details_mas_set_details d where a.job_no=b.job_no_mst and b.id=c.po_break_down_id and a.job_no=d.job_no and d.job_no=b.job_no_mst and a.status_active=1 $shipment_date $smv_cond_d $item_name_cond_d $yearCon_a $conds group by a.job_no";
		//echo $sql;die;
		...
		//$sql = "SELECT b.PLAN_QNTY,b.PO_BREAK_DOWN_ID,b.ITEM_NUMBER_ID,b.COLOR_NUMBER_ID from ppl_sewing_plan_board a, ppl_sewing_plan_board_powise b,ppl_sewing_plan_board_dtls c where a.plan_id=b.plan_id  and  a.plan_id=c.plan_id and  b.plan_id=c.plan_id  and a.PO_BREAK_DOWN_ID=b.PO_BREAK_DOWN_ID and a.is_deleted=0 and a.status_active=1 ";
		...
		if ($plan_level == 1) {
			if ($this->db->dbdriver == 'mysqli') {
				$sql = "SELECT '' as COLOR_SIZE_ID,b.IS_CONFIRMED,a.JOB_NO_PREFIX_NUM, a.JOB_NO,a.COMPANY_NAME,a.BUYER_NAME,a.STYLE_REF_NO,a.JOB_QUANTITY,a.GARMENTS_NATURE, b.ID PO_BREAK_DOWN_ID,b.PO_NUMBER, b.PO_QUANTITY,b.SHIPMENT_DATE AS SHIPMENT_DATE,b.PUB_SHIPMENT_DATE AS PUB_SHIPMENT_DATE, b.PLAN_CUT, YEAR(a.insert_date) as YEAR,b.ID,SET_BREAK_DOWN,TOTAL_SET_QNTY, SET_SMV,c.ID AS SET_DTLS_ID ,c.GMTS_ITEM_ID,c.SET_ITEM_RATIO,a.CLIENT_ID,c.SMV_PCS,b.GROUPING,b.PACK_HANDOVER_DATE  from wo_po_details_master a, wo_po_break_down b,wo_po_details_mas_set_details c where a.job_no=b.job_no_mst and a.job_no=c.job_no and b.job_no_mst=c.job_no and a.status_active=1 and b.status_active=1 and b.SHIPING_STATUS<3 $str_shi $company $buyer $job_cond $order_cond $shipment_date $set_cond $style_cond $is_confirmed_cond $smv_cond_c $item_name_cond_c $internal_ref_con";
			} else {
				$sql = "SELECT '' as COLOR_SIZE_ID,b.IS_CONFIRMED,a.JOB_NO_PREFIX_NUM, a.JOB_NO,a.COMPANY_NAME,a.BUYER_NAME,a.STYLE_REF_NO,a.JOB_QUANTITY,a.GARMENTS_NATURE, b.ID PO_BREAK_DOWN_ID,b.PO_NUMBER, b.PO_QUANTITY,b.SHIPMENT_DATE AS SHIPMENT_DATE,b.PUB_SHIPMENT_DATE AS PUB_SHIPMENT_DATE, b.PLAN_CUT, to_char(a.insert_date,'YYYY') as YEAR,b.ID,SET_BREAK_DOWN,TOTAL_SET_QNTY, SET_SMV,c.ID AS SET_DTLS_ID ,c.GMTS_ITEM_ID,c.SET_ITEM_RATIO,a.CLIENT_ID,c.SMV_PCS,b.GROUPING,b.PACK_HANDOVER_DATE  from wo_po_details_master a, wo_po_break_down b,wo_po_details_mas_set_details c where a.job_no=b.job_no_mst and a.job_no=c.job_no and b.job_no_mst=c.job_no and a.status_active=1 and b.status_active=1 and b.SHIPING_STATUS<3 $str_shi $company $buyer $job_cond $set_cond $order_cond $shipment_date $style_cond $is_confirmed_cond $smv_cond_c $item_name_cond_c $internal_ref_con";
			}
		...
		if ($plan_level != 1) {
			$sql .= " group by c.item_number_id, b.is_confirmed,a.job_no_prefix_num, a.job_no,a.CLIENT_ID,a.company_name,a.buyer_name, a.style_ref_no,a.job_quantity, b.id,b.po_number,b.PACK_HANDOVER_DATE,b.shipment_date,c.country_ship_date,a.garments_nature,a.insert_date,b.id,b.GROUPING, set_break_down,total_set_qnty,set_smv,d.id,d.gmts_item_id,d.set_item_ratio,d.smv_pcs $group_by";
		}
   ...
   a.next_increment, a.next_terget,a.increment_qty,a.terget,a.day_wise_plan,a.company_id,a.location_id,a.item_number_id, a.off_day_plan,a.order_complexity,a.ship_date, extra_param,a.plan_level,a.first_day_capacity,a.last_day_capacity,a.seq_no,a.po_company_id,a.use_learning_curve,a.current_production_date,a.production_percent, a.top_border_color,a.bottom_border_color,a.left_color,a.right_color , a.MERGE_COMMENTS,a.MERGE_TYPE,a.INSERT_DATE ,a.NOTES,a.CLOSING_STATUS,a.CLOSED_BY,a.CLOSING_DATE,a.CLOSING_NOTE,a.RE_OPEN_DATE,a.RE_OPENED_BY,a.RE_OPEN_NOTE,a.set_dtls_id,a.color_size_id,a.ALLOCATED_MP,a.BYPASS_MP,a.remaining_work_hour,a.AUTO_TARGET order by a.id desc  ";
		...
		$daywise_sql = "SELECT b.IS_OFFDAY, b.WORKING_HOUR, c.PO_BREAK_DOWN_ID,a.LINE_ID,a.START_DATE,a.END_DATE, a.BALANCE_QTY,a.FIRST_DAY_OUTPUT,a.INCREMENT_QTY,a.TERGET ,a.COMPANY_ID, a.LOCATION_ID,a.ITEM_NUMBER_ID ,a.OFF_DAY_PLAN,a.ORDER_COMPLEXITY,b.PLAN_DATE,b.PLAN_QNTY,b.PLAN_ID  from ppl_sewing_plan_board a,ppl_sewing_plan_board_dtls b,PPL_SEWING_PLAN_BOARD_POWISE c where a.plan_id=c.plan_id and  b.plan_id=c.plan_id and a.plan_id=b.plan_id and a.is_deleted=0 and a.status_active=1 ";
		...
		foreach ($plan_data as $rows) {
			...
			if (1 == 1) {
				...
				$plan_production_qnty[$rows->PLAN_ID]=($plan_production_qnty[$rows->PLAN_ID]-$plan_wise_balance_qty_arr[$rows->PLAN_ID]) * 1;
				
				$data_array[$i]["IS_PRODUCTION"] = ($plan_production_qnty[$rows->PLAN_ID] <= 0) ? false : true;
				$data_array[$i]["THIS_PRODUCTION"] = ($plan_production_qnty[$rows->PLAN_ID]>0)?$plan_production_qnty[$rows->PLAN_ID]:0;
 ...
 */
	function create_plan_data_archive($is_archive=0) {
		if ($is_archive == 1) {
			$this->db->trans_start();
			...
			$this->db->query("DELETE FROM PPL_SEWING_PLAN_BOARD_POWISE_A");
			$SEWING_PLAN_BOARD_POWISE_SQL="INSERT INTO PPL_SEWING_PLAN_BOARD_POWISE_A (ID,PLAN_ID,PO_BREAK_DOWN_ID,PLAN_QNTY,PO_DTLS,ITEM_NUMBER_ID,COUNTRY_ID ,COLOR_NUMBER_ID,SIZE_NUMBER_ID,JOB_NO,PO_ID, SMV,SET_DTLS_ID,COLOR_SIZE_ID,PUB_SHIPMENT_DATE)
	...
	function create_plan($plan_obj) {
		...
		//This method archive all plan data before save plan
		if($response_obj->is_archive==1){
			$plan_archive_info = $this->create_plan_data_archive($response_obj->is_archive);
		}
		...
		if ($response_obj->Status == 1) {
			...
			foreach ($response_obj->SewingPlanBoard as $sewing_plan_row) {
				...
				if ($this->db->dbdriver == 'mysqli') {
					$ppl_sewing_plan_board_data = array(
						...
						'BALANCE_QTY' => $sewing_plan_row->BALANCE_QTY,
			...
			//echo "delete from  ppl_sewing_plan_board_dtls where plan_id in($dtls_plan_delete)";die;
			...
			foreach ($response_obj->SewingPlanBoardDtls as $sewing_plan_dtls_row) {
				if ($sewing_plan_dtls_row->RowState != "delete") {
					...
					if ($this->db->dbdriver == 'mysqli') {
						...
						//echo $sewing_plan_dtls_row->RowState;echo"<br>";
						//print_r($ppl_sewing_plan_board_dtls_data);echo"<br>";
						$this->insertData($ppl_sewing_plan_board_dtls_data, "PPL_SEWING_PLAN_BOARD_DTLS");
					}
				}
			}
