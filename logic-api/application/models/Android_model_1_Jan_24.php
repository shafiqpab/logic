...
class Android_model extends CI_Model {
	...
	public function login($user_id, $password, $device_id="",$fcm_token ="") {
		$empty_arrr = [];
		...
		if ($query->num_rows() == 1) {
			...
			if(!empty($fcm_token) && !empty($device_id))
			{
				...
				$token = $this->insert_fcm_token($user_info->ID,$device_id,$fcm_token);
				if($token !=1)
				{
					return $token;
				}
 ...
 ////====End=======////=====
	...
	public function program_create_data($job_id) {
		...
		foreach ($job_sql as $row) 
		{
			...
			$al_job_data[$i]['GreigeItemDescription']=$constructtion_arr[$row->DETERMINATION_ID].', '.$composition_arr[$row->DETERMINATION_ID].', '.$row->GSM_WEIGHT.', '.$row->DIA;
			...
			$al_job_data[$i]['GreyFabricDia']=$dia_width;
	...
	public function insert_fcm_token($user_id,$device_id,$fcm_token)
	{
        ...
        {
           $res = execute_query("UPDATE APPROVAL_NOTI_USER_DEVICES SET FCM_TOKEN ='".$fcm_token."' WHERE  USER_ID = $user_id and DEVICE_ID = '".$device_id."'");
           if($res !=1)
           {
                $this->db->trans_rollback();
                return 0;
	...
	public function rfid_yarn_store_location_update($response_arr)
	{
		...
		if($response_obj->RFID){
			$trans_id = return_field_value("TRANS_ID", "RFID_YARN_DTLS", "ID = $response_obj->RFID","");
			$id = $this->db->query("SELECT ID FROM INV_TRANSACTION WHERE ID = $trans_id and STATUS_ACTIVE = 1 and IS_DELETED = 0")->row();
			$trans_id = $id->ID;
			//print_r($id->ID);die;

			$room_rack_dtls_id = $this->db->query("SELECT * FROM LIB_FLOOR_ROOM_RACK_DTLS  WHERE FLOOR_ROOM_RACK_DTLS_ID = $response_obj->FLOOR_ROOM_RACK_DTLS_ID and STATUS_ACTIVE = 1 and IS_DELETED = 0")->row();
		}
		

		if($trans_id && $room_rack_dtls_id){

			$room_id =$room_rack_dtls_id->ROOM_ID;
			if(empty($room_rack_dtls_id->ROOM_ID)){
				$room_id = 0;
			}
			$shelf_id =$room_rack_dtls_id->SHELF_ID;
			if(empty($room_rack_dtls_id->SHELF_ID)){
				$room_rack_dtls_id = 0;
			}
			$bin_id =$room_rack_dtls_id->BIN_ID;
			if(empty($room_rack_dtls_id->BIN_ID)){
				$bin_id = 0;
			}
			$rack_id = $room_rack_dtls_id->RACK_ID;
			if(empty($room_rack_dtls_id->RACK_ID)){
				$rack_id = 0;
			}
			$floor_id =$room_rack_dtls_id->FLOOR_ID;
			if(empty($room_rack_dtls_id->FLOOR_ID)){
				$floor_id = 0;
			}
			//print_r("UPDATE INV_TRANSACTION SET ROOM=$room_id,SELF=$shelf_id,BIN_BOX=$bin_id,RACK=$rack_id,FLOOR_ID=$floor_id, is_deleted=0 , UPDATED_BY=$response_obj->USER_ID, UPDATE_DATE ='".date("d-M-Y h:i:s a", time())."'  where ID = $trans_id and STATUS_ACTIVE = 1 and IS_DELETED = 0");die;
			$status = $this->db->query("UPDATE INV_TRANSACTION SET ROOM=$room_id,SELF=$shelf_id,BIN_BOX=$bin_id,RACK=$rack_id,FLOOR_ID=$floor_id, is_deleted=0 , UPDATED_BY=$response_obj->USER_ID, UPDATE_DATE ='".date("d-M-Y h:i:s a", time())."'  where ID = $trans_id and STATUS_ACTIVE = 1 and IS_DELETED = 0");
			$msg.= " ***Trans updated";
		}else{
			$msg.= " ***No Trans Found or room rack id found";
		}
