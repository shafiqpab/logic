...
class Planning_model extends CI_Model
{
	...
	public function login($user_id, $password)
	{
		...
		if ($query->num_rows() == 1) {
			...
			if ($user_info->PASSWORD == $this->encrypt($password)) {
				...
				$data = [
					"privilege" => $this->get_menu_by_privilege($user_info->ID),
					"pub_msg" => $msg,
				];
				//print_r($data);die;
				//return $data;
	...
	public function get_menu_by_privilege($user_id)
	{
		...
		$app_version = $this->db->query("SELECT APP_VERSION FROM LR_APP_VERSION ORDER BY ID DESC FETCH FIRST 1 ROW ONLY")->row();
		$data_arr['APP_VERSION'] = $app_version->APP_VERSION;


		$user_credentials = "SELECT UNIT_ID,COMPANY_LOCATION_ID,IS_PLANNER,CREDENTIAL_USE_FOR_PLANNER FROM user_passwd Where ID='$user_id' ";
		...
		foreach (sql_select($user_credentials) as $v) {
			...
			$credential_use_for_planner = $v->CREDENTIAL_USE_FOR_PLANNER;
		}
		...
		if ($credential_use_for_planner == 1) {
			$comp_sql = "SELECT ID,COMPANY_NAME from LIB_COMPANY where status_active =1 and is_deleted=0 and CORE_BUSINESS=1 $wher_com_con order by company_name";
			$com_res = $this->db->query($comp_sql)->result();
		}
	...
	function get_item_wise_line_efficiency_info($company_id, $item_id)
	{
		...
		*/
		...
		if ($prod_reso_allo == 1) {
			...
			$data_sql = "SELECT a.ID, a.LOCATION_ID, a.FLOOR_ID, a.LINE_NUMBER, b.ACTIVE_MACHINE, b.PR_DATE, b.MAN_POWER, b.OPERATOR, b.HELPER, b.SMV_ADJUST, b.SMV_ADJUST_TYPE, b.LINE_CHIEF, b.TARGET_PER_HOUR, ROUND(b.WORKING_HOUR) as WORKING_HOUR, c.FROM_DATE, c.TO_DATE, c.CAPACITY as MC_CAPACITY from prod_resource_mst a, prod_resource_dtls b, prod_resource_dtls_mast c where a.id=c.mst_id and a.id=b.mst_id and c.id=b.mast_dtl_id    "; //--and pr_date=$txt_date
	...
	function get_sewing_trend_graph_info($company_id = 0, $serving_company = 0, $location = 0, $floor = 0)
	{
		...
		$tpdArr = array();
		$tsmvArr = array();
		$tpd_data_arr = sql_select("SELECT a.ID, b.PR_DATE, b.MAN_POWER, b.SMV_ADJUST,b.SMV_ADJUST_TYPE, b.TARGET_PER_HOUR, ROUND(b.WORKING_HOUR) as WORKING_HOUR from prod_resource_mst a, prod_resource_dtls b,prod_resource_dtls_mast c where a.id=c.mst_id and c.id=b.mast_dtl_id $companyCon and b.pr_date between '$firstDate' and '$lastDate' and b.is_deleted=0 and c.is_deleted=0 $location_cond $floor_con group by a.id, b.pr_date, b.man_power, b.smv_adjust,b.smv_adjust_type, b.target_per_hour, b.working_hour ");
	...
	function get_daywise_plan_data_info($company_id, $po_id, $txt_date_from = "", $txt_date_to = "")
	{
		...
		$daywise_sql = "SELECT b.IS_OFFDAY, ROUND(b.WORKING_HOUR) as  WORKING_HOUR, a.PO_BREAK_DOWN_ID,a.LINE_ID,a.START_DATE,a.END_DATE,a.PLAN_QNTY,a.FIRST_DAY_OUTPUT,a.INCREMENT_QTY,a.TERGET ,a.COMPANY_ID, a.LOCATION_ID,a.ITEM_NUMBER_ID ,a.OFF_DAY_PLAN,a.ORDER_COMPLEXITY,b.PLAN_DATE,b.PLAN_QNTY,b.PLAN_ID  from ppl_sewing_plan_board a,ppl_sewing_plan_board_dtls b where a.plan_id=b.plan_id and a.is_deleted=0 and a.status_active=1 $po_id_cond $company_cond $plan_date";
	...
	function line_po_day_wise_plan_info($company_id, $po_id = "", $line_id, $plan_id, $order_type)
	{
		...
		$daywise_plan = $this->db->query($daywise_sql)->result();
		$i = 0;
		// print_r($plan_pro_blance_data_arr);
		// die;
		foreach ($daywise_plan as $rows) {
			...
			// print_r($production_line_color_qnty);
			// die;
	...
	function line_info($company_id, $location_id = "0", $floor_id = "0", $user_id = "", $date)
	{
		...
		for ($i = 0; $i <= $count; $i++) {
			...
			foreach ($items_id_arr[$line_res[$i]->LINE_ID] as $value) {
				$item_name_arr[$line_res[$i]->LINE_ID][] = $lib_garments_item[$value];
			}
			//print_r($item_name_arr);die;
			$dataArr[$j]["LINE_ID"] = $line_res[$i]->LINE_ID;
			$dataArr[$j]["USER_IDS"] = $line_res[$i]->USER_IDS;
			$dataArr[$j]["LINE_NAME"] = $line_res[$i]->LINE_NAME;
			$dataArr[$j]["ITEMS"] = implode(",", $item_name_arr[$line_res[$i]->LINE_ID]);
	...
	function create_plan_data_archive($dataArr = array())
	{
		...
		$SEWING_PLAN_BOARD_DTLS_SQL = "SELECT ID,PLAN_ID,PLAN_DATE,PLAN_QNTY,IS_OFFDAY, ROUND(WORKING_HOUR) as  WORKING_HOUR
		...
		$SEWING_PLAN_BOARD_DTLS_ARC_SQL = "SELECT ID,PLAN_ID,PLAN_DATE,PLAN_QNTY,IS_OFFDAY, ROUND(WORKING_HOUR) as  WORKING_HOUR
	...
	function create_plan($plan_obj)
	{
		...
		if ($is_delete != 'delete' && count($temp_plan_id_arr) != 0) {
			...
			$plan_sql = "SELECT a.PO_COMPANY_ID,a.BALANCE_QTY,a.LINE_ID,b.PLAN_ID,b.PO_BREAK_DOWN_ID,b.COLOR_NUMBER_ID FROM PPL_SEWING_PLAN_BOARD a, PPL_SEWING_PLAN_BOARD_POWISE b where a.PLAN_ID=b.plan_id and b.PLAN_ID in(" . implode(',', $temp_plan_id_arr) . ")";
			//echo $plan_sql;die;
			...
			//print_r($blance_plan_data_arr);die;
			$prod_reso_allo = get_resource_allocation_variable($company_id);
			$temp_po_id_arr[0] = 0;
			if ($prod_reso_allo == 1) {
			...
			//print_r($plan_vs_pro_data_arr);die;
		...
		if ($response_obj->Status == 1) {
			...
			foreach ($response_obj->SewingPlanBoard as $sewing_plan_row) {
				...
				$ppl_sewing_plan_board_data = array(
					...
					'PLAN_CUT_QTY_H' => $sewing_plan_row->PLAN_CUT_QTY_H
				);
				...
				if ($sewing_plan_row->RowState == "add" || $sewing_plan_row->MERGE_TYPE == 'Merged') {
					...
					if ($sewing_plan_row->RowState == "update") {
						//$save_plan_data_arr[] = [];
						// print_r($sewing_plan_row->PLAN_ID);
						// die;
	...
	function powise_backup($plan_id_from, $plan_id_to)
	{
		...
		$query_wo_po_break_down = "SELECT ID,PO_NUMBER,PLAN_CUT FROM WO_PO_BREAK_DOWN where STATUS_ACTIVE = 1 and IS_DELETED = 0";
		...
		foreach ($table_sewing_plan_board_powise as $row) {
			...
			$plan_cut_qt .= $plan_cut[$row->PO_BREAK_DOWN_ID] . ',';
			$job_no .= $row->JOB_NO . ',';
			...
			$plan_data[$row->PLAN_ID] = [
				...
				'PLAN_CUT_QTY_H' => substr($plan_cut_qt, 0, -1),
				'UPDATED_BY' => -1,
				'UPDATE_DATE' => $today,
			];
		}
	...
	function Monthly_plan_summary($start_date, $end_date, $company_id, $user_id, $location)
	{
		...
		$query_working_hour = "SELECT applying_period_date, applying_period_to_date,ROUND(working_hour) as working_hour FROM lib_standard_cm_entry WHERE company_id = $company_id and STATUS_ACTIVE =1 AND IS_DELETED = 0";
		...
		foreach ($table_plan as $row) {
			...
			$data_array[$year][$month_digi] = [
				...
				"sah" => number_format($sah[$year][$month_digi], 2),
				"sam" => number_format($sam[$year][$month_digi], 2),
				"no_of_lines" => (int)count($lines[$year][$month_digi]),
				"avg_smv" => number_format($avg_smv[$year][$month_digi], 2),
				...
				"efficiency" => ($avg_efficiency[$year][$month_digi]) ? number_format($avg_efficiency[$year][$month_digi] * 100, 2) : 0,
				"order_value" => number_format($po_total_price[$year][$month_digi], 2),
				"cm_cost" => number_format($cm_cost[$year][$month_digi], 2),
				"cm_value" => number_format($cm_value[$year][$month_digi], 2),
				"approve_status" => 4,
			];
		}
		//print_r($avg_efficiency);die;
	...
	{
		$query_lib_standard_cm_entry = "SELECT COMPANY_ID,LOCATION_ID,ROUND(WORKING_HOUR) as WORKING_HOUR,APPLYING_PERIOD_DATE,UPDATE_DATE FROM LIB_STANDARD_CM_ENTRY WHERE APPLYING_PERIOD_DATE between TO_DATE('$start_date', 'DD/MM/YYYY') AND  TO_DATE('$end_date', 'DD/MM/YYYY') AND COMPANY_ID = $company_id AND STATUS_ACTIVE = 1 AND IS_DELETED = 0 order by APPLYING_PERIOD_DATE DESC ";
