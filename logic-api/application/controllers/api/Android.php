...
';*/
	...
	// function get_roll_info_for_transfer_get()
	// {
	// 	$this->load->model('android_model');
	// 	if (!$this->get('barcode_no')) {
	// 		$this->response('Barcode No Is Required', 400);
	...
	// 	$roll_info = $this->android_model->get_roll_info_for_transfer($this->get('barcode_no'));
	...
	function get_roll_info_for_transfer_get()
	{
		...
		if (!$this->get('room_rack_id')) {
			$room_rack_id = 0;
		}else{
			$room_rack_id = $this->get('room_rack_id');
		}

		$roll_info = $this->android_model->get_roll_info_for_transfer($this->get('barcode_no'),$room_rack_id);


		if (empty($roll_info)) {
			$status = false;
		} else {
			$status = $roll_info["status"];
		}
		$response = array(
			'status' => $status,
			'resultset' => $roll_info["barcode_data"],
			'msg' => $roll_info["msg"]
		);
		...
		if ($response) {
			...
			$this->response(array('msg' => 'Invalid Request'), 404);
		}
	}
	...
	function notification_details_get()
	{
		...
		} else if ($entry_form == 59) { 	//print_r(5);die;
	...
	public function roll_wise_grey_sales_order_to_sales_order_out_POST()
	{
		$response_arr = file_get_contents("php://input");
		//print_r($response_arr);die;
		$this->load->model('android_model');

		$data = $this->android_model->roll_wise_grey_sales_order_to_sales_order_out($response_arr);
	...
	public function roll_wise_grey_sales_order_to_sales_order_in_POST()
	{
		$response_arr = file_get_contents("php://input");
		//print_r($response_arr);die;
		$this->load->model('android_model');

		$data = $this->android_model->roll_wise_grey_sales_order_to_sales_order_in($response_arr);
