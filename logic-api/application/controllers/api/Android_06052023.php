...
class Android extends REST_Controller {
	...
	function version_check_get() { 
		//echo $version = file_get_contents('../ext_resource/smart_track/smart_track_v2.0.1.apk');
	...
	function login_get() { 
		...
		$userInfo = $this->android_model->login($this->get('user_id'), $this->get('pwd'), $this->get('device_id'));
		$status = true;
		if (empty($userInfo)) {
			$status = false;
		}
...
//$response_arr='{"status":true,"mode":"save","UPDATE_ID":0,"data":{"index":{"company_id":"3","app_id":"14263","page_id":"1257","user_id":1}}}';  
	...
	function bundle_wise_sewing_barcode_scan_get() 
	{
		...
		$data = $this->android_model->bundle_wise_sewing_barcode_data($this->get('barcode_no'));
		$status = true;
		if (empty($data)) {
			$status = false;
		}
	...
	function style_wise_operation_list_get(){
		...
		$this->load->model('Android_model');
		$data = $this->Android_model->style_wise_operation_list_model($JOB_NO_MST,$PO_BREAK_DOWN_ID,$ITEM_NUMBER_ID,$COUNTRY_ID,$LINE_ID,$STYLE_NUMBER,$USER_ID);
		$status = true;
		if (empty($receive_info)) {
			$status = false;
		}
	...
	{ 
		$this->load->model('android_model');		 
		if (!$this->get('COLOR_SIZE_ID'))
		{
			$this->response('Color Size Breakdown Id Is Required', 400);
		}
		...
		$result = $this->android_model->defected_details_list_to_rectify($this->get('COLOR_SIZE_ID'), $this->get('LINE_ID'));
		$status = true;
		if (empty($result)) {
			$status = false;
		}
