...
//$page_id = 724;//Asrotex live
$page_id = 869;//Development
...
{	
    ...
    */
    ...
    foreach ($result as $row) {
        ...
        $cost_com_app_data[$row[csf(approved_by)]][$row[csf(job_no)]][is_mail_sent] = $row[csf(is_mail_sent)];
        $mail_send_to_euser_precost_id[$row[csf(id)]] = $row[csf(id)];
    }
    ...
    foreach ($userWiseApprovedJobMailData as $approval_user=>$approvedJobArr) {
        ...
        <table border="1" rules="all" class="rpt_table" id="table_body3" style="font-size:13px;">
            ...
            {          
                ...
                {
                    if($cost_com_app_data[$approval_user][$appJob][is_mail_sent]==2)
                    {
                        $bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
                        $garments_items = "";
		...
		if($to!="" && $htmlBody_approved!="") 
        {
            ...
            /* $query = "UPDATE wo_pre_cost_mst SET is_mail_sent=1 WHERE id in(".implode(",",$mail_send_to_euser_precost_id).")"; 
    ...
    foreach ($un_app_result as $un_app_row) {
        ...
        $cost_com_un_app_data[$un_app_row[csf(approved_by)]][$un_app_row[csf(job_no)]][is_mail_sent] = $un_app_row[csf(is_mail_sent)];
        ...
        $mail_send_un_app_precost_id[$un_app_row[csf(precost_id)]] = $un_app_row[csf(precost_id)];
    }
    
    //var_dump($mail_send_un_app_booking_id);
        ...
        <table border="1" rules="all" class="rpt_table" id="table_body3" style="font-size:13px;">
            ...
            foreach ($final_un_app_jobs_arr as $unAppJob) {
                if($cost_com_un_app_data[$applier_user][$unAppJob][job_no]!='') {               
                    //$user_sequence[$user];
                    if($cost_com_un_app_data[$applier_user][$unAppJob][is_mail_sent]==2)
                    {
                        $bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
                        $garments_items = "";
    ...
    //var_dump($higherAuthority_app_jobs_arr); die;
    ...
    $finalAuthorityJobs = "";  
    foreach ($higherAuthority_app_jobs_arr as $key=>$value) {
        if($finalAuthorityJobs == "") {$finalAuthorityJobs = "'".$value."'";} else {$finalAuthorityJobs .=","."'".$value."'"; }
    }
    ...
    $marchant_approve_precost_id = array();
    foreach ($result as $row) {
        ...
        $marchant_approve_data[$row[csf(inserted_by)]][$row[csf(job_no)]][is_mail_sent] = $row[csf(is_mail_sent)];
        $marchant_approve_precost_id[$row[csf(id)]] = $row[csf(id)];        
    }

    ///var_dump($marchant_approve_data); die;
    ...
    foreach ($marchantigerWiseApprovedJobMailData as $marchantiger=>$approvedJobArr) {
        ...
        <table border="1" rules="all" class="rpt_table" id="table_body3" style="font-size:13px;">
            ...
            foreach ($approvedJobArr as $appJob) {          
				...
				if($marchant_approve_data[$marchantiger][$appJob][job_no] !='') {
                    if($marchant_approve_data[$marchantiger][$appJob][is_mail_sent]==2)
                    {
                        $bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
                        $garments_items = "";
		...
		if($to!="" && $htmlBody_approved!="") {          
            echo $mailSendToMarchentiger = send_mail_mailer( $to, $subject, $message, $from_mail );
        };
    ...
    $finalAuthorityUnAppJobs = "";
    foreach ($higherAuthority_un_app_jobs_arr as $key=>$value) {
        if($finalAuthorityUnAppJobs == "") {$finalAuthorityUnAppJobs = "'".$value."'";} else {$finalAuthorityUnAppJobs .=","."'".$value."'"; }
    }
    //echo "143==".$finalAuthorityUnAppJobs."<br>";
    
    $finalAuthorityUnAppSql = "select a.gmts_item_id,a.job_no,a.buyer_name, a.style_ref_no,b.id as precost_id,b.costing_date,b.inserted_by,b.is_mail_sent,c.id,c.approved_by,c.cost_component_id,c.current_approval_status from wo_pre_cost_mst b,wo_po_details_master a,co_com_pre_costing_approval c where a.job_no=b.job_no and b.id= c.mst_id and a.company_name=$compid and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0  $buyer_id_cond2 and a.job_no IN($finalAuthorityUnAppJobs) and c.current_approval_status in(0) order by a.job_no";
    $un_app_result = sql_select( $finalAuthorityUnAppSql );

    $marchant_un_app_data = array();
    $com_component_status = array();
    $marchant_un_app_precost_id = array();
    $appoved_aby = array();
    foreach ($un_app_result as $un_app_row) {
        ...
        $marchant_un_app_data[$un_app_row[csf(inserted_by)]][$un_app_row[csf(job_no)]][is_mail_sent] = $un_app_row[csf(is_mail_sent)];           
        
        $com_component_status[$un_app_row[csf(id)]][$un_app_row[csf(approved_by)]][$un_app_row[csf(job_no)]][$un_app_row[csf(cost_component_id)]] = $un_app_row[csf(current_approval_status)];           
        $appoved_byArr[$un_app_row[csf(approved_by)]][$un_app_row[csf(job_no)]] = $un_app_row[csf(job_no)];
        
        $marchant_un_app_precost_id[$un_app_row[csf(precost_id)]] = $un_app_row[csf(precost_id)];
    }
    //var_dump($marchant_un_app_data);
    //var_dump($marchant_un_app_precost_id);
        ...
        <table border="1" rules="all" class="rpt_table" id="table_body3" style="font-size:13px;">
            ...
            foreach ($higherAuthority_un_app_jobs_arr as $unAppJob) {
                if($marchant_un_app_data[$marchantiger][$unAppJob][job_no]!='') {               
                    //$user_sequence[$user];
                    if($marchant_un_app_data[$marchantiger][$unAppJob][is_mail_sent]==2)
                    {
                        $bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
                        $garments_items = "";
    ...
    echo "//====================<b>Merchandiser Unapproved mail data End</b> // ==============================//<br>";
     ...
     $con = connect();
     //$query = "UPDATE wo_pre_cost_mst SET is_mail_sent=1 WHERE id in(3435,3460,3557,3560)";
     $query = "UPDATE wo_pre_cost_mst SET is_mail_sent=1 WHERE id in(".implode(",",$mail_send_to_euser_precost_id).")"; 
     $feedback_id = execute_query($query,1);

     $unappquery = "UPDATE wo_pre_cost_mst SET is_mail_sent=1 WHERE id in(".implode(",",$mail_send_un_app_precost_id).")"; 
     $unappfeedback_id = execute_query($unappquery,1);
     
     if( $unappfeedback_id==1 && $feedback_id)
     {
         oci_commit($con); 
     } else {
         oci_rollback($con);
     }
     //echo $unappfeedback_id."hhhh";  
     //echo $feedback_id."hhhh";   
