...
/* -------------------------------------------- Comments -----------------------
  ...
  QC Performed BY   :
  QC Date           :
  Comments          :   URL: dashboard\production\location_wise_finish_stock_display_board.php
 */
...
<script>
  ...
  {   
      ...
      {*/
        ...
        // freeze_window(3);
        http.open("POST","requires/location_wise_finish_stock_display_board_controller_v1.php",true);
               ...
               <div class="menu_icon" id="menu_icon"> Open </div>
                <div id="searchPanel">
                    <div id="searchPanelInner">
                        ...
                        <p>Working Company:</p>
                        <? 
                            echo create_drop_down( "cbo_company_id", "", "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", 0, "load_drop_down('requires/location_wise_finish_stock_display_board_controller_v1', this.value, 'load_drop_down_location_lc', 'location_td' );reset_floor_and_line();load_drop_down( 'requires/location_wise_finish_stock_display_board_controller_v1',this.value, 'load_drop_down_buyer', 'buyer_td' );",0 );
                            ?>
                        <p>Location:</p>
                        <div id="location_td">
                            <? 
...
<script>
    $(document).ready(function(){
        ...
        /*function autoScrollAndCallFunction() 
        {
          ...
          function scroll(speed) 
          {
            if (window.scrollY + window.innerHeight >= document.body.scrollHeight) {
              // Call your custom function here when you reach the end
              yourCustomFunction();
              fn_report_generated();
            } else {
              window.scrollBy(0, scrollSpeed);
              setTimeout(scroll, speed); // Adjust the scroll speed and interval as needed
            }
          }

          
          speed = 10000;
          scroll(speed); // Start scrolling
          setInterval(function(){scroll(speed)}, speed * 2);
        }*/

        function autoScrollTopToBottomAndBottomToTop() {
          ...
          function scroll() {
            ...
            if (scrollingDown && window.scrollY + window.innerHeight >= document.body.scrollHeight) {
              scrollingDown = false; // Change direction when reaching the bottom
              // alert('end');
            } else if (!scrollingDown && window.scrollY <= 0) {
              // alert('start');
              scrollingDown = true; // Change direction when reaching the top
            }
        ...
        autoScrollTopToBottomAndBottomToTop();
