...
{ 	
	...
	if($prod_reso_allo == 1)
	{
		$dataArray = sql_select("SELECT a.id as ID, a.line_number as LINE_NUMBER, a.location_id as LOCATION_ID, a.floor_id as FLOOR_ID, b.pr_date as PR_DATE, b.target_per_hour as TARGET_PER_HOUR, b.working_hour as WORKING_HOUR, b.man_power as MAN_POWER, b.active_machine as ACTIVE_MACHINE, b.operator as OPERATOR, b.helper as HELPER, c.target_efficiency as TARGET_EFFICIENCY from prod_resource_mst a, prod_resource_dtls b, prod_resource_dtls_mast c where a.id=b.mst_id and b.mast_dtl_id=c.id and a.company_id=$cbo_company_id and b.pr_date='$txt_date'");
	  
		foreach($dataArray as $row)
		{
			$prod_resource_array[$row['LOCATION_ID']][$row['FLOOR_ID']][$row['ID']]['target_per_hour']=$row['TARGET_PER_HOUR'];
			$prod_resource_array[$row['LOCATION_ID']][$row['FLOOR_ID']][$row['ID']]['working_hour']=$row['WORKING_HOUR'];
			$prod_resource_array[$row['LOCATION_ID']][$row['FLOOR_ID']][$row['ID']]['active_machine']=$row['ACTIVE_MACHINE'];
			$prod_resource_array[$row['LOCATION_ID']][$row['FLOOR_ID']][$row['ID']]['target_efficiency']=$row['TARGET_EFFICIENCY'];
			$prod_resource_array[$row['LOCATION_ID']][$row['FLOOR_ID']][$row['ID']]['operator']=$row['OPERATOR'];
			$prod_resource_array[$row['LOCATION_ID']][$row['FLOOR_ID']][$row['ID']]['helper']=$row['HELPER'];
			$prod_resource_array[$row['LOCATION_ID']][$row['FLOOR_ID']][$row['ID']]['tpd']=$row['TARGET_PER_HOUR']*$row['WORKING_HOUR'];
	...
	for($h=$hour;$h<$last_hour;$h++)
	{
		...
		{
			$sql.=" sum(CASE WHEN TO_CHAR(a.production_hour,'HH24:MI')>'$bg_hour' and  TO_CHAR(a.production_hour,'HH24:MI')<='$end' and a.production_type in(5) THEN production_quantity else 0 END) AS $prod_hour,
				sum(CASE WHEN TO_CHAR(a.production_hour,'HH24:MI')>'$bg_hour' and  TO_CHAR(a.production_hour,'HH24:MI')<='$end'  and a.production_type=5 THEN alter_qnty else 0 END) AS $alter_hour,
				sum(CASE WHEN TO_CHAR(a.production_hour,'HH24:MI')>'$bg_hour' and  TO_CHAR(a.production_hour,'HH24:MI')<='$end' and a.production_type=5 THEN reject_qnty else 0 END) AS $reject_hour,
				sum(CASE WHEN TO_CHAR(a.production_hour,'HH24:MI')>'$bg_hour' and  TO_CHAR(a.production_hour,'HH24:MI')<='$end' and a.production_type=5 THEN spot_qnty else 0 END) AS $spot_hour,
				sum(CASE WHEN TO_CHAR(a.production_hour,'HH24:MI')>'$bg_hour' and  TO_CHAR(a.production_hour,'HH24:MI')<='$end' and a.production_type=5 THEN replace_qty else 0 END) AS $replace_hour,";
		}
		$first=$first+1;
	}
	...
	$sql.=" FROM pro_garments_production_mst a, wo_po_details_master b, wo_po_break_down c
		where a.production_type in (5) and a.po_break_down_id=c.id and c.job_no_mst=b.job_no and a.status_active=1 and a.is_deleted=0 $company_cond $txt_date_cond 
		group by a.prod_reso_allo, a.production_date, a.location, a.floor_id, a.item_number_id, a.po_break_down_id, a.sewing_line, b.job_no, b.style_ref_no, b.buyer_name, b.set_break_down, c.po_number,c.grouping
	...
	foreach($result as $row)
	{
		...
		{
			$operator          = $prod_resource_array[$row['LOCATION_ID']][$row['FLOOR_ID']][$row['SEWING_LINE']]['operator'];
			$helper            = $prod_resource_array[$row['LOCATION_ID']][$row['FLOOR_ID']][$row['SEWING_LINE']]['helper'];
			$hourly_target     = $prod_resource_array[$row['LOCATION_ID']][$row['FLOOR_ID']][$row['SEWING_LINE']]['target_per_hour'];
			$working_hour      = $prod_resource_array[$row['LOCATION_ID']][$row['FLOOR_ID']][$row['SEWING_LINE']]['working_hour'];
			$target_efficiency = $prod_resource_array[$row['LOCATION_ID']][$row['FLOOR_ID']][$row['SEWING_LINE']]['target_efficiency'];
		    ...
		    $production_data[$row['LOCATION_ID']][$row['FLOOR_ID']][$line_name]["operator"]=$operator;
			$production_data[$row['LOCATION_ID']][$row['FLOOR_ID']][$line_name]["helper"]=$helper;
			$production_data[$row['LOCATION_ID']][$row['FLOOR_ID']][$line_name]["hourly_target"]=$hourly_target;
			$production_data[$row['LOCATION_ID']][$row['FLOOR_ID']][$line_name]["working_hour"]=$working_hour;
			$production_data[$row['LOCATION_ID']][$row['FLOOR_ID']][$line_name]["day_terget"]=$hourly_target*$working_hour;
			$production_data[$row['LOCATION_ID']][$row['FLOOR_ID']][$line_name]["hourly_target"]=$hourly_target;
			$production_data[$row['LOCATION_ID']][$row['FLOOR_ID']][$line_name]["target_efficiency"]=$target_efficiency;			
	...
	foreach($production_data as $location_id => $floor_data)
	{
			...
			<tr style="height: 5%;">
    			...
    			<td rowspan="3" colspan="2" style="color: white; font-size: 2vw; font-weight:bold; width: 33%; height: 75%;">
    				<table style="width: 100%;height: 100%;">
    					...
    					<tr>
    						<td colspan="2" style="border:0; text-align: center; width: 100%" align="center" ><div style="font-size: 2vw; text-align: center; vertical-align: middle;"><span>Planned</span>&nbsp;<span style="font-weight:900;" title="Total Target Efficiency/Total Line Number"><?= number_format($planned/$countLineNumber,2); ?>%</span></div>
    						</td>
    					</tr>
    				</table> 					
    			</td>
