...
{
	$data=explode("_",$data);
	echo create_drop_down( "cbo_location", "", "SELECT id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_drop_down('requires/location_wise_finish_stock_display_board_controller', $data[0]+'_'+this.value, 'load_drop_down_store','store_td');" );
	exit();
}
...
if ($action=="load_drop_down_store")
{
	...
	echo create_drop_down( "cbo_store_name", "", "SELECT a.id, a.store_name from lib_store_location a, lib_store_location_category b where a.id=b.store_location_id and a.company_id='$data[0]' and b.category_type=2 and a.is_textile_store=0 and a.status_active=1 and a.is_deleted=0 $location_cond group by a.id, a.store_name order by a.store_name","id,store_name", 1,"--Select Store--",0,"load_drop_down('requires/location_wise_finish_stock_display_board_controller', $data[0]+'_'+$data[1]+'_'+this.value, 'load_drop_down_floor', 'floor_td');");
	exit();
}
...
if($action=="report_generate")
{
	...
	if($buyer_id==0) $buyer_id_cond=""; else $buyer_id_cond=" and c.BUYER_ID=$buyer_id";    
	...
	$store_sql="SELECT A.ID, A.STORE_NAME from lib_store_location a, lib_store_location_category b where a.id= b.store_location_id and a.company_id=$company_id and b.category_type=2 and a.status_active=1 and a.is_deleted=0 and a.is_textile_store=0 $location_cond";
	            			...
	            			foreach ($floor_data_id as $str_ref => $row) 
	            			{
								...
								if($stock_qnty!=0)
								{
				                	...
				                	<tr height="<?=$tr_height;?>" bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i;?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
										...
										<td width="150" title="<?=$prod_id;?>"><p <?=$font_size;?>><?=$fabric_type[0];?></p></td>
										<td width="150" title="<?=$color_id;?>"><p <?=$font_size;?>><?=$colorArr[$color_id];?></p></td>
		...
		$(document).ready(function () 
		{
	        ...
	        function autoScrollWithinElement() {
			  ...
			  /*function callCustomFunction() {
			    // Implement your custom function to process data or perform any action
			    console.log("Reached the end of the container, now calling your custom function.");
			  }*/
