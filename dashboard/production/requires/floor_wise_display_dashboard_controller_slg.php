...
if($action==='report_generate')
{
	...
	where a.id=d.mst_id and  a.po_break_down_id=c.id and c.job_id=b.id and a.production_type in (5) and a.status_active=1 and a.is_deleted=0and a.serving_company=$cbo_company_id and a.location=$cbo_location_id and a.floor_id=$cbo_floor_id and a.production_date='$txt_date'
	...
	// echo $avg_count_hour.'='.$time; 
	$efficiency=(($totalProdQnty*$smv_average)/(($operator+$helper)*$time*60))*100;  
	// echo $totalProdQnty.'*'.$smv_average.'/'.$operator.'+'.$helper.'*'.$avg_count_hour;
	...
	<div style="width:100%; height: <? echo $max_height; ?>px; font-family: Arial, Helvetica, sans-serif; display: none;" id="firsttime">
		<table style="width: 100%; height: 100%; background-color: #262626; overflow: hidden; clear: both; margin-left: 0; float: left;" class="rpt_table" rules="all" cellpadding="0" cellspacing="0">
          ...
          <span><img class="flip" src="../../images/logic/slg_logo.png" id="logininfo" width="70"></span>  
    			...
    			<table style="width: 100%;height:100%;">
    					...
    					<tr style="height:30%">	
    						<td colspan="3" style="border:1;vertical-align:middle;">
    							...
    							<input type="hidden" id="efficiency" value="<?= fn_number_format($efficiency,0,'','','0'); ?>">
                             	</p>
    						</td>    						
    					</tr>
	...
	<div style="width:100%; height: <? //echo $max_height; ?>px; font-family: Arial, Helvetica, sans-serif; display: none; background-color: #262626;" id="secondtime">
    	<table style="width: 100%; background-color: #262626; overflow: hidden; clear: both; margin-left: 0; float: left;" class="rpt_table" border="0" rules="all" cellpadding="0" cellspacing="0">
    		<tr style="height: 10%;">    			
				<td class="verticalMiddle textCenter" style="width:6%;"><span><img class="flip" src="../../images/logic/slg_logo.png" id="logininfo" width="70"></span></td>
    			<td colspan="5" class="fontSizeHeader textCenter verticalMiddle" style="width: 32%;"><u>FLOOR</u><br><marquee width=85% behavior="scroll" scrolldelay="200" direction="left" onmouseover="this.stop();" onmouseleave="this.start();"class="margin-top-8" ><? echo $cbo_floor_name; ?></marquee></td>	
			...
			foreach ($production_data as $line_name => $row) 
			{
				...
				//$line_wise_efficiency=(($row['totalProdQnty']*$line_wise_smv_avg)/(($row['operator']+$row['helper'])*$time*60))*100;
				$line_wise_efficiency=(($row['totalProdQnty']*$line_wise_smv_avg)/(($row['man_power'])*$time*60))*100;
				// echo $row['totalProdQnty'].'*'.$line_wise_smv_avg.'/'.$row['operator'].'+'.$row['helper'].'*'.$time.'<br>';
				$second_trend=($row['totalProdQnty']/$avg_count_hour)*$working_hour;
				//$second_current_target=floor($row['current_target']*$smv_average);
				$second_current_target=floor($row['current_target']*$avg_count_hour);
