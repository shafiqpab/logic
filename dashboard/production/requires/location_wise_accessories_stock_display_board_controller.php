...
{
	$data=explode("_",$data);
	echo create_drop_down( "cbo_location", "", "SELECT id,location_name from lib_location where company_id='$data[0]' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", 0, "load_drop_down('requires/location_wise_accessories_stock_display_board_controller', $data[0]+'_'+this.value, 'load_drop_down_store','store_td');" );
	exit();
}
...
if ($action=="load_drop_down_store")
{
	...
	echo create_drop_down( "cbo_store_name", "", "SELECT a.id, a.store_name from lib_store_location a, lib_store_location_category b where a.id= b.store_location_id and a.company_id='$data[0]' and b.category_type=4 and a.status_active=1 and a.is_deleted=0 $location_cond group by a.id, a.store_name order by a.store_name","id,store_name", 1,"--Select Store--",0,"load_drop_down('requires/location_wise_accessories_stock_display_board_controller', $data[0]+'_'+$data[1]+'_'+this.value, 'load_drop_down_floor', 'floor_td');");
	exit();
}
...
if($action=="report_generate")
{
	...
	$scroll_speed_control=str_replace("'","",$scroll_speed_control);
	
	if($cbo_location_id==0) $location_cond=""; else $location_cond=" and a.location_id=$cbo_location_id";
	if($cbo_store_name==0) $store_cond=""; else $store_cond=" and b.store_id=$cbo_store_name";
	if($cbo_floor_id==0) $floor_cond=""; else $floor_cond=" and b.floor_id=$cbo_floor_id";
	if($cbo_room_id==0) $room_cond=""; else $room_cond=" and b.room=$cbo_room_id";
	if($cbo_rack_id==0) $rack_cond=""; else $rack_cond=" and b.rack=$cbo_rack_id";
	if($cbo_shelf_id==0) $shelf_cond=""; else $shelf_cond=" and b.self=$cbo_shelf_id";
	if($buyer_id==0) $buyer_id_cond=""; else $buyer_id_cond=" and e.buyer_name=$buyer_id";    
	...
	$store_sql="SELECT a.ID, A.STORE_NAME from lib_store_location a, lib_store_location_category b where a.id= b.store_location_id and a.company_id=$company_id and b.category_type=4 and a.status_active=1 and a.is_deleted=0 $location_cond";
	$store_sql_result=sql_select($store_sql);

	if(empty($store_sql_result))
	{
		echo "<p style='font-size:20px;color:red;', align='center'>Accessories Store Not Found.<p/>";
		disconnect($con);
		die;		
	}

	$accessories_store_id_arr=array();
	foreach ($store_sql_result as $key => $row) 
	{
		$accessories_store_id_arr[$row[ID]]=$row[ID];
	}
	$all_store_id=implode(",", $accessories_store_id_arr);
	
	/*===================================================================================== /
	/								get accessories data											/
	/===================================================================================== */
	$sql="SELECT A.ITEM_GROUP_ID, B.COMPANY_ID, B.STORE_ID, B.FLOOR_ID, B.ROOM, B.RACK, B.SELF, B.BIN_BOX, B.PROD_ID, B.MST_ID, B.ID AS TRANS_ID, B.TRANSACTION_TYPE, B.CONS_QUANTITY, C.QUANTITY, C.PO_BREAKDOWN_ID, C.ENTRY_FORM, E.buyer_name as BUYER_ID
	from product_details_master a, inv_transaction b, order_wise_pro_details c, wo_po_break_down d, wo_po_details_master e 
	where a.id=b.prod_id and b.id=c.trans_id and c.po_breakdown_id=d.id and d.job_no_mst=e.job_no and c.entry_form in(24,25,49,73,78,112) and b.ITEM_CATEGORY=4 and a.entry_form=24 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 AND c.quantity > 0 and b.COMPANY_ID=1 and b.store_id in($all_store_id) $store_cond $floor_cond $room_cond $rack_cond $shelf_cond $buyer_id_cond and B.RACK!=0 and B.SELF!=0";
	// echo $sql;die;
	$sql_resqlt=sql_select($sql);
	foreach ($sql_resqlt as $key => $row) 
	{
		if ($trans_id_check[$row["TRANS_ID"]]=="") // recv mst_id > and b.mst_id=78070 and B.PROD_ID=69002
		{
			$ref_str = $row["ROOM"]."*".$row["RACK"]."*".$row["SELF"]."*".$row["ITEM_GROUP_ID"]."*".$row["BUYER_ID"];
        ...
        <table width="<? echo $tbl_width;?>" cellpadding="1" cellspacing="0" border="0" rules="all" id="table_body" align="left" style="border-collapse:seperate;border:.001em solid #444;">
			<thead>
				<tr height="<?=$tr_height;?>">
					...
					<th width="150"><p <?=$font_size;?>>Item Group</p></th>
					<th width="150"><p <?=$font_size;?>>Stock</p></th>
				</tr>
			</thead>
		</table>
		<div style="width:<?= $tbl_width; ?>px;  max-height:670px; overflow-x:hidden;" id="scroll_body">
	            			...
	            			foreach ($floor_data_id as $str_ref => $row) 
	            			{
								...
								if($stock_qnty!=0)
								{
				                	...
				                	<tr height="<?=$tr_height;?>" bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i;?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
										...
										<td width="150" title="<?=$item_group_id;?>"><p <?=$font_size;?>><?=$trim_group[$item_group_id];?></p></td>
	...
	<script type="text/javascript">
		...
		{
			var scrollSpeedControl='<? echo $scroll_speed_control; ?>';
			// alert(scrollSpeedControl);
			document.querySelector('#scroll_body').addEventListener('scroll', scrollTrigger);
	        ...
	        function autoScrollWithinElement() {
			  ...
			  function scroll() {
			    ...
			    setTimeout(scroll, scrollSpeedControl); // Adjust the scroll speed and interval as needed
			  }
