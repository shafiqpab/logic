...
if($action==='report_generate')
{
	...
	//$query = "SELECT * FROM pro_batch_create_mst WHERE INSERT_DATE >= TO_DATE('$formattedStartDate', 'YYYY-MM-DD HH24:MI:SS') AND INSERT_DATE < TO_DATE('$formattedEndDate', 'YYYY-MM-DD HH24:MI:SS')";
	//$query = "SELECT * FROM pro_batch_create_mst WHERE INSERT_DATE < $insert_date_cond order by INSERT_DATE desc";
	//echo $query;die;
	...
	//$date_cond=" and f.PROCESS_END_DATE between '12-Nov-2023' and '13-Nov-2023'";
	...
	where f.batch_id=a.id and a.id=f.batch_id and f.id=c.mst_id and f.MACHINE_ID=d.id and a.entry_form=0
	...
	$single_part_dyeing=0;$double_part_dyeing=0;$bulk_totQty=0;$sample_totQty=0;$others_total_qty=0; 
	...
	foreach ($prod_sql_result as $key => $row) 
	{
		if ($row['LOAD_UNLOAD_ID']==2) 
		{
			if ($row['RESULT']==1) // Shade Match Only
			{
				...
				{
					$shiftA_totQnty += $row['PRODUCTION_QTY'];
					$bulk_shiftA_mc_no_count++;
				}
				if ($row['SHIFT_NAME']==2) // Shift B
				{
					$shiftB_totQnty+=$row['PRODUCTION_QTY'];
				...
				if ($row['DOUBLE_DYEING']==2) // Multi Dyeing No
				{
	...
	// echo "<pre>";print_r($ready_for_dyeing_check_arr);die;
	// echo $used_bulk_total_mc_count.'==';

	// READY FOR DYEING
	$batch_sql="SELECT A.ID, a.INSERT_DATE, B.BATCH_QNTY FROM pro_batch_create_mst a, pro_batch_create_dtls b
	WHERE  a.INSERT_DATE <= $insert_date_cond and a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form in(0,36) and a.batch_against in(1,11,2,3)
	order by a.INSERT_DATE desc";//--and a.batch_no='23-Today-Batch'
	$batch_sql_result=sql_select($batch_sql);
	// echo $batch_sql;die;
	$today_number_of_batch=0;$previous_number_of_batch=0;
	$today_batch_qty=0;$previous_batch_qty=0;
	foreach ($batch_sql_result as $row) 
	{
		if ($ready_for_dyeing_check_arr[$row['ID']]=="") 
		{
			// insert_date update > UPDATE pro_batch_create_mst SET insert_date = TO_DATE('2023-11-15 11:29:00', 'YYYY-MM-DD HH24:MI:SS') WHERE id=22368;
			// echo strtotime($formattedStartDate) .'>='. strtotime($row[csf('INSERT_DATE')]).'<br>';
			//echo strtotime($formattedEndDate) .'<='. strtotime($row[csf('INSERT_DATE')]).'<br>';
			//echo $formattedStartDate.'='.$row[csf('INSERT_DATE')].'<br>';
			//echo $formattedEndDate.'='.$row[csf('INSERT_DATE')].'<br>';
			if ( strtotime($formattedStartDate) <= strtotime($row[csf('INSERT_DATE')]) && strtotime($formattedEndDate) >= strtotime($row[csf('INSERT_DATE')]) ) // Today
			{
			...
			else if( strtotime($formattedStartDate) > strtotime($row[csf('INSERT_DATE')]) )
			{
			...
			<div class="right_section">
				<div>
					<table style="width: 100%; border-collapse: collapse; text-align: center;">
						...
						<tr>
							<td style="height: 178px; font-size: 2.5vw;" class="nr_font_css borderTop"><? echo fn_number_format($bulk_total_mc_count-$used_bulk_total_mc_count,2,'.','','0'); ?></td>
							<td style="height: 178px; font-size: 2.5vw;" class="nr_font_css borderTop"><? echo fn_number_format($sample_total_mc_count-$used_sample_total_mc_count,2,'.','','0'); ?></td>
						</tr>					  
					</table>
				</div>
				<br>
				...
				<div style="height: 44.5%;">
					<table class="reject_qty_table_css">
					  ...
					  <tr>
					    <td class="nr_font_css borderTop"><? echo fn_number_format($previous_number_of_batch,2,'.','','0'); ?></td>
					  ...
					  <tr>
					    <td class="nr_font_css borderTop"><? echo fn_number_format($today_number_of_batch,2,'.','','0'); ?></td>
		...
		<div class="foot">
			<table width="74.7%" style="float: left;" cellpadding="0" cellspacing="0" border="1" rules="all" >
				...
				<tr>
					...
					<td style="text-align:center; font-size: 1.5vw; font-weight: bold;"><? echo fn_number_format($others_total_qty[1],2,'.','','0'); ?></td>
				</tr>
				<tr>
