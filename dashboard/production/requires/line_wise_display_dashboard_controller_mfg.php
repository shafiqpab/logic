...
{ 	
	...
	if ($order_ids != "")
	{
		...
		$sql_defect = "SELECT TO_CHAR(a.production_hour,'HH24') as production_hour, b.defect_qty as defect_qty,b.INSERT_DATE  
		...
		and c.status_active=1 group by TO_CHAR(a.production_hour,'HH24'), b.defect_qty,b.INSERT_DATE";// and c.production_qnty=0 and c.reject_qty=0
	...
	if ($order_ids != "")
	{
		...
		$sql_defect_count = "SELECT * from (SELECT x.DEFECT_TYPE_ID,x.DEFECT_POINT_ID,sum(x.DEFECT_QTY) as POINT_COUNT
		from(
		SELECT B.DEFECT_TYPE_ID,B.DEFECT_POINT_ID,B.DEFECT_QTY,b.INSERT_DATE 
		FROM pro_garments_production_mst a, pro_gmts_prod_dft b,pro_garments_production_dtls c WHERE a.id=b.mst_id and b.dtls_id=c.id and b.production_type=5 and b.po_break_down_id in($order_ids)  and a.serving_company=$cbo_company_id and a.location=$cbo_location_id and a.floor_id=$cbo_floor_id and a.sewing_line in($lineIds) and a.production_date='$txt_date' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 
		group by b.defect_type_id,b.defect_point_id,B.DEFECT_QTY,b.INSERT_DATE 
		) x group by x.DEFECT_TYPE_ID,x.DEFECT_POINT_ID order by point_count desc)  where rownum<=3";// and TRUNC(b.insert_date)='6/June/2023' // and c.production_qnty=0 and c.reject_qty=0
	...
	$dhu_title = "(Total Defect Count / (Good + Rectified + Reject)) X 100 \n\n".$totDefectQnty . "/(" . ($totalProdQnty+$totalRejectQnty) .")" . "*100"; 
	//$dhu = ($totalAlrSpotRejectQnty/($totalProdQnty+$totalReplaceQnty))*100; //(qcQty/total_qty)*100-100;
	$dhu = ($totDefectQnty/($totalProdQnty+$totalRejectQnty))*100; //(qcQty/total_qty)*100-100;
			...
			<tr style="height: 18%;">
    			<td colspan="3" style="color:white; font-size: 2vw; font-weight:bold; text-align:center; vertical-align: middle;">
	        		...
	        		if ($current_target<=$totalProdQnty)
	        		{
    					...
    					<div style="font-size:6vw; height: 80%; font-weight:900; text-align: right; "><span style="width: 80%; text-align: right;"><?=  ($totalProdQnty+$totalRejectQnty); ?></span><span style="width: 20%; text-align: right; color: green; font-weight:900;"></span></div>
    					<?
    				}
    				else
    				{
    					...
    					<div style="font-size:6vw; height: 80%; font-weight:900; text-align: right; "><span style="width: 80%; text-align: right;"><?=  ($totalProdQnty+$totalRejectQnty); ?></span><span style="width: 20%; text-align: right;  font-weight:900;"></span></div>
    					<?
    				}
    				?>	
    			</td>
    	...
    	<table style="width: 100%; background-color: #262626; overflow: hidden; clear: both; margin-left: 0; float: left;" class="rpt_table" border="0" rules="all" cellpadding="0" cellspacing="0">
			...
			foreach($production_data as $line => $row)
			{
				for($h=$hour; $h<=$last_hour; $h++)
				{
					...
					$hourlyDhu = ($row[$prod_hour]>0) ? ($totalAlterSpotRejectQty/$row[$prod_hour])*100 : 0; 
					$cumulity_hourlyDhu = ($tot_good_qty>0) ? ($tot_defect_qty/$tot_good_qty)*100 : 0;
