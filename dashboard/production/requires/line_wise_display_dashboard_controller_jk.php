...
{ 	
	...
	$sql.=" FROM pro_garments_production_mst a,pro_garments_production_dtls d, wo_po_details_master b, wo_po_break_down c
		where a.id=d.mst_id and  a.po_break_down_id=c.id and c.job_id=b.id and a.production_type in (5) and a.status_active=1 and a.is_deleted=0 $company_cond $floor_cond $location_cond $line $txt_date_cond
	...
	if ($order_ids != "")
	{
		...
		WHERE a.id=b.mst_id and b.dtls_id=c.id and b.production_type=5  and b.po_break_down_id in($order_ids) $company_cond $floor_cond $location_cond $line $txt_date_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
	...
	$varriance=($current_target-($totalProdQnty+$totalReplaceQnty));
	...
	$dhu = (($totalAlrSpotRejectQnty)/($totalProdQnty+$totalAlterQnty+$totalSpotQnty+$totalReplaceQnty))*100; //(qcQty/total_qty)*100-100;
	$rft = (($totalGoodAlrSpotRejectQnty-$totalAlrSpotQnty)/$totalGoodAlrSpotRejectQnty)*100;
	$efficiency=((($totalProdQnty+$totalReplaceQnty)*$smv_average)/(($operator+$helper)*$total_count_hour*60))*100;   
	?>
	...
	<div style="width:100%; height: <? echo $max_height; ?>px; font-family: Arial, Helvetica, sans-serif; display: none;" id="firsttime">
		<table style="width: 100%; height: 100%; background-color: #262626; overflow: hidden; clear: both; margin-left: 0; float: left;" class="rpt_table" rules="all" cellpadding="0" cellspacing="0">
          ...
          <span><img class="flip" src="../../images/logic/mfg_logo.png" id="logininfo" width="70"></span>  
				...
				<td style="width:17%;color:#0FF;font-size:1.8vw;font-weight:bold;text-align:center;vertical-align:middle;"><u>LINE</u><marquee width=85%  direction="left" onmouseover="this.stop();" onmouseleave="this.start();"class="margin-top-8" ><? echo $cbo_line_name; ?></marquee></td>
    			...
    			<tr style="height:30%">	
                           ...
                           <div class="marquee">
                			<div class="marqueeText">
								...
								?>
                                <img src="../../<? echo $image_location; ?>" width="120" height="40">  
                                <?
                ...
                <span class="rounded-bg" style="font-weight: bold;"title="Reject Qty: <? echo $totalRejectQnty;?>"><?=fn_number_format(($totalRejectQnty/$totalProdQnty)*100,2,'.','','0.00');?>% <span style="border-left:2px solid; padding-left:5px;"> <? echo $totalRejectQnty;?></span></span>
					 ...
					 $rectifiedPer=($totalReplaceQnty/$totalAlrSpotQnty)*100;
						...
						{			    				
							?>
								<span>RECTIFIED</span>
                                <span class="rounded-bg" style="font-weight: bold;"title="Rectify Qty: <? echo $rectifiedPer;?>"><?=fn_number_format(($totalReplaceQnty/$totalAlrSpotQnty)*100,0,'.','','0');?>,% <span style="border-left:2px solid; padding-left:5px;"> <? echo $totalReplaceQnty;?></span></span>
                    		<? 
                         }
                         else
						{
							?>
								<span>RECTIFIED</span>
                                <span class="rounded-bg" style="font-weight: bold;"title="Rectify Qty: <? echo $rectified_title;?>"><?=fn_number_format(($totalReplaceQnty/$totalAlrSpotQnty)*100,2,'.','','0.00');?>% <span style="border-left:2px solid; padding-left:5px;"> <? echo $totalReplaceQnty;?></span></span>
							<?
						}
						?>
                    </div>
    	...
    	<table style="width: 100%; background-color: #262626; overflow: hidden; clear: both; margin-left: 0; float: left;" class="rpt_table" border="0" rules="all" cellpadding="0" cellspacing="0">
			...
			foreach($production_data as $line => $row)
			{
				for($h=$hour; $h<=$last_hour; $h++)
				{
					...
					$hourlyDhu = ($totalAlterSpotRejectQty/$row[$prod_hour])*100; //(qcQty/total_qty)*100-100;
					...
					<tr id="tr_<?= $h; ?>"style="height: 7%;" >
						...
						{			    				
							?>
							<td style="width: 8%; color: white; font-size: 2vw; font-weight:bold;text-align:right;padding-right:20px;vertical-align: middle;"><span style="color: red;"><?= number_format($row[$prod_hour],0); ?></span></td>
						...
						{
							?>
							<td style="width: 8%; color: white; font-size: 2vw; font-weight:bold; text-align:right; padding-right:20px; vertical-align: middle;"><span style="color:#0bc300;"><?=number_format($row[$prod_hour],0); ?></span></td>
							<?
						}
    	...
    	<table style="width: 100%; background-color: #262626; overflow: hidden; clear: both; margin-left: 0; float: left;" class="rpt_table" border="0" rules="all" cellpadding="0" cellspacing="0">
            ...
            <tr>
            	...
            	<td style="width:3%;vertical-align: middle;" align="center">
          				<span><img src="../../images/logic/auko_group_logo_new.png" id="logininfo" width="65"></span>  
            	</td>
