...
http://localhost/platform-v3.5/production/reports/all_company_wise_display_dashboard.php?url=company name/
http://118.179.205.12/production/reports/all_company_wise_display_dashboard.php?url=all company/
*/
require_once('../../includes/common.php');
$url_title = ltrim($_SERVER['PHP_SELF'], '/');
//$url_com_name = rtrim($_GET['url'],'/');
//echo $url;die;
...
<script> 
    ...
    {  
        ...
        var data="action=report_generate"+"&max_height='"+max_height+"'"+"&txt_date='"+txt_date+"'";
        //alert(data);
        http.open("POST","requires/all_company_wise_display_dashboard_controller.php",true);
    ...
    {    
        if(http.readyState == 4) 
        {
            ...
            var totalData = $("#total_row").val()*1;
            //alert(totalData);
            totalData=totalData/10;

            var firstAfter=35;
            var secondAfter=5;
            var totalSecondToAdd=(firstAfter+(secondAfter*totalData));

            var currentDate = new Date();
            var countDownDate = new Date(currentDate.getTime() + totalSecondToAdd*1000);
            var addTime=0;
            ...
            var x = setInterval(function() {
                ...
                else if(seconds==(totalSecondToAdd-(firstAfter+addTime)) ) //&& distance  > 0
                {
                    var txt_date = '<? echo date('d-M-Y'); ?>';
                    var data="action=report_generate"+"&max_height='"+max_height+"'"+"&txt_date='"+txt_date+"'";
                    $.ajax({
                        type:'POST',
                        url:'requires/all_company_wise_display_dashboard_controller.php',
                        data:data,

                        success:function(response){
                            numSuccessfullAjaxCallCount++;
                            //alert(numSuccessfullAjaxCallCount);
                            k=numSuccessfullAjaxCallCount*10;
                            //alert(k);
                            $('#report').html(response);
                            $("#secondtime").css('display','block');
                            total_row=$("#total_row").val()*1;

                            if (numSuccessfullAjaxCallCount==1) j=numSuccessfullAjaxCallCount;
                            else j=j+10;
...
<script type="text/javascript">max_height=screen.height-142;</script>
