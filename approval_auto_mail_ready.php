    ...
    foreach($sequence_wise_user_id_arr as $userSequence=>$userid)
    {
        ...
        if($action == "report_generate")
        {
            ...
            */
            ...
            if($approval_type==2)
            {
                ...
                if($user_sequence_no==$min_sequence_no)
                {
                    ...
                    $sql="select b.id,a.job_no_prefix_num,$year_cond,a.job_no, a.buyer_name, a.style_ref_no,b.costing_date,'0' as approval_id, b.partial_approved,b.inserted_by from wo_pre_cost_mst b,  wo_po_details_master a where a.job_no=b.job_no and a.company_name=$company_name and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and b.ready_to_approved=1 and b.partial_approved=2 $buyer_id_cond $buyer_id_cond2 $date_cond";
                }
                else if($sequence_no == "")
                {
                    $buyer_ids=$buyer_ids_array[$user_id]['u'];
         ...
         group_concat(case when bypass=2 and buyer_id<>'' then buyer_id end) as buyer_ids from electronic_approval_setup where company_id=$cbo_company_name and page_id=$menu_id and sequence_no<$user_sequence_no and is_deleted=0";
                    ...
                    echo $pre_cost_id=implode(",",$result);
                    ...
                    $sql="select b.id,a.job_no_prefix_num,$year_cond,a.job_no, a.buyer_name, a.style_ref_no,b.costing_date,0 as approval_id,
                    ...
                    and b.is_deleted=0 and b.ready_to_approved=1 and b.partial_approved in (0,2) $buyer_id_cond $date_cond $pre_cost_id_cond $buyerIds_cond $buyer_id_cond2";
                    if($pre_cost_id!="")
                    {
                        $sql.=" union all
                        select b.id,a.job_no_prefix_num,$year_cond,a.job_no, a.buyer_name, a.style_ref_no,b.costing_date,0 as approval_id,
                        b.approved,b.inserted_by 
                        from wo_pre_cost_mst b,wo_po_details_master a
                        where  a.job_no=b.job_no and a.company_name=$company_name  and a.is_deleted=0 and a.status_active=1 and b.status_active=1
                        and b.is_deleted=0 and b.ready_to_approved=1 and b.partial_approved=1 and (b.id in($pre_cost_id)) $buyer_id_cond $buyer_id_cond2 $date_cond";
                    }
                ...
                else
                {
                    ...
                    $sql="select b.id,a.job_no_prefix_num,a.job_no, a.buyer_name, a.style_ref_no,b.costing_date, b.partial_approved,b.inserted_by,
            ...
            else
            {
                ...
                $sql="select b.id,a.job_no_prefix_num,$year_cond,a.job_no, a.buyer_name, a.style_ref_no,b.costing_date, b.partial_approved,b.inserted_by,
