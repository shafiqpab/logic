...
<script>
	...
	function fnc_chemical_dyes_issue_entry(operation)
	{
		...
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][250]);?>'){
				if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][250]);?>','<? echo implode('*',$_SESSION['logic_erp']['field_message'][250]);?>')==false)
				{				
					return;
				}
			}
	...
	function open_reqpopup()
	{
		...
		emailwindow.onclose=function()
		{
			...
			var recipe_id=ereqsn_data[2];
			if(reqnId!="")
			{
				reset_form('','list_container_recipe_items','','','','');
				get_php_form_data(reqnId, "populate_data_from_req_data", "requires/chemical_dyes_issue_controller");
				load_drop_down( 'requires/chemical_dyes_issue_controller', company+"**"+reqnId, 'load_drop_down_sub_process', 'sub_process_td');
				var store_name = $("#cbo_store_name").val();

				fn_sub_process_enable(store_name);
                    ...
                    <table  width="950" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                        ...
                        <tr>
                         ...
                         <tr>
                            ...
                            <td>
                            <? 
                            echo create_drop_down( "cbo_issue_purpose", 170, $general_issue_purpose,"", 1, "-- Select Purpose --", 77, "", "","1,5,15,37,38,61,63,65,69,77,81,82");
                            ?>
                            </td>
