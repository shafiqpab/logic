...
if ($action == "create_requisition_search_list_view") 
{
   ...
   b.buyer_buyer from subcon_ord_mst a, subcon_ord_dtls b where a.embellishment_job=b.job_no_mst and a.entry_form=204  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	...
	$buyer_po_arr=array(); $po_id_arr=array();
	$order_sql ="select a.job_no_prefix_num,b.id,b.buyer_po_no,b.buyer_style_ref, b.job_no_mst,b.order_no, b.buyer_buyer from subcon_ord_mst a, subcon_ord_dtls b where a.embellishment_job=b.job_no_mst and a.id=b.mst_id and a.entry_form='204' $search_com_cond";
//echo $order_sql; die;
	...
	if($search_com_cond!='' || $job_cond!=''){
		...
		$subcon_cond=" and b.job_no in (select job_no from tmp_job_no where userid=$user_id and entry_form=220) ";
	}
	...
	/*	$ord_sql = "SELECT b.id,a.subcon_job,a.aop_reference,b.order_no,b.buyer_style_ref,b.buyer_po_no from subcon_ord_mst a ,subcon_ord_dtls b where  company_id =$company_id $aop_ref_cond and a.entry_form=278 and  a.subcon_job=b.job_no_mst and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0";
	...
	$sql = "SELECT a.id, a.requ_no, a.requ_prefix_num, $year_field, a.company_id, a.requisition_date, a.recipe_id, a.order_ids as order_id, a.job_no, a.buyer_po_ids as buyer_po_id ,a.color_id from dyes_chem_issue_requ_mst a, pro_recipe_entry_mst b where a.recipe_id=b.id and  a.company_id=$company and a.entry_form=221 and a.status_active =1 and a.is_deleted=0 $date_cond $search_field_cond $spo_idsCond $year_cond $po_idsCond $subcon_cond order by id DESC ";//$search_field_cond // 
	//echo $sql;
	...
	<div style="width:850px; max-height:250px; overflow-y:scroll" id="list_container_batch" align="left">
		...
		foreach ($result as $row) 
		{
			...
			foreach($order_id as $po_id)
			{
				...
				if($order_no=="") $order_no=$buyer_po_arr[$po_id]['order_no']; else $order_no.=','.$buyer_po_arr[$po_id]['order_no'];
...
if ($action == "populate_data_from_req_data") 
{
	...
	$recipe_arr = return_library_array("SELECT id, recipe_no from pro_recipe_entry_mst where status_active =1 and is_deleted=0", "id", "recipe_no");
	$recipe_arr = array();
	$recipe_sql ="SELECT id, recipe_no,recipe_for from pro_recipe_entry_mst where status_active =1 and is_deleted=0";
	...
	foreach ($sql as $row) 
	{
		...
		echo "document.getElementById('txt_req_id').value 		= '" . $row[csf("id")] . "';\n";
		...
		echo "document.getElementById('hidden_buyer_po_id').value = '" . $row[csf("buyer_po_id")] . "';\n";
		...
		echo "document.getElementById('list_container_recipe_items').innerHTML 		= '';\n";

		//echo  "load_drop_down( 'requires/chemical_dyes_issue_controller', '".$row[csf("company_id")]."_".$row[csf("id")]."', 'load_drop_down_sub_process', 'sub_process_td' );\n";
		//echo "fn_sub_process_enable(" . $row[csf("store_id")] . ");\n";
...
if($action=="create_mrr_search_list_view")
{
	...
	else
	{
		...
		//echo "SELECT a.issue_number_prefix_num,a.req_no, a.issue_number, a.company_id, a.issue_date, a.issue_basis, a.issue_purpose, a.batch_no, a.issue_date, a.id, $year_id, a.is_posted_account from inv_issue_master a where a.company_id=$company and a.entry_form=250 $sql_cond  $batch_cond $year_cond and a.status_active =1 and a.is_deleted=0 order by a.id "; die;
	...
	?>
	<br/>
    <table align="center" cellspacing="0" width="920"  border="1" rules="all" class="rpt_table"  >
 ...
 <div style="width:930px;max-height:300px; padding-left:18px; overflow-y:scroll" id="scroll_body">
 	<table align="center" cellspacing="0" width="920"  border="1" rules="all" class="rpt_table"  id="list_view" >
		...
		?>
         	<tr bgcolor="<? echo $bgcolor; ?>" onClick='js_set_value("<? echo $inf[csf("issue_number")]."_".$inf[csf("id")]."_".$inf[csf("issue_basis")]."_".$inf[csf("issue_purpose")]."_".$inf[csf("is_posted_account")];?>")' style="cursor:pointer">
                ...
                <td width="100" align="center"><?php echo $inf[csf("req_no")]; ?></td>
...
{
	$sql = sql_select("SELECT location_id, issue_date, issue_basis, req_no, req_id, issue_purpose, company_id, loan_party, lap_dip_no, batch_no, order_id, sub_order_ids, style_ref, store_id, buyer_job_no, is_posted_account, lc_company, floor_id, machine_id, remarks from inv_issue_master where id=$data and entry_form=250 and status_active =1 and is_deleted=0");
	$all_order_id="";$all_subcon_order="";
	foreach($sql as $row)
	{
		$all_order_id.=$row[csf("order_id")].",";
		$all_subcon_order.=$row[csf("sub_order_ids")].",";
	...
	$order_sql ="select a.job_no_prefix_num,b.id,b.buyer_po_no,b.buyer_style_ref, b.job_no_mst,b.order_no,b.order_uom,c.color_id,c.qnty from subcon_ord_mst a, subcon_ord_dtls b, subcon_ord_breakdown c where a.id=b.mst_id and b.id=c.mst_id and a.embellishment_job=c.job_no_mst and a.entry_form='204' and b.id in($all_subcon_order)";
	//echo $order_sql; die;
	$order_sql_res=sql_select($order_sql); $all_subcon_job='';
	foreach ($order_sql_res as $row)
	{
	...
	foreach($sql as $row)
	{
		...
		//$po_id=explode(",",chop($row[csf('sub_order_ids')]),',');
		$po_id = explode(",", $row[csf('sub_order_ids')]);
		//print_r($po_id);
		//echo $row[csf('sub_order_ids')].'==';
		foreach($po_id as $val) 
		{
			//echo $val;
			if($buyer_po=="") $buyer_po=$buyer_po_arr[$val]['po']; else $buyer_po.=','.$buyer_po_arr[$val]['po'];
		...
		//echo "document.getElementById('txt_job_no').value = '".$subcon_order_data[$row[csf("sub_order_ids")]]['job_no_mst']."';\n"; 
		
		//echo "document.getElementById('txt_order_no').value = '".$row[csf("buyer_job_no")]."';\n"; 
		echo "document.getElementById('hidden_order_id').value = '".$row[csf("sub_order_ids")]."';\n"; 
		echo "document.getElementById('cbo_store_name').value = '".$row[csf("store_id")]."';\n";
...
{	  
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			$field_array="id, issue_number_prefix, issue_number_prefix_num, issue_number,issue_basis, issue_purpose, entry_form, company_id, location_id, store_id, buyer_job_no, style_ref, req_no, req_id, issue_date, loan_party, lap_dip_no, batch_no, order_id, sub_order_ids, inserted_by, insert_date, lc_company,floor_id,machine_id,remarks";
		...
		{
			$field_array_update="issue_purpose*location_id*buyer_job_no*style_ref*req_no*req_id*issue_date*loan_party*lap_dip_no*batch_no*order_id*sub_order_ids*updated_by*update_date*lc_company*floor_id*machine_id*remarks";
		...
		for ($i=1;$i<$total_row;$i++)
		{
			...
			if(str_replace("'",'',$$txt_reqn_qnty_edit)>0)
			{
				...
				else
				{
					...
					//echo "20**".str_replace("'",'',$$txt_prod_id)."_".$lot_no; disconnect($con); die;
					//20**2.4000_-18365.0084_-18365.0084
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		for ($i=1;$i<$total_row;$i++)
		{
			...
			$hidtxt_reqn_qnty_edit="hidtxt_reqn_qnty_edit_".$i;
			$txt_reqn_qnty_edit="txt_reqn_qnty_edit_".$i;
			//echo "10**".$$txt_reqn_qnty_edit.'==';
			//'1.8000'==10**'1.8000'==10**'2.4000'== 
			if(str_replace("'",'',$$txt_item_cat)==5) $all_prod_id_c.=str_replace("'",'',$$txt_prod_id).",";
			else if(str_replace("'",'',$$txt_item_cat)==6) $all_prod_id_d.=str_replace("'",'',$$txt_prod_id).",";
			else $all_prod_id_ac.=str_replace("'",'',$$txt_prod_id).",";
			...
			if(str_replace("'",'',$$txt_reqn_qnty_edit)>0)
			{
				...
				if(str_replace("'",'',$$txt_item_cat)==22)
				{
					...
					if($txt_reqn_qnty_edit*1 > $store_c_stock || $txt_reqn_qnty_edit*1 > $prod_stock)
					{
						echo "20**Issue Quantity Not Allow Over Stock Quantity";disconnect($con); die;
					}
				}
				else
				{
					...
					if($txt_reqn_qnty_edit*1 > $store_c_stock  || $txt_reqn_qnty_edit*1 > $prod_stock || $txt_reqn_qnty_edit*1 > $store_tans_c_stock)
					//if(str_replace("'",'',$$txt_reqn_qnty_edit)*1 > $store_c_stock || str_replace("'",'',$$txt_reqn_qnty_edit)*1 > $store_tans_c_stock)
					{
						/*if($txt_reqn_qnty_edit*1 > $store_c_stock ){
							echo "20**1";
						}else if($txt_reqn_qnty_edit*1 > $prod_stock ){
							echo "20**2";
						}
						else if($txt_reqn_qnty_edit*1 > $store_tans_c_stock ){
							echo "20**3";
						}else {
							echo "20**4";
						}*/
						//echo "20**".$txt_reqn_qnty_edit.'_'.$store_c_stock.'_'.$prod_stock.'_'.$store_tans_c_stock;disconnect($con); die;
						//echo "20**".$txt_reqn_qnty_edit.'_'.$store_tans_c_stock.'_'.$store_tans_qnty;disconnect($con); die;
						//2.4000_9981635.2_9988219.1_-18365.0084
						echo "20**Issue Quantity Not Allow Over Stock Quantity";disconnect($con); die;
					}
				}
			}
...
if($action=="chemical_dyes_issue_print")
{
    ...
    <table width="1100" cellspacing="0" align="">
        ...
        <tr>
            <td colspan="6" align="center" style="font-size:16px"><strong><u><? echo 'Owner Company : '.$company_library[$data[0]];?></u></strong></td>
        </tr>
        <tr>
