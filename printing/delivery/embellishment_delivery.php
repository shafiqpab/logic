...
<script>
	...
	function fnc_job_no()
	{
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				var company = $('#cbo_company_name').val();
				var list_view_orders = return_global_ajax_value( 0+'**'+ex_data[0]+'**'+1+'**'+company, 'load_php_dtls_form', '', 'requires/embellishment_delivery_controller');
				if(list_view_orders!='')
				{
	...
	function fnc_embl_delivery( operation )
	{
		...
		if (operation==5) {
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#txt_update_id').val()+'*'+report_title+'*'+$('#cbo_location_name').val()+'*'+$('#txt_delv_no').val()+'*'+show_item+'*'+$('#cbo_within_group').val()+'*'+$('#cbo_template_id').val(), "embl_delivery_entry_print_1", "requires/embellishment_delivery_controller") 
			//return;
			show_msg("3");
		...
		{
			var show_item='';
			var r=confirm("if Select  \"OK\"  previous Delivery Column will show\n if Select  \"Cancel\"  previous Delivery Column will Not Show");
			var show_item = (r) ? "1" : "0";
			var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_name').val()+'*'+$('#txt_update_id').val()+'*'+report_title+'*'+$('#cbo_location_name').val()+'*'+$('#txt_delv_no').val()+'*'+show_item+'*'+$('#cbo_within_group').val()+'*'+$('#cbo_template_id').val(), "embl_delivery_entry_print", "requires/embellishment_delivery_controller") 
			//return;
			show_msg("3");
		}
		else
		{
			...
			for (var i=1; i<=tot_row; i++)
			{
				...
				if(qty*1>0)
				{
					if($('#deliveryChkbox_'+i).is(':checked'))
					{
						k++;
						data_str+="&txtbuyerPoId_" + k + "='" + $('#txtbuyerPoId_'+i).val()+"'"+"&txtCurrDelv_" + k + "='" + $('#txtCurrDelv_'+i).val()+"'"+"&txtRemarks_" + k + "='" + $('#txtRemarks_'+i).val()+"'"+"&txtpoid_" + k + "='" + $('#txtpoid_'+i).val()+"'"+"&txtColorSizeid_" + k + "='" + $('#txtColorSizeid_'+i).val()+"'"+"&txtsort_" + k + "='" + $('#txtsort_'+i).val()+"'"+"&txtreject_" + k + "='" + $('#txtreject_'+i).val()+"'"+"&txtcuttingNo_" + k + "='" + $('#txtcuttingNo_'+i).val()+"'"+"&txtDtlsUpdateId_" + k + "='" + $('#txtDtlsUpdateId_'+i).val()+"'"+"&cboshipingStatus_" + k + "='" + $('#cboshipingStatus_'+i).val()+"'"+"&hdnDtlsdata_" + k + "='" + $('#hdnDtlsdata_'+i).val()+"'";
					}
				}
			} 
			if(k==0)
	...
	function fnc_embl_delivery_response()
	{
		if(http.readyState == 4) 
		{
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				var company = $('#cbo_company_name').val();
				var list_view_orders = return_global_ajax_value( response[1]+'**'+$('#txtJob_no').val()+'**'+2+'**'+company, 'load_php_dtls_form', '', 'requires/embellishment_delivery_controller');
				if(list_view_orders!='')
				{
					$("#dtls_tbody tr").remove();
					$("#dtls_tbody").append(list_view_orders);
			...
			if(response[0]==11)
			{
				alert("Update Not Allow Bill Found!!!");
			}
			if(response[0]==2)
			{
				location.reload(); 
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				if (emb_data[0]!="")
				{
					...
					load_drop_down( 'requires/embellishment_delivery_controller', $('#cbo_company_name').val()+'_'+emb_data[3], 'load_drop_down_buyer', 'buyer_td' );
					...
					//load_drop_down( 'embellishment_delivery_controller', company+'_'+emb_data[3]+'_'+party_name, 'load_drop_down_buyer_pop', 'buyer_td' );
					
					
					var list_view_orders = return_global_ajax_value( emb_data[0]+'**'+emb_data[7]+'**'+1+'**'+company, 'load_php_dtls_form', '', 'requires/embellishment_delivery_controller');
					if(list_view_orders!='')
					{
						$("#dtls_tbody tr").remove();
						$("#dtls_tbody").append(list_view_orders);
	...
	function fnc_production_qty_ability(value,i)
	{
		...
		if(variable_status==3) // qc qty 
		{
			...
			if((value*1)+pre_delv_qty<order_qty_with_wastage)
			{
				...
				if(((value*1)+pre_delv_qty)>productQty)
				{
					//alert("Qnty Excceded");
	...
	{
		var work_no = $('#txt_wo_no').val();
		var Jobno= $('#txtJob_no').val();
		var jobdetailsid= $('#txtpoid_'+row).val();
		var buyerPoId = $('#txtbuyerPoId_'+row).val();
		var breakdownid = $('#txtColorSizeid_'+row).val();
		var data_break=$('#hdnDtlsdata_'+row).val();
		var hdnDtlsUpdateId=$('#txtDtlsUpdateId_'+row).val();
		
		var page_link = 'requires/embellishment_delivery_controller.php?job_dtls_id='+job_dtls_id+'&data_break='+data_break+'&hdnDtlsUpdateId='+hdnDtlsUpdateId+'&buyerPoId='+buyerPoId+'&Jobno='+Jobno+'&breakdownid='+breakdownid+'&jobdetailsid='+jobdetailsid+'&work_no='+work_no+'&action=reject_qty_popup';
		
		emailwindow=dhtmlmodal.open('EmailBox','iframe',page_link,'Reject Qty Popup', 'width=470px, height=100px, center=1, resize=0, scrolling=0','../')
		emailwindow.onclose=function()
		{
			...
			var tot_defect_qty= (defect_qty[0])*1+(defect_qty[1])*1;
			$('#hdnDtlsdata_'+row).val(break_data);
			$('#txtreject_'+row).val(tot_defect_qty);
		}		
	}
	
	function chk_qty_level_variabe(company)
	{
	   var status = return_global_ajax_value(company, 'chk_qty_level_variable', '', 'requires/embellishment_delivery_controller').trim();
	   status = status.split("**");
	...
	{
		for (Looper=0; Looper < document.embdeliventry_1.length ; Looper++ )
		{
			var strType = document.embdeliventry_1.elements[Looper].type;
			if (strType=="checkbox")
			{
				document.embdeliventry_1.elements[Looper].checked=checkVal;
			}
		}
	}
	...
	function set_all_delivery(value){
    	...
    	var row_num = document.getElementById("dtls_tbody").rows.length*1-1; 
    	...
    	if(row_num>0)
    	{
    		if( document.getElementById('txt_check_box_advance').checked==true)
			{
				for (var i =1; i<=row_num; i++) {
					...
					var value = document.getElementById("txtCurrDelv_"+i).getAttribute("placeholder");

					var disabled_data = document.getElementById('txtCurrDelv_'+i);
					
					if (disabled_data.disabled==false && $('#txtCurrDelv_'+i+':visible').length == 1)
					{
		    			$('#txtCurrDelv_'+i).val(value);

		    			fnc_production_qty_ability(value,i);
		    			fnc_total_calculate();

		    			$("#deliveryChkbox_"+i).prop( "checked", true );
		    		}
		    		else
		    		{
		    			$("#deliveryChkbox_"+i).prop( "checked", false );
		    		}
	    		}
			}
			else
	...
	<div style="width:100%;" align="center">
        ...
        <form name="embdeliventry_1" id="embdeliventry_1" autocomplete="off"> 
            ...
            <legend>Embellishment Delivery Details</legend>
                <table style="width:1200px;" cellpadding="0" cellspacing="2" border="1" class="rpt_table" rules="all" id="tbl_dtls" >
                    <thead class="form_table_header">
                	    ...
                	    <tr>
                	    	<th width="30"><input form="form_all" type="checkbox" name="check_all" id="check_all" value=""  onclick="fnCheckUnCheckAll(this.checked)"/></th>
	                        <th width="30">SL</th>
                            <th width="90">Buyer PO</th>
                            <th width="90">Style Ref.</th>
	                        <th width="200">Gmts Item</th>
                    ...
                    <tbody id="dtls_tbody">
                    	<tr bgcolor="#FFFFFF">
                    		<td align="center">
				        		<input type="checkbox" name="deliveryChkbox[]" id="deliveryChkbox_1" value="" checked />
				        	</td>
                            ...
                            <td align="center"><input type="text" name="txtreject_1" id="txtreject_1" class="text_boxes_numeric" style="width:40px;" onBlur="fnc_total_calculate();"  onClick="openmypage_reject_qty(1,'0',1)"/>
                             <input type="hidden" name="hdnDtlsdata[]" id="hdnDtlsdata_1">
                            </td>
                            <td>&nbsp;</td>
                            <td align="center"><input type="text" name="txtcuttingNo_1" id="txtcuttingNo_1" class="text_boxes" style="width:60px;" /></td>
                        ...
                        <td align="center" valign="middle" class="button_container">
                         <? echo create_drop_down( "cbo_template_id", 100, $report_template_list,'', 0, '', 0, "");?>&nbsp;
                        	<? echo load_submit_buttons($permission,"fnc_embl_delivery",0,1,"reset_form('embdeliventry_1', '','','','')",1); ?>
							 <input id="Print2" class="formbutton" type="button" style="width:80px;" onClick="fnc_embl_delivery(5)" name="print" value="Print 1"> 
