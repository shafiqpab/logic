...
echo load_html_head_contents("Embl. Bill Issue", "../../", 1, 1,'','','');
?>
<script>
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				if(list_view_orders!='')
				{
					...
					$("#tbl_dtls_body tbody").html("");
					$("#tbl_dtls_body tbody").html(list_view_orders);
					
					setFilterGrid("tbl_dtls_body",-1,tableFilters);
					fnc_final_rate_calc();
					fnc_total_calculate();
				}
	...
	function fnc_embl_bill_issue( operation )
	{
		...
		var txtTotbillqty = $('#txtTotbillqty').val()*1;
		var txtTotbillqty1 = $('#txtTotbillqty1').val()*1;

		if(txtTotbillqty1>txtTotbillqty){
			alert("Toatl Bill Quanty Can Not Be Gretter Than Total Delivery Quantity");
			return;
		}
		
		if(variable_status==2 && operation!=4 && operation!=5)
		{
			if(confirm("Auto Balancing Sure?")){
			...
			for (var jj=1; jj<=tot_row1; jj++)
			{
				...
				if(variable_qty_status==2)
				{
					...
					{ 
						...
						totalbillqty2+=$('#hiddenbilqty_'+k).attr("title")*1;
					...
					if(totalbillqty2>balnceQty)
					{
						...
						//$('#txtbillqty_'+jj).val('');
						$('#hiddenbilqty_'+jj).attr("title","");
						$('#hiddenbilqty_'+jj).html("");
				...
				var billQuantity = $('#hiddenbilqty_'+jj).attr("title")*1;
				...
				if(variable_qty_status==1)
				{
					...
					{ 
						...
						{ 
							...
							totalbillqty+=$('#hiddenbilqty_'+k).attr("title")*1;							
						}
					}
		...
		if ( form_validation('cbo_company_name*cbo_party_name*txt_bill_date*txtJob_no*cbo_currency_id', 'Company Name*Party*Bill Date*Job No*Currency')==false )
		{
			return;
		}
		else
		{
			...
			else if(operation==5)
			{
				...
				//alert(1);
				var report_title=$( "div.form_caption" ).html();
				print_report( $('#cbo_company_name').val()+'*'+$('#txt_update_id').val()+'*'+report_title+'*'+$('#cbo_template_id').val()+'*'+$('#cbo_location_name').val(), "embl_bill_issue_print_2", "requires/embl_bill_issue_controller") 
				//return;
				show_msg("3");
			}
			else
			{
				...
				for (var i=1; i<=tot_row; i++)
				{
					...
					var billqty=$('#hiddenbilqty_'+i).attr("title")*1;
					
					if(billqty*1>0 && rate*1>0)
					{
						k++;
						data_str+="&txtbuyerPoId_" + k + "='" + $('#txtbuyerPoId_'+i).val()+"'"+"&txtdeliveryid_" + k + "='" + $('#txtdeliveryid_'+i).val()+"'"+"&sysnotd_" + k + "='" + $('#sysnotd_'+i).text()+"'"+"&deliverydatetd_" + k + "='" + $('#deliverydatetd_'+i).text()+"'"+"&txtbillqty_" + k + "='" + $('#hiddenbilqty_'+i).attr("title")+"'"+"&txtbillrate_" + k + "='" + $('#txtbillrate_'+i).val()+"'"+"&txtbillamount_" + k + "='" + $('#txtbillamount_'+i).val()+"'"+"&txtRemarks_" + k + "='" + $('#txtRemarks_'+i).val()+"'"+"&txtpoid_" + k + "='" + $('#txtpoid_'+i).val()+"'"+"&txtColorSizeid_" + k + "='" + $('#txtColorSizeid_'+i).val()+"'"+"&txtdomisticamount_" + k + "='" + $('#txtdomisticamount_'+i).val()+"'"+"&txtDtlsUpdateId_" + k + "='" + $('#txtDtlsUpdateId_'+i).val()+"'"+"&txtOrderQuantity_" + k + "='" + $('#txtOrderQuantity_'+i).html()+"'"+"&txtdeliveryqty_" + k + "='" + $('#txtdeliveryqty_'+i).html()+"'"+"&txttotalbillqty_" + k + "='" + $('#txttotalbillqty_'+i).val()+"'"+"&txt_gmts_item_id_" + k + "='" + $('#txt_gmts_item_id_'+i).val()+"'"+"&txt_color_id_" + k + "='" + $('#txt_color_id_'+i).val()+"'"+"&txt_size_id_" + k + "='" + $('#txt_size_id_'+i).val()+"'"+"&txt_bodypart_id_" + k + "='" + $('#txt_bodypart_id_'+i).val()+"'"+"&txt_embl_name_id_" + k + "='" + $('#txt_embl_name_id_'+i).val()+"'";
					}
				}
				if(k==0)
				{
		...
		else
		{
			...
			emailwindow.onclose=function()
			{
				...
				if (bill_data[0]!="")
				{
					...
					fnc_load_party(1,$('#cbo_within_group').val())
				   ...
				   // $('#txt_exchange_rate').attr('disabled','disabled');
					...
					if(list_view_orders!='')
					{
						//$("#dtls_tbody tr").remove();
						//$("#dtls_tbody").append(list_view_orders);
						//setFilterGrid("dtls_tbody",-1,tableFilters);
						$("#tbl_dtls_body tbody").html("");
						$("#tbl_dtls_body tbody").html(list_view_orders);
						setFilterGrid("tbl_dtls_body",-1,tableFilters);
					}
	...
	function fnc_amount_calculation(val,inc)
	{
		...
		var qty=$("#txttotalbillqty_"+inc).val()*1;
	...
	function fnc_total_calculate()
	{
		...
		//var tot_delivery_amount=0;
		//for(var i=1; i<=rowCount; i++)
		//{
			//tot_delivery_amount+=$("#hiddenbilqty_"+i).html()*1;
		//}
		//$("#Value_txtTotbillqty").html(tot_delivery_amount.toFixed(2));
		
		math_operation( "txtTotbillqty1", "txttotalbillqty_", "+", rowCount );
	...
	function fnc_copy_final_rate(id, value)
	{
			var id = id;
			var id_num_arr = id.split("_");
			var id_num = id_num_arr[1]; 
 			var txtBuyerPo= trim($("#txtBuyerPo_"+id_num).html());
 			var rate  = value*1;  
			var copy_val=document.getElementById('copy_po_wise_bill_rate').checked;
 			//alert(copy_val);
			...
			var row_num=$('#dtls_tbody tr').length-1;
			for (var j=1; j<=row_num; j++)
			{
				var newBuyerPo= trim($("#txtBuyerPo_"+j).html());
				//alert(newBuyerPo);
					if(newBuyerPo==txtBuyerPo)
					{
						$('#txtbillrate_'+j).val(rate);
						fnc_amount_calculation(rate,j);
					}
				}
			}
			/*else
	...
	function fnc_copy_amount(id, value)
	{
		var id = id;
		var id_num_arr = id.split("_");
		var id_num = id_num_arr[1];

		var bodypart= trim($("#txtBodtyPart_"+id_num).html());
		var amount  = value;

		var copy_val=document.getElementById('copy_body_part_bill_rate').checked;
		if(copy_val==true)
		{
			var rowCount = $('#dtls_tbody tr').length-1;
			...
			for(var i=1; i<=rowCount; i++)
			{
				var new_bodypart= trim($("#txtBodtyPart_"+i).html());

				if(new_bodypart==bodypart)
				{
					$("#txtbillrate_"+i).val(amount);

					fnc_amount_calculation(amount,i);
				}
			}
...
function chk_qty_level_variabe(company)
{
   ...
   var delivery_status = return_global_ajax_value(company, 'delivery_chk_qty_level_variable', '', 'requires/embl_bill_issue_controller').trim();
...
function calculate_amount(i)
{
	...
	var txtbillqty=$('#hiddenbilqty_'+i).attr("title")*1;	
	var txttotalbillqty=$("#txttotalbillqty_"+i).val()*1;
	...
	if (txttotalbillqty>txtbillqty) {
		alert("Bill Quantity can Not Be Gretter Than Delivery Quanty");

		$("#txttotalbillqty_"+i).val( number_format (txtbillqty, 0,'.' , ""));

	}
	var amount=txttotalbillqty*txtbillrate;
	var domisticamount=amount*exchange_rate;
	...
	var Curbillqty=$('#hiddenbilqty_'+i).attr("title")*1;	
	...
	*/
	...
	if(variable_qty_status==2)
	{
		...
		if(totalbillqty>balnceQty)
		{
			alert("Bill Qty Over Order Balance");
			//$('#txtbillqty_'+i).val('');
			$('#hiddenbilqty_'+i).attr("title","");
			$('#hiddenbilqty_'+i).html("");			
			return;
		}
...
function bill_auto_blancing(str,colorSize){
	...
	for(var s=0;s<colorSizeSlIdArr.length;s++){
		...
		var runDeliveryQty=$('#hiddenbilqty_'+skey).attr("title")*1;			
		...
		//if(totalCapacityBillQty>0){
			...
			if(totalCapacityBillQty>0 && totalCapacityBillQty>runDeliveryQty){
				...
				$("#hiddenbilqty_"+skey).attr("title",runDeliveryQty);
				$("#hiddenbilqty_"+skey).html(runDeliveryQty);				
				$("#hiddenbilqty_"+skey).css("background-color", "yellow");
				...
				$("#hiddenbilqty_"+skey).attr("title",totalCapacityBillQty);
				$("#hiddenbilqty_"+skey).html(totalCapacityBillQty);				
				$("#hiddenbilqty_"+skey).css("background-color", "yellow");
				 totalCapacityBillQty=0;
			}
			else{
				//$("#txtbillqty_"+skey).val(0);
				$("#hiddenbilqty_"+skey).attr("title",0);
				$("#hiddenbilqty_"+skey).html(0);				
				totalCapacityBillQty=0;
			}
...
function fn_bill_qnty_chek()
{
	if($('#chk_bill_qnty').prop('checked', false))
	{
		var rowCount = $('#dtls_tbody tr').length-1;
		for(var i=1; i<=rowCount; i++)
		{
			$("#txttotalbillqty_"+i).val("");
		}
		$("#txtTotbillqty1").val("");
	}
}
	...
	<div style="width:100%;" align="center">
        ...
        <form name="embbillissue_1" id="embbillissue_1" autocomplete="off"> 
			...
			<legend>Embl. Bill Issue</legend>
                <table width="990" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    <tr>
                        ...
                        <input type="hidden" id="delivery_status" name="qty_validation_status" value="" />
                        </td>
                    </tr>
                    <tr>
            ...
            <legend>Embellishment Bill Details</legend>
                <table style="width:1450px;" cellpadding="0" cellspacing="2" border="1" class="rpt_table" rules="all" id="tbl_dtls" >
                    <thead class="form_table_header">
                        <tr>
                            ...
                            <th><? echo create_drop_down( "cbo_currency", 60, $currency,"", 1, "-- Select Currency --",1,"", 1,"" ); ?>
                            ...
                            <th colspan="3">
                            	Final Rate Copy <input type="checkbox" name="copy_body_part_bill_rate" id="copy_body_part_bill_rate"/>Body Part Wise 
                            </th>
                            <th colspan="3"> 
                                <input type="checkbox" name="copy_po_wise_bill_rate" id="copy_po_wise_bill_rate"/>PO Wise
                            </th>
                            <th colspan="5">
                                <div style="color: red; font-size: 14px;" id="display_delivery_status"></div>
                            </th>
                        </tr>
                        <tr>
                            <th width="30">SL</th>
                            ...
                            <th width="60">Process /Type</th>
                            <th width="80">Color</th>
                            <th width="60">Size</th>
                            <th width="60">Order Qty</th>
                            <th width="60">Cum. Bill. Qty</th>
                            <th width="60">Delv. Qty.</th>
                            <th width="60">Bill Qty. <br> <input id="chk_bill_qnty" name="chk_bill_qnty" type="checkbox" onChange="fn_bill_qnty_chek()" checked /></th>
                            <th width="60">Initial Rate</th>
                            <th width="60" class="must_entry_caption">Final Rate<input id="copy_basis" name="copy_basis" type="checkbox" onChange="copy_values(1)" /></th>
                ...
                <table style="width:1450px;" cellpadding="0" cellspacing="2" border="1" class="rpt_table" rules="all" id="tbl_dtls_body" >
                	<tbody id="dtls_tbody">
                    	<tr bgcolor="#FFFFFF">
                    		<td align="center" width="30">1<input name="txtbuyerPoId_1" id="txtbuyerPoId_1" type="hidden" class="text_boxes" style="width:70px" value=""/></td>
                            <td width="50">&nbsp;</td>
                    		<td width="60">&nbsp;</td>
                    		<td width="100">&nbsp;</td>
                    		<td width="100">&nbsp;</td>
                            ...
                            <td width="60">&nbsp;</td>
                    		<td width="60">&nbsp;</td>
                    		<td width="60" align="center" id="hiddenbilqty_1"></td>
                    		<td width="60" align="center"><input type="text" name="txttotalbillqty_1" id="txttotalbillqty_1" class="text_boxes_numeric" style="width:50px;" onBlur="calculate_amount(1)"  /></td>
                    		<td width="60" align="center"><input type="text" name="txtInitialRate_1" id="txtInitialRate_1" class="text_boxes_numeric" style="width:50px;" disabled /></td>
                            <td width="60" align="center"><input type="text" name="txtbillrate_1" id="txtbillrate_1" class="text_boxes_numeric" style="width:50px;" onBlur="fnc_amount_calculation(this.value,1); fnc_total_calculate();" onChange="fnc_copy_amount(this.id,this.value);fnc_copy_final_rate(this.id,this.value);"/></td>
                            <td width="60" align="center"><input type="text" name="txtbillamount_1" id="txtbillamount_1" class="text_boxes_numeric" style="width:50px;" readonly /></td>
                            <td width="80" align="center"><input type="text" name="txtdomisticamount_1" id="txtdomisticamount_1" class="text_boxes_numeric" style="width:70px;" readonly /></td>
                    		<td align="center"><input type="text" name="txtRemarks_1" id="txtRemarks_1" class="text_boxes" style="width:70px;" />
                ...
                <table style="width:1450px;" cellpadding="0" cellspacing="2" border="1" class="rpt_table" rules="all" id="tbl_dtls_foot" >     
                    <tfoot id="dtls_tfoot">
                        <tr>
                            <th width="30">&nbsp;</th>
                            <th width="50">&nbsp;</th>
                            ...
                            <th width="60" align="right" id="Value_txtTotbillqty"></th>
                            <th width="60" align="center"><input type="text" name="txtTotbillqty1" id="txtTotbillqty1" class="text_boxes_numeric" style="width:50px;" readonly /></th>
                            <th width="60">&nbsp;</th>
                            <th width="60">&nbsp;</th>
                            <th width="60" align="center"><input type="text" name="txtTotbillamount" id="txtTotbillamount" class="text_boxes_numeric" style="width:50px;" readonly /></th>
                             <th width="80" align="center"><input type="text" name="txtTotdomisticamount" id="txtTotdomisticamount" class="text_boxes_numeric" style="width:70px;" readonly /></th>
                            <th>&nbsp;</th> 
                        </tr>
                        <tr>
                ...
                <table width="830" cellspacing="2" cellpadding="0" border="0">
                    <tr>
                        <td align="center" valign="middle" class="button_container">
							...
							<input type="button" id="show_button" class="formbutton" style="width:80px" value="Print 2" onClick="fnc_embl_bill_issue(5)" />
                        </td>
                    </tr>   
                </table>
            </fieldset>          
