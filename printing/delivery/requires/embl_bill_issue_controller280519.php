...
if ($action=="load_drop_down_buyer_pop")
{
	...
	if($data[1]==1)
	{
		echo create_drop_down( "cbo_party_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Party --", $data[0], "");
		exit();
	}
	else
	{
...
if($action=="load_php_dtls_form")
{
	...
	foreach ($sql_result as $row)
	{
        ...
        <tr bgcolor="<? echo $bgcolor; ?>">
            ...
            <td align="center" id="sysnotd_<? echo $k; ?>"><? echo $row[csf("delivery_prefix_num")]; ?></td>
            ...
            <td align="center"><input type="text" name="txtInitialRate_<? echo $k; ?>" id="txtInitialRate_<? echo $k; ?>" class="text_boxes_numeric" style="width:50px;" value="<? echo $row[csf("colorSizeRate")]; ?>" /></td>
...
if($action=="create_bill_search_list_view")
{
	...
	$sql= "select a.id, a.bill_no, a.prefix_no_num, $insert_date_cond as year, a.location_id, a.bill_date, a.party_id, a.party_source, a.party_location_id, a.remarks, $order_id_cond as order_id, $buyer_po_id_cond as buyer_po_id, $delivery_id_cond as delivery_id from subcon_inbound_bill_mst a, subcon_inbound_bill_dtls b where a.id=b.mst_id and a.status_active=1 and a.process_id=13 $company $buyer_cond $withinGroup $bill_date $bill_id_cond $spo_idsCond $po_idsCond  group by a.id, a.bill_no, a.prefix_no_num, a.insert_date, a.location_id, a.bill_date, a.party_id, a.party_source, a.party_location_id, a.remarks order by a.id DESC";
	//echo $sql; 
     ...
     <div style="width:820px; max-height:270px;overflow-y:scroll;" >	 
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="800" class="rpt_table" id="tbl_po_list">
            ...
            foreach($result as $row)
            {
				...
				$str_data="";
				$str_data=$row[csf('id')].'***'.$row[csf('bill_no')].'***'.$row[csf('location_id')].'***'.$row[csf('party_source')].'***'.$row[csf('party_id')].'***'.$row[csf('party_location_id')].'***'.change_date_format($row[csf('bill_date')]).'***'.$row[csf('remarks')].'***'.$job_no.'***'.$order_no.'***'.$buyer_style.'***'.$buyer_po.'***'.$buyer_name;
				?>
    ...
    <div style="width:930px; font-size:6px">
        ...
        <div style="width:100%;">
            <table align="right" cellspacing="0" width="930" border="1" rules="all" class="rpt_table" style="font-size:12px">
				...
				and a.entry_form=204 and d.process_id=13 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and a.company_id='$data[0]' and d.mst_id='$data[1]' order by c.id ASC";
					...
					if (!in_array($row[csf("order_no")],$order_array) )
					{
						...
						?>
							<tr bgcolor="#dddddd">
                                ...
                                <td colspan="5" align="left" style="font-size:12px"><b>Buyer Style: <? echo $buyer_po_arr[$row[csf("buyer_po_id")]]['style']; ?>;</b></td>
							</tr>
						<?
            ...
            <br>
			<? echo signature_table(154, $com_id, "930px"); ?>
        </div>
    </div>
	<?
	exit();
