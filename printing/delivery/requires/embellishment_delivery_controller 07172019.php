...
{	
	...
	else if($db_type==2)
	{
	 ...
	 where a.entry_form=204 and a.embellishment_job=b.job_no_mst and a.id=b.mst_id and a.status_active=1 and b.status_active=1 $order_rcv_date $company $buyer $withinGroup $search_com_cond $po_idsCond  and b.id=c.mst_id  
...
if($action=="load_php_dtls_form")
{
	...
	foreach ($sql_result as $row)
	{
        ...
        <tr bgcolor="<? echo $bgcolor; ?>">
            ...
            <td align="right"><?php echo create_drop_down( "cboshipingStatus_".$k,100, $shipment_status,'', 1, '-Select-',$row[csf('yarn_comp_type1st')],"onchange=fnc_cboshipingStatus(".$k.",this.value);",0,"","","","","","","cboshipingStatus[]"); ?></td>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		$delivery_qty_sql=sql_select("select b.delivery_qty, b.order_id from subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id and a.id!=$id and a.entry_form=254 and a.job_no=$txtJob_no");
...
if($action=="embl_delivery_entry_print")
{
	...
	$sql_mst = "select id, delivery_no, company_id, location_id, within_group, party_id, delivery_date, job_no, challan_no, remarks from subcon_delivery_mst where entry_form=254 and id='$data[1]'";
...
\								<td>&nbsp;</td>
						...
						?>
							<tr bgcolor="#dddddd">
								<td colspan="11" align="left" style="font-size:12px"><b>Embl. Job No: <? echo $row[csf("embellishment_job")]; ?>;</b>&nbsp;&nbsp;&nbsp;&nbsp;<b>Work Order No: <? echo $row[csf("order_no")]; ?>;</b>&nbsp;&nbsp;&nbsp;&nbsp;<b>Buyer Style: <? echo $buyer_po_arr[$row[csf("buyer_po_id")]]['style']; ?>;</b>&nbsp;&nbsp;&nbsp;&nbsp;<b>Buyer Name: <? echo $buyer_library[$buyer_po_arr[$row[csf("buyer_po_id")]]['buyer']]; ?>;</b>
                                </td>
							</tr>
						<?
