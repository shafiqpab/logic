                        ...
                        <tr class="general">
                            ...
                            <td>
								...
								//	if($data[4]==2){ $order_wise=1;}else{$order_wise=0; }
									...
									echo create_drop_down( "cbo_type",100, $search_by_arr,"",0, "",6,'search_by(this.value)',$order_wise,'','','',3 );
								?>
                            </td>
                            <td align="center">
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+<? echo $data[3];?>+'_'+<? echo $data[4];?>, 'create_job_search_list_view', 'search_div', 'embl_bill_issue_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:70px;" />
                            </td>
                        </tr>
                        <tr>
...
if($action=="load_php_dtls_form")
{
	...
	//$sql_del="select a.id, a.mst_id, a.order_id, a.delivery_qty, a.remarks, a.color_size_id from subcon_delivery_dtls a, subcon_delivery_mst b where b.id=a.mst_id and b.job_no='$jobno' and b.entry_form=254 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	/*$sql_bill="select id, delivery_id, order_id, delivery_qty, rate, amount, remarks, color_size_id from subcon_inbound_bill_dtls where mst_id='$update_id' and process_id=13 and status_active=1 and is_deleted=0";
	...
	{
		$updtls_data_arr[$row[csf("order_id")]][$row[csf("id")]]['qty']+=$row[csf("delivery_qty")];
	}
	unset($sql_del_res);*/
	$sql_del="SELECT a.id, a.mst_id, a.delivery_id,a.order_id, a.delivery_qty,a.color_size_id  from subcon_inbound_bill_dtls a, subcon_inbound_bill_mst b where b.id=a.mst_id   and a.entry_form=395  and b.entry_form=395 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	$sql_del_res =sql_select($sql_del);
	$updtls_data_arr=array(); $pre_qty_arr=array();
	...
	if(($bundle_chk)*1==1)
	{
		if($update_id>0)
		{
			$sql_job="SELECT  f.id as upid, a.id, a.embellishment_job, a.job_no_prefix_num, a.company_id, b.id as po_id, b.buyer_po_id, b.gmts_item_id, b.embl_type, b.body_part, b.main_process_id, b.order_uom, c.id as breakdown_id, c.description, c.color_id, c.size_id, c.qnty , c.rate as colorsizerate,e.issue_number as delivery_no,e.issue_num_prefix_no as delivery_prefix_num,e.id as delvID, a.location_id, a.party_id, a.receive_date, a.order_no, a.currency_id,a.within_group, e.issue_date as delivery_date ,sum(d.quantity) as delivery_qty,f.delivery_qty as bil_qty
			from subcon_ord_mst a, subcon_ord_dtls b, subcon_ord_breakdown c, printing_bundle_issue_dtls d, printing_bundle_issue_mst e , subcon_inbound_bill_dtls f ,subcon_inbound_bill_mst g  
			where a.entry_form=204 and e.entry_form=499 and a.embellishment_job=b.job_no_mst and b.job_no_mst=c.job_no_mst and a.id=b.mst_id and c.id=d.wo_break_id and d.mst_id=e.id and a.id=d.wo_id and b.id=d.wo_dtls_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and c.qnty>0  and a.embellishment_job='$jobno' and g.id=f.mst_id and f.process_id=13  and f.entry_form=395 and e.id=g.delivery_id and f.mst_id='$update_id'  and b.id=f.order_id and c.id=f.color_size_id group by f.id,a.id, a.embellishment_job, a.job_no_prefix_num, a.company_id, b.id , b.buyer_po_id, b.gmts_item_id, b.embl_type, b.body_part, b.main_process_id, b.order_uom, c.id, c.description, c.color_id, c.size_id, c.qnty , c.rate ,e.issue_number,e.issue_num_prefix_no,e.id, a.location_id, a.party_id, a.receive_date, a.order_no, a.currency_id,a.within_group, e.issue_date, f.delivery_qty order by issue_num_prefix_no ASC";
		}else{
			//, d.id as delivery_id
			$sql_job="SELECT a.id, a.embellishment_job, a.job_no_prefix_num, a.company_id, b.id as po_id, b.buyer_po_id, b.gmts_item_id, b.embl_type, b.body_part, b.main_process_id, b.order_uom, c.id as breakdown_id, c.description, c.color_id, c.size_id, c.qnty , c.rate as colorsizerate,e.issue_number as delivery_no,e.issue_num_prefix_no as delivery_prefix_num,e.id as delvID, a.location_id, a.party_id, a.receive_date, a.order_no, a.currency_id,a.within_group, e.issue_date as delivery_date , 0 as upid,sum(d.quantity) as delivery_qty,0 as bil_qty
			from subcon_ord_mst a, subcon_ord_dtls b, subcon_ord_breakdown c, printing_bundle_issue_dtls d, printing_bundle_issue_mst e   
			where a.entry_form=204 and e.entry_form=499 and a.embellishment_job=b.job_no_mst and b.job_no_mst=c.job_no_mst and a.id=b.mst_id and c.id=d.wo_break_id and d.mst_id=e.id and a.id=d.wo_id and b.id=d.wo_dtls_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and c.qnty>0  and a.embellishment_job='$jobno' and e.id in ($job_wise_delivery_id) group by a.id, a.embellishment_job, a.job_no_prefix_num, a.company_id, b.id , b.buyer_po_id, b.gmts_item_id, b.embl_type, b.body_part, b.main_process_id, b.order_uom, c.id, c.description, c.color_id, c.size_id, c.qnty , c.rate ,e.issue_number,e.issue_num_prefix_no,e.id, a.location_id, a.party_id, a.receive_date, a.order_no, a.currency_id,a.within_group, e.issue_date order by issue_num_prefix_no ASC";
			//and e.id='$delivery_id'
		}
	}
	else
	{
	...
	foreach ($sql_result as $row)
	{
        ...
        <tr bgcolor="<? echo $bgcolor; ?>">
            ...
            <td align="center" id="sysnotd_<? echo $k; ?>"><a href='##' style='color:#000' onClick="print_report('<? echo $row[csf("company_id")].'*'.$row[csf("delvID")].'*'."Printing Delivery";?>','embl_delivery_entry_print', 'requires/embellishment_delivery_controller')"><font color="blue"><strong><? echo $row[csf("delivery_prefix_num")]; ?></strong></font></a></td>
            <td id="deliverydatetd_<? echo $k; ?>"><? echo change_date_format($row[csf("delivery_date")]); ?></td>
            <td style="word-break:break-all" id="txtBuyerPo_<? echo $k; ?>"><? echo $buyer_po_arr[$row[csf("po_id")]]['po']; ?></td>
            ...
            <td align="center" id="hiddenbilqty_<? echo $k; ?>" title="<? echo $row[csf("delivery_qty")];// $palace_bil_qty;//?>">
            	<input type="text" name="txtbillqty_<? echo $k; ?>" id="txtbillqty_<? echo $k; ?>" class="text_boxes_numeric" value="<? echo $bil_qty;//$row[csf("delivery_qty")]; ?>" style="width:50px;"  onBlur="calculate_amount(<? echo $k; ?>);"  <? echo $disabled; ?>  placeholder="<? echo $row[csf("delivery_qty")];//$palace_bil_qty; ?>" />
            </td>
