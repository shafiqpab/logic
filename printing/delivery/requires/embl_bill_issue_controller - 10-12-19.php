                ...
                <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+<? echo $data[3];?>, 'create_job_search_list_view', 'search_div', 'embl_bill_issue_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:70px;" />
                            </td>
                        </tr>
                        <tr>
...
{	
	...
	$sql= "SELECT a.id, a.embellishment_job, a.job_no_prefix_num, $ins_year_cond as year, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date,a.currency_id, $color_id_str, $buyer_po_id_cond  
	 from subcon_ord_mst a, subcon_ord_dtls b, subcon_ord_breakdown c, subcon_delivery_dtls d  
	 where a.entry_form=204 and d.bill_status!=1 and a.embellishment_job=b.job_no_mst and a.id=b.mst_id and c.id=d.color_size_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0 $order_rcv_date $company $buyer $withinGroup $search_com_cond $po_idsCond $job_cond $style_cond $po_cond and b.id=c.mst_id  
	 group by a.id, a.embellishment_job, a.job_no_prefix_num, a.insert_date, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date,a.currency_id
	 ";
...
if($action=="load_php_dtls_form")
{
	...
	//if($update_id==0) 
		...
		where a.entry_form=204 and a.embellishment_job=b.job_no_mst and a.id=b.mst_id and d.id=e.mst_id and c.id=e.color_size_id and e.id=f.delivery_id and f.process_id=13 and f.mst_id='$update_id' and d.entry_form=254 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0  and b.id=c.mst_id and a.embellishment_job='$jobno')
		union all
		(
			select a.id, a.embellishment_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date,a.currency_id, b.id as po_id, b.buyer_po_id, b.gmts_item_id, b.embl_type, b.body_part, b.main_process_id, b.order_uom, c.id as breakdown_id, c.description, c.color_id, c.size_id, c.qnty , c.rate as colorSizeRate,d.id as delvID, d.company_id ,d.delivery_prefix_num, d.delivery_date, e.id as delivery_id, e.delivery_qty,  0 as upid,  0 as rate, 0 as amount, null as remarks
		from subcon_ord_mst a, subcon_ord_dtls b, subcon_ord_breakdown c, subcon_delivery_mst d, subcon_delivery_dtls e
		where a.entry_form=204 and a.embellishment_job=b.job_no_mst and a.id=b.mst_id and d.id=e.mst_id and c.id=e.color_size_id and e.bill_status!=1 and d.entry_form=254 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 and b.id=c.mst_id and a.embellishment_job='$jobno'
		)
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$txt_update_id)=="")
		{
			...
			$new_bill_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'EBI', date("Y",time()), 5, "select prefix_no, prefix_no_num from  subcon_inbound_bill_mst where company_id=$cbo_company_name and process_id=$bill_process_id $year_cond=".date('Y',time())." order by id desc ", "prefix_no", "prefix_no_num" ));
			
			$field_array="id, prefix_no, prefix_no_num, bill_no, company_id, location_id, party_source, party_id, party_location_id, bill_date, process_id, remarks, inserted_by, insert_date, status_active, is_deleted";
			$data_array="(".$id.",'".$new_bill_no[1]."','".$new_bill_no[2]."','".$new_bill_no[0]."',".$cbo_company_name.",".$cbo_location_name.",".$cbo_within_group.",".$cbo_party_name.",".$cbo_party_location.",".$txt_bill_date.",'".$bill_process_id."',".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)"; 
			...
			$field_array1 ="id, mst_id, delivery_id, delivery_date, order_id, delivery_qty, rate, amount, remarks, process_id, buyer_po_id, color_size_id, inserted_by, insert_date, status_active, is_deleted";
			...
			for($i=1; $i<=$total_row; $i++)
			{
				...
				if(str_replace("'", '',$$txtbillamount)!="")
				{
					...
					$data_array1 .="(".$id1.",".$id.",".$$txtdeliveryid.",'".$deliverydate."',".$$txtpoid.",".$$txtbillqty.",".$$txtbillrate.",".$$txtbillamount.",".$$txtRemarks.",'".$bill_process_id."',".$$txtbuyerPoId.",".$$txtColorSizeid.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
					$id1=$id1+1;
					$add_comma++;
  	...
  	else if ($operation==1) // Update Here End------------------------------------------------------
	{
		...
		$field_array="location_id*party_source*party_id*party_location_id*bill_date*remarks*updated_by*update_date";
		$data_array="".$cbo_location_name."*".$cbo_within_group."*".$cbo_party_name."*".$cbo_party_location."*".$txt_bill_date."*".$txt_remarks."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		$id1=return_next_id( "id", "subcon_inbound_bill_dtls",1);
		$field_array1 ="id, mst_id, delivery_id, delivery_date, order_id, delivery_qty, rate, amount, remarks, process_id, buyer_po_id, color_size_id, inserted_by, insert_date, status_active, is_deleted";
		$field_array_up ="delivery_id*delivery_date*order_id*delivery_qty*rate*amount*remarks*buyer_po_id*color_size_id*updated_by*update_date";
		$field_array_delivery="bill_status";
		$add_comma=0;
		for($i=1; $i<=$total_row; $i++)
		{
			...
			else
			{
				...
				$data_array_up[str_replace("'",'',$$txtDtlsUpdateId)] =explode("*",("".$$txtdeliveryid."*'".$deliverydate."'*".$$txtpoid."*".$$txtbillqty."*".$$txtbillrate."*".$$txtbillamount."*".$$txtRemarks."*".$$txtbuyerPoId."*".$$txtColorSizeid."*'".$_SESSION['logic_erp']['user_id']."'*'".$pc_date_time."'"));
...
if($action=="create_bill_search_list_view")
{
	...
	$sql= "SELECT a.id, a.bill_no, a.prefix_no_num, $insert_date_cond as year, a.location_id, a.bill_date, a.party_id, a.party_source, a.party_location_id, a.remarks, $order_id_cond as order_id, $buyer_po_id_cond as buyer_po_id, $delivery_id_cond as delivery_id from subcon_inbound_bill_mst a, subcon_inbound_bill_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.process_id=13 $company $buyer_cond $withinGroup $bill_date $bill_id_cond $spo_idsCond $po_idsCond  group by a.id, a.bill_no, a.prefix_no_num, a.insert_date, a.location_id, a.bill_date, a.party_id, a.party_source, a.party_location_id, a.remarks order by a.id DESC";
	//echo $sql; 
     ...
     <div style="width:820px; max-height:270px;overflow-y:scroll;" >	 
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="800" class="rpt_table" id="tbl_po_list">
            ...
            foreach($result as $row)
            {
				...
				$str_data="";
				$str_data=$row[csf('id')].'***'.$row[csf('bill_no')].'***'.$row[csf('location_id')].'***'.$row[csf('party_source')].'***'.$row[csf('party_id')].'***'.$row[csf('party_location_id')].'***'.change_date_format($row[csf('bill_date')]).'***'.$row[csf('remarks')].'***'.$job_no.'***'.$order_no.'***'.$buyer_style.'***'.$buyer_po.'***'.$buyer_name.'***'.$curr_id.'***'.$book_approved;
				?>
...
if($action=="embl_bill_issue_print")
{
	...
	$sql_mst="SELECT id, prefix_no_num, bill_no, location_id, bill_date, party_id, party_source, process_id, party_location_id, remarks from subcon_inbound_bill_mst where process_id=13 and status_active=1 and is_deleted=0 and id='$data[1]'";
    ...
    <div style="width:1020px; font-size:6px">
        ...
        <div style="width:100%;">
            <table align="right" cellspacing="0" width="1020" border="1" rules="all" class="rpt_table" style="font-size:12px">
				...
				and a.entry_form=204 and d.process_id=13 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and a.company_id='$data[0]' and d.mst_id='$data[1]' order by d.delivery_id ASC";
