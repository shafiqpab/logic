...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Start Here=================================================================================
	{
		...
		$rID3=true;
		$field_array="id, entry_form, embellishment_job, job_no_prefix, job_no_prefix_num, company_id, location_id, within_group, party_id, party_location, currency_id, receive_date, delivery_date, rec_start_date, rec_end_date, order_id, order_no,exchange_rate,booking_type, inserted_by, insert_date";
		...
		$data_array="(".$id.", 204, '".$new_job_no[0]."', '".$new_job_no[1]."', '".$new_job_no[2]."', ".$cbo_company_name.", ".$cbo_location_name.", ".$cbo_within_group.", ".$cbo_party_name.", ".$cbo_party_location.", ".$cbo_currency.", ".$txt_order_receive_date.", ".$txt_delivery_date.",".$txt_rec_start_date.",".$txt_rec_end_date.", ".$hid_order_id.", ".$txt_order_no.",".$txt_exchange_rate.",".$hid_booking_type.", ".$_SESSION['logic_erp']['user_id'].", '".$pc_date_time."')";
		...
		$field_array3="id, mst_id, order_id, job_no_mst, book_con_dtls_id, item_id, body_part, embellishment_type, description, color_id, size_id, qnty, rate, amount,cutting_no,domestic_amount";
			...
			for($j=0; $j<count($dtls_data); $j++)
			{
				...
				if (str_replace("'", "", trim($sizename)) != "") {
					if (!in_array(str_replace("'", "", trim($sizename)),$new_array_size)){
						$size_id = return_id( str_replace("'", "", trim($sizename)), $size_library_arr, "lib_color", "id,size_name","204");
				...
				$data_array3.="(".$id3.",".$id1.",".$hid_order_id.",'".$new_job_no[0]."',".$book_con_dtls_id.",".$$cboGmtsItem.",".$$cboBodyPart.",".$$cboembtype.",".$description.",'".$color_id."','".$size_id."',".$qty.",".$rate.",".$amount.",".$cutting_no.",".$domistic_amount.")";
		...
		{
			if($flag==1 && str_replace("'","",$hid_booking_type)==1 )
			{
	...
	else
	{
		...
		$field_array3="id, mst_id, order_id, job_no_mst, book_con_dtls_id, item_id, body_part, embellishment_type, description, color_id, size_id, qnty, rate, amount,cutting_no,domestic_amount";
		$field_array4="order_id*book_con_dtls_id*item_id*body_part*embellishment_type*description*color_id*size_id*qnty*rate*amount*cutting_no*domestic_amount";
		...
		{			
			...
			*/
			for($j=0; $j<count($dtls_data); $j++)
			{
				...
				{
					if ($add_commadtls!=0) $data_array3 .=",";
					$data_array3.="(".$id3.",".$hdnDtlsUpId.",".$hid_order_id.",".$txt_job_no.",".$book_con_dtls_id.",".$$cboGmtsItem.",".$$cboBodyPart.",".$$cboembtype.",".$description.",'".$color_id."','".$size_id."',".$qty.",".$rate.",".$amount.",".$cuttingno.",".$domistic_amount.")";
					$id3=$id3+1; $add_commadtls++;
				}
				else if($breakId!=0)
				{
					$data_array4[$breakId]=explode("*",("".$hid_order_id."*".$book_con_dtls_id."*".$$cboGmtsItem."*".$$cboBodyPart."*".$$cboembtype."*".$description."*'".$color_id."'*'".$size_id."'*".$qty."*".$rate."*".$amount."*".$cuttingno."*".$domistic_amount."*".$user_id."*'".$pc_date_time."'"));
...
{
	$nameArray=sql_select( "SELECT id, embellishment_job, company_id, location_id, party_id, currency_id, party_location, delivery_date, rec_start_date, rec_end_date, receive_date, within_group, party_location, order_id, order_no,exchange_rate,booking_type from subcon_ord_mst where embellishment_job='$data' and status_active=1 and  is_deleted=0" );
...
{	
	...
	/*$sql= "SELECT $wo_year as year, a.id, a.booking_type, a.booking_no, a.booking_no_prefix_num, a.company_id, a.buyer_id, a.booking_date, a.currency_id, $pre_cost_trims_cond as pre_cost_trims_id, $gmts_item_cond as gmts_item, $po_id_cond as po_id, 1 as booking_type from  wo_booking_mst a, wo_booking_dtls b, wo_pre_cost_embe_cost_dtls c where a.booking_no=b.booking_no and b.pre_cost_fabric_cost_dtls_id=c.id and a.booking_type=6 and a.status_active=1 and c.emb_name=1 and a.lock_another_process!=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $booking_date $company $party_cond $woorder_cond $year_cond $po_idsCond group by a.insert_date, a.id, a.booking_type, a.booking_no, a.booking_no_prefix_num, a.company_id, a.buyer_id, a.booking_date, a.currency_id 
		UNION ALL
		SELECT $wo_year as year, a.id, a.booking_type, a.booking_no, a.booking_no_prefix_num, a.company_id, a.buyer_id, a.booking_date, a.currency_id, '0' as pre_cost_trims_id, $gmts_item_id_cond as gmts_item, '0' as po_id, 2 as booking_type  from wo_non_ord_embl_booking_mst a, wo_non_ord_embl_booking_dtls b where a.booking_no=b.booking_no and a.booking_type=6 and a.entry_form_id=399 and a.status_active=1 and b.status_active=1 $booking_date $company $party_cond $woorder_cond $year_cond group by a.insert_date, a.id, a.booking_type, a.booking_no, a.booking_no_prefix_num, a.company_id, a.buyer_id, a.booking_date, a.currency_id";*/
		//
    ...
    <div style="width:1140px; max-height:270px;overflow-y:scroll;" >	 
        ...
        <tbody>
            ...
            {  
				...
				if($row[csf('booking_type')]==1) $booking_type='With Order'; else $booking_type='Without Order';
                ?>
                <tr bgcolor="<? echo $bgcolor; ?>" onClick='js_set_value("<? echo $row[csf('id')].'_'.$row[csf('booking_no')].'_'.$row[csf('currency_id')].'_'.$row[csf('booking_type')]; ?>")' style="cursor:pointer" >
                    <td width="30"><? echo $i; ?></td>
                    <td width="40" align="center"><? echo $row[csf('year')]; ?></td>
                    <td width="40" align="center"><? echo $row[csf('booking_no_prefix_num')]; ?></td>
                    ...
                    <td width="150" style="word-break:break-all"><? echo $gmts_item_name; ?></td>
...
if( $action=='order_dtls_list_view' ) 
{
	...
	if($data[0]==2)
	{
		$qry_result=sql_select( "SELECT id, mst_id, order_id, job_no_mst, book_con_dtls_id, item_id, body_part, embellishment_type, description, color_id, size_id, qnty, rate, amount,cutting_no from subcon_ord_breakdown where job_no_mst='$data[1]'");	
		...
		foreach ($qry_result as $row)
		{
			...
			$data_dreak_arr[$row[csf('mst_id')]].=$row[csf('description')].'_'.$color_library[$row[csf('color_id')]].'_'.$size_arr[$row[csf('size_id')]].'_'.$row[csf('qnty')].'_'.$row[csf('rate')].'_'.$row[csf('amount')].'_'.$row[csf('book_con_dtls_id')].'_'.$row[csf('id')].'_'.$row[csf('cutting_no')].',';
			$add_comma++;
	...
	if($data[2]==1)
	{
		...
		if($data[0]==2)
		{
			...
			$sql="select a.id as booking_dtls_id, a.embl_cost_dtls_id as embe_cost_dtls_id , a.booking_no, a.booking_type,a.delivery_date,a.gmt_item_id as gmt_item,a.req_id,a.req_no,a.req_booking_no,a.emb_name,a.emb_type,a.body_part_id,a.uom_id, a.sensitivity,a.cons_break_down,a.wo_qnty, a.exchange_rate,a.rate, a.amount, (a.wo_qnty) AS wo_qnty,a.rate,a.amount,b.fin_fab_qnty as req_qty,b.amount as req_amt , 0 as po_break_down_id
	...
	if(count($data_array) > 0)
	{
		foreach($data_array as $row)
		{
			...
			<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
				...
				<td><input name="txtOrderQuantity[]" id="txtOrderQuantity_<? echo $tblRow; ?>" value="<? echo number_format($wo_qnty,4,'.',''); ?>" class="text_boxes_numeric" type="text"  style="width:60px" onClick="openmypage_order_qnty(1,'<? echo $qty_popup_data; ?>',<? echo $tblRow; ?>,<? echo $data[4]; ?>)" placeholder="Click To Search" readonly /></td> 
				<td><?
				...
				<td><input name="txtAmount[]" id="txtAmount_<? echo $tblRow; ?>" value="<? echo number_format($row[csf('amount')],4,'.',''); ?>" type="text" style="width:70px"  class="text_boxes_numeric" readonly /></td> 
                 <td align="center"><input type="text" name="txtdomisticamount_<? echo $tblRow; ?>" id="txtdomisticamount_<? echo $tblRow; ?>" class="text_boxes_numeric" style="width:90px;" value="<? echo number_format($domestic_amount,4,'.',''); ?>" readonly /></td>
	...
	else
	{
		...
		<tr id="row_1">
            ...
            <td><input name="txtOrderQuantity[]" id="txtOrderQuantity_1" class="text_boxes_numeric" type="text"  style="width:60px" onClick="openmypage_order_qnty(1,'0',1,'0')" placeholder="Click To Search" readonly /></td>
            <td><? 
...
if($action=="order_qty_popup")
{
	...
	function fnc_close()
	{
		...
		for(var i=1; i<=tot_row; i++)
		{
			...
			if(data_break_down=="")
			{
				data_break_down+=$('#txtdescription_'+i).val()+'_'+$('#txtcolor_'+i).val()+'_'+$('#txtsize_'+i).val()+'_'+$('#txtorderquantity_'+i).val()+'_'+$('#txtorderrate_'+i).val()+'_'+$('#txtorderamount_'+i).val()+'_'+$('#hidbookingconsid_'+i).val()+'_'+$('#hiddenid_'+i).val()+'_'+$('#txtcuttinno_'+i).val();
			}
			else
			{
				data_break_down+="__"+$('#txtdescription_'+i).val()+'_'+$('#txtcolor_'+i).val()+'_'+$('#txtsize_'+i).val()+'_'+$('#txtorderquantity_'+i).val()+'_'+$('#txtorderrate_'+i).val()+'_'+$('#txtorderamount_'+i).val()+'_'+$('#hidbookingconsid_'+i).val()+'_'+$('#hiddenid_'+i).val()+'_'+$('#txtcuttinno_'+i).val();
			}
		}
		...
		?>
			...
			<table class="rpt_table" width="700px" cellspacing="0" cellpadding="0" rules="all" id="tbl_share_details_entry">
				...
				<tbody>
					...
					<?
					$sql_break_down="SELECT id, mst_id, order_id, job_no_mst, book_con_dtls_id, item_id, body_part, embellishment_type, description, color_id, size_id, qnty, rate, amount,cutting_no from subcon_ord_breakdown where mst_id='$hdnDtlsUpdateId' and job_no_mst='$job_no'";
					...
					foreach($data_break_down as $row)
					{
						...
						if($data_dreak=="") $data_dreak.=$row[csf('description')].'_'.$row[csf('color_id')].'_'.$row[csf('size_id')].'_'.$row[csf('qnty')].'_'.$row[csf('rate')].'_'.$row[csf('amount')].'_'.$row[csf('id')].'_'.$row[csf('id')].'_'.$row[csf('cutting_no')];
						else $data_dreak.='__'.$row[csf('description')].'_'.$row[csf('color_id')].'_'.$row[csf('size_id')].'_'.$row[csf('qnty')].'_'.$row[csf('rate')].'_'.$row[csf('amount')].'_'.$row[csf('id')].'_'.$row[csf('id')].'_'.$row[csf('cutting_no')];
					}
					...
					{	
						...
						{ // with Order
							...
							foreach($data_arr as $row){
								...
								if($data_dreak=="") $data_dreak.=$row[csf('description')].'_'.$row[csf('color_number_id')].'_'.$row[csf('gmts_sizes')].'_'.$row[csf('requirment')].'_'.$row[csf('rate')].'_'.$row[csf('amount')].'_'.$row[csf('id')].'_'.$break_down_arr[$row[csf('id')]].'_'.$cutting_no_arr[$row[csf('id')]];
								else $data_dreak.='__'.$row[csf('description')].'_'.$row[csf('color_number_id')].'_'.$row[csf('gmts_sizes')].'_'.$row[csf('requirment')].'_'.$row[csf('rate')].'_'.$row[csf('amount')].'_'.$row[csf('id')].'_'.$break_down_arr[$row[csf('id')]].'_'.$cutting_no_arr[$row[csf('id')]];
							}
						}
						else
						{
							...
							if ( count($data_arr)>0)
							{
								...
								foreach( $data_arr as $row )
								{
									...
									if($data_dreak=="") $data_dreak.=$description.'_'.$row[csf('color_number_id')].'_'.$row[csf('size_number_id')].'_'.$txtwoq_cal.'_'.$rate.'_'.$amount.'_'.$embl_book_cons_dtls_id.'_0_0';
									else $data_dreak.='__'.$description.'_'.$row[csf('color_number_id')].'_'.$row[csf('size_number_id')].'_'.$txtwoq_cal.'_'.$rate.'_'.$amount.'_'.$embl_book_cons_dtls_id.'_0_0';
									$i++;
								}
							}
						}
