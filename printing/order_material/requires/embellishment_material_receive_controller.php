...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   
	{
		...
		$field_array="id, entry_form, prefix_no, prefix_no_num, sys_no, trans_type, company_id, location_id, party_id, chalan_no, subcon_date, within_group, embl_job_no,from_company_name,from_location_name,remarks,boe_mushak_challan_no,boe_mushak_challan_date,issue_chalan_no,challan_id,inserted_by, insert_date, status_active, is_deleted";
		$data_array="(".$id.",'205','".$new_receive_no[1]."','".$new_receive_no[2]."','".$new_receive_no[0]."','".$trans_Type."',".$cbo_company_name.",".$cbo_location_name.",".$cbo_party_name.",".$txt_receive_challan.",".$txt_receive_date.",".$cbo_within_group.",".$txt_job_no.",".$cbo_from_company_name.",".$cbo_from_location_name.",".$txt_receive_remarks.",".$txt_boe_mushak_challan_no.",".$txt_boe_mushak_challan_date.",".$printing_issue_challan_no.",".$hid_challan_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";  
		...
		$field_array2="id, mst_id, emb_name_id, quantity, uom, job_dtls_id, job_break_id, buyer_po_id, remarks,challan_id,color_size_break_down_id, inserted_by, insert_date, status_active, is_deleted";
		...
		for($i=1; $i<=$total_row; $i++)
		{
			...
			$colorsizebreakdownid			= "colorsizebreakdownid_".$i;
			$updatedtlsid		= "updatedtlsid_".$i;
			
			if ($add_commaa!=0) $data_array2 .=",";
			 
			$data_array2.="(".$id1.",'".$id."',".$$cboProcessName.",".$$txtreceiveqty.",".$$cbouom.",".$$ordernoid.",".$$breakdownid.",".$$txtbuyerPoId.",".$$txtremarks.",".$hid_challan_id.",".$$colorsizebreakdownid.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		if(str_replace("'","",$hid_challan_id)!="")///// chsllsn status
		 {
	...
	else if ($operation==1)   // Update Here
	{
		...
		else
		{
			...
			$field_array="location_id*party_id*chalan_no*subcon_date*embl_job_no*from_company_name*from_location_name*remarks*boe_mushak_challan_no*boe_mushak_challan_date*updated_by*update_date";
			$data_array="".$cbo_location_name."*".$cbo_party_name."*".$txt_receive_challan."*".$txt_receive_date."*".$txt_job_no."*".$cbo_from_company_name."*".$cbo_from_location_name."*".$txt_receive_remarks."*".$txt_boe_mushak_challan_no."*".$txt_boe_mushak_challan_date."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"; 
...
if($action=="create_receive_search_list_view")
{
	...
	else
	{
		...
		if($db_type==2 && $spo_ids!="") $spo_ids = $spo_ids->load();
		// if ( $spo_ids!="") $spo_idsCond=" and b.job_dtls_id in ($spo_ids)"; else $spo_idsCond="";
		if ($spo_ids!="")
		{
			$spo_ids=explode(",",$spo_ids);
			$po_idsCond=""; $poIdsCond="";
			foreach($spo_ids as $row)
			{
				$po_id_arr[$row]=$row;
			}
			
			$po_idsCond=where_con_using_array($po_id_arr,0,'b.job_dtls_id');
			$poIdsCond=where_con_using_array($po_id_arr,0,'a.id');
		}

		$style_sql ="SELECT a.id,a.buyer_po_no,a.buyer_style_ref from subcon_ord_dtls a where a.status_active =1 and a.is_deleted =0 $poIdsCond";
...
{
	$nameArray=sql_select( "SELECT id, sys_no, company_id, location_id, party_id, subcon_date, chalan_no,within_group, embl_job_no,from_company_name,from_location_name,remarks,challan_id, issue_chalan_no, boe_mushak_challan_no, boe_mushak_challan_date from sub_material_mst where id='$data' and status_active =1 and is_deleted=0" ); 
	foreach ($nameArray as $row)
	{	
		...
		echo "document.getElementById('printing_issue_challan_no').value		= '".$row[csf("issue_chalan_no")]."';\n"; 
		echo "document.getElementById('hid_challan_id').value		= '".$row[csf("challan_id")]."';\n"; 
		echo "$('#printing_issue_challan_no').attr('disabled','true')".";\n";
                ...
                <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+<? echo $data[3];?>+'_'+document.getElementById('cbo_year_selection').value, 'create_job_search_list_view', 'search_div', 'embellishment_material_receive_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:70px;" />
                            </td>
                        </tr>
                        <tr>
                ...
                <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+<? echo $data[3];?>+'_'+document.getElementById('cbo_year_selection').value, 'create_printing_to_issue_challan_search_list_view', 'search_div', 'embellishment_material_receive_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:70px;" />
                            </td>
                        </tr>
                        <tr>
                            <td colspan="6" align="center" valign="middle">
...
if ($action=="matarial_receive_print")
{
	...
	$sql= "SELECT id, sys_no, company_id, location_id, party_id, subcon_date, chalan_no,within_group, embl_job_no,from_company_name,from_location_name,remarks,boe_mushak_challan_no,boe_mushak_challan_date from sub_material_mst where id='$data[1]' and status_active =1 and is_deleted=0";
        ...
        <tr>
         ...
         <tr>
			...
			<td><strong>BOE/Mushak Challan No</strong></td>
           	<td  ><strong>: </strong><? echo $dataArray[0][csf('boe_mushak_challan_no')]; ?></td>
       </tr>
	   <tr>
	        <td><strong>BOE/Mushak Challan Date</strong></td>
           	<td><strong>: </strong><? echo change_date_format($dataArray[0][csf('boe_mushak_challan_date')]); ?></td>
	   </tr>
    </table>
	<div style="width:100%;">
...
if ($action=="matarial_receive_print2")
{
    extract($_REQUEST);
	$data=explode('*',$data);
	//print_r ($data); die;
	...
	//$k=0; 
	$i=1; $pre_recei_qty=0; $receive_qty=0; $orderQty=0;
	$num_rowss=count($sql_result);
	foreach ($sql_result as $row)
	{
		//$k++;
		...
		$quantity=0; $dtlsup_id=""; $balanceQty=0; $prerec_qty=0; $preissue_qty=0; $orderQty=0; $remarks='';$uom=0;
		$prerec_qty=$pre_qty_arr[$row[csf("po_id")]][$row[csf("breakdown_id")]]['qty'];
		$preissue_qty=$pre_issue_qty_arr[$row[csf("po_id")]][$row[csf("breakdown_id")]]['qty'];
	...
	?>
		<tr bgcolor="<? echo $bgcolor; ?>">
			...
			<td align="center"><? echo $garments_item[$row[csf("gmts_item_id")]]; ?></td>
            <td align="center"><? echo $material_description; ?></td>
            <td align="center"><? echo $body_part[$row[csf("body_part")]]; ?></td>
            <td align="center"><? echo $color_arrey[$row[csf("color_id")]]; ?></td>
			<td align="center"><? echo $size_arrey[$row[csf("size_id")]]; ?></td>
			<td align="right"><? echo number_format($orderQty,2); ?></td>
	...
	if(count($sql_bundle_copy)!=0)
	{
		foreach($sql_bundle_copy as $inf)
		{
			...
			foreach($color_sizeID_arr as $val)
			{
				...
				$pdf->Code40($i, $j+1.2, "Sty- ". $buyer_style_ref, $ext = true, $cks = false, $w =0.15, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i, $j+3.8, "Buy- ".$buyer_buyer.", PO- ".$order_no.", Cut No- ".$val[csf("cut_no")] , $ext = true, $cks = false, $w =0.15, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i, $j+6, "Color- ".$color_library[$color_id], $ext = true, $cks = false, $w =0.15, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i, $j+8.5, "BNDL Qty- ".$val[csf("dtls_bndl_qty")].',  '. $inf[csf("bundle_use_for")].", Size- ". $size_arr[$size_id], $ext = true, $cks = false, $w = 0.2, $h = 1, $wide=true,true,7);
				//$pdf->Code40($i, $j+22, $val[csf("bundle_no")],$w =0.2, $h = 8);
