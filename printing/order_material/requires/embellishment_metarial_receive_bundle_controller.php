...
if ($action == "challan_duplicate_check")
{
    $data=explode("__",$data);
   	/*$sql_bundle_no="SELECT c.bundle_no_prefix_num,c.bundle_no,c.barcode_no,b.sys_no from subcon_ord_mst a, sub_material_mst b, prnting_bundle_dtls c, printing_bundle_receive_dtls d where a.embellishment_job=b.embl_job_no and b.id=c.item_rcv_id and c.barcode_no='$data[0]' and  d.entry_form=614 and  d.bundle_dtls_id=c.id and d.job_id=b.id and d.order_id=a.id and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0  and c.status_active =1 and c.is_deleted =0";
	...
	$sql_next_process="SELECT e.recv_number from prnting_bundle_dtls c, printing_bundle_receive_dtls d, printing_bundle_receive_mst e where e.id=d.mst_id and c.challan_no='$data[0]' and d.entry_form=614 and  d.bundle_dtls_id=c.id and e.status_active =1 and e.is_deleted =0 and c.status_active =1 and c.is_deleted =0 and d.status_active =1 and d.is_deleted =0";
	$next_process_data =sql_select($sql_next_process); 

	foreach ($next_process_data as $row) 
	{
		echo "Already Issue found, System id " . $row[csf('recv_number')] . ".";
	}
	exit();
...
if($action=="create_receive_search_list_view")
{
	...
	if($db_type==0)
	{ 
		if ($data[2]!="" &&  $data[3]!="") $recieve_date = "and b.production_date between '".change_date_format($data[2],'yyyy-mm-dd')."' and '".change_date_format($data[3],'yyyy-mm-dd')."'"; else $recieve_date ="";
	}
	else
	{
		if ($data[2]!="" &&  $data[3]!="") $recieve_date = "and b.production_date between '".change_date_format($data[2], "", "",1)."' and '".change_date_format($data[3], "", "",1)."'"; else $recieve_date ="";
	}
	...
	if($search_type==1)
	{
		if($search_str!="") 
		{
			...
			else if($search_by==2) $search_com_cond="and g.order_no='$search_str'";
			
			if ($search_by==3) $job_cond=" and f.job_no = '$search_str' ";
			else if ($search_by==4){
	...
	else if($search_type==4 || $search_type==0)
	{
		if($search_str!="")
		{
			...
			else if($search_by==2) $search_com_cond="and g.order_no like '%$search_str%'";  
			
			if ($search_by==3) $job_cond=" and f.job_no like '%$search_str%'";  
			else if ($search_by==4){
	...
	//and d.id not in (select barcode_no from tmp_barcode_no where userid=$user_id and entry_form=614) 
	...
	if($db_type==2) {$year_cond=" and to_char(b.insert_date,'YYYY')=$data[11]";}
		 ...
		 $sql="SELECT c.id as bundle_dtls_id,d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, $year_field as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.cut_no,c.bundle_no,c.barcode_no,c.production_qnty,c.is_rescan, e.po_number,b.company_id,b.location,b.production_source,b.serving_company,b.embel_name, b.embel_type,b.wo_order_no,b.challan_no,b.wo_order_id,g.id as job_dtls_id,b.id as bundl_mst_id,e.job_id,g.buyer_po_id,e.grouping  from pro_garments_production_mst b,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f,subcon_ord_dtls g,subcon_ord_breakdown h  where  b.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_id=f.id and d.job_id=f.id and b.wo_order_no=g.order_no  and c.production_type=2 and  g.buyer_po_id=e.id and h.mst_id=g.id  and  h.order_id=b.wo_order_id and  g.gmts_item_id=b.item_number_id and  g.embl_type=b.embel_type  and  h.color_id=d.color_number_id   and  h.size_id=d.size_number_id   and h.status_active=1 and h.is_deleted=0   and c.status_active=1 and c.is_deleted=0 and c.print_receive_status=0 $job_cond $buyer_po_cond $buyer_style_cond  $company $challan_no_cond $recieve_date $year_cond $search_com_cond $withinGroup  order by  c.cut_no,length(c.bundle_no) asc, c.bundle_no asc ";  
		
		
	$result = sql_select($sql);
	foreach($result as $row)
	{
		$job_dtls_id_arr[$row[csf('job_dtls_id')]] =$row[csf('po_id')];
	...
	if(count($job_dtls_id_arr)>0)
	{
		...
		if($db_type==2 && count($job_dtls_id_arr)>999)
		{
			...
			foreach($job_dtls_id_arr_chunk as $chunk_arr)
			{
				...
				$jobIdCond.=" b.buyer_po_id in($chunk_arr_value) or ";
			}
		...
		{
			$job_dtls_id_cond=" and b.buyer_po_id in($job_dtls_ids)";
		}

		  $sql_job="SELECT a.within_group,a.company_id , a.id, a.embellishment_job, a.order_id, a.order_no, b.id as po_id, c.id as breakdown_id, c.description, c.color_id, c.size_id, c.qnty, b.order_uom,b.gmts_item_id,b.buyer_buyer,b.buyer_style_ref,b.main_process_id,b.embl_type,b.body_part,b.buyer_po_id
		...
		foreach ($dataArray as $row) 
		{
			...
			$job_arr[$row[csf('po_id')]]['order_id']=$row[csf('id')];
	...
	if(count($bundle_dtls_id_arr)>0)
	{
		...
		$sql_issue=sql_select("SELECT id,mst_id,company_id,entry_form,order_id,job_dtls_id,color_size_id,job_id,buyer_po_id,bundle_mst_id,bundle_dtls_id,challan_no,quantity,issue_dtls_id from printing_bundle_receive_dtls where entry_form=614 and status_active=1 and is_deleted=0 $bundle_dtls_id_cond");
						 ...
						 if($issued_bundle_barcode[$row[csf('bundle_dtls_id')]] =="" )
						 {
							...
							<tr bgcolor="<? echo $bgcolor; ?>">
								<td width="50" align="center">
									...
									<input type="hidden" name="rcvID[]" id="rcvID_<? echo $i; ?>" value="<? echo $row[csf("job_id")]; ?>">
									<input type="hidden" name="rcvDtlsID[]" id="rcvDtlsID_<? echo $i; ?>" value="<? echo $row[csf("buyer_po_id")]; ?>"  >
								...
								<td width="60">
									<? echo create_drop_down( "cboWithinGroup_".$i, 50, $yes_no,"", 1, "-- Select --",$row[csf("production_source")], "",1,'','','','','','',"cboWithinGroup[]"); ?>
								</td>
								<td width="100">
								...
								<td width="80">
									<input name="txtInterRef[]" id="txtInterRef_<? echo $i; ?>" type="text" class="text_boxes" value="<? echo $row[csf("grouping")]; ?>"  style="width:65px" disabled />
								</td>
								<td width="60">
									<input name="txtIssueCh[]" id="txtIssueCh_<? echo $i; ?>" type="text" class="text_boxes" value="<? echo $row[csf("challan_no")] ?>"  style="width:47px"  readonly />
								</td>
								<td width="100">
								...
								<td width="80" align="right">
									<? echo $row[csf("production_qnty")]; ?>
									<input type="hidden" name="txtBundleQty[]" id="txtBundleQty_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo $row[csf("production_qnty")]; ?>" style="width:70px" readonly />
									<input name="txtProdQty[]" id="txtProdQty_<? echo $i; ?>" type="hidden" class="text_boxes_numeric" value="" onKeyUp="fnc_total_calculate ();" />
								</td>
								
							</tr>
							<?
							$totalBndlQty+=$row[csf("production_qnty")];
							$i++;
						 }
	                }
	                ?>
    ...
    <? //die;
    /* $r_id3=execute_query("delete from tmp_barcode_no where userid=$user_id and entry_form=614");
	if($db_type==0)
	{
		if($r_id3)
		{
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   
	{
		...
		$id=return_next_id("id","printing_bundle_receive_mst",1) ;
		$id1=return_next_id("id","printing_bundle_receive_dtls",1) ;
		...
		$new_iss_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name), '', 'PMRB', date("Y",time()), 5, "select recv_num_prefix,recv_num_prefix_no from printing_bundle_receive_mst where entry_form=614 and company_id=$cbo_company_name $insert_date_con order by id desc ", "recv_num_prefix", "recv_num_prefix_no" ));
		
		$field_array="id,company_id,entry_form , recv_num_prefix, recv_num_prefix_no, recv_number ,recv_date,floor_id,table_id,machine_id,within_group,inserted_by, insert_date, status_active, is_deleted";
		$data_array="(".$id.",".$cbo_company_name.",614,'".$new_iss_no[1]."','".$new_iss_no[2]."','".$new_iss_no[0]."',".$txt_receive_date.",".$cbo_floor.",".$cbo_table.",".$cbo_machine_name.",".$cbo_within_group.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		$recv_number=$new_iss_no[0]; 

		$field_array1="id,mst_id,company_id,entry_form,order_id,job_dtls_id,color_size_id,job_id,buyer_po_id,bundle_mst_id,bundle_dtls_id,challan_no,quantity,barcode_no, inserted_by, insert_date, status_active, is_deleted";
		...
		$all_wo_ids=implode(",",array_unique(explode(",",chop($all_wo_id,","))));
	   	$sql_prev_issue="SELECT a.recv_number,b.bundle_dtls_id from printing_bundle_receive_mst a, printing_bundle_receive_dtls b where a.id=b.mst_id and a.entry_form=614 and b.entry_form=614 and b.order_id in ($all_wo_ids) and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0"; 
		$prev_issue_res =sql_select($sql_prev_issue); 
		foreach ($prev_issue_res as $row) 
		{
			$prev_issue_arr[$row[csf('bundle_dtls_id')]]=$row[csf('recv_number')];
		}
		...
		for($i=1; $i<=$total_row; $i++)
		{
			...
			$data_array1.="(".$id1.",".$id.",".$$cboCompanyId.",614,".$$woID.",".$$woDtlsID.",".$$woBreakID.",".$$rcvID.",".$$rcvDtlsID.",".$$bundleMstID.",".$$bundleDtlsID.",".$$txtIssueCh.",".$$txtProdQty.",".$$txtBarcodeNo.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
		...
		$flag=1;
		//echo "INSERT INTO printing_bundle_receive_dtls (".$field_array1.") VALUES ".$data_array1; die;
		$rID=sql_insert("printing_bundle_receive_mst",$field_array,$data_array,0);
		if($flag==1 && $rID==1) $flag=1; else $flag=0;

		$rID1=sql_insert("printing_bundle_receive_dtls",$field_array1,$data_array1,0);
		if($flag==1 && $rID1==1 && $rID==1) $flag=1; else $flag=0;
		
		if(count($id_arr_trans)>0)
		{
			$field_array_trans_update="print_receive_status";
			$rID4=execute_query(bulk_update_sql_statement("pro_garments_production_dtls","id",$field_array_trans_update,$data_array_trans_update,$id_arr_trans),1);
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");
				echo "0**".str_replace("'",'',$recv_number)."**".str_replace("'",'',$id);
			}
			else
			{
				mysql_query("ROLLBACK"); 
				echo "10**".str_replace("'",'',$recv_number)."**".str_replace("'",'',$id);
			}
		}
		else if($db_type==2)
		{
	...
	else if ($operation==1)   // Update Here
	{
		...
		$id1=return_next_id("id","printing_bundle_receive_dtls",1) ;
		$field_array="recv_date*floor_id*table_id*machine_id*within_group*updated_by*update_date";
		$data_array="".$txt_receive_date."*".$cbo_floor."*".$cbo_table."*".$cbo_machine_name."*".$cbo_within_group."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"; 
		...
		$all_wo_ids=implode(",",array_unique(explode(",",chop($all_wo_id,","))));
	   	$sql_prev_issue="SELECT a.recv_number,b.bundle_dtls_id from printing_bundle_receive_mst a, printing_bundle_receive_dtls b where a.id=b.mst_id and a.entry_form=614 and b.entry_form=614 and b.order_id in ($all_wo_ids) and a.id!=$update_id and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0"; 
		$prev_issue_res =sql_select($sql_prev_issue); 
		foreach ($prev_issue_res as $row) 
		{
			$prev_issue_arr[$row[csf('bundle_dtls_id')]]=$row[csf('recv_number')];
		...
		$sql_production_check = sql_select("SELECT a.recv_number as PRODUCTION_NO from printing_bundle_receive_dtls b, printing_bundle_receive_dtls c, printing_bundle_receive_mst a where b.entry_form=614 and b.mst_id=$update_id and b.status_active =1 and b.is_deleted =0 and b.bundle_dtls_id = c.bundle_dtls_id and c.entry_form=497 and c.status_active=1 and a.status_active=1 and c.mst_id=a.id and a.entry_form= 497");
		...
		for($i=1; $i<=$total_row; $i++)
		{
			...
			{
				if ($add_commaa!=0) $data_array1 .=",";
				$data_array1.="(".$id1.",".$update_id.",".$$cboCompanyId.",614,".$$woID.",".$$woDtlsID.",".$$woBreakID.",".$$rcvID.",".$$rcvDtlsID.",".$$bundleMstID.",".$$bundleDtlsID.",".$$txtIssueCh.",".$$txtProdQty.",".$$txtBarcodeNo.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
				$id_arr_trans[]=str_replace("'",'',$$bundleDtlsID);
				$data_array_trans_update[str_replace("'",'',$$bundleDtlsID)] = explode("*",(1)); 
				$id1=$id1+1; $add_commaa++;
			}
		...
		$flag=1;
		$rID=sql_update("printing_bundle_receive_mst",$field_array,$data_array,"id",$update_id,0); 
		if($rID==1 && $flag==1) $flag=1; else $flag=0;	

		if($flag==1)
		{
			$rID1=sql_multirow_update("printing_bundle_receive_dtls",$field_array_status,$data_array_status,"mst_id",$update_id,0);
		...
		if($data_array1!="" && $flag==1)
		{
			...
			$rID2=sql_insert("printing_bundle_receive_dtls",$field_array1,$data_array1,1);
		...
		if($data_arr_up!=""  && $flag==1)
		{
			...
			$rID3=execute_query(bulk_update_sql_statement( "printing_bundle_receive_dtls", "id", $field_arr_up,$data_arr_up,$id_arr_iss),1);
		...
		//echo "10**".$data_delete; die;
		    ...
		    if($remove_id!="")
			{
				$field_array_del_sales="print_receive_status";
				$data_array_del_sales=0;
				$rID9=sql_multirow_update("pro_garments_production_dtls",$field_array_del_sales,$data_array_del_sales,"id",$remove_id,1);
	...
	else if ($operation==2)   // delete Here
	{
		...
		$sql_next_trans="SELECT a.issue_number,b.rcv_dtls_id from printing_bundle_issue_mst a, printing_bundle_issue_dtls b where a.id=b.mst_id and a.entry_form=495 and b.entry_form=495 and b.rcv_dtls_id in ($all_wo_ids) and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0";
		$sql_next_trans_res =sql_select($sql_next_trans); 
		foreach ($sql_next_trans_res as $row) 
		{
			$next_trans_arr[$row[csf('rcv_dtls_id')]]=$row[csf('issue_number')];
		}
     //echo "10**";
     //echo count($sql_next_trans_res); die;
		$field_array_status="updated_by*update_date*status_active*is_deleted";
		$data_array_status=$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*0*1";
		...
		for($i=1; $i<=$total_row; $i++)
		{
    		...
    		if($next_trans)
			{
    			echo "121**Issue Found. System ID : $next_trans "; die;
    		}
			if(str_replace("'","",$$updatedtlsid)!="")
			{
		...
		// echo "10**".$bundleDtlsIDDeletes; die;
			$rID = sql_multirow_update("printing_bundle_receive_dtls", $field_array_status, $data_array_status, "id", $updatedtlsDelIds, 1);
			if($rID==1 && $flag==1) $flag=1; else $flag=0;	
			$rID2=sql_update("printing_bundle_receive_mst",$field_array_status,$data_array_status,"id",$update_id,0); 
			if($rID2==1 && $flag==1) $flag=1; else $flag=0;	
				 //$remove_id=chop($data_delete,",");
			$field_array_del_sales="print_receive_status";
			$data_array_del_sales=0;
			$rID9=sql_multirow_update("pro_garments_production_dtls",$field_array_del_sales,$data_array_del_sales,"id",$bundleDtlsIDDeletes,1);
			if($rID9==1 && $flag==1) $flag=1; else $flag=0;	
		}
		else
		{
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");
				echo "2**".str_replace("'",'',$txt_receive_no)."**".str_replace("'",'',$update_id);
			}
			else
			{
				mysql_query("ROLLBACK"); 
...
if( $action == 'issue_item_details_update')
{
	...
	$mst_id=$data;
	$con = connect();
	  $sql_issue="SELECT id,mst_id,company_id,entry_form,order_id,job_dtls_id,color_size_id,job_id,buyer_po_id,bundle_mst_id,bundle_dtls_id,challan_no,quantity
			from printing_bundle_receive_dtls where mst_id=$mst_id and  entry_form=614 and status_active=1 and is_deleted=0 order by id ASC";
	
	$issue_result =sql_select($sql_issue);
	
	$bundle_dtls_id=""; $order_id='';
	foreach($issue_result as $iss_id)
	{
		...
		$issue_item_arr[$iss_id[csf('job_dtls_id')]][$iss_id[csf('color_size_id')]][$iss_id[csf('bundle_dtls_id')]]['challan_no']=$iss_id[csf('challan_no')];
	 	$issue_item_arr[$iss_id[csf('job_dtls_id')]][$iss_id[csf('color_size_id')]][$iss_id[csf('bundle_dtls_id')]]['quantity']=$iss_id[csf('quantity')];
	 	$issue_item_arr[$iss_id[csf('job_dtls_id')]][$iss_id[csf('color_size_id')]][$iss_id[csf('bundle_dtls_id')]]['updateIdDtls']=$iss_id[csf('id')];
		$order_id .=$iss_id[csf('order_id')].',';
	}
	//print_r($issue_item_arr);
	$wo_ids=implode(",",array_unique(explode(",",(chop($order_id,',')))));
	if($db_type==0)
	{
		if($r_id2)
		{
	...
	////
		...
		$sql="SELECT c.id as bundle_dtls_id,d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, $year_field as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.cut_no,c.bundle_no,c.barcode_no,c.production_qnty,c.is_rescan, e.po_number,b.company_id,b.location,b.production_source,b.serving_company,b.embel_name, b.embel_type,b.wo_order_no,b.challan_no,b.wo_order_id,g.job_dtls_id as job_dtls_id,b.id as bundl_mst_id,e.job_id,g.buyer_po_id,e.grouping  from pro_garments_production_mst b,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f,printing_bundle_receive_dtls g, tmp_poid e where  b.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_id=f.id and d.job_id=f.id  and c.production_type=2 and  g.bundle_dtls_id=c.id and e.poid=c.id and c.status_active=1 and c.is_deleted=0 and c.print_receive_status=1  group by c.id,d.id, e.id, f.job_no_prefix_num, f.insert_date, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.cut_no,c.bundle_no,c.barcode_no,c.production_qnty,c.is_rescan, e.po_number,b.company_id,b.location,b.production_source,b.serving_company,b.embel_name, b.embel_type,b.wo_order_no,b.challan_no,b.wo_order_id,g.job_dtls_id,b.id,e.job_id,g.buyer_po_id,e.grouping   order by  c.cut_no,length(c.bundle_no) asc, c.bundle_no asc "; 
		
	 

	////

	/*$sql="SELECT a.id as bundl_mst_id, a.company_id, a.po_id, a.order_breakdown_id, a.item_rcv_dtls_id, a.pcs_per_bundle, a.no_of_bundle, a.bundle_qty, a.remarks,a.bundle_num_prefix_no, b.id as bundle_dtls_id, b.mst_id, b.item_rcv_id, b.pcs_per_bundle, b.bundle_qty, b.barcode_no, b.barcode_id , b.bundle_no, c.job_dtls_id, c.job_break_id, c.buyer_po_id, d.embl_job_no,d.party_id,d.location_id,c.id as buyer_po_id,d.id as job_id,d.subcon_date
            	...
            	<table cellpadding="0" cellspacing="2"  width="1670" id="table_body" class="rpt_table" rules="all" align="left">
		         	...
		         	<?
			            ...
			            foreach($result as $row)
			            {
							...
							$order_id=$job_arr[$row[csf('job_dtls_id')]][$row[csf('job_break_id')]]['order_id'];
			                ...
			                <tr>
			                    ...
			                    <td width="60">
			                    	<input name="txtInterRef[]" id="txtInterRef_<? echo $i; ?>" type="text" class="text_boxes" value="<? echo $row[csf("grouping")]; ?>" disabled style="width:65px" />
			                    </td>
			                    <td width="60">
			                    ...
			                    <td width="80" align="right">
			                    	<? echo $row[csf("production_qnty")]; ?>
			                    	<input type="hidden" name="txtBundleQty[]" id="txtBundleQty_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo $row[csf("production_qnty")]; ?>"  readonly />
...
if ($action=="issue_popup_in_dtls")
{
	...
	<script>
		...
		{   //alert(company+'_'+party_name+'_'+within_group);	
			load_drop_down( 'embellishment_metarial_receive_bundle_controller', company+'_'+within_group+'_'+party_name, 'load_drop_down_buyer_pop', 'buyer_td' );
		}
                ...
                <table width="870" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
							...
							<? 
								echo create_drop_down( "cbo_company_name", 140, "SELECT id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $company, "load_drop_down( 'embellishment_metarial_receive_bundle_controller', this.value+'_'+".$within_group.", 'load_drop_down_buyer_pop', 'buyer_td' );"); ?>
                            </td>
                            <td>
							<?
								echo create_drop_down( "cbo_within_group", 50, $yes_no,"", 0, "--  --",$within_group, "load_drop_down( 'embellishment_metarial_receive_bundle_controller',document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_buyer_pop', 'buyer_td' );" ); ?>
							</td>
                            <td id="buyer_td">
								<? 
                ...
                <table width="400" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_challan_no').value, 'create_issue_search_list_view', 'popup_search_div', 'embellishment_metarial_receive_bundle_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:70px;" /></td>
                        </tr>
                        <tr>
...
if($action=="create_issue_search_list_view")
{
	...
	if($db_type==0)
	{ 
		if ($data[2]!="" &&  $data[3]!="") $recv_date = "and a.recv_date between '".change_date_format($data[2],'yyyy-mm-dd')."' and '".change_date_format($data[3],'yyyy-mm-dd')."'"; else $recv_date ="";
	}
	else
	{
		if ($data[2]!="" &&  $data[3]!="") $recv_date = "and a.recv_date between '".change_date_format($data[2], "", "",1)."' and '".change_date_format($data[3], "", "",1)."'"; else $recv_date ="";
	}
	if($search_str !='') $search_com_cond="and a.recv_number like '%$search_str%'";  
	...
	$sql= "SELECT a.id, a.company_id, a.recv_number, a.recv_date, a.floor_id, a.table_id,b.challan_no  from printing_bundle_receive_mst a, printing_bundle_receive_dtls b where a.id=b.mst_id and a.entry_form=614 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $search_com_cond $recv_date $company group by a.id, a.company_id, a.recv_number, a.recv_date, a.floor_id, a.table_id,b.challan_no order by a.id DESC ";
//echo $sql; 
	$result = sql_select($sql);
	?>
    <div>
	            ...
	            foreach( $result as $row )
	            {
					...
					?>
						<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value('<? echo $row[csf("id")];?>');" > 
	                        ...
	                        <td  align="center"><? echo $row[csf("recv_date")]; ?></td>
						</tr>
					<? 
					$i++;
	            }
...
{
	$nameArray=sql_select( "SELECT a.id, a.company_id, a.recv_number, a.recv_date, a.floor_id, a.table_id,machine_id from printing_bundle_receive_mst a where a.id='$data' and a.status_active =1 and a.is_deleted=0 and a.entry_form=614" ); 
	foreach ($nameArray as $row)
	{	
		echo "document.getElementById('txt_receive_no').value 		= '".$row[csf("recv_number")]."';\n";
		echo "document.getElementById('txt_receive_date').value 		= '".change_date_format($row[csf("recv_date")])."';\n";
		echo "load_drop_down( 'requires/embellishment_metarial_receive_bundle_controller', '" . $row[csf('floor_id')] . "', 'load_drop_down_table', 'table_td' );\n";
		echo "load_drop_down( 'requires/embellishment_metarial_receive_bundle_controller', '" . $row[csf('floor_id')] . "', 'load_drop_down_machine', 'td_machine' );\n";
                ...
                <table width="870" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td>
							<?
								echo create_drop_down( "cbo_within_group", 50, $yes_no,"", 0, "--  --",2, "load_drop_down( 'embellishment_metarial_receive_bundle_controller',document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_buyer_pop', 'buyer_td' );" ); ?>
							</td>
                            <td id="buyer_td">
								<? 
								echo create_drop_down( "cbo_party_name", 120, $blank_array,"", 1, "-- Select Party --", $selected, "" );?>
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_search_challan').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+document.getElementById('cbo_within_group').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value, 'create_receive_search_list_view_browse', 'search_div', 'embellishment_metarial_receive_bundle_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:70px;" /></td>
                        </tr>
                        <tr>
                            <td colspan="10" align="center" valign="middle">
								<? echo load_month_buttons(1);  ?>
...
if($action=="create_receive_search_list_view_browse")
{
	...
	if($search_type==1)
	{
		if($search_str!="")
		{
			...
			else if ($search_by==6) $inter_ref=" and e.grouping = '$search_str' ";
		}
		if ($data[4]!='') $rec_id_cond=" and a.prefix_no_num='$data[4]'"; else $rec_id_cond="";
	...
	else if($search_type==4 || $search_type==0)
	{
		if($search_str!="")
		{
			...
			else if ($search_by==6) $inter_ref=" and e.grouping  like '%$search_str%' ";    
		}
		if ($data[4]!='') $rec_id_cond=" and a.prefix_no_num like '%$data[4]%'"; else $rec_id_cond="";
	...
	if($within_group==1)
	{
		...
		// echo "select $id_cond as id from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst $job_cond $style_cond $po_cond $inter_ref";
		/*if(($job_cond!="" && $search_by==3) || ($po_cond!="" && $search_by==4) || ($style_cond!="" && $search_by==5))
		{
	...
	$con = connect();
	$sql_issue="SELECT id,mst_id,company_id,entry_form,order_id,job_dtls_id,color_size_id,job_id,buyer_po_id,bundle_mst_id,bundle_dtls_id,challan_no,quantity,issue_dtls_id
			from printing_bundle_receive_dtls where entry_form=614 and status_active=1 and is_deleted=0 order by id ASC";
	$issue_result =sql_select($sql_issue);
	$bundle_dtls_id=""; $order_id='';
	foreach($issue_result as $row)
	{
		...
		if($bundle_dtls_id!=0)
		{
			$r_id2=execute_query("insert into tmp_barcode_no (userid, barcode_no, entry_form) values ($user_id,$bundle_dtls_id,614)");
		}
	}
	//print_r($issue_item_arr);
	//$wo_ids=implode(",",array_unique(explode(",",(chop($order_id,',')))));
...
/*	$sql="SELECT c.id as bundl_mst_id, c.company_id, c.po_id, c.order_breakdown_id, c.item_rcv_dtls_id, c.pcs_per_bundle, c.no_of_bundle, c.bundle_qty, c.remarks,c.bundle_num_prefix_no,d.id as bundle_dtls_id, d.mst_id, d.item_rcv_id, d.pcs_per_bundle, d.bundle_qty, d.barcode_no, d.barcode_id , b.job_dtls_id, b.job_break_id, b.buyer_po_id, b.id as buyer_po_id, a.embl_job_no,a.party_id,a.location_id,a.id as job_id,a.subcon_date,a.sys_no, $insert_date_cond as year,e.order_no
	...
	group by c.id , c.company_id, c.po_id, c.order_breakdown_id, c.item_rcv_dtls_id, c.pcs_per_bundle, c.no_of_bundle, c.bundle_qty, c.remarks,c.bundle_num_prefix_no,d.id , d.mst_id, d.item_rcv_id, d.pcs_per_bundle, d.bundle_qty, d.barcode_no, d.barcode_id , b.job_dtls_id, b.job_break_id, b.buyer_po_id, b.id, a.embl_job_no,a.party_id,a.location_id,a.id ,a.subcon_date,a.sys_no, a.insert_date,e.order_no order by b.id";*/
	 ...
	 $sql="SELECT c.id as bundle_dtls_id,d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, $year_field as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.cut_no,c.bundle_no,c.barcode_no,c.production_qnty,c.is_rescan, e.po_number,f.company_name,b.location,b.production_source,b.serving_company,b.embel_name, b.embel_type,b.wo_order_no,b.challan_no,b.wo_order_id,g.id as job_dtls_id,b.id as bundl_mst_id,e.job_id,g.buyer_po_id,e.grouping,b.production_date  from pro_garments_production_mst b,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f,subcon_ord_dtls g  where  b.id=c.mst_id and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_id=f.id and d.job_id=f.id  and c.production_type=2 and  g.buyer_po_id=e.id  and c.status_active=1 and c.is_deleted=0 $company $challan_no_cond $recieve_date $inter_ref order by  c.cut_no,length(c.bundle_no) asc, c.bundle_no asc ";  
     ...
     <div style="width:860px; max-height:270px;overflow-y:scroll;" >	 
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="840" class="rpt_table" id="tbl_po_list">
            ...
            foreach($result as $row)
            {
                if ($i%2==0)  $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
				$party_name="";
				if($row[csf("within_group")]==1) $party_name=$comp[$row[csf("serving_company")]]; else $party_name=$party_arr[$row[csf("serving_company")]];
				?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value('<? echo $row[csf("job_id")]."_".$row[csf("challan_no")];?>');" > 
						<td width="40" align="center"><? echo $i; ?></td>
						<td width="100" align="center"><? echo $row[csf("job_no_prefix_num")]; ?></td>
						<td width="100" align="center"><? echo $row[csf("wo_order_no")]; ?></td>
						<td width="100" align="center"><? echo $row[csf("bundle_no")]; ?></td>
                        <td width="50" align="center"><? echo $row[csf("year")]; ?></td>
                        <td width="100"><? echo $party_name; ?></td>		
						<td width="100"><? echo $row[csf("challan_no")]; ?></td>
						<td width="60"><? echo change_date_format($row[csf("production_date")]);  ?></td>
        ...
        <table cellpadding="0" cellspacing="2" border="1" width="1650" id="tbl_footer" rules="all" align="left">
	        <tfoot>
	        	<tr class="tbl_bottom" name="tr_btm" id="tr_btm">
					...
					<td width="80">
						<input name="txtTotBundleqty" id="txtTotBundleqty" class="text_boxes_numeric" type="text" value="<? echo $totalBndlQty; ?>" style="width:70px" readonly />
						<input type="hidden" name="txt_row_no" id="txt_row_no" class="text_boxes_numeric" value="" style="width:70px" readonly />
					</td>
...
if($action=="append_new_item")
{
	...
	if($type==2)
	{
		...
		if($db_type==2 && $bundle_count>400)
		{
			...
			foreach($bundleArr as $bundleNos)
			{
				...
				$cutbundle_nos_cond.=" b.challan_no in($bundleNos) or ";
			}
		...
		else
		{
			...
			$cutbundle_nos_cond=" and b.challan_no in ($bundle_nos)";
		}
	}
	else
	{
		...
		$total_row =$exdata[3];
		$cutbundle_nos_cond=" and b.challan_no='$bundle_dtls_id' ";
	}
	...
	else if($db_type==2)
	{
	 ...
	 // $sql="SELECT c.id as bundle_dtls_id,d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, $year_field as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.cut_no,c.bundle_no,c.barcode_no,c.production_qnty,c.is_rescan, e.po_number,b.company_id,b.location,b.production_source,b.serving_company,b.embel_name, b.embel_type,b.wo_order_no,b.challan_no,b.wo_order_id,g.id as job_dtls_id,b.id as bundl_mst_id,e.job_id,g.buyer_po_id,e.grouping  from pro_garments_production_mst b,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f,subcon_ord_dtls g  where  b.id=c.mst_id $cutbundle_nos_cond and  c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_id=f.id and d.job_id=f.id  and c.production_type=2 and  g.buyer_po_id=e.id  and c.status_active=1 and c.is_deleted=0 and c.print_receive_status=0 $company $challan_no_cond $recieve_date order by  c.cut_no,length(c.bundle_no) asc, c.bundle_no asc ";   
	  
	  
	   $sql="SELECT c.id as bundle_dtls_id,d.id as colorsizeid, e.id as po_id, f.job_no_prefix_num, $year_field as year, f.buyer_name, d.item_number_id, d.country_id, d.size_number_id, d.color_number_id, c.cut_no,c.bundle_no,c.barcode_no,c.production_qnty,c.is_rescan, e.po_number,b.company_id,b.location,b.production_source,b.serving_company,b.embel_name, b.embel_type,b.wo_order_no,b.challan_no,b.wo_order_id,g.id as job_dtls_id,b.id as bundl_mst_id,e.job_id,g.buyer_po_id,e.grouping  from pro_garments_production_mst b,pro_garments_production_dtls c, wo_po_color_size_breakdown d, wo_po_break_down e, wo_po_details_master f,subcon_ord_dtls g,subcon_ord_breakdown h  where  b.id=c.mst_id $cutbundle_nos_cond and c.color_size_break_down_id=d.id and d.po_break_down_id=e.id and e.job_id=f.id and d.job_id=f.id and b.wo_order_no=g.order_no  and c.production_type=2 and  g.buyer_po_id=e.id and h.mst_id=g.id  and  h.order_id=b.wo_order_id and  g.gmts_item_id=b.item_number_id and  g.embl_type=b.embel_type  and  h.color_id=d.color_number_id   and  h.size_id=d.size_number_id   and h.status_active=1 and h.is_deleted=0   and c.status_active=1 and c.is_deleted=0 and c.print_receive_status=0 $job_cond $buyer_po_cond $buyer_style_cond  $company $challan_no_cond $recieve_date $search_com_cond $withinGroup  order by  c.cut_no,length(c.bundle_no) asc, c.bundle_no asc ";  
    ...
    foreach($result as $row)
    {
        ...
        <tr>
            ...
            <td width="80" align="right">
            	<? echo $row[csf("production_qnty")]; ?>
            	<input name="txtBundleQty[]" id="txtBundleQty_<? echo $i; ?>" type="hidden" class="text_boxes_numeric" value="<? echo $row[csf("production_qnty")]; ?>" readonly />
            	<input name="txtProdQty[]" id="txtProdQty_<? echo $i; ?>" type="hidden" class="text_boxes_numeric" value="" onKeyUp="fnc_total_calculate ();" />
            </td>
        </tr>
    <?
