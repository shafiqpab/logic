...
<script>
	...
	{ 
		...
		var page_link='requires/embellishment_material_issue_bundle_controller.php?action=issue_popup&data='+data;
		...
		emailwindow.onclose=function()
		{
			...
			if (splt_val[0]!="")
			{
				...
				get_php_form_data( splt_val[0], "load_php_data_to_form", "requires/embellishment_material_issue_bundle_controller" );
				
				var list_view_orders = return_global_ajax_value( splt_val[0]+'**'+splt_val[1]+'**'+1, 'load_php_dtls_form', '', 'requires/embellishment_material_issue_bundle_controller');
				if(list_view_orders!='')
				{
	...
	function job_search_popup()
	{
		...
		{
			var data=document.getElementById('cbo_company_name').value+"_"+document.getElementById('cbo_location_name').value+"_"+document.getElementById('cbo_party_name').value+"_"+document.getElementById('cbo_within_group').value;
			var page_link='requires/embellishment_material_issue_bundle_controller.php?action=job_popup&data='+data;
			...
			emailwindow.onclose=function()
			{
				...
				var list_view_orders = return_global_ajax_value( 0+'**'+theemail+'**'+1, 'load_php_dtls_form', '', 'requires/embellishment_material_issue_bundle_controller');
				if(list_view_orders!='')
				{
					$("#rec_issue_table tr").remove();
					$("#rec_issue_table").append(list_view_orders);
	...
	{
		if ( form_validation('txt_issue_date*cbo_floor', 'Issue Date*floor')==false )
		{
			return;
		}
		else
		{
			...
			var data_str=get_submitted_data_string('txt_issue_no*cbo_company_name*txt_issue_date*cbo_floor*cbo_table*cbo_machine_name*cbo_within_group*update_id',"../../");
			...
			http.open("POST","requires/embellishment_material_issue_bundle_controller.php",true);
		...
		if(http.readyState == 4) 
		{
			...
			if(response[0]==0 || response[0]==1 || response[0]==2)
			{
				...
				if(within_group==1)
				{
						...
						show_list_view(response[2],'issue_item_details_update_bundle','search_div','requires/embellishment_material_issue_bundle_controller','setFilterGrid("rec_issue_table",-1)');
				}
				else
				{
 					show_list_view(response[2],'issue_item_details_update','search_div','requires/embellishment_material_issue_bundle_controller','setFilterGrid("rec_issue_table",-1)');	
 				}
				...
				var list_view_orders = return_global_ajax_value( response[2]+'**'+response[3]+'**'+2, 'load_php_dtls_form', '', 'requires/embellishment_material_issue_bundle_controller');
				if(list_view_orders!='')
				{
					$("#rec_issue_table tr").remove();
					$("#rec_issue_table").append(list_view_orders);
	...
	{ 
		...
		emailwindow.onclose=function()
		{
			...
			{
				freeze_window(5);
				get_php_form_data( theemail, "load_php_data_to_form", "requires/embellishment_material_issue_bundle_controller" );
				
				if(within_group==1)
				{
					
						show_list_view(theemail,'issue_item_details_update_bundle','search_div','requires/embellishment_material_issue_bundle_controller','setFilterGrid("rec_issue_table",-1)');
				}
				else
				{
 					show_list_view(theemail,'issue_item_details_update','search_div','requires/embellishment_material_issue_bundle_controller','setFilterGrid("rec_issue_table",-1)');
 				}
	...
	{ 
		...
		var data=document.getElementById('cbo_company_name').value;
		var page_link='requires/embellishment_material_issue_bundle_controller.php?action=receive_popup&data='+data;
		...
		emailwindow.onclose=function()
		{
			...
			if (splt_val[0]!="")
			{
				...
				//alert(tot_row);
				/*if(tot_row<1){
					var new_table = return_global_ajax_value( 2+'**'+tot_row, 'create_new_table', '', 'requires/embellishment_material_issue_bundle_controller');
					if (trim(new_table) != '')
			        {
			            $("#search_div").append(new_table);
			        }
				}*/
				var list_view_orders = return_global_ajax_value( splt_val[0]+'**'+splt_val[1]+'**'+1+'**'+tot_row, 'append_new_item', '', 'requires/embellishment_material_issue_bundle_controller');
				if(list_view_orders!='')
				{
    ...
    {
        var challan_duplicate = return_ajax_request_value(bundle_no + "__" + $('#cbo_company_name').val(), "challan_duplicate_check", "requires/embellishment_material_issue_bundle_controller");
    ...
    function create_row(bundle_nos, vscan, hidden_source_cond)
    {
		...
		if(row_num<1){
			/*var new_table = return_global_ajax_value( 2+'**'+row_num, 'create_new_table', '', 'requires/embellishment_material_issue_bundle_controller');
			if (trim(new_table) != '')
	        {
	            $("#search_div").append(new_table);
		...
		if(within_group==1)
		{
			...
			var list_view_orders = return_global_ajax_value( bundle_nos+'**'+''+'**'+2+'**'+row_num, 'append_new_item_bundle', '', 'requires/embellishment_material_issue_bundle_controller');
			
			
		}
		else
		{
 		   var list_view_orders = return_global_ajax_value( bundle_nos+'**'+''+'**'+2+'**'+row_num, 'append_new_item', '', 'requires/embellishment_material_issue_bundle_controller');
        ...
        <div align="center" style="width:100%;" >
            <form name="searchreceivefrm_1"  id="searchreceivefrm_1" autocomplete="off">
                <table width="870" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td>
							<?
								echo create_drop_down( "cbo_within_group", 50, $yes_no,"", 0, "--  --",'2', "load_drop_down( 'requires/embellishment_material_issue_bundle_controller',document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_buyer_pop', 'buyer_td' );" ); ?>
							</td>
                            <td id="buyer_td">
								<? 
                ...
                <table  width="1050" cellspacing="2" cellpadding="0" border="0">
                	<tr>
                    	<td width="100" align="right"><strong>Sys. Issue No</strong></td>
                    	<td width="120" align="left">
	                    	<input class="text_boxes"  type="text" name="txt_issue_no" id="txt_issue_no" onDblClick="openmypage_issue_id();"  placeholder="Double Click" style="width:107px;" readonly/>
	                    	<input type="hidden" name="update_id" id="update_id">
	                    	<input type="hidden" name="txt_tot_row" id="txt_tot_row" value="0" readonly >
	                    </td>
	                    <td width="80" align="right" class="must_entry_caption"><strong>Issue Date</strong></td>
                    	<td width="80" align="left">
                    		<input type="text" class="datepicker" autocomplete="off"  name="txt_issue_date" id="txt_issue_date"  value="<? echo date("d-m-Y");?>"  style="width:67px;"  />
	                    </td>
                    	...
                    	<td width="120" align="left">
	                    	<?
                              	echo create_drop_down( "cbo_floor",120, "select id,floor_name from lib_prod_floor where production_process=8 and status_active =1 and is_deleted=0 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/embellishment_material_issue_bundle_controller',this.value, 'load_drop_down_table', 'table_td' );load_drop_down( 'requires/embellishment_material_issue_bundle_controller',this.value, 'load_drop_down_machine', 'td_machine' );",0,"","","","",4 );
                         	?> 
	                    </td>
						...
						<table cellpadding="0" cellspacing="2"  width="1770" id="table_body" class="rpt_table" rules="all" align="left">
							<tbody id="rec_issue_table">
								 <tr bgcolor="<? echo $bgcolor; ?>" id="dammy_row">
									...
									<td width="80">
										<input name="txtRecDate[]" id="txtRecDate_1" type="text" class="datepicker" value=""  style="width:55px" disabled />
									</td>
									<td width="80">
					...
					<table cellpadding="0" cellspacing="2" border="1" width="1770" id="tbl_footer" rules="all" align="left">
						<tfoot>
							<tr class="tbl_bottom" name="tr_btm" id="tr_btm">
								...
								<td colspan="17" width="80">Total:</td> 
								<td width="80">
