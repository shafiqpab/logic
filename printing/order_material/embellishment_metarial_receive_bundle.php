...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Embellishment Material Issue [Bundle] Info", "../../", 1,1, $unicode,1,'');
...
<script>
	...
	function job_search_popup()
	{
		...
		else
		{
			...
			var page_link='requires/embellishment_metarial_receive_bundle_controller.php?action=job_popup&data='+data;
			...
			emailwindow.onclose=function()
			{
				...
				var list_view_orders = return_global_ajax_value( 0+'**'+theemail+'**'+1, 'load_php_dtls_form', '', 'requires/embellishment_metarial_receive_bundle_controller');
				if(list_view_orders!='')
				{
	...
	{
		if ( form_validation('txt_receive_date', 'Receive Date')==false )
		{
			return;
		}
		else
		{
			...
			var data_str=get_submitted_data_string('txt_receive_no*cbo_company_name*txt_receive_date*cbo_floor*cbo_table*cbo_machine_name*update_id*cbo_within_group',"../../");
			...
			for (var j=1; j<=tot_row; j++)
			{
				...
				if($('#barcodeChkbox_'+j).is(':checked'))
				{
					...
					if($('#txtBundleQty_'+j).val()=='' || $('#txtBundleQty_'+j).val()==0){  
						alert('Please Fill up Prod. Qty.'); return;
					}else{
						i++;
						data_all+="&woID_" + i + "='" + $('#woID_'+j).val()+"'"+"&woDtlsID_" + i + "='" + $('#woDtlsID_'+j).val()+"'"+"&woBreakID_" + i + "='" + $('#woBreakID_'+j).val()+"'"+"&rcvID_" + i + "='" + $('#rcvID_'+j).val()+"'"+"&rcvDtlsID_" + i + "='" + $('#rcvDtlsID_'+j).val()+"'"+"&bundleMstID_" + i + "='" + $('#bundleMstID_'+j).val()+"'"+"&bundleDtlsID_" + i + "='" + $('#bundleDtlsID_'+j).val()+"'"+"&cboCompanyId_" + i + "='" + $('#cboCompanyId_'+j).val()+"'"+"&txtRcvDate_" + i + "='" + $('#txtRcvDate_'+j).val()+"'"+"&txtIssueCh_" + i + "='" + $('#txtIssueCh_'+j).val()+"'"+"&txtBundleQty_" + i + "='" + $('#txtBundleQty_'+j).val()+"'"+"&updatedtlsid_" + i + "='" + $('#updatedtlsid_'+j).val()+"'"+"&txtBarcodeNo_" + i + "='" + $('#txtBarcodeNo_'+j).val()+"'";						
					}
				}
				
				if(updateIdDtls!="" && $('#barcodeChkbox_'+j).is(':not(:checked)'))
				{
					data_delete+=$('#bundleDtlsID_'+j).val()+",";
			...
			http.open("POST","requires/embellishment_metarial_receive_bundle_controller.php",true);
	...
	function fnc_material_receive_response()
	{
		if(http.readyState == 4) 
		{
			...
			if(response[0]==0 || response[0]==1 || response[0]==2)
			{
				...
				show_list_view(response[2],'issue_item_details_update','search_div','requires/embellishment_metarial_receive_bundle_controller','setFilterGrid("rec_issue_table",-1)');
				//fnc_total_calculate();
				...
				var list_view_orders = return_global_ajax_value( response[2]+'**'+response[3]+'**'+2, 'load_php_dtls_form', '', 'requires/embellishment_metarial_receive_bundle_controller');
				if(list_view_orders!='')
				{
					$("#rec_issue_table tr").remove();
					$("#rec_issue_table").append(list_view_orders);
	...
	function openmypage_receive_id4545()
	{ 
	
	alert(1);
		var data=document.getElementById('cbo_company_name').value+"_"+document.getElementById('cbo_location_name').value+"_"+document.getElementById('cbo_party_name').value+"_"+document.getElementById('cbo_within_group').value;
		var page_link='requires/embellishment_metarial_receive_bundle_controller.php?action=issue_popup&data='+data;
		...
		emailwindow.onclose=function()
		{
			...
			{
				freeze_window(5);
				reset_form('','','txt_receive_no*cbo_company_name*cbo_location_name*cbo_party_name*txt_issue_challan*txt_receive_date*cbo_within_group*txt_job_no*update_id','','');
				get_php_form_data( splt_val[0], "load_php_data_to_form", "requires/embellishment_metarial_receive_bundle_controller" );
				
				var list_view_orders = return_global_ajax_value( splt_val[0]+'**'+splt_val[1]+'**'+1, 'load_php_dtls_form', '', 'requires/embellishment_metarial_receive_bundle_controller');
				if(list_view_orders!='')
				{
					$("#rec_issue_table tr").remove();
					$("#rec_issue_table").append(list_view_orders);
				}
				show_list_view(theemail,'issue_item_details_update','search_div','requires/embellishment_metarial_receive_bundle_controller','setFilterGrid("rec_issue_table",-1)');
				fnc_total_calculate();
				set_button_status(1, permission, 'fnc_material_receive',1);
				release_freezing();
			}
	...
	function openmypage_receive_id()
	{ 
	...
	//alert(2);
		...
		emailwindow.onclose=function()
		{
			...
			{
				freeze_window(5);
				get_php_form_data( theemail, "load_php_data_to_form", "requires/embellishment_metarial_receive_bundle_controller" );
				show_list_view(theemail,'issue_item_details_update','search_div','requires/embellishment_metarial_receive_bundle_controller','setFilterGrid("rec_issue_table",-1)');
				//fnc_total_calculate();
				set_button_status(1, permission, 'fnc_material_receive',1);
				release_freezing();
	...
	{ 
		...
		var data=document.getElementById('cbo_company_name').value;
		var page_link='requires/embellishment_metarial_receive_bundle_controller.php?action=receive_popup&data='+data;
		var title="Receive Id Popup Info";	
		...
		emailwindow.onclose=function()
		{
			...
			if (splt_val[0]!="")
			{
				...
				//alert(tot_row);
				/*if(tot_row<1){
					var new_table = return_global_ajax_value( 2+'**'+tot_row, 'create_new_table', '', 'requires/embellishment_metarial_receive_bundle_controller');
					if (trim(new_table) != '')
			        {
				...
				var list_view_orders = return_global_ajax_value( splt_val[0]+'**'+splt_val[1]+'**'+1+'**'+tot_row, 'append_new_item', '', 'requires/embellishment_metarial_receive_bundle_controller');
	...
	/*$('#txt_bundle_no').live('keydown', function(e) {
        if (e.keyCode === 13)
        {
            ...
            {
                var bundleNo = $(this).find("td:eq(18)").text();
				var barcodeNo=$(this).find("td:eq(18)").attr('title');
    ...
    $('#txt_bundle_no').live('keyup', function(e) {
		...
		var full_challan = trim($('#txt_bundle_no').val().toUpperCase());
		var challan_slit = full_challan.split("-");
		var txt_bundle_no = challan_slit[3] * 1;
        if (e.keyCode === 13 ) //|| (barcode.length==14)
        {
            ...
            $("#table_body").find('tbody tr').not( ".fltrow" ).each(function()
            {
            	...
            	var barcodeNo = trim($(this).find('input[name="txtIssueCh[]"]').val().toUpperCase());
            	if (txt_bundle_no == barcodeNo) {
    ...
    {
        var challan_duplicate = return_ajax_request_value(bundle_no + "__" + $('#cbo_company_name').val(), "challan_duplicate_check", "requires/embellishment_metarial_receive_bundle_controller");
    ...
    function create_row(bundle_nos, vscan, hidden_source_cond)
    {
		...
		if(row_num<1){
			/*var new_table = return_global_ajax_value( 2+'**'+row_num, 'create_new_table', '', 'requires/embellishment_metarial_receive_bundle_controller');
			if (trim(new_table) != '')
	        {
	            $("#search_div").append(new_table);
		...
		var list_view_orders = return_global_ajax_value( bundle_nos+'**'+''+'**'+2+'**'+row_num, 'append_new_item', '', 'requires/embellishment_metarial_receive_bundle_controller');
...
<div style="width:100%;" align="center">
	<? echo load_freeze_divs ("../../",$permission);  ?>
        ...
        <div align="center" style="width:100%;" >
            <form name="searchreceivefrm_1"  id="searchreceivefrm_1" autocomplete="off">
                <table width="870" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    <thead>
                    	...
                    	<tr>                	 
                            ...
                            <th width="70" style="display:none">Receive ID</th>
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td>
							<?
								echo create_drop_down( "cbo_within_group", 50, $yes_no,"", 0, "--  --",'2', "load_drop_down( 'requires/embellishment_metarial_receive_bundle_controller',document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_buyer_pop', 'buyer_td' );" ); ?>
							</td>
                            <td id="buyer_td">
								<? 
                ...
                <table  width="1050" cellspacing="2" cellpadding="0" border="0">
                	...
                	<tr>
                    	<td width="100" align="right"><strong>System Recv. No</strong></td>
                    	<td width="120" align="left">
	                    	<input class="text_boxes"  type="text" name="txt_receive_no" id="txt_receive_no" onDblClick="openmypage_receive_id();"  placeholder="Double Click" style="width:107px;" readonly/>
	                    	<input type="hidden" name="update_id" id="update_id">
	                    	<input type="hidden" name="txt_tot_row" id="txt_tot_row" value="0" readonly >
	                    </td>
	                    <td width="80" align="right" class="must_entry_caption"><strong>Recv Date</strong></td>
                    	<td width="80" align="left">
                    		<input type="text" class="datepicker" autocomplete="off"  name="txt_receive_date" id="txt_receive_date"  value="<? echo date("d-m-Y");?>"  style="width:67px;"  />
	                    </td>
	                    ...
	                    <td width="60" align="right" style="display:none"><strong>Floor</strong></td>
                    	<td width="120" align="left" style="display:none">
	                    	<?
                              	echo create_drop_down( "cbo_floor",120, "select id,floor_name from lib_prod_floor where production_process=8 and status_active =1 and is_deleted=0 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/embellishment_metarial_receive_bundle_controller',this.value, 'load_drop_down_table', 'table_td' );load_drop_down( 'requires/embellishment_metarial_receive_bundle_controller',this.value, 'load_drop_down_machine', 'td_machine' );",0,"","","","",4 );
                         	?> 
	                    </td>
	                    <td width="60" align="right" style="display:none"><strong>Table</strong></td>
                    	<td id="table_td" width="100" style="display:none">
                            <? 
                                echo create_drop_down( "cbo_table", 100, $blank_array,"", 1, "-- Select Table --", "", "" );
                            ?>                            
                        </td>
                        <td width="60" align="right" style="display:none"><strong>Machine</strong></td>
						<td id="td_machine" width="100"  align="left" style="display:none"><?
        ...
        <form name="pimasterform_2" id="pimasterform_2" autocomplete="off">
            <fieldset style="width:1690px; margin-top:10px;">
							...
							<tbody id="rec_issue_table">
								 <tr bgcolor="<? echo $bgcolor; ?>" id="dammy_row">
									<td width="50" align="center">
										1
										<input type="checkbox" name="barcodeChkbox[]" id="barcodeChkbox_1" value="" />
										...
										<input type="hidden" name="updateIdDtls[]" id="updatedtlsid_1" value=""  >
									</td>
										
									<td width="90">
										<input type="hidden" name="txtBarcodeNo[]" id="txtBarcodeNo_1" class="text_boxes" value="" style="width:90px" readonly />
									</td>
									<td width="100">
										<? 
									...
									<td width="100">
										<input type="hidden" name="txtCustBuyer[]" id="txtCustBuyer_1" class="text_boxes" value=""  style="width:87px" readonly />
									</td>
									<td width="80">
										<input name="txtInterRef[]" id="txtInterRef_1" type="text" class="text_boxes" value=""  style="width:65px" disabled />
									</td>
									<td width="60">
										<input name="txtIssueCh[]" id="txtIssueCh_1" type="text" class="text_boxes" value=""  style="width:47px"  />
									</td>
									<td width="100">
										<input name="txtEmbJob[]" id="txtEmbJob_1" type="hidden" class="text_boxes" value=""  style="width:107px" readonly />
									</td>
									<td width="140" class="word_wrap_break">
										<input type="hidden" name="txtOrder_1" id="txtOrder_1" class="text_boxes" value=""  style="width:87px" readonly />
									</td>
									<td width="140" class="word_wrap_break">
										<input name="txtstyleRef[]" id="txtstyleRef_1" type="hidden" class="text_boxes" value=""  style="width:87px" readonly />
									</td>
									<td width="60">
										<? echo create_drop_down( "cboProcessName_1", 50, $emblishment_name_array,"", 1, "--Select--",$main_process_id, "",1,'','','','','','',"cboEmbType[]"); ?>
									</td>
									<td width="60">
										<? 
										echo create_drop_down( "cboEmbType_1", 60, $blank_array,"", 1, "-- Select --", $selected, "","1","","","","","","","cboEmbType[]" );
										?>
									...
									<td width="80">
										<input name="txtBundleNo[]" id="txtBundleNo_1" type="hidden" class="text_boxes" value=""  style="width:67px" readonly  />
									</td>
									<td width="80" align="right">
										<input type="hidden" name="txtBundleQty[]" id="txtBundleQty_1" class="text_boxes_numeric" value="" style="width:70px" readonly />
										<input name="txtProdQty[]" id="txtProdQty_1" type="hidden" class="text_boxes_numeric" value="" onKeyUp="fnc_total_calculate ();" />
									</td>
                ...
                <table width="1670" cellspacing="2" cellpadding="0" border="0">
                <tr>
                    <td align="center" colspan="20" valign="middle" class="button_container">
                        <? echo load_submit_buttons($permission, "fnc_material_receive", 0,0,"reset_form('materialissue_1','issue_list_view','','cbouom_1,1', '$(\'#details_tbl tbody tr:not(:first)\').remove(); disable_enable_fields(\'cbo_company_name*cbo_within_group*cbo_party_name*cboProcessName_1*cboReType_1*cboGmtsItem_1*cboBodyPart_1*txtmaterialdescription_1*txtcolor_1*txtsize_1\',0)')",1); ?>
                    </td>
                </tr>  
          	</table> 
            </fieldset>
