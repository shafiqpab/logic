...
<script>
	...
	function show_details()
	{
		...
		var list_view_orders = return_global_ajax_value( cbo_company_id+"******"+cbo_store_id, 'item_details', '', 'requires/recipe_entry_controller');
		if(list_view_orders!='')
		{
	...
	function fnc_recipe_entry(operation)
	{
		...
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			if(recipe_for!=3)
			{
				if($('#copy_id').val()==2)
				{
					if( form_validation('cbo_recipe_for*cbo_company_id*txt_recipe_date*cbo_within_group*txt_po_color*txt_multi_color*txt_total_ratio*cbo_store_id','Recipe For*Company*Recipe Date*Within Group*Color*Color*Total Ratio*Store')==false )
					{
						return;
					}
				}
			...
			else
			{
				if($('#copy_id').val()==2)
				{
					if( form_validation('cbo_recipe_for*cbo_company_id*txt_recipe_date*txt_multi_color*cbo_store_id*cbo_item_category*txt_item_group*txt_total_ratio*cbo_store_id','Recipe For*Company*Recipe Date*Color*Store*Item Category*Item Group*Total Ratio*Store')==false )
					{
						return;
					}
				}
			...
			for(var j=1; j<=row_num; j++)
			{
				...
				{
					i++;
					data_all+="&txt_seqno_" + i + "='" + $('#txt_seqno_'+j).val()+"'"+"&product_id_" + i + "='" + $('#product_id_'+j).text()+"'"+"&product_lot_" + i + "='" + $('#td_lot_'+j).text()+"'"+"&txt_comments_" + i + "='" + $('#txt_comments_'+j).val()+"'"+"&txt_ratio_" + i + "='" + $('#txt_ratio_'+j).val()+"'"+"&updateIdDtls_" + i + "='" + $('#updateIdDtls_'+j).val()+"'";
