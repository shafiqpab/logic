...
<script>
		...
		{ 	
			...
			emailwindow.onclose=function()
			{
				...
				if(erecipedata[1]!="")
				{
					...
					$('#cbo_store_name').val(erecipedata[8]);
					$('#cbo_color_id').val(erecipedata[9]);
					load_drop_down('requires/chemical_dyes_issue_requisition_controller',erecipedata[9], 'load_drop_down_color', 'color_td');
					$('#cbo_recipe_for').val(erecipedata[10]);
					$('#txt_ord_qnty').val(erecipedata[11]);
					fnc_disable_prod(erecipedata[10]);
	...
	function fnc_chemical_dyes_issue_requisition(operation)
	{
		...
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			var mst_data=get_submitted_data_string('txt_req_no*update_id*cbo_company_name*cbo_location_name*txt_requisition_date*cbo_receive_basis*txt_recipe_id*cbo_store_name*txt_order_id*txt_job_no*is_apply_last_update*txtbuyerPoId*cbo_color_id*cbo_recipe_for*txt_added_weight',"../../");
	...
	function fnc_req_calculate(type,inc)
	{
		if(type==1)
		{
			...
			var reqsn_tot=0; var balance_qnty=0;
			for (var m=1; m<=colorrow; m++)
			{
				...
				if(m==colorrow && m!=1)
				{
					$('#txt_reqn_qnty_'+inc+'_'+m).val( balance_qnty );
					$('#txt_tot_qnty_'+inc+'_'+m).val( balance_qnty );
					reqsn_tot=(reqsn_tot*1)+(balance_qnty*1);
				}
				else
				{
					$('#txt_reqn_qnty_'+inc+'_'+m).val( req_qty );
					$('#txt_tot_qnty_'+inc+'_'+m).val( req_qty );
					if(m==1) balance_qnty=past_weight-number_format(req_qty,4,'.','' );
					else balance_qnty=balance_qnty-number_format(req_qty,4,'.','' );
	...
	function fnc_disable_prod(val)
	{
		if(val==3)
		{
			$("#txt_added_weight").attr("disabled",false);
		}
		else
		{
			$("#txt_added_weight").attr("disabled",true);
		}
	}
            ...
            <legend>Dyes And Chemical Issue Requisition</legend>
                <table width="810" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td>
                            <input class="text_boxes" type="text" name="txt_order_no" id="txt_order_no" placeholder="Display" style="width:140px;" /> 
                    ...
                    <tr>
                        ...
                        <td><? echo create_drop_down( "cbo_store_name", 150, "select lib_store_location.id,lib_store_location.store_name from lib_store_location,lib_store_location_category where lib_store_location.id=lib_store_location_category.store_location_id and lib_store_location.status_active=1 and lib_store_location.is_deleted=0  and lib_store_location_category.category_type in(5,6,7,23,22) group by lib_store_location.id,lib_store_location.store_name order by lib_store_location.store_name","id,store_name", 1, "-- Select Store --", $storeName, "",1 ); ?></td>
                   	...
                   	<tr>
                         ...
                         <td  id="color_td">
						 <?  
 						 echo create_drop_down( "cbo_color_id", 150, $blank_array,"", 1, "-- Select --",'', "",1 ); ?>
                         </td>
                    	<td align="right">Recipe For</td>
                        <td><? echo create_drop_down("cbo_recipe_for", 150, $recipe_for,"", 1,"-Select-", 0,"fnc_disable_prod(this.value);",1); ?></td>
                        <td align="right">Added Weight</td>
                        <td>
                            <input class="text_boxes" type="text" name="txt_added_weight" id="txt_added_weight" style="width:140px;" disabled="disabled"   /> 
                        </td>
                    </tr> 
                    <tr>
						<td colspan="2" align="right">
							...
							terms_condition(221,'txt_req_no','../../');
							?>
						</td>
                ...
                <table cellpadding="0" cellspacing="1" width="100%">
                    ...
                    <tr>
                        ...
                        <input type="hidden" name="update_dtls_id" id="update_dtls_id" readonly>
                            ...
                            <input type="button" name="last_update" class="formbuttonplasminus" value="Apply Last Update"  id="last_update" style="display:none"  onClick="apply_last_update();"/>
