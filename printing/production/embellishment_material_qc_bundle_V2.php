...
<script>
	...
	{ 
		...
		var page_link='requires/embellishment_material_qc_bundle_controller_V2.php?action=production_popup&data='+data;
		...
		emailwindow.onclose=function()
		{
			...
			{
				freeze_window(5);
				reset_form('','','txt_production_no*cbo_company_name*cbo_location_name*cbo_party_name*txt_issue_challan*txt_qc_date*cbo_within_group*txt_job_no*update_id*production_update_id','','');
				get_php_form_data( splt_val[0], "load_php_data_to_form", "requires/embellishment_material_qc_bundle_controller_V2" );
				
				var list_view_orders = return_global_ajax_value( splt_val[0]+'**'+splt_val[1]+'**'+1, 'load_php_dtls_form', '', 'requires/embellishment_material_qc_bundle_controller_V2');
				if(list_view_orders!='')
				{
	...
	function job_search_popup()
	{
		...
		{
			var data=document.getElementById('cbo_company_name').value+"_"+document.getElementById('cbo_location_name').value+"_"+document.getElementById('cbo_party_name').value+"_"+document.getElementById('cbo_within_group').value;
			var page_link='requires/embellishment_material_qc_bundle_controller_V2.php?action=job_popup&data='+data;
			...
			emailwindow.onclose=function()
			{
				...
				var list_view_orders = return_global_ajax_value( 0+'**'+theemail+'**'+1, 'load_php_dtls_form', '', 'requires/embellishment_material_qc_bundle_controller_V2');
				if(list_view_orders!='')
				{
					$("#rec_issue_table tr").remove();
					$("#rec_issue_table").append(list_view_orders);
	...
	function fnc_material_qc( operation )
	{
		...
		else
		{
			...
			var data_str=get_submitted_data_string('txt_production_no*cbo_company_name*txt_qc_date*cbo_floor*cbo_table*cbo_machine_name*txt_qc_name*cbo_shift_name*cbo_within_group*update_id*production_update_id',"../../");
			var tot_row=$('#table_body tbody tr:not(".fltrow")').length;
			var k=0;
			var data_all=""; var i=0; var selected_row=0;var data_delete=""; data_delete_issue ="";
			...
			for (var j=1; j<=tot_row; j++)
			{
				...
				if(updateIdDtls!="" && $('#barcodeChkbox_'+j).is(':not(:checked)'))
				{
					...
					data_delete_issue+=$('#issueDtlsID_'+j).val()+",";
			...
			var data="action=save_update_delete&operation="+operation+'&total_row='+i+'&data_delete='+data_delete+'&data_delete_issue='+data_delete_issue+data_all+data_str;//+'&zero_val='+zero_val
			//alert (data); return;
			freeze_window(operation);
			http.open("POST","requires/embellishment_material_qc_bundle_controller_V2.php",true);
	...
	function fnc_material_qc_response()
	{
		if(http.readyState == 4) 
		{
			...
			if(response[0]==0 || response[0]==1 || response[0]==2)
			{
				...
				document.getElementById('production_update_id').value = response[3];
				//alert();
				...
				if(within_group==1)
				{
						...
						show_list_view(response[2],'issue_item_details_update_bundle','search_div','requires/embellishment_material_qc_bundle_controller_V2','setFilterGrid(\'rec_issue_table\',-1)');
				
				}
				else
				{
 					show_list_view(response[2],'issue_item_details_update','search_div','requires/embellishment_material_qc_bundle_controller_V2','setFilterGrid(\'rec_issue_table\',-1)');
				 }
	...
	{ 
		...
		emailwindow.onclose=function()
		{
			...
			if (theemail!="")
			{
				...
				get_php_form_data( theemail, "load_php_data_to_form", "requires/embellishment_material_qc_bundle_controller_V2" );
				
				var within_group = document.getElementById('cbo_within_group').value;
				if(within_group==1)
				{
  					show_list_view(theemail,'issue_item_details_update_bundle','search_div','requires/embellishment_material_qc_bundle_controller_V2','setFilterGrid(\'rec_issue_table\',-1)');
					
				}
				else
				{
					show_list_view(theemail,'issue_item_details_update','search_div','requires/embellishment_material_qc_bundle_controller_V2','setFilterGrid(\'rec_issue_table\',-1)');
 				}
	...
	{ 
		...
		var data=document.getElementById('cbo_company_name').value;
		var page_link='requires/embellishment_material_qc_bundle_controller_V2.php?action=receive_popup&data='+data;
		...
		emailwindow.onclose=function()
		{
			...
			if (splt_val[0]!="")
			{
				...
				var list_view_orders = return_global_ajax_value( splt_val[0]+'**'+splt_val[1]+'**'+1+'**'+tot_row+'**'+splt_val[2], 'append_new_item', '', 'requires/embellishment_material_qc_bundle_controller_V2');
				if(list_view_orders =='')
				{
	...
	function openmypage_reject_qty(type,job_dtls_id,row)
	{
		...
		//var page_link = 'requires/embellishment_material_qc_bundle_controller_V2.php?job_dtls_id='+job_dtls_id+'&data_break='+data_break+'&hdnDtlsUpdateId='+hdnDtlsUpdateId+'&booking_po_id='+booking_po_id+'&RecipeNo='+RecipeNo+'&production_id='+production_id+'&action=reject_qty_popup';
		var page_link = 'requires/embellishment_material_qc_bundle_controller_V2.php?job_dtls_id='+job_dtls_id+'&data_break='+data_break+'&hdnDtlsUpdateId='+hdnDtlsUpdateId+'&production_id='+production_id+'&action=reject_qty_popup';
    ...
    {
        var challan_duplicate = return_ajax_request_value(bundle_no + "__" + $('#cbo_company_name').val(), "challan_duplicate_check", "requires/embellishment_material_qc_bundle_controller_V2");
    ...
    function create_row(bundle_nos, vscan, hidden_source_cond)
    {
        freeze_window(5);
           ...
           var row_num =  $('#table_body tbody tr:not(".fltrow, #dammy_row")').length;
			...
			if(within_group==1)
			{
 					var list_view_orders = return_global_ajax_value( bundle_nos+'**'+''+'**'+2+'**'+row_num, 'append_new_item_bundle', '', 'requires/embellishment_material_qc_bundle_controller_V2');
 			}
			else
			{
				var list_view_orders = return_global_ajax_value( bundle_nos+'**'+''+'**'+2+'**'+row_num, 'append_new_item', '', 'requires/embellishment_material_qc_bundle_controller_V2');
  			}
                ...
                <table width="870" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td>
							<?
								echo create_drop_down( "cbo_within_group", 50, $yes_no,"", 0, "--  --",'2', "load_drop_down( 'requires/embellishment_material_qc_bundle_controller_V2',document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_buyer_pop', 'buyer_td' );" ); ?>
							</td>
                            <td id="buyer_td">
								<? 
                ...
                <table  width="1300" cellspacing="2" cellpadding="0" border="0">
                	<tr>
                    	...
                    	<td width="130" align="left">
                            ...
                            <input type="hidden" name="production_update_id" id="production_update_id">
	                    </td>
                    	...
                    	<td width="120" align="left">
	                    	<?
                              	echo create_drop_down( "cbo_floor",120, "select id,floor_name from lib_prod_floor where  production_process=8 and status_active =1 and is_deleted=0 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/embellishment_material_qc_bundle_controller_V2',this.value, 'load_drop_down_table', 'table_td' );load_drop_down( 'requires/embellishment_material_qc_bundle_controller_V2',this.value, 'load_drop_down_machine', 'td_machine' );",0,"","","","",4 );
                         	?> 
	                    </td>
