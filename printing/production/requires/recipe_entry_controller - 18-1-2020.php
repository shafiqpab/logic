...
if ($action == 'populate_data_from_search_popup') 
{
	...
	$data_array = sql_select("SELECT id, recipe_no, company_id, location_id, recipe_description, recipe_for, recipe_date, within_group, po_id, job_no, buyer_po_id, buyer_id, gmts_item, body_part, embl_name, embl_type, color_id, remarks, store_id from pro_recipe_entry_mst where id='$data' and entry_form=220 and status_active =1 and is_deleted=0");
...
if($action=="item_details")
{
	...
	where a.id=b.prod_id and a.company_id='$company_id' and b.store_id=$store_id and a.item_category_id in(5,6,7,23) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
	union all
	...
	where a.id=b.prod_id and a.company_id='$company_id' and b.store_id=$store_id and a.item_category_id in(22) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
					...
					if(count($recipe_prod_id_arr)>0)
					{
						foreach($recipe_prod_id_arr as $prodId)
						{
                            ...
                            <tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>">
                                ...
                                <td width="70" align="center" id="td_lot_<? echo $i; ?>" title="<? echo $prodId; ?>"><? echo $product_lot; ?><input type="hidden" name="txt_lot[]" id="txt_lot_<? echo $i; ?>" value="<? echo $product_lot; ?>"></td>
	...
	if ($operation == 0)  // Insert Here
	{
		...
		if(str_replace("'", "", $copy_id) == 2)
		{
			if (str_replace("'", "", $update_id) == "") 
			{
				...
				$field_array = "id, entry_form, recipe_no_prefix, recipe_no_prefix_num, recipe_no, company_id, location_id, recipe_description, recipe_for, recipe_date, within_group, po_id, job_no, buyer_id, buyer_po_id, gmts_item, body_part, embl_name, embl_type, color_id, remarks, inserted_by, insert_date, status_active, is_deleted, store_id";
				//echo $txt_liquor;
			...
			{ 
				...
				{
					$color_id = return_id( str_replace("'","",$txt_multi_color), $color_arr, "lib_color", "id,color_name","220");  
			...
			$rece_sql="SELECT a.po_id,b.ratio,b.color_id  from pro_recipe_entry_mst a ,pro_recipe_entry_dtls b  where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and a.entry_form=220 and a.id =$update_id";
...
if ($action == "recipe_entry_print") 
{
	...
	$sql_mst = "SELECT id, recipe_no, company_id, location_id, recipe_description, recipe_for, recipe_date, within_group, po_id, job_no, buyer_id, buyer_po_id, gmts_item, body_part, embl_name, embl_type, color_id, remarks from pro_recipe_entry_mst where id='$mst_id' and status_active =1 and is_deleted=0";
	//echo $sql_mst;
