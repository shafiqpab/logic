...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   
	{
		...
		$field_array="id,company_id,entry_form , issue_num_prefix, issue_num_prefix_no, issue_number ,issue_date,floor_id,table_id,machine_id,qc_name,shift_id,within_group,inserted_by, insert_date, status_active, is_deleted";
		$data_array="(".$id.",".$cbo_company_name.",498,'".$new_iss_no[1]."','".$new_iss_no[2]."','".$new_iss_no[0]."',".$txt_qc_date.",".$cbo_floor.",".$cbo_table.",".$cbo_machine_name.",".$txt_qc_name.",".$cbo_shift_name.",".$cbo_within_group.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
	...
	else if ($operation==1)   // Update Here
	{
		...
		for($i=1; $i<=$total_row; $i++)
		{
			...
			{
				if ($add_commaa!=0) $data_array1 .=",";
				$data_array1.="(".$id1.",".$update_id.",".$$cboCompanyId.",498,".$$woID.",".$$woDtlsID.",".$$woBreakID.",".$$rcvID.",".$$rcvDtlsID.",".$$bundleMstID.",".$$bundleDtlsID.",".$$issueDtlsID.",".$$txtIssueCh.",".$$txtQcQty.",".$$productionDtlsIds.",".$$txtRejQty.",".$$hdnDtlsdata.",".$$txtRemark.",".$$txtBarcodeNo.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
				$id_arr_trans[]=str_replace("'",'',$$productionDtlsIds);
				$data_array_trans_update[str_replace("'",'',$$productionDtlsIds)] = explode("*",(1));
...
/*	$sql="SELECT c.id as bundl_mst_id, c.company_id, c.po_id, c.order_breakdown_id, c.item_rcv_dtls_id, c.pcs_per_bundle, c.no_of_bundle, c.bundle_qty, c.remarks,c.bundle_num_prefix_no,d.id as bundle_dtls_id, d.mst_id, d.item_rcv_id, d.pcs_per_bundle, d.bundle_qty, d.barcode_no, d.barcode_id, d.bundle_no , b.job_dtls_id, b.job_break_id, b.buyer_po_id, b.id as rcv_dtls_id, a.embl_job_no,a.party_id,a.location_id,a.id as rcv_id,a.subcon_date,a.sys_no, e.id as production_dtls_id , f.issue_date ,e.id as issue_dtls_id ,e.challan_no
	...
	?>
	    <div>
	        ...
	        <div style="width:1990px; max-height:270px;overflow-y:scroll;"  align="left">
                ...
                <table width="400" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_challan_no').value, 'create_issue_search_list_view', 'popup_search_div', 'embellishment_material_qc_bundle_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:70px;" /></td>
                        </tr>
                        <tr>
...
if($action=="create_issue_search_list_view")
{
	...
	$sql= "SELECT a.id, a.company_id, a.issue_number, a.issue_date, a.floor_id, a.table_id,b.challan_no  from printing_bundle_issue_mst a, printing_bundle_issue_dtls b where a.id=b.mst_id and a.entry_form=498 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $search_com_cond $issue_date $company group by a.id, a.company_id, a.issue_number, a.issue_date, a.floor_id, a.table_id,b.challan_no order by a.id DESC ";
	//echo $sql; 
	$result = sql_select($sql);
	?>
    <div>
...
{
	$nameArray=sql_select( "SELECT a.id, a.company_id, a.issue_number, a.issue_date, a.floor_id, a.table_id, a.qc_name,a.shift_id,a.machine_id from printing_bundle_issue_mst a where a.id='$data' and a.status_active =1 and a.is_deleted=0 and a.entry_form=498" ); 
	...
	{	
		...
		echo "load_drop_down( 'requires/embellishment_material_qc_bundle_controller', '" . $row[csf('floor_id')] . "', 'load_drop_down_table', 'table_td' );\n";
		echo "load_drop_down( 'requires/embellishment_material_qc_bundle_controller', '" . $row[csf('floor_id')] . "', 'load_drop_down_machine', 'td_machine' );\n";
...
if ($action=="receive_popup")
{
	...
	<script>
		...
		{   //alert(company+'_'+party_name+'_'+within_group);	
			load_drop_down( 'embellishment_material_qc_bundle_controller', company+'_'+within_group+'_'+party_name, 'load_drop_down_buyer_pop', 'buyer_td' );
		}
                ...
                <table width="870" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            <td> <input type="hidden" id="selected_job">  <!--  echo $data;-->
							<? 
								echo create_drop_down( "cbo_company_name", 140, "SELECT id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $company, "load_drop_down( 'embellishment_material_qc_bundle_controller', this.value+'_'+".$within_group.", 'load_drop_down_buyer_pop', 'buyer_td' );"); ?>
                            </td>
                            <td>
							<?
								echo create_drop_down( "cbo_within_group", 50, $yes_no,"", 0, "--  --",2, "load_drop_down( 'embellishment_material_qc_bundle_controller',document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_buyer_pop', 'buyer_td' );" ); ?>
							</td>
                            <td id="buyer_td">
								<? 
...
if($action=="create_receive_search_list_view_browse")
{
	...
	from sub_material_mst a , sub_material_dtls b ,prnting_bundle_mst c, prnting_bundle_dtls d , printing_bundle_issue_dtls e , printing_bundle_issue_mst f , subcon_ord_mst g where c.id = d.mst_id and c.item_rcv_dtls_id = b.id and d.item_rcv_id = b.mst_id and d.item_rcv_id = a.id and a.id = b.mst_id and a.id=e.rcv_id and b.id=rcv_dtls_id and c.id=bundle_mst_id and d.id=e.bundle_dtls_id and e.mst_id=f.id and  g.embellishment_job=a.embl_job_no and e.wo_id=g.id and g.entry_form=204 and f.entry_form=497 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0  $recieve_date $company $buyer_cond $withinGroup $rec_id_cond $challan_no_cond $spo_idsCond $po_idsCond $location_cond and d.id not in (select barcode_no from tmp_barcode_no where userid=$user_id and entry_form=498) 
