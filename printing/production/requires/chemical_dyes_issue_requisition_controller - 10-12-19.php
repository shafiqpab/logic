...
if ($action == "create_requisition_search_list_view") 
{
	...
	$sql = "SELECT id, requ_no, requ_prefix_num, $year_field, company_id, requisition_date, recipe_id, order_id, job_no, buyer_po_id from dyes_chem_issue_requ_mst where company_id=$company and entry_form=221 and status_active =1 and is_deleted=0 $date_cond $search_field_cond $po_idsCond $spo_idsCond order by id DESC ";//$search_field_cond
	//echo $sql;
...
if ($action == "populate_data_from_data") 
{
	...
	$sql = sql_select("SELECT id, requ_no, company_id, location_id, requisition_date, requisition_basis, recipe_id, order_id, job_no, buyer_po_id, is_apply_last_update, store_id from dyes_chem_issue_requ_mst where id=$data and status_active =1 and is_deleted=0");
	foreach ($sql as $row) 
	{
            ...
            <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
                <table cellpadding="0" cellspacing="0" width="750" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tr class="general">
                        ...
                        <td>
                            <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_search_recipe').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+'<? echo $recipe_id; ?>', 'create_recipe_search_list_view', 'search_div', 'chemical_dyes_issue_requisition_controller', 'setFilterGrid(\'tbl_list_search\',-1);')"  style="width:100px;"/>
                        </td>
                    </tr>
                    <tr class="general">
...
if ($action == "create_recipe_search_list_view") 
{
	...
	$sql = "SELECT a.id, a.recipe_no_prefix_num, a.recipe_no, a.recipe_date, a.color_id, a.store_id, a.job_no, b.job_no_prefix_num, c.id as order_id, c.order_no, c.buyer_po_id 
	from pro_recipe_entry_mst a, subcon_ord_mst b, subcon_ord_dtls c 
	where a.job_no=b.embellishment_job and a.po_id=c.id and b.embellishment_job=c.job_no_mst and b.entry_form=204 and a.company_id='$company_id' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $search_recipe_cond $search_com_cond $date_cond $po_idsCond order by a.id Desc";
		...
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="850" class="rpt_table" id="tbl_list_search">
            ...
            foreach ($nameArray as $selectResult) 
            {
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>" onClick="js_set_value('<? echo $selectResult[csf('id')].'___'.$selectResult[csf('recipe_no')].'___'.$selectResult[csf('job_no')].'___'.$selectResult[csf('order_id')].'___'.$selectResult[csf('order_no')].'___'.$selectResult[csf('buyer_po_id')].'___'.$buyer_po_arr[$selectResult[csf("buyer_po_id")]]['po'].'___'.$buyer_po_arr[$selectResult[csf("buyer_po_id")]]['style'].'___'.$selectResult[csf('store_id')]; ?>')">
                    ...
                    <td width="70" align="center"><? echo $selectResult[csf('recipe_no_prefix_num')]; ?></td>
                    <td width="80" align="center"><? echo change_date_format($selectResult[csf('recipe_date')]); ?>&nbsp;</td>
                    <td width="120"><? echo $selectResult[csf('job_no')]; ?>&nbsp;</td>
...
if ($action == "save_update_delete") 
{
	...
	if ($operation == 0)  // Insert Here
	{
		...
		if (str_replace("'", "", $update_id) == "") 
		{
			...
			$new_requ_no = explode("*", return_mrr_number(str_replace("'", "", $cbo_company_name), '', 'EDCR', date("Y", time()), 5, "select requ_no_prefix, requ_prefix_num from dyes_chem_issue_requ_mst where company_id=$cbo_company_name and entry_form=221 and $year_cond=" . date('Y', time()) . " order by id desc ", "requ_no_prefix", "requ_prefix_num"));
			...
			$field_array = "id, requ_no, requ_no_prefix, requ_prefix_num, company_id, location_id, requisition_date, requisition_basis, recipe_id, order_id, job_no, buyer_po_id, inserted_by, insert_date, entry_form, store_id";
			$data_array = "(" . $id . ",'" . $new_requ_no[0] . "','" . $new_requ_no[1] . "'," . $new_requ_no[2] . "," . $cbo_company_name . "," . $cbo_location_name . "," . $txt_requisition_date . "," . $cbo_receive_basis . "," . $txt_recipe_id . "," . $txt_order_id . "," . $txt_job_no . "," . $txtbuyerPoId . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "',221," . $cbo_store_name . ")";
		...
		{
			$field_array_up = "location_id*requisition_date*requisition_basis*recipe_id*order_id*job_no*buyer_po_id*updated_by*update_date";
			$data_array_up = "" . $cbo_location_name . "*" . $txt_requisition_date . "*" . $cbo_receive_basis . "*" . $txt_recipe_id . "*" . $txt_order_id . "*" . $txt_job_no . "*" . $txtbuyerPoId . "*" . $user_id . "*'" . $pc_date_time . "'";
	...
	else if ($operation == 1)  // Update Here
	{
		...
		{
			$field_array_up = "location_id*requisition_date*requisition_basis*recipe_id*is_apply_last_update*order_id*job_no*buyer_po_id*updated_by*update_date";
			$data_array_up = "" . $cbo_location_name . "*" . $txt_requisition_date . "*" . $cbo_receive_basis . "*" . $txt_recipe_id . "*0*" . $txt_order_id . "*" . $txt_job_no . "*" . $txtbuyerPoId . "*" . $user_id . "*'" . $pc_date_time . "'";
		} 
		else 
		{
			$field_array_up = "location_id*requisition_date*requisition_basis*recipe_id*order_id*job_no*buyer_po_id*updated_by*update_date";
