					...
					<div style="width:1070px; overflow-y:scroll; max-height:300px;">
						<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1050" class="rpt_table" id="list_view" >
							...
							foreach($sql_res as $row)
							{
								...
								$str="";
								$str=$row[csf('production_id')].'___'.$row[csf('sys_no')].'___'.$row[csf('embellishment_job')].'___'.$row[csf('order_id')].'___'.$row[csf('order_no')].'___'.$row[csf('party_id')].'___'.$row[csf('within_group')].'___'.$qty.'___'.$row[csf('buyer_po_id')].'___'.$buyer_po.'___'.$buyer_style;
...
if($action=="order_details")
{
	...
	//if($update_id!=0)
	//{	
		$qcData=sql_select("select b.id, b.mst_id, b.color_size_id, b.qcpass_qty, b.physical_qty, b.reje_qty, b.shift_id, b.remarks from subcon_embel_production_mst a, subcon_embel_production_dtls b where a.recipe_id='$production_id' and a.id=b.mst_id and a.entry_form=223 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
	...
	foreach($sql_res as $row)
	{
		...
		<tr class="general" name="tr[]" id="tr_<? echo $i;?>">
            ...
            <td>
                <input type="text" name="txtRejQty[]" id="txtRejQty_<? echo $i;?>" class="text_boxes_numeric" style="width:55px" placeholder="Write" value="<? echo $rej_qty; ?>" onBlur="fnc_total_calculate();"  onClick="openmypage_order_rate(1,'<? echo $row[csf('order_id')]; ?>',<? echo $i; ?>)" /><!--onBlur="fnc_calculate_qcqty(<? //echo $i;?>);"-->
                <input type="hidden" name="hdnDtlsdata_<? echo $i; ?>" id="hdnDtlsdata_<? echo $i; ?>" value="<? echo implode("__",array_filter(explode(',',$data_break_arr[$dtls_id]))); ?>">
            </td>
            <td>
				...
				<form name="searchprocessfrm_1"  id="searchprocessfrm_1" autocomplete="off">
					...
					<div style="width:820px; overflow-y:scroll; max-height:300px;">
						<table cellspacing="0" cellpadding="0" border="1" rules="all" width="800" class="rpt_table" id="tbl_list_search" >
							...
							$sql="select a.id, a.prefix_no_num, a.sys_no, a.location_id, a.recipe_id, a.job_no, a.order_id, a.buyer_po_id, $color_size_id_str as color_size_id, sum(b.qcpass_qty) as qc_qty from subcon_embel_production_mst a, subcon_embel_production_dtls b where a.id=b.mst_id and a.entry_form=223 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.company_id='$company_id' $location_cond $system_no_cond $production_date $qc_no_cond $po_idsCond group by a.id, a.prefix_no_num, a.sys_no, a.location_id, a.recipe_id, a.job_no, a.order_id, a.buyer_po_id order by a.id DESC";// $date_cond
							//echo $sql; die;
							$sql_res=sql_select($sql);
							...
							foreach($sql_res as $row)
							{
								...
								$str_data=$row[csf('id')].'***'.$row[csf('sys_no')].'***'.$row[csf('location_id')].'***'.$row[csf('recipe_id')].'***'.$prodno_arr[$row[csf('recipe_id')]].'***'.$row[csf('job_no')].'***'.$row[csf('order_id')].'***'.$order_arr[$row[csf('order_id')]]['po'].'***'.$order_arr[$row[csf('order_id')]]['within_group'].'***'.$order_arr[$row[csf('order_id')]]['party_id'].'***'.$order_arr[$row[csf('order_id')]]['qty'].'***'.$prod_data_arr[$row[csf('id')]]['production_date'].'***'.$prod_data_arr[$row[csf('id')]]['operator_name'].'***'.$row[csf('buyer_po_id')].'***'.$buyer_po_arr[$row[csf("order_id")]]['po'].'***'.$buyer_po_arr[$row[csf("order_id")]]['style'].'***'.$prod_data_arr[$row[csf('id')]]['shift_id'];
								...
								<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value('<? echo $str_data; ?>')"> 
                                    ...
                                    <td width="100" style="word-break:break-all"><?php echo $buyer_po_arr[$row[csf("order_id")]]['po']; ?></td>
                                    <td width="100" style="word-break:break-all"><?php echo $buyer_po_arr[$row[csf("order_id")]]['style']; ?></td>
...
<body>
	<div align="center" style="width:100%;" >
		<form name="ratepopup_1"  id="ratepopup_1" autocomplete="off">
			<table class="rpt_table" width="230px" cellspacing="0" cellpadding="0" rules="all" id="tbl_share_details_entry">
				<thead>
					<th width="130">Reject Name</th>
                    <th>Defect Qty</th>
				</thead>
				<tbody>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   // Insert Here==============================================================
	{
		...
		$new_return_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'EQC', date("Y",time()), 5, "select id,prefix_no,prefix_no_num from  subcon_embel_production_mst where company_id=$cbo_company_id and entry_form=223 $year_cond=".date('Y',time())." order by id desc ", "prefix_no", "prefix_no_num" ));
		...
		$data_array="(".$id.",'".$new_return_no[1]."','".$new_return_no[2]."','".$new_return_no[0]."',".$cbo_company_id.",".$cbo_location.",".$txt_production_id.",".$txt_job_no.",".$txt_order_id.",".$txtbuyerPoId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0,223)"; 
...
$sql="select id, prefix_no_num, sys_no, location_id, recipe_id, job_no, order_id from subcon_embel_production_mst where entry_form=222 and status_active = 1 and is_deleted = 0 and company_id='$company_id' $location_cond $system_no_cond $recipe_no_cond order by id DESC";
	...
	$sql_mst = "select id, prefix_no_num, sys_no, location_id, recipe_id, job_no, order_id from subcon_embel_production_mst where entry_form=222 and id='$data[1]'";
    ...
    <div style="width:930px; font-size:6px">
        ...
        <div style="width:100%;">
            <table align="right" cellspacing="0" width="930" border="1" rules="all" class="rpt_table">
				...
				and a.entry_form=204 and d.entry_form=220 and a.status_active=1 and a.company_id='$com_id' and d.id='$recipe_id' group by a.id, a.embellishment_job, b.main_process_id, c.id, c.item_id, c.embellishment_type, c.body_part, c.color_id, c.size_id, d.recipe_no_prefix_num order by d.recipe_no_prefix_num DESC";
				//echo $sql; die;
				$sql_res=sql_select($sql);
...
if ($action == 'show_production_no')
{
	...
	$sql = "select a.id, a.sys_no, a.prefix_no_num, a.job_no, a.order_id, a.buyer_po_id, sum(b.qcpass_qty) as qty from subcon_embel_production_mst a, subcon_embel_production_dtls b where a.id=b.mst_id and a.entry_form=222 and a.company_id='$company_id' $job_no_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
	...
	<!--<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="500">-->
     <table width="430" class="rpt_table" cellpadding="0" cellspacing="0" rules="all" border="1">
		<thead>
	...
	<div style="width:450px; max-height:250px; overflow-y:scroll" align="left">
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="430" class="rpt_table" id="tbl_prod_list_search">
			<?
			$i = 1;
			foreach ($data_array as $row)
			{
				...
				if($balance_qty>0)
				{
					...
					<tr bgcolor="<? echo $bgcolor; ?>" onClick='set_form_data("<? echo $str; ?>")' style="cursor:pointer">
						...
						<td width="50" align="right"><? echo $row[csf("qty")]; ?></td>
                        <td width="50" align="right"><? echo $qc_qty; ?></td>
                        <td width="50" align="right"><? echo $reje_qty; ?></td>
