...
{  
    ...
    from sub_material_mst a,sub_material_dtls b,subcon_ord_dtls c where a.id=b.mst_id  and c.id=b.job_dtls_id $sql_con and a.entry_form in( 205,207) and a.is_deleted=0 and a.status_active=1  and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 group by a.embl_job_no,a.entry_form";
	...
	$sql_production="SELECT a.job_no,a.entry_form,sum(b.qcpass_qty) as qcpass_qty,sum(b.reje_qty) as reje_qty
  	from subcon_embel_production_mst a,subcon_embel_production_dtls b,subcon_ord_dtls c 
  	where a.id=b.mst_id and c.id=a.order_id $sql_con $prod_date_cond and a.entry_form in( 222,223)  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1
   	group by a.job_no,a.entry_form";
	...
	$sql_bill="SELECT c.job_no_mst as job_no, sum(b.delivery_qty) as tbill_qty, sum(b.amount) as total_bill_amount from subcon_inbound_bill_mst a,subcon_inbound_bill_dtls b,subcon_ord_dtls c where a.id=b.mst_id and c.id=b.order_id $company_name  $sql_con $bill_date_cond and b.process_id=13 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 group by c.job_no_mst"; 
	// echo $sql_bill;
                ...
                <table cellspacing="0" border="1" class="rpt_table"  width="1795" rules="all" id="table_body" cellpadding="0">
					<tbody>
						...
						foreach ($sql_result as $val) 
						{
							...
							$prod_qty 		= $productionDataArr[$val[csf('job_no')]][222]['qcpass_qty'];
							$qc_qty 		= $productionDataArr[$val[csf('job_no')]][223]['qcpass_qty'];
							$rej_qty 		= $productionDataArr[$val[csf('job_no')]][223]['reje_qty'];
							...
							if($mat_rcv_qty !=0 || $mat_issue_qty !=0 || $prod_qty !=0 || $qc_qty !=0 || $delivery_qty !=0 || $bill_qty !=0)
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
			                        ...
			                        <td width="80" align="right">
			                        	<a href="javascript:void()" onclick="open_popup('<? echo $job_no.'_'.$order_id.'_'.$within_group.'_'.$date_from.'_'.$date_to;?>',0,'Order Qnty Popup','order_popup');">
			                        ...
			                        <td width="80" align="right">
			                        	<a href="javascript:void()" onclick="open_popup('<? echo $job_no.'_'.$order_id.'_'.$within_group.'_'.$date_from.'_'.$date_to;?>',1,'Production Qnty Popup','production_popup');">
			                        	<? echo number_format($prod_qty,0); ?>
			                        	</a>		                        		
			                        </td>
			                        <td width="80" align="right">
			                        	<a href="javascript:void()" onclick="open_popup('<? echo $job_no.'_'.$order_id.'_'.$within_group.'_'.$date_from.'_'.$date_to;?>',2,'QC Pass Qnty Popup','production_popup');">
                ...
                <table width="1795" border="1" class="rpt_table" rules="all" id="report_table_footer" cellpadding="0" cellspacing="0">
                    <tfoot>	                    
	                    <tr>
	                        ...
	                        <th id="tot_bill_amt" width=""><? echo number_format($tot_bill_amt,2); ?></th>                        
	                    </tr>
                    </tfoot>
                </table>
	           </div>     	
