...
if($action=="embl_bill_issue_print")
{
	...
	/*$delivery_arr=array();
	$delivery_sql = "SELECT b.id, a.delivery_prefix_num, a.delivery_no from subcon_delivery_mst a, subcon_delivery_dtls b where a.id=b.mst_id and a.company_id='$company' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
    ...
    <div style="width:1020px; font-size:6px">
        ...
        <table width="100%" cellpadding="0" cellspacing="0" >  
            ...
            <tr>
                ...
                <td width="100" colspan=""><? //echo $dataArray[0][csf('remarks')]; ?></td> -->
            </tr>
        ...
        <div style="width:100%;">
            <table align="right" cellspacing="0" width="1020" border="1" rules="all" class="rpt_table" style="font-size:12px">
                <thead bgcolor="#dddddd" align="right"><!-- style="font-size:12px"-->
                    <th width="30">SL</th>
				...
				//$sql= "SELECT  a.id, a.embellishment_job,a.currency_id, b.id as order_id, b.order_no, b.buyer_po_id, b.main_process_id, b.gmts_item_id, b.embl_type, b.body_part, c.id as color_size_id, c.color_id, c.size_id, c.qnty as qty, d.delivery_qty, d.rate, d.amount, d.remarks, d.delivery_id, d.delivery_date, d.mst_id 
				//from subcon_ord_mst a, subcon_ord_dtls b, subcon_ord_breakdown c, subcon_inbound_bill_dtls d 
				//where a.embellishment_job=b.job_no_mst and b.id=c.mst_id and c.id=d.color_size_id and a.entry_form in(204,311) and d.process_id in(13,14)  and d.entry_form in(395,332)  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and a.company_id='$com_id' and d.mst_id in($bill_mst_id) order by d.delivery_id ASC";

				//and b.main_process_id in($main_process_id) and b.id in($order_dtls_id) and c.color_id in($color_id) and b.gmts_item_id in($gmts_item_id)
				...
				foreach($sql_res as $row)
		        {
		 		...
		 		/*$dtls_sql= "SELECT  d.delivery_qty, d.rate, d.amount, d.mst_id from subcon_inbound_bill_dtls d where d.mst_id in($bill_mst_id) and d.status_active=1 and d.is_deleted=0 order by d.delivery_id ASC";
		 		//echo $dtls_sql; //die;
				...
				foreach($dtls_sql_res as $row)
		        {
		        	...
		        	$dtls_data_array[$row[csf('mst_id')]]['rate']=$row[csf('amount')]/$row[csf('delivery_qty')];
		        	$dtls_data_array[$row[csf('mst_id')]]['amount']+=$row[csf('amount')];
		        }*/
				...
				{
					$row_span_arr[$delivery_arr[$row["delivery_id"]]]+=1;
				}*/
						
				//print_r($row_span_arr);
 				$i=1; $grand_tot_qty=0; $k=1;  //$del_id_chk=array();
						...
						foreach($order_id_arr as $gmts_item_id=>$gmts_item_id_arr)
						{
							foreach($gmts_item_id_arr as $main_process_id=>$main_process_id_data)
							{
								foreach($main_process_id_data as $color_id=>$row)
								{
									...
									/*if (!in_array($delivery_arr[$row["delivery_id"]],$del_id_chk))
									{
										$i++; 
										$del_id_chk[]=$delivery_arr[$row["delivery_id"]];
									}
									else
									{
										$i=$i;
									...
									//$r_span=$row_span_arr[$delivery_arr[$row["delivery_id"]]];
									if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
									?>
				                    <tr bgcolor="<? echo $bgcolor; ?>">
				                    	<?
				                    	//if(!$chk_span_arr[$delivery_arr[$row["delivery_id"]]])
				                    	//{
				                    		?>
				                    			<td rowspan="<? //echo $r_span; ?>"><? echo $i; ?></td>
				                    			<!-- <td rowspan="<? //echo $r_span; ?>" style="word-break:break-all"><? //echo $delivery_arr[$row["delivery_id"]]; ?></td> -->
				                    			<!-- <td rowspan="<? //echo $r_span; ?>" style="word-break:break-all"><? //echo change_date_format($row["delivery_date"]); ?></td> -->
				                    		<?
				                    		//$chk_span_arr[$delivery_arr[$row["delivery_id"]]]=420;
				                    	//}

				                    	?>
				                       
				                       
				                        <td style="word-break:break-all"><? echo $bill_id_arr[$row['mst_id']]; ?></td>
				                        <td style="word-break:break-all"><? echo $bill_date_arr[$row['mst_id']]; ?></td>
				                        ...
				                        <td align="right"><? $rate=$dtls_data_array[$row['mst_id']]['rate']; echo number_format($row['rate'], 4, '.', ''); ?>&nbsp;</td>
				                        <td align="right"><? $amount=$dtls_data_array[$row['mst_id']]['amount']; echo number_format($row['amount'], 2, '.', ''); ?>&nbsp;</td>
