...
if ($action=="load_drop_down_material_party_location")
{
	echo create_drop_down( "cbo_material_party_location_id", 150, "SELECT id,location_name from lib_location where company_id in($data) and status_active =1 and is_deleted=0 order by location_name", 'id,location_name', 0, '', $selected, '' );	
	exit();	 
}
...
{
	function returnUnique($innerArr) {
		return array_unique($innerArr);
	}
	...
	$cbo_material_party_id = str_replace("'","",trim($cbo_material_party_id));
	$cbo_material_party_location_id = str_replace("'","",trim($cbo_material_party_location_id));
	...
	$location_librarry = return_library_array("SELECT id, location_name from lib_location where status_active=1 and is_deleted=0", 'id', 'location_name');
	...
	if(str_replace("'","",$cbo_buyer_id)==0)$buyer_con="";else $buyer_con=" and b.buyer_buyer=$cbo_buyer_id";
	...
	if(str_replace("'","",$txt_buyer_po_id)==0)$po_id_con="";else $po_id_con=" and b.buyer_po_id = $txt_buyer_po_id";
	
	if(str_replace("'","",$txt_buyer_po)=='')$po_con="";else $po_con=" and b.buyer_po_no = $txt_buyer_po";
	
	if(str_replace("'","",$txt_style_ref)=='')$style_con="";else $style_con=" and b.buyer_style_ref like('%".trim(str_replace("'","",$txt_style_ref))."%')";
	...
	if($cbo_material_party_id=='') $material_party_cond = ''; else $material_party_cond=" and d.from_company_name in($cbo_material_party_id)";
	if($cbo_material_party_location_id=='') $material_party_location_cond = ''; else $material_party_location_cond=" and d.from_location_name in($cbo_material_party_location_id)";
...
/*	$sql="select
	...
	if($cbo_shiping_status != ""){
		$sql="SELECT a.job_no from subcon_delivery_mst a,subcon_delivery_dtls b where a.id=b.mst_id and b.delivery_status in($cbo_shiping_status)  and a.entry_form=254 and a.within_group=$cbo_within_group and a.company_id=$cbo_company_id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1";
		$pro_sql_result=sql_select($sql);
		foreach($pro_sql_result as $row)
		{
			$deliveryJobArr[$row[csf('job_no')]]=$row[csf('job_no')];
	...
	// else if($db_type==2) $group_concat="rtrim(xmlagg(xmlelement(e,c.id,',').extract('//text()') order by c.id).GetClobVal(),',') as color_size_id";
	...
	sum((c.qnty+((b.wastage*c.qnty)/100))*12) as plan_qty
  from subcon_ord_mst a, subcon_ord_breakdown c ,subcon_ord_dtls b
	left join wo_po_break_down d on b.buyer_po_id=d.id and d.is_deleted=0 and d.status_active=1
	...
	//  sum(c.qnty*12) as order_qty,
	//sum((c.qnty+((b.wastage*c.qnty)/100))*12) as plan_qty
	//sum(c.qnty) as order_qty,
	//sum((b.wastage*c.qnty)/100)) as plan_qty,b.order_uom

	if($material_party_cond != '') {
		/*$sql="select b.id as po_id,a.embellishment_job as job_no ,b.order_no,b.buyer_style_ref as style_ref_no,b.buyer_buyer as buyer_name,b.buyer_po_id, a.job_no_prefix_num as job_prefix, b.buyer_po_no as po_number,b.body_part ,c.color_id, a.delivery_date, $group_concat, sum(c.qnty) as order_qty, sum((c.qnty*b.wastage)+c.qnty*12) as plan_qty, b.order_uom
  		from subcon_ord_mst a, subcon_ord_dtls b, subcon_ord_breakdown c, sub_material_mst d, sub_material_dtls e
		where b.job_no_mst=a.embellishment_job and b.id=c.mst_id and d.id = e.mst_id and b.id = e.job_dtls_id and a.entry_form=204 and a.within_group=$cbo_within_group $material_party_cond $material_party_location_cond $sql_con $job_cond $wo_cond $year_cond $party_con $buyer_con $location_con $party_location_con $po_con $po_id_con $style_con $company_name and a.is_deleted =0 group by a.embellishment_job,b.buyer_style_ref,b.buyer_po_no, b.buyer_buyer, b.buyer_po_id, b.id, b.order_no, a.job_no_prefix_num, a.delivery_date, a.company_id,b.body_part,c.color_id,b.order_uom";*/

		$sql="select b.id as po_id, a.embellishment_job as job_no, b.order_no, e.style_ref_no, e.buyer_name, b.buyer_po_id, a.job_no_prefix_num as job_prefix, d.po_number, b.body_part, c.color_id, a.delivery_date, $group_concat, sum(c.qnty) as order_qty, sum(b.wastage) as wastage_qty, b.order_uom
  		from subcon_ord_mst a, subcon_ord_breakdown c ,subcon_ord_dtls b
  		left join wo_po_break_down d on b.buyer_po_id=d.id and d.is_deleted=0 and d.status_active=1
		left join wo_po_details_master e on d.job_no_mst=e.job_no and e.is_deleted=0 and e.status_active=1
		where b.job_no_mst=a.embellishment_job and b.id=c.mst_id and a.entry_form=204 and a.within_group=$cbo_within_group 
		$sql_con $job_cond $wo_cond $year_cond $party_con $buyer_con $location_con $party_location_con
		$po_con $po_id_con $style_con and a.is_deleted =0
		group by a.embellishment_job,e.style_ref_no,d.po_number, e.buyer_name, b.buyer_po_id, b.id,b.order_no, a.job_no_prefix_num,a.delivery_date, a.company_id,b.body_part,c.color_id, b.order_uom";
	} else {
		$sql="select b.id as po_id,a.embellishment_job as job_no ,b.order_no,b.buyer_style_ref as style_ref_no,b.buyer_buyer as buyer_name,b.buyer_po_id, a.job_no_prefix_num as job_prefix, b.buyer_po_no as po_number,b.body_part ,c.color_id, a.delivery_date, $group_concat, sum(c.qnty) as order_qty, sum((c.qnty*b.wastage)+c.qnty*12) as plan_qty, b.order_uom
  		from subcon_ord_mst a, subcon_ord_breakdown c, subcon_ord_dtls b
		where b.job_no_mst=a.embellishment_job and b.id=c.mst_id and a.entry_form=204 and a.within_group=$cbo_within_group $sql_con $job_cond $wo_cond $year_cond $party_con $buyer_con $location_con $party_location_con $po_con $po_id_con $style_con $company_name and a.is_deleted =0 group by a.embellishment_job,b.buyer_style_ref,b.buyer_po_no, b.buyer_buyer, b.buyer_po_id, b.id, b.order_no, a.job_no_prefix_num, a.delivery_date, a.company_id,b.body_part,c.color_id,b.order_uom";
	}
	// echo $sql;
	...
	a.delivery_date, $group_concat,
	sum(c.qnty) as order_qty,
	sum(b.wastage) as plan_qty,b.order_uom
  from subcon_ord_mst a, subcon_ord_breakdown c ,subcon_ord_dtls b
	left join wo_po_break_down d on b.buyer_po_id=d.id and d.is_deleted=0 and d.status_active=1
	left join wo_po_details_master e on d.job_no_mst=e.job_no and e.is_deleted=0 and e.status_active=1
	...
	$po_con $po_id_con $style_con
	  ...
	  and a.is_deleted =0 group by a.embellishment_job,e.style_ref_no,d.po_number, e.buyer_name, b.buyer_po_id, b.id,b.order_no, a.job_no_prefix_num,a.delivery_date, a.company_id,b.body_part,c.color_id,b.order_uom";*/
	...
	foreach($sql_result as $row)
	{
		if($row[csf('order_uom')]==1)
		{
			...
			$totalPlanQtyPcs+=($row[csf('order_qty')]+(($row[csf('wastage_qty')]*$row[csf('order_qty')])/100));
		}
		if($row[csf('order_uom')]==2)
		{ 
		 	$totalOrderQtyPcs+=$row[csf('order_qty')]*12;
	     	$totalPlanQtyPcs+=($row[csf('order_qty')]+(($row[csf('wastage_qty')]*$row[csf('order_qty')])/100))*12;
		}
		
		$poArr[$row[csf('buyer_po_id')]]=$row[csf('buyer_po_id')];
		$embJobArr[$row[csf('job_no')]]=$row[csf('job_no')];

		$key=$row[csf('job_no')].$row[csf('po_id')].$row[csf('body_part')];

		/*$color_size_id = $row[csf('color_size_id')]->load();
		$color_size_id=array_unique(explode(",",$color_size_id));
		$color_size_id= implode(',', $color_size_id);*/

		$color_size_id = $row[csf('color_size_id')];

		// b.id as po_id,a.embellishment_job as job_no ,b.order_no,b.buyer_style_ref as style_ref_no,b.buyer_buyer as buyer_name,b.buyer_po_id, a.job_no_prefix_num as job_prefix, b.buyer_po_no as po_number,b.body_part ,c.color_id, a.delivery_date, $group_concat, sum(c.qnty*12) as order_qty, sum((c.qnty*b.wastage)+c.qnty*12) as plan_qty, b.order_uom
		$data_arr[$index]['key'] = $key;
		$data_arr[$index]['po_id'] = $row[csf('po_id')];
		$data_arr[$index]['job_no'] = $row[csf('job_no')];
		$data_arr[$index]['order_no'] = $row[csf('order_no')];
		$data_arr[$index]['style_ref_no'] = $row[csf('style_ref_no')];
		$data_arr[$index]['buyer_name'] = $row[csf('buyer_name')];
		$data_arr[$index]['buyer_po_id'] = $row[csf('buyer_po_id')];
		$data_arr[$index]['job_prefix'] = $row[csf('job_prefix')];
		$data_arr[$index]['po_number'] = $row[csf('po_number')];
		$data_arr[$index]['body_part'] = $row[csf('body_part')];
		$data_arr[$index]['color_id'] = $row[csf('color_id')];
		$data_arr[$index]['delivery_date'] = $row[csf('delivery_date')];
		$data_arr[$index]['color_size_id'] = $color_size_id;
		$data_arr[$index]['order_qty'] = $row[csf('order_qty')];
		$data_arr[$index]['wastage_qty'] = $row[csf('wastage_qty')];
		$data_arr[$index]['order_uom'] = $row[csf('order_uom')];

		array_push($wo_no_arr, $row[csf('order_no')]);
	...
	$wo_no_arr=array_unique($wo_no_arr);

	$wo_no_cond=where_con_using_array($wo_no_arr,1,"booking_no");
	$wo_sql= sql_select("select booking_no,is_approved from wo_booking_mst  where   status_active=1 and is_deleted=0 $wo_no_cond");
	$wo_data=array();
	foreach ($wo_sql as $val) {
		$wo_data[$val[csf('booking_no')]]=$val[csf('is_approved')];
	}
	...
	foreach($po_chunk_arr as $po_arr)
	{
		if($p==1){$sql_con =" and (d.embl_job_no in('".implode("','",$po_arr)."')";} 
		else{$sql_con .=" or d.embl_job_no in('".implode("','",$po_arr)."')";}
		$p++;
	}
	$sql_con .=")";
	
    $sql_receive_materials="select d.embl_job_no, c.body_part,c.id as po_id,b.job_break_id,d.entry_form, d.from_company_name, d.from_location_name,
    sum(case when d.subcon_date = $txt_date_from then b.quantity else 0 end) as today_rec_qty,
    sum(case when d.subcon_date < $txt_date_from then b.quantity else 0 end) as prev_rec_qty,
    sum(b.quantity) as total_rec_qty
    from sub_material_mst d,sub_material_dtls b,subcon_ord_dtls c
    where d.id=b.mst_id and c.id=b.job_dtls_id $sql_con $material_party_cond $material_party_location_cond and d.entry_form in(205, 207) and d.is_deleted=0 and d.status_active=1  and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 
    group by d.embl_job_no,c.body_part,c.id,b.job_break_id,d.entry_form, d.from_company_name, d.from_location_name";
    // echo $sql_receive_materials;
	$sql_receive_materials_result=sql_select($sql_receive_materials);
	$materialReceiveDataArr=array();
	$materialIssueDataArr=array();
	$materialPartyDataArr=array();
	foreach($sql_receive_materials_result as $row)
	{
		if($row[csf('entry_form')]==205){//205=Receive;
			...
			$materialPartyDataArr[$row[csf('embl_job_no')]][$row[csf('from_company_name')]][]=$row[csf('from_location_name')];
		}
	...
	foreach ($materialPartyDataArr as $key => $value) {
		$materialPartyDataArr[$key] = array_map('returnUnique', $value);
	}

	// $materialPartyDataArr = array_unique($materialPartyDataArr);
  ...
  where a.id=b.mst_id and c.id=a.order_id $sql_con and a.entry_form in( 222,223)  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1
   group by a.job_no,c.body_part ,c.id,b.color_size_id,a.entry_form";
	...
	sum(b.delivery_qty) as total_delivery_qty, a.deli_party, a.deli_party_location, b.delivery_status
  ...
  where a.id=b.mst_id and c.id=b.order_id $sql_con and a.entry_form=254 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 
   group by a.job_no,c.body_part ,c.id,b.color_size_id, a.deli_party, a.deli_party_location, b.delivery_status";
	...
	foreach($sql_delivery_result as $row)
	{
		...
		$deliveryDataArr[$row[csf('job_no')]]['delivery_location']=$row[csf('deli_party_location')];
		$deliveryDataArr[$row[csf('job_no')]]['delivery_status']=$row[csf('delivery_status')];
	}
   ...
   group by c.job_no_mst,c.body_part ,c.id,b.color_size_id";
        ...
        <table width="100%" cellspacing="0" >
            <tr style="border:none;">
                <td colspan="37" align="center" style="border:none; font-size:14px;">
                    <b><?php echo $company_library[str_replace("'","",$cbo_company_id)]; ?></b>
                </td>
            </tr>
        </table>
        
        <div style="float:left; width:3535px">
            <table width="3535" cellspacing="0" border="1" class="rpt_table" rules="all" id="table_header_1" >
                <thead>
                    <tr>
                        <th rowspan="2" width="35">SL</th>
                        <th rowspan="2" width="200">Material party, Location</th>
                        ...
                        <th colspan="4" width="320">Cut Piece Received Status</th>
                        <th colspan="4" width="320">Cut Panel Issue Status</th>
                        <th colspan="5" width="400">Print Status</th>
                        <th colspan="4" width="320">QC Status</th>
                        <th colspan="5" width="400">Delivery Status</th>
                        <th colspan="3" width="240">Bill Status</th>
                        <th rowspan="2" width="120">Delivery To Location</th>
                        <th rowspan="2" width="100">Delivery Status</th>
                    </tr>
                    <tr>
                        ...
                        <th width="80">Balance Bill Qty</th>
                    </tr>
                </thead>
            </table>
            <div style="max-height:350px; width:3555px; overflow-y:auto;" id="scroll_body" >
                <table cellspacing="0" border="1" class="rpt_table" width="3535" rules="all" id="table_body" >
					<?php
						$i=1;

						foreach($data_arr as $row) {
							$matRcvPartyArr = array();
							$matPartyInfo = '';
							...
							foreach(explode(',',$row['color_size_id']) as $csi){
								$key=$row['job_no'].$row['po_id'].$row['body_part'].$csi;
							...
							// material receive party info.............................................
							$matRcvPartyArr = $materialPartyDataArr[$row['job_no']];
							foreach ($matRcvPartyArr as $partyId => $locationArr) {
								foreach ($locationArr as $value) {
									$matPartyInfo .= $company_library[$partyId] . ', ' . $location_librarry[$value] . '<br>';
								}
							}

							// delivery info.............................................
							$deliveryLocation = $deliveryDataArr[$row['job_no']]['delivery_location'];
							$deliveryStatus = $deliveryDataArr[$row['job_no']]['delivery_status'];
							...
							{
								 $order_qty=$row['order_qty'];
								 $plan_qty=($row['order_qty']+(($row['wastage_qty']*$row['order_qty'])/100));
							}
							if($row['order_uom']==2)
							{ 
								$order_qty=$row['order_qty']*12;
								$plan_qty=($row['order_qty']+(($row['wastage_qty']*$row['order_qty'])/100))*12;
							}

							$summery_arr[$matPartyInfo]['mat_party_info'] = $matPartyInfo;
							$summery_arr[$matPartyInfo]['order_qty'] += $order_qty;
							$summery_arr[$matPartyInfo]['plan_qty'] += $plan_qty;
							$summery_arr[$matPartyInfo]['recv_qty'] += $totalMaterRec;
							$summery_arr[$matPartyInfo]['issue_qty'] += $totalMaterIssue;
							$summery_arr[$matPartyInfo]['production_qty'] += $totalProduc;
							$summery_arr[$matPartyInfo]['qc_qty'] += $totalQc;
							$summery_arr[$matPartyInfo]['delivery_qty'] += $totalDelivery;
							$summery_arr[$matPartyInfo]['delivery_balance_qty'] += ($totalProduc-$totalDelivery);
							
							?>
							<tbody>
							<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
								<td width="35"><?php echo $i; ?></td>
	                            <td width="200"><?php echo $matPartyInfo; ?></td>
	                            <td width="100"><p><? echo $row['job_no'];?></p></td>

	                            <td width="100" align="center"><a href="##" onClick="print_popup('<?php echo $row['order_no']; ?>','<?=$wo_data[$row['order_no']]?>')"><?php echo $row['order_no']; ?></a></td> 

	                            <!-- <td width="100"><p><? //echo $row['order_no'];?></p></td> -->
	                            <td width="100">
	                            	<p style="width:95px; word-break:break-all">
		                            	<?php
		                            		if(str_replace("'","",$cbo_within_group)==1) {
												echo $buyer_arr[$row['buyer_name']];
											}
	                            ...
	                            <td width="130"><p style="width:125px; word-break:break-all"><? echo $row['po_number']; ?></p></td>
	                            <td width="130"><p style="width:125px; word-break:break-all"><? echo $row['style_ref_no']; ?></p></td>
	                            <td width="100"><p style="width:95px; word-break:break-all"><? echo $color_library_arr[$row['color_id']]; ?></p></td>
	                            <td width="100"><p style="width:95px; word-break:break-all"><? echo $body_part[$row['body_part']]; ?></p></td>
	                            <td width="80" align="right" title="<? echo $unit_of_measurement[$row['order_uom']]; ?>"><?php echo number_format($order_qty,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($plan_qty,2); ?></td>
	                            <td width="80" align="center"><? echo change_date_format($buyerCutupDateArr[$row['buyer_po_id']]); ?></td>
	                            <td width="80" align="center"><? echo change_date_format($row['delivery_date']); ?></td>
	                            <td width="80" align="right"><? echo number_format($prevMaterRec,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($toDayMaterRec,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($totalMaterRec,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($plan_qty-$totalMaterRec,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($prevMaterIssue,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($toDayMaterIssue,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($totalMaterIssue,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($totalMaterRec-$totalMaterIssue,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($prevProduc,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($toDayProduc,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($totalProduc,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($plan_qty-$totalProduc,2); ?></td>
	                            <td width="80" align="right"><? echo number_format(($plan_qty-$totalProduc)-($plan_qty-$totalMaterRec),2);?></td>
	                            <td width="80" align="right"><? echo number_format($prevQc,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($toDayQc,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($totalQc,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($totalProduc-$totalQc,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($prevDelivery,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($toDayDelivery,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($totalDelivery,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($totalProduc-$totalDelivery,2); ?></td>
	                            <td width="80" align="right"><? echo number_format(($plan_qty-$totalDelivery),2);?></td>
	                            <td width="80" align="right"><? echo number_format($totalBillQty,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($totalBillAmount,2); ?></td>
	                            <td width="80" align="right"><? echo number_format($plan_qty-$totalBillQty,2); ?></td>
	                            <td width="120"><?php echo $location_librarry[$deliveryLocation]; ?></td>
	                            <td width="100"><?php echo $delivery_status[$deliveryStatus]; ?></td>
							</tr>
	                        </tbody> 	
							<?		
							$i++;
                ...
                <table width="3535" border="1" class="tbl_bottom" rules="all" id="report_table_footer" >
                    <tfoot>
	                    <tr> 
	                        <td width="35"></td>
	                        <td width="200"></td>
	                        ...
	                        <td width="100"></td>
	                        <td width="100"></td>
	                        <td width="80" align="right" id="gt_order_qty_id"></td>
	                        <td width="80" align="right" id="gt_plan_qty_id"></td>
	                        <td width="80"></td>
	                        <td width="80"></td>
	                       
	                        <td width="80" align="right" id="gt_rec_prev_qty_id"></td>
	                        <td width="80" align="right" id="gt_rec_today_qty_id"></td>
	                        <td width="80" align="right" id="gt_rec_total_qty_id"></td>
	                        <td width="80" align="right" id="gt_rec_bal_qty_id"><? // echo number_format($totalPlanQtyPcs-$grand_totalMaterRec,2);?></td>
	                        <td width="80" align="right" id="gt_issue_prev_qty_id"></td>
	                        <td width="80" align="right" id="gt_issue_today_qty_id"></td>
	                        <td width="80" align="right" id="gt_issue_total_qty_id"></td>
	                        <td width="80" align="right" id="gt_issue_bal_qty_id"></td>
	                        
	                        <td width="80" align="right" id="gt_print_prev_qty_id"></td>
	                        <td width="80" align="right" id="gt_print_today_qty_id"></td>
	                        <td width="80" align="right" id="gt_print_total_qty_id"></td>
	                        <td width="80" align="right" id="gt_print_bal_qty_id"></td>
	                        <td width="80" align="right" id="gt_print_wip_qty_id"></td>                        
	                        
	                        <td width="80" align="right" id="gt_qc_prev_qty_id"></td>
	                        <td width="80" align="right" id="gt_qc_today_qty_id"></td>
	                        <td width="80" align="right" id="gt_qc_total_qty_id"></td>
	                        <td width="80" align="right" id="gt_qc_bal_qty_id"></td>
	                           
	                        <td width="80" align="right" id="gt_delivery_prev_qty_id"></td>
	                        <td width="80" align="right" id="gt_delivery_today_qty_id"></td>
	                        <td width="80" align="right" id="gt_delivery_total_qty_id"></td>
	                        <td width="80" align="right" id="gt_delivery_bal_qty_id"></td>
	                        <td width="80" align="right" id="gt_delivery_loq_qty_id"></td>
	                        
	                        <td width="80" align="right" id="gt_bill_total_qty_id"></td>
	                        <td width="80" align="right" id="gt_bill_total_amount_id"></td>
	                        <td width="80" align="right" id="gt_bill_bal_qty_id"></td>
	                        <td width="120"></td>
	                        <td width="100"></td>
	                	</tr>
                    </tfoot>
	...
	<div class="summery">
		<table width="90%" border="1" rules="all" class="rpt_table" id="summery1">
			<thead>
				<tr>
					<th colspan="10">Summery</th>
				</tr>
				<tr>
					<th>Material party,Location</th>
					<th>Order Qty</th>
					<th>Plan Qty</th>
					<th>Rcvd Qty</th>
					<th>Issue Qty</th>
			...
			<tbody>
				<?php
					...
					$tot_qcQty = 0;
					$tot_deliveryQty = 0;
					$tot_deliveryBalance = 0;
					foreach ($summery_arr as $value) {
						$matPartyInfo = $value['mat_party_info'];
						$orderQty = $value['order_qty'];
						$planQty = $value['plan_qty'];
						$rcvQty = $value['recv_qty'];
						$issueQty = $value['issue_qty'];
						$productionQty = $value['production_qty'];
						$qcQty = $value['qc_qty'];
						$deliveryQty = $value['delivery_qty'];
						$deliveryBalance = $productionQty - $deliveryQty;
						$deliveryParcent = ($deliveryQty / $planQty) * 100;

						$tot_orderQty += $orderQty;
						$tot_planQty += $planQty;
						$tot_rcvQty += $rcvQty;
						$tot_issueQty += $issueQty;
						$tot_productionQty += $productionQty;
						$tot_qcQty += $qcQty;
						$tot_deliveryQty += $deliveryQty;
						$tot_deliveryBalance += $deliveryBalance;
						?>
						<tr>
							<td><?php echo $matPartyInfo; ?></td>
							<td><?php echo number_format($orderQty, 2); ?></td>
							<td><?php echo number_format($planQty, 2); ?></td>
							<td><?php echo number_format($rcvQty, 2); ?></td>
							<td><?php echo number_format($issueQty, 2); ?></td>
							<td><?php echo number_format($productionQty, 2); ?></td>
							<td><?php echo number_format($qcQty, 2); ?></td>
							<td><?php echo number_format($deliveryQty, 2); ?></td>
							<td><?php echo number_format($deliveryBalance, 2); ?></td>
							<td><?php echo number_format($deliveryParcent, 2); ?></td>
						</tr>
						<?php
					}
				?>
			...
			<tfoot>
				<th>Total:</th>
				<th><?php echo number_format($tot_orderQty, 2); ?></th>
				<th><?php echo number_format($tot_planQty, 2); ?></th>
				<th><?php echo number_format($tot_rcvQty, 2); ?></th>
				<th><?php echo number_format($tot_issueQty, 2); ?></th>
				<th><?php echo number_format($tot_productionQty, 2); ?></th>
				<th><?php echo number_format($tot_qcQty, 2); ?></th>
				<th><?php echo number_format($tot_deliveryQty, 2); ?></th>
				<th><?php echo number_format($tot_deliveryBalance, 2); ?></th>
				<th></th>
			</tfoot>
		</table>
	</div>
	<?php
	$summeryData=ob_get_contents();
	ob_clean();

	$html = $mainReportData.'!!!!!'.$summeryData;
	$excelFile = $mainReportData.$summeryData;

	$new_link=create_delete_report_file( $excelFile, 1, 1, '../../../' );
	...
	$is_created = fwrite($create_new_doc, $excelFile);
	echo "$html**$filename";
	exit();
}
...
/*	$sql="select
	...
	//sum(((c.qnty*12)*b.wastage)/100+(c.qnty*12)) as plan_qty according to Metro pepul
	/*$sql="SELECT  b.id as po_id,a.embellishment_job as job_no , b.order_no,e.style_ref_no,e.buyer_name,b.buyer_po_id, a.job_no_prefix_num as job_prefix, d.po_number,b.body_part ,c.color_id,
	a.delivery_date, $group_concat,
	sum(c.qnty*12) as order_qty,
	sum(((c.qnty*12)*b.wastage)/100+(c.qnty*12)) as plan_qty
  from subcon_ord_mst a, subcon_ord_breakdown c ,subcon_ord_dtls b
	left join wo_po_break_down d on b.buyer_po_id=d.id and d.is_deleted=0 and d.status_active=1
	left join wo_po_details_master e on d.job_no_mst=e.job_no and e.is_deleted=0 and e.status_active=1
	...
	where b.job_no_mst=a.embellishment_job and b.id=c.mst_id and a.entry_form=204 and a.within_group=$cbo_within_group 
	$sql_con $job_cond $wo_cond $year_cond $party_con $buyer_con $location_con $party_location_con
	$po_con $po_id_con $style_con and c.qnty<>0
  ...
  from subcon_ord_mst a, subcon_ord_breakdown c ,subcon_ord_dtls b
	...
	foreach($sql_result as $row)
	{
		...
		/*if($row[csf('order_uom')]==1)
		{
			...
			//sum((c.qnty+((b.wastage*c.qnty)/100))*12) as plan_qty
			 $totalOrderQtyPcs+=$row[csf('order_qty')];
			 $totalPlanQtyPcs+=($row[csf('order_qty')]+(($row[csf('plan_qty')]*$row[csf('order_qty')])/100));
		}
		if($row[csf('order_uom')]==2)
		{ 
		 	$totalOrderQtyPcs+=$row[csf('order_qty')]*12;
	     	$totalPlanQtyPcs+=($row[csf('order_qty')]+(($row[csf('plan_qty')]*$row[csf('order_qty')])/100))*12;
		}*/
		...
		{
			 $totalOrderQtyPcs+=$row[csf('order_qty')];
			 $totalPlanQtyPcs+=(($row[csf('order_qty')])*$row[csf('plan_qty')])/100+($row[csf('order_qty')]);
			 
		}
		if($row[csf('order_uom')]==2)
		{ 
		 $totalOrderQtyPcs+=$row[csf('order_qty')]*12;
	     $totalPlanQtyPcs+=(($row[csf('order_qty')]*12)*$row[csf('plan_qty')])/100+($row[csf('order_qty')]*12);
		}
            ...
            <div style="max-height:350px; width:3220px; overflow-y:auto;" id="scroll_body" >
					...
					foreach($sql_result as $row)
					{
						...
						foreach(explode(',',$row[csf('color_size_id')]) as $csi){
							...
							if($row[csf('order_uom')]==1)
							{
								...
								$plan_qty=(($row[csf('order_qty')])*$row[csf('plan_qty')])/100+($row[csf('order_qty')]);
								$po_totalOrderQtyPcs+=$order_qty;
								$po_totalPlanQtyPcs+=$plan_qty;
								
								$buyer_totalOrderQtyPcs+=$order_qty;
								$buyer_totalPlanQtyPcs+=$plan_qty;
							
								$grand_totalOrderQtyPcs+=$order_qty;
								$grand_totalPlanQtyPcs+=$plan_qty;								
							
							}
							if($row[csf('order_uom')]==2)
							{ 
								$order_qty=$row[csf('order_qty')]*12;
								$plan_qty=(($row[csf('order_qty')]*12)*$row[csf('plan_qty')])/100+($row[csf('order_qty')]*12);
								$po_totalOrderQtyPcs+=$order_qty;
								$po_totalPlanQtyPcs+=$plan_qty;
						...
						?>
                          ...
                          <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                            ...
                            <td width="80" align="right" title="<? echo $unit_of_measurement[$row[csf('order_uom')]]; ?>"><? echo number_format($order_qty,2); ?></td>
                            <td width="80" align="right"><? echo number_format($plan_qty,2); ?></td>
                            ...
                            <td align="right" width="80"><? echo number_format($plan_qty-$totalMaterRec,2); ?></td>
                            ...
                            <td align="right" width="80"><? echo number_format($plan_qty-$totalProduc,2); ?></td>
                            <td align="right" width="80"><? echo number_format(($plan_qty-$totalProduc)-($plan_qty-$totalMaterRec),2);?></td>
                            ...
                            <td align="right" width="80"><? echo number_format(($plan_qty-$totalDelivery),2);?></td>
                            ...
                            <td align="right"><? echo number_format($plan_qty-$totalBillQty,2); ?></td>
						  </tr>
                ...
                <table width="3200" border="1" class="tbl_bottom" rules="all" id="report_table_footer" >
                    ...
                    <tr> 
                        ...
                        <td width="130" class="wrd_brk"></td>
                        <td width="100" class="wrd_brk"> </td>
                        <td width="100" class="wrd_brk" align="right">Grand Total: </td>
                        <td width="80" class="wrd_brk" align="right" id="gt_order_qty_id"><? echo number_format($grand_totalOrderQtyPcs,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_plan_qty_id"><? echo number_format($grand_totalPlanQtyPcs,2);?></td>
                        <td width="80" class="wrd_brk"></td>
                        <td width="80" class="wrd_brk"></td>
                       
                        <td width="80" class="wrd_brk" align="right" id="gt_rec_prev_qty_id"><? echo number_format($grand_prevMaterRec,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_rec_today_qty_id"><? echo number_format($grand_toDayMaterRec,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_rec_total_qty_id"><? echo number_format($grand_totalMaterRec,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_rec_bal_qty_id"><? echo number_format($grand_totalPlanQtyPcs-$grand_totalMaterRec,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_issue_prev_qty_id"><? echo number_format($grand_prevMaterIssue,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_issue_today_qty_id"><? echo number_format($grand_toDayMaterIssue,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_issue_total_qty_id"><? echo number_format($grand_totalMaterIssue,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_issue_bal_qty_id"><? echo number_format($grand_totalMaterRec-$grand_totalMaterIssue,2);?></td>
                        
                        <td width="80" class="wrd_brk" align="right" id="gt_print_prev_qty_id"><? echo number_format($grand_prevProduc,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_print_today_qty_id"><? echo number_format($grand_toDayProduc,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_print_total_qty_id"><? echo number_format($grand_totalProduc,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_print_bal_qty_id"><? echo number_format($totalPlanQtyPcs-$grand_totalProduc,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_print_wip_qty_id"><? echo number_format(($grand_totalPlanQtyPcs-$grand_totalProduc)-($grand_totalPlanQtyPcs-$grand_totalMaterRec),2);?></td>
                        
                        
                        <td width="80" class="wrd_brk" align="right" id="gt_qc_prev_qty_id"><? echo number_format($grand_prevQc,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_qc_today_qty_id"><? echo number_format($grand_toDayQc,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_qc_total_qty_id"><? echo number_format($grand_totalQc,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_qc_bal_qty_id"><? echo number_format($grand_totalProduc-$grand_totalQc,2);?></td>
                        
                        
                        
                        
                        <td width="80" class="wrd_brk" align="right" id="gt_delivery_prev_qty_id"><? echo number_format($grand_prevDelivery,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_delivery_today_qty_id"><? echo number_format($grand_toDayDelivery,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_delivery_total_qty_id"><? echo number_format($grand_totalDelivery,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_delivery_bal_qty_id"><? echo number_format($grand_totalProduc-$grand_totalDelivery,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_delivery_loq_qty_id"><? echo number_format(($grand_totalPlanQtyPcs-$grand_totalDelivery),2);?></td>
                        
                        <td width="80" class="wrd_brk" align="right" id="gt_bill_total_qty_id"><? echo number_format($grand_totalBillQty,2);?></td>
                        <td width="80" class="wrd_brk" align="right" id="gt_bill_total_amount_id"><? echo number_format($grand_totalBillAmount,2);?></td>
                        <td class="wrd_brk" align="right" id="gt_bill_bal_qty_id"><? echo number_format($grand_totalPlanQtyPcs-$grand_totalBillQty,2);?></td>
