...
if ($action=="load_drop_down_party")
{
	...
	if($data[1]==1)
	{
		echo create_drop_down( "cbo_party_id", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "--Select Party--", $data[2], "load_drop_down( 'requires/embellishment_production_status_report_controller', this.value, 'load_drop_down_party_location', 'party_location_td' );");
	}
	else
	{
                ...
                <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_party_id').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+<? echo $data[2];?>, 'job_search_list_view', 'search_div', 'embl_production_status_report_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />
                            </td>
                        </tr>
                        <tr>
...
{	
	...
	$sql="select a.id, a.embellishment_job, a.job_no_prefix_num, $ins_year_cond as year, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date, $color_id_str as color_id, b.id, b.buyer_po_id 
  from subcon_ord_mst a, subcon_ord_breakdown c, subcon_ord_dtls b
	
	where b.job_no_mst=a.embellishment_job and b.id=c.mst_id and a.entry_form=204 $company $party $year_field_cond $search_com_cond $withinGroup $po_idsCond and a.is_deleted =0 group by a.id, a.embellishment_job, a.job_no_prefix_num, a.insert_date, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date, b.id, b.buyer_po_id order by b.id desc";
	 //echo $sql;
	...
	?>
     <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="885" >
        ...
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="865" class="rpt_table" id="list_view">
        <tbody>
            <? 
            $i=1;
            ...
            {  
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" onClick='js_set_value("<? echo $row[csf('embellishment_job')]; ?>")' style="cursor:pointer" >
                    ...
                    <td width="100" style="word-break:break-all"><? echo $buyer_po_arr[$row[csf('buyer_po_id')]]['po']; ?></td>
                    <td width="100" style="word-break:break-all"><? echo $buyer_po_arr[$row[csf('buyer_po_id')]]['style']; ?></td>
...
if($action=="order_search_list_view")
{
	...
	if($search_type==1)
	{
		if($search_str!="")
		{
			...
			else if ($search_by==3) $job_cond=" and e.job_no_prefix_num = '$search_str' ";
			else if ($search_by==4) $po_cond=" and d.po_number = '$search_str' ";
			else if ($search_by==5) $style_cond=" and e.style_ref_no = '$search_str' ";
		}
	}
	else if($search_type==4 || $search_type==0)
	{
		if($search_str!="")
		{
			...
			else if ($search_by==3) $job_cond=" and e.job_no_prefix_num like '%$search_str%'";  
			else if ($search_by==4) $po_cond=" and d.po_number like '%$search_str%'";
			else if ($search_by==5) $style_cond=" and e.style_ref_no like '%$search_str%'";    
		}
	}
	else if($search_type==2)
	{
...
if ($action=="style_no_popup")
{
	...
	<script>
		...
		function fnc_load_party_order_popup(company,within_group,party)
		{
			load_drop_down( 'embl_production_status_report_controller', company+'_'+within_group+'_'+party, 'load_drop_down_party', 'party_td' );
                ...
                <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_party_id').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+<? echo $within_group;?>+'_'+<? echo $year;?>, 'style_no_search_list_view', 'search_div', 'embl_production_status_report_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />
                            </td>
                        </tr>
                        <tr>
                            <td colspan="6" align="center" height="40" valign="middle">
...
{  
	...
	if(str_replace("'","",trim($txt_job_no))!="") $job_cond="and a.embellishment_job = $txt_job_no"; else $job_cond="";  
	...
	$sql="select
	a.job_no, a.order_id, a.buyer_po_id, b.qcpass_qty, b.color_size_id from subcon_embel_production_mst a,subcon_embel_production_dtls b where a.id=b.mst_id and a.entry_form=222 $year_field_cond $date_con and a.company_id=$cbo_company_id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1";
	...
	//Emb. Order Entry..............................................................
	$p=1;
	$job_chunk_arr=array_chunk($embJobArr,999);
  ...
  from subcon_ord_mst a, subcon_ord_breakdown c ,subcon_ord_dtls b
    ...
    a.embl_job_no,c.body_part,c.id as po_id,b.job_break_id,
    ...
    group by a.embl_job_no,c.body_part,c.id,b.job_break_id";
	...
	$sql_con .=")";
	$sql_production="select a.job_no,c.body_part,c.id as po_id,b.color_size_id,
            ...
            <div style="max-height:425px; width:2330px" >
				...
				<?  
					...
					foreach($sql_result as $row)
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_1nd<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_1nd<? echo $i; ?>">
                            ...
                            <td width="100"><? echo $color_library_arr[$row[csf('color_id')]]; ?></td>
                ...
                <table width="2300" border="1" class="tbl_bottom" rules="all" id="" >
                    <tr> 
                        ...
                        <td width="80" align="right"><? echo number_format($grand_prevMaterRec,2);?></td>
                        <td width="80" align="right"><? echo number_format($grand_toDayMaterRec,2);?></td>
                        <td width="80" align="right"><? echo number_format($grand_totalMaterRec,2);?></td>
                        <td width="80" align="right"><? echo number_format($totalPlanQtyPcs-$grand_totalMaterRec,2);?></td>
                        
                        <td width="80" align="right"><? echo number_format($grand_prevProduc,2);?></td>
                        <td width="80" align="right"><? echo number_format($grand_toDayProduc,2);?></td>
                        <td width="80" align="right"><? echo number_format($grand_totalProduc,2);?></td>
                        <td width="80" align="right"><? echo number_format($totalPlanQtyPcs-$grand_totalProduc,2);?></td>
                        <td width="80" align="right"><? echo number_format(($totalPlanQtyPcs-$grand_totalProduc)-($totalPlanQtyPcs-$grand_totalMaterRec),2);?></td>
