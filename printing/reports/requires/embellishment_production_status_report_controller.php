...
{	
	...
	$party_id=trim(str_replace("'","",$data[1]));
	$search_type =$data[6];
	$within_group =$data[7];
	...
	if($search_type==1)
	{
		if($search_str!="")
		{
			...
			else if ($search_by==4) {$search_com_cond=" and d.po_number = '$search_str' "; $search_com_cond=" and b.buyer_po_no = '$search_str'";}
			//else if ($search_by==5) $search_com_cond=" and e.style_ref_no = '$search_str' ";
			else if ($search_by==5){ $search_com_cond=" and e.style_ref_no = '$search_str' ";  $search_com_cond=" and b.buyer_style_ref = '$search_str'";}
	...
	$sql="SELECT a.id, a.embellishment_job, a.job_no_prefix_num, $ins_year_cond as year, a.company_id, a.location_id, a.party_id, a.receive_date, a.within_group, b.buyer_po_no, b.buyer_style_ref, a.order_no, a.delivery_date, $color_id_str as color_id, b.buyer_po_id 
            ...
            {  
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" onClick='js_set_value("<? echo $row[csf('embellishment_job')]; ?>")' style="cursor:pointer" >
                    ...
                    if ($row[csf('within_group')]==1) {
                     ...
                     //echo $buyer_po_arr[$row[csf('buyer_po_id')]]['style']; ?></td>
...
/*	$sql="select
  ...
  from subcon_ord_mst a, subcon_ord_breakdown c ,subcon_ord_dtls b
	...
	if(str_replace("'","",trim($cbo_within_group))==1)
	{
 		if(str_replace("'","",$txt_int_ref)=='') $int_ref_con="";else $int_ref_con=" and b.grouping like('%".trim(str_replace("'","",$txt_int_ref))."%')";
		if($db_type==0) $id_cond="group_concat(b.id)";
		else if($db_type==2) $id_cond="rtrim(xmlagg(xmlelement(e,b.id,',').extract('//text()') order by b.id).GetClobVal(),',') ";
		// echo "select $id_cond as id from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst $int_ref_con";die;
		if(($int_ref_con!=""))
		{
			$po_ids = return_field_value("$id_cond as id", "wo_po_details_master a, wo_po_break_down b", "a.job_no=b.job_no_mst $int_ref_con and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0", "id");
		}
		//echo $po_ids;
		...
		$po_sql ="SELECT a.job_no_prefix_num, a.style_ref_no, b.id, b.po_number,b.grouping from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst $poIdsCond and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0";
		$po_sql_res=sql_select($po_sql);
		foreach ($po_sql_res as $row)
		{
			$buyer_po_arr[$row[csf("id")]]['style']=$row[csf("style_ref_no")];
	...
	$sql="select  b.id as po_id,a.embellishment_job as job_no ,b.order_no,b.buyer_style_ref as style_ref_no,b.buyer_buyer as buyer_name,b.buyer_po_id, a.job_no_prefix_num as job_prefix, b.buyer_po_no as po_number,b.body_part ,c.color_id,
	a.delivery_date, $group_concat,
	sum(c.qnty) as order_qty,
	sum(b.wastage) as plan_qty,b.order_uom
	from subcon_ord_mst a, subcon_ord_breakdown c ,subcon_ord_dtls b
	where b.job_no_mst=a.embellishment_job and a.id=b.mst_id and b.id=c.mst_id and c.job_no_mst=b.job_no_mst and a.entry_form=204  and a.within_group=$cbo_within_group 
	$sql_con $job_cond $wo_cond $year_cond $party_con $buyer_con $location_con $party_location_con $po_idsCond
	$po_con $po_id_con $style_con $company_name and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1
	and a.is_deleted =0 group by a.embellishment_job,b.buyer_style_ref,b.buyer_po_no, b.buyer_buyer, b.buyer_po_id, b.id,b.order_no, a.job_no_prefix_num,a.delivery_date, a.company_id,b.body_part,c.color_id,b.order_uom";
	//sum(c.qnty*12) as order_qty,
	//sum((c.qnty*b.wastage)+c.qnty*12) as plan_qty
  ...
  where a.id=b.mst_id and c.id=a.order_ids $sql_con and a.entry_form in( 222,223)  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1
  ...
  where a.id=b.mst_id and c.id=b.order_id $sql_con and b.process_id=13 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 
   group by c.job_no_mst,c.body_part ,c.id,b.color_size_id";
	...
	ob_start();
	?>
    <div style="width:3320px"> 
        <table width="100%" cellspacing="0" >
            <tr style="border:none;">
                <td colspan="38" align="center" style="border:none; font-size:14px;">
                    <b><? echo $company_library[str_replace("'","",$cbo_company_id)]; ?></b>
                </td>
            </tr>
        </table>
        
        <div style="float:left; width:3320px">
            <table width="3300" cellspacing="0" border="1" class="rpt_table" rules="all" id="table_header_1" >
                <thead>
                    <tr>
                        <th rowspan="2" width="35">SL</th>
                        <th rowspan="2" width="100">Emb. Job</th>
                        <th rowspan="2" width="100">Work Order</th>
                        <th rowspan="2" width="100">IR/IB No.</th>
            ...
            <div style="max-height:350px; width:3320px; overflow-y:auto;" id="scroll_body" >
                <table cellspacing="0" border="1" class="rpt_table"  width="3300" rules="all" id="table_body" >
				<?  
					$i=1;
					...
					foreach($sql_result as $row)
					{
						...
						if($row[csf('order_uom')]==1)
						{
							 ...
							 $plan_qty=($row[csf('order_qty')]+(($row[csf('plan_qty')]*$row[csf('order_qty')])/100));
						}
						if($row[csf('order_uom')]==2)
						{ 
							$order_qty=$row[csf('order_qty')]*12;
							$plan_qty=($row[csf('order_qty')]+(($row[csf('plan_qty')]*$row[csf('order_qty')])/100))*12;
						}
						...
						<tbody>
                          <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                            ...
                            <td width="100"><p><? echo $row[csf('order_no')];?></p></td>
                             <td width="100"><p><? echo $buyer_po_arr[$row[csf('buyer_po_id')]]['grouping'];?></p></td>
                            <td width="100"><p style="width:95px; word-break:break-all"><? 
							...
							if(str_replace("'","",$cbo_within_group)==2)
							{
							 ...
							 ?></p></td>
                            <td width="130" title="<? echo $buyer_po_arr[$row[csf('buyer_po_id')]]['grouping']; ?>"><p style="width:125px; word-break:break-all"><? echo $row[csf('po_number')]; ?></p></td>
                ...
                <table width="3300" border="1" class="tbl_bottom" rules="all" id="report_table_footer" >
                    <tfoot>
                    <tr> 
                        <td width="35"></td>
                        <td width="100"></td>
...
/*	$sql="select
  ...
  from subcon_ord_mst a, subcon_ord_breakdown c ,subcon_ord_dtls b
	...
	where b.job_no_mst=a.embellishment_job and a.id=b.mst_id and b.id=c.mst_id and c.job_no_mst=b.job_no_mst and a.entry_form=204 and a.within_group=$cbo_within_group 
	$sql_con $job_cond $wo_cond $year_cond $party_con $buyer_con $location_con $party_location_con $po_idsCond and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1
	$po_con $po_id_con $style_con and c.qnty<>0
...
if($action=="report_generate_bundle_wise")
{  
	$process = array( &$_POST );
	extract(check_magic_quote_gpc( $process ));
	$txt_wo_order_no=str_replace("'","",trim($txt_wo_order_no));
...
/*	$sql="select
  ...
  from subcon_ord_mst a, subcon_ord_breakdown c ,subcon_ord_dtls b
	...
	//PRInting receive .....................................................................
	$sql_receive_materials="SELECT 
    c.job_no_mst as embl_job_no,c.body_part,c.id as po_id,d.color_number_id,a.entry_form,
    sum(case when a.RECV_DATE = $txt_date_from then b.quantity else 0 end) as today_rec_qty,
    sum(case when a.RECV_DATE < $txt_date_from then b.quantity else 0 end) as prev_rec_qty,
    sum(b.quantity) as total_rec_qty
    from printing_bundle_receive_mst a,printing_bundle_receive_dtls b,subcon_ord_dtls c, wo_po_color_size_breakdown d
	
	where a.id=b.mst_id and c.id=b.job_dtls_id and b.color_size_id=d.id
	$sql_con and a.entry_form in( 614) and a.is_deleted=0 and a.status_active=1  and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 
    group by c.job_no_mst,c.body_part,c.id,d.color_number_id,a.entry_form"; 
	//echo $sql_receive_materials; exit();
	$sql_receive_materials_result=sql_select($sql_receive_materials);
	$materialReceiveDataArr=array();
	foreach($sql_receive_materials_result as $row)
	{
		$key=$row[csf('embl_job_no')].$row[csf('po_id')].$row[csf('body_part')].$row[csf('color_number_id')]; //
		//echo $row[csf('embl_job_no')]."#".$row[csf('po_id')]."#".$row[csf('body_part')]."#".$row[csf('color_size_id')]."<br>";
		$materialReceiveDataArr['today'][$key]=$row[csf('today_rec_qty')];
		$materialReceiveDataArr['prev'][$key]=$row[csf('prev_rec_qty')];
		$materialReceiveDataArr['total'][$key]=$row[csf('total_rec_qty')];
	...
	$sql_issue_materials="SELECT 
    c.job_no_mst as embl_job_no,c.body_part,c.id as po_id,d.color_number_id,a.entry_form,
    sum(case when a.ISSUE_DATE = $txt_date_from then b.quantity else 0 end) as today_rec_qty,
    sum(case when a.ISSUE_DATE < $txt_date_from then b.quantity else 0 end) as prev_rec_qty,
    sum(b.quantity) as total_rec_qty
    from printing_bundle_issue_mst a,printing_bundle_issue_dtls b,subcon_ord_dtls c, wo_po_color_size_breakdown d, printing_bundle_receive_dtls e
	
	where a.id=b.mst_id and c.id=b.WO_dtls_id AND E.COLOR_SIZE_ID=D.ID and b.rcv_dtls_id=e.id
	$sql_con and a.entry_form in( 495) and a.is_deleted=0 and a.status_active=1  and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 
    group by c.job_no_mst,c.body_part,c.id,d.color_number_id,a.entry_form"; 
	//echo $sql_issue_materials; exit();
	$sql_issue_materials_result=sql_select($sql_issue_materials);
	$materialIssueDataArr=array();
	foreach($sql_issue_materials_result as $row)
	{
		$key=$row[csf('embl_job_no')].$row[csf('po_id')].$row[csf('body_part')].$row[csf('color_number_id')]; //
		//echo $row[csf('embl_job_no')]."#".$row[csf('po_id')]."#".$row[csf('body_part')]."#".$row[csf('color_size_id')]."<br>";
		$materialIssueDataArr['today'][$key]=$row[csf('today_rec_qty')];
	...
	$sql_con .=")"; */
	$sql_production="SELECT c.job_no_mst as job_no,c.body_part,c.id as po_id,d.color_number_id,a.entry_form,
	sum(case when a.issue_date = $txt_date_from then b.QUANTITY else 0 end) as today_rec_qty,
	sum(case when a.issue_date < $txt_date_from then b.QUANTITY else 0 end) as prev_rec_qty, 
	sum(b.QUANTITY) as total_rec_qty
  from printing_bundle_issue_mst a,printing_bundle_issue_dtls b,subcon_ord_dtls c, wo_po_color_size_breakdown d
  where a.id=b.mst_id and b.wo_dtls_id=c.id and b.wo_break_id=d.id
  $sql_con and a.entry_form in( 497, 498, 499)  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1
   group by c.job_no_mst,c.body_part ,c.id,d.color_number_id,a.entry_form";

   //echo $sql_production; exit();
	$sql_production_result=sql_select($sql_production);
	$productionDataArr = array();
	$QcDataArr = array();
	$deliveryDataArr = array();
	foreach($sql_production_result as $row)
	{
		if($row[csf('entry_form')]==497){//497=Production;
			$key=$row[csf('job_no')].$row[csf('po_id')].$row[csf('body_part')].$row[csf('color_number_id')];
			$productionDataArr['today'][$key]=$row[csf('today_rec_qty')];
			$productionDataArr['prev'][$key]=$row[csf('prev_rec_qty')];
			$productionDataArr['total'][$key]=$row[csf('total_rec_qty')];
		}
		if($row[csf('entry_form')]==498){//498=Qc;
			$key=$row[csf('job_no')].$row[csf('po_id')].$row[csf('body_part')].$row[csf('color_number_id')];
			$QcDataArr['today'][$key]=$row[csf('today_rec_qty')];
			$QcDataArr['prev'][$key]=$row[csf('prev_rec_qty')];
			$QcDataArr['total'][$key]=$row[csf('total_rec_qty')];
		}
		if($row[csf('entry_form')]==499){//499=Delivery.;
			$key=$row[csf('job_no')].$row[csf('po_id')].$row[csf('body_part')].$row[csf('color_number_id')];
			$deliveryDataArr['today'][$key]=$row[csf('today_rec_qty')];
			$deliveryDataArr['prev'][$key]=$row[csf('prev_rec_qty')];
			$deliveryDataArr['total'][$key]=$row[csf('total_rec_qty')];
		}
	}
            ...
            <div style="max-height:350px; width:3320px; overflow-y:auto;" id="scroll_body" >
				...
				<?  
					...
					foreach($sql_result as $row)
					{
						...
						foreach(explode(',',$row[csf('color_id')]) as $csi){
							$key=$row[csf('job_no')].$row[csf('po_id')].$row[csf('body_part')].$csi; //.$csi
							//echo $row[csf('job_no')]."#".$row[csf('po_id')]."#".$row[csf('body_part')]."#".$csi."<br>";
							//Receive.............................................
							$toDayMaterRec+=$materialReceiveDataArr['today'][$key];
							$prevMaterRec+=$materialReceiveDataArr['prev'][$key];
							$totalMaterRec+=$materialReceiveDataArr['total'][$key];
