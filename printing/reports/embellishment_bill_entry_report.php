...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Printing Bill Entry Report","../../", 1, 1, $unicode,1,1); 
?>	
<script>
...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../../logout.php";
	...
	function generate_report(report_type)
	{
		...
		//alert(data);
		freeze_window(3);
		http.open("POST","requires/embellishment_bill_entry_report_controller.php",true);
	...
	function print_formet(){
		...
		var response = return_global_ajax_value(data, "embl_bill_issue_print", "", "requires/embellishment_bill_entry_report_controller");
        var w = window.open("Surprise", "#");
        var d = w.document.open();
        d.write(response);
        d.close();
                        ...
                        <table class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
                            ...
                            <tbody>
                            	<tr>
                                    <td>
										 <? 
                                       		 echo create_drop_down( "cbo_company_id",120,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", $selected, "load_drop_down( 'requires/embellishment_bill_entry_report_controller', this.value, 'load_drop_down_location', 'location_td');","","","","","",2);  ?>                            
                                    </td>
                                    <td id="location_td">
		                                <?php 
                                    ...
                                    <td>
										<? 
                                       		 echo create_drop_down( "cbo_customer_source",100,$yes_no,"", 1, "--All--",0, "load_drop_down( 'requires/embellishment_bill_entry_report_controller', (document.getElementById('cbo_company_id').value+'_'+this.value), 'load_drop_down_buyer', 'buyer_td' );","","","","","",2);
                                        ?>
                                    </td>
