...
<script>
	...
	function fn_report_generated(type)
	{
		...
		if(type==1){
			var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_within_group*cbo_party_id*cbo_party_location_id*cbo_year*txt_job_no*txt_wo_order_no*cbo_buyer_id*txt_buyer_po*txt_buyer_po_id*txt_style_ref*txt_date_from*cbo_shiping_status*txt_int_ref',"../../")+'&report_title='+report_title;
		}
		else if(type==2){
			var data="action=report_generate_group_by_buyer_job"+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_within_group*cbo_party_id*cbo_party_location_id*cbo_year*txt_job_no*txt_wo_order_no*cbo_buyer_id*txt_buyer_po*txt_buyer_po_id*txt_style_ref*txt_date_from*cbo_shiping_status*txt_int_ref',"../../")+'&report_title='+report_title;
		}else if(type==3){
			var data="action=report_generate_bundle_wise"+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_within_group*cbo_party_id*cbo_party_location_id*cbo_year*txt_job_no*txt_wo_order_no*cbo_buyer_id*txt_buyer_po*txt_buyer_po_id*txt_style_ref*txt_date_from*cbo_shiping_status*txt_int_ref',"../../")+'&report_title='+report_title;
		}
		
		
		freeze_window(3);
		http.open("POST","requires/embellishment_production_status_report_controller.php",true);
		...
		http.onreadystatechange = function fn_report_generated_reponse(){
			...
			{   
				...
				if(type==3){setFilterGrid("table_body",-1, tableFiltersBundle);}
				release_freezing();
			}
	...
	function job_search_popup(page_link,title)
	{
		...
		else
		{
			...
			page_link='requires/embellishment_production_status_report_controller.php?action=job_popup&data='+data
	...
	{ 
		...
		var page_link="requires/embellishment_production_status_report_controller.php?action=order_no_popup&data="+data;
	...
	{ 
		...
		var page_link="requires/embellishment_production_status_report_controller.php?action=style_no_popup&data="+data;
		var title="Style Ref.";
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=800px,height=420px,center=1,resize=0,scrolling=0','../')
...
<form>
    <div style="width:100%;" align="center">    
        <? echo load_freeze_divs ("../../",'');  ?>
         <h3 style="width:1760px; margin-top:20px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3> 
         ...
         <fieldset style="width:1760px;">
            <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr >
                        <td> 
                        <? echo create_drop_down( "cbo_company_id", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/embellishment_production_status_report_controller', this.value, 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/embellishment_production_status_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );"); ?>
                    	</td>
                    	...
                    	<td>
							<?php echo create_drop_down( "cbo_within_group", 80, $yes_no,"", 0, "--  --", 0, "load_drop_down( 'requires/embellishment_production_status_report_controller',document.getElementById('cbo_company_id').value+'_'+this.value, 'load_drop_down_party', 'party_td' );$('#cbo_party_location_id').val(0);$('#cbo_party_location_id').prop('disabled','disabled');if(this.value==2){ $('#txt_buyer_po').prop('disabled','disabled');$('#txt_buyer_po').val('');$('#txt_style_ref').prop('disabled','disabled');$('#txt_style_ref').val('');}else{ $('#txt_buyer_po').prop('disabled','');$('#txt_style_ref').prop('disabled','');}" ); ?>
						</td>
                        <td id="party_td">
                        	<? 
                        		echo create_drop_down( "cbo_party_id", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/embellishment_production_status_report_controller', this.value, 'load_drop_down_party_location', 'party_location_td' );"); 
							?>
                    	</td>
                        ...
                        <td><?php echo create_drop_down( "cbo_shiping_status", 110, $shipment_status,"", 0, "", 2, "","","1,2,3" ); ?></td>
                        <td align="center">
                            <input name="txt_date_from" id="txt_date_from" class="datepicker" style="width:90px" placeholder="Choose Date" value="<? echo date('d-m-Y',time());?>" >
                        </td>
                        <td align="center">
                            <input type="button" id="show_button" class="formbutton" style="width:70px" value="Show" onClick="fn_report_generated(1)" />
							<input type="button" id="show_button2" class="formbutton" style="width:70px" value="Show2" onClick="fn_report_generated(2)" />
							<input type="button" id="show_button3" class="formbutton" style="width:70px" value="Show3" onClick="fn_report_generated(3)" />
