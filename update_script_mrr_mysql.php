...
//and a.transfer_criteria=1
$sql_issue="select b.prod_id, b.id as trans_id, b.cons_quantity, b.transaction_type  from inv_transaction b where b.item_category in(1) and b.company_id=1 and b.transaction_type in(2,3) and b.status_active=1 and b.is_deleted=0
union all
select b.prod_id, b.id as trans_id, b.cons_quantity, b.transaction_type  from inv_transaction b, inv_item_transfer_mst a where a.id=b.mst_id  and a.item_category=1 and b.item_category in(1) and b.company_id=1 and b.transaction_type in(6) and b.status_active=1 and b.is_deleted=0 and a.transfer_criteria=1
 order by trans_id";
...
foreach($result_issue as $row)
{
	...
	$tot_isue_qnty+=$row[csf('cons_quantity')];
}

//and a.transfer_criteria=1

$sql_receive="select b.prod_id, b.id as trans_id, b.cons_quantity, b.cons_rate,b.transaction_type  from inv_transaction b where b.item_category in(1) and b.company_id=1 and b.transaction_type in(1,4) and b.status_active=1 and b.is_deleted=0 
union all
select b.prod_id, b.id as trans_id, b.cons_quantity, b.cons_rate,b.transaction_type  from inv_transaction b, inv_item_transfer_mst a where a.id=b.mst_id  and a.item_category=1 and b.item_category in(1) and b.company_id=1 and b.transaction_type in(5) and b.status_active=1 and b.is_deleted=0  and a.transfer_criteria=1
order by trans_id";
...
foreach($result_receive as $row)
{
	...
	$tot_rcv_qnty+=$row[csf('cons_quantity')];
	
}


echo $tot_rcv_qnty."<br>".$tot_isue_qnty;die;
