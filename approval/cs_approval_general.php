...
<script>
	...
	function fn_report_generated()
	{
		...
		freeze_window(3);
		http.open("POST","requires/cs_approval_general_controller.php",true);
	...
	function fnc_cs_approval_Reply_info()
	{
		...
		{ 
			...
			if((reponse[0]==19 || reponse[0]==20))
			{
				...
				show_list_view(cbo_item_category_id+"**"+cbo_cs_year+"**"+txt_cs_no+"**"+txt_date_from+"**"+txt_date_to+"**"+cbo_approval_type+"**"+txt_alter_user_id+"**"+all_cs_ids,'report_generate_after_approve_unapprove','report_container','requires/cs_approval_general_controller','');
				show_msg(reponse[0]);
			}				
			release_freezing();
		}
	...
	function change_user()
	{
		...
		var page_link = 'requires/cs_approval_general_controller.php?action=user_popup';
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/cs_approval_general_controller',data_arr[0], 'load_drop_down_item_category_new_user', 'item_category_td' );
