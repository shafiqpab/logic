<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Import Document Acceptance Approval
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Import Document Acceptance Approval", "../", 1, 1,'','','');
?>	
<script>
	...
	function fn_report_generated()
	{
		...
		http.open("POST","requires/import_document_acceptance_approval_controller.php",true);
	...
	{ 
		...
		if($('#cbo_approval_type').val()==1)
		{
			var import_payment_data_res=return_global_ajax_value( invoice_ids, 'check_import_payment', '', 'requires/import_document_acceptance_approval_controller');
	...
	function change_user()
	{
		...
		var page_link = 'requires/import_document_acceptance_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			//load_drop_down( 'requires/import_document_acceptance_approval_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
	...
	function downloiadFile(id,company_name)
	{
		...
		var page_link = 'requires/import_document_acceptance_approval_controller.php?action=get_user_pi_file&id='+id+'&company_name='+company_name;
	...
	function check_on_scan_____________(scan_no)
	{
		...
		{
			var last_update=return_global_ajax_value( trim(scan_no), 'check_booking_last_update', '', 'requires/import_document_acceptance_approval_controller');
			if(last_update==2)
			{
	...
	function check_last_update_____________(rowNo)
	{
		...
		{
			var last_update=return_global_ajax_value( trim(booking_id), 'check_booking_last_update', '', 'requires/import_document_acceptance_approval_controller');
			if(last_update==2)
			{
	...
	function fnc_pi_cross_check_____________(pi_id,company_id) {
		...
		var page_link = 'requires/import_document_acceptance_approval_controller.php?item_category=1&importer_id='+company_id+'&pi_id='+pi_id+'&action=cross_check_popup';
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/import_document_acceptance_approval_controller',this.value, 'load_supplier_dropdown', 'supplier_td_id' );" );
                                    ?>
                                </td>
