<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create PI Approval Group by
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("PI Approval Group By", "../", 1, 1,'','','');
?>	
<script>
 	//fnSendMail('../','',1,0,0,1,1,'1_63_1',"call_print_button_for_mail('','','1')");

	if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../logout.php"; 
	var permission='<? echo $permission; ?>';

	let fn_report_generated=(type)=>{
		...
		if($('#cbo_approval_type').val() == 1){
			if (form_validation('cbo_buyer_name','Buyer Name')==false && form_validation('txt_style_ref','Style Ref')==false && form_validation('txt_pi_no','PI No')==false){
				if (form_validation('txt_date_from*txt_date_to','Costing From Date*Costing To Date')==false)
				{
					release_freezing();
					return;
				}
					...
					$("#txt_style_ref").css({ 'background-image': '' });
					$("#txt_pi_no").css({ 'background-image': '' });
				}	
			}
		}

		var data="action=report_generate&type="+type+get_submitted_data_string('cbo_company_name*cbo_supplier_id*txt_date_from*txt_date_to*cbo_approval_type*txt_alter_user_id*txt_pi_no*txt_pi_sys_id_no*cbo_buyer_name*cbo_brand_id*cbo_season_name*cbo_season_year*txt_style_ref',"../");
		//alert(data);
		
		http.open("POST","requires/pi_approval_group_by_controller.php",true);
	...
	let submit_approved=(total_tr,type,permission)=>{ 
		var booking_no_arr = Array();  var booking_id_arr = Array(); var appv_instras="";
		freeze_window(0);
        if (permission ==2) {            
		...
		for(i=1; i<total_tr; i++)
		{
			if ($('#tbl_'+i).is(":checked"))
			{
				...
				if(booking_id!=""){booking_id_arr.push(booking_id);}
				
				booking_no = $('#booking_no_'+i).val();
                if(booking_no!=""){booking_no_arr.push(booking_no);}
		...
		fnSendMail('../','',1,0,0,1,type,$('#cbo_company_name').val()+'_63_1',"call_print_button_for_mail('','',"+type+")");
		//............mail send end;

		var alterUserID = $('#txt_alter_user_id').val();
		var data="action=approve&operation="+operation+'&approval_type='+type+'&booking_nos='+booking_nos+'&booking_ids='+booking_ids+'&txt_alter_user_id='+alterUserID+'&appv_instras='+appv_instras+get_submitted_data_string('cbo_company_name',"../");
	...
	function check_on_scan(scan_no)
	{
		...
		if(!(hide_approval_type==1))
		{
			var last_update=return_global_ajax_value( trim(scan_no), 'check_booking_last_update', '', 'requires/pi_approval_group_by_controller');
			if(last_update==2)
			{
	...
	function check_last_update(rowNo)
	{
		...
		{
			var last_update=return_global_ajax_value( trim(booking_id), 'check_booking_last_update', '', 'requires/pi_approval_group_by_controller');
			if(last_update==2)
			{
				alert("Booking ("+trim(booking_no)+") Info not synchronized with order entry and pre-costing. Contact To "+dealing_merchant+".");
	...
	function change_user()
	{
		...
		var page_link = 'requires/pi_approval_group_by_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			//load_drop_down( 'requires/pi_approval_group_by_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
	...
	function downloiadFile(id,company_name)
	{
		...
		var page_link = 'requires/pi_approval_group_by_controller.php?action=get_user_pi_file&id='+id+'&company_name='+company_name;
	...
	function fnc_pi_cross_check(pi_id,company_id) {
		...
		var page_link = 'requires/pi_approval_group_by_controller.php?item_category=1&importer_id='+company_id+'&pi_id='+pi_id+'&action=cross_check_popup';
	...
	function fnc_file_popup(pi_id,company_id) {
		...
		var page_link = 'requires/pi_approval_group_by_controller.php?company_id='+company_id+'&pi_id='+pi_id+'&action=file_no_popup';
	...
	function openmypage_app_cause(wo_id,app_type,i,app_from,user_id)
	{
		...
		var page_link = 'requires/pi_approval_group_by_controller.php?data='+data+'&action=appcause_popup';
	...
	function openPopup(pi_id)
	{
		...
		var page_link = 'requires/pi_approval_group_by_controller.php?action=all_job_by_pi_popup&pi_id='+pi_id+'&company_name='+$('#cbo_company_name').val();
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=500px,height=250px,center=1,resize=1,scrolling=0','');
		emailwindow.onclose=function()
		{
		}
	...
	function call_print_button_for_mail(mail,mail_body,type){
		...
		var mail=mail.split(',');
		var ret_data=return_global_ajax_value(sysIdArr.join('*')+'__'+mail.join('*')+'__'+type+'__'+$('#txt_alter_user_id').val(), 'pi_approval_mail', '', 'requires/pi_approval_group_by_controller');
	}


	function fn_resend_mail(mst_id){
		$('#txt_selected_id').val(mst_id);
		fnSendMail('../','',1,0,0,1,1,$('#cbo_company_name').val()+'_63_1',"call_print_button_for_mail('','',1)");	
	}
	

	function generate_report(job_no)
	{
		var data=return_global_ajax_value( job_no, 'get_print_button_data', '', 'requires/pi_approval_group_by_controller');
	...
	function print_button_setting(company)
	{
		...
		// alert(company);
		get_php_form_data($('#cbo_company_name').val(),'print_button_variable_setting','requires/pi_approval_group_by_controller' );
	}
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        <thead>
                        	<tr> 
								...
								?> 
                                Alter User:
                                <input type="text" id="txt_alter_user" name="txt_alter_user" class="text_boxes"  onDblClick="change_user()" placeholder="Browse " readonly style="width:200px;">
                                <?php 
								}
								?>
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 100, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/pi_approval_group_by_controller',this.value, 'load_supplier_dropdown', 'supplier_td_id' );load_drop_down( 'requires/pi_approval_group_by_controller',this.value, 'load_dropdown_buyer', 'buyer_td' );print_button_setting();" );
                                    ?>
                                </td>
                                ...
                                <td> 
					         	...
					         	<td align="center">
                                    <input type="button" value="Show" name="show" id="show" class="formbutton" style="width:100px" onClick="fn_report_generated(1)"/>   
                                </td>
                            </tr>
							<tr>
