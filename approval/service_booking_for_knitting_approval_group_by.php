...
$approval_setup=is_duplicate_field( "entry_form", "electronic_approval_setup", "entry_form=29 and is_deleted=0" );
//------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Service Booking For Knitting Approval group By", "../", 1, 1,'','','');
...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate&previous_approved="+previous_approved+get_submitted_data_string('cbo_company_name*cbo_approval_type*txt_alter_user_id*cbo_buyer_name*txt_booking_no*txt_date',"../");

		// alert(data);return;
		http.open("POST","requires/service_booking_for_knitting_approval_group_by_controller.php",true);
	...
	function change_user()
	{
		...
		var page_link = 'requires/service_booking_for_knitting_approval_group_by_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
...
<body>
	...
	<? echo load_freeze_divs ("../",''); ?>
         ...
         <h3 style="width:990px;margin-top:10px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3>
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td>
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0  $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/service_booking_for_knitting_approval_group_by_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );" );
                                    ?>
                                </td>
                                ...
                                <td width=""><input type="text" name="txt_booking_no"  id="txt_booking_no"  style="width:100px" class="text_boxes" placeholder="Booking No" />
								</td>
                                <td>
                                    <?
