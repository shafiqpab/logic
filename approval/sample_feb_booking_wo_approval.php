<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Sample Fabric Booking Approval
Functionality	:	
JS Functions	:
...
<script>
	...
	function check_on_scan(scan_no)
	{
		...
		var last_update=return_global_ajax_value( trim(scan_no), 'check_booking_last_update', '', 'requires/sample_feb_booking_wo_approval_controller');
		if(last_update==2)
		{
		...
		if( hide_approval_type==1)
		{
			var salse_order_approved=return_global_ajax_value( trim(scan_no), 'check_sales_order_approved', '', 'requires/sample_feb_booking_wo_approval_controller');
			if(salse_order_approved==1 || salse_order_approved==3)
			{
				alert("Corresponding Sales Order is approved.So Booking Unapproved Not Allow.");
				$('#tbl_'+row_no).attr('checked', false);
	...
	{ 
		...
		if(type==5){
			//alert(booking_ids);release_freezing();	return;
	...
	function generate_worder_report(type,booking_no,company_id,order_id,fabric_nature,fabric_source,job_no,approved,action,i)
	{
	...
	{
		var page_link='requires/sample_feb_booking_wo_approval_controller.php?action='+action+'&job_no='+job_no;
	...
	function change_user()
	{
		...
		var page_link = 'requires/sample_feb_booking_wo_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/sample_feb_booking_wo_approval_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/sample_feb_booking_wo_approval_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );" );
                                    ?>
                                </td>
                                <td id="buyer_td_id"> 
									<?
