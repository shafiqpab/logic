...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Pre Costing Approval WVN V2", "../", 1, 1,'','','');
...
<script>
	...
	function fn_report_generated()
	{
		...
		http.open("POST","requires/pre_costing_approval_wvn_v2_controller.php",true);
	...
	{ 
		var booking_nos = "";  var booking_ids = ""; var approval_ids = ""; var un_app_case_arr = Array();
		freeze_window(0);
		// confirm message   ***************************************************************
		...
		for(i=1; i<total_tr; i++)
		{
			if ($('#tbl_'+i).is(":checked"))
			{
				...
				var ref_cause = $('#txtCause_'+booking_id).val();
				un_app_case_arr.push(ref_cause);
				if(type==5){
		...
		var un_app_cases = un_app_case_arr.join('**');
		...
		var data="action=approve&operation="+operation+'&approval_type='+type+'&booking_nos='+booking_nos+'&booking_ids='+booking_ids+'&approval_ids='+approval_ids+'&un_app_cases='+un_app_cases+get_submitted_data_string('cbo_company_name*txt_alter_user_id',"../");
	   //alert(data);
	...
	function call_print_button_for_mail(mail,mail_body,type){
		...
		var mail=mail.split(',');
			...
			var ret_data=return_global_ajax_value(sysIdArr.join('*')+'__'+mail.join('*')+'__'+txt_alter_user_id+'__'+mail_body+'__'+type, 'app_mail_notification', '', '../approval/requires/pre_costing_approval_wvn_v2_controller');
	...
	{
		var page_link='requires/pre_costing_approval_wvn_v2_controller.php?action='+action+'&id='+id;
	...
	function fnc_load_cm_compulsory(data)
	{
		...
		var cm_compulsory = return_global_ajax_value( data, 'populate_cm_compulsory', '', 'requires/pre_costing_approval_wvn_v2_controller');
	...
	function change_user()
	{
		...
		var page_link = 'requires/pre_costing_approval_wvn_v2_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/pre_costing_approval_wvn_v2_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
             ...
             <fieldset style="width:1300px;">
             	<input type="hidden" name="txt_style_ref" id="txt_style_ref">
                 	<table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td><? echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/pre_costing_approval_wvn_v2_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );fnc_load_cm_compulsory(this.value);" ); ?></td>
