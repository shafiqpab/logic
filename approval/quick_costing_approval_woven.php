<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Quick Costing Approval WOVEN
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Quick Costing Approval Woven", "../", 1, 1,'','','');
$approval_setup=is_duplicate_field( "page_id", "electronic_approval_setup", "entry_form=45 and is_deleted=0" );
...
let fn_report_generated = ()=>{
		...
		var data="action=report_generate&previous_approved="+previous_approved+get_submitted_data_string('cbo_company_id*cbo_buyer_name*cbo_brand*cbo_season_id*cbo_season_year*txt_costshit_no*txt_style_ref*cbo_get_upto*txt_date*cbo_approval_type*cbo_year*txt_alter_user_id',"../");
		
		http.open("POST","requires/quick_costing_approval_woven_controller.php",true);
	...
	let submit_approved = (total_tr,type)=>{ 
        ...
        var booking_id_arr = Array();
        var confirm_id_arr = Array();
        var booking_no_arr = Array();
        var approval_id_arr = Array();
        ...
        for(i=1; i<total_tr; i++)
		{
			if ($('#tbl_'+i).is(":checked"))
			{
                ...
                confirm_id_arr.push($('#confirm_id_'+i).val());
                if(approval_id>0){approval_id_arr.push(approval_id);}

			}
		}

        var booking_ids = booking_id_arr.join(',');
        var confirm_ids =  confirm_id_arr.join(',');
        var booking_nos = booking_no_arr.join(',');
        var approval_ids = approval_id_arr.join(',');

		//Mail send.......
		$('#txt_selected_id').val(booking_ids);
		fnSendMail('../','',1,0,0,1,type,$('#cbo_company_id').val()+'_0_1');
		//............mail send end;
		...
		var data="action=approve&operation="+operation+'&approval_type='+type+'&booking_nos='+booking_nos+'&booking_ids='+booking_ids+'&approval_ids='+approval_ids+'&confirm_ids='+confirm_ids+get_submitted_data_string('txt_alter_user_id*cbo_company_id',"../");
	   //alert(data);
	...
	{
		var page_link='requires/quick_costing_approval_woven_controller.php?action='+action+'&id='+id;
	...
	function fnc_load_cm_compulsory(data)
	{
		...
		var cm_compulsory = return_global_ajax_value( data, 'populate_cm_compulsory', '', 'requires/quick_costing_approval_woven_controller');
	...
	function change_user()
	{
		...
		var page_link = 'requires/quick_costing_approval_woven_controller.php?action=user_popup&cbo_company_id='+$('#cbo_company_id').val();
	...
	function fnc_print_report2(qc_no,cost_sheet_no,company_id,revise_no,action)
	{

			//var report_title="Quick Costing V3";
			var data=qc_no+'*'+cost_sheet_no+'*'+company_id+'*'+revise_no;
			window.open("../order/spot_costing/requires/quick_costing_woven_controller.php?data=" + data+'&action='+action, true );
			return;
	...
	function fnc_confirm_style(qc_no,update_id)
	{
		...
		var page_link='requires/quick_costing_approval_woven_controller.php?action=confirmStyle_popup';
	...
	function call_print_button_for_mail(mail,mail_body,type){
		...
		var mail=mail.split(',');
		var ret_data=return_global_ajax_value(sysIdArr.join('***')+'__'+mail.join('*')+'__'+type+'__'+$('#txt_alter_user_id').val(), 'quick_costing_approval_mail', '', 'requires/quick_costing_approval_woven_controller');
	}
             ...
             <fieldset style="width:1050px;">
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td>
                                    <?= create_drop_down( "cbo_company_id", 100, "select id,company_name from lib_company where status_active =1 and is_deleted=0 order by company_name","id,company_name", 1, "--Select--", 0, "load_drop_down('requires/quick_costing_approval_woven_controller', this.value, 'load_drop_down_buyer', 'buyer_td_id');$('#txt_alter_user_id').val('');$('#txt_alter_user').val('');" ); ?>
                                </td>
                                <td id="buyer_td_id"><?= create_drop_down( "cbo_buyer_name", 140, "","", 1, "-- All Buyer --", $selected, "" ); ?></td>
