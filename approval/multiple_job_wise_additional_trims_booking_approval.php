...
//--------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Multiple Job Wise Additional Trims Booking Approval", "../", 1, 1,'','','');
...
<script>
	...
	function fn_report_generated()
	{
		...
		http.open("POST","requires/multiple_job_wise_additional_trims_booking_approval_controller.php",true);
	...
	{ 
		...
		var data="action=approve&operation="+operation+'&approval_type='+type+'&booking_nos='+booking_nos+'&booking_ids='+booking_ids+'&approval_ids='+approval_ids+'&txt_alter_user_id='+$('#txt_alter_user_id').val()+'&appv_instras='+appv_instras+get_submitted_data_string('cbo_company_name',"../");
		//alert(data); return;
		http.open("POST","requires/multiple_job_wise_additional_trims_booking_approval_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
    ...
    function generate_comment_popup(booking_no,company,action)
	{
		...
		var page_link='requires/multiple_job_wise_additional_trims_booking_approval_controller.php?action='+action+'&booking_no='+booking_no+'&company='+company;
    ...
    function change_user()
	{
		...
		var page_link = 'requires/multiple_job_wise_additional_trims_booking_approval_controller.php?action=user_popup&company_id='+$("#cbo_company_name").val();
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/multiple_job_wise_additional_trims_booking_approval_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );" );
                                    ?>
                                </td>
