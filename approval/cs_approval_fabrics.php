...
//----------------------------------------------------------------------------------------------
echo load_html_head_contents("CS Approval Accessories", "../", 1, 1,'','','');
...
<script>
	...
	function fn_report_generated()
	{
		...
		freeze_window(3);
		http.open("POST","requires/cs_approval_fabrics_controller.php",true);
	...
	function submit_approved(total_tr,type)
	{
		...
		var sl_str="";
		var supp_validate_costing_price="";
		...
		$('input[name="tbl[]"]:checked').each(function()
		{
			...
			for(j=1; j<=item_row; j++)
			{
				...
				if (supplier_ids=="") supplier_ids=supplier_id; 
				...
				var costingprice=$('#costingprice_'+i+'_'+j).attr('title');	
				var supplier_number_arr = supplier_id.split(',');
	            ...
	            for (var m=0; m<supp_num; m++)
	            {
					...
					if(supplier_number_arr.indexOf(mm) !== -1){
						if (costingprice < $('#txtApprovedPriceSpplier_'+i+'_'+j+'_'+mm).val()){
							supp_validate_costing_price=1;							
							release_freezing();
							return;
						}
					}
	            ...
	            for (var m=0; m<company_num; m++)
	            {
	                ...
	                //neg_validationCompany += +$('#txtApprovedPriceCompany_'+i+'_'+j+'_' + mm).val();
					if (costingprice < $('#txtApprovedPriceCompany_'+i+'_'+j+'_'+mm).val()){
						supp_validate_costing_price=1;						
						release_freezing();
						return;
					}
	            ...
	            data_all += '&item_description_'+i+'_'+j+ '=' + encodeURIComponent($('#item_description_'+i+'_'+j).attr('title')) + '&detarmination_id_'+i+'_'+j+ '=' + $('#detarmination_id_'+i+'_'+j).val() + '&txtweight_'+i+'_'+j+ '=' + $('#txtweight_'+i+'_'+j).val() + '&uom_'+i+'_'+j+ '=' + $('#uom_'+i+'_'+j).attr('title') + '&txt_supp_id_'+i+'_'+j+ '=' + $('#txt_supp_id_'+i+'_'+j).val() + '&txt_comp_id_'+i+'_'+j+ '=' + $('#txt_comp_id_'+i+'_'+j).val();
		...
		if (supp_validate_costing_price==1){
	...
	function fnc_cs_approval_Reply_info()
	{
		...
		{ 
			...
			if((reponse[0]==19 || reponse[0]==20))
			{
				...
				show_list_view(cbo_item_category_id+"**"+cbo_cs_year+"**"+txt_cs_no+"**"+txt_date_from+"**"+txt_date_to+"**"+cbo_approval_type+"**"+txt_alter_user_id+"**"+all_cs_ids,'report_generate_after_approve_unapprove','report_container','requires/cs_approval_fabrics_controller','');
				show_msg(reponse[0]);
			}				
			release_freezing();	
		}
	...
	function change_user()
	{
		...
		var page_link = 'requires/cs_approval_fabrics_controller.php?action=user_popup';
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                        	<tr class="general">                                
                                <td>
                                    <? 
                                        echo create_drop_down( "cbo_item_category_id", 160, $item_category,"", 1, "-- All Category --", $selected,"",1,3,"","","1,2,4,12,13,14");
                                    ?>
                                </td>
                                <td>
