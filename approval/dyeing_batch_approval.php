<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Dyeing Batch Approval
Functionality	:	
JS Functions	:
Created by		:	Ashraful 
Creation date 	: 	20-4-2015
Updated by 		: 		
Update date		: 		   
QC Performed BY	:		
QC Date			:	
...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate&previous_approved="+previous_approved+get_submitted_data_string('cbo_company_name*cbo_batch_type*cbo_get_upto*txt_date*cbo_approval_type*txt_alter_user_id',"../");
		// alert(data);
		
		http.open("POST","requires/dyeing_batch_approval_controller.php",true);
	...
	function check_last_update(rowNo)
	{
		...
		{
			var last_update=return_global_ajax_value( trim(booking_no), 'check_booking_last_update', '', 'requires/dyeing_batch_approval_controller');
			if(last_update==2)
			{
	...
	{ 
		...
		if($('#cbo_approval_type').val()==1)
		{
			...
			{
				first_confirmation=confirm("Are You Want to UnApproved All Batch No");
				...
				{
					second_confirmation=confirm("Are You Sure Want to UnApproved All Batch No");
		...
		else if($('#cbo_approval_type').val()==0)
		{
			if($("#all_check").is(":checked"))
			{
				first_confirmation=confirm("Are You Want to Approved All Batch No");
				if(first_confirmation==false)
				{
					release_freezing();
				 	return;	
				...
				{
					second_confirmation=confirm("Are You Sure Want to Approved All Batch No");
					if(second_confirmation==false)
					{
						release_freezing();
						return;					
		...
		for(i=1; i<total_tr; i++)
		{
			if ($('#tbl_'+i).is(":checked"))
			{
				...
				entry_form_no = $('#txt_entry_form_'+i).val();
				if(entry_form_nos=="") entry_form_nos="'"+entry_form_no+"'"; else entry_form_nos +=",'"+entry_form_no+"'";
		...
		if(booking_nos=="")
		{
			alert("Please Select At Least One Batch No");
			release_freezing();
			return;
		}
		var alterUserID = $('#txt_alter_user_id').val();
		var data="action=approve&operation="+operation+'&approval_type='+type+'&booking_nos='+booking_nos+'&entry_form_nos='+entry_form_nos
		+'&booking_ids='+booking_ids+'&txt_alter_user_id='+alterUserID+'&approval_ids='+approval_ids+'&appv_instras='+appv_instras+get_submitted_data_string('cbo_company_name',"../");
	...
	function generate_worder_report(type,batch_update_id,company_id,order_id,serial_no,fabric_source,job_no,approved,action,i)
	{		
		var data = 	"action="+action+'&data='+company_id+'*'+batch_update_id+'*'+serial_no+'*'+order_id+'*'+fabric_source+'*'+approved+'*'+job_no;
		// alert(data);
					
		http.open("POST","../production/requires/batch_creation_controller.php",true);				
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
	...
	{
		var page_link='requires/dyeing_batch_approval_controller.php?action='+action+'&job_no='+job_no;
	...
	function change_user()
	{
		...
		var title = 'Dyeing Batch Approval';	
		var page_link = 'requires/dyeing_batch_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			// load_drop_down( 'requires/dyeing_batch_approval_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                ...
                                <td id="buyer_td_id"> 
										<?
											echo create_drop_down( "cbo_batch_type", 150, $order_source,"",1, "--Select--", 1,0,0 );
										?>
                                </td>
                                <td> 
									<?
