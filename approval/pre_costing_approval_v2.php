...
/*-------------------------------------------- Comments
Purpose			: 	This form will create Pre Costing Approval V2
Functionality	:	
JS Functions	:
Created by		:	Kausar
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Pre Costing Approval V2", "../", 1, 1,'','','');
...
<script language="Javascript" type="text/javascript">
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate&previous_approved="+previous_approved+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_internal_ref*txt_file_no*cbo_get_upto*txt_date*cbo_approval_type*txt_job_no*cbo_year*txt_alter_user_id',"../");
		
		http.open("POST","requires/pre_costing_approval_v2_controller.php",true);
	...
	function fnc_purchase_requisition_approval_Reply_info()
	{
		...
		{ 
			...
			{
				release_freezing();	
				alert("Ready to approved No is not allowed");
			}	
			$('#txt_bar_code').val('');
			$('#txt_bar_code').focus();
			release_freezing();	
	...
	{
		var page_link='requires/pre_costing_approval_v2_controller.php?action='+action+'&id='+id;
	...
	function fnc_load_cm_compulsory(data)
	{
		...
		var cm_compulsory = return_global_ajax_value( data, 'populate_cm_compulsory', '', 'requires/pre_costing_approval_v2_controller');
	...
	function change_user()
	{
		...
		var page_link = 'requires/pre_costing_approval_v2_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			//load_drop_down( 'requires/pre_costing_approval_v2_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
			load_drop_down( 'requires/pre_costing_approval_v2_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
             ...
             <fieldset style="width:1000px;">
             	<input type="hidden" name="txt_style_ref" id="txt_style_ref">
                 	<table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td><? echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- All --", $selected, "load_drop_down( 'requires/pre_costing_approval_v2_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );fnc_load_cm_compulsory(this.value);" ); ?></td>
