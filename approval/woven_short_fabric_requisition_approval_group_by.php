...
$approval_setup=is_duplicate_field( "entry_form", "electronic_approval_setup", "entry_form=93 and is_deleted=0" );
//------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Woven Short Fabric Req App Group By", "../", 1, 1,'','','');
...
<script>
	...
	function fn_report_generated()
	{
		...
		// alert(data);return;
		http.open("POST","requires/woven_short_fabric_requisition_approval_group_by_controller.php",true);
	...
	function change_user()
	{
		...
		var page_link = 'requires/woven_short_fabric_requisition_approval_group_by_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td>
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0  $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/woven_short_fabric_requisition_approval_group_by_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );" );
                                    ?>
                                </td>
                                ...
                                <td width=""><input type="text" name="txt_req_no"  id="txt_req_no"  style="width:100px" class="text_boxes" placeholder="Req No" />
								</td>
                                <td>
                                    <?
