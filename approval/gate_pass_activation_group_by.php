...
//-------------------------------------------------------------------------------------------
echo load_html_head_contents("Gate Pass Approval Group By", "../", 1, 1,'','','');
...
<script>
	...
	{		
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_get_pass_basis*cbo_year*txt_gate_pass_no*cbo_get_upto*txt_date_from*txt_date_to*cbo_approval_type*txt_alter_user_id',"../");
		// alert(data);return;
		freeze_window(3);
		http.open("POST","requires/gate_pass_activation_group_by_controller.php",true);
	...
	{
		// alert(type);return;
		var gatePass_nos = "";  var gatePass_ids = ""; var approval_ids = "";  var cause_arr = Array();
		freeze_window(0);
		// Confirm Message  ******************************************************
		...
		for(i=1; i<total_tr; i++)
		{
			if ($('#tbl_'+i).is(":checked"))
			{
				...
				cause_arr.push($('#txtCause_'+gate_id).val());
		...
		var causes = cause_arr.join('***');
		var alterUserID = $('#txt_alter_user_id').val();
		var data="action=approve&operation="+operation+'&txt_alter_user_id='+alterUserID+'&approval_type='+type+'&gatePass_ids='+gatePass_ids+'&approval_ids='+approval_ids+'&causes='+causes+get_submitted_data_string('cbo_company_name',"../");
		//alert(data);return;
	...
	function change_user()
	{
		...
		var page_link = 'requires/gate_pass_activation_group_by_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
	...
	{
		var page_link='requires/gate_pass_activation_group_by_controller.php?action='+action+'&id='+id;
                    	...
                    	<tr class="general">
							...
							<td> 
									...
									$get_pass_basis=array(1=>"Independent",2=>"Challan(Yarn)",3=>"Challan(Gray Fabric)",4=>"Challan(Finish Fabric)",5=>"Challan(General Item)",6=>"Challan(Trims)",6=>"Challan(Dyes & Chemical)",7=>"Challan(Trims)",8=>"Challan Subcon(grey fabric)",9=>"Challan Subcon (finish fabric)");
                                     echo create_drop_down( "cbo_get_pass_basis", 150, $get_pass_basis,"", 1, "-- Select Purpose --", $selected, "","","","","","" );
                                    ?>
                                </td>
							<td> <? echo create_drop_down( "cbo_year", 150, $year,"", 1, "-- Select --", 0, "" ); ?></td>
							<td>
