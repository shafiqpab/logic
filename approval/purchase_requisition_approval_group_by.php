<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Purchase Requsition Approval Group By		
Functionality	:				
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Purchase Requsition Approval Group By", "../", 1, 1,'','','');
$permitted_item_category=return_field_value("item_cate_id","user_passwd","id='".$_SESSION['logic_erp']['user_id']."'");
$approval_setup=is_duplicate_field( "page_id", "electronic_approval_setup", "entry_form=1 and is_deleted=0" );
...
<script>
	...
	function fn_report_generated()
	{
		...
		freeze_window(3);
		http.open("POST","requires/purchase_requisition_approval_group_by_controller.php",true);
	 ...
	 if (type == 5)
		{
			...
			{
				first_confirmation=confirm("Are You Want to Deny All");
				...
				{
					second_confirmation=confirm("Are You Sure Want to Deny All");
		...
		else if($('#cbo_approval_type').val()==1)
		{
            if($("#all_check").is(":checked"))
			{
		...
		if(confirm('Mail Send! Sure?')==true){
			...
			return_global_ajax_value(target_ids+'__'+approved_user_id+'__'+company_id+'__'+type, 'send_requisition_app_mail', '', 'requires/purchase_requisition_approval_group_by_controller');
			//return;
		}
	...
	function change_user()
	{
		...
		var page_link = 'requires/purchase_requisition_approval_group_by_controller.php?action=user_popup&company_id='+$("#cbo_company_name").val();
	...
	function openmypage(req_id)
	{
		...
		var page_link = 'requires/purchase_requisition_approval_group_by_controller.php?data='+data+'&action=unapprove_request_action';
                 ...
                 <table class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/purchase_requisition_approval_group_by_controller', this.value, 'load_drop_down_store', 'store_td' )" );
                                    ?>
                                </td>
                                <td id="store_td">
                                    <? 
