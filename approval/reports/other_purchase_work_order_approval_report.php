<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Other Purchase Work Order App Status Report
Functionality	:	
JS Functions	:
...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_supplier_id*cbo_item_cat*cbo_year*txt_wo_no*cbo_type*txt_date_from*txt_date_to',"../../")+'&report_title='+report_title;
		freeze_window(3);
		http.open("POST","requires/other_purchase_work_order_approval_report_controller.php",true);
             ...
             <table width="100%" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr class="general">
                        <td>
                            <?
                                echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/other_purchase_work_order_approval_report_controller',this.value, 'load_drop_supplier', 'supplier_td' );" );
                            ?>
                        </td>
                        <td id="supplier_td">
                            <? 
                        ...
                        <td>
                        	<?
								echo create_drop_down("cbo_item_cat", 150, $item_category, "", 1, "-- All --", "", "", 0);
							?>
                        </td>
                        <td>
                        	<?
								echo create_drop_down("cbo_year", 100, $year, "", 1, "-- All --", "", "", 0);
							?>
                        </td>
                         <td>
