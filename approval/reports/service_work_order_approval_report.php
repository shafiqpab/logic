<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Service Work Order Approval Report.
Functionality	:	
JS Functions	:
Created by		:	Md Rakib
...
echo load_html_head_contents("Service Work Order Approval", "../../", 1, 1, '', 1, 1);
?>
<script>
	...
	function fn_report_generated()
	{
		...
		freeze_window(3);
		http.open("POST","requires/service_work_order_approval_report_controller.php",true);
	...
	function openmypage(type)
	{
		...
		var page_link='requires/service_work_order_approval_report_controller.php?action=search_popup&companyID='+companyID+'&buyerID='+buyerID+'&type='+type;
	...
	{
		var page_link='requires/service_work_order_approval_report_controller.php?action='+action+'&quotation_no='+quotation_no;
             ...
             <table class="rpt_table" width="1010" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr class="general">
                        <td>
                            <?
                                echo create_drop_down( "cbo_company_name", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/service_work_order_approval_report_controller', this.value, 'load_drop_down_supplier', 'supplier_td' );" );
                            ?>
                        </td>
						<td id="supplier_td">
								<?
