...
function fn_report_generated(type)
{
	...
	if(type==1){
		var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_booking_no*hide_booking_id*txt_job_no*hide_job_id*cbo_type*cbo_date_by*txt_date_from*txt_date_to*cbo_booking_type*txt_app_date*txt_internal_ref',"../../")+'&report_title='+report_title;
	}else{
		var data="action=report_generate2"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_booking_no*hide_booking_id*txt_job_no*hide_job_id*cbo_type*cbo_date_by*txt_date_from*txt_date_to*cbo_booking_type*txt_app_date*txt_internal_ref',"../../")+'&report_title='+report_title;
	}
	

	freeze_window(3);
	http.open("POST","requires/fabric_booking_approval_status_report_2_controller.php",true);
...
function openmypage(type)
{
	...
	var page_link='requires/fabric_booking_approval_status_report_2_controller.php?action=search_popup&companyID='+companyID+'&buyerID='+buyerID+'&type='+type;
...
{
	var page_link='requires/fabric_booking_approval_status_report_2_controller.php?action='+action+'&job_no='+job_no;
...
function new_window()
{
	...
	var d = w.document.open();
	d.write ('<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">'+'<html><head><link rel="stylesheet" href="../../css/style_common.css" type="text/css"/><title></title></head><body>'+document.getElementById('report_container2').innerHTML+'</body</html>');
	d.close();
             ...
             <table class="rpt_table" width="1240" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr class="general">
                        <td> 
                            <?
                                echo create_drop_down( "cbo_company_name", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/fabric_booking_approval_status_report_2_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
                        <td id="buyer_td">
                            <? 
                        ...
                        <td>
                            <input type="text" name="txt_internal_ref" id="txt_internal_ref" class="text_boxes" style="width:60px" placeholder="Write">
                        </td>
                        <td>
                        	<?
								$search_by_date=array(0=>"All",1=>"Main",2=>"Short",3=>"Sample",4=>"Partial");
                        ...
                        <td>
                        	<?
								$search_by_arr=array(1=>"Pending",2=>"Approved");
