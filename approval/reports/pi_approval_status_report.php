...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Knitting Status Report", "../../",  1, 1, $unicode,1,'');
...
{	
	...
	var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_supplier_name*txt_pi_no*hide_pi_id*cbo_type*cbo_date_by*txt_date_from*txt_date_to*cbo_item_category_id',"../../")+'&report_title='+report_title+'&type='+type;
	//alert(data);return;
	freeze_window(3);
	http.open("POST","requires/pi_approval_status_report_controller.php",true);
...
function openmypage(type)
{
	...
	var page_link='requires/pi_approval_status_report_controller.php?action=search_popup&companyID='+companyID+'&supplierID='+supplierID+'&type='+type;
	if(type==1) var title='Booking No Search'; else var title='Pro Forma Invoice No Search';
	...
	emailwindow.onclose=function()
	{
		...
		{
			$('#txt_pi_no').val(hide_no);
			$('#hide_pi_id').val(hide_id);	
		}
	}
}
...
{
	var page_link='requires/pi_approval_status_report_controller.php?action='+action+'&job_no='+job_no;
...
function downloiadFile(id)
{
	...
	var page_link = 'requires/pi_approval_status_report_controller.php?action=get_user_pi_file&id='+id; 
             ...
             <table class="rpt_table" width="1000" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr class="general">
                        <td> 
                            <?
                                echo create_drop_down( "cbo_company_name", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/pi_approval_status_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );get_php_form_data(this.value,'print_button_variable_setting','requires/pi_approval_status_report_controller' );" );
                            ?>
                        </td>
                        <td id="buyer_td">
                            <? 
                        ...
                        <td>
                  		<? 
                        	echo create_drop_down( "cbo_item_category_id", 130,$item_category,"", 1, "-- Select --", $selected, "","","","","","");
                        ?>
                        </td>
                        <td>
                        	<?
								$search_by_date=array(1=>"PI Date",2=>"Approved");
                        ...
                        <td>
                            ...
                            To
                            <input type="text" name="txt_date_to" id="txt_date_to" placeholder="To Date" class="datepicker" style="width:55px;" readonly />
                        </td>
                         <td>
                            <input type="text" name="txt_pi_no" id="txt_pi_no" class="text_boxes" style="width:80px" placeholder="Browse" onDblClick="openmypage(3);" readonly>
                            <input type="hidden" name="hide_pi_id" id="hide_pi_id" readonly>
                        </td>
                        <td>
                        	<?
								$search_by_arr=array(0=>"All",1=>"Pending",2=>"Full Approved",3=>"Partial Approved");
								echo create_drop_down( "cbo_type", 80, $search_by_arr,"",0, "", "",'',0 );
							?>
                        </td> 
                        <td>
                            <!-- <input type="button" id="show_button" class="formbutton" style="width:50px" value="Show" onClick="fn_report_generated(1);" />
                            <input type="button" id="show_button2" class="formbutton" style="width:50px" value="Show 2" onClick="fn_report_generated(2);" /> -->
