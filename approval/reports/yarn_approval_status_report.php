<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Yarn Purchase Requisition Approval Status Report.
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Yarn Purchase Requisition Approval Status Report", "../../", 1, 1,'',1,1);
...
function fn_report_generated()
{
	...
	var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_wo_no*hide_req_id*cbo_type*txt_date_from*txt_date_to*txt_app_date',"../../")+'&report_title='+report_title;
	freeze_window(3);
	http.open("POST","requires/yarn_approval_status_report_controller.php",true);
...
function fn_report_generated_reponse()
{
 	if(http.readyState == 4) 
	{
		...
		document.getElementById('report_container').innerHTML='<a href="'+response[1]+'" style="text-decoration:none" ><input type="button" value="Convert To Excel" name="excel" id="excel" class="formbutton" style="width:155px"/></a>&nbsp;&nbsp;&nbsp;<input type="button" onClick="new_window()" value="Print Preview" name="Print" class="formbutton" style="width:100px"/>';
...
function openmypage(type)
{
	...
	var page_link='requires/yarn_approval_status_report_controller.php?action=req_popup&companyID='+companyID+'&buyerID='+buyerID+'&type='+type;
	...
	emailwindow.onclose=function()
	{
		...
		$('#txt_wo_no').val(hide_no);
		$('#hide_req_id').val(hide_id);	
	}
}

function openImgFile(job_no,action)
{
	var page_link='requires/yarn_approval_status_report_controller.php?action='+action+'&job_no='+job_no;
...
<form id="approvalStatusReport_1">
    <div style="width:100%;" align="center">    
        <? echo load_freeze_divs ("../../",''); ?>
         <h3 style="width:770px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3> 
         ...
         <fieldset style="width:770px;">
             <table class="rpt_table" width="770" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
             	<thead>
                    ...
                    <th width="130" colspan="2">Yarn Wo Date Range</th>
                ...
                <tbody>
                    <tr class="general">
                        <td><? echo create_drop_down( "cbo_company_name", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/yarn_approval_status_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?></td>
                        ...
                        <td><input type="text" name="txt_date_to" id="txt_date_to" placeholder="To Date" class="datepicker" style="width:60px;" /></td>
                        <td><input type="text" id="txt_app_date" name="txt_app_date" style="width:70px;" class="datepicker"></td>
                        <td>
						    <?
