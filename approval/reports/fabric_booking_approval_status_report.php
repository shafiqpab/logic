...
function generate_mkt_report(job_no,booking_no,order_id,fab_nature,fab_source,action)
	{     
		var page_link='requires/fabric_booking_approval_status_report_controller.php?action='+action+'&job_no='+job_no+'&booking_no='+booking_no+'&order_id='+order_id+'&fab_nature='+fab_nature+'&job_ids='+fab_source;
...
function fn_report_generated()
{
	...
	var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_booking_no*hide_booking_id*txt_job_no*hide_job_id*cbo_type*cbo_date_by*txt_date_from*txt_date_to*cbo_booking_type*txt_internal_ref*cbo_ascending_by',"../../")+'&report_title='+report_title;
	freeze_window(3);
...
function generate_worder_report3(txt_booking_no,cbo_company_name,txt_order_no_id,cbo_fabric_natu,cbo_fabric_source,txt_job_no,id_approved_id,print_id,entry_form,type,i,fabric_nature)
	{
		...
		else if(print_id==46){var type="show_fabric_booking_report_urmi";}
		...
		else if(print_id==191){var type="show_fabric_booking_report3";}


		if(print_id==46 || print_id==8 || print_id==9 || print_id==10  || print_id==191 )
		{
			var show_yarn_rate='';
			var r=confirm("Press  \"Cancel\"  to hide  Yarn Rate\nPress  \"OK\"  to Show Yarn Rate");
			if (r==true)
		...
		else if(print_id==46 || print_id==8 || print_id==9 || print_id==10 || print_id==191)
		{
			http.open("POST","../../order/woven_order/requires/short_fabric_booking_controller.php",true);
		}
	...
	$(document).ready(function(){
			$('#cbo_ascending_by').prop("disabled",true)
		});
...
<form id="approvalStatusReport_1">
    <div style="width:100%;" align="center">    
        <? echo load_freeze_divs ("../../",''); ?>
         <h3 style="width:1270px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3> 
         <div id="content_search_panel">      
         <fieldset style="width:1270px;">
             <table class="rpt_table" width="1270" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
             	<thead>
                ...
                <tbody>
                    <tr class="general">
                        <td> 
                            <?
                                echo create_drop_down( "cbo_company_name", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/fabric_booking_approval_status_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
                        <td id="buyer_td">
                            <? 
                        ...
                        <td>
                        	<?
								$search_by_date=array(1=>"Booking",2=>"Insert",3=>"Approval");
                        ...
                        <td onchange="assending_func()">
                        	<?
								$search_by_arr=array(0=>"All",1=>"Pending",3=>"Partial Approved",2=>"Full Approved");
