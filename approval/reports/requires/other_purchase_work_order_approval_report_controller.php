...
{ 
	...
	$signatory_sql_res = sql_select("SELECT USER_ID, sequence_no, BYPASS from electronic_approval_setup where company_id=$cbo_company_name and is_deleted=0 and entry_form=17 order by sequence_no");
	...
	$queryApp = "SELECT mst_id, approved_no, approved_by, approved_date, user_ip, entry_form from approval_history where entry_form=17 and un_approved_by=0 ".where_con_using_array($app_mst_id_arr,0,'mst_id')." ".where_con_using_array($userArr,1,'approved_by')." order by id";
	 //echo $queryApp;
                ...
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="<?=$width;?>" class="rpt_table" id="tbl_list_search" align="left">
                    <tbody>
						...
						foreach ($sql_result as $row)
                        {
							...
							$other_purchase_wo_app_arr[$app_type][$row[csf('id')]]['company_name']=$row[csf('id')];
							$other_purchase_wo_app_arr[$app_type][$row[csf('id')]]['wo_number']=$row[csf('wo_number')];
							$other_purchase_wo_app_arr[$app_type][$row[csf('id')]]['quot_date']=$row[csf('quot_date')];
							$other_purchase_wo_app_arr[$app_type][$row[csf('id')]]['supplier_id']=$row[csf('supplier_id')];
							$other_purchase_wo_app_arr[$app_type][$row[csf('id')]]['wo_date']=$row[csf('wo_date')];
							$other_purchase_wo_app_arr[$app_type][$row[csf('id')]]['inserted_by']=$row[csf('inserted_by')];
							$other_purchase_wo_app_arr[$app_type][$row[csf('id')]]['is_approved']=$row[csf('is_approved')];
						}



						 foreach ($other_purchase_wo_app_arr[$type] as $wo_id=>$row)
                        {
						...
						?>
                                ...
                                <tr bgcolor="<?= $bgcolor; ?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor; ?>')" id="tr_<?= $i; ?>">
                                 ...
                                 ?>   
                                    ...
                                    <td width="100" align="center"><?= $approved_ip_array[$wo_id][$signator]; ?></td>
                                    <td width="80" align="center"><?  if($approved_date_array[$wo_id][$signator]){echo date('d-m-y',strtotime($approved_date_array[$wo_id][$signator]));} ?></td>
                                    <td width="60" align="center"><? if($approved_date_array[$wo_id][$signator]){echo date('h-i-s',strtotime($approved_date_array[$wo_id][$signator]));} ?></td>
                                    <td align="center"><?= $approved_no_array[$wo_id][$signator]; ?></td>
                                </tr>
                         <? 
						 $flag=1;}
                         $i++;
