...
{ 
	...
	if($approval_type==2) // Un-Approve/pending
	{   $approved_cond=" and a.APPROVED in(0,2)";  
	  $sql .= "select a.ID,a.APPROVED,a.SYS_NUMBER,a.WO_REASON,a.COMPANY_ID,a.WO_DATE,c.JOB_NO_MST,c.PO_NUMBER,b.JOB_ID,b.PO_ID,b.BUYER_ID,b.AMOUNT from garments_service_wo_mst a,garments_service_wo_dtls b,WO_PO_BREAK_DOWN c where a.id=b.mst_id and b.po_id=c.id and b.job_id=c.job_id and a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0 and a.READY_TO_APPROVE=1  and a.COMPANY_ID=$cbo_company_id $approved_cond  $where_con ";
			
		
	}
	else if($approval_type==1){ //Full_approved)
	...
	else
	{
	 ...
	 $sql_cause="select MST_ID,REFUSING_REASON from refusing_cause_history where entry_form=68 ".where_con_using_array($sys_id_arr,0,'mst_id')." order by id asc";
	//echo $sql_cause;	
    ...
    // echo 12224444423;die();
// 	echo "<pre>";
//    print_r($app_cause_arr); 
//   echo "</pre>";die();
    ob_start();
	$width=950;
    ?> 
        ...
        <div style="width:<? echo $width; ?>px; margin:0 auto;">
            ...
            <table width="<? echo $width; ?>" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="left" >
                <thead>
                    ...
                    <th width="150"> Subcontract Reason</th>
                    ...
                    <?
						...
						?>
                          ...
                          <tr bgcolor="<?= $bgcolor; ?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor; ?>')" id="tr_<?= $i; ?>"> 
                                    ...
                                    <td width="150"><? echo $row[csf('WO_REASON')]; ?></td>                                    
									...
									<td width="100" align="right"><?=$precostArr[$row[csf('JOB_NO_MST')]];?></td>
									 ...
									 <td width="100" align="right"><input type="button" class="image_uploader" style="width:70px" value="view" onClick="openImgFile ('<?=$row[csf('ID')];?>','garments_service_work_order')"></td>

                                    <td width="150"><? if($row[csf('APPROVED')]==1){
									?>
...
if($action=="refusing_cause_popup")
{
	...
	function fnc_cause_info( operation )
	{
		...
		else
		{
			...
			http.open("POST","garments_service_work_order_report_controller.php",true);
...
if($action=='app_mail_notification'){
	//http://localhost/platform-v3.5/approval/requires/garments_service_work_order_report_controller.php?data=20__reza@logicsoftbd.com__3&action=app_mail_notification
