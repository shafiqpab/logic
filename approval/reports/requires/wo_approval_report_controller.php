...
{ 
	...
	$queryApp = "SELECT mst_id, approved_by, approved_no from approval_history where entry_form=5 and un_approved_by=0";
	...
	$buyer_id_arr = return_library_array( "SELECT user_id, buyer_id from electronic_approval_setup where company_id=$cbo_company_name and is_deleted=0 and page_id=627 and bypass=2", "user_id", "buyer_id" );
	$supplier_arr=return_library_array( "select id, supplier_name from lib_supplier", "id", "supplier_name");	
	$signatory_data_arr = sql_select("SELECT user_id, sequence_no, bypass from electronic_approval_setup where company_id=$cbo_company_name and is_deleted=0 and page_id=627 order by sequence_no");
	...
	$query = "SELECT mst_id, approved_no, approved_by, approved_date, user_ip, entry_form from approval_history where entry_form=5 and un_approved_by=0";
                ...
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1000" class="rpt_table" id="tbl_list_search">
                    <tbody>
                        ...
                        foreach ($sql_result as $row)
                        {
							...
							if(((($type==1) || $row[csf('is_approved')]==2 || $row[csf('is_approved')]==0 || $row[csf('is_approved')]==3 || $row[csf('is_approved')]==1) || ($type==2)))
							{
								...
								foreach($signatory_main as $user_id=>$val)
								{
									...
									if($z==0)
									{
										...
										{
											$date_all="Q Date : ".change_date_format($row[csf('quot_date')]);
										}
										...
										<td width="80" rowspan="<? echo $rowspan; ?>"><p><? echo $supplier_arr[$row[csf('supplier_id')]]; ?>&nbsp;</p></td>
										<td width="80" rowspan="<? echo $rowspan; ?>"><p><? echo change_date_format($row[csf('wo_date')]); ?>&nbsp;</p></td>
