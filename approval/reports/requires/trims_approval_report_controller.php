...
{ 
	...
	if($cbo_booking_type==1) 
		...
		// else if($cbo_booking_type==5) //Trims Additional
		// {
		// 	$booking_type_cond="and a.booking_type=8";
		// 	//$type=3;
		// }
	...
	$queryApp="select entry_form, mst_id, approved_no,approved_by from approval_history where entry_form=8 and un_approved_by=0";
	//echo $queryApp;die;
	...
	//cho '<pre>';print_r($approved_no_array);
	
	
	$buyer_id_arr=return_library_array( "select user_id, buyer_id from electronic_approval_setup where company_id=$cbo_company_name and is_deleted=0 and entry_form=8 and bypass=2", "user_id", "buyer_id" );
	...
	/*if($db_type==0)
	{
		...
		$signatory_data_sql=sql_select("select  user_id ,bypass from electronic_approval_setup where company_id=$cbo_company_name and is_deleted=0 and entry_form=8 order by sequence_no");
		foreach($signatory_data_sql as $row)
		{
			$signatory_main[]=$row[csf('user_id')];
			if($row[csf('bypass')]==2)
			{
				$bypass_no_user_id_main[]=$row[csf('user_id')];
			}
		}
	}
	else
	{
		$signatory_data_arr=sql_select("select LISTAGG(case when entry_form=8 then user_id end, ',') WITHIN GROUP (ORDER BY sequence_no) as user_id,  LISTAGG(case when entry_form=8 and bypass=2 then user_id end, ',') WITHIN GROUP (ORDER BY sequence_no) as user_idby from electronic_approval_setup where company_id=$cbo_company_name and is_deleted=0");
		$signatory_main=array_unique(explode(",",$signatory_data_arr[0][csf('user_id')]));
		$bypass_no_user_id_main=$signatory_data_arr[0][csf('user_idby')];
		$bypass_no_user_id_main=array_unique(explode(",",$bypass_no_user_id_main));	
	}*/
	
	$signatory_data_sql=sql_select("select  user_id ,bypass from electronic_approval_setup where company_id=$cbo_company_name and is_deleted=0 and entry_form=8  order by sequence_no");

	// echo "select  user_id ,bypass from electronic_approval_setup where company_id=$cbo_company_name and is_deleted=0 and entry_form=8  order by sequence_no";die;
	
		foreach($signatory_data_sql as $row)
		{  
			$userArr[$row[csf('user_id')]]=$row[csf('user_id')]; 
			$signatory_main[]=$row[csf('user_id')];
			if($row[csf('bypass')]==2)
			{
				$bypass_no_user_id_main[]=$row[csf('user_id')];
	...
	*/
	...
	$query="select entry_form, mst_id, approved_no, approved_by, approved_date, user_ip from approval_history where entry_form =8 ".where_con_using_array($userArr,1,'approved_by')."";
	//echo $query;die;
	...
	// 	  echo "<pre>";
	// 	  print_r($user_approval_array);die;
	//    echo "</pre>";die();
	$print_report_format_ids=return_field_value("format_id","lib_report_template","template_name=".$cbo_company_name."  and module_id=2 and report_id in(26) and is_deleted=0 and status_active=1");
	$format_ids=explode(",",$print_report_format_ids);
		
	$print_report_format_ids2=return_field_value("format_id","lib_report_template","template_name=".$cbo_company_name."  and module_id=2 and report_id in(5) and is_deleted=0 and status_active=1");
                ...
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1622" class="rpt_table" id="tbl_list_search">
                    <tbody>
                        <? 
							...
							if($cbo_type==2){$approved_cond=" and a.is_approved=1";} 
							else if($cbo_type==3){$approved_cond=" and a.is_approved in(2,3)";} 
							else if($cbo_type==1){$approved_cond=" and a.is_approved=0";} 
							// and a.booking_type in(2,5)
							 $sql="select a.id,a.entry_form,b.company_name,a.is_short,a.booking_no, b.buyer_name,a.booking_date, b.job_no, a.is_approved as approved, a.insert_date,a.remarks from  wo_booking_mst a, wo_po_details_master b,wo_booking_dtls c where c.job_no=b.job_no and a.booking_no=c.booking_no and b.company_name=$cbo_company_name and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 and c.booking_type in(2,5) $buyer_id_cond  $job_cond $approved_cond $date_cond  $booking_cond $booking_type_cond group by a.id,a.entry_form,b.company_name, b.buyer_name,a.booking_date,a.is_short,a.booking_no, b.job_no, a.is_approved,a.remarks, a.insert_date order by a.insert_date desc";
							 //echo $sql;die;
                            ...
                            foreach ($nameArray as $row)
                            {
								...
								//$full_approval=false;
								//echo $approvedStatus."==".$full_approval.jahid;die;
								...
								//echo $cbo_type.'='.$full_approval.'='.$row[csf('approved')].'='.$print_cond;die;
								...
								if($cbo_type==2 )
								{
									...
									?>
											...
											<td width="120" rowspan="<? echo $rowspan; ?>">
												<p>
                                                	<?
														...
														if($row[csf('entry_form')]==43) //Country Wise Trims Booking Urmi
														{
															foreach($format_ids2 as $row_id)
															{
																...
																?>
																  ...
																  <a href="##" onClick="generate_trim_booking_report('<? echo $row[csf('booking_no')]; ?>',1,<? echo $cbo_company_name; ?>,'<? echo $row[csf('is_short')]; ?>',<? echo $row[csf('approved')]; ?>,'<? echo $row[csf('entry_form')]; ?>','show_trim_booking_report2')"  ><?php echo $row[csf('booking_no')]; ?></a>
														...
														else if($row[csf('entry_form')]==87)
														{
															foreach($format_ids as $row_id)
															{
																//echo  $row_id;
																if($row_id==67) //Multi Job Wise Trims Booking Urmi
																{ 
																?>
																<!--<a href='#' onClick="generate_trim_booking_report( <?php //echo "'".$row[csf('booking_no')]."','".$report_type."','".$row[csf('company_id')]."','".$row[csf('is_short')]."','".$row[csf('is_approved')]."','".$row[csf('entry_form')]."','show_trim_booking_report2','".$i."'" ; ?>)"><?php //echo $row[csf('booking_no')]; ?><a/>-->
                                                                <a href="##" onClick="generate_trim_booking_report('<? echo $row[csf('booking_no')]; ?>',1,<? echo $cbo_company_name; ?>,'<? echo $row[csf('is_short')]; ?>',<? echo $row[csf('approved')]; ?>,'<? echo $row[csf('entry_form')]; ?>','show_trim_booking_report2')"  ><?php echo $row[csf('booking_no')]; ?></a>
																<?
																}
															}
										...
										$approval_date=$user_approval_array[$row[csf('id')]][$val];
										
										
										$user_ip=$user_ip_array[$row[csf('id')]][$approved_no_array[8][$row[csf('id')]]][$val];
										if($approval_date!="") $approved_no=$approved_no_array[$row[csf('id')]][$val];
										...
										?>
											<td width="140" title="<? echo $row[csf('id')]."==".$approved_no_array[8][$row[csf('id')]]."==".$val;?> "><p><? echo" (".$user_name_array[$val]['name'].")"; ?>&nbsp;</p></td>
								...
								elseif($cbo_type==1 || $cbo_type==3 || $cbo_type==0 )
								{
									if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
									//echo 'nss';die;
									//print_r ($signatory_main);echo 'nss';die;
									$dealing_merchant=$dealing_merchant_array[$job_dealing_merchant_array[$row[csf('job_no')]]];
									$z=0; 
									foreach($signatory_main as $val)
									{
...
{ 
	...
	//$approved_no_array=return_library_array( "select mst_id, max(approved_no) as approved_no from approval_history where entry_form=7 group by mst_id","mst_id","approved_no");
	$approved_no_array=array();
	$queryApp="select entry_form, mst_id, approved_no,approved_by from approval_history where entry_form=67 and un_approved_by=0";
	//echo $queryApp;die;
	$resultApp=sql_select( $queryApp );
	foreach ($resultApp as $row)
	{
	...
	$signatory_data_sql=sql_select("select  user_id ,bypass from electronic_approval_setup where company_id=$cbo_company_name and is_deleted=0 and entry_form=67  order by sequence_no");
	...
	*/
	...
	$user_approval_array=array(); $user_ip_array=array(); $max_approval_date_array=array();
	$query="select entry_form, mst_id, approved_no, approved_by, approved_date, user_ip from approval_history where entry_form=67 ".where_con_using_array($userArr,1,'approved_by')."";
	//echo $query;die;
	$result=sql_select( $query );
	foreach ($result as $row)
	{
                ...
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1622" class="rpt_table" id="tbl_list_search">
                    <tbody>
                        <? 
							...
							// and a.booking_type in(2,5)
							 $sql="select a.id,a.entry_form,b.company_name,a.is_short,a.booking_no, b.buyer_name,a.booking_date, b.job_no, a.is_approved as approved, a.insert_date,a.remarks from  wo_booking_mst a, wo_po_details_master b,wo_booking_dtls c where c.job_no=b.job_no and a.booking_no=c.booking_no and b.company_name=$cbo_company_name and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 and c.booking_type=8 and a.entry_form=555 $buyer_id_cond  $job_cond $approved_cond $date_cond  $booking_cond $booking_type_cond group by a.id,a.entry_form,b.company_name, b.buyer_name,a.booking_date,a.is_short,a.booking_no, b.job_no, a.is_approved,a.remarks, a.insert_date order by a.insert_date desc";
							 //echo $sql;die;
                            ...
                            foreach ($nameArray as $row)
                            {
								...
								{
									if($row[csf('is_short')]==2) $booking_type="Adissional"; else $booking_type="Adissional"; 
									$type=$row[csf('is_short')];
								}
								...
								foreach($bypass_no_user_id_main as $uId)
								{
									...
									if($buyer_ids=="" || in_array($row[csf('buyer_name')],$buyer_ids_array))
									{
										...
										//echo "<br>";
										//echo $row[csf('id')]."=".$approved_no_array[8][$row[csf('id')]]."=".$uId;
										$approvedStatus=$user_approval_array[$row[csf('id')]][$approved_no_array[67][$row[csf('id')]]][$uId];
								...
								if($cbo_type==2)
								{
									...
									?>
										...
										$approved_no=''; $user_ip='';
									    // echo $approved_no_array[67][$row[csf('id')]];
										//echo $row[csf('id')]."==>".$val."<br>";
										$approval_date=$user_approval_array[$row[csf('id')]][$val];
										
										
										$user_ip=$user_ip_array[$row[csf('id')]][$approved_no_array[67][$row[csf('id')]]][$val];
										if($approval_date!="") $approved_no=$approved_no_array[$row[csf('id')]][$val];
										...
										?>
											<td width="140" title="<? echo $row[csf('id')]."==".$approved_no_array[67][$row[csf('id')]]."==".$val;?> "><p><? echo $user_name_array[$val]['full_name']." (".$user_name_array[$val]['name'].")"; ?>&nbsp;</p></td>
                                            <td width="130"><p><? echo $user_name_array[$val]['designation']; ?>&nbsp;</p></td>
