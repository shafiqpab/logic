...
{ 
	...
	if($cbo_sample_type==2){$book_cond=" and a.booking_type=4";}
	if($cbo_sample_type==4){$book_cond=" and a.booking_type=5";}
	...
	$approved_no_array=return_library_array("select mst_id, max(approved_no) as approved_no from approval_history where entry_form=13 group by mst_id","mst_id","approved_no");
	...
	$query="select mst_id, approved_no, approved_by, approved_date, user_ip from approval_history where entry_form=13";
			...
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1122" class="rpt_table" id="tbl_list_search">
				<tbody>
					<? 
						...
						if($db_type==0) 
						{
							$sql="select a.id, a.company_id,a.item_category, a.booking_no, a.booking_no_prefix_num, a.buyer_id, a.booking_date, a.is_approved, a.insert_date, b.style_id from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and a.company_id=$cbo_company_name and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 $buyer_id_cond $booking_cond $approved_cond $date_cond $cbo_sample_type_cond $book_cond group by a.id, a.company_id,a.item_category, a.booking_no, a.booking_no_prefix_num, a.buyer_id, a.booking_date, a.is_approved, a.insert_date order by a.insert_date desc";
						}
						else
						{
							
							$sql="select a.id, a.company_id,a.item_category, a.booking_no, a.booking_no_prefix_num, a.buyer_id, a.booking_date, a.is_approved, a.insert_date, min(b.style_id) as style_id  from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and a.company_id=$cbo_company_name and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 $buyer_id_cond $booking_cond $approved_cond $date_cond $cbo_sample_type_cond $book_cond group by a.id, a.company_id,a.item_category, a.booking_no, a.booking_no_prefix_num, a.buyer_id, a.booking_date, a.is_approved, a.insert_date order by a.insert_date desc";
						}
