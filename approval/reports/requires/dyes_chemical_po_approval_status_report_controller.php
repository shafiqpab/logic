...
{ 
 	...
 	if(str_replace("'","",trim($txt_date_from))!="" && str_replace("'","",trim($txt_date_to))!="")
	{
		...
		{
			$date_cond=" and d.requisition_date between $txt_date_from and $txt_date_to";
		}
		else if(str_replace("'","",trim($cbo_date_by))==2)
		{
			$date_cond=" and c.approved_date between $txt_date_from and $txt_date_to";
		}
		else
		{
	...
	if($txt_requistion_no=="") $ref_cond=""; else $ref_cond=" and d.requ_no in('".implode("','",explode("*",$txt_requistion_no))."')";
	if($txt_po_no=="") $po_cond=""; else $po_cond=" and a.wo_number in('".implode("','",explode("*",$txt_po_no))."')";
	...
	$query="select entry_form, mst_id, approved_no, approved_by, approved_date, user_ip from approval_history where entry_form =3 and un_approved_by=0";
        ...
        <div style="width:1080px; overflow-y:scroll; max-height:310px;" id="scroll_body">
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1040" class="rpt_table" id="tbl_list_search">
				...
				//===================================== report setting==========================================
                    ...
                    $print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$cbo_company_name."' and module_id=19 and report_id in(132) and is_deleted=0 and status_active=1");

					$print_reportSqlRes=explode(",",$print_report_format);

	
						if($print_reportSqlRes[0]==78){ $type=4; }
						else if($print_reportSqlRes[0]==84){ $type=2;}
                        else if($print_reportSqlRes[0]==732){ $type=3; }

					$i=1; 
                        
                    $bypass_no_user_id_main=explode(",",$bypass_no_user_id_main);
                    if($type==2) $approved_cond=" and a.is_approved=1";
                    elseif($type==1) $approved_cond=" and a.is_approved=3";
					...
					$sql="select a.id, a.company_name, a.wo_number_prefix_num, a.wo_number, 
							a.supplier_id, a.wo_date, a.delivery_date, a.inserted_by, c.id as approval_id, c.sequence_no, c.approved_by,c.approved_date,c.approved_no,a.requisition_no
							from wo_non_order_info_mst a, wo_non_order_info_dtls b, approval_history c ,inv_purchase_requisition_mst d
							where a.id = b.mst_id and a.id=c.mst_id and c.entry_form=3 and c.current_approval_status=1 and a.ready_to_approved=1
							and a.entry_form = 145 and a.company_name=$cbo_company_name and a.is_approved in (1,3) and a.requisition_no=d.id  and a.status_active=1 and a.is_deleted=0 and
							b.status_active=1 and b.is_deleted=0 and b.item_category_id in (5,6,7,23) and c.sequence_no='1' $po_cond $ref_cond $date_cond
							group by a.id, a.company_name,a.requisition_no , a.wo_number_prefix_num, a.wo_number, a.pay_mode, a.supplier_id, a.wo_date, a.delivery_date, a.inserted_by, c.id, c.sequence_no, c.approved_by,c.approved_date,c.approved_no order by a.id Desc ";
					...
					foreach ($nameArray as $row)
					{
							...
							// {
								...
								<?
									...
									<td width="110"  style="word-break:break-all"><a href="##" onClick="report_generated('<?=$row[csf('company_name')]; ?>','<?=$row[csf('id')]; ?>','<?=$row[csf('requisition_no')]; ?>','<?=$type; ?>');"><?= $row[csf('wo_number_prefix_num')]?></a></td>
									<td width="110"  style="word-break:break-all"><?= $row[csf('wo_date')];?></td>
									<td width="80" style="word-break:break-all"><?=$supplier_arr[$row[csf('supplier_id')]]; ?>&nbsp;</td>			
								...
								$approved_no=''; $user_ip='';
									...
									<td width="100" align="center" style="word-break:break-all"><?=$date; ?>&nbsp;</td>
									<td width="100" align="center" style="word-break:break-all"><?=$time; ?>&nbsp;</td>
									<td align="center" style="word-break:break-all"><a href="##" onClick="openApproved_no('<?=$row[csf('id')];?>','approve_no_popup');"><?= $approved_no; ?></a></td>
								</tr>
								<?
								$z++;
						//	}
