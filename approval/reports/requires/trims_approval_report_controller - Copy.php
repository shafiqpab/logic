...
{ 
	...
	$queryApp="select entry_form, mst_id, max(approved_no) as approved_no from approval_history where entry_form=15 group by entry_form, mst_id";
	...
	if($db_type==0)
	{
		$signatory_data_arr=sql_select("select group_concat(case when entry_form=15 then user_id end) as user_id, group_concat(case when entry_form=15 and bypass=2 then user_id end) as user_idby from electronic_approval_setup where company_id=$cbo_company_name and is_deleted=0");
	}
	else
	{
		$signatory_data_arr=sql_select("select LISTAGG(case when entry_form=15 then user_id end, ',') WITHIN GROUP (ORDER BY user_id) as user_id,  LISTAGG(case when entry_form=15 and bypass=2 then user_id end, ',') WITHIN GROUP (ORDER BY user_id) as user_idby from electronic_approval_setup where company_id=$cbo_company_name and is_deleted=0");	
	}
                        ...
                        <? 
                            ...
                            foreach ($nameArray as $row)
                            {
								...
								foreach($bypass_no_user_id_main as $uId)
								{
									...
									{
										//$user_approval_array[$row[csf('mst_id')]][$row[csf('approved_no')]][$row[csf('approved_by')]][$row[csf('entry_form')]]=$row[csf('approved_date')];
										$approvedStatus=$user_approval_array[$row[csf('id')]][$approved_no_array[15][$row[csf('id')]]][$uId][15];
								...
								if(((($type==1 && $full_approval==false) || $row[csf('approved')]==2) || ($type==2 && $full_approval==true))  && $print_cond==1)
								{
									...
									?>
										...
										$approval_date=$user_approval_array[$row[csf('id')]][$approved_no_array[15][$row[csf('id')]]][$val][15];
										$user_ip=$user_ip_array[$row[csf('id')]][$approved_no_array[15][$row[csf('id')]]][$val][15];
										if($approval_date!="") $approved_no=$approved_no_array[15][$row[csf('id')]];
