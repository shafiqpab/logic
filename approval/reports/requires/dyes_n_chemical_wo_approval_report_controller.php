...
{ 
	...
	$queryApp = "SELECT mst_id, approved_by, approved_no from approval_history where entry_form=3 and un_approved_by=0";
	...
	$buyer_id_arr = return_library_array( "SELECT user_id, buyer_id from electronic_approval_setup where company_id=$cbo_company_name and is_deleted=0 and entry_form=3 and bypass=2", "user_id", "buyer_id" );
	$supplier_arr=return_library_array( "select id, supplier_name from lib_supplier", "id", "supplier_name");	
	$signatory_data_arr = sql_select("SELECT user_id, sequence_no, bypass from electronic_approval_setup where company_id=$cbo_company_name and is_deleted=0 and entry_form=3 and page_id=626 order by sequence_no");
	...
	$user_approval_array=array(); $user_ip_array=array(); $max_approval_date_array=array();$user_approval_mst_count=array();
	$query = "SELECT mst_id, approved_no, approved_by, approved_date, user_ip, entry_form from approval_history where entry_form=3 ".where_con_using_array($userArr,1,'approved_by')."	and un_approved_by=0";
			...
			<div style="width:1020px; overflow-y:scroll; max-height:310px;" id="scroll_body">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1000" class="rpt_table" id="tbl_list_search">
                    <tbody>
						...
						elseif($type==1) $approved_cond=" nd a.is_approved in (0,3)";
						elseif($type==2) $approved_cond=" and a.is_approved in (1,3)";
						elseif($type==0) $approved_cond="";
						else $approved_cond=" and a.is_approved in (0,2)";
						
						$sql="select a.id, a.company_name, a.wo_number_prefix_num, a.wo_number,a.supplier_id, a.wo_date, a.delivery_date, a.inserted_by,a.is_approved	from wo_non_order_info_mst a, wo_non_order_info_dtls b
						where a.id = b.mst_id   and a.ready_to_approved=1 and a.entry_form = 145 and a.company_name=$cbo_company_name  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.item_category_id in (5,6,7,23) $woYearCond  $date_cond $item_category_cond	$supplier_cond $wo_no_cond 	group by a.id, a.company_name, a.wo_number_prefix_num, a.wo_number, a.pay_mode, a.supplier_id,a.wo_date, a.delivery_date, a.inserted_by,a.is_approved order by a.id Desc ";
						...
						foreach ($sql_result as $row)
                        {
							...
							$dye_chemical_app_arr[$app_type][$row[csf('id')]]['company_name']=$row[csf('id')];
							$dye_chemical_app_arr[$app_type][$row[csf('id')]]['wo_number']=$row[csf('wo_number')];
							$dye_chemical_app_arr[$app_type][$row[csf('id')]]['quot_date']=$row[csf('quot_date')];
							$dye_chemical_app_arr[$app_type][$row[csf('id')]]['supplier_id']=$row[csf('supplier_id')];
							$dye_chemical_app_arr[$app_type][$row[csf('id')]]['wo_date']=$row[csf('wo_date')];
							$dye_chemical_app_arr[$app_type][$row[csf('id')]]['inserted_by']=$row[csf('inserted_by')];
							$dye_chemical_app_arr[$app_type][$row[csf('id')]]['is_approved']=$row[csf('is_approved')];
						}
						// echo "<pre>";
						// print_r($dye_chemical_app_arr[$type]);



					
                        foreach ($dye_chemical_app_arr[$type] as $wo_id=>$row)
                        {
							$full_approval='';
							$rowspan=$rowspanMain;							
							$full_approval=true; $approvedStatus="";
								...
								foreach($signatory_main as $user_id=>$val)
								{
									...
									?>
										...
										<td width="100" align="center" title="<?=$wo_id;?>"><p><? if($row['is_approved']!=0) echo $date; ?>&nbsp;</p></td>
