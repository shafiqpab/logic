...
Updated by 		:   Md.Didarul Alam, Md. Saidul Islam	
Update date		: 		   
QC Performed BY	:		
QC Date			:	
Comments		:
...
exit('This page is vary old. Please use Fabric Booking Approval Group By');
//--------------------------------------------------------------------------------------------------------------------
...
<script>
	...
	function check_on_scan(scan_no)
	{
		...
		else
		{
			if( hide_approval_type==1)
			{
				var salse_order_approved=return_global_ajax_value( trim(scan_no), 'check_sales_order_approved', '', 'requires/fabric_booking_approval_controller');
				if(salse_order_approved==1 || salse_order_approved==3)
				{
					alert("Corresponding Sales Order is approved.So Booking Unapproved Not Allow.");
					$('#tbl_'+row_no).attr('checked', false);
	...
	function fnc_purchase_requisition_approval_Reply_info()
	{
		...
		{ 
			...
			else if((reponse[0]==19 || reponse[0]==20 || reponse[0]==50))
			{
				fnc_remove_tr();
			}
			$('#txt_bar_code').val('');
	...
	function generate_fabric_report_history(txt_booking_no,cbo_company_name,txt_order_no_id,cbo_fabric_natu,cbo_fabric_source,revised_no,txt_job_no) {
            var path = '../';
            print_report(txt_booking_no + '**'+cbo_company_name+ '**'+txt_order_no_id+ '**'+cbo_fabric_natu+ '**'+cbo_fabric_source+ '**'+revised_no+ '**'+txt_job_no+'**' + path , "show_fabric_booking_report_libas", "requires/fabric_booking_approval_controller");
    }
	...
	//a.job_no, a.company_name, a.buyer_name, a.style_ref_no,b.costing_date,b.costing_per
		...
		if(job_data_arr[6]==425){
			var format_ids=return_global_ajax_value( job_data_arr[1]+'**122', 'get_pre_cost_print_button', '', 'requires/fabric_booking_approval_controller');
			...
			var format_ids=return_global_ajax_value( job_data_arr[1]+'**161', 'get_pre_cost_print_button', '', 'requires/fabric_booking_approval_controller');
			if(format_ids==50){action ='preCostRpt';}
			if(format_ids==51){action ='preCostRpt2';}
			if(format_ids==52){action ='bomRpt';}
			if(format_ids==445){action ='preCostRpt8';}
			...
			var format_ids=return_global_ajax_value( job_data_arr[1]+'**43', 'get_pre_cost_print_button', '', 'requires/fabric_booking_approval_controller');
	...
	{
		var page_link='requires/fabric_booking_approval_controller.php?action='+action+'&job_no='+job_no;
	...
	function generate_mkt_report(job_no,booking_no,order_id,fab_nature,fab_source,action)
	{
		...
		var page_link='requires/fabric_booking_approval_controller.php?action='+action+'&company_name='+company_name+'&job_no='+job_no+'&booking_no='+booking_no+'&order_id='+order_id+'&fab_nature='+fab_nature+'&job_ids='+fab_source;
	...
	function call_print_button_for_mail(mail){
		...
		var mail=mail.split(',');
		var action=(booking_id_arr[1]==1)?'unapproved_mail':'deny_mail';
		
		var ret_data=return_global_ajax_value(sysIdArr.join('*')+'__'+mail.join('*'), action, '', '../auto_mail/approval/fabric_booking_approval_auto_mail');
		alert(ret_data);
	}
	...
	function generate_fabric_report(txt_booking_no,cbo_company_name,txt_order_no_id,cbo_fabric_natu,cbo_fabric_source,revised_no,id_approved_id,txt_job_no,entry_form)
	{
		...
		print_report(txt_booking_no + '**'+cbo_company_name+ '**'+txt_order_no_id+ '**'+cbo_fabric_natu+ '**'+cbo_fabric_source+ '**'+revised_no+ '**'+txt_job_no+ '**'+show_yarn_rate+ '**'+report_title+'**' + path+'**'+id_approved_id , action, "requires/fabric_booking_approval_controller");
	}
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        <thead>
                            ...
                            <tr>
                                ...
                                <th>Booking Date</th>
                                <th>Approval Type</th>
                                <th><input type="reset" name="res" id="res" value="Reset" onClick="reset_form('requisitionApproval_1','report_container','','','')" class="formbutton" style="width:140px" /></th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr class="general">
                                <td><?=create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/fabric_booking_approval_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );get_php_form_data(this.value,'report_formate_setting','requires/fabric_booking_approval_controller')" ); ?></td>
