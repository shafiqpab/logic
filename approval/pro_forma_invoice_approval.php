...
Purpose			: 	This form will create PI Invoice Approval
...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_importer_id*cbo_item_category_id*cbo_approval_type',"../");
		
		http.open("POST","requires/pro_forma_invoice_approval_controller.php",true);
	...
	function fn_report_generated_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			var tableFilters = { col_0:"none",col_4:"select", display_all_text: " --- All Category ---" }
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down("cbo_importer_id", 160,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "" );
                                    ?>
                                </td>
                                <td>
                                    <? 
                                        echo create_drop_down( "cbo_item_category_id", 160, $item_category,"", 1, "-- All Category --", $selected, "",0);
                                    ?>
                                </td>
                                <td> 
                                    <?
