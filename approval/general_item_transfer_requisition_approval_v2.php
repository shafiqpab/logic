...
$approval_setup=is_duplicate_field( "entry_form", "electronic_approval_setup", "entry_form=52 and is_deleted=0" );
?>	
...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate&"+get_submitted_data_string('cbo_company_name*cbo_req_year*txt_req_no*txt_date_from*txt_date_to*cbo_approval_type*txt_alter_user_id*cbo_store_id*cbo_transfer_criteria',"../");
		
		http.open("POST","requires/general_item_transfer_requisition_approval_v2_controller.php",true);
	...
	{ 
		...
		var data="action=approve&operation="+operation+'&approval_type='+type+'&req_nos='+req_nos+'&approval_ids='+approval_ids+get_submitted_data_string('cbo_company_name*cbo_store_id*txt_alter_user_id',"../");
	 	 //alert(data);
	...
	function change_user()
	{
		...
		var page_link = 'requires/general_item_transfer_requisition_approval_v2_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			// load_drop_down( 'requires/general_item_transfer_requisition_approval_v2_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
	...
	function openmypage(req_id)
	{
		...
		var page_link = 'requires/general_item_transfer_requisition_approval_v2_controller.php?data='+data+'&action=unapprove_request_action';
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/general_item_transfer_requisition_approval_v2_controller', this.value, 'load_drop_down_store', 'store_td' ); " );
                                    ?>
                                </td>
								<td> 
									<? 
