...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_get_upto*txt_date*cbo_approval_type*txt_alter_user_id',"../");
		
		http.open("POST","requires/fabric_sales_order_approval_controller.php",true);
		...
		{ 
			$("#tbl_list_search tbody").find('tr').each(function()
			{
				try 
				{
					...
					{
						var last_update=return_global_ajax_value( trim(booking_no), 'check_booking_last_update', '', 'requires/fabric_sales_order_approval_controller');
						if(last_update==2)
						{
	...
	function check_on_scan(scan_no)
	{
		...
		if(!(hide_approval_type==1))
		{
			var last_update=return_global_ajax_value( trim(scan_no), 'check_booking_last_update', '', 'requires/fabric_sales_order_approval_controller');
			if(last_update==2)
			{
	...
	{
		var page_link='requires/fabric_sales_order_approval_controller.php?action='+action+'&job_no='+job_no;
	...
	{
		//alert(action);return;
		var page_link='requires/fabric_sales_order_approval_controller.php?action='+action+'&job_no='+job_no+'&booking_no='+booking_no+'&order_id='+order_id+'&fab_nature='+fab_nature+'&fab_source='+fab_source;
	...
	function change_user()
	{
		...
		var page_link = 'requires/fabric_sales_order_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/fabric_sales_order_approval_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/fabric_sales_order_approval_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );" );
                                    ?>
                                </td>
                                <td id="buyer_td_id"> 
									<?
