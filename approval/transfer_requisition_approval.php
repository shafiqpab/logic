...
//-------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Transfer Requisition Approval", "../", 1, 1,'','','');
...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate&previous_approved="+previous_approved+get_submitted_data_string('cbo_company_name*cbo_transfer_criteria*cbo_get_upto*txt_date*cbo_approval_type*txt_requisition_id*txt_bar_code*txt_alter_user_id',"../");
		// alert(data);return;
		
		http.open("POST","requires/transfer_requisition_approval_controller.php",true);
	...
	function submit_approved(total_tr,type)
	{
		...
		var data="action=approve&operation="+operation+'&txt_alter_user_id='+alterUserID+'&approval_type='+type+'&booking_nos='+booking_nos+'&booking_ids='+booking_ids+'&approval_ids='+approval_ids+get_submitted_data_string('cbo_company_name',"../");
		//alert(data);return;
	...
	function generate_yarn_report(company_id,system_num,title,approve,action)
	{
		...
		var data="action="+action+
					'&data='+data_string;
					//alert(data);
			        http.open("POST","../inventory/finish_fabric/requires/grey_transfer_requ_entry_controller.php",true);
					http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
					http.send(data);
	...
	function change_user()
	{
		...
		var page_link = 'requires/transfer_requisition_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
	...
	function print_report(company_name,requisition_no,update_id,action_type)
	{
		var report_title='Roll Wise Grey Fabric Requisition For Transfer';

		var data=company_name+'*'+requisition_no+'*'+update_id+'*'+report_title;
		var action='';
		if(action_type==1) action="grey_issue_print";
		else if(action_type==2) action="grey_issue_print_2";		
		else if(action_type==3) action="grey_issue_print_gropping3";
		else if(action_type==4) action="grey_issue_print_gropping";
		else if(action_type==5) action="grey_issue_print_gropping_2";

		freeze_window(5);

		http.open("POST","../inventory/grey_fabric/requires/roll_wise_grey_fabric_requisition_for_transfer_controller.php",true);
		...
		http.onreadystatechange = function()
		{
			if(http.readyState == 4) 
		    {
				...
				window.open("../inventory/grey_fabric/requires/roll_wise_grey_fabric_requisition_for_transfer_controller.php?data="+data+'&action='+action, "_blank");
				release_freezing();
		   }	
		}
	}
             ...
             <fieldset style="width:900px;">
                 <table class="rpt_table" width="900" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        <thead>
                        	<tr>
                        		<tr>
                        		    <th colspan="2">Scan: <input type="text" id="txt_bar_code" name="txt_bar_code" class="text_boxes"  /></th> 
                        ...
                        <tbody>
                            <tr class="general">
                                ...
                                <td> 
									<? 
										echo create_drop_down("cbo_transfer_criteria", 170,$item_transfer_criteria,"", 1,"-- Select Criteria --",'0',"active_inactive(this.value);",'','1,2,4,8,7,6');
                                    ?>
                                </td>
                                <td> 
									<?
                                ...
                                <td><input type="text" name="txt_requisition_id" id="txt_requisition_id" class="text_boxes_numeric" style="width:80px"/></td>
                                <td> 
                                    <?
