...
if ($action == "sys_id_popup")
{
		...
		<table cellpadding="0" cellspacing="0" border="0" class="rpt_table">
        	...
        	<tr>
            	<td>
                	<? 
						echo create_drop_down( "cbo_company_id", 100, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select--", $cbo_company_id, "load_drop_down( 'shipping_approval_request_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );",0,"" ); 
                   ?>
            	...
            	<td> <input type="text" style="width:100px" class="text_boxes" name="txt_sys_no" id="txt_sys_no" /></td>
                ...
                <td>
                <input type="button" name="button" class="formbutton" value="Show" onClick="if(document.getElementById('txt_sys_no').value=='' && document.getElementById('txt_job_no').value=='' && document.getElementById('txt_order_no').value=='' && (document.getElementById('txt_date_from').value=='' || document.getElementById('txt_date_to').value=='') && document.getElementById('cbo_buyer_name').value==0){alert('Please Select Style or Job or Order or Ship Date');return;};show_list_view (document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('txt_sys_no').value+'**'+document.getElementById('txt_job_no').value+'**'+document.getElementById('txt_order_no').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value+'**'+document.getElementById('cbo_shipment_type').value, 'sys_id_popup_list_view', 'sys_id_popup_list_container', 'shipping_approval_request_controller', 'setFilterGrid(\'tbl_list_search\',-1)');" style="width:80px;" />
                </td>
            </tr>
            <tr>
...
if($action =='sys_id_popup_list_view'){
		...
		list($cbo_company_id,$cbo_buyer_name,$txt_sys_no,$txt_job_no,$txt_order_no,$txt_date_from,$txt_date_to,$cbo_shipment_type)=explode('**',$data);
		...
		if($txt_sys_no!=''){$where_con .=" and c.SHIP_APP_REQ_NO like('%$txt_sys_no')";}
...
if($action =='get_sys_info'){
		$sql="select a.ID,a.COMPANY_ID,a.SHIP_APP_REQ_PREFIX,a.SHIP_APP_REQ_NUM,a.SHIP_APP_REQ_NO,a.PO_BREAK_DOWN_ID,a.SHIPMENT_TYPE,a.SHIP_APP_REQ_DATE,a.TEAM_LEADER_COMMENTS,a.MERCHANT_COMMENTS,a.PLAN_HEAD_COMMENTS,a.APPROVAL_AUTHORITY_COMMENTS,b.PO_NUMBER from wo_po_break_down b,shipping_approval_request a  where b.id=a.PO_BREAK_DOWN_ID and  a.is_deleted=0 and a.status_active=1 and a.id=$data";
		...
		foreach ($data_array as $row)
		{
			...
			echo "document.getElementById('txt_sys_no').value = '".$row[SHIP_APP_REQ_NO]."';\n";
			echo "document.getElementById('cbo_shipment_type').value = '".$row[SHIPMENT_TYPE]."';\n";
			echo "document.getElementById('hidden_order_id').value = '".$row[PO_BREAK_DOWN_ID]."';\n";
			echo "document.getElementById('txt_order_no').value = '".$row[PO_NUMBER]."';\n";
			echo "document.getElementById('txt_date').value = '".change_date_format($row[SHIP_APP_REQ_DATE])."';\n";
			echo "document.getElementById('txt_team_leader_comments').value = '".$row[TEAM_LEADER_COMMENTS]."';\n";
			echo "document.getElementById('txt_merchant_comments').value = '".$row[MERCHANT_COMMENTS]."';\n";
			echo "document.getElementById('txt_plan_head_comments').value = '".$row[PLAN_HEAD_COMMENTS]."';\n";
			echo "document.getElementById('txt_approval_authority_comments').value = '".$row[APPROVAL_AUTHORITY_COMMENTS]."';\n";
...
if ($action == "order_no_popup")
{
		...
		<script>
			function js_set_value(str)
			{
				...
				$("#order_id").val(splitData[1]); 
				$("#order_no").val(splitData[2]); 
				parent.emailwindow.hide();
			}
		</script>
        <form>
		...
		<table cellpadding="0" cellspacing="0" border="0" class="rpt_table">
        	...
        	<tr>
            	<td>
                	<? 
						echo create_drop_down( "cbo_company_id", 120, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select--", $cbo_company_id, "load_drop_down( 'shipping_approval_request_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );",0,"" ); 
                   ?>
            	...
            	<td> <input type="text" style="width:110px" class="text_boxes" name="txt_order_no" id="txt_order_no" /></td>
            	...
            	<td>
                <input type="button" name="button" class="formbutton" value="Show" onClick="if(document.getElementById('txt_style_ref').value=='' && document.getElementById('txt_job_no').value=='' && document.getElementById('txt_order_no').value=='' && (document.getElementById('txt_date_from').value=='' || document.getElementById('txt_date_to').value=='')){alert('Please Select Style or Job or Order or Ship Date');return;};show_list_view (document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('txt_style_ref').value+'**'+document.getElementById('txt_job_no').value+'**'+document.getElementById('txt_order_no').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value, 'order_list_view', 'order_list_container', 'shipping_approval_request_controller', 'setFilterGrid(\'tbl_list_search\',-1)');" style="width:80px;" />
                </td>
            </tr>
            <tr>
            	<td colspan="8" align="center"><?= load_month_buttons(1);?></td>
...
if($action =='order_list_view'){
		...
		list($cbo_company_id,$cbo_buyer_name,$txt_style_ref,$txt_job_no,$txt_order_no,$txt_date_from,$txt_date_to)=explode('**',$data);
...
if ($action == "save_update_delete")
{
	...
	$cbo_shipment_type = str_replace("'", "", $cbo_shipment_type);
	$txt_team_leader_comments = str_replace("'", "", $txt_team_leader_comments);
	$txt_merchant_comments = str_replace("'", "", $txt_merchant_comments);
	$txt_plan_head_comments	= str_replace("'", "", $txt_plan_head_comments);
	$txt_approval_authority_comments = str_replace("'", "", $txt_approval_authority_comments);
	$txt_date = str_replace("'", "", $txt_date);
	$txt_sys_no = str_replace("'", "", $txt_sys_no);
	...
	if ($operation == 0)
	{
		...
		if (str_replace("'", "", $update_id) == "")
		{
			...
			$system_entry_form=0; 
			$new_system_id = explode("*", return_next_id_by_sequence("shipping_approval_request_PK_SEQ", "shipping_approval_request",$con,1,$cbo_company_id,"SAR",$system_entry_form,date("Y",time()),13 ));
			$id = return_next_id_by_sequence("shipping_approval_request_PK_SEQ", "shipping_approval_request", $con);
			
			if (is_duplicate_field("PO_BREAK_DOWN_ID", "shipping_approval_request", "PO_BREAK_DOWN_ID=$hidden_order_id and status_active=1 and is_deleted=0") == 1)
			{
				echo "11**0";
				disconnect($con);
				die;
			...
			$field_array_mst="ID,COMPANY_ID,SHIP_APP_REQ_PREFIX,SHIP_APP_REQ_NUM,SHIP_APP_REQ_NO,PO_BREAK_DOWN_ID,SHIPMENT_TYPE,SHIP_APP_REQ_DATE,TEAM_LEADER_COMMENTS,MERCHANT_COMMENTS,PLAN_HEAD_COMMENTS,APPROVAL_AUTHORITY_COMMENTS,INSERTED_BY,INSERT_DATE,UPDATED_BY,UPDATE_DATE,STATUS_ACTIVE,IS_DELETED";
			
			$data_array_mst = "(".$id.",".$cbo_company_id."','".$new_system_id[1]."','".$new_system_id[2]."','".$new_system_id[0]."','".$hidden_order_id."','".$cbo_shipment_type."','".$txt_date."','".$txt_team_leader_comments."','".$txt_merchant_comments."','" . $txt_plan_head_comments."','".$txt_approval_authority_comments."',".$_SESSION['logic_erp']['user_id'].",'" .$pc_date_time. "',0,'',1,0)";
			
			$flag=1;
			
			$rID=sql_insert("shipping_approval_request",$field_array_mst,$data_array_mst,1);
			if($rID==1 && $flag==1) $flag=1; else $flag=0;
		
		}

		//echo "10**insert into shipping_approval_request ($field_array_mst)" . $data_array_mst;die;
	...
	else if ($operation == 1)//Update
	{
		...
		if ($db_type == 0) {
			...
			if (is_duplicate_field("PO_BREAK_DOWN_ID", "shipping_approval_request", "PO_BREAK_DOWN_ID=$hidden_order_id and status_active=1 and is_deleted=0 and id not in($update_id)") == 1)
			{
				echo "11**0";
				disconnect($con);
				die;
			...
			$field_array_update = "COMPANY_ID*PO_BREAK_DOWN_ID*SHIPMENT_TYPE*SHIP_APP_REQ_DATE*TEAM_LEADER_COMMENTS*MERCHANT_COMMENTS*PLAN_HEAD_COMMENTS*APPROVAL_AUTHORITY_COMMENTS*UPDATED_BY*UPDATE_DATE";
			
			$data_array_update = "'".$cbo_company_id."'*'".$hidden_order_id."'*'".$cbo_shipment_type."'*'".$txt_date."'*'".$txt_team_leader_comments."'*'".$txt_merchant_comments."'*'".$txt_plan_head_comments."'*'".$txt_approval_authority_comments."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
			
			$rID = sql_update("shipping_approval_request", $field_array_update, $data_array_update, "id", $update_id, 1);
	...
	else if ($operation == 2)//Delete..................
	{
		...
		$changeStatus = sql_update("shipping_approval_request", $field_array_status, $data_array_status, "id", $update_id, 1);
...
if($action=="generate_print")
{
	...
	$operation = str_replace("'", "", $operation);
		...
		$shipment_type=array(1=>"Discount Shipment",2=>"Sea-Air Shipment",3=>"Air Shipment");
 		...
 		$sql="select c.COMPANY_ID,c.SHIP_APP_REQ_PREFIX,c.SHIP_APP_REQ_NUM,c.SHIP_APP_REQ_NO,c.PO_BREAK_DOWN_ID,c.SHIPMENT_TYPE,c.SHIP_APP_REQ_DATE,c.TEAM_LEADER_COMMENTS,c.MERCHANT_COMMENTS,c.PLAN_HEAD_COMMENTS,c.APPROVAL_AUTHORITY_COMMENTS,b.PUB_SHIPMENT_DATE,b.SHIPMENT_DATE, b.PO_NUMBER,b.PO_RECEIVED_DATE,b.PO_QUANTITY,a.GMTS_ITEM_ID, a.TOTAL_SET_QNTY, a.JOB_NO, a.BUYER_NAME, a.STYLE_REF_NO from wo_po_details_master a, wo_po_break_down b,shipping_approval_request c  where a.job_no=b.job_no_mst and b.id=c.PO_BREAK_DOWN_ID and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.id=$update_id";
		 //echo $sql;
	...
	$tna_task_id_arr=array(10,31,48,50,270,32,60,61,70,84,73,86,101,110);
	...
	$task_short_name_arr=return_library_array( "select TASK_NAME,TASK_SHORT_NAME from LIB_TNA_TASK where TASK_TYPE=1 and STATUS_ACTIVE=1", "TASK_NAME", "TASK_SHORT_NAME");
	
	
	$pro_sql="select a.PO_BREAK_DOWN_ID,a.PRODUCTION_TYPE,a.EMBEL_NAME,sum(b.production_qnty) as QTY from pro_garments_production_mst a,pro_garments_production_dtls b where a.id=b.mst_id and a.po_break_down_id={$row[PO_BREAK_DOWN_ID]} and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.po_break_down_id,a.production_type,a.EMBEL_NAME";
	  //echo $pro_sql;
    ...
    <table cellspacing="0" cellpadding="0" border="0">
     	<tr>
            <td rowspan="2" width="400"><img  src='../<?=$imge_arr[$cbo_company_id]; ?>' style="height:50px;" /></td>
            <td><strong style="font-size:24px;"><?=$lib_company[$row[COMPANY_ID]]; ?></strong></td>
            <td rowspan="2" width="300"></td>
			
        </tr>
    	<tr><td><strong style="font-size:18px;">Shipping Approval</strong></td></tr>
    </table><br />
    
    
    <table border="1" rules="all">
        <tr>
            <td colspan="2"><strong>Shipment Type : </strong><?=$shipment_type[$row[SHIPMENT_TYPE]]; ?></td>
            <td colspan="2"><strong>Shipping Approval No : </strong><?=$row[SHIP_APP_REQ_NO];?></td>
            <td colspan="2"><strong>Shiping Date : </strong><?=change_date_format($row[SHIPMENT_DATE]); ?></td>
        </tr>
        <tr>
            <td>Company</td>
        ...
        <tr>
            ...
            <td>CM Value</td>
            <td><?=number_format($precost_data_arr[CMValue],2); ?></td>
            <td>Net Profit </td>
            <td><?=number_format($precost_data_arr[TotalMargin],2); ?></td>
        </tr>
        <tr>
            <td>Order Recv. Date</td>
            <td><?=change_date_format($row[PO_RECEIVED_DATE]); ?></td>
            <td>Ship Date</td>
        ...
        <tr>
            <td>Quantity (PCS)</td>
            <td><?= $po_qty = $row[TOTAL_SET_QNTY]*$row[PO_QUANTITY]; ?></td>
            <td>Finish Req. (KG)</td>
            <td><?= number_format($precost_data_arr[FinishFabQty],2); ?></td>
     ...
     <tbody>
         <? 
			...
			if($tna_data_arr[$task_id][ACTUAL_START_DATE]!=''){
				$startDelayEarly=datediff( "d", $tna_data_arr[$task_id][TASK_START_DATE],$tna_data_arr[$task_id][ACTUAL_START_DATE] );
			}
			
			if($tna_data_arr[$task_id][ACTUAL_FINISH_DATE]!=''){
				$finishDelayEarly=datediff( "d", $tna_data_arr[$task_id][TASK_FINISH_DATE], $tna_data_arr[$task_id][ACTUAL_FINISH_DATE] );
			}
 ...
 <table border="1" rules="all">
    ...
    <tr>     
        ...
        <td align="right">
		<? 
			$exc_cut_out=$pro_data_arr[1]-$po_qty; 
			if($exc_cut_out>0){
				echo number_format($exc_cut_out,0,"",",");
				$exc_cut_per=(100/$pro_data_arr[1])*$exc_cut_out;
			}
		?>
        </td> 
        <td align="right"><?=number_format($exc_cut_per,2);?></td> 
    </tr>
    <tr>     
        <td>Embellishment</td>
        ...
        <td align="right">
		<? 
			$exc_emb=$emb_pro_data_arr[3]-$po_qty; 
			if($exc_emb>0){
				echo number_format($exc_emb,0,"",",");
				$exc_emb_per=(100/$emb_pro_data_arr[3])*$exc_emb;
			}
		?>
        </td> 
        <td align="right"><?=number_format($exc_emb_per,2);?></td> 
    </tr>
    <tr>     
        <td>Sewing Input</td>
        ...
        <td align="right">
			<? 
			$exc_sewing_in=$pro_data_arr[4]-$po_qty; 
			if($exc_sewing_in>0){
				echo number_format($exc_sewing_in,0,"",",");
				$exc_sewing_in_per=(100/$pro_data_arr[4])*$exc_sewing_in;
			}
			?>
        </td> 
        <td align="right"><?=number_format($exc_sewing_in_per,2);?></td> 
    </tr>
    <tr>     
        <td>Sewing Output</td>
        ...
        <td align="right">
		<? 
			$exc_sewing_out=$pro_data_arr[5]-$po_qty; 
			if($exc_sewing_out>0){
				echo number_format($exc_sewing_out,0,"",",");
				$exc_sewing_out_per=(100/$pro_data_arr[4])*$exc_sewing_out;
			}
		?>
        </td> 
        <td align="right"><?=number_format($exc_sewing_out_per,2);?></td> 
    </tr>
    <tr>     
        ...
        <td align="right">
		<? 
			$exc_finish=$pro_data_arr[8]-$po_qty; 
			if($exc_finish>0){
				echo number_format($exc_finish,0,"",",");
				$exc_finish_per=(100/$pro_data_arr[8])*$exc_finish;
			}
		?>
        </td> 
        <td align="right"><?=number_format($exc_finish_per,2);?></td> 
    </tr>
        <td>Total Inspection Gmts</td>
        <td align="right"><?=number_format($insfection_qty,0,"",",");?></td> 
        <td align="right">
		<? 
			$exc_inspection = $insfection_qty-$po_qty;
			if($exc_inspection>0){
				echo number_format($exc_inspection,0,"",",");
				$exc_inspection_per=(100/$insfection_qty)*$exc_inspection;
			}
 		?>
        </td> 
        <td align="right"><?=number_format($exc_inspection_per,2);?></td> 
    </tr>
    <tr>     
        ...
        <td align="right">
		<? 
			$tot_finish=$exfac_data_arr[$row[PO_BREAK_DOWN_ID]]-$po_qty; 
			if($tot_finish>0){
				echo number_format($tot_finish,0,"",",");
				$exc_tot_finish_per=(100/$exfac_data_arr[$row[PO_BREAK_DOWN_ID]])*$tot_finish;
			}
		?>
        </td> 
        <td align="right"><?=number_format($exc_tot_finish_per,2);?></td> 
    </tr>
  </table>
  ...
  <table>
      ...
      <tr>
          <td><strong>Merchant Comments</strong></td>
          <td>:</td>
          <td valign="top"><?=$row[MERCHANT_COMMENTS];?></td>
      </tr>
      <tr>
          <td><strong>Team Leader Comments</strong></td>
          <td>:</td>
          <td valign="top"><?=$row[TEAM_LEADER_COMMENTS];?></td>
      </tr>
      
      <tr>
          <td><strong>Plant Head Comments</strong></td>
          <td>:</td>
          <td valign="top"><?=$row[PLAN_HEAD_COMMENTS];?></td>
      </tr>
      
       <tr>
          <td><strong>Approval  Authority</strong></td>
          <td>:</td>
          <td valign="top"><?=$row[APPROVAL_AUTHORITY_COMMENTS];?></td>
      </tr>
     </table> 
...
//echo signature_table($report_id, $company, $width, $template_id="", $padding_top = 70,$prepared_by='',$userSignatureArr=array());
