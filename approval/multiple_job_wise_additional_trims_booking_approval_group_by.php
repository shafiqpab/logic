<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Multiple Job Wise Additional Trims Booking Approval Group By
Functionality	:	
JS Functions	:
Created by		:	
...
//--------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Multiple Job Wise Additional Trims Booking Approval Group By", "../", 1, 1,'','','');
$approval_setup=is_duplicate_field( "page_id", "electronic_approval_setup", "entry_form=67  and is_deleted=0" );
?>	
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate&previous_approved="+previous_approved+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_get_upto*txt_date*txt_booking_no*cbo_approval_type*cbo_booking_year*txt_alter_user_id',"../");
		//alert(data);return;
		http.open("POST","requires/multiple_job_wise_additional_trims_booking_approval_group_by_controller.php",true);
	...
	{ 
		 var booking_no_arr = Array();  var booking_id_arr = Array(); var appv_instra_arr = Array();
		freeze_window(0);
		// Confirm Message....................
		...
		var data="action=approve&operation="+operation+'&approval_type='+type+'&booking_nos='+booking_nos+'&booking_ids='+booking_ids+'&txt_alter_user_id='+$('#txt_alter_user_id').val()+'&appv_instras='+appv_instras+get_submitted_data_string('cbo_company_name',"../");
		//alert(data); return;
		http.open("POST","requires/multiple_job_wise_additional_trims_booking_approval_group_by_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
    ...
    function generate_worder_report11(txt_booking_no,cbo_company_name,id_approved_id,action,i)
	{  
		var data="action="+action+
        '&txt_booking_no='+"'"+txt_booking_no+"'"+
		'&cbo_company_name='+"'"+cbo_company_name+"'"+
		'&id_approved_id='+"'"+id_approved_id+"'"+
		'&report_title='+"Multiple Job Wise Additional Trims Booking "+
		'&path=../';
			
		freeze_window(5);
		http.open("POST","../order/woven_order/requires/multi_job_additional_trims_booking_controller.php ",true);
	...
	{
		var page_link='requires/multiple_job_wise_additional_trims_booking_approval_group_by_controller.php?action='+action+'&booking_no='+booking_no+'&company='+company;
    ...
    function change_user()
	{
		...
		var page_link = 'requires/multiple_job_wise_additional_trims_booking_approval_group_by_controller.php?action=user_popup&company_id='+$("#cbo_company_name").val();
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/multiple_job_wise_additional_trims_booking_approval_group_by_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );" );
                                    ?>
                                </td>
