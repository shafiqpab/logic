...
function getFinalUser($parameterArr=array()){
	...
	//Electronic app setup data.....................
	$sql="SELECT COMPANY_ID,PAGE_ID,USER_ID,BYPASS,SEQUENCE_NO,BUYER_ID,BRAND_ID,BUYER_ID,DEPARTMENT FROM ELECTRONIC_APPROVAL_SETUP WHERE COMPANY_ID = {$parameterArr['company_id']} AND PAGE_ID = {$parameterArr['page_id']} AND IS_DELETED = 0  order by SEQUENCE_NO";
	   //echo $sql;die;
	$sql_result=sql_select($sql);
	foreach($sql_result as $rows){
		if($rows['BUYER_ID']==''){$rows['BUYER_ID']=$lib_buyer_arr;}
...
{  
	...
	if($cbo_buyer_id){$where_con .= " and a.BUYER_ID =".$cbo_buyer_id.""; }
	...
	{  
		...
		$data_mast_sql = "SELECT a.id,a.do_no,a.company_id,a.wo_date,a.delivery_date,a.booking_month
		from dyeing_work_order_mst a , dyeing_work_order_dtls b 
		where a.id=b.mst_id  and b.STATUS_ACTIVE=1 and b.IS_DELETED=0
		and a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and a.APPROVED<>1 and a.READY_APPROVAL=1 and a.COMPANY_ID=$cbo_company_id $where_con";
		...
		for($seq=0;$seq<=count($electronicDataArr['sequ_arr']); $seq++ ){
			...
			if($tmp_sys_id_arr[$seq]){
				...
				$sql .= "SELECT a.ID,a.DO_NO,a.COMPANY_ID,a.BUYER_ID,a.WO_DATE,a.DELIVERY_DATE,a.BOOKING_MONTH
                from DYEING_WORK_ORDER_MST a , DYEING_WORK_ORDER_DTLS b 
                where a.id=b.mst_id  and b.STATUS_ACTIVE=1 and b.IS_DELETED=0
                and a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and a.APPROVED<>1 and a.READY_APPROVAL=1 and a.APPROVED_SEQU_BY=$seq $sys_con";
			}
		}
	}
	else
	{
		...
		$sql = "SELECT a.ID,a.DO_NO,a.COMPANY_ID,a.BUYER_ID,a.WO_DATE,a.DELIVERY_DATE,a.BOOKING_MONTH
        from DYEING_WORK_ORDER_MST a , DYEING_WORK_ORDER_DTLS b,APPROVAL_MST c 
        where a.id=b.mst_id  and b.STATUS_ACTIVE=1 and b.IS_DELETED=0
        and a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and a.APPROVED<>0 and a.READY_APPROVAL=1 and c.SEQUENCE_NO={$electronicDataArr['user_by'][$user_id_approval]['SEQUENCE_NO']}  and a.APPROVED_SEQU_BY=c.SEQUENCE_NO and c.entry_form=74 and c.mst_id=a.id";
	...
	//
	 ...
	 $sql_cause="select MST_ID,REFUSING_REASON from refusing_cause_history where entry_form=74 ".where_con_using_array($sys_id_arr,0,'mst_id')."  and INSERTED_BY=$user_id_approval order by id asc";
	// echo $sql_cause;	
                        ...
                        {                                                   
                            $bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
                                ...
                                <tr bgcolor="<?= $bgcolor; ?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor; ?>')" id="tr_<?= $i; ?>"> 
                                    ...
                                    <td width="150" align="center"><a href="javascript:fn_generate_print(<? echo $row['ID']; ?>,<? echo $row['COMPANY_ID']?>)"><? echo $row[csf('DO_NO')]; ?></a></td>                                    
...
{  
	...
	$sql = "select a.ID  from dyeing_work_order_mst a where a.COMPANY_ID=$cbo_company_id and a.STATUS_ACTIVE=1 and a.IS_DELETED=0  and a.id in($target_ids)";
	...
	$finalDataArr=getFinalUser(array('company_id'=>$cbo_company_id,'page_id'=>$menu_id,'lib_buyer_arr'=>$lib_buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>$item_cat_arr,'lib_store_arr'=>0,'product_dept_arr'=>0,'match_data'=>$matchDataArr));
	...
	if($approval_type==5)
	{
		...
		$rID1=sql_multirow_update("dyeing_work_order_mst","approved*ready_approval*APPROVED_SEQU_BY",'0*0*0',"id",$target_ids,0);
		if($rID1) $flag=1; else $flag=0;

		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0, un_approved_by='".$user_id_approval."', un_approved_date='".$pc_date_time."', updated_by='".$user_id."', update_date='".$pc_date_time."' WHERE entry_form=74 and current_approval_status=1 and mst_id in ($target_ids)";
		...
		if($flag==1) 
		{
			$query="delete from approval_mst  WHERE entry_form=74 and mst_id in ($target_ids)";
	...
	{      
        ...
        {		
			...
			$data_array.="(".$id.",74,".$mst_id.",".$user_sequence_no.",".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			
			$approved_no=$max_approved_no_arr[$mst_id][$user_id_approval]+1;
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",74,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1)";
			$ahid++;
		...
		if($flag==1) 
		{
			...
			$rID2=execute_query(bulk_update_sql_statement( "dyeing_work_order_mst", "id", $field_array_up, $data_array_up, $target_app_id_arr ));
			if($rID2) $flag=1; else $flag=0; 
		}

		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=74 and mst_id in ($target_ids)";
			$rID3=execute_query($query,1);
			if($rID3) $flag=1; else $flag=0;
		}
	...
	{              
		...
		$next_user_app = sql_select("select id from approval_history where mst_id in($target_ids) and entry_form=74 and sequence_no >$user_sequence_no and current_approval_status=1 group by id");
    ...
    <form>
           ...
           <?php
            $custom_designation=return_library_array( "select id,custom_designation from lib_designation ",'id','custom_designation');  
             ...
             $sql="select a.id, a.user_name, a.department_id, a.user_full_name, a.designation,b.SEQUENCE_NO from  user_passwd a,electronic_approval_setup b where a.id=b.user_id and b.company_id=$cbo_company_id and b.entry_form=74 and valid=1 and a.id!=$user_id  and b.is_deleted=0 order by b.SEQUENCE_NO";
                //echo $sql;
