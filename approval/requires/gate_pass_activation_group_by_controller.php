...
if($action=="report_generate")
{
   ...
   //print_r($process);die;
    ...
    $cbo_get_pass_basis=str_replace("'","",$cbo_get_pass_basis);
    ...
    $electronicDataArr=getSequence(array('company_id'=>$cbo_company_name,'page_id'=>$menu_id,'entry_form'=>19,'user_id'=>$app_user_id, 'lib_department_id_arr'=>$department_arr));
    ...
    if($approval_type==0){
        ...
        $data_mas_sql="SELECT a.ID, a.DEPARTMENT_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY from INV_GATE_PASS_MST a where  a.COMPANY_ID=$cbo_company_name and a.is_deleted=0 and a.status_active=1 and a.approved<>1 $where_con";   
		...
		for($group=0;$group<=$my_group; $group++ ){
			foreach($electronicDataArr['group_seq_arr'][$group] as $seq){
				...
				if(count($tmp_sys_id_arr[$group][$seq])){
					...
					$sql .= "select a.id,$year_cond,a.time_hour,a.time_minute,  a.within_group,a.sys_number_prefix_num,a.challan_no, a.sent_to,a.challan_no, a.company_id,a.out_date,'0' as approval_id from  inv_gate_pass_mst a, inv_gate_pass_dtls b where a.id=b.mst_id  and a.company_id=$cbo_company_name  and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0  and a.ready_to_approved=1 and   a.sys_number not in(select gate_pass_id from inv_gate_out_scan where  gate_pass_id is not null and status_active=1 and is_deleted=0)and a.approved<>1 and a.APPROVED_SEQU_BY=$seq and a.APPROVED_GROUP_BY=$group  $sys_con $basis_cond $gate_pass_cond $date_cond $gatepass_year_cond  group by a.id,a.insert_date,a.time_hour,a.time_minute, a.within_group,a.challan_no, a.sent_to,a.sys_number_prefix_num,a.challan_no, a.company_id,a.out_date";
				}
		...
		$sql="select a.id,$year_cond,a.time_hour,a.time_minute,  a.within_group,a.sys_number_prefix_num,a.challan_no, a.sent_to,a.challan_no, a.company_id,a.out_date,'0' as approval_id from  inv_gate_pass_mst a, inv_gate_pass_dtls b, APPROVAL_MST c  where a.id=b.mst_id and a.id=c.mst_id  and a.company_id=$cbo_company_name   and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0  and a.ready_to_approved=1 and   a.sys_number not in(select gate_pass_id from inv_gate_out_scan where  gate_pass_id is not null and status_active=1 and is_deleted=0)and a.approved<>0 and a.APPROVED_GROUP_BY=c.GROUP_NO  and c.GROUP_NO={$electronicDataArr['user_by'][$app_user_id]['GROUP_NO']}   $basis_cond $gate_pass_cond $date_cond $gatepass_year_cond  group by a.id,a.insert_date,a.time_hour,a.time_minute, a.within_group,a.challan_no, a.sent_to,a.sys_number_prefix_num,a.challan_no, a.company_id,a.out_date";
...
<script>
        function open_print_btn_popup(data){
            ...
            var page_link = 'requires/gate_pass_activation_group_by_controller.php?action=print_button_variable&print_data='+data;
        ...
        <legend>Gate Pass List View</legend>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1250" class="rpt_table" align="left" >
                <thead>
                    ...
                    <th width="150">Gate Pass Time</th>
                        ...
                        <?						
                            ...
                            foreach ($nameArray as $row)
                            {
								...
								<tr bgcolor="<?= $bgcolor; ?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor; ?>')" id="tr_<?= $i; ?>" align="center"> 
                                    ...
                                    <td width="120" align="center"> <p><? echo  $row[csf('sys_number_prefix_num')]; ?></p></td>

									<td width="120"><? echo $row[csf('year')]; ?>&nbsp;</p></td>

									<td width="180" align="center"><?=$row['CHALLAN_NO']; ?></td>                             
                                    <td width="150" align="center"><p><?  echo $gate_pass_time;//$row[csf('time_hour')].':'. $row[csf('time_minute')]; ?>&nbsp;</p></td>
                                    <td width="150" align="center"><p><? if($row[csf('out_date')]!="0000-00-00") echo change_date_format($row[csf('out_date')]); ?>&nbsp;</p></td>
                                     <td  width="150"><? echo $yes_no[$row[csf('within_group')]];?></p>&nbsp;</td>	

                               <td width="100" align="center"><p><? echo $sent_to;//$row[csf('sent_to')]; ?>&nbsp;</p></td>
							   <?
									$mst_id = $row[csf('id')];
									...
									<td > <input style="width:100px;" type="text" class="text_boxes" name="txtCause_<? echo $row[csf('id')]; ?>" id="txtCause_<? echo $row[csf('id')]; ?>" placeholder="browse" onClick="openmypage_refusing_cause('requires/gate_pass_activation_group_by_controller.php?action=refusing_cause_popup','Refusing Cause','<? echo $row[csf('id')]; ?>');" value="<? echo $refusing_reason_arr[0][csf('refusing_reason')];?>" /></td>    
...
if ($action == "refusing_cause_popup") {
	...
	$sql_cause = "select refusing_reason from refusing_cause_history where entry_form=19 and mst_id='$quo_id'";
 ...
 ?>
	<script>
		...
		function fnc_cause_info(operation) {
			...
			if (form_validation('txt_refusing_cause', 'Refusing Cause') == false) {
				...
				http.open("POST", "gate_pass_activation_group_by_controller.php", true);
	...
	<body onload="set_hotkey();">
		<div align="center" style="width:100%;">
			...
			<?
			$sqlHis = "select approval_cause from approval_cause_refusing_his where entry_form=19 and approval_type=1 and booking_id='$quo_id' and status_active=1 and is_deleted=0 order by id Desc";
...
if($action=="save_update_delete_refusing_cause")
{
	...
	$flag=1;
	if(is_duplicate_field( "approval_cause", "approval_cause_refusing_his", "approval_cause='".$refusing_cause."' and entry_form=19 and booking_id='".str_replace("'", "", $quo_id)."' and approval_type=1 and status_active=1 and is_deleted=0" )==1)
	{
		//
	}
	else
	{
		...
		$idpre=return_field_value("max(id) as id", "refusing_cause_history", "mst_id=".$quo_id." and entry_form=19 group by mst_id","id");
		$sqlHis="insert into approval_cause_refusing_his( id, cause_id, entry_form, booking_id, approval_type, approval_cause, inserted_by, insert_date, updated_by, update_date)
				select '', id, entry_form, mst_id, 1, refusing_reason, inserted_by, insert_date, updated_by, update_date from refusing_cause_history where mst_id=".$quo_id." and entry_form=19 and id=$idpre"; //die;
	...
	if ($operation==0)  // Insert Here
	{
		...
		$data_array = "(".$id.",19,".$quo_id.",'".$refusing_cause."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
...
if ($action=="approve")
{
    ...
    $finalDataArr=getFinalUser(array('company_id'=>$cbo_company_name,'page_id'=>$menu_id,'entry_form'=>19,'lib_buyer_arr'=>0,'lib_brand_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0,'lib_department_id_arr'=>$department_arr,'match_data'=>$matchDataArr));
	...
	$max_approved_no_arr = return_library_array("SELECT mst_id, max(approved_no) as approved_no from approval_history where mst_id in($gatePass_ids) and entry_form=19 group by mst_id","mst_id","approved_no");
	...
	if($approval_type==0)
	{
		...
		foreach($gatePass_ids_all as $key => $gatePass_id)
		{
			...
			$data_array.="(".$id.",19,".$gatePass_id.",".$approved_no.",'".$user_sequence_no."',1,".$app_user_id.",'".$pc_date_time."',".$user_id.",'".$pc_date_time."',".$approved.",'".$cause_arr[$key]."')"; 
			$id=$id+1;

			//App mst data.......................
			if($app_data_array!=''){$app_data_array.=",";}
			$app_data_array.="(".$appid.",19,".$gatePass_id.",".$user_sequence_no.",".$app_user_id.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."',".$approved.",".$user_group_no.")"; 
			$appid++;
		...
		if($flag==1) 
		{
			$query="update approval_history set current_approval_status=0  WHERE entry_form=19 and mst_id in ($gatePass_ids)";
		...
		foreach($gatePass_ids_all as $key => $gatePass_id)
		{
			...
			if($last_app_status_arr[$gatePass_id] == 0 || $last_app_status_arr[$gatePass_id] == 2){
 			...
 			//History data.......................
			if($data_array!="") $data_array.=",";
			$data_array.="(".$id.",19,".$gatePass_id.",".$approved_no.",'".$user_sequence_no."',0,".$app_user_id.",'".$pc_date_time."',".$user_id.",'".$pc_date_time."',2,'".$cause_arr[$key]."')"; 
			$id=$id+1;
		}
		...
		if($flag==1) 
		{
			$query="delete from approval_mst  WHERE entry_form=19 and mst_id in ($gatePass_ids)";
	...
	else
	{
		...
		foreach($gatePass_ids_all as $key => $gatePass_id)
		{
			...
			if($last_app_status_arr[$gatePass_id] == 0 || $last_app_status_arr[$gatePass_id] == 2){
 			...
 			//History data.......................
			if($data_array!="") $data_array.=",";
			$data_array.="(".$id.",19,".$gatePass_id.",".$approved_no.",'".$user_sequence_no."',0,".$app_user_id.",'".$pc_date_time."',".$user_id.",'".$pc_date_time."',0,'".$cause_arr[$key]."')"; 
			$id=$id+1;
		}
