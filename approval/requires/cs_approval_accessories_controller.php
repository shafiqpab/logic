...
if($action=="report_generate")
{
	...
	foreach ($sql_company as $val) {
		...
		$company_contactNo_library[$val[csf('id')]]=$val[csf('contact_no')];
	}
	...
	foreach ($sql_supplier as $val) {
		...
		$supplier_contactNo_library[$val[csf('id')]]=$val[csf('contact_no')];
	}
	...
	if($previous_approved==1 && $approval_type==1)	//approval process with prevous approve start
	{
		...
		$sql="SELECT a.id as ID, a.sys_number as CS_NUMBER, a.sys_number_prefix_num as CS_NUMBER_PREFIX_NUM, $year_field,  a.cs_date as CS_DATE, a.supp_id as SUPP_ID, a.company_id as COMPANY_ID, a.inserted_by as CS_INSERT_BY, b.id as DTLS_ID, b.item_category_id as ITEM_CATEGORY_ID, b.item_description as ITEM_DESCRIPTION, b.item_group_id as ITEM_GROUP_ID, b.main_group_id as MAIN_GROUP_ID, b.brand_supplier as ITEM_REF, b.uom as UOM, b.req_qty as REQ_QTY, c.id as APPROVAL_ID, c.approved_date as APPROVED_DATE 
		from req_comparative_mst a, req_comparative_dtls b, approval_history c 
		where a.id=b.mst_id and a.id=c.mst_id and a.entry_form=482 and a.approved in (1,3) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.entry_form=50 and c.current_approval_status=1 $item_category_cond $cs_no_cond $cs_date_cond $cs_year_cond $sequence_no_cond
		group by a.id, a.sys_number, a.sys_number_prefix_num, $year_cond_prefix, a.cs_date, a.supp_id, a.company_id, a.inserted_by, b.id, b.item_category_id, b.item_description, b.item_group_id, b.main_group_id, b.brand_supplier, b.uom, b.req_qty, c.id, c.approved_date
		order by a.id";
		//echo "$sql";
	}
	else if($approval_type==0)	// unapproval process start
	{
		...
		if($user_sequence_no==$min_sequence_no)  // First user
		{
		 	...
		 	$sql="SELECT a.id as ID, a.sys_number as CS_NUMBER, a.req_item_dtls_id as REQ_ITEM_DTLS_ID, a.sys_number_prefix_num as CS_NUMBER_PREFIX_NUM, $year_field,  a.cs_date as CS_DATE, a.supp_id as SUPP_ID, a.company_id as COMPANY_ID, a.inserted_by as CS_INSERT_BY, b.id as DTLS_ID, b.item_category_id as ITEM_CATEGORY_ID, b.item_description as ITEM_DESCRIPTION, b.item_group_id as ITEM_GROUP_ID, b.main_group_id as MAIN_GROUP_ID, b.brand_supplier as ITEM_REF, b.uom as UOM, b.req_qty as REQ_QTY,b.REQ_RATE,b.REQ_AMOUNT 
		 	from req_comparative_mst a, req_comparative_dtls b 
		 	where a.id=b.mst_id and a.entry_form=482 and a.approved=$approval_type and a.ready_to_approved=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $item_category_cond $cs_no_cond $cs_date_cond $cs_year_cond 
		 	group by a.id, a.sys_number, a.req_item_dtls_id, a.sys_number_prefix_num, $year_cond_prefix, a.cs_date, a.supp_id, a.company_id, a.inserted_by, b.id, b.item_category_id, b.item_description, b.item_group_id, b.main_group_id, b.brand_supplier, b.uom, b.req_qty,b.REQ_RATE,b.REQ_AMOUNT
			order by a.id";
			//echo $sql;
		}
		else // Next user
		{
			...
			if($sequence_no=="")  // bypass if previous user Yes
			{
				if($db_type==0)
				{
					...
					$req_comp_id=return_field_value("LISTAGG(b.mst_id, ',') WITHIN GROUP (ORDER BY b.mst_id) as req_comp_id","req_comparative_mst a, req_comparative_dtls b, approval_history c","a.id=b.mst_id and a.id=c.mst_id and a.ready_to_approved=1  and a.approved in (3,1) and a.is_deleted=0 and c.sequence_no in ($sequence_no_by) and c.entry_form=50 and c.current_approval_status=1 $cs_date_cond","req_comp_id");
					$req_comp_id=implode(",",array_unique(explode(",",$req_comp_id)));
					
					$req_comp_id_app_byuser=return_field_value("LISTAGG(b.mst_id, ',') WITHIN GROUP (ORDER BY b.mst_id) as req_comp_id","req_comparative_mst a, req_comparative_dtls b, approval_history c","a.id=b.mst_id and a.id=c.mst_id and a.ready_to_approved=1  and a.approved in (3,1) and a.is_deleted=0 and c.sequence_no=$user_sequence_no and c.entry_form=50 and c.current_approval_status=1","req_comp_id");
				...
				{					
					...
					from req_comparative_mst a, req_comparative_dtls b
					where a.id=b.mst_id and a.entry_form=482 and a.approved in(1,3) and a.ready_to_approved=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.id in ($req_comp_id) $item_category_cond $cs_no_cond $cs_date_cond $cs_year_cond 
		 			group by a.id, a.sys_number, a.req_item_dtls_id, a.sys_number_prefix_num, $year_cond_prefix, a.cs_date, a.supp_id, a.company_id, a.inserted_by, b.id, b.item_category_id, b.item_description, b.item_group_id, b.main_group_id, b.brand_supplier, b.uom, b.req_qty,b.REQ_RATE,b.REQ_AMOUNT) x  order by x.ID";
					//echo $sql;
				}
				else
				{ 
					$sql="SELECT a.id as ID, a.sys_number as CS_NUMBER, a.req_item_dtls_id as REQ_ITEM_DTLS_ID, a.sys_number_prefix_num as CS_NUMBER_PREFIX_NUM, $year_field,  a.cs_date as CS_DATE, a.supp_id as SUPP_ID, a.company_id as COMPANY_ID, a.inserted_by as CS_INSERT_BY, b.id as DTLS_ID, b.item_category_id as ITEM_CATEGORY_ID, b.item_description as ITEM_DESCRIPTION, b.item_group_id as ITEM_GROUP_ID, b.main_group_id as MAIN_GROUP_ID, b.brand_supplier as ITEM_REF, b.uom as UOM, b.req_qty as REQ_QTY 
					from req_comparative_mst a, req_comparative_dtls b
					where a.id=b.mst_id and a.entry_form=482 and a.approved=$approval_type and a.ready_to_approved=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $item_category_cond $cs_no_cond $cs_date_cond $cs_year_cond 
		 			group by a.id, a.sys_number, a.req_item_dtls_id, a.sys_number_prefix_num, $year_cond_prefix, a.cs_date, a.supp_id, a.company_id, a.inserted_by, b.id, b.item_category_id, b.item_description, b.item_group_id, b.main_group_id, b.brand_supplier, b.uom, b.req_qty
		 			order by a.id";
					//echo $sql;
				}
				//echo $sql;
			...
			else // if previous user bypass No 
			{
				...
				$sql="SELECT a.id as ID, a.sys_number as CS_NUMBER, a.req_item_dtls_id as REQ_ITEM_DTLS_ID, a.sys_number_prefix_num as CS_NUMBER_PREFIX_NUM, $year_field,  a.cs_date as CS_DATE, a.supp_id as SUPP_ID, a.company_id as COMPANY_ID, a.inserted_by as CS_INSERT_BY, b.id as DTLS_ID, b.item_category_id as ITEM_CATEGORY_ID, b.item_description as ITEM_DESCRIPTION, b.item_group_id as ITEM_GROUP_ID, b.main_group_id as MAIN_GROUP_ID, b.brand_supplier as ITEM_REF, b.uom as UOM, b.req_qty as REQ_QTY,b.REQ_RATE,b.REQ_AMOUNT, c.id as APPROVAL_ID, c.approved_date as APPROVED_DATE 
				from req_comparative_mst a, req_comparative_dtls b, approval_history c 
				where a.id=b.mst_id and a.id=c.mst_id and a.entry_form=482 and a.approved in (1,3) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.entry_form=50 and c.current_approval_status=1 $item_category_cond $cs_no_cond $cs_date_cond $cs_year_cond $sequence_no_cond
				group by a.id, a.sys_number, a.req_item_dtls_id, a.sys_number_prefix_num, $year_cond_prefix, a.cs_date, a.supp_id, a.company_id, a.inserted_by, b.id, b.item_category_id, b.item_description, b.item_group_id, b.main_group_id, b.brand_supplier, b.uom, b.req_qty,b.REQ_RATE,b.REQ_AMOUNT, c.id, c.approved_date
				order by a.id";
				//echo $sql;
			}
		}	
	...
	if ($company_count > 0) $company_width=$company_count*400;
	else $company_width=0;

	$supplierArr=array_unique(explode(",", implode(",", $supplier_arr)));
	$supplier_width=count($supplierArr)*400;
	...
	$sql_demand_res=sql_select("SELECT id as ID, item_group_id as ITEM_GROUP_ID, brand_supplier as ITEM_REF, item_description as ITEM_DESCRIPTION, uom as UOM, nominate_supplier_id as NOMINATE_SUPPLIER_ID from scm_demand_dtls where id in($req_item_dtls_ids) and is_deleted=0 and status_active=1 order by id asc");
	$nominate_supplier_arr=array();
	foreach ($sql_demand_res as $val) {
		$key=$val["ITEM_GROUP_ID"]."**".$val["ITEM_REF"]."**".$val["ITEM_DESCRIPTION"]."**".$val["UOM"];
		$nominate_supplier_arr[$key]=$val["NOMINATE_SUPPLIER_ID"];
	}
	...
	foreach ($supp_dtls_res as $row) {
		if ($row["SUPP_TYPE"] == 1)
		{
			...
			$supp_dtls_arr[$row["DTLS_ID"]][$row["SUPP_ID"]]['ALLOCATED_QTY']=$row["ALLOCATED_QTY"];
			$supp_dtls_arr[$row["DTLS_ID"]][$row["SUPP_ID"]]['TOTAL_VALUE']=$row["NEG_PRICE"]*$row["ALLOCATED_QTY"];
		...
		else if ($row["SUPP_TYPE"] == 2)
		{
			...
			$comp_dtls_arr[$row["DTLS_ID"]][$row["SUPP_ID"]]['ALLOCATED_QTY']=$row["ALLOCATED_QTY"];
			$comp_dtls_arr[$row["DTLS_ID"]][$row["SUPP_ID"]]['TOTAL_VALUE']=$row["NEG_PRICE"]*$row["ALLOCATED_QTY"];
	...
	$tbl_width=1308+$company_width*1+$supplier_width*1;
	?>
	<script>
		...
		function openmypage_supplier(sl, row_num, cs_number, dtls_id)
		{
			...
			var page_link="requires/cs_approval_accessories_controller.php?action=supplier_popup&txt_supp_comp_dtlsid="+txt_supp_comp_dtlsid+"&txt_dtlsid="+txt_dtlsid+"&dtls_id="+dtls_id;
			...
			emailwindow.onclose=function()
			{
				...
				if (theemailsuppID != '' || theemailcompID != '')
				{
					...
					get_php_form_data( sl+"**"+row_num+"**"+theemailDtlsID+"**"+supp_ids+"**"+comp_ids+"**"+type, "populate_data_from_supplier_popup", "requires/cs_approval_accessories_controller" );				
				}
				else
				{
					var type=2;
					...
					get_php_form_data( sl+"**"+row_num+"**"+txt_dtlsid+"**"+supp_ids+"**"+comp_ids+"**"+type, "populate_data_from_supplier_popup", "requires/cs_approval_accessories_controller" );
				}	
		...
		function fn_cs_check()
		{
			if ($(copy_cs).is(":checked"))
			{
				...
				get_php_form_data( txtDtlsid+"**"+txtSuppid+"**"+txtCompid+"**"+txtSuppCompDtlsid+"**"+tbl_row_count+"**"+type, "populate_data_from_copy_cs_check", "requires/cs_approval_accessories_controller" );
				...
				{					
					...
					get_php_form_data( sl+"**"+row_num+"**"+txtDtlsidWithoutFirstRow+"**"+txtSuppidWithoutFirstRow+"**"+txtCompidWithoutFirstRow+"**"+type, "populate_data_from_supplier_popup", "requires/cs_approval_accessories_controller" );
				}
			}
			else
			{
				...
				for( var i = 1; i <= tbl_row_count; i++ ) 
				{
					...
					get_php_form_data( sl+"**"+row_num+"**"+txtDtlsid+"**"+txtSuppid+"**"+txtCompid+"**"+type, "populate_data_from_supplier_popup", "requires/cs_approval_accessories_controller" );
		...
		function calculate_value___()
	    {
	        var row_num=$('#tbl_cs_list tbody tr').length;
	        // alert(row_num);return;
	        for (var i=1; i<=row_num; i++)
	        {
	        	var supplier_id = $('#txt_supp_id_'+i).val();
		        var col_num_arr = supplier_id.split(',');
		        var col_num = col_num_arr.length; 
		        var company_name_id = $('#txt_comp_id_'+i).val();
	            ...
	            var req_qty = $('#txtreqqty_'+i).attr('title')*1;  //alert(req_qty);return;	            
	            for (var m=0; m<col_num; m++)
	            {
	                var mm=col_num_arr[m];
	                supp_neg_price = $('#txtApprovedPriceSpplier_'+i+'_'+mm).val()*1;	                
	                supp_allocated_qty = $('#txtallocatedqty_'+i+'_'+mm).attr('title')*1;           
	                total_value = supp_neg_price*supp_allocated_qty;
	                if (total_value==0) $('#txttotalvalue_'+i+'_'+mm).html("");
	                else $('#txttotalvalue_'+i+'_'+mm).html(total_value.toFixed(2));
	            ...
	            for (var m=0; m<company_num; m++)
	            {
	                ...
	                com_neg_price = $('#txtApprovedPriceCompany_'+i+'_'+mm).val()*1;
	                com_allocated_qty = $('#txtCompanyAllocatedQty_'+i+'_'+mm).attr('title')*1;
	                total_value = com_neg_price*com_allocated_qty;
	                if (total_value==0)  $('#txtCompanyTotalValue_'+i+'_'+mm).html("");
	                else $('#txtCompanyTotalValue_'+i+'_'+mm).html(total_value.toFixed(2));
		...
		function calculate_value(input_id)
	    {
	       
			var cs_row_num = $( "[name^='tbl']" ).length;		
			
	        for (var cs=1; cs<=cs_row_num; cs++)
	        {
				var item_row_num = $( "[id^='booking_id_"+cs+"_']" ).length;
				for (var i=1; i<=item_row_num; i++)
	        	{
					var key = cs+'_'+i;
					
					var supplier_id = $('#txt_supp_id_'+key).val();
					var col_num_arr = supplier_id.split(',');
					var col_num = col_num_arr.length; 

					var company_name_id = $('#txt_comp_id_'+key).val();
					var company_num_arr = company_name_id.split(',');
					if(company_name_id!=''){var company_num = company_num_arr.length}else{var company_num =0;}  
					...
					var req_qty = $('#txtreqqty_'+key).attr('title')*1;  //alert(req_qty);return;	            
					for (var m=0; m<col_num; m++)
					{
						var mm=col_num_arr[m];
						supp_neg_price = $('#txtApprovedPriceSpplier_'+key+'_'+mm).val()*1;	                
						supp_allocated_qty = $('#txtallocatedqty_'+key+'_'+mm).attr('title')*1;           
						total_value = supp_neg_price*supp_allocated_qty;
						if (total_value==0) $('#txttotalvalue_'+key+'_'+mm).html('');
						else $('#txttotalvalue_'+key+'_'+mm).html(total_value.toFixed(2));
					}

					for (var m=0; m<company_num; m++)
					{
						var mm=company_num_arr[m];
						com_neg_price = $('#txtApprovedPriceCompany_'+key+'_'+mm).val()*1;
						com_allocated_qty = $('#txtCompanyAllocatedQty_'+key+'_'+mm).attr('title')*1;
						total_value = com_neg_price*com_allocated_qty;
						if (total_value==0)  $('#txtCompanyTotalValue_'+key+'_'+mm).html('');
						else $('#txtCompanyTotalValue_'+key+'_'+mm).html(total_value.toFixed(2));
			...
			var supplier_id_arr = <?= json_encode($supplierArr ); ?>;
			var cs_row_num = $( "[name^='tbl']" ).length;
			for (var cs=1; cs<=cs_row_num; cs++)
			{	
				$.each( supplier_id_arr, function( key, sid ) {
					var total_supp_allocated_val=0;
					var item_row_num = $( "[id^='booking_id_"+cs+"_']" ).length;
					for (var i=1; i<=item_row_num; i++)
					{ 
						supp_allocated_val = $(`#txttotalvalue_${cs}_${i}_${sid}`).text();
						if(supp_allocated_val>0){total_supp_allocated_val += supp_allocated_val*1;}
					}
					$(`#txt_allocated_val_${cs}_${sid}`).text(total_supp_allocated_val.toFixed(2));

					var [__,cs_row,item_row,supplyer_id]=input_id.split('_');
					var req_val = $(`#txt_req_val_${cs}`).text()*1;
					var allocated_val = $(`#txt_allocated_val_${cs}_${sid}`).text()*1;
					if(req_val < allocated_val && cs_row == cs && supplyer_id == sid){
						$(`#${input_id}`).val(0);
						calculate_value(input_id);
					}
				});
        	...
        	<thead>
        		<tr>
	        		...
	        		if ($company_count > 0)
	        		{
						...
						{
							?>
		                	<th colspan="5" align="center"><?= $company_contactNo_library[$comp_id]; ?></th>
		                	<?
		                }
	        		}
					...
					{
						?>
	                	<th colspan="5" align="center"><?= $supplier_contactNo_library[$supp_id]; ?></th>
	                	<?
	                }
	                ?>
	                <th colspan="2">&nbsp;</th> 
            	...
            	<tr>
	                ...
	                <th width="80" rowspan="2">CS No</th>
	                <th width="60" rowspan="2">CS Year</th>
	                <th width="80" rowspan="2">CS Date</th>
	                <th width="100" rowspan="2">Item Category</th>
	                <th width="100" rowspan="2">Main Group</th>
	                <th width="100" rowspan="2">Items Name</th>
	                <th width="80" rowspan="2">Item Ref.</th>
	                <th width="150" rowspan="2">Items Description</th>
	               	<th width="80" rowspan="2">Req. Qty.</th>
	               	<th width="80" rowspan="2">Unit Price</th>
	               	<th width="80" rowspan="2">TTL Amount</th>
	               	<th width="60" rowspan="2">UOM</th>
	                <th width="150" rowspan="2" style="color: blue;">Supplier</th>
	                <?
	                if ($company_count > 0)
	        		{
		                foreach($companyArr as $comp_id)
						{
							?>
		                	<th colspan="5" width="400"><?= $company_library[$comp_id]; ?></th>
		                	<?
		                }
	            	}
					foreach($supplierArr as $supp_id)
					{
						?>
	                	<th colspan="5" width="400"><?= $supplier_library[$supp_id]; ?></th>
	                	<?
	                }
	                ?>                
	                <th width="80" rowspan="2">Approved Date</th>
	               	<th width="100" rowspan="2">CS Insert By</th>
                </tr>
                <tr>
	               	<?
	                if ($company_count > 0 )
					{
	                    foreach ($companyArr as $comp_id)
						{
							...
							<th width="80" class="wrd_brk" style="vertical-align:middle;" align="center"><p>Allocated Qty</p></th>
                ...
                foreach ($sql_res as $row) 
                {
					...
					<tr bgcolor="<?= $bgcolor; ?>" id="tr_<?= $i; ?>" align="center" >
						...
						if ($check_cs_number[$row['CS_NUMBER']]=="")
						{
							...
							<td rowspan="<?= $rowspan_arr[$row['CS_NUMBER']]; ?>" width="80" class="wrd_brk" style="vertical-align:top;"><p><a href='##' style='color:#000' onClick="print_report(<? echo $row['ID']; ?>,'comparative_statement_print', '../commercial/work_order/requires/comparative_statement_accessories_controller')"><? echo $row['CS_NUMBER_PREFIX_NUM']; ?></a></p></td>
                        ...
                        ?>                      
                        <td width="100"  class="item_class_<?= $sl; ?>  wrd_brk" style="vertical-align:middle;" align="left" id="item_category_id_<?= $sl.'_'.$row_num; ?>" title="<?= $row['ITEM_CATEGORY_ID']; ?>"><p><?= $item_category[$row['ITEM_CATEGORY_ID']]; ?>	  </p>
                        ...
                        <td width="100"  class="wrd_brk" style="vertical-align:middle;" align="left"><p><?= $main_group_library[$row['MAIN_GROUP_ID']]; ?></p></td>
                        <td width="100"  class="wrd_brk" style="vertical-align:middle;" align="left" id="item_group_id_<?= $sl.'_'.$row_num; ?>" title="<?= $row['ITEM_GROUP_ID']; ?>"><p><?= $item_group_library[$row['ITEM_GROUP_ID']]; ?></p></td>
                        
                        <td width="80" class="wrd_brk" style="vertical-align:middle;" align="left" id="item_ref_<?= $sl.'_'.$row_num; ?>" title="<?= $row['ITEM_REF']; ?>"><p><?= $row['ITEM_REF']; ?></p></td>
                        <td width="150" class="wrd_brk" style="vertical-align:middle;" align="left" id="item_description_<?= $sl.'_'.$row_num; ?>" title="<?= $row['ITEM_DESCRIPTION']; ?>"><p><?= $row['ITEM_DESCRIPTION']; ?></p></td>
                        <td width="80" class="wrd_brk" style="vertical-align:middle;" align="right" id="txtreqqty_<?= $sl.'_'.$row_num; ?>" title="<?= $row['REQ_QTY']; ?>"><p><?= $row['REQ_QTY']; ?></p></td>

						<td width="80" class="wrd_brk"><?= $row['REQ_RATE']; ?></td>
						<td align="right" width="80" id="txtreqval_<?= $sl.'_'.$row_num; ?>" class="wrd_brk"><?= $row['REQ_AMOUNT']; ?></td>
											...
											if ($company_id==$comp_id)
											{
												...
												else
												{
													...
													<td width="80" class="wrd_brk" style="vertical-align:middle;" align="center">
														<input type="text" name="" id="txtApprovedPriceCompany_<?= $sl.'_'.$row_num.'_'.$comp_id; ?>" class="text_boxes_numeric" style="width: 70px;" disabled value="<?= $val['LAST_APPROVAL_RATE']; ?>" onKeyUp="calculate_value(this.id)"/></td>
														<input type="hidden" name="" id="suppDtlsidCompany_<?= $sl.'_'.$row_num.'_'.$comp_id; ?>" value="<?= $dtls_id; ?>" />
													</td>	
													<?
												}
												?>
												<td width="80" class="wrd_brk" style="vertical-align:middle;" id="txtCompanyAllocatedQty_<?= $sl.'_'.$row_num.'_'.$comp_id; ?>" title="<?= $val['ALLOCATED_QTY']; ?>" align="center"><?= $val['ALLOCATED_QTY']; ?>&nbsp;</td>
												<td width="80" class="wrd_brk" style="vertical-align:middle;" id="txtCompanyTotalValue_<?= $sl.'_'.$row_num.'_'.$comp_id; ?>"  align="center"><? if ($val['TOTAL_VALUE'] != 0) echo $val['TOTAL_VALUE']; else echo ""; ?>&nbsp;</td>
												<?
						                		$TOTAL_VALUE_ARR[$row['ID']][$supp_id] += $val['TOTAL_VALUE'];
												$ALLOCATED_QTY_ARR[$row['ID']][$supp_id] += $val['ALLOCATED_QTY'];
						                	}				                		
								...
								foreach ($supplierArr as $supplier_id) 
								{
									...
									foreach ($supp_data as $supp_id => $val)
									{
										if ($supplier_id==$supp_id)
										{
											$k++;
											$background_color="";
											if ($nominate_supplier_arr[$item]==$supp_id) $background_color = "background-color: yellow";
											//if ($val['NEG_PRICE'] == "") $disabled ="disabled";											
											?>
						                	<td width="80" class="wrd_brk" title="<?= $nominate_supplier_arr[$item]; ?>" style="vertical-align:middle; <? echo $background_color; ?>" align="center" ><?= $val['QUOTED_PRICE']; ?>&nbsp;</td>
											...
											{	
												...
												<td width="80" class="wrd_brk" style="vertical-align:middle; <? echo $background_color; ?>" align="center">
													<input type="text" name="" id="txtApprovedPriceSpplier_<?= $sl.'_'.$row_num.'_'.$supp_id; ?>" class="text_boxes_numeric" style="width: 70px; <? echo $background_color; ?>" disabled value="<?= $val['LAST_APPROVAL_RATE']; ?>" onKeyUp="calculate_value(this.id)"/>
													<input type="hidden" name="" id="suppDtlsidSupplier_<?= $sl.'_'.$row_num.'_'.$supp_id; ?>" value="<?= $dtls_id; ?>"/>
												</td>
												<?
											}
						                	?>
						                	<td width="80" class="wrd_brk" style="vertical-align:middle; <? echo $background_color; ?>" align="center" id="txtallocatedqty_<?= $sl.'_'.$row_num.'_'.$supp_id; ?>" title="<?= $val['ALLOCATED_QTY']; ?>"><?= $val['ALLOCATED_QTY']; ?>&nbsp;</td>
						                	<td width="80" class="wrd_brk" style="vertical-align:middle; <? echo $background_color; ?>" align="center" id="txttotalvalue_<?= $sl.'_'.$row_num.'_'.$supp_id; ?>"><? if ($val['TOTAL_VALUE'] != 0) echo $val['TOTAL_VALUE']; else echo ""; ?>&nbsp;</td>
						                	<?
											$TOTAL_VALUE_ARR[$row['ID']][$supp_id] += $val['TOTAL_VALUE'];
											$ALLOCATED_QTY_ARR[$row['ID']][$supp_id] += $val['ALLOCATED_QTY'];
						                }	
					...
					if ($row_num == $rowspan_arr[$row['CS_NUMBER']]){ 
						
					?>
					<tr style="background-color: #ccc;">
						<td colspan="10" align="right">Total</td>
						<td align="right"><?= $REQ_QTY;?></td>
						<td></td>
						<td align="right" id="txt_req_val_<?= $sl; ?>"><?= $REQ_AMOUNT;?></td>
						<td></td>					
						<td></td>
						<?
						foreach($supplierArr as $supp_id)
						{
							...
							<td></td>
							<td></td>
							<td align="center" id="txt_allocated_qty_<?= $sl.'_'.$supp_id; ?>"><?= $ALLOCATED_QTY_ARR[$row['ID']][$supp_id];?></td>
							<td align="center" id="txt_allocated_val_<?= $sl.'_'.$supp_id; ?>"><?= $TOTAL_VALUE_ARR[$row['ID']][$supp_id];?></td>
							<?
						}
						?>
						<td colspan="2"></td>
        ...
        <table cellspacing="0" cellpadding="0" width="200" align="left">
			<tfoot>
                <td width="50" align="center" >
					<input type="checkbox" id="all_check" onClick="check_all('all_check',<?= $sl; ?>)" />
				</td>
	        ...
	        <form name="searchbuyerfrm_1"  id="searchbuyerfrm_1" autocomplete="off">
	            ...
	            <div style="width:350px; overflow-y:scroll; max-height:280px;" align="center">
	                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="332" class="rpt_table" id="tbl_list_search" >
						...
						$supp_dtls="SELECT a.approved as APPROVED_MST, c.id as ID, c.mst_id as MST_ID, c.dtls_id as DTLS_ID, c.supp_id as SUPP_ID, c.supp_type as SUPP_TYPE, c.approved as APPROVED from req_comparative_mst a, req_comparative_supp_dtls c where a.id=c.mst_id and a.entry_form=482 and a.status_active=1 and a.is_deleted=0 and c.dtls_id in($supp_comp_dtlsid) and c.neg_price is not null and c.is_deleted=0 and c.status_active=1 order by c.id asc";
						$supp_dtls_res=sql_select($supp_dtls);
			...
			foreach ($supp_dtls_arr as $val)
			{
				...
				foreach ($supp_ids_arr as $supplier_id)
				{
					if ($supp_id==$supplier_id){
						$idsArr[$i][$val['ID']] = $val['ID'];
						//$dtls_id.=$val['DTLS_ID'];
						$supp_idsArr[$i][$supp_id]=$supp_id;
						$supp_comp_namesArr[$i][$supp_id]=$supplier_library[$supp_id];
			
					}
				}	
			}

			//print_r($supp_ids_arr[$i]);die;

			echo "$('#txt_supp_id_1_".$i."').val('".implode(',',$supp_idsArr[$i])."');\n";
			echo "$('#txt_supp_comp_name_1_".$i."').val('".implode(',',$supp_comp_namesArr[$i])."');\n";
			echo "$('#txt_supp_comp_dtlsid_1_".$i."').val('".implode(',',$idsArr[$i])."');\n";
			...
			foreach ($comp_dtls_arr as $val)
			{
				...
				foreach ($comp_ids_arr as $company_id)
				{
					if ($comp_id==$company_id){
						$idsArr[$i][$val['ID']] = $val['ID'];
						//$dtls_id.=$val['DTLS_ID'];
						$comp_idsArr[$i][$comp_id]=$comp_id;
						$supp_comp_namesArr[$i][$comp_id]=$company_library[$comp_id];			
					}
				}	
			}
			//echo $comp_ids.'system';

			echo "$('#txt_comp_id_1_".$i."').val('".implode(',',$comp_idsArr[$i])."');\n";
			echo "$('#txt_supp_comp_name_1_".$i."').val('".implode(',',$supp_comp_namesArr[$i])."');\n";
			echo "$('#txt_supp_comp_dtlsid_1_".$i."').val('".implode(',',$idsArr[$i])."');\n";
...
if ($action=="approve")
{
	...
	if($approval_type == 0)
	{
		...
		$max_approved_no_arr = return_library_array("SELECT mst_id, max(approved_no) as approved_no from approval_history where mst_id in($booking_ids_all) and entry_form=50 group by mst_id","mst_id","approved_no");
		...
		for($i=0; $i<count($booking_ids_allArr); $i++)
		{
			...
			$data_array.="(".$id.",50,".$booking_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."',".$user_id.",'".$pc_date_time."')"; 
			$id=$id+1;
		}
		...
		$field_array_prod="id, company_id, entry_form, item_category_id, item_group_id, brand_supplier, item_description, unit_of_measure, color, gmts_size, item_color, item_size, inserted_by, insert_date, status_active, is_deleted";
		...
		$sql_prod_res=sql_select("select ID, COMPANY_ID, ITEM_CATEGORY_ID, ITEM_GROUP_ID, BRAND_SUPPLIER, ITEM_DESCRIPTION, UNIT_OF_MEASURE, COLOR, GMTS_SIZE, ITEM_COLOR, ITEM_SIZE, IS_SUPP_COMP from product_details_master where item_category_id=4 and entry_form=24 and status_active=1 and is_deleted=0");
		foreach ($sql_prod_res as $val) 
		{
			$key=$val['ITEM_CATEGORY_ID'].'**'.$val['ITEM_GROUP_ID'].'**'.$val['BRAND_SUPPLIER'].'**'.$val['ITEM_DESCRIPTION'].'**'.$val['UNIT_OF_MEASURE'].'**'.$val['COLOR'].'**'.$val['GMTS_SIZE'].'**'.$val['ITEM_COLOR'].'**'.$val['ITEM_SIZE'];
			...
			for($j=1; $j<=$$item_row; $j++)
			{
				...
				for($as=0; $as<$all_supp_num_total_row; $as++)
				{
					...
					if (in_array($supp_id, $supp_num_arr))
					{
						...
						$supp_data=$$item_category_id.'**'.$$item_group_id.'**'.$$item_ref.'**'.$$item_description.'**'.$$uom.'**'.$gmts_color_id.'**'.$gmts_size_id.'**'.$item_color_id.'**'.$item_size;
						//echo $supp_data;
						$companyArr=array();
						...
						if (array_key_exists($supp_data,$product_data_arr))
						{
							...
							foreach ($company_diff_arr as $compID) {
								...
								$data_array_prod.="(".$prod_id.",".$compID.",24,".$$item_category_id.",".$$item_group_id.",'".$$item_ref."','".$$item_description."',".$$uom.",".$gmts_color_id.",".$gmts_size_id.",".$item_color_id.",'".$item_size."',".$user_id.",'".$pc_date_time."',1,0)";
								//$prod_id = $prod_id+1;
							}							
						...
						foreach ($all_product_arr as $product_id) {
							...
							$data_array_supplier_wise_rate.="(".$id.",".$product_id.",".$supp_id.",482,".$$item_category_id.",".$$item_group_id.",'".$pc_date_time."','".$$txtApprovedPriceSpplier."',".$is_supp_comp.",".$user_id.",'".$pc_date_time."',0)";
							$id = $id+1;
						}
				...
				if ($$all_comp_ids != "")
				{
					for($ac=0; $ac<$all_comp_num_total_row; $ac++)
					{
						...
						if (in_array($comp_id, $comp_num_arr))
						{
							...
							$comp_data=$$item_category_id.'**'.$$item_group_id.'**'.$$item_ref.'**'.$$item_description.'**'.$$uom.'**'.$gmts_color_id.'**'.$gmts_size_id.'**'.$item_color_id.'**'.$item_size;
							//echo $comp_data;
							$companyArr=array();
							$all_product_arr=array();
							...
							foreach ($all_product_arr as $product_id) {
								if ($data_array_supplier_wise_rate != '') $data_array_supplier_wise_rate .=",";
								$data_array_supplier_wise_rate.="(".$id.",".$product_id.",".$comp_id.",482,".$$item_category_id.",".$$item_group_id.",'".$pc_date_time."','".$$txtApprovedPriceCompany."',".$is_supp_comp.",".$user_id.",'".$pc_date_time."',0)";
								$id = $id+1;
							}
		...
		$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=50 and mst_id in ($booking_ids_all)";
...
if($action=="report_generate_after_approve_unapprove")
{
		...
		else // Next user
		{
			...
			if($sequence_no=="")  // bypass if previous user Yes
			{
				...
				{					
					...
					from req_comparative_mst a, req_comparative_dtls b
					where a.id=b.mst_id and a.entry_form=482 and a.approved in(1,3) and a.ready_to_approved=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.id in ($req_comp_id) $item_category_cond $cs_no_cond $cs_date_cond $cs_year_cond 
		 			group by a.id, a.sys_number, a.req_item_dtls_id, a.sys_number_prefix_num, $year_cond_prefix, a.cs_date, a.supp_id, a.company_id, a.inserted_by, b.id, b.item_category_id, b.item_description, b.item_group_id, b.main_group_id, b.brand_supplier, b.uom, b.req_qty) x  order by x.ID";
					//echo $sql;
				}
				else
				{ 
			...
			else // if previous user bypass No 
			{
				...
				from req_comparative_mst a, req_comparative_dtls b, approval_history c 
				where a.id=b.mst_id and a.id=c.mst_id and a.entry_form=482 and a.approved in (1,3) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.entry_form=50 and c.current_approval_status=1 $item_category_cond $cs_no_cond $cs_date_cond $cs_year_cond $sequence_no_cond
				group by a.id, a.sys_number, a.req_item_dtls_id, a.sys_number_prefix_num, $year_cond_prefix, a.cs_date, a.supp_id, a.company_id, a.inserted_by, b.id, b.item_category_id, b.item_description, b.item_group_id, b.main_group_id, b.brand_supplier, b.uom, b.req_qty, c.id, c.approved_date
				order by a.id";
				//echo $sql;
			}
		}	
	...
	/*$supp_dtls="SELECT a.sys_number as CS_NUMBER, b.item_group_id as ITEM_GROUP_ID, b.brand_supplier as ITEM_REF, b.item_description as ITEM_DESCRIPTION, b.uom as UOM, c.id as ID, c.mst_id as MST_ID, c.dtls_id as DTLS_ID, c.supp_id as SUPP_ID, c.supp_type as SUPP_TYPE, c.prod_id as PROD_ID, c.quoted_price as QUOTED_PRICE, c.neg_price as NEG_PRICE, c.con_price as CON_PRICE, c.last_approval_rate as LAST_APPROVAL_RATE, c.brand as BRAND, c.model as MODEL, c.origin as ORIGIN, c.approved as APPROVED 
		from req_comparative_mst a, req_comparative_dtls b, req_comparative_supp_dtls c where a.id=b.mst_id and b.id=c.dtls_id and a.id in(126) and a.entry_form=482 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 
		order by c.id asc";*/
	$supp_dtls_res=sql_select($supp_dtls);
	$supp_dtls_arr=array();
	$comp_dtls_arr=array();
	...
	<script>
		...
		//txttotalvalue_1_1_663 txtApprovedPriceSpplier_1_1_663
		function calculate_value()
	    {
	        ...
	        for (var i=1; i<=row_num; i++)
	        { 
	        	var supplier_id = $('#txt_supp_id_'+i).val(); //alert(supplier_id)
		        var col_num_arr = supplier_id.split(',');
		        var col_num = col_num_arr.length; 
		        var company_name_id = $('#txt_comp_id_'+i).val();
		        var company_num_arr = company_name_id.split(',');
	            ...
	            var req_qty = $('#txtreqqty_'+i).attr('title')*1;	            
	            for (var m=0; m<col_num; m++)
	            {
	                var mm=col_num_arr[m];
	                supp_neg_price = $('#txtApprovedPriceSpplier_'+i+'_'+mm).val()*1;	                
