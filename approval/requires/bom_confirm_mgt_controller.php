...
if($action=="button_variable_setting")
{
	$print_report_format = 0;
    $print_report_format = return_field_value("format_id","lib_report_template","template_name ='".$data."' and module_id=12 and report_id=303 and is_deleted=0 and status_active=1");
    echo "report_button_setting('".$print_report_format."');\n";
    exit();
}
...
{  //echo 111;die;
 ...
 ?>
    <form name="requisitionApproval_2" id="requisitionApproval_2">
        <fieldset style="width:1730px; margin-top:10px">
        <legend>BOM Confirmation Before Approval</legend>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1730" class="rpt_table" >
                <thead>
                    ...
                    <th width="80">Total Value</th>
                    <th width="60">SMV</th>
                    <th width="60">Sewing Efficiency</th>
                    <th width="60">CM</th>
                    <th width="60">EPM</th>
					<th width="60">CPM</th>
                    <th width="60">CM %</th>
            ...
            <div style="width:1730px; overflow-y:scroll; max-height:330px;" id="buyer_list_view" align="center">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1730" class="rpt_table" id="tbl_list_search">
					...
					//echo 111;die;
                        ...
                        if($jobIds!="")
                        {
							...
							$sql_std_para=sql_select("select interest_expense,income_tax,cost_per_minute,applying_period_date, applying_period_to_date from lib_standard_cm_entry where company_id=$company_name and status_active=1 and is_deleted=0 order by id");

							// echo "select interest_expense,income_tax,cost_per_minute,applying_period_date, applying_period_to_date from lib_standard_cm_entry where company_id=$cbo_company_name and status_active=1 and is_deleted=0 order by id";die;
						...
						$print_report_format_ids_2 = return_field_value("format_id","lib_report_template","template_name=".$company_name." and module_id=2 and report_id in (122) and is_deleted=0 and status_active=1");
                        $format_ids_2=explode(",",$print_report_format_ids_2);
						//print_r($format_ids_2);die;
                        $row_ids=$format_ids_2[0];
                       //print_r($row_id);
                        //Order Wise Budget Report button---------------------------------
                        $print_report_format_ids2 = return_field_value("format_id","lib_report_template","template_name=".$company_name." and module_id=11 and report_id=18 and is_deleted=0 and status_active=1");
                        $format_ids2=explode(",",$print_report_format_ids2);
                        ...
                        foreach ($nameArray as $row)
                        {
                            ...
                            if($print_cond==1)
                            {
                                if($row[csf('entry_from')]==158){
									...
									else if($row_id==874){$action='preCostRpt13';}
								 } else{
									if($row_ids==171){$action='preCostRpt4'; }
                           ...
                           if($isReq==1)
						   {
							  if($cm_std_per>$cmper && $cm_std_per!=0)
							  {
							   if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
								...
								<tr bgcolor="<?=$bgcolor; ?>" onClick="change_color('tr_<?=$i; ?>','<?=$bgcolor; ?>')" id="tr_<?=$i; ?>" align="center">
									...
									<td width="60" align="right" style="word-break:break-all;"><?=number_format($sew_efficient,2); ?></td>
									<td width="60" align="right" style="word-break:break-all;"><?=number_format($calCMarr[$booking_no],3); ?></td>
									<?
									$cpm=$calCMarr[$booking_no];
									?>
									<td width="60" align="right" style="word-break:break-all;"><?=number_format($cpm/$set_smv,3); ?></td>
                                      
									<td width="60" align="right" style="word-break:break-all;"><?=number_format($cpmCalarr[$booking_no],3); ?></td>

									
									<td width="60" align="right" style="word-break:break-all;" id="tdCm_<?=$i;?>" title="<?=$cmCost; ?>"><?=number_format($cm[$booking_no],2).' %';//$summary_data[cm_cost_percent].' %'; ?></td>

									<td width="60" align="right" style="word-break:break-all;" title="<?=$marginval; ?>"><?=number_format($margin[$booking_no],2).' %';//$summary_data[cm_cost_percent].' %'; ?></td>
                                 ...
                                 <?
									...
									<td align="center"> <input style="width:130px;" type="text" class="text_boxes" name="txtCause_<? echo $value; ?>" id="txtCause_<? echo $i; ?>" placeholder="browse" onClick="openmypage_refusing_cause('requires/bom_confirm_mgt_controller.php?action=refusing_cause_popup','Refusing Cause','<? echo $value; ?>');" value="<? echo $refusing_reason_arr[0][csf('refusing_reason')];?>" /></td>
								</tr>
								<?
								$i++;
							  }
							 ...
							 else if($margin_std_value>$marginval && $margin_std_value!=0)
							  {
								...
								<tr bgcolor="<?=$bgcolor; ?>" onClick="change_color('tr_<?=$i; ?>','<?=$bgcolor; ?>')" id="tr_<?=$i; ?>" align="center">
									...
									?>
									<td width="60" align="right" style="word-break:break-all;"><?=number_format($cpmCalarr[$booking_no],3); ?></td>
									<td width="60" align="right" style="word-break:break-all;"><?=fn_number_format($cpmCal,3); ?></td>
...
if ($action=="save_update_delete")
{
	...
	else if($approval_type==5)
	{
		...
		$denunapp="UPDATE wo_pre_cost_mst SET READY_TO_APPROVED=0 WHERE READY_TO_APPROVED=1 and id in ($mstids) and status_active=1 and is_deleted=0";
		...
		$rID2=execute_query($denunapp,1);
		if($rID2==1 && $flag==1) $flag=1; else $flag=0;

		
		//echo  "0**21,".$rID.','.$rID1.','.$rID2;oci_rollback($con);die;
...
if($action=='bom_approval_mail'){
    ...
    list($sysId, $mailId,$company_name, $type, $mail_body, $cause_arr)=explode('__',$data);
	...
	// if(str_replace("'","",$cbo_get_upto)==1) $date_cond=" and b.costing_date>$prev_date";
	// else if(str_replace("'","",$cbo_get_upto)==2) $date_cond=" and b.costing_date<=$prev_date";
	// else if(str_replace("'","",$cbo_get_upto)==3) $date_cond=" and b.costing_date=$prev_date";
	// else $date_cond='';
...
$sql= "select b.id,b.sew_smv,a.set_smv,b.sew_effi_percent,b.exchange_rate, a.quotation_id,a.avg_unit_price, a.job_no_prefix_num, $year_cond, a.id as job_id, a.job_no,a.company_name, a.buyer_name, a.style_ref_no, b.costing_date, '0' as approval_id, b.approved, b.inserted_by, b.entry_from, min(d.shipment_date) as minship_date, max(d.shipment_date) as maxship_date, a.job_quantity, (a.job_quantity*a.total_set_qnty) as job_qty_pcs, (a.job_quantity * a.avg_unit_price)    AS total_value, a.total_price, $internalRefCond as internalRef, $fileNoCond as fileNo from wo_pre_cost_mst b, wo_po_details_master a, wo_po_break_down d where a.job_no=b.job_no   and a.job_no=d.job_no_mst and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.company_name=$company_name  and b.approved in(0,2) and b.confirm_approval=0 $buyer_id_cond  $job_no_cond $job_year_cond $internal_ref_cond $file_no_cond  AND b.id IN ($sysId) group by b.id,b.exchange_rate, a.quotation_id,a.set_smv,b.sew_smv,b.sew_effi_percent,a.avg_unit_price, a.job_no_prefix_num, $year_cond_groupby,  a.id, a.job_no,a.company_name, a.buyer_name, a.style_ref_no, b.costing_date, '0', b.approved, b.inserted_by, b.entry_from, a.job_quantity, a.total_set_qnty, a.total_price  ";
//$sql_dtls=sql_select($sql);
   ...
   //echo $sql;die;
	...
	$sql = "SELECT c.EMAIL_ADDRESS FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id and a.mail_item=141 and b.mail_user_setup_id=c.id and a.company_id=$company   and a.IS_DELETED=0 and a.STATUS_ACTIVE=1  and b.IS_DELETED=0 and b.STATUS_ACTIVE=1   and c.IS_DELETED=0 and c.STATUS_ACTIVE=1 and a.MAIL_TEMPLATE<>1";
	//echo $sql;die; 
	...
	if($jobIds!="")
	{
		...
		// echo "select interest_expense,income_tax,cost_per_minute,applying_period_date, applying_period_to_date from lib_standard_cm_entry where company_id=$company_name and status_active=1 and is_deleted=0 order by id";die;
	...
	foreach ($nameArray as $row)
	{
		...
		{ 
			...
			$app_data_arr[$ii]['booking_id'] = $value;
			$app_data_arr[$ii]['booking_no'] = $row[csf('job_no')];
			$app_data_arr[$ii]['company_name'] = $row[csf('company_name')];
			$app_data_arr[$ii]['set_smv'] = $row[csf('set_smv')];
			$app_data_arr[$ii]['approval_id'] = $row[csf('approval_id')];
			$app_data_arr[$ii]['no_joooob'] = $ii;
			$app_data_arr[$ii]['no_joooob_id'] = strtoupper($row[csf('job_no')]);
			...
			$app_data_arr[$ii]['cmper'] = $cmper;
			$app_data_arr[$ii]['marginper'] = $marginper;
			$app_data_arr[$ii]['approved'] = $approved;
			$app_data_arr[$ii]['approval_type'] = $approval_type;
			$app_data_arr[$ii]['unapproved_request'] = $unapproved_request_arr[$value];
			$app_data_arr[$ii]['inserted_by'] = ucfirst($user_arr[$row[csf('inserted_by')]]);
			$app_data_arr[$ii]['approved_date'] = $row[csf('approved_date')];
	...
	?>
	 ...
	 <table rules="all">
    <tr>
        <td colspan="14"><b>This Job has been <?= $statusMsg; ?> by <?= $user_arr[$user_id]; ?>, Please Proceed.</b></td>
    </tr>
   </table>

	<table border="1" rules="all" width="1520">
	<tr> 
		<td width="30">SL</td>
		...
		<td width="100">Costing Date</td>
		<td width="100">Ship Start</td>
		<td width="100">Ship End</td>
		<td width="70">Job Qty[PCS]</td>

		<td width="70">Avg. Rate</td>
		<td width="80">Total Value</td>
		<td width="70">SMV</td>
		<td width="70">CM</td>
		<td width="70">EPM</td>
		<td width="70">CM %</td>
		<td width="70">Margin [PCS] %</td>
		<td width="120">Approved Date</td>
		<td width="120">Insert By</td> 
	</tr>
			...
			{     
				...
				if($isReq==1)
				{
					...
					<tr bgcolor="<?=$bgcolor; ?>" onClick="change_color('tr_<?=$i; ?>','<?=$bgcolor; ?>')" id="tr_<?=$i; ?>" align="center">
						...
						<td align="center"><?=$lib_com_arr[$company];?></td>
						...
						<td align="right" style="word-break:break-all;"><?=number_format($calCMarr[$booking_no],3); ?></td>
						             <?
									$cpm=$calCMarr[$booking_no];
									?>
						<td align="right" style="word-break:break-all;"><?=number_format($cpm/$set_smv,3); ?></td>
						<td align="right" style="word-break:break-all;" id="tdCm_<?=$i;?>" title="<?=$cmCost; ?>"><?=number_format($cm[$booking_no],2).' %';//$summary_data[cm_cost_percent].' %'; ?></td>
						<td align="right" style="word-break:break-all;" title="<?=$marginval; ?>"><?=number_format($margin[$booking_no],2).' %';//$summary_data[cm_cost_percent].' %'; ?>aa</td>
					...
					?>
						...
						<td align="right" style="word-break:break-all;"><?=number_format($cpm/$set_smv,3); ?></td>
						<td align="right" style="word-break:break-all;" id="tdCm_<?=$i;?>" title="<?=$cmCost; ?>"><?=number_format($cm[$booking_no],2).' %';//$summary_data[cm_cost_percent].' %'; ?></td>
						<td align="right" style="word-break:break-all;" title="<?=$marginval; ?>"><?=number_format($margin[$booking_no],2).' %';//$summary_data[cm_cost_percent].' %'; ?></td>
						<td><? if($approved_date!="0000-00-00") echo change_date_format($approved_date); ?>&nbsp;</td>
						<td style="word-break:break-all;"><?=$inserted_by;?>&nbsp;</td>
					...
					<tr bgcolor="<?=$bgcolor; ?>" onClick="change_color('tr_<?=$i; ?>','<?=$bgcolor; ?>')" id="tr_<?=$i; ?>" align="center">
						...
						<td align="right" style="word-break:break-all;"><?=number_format($cmPcs,2); ?>dd</td>
						<td align="right" style="word-break:break-all;"><?=number_format($btwob_per,2); ?></td>
						<td align="right" style="word-break:break-all;"><?=number_format($msper,2); ?></td>
						<td align="right" style="word-break:break-all;" id="tdCm_<?=$i;?>" title="<?=$cmCost; ?>"><?=number_format($cmper,2); ?></td>
						<td><? if($approved_date!="0000-00-00") echo change_date_format($approved_date); ?>&nbsp;</td>
						<td style="word-break:break-all;"><?=$inserted_by;?>&nbsp;</td>
					</tr>
					<?
