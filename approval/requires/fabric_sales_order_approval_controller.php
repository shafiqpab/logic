...
if($action=="report_generate")
{
	...
	if($txt_alter_user_id!="")
	{
		if(str_replace("'","",$cbo_buyer_name)==0)
		{
			...
			foreach($log_sql as $r_log)
			{
				if($r_log[csf('is_data_level_secured')]==1)
				{
					...
					if($r_log[csf('buyer_id')]!="") $pobuyer_id_cond=" and a.po_buyer in (".$r_log[csf('buyer_id')].")"; else $pobuyer_id_cond="";
				}
				else
				{
					$buyer_id_cond="";
		...
		else
		{
			...
			$pobuyer_id_cond=" and a.po_buyer=$cbo_buyer_name";
		}
		...
		if(str_replace("'","",$cbo_buyer_name)==0)
		{
			if ($_SESSION['logic_erp']["data_level_secured"]==1)
			{
				if($_SESSION['logic_erp']["buyer_id"]!="") 
				{
					...
					$pobuyer_id_cond=" and a.po_buyer in (".$_SESSION['logic_erp']["buyer_id"].")";
				}
				else 
				{
					$buyer_id_cond="";
    ...
    <?
		...
		$print_report_format_ids2=return_field_value("format_id","lib_report_template","template_name=".$cbo_company_name."  and module_id=7 and report_id=67 and is_deleted=0 and status_active=1");
        ...
        <legend>Fabric Booking Approval</legend>	
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $table1; ?>" class="rpt_table" >
                <thead>
                    ...
                    <th width="150">Sale-Booking No</th>
                            ...
                            foreach ($nameArray as $row)
                            {
								...
								if($print_cond==1)
								{
									...
									<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>"> 
                                        ...
                                        <td width="120" align="center"><? echo $unit; ?></td>
										...
										<td width="140" align="center"><? echo $buyer_name; ?></td>
...
//echo "21**";
	...
	$sql_buyer=sql_select( "select a.id, a.buyer_id,a.po_buyer,a.within_group   from fabric_sales_order_mst a  where  a.is_deleted=0 and  a.status_active=1 and a.id in ($booking_ids)" );
	$buyer_arr=array();
	foreach ($sql_buyer as  $b_val) {
		if($b_val[csf('buyer_id')]==1)  $buyer_arr[$b_val[csf('id')]]=$b_val[csf('po_buyer')];
		else 							$buyer_arr[$b_val[csf('id')]]=$b_val[csf('buyer_id')];		
	}
	...
	if($approval_type==0)
	{
		...
		$max_approved_no_arr = return_library_array("select mst_id, max(approved_no) as approved_no from approval_history where mst_id in($booking_ids) and entry_form=24 group by mst_id","mst_id","approved_no");
		
		$approved_status_arr = return_library_array("select id, is_approved from fabric_sales_order_mst where id in($booking_ids)","id","is_approved");
		...
		for($i=0;$i<count($booking_nos_all);$i++)
		{
			...
			$data_array.="(".$id.",24,".$booking_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."',".$user_id.",'".$pc_date_time."')";
			$id=$id+1;
		}
		...
		if(count($approved_no_array)>0)
		{
			...
			$sql_insert_dtls="insert into fabric_sales_order_dtls_his(id,approved_no, sales_dtls_id, mst_id,  job_no_mst, body_part_id, color_type_id, determination_id, fabric_desc, gsm_weight,  dia, width_dia_type, color_id, color_range_id, finish_qty, avg_rate,  amount, process_loss, grey_qty,  work_scope, yarn_data, inserted_by,  insert_date, updated_by, update_date,  status_active, is_deleted, order_uom,  pre_cost_remarks, rmg_qty, pre_cost_fabric_cost_dtls_id,  item_number_id, grey_qnty_by_uom, cons_uom ) 
				select	
				'', $approved_string_dtls,  id, mst_id, job_no_mst, body_part_id, color_type_id, determination_id,  fabric_desc, gsm_weight, dia, width_dia_type, color_id, color_range_id,   finish_qty, avg_rate, amount,  process_loss, grey_qty, work_scope,  yarn_data, inserted_by, insert_date,  updated_by, update_date, status_active,  is_deleted, order_uom, pre_cost_remarks,  rmg_qty, pre_cost_fabric_cost_dtls_id, item_number_id, grey_qnty_by_uom, cons_uom from fabric_sales_order_dtls where job_no_mst in ($book_nos)";
		...
		if(count($full_approve_booking_id_arr)>0)
		{
			...
			//echo "21**".bulk_update_sql_statement( "fabric_sales_order_mst", "id", $field_array_full_approved_booking_update, $data_array_full_approve_booking_update, $full_approve_booking_id_arr);die;
			$rID9=execute_query(bulk_update_sql_statement( "fabric_sales_order_mst", "id", $field_array_full_approved_booking_update, $data_array_full_approve_booking_update, $full_approve_booking_id_arr));
			if($flag==1)
			{
				if($rID9) $flag=1; else $flag=120;
			}
		...
		$rID=execute_query(bulk_update_sql_statement( "fabric_sales_order_mst", "id", $field_array_booking_update, $data_array_booking_update, $booking_id_arr));
		...
		$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=24 and mst_id in ($booking_ids)";
		$rIDapp=execute_query($query,1);
		if($flag==1) 
		{
			if($rIDapp) $flag=1; else $flag=20; 
		} 

		$rID2=sql_insert("approval_history",$field_array,$data_array,0);
		if($flag==1) 
		{
			if($rID2) $flag=1; else $flag=30; 
		} 
		...
		if(count($approved_no_array)>0)
		{
			...
			if($flag==1) 
			{
				if($rID3) $flag=1; else $flag=40; 
			} 

			$rID4=execute_query($sql_insert_dtls,1);
			if($flag==1) 
			{
				if($rID4) $flag=1; else $flag=50; 
			} 
		}
//echo "21**".$flag;die;
	...
	{
		$rID=sql_multirow_update("fabric_sales_order_mst","is_approved*ready_to_approved",'0*0',"id",$booking_ids,0);
