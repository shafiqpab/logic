...
if($action=="report_generate")
{
	...
	$txt_currier_no = str_replace("'","",$txt_currier_no);
    ...
    $electronicDataArr=getSequence(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>87,'user_id'=>$user_id_approval,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'product_dept_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0));
   //print_r($buyer_arr);
	...
	if($txt_bill_no!='') 
	{		
		$where_bill_cond.=" and a.bill_prefix_number ='".$txt_bill_no."'"; 
	}
	...
	if ($txt_currier_no == "") $txt_currier_no_cond = "";
	else $txt_currier_no_cond = " and a.currier_name='" . trim($txt_currier_no) . "' ";
	...
	{  
		...
		//  }
        $data_mast_sql ="select a.ID, a.company_id, a.buyer_id, a.ready_to_approve from air_way_bill_entry_mst a where a.company_id = $company_name and a.status_active = 1 and a.is_deleted = 0 and a.ready_to_approve = 1 and a.IS_APPROVED<>1 ";
		...
		for($seq=0;$seq<=count($electronicDataArr['sequ_arr']); $seq++ ){
			...
			if($tmp_sys_id_arr[$seq]){
				...
				$sql.=" select a.id, a.bill_prefix, a.bill_prefix_number, a.bill_system_id, a.company_id, a.buyer_id, a.currier_name, a.air_way_bill, a.country_id, a.team_leader, a.dealing_merchant, a.style_status, a.style_name, a.style_qty, a.bill_date, a.weight, a.charge_usd, a.total_charge_usd, a.charge_bdt, a.rate, a.dfs_charge_usd, a.ready_to_approve, a.is_posted_account from air_way_bill_entry_mst a where a.status_active = 1 and a.is_deleted = 0 and a.company_id = $company_name $where_id_cond $buyer_id_cond $txt_currier_no_cond $where_bill_cond  and a.IS_APPROVED<>1  and a.APPROVED_SEQU_BY=$seq $sys_con group by a.id, a.bill_prefix, a.bill_prefix_number, a.bill_system_id, a.company_id, a.buyer_id, a.currier_name, a.air_way_bill, a.country_id, a.team_leader, a.dealing_merchant, a.style_status, a.style_name, a.style_qty, a.bill_date, a.weight, a.charge_usd, a.total_charge_usd, a.charge_bdt, a.rate, a.dfs_charge_usd, a.ready_to_approve, a.is_posted_account ";
	...
	{   $sql= "select a.id, a.bill_prefix, a.bill_prefix_number, a.bill_system_id, a.company_id, a.buyer_id, a.currier_name, a.air_way_bill, a.country_id, a.team_leader, a.dealing_merchant, a.style_status, a.style_name, a.style_qty, a.bill_date, a.weight, a.charge_usd, a.total_charge_usd, a.charge_bdt, a.rate, a.dfs_charge_usd, a.ready_to_approve, a.is_posted_account from air_way_bill_entry_mst a,APPROVAL_MST b where a.status_active = 1 and a.is_deleted = 0 and a.company_id = $company_name $where_id_cond $buyer_id_cond $txt_currier_no_cond $where_bill_cond and b.SEQUENCE_NO={$electronicDataArr['user_by'][$user_id_approval]['SEQUENCE_NO']} and a.APPROVED_SEQU_BY=b.SEQUENCE_NO and a.IS_APPROVED<>0 and b.mst_id=a.id group by a.id, a.bill_prefix, a.bill_prefix_number, a.bill_system_id, a.company_id, a.buyer_id, a.currier_name, a.air_way_bill, a.country_id, a.team_leader, a.dealing_merchant, a.style_status, a.style_name, a.style_qty, a.bill_date, a.weight, a.charge_usd, a.total_charge_usd, a.charge_bdt, a.rate, a.dfs_charge_usd, a.ready_to_approve, a.is_posted_account";
 ...
 $nameArray = sql_select($sql);
	...
	$bokings= implode(',', $bokings_arr);
	 ...
	 $booking_sql=sql_select("SELECT a.ID,a.booking_no,b.job_no from air_way_bill_entry_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and a.company_id = $company_name $booking_cond and  a.status_active=1  and a.is_deleted=0 and  a.status_active=1 and a.is_deleted=0 group by  a.ID,a.booking_no,b.job_no ");
 
	//   echo "SELECT a.ID,a.booking_no,b.job_no from air_way_bill_entry_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and a.company_id = $company_name $booking_cond and  a.status_active=1  and a.is_deleted=0 and  a.status_active=1 and a.is_deleted=0 group by  a.ID,a.booking_no,b.job_no ";die;
        ...
        <div style="width:<? echo $table1+25; ?>px; margin:0 auto;">
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="left" >
                <thead>
                    ...
                    <th width="125"> Bill Date </th>
                    <th width="125">Currier Name</th>
                    <th width="100">Buyer</th>
                    <th width="150">Team Leader</th> 
                    <th width="150">Dealing Mar</th>                   
                    <th width="100">Style Qty</th>                   
                    <th width="100">Style Status</th>                   
                    <th width="100">Weight(KG)</th>                  
                    <th width="100">Charge ($)</th>                 
                    <th width="100">DFS Charge ($)</th>                  
                    <th width="100">Total Charge ($)</th>                
                        ...
                        {                                                   
							...
							if($report_ids[0]==163){ $action="show_trim_booking_report1";}
                                ...
                                <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>"> 
                                    ...
                                    <td width="100"><p><? echo $row[csf('bill_system_id')]; ?>&nbsp;</p></td>
                                    <td width="125"><p><? echo change_date_format($row[csf('bill_date')]); ?>&nbsp;</p></td>                                   
                                    <td width="125" align="center"><? echo $supplier_arr[$row[csf('currier_name')]]; ?> &nbsp;</p></td>
									<td width="100" align="center"><? echo $buyer_arr[$row[csf('buyer_id')]]; ?> &nbsp;</p></td>
                                    <td width="150"><p><? echo $team_leader_arr[$row[csf('team_leader')]]; ?> &nbsp;</p></td>
									
                                    <td width="150"><p><? echo $dealing_merc_arr[$row[csf('dealing_merchant')]]; ?> &nbsp;</p></td>				
                                    <td width="100"><p><? echo $row[csf('style_qty')]; ?>&nbsp;</p></td>
                                    <td width="100"><p><? echo $style_status_arr[$row[csf('style_status')]]; ?> &nbsp;</p></td>				                                  
                                    <td width="100"><p><? echo $row[csf('weight')]; ?>&nbsp;</p></td>				                                  
                                    <td width="100"><p><? echo $row[csf('charge_usd')]; ?>&nbsp;</p></td>				                                  
                                    <td width="100"><p><? echo $row[csf('dfs_charge_usd')]; ?>&nbsp;</p></td>				                                  
                                    <td width="100"><? echo $row[csf('total_charge_usd')]; ?>&nbsp;</p></td>				                                  
                                    <td width="100"><p><? echo $destination_arr[$row[csf('country_id')]]; ?> &nbsp;</p></td>				                                  
                                </tr>
                                <?
                                $i++;
...
{ 
	...
	$sql = "select a.ID,a.BUYER_ID,a.READY_TO_APPROVE  from air_way_bill_entry_mst a where a.COMPANY_ID=$cbo_company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0  and a.id in($target_ids)";
	//echo $sql;die();
	...
	$finalDataArr=getFinalUser(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>87,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0,'product_dept_arr'=>0,'match_data'=>$matchDataArr));
  ...
  //print_r($user_sequence_no) ;die;
	...
	{ 
        ...
        foreach($target_app_id_arr as $mst_id)
        {
			...
			$data_array.="(".$id.",87,'".$mst_id."','".$user_sequence_no."',".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			
			$approved_no=$max_approved_no_arr[$mst_id][$user_id_approval]+1;
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",87,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1)";
			$ahid++;
		...
		if($flag==1) 
		{
			...
			$rID2=execute_query(bulk_update_sql_statement( "air_way_bill_entry_mst", "id", $field_array_up, $data_array_up, $target_app_id_arr ));
			if($rID2) $flag=1; else $flag=0; 
		}

		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=87 and mst_id in ($target_ids)";
	...
	{
		$max_approved_no_arr=return_library_array( "select mst_id, max(approved_no) as approved_no from approval_history where entry_form=87 and mst_id in($target_ids)  and APPROVED=2 and APPROVED_BY=$user_id_approval group by mst_id", "mst_id", "approved_no"  );
		$ahid=return_next_id( "id","approval_history", 1 ) ;	
		
		$rID1=sql_multirow_update("air_way_bill_entry_mst","IS_APPROVED*READY_TO_APPROVE*APPROVED_SEQU_BY",'2*0*0',"id",$target_ids,0); ; 
		if($rID1) $flag=1; else $flag=0;

		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0, un_approved_by='".$user_id_approval."', un_approved_date='".$pc_date_time."', updated_by='".$user_id."', update_date='".$pc_date_time."' WHERE entry_form=87 and current_approval_status=1 and mst_id in ($target_ids)";
			$rID3=execute_query($query,1);
			if($rID3) $flag=1; else $flag=0;
	...
	{            
		...
		$next_user_app = sql_select("select id from approval_history where mst_id in($target_ids) and entry_form=87 and sequence_no >$user_sequence_no and current_approval_status=1 group by id");
		...
		$rID1=sql_multirow_update("air_way_bill_entry_mst","IS_APPROVED*READY_TO_APPROVE*APPROVED_SEQU_BY",'0*0*0',"id",$target_ids,0);
		if($rID1) $flag=1; else $flag=0;
		...
		if($flag==1) 
		{
			$query="delete from approval_mst  WHERE entry_form=87 and mst_id in ($target_ids)";
			$rID3=execute_query($query,1); 
			if($rID3) $flag=1; else $flag=0; 
		}
    ...
    <form>
           ...
           <?php
            $custom_designation=return_library_array( "select id,custom_designation from lib_designation ",'id','custom_designation');  
             ...
             $sql="select a.id, a.user_name, a.department_id,b.sequence_no, a.user_full_name, a.designation from  user_passwd a,electronic_approval_setup b where a.id=b.user_id and b.company_id=$cbo_company_name and b.entry_form=87 and valid=1 and a.id!=$user_id   and a.is_deleted=0 AND b.is_deleted = 0 group by a.id,a.user_name,a.department_id,b.sequence_no,a.user_full_name,a.designation order by b.sequence_no ";
               // echo $sql;die;
...
if ($action=="show_work_order_aop_report")
{
	...
	$com_supplier_id=return_field_value( "supplier_id as supplier_id", "air_way_bill_entry_mst","booking_no=$txt_booking_no","supplier_id");
	$nameArray_approved=sql_select( "select max(b.approved_no) as approved_no from air_way_bill_entry_mst a, approval_history b where a.id=b.mst_id and booking_no=$txt_booking_no and b.entry_form=162"); 
	...
	<div style="width:1540px; margin: 0 auto" align="center">       
       ...
       <?
		...
		$fabric_source='';
		$booking_info=sql_select( "select id,buyer_id,booking_no,booking_date,supplier_id,exchange_rate,currency_id,attention,delivery_date,fabric_source,entry_form from air_way_bill_entry_mst where booking_no=$txt_booking_no"); 
	...
	if($db_type==0)
	{
		$sql= sql_select("select a.booking_no, a.booking_type, a.booking_date, a.delivery_date, a.buyer_id, a.supplier_id, a.job_no, a.item_category, a.source, a.attention, a.process, b.dia_width, b.uom, a.pay_mode, a.exchange_rate, a.currency_id, b.wo_qnty, b.amount, b.pre_cost_remarks FROM air_way_bill_entry_mst a, wo_booking_dtls b  WHERE a.booking_no=$txt_booking_no and a.booking_no=b.booking_no and a.status_active=1 and a.is_deleted=0 order by a.id"); 
	}
	if($db_type==2)
	{
		$sql= sql_select("select a.booking_no, a.booking_type, a.booking_date, a.delivery_date, a.buyer_id, a.supplier_id, a.job_no, a.item_category, a.source, a.attention, a.process, b.dia_width, b.uom, a.pay_mode, a.exchange_rate, a.currency_id, b.wo_qnty, b.amount, b.pre_cost_remarks FROM air_way_bill_entry_mst a, wo_booking_dtls b  WHERE a.booking_no=$txt_booking_no and a.booking_no=b.booking_no and a.status_active=1 and a.is_deleted=0 order by a.id"); 
            ...
            <tr>
                <td width="100%">
                    ...
                    <?
 					...
 					$sel=sql_select("select a.approved_by, a.approved_date, a.id as app_no from approval_history a, electronic_approval_setup b where a.mst_id in(select id from air_way_bill_entry_mst where id=$bookingId) and a.entry_form=162 and a.approved_by=b.user_id and b.company_id=$cbo_company_name and b.is_deleted=0 group by a.approved_by, a.approved_date, a.id, b.sequence_no order by b.sequence_no asc");
