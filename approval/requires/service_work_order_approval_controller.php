...
{  
    ...
    $system_id_cond="";
	if($txt_system_id!="") $system_id_cond=" and a.wo_number_prefix_num=$txt_system_id";
	
	$date_cond='';
    if($txt_date_from !="" && $txt_date_to!="") $date_cond=" and a.wo_date between '".$txt_date_from."' and '".$txt_date_to."'";
	...
	if($previous_approved==1 && $approval_type==1)	//approval process with prevous approve start
	{
		...
		$sql="SELECT a.id, a.wo_number_prefix_num, a.wo_number, a.company_name, $select_year(a.insert_date $year_format) as year, a.wo_date, c.id as approval_id, a.is_approved 
		from wo_non_order_info_mst a, approval_history c 
		where a.id=c.mst_id and a.entry_form=484 and c.entry_form=60 and a.company_name=$cbo_company_name and a.is_deleted=0 and a.status_active=1 and a.is_approved=0 and a.ready_to_approved=1 $system_id_cond $date_cond 
		group by a.id, a.wo_number_prefix_num, a.wo_number, a.company_name, a.insert_date, a.wo_date, c.id, a.is_approved
       	order by a.id";
		//echo "$sql";
	}
	else if($approval_type==0) 
	{
		...
		{
            $sql="SELECT a.id, a.wo_number_prefix_num, a.wo_number, a.company_name, $select_year(a.insert_date $year_format) as year, a.wo_date, '0' AS approval_id, a.is_approved 
            from wo_non_order_info_mst a
            where a.company_name=$cbo_company_name and a.entry_form=484 and a.is_deleted=0 and a.status_active=1 and a.is_approved=0 and a.ready_to_approved=1 $system_id_cond $date_cond 
            group by a.id, a.wo_number_prefix_num, a.wo_number, a.company_name, a.insert_date, a.wo_date, a.is_approved
            order by a.id";		 	
		 	//echo $sql;die;
		}
		else // Next user
		{
			...
			if($sequence_no=="") // bypass if previous user Yes
			{
                ...
                $work_order_id=return_field_value("LISTAGG(b.mst_id, ',') WITHIN GROUP (ORDER BY b.mst_id) as work_order_id","wo_non_order_info_mst a, approval_history b","a.id=b.mst_id and a.entry_form=484 and a.company_name=$cbo_company_name and b.sequence_no in ($sequence_no_by) and a.ready_to_approved=1 and b.entry_form=60 and b.current_approval_status=1 and a.is_approved in (3,1)","work_order_id");

                $work_order_id=implode(",",array_unique(explode(",",$work_order_id)));
                
                $work_order_id_app_byuser=return_field_value("LISTAGG(b.mst_id, ',') WITHIN GROUP (ORDER BY b.mst_id) as work_order_id","wo_non_order_info_mst a, approval_history b","a.id=b.mst_id and a.entry_form=484 and a.company_name=$cbo_company_name and a.ready_to_approved=1  and b.sequence_no=$user_sequence_no  and b.entry_form=60 and b.current_approval_status=1 and a.is_approved in (3,1)","work_order_id");
				...
				if($work_order_id!="")
				{
					$sql=" SELECT x.* from  (SELECT a.id, a.wo_number_prefix_num,a.ready_to_approved, a.wo_number, a.company_name, $select_year(a.insert_date $year_format) as year, a.wo_date, '0' AS approval_id, a.is_approved  
					from wo_non_order_info_mst a
					where a.company_name=$cbo_company_name and a.entry_form=484 and a.ready_to_approved=1 and  a.is_deleted=0 and a.status_active=1 and a.is_approved in(0,3) and a.id in ($work_order_id) $system_id_cond $date_cond    
					GROUP by a.id, a.wo_number_prefix_num,a.ready_to_approved, a.wo_number, a.company_name, a.insert_date, a.wo_date, a.is_approved 

					UNION ALL

					SELECT a.id, a.wo_number_prefix_num,a.ready_to_approved, a.wo_number, a.company_name, $select_year(a.insert_date $year_format) as year, a.wo_date, '0' AS approval_id, a.is_approved
					from wo_non_order_info_mst a
					where a.id not in ($work_order_id) and a.entry_form=484 and a.ready_to_approved=1 and a.is_approved=$approval_type and a.company_name=$cbo_company_name  and a.is_deleted=0 and a.status_active=1 $system_id_cond $date_cond
					GROUP by a.id, a.wo_number_prefix_num,a.ready_to_approved, a.wo_number, a.company_name, a.insert_date, a.wo_date, a.is_approved  ) x  order by x.id";
					//echo $sql;
				}
				else
				{ 
					$sql=" SELECT a.id, a.wo_number_prefix_num, a.wo_number, a.company_name, $select_year(a.insert_date $year_format) as year, a.wo_date, '0' AS approval_id, a.is_approved
					from wo_non_order_info_mst a 
					where a.is_approved=$approval_type and a.entry_form=484 and a.company_name=$cbo_company_name and a.is_deleted=0 and a.status_active=1 $system_id_cond $date_cond
					GROUP by a.id, a.wo_number_prefix_num, a.wo_number, a.company_name, a.insert_date, a.wo_date, a.is_approved 
					order by a.id";
					//echo $sql;
				}
			...
			else // if previous user bypass No 
			{
				...
				$sql=" SELECT a.id, a.wo_number_prefix_num, a.wo_number, a.company_name, $select_year(a.insert_date $year_format) as year, a.wo_date, c.id AS approval_id, a.is_approved
				from wo_non_order_info_mst a, approval_history c 
				where a.is_approved in (1,3) and a.id=c.mst_id and a.entry_form=484 and c.entry_form in(60) and a.company_name=$cbo_company_name and a.is_deleted=0 and a.status_active=1 and c.current_approval_status=1 $sequence_no_cond  $system_id_cond $date_cond
				GROUP by a.id, a.wo_number_prefix_num, a.wo_number, a.company_name, a.insert_date, a.wo_date, c.id, a.is_approved
				order by a.id";
				//echo $sql;
			}
	...
	else
	{
		...
		$sql=" SELECT a.id, a.wo_number_prefix_num,a.ready_to_approved, a.wo_number, a.company_name, $select_year(a.insert_date $year_format) as year, a.wo_date, c.id AS approval_id, a.is_approved
		from wo_non_order_info_mst a, approval_history c 
		where a.is_approved in (1,3) and a.id=c.mst_id and a.entry_form in(484) and a.ready_to_approved=1 and c.entry_form in(60) and a.company_name=$cbo_company_name and a.is_deleted=0 and a.status_active=1 and c.current_approval_status=1 $sequence_no_cond $system_id_cond $date_cond
		GROUP by a.id, a.wo_number_prefix_num,a.ready_to_approved, a.wo_number, a.company_name, a.insert_date, a.wo_date, c.id, a.is_approved
		order by a.id";
                        ...
                        <?						
                            ...
                            foreach ($nameArray as $row)
                            {
								...
								<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" align="center">
                                    ...
                                    <td width="120" align="center"><a href='##' onClick="generate_report('<?=$row[csf('company_name')];?>','<?=$row[csf('id')];?>','Service Work Order','<?=$format_ids[0]?>')"><b><? echo $row[csf('wo_number')]; ?></b></a></td>
...
if ($action=="approve")
{
	...
	if($approval_type==0)
	{
        ...
        $max_approved_no_arr = return_library_array("SELECT mst_id, max(approved_no) as approved_no from approval_history where mst_id in($workOrder_ids) and entry_form=60 group by mst_id","mst_id","approved_no");
		...
		for($i=0;$i<count($workOrder_ids_all);$i++)
		{
			...
			$data_array.="(".$id.",60,".$workOrder_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."',".$user_id.",'".$pc_date_time."')"; 
    	...
    	$rID=sql_multirow_update("wo_non_order_info_mst","is_approved",$partial_approval,"id",$workOrder_ids,1);
	...
	else
	{
		...
		$rID=sql_multirow_update("wo_non_order_info_mst","is_approved*ready_to_approved","0*0","id",$workOrder_ids,1);
		if($rID) $flag=1; else $flag=0;
