...
if($action=="report_generate")
{
	...
	if($previous_approved==1 && $approval_type==1)
	{
		...
		$sql="SELECT a.id,  a.company_id, a.buyer_id, a.style_ref, a.style_desc, a.quot_date,a.est_ship_date, a.approved, a.inserted_by, a.mkt_no, b.id as approval_id, a.garments_nature
		from wo_price_quotation a, approval_history b
		where a.id=b.mst_id and b.entry_form=10 and a.company_id=$company_name and a.status_active=1 and a.is_deleted=0 and a.ready_to_approved=1 and b.current_approval_status=1 and a.approved in(1,3) and b.approved_by!=$user_id $buyer_id_cond2 $sequence_no_cond $date_cond $quotation_cond $mkt_no_cond
		order by a.id ASC";
		//$buyer_id_cond
	...
	else if($approval_type==0)
	{
		...
		else
		{
			...
			$sql="SELECT a.id,  a.company_id,  a.buyer_id,  a.style_ref, a.style_desc, a.quot_date,a.est_ship_date, a.approved,a.inserted_by,b.approved_date, b.id as approval_id, a.garments_nature, a.mkt_no from wo_price_quotation a, approval_history b where a.id=b.mst_id and b.entry_form=10 and a.company_id=$company_name  and a.status_active=1 and a.is_deleted=0 and b.current_approval_status=1 and a.ready_to_approved=1 and  a.approved in(1,3) $buyer_id_cond $buyer_id_cond2 $sequence_no_cond $date_cond $quotation_cond $mkt_no_cond group by a.id,  a.company_id,  a.buyer_id,  a.style_ref, a.style_desc, a.quot_date,a.est_ship_date, a.approved,a.inserted_by,b.approved_date, b.id, a.garments_nature, a.mkt_no order by a.id ASC";
		}
		//echo $sql;
	}
	else
	{
		...
		{
			$sql="SELECT a.id,  a.company_id, a.buyer_id, a.style_ref, a.style_desc, a.quot_date,a.est_ship_date, a.approved, a.inserted_by,b.approved_date, b.id as approval_id, a.garments_nature, a.mkt_no from wo_price_quotation a, approval_history b, fabric_booking_approval_cause c where a.id=b.mst_id and a.id=c.booking_id and b.entry_form=10 and a.company_id=$company_name and a.status_active=1 and a.is_deleted=0 and a.ready_to_approved=1 and b.current_approval_status=1 and a.approved in(1,3) and c.entry_form=10 and c.approval_type=2 and c.is_deleted=0 and c.status_active=1 $buyer_id_cond $buyer_id_cond2 $sequence_no_cond $date_cond $quotation_cond $mkt_no_cond order by a.id ASC";
		}
		else
		{
			$sql="SELECT a.id,  a.company_id, a.buyer_id, a.style_ref, a.style_desc, a.quot_date,a.est_ship_date, a.approved, a.inserted_by,b.approved_date, b.id as approval_id, a.garments_nature, a.mkt_no from wo_price_quotation a, approval_history b where a.id=b.mst_id and b.entry_form=10 and a.company_id=$company_name and a.status_active=1 and a.is_deleted=0 and a.ready_to_approved=1 and b.current_approval_status=1 and a.approved in(1,3) $buyer_id_cond $buyer_id_cond2 $sequence_no_cond $date_cond $quotation_cond $mkt_no_cond order by a.id ASC";
		}
	...
	$tbl_width = ($approval_type==1) ? 1110 : 860;
	?>
                        ...
                        <?
                            ...
                            foreach ($nameArray as $row)
                            {
								...
								<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" align="center">
                                    ...
                                    <? }else{ ?>
                                    <td> <input style="width:80px;" type="text" class="text_boxes"  name="txtCause_<? echo $row[csf('id')];?>" id="txtCause_<? echo $row[csf('id')];?>" placeholder="browse" onClick="openmypage_refusing_cause('requires/price_quatation_approval_controller_iphone.php?action=refusing_cause_popup','Refusing Cause','<? echo $row[csf('id')];?>');" value="<? echo $row[csf('refusing_cause')];?>"/></td>
...
if($action=="refusing_cause_popup")
{
	...
	function fnc_cause_info( operation )
	{
		...
		else
		{
			...
			http.open("POST","price_quatation_approval_controller_iphone.php",true);
