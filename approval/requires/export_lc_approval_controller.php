...
if($action=="report_generate")
{
	...
	if($txt_system_id != '') {$searchCon .=" and a.export_lc_system_id='$txt_system_id'";}
	if($txt_lc_no != ''){$searchCon .=" and a.export_lc_no='$txt_lc_no'";}
	...
	if ($txt_date_from != '' && $txt_date_to != '')
	{
		if($db_type==0)
		{
			...
			$searchCon .= " and a.lc_date between '$txt_date_from' and '$txt_date_to'";
		}
		else
		{
	...
	{		
		...
		$data_mas_sql = "select a.id as ID, a.buyer_name as BUYER_NAME from com_export_lc a where a.status_active=1 and a.is_deleted=0 and a.approved<>1 and a.ready_to_approved=1 and a.beneficiary_name=$company_name $where_con $searchCon";
		//echo $data_mas_sql; 
		...
		{			
			...
			if($tmp_sys_id_arr[$seq])
			{
				...
				$sql .= " select a.id as ID, a.export_lc_system_id as SYSTEM_ID, a.export_lc_no as EXPORT_LC_NO, a.lc_date as LC_DATE, a.beneficiary_name as COMPANY_NAME, a.buyer_name as BUYER_NAME, a.lien_bank as LIEN_BANK, a.lc_value as LC_VALUE, a.internal_file_no as INTERNAL_FILE_NO, a.pay_term as PAY_TERM, a.inserted_by as INSERTED_BY, a.last_shipment_date as LAST_SHIPMENT_DATE, a.expiry_date as EXPIRY_DATE from com_export_lc a where a.status_active=1 and a.is_deleted=0 and a.approved<>1 and a.ready_to_approved=1 and a.approved_sequ_by=$seq $sys_con and a.beneficiary_name=$company_name $searchCon group by a.id, a.export_lc_system_id, a.export_lc_no, a.lc_date, a.beneficiary_name, a.buyer_name, a.lien_bank, a.lc_value, a.internal_file_no, a.pay_term, a.inserted_by, a.last_shipment_date, a.expiry_date";			
			}		
		}		
	}
	else
	{		
		$sql = " select a.id as ID, a.export_lc_system_id as SYSTEM_ID, a.export_lc_no as EXPORT_LC_NO, a.lc_date as LC_DATE, a.beneficiary_name as COMPANY_NAME, a.buyer_name as BUYER_NAME, a.lien_bank as LIEN_BANK, a.lc_value as LC_VALUE, a.internal_file_no as INTERNAL_FILE_NO, a.pay_term as PAY_TERM, a.inserted_by as INSERTED_BY, a.last_shipment_date as LAST_SHIPMENT_DATE, a.expiry_date as EXPIRY_DATE from com_export_lc a, approval_mst c where a.id=c.mst_id and a.status_active=1 and a.is_deleted=0 and a.ready_to_approved=1 and a.beneficiary_name=$company_name and c.sequence_no={$electronicDataArr[user_by][$user_id][SEQUENCE_NO]} and a.approved_sequ_by=c.sequence_no $searchCon group by a.id, a.export_lc_system_id, a.export_lc_no, a.lc_date, a.beneficiary_name, a.buyer_name, a.lien_bank, a.lc_value, a.internal_file_no, a.pay_term, a.inserted_by, a.last_shipment_date, a.expiry_date order by a.id";		
    }
	//echo $sql;die;
	...
	$hostory_sql=sql_select( "select mst_id as MST_ID, approved_by as APPROVED_BY, approved_date as APPROVED_DATE from approval_history where current_approval_status=1 and entry_form=62 and mst_id in (".implode(',',$mst_id_arr).")");
	...
	$report_id = 0;
	$print_report_format = return_field_value("format_id", "lib_report_template", "template_name ='".$company_name."' and module_id=5 and report_id=264 and is_deleted=0 and status_active=1");
            ...
            <div style="width:<?= $width+20; ?>px; overflow-y:scroll; max-height:330px; float:left;" id="buyer_list_view">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="<?= $width; ?>" class="rpt_table" id="tbl_list_search" align="left">
                    <tbody>
						...
						$img_val =  return_field_value("master_tble_id","common_photo_library","form_name='Export LC Entry'","master_tble_id");//master_tble_id='$value' and 
                        foreach ($nameArray as $row)
                        {                                               
							...
							{
								$app_id = sql_select("select id from approval_history where mst_id='".$row[ID]."' and entry_form='62'  order by id desc");									
							...
							<tr bgcolor="<?= $bgcolor; ?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor; ?>')" id="tr_<?= $i; ?>"> 
								...
								<td width="120" onClick="export_lc_approval_report('<?= $row['ID'];?>','<?= $row['SYSTEM_ID'];?>','<?= $company_name;?>','<?= $report_id;?>');"><a href="javascript:void(0)"><?= $row['SYSTEM_ID']; ?></a></td>
								<td width="120"><p><?= $row['EXPORT_LC_NO']; ?></p></td>
								...
								<td width="100" align="center"><p><?= change_date_format($row['LC_DATE']); ?></p></td>
								...
								<td width="100" align="right"><p><?= $row['LC_VALUE']; ?></p></td>
...
if ($action=="approve")
{
	...
	{      
		...
		//------------------
		$sql="select a.id as ID, a.buyer_name as BUYER_NAME from com_export_lc a where a.id in($target_ids)";
		...
		foreach($target_app_id_arr as $mst_id)
		{
			...
			$mst_data_array.="(".$id.",62,".$mst_id.",".$user_sequence_no.",".$user_id.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')";
			$id=$id+1;
        ...
        {
            $approved_no=return_field_value("max(approved_no) as approved_no","approval_history","mst_id='$val' and entry_form=62","approved_no");
            $approved_no=$approved_no+1;
        
            if($i!=0) $data_array.=",";             
            $data_array.="(".$id.",62,".$val.",".$approved_no.",'".$user_sequence_no."',1,".$user_id.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1)";
		...
		if($flag==1) 
		{
			$rID1=execute_query(bulk_update_sql_statement( "com_export_lc", "id", $mst_field_array_up, $mst_data_array_up, $target_app_id_arr ));
			if($rID1) $flag=1; else $flag=0;
		}
		
	    $query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=62 and mst_id in ($target_ids)"; //die;       
        if($flag==1) 
        {
	...
	{              
		$history_data_arr=return_library_array( "select id, id from approval_history where current_approval_status=1 and entry_form=62 and mst_id in ($target_ids)",'id','id');
		...
		$flag=1;
		$rID=sql_multirow_update("com_export_lc","approved*ready_to_approved*approved_sequ_by","0*2*0","id",$target_ids,0);
		if($flag==1)
		{
			if($rID) $flag=1; else $flag=0;
		}

		$query="delete from approval_mst WHERE entry_form=62 and mst_id in ($target_ids)";		
		if($flag==1) 
		{
			$rID2=execute_query($query,1);
			if($rID2) $flag=1; else $flag=0; 
		...
		$query="UPDATE approval_history SET current_approval_status=0,IS_SIGNING=0 WHERE entry_form=62 and mst_id in ($target_ids)";	
		if($flag==1) 
		{
...
if($action=="get_user_pi_file")
{
    ...
    $img_sql = "SELECT id,image_location,master_tble_id,real_file_name,FILE_TYPE from common_photo_library where form_name='Export LC Entry' and master_tble_id='$id'";

	// echo "SELECT id,image_location,master_tble_id,real_file_name,FILE_TYPE from common_photo_library where form_name='Export LC Entry' and master_tble_id='$id'";die;
