...
if($action=="report_generate")
{
	...
	if($previous_approved==1 && $approval_type==1)
	{
		...
		$sql="select a.id,  a.company_id, a.buyer_id, a.style_ref, a.style_desc, a.quot_date,a.est_ship_date, a.approved, a.inserted_by, b.id as approval_id from wo_price_quotation a, approval_history b where a.id=b.mst_id and b.entry_form=10 and a.company_id=$company_name and a.status_active=1 and a.is_deleted=0 and a.ready_to_approved=1 and b.current_approval_status=1 and a.approved in(1,3) and b.approved_by!=$user_id $buyer_id_cond2 $sequence_no_cond $date_cond";
		//$buyer_id_cond
	...
	else if($approval_type==0)
	{
		...
		else if($sequence_no=="")
		{
			...
			$sql="select a.id,  a.company_id,  a.buyer_id, a.style_ref, a.style_desc, a.quot_date,a.est_ship_date, '0' as approval_id,  a.approved , a.inserted_by from wo_price_quotation a, wo_price_quotation_costing_mst b where a.id=b.quotation_id and a.company_id=$company_name  and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 and a.approved=$approval_type $buyer_id_cond $buyer_id_cond2  $quotation_id_cond $date_cond  group by a.id, a.company_id, a.buyer_id, a.style_ref, a.style_desc, a.quot_date,a.est_ship_date, a.approved , a.inserted_by";
		}
		else
		{
			...
			$sql="select a.id,  a.company_id,  a.buyer_id,  a.style_ref, a.style_desc, a.quot_date,a.est_ship_date, a.approved,a.inserted_by, b.id as approval_id from wo_price_quotation a, approval_history b where a.id=b.mst_id and b.entry_form=10 and a.company_id=$company_name  and a.status_active=1 and a.is_deleted=0 and current_approval_status=1 and a.ready_to_approved=1 and  a.approved in(1,3) $buyer_id_cond $buyer_id_cond2 $sequence_no_cond $date_cond";
		}
	}
	else
	{
		...
		$sequence_no_cond=" and b.sequence_no='$user_sequence_no'";
		$sql="select a.id,  a.company_id, a.buyer_id, a.style_ref, a.style_desc, a.quot_date,a.est_ship_date, a.approved, a.inserted_by, b.id as approval_id from wo_price_quotation a, approval_history b where a.id=b.mst_id and b.entry_form=10 and a.company_id=$company_name and a.status_active=1 and a.is_deleted=0 and a.ready_to_approved=1 and b.current_approval_status=1 and a.approved in(1,3) $buyer_id_cond $buyer_id_cond2 $sequence_no_cond $date_cond";
	}
                        ...
                        <?
                            ...
                            foreach ($nameArray as $row)
                            {
								...
								<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" align="center"> 
									...
									?>
									<td width="130">
                                    	<p><a href='##' style='color:#000' onclick="generate_worder_report('<? echo $quotation_rep; ?>',<? echo $row[csf('id')]; ?>,<? echo $row[csf('company_id')]; ?>,<? echo $row[csf('buyer_id')]; ?>,'<? echo $row[csf('style_ref')];?>','<? echo $row[csf('quot_date')]; ?>')"><? echo $row[csf('id')]; ?></a></p>
                                    </td>
