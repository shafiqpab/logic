...
{ 
	...
	if($company_name>0){
		...
		$app_company_arr=return_library_array( "select company_id, company_id from electronic_approval_setup where page_id=$menu_id and entry_form=47 and user_id=$user_id and is_deleted=0",'company_id','company_id');
	}
    ...
    <legend>Sourcing Post Cost Approval</legend>
		...
		foreach($app_company_arr as $company_name){
			...
			$electronicDataArr=getSequence(array('company_id'=>$company_name,'ENTRY_FORM'=>47,'user_id'=>$user_id_approval,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'product_dept_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0));
			...
			$higher_authority_confirmation = return_field_value("IS_REQUIRED","VARIABLE_APPROVAL_SETTINGS","VARIABLE_LIST=2 and COMPANY_NAME=$company_name and is_deleted=0 and status_active=1");
			if($higher_authority_confirmation == 1){$where_con .= " and a.SOURCING_HA_APPROVED=1";}
			...
			{  
				...
				a.ID,a.SOURCING_READY_TO_APPROVED,a.SOURCING_APPROVED,b.BUYER_NAME,b.COMPANY_NAME
				from wo_pre_cost_mst a,wo_po_details_master b where b.id=a.job_id and a.status_active=1 and a.is_deleted=0 and a.sourcing_ready_to_approved=1 and b.status_active=1 and b.is_deleted=0 and a.SOURCING_APPROVED<>1 and b.COMPANY_NAME=$company_name $where_con";
		 ...
		 //echo $sql;die();
			...
			$sql_unapproved=sql_select("select * from fabric_booking_app_cause_source where  entry_form=47 and VALID=1 and approval_type=2 and is_deleted=0 and status_active=1");
						...
						foreach ($nameArray as $row)
						{
							...
							if($print_cond==1)
							{
								...
								<tr bgcolor="<?=$bgcolor; ?>" onClick="change_color('tr_<?=$i; ?>','<?=$bgcolor; ?>');" id="tr_<?=$i; ?>" align="center">
									...
									<td> <input style="width:150px;" type="text" class="text_boxes"  name="txtCause_<?=$i; ?>" id="txtCause_<?=$i; ?>" placeholder="Browse" onClick="openmypage_refusing_cause('requires/sourcing_approval_v2_controller.php?action=refusing_cause_popup','Refusing Cause','<?=$row[csf('id')]; ?>','<?=$row[csf('sourcinng_refusing_cause')]; ?>');" value="<?=$row[csf('sourcinng_refusing_cause')]; ?>"/></td>
								</tr>
								<?
								$i++;
							}
...
{ 
	...
	foreach($appCompanyArr as $cbo_company_name){
		...
		//............................................................................
		$higher_authority_confirmation = return_field_value("IS_REQUIRED","VARIABLE_APPROVAL_SETTINGS","VARIABLE_LIST=2 and COMPANY_NAME=$cbo_company_name and is_deleted=0 and status_active=1");
			if($higher_authority_confirmation == 1){$where_con = " and a.SOURCING_HA_APPROVED=1";}
		
		$sql = "select a.ID,a.SOURCING_APPROVED,b.BUYER_NAME,a.SOURCING_READY_TO_APPROVED  from wo_pre_cost_mst a, wo_po_details_master b  where a.job_no=b.job_no and  b.COMPANY_NAME=$cbo_company_name  and a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0 and a.SOURCING_READY_TO_APPROVED=1 $where_con and a.id in($booking_ids)";
		//echo $sql;die();
		$sqlResult=sql_select( $sql );
		foreach ($sqlResult as $row)
		{
			$matchDataArr[$row['ID']]=array('buyer_id'=>$row['BUYER_NAME'],'brand_id'=>0,'store'=>0);
			$approve_status_arr[$row['ID']] = $row['SOURCING_APPROVED'];
		}
		
		$finalDataArr=getFinalUser(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>47,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0,'product_dept_arr'=>0,'match_data'=>$matchDataArr));
	...
	{ 
        ...
        foreach($target_app_id_arr as $mst_id)
        {
			...
			$data_array.="(".$id.",47,'".$mst_id."','".$user_sequence_no."',".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			...
			$history_data_array.="(".$ahid.",47,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1)";
			$ahid++;
		...
		if($flag==1) 
		{
			$field_array_up="SOURCING_APPROVED*APPROVED_SEQU_BY_SOURCE*APPROVED_DATE_SOURCE*APPROVED_BY_SOURCE"; 
		...
		if(count($approved_no_array)>0)
		{
				...
				$sqljob="insert into wo_po_dtls_mst_his (id, job_id, approved_no, approval_page, garments_nature, job_no_prefix, job_no_prefix_num, job_no, quotation_id, order_repeat_no, company_name, buyer_name, style_ref_no, product_dept, product_code, location_name, style_description, ship_mode, region, team_leader, dealing_marchant, remarks, job_quantity, avg_unit_price, currency_id, total_price, packing, agent_name, product_category, order_uom, gmts_item_id, set_break_down, total_set_qnty, set_smv, is_deleted, status_active, inserted_by, insert_date, updated_by, update_date, pro_sub_dep, client_id, item_number_id, factory_marchant, qlty_label, is_excel, style_owner, booking_meeting_date, bh_merchant, copy_from, season_buyer_wise, is_repeat, repeat_job_no, ready_for_budget, working_location_id, gauge, fabric_composition, design_source_id, yarn_quality, season_year, brand_id, inquiry_id, body_wash_color, sustainability_standard, fab_material, quality_level, requisition_no, working_company_id, fit_id)
					select '', id, $approved_string_dtls3, 47, garments_nature, job_no_prefix, job_no_prefix_num, job_no, quotation_id, order_repeat_no, company_name, buyer_name, style_ref_no, product_dept, product_code, location_name, style_description, ship_mode, region, team_leader, dealing_marchant, remarks, job_quantity, avg_unit_price, currency_id, total_price, packing, agent_name, product_category, order_uom, gmts_item_id, set_break_down, total_set_qnty, set_smv, is_deleted, status_active, inserted_by, insert_date, updated_by, update_date, pro_sub_dep, client_id, item_number_id, factory_marchant, qlty_label, is_excel, style_owner, booking_meeting_date, bh_merchant, copy_from, season_buyer_wise, is_repeat, repeat_job_no, ready_for_budget, working_location_id, gauge, fabric_composition, design_source_id, yarn_quality, season_year, brand_id, inquiry_id, body_wash_color, sustainability_standard, fab_material, quality_level, requisition_no, working_company_id, fit_id
				...
				$sql_fabric_cost_dtls="insert into wo_pre_cost_fabric_cost_dtls_h(id, approved_no, pre_cost_fabric_cost_dtls_id, job_no, item_number_id, body_part_id, fab_nature_id, color_type_id, lib_yarn_count_deter_id, construction, composition, fabric_description, gsm_weight, color_size_sensitive, color, avg_cons, fabric_source, rate, amount, avg_finish_cons, avg_process_loss, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, company_id, costing_per, consumption_basis, process_loss_method, cons_breack_down, msmnt_break_down, color_break_down, yarn_breack_down, marker_break_down, width_dia_type, avg_cons_yarn, gsm_weight_yarn, uom, body_part_type, sample_id, job_id, gsm_weight_type, nominated_supp_multi, sourcing_rate, sourcing_amount, sourcing_inserted_by, sourcing_inserted_date, sourcing_updated_by, sourcing_update_date, sourcing_nominated_supp, quotdtlsid, budget_on, source_id, is_synchronized, approval_page)
					select '', $approved_string_dtls, id, job_no, item_number_id, body_part_id, fab_nature_id, color_type_id, lib_yarn_count_deter_id, construction, composition, fabric_description, gsm_weight, color_size_sensitive, color, avg_cons, fabric_source, rate, amount, avg_finish_cons, avg_process_loss, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, company_id, costing_per, consumption_basis, process_loss_method, cons_breack_down, msmnt_break_down, color_break_down, yarn_breack_down, marker_break_down, width_dia_type, avg_cons_yarn, gsm_weight_yarn, uom, body_part_type, sample_id, job_id, gsm_weight_type, nominated_supp_multi, sourcing_rate, sourcing_amount, sourcing_inserted_by, sourcing_inserted_date, sourcing_updated_by, sourcing_update_date, sourcing_nominated_supp, quotdtlsid, budget_on, source_id, is_synchronized, 47 from wo_pre_cost_fabric_cost_dtls where job_id in ($job_ids)";
				...
				$sql_fabric_cons_dtls="insert into wo_pre_fab_avg_con_dtls_h(id, approved_no, fab_con_id, pre_cost_fabric_cost_dtls_id, job_no, po_break_down_id, color_number_id, gmts_sizes, dia_width, item_size, cons, process_loss_percent, requirment, pcs, color_size_table_id, body_length, body_sewing_margin, body_hem_margin, sleeve_length, sleeve_sewing_margin, sleeve_hem_margin, half_chest_length, half_chest_sewing_margin, front_rise_length, front_rise_sewing_margin, west_band_length, west_band_sewing_margin, in_seam_length, in_seam_sewing_margin, in_seam_hem_margin, half_thai_length, half_thai_sewing_margin, total, marker_dia, marker_yds, marker_inch, gmts_pcs, marker_length, net_fab_cons, rate, amount, remarks, length, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, length_sleeve, width_sleeve, job_id, fina_char, sourcing_rate, sourcing_amount, sourcing_inserted_by, sourcing_inserted_date, sourcing_updated_by, sourcing_update_date, cons_pcs, item_color, approval_page)
					select '', $approved_string_dtls, id,  pre_cost_fabric_cost_dtls_id, job_no, po_break_down_id, color_number_id, gmts_sizes, dia_width, item_size, cons, process_loss_percent, requirment, pcs, color_size_table_id, body_length, body_sewing_margin, body_hem_margin, sleeve_length, sleeve_sewing_margin, sleeve_hem_margin, half_chest_length, half_chest_sewing_margin, front_rise_length, front_rise_sewing_margin, west_band_length, west_band_sewing_margin, in_seam_length, in_seam_sewing_margin, in_seam_hem_margin, half_thai_length, half_thai_sewing_margin, total, marker_dia, marker_yds, marker_inch, gmts_pcs, marker_length, net_fab_cons, rate, amount, remarks, length, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, length_sleeve, width_sleeve, job_id, fina_char, sourcing_rate, sourcing_amount, sourcing_inserted_by, sourcing_inserted_date, sourcing_updated_by, sourcing_update_date, cons_pcs, item_color, 47 from wo_pre_cos_fab_co_avg_con_dtls where job_id in ($job_ids)";
				...
				$sql_concolor_cst="insert into wo_pre_fab_concolor_dtls_h (id, approved_no, contrast_id, pre_cost_fabric_cost_dtls_id, job_no, gmts_color_id, gmts_color, contrast_color_id, job_id, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, approval_page)
					select
					'', $approved_string_dtls, id, pre_cost_fabric_cost_dtls_id, job_no, gmts_color_id, gmts_color, contrast_color_id, job_id, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, 47 from wo_pre_cos_fab_co_color_dtls where job_id in ($job_ids)";
				...
				$sql_stripecolor_cst="insert into wo_pre_stripe_color_h (id, approved_no, stripe_id, job_no, item_number_id, pre_cost_fabric_cost_dtls_id, color_number_id, stripe_color, measurement, uom, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, totfidder, fabreq, fabreqtotkg, yarn_dyed, sales_dtls_id, size_number_id, po_break_down_id, lenth, width, sample_color, sample_per, cons, excess_per, job_id, stripe_type, approval_page)
					select
					'', $approved_string_dtls, id, job_no, item_number_id, pre_cost_fabric_cost_dtls_id, color_number_id, stripe_color, measurement, uom, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, totfidder, fabreq, fabreqtotkg, yarn_dyed, sales_dtls_id, size_number_id, po_break_down_id, lenth, width, sample_color, sample_per, cons, excess_per, job_id, stripe_type, 47 from wo_pre_stripe_color where job_id in ($job_ids)";
				...
				$sql_precost_fab_con_cst_dtls="insert into  wo_pre_cost_fab_con_cst_dtls_h(id, approved_no, pre_cost_fab_conv_cst_dtls_id, job_no, fabric_description, cons_process, req_qnty, charge_unit, amount, color_break_down, charge_lib_id, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, avg_req_qnty, process_loss, job_id, approval_page)
					select '', $approved_string_dtls, id, job_no, fabric_description, cons_process, req_qnty, charge_unit, amount, color_break_down, charge_lib_id, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, avg_req_qnty, process_loss, job_id, 47 from wo_pre_cost_fab_conv_cost_dtls where job_id in ($job_ids)";
				...
				$sql_conv_color_dtls="insert into wo_pre_conv_color_dtls_h(id, approved_no, conv_color_id, fabric_cost_dtls_id, conv_cost_dtls_id, job_no, job_id, convchargelibraryid, gmts_color_id, fabric_color_id, cons, unit_charge, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, approval_page)
					select '', $approved_string_dtls, id, fabric_cost_dtls_id, conv_cost_dtls_id, job_no, job_id, convchargelibraryid, gmts_color_id, fabric_color_id, cons, unit_charge, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, 47 from wo_pre_cos_conv_color_dtls where job_id in ($job_ids)";
				...
				$sql_embe_cons_dtls="insert into wo_pre_emb_avg_con_dtls_h(id, approved_no, emb_cons_id, pre_cost_emb_cost_dtls_id, job_no, po_break_down_id, item_number_id, color_number_id, size_number_id, requirment, rate, amount, gmts_pcs, color_size_table_id, rate_lib_id, country_id, job_id, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, sourcing_rate, sourcing_amount, sourcing_inserted_by, sourcing_inserted_date, sourcing_updated_by, sourcing_update_date, approval_page)
					select '', $approved_string_dtls, id, pre_cost_emb_cost_dtls_id, job_no, po_break_down_id, item_number_id, color_number_id, size_number_id, requirment, rate, amount, gmts_pcs, color_size_table_id, rate_lib_id, country_id, job_id, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, sourcing_rate, sourcing_amount, sourcing_inserted_by, sourcing_inserted_date, sourcing_updated_by, sourcing_update_date, 47 from wo_pre_cos_emb_co_avg_con_dtls where job_id in ($job_ids)";
				...
				$sql_comarc_cost_dtls="insert into wo_pre_cost_comarc_cost_dtls_h( id, approved_no, pre_cost_comarci_cost_dtls_id, job_no, item_id, rate, amount, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, job_id, approval_page)
					select '', $approved_string_dtls, id, job_no, item_id, rate, amount, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, job_id, 47 from wo_pre_cost_comarci_cost_dtls where job_id in ($job_ids)";
				...
				$sql_commis_cost_dtls="insert into wo_pre_cost_commis_cost_dtls_h (id, approved_no, pre_cost_commiss_cost_dtls_id, job_no, particulars_id, commission_base_id, commision_rate, commission_amount, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, job_id, approval_page)
					select '', $approved_string_dtls, id, job_no, particulars_id, commission_base_id, commision_rate, commission_amount, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, job_id, 47 from wo_pre_cost_commiss_cost_dtls where job_id in ($job_ids)";
	...
	{            
		...
		$next_user_app = sql_select("select id from approval_history where mst_id in($booking_ids) and entry_form=47 and sequence_no >$user_sequence_no and current_approval_status=1 group by id");
		...
		$rID1=sql_multirow_update("wo_pre_cost_mst","SOURCING_APPROVED*SOURCING_READY_TO_APPROVED*APPROVED_SEQU_BY_SOURCE",'0*0*0',"id",$booking_ids,0);
		if($rID1) $flag=1; else $flag=0;
		...
		if($flag==1) 
		{
			$query="delete from approval_mst  WHERE entry_form=47 and mst_id in ($booking_ids)";
			$rID3=execute_query($query,1); 
			if($rID3) $flag=1; else $flag=0; 
		}
		...
		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0, un_approved_by=".$user_id_approval.", un_approved_date='".$pc_date_time."', updated_by='".$user_id."', update_date='".$pc_date_time."' WHERE entry_form=47 and current_approval_status=1 and mst_id in ($booking_ids)";
		...
		if($flag==1)
		{
			$query="UPDATE fabric_booking_app_cause_source SET VALID=0 WHERE entry_form=47 and VALID=1 and approval_type=2 and BOOKING_ID in ($booking_ids)";
...
if($action=='user_popup')
{
	...
	<form>
       ...
       <?php
		...
		$sql="select a.id, a.user_name, a.department_id, a.user_full_name, a.designation,b.sequence_no from  user_passwd a,electronic_approval_setup b where a.id=b.user_id and b.company_id=$cbo_company_name and valid=1   and b.is_deleted=0  and  b.entry_form=47  order by b.sequence_no";
			// echo $sql;die;
...
if($action=="refusing_cause_popup")
{
	...
	function fnc_cause_info( operation )
	{
		...
		else
		{
			...
			http.open("POST","sourcing_approval_v2_controller.php",true);
...
if($action=="save_update_delete_refusing_cause")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$rID2=execute_query("update wo_pre_cost_mst set sourcing_ready_to_approved=0, sourcing_approved=0, sourcinng_refusing_cause='".$refusing_cause."',APPROVED_SEQU_BY_SOURCE=0, sourcing_updated_by=".$_SESSION['logic_erp']['user_id'].", sourcing_update_date = '".$pc_date_time."' where id='$quo_id'");
		...
		$query="delete from approval_mst  WHERE entry_form=47 and mst_id in ($quo_id)";
		$rID3=execute_query($query,1); 
		if($rID3) $flag=1; else $flag=0; 
