...
if($action=="report_generate")
{
	...
	if($previous_approved==1 && $approval_type==1)
	{
		...
		$sql="SELECT a.id, a.booking_no_prefix_num as prefix_num,a.booking_no, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.supplier_id, a.delivery_date, a.insert_date, a.booking_date, a.job_no, a.po_break_down_id, a.is_approved, b.id as approval_id, b.sequence_no, b.approved_by, a.is_apply_last_update,c.grouping, c.file_no, a.pay_mode
		...
		group by a.id, a.booking_no_prefix_num,a.booking_no, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.supplier_id, a.delivery_date, a.insert_date, a.booking_date, a.job_no, a.po_break_down_id, a.is_approved, b.id, b.sequence_no, b.approved_by, a.is_apply_last_update,c.grouping, c.file_no, a.pay_mode
	...
	else if($approval_type==0)
	{
		...
		else if($sequence_no=="")
		{
			...
			{
				$sql.=" union all
				select a.id, a.booking_no_prefix_num as prefix_num,a.booking_no, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.supplier_id, a.delivery_date, a.booking_date, '0' as approval_id, a.job_no, a.is_approved, a.po_break_down_id, a.is_apply_last_update,a.insert_date,c.grouping, c.file_no, a.pay_mode from wo_booking_mst a, wo_booking_dtls b,wo_po_break_down c where a.booking_no=b.booking_no and a.job_no=c.job_no_mst and b.po_break_down_id =c.id and a.company_id=$company_name and a.is_short=1 and a.booking_type=1 and a.item_category in(2,3,13) and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 and a.is_approved=3 and b.fin_fab_qnty>0 ".where_con_using_array(explode(',',$booking_id),0,'a.id')." $buyer_id_cond $buyer_id_cond2 $booking_no_cond $internal_ref_cond $file_no_cond $date_cond group by a.id, a.booking_no,a.booking_no_prefix_num, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.supplier_id, a.delivery_date, a.booking_date, a.job_no, a.is_approved, a.po_break_down_id, a.is_apply_last_update,a.insert_date,c.grouping, c.file_no, a.pay_mode ";
                    ...
                    <tbody>
						...
						{	
							...
							<tr bgcolor="<?=$bgcolor; ?>" onClick="change_color('tr_<?=$i; ?>','<?=$bgcolor; ?>');" id="tr_<?=$i; ?>"> 
								...
								<td width="130">
									<p><a href='##' style='color:#000' onClick="generate_worder_report11(<? echo $type; ?>,<? echo $report_type; ?>,'<? echo $row[csf('booking_no')]; ?>',<? echo $row[csf('company_id')]; ?>,'<? echo $row[csf('po_break_down_id')]; ?>',<? echo $row[csf('item_category')].','.$row[csf('fabric_source')]; ?>,'<? echo $row[csf('job_no')]; ?>','<? echo $row[csf('is_approved')]; ?>','show_fabric_booking_report_urmi',' <? echo $i; ?>')"><? echo $row[csf('prefix_num')]; ?></a></p>
								</td>
            ...
            <table cellspacing="0" cellpadding="0" border="0" rules="all" width="<? echo $table1; ?>" class="rpt_table">
				<tfoot>
                    <td width="50" align="center" style=" <?=$isApp; ?>">
						<input type="checkbox" id="all_check" onClick="check_all('all_check')" /><font style="display:none"><? echo $all_approval_id; ?></font>
					</td>
...
if ($action=="approve")
{
	...
	if($approval_type==0)
	{
		...
		$partial_approval = ""; $higher_user_buyer_full_permission=0;
		if($is_not_last_user == "")
		{
		...
		else
		{
			...
			{
				$higher_user_buyer_full_permission=1;
			}
		}
		//echo "21**";
		...
		for($i=0;$i<count($booking_nos_all);$i++)
		{
			...
			else
			{
				...
				else if($higher_user_buyer_full_permission==1)
				{
					$partial_approval=3;
				}
				else 
			...
			$data_array.="(".$id.",12,".$booking_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."','".$user_ip."',".$app_instru.",".$user_id.",'".$pc_date_time."')"; 
    ...
    <body>
        ...
        <form name="size_1" id="size_1">
            ...
            <table align="center" cellspacing="0" width="450" class="rpt_table" border="1" rules="all" id="" >
                ...
                <tr>
                    <td align="center">
                        <input type="button" name="close" class="formbutton" value="Close" id="main_close" onClick="fnc_close();" style="width:100px; display:none;" />
                    </td>	  
                </tr>
            </table>
            </fieldset>
...
if($action=='app_mail_notification'){
	...
	$sql= "select a.COMPANY_ID,a.JOB_NO,a.BUYER_ID,a.BOOKING_DATE, a.BOOKING_NO,a.SHORT_BOOKING_TYPE, b.TEAM_LEADER,b.DEALING_MARCHANT,b.FACTORY_MARCHANT  from wo_booking_mst a, wo_po_details_master b where a.job_no=b.job_no and a.entry_form=88 and a.booking_type=1 and a.is_short=1 and a.id in($booking_id_str) and  a.status_active=1 and a.is_deleted=0 and  b.status_active=1 and b.is_deleted=0 order by a.id DESC";
	 //echo $sql; 
	...
	foreach($sql_res as $row){
		...
		?>
		<b>Dear Sir,</b><br>	
		Please check below Additional Fabric Booking Approval Request for your electronic approval<br>	
		...
		<table border="1" rules="all">
			...
			<tr><td>Budget Profit After Additional fabric booking</td><td></td></tr>
		</table>
		...
		//Mail send------------------------------------------
			...
			if($type == 0){
				...
				$elcetronicSql = "SELECT b.USER_EMAIL  from  user_passwd b where b.id=$alter_user_id";
				//echo $elcetronicSql;
				$elcetronicSqlRes=sql_select($elcetronicSql);
				foreach($elcetronicSqlRes as $rows){
					if($rows['USER_EMAIL']){$mailToArr[]=$rows['USER_EMAIL'];}
			...
			$msg = ($type == 0)?"Approval Request":"is Deny";
			$subject="Additional Fabric Booking $msg";
			//echo $subject;die;
			$header=mailHeader();
