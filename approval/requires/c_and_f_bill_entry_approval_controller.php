...
if($action=="report_generate")
{
	...
	$cbo_candf_name = str_replace("'","",$cbo_candf_name);
    ...
    $electronicDataArr=getSequence(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>88,'user_id'=>$user_id_approval,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'product_dept_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0));
   //print_r($buyer_arr);
	...
	if($txt_date_from && $txt_date_to){
		$where_id_cond .= " and a.invoice_date BETWEEN '".$txt_date_from."' AND '".$txt_date_to."'";	
	}
    ...
    $cbo_type_cond = '';
	if($cbo_type_name != 0){
		$cbo_type_cond .= " and a.cnf_type =$cbo_type_name";
	}
	...
	{  
		...
		//  }
        $data_mast_sql = "select a.ID, company_id, a.buyer_id,a.ready_to_approve from  cnf_bill_mst a where a.company_id = $company_name and a.status_active = 1 and a.is_deleted = 0 and a.ready_to_approve =1 and a.IS_APPROVED<>1 ";
     ...
     //echo $data_mast_sql;die;
		...
		for($seq=0;$seq<=count($electronicDataArr['sequ_arr']); $seq++ ){
			...
			if($tmp_sys_id_arr[$seq]){
				...
				$sql.="select a.id, a.sys_number,a.sys_number_prefix_num, a.company_id, a.cnf_type, a.cnf_name_id, a.ex_rate, a.invoice_no,a.invoice_id, a.invoice_date, a.invoice_value, a.invoice_value_bdt, a.invoice_qty, a.pack_qty, a.gross_weight, a.bill_no, a.bill_date, a.buyer_id, a.sb_no, a.job_no, a.ship_mod, a.container_id, a.container_rate, a.remarks, a.total_amount,a.ready_to_approve, a.is_posted_account from cnf_bill_mst a where a.status_active = 1 and a.is_deleted = 0 and a.company_id = $company_name $where_id_cond $buyer_id_cond $cbo_type_cond $cbo_candf_cond $txt_system_no_cond  and a.IS_APPROVED<>1  and a.APPROVED_SEQU_BY=$seq $sys_con";
	...
	{   $sql= "select a.id, a.sys_number,a.sys_number_prefix_num, a.company_id, a.cnf_type, a.cnf_name_id, a.ex_rate, a.invoice_no,a.invoice_id, a.invoice_date, a.invoice_value, a.invoice_value_bdt, a.invoice_qty, a.pack_qty, a.gross_weight, a.bill_no, a.bill_date, a.buyer_id, a.sb_no, a.job_no, a.ship_mod, a.container_id, a.container_rate, a.remarks, a.total_amount,a.ready_to_approve, a.is_posted_account from cnf_bill_mst a,APPROVAL_MST b where a.status_active = 1 and a.is_deleted = 0 and a.company_id = $company_name $where_id_cond $buyer_id_cond $cbo_type_cond $cbo_candf_cond $txt_system_no_cond and b.SEQUENCE_NO={$electronicDataArr['user_by'][$user_id_approval]['SEQUENCE_NO']} and a.APPROVED_SEQU_BY=b.SEQUENCE_NO and a.IS_APPROVED<>0 and b.mst_id=a.id";
 ...
 $nameArray = sql_select($sql);
    ...
    sum(b.amount) as amount from cnf_bill_mst a, cnf_bill_dtls b where a.id=b.mst_id and a.company_id =$company_name $sis_cond  and a.status_active=1 and b.status_active=1 group by  a.id, b.mst_id ");

    // echo "select a.id,b.mst_id, sum(b.deduction) as deduction ,sum (b.payable) as payable,
    // sum(b.amount) as amount from cnf_bill_mst a, cnf_bill_dtls b where a.id=b.mst_id and a.company_id =$company_name $sis_cond  and a.status_active=1 and b.status_active=1 group by  a.id, b.mst_id";die;
        ...
        <div style="width:<? echo $table1+25; ?>px; margin:0 auto;">
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="left" >
                <thead>
                    ...
                    <th width="100">Inv. Value BDT</th>                   
                    <th width="100">Pack. Qty.</th>                  
                    <th width="100">Ship Mode</th>                 
                    <th width="100">Amount (Tk)</th>                  
                    <th width="100">Deduction (TK)</th>                
                    <th width="100">Payable (TK)</th>               
                        ...
                        {                                                   
							...
							if($report_ids[0]==163){ $action="show_trim_booking_report1";}
                                ...
                                <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>"> 
									...
									<td width="100" align="center"><? echo $supplier_arr[$row[csf('cnf_name_id')]]; ?> &nbsp;</p></td>
                                    <td width="150"><p><? echo $buyer_arr[$row[csf('buyer_id')]]; ?> &nbsp;</p></td>
									
                                    <td width="150"><p><? echo $row[csf('bill_no')]; ?>&nbsp;</p></td>				
                                    <td width="100"><p><? echo change_date_format($row[csf('bill_date')]); ?>&nbsp;</p></td>
                                    <td width="100"><p><? echo $row[csf('invoice_value_bdt')]; ?> &nbsp;</p></td>				                                  
                                    <td width="100"><p><? echo $row[csf('pack_qty')]; ?>&nbsp;</p></td>				                                  
                                    <td width="100"><p><? echo $shipment_mode[$row[csf('ship_mod')]]; ?> &nbsp;</p></td>				                                  
                                    <td width="100"><p><? echo $total_amount_arr[$row[csf("id")]]; ?>&nbsp;</p></td>				                                  
                                    <td width="100"><? echo $total_deduce_arr[$row[csf("id")]]; ?>&nbsp;</p></td>				                                  
...
{ 
	...
	$sql = "select a.ID,a.BUYER_ID,a.READY_TO_APPROVE  from cnf_bill_mst a where a.COMPANY_ID=$cbo_company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0  and a.id in($target_ids)";
	//echo $sql;die();
	...
	$finalDataArr=getFinalUser(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>88,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0,'product_dept_arr'=>0,'match_data'=>$matchDataArr));
  ...
  //print_r($user_sequence_no) ;die;
	...
	{ 
        ...
        foreach($target_app_id_arr as $mst_id)
        {
			...
			$data_array.="(".$id.",88,'".$mst_id."','".$user_sequence_no."',".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			
			$approved_no=$max_approved_no_arr[$mst_id][$user_id_approval]+1;
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",88,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1)";
			$ahid++;
		...
		if($flag==1) 
		{
			...
			$rID2=execute_query(bulk_update_sql_statement( "cnf_bill_mst", "id", $field_array_up, $data_array_up, $target_app_id_arr ));
			if($rID2) $flag=1; else $flag=0; 
		}

		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=88 and mst_id in ($target_ids)";
	...
	{
		$max_approved_no_arr=return_library_array( "select mst_id, max(approved_no) as approved_no from approval_history where entry_form=88 and mst_id in($target_ids)  and APPROVED=2 and APPROVED_BY=$user_id_approval group by mst_id", "mst_id", "approved_no"  );
		$ahid=return_next_id( "id","approval_history", 1 ) ;	
		
		$rID1=sql_multirow_update("cnf_bill_mst","IS_APPROVED*READY_TO_APPROVE*APPROVED_SEQU_BY",'2*0*0',"id",$target_ids,0); ; 
		if($rID1) $flag=1; else $flag=0;

		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0, un_approved_by='".$user_id_approval."', un_approved_date='".$pc_date_time."', updated_by='".$user_id."', update_date='".$pc_date_time."' WHERE entry_form=88 and current_approval_status=1 and mst_id in ($target_ids)";
			$rID3=execute_query($query,1);
			if($rID3) $flag=1; else $flag=0;
	...
	{            
		...
		$next_user_app = sql_select("select id from approval_history where mst_id in($target_ids) and entry_form=88 and sequence_no >$user_sequence_no and current_approval_status=1 group by id");
		...
		$rID1=sql_multirow_update("cnf_bill_mst","IS_APPROVED*READY_TO_APPROVE*APPROVED_SEQU_BY",'0*0*0',"id",$target_ids,0);
		if($rID1) $flag=1; else $flag=0;
		...
		if($flag==1) 
		{
			$query="delete from approval_mst  WHERE entry_form=88 and mst_id in ($target_ids)";
			$rID3=execute_query($query,1); 
			if($rID3) $flag=1; else $flag=0; 
		}
...
if ($action=="show_work_order_aop_report")
{
	...
	$com_supplier_id=return_field_value( "supplier_id as supplier_id", "cnf_bill_mst","booking_no=$txt_booking_no","supplier_id");
	$nameArray_approved=sql_select( "select max(b.approved_no) as approved_no from cnf_bill_mst a, approval_history b where a.id=b.mst_id and booking_no=$txt_booking_no and b.entry_form=162"); 
	...
	<div style="width:1540px; margin: 0 auto" align="center">       
       ...
       <?
		...
		$fabric_source='';
		$booking_info=sql_select( "select id,buyer_id,booking_no,booking_date,supplier_id,exchange_rate,currency_id,attention,delivery_date,fabric_source,entry_form from cnf_bill_mst where booking_no=$txt_booking_no"); 
	...
	if($db_type==0)
	{
		$sql= sql_select("select a.booking_no, a.booking_type, a.booking_date, a.delivery_date, a.buyer_id, a.supplier_id, a.job_no, a.item_category, a.source, a.attention, a.process, b.dia_width, b.uom, a.pay_mode, a.exchange_rate, a.currency_id, b.wo_qnty, b.amount, b.pre_cost_remarks FROM cnf_bill_mst a, wo_booking_dtls b  WHERE a.booking_no=$txt_booking_no and a.booking_no=b.booking_no and a.status_active=1 and a.is_deleted=0 order by a.id"); 
	}
	if($db_type==2)
	{
		$sql= sql_select("select a.booking_no, a.booking_type, a.booking_date, a.delivery_date, a.buyer_id, a.supplier_id, a.job_no, a.item_category, a.source, a.attention, a.process, b.dia_width, b.uom, a.pay_mode, a.exchange_rate, a.currency_id, b.wo_qnty, b.amount, b.pre_cost_remarks FROM cnf_bill_mst a, wo_booking_dtls b  WHERE a.booking_no=$txt_booking_no and a.booking_no=b.booking_no and a.status_active=1 and a.is_deleted=0 order by a.id"); 
            ...
            <tr>
                <td width="100%">
                    ...
                    <?
 					...
 					$sel=sql_select("select a.approved_by, a.approved_date, a.id as app_no from approval_history a, electronic_approval_setup b where a.mst_id in(select id from cnf_bill_mst where id=$bookingId) and a.entry_form=162 and a.approved_by=b.user_id and b.company_id=$cbo_company_name and b.is_deleted=0 group by a.approved_by, a.approved_date, a.id, b.sequence_no order by b.sequence_no asc");
