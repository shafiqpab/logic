...
{  
	...
	{  
		...
		$data_mast_sql = "select a.id, a.ydw_no,a.company_id,a.booking_date,a.supplier_id,a.delivery_date from wo_yarn_dyeing_mst a,wo_yarn_dyeing_dtls b where a.id=b.mst_id and a.status_active=1 and a.IS_DELETED=0 and b.status_active=1 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0 and a.IS_APPROVED<>1 and READY_TO_APPROVED=1 and a.entry_form=94 and a.COMPANY_ID=$cbo_company_id $where_con ";
		...
		for($seq=0;$seq<=count($electronicDataArr['sequ_arr']); $seq++ ){
			...
			if($tmp_sys_id_arr[$seq]){
				...
				$sql .= "select a.id,a.pay_mode,service_type,a.ydw_no,a.company_id,a.booking_date,a.supplier_id,a.delivery_date from wo_yarn_dyeing_mst a,wo_yarn_dyeing_dtls b where a.id=b.mst_id and a.status_active=1 and a.IS_DELETED=0 and b.status_active=1 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0 and a.IS_APPROVED<>1 and READY_TO_APPROVED=1 and a.entry_form=94 and a.APPROVED_SEQU_BY=$seq $sys_con";
			}
		}
	}
	else
	{
		...
		$sql = "select a.id,a.pay_mode,service_type, a.ydw_no,a.company_id,a.booking_date,a.supplier_id,a.delivery_date from wo_yarn_dyeing_mst a,wo_yarn_dyeing_dtls b,APPROVAL_MST c where a.id=b.mst_id and a.status_active=1 and a.IS_DELETED=0 and b.status_active=1 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0 and a.IS_APPROVED<>0 and READY_TO_APPROVED=1 and a.entry_form=94 and c.SEQUENCE_NO={$electronicDataArr['user_by'][$user_id_approval]['SEQUENCE_NO']}  and a.APPROVED_SEQU_BY=c.SEQUENCE_NO and c.entry_form=75 and c.mst_id=a.id";
	...
	//
	 ...
	 $sql_cause="select MST_ID,REFUSING_REASON from refusing_cause_history where entry_form=75 ".where_con_using_array($sys_id_arr,0,'mst_id')."  and INSERTED_BY=$user_id_approval order by id asc";
	// echo $sql_cause;	
    ...
    <form name="requisitionApproval_2" id="requisitionApproval_2">
        ...
        <legend>Yarn Service Approval</legend>
                        ...
                        {                                                   
                            $bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
                                ...
                                <tr bgcolor="<?= $bgcolor; ?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor; ?>')" id="tr_<?= $i; ?>"> 
                                    ...
                                    <td width="150" align="center"><a href="javascript:fn_generate_print(<? echo $row['ID']; ?>,<? echo $row['COMPANY_ID']?>,'<? echo $row['YDW_NO']?>',<? echo $row['PAY_MODE']?>,<? echo $row['SERVICE_TYPE']?>,<? echo $row['SUPPLIER_ID']?>)"><? echo $row[csf('ydw_no')]; ?></a></td>                                    
...
{  
	...
	if($approval_type==5)
	{
		...
		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0, un_approved_by='".$user_id_approval."', un_approved_date='".$pc_date_time."', updated_by='".$user_id."', update_date='".$pc_date_time."' WHERE entry_form=75 and current_approval_status=1 and mst_id in ($target_ids)";
		...
		if($flag==1) 
		{
			$query="delete from approval_mst  WHERE entry_form=75 and mst_id in ($target_ids)";
	...
	{      
        ...
        {		
			...
			$data_array.="(".$id.",75,".$mst_id.",".$user_sequence_no.",".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			
			$approved_no=$max_approved_no_arr[$mst_id][$user_id_approval]+1;
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",75,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1)";
			$ahid++;
		...
		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=75 and mst_id in ($target_ids)";
			$rID3=execute_query($query,1);
			if($rID3) $flag=1; else $flag=0;
		}
	...
	{              
		...
		$next_user_app = sql_select("select id from approval_history where mst_id in($target_ids) and entry_form=75 and sequence_no >$user_sequence_no and current_approval_status=1 group by id");
    ...
    <form>
           ...
           <?php
            $custom_designation=return_library_array( "select id,custom_designation from lib_designation ",'id','custom_designation');  
             ...
             $sql="select a.id, a.user_name, a.department_id, a.user_full_name, a.designation,b.SEQUENCE_NO from  user_passwd a,electronic_approval_setup b where a.id=b.user_id and b.company_id=$cbo_company_id and b.entry_form=75 and valid=1 and a.id!=$user_id  and b.is_deleted=0 order by b.SEQUENCE_NO";
                //echo $sql;
