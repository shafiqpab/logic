...
{
	$load="load_drop_down( 'requires/fabric_booking_approval_group_by_controller', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'requires/fabric_booking_approval_group_by_controller', this.value, 'load_drop_down_brand', 'brand_td');";
...
function getSequence($parameterArr=array()){
	...
	$fabric_source_string=implode(',',(array_keys($parameterArr['fabric_source_arr'])));
	...
	foreach($electronic_app_sql_result as $rows){
		...
		if($rows['FABRIC_SOURCE']=='' || $rows['FABRIC_SOURCE'] == 0){
            $rows['FABRIC_SOURCE'] = $fabric_source_string;
        }
	...
	foreach($parameterArr['match_data'] as $sys_id=>$bbtsRows){
		foreach($userSeqDataArr as $user_id=>$seq){
			if(
				...
				&& (in_array($bbtsRows['fb_source'],$usersDataArr[$user_id]['FABRIC_SOURCE']) || $bbtsRows['fb_source']==0)
			){
...
if($action=="report_generate")
{
    ...
    $electronicDataArr=getSequence(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>7,'user_id'=>$app_user_id,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>$buyer_brand_arr,'product_dept_arr'=>0,'fabric_source_arr'=>$fabric_source,'lib_store_arr'=>0));
	...
	if($booking_year != 0){
		$where_con .= " and to_char(a.insert_date,'YYYY')='".trim($booking_year)."' ";
	}
	if($cbo_brand_id>0){$where_con .="and a.brand_id='$cbo_brand_id'";}
	if($cbo_season_id>0){$where_con .="and c.season_buyer_wise='$cbo_season_id'"; }
	if($cbo_season_year>0){$where_con .="and a.season_year='$cbo_season_year'";}
	...
	{  
		...
		if($electronicDataArr['user_by'][$app_user_id]['BRAND_ID']){
			$where_con .= " and e.BRAND_ID in(".$electronicDataArr['user_by'][$app_user_id]['BRAND_ID'].",0)";
			$electronicDataArr['sequ_by'][0]['BRAND_ID']=$electronicDataArr['user_by'][$app_user_id]['BRAND_ID'];
		}

		if($electronicDataArr['user_by'][$app_user_id]['FABRIC_SOURCE']){
			$where_con .= " and a.FABRIC_SOURCE in(".$electronicDataArr['user_by'][$app_user_id]['FABRIC_SOURCE'].",0)";
			$electronicDataArr['sequ_by'][0]['FABRIC_SOURCE']=$electronicDataArr['user_by'][$app_user_id]['FABRIC_SOURCE'];
		}
 
		$data_mast_sql = "select a.ID, a.BUYER_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY,a.FABRIC_SOURCE,e.BRAND_ID from wo_booking_mst a,WO_BOOKING_DTLS b,WO_PO_DETAILS_MASTER e where a.BOOKING_NO=b.BOOKING_NO and b.JOB_NO=e.job_no and a.COMPANY_ID=$company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and a.is_short in(2,3) and a.booking_type=1 and a.item_category in(2,3,13) and a.IS_APPROVED<>1 and a.READY_TO_APPROVED=1  $where_con group by a.ID, a.BUYER_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY,a.FABRIC_SOURCE,e.BRAND_ID";
			...
			for($group = ($row['APPROVED_GROUP_BY'] == $my_group && $electronicDataArr['sequ_by'][$my_seq]['BYPASS'] == 2)?$my_group:$my_group-1;$group >= 0; $group-- ){
				...
				foreach($electronicDataArr['group_seq_arr'][$group] as $seq){
					...
					if($seq<$my_seq){ 
						...
						if(
							(in_array($row['BUYER_ID'],explode(',',$electronicDataArr['sequ_by'][$seq]['BUYER_ID'])) || $row['BUYER_ID']==0) && (in_array($row['BRAND_ID'],explode(',',$electronicDataArr['sequ_by'][$seq]['BRAND_ID'])) || $row['BRAND_ID']==0) && (in_array($row['FABRIC_SOURCE'],explode(',',$electronicDataArr['sequ_by'][$seq]['FABRIC_SOURCE'])) || $row['FABRIC_SOURCE']==0) && ($row['APPROVED_GROUP_BY'] <= $group)
							)
						{ 	
		...
		for($group=0;$group<=$my_group; $group++ ){
			foreach($electronicDataArr['group_seq_arr'][$group] as $seq){
				...
				if(count($tmp_sys_id_arr[$group][$seq])){
					...
					$sql .= "select a.id,a.entry_form, $year_field, a.booking_no_prefix_num, a.booking_no, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.supplier_id, a.delivery_date, a.booking_date, '0' as approval_id, a.is_approved, a.is_apply_last_update,0 as revised_no from wo_booking_mst a, wo_booking_dtls b , wo_po_break_down c where a.booking_no=b.booking_no  and b.PO_BREAK_DOWN_ID=c.ID and a.company_id=$company_name and a.APPROVED_SEQU_BY=$seq and a.APPROVED_GROUP_BY=$group $sys_con  and a.is_short in(2,3) and a.booking_type=1 and a.item_category in(2,3,13) and a.is_deleted=0 and a.status_active=1  and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 and a.is_approved<>1  $internal_ref_con group by a.id, a.booking_no, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id,a.ready_to_approved, a.APPROVED_SEQU_BY, a.supplier_id, a.delivery_date, a.booking_date, a.is_approved, a.is_apply_last_update, a.insert_date, a.update_date, a.booking_no_prefix_num,a.entry_form ";// and b.fin_fab_qnty>0
				}
	...
	{   
		...
		$where_con .= "and d.GROUP_NO={$electronicDataArr['user_by'][$app_user_id]['GROUP_NO']}";
	 	
		$approved_user_cond=" and c.approved_by='$user_id'";
		$sql = "select a.id,a.entry_form, $year_field, a.booking_no_prefix_num, a.booking_no, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.supplier_id, a.delivery_date, a.booking_date, '0' as approval_id, a.is_approved, a.is_apply_last_update,0 as revised_no from wo_booking_mst a, wo_booking_dtls b , wo_po_break_down c,APPROVAL_MST d  where a.booking_no=b.booking_no  and b.PO_BREAK_DOWN_ID=c.ID and a.company_id=$company_name and a.APPROVED_GROUP_BY=d.GROUP_NO  and a.is_short in(2,3) and a.booking_type=1 and a.item_category in(2,3,13) and a.is_deleted=0 and a.status_active=1  and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 and a.is_approved<>0 and  d.entry_form=7   $where_con $internal_ref_con group by a.id, a.booking_no, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id,a.ready_to_approved, a.APPROVED_SEQU_BY, a.supplier_id, a.delivery_date, a.booking_date, a.is_approved, a.is_apply_last_update, a.insert_date, a.update_date, a.booking_no_prefix_num,a.entry_form ";// and b.fin_fab_qnty>0
   ...
   //echo $sql; 
	...
	$booknoId = implode(',',$booking_id_arr);

	$max_approved_no_arr = return_library_array("select mst_id, max(approved_no) as approved_no from approval_history where mst_id in($booknoId) and entry_form=7 group by mst_id","mst_id","approved_no");


	$bookingidCond = where_con_using_array($booking_id_arr,0,'a.id');

	if($cbo_brand_id > 0){$brand_cond="and d.brand_id='$cbo_brand_id'";}
	if($cbo_season_id > 0){	$season_cond="and d.season_buyer_wise='$cbo_season_id'";}
	if($cbo_season_year > 0){$season_year_cond="and d.season_year='$cbo_season_year'";}
	...
	foreach( $sql_job as $jval)
	{
		...
		$job_information_arr[$jval['BOOKING_NO']]['quot_no'][]=$jval['QUOTATION_ID'];
		$job_information_arr[$jval['BOOKING_NO']]['deal_march'][]=$jval['DEALING_MARCHANT'].'=='.$jval['JOB_NO'];
		$job_information_arr[$jval['BOOKING_NO']]['po_break_down_id'][]=$jval['PO_BREAK_DOWN_ID'];
		$job_information_arr[$jval['BOOKING_NO']]['grouping'][]=$jval['GROUPING'];
		$job_information_arr[$jval['BOOKING_NO']]['file_no'][]=$jval['FILE_NO'];
		$job_paymode_arr[$jval['BOOKING_NO']]=$jval['PAY_MODE'];

		$job_sise_data[$jval['BOOKING_NO']]['brand'] =$brand[$jval['BRAND_ID']];
		$job_sise_data[$jval['BOOKING_NO']]['season'] =$season[$jval['SEASON_BUYER_WISE']];
		$job_sise_data[$jval['BOOKING_NO']]['style_ref'] =$jval['STYLE_REF_NO'];
		$job_sise_data[$jval['BOOKING_NO']]['season_year'] =$jval['SEASON_YEAR'];
		$season_data_arr[$jval['BOOKING_NO_PREFIX_NUM']]['season'] =$season[$jval['SEASON_BUYER_WISE']];
	}
	?>
    <script>
		...
		{ 
			...
			var page_link = 'requires/fabric_booking_approval_group_by_controller.php?data='+data+'&action=appcause_popup&txt_alter_user_id='+txt_alter_user_id;
		...
		function openmypage_app_instrac(wo_id,app_type,i)
		{
			...
			var page_link = 'requires/fabric_booking_approval_group_by_controller.php?data='+data+'&action=appinstra_popup';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=500px,height=250px,center=1,resize=1,scrolling=0','');
			emailwindow.onclose=function()
			{
				var appv_cause=this.contentDoc.getElementById("hidden_appv_cause");
		...
		function openmypage_unapp_request(wo_id,app_type,i)
		{
			...
			var page_link = 'requires/fabric_booking_approval_group_by_controller.php?data='+data+'&action=unappcause_popup';
    ...
    <form name="requisitionApproval_2" id="requisitionApproval_2">
        ...
        <legend>Fabric Booking Approval</legend>
            ...
            <div style="width:<?=$table1; ?>px; overflow-y:scroll; max-height:330px;" id="buyer_list_view" align="center">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="<?=$table2; ?>" class="rpt_table" id="tbl_list_search" align="left">
                    <tbody>
                        <?
					//	 echo $sql;
                            ...
                            { 
								...
								$row['REVISED_NO'] = $max_approved_no_arr[$row['ID']];
								...
								if($row[csf('entry_form')]==108){
									...
									$variable="<a href='#' onClick=\"generate_worder_report('".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$all_po_id."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$all_job_no."','".$row[csf('is_approved')]."','".$row_id."','".$row[csf('entry_form')]."','print_booking_20','".$i."',".$fabric_nature.")\"> ".$row[csf('booking_no_prefix_num')]."<a/>";
								}
								else if($row_id==175){
									$variable="<a href='#' onClick=\"generate_worder_report('".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$all_po_id."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$all_job_no."','".$row[csf('is_approved')]."','".$row_id."','".$row[csf('entry_form')]."','print_booking_6','".$i."',".$fabric_nature.")\"> ".$row[csf('booking_no_prefix_num')]."<a/>";
								}
								else if($row_id==28){
									$variable="<a href='#' onClick=\"generate_worder_report('".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$all_po_id."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$all_job_no."','".$row[csf('is_approved')]."','".$row_id."','".$row[csf('entry_form')]."','print_booking_13','".$i."',".$fabric_nature.")\"> ".$row[csf('booking_no_prefix_num')]."<a/>";
								}
								else if($row_id==304){
									$variable="<a href='#' onClick=\"generate_worder_report('".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$all_po_id."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$all_job_no."','".$row[csf('is_approved')]."','".$row_id."','".$row[csf('entry_form')]."','print_booking_15','".$i."',".$fabric_nature.")\"> ".$row[csf('booking_no_prefix_num')]."<a/>";
								}
								else if($row_id==339){
									$variable="<a href='#' onClick=\"generate_worder_report('".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$all_po_id."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$all_job_no."','".$row[csf('is_approved')]."','".$row_id."','".$row[csf('entry_form')]."','print_booking_18','".$i."',".$fabric_nature.")\"> ".$row[csf('booking_no_prefix_num')]."<a/>";
								}
								else if($row_id==719){
									$variable="<a href='#' onClick=\"generate_worder_report('".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$all_po_id."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$all_job_no."','".$row[csf('is_approved')]."','".$row_id."','".$row[csf('entry_form')]."','print_booking_16','".$i."',".$fabric_nature.")\"> ".$row[csf('booking_no_prefix_num')]."<a/>";
								}
								else if($row_id==865){
									$variable="<a href='#' onClick=\"generate_worder_report('".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$all_po_id."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$all_job_no."','".$row[csf('is_approved')]."','".$row_id."','".$row[csf('entry_form')]."','show_fabric_booking_report28','".$i."',".$fabric_nature.")\"> ".$row[csf('booking_no_prefix_num')]."<a/>";
									...
									<tr bgcolor="<?=$bgcolor; ?>" onClick="change_color('tr_<?=$i; ?>','<?=$bgcolor; ?>');" id="tr_<?=$i; ?>">
										<td width="35" align="center" valign="middle">
											<input type="checkbox" id="tbl_<?=$i;?>" onClick="check_last_update(<?=$i;?>);"  />
                                        ...
                                        &nbsp;</td>
											...
											<td width="70" align="center" style="word-break:break-all">
												<p>
													<a href='##' style='color:#000' onClick="generate_worder_report11('<?php echo $row[csf('booking_no')]; ?>', '<?php echo $row[csf('company_id')]; ?>', '<?php echo $all_po_id; ?>', '<?php echo $row[csf('item_category')]; ?>', '<?php echo $row[csf('fabric_source')]; ?>', '<?php echo $all_job_no; ?>', '<?php echo $row[csf('is_approved')]; ?>', <?php echo $row_id; ?>, '<?php echo $row[csf('entry_form')]; ?>', 'show_fabric_booking_report_print23', '<?php echo $i; ?>', '<?php echo $fabric_nature; ?>')">
														<?php echo $fabric_source[$row[csf('fabric_source')]]; ?>
													</a>
												</p>
											</td>
            ...
            <table cellspacing="0" cellpadding="0" border="0" rules="all" width="<?=$table1; ?>" class="rpt_table">
				<tfoot>
                    <td width="35" align="center" style=" <?=$isApp; ?>">
...
{ 
	...
	// $sql = "select a.IS_APPROVED,a.ID,a.READY_TO_APPROVED, a.BUYER_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY,a.FABRIC_SOURCE,e.BRAND_ID from wo_booking_mst a,WO_PO_DETAILS_MASTER e where a.JOB_NO=e.job_no and a.COMPANY_ID=$cbo_company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0   and a.READY_TO_APPROVED=1 and a.id in($booking_ids)";
	$sql = "select a.IS_APPROVED,a.ID,a.READY_TO_APPROVED, a.BUYER_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY,a.FABRIC_SOURCE,e.BRAND_ID from wo_booking_mst a,WO_BOOKING_DTLS b,WO_PO_DETAILS_MASTER e where a.BOOKING_NO=b.BOOKING_NO and b.JOB_NO=e.job_no and a.COMPANY_ID=$company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0  and a.IS_APPROVED<>1 and a.READY_TO_APPROVED=1   and a.id in($booking_ids) group by a.IS_APPROVED,a.ID,a.READY_TO_APPROVED, a.BUYER_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY,a.FABRIC_SOURCE,e.BRAND_ID";
	...
	foreach ($sqlResult as $row)
	{
		...
		$matchDataArr[$row['ID']]=array('buyer_id'=>$row['BUYER_ID'],'brand_id'=>$row['BRAND_ID'],'supplier_id'=>0,'fb_source'=>$row['FABRIC_SOURCE']);
		$last_app_status_arr[$row['ID']] = $row['IS_APPROVED'];
	}
	
	$finalDataArr=getFinalUser(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>7,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>$item_cat_arr,'lib_store_arr'=>0,'fabric_source_arr'=>$fabric_source,'match_data'=>$matchDataArr));
	...
	{ 
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			...
			if($last_app_status_arr[$mst_id] == 0 || $last_app_status_arr[$mst_id] == 2){
				$approved_no = $max_approved_no_arr[$mst_id]+1;
				$approved_no_array[$booking_no_arr[$key]] = $approved_no;
			}


			if($data_array!=''){$data_array.=",";}
			$data_array.="(".$id.",7,'".$mst_id."','".$user_sequence_no."','".$user_group_no."',".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."',".$approved.")"; 
			$id=$id+1;
			
		
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",7,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,".$approved.")";
			$ahid++;
		...
		if(count($approved_no_array)>0 && $flag)
		{
			...
			$sql_insert_dtls="insert into wo_booking_dtls_hstry(id, approved_no, booking_dtls_id, job_no, po_break_down_id, pre_cost_fabric_cost_dtls_id, color_size_table_id, booking_no, booking_type, is_short, fabric_color_id, item_size, fin_fab_qnty, grey_fab_qnty, rate, amount, color_type, construction, copmposition, gsm_weight, dia_width, process_loss_percent, trim_group, description, brand_supplier, uom, process, sensitivity, wo_qnty, delivery_date, cons_break_down, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, rmg_qty, gmt_item, responsible_dept, responsible_person, reason, gmts_size, gmts_color_id,revised_date)
				select
				'', $approved_string_dtls, id, job_no, po_break_down_id, pre_cost_fabric_cost_dtls_id, color_size_table_id, booking_no, booking_type, is_short, fabric_color_id, item_size, fin_fab_qnty, grey_fab_qnty, rate, amount, color_type, construction, copmposition, gsm_weight, dia_width, process_loss_percent, trim_group, description, brand_supplier, uom, process, sensitivity, wo_qnty, delivery_date, cons_break_down, inserted_by, insert_date, updated_by, update_date, status_active, is_deleted, rmg_qty, gmt_item, responsible_dept, responsible_person, reason, gmts_size, gmts_color_id,'".date('d-M-Y',time())."' from wo_booking_dtls where booking_no in ($booking_nos)";
	...
	{              
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			$approved_no = $max_approved_no_arr[$mst_id]*1;
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",7,".$mst_id.",".$approved_no.",'".$user_sequence_no."',0,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,2)";
			$ahid++;
        }

		$checkSql="select BOOKING_ID from INV_RECEIVE_MASTER WHERE BOOKING_ID  in ($booking_ids) and ITEM_CATEGORY IN(2,3,13) and STATUS_ACTIVE=1
		UNION ALL
		...
		if($flag==1)
        {
            $query="UPDATE approval_history SET current_approval_status=0, un_approved_by='".$app_user_id."', un_approved_date='".$pc_date_time."', updated_by='".$user_id."', update_date='".$pc_date_time."' WHERE entry_form=7 and current_approval_status=1 and mst_id in ($booking_ids)";
            $rID5=execute_query($query,1);
            if($rID5) $flag=1; else $flag=0;
        }


		 // echo "5**".$rID1.",".$rID2.",".$rID3.",".$history_data_array.",".$rID5;oci_rollback($con);die;
	...
	{              
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			...
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",7,".$mst_id.",".$approved_no.",'".$user_sequence_no."',0,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			$ahid++;
        }
		...
		if($flag==1)
		{
		 ...
		 //echo "5**".$rID1.",".$rID2.",".$rID3.",".$rID4.$query;oci_rollback($con);die;
...
if ($action=="appcause_popup")
{
	...
	if($app_cause=="")
	{
		$sql_cause="select MAX(id) as id from fabric_booking_approval_cause where  entry_form=7 and user_id=$txt_alter_user and booking_id='$wo_id' and approval_type=$app_type and status_active=1 and is_deleted=0";
		//echo $sql_cause; //die;
    ...
    <script>
		...
		function fnc_appv_entry(operation)
		{
			...
			else
			{
				...
				http.open("POST","fabric_booking_approval_group_by_controller.php",true);
...
if($action=='approval_notification'){
	list($sysId,$mailId)=explode('__',$data);
	$sysId=str_replace('*',',',$sysId);

	$sql="select a.APPROVED_BY,a.PAY_MODE,a.FABRIC_SOURCE,a.COMPANY_ID,a.INSERTED_BY,A.BUYER_ID,A.SUPPLIER_ID,A.ID, A.BOOKING_NO,a.IS_APPROVED,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY, e.REFUSING_REASON as APPROVAL_CAUSE,f.STYLE_REF_NO,f.JOB_NO ,c.PO_NUMBER  AS PO_NUMBER,f.BRAND_ID from wo_po_break_down c,wo_booking_dtls d, wo_po_details_master f, wo_booking_mst a left join REFUSING_CAUSE_HISTORY e on a.id = e.MST_ID where  a.booking_no=d.booking_no and d.job_no=c.job_no_mst  and f.job_no=d.job_no and a.is_short in(2,3) and a.booking_type=1 and a.item_category in(2,3,13) and a.status_active=1 and a.is_deleted=0 and a.id in($sysId) group by a.APPROVED_BY,a.PAY_MODE,a.FABRIC_SOURCE,a.COMPANY_ID,c.PO_NUMBER,a.INSERTED_BY,A.BUYER_ID,A.SUPPLIER_ID,A.ID, A.BOOKING_NO,a.IS_APPROVED,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY, e.REFUSING_REASON,f.STYLE_REF_NO,f.JOB_NO,f.BRAND_ID";
	 //echo $sql;die;
	$sqlDataArr=sql_select($sql);
	$dataArr = array();
	foreach($sqlDataArr as $row){ 
	...
	$company_arr=return_library_array( "select id, company_short_name from lib_company",'id','company_short_name');
		...
		foreach($dataArr as $row){ 
			...
			if($IS_APPROVED == 3){
				$mail_sql = "SELECT c.EMAIL_ADDRESS FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id and a.mail_item=87 and b.mail_user_setup_id=c.id and a.company_id=$COMPANY_ID AND a.MAIL_TYPE=4 and c.IS_DELETED=0 and c.STATUS_ACTIVE=1";

				//echo $mail_sql;die;
	
				$mail_sql_res=sql_select($mail_sql);
				foreach($mail_sql_res as $mail_row)
				{
					if($mail_row['EMAIL_ADDRESS']){$mailArr[$mail_row['EMAIL_ADDRESS']] = $mail_row['EMAIL_ADDRESS'];}
				}
			}
			...
			if(!empty($BRAND_ID))
			{
				$where_con .= " AND  ( a.BRAND_ID like '%,$BRAND_ID,%' OR a.BRAND_ID like '%,$BRAND_ID' OR a.BRAND_ID like '$BRAND_ID,%' OR a.BRAND_ID = '$BRAND_ID' OR a.BRAND_ID IS NULL OR a.BRAND_ID = '0') ";
			}
			if(!empty($BUYER_ID))
			{
				$where_con .= " AND  ( a.BUYER_ID like '%,$BUYER_ID,%' OR a.BUYER_ID like '%,$BUYER_ID' OR a.BUYER_ID like '$BUYER_ID,%' OR a.BUYER_ID = '$BUYER_ID' OR a.BUYER_ID IS NULL OR a.BUYER_ID = '0') ";
			}
			
			
			$user_sql = "select a.USER_ID,a.GROUP_NO,a.SEQUENCE_NO,b.USER_EMAIL from ELECTRONIC_APPROVAL_SETUP a,USER_PASSWD b where a.USER_ID=b.id and a.ENTRY_FORM=7 and a.IS_DELETED=0 and b.IS_DELETED=0 and b.STATUS_ACTIVE=1 and a.COMPANY_ID={$row['COMPANY_ID']} and a.SEQUENCE_NO > {$row['APPROVED_SEQU_BY']} $where_con order by a.SEQUENCE_NO";
			
			//echo $user_sql;die;
			
			$user_sql_res = sql_select($user_sql);
			$flag = 0;
			foreach($user_sql_res as $urow){
				if($flag == 0){$flag = $urow['GROUP_NO'];}
				if($urow['USER_EMAIL']){$mailArr[$urow['USER_EMAIL']] = $urow['USER_EMAIL'];}
				if($flag != $urow['GROUP_NO']){break;}
			...
			?>
			Dear Concerned,<br />			
			Your approval request against the following reference.				
