...
if($action=="report_generate")
{
	...
	if($previous_approved==1 && $approval_type==1)	//approval process with prevous approve start
	{
		...
		where a.id=c.mst_id and a.company_id=$company_name and a.entry_form in(352) and a.is_deleted=0 and a.status_active=1 and a.is_approved in(1,3) and a.ready_to_approve=1 $date_cond $requ_no_cond $barcode_cond 
	...
	else if($approval_type==0) 
	{
		if($user_sequence_no==$min_sequence_no) // First user
		{
			...
			where a.company_id=$company_name  and a.is_deleted=0 and a.status_active=1 and a.entry_form in(352) and a.is_approved=0 and a.ready_to_approve=1 $date_cond $requ_no_cond $barcode_cond 
		...
		else // Next user
		{
			...
			if($sequence_no=="") // bypass if previous user Yes
			{
				...
				else
				{
					...
					$requsition_id=return_field_value("LISTAGG(b.mst_id, ',') WITHIN GROUP (ORDER BY b.mst_id) as requsition_id","inv_item_transfer_requ_mst a, approval_history b","a.id=b.mst_id  and a.company_id=$company_name and b.sequence_no in ($sequence_no_by) and a.ready_to_approve=1 and b.entry_form=35 and b.current_approval_status=1 and a.is_approved in (3,1)","requsition_id");

					$requsition_id=implode(",",array_unique(explode(",",$requsition_id)));
					
					$requsition_id_app_byuser=return_field_value("LISTAGG(b.mst_id, ',') WITHIN GROUP (ORDER BY b.mst_id) as requsition_id","inv_item_transfer_requ_mst a, approval_history b","a.id=b.mst_id and a.company_id=$company_name and a.ready_to_approve=1  and b.sequence_no=$user_sequence_no  and b.entry_form=35 and b.current_approval_status=1 and a.is_approved in (3,1)","requsition_id");
				...
				{					
					...
					from inv_item_transfer_requ_mst a
					where a.company_id=$company_name  and a.is_deleted=0 and a.status_active=1 and a.entry_form in(352) and a.is_approved in(0,3) and a.id in ($requsition_id) and a.ready_to_approve=1 and a.status_active=1 and a.is_deleted=0 $date_cond $requ_no_cond  $barcode_cond
					GROUP by a.id,  a.transfer_prefix_number, a.transfer_system_id,a.challan_no, a.company_id,a.insert_date,  a.transfer_criteria, a.transfer_date, a.is_approved, a.entry_form
					...
					from inv_item_transfer_requ_mst a
					where  a.id not in ($requsition_id) and a.is_approved=$approval_type and a.company_id=$company_name  and a.is_deleted=0 and a.status_active=1 and a.entry_form in(352) $date_cond $requ_no_cond  $barcode_cond
					GROUP by a.id,  a.transfer_prefix_number, a.transfer_system_id,a.challan_no, a.company_id,a.insert_date,  a.transfer_criteria, a.transfer_date, a.is_approved) x  order by x.id, a.entry_form";
					//echo $sql;
				}
				else
				{ 
					...
					from inv_item_transfer_requ_mst a 
					where a.is_approved=$approval_type and a.company_id=$company_name and a.is_deleted=0 and a.status_active=1 and a.entry_form in(352) $date_cond $requ_no_cond  $barcode_cond
					GROUP by a.id,  a.transfer_prefix_number, a.transfer_system_id,a.challan_no, a.company_id,a.insert_date,  a.transfer_criteria, a.transfer_date, a.is_approved, a.entry_form";
					//echo $sql;
				}
				//echo $sql;
			...
			else // bypass No
			{
				...
				from inv_item_transfer_requ_mst a, approval_history c 
				where   a.is_approved in (1,3) and a.id=c.mst_id and c.entry_form in(40)  and a.company_id=$company_name  and a.is_deleted=0 and a.status_active=1 and a.entry_form in(352) and c.current_approval_status=1 $sequence_no_cond $date_cond $requ_no_cond $barcode_cond
                            ...
                            foreach ($nameArray as $row)
                            {
								...
								<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" align="center"> 
									...
									<td width="70">
                                    	...
                                    	<p><? echo $row[csf('transfer_prefix_number')]; ?></p>
                                    </td>                                    
...
if ($action=="approve")
{
	...
	if($approval_type==0)
	{
        ...
        $max_approved_no_arr = return_library_array("SELECT mst_id, max(approved_no) as approved_no from approval_history where mst_id in($booking_ids) and entry_form=40 group by mst_id","mst_id","approved_no");
		...
		for($i=0;$i<count($booking_nos_all);$i++)
		{
			...
			$data_array.="(".$id.",40,".$booking_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."',".$user_id.",'".$pc_date_time."')"; 
