...
if($action=="load_drop_down_buyer_new_user")
{
	...
	$electronicDataArr=getSequence(array('company_id'=>$company_id,'ENTRY_FORM'=>12,'user_id'=>$app_user_id,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'product_dept_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0));
	...
	{  
		...
		$data_mast_sql = "SELECT A.ID, A.BUYER_ID,a.BRAND_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY from wo_booking_mst a where  a.is_short=1 and a.booking_type=1 and a.company_id=$company_id and a.status_active=1 and a.is_deleted=0 and  a.ready_to_approved=1 and a.is_approved<>1  $where_con";
		...
		{ 
			...
			for($group = $my_group;$group >= 0; $group-- ){
				...
				foreach($electronicDataArr['group_seq_arr'][$group] as $seq){
					if($seq<$my_seq){ 
						...
						{ 
							...
							if( (in_array($row['APPROVED_SEQU_BY'],$electronicDataArr['group_seq_arr'][$my_group]) && ($row['APPROVED_SEQU_BY'] != $my_previous_bypass_no_seq ) && $electronicDataArr['group_bypass_arr'][$my_group][2] !=2 ) || (count($group_stage_arr) > 3)  ){ 
		...
		for($group=0;$group<=$my_group; $group++ ){
			foreach($electronicDataArr['group_seq_arr'][$group] as $seq){
				if(count($tmp_sys_id_arr[$group][$seq])){
					...
					$sql .= "SELECT a.id, a.booking_no_prefix_num as prefix_num, a.booking_no, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.supplier_id, a.delivery_date, a.booking_date, '0' as approval_id, a.job_no, a.is_approved, a.po_break_down_id, a.is_apply_last_update, c.grouping, c.file_no, a.pay_mode, a.inserted_by, (SELECT MAX(d.approved_no) as revised_no FROM approval_history d WHERE d.mst_id = a.id AND d.entry_form = 12) as revised_no,(SELECT MAX (d.approved_date)     AS approved_date FROM approval_history d WHERE d.mst_id = a.id AND d.entry_form = 12) AS approved_date, a.insert_date, a.remarks FROM wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c  
					WHERE a.booking_no = b.booking_no AND a.job_no = c.job_no_mst AND b.po_break_down_id = c.id AND a.company_id = $company_id AND a.is_short = 1 AND a.booking_type = 1 AND a.item_category IN (2, 3, 13) AND a.is_deleted = 0 AND a.status_active = 1 AND b.status_active = 1 AND b.is_deleted = 0 AND a.ready_to_approved = 1 AND b.fin_fab_qnty > 0 AND a.is_approved <> 1 AND a.APPROVED_SEQU_BY = $seq AND a.APPROVED_GROUP_BY = $group $sys_con $internal_ref_cond $file_no_cond $select_year_cond GROUP BY a.id, a.booking_no_prefix_num, a.booking_no, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.supplier_id, a.delivery_date, a.booking_date, a.job_no, a.is_approved, a.po_break_down_id, a.is_apply_last_update, a.insert_date, c.grouping, c.file_no, a.pay_mode, a.inserted_by, a.remarks";
				}

			}
		}

		$sql = "SELECT *  FROM ($sql) x ORDER BY x.insert_date DESC";

	}
	else
	{   
		$sql="SELECT a.id, a.booking_no_prefix_num as prefix_num,a.booking_no, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.supplier_id, a.delivery_date, a.booking_date, '0' as approval_id, a.job_no, a.is_approved, a.po_break_down_id, a.is_apply_last_update,c.grouping, c.file_no, a.pay_mode,a.inserted_by, (select max(d.approved_no) as revised_no from approval_history d where d.mst_id =a.id and d.entry_form=12) as revised_no , (SELECT MAX ( d.approved_date) AS approved_date
		FROM approval_history d WHERE d.mst_id = a.id AND d.entry_form = 12)    AS approved_date,a.insert_date,a.remarks from wo_booking_mst a, wo_booking_dtls b,wo_po_break_down c,APPROVAL_MST e  where e.mst_id=a.id and a.booking_no=b.booking_no and a.job_no=c.job_no_mst and b.po_break_down_id =c.id and a.company_id=$company_id and a.is_short=1 and a.booking_type=1 and a.item_category in(2,3,13) and  a.APPROVED_GROUP_BY=e.GROUP_NO and  a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 and b.fin_fab_qnty>0 and a.is_approved<>0 $buyer_id_cond and e.GROUP_NO={$electronicDataArr['user_by'][$app_user_id]['GROUP_NO']} $whereCon $internal_ref_cond $file_no_cond $where_con $select_year_cond group by a.id, a.booking_no_prefix_num,a.booking_no, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.supplier_id, a.delivery_date, a.booking_date, a.job_no, a.is_approved, a.po_break_down_id, a.is_apply_last_update,a.insert_date,c.grouping, c.file_no, a.pay_mode ,a.inserted_by ,a.insert_date,a.remarks order by a.insert_date desc";
    }
	//echo $sql;die;
		...
		<script>
			function openmypage_app_cause_show_2(wo_id,app_type,i)
			{
				...
				var page_link = 'requires/short_feb_booking_approval_controller.php?data='+data+'&action=appcause_popup_show_2';
			...
			function openmypage_unapp_request(wo_id,app_type,i)
			{
				...
				var page_link = 'requires/short_feb_booking_approval_controller.php?data='+data+'&action=unappcause_popup';
		...
		<?
			 ...
			 if($approval_type==0)
			 {
				...
				$table1=1430+$new_column; //50-32=18
			...
			<legend>Short Fabric Booking Approval</legend>	
				<table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $table1; ?>" class="rpt_table" >
					<thead>
						...
						<th style="word-break: break-all;" width="70">Last Approve Date</th>
						...
						<th style="word-break: break-all;" width="100"> IR/IB</th>
						...
						<tbody>
							<?
							  ...
							  $sql_cause="select approval_cause,booking_id from fabric_booking_approval_cause where page_id='$menu_id' and entry_form=12   and approval_type='$approval_type' and status_active=1 and is_deleted=0 $booking_id_cond";	
								...
								$main_fab_res =sql_select("SELECT a.id as aid,b.booking_no,a.job_no_mst as job_no,c.entry_form,b.id,c.fabric_source,c.item_category,c.is_approved,c.company_id,c.booking_no_prefix_num,b.po_break_down_id  from   wo_po_break_down a, wo_booking_dtls b,wo_booking_mst c where a.id=b.po_break_down_id and b.booking_no=c.booking_no  and a.is_deleted=0 and b.is_deleted=0 and c.is_deleted=0 and b.booking_type=1 and b.is_short=2 and c.entry_form in (118,108,86)  $po_cond group by a.id,b.booking_no,a.job_no_mst,c.entry_form,b.id,c.fabric_source,c.item_category,c.is_approved,c.company_id,c.booking_no_prefix_num,b.po_break_down_id ");
	
	
								$job_wise_main_fabric_data=array();
	
								foreach ($main_fab_res as $row) {
									$job_wise_main_fabric_data[$row[csf('job_no')]][$row[csf('booking_no')]]['booking_no']=$row[csf('booking_no')];
									...
									$job_wise_main_fabric_data[$row[csf('job_no')]][$row[csf('booking_no')]]['po_id'].=$row[csf('aid')].",";
									
								}
								
								//echo '<pre>';print_r($job_wise_main_fabric_data);die;
							...
							// short fabric booking hyperlink//
									$report_action='';
							if($type==1){
								$print_report_format1=return_field_value("format_id","lib_report_template","template_name ='".$company_id."' and module_id=2 and report_id=2 and is_deleted=0 and status_active=1");
								...
								if($report_id[0]==8){
									...
									$report_action='print_booking_4';
								}elseif($report_id[0]==53){
									$report_action='print_booking_5';
								}elseif($report_id[0]==72){
									$report_action='print_booking_6';
								}elseif($report_id[0]==220){
									$report_action='print_booking_8';
								}
	
	
							}elseif($type==2){
								$print_report_format2=return_field_value("format_id","lib_report_template","template_name ='".$company_id."' and module_id=2 and report_id=1 and is_deleted=0 and status_active=1");
								...
								// job hyperling
										...
										else if($row_id==445){$action='preCostRpt8';}
									...
									foreach($job_wise_main_fabric_data[$row[csf('job_no')]] as $main_booking_no => $booking_data) 
									{
										...
										if($booking_data['entry_form']==86) //Budget wise fab booking
										{
											...
											else  if($row_id==786) $action='show_fabric_booking_report25';
											else if($row_id==120){$action='budgetsheet3';}
											else  if($row_id==502) $action='show_fabric_booking_report26';	 
										...
										$variable='';
										$variable="<a href='#' onClick=\"generate_worder_report_main('".$main_booking_no."','".$booking_data['company_id']."','".$all_po_id."','".$booking_data['item_category']."','".$booking_data['fabric_source']."','".$row[csf('job_no')]."','".$booking_data['is_approved']."','".$row_id."','".$booking_data['entry_form']."','".$action."','".$i."',".$booking_data['item_category'].")\"> ".$booking_data['booking_no']."<a/>";
									...
									{	
										...
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>"> 
											...
											<td style="word-break: break-all;" width="40" id="td_<? echo $i; ?>" style="cursor:pointer" align="center" onClick="generate_worder_report2(<? echo $type; ?>,'<? echo $row[csf('booking_no')]; ?>',<? echo $row[csf('company_id')]; ?>,'<? echo $row[csf('po_break_down_id')]; ?>',<? echo $row[csf('item_category')].','.$row[csf('fabric_source')]; ?>,'<? echo $row[csf('job_no')]; ?>','<? echo $row[csf('is_approved')]; ?>','show_fabric_booking_report3')"><? echo $i; ?></td>
											<td style="word-break: break-all;" width="70"><?=$lib_user[$row[csf('inserted_by')]];?></td>
											<td width="50" onClick="openmypage_image('requires/short_feb_booking_approval_controller.php?action=show_image&job_no=<?=$row[csf('job_no')]; ?>','Image View')"><img src='../<?=$imge_arr[$row[csf('job_no')]]; ?>' height='25' width='30' /></td>
											...
											<td style="word-break: break-all;" width="130" align="center">
												<a href='##'  onClick="generate_worder_report(<? echo $type; ?>,<? echo $report_type; ?>,'<? echo $row[csf('booking_no')]; ?>',<? echo $row[csf('company_id')]; ?>,'<? echo $row[csf('po_break_down_id')]; ?>',<? echo $row[csf('item_category')].','.$row[csf('fabric_source')]; ?>,'<? echo $row[csf('job_no')]; ?>','<? echo $row[csf('is_approved')]; ?>','<?=$report_action;?>',' <? echo $i; ?>')"><? echo $row[csf('booking_no')]; ?></a>
											</td>
											<td style="word-break: break-all;" width="70" align="center" style="word-break:break-all">&nbsp;&nbsp;<?=$variable1; ?></td>
											...
											<td style="word-break: break-all;" width="100" align="center"><? echo $row[csf('grouping')]; ?></td>
											...
											<td style="word-break: break-all;" width="80" align="center" style="word-break:break-all">
												<?=$job_wise_data[$row[csf('job_no')]]['brand_id']; ?>&nbsp;
											</td>
											<td style="word-break: break-all;" width="70" align="center" style="word-break:break-all">
...
if ($action=="approve")
{
	...
	$sql = "select a.ID,a.BUYER_ID,a.BRAND_ID,a.READY_TO_APPROVED,a.IS_APPROVED  from WO_BOOKING_MST a where a.COMPANY_ID=$cbo_company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and a.READY_TO_APPROVED=1 and a.item_category in(2,3,13)  and a.id in($booking_ids)";
	//echo $sql;die();
	...
	$finalDataArr = getFinalUser(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>12,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>$item_cat_arr,'lib_store_arr'=>0,'product_dept_arr'=>0,'match_data'=>$matchDataArr));
	 //print_r($finalDataArr['user_seq']);die;
	...
	if($approval_type==0)
	{
		...
		foreach($booking_ids_all as $mst_id)
		{
			...
			$data_array.="(".$id.",12,".$mst_id.",".$approved_no.",".$user_sequence_no.",1,".$user_id_approval.",'".$pc_date_time."','".$user_ip."',".$app_instru.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$approved.")"; 
			$id=$id+1;
			//App Mst........................
			if($app_data_array!=''){$app_data_array.=",";}
			$app_data_array.="(".$app_mst_id.",12,".$mst_id.",".$user_sequence_no.",".$user_group_no.",".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."',".$approved.")"; 
			$app_mst_id=$app_mst_id+1;
		
			//mst data.......................
			$data_array_up[$mst_id] =explode(",",("".$approved.",".$user_sequence_no.",".$user_group_no.",".$user_id_approval."")); 
		...
		if($flag==1) 
		{
			$field_array_booking_update = "is_approved*APPROVED_SEQU_BY*APPROVED_GROUP_BY*APPROVED_BY";
			$rID1=execute_query(bulk_update_sql_statement( "wo_booking_mst", "id", $field_array_booking_update, $data_array_up, $booking_id_arr));
			if($rID1) $flag=1; else $flag=0;
		}
		
		if($flag==1) 
		{
			$app_mst_field_array="id, entry_form, mst_id,sequence_no,group_no,approved_by, approved_date,INSERTED_BY,INSERT_DATE,user_ip,approved";
			$rID2=sql_insert("approval_mst",$app_mst_field_array,$app_data_array,0);
		...
		if($flag==1) 
		{
			...
			$rID4=sql_insert("approval_history",$field_array,$data_array,0);
			if($rID4) $flag=1; else $flag=0; 
		}
		
		  
		//echo "insert into approval_history ($field_array) values $data_array";oci_rollback($con); die;
	...
	else if($approval_type==1)
	{
		...
		foreach($booking_ids_all as $mst_id)
		{
			...
			//History................
			if($data_array!="") $data_array.=",";
			$data_array.="(".$id.",12,".$mst_id.",".$approved_no.",".$user_sequence_no.",0,".$user_id_approval.",'".$pc_date_time."','".$user_ip."',".$app_instru.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0)"; 
			$id=$id+1;
			$i++;	
		}
		...
		if($flag==1){
			$rID2=sql_multirow_update("wo_booking_mst","is_approved*ready_to_approved*APPROVED_SEQU_BY*APPROVED_GROUP_BY*APPROVED_BY","0*0*0*0*$user_id_approval","id",$booking_ids,0);
			if($rID2) $flag=1; else $flag=0;
		}
		...
		if($flag==1) 
		{
			$query="UPDATE fabric_sales_order_mst SET is_apply_last_update=2 WHERE within_group=1 and booking_without_order=0 and booking_id in ($booking_ids)";
		...
		$response=$booking_ids;
	 	...
	 	//echo "22**". $rID1."**".$rID2."**".$rID3."**".$rID4."**".$rID5."**".$data_array;oci_rollback($con);die;
	...
	else if($approval_type==5)//Deny
	{
		...
		foreach($booking_ids_all as $mst_id)
		{
			...
			//History................
			if($data_array!="") $data_array.=",";
			$data_array.="(".$id.",12,".$mst_id.",".$approved_no.",".$user_sequence_no.",0,".$user_id_approval.",'".$pc_date_time."','".$user_ip."',".$app_instru.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',2)"; 
			$id=$id+1;
			$i++;	
		}
		...
		if($flag==1){
			$rID2=sql_multirow_update("wo_booking_mst","is_approved*ready_to_approved*APPROVED_SEQU_BY*APPROVED_GROUP_BY*APPROVED_BY","2*0*0*0*$user_id_approval","id",$booking_ids,0);
			if($rID2) $flag=1; else $flag=0;
		}
		...
		if($flag==1) 
		{
		 ...
		 // echo "51**".$rID1."**".$rID2."**".$rID3."**".$flag;oci_rollback($con);die;
       ...
       <?php
        $custom_designation=return_library_array( "select id,custom_designation from lib_designation ",'id','custom_designation');	
		 $Department=return_library_array( "select id,department_name from  lib_department ",'id','department_name');	;
			...
			$sql="select a.id, a.user_name, a.department_id, a.user_full_name, a.designation,b.SEQUENCE_NO,b.GROUP_NO from  user_passwd a,electronic_approval_setup b where a.id=b.user_id and b.company_id=$cbo_company_name and b.entry_form=12 and valid=1 and a.id!=$user_id and b.is_deleted=0 order by sequence_no ASC";
			//echo $sql;
