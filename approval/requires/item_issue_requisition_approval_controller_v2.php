...
if($action=='user_popup')
{
	...
	<form>
	       ...
	       <?php
			...
			$sql="select a.id, a.user_name, a.department_id, a.user_full_name, a.designation from  user_passwd a,electronic_approval_setup b where a.id=b.user_id and b.company_id=$cbo_company_name and a.id!=$user_id  and b.is_deleted=0 and b.entry_form=56 order by sequence_no";
...
function getSequence($parameterArr=array()){
	...
	$sql="SELECT COMPANY_ID,PAGE_ID,USER_ID,BYPASS,SEQUENCE_NO,BUYER_ID,BRAND_ID,DEPARTMENT,ITEM_CATEGORY as ITEM_CATE_ID FROM ELECTRONIC_APPROVAL_SETUP WHERE COMPANY_ID = {$parameterArr['company_id']} AND ENTRY_FORM = {$parameterArr['entry_form']} AND IS_DELETED = 0 order by SEQUENCE_NO";
	 //echo $sql;die;
...
{   	?>
		...
		<script>
			function openmypage_reqdetails(requ_id,requ_no)
			{
				...
				var page_link = 'requires/item_issue_requisition_approval_controller_v2.php?data='+data+'&action=reqdetails_popup';
	...
	$electronicDataArr=getSequence(array('company_id'=>$company_name,'entry_form'=>56,'user_id'=>$user_id_approval,'lib_buyer_arr'=>0,'lib_brand_arr'=>0,'product_dept_arr'=>$lib_department_arr,'lib_item_cat_arr'=>$item_category,'lib_store_arr'=>$lib_store_arr));
 ...
 //var_dump($electronicDataArr[seq_store_id][5]);die;
	...
	if($approval_type==0)// Un-Approve
	{
		...
		// if($electronicDataArr['user_by'][$user_id_approval]['ITEM_CATE_ID']){
		// 	$where_condition .= " and c.ITEM_CATEGORY_ID in(".$electronicDataArr['user_by'][$user_id_approval]['ITEM_CATE_ID'].",0)";
		// 	$electronicDataArr['sequ_by'][0]['ITEM_CATE_ID']=$electronicDataArr['user_by'][$user_id_approval]['ITEM_CATE_ID'];
		// }
		...
		{ 
			for($seq=($electronicDataArr['user_by'][$user_id_approval]['SEQUENCE_NO']-1);$seq>=0; $seq-- ){
				//(in_array($row['ITEM_CATEGORY_ID'],explode(',',$electronicDataArr['sequ_by'][$seq]['ITEM_CATE_ID']))) && 
				
				if( (in_array($row['DEPARTMENT_ID'],explode(',',$electronicDataArr['sequ_by'][$seq]['DEPARTMENT'])) || $row['DEPARTMENT']==0)
                        && (in_array($row['STORE_ID'],explode(',',$electronicDataArr['sequ_by'][$seq]['STORE_ID'])) )
                    )
					{
	...
	$sql_cause="select MST_ID,REFUSING_REASON from refusing_cause_history where entry_form=56 ".where_con_using_array($sys_id_arr,0,'mst_id')." order by id asc";
    //echo $sql_cause;	
                            ...
                            foreach ($nameArray as $row)
                            {
								...
								?>
									<tr bgcolor="<?= $bgcolor; ?>" onClick="change_color('tr_<?= $i; ?>','<?= $bgcolor; ?>')" id="tr_<?= $i; ?>"> 
										...
										<td width="100" style="cursor:pointer;"><input name="comments_[]" class="text_boxes" readonly placeholder="Please Browse" id="comments_<?= $row['ID'];?>" onClick="openmypage_refusing_cause('requires/item_issue_requisition_approval_controller_v2.php?action=refusing_cause_popup','Comments','<?= $row['ID'];?>');" value="<?=$app_cause_arr[$row['ID']];?>"></td>	
...
// 	ALTER TABLE PLATFORMERPV3.INV_ITEM_ISSUE_REQUISITION_MST
//  ADD (approved_date  TIMESTAMP(0));

// ALTER TABLE PLATFORMERPV3.INV_ITEM_ISSUE_REQUISITION_MST
//  ADD (APPROVED_BY  NUMBER(11));
	
	if($approval_type==0)
	{
		 //$max_approved_no_arr = return_library_array("select APPROVED_BY,mst_id, max(approved_no) as approved_no from approval_history where mst_id in($requisition_ids) and entry_form=56 group by APPROVED_BY,mst_id","mst_id","approved_no");
		 
		 $sql="select APPROVED_BY,MST_ID, max(approved_no) as APPROVED_NO from approval_history where mst_id in($requisition_ids) and entry_form=56 group by APPROVED_BY,mst_id";
		 //echo $sql;die;
		 $sqlResult=sql_select( $sql );
		 foreach( $sqlResult as $row){
			$max_approved_no_arr[$row['MST_ID']][$row['APPROVED_BY']] =$row['APPROVED_NO'] ;
		 }
		 
		//------------------
		$sql="SELECT a.ID,a.STORE_ID, a.DEPARTMENT_ID,c.ITEM_CATEGORY_ID
			from inv_item_issue_requisition_mst a, inv_itemissue_requisition_dtls b, product_details_master c
		...
		$finalDataArr=getFinalUser(array('company_id'=>$cbo_company_name,'entry_form'=>56,'lib_buyer_arr'=>0,'lib_brand_arr'=>0,'lib_item_cat_arr'=>$item_category,'lib_store_arr'=>$lib_store_arr,'product_dept_arr'=>$lib_department_arr,'match_data'=>$matchDataArr));
	 ...
	 //---------------------
		...
		foreach($requisition_id_arr as $mst_id)
		{		
			if($data_array!=''){$data_array.=",";}
			$data_array.="(".$id.",56,".$mst_id.",".$user_sequence_no.",".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			
			
			$approved_no=$max_approved_no_arr[$mst_id][$user_id_approval]+1;
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$hmst_id.",56,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			$hmst_id=$hmst_id+1;
		...
		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=56 and mst_id in ($requisition_ids)";
		...
		// echo "21**$flag,".$rID.",".$rID2.",".$rID3;oci_rollback($con); die;
	...
	{ //echo 1;
		$rID1=sql_multirow_update("inv_item_issue_requisition_mst","is_approved*ready_to_approved*APPROVED_SEQU_BY",'0*0*0',"id",$requisition_ids,0);
		
		if($rID1) $flag=1; else $flag=0;

		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0,APPROVED=2, un_approved_by='".$user_id_approval."', un_approved_date='".$pc_date_time."', updated_by='".$user_id."', update_date='".$pc_date_time."' WHERE entry_form=56 and current_approval_status=1 and mst_id in ($requisition_ids)";
		...
		if($flag==1)
		{
			$query="UPDATE refusing_cause_history SET CURR_APP_STATUS=0  WHERE entry_form=56 and CURR_APP_STATUS=1 and mst_id in ($requisition_ids)";
		...
		$flag=1;
		$rID=sql_multirow_update("INV_ITEM_ISSUE_REQUISITION_MST","IS_APPROVED*APPROVED_SEQU_BY*READY_TO_APPROVED","0*0*0","id",$requisition_ids,0); 
		if($flag==1) 
		{
			if($rID) $flag=1; else $flag=0; 
		}

		$query="delete from approval_mst  WHERE entry_form=56 and mst_id in ($requisition_ids)";
		$rID2=execute_query($query,1); 
		if($flag==1) 
		{
		...
		$query="UPDATE approval_history SET current_approval_status=0, un_approved_by='".$user_id_approval."', un_approved_date='".$pc_date_time."', updated_by='".$user_id."', update_date='".$pc_date_time."' WHERE entry_form=56 and current_approval_status=1 and mst_id in ($requisition_ids)";
		$rID3=execute_query($query,1);
		if($flag==1)
		{
			if($rID3) $flag=1; else $flag=0;
...
if($action=="refusing_cause_popup")
{
	...
	$sql_cause="SELECT MST_ID,REFUSING_REASON from refusing_cause_history where entry_form=56 and mst_id='$quo_id' and INSERTED_BY=$app_user_id order by id asc";	
	$nameArray_cause=sql_select($sql_cause);
	$app_cause_arr=array();
	foreach($nameArray_cause as $row)
	{
	...
	function fnc_cause_info( operation )
	{
		...
		else
		{
			...
			http.open("POST","item_issue_requisition_approval_controller_v2.php",true);
...
if($action=="save_update_delete_refusing_cause")
{
	...
	// if(is_duplicate_field( "approval_cause", "approval_cause_refusing_his", "approval_cause='".$refusing_cause."' and entry_form=56 and booking_id='".str_replace("'", "", $quo_id)."' and approval_type=1 and status_active=1 and is_deleted=0" )==1)
	// {
	// 	//
	// }
	// else
	...
	// 	$idpre=return_field_value("max(id) as id", "refusing_cause_history", "mst_id=".$quo_id." and entry_form=56 group by mst_id","id");
	...
	// 			select '', id, entry_form, mst_id, 1, refusing_reason, inserted_by, insert_date, updated_by, update_date from refusing_cause_history where mst_id=".$quo_id." and entry_form=56 and id=$idpre"; //die;
	...
	if ($operation==0)  // Insert Here
	{
		...
		$data_array = "(".$id.",56,".$quo_id.",'".$refusing_cause."',".$app_user_id.",'".$pc_date_time."')";
...
if($action=='app_mail_notification'){
	...
	$user_sequence_no=return_field_value("sequence_no","electronic_approval_setup","company_id=$company_name and entry_form=56 and user_id=$approved_user_id and is_deleted=0");

	$mailToArr=array();
	if($email!=''){$mailToArr[]=$email;}

	$elcetronicSql = "SELECT a.BUYER_ID,a.SEQUENCE_NO,a.BYPASS,b.USER_EMAIL  from electronic_approval_setup a join user_passwd b on a.user_id=b.id where b.valid=1 AND a.IS_DELETED=0 and a.entry_form=56 and a.company_id=$company_name and a.SEQUENCE_NO > $user_sequence_no order by a.SEQUENCE_NO"; //and a.page_id=2150 and a.entry_form=46
