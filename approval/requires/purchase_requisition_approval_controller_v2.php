...
function getSequence($parameterArr=array()){
	...
	$sql="SELECT COMPANY_ID,PAGE_ID,USER_ID,BYPASS,SEQUENCE_NO,BUYER_ID,BRAND_ID,DEPARTMENT as DEPARTMENT_ID,ITEM_CATEGORY as ITEM_ID,LOCATION as LOCATION_ID FROM ELECTRONIC_APPROVAL_SETUP WHERE COMPANY_ID = {$parameterArr['company_id']} AND PAGE_ID = {$parameterArr['page_id']} AND IS_DELETED = 0 order by SEQUENCE_NO";
	 //echo $sql;die;
...
if ($action=="load_drop_down_store")
{
	...
	echo create_drop_down( "cbo_store_id", 130, "select id,store_name from lib_store_location where status_active=1 and is_deleted=0 and company_id=$data $storCon order by store_name","id,store_name", 1, "-- All --","","load_drop_down( 'requires/purchase_requisition_approval_controller_v2', this.value, 'load_drop_down_item', 'category_id' );",0,"","","","");
	exit();
}
...
if($action=="report_generate")
{
	...
	<script>
		function openmypage_app_cause(wo_id,app_type,i)
		{
			...
			var page_link = 'requires/purchase_requisition_approval_controller_v2.php?data='+data+'&action=appcause_popup';
		...
		function openmypage_reqdetails(requ_id,requ_no)
		{
			...
			var page_link = 'requires/purchase_requisition_approval_controller_v2.php?data='+data+'&action=reqdetails_popup';
	...
	if ($txt_date_from != '' && $txt_date_to != '')
	{
		...
		if($db_type==0)
		{
			if ($cbo_req_year != 0) $searchCon.= " and year(a.insert_date)=$cbo_req_year";
		}
		else
		{
	...
	$electronicDataArr=getSequence(array('company_id'=>$company_name,'page_id'=>$menu_id,'user_id'=>$app_user_id,'lib_buyer_arr'=>0,'lib_brand_arr'=>0,'lib_item_cat_arr'=>$item_cat_arr,'lib_store_arr'=>$lib_store_arr,'lib_department_id_arr'=>$department_arr,'category_mixing'=>$category_mixing_variable,'lib_location_id_arr'=>$location_arr));
	...
	// $sql_unapproved=sql_select("select * from fabric_booking_approval_cause where  entry_form=1  and is_deleted=0 and status_active=1 and BOOKING_ID in(".implode(',',$mst_id_arr).")");
	...
	//echo "select * from fabric_booking_approval_cause where  entry_form=1  and is_deleted=0 and status_active=1 and PAGE_ID in(".implode(',',$mst_id_arr).")";
...
if ($action == "approve")
{
	...
	$txt_appv_cause_arr = explode('**',$txt_appv_causes);
	 ...
	 if($approval_type==5)
	 {
		...
		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0, un_approved_by='".$app_user_id."', un_approved_date='".$pc_date_time."', updated_by='".$user_id."', update_date='".$pc_date_time."' WHERE entry_form=1 and current_approval_status=1 and mst_id in ($target_ids)";
		...
		if($flag == 1 && $_SESSION['app_notification'] == 1)
		{
			...
			foreach($reqs_ids as $ref_id)
			{
				...
				$appr_data = array("USER_ID"=>'',"SEQUENCE_NO" =>'',"COMPANY_ID"=> '',"NOTIFICATION_TYPE"=>0,'approval_desc'=>$approval_desc,'approval_data'=>$approval_data);
	...
	{      
		...
		{		
			...
			$mst_data_array.="(".$id.",1,".$mst_id.",".$user_sequence_no.",".$app_user_id.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
	...
	{              
		$history_data_arr = return_library_array( "select id, id from approval_history where current_approval_status=1 and entry_form=1 and mst_id in ($target_ids)",'id','id');
		$app_ids= implode(',',$history_data_arr);

		
		$flag=1;
		$rID=sql_multirow_update("inv_purchase_requisition_mst","is_approved*ready_to_approve*APPROVED_SEQU_BY","0*0*0","id",$target_ids,0); 
		if($flag==1) 
		{
			if($rID) $flag=1; else $flag=0; 
		}
		...
		//-----------------------History
		$rID3=sql_multirow_update("inv_purchase_requisition_mst","is_approved*ready_to_approve",'0*2',"id",$target_ids,0);
		if($rID3) $flag=1; else $flag=0;
		...
		$data=$app_user_id."*'".$pc_date_time."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*0";
		$rID5=sql_multirow_update("approval_history","un_approved_by*un_approved_date*updated_by*update_date*APPROVED",$data,"id",$app_ids,1);
		if($flag==1) 
		{
			if($rID5) $flag=1; else $flag=0; 
		} 
...
echo $receive_array[$row['CONS_RATE']] */
	...
	<script>
		...
		function fn_requisition_details_qtyupdate(operation)
		{
			...
			http.open("POST","purchase_requisition_approval_controller_v2.php",true);
...
if ($action=="send_requisition_app_mail")
{
	...
	//  echo $data;
		...
		$user_sequence_no=return_field_value("sequence_no","electronic_approval_setup","entry_form=1 and user_id=$app_user_id and company_id ={$sql_dtls[0][COMPANY_ID]} and is_deleted = 0");

		$elcetronicSql = "SELECT a.BUYER_ID,a.SEQUENCE_NO,a.BYPASS,b.USER_EMAIL  from electronic_approval_setup a join user_passwd b on a.user_id=b.id where b.valid=1  AND a.entry_form=1 and a.SEQUENCE_NO>$user_sequence_no and a.company_id={$sql_dtls[0][COMPANY_ID]} order by a.SEQUENCE_NO"; //and a.page_id=2150 and a.entry_form=46
