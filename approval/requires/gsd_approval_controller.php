...
if($action=="report_generate")
{
 ...
 group_concat(case when bypass=2 and buyer_id<>'' then buyer_id end) as buyer_ids from electronic_approval_setup where company_id=$cbo_company_name and page_id=$menu_id and sequence_no<$user_sequence_no and is_deleted=0";
			...
			{                   
                $sql = "select a.id,a.company_id,a.buyer_id,b.buyer_name,a.system_no,a.po_job_no,a.style_ref,a.gmts_item_id from ppl_gsd_entry_mst a, wo_po_details_master b where a.po_job_no = b.job_no and a.company_id = $company_name and a.is_deleted=0 and a.status_active=1 and a.ready_to_approved = 1  $buyer_id_cond $buyer_id_cond2 $buyerIds_cond group by a.id,a.company_id,a.buyer_id,a.system_no,a.po_job_no,a.style_ref,a.gmts_item_id,a.is_approved,a.update_date, a.insert_date,b.buyer_name order by $orderBy_cond(a.update_date, a.insert_date) desc";	                             
            }
		...
		{   
			...
			{                  
                ...
                $sql = "select a.id,a.company_id,a.buyer_id,b.buyer_name,a.system_no,a.po_job_no,a.style_ref,a.gmts_item_id,a.is_approved, c.id as approval_id, c.sequence_no, c.approved_by from ppl_gsd_entry_mst a, wo_po_details_master b, approval_history c where a.id=c.mst_id and c.entry_form=23 and c.current_approval_status=1 and a.po_job_no = b.job_no and a.company_id = $company_name and a.is_deleted=0 and a.status_active=1 and a.ready_to_approved = 1 and a.is_approved=1 $buyer_id_cond $buyer_id_cond2 $sequence_no_cond group by a.id,a.company_id,a.buyer_id,a.system_no,a.po_job_no,a.style_ref,a.gmts_item_id,a.is_approved,a.update_date, a.insert_date,b.buyer_name,c.id, c.sequence_no, c.approved_by order by $orderBy_cond(a.update_date, a.insert_date) desc";	                
...
if ($action=="approve")
{
	...
	{      
	   ...
	   // $approved_status_arr = return_library_array("select id, is_approved from wo_booking_mst where id in($booking_ids)","id","is_approved");
		...
		{		
			...
			$data_array.="(".$id.",23,".$val.",".$approved_no.",'".$user_sequence_no."',1,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			$i++;
		}
	...
	{              
        $rID=sql_multirow_update("ppl_gsd_entry_mst","is_approved*ready_to_approved","0*0","id",$target_ids,0); 

        if($rID) $flag=1; else $flag=0;

		$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=23 and mst_id in ($target_ids)";
