...
{
	echo create_drop_down( "cbo_buyer_name", 152, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,30,90)) order by buy.buyer_name","id,buyer_name", 1, "-- All Buyer --", $selected, "load_drop_down( 'requires/sample_requisition_approval_group_by_controller', this.value, 'load_drop_down_season_buyer', 'season_td');" );  
	exit();
}
...
if($action=="report_generate")
{
	...
	$electronicDataArr=getSequence(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>25,'user_id'=>$app_user_id,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'product_dept_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0));
	...
	{ 
		...
		if($electronicDataArr['user_by'][$app_user_id]['BUYER_ID']){
			$where_con .= " and a.BUYER_NAME in(".$electronicDataArr['user_by'][$app_user_id]['BUYER_ID'].",0)";
		...
		$data_mast_sql ="select a.ID,a.company_id,a.requisition_number_prefix_num,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY,a.BUYER_NAME as BUYER_ID  from sample_development_mst a where a.entry_form_id in(449,117,203) and a.company_id=$company_name  and a.STATUS_ACTIVE=1 and a.IS_DELETED=0  and a.IS_APPROVED<>1 $where_con and a.req_ready_to_approved=1";
		...
		for($group=0;$group<=$my_group; $group++ ){
			foreach($electronicDataArr['group_seq_arr'][$group] as $seq){
				...
				if(count($tmp_sys_id_arr[$group][$seq])){
					...
					$sql .= "select a.id,a.company_id,a.requisition_date,a.requisition_number_prefix_num,a.style_ref_no,a.buyer_name,a.season,a.product_dept,a.dealing_marchant,a.agent_name,a.buyer_ref,a.bh_merchant,estimated_shipdate,a.remarks,a.status_active,a.is_deleted from sample_development_mst a where a.entry_form_id in(449,117,203) and a.company_id=$company_name  $buyer_id_cond $buyer_id_cond2 $season_cond $style_id_cond $req_id_cond $date_cond $cbo_year_cond and a.APPROVED_SEQU_BY=$seq and a.APPROVED_GROUP_BY=$group $sys_con and  a.is_approved<>1 and a.req_ready_to_approved=1 and a.status_active=1 and a.is_deleted=0";
				}
	...
	{   
		...
		$sql = "select a.id,a.company_id,a.requisition_date,a.requisition_number_prefix_num,a.style_ref_no,a.buyer_name,a.season,a.product_dept,a.dealing_marchant,a.agent_name,a.buyer_ref,a.bh_merchant,estimated_shipdate,a.remarks,a.status_active,a.is_deleted from sample_development_mst a,APPROVAL_MST b where a.entry_form_id in(449,117,203) and a.company_id=$company_name  $buyer_id_cond $buyer_id_cond2 $season_cond $style_id_cond $req_id_cond $date_cond $cbo_year_cond  and a.APPROVED_GROUP_BY=b.GROUP_NO  and b.GROUP_NO={$electronicDataArr['user_by'][$app_user_id]['GROUP_NO']} and  a.is_approved<>0 and a.req_ready_to_approved=1 and a.status_active=1 and a.is_deleted=0 and a.id=b.mst_id"; 
	...
	?>
	   <script>
		function openmypage_app_cause(wo_id,app_type,i)
		{
			...
			var page_link = 'requires/sample_requisition_approval_group_by_controller.php?data='+data+'&action=appcause_popup';
        ...
        <legend>Sample Requisition Approval</legend>	
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1060" class="rpt_table" >
                <thead>
					...
					<th width=""><? if($approval_type==0) echo "Not Appv. Cause" ; else echo "Not Un-Appv. Cause"; ?></th>
                </thead>
            </table>
            <div style="width:1060px; overflow-y:scroll; max-height:380px;" id="buyer_list_view">
                <table align="left" cellspacing="0" cellpadding="0" border="1" rules="all" width="1040" class="rpt_table" id="tbl_list_search">
                    <tbody>
                        <? 
                            $i=1;
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1060" class="rpt_table">
				<tfoot>
                    ...
                    <td colspan="2" align="left"><input type="button" value="<? if($approval_type==1) echo "Un-Approve"; else echo "Approve"; ?>" class="formbutton" style="width:100px" onClick="submit_approved(<? echo $i; ?>,<? echo $approval_type; ?>)"/>&nbsp;&nbsp;&nbsp;<input type="button" value="<? if($approval_type==0) echo "Deny"; ?>" class="formbutton" style="width:100px;<?=$denyBtn; ?>" onClick="submit_approved(<?=$i; ?>,5);"/></td>
				</tfoot>
			</table>
        </fieldset>
    </form>         
...
if ($action=="approve")
{
	...
	$sql = "SELECT a.ID,a.IS_APPROVED,a.APPROVED_SEQU_BY,a.BUYER_NAME as BUYER_ID  from sample_development_mst a   where  a.COMPANY_ID=$cbo_company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0  and a.IS_APPROVED<>1 and a.REQ_READY_TO_APPROVED=1 and a.id in($req_ids) and a.entry_form_id in(449,117,203)";
	...
	$finalDataArr=getFinalUser(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>25,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0,'product_dept_arr'=>0,'match_data'=>$matchDataArr));
   ...
   // print_r($user_sequence_no);die;
	...
	$max_approved_no_arr = return_library_array("select mst_id, max(approved_no) as approved_no from approval_history where mst_id in($req_ids) and entry_form=25 group by mst_id","mst_id","approved_no");
	...
	{ 
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			...
			$data_array.="(".$id.",25,'".$mst_id."','".$user_sequence_no."','".$user_group_no."',".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			
		
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",25,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,".$approved.")";
			$ahid++;
		...
		if($flag==1) 
		{
			...
			$usql = bulk_update_sql_statement( "sample_development_mst", "id", $field_array_up, $data_array_up, $target_app_id_arr );
		...
		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=25 and mst_id in ($req_ids)";
		...
		if(count($approved_no_array)>0 && $flag)
		{
			...
			$sql_insert="insert into sample_development_mst_history(id,hist_mst_id,approved_no,requisition_number_prefix,requisition_number_prefix_num,requisition_number,sample_stage_id,requisition_date,quotation_id,style_ref_no,company_id,location_id,buyer_name,season,product_dept,dealing_marchant,agent_name,buyer_ref,bh_merchant,estimated_shipdate,remarks,inserted_by,insert_date,status_active,is_deleted,entry_form_id,updated_by,update_date) 
			 ...
			 $sql_insert_dtls="insert into sample_development_dtls_hist(id,approved_no,sample_mst_id,sample_name,gmts_item_id,smv,article_no,sample_color,sample_prod_qty,submission_qty,delv_start_date,delv_end_date,sample_charge,sample_curency,inserted_by,insert_date,status_active,is_deleted,entry_form_id,size_data,fabric_status,acc_status,embellishment_status,fab_status_id,acc_status_id,embellishment_status_id,updated_by,update_date) 
				select	
				'', $approved_string_dtls,sample_mst_id,sample_name,gmts_item_id,smv,article_no,sample_color,sample_prod_qty,submission_qty,delv_start_date,delv_end_date,sample_charge,sample_curency,inserted_by,insert_date,status_active,is_deleted,entry_form_id,size_data,fabric_status,acc_status,embellishment_status,fab_status_id,acc_status_id,embellishment_status_id,updated_by,update_date from  sample_development_dtls where sample_mst_id in ($req_ids) and entry_form_id in(449,117,203)";
	...
	{              
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			...
			$history_data_array.="(".$ahid.",25,".$mst_id.",'".$approved_no."','".$user_sequence_no."',0,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,2)";
			$ahid++;
        }

		

		$rID1=sql_multirow_update("sample_development_mst","IS_APPROVED*REQ_READY_TO_APPROVED*APPROVED_SEQU_BY*APPROVED_GROUP_BY",'2*0*0*0',"id",$req_ids,0);
		if($rID1) $flag=1; else $flag=0;
		...
		if($flag==1) 
		{
			$query="delete from approval_mst  WHERE entry_form=25 and mst_id in ($req_ids)";
			$rID3=execute_query($query,1); 
			if($rID3) $flag=1; else $flag=0; 
		}
	...
	{              
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			$approved_no = $max_approved_no_arr[$mst_id]*1;
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",25,".$mst_id.",".$approved_no.",'".$user_sequence_no."',0,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			$ahid++;
        }

		//echo $req_ids;die;

		$rID1=sql_multirow_update("sample_development_mst","IS_APPROVED*REQ_READY_TO_APPROVED*APPROVED_SEQU_BY*APPROVED_GROUP_BY",'0*0*0*0',"id",$req_ids,0);
		if($rID1) $flag=1; else $flag=0;
...
if ($action=="appcause_popup")
{
	...
	if($app_cause=="")
	{
		$sql_cause="select MAX(id) as id from fabric_booking_approval_cause where page_id='$menu_id' and entry_form=25 and user_id='$user_id' and booking_id='$wo_id' and approval_type=$app_type and status_active=1 and is_deleted=0";
    ...
    <script>
		...
		function fnc_appv_entry(operation)
		{
			...
			else
			{
				...
				http.open("POST","sample_requisition_approval_group_by_controller.php",true);
...
if ($action=="save_update_delete_appv_cause")
{
	...
	if($approval_type==0)
	{
		...
		if ($operation==0)  // Insert Here
		{
			...
			$approved_no_history=return_field_value("approved_no","approval_history","entry_form=25 and mst_id=$wo_id and approved_by=$user_id");
			$approved_no_cause=return_field_value("approval_no","fabric_booking_approval_cause","entry_form=25 and booking_id=$wo_id and user_id=$user_id and approval_type=$appv_type");
			...
			if($approved_no_history=="" && $approved_no_cause=="")
			{
				...
				$data_array="(".$id_mst.",".$page_id.",25,".$user_id.",".$wo_id." ,".$appv_type.",0,".$appv_cause.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			...
			else if($approved_no_history=="" && $approved_no_cause!="")
			{
				...
				$id_cause=return_field_value("max(id)","fabric_booking_approval_cause","entry_form=25 and booking_id=$wo_id and user_id=$user_id and approval_type=$appv_type");
				...
				$data_array="".$page_id."*25*".$user_id."*".$wo_id."*".$appv_type."*0*".$appv_cause."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0";
			...
			{
				$max_appv_no_his=return_field_value("max(approved_no)","approval_history","entry_form=25 and mst_id=$wo_id and approved_by=$user_id");
				$max_appv_no_cause=return_field_value("max(approval_no)","fabric_booking_approval_cause","entry_form=25 and booking_id=$wo_id and user_id=$user_id and approval_type=$appv_type");
				...
				if($max_appv_no_his!=$max_appv_no_cause)
				{
					...
					$field_array="id,page_id,entry_form,user_id,booking_id,approval_type,approval_no,approval_cause,inserted_by,insert_date,status_active,is_deleted";
					$data_array="(".$id_mst.",".$page_id.",25,".$user_id.",".$wo_id." ,".$appv_type.",".$max_appv_no_his.",".$appv_cause.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
					$rID=sql_insert("fabric_booking_approval_cause",$field_array,$data_array,0);
					//echo $rID; die;
				...
				else if($max_appv_no_his==$max_appv_no_cause)
				{
					...
					$field_array="page_id*entry_form*user_id*booking_id*approval_type*approval_no*approval_cause*updated_by*update_date*status_active*is_deleted";
					$data_array="".$page_id."*25*".$user_id."*".$wo_id."*".$appv_type."*".$max_appv_no_his."*".$appv_cause."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0";
	...
	if($approval_type==1)
	{
		...
		if ($operation==0)  // Insert Here
		{
			...
			$unapproved_cause_id=return_field_value("id","fabric_booking_approval_cause","entry_form=25 and user_id=$user_id and booking_id=$wo_id  and approval_type=$appv_type and approval_history_id=$approval_id");
			...
			if($unapproved_cause_id=="")
			{
				...
				$data_array="(".$id_mst.",".$page_id.",25,".$user_id.",".$wo_id." ,".$appv_type.",".$max_appv_no_his.",".$approval_id.",".$appv_cause.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			...
			else
			{
				...
				$id_cause=return_field_value("max(id)","fabric_booking_approval_cause","entry_form=25 and user_id=$user_id and booking_id=$wo_id  and approval_type=$appv_type and approval_history_id=$approval_id");
				...
				$data_array="".$page_id."*25*".$user_id."*".$wo_id."*".$appv_type."*".$max_appv_no_his."*".$approval_id."*".$appv_cause."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0";
