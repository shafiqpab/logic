...
{
	$load="load_drop_down( 'requires/fabric_booking_approval_controller_v2', this.value, 'load_drop_down_season', 'season_td'); load_drop_down( 'requires/fabric_booking_approval_controller_v2', this.value, 'load_drop_down_brand', 'brand_td');";
...
{   
    ...
    $electronicDataArr=getSequence(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>7,'user_id'=>$user_id_approval,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>$buyer_brand_arr,'product_dept_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0));
       //print_r( $buyer_arr);
	...
	if($cbo_buyer_name){$buyer_con .= " and a.BUYER_ID =".$cbo_buyer_name.""; }
	...
	{  
        ...
        $data_mast_sql = "select a.ID, a.BUYER_ID from wo_booking_mst a where a.COMPANY_ID=$company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0  and a.IS_APPROVED<>1 and a.READY_TO_APPROVED=1  $where_con";
	...
	{   
		...
		$approved_user_cond=" and c.approved_by='$user_id'";
				$sql .= "select a.id,a.entry_form, $year_field, a.booking_no_prefix_num, a.booking_no, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.supplier_id, a.delivery_date, a.booking_date, '0' as approval_id, a.is_approved, a.is_apply_last_update,(select max(c.approved_no) as revised_no from approval_history c where a.id=c.mst_id $approved_user_cond) as revised_no from wo_booking_mst a, wo_booking_dtls b , wo_po_break_down c,APPROVAL_MST d  where a.booking_no=b.booking_no and b.job_no=c.job_no_mst and a.company_id=$company_name and a.is_short in(2,3) and a.booking_type=1 and a.item_category in(2,3,13) and a.is_deleted=0 and a.status_active=1  and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 and a.is_approved<>0 and a.APPROVED_SEQU_BY=d.SEQUENCE_NO and  d.SEQUENCE_NO={$electronicDataArr['user_by'][$user_id_approval]['SEQUENCE_NO']} and a.APPROVED_SEQU_BY=d.SEQUENCE_NO and  d.entry_form=7  and b.fin_fab_qnty>0 $internal_ref_cond  $booking_no_cond $date_cond $booking_year_cond  $season_cond_3 $season_year_cond_3 $where_con $buyer_con group by a.id, a.booking_no, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id,a.ready_to_approved, a.APPROVED_SEQU_BY, a.supplier_id, a.delivery_date, a.booking_date, a.is_approved, a.is_apply_last_update, a.insert_date, a.update_date, a.booking_no_prefix_num,a.entry_form ";
	...
	$sql_cause="select MST_ID,REFUSING_REASON from refusing_cause_history where entry_form=7 ".where_con_using_array($sys_id_arr,0,'mst_id')." order by id asc";
    //echo $sql_cause;	
    ...
    <script>
		...
		{   
			...
			var page_link = 'requires/fabric_booking_approval_controller_v2.php?data='+data+'&action=appcause_popup&txt_alter_user_id='+txt_alter_user_id;
		...
		function openmypage_app_instrac(wo_id,app_type,i)
		{
			...
			var page_link = 'requires/fabric_booking_approval_controller_v2.php?data='+data+'&action=appinstra_popup';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=500px,height=250px,center=1,resize=1,scrolling=0','');
			emailwindow.onclose=function()
			{
				var appv_cause=this.contentDoc.getElementById("hidden_appv_cause");
		...
		function openmypage_unapp_request(wo_id,app_type,i)
		{
			...
			var page_link = 'requires/fabric_booking_approval_controller_v2.php?data='+data+'&action=unappcause_popup';
					...
					//	 echo $sql;
                            ...
                            { 
								...
								else
								{
                                   ...
                                   // elseif($row[csf('is_short')]==2) $booking_type="Partial";
								...
								if($print_cond==1)
								{
									...
									if($row[csf('entry_form')]==118){
										...
										$variable="<a href='#' onClick=\"generate_worder_report('".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$all_po_id."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$all_job_no."','".$row[csf('is_approved')]."','".$row_id."','".$row[csf('entry_form')]."','show_fabric_booking_report22','".$i."',".$fabric_nature.")\"> ".$row[csf('booking_no_prefix_num')]."<a/>";
								...
								if($row[csf('entry_form')]==108){
									...
									<tr bgcolor="<?=$bgcolor; ?>" onClick="change_color('tr_<?=$i; ?>','<?=$bgcolor; ?>');" id="tr_<?=$i; ?>">
                                        ...
                                        ?>
                                        <td width="100" style="cursor:pointer;"><input name="comments_[]" class="text_boxes" readonly placeholder="Please Browse" id="comments_<?= $row['ID'];?>" style="width:100px" onClick="openmypage_refusing_cause('requires/fabric_booking_approval_controller_v2.php?action=refusing_cause_popup','Comments','<?= $row['ID'];?>');" value="<?=$app_cause_arr[$row['ID']];?>"></td>
...
{ 
	...
	$finalDataArr=getFinalUser(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>7,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>$item_cat_arr,'lib_store_arr'=>0,'product_dept_arr'=>0,'match_data'=>$matchDataArr));
	...
	{ 
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			...
			$data_array.="(".$id.",7,'".$mst_id."','".$user_sequence_no."',".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			
	 
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",7,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1)";
			$ahid++;
	...
	{              
		...
		$next_user_app = sql_select("select id from approval_history where mst_id in($booking_ids) and entry_form=7 and sequence_no >$user_sequence_no and current_approval_status=1 group by id");
		...
		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0, un_approved_by='".$user_id_approval."', un_approved_date='".$pc_date_time."', updated_by='".$user_id."', update_date='".$pc_date_time."' WHERE entry_form=7 and current_approval_status=1 and mst_id in ($booking_ids)";
...
if($action=='user_popup')
{
	...
	<form>
       ...
       <?php
			...
			$sql="select a.id, a.user_name, a.department_id, a.user_full_name, a.designation,b.SEQUENCE_NO,b.GROUP_NO from  user_passwd a,electronic_approval_setup b where a.id=b.user_id and b.company_id=$cbo_company_name  and valid=1 and a.id!=$user_id  and b.is_deleted=0 and b.entry_form=7 order by b.sequence_no";
			//echo $sql;die;
		...
		<div style="width:100%; word-wrap:break-word" id="scroll_body">
             <table border="0" rules="all" width="100%" cellpadding="2" cellspacing="2">
             	<tr>
					...
					$sql="select image_location from common_photo_library where master_tble_id='$job_no' and file_type=1";
    ...
    <script>
		...
		function fnc_appv_entry(operation)
		{
			...
			else
			{
				...
				http.open("POST","fabric_booking_approval_controller_v2.php",true);
...
if($action=="refusing_cause_popup")
{
	...
	$sql_cause="SELECT MST_ID,REFUSING_REASON from refusing_cause_history where entry_form=7 and mst_id='$quo_id' and INSERTED_BY=$app_user_id order by id asc";	
	$nameArray_cause=sql_select($sql_cause);
	$app_cause_arr=array();
	foreach($nameArray_cause as $row)
	{
...
if($action=="save_update_delete_refusing_cause")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		$data_array = "(".$id.",7,".$quo_id.",'".$refusing_cause."',".$app_user_id.",'".$pc_date_time."')";
	...
	else if ($operation==1)  // Update Here
	{
		...
		$idpre=return_field_value("max(id) as id", "refusing_cause_history", "mst_id=".$quo_id." and entry_form=7 group by mst_id","id");
...
if($action=="show_fabric_booking_report_print23")//Print B23=>13-03-2022(md mamun ahmed sagor)
{
	...
	*/		
		...
		if($to!=""){
			...
			$header=mailHeader();
			sendMailMailer( $to, $subject, $emailBody, $from_mail );
		}
	}
	exit();
}
...
if($action=='send_app_unapp_notification'){
	list($sysId,$mailId,$txt_alter_user_id)=explode('__',$data);
	...
	$mailArr=array();
	if($mailId){$mailArr[]=str_replace('*',',',$mailId);}
	...
	$sql="select a.ID,a.IS_APPROVED,a.REVISED_NO,a.COMPANY_ID,A.BOOKING_NO,A.BUYER_ID,a.BOOKING_DATE,a.DELIVERY_DATE,a.INSERTED_BY from wo_booking_mst a where a.is_short in(2,3) and a.booking_type=1 and a.item_category in(2,3,13) and a.status_active=1 and a.is_deleted=0 and a.id in($sysId)";
	// echo $sql;die;
	$sql_res=sql_select($sql);
	foreach($sql_res as $row)
	{
		
		if($user_maill_arr[$row['INSERTED_BY']] ==''){
			$mailArr[$user_maill_arr[$row['INSERTED_BY']]] = $user_maill_arr[$row['INSERTED_BY']];
		}

		

		$electronic_data_arr = array();
		$sql2 = "SELECT a.MAIL_TYPE,c.EMAIL_ADDRESS FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id and a.mail_item=87 and a.MAIL_TYPE in(3,4,8,7,6) and b.mail_user_setup_id=c.id and a.company_id=".$row['COMPANY_ID']."  and  c.IS_DELETED=0 and c.STATUS_ACTIVE=1";
		//echo $sql;die;
		$mail_sql=sql_select($sql2);
		foreach($mail_sql as $rows)
		{
			$electronic_data_arr[$rows['MAIL_TYPE']][$rows['EMAIL_ADDRESS']]=$rows['EMAIL_ADDRESS'];
		}
		if($row['IS_APPROVED'] == 1 && $electronic_data_arr[3]){$mailArr[] = implode(',',$electronic_data_arr[3]);}
		if($row['IS_APPROVED'] == 3 && $electronic_data_arr[4]){$mailArr[] = implode(',',$electronic_data_arr[4]);}
		if($row['IS_APPROVED'] == 0 && $electronic_data_arr[7]){$mailArr[] = implode(',',$electronic_data_arr[7]);}
		if($row['IS_APPROVED'] == 2 && $electronic_data_arr[6]){$mailArr[] = implode(',',$electronic_data_arr[6]);}
		...
		$user_sequence_no=return_field_value("sequence_no","electronic_approval_setup","company_id={$row['COMPANY_ID']} and entry_form=7 and user_id=$user_id_approval and is_deleted=0");

		$elcetronicSql = "SELECT a.BUYER_ID,a.SEQUENCE_NO,a.BYPASS,b.USER_EMAIL  from electronic_approval_setup a join user_passwd b on a.user_id=b.id where b.valid=1 AND a.IS_DELETED=0 and a.entry_form=7 and a.company_id={$row['COMPANY_ID']} and a.SEQUENCE_NO > $user_sequence_no order by a.SEQUENCE_NO";
		...
		foreach($elcetronicSqlRes as $rows){
			...
			if($rows['BUYER_ID']!=''){
				foreach(explode(',',$rows['BUYER_ID']) as $bi){
					if($rows['USER_EMAIL']!='' && $bi==$row['BUYER_ID']){$mailArr[]=$rows['USER_EMAIL'];}
					if($rows['BYPASS']==2){break;}
				}
			}
			else{
				if($rows['USER_EMAIL']){$mailArr[]=$rows['USER_EMAIL'];}
				if($rows['BYPASS']==2){break;}
			}
		}
		 
		$max_approved_no_arr = return_library_array("select mst_id, max(approved_no) as approved_no from approval_history where mst_id ={$row['ID']} and entry_form=7 group by mst_id","mst_id","approved_no");

		if($max_approved_no_arr > 1 && $row['IS_APPROVED'] == 1 && count($electronic_data_arr[8])){
			$mailArr[] = implode(',',$electronic_data_arr[8]);
			$revised_title ="[Revised]";
		}
		
		$subject="Fabric Booking ".$app_status_arr[$row['IS_APPROVED']]." ".$revised_title;
		...
		?>
		Dear Concerned,	<br />			
		Please check <?= $app_status_arr[$row['IS_APPROVED']];?> booking list.				
		...
		<table rules="all" border="1">
			<tr bgcolor="#CCCCCC">
				...
				<td>Booking Date</td>
				<td>Delivery Date</td>
				<td><?= $app_status_arr[$row['IS_APPROVED']];?> Date</td>
				<td>Revised No</td>
			</tr>
			<?php 
			$i=1;
			foreach($sql_res as $row2){ 
				?>
				<tr>
					<td><?= $i;?></td>
					<td><?= $row2['BOOKING_NO']?></td>
					<td><?= $buyer_arr[$row['BUYER_ID']]?></td>
					<td><?= change_date_format($row2['BOOKING_DATE'])?></td>
					<td><?= change_date_format($row2['DELIVERY_DATE'])?></td>
					<td><?= date('d-m-Y',time());?></td>
					<td><?= $max_approved_no_arr[$row2['ID']] ?></td>
				</tr>
			<?php } ?>
		</table>
		<?	
		...
		$to=implode(',',$mailArr);
		 ...
		 // echo $to ."--".$message;die;
		//$to .= ",al.mamun@logicsoftbd.com";
		
		 //echo $subject;die;
		// $to="reza@logicsoftbd.com,al.mamun@logicsoftbd.com";
