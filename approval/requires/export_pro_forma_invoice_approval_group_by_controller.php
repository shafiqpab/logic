...
{
	echo create_drop_down( "cbo_buyer_name", 150, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,30,90)) order by buy.buyer_name","id,buyer_name", 1, "-- All Buyer --", $selected, "load_drop_down('requires/export_pro_forma_invoice_approval_group_by_controller', this.value, 'load_drop_down_season', 'season_td');" );
	exit();
}
...
{ 
    ...
    $electronicDataArr=getSequence(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>94,'user_id'=>$app_user_id,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'product_dept_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0));
	...
	{ 
		...
		$data_mast_sql = "SELECT a.ID,a.exporter_id, a.BUYER_ID, a.PI_REVISE,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY
		from com_export_pi_mst a where a.exporter_id=$cbo_company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0  and a.IS_APPROVED<>1 $where_con and a.READY_TO_APPROVED=1 and a.entry_form=152 ";

		// echo $data_mast_sql;die;

		 
		// echo $my_previous_bypass_no_seq;die;
		//echo $electronicDataArr['group_bypass_arr'][$my_group][2];die;
	 ...
	 //echo  ;  die;
		...
		//  echo count($group_stage_arr[47758]); 
		...
		for($group=0;$group<=$my_group; $group++ ){
			foreach($electronicDataArr['group_seq_arr'][$group] as $seq){
				...
				if(count($tmp_sys_id_arr[$group][$seq])){
					...
					$sql .= "SELECT a.ID,a.TOTAL_AMOUNT,a.REMARKS,a.NET_TOTAL_AMOUNT, a.PI_NUMBER, a.PI_DATE, a.ITEM_CATEGORY_ID, a.EXPORTER_ID, a.WITHIN_GROUP, a.LAST_SHIPMENT_DATE, a.HS_CODE, a.BUYER_ID, a.PI_REVISE, a.PI_REVISED_DATE, a.PAY_TERM , LISTAGG(c.sales_booking_no, ',') WITHIN GROUP (ORDER BY c.sales_booking_no) AS sales_booking_no from com_export_pi_mst a, com_export_pi_dtls b,fabric_sales_order_mst c where a.exporter_id=$cbo_company_name and  a.id=b.pi_id and b.work_order_id=c.id and a.APPROVED_SEQU_BY=$seq $where_con and a.is_approved<>1 and  a.ready_to_approved=1 and a.APPROVED_GROUP_BY=$group $sys_con and a.entry_form=152 and a.status_active in(1,2,3) and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id, a.pi_number, a.pi_date, a.item_category_id, a.exporter_id,a.total_amount,a.net_total_amount,a.within_group, a.last_shipment_date,a.REMARKS, a.hs_code, a.buyer_id, a.PI_REVISE, a.PI_REVISED_DATE, a.PAY_TERM";
				}
	...
	{   
		...
		$sql = "SELECT a.ID,a.TOTAL_AMOUNT,a.NET_TOTAL_AMOUNT,a.REMARKS, a.PI_NUMBER, a.PI_DATE, a.ITEM_CATEGORY_ID, a.EXPORTER_ID, a.WITHIN_GROUP, a.LAST_SHIPMENT_DATE, a.HS_CODE, a.BUYER_ID, a.PI_REVISE, a.PI_REVISED_DATE, a.PAY_TERM , LISTAGG(c.sales_booking_no, ',') WITHIN GROUP (ORDER BY c.sales_booking_no) AS sales_booking_no from com_export_pi_mst a, com_export_pi_dtls b,fabric_sales_order_mst c,APPROVAL_MST d where a.exporter_id=$cbo_company_name and  a.id=b.pi_id and a.id=d.mst_id and b.work_order_id=c.id and a.is_approved<>0  and a.APPROVED_GROUP_BY=d.GROUP_NO  and d.GROUP_NO={$electronicDataArr['user_by'][$app_user_id]['GROUP_NO']}  $where_con and  a.ready_to_approved=1 and a.entry_form=152 and a.status_active in(1,2,3) and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id, a.pi_number, a.pi_date, a.item_category_id, a.exporter_id,a.REMARKS,a.total_amount,a.net_total_amount,a.within_group, a.last_shipment_date, a.hs_code, a.buyer_id, a.PI_REVISE, a.PI_REVISED_DATE, a.PAY_TERM"; 
    ...
    <script>
		function openmypage_app_cause(wo_id,app_type,i)
		{
			...
			var page_link = 'requires/export_pro_forma_invoice_approval_group_by_controller.php?data='+data+'&action=appcause_popup';
    ...
    <form name="requisitionApproval_2" id="requisitionApproval_2">
        <fieldset style="width:1510px; margin-top:10px">
        <legend>Yarn Work Order Approval</legend>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1490" class="rpt_table" align="left" >
                <thead>
                	<th width="50">&nbsp;</th>
                    <th width="30">SL</th>
                    <th width="100">Pi Number</th>
                    <th width="70">Pi Sys Id</th>
                    <th width="120">Pi Date</th>
                    <th width="70">Exporter</th>
                    <th width="100">Customer/buyer</th>
                    <th width="100">Sales/ Booking No</th>
                    <th width="100">Pi Amount</th>
                    <th width="80">Pi Net Amount</th>
                    <th width="100">Image</th>
                    <th width="100">Lc No</th>
                    <th width="80">Lc Recieve Date</th>
                    <th width="100">Lc Documents</th>
                    <th width="100">Pi Remarks</th>
                    
                    <th><? if($approval_type==0) echo "Not Appv. Cause" ; else echo "Not Un-Appv. Cause"; ?></th>
                </thead>
            </table>
            <div style="width:1510px; overflow-y:scroll; max-height:330px;" id="buyer_list_view" align="left">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1490" class="rpt_table" id="tbl_list_search">
                    <tbody>
                        <?
                            $i=1;
							$supplier=return_library_array("select id, supplier_name from lib_supplier ",'id','supplier_name');

							$img_val =  return_field_value("master_tble_id","common_photo_library","form_name='export_pro_forma_invoice'","master_tble_id");//master_tble_id='$value' and 
                            ...
                            foreach ($nameArray as $row)
                            {
								...
								$value=$row[csf('id')];
								$sales_booking_no=implode(',',array_unique(explode(',',rtrim($row[csf("sales_booking_no")], ','))));
								...
								<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									<td width="100"><?= $row[csf('PI_NUMBER')]; ?></td>
									<td width="70"><?= $row[csf('ID')]; ?></td>
                                    <td width="120"><? if($row[csf('PI_DATE')]!="0000-00-00") echo change_date_format($row[csf('PI_DATE')]); ?></td>
									<td width="70" align="center"><?= $company_arr[$row['EXPORTER_ID']]; ?></td>
									<td width="100" align="center"><?= $buyer_arr[$row['BUYER_ID']]; ?></td>
									<td width="100" align="center"><?=$sales_booking_no?></td>
                                    
                                    <td width="100" style="word-break:break-all"><?= $row[csf('TOTAL_AMOUNT')]; ?></td>
                                    <td width="80" style="word-break:break-all"><?= $row[csf('NET_TOTAL_AMOUNT')]; ?></td>
									<td width="100" align="center"><a href="javascript:void()" onClick="downloiadFile('<? echo $row[csf('id')]; ?>','<? echo $row[csf('EXPORTER_ID')]; ?>');">
                                    <? if ($img_val != '') echo 'View File'; ?></a></td>

                                    <td width="100" style="word-break:break-all"></td>
                                    <td width="80" style="word-break:break-all"></td>
                                    <td width="100" style="word-break:break-all">view</td>
            ...
            <table cellspacing="0" align="left" cellpadding="0" border="1" rules="all" width="1460" class="rpt_table">
				<tfoot>
...
if ($action=="approve")
{
	...
	$sql = "SELECT a.ID, a.READY_TO_APPROVED,a.IS_APPROVED,a.exporter_id,a.APPROVED_GROUP_BY,a.APPROVED_SEQU_BY,a.BUYER_ID
	from com_export_pi_mst a where  a.exporter_id=$cbo_company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0  and a.IS_APPROVED<>1 and a.READY_TO_APPROVED=1 and a.id in($req_ids) and a.entry_form=152";
	...
	$finalDataArr=getFinalUser(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>94,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0,'product_dept_arr'=>0,'match_data'=>$matchDataArr));
   ...
   // print_r($user_sequence_no);die;
	...
	$max_approved_no_arr = return_library_array("select mst_id, max(approved_no) as approved_no from approval_history where mst_id in($req_ids) and entry_form=94 group by mst_id","mst_id","approved_no");
	...
	{ 
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			...
			$data_array.="(".$id.",94,'".$mst_id."','".$user_sequence_no."','".$user_group_no."',".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			
		
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",94,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,".$approved.")";
			$ahid++;
		...
		if($flag==1) 
		{
			...
			$usql = bulk_update_sql_statement( "com_export_pi_mst", "id", $field_array_up, $data_array_up, $target_app_id_arr );
		...
		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=94 and mst_id in ($req_ids)";
		...
		if(count($approved_no_array)>0 && $flag)
		{
			...
			$sql_insert="insert into com_export_pi_mst_history(id, mst_id, approved_no, garments_nature, wo_number_prefix, wo_number_prefix_num, wo_number, company_name, buyer_po, requisition_no, delivery_place, wo_date, supplier_id, attention, wo_basis_id, item_category, currency_id, delivery_date, source, pay_mode, terms_and_condition, is_approved, approved_by, status_active, is_deleted, inserted_by,insert_date,updated_by,update_date)
			select
			'', id, $approved_string_mst, garments_nature, wo_number_prefix, wo_number_prefix_num, wo_number, company_name, buyer_po, requisition_no, delivery_place, wo_date, supplier_id, attention, wo_basis_id, item_category, currency_id, delivery_date, source, pay_mode, terms_and_condition, is_approved, approved_by, status_active, is_deleted, inserted_by,insert_date,updated_by,update_date from  com_export_pi_mst where id in ($req_ids)";
	...
	{              
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			...
			$history_data_array.="(".$ahid.",94,".$mst_id.",'".$approved_no."','".$user_sequence_no."',0,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,2)";
			$ahid++;
        }

		

		$rID1=sql_multirow_update("com_export_pi_mst","IS_APPROVED*READY_TO_APPROVED*APPROVED_SEQU_BY*APPROVED_GROUP_BY",'2*0*0*0',"id",$req_ids,0);
		if($rID1) $flag=1; else $flag=0;
		...
		if($flag==1) 
		{
			$query="delete from approval_mst  WHERE entry_form=94 and mst_id in ($req_ids)";
			$rID3=execute_query($query,1); 
			if($rID3) $flag=1; else $flag=0; 
		}
	...
	{              
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			$approved_no = $max_approved_no_arr[$mst_id]*1;
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",94,".$mst_id.",".$approved_no.",'".$user_sequence_no."',0,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			$ahid++;
        }

		//echo $req_ids;die;

		$rID1=sql_multirow_update("com_export_pi_mst","IS_APPROVED*READY_TO_APPROVED*APPROVED_SEQU_BY*APPROVED_GROUP_BY",'0*0*0*0',"id",$req_ids,0);
		if($rID1) $flag=1; else $flag=0;
...
if($action=="get_user_pi_file")
{
    ...
    $img_sql = "SELECT id,image_location,master_tble_id,real_file_name,FILE_TYPE from common_photo_library where form_name='export_pro_forma_invoice' and master_tble_id='$id'";
...
if ($action=="appcause_popup")
{
	...
	if($app_cause=="")
	{
		$sql_cause="select MAX(id) as id from fabric_booking_approval_cause where page_id='$menu_id' and entry_form=94 and user_id='$user_id' and booking_id='$wo_id' and approval_type=$app_type and status_active=1 and is_deleted=0";
    ...
    <script>
		...
		function fnc_appv_entry(operation)
		{
			...
			else
			{
				...
				http.open("POST","export_pro_forma_invoice_approval_group_by_controller.php",true);
...
if ($action=="save_update_delete_appv_cause")
{
	...
	if($approval_type==0)
	{
		...
		if ($operation==0)  // Insert Here
		{
			...
			$approved_no_history=return_field_value("approved_no","approval_history","entry_form=94 and mst_id=$wo_id and approved_by=$user_id");
			$approved_no_cause=return_field_value("approval_no","fabric_booking_approval_cause","entry_form=94 and booking_id=$wo_id and user_id=$user_id and approval_type=$appv_type");
			...
			if($approved_no_history=="" && $approved_no_cause=="")
			{
				...
				$data_array="(".$id_mst.",".$page_id.",94,".$user_id.",".$wo_id." ,".$appv_type.",0,".$appv_cause.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			...
			else if($approved_no_history=="" && $approved_no_cause!="")
			{
				...
				$id_cause=return_field_value("max(id)","fabric_booking_approval_cause","entry_form=94 and booking_id=$wo_id and user_id=$user_id and approval_type=$appv_type");
				...
				$data_array="".$page_id."*94*".$user_id."*".$wo_id."*".$appv_type."*0*".$appv_cause."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0";
			...
			{
				$max_appv_no_his=return_field_value("max(approved_no)","approval_history","entry_form=94 and mst_id=$wo_id and approved_by=$user_id");
				$max_appv_no_cause=return_field_value("max(approval_no)","fabric_booking_approval_cause","entry_form=94 and booking_id=$wo_id and user_id=$user_id and approval_type=$appv_type");
				...
				if($max_appv_no_his!=$max_appv_no_cause)
				{
					...
					$field_array="id,page_id,entry_form,user_id,booking_id,approval_type,approval_no,approval_cause,inserted_by,insert_date,status_active,is_deleted";
					$data_array="(".$id_mst.",".$page_id.",94,".$user_id.",".$wo_id." ,".$appv_type.",".$max_appv_no_his.",".$appv_cause.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
					$rID=sql_insert("fabric_booking_approval_cause",$field_array,$data_array,0);
					//echo $rID; die;
				...
				else if($max_appv_no_his==$max_appv_no_cause)
				{
					...
					$field_array="page_id*entry_form*user_id*booking_id*approval_type*approval_no*approval_cause*updated_by*update_date*status_active*is_deleted";
					$data_array="".$page_id."*94*".$user_id."*".$wo_id."*".$appv_type."*".$max_appv_no_his."*".$appv_cause."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0";
	...
	if($approval_type==1)
	{
		...
		if ($operation==0)  // Insert Here
		{
			...
			$unapproved_cause_id=return_field_value("id","fabric_booking_approval_cause","entry_form=94 and user_id=$user_id and booking_id=$wo_id  and approval_type=$appv_type and approval_history_id=$approval_id");
			...
			if($unapproved_cause_id=="")
			{
				...
				$data_array="(".$id_mst.",".$page_id.",94,".$user_id.",".$wo_id." ,".$appv_type.",".$max_appv_no_his.",".$approval_id.",".$appv_cause.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			...
			else
			{
				...
				$id_cause=return_field_value("max(id)","fabric_booking_approval_cause","entry_form=94 and user_id=$user_id and booking_id=$wo_id  and approval_type=$appv_type and approval_history_id=$approval_id");
				...
				$data_array="".$page_id."*94*".$user_id."*".$wo_id."*".$appv_type."*".$max_appv_no_his."*".$approval_id."*".$appv_cause."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0";
