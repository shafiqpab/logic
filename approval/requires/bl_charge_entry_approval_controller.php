...
$style_status_arr= array(1=>"Before Order",2=>"After Order");
$dealing_merc_arr=return_library_array( "select id, team_member_name from lib_mkt_team_member_info", "id", "team_member_name"  );

$destination_arr=return_library_array( "select id, country_name from LIB_COUNTRY", "id", "country_name"  );
...
if($action=="report_generate")
{
   ...
   // print_r($txt_system_no);die;
    ...
    $electronicDataArr=getSequence(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>90,'user_id'=>$user_id_approval,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'product_dept_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0));
   //print_r($buyer_arr);
	...
	if($txt_bill_no!='') 
	{		
		$where_id_cond.=" and a.bl_no ='".$txt_bill_no."'"; 
	}
	...
	if($txt_date_from && $txt_date_to){
		$where_id_cond .= " and b.invoice_date BETWEEN '".$txt_date_from."' AND '".$txt_date_to."'";	
	}

	$buyer_id_cond = '';
	if($cbo_buyer_name != 0){
		$buyer_id_cond .= " and b.buyer_id =$cbo_buyer_name";
	}
  

   $forwarder_id_cond = '';
	if($cbo_forwarder_name != 0){
		$forwarder_id_cond .= " and b.forwarder_name =$cbo_forwarder_name";
	}
   // echo $forwarder_id_cond ;die;
	if($txt_invoice_no!='') 
	{		
		$invoice_cond.=" and b.invoice_no ='".$txt_invoice_no."'"; 
	}
     // echo  $invoice_cond;die;

	if ($txt_system_no == "") $txt_system_no_cond = "";
	else $txt_system_no_cond = " and a.sys_number_prefix_num='" . trim($txt_system_no) . "' ";

	

	// echo $txt_system_no_cond;die();
	...
	{  
		...
		//  }
        $data_mast_sql =  "select a.ID,a.sys_number as sys_number, a.company_id as company_id,b.buyer_id as buyer_id
        from bl_charge a, com_export_invoice_ship_mst b 
        where  a.company_id = $company_name and a.status_active = 1 and a.is_deleted = 0 and a.ready_to_approve =1 and a.IS_APPROVED<>1 and a.invoice_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ";
		...
		for($seq=0;$seq<=count($electronicDataArr['sequ_arr']); $seq++ ){
			...
			if($tmp_sys_id_arr[$seq]){
				...
				$sql.="select a.ID,a.sys_number as sys_number,a.sys_number_prefix_num, a.company_id as company_id,a.invoice_id as invoice_id,a.bl_charge_date as bl_charge_date, a.remarks as reamrks, a.bl_charge as bl_charge, a.stamp_charge as stamp_charge, a.air_company_charge as air_company_charge, a.air_buyer_charge as air_buyer_charge,a.adjustment_charge as adjustment_charge,a.surrendered_charge as surrendered_charge,a.special_charge as special_charge , a.others_charge as others_charge,b.buyer_id as buyer_id,b.invoice_no,b.invoice_date as invoice_date,invoice_value as invoice_value,b.invoice_quantity as invoice_quantity,a.bl_no as bl_no,a.bl_date as bl_date,b.bl_rev_date as bl_rev_date,b.forwarder_name as forwarder_name,b.shipping_mode as shipping_mode,b.ex_factory_date as ex_factory_date, a.is_posted_account as is_posted_account from bl_charge a, com_export_invoice_ship_mst b 
                where a.company_id = $company_name $where_id_cond $buyer_id_cond $invoice_cond $cbo_candf_cond $txt_system_no_cond $forwarder_id_cond and a.invoice_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.IS_APPROVED<>1  and a.APPROVED_SEQU_BY=$seq $sys_con";
	...
	{   $sql= "select a.ID,a.sys_number as sys_number,a.sys_number_prefix_num, a.company_id as company_id,a.invoice_id as invoice_id,a.bl_charge_date as bl_charge_date, a.remarks as reamrks, a.bl_charge as bl_charge, a.stamp_charge as stamp_charge, a.air_company_charge as air_company_charge, a.air_buyer_charge as air_buyer_charge,a.adjustment_charge as adjustment_charge,a.surrendered_charge as surrendered_charge,a.special_charge as special_charge , a.others_charge as others_charge,b.buyer_id as buyer_id,b.invoice_no,b.invoice_date as invoice_date,invoice_value as invoice_value,b.invoice_quantity as invoice_quantity,a.bl_no as bl_no,a.bl_date as bl_date,b.bl_rev_date as bl_rev_date,b.forwarder_name as forwarder_name,b.shipping_mode as shipping_mode,b.ex_factory_date as ex_factory_date, a.is_posted_account as is_posted_account from bl_charge a, com_export_invoice_ship_mst b ,APPROVAL_MST c where a.company_id = $company_name $where_id_cond $buyer_id_cond $invoice_cond $cbo_candf_cond $txt_system_no_cond $forwarder_id_cond   and c.SEQUENCE_NO={$electronicDataArr['user_by'][$user_id_approval]['SEQUENCE_NO']} and a.APPROVED_SEQU_BY=c.SEQUENCE_NO  and a.invoice_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.IS_APPROVED<>0 and c.mst_id=a.id";
 ...
 $nameArray = sql_select($sql);
	...
	foreach ($nameArray as $row) {
		$sis_arr[$row['ID']] = $row['ID'];
	
	}
	$sis_id= implode(',', $sis_arr);
     //print_r($bokings);
	 $sis_cond = where_con_using_array($sis_arr, 0, 'a.id');

    $total_amount_arr=array();$total_deduce_arr=array();$total_payble_arr=array();

	$total_qty_sql=sql_select("select a.id,b.mst_id, sum(b.deduction) as deduction ,sum (b.payable) as payable,
    sum(b.amount) as amount from bl_charge a, cnf_bill_dtls b where a.id=b.mst_id and a.company_id =$company_name $sis_cond  and a.status_active=1 and b.status_active=1 group by  a.id, b.mst_id ");

    // echo "select a.id,b.mst_id, sum(b.deduction) as deduction ,sum (b.payable) as payable,
    // sum(b.amount) as amount from bl_charge a, cnf_bill_dtls b where a.id=b.mst_id and a.company_id =$company_name $sis_cond  and a.status_active=1 and b.status_active=1 group by  a.id, b.mst_id";die;


	//echo $sql;die();
							
							foreach($total_qty_sql as $row)
							{
								$total_amount_arr[$row[csf("id")]]=$row[csf("amount")];
								$total_deduce_arr[$row[csf("id")]]=$row[csf("deduction")];
								$total_payble_arr[$row[csf("id")]]=$row[csf("payable")];
							}
        ...
        <div style="width:<? echo $table1+25; ?>px; margin:0 auto;">
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="left" >
                <thead>
                    ...
                    <th width="125"> Invoice No </th>
                    <th width="125">Invoice Date</th>
                    
                    <th width="150">Buyer</th> 
                    <th width="150">Bill NO</th>                   
                    <th width="100">Bill Date</th>                   
                    <th width="100">Inv. Value</th>                   
                    <th width="100">Inv Qty.</th>                  
                    <th width="100">Total B/L Charge</th>                 
                ...
                <table cellspacing="0" cellpadding="0" border="1" rules="all"  class="rpt_table" id="tbl_list_search" align="left">
                    <tbody>
                        ...
                        {                                                   
							...
							else if($report_ids[0]==290){ $action="show_trim_booking_report5";}
							else{	$action="show_work_order_aop_report";}


                            $total_amount=$row[csf('bl_charge')]+$row[csf('stamp_charge')]+$row[csf('air_buyer_charge')]+$row[csf('air_company_charge')]+$row[csf('adjustment_charge')]+$row[csf('surrendered_charge')]+$row[csf('special_charge')]+$row[csf('others_charge')];	
                            ...
                            //echo $total_amount;die;
                                ...
                                <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>"> 
                                    ...
                                    <td width="125"><p><? echo $row[csf('invoice_no')]; ?>&nbsp;</p></td>                                   
                                    <td width="125" align="center"><p><? echo change_date_format($row[csf('invoice_date')]); ?>&nbsp;</p></td>
									
                                    <td width="150"><p><? echo $buyer_arr[$row[csf('buyer_id')]]; ?> &nbsp;</p></td>
									
                                    <td width="150"><p><? echo $row[csf('bl_no')]; ?>&nbsp;</p></td>				
                                    <td width="100"><p><? echo change_date_format($row[csf('bl_date')]); ?>&nbsp;</p></td>
                                    <td width="100"><p><? echo $row[csf('invoice_value')]; ?> &nbsp;</p></td>				                                  
                                    <td width="100"><p><? echo $row[csf('invoice_quantity')]; ?>&nbsp;</p></td>				                                  
                                    <td width="100"><p><? echo $row[csf('bl_charge')]; ?> &nbsp;</p></td>				                                  
                                    <td width="100"><p><? echo $total_amount; ?>&nbsp;</p></td>				                                  
...
{ 
	...
	$sql = "select a.ID,b.BUYER_ID,a.READY_TO_APPROVE  from bl_charge a,com_export_invoice_ship_mst b  where a.COMPANY_ID=$cbo_company_name and a.invoice_id=b.id and a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and b.status_active=1 and b.is_deleted=0 and a.id in($target_ids)";
	//echo $sql;die();
	...
	$finalDataArr=getFinalUser(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>90,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0,'product_dept_arr'=>0,'match_data'=>$matchDataArr));
  ...
  //print_r($user_sequence_no) ;die;
	...
	{ 
        ...
        foreach($target_app_id_arr as $mst_id)
        {
			...
			$data_array.="(".$id.",90,'".$mst_id."','".$user_sequence_no."',".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			
			$approved_no=$max_approved_no_arr[$mst_id][$user_id_approval]+1;
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",90,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1)";
			$ahid++;
		...
		if($flag==1) 
		{
			...
			$rID2=execute_query(bulk_update_sql_statement( "bl_charge", "id", $field_array_up, $data_array_up, $target_app_id_arr ));
			if($rID2) $flag=1; else $flag=0; 
		}

		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=90 and mst_id in ($target_ids)";
	...
	{
		$max_approved_no_arr=return_library_array( "select mst_id, max(approved_no) as approved_no from approval_history where entry_form=90 and mst_id in($target_ids)  and APPROVED=2 and APPROVED_BY=$user_id_approval group by mst_id", "mst_id", "approved_no"  );
		$ahid=return_next_id( "id","approval_history", 1 ) ;	
		
		$rID1=sql_multirow_update("bl_charge","IS_APPROVED*READY_TO_APPROVE*APPROVED_SEQU_BY",'2*0*0',"id",$target_ids,0); ; 
		if($rID1) $flag=1; else $flag=0;

		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0, un_approved_by='".$user_id_approval."', un_approved_date='".$pc_date_time."', updated_by='".$user_id."', update_date='".$pc_date_time."' WHERE entry_form=90 and current_approval_status=1 and mst_id in ($target_ids)";
			$rID3=execute_query($query,1);
			if($rID3) $flag=1; else $flag=0;
			...
			{		
				...
				if($history_data_array!="") $history_data_array.=",";
				$history_data_array.="(".$ahid.",90,".$mst_id.",".$approved_no.",'".$user_sequence_no."',0,".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,2)";
				$ahid++;
			}		
		...
		//echo "191**".$rID1.",".$rID3.",".$rID4;oci_rollback($con);die;
	...
	{            
		...
		$next_user_app = sql_select("select id from approval_history where mst_id in($target_ids) and entry_form=90 and sequence_no >$user_sequence_no and current_approval_status=1 group by id");
		...
		$rID1=sql_multirow_update("bl_charge","IS_APPROVED*READY_TO_APPROVE*APPROVED_SEQU_BY",'0*0*0',"id",$target_ids,0);
		if($rID1) $flag=1; else $flag=0;
		...
		if($flag==1) 
		{
			$query="delete from approval_mst  WHERE entry_form=90 and mst_id in ($target_ids)";
			$rID3=execute_query($query,1); 
			if($rID3) $flag=1; else $flag=0; 
		}
    ...
    <form>
           ...
           <?php
            $custom_designation=return_library_array( "select id,custom_designation from lib_designation ",'id','custom_designation');  
             ...
             $sql="select a.id, a.user_name,b.sequence_no, a.department_id, a.user_full_name, a.designation from  user_passwd a,electronic_approval_setup b where a.id=b.user_id and b.company_id=$cbo_company_name and b.entry_form=90 and valid=1 and a.id!=$user_id   and a.is_deleted=0 AND b.is_deleted = 0 group by a.id,a.user_name,a.department_id,a.user_full_name, b.sequence_no,a.designation order by b.sequence_no";
                //echo $sql;die;
...
if ($action=="show_work_order_aop_report")
{
	...
	$com_supplier_id=return_field_value( "supplier_id as supplier_id", "bl_charge","booking_no=$txt_booking_no","supplier_id");
	$nameArray_approved=sql_select( "select max(b.approved_no) as approved_no from bl_charge a, approval_history b where a.id=b.mst_id and booking_no=$txt_booking_no and b.entry_form=162"); 
	...
	<div style="width:1540px; margin: 0 auto" align="center">       
       ...
       <?
		...
		$fabric_source='';
		$booking_info=sql_select( "select id,buyer_id,booking_no,booking_date,supplier_id,exchange_rate,currency_id,attention,delivery_date,fabric_source,entry_form from bl_charge where booking_no=$txt_booking_no"); 
	...
	if($db_type==0)
	{
		$sql= sql_select("select a.booking_no, a.booking_type, a.booking_date, a.delivery_date, a.buyer_id, a.supplier_id, a.job_no, a.item_category, a.source, a.attention, a.process, b.dia_width, b.uom, a.pay_mode, a.exchange_rate, a.currency_id, b.wo_qnty, b.amount, b.pre_cost_remarks FROM bl_charge a, wo_booking_dtls b  WHERE a.booking_no=$txt_booking_no and a.booking_no=b.booking_no and a.status_active=1 and a.is_deleted=0 order by a.id"); 
	}
	if($db_type==2)
	{
		$sql= sql_select("select a.booking_no, a.booking_type, a.booking_date, a.delivery_date, a.buyer_id, a.supplier_id, a.job_no, a.item_category, a.source, a.attention, a.process, b.dia_width, b.uom, a.pay_mode, a.exchange_rate, a.currency_id, b.wo_qnty, b.amount, b.pre_cost_remarks FROM bl_charge a, wo_booking_dtls b  WHERE a.booking_no=$txt_booking_no and a.booking_no=b.booking_no and a.status_active=1 and a.is_deleted=0 order by a.id"); 
            ...
            <tr>
                <td width="100%">
                    ...
                    <?
 					...
 					$sel=sql_select("select a.approved_by, a.approved_date, a.id as app_no from approval_history a, electronic_approval_setup b where a.mst_id in(select id from bl_charge where id=$bookingId) and a.entry_form=162 and a.approved_by=b.user_id and b.company_id=$cbo_company_name and b.is_deleted=0 group by a.approved_by, a.approved_date, a.id, b.sequence_no order by b.sequence_no asc");
