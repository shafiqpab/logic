...
if($action=="report_generate")
{
	...
	else
	{
		if(str_replace("'","",$cbo_buyer_name)==0)
		{
			if ($_SESSION['logic_erp']["data_level_secured"]==1)
			{
				if($_SESSION['logic_erp']["buyer_id"]!="")  $buyer_id_cond2=" and a.buyer_id in (".$_SESSION['logic_erp']["buyer_id"].")"; else $buyer_id_cond2="";
			}
			else $buyer_id_cond2="";
		}
		else $buyer_id_cond2=" and a.buyer_id=$cbo_buyer_name";
	...
	if($previous_approved==1 && $approval_type==1)
	{
		...
		$sql="SELECT a.id, a.booking_no_prefix_num as prefix_num,a.booking_no, a.item_category, a.entry_form, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.pay_mode, a.supplier_id, a.delivery_date, a.booking_date, a.job_no, a.po_break_down_id, a.is_approved, b.id as approval_id, b.sequence_no, b.approved_by, a.is_apply_last_update,c.grouping, c.file_no,a.pay_mode,(select max(h.approved_no) from approval_history h where a.id=h.mst_id and h.entry_form=13 ) as revised_no from wo_booking_mst a, approval_history b,wo_po_break_down c where a.id=b.mst_id and a.job_no=c.job_no_mst and b.entry_form=13 and a.company_id=$company_name and a.is_short=2 and a.booking_type=4 and a.item_category in(2,3,13) and a.status_active=1 and a.is_deleted=0 and b.current_approval_status=1 and a.ready_to_approved=1 and a.is_approved in(1,3) $buyer_id_cond2 $sequence_no_cond $internal_ref_cond $file_no_cond $date_cond $booking_no_cond order by a.insert_date desc";
		//echo $sql;die;
		//$buyer_id_cond
	...
	else if($approval_type==0)
	{
 		...
 		if($user_sequence_no==$min_sequence_no)
		{
			...
			$sql="SELECT a.id, a.booking_no_prefix_num as prefix_num,a.booking_no, a.entry_form, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.pay_mode, a.supplier_id, a.delivery_date, a.booking_date, '0' as approval_id, a.job_no, a.is_approved, a.po_break_down_id, a.is_apply_last_update,c.grouping, c.file_no,a.pay_mode from wo_booking_mst a, wo_booking_dtls b,wo_po_break_down c where a.booking_no=b.booking_no and b.job_no=c.job_no_mst and b.po_break_down_id =c.id and a.company_id=$company_name and a.is_short=2 and a.booking_type=4 and a.item_category in(2,3,13) and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 and a.is_approved=$approval_type and b.fin_fab_qnty>0  $buyer_id_cond2 $internal_ref_cond $file_no_cond $date_cond $booking_no_cond group by a.id,a.booking_no_prefix_num, a.booking_no, a.entry_form, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.pay_mode, a.supplier_id, a.delivery_date, a.booking_date, a.job_no, a.is_approved, a.po_break_down_id, a.is_apply_last_update,a.insert_date,c.grouping, c.file_no,a.pay_mode order by a.insert_date desc";
		}
		else if($sequence_no=="")
		{
			$buyer_ids=$buyer_ids_array[$user_id]['u'];
			...
			if($db_type==0)
			{
				...
				$booking_id=return_field_value("group_concat(distinct(mst_id)) as booking_id","wo_booking_mst a, approval_history b","a.id=b.mst_id and a.company_id=$company_name and a.is_short=2 and a.booking_type=4 and a.item_category in(2,3,13) and b.sequence_no in ($sequence_no_by) and b.entry_form=13 and b.current_approval_status=1 $buyer_id_cond $date_cond","booking_id");
				
				$booking_id_app_byuser=return_field_value("group_concat(distinct(mst_id)) as booking_id","wo_booking_mst a, approval_history b","a.id=b.mst_id and a.company_id=$company_name and a.is_short=2 and a.booking_type=4 and a.item_category in(2,3,13) and b.sequence_no=$user_sequence_no and b.entry_form=13 and b.current_approval_status=1","booking_id");
			}
			else
			{
				...
				$booking_id=return_field_value("LISTAGG(mst_id, ',') WITHIN GROUP (ORDER BY mst_id) as booking_id","wo_booking_mst a, approval_history b","a.id=b.mst_id and a.company_id=$company_name and a.is_short=2 and a.booking_type=4 and a.item_category in(2,3,13) and b.sequence_no in ($sequence_no_by) and b.entry_form=13 and b.current_approval_status=1 $buyer_id_cond $date_cond","booking_id");
				$booking_id=implode(",",array_unique(explode(",",$booking_id)));
				
				$booking_id_app_byuser=return_field_value("LISTAGG(mst_id, ',') WITHIN GROUP (ORDER BY mst_id) as booking_id","wo_booking_mst a, approval_history b","a.id=b.mst_id and a.company_id=$company_name and a.is_short=2 and a.booking_type=4 and a.item_category in(2,3,13) and b.sequence_no=$user_sequence_no and b.entry_form=13 and b.current_approval_status=1","booking_id");
			...
			if($booking_id!="")
			{
				$sql="SELECT a.id, a.booking_no_prefix_num as prefix_num,a.booking_no, a.entry_form, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.pay_mode, a.supplier_id, a.delivery_date, a.booking_date, '0' as approval_id, a.job_no, a.is_approved, a.po_break_down_id, a.is_apply_last_update,a.insert_date,c.grouping, c.file_no,a.pay_mode from wo_booking_mst a, wo_booking_dtls b,wo_po_break_down c where a.booking_no=b.booking_no and b.job_no=c.job_no_mst and b.po_break_down_id =c.id and a.company_id=$company_name and a.is_short=2 and a.booking_type=4 and a.item_category in(2,3,13) and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 and a.is_approved=$approval_type and b.fin_fab_qnty>0 $buyer_id_cond $buyer_id_cond2 $internal_ref_cond $file_no_cond $date_cond $buyerIds_cond $booking_no_cond group by a.id, a.booking_no, a.entry_form, a.booking_no_prefix_num, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.pay_mode, a.supplier_id, a.delivery_date, a.booking_date, a.job_no, a.is_approved, a.po_break_down_id, a.is_apply_last_update,a.insert_date,c.grouping, c.file_no,a.pay_mode
					union all
					SELECT a.id, a.booking_no_prefix_num as prefix_num,a.booking_no, a.entry_form,  a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.pay_mode, a.supplier_id, a.delivery_date, a.booking_date, '0' as approval_id, a.job_no, a.is_approved, a.po_break_down_id, a.is_apply_last_update,a.insert_date,c.grouping, c.file_no,a.pay_mode from wo_booking_mst a, wo_booking_dtls b,wo_po_break_down c where a.booking_no=b.booking_no and b.job_no=c.job_no_mst and b.po_break_down_id =c.id and a.company_id=$company_name and a.is_short=2 and a.booking_type=4 and a.item_category in(2,3,13) and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 and a.is_approved in(1,3) and b.fin_fab_qnty>0 and a.id in($booking_id) $buyer_id_cond $buyer_id_cond2 $internal_ref_cond $file_no_cond $date_cond $booking_no_cond group by a.id, a.booking_no, a.entry_form, a.booking_no_prefix_num, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.pay_mode, a.supplier_id, a.delivery_date, a.booking_date, a.job_no, a.is_approved, a.po_break_down_id, a.is_apply_last_update,a.insert_date,c.grouping, c.file_no,a.pay_mode order by insert_date desc";
			}
			else
			{
				$sql="SELECT a.id,a.booking_no_prefix_num as prefix_num, a.booking_no, a.item_category, a.entry_form, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.pay_mode, a.supplier_id, a.delivery_date, a.booking_date, '0' as approval_id, a.job_no, a.is_approved, a.po_break_down_id, a.is_apply_last_update,c.grouping, c.file_no,a.pay_mode from wo_booking_mst a, wo_booking_dtls b,wo_po_break_down c where a.booking_no=b.booking_no and b.job_no=c.job_no_mst and b.po_break_down_id =c.id and a.company_id=$company_name and a.is_short=2 and a.booking_type=4 and a.item_category in(2,3,13) and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 and a.is_approved in(0,3) and b.fin_fab_qnty>0  $buyer_id_cond2 $internal_ref_cond $file_no_cond $date_cond  $booking_no_cond group by a.id, a.booking_no,a.booking_no_prefix_num, a.item_category, a.entry_form, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.pay_mode, a.supplier_id, a.delivery_date, a.booking_date, a.job_no, a.is_approved, a.po_break_down_id, a.is_apply_last_update,a.insert_date,c.grouping, c.file_no,a.pay_mode order by a.insert_date desc";
			}
			// echo $sql;
		}
		else
		{ 
			...
			/*$sql="SELECT a.id, a.booking_no_prefix_num as prefix_num,a.booking_no, a.item_category, a.entry_form, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.pay_mode, a.supplier_id, a.delivery_date, a.booking_date, a.job_no, a.po_break_down_id, a.is_approved, b.id as approval_id, b.sequence_no, b.approved_by, a.is_apply_last_update,c.grouping, c.file_no,a.pay_mode from wo_booking_mst a, approval_history b, wo_po_break_down c where a.id=b.mst_id and a.job_no=c.job_no_mst and b.entry_form=13 and a.company_id=$company_name and a.is_short=2 and a.booking_type=4 and a.item_category in(2,3,13) and a.status_active=1 and a.is_deleted=0 and b.current_approval_status=1 and a.ready_to_approved=1 and a.is_approved=1 $buyer_id_cond $buyer_id_cond2 $sequence_no_cond $internal_ref_cond $file_no_cond $date_cond $booking_no_cond order by a.insert_date desc";*/
			$sql="SELECT a.id, a.booking_no_prefix_num as prefix_num,a.booking_no, a.item_category, a.entry_form, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.pay_mode, a.supplier_id, a.delivery_date, a.booking_date, a.job_no, a.po_break_down_id, a.is_approved, b.id as approval_id, b.sequence_no, b.approved_by, a.is_apply_last_update,c.grouping, c.file_no,a.pay_mode from wo_booking_mst a, approval_history b, wo_po_break_down c where a.id=b.mst_id and a.job_no=c.job_no_mst and b.entry_form=13 and a.company_id=$company_name and a.is_short=2 and a.booking_type=4 and a.item_category in(2,3,13) and a.status_active=1 and a.is_deleted=0 and b.current_approval_status=1 and a.ready_to_approved=1 and a.is_approved<>1 $buyer_id_cond $buyer_id_cond2 $sequence_no_cond $internal_ref_cond $file_no_cond $date_cond $booking_no_cond order by a.insert_date desc";
		}
	}
	else
    ...
    <form name="requisitionApproval_2" id="requisitionApproval_2">
        <fieldset style="width:<? echo $table1; ?>px; margin-top:10px">
        <legend>Sample Booking Approval (With Order)</legend>	
            ...
            <div style="width:<? echo $table1; ?>px; overflow-y:scroll; max-height:330px;" id="buyer_list_view" align="center">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $table1; ?>" class="rpt_table" id="tbl_list_search">
                    <tbody>
                        <?
						 //echo $sql; die;
                            $i=1; $all_approval_id='';
                            ...
                            foreach ($nameArray as $row)
                            {
								...
								if($row_id==38)
								{	
									$variable="<a href='##' style='color:#000' onClick=\"generate_worder_report('".$type."','".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$row[csf('po_break_down_id')]."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$row[csf('job_no')]."','".$row[csf('is_approved')]."','show_fabric_booking_report','".$i."')\">".$row[csf('prefix_num')]."</a>";
								}

								
								else if($row_id==17)
								{
									$variable="<a href='##' style='color:#000' onClick=\"generate_worder_report('".$type."','".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$row[csf('po_break_down_id')]."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$row[csf('job_no')]."','".$row[csf('is_approved')]."','show_fabric_booking_report2','".$i."')\">".$row[csf('prefix_num')]."</a>";
								}
								else if($row_id==39)
								{
									$variable="<a href='##' style='color:#000' onClick=\"generate_worder_report('".$type."','".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$row[csf('po_break_down_id')]."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$row[csf('job_no')]."','".$row[csf('is_approved')]."','show_fabric_booking_report2','".$i."')\">".$row[csf('prefix_num')]."</a>";
								...
								{
									$variable="<a href='##' style='color:#000' onClick=\"generate_worder_report('".$type."','".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$row[csf('po_break_down_id')]."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$row[csf('job_no')]."','".$row[csf('is_approved')]."','show_fabric_booking_report3','".$i."')\">".$row[csf('prefix_num')]."</a>";
								}
								else if($row_id==155)
								{
									$variable="<a href='##' style='color:#000' onClick=\"generate_worder_report('".$type."','".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$row[csf('po_break_down_id')]."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$row[csf('job_no')]."','".$row[csf('is_approved')]."','show_fabric_booking_report','".$i."')\">".$row[csf('prefix_num')]."</a>";
								...
								{
									$variable="<a href='##' style='color:#000' onClick=\"generate_worder_report('".$type."','".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$row[csf('po_break_down_id')]."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$row[csf('job_no')]."','".$row[csf('is_approved')]."','show_fabric_booking_report_print_booking_3','".$i."')\">".$row[csf('prefix_num')]."</a>";
								}
								else if($row_id==17)
								{
									$variable="<a href='##' style='color:#000' onClick=\"generate_worder_report('".$type."','".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$row[csf('po_break_down_id')]."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$row[csf('job_no')]."','".$row[csf('is_approved')]."','show_fabric_booking_report2','".$i."')\">".$row[csf('prefix_num')]."</a>";
								...
								{
									$variable="<a href='##' style='color:#000' onClick=\"generate_worder_report('".$type."','".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$row[csf('po_break_down_id')]."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$row[csf('job_no')]."','".$row[csf('is_approved')]."','show_fabric_booking_report4','".$i."')\">".$row[csf('prefix_num')]."</a>";
								}
								else if($row_id==0)
								{
									$variable="<a href='##' style='color:#000' onClick=\"generate_worder_report('".$type."','".$row[csf('booking_no')]."','".$row[csf('company_id')]."','".$row[csf('po_break_down_id')]."','".$row[csf('item_category')]."','".$row[csf('fabric_source')]."','".$row[csf('job_no')]."','".$row[csf('is_approved')]."','show_fabric_booking_report4','".$i."')\">".$row[csf('prefix_num')]."</a>";
								...
								{	
									...
									<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>"> 
										...
										<td width="130">
											<p>
												<?
													//echo $variable."***".$row[csf('entry_form')]."***".$type;
													echo $variable;
												?>
											</p>
										</td>
            ...
            <table cellspacing="0" cellpadding="0" border="0" rules="all" width="<? echo $table1; ?>" class="rpt_table">
				<tfoot>
                    <td width="50" align="center" style=" <?=$isApp; ?>"><input type="checkbox" id="all_check" onClick="check_all('all_check')" /><font style="display:none"><? echo $all_approval_id; ?></font></td>
...
if ($action=="approve")
{
	...
	if($approval_type==0)
	{
		...
		//-------------------------------
		$sql="select A.ID,a.BUYER_ID from wo_booking_mst a where a.id in($booking_ids)";
		...
		$finalDataArr=getFinalUser(array(company_id=>$cbo_company_name,page_id=>$menu_id,lib_buyer_arr=>$buyer_arr,lib_brand_arr=>0,lib_item_cat_arr=>0,lib_store_arr=>0,product_dept_arr=>0,match_data=>$matchDataArr));
		...
		for($i=0;$i<count($booking_nos_all);$i++)
		{
			...
			$partial_approval =(max($finalDataArr[final_seq][$booking_id])==$user_sequence_no)?1:3;
			...
			$data_array.="(".$id.",13,".$booking_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."','".$user_ip."',".$app_instru.",".$user_id.",'".$pc_date_time."')"; 
			$id=$id+1;
	...
	{
		$sqlBookinghistory="select id, mst_id from approval_history where current_approval_status=1 and entry_form=13 and mst_id in ($booking_ids) ";
		...
		if($approval_ids!="")
		{
			$query="UPDATE approval_history SET current_approval_status=0, un_approved_by='".$user_id_approval."', un_approved_date='".$pc_date_time."', updated_by='".$user_id."', update_date='".$pc_date_time."' WHERE entry_form=13 and current_approval_status=1 and id in ($approval_ids)";
			//echo "10**".$query;
