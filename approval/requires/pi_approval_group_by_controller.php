...
{
    echo create_drop_down( "cbo_buyer_name", 100, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,30,90)) order by buy.buyer_name","id,buyer_name", 1, "-- All Buyer --", $selected, "load_drop_down( 'requires/pi_approval_group_by_controller',this.value, 'load_dropdown_season', 'season_td' );load_drop_down( 'requires/pi_approval_group_by_controller',this.value, 'load_dropdown_brand', 'brand_td' );" );  
    exit();
}
...
function getSequence($parameterArr=array()){
	...
	$sql="SELECT ITEM_CATEGORY,COMPANY_ID,PAGE_ID,USER_ID,BYPASS,SEQUENCE_NO,BUYER_ID,BRAND_ID,DEPARTMENT,GROUP_NO FROM ELECTRONIC_APPROVAL_SETUP WHERE COMPANY_ID = {$parameterArr['company_id']} AND ENTRY_FORM = {$parameterArr['entry_form']} AND IS_DELETED = 0 order by SEQUENCE_NO";
	  //echo $sql;die;
	$sql_result=sql_select($sql);
	$dataArr=array();
	foreach($sql_result as $rows){

        $rows['ITEM_ID']=($rows['ITEM_CATEGORY']!='')?$rows['ITEM_CATEGORY']:$lib_item_cat_id_string;
...
function getFinalUser($parameterArr=array()){
	...
	//Electronic app setup data.....................
	$sql="SELECT ITEM_CATEGORY,COMPANY_ID,PAGE_ID,USER_ID,BYPASS,SEQUENCE_NO,GROUP_NO,BUYER_ID,BRAND_ID,DEPARTMENT FROM ELECTRONIC_APPROVAL_SETUP WHERE COMPANY_ID = {$parameterArr['company_id']} AND ENTRY_FORM = {$parameterArr['entry_form']} AND IS_DELETED = 0  order by SEQUENCE_NO";
	   //echo $sql;die;
	$sql_result=sql_select($sql);
	foreach($sql_result as $rows){
		...
		$userByPassDataArr[$rows['USER_ID']]=$rows['BYPASS'];
	...
	foreach($parameterArr['match_data'] as $sys_id=>$bbtsRows){
		...
		foreach($userSeqDataArr as $user_id=>$seq){
			if(
				(in_array($bbtsRows['item'],$usersDataArr[$user_id]['ITEM_ID']) || $bbtsRows['item']==0)
                ...
                $finalSeq[$sys_id][$userByPassDataArr[$user_id]][$user_id]=$seq;
			}
...
if($action=="report_generate")
{
	...
	{
		$where_con .= " and a.pi_date between '$txt_date_from' and '$txt_date_to'";	
	}
   ...
   // $item_cateory_id_list = " listagg(b.item_category_id,',') within group (order by b.item_category_id)";	
	...
	$electronicDataArr=getSequence(array('company_id'=>$company_name,'entry_form'=>27,'user_id'=>$app_user_id,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>'','lib_item_cat_arr'=>$item_cat_arr,'lib_store_arr'=>0,'lib_department_id_arr'=>0));
    ...
    if($approval_type==0) // Un-Approve
	{
        ...
        $data_mas_sql="SELECT a.ID, a.IMPORTER_ID, a.ITEM_CATEGORY_ID,a.BUYER_ID,a.BRAND_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id and a.importer_id=$company_name and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.ready_to_approved=1 and a.approved<>1 and b.amount>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $where_con group by a.ID, a.IMPORTER_ID, a.ITEM_CATEGORY_ID,a.BUYER_ID,a.BRAND_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY"; 
        ...
        { 
            ...
            for($group = ($row['APPROVED_GROUP_BY'] == $my_group && $electronicDataArr['sequ_by'][$my_seq]['BYPASS'] == 2)?$my_group:$my_group-1;$group >= 0; $group-- ){
                ...
                foreach($electronicDataArr['group_seq_arr'][$group] as $seq){
                    ...
                    if($seq < $my_seq){
                        if(
                            (in_array($row['ITEM_CATEGORY_ID'],explode(',',$electronicDataArr['sequ_by'][$seq]['ITEM_ID'])) || $row['ITEM_CATEGORY_ID'] == 0)
                        ...
                        {
                           //echo $row['APPROVED_GROUP_BY'].'='.$group;die;
	...
	else
	{
        ...
        if(count($electronicDataArr['group_seq_arr'])==2){
			$where_con .= "and c.SEQUENCE_NO={$electronicDataArr['user_by'][$app_user_id]['SEQUENCE_NO']}";
		}
		else{
			$where_con .= "and c.GROUP_NO={$electronicDataArr['user_by'][$app_user_id]['GROUP_NO']}";
		} 
        
        $sql="SELECT a.ID, a.IMPORTER_ID, a.SOURCE, a.SUPPLIER_ID, a.PI_NUMBER, a.PI_DATE, a.EXPORT_PI_ID, to_char(a.insert_date,'YYYY') as YEAR, a.LAST_SHIPMENT_DATE, a.INTERNAL_FILE_NO, a.APPROVED, a.REMARKS, sum(b.net_pi_amount) as NET_PI_AMOUNT, a.NET_TOTAL_AMOUNT, a.IS_APPLY_LAST_UPDATE, $year_field as YEAR,a.WITHIN_GROUP,a.IMPORT_PI ,c.IS_MAIL_SEND
        FROM com_pi_master_details a, com_pi_item_details b, APPROVAL_MST c 
        WHERE a.id=b.pi_id and a.id=c.mst_id and c.entry_form=27 and a.importer_id=$company_name  and a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0  and a.ready_to_approved=1 and a.approved in (1,3) and a.APPROVED_SEQU_BY=c.SEQUENCE_NO  and c.SEQUENCE_NO={$electronicDataArr['user_by'][$app_user_id]['SEQUENCE_NO']}  $where_con
        GROUP by a.id, a.importer_id, a.source, a.supplier_id, a.pi_number, a.pi_date, a.export_pi_id, to_char(a.insert_date,'YYYY'), a.last_shipment_date, a.internal_file_no, a.approved, a.remarks, a.is_apply_last_update,  a.net_total_amount, $year_field,a.within_group,a.import_pi,c.IS_MAIL_SEND order by a.pi_date desc";
    }
    ...
    <script>
        function open_print_btn_popup(data){
            ...
            var page_link = 'requires/pi_approval_group_by_controller.php?action=print_button_variable&print_data='+data;
        ...
        <fieldset style="width:<? echo $width; ?>px; margin-top:10px">
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $width; ?>" class="rpt_table" align="left">
                <thead>
                    ...
                    <th width="70">PI Amount</th>
                    <th width="70">PI Net Amount</th>
                    ...
                    <th width="50">Cross Check</th>
                    <th colspan="2">Mail Status</th>
                </thead>
            </table>            
                            ...
                            foreach ($nameArray as $row)
                            {
                               ...
                               // print_r($format_ids[$j]);exit;
                                ...
                                <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>"> 
                                    ...
                                    <td width="70" align="right"><p><? echo $row[csf('net_pi_amount')]; ?></p></td>
                                    <td width="70" align="right"><p><? echo $row[csf('net_total_amount')]; ?></p></td>
                                    ...
                                    <td width="50" align="center"><a href="javascript:void();" onClick="fnc_pi_cross_check('<?=$row[csf('id')]; ?>','<?=$row[csf('importer_id')]; ?>');">View</a>
                                    <td align="center" valign="middle"><?= ($row['IS_MAIL_SEND'] == 1)?"Send":"Not Send";?></td>             
                                    <td align="center" valign="middle";>
                                        <? if($approval_type==1){ ?>
                                            <input type="button" value="<?=(($row['IS_MAIL_SEND'] == 1)?"Resend":"Send");?>" name="show" id="show" class="formbutton" style="width:60px" onClick="fn_resend_mail(<?= $row['ID']; ?>)"/> 
                                        <? } ?>
...
if ($action=="approve")
{
    ...
    $sql="SELECT a.ID, a.APPROVED,a.IMPORTER_ID, a.ITEM_CATEGORY_ID,a.BUYER_ID,a.BRAND_ID,a.READY_TO_APPROVED from com_pi_master_details a  where a.importer_id=$company_name and a.is_deleted=0 and a.status_active=1 and a.READY_TO_APPROVED=1  and a.id in($booking_ids)";    

    $sqlResult=sql_select( $sql );
    foreach ($sqlResult as $row)
    {
        if($row['READY_TO_APPROVED'] != 1){echo '25**Please select ready to approved yes for approved this PI';exit();}
    ...
    $finalDataArr=getFinalUser(array('company_id'=>$company_name,'entry_form'=>27,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>'','lib_item_cat_arr'=>$item_cat_arr,'lib_store_arr'=>0,'lib_department_id_arr'=>0,'match_data'=>$matchDataArr));
    ...
    $max_approved_no_arr = return_library_array( "select mst_id, max(approved_no) as approved_no from approval_history where entry_form=27 and mst_id in($booking_ids) group by mst_id", "mst_id", "approved_no" );
    ...
    if($approval_type == 0)
	{
		...
		foreach($target_app_id_arr as $mst_id)
		{
            ...
            $approved = ((max($finalDataArr['final_seq'][$mst_id][2])==$user_sequence_no) 
            || (in_array($user_sequence_no,$finalDataArr['final_seq'][$mst_id][1]) && max($finalDataArr['final_seq'][$mst_id][2]) =='' && $max_group_no == $user_group_no) 
            || ( (max($finalDataArr['group_bypass_no_data_arr'][$max_group_no][2])==$user_sequence_no) && ($max_group_no == $user_group_no))  
            || ( (max($finalDataArr['group_bypass_no_data_arr'][$max_group_no][2]) == '') && ($max_group_no == $user_group_no) ) )?1:3;
			...
			$data_array.="(".$id.",27,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$app_user_id.",'".$pc_date_time."','".$user_ip."','".$app_instru."',".$user_id.",'".$pc_date_time."',".$approved.")"; 
			$id++;	
            //App mst...............................
            if($mst_data_array!=''){$mst_data_array.=",";}
			$mst_data_array.="(".$app_mst_id.",27,".$mst_id.",".$user_sequence_no.",".$user_group_no.",".$app_user_id.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."',".$approved.")"; 
			$app_mst_id++;
            
            //Mst data...........................
			$mst_data_array_up[$mst_id] =explode(",",("".$approved.",".$user_sequence_no.",".$user_group_no.",'".$pc_date_time."',".$app_user_id."")); 
        ...
        if(count($approved_no_array)>0)
        {
            ...
            if($flag==1) 
            {
                $rID1=execute_query($sql_insert_his_mst,0); 
                if($rID1) $flag=1; else $flag=0; 
            }
    
            if($flag==1) 
            {
                $rID2=execute_query($sql_insert_his_dtls,0); 
		...
		if($flag==1) 
		{
			$mst_field_array="id, entry_form, mst_id,sequence_no,group_no,approved_by,approved_date,inserted_by,insert_date,user_ip,approved";
            $rID3=sql_insert("approval_mst",$mst_field_array,$mst_data_array,0);
        ...
        if($flag==1) 
        {
            $rID4=execute_query("UPDATE approval_history SET current_approval_status=0 WHERE entry_form=27 and mst_id in ($booking_ids)",1);
        ...
        if($flag==1) 
        {
            ...
            $rID5=sql_insert("approval_history",$field_array,$data_array,0);
		...
		if($flag==1) 
		{
			$mst_field_array_up="APPROVED*APPROVED_SEQU_BY*APPROVED_GROUP_BY*APPROVED_DATE*APPROVED_BY"; 
            $rID6=execute_query(bulk_update_sql_statement( "com_pi_master_details", "id", $mst_field_array_up, $mst_data_array_up, $target_app_id_arr ));
    ...
    elseif($approval_type == 1){
		...
		foreach($target_app_id_arr as $mst_id)
		{
            $approved_no=$max_approved_no_arr[$mst_id]*1;
			if($data_array!="") $data_array.=",";
			$data_array.="(".$id.",27,".$mst_id.",".$approved_no.",'".$user_sequence_no."',0,".$app_user_id.",'".$pc_date_time."','".$user_ip."','".$app_instru."',".$user_id.",'".$pc_date_time."',0)"; 
			$id++;	
		}
		...
		if($flag==1) 
		{
			$rID1=sql_multirow_update("com_pi_master_details","approved*ready_to_approved*APPROVED_SEQU_BY*APPROVED_GROUP_BY","0*0*0*0","id",$booking_ids,0); 
            if($rID1) $flag=1; else $flag=0; 
		}
    ...
    elseif($approval_type == 5){
		...
		foreach($target_app_id_arr as $mst_id)
		{
            $approved_no=$max_approved_no_arr[$mst_id]*1;
			if($data_array!="") $data_array.=",";
			$data_array.="(".$id.",27,".$mst_id.",".$approved_no.",'".$user_sequence_no."',0,".$app_user_id.",'".$pc_date_time."','".$user_ip."','".$app_instru."',".$user_id.",'".$pc_date_time."',2)"; 
			$id++;	
		}
		...
		if($flag==1) 
		{
			$rID1=sql_multirow_update("com_pi_master_details","approved*ready_to_approved*APPROVED_SEQU_BY*APPROVED_GROUP_BY","2*0*0*0","id",$booking_ids,0); 
            if($rID1) $flag=1; else $flag=0; 
		}
       ...
       $( document ).ready(function() {
        ...
        function fnc_appv_entry(operation)
        {
            ...
            else
            {
                ...
                http.open("POST","pi_approval_group_by_controller.php",true);
...
if($action=='user_popup')
{
    ...
    <form>
        ...
        <?php
            $sql="select a.id, a.user_name, a.department_id, a.user_full_name, a.designation,b.SEQUENCE_NO,b.GROUP_NO from  user_passwd a,electronic_approval_setup b where a.id=b.user_id and b.company_id=$cbo_company_name and b.entry_form=27  and a.id!=$user_id  and b.is_deleted=0 order by b.SEQUENCE_NO";
             $arr=array (2=>$designation_lib,3=>$Department);
             echo  create_list_view ( "list_view", "User ID,Full Name,Designation,Department,Seq,Group", "100,120,140,140,40,50,","640","220",0, $sql, "js_set_value", "id,user_name", "", 1, "0,department_id,designation,department_id,0,0", $arr , "user_name,user_full_name,designation,department_id,sequence_no,group_no", "requires/user_creation_controller", 'setFilterGrid("list_view",-1);' ) ;
        ?>
    </form>
...
if($action=='pi_approval_mail'){
   ...
   // echo $app_user_id;die;
    ...
    foreach($sql_dtls as $row){ 
        ...
        if($row['APPROVED'] ==1){$statusMsg = 'Full Approved';$mail_type=3;}
        else if($row['APPROVED'] ==3){$statusMsg = 'Partial Approved';$mail_type=4;}
        else if($row['APPROVED'] == 0){$statusMsg = 'Unapproved';$mail_type=7;}
        else if($row['APPROVED'] ==2){$statusMsg = 'Denied';$mail_type=6;}

        $sql = "SELECT c.EMAIL_ADDRESS FROM mail_group_mst a, mail_group_child b, user_mail_address c where b.mail_group_mst_id=a.id and a.mail_item=63 and b.mail_user_setup_id=c.id and a.company_id={$row['IMPORTER_ID']}   and a.IS_DELETED=0 and a.STATUS_ACTIVE=1  and b.IS_DELETED=0 and b.STATUS_ACTIVE=1   and c.IS_DELETED=0 and c.STATUS_ACTIVE=1 and a.mail_type = $mail_type";
        ...
        $elcetronicSql = "SELECT a.BUYER_ID,a.SEQUENCE_NO,a.BYPASS,b.USER_EMAIL,b.ITEM_CATE_ID   from electronic_approval_setup a join user_passwd b on a.user_id=b.id where a.entry_form=27 and a.SEQUENCE_NO>$user_sequence_no and a.company_id={$row['IMPORTER_ID']} order by a.SEQUENCE_NO"; //and a.page_id=2150 and a.entry_form=46
        ...
        $subject = "Pi Approval";
        if($to!=""){
            $send_res = sendMailMailer( $to, $subject, $message, $from_mail,$att_file_arr,1);
            if($send_res == 1){
                $con = connect();
                    $rID = execute_query("UPDATE APPROVAL_MST SET IS_MAIL_SEND=1 WHERE entry_form=27 and APPROVED_BY=$app_user_id and mst_id={$row['ID']}",1);
                oci_commit($con);
                disconnect($con);
            }


           // echo "UPDATE APPROVAL_MST SET IS_MAIL_SEND=1 WHERE entry_form=27 and APPROVED_BY=$app_user_id and mst_id={$row['ID']}";
