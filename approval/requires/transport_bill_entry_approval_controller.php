...
$trns_arr=array(1=>"Export",2=>"Import");
...
{ 
	...
	$cbo_trns_type = str_replace("'","",$cbo_trns_type);
	$cbo_transport_company = str_replace("'","",$cbo_transport_company);
    ...
    $electronicDataArr=getSequence(array('company_id'=>$company_name,'ENTRY_FORM'=>89,'user_id'=>$user_id_approval,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'product_dept_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0));
  // print_r($company_name);
	...
	if($txt_booking_no!='') 
	{		
		$where_id_cond.=" and a.bill_no ='".$txt_bill_no."'"; 
	}
	...
	if($txt_date_from && $txt_date_to){
		$where_id_cond .= " and a.bill_date BETWEEN '".$txt_date_from."' AND '".$txt_date_to."'";	
	}
    ...
    $cbo_type_cond = '';
	if($cbo_trns_type != 0){
		$cbo_type_cond .= " and a.type_id =$cbo_trns_type";
	}

    $cbo_candf_cond = '';
	if($cbo_candf_name != 0){
		$cbo_candf_cond .= " and a.cnf_name_id =$cbo_candf_name";
	}
	...
	{  
		...
		//  }
        $data_mast_sql = "select a.ID,a.ready_to_approve, a.company_id  
        from transport_bill_mst a  where a.company_id = $company_name and a.status_active = 1 and a.is_deleted = 0 and a.ready_to_approve = 1 and a.IS_APPROVED<>1 ";
		...
		for($seq=0;$seq<=count($electronicDataArr['sequ_arr']); $seq++ ){
			...
			if($tmp_sys_id_arr[$seq]){
				...
				$sql.="select a.ID,a.sys_number,a.sys_number_prefix_num,a.ready_to_approve, a.company_id ,a.location_id ,a.trans_com_id ,a.bill_date ,a.bill_no ,a.type_id ,a.ship_mode ,a.depo,a.port_name ,a.remarks 
                from transport_bill_mst a where a.status_active = 1 and a.is_deleted = 0 and a.company_id = $company_name $where_id_cond  $cbo_type_cond $cbo_candf_cond $txt_system_no_cond   and a.IS_APPROVED<>1  and a.APPROVED_SEQU_BY=$seq $sys_con group by a.ID,a.sys_number,a.ready_to_approve,
				a.company_id,a.location_id,a.trans_com_id,a.sys_number_prefix_num,a.bill_date,a.bill_no,a.type_id,a.ship_mode,a.depo,
				a.port_name,a.remarks";
	...
	{   $sql= "select a.ID,a.sys_number,a.sys_number_prefix_num,a.ready_to_approve, a.company_id ,a.location_id ,a.trans_com_id ,a.   bill_date ,a.bill_no ,a.type_id ,a.ship_mode ,a.depo,a.port_name ,a.remarks from transport_bill_mst a,APPROVAL_MST b where a.status_active = 1 and a.is_deleted = 0 and a.company_id = $company_name $where_id_cond  $cbo_type_cond $cbo_candf_cond $txt_system_no_cond and b.SEQUENCE_NO={$electronicDataArr['user_by'][$user_id_approval]['SEQUENCE_NO']} and a.APPROVED_SEQU_BY=b.SEQUENCE_NO and a.IS_APPROVED<>0 and b.mst_id=a.id group by a.ID,a.sys_number,a.ready_to_approve,a.sys_number_prefix_num,
	a.company_id,a.location_id,a.trans_com_id,a.bill_date,a.bill_no,a.type_id,a.ship_mode,a.depo,
	a.port_name,a.remarks";
    ...
    sum(b.amount) as amount,sum (b.cbm_amt) as cbm_amt from transport_bill_mst a, TRANSPORT_BILL_DTLS b where a.id=b.mst_id and a.company_id =$company_name $sis_cond  and a.status_active=1 and b.status_active=1 group by  a.id, b.mst_id ");
							...
							foreach($total_qty_sql as $row)
							{
								...
								$total_cbm_arr[$row[csf("id")]]=$row[csf("cbm_amt")];
							}
        ...
        <div style="width:<? echo $table1+25; ?>px; margin:0 auto;">
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="left" >
                <thead>
                    ...
                    <th width="125"> Trans. Type </th>
                    <th width="125">Bill NO</th>
                    <th width="100">Bill Date</th>
                    <th width="150">Ship Mode</th> 
                    <th width="150">Carton Qty</th>                   
                    <th width="100">CBM</th>                   
                    <th width="100">Total Amnt (TK)</th>                   
                    <th width="100">Total Deduction (TK)</th>                  
                    <th width="100">Payable Amnt (TK)</th>                 
                ...
                <table cellspacing="0" cellpadding="0" border="1" rules="all"  class="rpt_table" id="tbl_list_search" align="left">
                    <tbody>
                        ...
                        {                                                   
							...
							if($report_ids[0]==163){ $action="show_trim_booking_report1";}
							...
							else if($report_ids[0]==289){ $action="show_trim_booking_report5";}
                                ...
                                <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>"> 
                                    ...
                                    <td width="125"><? echo $trns_arr[$row[csf('type_id')]]; ?> &nbsp;</p></td>                                   
                                    <td width="125" align="center"><p><? echo $row[csf('bill_no')]; ?>&nbsp;</p></td>
									<td width="100" align="center"><p><? echo change_date_format($row[csf('bill_date')]); ?>&nbsp;</p></td>
                                    <td width="150"><p><? echo $shipment_mode[$row[csf('ship_mode')]]; ?> &nbsp;</p></td>
									
                                    <td width="150"><p></td>				
                                    <td width="100"><p><? echo $total_cbm_arr[$row[csf("id")]]; ?>&nbsp;</p></td>
                                    <td width="100"><p><? echo $total_amount_arr[$row[csf("id")]]; ?> &nbsp;</p></td>				                                  
                                    <td width="100"><p><? echo $total_deduce_arr[$row[csf("id")]]; ?>&nbsp;</p></td>				                                  
                                    <td width="100"><p><? echo $total_payble_arr[$row[csf("id")]]; ?> &nbsp;</p></td>				                                  
...
{ 
	...
	$sql = "select a.ID,a.READY_TO_APPROVE  from transport_bill_mst a where a.COMPANY_ID=$cbo_company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0  and a.id in($target_ids)";
	//echo $sql;die();
	$sqlResult=sql_select( $sql );
	foreach ($sqlResult as $row)
	{
		//if($row['READY_TO_APPROVED'] != 1){echo '21**Ready to approve Yes is mandatory';exit();}
		$matchDataArr[$row['ID']]=array('buyer_id'=>0,'brand_id'=>0,'store'=>0);
	}
	
	$finalDataArr=getFinalUser(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>89,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0,'product_dept_arr'=>0,'match_data'=>$matchDataArr));
  ...
  //print_r($user_sequence_no) ;die;
	...
	{ 
        ...
        foreach($target_app_id_arr as $mst_id)
        {
			...
			$data_array.="(".$id.",89,'".$mst_id."','".$user_sequence_no."',".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			
			$approved_no=$max_approved_no_arr[$mst_id][$user_id_approval]+1;
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",89,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1)";
			$ahid++;
		...
		if($flag==1) 
		{
			...
			$rID2=execute_query(bulk_update_sql_statement( "transport_bill_mst", "id", $field_array_up, $data_array_up, $target_app_id_arr ));
			if($rID2) $flag=1; else $flag=0; 
		}

		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=89 and mst_id in ($target_ids)";
	...
	{
		$max_approved_no_arr=return_library_array( "select mst_id, max(approved_no) as approved_no from approval_history where entry_form=89 and mst_id in($target_ids)  and APPROVED=2 and APPROVED_BY=$user_id_approval group by mst_id", "mst_id", "approved_no"  );
		$ahid=return_next_id( "id","approval_history", 1 ) ;	
		
		$rID1=sql_multirow_update("transport_bill_mst","IS_APPROVED*READY_TO_APPROVE*APPROVED_SEQU_BY",'2*0*0',"id",$target_ids,0); ; 
		if($rID1) $flag=1; else $flag=0;

		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0, un_approved_by='".$user_id_approval."', un_approved_date='".$pc_date_time."', updated_by='".$user_id."', update_date='".$pc_date_time."' WHERE entry_form=89 and current_approval_status=1 and mst_id in ($target_ids)";
			$rID3=execute_query($query,1);
			if($rID3) $flag=1; else $flag=0;
			...
			{		
				...
				if($history_data_array!="") $history_data_array.=",";
				$history_data_array.="(".$ahid.",89,".$mst_id.",".$approved_no.",'".$user_sequence_no."',0,".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,2)";
				$ahid++;
			}		
	...
	{            
		...
		$next_user_app = sql_select("select id from approval_history where mst_id in($target_ids) and entry_form=89 and sequence_no >$user_sequence_no and current_approval_status=1 group by id");
		...
		$rID1=sql_multirow_update("transport_bill_mst","IS_APPROVED*READY_TO_APPROVE*APPROVED_SEQU_BY",'0*0*0',"id",$target_ids,0);
		if($rID1) $flag=1; else $flag=0;
		...
		if($flag==1) 
		{
			$query="delete from approval_mst  WHERE entry_form=89 and mst_id in ($target_ids)";
			$rID3=execute_query($query,1); 
			if($rID3) $flag=1; else $flag=0; 
		}
    ...
    <form>
           ...
           <?php
            $custom_designation=return_library_array( "select id,custom_designation from lib_designation ",'id','custom_designation');  
             ...
             $sql="select a.id, a.user_name, a.department_id,b.sequence_no, a.user_full_name, a.designation from  user_passwd a,electronic_approval_setup b where a.id=b.user_id and b.company_id=$cbo_company_name and b.entry_form=89 and valid=1 and a.id!=$user_id   and a.is_deleted=0 AND b.is_deleted = 0 group by a.id,a.user_name,a.department_id,a.user_full_name,b.SEQUENCE_NO,a.designation order by b.SEQUENCE_NO";
                //echo $sql;die;
...
if ($action=="show_work_order_aop_report")
{
	...
	$com_supplier_id=return_field_value( "supplier_id as supplier_id", "transport_bill_mst","booking_no=$txt_booking_no","supplier_id");
	$nameArray_approved=sql_select( "select max(b.approved_no) as approved_no from transport_bill_mst a, approval_history b where a.id=b.mst_id and booking_no=$txt_booking_no and b.entry_form=162"); 
	...
	<div style="width:1540px; margin: 0 auto" align="center">       
       ...
       <?
		...
		$fabric_source='';
		$booking_info=sql_select( "select id,buyer_id,booking_no,booking_date,supplier_id,exchange_rate,currency_id,attention,delivery_date,fabric_source,entry_form from transport_bill_mst where booking_no=$txt_booking_no"); 
	...
	if($db_type==0)
	{
		$sql= sql_select("select a.booking_no, a.booking_type, a.booking_date, a.delivery_date, a.buyer_id, a.supplier_id, a.job_no, a.item_category, a.source, a.attention, a.process, b.dia_width, b.uom, a.pay_mode, a.exchange_rate, a.currency_id, b.wo_qnty, b.amount, b.pre_cost_remarks FROM transport_bill_mst a, wo_booking_dtls b  WHERE a.booking_no=$txt_booking_no and a.booking_no=b.booking_no and a.status_active=1 and a.is_deleted=0 order by a.id"); 
	}
	if($db_type==2)
	{
		$sql= sql_select("select a.booking_no, a.booking_type, a.booking_date, a.delivery_date, a.buyer_id, a.supplier_id, a.job_no, a.item_category, a.source, a.attention, a.process, b.dia_width, b.uom, a.pay_mode, a.exchange_rate, a.currency_id, b.wo_qnty, b.amount, b.pre_cost_remarks FROM transport_bill_mst a, wo_booking_dtls b  WHERE a.booking_no=$txt_booking_no and a.booking_no=b.booking_no and a.status_active=1 and a.is_deleted=0 order by a.id"); 
            ...
            <tr>
                <td width="100%">
                    ...
                    <?
 					...
 					$sel=sql_select("select a.approved_by, a.approved_date, a.id as app_no from approval_history a, electronic_approval_setup b where a.mst_id in(select id from transport_bill_mst where id=$bookingId) and a.entry_form=162 and a.approved_by=b.user_id and b.company_id=$cbo_company_name and b.is_deleted=0 group by a.approved_by, a.approved_date, a.id, b.sequence_no order by b.sequence_no asc");
