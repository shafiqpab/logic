...
function getFinalUser($parameterArr=array()){
	...
	foreach($sql_result as $rows){
		...
		if($userDataArr[$rows['USER_ID']]['BRAND_ID']==''){
			$userDataArr[$rows['USER_ID']]['BRAND_ID']=implode(',',$temp_brand_arr);
		}
	...
	foreach($parameterArr['match_data'] as $sys_id=>$bbtsRows){
		...
		foreach($userSeqDataArr as $user_id=>$seq){
			if(
				...
				&& in_array($bbtsRows['pro_department'],$usersDataArr[$user_id]['DEPARTMENT'])
				&& $bbtsRows['pro_department']!='' && $bbtsRows['buyer']>0 && $bbtsRows['brand']!=''
			){
...
$brandSql = "select ID,BUYER_ID from LIB_BUYER_BRAND where STATUS_ACTIVE=1 and IS_DELETED=0";
$brandSqlRes=sql_select($brandSql);
foreach($brandSqlRes as $rows){
	$brand_arr[$rows['BUYER_ID']][$rows['ID']] = $rows['ID'];
}
...
if($action=="report_generate")
{
	...
	$user_id_approval=($alter_user_id!='')?$alter_user_id:$user_id;
	...
	{
		if($db_type==2) $where_con.=" and to_char(a.insert_date,'YYYY')='".trim($cbo_year)."'";
		else $where_con.=" and YEAR(a.insert_date)='".trim($cbo_year)."'";
	}
	...
	$electronicDataArr=getSequence(array('company_id'=>0,'page_id'=>$page_id,'entry_form'=>70,'user_id'=>$user_id_approval,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>$brand_arr,'product_dept_arr'=>$product_dept,'lib_item_cat_arr'=>0,'lib_store_arr'=>0));
	 //var_dump($electronicDataArr[seq_buyer]);die;
	...
	if($approval_type==2) // Un-Approve
	{
		...
		if($electronicDataArr['user_by'][$user_id_approval]['BRAND_ID']){
			$where_con .= " and a.BRAND_ID in(".$electronicDataArr['user_by'][$user_id_approval]['BRAND_ID'].",0)";
		...
		if($electronicDataArr['user_by'][$user_id_approval]['DEPARTMENT']){
			$where_con .= " and a.PROD_DEPT in(".$electronicDataArr['user_by'][$user_id_approval]['DEPARTMENT'].",0)";
			$electronicDataArr['sequ_by'][0]['DEPARTMENT']=$electronicDataArr['user_by'][$user_id_approval]['DEPARTMENT'];
		}
 

		$data_mast_sql = " select a.ID,a.BRAND_ID, a.BUYER_ID,a.PROD_DEPT from qc_mst a,qc_tot_cost_summary b,qc_confirm_mst c where a.qc_no=b.mst_id and b.mst_id=c.cost_sheet_id and a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0 and c.STATUS_ACTIVE=1 and c.IS_DELETED=0 and a.approved<>1 and c.READY_TO_APPROVE=1 $where_con";
		...
		foreach ($data_mas_sql_res as $row){ 
			for($seq=($electronicDataArr['user_by'][$user_id_approval]['SEQUENCE_NO']-1);$seq>=0; $seq-- ){
				 ...
				 if($electronicDataArr['sequ_by'][$seq]['DEPARTMENT']==''){$electronicDataArr['sequ_by'][$seq]['DEPARTMENT']=0;}

					// echo $electronicDataArr['sequ_by'][$seq]['DEPARTMENT'];die;

				 
				 if(
					(in_array($row['BUYER_ID'],explode(',',$electronicDataArr['sequ_by'][$seq]['BUYER_ID'])))
					&& (in_array($row['BRAND_ID'],explode(',',$electronicDataArr['sequ_by'][$seq]['BRAND_ID'])) || $row['BRAND_ID']==0)
					&& (in_array($row['PROD_DEPT'],explode(',',$electronicDataArr['sequ_by'][$seq]['DEPARTMENT'])))
    ...
    <script>
		function openmypage_app_cause(wo_id,app_type,i)
		{
			...
			var page_link = 'requires/quick_costing_approval_v3_controller.php?data='+data+'&action=appcause_popup';
		...
		function openmypage_app_instrac(wo_id,app_type,i)
		{
			...
			var page_link = 'requires/quick_costing_approval_v3_controller.php?data='+data+'&action=appinstra_popup';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=500px,height=250px,center=1,resize=1,scrolling=0','');
			emailwindow.onclose=function()
			{
				var appv_cause=this.contentDoc.getElementById("hidden_appv_cause");
		...
		function openmypage_unapp_request(wo_id,app_type,i)
		{
			...
			var page_link = 'requires/quick_costing_approval_v3_controller.php?data='+data+'&action=unappcause_popup';
						...
						foreach ($nameArray as $row)
						{
							...
							<tr bgcolor="<?=$bgcolor; ?>" onClick="change_color('tr_<?=$i; ?>','<?=$bgcolor; ?>')" id="tr_<?=$i; ?>" align="center">
                                ...
                                <td align='center' width='100'>
										<input style="width:80px;" type="text" class="text_boxes"  name="txtCause_<? echo $row[csf('qc_no')];?>" id="txtCause_<? echo $row[csf('qc_no')];?>" placeholder="browse" onClick="openmypage_refusing_cause('requires/quick_costing_approval_v3_controller.php?action=refusing_cause_popup','Refusing Cause','<? echo $row[csf('qc_no')];?>');" value="<? echo $refusingCaseArr[$row[csf('qc_no')]];?>"/></td>
...
if ($action=="approve")
{
	...
	{      
		...
		$qc_con_sql = "select READY_TO_APPROVE from qc_confirm_mst where COST_SHEET_ID in($target_ids)  and IS_DELETED=0  and STATUS_ACTIVE=1";
		$qc_con_sql_res=sql_select( $qc_con_sql );
		if(count($qc_con_sql_res)){
			foreach ($qc_con_sql_res as $row)
			{
		...
		$finalDataArr=getFinalUser(array('company_id'=>0,'entry_form'=>70,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>$brand_arr,'lib_item_cat_arr'=>$item_cat_arr,'lib_store_arr'=>$lib_store_arr,'product_dept_arr'=>$product_dept,'match_data'=>$matchDataArr));
...
if($action=='user_popup')
{
	...
	<form>
       ...
       <?php
		...
		$sql="select a.id,b.sequence_no, a.user_name, a.department_id, a.user_full_name, a.designation from  user_passwd a,electronic_approval_setup b where a.id=b.user_id and b.company_id=0 and b.entry_form=70 and valid=1 and a.id!=$user_id  and b.is_deleted=0  order by b.sequence_no";
			//echo $sql;
		$arr=array (2=>$custom_designation,3=>$Department);
		echo  create_list_view ( "list_view", "User ID,Full Name,Designation,Department,App Seq", "100,120,130,120,50","630","320",0, $sql, "js_set_value", "id,user_name", "", 1, "0,department_id,designation,department_id", $arr , "user_name,user_full_name,designation,department_id,sequence_no", "requires/user_creation_controller", 'setFilterGrid("list_view",-1);' ) ;
		?>
	</form>
    ...
    <script>
		...
		function fnc_appv_entry(operation)
		{
			...
			else
			{
				...
				http.open("POST","quick_costing_approval_v3_controller.php",true);
