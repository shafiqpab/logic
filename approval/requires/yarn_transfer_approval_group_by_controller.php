...
{
	echo create_drop_down( "cbo_buyer_name", 150, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,30,90)) order by buy.buyer_name","id,buyer_name", 1, "-- All Buyer --", $selected, "load_drop_down('requires/yarn_transfer_approval_group_by_controller', this.value, 'load_drop_down_season', 'season_td');" );
	exit();
}
...
{ 
    ...
    $electronicDataArr=getSequence(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>95,'user_id'=>$app_user_id,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'product_dept_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0));
	...
	if($txt_style_ref!=''){$where_style_con .= " and c.style_ref_no ='".$txt_style_ref."'"; }
	if($txt_date) {$where_con .= " and a.transfer_date='".$txt_date."'";	}
	if($cbo_buyer_name != 0){$where_con .= " and a.buyer_id =$cbo_buyer_name";}
	if($txt_system_no != ""){$where_con .= " and a.transfer_system_id='" . trim($txt_system_no) . "' ";}
	if($txt_req_no != ''){$where_con .= " and a.booking_no LIKE '%$txt_req_no'";}
 ...
 $date_cond='';
	...
	{ 
		...
		$data_mast_sql = "select a.ID,b.BUYER_ID, a.transfer_system_id,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY, a.company_id from inv_item_transfer_mst a,inv_item_transfer_dtls b where a.COMPANY_ID=$cbo_company_name  and a.id=b.mst_id and a.STATUS_ACTIVE=1 and a.IS_DELETED=0  and a.IS_APPROVED<>1 $where_con and a.READY_TO_APPROVED=1 and a.entry_form=10 ";
		...
		for($group=0;$group<=$my_group; $group++ ){
			foreach($electronicDataArr['group_seq_arr'][$group] as $seq){
				...
				if(count($tmp_sys_id_arr[$group][$seq])){
					...
					$sql .= "select a.id,a.transfer_system_id, a.company_id,a.to_company,a.transfer_date, b.from_store, b.to_store,$select_year(a.insert_date $year_format) as year from inv_item_transfer_mst a,inv_item_transfer_dtls b where a.COMPANY_ID=$cbo_company_name $where_con $date_cond and a.id=b.mst_id and a.entry_form=10  and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and a.APPROVED_SEQU_BY=$seq and a.is_approved<>1 and  a.ready_to_approved=1 and a.APPROVED_GROUP_BY=$group $sys_con";
				}
	...
	{   
		...
		$sql = "select a.id,a.transfer_system_id, a.company_id,a.to_company,a.transfer_date, b.from_store, b.to_store,$select_year(a.insert_date $year_format) as year from inv_item_transfer_mst a,inv_item_transfer_dtls b,APPROVAL_MST c where a.COMPANY_ID=$cbo_company_name $where_con $date_cond and a.id=b.mst_id and a.entry_form=10  and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0  and a.id=c.mst_id and a.is_approved<>0 and  a.ready_to_approved=1 and a.APPROVED_GROUP_BY=c.GROUP_NO  and c.GROUP_NO={$electronicDataArr['user_by'][$app_user_id]['GROUP_NO']}"; 
    ...
    <script>
		function openmypage_app_cause(wo_id,app_type,i)
		{
			...
			var page_link = 'requires/yarn_transfer_approval_group_by_controller.php?data='+data+'&action=appcause_popup';
    ...
    <form name="requisitionApproval_2" id="requisitionApproval_2">
        ...
        <legend>Yarn Work Order Approval.</legend>
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1040" class="rpt_table" align="left" >
                <thead>
                	<th width="50">&nbsp;</th>
                    <th width="30">SL</th>
            ...
            <div style="width:1060px; overflow-y:scroll; max-height:330px;" id="buyer_list_view" align="left">
                    ...
                    <tbody>
                        <?
                            ...
                            foreach ($nameArray as $row)
                            {
								...
								$print_report_format=return_field_value("format_id","lib_report_template","template_name =".$cbo_company_name." and module_id=6 and report_id =313 and is_deleted=0 and status_active=1");
                            	$format_idss=explode(",",$print_report_format);    
       							//print_r($format_idss);die;
       							foreach ($format_idss as $key => $format_ids) 
       							{
	                                if($format_ids == 66){
	                                    $variable = "<a href='../inventory/yarn/requires/yarn_transfer_controller.php?data=$cbo_company_name*".$row[csf('id')]."*Yarn Purchase Challan*0*3*".$row[csf('is_approved')]."*&action=yarn_transfer_print2' style='color:#475DEC' target='_blank'>". $row[csf('transfer_system_id')]."</a>";
	                                }elseif($format_ids == 84){
	                                    $variable = "<a href='../commercial/work_order/requires/yarn_work_order_controller.php?data=$cbo_company_name*".$row[csf('id')]."*Yarn Purchase Order*1*0*".$row[csf('is_approved')]."&action=print_to_html_report' style='color:#000' target='_blank'>". $row[csf('transfer_system_id')]."</a>";
	                                }elseif($format_ids == 85){
	                                	//echo 'system';
	                                    $variable = "<a href='../commercial/work_order/requires/yarn_work_order_controller.php?data=$cbo_company_name*".$row[csf('id')]."*Yarn Purchase Order*2*0*".$row[csf('is_approved')]."&action=print_to_html_report2' style='color:#000' target='_blank'>". $row[csf('transfer_system_id')]."</a>";
	                                }elseif($format_ids == 193){
	                                	//echo 'systemfalse';
	                                    $variable = "<a href='../commercial/work_order/requires/yarn_work_order_controller.php?data=$cbo_company_name*".$row[csf('id')]."*Yarn Purchase Order*4*0*".$row[csf('is_approved')]."&action=print_to_html_report4' style='color:#000' target='_blank'>". $row[csf('transfer_system_id')]."</a>";
	                                }else{
	                                    $variable = $row[csf('transfer_system_id')];
	                            	}
                            	}
								...
								<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
									...
									<td width="70"><? if($row[csf('transfer_date')]!="0000-00-00") echo change_date_format($row[csf('transfer_date')]); ?></td>
                                    <td width="120" style="word-break:break-all" align="center"><?= $row[csf('year')]; ?></td>
									<td width="100" align="center"><?=$company_arr[$row[csf('company_id')]]; ?></td>
									<td width="100" align="center"><?=$company_arr[$row[csf('to_company')]]; ?></td>
                                    
                                    <td width="100" style="word-break:break-all"><?=$store_name_arr[$row[csf('from_store')]]; ?></td>
                                    <td width="100" style="word-break:break-all"><?=$store_name_arr[$row[csf('to_store')]]; ?></td>
            ...
            <table cellspacing="0" align="left" cellpadding="0" border="1" rules="all" width="1040" class="rpt_table">
				<tfoot>
...
if ($action=="approve")
{
	...
	$sql = "SELECT a.ID, a.READY_TO_APPROVED,a.IS_APPROVED,a.COMPANY_ID,a.APPROVED_GROUP_BY,a.APPROVED_SEQU_BY,b.BUYER_ID
	from inv_item_transfer_mst a,inv_item_transfer_dtls b where  a.COMPANY_ID=$cbo_company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0  and a.IS_APPROVED<>1 and a.READY_TO_APPROVED=1 and a.id=b.mst_id and a.id in($req_ids) and a.entry_form=10";
	...
	$finalDataArr=getFinalUser(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>95,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0,'product_dept_arr'=>0,'match_data'=>$matchDataArr));
   ...
   // print_r($user_sequence_no);die;
	...
	$max_approved_no_arr = return_library_array("select mst_id, max(approved_no) as approved_no from approval_history where mst_id in($req_ids) and entry_form=95 group by mst_id","mst_id","approved_no");
	...
	{ 
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			...
			$data_array.="(".$id.",95,'".$mst_id."','".$user_sequence_no."','".$user_group_no."',".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			
		
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",95,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,".$approved.")";
			$ahid++;
		...
		if($flag==1) 
		{
			...
			$usql = bulk_update_sql_statement( "inv_item_transfer_mst", "id", $field_array_up, $data_array_up, $target_app_id_arr );
		...
		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=95 and mst_id in ($req_ids)";
		...
		if(count($approved_no_array)>0 && $flag)
		{
			...
			$sql_insert="insert into inv_item_transfer_mst_history(id, mst_id, approved_no, garments_nature, wo_number_prefix, wo_number_prefix_num, wo_number, company_name, buyer_po, requisition_no, delivery_place, wo_date, supplier_id, attention, wo_basis_id, item_category, currency_id, delivery_date, source, pay_mode, terms_and_condition, is_approved, approved_by, status_active, is_deleted, inserted_by,insert_date,updated_by,update_date)
			select
			'', id, $approved_string_mst, garments_nature, wo_number_prefix, wo_number_prefix_num, wo_number, company_name, buyer_po, requisition_no, delivery_place, wo_date, supplier_id, attention, wo_basis_id, item_category, currency_id, delivery_date, source, pay_mode, terms_and_condition, is_approved, approved_by, status_active, is_deleted, inserted_by,insert_date,updated_by,update_date from  inv_item_transfer_mst where id in ($req_ids)";
	...
	{              
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			...
			$history_data_array.="(".$ahid.",95,".$mst_id.",'".$approved_no."','".$user_sequence_no."',0,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,2)";
			$ahid++;
        }

		

		$rID1=sql_multirow_update("inv_item_transfer_mst","IS_APPROVED*READY_TO_APPROVED*APPROVED_SEQU_BY*APPROVED_GROUP_BY",'2*0*0*0',"id",$req_ids,0);
		if($rID1) $flag=1; else $flag=0;
		...
		if($flag==1) 
		{
			$query="delete from approval_mst  WHERE entry_form=95 and mst_id in ($req_ids)";
			$rID3=execute_query($query,1); 
			if($rID3) $flag=1; else $flag=0; 
		}
	...
	{              
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			$approved_no = $max_approved_no_arr[$mst_id]*1;
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",95,".$mst_id.",".$approved_no.",'".$user_sequence_no."',0,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			$ahid++;
        }

		//echo $req_ids;die;

		$rID1=sql_multirow_update("inv_item_transfer_mst","IS_APPROVED*READY_TO_APPROVED*APPROVED_SEQU_BY*APPROVED_GROUP_BY",'0*0*0*0',"id",$req_ids,0);
		if($rID1) $flag=1; else $flag=0;
...
if ($action=="appcause_popup")
{
	...
	if($app_cause=="")
	{
		$sql_cause="select MAX(id) as id from fabric_booking_approval_cause where page_id='$menu_id' and entry_form=95 and user_id='$user_id' and booking_id='$wo_id' and approval_type=$app_type and status_active=1 and is_deleted=0";
    ...
    <script>
		...
		function fnc_appv_entry(operation)
		{
			...
			else
			{
				...
				http.open("POST","yarn_transfer_approval_group_by_controller.php",true);
...
if ($action=="save_update_delete_appv_cause")
{
	...
	if($approval_type==0)
	{
		...
		if ($operation==0)  // Insert Here
		{
			...
			$approved_no_history=return_field_value("approved_no","approval_history","entry_form=95 and mst_id=$wo_id and approved_by=$user_id");
			$approved_no_cause=return_field_value("approval_no","fabric_booking_approval_cause","entry_form=95 and booking_id=$wo_id and user_id=$user_id and approval_type=$appv_type");
			...
			if($approved_no_history=="" && $approved_no_cause=="")
			{
				...
				$data_array="(".$id_mst.",".$page_id.",95,".$user_id.",".$wo_id." ,".$appv_type.",0,".$appv_cause.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			...
			else if($approved_no_history=="" && $approved_no_cause!="")
			{
				...
				$id_cause=return_field_value("max(id)","fabric_booking_approval_cause","entry_form=95 and booking_id=$wo_id and user_id=$user_id and approval_type=$appv_type");
				...
				$data_array="".$page_id."*95*".$user_id."*".$wo_id."*".$appv_type."*0*".$appv_cause."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0";
			...
			{
				$max_appv_no_his=return_field_value("max(approved_no)","approval_history","entry_form=95 and mst_id=$wo_id and approved_by=$user_id");
				$max_appv_no_cause=return_field_value("max(approval_no)","fabric_booking_approval_cause","entry_form=95 and booking_id=$wo_id and user_id=$user_id and approval_type=$appv_type");
				...
				if($max_appv_no_his!=$max_appv_no_cause)
				{
					...
					$field_array="id,page_id,entry_form,user_id,booking_id,approval_type,approval_no,approval_cause,inserted_by,insert_date,status_active,is_deleted";
					$data_array="(".$id_mst.",".$page_id.",95,".$user_id.",".$wo_id." ,".$appv_type.",".$max_appv_no_his.",".$appv_cause.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
					$rID=sql_insert("fabric_booking_approval_cause",$field_array,$data_array,0);
					//echo $rID; die;
				...
				else if($max_appv_no_his==$max_appv_no_cause)
				{
					...
					$field_array="page_id*entry_form*user_id*booking_id*approval_type*approval_no*approval_cause*updated_by*update_date*status_active*is_deleted";
					$data_array="".$page_id."*95*".$user_id."*".$wo_id."*".$appv_type."*".$max_appv_no_his."*".$appv_cause."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0";
	...
	if($approval_type==1)
	{
		...
		if ($operation==0)  // Insert Here
		{
			...
			$unapproved_cause_id=return_field_value("id","fabric_booking_approval_cause","entry_form=95 and user_id=$user_id and booking_id=$wo_id  and approval_type=$appv_type and approval_history_id=$approval_id");
			...
			if($unapproved_cause_id=="")
			{
				...
				$data_array="(".$id_mst.",".$page_id.",95,".$user_id.",".$wo_id." ,".$appv_type.",".$max_appv_no_his.",".$approval_id.",".$appv_cause.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			...
			else
			{
				...
				$id_cause=return_field_value("max(id)","fabric_booking_approval_cause","entry_form=95 and user_id=$user_id and booking_id=$wo_id  and approval_type=$appv_type and approval_history_id=$approval_id");
				...
				$data_array="".$page_id."*95*".$user_id."*".$wo_id."*".$appv_type."*".$max_appv_no_his."*".$approval_id."*".$appv_cause."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0";
