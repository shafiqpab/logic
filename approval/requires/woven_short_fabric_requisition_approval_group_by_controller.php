...
{ 
	...
	$electronicDataArr=getSequence(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>93,'user_id'=>$app_user_id,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>$buyer_brand_arr,'product_dept_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0));
	...
	{ 
		...
		$data_mast_sql = "select a.ID, a.booking_no,a.BUYER_ID,b.BRAND_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY, a.company_id from wo_booking_mst a, wo_po_details_master b where a.COMPANY_ID=$cbo_company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0  and a.IS_APPROVED<>1 $where_con and a.entry_form=729 and a.job_no=b.job_no and a.READY_TO_APPROVED=1 ";
		...
		//echo "<pre>"; print_r($tmp_sys_id_arr);echo "</pre>"; die;
		// print_r($group_stage_arr); 
		// //echo array_key_first($group_stage_arr[48163]);die;
		// echo count($group_stage_arr); 
		...
		for($group=0;$group<=$my_group; $group++ ){
			foreach($electronicDataArr['group_seq_arr'][$group] as $seq){
				...
				if(count($tmp_sys_id_arr[$group][$seq])){
					...
					$sql .= "SELECT a.id,a.booking_no, a.booking_no_prefix_num,b.style_ref_no,a.season_id,b.dealing_marchant, b.job_no_prefix_num, a.booking_date, a.company_id, a.buyer_id, a.job_no, a.po_break_down_id, a.item_category, a.fabric_source, a.is_approved, a.ready_to_approved, b.brand_id, b.season_buyer_wise, b.season_year from wo_booking_mst a, wo_po_details_master b where a.entry_form=729 and a.job_no=b.job_no   and a.booking_type=14 and a.is_short=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.company_id=$cbo_company_name $where_con $where_style_con and a.APPROVED_SEQU_BY=$seq  and a.is_approved<>1 and  a.ready_to_approved=1 and a.APPROVED_GROUP_BY=$group $sys_con group by  a.id, a.booking_no_prefix_num, b.job_no_prefix_num,a.booking_no,a.season_id, a.booking_date, a.company_id, a.buyer_id, a.job_no,b.dealing_marchant, a.po_break_down_id, a.item_category, a.fabric_source, a.is_approved, a.ready_to_approved, b.brand_id ,b.season_buyer_wise,b.style_ref_no, b.season_year";
				}
    ...
    <form name="requisitionApproval_2" id="requisitionApproval_2">
        ...
        <legend>Yarn Work Order Approval</legend>
            ...
            <table cellspacing="0" align="left" ali cellpadding="0" border="1" rules="all" width="960" class="rpt_table">
				<tfoot>
...
if ($action=="approve")
{
	...
	$sql = "SELECT a.ID, a.READY_TO_APPROVED,a.IS_APPROVED,a.COMPANY_ID, a.SUPPLIER_ID,a.APPROVED_SEQU_BY,a.BUYER_ID,b.BRAND_ID from wo_booking_mst a ,wo_po_details_master b  where  a.COMPANY_ID=$cbo_company_name and a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and a.job_no=b.job_no  and a.IS_APPROVED<>1 and a.READY_TO_APPROVED=1 and a.id in($req_ids) and a.entry_form=729";
	...
	$finalDataArr=getFinalUser(array('company_id'=>$cbo_company_name,'ENTRY_FORM'=>93,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0,'product_dept_arr'=>0,'match_data'=>$matchDataArr));
   ...
   // print_r($user_sequence_no);die;
	...
	$max_approved_no_arr = return_library_array("select mst_id, max(approved_no) as approved_no from approval_history where mst_id in($req_ids) and entry_form=93 group by mst_id","mst_id","approved_no");
	...
	{ 
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			...
			$data_array.="(".$id.",93,'".$mst_id."','".$user_sequence_no."','".$user_group_no."',".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			
		
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",93,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,".$approved.")";
			$ahid++;
		...
		if($flag==1)
		{
			$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=93 and mst_id in ($req_ids)";
	...
	{              
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			...
			$history_data_array.="(".$ahid.",93,".$mst_id.",'".$approved_no."','".$user_sequence_no."',0,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,2)";
			$ahid++;
        }
		...
		if($flag==1) 
		{
			$query="delete from approval_mst  WHERE entry_form=93 and mst_id in ($req_ids)";
			$rID3=execute_query($query,1); 
			if($rID3) $flag=1; else $flag=0; 
		}
	...
	{              
        ...
        foreach($target_app_id_arr as $key => $mst_id)
        {
			$approved_no = $max_approved_no_arr[$mst_id]*1;
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$ahid.",93,".$mst_id.",".$approved_no.",'".$user_sequence_no."',0,".$user_id_approval.",'".$appv_causes_arr[$key]."','".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			$ahid++;
        }
...
if ($action=="appcause_popup")
{
	...
	if($app_cause=="")
	{
		$sql_cause="select MAX(id) as id from fabric_booking_approval_cause where page_id='$menu_id' and entry_form=93 and user_id='$user_id' and booking_id='$wo_id' and approval_type=$app_type and status_active=1 and is_deleted=0";
...
if ($action=="save_update_delete_appv_cause")
{
	...
	if($approval_type==0)
	{
		...
		if ($operation==0)  // Insert Here
		{
			...
			$approved_no_history=return_field_value("approved_no","approval_history","entry_form=93 and mst_id=$wo_id and approved_by=$user_id");
			$approved_no_cause=return_field_value("approval_no","fabric_booking_approval_cause","entry_form=93 and booking_id=$wo_id and user_id=$user_id and approval_type=$appv_type");
			...
			if($approved_no_history=="" && $approved_no_cause=="")
			{
				...
				$data_array="(".$id_mst.",".$page_id.",93,".$user_id.",".$wo_id." ,".$appv_type.",0,".$appv_cause.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			...
			else if($approved_no_history=="" && $approved_no_cause!="")
			{
				...
				$id_cause=return_field_value("max(id)","fabric_booking_approval_cause","entry_form=93 and booking_id=$wo_id and user_id=$user_id and approval_type=$appv_type");
				...
				$data_array="".$page_id."*93*".$user_id."*".$wo_id."*".$appv_type."*0*".$appv_cause."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0";
			...
			{
				$max_appv_no_his=return_field_value("max(approved_no)","approval_history","entry_form=93 and mst_id=$wo_id and approved_by=$user_id");
				$max_appv_no_cause=return_field_value("max(approval_no)","fabric_booking_approval_cause","entry_form=93 and booking_id=$wo_id and user_id=$user_id and approval_type=$appv_type");
				...
				if($max_appv_no_his!=$max_appv_no_cause)
				{
					...
					$field_array="id,page_id,entry_form,user_id,booking_id,approval_type,approval_no,approval_cause,inserted_by,insert_date,status_active,is_deleted";
					$data_array="(".$id_mst.",".$page_id.",93,".$user_id.",".$wo_id." ,".$appv_type.",".$max_appv_no_his.",".$appv_cause.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
					$rID=sql_insert("fabric_booking_approval_cause",$field_array,$data_array,0);
					//echo $rID; die;
				...
				else if($max_appv_no_his==$max_appv_no_cause)
				{
					...
					$field_array="page_id*entry_form*user_id*booking_id*approval_type*approval_no*approval_cause*updated_by*update_date*status_active*is_deleted";
					$data_array="".$page_id."*93*".$user_id."*".$wo_id."*".$appv_type."*".$max_appv_no_his."*".$appv_cause."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0";
	...
	if($approval_type==1)
	{
		...
		if ($operation==0)  // Insert Here
		{
			...
			$unapproved_cause_id=return_field_value("id","fabric_booking_approval_cause","entry_form=93 and user_id=$user_id and booking_id=$wo_id  and approval_type=$appv_type and approval_history_id=$approval_id");
			...
			if($unapproved_cause_id=="")
			{
				...
				$data_array="(".$id_mst.",".$page_id.",93,".$user_id.",".$wo_id." ,".$appv_type.",".$max_appv_no_his.",".$approval_id.",".$appv_cause.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			...
			else
			{
				...
				$id_cause=return_field_value("max(id)","fabric_booking_approval_cause","entry_form=93 and user_id=$user_id and booking_id=$wo_id  and approval_type=$appv_type and approval_history_id=$approval_id");
				...
				$data_array="".$page_id."*93*".$user_id."*".$wo_id."*".$appv_type."*".$max_appv_no_his."*".$approval_id."*".$appv_cause."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*1*0";
