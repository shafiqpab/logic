...
{
	echo create_drop_down( "cbo_buyer_name", 120, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id  $buyer_cond and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,90))  and b.tag_company='$data' group by buy.id, buy.buyer_name order by buy.buyer_name","id,buyer_name", 1, "-- All Buyer --", $selected, "load_drop_down( 'requires/quick_costing_approval_controller_knit', this.value, 'load_drop_down_brand', 'brand_td'); load_drop_down('requires/quick_costing_approval_controller_knit', this.value, 'load_drop_down_season', 'season_td');" );
	exit();
}
...
if($action=="report_generate")
{
	...
	$brand_arr[0]=0;$product_dept[0]=0;
	$electronicDataArr=getSequence(array('company_id'=>$cbo_company_id,'page_id'=>$menu_id,'user_id'=>$app_user_id,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>$brand_arr,'product_dept_arr'=>$product_dept,'lib_item_cat_arr'=>0,'lib_store_arr'=>0));
	 
	//var_dump($electronicDataArr['user_by'][$app_user_id]['BRAND_ID']);die;
	...
	if($approval_type==2) // Un-Approve
	{
		...
		if($electronicDataArr['user_by'][$app_user_id]['BUYER_ID']){
			$where_con .= " and a.BUYER_ID in(".$electronicDataArr['user_by'][$app_user_id]['BUYER_ID'].")";
		...
		if($electronicDataArr['user_by'][$app_user_id]['BRAND_ID']){
			...
			$data_mas_sql = "SELECT A.ID,a.BUYER_ID, A.DEPARTMENT_ID, A.BRAND_ID from QC_MST a where a.entry_form in(458,471)  and a.status_active=1 and a.is_deleted=0 $where_con";
	 ...
	 //var_dump($tmp_sys_id_arr);die;
		...
		for($seq=0;$seq<=count($electronicDataArr['sequ_arr']); $seq++ ){
			...
			if($tmp_sys_id_arr[$seq]){
				...
				$sql .= "  SELECT a.ID,a.QC_NO, a.INQUERY_ID, b.TOT_FOB_COST,a.COMPANY_ID,a.COSTING_PER, a.BRAND_ID, a.SEASON_ID, a.SEASON_YEAR, a.COST_SHEET_ID, a.COST_SHEET_NO, a.ENTRY_FORM, TO_CHAR(a.insert_date,'YYYY') as YEAR, a.STYLE_REF, a.BUYER_ID, a.DELIVERY_DATE, a.EXCHANGE_RATE, a.OFFER_QTY, a.COSTING_DATE, a.REVISE_NO, a.OPTION_ID,a.APPROVED_SEQU_BY, a.APPROVED_BY,  a.APPROVED_DATE, a.APPROVED, a.INSERTED_BY, a.REVISE_NO, a.OPTION_ID, c.JOB_ID,MAX(d.JOB_NO) AS JOB_NO, c.id as CONFIRM_ID from QC_MST a left join wo_po_details_master  d on  a.STYLE_REF = d.STYLE_REF_NO ,QC_TOT_COST_SUMMARY b,QC_CONFIRM_MST c where a.qc_no=b.mst_id and b.mst_id=c.cost_sheet_id and a.STATUS_ACTIVE=1  and  a.IS_DELETED=0 and b.STATUS_ACTIVE=1  and b.IS_DELETED=0 and c.STATUS_ACTIVE=1 and c.IS_DELETED=0 and a.approved<>1 and c.READY_TO_APPROVE=1 and a.APPROVED_SEQU_BY=$seq $where_con and a.entry_form in(458,471) and a.COMPANY_id=$cbo_company_id $sys_con group by a.ID,a.QC_NO, a.INQUERY_ID, b.TOT_FOB_COST,a.COMPANY_ID,a.COSTING_PER, a.BRAND_ID, a.SEASON_ID, a.SEASON_YEAR, a.COST_SHEET_ID, a.COST_SHEET_NO, a.ENTRY_FORM, TO_CHAR(a.insert_date,'YYYY'), a.STYLE_REF, a.BUYER_ID, a.DELIVERY_DATE, a.EXCHANGE_RATE, a.OFFER_QTY, a.COSTING_DATE, a.REVISE_NO, a.OPTION_ID,a.APPROVED_SEQU_BY, a.APPROVED_BY,  a.APPROVED_DATE, a.APPROVED, a.INSERTED_BY, a.REVISE_NO, a.OPTION_ID, c.JOB_ID, c.id
				";
			}
	...
	{
		$sql = "select a.ENTRY_FORM,a.id,a.qc_no, a.inquery_id, b.tot_fob_cost,a.company_id,a.costing_per,a.brand_id, a.season_id, a.season_year, a.cost_sheet_id, a.cost_sheet_no, a.entry_form, TO_CHAR(a.insert_date,'YYYY') as year, a.style_ref, a.buyer_id, a.delivery_date, a.exchange_rate, a.offer_qty, a.costing_date, a.revise_no, a.option_id,a.APPROVED_SEQU_BY, a.APPROVED_BY, a.APPROVED_DATE, a.approved, a.inserted_by, a.revise_no, a.option_id, d.id as JOB_ID, c.id as confirm_id,e.JOB_NO from qc_mst a left join wo_po_details_master e on a.STYLE_REF=e.STYLE_REF_NO,qc_tot_cost_summary b,qc_confirm_mst c,APPROVAL_MST d  where a.qc_no=b.mst_id and b.mst_id=c.cost_sheet_id and d.mst_id=a.qc_no and a.entry_form in(458,471) and  a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0  and c.STATUS_ACTIVE=1 and c.IS_DELETED=0 and c.READY_TO_APPROVE=1   and d.SEQUENCE_NO={$electronicDataArr['user_by'][$app_user_id]['SEQUENCE_NO']} and a.APPROVED_SEQU_BY=d.SEQUENCE_NO $where_con";
	}
	...
	{
		if($row['JOB_ID'])$job_id_arr[$row['JOB_ID']] = $row['JOB_ID'];
	}
 

	$costSql = "select JOB_ID,MARGIN_PCS_BOM from WO_PRE_COST_DTLS where IS_DELETED=0 and STATUS_ACTIVE=1 ".where_con_using_array($job_id_arr,0,'JOB_ID')."";
	$costSqlRes=sql_select( $costSql );
	$margin_pcs_bom_arr=array();
	foreach ($costSqlRes as $row)
	{
		$margin_pcs_bom_arr[$row['JOB_ID']] = $row['MARGIN_PCS_BOM'];
	}
    ...
    <script>
		function openmypage_app_cause(wo_id,app_type,i)
		{
			...
			var page_link = 'requires/quick_costing_approval_controller_knit.php?data='+data+'&action=appcause_popup';
		...
		function openmypage_app_instrac(wo_id,app_type,i)
		{
			...
			var page_link = 'requires/quick_costing_approval_controller_knit.php?data='+data+'&action=appinstra_popup';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=500px,height=250px,center=1,resize=1,scrolling=0','');
			emailwindow.onclose=function()
			{
				var appv_cause=this.contentDoc.getElementById("hidden_appv_cause");
		...
		function openmypage_unapp_request(wo_id,app_type,i)
		{
			...
			var page_link = 'requires/quick_costing_approval_controller_knit.php?data='+data+'&action=unappcause_popup';
	...
	$row_id=$format_ids[0];
	$report_action="quick_costing_print_p2";
	$report_action_1="preCostRpt2";
	...
	$refusingCaseArr=return_library_array( "select MST_ID, REFUSING_REASON from refusing_cause_history where ENTRY_FORM=28", "MST_ID", "REFUSING_REASON");
    ...
    <form name="requisitionApproval_2" id="requisitionApproval_2">
        <fieldset style="width:1670px; margin-top:10px">
        <legend>Quick Costing Approval V2</legend>
            ...
            <div style="width:1670px; overflow-y:scroll; max-height:330px;" id="buyer_list_view" align="center">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1652" class="rpt_table" id="tbl_list_search">
                    <tbody>
                        <?
                        $i=1;
						...
						foreach ($nameArray as $row)
						{
							...
							$bgcolor=($margin_pcs_bom_arr[$row['JOB_ID']]<0)?"#ff0000":$bgcolor;
							...
							<tr bgcolor="<?=$bgcolor; ?>" onClick="change_color('tr_<?=$i; ?>','<?=$bgcolor; ?>')" id="tr_<?=$i; ?>" align="center">
								...
								<td width="120">
    <p><a href='##' style='color:#000' onClick="fnc_print_report_1('<? echo $row[csf('job_no')]; ?>','<? echo $row[csf('COMPANY_ID')]; ?>','<? echo $row[csf('buyer_id')]; ?>','<? echo $row[csf('style_ref')]; ?>','<? echo $row[csf('costing_date')];?>','<? echo $cbo_costing_per; ?>','preCostRpt2')">
        View
    </a></p>
</td>
                                ...
                                <td align='center' width='100'>
										<input style="width:80px;" type="text" class="text_boxes"  name="txtCause_<? echo $row[csf('qc_no')];?>" id="txtCause_<? echo $row[csf('qc_no')];?>" placeholder="browse" onClick="openmypage_refusing_cause('requires/quick_costing_approval_controller_knit.php?action=refusing_cause_popup','Refusing Cause','<? echo $row[csf('qc_no')];?>');" value="<? echo $refusingCaseArr[$row[csf('qc_no')]];?>"/></td>
            ...
            <table cellspacing="0" cellpadding="0" border="0" rules="all" width="1670" class="rpt_table">
				<tfoot>
...
if ($action=="approve")
{
	...
	if($approval_type==5)
	{
		$sqlBookinghistory="select id, mst_id from approval_history where current_approval_status=1 and entry_form=70 and mst_id in ($target_ids) ";
		...
		$query="delete from approval_mst  WHERE entry_form=70 and mst_id in ($target_ids)";
		...
		if($approval_ids!="")
		{
			$query="UPDATE approval_history SET current_approval_status=0, un_approved_by='".$user_id_approval."', un_approved_date='".$pc_date_time."', updated_by='".$user_id."', update_date='".$pc_date_time."' WHERE entry_form=70 and current_approval_status=1 and mst_id in ($confirm_ids)";
	...
	{      
		...
		//$max_approved_no_arr = return_library_array("select mst_id, max(approved_no) as approved_no from approval_history where mst_id in($target_ids) and entry_form=70 group by mst_id","mst_id","approved_no");
		...
		{
			$matchDataArr[$row['QC_NO']]=array('buyer'=>$row['BUYER_ID'],'brand'=>$row['BRAND_ID'],'pro_department'=>$row['PROD_DEPT']);
			$qc_id_arr[$row['QC_NO']]=$row['QC_NO'];
		}
		
		//$matchDataArr[333]=array('buyer'=>0,'brand'=>0,'item'=>15,'store'=>358);
		$product_dept[0]=0;$brand_arr[0]=0;
		$finalDataArr=getFinalUser(array('company_id'=>$cbo_company_id,'page_id'=>$menu_id,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>$brand_arr,'lib_item_cat_arr'=>$item_cat_arr,'lib_store_arr'=>$lib_store_arr,'product_dept_arr'=>$product_dept,'match_data'=>$matchDataArr));
	 ...
	 //---------------------
		...
		$idd=return_next_id( "id","approval_history", 1 ) ;
		foreach($target_app_id_arr as $mst_id)
		{		
			if($data_array!=''){$data_array.=",";}
			$data_array.="(".$id.",70,".$mst_id.",".$user_sequence_no.",".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
			$id=$id+1;
			
			
			$approved_no=$max_approved_no_arr[$mst_id]+1;
			if($history_data_array!="") $history_data_array.=",";
			$history_data_array.="(".$idd.",70,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$user_id_approval.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			$idd=$idd+1;
		...
		//echo "insert into approval_mst $field_array value$data_array";die;
		$flag=1;
		...
		$query="UPDATE approval_history SET current_approval_status=0 WHERE entry_form=70 and mst_id in ($target_ids)";
		...
		//cho "0**".$rID,"**".$rID2,"**".$rID3,"**".$rID4,"**".$rID5,"**".$rID6;oci_rollback($con);die;
	...
	//echo "insert into approval_history ($field_array) values($history_data_array);";
...
if($action=='user_popup')
{
	...
	<form>
       ...
       <?php
		...
		$sql="select a.id, a.user_name, a.department_id, a.user_full_name, a.designation from  user_passwd a,electronic_approval_setup b where a.id=b.user_id and b.company_id=$cbo_company_id and b.entry_form=70 and valid=1 and a.id!=$user_id  and b.is_deleted=0  and b.page_id=$menu_id order by b.sequence_no";
			//echo $sql;
    ...
    <script>
		...
		function fnc_appv_entry(operation)
		{
			...
			else
			{
				...
				http.open("POST","quick_costing_approval_controller_knit.php",true);
...
if($action=="save_update_delete_refusing_cause")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		execute_query("delete refusing_cause_history where mst_id='$quo_id' and entry_form =28");
		$id=return_next_id( "id", "refusing_cause_history", 1);
		$field_array = "id,entry_form,mst_id,refusing_reason,inserted_by,insert_date";
		$data_array = "(".$id.",28,".$quo_id.",'".$refusing_cause."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
