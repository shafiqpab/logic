...
if ($action=="cbo_root_menu_under")
{
	...
	echo create_drop_down( "cbo_sub_main_menu_name", 155, "select m_menu_id,menu_name from main_menu where position='2' and root_menu ='$data' order by m_menu_id","m_menu_id,menu_name", 1, "-- Select Menu Name --", $selected, "load_drop_down( '../tools/requires/user_priviledge_controller', this.value, 'cbo_sub_root_menu_under', 'sub_subrootdiv' )" );
	
}

if ($action=="cbo_sub_root_menu_under")
{
	echo create_drop_down( "cbo_sub_menu_name", 155, "select m_menu_id,menu_name from main_menu where position='3' and sub_root_menu='$data' order by m_menu_id","m_menu_id,menu_name", 1, "-- Select Sub Menu --", $selected, "" );
...
if ($action=="save_update_delete")
{
	...
	else if( $cbo_set_module_privt == 0  ) 
	{
		...
		else if ($cbo_main_menu_name!=0 && $cbo_sub_main_menu_name!=0 && $cbo_sub_menu_name==0)
		{
			...
			$sql1=execute_query( "delete from user_priv_mst where main_menu_id in ( select m_menu_id from main_menu where m_module_id=$cbo_main_module and sub_root_menu in ($cbo_sub_main_menu_name)) and user_id=$cbo_user_name",1);
			
			$sql2=execute_query( "delete from user_priv_mst where main_menu_id in ( select m_menu_id from main_menu where  m_module_id=$cbo_main_module ) and user_id=$cbo_user_name and main_menu_id=$cbo_main_menu_name",1 );
			...
			$sql3=execute_query( "delete from user_priv_mst where main_menu_id in( select m_menu_id from main_menu where m_module_id=$cbo_main_module) and user_id=$cbo_user_name and main_menu_id=$cbo_sub_main_menu_name", 1 );
			...
			{
				$id4=$id4+1;
				$data_array1 .="( ".$id4.",".$cbo_user_name.",".$cbo_sub_main_menu_name.", ".$cbo_visibility.",".$cbo_delete.", ".$cbo_insert.", ".$cbo_edit.",".$cbo_approve.", 1 ),";
				...
				foreach ($nameArray as $inf)
				{
					$id4=$id4+1;
					$i++;
					if ($i!=$count) $data_array1 .="( ".$id4.",".$cbo_user_name.",".$inf[csf("m_menu_id")].", ".$cbo_visibility.",".$cbo_delete.", ".$cbo_insert.", ".$cbo_edit.",".$cbo_approve.", 1 ),";
					else $data_array1 .="( ".$id4.",".$cbo_user_name.",".$inf[csf("m_menu_id")].", ".$cbo_visibility.",".$cbo_delete.", ".$cbo_insert.", ".$cbo_edit.",".$cbo_approve.", 1 )";
		...
		else if ($cbo_main_menu_name!=0 && $cbo_sub_main_menu_name==0 && $cbo_sub_menu_name==0)
		{
			...
			$sql1=execute_query( "delete from user_priv_mst where main_menu_id in( select m_menu_id from main_menu where m_module_id=$cbo_main_module and root_menu=$cbo_main_menu_name) and user_id=$cbo_user_name", 1 );
			...
			if ($count>0)
			{
				...
				$data_array1 .="( ".$id4.",".$cbo_user_name.",".$cbo_main_menu_name.", ".$cbo_visibility.",".$cbo_delete.", ".$cbo_insert.", ".$cbo_edit.",".$cbo_approve.", 1 ),";
    ...
    <table width="100%" height="123" border="0" cellpadding="0" cellspacing="2">
		...
		<tr>
			<td>
							...
							echo create_drop_down( "cbo_main_menu_name", 260, "select m_menu_id,menu_name from main_menu where position='1' and m_module_id='".$data[1]."' order by m_menu_id","m_menu_id,menu_name", 1, "-- Select Menu --", $selected, "load_drop_down( '../tools/requires/user_priviledge_controller', this.value, 'cbo_root_menu_under', 'subrootdiv' )" );
							?>
