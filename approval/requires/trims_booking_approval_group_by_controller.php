...
if($action=='user_popup'){
	...
	<form>
       ...
       <?php
		...
		$sql="select a.id, a.user_name, a.department_id, a.user_full_name, a.designation,b.SEQUENCE_NO,b.GROUP_NO from  user_passwd a,electronic_approval_setup b where a.id=b.user_id and b.company_id=$cbo_company_name and b.entry_form=8 and valid=1 and a.id!=$user_id  and b.is_deleted=0  order by b.SEQUENCE_NO";
		 $arr=array (2=>$custom_designation,3=>$Department);
		 echo  create_list_view ( "list_view", "User ID,Full Name,Designation,Department,Seq,Group", "100,120,150,150,50,50,","680","220",0, $sql, "js_set_value", "id,user_name", "", 1, "0,department_id,designation,department_id,0,0", $arr , "user_name,user_full_name,designation,department_id,SEQUENCE_NO,GROUP_NO", "requires/user_creation_controller", 'setFilterGrid("list_view",-1);',"0,0,0,0,7,7" ) ;
		?>
	...
	foreach($parameterArr['match_data'] as $sys_id=>$bbtsRows){
		...
		foreach($userSeqDataArr as $user_id=>$seq){
			if(
				...
				&& (in_array($bbtsRows['brand'],$usersDataArr[$user_id]['BRAND_ID']) || $bbtsRows['brand']==0 || $bbtsRows['brand']=='')
			){
...
{ 
	...
	if($booking_year!=0){ $where_con .= " and to_char(a.insert_date,'YYYY')=$booking_year";}	
	...
	if($type==2){$where_con .= "and a.is_short=2";}//Short
    else if($type==3){$where_con .= "and a.is_short=1";}//Main
    else if($type==4){$where_con .= "and a.booking_type=5";}//Trims Sample
    
	if($txt_booking_no!=""){$where_con .= " and a.booking_no like '%$txt_booking_no%'";}
	
	
    $electronicDataArr=getSequence(array('company_id'=>$company_name,'ENTRY_FORM'=>8,'user_id'=>$app_user_id,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0,'lib_department_id_arr'=>0));
	...
	{		
		...
		if($electronicDataArr['user_by'][$app_user_id]['BRAND_ID']){
			$where_con .= " and (c.BRAND_ID in(".$electronicDataArr['user_by'][$app_user_id]['BRAND_ID'].",0)  or c.BRAND_ID is null)";
        ...
        {
            $data_mast_sql = "select a.ID,A.BUYER_ID,c.BRAND_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY from wo_booking_mst a,WO_BOOKING_DTLS b,wo_po_details_master c where a.booking_no =b.booking_no and b.job_no = c.job_no and a.company_id=$company_name $where_con and a.item_category=4 and a.SHORT_BOOKING_AVAILABLE in (0,1) and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and a.is_approved <> 1 and a.ready_to_approved=1 group by a.ID,A.BUYER_ID,c.BRAND_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY";
        }
        else  //WithOut Order
        {
            $data_mast_sql = "select a.ID,A.BUYER_ID,a.BRAND_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY from wo_non_ord_samp_booking_mst a where a.company_id=$company_name $where_con and a.item_category =4 and a.booking_type=5  and a.is_deleted=0 and a.status_active=1 and a.is_approved <>1 and a.ready_to_approved=1";
    	}
	 // echo $data_mast_sql;;		
		...
		foreach ($data_mas_sql_res as $row)
		{
			...
			for($group = ($row['APPROVED_GROUP_BY'] == $my_group && $electronicDataArr['sequ_by'][$my_seq]['BYPASS'] == 2)?$my_group:$my_group-1;$group >= 0; $group-- ){
				...
				foreach($electronicDataArr['group_seq_arr'][$group] as $seq){
					...
					if($seq<$my_seq){ 
						if(
							(in_array($row['BUYER_ID'],explode(',',$electronicDataArr['sequ_by'][$seq]['BUYER_ID'])) || $row['BUYER_ID']==0) && (in_array($row['BRAND_ID'],explode(',',$electronicDataArr['sequ_by'][$seq]['BRAND_ID'])) || $row['BRAND_ID'] == 0 || $row['BRAND_ID'] == '') && ($row['APPROVED_GROUP_BY'] <= $group)
							)
						{ 
		...
		for($group=0;$group<=$my_group; $group++ ){
			foreach($electronicDataArr['group_seq_arr'][$group] as $seq){
				...
				if(count($tmp_sys_id_arr[$group][$seq])){
                    ...
                    if($cbo_booking_type==1) //With Order
                    {
                        $sql .= "select A.ID,A.ENTRY_FORM, A.BOOKING_NO_PREFIX_NUM AS PREFIX_NUM,A.BOOKING_NO,A.ITEM_CATEGORY, A.FABRIC_SOURCE, A.COMPANY_ID,A.BOOKING_TYPE, A.IS_SHORT, A.PAY_MODE, A.SUPPLIER_ID, A.DELIVERY_DATE, A.BOOKING_DATE, '0' AS APPROVAL_ID, A.JOB_NO, A.IS_APPROVED, A.PO_BREAK_DOWN_ID,a.BUYER_ID from wo_booking_mst a, wo_booking_dtls b,wo_po_details_master c where a.booking_no=b.booking_no and b.job_no=c.job_no and a.company_id=$company_name and a.item_category in(4) and a.SHORT_BOOKING_AVAILABLE in (0,1) and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.is_approved <> 1 and a.APPROVED_SEQU_BY=$seq and a.APPROVED_GROUP_BY=$group $sys_con and a.ready_to_approved=1 group by a.id,a.booking_no_prefix_num,a.booking_no,a.entry_form, a.item_category, a.fabric_source, a.company_id, a.booking_type,a.entry_form, a.is_short, a.buyer_id, a.pay_mode, a.supplier_id, a.delivery_date, a.booking_date,a.job_no, a.is_approved, a.po_break_down_id ";
                        //echo $sql;die;//die("with hot pic");
                    }
                    else  //WithOut Order
                    {
                        $sql .= "select A.ID,a.ENTRY_FORM_ID as ENTRY_FORM, A.BOOKING_NO_PREFIX_NUM AS PREFIX_NUM,A.BOOKING_NO,A.ITEM_CATEGORY, A.FABRIC_SOURCE, A.COMPANY_ID, A.BOOKING_TYPE, A.IS_SHORT, A.PAY_MODE, A.SUPPLIER_ID, A.DELIVERY_DATE, A.BOOKING_DATE, '0' AS APPROVAL_ID,a.JOB_NO, A.IS_APPROVED, A.PO_BREAK_DOWN_ID,a.BUYER_ID from wo_non_ord_samp_booking_mst a,wo_non_ord_samp_booking_dtls b where a.booking_no=b.booking_no and a.company_id=$company_name and a.item_category in(4) and a.booking_type=5  and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.is_approved <>1  and a.APPROVED_SEQU_BY=$seq and a.APPROVED_GROUP_BY=$group $sys_con and a.ready_to_approved=1 group by A.ID,a.ENTRY_FORM_ID as ENTRY_FORM, A.BOOKING_NO_PREFIX_NUM AS PREFIX_NUM,A.BOOKING_NO,A.ITEM_CATEGORY, A.FABRIC_SOURCE, A.COMPANY_ID, A.BOOKING_TYPE, A.IS_SHORT,A.PAY_MODE, A.SUPPLIER_ID, A.DELIVERY_DATE, A.BOOKING_DATE, '0' AS APPROVAL_ID,a.JOB_NO, A.IS_APPROVED, A.PO_BREAK_DOWN_ID,a.BUYER_ID";
                        //echo $sql;die();
                    }   
       ...
       $sql = "SELECT * FROM ($sql) dum ORDER BY dum.booking_date";
	...
	else
	{
        ...
        if($cbo_booking_type==1) //With Order
        {
            $sql = "select A.ID,A.ENTRY_FORM, A.BOOKING_NO_PREFIX_NUM AS PREFIX_NUM,A.BOOKING_NO,A.ITEM_CATEGORY, A.FABRIC_SOURCE, A.COMPANY_ID, A.ENTRY_FORM,A.BOOKING_TYPE, A.IS_SHORT, A.PAY_MODE, A.SUPPLIER_ID, A.DELIVERY_DATE, A.BOOKING_DATE, '0' AS APPROVAL_ID, A.JOB_NO, A.IS_APPROVED, A.PO_BREAK_DOWN_ID,a.BUYER_ID from wo_booking_mst a, wo_booking_dtls b,wo_po_details_master c,approval_mst d where d.mst_id=a.id and a.booking_no=b.booking_no and b.job_no=c.job_no and d.REF_TYPE=1 and a.company_id=$company_name and a.item_category =4 and a.SHORT_BOOKING_AVAILABLE in (0,1) and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.is_approved <> 0  and a.APPROVED_GROUP_BY=d.GROUP_NO and d.GROUP_NO={$electronicDataArr['user_by'][$app_user_id]['GROUP_NO']} and d.entry_form=8  and a.ready_to_approved=1 group by a.id,a.booking_no_prefix_num,a.booking_no,a.entry_form, a.item_category, a.fabric_source, a.company_id, a.booking_type,a.entry_form, a.is_short, a.buyer_id, a.pay_mode, a.supplier_id, a.delivery_date, a.booking_date,a.job_no, a.is_approved, a.po_break_down_id ORDER BY a.booking_date  desc,a.booking_no";
        }
        else  //WithOut Order
        {
            $sql = "select
			A.ID,a.ENTRY_FORM_ID as ENTRY_FORM, A.BOOKING_NO_PREFIX_NUM AS PREFIX_NUM,A.BOOKING_NO,A.ITEM_CATEGORY, A.FABRIC_SOURCE, A.COMPANY_ID, A.BOOKING_TYPE, A.IS_SHORT,A.PAY_MODE, A.SUPPLIER_ID,  A.DELIVERY_DATE, A.BOOKING_DATE, '0' AS APPROVAL_ID,a.JOB_NO, A.IS_APPROVED, A.PO_BREAK_DOWN_ID,a.BUYER_ID from wo_non_ord_samp_booking_mst a,wo_non_ord_samp_booking_dtls b ,approval_mst d where a.booking_no=b.booking_no  and d.REF_TYPE=2 and a.company_id=$company_name and a.item_category =4 and a.booking_type=5  and a.is_deleted=0 and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.is_approved <> 0  and a.APPROVED_GROUP_BY=d.GROUP_NO and d.GROUP_NO={$electronicDataArr['user_by'][$app_user_id]['GROUP_NO']} and d.entry_form=8  and a.ready_to_approved=1 group by a.id,a.booking_no_prefix_num, a.booking_no, a.item_category, a.fabric_source, a.company_id, a.booking_type, a.is_short, a.buyer_id, a.pay_mode, a.supplier_id, a.delivery_date, a.booking_date,a.is_approved, a.po_break_down_id ORDER BY a.booking_date  desc,a.booking_no";
        } 
    ...
    //     $nameArray_buyer=sql_select( "select distinct c.buyer_name,b.booking_no,a.ID from wo_po_details_master c, wo_booking_dtls b,wo_booking_mst a where c.job_no=b.job_no and a.booking_no=b.booking_no and c.status_active=1 and c.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $where_con "); 
    ...
    // }
    // else{
    //     $nameArray_buyer=sql_select( "select  a.buyer_id as buyer_name,a.booking_no  from wo_non_ord_samp_booking_mst a   where a.status_active=1 and a.is_deleted=0 $where_con"); 
  ...
  //print_r($unappv_req_arr);
    ...
    <script>
        function openmypage_app_instrac(wo_id,app_type,i)
        {
            ...
            var page_link = 'requires/trims_booking_approval_group_by_controller.php?data='+data+'&action=appinstra_popup';
        ...
        function openmypage_app_cause(wo_id,app_type,i)
        {
            ...
            var page_link = 'requires/trims_booking_approval_group_by_controller.php?data='+data+'&action=appcause_popup';
            emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=500px,height=250px,center=1,resize=1,scrolling=0','');
            emailwindow.onclose=function()
            {
                var appv_cause=this.contentDoc.getElementById("hidden_appv_cause");
        ...
        function openmypage_unapp_request(wo_id,app_type,i)
        {
            ...
            var page_link = 'requires/trims_booking_approval_group_by_controller.php?data='+data+'&action=unappcause_popup';
                        ...
                        <? 
                            ...
                            foreach ($nameArray as $row)
                            {
								...
								if($row['PAY_MODE']==3 ||  $row['PAY_MODE']==5){
									$supplierName=$company_arr[$row['SUPPLIER_ID']];
								}
								else {
									$supplierName=$supplier_arr[$row['SUPPLIER_ID']];
								}
								?>
								<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" title="<?=$row[csf('entry_form')]?>"> 
                                    ...
                                    <td width="125"><p><?= $buyer_arr[$row['BUYER_ID']]; ?></p></td>
									<td width="160" style="word-break:break-all" title="Paymode_id=<? echo $row[csf('pay_mode')];?>"><? echo $supplierName; ?></td>
									<td align="center"><? if($row['DELIVERY_DATE']!="0000-00-00") echo change_date_format($row['DELIVERY_DATE']); ?></td>
...
if ($action=="approve")
{
	...
	if($cbo_booking_type==1) //With Order
	{
		//$data_mast_sql = "select a.ID,A.BUYER_ID,a.BRAND_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY,a.IS_APPROVED from wo_booking_mst a where a.company_id=$cbo_company_name and a.item_category=4 and a.SHORT_BOOKING_AVAILABLE in (0,1) and a.is_deleted=0 and a.status_active=1 and a.ready_to_approved=1  and a.id in($booking_ids)";

		$data_mast_sql = "select a.ID,A.BUYER_ID,c.BRAND_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY from wo_booking_mst a,WO_BOOKING_DTLS b,wo_po_details_master c where a.booking_no =b.booking_no and b.job_no = c.job_no and a.company_id=$company_name $where_con and a.item_category=4 and a.SHORT_BOOKING_AVAILABLE in (0,1) and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1  and a.ready_to_approved=1 and a.id in($booking_ids) group by a.ID,A.BUYER_ID,c.BRAND_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY";

	}
	else  //WithOut Order
	{
		$data_mast_sql = "select a.ID,A.BUYER_ID,a.BRAND_ID,a.APPROVED_SEQU_BY,a.APPROVED_GROUP_BY,a.IS_APPROVED from wo_non_ord_samp_booking_mst a where a.company_id=$company_name and a.item_category =4 and a.booking_type=5  and a.is_deleted=0 and a.status_active=1 and a.ready_to_approved=1 and a.id in($booking_ids)";
	}
	
	//echo $sql;die; 

    $sqlResult=sql_select( $data_mast_sql );
    foreach ($sqlResult as $row)
    {
		$matchDataArr[$row['ID']]=array('buyer'=>$row['BUYER_ID'],'brand'=>$row['BRAND_ID']*1,'item'=>0,'store'=>0,'department'=>0);
		$last_app_status_arr[$row['ID']] = $row['IS_APPROVED'];
    } 
   
    $finalDataArr=getFinalUser(array('company_id'=>$cbo_company_name,'page_id'=>$menu_id,'ENTRY_FORM'=>8,'lib_buyer_arr'=>$buyer_arr,'lib_brand_arr'=>0,'lib_item_cat_arr'=>0,'lib_store_arr'=>0,'lib_department_id_arr'=>0,'match_data'=>$matchDataArr));
	...
	$max_group_no = max($finalDataArr['user_group']);
	 ...
	 if($cbo_booking_type==1) //With Order
	 {
		$max_approved_no_arr = return_library_array("SELECT mst_id, max(approved_no) as approved_no from approval_history where mst_id in($booking_ids) and entry_form=8 and REF_TYPE=1 group by mst_id","mst_id","approved_no");
	 }
	 else{//WithOut Order
		$max_approved_no_arr = return_library_array("SELECT mst_id, max(approved_no) as approved_no from approval_history where mst_id in($booking_ids) and entry_form=8 and REF_TYPE=2 group by mst_id","mst_id","approved_no");
	 }
	...
	if($cbo_booking_type==1) //With Order
	{
		if($approval_type==0)
		{
			...
			for($i=0;$i<count($booking_ids_all);$i++)
			{
			 ...
			 //echo $approved;die;
				...
				if($last_app_status_arr[$mst_id] == 0 || $last_app_status_arr[$mst_id] == 2){
					$approved_no = $max_approved_no_arr[$mst_id]+1;
					$approved_no_array[$booking_nos_all[$i]] = $approved_no;
				}

				if($data_array!=''){$data_array.=",";}
				$data_array.="(".$id.",8,1,'".$mst_id."','".$user_sequence_no."','".$user_group_no."',".$app_user_id.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."',".$approved.")"; 
				$id=$id+1;
		
				if($history_data_array!="") $history_data_array.=",";
				$history_data_array.="(".$ahid.",8,1,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,".$app_user_id.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$approved.")";
				$ahid++;
				
				//Mst data.......................
				$data_array_up[$mst_id] = explode(",",("".$approved.",".$user_sequence_no.",".$user_group_no.",'".$pc_date_time."'"));
			}	

			$flag=1;
			if($flag==1) 
			{  
				$field_array="id, entry_form,REF_TYPE, mst_id,  sequence_no,group_no,approved_by, approved_date,INSERTED_BY,INSERT_DATE,user_ip,APPROVED";
			...
			if($flag==1) 
			{
				$history_field_array="id, entry_form,REF_TYPE, mst_id, approved_no, sequence_no, current_approval_status, approved_by, approved_date, inserted_by, insert_date,APPROVED"; 
				$rID2=sql_insert("approval_history",$history_field_array,$history_data_array,0);
				if($rID2) $flag=1; else $flag=0; 
			}

			if($flag==1) 
			{
				$field_array_up="IS_APPROVED*APPROVED_SEQU_BY*APPROVED_GROUP_BY*APPROVED_DATE"; 
				//echo bulk_update_sql_statement( "WO_BOOKING_MST", "id", $field_array_up, $data_array_up, $target_app_id_arr );die;
				$rID3=execute_query(bulk_update_sql_statement( "WO_BOOKING_MST", "id", $field_array_up, $data_array_up, $booking_ids_all ));
			...
			if(count($approved_no_array)>0)
			{
				...
				if($flag==1) 
				{
					$rID6=execute_query($sql_insert_cons_dtls,1);
			...
			for($i=0;$i<count($booking_ids_all);$i++)
			{
				...
				if($history_data_array!="") $history_data_array.=",";
				$history_data_array.="(".$ahid.",8,1,".$mst_id.",".$approved_no.",'".$user_sequence_no."',0,".$app_user_id.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',2,'".$app_instru."')";
				$ahid++;
			}	
			...
			if($flag==1) 
			{
				...
				$history_field_array="id, entry_form,REF_TYPE, mst_id, approved_no, sequence_no, current_approval_status, approved_by, approved_date, inserted_by, insert_date,APPROVED,COMMENTS"; 
				$rID1=sql_insert("approval_history",$history_field_array,$history_data_array,0);
				if($rID1) $flag=1; else $flag=0; 
			}
			
			if($flag==1) 
			{
				$rID2=sql_multirow_update("wo_booking_mst","IS_APPROVED*READY_TO_APPROVED*APPROVED_SEQU_BY*APPROVED_GROUP_BY",'0*2*0*0',"id",$booking_ids,0);
				if($rID2) $flag=1; else $flag=0;
			}

			if($flag==1) 
			{
				$query="delete from approval_mst  WHERE entry_form=8 and REF_TYPE=1 and mst_id in ($booking_ids)";
			...
			if($flag==1) 
			{
				$query="update approval_history set current_approval_status=0  WHERE entry_form=8 and REF_TYPE=1 and mst_id in ($booking_ids)";
		...
		else
		{
			...
			for($i=0;$i<count($booking_ids_all);$i++)
			{
				...
				if($history_data_array!="") $history_data_array.=",";
				$history_data_array.="(".$ahid.",8,1,".$mst_id.",".$approved_no.",'".$user_sequence_no."',0,".$app_user_id.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0,'".$app_instru."')";
				$ahid++;
			...
			if($flag==1) 
			{
				$rID=sql_multirow_update("wo_booking_mst","IS_APPROVED*READY_TO_APPROVED*APPROVED_SEQU_BY*APPROVED_GROUP_BY",'0*0*0*0',"id",$booking_ids,0);
	...
	else //WithOut Order
	{
		if($approval_type==0)
		{
			...
			for($i=0;$i<count($booking_ids_all);$i++)
			{
				...
				if($data_array!=''){$data_array.=",";}
				$data_array.="(".$id.",8,2,'".$mst_id."','".$user_sequence_no."','".$user_group_no."',".$app_user_id.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."','".$user_ip."')"; 
				$id=$id+1;
		
				if($history_data_array!="") $history_data_array.=",";
				$history_data_array.="(".$ahid.",8,2,".$mst_id.",".$approved_no.",'".$user_sequence_no."',1,'',".$app_user_id.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',".$approved.")";
				$ahid++;
			...
			//  $history_field_array="id, entry_form, REF_TYPE, mst_id, approved_no, sequence_no, current_approval_status,comments, approved_by, approved_date, inserted_by, insert_date,APPROVED"; 
			//  echo "insert into approval_history $history_field_array values($history_data_array)";die;

			$flag=1;
			if($flag==1) 
			{  
				$field_array="id, entry_form,REF_TYPE, mst_id,  sequence_no,group_no,approved_by, approved_date,INSERTED_BY,INSERT_DATE,user_ip";
				$rID1=sql_insert("approval_mst",$field_array,$data_array,0);
				if($rID1) $flag=1; else $flag=0; 
			}
			...
			{
				$field_array_up="IS_APPROVED*APPROVED_SEQU_BY*APPROVED_GROUP_BY*APPROVED_DATE"; 
				 //echo bulk_update_sql_statement( "wo_non_ord_samp_booking_mst", "id", $field_array_up, $data_array_up, $booking_ids_all );die;
				$rID3=execute_query(bulk_update_sql_statement( "wo_non_ord_samp_booking_mst", "id", $field_array_up, $data_array_up, $booking_ids_all ));
				if($rID3) $flag=1; else $flag=0; 
			}
			...
			if(count($approved_no_array)>0)
			{
				...
				if($flag==1) 
				{
					//echo $sql_insert;oci_rollback($con);die;
					$rID4=execute_query($sql_insert,0);
					if($rID4) $flag=1; else $flag=0; 
				} 
				//echo $flag;die;
			...
			for($i=0;$i<count($booking_ids_all);$i++)
			{
				...
				if($history_data_array!="") $history_data_array.=",";
				$history_data_array.="(".$ahid.",8,2,".$mst_id.",".$approved_no.",'".$user_sequence_no."',0,".$app_user_id.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',2,'".$app_instru."')";
				$ahid++;
			}	
			...
			if($flag==1) 
			{
				$rID=sql_multirow_update("wo_non_ord_samp_booking_mst","IS_APPROVED*READY_TO_APPROVED*APPROVED_SEQU_BY*APPROVED_GROUP_BY",'0*2*0*0',"id",$booking_ids,0);
				if($rID) $flag=1; else $flag=0;
			}

			if($flag==1) 
			{
				$query="delete from approval_mst  WHERE entry_form=8 AND REF_TYPE=2 and mst_id in ($booking_ids)";
				$rID3=execute_query($query,1); 
				if($rID3) $flag=1; else $flag=0; 
			}
			...
			if($flag==1) 
			{
				$query="update approval_history set current_approval_status=0  WHERE entry_form=8 AND REF_TYPE=2 and mst_id in ($booking_ids)";
				$rID4=execute_query($query,1); 
				if($rID4) $flag=1; else $flag=0; 
			}
		...
		else
		{
			...
			for($i=0;$i<count($booking_ids_all);$i++)
			{
				...
				if($history_data_array!="") $history_data_array.=",";
				$history_data_array.="(".$ahid.",8,2,".$mst_id.",".$approved_no.",'".$user_sequence_no."',0,".$app_user_id.",'".$pc_date_time."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',0,'".$app_instru."')";
				$ahid++;
			...
			if($flag==1) 
			{
				$rID2=sql_multirow_update("wo_non_ord_samp_booking_mst","IS_APPROVED*READY_TO_APPROVED*APPROVED_SEQU_BY*APPROVED_GROUP_BY",'0*0*0*0',"id",$booking_ids,0);
				if($rID2) $flag=1; else $flag=0;
			}
    ...
    <body>
        ...
        <form name="size_1" id="size_1">
            ...
            <table align="center" cellspacing="0" width="450" class="rpt_table" border="1" rules="all" id="" >
                <tr>
                    <td align="center">
					<input type="hidden" name="hidden_appv_cause" id="hidden_appv_cause">
