...
$approval_setup=is_duplicate_field( "entry_form", "electronic_approval_setup", "entry_form=94 and is_deleted=0" );
//------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Export Pro Forma Invoice Approval Group By", "../", 1, 1,'','','');
...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate&previous_approved="+previous_approved+get_submitted_data_string('cbo_company_name*cbo_approval_type*txt_alter_user_id*txt_pi_no*txt_job_no*txt_booking_no*txt_sys_no*txt_date',"../");

		// alert(data);return;
		http.open("POST","requires/export_pro_forma_invoice_approval_group_by_controller.php",true);
	...
	function downloiadFile(id,company_name)
	{
		...
		var page_link = 'requires/export_pro_forma_invoice_approval_group_by_controller.php?action=get_user_pi_file&id='+id+'&company_name='+company_name;
	...
	function change_user()
	{
		...
		var page_link = 'requires/export_pro_forma_invoice_approval_group_by_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			/* load_drop_down( 'requires/export_pro_forma_invoice_approval_group_by_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' ); */
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td>
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0  $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/export_pro_forma_invoice_approval_group_by_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );" );
                                    ?>
                                </td>
								<td><input name="txt_pi_no" id="txt_pi_no" class="text_boxes" style="width:100px"></td>
                                ...
                                <td><input name="txt_sys_no" id="txt_sys_no" class="text_boxes" style="width:100px"></td>

                                <td><input type="text" name="txt_date" id="txt_date" class="datepicker" style="width:100px"/></td>
