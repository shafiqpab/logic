...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Short Quotation [Sweater]", "../", 1, 1,'','','');
...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate&previous_approved="+previous_approved+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_brand*cbo_season_id*cbo_season_year*txt_costshit_no*txt_style_ref*cbo_get_upto*txt_date*cbo_approval_type*cbo_year*txt_alter_user_id',"../");
		
		http.open("POST","requires/price_quatation_approval_sweater_controller.php",true);
	...
	{ 
		...
		var data="action=approve&operation="+operation+'&approval_type='+type+'&booking_nos='+booking_nos+'&booking_ids='+booking_ids+'&approval_ids='+approval_ids+'&confirm_ids='+confirm_ids+get_submitted_data_string('txt_alter_user_id*cbo_company_name',"../");
	   //alert(data);
	...
	{
		var page_link='requires/price_quatation_approval_sweater_controller.php?action='+action+'&id='+id;
	...
	function fnc_load_cm_compulsory(data)
	{
		...
		var cm_compulsory = return_global_ajax_value( data, 'populate_cm_compulsory', '', 'requires/price_quatation_approval_sweater_controller');
	...
	function change_user()
	{
		...
		var page_link = 'requires/price_quatation_approval_sweater_controller.php?action=user_popup&cbo_company_name='+cbo_company_name;
	...
	{ 
		...
		if(entry_form==511)
		{
			var report_title="Short Quotation [Sweater]";
			var data=qc_no+'*'+cost_sheet_no+'*'+report_title;
			window.open("../order/spot_costing/requires/short_quotation_v4_controller.php?data=" + data+'&action='+action, true );
			return;
		}
	...
	function fnc_confirm_style(qc_no,update_id)
	{
		...
		var page_link='requires/price_quatation_approval_sweater_controller.php?action=confirmStyle_popup';
	...
	// 	var mail=mail.split(',');
	// 	var ret_data=return_global_ajax_value(sysIdArr.join('*')+'__'+mail.join('*'), 'app_deny_mail', '', '../approval/requires/price_quatation_approval_sweater_controller');
	// 	//alert(ret_data);
	// }
...
<body>
	...
	<? echo load_freeze_divs ("../",''); ?>
         ...
         <h3 style="width:1100px;margin-top:10px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3> 
			 ...
			 <table class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
								<td align="center"> 
								<?
                                    echo create_drop_down( "cbo_company_name", 140, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/price_quatation_approval_sweater_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                                ?>
                                </td>

                                <td id="buyer_td"><?=create_drop_down("cbo_buyer_name", 130, $blank_array,"",1, "--All--", $selected,""); ?>
								</td>
                                <td id="brand_td"><?=create_drop_down("cbo_brand", 80, $blank_array,"",1, "--All--", $selected,""); ?></td>
                                <td id="season_td"><?=create_drop_down("cbo_season_id", 80, $blank_array,'', 1, "--All--",$selected, "" ); ?></td>
