...
<script>
	...
	function check_on_scan(scan_no)
	{
		...
		{
			var salse_order_approved=return_global_ajax_value( trim(scan_no), 'check_sales_order_approved', '', 'requires/non_order_sample_booking_approval_controller');
			if(salse_order_approved==1 || salse_order_approved==3)
			{
				alert("Corresponding Sales Order is approved.So Booking Unapproved Not Allow.");
	...
	{ 
		...
		var data="action=approve&operation="+operation+'&approval_type='+type+'&booking_nos='+booking_nos+'&booking_ids='+booking_ids+'&txt_alter_user_id='+alterUserID+'&approval_ids='+approval_ids+get_submitted_data_string('cbo_company_name',"../");
	...
	function generate_worder_report(booking_no,company_id,is_approved, entry_form_id,Print_id,type,i)
	{
		var report_title="";

		if(Print_id==34 || Print_id==35  || Print_id==36  || Print_id==37 || Print_id==64 || Print_id==72  || Print_id==174) report_title='Sample Fabric Booking -Without order'; else report_title='Sample Requisition Fabric Booking -Without order';
		...
		else if(entry_form_id=='' && (Print_id==34 || Print_id==35  || Print_id==36  || Print_id==37 || Print_id==64 || Print_id==72  || Print_id==174))
		{
	...
	{
		 var requisition_id = trim(return_global_ajax_value(booking_id, 'get_requisition_no_from_booking', '', 'requires/non_order_sample_booking_approval_controller'));
	...
	function change_user()
	{
		...
		var page_link = 'requires/non_order_sample_booking_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/non_order_sample_booking_approval_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/non_order_sample_booking_approval_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );" );
                                    ?>
                                </td>
								<td>
								<?
