<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Price Quotation Approval Single Layer Approval
Functionality	:	
JS Functions	:
Created by		:	Abdul Kaiyum
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Price Quotation Approval Single Layer", "../", 1, 1,'','','');
...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_get_upto*txt_date*cbo_approval_type*txt_quotation_no',"../");
		
		http.open("POST","requires/price_quotation_approval_single_layer_controller.php",true);
	...
	{ 
		...
		freeze_window(0);
        if (permission ==2) {            
            alert('You Have No Authority To Price Quotation Approval Single Layer'); 
			release_freezing();
            return false;	    
        }
		// Confirm Message  *********************************************************************************************************
	...
	{
		var page_link='requires/price_quotation_approval_single_layer_controller.php?action='+action+'&id='+id;
	...
	var cm_compulsory = return_global_ajax_value( data, 'populate_cm_compulsory', '', 'requires/price_quotation_approval_single_layer_controller');
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/price_quotation_approval_single_layer_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );fnc_load_cm_compulsory(this.value);" );
                                    ?>
                                </td>
                                <td id="buyer_td_id"> 
									<?
								...
								<td><input type="text"  name="txt_quotation_no" id="txt_quotation_no"  class="text_boxes" /></td>
                                <td> 
									<?
                                ...
                                <td><input type="text" name="txt_date" id="txt_date" class="datepicker" readonly style="width:80px"/>
