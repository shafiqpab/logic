...
<script>
	...
	{   
		...
		//alert(data);
		freeze_window(3);
		http.open("POST","requires/knitting_work_order_approval_controller.php",true);
	...
	let openImgFile=(id,action)=>{
		var page_link='requires/knitting_work_order_approval_controller.php?action='+action+'&id='+id;
...
function fn_generate_print_res(){
	...
	function change_user()
	{
		...
		var page_link = 'requires/knitting_work_order_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_id',"../");
	...
	function openmypage_refusing_cause(page_link,title,quo_id)
	{
	 ...
	 function call_print_button_for_mail(mail,mail_body,type){
		 ...
		 var mail=mail.split(',');
		 var ret_data=return_global_ajax_value(sysIdArr.join(',')+'__'+mail.join(',')+'__'+txt_alter_user_id+'__'+cbo_company_id+'__'+type, 'app_mail_notification', '', 'requires/knitting_work_order_approval_controller');
		 //alert(ret_data);
	 }
                 ...
                 <table class="rpt_table" width="950px%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            <td> 
                                <?
                                    echo create_drop_down( "cbo_company_id", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/knitting_work_order_approval_controller',this.value, 'load_supplier_dropdown', 'supplier_td_id' );" );
                                ?>
                            </td>
