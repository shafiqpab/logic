...
<script>
	...
	{   
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_buyer_id*txt_wo_no*txt_date_from*txt_date_to*cbo_approval_type*txt_alter_user_id',"../");
		//alert(data);
		freeze_window(3);
		http.open("POST","requires/dyeing_work_order_approval_controller.php",true);
	...
	{  //alert(total_tr);
		...
		var data="action=approve&operation="+operation+"&target_ids="+target_ids+'&cbo_approval_type='+type+get_submitted_data_string('cbo_company_id*cbo_buyer_id*txt_wo_no*txt_date_from*txt_date_to*txt_alter_user_id',"../");
		//alert(data);return;
		freeze_window(0);
		http.open("POST","requires/dyeing_work_order_approval_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	...
	let openImgFile=(id,action)=>{
		var page_link='requires/dyeing_work_order_approval_controller.php?action='+action+'&id='+id;
	...
	function fn_generate_print(update_id,cbo_company_name){
	   ...
	   var all_data=update_id+'**'+cbo_company_name+'**1'
		...
		http.open("POST", "../subcontract_bill/wo/requires/dyeing_work_order_controller.php", true);
		http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		http.send(data);
...
function fn_generate_print_res(){
	...
	function change_user()
	{
		...
		var page_link = 'requires/dyeing_work_order_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_id',"../");
