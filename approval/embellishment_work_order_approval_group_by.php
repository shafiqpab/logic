...
$approval_setup=is_duplicate_field( "entry_form", "electronic_approval_setup", "entry_form=32 and is_deleted=0" );
...
<script>
	...
	function fn_report_generated()
	{
		...
		http.open("POST","requires/embellishment_work_order_approval_group_by_controller.php",true);
		...
		{ 
			$("#tbl_list_search tbody").find('tr').each(function()
			{
				try 
				{
					...
					{
						var last_update=return_global_ajax_value( trim(booking_no), 'check_booking_last_update', '', 'requires/embellishment_work_order_approval_group_by_controller');
						if(last_update==2)
						{
	...
	function check_on_scan(scan_no)
	{
		...
		if(!(hide_approval_type==1))
		{
			var last_update=return_global_ajax_value( trim(scan_no), 'check_booking_last_update', '', 'requires/embellishment_work_order_approval_group_by_controller');
			if(last_update==2)
			{
	...
	{ 
		...
		var data="action=approve&operation="+operation+'&approval_type='+type+'&booking_nos='+booking_nos+'&booking_ids='+booking_ids+'&appv_causes='+appv_causes+'&approval_ids='+approval_ids+'&appv_instras='+appv_instras+get_submitted_data_string('cbo_company_name*txt_alter_user_id',"../");
	...
	function generate_worder_report4(job_no,action)
	{
		...
		var job_data=return_global_ajax_value( trim(job_no), 'pre_cost_data', '', 'requires/embellishment_work_order_approval_group_by_controller');
	...
	{
		var page_link='requires/embellishment_work_order_approval_group_by_controller.php?action='+action+'&job_no='+job_no;
	...
	{
		//alert(action);return;
		var page_link='requires/embellishment_work_order_approval_group_by_controller.php?action='+action+'&job_no='+job_no+'&booking_no='+booking_no+'&order_id='+order_id+'&fab_nature='+fab_nature+'&fab_source='+fab_source;
	...
	function change_user()
	{
		...
		var page_link = 'requires/embellishment_work_order_approval_group_by_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/embellishment_work_order_approval_group_by_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/embellishment_work_order_approval_group_by_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );" );
                                    ?>
                                </td>
                                <td id="buyer_td_id"> 
									<?
