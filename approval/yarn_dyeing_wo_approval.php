...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Yarn Dyeing Work With/Without Order Approval", "../", 1, 1,'','','');
...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate&previous_approved="+previous_approved+get_submitted_data_string('cbo_company_name*cbo_get_upto*txt_date*cbo_approval_type*txt_booking_no*cbo_booking_year*txt_alter_user_id',"../");
		
		http.open("POST","requires/yarn_dyeing_wo_approval_controller.php",true);
	...
	function change_user()
	{
		...
		var title = 'Yarn Dyeing Work Order Approval - Alter user';
		var page_link = 'requires/yarn_dyeing_wo_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
	...
	function generate_worder_report(wo_no,company_id,update_id,budget_version_id,action,entry_form=""){
		var approval_setup =<? echo $approval_setup; ?>;
		...
		var data="action="+action+"&form_name="+form_name+"&wo_no="+wo_no+"&cbo_company_name="+company_id+"&update_id="+update_id+"&show_comment="+show_comment+'&path=../../';

		//get_submitted_data_string('wo_no*company_id*button_id',"../");
		//alert(budget_version_id);
		freeze_window(3);
		...
		{
			if(budget_version_id==2)
			{
				http.open("POST","../order/woven_order/requires/yarn_dyeing_charge_booking_controller2.php",true);
			}else{
				if(entry_form == 42){
					http.open("POST","../order/woven_order/requires/yarn_dyeing_wo_without_order_controller.php?entry_form="+entry_form,true);
				}else if(entry_form == 114){
					http.open("POST","../order/woven_order/requires/yarn_dyeing_booking_without_lot_controller.php",true);
				}
			}
		...
		//http.open("POST","order/woven_order/yarn_dyeing_charge_booking.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		//alert(data);
		http.send(data);
...
<body>
	...
	<? echo load_freeze_divs ("../",''); ?>
         ...
         <h3 style="width:803px;margin-top:10px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3>
