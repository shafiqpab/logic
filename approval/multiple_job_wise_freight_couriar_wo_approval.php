...
$approval_setup=is_duplicate_field( "entry_form", "electronic_approval_setup", "entry_form=84 and is_deleted=0" );
...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_approval_type*txt_wo_no*txt_date*txt_alter_user_id',"../");
		  
		http.open("POST","requires/multiple_job_wise_freight_couriar_wo_approval_controller.php",true);
...
function generate_order_report(booking_no,company_id,is_approved, entry_form_id,report_id,fab_booking,action)
	{
		...
		if(report_id==163 ||  report_id==164  || report_id==177 || report_id==16 || report_id==176 || report_id==288){	
		    show_comment="1";	
			var data="action="+action+'&txt_booking_no='+"'"+booking_no+"'"+'&cbo_company_name='+"'"+company_id+"'"+'&txt_fab_booking='+"'"+fab_booking+"'";+'&show_comment='+show_comment+'&path=../../';		
...
function generate_fabric_report_reponse()
	...
	function change_user()
	{
		...
		var page_link = 'requires/multiple_job_wise_freight_couriar_wo_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            <td> 
                                <?
                                    echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/multiple_job_wise_freight_couriar_wo_approval_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );" );
                                ?>
                            </td>
                            <td id="buyer_td_id"> 
                                <?
                            ...
                            <td>
                            	<input type="text" name="txt_wo_no" id="txt_wo_no" value="" class="text_boxes" style="width:100px" /> 
                            </td> 
							<td> <? echo create_drop_down( "cbo_wo_year", 140, $year,"", 1, "-- Select --", "", "" ); ?></td>
