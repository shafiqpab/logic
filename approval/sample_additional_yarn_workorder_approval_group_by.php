<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Yern Work Woder Approval Group By
Functionality	:
JS Functions	:
...
$approval_setup=is_duplicate_field( "page_id", "electronic_approval_setup", "entry_form=53 and is_deleted=0" );
//------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Sample Or Additional Yarn WO Approval Group By", "../", 1, 1,'','','');
...
<script>
	...
	function fn_report_generated()
	{
		...
		//alert(data);return;		
		http.open("POST","requires/sample_additional_yarn_workorder_approval_group_by_controller.php",true);
	...
	function submit_approved(total_tr,type)
	{
		...
		for(i=1; i<total_tr; i++)
		{
			if ($('#tbl_'+i).is(":checked"))
			{
                ...
                req_id_arr.push(req_id);
			}
		}
        var req_ids = req_id_arr.join(',');
		var alteruserid = $('#txt_alter_user_id').val();
		var cbo_company_name = $('#cbo_company_name').val();
		var data="action=approve&operation="+operation+'&txt_alter_user_id='+alteruserid+'&approval_type='+type+'&req_ids='+req_ids+'&cbo_company_name='+cbo_company_name; //alert(data);
	...
	function change_user()
	{
		...
		var page_link = 'requires/sample_additional_yarn_workorder_approval_group_by_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/sample_additional_yarn_workorder_approval_group_by_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
                ...
                <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            <td>
                                <?
                                    echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0  $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/sample_additional_yarn_workorder_approval_group_by_controller',this.value, 'load_supplier_dropdown', 'supplier_td_id' );" );
                                ?>
                            </td>
