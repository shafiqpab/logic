...
Purpose			: 	This form will create Item Issue Requisition Approval Group By
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Item Issue Requisition Approval Group By", "../", 1, 1,'','','');
...
<script>
	...
	function fn_report_generated()
	{
		...
		http.open("POST","requires/item_issue_requisition_approval_group_by_controller.php",true);
	...
	{ 
		...
		// Confirm Message End *******************************************************************
		var requisition_id_arr = Array();var approval_id_arr = Array();var comment_arr = Array();
		for(i=1; i<total_tr; i++)
		{
			if ($('#tbl_'+i).is(":checked"))
			{								
                var target_id = parseInt($('#requisition_id_'+i).val());
                requisition_id_arr.push(target_id);
                var approval_id = parseInt($('#approval_id_'+i).val())*1;
                approval_id_arr.push(approval_id);

                var comment = parseInt($('#comments_'+target_id).val());
                comment_arr.push(comment);
				
			}
		}
        var requisition_ids = requisition_id_arr.join(',');
        var approval_ids = approval_id_arr.join(',');
        var comments = comment_arr.join('**');
		...
		var data="action=approve&operation="+operation+'&approval_type='+type+'&requisition_ids='+requisition_ids+'&approval_ids='+approval_ids+'&comments='+comments+get_submitted_data_string
	...
	function change_user()
	{
		...
		var title = 'User Info';	
		var page_link = 'requires/item_issue_requisition_approval_group_by_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
	...
	function call_print_button_for_mail(mail,mail_body,type){
		 ...
		 var mail=mail.split(',');
		 var ret_data=return_global_ajax_value(sysIdArr.join(',')+'__'+mail.join(',')+'__'+txt_alter_user_id+'__'+cbo_company_name+'__'+type, 'app_mail_notification', '', 'requires/item_issue_requisition_approval_group_by_controller');
	 }
</script>
</head>
<body>
