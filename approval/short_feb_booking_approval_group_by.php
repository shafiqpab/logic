<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Short Fabric Booking Approval V2
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Short Fabric Booking Approval Group By", "../", 1, 1,'','','');
$approval_setup = is_duplicate_field( "page_id", "electronic_approval_setup", "entry_form=12 and is_deleted=0" );
?>	
<script>
	...
	function fn_report_generated(type)
	{
		...
		var data="action=report_generate&previous_approved="+previous_approved+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_internal_ref*txt_file_no*txt_date_from*txt_date_to*cbo_approval_type*txt_booking_no*cbo_year_selection*txt_alter_user_id',"../");
	
		http.open("POST","requires/short_feb_booking_approval_group_by_controller.php",true);
		...
		{ 
			$("#tbl_list_search tbody").find('tr').each(function()
			{
				if(i>1)
				{
					...
					var last_update=return_global_ajax_value( trim(booking_no), 'check_booking_last_update', '', 'requires/short_feb_booking_approval_group_by_controller');
					...
					if(approval_type==1)
					{
						var salse_order_approved=return_global_ajax_value( trim(booking_no), 'check_sales_order_approved', '', 'requires/short_feb_booking_approval_group_by_controller');
	...
	function check_on_scan(scan_no)
	{
		...
		if(approval_type==1)
		{
			var salse_order_approved=return_global_ajax_value( trim(scan_no), 'check_sales_order_approved', '', 'requires/short_feb_booking_approval_group_by_controller');
			if(salse_order_approved==1 || salse_order_approved==3)
			{
				alert("Corresponding Sales Order is Approved.So Booking Unapproved Not Allow.");
				$('#tbl_'+row_no).attr('checked', false);
	...
	{
		var page_link='requires/short_feb_booking_approval_group_by_controller.php?action='+action+'&job_no='+job_no;
	...
	function change_user()
	{
		...
		var page_link = 'requires/short_feb_booking_approval_group_by_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		  
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=755px,height=390px,center=1,resize=1,scrolling=0','');
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/short_feb_booking_approval_group_by_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
	...
	function call_print_button_for_mail(mail,mail_body,type){
		 ...
		 var mail=mail.split(',');
		 var ret_data=return_global_ajax_value(sysIdArr.join(',')+'__'+mail.join(',')+'__'+txt_alter_user_id+'__'+cbo_company_name+'__'+type, 'app_mail_notification', '', 'requires/short_feb_booking_approval_group_by_controller');
		 //alert(ret_data);
	 }
         ...
         <div id="content_search_panel">      
             <fieldset style="width:1000px;">
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        <thead>
                        	<tr> 
                            	<th colspan="3"><input type="hidden" id="txt_bar_code" name="txt_bar_code" class="text_boxes"  /></th>
                            ...
                            <tr>
                                ...
                                <th>Approval Type</th>
                                <th><input type="reset" name="res" id="res" value="Reset" onClick="reset_form('requisitionApproval_1','report_container','','','')" class="formbutton" style="width:90px" /></th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/short_feb_booking_approval_group_by_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );" );
                                    ?>
                                </td>
                                <td id="buyer_td_id"> 
                                ...
                                <td>
                                	<input type="button" value="Show" name="show" id="show" class="formbutton" style="width:90px;" onClick="fn_report_generated(1);" />
