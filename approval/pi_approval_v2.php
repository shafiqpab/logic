<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create PI Approval V2
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("PI Approval V2", "../", 1, 1,'','','');
?>	
<script>
	...
	let fn_report_generated=(type)=>{
		...
		var data="action=report_generate&type="+type+get_submitted_data_string('cbo_company_name*cbo_supplier_id*cbo_get_upto*txt_date*cbo_approval_type*txt_alter_user_id*txt_pi_no*txt_pi_sys_id_no*cbo_buyer_name*cbo_brand_id*cbo_season_name*cbo_season_year*txt_style_ref',"../");
		//alert(data);
		
		http.open("POST","requires/pi_approval_v2_controller.php",true);
	...
	function check_on_scan(scan_no)
	{
		...
		if(!(hide_approval_type==1))
		{
			var last_update=return_global_ajax_value( trim(scan_no), 'check_booking_last_update', '', 'requires/pi_approval_v2_controller');
			if(last_update==2)
			{
	...
	function check_last_update(rowNo)
	{
		...
		{
			var last_update=return_global_ajax_value( trim(booking_id), 'check_booking_last_update', '', 'requires/pi_approval_v2_controller');
			if(last_update==2)
			{
				alert("Booking ("+trim(booking_no)+") Info not synchronized with order entry and pre-costing. Contact To "+dealing_merchant+".");
	...
	function change_user()
	{
		...
		var page_link = 'requires/pi_approval_v2_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			//load_drop_down( 'requires/pi_approval_v2_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
	...
	function downloiadFile(id,company_name)
	{
		...
		var page_link = 'requires/pi_approval_v2_controller.php?action=get_user_pi_file&id='+id+'&company_name='+company_name;
	...
	function fnc_pi_cross_check(pi_id,company_id) {
		...
		var page_link = 'requires/pi_approval_v2_controller.php?item_category=1&importer_id='+company_id+'&pi_id='+pi_id+'&action=cross_check_popup';
	...
	function fnc_file_popup(pi_id,company_id) {
		...
		var page_link = 'requires/pi_approval_v2_controller.php?company_id='+company_id+'&pi_id='+pi_id+'&action=file_no_popup';
	...
	function openmypage_app_cause(wo_id,app_type,i,app_from,user_id)
	{
		...
		var page_link = 'requires/pi_approval_v2_controller.php?data='+data+'&action=appcause_popup';
	...
	function openPopup(pi_id)
	{
		...
		var page_link = 'requires/pi_approval_v2_controller.php?action=all_job_by_pi_popup&pi_id='+pi_id+'&company_name='+$('#cbo_company_name').val();
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=500px,height=250px,center=1,resize=1,scrolling=0','');
		emailwindow.onclose=function()
		{
		}
	...
	function call_print_button_for_mail(mail,mail_body,type)
	{
		...
		var mail = mail.split(',');
		var ret_data = return_global_ajax_value(sysIdArr.join('*')+'__'+mail.join('*')+'__'+type+'__'+$('#txt_alter_user_id').val()+'__'+mail_body+'__'+cause_arr+'__'+alterUserID, 'pi_approval_mail', '', 'requires/pi_approval_v2_controller');
		comment_empty();
		//alert(22);
		//var cause_arr=Array();
	...
	{
		var data=return_global_ajax_value( job_no, 'get_print_button_data', '', 'requires/pi_approval_v2_controller');
	...
	function print_button_setting(company)
	{
		...
		// alert(company);
		get_php_form_data($('#cbo_company_name').val(),'print_button_variable_setting','requires/pi_approval_v2_controller' );
	}
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 100, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/pi_approval_v2_controller',this.value, 'load_supplier_dropdown', 'supplier_td_id' );load_drop_down( 'requires/pi_approval_v2_controller',this.value, 'load_dropdown_buyer', 'buyer_td' );print_button_setting();" );
                                    ?>
                                </td>
