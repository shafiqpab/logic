...
//------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Yarn Test Approval", "../", 1, 1,'','','');
...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_supplier_id*cbo_get_upto*txt_date*txt_lot_no*cbo_yarn_count*cbo_yarn_type*txt_product_id*cbo_approval_type*txt_alter_user_id',"../");
		// alert(data);return;
		http.open("POST","requires/yarn_test_approval_controller.php",true);
	...
	function submit_approved(total_tr,type)
	{
		...
		var yarnTestIdArr=Array();var approvalIdArr=Array();
		for(i=1; i<total_tr; i++)
		{
			if ($('#tbl_'+i).is(":checked"))
			{
				yarnTestIdArr.push($('#yarn_test_id_'+i).val());
				approvalIdArr.push($('#approval_id_'+i).val());
			}
		}
		var yarn_test_id=yarnTestIdArr.join(',');
		var approved_id=approvalIdArr.join(',');

		
		var alterUserID = $('#txt_alter_user_id').val();
		var cbo_company_name = $('#cbo_company_name').val();
		var data="action=approve&operation="+operation+'&txt_alter_user_id='+alterUserID+'&approval_type='+type+'&yarn_test_id='+yarn_test_id+'&approved_id='+approved_id+'&cbo_company_name='+cbo_company_name;
	...
	function change_user()
	{
		...
		var page_link = 'requires/yarn_test_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/yarn_test_approval_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td>
                                    <?
                                        echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0  $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/yarn_test_approval_controller',this.value, 'load_supplier_dropdown', 'supplier_td_id' );" );
                                    ?>
                                </td>
                                <td id="supplier_td_id"> 
									<?
                                       echo create_drop_down( "cbo_supplier_id", 120, $blank_array,"", 1, "-- All Supplier --", 0, "" );
                                    ?>
                                ...
                                <td><input type="text" name="txt_lot_no"  id="txt_lot_no"  style="width:100px" class="text_boxes"  /></td>
                                <td>
                                	<?
                                ...
                                <td><input type="text" name="txt_product_id"  id="txt_product_id"  style="width:100px" class="text_boxes"  /></td>
                                <td>
                                    <?
