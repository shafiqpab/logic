...
<script>
	...
	{   
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_supplier_id*txt_work_order*txt_date_from*txt_date_to*cbo_approval_type*txt_alter_user_id',"../");
		
		freeze_window(3);
		http.open("POST","requires/yarn_dyeing_sales_approval_controller.php",true);
	...
	{ 
		...
		var data="action=approve&operation="+operation+"&target_ids="+target_ids+'&cbo_approval_type='+type+get_submitted_data_string('cbo_company_id*cbo_supplier_id*txt_work_order*txt_date_from*txt_date_to*txt_alter_user_id',"../");
		//alert(data);return;
		freeze_window(0);
		http.open("POST","requires/yarn_service_work_order_approval_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
		http.onreadystatechange=fnc_purchase_requisition_approval_Reply_info;
	}	
	let openImgFile=(id,action)=>{
		var page_link='requires/yarn_dyeing_sales_approval_controller.php?action='+action+'&id='+id;
	...
	function fn_generate_print(update_id,cbo_company_name,txt_job_no,txt_job_id,txt_booking_no,txt_within_group){
	//    alert(update_id);
	   var form_name="yarn_dyeing_wo_sales_order";
	   var show_comment = "1";
	  
		var data = "action=generate_report&form_name="+form_name+'&cbo_company_name='+cbo_company_name+'&txt_booking_no='+txt_booking_no+'&txt_job_no='+txt_job_no+'&txt_job_id='+txt_job_id+'&update_id='+update_id+'&txt_within_group='+txt_within_group;
		freeze_window(operation);
		http.open("POST", "../order/woven_order/requires/yarn_dyeing_charge_booking_sales_controller.php", true);
		http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		http.send(data);
...
function fn_generate_print_res(){
	...
	function change_user()
	{
		...
		var page_link = 'requires/yarn_dyeing_sales_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_id',"../");
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            <td> 
                                <?
                                    echo create_drop_down( "cbo_company_id", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/yarn_dyeing_sales_approval_controller',this.value, 'load_supplier_dropdown', 'buyer_td_id' );" );
                                ?>
                            </td>
                            <td id="buyer_td_id"> 
                                <?
                                   echo create_drop_down( "cbo_supplier_id", 152, $blank_array,"", 1, "-- All Buyer --", 0, "" );
                                ?>
                            </td>                                
							
							<td>
                            	<input type="text" name="txt_work_order" id="txt_work_order" value="" class="text_boxes" style="width:100px" /> 
                            </td> 
