...
<script>
	...
	{		
		...
		// alert(data);return;
		freeze_window(3);
		http.open("POST","requires/service_work_order_approval_controller.php",true);
	...
	function submit_approved(total_tr,type)
	{
		...
		if(workOrder_ids=="")
		{
			alert("Please Select At Least One Work Order No");
			release_freezing();
			return;
		}
	...
	function change_user()
	{
		...
		var page_link = 'requires/service_work_order_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
	...
	{
		var page_link='requires/service_work_order_approval_controller.php?action='+action+'&id='+id;
	...
	function generate_report(company_name,wo_id,title,format_ids)
	{
		if(format_ids==86)
		{
