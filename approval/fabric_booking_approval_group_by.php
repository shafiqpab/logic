...
<script>
	...
	function fn_report_generated(type)
	{
		...
		if($('#cbo_approval_type').val() == 1){
			if (form_validation('cbo_buyer_name','Buyer Name')==false && form_validation('txt_file_no','File No')==false && form_validation('txt_booking_no','Booking No')==false){
				if (form_validation('txt_date_from*txt_date_to','From Date*To Date')==false)
				{
					...
					$("#txt_file_no").css({ 'background-image': '' });
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_internal_ref*txt_file_no*txt_date_from*txt_date_to*cbo_approval_type*txt_booking_no*cbo_booking_year*txt_alter_user_id*cbo_brand_id*cbo_season_id*cbo_season_year*cbo_style_owner_id',"../");

		http.open("POST","requires/fabric_booking_approval_group_by_controller.php",true);
		...
		{ 
			...
			$("#tbl_list_search tbody").find('tr').each(function()
			{
				try 
				{
					...
					if( hide_approval_type==0)
					{
						var last_update = return_global_ajax_value( trim(booking_no), 'check_booking_last_update', '', 'requires/fabric_booking_approval_group_by_controller');
						if(last_update==2)
						{
					...
					if( hide_approval_type==1)
					{
						...
						var salse_order_approved = return_global_ajax_value( trim(booking_no), 'check_sales_order_approved', '', 'requires/fabric_booking_approval_group_by_controller');
	...
	{ 
		...
		var approval_id_arr = Array(); var appv_in_arr = Array(); var appv_cause_arr = Array();
		for(i=1; i<total_tr; i++)
		{
			if ($('#tbl_'+i).is(":checked"))
			{
				...
				approval_id_arr.push($('#approval_id_'+i).val());
				appv_in_arr.push($('#txt_appv_instra_'+i).val());
		...
		var appv_instras = appv_in_arr.join("','");
		...
		var data="action=approve&operation="+operation+"&approval_type="+type+"&booking_nos='"+booking_nos+"'&booking_ids="+booking_ids+"&approval_ids="+approval_ids+"&appv_instras='"+appv_instras+"'"+"&appv_causes='"+appv_causes+"'"+get_submitted_data_string('cbo_company_name*txt_alter_user_id',"../");
	...
	function generate_fabric_report_history(txt_booking_no,cbo_company_name,txt_order_no_id,cbo_fabric_natu,cbo_fabric_source,revised_no,txt_job_no) {
            var path = '../';
            print_report(txt_booking_no + '**'+cbo_company_name+ '**'+txt_order_no_id+ '**'+cbo_fabric_natu+ '**'+cbo_fabric_source+ '**'+revised_no+ '**'+txt_job_no+'**' + path , "show_fabric_booking_report_libas", "requires/fabric_booking_approval_group_by_controller");
    }
	
	function generate_worder_report4(job_no,action)
	{
		var job_data=return_global_ajax_value( trim(job_no), 'pre_cost_data', '', 'requires/fabric_booking_approval_group_by_controller');
		...
		if(job_data_arr[6]==425){
			var format_ids=return_global_ajax_value( job_data_arr[1]+'**122', 'get_pre_cost_print_button', '', 'requires/fabric_booking_approval_group_by_controller');
			...
			var format_ids=return_global_ajax_value( job_data_arr[1]+'**161', 'get_pre_cost_print_button', '', 'requires/fabric_booking_approval_group_by_controller');
			...
			var format_ids=return_global_ajax_value( job_data_arr[1]+'**43', 'get_pre_cost_print_button', '', 'requires/fabric_booking_approval_group_by_controller');
	...
	{
		var page_link='requires/fabric_booking_approval_group_by_controller.php?action='+action+'&job_no='+job_no;
	...
	function generate_mkt_report(job_no,booking_no,order_id,fab_nature,fab_source,action)
	{
		...
		var page_link='requires/fabric_booking_approval_group_by_controller.php?action='+action+'&company_name='+company_name+'&job_no='+job_no+'&booking_no='+booking_no+'&order_id='+order_id+'&fab_nature='+fab_nature+'&job_ids='+fab_source;
	...
	function change_user()
	{
		...
		var title = 'User Info';	
		var page_link = 'requires/fabric_booking_approval_group_by_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/fabric_booking_approval_group_by_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
	...
	function call_print_button_for_mail(mail){
		...
		var mail=mail.split(',');
		var ret_data=return_global_ajax_value(sysIdArr.join('*')+'__'+mail.join('*'), 'approval_notification', '', 'requires/fabric_booking_approval_group_by_controller');
		alert(ret_data);
	}
	...
	function generate_fabric_report(txt_booking_no,cbo_company_name,txt_order_no_id,cbo_fabric_natu,cbo_fabric_source,revised_no,id_approved_id,txt_job_no,entry_form)
	{
		...
		print_report(txt_booking_no + '**'+cbo_company_name+ '**'+txt_order_no_id+ '**'+cbo_fabric_natu+ '**'+cbo_fabric_source+ '**'+revised_no+ '**'+txt_job_no+ '**'+show_yarn_rate+ '**'+report_title+'**' + path+'**'+id_approved_id , action, "requires/fabric_booking_approval_group_by_controller");
	}
             ...
             <fieldset style="width:1400px;">
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        <thead>
                        	<tr> 
                            	<th colspan="8">
									Barcode Scan : <input type="text" id="txt_bar_code" name="txt_bar_code" class="text_boxes"  />
								</th>
                                <th colspan="10">
                                <?php 
                        ...
                        <tbody>
                            <tr>
                                <td align="center">
									<?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 and core_business not in(3) $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/fabric_booking_approval_group_by_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );" );
                                    ?>
								</td>

								<td align="center">
									<? echo create_drop_down( "cbo_style_owner_id", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- All --", $selected, "" ); ?>
								</td>


                                <td align="center" id="buyer_td_id"><?=create_drop_down( "cbo_buyer_name", 100, $blank_array,"", 1, "-- All Buyer --", 0, "" ); ?></td>
								<td align="center" id="brand_td"><?= create_drop_down( "cbo_brand_id", 60, $blank_array,'', 1, "--Brand--",$selected );?></td>
								<td align="center" id="season_td"><?echo create_drop_down( "cbo_season_id", 70, $blank_array,'', 1, "--Season--",$selected, "" );                     
                        ?></td>
								<td align="center"><? echo create_drop_down( "cbo_season_year", 60, create_year_array(),"", 1,"-Year-", 1, "",0,"" );?></td>
                                <td align="center"><input name="txt_internal_ref" id="txt_internal_ref" class="text_boxes" style="width:100px"></td>
                      			<td align="center"><input name="txt_file_no" id="txt_file_no" class="text_boxes" style="width:65px"></td>
                                <td align="center"><?=create_drop_down( "cbo_booking_year",60, $year,"", 1, "-- Year --", date('Y'), "" ); ?></td>
								<td align="center"><input name="txt_booking_no" id="txt_booking_no" class="text_boxes" style="width:65px"></td>
                                <td align="center">
                                ...
                                <td align="center"><?=create_drop_down( "cbo_approval_type", 90, $approval_type_arr,"", 0, "", $selected,"","", "" ); ?></td>
                                <td align="center">
                                	<input type="button" value="Show" name="show" id="show" class="formbutton" style="width:70px;" onClick="fn_report_generated(1);" />
                                </td>
                            </tr>
							<tr>
