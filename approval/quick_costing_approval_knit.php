...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate&previous_approved="+previous_approved+get_submitted_data_string('cbo_buyer_name*cbo_brand*cbo_season_id*cbo_season_year*txt_costshit_no*txt_style_ref*cbo_get_upto*txt_date*cbo_approval_type*cbo_year*txt_alter_user_id*cbo_company_id',"../");
		
		http.open("POST","requires/quick_costing_approval_controller_knit.php",true);
	...
	{
		var page_link='requires/quick_costing_approval_controller_knit.php?action='+action+'&id='+id;
	...
	function fnc_load_cm_compulsory(data)
	{
		...
		var cm_compulsory = return_global_ajax_value( data, 'populate_cm_compulsory', '', 'requires/quick_costing_approval_controller_knit');
	...
	function change_user()
	{
		...
		var page_link = 'requires/quick_costing_approval_controller_knit.php?action=user_popup&cbo_company_id='+$("#cbo_company_id").val();
	...
	{ 
		...
		if(entry_form==458)
		{
			...
			window.open("../order/spot_costing/requires/quick_costing_v3_controller.php?data=" + data+'&action='+action, true );
			return;
		}
	...
	function fnc_print_report_1(txt_job_no,cbo_company_name,cbo_buyer_name,txt_style_ref,txt_costing_date,cbo_costing_per,action)
	{  
		...
		'&cbo_costing_per='+"'"+cbo_costing_per+"'"+
	...
	function fnc_confirm_style(qc_no,update_id)
	{
		...
		var page_link='requires/quick_costing_approval_controller_knit.php?action=confirmStyle_popup';
	...
	function call_print_button_for_mail(mail){
		...
		var mail=mail.split(',');
		var ret_data=return_global_ajax_value(sysIdArr.join('*')+'__'+mail.join('*'), 'app_deny_mail', '', '../approval/requires/quick_costing_approval_controller_knit');
		//alert(ret_data);
	}
             ...
             <fieldset style="width:1110px;">
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
							...
							<td> 
                                    <?
                                        echo create_drop_down( "cbo_company_id", 120, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/quick_costing_approval_controller_knit',this.value, 'load_dropdown_buyer', 'buyer_td_id' );" );
                                    ?>
                                </td>

                                <td id="buyer_td_id"><?=create_drop_down( "cbo_buyer_name", 120, $blank_array,"", 1, "-- All Buyer --", $selected, "" ); ?></td>
