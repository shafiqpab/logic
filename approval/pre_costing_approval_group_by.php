<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Pre Costing Approval Group By
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Pre Costing Approval Group By", "../", 1, 1,'','','');
$approval_setup=is_duplicate_field( "page_id", "electronic_approval_setup", "entry_form=77 and is_deleted=0" );
?>	
<script>
	...
	function fn_report_generated()
	{
		...
		if($('#cbo_approval_type').val() == 1){
			if (form_validation('cbo_buyer_name','Buyer Name')==false && form_validation('txt_job_no','Job No')==false && form_validation('txt_file_no','File No')==false){
				if (form_validation('txt_date_from*txt_date_to','Costing From Date*Costing To Date')==false)
				{
					...
					$("#txt_job_no").css({ 'background-image': '' });
		...
		var data="action=report_generate&previous_approved="+previous_approved+get_submitted_data_string('cbo_company_name*cbo_style_owner_id*cbo_buyer_name*cbo_brand*cbo_season_id*cbo_season_year*txt_styleref*txt_internal_ref*txt_file_no*txt_date_from*txt_date_to*cbo_approval_type*txt_job_no*cbo_year*txt_alter_user_id',"../");
		
		http.open("POST","requires/pre_costing_approval_group_by_controller.php",true);
	...
	{ 
		var booking_nos = "";  var booking_ids = ""; var approval_ids = "";  var refuse_cause_arr = Array(); 
		freeze_window(0);
		// confirm message .....................................
		if($('#cbo_approval_type').val()==1)
		{
		...
		var refuse_causes = refuse_cause_arr.join('**');
		
		var data="action=approve&operation="+operation+'&approval_type='+type+'&booking_nos='+booking_nos+'&booking_ids='+booking_ids+'&approval_ids='+approval_ids+'&refuse_causes='+refuse_causes+get_submitted_data_string('cbo_company_name*txt_alter_user_id',"../");
	   //alert(data);
	...
	function call_print_button_for_mail(mail){
		...
		var mail=mail.split(',');
		var ret_data=return_global_ajax_value(sysIdArr.join('*')+'__'+mail.join('*')+'__'+txt_alter_user_id, 'app_mail_notification', '', '../approval/requires/pre_costing_approval_group_by_controller');
		//alert(ret_data);
	}
	...
	{ 
		...
		if(type == 'preCostRpt2'){
			$path = "../order/woven_order/requires/pre_cost_entry_controller_v2.php";
		}
		else{
			$path = "../order/woven_order/requires/pre_cost_entry_report_controller_v2.php";
		}
	...
	{
		var page_link='requires/pre_costing_approval_group_by_controller.php?action='+action+'&id='+id;
	...
	function fnc_load_cm_compulsory(data)
	{
		...
		var cm_compulsory = return_global_ajax_value( data, 'populate_cm_compulsory', '', 'requires/pre_costing_approval_group_by_controller');
	...
	function change_user()
	{
		...
		var page_link = 'requires/pre_costing_approval_group_by_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/pre_costing_approval_group_by_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
...
<body>
	...
	<? echo load_freeze_divs ("../",''); ?>
         ...
         <h3 style="width:1260px;margin-top:10px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3> 
                 	...
                 	<table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        <thead>
                            ...
                            <tr>
                                ...
                                <th width="80" colspan="2">Costing  Date</th>
                        ...
                        <tbody>
                            <tr class="general">
                                <td><? echo create_drop_down( "cbo_company_name", 100, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/pre_costing_approval_group_by_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );fnc_load_cm_compulsory(this.value);" ); ?></td>

								<td><? echo create_drop_down( "cbo_style_owner_id", 100, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- All --", 0, "" ); ?></td>


                                <td id="buyer_td_id"><? echo create_drop_down( "cbo_buyer_name", 100, $blank_array,"", 1, "-- All Buyer --", 0, "" ); ?></td>
                                ...
                                <td><input type="text" name="txt_date_from" id="txt_date_from" class="datepicker" readonly style="width:70px"/></td>
                                <td><input type="text" name="txt_date_to" id="txt_date_to" class="datepicker" readonly style="width:70px"/></td>
                                <td> 
                                    <?
