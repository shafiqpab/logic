...
<script>
	...
	{   
		...
		var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_supplier_id*txt_wo_no*txt_date_from*txt_date_to*cbo_approval_type*txt_alter_user_id',"../");
		//alert(data);
		freeze_window(3);
	...
	{ 
		...
		var data="action=approve&operation="+operation+"&target_ids="+target_ids+'&cbo_approval_type='+type+get_submitted_data_string('cbo_company_id*cbo_supplier_id*txt_wo_no*txt_date_from*txt_date_to*txt_alter_user_id',"../");
		//alert(data);return;
		freeze_window(0);
		http.open("POST","requires/yarn_service_work_order_approval_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
	...
	let openImgFile=(id,action)=>{
		var page_link='requires/yarn_service_work_order_approval_controller.php?action='+action+'&id='+id;
	...
	function fn_generate_print(update_id,cbo_company_name,txt_booking_no,cbo_pay_mode,cbo_service_type,cbo_supplier_name){
	   //alert(update_id);
	   var form_name="yarn_dyeing_wo_booking_without_order";
	  
		var data = "action=show_trim_booking_report&form_name="+form_name+'&cbo_company_name='+cbo_company_name+'&txt_booking_no='+txt_booking_no+'&cbo_pay_mode='+cbo_pay_mode+'&cbo_supplier_name='+cbo_supplier_name+'&update_id='+update_id;
...
function fn_generate_print_res(){
	...
	function change_user()
	{
		...
		var page_link = 'requires/yarn_service_work_order_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_id',"../");
	...
	function openmypage_refusing_cause(page_link,title,quo_id)
	{
	 ...
	 function call_print_button_for_mail(mail,mail_body,type){
		 ...
		 var mail=mail.split(',');
		 var ret_data=return_global_ajax_value(sysIdArr.join(',')+'__'+mail.join(',')+'__'+txt_alter_user_id+'__'+cbo_company_id+'__'+type, 'app_mail_notification', '', 'requires/yarn_service_work_order_approval_controller');
		 //alert(ret_data);
	 }
                 ...
                 <table class="rpt_table" width="950px%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            <td> 
                                <?
                                    echo create_drop_down( "cbo_company_id", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/yarn_service_work_order_approval_controller',this.value, 'load_supplier_dropdown', 'supplier_td_id' );" );
                                ?>
                            </td>
