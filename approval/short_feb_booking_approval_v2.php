...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Short Fabric Booking Approval V2", "../", 1, 1,'','','');
$approval_setup=is_duplicate_field( "page_id", "electronic_approval_setup", "page_id=$menu_id and is_deleted=0" );
?>	
<script>
	...
	function fn_report_generated(type)
	{
		...
		http.open("POST","requires/short_feb_booking_approval_controller_v2.php",true);
		...
		{ 
			$("#tbl_list_search tbody").find('tr').each(function()
			{
				if(i>1)
				{
					...
					var last_update=return_global_ajax_value( trim(booking_no), 'check_booking_last_update', '', 'requires/short_feb_booking_approval_controller_v2');
					...
					if(approval_type==1)
					{
						var salse_order_approved=return_global_ajax_value( trim(booking_no), 'check_sales_order_approved', '', 'requires/short_feb_booking_approval_controller_v2');
	...
	function check_on_scan(scan_no)
	{
		...
		if(approval_type==1)
		{
			var salse_order_approved=return_global_ajax_value( trim(scan_no), 'check_sales_order_approved', '', 'requires/short_feb_booking_approval_controller_v2');
			if(salse_order_approved==1 || salse_order_approved==3)
			{
				alert("Corresponding Sales Order is Approved.So Booking Unapproved Not Allow.");
				$('#tbl_'+row_no).attr('checked', false);
	...
	{
		var page_link='requires/short_feb_booking_approval_controller_v2.php?action='+action+'&job_no='+job_no;
	...
	function change_user()
	{
		...
		var page_link = 'requires/short_feb_booking_approval_controller_v2.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/short_feb_booking_approval_controller_v2',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
	...
	function call_print_button_for_mail(mail,mail_body,type){
		 ...
		 var mail=mail.split(',');
		 var ret_data=return_global_ajax_value(sysIdArr.join(',')+'__'+mail.join(',')+'__'+txt_alter_user_id+'__'+cbo_company_name+'__'+type, 'app_mail_notification', '', 'requires/short_feb_booking_approval_controller_v2');
		 //alert(ret_data);
	 }
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/short_feb_booking_approval_controller_v2',this.value, 'load_drop_down_buyer', 'buyer_td_id' );get_php_form_data(this.value,'report_formate_setting','requires/short_feb_booking_approval_controller_v2')" );
                                    ?>
                                </td>
                                <td id="buyer_td_id"> 
									<?
