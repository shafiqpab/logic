<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Sourcing Higher Autohority Approval
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Sourcing Higher Authority Approval", "../", 1, 1,'','','');
$approval_setup=is_duplicate_field( "ENTRY_FORM", "electronic_approval_setup", "ENTRY_FORM=86 and is_deleted=0" );
?>	
<script>
	...
	function fn_report_generated()
	{
		...
		http.open("POST","requires/sourcing_higher_authority_approval_controller.php",true);
	...
	{
		var page_link='requires/sourcing_higher_authority_approval_controller.php?action='+action+'&id='+id;
	...
	function change_user()
	{
		...
		var page_link = 'requires/sourcing_higher_authority_approval_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			//load_drop_down( 'requires/sourcing_higher_authority_approval_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
			load_drop_down( 'requires/sourcing_higher_authority_approval_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
	...
	function generat_print_report_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			var w = window.open("Surprise", "_blank");
			var d = w.document.open();
			d.write('<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01//EN""http://www.w3.org/TR/html4/strict.dtd">'+'<html><head><title></title></head><body>'+document.getElementById('report_container2').innerHTML+'<link rel="stylesheet" href="../css/style_common.css" type="text/css" /></body</html>');//
			d.close();
         ...
         <div id="content_search_panel">      
             <fieldset style="width:930px;">
                 	<table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td><? echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- All --", $selected, "load_drop_down( 'requires/sourcing_higher_authority_approval_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );" ); ?></td>
