<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Pre Costing Approval WVN V2
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Pre Costing Approval V3", "../", 1, 1,'','','');
...
<script>
	...
	function fn_report_generated()
	{
		...
		var data="action=report_generate&previous_approved="+previous_approved+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_brand*cbo_season_id*cbo_season_year*txt_styleref*txt_internal_ref*txt_file_no*cbo_get_upto*txt_date*cbo_approval_type*txt_job_no*cbo_year*txt_alter_user_id*cbo_style_owner_id',"../");
		
		http.open("POST","requires/pre_costing_approval_v3_controller.php",true);
	...
	function openmypage_approve_cause(page_link,title,quo_id)
	{
		var cause=document.getElementById("txtAppCause_"+quo_id).value;
		...
		emailwindow.onclose=function()
		{
			...
			var cause=this.contentDoc.getElementById("txt_approved_cause").value;
			document.getElementById("txtAppCause_"+quo_id).value=cause;
			/*if (cause!="")
			{
				fn_report_generated();
			}*/
	...
	{ 
		...
		for(i=1; i<total_tr; i++)
		{
			if ($('#tbl_'+i).is(":checked"))
			{
				...
				job_id_arr.push($('#hidden_job_id_'+i).val());
		...
		var data="action=approve&operation="+operation+'&approval_type='+type+'&booking_nos='+booking_nos+'&booking_ids='+booking_ids+'&approval_ids='+approval_ids+'&job_ids='+job_id_arr.join(',')+get_submitted_data_string('cbo_company_name*txt_alter_user_id',"../");
	   //alert(data);
	...
	function call_print_button_for_mail(mail,mail_body,type){
		...
		var mail=mail.split(',');
		var ret_data=return_global_ajax_value(sysIdArr.join('*')+'__'+mail.join('*')+'__'+txt_alter_user_id+'__'+type, 'app_mail_notification', '', '../approval/requires/pre_costing_approval_v3_controller');
		//alert(ret_data);
	}
	...
	function generate_worder_report(type,job_no,company_id,buyer_id,style_ref,costing_date,breack_down_id,costing_per,version,entry_form,job_id)
	{  
		...
		if(entry_form == 158){
		 ...
		 if(type=='budgetsheet')
			...
			var data=`action=${type}&version=${version}&zero_value=${zero_val}&rate_amt=${rate_amt}&txt_job_no='${job_no}'&cbo_company_name=${company_id}&cbo_buyer_name=${buyer_id}&txt_style_ref='${style_ref}'&txt_costing_date=${costing_date}&txt_po_breack_down_id='${breack_down_id}'&cbo_costing_per=${costing_per}&path=../&image_path=../`;
			var path = "../order/woven_order/requires/pre_cost_entry_controller_v2.php";

			}

			else if(type=='budget5')
			{
				var rpt_type = 8;var comments_head = 1; var type = 'report_generate';

				var data=`action=${type}&version=${version}&reporttype=${rpt_type}&txt_style_ref_id=${job_id}&comments_head=${comments_head}&sign='0'&zero_value=${zero_val}&rate_amt=${rate_amt}&txt_job_no='${job_no}'&cbo_company_name=${company_id}&cbo_buyer_name=${buyer_id}&txt_style_ref='${style_ref}'&txt_costing_date=${costing_date}&txt_po_breack_down_id='${breack_down_id}'&cbo_costing_per=${costing_per}&path=../../../&image_path=../../../&report_title='Budget/Cost Sheet'`;
			   path = "../reports/management_report/merchandising_report/requires/cost_breakup_report2_controller.php";


			}

			else if(type=='bomRpt2')
			{
				var rate_amt=2; var zero_val='';
			var r=confirm("Press  \"OK\"  to open with zero value\nPress  \"Cancel\"  to open without zero value");
			if (r==true) zero_val="1"; else zero_val="0";
			...
			var data=`action=${type}&version=${version}&zero_value=${zero_val}&rate_amt=${rate_amt}&txt_job_no='${job_no}'&cbo_company_name=${company_id}&cbo_buyer_name=${buyer_id}&txt_style_ref='${style_ref}'&txt_costing_date=${costing_date}&txt_po_breack_down_id='${breack_down_id}'&cbo_costing_per=${costing_per}&path=../&image_path=../`;
			var path = "../order/woven_order/requires/pre_cost_entry_controller_v3.php";
		}
	...
	{
		var page_link='requires/pre_costing_approval_v3_controller.php?action='+action+'&id='+id;
	...
	function fnc_load_cm_compulsory(data)
	{
		...
		var cm_compulsory = return_global_ajax_value( data, 'populate_cm_compulsory', '', 'requires/pre_costing_approval_v3_controller');
	...
	function change_user()
	{
		...
		var page_link = 'requires/pre_costing_approval_v3_controller.php?action=user_popup'+get_submitted_data_string('cbo_company_name',"../");
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/pre_costing_approval_v3_controller',$("#cbo_company_name").val()+"_"+data_arr[0], 'load_drop_down_buyer_new_user', 'buyer_td_id' );
...
<body>
	...
	<? echo load_freeze_divs ("../",''); ?>
         ...
         <h3 style="width:1460px;margin-top:10px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3> 
             ...
             <fieldset style="width:1450px;">
             	<input type="hidden" name="txt_style_ref" id="txt_style_ref">
                 	<table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td><? echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/pre_costing_approval_v3_controller',this.value, 'load_drop_down_buyer', 'buyer_td_id' );fnc_load_cm_compulsory(this.value);" ); ?></td>
								<td><? echo create_drop_down( "cbo_style_owner_id", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- All --", $selected, "" ); ?></td>
