...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Other Purchase WO Approval Group By", "../", 1, 1,'','','');
...
<script>
	...
	{ 
		...
		http.open("POST","requires/other_purchase_work_order_approval_group_by_controller.php",true);
	...
	function submit_approved(total_tr,type)
	{
		...
		// Confirm Message End *******************************************************************************
		var mst_id_company_ids='';var appv_cause_arr = Array();
		for(i=1; i<total_tr; i++)
		{
			if ($('#tbl_'+i).is(":checked"))
			{
				...
				if(trim($('#txtCause_'+req_id).val())=='' && $('#cbo_approval_type').val() == 1){
					alert('Please Insert Unapprove Cause');
					release_freezing();
					$('#txtCause_'+req_id).focus()
					return;
				}
				...
				//appv_cause_arr.push($('#txt_refusing_cause'+i).val());

				var mst_id_company_id = $('#mst_id_company_id_'+i).val();
				var [req_id,_,company_id] = mst_id_company_id.split('*');
				appv_cause_arr.push(company_id+'###'+req_id+'###'+$('#txtCause_'+req_id).val());
		...
		var data="action=approve&operation="+operation+'&approval_type='+type+'&req_nos='+req_nos+'&approval_ids='+approval_ids+'&appv_causes='+appv_causes+'&mst_id_company_ids='+mst_id_company_ids+get_submitted_data_string('cbo_company_name*txt_alter_user_id*cbo_supplier_id',"../"); 
		//alert(data);
	...
	function change_user()
	{
		...
		var page_link = 'requires/other_purchase_work_order_approval_group_by_controller.php?action=user_popup&company_id='+$("#cbo_company_name").val();
         ...
         <div id="content_search_panel">
             <fieldset style="width:950px;">
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td><?=create_drop_down( "cbo_company_name", 160, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0  $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/other_purchase_work_order_approval_group_by_controller',this.value, 'load_supplier_dropdown', 'supplier_td_id' );" ); ?></td>
