...
<script>
			...
			for(var i=1; i<=tot_row; i++)
			{
				if(i!=rowNo)
				{
					...
					if(check==1 && currentRowColor=='yellow')
					{
						...
						if(body_part_type==40 || body_part_typeCur==50)
						{
							...
							{
								alert("Do Not Allow Collar and Cuff Merge");
								return;
							}
						}
					}
	...
	{
		var response=return_global_ajax_value( companyID, 'check_collar_cuff_variable', '', 'requires/planning_info_entry_controller');
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td>
                                    <?
                                        echo create_drop_down( "cbo_company_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and core_business not in(3) $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/planning_info_entry_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );get_php_form_data(this.value,'approval_needed_or_not','requires/planning_info_entry_controller' );collar_cuff_variable_chk(this.value);" );
                                    ?>
