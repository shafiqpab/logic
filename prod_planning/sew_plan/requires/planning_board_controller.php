...
if($action=="show_plan_details")
{
	...
	$line_names=return_library_array("select id,line_name from lib_sewing_line ", "id","line_name",1);   	 	
	$new_line_res=return_library_array("select id,line_number from  prod_resource_mst where company_id='$data[0]' and location_id='$data[1]' and is_deleted=0 order by id ", "id","line_number",1);
	...
	foreach($new_line_res as $key=>$line)
	{
		...
		{
			if($ln=="") $ln="Line: ".$line_names[$nline]; else $ln .= ",Line: ".$line_names[$nline];
		}
		$new_line_resource[$key]=$ln;
	}
	...
	{
		$next_month=add_month($from_date,$i);
	...
	<tr>
        ...
        <input type="hidden" id="txt_overlaaped_id">
         ...
         <?  	for($j=0; $j<count($days); $j++)
				{
					$next_month=add_month($from_date,$j);
	...
	<?  
		foreach( $new_line_resource as $i=>$line)
		{
            ...
            <?
				...
				{
					if( $j==0 ) { $width=""; $idd=""; $text=" ".$line; $tdcls="left_header";  } else { $tdate=date("dmY",strtotime(add_date($from_date,$j-1))); $idd="-".$i."-".$tdate; $width="20"; $text="&nbsp;"; $tdcls="verticalStripes1"; }
...
if ($action=="order_popup")
{
	...
	extract($_REQUEST);
	 // echo $company_id;
	$line_names=return_library_array("select id,line_name from lib_sewing_line ", "id","line_name",1);   	 	
	$new_line_ress=return_library_array("select id,line_number from  prod_resource_mst where company_id='$company_id' and location_id='$location_id' and is_deleted=0 order by id ", "id","line_number",1);
	$new_line_res=array(); // 
	foreach($new_line_ress as $key=>$line)
	{
		$lines=explode(",",$line);
		$ln="";
		foreach($lines as $nline )
	...
	//$new_line_res=return_library_array("select id,line_name from lib_sewing_line where id<31 order by id ", "id","line_name",1);
...
if($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
	  ...
	  //echo "0**"; print_r(bulk_update_sql_statement( "ppl_sewing_plan_board", "id", $field_array_up, $data_array_up, $id_arr )); die;
		if(count( $id_arr )>0)
		{
	...
	// echo "insert into ppl_sewing_plan_board ( ".$field_array. ") values " .$data_array;	
	 	if($data_array!="")
