...
<script>
	...
	//show_list_view(1, 'booking_item_details', 'list_container_fabric_desc', 'requires/planning_info_entry_for_sales_order_v3_controller', '');
	}
    ...
    function openmypage_job()
	{
        ...
        var page_link = 'requires/planning_info_entry_for_sales_order_v3_controller.php?action=style_ref_search_popup&companyID=' + companyID + '&buyerID=' + buyerID + '&within_group=' + within_group;
        ;
    ...
    function openmypage_internal_ref() {
        ...
        var page_link = 'requires/planning_info_entry_for_sales_order_v3_controller.php?action=internal_ref_no_search_popup&companyID=' + companyID + '&cbo_within_group=' + cbo_within_group;
    ...
    function openmypage_booking() {
		...
		isLoadDataByCrossBtn=1;
        var page_link = 'requires/planning_info_entry_for_sales_order_v3_controller.php?action=booking_no_search_popup&companyID=' + companyID + '&cbo_within_group=' + cbo_within_group;
    ...
    function openmypage_prog() {
        ...
        for (var j = 1; j <= tot_row; j++) {
            ...
            if (currentRowColor == 'yellow') {
                ...
                color_range_id = $('#color_range_id_' + j).val();
        ...
        var page_link = 'requires/planning_info_entry_for_sales_order_v3_controller.php?action=prog_qnty_popup&gsm=' + gsm + '&dia=' + dia + '&desc=' + desc + '&within_group=' + within_group + '&job_id=' + job_id + '&booking_qnty=' + booking_qnty + '&companyID=' + companyID + '&data="' + data + '"' + '&plan_id=' + plan_id + '&determination_id=' + determination_id + '&booking_no=' + encodeURIComponent(String(booking_no)) + '&body_part_id=' + body_part_id + '&fabric_type=' + fabric_typee + '&pre_cost_id=' + pre_cost_id + '&buyer_id=' + buyer_id + '&job_dtls_id=' + job_dtls_id + '&color_type_id=' + color_type_id + '&color_range_id=' + color_range_id + '&sales_order_dtls_id=' + sales_order_dtls + '&pre_cost=' + pre_cost + '&balance_qnty=' + balance_qnty+'&hdnVariableCollarCuff='+hdnVariableCollarCuff+'&bookingWithoutOrder='+bookingWithoutOrder;
    ...
    function selected_row(rowNo, status) {
        ...
        var color_range_id = $('#color_range_id_' + rowNo).val();
            ...
            for (var i = 1; i <= tot_row; i++) {
                if (i != rowNo) {
                    ...
                    if (currentRowColor == 'yellow') {
                        ...
                        var color_range_idCur = $('#color_range_id_' + i).val();
                        ...
                        //Note: color type id must be validate, don't marge different color type. because color type saved on planing and production. crm id 18584,18588

                        //alert(bodyPartType +'='+ bodyPartTypeCur);
                        if (plan_id == "" || plan_idCur == "") {
                            if (!(job_id == job_idCur && bookingNo == bookingNoCur && determinationId == determinationIdCur && widthDiaType == widthDiaTypeCur && gsm == gsmCur && fabricDia == fabricDiaCur && stripe_or_not == stripe_or_notCur && job_id == job_idCur  && color_type_id == color_type_idCur && bodyPartType == bodyPartTypeCur && color_range_id == color_range_idCur )) {
                            ...
                            if (!(plan_id == plan_idCur && job_id == job_idCur && bookingNo == bookingNoCur && determinationId == determinationIdCur && widthDiaType == widthDiaTypeCur && gsm == gsmCur && fabricDia == fabricDiaCur && stripe_or_not == stripe_or_notCur && job_id == job_idCur  && color_type_id == color_type_idCur  && bodyPartType == bodyPartTypeCur && color_range_id == color_range_idCur )) {
    ...
    function active_inactive() {
        ...
        if (company_id == 0) {
            ...
            load_drop_down('requires/planning_info_entry_for_sales_order_v3_controller', company_id + '_' + within_group, 'load_drop_down_buyer', 'buyer_td');
        }
    }
    ...
    function generate_report2(company_id, program_id,format_id="") {
       //action name= print
        ...
        if(format_id==273)
        {
        	print_report(company_id + '*' + program_id + '*' + path, "prog_info_print", "requires/planning_info_entry_for_sales_order_v3_controller");
        }
    ...
    function generate_delete_booking_report(company_id, program_id,format_id="") {
       //action name= print
        ...
        if(format_id==273)
        {
            print_report(company_id + '*' + program_id + '*' + path, "delete_booking_prog_info_print", "requires/planning_info_entry_for_sales_order_v3_controller");
        }
        else if(format_id==78)
        {
            print_report(company_id + '*' + program_id, "print_popup", "requires/yarn_requisition_entry_sales_controller");
    ...
    function activePlan(e,plan_id,program_ids,balance_qnty,dtls_ids,prog_qty){
        if(confirm("Are you sure?")){
            if(balance_qnty >= 0){
                ...
                var response = trim(return_global_ajax_value(data, 'activePlan', '', 'requires/planning_info_entry_for_sales_order_v3_controller'));
                if(response == 1){
    ...
    function openmypage_barcode() {
        ...
        var within_group = $("#cbo_within_group").val();
        var page_link = 'requires/planning_info_entry_for_sales_order_v3_controller.php?action=barcode_popup&companyID=' + companyID + '&within_group=' + within_group;
        ;
        var title = 'Style Ref./ Job No. Search';
    ...
    {
        var response=return_global_ajax_value( companyID, 'check_collar_cuff_variable', '', 'requires/planning_info_entry_for_sales_order_v3_controller');
