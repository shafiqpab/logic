...
<script>
	...
	function show_details(type)
	{
		...
		{
			var data="action=booking_item_details"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*hide_job_id*cbo_planning_status*txt_order_no*hide_order_id*txt_booking_no*approval_needed_or_not*txt_internal_ref*txt_file_no*txt_date_from*txt_date_to*cbo_booking_type',"../")+'&type='+type;
		}
		else
		{
			if(form_validation('txt_booking_no','Booking No.')==false)
	...
	function openmypage_style()
	{
		...
		var page_link='requires/planning_info_entry_controller.php?action=style_ref_search_popup&companyID='+companyID+'&buyerID='+buyerID;
	...
	function openmypage_order()
	{
		...
		var companyID = $("#cbo_company_name").val();
		var buyerID = $("#cbo_buyer_name").val();
		var page_link='requires/planning_info_entry_controller.php?action=order_no_search_popup&companyID='+companyID+'&buyerID='+buyerID;
	...
	{
		var page_link='requires/planning_info_entry_controller.php?action=yarn_desc_pop_up&desc='+desc;
	...
	function openmypage_booking()
	{
		...
		var page_link='requires/planning_info_entry_controller.php?action=booking_no_search_popup&companyID='+companyID+'&buyerID='+buyerID+'&cbo_booking_type='+cbo_booking_type;;
	...
	function openmypage_prog()
	{
		...
		var page_link='requires/planning_info_entry_controller.php?action=prog_qnty_popup&gsm='+gsm+'&dia='+dia+'&desc='+desc+'&start_date='+start_date+'&end_date='+end_date+'&booking_qnty='+booking_qnty+'&companyID='+companyID+'&data='+data+'&desc_of_yarn='+desc_of_yarn+'&determination_id='+determination_id+'&booking_no='+booking_no+'&body_part_id='+body_part_id+'&fabric_type='+fabric_typee+'&buyer_id='+buyer_id+'&po_id='+po_id+'&pre_cost_id='+pre_cost_id+'&color_type_id='+color_type_id+'&balance_qnty='+balance_qnty+'&plan_id='+plan_id+'&prog_id='+prog_id;
                 ...
                 <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?
                                        echo create_drop_down( "cbo_company_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/planning_info_entry_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );get_php_form_data(this.value,'approval_needed_or_not','requires/planning_info_entry_controller' );" );
                                    ?>
                                </td>
                                <td id="buyer_td">
                                    <? 
