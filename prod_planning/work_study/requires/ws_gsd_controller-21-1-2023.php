        ...
        <form name="system_1" id="system_1" autocomplete="off">
            <table width="960" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                ...
                <tr class="general">
                    ...
                    <td>
                        <? echo create_drop_down( "cbo_bulletin_type", 120, $bulletin_type_arr,'', 1, "-Select Bulletin Type-","","","","" ); ?>
                    </td>
                    <td>
                    	<input type="text" style="width:120px" class="text_boxes"  name="txt_search_prod" id="txt_search_prod" placeholder="Write" />	
                    </td>
                    <td>
                    	<input type="text" style="width:100px" class="text_boxes_numeric"  name="txt_system_id" id="txt_system_id" placeholder="Write" />	
                    </td>
                    <td align="center">
                    	<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_gmt_item').value+'_'+document.getElementById('txt_system_id').value+'_'+document.getElementById('txt_search_prod').value+'_'+document.getElementById('cbo_bulletin_type').value+'_'+document.getElementById('txt_internal_ref').value+'_'+document.getElementById('cbo_string_search_type').value, 'systemId_list_view', 'search_div', 'ws_gsd_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />
                    </td>
                </tr>
            </table>
...
if ($action=="systemId_list_view")
{
	...
	$process_arr = array(7 => "Cutting", 4 => "Finishing", 8 => "Sewing", 4 => "Production");
	$arr=array (2=>$buyer_name_arr,6=>$garments_item,8=>$color_type,9=>$process_arr,10=>$bulletin_type_arr,12=>$user_arr,13=>$user_arr);
...
{ //echo $data;
	...
	$sql ="select id, operation_name, resource_sewing, operator_smv, helper_smv, bodypart_id, inserted_by, fabric_type from lib_sewing_operation_entry where status_active=1 and is_deleted=0 and department_code=$cbo_process_id $gmts_item_id_cond $product_dept_id_cond $body_part_cond order by bodypart_id, operation_name";
...
-->            <?
			...
			foreach($result as $row)	
			{
				...
				if($prev_entry_opa_arr[$row[csf('id')]]!=''){$bgcolor="reen";}
				 ...
				 $data=$row[csf('id')]."_".$row[csf('operation_name')]."_".$row[csf('resource_sewing')]."_".$row[csf('operator_smv')]."_".$row[csf('helper_smv')]."_".$row[csf('bodypart_id')]."_".$row[csf('bodypart_id')];
			?>
			...
			<?
				...
				?>
            		<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="get_php_form_data('<? echo $row[csf('id')]; ?>', 'populate_details_form_data', 'requires/ws_gsd_controller');"> 
						<td align="center" width="25"><input type="checkbox" class="operation_check" value="<?=$row[csf('id')]; ?>"></td>
                        ...
                        <td width="100"><? echo $production_resource_arr[$row[csf('resource_gsd')]]; ?>&nbsp;</td>
...
if($action=='populate_details_form_data')
{
	...
	$sql=sql_select("SELECT id, mst_id, row_sequence_no, body_part_id, lib_sewing_id, resource_gsd, attachment_id, oparetion_type_id, operator_smv, helper_smv, efficiency,target_on_full_perc, target_on_effi_perc from ppl_gsd_entry_dtls where id=$data and is_deleted=0 ");
	/*
...
if ($action=="save_update_delete")
{
	...
	//echo "10**".str_replace("'","",$txt_applicable_period);die;
	...
	if ( $operation==0 )   // Insert Here
	{
		...
		if(str_replace("'","",$update_id)==''){
			$mst_sql = "select ID from PPL_GSD_ENTRY_MST where STYLE_REF = $txt_style_ref AND GMTS_ITEM_ID = $cbo_gmt_item AND BUYER_ID = $cbo_buyer AND PROCESS_ID =$cbo_process_id AND BULLETIN_TYPE in( 2,3 )  AND BULLETIN_TYPE =$cbo_bulletin_type AND IS_DELETED = 0 AND STATUS_ACTIVE = 1";
		 //echo "duplicate**". $mst_sql;die;
		...
		if(str_replace("'",'',$update_id)=="")
		{
			...
			if(str_replace("'",'',$cbo_resource)==40 || str_replace("'",'',$cbo_resource)==41 || str_replace("'",'',$cbo_resource)==43 || str_replace("'",'',$cbo_resource)==44 || str_replace("'",'',$cbo_resource)==48 || str_replace("'",'',$cbo_resource)==68 || str_replace("'",'',$cbo_resource)==69)
			{
				$txt_mc_smv=0;
				$txt_finishing_smv=0;
			...
			$field_array="id, system_no_prefix, system_no, buyer_id, style_ref, custom_style, remarks, fabric_type,bulletin_type,prod_description, gmts_item_id, working_hour, operation_count, mc_operation_count, total_smv, tot_mc_smv, tot_manual_smv, tot_finishing_smv, quotation_id, product_dept, color_type,applicable_period,internal_ref, complexity_level,process_id,inserted_by, insert_date";
			$data_array="(".$id.",".$id.",".$id.",".$cbo_buyer.",".$txt_style_ref.",".$txt_custom_style.",".$txt_remarks.",".$txt_fabric_type.",".$cbo_bulletin_type.",".$txt_product_description.",".$cbo_gmt_item.",".$txt_working_hour.",".$txt_operation_count.",".$txt_mcOperationCount.",".$tot_smv_operation.",".$txt_mc_smv.",".$txt_manual_smv.",".$txt_finishing_smv.",".$hidden_quotation_id.",".$cbo_product_department.",".$cbo_colortype.",".$txt_applicable_period.",".$txt_internal_ref.",".$complexity_level.",".$cbo_process_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')"; 
			$next_seq_no=2;
		}
		else
		{
			...
			sum(case when resource_gsd not in(40,41,43,44,48,68,69,53,54,55,56,70,176) then total_smv end) as tot_smv_mc, 
			...
			$field_array="buyer_id*style_ref*custom_style*remarks*prod_description*gmts_item_id*working_hour*operation_count*mc_operation_count*total_smv*tot_mc_smv*tot_manual_smv*tot_finishing_smv*quotation_id*product_dept*color_type*applicable_period*internal_ref*complexity_level*process_id*updated_by*update_date";
			$data_array=$cbo_buyer."*".$txt_style_ref."*".$txt_custom_style."*".$txt_remarks."*".$txt_product_description."*".$cbo_gmt_item."*".$txt_working_hour."*".$txt_operation_count."*".$txt_mcOperationCount."*".$total_smv."*'".$txt_mc_smv."'*'".$txt_manual_smv."'*'".$txt_finishing_smv."'*".$hidden_quotation_id."*".$cbo_product_department."*".$cbo_colortype."*".$txt_applicable_period."*".$txt_internal_ref."*".$complexity_level."*".$cbo_process_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";  
		...
		$data_array_dtls="(".$id_dtls.",".$id.",".$txt_seqNo.",".$cbo_resource.",".$cbo_body_part.",".$hidden_operation.",".$txt_attachment_id.",".$txt_efficiency.",".$txt_tgt_perc.",".$txt_tgt_eff.",".$txt_operator.",".$txt_helper.",".$tot_smv_operation.")"; 
	...
	else if ($operation==1)   // Update Here=================
	{
		...
		count(case when resource_gsd not in(40,41,43,44,48,68,69,53,54,55,56,70,176) then id end) as m_op_count,
		sum(total_smv) as tot_smv, 
		sum(case when resource_gsd not in(40,41,43,44,48,68,69,53,54,55,56,70,176) then total_smv end) as tot_smv_mc,
		sum(case when resource_gsd in(40,41,43,44,48,68,69) then total_smv end) as tot_smv_mn, 
		...
		//$next_seq_no=$dataArray[0][csf('seq_no')]+1;
		$field_array="buyer_id*style_ref*custom_style*remarks*fabric_type*bulletin_type*prod_description*gmts_item_id*working_hour*operation_count*mc_operation_count*total_smv*tot_mc_smv*tot_manual_smv*tot_finishing_smv*quotation_id*product_dept*color_type*applicable_period*internal_ref*complexity_level*process_id*updated_by*update_date";
		$data_array=$cbo_buyer."*".$txt_style_ref."*".$txt_custom_style."*".$txt_remarks."*".$txt_fabric_type."*".$cbo_bulletin_type."*".$txt_product_description."*".$cbo_gmt_item."*".$txt_working_hour."*".$txt_operation_count."*".$txt_mcOperationCount."*".$total_smv."*'".$txt_mc_smv."'*'".$txt_manual_smv."'*'".$txt_finishing_smv."'*".$hidden_quotation_id."*".$cbo_product_department."*".$cbo_colortype."*".$txt_applicable_period."*".$txt_internal_ref."*".$complexity_level."*".$cbo_process_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"; 
		
		$field_array_dtls ="row_sequence_no*resource_gsd*body_part_id*lib_sewing_id*attachment_id*efficiency*target_on_full_perc*target_on_effi_perc*operator_smv*helper_smv*total_smv";
		$data_array_dtls=$txt_seqNo."*".$cbo_resource."*".$cbo_body_part."*".$hidden_operation."*".$txt_attachment_id."*".$txt_efficiency."*".$txt_tgt_perc."*".$txt_tgt_eff."*".$txt_operator."*".$txt_helper."*".$tot_smv_operation; 
		...
		//$smv=echo $smv; die;
		 ...
		 //echo "10**".$rID."**".$data_array_dtls;die;
...
if ($action=="copy_bulletin")
{
	...
	else
	{
		...
		if($mst_data[0][csf('style_ref')]==$txt_style_ref && $cbo_bulletin_copy==1 && $cbo_bulletin_type==$mst_data[0][csf('bulletin_type')] && $mst_data[0][csf('buyer_id')]==$cbo_buyer){echo "11**";disconnect($con);die;}
...
{
	$value=return_field_value("work_study_mapping_id", "variable_order_tracking", "variable_list=55 and status_active=1 and is_deleted=0 and  id in(select min(id) from variable_order_tracking where variable_list=55 and status_active=1 and is_deleted=0) order by id asc");
	echo $value;
}
            ...
            <tbody>
                <tr>
                   ...
                   <input type="hidden" style="width:80px" class="text_boxes"  name="txt_requst_no" id="txt_requst_no" />	  
                    ...
                    <td align="center">
                        <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_style').value+'_'+document.getElementById('txt_date_from').value+'_'+<? echo "0"; ?>+'_'+document.getElementById('cbo_temp_id').value+'_'+document.getElementById('cbo_year').value+'_'+document.getElementById('txt_requst_no').value+'_'+document.getElementById('cbo_string_search_type').value, 'create_quotation_quick_cost_search_list_view', 'search_div', 'ws_gsd_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:80px;" />				
                    </td>
            </tr>
        	<tr>                  
...
if($action=="create_quotation_quick_cost_search_list_view")
{
	...
	$arr=array(0=>$buyer_arr,1=>$lib_temp_arr,5=>$season_buyer_wise_arr,6=>$row_status);
	 $sql = "select id, cost_sheet_id, cost_sheet_no, temp_id, lib_item_id, style_ref, buyer_id, cons_basis, season_id, style_des, department_id, delivery_date, exchange_rate, offer_qty, quoted_price, tgt_price, stage_id, costing_date,status_active,extract(year from insert_date) as year from qc_mst where is_deleted=0  $buyer_name $temp_id  $inquery_date  order by id desc ";
	// echo $sql;die;
	echo create_list_view("list_view", "Buyer Name,Template ID,Year,Style Reff.,Costing Date,Season,Status","120,120,50,120,110,80,80","800","260",0, $sql , "js_set_value", "id", "", 1, "buyer_id,temp_id,0,0,0,season_id,status_active", $arr, "buyer_id,temp_id,year,style_ref,costing_date,season_id,status_active", "",'','0') ;
	?>
	</body>           
	<script src="../../../includes/functions_bottom.js" type="text/javascript"></script>
	</html>
...
{
    $sql = sql_select("select  id,style_ref from qc_mst where id='$data'");
	foreach($sql as $row)
	{
            ...
            <?
                ...
                ?>
                    <tr>
						...
						<td align="right"><? echo number_format($manuel,2,'.',''); ?></td>
