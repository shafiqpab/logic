...
class commercial extends report{
	...
	private function _setQuery(){
		...
		$this->_query='select a.job_no AS "job_no",a.total_set_qnty AS "total_set_qnty",b.po_id AS "id",c.item_number_id AS "item_number_id",c.country_id AS "country_id",c.color_number_id AS "color_number_id",c.size_number_id AS "size_number_id",c.order_quantity AS "order_quantity" ,c.plan_cut_qnty As "plan_cut_qnty" ,d.pre_cost_comarci_cost_dtls_id AS "pre_cost_dtls_id",d.item_id AS "item_id",d.rate AS "rate",d. amount AS "amount"    from wo_po_dtls_mst_his a, wo_po_break_down_his b,wo_po_color_size_his c,wo_pre_cost_comarc_cost_dtls_h d where 1=1 '.$this->cond.' and a.job_id=b.job_id and a.approved_no=b.approved_no and a.approval_page=b.approval_page and a.job_id=c.job_id and a.approved_no=c.approved_no and a.approval_page=c.approval_page and a.job_id=d.job_id and a.approved_no=d.approved_no and a.approval_page=d.approval_page  and b.po_id=c.po_break_down_id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1';//order by b.id,d.id
		}
		else{
