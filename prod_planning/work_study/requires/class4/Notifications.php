...
class Notifications
{
    ...
    public function notificationEngine($id,$company,$entry_form,$array_param = array(),$user_id ='')
    {
        ...
        if(!empty($user_ids))
        {
            ...
            {
                $data = json_encode($array_param['approval_data']);
            }
            $desc = "";
            ...
            if($res)
            {
                foreach($users as $user_id)
                {
                    ...
                    $result = sql_select("select FCM_TOKEN from APPROVAL_NOTI_USER_DEVICES where user_id = $user_id order by ID DESC");

                    foreach($result as $row)
                    {
                        $key = $row['FCM_TOKEN'];
                        $this->sendMessageToFCM($key,$title,$data,1);
                    }
                }
            }
            return $res;
    ...
    public function sendMessageToFCM($key,$title,$data,$type ='')
    {
    ...
    public function pushAll($ref_id,$entry_form,$array_param = array())
    {
        ...
        if(count($user_arr) > 0)
        {
            ...
            $result  = sql_select("select FCM_TOKEN from APPROVAL_NOTI_USER_DEVICES where user_id IN (".implode(",",$user_arr).") order by ID DESC");
            $flag = 1;

            foreach($result as $row)
            {
                $key = $row['FCM_TOKEN'];
                $ret =  $this->sendMessageToFCM($key,'',$json,0);
                if($flag == 0 || !isset($ret))
                {
                    $flag = 0;
                }
