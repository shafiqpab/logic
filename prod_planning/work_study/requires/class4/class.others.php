...
class other extends report{
	...
	private $_design_cost='design_cost';
	private $_studio_cost='studio_cost';
	private $_deffdlc_cost='deffdlc_cost';
	private $_interest_cost ='interest_cost';
	private $_incometax_cost = 'incometax_cost';
	private $_margin_pcs_set = 'margin_pcs_set';
	private $_incentives_cost = 'incentives_cost';
	private $_job_qty = 'job_qty';
	...
	private function _setQuery(){
		...
		$this->_query='select a.job_no AS "job_no",a.total_set_qnty AS "total_set_qnty",b.po_id AS "id",c.item_number_id AS "item_number_id",c.country_id AS "country_id",c.color_number_id AS "color_number_id",c.size_number_id AS "size_number_id",c.order_quantity AS "order_quantity" ,c.plan_cut_qnty AS "plan_cut_qnty" ,d.pre_cost_dtls_id AS "pre_cost_dtls_id",d.lab_test AS "lab_test",d.inspection AS "inspection" ,d.cm_cost AS "cm_cost",d.freight AS "freight",d.currier_pre_cost AS "currier_pre_cost",d.certificate_pre_cost AS "certificate_pre_cost",d.common_oh AS "common_oh",d.depr_amor_pre_cost AS "depr_amor_pre_cost", d.design_cost AS "design_cost", d.studio_cost AS "studio_cost",d.deffdlc_cost AS "deffdlc_cost", d.interest_cost AS "interest_cost", d.incometax_cost AS "incometax_cost", d.margin_pcs_set AS "margin_pcs_set", d.incentives_pre_cost as "incentives_pre_cost" from wo_po_dtls_mst_his a, wo_po_break_down_his b,wo_po_color_size_his c,wo_pre_cost_dtls_histry d where 1=1  '.$this->cond.' and  a.job_id=b.job_id and a.approved_no=b.approved_no and a.approval_page=b.approval_page and a.job_id=c.job_id  and a.approved_no=c.approved_no and a.approval_page=c.approval_page and a.job_id=d.job_id  and a.approved_no=d.approved_no and a.approval_page=d.approval_page and b.po_id=c.po_break_down_id  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and d.status_active=1';//order by b.id,d.id
		}
		else{
			$this->_query='select a.job_no AS "job_no",a.total_set_qnty AS "total_set_qnty",b.id AS "id",c.item_number_id AS "item_number_id",c.country_id AS "country_id",c.color_number_id AS "color_number_id",c.size_number_id AS "size_number_id",c.order_quantity AS "order_quantity" ,c.plan_cut_qnty AS "plan_cut_qnty" ,d.id AS "pre_cost_dtls_id",d.lab_test AS "lab_test",d.inspection AS "inspection" ,d.cm_cost AS "cm_cost",d.freight AS "freight",d.currier_pre_cost AS "currier_pre_cost",d.certificate_pre_cost AS "certificate_pre_cost",d.common_oh AS "common_oh",d.depr_amor_pre_cost AS "depr_amor_pre_cost", d.design_cost AS "design_cost", d.studio_cost AS "studio_cost",d.deffdlc_cost AS "deffdlc_cost", d.interest_cost AS "interest_cost", d.incometax_cost AS "incometax_cost", d.margin_pcs_set AS "margin_pcs_set", d.incentives_pre_cost as "incentives_pre_cost" from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c,wo_pre_cost_dtls d where 1=1  '.$this->cond.' and  a.id=b.job_id and a.id=c.job_id and a.id=d.job_id and b.id=c.po_break_down_id  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and d.status_active=1';//order by b.id,d.id
		}
	...
	private function _setAmount($level){
		...
		foreach($this->_dataArray as $row)
		{
			...
			$design_cost_amount=$this->_calculateAmount($orderQuantity,$costingPerQty,$total_set_qnty,$design_cost);
			$studio_cost_amount=$this->_calculateAmount($orderQuantity,$costingPerQty,$total_set_qnty,$studio_cost);
			$deffdlc_cost_amount=$this->_calculateAmount($orderQuantity,$costingPerQty,$total_set_qnty,$deffdlc_cost);
			$interest_cost_amount=$this->_calculateAmount($orderQuantity,$costingPerQty,$total_set_qnty,$interest_cost);
			$incometax_cost_amount=$this->_calculateAmount($orderQuantity,$costingPerQty,$total_set_qnty,$incometax_cost);
			$margin_pcs_cost_amount=$this->_calculateAmount($orderQuantity,$costingPerQty,$total_set_qnty,$margin_pcs_set);
			$incentives_pre_cost_amount=$this->_calculateAmount($orderQuantity,$costingPerQty,$total_set_qnty,$incentives_pre_cost);

			if($level==$this->_By_Job){
				if(isset($Amount[$jobNo][$this->_job_qty])){
					$Amount[$jobNo][$this->_job_qty]+=$orderQuantity;
				}
				else{
					$Amount[$jobNo][$this->_job_qty]=$orderQuantity;
				}
				...
				if(isset($Amount[$jobNo][$this->_design_cost])){
					$Amount[$jobNo][$this->_design_cost]+=$design_cost_amount;
				}
				else{
					$Amount[$jobNo][$this->_design_cost]=$design_cost_amount;
				}
				if(isset($Amount[$jobNo][$this->_studio_cost])){
					$Amount[$jobNo][$this->_studio_cost]+=$studio_cost_amount;
				}
				else{
					$Amount[$jobNo][$this->_studio_cost]=$studio_cost_amount;
				}
				if(isset($Amount[$jobNo][$this->_deffdlc_cost])){
					$Amount[$jobNo][$this->_deffdlc_cost]+=$deffdlc_cost_amount;
				}
				else{
					$Amount[$jobNo][$this->_deffdlc_cost]=$deffdlc_cost_amount;
				}
				if(isset($Amount[$jobNo][$this->_interest_cost])){
					$Amount[$jobNo][$this->_interest_cost]+=$interest_cost_amount;
				}
				else{
					$Amount[$jobNo][$this->_interest_cost]=$interest_cost_amount;
				}
				if(isset($Amount[$jobNo][$this->_incometax_cost])){
					$Amount[$jobNo][$this->_incometax_cost]+=$incometax_cost_amount;
				}
				else{
					$Amount[$jobNo][$this->_incometax_cost]=$incometax_cost_amount;
				}

				if(isset($Amount[$jobNo][$this->_incentives_cost])){
					$Amount[$jobNo][$this->_incentives_cost]+=$incentives_pre_cost_amount;
				}
				else{
					$Amount[$jobNo][$this->_incentives_cost]=$incentives_pre_cost_amount;
				}
			...
			elseif($level==$this->_By_Order){
				...
				if(isset($Amount[$poId][$this->_design_cost])){
					$Amount[$poId][$this->_design_cost]+=$design_cost_amount;
				}
				else{
					$Amount[$poId][$this->_design_cost]=$design_cost_amount;
				}
				if(isset($Amount[$poId][$this->_studio_cost])){
					$Amount[$poId][$this->_studio_cost]+=$studio_cost_amount;
				}
				else{
					$Amount[$poId][$this->_studio_cost]=$studio_cost_amount;
				}
				if(isset($Amount[$poId][$this->_deffdlc_cost])){
					$Amount[$poId][$this->_deffdlc_cost]+=$deffdlc_cost_amount;
				}
				else{
					$Amount[$poId][$this->_deffdlc_cost]=$deffdlc_cost_amount;
				}
				if(isset($Amount[$poId][$this->_interest_cost])){
					$Amount[$poId][$this->_interest_cost]+=$interest_cost_amount;
				}
				else{
					$Amount[$poId][$this->_interest_cost]=$interest_cost_amount;
				}
				if(isset($Amount[$poId][$this->_incometax_cost])){
					$Amount[$poId][$this->_incometax_cost]+=$incometax_cost_amount;
				}
				else{
					$Amount[$poId][$this->_incometax_cost]=$incometax_cost_amount;
				}
				if(isset($Amount[$poId][$this->_incentives_cost])){
					$Amount[$poId][$this->_incentives_cost]+=$incentives_pre_cost_amount;
				}
				else{
					$Amount[$poId][$this->_incentives_cost]=$incentives_pre_cost_amount;
				}
			...
			elseif($level==$this->_By_OrderAndCountry){
				...
				if(isset($Amount[$poId][$countryId][$this->_design_cost])){
					$Amount[$poId][$countryId][$this->_design_cost]+=$design_cost_amount;
				}
				else{
					$Amount[$poId][$countryId][$this->_design_cost]=$design_cost_amount;
				}
				if(isset($Amount[$poId][$countryId][$this->_studio_cost])){
					$Amount[$poId][$countryId][$this->_studio_cost]+=$studio_cost_amount;
				}
				else{
					$Amount[$poId][$countryId][$this->_studio_cost]=$studio_cost_amount;
				}
				if(isset($Amount[$poId][$countryId][$this->_deffdlc_cost])){
					$Amount[$poId][$countryId][$this->_deffdlc_cost]+=$deffdlc_cost_amount;
				}
				else{
					$Amount[$poId][$countryId][$this->_deffdlc_cost]=$deffdlc_cost_amount;
				}
				if(isset($Amount[$poId][$countryId][$this->_interest_cost])){
					$Amount[$poId][$countryId][$this->_interest_cost]+=$interest_cost_amount;
				}
				else{
					$Amount[$poId][$countryId][$this->_interest_cost]=$interest_cost_amount;
				}
				if(isset($Amount[$poId][$countryId][$this->_incometax_cost])){
					$Amount[$poId][$countryId][$this->_incometax_cost]+=$incometax_cost_amount;
				}
				else{
					$Amount[$poId][$countryId][$this->_incometax_cost]=$incometax_cost_amount;
				}
				if(isset($Amount[$poId][$this->_incentives_cost])){
					$Amount[$poId][$countryId][$this->_incentives_cost]+=$incentives_pre_cost_amount;
				}
				else{
					$Amount[$poId][$countryId][$this->_incentives_cost]=$incentives_pre_cost_amount;
				}
			...
			elseif($level==$this->_By_OrderAndGmtsitem){
				...
				if(isset($Amount[$poId][$itemNumberId][$this->_margin_pcs_set])){
					$Amount[$poId][$itemNumberId][$this->_margin_pcs_set]+=$margin_pcs_cost_amount;
				}
				else{
					$Amount[$poId][$itemNumberId][$this->_margin_pcs_set]=$margin_pcs_cost_amount;
				}
				if(isset($Amount[$poId][$this->_incentives_cost])){
					$Amount[$poId][$itemNumberId][$this->_incentives_cost]+=$incentives_pre_cost_amount;
				}
				else{
					$Amount[$poId][$itemNumberId][$this->_incentives_cost]=$incentives_pre_cost_amount;
				}
			...
			elseif($level==$this->_By_OrderAndGmtscolor){
				...
				if(isset($Amount[$poId][$this->_incentives_cost])){
					$Amount[$poId][$colorId][$this->_incentives_cost]+=$incentives_pre_cost_amount;
				}
				else{
					$Amount[$poId][$colorId][$this->_incentives_cost]=$incentives_pre_cost_amount;
				}
			...
			elseif($level==$this->_By_OrderAndGmtssize){
				...
				if(isset($Amount[$poId][$this->_incentives_cost])){
					$Amount[$poId][$sizeId][$this->_incentives_cost]+=$incentives_pre_cost_amount;
				}
				else{
					$Amount[$poId][$sizeId][$this->_incentives_cost]=$incentives_pre_cost_amount;
				}
