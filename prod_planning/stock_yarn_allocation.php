...
<script>
	...
	*/
	function func_show_details(type)
	{
		...
		{
			if(form_validation('cbo_company_name*txt_stock_file_no','Company*Stock File No')==false)
			{
				return;
			}
		}
		
		if($('#txt_stock_file_no').val() == '')
		{
			if(form_validation('cbo_company_name*txt_from_date*txt_to_date','Company*from Date*To Date')==false)
			{
				return;
			}
		}
		
		var data="action=pi_item_details"+get_submitted_data_string('cbo_company_name*cbo_supplier_name*cbo_buyer_id*txt_stock_file_no*txt_from_date*txt_to_date',"../")+'&type='+type;
	...
	*/
	function func_file_no()
    {
        ...
        var page_link = 'requires/stock_yarn_allocation_controller.php?action=file_popup&company_id='+company_id+'&supplier_id='+supplier_id+'&buyer_id='+buyer_id;
        ...
        emailwindow.onclose = function ()
        {
            ...
            $('#txt_stock_file_no').val(file_ref);
        }
    }
</script>
</head>
<body onLoad="set_hotkey();">
	<div style="width:100%;" align="center">
		<? echo load_freeze_divs ("../",$permission); ?>
         ...
         <h3 style="width:720px;margin-top:10px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3> 
                 	...
                 	<table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        ...
                        <tbody>
                            <tr class="general">
                                <td> 
                                    <?php
                                    echo create_drop_down( "cbo_company_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/stock_yarn_allocation_controller',this.value, 'load_drop_down_supplier', 'supplier_td' );load_drop_down( 'requires/stock_yarn_allocation_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/stock_yarn_allocation_controller',this.value, 'load_drop_down_buyer_id', 'buyer_id_td' );" );
                                    ?>
                                </td>
                                <td id="supplier_td">
                                    <? 
                                    echo create_drop_down( "cbo_supplier_name", 120, $blank_array,"", 1, "-- All Supplier --", $selected, "",0,"" );
                                    ?>
                                </td>
                                <td id="buyer_id_td">
                                    <? 
                                ...
                                <td>
                                    <input type="text" name="txt_stock_file_no" id="txt_stock_file_no" class="text_boxes" style="width:100px" placeholder="Browse Or Write" onDblClick="func_file_no();">
                                </td>
                                <td>
                                    <input type="text" name="txt_from_date" id="txt_from_date" class="datepicker" style="width:70px">
                                </td>
                                <td>
                                    <input type="text" name="txt_to_date" id="txt_to_date" class="datepicker" style="width:70px">
                                </td>
                                <td>
