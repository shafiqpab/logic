...
<script>
	...
	//show_list_view(1, 'booking_item_details', 'list_container_fabric_desc', 'requires/planning_info_entry_for_sales_order_v2_admin_controller', '');
	}
    ...
    function openmypage_job()
	{
        ...
        var page_link = 'requires/planning_info_entry_for_sales_order_v2_admin_controller.php?action=style_ref_search_popup&companyID=' + companyID + '&buyerID=' + buyerID + '&within_group=' + within_group;
        ;
    ...
    function openmypage_booking() {
        ...
        var page_link = 'requires/planning_info_entry_for_sales_order_v2_admin_controller.php?action=booking_no_search_popup&companyID=' + companyID + '&cbo_within_group=' + cbo_within_group;
    ...
    function active_inactive() {
        ...
        if (company_id == 0) {
            ...
            load_drop_down('requires/planning_info_entry_for_sales_order_v2_admin_controller', company_id + '_' + within_group, 'load_drop_down_buyer', 'buyer_td');
        }
    }
    ...
    function generate_report2(company_id, program_id, format_id="")
	{
		...
		/*if(format_id==273)
		{
			print_report(company_id + '*' + program_id + '*' + path, "prog_info_print", "requires/planning_info_entry_for_sales_order_v2_admin_controller");
		}
		else
		{
			//print_report(company_id + '*' + program_id + '*' + path, "print", "requires/yarn_requisition_entry_sales_controller");
			//print_report(company_id + '*' + program_id + '*' + path, "print", "requires/yarn_requisition_entry_sales_controller");
			//print_report(company_id + '*' + program_id + '*' + path, "requisition_print_two", "requires/planning_info_entry_for_sales_order_v2_admin_controller");
    ...
    function activePlan(e,plan_id,program_ids,balance_qnty,dtls_ids,prog_qty)
	{
        if(confirm("Are you sure?")){
            if(balance_qnty >= 0){
                ...
                var response = trim(return_global_ajax_value(data, 'activePlan', '', 'requires/planning_info_entry_for_sales_order_v2_admin_controller'));
                if(response == 1){
    ...
    function openmypage_barcode() {
        ...
        var within_group = $("#cbo_within_group").val();
        var page_link = 'requires/planning_info_entry_for_sales_order_v2_admin_controller.php?action=barcode_popup&companyID=' + companyID + '&within_group=' + within_group;
        ;
        var title = 'Style Ref./ Job No. Search';
