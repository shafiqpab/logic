...
<script>
	...
	//show_list_view(1, 'booking_item_details', 'list_container_fabric_desc', 'requires/planning_info_entry_for_sales_order_controller', '');
	}
    ...
    function openmypage_job()
	{
        ...
        var page_link = 'requires/planning_info_entry_for_sales_order_controller.php?action=style_ref_search_popup&companyID=' + companyID + '&buyerID=' + buyerID + '&within_group=' + within_group;
        ;
    ...
    function openmypage_internal_ref() {
        ...
        var page_link = 'requires/planning_info_entry_for_sales_order_controller.php?action=internal_ref_no_search_popup&companyID=' + companyID + '&cbo_within_group=' + cbo_within_group;
    ...
    function openmypage_booking() {
		...
		isLoadDataByCrossBtn=1;
        var page_link = 'requires/planning_info_entry_for_sales_order_controller.php?action=booking_no_search_popup&companyID=' + companyID + '&cbo_within_group=' + cbo_within_group;
    ...
    function openmypage_prog() {
        ...
        var page_link = 'requires/planning_info_entry_for_sales_order_controller.php?action=prog_qnty_popup&gsm=' + gsm + '&dia=' + dia + '&desc=' + desc + '&within_group=' + within_group + '&job_id=' + job_id + '&booking_qnty=' + booking_qnty + '&companyID=' + companyID + '&data="' + data + '"' + '&plan_id=' + plan_id + '&determination_id=' + determination_id + '&booking_no=' + encodeURIComponent(String(booking_no)) + '&body_part_id=' + body_part_id + '&fabric_type=' + fabric_typee + '&pre_cost_id=' + pre_cost_id + '&buyer_id=' + buyer_id + '&job_dtls_id=' + job_dtls_id + '&color_type_id=' + color_type_id + '&sales_order_dtls_id=' + sales_order_dtls + '&pre_cost=' + pre_cost + '&balance_qnty=' + balance_qnty+'&hdnVariableCollarCuff='+hdnVariableCollarCuff+'&bookingWithoutOrder='+bookingWithoutOrder;
    ...
    function active_inactive() {
        ...
        if (company_id == 0) {
            ...
            load_drop_down('requires/planning_info_entry_for_sales_order_controller', company_id + '_' + within_group, 'load_drop_down_buyer', 'buyer_td');
        }
    }
    ...
    function generate_delete_booking_report(company_id, program_id,format_id="") {
       //action name= print
        ...
        if(format_id==273)
        {
            print_report(company_id + '*' + program_id + '*' + path, "delete_booking_prog_info_print", "requires/planning_info_entry_for_sales_order_controller");
        }
        else if(format_id==78)
        {
            print_report(company_id + '*' + program_id, "print_popup", "requires/yarn_requisition_entry_sales_controller");
    ...
    function activePlan(e,plan_id,program_ids,balance_qnty,dtls_ids,prog_qty){
        if(confirm("Are you sure?")){
            if(balance_qnty >= 0){
                ...
                var response = trim(return_global_ajax_value(data, 'activePlan', '', 'requires/planning_info_entry_for_sales_order_controller'));
                if(response == 1){
    ...
    function openmypage_barcode() {
        ...
        var within_group = $("#cbo_within_group").val();
        var page_link = 'requires/planning_info_entry_for_sales_order_controller.php?action=barcode_popup&companyID=' + companyID + '&within_group=' + within_group;
        ;
        var title = 'Style Ref./ Job No. Search';
    ...
    {
        var response=return_global_ajax_value( companyID, 'check_collar_cuff_variable', '', 'requires/planning_info_entry_for_sales_order_controller');
        var response=response.split("_");
        $('#txtVariableCollarCuff').val(response[1]);
    }

    function generate_worder_report3(txt_booking_no,cbo_company_name,cbo_fabric_source,id_approved_id,txt_job_no,txt_order_no_id,cbo_fabric_natu,print_id,type,i)
    {
        var is_mail_send;
        var mail_id;
		...
		{
			var fabric_source = cbo_fabric_source;
		...
		freeze_window(5);
		if(print_id==1 || print_id==2 || print_id==849 || print_id==3 || print_id==4 || print_id==5 || print_id==6 || print_id==7 || print_id==13 || print_id==28 || print_id==45 || print_id==53 || print_id==432 || print_id==73 || print_id==84 || print_id==93 || print_id==129 || print_id==193 || print_id==269 || print_id==280 || print_id==39 || print_id==304 || print_id==719 || print_id==723 || print_id==833 || print_id==339 || print_id==370 || print_id==383 || print_id==404 || print_id==419 || print_id==426 || print_id==452 || print_id==786 || print_id==502 || print_id==437 || print_id==865|| print_id==892)
		{
