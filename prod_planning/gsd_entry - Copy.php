...
counter++;
	document.getElementById("txt_operator_tot").value=operator_smv_tot.toFixed( 4 );
	document.getElementById("txt_helper_tot").value=helper_smv_tot.toFixed( 4 );
	document.getElementById("txt_total_tot").value=smv_tot.toFixed( 4 );
...
function fnc_gsd_entry( operation )
{
	...
	var data1="action=save_update_delete&operation="+operation+"&num_row="+i+get_submitted_data_string('cbo_company_id*txt_style_ref*ord_id*cbo_buyer*job_no*txt_order_no*cbo_gmt_item*wo_po_id*txt_working_hour*cbo_action*txt_allowance*txt_sam_for_style*txt_operation_count*txt_pitch_time*txt_total_tot*txt_where_man_power*txt_where_man_power1*update_id*deleted_id',"../");
...
{
	print_report( $('#cbo_company_id').val()+'*'+$('#txt_job_no').val(), "print_gsd_report", "requires/gsd_entry_controller") 
	//return;
	show_msg("3");
}
        ...
        <legend>GSD Entry Info </legend>
            <table cellpadding="0" cellspacing="2" width="100%">
                ...
                <tr>
                    ...
                    <td width="150">
                        <?
                            echo create_drop_down( "cbo_action",148,$row_status,"", 1, "--Select Action--", 1, "","","","","","","");	
                        ?>
                    </td>
