...
if ($action == "create_booking_search_list_view")
{
	...
	$sql = "SELECT ID, BOOKING_NO, BOOKING_DATE, BUYER_ID, JOB_NO, PAY_MODE, SUPPLIER_ID FROM wo_booking_mst WHERE ITEM_CATEGORY = 2 AND FABRIC_SOURCE != 2 AND PAY_MODE = 3 AND company_id = ".$company_id.$search_cond.$buyer_id_cond.$year_cond.$booking_type_cond."
	UNION ALL
        ...
        function fnc_program_entry_Reply_info() {
        	if (http.readyState == 4) {
                ...
                if ((reponse[0] == 0 || reponse[0] == 1 || reponse[0] == 2)) {
                	...
                	reset_form('programQnty_1', '', '', 'txt_start_date,<? echo $start_date; ?>*txt_end_date,<? echo $end_date;?>*txt_program_date,<? echo $current_date;?>', '', 'hdn_booking_qnty*txt_fabric_dia');
...
if ($action == "planning_info_details")
{
	...
	<div style="width:1100px; max-height:140px; overflow-y:scroll" id="list_container_batch" align="left">
		...
		foreach ($result as $row) {
			...
			?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
				onClick="get_php_form_data('<? echo $row[csf('id')]."_".$expData[1]; ?>','populate_data_from_planning_info', 'planning_info_entry_controller' );balance_cal();">
			...
			<fieldset style="width:630px; margin-top:10px; margin-left:20px">
				<div>
					...
					<div style="width:600px; overflow-y:scroll; max-height:210px;" id="buyer_list_view" align="center">
						...
						if($body_part_type[$body_part_id] == 40 || $body_part_type[$body_part_id] == 50){
							$popupOnOff = 1;
						} else {
							$popupOnOff = 0;
						}
...
if ($action == "color_and_sizewise_info_popup")
{
	...
	<script>
		...
		function func_onkeyup_color_size_qty(rowId)
		{
			...
			var tot_kg=color_size_qty_pcs/per_pcs_qty;
		...
		function fnc_close() 
		{
			...
			$("#tbl_list_search").find('tbody tr').not(":first").each(function () 
			{
				...
				if(txtSizePerKgQty>0 || (coloProgUpdateId !=0 && updateQty > 0))
				{
					if (save_string == "")
					{
						save_string = txtColorId + "_" + txtGreySizeId + "_" + txtFinishSizeId + "_" + txtSizePerKgQty+ "_" + hidden_size_tot_kg+ "_" + coloProgUpdateId+ "_" + txtBodyPartId;
						color_name_string = txtColorName;
						color_id_string = txtColorId;
...
<body> 
	<div align="center" style="width:530px;">
		<form name="searchwofrm" id="searchwofrm">
			<fieldset style="width:530px; margin-top:10px; margin-left:20px">
				<div>
					<table cellspacing="0" cellpadding="0" border="1" rules="all" width="500" class="rpt_table">
						<thead>
							<th width="40">SL</th>
							<th width="60">Grey Size</th>
					...
					<div style="width:500px; overflow-y:scroll; max-height:210px;" id="buyer_list_view" align="center">
						...
						id="tbl_list_search">
							<tbody>
								...
								//$sql = "select b.fabric_color_id, sum(b.grey_fab_qnty) as qnty from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and a.company_id=$companyID and a.item_category=2 and a.booking_no='$booking_no' and $dia_cond and b.po_break_down_id in ($po_id) and b.pre_cost_fabric_cost_dtls_id in ($pre_cost_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.fabric_color_id";
								$sql = "select b.fabric_color_id, c.body_part_id,d.size_number_id,sum(b.grey_fab_qnty) as qnty ,sum(d.order_quantity) as order_quantity 
								from wo_booking_mst a, wo_booking_dtls b ,wo_pre_cost_fabric_cost_dtls c,wo_po_color_size_breakdown d
								where a.booking_no=b.booking_no and b.pre_cost_fabric_cost_dtls_id=c.id
								and  b.color_size_table_id=d.id and c.job_no=d.job_no_mst and c.item_number_id=d.item_number_id and b.po_break_down_id=d.po_break_down_id and a.company_id=$companyID and a.item_category=2 and a.booking_no='$booking_no' and $dia_cond and b.po_break_down_id in ($po_id) and b.pre_cost_fabric_cost_dtls_id in ($pre_cost_id) 
								...
								group by b.fabric_color_id, c.body_part_id,d.size_number_id order by d.size_number_id";
								//echo $sql;
								...
								if($prog_no!="")
								{
									...
									GROUP BY b.id, b.plan_id, b.program_no, b.color_id, b.color_prog_qty";*/
									 ...
									 $size_plan_sql = "select b.id, b.plan_id, b.program_no, b.color_id,c.id as size_tbl_id,c.grey_size_id,c.finish_size_id,c.per_kg,c.kg_wise_total_qnty
									from ppl_planning_entry_plan_dtls a, ppl_color_wise_break_down b , ppl_size_wise_break_down c
									where a.dtls_id = b.program_no and a.mst_id = b.plan_id and b.id=c.color_wise_mst_id and b.program_no=c.program_no and b.plan_id=c.plan_id and b.color_id=c.color_id and a.po_id in(".$po_id.") and b.plan_id =".$plan_id." and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.program_no=$prog_no and b.id=$color_wise_break_down_mst_id
									GROUP BY b.id, b.plan_id, b.program_no, b.color_id,c.id,c.grey_size_id,c.finish_size_id,c.per_kg,c.kg_wise_total_qnty";
									...
									{
										$size_prog_data[$row[csf('plan_id')]][$row[csf('program_no')]][$row[csf('color_id')]][$row[csf('grey_size_id')]][$row[csf('finish_size_id')]]['per_kg'] = $row[csf('per_kg')];
										$size_prog_data[$row[csf('plan_id')]][$row[csf('program_no')]][$row[csf('color_id')]][$row[csf('grey_size_id')]][$row[csf('finish_size_id')]]['kg_wise_total_qnty'] = $row[csf('kg_wise_total_qnty')];
										$size_prog_data[$row[csf('plan_id')]][$row[csf('program_no')]][$row[csf('color_id')]][$row[csf('grey_size_id')]][$row[csf('finish_size_id')]]['size_prog_update_id'] = $row[csf('id')];
									}
								}
								...
								foreach ($result as $row) 
								{
									...
									$per_kg=$size_prog_data[$plan_id][$prog_no][$row[csf('fabric_color_id')]][$row[csf('size_number_id')]][$finshSizeId]['per_kg'];
									$kg_wise_total_qnty=$size_prog_data[$plan_id][$prog_no][$row[csf('fabric_color_id')]][$row[csf('size_number_id')]][$finshSizeId]['kg_wise_total_qnty'];
									$size_prog_update_id=$size_prog_data[$plan_id][$prog_no][$row[csf('fabric_color_id')]][$row[csf('size_number_id')]][$finshSizeId]['size_prog_update_id'];
									...
									<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>" onClick="js_set_value(<? echo $i; ?>)"> 
										...
										<td width="80" align="right">
											...
											<input type="hidden" name="hidden_color_size_qty_pcs[]" id="hidden_color_size_qty_pcs<? echo $i;?>" value="<? echo number_format($row[csf('order_quantity')], 2, '.', ''); ?>"/>
										</td>
										<td width="80" align="center">
											...
											?>" style="max-width: 80px; text-align: center;" placeholder="Write" data-update-qty="<?php echo $text_size_prog_qty; ?>" onKeyUp="func_onkeyup_color_size_qty('<?php echo $i; ?>')"  />
										</td>
										<td align="center">
...
if ($action == "save_update_delete")
{
	...
	if ($operation == 0)
	{
		...
		$field_array_dtls = "id, mst_id, knitting_source, knitting_party, subcontract_party, color_id, color_range, machine_dia, width_dia_type, machine_gg, machine_group, fabric_dia, program_qnty, stitch_length, spandex_stitch_length, draft_ratio,  machine_id, machine_capacity, distribution_qnty, status, start_date, end_date, program_date, feeder, remarks, save_data, no_fo_feeder_data, collar_cuff_data, location_id, advice, grey_dia,attention, co_efficient,batch_no, inserted_by, insert_date";
		$data_array_dtls = "(" . $dtls_id . "," . $id . "," . $cbo_knitting_source . "," . $cbo_knitting_party . "," . $cbo_subcontract_party . "," . $hidden_color_id . "," . $cbo_color_range . "," . $txt_machine_dia . "," . $cbo_dia_width_type . "," . $txt_machine_gg . "," . $cbo_machine_group . "," . $txt_fabric_dia . "," . $txt_program_qnty . "," . $txt_stitch_length . "," . $txt_spandex_stitch_length . "," . $txt_draft_ratio . "," . $machine_id . "," . $txt_machine_capacity . "," . $txt_distribution_qnty . "," . $cbo_knitting_status . "," . $txt_start_date . "," . $txt_end_date . "," . $txt_program_date . "," . $cbo_feeder . "," . $txt_remarks . "," . $save_data . "," . $hidden_no_of_feeder_data . "," . $hidden_bodypartID_data . "," . $cbo_location_name . "," . $hidden_advice_data . "," . $txt_grey_dia . "," . $txt_attention . ",". $txt_co_efficient . ",". $txt_batch_no . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
		...
		if( $hidden_color_wise_prog_data != "" && $hidden_size_wise_prog_string != "") 
		{
			...
			$field_array_size_wise_break_down = "id,color_wise_mst_id, plan_id, program_no, color_id,grey_size_id,finish_size_id,per_kg, kg_wise_total_qnty,body_part_id, inserted_by, insert_date";
				...
				for ($i = 0; $i < count($size_wise_prog_dataz); $i++) 
				{
					if($size_wise_prog_dataz[$i] != '')
					{
						...
						if($size_kg_wise_prog_qty>0)
						{
							...
							$data_array_size_wise_break_down .= "(" . $size_wise_break_down_id . "," . $colorWiseMstId . "," . $id . "," . $dtls_id . ",'" . $color_id . "','" . $gmts_size_id . "','" . $finish_size_id . "','" . $per_kg . "','" . $size_kg_wise_prog_qty . "','" . $size_wise_body_part_id . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
	...
	else if ($operation == 1)   // Update Here
	{
		...
		*/
		...
		if ($knit_qty > 0)
		{
			$field_array_update = "machine_dia*width_dia_type*color_range*machine_gg*machine_group*stitch_length*spandex_stitch_length*draft_ratio*program_qnty*machine_id*machine_capacity*distribution_qnty*status*start_date*end_date*remarks*attention*co_efficient*batch_no*save_data*advice*grey_dia*updated_by*update_date";
			$data_array_update = $txt_machine_dia . "*" . $cbo_dia_width_type . "*" . $cbo_color_range . "*" . $txt_machine_gg . "*" . $cbo_machine_group . "*" . $txt_stitch_length . "*" . $txt_spandex_stitch_length . "*" . $txt_draft_ratio . "*" . $txt_program_qnty . "*" . $machine_id . "*" . $txt_machine_capacity . "*" . $txt_distribution_qnty . "*" . $cbo_knitting_status . "*'" . $txt_start_date . "'*" . $txt_end_date . "*" . $txt_remarks . "*" . $txt_attention . "*" . $txt_co_efficient . "*" . $txt_batch_no . "*" . $save_data . "*" .$hidden_advice_data . "*" . $txt_grey_dia. "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'";
				...
				for ($inc = 0; $inc < count($size_wise_prog_datas); $inc++) 
				{
					...
					for ($i = 0; $i < count($size_wise_prog_dataz); $i++) 
					{
						if($size_wise_prog_dataz[$i] != '')
						{
							...
							{
								$colorWiseMstId=$color_wise_mstID[$color_id];
								$data_array_size_wise_break_down .= "(" . $size_wise_break_down_id . "," . $colorWiseMstId . "," . $updateId . "," . $update_dtls_id . ",'" . $color_id . "','" . $gmts_size_id . "','" . $finish_size_id . "','" . $per_kg . "','" . $size_kg_wise_prog_qty . "','" . $size_wise_body_part_id . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
		...
		else
		{
			...
			$data_array_update = $cbo_knitting_source . "*" . $cbo_knitting_party . "*" . $cbo_subcontract_party . "*" . $hidden_color_id . "*" . $cbo_color_range . "*" . $txt_machine_dia . "*" . $cbo_dia_width_type . "*" . $txt_machine_gg . "*" . $cbo_machine_group . "*" . $txt_fabric_dia . "*" . $txt_program_qnty . "*" . $txt_stitch_length . "*" . $txt_spandex_stitch_length . "*" . $txt_draft_ratio . "*" . $machine_id . "*" . $txt_machine_capacity . "*" . $txt_distribution_qnty . "*" . $cbo_knitting_status . "*'" . $txt_start_date . "'*" . $txt_end_date . "*" . $txt_program_date . "*" . $cbo_feeder . "*" . $txt_remarks . "*" . $txt_attention ."*". $txt_co_efficient ."*" . $txt_batch_no ."*" .$save_data . "*" . $hidden_no_of_feeder_data . "*" . $hidden_collarCuff_data . "*" . $cbo_location_name . "*" . $hidden_advice_data . "*" . $txt_grey_dia . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'";
