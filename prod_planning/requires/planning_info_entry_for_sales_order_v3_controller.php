...
if ($action == "booking_item_details_popup")
{
	...
	function show_details(type) {
		...
		freeze_window(5);
		http.open("POST", "planning_info_entry_for_sales_order_v3_controller.php", true);
		http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		http.send(data);
		http.onreadystatechange = fn_show_details_reponse;
		//show_list_view(cbo_company_id, 'booking_item_details', 'list_container_fabric_desc', 'requires/planning_info_entry_for_sales_order_v3_controller', '');
	}
	...
	function openmypage_job() {
		...
		var page_link = 'planning_info_entry_for_sales_order_v3_controller.php?action=style_ref_search_popup&companyID=' + companyID + '&buyerID=' + buyerID;
	...
	function openmypage_internal_ref() {
		...
		var companyID = $("#cbo_company_name").val();
		var buyerID = $("#cbo_buyer_name").val();
		var page_link = 'planning_info_entry_for_sales_order_v3_controller.php?action=internal_ref_search_popup&companyID=' + companyID + '&buyerID=' + buyerID;
	...
	function openmypage_booking() {
		...
		var companyID = $("#cbo_company_name").val();
		var page_link = 'planning_info_entry_for_sales_order_v3_controller.php?action=booking_no_search_popup&companyID=' + companyID;
	...
	function openmypage_prog() {
		...
		var page_link = 'planning_info_entry_for_sales_order_v3_controller.php?action=prog_qnty_popup&gsm=' + gsm + '&dia=' + dia + '&desc=' + desc + '&within_group=' + within_group + '&job_id=' + job_id + '&booking_qnty=' + encodeURIComponent(String(booking_no)) + '&companyID=' + companyID + '&data="' + data + '"' + '&plan_id=' + plan_id + '&determination_id=' + determination_id + '&booking_no=' + booking_no + '&body_part_id=' + body_part_id + '&fabric_type=' + fabric_typee + '&pre_cost_id=' + pre_cost_id + '&buyer_id=' + buyer_id + '&job_dtls_id=' + job_dtls_id + '&color_type_id=' + color_type_id + '&sales_order_dtls_id=' + sales_order_dtls_id;
 ...
 function selected_row(rowNo) {
	...
	function delete_prog() {
		...
		http.open("POST", "requires/planning_info_entry_for_sales_order_v3_controller.php", true);
		http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		http.send(data);
	...
	function active_inactive() {
		...
		if (company_id == 0) {
			...
			load_drop_down('planning_info_entry_for_sales_order_v3_controller', company_id + '_' + within_group, 'load_drop_down_buyer', 'buyer_td');
		}
	}
...
if ($action == "booking_item_details") 
{
	...
	else
	{
		...
		else
		{
			...
			{
				//$sales_order_dtls_id="listagg(b.id, ',') within group (order by b.id) as sales_order_dtls_id"; color_range_id
				$sales_order_dtls_id = "RTRIM(XMLAGG(XMLELEMENT(e,b.id,',').EXTRACT('//text()') ORDER BY b.id).GETCLOBVAL(),',') AS sales_order_dtls_id";
			}
			
			$sql = "SELECT a.id, a.company_id, a.within_group, a.job_no, a.sales_booking_no, a.booking_id, a.buyer_id, a.style_ref_no, a.booking_date, b.body_part_id, b.color_type_id, b.fabric_desc, b.determination_id, b.gsm_weight, upper(b.dia) as dia, b.width_dia_type, sum(b.finish_qty) finish_qty, sum(b.grey_qty) grey_qty,b.color_range_id,(select c.po_break_down_id,a.booking_without_order from wo_booking_mst c where a.sales_booking_no = c.booking_no) po_break_down_id,a.is_apply_last_update,a.is_master_part_updated,$sales_order_dtls_id,b.pre_cost_fabric_cost_dtls_id, a.po_job_no from fabric_sales_order_mst a,fabric_sales_order_dtls b,ppl_planning_entry_plan_dtls d where a.id=b.mst_id and a.id=d.po_id $active_status_sql and a.company_id=$company_name $booking_cond $job_no_cond $buyer_id_cond_to $within_group_cond $date_cond $barcode_cond $booking_nos_internal_ref_cond group by a.id, a.company_id, a.within_group, a.job_no, a.sales_booking_no, a.booking_id, a.buyer_id, a.style_ref_no, a.booking_date, b.body_part_id, b.color_type_id, b.fabric_desc, b.determination_id, b.gsm_weight, b.dia, b.width_dia_type,a.is_apply_last_update,a.is_master_part_updated,b.pre_cost_fabric_cost_dtls_id, a.po_job_no,a.booking_without_order,b.color_range_id order by b.dia";
		}
		//echo $sql;
	}
	//echo $sql;
	...
	else
	{
		...
		from ppl_planning_info_entry_mst a,ppl_planning_entry_plan_dtls b, ppl_planning_info_entry_dtls c
		where a.id=b.mst_id and b.dtls_id=c.id and b.mst_id=c.mst_id and b.is_sales=1 and b.is_revised=0  $sales_booking_cond
		group by b.id,b.mst_id,b.booking_no, b.po_id, b.yarn_desc, b.body_part_id, b.fabric_desc, b.gsm_weight, b.dia,b.width_dia_type, b.color_type_id,b.sales_order_dtls_ids,b.pre_cost_fabric_cost_dtls_id,b.status_active ,a.determination_id,c.color_range ";
	...
	foreach ($res_plan as $rowPlan)
	{
		...
		$program_data_array[$rowPlan[csf('booking_no')]][$rowPlan[csf('po_id')]][$rowPlan[csf('body_part_id')]][$rowPlan[csf('determination_id')]][$rowPlan[csf('gsm_weight')]][$rowPlan[csf('dia')]][$rowPlan[csf('width_dia_type')]][$rowPlan[csf('color_type_id')]][$rowPlan[csf('pre_cost_fabric_cost_dtls_id')]][$rowPlan[csf('status_active')]][$rowPlan[csf('color_range')]]['mst_id'] = $rowPlan[csf('mst_id')];

		$program_data_array[$rowPlan[csf('booking_no')]][$rowPlan[csf('po_id')]][$rowPlan[csf('body_part_id')]][$rowPlan[csf('determination_id')]][$rowPlan[csf('gsm_weight')]][$rowPlan[csf('dia')]][$rowPlan[csf('width_dia_type')]][$rowPlan[csf('color_type_id')]][$rowPlan[csf('pre_cost_fabric_cost_dtls_id')]][$rowPlan[csf('status_active')]][$rowPlan[csf('color_range')]]['prog_no'][] = $rowPlan[csf('prog_no')];
		$program_data_array[$rowPlan[csf('booking_no')]][$rowPlan[csf('po_id')]][$rowPlan[csf('body_part_id')]][$rowPlan[csf('determination_id')]][$rowPlan[csf('gsm_weight')]][$rowPlan[csf('dia')]][$rowPlan[csf('width_dia_type')]][$rowPlan[csf('color_type_id')]][$rowPlan[csf('pre_cost_fabric_cost_dtls_id')]][$rowPlan[csf('status_active')]][$rowPlan[csf('color_range')]]['program_qnty'] += $rowPlan[csf('program_qnty')];
		$program_data_array[$rowPlan[csf('booking_no')]][$rowPlan[csf('po_id')]][$rowPlan[csf('body_part_id')]][$rowPlan[csf('determination_id')]][$rowPlan[csf('gsm_weight')]][$rowPlan[csf('dia')]][$rowPlan[csf('width_dia_type')]][$rowPlan[csf('color_type_id')]][$rowPlan[csf('color_range')]]['sales_order_dtls_ids'] = $rowPlan[csf('sales_order_dtls_ids')];
	...
	if ($type == 1)
	{
		...
		//for grey qty
		$sqlGreyQty = " SELECT a.id, a.sales_booking_no, b.body_part_id, b.color_type_id, b.fabric_desc, b.determination_id, b.gsm_weight, upper(b.dia) as dia, b.width_dia_type,b.pre_cost_fabric_cost_dtls_id, sum(b.grey_qty) as grey_qty,b.color_range_id from fabric_sales_order_mst a, fabric_sales_order_dtls b where a.id=b.mst_id $active_status_sql and a.company_id=$company_name ".where_con_using_array($salesMstIdArr,0,'a.id')." ".where_con_using_array($salesDtlsIdArr,0,'b.id')." 
		group by a.id, a.company_id, a.within_group, a.job_no, a.sales_booking_no, a.booking_id, a.buyer_id, a.style_ref_no, a.booking_date, b.body_part_id, b.color_type_id, b.fabric_desc, b.determination_id, b.gsm_weight, b.dia, b.width_dia_type,b.pre_cost_fabric_cost_dtls_id,b.color_range_id";
		//echo $sqlGreyQty;
		...
		foreach($sqlGreyQtyRslt as $row)
		{
			...
			$greyQtyData[$row[csf('sales_booking_no')]][$row[csf('id')]][$row[csf('body_part_id')]][$row[csf('determination_id')]][$row[csf('gsm_weight')]][$row[csf('dia')]][$row[csf('width_dia_type')]][$row[csf('color_type_id')]][$row[csf('pre_cost_fabric_cost_dtls_id')]][$row[csf('color_range_id')]]['grey_qty'] += $row[csf('grey_qty')];
		}
		//echo "<pre>";
				...
				<table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" id="tbl_list_search">
					...
					<tbody>
						...
						foreach ($nameArray as $row)
						{
							...
							$grey_qty = $greyQtyData[$sales_booking_no][$sales_id][$body_part_id][$determination_id][$gsm][$dia][$width_dia_type][$color_type_id][$pre_cost_fabric_cost_dtls_id][$color_range_id]['grey_qty'];
							...
							if($within_group == 1)
							{
								...
								$program_qnty = $program_data_array[$sales_booking_no][$sales_id][$body_part_id][$determination_id][$gsm][$dia][$width_dia_type][$color_type_id][$pre_cost_fabric_cost_dtls_id][$status][$color_range_id]['program_qnty'];
								$plan_id = $program_data_array[$sales_booking_no][$sales_id][$body_part_id][$determination_id][$gsm][$dia][$width_dia_type][$color_type_id][$pre_cost_fabric_cost_dtls_id][$status][$color_range_id]['mst_id'];
								$prog_no = $program_data_array[$sales_booking_no][$sales_id][$body_part_id][$determination_id][$gsm][$dia][$width_dia_type][$color_type_id][$pre_cost_fabric_cost_dtls_id][$status][$color_range_id]['prog_no'];
								...
								if (!in_array($dia, $dia_array)) {
									...
									if($type == 3){
										...
										<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer; <? echo $hidden; ?>" >
											...
											<td width="100" align='center'><p><? echo $buyer_arr[$row[csf('customer_buyer')]]; ?></p></td>
											...
											<input type="hidden" name="color_range_id[]" id="color_range_id_<? echo $i; ?>"
											value="<? echo $color_range_id; ?>"/>
							...
							<tfoot>
								<th colspan="17" align="right">Grand Total<input type="hidden" name="company_id" id="company_id" value="<? echo $company_name; ?>"/></th>
...
if ($action == "prog_qnty_popup") 
{
	...
	<script>
		...
		{
			if (form_validation('cbo_knitting_source*cbo_knitting_party*cbo_color_range', 'Knitting Source*Knitting Party*Color Range') == false)
			{
				return;
			}
		...
		function fnc_program_entry_Reply_info()
		{
			if (http.readyState == 4)
			{
				...
				if ((reponse[0] == 0 || reponse[0] == 1 || reponse[0] == 2))
				{
					...
					show_list_view(reponse[1], 'planning_info_details', 'list_view', 'planning_info_entry_for_sales_order_v3_controller', '');
		...
		function openpage_feeder() {
			...
			var page_link = 'planning_info_entry_for_sales_order_v3_controller.php?action=feeder_info_popup&no_of_feeder_data=' + no_of_feeder_data + '&pre_cost_id=' + '<? echo $pre_cost_id; ?>'+'&hidden_color_id='+hidden_color_id+ '&sales_order_dtls_id=' + '<? echo $sales_order_dtls_id; ?>'+ '&within_group=' + '<? echo $within_group; ?>'+ '&job_id=' + '<? echo $job_id; ?>'+'&hidden_sales_dia='+hidden_sales_dia;     
		...
		function openpage_collarCuff() {
			...
			var page_link = 'planning_info_entry_for_sales_order_v3_controller.php?action=collarCuff_info_popup&collarCuff_data=' + collarCuff_data + '&pre_cost_id=' + '<? echo $pre_cost_id; ?>&update_dtls_id=' + update_dtls_id +'&body_part_id='+'<? echo $body_part_id; ?>&booking_no=' + '<? echo $booking_no; ?>&bodyPartType=' + bodyPartType;
		...
		function openpage_cam_design() {
			...
			var page_link = 'planning_info_entry_for_sales_order_v3_controller.php?action=cam_design_info_popup&hidden_came_dsign_string_data=' + came_dsign_string_data + '&update_dtls_id='+updateDtlsId;
		...
		function openpage_advice() {
			...
			var page_link = 'planning_info_entry_for_sales_order_v3_controller.php?action=advice_info_popup&hidden_advice_data=' + hidden_advice_data;
		...
		function openpage_count_feeding()
		{
			...
			var page_link = 'planning_info_entry_for_sales_order_v3_controller.php?action=count_feeding_data_popup&count_feeding_data=' + count_feeding_data + '&pre_cost_id=' + '<? echo $pre_cost_id; ?>&update_dtls_id=' + update_dtls_id + '&job_id=' + '<? echo $job_id; ?>';
		...
		<fieldset style="width:900px; margin-top:5px;">
			...
			<table width="900" align="center" border="0">
				<tr>
					...
					<td>
						<?
						echo create_drop_down("cbo_knitting_source", 152, $knitting_source, "", 1, "-- Select --", 1, "active_inactive();load_drop_down( 'planning_info_entry_for_sales_order_v3_controller', this.value+'**'+$companyID, 'load_drop_down_knitting_party','knitting_party');", 0, '1,3');
						?>
					</td>
					...
					<td id="knitting_party">
						...
						//load_drop_down( 'planning_info_entry_for_sales_order_v3_controller', this.value, 'load_drop_down_location','location_td');
						?>
					</td>
					<td>Color</td>
					<td>
				...
				<tr>
					...
					<td>
						<?
						echo create_drop_down("cbo_color_range", 152, $color_range, "", 1, "-- Select --", $color_range_id, "",1);
						?>
					</td>
			...
			<div id="list_view" style="margin-top:5px">
				...
				if (str_replace("'", '', $plan_id) != "") {
					?>
					<script>
						show_list_view('<? echo str_replace("'", '', $plan_id); ?>', 'planning_info_details', 'list_view', 'planning_info_entry_for_sales_order_v3_controller', '');
					</script>
					<?
				}
				?>
...
<script type="text/javascript">
	...
	load_drop_down( 'planning_info_entry_for_sales_order_v3_controller', 1+'**'+ <?php echo $companyID; ?>,'load_drop_down_knitting_party','knitting_party');
	load_drop_down( 'planning_info_entry_for_sales_order_v3_controller',  $('#cbo_knitting_party').val(),'load_drop_down_location','location_td');
	...
	<script>
		...
		function check_all_data()
		{
			...
			for (var i = 1; i <= tbl_row_count; i++)
			{
				...
				var progQnty = $('#txt_individual_color_blqty'+ i).val()*1;
				$('#text_color_prog_qty_'+ i).val(progQnty); 
				totalQnty+=progQnty;
			}
			$('#totProgQty').val(totalQnty); 
		}
				...
				<div>
					...
					<div style="width:700px; overflow-y:scroll; max-height:210px;" id="buyer_list_view" align="center">
						...
						<tbody>
							...
							if($plan_id!="")
							{
								$plan_sql = "SELECT b.id AS ID, b.plan_id AS PLAN_ID, b.program_no AS PROGRAM_NO, b.color_id AS COLOR_ID, b.color_prog_qty COLOR_PROG_QTY,b.size_wise_prog_string as SIZE_WISE_PROG_STRING FROM ppl_planning_info_entry_dtls c,ppl_planning_entry_plan_dtls a, ppl_color_wise_break_down b WHERE c.id=a.dtls_id and c.mst_id=a.mst_id and a.dtls_id = b.program_no AND a.mst_id = b.plan_id AND a.po_id IN(".$job_id.") AND c.color_range IN(".$color_range_id.") AND b.plan_id = ".$plan_id." AND b.status_active=1 AND b.is_deleted=0 AND c.status_active=1 AND c.is_deleted=0 GROUP BY b.id, b.plan_id, b.program_no, b.color_id, b.color_prog_qty,b.size_wise_prog_string";
							...
							$dia=strtoupper($dia);
							$sql_colorRange = "SELECT color_id AS COLOR_ID, COLOR_RANGE_ID FROM fabric_sales_order_dtls WHERE status_active=1 AND is_deleted=0 AND mst_id=".$job_id." AND id IN(".$sales_dtls_id.") AND body_part_id IN(".$bodypartId.") AND trim(fabric_desc) = '".$desc."' AND upper(dia) = '".$dia."' AND gsm_weight = '".$gsm."' AND width_dia_type = '".$width_dia_type."' AND color_range_id IN(".$color_range_id.")  GROUP BY color_id,color_range_id";
							...
							$sql = "SELECT color_id AS COLOR_ID, sum(grey_qty) AS QTY FROM fabric_sales_order_dtls WHERE status_active=1 AND is_deleted=0 AND mst_id=".$job_id." AND id IN(".$sales_dtls_id.") AND body_part_id IN(".$bodypartId.") AND trim(fabric_desc) = '".$desc."' AND upper(dia) = '".$dia."' AND gsm_weight = '".$gsm."' AND width_dia_type = '".$width_dia_type."' AND color_range_id IN(".$color_range_id.")  GROUP BY color_id";
							//echo $sql;
							$result = sql_select($sql);
						...
						id="tbl_list_search">
							<tbody>
								...
								else
								{
									...
									sum(plan_cut_qnty) as plan_cut_qnty,b.colar_cuff_per,e.body_part_type,a.colar_excess_percent, a.cuff_excess_percent 
									from wo_booking_mst a, wo_booking_dtls b ,wo_pre_cost_fabric_cost_dtls c,wo_po_color_size_breakdown d,lib_body_part e 
									where a.booking_no=b.booking_no and b.pre_cost_fabric_cost_dtls_id=c.id
									...
									group by b.fabric_color_id, c.body_part_id,d.size_number_id,e.body_part_type,b.colar_cuff_per,a.colar_excess_percent, a.cuff_excess_percent
									) x group by x.fabric_color_id, x.body_part_id,x.size_number_id,x.qnty ,x.order_quantity,x.plan_cut_qnty,x.colar_cuff_per,x.body_part_type,x.colar_excess_percent, x.cuff_excess_percent order by x.size_number_id";
	...
	<div style="width:1110px; max-height:140px; overflow-y:scroll" id="list_container_batch" align="left">
		...
		foreach ($result as $row) {
			...
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
				onClick="get_php_form_data('<? echo $row[csf('id')]."**".$row[csf('salesDtlsId_wise_bodyPartId')]; ?>','populate_data_from_planning_info', 'planning_info_entry_for_sales_order_v3_controller' );balance_cal();">
...
if ($action == "populate_data_from_planning_info")
{
	...
	foreach ($data_array as $row) 
	{
		...
		echo "load_drop_down('planning_info_entry_for_sales_order_v3_controller', " . $row[csf("knitting_source")] . "+'**'+" . $row[csf("knitting_party")] . "+'**1', 'load_drop_down_knitting_party','knitting_party');\n";
		...
		echo "load_drop_down('planning_info_entry_for_sales_order_v3_controller', " . $row[csf("knitting_party")] . ", 'load_drop_down_location','location_td');\n";
...
if ($action == "count_feeding_data_popup")
{
	...
	<script>
		...
		function browse_desc_popup(sl,salesId) {
			//var companyID = $("#cbo_company_name").val();
			//var buyerID = $("#cbo_buyer_name").val();
			var page_link = 'planning_info_entry_for_sales_order_v3_controller.php?action=browse_desc_popup_action&sl=' + sl + '&salesId=' + salesId;
	...
	<script>
		...
		function fn_add_date_field(row_no) {
			...
			if ($('#txt_startDate_' + row_no).val() != "") {
				...
				var response = return_global_ajax_value(data, 'date_duplication_check', '', 'planning_info_entry_for_sales_order_v3_controller');
		...
		function fn_machine_book_dates(row_no)
		{
			...
			if(machine_id!="" && machine_id!="undefined")
			{
				...
				$.ajax({
					type: "POST",
					url: "planning_info_entry_for_sales_order_v3_controller.php?action=machine_allready_book_dates",
					data: data,
					cache: false,
					dataType: "json",
				...
				<tbody>
					<tr>
						...
						<td align="center">
							...
							onClick="show_list_view ('<? echo $companyID; ?>**' +'<? echo $buyerID; ?>'+'**'+document.getElementById('cbo_po_buyer_name').value + '**'+'<? echo $within_group; ?>**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**', 'create_job_search_list_view', 'search_div', 'planning_info_entry_for_sales_order_v3_controller', 'setFilterGrid(\'tbl_list_search\',-1)');"
							style="width:90px;"/>
						</td>
					</tr>
				</tbody>
			...
			<fieldset style="width:100%;">
				...
				<table cellpadding="0" cellspacing="0" width="835" class="rpt_table" border="1" rules="all">
					...
					<tr>
						...
						<td align="center">
							<input type="button" name="button2" class="formbutton" value="Show"
							onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('cbo_po_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_within_group').value+'_'+document.getElementById('cbo_booking_type').value, 'create_booking_search_list_view', 'search_div', 'planning_info_entry_for_sales_order_v3_controller', 'setFilterGrid(\'tbl_list_search\',-1);')"
							style="width:90px;"/>
						</td>
					</tr>
					<tr>
...
if ($action == "load_drop_down_knitting_party")
{
	$data = explode("**", $data);
	if ($data[0] == 1) {
		echo create_drop_down("cbo_knitting_party", 177, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "--Select Knit Party--", $data[1], "load_drop_down( 'planning_info_entry_for_sales_order_v3_controller', this.value, 'load_drop_down_location','location_td');", "");
	} else if ($data[0] == 3) {
		if ($data[2] == 1) $selected_id = $data[1]; else $selected_id = 0;
		echo create_drop_down("cbo_knitting_party", 177, "select a.id, a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=20 and a.status_active=1 and a.is_deleted=0 group by a.id, a.supplier_name order by a.supplier_name", "id,supplier_name", 1, "--Select Knit Party--", $selected_id, "load_drop_down( 'planning_info_entry_for_sales_order_v3_controller', this.value, 'load_drop_down_location','location_td');");
	} else {
		echo create_drop_down("cbo_knitting_party", 177, $blank_array, "", 1, "--Select Knit Party--", 0, "load_drop_down( 'planning_info_entry_for_sales_order_v3_controller', this.value, 'load_drop_down_location','location_td');");
	}
	exit();
}
