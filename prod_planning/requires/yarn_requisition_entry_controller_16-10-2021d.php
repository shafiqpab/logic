				...
				<tbody>
					<tr>
						...
						<td align="center">
							...
							onClick="show_list_view ('<? echo $companyID; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value, 'create_order_no_search_list_view', 'search_div', 'yarn_requisition_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1)');"
							style="width:100px;"/>
						</td>
					</tr>
					<tr>
...
if ($action == "create_booking_no_search_list_view")
{
	...
	if ($data[1] == 0) {
		if ($_SESSION['logic_erp']["data_level_secured"] == 1) {
			if ($_SESSION['logic_erp']["buyer_id"] >0) $buyer_id_cond = " and buyer_id in (" . $_SESSION['logic_erp']["buyer_id"] . ")"; else $buyer_id_cond = "";
		} else {
			$buyer_id_cond = "";
		}
	} else {
	...
	if (trim($data[2]) == "") $booking_cond = ""; else $booking_cond = "and booking_no_prefix_num='$data[2]'";
	...
	//defined Later
	$sql = "select id, is_short, booking_no_prefix_num, booking_no, $year_field buyer_id, job_no, booking_date from wo_booking_mst where company_id='$company_id' and fabric_source=1 and status_active=1 and is_deleted=0 $buyer_id_cond $booking_cond $date_cond order by id DESC";
	//$sql= "select b.id, $year_field a.job_no, a.job_no_prefix_num, a.company_name, a.buyer_name, a.style_ref_no, b.po_number, b.pub_shipment_date from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.company_name=$company_id and $search_field like '$search_string' $buyer_id_cond $date_cond order by b.id, b.pub_shipment_date";
...
if ($action == "report_generate")
{
	...
	if ($file_no == "") $file_no_cond = ""; else $file_no_cond = " and file_no='" . trim($file_no) . "' ";
	if ($internal_ref == "") $internal_ref_cond = ""; else $internal_ref_cond = " and grouping='" . trim($internal_ref) . "' ";
	if ($booking_no == "") $booking_no_cond = ""; else $booking_no_cond = " and a.booking_no='" . trim($booking_no) . "' ";
	...
	if ($booking_no_all == "") $po_to_booking_no_cond = $po_id_cond; else $po_to_booking_no_cond = " and a.booking_no in(" . trim($booking_no_all) . ") ";
	//echo $po_id_cond.'55A';
	...
	<fieldset style="width:2200px; margin-top:10px">
		...
		<div style="width:2180px; overflow-y:scroll; max-height:330px;" id="buyer_list_view" align="center">
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="2162" class="rpt_table"
			id="tbl_list_search">
			<tbody>
				<?
				$i = 1;
						...
						if (!in_array($machine_dia_gg, $machine_dia_gg_array)) {
							if ($k != 1) {
								...
								<tr bgcolor="<? echo $bgcolor; ?>"
									...
									<td width="140"><p><? echo $po_no; ?></p></td>
									<td width="80"><p><? echo $plan_details_array[$row[csf('id')]]; ?></p></td>
									<td width="110"><p><? echo $style_ref; ?></p></td>
									<td width="100"><p><? echo implode(",", array_unique(explode(",", $int_ref))); ?></p></td>
									<td width="100"><p><? echo implode(",", array_unique(explode(",", $file_no))); ?></p></td>
									...
									<td align="right" width="105">
										...
										onClick="openmypage_yarnReq(<? echo $i; ?>,'<? echo $row[csf('id')]; ?>',<? echo $company_name; ?>,'<? echo $comps; ?>','<? echo $job_no; ?>','<? echo $reqs_no; ?>','<? echo $row[csf('booking_no')]; ?>','<? echo $plan_details_array[$row[csf('id')]]; ?>','<? echo $row[csf('program_qnty')]; ?>')"/>
									</td>
									<td align="center"
...
<?php
	...
	if($page=='planning_info_entry') {
		echo signature_table(208, $company_id, '850px');
	}
	else {
		echo signature_table(209, $company_id, '850px');
	}
?>
	...
	<script>
		...
		function fnc_yarn_req_entry(operation)
		{
			...
			else
			{
				...
				var data = "action=save_update_delete&operation=" + operation + "&is_auto_allocation_from_requisition="+is_auto_allocation_from_requisition+"&qnty_breck_down="+qnty_breck_down+ "&txt_old_qnty="+txt_old_qnty+  get_submitted_data_string('prod_id*txt_no_of_cone*txt_reqs_date*txt_yarn_qnty*updateId*update_dtls_id*txt_requisition_no*is_dyed_yarn*companyID*booking_no*txt_job_no*original_prod_id*txt_po_ids*is_dist_qnty_valiable_set*pre_qnty_breck_down*dyed_yarn_qnty_from*hiddenRequisitionString', "../../");
			}
		...
		function func_onclick_qty()
		{
			...
			//page_link = page_link + '&txt_order_id=' + txt_order_id + '&txt_booking_qnty=' + txt_booking_qnty + '&txt_job_no=' + txt_job_no + '&txt_booking_no=' + txt_booking_no + '&txt_company_id='+ txt_company_id+ '&available_qnty=' + available_qnty + '&txt_qnty=' + txt_qnty +'&txt_old_qnty=' + txt_old_qnty +'&qnty_breck_down=' + qnty_breck_down + '&txt_item='+txt_item + '&prod_id='+prod_id + '&is_dyed_yarn='+is_dyed_yarn + '&program_no='+program_no;
			page_link = page_link + '&prod_id=' + prod_id + '&order_id=' + order_id + '&program_no=' + program_no + '&program_qty=' + program_qty + '&yarn_qty=' + yarn_qty + '&requisition_no=' + requisition_no;
			emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=920px,height=350px,center=1,resize=0,scrolling=0', '../')
			emailwindow.onclose = function ()
			{
				var theform = this.contentDoc.forms[0];
		...
		<table width="800" align="center" border="0">
			...
			<tr>
				...
				<td>
					...
					{
						// onKeyUp="func_onkeyup_qty()"
						?>
						<input type="text" name="txt_yarn_qnty" id="txt_yarn_qnty" class="text_boxes_numeric" style="width:130px;" onClick="func_onclick_qty()"/>						
						<?
					}
					?>
						...
						<div style="width:100%; overflow-y:scroll; max-height:140px;" id="scroll_body" align="left">
							<table class="rpt_table" rules="all" border="1" width="810" id="tbl_list_search_dyied">
								<?
									...
									$sql_requs = "select c.prod_id, sum(c.yarn_qnty) as yarn_qnty from ppl_planning_info_entry_dtls a, ppl_planning_info_entry_mst b, ppl_yarn_requisition_entry c where b.id=a.mst_id and a.id=c.knit_id and a.id in ($all_knit_id) and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.booking_no = '".$selected_booking_no."' group by c.prod_id";
									//$product_id = "";
									...
									$i = 1;
									$bal_alloc_qnty = 0;
									$sql_dyied_yarn_sql = "select a.job_no,a.po_break_down_id,a.item_id,sum(a.qnty) as qnty,sum(c.allocated_qnty) allo_qty,b.company_name,b.buyer_name,b.location_name,c.id prod_id,c.lot,c.yarn_count_id, c.yarn_type, c.yarn_comp_percent1st,c.yarn_comp_percent2nd,c.yarn_comp_type1st,c.yarn_comp_type2nd,c.color, c.supplier_id, c.is_within_group from inv_material_allocation_mst a,wo_po_details_master b,product_details_master c where a.job_no=b.job_no and a.booking_no = '".$selected_booking_no."' and a.job_no='$job_no' and a.item_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.is_dyied_yarn=1 and a.booking_without_order = 0 group by a.job_no,a.po_break_down_id,a.item_id, b.company_name,b.buyer_name,b.location_name,c.id,c.lot,c.yarn_count_id,c.yarn_type, c.yarn_comp_percent1st,c.yarn_comp_percent2nd,c.yarn_comp_type1st,c.yarn_comp_type2nd,c.color,c.supplier_id, c.is_within_group";
...
if ($action == "actn_onclick_qty")
{
	echo load_html_head_contents("Item List", "../../", 1, 1, '', '', '');
	extract($_REQUEST);
	//echo $prod_id."=".$order_id."=".$program_no."=".$program_qty."=".$yarn_qty."=".$requisition_no;
	?>
	<script>
		...
		function func_onkeyup_total_requisition_qty()
		{
			...
			else
			{
				for(i; i < dataArr.length; i++)
				{
					...
					var orderQty = data[0];
					var programQty = data[1];
					$('#textRequisitionQty_'+sl).removeAttr('readonly').val(orderQty);
					*/
...
<body>
	<?php
	//for program qty
	$sqlProgram = "SELECT dtls_id, po_id, program_qnty, booking_no FROM ppl_planning_entry_plan_dtls WHERE dtls_id = ".$program_no." AND status_active = 1 AND is_deleted=0";
	...
	foreach($sqlProgramResultSet as $row)
	{
		...
		//$dataArrProgram[$row[csf('po_id')]]['program_qnty'] += number_format($row[csf('program_qnty')], 2, '.', '');
		$dataArrProgram[$row[csf('po_id')]]['program_qnty'] += number_format($row[csf('program_qnty')], 2, '.', '');
	}
	//echo "<pre>";
	...
	//for allocation qty
	$sqlAllocation = "SELECT po_break_down_id, item_id, qnty FROM inv_material_allocation_dtls WHERE item_id = ".$prod_id." AND po_break_down_id IN (".implode(',', $poIdArr).") AND booking_no IN('".implode("','", $bookingNoArr)."') AND status_active = 1 AND is_deleted=0";
	$sqlAllocationResultSet = sql_select($sqlAllocation);
	$dataArrAllocation = array();
	$itemIdArr = array();
	foreach($sqlAllocationResultSet as $row)
	{
		$itemIdArr[$row[csf('item_id')]] = $row[csf('item_id')];
		$dataArrAllocation[$row[csf('po_break_down_id')]][$row[csf('item_id')]]['allocation_qnty'] += $row[csf('qnty')];
		$dataArrAllocation[$row[csf('po_break_down_id')]][$row[csf('item_id')]]['item_id'] = $row[csf('item_id')];
	}
	//echo "<pre>";
	//print_r($dataArrAllocation); die;
	
	//for requisition qty
	//$sql = "SELECT id, requisition_id, program_id, order_id, item_id, order_requisition_qty, requisition_qty, distribution_method, status_active, is_deleted FROM ppl_yarn_requisition_breakdown WHERE order_id IN (".implode(',', $poIdArr).") AND item_id IN(".implode(',', $itemIdArr).") AND booking_no IN('".implode("','", $bookingNoArr)."') AND status_active = 1 AND is_deleted=0";
	
	$sql = "SELECT a.id, a.requisition_id, a.program_id, a.order_id, a.item_id, a.order_requisition_qty,a.requisition_qty, a.distribution_method FROM ppl_yarn_requisition_breakdown a, PPL_PLANNING_ENTRY_PLAN_DTLS b WHERE  a.program_id=b.dtls_id AND a.order_id IN (".implode(',', $poIdArr).") AND a.item_id IN(".implode(',', $itemIdArr).") AND a.booking_no IN('".implode("','", $bookingNoArr)."') AND a.status_active = 1 AND a.is_deleted=0 AND b.status_active = 1 AND b.is_deleted=0 GROUP BY a.id, a.requisition_id, a.program_id, a.order_id, a.item_id, a.order_requisition_qty,a.requisition_qty, a.distribution_method, a.status_active, a.is_deleted";
	//echo $sql;
	$sqlRslt = sql_select($sql);
	$dataReqArr = array();
	$dataArrRequisition = array();
	foreach($sqlRslt as $row)
	{
		$dataReqArr[$row[csf('requisition_id')]][$row[csf('order_id')]][$row[csf('item_id')]]['distribution_method'] = $row[csf('distribution_method')];
		$dataReqArr[$row[csf('requisition_id')]][$row[csf('order_id')]][$row[csf('item_id')]]['order_requisition_qty'] += $row[csf('order_requisition_qty')];
		$dataArrRequisition[$row[csf('order_id')]][$row[csf('item_id')]]['requisition_qnty'] += $row[csf('order_requisition_qty')];
	}
	//echo "<pre>";
	//print_r($dataArrRequisition); die;

	/*
	$sqlRequisition = "SELECT distribution_qnty_breakdown, yarn_qnty FROM ppl_yarn_requisition_entry WHERE knit_id = ".$program_no." AND status_active = 1 AND is_deleted=0";
	$sqlRequisitionResultSet = sql_select($sqlRequisition);
	foreach($sqlRequisitionResultSet as $row)
	{
		$distributionBreakdown = rtrim($row[csf('distribution_qnty_breakdown')], ",");
		$expDistributionBreakdown = explode(',', $distributionBreakdown);
		$s = 0;
		for($s; $s < count($expDistributionBreakdown); $s++)
		{
			$expData = explode('_', $expDistributionBreakdown[$s]);
			$dataArrRequisition[$expData[1]]['requisition_qnty'] += $expData[3];
		}
	}
	*/
	//echo "<pre>";
	//print_r($dataArrRequisition);
	...
	$isError = 0;	
	$data = array();
	foreach($dataArrProgram as $order_id=>$row)
	{
		//for allocation calculation
		foreach($dataArrAllocation[$order_id] as $itemId=>$itemArr)
		{
			$data[$itemId][$order_id]['program_qnty'] = $row['program_qnty'];
			$data[$itemId][$order_id]['allocation_qnty'] = $itemArr['allocation_qnty'];
		}
		
		//for requisition calculation
		if(!empty($dataReqArr[$requisition_no][$order_id]))
		{
			//update code gose here
			foreach($dataArrRequisition[$order_id] as $itemId=>$itemArr)
			{
				$distribution_method = $dataReqArr[$requisition_no][$order_id][$itemId]['distribution_method'];
				$data[$itemId][$order_id]['previous_requisition_qnty'] = $itemArr['requisition_qnty']-$dataReqArr[$requisition_no][$order_id][$itemId]['order_requisition_qty'];
				$data[$itemId][$order_id]['allocation_balance_qnty'] = $data[$itemId][$order_id]['allocation_qnty']-($data[$itemId][$order_id]['previous_requisition_qnty']+$dataReqArr[$requisition_no][$order_id][$itemId]['order_requisition_qty']);
				$data[$itemId][$order_id]['hdn_allocation_balance_qnty'] = $data[$itemId][$order_id]['allocation_balance_qnty']+$dataReqArr[$requisition_no][$order_id][$itemId]['order_requisition_qty'];
				$data[$itemId][$order_id]['order_requisition_qty'] = $dataReqArr[$requisition_no][$order_id][$itemId]['order_requisition_qty'];
			}
		}
		else
		{
			//insert code gose here
			if(empty($dataReqArr[$requisition_no]))
			{
				$distribution_method = 1;
			}

			if(!empty($dataArrRequisition[$order_id]))
			{
				foreach($dataArrRequisition[$order_id] as $itemId=>$itemArr)
				{
					$data[$itemId][$order_id]['previous_requisition_qnty'] = $itemArr['requisition_qnty'];
					$data[$itemId][$order_id]['allocation_balance_qnty'] = $data[$itemId][$order_id]['allocation_qnty']-$data[$itemId][$order_id]['previous_requisition_qnty'];
					$data[$itemId][$order_id]['hdn_allocation_balance_qnty'] = $data[$itemId][$order_id]['allocation_balance_qnty'];
					$data[$itemId][$order_id]['order_requisition_qty'] = '';
				}
			}
			else
			{
				if(!empty($dataArrAllocation[$order_id]))
				{
					foreach($dataArrAllocation[$order_id] as $itemId=>$itemArr)
					{
						$data[$itemId][$order_id]['previous_requisition_qnty'] = 0;
						$data[$itemId][$order_id]['allocation_balance_qnty'] = $data[$itemId][$order_id]['allocation_qnty'];
						$data[$itemId][$order_id]['hdn_allocation_balance_qnty'] = $data[$itemId][$order_id]['allocation_balance_qnty'];
						$data[$itemId][$order_id]['order_requisition_qty'] = '';
						
						if($data[$itemId][$order_id]['allocation_qnty'] == 0)
						{
							$isError = 1;
							$orderError[$order_id] = $order_details[$order_id];
						}
					}
				}
				else
		...
		<div style="margin-top:10px;">
            <form name="yarnReqQnty_2" id="yarnReqQnty_2" autocomplete="off">
                <table id="tblRequisitionQty" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
					...
					if($isError == 1)
					{
						echo "<tr><td align='center' colspan='7'><h3 style=\"color:red;\">Order no : ".implode(', ', $orderError)." allocation balance is zero.</h3></td></tr>";
						die;		
					}
					...
					$hdnAllocationBalanceQty = 0;
					foreach($data as $itemId=>$itemArr)
					{
						foreach($itemArr as $orderId=>$row)
						{
							$sl++;
							//$itemId = $dataArrAllocation[$orderId]['item_id'];
							$totalOrderAllocationQty += number_format($row['allocation_qnty'], 2, '.', '');
							$totalRequisitionQty += number_format($row['previous_requisition_qnty'], 2, '.', '');
							$allocationBalanceQty += number_format($row['allocation_balance_qnty'], 2, '.', '');
							$hdnAllocationBalanceQty += number_format($row['hdn_allocation_balance_qnty'], 2, '.', '');
	
							if($hdnRequisitionQty != '')
							{
								$hdnRequisitionQty .= ',';
							}
							$hdnRequisitionQty .= $row['program_qnty'].'_'.$program_qty;
							?>
							<tr valign="middle">
								<td><?php echo $sl; ?></td>
								<td title="<?php echo $order_id;?>"> <?php echo $order_details[$orderId]; ?></td>
								...
								<td align="right"><?php echo number_format($row['previous_requisition_qnty'], 2, '.', ''); ?></td>
								<td align="right"><?php echo number_format($row['allocation_balance_qnty'], 2, '.', ''); ?></td>
								<td>
									<input type="text" name="textRequisitionQty[]" id="textRequisitionQty_<?php echo $sl; ?>" class="text_boxes_numeric" style="text-align:right" readonly value="<?php echo number_format($row['order_requisition_qty'], 2, '.', ''); ?>" />
									...
									<input type="hidden" name="hdnOrderBalanceQty[]" id="hdnOrderBalanceQty_<?php echo $sl; ?>" class="text_boxes_numeric" value="<?php echo number_format($row['hdn_allocation_balance_qnty'], 2, '.', ''); ?>" style="text-align:right" readonly />
									<input type="hidden" name="hdnProgramNo[]" id="hdnProgramNo_<?php echo $sl; ?>" class="text_boxes_numeric" value="<?php echo $program_no; ?>" style="text-align:right" readonly />
									...
									<input type="hidden" name="hdnItemId[]" id="hdnItemId_<?php echo $sl; ?>" class="text_boxes_numeric" value="<?php echo $itemId; ?>" style="text-align:right" readonly />
								</td>
							</tr>
							<?php
						}
                    ...
                    <tfoot>
                    	<tr>
                        	...
                        	<th><?php echo number_format($program_qty, 2, '.', ''); ?></th>
                        	...
                        	<th>
                                ...
                                <input type="hidden" name="hdnAllocationBalanceQty" id="hdnAllocationBalanceQty" class="text_boxes" value="<? echo number_format($hdnAllocationBalanceQty, 2, '.', ''); ?>" style="text-align:right" />
                            </th>
                        </tr>
                    </tfoot>                   
                </table>
...
<script>func_onkeyup_total_requisition_qty();</script>
<script src="../../includes/functions_bottom.js" type="text/javascript"></script>
</html>
<?
exit();
	...
	<div style="width:960px; overflow-y:auto; max-height:300px;" id="scroll_body" align="left">
		<table class="rpt_table" rules="all" border="1" width="960" id="tbl_list_search">
			...
			foreach ($query as $selectResult)
			{
				...
				?>
				<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>" style="cursor:pointer"
					onClick="get_php_form_data(<? echo $selectResult[csf('id')]; ?>+'_'+document.getElementById('companyID').value, 'populate_requisition_data', 'yarn_requisition_entry_controller' );">
					<td width="70"><p><? echo $dataArray[0][csf('lot')]; ?></p></td>
					<td width="60" align="center"><p><? echo $count_arr[$dataArray[0][csf('yarn_count_id')]]; ?></p></td>
					<td width="70"><p><? echo $company_supplier_name; ?></p></td>
...
if ($action == "save_update_delete")
{
	...
	if ($operation == 0)  // Insert Here
	{
		...
		if($is_dist_qnty_valiable_set == 1)
		{
			...
			$rID2 = sql_insert("ppl_yarn_req_distribution", $field_dist_array, $data_dist_array, 1);
		}
		else
		{
		...
		if($dataRequisition != '')
		{
			$rID3 = sql_insert("ppl_yarn_requisition_breakdown", $fieldRequisition, $dataRequisition, 1);
		}
		//echo "10**INSERT INTO inv_material_allocation_mst (".$field_allocation.") VALUES ".$data_allocation.""; die;
		//echo "10**".$rID ."&&". $rID2 ."&&".$alocationFlag;die;
	...
	else if ($operation == 1)   // Update Here
	{
		...
		if($prod_id != str_replace("'", "", $original_prod_id))
		{
			...
			$reqBreakdown1 = execute_query("DELETE FROM ppl_yarn_requisition_breakdown WHERE requisition_id = ".$reqNo." AND program_id IN(".implode(',', $programIdArr).") AND order_id IN(".implode(',', $orderIdArr).") AND item_id IN(".$originalItemId.")", 0);
		}
		...
		if($dataRequisition != '')
		{
			$reqBreakdown2 = execute_query("DELETE FROM ppl_yarn_requisition_breakdown WHERE requisition_id = ".$txt_requisition_no." AND program_id IN(".implode(',', $programIdArr).") AND order_id IN(".implode(',', $orderIdArr).") AND item_id IN(".implode(',', $itemIdArr).")", 0);
			$rID3 = sql_insert("ppl_yarn_requisition_breakdown", $fieldRequisition, $dataRequisition, 1);
