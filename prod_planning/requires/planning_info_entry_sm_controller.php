...
if ($action == "load_drop_down_knitting_party")
{
	...
	if ($data[0] == 1)
	{
		...
		{
			echo create_drop_down("cbo_knitting_party", 152, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0  AND comp.id IN(".$users_working_unit.") order by comp.company_name", "id,company_name", 1, "--Select Knit Party--", $data[1], "load_drop_down( 'planning_info_entry_sm_controller', this.value, 'load_drop_down_location', 'location_td' ); getValutAttention(this.value);", "");
		}
		else
		{
			echo create_drop_down("cbo_knitting_party", 152, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "--Select Knit Party--", $data[1], "load_drop_down( 'planning_info_entry_sm_controller', this.value, 'load_drop_down_location', 'location_td' ); getValutAttention(this.value);", "");
		}
	}
	else if ($data[0] == 3)
	{
		if ($data[2] == 1) $selected_id = $data[1]; else $selected_id = 0;
		echo create_drop_down("cbo_knitting_party", 152, "select a.id, a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=20 and a.status_active=1 and a.is_deleted=0 group by a.id, a.supplier_name order by a.supplier_name", "id,supplier_name", 1, "--Select Knit Party--", $selected_id, "load_drop_down( 'planning_info_entry_sm_controller', this.value, 'load_drop_down_location', 'location_td' ); getValutAttention(this.value);");
	}
	else
	{
		echo create_drop_down("cbo_knitting_party", 152, $blank_array, "", 1, "--Select Knit Party--", 0, "load_drop_down( 'planning_info_entry_sm_controller', this.value, 'load_drop_down_location', 'location_td' ); getValutAttention(this.value);");
	}
	exit();
}
				...
				<tbody>
					<tr>
						...
						<td align="center">
							...
							onClick="show_list_view ('<? echo $companyID; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value, 'create_style_ref_search_list_view', 'search_div', 'planning_info_entry_sm_controller', 'setFilterGrid(\'tbl_list_search\',-1)');"
							style="width:100px;"/>
						</td>
					</tr>
				</tbody>
			...
			<fieldset style="width:980px;">
				...
				<tbody>
					<tr>
						...
						<td align="center">
							<input type="button" name="button" class="formbutton" value="Show"
							onClick="show_list_view ('<? echo $companyID; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value+'**'+document.getElementById('cbo_date_type').value, 'create_order_no_search_list_view', 'search_div', 'planning_info_entry_sm_controller', 'setFilterGrid(\'tbl_list_search\',-1)');"
							style="width:100px;"/>
						</td>
					</tr>
					<tr>
			...
			<fieldset style="width:600px;">
				...
				<tbody>
					<tr class="general">
						...
						<td align="center">
							<input type="button" name="button" class="formbutton" value="Show"
							onClick="show_list_view ('<? echo $companyID; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('cbo_year').value+'**'+'<? echo $cbo_booking_type; ?>', 'create_booking_search_list_view', 'search_div', 'planning_info_entry_sm_controller', 'setFilterGrid(\'tbl_list_search\',-1)');"
							style="width:100px;"/>
						</td>
					</tr>
				</tbody>
...
if ($action == "create_booking_search_list_view")
{
	...
	$sql = "SELECT ID, BOOKING_NO, BOOKING_DATE, BUYER_ID, JOB_NO, PAY_MODE, SUPPLIER_ID FROM wo_booking_mst WHERE REF_CLOSING_STATUS=0 AND ITEM_CATEGORY = 2 AND FABRIC_SOURCE != 2 AND PAY_MODE in(3,5) AND company_id = ".$company_id.$search_cond.$buyer_id_cond.$year_cond.$booking_type_cond."
	UNION ALL
	SELECT ID, BOOKING_NO, BOOKING_DATE, BUYER_ID, JOB_NO, PAY_MODE, SUPPLIER_ID FROM WO_NON_ORD_SAMP_BOOKING_MST WHERE REF_CLOSING_STATUS=0 AND ITEM_CATEGORY=2 AND FABRIC_SOURCE != 2 AND PAY_MODE = 3 AND COMPANY_ID = ".$company_id.$search_cond.$buyer_id_cond.$year_cond.$booking_type_cond." ORDER BY ID DESC";
	//echo $sql;
	?>
	<div>
...
if ($action == "booking_item_details")
{
	...
	if (str_replace("'", "", $cbo_buyer_name) == 0)
	{
		if ($_SESSION['logic_erp']["data_level_secured"] == 1)
		{
			if ($_SESSION['logic_erp']["buyer_id"] >0){
				...
				$buyer_id_cond2 = " and d.buyer_id in (" . $_SESSION['logic_erp']["buyer_id"] . ")";
			}
			else{
				$buyer_id_cond = "";
				$buyer_id_cond2 = "";
	...
	else
	{
		...
		$buyer_id_cond2 = " and d.buyer_id=$cbo_buyer_name";
	}
	...
	else
	{
		...
		$job_no_cond2 = "and a.id in(" . str_replace("'", "", $hide_job_id) . ")";
	}
	...
	else
	{
		...
		$po_id_cond2 = "and c.po_break_down_id in(" . $po_id . ")";
	}
	...
	if(str_replace("'","",trim($txt_booking_no))!="")
	{
		...
		$booking_cond2="and d.booking_no like '$txt_booking'";
	...
	if ($date_from != "" && $date_to != "")
	{
		if ($db_type == 0)
		{
			...
			$booking_date2 = "and d.booking_date between '" . change_date_format(trim($date_from), "yyyy-mm-dd", "-") . "' and '" . change_date_format(trim($date_to), "yyyy-mm-dd", "-") . "'";
		}
		else
		{
			$booking_date = "and a.booking_date between '" . change_date_format(trim($date_from), '', '', 1) . "' and '" . change_date_format(trim($date_to), '', '', 1) . "'";
			$booking_date2 = "and d.booking_date between '" . change_date_format(trim($date_from), '', '', 1) . "' and '" . change_date_format(trim($date_to), '', '', 1) . "'";
		}
	}
	...
	if($cbo_booking_type>0)
	{
		...
		$booking_type_cond2 = "and d.booking_type=$cbo_booking_type and d.is_short=$is_short";
	}
	else
	{
		$booking_type_cond = "";
	...
	else
	{
		...
		else
		{
			...
			//$sql = "SELECT a.id, a.company_id, a.item_category, a.booking_no_prefix_num as booking_prefix,a.fabric_source, a.booking_type, a.is_short, a.booking_no, a.booking_date, a.job_no, a.buyer_id, a.is_approved, b.construction,b.copmposition , b.pre_cost_fabric_cost_dtls_id, b.po_break_down_id, b.gsm_weight, b.dia_width, sum(b.grey_fab_qnty) as qnty, c.style_ref_no from wo_booking_mst a, wo_booking_dtls b, wo_po_details_master c where a.entry_form!=140 and a.booking_type!=4 and a.booking_no=b.booking_no and b.job_no=c.job_no and a.company_id=$company_name and a.item_category=2  and (a.fabric_source=1 or b.fabric_source=1) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.grey_fab_qnty>0 and a.pay_mode in(3,5) $booking_cond $job_no_cond $buyer_id_cond $po_id_cond $po_data_file_ref $booking_date $booking_type_cond and a.ref_closing_status=0 group by a.id, a.company_id, a.fabric_source, a.booking_no_prefix_num,a.booking_type, a.is_short, a.booking_no, a.booking_date, a.job_no, a.buyer_id, a.is_approved, a.item_category, b.construction, b.copmposition, b.pre_cost_fabric_cost_dtls_id, b.po_break_down_id, b.gsm_weight, b.dia_width, c.style_ref_no order by b.dia_width, a.booking_no";

			$sql = "SELECT x.id, x.company_id, x.item_category, x.booking_prefix,x.fabric_source, x.booking_type, x.is_short, x.booking_no, x.booking_date, x.job_no, x.buyer_id, x.is_approved, x.construction, x.copmposition, x.pre_cost_fabric_cost_dtls_id, x.po_break_down_id, x.gsm_weight, x.dia_width, x.qnty, x.style_ref_no,x.determination_id_sample from (SELECT a.id, a.company_id, a.item_category, a.booking_no_prefix_num as booking_prefix,a.fabric_source, a.booking_type, a.is_short, a.booking_no, a.booking_date, a.job_no, a.buyer_id, a.is_approved, b.construction,CAST (b.copmposition AS CHAR(500)) as copmposition, b.pre_cost_fabric_cost_dtls_id, b.po_break_down_id, b.gsm_weight, b.dia_width, sum(b.grey_fab_qnty) as qnty, c.style_ref_no,null as determination_id_sample from wo_booking_mst a, wo_booking_dtls b, wo_po_details_master c where a.entry_form!=140 and a.booking_type!=4 and a.booking_no=b.booking_no and b.job_no=c.job_no and a.company_id=$company_name and a.item_category=2  and (a.fabric_source=1 or b.fabric_source=1) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.grey_fab_qnty>0 and a.pay_mode in(3,5) $booking_cond $job_no_cond $buyer_id_cond $po_id_cond $po_data_file_ref $booking_date $booking_type_cond and a.ref_closing_status=0 group by a.id, a.company_id, a.fabric_source, a.booking_no_prefix_num,a.booking_type, a.is_short, a.booking_no, a.booking_date, a.job_no, a.buyer_id, a.is_approved, a.item_category, b.construction, b.copmposition, b.pre_cost_fabric_cost_dtls_id, b.po_break_down_id, b.gsm_weight, b.dia_width, c.style_ref_no 
			union all 
				SELECT d.id, d.company_id, d.item_category, d.booking_no_prefix_num as booking_prefix,c.fabric_source,d.booking_type, c.is_short,d.booking_no,d.booking_date, a.job_no, d.buyer_id, d.is_approved ,null as construction ,null as copmposition,c.pre_cost_fabric_cost_dtls_id as  pre_cost_fabric_cost_dtls_id , c.po_break_down_id, e.gsm as gsm_weight, e.dia as dia_width, sum(c.grey_fab_qnty) as qnty, a.style_ref_no,e.determination_id as determination_id_sample  from wo_po_details_master a join wo_po_break_down b on a.id=b.job_id join wo_booking_dtls c on b.id=c.PO_BREAK_DOWN_ID join wo_booking_mst d on c.BOOKING_MST_ID=d.id join sample_development_fabric_acc e on e.id=c.PRE_COST_FABRIC_COST_DTLS_ID  where  c.entry_form_id=140 and d.booking_type=4  and d.item_category=2 and (d.fabric_source=1 or c.fabric_source=1) and c.grey_fab_qnty>0 and d.pay_mode in(3,5) and d.company_id=$company_name $booking_cond2 $job_no_cond2 $buyer_id_cond2 $po_id_cond2 $po_data_file_ref $booking_date2 $booking_type_cond2 group by d.id, d.company_id, d.item_category, d.booking_no_prefix_num ,c.fabric_source, d.booking_type, c.is_short,d.booking_no,d.booking_date, a.job_no, d.buyer_id, d.is_approved , c.pre_cost_fabric_cost_dtls_id , c.po_break_down_id, e.gsm, e.dia, a.style_ref_no,e.determination_id) x order by x.dia_width, x.booking_no";

			

			
				//$sqlColor = "SELECT a.id, a.company_id, a.item_category, a.booking_no_prefix_num as booking_prefix,a.fabric_source, a.booking_type, a.is_short, a.booking_no, a.booking_date, a.job_no, a.buyer_id, a.is_approved, b.construction,b.copmposition , b.pre_cost_fabric_cost_dtls_id, b.po_break_down_id, b.gsm_weight, b.dia_width, b.fabric_color_id, sum(b.grey_fab_qnty) as qnty, c.style_ref_no from wo_booking_mst a, wo_booking_dtls b, wo_po_details_master c where a.entry_form!=140 and a.booking_no=b.booking_no and b.job_no=c.job_no and a.company_id=$company_name and a.item_category=2  and (a.fabric_source=1 or b.fabric_source=1) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.grey_fab_qnty>0 and a.pay_mode in(3,5) $booking_cond $job_no_cond $buyer_id_cond $po_id_cond $po_data_file_ref $booking_date $booking_type_cond group by a.id, a.company_id, a.fabric_source, a.booking_no_prefix_num, a.booking_type, a.is_short, a.booking_no, a.booking_date, a.job_no, a.buyer_id, a.is_approved, a.item_category, b.construction, b.copmposition, b.pre_cost_fabric_cost_dtls_id, b.po_break_down_id, b.gsm_weight, b.dia_width, b.fabric_color_id, c.style_ref_no order by b.dia_width, a.booking_no";


				$sqlColor = "SELECT x.id, x.company_id, x.item_category, x.booking_prefix,x.fabric_source, x.booking_type, x.is_short, x.booking_no, x.booking_date, x.job_no, x.buyer_id, x.is_approved, x.construction, x.copmposition, x.pre_cost_fabric_cost_dtls_id, x.po_break_down_id, x.gsm_weight, x.dia_width, x.fabric_color_id, x.qnty, x.style_ref_no,x.determination_id_sample from (SELECT a.id, a.company_id, a.item_category, a.booking_no_prefix_num as booking_prefix,a.fabric_source, a.booking_type, a.is_short, a.booking_no, a.booking_date, a.job_no, a.buyer_id, a.is_approved, b.construction,CAST (b.copmposition AS CHAR(500)) as copmposition, b.pre_cost_fabric_cost_dtls_id, b.po_break_down_id, b.gsm_weight, b.dia_width, b.fabric_color_id, sum(b.grey_fab_qnty) as qnty, c.style_ref_no,null as determination_id_sample from wo_booking_mst a, wo_booking_dtls b, wo_po_details_master c where a.entry_form!=140 and a.booking_no=b.booking_no and b.job_no=c.job_no and a.company_id=$company_name and a.item_category=2  and (a.fabric_source=1 or b.fabric_source=1) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.grey_fab_qnty>0 and a.pay_mode in(3,5) $booking_cond $job_no_cond $buyer_id_cond $po_id_cond $po_data_file_ref $booking_date $booking_type_cond group by a.id, a.company_id, a.fabric_source, a.booking_no_prefix_num, a.booking_type, a.is_short, a.booking_no, a.booking_date, a.job_no, a.buyer_id, a.is_approved, a.item_category, b.construction, b.copmposition, b.pre_cost_fabric_cost_dtls_id, b.po_break_down_id, b.gsm_weight, b.dia_width, b.fabric_color_id, c.style_ref_no 
			union all 
				SELECT d.id, d.company_id, d.item_category, d.booking_no_prefix_num as booking_prefix,c.fabric_source, d.booking_type, c.is_short,d.booking_no,d.booking_date, a.job_no, d.buyer_id, d.is_approved ,null as construction ,null as copmposition,c.pre_cost_fabric_cost_dtls_id as  pre_cost_fabric_cost_dtls_id,c.po_break_down_id, e.gsm as gsm_weight, e.dia as dia_width, c.fabric_color_id,sum(c.grey_fab_qnty) as qnty, a.style_ref_no,e.determination_id as determination_id_sample  from wo_po_details_master a join wo_po_break_down b on a.id=b.job_id join 
				wo_booking_dtls c on b.id=c.PO_BREAK_DOWN_ID join wo_booking_mst d on c.BOOKING_MST_ID=d.id join sample_development_fabric_acc e on e.id=c.PRE_COST_FABRIC_COST_DTLS_ID  where  c.entry_form_id=140 and d.booking_type=4  and d.item_category=2 and (d.fabric_source=1 or c.fabric_source=1) and c.grey_fab_qnty>0 and d.pay_mode in(3,5) and d.company_id=$company_name $booking_cond2 $job_no_cond2 $buyer_id_cond2 $po_id_cond2 $po_data_file_ref $booking_date2 $booking_type_cond2 group by d.id, d.company_id, d.item_category, d.booking_no_prefix_num ,c.fabric_source, d.booking_type, c.is_short,d.booking_no,d.booking_date, a.job_no, d.buyer_id, d.is_approved , c.pre_cost_fabric_cost_dtls_id , c.po_break_down_id, e.gsm, e.dia, c.fabric_color_id,a.style_ref_no,e.determination_id) x order by x.dia_width, x.booking_no";
		}
	}
	//echo $sql; die;
	...
	foreach ($nameArray as $row)
	{
		...
		if($row[csf("determination_id_sample")]>0)
		{
			$determination_id_sample_arr[] = $row[csf("determination_id_sample")];
			$sample_pre_cost_id_arr[] = $row[csf("pre_cost_fabric_cost_dtls_id")];
		}
	...
	if(!empty($pre_cost_id_arr))
	{
		...
		//$pre_cost_cond2 = " and a.id in(".implode(",", array_unique($pre_cost_id_arr)).")";
		...
		else
		{
			...
			//$costing_sql = sql_select("SELECT a.id, a.body_part_id, a.color_type_id, a.width_dia_type, a.gsm_weight, a.construction || ',' || a.composition as fab_desc, a.lib_yarn_count_deter_id, 1 as booking_type from wo_pre_cost_fabric_cost_dtls a where a.status_active=1 $pre_cost_cond");
			$costing_sql = sql_select("SELECT a.id, a.body_part_id, a.color_type_id, a.width_dia_type, a.gsm_weight, a.construction || ',' || a.composition as fab_desc, a.lib_yarn_count_deter_id, 1 as booking_type from wo_pre_cost_fabric_cost_dtls a where a.status_active=1 $pre_cost_cond ");
	...
	if(!empty($determination_id_sample_arr))
	{
		$pre_cost_cond2 = " and a.id in(".implode(",", array_unique($determination_id_sample_arr)).")";
		$pre_cost_cond3 = " and b.id in(".implode(",", array_unique($determination_id_sample_arr)).")";
		$pre_cost_cond_sample = " and a.id in(".implode(",", array_unique($sample_pre_cost_id_arr)).")";
		
			 
			$costing_sql = sql_select("SELECT a.id, a.body_part_id, a.color_type_id, a.width_dia_id as width_dia_type, a.gsm as gsm_weight,b.construction || ',' || d.composition_name || ' ' || c.percent ||'%' as fab_desc,a.fabric_description, a.determination_id,4 as booking_type from  wo_booking_mst e, wo_booking_dtls f, wo_po_details_master g , sample_development_fabric_acc a,lib_yarn_count_determina_mst b,lib_yarn_count_determina_dtls c, lib_composition_array d where f.entry_form_id=140 and f.booking_type=4 and e.booking_no=f.booking_no and f.job_no=g.job_no and f.pre_cost_fabric_cost_dtls_id=a.id and a.determination_id=b.id and b.id=c.mst_id and c.copmposition_id=d.id and a.status_active=1  $pre_cost_cond3 $pre_cost_cond_sample");
	...
	{
		$colorWiseQty[$row[csf('booking_no')]][$row[csf('po_break_down_id')]][$row[csf('pre_cost_fabric_cost_dtls_id')]][$costing_per_id_library[$row[csf('pre_cost_fabric_cost_dtls_id')]]['body_part']][$costing_per_id_library[$row[csf('pre_cost_fabric_cost_dtls_id')]]['desc']][$costing_per_id_library[$row[csf('pre_cost_fabric_cost_dtls_id')]]['gsm']][$row[csf('dia_width')]][$row[csf('booking_type')]][$costing_per_id_library[$row[csf('pre_cost_fabric_cost_dtls_id')]]['color_type']][$row[csf('fabric_color_id')]]['booking_qty'] = $row[csf('qnty')];
	}
	//echo "<pre>";
	//print_r($colorWiseQty);
	...
	foreach ($nameArray as $row)
	{
		//echo $fabricDataArr[$row[csf('determination_id_sample')]]['construction'].'='.$composition_arr[$row[csf('determination_id_sample')]]."<br/>";
			...
			<fieldset style="width:2410px;">
				...
				<div style="width:2600px; overflow-y:scroll; max-height:330px;" id="scroll_body">
					...
					foreach ($nameArray as $row) {
						...
						if($ref_closing_arr[$row[csf('id')]]!=1)
						{
			 			?>
					...
					<table cellspacing="0" cellpadding="0" border="1" rules="all" width="2080" class="rpt_table" id="tbl_list_search">
						...
						foreach ($nameArray as $row)
						{
							...
							if ($booking_type == 2) //Main Fabric booking
							{
								...
								{
									$po_id_str = implode(',', $booking_po_arr[$row[csf("booking_no")]]);
									$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . $booking_type . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $po_id_str . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row_id . "','show_fabric_booking_report_fn','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
								}
							}
							...
							if (($planning_status == 2 && $dataPlanQty[$plan_id]['bookingQty'] <= $dataPlanQty[$plan_id]['planQty'] &&  $plan_id != '') || ($planning_status == 1 && ($dataPlanQty[$plan_id]['bookingQty'] > $dataPlanQty[$plan_id]['planQty'] || $plan_id == '')))
							{
                            	...
                            	<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer"
                                    ...
                                    <td width="130" title="<? echo $determination_id; ?>" id="desc_<? echo $i; ?>" align="center"><p><? echo $desc; ?></p></td>
                                    ...
                                    //echo $row[csf('booking_no')].'='.$row[csf('po_break_down_id')].'='.$row[csf('pre_cost_fabric_cost_dtls_id')].'='.$costing_per_id_library[$row[csf('pre_cost_fabric_cost_dtls_id')]]['body_part'].'='.$desc.'='.$gsm.'='.$row[csf('dia_width')].'='.$row[csf('booking_type')].'='.$color_type_id."<br/>";

                                    foreach($colorWiseQty[$row[csf('booking_no')]][$row[csf('po_break_down_id')]][$row[csf('pre_cost_fabric_cost_dtls_id')]][$costing_per_id_library[$row[csf('pre_cost_fabric_cost_dtls_id')]]['body_part']][$desc][$gsm][$row[csf('dia_width')]][$row[csf('booking_type')]][$color_type_id] as $colorId=>$colorArr)
                                    {
                                        if($colorQtyStr != '')
                                            $colorQtyStr .= ',';
...
if ($action == "prog_qnty_popup")
{
	...
	<script>
		...
		function openpage_machine() {
			...
			var page_link = 'planning_info_entry_sm_controller.php?action=machine_info_popup&save_string=' + save_string + '&companyID=' + companyID + '&txt_machine_dia=' + txt_machine_dia + '&update_dtls_id=' + update_dtls_id + '&allowed_date_qnty_string=' +allowed_date_qnty_string;
        ...
        function fnc_program_entry(operation)
        {
        	...
        	http.open("POST", "planning_info_entry_sm_controller.php", true);
        ...
        function fnc_program_entry_Reply_info() {
        	if (http.readyState == 4) {
                ...
                if ((reponse[0] == 0 || reponse[0] == 1 || reponse[0] == 2)) {
                	...
                	reset_form('programQnty_1', '', '', 'txt_start_date,<? echo $start_date; ?>*txt_end_date,<? echo $end_date;?>*txt_program_date,<? echo $current_date;?>', '', 'hdn_booking_qnty*txt_fabric_dia*hdnVariableCollarCuff*hidden_bodypartID_data*hdn_data_text_frontEnd');
                	...
                	show_list_view(reponse[1]+'_'+'<?php echo trim($dia);?>'+'_'+dtls_id, 'planning_info_details', 'list_view', 'planning_info_entry_sm_controller', '');
        ...
        function openpage_feeder() {
        	...
        	var page_link = 'planning_info_entry_sm_controller.php?action=feeder_info_popup&no_of_feeder_data=' + no_of_feeder_data + '&pre_cost_id=' + '<? echo $pre_cost_id; ?>';
        ...
        function openpage_collarCuff() {
        	...
        	var page_link = 'planning_info_entry_sm_controller.php?action=collarCuff_info_popup&collarCuff_data=' + collarCuff_data + '&pre_cost_id=' + '<? echo $pre_cost_id; ?>&update_dtls_id=' + update_dtls_id +'&hidden_bodypartID_data='+hidden_bodypartID_data+ '&bookingNo=' + '<? echo $booking_no; ?>'+ '&body_part_id=' + '<? echo $body_part_id; ?>'; 
        ...
        function openpage_advice() {
        	...
        	var page_link = 'planning_info_entry_sm_controller.php?action=advice_info_popup&hidden_advice_data=' + hidden_advice_data;
		...
		function openpage_count_feeding(){
			...
			var page_link = 'planning_info_entry_sm_controller.php?action=count_feeding_data_popup&count_feeding_data=' + count_feeding_data + '&pre_cost_id=' + '<? echo $pre_cost_id; ?>&update_dtls_id=' + update_dtls_id;
		...
		function getValutAttention(knit_source_id)
		{
				...
				if(knit_source_id==1)
				{
					...
					get_php_form_data(knit_source_id+'*'+inHouse_knit_party_id, "check_last_attention_action", "planning_info_entry_sm_controller" );
				}
				else
				{
					var outBound_knit_party_id=$("#cbo_knitting_party").val()*1;
					get_php_form_data(knit_source_id+'*'+outBound_knit_party_id, "check_last_attention_action", "planning_info_entry_sm_controller" );
		...
		function fn_knit_production(knit_source_id){
        	...
        	load_drop_down( 'planning_info_entry_sm_controller', hidden_knit_source+'**'+companyID, 'load_drop_down_knitting_party','knitting_party');//class="must_entry_caption"
    		}
		    return;
		}
		...
		<div align="center">
			...
			<fieldset style="width:900px; margin-top:5px;">
				...
				<table width="900" align="center" border="0">
					<tr>
						...
						<td>
							<?
							echo create_drop_down("cbo_knitting_source", 152, $knitting_source, "", 1, "-- Select --", 0, "active_inactive();load_drop_down( 'planning_info_entry_sm_controller', this.value+'**'+$companyID, 'load_drop_down_knitting_party','knitting_party'); load_drop_down( 'planning_info_entry_sm_controller',$companyID, 'load_drop_down_location', 'location_td' ); getValutAttention(this.value);fn_knit_production(this.value);", 0, '1,3');
							?>
			...
			<div id="list_view" style="margin-top:5px">
				...
				if (str_replace("'", '', $plan_id) != "") {
					?>
					<script>
						show_list_view('<? echo str_replace("'", '', $plan_id)."_".trim($dia)."_".$prog_id; ?>', 'planning_info_details', 'list_view', 'planning_info_entry_sm_controller', '');
					</script>
					<?
				}
				?>
	...
	<script>
		...
		var dataSplit=datas.split("_");
		var splitData=dataSplit[0].split("**");
		$("#desc_of_yarn").text(splitData[17]);
...
if ($action == "planning_info_details")
{
	...
	<div style="width:1100px; max-height:140px; overflow-y:scroll" id="list_container_batch" align="left">
		...
		foreach ($result as $row) {
			...
			?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
				onClick="get_php_form_data('<? echo $row[csf('id')]."_".$expData[1]."_".$yarnIssueCheck."_".$row[csf('body_part_id')]."_".$row[csf('po_id')]."_".$row[csf('po_wise_bodyPartId')]; ?>','populate_data_from_planning_info', 'planning_info_entry_sm_controller' );balance_cal();">
...
if ($action == "populate_data_from_planning_info")
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down('planning_info_entry_sm_controller', " . $row[csf("knitting_source")] . "+'**'+" . $row[csf("knitting_party")] . "+'**1', 'load_drop_down_knitting_party','knitting_party');\n";
		...
		echo "load_drop_down('planning_info_entry_sm_controller', " .$row[csf("knitting_party")] . ", 'load_drop_down_location','location_td');\n";
		//echo "load_drop_down('planning_info_entry_sm_controller', " .$row[csf("knitting_party")] . ", 'load_drop_down_machine_group','machine_group_td');\n";
	...
	<script>
		...
		function fn_add_date_field(row_no) {
			...
			if ($('#txt_startDate_' + row_no).val() != "") {
				...
				var response = return_global_ajax_value(data, 'date_duplication_check', '', 'planning_info_entry_sm_controller');
...
if ($action == "color_info_popup")
{
	...
	<script>
        ...
        function distribute_prog_qnty(str)
		{
			if(str==1)
			{
				var txt_tot_blnc_qnty=$('#txt_tot_blnc_qnty').val()*1;
				var txt_propo_prog_qnty=$('#txt_propo_prog_qnty').val()*1;
				var is_chkboxChecked=0;
				$("#tbl_list_search").find('tbody tr').not(":first").each(function()
				{
					//var isDisbled=$(this).find('input[name="text_color_prog_qty[]"]').is(":disabled");

					
					//if(isDisbled==false )
					//{
				
				        var isChecked = $(this).find('input[name="check_box[]').prop("checked");
				        if (isChecked) {
				        	is_chkboxChecked=1;
				        }
			    	
						var per_row_wise_blnc=$(this).find('input[name="txt_individual_color_blqty[]').val()*1;
						var per_row_wise_required=$(this).find('input[name="hidden_color_allowed_qty[]"]').val()*1;
						
						var prog_qnty=((txt_propo_prog_qnty/txt_tot_blnc_qnty)*per_row_wise_blnc);
						//alert(txt_propo_prog_qnty+'/('+per_row_wise_blnc+'*'+txt_tot_blnc_qnty);
						if(prog_qnty>0){prog_qnty=prog_qnty;}else{prog_qnty="";}
						$(this).find('input[name="text_color_prog_qty[]"]').val(prog_qnty.toFixed(3));
						if(txt_propo_prog_qnty==0){
							$(this).find('input[name="text_color_prog_qty[]"]').val('');
						}
					//}
				});
				if (is_chkboxChecked==1) {
					check_selected_row();
				}
			}
			else
			{
				$('#txt_propo_prog_qnty').val('');
				$("#tbl_list_search").find('tr').each(function()
				{
					if($(this).find('input[name="text_color_prog_qty[]"]').is(":disabled")==false)
					{
						$(this).find('input[name="text_color_prog_qty[]"]').val('');
						$(this).find('input[name="check_box[]').prop("checked", false);
					}
				});
			}
		}

		function check_selected_row() {
			var cbo_distribiution_method=$("#cbo_distribiution_method").val()*1;
			var txt_propo_prog_qnty=$("#txt_propo_prog_qnty").val()*1;
			if(cbo_distribiution_method==1 && txt_propo_prog_qnty>0)
			{
				var tbl_row_count = $('#tbl_list_search tbody tr').length;
				tbl_row_count = tbl_row_count - 1;			
				var txt_tot_blnc_qnty=0;
				

				for (var i = 1; i <= tbl_row_count; i++) {
					var isChecked = $("#check_box"+i).prop("checked");
					if(isChecked==true)
					{
						 txt_tot_blnc_qnty+=$("#txt_individual_color_blqty"+i).val()*1;
					}
				}
				for (var j = 1; j <= tbl_row_count; j++) {
					var isChecked = $("#check_box"+j).prop("checked");
					if(isChecked==true)
					{
						var per_row_wise_blnc=$("#txt_individual_color_blqty"+j).val()*1;
						var prog_qnty=((txt_propo_prog_qnty/txt_tot_blnc_qnty)*per_row_wise_blnc);
						//alert(txt_propo_prog_qnty+'/('+per_row_wise_blnc+'*'+txt_tot_blnc_qnty);
						if(prog_qnty>0){prog_qnty=prog_qnty;}else{prog_qnty="";}
						$("#text_color_prog_qty_"+j).val(prog_qnty.toFixed(3));
						if(txt_propo_prog_qnty==0){
							$("#text_color_prog_qty_"+j).val('');
						}
					}
					else
					{
...
<body>
	<div align="center" style="width:630px;">
			...
			<fieldset style="width:630px; margin-top:10px; margin-left:20px">
				...
				<div style="width:<? echo $width; ?>px;" align="center">
					<table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="350" align="center">
						<thead>
							<th>Total Program Qnty</th>
							<th>Distribution Method</th>
						</thead>
						<tr class="general">
							<td><input type="text" name="txt_propo_prog_qnty" id="txt_propo_prog_qnty" class="text_boxes_numeric" value="<? echo $txt_receive_qnty; ?>" style="width:120px" onBlur="distribute_prog_qnty(document.getElementById('cbo_distribiution_method').value)" <? echo $disabled; ?>></td>
							<td>
								<?
									//$prev_distribution_method
									$distribiution_method=array(0=>"--Select--",1=>"Proportionately",2=>"Manually");
									echo create_drop_down( "cbo_distribiution_method", 160, $distribiution_method,"",0, "",1, "distribute_prog_qnty(this.value);", $disable_drop_down );
								?>
							</td>
						</tr>
					</table>
				...
				<div>
					...
					<div style="width:600px; overflow-y:scroll; max-height:210px;" id="buyer_list_view" align="center">
						...
						if ($dia!= "" || $db_type == 0) {
							$dia_cond = "b.dia_width like '%$dia%'";
							$dia_cond2 = "d.dia like '%$dia%'";
							$dia_cond3 = "e.dia like '%$dia%'";
							//$dia_cond = "b.dia_width = '$dia'";
						} else {
							$dia_cond = "b.dia_width is null";
							$dia_cond2 = "d.dia is null";
							$dia_cond3 = "e.dia is null";
						}
						...
						SELECT c.po_break_down_id,e.body_part_id,c.fabric_color_id, sum(c.grey_fab_qnty) as qnty from wo_po_details_master a join wo_po_break_down b on a.id=b.job_id join wo_booking_dtls c on b.id=c.PO_BREAK_DOWN_ID join wo_booking_mst d on c.BOOKING_MST_ID=d.id join sample_development_fabric_acc e on e.id=c.PRE_COST_FABRIC_COST_DTLS_ID  where  c.entry_form_id=140 and d.booking_type=4  and d.item_category=2 and (d.fabric_source=1 or c.fabric_source=1) and c.grey_fab_qnty>0 and d.pay_mode in(3,5) and d.company_id=$companyID and e.body_part_id in($bodypartId ) and d.booking_no='$booking_no' and $dia_cond3 and c.po_break_down_id in ($po_id) and c.pre_cost_fabric_cost_dtls_id in ($pre_cost_id) and d.ref_closing_status=0  group by c.po_break_down_id,e.body_part_id,c.fabric_color_id");
						...
						select b.fabric_color_id, sum(b.grey_fab_qnty) as qnty 
						from wo_booking_mst a, wo_booking_dtls b,WO_PRE_COST_FABRIC_COST_DTLS c 
						where a.entry_form!=140 and a.booking_no=b.booking_no and b.booking_type=1 and b.is_short=1 and b.pre_cost_fabric_cost_dtls_id=c.id and b.job_no=c.job_no and c.body_part_id in($bodypartId ) and a.company_id=$companyID and a.item_category=2 and a.booking_no='$booking_no' and $dia_cond and b.po_break_down_id in ($po_id) and b.pre_cost_fabric_cost_dtls_id in ($pre_cost_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.fabric_color_id 
						union all  
						 SELECT c.fabric_color_id, sum(c.grey_fab_qnty) as qnty from wo_po_details_master a join wo_po_break_down b on a.id=b.job_id join wo_booking_dtls c on b.id=c.PO_BREAK_DOWN_ID join wo_booking_mst d on c.BOOKING_MST_ID=d.id join sample_development_fabric_acc e on e.id=c.PRE_COST_FABRIC_COST_DTLS_ID  where  c.entry_form_id=140 and d.booking_type=4  and d.item_category=2 and (d.fabric_source=1 or c.fabric_source=1) and c.grey_fab_qnty>0 and d.pay_mode in(3,5) and d.company_id=$companyID and e.body_part_id in($bodypartId ) and d.booking_no='$booking_no' and $dia_cond3 and c.po_break_down_id in ($po_id) and c.pre_cost_fabric_cost_dtls_id in ($pre_cost_id) and d.ref_closing_status=0  group by c.fabric_color_id";
						...
						foreach ($result as $row)
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>" onClick="js_set_value(<? echo $i; ?>)">
								...
								<td width="50" align="center"><input type="checkbox" name="check_box[]" id="check_box<?php echo $i; ?>" onClick="check_selected_row();" /> <? echo $i; ?>
					...
					<tfoot>
						<tr>
							...
							<th align="right">&nbsp;</th>
							<th align="right">&nbsp;</th>
							<th><input type="hidden" name="txt_tot_blnc_qnty[]" id="txt_tot_blnc_qnty" value="<? echo $totalBalance ; ?>"/></th>
						</tr>
					</tfoot>
				</table>
			</div>
		...
		$sql_color_not_exists = sql_select("select b.fabric_color_id from wo_booking_mst a, wo_booking_dtls b where a.entry_form!=140 and a.booking_no=b.booking_no and a.company_id=$companyID and a.item_category=2 and a.booking_no='$booking_no' and $dia_cond and b.po_break_down_id in ($po_id) and b.fabric_color_id in($hidden_color_id_for_revise_qnty) and b.pre_cost_fabric_cost_dtls_id in ($pre_cost_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.fabric_color_id 
			union all 
			select h.color_number_id as fabric_color_id 
			from wo_booking_mst a, wo_booking_dtls b,wo_pre_cost_fabric_cost_dtls d,wo_pre_cos_fab_co_avg_con_dtls h 
			where  b.entry_form_id=140 and a.booking_no=b.booking_no and b.job_no=d.job_no 
			and d.job_no=h.job_no and b.po_break_down_id= h.po_break_down_id 
			and a.company_id=$companyID and a.item_category=2 and a.booking_no='$booking_no' and $dia_cond and b.po_break_down_id in ($po_id)
			and h.color_number_id in($hidden_color_id_for_revise_qnty) and h.pre_cost_fabric_cost_dtls_id in($pre_cost_id)
			and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 
			group by h.color_number_id");
		$color_not_exist="";
					...
					<div style="width:600px; overflow-y:scroll; max-height:210px;" id="buyer_list_view" align="left">
						...
						id="tbl_list_search">
							<tbody>
								...
								from wo_booking_mst a, wo_booking_dtls b ,wo_pre_cost_fabric_cost_dtls c,wo_po_color_size_breakdown d,lib_body_part e
								where a.booking_no=b.booking_no and b.pre_cost_fabric_cost_dtls_id=c.id
								and b.po_break_down_id=d.po_break_down_id and b.gmts_size=d.size_number_id and b.gmts_color_id =d.color_number_id and c.body_part_id=e.id and c.job_no=d.job_no_mst and c.item_number_id=d.item_number_id and b.po_break_down_id=d.po_break_down_id and a.company_id=$companyID and a.item_category=2 and a.booking_no='$booking_no' and $dia_cond and b.po_break_down_id in ($po_id) and b.pre_cost_fabric_cost_dtls_id in ($pre_cost_id)
...
if ($action == "collarCuff_info_popup")
{
	...
	extract($_REQUEST);
	$mts_size_library=return_library_array( "select id,size_name from lib_size where status_active=1 and is_deleted=0", "id", "size_name");
	?>
	<script>
		...
		function fnc_close() {
			...
			$("#tbl_list_search").find('tbody tr').each(function () {
				...
				if (save_string == "") {
					save_string = bodyPartId + "_" + txtGrey + "_" + txtFinish + "_" + txtQtyPcs + "_" + txtNeedlePerCm + "_" + txtGmtSize;
				}
				else {
					save_string += "," + bodyPartId + "_" + txtGrey + "_" + txtFinish + "_" + txtQtyPcs  + "_" + txtNeedlePerCm + "_" + txtGmtSize;
				}
		...
		{
			//alert('su..re');
        	var page_link = 'planning_info_entry_sm_controller.php?action=action_finishSize&pre_cost_id=' + '<? echo $pre_cost_id; ?>';
						...
						<tbody>
							...
							if (!empty($collarCuff_data))
							{
								...
								$booking_No=explode("-",$bookingNo);
									...
									{
										$qty = 0;
										$dataArr[$row[csf('body_part_id')]][$row[csf('item_size')]][$row[csf('size_number_id')]]['qty'] += $qty;
									}
									/*echo "<pre>";
									print_r($collarCuff_data);
									echo "</pre>";*/
									for ($ii = 0; $ii < count($collarCuff_data); $ii++)
									{
										$body_part_wise_data = explode("_", $collarCuff_data[$ii]);
										...
										$gmtsSizeId = $body_part_wise_data[5];
										$totQtyPcs += $qty;

										$dataArrSavedData[$body_part_id][$finish][$gmtsSizeId]['body_part_id'] = $body_part_id;
										$dataArrSavedData[$body_part_id][$finish][$gmtsSizeId]['grey'] = $grey;
										$dataArrSavedData[$body_part_id][$finish][$gmtsSizeId]['finish'] = $finish;
										$dataArrSavedData[$body_part_id][$finish][$gmtsSizeId]['gmtsSizeId'] = $gmtsSizeId;
										$dataArrSavedData[$body_part_id][$finish][$gmtsSizeId]['needlePerCm'] = $needlePerCm;
										$dataArrSavedData[$body_part_id][$finish][$gmtsSizeId]['qty'] = $qty;
										$dataArrSavedData[$body_part_id][$finish][$gmtsSizeId]['totQtyPcs'] += $qty;
									...
									{
										foreach ($bodyPartArr as $itemSize=>$itemSizeData)
										{
											foreach ($itemSizeData as $gmtsSize=>$row)
											{
												$totQtyPcs += $row['qty'];

												$body_part_ids = $dataArrSavedData[$bodyPartId][$itemSize][$gmtsSize]['body_part_id'];
												$greys = $dataArrSavedData[$bodyPartId][$itemSize][$gmtsSize]['grey'];
												//$greys =rtrim($greys," ");
												$finishs = $dataArrSavedData[$bodyPartId][$itemSize][$gmtsSize]['finish'];
												$qtys = $dataArrSavedData[$bodyPartId][$itemSize][$gmtsSize]['qty'];
												$needlePerCms = $dataArrSavedData[$bodyPartId][$itemSize][$gmtsSize]['needlePerCm'];
												$gmtsSizeIds = $dataArrSavedData[$bodyPartId][$itemSize][$gmtsSize]['gmtsSizeId'];
												//$totQtyPcss = $dataArrSavedData[$bodyPartId][$itemSize][$gmtsSize]['totQtyPcs'];

												if($body_part_ids!=""){$body_part_id=$body_part_ids;}
												if($greys!=" "){$grey=$greys;}
												if($finishs!=""){$finish=$finishs;}
												if($gmtsSizeIds!=""){$gmtsSizeId=$gmtsSizeIds;}
												if($needlePerCms!=""){$needlePerCm=$needlePerCms;}
												if($qtys!=""){$qty=$qtys;}
												//if($totQtyPcss!=""){$totQtyPcs=$totQtyPcss;}

												?>
												<tr align="center" bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $row_no; ?>">
													<td width="30" align="center" title="<?=$row_no;?>"><? echo $i; ?></td>
													<td width="100">
													...
													<td width="100"><input type="text" name="txtFinish[]" id="txtFinish_<? echo $row_no; ?>" disabled
														class="text_boxes" style="width:80px"
														value="<? echo $itemSize; ?>"/>
													</td>
													<td width="100"><input type="text" name="txtGmtSize[]" id="txtGmtSize_<? echo $row_no; ?>" disabled
														class="text_boxes" style="width:80px"
														value="<? echo $mts_size_library[$gmtsSize]; ?>"/>
														<input type="hidden" name="hdnGmtSize[]" id="hdnGmtSize_<?php echo $row_no ?>" value="<? echo $gmtsSize; ?>"/>
													</td>
													<td width="100">
														<input type="text" name="txtQtyPcs[]" id="txtQtyPcs_<? echo $row_no; ?>"
														class="text_boxes_numeric" style="width:80px"
														value="<? echo $qty; ?>" onKeyUp="calculate_tot_qnty();"/>
													</td>
													<td width="100">
														<input type="text" name="txtNeedlePerCm[]" id="txtNeedlePerCm_<? echo $row_no; ?>"
														class="text_boxes_numeric" style="width:80px"
														value="<? echo $needlePerCm; ?>" />
													</td>
													<!--<td>
...
/*if($action == 'action_finishSize')
{
	...
	<script>
        show_list_view ('<? echo $pre_cost_id; ?>', 'action_finishSize_listview', 'search_div', 'planning_info_entry_sm_controller', 'setFilterGrid(\'tbl_list_search\',-1)');
    </script>
    <script src="../../includes/functions_bottom.js" type="text/javascript"></script>
    </html>
	<?
	...
	else if ($operation == 1)   // Update Here
	{
		...
		*/
		...
		else
		{
			...
			if ($hidden_collarCuff_data != "")
			{
				//echo "10**".$hidden_collarCuff_data;
				$collar_cuff_dtls_id = return_next_id("id", "ppl_planning_collar_cuff_dtls", 1);
				$field_array_collar_cuff_dtls = "id, mst_id, dtls_id, body_part_id, grey_size, finish_size, qty_pcs, inserted_by, insert_date";
			...
			//oci_rollback($con);disconnect($con);
			//die;
			if ($db_type == 0)
			{
				if ($flag == 1)
...
if ($action == "update_revise_qnty")
{
	...
	$knit_qty = return_field_value("sum(b.grey_receive_qnty) as knitting_qnty", "inv_receive_master a, pro_grey_prod_entry_dtls b", "a.id=b.mst_id and a.item_category=13 and a.entry_form=2 and a.receive_basis=2 and a.booking_id=$txt_programID and b.color_id='$txt_colorID' and b.status_active=1 and b.is_deleted=0", "knitting_qnty");
	...
	if($knit_qty=="")
	{
		$sql_production_and_prog_qnty=sql_select("select a.dtls_id as program_no,c.grey_receive_qnty,a.program_qnty from ppl_planning_entry_plan_dtls a ,inv_receive_master b, pro_grey_prod_entry_dtls c where  a.dtls_id=b.booking_id and b.id=c.mst_id  and b.item_category=13 and b.entry_form=2 and b.receive_basis=2 and b.booking_id=$txt_programID and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0"); 
		$qnty_arr=array();
		foreach ($sql_production_and_prog_qnty as $rows) {
			$qnty_arr[$rows[csf("program_no")]]['productionQnty']+=$rows[csf("grey_receive_qnty")];
			$qnty_arr[$rows[csf("program_no")]]['programQnty']=$rows[csf("program_qnty")];
		}
		...
		$totalProductionQnty=$qnty_arr[$txt_programID]['productionQnty'];
		$totalProgramQnty=$qnty_arr[$txt_programID]['programQnty'];

		if ($totalProductionQnty > $totalProgramQnty) {
			echo "20**$totalProductionQnty".'-'.$totalProgramQnty;
			disconnect($con);
			exit();
		}
	}
	...
	//echo "10**".$rID ."=". $rID2."=". $rID3 ."=". $flag;oci_rollback($con);die;
