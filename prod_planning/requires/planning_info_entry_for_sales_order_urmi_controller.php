...
if ($action == "booking_item_details_popup")
{
	...
	function show_details(type) {
		...
		freeze_window(5);
		http.open("POST", "planning_info_entry_for_sales_order_urmi_controller.php", true);
		http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		http.send(data);
		http.onreadystatechange = fn_show_details_reponse;
		//show_list_view(cbo_company_id, 'booking_item_details', 'list_container_fabric_desc', 'requires/planning_info_entry_for_sales_order_urmi_controller', '');
	}
	...
	function openmypage_job() {
		...
		var page_link = 'planning_info_entry_for_sales_order_urmi_controller.php?action=style_ref_search_popup&companyID=' + companyID + '&buyerID=' + buyerID;
	...
	function openmypage_internal_ref() {
		...
		var companyID = $("#cbo_company_name").val();
		var buyerID = $("#cbo_buyer_name").val();
		var page_link = 'planning_info_entry_for_sales_order_urmi_controller.php?action=internal_ref_search_popup&companyID=' + companyID + '&buyerID=' + buyerID;
	...
	function openmypage_booking() {
		...
		var companyID = $("#cbo_company_name").val();
		var page_link = 'planning_info_entry_for_sales_order_urmi_controller.php?action=booking_no_search_popup&companyID=' + companyID;
	...
	function openmypage_prog() {
		...
		var page_link = 'planning_info_entry_for_sales_order_urmi_controller.php?action=prog_qnty_popup&gsm=' + gsm + '&dia=' + dia + '&desc=' + desc + '&within_group=' + within_group + '&job_id=' + job_id + '&booking_qnty=' + encodeURIComponent(String(booking_no)) + '&companyID=' + companyID + '&data="' + data + '"' + '&plan_id=' + plan_id + '&determination_id=' + determination_id + '&booking_no=' + booking_no + '&body_part_id=' + body_part_id + '&fabric_type=' + fabric_typee + '&pre_cost_id=' + pre_cost_id + '&buyer_id=' + buyer_id + '&job_dtls_id=' + job_dtls_id + '&color_type_id=' + color_type_id + '&sales_order_dtls_id=' + sales_order_dtls_id;
 ...
 function selected_row(rowNo) {
	...
	function active_inactive() {
		...
		if (company_id == 0) {
			...
			load_drop_down('planning_info_entry_for_sales_order_urmi_controller', company_id + '_' + within_group, 'load_drop_down_buyer', 'buyer_td');
		}
	}
...
if ($action == "booking_item_details") 
{
	...
	else if($type == 3)
	{
		...
		foreach ($res_plan as $rowPlan) {
			...
			$program_data_qnty_array[$rowPlan[csf('prog_no')]][$rowPlan[csf('status_active')]]['prog_no'] += $rowPlan[csf('program_qnty')];
		}
							...
							<tbody>
								...
								foreach ($nameArray as $row) {
									...
									if($within_group == 1){
										...
										//echo $sales_booking_no ."=".$sales_id."=".$body_part_id."=".$desc."=".$gsm."=".$dia."=".$color_type_id."=".$pre_cost_fabric_cost_dtls_id."=".$status."##<br>";
										...
										if($type!=3){
											$programs_no = array_unique(explode(",", trim($prog_no, ", ")));
											foreach ($programs_no as $program_no_row) {
												$program_qnty += $program_data_qnty_array[$program_no_row][$status]['prog_no'];
											}
										}
									}
											...
											if($type == 3){	
												...
												<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer; <? echo $hidden; ?>" >
													...
													if($type == 3){
														...
														$plan_id = implode("_",array_unique(explode(",", rtrim($plan_id,', '))));
														?>
														<td>xxx<span class="formbutton" title="Click to Active" onClick="activePlan(this,'<? echo $plan_id; ?>','<? echo $prog_no; ?>','<? echo $balance_qnty; ?>','<? echo $sales_order_dtls_id; ?>','<? echo $grey_qty; ?>')">Active</span>
														</td>
														<? 
													} 
													?>
...
if ($action == "prog_qnty_popup") 
{
	...
	<script>
		...
		function fnc_program_entry_Reply_info()
		{
			if (http.readyState == 4)
			{
				...
				if ((reponse[0] == 0 || reponse[0] == 1 || reponse[0] == 2))
				{
					...
					{
						reset_form('programQnty_1', '', '', 'txt_program_date,<? echo $current_date;?>', '', 'hdn_booking_qnty*cbo_dia_width_type*hdnVariableCollarCuff');
					}
					
					$('#updateId').val(reponse[1]);
					show_list_view(reponse[1], 'planning_info_details', 'list_view', 'planning_info_entry_for_sales_order_urmi_controller', '');
		...
		function openpage_feeder() {
			...
			var page_link = 'planning_info_entry_for_sales_order_urmi_controller.php?action=feeder_info_popup&no_of_feeder_data=' + no_of_feeder_data + '&pre_cost_id=' + '<? echo $pre_cost_id; ?>'+'&hidden_color_id='+hidden_color_id+ '&sales_order_dtls_id=' + '<? echo $sales_order_dtls_id; ?>'+ '&within_group=' + '<? echo $within_group; ?>';    
		...
		function openpage_collarCuff() {
			...
			var page_link = 'planning_info_entry_for_sales_order_urmi_controller.php?action=collarCuff_info_popup&collarCuff_data=' + collarCuff_data + '&pre_cost_id=' + '<? echo $pre_cost_id; ?>&update_dtls_id=' + update_dtls_id +'&body_part_id='+'<? echo $body_part_id; ?>&booking_no=' + '<? echo $booking_no; ?>&bodyPartType=' + bodyPartType;
		...
		function openpage_cam_design() {
			...
			var page_link = 'planning_info_entry_for_sales_order_urmi_controller.php?action=cam_design_info_popup&hidden_came_dsign_string_data=' + came_dsign_string_data + '&update_dtls_id='+updateDtlsId;
		...
		function openpage_advice() {
			...
			var page_link = 'planning_info_entry_for_sales_order_urmi_controller.php?action=advice_info_popup&hidden_advice_data=' + hidden_advice_data;
		...
		function openpage_count_feeding()
		{
			...
			var page_link = 'planning_info_entry_for_sales_order_urmi_controller.php?action=count_feeding_data_popup&count_feeding_data=' + count_feeding_data + '&pre_cost_id=' + '<? echo $pre_cost_id; ?>&update_dtls_id=' + update_dtls_id + '&job_id=' + '<? echo $job_id; ?>';
		...
		<fieldset style="width:900px; margin-top:5px;">
			...
			<table width="900" align="center" border="0">
				<tr>
					...
					<td>
						<?
						echo create_drop_down("cbo_knitting_source", 152, $knitting_source, "", 1, "-- Select --", 1, "active_inactive();load_drop_down( 'planning_info_entry_for_sales_order_urmi_controller', this.value+'**'+$companyID, 'load_drop_down_knitting_party','knitting_party');", 0, '1,3');
						?>
					</td>
					...
					<td id="knitting_party">
						...
						//load_drop_down( 'planning_info_entry_for_sales_order_urmi_controller', this.value, 'load_drop_down_location','location_td');
						?>
					</td>
					<td>Color</td>
					<td>
			...
			<div id="list_view" style="margin-top:5px">
				...
				if (str_replace("'", '', $plan_id) != "") {
					?>
					<script>
						show_list_view('<? echo str_replace("'", '', $plan_id); ?>', 'planning_info_details', 'list_view', 'planning_info_entry_for_sales_order_urmi_controller', '');
					</script>
					<?
				}
				?>
...
<script type="text/javascript">
	...
	load_drop_down( 'planning_info_entry_for_sales_order_urmi_controller', 1+'**'+ <?php echo $companyID; ?>,'load_drop_down_knitting_party','knitting_party');
	load_drop_down( 'planning_info_entry_for_sales_order_urmi_controller',  $('#cbo_knitting_party').val(),'load_drop_down_location','location_td');
					...
					<div style="width:600px; overflow-y:scroll; max-height:210px;" id="buyer_list_view" align="left">
						...
						id="tbl_list_search">
							<tbody>
								...
								{
									$sql = " select b.fabric_color as fabric_color_id,b.body_part as body_part_id,f.size_id as size_number_id,sum(b.grey_fabric) as qnty ,null as order_quantity, 
									null as plan_cut_qnty,null as colar_cuff_per,e.body_part_type ,null as colar_excess_percent, null as cuff_excess_percent ,f.total_qty ,c.sample_color as gmts_color,d.collar_cuff_breakdown 
									...
									and b.body_part=e.id and a.booking_no='$booking_no' and a.company_id=$companyID and a.item_category=2 and b.fabric_color=$colorId   
									and $dia_cond2  and b.body_part in($body_part_id) and b.lib_yarn_count_deter_id in ($determination_id) 
									and a.booking_type=4 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0  and f.status_active=1 and f.is_deleted=0 
									group by b.fabric_color,b.body_part ,e.body_part_type ,f.size_id,f.total_qty ,c.sample_color,d.collar_cuff_breakdown";
								...
								foreach ($result as $row)
								{
									...
									if($bookingWithoutOrder==1)
									{
										$finishedSize=explode("_",$row[csf('collar_cuff_breakdown')]);
										$finshSizeId=$finishedSize[2];
									}
									else
									{
								    ...
								    if($bookingWithoutOrder==1)
									{
										$finishedQntPcs=explode("_",$row[csf('collar_cuff_breakdown')]);
										$collerqty=$finishedQntPcs[3];
										if($kg_wise_total_qnty>0)
										{
											$kg_wise_total_qnty=$kg_wise_total_qnty;
										}
										else
										{
											$kg_wise_total_qnty=$row[csf('total_qty')];	
										}
	...
	<div style="width:1110px; max-height:140px; overflow-y:scroll" id="list_container_batch" align="left">
		...
		foreach ($result as $row) {
			...
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
				onClick="get_php_form_data('<? echo $row[csf('id')]."**".$row[csf('salesDtlsId_wise_bodyPartId')]; ?>','populate_data_from_planning_info', 'planning_info_entry_for_sales_order_urmi_controller' );balance_cal();">
...
if ($action == "populate_data_from_planning_info")
{
	...
	foreach ($data_array as $row) 
	{
		...
		echo "load_drop_down('planning_info_entry_for_sales_order_urmi_controller', " . $row[csf("knitting_source")] . "+'**'+" . $row[csf("knitting_party")] . "+'**1', 'load_drop_down_knitting_party','knitting_party');\n";
		...
		echo "load_drop_down('planning_info_entry_for_sales_order_urmi_controller', " . $row[csf("knitting_party")] . ", 'load_drop_down_location','location_td');\n";
...
if ($action == "count_feeding_data_popup")
{
	...
	<script>
		...
		function browse_desc_popup(sl,salesId) {
			//var companyID = $("#cbo_company_name").val();
			//var buyerID = $("#cbo_buyer_name").val();
			var page_link = 'planning_info_entry_for_sales_order_urmi_controller.php?action=browse_desc_popup_action&sl=' + sl + '&salesId=' + salesId;
	...
	<script>
		...
		function fn_add_date_field(row_no) {
			...
			if ($('#txt_startDate_' + row_no).val() != "") {
				...
				var response = return_global_ajax_value(data, 'date_duplication_check', '', 'planning_info_entry_for_sales_order_urmi_controller');
		...
		function fn_machine_book_dates(row_no)
		{
			...
			if(machine_id!="" && machine_id!="undefined")
			{
				...
				$.ajax({
					type: "POST",
					url: "planning_info_entry_for_sales_order_urmi_controller.php?action=machine_allready_book_dates",
					data: data,
					cache: false,
					dataType: "json",
				...
				<tbody>
					<tr>
						...
						<td align="center">
							...
							onClick="show_list_view ('<? echo $companyID; ?>**' +'<? echo $buyerID; ?>'+'**'+document.getElementById('cbo_po_buyer_name').value + '**'+'<? echo $within_group; ?>**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**', 'create_job_search_list_view', 'search_div', 'planning_info_entry_for_sales_order_urmi_controller', 'setFilterGrid(\'tbl_list_search\',-1)');"
							style="width:90px;"/>
						</td>
					</tr>
				</tbody>
			...
			<fieldset style="width:100%;">
				...
				<table cellpadding="0" cellspacing="0" width="835" class="rpt_table" border="1" rules="all">
					...
					<tr>
						...
						<td align="center">
							<input type="button" name="button2" class="formbutton" value="Show"
							onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('cbo_po_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_within_group').value+'_'+document.getElementById('cbo_booking_type').value, 'create_booking_search_list_view', 'search_div', 'planning_info_entry_for_sales_order_urmi_controller', 'setFilterGrid(\'tbl_list_search\',-1);')"
							style="width:90px;"/>
						</td>
					</tr>
					<tr>
...
if ($action == "load_drop_down_knitting_party")
{
	$data = explode("**", $data);
	if ($data[0] == 1) {
		echo create_drop_down("cbo_knitting_party", 177, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "--Select Knit Party--", $data[1], "load_drop_down( 'planning_info_entry_for_sales_order_urmi_controller', this.value, 'load_drop_down_location','location_td');", "");
	} else if ($data[0] == 3) {
		if ($data[2] == 1) $selected_id = $data[1]; else $selected_id = 0;
		echo create_drop_down("cbo_knitting_party", 177, "select a.id, a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and b.party_type=20 and a.status_active=1 and a.is_deleted=0 group by a.id, a.supplier_name order by a.supplier_name", "id,supplier_name", 1, "--Select Knit Party--", $selected_id, "load_drop_down( 'planning_info_entry_for_sales_order_urmi_controller', this.value, 'load_drop_down_location','location_td');");
	} else {
		echo create_drop_down("cbo_knitting_party", 177, $blank_array, "", 1, "--Select Knit Party--", 0, "load_drop_down( 'planning_info_entry_for_sales_order_urmi_controller', this.value, 'load_drop_down_location','location_td');");
	}
	exit();
}
...
echo $data .'=';
	...
	echo $prog_no .'='.$plan_id.'='.$program_qnty.'='.$dtls_ids; die;
