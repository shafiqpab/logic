...
if ($action == "booking_item_details_popup")
{
	...
	function show_details(type) {
		...
		freeze_window(5);
		http.open("POST", "planning_info_entry_for_sales_order_v2_admin_controller.php", true);
		http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		http.send(data);
		http.onreadystatechange = fn_show_details_reponse;
		//show_list_view(cbo_company_id, 'booking_item_details', 'list_container_fabric_desc', 'requires/planning_info_entry_for_sales_order_v2_admin_controller', '');
	}
	...
	function openmypage_job() {
		...
		var page_link = 'planning_info_entry_for_sales_order_v2_admin_controller.php?action=style_ref_search_popup&companyID=' + companyID + '&buyerID=' + buyerID;
	...
	function openmypage_booking() {
		...
		var companyID = $("#cbo_company_name").val();
		var page_link = 'planning_info_entry_for_sales_order_v2_admin_controller.php?action=booking_no_search_popup&companyID=' + companyID;
	...
	function openmypage_prog() {
		...
		var page_link = 'planning_info_entry_for_sales_order_v2_admin_controller.php?action=prog_qnty_popup&gsm=' + gsm + '&dia=' + dia + '&desc=' + desc + '&within_group=' + within_group + '&job_id=' + job_id + '&booking_qnty=' + booking_qnty + '&companyID=' + companyID + '&data="' + data + '"' + '&plan_id=' + plan_id + '&determination_id=' + determination_id + '&booking_no=' + booking_no + '&body_part_id=' + body_part_id + '&fabric_type=' + fabric_typee + '&pre_cost_id=' + pre_cost_id + '&buyer_id=' + buyer_id + '&job_dtls_id=' + job_dtls_id + '&color_type_id=' + color_type_id + '&sales_order_dtls_id=' + sales_order_dtls_id;
 ...
 function selected_row(rowNo) {
	...
	function delete_prog() {
		...
		http.open("POST", "requires/planning_info_entry_for_sales_order_v2_admin_controller.php", true);
		http.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		http.send(data);
	...
	function active_inactive() {
		...
		if (company_id == 0) {
			...
			load_drop_down('planning_info_entry_for_sales_order_v2_admin_controller', company_id + '_' + within_group, 'load_drop_down_buyer', 'buyer_td');
		}
	}
...
if ($action == "booking_item_details") 
{
	...
	else
	{
		...
		foreach($sqlRevisedRslt as $row)
		{
			...
			$revisedDataArr[$row[csf('dia')]][$row[csf('booking_no')]][$row[csf('body_part_id')]][$row[csf('color_type_id')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]][$row[csf('width_dia_type')]]['plan_no'] = $row[csf('mst_id')];
			$revisedDataArr[$row[csf('dia')]][$row[csf('booking_no')]][$row[csf('body_part_id')]][$row[csf('color_type_id')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]][$row[csf('width_dia_type')]]['program_no'][] = $row[csf('id')];
			$revisedDataArr[$row[csf('dia')]][$row[csf('booking_no')]][$row[csf('body_part_id')]][$row[csf('color_type_id')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]][$row[csf('width_dia_type')]]['booking_date'] = $row[csf('booking_date')];
			$revisedDataArr[$row[csf('dia')]][$row[csf('booking_no')]][$row[csf('body_part_id')]][$row[csf('color_type_id')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]][$row[csf('width_dia_type')]]['buyer_id'] = ($row[csf('within_group')]==1?$company_arr[$row[csf('buyer_id')]]:$buyer_arr[$row[csf('buyer_id')]]);
			$revisedDataArr[$row[csf('dia')]][$row[csf('booking_no')]][$row[csf('body_part_id')]][$row[csf('color_type_id')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]][$row[csf('width_dia_type')]]['sales_order_no'] = $row[csf('job_no')];
			$revisedDataArr[$row[csf('dia')]][$row[csf('booking_no')]][$row[csf('body_part_id')]][$row[csf('color_type_id')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]][$row[csf('width_dia_type')]]['style_ref_no'] = $row[csf('style_ref_no')];
			
			$revisedDataArr[$row[csf('dia')]][$row[csf('booking_no')]][$row[csf('body_part_id')]][$row[csf('color_type_id')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]][$row[csf('width_dia_type')]]['color_id'][] = $row[csf('color_id')];
			$revisedDataArr[$row[csf('dia')]][$row[csf('booking_no')]][$row[csf('body_part_id')]][$row[csf('color_type_id')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]][$row[csf('width_dia_type')]]['program_qnty'][$row[csf('id')]] += $row[csf('program_qnty')];
			$revisedDataArr[$row[csf('dia')]][$row[csf('booking_no')]][$row[csf('body_part_id')]][$row[csf('color_type_id')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]][$row[csf('width_dia_type')]]['po_id'] = $row[csf('po_id')];
			$revisedDataArr[$row[csf('dia')]][$row[csf('booking_no')]][$row[csf('body_part_id')]][$row[csf('color_type_id')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]][$row[csf('width_dia_type')]]['after_wash_gsm'] = $row[csf('after_wash_gsm')];
		}
		//echo "<pre>";
...
{
	$data = array();
	$sql = "SELECT ID, KNITTING_PARTY, COLOR_ID, COLOR_RANGE, MACHINE_GG, MACHINE_DIA, WIDTH_DIA_TYPE, FABRIC_DIA, STITCH_LENGTH FROM PPL_PLANNING_INFO_ENTRY_DTLS WHERE ID = ".$arr['program_no'];
	$sql_rslt = sql_select($sql);
	//for sql return
	if($arr['sql_query'] == 1)
	{
...
if ($action == "save_update_delete")
{
	...
	else if ($operation == 1)// Update Here
	{
		...
		*/
		...
		$color_id = 0;
		$field_array_update = "knitting_source*knitting_party*color_id*color_range*machine_dia*width_dia_type*machine_gg*fabric_dia*program_qnty*stitch_length*spandex_stitch_length*draft_ratio*machine_id*machine_capacity*distribution_qnty*status*start_date*end_date*program_date*feeder*remarks*save_data*location_id*advice*batch_no*no_of_ply*dye_type*updated_by_without_validation*updated_by*update_date";

		$data_array_update = $cbo_knitting_source . "*" . $cbo_knitting_party . "*" . $hidden_color_id . "*" . $cbo_color_range . "*" . $txt_machine_dia . "*" . $cbo_dia_width_type . "*" . $txt_machine_gg . "*" . $txt_fabric_dia . "*" . $txt_program_qnty . "*" . $txt_stitch_length . "*" . $txt_spandex_stitch_length . "*" . $txt_draft_ratio . "*" . $machine_id . "*" . $txt_machine_capacity . "*" . $txt_distribution_qnty . "*" . $cbo_knitting_status . "*" . $txt_start_date . "*" . $txt_end_date . "*" . $txt_program_date . "*" . $cbo_feeder . "*" . $txt_remarks . "*" . $save_data . "*" . $cbo_location_name . "*" . $hidden_advice_data . "*" . $txt_batch_no . "*" . $txt_no_of_ply . "*" . $txt_dye_type . "*'" .$updatedByAdmin. "'*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'";

		$plan_dtls_id = return_next_id("id", "ppl_planning_entry_plan_dtls", 1);
		$field_array_plan_dtls = "id, mst_id, dtls_id, company_id, within_group, buyer_id, booking_no, po_id, body_part_id, color_type_id, determination_id, fabric_desc, gsm_weight, dia, after_wash_gsm, width_dia_type, color_id, program_qnty, is_sales,sales_order_dtls_ids,pre_cost_fabric_cost_dtls_id, customer_buyer, inserted_by, insert_date,update_sl,updated_by_without_validation";
...
if ($action == "prog_qnty_popup") 
{
	...
	<script>
		...
		function openpage_machine()
		{
			...
			var page_link = 'planning_info_entry_for_sales_order_v2_admin_controller.php?action=machine_info_popup&save_string=' + save_string + '&companyID=' + '<? echo $companyID; ?>' + '&txt_machine_dia=' + txt_machine_dia + '&update_dtls_id=' + update_dtls_id + '&txt_program_qnty='+txt_program_qnty + '&txt_machine_gg='+txt_machine_gg+ '&show_item='+show_item;
		...
		function fnc_program_entry_Reply_info()
		{
			if (http.readyState == 4)
			{
				...
				if ((reponse[0] == 0 || reponse[0] == 1 || reponse[0] == 2))
				{
					...
					show_list_view(reponse[1], 'planning_info_details', 'list_view', 'planning_info_entry_for_sales_order_v2_admin_controller', '');
					set_button_status(1, permission, 'fnc_program_entry', 1);
		...
		function openpage_feeder()
		{
			...
			var page_link = 'planning_info_entry_for_sales_order_v2_admin_controller.php?action=feeder_info_popup&no_of_feeder_data=' + no_of_feeder_data + '&pre_cost_id=' + '<? echo $pre_cost_id; ?>'+'&hidden_color_id='+hidden_color_id;
		...
		function openpage_collarCuff()
		{
			...
			var page_link = 'planning_info_entry_for_sales_order_v2_admin_controller.php?action=collarCuff_info_popup&collarCuff_data=' + collarCuff_data + '&mst_id=' + '<? echo $job_id; ?>' + '&color_type_id=' + '<? echo $color_type_id; ?>&update_dtls_id=' + update_dtls_id +'&body_part_id='+'<? echo $body_part_id; ?>';
		...
		function openpage_collarCuff_03112021()
		{
			...
			var page_link = 'planning_info_entry_for_sales_order_v2_admin_controller.php?action=collarCuff_info_popup&collarCuff_data=' + collarCuff_data + '&pre_cost_id=' + '<? echo $pre_cost_id; ?>&update_dtls_id=' + update_dtls_id +'&body_part_id='+'<? echo $body_part_id; ?>&booking_no=' + '<? echo $booking_no; ?>&bodyPartType=' + bodyPartType;
			var title = 'Collar & Cuff Measurement Info';
			emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=840px,height=300px,center=1,resize=1,scrolling=0', '../');
			emailwindow.onclose = function ()
			{
		...
		function openpage_cam_design()
		{
			...
			var page_link = 'planning_info_entry_for_sales_order_v2_admin_controller.php?action=cam_design_info_popup&hidden_came_dsign_string_data=' + came_dsign_string_data + '&update_dtls_id='+updateDtlsId;
		...
		function openpage_advice()
		{
			...
			var page_link = 'planning_info_entry_for_sales_order_v2_admin_controller.php?action=advice_info_popup&hidden_advice_data=' + hidden_advice_data;
		...
		function openpage_count_feeding()
		{
			...
			var page_link = 'planning_info_entry_for_sales_order_v2_admin_controller.php?action=count_feeding_data_popup&count_feeding_data=' + count_feeding_data + '&pre_cost_id=' + '<? echo $pre_cost_id; ?>&update_dtls_id=' + update_dtls_id;
		...
		<fieldset style="width:900px; margin-top:5px;">
			...
			<table width="900" align="center" border="0">
				<tr>
					...
					<td>
						<?
						echo create_drop_down("cbo_knitting_source", 152, $knitting_source, "", 1, "-- Select --", 1, "active_inactive();load_drop_down( 'planning_info_entry_for_sales_order_v2_admin_controller', this.value+'**'+$companyID, 'load_drop_down_knitting_party','knitting_party');", 1, '1,3');
						?>
					</td>
					<td>Knitting Party</td>
					<td id="knitting_party">
						<?
						echo create_drop_down("cbo_knitting_party", 177, $blank_array, "", 1, "--Select Knit Party--", 1, "",1);
						?>
					</td>
					<td>Color</td>
					<td>
				...
				<tr>
					...
					<td>
						<?
						echo create_drop_down("cbo_color_range", 152, $color_range, "", 1, "-- Select --", 0, "",1);
						?>
					</td>
					...
					<td>
						...
						style="width:60px;" maxlength="3" title="Maximum 3 Character" value="" disabled />
						<?
						echo create_drop_down("cbo_dia_width_type", 100, $fabric_typee, "", 1, "-- Select --", $fabric_type, "",1);
						?>
					</td>
				...
				<tr>
					...
					<td>
						...
						style="width:140px;" value="<? echo $dia; ?>" disabled/>
					</td>
				...
				<tr>
					...
					<td>
						...
						class="text_boxes" style="width:165px;" disabled />
					</td>
					<td>Draft Ratio</td>
					<td>
				...
				<tr>
					...
					<td>
						...
						onDblClick="openpage_machine();" readonly disabled />
				...
				<tr>
					...
					<td>
						...
						style="width:140px" value="<? echo $start_date; ?>" readonly disabled>
					</td>
					<td>End Date</td>
					<td>
						<input type="text" name="txt_end_date" id="txt_end_date" class="datepicker"
						style="width:165px" value="<? echo $end_date; ?>" readonly disabled>
					</td>
					<td>Status</td>
					<td>
						<?
						echo create_drop_down("cbo_knitting_status", 152, $knitting_program_status, "", 1, "--Select Status--", 0, "",1);
						?>
					</td>
				</tr>
				<tr>
                    ...
                    <td id="location_td">
                        <?
                        echo create_drop_down("cbo_location_name", 152, "select id,location_name from lib_location where company_id='$companyID' and status_active =1 and is_deleted=0 order by location_name", "id,location_name", 1, "-- Select Location --", 0, "",1);
                        ?>
                    </td>
					<td>
					...
					<td>
						...
						echo create_drop_down("cbo_feeder", 152, $feeder, "", 1, "--Select Feeder--", 0, "",1);
						?>
					</td>
                    <td colspan="4">
					...
					<tr>
						<td>Batch No</td>
						<td><input type="text" name="txt_batch_no" id="txt_batch_no" class="text_boxes" style="width:140px" disabled></td>
                    </tr>
					<tr>
						<td> Dye Type</td>
						<td><input type="text" name="txt_dye_type" id="txt_dye_type" class="text_boxes" style="width:140px" disabled></td>
                    </tr>
                    <tr>
						<td id="td_no_of_ply">No Of Ply</td>
						<td><input type="text" name="txt_no_of_ply" id="txt_no_of_ply" class="text_boxes_numeric" style="width:140px" disabled></td>
					</tr>
					<tr>
						<td colspan="4" align="right" class="button_container">
							<?
							echo load_submit_buttons($permission, "fnc_program_entry", 1, 0, "reset_form('programQnty_1','','','txt_start_date,$start_date*txt_end_date,$end_date*txt_program_date,$current_date','','updateId*txt_color')", 1);
							?>
						</td>
			...
			<div id="list_view" style="margin-top:5px">
				...
				if (str_replace("'", '', $plan_id) != "") {
					?>
					<script>
						show_list_view('<? echo str_replace("'", '', $plan_id); ?>', 'planning_info_details', 'list_view', 'planning_info_entry_for_sales_order_v2_admin_controller', '');
					</script>
					<?
				}
				?>
...
<script type="text/javascript">
	...
	load_drop_down( 'planning_info_entry_for_sales_order_v2_admin_controller', 1+'**'+ <?php echo $companyID; ?>,'load_drop_down_knitting_party','knitting_party');
	$('#cbo_knitting_party').attr('disabled', 'disabled');
</script>
</html>
<?
exit();
...
if ($action == "planning_info_details")
{
	...
	<div style="width:1050px; max-height:140px; overflow-y:scroll" id="list_container_batch" align="left">
		...
		foreach ($result as $row) {
			...
			?>
			<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
				onClick="get_php_form_data(<? echo $row[csf('id')]; ?>,'populate_data_from_planning_info', 'planning_info_entry_for_sales_order_v2_admin_controller' );balance_cal();">
...
if ($action == "populate_data_from_planning_info")
{
	...
	foreach ($data_array as $row) 
	{
		...
		echo "load_drop_down('planning_info_entry_for_sales_order_v2_admin_controller', " . $row[csf("knitting_source")] . "+'**'+" . $row[csf("knitting_party")] . "+'**1', 'load_drop_down_knitting_party','knitting_party');\n";
	...
	echo "$('#txt_machine_no').attr('disabled', 'disabled');\n";
	exit();
}
	...
	<script>
		...
		function fn_add_date_field(row_no) {
			...
			if ($('#txt_startDate_' + row_no).val() != "") {
				...
				var response = return_global_ajax_value(data, 'date_duplication_check', '', 'planning_info_entry_for_sales_order_v2_admin_controller');
		...
		function fn_machine_book_dates(row_no)
		{
			...
			if(machine_id!="" && machine_id!="undefined")
			{
				...
				$.ajax({
					type: "POST",
					url: "planning_info_entry_for_sales_order_v2_admin_controller.php?action=machine_allready_book_dates",
					data: data,
					cache: false,
					dataType: "json",
				...
				<tbody>
					<tr>
						...
						<td align="center">
							...
							onClick="show_list_view ('<? echo $companyID; ?>**' +'<? echo $buyerID; ?>'+'**'+document.getElementById('cbo_po_buyer_name').value + '**'+'<? echo $within_group; ?>**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**', 'create_job_search_list_view', 'search_div', 'planning_info_entry_for_sales_order_v2_admin_controller', 'setFilterGrid(\'tbl_list_search\',-1)');"
							style="width:90px;"/>
						</td>
					</tr>
				</tbody>
				...
				<table cellpadding="0" cellspacing="0" width="835" class="rpt_table" border="1" rules="all">
					...
					<tr>
						...
						<td align="center">
							...
							onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('cbo_po_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_within_group').value+'_'+document.getElementById('cbo_booking_type').value, 'create_booking_search_list_view', 'search_div', 'planning_info_entry_for_sales_order_v2_admin_controller', 'setFilterGrid(\'tbl_list_search\',-1);')"
							style="width:90px;"/>
						</td>
					</tr>
					<tr>
