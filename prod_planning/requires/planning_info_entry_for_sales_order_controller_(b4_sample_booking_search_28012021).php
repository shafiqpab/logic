...
if ($action == "booking_item_details_16092020")
{
	$process = array(&$_POST);
	extract(check_magic_quote_gpc($process));
										...
										foreach($fabDescArr as $gsm=>$gsmArr)
										{
											foreach($gsmArr as $widthDia=>$row)
											{
                                                ...
                                                <tr>
                                                    ...
                                                    <input id="promram_id_<? echo $i; ?>" name="promram_id[]" type="hidden" value="<? echo trim($program_id); ?>"/>
                                                    </td>
                                                    <td><?php echo $i;?></td>
...
if ($action == "booking_item_details") 
{
	...
	if ($within_group == 1) {
		if ($buyer_name == 0) $buyer_id_cond_to = ""; else $buyer_id_cond_to = " and a.buyer_id=$buyer_name";
	}
			...
			<fieldset>
				...
				<tbody>
					...
					foreach ($nameArray as $row) {
						...
						$knitting_production = sql_select("select sum(b.grey_receive_qnty) as qnty from inv_receive_master a, pro_grey_prod_entry_dtls b where a.id=b.mst_id and a.entry_form=2 and a.booking_id=".$program_id." and a.item_category=13");
						...
						if (!in_array($dia, $dia_array)) {
							...
							<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer"
                                ...
                                <!--<td align="right" id="booking_qnty_<? //echo $i; ?>" width="70"><? //echo number_format($grey_qty,2,'.',''); ?></td>-->
                                <td align="right" width="70">
	...
	else 
	{
		...
		//for grey qty
		$sqlGreyQty = " SELECT a.id, a.sales_booking_no, b.body_part_id, b.color_type_id, b.fabric_desc, b.gsm_weight, b.dia, sum(b.grey_qty) as grey_qty from fabric_sales_order_mst a, fabric_sales_order_dtls b where a.id=b.mst_id $active_status_sql and a.company_id=$company_name ".where_con_using_array($salesMstIdArr,0,'a.id')." ".where_con_using_array($salesDtlsIdArr,0,'b.id')." 
		group by a.id, a.company_id, a.within_group, a.job_no, a.sales_booking_no, a.booking_id, a.buyer_id, a.style_ref_no, a.booking_date, b.body_part_id, b.color_type_id, b.fabric_desc, b.determination_id, b.gsm_weight, b.dia, b.width_dia_type";
		//echo $sqlGreyQty;
		...
		foreach($sqlGreyQtyRslt as $row)
		{
			...
			$greyQtyData[$row[csf('sales_booking_no')]][$row[csf('id')]][$row[csf('body_part_id')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]][$row[csf('dia')]][$row[csf('color_type_id')]]['grey_qty'] += $row[csf('grey_qty')];
		}
		//echo "<pre>";
				...
				<table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" id="tbl_list_search">
					...
					<tbody>
						...
						foreach ($nameArray as $row)
						{
							...
							//for grey qty
							//$grey_qty = $row[csf('grey_qty')];
							$grey_qty = $greyQtyData[$sales_booking_no][$sales_id][$body_part_id][$desc][$gsm][$dia][$color_type_id]['grey_qty'];
...
if ($action == "save_update_delete")
{
	...
	if ($operation == 0)  // Insert Here
	{
		...
		if (str_replace("'", '', $updateId) == "") {
			...
			$data_array = "(" . $id . "," . $companyID . "," . $within_group . "," . $buyer_id . ",'" . $booking_no . "'," . $body_part_id . "," . $color_type_id . "," . $determination_id . "," . $hdn_fab_desc . ",'" . $gsm . "','" . $dia . "'," . $fabric_typee . ",1," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
		} else {
...
if ($action == "prog_qnty_popup") 
{
	...
	<script>
		...
		function openpage_feeder() {
			...
			var page_link = 'planning_info_entry_for_sales_order_controller.php?action=feeder_info_popup&no_of_feeder_data=' + no_of_feeder_data + '&pre_cost_id=' + '<? echo $pre_cost_id; ?>'+'&hidden_color_id='+hidden_color_id;
...
if ($action == "prog_info_print")
{
	...
	if($program_id!="")
	{
		...
		where a.id=b.po_id and b.dtls_id=c.id and b.dtls_id=".$program_id."");
	}
	//echo "<pre>";
