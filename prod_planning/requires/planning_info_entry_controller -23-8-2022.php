        ...
        function fnc_program_entry_Reply_info() {
        	if (http.readyState == 4) {
                ...
                if ((reponse[0] == 0 || reponse[0] == 1 || reponse[0] == 2)) {
                	...
                	reset_form('programQnty_1', '', '', 'txt_start_date,<? echo $start_date; ?>*txt_end_date,<? echo $end_date;?>*txt_program_date,<? echo $current_date;?>', '', 'hdn_booking_qnty*txt_fabric_dia*hdnVariableCollarCuff');
		...
		function fnc_close() 
		{
			...
			$("#tbl_list_search").find('tbody tr').not(":first").each(function () 
			{
				...
				if(txtSizePerKgQty>0 || (coloProgUpdateId !=0 && updateQty > 0))
				{
					...
					if (save_string == "")
					{
						save_string = txtColorId + "_" + txtGreySizeId + "_" + txtFinishSizeId + "_" + txtSizePerKgQty+ "_" + hidden_size_tot_kg+ "_" + coloProgUpdateId+ "_" + txtBodyPartId+sizeProgUpdatePrimaryIds;
						color_name_string = txtColorName;
						color_id_string = txtColorId;
							...
							<tbody>
								...
								if($prog_no!="")
								{
									...
									GROUP BY b.id, b.plan_id, b.program_no, b.color_id, b.color_prog_qty";*/
									 ...
									 $size_plan_sql = "select b.id, b.plan_id, b.program_no, b.color_id,c.id as size_tbl_id,c.grey_size_id,c.finish_size_id,c.per_kg,c.kg_wise_total_qnty,c.body_part_id 
									from ppl_planning_entry_plan_dtls a, ppl_color_wise_break_down b , ppl_size_wise_break_down c
									where a.dtls_id = b.program_no and a.mst_id = b.plan_id and b.id=c.color_wise_mst_id and b.program_no=c.program_no and b.plan_id=c.plan_id and b.color_id=c.color_id and a.po_id in(".$po_id.") and b.plan_id =".$plan_id." and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.program_no=$prog_no and b.id=$color_wise_break_down_mst_id
									GROUP BY b.id, b.plan_id, b.program_no, b.color_id,c.id,c.grey_size_id,c.finish_size_id,c.per_kg,c.kg_wise_total_qnty,c.body_part_id";
								...
								foreach ($result as $row) 
								{
									...
									<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>" onClick="js_set_value(<? echo $i; ?>)"> 
										...
										<td width="40" align="center"><? echo $i; ?>								
											...
											<input type="text" name="size_prog_update_primary_id[]" id="size_prog_update_primary_id_<?php echo $i; ?>" value="<? echo  $size_primary_update_id= ($size_prog_update_primary_id!="")?$size_prog_update_primary_id:"0"; ?>"/>
				...
				for ($inc = 0; $inc < count($size_wise_prog_datas); $inc++) 
				{
					...
					for ($i = 0; $i < count($size_wise_prog_dataz); $i++) 
					{
						if($size_wise_prog_dataz[$i] != '')
						{
							...
							$size_kg_wise_prog_qty = $sizeWiseProgData[4];
							$size_wise_body_part_id = $sizeWiseProgData[6];
							$size_wise_primary_update_id = $sizeWiseProgData[7];
		...
		else
		{
			...
			if ($data_array_size_wise_break_down != "") {
				...
				foreach ($color_wise_mstID as $colorIDS => $colorMstID) {
					...
					{
						//echo "10**delete from ppl_size_wise_break_down where color_wise_mst_id=$colorMstID and id=$sizeWiseMstId";
						$delete_size_tbl = execute_query("delete from ppl_size_wise_break_down where color_wise_mst_id=$colorMstID and id=$sizeWiseMstId", 0);
					}
