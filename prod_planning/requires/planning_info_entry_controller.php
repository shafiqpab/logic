...
if ($action == "load_drop_down_knitting_party_new")
{
	...
	else if ($data[0] == 3)
	{
		...
		echo create_drop_down("cbo_knitting_party", 152, "$sql", "id,supplier_name", 1, "--Select Knit Party--", $selected_id, "load_drop_down( 'planning_info_entry_controller', this.value, 'load_drop_down_location', 'location_td' ); getValutAttention(this.value);");
	}
	else
	{
		echo create_drop_down("cbo_knitting_party", 152, $blank_array, "", 1, "--Select Knit Party--", 0, "load_drop_down( 'planning_info_entry_controller', this.value, 'load_drop_down_location', 'location_td' ); getValutAttention(this.value);");
...
if ($action == "prog_qnty_popup")
{
	...
	<script>
        ...
        function openpage_collarCuff() {
        	...
        	var page_link = 'planning_info_entry_controller.php?action=collarCuff_info_popup&collarCuff_data=' + collarCuff_data + '&pre_cost_id=' + '<? echo $pre_cost_id; ?>&update_dtls_id=' + update_dtls_id +'&hidden_bodypartID_data='+hidden_bodypartID_data+ '&bookingNo=' + '<? echo $booking_no; ?>'+ '&body_part_id=' + '<? echo $body_part_id; ?>'; 
...
if ($action == "populate_data_from_planning_info")
{
	...
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down('planning_info_entry_controller', " . $row[csf("knitting_source")] . "+'**'+" . $row[csf("knitting_party")] . "+'**1', 'load_drop_down_knitting_party_new','knitting_party');\n";
