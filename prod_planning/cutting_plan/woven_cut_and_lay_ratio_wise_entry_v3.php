<?
/*-------------------------------------------- Comments
Purpose			: Woven Cut and Lay Entry Ratio Wise V3
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Woven Cut and Lay Entry Ratio Wise V3","../../", 1, 1, $unicode,'','');
...
<script>
	...
	function add_break_down_tr(i)
   	{ 
		...
		{ 
			...
			//$('#cbocolor_'+i).removeAttr("onchange").attr("onchange","reset_fld("+i+");check_color("+i+");");
			$('#cbocolor_'+i).removeAttr("onDblClick").attr("onDblClick","openmypage_color("+i+");");
		...
		if(cbocolor*1>0)
		{
			for(var j=1; j<=row_num; j++)
			{
				...
				else
				{
					...
					if(cbocolor==cbocolor_check)
					{
						alert("Same Color Not Allow.");
						$('#cbocolor_'+row_id).val(0);
						return;
					}
				}
			}
	...
	function openmypage_jobNo(id)
	{
		...
		var page_link = 'requires/woven_cut_and_lay_ratio_wise_entry_v3_controller.php?cbo_company_id='+cbo_company_id+'&action=job_search_popup';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1330px,height=400px,center=1,resize=0,scrolling=0','../');
		emailwindow.onclose=function()
		{
			...
			// load_drop_down('requires/woven_cut_and_lay_ratio_wise_entry_v3_controller', job_no[0]+'_'+1, 'load_drop_down_color','color_1');
			load_drop_down('requires/woven_cut_and_lay_ratio_wise_entry_v3_controller', job_no[0]+'_'+1, 'load_drop_down_order_garment','garment_1');
	...
	function openmypage_color(row_no)
	{
		var cbo_company_id = $('#cbo_company_name').val();
		var txt_job=$("#txt_job_no").val();
		var cbocolor=$('#colorId_'+row_no).val();
		...
		var title = 'Color Selection Form';	
		var page_link = 'requires/woven_cut_and_lay_ratio_wise_entry_v3_controller.php?txt_job='+txt_job+'&cbocolor='+cbocolor+'&company_id='+cbo_company_id+'&action=color_popup';
		...
		emailwindow.onclose=function()
		{
			...
			var color_id=this.contentDoc.getElementById("hidden_search_id").value;
			var color_name=this.contentDoc.getElementById("hidden_search_name").value;

			$('#colorId_'+row_no).val(color_id);
			$('#cbocolor_'+row_no).val(color_name);
	...
	function openmypage_po(row_no)
	{
		...
		var poId = $('#poId_'+row_no).val();
		var gmt_id = $('#cbogmtsitem_'+row_no).val();
		var hiddisorderseq=$("#hiddisorderseq_"+row_no).val();
		var posecno=$("#posecno_"+row_no).val();
		
		if(form_validation('txt_job_no*colorId_'+row_no+'*cbogmtsitem_'+row_no,'Job No.*Color*Gmts Item')==false)
		{
			return;
		}
		
		var title = 'PO Selection Form';	
		var page_link = 'requires/woven_cut_and_lay_ratio_wise_entry_v3_controller.php?txt_job='+txt_job+'&cbocolor='+cbocolor+'&gmt_id='+gmt_id+'&poId='+poId+'&company_id='+cbo_company_id+'&hiddisorderseq='+hiddisorderseq+'&posecno='+posecno+'&action=po_popup';
		...
		emailwindow.onclose=function()
		{
			...
			var po_sec_no=this.contentDoc.getElementById("po_sec_no").value;
			
			$('#poId_'+row_no).val(po_id);
			$('#cboPoNo_'+row_no).val(po_no);
			$('#cboCountry_'+row_no).val('');
			$('#countryId_'+row_no).val('');
			$("#hiddisorderseq_"+row_no).val(is_seq);
			$("#posecno_"+row_no).val(po_sec_no);
			
			
			load_drop_down('requires/woven_cut_and_lay_ratio_wise_entry_v3_controller', po_id+"_"+cbocolor+"_"+row_no, 'load_drop_down_batch', 'batch_'+row_no);
			
			
			load_drop_down( 'requires/woven_cut_and_lay_ratio_wise_entry_v3_controller', po_id+"_"+row_no+"_"+cbocolor+"_"+gmt_id, 'load_drop_down_color_type', 'colorTypeId_'+row_no);
			...
			get_php_form_data( po_id+"_"+gmt_id+"_"+cbocolor+"_"+row_no, "load_drop_down_order_qty", "requires/woven_cut_and_lay_ratio_wise_entry_v3_controller" );
		}
	}
	...
	function openmypage_country1()
	{
		...
		var title = 'PO Selection Form';	
		var page_link = 'requires/woven_cut_and_lay_ratio_wise_entry_v3_controller.php?hidden_body_partstring='+hidden_body_partstring+'&action=country_popup';
	...
	{
		var hidden_country_id=$("#countryId_"+row_no).val();
		// alert(hidden_country_id)
		// var cbo_company_id = $('#cbo_company_name').val();
		// var txt_job=$("#txt_job_no").val();
		...
		// alert(country_id)
		var hiddiscountryseq=$("#hiddiscountryseq_"+row_no).val();
		var title = 'Country Selection Form';	
		var page_link = 'requires/woven_cut_and_lay_ratio_wise_entry_v3_controller.php?hidden_country_id='+hidden_country_id+'&action=country_popup'+'&txt_job='+txt_job+'&poId='+poId+'&company_id='+cbo_company_id+'&gmt_id='+gmt_id+'&cbocolor='+cbocolor+'&hiddiscountryseq='+hiddiscountryseq+'&action=country_popup';
		...
		emailwindow.onclose=function()
		{
			...
			$("#countryName_"+row_no).val(country_name);
			$("#hiddiscountryseq_"+row_no).val(is_seq);
			get_php_form_data(poId+"_"+gmt_id+"_"+cbocolor+"_"+row_no+"_"+country_id, "load_drop_down_order_qty_with_country", "requires/woven_cut_and_lay_ratio_wise_entry_v3_controller" );
		}
	}
    ...
    { 
		...
		load_drop_down( 'requires/woven_cut_and_lay_ratio_wise_entry_v3_controller', value+"_"+ship_id, 'load_drop_down_ship', ship_id);
		load_drop_down( 'requires/woven_cut_and_lay_ratio_wise_entry_v3_controller', value+"_"+gmt_id, 'load_drop_down_order_garment', gmt_id);
	...
	function change_color(id,value)
	{
		...
		load_drop_down('requires/woven_cut_and_lay_ratio_wise_entry_v3_controller', order_id+"_"+value+"_"+id[1], 'load_drop_down_color', color_id);
	...
	function change_marker(id,value)
	{
		...
		get_php_form_data( order_id+"_"+gmt_id+"_"+value+"_"+id[1], "load_drop_down_order_qty", "requires/woven_cut_and_lay_ratio_wise_entry_v3_controller" );
		load_drop_down('requires/woven_cut_and_lay_ratio_wise_entry_v3_controller', order_id+"_"+value+"_"+id[1], 'load_drop_down_batch', 'batch_'+id[1]);
	...
	function openmypage_sizeNo(id)
	{
		...
		var cbo_color_id = $('#colorId_'+size).val();
	...
	//	$("#tbl_order_details tbody tr_"+size).css({"background-color":"red"});
		...
		{ 
			...
			$('#txtmarkerqty_'+marker_no[0]).val(marker_no[1]*1);
			$('#txtorderqty_'+marker_no[0]).val(marker_no[2]);
			$('#txttotallay_'+marker_no[0]).val(marker_no[3]);
			$('#txtlaybalanceqty_'+marker_no[0]).val((marker_no[2]*1)-(marker_no[3]*1));
			//freeze_window(5);
	...
	{ 
		...
		if(operation==2)
	  	{
	  		if(!confirm("Do you really want to do this? There is no undo option!")) 
	  		{
			    return false;
			}
			// show_msg('13');
		...
		if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][289]); ?>') 
		{
			if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][289]); ?>','<? echo implode('*',$_SESSION['logic_erp']['mandatory_message'][289]); ?>')==false) {return;}
		}
        ...
        var data1="action=save_update_delete&operation="+operation+"&row_num="+row_num+get_submitted_data_string('update_id*update_tbl_id*txt_job_no*txt_batch_no*cbo_company_name*cbo_working_company_name*cbo_floor_name*cbo_location_name*txt_cutting_no*txt_table_no*txt_entry_date*txt_marker_length*txt_marker_width*txt_fabric_width*txt_gsm*txt_cutting_no*txt_in_time_hours*txt_in_time_minuties*txt_out_time_hours*txt_out_time_minuties*txt_end_date*cbo_width_dia*txt_marker_cons*txt_marker_excess*txt_marker_total*roll_maintained',"../../");
		//alert(row_num)
	    var data2='';
		for(var i=1; i<=row_num; i++)
		{
			if(color_type_mandatory_arr[comp]==2)
			{
				if(form_validation('cboPoNo_'+i+'*colorId_'+i+'*orderCutNo_'+i+'*txtplics_'+i,'Order No*Color*Order Cut No*Plies')==false)//+'*cbobatch_'+i*Batch
				{
					return;
				}
			...
			{
				if(form_validation('cboPoNo_'+i+'*cboColorType_'+i+'*colorId_'+i+'*orderCutNo_'+i+'*txtplics_'+i,'Order No*Color Type*Color*Order Cut No*Plies')==false)//+'*cbobatch_'+i*Batch
				{
					return;
				}
			}
			
			data2+=get_submitted_data_string('updateDetails_'+i+'*cboPoNo_'+i+'*cbogmtsitem_'+i+'*colorId_'+i+'*txtplics_'+i+'*txtorderqty_'+i+'*orderCutNo_'+i+'*rollData_'+i+'*cbobatch_'+i+'*poId_'+i+'*cboColorType_'+i+'*countryId_'+i+'*hiddiscountryseq_'+i+'*posecno_'+i+'*hiddisorderseq_'+i,"../../",2);
		}
	    var data=data1+data2;
		//alert(data1);orderCutNo_1
		freeze_window(operation);
		http.open("POST","requires/woven_cut_and_lay_ratio_wise_entry_v3_controller.php",true);
	...
	function fnc_cut_lay_info_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				show_list_view( reponse[1], 'order_details_list', 'cut_details_container', 'requires/woven_cut_and_lay_ratio_wise_entry_v3_controller', '' ) ;
	...
	{ 
		...
		var page_link='requires/woven_cut_and_lay_ratio_wise_entry_v3_controller.php?action=cutting_number_popup&company_id='+company_id; 
		var title="Search Cutting Number Popup";
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1200px,height=420px,center=1,resize=0,scrolling=0',' ../');
		emailwindow.onclose=function()
		{
			var sysNumber = this.contentDoc.getElementById("update_mst_id"); 
			var sysNumber=sysNumber.value.split('_');
			get_php_form_data( sysNumber[0], "load_php_mst_form", "requires/woven_cut_and_lay_ratio_wise_entry_v3_controller" );
			show_list_view( sysNumber[0], 'order_details_list', 'cut_details_container', 'requires/woven_cut_and_lay_ratio_wise_entry_v3_controller', '' ) ;
			$("#cbo_company_name").attr("disabled",true);
			$("#txt_job_no").attr("disabled",true);
	...
	function openmypage_roll(row_no)
	{
		...
		var color=$('#colorId_'+row_no).val();
		var rollData=$('#rollData_'+row_no).val();
		// var color_name=$('#cbocolor_'+row_no).val();
		
		var title = 'Plies Entry Roll Wise Form';	
		var page_link = 'requires/woven_cut_and_lay_ratio_wise_entry_v3_controller.php?roll_maintained='+roll_maintained+'&order_no='+order_no+'&color='+color+'&rollData='+rollData+'&action=roll_popup';
...
// for report lay chart
	function generate_report_lay_chart(data,action)
	{
		...
		window.open("requires/woven_cut_and_lay_ratio_wise_entry_v3_controller.php?data=" + data+'&action='+action, true );
	}

	function fn_auto_fillup_combo_boxes()
	{
		// alert('ok'); cbo_location_name
		$( ".combo_boxes" ).each(function( index ) {
			if($('#'+this.id!='cbo_working_company_name')){
	...
	function calculate_process_loss()
	{
		var cons = document.getElementById('txt_marker_cons').value*1;
		var excess = document.getElementById('txt_marker_excess').value*1;
		var process_loss_method = document.getElementById('process_loss_method').value;
		var total = 0;
		// alert(process_loss_method);
		if(process_loss_method=='1')
		{
			total=parseFloat(cons+cons*(excess/100));
		}
		else
		{
			var devided_val = 1-(excess/100);
			total=parseFloat(cons/devided_val);
		}
		document.getElementById('txt_marker_total').value=total;
	}
</script>
    ...
    <table width="95%" cellpadding="0" cellspacing="2" align="center">
     	<tr>
        	<td width="95%" align="center" valign="top">  <!--   Form Left Container -->
            	<fieldset style="width:1000px;">
                     <legend> Woven Cut  And  Lay Entry</legend>
                        <table  width="1050" cellspacing="2" cellpadding="0" border="0">
                         <tr>
                           ...
                           <tr>
                                ...
                                <td  width="150" align="" colspan="2">
                                 <? 
                                 	...
                                 	$company_cond = ($_SESSION['logic_erp']['company_id']) ? " and id in(".$_SESSION['logic_erp']['company_id'].")" : "";
									echo create_drop_down( "cbo_company_name", 152, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "get_php_form_data(this.value,'roll_maintained','requires/woven_cut_and_lay_ratio_wise_entry_v3_controller');change_type_color(this.value);get_php_form_data(this.value,'size_wise_repeat_cut_no','requires/woven_cut_and_lay_ratio_wise_entry_v3_controller' );" ); 
                                ?>
                                </td>
	                                <td  width="110"  align="left" class="must_entry_caption">Working Company</td>              <!-- 11-00030  -->
	                                <td  width="150" align="">
	                                 <? 
	                                    echo create_drop_down( "cbo_working_company_name", 142, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $working_credential_cond order by company_name","id,company_name", 1, "-- Select Working Company --",$selected, "load_drop_down( 'requires/woven_cut_and_lay_ratio_wise_entry_v3_controller', this.value, 'load_drop_down_location', 'location_td' );fn_auto_fillup_combo_boxes();" );
                                ...
                                <td width="120" align="left" class="must_entry_caption">Floor/Unit</td>
                          ...
                          <tr>
                           ...
                           <tr>
                                   <td  width="" class="must_entry_caption">Job No</td>
                                     ...
                                     <td colspan="">CAD Marker Cons/Dzn</td>
                                     <td width="">
                                     	<input style="width:30px;" type="text"  class="text_boxes_numeric" autocomplete="off" name="txt_marker_cons" id="txt_marker_cons" title="Cons" placeholder="Cons" onkeyup="calculate_process_loss()" />
                                     	<input style="width:30px;" type="text"  class="text_boxes_numeric" autocomplete="off" name="txt_marker_excess" id="txt_marker_excess" title="Excess" placeholder="Excess" onkeyup="calculate_process_loss()" />
                                     	<input style="width:40px;" type="text"  class="text_boxes_numeric" autocomplete="off" name="txt_marker_total" id="txt_marker_total" title="Total" placeholder="Total" readonly="true" />
                                     </td>
                          ...
                          <tr>
                               <td  width="" style="display:none;" >Width/Dia Type</td>
                                <td width="" style="display:none;" >
                                	<? echo create_drop_down( "cbo_width_dia", 152, $fabric_typee,"", 1, "-- Select --", "", "",$disabled,"" ); ?>
                                </td>
                                <td style="display:none;" width="" align="left">Batch   </td>
               ...
               <fieldset style="width:1200px; margin-top:10px">
                ...
                <table cellpadding="0" cellspacing="0" width="1200" class="rpt_table" border="1" rules="all" id="tbl_order_details">
                    <thead>
                        ...
                        <th style="display:none;">Batch </th><!-- class="must_entry_caption"-->
                    ...
                    <tbody id="cut_details_container">
                        <tr class="" id="tr_1" style="height:10px;">
                            <td align="center" id="color_1">
                                 ...
                                 ?>
								 <input type="text" name="cbocolor_1" id="cbocolor_1" class="text_boxes" style="width:100px;" placeholder="Double Click to Search" onDblClick="openmypage_color(1);" readonly />		 
                            ...
                            <td align="center" id="orderId_1">
                                ...
                                <input class="text_boxes" type="hidden" name="hiddisorderseq_1" id="hiddisorderseq_1" />
                                <input class="text_boxes" type="hidden" name="posecno_1" id="posecno_1" />
                            </td>
							<td align="center" id="">
                            	<input style="width:70px;" class="text_boxes" type="text" name="countryName_1" id="countryName_1" placeholder="Double Click to Search"  onDblClick="openmypage_country(1);"/>
                            ...
                            <td align="center" style="display:none;" id="batch_1">
                                 <? 
        ...
        <tr>
			<td colspan="4" align="center" class="">
				<?= load_submit_buttons( $permission, "fnc_cut_lay_info", 0,0,"reset_form('cutandlayentry_1','','','','clear_tr()')",1);?>
