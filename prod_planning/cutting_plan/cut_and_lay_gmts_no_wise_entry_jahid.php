...
<script>
	...
	function openmypage_jobNo(id)
	{
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/cut_and_lay_gmts_no_wise_entry_controller',job_no[0], 'load_drop_down_order', 'order_id' );
  	...
  	function change_order(value)
	{
		...
		var txt_job=$("#txt_job_no").val();
		load_drop_down( 'requires/cut_and_lay_gmts_no_wise_entry_controller',txt_job, 'load_drop_down_order', 'order_id' );
  	}
		...
		for(var i=1; i<=row_num; i++)
			{
				...
				data2+=get_submitted_data_string('updateDetails_'+i+'*cboorderno_'+i+'*cbogmtsitem_'+i+'*txtshipdate_'+i+'*cbocolor_'+i+'*txtplics_'+i+'*txtorderqty_'+i+'*orderCutNo_'+i+'*orderCutNo_'+i+'*rollData_'+i+'*cbobatch_'+i,"../../",2);
			}
...
function openmypage_roll(row_no)
{
	...
	var page_link = 'requires/cut_and_lay_gmts_no_wise_entry_controller.php?roll_maintained='+roll_maintained+'&order_no='+order_no+'&color='+color+'&rollData='+rollData+'&action=roll_popup';
                         ...
                         <tr>
                           ...
                           <tr>
                                ...
                                <td  width="150" align="" colspan="2">
                                 <? 
                                    echo create_drop_down( "cbo_company_name", 152, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/cut_and_lay_gmts_no_wise_entry_controller', this.value, 'load_drop_down_location', 'location_td' );get_php_form_data(this.value,'roll_maintained','requires/cut_and_lay_gmts_no_wise_entry_controller');" );
                                ?>
                                </td>
         ...
         <tr>
             <td align="center" valign="top" id="po_list_view">
               <fieldset style="width:1100px; margin-top:10px">
                ...
                <table cellpadding="0" cellspacing="0" width="1100" class="rpt_table" border="1" rules="all" id="tbl_order_details">
                    <thead>
