...
<script>
	...
	function add_break_down_tr(i)
   	{ 
		...
		{ 
			...
			$("#tbl_order_details tbody tr:last td:nth-child(2)").removeAttr('id').attr('id','color_'+i);
	...
	function openmypage_jobNo(id)
	{
		...
		var page_link = 'requires/contrast_cutting_entry_controller.php?cbo_company_id='+cbo_company_id+'&action=job_search_popup';
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down('requires/contrast_cutting_entry_controller', job_no[0]+'_'+1, 'load_drop_down_color','color_1');
			load_drop_down('requires/contrast_cutting_entry_controller', job_no[0]+'_'+1, 'load_drop_down_order_garment','garment_1');
	...
	function openmypage_po(row_no)
	{
		...
		var page_link = 'requires/contrast_cutting_entry_controller.php?txt_job='+txt_job+'&cbocolor='+cbocolor+'&gmt_id='+gmt_id+'&poId='+poId+'&company_id='+cbo_company_id+'&action=po_popup';
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down('requires/contrast_cutting_entry_controller', po_id+"_"+cbocolor+"_"+row_no, 'load_drop_down_batch', 'batch_'+row_no);
			
			
			load_drop_down( 'requires/contrast_cutting_entry_controller', po_id+"_"+row_no+"_"+cbocolor+"_"+gmt_id, 'load_drop_down_color_type', 'colorTypeId_'+row_no);
			...
			get_php_form_data( po_id+"_"+gmt_id+"_"+cbocolor+"_"+row_no, "load_drop_down_order_qty", "requires/contrast_cutting_entry_controller" );
		}
	}
    ...
    { 
		...
		load_drop_down( 'requires/contrast_cutting_entry_controller', value+"_"+ship_id, 'load_drop_down_ship', ship_id);
		load_drop_down( 'requires/contrast_cutting_entry_controller', value+"_"+gmt_id, 'load_drop_down_order_garment', gmt_id);
	...
	function change_color(id,value)
	{
		...
		load_drop_down('requires/contrast_cutting_entry_controller', order_id+"_"+value+"_"+id[1], 'load_drop_down_color', color_id);
	...
	function change_marker(id,value)
	{
		...
		get_php_form_data( order_id+"_"+gmt_id+"_"+value+"_"+id[1], "load_drop_down_order_qty", "requires/contrast_cutting_entry_controller" );
		load_drop_down('requires/contrast_cutting_entry_controller', order_id+"_"+value+"_"+id[1], 'load_drop_down_batch', 'batch_'+id[1]);
	...
	function openmypage_sizeNo(id)
	{
		...
		var cont_color = $('#concolor_'+size).val();
		$("#tr_"+size).css({"background-color":"yellow"});
	//	$("#tbl_order_details tbody tr_"+size).css({"background-color":"red"});
		var title = 'Size Ratio Form';
		
		var page_link = 'requires/contrast_cutting_entry_controller.php?cbo_company_id='+cbo_company_id+'&job_id='+job_id+'&mst_id='+mst_id+'&details_id='+details_id+'&cbo_gmt_id='+cbo_gmt_id+'&cbo_color_id='+cbo_color_id+'&size='+size+'&txt_piles='+piles+'&cutting_no='+cutting_no+'&order_id='+order_id+'&marker_quantity='+marker_quantity+'&order_quantity='+order_quantity+'&total_lay_qty='+total_lay_qty+'&total_lay_balance='+total_lay_balance+'&rollData='+rollData+'&action=size_popup'+'&cbo_color_type='+cbo_color_type+'&cont_color='+cont_color;//+'&size_wise_repeat_cut_no='+size_wise_repeat_cut_no
		//alert(page_link)
	...
	{      
		...
		for(var i=1; i<=row_num; i++)
		{
			...
			data2+=get_submitted_data_string('updateDetails_'+i+'*cboPoNo_'+i+'*cbogmtsitem_'+i+'*cbocolor_'+i+'*txtplics_'+i+'*txtorderqty_'+i+'*orderCutNo_'+i+'*rollData_'+i+'*cbobatch_'+i+'*poId_'+i+'*cboColorType_'+i+'*cbogmtsitem_'+i+'*concolor_'+i,"../../",2);
		}
	    var data=data1+data2;
		//alert(data1);orderCutNo_1
		freeze_window(operation);
		http.open("POST","requires/contrast_cutting_entry_controller.php",true);
	...
	function fnc_cut_lay_info_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				show_list_view( reponse[1], 'order_details_list', 'cut_details_container', 'requires/contrast_cutting_entry_controller', '' ) ;
	...
	{ 
		...
		var page_link='requires/contrast_cutting_entry_controller.php?action=cutting_number_popup&company_id='+company_id; 
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data( sysNumber[0], "load_php_mst_form", "requires/contrast_cutting_entry_controller" );
			show_list_view( sysNumber[0], 'order_details_list', 'cut_details_container', 'requires/contrast_cutting_entry_controller', '' ) ;
			$("#cbo_company_name").attr("disabled",true);
			$("#txt_job_no").attr("disabled",true);
	...
	function openmypage_roll(row_no)
	{
		...
		var page_link = 'requires/contrast_cutting_entry_controller.php?roll_maintained='+roll_maintained+'&order_no='+order_no+'&color='+color+'&rollData='+rollData+'&action=roll_popup';
...
// for report lay chart
 	function generate_report_lay_chart(data,action)
	{
		...
		window.open("requires/contrast_cutting_entry_controller.php?data=" + data+'&action='+action, true );
	}
                        ...
                        <table  width="1050" cellspacing="2" cellpadding="0" border="0">
                             ...
                             <tr>
                                 ...
                                 <td  width="150" colspan="2">
                                     <? echo create_drop_down( "cbo_company_name", 152, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --", $selected, "get_php_form_data(this.value,'roll_maintained', 'requires/contrast_cutting_entry_controller');change_type_color(this.value);" ); ?>
                                 </td>
                                 <td  width="110"  align="left" class="must_entry_caption">Working Company</td>              <!-- 11-00030  -->
                                 <td  width="150" align="">
                                     <?
                                        echo create_drop_down( "cbo_working_company_name", 142, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Working Company --", $selected, "load_drop_down( 'requires/contrast_cutting_entry_controller', this.value, 'load_drop_down_location', 'location_td' ); " );// location_select(); get_php_form_data(this.value,'size_wise_repeat_cut_no','requires/contrast_cutting_entry_controller' );
                                    ?>
                                </td>
                            ...
                            <tr>
                                ...
                                <td>Shipment Part</td>
                                <td>
                                    <input style="width:120px;" type="text" class="text_boxes" autocomplete="off" name="txt_shipment_part" id="txt_shipment_part" />
                                </td>
                            </tr>
                      </table>
                 </fieldset>
               ...
               <fieldset style="width:1300px; margin-top:10px">
                ...
                <table cellpadding="0" cellspacing="0" width="1300" class="rpt_table" border="1" rules="all" id="tbl_order_details">
                    <thead>
                        ...
                        <th class="must_entry_caption">Contrast Color</th>
                        <th>Gmt Item</th>
                    ...
                    <tbody id="cut_details_container">
                        <tr class="" id="tr_1" style="height:10px;">
                            ...
                            <td align="center" id="concolortd_1">
								<input type="text" name="concolor_1" id="concolor_1" class="text_boxes" style="width:100px;" placeholder="Write..." />	
                            </td> 
