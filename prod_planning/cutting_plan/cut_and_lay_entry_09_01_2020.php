	   ...
	   { 
			...
			{ 
		       ...
		       var k=i-1;
				...
				$("#tbl_order_details tbody tr:last td:nth-child(9)").removeAttr('id').attr('id','order_'+i);
				$("#tbl_order_details tbody tr:last td:nth-child(8)").removeAttr('id').attr('id','marker_'+i);
...
function openmypage_jobNo(id)
			...
			emailwindow.onclose=function()
			 {
				...
				load_drop_down( 'requires/cut_and_lay_entry_controller',job_no[0], 'load_drop_down_order', 'order_id' );
  	...
  	function change_order(value)
	{
		...
		var txt_job=$("#txt_job_no").val();
		load_drop_down( 'requires/cut_and_lay_entry_controller',txt_job, 'load_drop_down_order', 'order_id' );
		...
		for(var i=1; i<=row_num; i++)
			{
				...
				data2+=get_submitted_data_string('updateDetails_'+i+'*cboorderno_'+i+'*cbogmtsitem_'+i+'*txtshipdate_'+i+'*cbocolor_'+i+'*txtplics_'+i+'*txtorderqty_'+i+'*orderCutNo_'+i+'*rollData_'+i+'*hiddenExtralRollData_'+i+'*orderCutNo_'+i,"../../",i);
			}
