...
<script>
	...
	function add_break_down_tr(i)
   	{ 
		...
		{ 
			...
			$("#tbl_order_details tbody tr:last td:nth-child(4)").removeAttr('id').attr('id','colorTypeId_'+i);

			$("#tbl_order_details tbody tr:last td:nth-child(5)").removeAttr('id').attr('id','cutNo_'+i);
			$("#tbl_order_details tbody tr:last td:nth-child(6)").removeAttr('id').attr('id','batch_'+i);
	...
	function openmypage_jobNo(id)
	{
		...
		var page_link = 'requires/woven_cut_and_lay_ratio_wise_entry_controller_v2.php?cbo_company_id='+cbo_company_id+'&action=job_search_popup';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1430px,height=400px,center=1,resize=0,scrolling=0','../');
		emailwindow.onclose=function()
		{
			...
			load_drop_down('requires/woven_cut_and_lay_ratio_wise_entry_controller_v2', job_no[0]+'_'+1, 'load_drop_down_color','color_1');
			load_drop_down('requires/woven_cut_and_lay_ratio_wise_entry_controller_v2', job_no[0]+'_'+1, 'load_drop_down_order_garment','garment_1');
	...
	function openmypage_po(row_no)
	{
		...
		var page_link = 'requires/woven_cut_and_lay_ratio_wise_entry_controller_v2.php?txt_job='+txt_job+'&cbocolor='+cbocolor+'&gmt_id='+gmt_id+'&poId='+poId+'&company_id='+cbo_company_id+'&action=po_popup';
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down('requires/woven_cut_and_lay_ratio_wise_entry_controller_v2', po_id+"_"+cbocolor+"_"+row_no, 'load_drop_down_batch', 'batch_'+row_no);
			
			
			load_drop_down( 'requires/woven_cut_and_lay_ratio_wise_entry_controller_v2', po_id+"_"+row_no+"_"+cbocolor+"_"+gmt_id, 'load_drop_down_color_type', 'colorTypeId_'+row_no);
			...
			get_php_form_data( po_id+"_"+gmt_id+"_"+cbocolor+"_"+row_no, "load_drop_down_order_qty", "requires/woven_cut_and_lay_ratio_wise_entry_controller_v2" );
		}
	}
    ...
    { 
		...
		load_drop_down( 'requires/woven_cut_and_lay_ratio_wise_entry_controller_v2', value+"_"+ship_id, 'load_drop_down_ship', ship_id);
		load_drop_down( 'requires/woven_cut_and_lay_ratio_wise_entry_controller_v2', value+"_"+gmt_id, 'load_drop_down_order_garment', gmt_id);
	...
	function change_color(id,value)
	{
		...
		load_drop_down('requires/woven_cut_and_lay_ratio_wise_entry_controller_v2', order_id+"_"+value+"_"+id[1], 'load_drop_down_color', color_id);
	...
	function change_marker(id,value)
	{
		...
		get_php_form_data( order_id+"_"+gmt_id+"_"+value+"_"+id[1], "load_drop_down_order_qty", "requires/woven_cut_and_lay_ratio_wise_entry_controller_v2" );
		load_drop_down('requires/woven_cut_and_lay_ratio_wise_entry_controller_v2', order_id+"_"+value+"_"+id[1], 'load_drop_down_batch', 'batch_'+id[1]);
	...
	{
		var rowCount = $('#cut_details_container tr').length;
		var ids = id.split('_');
		var prev_id = ids[1]*1 - 1;
		// alert(prev_id);
		if(ids[1]*1>1 && $('#txtmarkerqty_'+prev_id).val()=="")
		{
			alert('Please, first save upper part size ratio!');
			return false;
		}
		...
		var sequence_no= $('#sequence_'+size).val();
	...
	{      
		...
		for(var i=1; i<=row_num; i++)
		{
			...
			data2+=get_submitted_data_string('updateDetails_'+i+'*cboPoNo_'+i+'*cbogmtsitem_'+i+'*cbocolor_'+i+'*txtplics_'+i+'*txtorderqty_'+i+'*orderCutNo_'+i+'*rollData_'+i+'*cbobatch_'+i+'*poId_'+i+'*cboColorType_'+i,"../../",2);
		}
	    var data=data1+data2;
		//alert(data1);orderCutNo_1
		freeze_window(operation);
		http.open("POST","requires/woven_cut_and_lay_ratio_wise_entry_controller_v2.php",true);
	...
	function fnc_cut_lay_info_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				show_list_view( reponse[1], 'order_details_list', 'cut_details_container', 'requires/woven_cut_and_lay_ratio_wise_entry_controller_v2', '' ) ;
	...
	{ 
		...
		var page_link='requires/woven_cut_and_lay_ratio_wise_entry_controller_v2.php?action=cutting_number_popup&company_id='+company_id; 
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data( sysNumber[0], "load_php_mst_form", "requires/woven_cut_and_lay_ratio_wise_entry_controller_v2" );
			show_list_view( sysNumber[0], 'order_details_list', 'cut_details_container', 'requires/woven_cut_and_lay_ratio_wise_entry_controller_v2', '' ) ;
			$("#cbo_company_name").attr("disabled",true);
			$("#txt_job_no").attr("disabled",true);
	...
	function openmypage_roll(row_no)
	{
		...
		var page_link = 'requires/woven_cut_and_lay_ratio_wise_entry_controller_v2.php?roll_maintained='+roll_maintained+'&order_no='+order_no+'&color='+color+'&rollData='+rollData+'&action=roll_popup';
...
// for report lay chart
	function generate_report_lay_chart(data,action)
	{
		...
		window.open("requires/woven_cut_and_lay_ratio_wise_entry_controller_v2.php?data=" + data+'&action='+action, true );
	}
                         ...
                         <tr>
                           ...
                           <tr>
                                ...
                                <td  width="150" align="" colspan="2">
                                 <? 
                                    //echo create_drop_down( "cbo_company_name", 152, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/woven_cut_and_lay_ratio_wise_entry_controller_v2', this.value, 'load_drop_down_location', 'location_td' );get_php_form_data(this.value,'roll_maintained','requires/woven_cut_and_lay_ratio_wise_entry_controller_v2');" );// get_php_form_data(this.value,'size_wise_repeat_cut_no','requires/woven_cut_and_lay_ratio_wise_entry_controller_v2' ); //old
                                 	$company_cond = ($_SESSION['logic_erp']['company_id']) ? " and id in(".$_SESSION['logic_erp']['company_id'].")" : "";
									echo create_drop_down( "cbo_company_name", 152, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "get_php_form_data(this.value,'roll_maintained','requires/woven_cut_and_lay_ratio_wise_entry_controller_v2');change_type_color(this.value);get_php_form_data(this.value,'size_wise_repeat_cut_no','requires/woven_cut_and_lay_ratio_wise_entry_controller_v2' );" ); //new
                                ?>
                                </td>
	                                <td  width="110"  align="left" class="must_entry_caption">Working Company</td>              <!-- 11-00030  -->
	                                <td  width="150" align="">
	                                 <? 
	                                    echo create_drop_down( "cbo_working_company_name", 142, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $working_credential_cond order by company_name","id,company_name", 1, "-- Select Working Company --",$selected, "load_drop_down( 'requires/woven_cut_and_lay_ratio_wise_entry_controller_v2', this.value, 'load_drop_down_location', 'location_td' );" );//fn_auto_fillup_combo_boxes();
																			
																			// get_php_form_data(this.value,'size_wise_repeat_cut_no','requires/woven_cut_and_lay_ratio_wise_entry_controller_v2' );
