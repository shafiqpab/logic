...
<script>
	...
	function openmypage_jobNo(id)
	{
		...
		var page_link = 'requires/cut_and_lay_ratio_wise_entry_controller.php?cbo_company_id='+cbo_company_id+'&action=job_search_popup';
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down( 'requires/cut_and_lay_ratio_wise_entry_controller',job_no[0]+'**'+cbo_company_id, 'load_drop_down_order', 'order_id' );
  	...
  	function change_order(value)
	{
		...
		if(txt_job.length<5)
		{
			load_drop_down( 'requires/cut_and_lay_ratio_wise_entry_controller',txt_job+'**'+value+'**'+cbo_company_id, 'load_drop_down_job', 'job_change_id' );
			load_drop_down( 'requires/cut_and_lay_ratio_wise_entry_controller',txt_job+'**'+value+'**'+cbo_company_id, 'load_drop_down_buyer', 'buyer_id' );
		}
		var txt_job=$("#txt_job_no").val();
		load_drop_down( 'requires/cut_and_lay_ratio_wise_entry_controller',txt_job+'**'+cbo_company_id, 'load_drop_down_order', 'order_id' );
  	}
    ...
    { 
		...
		load_drop_down( 'requires/cut_and_lay_ratio_wise_entry_controller', value+"_"+ship_id, 'load_drop_down_ship', ship_id);
		load_drop_down( 'requires/cut_and_lay_ratio_wise_entry_controller', value+"_"+gmt_id, 'load_drop_down_order_garment', gmt_id);
	...
	function change_color(id,value)
	{
		...
		load_drop_down( 'requires/cut_and_lay_ratio_wise_entry_controller', order_id+"_"+value+"_"+id[1], 'load_drop_down_color', color_id);
	...
	function change_marker(id,value)
	{
		...
		get_php_form_data( order_id+"_"+gmt_id+"_"+value+"_"+id[1], "load_drop_down_order_qty", "requires/cut_and_lay_ratio_wise_entry_controller" );
		load_drop_down('requires/cut_and_lay_ratio_wise_entry_controller', order_id+"_"+value+"_"+id[1], 'load_drop_down_batch', 'batch_'+id[1]);
		load_drop_down('requires/cut_and_lay_ratio_wise_entry_controller', order_id+"_"+gmt_value+"_"+value+"_"+id[1], 'load_drop_down_color_type', 'cboColorType_'+id[1]);
	...
	//	$("#tbl_order_details tbody tr_"+size).css({"background-color":"red"});
		...
		var page_link = 'requires/cut_and_lay_ratio_wise_entry_controller.php?cbo_company_id='+cbo_company_id+'&job_id='+job_id+'&mst_id='+mst_id+'&details_id='+details_id+'&cbo_gmt_id='+cbo_gmt_id+'&cbo_color_id='+cbo_color_id+'&size='+size+'&txt_piles='+piles+'&cutting_no='+cutting_no+'&order_id='+order_id+'&marker_quantity='+marker_quantity+'&order_quantity='+order_quantity+'&total_lay_qty='+total_lay_qty+'&total_lay_balance='+total_lay_balance+'&rollData='+rollData+'&action=size_popup';//+'&size_wise_repeat_cut_no='+size_wise_repeat_cut_no
		//alert(page_link)
	...
	{      
		...
		var tna_date=return_ajax_request_value(tna_data, "tna_date_status", "requires/cut_and_lay_ratio_wise_entry_controller");
		if(tna_date!=2)
		{
		...
		for(var i=1; i<=row_num; i++)
			{
				if(form_validation('cboorderno_'+i+'*cbocolor_'+i+'*txtplics_'+i,'Order No*Color*Plies')==false)//+'*cbobatch_'+i*Batch
				{
					return;
				}
		...
		http.open("POST","requires/cut_and_lay_ratio_wise_entry_controller.php",true);
...
function fnc_cut_lay_info_reponse()
{
	if(http.readyState == 4) 
	{
		...
		if(reponse[0]==0 || reponse[0]==1)
		{
			...
			show_list_view( reponse[1], 'order_details_list', 'cut_details_container', 'requires/cut_and_lay_ratio_wise_entry_controller', '' ) ;
...
{ 
	...
	var page_link='requires/cut_and_lay_ratio_wise_entry_controller.php?action=cutting_number_popup&company_id='+company_id; 
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data( sysNumber[0], "load_php_mst_form", "requires/cut_and_lay_ratio_wise_entry_controller" );
		show_list_view( sysNumber[0], 'order_details_list', 'cut_details_container', 'requires/cut_and_lay_ratio_wise_entry_controller', '' ) ;
		$("#cbo_company_name").attr("disabled",true);
		$("#txt_job_no").attr("disabled",true);
...
function openmypage_roll(row_no)
{
	...
	var page_link = 'requires/cut_and_lay_ratio_wise_entry_controller.php?roll_maintained='+roll_maintained+'&order_no='+order_no+'&color='+color+'&rollData='+rollData+'&action=roll_popup';
 ...
 function generate_report_lay_chart(data,action)
	{
		...
		window.open("requires/cut_and_lay_ratio_wise_entry_controller.php?data=" + data+'&action='+action, true );
	}
                         ...
                         <tr>
                           ...
                           <tr>
                                ...
                                <td  width="150" align="" colspan="2">
                                 <? 
                                    echo create_drop_down( "cbo_company_name", 152, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/cut_and_lay_ratio_wise_entry_controller', this.value, 'load_drop_down_location', 'location_td' );get_php_form_data(this.value,'roll_maintained','requires/cut_and_lay_ratio_wise_entry_controller');" );// get_php_form_data(this.value,'size_wise_repeat_cut_no','requires/cut_and_lay_ratio_wise_entry_controller' );
                                ?>
                                </td>
                                ...
                                <td  width="140" id="floor_td">
                                   ...
                                   echo create_drop_down( "cbo_floor_name", 142,"select id,floor_name from lib_prod_floor where production_process=1  and status_active =1 and is_deleted=0","id,floor_name", 1, "-- Select Floor --", $selected, "" );
                                     ?>
         ...
         <tr>
             <td align="center" valign="top" id="po_list_view">
               <fieldset style="width:1180px; margin-top:10px">
                ...
                <table cellpadding="0" cellspacing="0" width="1180" class="rpt_table" border="1" rules="all" id="tbl_order_details">
                    <thead>
                    ...
                    <tbody id="cut_details_container">
                        <tr class="" id="tr_1" style="height:10px;">
                            ...
                            <td align="center" id="colorTypeId_1">
                                <?
                                echo create_drop_down( "cboColorType_1", 80, $blank_array,"", 1, "--Select--", $selected, "","",0 );
                                ?>
                            </td>
