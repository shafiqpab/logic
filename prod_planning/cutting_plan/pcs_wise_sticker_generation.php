...
<script>
	...
	function openmypage_lotratio(id)
	{
		...
		var page_link = 'requires/pcs_wise_sticker_generation_controller.php?cbo_company_id='+cbo_company_id+'&action=lotratio_search_popup';
		...
		emailwindow.onclose=function()
		{
			...
			var job_no=(this.contentDoc.getElementById("hidden_system_no").value).split('_');
			$('#hidden_lot_ratio_id').val(job_no[0]);
			document.getElementById('txt_lot_ratio_no').value=job_no[1];
			release_freezing();
		}
	}
	...
	{		
		if(form_validation('cbo_company_name*txt_lot_ratio_no','Company Name*Lot Ratio No')==false)
	  	{
			return;
		}
		
        var data="action=generate_bundle"+get_submitted_data_string('cbo_company_name*cbo_working_company_name*cbo_floor*cbo_location_name*cbo_wo_location_name*txt_lot_ratio_no*hidden_lot_ratio_id*txt_date_from*txt_date_to',"../../");    
	    
		freeze_window(operation);
		http.open("POST","requires/pcs_wise_sticker_generation_controller.php",true);
	...
	function fnc_bundle_report_qr_code()
	{
		...
		http.open( 'POST', 'requires/pcs_wise_sticker_generation_controller.php?action='+action+'&data='+ unique_ids );
...
<div style="width:100%" align="center">
    ...
    <form name="cutandlayentry_1" id="cutandlayentry_1">
         ...
         <fieldset style="width:950px;margin: 0 auto;">
             <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
             	<thead>
                   	<th>Working Company</th>
                   	<th>WO Location</th>
                    <th class="must_entry_caption">LC Company Name</th>
                   	<th>LC Location</th>
                   	<th>Floor</th>
                    <th class="must_entry_caption">Yarn Lot Ratio</th>
                    <th colspan="2" id="cap_cut_date">Plan Date</th>
                ...
                <tbody>
                    <tr class="general">
                        ...
                        <td>
                        <? 
	                       echo create_drop_down( "cbo_working_company_name", 130, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select  --", $selected, "load_drop_down( 'requires/pcs_wise_sticker_generation_controller',this.value, 'load_drop_down_wo_location', 'wo_location_td' );" );
	                     ?>
                        </td>
                        <td id="wo_location_td">
                        <? 
	                       echo create_drop_down( "cbo_wo_location_name", 130, "select id,location_name from lib_location comp where status_active=1 and is_deleted=0  order by location_name","id,location_name", 1, "-- All  --", $selected, "" );
	                     ?>
                        </td>
                        <td>
							<? echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/pcs_wise_sticker_generation_controller',this.value, 'load_drop_down_location', 'lc_location_td' );" ); 
						...
						<td id="floor_td">
							<? 
								echo create_drop_down( "cbo_floor", 130, $blank_array,"", 1, "-- All --", $selected, "",1,"" );
                            ?>
                        </td> 
                        <td> 
                            <input type="text" id="txt_lot_ratio_no" name="txt_lot_ratio_no" class="text_boxes" style="width:100px" onDblClick="openmypage_lotratio();" placeholder="Browse" readonly="true" />
                            <input type="hidden" name="hidden_lot_ratio_id"  id="hidden_lot_ratio_id"  />
                        </td>
