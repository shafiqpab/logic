...
if($action=="check_batch_no")
{
	...
	$barcodeData=sql_select( "select barcode_no from pro_roll_details where entry_form=289 and status_active=1 and is_deleted=0");
...
if($action=="roll_popup")
{
	...
	<script>
		...
		{ 
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','woven_cut_and_lay_ratio_wise_entry_controller.php?order_no='+<? echo $order_no; ?>+'&color='+<? echo $color; ?>+'&added_barcode_no='+added_barcode_no+'&action=batch_popup','Batch Barcode Popup', 'width=580px,height=300px,center=1,resize=1,scrolling=0','../../')
		...
		$('#txt_batch_no').live('keydown', function(e) {
			if (e.keyCode === 13) 
			{
				...
				var response_data=return_global_ajax_value( batch_no+"**"+order_id+"**"+added_barcode_no, 'check_batch_no', '', 'woven_cut_and_lay_ratio_wise_entry_controller');
		...
		function openmypage_barcode()
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','woven_cut_and_lay_ratio_wise_entry_controller.php?order_no='+<? echo $order_no; ?>+'&color='+<? echo $color; ?>+'&action=barcode_popup','Barcode Popup', 'width=480px,height=300px,center=1,resize=1,scrolling=0','../../')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
				var barcode_nos=this.contentDoc.getElementById("hidden_barcode_nos").value; //Barcode Nos
   ...
   $country_con = ($country_id!="") ? " and country_id in($country_id)" : "";
	...
	<script>
		...
		{   
			...
			// freeze_window(operation);
			http.open("POST","woven_cut_and_lay_ratio_wise_entry_controller.php",true);
			    ...
			    if(reponse[0]==0 || reponse[0]==1)
				 {
					...
					show_list_view(reponse[1]+'**'+reponse[2]+'**'+reponse[3]+'**'+reponse[7],'show_bundle_list_view','search_div','woven_cut_and_lay_ratio_wise_entry_controller','setFilterGrid("list_view",-1)');
		...
		function fnc_print_bundle(type)
		{
		   ...
		   print_report(<? echo $cbo_company_id; ?>+'*'+<? echo $mst_id; ?>+'*'+<? echo $details_id; ?>+'*'+report_title+'*'+country+'*'+type, "cut_lay_bundle_print", "woven_cut_and_lay_ratio_wise_entry_controller")
		}
		...
		function fnc_print_bundle_lay__22212(type)
		{
		   var report_title="Cut and Lay bundle ";
		   var country=$('#cboCountryBundle').val();
		   print_report(<? echo $cbo_company_id; ?>+'*'+<? echo $mst_id; ?>+'*'+<? echo $details_id; ?>+'*'+report_title+'*'+country+'*'+type, "lay_bundle_print", "woven_cut_and_lay_ratio_wise_entry_controller")
		}
		...
		function fnc_print_qc_bundle(type)
		{
		   ...
		   var country=$('#cboCountryBundle').val();
		   if (type==1) {
			   print_report(<? echo $cbo_company_id; ?>+'*'+<? echo $mst_id; ?>+'*'+<? echo $details_id; ?>+'*'+report_title+'*'+country+'*'+type, "cut_lay_qc_bundle_print", "woven_cut_and_lay_ratio_wise_entry_controller")
		   }else if(type==2){
			    print_report(<? echo $cbo_company_id; ?>+'*'+<? echo $mst_id; ?>+'*'+<? echo $details_id; ?>+'*'+report_title+'*'+country+'*'+type, "cut_lay_qc_bundle_print_2", "woven_cut_and_lay_ratio_wise_entry_controller")
		   }
		}
		...
		function fnc_bundle_report(column_list)
		{
			...
			if(column_list==6)
			{
				var url=return_ajax_request_value(data, "print_report_bundle_barcode", "woven_cut_and_lay_ratio_wise_entry_controller");
				window.open(url,"##");
			}
			else
			{
				var url=return_ajax_request_value(data, "print_report_bundle_barcode_eight", "woven_cut_and_lay_ratio_wise_entry_controller");
				window.open(url,"##");	
			}
		}
		...
		function fnc_bundle_report_eight()
		{
			...
			var page_link = 'woven_cut_and_lay_ratio_wise_entry_controller.php?data='+data+'&action=print_report_bundle_barcode_eight';
			...
			emailwindow.onclose=function()
			 {
				...
				data=data+'***'+prodID;
				var url=return_ajax_request_value(data, "print_barcode_eight", "woven_cut_and_lay_ratio_wise_entry_controller");
				window.open(url,"##");	
			 }
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				//var url=return_ajax_request_value(data, "print_barcode_one_pdf", "woven_cut_and_lay_ratio_wise_entry_controller");
				//window.open(url,"##");	
				window.open("woven_cut_and_lay_ratio_wise_entry_controller.php?data=" + data+'&action=print_barcode_one', true );
			 }
			...
			emailwindow.onclose=function()
			 {
				...
				{
					var url=return_ajax_request_value(data, "print_barcode_one_urmi", "woven_cut_and_lay_ratio_wise_entry_controller");
					// alert(url);return;
					window.open(url,"##");
				}else if(type == 5){
					var url=return_ajax_request_value(data, "print_barcode_one_urmi_eg", "woven_cut_and_lay_ratio_wise_entry_controller");
					window.open(url,"##");
				}else if(type == 6){
                    var url=return_ajax_request_value(data, "print_barcode_one_urmi_eg_a4", "woven_cut_and_lay_ratio_wise_entry_controller");
                    window.open(url,"##");
                }
                else
				{
					var url=return_ajax_request_value(data, "print_barcode_one_urmi_v5", "woven_cut_and_lay_ratio_wise_entry_controller");
					// alert(url);return;
					window.open(url,"##");
				}
				//window.open("woven_cut_and_lay_ratio_wise_entry_controller.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				var url=return_ajax_request_value(data, "print_barcode_one_128", "woven_cut_and_lay_ratio_wise_entry_controller");
				window.open(url,"##");	
				//window.open("woven_cut_and_lay_ratio_wise_entry_controller.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				var url=return_ajax_request_value(data, "print_barcode_one_129", "woven_cut_and_lay_ratio_wise_entry_controller");
				window.open(url,"##");	
				//window.open("woven_cut_and_lay_ratio_wise_entry_controller.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
		...
		function fnc_bundle_report_qrcode(work_comp,work_location)
		{
			...
			var report_title="Cut and Lay bundle ";
		   	...
		   	var title = 'Search Job No';	
			var page_link = 'woven_cut_and_lay_ratio_wise_entry_controller.php?data='+data+'&action=print_report_bundle_list_popup';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=270px,height=250px,center=1,resize=0,scrolling=0','../../');
			emailwindow.onclose=function()
			 {
				var theform=this.contentDoc.forms[0]; 
				...
				//print_report(<? //echo $cbo_company_id; ?>+'*'+<?// echo $mst_id; ?>+'*'+<? //echo $details_id; ?>+'*'+report_title+'*'+country+'*'+bundle_use_for, "cut_lay_bundle_print", "woven_cut_and_lay_ratio_wise_entry_controller")
			 }	   		
		}
		...
		function fnc_bundle_display_qrccode(bundle_use_for)
		{
			...
			http.open( 'POST', 'woven_cut_and_lay_ratio_wise_entry_controller.php?action=print_qrcode&data='+ data );
		...
		function fnc_send_printer_text()
		{
			...
			var job_id=$("#hidden_update_job_id").val();
			data=data+"***"+job_id+'***'+<? echo $mst_id; ?>+'***'+<? echo $details_id; ?>+'***'+<? echo $cbo_gmt_id; ?>+'***'+<? echo $cbo_color_id; ?>+'***'+<? echo $order_id; ?>;
			var url=return_ajax_request_value(data, "report_bundle_text_file", "woven_cut_and_lay_ratio_wise_entry_controller");
		...
		function fnc_cut_lay_bundle_reply_info()
		{
			if(http.readyState == 4) 
			{
				...
				if((reponse[0]==0 || reponse[0]==1))
				{
					...
					show_list_view(reponse[1]+'**'+reponse[2]+'**'+reponse[3],'show_bundle_list_view','search_div','woven_cut_and_lay_ratio_wise_entry_controller','setFilterGrid("list_view",-1)');
				}
				else if(reponse[0]==200)
				{
		...
		function fnc_rollWiseSizeQty()
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','woven_cut_and_lay_ratio_wise_entry_controller.php?rollData='+'<? echo $rollData; ?>'+'&size_data='+size_data+'&action=rollSize_popup','Roll Popup', 'width=680px,height=300px,center=1,resize=1,scrolling=0','../../')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
		...
		<div align="center" style="width:100%; overflow-y:hidden; position:absolute; top:5px;">
	        ...
	        <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
	            ...
	            <?
	                ...
	                $sizeDaraArr=array();
	                $manualSizeArr=array();
	                $sizeData=sql_select("SELECT a.id, a.size_ratio, a.size_id, a.marker_qty, a.bundle_sequence, a.order_id, a.country_type, a.country_id, a.excess_perc,a.manual_size_name from ppl_cut_lay_size a where a.mst_id=".$mst_id." and a.dtls_id=".$details_id." and a.status_active=1");
	                if(count($sizeData)>0)
	                {
	                    $is_update=1;
	                ...
	                $lay_blData=sql_select("SELECT sum(a.size_qty) as marker_qty, a.size_id from ppl_cut_lay_bundle a where a.mst_id=".$mst_id." and a.dtls_id=".$details_id." $country_con and a.status_active=1 group by a.size_id");
					...
					$size_bl_qty_arr=return_library_array("select sum(a.size_qty) as size_qty, a.size_id from ppl_cut_lay_bundle a, ppl_cut_lay_dtls b where b.id=a.dtls_id and a.order_id in($order_id) and b.gmt_item_id=$cbo_gmt_id and b.color_id=$cbo_color_id $country_con and a.status_active=1 group by a.size_id",'size_id','size_qty');

					$size_bl_prev_qty_arr=return_library_array("SELECT sum(a.size_qty) as size_qty, a.size_id from ppl_cut_lay_bundle a, ppl_cut_lay_dtls b where b.id=a.dtls_id and a.order_id in($order_id) and b.gmt_item_id=$cbo_gmt_id and b.color_id=$cbo_color_id $country_con and a.status_active=1 and a.mst_id<>".$mst_id." and a.dtls_id<>".$details_id."  group by a.size_id",'size_id','size_qty');
...
{	
	...
	$roll_sql=sql_select("select roll_id, batch_no from pro_roll_details where entry_form=289 and status_active=1");
	$roll_data_arr=array();
...
{	
	...
	// echo "<pre>";print_r($data_array);die;
	$i=2; $j=4; $k=0; $bundle_array=array(); $br=0; $n=0;
	if(count($sql_bundle_copy)!=0)
	{
		foreach($data_array as $pattern_no=>$pattern_data)
		{
...
{	
	...
	$roll_sql=sql_select("select roll_id, batch_no, shade from pro_roll_details where entry_form=289 and status_active=1");
	$roll_data_arr=array();
	foreach($roll_sql as $row)
	{
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		$field_array="id,mst_id,dtls_id,color_id,size_id,size_ratio,marker_qty,country_type,country_id,excess_perc,order_id,manual_size_name,size_wise_repeat,inserted_by,insert_date";
		for($i=1; $i<=$row_num; $i++)
		{
			...
			// {
				if($marker_qty>0)
				{
					...
					if($data_array!="") $data_array.= ",";
					$data_array.="(".$id.",".$mst_id.",".$dtls_id.",".$color_id.",".$$txt_size_id.",0,".$marker_qty.",".$$cboCountryType.",".$$cboCountry.",'".$$excess_perc."',".$$po_id.",'".$manualSizeArr[$$txt_size_id]."','".$size_wise_repeat_cut_no."',".$user_id.",'".$pc_date_time."')";
					$id=$id+1;
				}
		...
		{
			$erange=return_field_value("max(a.number_end) as last_rmg","ppl_cut_lay_bundle a, ppl_cut_lay_mst b","a.mst_id=b.id and b.entry_form=289 and b.job_no='$job_id' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","last_rmg");
		}
		...
		{
			$max_erange = 999;
			$erange=return_field_value("a.number_end as last_rmg","ppl_cut_lay_bundle a, ppl_cut_lay_mst b","a.mst_id=b.id and b.entry_form=289 and b.job_no='$job_id' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.id desc","last_rmg");
		...
		if(!$bundle_qty_check)
		{
			echo "10**Something happend wrong.Please check bundle qty.";die;
		}
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$sizeRatioArr=array(); $sizeQtyArr=array(); $sizeQtyArrForC=array(); $sizeIdAgainstSeq=array();$sizeChkArr=array();
		$manualSizeArr=array();
		$id_size=return_next_id("id", "ppl_cut_lay_size_dtls", 1);	
		$field_array_size="id,mst_id,dtls_id,color_id,size_id,size_ratio,marker_qty,bundle_sequence,inserted_by,insert_date";
		for($i=1; $i<=$size_row_num; $i++)
		...
		{
			$erange=return_field_value("max(a.number_end) as last_rmg","ppl_cut_lay_bundle a, ppl_cut_lay_mst b, ppl_cut_lay_dtls c","a.mst_id=b.id and b.id=c.mst_id and b.entry_form=289 and b.job_no='$job_id' and c.id!=$dtls_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","last_rmg");
		}
		...
		{
			$max_erange = 999;
			$erange=return_field_value("a.number_end as last_rmg","ppl_cut_lay_bundle a, ppl_cut_lay_mst b, ppl_cut_lay_dtls c","a.mst_id=b.id and b.id=c.mst_id and b.entry_form=289 and b.job_no='$job_id' and c.id!=$dtls_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.id desc","last_rmg");
			$erange = ($erange>$max_erange) ? 0 : $erange;
		}
		else if($rmg_no_creation==7) // up to 9999
		{
											...
											if($bl_size_qty>0 && $sizeRatioBlArr[$size_id][$pattern_no][$rollId]>0)
											{
												...
												if($bundle_qty>0)
												{
													...
													if($rmg_no_creation==7)// chk variable setting
													{
														...
														$rest_of_bndl_qty = 0;
														$rest_of_bndl_qty = $bundle_per_pcs - $bundle_qty;
														// echo "10**rest_of_bndl_qty=".$rest_of_bndl_qty."==".$bundle_per_pcs ."-". $bundle_qty."<br>";
													}
	                ...
	                <tr>                    
	                    ...
	                    <td align="center" width="150">
	                             <?  
									...
									echo create_drop_down( "cbo_buyer_name", 140,$sql,"id,buyer_name", 1, "-- Select --", 0, "load_drop_down( 'woven_cut_and_lay_ratio_wise_entry_controller',this.value, 'load_drop_down_brand', 'brand_td');load_drop_down( 'woven_cut_and_lay_ratio_wise_entry_controller',this.value, 'load_drop_down_buyer_season', 'buyer_season_td')", 0,"5,6,7","","","" );
	                            ?>
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		$dataArrayMst=sql_select("select a.cutting_no, b.color_id, b.order_cut_no from ppl_cut_lay_mst a, ppl_cut_lay_dtls b where a.company_id=".$cbo_company_name." and a.entry_form=289 and a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and a.job_no=$txt_job_no");
		...
		$data_array="(".$id.",289,'".$new_cutting_prifix."',".$cut_no_prifix[0].",'".$new_cutting_number."',".$tbl_id.",".$txt_job_no.",".$txt_batch_no.",".$cbo_company_name.",".$cbo_working_company_name.",".$cbo_location_name.",".$cbo_floor_name.",".$txt_entry_date.",'".$start_time."',".$txt_end_date.",'".$end_time."',".$txt_marker_length.",".$txt_marker_width.",".$txt_fabric_width.",".$txt_gsm.",".$cbo_width_dia.",".$txt_marker_cons.",".$txt_marker_excess.",".$txt_marker_total.",'".$user_id."','".$pc_date_time."',1,0)";

		$detls_id= return_next_id_by_sequence(  "ppl_cut_lay_dtls_seq",  "ppl_cut_lay_dtls", $con );
		$field_array1="id, mst_id, order_ids,country_ids, color_type_id, order_cut_no, color_id, batch_id, gmt_item_id, plies, order_qty, roll_data, inserted_by, insert_date, status_active, is_deleted";
		...
		for($i=1; $i<=$row_num; $i++)
		{
			...
			for($x=0;$x<count($save_string);$x++)
			{
				...
				$data_array_roll.="(".$id_roll.",".$barcode_no.",".$id.",".$detls_id.",".$$cbo_order_id.",289,'".$roll_qnty."','".$roll_id."','".$roll_no."','".$plies."','".$batchNo."','".$shade."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			...
			$data_array1.="(".$detls_id.",".$id.",".$$cbo_order_id.",".$$cbo_country_id.",".$$cboColorType.",".$$order_cut_no.",".$$cbocolor.",".$$cbobatch.",".$$cbo_gmt_id.",".$$txt_plics.",".$$order_qty.",'".$response_data."','".$user_id."','".$pc_date_time."',1,0)";   
	...
	{
		  	$prev_cut_no_arr=array();
			$dataArrayMst=sql_select("select a.cutting_no, b.color_id, b.order_cut_no from ppl_cut_lay_mst a, ppl_cut_lay_dtls b where a.company_id=".$cbo_company_name." and a.entry_form=289 and a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and a.job_no=$txt_job_no and a.id!=$update_id");
			foreach($dataArrayMst as $row)
			{
		    ...
		    $detls_id= return_next_id_by_sequence(  "ppl_cut_lay_dtls_seq",  "ppl_cut_lay_dtls", $con );
			$field_array1="id, mst_id,order_ids,country_ids,color_type_id,order_cut_no,ship_date,color_id,batch_id,gmt_item_id,plies,order_qty,roll_data,inserted_by,insert_date,status_active,is_deleted";
			$field_array_up="order_ids*country_ids*color_type_id*order_cut_no*ship_date*color_id*batch_id*gmt_item_id*plies*order_qty*roll_data*updated_by*update_date";
			$field_array_roll="id, barcode_no, mst_id, dtls_id, po_ids, entry_form, qnty, roll_id, roll_no, plies, batch_no, shade, inserted_by, insert_date";
			...
			for($i=1; $i<=$row_num; $i++)
				{
					...
					for($x=0;$x<count($save_string);$x++)
					{
						...
						if($data_array_roll!="") $data_array_roll.= ",";
						$data_array_roll.="(".$id_roll.",".$barcode_no.",".$msster_id.",".$dtlsId.",".$$cbo_order_id.",289,'".$roll_qnty."','".$roll_id."','".$roll_no."','".$plies."','".$batchNo."','".$shade."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
						$response_data.=$barcode_no."=".$roll_no."=".$roll_id."=".$roll_qnty."=".$plies."=".$batchNo."=".$shade."**";
						//$id_roll = $id_roll+1;
					}
					...
					if(str_replace("'",'',$$update_details_id)!="")  
					  {
							...
							$data_array_up[str_replace("'",'',$$update_details_id)]=explode("_",("".$$cbo_order_id."_".$$cbo_country_id."_".$$cboColorType."_".$$order_cut_no."_".$$txt_ship_date."_".$$cbocolor."_".$$cbobatch."_".$$cbo_gmt_id."_".$$txt_plics."_".$$order_qty."_'".$response_data."'_'".$user_id."'_'".$pc_date_time."'_1_0"));
					  ...
					  else
					  {
							...
							$data_array1.="(".$detls_id.",".$msster_id.",".$$cbo_order_id.",".$$cbo_country_id.",".$$cboColorType.",".$$order_cut_no.",'".$$txt_ship_date."',".$$cbocolor.",".$$cbobatch.",".$$cbo_gmt_id.",".$$txt_plics.",".$$order_qty.",'".$response_data."',".$user_id.",'".$pc_date_time."',1,0)";   
							$detls_id_array.=$detls_id."#".str_replace("'",'',$$order_cut_no);
							//$dtlsId=$detls_id; 
							//$detls_id=$detls_id+1;
							$detls_id= return_next_id_by_sequence(  "ppl_cut_lay_dtls_seq",  "ppl_cut_lay_dtls", $con );
			...
			$delete_roll=execute_query("delete from pro_roll_details where mst_id=$msster_id and entry_form=289",0);	
			if($data_array_roll!="")
			{
				$rID4=sql_insert("pro_roll_details",$field_array_roll,$data_array_roll,0);
			}
...
if($action=="create_cutting_search_list_view")
{
 ...
 //	if(str_replace("'","",$job_no)=="") $job_cond=""; else $job_cond="and b.job_no_prefix_num='".str_replace("'","",$job_no)."'";
	...
	//$sql_order="select a.id,a.cut_num_prefix_no, a.table_no, a.job_no, a.batch_id, a.entry_date, a.marker_length, a.marker_width, a.fabric_width, c.po_number, d.order_id,d.color_id,$year FROM ppl_cut_lay_mst a,wo_po_details_master b,wo_po_break_down c,ppl_cut_lay_dtls d where a.id=d.mst_id and a.job_no=b.job_no and b.job_no=c.job_no_mst and c.id=d.order_id and a.entry_form=289 $conpany_cond $cut_cond $job_cond $sql_cond $order_cond order by id";
	$buyer_library = return_library_array('SELECT id, buyer_name FROM lib_buyer','id','buyer_name');
	?>
	<script type="text/javascript">
    ...
    foreach($sql_data as $val)
	  {
			...
			echo "load_drop_down( 'requires/woven_cut_and_lay_ratio_wise_entry_controller','".$val[csf("working_company_id")]."', 'load_drop_down_location', 'location_td') ;";
...
if($action=="order_details_list")
{
	// $sql_gmt_arr="select ";
	 $tbl_row=0;
	 $sql_dtls=sql_select("SELECT a.id, a.order_ids,a.country_ids, a.ship_date, a.color_id, a.color_type_id, a.batch_id, a.gmt_item_id, a.plies, a.marker_qty, a.order_qty, a.total_lay_qty, a.lay_balance_qty, b.job_no, b.job_year, b.company_id, a.order_cut_no, a.roll_data from ppl_cut_lay_dtls a, ppl_cut_lay_mst b where b.id=a.mst_id and mst_id=".$data." and a.status_active=1 and a.is_deleted=0 order by a.id");
	...
	foreach($sql_dtls as $val)
	{
		...
		{
			$sql="select sum(plan_cut_qnty) as plan_qty from wo_po_color_size_breakdown where po_break_down_id in(".$val[csf("order_ids")].") and country_id in(".$val[csf("country_ids")].") and item_number_id=".$val[csf("gmt_item_id")]." and color_number_id=".$val[csf("color_id")]." and status_active=1";
		}
		else
		{
