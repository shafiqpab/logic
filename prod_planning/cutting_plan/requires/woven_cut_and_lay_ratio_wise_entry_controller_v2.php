...
if ($action=="load_drop_down_order_qty")
{
	...
	$result=sql_select($sql);$plan_qty=0;
	foreach($result as $row)
	{
	...
	$result=sql_select($sql_marker); $marker_qty=0;
...
if($action=="check_batch_no")
{
	...
	$barcodeData=sql_select( "select barcode_no from pro_roll_details where entry_form=546 and status_active=1 and is_deleted=0");
...
if($action=="roll_popup")
{
	...
	<script>
		...
		<?
			...
			$data_array=sql_select("SELECT a.batch_no, c.barcode_no, c.roll_id, c.roll_no, c.qnty, b.item_description, c.shade 
			from  pro_roll_details c, pro_batch_create_dtls b,pro_batch_create_mst a 
			where a.id=b.mst_id and b.id=c.dtls_id and  c.entry_form in (64,37) and c.po_breakdown_id in(".str_replace("'","",$order_no).") and c.status_active=1 and c.is_deleted=0"); //My new and b.color_id=$color
 			$roll_details_array=array(); $barcode_array=array(); 
			foreach($data_array as $row)
			{
		...
		{ 
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','woven_cut_and_lay_ratio_wise_entry_controller_v2.php?order_no='+<? echo $order_no; ?>+'&color='+<? echo $color; ?>+'&added_barcode_no='+added_barcode_no+'&action=batch_popup','Batch Barcode Popup', 'width=580px,height=300px,center=1,resize=1,scrolling=0','../../')
		...
		$('#txt_batch_no').live('keydown', function(e) {
			if (e.keyCode === 13) 
			{
				...
				var response_data=return_global_ajax_value( batch_no+"**"+order_id+"**"+added_barcode_no, 'check_batch_no', '', 'woven_cut_and_lay_ratio_wise_entry_controller_v2');
		...
		function openmypage_barcode()
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','woven_cut_and_lay_ratio_wise_entry_controller_v2.php?order_no='+<? echo $order_no; ?>+'&color='+<? echo $color; ?>+'&action=barcode_popup','Barcode Popup', 'width=480px,height=300px,center=1,resize=1,scrolling=0','../../')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
				var barcode_nos=this.contentDoc.getElementById("hidden_barcode_nos").value; //Barcode Nos
	        ...
	        <table width="690" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" id="tbl_list_search">
	            ...
	            <tbody>
	                <tr id="tr_1" class="general">
	                    ...
	                    <td><input type="text" id="batchNo_1" name="batchNo[]" class="text_boxes" value="" style="width:100px"/></td>
	                    <td><input type="text" id="txtshade_1" name="txtshade[]" class="text_boxes" value="" style="width:100px"/></td>
	                    <td><input type="text" id="rollWgt_1" name="rollWgt[]" class="text_boxes_numeric" value="" style="width:100px" <? echo $disbled; ?>/></td>
	                    <? if($roll_maintained==1) 
	                    { 
	                    ?>
	                    ...
	                    ?>
	                    <td><input type="text" id="plies_1" name="plies[]" class="text_boxes_numeric" value="" style="width:100px"/></td>
	                    <td width="70">
	                    	<? if($roll_maintained!=1) 
							{ 
							?>
   ...
   $print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$cbo_company_id."' and module_id=4 and report_id=117 and is_deleted=0 and status_active=1");
	...
	<script>
		...
		{   
			...
			var sequence_no='<? echo $sequence_no; ?>';
			...
			http.open("POST","woven_cut_and_lay_ratio_wise_entry_controller_v2.php",true);
			    ...
			    if(reponse[0]==0 || reponse[0]==1)
				 {
					 ...
					 else if(reponse[0]==1)
					 {
						...
						if(reponse[7]!=undefined)
						{
							alert(reponse[7]);
						}
					 }
					
					show_list_view(reponse[1]+'**'+reponse[2]+'**'+reponse[3]+'**'+reponse[7],'show_bundle_list_view','search_div','woven_cut_and_lay_ratio_wise_entry_controller_v2','setFilterGrid("list_view",-1)');
		...
		function fnc_print_bundle(type)
		{
		   ...
		   print_report(<? echo $cbo_company_id; ?>+'*'+<? echo $mst_id; ?>+'*'+<? echo $details_id; ?>+'*'+report_title+'*'+country+'*'+type, "cut_lay_bundle_print", "woven_cut_and_lay_ratio_wise_entry_controller_v2")
		}
		...
		function fnc_print_bundle_lay__22212(type)
		{
		   var report_title="Cut and Lay bundle ";
		   var country=$('#cboCountryBundle').val();
		   print_report(<? echo $cbo_company_id; ?>+'*'+<? echo $mst_id; ?>+'*'+<? echo $details_id; ?>+'*'+report_title+'*'+country+'*'+type, "lay_bundle_print", "woven_cut_and_lay_ratio_wise_entry_controller_v2")
		}
		...
		function fnc_print_qc_bundle(type)
		{
		   ...
		   var country=$('#cboCountryBundle').val();
		   if (type==1) {
			   print_report(<? echo $cbo_company_id; ?>+'*'+<? echo $mst_id; ?>+'*'+<? echo $details_id; ?>+'*'+report_title+'*'+country+'*'+type, "cut_lay_qc_bundle_print", "woven_cut_and_lay_ratio_wise_entry_controller_v2")
		   }else if(type==2){
			    print_report(<? echo $cbo_company_id; ?>+'*'+<? echo $mst_id; ?>+'*'+<? echo $details_id; ?>+'*'+report_title+'*'+country+'*'+type, "cut_lay_qc_bundle_print_2", "woven_cut_and_lay_ratio_wise_entry_controller_v2")
		   }
		}
		...
		function fnc_bundle_report(column_list)
		{
			...
			if(column_list==6)
			{
				var url=return_ajax_request_value(data, "print_report_bundle_barcode", "woven_cut_and_lay_ratio_wise_entry_controller_v2");
				window.open(url,"##");
			}
			else
			{
				var url=return_ajax_request_value(data, "print_report_bundle_barcode_eight", "woven_cut_and_lay_ratio_wise_entry_controller_v2");
				window.open(url,"##");	
			}
		}
		...
		function fnc_bundle_report_eight()
		{
			...
			var page_link = 'woven_cut_and_lay_ratio_wise_entry_controller_v2.php?data='+data+'&action=print_report_bundle_barcode_eight';
			...
			emailwindow.onclose=function()
			 {
				...
				data=data+'***'+prodID;
				var url=return_ajax_request_value(data, "print_barcode_eight", "woven_cut_and_lay_ratio_wise_entry_controller_v2");
				window.open(url,"##");	
			 }
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				//var url=return_ajax_request_value(data, "print_barcode_one_pdf", "woven_cut_and_lay_ratio_wise_entry_controller_v2");
				//window.open(url,"##");	
				window.open("woven_cut_and_lay_ratio_wise_entry_controller_v2.php?data=" + data+'&action=print_barcode_one', true );
			 }
			...
			emailwindow.onclose=function()
			 {
				...
				{
					var url=return_ajax_request_value(data, "print_barcode_one_urmi", "woven_cut_and_lay_ratio_wise_entry_controller_v2");
					// alert(url);return;
					window.open(url,"##");
				}else if(type == 5){
					var url=return_ajax_request_value(data, "print_barcode_one_urmi_eg", "woven_cut_and_lay_ratio_wise_entry_controller_v2");
					window.open(url,"##");
				}else if(type == 6){
                    var url=return_ajax_request_value(data, "print_barcode_one_urmi_eg_a4", "woven_cut_and_lay_ratio_wise_entry_controller_v2");
                    window.open(url,"##");
                }
                else
				{
					var url=return_ajax_request_value(data, "print_barcode_one_urmi_v5", "woven_cut_and_lay_ratio_wise_entry_controller_v2");
					// alert(url);return;
					window.open(url,"##");
				}
				//window.open("woven_cut_and_lay_ratio_wise_entry_controller_v2.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				var url=return_ajax_request_value(data, "print_barcode_one_128", "woven_cut_and_lay_ratio_wise_entry_controller_v2");
				window.open(url,"##");	
				//window.open("woven_cut_and_lay_ratio_wise_entry_controller_v2.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				var url=return_ajax_request_value(data, "print_barcode_one_129", "woven_cut_and_lay_ratio_wise_entry_controller_v2");
				window.open(url,"##");	
				//window.open("woven_cut_and_lay_ratio_wise_entry_controller_v2.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
		...
		function fnc_bundle_report_qrcode(work_comp,work_location)
		{
			...
			var report_title="Cut and Lay bundle ";
		   	...
		   	var title = 'Search Job No';	
			var page_link = 'woven_cut_and_lay_ratio_wise_entry_controller_v2.php?data='+data+'&action=print_report_bundle_list_popup';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=270px,height=250px,center=1,resize=0,scrolling=0','../../');
			emailwindow.onclose=function()
			 {
				var theform=this.contentDoc.forms[0]; 
				...
				//print_report(<? //echo $cbo_company_id; ?>+'*'+<?// echo $mst_id; ?>+'*'+<? //echo $details_id; ?>+'*'+report_title+'*'+country+'*'+bundle_use_for, "cut_lay_bundle_print", "woven_cut_and_lay_ratio_wise_entry_controller_v2")
			 }	   		
		}
		...
		function fnc_bundle_display_qrccode(bundle_use_for)
		{
			...
			http.open( 'POST', 'woven_cut_and_lay_ratio_wise_entry_controller_v2.php?action=print_qrcode&data='+ data );
		...
		function fnc_send_printer_text()
		{
			...
			var job_id=$("#hidden_update_job_id").val();
			data=data+"***"+job_id+'***'+<? echo $mst_id; ?>+'***'+<? echo $details_id; ?>+'***'+<? echo $cbo_gmt_id; ?>+'***'+<? echo $cbo_color_id; ?>+'***'+<? echo $order_id; ?>;
			var url=return_ajax_request_value(data, "report_bundle_text_file", "woven_cut_and_lay_ratio_wise_entry_controller_v2");
		...
		function fnc_cut_lay_bundle_reply_info()
		{
			if(http.readyState == 4) 
			{
				...
				if((reponse[0]==0 || reponse[0]==1))
				{
					...
					show_list_view(reponse[1]+'**'+reponse[2]+'**'+reponse[3],'show_bundle_list_view','search_div','woven_cut_and_lay_ratio_wise_entry_controller_v2','setFilterGrid("list_view",-1)');
				}
				else if(reponse[0]==200)
				{
		...
		function fnc_rollWiseSizeQty()
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','woven_cut_and_lay_ratio_wise_entry_controller_v2.php?rollData='+'<? echo $rollData; ?>'+'&size_data='+size_data+'&action=rollSize_popup','Roll Popup', 'width=680px,height=300px,center=1,resize=1,scrolling=0','../../')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
		...
		<div align="center" style="width:100%; overflow-y:hidden; position:absolute; top:5px;">
	        ...
	        <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
	            ...
	            <?
	                ...
	                $sql_query=sql_select("SELECT po_break_down_id, country_type, country_id, size_number_id, plan_cut_qnty, country_ship_date, size_order from wo_po_color_size_breakdown where item_number_id=$cbo_gmt_id and po_break_down_id in($order_id) and color_number_id=$cbo_color_id and status_active=1 and is_deleted=0 order by size_order,country_ship_date, country_type");
	                ...
	                $size_details=array(); $sizeId_arr=array(); $shipDate_arr=array(); $distributed_qty_arr=array(); $planQty=0;
	                foreach($sql_query as $row)
	                {
						...
						if($row[csf('country_type')]==""){ $row[csf('country_type')] = 1;}
					...
					$size_bl_qty_arr=return_library_array("select sum(a.size_qty) as size_qty, a.size_id from ppl_cut_lay_bundle a, ppl_cut_lay_dtls b where b.id=a.dtls_id and a.order_id in($order_id) and b.gmt_item_id=$cbo_gmt_id and b.color_id=$cbo_color_id and a.status_active=1 group by a.size_id",'size_id','size_qty');

					$size_bl_prev_qty_arr=return_library_array("SELECT sum(a.size_qty) as size_qty, a.size_id from ppl_cut_lay_bundle a, ppl_cut_lay_dtls b where b.id=a.dtls_id and a.order_id in($order_id) and b.gmt_item_id=$cbo_gmt_id and b.color_id=$cbo_color_id and a.status_active=1 and a.mst_id<>".$mst_id." and a.dtls_id<>".$details_id."  group by a.size_id",'size_id','size_qty');
	                ...
	                <table cellpadding="0" cellspacing="0" width="530" id="tbl_size">
	                    ...
	                    <?  
	                        ...
	                        {
	                            // echo $plan_cut_qty."-".$size_bl_prev_qty_arr[$size_id]."<br>";
	                ...
	                <script>fnc_printBtnShowHide();</script>
	                <br>
								...
								foreach($po_val as $country_type_id=>$country_val)
								{
									foreach($country_val as $country_id=>$size_data)
									{
										foreach($size_data as $size_id=>$plan_cut_qnty)
										{
											...
											// echo $plan_cut_qnty.'=='.$lay_bl_qty_arr[$po_id][$country_type_id][$country_id][$size_id].'=='.$data[1].'<br>';
...
{	
	...
	$roll_sql=sql_select("select roll_id, batch_no from pro_roll_details where entry_form=546 and status_active=1");
	$roll_data_arr=array();
...
{	
	...
	unset($sql_name);
	//echo "select roll_id, batch_no, shade from pro_roll_details where mst_id='$data[2]' and entry_form=546 and status_active=1";
	$roll_sql=sql_select("select roll_id, batch_no, shade from pro_roll_details where mst_id='$data[2]' and entry_form=546 and status_active=1");
	$roll_data_arr=array();
	foreach($roll_sql as $row)
	{
...
{	
	...
	$br=0;
	$n=0;
	$sql_roll="select roll_id, batch_no, shade from pro_roll_details where mst_id='$data[2]' and entry_form=546 and status_active=1";
	...
	if(count($sql_bundle_copy)!=0)
	{
		foreach($sql_bundle_copy as $inf)
		{
			...
			foreach($color_sizeID_arr as $val)
			{
				...
				$pdf->Code40($i+30, $j-2, "Bun# ".$val[csf("bundle_no")], $ext = true, $cks = false, $w = 0.7, $h = 1, $wide = true, true,7);

				$pdf->Code40($i, $j+1.5, "Sty# ".$style_name, $ext = true, $cks = false, $w = 0.7, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i+35, $j+1.5, "Stkr# ".$val[csf("number_start")]."-".$val[csf("number_end")], $ext = true, $cks = false, $w = 0.7, $h = 1, $wide = true, true,7) ;

				$pdf->Code40($i, $j+5, "PO# ".$po_number, $ext = true, $cks = false, $w =0.7, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i+45, $j+5, "SH# ". $shade, $ext = true, $cks = false, $w =0.7, $h = 1, $wide = true, true,7) ;

				$pdf->Code40($i, $j+8.5, "Color#".$color_library[$data[5]], $ext = true, $cks = false, $w = 0.7, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i+45, $j+8.5, "L/N# ".$lot_no, $ext = true, $cks = false, $w = 0.7, $h = 1, $wide = true, true,7) ;

				$pdf->Code40($i, $j+12, "Cut/N# ".$order_cut_no, $ext = true, $cks = false, $w = 0.7, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i+20, $j+12, "Size#".$size_arr[$val[csf("size_id")]]."(".$val[csf("pattern_no")].")", $ext = true, $cks = false, $w = 0.7, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i+45, $j+12, "Qty#".$col_size_data_arr[$val[csf("barcode_no")]], $ext = true, $cks = false, $w = 0.7, $h = 1, $wide = true, true,7) ;

				$pdf->Code40($i, $j+15.5, "P/N#".$inf[csf('bundle_use_for')], $ext = true, $cks = false, $w = 0.7, $h = 1, $wide=true,true,7);
				$pdf->Code40($i+30, $j+15.5, "Coun# ".$country, $ext = true, $cks = false, $w = 0.7, $h = 1, $wide=true,true,7);
				$pdf->Code40($i+45, $j+15.5, "T/N# ".$table_no_library[$table_name], $ext = true, $cks = false, $w = 0.7, $h = 1, $wide=true,true,7);
			

				$pdf->Code40($i, $j+19, $val[csf("barcode_no")], $ext = true, $cks = false, $w =0.15, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i+30, $j+19, "Cut/N# ".$cutting_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide=true,true,7);


				//$pdf->Code40($i, $j+22, $val[csf("bundle_no")],$w =0.2, $h = 8);
				$pdf->Code39($i, $j+26, $val[csf("barcode_no")],true,false,$w =0.24, $h = 8);
	...
	if(count($sql_bundle_copy)!=0)
	{
		foreach($sql_bundle_copy as $inf)
		{
			foreach($color_sizeID_arr as $val)
			{
				...
				$data_array[$val[csf("pattern_no")]][$inf[csf('bundle_use_for')]][$val[csf('barcode_no')]]['id'] = $inf[csf("id")];
			}
		...
		foreach($data_array as $pattern_no=>$pattern_data)
		{
			...
			foreach($pattern_data as $bundle_use_for=>$bundle_use_for_data)
			{
				...
				{					
					...
					if($seq_first==$val["id"]) $symb= "@@"; else $symb= "";
					//BNDL
					// print_r($pro_roll_data_arr); die;
					...
					$buyer_name_str=""; if($client_id!=0) $buyer_name_str=$buyer_library[$buyer_name].'-'.$buyer_library[$client_id]; else $buyer_name_str=$buyer_library[$buyer_name];
					//$dbl_no="17910000000012";
					if($val['is_excess']==1) $country="EXCESS"; else $country=$country_arr[$val["country_id"]];
					...
					$pdf->Code40($i, $j+22, $barcode, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide=true,true,7);
					...
					// $pdf->Code40($i, $j+19, $val[csf("barcode_no")], $ext = true, $cks = false, $w =0.15, $h = 1, $wide = true, true,7) ;
					//$pdf->Code40($i, $j+22, $val[csf("bundle_no")],$w =0.2, $h = 8);
					$pdf->Code39($i, $j+28, $barcode,true,false,$w =0.24, $h = 6);
...
{	
	...
	$roll_sql=sql_select("select roll_id, batch_no, shade from pro_roll_details where entry_form=546 and status_active=1");
	$roll_data_arr=array();
	foreach($roll_sql as $row)
	{
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		{
			$sql_bundle=sql_select("SELECT size_id, max(number_end) as last_rmg FROM ppl_cut_lay_bundle WHERE mst_id='".$mst_id."'  and status_active=1 and is_deleted=0 group by size_id");
		...
		if($variable_setting_for_bundle_creation==2) // job wise
		{
			$bundleNoData=sql_select("select max(b.bundle_num_prefix_no) as last_bundle_no from ppl_cut_lay_mst a, ppl_cut_lay_bundle b where a.id=b.mst_id and a.job_no='$job_id'  and b.status_active=1 and b.is_deleted=0 order by b.id desc");
			$last_bundle_no=$bundleNoData[0][csf('last_bundle_no')];
		}
		else
		{
			$bundleNoData=sql_select("select max(number_end) as last_rmg, max(bundle_num_prefix_no) as last_prefix from ppl_cut_lay_bundle where mst_id=$mst_id  and status_active=1 and is_deleted=0");
			$last_bundle_no=$bundleNoData[0][csf('last_prefix')];
		}

		$bundleData=sql_select("select max(number_end) as last_rmg, max(bundle_num_prefix_no) as last_prefix from ppl_cut_lay_bundle where mst_id=$mst_id  and status_active=1 and is_deleted=0");
		...
		for($i=1; $i<=$row_num; $i++)
		{
			...
			if($marker_qty>0)
			{
				...
				if($data_array!="") $data_array.= ",";
				$data_array.="(".$id.",".$mst_id.",".$dtls_id.",".$color_id.",".$$txt_size_id.",0,".$marker_qty.",".$$cboCountryType.",".$$cboCountry.",'".$$excess_perc."',".$$po_id.",'".$size_wise_repeat_cut_no."',".$user_id.",'".$pc_date_time."')";
				$id=$id+1;
			}
		}
		...
		{
			$size_ratio=$sizeRatioArr[$size_id]; $size_qty=$sizeQtyArr[$size_id]; 
		...
		{
			$erange=return_field_value("max(a.number_end) as last_rmg","ppl_cut_lay_bundle a, ppl_cut_lay_mst b","a.mst_id=b.id and b.entry_form=546 and b.job_no='$job_id' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","last_rmg");
		}
		...
		{
			$max_erange = 999;
			$erange=return_field_value("a.number_end as last_rmg","ppl_cut_lay_bundle a, ppl_cut_lay_mst b","a.mst_id=b.id and b.entry_form=546 and b.job_no='$job_id' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.id desc","last_rmg");
		...
		{
			$sql="SELECT b.size_id, max(b.number_end) as last_rmg FROM ppl_cut_lay_dtls a, ppl_cut_lay_bundle b WHERE a.id = b.dtls_id and a.mst_id=$mst_id  and b.status_active=1 and b.is_deleted=0 group by  b.size_id order by  b.size_id"; 
			$res = sql_select($sql);
			$erange_arr = array();
			foreach ($res as $val) 
			{
		...
		// $year_id=date('Y',time());
		if (strlen($year_id)==4) $year_id=substr($year_id,2,2);
		$barcode_suffix_no=return_field_value("max(barcode_prifix) as suffix_no","ppl_cut_lay_bundle","barcode_year=$year_id and status_active=1","suffix_no");
		...
		foreach($sizeSeqArr as $size_id=>$size_seq)
		{
			...
			if($rmg_no_creation==8) { $erange=$erange_arr[$size_id]; }
			for($k=1; $k<=$size_ratio; $k++)
			{
				/*if($size_wise_repeat_cut_no!=1)
				{
							...
							if($sizeRatioBlArr[$size_id][$pattern_no][$rollId]>0)
							{
								...
								if($plies>0 && $tmp_bl_arr[$size_id][$rollId][1]>0 && $bl_size_qty>0)
								{
									...
									// {
										...
										$barcode_suffix_no=$barcode_suffix_no+1;
										$barcode_no=$year_id."546".str_pad($barcode_suffix_no,10,"0",STR_PAD_LEFT);
								...
								if($plies>0 && $bl_roll_plies>0 && $bl_size_qty>0 && $temp_bal_flag==1)
								{
									...
									else
									{
										...
										// {
											...
											{
												$erange = ($erange>9999) ? 9999 : $erange;
												$bundle_qty2 = ($erange>=9999) ? 9999 - $srange : $bundle_qty2;
												$tot_bundle_qty+=$bundle_qty2;
											}
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		// =================== getting last bundle no ======================
		$prev_dtls_id = return_field_value("id","ppl_cut_lay_dtls","mst_id=$mst_id and sequence<$sequence_no and status_active=1 and is_deleted=0","id");
		if($variable_setting_for_bundle_creation==2) // job wise
		{
			$bundleNoData=sql_select("select max(b.bundle_num_prefix_no) as last_bundle_no from ppl_cut_lay_mst a, ppl_cut_lay_bundle b where a.id=b.mst_id and a.job_no='$job_id'  and status_active=1 and is_deleted=0 order by b.id desc");
			$last_bundle_no=$bundleNoData[0][csf('last_bundle_no')];
		}
		else
		{
			$bundleNoData=sql_select("select max(number_end) as last_rmg, max(bundle_num_prefix_no) as last_prefix from ppl_cut_lay_bundle where mst_id=$mst_id and dtls_id=$prev_dtls_id  and status_active=1 and is_deleted=0");
			$last_bundle_no=$bundleNoData[0][csf('last_prefix')];
		}
		
		$bundleData=sql_select("select max(number_end) as last_rmg, max(bundle_num_prefix_no) as last_prefix from ppl_cut_lay_bundle where mst_id=$mst_id and dtls_id!=$dtls_id  and status_active=1 and is_deleted=0");
		if($rmg_no_creation==8) // Size wise multi color
		{
			$bundleData=sql_select("SELECT  max(a.bundle_num_prefix_no) as last_prefix from ppl_cut_lay_bundle a, ppl_cut_lay_dtls b where a.dtls_id=b.id and a.mst_id=$mst_id and a.dtls_id!=$dtls_id and b.sequence<$sequence_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
		}
		$last_rmg_no=$bundleData[0][csf('last_rmg')];
		$last_bundle_no=$bundleData[0][csf('last_prefix')];
		...
		{
			$erange=return_field_value("max(a.number_end) as last_rmg","ppl_cut_lay_bundle a, ppl_cut_lay_mst b, ppl_cut_lay_dtls c","a.mst_id=b.id and b.id=c.mst_id and b.entry_form=546 and b.job_no='$job_id' and c.id!=$dtls_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0","last_rmg");
		}
		...
		{
			$max_erange = 999;
			$erange=return_field_value("a.number_end as last_rmg","ppl_cut_lay_bundle a, ppl_cut_lay_mst b, ppl_cut_lay_dtls c","a.mst_id=b.id and b.id=c.mst_id and b.entry_form=546 and b.job_no='$job_id' and c.id!=$dtls_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.id desc","last_rmg");
			$erange = ($erange>$max_erange) ? 0 : $erange;
		}
		else if($rmg_no_creation==7) // up to 9999
		{
		...
		{
			$sql="SELECT b.size_id, max(b.number_end) as last_rmg FROM ppl_cut_lay_dtls a, ppl_cut_lay_bundle b WHERE a.id = b.dtls_id and a.mst_id=$mst_id and a.id!=$dtls_id and a.sequence<$sequence_no  and b.status_active=1 and b.is_deleted=0 group by  b.size_id order by  b.size_id"; 
			// echo "10**".$sql;die();
			$res = sql_select($sql);
			$erange_arr = array();
			foreach ($res as $val) 
					...
					foreach($size_country_array[$size_id] as $order_id=>$order_data)
					{
						...
						{						
							if($sizeRatioBlArr[$size_id][$pattern_no][$rollId]>0)
							{
								...
								//echo $bl_roll_plies."**".$bl_size_qty."**".$plies."pp";
								//$bl_size_qty=$size_country_array[$size_id][$country_id];
								if($plies>0 && $bl_roll_plies>0 && $bl_size_qty>0 && $temp_bal_flag==1)
								{
									// echo $bl_roll_plies.">=".$bundle_per_pcs."==".$bundle_prif_no;die();
									if($bl_roll_plies>=$bundle_per_pcs)
									{ 
										$bundle_per_size=ceil($bl_roll_plies/$bundle_per_pcs);
										for($z=1; $z<=$bundle_per_size; $z++)
		...
		/*================================================================================/
		/							Bundle and RMG NO Adjust							  /
		/================================================================================*/
		if($rmg_no_creation==8)
		{
			$sql = sql_select("SELECT id as dtls_id,color_id,gmt_item_id,order_ids,roll_data,pcs_per_bundle,sequence from ppl_cut_lay_dtls where status_active=1 and is_deleted=0 and sequence>$sequence_no and mst_id=$mst_id");
			if(count($sql)>0)
			{
				foreach ($sql as $val) 
				{				
					$dtls_id = $val['DTLS_ID'];
					$gmt_id = $val['GMT_ITEM_ID'];
					$color_id = $val['COLOR_ID'];
					$order_id = $val['ORDER_IDS'];
					$roll_data = $val['ROLL_DATA'];
					$sequence_no = $val['SEQUENCE']-1; // always take prev sequence no


					$previous_barcode_data=sql_select("select bundle_no,barcode_no,barcode_year,barcode_prifix from ppl_cut_lay_bundle where mst_id=".$mst_id."  and  dtls_id=".$dtls_id." and status_active=1 and is_deleted=0 ");
					$previous_barcode_arr = array();
					foreach($previous_barcode_data as $b_val)
					{
						$previous_barcode_arr[$b_val[csf("bundle_no")]]['year']=$b_val[csf("barcode_year")];
						$previous_barcode_arr[$b_val[csf("bundle_no")]]['prifix']=$b_val[csf("barcode_prifix")];
					...
					$sizeRatioArr=array(); $sizeQtyArr=array(); $sizeQtyArrForC=array(); $sizeIdAgainstSeq=array();$sizeSeqArr = array();
					$sql_size_dtls = sql_select("SELECT size_id,size_ratio,bundle_sequence,marker_qty from ppl_cut_lay_size_dtls where status_active=1 and is_deleted=0 and color_id=$color_id and mst_id=$mst_id and dtls_id=$dtls_id");

					if(count($sql_size_dtls)>0)
					{
						foreach($sql_size_dtls as $size_dtls)
						{
							$size_id=$size_dtls['SIZE_ID'];
							// $txt_layf_balance=$size_dtls['MARKER_QTY'];
							$txt_sizef_ratio=$size_dtls['SIZE_RATIO'];
							$txt_sizef_qty=$size_dtls['MARKER_QTY'];
							$bundle_sequence=$size_dtls['BUNDLE_SEQUENCE'];
							...
							if($txt_sizef_qty>0)
							{
								...
								$sizeRatioArr[$size_id]=$txt_sizef_ratio;
								$sizeQtyArr[$size_id]=$txt_sizef_qty;
								$sizeQtyArrForC[$size_id]=$txt_sizef_qty;
								$sizeSeqArr[$size_id]=$seq;
							}
						}
						...
						// echo "<pre>";print_r($roll_no_arr);die();
						
						$bundle_no_array=array(); 
						$last_rmg_no='';
						// $bundle_ids=return_next_id("id", "ppl_cut_lay_bundle",1);
						$bundle_id=$bundle_id+1;
						// echo "10**$bundle_ids**".$bundle_id;die;
						...
						$tot_marker_qnty_curr=0; $bundle_prif_no=$last_bundle_no; $size_country_array=array(); $country_type_array=array(); 
						//echo "10**"."SELECT size_id, max(number_end) as last_rmg FROM ppl_cut_lay_bundle WHERE mst_id='".$mst_id."' and dtls_id!='".$dtls_id."' group by size_id";die;
						$sql_size_dtls = sql_select("SELECT order_id,size_id,size_ratio,bundle_sequence,marker_qty,country_id,country_type from ppl_cut_lay_size where status_active=1 and is_deleted=0 and color_id=$color_id and mst_id=$mst_id and dtls_id=$dtls_id");
						foreach($sql_size_dtls as $size_dtls)
						{							
							$marker_qty=0;
							$order_id=$size_dtls['ORDER_ID'];
							$size_id=$size_dtls['SIZE_ID'];
							$country_id=$size_dtls['COUNTRY_ID'];
							$country_type=$size_dtls['COUNTRY_TYPE'];
							$marker_qty=$size_dtls['MARKER_QTY'];
							$country_type_array[$order_id][$country_id]=$country_type;
							
							if($marker_qty>0)
							{
								$size_country_array[$size_id][$order_id][$country_id]+=$marker_qty;
								$tot_marker_qnty_curr+=$marker_qty;
							}
						}
						...
						$rID4=sql_update("ppl_cut_lay_bundle",$field_array_up,$data_array_up,"dtls_id",$dtls_id,0); 					
						
						$rID2=sql_insert("ppl_cut_lay_bundle",$field_array_bundle,$data_array_bundle,0);
						// echo "10**insert into ppl_cut_lay_bundle($field_array_bundle) values".$data_array_bundle;die;
						
						// echo "10**dfdfd".$rID2."**".$rID4;die;	
						...
						if($db_type==2 || $db_type==1)
						{
							...
							{
								oci_commit($con);  
								echo "**RMG Serial Successfully Adjust";
							}
							else
							{
								oci_rollback($con);
								echo "**RMG Serial Not Adjust";
							}
						}
					}	
				}
...
if($action=="cut_lay_bundle_print")
{
    ...
    <table width="990" cellspacing="0" align="center">
        ...
        <tr>
            ...
            <td><strong>Gsm:</strong></td><td><? echo $dataArray[0][csf('gsm')]; ?></td>
        </tr>
        <tr>
...
if($action=="lay_bundle_print")
{
    ...
    <table width="990" cellspacing="0" align="center">
        ...
        <tr>
        	<td width="130"><strong>Buyer:</strong></td><td width="200" align="left"><? echo $buyer_library[$sql_buyer[0][csf('buyer_name')]]; ?></td>
            <td width="130"><strong>Style Ref :</strong></td> <td width="200"><?  echo $style_ref; ?></td>
            <td width="130"><strong>Job No :</strong></td> <td><? echo $dataArray[0][csf('job_no')]; ?></td>
        </tr>
        <tr>
            <td><strong>Lay Qty:</strong></td><td align="left"><? echo $dataArray[0][csf('plies')]; ?></td>
            <td><strong>Sys Cut No:</strong></td><td><? echo $cut_no; ?></td>
            <td><strong>Gmt Item:</strong></td><td><? echo $garments_item[$dataArray[0][csf('gmt_item_id')]]; ?></td>
        </tr>
        <tr>
       		 <td><strong>W. Company:</strong></td> <td align="left"><? echo $working_comp_library[$dataArray[0][csf('working_company_id')]]; ?></td>
             <td><strong>W. Location:</strong></td> <td><? echo $working_location_library[$dataArray[0][csf('location_id')]]; ?></td>
             <td><strong>W. Floor:</strong></td> <td><? echo $working_floor_library[$dataArray[0][csf('floor_id')]]; ?></td>
        </tr>
    </table>
                     ...
                     foreach($size_data as $size_val)
                     {
                        ...
                        { 
                           ...
                           <tr>
                               <td align="center"><? echo $j;  ?></td>
                               <td align="center"><? echo $row[csf('bundle_num_prefix_no')]; //$row[csf('bundle_num_prefix_no')];  ?></td>
                               <td align="center"><? echo $order_cut_arr[$row[csf('dtls_id')]]['order_cut_no']; //$cut_no; //$order_cut_no; ?></td>
...
if($action=="cut_lay_qc_bundle_print_2")//[QC Bundle-2
{
	...
	if($btnType==2) { $tblwidth="1605"; $totTrSpan="7"; }
	                ...
	                <tr>                    
	                    ...
	                    <td align="center" width="150">
	                             <?  
									...
									echo create_drop_down( "cbo_buyer_name", 140,$sql,"id,buyer_name", 1, "-- Select --", 0, "load_drop_down( 'woven_cut_and_lay_ratio_wise_entry_controller_v2',this.value, 'load_drop_down_brand', 'brand_td');load_drop_down( 'woven_cut_and_lay_ratio_wise_entry_controller_v2',this.value, 'load_drop_down_buyer_season', 'buyer_season_td')", 0,"5,6,7","","","" );
	                            ?>
...
if($action=="create_job_search_list_view")
{
	...
	if(str_replace("'","",$brand_name)==0) $brand_name_cond=""; else $brand_name_cond="and b.brand_id=".str_replace("'","",$brand_name)."";
	if(str_replace("'","",$season_name)==0) $season_name_cond=""; else $season_name_cond="and b.season_buyer_wise=".str_replace("'","",$season_name)."";
	if(str_replace("'","",$season_year)==0) $season_year_cond=""; else $season_year_cond="and b.season_year=".str_replace("'","",$season_year)."";
	...
	$arr=array (1=>$buyer_brand_arr,2=>$buyer_season_arr,6=>$body_wash_color_arr,7=>$buyer_arr);

	echo create_list_view("list_view", "Job NO,Brand, Season, Season Year,Year,Style Ref,Body/Wash Color,Buyer Name,File No,Internal Ref. No, Order No,Shipment Date","60,100,100,100,60,150,100,150,100,100,150,100","1400","270",0, $sql_order , "js_set_order", "job_no,buyer_name,year", "", 1, "0,brand_id,season_buyer_wise,0,0,0,body_wash_color,buyer_name,0,0,0,0,0", $arr, "job_prefix,brand_id,season_buyer_wise,season_year,year,style_ref_no,body_wash_color,buyer_name,file_no,grouping,po_number,pub_shipment_date", "","setFilterGrid('list_view',-1)") ;	
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		$dataArrayMst=sql_select("select a.cutting_no, b.color_id, b.order_cut_no from ppl_cut_lay_mst a, ppl_cut_lay_dtls b where a.company_id=".$cbo_company_name." and a.entry_form=546 and a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and a.job_no=$txt_job_no");
		...
		$data_array="(".$id.",546,'".$new_cutting_prifix."',".$cut_no_prifix[0].",'".$new_cutting_number."',".$tbl_id.",".$txt_job_no.",".$txt_batch_no.",".$cbo_company_name.",".$cbo_working_company_name.",".$cbo_location_name.",".$cbo_floor_name.",".$txt_entry_date.",'".$start_time."',".$txt_end_date.",'".$end_time."',".$txt_marker_length.",".$txt_marker_width.",".$txt_fabric_width.",".$txt_gsm.",".$cbo_width_dia.",".$txt_marker_cons.",".$txt_marker_excess.",".$txt_marker_total.",'".$user_id."','".$pc_date_time."',1,0)";

		$detls_id= return_next_id_by_sequence(  "ppl_cut_lay_dtls_seq",  "ppl_cut_lay_dtls", $con );
		$field_array1="id, mst_id, order_ids, color_type_id, order_cut_no, color_id, batch_id, gmt_item_id, plies, order_qty, roll_data, sequence, inserted_by, insert_date, status_active, is_deleted";
		...
		for($i=1; $i<=$row_num; $i++)
		{
			...
			for($x=0;$x<count($save_string);$x++)
			{
				...
				$data_array_roll.="(".$id_roll.",".$barcode_no.",".$id.",".$detls_id.",".$$cbo_order_id.",546,'".$roll_qnty."','".$roll_id."','".$roll_no."','".$plies."','".$batchNo."','".$shade."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			...
			$data_array1.="(".$detls_id.",".$id.",".$$cbo_order_id.",".$$cboColorType.",".$$order_cut_no.",".$$cbocolor.",".$$cbobatch.",".$$cbo_gmt_id.",".$$txt_plics.",".$$order_qty.",'".$response_data."',".$i.",'".$user_id."','".$pc_date_time."',1,0)";   
	...
	{
	  	$prev_cut_no_arr=array();
		$dataArrayMst=sql_select("select a.cutting_no, b.color_id, b.order_cut_no from ppl_cut_lay_mst a, ppl_cut_lay_dtls b where a.company_id=".$cbo_company_name." and a.entry_form=546 and a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and a.job_no=$txt_job_no and a.id!=$update_id");
		foreach($dataArrayMst as $row)
		{
	    ...
	    $detls_id= return_next_id_by_sequence(  "ppl_cut_lay_dtls_seq",  "ppl_cut_lay_dtls", $con );
		$field_array1="id, mst_id,order_ids,color_type_id,order_cut_no,ship_date,color_id,batch_id,gmt_item_id,plies,order_qty,roll_data,sequence,inserted_by,insert_date,status_active,is_deleted";
		...
		$prev_roll_id_arr = return_library_array( "SELECT id,id as roll_id from PRO_ROLL_DETAILS where mst_id=$update_id and entry_form=546", "id", "roll_id"  );
		...
		for($i=1; $i<=$row_num; $i++)
		{
			...
			if(str_replace("'",'',$$update_details_id)!="") $dtlsId=str_replace("'",'',$$update_details_id); else $dtlsId=$detls_id; 
		    
			$save_string=explode("**",str_replace("'",'',$$rollData)); $response_data='';			
			
			/* if($prev_roll_data_arr[$dtlsId]!="")
			{
				$response_data.=$prev_roll_data_arr[$dtlsId]."**";
			} */
			...
			for($x=0;$x<count($save_string);$x++)
			{
				...
				if($prev_roll_id_arr[$roll_id]=="")
				{
					...
					if($data_array_roll!="") $data_array_roll.= ",";
					$data_array_roll.="(".$id_roll.",".$barcode_no.",".$msster_id.",".$dtlsId.",".$$cbo_order_id.",546,'".$roll_qnty."','".$roll_id."','".$roll_no."','".$plies."','".$batchNo."','".$shade."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					//$id_roll = $id_roll+1;
				}
				$response_data.=$barcode_no."=".$roll_no."=".$roll_id."=".$roll_qnty."=".$plies."=".$batchNo."=".$shade."**";
			}
			...
			if(str_replace("'",'',$$update_details_id)!="")  
			{
			  ...
			  {
			  		$sequence_no = return_field_value("max(sequence)","ppl_cut_lay_dtls","mst_id=$msster_id");
					$sequence_no = $sequence_no+1;
					if ($data_array1){ $data_array1 .=","; $detls_id_array .="_"; }
					$data_array1.="(".$detls_id.",".$msster_id.",".$$cbo_order_id.",".$$cboColorType.",".$$order_cut_no.",'".$$txt_ship_date."',".$$cbocolor.",".$$cbobatch.",".$$cbo_gmt_id.",".$$txt_plics.",".$$order_qty.",'".$response_data."',".$sequence_no.",".$user_id.",'".$pc_date_time."',1,0)";   
					$detls_id_array.=$detls_id."#".str_replace("'",'',$$order_cut_no);
					//$dtlsId=$detls_id; 
					//$detls_id=$detls_id+1;
					$detls_id= return_next_id_by_sequence(  "ppl_cut_lay_dtls_seq",  "ppl_cut_lay_dtls", $con );
		...
		if(count($update_roll_id_arr)>0)
		{
			...
			{		
				...
				$delete_roll=execute_query("delete from pro_roll_details where mst_id=$msster_id and id in($delete_roll_ids) and entry_form=546",0);	
			}
		}
		$rID4 = 1;
		if($data_array_roll!="")
		...
		// echo "10**".$rID ."**". $rID1 ."**". $rID2 ."**". $rID3 ."**". $rID4."**".$delete_roll."==".$data_array_roll;die;
	        ...
	        <form name="searchprocessfrm_1"  id="searchprocessfrm_1" autocomplete="off">
	            ...
	            <div style="width:390px; overflow-y:scroll; max-height:280px;" id="buyer_list_view" align="center">
	                ...
	                <?
	                    ...
	                    $i=1; $po_row_id=''; $poIdArr=explode(",",$poId);
...
if($action=="create_cutting_search_list_view")
{
	...
	if(str_replace("'","",$wo_company)==0) $wo_conpany_cond=""; else $wo_conpany_cond="and a.working_company_id=".str_replace("'","",$wo_company)."";
 ...
 //	if(str_replace("'","",$job_no)=="") $job_cond=""; else $job_cond="and b.job_no_prefix_num='".str_replace("'","",$job_no)."'";
	...
	if ($job_year != 0){
			if($db_type==2) { $job_year_cond=" and extract(year from b.insert_date)=$job_year";}
			if($db_type==0) {$job_year_cond=" and SUBSTRING_INDEX(b.insert_date, '-', 1)=$job_year";}
		 }
	...
	//$sql_order="select a.id,a.cut_num_prefix_no, a.table_no, a.job_no, a.batch_id, a.entry_date, a.marker_length, a.marker_width, a.fabric_width, c.po_number, d.order_id,d.color_id,$year FROM ppl_cut_lay_mst a,wo_po_details_master b,wo_po_break_down c,ppl_cut_lay_dtls d where a.id=d.mst_id and a.job_no=b.job_no and b.job_no=c.job_no_mst and c.id=d.order_id and a.entry_form=546 $conpany_cond $cut_cond $job_cond $sql_cond $order_cond order by id";
	$buyer_library = return_library_array('SELECT id, buyer_name FROM lib_buyer','id','buyer_name');
	?>
	<script type="text/javascript">
    ...
    foreach($sql_data as $val)
	  {
			...
			echo "load_drop_down( 'requires/woven_cut_and_lay_ratio_wise_entry_controller_v2','".$val[csf("working_company_id")]."', 'load_drop_down_location', 'location_td') ;";
			...
			echo "load_drop_down( 'requires/woven_cut_and_lay_ratio_wise_entry_controller_v2','".$val[csf("location_id")]."', 'load_drop_down_floor', 'floor_td') ;"; 
			echo "document.getElementById('cbo_floor_name').value  = '".($val[csf("floor_id")])."';\n"; 
...
if($action=="order_details_list")
{
	// $sql_gmt_arr="select ";
	 $tbl_row=0;
	 $sql_dtls=sql_select("SELECT a.id, a.order_ids, a.ship_date, a.color_id, a.color_type_id, a.batch_id, a.gmt_item_id, a.plies, a.marker_qty, a.order_qty, a.total_lay_qty, a.lay_balance_qty, b.job_no, b.job_year, b.company_id, a.order_cut_no, a.roll_data,a.sequence from ppl_cut_lay_dtls a, ppl_cut_lay_mst b where b.id=a.mst_id and mst_id=".$data." and a.status_active=1 and a.is_deleted=0 order by a.id");
	...
	{
		$sql="select sum(plan_cut_qnty) as plan_qty from wo_po_color_size_breakdown where po_break_down_id in(".$val[csf("order_ids")].") and item_number_id=".$val[csf("gmt_item_id")]." and color_number_id=".$val[csf("color_id")]." and status_active=1";
		$result=sql_select($sql); $plan_qty=0;
		foreach($result as $row)
		{
			$plan_qty+=$row[csf("plan_qty")];
		...
		$sql_marker="select sum(marker_qty) as mark_qty from ppl_cut_lay_dtls where order_ids in ('".$val[csf("order_ids")]."') and gmt_item_id=".$val[csf("gmt_item_id")]." and color_id=".$val[csf("color_id")]." and status_active=1";
	   ...
	   <tr class="" id="tr_<? echo $tbl_row; ?>" style="height:10px;">
       		<td align="center" id="color_<? echo $tbl_row; ?>">
				 <? 
					 echo create_drop_down( "cbocolor_".$tbl_row, 100, $color_item_arr,"", 1, "select color", $val[csf('color_id')], "reset_fld(".$tbl_row.");check_color(".$tbl_row.")");
				 ?>
			</td>                             
			...
			<td align="center" id="orderId_<? echo $tbl_row; ?>">
                ...
                <input type="hidden" name="sequence_<? echo $tbl_row; ?>"  id="sequence_<? echo $tbl_row; ?>" value="<? echo $val[csf('sequence')]; ?>" />
			</td> 
