...
if($action=="size_popup")
{
   ...
   $print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$cbo_company_id."' and module_id=4 and report_id=117 and is_deleted=0 and status_active=1");
	...
	<script>
		...
		function fnc_print_bundle_lay(type)
		{
		   var report_title="Cut and Lay bundle ";
		   var country=$('#cboCountryBundle').val();
	                        ...
	                        <table>
	                            ...
	                            <tr>
	                                <td align="center" colspan="7" >
	                                    ...
	                                    <input type="button" id="btn15" name="btn15" value="Lay Bundle" class="formbutton" onClick="fnc_print_bundle_lay(1);" style="display:none"/>
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		{
			$erange=return_field_value("max(a.number_end) as last_rmg","ppl_cut_lay_bundle a, ppl_cut_lay_mst b","a.mst_id=b.id and b.entry_form=289 and b.job_no='$job_id' and a.status_active=1 and a.is_deleted=0","last_rmg");
		}
		else if($rmg_no_creation==4) // order wise
		{
			$erange=return_field_value("max(a.number_end) as last_rmg","ppl_cut_lay_bundle a, ppl_cut_lay_dtls b","a.dtls_id=b.id and a.order_id=$order_id and a.status_active=1 and a.is_deleted=0","last_rmg");
		}
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		{
			$erange=return_field_value("max(a.number_end) as last_rmg","ppl_cut_lay_bundle a, ppl_cut_lay_mst b, ppl_cut_lay_dtls c","a.mst_id=b.id and b.id=c.mst_id and b.entry_form=289 and b.job_no='$job_id' and c.id!=$dtls_id and a.status_active=1 and a.is_deleted=0","last_rmg");
		}
		else if($rmg_no_creation==4)
		{
			$erange=return_field_value("max(a.number_end) as last_rmg","ppl_cut_lay_bundle a, ppl_cut_lay_dtls b","a.dtls_id=b.id and a.order_id='$order_id' and b.id!=$dtls_id and a.status_active=1 and a.is_deleted=0","last_rmg");
...
if($action=="lay_bundle_print")
{
    ...
    <table width="990" cellspacing="0" align="center">
        ...
        <tr>
        	<td width="130"><strong>Buyer:</strong></td><td width="200"><? echo $buyer_library[$sql_buyer[0][csf('buyer_name')]]; ?></td>
            <td width="130"><strong>Style Ref :</strong></td> <td width="200"><?  echo $style_ref; ?></td>
            <td width="130"><strong>Job No :</strong></td> <td><? echo $dataArray[0][csf('job_no')]; ?></td>
        </tr>
        <tr>
            <td><strong>Lay Qty:</strong></td><td><? echo $dataArray[0][csf('plies')]; ?></td>
