	...
	<script>
		...
		function fnc_print_qc_bundle(type) {
			...
			if (type == 1) {
				...
				print_report(<? echo $cbo_company_id; ?> + '*' + <? echo $mst_id; ?> + '*' + <? echo $details_id; ?> + '*' + report_title + '*' + country + '*' + type, "cut_lay_qc_bundle_print_3", "woven_cut_and_lay_ratio_wise_entry_controller")
			}
		}
		...
		function fnc_bundle_report_qrcode(work_comp, work_location) {
			...
			emailwindow.onclose = function() {
				...
				//print_report(<? //echo $cbo_company_id;
								?>+'*'+<? // echo $mst_id;
																	?>+'*'+<? //echo $details_id;
																							?>+'*'+report_title+'*'+country+'*'+bundle_use_for, "cut_lay_bundle_print", "woven_cut_and_lay_ratio_wise_entry_controller")
			}
		}
		...
		function fnc_bundle_report_qrcode_v2(type)
		{
			...
			emailwindow.onclose = function()
			{
				...
				http.open('POST', 'woven_cut_and_lay_ratio_wise_entry_controller.php?action=' + action + '&data=' + data);
			...
			<form name="searchorderfrm_1" id="searchorderfrm_1" autocomplete="off">
				...
				<fieldset style="width:905px;">
					...
					foreach ($sql_query as $row) {
						...
						$size_details[$row[csf('sequence_no')]][$po_id][$row[csf('country_type')]][$country_id][$row[csf('size_number_id')]] += $row[csf("plan_cut_qnty")];
					...
					$distributed_qty_arr = array();
					$lay_blData = sql_select("SELECT sum(a.size_qty) as marker_qty, a.size_id from ppl_cut_lay_bundle a where a.mst_id=" . $mst_id . " and a.dtls_id=" . $details_id . " $size_contry_cond and a.status_active=1 group by a.size_id");
					foreach ($lay_blData as $value) {
						$distributed_qty_arr[$value[csf('size_id')]] += $value[csf('marker_qty')];
					}

					$lay_bl_qty_arr = array();
					$lay_blData = sql_select("SELECT a.order_id, sum(a.size_qty) as marker_qty, a.country_type, a.country_id, a.size_id from ppl_cut_lay_bundle a, ppl_cut_lay_dtls b where b.id=a.dtls_id and a.order_id in($order_id) and b.gmt_item_id=$cbo_gmt_id and b.color_id=$cbo_color_id $size_contry_cond and a.status_active=1 group by a.order_id, a.country_type,a.country_id, a.size_id");
					foreach ($lay_blData as $value) {
...
if ($action == "print_qrcode_operation_69_38") // QR 69X38
{
	...
	$shade_arr = return_library_array("SELECT id, shade from PRO_ROLL_DETAILS where mst_id=$data[2] and entry_form=289", 'id', 'shade');
		...
		if (count($sql_bundle_copy) != 0) {
			foreach ($sql_bundle_copy as $inf) {
				...
				foreach ($color_sizeID_arr as $val) {
					...
					$pdf->Code40($i, $j + 1.6, "STY- " . substr($style_name, 0, 18), $ext = true, $cks = false, $w = 0.7, $h = 1, $wide = true, true, 7);
	            ...
	            <div style="width:390px; overflow-y:scroll; max-height:280px;" id="buyer_list_view" align="center">
	                ...
	                <?
						...
						if($country_seq==1)
						{
							...
							$sql_country="SELECT a.country_id, b.country_name, a.country_ship_date from  wo_po_color_size_breakdown a,lib_country b where a.country_id=b.id and  a.po_break_down_id in($poId) and a.item_number_id=".$gmt_id." and a.color_number_id=".$cbocolor." and a.status_active=1 order by a.country_ship_date ASC";
						}
						else
						{
...
if ($action == "save_update_delete") {
	...
	if ($operation == 0) // Insert Here----------------------------------------------------------
	{
		...
		$field_array1 = "id, mst_id, order_ids,country_ids, iscountry_seq,color_type_id, order_cut_no, color_id, batch_id, gmt_item_id, plies, order_qty, roll_data,isorder_seq,order_seq_data, inserted_by, insert_date, status_active, is_deleted";
	...
	else if ($operation == 1) // Update Here----------------------------------------------------------
	{
		...
		$detls_id = return_next_id_by_sequence("ppl_cut_lay_dtls_seq",  "ppl_cut_lay_dtls", $con);
		$field_array1 = "id, mst_id,order_ids,country_ids,iscountry_seq,color_type_id,order_cut_no,ship_date,color_id,batch_id,gmt_item_id,plies,order_qty,roll_data,isorder_seq,order_seq_data,inserted_by,insert_date,status_active,is_deleted";
		$field_array_up = "order_ids*country_ids*iscountry_seq*color_type_id*order_cut_no*ship_date*color_id*batch_id*gmt_item_id*plies*order_qty*roll_data*isorder_seq*order_seq_data*updated_by*update_date";
		$field_array_roll = "id, barcode_no, mst_id, dtls_id, po_ids, entry_form, qnty, roll_id, roll_no, plies, batch_no, shade, inserted_by, insert_date";
		...
		// echo "10**$rID && $rID1 && $rID2 && $rID3 && $rID4 && $rID5 && $rID6 && $delete_roll";die;
		if ($db_type == 0) {
...
if ($action == "order_details_list") {
	...
	foreach ($sql_dtls as $val) {
		if ($val[csf("country_ids")] != "")
		{
			...
			$sql = "select sum(plan_cut_qnty) as plan_qty from wo_po_color_size_breakdown where po_break_down_id in(" . $val[csf("order_ids")] . ") and country_id in($country_ids) and item_number_id=" . $val[csf("gmt_item_id")] . " and color_number_id=" . $val[csf("color_id")] . " and status_active=1";
		}
		else
		{
		...
		if ($val[csf("country_ids")] != "") {
			$sql_marker = "select sum(b.size_qty) as mark_qty from ppl_cut_lay_dtls a,ppl_cut_lay_bundle b where a.id=b.dtls_id and b.order_id in(" . $val[csf("order_ids")] . ") and b.country_id in($country_ids) and a.gmt_item_id=" . $val[csf("gmt_item_id")] . " and a.color_id=" . $val[csf("color_id")] . " and a.status_active=1";
		} else {
