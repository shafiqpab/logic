...
if ($action == "load_drop_down_order_qty") {
	...
	$sql = "SELECT sum(plan_cut_qnty) as plan_qty from  wo_po_color_size_breakdown where po_break_down_id in(" . $ex_data[0] . ") and item_number_id=" . $ex_data[1] . " and color_number_id=" . $ex_data[2] . " and country_id in(" . $ex_data[4] . ") and status_active=1 and is_deleted=0";
	$result = sql_select($sql);
	foreach ($result as $row) {
	...
	/* $sql_marker="SELECT sum(b.marker_qty) as mark_qty from  ppl_cut_lay_dtls a, ppl_cut_lay_size b where a.id=b.dtls_id and b.order_id in(".$ex_data[0].") and a.gmt_item_id=".$ex_data[1]." and a.color_id=".$ex_data[2]." and b.country_id=".$ex_data[4]." and a.status_active=1 and b.status_active=1 and b.is_deleted=0"; */
	$sql_marker = "SELECT sum(b.size_qty) as mark_qty from  ppl_cut_lay_dtls a, ppl_cut_lay_bundle b where a.id=b.dtls_id and b.order_id in(" . $ex_data[0] . ") and a.gmt_item_id=" . $ex_data[1] . " and a.color_id=" . $ex_data[2] . " and b.country_id in(" . $ex_data[4] . ") and a.status_active=1 and b.status_active=1 and b.is_deleted=0";
...
?>
	<script>
		var batch_selection_in_plies_popup = '<?= $batch_selection_in_plies_popup; ?>';
		...
		function openmypage_batch() {
			...
			emailwindow = dhtmlmodal.open('EmailBox', 'iframe', 'cut_and_lay_ratio_wise_entry_controller_urmi_for_qc.php?order_no=' + <? echo $order_no; ?> + '&color=' + <? echo $color; ?> + '&added_barcode_no=' + added_barcode_no + '&action=batch_popup', 'Batch Barcode Popup', 'width=580px,height=300px,center=1,resize=1,scrolling=0', '../../')
		...
		$(document).ready(function(e) {
			...
			if (rollData != "") {
				...
				for (var k = 0; k < data.length; k++) {
					...
					$("#shade_" + row_num).val(shade);
					$("#plies_" + row_num).val(plies);
					$("#batchNo_" + row_num).val(batchNo);
					$("#cbobatchid_" + row_num).val(batchId);
		...
		function add_break_down_tr(i) {
			...
			$("#rollNo_" + row_num).val('');
			$("#rollId_" + row_num).val('');
			$("#cbobatchid_" + row_num).val(0);
			$("#batchNo_" + row_num).val('');
			$("#shade_" + row_num).val('');
			$("#rollWgt_" + row_num).val('');
			$("#shade_" + row_num).val('');
			$("#plies_" + row_num).val('');
		...
		$('#txt_batch_no').live('keydown', function(e) {
			if (e.keyCode === 13) {
				...
				var response_data = return_global_ajax_value(batch_no + "**" + order_id + "**" + added_barcode_no, 'check_batch_no', '', 'cut_and_lay_ratio_wise_entry_controller_urmi_for_qc');
		...
		function openmypage_barcode() {
			...
			emailwindow = dhtmlmodal.open('EmailBox', 'iframe', 'cut_and_lay_ratio_wise_entry_controller_urmi_for_qc.php?order_no=' + <? echo $order_no; ?> + '&color=' + <? echo $color; ?> + '&action=barcode_popup', 'Barcode Popup', 'width=480px,height=300px,center=1,resize=1,scrolling=0', '../../')
			emailwindow.onclose = function() {
				var theform = this.contentDoc.forms[0] //("search_order_frm"); //Access the form inside the modal window
				var barcode_nos = this.contentDoc.getElementById("hidden_barcode_nos").value; //Barcode Nos
		...
		function fnc_close() {
			...
			$("#tbl_list_search tbody").find('tr').each(function() {
				...
				var batchId = $('#cbobatchid_' + i).val();
				var shade = $(this).find('input[name="shade[]"]').val();
				// alert(batchId);
				if (batch_selection_in_plies_popup == '1') {
					if (batchId == 0) {
						flag = 0;
					}
				}
				...
				if (plies * 1 > 0) {
					...
					if (save_string == "") {
						save_string = barcodeNo + "=" + rollNo + "=" + rollId + "=" + rollWgt + "=" + plies + "=" + batchNo + "=" + batchId + "=" + shade;
					} else {
						save_string += "**" + barcodeNo + "=" + rollNo + "=" + rollId + "=" + rollWgt + "=" + plies + "=" + batchNo + "=" + batchId + "=" + shade;
					}
				}
					...
					<tbody>
						<tr id="tr_1" class="general">
							...
							<td>
								<?
								$batch_arr = array();
								if ($batch_selection_in_plies_popup == 1) // when yes
								{

									$all_po_id_arr = return_library_array("SELECT b.id,b.id as odr_id from wo_po_break_down b where b.status_active=1 and b.job_no_mst='$job_no'", 'id', 'odr_id');
									$all_po_ids = implode(",", $all_po_id_arr);

									$order_no = str_replace("'", "", $order_no);

									// echo "select c.quantity from order_wise_pro_details c where c.po_breakdown_id in($all_po_ids) and c.COLOR_ID=$color  and c.entry_form=18 and c.trans_type=2 and c.is_deleted=0 and c.status_active=1";
									$is_ff_issue = return_field_value("quantity", "order_wise_pro_details", "po_breakdown_id in($all_po_ids) and COLOR_ID=$color  and entry_form=18 and trans_type=2 and is_deleted=0 and status_active=1"); // and color_id=$color
									$ff_issue_color = '';

									if ($is_ff_issue) {
										$fso_id_arr = return_library_array("SELECT a.id, a.id as ids from fabric_sales_order_mst a,fabric_sales_order_dtls b where a.id=b.mst_id and a.status_active=1 and b.COLOR_ID=$color and a.po_job_no='$job_no'", 'id', 'ids');
										$fso_ids = implode(",", $fso_id_arr);
										if ($fso_ids != "") {
											$order_no = $order_no . "," . $fso_ids;
										}
										// ======================================================
										// get finish fabric issue batch
										$sql = sql_select("SELECT c.id, c.batch_no from inv_issue_master a, inv_finish_fabric_issue_dtls b, pro_batch_create_mst c, wo_booking_dtls d	where a.id=b.mst_id and a.entry_form=18 and b.batch_id=c.id and c.booking_no=d.booking_no and b.status_active=1 and c.status_active=1 and d.job_no='" . $job_no . "' and c.color_id=$color group by c.id, c.batch_no"); // and a.color_id=$color
										// echo "SELECT c.id, c.batch_no from inv_issue_master a, inv_finish_fabric_issue_dtls b, pro_batch_create_mst c, wo_booking_dtls d	where a.id=b.mst_id and a.entry_form=18 and b.batch_id=c.id and c.booking_no=d.booking_no and b.status_active=1 and c.status_active=1 and d.job_no='".$job_no ."' and c.color_id=$color group by c.id, c.batch_no";
										$issue_batch_ids = "";
										foreach ($sql as $val) {
											//$batch_arr[$val['ID']] = $val['BATCH_NO'];
											$issue_batch_ids .= $val['ID'] . ", ";
										}
										$issue_batch_ids = rtrim($issue_batch_ids, ", ");
										$sql = sql_select("SELECT a.id, a.batch_no from pro_batch_create_mst a, pro_batch_create_dtls b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and b.po_id in($order_no) and a.color_id=$color and a.id in($issue_batch_ids) group by a.id, a.batch_no order by a.batch_no"); // and a.color_id=$color
										foreach ($sql as $val) {
											$batch_arr[$val['ID']] = $val['BATCH_NO'];
										}

										// get transfer batch with same color
										$sql = sql_select("SELECT a.id, a.batch_no from pro_batch_create_mst a, pro_batch_create_dtls b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and b.po_id in($order_no) and a.color_id=$color and a.entry_form=14 group by a.id, a.batch_no order by a.batch_no"); // and a.color_id=$color
										// echo "SELECT a.id, a.batch_no from pro_batch_create_mst a, pro_batch_create_dtls b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and b.po_id in($order_no) and a.color_id!=$color and a.entry_form=14 group by a.id, a.batch_no order by a.batch_no";
										foreach ($sql as $val) {
											$batch_arr[$val['ID']] = $val['BATCH_NO'];
										}
									}
									...
									// ============get contrast color ================
									$const_col_id_arr = return_library_array("SELECT contrast_color_id, contrast_color_id from wo_pre_cos_fab_co_color_dtls where status_active=1 and gmts_color_id=$color and job_no='$job_no'", 'contrast_color_id', 'contrast_color_id');
									$const_col_ids = implode(",", $const_col_id_arr);
									$ff_issue_color = ($const_col_ids != "") ? $color . "," . $const_col_ids : $color;

									$is_ff_issue = return_library_array("SELECT b.batch_id, b.batch_id as ids from inv_issue_master a, inv_finish_fabric_issue_dtls b,order_wise_pro_details c where a.id=b.mst_id and b.id=c.dtls_id and c.entry_form=18 and c.trans_type=2 and c.status_active=1 and c.po_breakdown_id in($all_po_ids) and c.COLOR_ID in($const_col_ids)", 'batch_id', 'ids');

									// echo "SELECT b.batch_id, b.batch_id as ids from inv_issue_master a, inv_finish_fabric_issue_dtls b,order_wise_pro_details c where a.id=b.mst_id and b.id=c.dtls_id and c.entry_form=18 and c.trans_type=2 and c.status_active=1  and c.po_breakdown_id in($all_po_ids) and c.COLOR_ID in($const_col_ids)";
									if ($is_ff_issue) {
										$batch_id = implode(",", $is_ff_issue);
										$sql = sql_select("SELECT a.id, a.batch_no from pro_batch_create_mst a, pro_batch_create_dtls b
									where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and a.color_id in($const_col_ids) and a.id in($batch_id) group by a.id, a.batch_no"); // and a.color_id=$color
										/* echo "SELECT a.id, a.batch_no from pro_batch_create_mst a, pro_batch_create_dtls b
									where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and a.color_id in($const_col_ids) and a.id in($batch_id) group by a.id, a.batch_no"; */
										foreach ($sql as $val) {
											$batch_arr[$val['ID']] = $val['BATCH_NO'];
										}

										// echo create_drop_down( "cbobatchid_1", 100, $batch_arr,"", 1, "-- Select --", $selected, "","0");
									}

									// ================== get all color of job ==================
									$all_job_clr_arr = return_library_array("SELECT b.color_number_id,b.color_number_id as clr_id from WO_PO_COLOR_SIZE_BREAKDOWN b where b.status_active=1 and b.is_deleted=0 and b.job_no_mst='$job_no'", 'color_number_id', 'clr_id');
									$all_job_clr_ids = implode(",", $all_job_clr_arr);

									$ff_issue_color = ($all_job_clr_ids != "") ? $ff_issue_color . "," . $all_job_clr_ids : $ff_issue_color;

									// ============get contrast color of others gmts color================
									$const_col_id_arr = return_library_array("SELECT contrast_color_id, contrast_color_id from wo_pre_cos_fab_co_color_dtls where status_active=1 and gmts_color_id!=$color and job_no='$job_no'", 'contrast_color_id', 'contrast_color_id');
									// echo "SELECT contrast_color_id, contrast_color_id from wo_pre_cos_fab_co_color_dtls where status_active=1 and gmts_color_id!=$color and job_no='$job_no'";
									$const_col_ids = implode(",", $const_col_id_arr);
									$ff_issue_color = ($const_col_ids != "") ? $ff_issue_color . "," . $const_col_ids : $ff_issue_color;


									$is_ff_issue = return_library_array("SELECT b.batch_id, b.batch_id as ids from inv_issue_master a, inv_finish_fabric_issue_dtls b,order_wise_pro_details c where a.id=b.mst_id and b.id=c.dtls_id and c.entry_form=18 and c.trans_type=2 and c.status_active=1 and c.po_breakdown_id in($all_po_ids) and c.COLOR_ID not in($ff_issue_color)", 'batch_id', 'ids');
									// echo "SELECT b.batch_id, b.batch_id as ids from inv_issue_master a, inv_finish_fabric_issue_dtls b,order_wise_pro_details c where a.id=b.mst_id and b.id=c.dtls_id and c.entry_form=18 and c.trans_type=2 and c.status_active=1 and c.po_breakdown_id in($all_po_ids) and c.COLOR_ID not in($ff_issue_color)";
									if ($is_ff_issue) {
										$batch_id = implode(",", $is_ff_issue);
										$sql = sql_select("SELECT a.id, a.batch_no from pro_batch_create_mst a, pro_batch_create_dtls b
									where a.id=b.mst_id and a.status_active=1 and b.status_active=1  and a.id in($batch_id) group by a.id, a.batch_no"); // and a.color_id=$color
										/* echo "SELECT a.id, a.batch_no from pro_batch_create_mst a, pro_batch_create_dtls b
									where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and a.color_id in($const_col_ids) and a.id in($batch_id) group by a.id, a.batch_no"; */
										foreach ($sql as $val) {
											$batch_arr[$val['ID']] = $val['BATCH_NO'];
									...
									// ============================== transfer batch with same color ==============================
									$trans_batch_id_arr = return_library_array("SELECT b.TO_BATCH_ID,b.TO_BATCH_ID as b_id from INV_ITEM_TRANSFER_MST a, INV_ITEM_TRANSFER_DTLS b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and b.color_id=$color and b.TO_ORDER_ID in($all_po_ids) and b.ITEM_CATEGORY=2", 'TO_BATCH_ID', 'b_id');
									if (count($trans_batch_id_arr) > 0) {
										$batch_ids = implode(",", $trans_batch_id_arr);

										$is_ff_issue = return_library_array("SELECT b.batch_id, b.batch_id as ids from inv_issue_master a, inv_finish_fabric_issue_dtls b,order_wise_pro_details c where a.id=b.mst_id and b.id=c.dtls_id and c.entry_form=18 and c.trans_type=2 and c.status_active=1 and c.po_breakdown_id in($all_po_ids) and c.COLOR_ID in($color) and b.batch_id in($batch_ids)", 'batch_id', 'ids');

										if ($is_ff_issue) {
											$sql = sql_select("SELECT a.id, a.batch_no from pro_batch_create_mst a, pro_batch_create_dtls b
										where a.id=b.mst_id and a.status_active=1 and b.status_active=1  and a.id in($batch_ids) group by a.id, a.batch_no");
											foreach ($sql as $val) {
												$batch_arr[$val['ID']] = $val['BATCH_NO'];
											}
										}
									...
									// ============================== transfer batch without same color ==============================
									$trans_batch_id_arr = return_library_array("SELECT b.TO_BATCH_ID,b.TO_BATCH_ID as b_id from INV_ITEM_TRANSFER_MST a, INV_ITEM_TRANSFER_DTLS b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and b.color_id!=$color and b.TO_ORDER_ID in($all_po_ids) and b.ITEM_CATEGORY=2", 'TO_BATCH_ID', 'b_id');
									if (count($trans_batch_id_arr) > 0) {
										$batch_ids = implode(",", $trans_batch_id_arr);
									...
									echo create_drop_down("cbobatchid_1", 130, $batch_arr, "", 1, "-- Select --", $selected, "", "0");

								?>
									<input type="hidden" id="batchNo_1" name="batchNo[]" class="text_boxes" value="" style="width:130px" />
								<?
								} else {
								?>
									<input type="text" id="batchNo_1" name="batchNo[]" class="text_boxes" value="" style="width:130px" />
									<input type="hidden" id="cbobatchid_1" name="cbobatchid_1" class="text_boxes" value="" style="width:120px" />
								<?
								}
								?>
							</td>
							<td>
								<input type="text" id="shade_1" name="shade[]" class="text_boxes" value="" maxlength="15" style="width:120px" onKeyDown="return /[A-za-z0-9]/i.test(event.key)" oninput="this.value = this.value.toUpperCase()" />
							</td>
							<td>
...
if ($action == "roll_popup_bk") // 5-4-2023
{
	echo load_html_head_contents("Plies Info Roll Wise", "../../../", 1, 1, '', '1', '');
	extract($_REQUEST);
	//$order_no=str_replace("'","",$order_no);
...
?>
	<script>
		var roll_maintained = <? echo $roll_maintained; ?>;
		// var batch_selection_in_plies_popup=<? echo $batch_selection_in_plies_popup; ?>;
		var rollData = '<? echo $rollData; ?>';
		var scanned_barcode = new Array();
		var roll_details_array = new Array();
		var barcode_array = new Array();
		<?
		$scanned_barcode_array = array();
		$scanned_barcode_data = sql_select("select barcode_no from pro_roll_details where entry_form=93 and status_active=1 and is_deleted=0 and po_breakdown_id in(" . str_replace("'", "", $order_no) . ")");
		foreach ($scanned_barcode_data as $row) {
			$scanned_barcode_array[] = $row[csf('barcode_no')];
		}
		$jsscanned_barcode_array = json_encode($scanned_barcode_array);
		...
		<div align="center" style="width:100%; overflow-y:hidden;">
			<fieldset style="width:590px">
					...
					<tbody>
						<tr id="tr_1" class="general">
							...
							<td>
								...
								{
									$order_no = str_replace("'", "", $order_no);
									$trans_po_id_arr = return_library_array("SELECT b.FROM_ORDER_ID,b.FROM_ORDER_ID as odr_id from INV_ITEM_TRANSFER_MST a, INV_ITEM_TRANSFER_DTLS b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and b.color_id=$color and b.TO_ORDER_ID in($order_no) and b.ITEM_CATEGORY=2", 'FROM_ORDER_ID', 'odr_id');

									$is_ff_issue = return_field_value("quantity", "order_wise_pro_details", "po_breakdown_id in($order_no) and color_id=$color and entry_form=18 and trans_type=2 and is_deleted=0 and status_active=1");
									if ($is_ff_issue) {
										$fso_id_arr = return_library_array("SELECT a.id, a.id as ids from fabric_sales_order_mst a,fabric_sales_order_dtls b where a.id=b.mst_id and a.status_active=1 and b.COLOR_ID=$color and a.po_job_no='$job_no'", 'id', 'ids');
										$fso_ids = implode(",", $fso_id_arr);
										if ($fso_ids != "") {
										...
										$batch_arr = return_library_array("SELECT a.id, a.batch_no from pro_batch_create_mst a, pro_batch_create_dtls b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and a.color_id=$color and b.po_id in($order_no)  group by a.id, a.batch_no order by a.batch_no", 'id', 'batch_no');

										echo create_drop_down("cbobatchid_1", 100, $batch_arr, "", 1, "-- Select --", $selected, "", "0");
									} else {
										echo create_drop_down("cbobatchid_1", 100, $blank_array, "", 1, "-- Select --", $selected, "", "0");
									}
								?>
									<input type="hidden" id="batchNo_1" name="batchNo[]" class="text_boxes" value="" style="width:100px" />
								<?
								} else {
								?>
									<input type="text" id="batchNo_1" name="batchNo[]" class="text_boxes" value="" style="width:100px" />
									<input type="hidden" id="cbobatchid_1" name="cbobatchid_1" class="text_boxes" value="" style="width:100px" />
								<?
								}
								?>
							</td>
							<td>
								<input type="text" id="shade_1" name="shade[]" class="text_boxes" value="" maxlength="1" style="width:50px" onkeydown="return /[a-z]/i.test(event.key)" />
							</td>
							<td>
							...
							?>
							<td>
								<input type="text" id="plies_1" name="plies[]" class="text_boxes_numeric" value="" style="width:50px" />
							</td>
							<td width="70">
								<? if ($roll_maintained != 1) {
								?>
				...
				<div align="center" style="margin-top:10px">
					...
					<input type="hidden" id="hide_data" />
					<input type="hidden" name="txt_tot_row" id="txt_tot_row" class="text_boxes" value="1">
					<input type="hidden" id="batch_selection_in_plies_popup" value="<?= $batch_selection_in_plies_popup; ?>" class="text_boxes" value="1">
				</div>
			</fieldset>
		</div>
	</body>
	...
	<script>
		...
		function fnc_cut_lay_size_info(operation) {
			...
			for (var i = 1; i <= roll_row_num; i++) {
				...
				if (roll_data == "") {
					roll_data = barcode_no + "__" + roll_no + "__" + roll_id + "__" + roll_wgt + "__" + plies;
				} else {
					roll_data += "|" + barcode_no + "__" + roll_no + "__" + roll_id + "__" + roll_wgt + "__" + plies;
				}
		...
		function fnc_bundle_report_one_urmi_v2(type) {
			...
			if (type == 1) {
				...
				action = "print_barcode_five_per_page";
				body_part_action = "print_report_bundle_barcode_eight_v2";
				bundle_source = '***1';
			} else if (type == 5) {
				action = "print_barcode_24_per_page";
			} else if (type == 6) {
				action = "print_barcode_seven_per_page";
			} else if (type == 8) {
				action = "print_barcode_68_8";
			} else if (type == 9) {
				action = "bundel_sticker_14";
			} else if (type == 10) {
				action = "print_barcode_68_10";
			} 
			
			else {
				action = "print_barcode_six_per_page";
			}
			var page_link = 'cut_and_lay_ratio_wise_entry_controller_urmi.php?data=' + data + bundle_source + '&action=' + body_part_action;
			emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=270px,height=250px,center=1,resize=0,scrolling=0', '../../');
			emailwindow.onclose = function() {
				var theform = this.contentDoc.forms[0];
		...
		function fnc_bundle_sticker_v2_14() {
			var data = "";
			var error = 1;
			let source="***1";
			...
			var page_link = 'cut_and_lay_ratio_wise_entry_controller_urmi.php?data=' + data + source + '&action=print_report_bundle_barcode_eight';
			emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=270px,height=250px,center=1,resize=0,scrolling=0', '../../');
			emailwindow.onclose = function() {
				var theform = this.contentDoc.forms[0];
				var prodID = this.contentDoc.getElementById("txt_selected_id").value;
				data = data + '***' + prodID;
				var url = return_ajax_request_value(data, 'print_bundle_sticker_v2_14', "cut_and_lay_ratio_wise_entry_controller_urmi");
				window.open(url, "##");
			}
		}
		...
		function fnc_bundle_report_qr_code(type) {
			...
			emailwindow.onclose = function() {
				...
				if (type == 1) {
					...
					var action = "print_qrcode_operation3";

				} else if (type == 8) {
					var action = "print_qrcode_operation_69_38";
				} else if (type == 9) {
					var action = "print_qr_operation_v2";
		...
		function fnc_print_bundle(work_comp, work_location) {
			...
			var data2 = '<?php echo $cbo_company_id; ?>';
			...
			emailwindow.onclose = function() {
				...
				print_report_post(<? echo $cbo_company_id; ?> + '*' + <? echo $mst_id; ?> + '*' + <? echo $details_id; ?> + '*' + report_title + '*' + country + '*' + bundle_use_for + '*' + data, "cut_lay_bundle_print", "cut_and_lay_ratio_wise_entry_controller_urmi");
			}
		...
		function bundle_calclution(rowNo) {
			...
			var pattern = $("#patternNo_" + rowNo).val();
			var rollId = $("#rollId_" + rowNo).val();
			if (rmg_no_creation == '6') {
				var max_rmg = 999;
			}
			if (rmg_no_creation == '7') {
				var max_rmg = 9999;
			}
		...
		<div align="center" style="width:100%; overflow-y:hidden; position:absolute; top:5px;">
			...
			<form name="searchorderfrm_1" id="searchorderfrm_1" autocomplete="off">
				...
				<fieldset style="width:905px;">
					...
					$poArr = array();
					$sql_query = sql_select("SELECT po_break_down_id, country_type, country_id, size_number_id, plan_cut_qnty, country_ship_date, size_order from wo_po_color_size_breakdown where item_number_id=$cbo_gmt_id and po_break_down_id in($order_id) and color_number_id=$cbo_color_id $country_con and status_active=1 and is_deleted=0 order by country_ship_date, size_order,country_type");
					...
					$sql = sql_select("SELECT a.order_id,a.country_type, a.country_id,a.size_qty as SIZE_QTY, a.SIZE_ID from ppl_cut_lay_bundle a, ppl_cut_lay_dtls b where b.id=a.dtls_id and a.order_id in($order_id) and b.gmt_item_id=$cbo_gmt_id and b.color_id=$cbo_color_id $country_con and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");
					foreach ($sql as $val) {
									...
									<td align="center" colspan="7">
										...
										foreach ($report_id as $res) {
											...
											<? } elseif ($res == 454) { ?>
												...
												<input type="button" id="btn_bundle_qr_code2" name="btn_bundle_qr_code2" value=" Sticker 6/page" class="formbutton" onClick="fnc_bundle_report_one_urmi_v2(4)" />
											<? } elseif ($res == 443) { ?>
											...
											?>
												...
												<input type="button" id="btn_bundle_qr_code2" name="btn_bundle_qr_code2" value="Barcode 69X38" class="formbutton" onClick="fnc_bundle_report_one_urmi_v2(8)" />
											<?
											} elseif ($res == 784) {
											?>

												<input type="button" id="btn_bundle_qr_code24" name="btn_bundle_qr_code24" value="Bundle Sticker 24/Page" class="formbutton" onClick="fnc_bundle_report_one_urmi_v2(5)" />
											<?
											} elseif ($res == 785) {
											?>

												<input type="button" id="btn_bundle_qr_code_7" name="btn_bundle_qr_code_7" value="Bundle Sticker7" class="formbutton" onClick="fnc_bundle_report_one_urmi_v2(6)" />
											<?

											} elseif ($res == 869) {
											?>

												<input type="button" id="bundel_sticker14" name="bundel_sticker14" value="Bundle Sticker 14" class="formbutton" onClick="fnc_bundle_report_one_urmi_v2(9)" />
											<?
											}
											elseif ($res == 337) {
												?>

													<input type="button" id="bundel_sticker_alliance" name="bundel_sticker_alliance" value="Bundle Sticker Alliance" class="formbutton" onClick="fnc_bundle_report_one_urmi_v2(10)" />
												<?
												} elseif ($res == 838) {
											?>
												<input type="button" id="btn_bundle_qr_code_69_38" name="btn_bundle_qr_code_69_38" value="QR 69X38" class="formbutton" onClick="fnc_bundle_report_qr_code(8)" />
											<?
											} elseif ($res == 842) {
											?>
												<input type="button" id="btn_bundle_qr_code" name="btn_bundle_qr_code" value="Bundle Sticker 3" class="formbutton" onClick="fnc_bundle_report_qr_code(7)" />
											<?
											} elseif ($res == 380) {
											?>
											...
											?>
												<input type="button" id="btn_bundle_sticker_v2" name="btn_bundle_sticker_v2_14" value="Sticker V2 Bundle 14" class="formbutton" onClick="fnc_bundle_sticker_v2_14()" />
										<?
											}
											 elseif ($res == 856){
											?>
												<input type="button" id="btn_bundle_sticker__qr_v2" name="btn_bundle_sticker__qr_v2" value="Bundle Sticker-QR V2" class="formbutton" onClick="fnc_bundle_report_qr_code(9)" />
											<?
											}
										}
...
?>
	...
	$body_part_id_con = "";
	if ($data[7] == 1) //Bundle Copy come from Style Wise Body part Entry page
	{
		$style_wise_body_part_sql = "SELECT id,body_part_ids from style_wise_body_part_mst where company_name=$company_id and job_no= '$data[1]' and item_id = '$data[4]' and status_active=1 and is_deleted=0";
	 ...
	 $sql_bundle_copy = "SELECT id,bundle_use_for from ppl_bundle_title where company_id=$company_id $body_part_id_con order by bundle_use_for"; //$body_part_id_con
...
<?
	...
	$sql_bundle_copy = "SELECT id,bundle_use_for from ppl_bundle_title where company_id=$data  order by bundle_use_for";
	//echo $sql_bundle_copy;
	echo  create_list_view("tbl_list_search", "Bundle Use For", "240", "240", "180", 0, $sql_bundle_copy, "js_set_value", "id,bundle_use_for", "", 1, "0", $arr, "bundle_use_for", "", "setFilterGrid('tbl_list_search',-1)", '0', "", 1);
	echo "<input type='hidden' id='txt_selected_id' />";
	echo "<input type='hidden' id='txt_selected' />";
...
if ($action == "print_qrcode_operation3_bk") // 21-03-2023
{
	//echo "1000".$data;die;
	$data = explode("***", $data);
	$detls_id = $data[3];
	...
	$color_sizeID_arr = sql_select("SELECT
										...
										a.country_id,
										a.roll_no,
										a.bundle_num_prefix_no as bndl_pri,
										b.bundle_sequence,
										b.color_id,
										c.gmt_item_id ,
										c.roll_data,
	...
	$order_arr = return_library_array("select id, po_number from wo_po_break_down where status_active=1 $order_id_cond", 'id', 'po_number');
	$int_ref_arr = return_library_array("select id, grouping from wo_po_break_down where status_active=1 $order_id_cond", 'id', 'grouping');
	$bundle_array = array();
	$sql_name = sql_select("SELECT
							b.buyer_name,
							b.style_ref_no,
	...
	foreach ($sql_cut_name as $cut_value) {
		...
		$shift_id 			= $cut_value[csf('shift_name')];
		$cut_date 			= change_date_format($cut_value[csf('entry_date')]);
		$cut_prifix 		= $cut_value[csf('cut_num_prefix_no')];
		$company_id 		= $cut_value[csf('company_id')];
		$batch_no 			= $cut_value[csf('batch_id')];
	...
	$emb_arr = return_library_array("SELECT JOB_NO, EMB_NAME from wo_pre_cost_embe_cost_dtls where status_active=1 and job_no='$job_no'", 'job_no', 'emb_name');
	$emb_name = "";
	foreach ($emb_arr as $val) {
		$emb_name .= ($emb_name == "") ? $emblishment_name_array[$val] : "," . $emblishment_name_array[$val];
	}
	...
	$mpdf = new mPDF(
		'',    // mode - default ''
		array(75, 50),		// array(69,38),    // format - A4, for example, default ''
	...
	if (count($sql_bundle_copy) != 0) {
		foreach ($color_sizeID_arr as $val) {
			foreach ($sql_bundle_copy as $inf) {
				...
				$mpdf->AddPage(
					'',    // mode - default ''
					array(75, 50),		// array(65,210),    // format - A4, for example, default ''
					6,     // font size - default 0
					'',    // default font family
					0,    // margin_left
					0,    // margin right
				...
				$html .= '<table border="0"  width="100%" cellspacing="1" cellpadding="0" style="margin-top:3px;" rules="all">
						<tr style="border:0px">
							<td width="25%" rowspan="4" style="border:none;">
								<div id="div_' . $i . '"><img src="' . $PNG_WEB_DIR . basename($filename) . '" height="70" width=""></div>
							</td>
							<td style="border:1px solid black;" width="15%">Buyer</td>
							<td style="border:1px solid black;" width="35%">' . $buyer_short_name . '</td>
							<td width="25%" rowspan="3" colspan="3" align="center"><img src="../../../images/mfg.jpg" height="55" width=""></td>
						</tr>
						<tr>
							<td bgcolor="black" style="color:white;">Int Ref</td>
							<td bgcolor="black" style="color:white;">' . substr($int_ref_arr[$val['ORDER_ID']], 0, 22) . '</td>
						</tr>
						<tr>
							<td style="border:1px solid black;">Style</td>
							<td style="border:1px solid black;">' . substr($style_name, 0, 22) . '</td>
						</tr>
						<tr>
							<td style="border:1px solid black;">Color</td>
							<td style="border:1px solid black;">' . substr($color_library[$data[5]], 0, 22) . '</td>
							<td colspan="3" style="border:0px">
								<table border="0" width="100%" cellspacing="1" cellpadding="0" rules="none">
									<tr>
										<td style="border:1px solid black;height:8px;font-size:5px;">C.Unit</td>
										<td style="border:1px solid black;height:8px;font-size:5px;">Tbl</td>
										<td style="border:1px solid black;height:8px;font-size:5px;">Sft</td>
									</tr>
									<tr>
										<td style="border:1px solid black;height:8px;font-size:5px;">' . substr($floor_arr[$floor_id], 0, 6) . '</td>
										<td align="center" style="border:1px solid black;height:8px;font-size:5px;">' . $table_no . '</td>
										<td align="center" style="border:1px solid black;height:8px;font-size:5px;">' . $shift_name[$shift_id] . '</td>
									</tr>
								</table>
							</td>
						</tr>
						<tr>
							<td colspan="6" style="border:0px">
								<table border="0" width="100%" cellspacing="1" cellpadding="0" rules="none">
									<tr>
										<td style="border:1px solid black;" width="14%">Item:</td>
										<td style="border:1px solid black;"  width="23%">' . $garments_item[$val['GMT_ITEM_ID']] . '</td>
										<td style="border:1px solid black;"  width="10%">Com:</td>
										<td style="border:1px solid black;"  width="30%"></td>
										<td style="border:1px solid black;"  width="10%">C/NO:</td>
										<td style="border:1px solid black;"  width="13%">' . $cut_prifix . '</td>
									</tr>
									<tr>
										<td style="border:1px solid black;">Emb:</td>
										<td style="border:1px solid black;">' . substr($emb_name, 0, 13) . '</td>
										<td style="border:1px solid black;">Parts:</td><td>' . $inf[csf("bundle_use_for")] . '</td>
										<td bgcolor="black" style="color:white;">Size:</td>
										<td bgcolor="black" style="color:white;">' . $size_arr[$val['SIZE_ID']] . '</td>
									</tr>
									<tr>
										<td align="center" colspan="2" bgcolor="black" style="color:white;">Remarks</td>
										<td style="border:1px solid black;">PO:</td>
										<td style="border:1px solid black;">' . substr($order_arr[$val['ORDER_ID']], 0, 14) . '</td>
										<td style="border:1px solid black;">B/NO:</td>
										<td style="border:1px solid black;">' . $val['BNDL_PRI'] . '</td>
									</tr>
									<tr>
										<td style="border:1px solid black;" colspan="2">' . substr($val['REMARKS'], 0, 21) . '</td>
										<td style="border:1px solid black;">SL:</td>
										<td style="border:1px solid black;">' . $val['NUMBER_START'] . ' to ' . $val['NUMBER_END'] . '</td>
										<td bgcolor="black" style="color:white;">Qty:</td>
										<td bgcolor="black" style="color:white;">' . $val['SIZE_QTY'] . '</td>
									</tr>
									<tr>
										<td style="border:1px solid black;">QC Sign:</td>
										<td style="border:1px solid black;"></td>
										<td style="border:1px solid black;">Batch</td>
										<td style="border:1px solid black;">' . $batch_no . '</td>
										<td style="border:1px solid black;">Shade</td>
										<td style="border:1px solid black;">' . $shade . '</td>
									</tr>
								</table>
							</td>
						...
						<tr>
							<td style="border:0px slid #000000;" colspan="6" align="center"><b>QR : ' . $val['BARCODE_NO'] . '</b></td>
						</tr>
					</table>';
		...
		foreach ($color_sizeID_arr as $val) {
			...
			$html .= '<table border="0"  width="100%" cellspacing="1" cellpadding="0" style="margin-top:3px;" rules="all">
					<tr style="border:0px">
						<td width="25%" rowspan="4" style="border:none;">
							<div id="div_' . $i . '"><img src="' . $PNG_WEB_DIR . basename($filename) . '" height="58" width=""></div>
						</td>
						<td style="border:1px solid black;" width="15%">Buyer</td>
						<td style="border:1px solid black;" width="35%">' . $buyer_short_name . '</td>
						<td width="25%" rowspan="3" colspan="3" align="center"><img src="../../../images/mfg.jpg" height="40" width=""></td>
					</tr>
					<tr>
						<td bgcolor="black" style="color:white;">Int Ref</td>
						<td bgcolor="black" style="color:white;">' . $int_ref_arr[$val['ORDER_ID']] . '</td>
					</tr>
					<tr>
						<td style="border:1px solid black;">Style</td>
						<td style="border:1px solid black;">' . $style_name . '</td>
					</tr>
					<tr>
						<td style="border:1px solid black;">Color</td>
						<td style="border:1px solid black;">' . substr($color_library[$data[5]], 0, 25) . '</td>
						<td colspan="3" style="border:0px">
							<table border="0" width="100%" cellspacing="1" cellpadding="0" rules="none">
								<tr>
									<td style="border:1px solid black;height:8px;font-size:5px;">C.Unit</td>
...
if ($action == "print_qrcode_operation3") // for mfg
{
	//echo "1000".$data;die;
	$data = explode("***", $data);
	$detls_id = $data[3];
	...
	$mpdf = new mPDF(
		'',    // mode - default ''
		array(75, 50),		// array(69,38),    // format - A4, for example, default ''//array(75,50)
		7,     // font size - default 0
		'chelvetica',    // default font family
		0,    // margin_left
		1,    // margin right
		0,     // margin top
		0,    // margin bottom
		...
		foreach ($color_sizeID_arr as $val) {
			foreach ($sql_bundle_copy as $inf) {
				...
				$html .= '<table border="0"  width="100%" cellspacing="1" cellpadding="0" style="margin-top:3px;" rules="all">
						...
						<tr>
							<td style="border:1px solid black;">Int Ref</td>
							<td style="border:1px solid black;">' . substr($int_ref_arr[$val['ORDER_ID']], 0, 22) . '</td>
						</tr>
						<tr>
							<td style="border:1px solid black;">Style</td>
							<td style="border:1px solid black;">' . substr($style_name, 0, 22) . '</td>
						...
						<tr>
							...
							<td colspan="3" style="border:0px">
								<table border="0" width="100%" cellspacing="1" cellpadding="0" rules="none">
									<tr>
										<td style="border:1px solid black;height:8px;font-size:7px;">C.Unit</td>
										<td style="border:1px solid black;height:8px;font-size:7px;">Tbl</td>
										<td style="border:1px solid black;height:8px;font-size:7px;">Sft</td>
									</tr>
									<tr>
										<td style="border:1px solid black;height:8px;font-size:7px;">' . substr($floor_arr[$floor_id], 0, 6) . '</td>
										<td align="center" style="border:1px solid black;height:8px;font-size:7px;">' . $table_no . '</td>
										<td align="center" style="border:1px solid black;height:8px;font-size:7px;">' . $shift_name[$shift_id] . '</td>
									</tr>
								</table>
							</td>
						</tr>
...
if ($action == "print_qrcode_operation_2") // for Charka
{
	...
	a.number_start,a.number_end,a.size_qty,a.country_id,a.roll_no,b.color_id
	...
	// echo "<pre>"print_r($total_size_qty_arr);die;
	$bundle_array = array();
	//return_library_array( "select id,short_name from lib_buyer where id=$buyer_name ", "id", "short_name");
...
if ($action == "print_qrcode_operation_3") //
{
	...
	$country_arr = return_library_array("SELECT id, country_name from lib_country", 'id', 'country_name');
	$brand_arr = return_library_array("SELECT id, brand_name from lib_buyer_brand", 'id', 'brand_name');
	$batch_arr = return_library_array("SELECT id, batch_no from PRO_ROLL_DETAILS where mst_id=$data[2] and entry_form=99", 'id', 'batch_no');


	$color_sizeID_arr = sql_select("SELECT a.id,a.size_id,a.bundle_no,a.barcode_no,a.order_id,a.number_start,a.number_end,a.size_qty,a.country_id,a.pattern_no,a.roll_no,b.color_id,a.roll_id
	from ppl_cut_lay_bundle a, ppl_cut_lay_dtls  b
	where a.dtls_id=b.id and a.status_active=1 and b.status_active=1 and a.id in ($data[0])
	order by b.color_id,a.id");
	...
	$bundle_array = array();
	//return_library_array( "select id,short_name from lib_buyer where id=$buyer_name ", "id", "short_name");
	$sql_cut_name = sql_select("SELECT a.entry_date, a.table_no, a.cut_num_prefix_no,a.batch_id,a.company_id,a.cutting_no,b.order_cut_no,b.gmt_item_id,b.color_id,a.floor_id,b.roll_data
	from ppl_cut_lay_mst a, ppl_cut_lay_dtls b
	where a.id=b.mst_id and a.id=$data[2] and a.status_active=1 and b.status_active=1");
			...
			foreach ($sql_bundle_copy as $inf) {
				...
				$html .= '<table width="100%" cellpadding="0" cellspacing="0" width="" class="" style="font-size:10px; font-weight:bold;margin:0px" rules="all" id="" align="center">
					...
					<table width="100%" cellpadding="0" cellspacing="0" width="" class="" style="font-size:10px; font-weight:bold;margin:0px" rules="all" id="" border="1" align="center">
			            ...
			            <tr>
			            	<td>Int. Ref:' . $int_ref . '</td>
			            	<td>BN:' . substr($batch_arr[$val[csf("roll_id")]], 0, 13) . '</td>

			            </tr>

			            <tr>
			            	<td>Gmts. No:' . $val[csf("number_start")] . '-' . $val[csf("number_end")] . ', Size:' . $size_arr[$val[csf("size_id")]] . '(' . $val[csf('pattern_no')] . ')</td>
			            	<td>Gmts. Qnty:' . $val[csf("size_qty")] . '</td>
			            </tr>
...
if ($action == "print_qrcode_operation_69_38") // QR 69X38 //kamrul
{
	//echo "1000".$data;die;
	$data = explode("***", $data);
	$detls_id = $data[3];
	...
	$color_sizeID_arr = sql_select("SELECT a.id,a.size_id,a.bundle_no,a.barcode_no,a.order_id,a.number_start,a.number_end,a.size_qty,a.country_id,a.pattern_no,a.roll_no,b.color_id,a.roll_id
	from ppl_cut_lay_bundle a, ppl_cut_lay_dtls b
	where a.dtls_id=b.id and a.status_active=1 and b.status_active=1 and a.size_qty > 0 and a.id in ($data[0])
	order by b.color_id,a.id");
		...
		foreach ($sql_bundle_copy as $inf)
		{
			foreach ($color_sizeID_arr as $val) {
				...
				$html .= '
					...
					<table cellpadding="0" cellspacing="0" width="" class="" style="font-size:9px; font-weight:bold;margin:0px;width:260px;" rules="all" id="" border="1" align="center">

						<tr>
							<td style="width:130px">B:' . ucwords(strtolower($buyer_short_name)) . '</td>
							<td style="width:130px">Date:' . $cut_date . '</td>

			            </tr>
			            <tr>
							<td style="font-size:8px;">Style:' . substr($style_name, 0, 20) . ' </td>
							<td style="font-size:8px;">C.COM:' . substr($working_company, 0, 20) . '</td>
			            </tr>
			            <tr>

							<td>PO:' . substr($po_number, 0, 18) . '</td>
							<td>Batch No:' . substr($batch_no, 0, 13) . '</td>
			            </tr>

			            <tr>
							<td>Size:'. substr($size_arr[$val[csf("size_id")]] ,0,13). '(' . $val[csf('pattern_no')] . ')</td>

							<td >Table No:' .($table_no) .   ','  .'R/N:'.($val[csf('roll_no')]).'</td>
			            </tr>

			            <tr>
			            	<td>Qnty:' . $val[csf("size_qty")] . '</td>
			            	<td>Cuttting No:' . $order_cut_no . '</td>
			            </tr>

			            <tr >
			            	<td style="border:1px solid #000;">ST.S/L.No:' . substr($val[csf("number_start")], 0, 20) . '-' . substr($val[csf("number_end")], 0, 20) . '</td>
                            <td rowspan="3">' . $val[csf("barcode_no")] . '<div><img src="' . $PNG_WEB_DIR . basename($filename) . '" height="40" width="80"></div> B/No.:' . substr($val[csf("bundle_no")], 7, 12) . '  </td>
			            </tr>


			            <tr >
							<td style="font-size: 8px !important;border:1px solid #000;">C.Qty:' . substr($total_cut_qty, 0, 10) . ',' . substr($inf["BUNDLE_USE_FOR"], 0, 18) . '</p></td>
			            </tr>

			            <tr >
						<td  style="font-size: 9px; border:1px solid #000;">Color:' . substr($color_library[$data[5]], 0, 15) ."\n". substr($color_library[$data[5]], 16, 30).'</td>
			            </tr>
				    </table>';
				$mpdf->WriteHTML($html);
				$html = '';
	...
	} else
	{
		foreach ($color_sizeID_arr as $val)
		{
			...
			$html .= '<table cellpadding="0" cellspacing="0" width="" class="" style="font-size:9px; font-weight:bold;margin:0px;width:260px;" rules="all" id="" border="1" align="center">
			...
			<tr>
				<td style="font-size:8px;">Style:' . substr($style_name, 0, 18) . ' </td>
				<td style="font-size:8px;">C.COM:' . substr($working_company, 0, 18) . '</td>
			</tr>
			<tr>
			...
			<tr >
				<td style="font-size: 8px !important;border:1px solid #000;">C.Qty:' . substr($total_cut_qty, 0, 10) . ',' . substr($inf["BUNDLE_USE_FOR"], 0, 15) . '</p></td>
			</tr>
...
if ($action == "print_qrcode_operation_twenty_four") {
	...
	$color_sizeID_arr = sql_select("SELECT a.id, a.size_id, a.bundle_no, a.barcode_no, a.order_id, a.number_start, a.number_end, a.size_qty, a.country_id, a.roll_no, b.color_id from ppl_cut_lay_bundle a, ppl_cut_lay_dtls b where a.dtls_id=b.id and a.status_active=1 and b.status_active=1   and a.id in ($data[0]) order by b.color_id,a.id");
	foreach ($color_sizeID_arr as $val_qty) {
		$total_cut_qty += $val_qty[csf('size_qty')];
		$total_size_qty_arr[$val_qty[csf('size_id')]] += $val_qty[csf('size_qty')];
	}
...
if ($action == "print_barcode_one_urmi") {
	...
	from ppl_cut_lay_bundle a, ppl_cut_lay_size_dtls b where a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $dynamic_cond
	...
	foreach ($color_sizeID_arr as $row) {
		$roll_id_arr[$row[csf("roll_id")]] = $row[csf("roll_id")];
	}
	$i = 2;
	$j = 0;
	$k = 0;
	...
	unset($sql_name);
	$roll_id_cond = where_con_using_array($roll_id_arr, 0, "roll_id");
	$roll_sql = sql_select("select roll_id, batch_no from pro_roll_details where entry_form=99 and status_active=1 $roll_id_cond");
	$roll_data_arr = array();
	foreach ($roll_sql as $row) {
		$roll_data_arr[$row[csf("roll_id")]] = $row[csf("batch_no")];
	}
...
if ($action == "print_qr_operation") {
	...
	$color_sizeID_arr = sql_select("SELECT a.id,a.size_id,a.bundle_no,a.barcode_no,a.order_id,a.number_start,a.number_end,a.size_qty,a.country_id,a.roll_no,b.color_id,a.roll_id
	from ppl_cut_lay_bundle a, ppl_cut_lay_dtls  b
	where a.dtls_id=b.id and a.status_active=1 and b.status_active=1 and a.id in ($data[0])
	order by b.color_id,a.id");
	// echo $color_sizeID_arr);die;"<pre>";print_r($color_sizeID_arr);die;
	foreach ($color_sizeID_arr as $val_qty) {
		$total_cut_qty += $val_qty[csf('size_qty')];
		$total_size_qty_arr[$val_qty[csf('size_id')]] += $val_qty[csf('size_qty')];
	}
	...
	$total_number_of_bundle = count($color_sizeID_arr);
	$sql_bundle_copy = sql_select("select id,bundle_use_for from ppl_bundle_title where company_id=$company_id and id in ($data[7])");
	// echo count($sql_bundle_copy); die;
	if (count($sql_bundle_copy) != 0) {
		foreach ($sql_bundle_copy as $inf) {
			foreach ($color_sizeID_arr as $val) {
				...
				$html .= '
					<table cellpadding="0" cellspacing="0" width="" class="" style="width:100%; font-weight:bold;margin:0px;font-size:9px;" rules="all" id="" border="1">
						...
						<tr>
			            	...
			            	<td width="" colspan="2">BNDL#' . substr($val[csf("bundle_no")], 0, 15) . '</td>
			            </tr>
						<tr>
						...
						<tr>
						<td>BATCH:' . substr($batch_arr[$val[csf("roll_id")]], 0, 13) . '</td>
			            </tr>
						<tr>
							<td>Date#' . $cut_date . '</td>
			            </tr>
...
if ($action == "print_qr_operation_v2") {
	//echo "1000".$data;die;
	$data = explode("***", $data);
	$detls_id = $data[3];
	$garments_item_name = $garments_item[$data[4]];
	...
	$sql_name = sql_select("SELECT b.job_no,b.buyer_name,b.style_ref_no,b.product_dept,a.po_number,a.id,a.grouping,b.season_buyer_wise
	from wo_po_details_master b,wo_po_break_down a
	where a.job_no_mst=b.job_no and a.job_no_mst='" . $data[1] . "'");
	// echo $sql_name;die;
	...
	foreach ($sql_name as $value) {
		...
		$po_number_arr[$value[csf('id')]] 		= $value[csf('po_number')];
		$job_number 		= $value[csf('job_no')];
		$ir_no 		= $value[csf('grouping')];
		$season = $value[csf('season_buyer_wise')];
	}

	// echo"<pre>";print_r($sql_name);die;
	$buyer_short_name = return_field_value("short_name", "lib_buyer", "id=$buyer_name ");
		...
		foreach ($sql_bundle_copy as $inf) {
			foreach ($color_sizeID_arr as $val) {
				...
				$html .= '
					<table cellpadding="0" cellspacing="0" width="" class="" style="width:100%; font-weight:bold;margin:0px;font-size:9px;" rules="all" id="" border="1">
						<tr>
			            	<td>BR#' . substr($buyer_short_name, 0, 9) . '</td>
			            	<td colspan="2">IR#:' . substr($ir_no, 0, 18) . '</td>
			            </tr>
						<tr>
			            	<td width="50%">STY#' . substr($style_name, 0, 12) . ' </td>
			            	<td width="50%" colspan="2">PO#' . substr($po_number, 0, 28) . ' </td>
						...
						<tr>
							<td>BN#' . substr($val[csf("bundle_no")], 0, 15) . '</td>
			            	<td width="" colspan="2">QTY#' . $val[csf("size_qty")] . '</td>
			            </tr>
						<tr>
							<td>STKR#' . substr($val[csf("number_start")] . '-' . $val[csf("number_end")], 0, 12) . '</td>

							<td width="" colspan="2">S#'. substr($season_library[$season], 0,20) . '</td>
			            </tr>
						<tr>
							<td>BP#' . substr($inf[csf("bundle_use_for")], 0, 16) . '</td>
			            	<td width="" >MC#' . substr($order_cut_no, 0, 6) . '</td>
			            	<td width="" >ROLL#' . $val[csf("roll_no")] . '</td>
			            </tr>
						<tr>
							<td>Size#' . $size_arr[$val[csf("size_id")]] . '</td>
						...
						<tr>
						<td>BT:' . substr($batch_arr[$val[csf("roll_id")]], 0, 13) . '</td>
			            </tr>
						<tr>
							<td>Date#' . $cut_date . '</td>
			            </tr>
						...
						<tr>
							<td align="left" colspan="3"> COLOR#' . substr($color_library[$val[csf("color_id")]], 0, 30) . ' </td>
			            </tr>
				    </table>';
				$mpdf->WriteHTML($html);
				$html = '';
...
if ($action == "print_barcode_68") {
	...
	// 	$roll_data_arr[$row[csf("roll_id")]]=$row[csf("batch_no")];
	// }
	$roll_sql = ("SELECT id, batch_no,shade from pro_roll_details where entry_form=99 and status_active=1 and mst_id='$mst_id' and dtls_id='$detls_id'");
	// echo $roll_sql;die;
	$roll_data_arr = array();
	foreach (sql_select($roll_sql) as $row) {
	...
	if (count($sql_bundle_copy) != 0) {
		foreach ($sql_bundle_copy as $inf) {
			...
			foreach ($color_sizeID_arr as $val) {
				...
				$internal_ref = $po_data_arr[$val[csf('order_id')]]["grouping"];
				// $batch_no=$roll_data_arr[$val[csf('roll_id')]];
				$batch_no = $roll_data_arr[$val[csf('roll_id')]]['batch_no'];
				...
				$pdf->Code40($i, $j - 2, $symb . "BUYER#" . substr($buyer_name_str, 0, 18) . "  C#" . $country_arr[$val[csf("country_id")]], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 6.5);
				...
				$pdf->Line(17, 16, 17, 12); // after STKR

				$pdf->Code40($i, $j + 2, "STY#" . substr($style_name, 0, 17), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 34, $j + 2, "PO#" . substr($po_number, 0, 18), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line(0, 8, 68, 8);

				$pdf->Code40($i, $j + 6, "CLR#" . substr($color_library[$val[csf("color_id")]], 0, 16), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 34, $j + 6, "QTY#" . $val[csf("size_qty")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 46, $j + 6, "ITEM#" . substr($garments_item[$val['GMT_ITEM_ID']], 0, 18), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line(0, 12, 68, 12);

				$pdf->Code40($i, $j + 10, "STKR#" . $val[csf("number_start")] . "-" . $val[csf("number_end")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 15, $j + 10, "SH#" . $shade, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 6);
				...
				$pdf->Code40($i + 34, $j + 22, "Date#" . $cut_date, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 8);
		...
		foreach ($color_sizeID_arr as $val) {
			...
			$pdf->Line(67.8, 0, 67.8, 28); // right border
			$pdf->Line(51.7, 20, 51.7, 16); // after cut no
			$pdf->Line(21, 16, 21, 12); // after STKR
			...
			$pdf->Code40($i + 20, $j + 10, "SH#" . $shade, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
			$pdf->Code40($i + 34, $j + 10, "BNDL#" . $val[csf("bundle_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
...
if ($action == "print_bundle_sticker_v2_14") {
	define('FPDF_FONTPATH', '../../../ext_resource/pdf/fpdf/font/');
	require('../../../ext_resource/pdf/code39.php');
	//require('file://///192.168.11.252/logic_erp_3rd_version/production/requires/html_table.php');//order_cut_no
	// print_r($data);die();
	...
	if (count($sql_bundle_copy) != 0) {
		foreach ($sql_bundle_copy as $inf) {
			...
			foreach ($color_sizeID_arr as $val) {
				...
				$pdf->Line($x1, $y1, $x1, $y1 + 28); // left border
				$pdf->Line($x1 + 52.5, $y1, $x1 + 52.5, $y1 + 4); // after cut
				$pdf->Line($x1 + 35.5, $y1, $x1 + 35.5, $y1 + 28); // verticale border
				$pdf->Line($x1 + 67.8, $y1, $x1 + 67.8, $y1 + 28); // right border
				$pdf->Line($x1 + 52.5, $y1 + 12, $x1 + 52.5, $y1 + 16); // after size

				$pdf->Line($x1, $y1 + 0.2, $x2,  $y1 + 0.2);
				$pdf->Code40($i, $j - 2, "BUY: " . substr($buyer_name_str, 0, 18), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 6.5);
				$pdf->Code40($i + 34, $j - 2, "CUT:" . $new_cut_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 51, $j - 2, "Tab: " . $table_name, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line($x1, $y1 + 4, $x2,  $y1 + 4);
				$pdf->Code40($i, $j + 2, "STY: " . substr($style_name, 0, 17), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 34, $j + 2, "Date: " . $cut_date, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line($x1, $y1 + 8, $x2,  $y1 + 8);
				$pdf->Code40($i, $j + 6, "PO: " . substr($po_number, 0, 18), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 34, $j + 6, "SIZE: " . $size_arr[$val[csf("size_id")]], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line($x1, $y1 + 12, $x2,  $y1 + 12);
				$pdf->Code40($i, $j + 10, "ITEM:" . substr($garments_item[$val['GMT_ITEM_ID']], 0, 18), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 34, $j + 10, "QTY: " . $val[csf("size_qty")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 51, $j + 10, "ROLL:" . $val[csf("roll_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line($x1, $y1 + 16, $x2,  $y1 + 16);
				$pdf->Code40($i, $j + 14, "CLR:" . substr($color_library[$val[csf("color_id")]], 0, 16), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 34, $j + 14, "BN: " . $val[csf("bundle_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line($x1, $y1 + 20, $x2,  $y1 + 20);
				$pdf->Code40($i, $j + 18, "Parts:" . substr($inf[csf("bundle_use_for")], 0, 19), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 34, $j + 18, "SL: " . $val[csf("number_start")] . "-" . $val[csf("number_end")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line($x1, $y1 + 24, $x2,  $y1 + 24);
				$pdf->Code40($i, $j + 22, $val[csf("barcode_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 34, $j + 22, "Batch/Lot:" . $batch_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 8);

				$pdf->Line($x1, $y1 + 28, $x2,  $y1 + 28);
				$pdf->Code39($i + 1.3, $j + 30, $val[csf("barcode_no")], '', '', 0.24, 8, true, false, 0.000001);
...
if ($action == "print_barcode_five_per_page") {
	define('FPDF_FONTPATH', '../../../ext_resource/pdf/fpdf/font/');
	require('../../../ext_resource/pdf/code39.php');
	//require('file://///192.168.11.252/logic_erp_3rd_version/production/requires/html_table.php');//order_cut_no
	// print_r($data);die();
	...
	$color_sizeID_arr = sql_select("SELECT c.batch_id, a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence,b.color_id,d.gmt_item_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,c.floor_id,c.inserted_by,c.working_company_id,d.remarks
	from ppl_cut_lay_mst c, ppl_cut_lay_bundle a, ppl_cut_lay_size_dtls b,ppl_cut_lay_dtls d where c.id=a.mst_id and a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id and c.id=d.mst_id and d.id=a.dtls_id and a.status_active=1 and b.status_active=1 and c.status_active=1 $dynamic_cond
	group by c.batch_id,a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence,b.color_id,d.gmt_item_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,c.floor_id,c.inserted_by,c.working_company_id,d.remarks order by b.bundle_sequence,a.id");
	foreach ($color_sizeID_arr as $row) {
		$test_data[$row[csf("roll_id")]] = $row[csf("roll_id")];
		$bundle_no_arr[$row[csf("bundle_no")]] = $row[csf("bundle_no")];
		$last_bundle = $row[csf("bundle_no")];
	}
	...
	$roll_sql = sql_select("SELECT id, batch_no,shade from pro_roll_details where entry_form=99 and status_active=1 and mst_id='$mst_id' and dtls_id='$detls_id'");

	// echo $sql="SELECT id, batch_no,shade from pro_roll_details where entry_form=99 and status_active=1 and mst_id='$mst_id' and dtls_id='$detls_id'";
	...
	foreach ($roll_sql as $row) {
		$roll_data_arr[$row[csf("id")]] = $row[csf("batch_no")];
		$shade_arr[$row[csf("id")]]['shade'] = $row[csf("shade")];
	}

	

	unset($roll_sql);
	$sql_cut_name = sql_select("SELECT entry_date,table_no,cut_num_prefix_no,batch_id,cutting_no, company_id from ppl_cut_lay_mst where id=$data[2]");
	foreach ($sql_cut_name as $cut_value) {
		$table_name = $cut_value[csf('table_no')];
		$table_no = $table_no_library[$cut_value[csf('table_no')]];
		$cut_date = change_date_format($cut_value[csf('entry_date')]);
		$cut_no = $cut_value[csf('cutting_no')];

		$cut_prifix = $cut_value[csf('cut_num_prefix_no')];
		$company_id = $cut_value[csf('company_id')];
		$comp_name = return_field_value("company_name", "lib_company", "id=$company_id");
		$new_cut_no = $comp_name . "-" . $cut_prifix;
		$bundle_title = "";
	}
	if ($data[7] == "") $data[7] = 0;
	...
	$tot_col = count($color_sizeID_arr);
	$tot_bp = count($sql_bundle_copy);
	$tot_page = ceil(($tot_col * $tot_bp) / 6);
	$cur_page = 1;
	$sl = 1;

	if (count($sql_bundle_copy) != 0) {
		foreach ($sql_bundle_copy as $inf) {
			// if($br==5) { $pdf->AddPage(); $br=0; $i=2; $j=4; $k=0;$x=22;$y=22;$z=40;$m=24;$n=36; }
			...
			foreach ($color_sizeID_arr as $val) {
				...
				$po_number = $po_data_arr[$val[csf('order_id')]]["po_number"];
				$internal_ref = $po_data_arr[$val[csf('order_id')]]["grouping"];
				// $remarks=$po_data_arr[$val[csf('order_id')]]["remarks"];
				$batch_no = $roll_data_arr[$val[csf('roll_id')]];
				$batch_no = $roll_data_arr[$val[csf('roll_id')]];
				$shade     =$shade_arr[$val[csf('roll_id')]]['shade'];
				$bundle_no_prifix = $val[csf("bundle_num_prefix_no")];
				$bundle_no_array = explode("-", $val[csf("bundle_no")]);
				$ext_no = "";
				if ($bundle_no_array[4] != "") {
					$ext_no = "-" . $bundle_no_array[4];
				}
				if ($sl == 1) {
					$bndl_Start = $bundle_no_array[3];
				}
				$lst_bundle_no_array = explode("-", $last_bundle);
				$tot_bundle = "T. Bundle : " . $bndl_Start . "-" . $lst_bundle_no_array[3];
				$buyer_name_str = "";
				if ($client_id != 0) $buyer_name_str = $buyer_library[$buyer_name] . '-' . $buyer_library[$client_id];
				else $buyer_name_str = $buyer_library[$buyer_name];
				...
				$pdf->Code40($i + 15, $j, $working_comp_library[$val['WORKING_COMPANY_ID']], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

				$pdf->Code40($i + 90, $j, "Bundle Sheet", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

				$pdf->Code40($i + 160, $j, "Page" . $cur_page . " of " . $tot_page, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				...
				// verticale line
				$pdf->Line(5, $y, 5, $z); // left border
				$pdf->Line(25, $y, 25, $z);
				$pdf->Line(112, $y, 112, $z);
				$pdf->Line(130, $y, 130, $z);


				$pdf->Line(203, $y, 203, $z); // right border
				$y = $y + 50;
				$z = $z + 50;

				$pdf->Line(160, $m, 160, $n); // after cut no
				
				$m = $m + 50;
				$n = $n + 50;
				$remarks = $val[csf("remarks")] ? '[' . $val[csf("remarks")] . ']' : '';
				$order_cut_data = $order_cut_no . '' . $remarks;
				$pdf->Code40($i + 4, $j + 4, "Buyer Name", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 24, $j + 4, substr($buyer_name_str, 0, 18), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 110, $j + 4, " Cutting No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 150, $j + 4, $order_cut_data, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 160, $j + 4, $tot_bundle, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);


				$pdf->Code40($i + 4, $j + 8, "IR No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 24, $j + 8, $internal_ref, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 121, $j + 8, "Date", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 150, $j + 8, $cut_date, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

				$pdf->Code40($i + 4, $j + 12, "Style No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 24, $j + 12, substr($style_name, 0, 22), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

				$pdf->Code40($i + 121, $j + 12, "Size", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 150, $j + 12, $size_arr[$val[csf("size_id")]], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

				$pdf->Code40($i + 4, $j + 16, "PO No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 24, $j + 16, substr($po_number, 0, 18), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

				$pdf->Code40($i + 115, $j + 16, "Quantity", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 150, $j + 16, $val[csf("size_qty")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 161, $j + 16, "Shade:" . $shade, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

				$pdf->Code40($i + 4, $j + 20, "Item",  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 24, $j + 20, substr($garments_item[$val['GMT_ITEM_ID']], 0, 18),  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 112, $j + 20, "Bundle No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 130, $j + 20, $bundle_no_array[3] . $ext_no . " of " . $lst_bundle_no_array[3], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 160, $j + 20, "ROLL:" . $val[csf("roll_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

				$pdf->Code40($i + 4, $j + 24, "Color Name", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 24, $j + 24, substr($color_library[$val[csf("color_id")]], 0, 46) . "#" . chop($print_text . "," . $emb_text, ","),  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 113, $j + 24, "Serial No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 130, $j + 24, $val[csf("number_start")] . "-" . $val[csf("number_end")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 159, $j + 24, " Table:" . $table_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

				$pdf->Code40($i + 4, $j + 28, "Parts Name", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 24, $j + 28, $inf[csf("bundle_use_for")],  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 112, $j + 28, "Batch/Lot", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 130, $j + 28, $batch_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 160, $j + 28, "Unit/User:" . substr($floor_arr[$val['FLOOR_ID']], 0, 12) . "/" . $user_arr[$val['INSERTED_BY']], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Code40($i + 4, $j + 32, "Barcode No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 24, $j + 32, $val[csf("barcode_no")],  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 117, $j + 32, "Sticker", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 140, $j + 34.5, $val[csf("barcode_no")], '', '', 0.20, 5,  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 6);

				if ($br < 5) {

					$pdf->Code40($i, $j + 38, "--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 10);
				}
		...
		foreach ($color_sizeID_arr as $val) {
			...
			$pdf->Code40($i + 24, $j + 4, substr($buyer_name_str, 0, 18), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
			$pdf->Code40($i + 110, $j + 4, " Cutting No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
			$pdf->Code40($i + 150, $j + 4, $order_cut_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
			$pdf->Code40($i + 160, $j + 4, $tot_bundle, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
			...
			$pdf->Code40($i + 4, $j + 24, "Color Name", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
			$pdf->Code40($i + 24, $j + 24, substr($color_library[$val[csf("color_id")]], 0, 46),  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
			$pdf->Code40($i + 113, $j + 24, "Serial No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
			$pdf->Code40($i + 130, $j + 24, $val[csf("number_start")] . "-" . $val[csf("number_end")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
			$pdf->Code40($i + 159, $j + 24, " Table:" . $table_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
			...
			if ($br < 4) {
				...
				$pdf->Code40($i, $j + 47, "--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 10);
			}
			...
			foreach ($color_sizeID_arr as $val)
			 {
				...
				if($sl % 2!=0)
				{
					$pdf->Code40($i + .5, $j, $working_comp_library[$val['WORKING_COMPANY_ID']], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 75, $j, "Date: " .$cut_date, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					...
					$pdf->Line(2, $y, 2, $z); // left border
					$pdf->Line(23, $y, 23, $z);
					$pdf->Line(72, $y, 72, $z);
					$pdf->Line(90, $y, 90, $a);

					$pdf->Line(103, $y, 103, $z); // right border
					$y = $y + 42;
					$z = $z + 42;
					$a = $a + 42;

					$order_cut_data = $order_cut_no;

					$pdf->Code40($i + 2, $j + 4, "Buyer Name", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 22, $j + 4, substr($buyer_name_str, 0, 18), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i +70, $j + 4, " Cutting No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 89, $j + 4, $order_cut_data, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i + 2, $j + 8, "IR No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 22, $j + 8, $internal_ref, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);


					$pdf->Code40($i + 71, $j + 8, "Size", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 88.5, $j + 8, $size_arr[$val[csf("size_id")]], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);


					$pdf->Code40($i + 2, $j + 12, "Style No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 21.5, $j + 12, substr($style_name, 0, 25), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i + 2, $j + 16, "PO No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 22, $j + 16, substr($po_number, 0, 18), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i + 71, $j + 12, "Quantity", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 89, $j + 12, $val[csf("size_qty")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i + 2, $j + 20, "Item",  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 22, $j + 20, substr($garments_item[$val['GMT_ITEM_ID']], 0, 18),  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i + 71, $j + 16, "Bundle No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 88, $j + 16, $bundle_no_array[3] . $ext_no . " of " . $lst_bundle_no_array[3], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i + 71, $j + 20, "Serial No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 89, $j + 20, $val[csf("number_start")] . "-" . $val[csf("number_end")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i + 71, $j + 28, "ROLL:", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 89, $j + 28, $val[csf("roll_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i + 2, $j + 24, "Color Name", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 22, $j + 24, substr($color_library[$val[csf("color_id")]], 0, 46) . "#" . chop($print_text . "," . $emb_text, ","),  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i + 2, $j + 28, "Parts Name", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 22, $j + 28, $inf[csf("bundle_use_for")],  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i + 71, $j + 24, "Batch/Lot", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 88.5, $j + 24, $batch_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i + 2, $j + 32, "Barcode No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 22, $j + 32, $val[csf("barcode_no")],  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					// $pdf->Code40($i + 53, $j + 32, "Sticker", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 70, $j + 34.5, $val[csf("barcode_no")], '', '', 0.12, 5,  $ext = true, $cks = false, $w = 0.5, $h = 1, $wide = true, true, 6);
				...
				else
				{
					...
					$pdf->Code40($ii + 106, $jj, $working_comp_library[$val['WORKING_COMPANY_ID']], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 180, $jj, "Date: " .$cut_date, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					//horizontal line
					$pdf->Line(108, $xx, 208, $xx);
					$xx = $xx + 4;
					$pdf->Line(108, $xx, 208, $xx);
					$xx = $xx + 4;
					$pdf->Line(108, $xx, 208, $xx);
					...
					// verticale line
					$pdf->Line(108, $yy, 108, $zz); // left border
					$pdf->Line(129, $yy, 129, $zz);
					$pdf->Line(177, $yy, 177, $zz);
					$pdf->Line(195, $yy, 195, $aa);




					$pdf->Line(208, $yy, 208, $zz); // right border
					$yy = $yy + 42;
					$zz = $zz+ 42;
					$aa = $aa + 42;

					$pdf->Code40($ii + 107, $jj +4, "Buyer Name", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 128, $jj + 4, substr($buyer_name_str, 0, 18), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($ii +175.3, $jj + 4, " Cutting No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 194, $jj + 4, $order_cut_data, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($ii + 107, $jj + 8, "IR No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 128, $jj + 8, $internal_ref, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($ii + 176, $jj + 8, "Size", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 193.5, $jj + 8, $size_arr[$val[csf("size_id")]], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($ii + 107, $jj + 12, "Style No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 127.5, $jj + 12, substr($style_name, 0, 25), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($ii + 107, $jj + 16, "PO No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 128, $jj + 16, substr($po_number, 0, 18), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($ii + 176, $jj + 12, "Quantity", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 194, $jj + 12, $val[csf("size_qty")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($ii + 107, $jj + 20, "Item",  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 128, $jj + 20, substr($garments_item[$val['GMT_ITEM_ID']], 0, 18),  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($ii + 175.5, $jj + 16, "Bundle No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 193.5, $jj + 16, $bundle_no_array[3] . $ext_no . " of " . $lst_bundle_no_array[3], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($ii + 175.5, $jj + 20, "Serial No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 194, $jj + 20, $val[csf("number_start")] . "-" . $val[csf("number_end")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($ii + 176, $jj + 28, "ROLL:", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 194, $jj + 28, $val[csf("roll_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);


					$pdf->Code40($ii + 107, $jj + 24, "Color Name", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 128, $jj + 24, substr($color_library[$val[csf("color_id")]], 0, 46) . "#" . chop($print_text . "," . $emb_text, ","),  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($ii + 107, $jj + 28, "Parts Name", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 128, $jj + 28, $inf[csf("bundle_use_for")],  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);


					$pdf->Code40($ii + 176, $jj + 24, "Batch/Lot", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 193.3, $jj + 24, $batch_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($ii + 107, $jj + 32, "Barcode No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($ii + 128, $jj + 32, $val[csf("barcode_no")],  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($ii + 175, $jj + 34.5, $val[csf("barcode_no")], '', '', 0.12, 5,  $ext = true, $cks = false, $w = 0.5, $h = 1, $wide = true, true, 6);
...
if ($action == "print_barcode_24_per_page") {
	define('FPDF_FONTPATH', '../../../ext_resource/pdf/fpdf/font/');
	require('../../../ext_resource/pdf/code39.php');
	//require('file://///192.168.11.252/logic_erp_3rd_version/production/requires/html_table.php');//order_cut_no
	// print_r($data);die();
	...
	$color_sizeID_arr = sql_select("SELECT c.batch_id, a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence,b.color_id,d.gmt_item_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,c.floor_id,c.inserted_by,c.working_company_id
	from ppl_cut_lay_mst c, ppl_cut_lay_bundle a, ppl_cut_lay_size_dtls b,ppl_cut_lay_dtls d where c.id=a.mst_id and a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id and c.id=d.mst_id and d.id=a.dtls_id and a.status_active=1 and b.status_active=1 and c.status_active=1 $dynamic_cond
	group by c.batch_id,a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence,b.color_id,d.gmt_item_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,c.floor_id,c.inserted_by,c.working_company_id order by b.bundle_sequence,a.id");
	// echo $color_sizeID_arr;die;
	foreach (($color_sizeID_arr) as $row) {
		$test_data[$row[csf("roll_id")]] = $row[csf("roll_id")];
		$bundle_no_arr[$row[csf("bundle_no")]] = $row[csf("bundle_no")];
	...
	$tot_col = count($color_sizeID_arr);
	$tot_bp = count($sql_bundle_copy);
	$tot_page = ceil(($tot_col * $tot_bp) / 5);
	$cur_page = 1;
	$sl = 1;
	...
	if (count($sql_bundle_copy) != 0) {
		foreach ($sql_bundle_copy as $inf) {
			...
			foreach ($color_sizeID_arr as $val) {
				...
				if ($k <= 7) {
					...
					// // verticale line
					$pdf->Line(0.5, $y, 0.5, $z); // left border
					$pdf->Line(35, $yy, 35, $zz); // middle border up
					$pdf->Line(35, $yyy, 35, $zzz); // middle border down
					$pdf->Line(70, $y, 70, $z); //right border
					// $pdf -> Line(130, $y, 130, $z);
					...
					$pdf->Line(14, $m, 14, $n); // border after qty
					$m += 36.5;
					$n += 36.5;

					$pdf->Line(24, $o, 24, $p); // border after cut no
					$o += 36.5;
					$p += 36.5;

					$pdf->Line(18, $a, 18, $b); // border after cut no
					$a += 36.5;
					$b += 36.5;

					$pdf->Line(26.5, $c, 26.5, $d); // border after cut no
					$c += 36.5;
					$d += 36.5;
					...
					$pdf->Code40($i, $j, "Buy#" . substr($buyer_name_str, 0, 10), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 35, $j, "Job#" . $job_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);


					$pdf->Code40($i, $j + 4, "STY#" . substr($style_name, 0, 10), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 35, $j + 4, "PO#" . substr($po_number, 0, 10), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i, $j + 8, "Color#" . substr($color_library[$val[csf("color_id")]], 0, 16), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i, $j + 12, "QTY#" . $val[csf("size_qty")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 13, $j + 12, "STK#" . $val[csf("number_start")] . "-" . $val[csf("number_end")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i + 35, $j + 12, "BN#" . $val[csf("bundle_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i, $j + 16, "BP#" . substr($inf[csf("bundle_use_for")], 0, 10), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 23, $j + 15.8, "CUT#" . $order_cut_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 8);
					$pdf->Code40($i + 35, $j + 16, "SH#" . $shade, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i, $j + 20, "SIZE#" . substr($size_arr[$val[csf("size_id")]], 0, 10), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 18, $j + 20, "ROLL#" . $val[csf("roll_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 35, $j + 20, "BATCH# " . substr($batch_no, 0, 10), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i, $j + 24, $val[csf("barcode_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i + 26, $j + 24, "PT#" . substr($val['PATTERN_NO'], 0, 10), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
					$pdf->Code40($i + 35, $j + 24, "ITEM#" . substr($garments_item[$val['GMT_ITEM_ID']], 0, 12), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);

					$pdf->Code40($i + 2, $j + 31, $val[csf("barcode_no")], '', '', 0.25, 5,  $ext = true, $cks = false, $w = 0.20, $h = 1, $wide = true, true, 6);

					$i = 1;
					$j += 36.5; // space for 2 sticker
					$x += 8.5;
					$br++;
					$sl++;
					$k++;
					...
					// // verticale line
					$pdf->Line(72, $y, 72, $z); // left border
					$pdf->Line(107, $yy, 107, $zz); // middle border up
					$pdf->Line(107, $yyy, 107, $zzz); // middle border down
					$pdf->Line(140, $y, 140, $z); //right border
					// $pdf -> Line(130, $y, 130, $z);
					...
					$pdf->Line(85.5, $m, 85.5, $n); // border after qty
					$m += 36.5;
					$n += 36.5;

					$pdf->Line(95.5, $o, 95.5, $p); // border after cut no
					$o += 36.5;
					$p += 36.5;


					$pdf->Line(90, $a, 90, $b); // border after cut no
					$a += 36.5;
					$b += 36.5;

					$pdf->Line(98, $c, 98, $d); // border after cut no
					$c += 36.5;
					$d += 36.5;
					...
					// // verticale line
					$pdf->Line(141.5, $y, 141.5, $z); // left border
					$pdf->Line(176, $yy, 176, $zz); // middle border up
					$pdf->Line(176, $yyy, 176, $zzz); // middle border down
					$pdf->Line(209.5, $y, 209.5, $z); //right border
					// $pdf -> Line(130, $y, 130, $z);
					...
					$pdf->Line(154.5, $m, 154.5, $n); // border after qty
					$m += 36.5;
					$n += 36.5;
					$pdf->Line(163.5, $o, 163.5, $p); // border after cut no
					$o += 36.5;
					$p += 36.5;
					$pdf->Line(159, $a, 159, $b); // border after cut no
					$a += 36.5;
					$b += 36.5;

					$pdf->Line(167, $c, 167, $d); // border after cut no
					$c += 36.5;
					$d += 36.5;
		...
		foreach ($color_sizeID_arr as $val) {
			...
			$pdf->Code40($i + 4, $j + 24, "Color Name", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
			$pdf->Code40($i + 24, $j + 24, substr($color_library[$val[csf("color_id")]], 0, 16),  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
			$pdf->Code40($i + 113, $j + 24, "Serial No", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
			$pdf->Code40($i + 130, $j + 24, $val[csf("number_start")] . "-" . $val[csf("number_end")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
			$pdf->Code40($i + 159, $j + 24, " Table:" . $table_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
...
if ($action == "print_barcode_68_10") 
{
	...
	$color_sizeID_arr = sql_select("SELECT c.batch_id, a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, c.shift_name, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence,b.color_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,d.roll_data,c.floor_id,d.gmt_item_id
	from ppl_cut_lay_mst c, ppl_cut_lay_bundle a, ppl_cut_lay_size_dtls b,ppl_cut_lay_dtls d where c.id=a.mst_id and a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id and c.id=d.mst_id and d.id=a.dtls_id and d.id=b.dtls_id and a.status_active=1 and b.status_active=1 and c.status_active=1 $dynamic_cond
	group by c.batch_id,a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, c.shift_name, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence,b.color_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,d.roll_data,c.floor_id,d.gmt_item_id order by b.bundle_sequence,a.id");
	...
	$roll_sql = sql_select("SELECT roll_id, batch_no from pro_roll_details where entry_form=99 and mst_id='$mst_id' and dtls_id='$detls_id'  and status_active=1");
	...
	if (count($sql_bundle_copy) != 0) {
		foreach ($sql_bundle_copy as $inf) {
			...
			foreach ($color_sizeID_arr as $val) {
				...
				$pdf->Code40($i, $j - 2, $symb . "BUY:" . $buyer_library[$buyer_name], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j - 2, "Date:" . $cut_date, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				...
				$pdf->Line(0.2, 0, 0.2, 28); // left border
				$pdf->Line(67.8, 0, 67.8, 28); // right border
				$pdf->Line(54, 12, 54, 8); // after cut no
				$pdf->Line(54, 16, 54, 12); // after Qty
				// $pdf->Line(16.7, 24, 16.7, 16); // after Item
				
				// $pdf->Line(51.7, 24, 51.7, 20); // after batch
				
				$pdf->Line(54, 20, 54, 16); // after LND
				...
				$pdf->Code40($i, $j + 6, $symb . "COLOR:" . substr($color_library[$val[csf("color_id")]], 0, 16), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j + 6, $symb ."S.N:". $val[csf("number_start")] . "-" . $val[csf("number_end")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 54.3, $j + 6, "QTY:" . $val[csf("size_qty")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);



				$pdf->Line(0, 12, 68, 12);

				$pdf->Code40($i, $j + 10, $symb ."Bundle:" . $val[csf("bundle_no")] , $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j + 10,"SIZE:" . $size_arr[$val[csf("size_id")]], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 54.3, $j + 10,"C/NO:" . $order_cut_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line(0, 16, 68, 16);

				$pdf->Code40($i, $j + 14, "Item:" . $garments_item[$val[csf("gmt_item_id")]], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 16, $j + 14, $symb . "A" .$ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j + 14, "L.N:" . $batch_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i+54.3, $j+14, "ROLL:".$val[csf("roll_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;

				$pdf->Line(0, 20, 68, 20);
				$pdf->Code40($i, $j + 18, "PART:" . $inf[csf("bundle_use_for")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Code40($i + 35, $j + 18,  $val[csf("barcode_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
...
if ($action == "print_barcode_68_8") {
	...
	$order_cut_no = return_field_value("order_cut_no", "ppl_cut_lay_dtls", "id=$detls_id and status_active=1 and is_deleted=0", "order_cut_no");
	$working_comp_library = return_library_array("select id, company_name from lib_company", "id", "company_name");
	$pdf = new PDF_Code39('P', 'mm', array(69, 38));
	...
	$color_sizeID_arr = sql_select("SELECT c.batch_id, a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, c.shift_name, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence,b.color_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,d.roll_data,c.floor_id,d.gmt_item_id
	from ppl_cut_lay_mst c, ppl_cut_lay_bundle a, ppl_cut_lay_size_dtls b,ppl_cut_lay_dtls d where c.id=a.mst_id and a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id and c.id=d.mst_id and d.id=a.dtls_id and d.id=b.dtls_id and a.status_active=1 and b.status_active=1 and c.status_active=1 and a.size_qty>0 $dynamic_cond
	group by c.batch_id,a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, c.shift_name, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence,b.color_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,d.roll_data,d.gmt_item_id,c.floor_id order by b.bundle_sequence,a.id");

	$batch_id_arr = array();
	foreach ($color_sizeID_arr as $row) {
		$total_cut_qty += $row[csf('size_qty')];
		$test_data[$row[csf("roll_id")]] = $row[csf("roll_id")];
		$batch_id_arr[$row[csf("batch_id")]] = $row[csf("batch_id")];
	}
	$i = 1;
	...
	$roll_sql = ("select roll_id,ROLL_NO, batch_no, shade from pro_roll_details where mst_id='$data[2]' and entry_form=99 and status_active=1");
	//echo 	$roll_sql;die;
	$roll_data_arr = array();
	foreach (sql_select($roll_sql)  as $row) {
		$roll_data_arr[$row[csf("roll_id")]] = $row[csf("batch_no")];
		//$roll_data_arr[$row[csf("roll_id")]]=$row[csf("roll_no")];


	}
	//print_r($roll_data_arr);
	unset($roll_sql);
	$sql_cut_name = sql_select("SELECT entry_date,table_no,cut_num_prefix_no,company_id,working_company_id from ppl_cut_lay_mst where id=$data[2]");
	foreach ($sql_cut_name as $cut_value) {
		$table_name = $cut_value[csf('table_no')];
		$cut_date = change_date_format($cut_value[csf('entry_date')]);
		$cut_prifix = $cut_value[csf('cut_num_prefix_no')];
	...
	$seq = explode(",", $data[7]);
	$seq_first = $seq[0];
	$table_no = return_field_value("table_no", "lib_cutting_table", "id=$table_name");
	...
	if (count($sql_bundle_copy) != 0) {
		foreach ($sql_bundle_copy as $inf) {
			...
			foreach ($color_sizeID_arr as $val) {
				...
				$pdf->Code40($i, $j - 2, "Cut Qty:" . substr($total_cut_qty, 0, 19) . "," . $inf[csf("bundle_use_for")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j - 2, "Table No:" . $table_no . ",RN-" . $val[csf("roll_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line(0, 0.2, 68, 0.2); // top
				$pdf->Line(0, 4, 68, 4);
				$pdf->Line(35.5, 0, 35.5, 32); // verticale border
				$pdf->Line(0.2, 0, 0.2, 32); // left border
				$pdf->Line(67.8, 0, 67.8, 32); // right border
				// $pdf -> Line(49, 12, 49, 8);// after cut no
				// $pdf -> Line(51.7, 24, 51.7, 20);// after batch
				// $pdf -> Line(16.7, 24, 16.7, 20);// after size
				$pdf->Line(60, 32, 60, 28); //after floor

				$pdf->Code40($i, $j + 2, substr($buyer_name_str, 0, 19), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j + 2, "Date:" . $cut_date, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line(0, 8, 68, 8);

				$pdf->Code40($i, $j + 6, "STYLE:" . substr($style_name, 0, 19), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j + 6, "Batch:" . substr($val[csf("batch_id")], 0, 19), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				// $pdf->Code40($i+48.3, $j+6, "F:".substr($floor_arr[$val['FLOOR_ID']],0,12), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;
				// $pdf->Code40($i+35, $j+18, "B:".substr($val[csf("batch_id")],0,12), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;

				$pdf->Line(0, 12, 68, 12);

				$pdf->Code40($i, $j + 10, "PO:" . substr($po_number, 0, 19), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j + 10, "C/NO:" . $order_cut_no . ";Gmt Qty:" . $val[csf("size_qty")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line(0, 16, 68, 16);

				$pdf->Code40($i, $j + 14, "ITEM:" . substr($garments_item[$val['GMT_ITEM_ID']], 0, 19), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				// $pdf->Code40($i, $j+10, $symb."Gmt No:".$val[csf("number_start")]."-".$val[csf("number_end")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i + 35, $j + 14, "Gmt No:" . $val[csf("number_start")] . "-" . $val[csf("number_end")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line(0, 20, 68, 20);

				$pdf->Code40($i, $j + 18, "Color:" . substr($color_library[$val[csf("color_id")]], 0, 19), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);


				$pdf->Code40($i + 35, $j + 18, "SIZE:" . $size_arr[$val[csf("size_id")]] . "(" . $val[csf("pattern_no")] . ")", $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line(0, 24, 68, 24);
				$pdf->Code40($i, $j + 22, "C.Com:" . substr($working_comp_library[$cut_value[csf("working_company_id")]], 0, 19), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j + 22, "B/NO:" . $val[csf("bundle_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line(0, 28, 68, 28);

				$pdf->Code40($i, $j + 26, $val[csf("barcode_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j + 26, "F:" . substr($floor_arr[$val['FLOOR_ID']], 0, 19), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 59.3, $j + 26, "P:" . $a, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line(0, 32, 68, 32);
				$pdf->Code39($i + 1.3, $j + 33, $val[csf("barcode_no")], '', '', 0.24, 4);
				$k++;
				$i = 1;
				$j = $j + 23;
				$br++;
...
if ($action == "print_barcode_seven_per_page") //kamrul
{
	define('FPDF_FONTPATH', '../../../ext_resource/pdf/fpdf/font/');
	require('../../../ext_resource/pdf/code39.php');
	//require('file://///192.168.11.252/logic_erp_3rd_version/production/requires/html_table.php');//order_cut_no
	...
	$color_sizeID_arr = sql_select("SELECT c.batch_id, a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, c.shift_name, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence,b.color_id,d.gmt_item_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,d.roll_data,c.floor_id
	from ppl_cut_lay_mst c, ppl_cut_lay_bundle a, ppl_cut_lay_size_dtls b,ppl_cut_lay_dtls d where c.id=a.mst_id and a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id and c.id=d.mst_id and d.id=a.dtls_id and d.id=b.dtls_id and a.status_active=1 and b.status_active=1 and c.status_active=1 $dynamic_cond
	group by c.batch_id,a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, c.shift_name, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence,b.color_id,d.gmt_item_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,d.roll_data,c.floor_id order by b.bundle_sequence,a.id");
	...
	unset($roll_sql);
	$sql_cut_name = sql_select("SELECT entry_date,table_no,batch_id,cut_num_prefix_no,company_id from ppl_cut_lay_mst where id=$data[2]");
	// echo $sql_cut_name;die;
	foreach ($sql_cut_name as $cut_value) {
		$table_name = $cut_value[csf('table_no')];
		$table_no = $table_no_library[$cut_value[csf('table_no')]];
	...
	$sql_bundle_copy = sql_select("SELECT id,bundle_use_for from ppl_bundle_title where company_id=$company_id and id in ($data[7])");
	// echo "<pre>"; print_r($color_sizeID_arr); die;
	if (count($sql_bundle_copy) != 0) {
		foreach ($sql_bundle_copy as $inf) {
			if ($br == 1) {
				$pdf->AddPage();
			...
			foreach ($color_sizeID_arr as $val) {
				...
				$pdf->Code40($i, $j - 2, substr($buyer_name_str, 0, 10), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 8);
				$pdf->Code40($i + 35, $j - 2, "Date:" . $cut_date, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 8);

				$pdf->Line(0, 0.2, 68, 0.2); // top
				$pdf->Line(0, 4, 68, 4);
				$pdf->Line(35.5, 0, 35.5, 24); // verticale border
				$pdf->Line(0.2, 0, 0.2, 24); // left border
				$pdf->Line(67.8, 0, 67.8, 24); // right border
				$pdf->Line(50.5, 12, 50.5, 8); // after cut no
				$pdf->Line(50.5, 16, 50.5, 12); // after Qty
				$pdf->Line(50.5,20, 50.5, 16); // after table no
				$pdf->Line(20.5,20, 20.5, 16); // after garments item


				$pdf->Code40($i, $j + 2, substr($style_name, 0, 25), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j + 2, "PO:" . substr($po_number, 0, 16), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line(0, 8, 68, 8);

				$pdf->Code40($i, $j + 6, $symb . "" . substr($color_library[$val[csf("color_id")]], 0, 16), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j + 6, "S.N:" . $val[csf("number_start")] . "-" . $val[csf("number_end")] , $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 50, $j + 6, "C.N-" . $order_cut_no , $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);



				$pdf->Line(0, 12, 68, 12);

				$pdf->Code40($i, $j + 10, $symb . "" . $val[csf("bundle_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j + 10, "SIZE:" . $size_arr[$val[csf("size_id")]],  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 50, $j + 10, "Q:" .$val[csf("size_qty")] , $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);


				$pdf->Line(0, 16, 68, 16);

				$pdf->Code40($i, $j + 14, substr($garments_item[$val['GMT_ITEM_ID']], 0, 18),  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i+20, $j +14, substr($article_no, 0, 18),  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j + 14,  "L.N:" . $batch_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Code40($i+50, $j + 14.5, "T.N:" .$table_no , $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line(0, 20, 68, 20);


				$pdf->Code40($i, $j + 18, "" . $inf[csf("bundle_use_for")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i + 35, $j + 18, $val[csf("barcode_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);

				$pdf->Line(0, 24, 68, 24);
				$pdf->Code40($i + 1.5, $j + 26, $val[csf("barcode_no")], '', '', 0.25, 11,  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 10);
		...
		foreach ($color_sizeID_arr as $val) {
			...
			$pdf->Code40($i, $j - 2, $symb . "BUY:" . substr($buyer_name_str, 0, 10), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
			$pdf->Code40($i + 35, $j - 2, "IR:" . $internal_ref, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
			...
			$pdf->Code40($i, $j + 2, $symb . "STY:" . substr($style_name, 0, 25), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
			$pdf->Code40($i + 35, $j + 2, "PO:" . substr($po_number, 0, 16), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
...
if ($action == "print_barcode_ten_youth") {
	...
	$lib_season_arr = return_library_array("select id, season_name from lib_buyer_season where status_active=1 and is_deleted=0", 'id', 'season_name');
	$order_cut_no = return_field_value("order_cut_no", "ppl_cut_lay_dtls", "id=$detls_id and status_active=1 and is_deleted=0", "order_cut_no");
	$item_name = return_field_value("gmt_item_id", "ppl_cut_lay_dtls", "id=$detls_id and status_active=1 and is_deleted=0", "gmt_item_id");
	...
	if (count($sql_bundle_copy) != 0) {
		...
		foreach ($sql_bundle_copy as $inf) {
			...
			foreach ($color_sizeID_arr as $val) {
				...
				$pdf->Code40($i, $j + 20.5, "CLR# " . $color_library[$data[5]], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
				$pdf->Code40($i + 26, $j + 20.5, "Item# " . $garments_item[$item_name], $ext = true, $cks = false, $w = 0.2, $h = 1, $wide = true, true, 9);
...
if ($action == "save_update_delete_size") {
	...
	} else if ($operation == 1) // Update Here----------------------------------------------------------
	{
		...
		$delete = execute_query("UPDATE ppl_cut_lay_size set status_active=0,is_deleted=1 where mst_id=" . $mst_id . " and dtls_id=" . $dtls_id . "", 0);
		$delete_size = execute_query("UPDATE ppl_cut_lay_size_dtls set status_active=0,is_deleted=1 where mst_id=" . $mst_id . " and dtls_id=" . $dtls_id . "", 0);
		$delete_bundle = execute_query("UPDATE ppl_cut_lay_bundle set status_active=0,is_deleted=1 where mst_id=" . $mst_id . " and dtls_id=" . $dtls_id . "", 0);
		$delete_roll = execute_query("UPDATE ppl_cut_lay_roll_dtls set status_active=0,is_deleted=1 where mst_id=" . $mst_id . " and dtls_id=" . $dtls_id . "", 0);
	...
	<fieldset style="width:960px">
		...
		<table cellpadding="0" cellspacing="0" width="950" rules="all" border="1" class="rpt_table" id="tbl_bundle_list_save">
			...
			<tbody id="trBundleListSave">
				...
				$bundle_cond = where_con_using_array($barcode_no_arr, 1, "b.barcode_no");
				$bundle_cond2 = where_con_using_array($barcode_no_arr, 0, "c.barcode_no");
				...
				?>
					<tr id="trBundleListSave_<? echo $i;  ?>">
						...
						<td align="center">
							<input id="chk_status_<?= $i; ?>" type="checkbox" name="chk_status[]" value="<?= $row[csf('hold')]; ?>" onClick="check_status_value(<?= $i ?>);" <? if ($row[csf('hold')] == 1) {
																																													echo 'checked';
																																												} ?>>
...
if ($action == "cut_lay_bundle_print") {
	...
	from ppl_cut_lay_mst a, ppl_cut_lay_dtls b where a.id=b.mst_id and a.id='$data[1]' and a.company_id='$data[0]' and b.id='$data[2]' ";
	// echo $sql;
	$dataArray = sql_select($sql);
	$sql_buyer = sql_select("SELECT a.buyer_name,a.brand_id,a.fit_id,a.style_ref_no as ref,a.style_description as des,b.grouping from wo_po_details_master a,wo_po_break_down b where a.id=b.job_id and a.job_no='" . $dataArray[0][csf('job_no')] . "' and a.company_name=$data[0]");
	//print_r($sql_buyer);
	...
	$brand = $sql_buyer[0][csf('brand_id')];
	$fit_lists = $fit_list_arr[$sql_buyer[0][csf('fit_id')]];
	//echo $fit_list;
...
?>
	<div style="width:1000px; " align="center">
		<table width="990" cellspacing="0" align="center">
			...
			<tr>
				<td width="120"><strong>Cut No:</strong></td>
				<td width="160"><? echo $cut_no; ?></td>
				<td width="120"><strong>Table No :</strong></td>
				<td width="160"><? echo $table_no_library[$dataArray[0][csf('table_no')]]; ?></td>
				<td width="120"><strong>Job No :</strong></td>
				<td width="160"><? echo $dataArray[0][csf('job_no')]; ?></td>
			</tr>
			<tr>
				<td><strong>Buyer:</strong></td>
				<td width="160"><? echo $buyer_library[$sql_buyer[0][csf('buyer_name')]]; ?></td>
				<td><strong>Batch No:</strong></td>
				<td width="160"><? echo $batch_no; ?></td>
				<td><strong>Brand :</strong></td>
				<td width="160"><? echo $brand_library[$sql_buyer[0][csf('brand_id')]]; ?></td>
			</tr>
			</tr>
			<tr>
				<td><strong>Gmt Item:</strong></td>
				<td width="160"><? echo $garments_item[$dataArray[0][csf('gmt_item_id')]]; ?></td>
				<td><strong>Color :</strong></td>
				<td width="160"><? echo $color_library[$dataArray[0][csf('color_id')]]; ?></td>
				<td><strong>Marker Length :</strong></td>
				<td width="160"><? echo $dataArray[0][csf('marker_length')]; ?></td>
			</tr>
			<tr>
				<td><strong>Marker Width :</strong></td>
				<td width="160"><? echo $dataArray[0][csf('marker_width')]; ?></td>
				<td><strong>Fabric Width:</strong></td>
				<td width="160"><? echo $dataArray[0][csf('fabric_width')]; ?></td>
				<td><strong>Gsm:</strong></td>
				<td width="160"><? echo $dataArray[0][csf('gsm')]; ?></td>
			</tr>
			<tr>
				<td><strong>Order Cut No:</strong></td>
				<td width="160"><? echo $order_cut_no; ?></td>
				<td><strong>Plies:</strong></td>
				<td width="160"><? echo $dataArray[0][csf('plies')]; ?></td>
				<td><strong>Cut Date:</strong></td>
				<td width="160"><? echo $dataArray[0][csf('entry_date')]; ?></td>
			</tr>
			<tr>
				<td><strong>Style Ref:</strong></td>
				<td width="160"><? echo $style_ref; ?></td>
				<td><strong>Style Desc.:</strong></td>
				<td width="160"><? echo $style_desc; ?></td>
			...
			<tr>
				...
				<td width="160"><? echo $working_comp_library[$dataArray[0][csf('working_company_id')]]; ?></td>
				<td><strong>Working Location:</strong></td>
				<td width="160"><? echo $working_location_library[$dataArray[0][csf('location_id')]]; ?></td>


			</tr>
			<tr>
				<td><strong>Working Floor:</strong></td>
				<td width="160"><? echo $working_floor_library[$dataArray[0][csf('floor_id')]]; ?></td>
				<td><strong>Cutting Part:</strong></td>
				<td width="160" <? echo $data[5]; ?></td>
				<td><strong>Roll Weight:</strong></td>
				<td colspan="2"><? echo $RollDataArray[$row[csf('id')]]['roll_weight']; ?></td>
			</tr>

			<tr>
				<td><strong>Fit:</strong></td>
				<td width="160"><? echo $fit_lists; ?></td>
		...
		<div style="width:1270px;">
			<table align="center" cellspacing="0" width="1250" border="1" rules="all" class="rpt_table">
				...
				<tbody>
					...
					$size_data = sql_select("SELECT a.id,a.size_id,a.bundle_sequence,a.marker_qty from ppl_cut_lay_size_dtls a where a.mst_id='$data[1]' and a.dtls_id='$data[2]'  and a.status_active=1 and a.is_deleted=0 order by a.bundle_sequence ASC");
					$j = 1;
					$bundle_id_arr = explode(",", $data[6]);
					$bundle_id_cond = "";
					if ($data[6] > 0) {
					...
					foreach ($size_data as $size_val) {
						...
						$bundle_data = sql_select("SELECT a.id,a.bundle_no as bundle_no,a.bundle_num_prefix_no,a.size_id,a.number_start,a.number_end,a.size_qty, a.country_id, a.roll_no, a.roll_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,a.barcode_no,b.remarks,b.roll_data from ppl_cut_lay_bundle a, ppl_cut_lay_dtls b where b.id=a.dtls_id and a.mst_id='$data[1]' and a.dtls_id='$data[2]' and a.size_id=" . $size_val[csf('size_id')] . " and a.status_active=1 and a.is_deleted=0 $country_cond $bundle_id_cond order by a.id ASC");
						...
						foreach ($bundle_data as $row) {
							$roll_data_arr = explode("**", $row[csf('roll_data')]);
							$shade = "";
							foreach ($roll_data_arr as $val) {
								$rollData = explode("=", $val);
								if ($rollData[2] == $row[csf('roll_id')]) {
									$shade = $rollData[7];
								}
							}
					...
					?>
							<tr>
								...
								<td align="center"></td>
								<td align="center"></td>
								<td align="left"><?= $row[csf('remarks')];  ?></td>
							</tr>
						<?
							...
							$order_wise_qnty_arr[$row[csf('order_id')]][$row[csf('size_id')]]+=$row[csf('size_qty')];
						}
					...
					<tr bgcolor="#BBBBBB">
						<td align="center"></td>
						<td colspan="9" align="right"> Total marker qty.</td>
			...
			<div>
			<table align="middle" cellspacing="0" width="400" border="1" rules="all" class="rpt_table">
				<thead>
					<tr>
						<th colspan="3" align="left">Order Wise Summary</th>
					</tr>
					<tr>
						<th>Order No.</th>
						<th>Size</th>
				...
				<tbody>
					<?
                      $totalOrder = 0 ;
					foreach ($order_wise_qnty_arr as $po_id => $po_val)
					{


						foreach($po_val as $key=>$val)
						 {
					?>
						<tr>
							<td align="center"><? echo $po_arr[$po_id]; ?></td>
							<td align="center"><? echo $size_arr[$key]; ?></td>
					...
					<?
						...
						?>
                           <tr>
							<td colspan="2" align="right"> Order Wise Total = </td>

							<td align="center"><?  echo   array_sum( $po_val) ?></td>
						   </tr>
						<?
...
if ($action == "create_job_search_list_view") {
	...
	if (str_replace("'", "", $brand_id) != 0)  $brand_cond = "and b.brand_id=$brand_id";
	else $brand_cond = "";
	...
	echo create_list_view("list_view", "Job NO,Year,Style Ref,Buyer Name,Brand Name,File No,Internal Ref. No, Order No,Shipment Date", "60,60,100,100,100,100,100,100,100", "850", "270", 0, $sql_order, "js_set_order", "job_no,buyer_name,year,style_ref_no", "", 1, "0,0,0,buyer_name,brand_id,0,0,0,0,0", $arr, "job_prefix,year,style_ref_no,buyer_name,brand_id,file_no,grouping,po_number,pub_shipment_date", "", "setFilterGrid('list_view',-1)");
}
...
if ($action == "save_update_delete") {
	...
	if ($operation == 0) // Insert Here----------------------------------------------------------
	{
		...
		{
			$ffSql = "SELECT a.id as issue_id, c.QUANTITY from inv_issue_master a, inv_transaction b,order_wise_pro_details c,WO_PO_BREAK_DOWN d where a.id=b.mst_id and b.id=c.trans_id and c.po_breakdown_id=d.id and c.entry_form in(18,19) and c.trans_type=2 and c.status_active=1 and d.job_no_mst=$txt_job_no and a.issue_purpose in(9)";
			// echo "20**".$ffSql;die;
			...
			foreach ($ffres as $v)
			{
				...
				$issue_id_arr[$v['ISSUE_ID']] = $v['ISSUE_ID'];
			}



			// ======================== issue rtn ====================================
			$issue_id_cond = where_con_using_array($issue_id_arr, 0, "a.issue_id");
			$ffSql = "SELECT c.QUANTITY from INV_RECEIVE_MASTER a, INV_TRANSACTION b,ORDER_WISE_PRO_DETAILS c where a.id=b.mst_id and b.id=c.trans_id and a.entry_form in(209,52) and c.entry_form in(209,52) and b.status_active=1 and c.status_active=1 $issue_id_cond";
			// echo "20**".$ffSql;die;
			$ffres = sql_select($ffSql);
			$ff_issue_rtn_qty = 0;
			foreach ($ffres as $v)
			{
				$ff_issue_rtn_qty += $v['QUANTITY'];
			}
			$tot_ff_issue_qty = $ff_issue_qty - $ff_issue_rtn_qty;
			// echo "20**$ff_issue_qty - $ff_issue_rtn_qty";die;

			// ======================== prev ff cut qty ======================
			$sql = "SELECT a.id,a.OTHER_FABRIC_WEIGHT,a.PIPING_CONJ,b.QNTY from PPL_CUT_LAY_MST a, PRO_ROLL_DETAILS b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and a.job_no=$txt_job_no and b.ENTRY_FORM=99";
			$res = sql_select($sql);
			$prev_ff_cut_qty = 0;
			$chk_arr = array();
			foreach ($res as $v) {
				$prev_ff_cut_qty += $v['QNTY'];
				if ($chk_arr[$v['ID']] == "") {
					$prev_ff_cut_qty += $v['OTHER_FABRIC_WEIGHT'] + $v['PIPING_CONJ'];
					$chk_arr[$v['ID']] = $v['ID'];
				}
			}

			// ================== get current qty ======================
			$other_fabric_weight = str_replace("'", "", $txt_other_fabric_weight);
			$pip_conj = str_replace("'", "", $piping_conj);
			$roll_weight = 0;
			for ($i = 1; $i <= $row_num; $i++)
			{
				...
				for ($x = 0; $x < count($save_string); $x++)
				{
					...
					$roll_weight += $roll_dtls[3];
				}
			}
			$tot_cut_qty = $roll_weight + $other_fabric_weight + $pip_conj + $prev_ff_cut_qty;
			// echo "20**".$roll_weight ."+". $other_fabric_weight ."+". $pip_conj ."+". $prev_ff_cut_qty;die;
			$tot_cut_lay_qty = $roll_weight + $other_fabric_weight + $pip_conj;
			$excess_qty = $tot_ff_issue_qty - $tot_cut_qty;
			// echo "20**$tot_ff_issue_qty < $tot_cut_qty";die;
			if ($tot_ff_issue_qty > 0)
			{
				if ($tot_ff_issue_qty < $tot_cut_qty)
				{
					echo "20**Fabric issue qty=" . $tot_ff_issue_qty . ",Previous Cut Lay Fab. Issue=" . $prev_ff_cut_qty . ",Cut and lay Fab. Issue=" . $tot_cut_lay_qty . ",Excess Qty=" . $excess_qty;
					die;
				}
			}
			else
		...
		$field_array1 = "id, mst_id,order_ids,country_ids,color_type_id,order_cut_no,color_id,batch_id,gmt_item_id,plies,order_qty,roll_data,remarks,inserted_by,insert_date,status_active,is_deleted";

		$field_array_roll = "id, barcode_no, mst_id, dtls_id, po_ids, entry_form, qnty, roll_id, roll_no, plies,shade, batch_no,is_extra_roll, inserted_by, insert_date";
		...
		for ($i = 1; $i <= $row_num; $i++) {
			...
			for ($x = 0; $x < count($save_string); $x++) {
				...
				$data_array_roll .= "(" . $id_roll . "," . $barcode_no . "," . $id . "," . $detls_id . "," . $$cbo_order_id . ",99,'" . $roll_qnty . "','" . $roll_id . "','" . $roll_no . "','" . $plies . "','" . $shade . "','" . $batchNo . "','" . $batchId . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
				$response_data .= $barcode_no . "=" . $roll_no . "=" . $roll_id . "=" . $roll_qnty . "=" . $plies . "=" . $batchNo . "=" . $batchId . "=" . $shade . "**";
				//$id_roll = $id_roll+1;
			}
			...
			$data_array1 .= "(" . $detls_id . "," . $id . "," . $$cbo_order_id . "," . $$cbo_country_id . "," . $$cboColorType . "," . $$order_cut_no . "," . $$cbocolor . "," . $$cbobatch . "," . $$cbo_gmt_id . "," . $$txt_plics . "," . $$order_qty . ",'" . $response_data . "'," . $$txtremarks . ",'" . $user_id . "','" . $pc_date_time . "',1,0)";
	...
	{
           //=============================Permission Request============================================//
			$ua_sql="SELECT company_name,variable_list,cut_panel_delevery_hcode,cut_panel_delevery,planning_board_strip_caption from variable_settings_production where company_name=$cbo_company_name and variable_list=79 and cut_panel_delevery_hcode=490";
		    //  echo "20**".$ua_sql;die;
			$ua_time_sql=sql_select($ua_sql);
			// $update_hour_arr=array();
			// $permission_arr=array();
            foreach($ua_time_sql as $row)
			{
				$update_hour=$row['CUT_PANEL_DELEVERY'];
				$permission_team=$row['PLANNING_BOARD_STRIP_CAPTION'];

			}
			//   echo "20**".$update_hour;die;

			// echo '<pre>';
			// print_r($update_hour_arr);
			// echo '</pre>';
		   $cutting_sql="SELECT cutting_no,insert_date,updated_by from ppl_cut_lay_mst where cutting_no=$txt_cutting_no and status_active=1 and is_deleted=0";
		    // echo "20**".$cutting_sql;die;
            $cutting_main_sql=sql_select($cutting_sql);
			// $cutting_arr=array();
			foreach($cutting_main_sql as $v)
			{
				$insert_hour=$v[csf('insert_date')];
				$updated_by=$v[csf('updated_by')];
			}
			//  echo "20**".$insert_hour;die;
			//  echo "20**".$updated_by;die;

			$insert_date=$insert_hour;
			$today=time();
			$insert_time=strtotime($insert_date);
			$tot_hour = round(abs($today-$insert_time)/60/60);
			$user_id=$_SESSION['logic_erp']['user_id'];
			//    echo "20**".$user_id;die;

			if($permission_team==$user_id && $tot_hour>=$update_hour)
			{

			   echo "20**"."Sorry User! You can not update it--Thank you"; die;
			}
		...
		if ($cut_lay_fab_conj_validation == 1) // when yes
		{
			// $entryForm = (str_replace("'","",$garments_nature)==2) ? 18 : 19;
			$ffSql = "SELECT a.id as issue_id, c.QUANTITY from inv_issue_master a, inv_transaction b,order_wise_pro_details c,WO_PO_BREAK_DOWN d where a.id=b.mst_id and b.id=c.trans_id and c.po_breakdown_id=d.id and c.entry_form in(18,19) and c.trans_type=2 and c.status_active=1 and d.job_no_mst=$txt_job_no and a.issue_purpose in(9)";
			// echo "20**".$ffSql;die;
			$ffres = sql_select($ffSql);
			$ff_issue_qty = 0;
			...
			// ======================== issue rtn ====================================
			$issue_id_cond = where_con_using_array($issue_id_arr, 0, "issue_id");
			$issue_id_cond = where_con_using_array($issue_id_arr, 0, "a.issue_id");
			$ffSql = "SELECT c.QUANTITY from INV_RECEIVE_MASTER a, INV_TRANSACTION b,ORDER_WISE_PRO_DETAILS c where a.id=b.mst_id and b.id=c.trans_id and a.entry_form in(209,52) and c.entry_form in(209,52) and b.status_active=1 and c.status_active=1 $issue_id_cond";
			// echo "20**".$ffSql;die;
			$ffres = sql_select($ffSql);
			...
			// ======================== prev ff cut qty ======================
			$sql = "SELECT a.id,a.OTHER_FABRIC_WEIGHT,a.PIPING_CONJ,b.QNTY from PPL_CUT_LAY_MST a, PRO_ROLL_DETAILS b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and a.job_no=$txt_job_no and b.ENTRY_FORM=99 and a.id!=$update_id";
			$res = sql_select($sql);
			$prev_ff_cut_qty = 0;
			$chk_arr = array();
			foreach ($res as $v) {
		...
		$sql = "SELECT a.id, a.color_id, a.gmt_item_id,a.order_ids,a.country_ids from ppl_cut_lay_dtls a, ppl_cut_lay_bundle b where a.id=b.dtls_id and b.status_active=1 and b.is_deleted=0 and a.mst_id=$update_id";
		// echo $sql;die;
		$res = sql_select($sql);
		...
		//master table update*********************************************************************
		$field_array = "table_no*job_no*batch_id*company_id*working_company_id*location_id*floor_id*entry_date*start_time*end_date*end_time*marker_length*other_fabric_weight*marker_width*fabric_width*gsm*fabric_gsm*width_dia*cad_marker_cons*shift_name*cad_effi*piping_conj*updated_by*update_date";
		$start_time = str_replace("'", "", $txt_in_time_hours) . ":" . str_replace("'", "", $txt_in_time_minuties);
		$end_time = str_replace("'", "", $txt_out_time_hours) . ":" . str_replace("'", "", $txt_out_time_minuties);
		$data_array = "" . $tbl_id . "*" . $txt_job_no . "*" . $txt_batch_no . "*" . $cbo_company_name . "*" . $cbo_working_company_name . "*" . $cbo_location_name . "*" . $cbo_floor_name . "*" . $txt_entry_date . "*'" . $start_time . "'*" . $txt_end_date . "*'" . $end_time . "'*" . $txt_marker_length . "*" . $txt_other_fabric_weight . "*" . $txt_marker_width . "*" . $txt_fabric_width . "*" . $txt_gsm . "*" . $fabric_gsm . "*" . $cbo_width_dia . "*" . $txt_marker_cons . "*" . $cbo_shift_name . "*" . $cad_efficiency . "*" . $piping_conj . "*'" . $user_id . "'*'" . $pc_date_time . "'";


		//$detls_id=return_next_id("id", " ppl_cut_lay_dtls", 1);
		$detls_id = return_next_id_by_sequence("ppl_cut_lay_dtls_seq",  "ppl_cut_lay_dtls", $con);
		$field_array1 = "id, mst_id,order_ids,country_ids,color_type_id,order_cut_no,ship_date,color_id,batch_id,gmt_item_id,plies,order_qty,roll_data,remarks,inserted_by,insert_date,status_active,is_deleted";
		$field_array_up = "order_ids*country_ids*color_type_id*order_cut_no*ship_date*color_id*batch_id*gmt_item_id*plies*order_qty*roll_data*remarks*updated_by*update_date";

		$roll_array_up = "roll_no*plies*shade*batch_no*is_extra_roll*updated_by*update_date";
		$field_array_roll = "id, barcode_no, mst_id, dtls_id, po_ids, entry_form, qnty, roll_id, roll_no, plies,shade, batch_no,is_extra_roll, inserted_by, insert_date";
		// ================ get prev roll id ===============
		$prev_roll_data_arr = return_library_array("SELECT id,roll_data from PPL_CUT_LAY_DTLS where mst_id=$update_id", "id", "roll_data");
		$prev_roll_id_arr = return_library_array("SELECT id,id as roll_id from PRO_ROLL_DETAILS where mst_id=$update_id and entry_form=99", "id", "roll_id");
		// print_r($prev_roll_id_arr);die;
		$new_roll_id_arr = array();
		$update_roll_id_arr = array();
		//$id_roll = return_next_id( "id", "pro_roll_details", 1 );
		$add_comma = 0;
		$add_comma_roll = 0;
		...
		for ($i = 1; $i <= $row_num; $i++) {
			...
			if (count($prev_color_item_po_array) > 0 && str_replace("'", '', $$update_details_id) != "") {
				...
				if ($prev_color_item_po_array[str_replace("'", '', $$update_details_id)]['country_ids'] != str_replace("'", '', $$cbo_country_id)) {
					$flag = 1;
				}
				if ($flag == 1) {
					echo "201**Update restricted! Cause already bundle created. You can not change color,item,country and order.";
					disconnect($con);
					die;
				}
			}
			...
			for ($x = 0; $x < count($save_string); $x++) {
				...
				$batchId = $roll_dtls[6];
				$shade = $roll_dtls[7];
				$update_roll_id_arr[$roll_id] = $roll_id;
				if ($prev_roll_id_arr[$roll_id] == "") {
					$id_roll = return_next_id_by_sequence("PRO_ROLL_DTLS_PK_SEQ", "pro_roll_details", $con);
					if (str_replace("'", '', $$roll_maintained) != 1) $roll_id = $id_roll;
					$new_roll_id_arr[$id_roll] = $id_roll;

					if ($data_array_roll != "") $data_array_roll .= ",";
					$data_array_roll .= "(" . $id_roll . "," . $barcode_no . "," . $msster_id . "," . $dtlsId . "," . $$cbo_order_id . ",99,'" . $roll_qnty . "','" . $roll_id . "','" . $roll_no . "','" . $plies . "','" . $shade . "','" . $batchNo . "','" . $batchId . "'," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
				} else {
					$updateRollID_array[] = str_replace("'", '', $roll_id);
					$roll_data_array_up[str_replace("'", '', $roll_id)] = explode("_", ("'" . $roll_no . "'_'" . $plies . "'_'" . $shade . "'_'" . $batchNo . "'_'" . $batchId . "'_'" . $user_id . "'_'" . $pc_date_time . "'"));
				}
				$response_data .= $barcode_no . "=" . $roll_no . "=" . $roll_id . "=" . $roll_qnty . "=" . $plies . "=" . $batchNo . "=" . $batchId . "=" . $shade . "**";
				//$id_roll = $id_roll+1;
			}
			...
			if (str_replace("'", '', $$update_details_id) != "") {
				...
				$data_array_up[str_replace("'", '', $$update_details_id)] = explode("_", ("" . $$cbo_order_id . "_" . $$cbo_country_id . "_" . $$cboColorType . "_" . $$order_cut_no . "_" . $$txt_ship_date . "_" . $$cbocolor . "_" . $$cbobatch . "_" . $$cbo_gmt_id . "_" . $$txt_plics . "_" . $$order_qty . "_'" . $response_data . "'_" . $$txtremarks . "_'" . $user_id . "'_'" . $pc_date_time . "'_1_0"));
				//$dtlsId=str_replace("'",'',$$update_details_id);
				...
				$data_array1 .= "(" . $detls_id . "," . $msster_id . "," . $$cbo_order_id . "," . $$cbo_country_id . "," . $$cboColorType . "," . $$order_cut_no . ",'" . $$txt_ship_date . "'," . $$cbocolor . "," . $$cbobatch . "," . $$cbo_gmt_id . "," . $$txt_plics . "," . $$order_qty . ",'" . $response_data . "'," . $$txtremarks . "," . $user_id . ",'" . $pc_date_time . "',1,0)";
				$detls_id_array .= $detls_id . "#" . str_replace("'", '', $$order_cut_no);
				//$dtlsId=$detls_id;
				//$detls_id=$detls_id+1;
				$detls_id = return_next_id_by_sequence("ppl_cut_lay_dtls_seq",  "ppl_cut_lay_dtls", $con);
		...
		if (count($update_roll_id_arr) > 0) {
			$delete_roll_id_arr = array_diff($prev_roll_id_arr, $update_roll_id_arr);
			$delete_roll_ids = implode(",", $delete_roll_id_arr);
			if ($delete_roll_ids != "") {
				// echo "delete from pro_roll_details where mst_id=$msster_id and id in($delete_roll_ids) and entry_form=546";die;
				$delete_roll = execute_query("delete from pro_roll_details where mst_id=$msster_id and id in($delete_roll_ids) and entry_form=99", 0);
			}
		}

		// echo bulk_update_sql_statement("pro_roll_details","id",$roll_array_up,$roll_data_array_up,$updateRollID_array);die;
		if ($roll_data_array_up != "") {
			$rID2 = execute_query(bulk_update_sql_statement("pro_roll_details", "id", $roll_array_up, $roll_data_array_up, $updateRollID_array), 1);
		}
		if ($data_array_roll != "") {
			$rID4 = sql_insert("pro_roll_details", $field_array_roll, $data_array_roll, 0);
		}
		...
		if ($db_type == 0) {
			if ($rID && $rID1 && $rID2 && $rID3 && $rID4  && $status_change) {
				mysql_query("COMMIT");
				...
				<form name="searchprocessfrm_1" id="searchprocessfrm_1" autocomplete="off">
					...
					<div style="width:390px; overflow-y:scroll; max-height:280px;" id="buyer_list_view" align="center">
						<table cellspacing="0" cellpadding="0" border="1" rules="all" width="370" class="rpt_table" id="tbl_list_search">
							...
							$is_control=return_field_value("is_control","variable_settings_production","company_name=$company_id and variable_list=33 and page_category_id=318");
							...
							if($is_control==1)
							{
								$sql = "SELECT a.id, a.po_number, a.pub_shipment_date from wo_po_break_down a, wo_po_color_size_breakdown b,gmts_production_confirmation c where a.id=b.po_break_down_id and a.id=c.po_id and a.job_no_mst='$txt_job' and b.color_number_id='$cbocolor' and b.item_number_id='$gmt_id' $projected_po_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.shiping_status<>3 and c.confirm_status=1 group by a.id, a.po_number, a.pub_shipment_date order by a.pub_shipment_date";
							}
							else
							{
				...
				<form name="searchprocessfrm_1" id="searchprocessfrm_1" autocomplete="off">
					...
					<div style="width:390px; overflow-y:scroll; max-height:280px;" id="buyer_list_view" align="center">
						<table cellspacing="0" cellpadding="0" border="1" rules="all" width="370" class="rpt_table" id="tbl_list_search">
							...
							$i = 1;
							$po_row_id = '';
							$countryIdArr = explode(",", $country_id);
							$sql = "SELECT a.id, a.country_name,b.country_ship_date from lib_country a, wo_po_color_size_breakdown b where a.id=b.country_id and b.job_no_mst='$txt_job' and b.color_number_id='$cbocolor' and b.item_number_id='$gmt_id' and b.po_break_down_id in($poId) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  group by a.id, a.country_name,b.country_ship_date order by b.country_ship_date";
							// echo $sql;
							$result = sql_select($sql);
							foreach ($result as $row) {
								if ($i % 2 == 0) $bgcolor = "#E9F3FF";
								...
								if (in_array($row[csf('id')], $countryIdArr)) {
									if ($po_row_id == "") $po_row_id = $i;
									else $po_row_id .= "," . $i;
								}
							?>
								<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i; ?>" onClick="js_set_value(<? echo $i; ?>)">
									<td width="40" align="center"><?php echo "$i"; ?>
										<input type="hidden" name="txt_individual_id" id="txt_individual_id<?php echo $i ?>" value="<? echo $row[csf('id')]; ?>" />
										<input type="hidden" name="txt_individual" id="txt_individual<?php echo $i ?>" value="<? echo $row[csf('country_name')]; ?>" />
									</td>
									<td width="200">
										<p><? echo $row[csf('country_name')]; ?></p>
									</td>
									<td align="center">
										<p><? echo change_date_format($row[csf('country_ship_date')]); ?></p>
									</td>
								</tr>
							<?
								$i++;
...
if ($action == "create_cutting_search_list_view") {
	...
	if ($search_type == 1) // exact
	{
		...
		else $job_cond = "and b.job_no_prefix_num='" . str_replace("'", "", $job_no) . "'";
		if (str_replace("'", "", $order_cut_no) == "") $order_cut_no_cond = "";
		else $order_cut_no_cond = "and c.order_cut_no='" . str_replace("'", "", $order_cut_no) . "'";
		if (str_replace("'", "", $internal_ref) == "") $internal_ref_cond = "";
		else $internal_ref_cond = "and d.grouping='" . str_replace("'", "", $internal_ref) . "'";
		if (str_replace("'", "", $order_no) == "") $order_cond = "";
		else $order_cond = "and d.po_number='" . trim($order_no) . "' ";
	} else if ($search_type == 2) //starts with
	{
		...
		if (str_replace("'", "", $internal_ref) == "") $internal_ref_cond = "";
		else $internal_ref_cond = "and d.grouping like'" . str_replace("'", "", $internal_ref) . "%'";
		if (str_replace("'", "", $order_no) == "") $order_cond = "";
		else $order_cond = "and d.po_number like '" . trim($order_no) . "%' ";
	...
	} else // contents
	{
		...
		if (str_replace("'", "", $job_no) == "") $job_cond = "";
		else $job_cond = "and b.job_no_prefix_num like '%" . str_replace("'", "", $job_no) . "%'";
		if (str_replace("'", "", $order_cut_no) == "") $order_cut_no_cond = "";
		else $order_cut_no_cond = "and c.order_cut_no like '%" . str_replace("'", "", $order_cut_no) . "%'";
		if (str_replace("'", "", $internal_ref) == "") $internal_ref_cond = "";
		else $internal_ref_cond = "and d.grouping like '%" . str_replace("'", "", $internal_ref) . "%'";
		if (str_replace("'", "", $order_no) == "") $order_cond = "";
		else $order_cond = "and d.po_number like '%" . trim($order_no) . "%' ";
	...
	if (str_replace("'", "", $insert_year) == "") $insert_year_cond = "";
	else $insert_year_cond = "and to_char(a.insert_date,'YYYY')=$insert_year ";
	...
	if (str_replace("'", "", $txt_shift_name) == 0) $shift_cond = "";
	else $shift_cond = "and a.shift_name=" . str_replace("'", "", $txt_shift_name) . "";
	...
	<?
	if ($order_no != "" || $internal_ref != "") {
		$job_no_arr = return_library_array("SELECT d.job_no_mst, d.job_no_mst as job FROM wo_po_break_down d where status_active=1 $order_cond $internal_ref_cond", 'job_no_mst', 'job');
		$job_no_cond = where_con_using_array($job_no_arr, 1, "a.job_no");
	}
	// $sql_order="SELECT a.id,a.cut_num_prefix_no, a.table_no, a.job_no, a.batch_id, a.entry_date, a.marker_length, a.marker_width, a.fabric_width,a.shift_name,c.color_id,c.id as dtls_id, c.marker_qty, c.order_cut_no,$year,b.buyer_name,b.style_ref_no, d.grouping,d.po_number FROM ppl_cut_lay_mst a,wo_po_details_master b,ppl_cut_lay_dtls c, wo_po_break_down d where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and  c.mst_id=a.id and a.job_no=b.job_no and a.job_no = d.job_no_mst and a.entry_form=99 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 $conpany_cond $cut_cond $job_cond $sql_cond $internal_ref_cond $order_cut_no_cond $insert_year_cond $order_cond order by id desc";
	$sql_order = "SELECT a.id,a.cut_num_prefix_no, a.table_no, a.job_no, a.batch_id, a.entry_date, a.marker_length, a.marker_width, a.fabric_width,a.shift_name,c.color_id,c.id as dtls_id, c.marker_qty,c.order_ids, c.order_cut_no,$year,b.buyer_name,b.style_ref_no FROM ppl_cut_lay_mst a,wo_po_details_master b,ppl_cut_lay_dtls c where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and  c.mst_id=a.id and a.job_no=b.job_no and a.entry_form=99 and c.status_active=1 and c.is_deleted=0 $conpany_cond $cut_cond $job_cond $sql_cond  $order_cut_no_cond $insert_year_cond $job_no_cond order by id desc";
	// echo $sql_order;die;
	...
	<table class="rpt_table" id="list_view" rules="all" width="1160" height="" cellspacing="0" cellpadding="0" border="0">
		<tbody>
			<?
			$i = 1;
			...
			?>
					<tr onclick="js_set_cutting_value('<?= $cut_id; ?>')" style="cursor:pointer" id="tr_1" height="" cellspacing="0" cellpadding="0" border="0" width="1140" bgcolor="<?= $bgcolor; ?>">
						<td width="40"><?= $i; ?></td>
						<td width="60"><?= $val['cut_no']; ?></td>
						<td width="60"><?= $val['year']; ?></td>
						<td width="60"><?= $val['order_cut_no']; ?></td>
						<td width="60"><?= $table_no_arr[$val['table_no']]; ?></td>
						<td width="70"><?= $val['job_no']; ?></td>
						<td width="100" style="word-break:break-all"><?= $val['style_ref_no']; ?></td>
						<td width="100" style="word-break:break-all"><?= $po_number; ?></td>
						<td width="100"><?= $buyer_library[$val['buyer_name']]; ?></td>
						<td width="100"><?= $color_arr[$val['color_id']]; ?></td>
						<td align="center" width="70"><?= $val['marker_qty']; ?></td>
						<td align="center" width="70"><?= $val['marker_length']; ?></td>
						<td align="center" width="70"><?= $val['marker_width']; ?></td>
						<td align="center" width="70" style="word-break:break-all"><?= $val['fabric_width']; ?></td>
						<td width="70" style="word-break:break-all"><?= $shift_name[$val['shift_name']]; ?></td>
						<td><?= change_date_format($val['entry_date']); ?></td>
					</tr>
			<?
					$i++;
				}
...
if ($action == "load_php_mst_form") {
	$sql_data = sql_select("SELECT b.id as   tbl_id,b.table_no,b.location_id,b.floor_id,a.id,a.job_no,a.company_id,a.working_company_id,a.entry_date,end_date,a.marker_length,a.marker_width,a.fabric_width,a.gsm,a.fabric_gsm,a.width_dia,a.cad_marker_cons,a.cutting_no,a.batch_id,a.start_time,a.end_time,a.other_fabric_weight,a.shift_name,a.cad_effi,a.piping_conj
	...
	foreach ($sql_data as $val) {
		...
		echo "document.getElementById('fabric_gsm').value  = '" . ($val[csf("fabric_gsm")]) . "';\n";
		...
		echo "document.getElementById('cad_efficiency').value  = '" . ($val[csf("cad_effi")]) . "';\n";
		echo "document.getElementById('piping_conj').value  = '" . ($val[csf("piping_conj")]) . "';\n";
		if ($db_type == 0) {
...
if ($action == "order_details_list") {
	// $sql_gmt_arr="select ";
	$tbl_row = 0;
	$sql_dtls = sql_select("SELECT a.id,a.order_ids,a.country_ids,a.ship_date,a.color_id,a.color_type_id,a.batch_id,a.gmt_item_id,a.plies,a.marker_qty,a.order_qty,a.total_lay_qty,a.lay_balance_qty,b.job_no,b.job_year,b.company_id, a.order_cut_no, a.roll_data,a.remarks from ppl_cut_lay_dtls a, ppl_cut_lay_mst b where b.id=a.mst_id and mst_id=" . $data . " and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.id");
	// echo $sql_dtls;
	...
	//  $po_arr = return_library_array("SELECT a.id,a.po_number  from wo_po_break_down a,gmts_production_confirmation b where a.id=b.po_id and b.confirm_status=1 and a.status_active=1 and b.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and job_no_mst='" . $sql_dtls[0][csf('job_no')] . "'", "id", "po_number");
	$is_control=return_field_value("is_control","variable_settings_production","company_name=$company_id and variable_list=33 and page_category_id=318");
	...
	{
		$po_arr = return_library_array("SELECT id,po_number  from wo_po_break_down where status_active=1  and is_deleted=0 and job_no_mst='" . $sql_dtls[0][csf('job_no')] . "'", "id", "po_number");
	}
	...
	foreach ($sql_dtls as $val) {
		if ($val[csf("country_ids")] != "") {
			$sql = "SELECT sum(plan_cut_qnty) as plan_qty from wo_po_color_size_breakdown where po_break_down_id in(" . $val[csf("order_ids")] . ") and country_id in(" . $val[csf("country_ids")] . ") and item_number_id=" . $val[csf("gmt_item_id")] . " and color_number_id=" . $val[csf("color_id")] . " and status_active=1 "; //group by po_break_down_id,item_number_id,color_number_id
		} else {
		...
		if ($val[csf("country_ids")] != "") {
			$sql_marker = "select sum(b.size_qty) as mark_qty from ppl_cut_lay_dtls a,ppl_cut_lay_bundle b where a.id=b.dtls_id and b.order_id in(" . $val[csf("order_ids")] . ") and b.country_id in(" . $val[csf("country_ids")] . ") and a.gmt_item_id=" . $val[csf("gmt_item_id")] . " and a.color_id=" . $val[csf("color_id")] . " and a.status_active=1";
		} else {
			$sql_marker = "select sum(b.size_qty) as mark_qty from ppl_cut_lay_dtls a, ppl_cut_lay_bundle b where a.id=b.dtls_id and b.order_id in(" . $val[csf("order_ids")] . ") and a.gmt_item_id=" . $val[csf("gmt_item_id")] . " and a.color_id=" . $val[csf("color_id")] . " and a.status_active=1";
		}
		...
		$country_ids = explode(",", $val[csf('country_ids')]);
		foreach ($country_ids as $cId) {
			$selected_country_name .= $country_arr[$cId] . ",";
		}
	...
	?>
		<tr class="" id="tr_<? echo $tbl_row; ?>" style="height:10px;">
			...
			<td align="center" id="country_<? echo $tbl_row; ?>">
				<input type="text" name="cboCountry_<? echo $tbl_row; ?>" id="cboCountry_<? echo $tbl_row; ?>" class="text_boxes" style="width:100px;" placeholder="Double Click to Search" onDblClick="openmypage_country(<? echo $tbl_row; ?>)" value="<? echo chop($selected_country_name, ','); ?>" readonly />
				<input type="hidden" name="countryId_<? echo $tbl_row; ?>" id="countryId_<? echo $tbl_row; ?>" value="<?= $val[csf('country_ids')]; ?>" />
			</td>
			...
			<td align="center">
				<input type="text" name="txtremarks_<? echo $tbl_row; ?>" id="txtremarks_<? echo $tbl_row; ?>" class="text_boxes" placeholder="Write" style="width:60px" value="<? echo $val['REMARKS']; ?>" />
			</td>
			<td width="70">
...
if ($action == "cut_lay_entry_report_print_bk") {
	...
	<div style="width:1100px; position:relative">
		...
		<div style=" top:80px; width:270; height:200px; position:absolute; left:0; ">
			<table border="1" cellspacing="0" width="260" class="rpt_table" rules="all">
				...
				<tr>
					<td>Internal Ref.</td>
					<td align="center"> <? echo $internal_ref_arr[0][csf('grouping')]; ?></td>
				</tr>
				<tr>
					<td>Season</td>
					<td align="center"> <? echo $season_library[$sql_buyer_arr[0][csf('season_buyer_wise')]]; ?></td>
				</tr>
				<tr>
					<td>Item Name</td>
		...
		<!-- <div  style="width:250; position:absolute; height:30px; top:118px; left:280px">
          <table border="1" cellpadding="1" cellspacing="1"   width="220"class="rpt_table" rules="all">
              <tr >
              <td width="170"> CAD Fabric Width/Dia</td><td width="50" align="center" colspan="2"><? // echo $fabric_with;
																									?></td>
             </tr>
          </table>
    </div>
   ...
   <div  style="width:250; position:absolute; height:30px; top:160px; left:280px">
      <table border="1" cellpadding="1" cellspacing="1"   width="220"class="rpt_table" rules="all">
          <tr >
          <td width="170">CAD GSM</td><td width="50" align="center" colspan="2"><? // echo $gsm;
																				?></td>
         </tr>
      </table>
    </div> -->
		...
		<div style=" width:270; position:absolute; top:275px;  ">
			...
			<div style=" float:right;width:190px;">
				...
				<table border="1" cellpadding="1" cellspacing="1" width="180" class="rpt_table" rules="all">
					<tr height="30">
						<td width="80">Efficiency</td>
						<td width="80" align="center"></td>
					</tr>
					<tr height="30">
						<td width="80">CAD Fabric Width/Dia</td>
						<td width="80" align="center"><? echo $fabric_with;  ?></td>
					</tr>
					<tr height="30">
			...
			<table>
				<tr>
					...
					<td width="10%" align="right" colspan="2">
						<div style=" width:200; height:60px;" id="barcode_img_id"> </div>
					</td>
				</tr>
			</table>
	...
	<div style=" width:1100px; position:relative;">
	 ...
	 order by a.id");
		...
		<table border="1" cellpadding="1" cellspacing="1" width="1100" class="rpt_table" rules="all">
			...
			?>
					...
					<tr height="20">
						...
						?>
							<td width="<? echo $td_width; ?>" align="center" title="<?= $dtls_id; ?>"><? echo $size_ratio_arr[$dtls_id][$size_id]; ?> </td>
						<?
							$tot_cad_ratio_arr[$size_id] +=$size_ratio_arr[$dtls_id][$size_id];
						}
						?>
						<td width="" align="right" valign="bottom"><? echo $total_size_ratio;
																	$total_size_ratio = 0;  ?></td>
					...
					<tr height="20">
						...
						foreach ($size_id_arr as $size_id => $size_val) 
						{
							...
							<?
								$tot_gtms_qnty[$size_id] +=  $detali_data_arr[$dtls_id][$plice_id][$size_id]['marker_qty'];
						}
						$tot_gmts += $total_gmt_qty_roll;
						?>
						<td width="" align="right" valign="bottom"><? echo $total_gmt_qty_roll;
						...
						foreach ($size_id_arr as $size_id => $size_val)
						 {
							...
							<?
								$total_bundel_qnty_arr[$size_id] += $bdl_qty;
						}
						?>
						<td width="" align="center"></td>
						<td width=""></td>
			...
			<?
					$i = $i + 1;
						$total_cad_ratio_qnty +=	$size_ratio_arr[$dtls_id][$size_id];
				}
			}
			?>
			
			<tr height="20">
						<th width="" rowspan="4"></th>
						<th width="" align="center" rowspan="4"></th>
						<th width="" align="center" rowspan="4"></th>
						<th width="" align="center" rowspan="4"></th>
						<th width="" align="center" rowspan="4" clospan="5">Total</th>
						<th width="">Size</th>

						<?
						foreach ($size_id_arr as $size_id => $size_val) {
						?>
							<th width="<? echo $td_width; ?>" align="center"><?=$size_arr[$detali_data_arr[$dtls_id][$plice_id][$size_id]['size']];;?></th>

						<?
						}
						?>
						<th width="" align="right" valign="bottom"></th>
					...
					<tr height="20">
						<th width="">CAD Ratio</th>
						<?
						foreach ($size_id_arr as $size_id => $size_val) {
							$total_size_ratio += $size_ratio_arr[$dtls_id][$size_id];
						?>
							<th width="<? echo $td_width; ?>" align="center" title="<?= $dtls_id; ?>"><?=$tot_cad_ratio_arr[$size_id];?></th>
						<?
						}
						?>
						<th width="" align="right" valign="bottom"></th>
					...
					<th width="">Gmts Qty.</th>
						...
						foreach ($size_id_arr as $size_id => $size_val) 
						{
							...
							?>
								<th width="<? echo $td_width; ?>" align="center" title="<?= $dtls_id; ?>"><?=$tot_gtms_qnty[$size_id]; ?> </th>
							<?
						}
						?>
						<th width="" align="right" valign="bottom"></th>
					...
					<th width="">Bundle Qty.</th>
						...
						foreach ($size_id_arr as $size_id => $size_val) 
						{
							$total_size_ratio += $size_ratio_arr[$dtls_id][$size_id];
							?>
								<th width="<? echo $td_width; ?>" align="center" title="<?= $dtls_id; ?>"><?=$total_bundel_qnty_arr[$size_id]; ?> </th>
							<?
						}
						?>
						<th width="" align="right" valign="bottom"></th>
...
if ($action == "cut_lay_entry_report_print3") {
	// extract($_REQUEST);
	$data = explode('*', $data);
//    print_r($data);
	$sql = sql_select("SELECT a.id,a.job_no,a.cut_num_prefix_no,a.table_no,a.marker_length,a.marker_width,a.fabric_width,a.gsm,a.fabric_gsm,a.width_dia,a.cad_marker_cons,a.batch_id,a.company_id,a.cad_effi,b.color_id,b.gmt_item_id from ppl_cut_lay_mst a,ppl_cut_lay_dtls b   where a.id=b.mst_id and cutting_no='" . $data[0] . "' ");
	// $sql="SELECT a.id,a.job_no,a.cut_num_prefix_no,a.table_no,a.marker_length,a.marker_width,a.fabric_width,a.gsm,a.fabric_gsm,a.width_dia,a.cad_marker_cons,a.batch_id,a.company_id,a.cad_effi,b.color_id,b.gmt_item_id from ppl_cut_lay_mst a,ppl_cut_lay_dtls b   where a.id=b.mst_id and cutting_no='" . $data[0] . "' ";
	//  echo $sql;
	foreach ($sql as $val) {
		$mst_id = $val[csf('id')];
		$company_id = $val[csf('company_id')];
	...
	// Booking_Effiency Query
	$sql_booking = sql_select("SELECT id,booking_type,is_short,booking_no_prefix,booking_no_prefix_num,booking_no,company_id,buyer_id,po_break_down_id,item_category,fabric_source,currency_id,exchange_rate,pay_mode,source,booking_date,delivery_date,booking_month,booking_year,cad_eff_percent from wo_booking_mst where job_no= '$job_no'");
	// echo $sql_booking;

	foreach ($sql_booking as $value) {
		$booking_effiency = $value[csf('cad_eff_percent')];
	}
...
?>
	<div style="width:1100px; position:relative">
		...
		<div style="width:270; position:relative;float: left;">
			<table border="1" cellspacing="0" width="260" class="rpt_table" rules="all">
				<tr>
					<td width="100">Buyer</td>
					<td width="180" align="left"><? echo $buyer_arr[$sql_buyer_arr[0][csf('buyer_name')]]; ?></td>
				</tr>
				<tr>
					<td>Job No</td>
					<td align="left"> <? echo $data[1]; ?></td>
				</tr>
				<tr>
					<td>Style</td>
					<td align="left"> <? echo $sql_buyer_arr[0][csf('style_ref_no')]; ?></td>
				</tr>
				<tr>
					<td>Internal Ref.</td>
					<td align="left"> <? echo $internal_ref_arr[0][csf('grouping')]; ?></td>
				</tr>
				<tr>
					<td>Season</td>
					<td align="left"> <? echo $season_library[$sql_buyer_arr[0][csf('season_buyer_wise')]]; ?></td>
				</tr>
				<tr>
					<td>Item Name</td>
					<td align="left"><? echo $garments_item[$item_name]; ?></td>
				</tr>
				<tr>
					<td>Order Qty</td>
					<td align="left"><? echo $order_qty; ?></td>
				</tr>
		...
		<div style="width:300; position:relative;float: left;padding-left: 3px;">
			<table border="1" cellpadding="1" cellspacing="1" width="300" class="rpt_table" rules="all">
				...
				<tr height="30">
					<td width="155" colspan="1">Manual/Order Cut No</td>

					<td width="155" colspan="1" align="center"> <? echo $order_cut_no; ?></td>
		...
		<div style=" width:300; position:relative;float: left;padding-left: 3px;">
			<table border="1" cellpadding="1" cellspacing="1" width="400" class="rpt_table" rules="all">
				<tr height="30">
		...
		<!-- <div style=" width:270; position:relative;float: left;margin-top: 5px;margin-bottom: 5px;">
		<div style=" float:left; text-align:center; margin-top:20px; width:80px;"></div>
	    <div style=" float:right;width:190px;"> -->

		<table border="1" cellpadding="1" cellspacing="1" width="490" class="rpt_table" rules="all">
			<tr>
				<td width="250" align="center" colspan="4" style="font-weight: bold;">STEP LAY DETAILS</td>
			</tr>
			<tr height="30">
				<td width="40">Booking Efficiency</td>
				<td width="40" align="center"><? echo $booking_effiency; ?></td>
				<td width="40">CAD Efficiency%</td>
				<td width="40" align="center"><? echo $cad_effi; ?></td>


			</tr>
			<tr height="30">
				<td width="80" colspan="2">CAD Fabric Width/Dia</td>
				<td width="80" colspan="2" align="center"><? echo $fabric_with;  ?></td>

			</tr>
			<tr height="30">
				<td width="80" colspan="2">CAD Marker Length</td>
				<td width="80" colspan="2" align="center"><? echo $marker_length;  ?></td>
			</tr>
			<tr height="30">
				<td colspan="2">CAD Marker Width</td>
				<td colspan="2" align="center"><? echo $marker_with;  ?></td>
			</tr>
			<tr height="30">
				<td colspan="2"> GSM</td>
				<td colspan="2" align="center"><? echo $gsm;  ?></td>
			</tr>
			<tr height="30">
				<td colspan="2">Fabric GSM</td>
				<td colspan="2" align="center"><? echo $fabric_gsm;  ?></td>
			</tr>
		</table>
		<!-- </div>
    </div>
		...
		<div style=" width:1100px; position:relative;">
			...
			// $sql_main_qry = sql_select("SELECT a.id,a.color_id,c.size_id,c.roll_no,sum(c.roll_wgt) as roll_weight,c.plies, sum(c.size_qty) as size_qty,c.roll_id
			// from  ppl_cut_lay_roll_dtls c,ppl_cut_lay_dtls a,ppl_cut_lay_mst b
			// where  a.id=c.dtls_id and b.id=a.mst_id  and b.job_no='$data[1]' and a.color_id='$color' and a.gmt_item_id='$item' and c.status_active=1  and  c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
			// group by a.id,a.color_id,c.size_id,c.roll_no,c.plies,c.roll_id
			// order by a.id");
			// echo $sql_sub_main_qry="SELECT a.id,a.color_id,c.size_id,c.roll_no,sum(c.roll_wgt) as roll_weight,c.plies, sum(c.size_qty) as size_qty,c.roll_id
			// from  ppl_cut_lay_roll_dtls c,ppl_cut_lay_dtls a,ppl_cut_lay_mst b
			// where  a.id=c.dtls_id and b.id=a.mst_id and b.job_no='$data[1]' and a.color_id='$color' and a.gmt_item_id='$item' and c.status_active=1  and  c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0
			// group by a.id,a.color_id,c.size_id,c.roll_no,c.plies,c.roll_id
			// order by a.id";
			$sql_main_qry = sql_select("SELECT a.id,a.color_id,d.size_id,c.roll_no,c.roll_used as roll_weight,c.plies, sum(d.size_qty) as size_qty,c.roll_id
			from ppl_cut_lay_dtls a,ppl_cut_lay_mst b, pro_roll_details c,ppl_cut_lay_roll_dtls d
			where  a.id=d.dtls_id and b.id=a.mst_id and b.id=c.mst_id and b.job_no='$data[1]' and a.color_id='$color' and a.gmt_item_id='$item' and c.entry_form=99 and c.status_active=1  and  c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0
			group by a.id,a.color_id,d.size_id,c.roll_no,c.plies,c.roll_id,c.roll_used
			order by a.id");
		    // echo $sql_main="SELECT a.id,a.color_id,d.size_id,c.roll_no,c.roll_used as roll_weight,c.plies, sum(d.size_qty) as size_qty,c.roll_id
			// from ppl_cut_lay_dtls a,ppl_cut_lay_mst b, pro_roll_details c,ppl_cut_lay_roll_dtls d
			// where  a.id=d.dtls_id and b.id=a.mst_id and b.id=c.mst_id and b.job_no='$data[1]' and a.color_id='$color' and a.gmt_item_id='$item' and c.entry_form=99 and c.status_active=1  and  c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0
			// group by a.id,a.color_id,d.size_id,c.roll_no,c.plies,c.roll_id,c.roll_used
			// order by a.id";
			...
			$sql_without_size = "Select a.job_no_mst,a.size_number_id,a.color_number_id from wo_po_color_size_breakdown a where a.job_no_mst='$data[1]' and a.status_active=1 and a.is_deleted=0";

			//  echo $sql_without_size;

			$sql_size_color = sql_select($sql_without_size);
			//  $size_id_array_one=array();
			$color_size_array = array();

			foreach ($sql_size_color as $value) {
				// $size_id_array_one[$value[csf('size_number_id')]]=$value[csf('size_number_id')];
				// $color_number_id_array_one[$value[csf('color_number_id')]]=$value[csf('color_number_id')];
				$color_size_array[$value[csf('color_number_id')]][$value[csf('size_number_id')]] = $value[csf('size_number_id')];
			}
			//  echo "<pre>";
			//  print_r($color_size_array);
			//  echo "</pre>";

			// $sql_roll_details="Select a.roll_no,a.plies,b.job_no from ppl_cut_lay_roll_dtls a,ppl_cut_lay_mst b where a.mst_id=b.id and a.mst_id=$mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
			// echo $sql_roll_details;
			// Without Size Start from here
	...
	else if($cad_marker_cons<$net_cons)
	{
		...
		$loss=number_format(abs($cons_balance),4);
			...
			<div style="width:890px; margin-top:110px;" align=center>
				<table border="1" cellpadding="1" cellspacing="1" width="870" class="rpt_table" rules="all">
					<thead>
						<tr height="30">
							<th width="100">SL</th>
							<th width="160" align="center">Batch No </th>
							<th width="160" align="center">Roll No </th>
							<th width="120" align="center">Roll Weight </th>
							<th width="100" align="center"> Lay Qty</th>
							<th width="130" align="center">Wastage Cut Pcs</th>
							<th width="100" align="center">Piping</th>
						</tr>
					</thead>
					<tbody>
