			...
			<table width="590" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" id="tbl_list_search">
				...
				<tbody>
					<tr id="tr_1" class="general">
						...
						<td>
						<input type="text" id="shade_1" name="shade[]" class="text_boxes" value="" maxlength="1" style="width:50px" onKeyDown="return /[a-z]/i.test(event.key)" oninput="this.value = this.value.toUpperCase()"/> 
						</td>
						<td>
...
if($action=="roll_popup_bk")//05-12-2022
{
  	echo load_html_head_contents("Plies Info Roll Wise","../../../", 1, 1, '','1','');
	extract($_REQUEST);
	$batch_selection_in_plies_popup= return_field_value("is_locked","variable_settings_production","company_name=$company_name and variable_list=38 and status_active=1 and is_deleted=0","is_locked");
					...
					<tr id="tr_1" class="general">
						...
						<?
							if($batch_selection_in_plies_popup==1) // when yes
							{
								...
								// echo "select quantity from order_wise_pro_details where po_breakdown_id in($order_no)  and entry_form=18 and trans_type=2 and is_deleted=0 and status_active=1";
								
								$is_ff_issue = return_field_value("quantity","order_wise_pro_details","po_breakdown_id in($order_no)  and entry_form=18 and trans_type=2 and is_deleted=0 and status_active=1");// and color_id=$color
								...
								if($is_ff_issue)
								{
									...
									// =========================================================
									/*$ff_rcv_batch_id_arr=return_library_array("SELECT a.BATCH_ID, a.BATCH_ID as ids from PRO_FINISH_FABRIC_RCV_DTLS a,ORDER_WISE_PRO_DETAILS b where a.id=b.dtls_id and b.entry_form=37 and a.status_active=1 and a.COLOR_ID=$color and b.po_breakdown_id in($order_no)",'BATCH_ID','ids');
									$ff_rcv_batch_ids = implode(",", $ff_rcv_batch_id_arr);

									$sql=sql_select("SELECT a.id, a.batch_no from pro_batch_create_mst a, pro_batch_create_dtls b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and a.id in($ff_rcv_batch_ids)  group by a.id, a.batch_no order by a.batch_no"); 
									$batch_arr = array();
									foreach ($sql as $val) 
									{
										$batch_arr[$val['ID']] = $val['BATCH_NO'];
									...
									// get transfer batch with different color 
									$sql=sql_select("SELECT a.id, a.batch_no from pro_batch_create_mst a, pro_batch_create_dtls b where a.id=b.mst_id and a.status_active=1 and b.status_active=1 and b.po_id in($order_no) and a.color_id!=$color and a.entry_form=14 group by a.id, a.batch_no order by a.batch_no"); // and a.color_id=$color
									foreach ($sql as $val) 
									{
										$batch_arr[$val['ID']] = $val['BATCH_NO'];
									...
									$sql=sql_select("SELECT a.id, a.batch_no from wo_booking_mst c,pro_batch_create_mst a, pro_batch_create_dtls b 
									where c.booking_no=a.booking_no and a.id=b.mst_id and a.status_active=1 and b.status_active=1 and c.job_no='".$job_no ."' and a.id in($issue_batch_ids)  group by a.id, a.batch_no"); // and a.color_id=$color
									/* echo "SELECT a.id, a.batch_no from wo_booking_mst c,pro_batch_create_mst a, pro_batch_create_dtls b 
									where c.booking_no=a.booking_no and a.id=b.mst_id and a.status_active=1 and b.status_active=1 and c.job_no='".$job_no ."' and a.id in($issue_batch_ids) and a.color_id=$color group by a.id, a.batch_no"; */
									foreach ($sql as $val) 
									{
										$batch_arr[$val['ID']] = $val['BATCH_NO'];
									}
								...
								{
									// get finish fabric issue batch
									$sql=sql_select("SELECT c.id, c.batch_no from inv_issue_master a, inv_finish_fabric_issue_dtls b, pro_batch_create_mst c, wo_booking_mst d	where a.id=b.mst_id and a.entry_form=18 and b.batch_id=c.id and c.booking_no=d.booking_no and b.status_active=1 and c.status_active=1 and d.job_no='".$job_no ."' group by c.id, c.batch_no"); // and a.color_id=$color
									$issue_batch_ids = "";
									foreach ($sql as $val) 
									{
										//$batch_arr[$val['ID']] = $val['BATCH_NO'];
	...
	<script>
		...
		{ 
			...
			if(operation==2)
			{
				...
				const btndis = (operation==0) ? document.getElementById('save1') : document.getElementById('update1');
			...
			http.open("POST","cut_and_lay_ratio_wise_entry_controller_urmi_for_qc.php",true);
			    ...
			    if(reponse[0]==0 || reponse[0]==1)
				 {
					...
					show_list_view(reponse[1]+'**'+reponse[2]+'**'+reponse[3]+'**'+reponse[7],'show_bundle_list_view','search_div','cut_and_lay_ratio_wise_entry_controller_urmi_for_qc','setFilterGrid("list_view",-1)');
		...
		function fnc_bundle_report(column_list)
		{
			...
			if(column_list==6)
			{
				var url=return_ajax_request_value(data, "print_report_bundle_barcode", "cut_and_lay_ratio_wise_entry_controller_urmi_for_qc");
				window.open(url,"##");
			}
			else
			{
				var url=return_ajax_request_value(data, "print_report_bundle_barcode_eight", "cut_and_lay_ratio_wise_entry_controller_urmi_for_qc");
				window.open(url,"##");	
			}
		...
		function fnc_bundle_report_eight()
		{
			...
			var page_link = 'cut_and_lay_ratio_wise_entry_controller_urmi_for_qc.php?data='+data+'&action=print_report_bundle_barcode_eight';
			...
			emailwindow.onclose=function()
			 {
				...
				data=data+'***'+prodID;
				var url=return_ajax_request_value(data, "print_barcode_eight", "cut_and_lay_ratio_wise_entry_controller_urmi_for_qc");
				window.open(url,"##");	
			 }
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				//var url=return_ajax_request_value(data, "print_barcode_one_pdf", "cut_and_lay_ratio_wise_entry_controller_urmi_for_qc");
				//window.open(url,"##");	
				window.open("cut_and_lay_ratio_wise_entry_controller_urmi_for_qc.php?data=" + data+'&action=print_barcode_one', true );
			 }
		...
		function fnc_bundle_report_one_urmi()
		{
			...
			var title = 'Search Job No';	
			var page_link = 'cut_and_lay_ratio_wise_entry_controller_urmi_for_qc.php?data='+data2+'&action=print_report_bundle_barcode_eight';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=270px,height=250px,center=1,resize=0,scrolling=0','../../');
			emailwindow.onclose=function()
			 {
				var theform=this.contentDoc.forms[0]; 
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				var url=return_ajax_request_value_post(data, "print_barcode_one_urmi", "cut_and_lay_ratio_wise_entry_controller_urmi_for_qc");
				window.open(url,"##");	
				//window.open("cut_and_lay_ratio_wise_entry_controller_urmi_for_qc.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
		...
		function fnc_bundle_report_one_urmi__()
		{
			var data="";
			var error=1;		
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				var url=return_ajax_request_value(data, "print_barcode_one_urmi", "cut_and_lay_ratio_wise_entry_controller_urmi_for_qc");
				window.open(url,"##");	
				//window.open("cut_and_lay_ratio_wise_entry_controller_urmi_for_qc.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
		...
		function fnc_bundle_report_qr_code()
		{
			...
			emailwindow.onclose=function()
			{
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				//var url=return_ajax_request_value(data, "print_barcode_operation", "cut_and_lay_ratio_wise_entry_controller_urmi_for_qc");
				http.open( 'POST', 'cut_and_lay_ratio_wise_entry_controller_urmi_for_qc.php?action=print_qrcode_operation&data='+ data );
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				var url=return_ajax_request_value(data, "print_barcode_one_128", "cut_and_lay_ratio_wise_entry_controller_urmi_for_qc");
				window.open(url,"##");	
				//window.open("cut_and_lay_ratio_wise_entry_controller_urmi_for_qc.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
		...
		function fnc_print_bundle(work_comp,work_location)
		{
			...
			var report_title="Cut and Lay bundle ";
		   	...
		   	var title = 'Search Job No';	
			var page_link = 'cut_and_lay_ratio_wise_entry_controller_urmi_for_qc.php?data='+data+'&action=print_report_bundle_list_popup';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=270px,height=250px,center=1,resize=0,scrolling=0','../../');
			emailwindow.onclose=function()
			 {
				var theform=this.contentDoc.forms[0]; 
				...
				print_report(<? echo $cbo_company_id; ?>+'*'+<? echo $mst_id; ?>+'*'+<? echo $details_id; ?>+'*'+report_title+'*'+country+'*'+bundle_use_for, "cut_lay_bundle_print", "cut_and_lay_ratio_wise_entry_controller_urmi_for_qc")
			 }
		...
		function fnc_send_printer_text()
		{
			...
			var job_id=$("#hidden_update_job_id").val();
			data=data+"***"+job_id+'***'+<? echo $mst_id; ?>+'***'+<? echo $details_id; ?>+'***'+<? echo $cbo_gmt_id; ?>+'***'+<? echo $cbo_color_id; ?>+'***'+<? echo $order_id; ?>;
			var url=return_ajax_request_value(data, "report_bundle_text_file", "cut_and_lay_ratio_wise_entry_controller_urmi_for_qc");
		...
		function fnc_cut_lay_bundle_reply_info()
		{
			if(http.readyState == 4) 
			{
				...
				if((reponse[0]==0 || reponse[0]==1))
				{
					...
					show_list_view(reponse[1]+'**'+reponse[2]+'**'+reponse[3],'show_bundle_list_view','search_div','cut_and_lay_ratio_wise_entry_controller_urmi_for_qc','setFilterGrid("list_view",-1)');
				}
				else if(reponse[0]==200)
				{
		...
		function fnc_rollWiseSizeQty()
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','cut_and_lay_ratio_wise_entry_controller_urmi_for_qc.php?rollData='+'<? echo $rollData; ?>'+'&size_data='+size_data+'&action=rollSize_popup','Roll Popup', 'width=680px,height=300px,center=1,resize=1,scrolling=0','../../')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
	...
	<div align="center" style="width:100%; overflow-y:hidden; position:absolute; top:5px;">
        ...
        <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            ...
            <?
				...
				$sql = sql_select('SELECT a.size_qty as SIZE_QTY, a.SIZE_ID from ppl_cut_lay_bundle a, ppl_cut_lay_dtls b where b.id=a.dtls_id and a.order_id in($order_id) and b.gmt_item_id=$cbo_gmt_id and b.color_id=$cbo_color_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0');
				foreach ($sql as $val) 
				{
							...
							foreach($po_val as $country_type_id=>$country_val)
							{
								foreach($country_val as $country_id=>$size_data)
								{
									...
									?>
										<tr id="gsd_<? echo $i; ?>">
											<td align="center">	
												 <input type="text" style="width:200px" class="text_boxes" name="poNo_<? echo $i; ?>" id="poNo_<? echo $i; ?>"  value="<? echo $po_no_arr[$po_id]; ?>" disabled />	
...
{	
	...
	$color_sizeID_arr=sql_select("SELECT a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,c.roll_data
	from ppl_cut_lay_dtls c, ppl_cut_lay_bundle a, ppl_cut_lay_size_dtls b where c.id=a.dtls_id and c.id=b.dtls_id and a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id $dynamic_cond
	group by a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,c.roll_data order by b.bundle_sequence,a.id");
	foreach($color_sizeID_arr as $row)
	{
		$test_data[$row[csf("roll_id")]]=$row[csf("roll_id")];
	}
	...
	unset($sql_name);
	$roll_id_cond= where_con_using_array($test_data,0,"id");
	$roll_sql=sql_select("select roll_id, batch_no,is_extra_roll from pro_roll_details where entry_form=99 and status_active=1 $roll_id_cond");
	$roll_data_arr=array();
	$batch_id_arr=array();
	foreach($roll_sql as $row)
	{
		$roll_data_arr[$row[csf("roll_id")]]=$row[csf("batch_no")];
		$roll_data_arr2[$row[csf("roll_id")]]=$row[csf("is_extra_roll")];
		$batch_id_arr[$row[csf("is_extra_roll")]]=$row[csf("is_extra_roll")];
	}
	if(count($batch_id_arr)>0)
	{
		// $batch_id_cond = where_con_using_array($batch_id_arr,0,"id");
		$batch_ids = implode(",",array_filter($batch_id_arr));
		$batch_library = return_library_array("SELECT id, batch_no FROM PRO_BATCH_CREATE_MST where status_active=1 and id in($batch_ids)",'id','batch_no');
	}
	...
	if(count($sql_bundle_copy)!=0)
	{
		foreach($sql_bundle_copy as $inf)
		{
			...
			foreach($color_sizeID_arr as $val)
			{
				...
				$batch_no=$roll_data_arr[$val[csf('roll_id')]];
				$bundle_no_prifix=$val[csf("bundle_num_prefix_no")];
				$batch_id=$roll_data_arr2[$val[csf('roll_id')]];
				// $roll_data_array = explode("**",$val[csf("roll_data")]);
				/* foreach ($roll_data_array as $v) 
				{
					$roll_data_array_new = explode("=",$v);
				} */
				...
				$pdf->Code40($i, $j+10.8, $inf[csf("bundle_use_for")]."  B# ".$batch_no.$batch_library[$batch_id]."  S# ".$lib_season_arr[$matrix_season], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide=true,true,7);
    	...
    	<table align="center" cellspacing="0" width="1250" border="1" rules="all" class="rpt_table" >
              ...
              <thead bgcolor="#dddddd" align="center">
                ...
                <?  
					...
					$batch_roll_arr=return_library_array( "select a.id, a.is_extra_roll as batch_id from pro_roll_details a where a.mst_id='$data[1]' and a.dtls_id='$data[2]' and a.entry_form=99","id","batch_id" );

					$batchId_arr=return_library_array( "select a.is_extra_roll, a.is_extra_roll as batch_id from pro_roll_details a where a.mst_id='$data[1]' and a.dtls_id='$data[2]' and a.entry_form=99","is_extra_roll","batch_id" );
					if(count($batchId_arr)>0)
					{
						$batch_ids = implode(",",array_filter($batchId_arr));
						$batch_library=return_library_array( "select id, batch_no from PRO_BATCH_CREATE_MST where status_active=1 and id in($batch_ids)","id","batch_no" );
					}
                     ...
                     foreach($size_data as $size_val)
                     {
                        ...
                        { 
                           ...
                           <tr>
                               ...
                               <td style="word-wrap:break-word"><? echo $batchNo_arr[$row[csf('roll_id')]].$batch_library[$batch_roll_arr[$row[csf('roll_id')]]]; ?></td>
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		$data_array="(".$id.",99,'".$new_cutting_prifix."',".$cut_no_prifix[0].",'".$new_cutting_number."',".$tbl_id.",".$txt_job_no.",".$txt_batch_no.",".$cbo_company_name.",".$cbo_working_company_name.",".$cbo_location_name.",".$cbo_floor_name.",".$txt_entry_date.",'".$start_time."',".$txt_end_date.",'".$end_time."',".$txt_marker_length.",".$txt_marker_width.",".$txt_fabric_width.",".$txt_gsm.",".$cbo_width_dia.",".$txt_marker_cons.",'".$user_id."','".$pc_date_time."',1,0)";
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	  {
			...
			//master table update*********************************************************************
			$field_array="table_no*job_no*batch_id*company_id*working_company_id*location_id*floor_id*entry_date*start_time*end_date*end_time*marker_length*marker_width*fabric_width*gsm*width_dia*cad_marker_cons*updated_by*update_date";
			$start_time=str_replace("'","",$txt_in_time_hours).":".str_replace("'","",$txt_in_time_minuties);
			$end_time=str_replace("'","",$txt_out_time_hours).":".str_replace("'","",$txt_out_time_minuties);
			$data_array="".$tbl_id."*".$txt_job_no."*".$txt_batch_no."*".$cbo_company_name."*".$cbo_working_company_name."*".$cbo_location_name."*".$cbo_floor_name."*".$txt_entry_date."*'".$start_time."'*".$txt_end_date."*'".$end_time."'*".$txt_marker_length."*".$txt_marker_width."*".$txt_fabric_width."*".$txt_gsm."*".$cbo_width_dia."*".$txt_marker_cons."*'".$user_id."'*'".$pc_date_time."'";
        ...
        <form name="searchprocessfrm_1"  id="searchprocessfrm_1" autocomplete="off">
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="390" class="rpt_table" >
                <thead>
                	<tr>
	                    <th width="40"></th>
	                    <th width="200"><input type="button" class="formbutton" value="With Full Shipment Po" onClick="show_list_view ( '<?php echo $poId; ?>' +'_'+'<?php echo $txt_job; ?>'+'_'+'<?php echo $cbocolor; ?>'+'_'+'<?php echo $gmt_id; ?>'+'_'+'<?php echo $process_row_id; ?>', 'create_list_view_with_full_shipment_po', 'buyer_list_view', 'cut_and_lay_ratio_wise_entry_controller_urmi_for_qc', 'setFilterGrid(\'tbl_list_search\',-1)')"></th>
	                    <th>
...
<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            ...
            <tbody>
                  ...
                  <td></td>                   
                        ...
                        <td align="left">
                               <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_cut_no').value+'_'+document.getElementById('txt_job_search').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_order_search').value, 'create_cutting_search_list_view', 'search_div', 'cut_and_lay_ratio_wise_entry_controller_urmi_for_qc', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
                        </td>                                          
                  <td></td> 
                 </tr>
        		 <tr>                  
...
{
    $sql_data=sql_select("select b.id as   tbl_id,b.table_no,b.location_id,b.floor_id,a.id,a.job_no,a.company_id,a.working_company_id,a.entry_date,end_date,a.marker_length,a.marker_width,a.fabric_width,a.gsm,a.width_dia,a.cad_marker_cons,a.cutting_no,a.batch_id,a.start_time,a.end_time
    ...
    foreach($sql_data as $val)
	  {
			...
			echo "load_drop_down( 'requires/cut_and_lay_ratio_wise_entry_controller_urmi_for_qc','".$val[csf("working_company_id")]."', 'load_drop_down_location', 'location_td') ;";
