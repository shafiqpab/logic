...
if ($action=="load_drop_down_order_qty")
{
	...
	$sql_marker="select sum(b.marker_qty) as mark_qty from  ppl_cont_cut_lay_dtls a, ppl_cont_cut_lay_size b where a.id=b.dtls_id and b.order_id in(".$ex_data[0].") and a.gmt_item_id=".$ex_data[1]." and a.color_id=".$ex_data[2]." and a.status_active=1 and b.status_active=1 and b.is_deleted=0";
...
if($action=="check_batch_no")
{
	...
	$barcodeData=sql_select( "select barcode_no from pro_roll_details where entry_form=586 and status_active=1 and is_deleted=0");
...
?>
	<script>
		...
		function openmypage_batch()
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','contrast_cutting_entry_controller.php?order_no='+<? echo $order_no; ?>+'&color='+<? echo $color; ?>+'&added_barcode_no='+added_barcode_no+'&action=batch_popup','Batch Barcode Popup', 'width=580px,height=300px,center=1,resize=1,scrolling=0','../../')
		...
		$('#txt_batch_no').live('keydown', function(e) {
			if (e.keyCode === 13)
			{
				...
				var response_data=return_global_ajax_value( batch_no+"**"+order_id+"**"+added_barcode_no, 'check_batch_no', '', 'contrast_cutting_entry_controller');
		...
		function openmypage_barcode()
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','contrast_cutting_entry_controller.php?order_no='+<? echo $order_no; ?>+'&color='+<? echo $color; ?>+'&action=barcode_popup','Barcode Popup', 'width=480px,height=300px,center=1,resize=1,scrolling=0','../../')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
				var barcode_nos=this.contentDoc.getElementById("hidden_barcode_nos").value; //Barcode Nos
...
if($action=="size_popup")
{
	...
	$print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$cbo_company_id."' and module_id=4 and report_id=246 and is_deleted=0 and status_active=1");
	...
	<script>
		...
		function fnc_cut_lay_size_info( operation )
		{
			...
			http.open("POST","contrast_cutting_entry_controller.php",true);
			    ...
			    if(reponse[0]==0 || reponse[0]==1)
				 {
					...
					show_list_view(reponse[1]+'**'+reponse[2]+'**'+reponse[3]+'**'+reponse[7],'show_bundle_list_view','search_div','contrast_cutting_entry_controller','setFilterGrid("list_view",-1)');
		...
		function fnc_bundle_report(column_list)
		{
			...
			if(column_list==6)
			{
				var url=return_ajax_request_value(data, "print_report_bundle_barcode", "contrast_cutting_entry_controller");
				window.open(url,"##");
			}
			else
			{
				var url=return_ajax_request_value(data, "print_report_bundle_barcode_eight", "contrast_cutting_entry_controller");
				window.open(url,"##");
			}
		...
		function fnc_bundle_report_eight()
		{
			...
			var page_link = 'contrast_cutting_entry_controller.php?data='+data2+'&action=print_report_bundle_barcode_eight';
			...
			emailwindow.onclose=function()
			{
				...
				data=data+'***'+prodID;
				var url=return_ajax_request_value_post(data, "print_barcode_eight", "contrast_cutting_entry_controller");
				window.open(url,"##");
			}
		...
		function fnc_bundle_report_nine()
		{
			...
			var title = 'Search Job No';
			var page_link = 'contrast_cutting_entry_controller.php?data='+data2+'&action=print_report_bundle_barcode_nine';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=270px,height=250px,center=1,resize=0,scrolling=0','../../');
			emailwindow.onclose=function()
			 {
				var theform=this.contentDoc.forms[0];
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				var url=return_ajax_request_value_post(data, "print_barcode_nine", "contrast_cutting_entry_controller");
				window.open(url,"##");
			 }
		...
		function fnc_bundle_report_one()
		{
			...
			var title = 'Search Job No';
			var page_link = 'contrast_cutting_entry_controller.php?data='+data+'&action=print_report_bundle_barcode_eight';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=270px,height=250px,center=1,resize=0,scrolling=0','../../');
			emailwindow.onclose=function()
			 {
				var theform=this.contentDoc.forms[0];
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				//var url=return_ajax_request_value(data, "print_barcode_one_pdf", "contrast_cutting_entry_controller");
				//window.open(url,"##");
				window.open("contrast_cutting_entry_controller.php?data=" + data+'&action=print_barcode_one', true );
			 }
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				var url=return_ajax_request_value(data, "print_barcode_one_urmi", "contrast_cutting_entry_controller");
				window.open(url,"##");
				//window.open("contrast_cutting_entry_controller.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				var url=return_ajax_request_value(data, "print_barcode_ten_urmi", "contrast_cutting_entry_controller");
				window.open(url,"##");
				//window.open("contrast_cutting_entry_controller.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				var url=return_ajax_request_value(data, "print_barcode_ten_youth", "contrast_cutting_entry_controller");
				window.open(url,"##");
				//window.open("contrast_cutting_entry_controller.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				var url=return_ajax_request_value(data, "print_barcode_ten_youth_new", "contrast_cutting_entry_controller");
				window.open(url,"##");
				//window.open("contrast_cutting_entry_controller.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
		...
		function fnc_bundle_report_qr_code()
		{
			...
			emailwindow.onclose=function()
			{
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				//var url=return_ajax_request_value(data, "print_barcode_operation", "contrast_cutting_entry_controller");
				http.open( 'POST', 'contrast_cutting_entry_controller.php?action=print_qrcode_operation&data='+ data );
		...
		function fnc_bundle_report_qr_code3()
		{
			var data="";
			var error=1;
			$("input[name=chkbundle]").each(function(index, element) {
			...
			emailwindow.onclose=function()
			{
				...
				data=data+'***'+prodID;
				//var url=return_ajax_request_value(data, "print_barcode_operation", "contrast_cutting_entry_controller");
				http.open( 'POST', 'contrast_cutting_entry_controller.php?action=print_qrcode_operation_3&data='+ data+'***'+'<? echo $cont_color; ?>' );
        ...
        function fnc_bundle_report_qr_code1()
        {
            ...
            emailwindow.onclose=function()
            {
                ...
                data=data+'***'+prodID;
                //var url=return_ajax_request_value(data, "print_barcode_operation", "contrast_cutting_entry_controller");
                http.open( 'POST', 'contrast_cutting_entry_controller.php?action=print_qrcode_operation1&data='+ data );
                http.onreadystatechange = response_pdf_data;
                http.send(null);
            }
        }
		...
		function fnc_bundle_report_qr_code24()
        {
            var data="";
            var error=1;
            $("input[name=chkbundle]").each(function(index, element) {
            ...
            emailwindow.onclose=function()
            {
                ...
                data=data+'***'+prodID;
                //var url=return_ajax_request_value(data, "print_barcode_operation", "contrast_cutting_entry_controller");
                http.open( 'POST', 'contrast_cutting_entry_controller.php?action=print_barcode_24_per_page&data='+ data );
                http.onreadystatechange = response_pdf_data24;
                http.send(null);
            }
        }

		function response_pdf_data24()
		{
			if(http.readyState == 4)
			{
				var response = http.responseText.split('###');
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				var url=return_ajax_request_value(data, "print_barcode_one_128", "contrast_cutting_entry_controller");
				window.open(url,"##");
				//window.open("contrast_cutting_entry_controller.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
			...
			emailwindow.onclose=function()
			 {
				...
				var prodID=this.contentDoc.getElementById("txt_selected_id").value;
				data=data+'***'+prodID;
				//var url=return_ajax_request_value(data, "print_barcode_one_128_v2", "contrast_cutting_entry_controller");
				var url=return_ajax_request_value(data, action, "contrast_cutting_entry_controller");
				window.open(url,"##");
				//window.open("contrast_cutting_entry_controller.php?data=" + data+'&action=print_barcode_one_urmi', true );
			 }
		...
		function fnc_print_bundle(work_comp,work_location)
		{
			...
			var report_title="Cut and Lay bundle ";
		   	...
		   	var title = 'Search Job No';
			var page_link = 'contrast_cutting_entry_controller.php?data='+data+'&action=print_report_bundle_list_popup';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=270px,height=250px,center=1,resize=0,scrolling=0','../../');
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0];
				...
				print_report(<? echo $cbo_company_id; ?>+'*'+<? echo $mst_id; ?>+'*'+<? echo $details_id; ?>+'*'+report_title+'*'+country+'*'+bundle_use_for, "cut_lay_bundle_print", "contrast_cutting_entry_controller")
			}
		}
		...
		function fnc_print_bundle_list_tow(work_comp,work_location)
		{
			...
			emailwindow.onclose=function()
			{
				...
				print_report(<? echo $cbo_company_id; ?>+'*'+<? echo $mst_id; ?>+'*'+<? echo $details_id; ?>+'*'+report_title+'*'+country+'*'+bundle_use_for, "cut_lay_bundle_list_print2", "contrast_cutting_entry_controller")
			}
		}
		...
		function fnc_send_printer_text()
		{
			...
			var job_id=$("#hidden_update_job_id").val();
			data=data+"***"+job_id+'***'+<? echo $mst_id; ?>+'***'+<? echo $details_id; ?>+'***'+<? echo $cbo_gmt_id; ?>+'***'+<? echo $cbo_color_id; ?>+'***'+<? echo $order_id; ?>;
			var url=return_ajax_request_value(data, "report_bundle_text_file", "contrast_cutting_entry_controller");
		...
		function fnc_cut_lay_bundle_reply_info()
		{
			if(http.readyState == 4)
			{
				...
				if((reponse[0]==0 || reponse[0]==1))
				{
					...
					show_list_view(reponse[1]+'**'+reponse[2]+'**'+reponse[3],'show_bundle_list_view','search_div','contrast_cutting_entry_controller','setFilterGrid("list_view",-1)');
				}
				else if(reponse[0]==200)
				{
		...
		function fnc_rollWiseSizeQty()
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','contrast_cutting_entry_controller.php?rollData='+'<? echo $rollData; ?>'+'&size_data='+size_data+'&action=rollSize_popup','Roll Popup', 'width=680px,height=300px,center=1,resize=1,scrolling=0','../../')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
		...
		function fnc_print_qc_bundle(type)
		{
		   ...
		   var country=$('#cboCountryBundle').val();
		   print_report(<? echo $cbo_company_id; ?>+'*'+<? echo $mst_id; ?>+'*'+<? echo $details_id; ?>+'*'+report_title+'*'+country+'*'+type, "cut_lay_qc_bundle_print", "contrast_cutting_entry_controller")
		}

		function fnc_bundle_report_one_urmi_v2(type) { //Copied from platform-v3.5\prod_planning\cutting_plan\requires\cut_and_lay_ratio_wise_entry_controller_urmi.php
			var data = "";
			var error = 1;
			$("input[name=chkbundle]").each(function(index, element) {
				if ($(this).prop('checked') == true) {
					...
					var idd = $(this).attr('id').split("_");
					//console.log(idd);
					if (data == "") data = $('#hiddenid_' + idd[1]).val();
					else data = data + "," + $('#hiddenid_' + idd[1]).val();
				}
			});
			//alert(data);return;
			if (error == 1) {
			...
			var page_link = 'contrast_cutting_entry_controller.php?data=' + data + bundle_source + '&action=' + body_part_action;
			emailwindow = dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=270px,height=250px,center=1,resize=0,scrolling=0', '../../');
			emailwindow.onclose = function() {
				var theform = this.contentDoc.forms[0];
...
<div id="msg_box_popp" style=" height:15px; width:200px;  position:relative; left:250px "></div>
	<div align="center" style="width:100%; overflow-y:hidden; position:absolute; top:5px;">
		...
		<?
			...
			$pcs_per_bundle=return_field_value("pcs_per_bundle","ppl_cont_cut_lay_dtls","id=$details_id ","pcs_per_bundle");
		?>
        <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            ...
            <fieldset style="width:450px;">
                ...
                $size_wise_arr=array();
                $sizeWiseData=sql_select("SELECT size_ratio, size_id, marker_qty, bundle_sequence from ppl_cont_cut_lay_size_dtls where mst_id=".$mst_id." and dtls_id=".$details_id." and status_active=1");
                ...
                $sizeDaraArr=array();
                $sizeData=sql_select("SELECT a.id, a.size_ratio, a.size_id, a.marker_qty, a.bundle_sequence, a.order_id, a.country_type, a.country_id, a.excess_perc from ppl_cont_cut_lay_size a where a.mst_id=".$mst_id." and a.dtls_id=".$details_id." and a.status_active=1");
                if(count($sizeData)>0)
                {
                    $is_update=1;
				...
				$size_bl_qty_arr=return_library_array("SELECT sum(a.size_qty) as size_qty, a.size_id from ppl_cont_cut_lay_bundle a, ppl_cont_cut_lay_dtls b where b.id=a.dtls_id and a.order_id in($order_id) and b.gmt_item_id=$cbo_gmt_id and b.color_id=$cbo_color_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.size_id",'size_id','size_qty');
                        ...
                        <table>
                            ...
                            <tr>
                                <td align="center" colspan="7" >
                                    ...
                                    <?
								    $print_report_format_arr=return_library_array( "select template_name, format_id from lib_report_template where  module_id=4 and report_id=246 and is_deleted=0 and status_active=1 and template_name=$cbo_company_id", "template_name", "format_id");
                                    ...
                                    foreach($report_id as $res){
                                        ...
                                        <?}elseif($res==784){?>
                                            <input type="button" id="btn_bundle_qr_code24" name="btn_bundle_qr_code24" value="Bundle Sticker 24/Page" class="formbutton" onClick="fnc_bundle_report_qr_code24()"/>
                                        <?}elseif($res==787){?>
                                            <input type="button" id="btn13" name="btn13" value="QR Code Sticker" class="formbutton" onClick="fnc_bundle_report_qr_code1();"/>
										<?}elseif($res==429){?>
                                            <input type="button" id="btn_bundle_qr_code" name="btn_bundle_qr_code" value="QRCode 75x50 2" class="formbutton" onClick="fnc_bundle_report_qr_code3()" />
                                        <?}
                                    }
									?>
        ...
        <form name="searchorderfrm_2"  id="searchorderfrm_2" autocomplete="off">
            ...
            <div id="search_div" style="margin-top:10px">
                <?
				$sql_size_name=sql_select("select size_id from ppl_cont_cut_lay_size where mst_id=".$mst_id." and dtls_id=".$details_id." and status_active=1 and is_deleted=0");
...
if ($action == "print_barcode_five_per_page") {
	...
	$mst_id = $data[2];
	$detls_id = $data[3];
	$batch_id = return_field_value("batch_id", "ppl_cont_cut_lay_dtls", "id=$detls_id");
	...
	$order_cut_no = return_field_value("order_cut_no", "ppl_cont_cut_lay_dtls", "id=$detls_id and status_active=1 and is_deleted=0", "order_cut_no");
	...
	from ppl_cont_cut_lay_mst c, ppl_cont_cut_lay_bundle a, ppl_cont_cut_lay_size_dtls b,ppl_cont_cut_lay_dtls d where c.id=a.mst_id and a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id and c.id=d.mst_id and d.id=a.dtls_id and a.status_active=1 and b.status_active=1 and c.status_active=1 $dynamic_cond
	...
	$roll_sql =sql_select ("SELECT id, batch_no from pro_roll_details where entry_form=586 and status_active=1 and mst_id='$mst_id' and dtls_id='$detls_id'");
	// echo $roll_sql;die;
	$roll_data_arr = array();
	...
	unset($roll_sql);
	$sql_cut_name = sql_select("SELECT entry_date,table_no,cut_num_prefix_no,batch_id,cutting_no, company_id from ppl_cont_cut_lay_mst where id=$data[2]");
	foreach ($sql_cut_name as $cut_value) {
		$table_name = $cut_value[csf('table_no')];
	...
	if (count($sql_bundle_copy) != 0) {
		foreach ($sql_bundle_copy as $inf) {
			...
			foreach ($color_sizeID_arr as $val) {
				...
				$pdf->Code40($i + 112, $j + 28, "Batch/Lot",$batch_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 9);
...
if ($action=="save_update_delete_bundle")
{
	...
	if ($operation==1)  // Insert Here=======================================================
	{
		...
		$previous_barcode_data=sql_select("select bundle_no,barcode_no,barcode_year,barcode_prifix from ppl_cont_cut_lay_bundle where mst_id=".$bundle_mst_id."  and  dtls_id=".$bundle_dtls_id." and status_active=1 and is_deleted=0 ");
		...
		$id=return_next_id("id","ppl_cont_cut_lay_bundle",1);
		...
		$barcode_suffix_no=return_field_value("max(barcode_prifix) as suffix_no","ppl_cont_cut_lay_bundle","barcode_year=$year_id","suffix_no");
		...
		for($j=1;$j<=$tot_row;$j++)
		{
			...
			if(empty($previous_barcode_arr[str_replace("'","",$$new_bundle_no)]))
			{
				...
				$barcode_no=$year_id."586".str_pad($barcode_suffix_no,10,"0",STR_PAD_LEFT);
			}
			else
			{
		...
		//echo $data_array;die;
		//echo "10**insert into ppl_cont_cut_lay_bundle($field_array) values".$data_array;die;
		$rID=execute_query("delete from ppl_cont_cut_lay_bundle where mst_id=".$bundle_mst_id." and dtls_id=".$bundle_dtls_id."",0);
		$rID1=sql_insert("ppl_cont_cut_lay_bundle",$field_array,$data_array,1);
...
if($action=="report_bundle_printer")
{
	...
	$color_library=return_library_array( "select id, color_name from lib_color", "id", "color_name");
	$table_no_library=return_library_array( "select id,table_no  from  lib_cutting_table", "id", "table_no"  );
	$color_sizeID_arr=sql_select( "select id,size_id,bundle_no,number_start,number_end,size_qty from ppl_cont_cut_lay_bundle
	                              where mst_id=$data[1] and dtls_id=$data[2] order by id" );  //where id in ($data)
	$color_sizeID_arr=sql_select( "select id,size_id,bundle_no,number_start,number_end,size_qty from ppl_cont_cut_lay_bundle where id in ( $data[0] ) " );  //where id in ($data)
    ...
    foreach($sql_name as $value)
	 ...
	 $sql_cut_name=sql_select("select entry_date,table_no,cut_num_prefix_no,batch_id,company_id from ppl_cont_cut_lay_mst where id=$data[1]");
	 foreach($sql_cut_name as $cut_value)
	 {
	 ...
	 $bundle_calculate_id=return_next_id("id", "ppl_cont_cut_lay_bundle_history",1);
	 ...
	 foreach($color_sizeID_arr as $val)
		...
		// echo $data_array;die;
		 $rID=sql_insert("ppl_cont_cut_lay_bundle_history",$field_array,$data_array,1);
...
if($action=="report_bundle_text_file")
{
    ...
    $bundle_array=array();
	 ...
	 $sql_cut_name=sql_select("select entry_date,table_no,cut_num_prefix_no,batch_id,company_id from ppl_cont_cut_lay_mst where id=$data[2]");
	 foreach($sql_cut_name as $cut_value)
	 {
		 $table_name=$cut_value[csf('table_no')];
	     $cut_date=change_date_format($cut_value[csf('entry_date')]);
    	...
    	<table align="center" cellspacing="0" width="<?=$tblwidth-20; ?>" border="1" rules="all" class="rpt_table" >
            ...
            <tbody>
                <?
					 ...
					 $sqlRoll=sql_select("select a.id, a.batch_no, a.shade from pro_roll_details a, ppl_cont_cut_lay_bundle b, ppl_cont_cut_lay_mst c where a.id=b.roll_id and a.mst_id='$data[1]' and a.dtls_id='$data[2]' and a.entry_form=c.entry_form");
					//  var_dump($sqlRoll);
                     ...
                     $size_data=sql_select("select a.id,a.size_id,a.bundle_sequence,a.marker_qty from ppl_cont_cut_lay_size_dtls a where a.mst_id='$data[1]' and a.dtls_id='$data[2]'  order by a.bundle_sequence ASC");
                     $j=1;
                     foreach($size_data as $size_val)
                     {
						$total_marker_qty_size=0;
						   $bundle_data=sql_select("select a.id,a.bundle_no as bundle_no,a.bundle_num_prefix_no,a.size_id,a.number_start,a.number_end,a.size_qty, a.country_id, a.roll_no, a.roll_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,a.barcode_no from ppl_cont_cut_lay_bundle a where a.mst_id='$data[1]' and a.dtls_id='$data[2]' and a.size_id=".$size_val[csf('size_id')]." $country_cond order by a.id ASC");
					    //var_dump ($bundle_data);
						//echo "select a.id,a.bundle_no as bundle_no,a.bundle_num_prefix_no,a.size_id,a.number_start,a.number_end,a.size_qty, a.country_id, a.roll_no, a.roll_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no from ppl_cont_cut_lay_bundle a where a.mst_id='$data[1]' and a.dtls_id='$data[2]' and a.size_id=".$size_val[csf('size_id')]." $country_cond order by a.id ASC";
                        foreach($bundle_data as $row)
                        {
               	 			?>
                           <tr>
...
if($action=="print_qrcode_operation")
{
	...
	$color_sizeID_arr=sql_select("select
										...
										b.color_id
									from
										ppl_cont_cut_lay_bundle a,
										ppl_cont_cut_lay_size_dtls b
									where
...
if($action=="print_qrcode_operation_3") //
{
	...
	$batch_arr=return_library_array( "SELECT id, batch_no from PRO_ROLL_DETAILS where mst_id=$data[2] and entry_form=586",'id','batch_no');


	$color_sizeID_arr=sql_select("SELECT a.id,a.size_id,a.bundle_no,a.barcode_no,a.order_id,a.number_start,a.number_end,a.size_qty,a.country_id,a.pattern_no,a.roll_no,b.bundle_sequence,b.color_id,a.roll_id
	from ppl_cont_cut_lay_bundle a, ppl_cont_cut_lay_size_dtls b
	...
	from ppl_cont_cut_lay_mst a, ppl_cont_cut_lay_dtls b
			...
			foreach($sql_bundle_copy as $inf)
			{
				...
				$mpdf->AddPage('',    // mode - default ''
					        	...
					        	<tr >
									...
									<td  width="15%" valign="top" >
										<table  width="100%" border="0" cellpadding="0" cellspacing="0" style="margin-top:7px;">
											<tr >
												<td > <div style="font-size:9px;"> PG:'.$i.'</div></td>
											</tr>
										</table>
									</td>
								</tr>
					...
					<table width="100%" cellpadding="0" cellspacing="0" width="" class="" style="font-size:10px; font-weight:bold;margin:0px" rules="all" id="" border="1" align="center">
						...
						<tr>
							<td width="62%">Cont. Cut Qty: ('.$total_cut_qty.')</td>
	...
	/* if(!function_exists('fn_qrcode_images_delete'))
	{
		function fn_qrcode_images_delete($dir) {
			foreach(glob($dir . '/*') as $file) {
				if(is_dir($file))
					fn_qrcode_images_delete($file);
				else
					unlink($file);
			}
			rmdir($dir);
	...
	fn_qrcode_images_delete("qrcode_image/".$ful_cut_no); */
	exit();
...
if($action=="print_qrcode_operation1")
{
    ...
    $total_cut_qty_int=return_field_value("sum(a.size_qty) as total_qty", "ppl_cont_cut_lay_bundle a, ppl_cont_cut_lay_size_dtls b", "a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id and a.mst_id in ($data[2])", "total_qty");
    ...
    $sql_cut_name=sql_select("SELECT a.entry_date, a.table_no, a.cut_num_prefix_no, a.batch_id, a.company_id, a.cutting_no, a.shipment_part,b.roll_data  from ppl_cont_cut_lay_mst a, ppl_cont_cut_lay_dtls b  where a.id = b.mst_id and a.id=$data[2]");
    ...
    $roll_data_arr=return_library_array("select roll_id, (batch_no || '-' || shade) as batch_shade from pro_roll_details where entry_form=586 and status_active=1", "roll_id", "batch_shade");
    ...
    $mpdf = new mPDF('',    // mode - default ''
        array(190,236),		// array(65,210),    // format - A4, for example, default ''
        5,     // font size - default 0
        '',    // default font family
        3,    // margin_left
        3,    // margin right
            ...
            foreach(array_chunk($bundle_arr_main, 8) as $breakData)
            {
                ...
                foreach (array_chunk($breakData, 2) as $rowData) {
                    ...
                    foreach ($rowData as $val) {
                        ...
                        $html .= '<td style="border:none; padding-bottom:53px;padding-top:12px;"><table cellpadding="0" cellspacing="0" width="321" height="184" class="" style="font-weight:normal;margin:0 auto;font-size:9px;" rules="all" id="" border="1" align="left">
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td style="padding: 1.5px 0px 1.5px 1px;font-weight:bold;" width="50%" >PO NO# ' . substr($po_number, 0, 16) . ' </td>
                                </tr>
                                <tr>
                                    <td style="padding: 1.5px 0px 1.5px 1px;font-weight:bold;" width="50%">STY# ' . substr($style_name, 0, 17) . ' </td>
                                    <td style="padding: 1.5px 0px 1.5px 1px;font-weight:bold;">BODY PART# ' . substr($val["bundle_use_for"], 0, 15) . '</td>
                                </tr>
                                <tr>
                                    <td style="padding: 1.5px 0px 1.5px 1px;font-weight:bold;">COLOR# ' . substr($val["color"], 0, 16) . '</td>
                                    <td style="padding: 1.5px 0px 1.5px 1px;font-weight:bold;" width="">CUT NO# ' . substr($order_cut_no, 0, 16) . '</td>
                                </tr>
                                <tr>
                                    <td style="padding: 1.5px 0px 1.5px 1px;font-weight:bold;">GROUP# ' . substr($val["pattern_no"], 0, 16) . '</td>
                                ...
                                <tr>
                                    ...
                                    <td style="padding: 1.5px 0px 1.5px 1px;font-weight:bold;">PART# ' . substr($shipment_part, 0, 17) . '</td>
                                </tr>
                                <tr>
                                    <td style="padding: 1.5px 0px 1.5px 1px;font-weight:bold;">BUNDLE PCS# ' . $val["size_qty"] . '</td>
                                    <td style="padding: 1.5px 0px 1.5px 1px;font-weight:bold;" width="" rowspan="4" align="center"><div id="div_' . $i . '"><img src="' . $PNG_WEB_DIR . basename($filename) . '" height="60" width="" ></div></td>
                                </tr>
                                <tr>
                                    <td style="padding: 1.5px 0px 1.5px 1px;font-weight:bold;">SERIAL NO# ' . substr($val["number_start"] . '-' . $val["number_end"], 0, 16) . '</td>
                                </tr>
                                <tr>
                                    <td style="padding: 1.5px 0px 1.5px 1px;font-weight:bold;">BUNDLE# ' . substr($val['bundle_no'], 0, 16) . '</td>
                                </tr>
                                <tr>
...
if($action=="print_qrcode_operation13")
{
    ...
    $sql_cut_name=sql_select("SELECT entry_date, table_no, cut_num_prefix_no, batch_id, company_id,cutting_no, shipment_part  from ppl_cont_cut_lay_mst where id=$data[2]");
...
if($action=="print_barcode_eight")
{
	...
	$roll_sql=sql_select("select roll_id, batch_no, shade from pro_roll_details where mst_id='$data[2]' and entry_form=586 and status_active=1");
	$roll_data_arr=array();
	foreach($roll_sql as $row)
	{
...
if($action=="print_barcode_nine")
{
	...
	$bundle_id_arr = explode(",", $data[0]);
	$bundle_id_cond = where_con_using_array($bundle_id_arr,0,"a.id");
	$color_sizeID_arr=sql_select("SELECT a.id,a.size_id,a.bundle_no,a.number_start,a.number_end,a.size_qty,a.country_id,a.roll_id,a.roll_no,b.bundle_sequence, a.pattern_no,a.barcode_no,c.con_color_name
	from ppl_cont_cut_lay_bundle  a,ppl_cont_cut_lay_size_dtls b,ppl_cont_cut_lay_dtls c
	where a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id  and  a.mst_id=c.mst_id $bundle_id_cond and a.mst_id=$data[2] order by b.bundle_sequence,a.id");
	...
	$sql_name=sql_select("SELECT b.buyer_name,b.style_ref_no,b.product_dept,a.po_number,a.grouping from wo_po_details_master b,wo_po_break_down a where a.job_no_mst=b.job_no and a.id=$data[6]");
	//echo "<pre> $sql_name";
	foreach($sql_name as $value)
	{
		$product_dept_name=$value[csf('product_dept')];
		...
		$buyer_name=$value[csf('buyer_name')];
		$po_number=$value[csf('po_number')];
		$internal_ref_no=$value[csf('grouping')];
	}

	$sql_cut_name=sql_select("SELECT entry_date,table_no,cut_num_prefix_no,batch_id,company_id,remarks from ppl_cont_cut_lay_mst where id=$data[2]");
	foreach($sql_cut_name as $cut_value)
	{
		 $table_name=$cut_value[csf('table_no')];
	     $cut_date=change_date_format($cut_value[csf('entry_date')]);
			...
			foreach( $sql_bundle_copy as $inf)
			{
				...
				$z=$z+25;
						...
						$pdf->Code39($i+165, $j-0,  $inf[csf("bundle_use_for")], $ext = true, $cks = false, $w = 0.4, $h = 3, $wide = true, true,8) ;
							...
							$pdf->Code39($i+60, $j-(0.-0005), $val[csf("con_color_name")], $ext = true, $cks = false, $w = 0.2, $h = 10, $wide = true, true,7) ;
...
if($action=="print_barcode_one_urmi_real")
{
	...
	from ppl_cont_cut_lay_bundle a, ppl_cont_cut_lay_size_dtls b where a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id and a.id in ($data[0]) order by b.bundle_sequence,a.id");
	...
	$roll_sql=sql_select("select roll_id, batch_no from pro_roll_details where entry_form=586 and status_active=1");
	$roll_data_arr=array();
	foreach($roll_sql as $row)
	{
...
if($action=="print_barcode_one_urmi")
{
	...
	from ppl_cont_cut_lay_bundle a, ppl_cont_cut_lay_size_dtls b where a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id $dynamic_cond
...
if($action=="print_barcode_ten_youth_new")
{
	...
	unset($roll_sql);
	// $sql_cut_name=sql_select("select entry_date,table_no,cut_num_prefix_no,batch_id,company_id from ppl_cont_cut_lay_mst where id=$data[2]");
	$sql_cut_name=sql_select("select a.entry_date,a.table_no,a.cut_num_prefix_no,a.batch_id,a.company_id,b.floor_id from ppl_cont_cut_lay_mst a,lib_cutting_table b where a.table_no=b.id and  a.id=$data[2]");
	...
	if(count($sql_bundle_copy)!=0)
	{
		foreach($sql_bundle_copy as $inf)
		{
			...
			foreach($color_sizeID_arr as $val)
			{
				...
				$pdf->Code40($i, $j-2, $symb." ".$buyer_name_str."  QTY# ".$val[csf("size_qty")] . "  STKR# ".$val[csf("number_start")]."-".$val[csf("number_end")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				// $pdf->Code40($i, $j+4.4, $val[csf("bundle_no")]."  PO# ".$po_number, $ext = true, $cks = false, $w = 0.2, $h = 1, $wide = true, true,7) ;
				...
				$pdf->Code40($i, $j+7.2, "COLOR# ".$colorNameTrimmed, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i, $j+10.2, $inf[csf("bundle_use_for")]."  B# ".$batch_no."  S# ".$lib_season_arr[$matrix_season], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide=true,true, 7);
				$pdf->Code40($i, $j+14, "CUT & ROLL# ".$order_cut_no." & ".$val[csf("roll_no")]."  SIZE# ".$size_arr[$val[csf("size_id")]]."(".$val[csf("pattern_no")].")", $ext = true, $cks = false, $w =0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i, $j+17, "IR# ".$internal_ref." TAB# ".$table_no." CLT# ".$colorType." ".$floor_name, $ext = true, $cks = false, $w =0.15, $h = 1, $wide = true, true, 7);
				$pdf->Code40($i, $j+23, $val[csf("barcode_no")], $ext = true, $cks = false, $w =0.15, $h = 1, $wide = true, true, 7);
				// $pdf->Code40($i, $j+22, $val[csf("bundle_no")],$w =0.2, $h = 8);
...
if($action=="print_barcode_one_128ddddddddddd")
{
	...
	$color_sizeID_arr=sql_select("select a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no
	from ppl_cont_cut_lay_bundle a, ppl_cont_cut_lay_size_dtls b where a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id and a.id in ($data[0])
	group by a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no order by b.bundle_sequence,a.id");
	foreach($color_sizeID_arr as $row)
	{
		$test_data[$row[csf("roll_id")]]=$row[csf("roll_id")];
...
if($action=="print_barcode_one_128")
{
	...
	//from ppl_cont_cut_lay_bundle a, ppl_cont_cut_lay_size_dtls b,ppl_cont_cut_lay_dtls c  where c.id=a.dtls_id and a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id and a.id in ($data[0])
...
if($action=="print_barcode_24_per_page")
{
	...
	$color_sizeID_arr=sql_select("SELECT c.batch_id, a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence,b.color_id,d.gmt_item_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,c.floor_id,c.inserted_by,c.working_company_id,d.con_color_name
	from ppl_cont_cut_lay_mst c, ppl_cont_cut_lay_bundle a, ppl_cont_cut_lay_size_dtls b,ppl_cont_cut_lay_dtls d where c.id=a.mst_id and a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id and c.id=d.mst_id and d.id=a.dtls_id and a.status_active=1 and b.status_active=1 and c.status_active=1 $dynamic_cond
	group by c.batch_id,a.id, a.size_id, a.bundle_no,a.barcode_no, a.number_start, a.number_end, a.size_qty, a.country_id, a.roll_id, a.roll_no, b.bundle_sequence,b.color_id,d.gmt_item_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,c.floor_id,c.inserted_by,c.working_company_id,d.con_color_name order by b.bundle_sequence,a.id");
 ...
 // echo $color_sizeID_arr;die;
	...
	if(count($sql_bundle_copy)!=0)
	{
		foreach($sql_bundle_copy as $inf)
		{
			...
			foreach($color_sizeID_arr as $val)
			{
				if($br==24)
				{
					$pdf->AddPage(); $br=0; $i=1; $j=1; $k=0;$x=3;$y=3;$z=31;$m=15;	$n=19;$o=19;$p=23;$yy=3;$zz=11;$yyy=15;$zzz=31;$cur_page++;
				}
				...
				if($k<=7)
				{
					...
					$m+=36.5;
					$n+=36.5;
					$pdf -> Line(50.5, $o, 50.5, $p);// border after cut no
					$o+=36.5;
					$p+=36.5;
					...
					$pdf->Code40($i, $j+8, "Color#".substr($val[csf("con_color_name")],0,16), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,9) ;
					...
					$pdf->Code40($i+35, $j+12, "BNDL#".$val[csf("bundle_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,9) ;

					$pdf->Code40($i, $j+16, "BP#".substr($inf[csf("bundle_use_for")],0,10), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,9) ;
					$pdf->Code40($i+35, $j+16, "CUT#".$bundle_no_array[2], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,9) ;
					$pdf->Code40($i+50, $j+16, "ROLL#".$bundle_no_array[2], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,9) ;



					$pdf->Code40($i, $j+20, "SIZE#".substr($size_arr[$val[csf("size_id")]],0,10), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,9) ;

					$pdf->Code40($i+20, $j+20, "PAT#".$val[csf("pattern_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,9) ;
					...
					$pdf->Code40($i+35, $j+24, "ITEM#".substr($garments_item[$val['GMT_ITEM_ID']],0,10), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,9) ;

					$pdf->Code40($i, $j+31,$val[csf("barcode_no")],'','',0.25,5 ,  $ext = true, $cks = false, $w = 0.20, $h = 1, $wide = true, true,6) ;
				...
				else if($k>7 && $k<=15)
				{
					...
					$m+=36.5;
					$n+=36.5;
					$pdf -> Line(121.5, $o, 121.5, $p);// border after cut no
					$o+=36.5;
					$p+=36.5;
				...
				else if($k>15 && $k<=23)
				{
					...
					$m+=36.5;
					$n+=36.5;
					$pdf -> Line(190.5, $o, 190.5, $p);// border after cut no
					$o+=36.5;
					$p+=36.5;
	...
	else
	{
		foreach($color_sizeID_arr as $val)
		{
			if($br==5)
			{
				$pdf->AddPage(); $br=0; $i=2; $j=2; $k=0;$x=8;$y=8;$z=42;$m=24;	$n=36;
				$cur_page++;
			}
...
if($action=="print_report_bundle_barcode")
{
	...
	//$pdf->SetFont('Times', '', 20);
	$pdf->AddPage();
	$color_sizeID_arr=sql_select( "select id,size_id,bundle_no,number_start,number_end,size_qty,country_id from ppl_cont_cut_lay_bundle where id in ( $data[0] ) " );  //where id in ($data)
	$i=5; $j=10; $k=0;
    $bundle_array=array();
	$br=0;
	$n=0;
...
if($action=="save_update_delete_size")
{
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		$cutData=sql_select("select cut_num_prefix_no, job_no from ppl_cont_cut_lay_mst where cutting_no = '".$cut_no."'");
		...
		$total_marker_qty_prev= return_field_value("sum(b.marker_qty) as mark_qty","ppl_cont_cut_lay_dtls a, ppl_cont_cut_lay_size b","a.id=b.dtls_id and b.order_id in(".$order_id.") and a.gmt_item_id=".$gmt_id." and a.color_id=".$color_id." and a.status_active=1 and b.status_active=1 and b.is_deleted=0","mark_qty");

		$sizeRatioArr=array(); $sizeQtyArr=array(); $sizeQtyArrForC=array(); $sizeIdAgainstSeq=array(); $seqDatas='';
		$id_size=return_next_id("id", "ppl_cont_cut_lay_size_dtls", 1);
		...
		$rollDtls_id=return_next_id("id", "ppl_cont_cut_lay_roll_dtls",1);
		...
		$bundle_id=return_next_id("id", "ppl_cont_cut_lay_bundle",1);
		...
		$bundleData=sql_select("select max(number_end) as last_rmg, max(bundle_num_prefix_no) as last_prefix from ppl_cont_cut_lay_bundle where mst_id=$mst_id");
		...
		$id=return_next_id("id", "ppl_cont_cut_lay_size", 1);
		...
		if (strlen($year_id)==4) $year_id=substr($year_id,2,2);
		$barcode_suffix_no=return_field_value("max(barcode_prifix) as suffix_no","ppl_cont_cut_lay_bundle","barcode_year=$year_id","suffix_no");
		$barcodeSuffixNo=return_field_value("max(barcode_prifix) as suffix_no","ppl_cont_cut_lay_bundle","barcode_year=$year_id","suffix_no");
		...
		{
			$erange=return_field_value("max(number_end) as last_rmg","ppl_cont_cut_lay_bundle","mst_id=$mst_id and dtls_id!=$dtls_id and status_active=1 and is_deleted=0","last_rmg");
		}
		else if($rmg_no_creation==3) // job wise
		{
			$erange=return_field_value("max(a.number_end) as last_rmg","ppl_cont_cut_lay_bundle a, ppl_cont_cut_lay_mst b","a.mst_id=b.id and b.entry_form=586 and b.job_no='$job_id' and a.status_active=1 and a.is_deleted=0","last_rmg");
		}
		else if($rmg_no_creation==4) // order wise
		{
			// $erange=return_field_value("max(a.number_end) as last_rmg","ppl_cont_cut_lay_bundle a, ppl_cont_cut_lay_dtls b","a.dtls_id=b.id and a.order_id in($orderIds) and a.status_active=1 and a.is_deleted=0","last_rmg");
			$sql = "SELECT a.ORDER_ID,max(a.number_end) as LAST_RMG from ppl_cont_cut_lay_bundle a, ppl_cont_cut_lay_dtls b where a.dtls_id=b.id and a.order_id in($orderIds) and a.status_active=1 and a.is_deleted=0 group by a.order_id";
			$res = sql_select($sql);
		...
		//do not delete
		/*
		$bundleId=return_next_id("id", "ppl_cont_cut_lay_bundle",1);
		$data_array_bundle = '';
		...
		*/
		...
		//echo "10**insert into ppl_cont_cut_lay_bundle($field_array_bundle) values".$data_array_bundle; die;
		//echo "10**insert into ppl_cont_cut_lay_roll_dtls($field_array_roll_dtls) values".$data_array_roll_dtls; die;
		//echo "10**insert into ppl_cont_cut_lay_size_dtls($field_array_size)values".$data_array_size; die;
		//echo "10**insert into ppl_cont_cut_lay_size($field_array)values".$data_array;die;
		//echo "10**insert into ppl_cont_cut_lay_bundle($field_array_bundle)values".$data_array_bundle; die;
		$rID=sql_insert("ppl_cont_cut_lay_size",$field_array,$data_array,0);
		$rID_size=sql_insert("ppl_cont_cut_lay_size_dtls",$field_array_size,$data_array_size,0);
		$rID2=sql_insert("ppl_cont_cut_lay_bundle",$field_array_bundle,$data_array_bundle,0);
		$rID3=sql_insert("ppl_cont_cut_lay_roll_dtls",$field_array_roll_dtls,$data_array_roll_dtls,0);
		...
		$rID4=sql_update("ppl_cont_cut_lay_dtls",$field_array_up,$data_array_up,"id",$dtls_id,0);
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$cutting_qc_no= "";
		$cutData=sql_select("select cut_num_prefix_no, job_no from ppl_cont_cut_lay_mst where cutting_no = '".$cut_no."'");
		//$cut_on_prifix = return_field_value("cut_num_prefix_no","ppl_cont_cut_lay_mst","cutting_no = '".$cut_no."'");
		$cut_on_prifix=$cutData[0][csf('cut_num_prefix_no')];
		$job_no=$cutData[0][csf('job_no')];
		...
		$previous_barcode_data=sql_select("select bundle_no,barcode_no,barcode_year,barcode_prifix from ppl_cont_cut_lay_bundle where mst_id=".$mst_id."  and  dtls_id=".$dtls_id." and status_active=1 and is_deleted=0 ");
		foreach($previous_barcode_data as $b_val)
		{
			$previous_barcode_arr[$b_val[csf("bundle_no")]]['year']=$b_val[csf("barcode_year")];
			$previous_barcode_arr[$b_val[csf("bundle_no")]]['prifix']=$b_val[csf("barcode_prifix")];
		...
		$total_marker_qty_prev= return_field_value("sum(b.marker_qty) as mark_qty","ppl_cont_cut_lay_dtls a, ppl_cont_cut_lay_size b","a.id=b.dtls_id and a.id!=$dtls_id and  b.order_id in(".$order_id.") and a.gmt_item_id=".$gmt_id." and a.color_id=".$color_id." and a.status_active=1 and b.status_active=1 and b.is_deleted=0","mark_qty");
		...
		//$last_rmg_no=return_field_value("max(a.number_end) as last_rmg", "ppl_cont_cut_lay_bundle a, ppl_cont_cut_lay_mst b","a.mst_id=b.id and a.mst_id='".$mst_id."' and a.dtls_id!=$dtls_id","last_rmg");
		//$last_bundle_no=return_field_value("max(bundle_num_prefix_no) as last_prefix", "ppl_cont_cut_lay_bundle","mst_id=$mst_id and dtls_id!=$dtls_id","last_prefix");

		$bundleData=sql_select("select max(number_end) as last_rmg, max(bundle_num_prefix_no) as last_prefix from ppl_cont_cut_lay_bundle where mst_id=$mst_id and dtls_id!=$dtls_id");
		$last_rmg_no=$bundleData[0][csf('last_rmg')];
		$last_bundle_no=$bundleData[0][csf('last_prefix')];
		...
		//echo "10**"."SELECT size_id, max(number_end) as last_rmg FROM ppl_cont_cut_lay_bundle WHERE mst_id='".$mst_id."' and dtls_id!='".$dtls_id."' group by size_id";die;
		$id=return_next_id("id", "ppl_cont_cut_lay_size", 1);
		$field_array="id,mst_id,dtls_id,color_id,size_id,size_ratio,marker_qty,country_type,country_id,excess_perc,order_id,size_wise_repeat,inserted_by,insert_date";
		for($i=1; $i<=$row_num; $i++)
		{
		...
		*/
		...
		$delete=execute_query("delete from ppl_cont_cut_lay_size where mst_id=".$mst_id." and dtls_id=".$dtls_id."",0);
		$delete_size=execute_query("delete from ppl_cont_cut_lay_size_dtls where mst_id=".$mst_id." and dtls_id=".$dtls_id."",0);
		$delete_bundle=execute_query("delete from ppl_cont_cut_lay_bundle where mst_id=".$mst_id." and dtls_id=".$dtls_id."",0);
		$delete_roll=execute_query("delete from ppl_cont_cut_lay_roll_dtls where mst_id=".$mst_id." and dtls_id=".$dtls_id."",0);

		$rID=sql_insert("ppl_cont_cut_lay_size",$field_array,$data_array,0);
		$rID_size=sql_insert("ppl_cont_cut_lay_size_dtls",$field_array_size,$data_array_size,0);
		$rID2=sql_insert("ppl_cont_cut_lay_bundle",$field_array_bundle,$data_array_bundle,0);
		//echo "10**insert into ppl_cont_cut_lay_bundle($field_array_bundle) values".$data_array_bundle."**".$rID2;die;
		$rID3=sql_insert("ppl_cont_cut_lay_roll_dtls",$field_array_roll_dtls,$data_array_roll_dtls,0);
		$field_array_up="marker_qty*pcs_per_bundle*updated_by*update_date";
		$data_array_up="".$to_marker_qty."*'".$txt_bundle_pcs."'*'".$user_id."'*'".$pc_date_time."'";
		$rID4=sql_update("ppl_cont_cut_lay_dtls",$field_array_up,$data_array_up,"id",$dtls_id,0);
...
if($action=="show_bundle_list_view")
{
	...
	$po_no_arr=return_library_array("select a.id, a.po_number from wo_po_break_down a, ppl_cont_cut_lay_size b where a.id=b.order_id and b.mst_id=$mst_id and b.dtls_id=$dtls_id and b.status_active=1 and b.is_deleted=0 group by a.id, a.po_number",'id','po_number');
    ...
    <fieldset style="width:960px">
        ...
        <table cellpadding="0" cellspacing="0" width="950" rules="all" border="1" class="rpt_table" id="tbl_bundle_list_save">
            ...
            <tbody id="trBundleListSave">
            <?
            $sql_size_name=sql_select("select size_id from ppl_cont_cut_lay_size where mst_id=".$mst_id." and dtls_id=".$dtls_id."");
            $size_colour_arr=array();
            foreach($sql_size_name as $asf)
            {
                $size_colour_arr[$asf[csf("size_id")]]=$size_arr[$asf[csf("size_id")]];
...
if($action=="cut_lay_bundle_print")
{
	...
	$sql="select a.cut_num_prefix_no, a.table_no,a.cutting_no, a.job_no, a.entry_date,a.marker_length, a.marker_width, a.fabric_width, a.gsm, a.batch_id as batch_no,a.location_id,a.floor_id,a.working_company_id,a.remarks, b.order_id, b.color_id, b.gmt_item_id, b.plies, b.marker_qty, b.order_qty, b.order_ids, b.batch_id, b.order_cut_no
	 , b.con_color_name
	from ppl_cont_cut_lay_mst a, ppl_cont_cut_lay_dtls b where a.id='$data[1]' and a.company_id='$data[0]' and b.id='$data[2]' ";
	//echo $sql;die;
	...
	$con = $dataArray[0][csf('con_color_name')];
	...
	<div style="width:1000px; " align="center" >
		<table width="990" cellspacing="0" align="center">
			...
			<tr>
				<td></td><td></td>
				<td ><strong >Contrast color:</strong></td> <td><? echo $con; ?></span></td>
				<td></td><td></td>
			</tr>
			<tr>
			...
			<td><strong>Cutting Part:</strong></td> <td><? echo $data[5]; ?></td>
		...
		<div style="width:1200px;">
			<table align="center" cellspacing="0" width="1180" border="1" rules="all" class="rpt_table" >
				...
				<thead bgcolor="#dddddd" align="center">
					...
					<tbody>
					<?
						$batchNo_arr=return_library_array( "select a.id, a.batch_no from pro_roll_details a, ppl_cont_cut_lay_bundle b where a.id=b.roll_id and a.mst_id='$data[1]' and a.dtls_id='$data[2]' and a.entry_form=586","id","batch_no" );

						if($data[4]==0) $country_cond=""; else $country_cond=" and a.country_id='".$data[4]."'";
						$size_data=sql_select("select a.id,a.size_id,a.bundle_sequence,a.marker_qty,a.size_ratio from ppl_cont_cut_lay_size_dtls a where a.mst_id='$data[1]' and a.dtls_id='$data[2]'  order by a.bundle_sequence ASC");
						//echo "select a.id,a.size_id,a.bundle_sequence,a.marker_qty,a.size_ratio from ppl_cont_cut_lay_size_dtls a where a.mst_id='$data[1]' and a.dtls_id='$data[2]'  order by a.bundle_sequence ASC";
						...
						{
							$total_marker_qty_size=0;
							$bundle_data=sql_select("SELECT a.id,a.bundle_no as bundle_no,a.bundle_num_prefix_no,a.size_id,a.number_start,a.number_end,a.size_qty, a.country_id, a.roll_no, a.roll_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no,a.barcode_no,a.update_flag from ppl_cont_cut_lay_bundle a where a.mst_id='$data[1]' and a.dtls_id='$data[2]' and a.size_id=".$size_val[csf('size_id')]." $country_cond order by a.id ASC");
							//echo "select a.id,a.bundle_no as bundle_no,a.bundle_num_prefix_no,a.size_id,a.number_start,a.number_end,a.size_qty, a.country_id, a.roll_no, a.roll_id, a.pattern_no, a.is_excess, a.order_id,a.bundle_num_prefix_no from ppl_cont_cut_lay_bundle a where a.mst_id='$data[1]' and a.dtls_id='$data[2]' and a.size_id=".$size_val[csf('size_id')]." $country_cond order by a.id ASC";
							foreach($bundle_data as $row)
							{
...
if($action=="cut_lay_bundle_list_print2")
{
	...
	$sql="SELECT a.cut_num_prefix_no, a.table_no,a.cutting_no, a.job_no, a.entry_date,a.marker_length, a.marker_width, a.fabric_width, a.gsm, a.batch_id as batch_no,a.location_id,a.floor_id,a.working_company_id,a.remarks, b.order_id, b.color_id, b.gmt_item_id, b.plies, b.marker_qty, b.order_qty, b.order_ids, b.batch_id, b.order_cut_no,b.con_color_name from ppl_cont_cut_lay_mst a, ppl_cont_cut_lay_dtls b where a.id='$data[1]' and a.company_id='$data[0]' and b.id='$data[2]' ";
	...
	<div style="width:1000px; " align="center" >
		<table width="990" cellspacing="0" align="center">
			<tr>
				<td colspan="6" align="center" style="font-size:xx-large"><strong><? echo $working_comp_library[$dataArray[0][csf('working_company_id')]]; ?></strong></td>
			</tr>
			<tr>
				<td colspan="6" align="center" style="font-size:x-large"><strong><u>Lay and Bundle Information</u></strong></td>
			</tr>
			...
			<tr>
				<td><strong>Marker Width :</strong></td><td width="160"><? echo $dataArray[0][csf('marker_width')]; ?></td>
				<td><strong>Contrast Color:</strong></td><td width="160"><? echo $dataArray[0][csf('con_color_name')]; ?></td>
				<td><strong>Gsm:</strong></td> <td width="160"><? echo $dataArray[0][csf('gsm')]; ?></td>
			</tr>
			<tr>
				<td><strong>Order Cut No:</strong></td> <td width="160"><? echo $order_cut_no; ?></td>
			...
			<tr>
				<td><strong>Remarks:</strong></td><td width="100"><? echo $remarks;?></td>
				<td><strong>Working Location:</strong></td> <td width="160"><? echo $working_location_library[$dataArray[0][csf('location_id')]]; ?></td>
				<td><strong>Cutting Part:</strong></td> <td  colspan="4"><? echo $data[5]; ?></td>
				<td><strong>Page:</strong></td> <td width="120"><?echo $cur_page ?></td>
			</tr>
		</table>
		<br>
		<table align="left" cellspacing="0" width="400" border="1" rules="all" class="rpt_table" >
			...
			<tbody>
				<?
				$size_data_one=sql_select("select a.id,a.size_id,a.bundle_sequence,a.marker_qty,a.size_ratio from ppl_cont_cut_lay_size_dtls a where a.mst_id='$data[1]' and a.dtls_id='$data[2]'  order by a.bundle_sequence ASC");
				$size_ratio_arr=array();
...
<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            ...
            <tbody>
                <tr>
                    ...
                    <td align="center">
                         <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_job_prifix').value+'_'+document.getElementById('cbo_year_selection').value+'_'+document.getElementById('txt_po_no').value+'_'+document.getElementById('txt_file_no').value+'_'+document.getElementById('txt_internal_ref').value+'_'+document.getElementById('txt_style_no').value, 'create_job_search_list_view', 'search_div', 'contrast_cutting_entry_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:70px;" />
                    </td>
            </tr>
        		<tr>
...
if($action=="save_update_delete")
{
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		$dataArrayMst=sql_select("select a.cutting_no, b.color_id, b.order_cut_no from ppl_cont_cut_lay_mst a, ppl_cont_cut_lay_dtls b where a.company_id=".$cbo_company_name." and a.location_id=".$cbo_location_name." and a.floor_id=".$cbo_floor_name." and b.gmt_item_id in($gmtItems) and a.entry_form=586 and a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and a.job_no=$txt_job_no");
		...
		$new_sys_number = explode("*", return_next_id_by_sequence("", "ppl_cont_cut_lay_mst",$con,1,$cbo_company_name,'',0,date("Y",time()),0,0,0,0,0 ));
		...
		//$id=return_next_id("id", "ppl_cont_cut_lay_mst", 1);
		$id= return_next_id_by_sequence(  "ppl_cont_cut_lay_mst_seq",  "ppl_cont_cut_lay_mst", $con );

		$field_array="id,entry_form,cut_num_prefix,cut_num_prefix_no,cutting_no,table_no,job_no,batch_id,company_id,working_company_id,location_id,floor_id,entry_date,start_time,end_date,end_time,remarks,shipment_part,marker_length,marker_width,fabric_width,gsm,width_dia,cad_marker_cons,inserted_by,insert_date,status_active,is_deleted";
		...
		$data_array="(".$id.",586,'".$new_cutting_prifix."',".$cut_no_prifix[0].",'".$new_cutting_number."',".$tbl_id.",".$txt_job_no.",".$txt_batch_no.",".$cbo_company_name.",".$cbo_working_company_name.",".$cbo_location_name.",".$cbo_floor_name.",".$txt_entry_date.",'".$start_time."',".$txt_end_date.",'".$end_time."',".$txt_remark.",".$txt_shipment_part.",".$txt_marker_length.",".$txt_marker_width.",".$txt_fabric_width.",".$txt_gsm.",".$cbo_width_dia.",".$txt_marker_cons.",'".$user_id."','".$pc_date_time."',1,0)";

		$detls_id= return_next_id_by_sequence(  "ppl_cont_cut_lay_dtls_seq",  "ppl_cont_cut_lay_dtls", $con );
		$field_array1="id, mst_id,order_ids,color_type_id,order_cut_no,color_id,batch_id,gmt_item_id,plies,order_qty,roll_data,con_color_name,inserted_by,insert_date,status_active,is_deleted";
		...
		for($i=1; $i<=$row_num; $i++)
		{
			...
			for($x=0;$x<count($save_string);$x++)
			{
				...
				$data_array_roll.="(".$id_roll.",".$barcode_no.",".$id.",".$detls_id.",".$$cbo_order_id.",586,'".$roll_qnty."','".$roll_id."','".$roll_no."','".$plies."','".$batchNo."','".$shade."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			...
			$data_array1.="(".$detls_id.",".$id.",".$$cbo_order_id.",".$$cboColorType.",".$$order_cut_no.",".$$cbocolor.",".$$cbobatch.",".$$cbo_gmt_id.",".$$txt_plics.",".$$order_qty.",'".$response_data."',".$$concolor.",'".$user_id."','".$pc_date_time."',1,0)";
		...
		$rID2=sql_insert("ppl_cont_cut_lay_mst",$field_array,$data_array,0);

		if($data_array1!="")
		{
			$rID3=sql_insert("ppl_cont_cut_lay_dtls",$field_array1,$data_array1,0);
		}
		...
		if($data_array_roll!="")
		{
		 ...
		 //echo "10**insert into ppl_cont_cut_lay_mst( $field_array) values".$data_array;oci_rollback($con);die;
	...
	{
		  	$prev_cut_no_arr=array();
			$dataArrayMst=sql_select("select a.cutting_no, b.color_id, b.order_cut_no from ppl_cont_cut_lay_mst a, ppl_cont_cut_lay_dtls b where a.company_id=".$cbo_company_name." and a.entry_form=586 and a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and a.job_no=$txt_job_no and a.id!=$update_id");
			...
			//master table update*********************************************************************
			$field_array="table_no*job_no*batch_id*company_id*working_company_id*location_id*floor_id*entry_date*start_time*end_date*end_time*remarks*shipment_part*marker_length*marker_width*fabric_width*gsm*width_dia*cad_marker_cons*updated_by*update_date";
			$start_time=str_replace("'","",$txt_in_time_hours).":".str_replace("'","",$txt_in_time_minuties);
			$end_time=str_replace("'","",$txt_out_time_hours).":".str_replace("'","",$txt_out_time_minuties);
			$data_array="".$tbl_id."*".$txt_job_no."*".$txt_batch_no."*".$cbo_company_name."*".$cbo_working_company_name."*".$cbo_location_name."*".$cbo_floor_name."*".$txt_entry_date."*'".$start_time."'*".$txt_end_date."*'".$end_time."'*".$txt_remark."*".$txt_shipment_part."*".$txt_marker_length."*".$txt_marker_width."*".$txt_fabric_width."*".$txt_gsm."*".$cbo_width_dia."*".$txt_marker_cons."*'".$user_id."'*'".$pc_date_time."'";


		    //$detls_id=return_next_id("id", " ppl_cont_cut_lay_dtls", 1);
		    $detls_id= return_next_id_by_sequence(  "ppl_cont_cut_lay_dtls_seq",  "ppl_cont_cut_lay_dtls", $con );
			$field_array1="id, mst_id,order_ids,color_type_id,order_cut_no,ship_date,color_id,batch_id,gmt_item_id,plies,order_qty,roll_data,con_color_name,inserted_by,insert_date,status_active,is_deleted";
			$field_array_up="order_ids*color_type_id*order_cut_no*ship_date*color_id*batch_id*gmt_item_id*plies*order_qty*roll_data*con_color_name*updated_by*update_date";
			$field_array_roll="id, barcode_no, mst_id, dtls_id, po_ids, entry_form, qnty, roll_id, roll_no, plies, batch_no,shade, inserted_by, insert_date";
			//$id_roll = return_next_id( "id", "pro_roll_details", 1 );
			$add_comma=0;

			$duplicateMsg=''; $duplicateStatus=true;
			//$order_cut_no_arr=return_library_array( "select order_id,max(order_cut_no) as order_cut_no from ppl_cont_cut_lay_dtls group by order_id", "order_id", "order_cut_no"  );
			...
			for($i=1; $i<=$row_num; $i++)
				{
					...
					for($x=0;$x<count($save_string);$x++)
					{
						...
						if($data_array_roll!="") $data_array_roll.= ",";
						$data_array_roll.="(".$id_roll.",".$barcode_no.",".$msster_id.",".$dtlsId.",".$$cbo_order_id.",586,'".$roll_qnty."','".$roll_id."','".$roll_no."','".$plies."','".$batchNo."','".$shade."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
						$response_data.=$barcode_no."=".$roll_no."=".$roll_id."=".$roll_qnty."=".$plies."=".$batchNo."=".$shade."**";
						//$id_roll = $id_roll+1;
					}
					...
					if(str_replace("'",'',$$update_details_id)!="")
					  {
							...
							$data_array_up[str_replace("'",'',$$update_details_id)]=explode("_",("".$$cbo_order_id."_".$$cboColorType."_".$$order_cut_no."_".$$txt_ship_date."_".$$cbocolor."_".$$cbobatch."_".$$cbo_gmt_id."_".$$txt_plics."_".$$order_qty."_'".$response_data."'_".$$concolor."_'".$user_id."'_'".$pc_date_time."'_1_0"));
							//$dtlsId=str_replace("'",'',$$update_details_id);
					  ...
					  else
					  {
							...
							$data_array1.="(".$detls_id.",".$msster_id.",".$$cbo_order_id.",".$$cboColorType.",".$$order_cut_no.",'".$$txt_ship_date."',".$$cbocolor.",".$$cbobatch.",".$$cbo_gmt_id.",".$$txt_plics.",".$$order_qty.",'".$response_data."',".$$concolor.",".$user_id.",'".$pc_date_time."',1,0)";
							$detls_id_array.=$detls_id."#".str_replace("'",'',$$order_cut_no);
							//$dtlsId=$detls_id;
							//$detls_id=$detls_id+1;
							$detls_id= return_next_id_by_sequence(  "ppl_cont_cut_lay_dtls_seq",  "ppl_cont_cut_lay_dtls", $con );
			...
			$rID1=sql_update("ppl_cont_cut_lay_mst",$field_array,$data_array,"id",$update_id,0);

			$detls_id_update.=$detls_id_array;
			if(count($updateID_array)>0)
				{
					$rID2=execute_query(bulk_update_sql_statement("ppl_cont_cut_lay_dtls","id",$field_array_up,$data_array_up,$updateID_array),1);
				}

				if($data_array1!="")
				{
				   $rID3=sql_insert("ppl_cont_cut_lay_dtls",$field_array1,$data_array1,1);
				}

			$delete_roll=execute_query("delete from pro_roll_details where mst_id=$msster_id and entry_form=586",0);
			if($data_array_roll!="")
			{
				$rID4=sql_insert("pro_roll_details",$field_array_roll,$data_array_roll,0);
			}
	...
	else if ($operation==2) // Delete Here----------------------------------------------------------
	{
		...
		$rID = sql_delete("ppl_cont_cut_lay_mst","updated_by*update_date*status_active*is_deleted","".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*0*1",'id ',$update_id,1);
		$rID2 = sql_delete("ppl_cont_cut_lay_dtls","updated_by*update_date*status_active*is_deleted","".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*0*1",'mst_id ',$update_id,1);
		$rID3 = sql_delete("ppl_cont_cut_lay_bundle","updated_by*update_date*status_active*is_deleted","".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*0*1",'mst_id ',$update_id,1);
		$rID4 = sql_delete("ppl_cont_cut_lay_size","updated_by*update_date*status_active*is_deleted","".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*0*1",'mst_id ',$update_id,1);
		$rID5 = sql_delete("ppl_cont_cut_lay_size_dtls","updated_by*update_date*status_active*is_deleted","".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*0*1",'mst_id ',$update_id,1);
		$rID6 = sql_delete("ppl_cont_cut_lay_roll_dtls","updated_by*update_date*status_active*is_deleted","".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*0*1",'mst_id ',$update_id,1);
        ...
        <form name="searchprocessfrm_1"  id="searchprocessfrm_1" autocomplete="off">
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="390" class="rpt_table" >
                <thead>
                	<tr>
	                    <th width="40"></th>
	                    <th width="200"><input type="button" class="formbutton" value="With Full Shipment Po" onClick="show_list_view ( '<?php echo $poId; ?>' +'_'+'<?php echo $txt_job; ?>'+'_'+'<?php echo $cbocolor; ?>'+'_'+'<?php echo $gmt_id; ?>'+'_'+'<?php echo $process_row_id; ?>', 'create_list_view_with_full_shipment_po', 'buyer_list_view', 'contrast_cutting_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1)')"></th>
	                    <th>
...
if($action=="create_cutting_search_list_view")
{
	...
	else if($search_type==3)//starts with
	{
		if(str_replace("'","",$company)==0) $conpany_cond=""; else $conpany_cond="and a.company_id=".str_replace("'","",$company)."";
		if(str_replace("'","",$cutting_no)=="") $cut_cond=""; else $cut_cond="and a.cut_num_prefix_no='".str_replace("'","",$cutting_no)."'  $year_cond";
		if(str_replace("'","",$job_no)=="") $job_cond=""; else $job_cond="and b.job_no_prefix_num='".str_replace("'","",$job_no)."'";
	...
	$sql_order="SELECT a.id,a.cut_num_prefix_no, a.table_no, a.job_no, a.batch_id, a.entry_date, a.marker_length, a.marker_width, a.fabric_width,c.id as dtls_id,c.color_id, c.marker_qty, c.order_cut_no,$year,b.buyer_name,b.style_ref_no,c.order_ids,d.po_number FROM ppl_cont_cut_lay_mst a,wo_po_details_master b,ppl_cont_cut_lay_dtls c,wo_po_break_down d where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and  c.mst_id=a.id and a.job_no=b.job_no and b.id=d.job_id and a.entry_form=586 $conpany_cond $cut_cond $job_cond $sql_cond  $order_cond $order_cut_no_cond order by id desc";
	//echo $sql_order;die;
					...
					foreach ($cut_data as $dtls_id=>$val)
					{
						...
						<tr onclick="js_set_cutting_value('<?=$cut_id;?>')" style="cursor:pointer"  id="tr_1" height="" cellspacing="0" cellpadding="0"
                          border="0" width="1140"  bgcolor="<?=$bgcolor;?>">
							...
							<td align="center" width="70"><?=$val['fabric_width'];?></td>
    ...
    foreach($sql_data as $val)
	  {
			...
			echo "load_drop_down( 'requires/contrast_cutting_entry_controller','".$val[csf("working_company_id")]."', 'load_drop_down_location', 'location_td') ;";
...
if($action=="order_details_list")
{
	// $sql_gmt_arr="select ";
	 $tbl_row=0;
	 $sql_dtls=sql_select("SELECT a.id,a.order_ids,a.ship_date,a.color_id,a.color_type_id,a.batch_id,a.gmt_item_id,a.plies,a.marker_qty,a.order_qty,a.total_lay_qty,a.lay_balance_qty,b.job_no,b.job_year,b.company_id, a.order_cut_no, a.roll_data,a.con_color_name from ppl_cont_cut_lay_dtls a, ppl_cont_cut_lay_mst b where b.id=a.mst_id and mst_id=".$data." order by a.id");
	...
	foreach($sql_dtls as $val)
	{
		...
		$sql_marker="select sum(marker_qty) as mark_qty from ppl_cont_cut_lay_dtls where order_ids='".$val[csf("order_ids")]."' and gmt_item_id=".$val[csf("gmt_item_id")]." and color_id=".$val[csf("color_id")]." and status_active=1";
		$result=sql_select($sql_marker);
		foreach($result as $rows)
		{
	   ...
	   <tr class="" id="tr_<? echo $tbl_row; ?>" style="height:10px;">
       		...
       		<td align="center" id="concolortd_<? echo $tbl_row; ?>">
			   <input type="text" name="concolor_<? echo $tbl_row; ?>" id="concolor_<? echo $tbl_row; ?>" class="text_boxes" style="width:100px;" placeholder="Write..." value="<?=$val[csf('con_color_name')];?>" />
			</td>
...
if($action=="cut_lay_entry_report_print")
{
	...
	$data=explode('*',$data);
	//print_r($data);
	$sql=sql_select("select id,job_no,cut_num_prefix_no,table_no,marker_length,marker_width,fabric_width,gsm,width_dia,cad_marker_cons,batch_id,company_id from ppl_cont_cut_lay_mst where cutting_no='".$data[0]."' ");
	foreach($sql as $val)
		{
			$mst_id=$val[csf('id')];
	...
	$sql_order=sql_select("select order_ids,gmt_item_id,order_qty from ppl_cont_cut_lay_dtls where mst_id='$mst_id'");
	$buyer_arr=return_library_array( "select id, buyer_name from  lib_buyer",'id','buyer_name');
	$color_arr=return_library_array( "select id,color_name  from  lib_color", "id", "color_name"  );
 ...
 <div style=" width:1100px; position:absolute; top:385px; ">
   ...
   <?
     ...
     ppl_cont_cut_lay_size_dtls b,ppl_cont_cut_lay_dtls a where a.id=b.dtls_id  and a.mst_id=$mst_id and b.status_active=1  and  b.is_deleted=0 and a.status_active=1
     and a.is_deleted=0 ");
	...
	$sql_main_qry=sql_select("select c.id,a.id,a.color_id,c.size_id,c.roll_no,sum(c.roll_wgt) as roll_weight,c.plies, sum(c.size_qty) as size_qty,c.roll_id
	 from  ppl_cont_cut_lay_roll_dtls c,ppl_cont_cut_lay_dtls a
	 ...
	 foreach($sql_main_qry as $main_val)
	 {
		...
		$size_qty=return_field_value("size_qty","ppl_cont_cut_lay_bundle","mst_id =$mst_id AND dtls_id =".$main_val[csf('id')]." ");
...
{   list($po_id,$row_no,$color,$gmt_id)=explode('_',$data);
	...
	$sql_dtls=sql_select("select color_type_id from ppl_cont_cut_lay_dtls where order_ids in(".$po_id.") and color_id=$color and gmt_item_id=$gmt_id");
