		...
		foreach($data_array as $pattern_no=>$pattern_data)
		{
			...
			foreach($pattern_data as $bundle_use_for=>$bundle_use_for_data)
			{
				...
				{					
					...
					// $pdf->Code40($i, $j+12, "Cut/N# ".$order_cut_no, $ext = true, $cks = false, $w = 0.7, $h = 1, $wide = true, true,7) ;
					$pdf->Code40($i, $j+15.2, "Size# ".$size_arr[$val["size_id"]]."(".$pattern_no.")", $ext = true, $cks = false, $w = 0.7, $h = 1, $wide = true, true,7) ;
					$pdf->Code40($i+35, $j+15.2, "Qty#".$col_size_data_arr[$barcode], $ext = true, $cks = false, $w = 0.7, $h = 1, $wide = true, true,7) ;
								...
								if($plies>0 && $bl_roll_plies>0 && $bl_size_qty>0)
								{
									...
									else
									{
										...
										if($bundle_qty2>0)
										{
											if($bundle_qty2>$bl_size_qty) 
											{
												$tmp_bl_arr[$size_id][$rollId][1]=$bundle_qty2-$bl_size_qty;//echo 
												$tmp_bl_arr[$size_id][$rollId][2]=$bl_size_qty;
												$bundle_qty2=$bl_size_qty;
											}
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		$data_array="(".$id.",289,'".$new_cutting_prifix."',".$cut_no_prifix[0].",'".$new_cutting_number."',".$tbl_id.",".$txt_job_no.",".$txt_batch_no.",".$cbo_company_name.",".$cbo_working_company_name.",".$cbo_location_name.",".$cbo_floor_name.",".$txt_entry_date.",'".$start_time."',".$txt_end_date.",'".$end_time."',".$txt_marker_length.",".$txt_marker_width.",".$txt_fabric_width.",".$txt_gsm.",".$cbo_width_dia.",".$txt_marker_cons.",'".$user_id."','".$pc_date_time."',1,0)";
	...
	<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
	            ...
	            <tbody>
	                  <tr class="general">         
	                        ...
	                        <td>
	                             <? 
	                                   echo create_drop_down( "cbo_wo_company_name", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 order by company_name","id,company_name", 1, "-- Select Company --",0, "",0);
	                             ?>
	                        </td>
