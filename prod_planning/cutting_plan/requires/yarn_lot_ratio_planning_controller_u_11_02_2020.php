...
if ($action=="load_drop_down_order_garment")
{
	...
	{
		echo create_drop_down( "cbogmtsitem_$ex_data[1]", 120, $garments_item,"", 1, "-- Select Item --", $gmt_item_id, "style_order_data($ex_data[1]);","",$gmt_item_id);
	}
    else if(count($gmt_item_arr)>1)
	{
		echo create_drop_down( "cbogmtsitem_$ex_data[1]", 120, $garments_item,"", 1, "-- Select Item --", $selected, "style_order_data($ex_data[1]);","",$gmt_item_id);
	}
...
if ($action=="load_drop_down_color")
{
	...
	echo create_drop_down( "cbocolor_$ex_data[1]", 100, $color_item_arr,"", 1, "select color",'', "reset_fld($ex_data[1]);style_order_data($ex_data[1]);");
	exit();
	exit();
}
...
if ($action=="load_drop_down_order_qty")
{
	...
	$sql_size_data=sql_select("select a.sizeset_no FROM ppl_size_set_mst a,ppl_size_set_dtls b where a.id=b.mst_id and a.job_no='".$ex_data[0]."' and b.color_id=".$ex_data[2]." and  a.status_active=1 and a.is_deleted=0 and  b.status_active=1 and b.is_deleted=0 group by a.sizeset_no ");
		...
		<fieldset style="width:840px; margin-left:2px">
            ...
            <div style="width:820px; max-height:270px; overflow-y:scroll" id="list_container" align="left"> 
                <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="800" id="tbl_list_search">  
					...
					$data_array_strip=sql_select("select 
													  a.sample_color_id as sample_color,
													  a. yarn_color_id as stripe_color,
													  a. production_color_percentage as sample_per,
													  a.actual_consumption as measurement,
													  a.cons_per_dzn
												from 
				...
				if(count($data_array_strip)==0)
				{
                    ...
                    { 
						...
						$alocated_qty=$previous_allocated_arr[$row[csf('id')]]-$current_allocated_arr[$row[csf('id')]];
						$available_qty=$row[csf('stock_qty')]-$alocated_qty;
						$mesurement=(($strip_color_data_arr[$row[csf('color')]]['measurement']*2.2046226)/12); 
                      //	echo $alocated_qty;die;
							if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";	
						?>
							<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $i; ?>,<? echo $row[csf('color')]; ?>)"> 
                                <td width="40">
                                    <? echo $i; ?>
                                     <input type="hidden" name="txt_individual_id" id="txt_individual_id<?php echo $i; ?>" value="<?php echo $row[csf('id')]."_".$row[csf('color')]."_".$color_library[$row[csf('color')]]."_".$row[csf('lot')]."_".$row[csf('product_name_details')]."_".$mesurement."_".$available_qty; ?>"/>
                                </td>
                                <td width="100"><p><? echo $row[csf('id')]; ?>&nbsp;</p></td>
                                <td width="100" style="word-break:break-all"><p><? echo $row[csf('lot')]; ?></p></td>
                                <td width="100" align="right"><? echo number_format($row[csf('stock_qty')],2); ?></td>
                                <td width="100" style="word-break:break-all"><? echo $color_library[$row[csf('color')]]; ?></td>
                                <td width="150" style="word-break:break-all"><? echo $row[csf('product_name_details')]; ?></td>
                                <td width="100" align="right"><? echo number_format($alocated_qty,2); ?></td>
                                
                                <td align="right"><? echo number_format($available_qty,2); ?></td>
                            </tr>
						<? 
							$i++;
...
if($action=="size_popup")
{
	...
	extract($_REQUEST);
   $country_is_blank_sql=sql_select("SELECT 
   										country_id 
									FROM 
									...
									where 
										status_active=1 AND 
										is_deleted=0 AND 
										(country_id is null or country_id ='' or country_id=0) AND  
										job_no='".$job_id."'");
										echo $details_id;
...
<script>
	...
	{ 
		...
		var total_distributed_qty=$("#total_size_qty").text()*1;
	...
	function fnc_yarn_lot_ratio_report()
	{
		var report_title="Cut and Lay bundle ";
	   	...
	   	var title = 'Bundle List';	
			...
			print_report(<? echo $cbo_company_id; ?>+'*'+<? echo $mst_id; ?>+'*'+<? echo $details_id; ?>+'*'+title, "report_yarn_lot_ratio", "yarn_lot_ratio_planning_controller")
	...
	{ 
			...
			var data="action=save_update_delete_bundle&operation="+operation+'&tot_row='+tot_row+'&bundle_dtls_id='+bundle_dtls_id+'&bundle_mst_id='+bundle_mst_id+dataString_bundle+'&hidden_cutting_no='+hidden_cutting_no+'&color_type_id='+cbo_color_type+"&gmt_id="+gmt_id;
...
<div id="msg_box_popp" style=" height:15px; width:200px;  position:relative; left:250px "></div>
	<div align="center" style="width:100%; overflow-y:hidden; position:absolute; top:5px;">
        ...
        <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
                ...
                <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="550" id="tbl_bundle_size">
                	<thead>
                        <tr>
                            ...
                            <th width="" rowspan="<?php echo count($yarn_ratio_form_msert); ?>">
                                	<input name="txt_bundle_pcs" id="txt_bundle_pcs" class="text_boxes_numeric" style="width:120px;" value="<? echo $pcs_per_bundle; ?>"/>	
                            </th>
                        </tr>
                    </thead>
                	<thead>
					...
					//echo $rollData;
						...
						?>
                            <tr>
                                <td width="280">
                                    <input type="text" style="width:260px" class="text_boxes"  name="txtColorName[]" id="txtColorName_<?php echo $yarn_color_sl; ?>" value="<? echo $single_yarn_data_arr[2]; ?>" disabled readonly/>
                           
                                </td>
                                <td width="140">
                                    <input type="text" style="width:125px" class="text_boxes" name="txtLotName[]" id="txtLotName_<?php echo $yarn_color_sl; ?>" value="<? echo $single_yarn_data_arr[0]; ?>" disabled readonly/>
                                </td>
                                <td width="120">
                                    <input type="text" style="width:120px" class="text_boxes_numeric" name="txtLotQty[]" id="txtLotQty_<?php echo $yarn_color_sl; ?>" value="<? echo $single_yarn_data_arr[5]; ?>" disabled readonly/>
                                    <input type="hidden" style="width:110px" class="text_boxes" name="txtLotRatio[]" id="txtLotRatio_<?php echo $yarn_color_sl; ?>" value="<? echo $single_yarn_data_arr[3]; ?>" disabled readonly/>
                                </td>
                            </tr>
						<?php	
						}
                    ...
                    <tfoot>
                        <tr class="form_table_header">
                            <th  colspan="2" align="right">Total
                              	<input type="hidden" name="totalLotQty" id="totalLotQty" class="text_boxes_numeric" style="width:120px;" value="<? echo $total_lot_qty; ?>"/>
                            	<input name="consumptionPerPcs" id="consumptionPerPcs" class="text_boxes_numeric" style="width:120px;" type="hidden" value="<? echo $total_consumption_per_pcs; ?>"/>
                            </th>
                            <th align="right"><? echo $total_lot_qty; ?></th>
                        </tr>
                    </tfoot>
                </table>
            </fieldset>
            ...
            <?
				$po_no_arr=return_library_array("select id, po_number from wo_po_break_down where job_no_mst='".$job_id."'",'id','po_number');
				$po_country_array=array(); $size_order_arr=array(); $poArr=array();
                $sql_query=sql_select("select 
										po_break_down_id, 
										country_type, 
										country_id, 
										size_number_id, 
									...
									from 
										wo_po_color_size_breakdown 
									where 
										item_number_id=$cbo_gmt_id and 
										job_no_mst='".$job_id."' and 
										color_number_id=$cbo_color_id and 
										status_active=1 and 
										is_deleted=0 
									order by 
										size_order,
                ...
                $size_wise_arr=array();
                $sizeWiseData=sql_select("select 
											size_ratio, 
											size_id, 
											marker_qty, 
											bundle_sequence 
										from 
											ppl_cut_lay_size_dtls 
										where 
											mst_id=".$mst_id." and 
											dtls_id=".$details_id." and 
											status_active=1");
                ...
                $lay_blData=sql_select("select 
											a.order_id, 
											sum(a.marker_qty) as marker_qty, 
											a.country_type, 
											a.country_id, 
											a.size_id 
										from 
										...
										where 
											...
											b.id=a.dtls_id and 
											m.job_no='".$job_id."'  
											and b.gmt_item_id=$cbo_gmt_id 
											and a.color_id=$cbo_color_id and 
											a.status_active=1 
										group by 
											a.order_id, 
											a.country_type,
				...
				$size_bl_qty_arr=return_library_array("select 
															sum(a.size_qty) as size_qty, 
															a.size_id 
														from 
														...
														where 
															...
															m.job_no='".$job_id."' and  
															b.id=a.dtls_id and 
															b.gmt_item_id=$cbo_gmt_id and 
															b.color_id=$cbo_color_id and 
															a.status_active=1  
														group by 
															a.size_id",'size_id','size_qty');

				$table_one_width=250+(50*(count($sizeId_arr)));
                ?>
                <fieldset style="width:<?php echo ($table_one_width+100); ?>px">
                <legend>Size Wise Ratio Distribution:</legend>
                <table cellpadding="0" class="rpt_table" cellspacing="0" width="<?php echo $table_one_width; ?>" id="tbl_size">
                    <thead class="">
                        ...
                        <?php 
							...
							foreach($sizeId_arr as $size_id=>$plan_cut_qty)
							{
								...
								?>
                                <th width="50"><?php echo $size_arr[$size_id];?></th>
                                <?php
							}
						?>
                    ...
                    <tbody>
                        <tr>
                        	...
                        	foreach($sizeId_arr as $size_id=>$plan_cut_qty)
							{
                            	...
                            	?>
                                <td align="center">				
                                    <input type="text" style="width:50px" class="text_boxes_numeric"  name="txt_layf_balance_<? echo $i; ?>" id="txt_layf_balance_<? echo $i; ?>"  value="<? echo $size_wise_lay_balance[$size_id]; ?>" disabled />
                                </td>
                                <?php
                            }
							?>
                            <td align="center">				
                                <input type="text" style="width:80px" class="text_boxes_numeric"  name="txt_total_layf_balance" id="txt_total_layf_balance"  value="<? echo $total_layf_balance; ?>" disabled />
                            </td>
                        </tr>
                        <tr>
                            ...
                            foreach($sizeId_arr as $size_id=>$plan_cut_qty)
                            {
                                ...
                                ?>
                                <td align="center">				
                                    <input type="text" style="width:50px" class="text_boxes_numeric"  name="txt_size_ratio_<? echo $i; ?>" id="txt_size_ratio_<? echo $i; ?>"  value="<? echo number_format(($plan_cut_qty/$total_plan_cut_qty),4); ?>" disabled />
                                </td>
                                <?php
                            }
                            ?>
                            <td align="center">				
                                <input type="text" style="width:80px" class="text_boxes_numeric"   disabled />
                            </td>
                       </tr>
                       <tr>
                            ...
                            {
                                $i++;
								//$total_consumption_per_pcs
                                ?>
                                <td align="center">				
                                    <input type="text" style="width:50px" class="text_boxes_numeric"  name="txt_size_yarn_wgt_<? echo $i; ?>" id="txt_size_yarn_wgt_<? echo $i; ?>"  value="<? echo number_format($siqz_wise_lot_wgt[$size_id],2,".",""); ?>" disabled />
                                </td>
                                
                                <?php
								$total_size_yarn_wgt+=$siqz_wise_lot_wgt[$size_id];
                            }
                            ?>
                            <td align="center">				
                                <input type="text" id="total_size_yarn_wgt" style="width:80px" class="text_boxes_numeric" value="<?php echo $total_size_yarn_wgt; ?>"   disabled />
                            </td>
                       </tr>
                       <tr>
                            ...
                            foreach($sizeId_arr as $size_id=>$plan_cut_qty)
                            {
                                ...
                                ?>
                                <td align="center">				
                                    <input type="text" style="width:50px" class="text_boxes_numeric"  name="txt_sizef_qty_<? echo $i; ?>" id="txt_sizef_qty_<? echo $i; ?>"  value="<? echo $siqz_wise_lot_qty[$size_id]; ?>" disabled />
                                    <input type="hidden" style="width:50px" class="text_boxes_numeric"  name="hidden_sizef_id_<? echo $i; ?>" id="hidden_sizef_id_<? echo $i; ?>"  value="<? echo $size_id; ?>" disabled />
                                    
                                </td>
                                <?php
								$total_size_yarn_qty+=$siqz_wise_lot_qty[$size_id];
                            }
                            ?>
                            <td align="center">				
                                <input type="text" id="total_sizef_qty" style="width:80px" class="text_boxes_numeric" value="<?php echo $total_size_yarn_qty; ?>"   disabled />
                            </td>
                       </tr>
                    </tbody>
                </table>
								...
								foreach($country_val as $country_id=>$size_data)
								{
									...
									?>
										<tr id="gsd_<? echo $i; ?>">
											...
											<td align="center">				
												<input type="text" style="width:50px" class="text_boxes_numeric"  name="txt_excess_<? echo $i; ?>" id="txt_excess_<? echo $i; ?>" value="<? echo $data[4]; ?>" disabled/>	
                                               <!-- onKeyUp="copy_perc(<? echo $i; ?>);"-->
											</td>
											<td align="center">				
												<input type="text" style="width:80px" class="text_boxes_numeric" name="txt_size_qty_<? echo $i; ?>" id="txt_size_qty_<? echo $i; ?>" onKeyUp="calculate_perc(<? echo $i; ?>);" onBlur="check_size_qty(<? echo $i; ?>);"  value="<? echo $marker_qty; ?>"  />
        ...
        <form name="searchorderfrm_2"  id="searchorderfrm_2" autocomplete="off">    
            ...
            <div id="search_div" style="margin-top:10px">
                <?
				$sql_size_name=sql_select("select 
											size_id 
										from 
											ppl_cut_lay_size 
										where 
                            ...
                            ?>
                                <tr id="trBundleListSave_<? echo $i;  ?>">
                                    <td align="center" id="">
                                        ...
                                        <input type="hidden" name="isExcess[]" id="isExcess_<? echo $i; ?>" value="<? echo $row[csf('is_excess')]; ?>"/>
                                    </td>
                                    <td align="center">	
                                        <?
...
{  
	...
	if ($operation==1)  // Insert Here=======================================================
	{
		...
		$field_array="id,mst_id,dtls_id,size_id,bundle_num_prefix,bundle_num_prefix_no,bundle_no,barcode_year,barcode_prifix,barcode_no,number_start,number_end,size_qty,update_flag,update_value,country_type,country_id,is_excess,order_id,color_type_id,inserted_by,insert_date,status_active,is_deleted";
		...
		{ 	
			...
			if($data_array!="") $data_array.=",";
			 $data_array.="(".$id.",".$bundle_mst_id.",".$bundle_dtls_id.",".$$new_bundle_size_id.",'".$new_bundle_prifix."','".$new_bundle_prif_no[0]."','".$$new_bundle_no."','".$up_barcode_year."','".$up_barcode_suffix."','".$barcode_no."','".$$new_bundle_from."','".$$new_bundle_to."','".$$new_bundle_qty."',".$update_flag.",'".$update_flag_value."','".str_replace("'","",$$hiddenCountryType)."','".str_replace("'","",$$hiddenCountry)."','".str_replace("'","",$$isExcess)."','".str_replace("'","",$$cboPoId)."',".$color_type_id.",'".$user_id."','".$pc_date_time."',1,0)";
			if(count($sql_sewing_operation)>0)
			{
				foreach ($sql_sewing_operation as $op_val) {
					...
					$data_array_bundle_operation.="(".$id_operation.",".$id.",".$bundle_dtls_id.",'".$barcode_no."',".$op_val[csf('id')].",'".$op_val[csf('operation_name')]."','".$op_val[csf('code')]."','".$operation_barcode_no."','".$up_operation_suffix."','".$up_operation_year."',1,0)";
					$id_operation++;
				}
			}
			$id = $id+1;
...
{	
	...
	foreach($color_sizeID_arr as $val)
	{
		...
		$html.='<br/><table cellpadding="0" cellspacing="0" width="985" class="ff" style="border:1pt solid white;margin-bottom:8px;" rules="all" id="">
			    ...
			    <table cellpadding="0" cellspacing="0" width="985" class=""  rules="all" id="" style="margin-top: 2px;">';
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		{
			$bundleData=sql_select("select 
										max(a.number_end) as last_rmg, 
										max(a.bundle_num_prefix_no) as last_prefix 
									from 
										ppl_cut_lay_bundle a, 
										ppl_cut_lay_mst b 
									where 
		...
		$rID3=true;
		//echo "10**insert into ppl_cut_lay_bundle_operation($feild_array_operation) values".$data_array_bundle_operation;die;
...
//echo "10**".count($sql_sewing_operation); die;
		...
		//echo "10**".$rID ."**". $rID_size ."**". $rID2 ."**". $rID4 ."**". $delete ."**". $delete_size."**".$delete_bundle;die;	
...
if($action=="cut_lay_bundle_print")
{
	...
	$po_arr=return_library_array( "select id, po_number from wo_po_break_down where job_no_mst='".$dataArray[0][csf('job_no')]."'","id","po_number" );
	...
	<div style="width:1200px;">
    	<table align="center" cellspacing="0" width="1180" border="1" rules="all" class="rpt_table" >
              ...
              <thead bgcolor="#dddddd" align="center">
               ...
               <tr bgcolor="#BBBBBB">
                   <td align="center"></td>
                   <td  colspan="8"  align="right"> Total Ratio Qty.</td>
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		$field_array1="id, mst_id,color_type_id,order_cut_no,color_id,gmt_item_id,plies,order_qty,roll_data,inserted_by,insert_date,status_active,is_deleted";
		...
		for($i=1; $i<=$row_num; $i++)
		{
			...
			$data_array1.="(".$detls_id.",".$id.",".$$cboColorType.",".$$order_cut_no.",".$$cbocolor.",".$$cbo_gmt_id.",".$$txt_plics.",".$$order_qty.",".$$rollData.",'".$user_id."','".$pc_date_time."',1,0)";   
...
*/	
		...
		$detls_id= return_next_id_by_sequence(  "ppl_cut_lay_dtls_seq",  "ppl_cut_lay_dtls", $con );
		$field_array1="id, mst_id,color_type_id,order_cut_no,color_id,gmt_item_id,plies,order_qty,roll_data,inserted_by,insert_date,status_active,is_deleted";
		$field_array_up="color_type_id*order_cut_no*color_id*gmt_item_id*plies*order_qty*roll_data*updated_by*update_date";
		//**********************************************************************
		...
		//	print_r($previous_inserted_yarn);die;
			...
			if($update_details_id!="")  
			{
				...
				$data_array_up[$update_details_id]=explode("_",("".$$cboColorType."_".$$order_cut_no."_".$$cbocolor."_".$$cbo_gmt_id."_".$$txt_plics."_".$$order_qty."_".$$rollData."_'".$user_id."'_'".$pc_date_time."'_1_0"));
				
				if ($add_comma!=0) $detls_id_array .="_";
				$detls_id_array.=$update_details_id."#".str_replace("'",'',$$order_cut_no);
				$add_comma++;
			}
			else
			{
	...
	else if ($operation==2) // Delete Here----------------------------------------------------------
	{
		...
		{
			echo "500**".$sql_issue_number[0][csf("issue_number")];die;
		}
		$field_array_product = "available_qnty*allocated_qnty";
		$con = connect();		
		if($db_type==0)	{ mysql_query("BEGIN"); }
...
<div align="center" style="width:100%; overflow-y:hidden;" >
	...
	<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
		<?php
			$sql="select a.sizeset_no,a.extention_no FROM ppl_size_set_mst a,ppl_size_set_dtls b where a.id=b.mst_id and a.job_no='".$txt_job_no."' and b.color_id=".$cbocolor." and  a.status_active=1 and a.is_deleted=0 and  b.status_active=1 and b.is_deleted=0 group by a.sizeset_no,a.extention_no";
			//echo $sql;
	...
	foreach($sql_dtls as $val)
	{
	   ...
	   <tr class="" id="tr_<? echo $tbl_row; ?>" style="height:10px;">
       		<td align="center" id="color_<? echo $tbl_row; ?>">
				 <? 
					 echo create_drop_down( "cbocolor_".$tbl_row, 100, $color_item_arr,"", 1, "select color", $val[csf('color_id')], "reset_fld(".$tbl_row.")",1);
				 ?>
			</td>                             
			<td align="center" id="garment_<? echo $tbl_row; ?>">
				 <? 
					 echo create_drop_down( "cbogmtsitem_".$tbl_row, 120, $garments_item,"", 1, "-- Select Item --", $val[csf('gmt_item_id')], "",1,$gmt_item_id);
				 ?>
			</td>
			...
			<td align="center">
				   <input type="text" name="txtplics_<? echo $tbl_row; ?>"  id="txtplics_<? echo $tbl_row; ?>" class="text_boxes_numeric" style="width:60px" value="<? echo $val[csf('plies')];?>" placeholder="Double Click" onDblClick="openmypage_lot(<? echo $tbl_row; ?>)" readonly/>
