...
{	
	...
	$sql_name=sql_select("select b.buyer_name, b.client_id, b.style_ref_no, b.product_dept, b.season_buyer_wise as season_matrix, a.po_number, a.id as po_id from wo_po_details_master b,wo_po_break_down a where a.job_no_mst=b.job_no and a.id in(".$data[6].")");
	$po_data_arr=array();
	foreach($sql_name as $value)
	{
		$po_data_arr[$value[csf('po_id')]]["style_ref_no"]=$value[csf('style_ref_no')];
	...
	unset($sql_name);
	//echo "select roll_id, batch_no, shade from pro_roll_details where mst_id='$data[2]' and entry_form=99 and status_active=1";
	$roll_sql=sql_select("select roll_id, batch_no, shade from pro_roll_details where mst_id='$data[2]' and entry_form=99 and status_active=1");
	$roll_data_arr=array();
	foreach($roll_sql as $row)
	{
...
{	
	...
	$roll_sql=sql_select("select roll_id, batch_no, shade from pro_roll_details where entry_form=99 and status_active=1");
	$roll_data_arr=array();
	foreach($roll_sql as $row)
	{
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		for($i=1; $i<=$row_num; $i++)
		{
			...
			for($x=0;$x<count($save_string);$x++)
			{
				...
				$data_array_roll.="(".$id_roll.",".$barcode_no.",".$id.",".$detls_id.",".$$cbo_order_id.",99,'".$roll_qnty."','".$roll_id."','".$roll_no."','".$plies."','".$batchNo."','".$shade."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			...
			for($i=1; $i<=$row_num; $i++)
				{
					...
					for($x=0;$x<count($save_string);$x++)
					{
						...
						if($data_array_roll!="") $data_array_roll.= ",";
						$data_array_roll.="(".$id_roll.",".$barcode_no.",".$msster_id.",".$dtlsId.",".$$cbo_order_id.",99,'".$roll_qnty."','".$roll_id."','".$roll_no."','".$plies."','".$batchNo."','".$shade."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
						$response_data.=$barcode_no."=".$roll_no."=".$roll_id."=".$roll_qnty."=".$plies."=".$batchNo."=".$shade."**";
						//$id_roll = $id_roll+1;
					}
