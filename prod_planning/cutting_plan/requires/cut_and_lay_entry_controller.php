...
{    
     $data=explode("**",$data);
	 $sql="select distinct c.id,c.buyer_name from  wo_po_break_down a,wo_po_details_master b,lib_buyer c where  a.job_id=b.id and b.company_name=".$data[2]."  and job_no_prefix_num='".$data[0]."' and $insert_year='".$data[1]."' and b.buyer_name=c.id and a.status_active=1";
...
{    
     $data=explode("**",$data);
	 $sql="select distinct b.job_no from  wo_po_break_down a,wo_po_details_master b where  a.job_id=b.id and b.company_name=".$data[2]."  and job_no_prefix_num='".$data[0]."' and $insert_year='".$data[1]."' and  a.status_active=1 and  b.status_active=1";
	$result=sql_select($sql);
	foreach($result as $val)  
	{
...
if ($action=="load_drop_down_order_garment")
{
	...
	$gmt_item_arr=return_library_array( "select a.gmts_item_id from  wo_po_details_master a, wo_po_break_down b where a.id=b.job_id and b.id=".$ex_data[0]." and a.status_active=1",'id','gmts_item_id');
...
if ($action=="load_drop_down_color")
{
	...
	{ 
		echo create_drop_down( "cbocolor_$ex_data[2]", 130, "select distinct a.id,a.color_name from lib_color a,wo_po_color_size_breakdown b where a.id=b.color_number_id and b.po_break_down_id=".$ex_data[0]." and item_number_id=".$ex_data[1]." and b.status_active=1","id,color_name", 1, "select color",$color_item_id, "change_marker(this.id,this.value)");
		exit();
	}
	...
	{
		echo create_drop_down( "cbocolor_$ex_data[2]", 130, "select distinct a.id,a.color_name from lib_color a,wo_po_color_size_breakdown b where a.id=b.color_number_id and b.po_break_down_id=".$ex_data[0]." and item_number_id=".$ex_data[1]." and b.status_active=1","id,color_name", 1, "select color", $selected, "change_marker(this.id,this.value)");	
	}
	exit();		
}
...
if($action=="size_popup")
{
	...
	<script>
			...
			for(var i=1; i<=row_num; i++)
			{
				...
				if(size_id_curr==size_id)
				{
					if(balance>0)
					{
						...
						$("#txt_size_ratio_"+i).val(size_ratio);
						$("#txt_size_qty_"+i).val(marker_qty_curr);
						// $("#txt_size_qty_"+i).val(marker_qty);
					}
					else
					{
		...
		{   
			...
			if(size_wise_repeat_cut_no==1)
			{
				...
				for(var k=1; k<=size_row_num; k++)
				{
					...
					size_data += '&txt_layf_balance_' + k + '=' + $('#txt_layf_balance_' + k).val() + '&txt_sizef_ratio_' + k + '=' + $('#txt_sizef_ratio_' + k).val() + '&txt_sizef_qty_' + k + '=' + $('#txt_sizef_qty_' + k).val() + '&hidden_sizef_id_' + k + '=' + $('#hidden_sizef_id_' + k).val();
				}
				...
				for(var k=1; k<=row_num; k++)
				{
					...
					data2 += '&cboCountryType_' + k + '=' + $('#cboCountryType_' + k).val() + '&cboCountry_' + k + '=' + $('#cboCountry_' + k).val() + '&txt_lay_balance_' + k + '=' + $('#txt_lay_balance_' + k).val() + '&txt_size_ratio_' + k + '=' + $('#txt_size_ratio_' + k).val() + '&txt_size_qty_' + k + '=' + $('#txt_size_qty_' + k).val() + '&txt_bundle_' + k + '=' + $('#txt_bundle_' + k).val() + '&hidden_size_id_' + k + '=' + $('#hidden_size_id_' + k).val() + '&update_size_id_' + k + '=' + $('#update_size_id_' + k).val();
				}
			}
			else
			{
				for(var k=1; k<=row_num; k++)
				{
					...
					data2 += '&txt_lay_balance_' + k + '=' + $('#txt_lay_balance_' + k).val() + '&txt_size_ratio_' + k + '=' + $('#txt_size_ratio_' + k).val() + '&txt_size_qty_' + k + '=' + $('#txt_size_qty_' + k).val() + '&txt_bundle_' + k + '=' + $('#txt_bundle_' + k).val() + '&hidden_size_id_' + k + '=' + $('#hidden_size_id_' + k).val() + '&update_size_id_' + k + '=' + $('#update_size_id_' + k).val();
				}	
			}
		...
		{ 
				...
				//alert(bundle_dtls_id);return;
				var hidden_cutting_no=$("#hidden_cutting_no").val();
				var data="action=save_update_delete_bundle_country&operation="+operation+'&tot_row='+tot_row+'&bundle_dtls_id='+bundle_dtls_id+'&bundle_mst_id='+bundle_mst_id+'&order_id='+order_id+dataString_bundle+'&hidden_cutting_no='+hidden_cutting_no;
				//alert(data);return;//hidden_cutting_no
				freeze_window(operation);
				http.open("POST","cut_and_lay_entry_controller.php",true);
				http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		...
		function fnc_bundle_report_qr_code(type)
		{
			...
			emailwindow.onclose=function()
			{
				...
				if (type==1) {
					...
					http.open( 'POST', 'cut_and_lay_entry_controller.php?action=print_qrcode_operation2&data='+ data );
				}
		...
		if($size_wise_repeat_cut_no==1)
		{
			...
			<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
					...
					<table cellpadding="0" cellspacing="0" width="370" id="tbl_size">
                        ...
                        <?
							...
							foreach($sizeId_arr as $size_id=>$plan_cut_qty)
							{
								...
								// echo $size_id."=".$lay_balance."*".$plan_cut_qty."-".$lay_bl_qty_size_arr[$size_id]."+".$size_wise_arr[$size_id]['marker_qty']."<br>";
                        ...
                        <tfoot>
                            ...
                            <tr>
                                <td align="center"  colspan="7" >
                                    ...
                                    <input type="button" id="btn_stiker_qr" name="btn_stiker_qr" value="QR" class="formbutton" onClick="fnc_bundle_report_qr_code(1);"/>
                                    <input type="button" id="btn_stiker_qr2" name="btn_stiker_qr2" value="Without QR" class="formbutton" onClick="fnc_bundle_report_qr_code(2);"/>
                ...
                <div id="search_div" style="margin-top:10px">
                    ...
                    $bundle_data=sql_select("SELECT min(a.id) as id, a.bundle_no, a.barcode_no, a.size_id, a.number_start, a.number_end, a.size_qty, a.update_flag, a.update_value, a.country_type, a.country_id from ppl_cut_lay_bundle a, wo_po_color_size_breakdown b where a.size_id=b.size_number_id and a.country_id=b.country_id and a.order_id=b.po_break_down_id and a.mst_id=".$mst_id." and a.dtls_id=".$details_id." and a.status_active=1 and a.is_deleted=0 group by a.id, a.bundle_no, a.barcode_no, a.size_id, a.number_start, a.number_end, a.size_qty, a.update_flag, a.update_value, a.country_type, a.country_id, b.size_order order by b.size_order, id ASC");
					// echo "SELECT min(a.id) as id, a.bundle_no, a.barcode_no, a.size_id, a.number_start, a.number_end, a.size_qty, a.update_flag, a.update_value, a.country_type, a.country_id from ppl_cut_lay_bundle a, wo_po_color_size_breakdown b where a.size_id=b.size_number_id and a.country_id=b.country_id and a.order_id=b.po_break_down_id and a.mst_id=".$mst_id." and a.dtls_id=".$details_id." group by a.id, a.bundle_no, a.barcode_no, a.size_id, a.number_start, a.number_end, a.size_qty, a.update_flag, a.update_value, a.country_type, a.country_id, b.size_order order by b.size_order, id ASC";
                            ...
                            <table cellpadding="0" cellspacing="0" width="780" class="rpt_table" id="tbl_bundle_list_save">
                                ...
                                ?>
                                    <tr id="trBundleListSave_<? echo $i;  ?>">
                                        ...
                                        <input type="text" name="bundleNo[]" id="bundleNo_<? echo $i;  ?>" value="<? echo $row[csf('bundle_no')];  ?>" class="text_boxes"  style="width:90px;  text-align:center" disabled/>
                                        </td>
                                        <td align="center">
...
{  
	...
	{	
		...
		$cutting_qc_no= return_field_value("cutting_qc_no"," pro_gmts_cutting_qc_mst","cutting_no='".$hidden_cutting_no."' and status_active=1 and is_deleted=0 ");
		...
		$field_array="id,mst_id,dtls_id,size_id,bundle_num_prefix,bundle_num_prefix_no,bundle_no,number_start,number_end,size_qty,update_flag,update_value,country_type,country_id,order_id,barcode_year,barcode_prifix,barcode_no,inserted_by,insert_date,status_active,is_deleted";
		...
		for($j=1;$j<=$tot_row;$j++)
			{ 	
				...
				$data_array.="(".$id.",".$bundle_mst_id.",".$bundle_dtls_id.",".$$new_bundle_size_id.",'".$new_bundle_prifix."','".$new_bundle_prif_no[0]."','".$$new_bundle_no."',".$$new_bundle_from.",".$$new_bundle_to.",".$$new_bundle_qty.",".$update_flag.",'".$update_flag_value."','".str_replace("'","",$$hiddenCountryType)."','".str_replace("'","",$$hiddenCountry)."',".$order_id.",'".$up_barcode_year."','".$up_barcode_suffix."','".$barcode_no."','".$user_id."','".$pc_date_time."',1,0)";
				$id = $id+1;
			}
		//echo $data_array;die;	
...
if($action=="report_bundle_printer")
{
	...
	$sql_name=sql_select("select b.buyer_name,b.style_ref_no,b.product_dept,a.po_number from wo_po_details_master b,wo_po_break_down a 
	                      where a.job_id=b.id and a.id=$data[5]");
...
if($action=="report_bundle_text_file")
{
	...
	$color_sizeID_arr=sql_select( "select id,size_id,bundle_no,number_start,number_end,size_qty from ppl_cut_lay_bundle where id in ( $data[0] ) " );  //where id in ($data)
    $bundle_array=array();
	 $sql_name=sql_select("select b.buyer_name,b.style_ref_no,b.product_dept,a.po_number from wo_po_details_master b,wo_po_break_down a where a.job_id=b.id and a.id=$data[6]");
	 foreach($sql_name as $value)
	 {
		 $product_dept_name=$value[csf('product_dept')];
		 $style_name=$value[csf('style_ref_no')];
...
if($action=="print_barcode_one")
{
	...
	//echo $total_cut_qty;die;
	 $sql_name=sql_select("select b.buyer_name, b.style_ref_no, b.product_dept, b.job_no, a.po_number from wo_po_details_master b,wo_po_break_down a where a.job_id=b.id and a.id=$data[6]");
	 foreach($sql_name as $value)
	 {
		 $product_dept_name=$value[csf('product_dept')];
		 $style_name=$value[csf('style_ref_no')];
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if (strlen($year_id)==4) $year_id=substr($year_id,2,2);
		$barcode_suffix_no=return_field_value("max(barcode_prifix) as suffix_no","ppl_cut_lay_bundle","barcode_year=$year_id","suffix_no");
		// $barcode_suffix_no = explode("*", return_next_id_by_sequence("LAY_BARCODE_SUFFIX_NO_SEQ", "ppl_cut_lay_bundle",$con,1,0,'',2,date("Y",time()),13 ));
		// echo "10**";print_r($barcode_suffix_no);die();
		...
		if($size_wise_repeat_cut_no==1)
		{
			...
			$sql = "SELECT barcode_no from ppl_cut_lay_bundle where status_active=1 and is_deleted=0 and barcode_year=$year_id and barcode_no in($barcodes)";
			// echo "786**$sql";die();
			$res = sql_select($sql);
			if(count($res)>0)
			{
	...
	?>
        <fieldset style="width:800px">
            ...
            <table cellpadding="0" cellspacing="0" width="780" class="rpt_table" id="tbl_bundle_list_save">
                ...
                <tbody id="trBundleListSave">
                <?
                $sql_size_name=sql_select("SELECT size_id from ppl_cut_lay_size where mst_id=".$mst_id." and dtls_id=".$dtls_id." and is_deleted=0");
                $size_colour_arr=array();
                foreach($sql_size_name as $asf)
                {
                    $size_colour_arr[$asf[csf("size_id")]]=$size_arr[$asf[csf("size_id")]];	
                ...
                $bundle_data=sql_select("SELECT a.id,a.bundle_no,a.barcode_no,a.size_id,a.number_start,a.number_end,a.size_qty,a.update_flag,a.update_value, a.country_type, a.country_id from ppl_cut_lay_bundle a where a.mst_id=".$mst_id." and a.dtls_id=".$dtls_id." and a.is_deleted=0 order by a.id ASC");
                $i=1; 
                foreach($bundle_data as $row)
                { 
                    $update_f_value=""; 
                ...
                <table cellpadding="0" cellspacing="0" width="700" class="rpt_table" id="tbl_bundle_list_save">
                            ...
                            <?
                               ...
                               foreach($sql_size_name as $asf)
                                ...
                                $bundle_data=sql_select("SELECT DISTINCT a.id,a.bundle_no,a.size_id,a.number_start,a.number_end,a.size_qty,a.update_flag,a.update_value from ppl_cut_lay_bundle a, ppl_cut_lay_size b where a.mst_id=b.mst_id and a.dtls_id=b.dtls_id and a.size_id=b.size_id and a.mst_id=".$mst_id." and a.dtls_id=".$dtls_id." and a.is_deleted=0 and b.is_deleted=0 order by a.id ASC");
                                 $i=1; 
                                  foreach($bundle_data as $row)
                                   { 
                                      $update_f_value=""; 
...
if($action=="cut_lay_bundle_print")
{
	...
	$ful_cut_no=$dataArray[0][csf('cutting_no')];
	...
	// $matrixPointSize = 4;
		...
		QRcode::png($ful_cut_no, $file, $ecc, $pixel_Size, $frame_size); 

	// $filename = $PNG_TEMP_DIR.'test'.md5($val[csf("cutting_no")]).'.png';
	// QRcode::png($val[csf("cutting_no")], $filename, $errorCorrectionLevel, $matrixPointSize, 2);
	$cut_array[$i]=$val[csf("cutting_no")];
	?>
	     ...
	     <div style="width:1000px;">
			...
			if($data[4]==1)
			{
				...
				<table align="center" cellspacing="0" width="700"  border="1" rules="all" class="rpt_table" >
	                ...
	                $size_data=sql_select("SELECT distinct size_id,marker_qty as sqty from ppl_cut_lay_size_dtls where mst_id='$data[1]' and dtls_id='$data[2]' and status_active=1 and is_deleted=0 order by size_id ASC"); //order by id ASC   
	                // $size_data=sql_select("SELECT distinct a.size_id,a.marker_qty as sqty,b.sequence from ppl_cut_lay_size_dtls a,lib_size b where a.size_id=b.id and a.mst_id='$data[1]' and a.dtls_id='$data[2]' and a.status_active=1 and a.is_deleted=0 order by b.sequence "); //order by id ASC 
	                $j=1;
	                foreach($size_data as $size_val)
	                {
	              		$total_marker_qty_size=0;
	                    $bundle_data=sql_select("SELECT DISTINCT a.id,a.bundle_no as bundle_no,a.size_id,a.number_start,a.number_end,a.size_qty,a.country_id from ppl_cut_lay_bundle a where a.mst_id='$data[1]' and a.dtls_id='$data[2]' and a.size_id=".$size_val[csf('size_id')]." and a.status_active=1 and a.is_deleted=0  order by a.id ASC");
	                    // echo "SELECT DISTINCT a.id,a.bundle_no as bundle_no,a.size_id,a.number_start,a.number_end,a.size_qty,a.country_id from ppl_cut_lay_bundle a where a.mst_id='$data[1]' and a.dtls_id='$data[2]' and a.size_id=".$size_val[csf('size_id')]."  order by a.id ASC<br>";
	                    foreach($bundle_data as $row)
	                    { 
	                		...
	                		?>
	                           <tr>
	                               ...
	                               <td align="center"><? echo $bundle_prifix[3];if(isset($bundle_prifix[4])){echo "-".$bundle_prifix[4];} // $j;  ?></td>
	              ...
	              <thead bgcolor="#dddddd" align="center">
	                ...
	                ?>
	                           <tr>
	                               <td align="center"><? echo $j;  ?></td>
	                                <td align="center"><? echo str_replace("0","",$dataArray[0][csf('cutting_no')]);   ?></td>
	                               <td align="center"><? echo $j;//$bundle_prifix[2];  ?></td>
	                               <td align="center"><? echo $row[csf('size_qty')];  ?></td>
	                               <td align="center"><? echo $cut_no_prifix."-".$row[csf('number_start')];  ?></td>
	                               <td align="center"><? echo $cut_no_prifix."-".$row[csf('number_end')];  ?></td>
	                               <td align="center"><? echo $size_arr[$row[csf('size_id')]];  ?></td>
...
if($action=="create_job_search_list_view")
{
	...
	if($job_prifix=="" && $po_no=="" && $from_date=="")
	{
		?>
		<div style="color: red;font-weight:bold;text-align:center;font-size:16px;">Please enter value of job, order or date range field.</div>
		<?
		die;
	}
	$job_cond="";
...
{	 
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
	    ...
	    $field_array1="id, mst_id,order_id,order_cut_no,ship_date,color_id,gmt_item_id,plies,order_qty,roll_data,inserted_by,insert_date,status_active,is_deleted,extra_roll_data,country_ids";
		$field_array_up="order_id*order_cut_no*ship_date*color_id*gmt_item_id*plies*order_qty*roll_data*updated_by*update_date*extra_roll_data,country_ids";
		...
		//$new_grey_recv_system_id = explode("*", return_next_id_by_sequence("INV_RECEIVE_MASTER_PK_SEQ", "inv_receive_master",$con,1,$cbo_company_id,'GPE',2,date("Y",time()),13 ));
				...
				if(str_replace("'",'',$$update_details_id)!="")  
				{
					...
					$data_array_up[str_replace("'",'',$$update_details_id)]=explode("*",("".$$cbo_order_id."*".$$cbo_order_id."*".$$txt_ship_date."*".$$cbocolor."*".$$cbo_gmt_id."*".$$txt_plics."*".$$order_qty."*'".$response_data."'*'".$user_id."'*'".$pc_date_time."'*'".$extra_response_data."'*".$$country_id.""));
				...
				else
				{
				   ...
				   if ($add_comma!=0) $detls_id_array .="_";
					$data_array1.="(".$detls_id.",".$master_id.",".$$cbo_order_id.",".$$order_cut_no.",".$$txt_ship_date.",".$$cbocolor.",".$$cbo_gmt_id.",".$$txt_plics.",".$$order_qty.",'".$response_data."','".$user_id."','".$pc_date_time."',1,0,'".$extra_response_data."',".$$country_id.")";   
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	  {
		    ...
		    //$detls_id=return_next_id("id", " ppl_cut_lay_dtls", 1);
		    $detls_id= return_next_id_by_sequence(  "ppl_cut_lay_dtls_seq",  "ppl_cut_lay_dtls", $con );
			$field_array1="id, mst_id,order_id,order_cut_no,ship_date,color_id,gmt_item_id,country_ids,plies,order_qty,roll_data,inserted_by,insert_date,status_active,is_deleted,extra_roll_data";
			$field_array_up="order_id*order_cut_no*ship_date*color_id*gmt_item_id*country_ids*plies*order_qty*roll_data*updated_by*update_date*extra_roll_data";
			$add_comma=0;
			//$id_roll = return_next_id( "id", "pro_roll_details", 1 );
			$order_cut_no_arr=return_library_array( "select order_id,max(order_cut_no) as order_cut_no from ppl_cut_lay_dtls group by order_id", "order_id", "order_cut_no"  );
			$field_array_roll="id, barcode_no, mst_id, dtls_id, po_breakdown_id, entry_form, qnty, roll_id, roll_no, plies, inserted_by, insert_date,is_extra_roll";
 					...
 					if(str_replace("'",'',$$update_details_id)!="")  
					  {
						//$field_array_up="order_id*order_cut_no*ship_date*color_id*gmt_item_id*plies*order_qty*roll_data*updated_by*update_date*extra_roll_data";
							...
							$updateID_array[]=str_replace("'",'',$$update_details_id); 
							$data_array_up[str_replace("'",'',$$update_details_id)]=explode("*",("".$$cbo_order_id."*".$$order_cut_no."*".$$txt_ship_date."*".$$cbocolor."*".$$cbo_gmt_id."*".$$country_id."*".$$txt_plics."*".$$order_qty."*'".$response_data."'*'".$user_id."'*'".$pc_date_time."'*'".$extra_response_data."'"));
					 ...
					 else
					  {
						  //	$order_cut_no=$order_cut_no_arr[str_replace("'","",$$cbo_order_id)]+1;
							...
							if ($add_comma!=0) $data_array1 .=",";
							if ($add_comma!=0) $detls_id_array .="_";
							$data_array1.="(".$detls_id.",".$msster_id.",".$$cbo_order_id.",".$$order_cut_no.",".$$txt_ship_date.",".$$cbocolor.",".$$cbo_gmt_id.",".$$country_id.",".$$txt_plics.",".$$order_qty.",'".$response_data."','".$user_id."','".$pc_date_time."',1,0,'".$extra_response_data."')";   
							$detls_id_array.=$detls_id."#".str_replace("'",'',$$order_cut_no);
							//$detls_id=$detls_id+1;
							$detls_id= return_next_id_by_sequence(  "ppl_cut_lay_dtls_seq",  "ppl_cut_lay_dtls", $con );
							$add_comma++;
			...
			// echo "10**".bulk_update_sql_statement("ppl_cut_lay_dtls","id",$field_array_up,$data_array_up,$updateID_array);
...
if($action=="load_php_mst_form")
{
	...
	if($db_type==2){ $insert_year="extract(year from b.insert_date) as year";}
			$sql=sql_select("select distinct c.id,c.buyer_name,$insert_year from  wo_po_break_down a,wo_po_details_master b,lib_buyer c where  a.job_id=b.id and b.job_no='".$val[csf("job_no")]."' and b.buyer_name=c.id and a.status_active=1");
...
if($action=="order_details_list")
{
	$country_arr=return_library_array( "select id, country_name from lib_country",'id','country_name');
	$tbl_row=0;
	$sql_dtls=sql_select("SELECT a.id,a.order_id,a.ship_date,a.color_id,a.gmt_item_id,a.plies,a.marker_qty,a.order_qty,a.total_lay_qty,a.lay_balance_qty,b.job_no,b.job_year,b.company_id, a.order_cut_no ,a.roll_data,a.extra_roll_data,a.country_ids  from ppl_cut_lay_dtls a, ppl_cut_lay_mst b where b.id=a.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and mst_id=".$data." and b.entry_form=76");
	foreach($sql_dtls as $val)
	{
		...
		// $sql_marker="select sum(marker_qty) as mark_qty from  ppl_cut_lay_dtls where order_id=".$val[csf("order_id")]." and gmt_item_id=".$val[csf("gmt_item_id")]." and color_id=".$val[csf("color_id")]." and status_active=1 $count_id_cond2 group by order_id,gmt_item_id,color_id ";
       	...
       	<tr class="" id="tr_<? echo $tbl_row; ?>" style="height:10px;">
            <td align="center" id="order_id">
				<?   
					...
					{
					  $sql="SELECT a.id,a.po_number from  wo_po_break_down a,wo_po_details_master b where  a.job_id=b.id and b.company_name=".$val[csf('company_id')]."  and job_no_prefix_num='".$update_job_id[0]."' and SUBSTRING_INDEX(b.insert_date, '-', 1)='".$val[csf('job_year')]."' and a.status_active=1"; 
		...
		<fieldset style="width:400px;margin-left:10px">
	        ...
	        <form name="searchprocessfrm_1"  id="searchprocessfrm_1" autocomplete="off">
	            ...
	            <div style="width:390px; overflow-y:scroll; max-height:280px;" id="buyer_list_view" align="center">
	                ...
	                <?
	                    $i=1;
	                    $sql_country="select a.country_id,b.country_name  from  wo_po_color_size_breakdown a,lib_country b where a.country_id=b.id and  a.po_break_down_id=".$order_id." and a.item_number_id=".$gmt_id." and a.color_number_id=".$color_value." and a.status_active=1   ";
	                   // echo $sql_country;die;
						...
						//$country_id_arr=explode(",",$hidden_country_id);
	                    ...
	                    foreach($country_id_arr as $country_id)
	                    {
							...
							<tr bgcolor="<?=$bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<?=$country_id;?>" onClick="js_set_value(<?=$country_id;?>)"> 
								<td width="40" align="center"><?=$i; ?>
									...
									<input type="hidden" id="txt_individual<?=$country_id; ?>" name="txt_individual<?=$country_id; ?>" value="<?=$country_name_arr[$country_id]; ?>" />
								</td>	
								<td width="200"><p><?=$country_name_arr[$country_id]; ?></p></td>
							</tr>
							<?
							$i++;
	                    }
	                    ...
	                    <td align="center" height="30" valign="bottom">
	                        <div style="width:100%"> 
	                            ...
	                            <div style="width:50%; float:left" align="left">
	                                ...
	                                <input type="hidden" name="txt_po_row_id" id="txt_po_row_id" value="<?php echo $hidden_country_id; ?>"/>
	                            </div>
	                        </div>
	                    </td>
	                </tr>
...
if($action=="cut_lay_entry_report_print")
{
	...
	$data=explode('*',$data);
	//print_r($data);die;
	$sql=sql_select("select id,job_no,cut_num_prefix_no,table_no,remarks,marker_length,marker_width,fabric_width,gsm,width_dia,batch_id,company_id,lay_fabric_wght,cad_marker_cons,efficiency,wastage_qnty , extra_lay_wgt, entry_date,end_date,end_time, start_time from ppl_cut_lay_mst where cutting_no='".$data[0]."' and entry_form=76 ");
    ...
    <div style="position:absolute; right:0; top:90px; " id="qrcode"> </div>
   ...
   ?>
    <table border="1" cellpadding="1" cellspacing="1"   width="1080"class="rpt_table" rules="all">
      ...
      <?
	   ...
	   WHERE a.id = b.pre_cost_fabric_cost_dtls_id  AND b.po_break_down_id in (".$order_id.") and b.cons!=0  and a.status_active=1 and b.status_active=1
	   GROUP BY b.po_break_down_id,a.item_number_id,a.body_part_id,b.color_number_id,b.gmts_sizes");//and a.body_part_id in (1,20,28,30,40,50,59,125)
	   $con_per_dzn=array();
       ...
       foreach($sql_sewing as $row_sew)
       {
			...
			if(!isset($chk_arr[$row_sew[csf('po_break_down_id')]][$row_sew[csf('item_number_id')]][$row_sew[csf('color_number_id')]][$row_sew[csf('gmts_sizes')]]))
			{
				...
				$chk_arr[$row_sew[csf('po_break_down_id')]][$row_sew[csf('item_number_id')]][$row_sew[csf('color_number_id')]][$row_sew[csf('gmts_sizes')]] = 1;
			}
       }
			 ...
			 foreach($c_value as $s_id=>$s_value)
			 {
				 foreach($s_value as $b_id=>$b_value)
				 {
				  ...
				  // $order_qty=$po_item_qty_arr[$p_id][$i_id][$c_id][$b_id]['plan_cut_qty'];
				   ...
				   // echo "(".$order_color_size_qty."/".$order_qty.")*100<br>";
...
{	
	...
	$bundle_array=array();
	$sql_name=sql_select("SELECT b.buyer_name, b.style_ref_no, b.product_dept, a.po_number, a.id from wo_po_details_master b, wo_po_break_down a where a.job_id=b.id and a.job_no_mst='".$data[1]."'");
				...
				$html.='<table cellpadding="0" cellspacing="0" width="" class="" style="font-size:12px; font-weight:bold;margin:0px" rules="all" id="">
						...
						<table cellpadding="0" cellspacing="0" width="" class="" style="font-size:12px; font-weight:bold;margin:0px" rules="all" id="">			
							...
							<tr>
								...
								<td>Gmts.Qty:'.$val[csf("size_qty")].'</td>
							</tr>
							<tr>
								<td></td>
	...
	$files = glob($PNG_WEB_DIR.'*'); // get all file names
