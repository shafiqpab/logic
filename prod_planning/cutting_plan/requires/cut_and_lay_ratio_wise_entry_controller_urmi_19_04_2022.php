                        ...
                        <table>
                            ...
                            <tr>
								...
								<input type="button" id="close_size_id" name="close_size_id"  class="formbutton"  style="width:50px" onClick="size_popup_close(<? echo $size; ?>,$('#hidden_marker_qty').val(),$('#hidden_plant_qty').val(),$('#hidden_total_marker').val(),$('#hidden_lay_balance').val())" value="Close"/>
                                	...
                                	<?
										$print_report_format_arr=return_library_array( "select template_name, format_id from lib_report_template where  module_id=4 and report_id=145 and is_deleted=0 and status_active=1 and template_name=$cbo_company_id", "template_name", "format_id");
...
{	
	...
	$sql_name=sql_select("SELECT 
						...
						where 
							a.job_id=b.id and a.id=c.po_break_down_id and 
							a.job_no_mst='".$data[1]."' $item_id_cond $color_id_cond $order_id_cond ");
...
{	 
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		for($i=1; $i<=$row_num; $i++)
		{
			...
			// echo str_replace("'",'',$$cbocolor)."=".$$cbo_gmt_id."=".$$cbo_order_id."=".$$update_details_id."<br>";die();
