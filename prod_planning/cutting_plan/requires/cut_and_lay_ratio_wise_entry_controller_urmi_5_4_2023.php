...
if($action=="print_qrcode_operation_2") // for Charka
{
	...
	a.number_start,a.number_end,a.size_qty,a.country_id,a.roll_no,b.bundle_sequence,b.color_id
...
if($action=="print_bundle_sticker_v2_14")
{
	...
	$margin_left = 33; $width = 68; $x1 = $margin_left; $x2 = $x1 + $width ; $y1 = $j= 3.5;  $col_gap =109;$row_gap=42; $i=$x1+2;
	$bundle_array=array(); $br=0; $n=0;
	if(count($sql_bundle_copy)!=0)
	{
		foreach($sql_bundle_copy as $inf)
		{
			if($k >=14 && ($k % 14) ==1 )
			{
				$x1 = $margin_left; $x2 = $x1 + $width ; $y1 = $j= 3.5; $i=$x1+2;$k=1;
				$pdf->AddPage();
			...
			foreach($color_sizeID_arr as $val)
			{
				...
				if($br >=2 && ($br % 2) == 0 )
				{
					  $x1 = $margin_left; $x2 = $x1 + $width ; $y1 += $row_gap; $i=$x1+2; $j=$y1;
				}
				else
				{
					$x1+=$col_gap; $x2+=$col_gap; $i+=$col_gap;
				}
...
if($action=="print_barcode_seven_per_page")//kamrul
{
	...
	if(count($sql_bundle_copy)!=0)
	{
		foreach($sql_bundle_copy as $inf)
		{
			...
			//	$pdf -> Line(51.7, 16, 51.7, 16);// after lot
				$pdf -> Line(50.5, 20, 50.5, 16);// after size

				$pdf->Code40($i, $j+2, $symb."STL:".substr($style_name,0,17), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i+35, $j+2, "PO:".substr($po_number,0,16), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;

				$pdf -> Line(0, 8, 68, 8);

				$pdf->Code40($i, $j+6, $symb."COLOR:".substr($color_library[$val[csf("color_id")]],0,16), $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i+35, $j+6, "CT/NO:".$order_cut_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i+50, $j+6, "T.No:".$table_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;



				$pdf -> Line(0, 12, 68, 12);

				$pdf->Code40($i, $j+10, $symb."B.NO:".$val[csf("bundle_no")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i+35, $j+10, "STK:".$val[csf("number_start")]."-".$val[csf("number_end")],  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i+50, $j+10, "QTY:".$val[csf("size_qty")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;

				$pdf -> Line(0, 16, 68, 16);

				$pdf->Code40($i, $j+14,substr($garments_item[$val['GMT_ITEM_ID']],0,18),  $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i+35, $j+14, $symb."SIZE:".$size_arr[$val[csf("size_id")]], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;
				$pdf->Code40($i+50, $j+14,  "Lot/B:".$batch_no, $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;

				$pdf -> Line(0, 20, 68, 20);


				$pdf->Code40($i, $j+18,"B.PART:".$inf[csf("bundle_use_for")], $ext = true, $cks = false, $w = 0.15, $h = 1, $wide = true, true,7) ;
    ...
    <table width="990" cellspacing="0" align="center">
        ...
        <tr>
         ...
         <tr>
			...
			<td><strong>Brand  :</strong></td> <td width="160"><? echo $brand_library[$sql_buyer[0][csf('brand_id')]]; ?></td>
        </tr>
        </tr>
        <tr>
		...
		<tr>
		   <td><strong>Fit:</strong></td> <td width="160"><? echo $fit_lists; ?></td>
                    ...
                    foreach($size_data as $size_val)
                    {
                        ...
                        foreach($bundle_data as $row)
                        {
							...
							foreach($roll_data_arr as $val)
							{
								...
								if($rollData[2]==$row[csf('roll_id')]){$shade=$rollData[7];}
							}
        ...
        <form name="searchprocessfrm_1"  id="searchprocessfrm_1" autocomplete="off">
            ...
            <div style="width:390px; overflow-y:scroll; max-height:280px;" id="buyer_list_view" align="center">
                ...
                <?
                    ...
                    $i=1; $po_row_id=''; $countryIdArr=explode(",",$country_id);
                    ...
                    foreach($result as $row)
                    {
						...
						<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value(<? echo $i;?>)">
							...
							<td width="200"><p><? echo $row[csf('country_name')]; ?></p></td>
...
if($action=="order_details_list")
{
	...
	$country_array=return_library_array( "select id, country_name from lib_country where status_active=1" ,"id","country_name");
	...
	foreach($sql_dtls as $val)
	{
		...
		{
			$selected_country_name.=$country_array[$cId].",";
		}
...
if($action=="cut_lay_entry_report_print3")
{
	...
	$data=explode('*',$data);
	//print_r($data);
	$sql=sql_select("SELECT id,job_no,cut_num_prefix_no,table_no,marker_length,marker_width,fabric_width,gsm,fabric_gsm,width_dia,cad_marker_cons,batch_id,company_id,cad_effi from ppl_cut_lay_mst where cutting_no='".$data[0]."' ");
	// $sql="SELECT id,job_no,cut_num_prefix_no,table_no,marker_length,marker_width,fabric_width,gsm,width_dia,cad_marker_cons,batch_id,company_id,cad_effi from ppl_cut_lay_mst where cutting_no='".$data[0]."' ";
	//  echo $sql;
	foreach($sql as $val)
	{
		$mst_id=$val[csf('id')];
    ...
    <div style="width:270; position:relative;float: left;">
		<table border="1"  cellspacing="0"  width="260" class="rpt_table" rules="all">
	         <tr>
	              <td width="100">Buyer</td><td width="180" align="left"><? echo $buyer_arr[$sql_buyer_arr[0][csf('buyer_name')]]; ?></td>
	         </tr>
	         <tr>
	              <td>Job No</td><td align="left"> <? echo $data[1]; ?></td>
	         </tr>
	         <tr>
	              <td>Style</td><td align="left"> <? echo $sql_buyer_arr[0][csf('style_ref_no')]; ?></td>
	         </tr>
	         <tr>
	              <td>Internal Ref.</td><td align="left"> <? echo $internal_ref_arr[0][csf('grouping')]; ?></td>
	         </tr>
	         <tr>
	              <td>Season</td><td align="left"> <? echo $season_library[$sql_buyer_arr[0][csf('season_buyer_wise')]]; ?></td>
	         </tr>
	         <tr>
	              <td>Item Name</td> <td align="left"><? echo $garments_item[$item_name]; ?></td>
	        </tr>
			<tr>
	              <td>Order Qty</td><td align="left"><? echo $order_qty; ?></td>
	         </tr>
    ...
    <div style=" width:300; position:relative;float: left;padding-left: 3px;">
		<table border="1" cellpadding="1" cellspacing="1"   width="400" class="rpt_table" rules="all">
	         <tr height="30">
			         <td><strong>Quality</strong></td><td width="100" align="center" colspan="2"></td>
	         </tr>
	         <tr height="30">
	              <td><strong>Checked By Q.C</strong></td><td width="100" align="center" colspan="2"></td>
	         </tr>
	         <tr height="30">
	              <td><strong>Supervisor</strong></td> <td align="center" colspan="2"></td>
	        </tr >
	         <tr height="30">
	              <td>Start Time</td><td align="center" width="100"></td><td align="center" width="100"><strong>Total Time Taken</strong></td>
	         </tr >
	...
	<div style="width:100%">
		...
		<tr>
	      <td>Order No</td><td colspan="8"><p><? echo $order_number; ?></p></td>
        </tr>
		</table>
	</div>
	    ...
	    <div style=" float:right;width:190px;"> -->
			...
			<tr>
                <td width="250" align="center" colspan="4" style="font-weight: bold;">STEP LAY DETAILS</td>
		         ...
		         <tr height="30">
		              <td width="40">Booking Efficiency</td><td width="40" align="center" ><? echo $booking_effiency;?></td>
		              <td width="40">CAD Efficiency%</td><td width="40" align="center" ><? echo $cad_effi; ?></td>


		         </tr>
		         <tr height="30">
				 <td width="80" colspan="2">CAD Fabric Width/Dia</td><td width="80" colspan="2" align="center" ><? echo $fabric_with;  ?></td>

		         </tr>
		         <tr height="30">
		              <td width="80" colspan="2">CAD Marker Length</td><td width="80" colspan="2" align="center" ><? echo $marker_length;  ?></td>
		         </tr>
		         <tr height="30"  >
		              <td colspan="2">CAD Marker Width</td><td  colspan="2"align="center" ><? echo $marker_with;  ?></td>
		         </tr>
		         <tr height="30"  >
		              <td colspan="2"> GSM</td><td colspan="2" align="center" ><? echo $gsm;  ?></td>
		         </tr>
		         <tr height="30"  >
		              <td colspan="2">Fabric GSM</td><td colspan="2" align="center" ><? echo $fabric_gsm;  ?></td>
		         </tr>
		    </table>
	    <!-- </div>
    </div>
