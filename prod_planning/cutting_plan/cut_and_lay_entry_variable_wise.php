<?
/*-------------------------------------------- Comments
Purpose			: Cut and Lay Entry Variable Settings Wise(rmg no will be create on variable settings)
Functionality	:
JS Functions	:
Created by		:	Shafiq
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Cut & Lay Entry Ratio Wise V4","../../", 1, 1, $unicode,'','');
...
<script>
	...
	function add_break_down_tr(i)
   	{
		...
		else
		{
			...
			$("#tbl_order_details tbody tr:last td:nth-child(2)").removeAttr('id').attr('id','colorcontrast_'+i);
			$("#tbl_order_details tbody tr:last td:nth-child(3)").removeAttr('id').attr('id','garment_'+i);
			$("#tbl_order_details tbody tr:last td:nth-child(4)").removeAttr('id').attr('id','orderId_'+i);
	...
	function openmypage_jobNo(id)
	{
		...
		var page_link = 'requires/cut_and_lay_entry_variable_wise_controller.php?cbo_company_id='+cbo_company_id+'&action=job_search_popup';
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down('requires/cut_and_lay_entry_variable_wise_controller', job_no[0]+'_'+1, 'load_drop_down_color','color_1');
			load_drop_down('requires/cut_and_lay_entry_variable_wise_controller', job_no[0]+'_'+1, 'load_drop_down_fab_color','colorcontrast_1');
			load_drop_down('requires/cut_and_lay_entry_variable_wise_controller', job_no[0]+'_'+1, 'load_drop_down_order_garment','garment_1');
	...
	function openmypage_po(row_no)
	{
		...
		var page_link = 'requires/cut_and_lay_entry_variable_wise_controller.php?txt_job='+txt_job+'&cbocolor='+cbocolor+'&gmt_id='+gmt_id+'&poId='+poId+'&company_id='+cbo_company_id+'&action=po_popup';
		...
		emailwindow.onclose=function()
		{
			...
			load_drop_down('requires/cut_and_lay_entry_variable_wise_controller', po_id+"_"+cbocolor+"_"+row_no, 'load_drop_down_batch', 'batch_'+row_no);


			load_drop_down( 'requires/cut_and_lay_entry_variable_wise_controller', po_id+"_"+row_no+"_"+cbocolor+"_"+gmt_id, 'load_drop_down_color_type', 'colorTypeId_'+row_no);
			...
			get_php_form_data( po_id+"_"+gmt_id+"_"+cbocolor+"_"+row_no, "load_drop_down_order_qty", "requires/cut_and_lay_entry_variable_wise_controller" );
		}
	}
	...
	function change_data(value,id)
    {
		...
		load_drop_down( 'requires/cut_and_lay_entry_variable_wise_controller', value+"_"+ship_id, 'load_drop_down_ship', ship_id);
		load_drop_down( 'requires/cut_and_lay_entry_variable_wise_controller', value+"_"+gmt_id, 'load_drop_down_order_garment', gmt_id);
	...
	function change_color(id,value)
	{
		...
		load_drop_down('requires/cut_and_lay_entry_variable_wise_controller', order_id+"_"+value+"_"+id[1], 'load_drop_down_color', color_id);
		load_drop_down('requires/cut_and_lay_entry_variable_wise_controller', order_id+"_"+value+"_"+id[1], 'load_drop_down_fab_color', color_id);
	...
	function change_marker(id,value)
	{
		...
		get_php_form_data( order_id+"_"+gmt_id+"_"+value+"_"+id[1], "load_drop_down_order_qty", "requires/cut_and_lay_entry_variable_wise_controller" );
		load_drop_down('requires/cut_and_lay_entry_variable_wise_controller', order_id+"_"+value+"_"+id[1], 'load_drop_down_batch', 'batch_'+id[1]);
	...
	function fnc_cut_lay_info( operation )
	{
		...
		if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][509]); ?>')
		{
			if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][509]); ?>','<? echo implode('*',$_SESSION['logic_erp']['mandatory_message'][509]); ?>')==false) {return;}
		}
		...
		if(form_validation('cbo_company_name*cbo_working_company_name*cbo_location_name*cbo_floor_name*txt_job_no*txt_entry_date*txt_end_date','Company Name*Working Company*Location*Floor*Job No*Plan Start Date*Plan End Date')==false)
	  	{
			return;
		}
        var row_num=$('#tbl_order_details tbody tr').length;

        var data1="action=save_update_delete&operation="+operation+"&row_num="+row_num+get_submitted_data_string('update_id*update_tbl_id*txt_job_no*txt_batch_no*cbo_company_name*cbo_working_company_name*cbo_floor_name*cbo_location_name*txt_cutting_no*txt_table_no*txt_entry_date*txt_marker_length*txt_marker_width*txt_fabric_width*txt_gsm*txt_cutting_no*txt_in_time_hours*txt_in_time_minuties*txt_out_time_hours*txt_out_time_minuties*txt_end_date*cbo_width_dia*txt_marker_cons*roll_maintained*txt_remark*txt_shipment_part*txt_internal_ref*txt_wastage*cbo_marker_type*table_entry_id',"../../");
		//alert(row_num)
	    var data2='';
		for(var i=1; i<=row_num; i++)
		{
			...
			data2+=get_submitted_data_string('updateDetails_'+i+'*cboPoNo_'+i+'*cbogmtsitem_'+i+'*cbocolor_'+i+'*cbocontrastcolor_'+i+'*txtplics_'+i+'*txtorderqty_'+i+'*orderCutNo_'+i+'*rollData_'+i+'*cbobatch_'+i+'*poId_'+i+'*cboColorType_'+i+'*cbogmtsitem_'+i,"../../",2);
		}
	    var data=data1+data2;
		//alert(data1);orderCutNo_1
		freeze_window(operation);
		http.open("POST","requires/cut_and_lay_entry_variable_wise_controller.php",true);
	...
	function fnc_cut_lay_info_reponse()
	{
		if(http.readyState == 4)
		{
			...
			// {
			// 	alert("Can not delete. Please try again later.");
			// 	release_freezing();
			// 	return;
			// }
			if(reponse[0]==13)
			...
			if(reponse[0]==0 || reponse[0]==1)
			{
				...
				show_list_view( reponse[1], 'order_details_list', 'cut_details_container', 'requires/cut_and_lay_entry_variable_wise_controller', '' ) ;
	...
	function open_cutting_popup()
	{
		...
		var page_link='requires/cut_and_lay_entry_variable_wise_controller.php?action=cutting_number_popup&company_id='+company_id;
		...
		emailwindow.onclose=function()
		{
			...
			get_php_form_data( sysNumber[0], "load_php_mst_form", "requires/cut_and_lay_entry_variable_wise_controller" );
			show_list_view( sysNumber[0], 'order_details_list', 'cut_details_container', 'requires/cut_and_lay_entry_variable_wise_controller', '' ) ;
			$("#cbo_company_name").attr("disabled",true);
			$("#txt_job_no").attr("disabled",true);
	...
	function openmypage_roll(row_no)
	{
		...
		var page_link = 'requires/cut_and_lay_entry_variable_wise_controller.php?roll_maintained='+roll_maintained+'&order_no='+order_no+'&color='+color+'&rollData='+rollData+'&action=roll_popup';
...
// for report lay chart
 	function generate_report_lay_chart(data,action)
	{
		...
		window.open("requires/cut_and_lay_entry_variable_wise_controller.php?data=" + data+'&action='+action, true );
	}
	function openTablePopup()
	{
		if( form_validation('cbo_working_company_name*cbo_location_name*cbo_floor_name','Working Company*Location*Floor')==false )
		{
			return;
		}

		let companyID 	= $("#cbo_working_company_name").val();
		let location 	= $("#cbo_location_name").val();
		let floor 		= $("#cbo_floor_name").val();  

		var page_link='requires/cut_and_lay_entry_variable_wise_controller.php?action=table_popup&companyID='+companyID+'&location='+location+'&floor='+floor;
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, 'Table Name Popup', 'width=230px,height=300px,center=1,resize=1,scrolling=0','../../'); 
	} 
    ...
    <table width="95%" cellpadding="0" cellspacing="2" align="center">
                        ...
                        <table  width="1050" cellspacing="2" cellpadding="0" border="0">
                             ...
                             <tr>
                                 ...
                                 <td  width="150" colspan="2">
                                     <? echo create_drop_down( "cbo_company_name", 152, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Company --", $selected, "get_php_form_data(this.value,'roll_maintained', 'requires/cut_and_lay_entry_variable_wise_controller');change_type_color(this.value);" ); ?>
                                 </td>
                                 <td  width="110"  align="left" class="must_entry_caption">Working Company</td>              <!-- 11-00030  -->
                                 <td  width="150" align="">
                                     <?
                                        echo create_drop_down( "cbo_working_company_name", 142, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0  order by company_name","id,company_name", 1, "-- Select Working Company --", $selected, "load_drop_down( 'requires/cut_and_lay_entry_variable_wise_controller', this.value, 'load_drop_down_location', 'location_td' ); " );// location_select(); get_php_form_data(this.value,'size_wise_repeat_cut_no','requires/cut_and_lay_entry_variable_wise_controller' );
                                    ?>
                                </td>
                                <td  width="110" class="must_entry_caption" align="left" >Location </td>
                                <td width="150" id="location_td">
                                    <? echo create_drop_down( "cbo_location_name", 142, $blank_array,"", 1, "-- Select Location --", $selected, "" ); ?>
                                </td>
                                <td width="120" align="left" class="must_entry_caption">Floor</td>
                             ...
                             <tr>
                                 ...
                                 <td id="table_td" width="120">
									<?/*  echo create_drop_down( "txt_table_no", 135,"","id,table_name", 1, "-- Select Table --", $selected, "",1 ); */?>
									<input style="width:80px;" type="text"   class="text_boxes" autocomplete="off"  name="txt_table_no" id="txt_table_no"  />
									<input type="button" name="btn" id="btn" class="formbuttonplasminus" value="table" style="width:40px" onClick="openTablePopup(3);"/>
									<input type="hidden" name="table_entry_id" id="table_entry_id">
                                 </td>
                            ...
                            <tr>
                                ...
                                <td>Remarks</td>
                                <td>
                                    <input style="width: 120px;" type="text" class="text_boxes" name="txt_remark" id="txt_remark" placeholder="Max length 100 characters" maxlength="100" >
                                </td>
                            </tr>
                            <tr>
								<td>Wastage</td>
                                <td>
                                    <input style="width:140px;" type="text" class="text_boxes" autocomplete="off" name="txt_wastage" id="txt_wastage" />
                                </td>
                                <td></td>
                                ...
                                <td>Shipment Part</td>
                                <td>
                                    <input style="width:130px;" type="text" class="text_boxes" autocomplete="off" name="txt_shipment_part" id="txt_shipment_part" />
                                </td>
								<td>IR/IB</td>
                                <td id="internal_id">
                                    <input style="width:130px;" type="text" class="text_boxes" name="txt_internal_ref" id="txt_internal_ref" readonly />
                                </td>
								
                                <td  width="" align="left" >Marker Type</td>
                                <td>
                                    <? echo create_drop_down( "cbo_marker_type", 142, $marker_type_array,"", 1, "-- Select year --", $selected, "",0); ?>
                                </td>
                ...
                <table cellpadding="0" cellspacing="0" width="1300" class="rpt_table" border="1" rules="all" id="tbl_order_details">
                    ...
                    <tbody id="cut_details_container">
                        <tr class="" id="tr_1" style="height:10px;">
							...
							<td align="center" id="colorcontrast_1">
                                 <?
								     echo create_drop_down( "cbocontrastcolor_1", 100, $blank_array,"", 1, "select color", $selected, "");
                                 ?>
                            </td>
                            ...
                            <td align="center" id="cutNo_1">
                                <input style="width:60px;" class="text_boxes_numeric" type="text" name="orderCutNo_1" id="orderCutNo_1" placeholder="" />
                            </td>
        ...
        <tr>
               <td colspan="4" align="center" class="">
                    ...
                    ?>
                      <input type="button"  id="btn_cost_print" name="btn_cost_print"   style="width:100px;display:none"  class="formbutton" value="Lay Chart"  onClick="generate_report_lay_chart($('#txt_cutting_no').val()+'*'+$('#txt_job_no').val()+'*'+$('#cbo_working_company_name').val()+'*'+$('#cbo_location_name').val(),'cut_lay_entry_report_print');"/>
					  <input type="button"   id="btn_cost_print2" name="btn_cost_print2"   style="width:100px;display:none"  class="formbutton" value="Lay Chart 2"  onClick="generate_report_lay_chart($('#txt_cutting_no').val()+'*'+$('#txt_job_no').val()+'*'+$('#cbo_working_company_name').val()+'*'+$('#cbo_location_name').val(),'cut_lay_entry_report_print_two');"/>
                    </td>
