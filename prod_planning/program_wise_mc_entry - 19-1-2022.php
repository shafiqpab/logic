...
<script>
	...
	function show_details(type) {
		...
		var data = "action=booking_item_details" + get_submitted_data_string('cbo_company_name*cbo_within_group*cbo_buyer_name*hide_job_id*txt_booking_no*txt_internal_ref*cbo_planning_status*txt_date_from*txt_date_to', "../") + '&type=' + type;
		freeze_window(5);
    ...
    function generate_delete_booking_report(company_id, program_id,format_id="") {
       //action name= print
        ...
        if(format_id==273)
        {
            print_report(company_id + '*' + program_id + '*' + path, "delete_booking_prog_info_print", "requires/program_wise_mc_entry_controller");
        }else {
            print_report(company_id + '*' + program_id + '*' + path, "print", "requires/yarn_requisition_entry_sales_controller");
        }
    }
    ...
    function activePlan(e,plan_id,program_ids,balance_qnty,dtls_ids,prog_qty){
        if(confirm("Are you sure?")){
            if(balance_qnty >= 0){
                ...
                var response = trim(return_global_ajax_value(data, 'activePlan', '', 'requires/program_wise_mc_entry_controller'));
                if(response == 1){
...
<body>
    <div style="width:100%;" align="center">
        ...
        <form name="palnningEntry_1" id="palnningEntry_1">
          ...
          <h3 style="width:1210px;" align="left" id="accordion_h1" class="accordion_h"
