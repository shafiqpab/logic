...
if ($action=="show_operation_list")
{
	...
	$not_machine_array=array(129,40,41,43,44,48,53,54,55,56,68,69,70,90,176);
	
	$sql="SELECT id, row_sequence_no, lib_sewing_id, resource_gsd from ppl_gsd_entry_dtls where mst_id=$data and is_deleted=0 order by row_sequence_no asc";
	//echo $sql;die;
        ...
        <table cellpadding="0" cellspacing="0" border="1" rules="all" width="388" class="rpt_table" id="list_view_tc">
			...
			?>
				<tr bgcolor="<? echo $bgcolor; ?>" id="trTc_<? echo $i; ?>" style="cursor:pointer" onclick="js_set_value_tc('<? echo $data; ?>');change_color_tr('<? echo $i; ?>','<? echo $bgcolor; ?>')">
					...
					<td><p><? echo $production_resource[$row[csf('resource_gsd')]]; ?></p></td>
				</tr>
			<?	
				$i++;
			}
...
if($action=="details_list_view")
{
	...
	echo create_list_view("list_view_details", "Operation Name, Seam Length, Required", "400,170","850","300",0, $sql, "get_php_form_data", "id,operation_id,gsd_dtls_id,seam_length", "'load_php_data_from_tc'", 1, "operation_id,0,0", $arr, "operation_id,seam_length,req_qty","requires/thread_consumption_controller","",'0,2,2') ;
...
if($action=="load_php_data_from_tc")
{
	...
	$sql= "SELECT id, thread_type, thread_desc, thread_length, allowance, req_thread FROM ppl_thread_cons_op_dtls_entry where dtls_id=$dtls_id and is_deleted=0";
	$data_array=sql_select($sql);
	$i=0; $table='';
 	foreach ($data_array as $row)
	{
		...
		$table=$table.'<tr align="center" id="tr_'.$i.'" bgcolor="'.$bgcolor.'"><td align="left">'.$i.'</td><td>'.create_drop_down( "cboThreadType_$i", 130,$size_color_sensitive,"",1, "-- Select --", $row[csf('thread_type')], '', 0, "1,3", "", "", "", "", "", "cboThreadType[]").'</td><td><input type="text" name="txtThreadDesc[]" id="txtThreadDesc_'.$i.'" placeholder="Write" class="text_boxes" style="width:180px" value="'.$row[csf('thread_desc')].'" onFocus="add_auto_complete('.$i.')" /></td><td><input type="text" name="txtThreadLength[]" id="txtThreadLength_'.$i.'" placeholder="Write" class="text_boxes_numeric" style="width:100px" onKeyUp="calculate_thread('.$i.')" value="'.$row[csf('thread_length')].'" /></td><td><input type="text" name="txtAllowance[]" id="txtAllowance_'.$i.'" placeholder="Write" class="text_boxes_numeric" style="width:100px" onKeyUp="calculate_thread('.$i.')" value="'.$row[csf('allowance')].'" /></td><td><input type="text" name="txtRequired[]" id="txtRequired_'.$i.'" class="text_boxes_numeric" style="width:100px" placeholder="Calculative" readonly="readonly" value="'.$row[csf('req_thread')].'" /></td><td><input type="button" id="increaseT_'.$i.'" name="increase[]" style="width:30px" class="formbuttonplasminus" value="+" onClick="add_break_down_tr('.$i.')"/><input type="button" id="decreaseT_'.$i.'" name="decrease[]" style="width:30px" class="formbuttonplasminus" value="-" onClick="fn_deleteRow('.$i.');" /></td></tr>';
	}
	...
	if($i<4)
	{
		for($i=$i+1;$i<=4;$i++)
		{
			...
			$table=$table.'<tr align="center" id="tr_'.$i.'" bgcolor="'.$bgcolor.'"><td align="left">'.$i.'</td><td>'.create_drop_down( "cboThreadType_$i", 130,$size_color_sensitive,"",1, "-- Select --", 0, '', 0, "1,3", "", "", "", "", "", "cboThreadType[]").'</td><td><input type="text" name="txtThreadDesc[]" id="txtThreadDesc_'.$i.'" placeholder="Write" class="text_boxes" style="width:180px" value="" onFocus="add_auto_complete('.$i.')" /></td><td><input type="text" name="txtThreadLength[]" id="txtThreadLength_'.$i.'" placeholder="Write" class="text_boxes_numeric" style="width:100px" onKeyUp="calculate_thread('.$i.')" value="" /></td><td><input type="text" name="txtAllowance[]" id="txtAllowance_'.$i.'" placeholder="Write" class="text_boxes_numeric" style="width:100px" onKeyUp="calculate_thread('.$i.')" value="" /></td><td><input type="text" name="txtRequired[]" id="txtRequired_'.$i.'" class="text_boxes_numeric" style="width:100px" placeholder="Calculative" readonly="readonly" value="" /></td><td><input type="button" id="increaseT_'.$i.'" name="increase[]" style="width:30px" class="formbuttonplasminus" value="+" onClick="add_break_down_tr('.$i.')"/><input type="button" id="decreaseT_'.$i.'" name="decrease[]" style="width:30px" class="formbuttonplasminus" value="-" onClick="fn_deleteRow('.$i.');" /></td></tr>';
		}
	}
...
if ($action=="save_update_delete")
{
	...
	{ 
		...
		$field_array_dtls_tc="id, gsd_mst_id, mst_id, dtls_id, thread_type, thread_desc, thread_length, allowance, req_thread, entry_form";
		...
		{ 	
			...
			$data_array_dtls_tc.="(".$dtls_id_tc.",".$breakdown_id3.",".$id.",".$dtls_id.",'".$$cboThreadType."','".$$txtThreadDesc."','".$$txtThreadLength."','".$$txtAllowance."','".$$txtRequired."', 2)";
		...
		if(str_replace("'", '', $bl3_update_id)=="")
		{
			$data_array="(".$id.",".$breakdown_id3.",".$txt_body_size.",".$txt_cons_date.",".str_replace("'","",$cbo_uom).",".$totReq.",".$req_per_gmts_into_meter.",4,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."', 2)";
		}
		else
		{
		...
		$data_array_dtls="(".$dtls_id.",".$breakdown_id3.",".$dtlsId_gsd.",".$id.",".$operation_id.",".$txt_seam_length.",'".$op_req_qty."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."', 2)";
	...
	{ 
		...
		{ 	
			...
			if($data_array_dtls_tc!="") $data_array_dtls_tc.=",";
			$data_array_dtls_tc.="(".$dtls_id_tc.",".$breakdown_id3.",".$bl3_update_id.",".$update_dtlsId.",'".$$cboThreadType."','".$$txtThreadDesc."','".$$txtThreadLength."','".$$txtAllowance."','".$$txtRequired."')";
...
if($action=="print")
{
	$mstDataArray=sql_select("select a.body_size, a.thread_cons_date, a.input_uom, a.total_req, a.meter_per_gmts, b.style_ref, b.buyer_id from ppl_balancing_mst_entry a, ppl_gsd_entry_mst b where a.gsd_mst_id=b.id and a.id='".$data."' and a.balancing_page=4 and a.status_active=1 and a.is_deleted=0");
            ...
            <?
				...
				?>
					<tr>
						...
						<td style="word-wrap:break-word"><? echo $production_resource[$slectResult[csf('resource_sewing')]]; ?></td>
