            ...
            <div style="margin-left:10px; margin-top:10px">
                ...
                <div style="width:700px; max-height:300px; overflow-y:scroll" id="list_container" align="left"> 
                    <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="680" id="tbl_list_search">  
                        <? 
                        $i=1;
                      //  $production_resource[129]="Man";
                       // $production_resource[130]="Machine";
                        asort($production_resource);
						foreach($production_resource as $id=>$value)
                        { 
                        	if($id==129 || $id==130)
                        	{
	                            $bgcolor=($i%2==0)?"#E9F3FF":"#FFFFFF";
...
<body>
	<form name="searchdescfrm"  id="searchdescfrm">
		<fieldset style="width:965px;margin-left:5px">
            ...
            <div style="margin-top:5px; margin-left:5px">
                ...
                <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="935" id="tbl_list_tot">
                    ...
                    <tr bgcolor="#E9F3FF" align="center">
                        <td width="110" align="left" colspan="2"><b>&nbsp;Average BT</b></td>
                        <td width="60">&nbsp;</td>
                        <td width="60">
            ...
            <table width="920">
                 <tr>
                 	<td width="200"><b>SMV- <span id="smv"></span></b></td>
                    <td width="200"><b>Target/HR- <span  id="target_per_hour"></span></b></td>
                    <td align="left">
...
if ($action=="sewing_operation_list_view")
{
	...
	$sql_cond='';
	if($data[0]>0) $sql_cond=" and department_code='".$data[0]."'";
	...
	$sql="select product_dept,production_type,gmt_item_id, bodypart_id,code,operation_name,smv_basis,seam_length, resource_sewing, operator_smv, helper_smv, department_code, id from lib_sewing_operation_entry where is_deleted=0 $sql_cond order by id Desc";
	...
	<div style="width:990px; max-height:220px; overflow-y:scroll" id="list_container_batch" align="left">	 
		...
		<?
			...
			?>
				 <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" onClick="get_php_form_data(<? echo $row[csf('id')]; ?>,'load_php_data_to_form', 'requires/sewing_operation_controller');">  
					...
					<td width="110"><p><? echo $production_rate_type[$row[csf('production_type')]]; ?></p></td>
					 
					<td width="110"><p><? echo $body_part[$row[csf('bodypart_id')]]; ?></p></td>
					<td width="110"><p><? echo $machine_category[$row[csf('department_code')]]; ?>&nbsp;</p></td>
					<td width="110"><p><? echo $garments_item[$row[csf('gmt_item_id')]]; ?></p></td>              
					<td width="80"><p><? echo $row[csf('code')]; ?>&nbsp;</p></td>
					<td width="150"><p><? echo $row[csf('operation_name')]; ?></p></td>
                    ...
                    <td width="" align="right"><? echo number_format($row[csf('helper_smv')],2); ?>&nbsp;</td>
...
if ($action=="load_php_data_to_form")
{
	...
	foreach ($nameArray as $inf)
	{
		...
		echo "document.getElementById('cbo_resource_view').value  = '".($production_resource[$inf[csf("resource_sewing")]])."';\n";
		...
		if($inf[csf("resource_sewing")]==129 || $inf[csf("resource_sewing")]==40 || $inf[csf("resource_sewing")]==41 || $inf[csf("resource_sewing")]==43 || $inf[csf("resource_sewing")]==44 || $inf[csf("resource_sewing")]==48 || $inf[csf("resource_sewing")]==53 || $inf[csf("resource_sewing")]==54 || $inf[csf("resource_sewing")]==55)
		{
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)  // Insert Here
	{
		...
		if (is_duplicate_field( "operation_name", "lib_sewing_operation_entry", "company_id=$cbo_company_id  and gmt_item_id=$cbo_garment_item and bodypart_id=$cbo_bodypart and operation_name=$txt_operation and resource_sewing=$cbo_resource and $null_check(seam_length,0)=$seam_length and is_deleted=0" ) == 1)
		{
		...
		else
		{
			...
			$field_array="id,company_id, operation_name, rate, uom, resource_sewing, operator_smv, helper_smv, product_dept, bodypart_id, gmt_item_id, product_code, gmts_code, body_part_code, code_prefix, code, fabric_type, smv_basis, seam_length, department_code, observer_name, checked_by, avg_bt_pick, avg_bt_ex, avg_bt_dis, avg_bt_elm, tot_avg_bt, alter_perc, machine_perc, relaxation_perc, alter_avg_bt, machine_avg_bt, relaxation_avg_bt, grand_tot_avg_bt,production_type, inserted_by,  insert_date";
			$data_array="(".$id.",".$cbo_company_id.",".$txt_operation.",".$txt_rate.",".$cbo_uom.",".$cbo_resource.",".$txt_operator_smv.",".$txt_helper_smv.",".$cbo_product_dept.",".$cbo_bodypart.",".$cbo_garment_item.",".$txt_product_code.",".$txt_gmts_code.",".$txt_body_part_code.",'".$code_no."','".$code."',".$fabric_desc_id.",".$cbo_smv_basis.",".$txt_seam_length.",".$cbo_department_code.",'".$txt_observer_name."','".$txt_checked_by."','".$txtBtPickAvg."','".$txtBtExAvg."','".$txtBtDisAvg."','".$txtBtElmAvg."','".$txtTotAvgBt."','".$txtAlterPerc."','".$txtMachinePerc."','".$txtRelxPerc."','".$txtAlterPercBt."','".$txtMachinePercBt."','".$txtRelxPercBt."','".$txtGtot."',".$cbo_prod_type.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
			
			if(str_replace("'","",$cbo_resource)==129 || str_replace("'","",$cbo_resource)==40 || str_replace("'","",$cbo_resource)==41 || str_replace("'","",$cbo_resource)==43 || str_replace("'","",$cbo_resource)==44 || str_replace("'","",$cbo_resource)==48 || str_replace("'","",$cbo_resource)==53 || str_replace("'","",$cbo_resource)==54 || str_replace("'","",$cbo_resource)==55 || str_replace("'","",$cbo_resource)==56 || str_replace("'","",$cbo_resource)==68 || str_replace("'","",$cbo_resource)==69 || str_replace("'","",$cbo_resource)==70 || str_replace("'","",$cbo_resource)==90 || str_replace("'","",$cbo_resource)==176)
			{
	...
	else if ($operation==1)   // Update Here
	{
		...
		if (is_duplicate_field( "operation_name", "lib_sewing_operation_entry", "company_id=$cbo_company_id and gmt_item_id=$cbo_garment_item and bodypart_id=$cbo_bodypart and operation_name=$txt_operation and resource_sewing=$cbo_resource and $null_check(seam_length,0)=$seam_length and is_deleted=0 and id!=".$update_id."" ) == 1)
		{
			echo "11**0"; disconnect($con); die;
		}
		else
		{
			$field_array="operation_name*rate*uom*resource_sewing*operator_smv*helper_smv*total_smv*bodypart_id*gmt_item_id*updated_by*update_date*production_type*status_active*is_deleted";
			$data_array="".$txt_operation."*".$txt_rate."*".$cbo_uom."*".$cbo_resource."*".$txt_operator_smv."*".$txt_helper_smv."*".$txt_total_smv."*".$cbo_bodypart."*".$cbo_garment_item."*".$cbo_prod_type."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_status."*0";
			$save_data=explode("_",str_replace("'","",$smv_data));
			$txt_observer_name=$save_data[0];
			$txt_checked_by=$save_data[1];
			$target_per_hour=$save_data[2];
			...
			$data_array=$cbo_company_id."*".$txt_operation."*".$txt_rate."*".$cbo_uom."*".$cbo_resource."*".$txt_operator_smv."*".$txt_helper_smv."*".$cbo_product_dept."*".$cbo_bodypart."*".$cbo_garment_item."*".$txt_product_code."*".$txt_gmts_code."*".$txt_body_part_code."*'".$code_no."'*'".$code."'*".$fabric_desc_id."*".$cbo_smv_basis."*".$txt_seam_length."*".$cbo_department_code."*'".$txt_observer_name."'*'".$txt_checked_by."'*'".$txtBtPickAvg."'*'".$txtBtExAvg."'*'".$txtBtDisAvg."'*'".$txtBtElmAvg."'*'".$txtTotAvgBt."'*'".$txtAlterPerc."'*'".$txtMachinePerc."'*'".$txtRelxPerc."'*'".$txtAlterPercBt."'*'".$txtMachinePercBt."'*'".$txtRelxPercBt."'*'".$txtGtot."'*".$cbo_prod_type."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'*".$cbo_status."";
...
if($action=="time_study_sheet_print")
{
	...
	if($dataArray[0][csf('resource_sewing')]==129 || $dataArray[0][csf('resource_sewing')]==40 || $dataArray[0][csf('resource_sewing')]==41 || $dataArray[0][csf('resource_sewing')]==43 || $dataArray[0][csf('resource_sewing')]==44 || $dataArray[0][csf('resource_sewing')]==48 || $dataArray[0][csf('resource_sewing')]==53 || $dataArray[0][csf('resource_sewing')]==54 || $dataArray[0][csf('resource_sewing')]==55 || $dataArray[0][csf('resource_sewing')]==56 || $dataArray[0][csf('resource_sewing')]==68 || $dataArray[0][csf('resource_sewing')]==69 || $dataArray[0][csf('resource_sewing')]==70 || $dataArray[0][csf('resource_sewing')]==90 || $dataArray[0][csf('resource_sewing')]==176)
	{
