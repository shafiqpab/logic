...
if ($action=="show_operation_list")
{
	...
	$sql="SELECT id, lib_sewing_id, row_sequence_no, resource_gsd, total_smv, target_on_full_perc from ppl_gsd_entry_dtls where mst_id=".$data[0]." and is_deleted=0 order by row_sequence_no asc";
	 //echo $sql;die;
    ...
    <table width="388" cellspacing="0" cellpadding="0" border="0" rules="all" class="rpt_table">
        ...
        <table cellpadding="0" cellspacing="0" border="1" rules="all" width="388" class="rpt_table" id="list_view_lo">
			...
			foreach($result as $row)	
			{
				 ...
				 $data=$row[csf('id')]."**".$row[csf('lib_sewing_id')]."**".$row[csf('row_sequence_no')]."**".$row[csf('resource_gsd')]."**".number_format($row[csf('total_smv')],2,'.','')."**".number_format($row[csf('target_on_full_perc')],2,'.','')."**".$operation_arr[$row[csf('lib_sewing_id')]]."**".$production_resource[$row[csf('resource_gsd')]];
			?>
				<tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>" style="cursor:pointer">
					...
					<td width="150"><p><? echo $operation_arr[$row[csf('lib_sewing_id')]]; ?></p></td>
					<td width="80"><p><? echo $production_resource[$row[csf('resource_gsd')]]; ?></p></td>
...
if($action=="details_list_view")
{
	...
	$layoutDataArr=array(); $gsdDataArr=array(); 
...
//print_r($layoutDataArr);		
		...
		$operation_arr=return_library_array( "select id,operation_name from lib_sewing_operation_entry", "id","operation_name"  );
		$sql="SELECT id, lib_sewing_id, row_sequence_no, resource_gsd, total_smv, target_on_full_perc from ppl_gsd_entry_dtls where mst_id=$update_id and is_deleted=0";
		$result=sql_select($sql);
		foreach($result as $row)
		{
				...
				if(count($work_stations)>0)
				{
					...
					?>
						<tr bgcolor="<? echo $bgcolor; ?>" id="trSt_<? echo $i; ?>">
							<td><? echo $z; ?></td>
							<td>
								<input type="text" name="rescName[]" id="rescName_<? echo $i;?>" style="width:68px" class="text_boxes" readonly="readonly" onclick="copy_data(<? echo $i;?>,1);" placeholder="Click Copy Operation" value="<? echo $production_resource[$gsdDataArr[$dtlsId]['resource_gsd']]; ?>">
				...
				if(count($work_stations)>0)
				{
					...
					?>
						<tr bgcolor="<? echo $bgcolor; ?>" id="trSt_<? echo $i; ?>">
							<td><? echo $z; ?></td>
							<td>
								<input type="text" name="rescName[]" id="rescName_<? echo $i;?>" style="width:68px" class="text_boxes" readonly="readonly" onclick="copy_data(<? echo $i;?>,2);" placeholder="Click Copy Operation" value="<? echo $production_resource[$gsdDataArr[$dtlsId]['resource_gsd']]; ?>">
								<input type="hidden" name="rescId[]" id="rescId_<? echo $i; ?>" style="width:68px" value="<? echo $gsdDataArr[$dtlsId]['resource_gsd']; ?>">
								<input type="hidden" name="gsdDtlsId[]" id="gsdDtlsId_<? echo $i; ?>" style="width:68px" value="<? echo $dtlsId; ?>">
								<input type="hidden" name="txtOrginal[]" id="txtOrginal_<? echo $i; ?>" style="width:68px" value="1">
							</td>
	...
	else
	{
		...
		$noOfRow=0; $i=1; $work_stations_arr=array();
		for($z=$NoOfWorker;$z>=1;$z--)
		{
			$work_stations=explode(",",chop($layoutDataArr[$z],','));
			if(count($work_stations)>0)
			{
				...
				?>
					<tr bgcolor="<? echo $bgcolor; ?>" id="trSt_<? echo $i; ?>">
						<td><? echo $z; ?></td>
						<td>
							<input type="text" name="rescName[]" id="rescName_<? echo $i;?>" style="width:78px" class="text_boxes" readonly="readonly" onclick="copy_data(<? echo $i;?>,1);" placeholder="Click Copy Operation" value="<? echo $production_resource[$gsdDataArr[$dtlsId]['resource_gsd']]; ?>">
							<input type="hidden" name="rescId[]" id="rescId_<? echo $i; ?>" style="width:68px" value="<? echo $gsdDataArr[$dtlsId]['resource_gsd']; ?>">
							<input type="hidden" name="gsdDtlsId[]" id="gsdDtlsId_<? echo $i; ?>" style="width:68px" value="<? echo $dtlsId; ?>">
...
if($action=="show_summary")
{
	...
	$sqlDtls="SELECT id, mst_id, row_sequence_no, body_part_id, lib_sewing_id, resource_gsd, attachment_id, efficiency, total_smv, target_on_full_perc from ppl_gsd_entry_dtls where mst_id='".$data."' and is_deleted=0 order by row_sequence_no asc";
	$data_array_dtls=sql_select($sqlDtls);
	
	$tot_smv=0; $tot_mp=0; $helperSmv=0; $machineSmv=0; $sQISmv=0; $fIMSmv=0; $fQISmv=0; $polyHelperSmv=0; $pkSmv=0; $htSmv=0;   
            ...
            <td width="32%" valign="top">
                ...
                <table border="1" rules="all" class="rpt_table" width="100%">
                    ...
                    <tr bgcolor="#E9F3FF">
                        ...
                        <td id="totSmvSumm" align="right" style="padding-right:5px"><? echo number_format($helperSmv+$machineSmv+$sQISmv+$fIMSmv+$fQISmv+$polyHelperSmv+$pkSmv+$htSmv,2,'.',''); ?></td>
                    </tr>
                </table>
            </td>
...
if ($action=="save_update_delete")
{
	...
	{ 
		...
		$data_array="(".$id.",".$breakdown_id4.",".$balanceId.",".$cbo_line_shape.",".$txt_no_of_work_st.",".$txt_layout_date.",3,".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."', 2)";
		...
		{ 	
			...
			$data_array_dtls.="(".$dtls_id.",".$breakdown_id4.",'".$$gsdDtlsId."',".$id.",'".$$seqNo."','".$$sewingId."','".$$rescId."','".$$smv."','".$$tgtPerc."','".$$stNo."', 2)";
			$dtls_id = $dtls_id+1;
		}
	...
	{ 
		...
		//echo "10**".$rID."&&".$rID2."&&".$rID3.$field_array.$data_array;die;
...
if($action=="layout_print")
{
	...
	$workerTrackingArr=array(); $layoutDataArr=array(); $gsdDataArr=array();
	/*$blData=sql_select("select worker_tracking from ppl_balancing_dtls_entry where mst_id=$bl_update_id");
	foreach($blData as $row)
	{
		if($row[csf('worker_tracking')]=="" || !in_array($row[csf('worker_tracking')],$workerTrackingArr))
    ...
    <div style="width:900px">
        <table width="100%">
            ...
            <tr>
                ...
                <td width="190"><? echo $mstDataArray[0][csf('style_ref')]; if($mstDataArray[0][csf('custom_style')]!="") echo " (".$mstDataArray[0][csf('custom_style')].")"; ?></td>
		...
		if($line_shape==2)
		{
			$i=1; $firstHalf=round($NoOfWorker/2); $secondHalf=round($NoOfWorker-$firstHalf);
			...
			for($z=$NoOfWorker;$z>=1;$z--)
			{
				if($z%2==1)
				{
					...
					if(count($work_stations)>0)
					{
						$row_span=count($work_stations); $j=0;
						foreach($work_stations as $dtlsId)
						{
							?>
							<tr>
								...
								?>
								<td align="center"><? echo $production_resource[$gsdDataArr[$dtlsId]['resource_gsd']]; ?></td>
								...
								<td align="center" title="=Target/100*Efficiency"><? echo $gsdDataArr[$dtlsId]['target_on_full_perc']/100*$gsdDataArr[$dtlsId]['efficiency']; ?></td>
							</tr>
							<?	
							$j++;
							$i++;
