...
<script>
	...
	{ 
		...
		emailwindow.onclose=function()
		{
			...
			if (theemail.value!="")
			{
				...
				document.getElementById("cbo_company_id").value=response[23];
				document.getElementById("hidden_quotation_id").value=response[24];
				if(response[21]==4){
				...
				if(eval(response[8])>0) var tot_smv=eval(response[8]).toFixed(2); else var tot_smv='';
				if(eval(response[9])>0) var mc_smv=eval(response[9]).toFixed(2); else var mc_smv='';
				if(eval(response[10])>0) var manual_smv=eval(response[10]).toFixed(2); else var manual_smv='';
				if(eval(response[11])>0) var finishing_smv=eval(response[11]).toFixed(2); else var finishing_smv='';
				...
				document.getElementById("cbo_bulletin_copy").value=response[14];
				$('#cbo_bulletin_copy').attr('disabled',false);
				document.getElementById("cbo_action").value=1;
			
				show_list_view(response[3]+"__"+response[0]+"_"+response[14],'show_operation_list_view','list_operation_container','requires/ws_gsd_controller','setFilterGrid(\'list_view\',0);');
	...
	function fnc_gsd_entry( operation )
	{
		...
		if(operation!=2)
		{
			...
			// if(form_validation('txt_operator','Resource')==false && form_validation('txt_helper','Resource')==false){
			// 	return;
			// }
		}
		
		
		var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_style_ref*cbo_buyer*cbo_gmt_item*txt_working_hour*cbo_action*txt_seqNo*cbo_body_part*hidden_operation*cbo_resource*txt_attachment_id*txt_operator*txt_helper*txt_efficiency*txt_tgt_perc*txt_tgt_eff*txt_dtls_id*txt_operation_count*txt_mcOperationCount*txt_tot_smv*txt_mc_smv*txt_manual_smv*txt_finishing_smv*hidden_quotation_id*update_id*txt_ext_no*cbo_bulletin_copy*system_no*cbo_product_department*txt_custom_style*txt_remarks*txt_fabric_type*cbo_bulletin_type*cbo_colortype*cbo_approved_status*txt_product_description*txt_applicable_period*cbo_prod_type*cbo_company_id',"../../"); //
	...
	{
		var txt_dtls_id=$('#txt_dtls_id').val();
		if(txt_dtls_id=="")
		{
		...
		var response=return_global_ajax_value(txt_dtls_id, 'is_operation_balanced_entry', '', 'requires/ws_gsd_controller');
		if(response==1)
		{
		...
		var data="action=delete_operation"+get_submitted_data_string('update_id*txt_dtls_id',"../../");
		freeze_window(2);
		http.open("POST","requires/ws_gsd_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
...
*/		
		...
		var gmts_item_id=0; var body_part_id=0; var product_dept=0;
		...
		show_list_view(gmts_item_id+"_"+body_part_id+"_"+update_id+"_"+product_dept+"_"+cbo_company_id,'show_operation_list_view','list_operation_container','requires/ws_gsd_controller','setFilterGrid(\'list_view\',0);');
	...
	{ 
		...
		document.getElementById("cbo_prod_type").value=datas[6];
		fnc_smv_active();
		calculate_target();
	}
	...
	function fnc_smv_active()
	{
		...
		if(resource==129 || resource==40 || resource==41 || resource==43 || resource==44 || resource==48 || resource==53 || resource==54 || resource==55 || resource==56 || resource==68 || resource==69 || resource==70 || resource==176 )
		{
	...
	{
		var row_num=$('#gsd_tbl tbody tr').length;
		var seq_no_arr=new Array(); var breakOut = true; 
		for(var j=1; j<=row_num; j++)
		{
	...
	function re_arrange_table()
	{
		if($('input[name=seqRa]').is(":checked"))
		{
			...
			if(txt_seq==0){alert("Sequance Zero Not Allowed.");return;}
	...
	function copy_data(i,tbl_no) 
	{
		if($('input[name=copyRow]').is(":checked"))
		{
			...
			if(tbl_no==2) var tbl_id="tbl_list_layout2"; else var tbl_id="tbl_list_layout";
	...
	function calculate_total(fromField)
	{
		...
		var totSmv=0; var totTheoriticalMp=0; var totLayOut=0; var weight=''; var mpSumm=[]; var resource_array = <? echo json_encode($production_resource); ?>;
		var helperSmv=0; var machineSmv=0; var sQISmv=0; var fIMSmv=0; var fQISmv=0; var polyHelperSmv=0; var pkSmv=0; var htSmv=0;   
		var helperMp=0; var machineMp=0; var sQiMp=0; var fImMp=0; var fQiMp=0; var polyHelperMp=0; var pkMp=0; var htMp=0;
		...
		for(var i=1; i<=numRow; i++)
		{
			...
			if(rescId*1==129 || rescId*1==40 || rescId*1==41 || rescId*1==43 || rescId*1==44 || rescId*1==48 || rescId*1==68 || rescId*1==69 || rescId*1==70 || rescId*1==176)
			{
		...
		var html=''; var bgcolor=''; x=1; var tot=0;
		for (var i in mpSumm) {
		...
		var totSmvSumm=helperSmv*1+machineSmv*1+sQISmv*1+fIMSmv*1+fQISmv*1+polyHelperSmv*1+pkSmv*1+htSmv*1;
		...
		var totMPSumm=helperMp*1+machineMp*1+sQiMp*1+fImMp*1+fQiMp*1+polyHelperMp*1+pkMp*1+htMp*1;
		...
		var txt_allocated_mp=trim($('#txt_allocated_mp').val()); var pitch_time='';
		...
		for(var i=1; i<=numRow; i++)
		{
			...
			var theoriticalMp=''; var workLoad='';
			if(pitch_time*1>0)
			{
	...
	{  
		...
		if(operation==4)
		{
			 ...
			 generate_report_file($('#bl_update_id').val()+'**'+report_title,'balancing_print','requires/balancing_entry_controller');
			 return;
		}
	...
	function print_balancing()
	{
		...
		var report_title=$( "div.form_caption" ).html();
		generate_report_file(bl_update_id+'**'+report_title,'balancing_print2','requires/balancing_entry_controller');
		return;	
	}
	...
	$(document).ready(function(){
		...
		$("#graph").click(function() {
			...
			if(bl_update_id!="")
			{
				//var graphData=trim(return_global_ajax_value( bl_update_id, 'load_graph_data', '', 'requires/balancing_entry_controller')).split("**");
	...
	function list_generate()
	{
		...
		{
			var noOfWorker=txt_worker+3; var j=0;
	...
	function calculate_smv(i,j)
	{
		...
		if(isNaN(totOpWsmv)) totOpWsmv=0; else totOpWsmv=Math.round(totOpWsmv * 1e12) / 1e12; 
		...
		if(isNaN(totWsmv)) totWsmv=0; else totWsmv=totWsmv.toFixed(2);
	...
	function fnc_balancing2_entry( operation )
	{
		...
		var smvString=''; var tgtString=''; var wlString='';
		var txt_worker=trim($('#txt_worker').val());
		for(var z=1; z<=txt_worker*1; z++)
		{
			if(smvString=="") smvString=$('#smv_2_'+z).val(); else smvString+="_"+$('#smv_2_'+z).val();
			if(tgtString=="") tgtString=$('#tgt_3_'+z).val(); else tgtString+="_"+$('#tgt_3_'+z).val();
			if(wlString=="") wlString=$('#wl_4_'+z).val(); else wlString+="_"+$('#wl_4_'+z).val();
		}
	...
	function resetFormTC()
	{
		...
		var html='<tr id="tr_1" bgcolor="#FFFFFF" align="center"><td align="left">1</td><td><? echo create_drop_down( "cboThreadType_1", 130,$size_color_sensitive,"",1, "-- Select --", 1, "", "", "1,3", "", "", "", "", "", "cboThreadType[]" ); ?></td><td><input type="text" name="txtThreadDesc[]" id="txtThreadDesc_1" placeholder="Write" class="text_boxes" style="width:180px" onFocus="add_auto_complete( 1 )" /></td><td><input type="text" name="txtThreadLength[]" id="txtThreadLength_1" placeholder="Write" class="text_boxes_numeric" style="width:100px" onKeyUp="calculate_thread(1)" /></td><td><input type="text" name="txtAllowance[]" id="txtAllowance_1" placeholder="Write" class="text_boxes_numeric" style="width:100px" onKeyUp="calculate_thread(1)" onblur="copy_value(this.value,1);" /></td><td><input type="text" name="txtRequired[]" id="txtRequired_1" class="text_boxes_numeric" style="width:100px" placeholder="Calculative" readonly="readonly" /></td><td><input type="button" id="increaseT_1" name="increase[]" style="width:30px" class="formbuttonplasminus" value="+" onClick="add_break_down_tr(1)"/><input type="button" id="decreaseT_1" name="decrease[]" style="width:30px" class="formbuttonplasminus" value="-" onClick="fn_deleteRow(1);" /></td></tr><tr id="tr_2" bgcolor="#E9F3FF" align="center"><td align="left">2</td><td><? echo create_drop_down( "cboThreadType_2", 130,$size_color_sensitive,"",1, "-- Select --", 0, "", "", "1,3", "", "", "", "", "", "cboThreadType[]" ); ?></td><td><input type="text" name="txtThreadDesc[]" id="txtThreadDesc_2" placeholder="Write" class="text_boxes" style="width:180px" onFocus="add_auto_complete( 2 )" /></td><td><input type="text" name="txtThreadLength[]" id="txtThreadLength_2" placeholder="Write" class="text_boxes_numeric" style="width:100px" onKeyUp="calculate_thread(2)" /></td><td><input type="text" name="txtAllowance[]" id="txtAllowance_2" placeholder="Write" class="text_boxes_numeric" style="width:100px" onKeyUp="calculate_thread(2)" onblur="copy_value(this.value,2);" /></td><td><input type="text" name="txtRequired[]" id="txtRequired_2" class="text_boxes_numeric" style="width:100px" placeholder="Calculative" readonly="readonly" /></td><td align="center"><input type="button" id="increaseT_2" name="increase[]" style="width:30px" class="formbuttonplasminus" value="+" onClick="add_break_down_tr(2)"/><input type="button" id="decreaseT_2" name="decrease[]" style="width:30px" class="formbuttonplasminus" value="-" onClick="fn_deleteRow(2);" /></td></tr><tr id="tr_3" bgcolor="#FFFFFF" align="center"><td align="left">3</td><td><? echo create_drop_down( "cboThreadType_3", 130,$size_color_sensitive,"",1, "-- Select --", 0, "", "", "1,3", "", "", "", "", "", "cboThreadType[]" ); ?></td><td><input type="text" name="txtThreadDesc[]" id="txtThreadDesc_3" placeholder="Write" class="text_boxes" style="width:180px" onFocus="add_auto_complete( 3 )" /></td><td><input type="text" name="txtThreadLength[]" id="txtThreadLength_3" placeholder="Write" class="text_boxes_numeric" style="width:100px" onKeyUp="calculate_thread(3)" /></td><td><input type="text" name="txtAllowance[]" id="txtAllowance_3" placeholder="Write" class="text_boxes_numeric" style="width:100px" onKeyUp="calculate_thread(3)" onblur="copy_value(this.value,3);" /></td><td><input type="text" name="txtRequired[]" id="txtRequired_3" class="text_boxes_numeric" style="width:100px" placeholder="Calculative" readonly="readonly" /></td><td align="center"><input type="button" id="increaseT_3" name="increase[]" style="width:30px" class="formbuttonplasminus" value="+" onClick="add_break_down_tr(3)"/><input type="button" id="decreaseT_3" name="decrease[]" style="width:30px" class="formbuttonplasminus" value="-" onClick="fn_deleteRow(3);" /></td></tr><tr id="tr_4" bgcolor="#E9F3FF" align="center"><td align="left">4</td><td><? echo create_drop_down( "cboThreadType_4", 130,$size_color_sensitive,"",1, "-- Select --", 0, "", "", "1,3", "", "", "", "", "", "cboThreadType[]" ); ?></td><td><input type="text" name="txtThreadDesc[]" id="txtThreadDesc_4" placeholder="Write" class="text_boxes" style="width:180px" onFocus="add_auto_complete( 4 )" /></td><td><input type="text" name="txtThreadLength[]" id="txtThreadLength_4" placeholder="Write" class="text_boxes_numeric" style="width:100px" onKeyUp="calculate_thread(4)" /></td><td><input type="text" name="txtAllowance[]" id="txtAllowance_4" placeholder="Write" class="text_boxes_numeric" style="width:100px" onKeyUp="calculate_thread(4)" onblur="copy_value(this.value,4);" /></td><td><input type="text" name="txtRequired[]" id="txtRequired_4" class="text_boxes_numeric" style="width:100px" placeholder="Calculative" readonly="readonly" /></td><td align="center"><input type="button" id="increaseT_4" name="increase[]" style="width:30px" class="formbuttonplasminus" value="+" onClick="add_break_down_tr(4)"/><input type="button" id="decreaseT_4" name="decrease[]" style="width:30px" class="formbuttonplasminus" value="-" onClick="fn_deleteRow(4);" /></td></tr>';
		...
		var bl_update_id=document.getElementById('bl3_update_id').value;
		if(bl_update_id=="") var print_button=0; else var print_button=1;
	...
	function variable_setting_work_study()
	{
		...
		{
			// browse from Quotation Inquery
			$('#txt_style_ref').attr("ondblclick","openmypage_quotation(3)");
			$("#txt_style_ref").removeAttr("placeholder"); 
			$("#txt_style_ref").attr("placeholder",'Browse');
		...
		else if(response==4)
		{
			...
			$('#txt_style_ref').attr("ondblclick","openmypage_quotation(4)");
			$("#txt_style_ref").removeAttr("placeholder"); 
			$("#txt_style_ref").attr("placeholder",'Browse');
		...
		else
		{
			...
			$("#txt_style_ref").removeAttr("placeholder"); 
			$("#txt_style_ref").removeAttr("readonly");
		}
	}
	...
	function openmypage_quotation(type)
	{
		...
		if(type==1)//Quick Costing
		{
			...
			emailwindow.onclose=function()
			{
				...
				var quotation=this.contentDoc.getElementById("hidden_inquiry_id").value; // mrr number
				var [quotation_id,qc_no]=quotation.split('_');

				$("#hidden_quotation_id").val(qc_no);
  				get_php_form_data(quotation_id, "populate_data_from_data_quick_costing", "requires/ws_gsd_controller");
			}
		 }
		else if(type==2)//Quotation
		{
			var page_link='requires/ws_gsd_controller.php?action=quotation_popup_inq'; 
			var title="Search  Popup";
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=980px,height=400px,center=1,resize=0,scrolling=0','../')
			emailwindow.onclose=function()
			{
		...
		else if(type==3)//Order entry
		{
		 	var page_link='requires/ws_gsd_controller.php?action=quotation_popup_job_style'; 
			var title="Search  Popup";
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=980px,height=400px,center=1,resize=0,scrolling=0','../')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0]; 
				var quotation=this.contentDoc.getElementById("hidden_job_id").value; // mrr number
				var [quotation_id,qc_no]=quotation.split('_');
				$("#hidden_quotation_id").val(quotation_id);
  				get_php_form_data(quotation_id, "populate_data_from_data_quick_costing", "requires/ws_gsd_controller");
			}
		...
		{
		 	var page_link='requires/ws_gsd_controller.php?action=order_popup_job_style'; 
			var title="Search  Popup";
			...
			emailwindow.onclose=function()
			{
				...
				var [quotation_id,qc_no]=quotation.split('_');
				$("#hidden_quotation_id").val(quotation_id);
  				get_php_form_data(quotation_id, "populate_data_from_data_order_entry", "requires/ws_gsd_controller");
			}
		 }
...
<script>
	var generateChartData = function () { 
		var bl_update_id=$('#bl_update_id').val();
		var seqNos =''; var weights =''; var ucls =''; var pitchTimes =''; var lcls ='';
		if(bl_update_id!="")
		{
			var graphData=trim(return_global_ajax_value( bl_update_id, 'load_graph_data', '', 'requires/balancing_entry_controller')).split("**");
	...
	var generateChartData2 = function () { 
		var bl2_update_id=$('#bl2_update_id').val();
		var tittles =''; var weights =''; var maxWl =''; var minWl ='';
		if(bl2_update_id!="")
		{
