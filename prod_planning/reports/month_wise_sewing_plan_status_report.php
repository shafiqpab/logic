<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Monthly Sewing Plan Status Report.
Functionality	:	
JS Functions	:
Created by		:	REZA 
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Month Wise Sewing Plan","../../", 1, 1, $unicode,1,1);
?>	
<script>
	...
	function openmypage_job()
	{
		...
		var page_link='requires/month_wise_sewing_plan_status_report_controller.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
	...
	function openmypage_order()
	{
		...
		//alert (data);
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/month_wise_sewing_plan_status_report_controller.php?action=order_no_popup&data='+data,'Order No Popup', 'width=630px,height=420px,center=1,resize=0','../')
		emailwindow.onclose=function()
		{
	...
	function fn_report_generated(type)
	{
		...
		{	
			if(type==1){ var action="report_generate_btn_1";}
			else{ var action="report_generate_btn_2";}
			
			var data="action="+action+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_buyer_name*txt_date_from*txt_date_to*txt_job_no*txt_job_id*txt_order_id*txt_order_no*cbo_date_type*cbo_order_status',"../../");
			freeze_window(3);
			http.open("POST","requires/month_wise_sewing_plan_status_report_controller.php",true);
            ...
            <fieldset style="width:1250px;">
                <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                    ...
                    <tr class="general">
                        <td> 
                           <?
								echo create_drop_down( "cbo_company_id", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down('requires/month_wise_sewing_plan_status_report_controller', this.value, 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/month_wise_sewing_plan_status_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
                        ...
                        <td id="location_td">
                          ...
                          <td>
                            <? 
                                echo create_drop_down( "cbo_order_status", 100, $order_status,"", 1, "-- All  --", $selected, "",0,"" );
                            ?>
                        </td>
                       ...
                       <input type="text" id="txt_order_no" name="txt_order_no" class="text_boxes" style="width:80px" onDblClick="openmypage_order();" placeholder="Wr./Br. Order"  />
                        ...
                        <td>
                            <? 
                               $date_type_arr=array(1=>'Shipment Date',2=>' Plan Date');
							    echo create_drop_down( "cbo_date_type", 80, $date_type_arr,"", 1, "-- Select --", 2, "if(this.value==1)document.getElementById('td_date_caption').innerHTML='Shipment Date';else document.getElementById('td_date_caption').innerHTML='Plan Date';",0,"" );
                            ?>
                        </td>
                        ...
                        <td>
                            ...
                            <input type="button" id="show_button1" class="formbutton" style="width:100px; " value="Show2" onClick="fn_report_generated(2)" />
                        </td>
                    </tr>
                    <tr>
