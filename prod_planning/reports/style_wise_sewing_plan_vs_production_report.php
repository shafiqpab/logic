...
<script>
	 ...
	 function openmypage_job()
	{
		...
		var page_link='requires/style_wise_sewing_plan_vs_production_report_controller.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
	...
	function openmypage_order()
	{
		...
		//alert (data);
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/style_wise_sewing_plan_vs_production_report_controller.php?action=order_no_popup&data='+data,'Order No Popup', 'width=630px,height=420px,center=1,resize=0','../')
		emailwindow.onclose=function()
		{
	...
	function fn_report_generated()
	{
		...
		{	
			var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_buyer_name*txt_date_from*txt_date_to*txt_job_no*txt_job_id*txt_order_id*txt_order_no*cbo_location_id*cbo_date_type',"../../");
			freeze_window(3);
			http.open("POST","requires/style_wise_sewing_plan_vs_production_report_controller.php",true);
            ...
            <fieldset style="width:1060px;">
                <table class="rpt_table" width="1050" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                    ...
                    <tr class="general">
                        <td> 
                           <?
								echo create_drop_down( "cbo_company_id", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down('requires/style_wise_sewing_plan_vs_production_report_controller', this.value, 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/style_wise_sewing_plan_vs_production_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
...
$("#multiselect_dropdown_table_headercbo_company_id a").click(function(){
	...
	function load_buyer_location()
	{
		var company=$("#cbo_company_id").val();
 		load_drop_down( 'requires/style_wise_sewing_plan_vs_production_report_controller',company, 'load_drop_down_buyer', 'buyer_td' );
		load_drop_down( 'requires/style_wise_sewing_plan_vs_production_report_controller', company, 'load_drop_down_location', 'location_td' )
	}
</script>
