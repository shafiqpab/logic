...
function fn_show_report(type)
{
	...
	let sytle_ref_no = $("#txt_ref_no").val();
	if(job_no=="" && sytle_ref_no=="" && order_no=="")
	{
		alert(`Job, Style or Order No Required.`);
		return;
	}

	var data="action=report_generate&type="+type+get_submitted_data_string('cbo_company_name*cbo_buyer_name*cbo_season*cbo_season_year*cbo_job_year*txt_ref_no*txt_job_no*txt_date_from*txt_date_to*txt_order_no*hide_order_id',"../../");
	freeze_window('3');
	http.open("POST","requires/cutoff_wise_cutting_lay_balance_report_controller.php",true);
...
function openmypage_job()
{
	...
	var page_link='requires/cutoff_wise_cutting_lay_balance_report_controller.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&sytle_ref_no='+sytle_ref_no;
...
function openmypage_order()
{
	...
	var page_link='requires/cutoff_wise_cutting_lay_balance_report_controller.php?action=order_no_search_popup&companyID='+companyID+'&job_no='+job_no;
...
function openmypage_style()
{
	if( form_validation('cbo_company_name','Company Name')==false )
		...
		var page_link='requires/cutoff_wise_cutting_lay_balance_report_controller.php?action=style_search_popup&company='+company+'&buyer='+buyer+'&txt_style_ref_no='+txt_style_ref_no+'&txt_style_ref_id='+txt_style_ref_id+'&txt_style_ref='+txt_style_ref;
...
{
		//alert(job_no);
		load_drop_down( 'requires/cutoff_wise_cutting_lay_balance_report_controller',job_no,'load_drop_down_gmts_item','gmt_td' );	
}
         ...
         <fieldset style="width:1090px;">
             <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
             	<thead>
                    ...
                    <th colspan="2" id="cap_cut_date">Shipment Date</th>
                ...
                <tbody>
                    <tr class="general">
                        <td>
							<? echo create_drop_down( "cbo_company_name", 120, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/cutoff_wise_cutting_lay_balance_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" ); 
							?> 
                        </td>
                        ...
                        <td width="60" id="season_year_td"><!-- fpr show3 button -->
                            <? 
                                echo create_drop_down( "cbo_job_year", 60, $year,"", 1, "-- All --", $selected, "",0,"" );
                            ?>
                        </td>
                        
                        <td>  
                          <input type="text" id="txt_job_no" name="txt_job_no" class="text_boxes" style="width:100px" onDblClick="openmypage_job();" placeholder="Browse Or Write"  />
                            <input type="hidden" name="txt_job_no_hidden"  id="txt_job_no_hidden"  />
                        </td>
                       
                        <td>
							<input type="text" name="txt_ref_no" id="txt_ref_no" class="text_boxes" style="width:100px" placeholder="Write"/> 
						</td>
                        <td>
