<?php
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Knitting Plan Report Sample Without Order.
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Knitting Plan Report Sample Without Order", "../../", 1, 1,'',1,1);
...
function func_booking_popup()
{
	...
	//alert (data);
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/knitting_plan_report_sample_without_order_controller.php?action=action_booking_popup&data='+data,'Booking No Popup', 'width=1050px,height=420px,center=1,resize=0','../')
...
function func_machine_popup()
{
	...
	var page_link='requires/knitting_plan_report_sample_without_order_controller.php?action=action_machine_popup&companyID='+companyID;
...
function func_generate_report(presentationType)
{
	...
	var data="action=action_generate_report"+get_submitted_data_string('cbo_type*cbo_company_name*cbo_buyer_name*txt_machine_dia*cbo_party_type*txt_booking_no*txt_program_no*txt_machine_no*txt_date_from*txt_date_to*cbo_knitting_status*cbo_based_on*cbo_year*txtVariableAllocation*txtVariableSMNAllocation',"../../")+'&presentationType='+presentationType;
	//alert(data);
	freeze_window(3);
	http.open("POST","requires/knitting_plan_report_sample_without_order_controller.php",true);
...
function func_generate_report_reponse()
{
 	if(http.readyState == 4) 
	{
		...
		document.getElementById('report_container').innerHTML='<a href="'+response[1]+'" style="text-decoration:none" ><input type="button" value="Convert To Excel" name="excel" id="excel" class="formbutton" style="width:110px"/></a>&nbsp;&nbsp;&nbsp;<input type="button" onClick="new_window()" value="Html Preview" name="Print" class="formbutton" style="width:100px"/>&nbsp;&nbsp;<input type="button" onclick="generate_requisition_report_four()" value="Requisition Print4" name="Print" id="Print4" class="formbutton" style="width:120px;display:none;"/>&nbsp;&nbsp;<input type="button" onclick="generate_knitting_card(6)" value="Knitting Card 6" name="card" id="Print10" class="formbutton" style="width:100px;"/>'; 
		...
		get_php_form_data( company_id, 'company_wise_report_button_setting','requires/knitting_plan_report_sample_without_order_controller' );
...
{
	 print_report( company_id+'*'+program_id, "print", "requires/knitting_plan_report_sample_without_order_controller" ) ;
}
		...
		{ 
			if(i!=rowNo)
			{
				try 
				{
					...
					{
						var booking_noCurrent=$('#booking_no_'+i).val();
						if((booking_no != booking_noCurrent))
						{
							alert("Please Select Same Booking No.");
...
{ 
	...
	print_report(program_ids, "requisition_print", "requires/knitting_plan_report_sample_without_order_controller" ) ;
}
...
{ 
	...
	print_report(program_ids + "**" + typeForAttention, "requisition_print_two", "requires/knitting_plan_report_sample_without_order_controller" ) ;
}
...
{ 
	...
	print_report(program_ids + "**" + typeForAttention, "requisition_print_three", "requires/knitting_plan_report_sample_without_order_controller" ) ;
}
...
{ 
	...
	print_report(program_ids + "**" + typeForAttention, "requisition_print_four", "requires/knitting_plan_report_sample_without_order_controller" ) ;
}
...
function generate_report3(company_id,program_id,formatID,empty1,empty2,sampleWithoutFlat)
{
	var path='../../';  
...
{ 
	...
	if(type==1)
	{
		print_report(program_ids, "knitting_card_print", "requires/knitting_plan_report_sample_without_order_controller" ) ;
	}
	else if(type==2)
	{
		print_report(program_ids, "knitting_card_print_2", "requires/knitting_plan_report_sample_without_order_controller" ) ;
	}else if(type==3)
	{
		print_report(program_ids, "knitting_card_print_3", "requires/knitting_plan_report_sample_without_order_controller" ) ;
	}
	else if(type==4)
	{
		print_report(program_ids, "knitting_card_print_4", "requires/knitting_plan_report_sample_without_order_controller" ) ;
	}
	else if(type==5)
	{
		print_report(program_ids, "knitting_card_print_5", "requires/knitting_plan_report_sample_without_order_controller" ) ;	
	}else if(type==6)
	{
		print_report(program_ids, "knitting_card_print_6", "requires/knitting_plan_report_sample_without_order_controller" ) ;
	}
...
{
	var companyID = $("#cbo_company_name").val();
	var page_link='requires/knitting_plan_report_sample_without_order_controller.php?action='+action+'&companyID='+companyID+'&program_id='+program_id;
	var title='';
...
function variable_chk(companyID)
{
	var response=return_global_ajax_value( companyID, 'check_variable', '', 'requires/knitting_plan_report_sample_without_order_controller');
	var response=response.split("_");
	$('#txtVariableSMNAllocation').val(response[1]);	
	$('#txtVariableAllocation').val(response[2]);	
}
</script>
</head>
...
<form id="knittingStatusReport_1">
    <div style="width:100%;" align="center">    
        <? echo load_freeze_divs ("../../",'');  ?>
         <h3 style="width:1380px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu(this.id,'content_search_panel','')">-Search Panel</h3> 
             ...
             <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                ...
                <tbody>
                    <tr class="general">
                        <td> 
                        <?
                        echo create_drop_down( "cbo_company_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "- Select Company -", $selected, "load_drop_down( 'requires/knitting_plan_report_sample_without_order_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );variable_chk(this.value);" );
                        ?>
                        	<input type="hidden" name="txtVariableAllocation" id="txtVariableAllocation" readonly />
	                        <input type="hidden" name="txtVariableSMNAllocation" id="txtVariableSMNAllocation" readonly />
                        </td>
                        <td id="buyer_td">
                        <? 
                        ...
                        <td>
                        <input type="text" id="txt_booking_no" name="txt_booking_no" class="text_boxes_numeric" style="width:90px" onDblClick="func_booking_popup();" placeholder="Write/Browse Booking"  />
                        ...
                        echo create_drop_down( "cbo_type", 102, $search_by_arr,"",0, "", "0","load_drop_down( 'requires/knitting_plan_report_sample_without_order_controller',this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_party_type', 'party_type_td' );",0 );
                        ?>
                        </td> 
                        <td id="party_type_td">
                        <?
                        ...
                        <td>
                        <input name="txt_machine_no" id="txt_machine_no" class="text_boxes" style="width:90px" onDblClick="func_machine_popup()" placeholder="Browse or Write">
                        </td>
                        <td align="center">
                        <? 
...
<input type="button" id="show_button2" class="formbutton" style="width:60px;" value="Show" onClick="func_generate_report(1)" />
<!--
<input type="button" id="show_button1" class="formbutton" style="width:100px;" value="Job/Order Status" onClick="func_generate_report(2)" />
<input type="button" id="show_button3" class="formbutton" style="width:70px;" value="Summary" onClick="func_generate_report(3)" />
<input type="button" id="show_button4" class="formbutton" style="width:60px;" value="Short" onClick="func_generate_report(4)" />
<input type="button" id="show_button111" class="formbutton" style="width:65px;" value="Color Wise" onClick="func_generate_report(111)" />
-->
