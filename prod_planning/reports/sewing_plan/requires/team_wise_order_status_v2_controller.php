...
function getCorrespondingMonth($date,$type) {
    // Parse the input date
    $date = date_create_from_format('d-M-y', $date);
	if($type == 1){$days = ($date->format('d') <= 20)?0:20;}
	else{$days = "-10";}
    $correspondingMonth = date_modify($date, "$days days")->format('M-Y');
...
{ 
	...
	$cbo_start_month=str_replace("'","",$cbo_start_month);
	$cbo_start_year=str_replace("'","",$cbo_start_year);
	$category_by = str_replace("'","",$cbo_category_by);
	$cbo_end_month=str_replace("'","",$cbo_end_month);
	$cbo_end_year=str_replace("'","",$cbo_end_year);
	...
	if ($type == 1) 
	{
		...
		$daysinmonth=cal_days_in_month(CAL_GREGORIAN, $cbo_end_month, $cbo_end_year);
		$s_date=$cbo_start_year."-".$cbo_start_month."-"."01";
		$e_date=$cbo_end_year."-".$cbo_end_month."-".$daysinmonth;
		if($db_type==2)
		{
		...
		WHERE a.job_no=b.job_no_mst AND b.id=c.po_break_down_id $where_con $date_cond and a.status_active=1  and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 order by a.company_name";
		//echo $sql_con_po;die;
		...
		$date_cond_capacity="AND DATE_CALC between '$s_date' and '$e_date'";
		...
		$date_cond_sales_target="AND SALES_TARGET_DATE between '$s_date' and '$e_date'";
	...
	elseif ($type == 2) //Team Wise 	
	{
		...
		$s_date=$cbo_start_year."-".$cbo_start_month."-1"; // Month Start From 11th
		$e_date=$cbo_end_year."-".$cbo_end_month."-".$daysinmonth; // Month end to next month 10th
		...
		if($category_by == 1){
			$s_date = $cbo_start_year."-".$cbo_start_month."-"."1"; // Month Start From 11th
			$s_date = add_date($s_date,-11);
			$e_date = $cbo_end_year."-".$cbo_end_month."-".$daysinmonth; // Month end to next month 10th
			$e_date = add_date($e_date,-10);
		}
		else{
			$s_date=$cbo_start_year."-".$cbo_start_month."-"."1"; // Month Start From 11th
			$s_date = add_date($s_date,10);
			$e_date=$cbo_end_year."-".$cbo_end_month ."-".$daysinmonth; // Month end to next month 10th
			$e_date = add_date($e_date,10);
		}
		...
		if($cbo_company_id!=''){
			$where_con = " and a.company_name in($cbo_company_id)";
			$where_con2 = " and a.COMPANY_ID in($cbo_company_id)";
		}
		if($cbo_status!='' || $cbo_status!=0){
			$where_con .= " and b.is_confirmed in($cbo_status)";
		}
		if($cbo_team_leader!=''){
			$where_con .= " and a.team_leader in($cbo_team_leader)";
			$where_con2 .= " and a.team_leader in($cbo_team_leader)";
		}

		$date_cond = '';
		if($category_by == 1){
			$date_cond = "and b.pack_handover_date between '$s_date' and '$e_date'";
			$field ="b.pack_handover_date ";

			$date_cond2 = "and b.MATERIAL_RECV_DATE between '$s_date' and '$e_date'";
			$field2 ="b.MATERIAL_RECV_DATE ";
		}elseif($category_by == 2){
			$date_cond = "and b.pub_shipment_date between '$s_date' and '$e_date'";
			$field ="b.pub_shipment_date ";
			$date_cond2 = "and b.DELIVERY_DATE between '$s_date' and '$e_date'";
			$field2 ="b.DELIVERY_DATE ";
	    ...
	    $sql_con_po="SELECT * from(SELECT d.TEAM_NAME, a.COMPANY_NAME, a.SET_SMV,a.LOCATION_NAME,a.TOTAL_SET_QNTY, b.IS_CONFIRMED, b.PO_RECEIVED_DATE, $field as PUB_SHIPMENT_DATE, b.id as PO_ID, c.ORDER_QUANTITY as PO_QUANTITY FROM wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c,lib_marketing_team d WHERE a.job_no=b.job_no_mst and b.id=c.po_break_down_id and d.id=a.team_leader $where_con $date_cond and a.status_active=1  and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0
		union all
		select d.TEAM_NAME,a.COMPANY_ID as COMPANY_NAME,b.SMV as SET_SMV,a.LOCATION_ID as LOCATION_NAME,1 as TOTAL_SET_QNTY,3 as IS_CONFIRMED,b.ORDER_RCV_DATE as PO_RECEIVED_DATE,$field2 as PUB_SHIPMENT_DATE,b.id as PO_ID,b.ORDER_QUANTITY as PO_QUANTITY  from SUBCON_ORD_MST a,SUBCON_ORD_DTLS b,lib_marketing_team d  where a.SUBCON_JOB = b.JOB_NO_MST and d.id=a.team_leader $where_con2 $date_cond2 and a.is_deleted=0 and a.status_active=1  and b.is_deleted=0 and b.status_active=1) dum order by dum.TEAM_NAME,dum.COMPANY_NAME";
		...
		{ 
			...
			$monthKey=getCorrespondingMonth($row_po['PUB_SHIPMENT_DATE'],$category_by); //Declared on the top
			//echo $monthKey.',';
					...
					<thead>
						<tr>
							...
							{ 
								if($category_by == 1){
								?>
								<th colspan="3" title="21-<?= date('M-Y',strtotime(month_add($monthVal,-1))); ?> to 20-<?= $monthVal ; ?>"> <?=$monthVal ?>  </th>
								<?
								}
								else{
								?>
						...
						<?
							...
							{ 
								...
								if (in_array(1,$statusArr)) 
								{
									...
									<tr bgcolor="<?=$bgcolor;?>" onClick="change_color('tr_<?=$i; ?>','<?=$bgcolor;?>')" id="tr_<?=$i; ?>" style="font-size:12px">
										...
										foreach($month_arr as $monthVal)
										{
											...
											$team_po_min=$poMinuteArr['MIN'][1][$monthVal][$company_team] + $poMinuteArr['MIN'][2][$monthVal][$company_team] + $poMinuteArr['MIN'][3][$monthVal][$company_team];
											$g_po_min=array_sum($poMinuteArr['MIN'][1][$monthVal]) + array_sum($poMinuteArr['MIN'][2][$monthVal]) + array_sum($poMinuteArr['MIN'][3][$monthVal]);
								...
								{	
									?>
										<tr bgcolor="<?=$bgcolor;?>" onClick="change_color('tr_<?=$i; ?>','<?=$bgcolor;?>')" id="tr_<?=$i; ?>" style="font-size:12px">
											...
											foreach($month_arr as $monthVal)
											{
												$po_qty=$poMinuteArr['QTY'][3][$monthVal][$company_team];
												$po_min=$poMinuteArr['MIN'][3][$monthVal][$company_team];
												$total_po_qty += $po_qty;
												$total_po_min += $po_min;
												?>
												<td align="right"><?=fn_number_format($po_qty); ?></td>
								...
								<tr bgcolor='#C2DDF2'>
									...
									{
										if (in_array(1,$statusArr)){
											$tDataArr['QTY'][1] = $poMinuteArr['QTY'][1][$monthVal][$company_team];
											$tDataArr['MIN'][1] = $poMinuteArr['MIN'][1][$monthVal][$company_team];
											$tDataArr['SMIN'][1] = array_sum($poMinuteArr['MIN'][1][$monthVal]);
										}
										if (in_array(2,$statusArr)){
											$tDataArr['QTY'][2] = $poMinuteArr['QTY'][2][$monthVal][$company_team];
											$tDataArr['MIN'][2] = $poMinuteArr['MIN'][2][$monthVal][$company_team];
											$tDataArr['SMIN'][2] = array_sum($poMinuteArr['MIN'][2][$monthVal]);
										}
										if (in_array(3,$statusArr)){
											$tDataArr['QTY'][3] = $poMinuteArr['QTY'][3][$monthVal][$company_team];
											$tDataArr['MIN'][3] = $poMinuteArr['MIN'][3][$monthVal][$company_team];
											$tDataArr['SMIN'][3] = array_sum($poMinuteArr['MIN'][3][$monthVal]);
										}
										
										
										$team_po_qty = $tDataArr['QTY'][1] + $tDataArr['QTY'][2] + $tDataArr['QTY'][3];
										$team_po_min = $tDataArr['MIN'][1] + $tDataArr['MIN'][2] + $tDataArr['MIN'][3];
										$g_po_min = $tDataArr['SMIN'][1] + $tDataArr['SMIN'][2] + $tDataArr['SMIN'][3];
						...
						<tfoot>
							<tr bgcolor="#EDE0B7">
								...
								foreach($month_arr as $monthVal)
								{
									...
									if (in_array(1,$statusArr)){
										$gDataArr['SQTY'][1] = array_sum($poMinuteArr['QTY'][1][$monthVal]);
										$gDataArr['SMIN'][1] = array_sum($poMinuteArr['MIN'][1][$monthVal]);
									}
									if (in_array(2,$statusArr)){
										$gDataArr['SQTY'][2] = array_sum($poMinuteArr['QTY'][2][$monthVal]);
										$gDataArr['SMIN'][2] = array_sum($poMinuteArr['MIN'][2][$monthVal]);
									}
									if (in_array(3,$statusArr)){
										$gDataArr['SQTY'][3] = array_sum($poMinuteArr['QTY'][3][$monthVal]);
										$gDataArr['SMIN'][3] = array_sum($poMinuteArr['MIN'][3][$monthVal]);
									}	
									
									
									// $g_po_qty=array_sum($poMinuteArr['QTY'][1][$monthVal])+array_sum($poMinuteArr['QTY'][2][$monthVal])+array_sum($poMinuteArr['QTY'][3][$monthVal]);
									// $g_po_min=array_sum($poMinuteArr['MIN'][1][$monthVal])+array_sum($poMinuteArr['MIN'][2][$monthVal])+array_sum($poMinuteArr['MIN'][3][$monthVal]);

									$g_po_qty = $gDataArr['SQTY'][1] + $gDataArr['SQTY'][2] + $gDataArr['SQTY'][3];
									$g_po_min = $gDataArr['SMIN'][1] + $gDataArr['SMIN'][2] + $gDataArr['SMIN'][3];
	...
	elseif ($type == 3)  //Buyer Wise    
	{
		...
		$s_date=$cbo_start_year."-".$cbo_start_month."-"."1";
		$e_date=$cbo_end_year."-".$cbo_end_month."-".$daysinmonth; 
		$s_date=change_date_format($s_date,'yyyy-mm-dd','-',1);
		$e_date=change_date_format($e_date,'yyyy-mm-dd','-',1);
		...
		if($category_by == 1){
			...
			// $s_date=$cbo_start_year."-".$cbo_start_month."-"."11"; // Month Start From 11th
			// $e_date=$cbo_end_year."-".$cbo_end_month."-".'10'; // Month end to next month 10th
			$s_date=$cbo_start_year."-".$cbo_start_month."-"."1"; // Month Start From 11th
			$s_date = add_date($s_date,10);
			$e_date=$cbo_end_year."-".$cbo_end_month ."-".$daysinmonth; // Month end to next month 10th
			$e_date = add_date($e_date,10);
		...
		if($cbo_company_id!=''){
			$where_con = " and a.company_name in($cbo_company_id)";
			$where_con2 = " and a.company_name in($cbo_company_id)";
		}
		if($cbo_buyer_name!=''){
			$where_con = " and a.buyer_name in($cbo_buyer_name)";
			$where_con2 = " and a.buyer_name in($cbo_buyer_name)";
		}
		if($cbo_status!='' || $cbo_status!=0){
			$where_con.=" and b.is_confirmed in($cbo_status)";
		}
		...
		if($category_by == 1){
			$date_cond = "and b.pack_handover_date between '$s_date' and '$e_date'";
			$field ="b.pack_handover_date ";
			$date_cond2 = "and b.pack_handover_date between '$s_date' and '$e_date'";
			$field2 ="b.pack_handover_date ";
		}elseif($category_by == 2){
			$date_cond = "and b.pub_shipment_date between '$s_date' and '$e_date'";
			$field ="b.pub_shipment_date ";
			$date_cond2 = "and b.pub_shipment_date between '$s_date' and '$e_date'";
			$field2 ="b.pub_shipment_date ";
		}elseif($category_by == 3){
			$date_cond = "and b.shipment_date between '$s_date' and '$e_date'"; 
			$field ="b.shipment_date ";
			$date_cond2 = "and b.shipment_date between '$s_date' and '$e_date'"; 
			$field2 ="b.shipment_date ";
		}elseif($category_by == 4){
			$date_cond = "and c.country_ship_date between '$s_date' and '$e_date'";
			$field ="c.country_ship_date "; 
			$date_cond2 = "and c.country_ship_date between '$s_date' and '$e_date'";
			$field2 ="c.country_ship_date "; 
		}
		...
		WHERE a.job_no=b.job_no_mst AND b.id=c.po_break_down_id And a.team_leader=d.id $where_con $date_cond and a.status_active=1  and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 order by d.team_name, a.company_name";
		// echo $sql_con_po;die;
					...
					<thead>
						<tr>
							...
							{ 
								...
								if($category_by == 1){
									?>
										<th colspan="2" title="21-<?= date('M-Y',strtotime(month_add($monthVal,-1))); ?> to 20-<?= $monthVal ; ?>"> <?=$monthVal ?>  </th>
									<?
									}
									else{
									?>
										<th colspan="2" title="11-<?=$monthVal; ?> to 10-<?= get_next_month($monthVal) ; ?>"> <?=$monthVal ?> </th>
									<?
									}
