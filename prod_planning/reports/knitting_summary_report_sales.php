...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Knitting Summary Report[Sales]", "../../", 1, 1, '', 1, 1);
?>
<script>
    ...
    function fn_report_generated(presentationType)
	{
        ...
        freeze_window(3);
        http.open("POST", "requires/knitting_summary_report_sales_controller.php", true);
    ...
    function fn_report_generated_reponse()
	{
        if (http.readyState == 4)
		{
            ...
            get_php_form_data( company_id, 'company_wise_report_button_setting','requires/knitting_summary_report_sales_controller' );
            show_msg('3');
            release_freezing();
        }
    }
    ...
    function openmypage_booking()
	{
        ...
        var page_link = 'requires/knitting_summary_report_sales_controller.php?action=booking_no_search_popup&companyID=' + companyID;
    ...
    function openmypage_sales_no()
	{
        ...
        var companyID = $("#cbo_company_name").val();
        var page_link = 'requires/knitting_summary_report_sales_controller.php?action=job_no_search_popup&companyID=' + companyID;
    ...
    function fn_open_machine()
	{
        ...
        var companyID = $("#cbo_company_name").val();
        var page_link = 'requires/knitting_summary_report_sales_controller.php?action=machine_no_search_popup&companyID=' + companyID;
	...
	{
		var companyID = $("#cbo_company_name").val();
		var page_link='requires/knitting_summary_report_sales_controller.php?action='+action+'&companyID='+companyID+'&program_id='+program_id;
		var title='';
	...
	function func_req_qty(req_id)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/knitting_summary_report_sales_controller.php?req_id='+req_id+'&action=req_qty_popup', 'Requisition Details', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../');
	}
	
	//for generate_report
	function generate_report(company_id,program_id)
	{
		 print_report( company_id+'*'+program_id + '*' + '../../', "print", "../requires/yarn_requisition_entry_sales_controller" ) ;
	}
	...
	function openmypage(ids,action)
	{
		...
		var page_link='requires/knitting_summary_report_sales_controller.php?action='+action+'&companyID='+companyID+'&cbo_year='+cbo_year+'&ids='+ids;
            ...
            <fieldset style="width:1450px;">
                <table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                    ...
                    <tr class="general">
                        ...
                        <td>
							<?
							echo create_drop_down("cbo_buyer_name", 110, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "- All Company -", $selected, "load_drop_down( 'requires/knitting_summary_report_sales_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );", 0, "");
							?>
                        </td>
                        ...
                        <td>
							...
							echo create_drop_down("cbo_type", 102, $search_by_arr, "", 0, "", "", "load_drop_down( 'requires/knitting_summary_report_sales_controller',this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_party_type', 'party_type_td' );", 0);
							?>
                        </td>
                        <td id="party_type_td">
							<?
