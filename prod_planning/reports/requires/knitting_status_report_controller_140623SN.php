...
if ($action == "report_generate")
{
	...
	if ($template == 1)
	{
		...
		if ($presentationType == 1 || $presentationType == 6) // Show
		{
			if ($txt_machine_no != "")
			{
				if ($db_type == 0) {
		...
		$r_id8=execute_query("delete from tmp_prod_id where userid=$user_name");
		...
		if(!empty($program_arr))
		{
			if ($db_type == 0)
			{
				$field_pi = " group_concat(distinct(a.po_id)) as po_id";
				$field_prd = " group_concat(distinct(a.prod_id)) as prod_id,";
			}
			else
			{
				$field_pi = " LISTAGG(a.po_id, ',') WITHIN GROUP (ORDER BY a.po_id) as po_id";
				$field_prd = " LISTAGG(a.prod_id, ',') WITHIN GROUP (ORDER BY a.prod_id) as prod_id,";
			}

			$plan_details_array = return_library_array("select a.dtls_id, ".$field_pi." from ppl_planning_entry_plan_dtls a, tmp_prog_no b where a.dtls_id=b.prog_no and b.userid=$user_name and a.company_id=$company_name group by a.dtls_id", "dtls_id", "po_id");

			//for requisition
			$reqsDataArr = array();
			$reqsData = sql_select("select a.knit_id, max(a.requisition_no) as reqs_no, ".$field_prd." sum(a.yarn_qnty) as yarn_req_qnty from ppl_yarn_requisition_entry a, tmp_prog_no b where a.knit_id=b.prog_no and b.userid=$user_name and a.status_active=1 and a.is_deleted=0 group by a.knit_id, a.requisition_no");
			foreach ($reqsData as $row)
			{
				...
				if(!$prod_arr[$row[csf('prod_id')]])
				{
					...
					$prod_ids=array_unique(explode(",",$row[csf('prod_id')]));
					//var_dump($prod_ids);
					foreach($prod_ids as $prod_id)
					{
						$rID6=execute_query("insert into tmp_prod_id (userid, prod_id) values ($user_name,$prod_id)");
					}
			    	//$rID6=execute_query("insert into tmp_prod_id (userid, prod_id) values ($user_name,".$row[csf('prod_id')].")");
				}
			}
			...
			if(!empty($requisition_no_arr))
			{
				...
				$yarnIssueRtnData = sql_select("SELECT a.booking_id as reqsn_no, b.prod_id, sum(b.cons_quantity) as qnty, sum(b.cons_reject_qnty) as reject_qnty from inv_receive_master a, inv_transaction b, tmp_reqs_no c where a.id=b.mst_id and a.receive_basis=3 and a.entry_form=9 and b.item_category=1 and b.transaction_type=4 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.booking_id=c.reqs_no and c.userid=$user_name group by a.booking_id, b.prod_id");
			...
			if(!empty($receive_ids_arr))
			{
				...
				if(!empty($barcode_no_arr))
				{
					...
					//end for receive delivery

					//for delivery qty
					$deliveryquantityArr = sql_select("select a.booking_no, b.current_delivery as current_delivery ,count(b.barcode_num) as roll_no_delv from pro_roll_details a,pro_grey_prod_delivery_dtls b, tmp_barcode_no c where a.mst_id=b.grey_sys_id and a.barcode_no=b.barcode_num and a.entry_form=2 and a.receive_basis=2 and a.booking_without_order=0 and b.barcode_num=c.barcode_no and a.barcode_no=c.barcode_no and c.userid=$user_name and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0  group by a.booking_no,a.barcode_no,b.current_delivery,b.barcode_num");
		...
		if(!empty($prod_arr))
		{
			...
			$pro_sql = sql_select("select a.id, a.product_name_details, a.lot, a.supplier_id from product_details_master a, tmp_prod_id b where a.id = b.prod_id and a.company_id=$company_name and a.item_category_id=1 and b.userid=$user_name");
		...
		a, tmp_poid b where a.booking_no=c.booking_no and a.PO_BREAK_DOWN_ID=b.poid and b.userid=$user_name and c.company_id=$company_name group by a.booking_no,a.job_no,a.PO_BREAK_DOWN_ID,a.GSM_WEIGHT,a.FABRIC_COLOR_ID,a.color_type");
									...
									if (!in_array($machine_dia_gg, $machine_dia_gg_array[$knitting_source]))
									{
										if ($k != 1)
										{
											...
											<tr bgcolor="#CCCCCC">
												...
												<? if($presentationType == 6){ ?>
												<td width="100" align="right"><b><? echo number_format($sub_knit_ballance_iw_qnty, 2, '.', ''); ?></b></td>
												<? }?>
		...
		<table style="margin-top:10px; font-family: arial-narrow;" width="100%" border="1" rules="all" cellpadding="0" cellspacing="0" class="rpt_table" align="center">
				...
				foreach ($nameArray as $row)
				{
					...
					if (count($result_stripe) > 0) {
						...
						<tfoot>
							<th colspan="4">Total</th>
							<th style="text-align: center;"> <? echo $tot_masurement; ?>&nbsp;</th>
