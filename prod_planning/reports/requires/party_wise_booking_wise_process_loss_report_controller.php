				...
				<table width="770" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
					...
					<tbody>
						<tr>
							...
							<td align="center">
								<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view('<? echo $companyID; ?>' + '**' + document.getElementById('cbo_buyer_name').value + '**' + document.getElementById('cbo_search_by').value + '**' + document.getElementById('txt_search_common').value + '**' + document.getElementById('txt_date_from').value + '**' + document.getElementById('txt_date_to').value+'**', 'create_job_no_search_list_view', 'search_div', 'party_wise_booking_wise_process_loss_report_controller', 'setFilterGrid(\'tbl_list_search\',-1)');" style="width:100px;" />
							</td>
						</tr>
						<tr>
...
if ($action == "report_generate")
{
	...
	if ($report_type == 1) 
	{
		...
		$main_sql = "SELECT a.id, a.company_id,a.booking_no,a.uom, a.is_short,b.job_no,a.buyer_id, b.po_break_down_id,c.grouping, c.po_number, b.grey_fab_qnty as qnty,d.style_ref_no
			...
			and b.grey_fab_qnty>0 $job_cond $booking_cond $booking_date_cond $company_cond $internalRefNo_cond
        ...
        $progQnty_sql = "SELECT a.booking_no,a.buyer_id,a.po_id,
			...
			where d.id=a.mst_id and a.dtls_id = b.id and a.booking_no = c.booking_no $program_job_cond $company_cond
			and a.status_active=1 and a.is_deleted=0 and a.is_sales!=1
			and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0 order by b.knitting_party,b.id,a.booking_no";  
		...
		$prog_data_array[$row[csf("knitting_party")]][$row[csf("program_id")]]['buyer_id'] = $row[csf("buyer_id")];
		$prog_data_array[$row[csf("knitting_party")]][$row[csf("program_id")]]['booking_no'] = $row[csf("booking_no")];
		$prog_data_array[$row[csf("knitting_party")]][$row[csf("program_id")]]['knitting_source'] = $row[csf("knitting_source")];
		$prog_data_array[$row[csf("knitting_party")]][$row[csf("program_id")]]['buyer_id'] = $row[csf("buyer_id")];
		$prog_data_array[$row[csf("knitting_party")]][$row[csf("program_id")]]['po_id'] .= $row[csf("po_id")].',';
		
		$prog_data_array[$row[csf("knitting_party")]][$row[csf("program_id")]]['style_ref_no'] = $style_ref_no;
		$prog_data_array[$row[csf("knitting_party")]][$row[csf("program_id")]]['ref_no'] = $ref_no;
		...
		{
			$sql_grey_recv_trans="SELECT d.booking_no as prog_no,
			sum(b.reject_qty) as reject_qty, sum(b.quantity) as qnty from pro_grey_prod_entry_dtls a,inv_receive_master d, 
			order_wise_pro_details b,product_details_master e where d.id=a.mst_id and a.id=b.dtls_id and e.id=a.prod_id and e.id=b.prod_id and e.id=a.prod_id and 
			a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.entry_form in(2) and d.item_category=13 and b.trans_type in(1) $po_ids_cond
			group by d.booking_no ";

			// $sql_grey_recv_trans="SELECT c.booking_no as prog_no,sum(a.cons_reject_qnty) as reject_qty, sum(b.quantity) as qnty from inv_transaction a, order_wise_pro_details b,inv_receive_master c where a.id=b.trans_id and c.id=a.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.entry_form in(2) and a.item_category=13 and a.transaction_type in(1) and b.trans_type in(1) $po_ids_cond group by c.booking_no";
			//echo $sql_grey_recv_trans;
		...
		ob_start();
		?>
		<fieldset style="width:1210px;margin:0 auto;">
			...
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1190" class="rpt_table" >
				<thead>
					...
					<th width="80">Reject Yarn Issue Returned</th>
					<th width="80">Before Process Loss Balance</th>
			...
			<div style="width:1210px; overflow-y:scroll; max-height:330px;" id="scroll_body">
				<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1190" class="rpt_table" id="tbl_list_search">
					...
					<tbody>
						...
						$total_issue_qnty=$grand_grey_recv_qty=$grand_before_process_loss_balance=$grand_process_loss_qty=$grand_after_process_loss_balance=0;
						foreach($prog_data_array as $party_id => $party_data)
						{
							$sub_grey_recv_reject=$sub_fab_recv_qnty=$grand_grey_recv_reject=$sub_yarn_reject_qnty=$grand_yarn_reject_qnty=$sub_process_loss_qty=$sub_requ_balance=$sub_issue_qnty=$sub_issue_ret_qnty=$sub_issue_balance=$sub_prod_qnty=$sub_before_process_loss_balance=$sub_reject_qnty=$sub_balance_qnty=0;
						foreach($party_data as $prog_no => $row)
						 {
								if ($m % 2 == 0)
									$bgcolor = "#E9F3FF";
								else
								...
								//	if ($prog_Qnty == 0) $program_bgcolor = "background-color:red;";else $program_bgcolor= "";
									...
									if (!in_array($row["booking_no"],$booking_array) )
									{
										if($z!=1)
										{
											?>                                        
											<tr style="font-weight: bold;background-color: #e0e0e0">
		                                        ...
		                                        <td colspan="8" align="right" width="530">Fab. Booking Total: </td>
		                                        
		                                        <td width="80" align="right" ><p><? echo number_format($booking_sub_yarn_issue,2);//$sub_issue_qnty=0;?></p></td>
		                                        <td width="80" align="right" ><p><? echo number_format($booking_sub_grey_recv_qty,2);?></p></td>                                        
		                                        <td width="80" align="right" ><p><? echo number_format($booking_sub_grey_recv_reject,2);?></p></td>
		                                        <td width="80" align="right" ><p><? echo number_format($booking_sub_issue_ret_qnty,2);?></p></td>
		                                        <td width="80" align="right"><p><? echo number_format($booking_sub_yarn_reject_qnty,2);?></p></td>
		                                        <td width="80" align="right" ><p><? echo number_format($booking_sub_before_process_loss_balance,2);?></p></td>
		                                        <td width="80" align="right"><p><? echo number_format($booking_sub_process_loss_qty,2);?></p></td>
		                                        <td align="right"  ><p><? echo number_format($booking_sub_after_process_loss_balance,2);?></p></td>
		                                    </tr>
											<?	
											unset($booking_sub_yarn_issue);unset($booking_sub_grey_recv_qty);unset($booking_sub_grey_recv_reject);
											unset($booking_sub_grey_recv_reject);unset($booking_sub_yarn_reject_qnty);unset($booking_sub_before_process_loss_balance);
											unset($booking_sub_before_process_loss_balance);unset($booking_sub_process_loss_qty);unset($booking_sub_after_process_loss_balance);
										}
									
										$booking_array[$z]=$row["booking_no"];
										$z++;
									...
									{ ?>
                                        ...
                                        <tr bgcolor="#dddddd">
										<td colspan="16" align="left" ><b>Party Name: <? if ($row[("knitting_source")]==1) echo $company_library[$party_id]; else if ($row[("knitting_source")]==3) echo $supplier_arr[$party_id]; ?></b>
                                        </td>
										</tr>
                                        <?
									...
									<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $m; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $m; ?>"> 
										...
										<td width="50"><p>&nbsp;<? echo $prog_no;?></p></td>
										<td width="70"><? echo $reqs_array[$prog_no]['requ_no'];?></td>
										
										<td width="100"><p style="word-break: break-all;">&nbsp;<? echo $row["booking_no"];?></p></td>
										<td width="70"><p style="word-break: break-all;">&nbsp;<? echo $buyer_arr[$row['buyer_id']];?></p></td>													
										<td width="80"><p style="word-break: break-all; text-align: center;">&nbsp;<? 
											$internal_ref_no="";
										...
										<td  width="80"><p style="word-break: break-all;">&nbsp;<? echo chop($internal_ref_no,',');?></p></td>
										<td width="40"  align="center"><p><? echo $unit_of_measurement[12];?></p></td>
										<td width="80" align="right" style=""><a href='##' onClick="openmypage_issue('<? echo $reqs_array[$prog_no]['requ_no']."_".$row["booking_no"]. "_" . $po_ids. "_" . $prog_no; ?>', 'issue_popup')"><p><? echo number_format($issue_qnty,2);?></p></a></td>
										<td width="80" align="right" style=""><a href='##' onClick="openmypage_issue('<? echo $reqs_array[$prog_no]['requ_no']."_".$row["booking_no"]. "_" . $po_ids. "_" . $prog_no."_".$companyID; ?>', 'grey_recv_popup')"><p><? echo number_format($grey_recv_qty,2);?></p></a></td>
										<td width="80" align="right" style="">
											<!-- <a href='##' onClick="openmypage_program('<? echo $prog_no; ?>', 'program_popup')"> -->
												<p><? echo number_format($grey_recv_reject,2);?></p>
											<!-- </a> -->
										</td>
										<td width="80" align="right" ><p><? echo number_format($issue_ret_qnty,2);?></p></td>
										<td width="80" align="right" style="" title="Requisition no=<? echo $requ_nos;?>">
											<!-- <a href='##' onClick="openmypage_requisition('<? echo $program_ids; ?>', 'requisition_popup')"> -->
												<p><? echo number_format($yarn_reject_qnty,2);?></p>
											<!-- </a> -->
										</td>
										<td  width="80" title="Yarn Issue-(Grey Recv+Grey Reject+Yarn Issue Return)" align="right"><p><? echo number_format($before_process_loss_balance,2);?></p></td>
										<td width="80" title="<? if($ref_closing_status==1) echo "Ref Full Close";else echo " Not Ref Close";?>" align="right"><p><? 
										...
										{ 
										 ...
										 echo $process_loss_qty;
										 $after_process_loss_balance=$before_process_loss_balance-$process_loss_qty;
										 ?></p></td>
										<td   title="Before Process Loss-Full Process Loss Qty" align="right"><p><? echo number_format($after_process_loss_balance,2);?></p></td>
									</tr>
									<?
									...
									$sub_yarn_reject_qnty +=$yarn_reject_qnty;
									$sub_before_process_loss_balance += $before_process_loss_balance;
									$sub_reject_qnty += $reject_qnty;
									$sub_fab_recv_qnty += $grey_recv_qty;
									$sub_grey_recv_reject += $grey_recv_reject;
									$sub_process_loss_qty += $process_loss_qty;
									$sub_after_process_loss_balance += $after_process_loss_balance;
									...
									$show = true;
									$grand_grey_recv_qty +=  $grey_recv_qty;
									$grand_grey_recv_reject += $grey_recv_reject;
                                ...
                                <tr style="font-weight: bold;background-color: #e0e0e0">
									...
									<td colspan="8" align="right" width="530">Party Total </td>
									
									<td width="80" align="right"><p><? echo number_format($sub_issue_qnty,2);//$sub_issue_qnty=0;?></p></td>
									<td width="80" align="right"><p><? echo number_format($sub_fab_recv_qnty,2);?></p></td>                                        
									<td width="80" align="right"><p><? echo number_format($sub_grey_recv_reject,2);?></p></td>
									<td width="80" align="right"><p><? echo number_format($sub_issue_ret_qnty,2);?></p></td>
									<td width="80" align="right"><p><? echo number_format($sub_yarn_reject_qnty,2);?></p></td>
									<td width="80" align="right"><p><? echo number_format($sub_before_process_loss_balance,2);?></p></td>
									<td width="80" align="right"><p><? echo number_format($sub_process_loss_qty,2);?></p></td>
						...
						<tr style="font-weight: bold;background-color: #ccc;border-top: 2px solid">
							...
							<td colspan="8" align="right" width="530">Grand Total </td>
							
							<td width="80" align="right"><p><? echo number_format($grand_issue_qnty,2)?></p></td>
							<td width="80" align="right"><p><? echo number_format($grand_grey_recv_qty,2);?></p></td>                                        
							<td width="80" align="right"><p><? echo number_format($grand_grey_recv_reject,2);?></p></td>
							<td width="80" align="right"><p><? echo number_format($grand_issue_ret_qnty,2);?></p></td>
							<td width="80" align="right"><p><? echo number_format($grand_yarn_reject_qnty,2);?></p></td>
							<td width="80" align="right"><p><? echo number_format($grand_before_process_loss_balance,2);?></p></td>
							<td width="80" align="right"><p><? echo number_format($grand_process_loss_qty,2);?></p></td>
		...
		<div style="width:100%" id="report_container">
           ...
           <caption><b>Company: &nbsp;&nbsp; <? echo $company_library[$result[0][csf('company_id')]]; ?>  &nbsp;<b>Booking No: &nbsp;&nbsp; <? echo $bookingNo; ?> </b></caption>
				...
				<thead>
					...
					<th width="100">Req.No./ Booking No</th>
			...
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1040" class="rpt_table" id="tbl_list_search">	
				...
				foreach ($result as $row) {
					...
					<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
						...
						<td width="100" align="center" ><p><? echo $booking_no."<br/>"; if($row[csf('issue_basis')]==3){echo $bookingNo;}  ?>&nbsp;</p></td>
						<td width="70"><p><? echo $prog_no;//$row[csf('challan_no')]; ?>&nbsp;</p></td>
			...
			$sql = "SELECT a.recv_number, a.receive_date,a.receive_basis, a.supplier_id,a.knitting_source,a.knitting_company,a.booking_no, a.receive_basis, sum(b.quantity) as issue_qnty_ret,sum(b.reject_qty) as reject_qty, c.lot, c.yarn_type, c.id as prod_id, c.product_name_details,c.yarn_count_id, d.brand_id,d.requisition_no from inv_receive_master a, order_wise_pro_details b, product_details_master c, inv_transaction d where a.id=d.mst_id and d.item_category=1 and d.id=b.trans_id and a.entry_form=9 and a.item_category =1 and a.receive_basis = 3 and d.transaction_type=4 and d.item_category=1 and a.booking_no in ($requ_ids) and b.po_breakdown_id in($po_ids) and b.prod_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and  b.issue_purpose!=2  group by a.id,c.id,a.recv_number, a.receive_date, a.knitting_source,a.knitting_company, a.booking_no,a.supplier_id, a.receive_basis, c.lot, c.yarn_type, c.yarn_count_id,c.product_name_details, d.brand_id,d.requisition_no";
				...
				foreach ($result as $row) {
					...
					<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trd_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="trd_<? echo $i; ?>">
						...
						<td width="100" align="center" ><p><? echo $booking_no."<br/>"; if($row[csf('receive_basis')]==3){echo $bookingNo;}  ?>&nbsp;</p></td>
						<td width="70"><p><? echo $prog_no;//$row[csf('challan_no')]; ?>&nbsp;</p></td>
						<td width="70" align="center"><? echo $count_arr[$row[csf('yarn_count_id')]]; ?></td>
                        ...
                        <td align="right" width="">
							<?
							echo number_format($row[csf('reject_qty')], 2, '.', '');
							?>
						</td>
		...
		<div style="width:100%" id="report_container">
            ...
            <caption><b>Company: &nbsp;&nbsp; <? echo $company_library[$result_recv[0][csf('company_id')]]; ?>  &nbsp;<b>Booking No: &nbsp;&nbsp; <? echo $bookingNo; ?> </b></caption>
				...
				<thead>
					...
					<th width="70">Receive Reject Qty</th>
					<th width="">Roll No</th>
			...
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1040" class="rpt_table" id="tbl_list_search">	
				...
				foreach ($result_recv as $row) {
					...
					<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
						...
						<td width="70" align="right"><p><? echo $row[csf('reject_qty')]; ?></p></td>
						<td align="right" width="">
							<?
