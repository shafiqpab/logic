       ...
       if ($db_type == 0){
		...
		else
		{
		   //$reqsData = sql_select("select a.knit_id, max(a.requisition_no) as reqs_no, LISTAGG(a.prod_id, ',') WITHIN GROUP (ORDER BY a.prod_id) as prod_id from ppl_yarn_requisition_entry a,tmp_prog_no b where a.knit_id=b.prog_no and b.userid=$user_name and  a.status_active=1 and a.is_deleted=0  group by a.knit_id,a.requisition_no");
            ...
            $reqsData = sql_select("SELECT A.ID, A.KNIT_ID, A.REQUISITION_NO, A.PROD_ID, A.YARN_QNTY FROM PPL_YARN_REQUISITION_ENTRY A, TMP_PROG_NO B WHERE A.KNIT_ID=B.PROG_NO AND B.USERID=".$user_name." AND A.STATUS_ACTIVE=1 AND A.IS_DELETED=0 ORDER BY A.KNIT_ID, A.REQUISITION_NO ASC");
		}
		...
		if(!empty($reqsDataArr))
		{
			...
			$pro_sql = sql_select("select a.id, a.product_name_details, a.lot, a.supplier_id from product_details_master a, tmp_prod_id b where a.id = b.prod_id and b.userid = $user_name and a.company_id=$company_name and a.item_category_id=1");
		...
		if ($db_type == 0)
		{
           $knitting_dataArray = sql_select("select a.booking_id, group_concat(a.id) as knit_id,group_concat(b.gsm) as gsm, sum(b.grey_receive_qnty) as knitting_qnty,sum(reject_fabric_receive) as fabric_reject_qty,sum(b.grey_receive_qnty_pcs) as grey_receive_qnty_pcs from tmp_prog_no c, inv_receive_master a, pro_grey_prod_entry_dtls b where a.booking_id=c.prog_no and c.userid=$user_name and a.id=b.mst_id and item_category=13 and a.entry_form=2 and a.receive_basis=2 and b.status_active=1 and b.is_deleted=0 group by a.booking_id");
		}
		else
		{
        ...
        $r_id6=execute_query("delete from tmp_booking_id where userid=$user_name");
        if($r_id6) $flag=1; else $flag=0;
        
        $r_id7=execute_query("delete from tmp_prod_id where userid=$user_name");
        if($r_id7) $flag=1; else $flag=0;
    					...
    					foreach ($nameArray as $row) 
                        {
    						...
    						if (!in_array($machine_dia_gg, $machine_dia_gg_array[$knitting_source])) 
                            {
    							...
    							if ($knitting_source_tmp[$knitting_source] == '') {
    								...
    								?>
    								<tr bgcolor="#C6D9C9">
    									<td style="font-weight: 900" colspan="<? echo $colspan + 9+4+2; ?>"><b><?php echo $knitting_cond; ?></b></td>
    								</tr>
    								<?
    							...
    							?>
    							<tr bgcolor="#EFEFEF">
    								<td colspan="<? echo $colspan + 9+4+2; ?>"><b>Machine Dia:- <?php echo $machine_dia_gg; ?></b></td>
    							</tr>
    							<?
    						...
    						<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
    							...
    							<td align="right" width="80"><? echo number_format($req_bal_qnty, 2); ?></td>
...
if ($action == "report_generate_09122021")
{
    $started = microtime(true);
	$process = array(&$_POST);
	extract(check_magic_quote_gpc($process));
	...
	if ($template == 1) 
    {
       ...
       if ($db_type == 0)
		...
		if ($txt_machine_no != "")
		{
			...
			where
				a.id=b.mst_id and b.id=c.dtls_id and b.id=d.dtls_id and c.po_id=e.id and c.is_sales=1 and d.machine_id in($machine_id_ref) and a.company_id=$company_name and b.knitting_party like '$party_type' and b.machine_dia like '$machine_dia' and b.id like '$program_no' and a.is_sales=1 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 $buyer_id_cond $po_buyer_cond $sales_no_cond $year_cond $booking_search_cond $date_cond $status_cond $knitting_source_cond 
			group by b.id, a.company_id, a.buyer_id, a.body_part_id, a.color_type_id, a.fabric_desc, a.gsm_weight, a.dia, a.width_dia_type, b.knitting_source, b.knitting_party, b.color_id,e.po_buyer,e.within_group,e.po_company_id, b.color_range, b.machine_dia, b.machine_gg, b.program_qnty, b.program_date, b.stitch_length, b.spandex_stitch_length, b.draft_ratio, b.machine_id, b.distribution_qnty, b.status, b.start_date, b.end_date, b.remarks, b.color_id, e.sales_booking_no, e.style_ref_no, e.job_no, e.booking_entry_form, e.booking_id order by b.knitting_source,b.machine_dia,b.machine_gg, b.id";
		}
		else
		{
			$sql = "SELECT a.company_id, a.buyer_id, a.body_part_id, a.color_type_id, a.fabric_desc, a.gsm_weight, a.dia, a.width_dia_type, b.id, b.knitting_source, b.knitting_party, b.color_id, b.color_range, b.machine_dia, b.machine_gg, b.program_qnty, b.program_date, b.stitch_length, b.spandex_stitch_length, b.draft_ratio, b.machine_id, b.distribution_qnty, b.status, b.start_date, b.end_date, b.remarks, b.color_id,e.po_buyer,e.within_group,e.po_company_id, e.sales_booking_no, e.style_ref_no, e.job_no, e.booking_entry_form, e.booking_id  
        ...
        if ($noOfbooking>0) 
        {
            ...
            $booking_sql="SELECT booking_no, company_id, po_break_down_id, item_category, fabric_source, job_no ,entry_form, is_approved
            from wo_booking_mst where entry_form in(86,88,118) $bookingCondition and booking_type=1 and is_short in(1,2) and status_active=1 and is_deleted=0";
...
if($action == "knitting_popup")
{
    ...
    <div style="width:1117px" align="center"><input type="button" value="Print Preview" onClick="print_window()"
        style="width:100px" class="formbutton"/></div>
        <fieldset style="width:1117px;">
            <div id="report_container">
                ...
                <div style="width:1118px; max-height:330px; overflow-y:scroll" id="scroll_body">
                    <table border="1" class="rpt_table" rules="all" width="1100" cellpadding="0" cellspacing="0"
                    id="tbl_list_search">
                    <?
                    $i = 1;
...
<br>
<?
//$sql_strip = "select pre_cost_fabric_cost_dtls_id, color_number_id, stripe_color, measurement, uom,sales_dtls_id,totfidder from wo_pre_stripe_color where $withInGrpCondition status_active=1 and is_deleted=0 $colorCondition and sales_dtls_id is not null  order by  pre_cost_fabric_cost_dtls_id, color_number_id, stripe_color asc";

 $sql_measurement = "SELECT b.pre_cost_id,b.color_id as color_number_id, b.stripe_color_id as stripe_color, b.dtls_id, b.no_of_feeder as no_of_feeder from  ppl_planning_feeder_dtls b where b.dtls_id in($program_ids) and b.status_active=1 and b.is_deleted=0 GROUP BY  b.pre_cost_id,b.color_id, b.stripe_color_id, b.dtls_id, b.no_of_feeder order by b.color_id, b.stripe_color_id asc";

$result_measurement = sql_select($sql_measurement);
$sl=1;
foreach($result_measurement as $row)
{
    $feedarDataArr[$row[csf('dtls_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$sl]['no_of_feeder']=$row[csf('no_of_feeder')];
    $sl++;
}

/*echo "<pre>";
print_r($feedarDataArr);
echo "</pre>";
*/
$sql_strip = "SELECT a.color_number_id, a.stripe_color, a.measurement, a.uom, b.dtls_id, b.no_of_feeder as no_of_feeder from wo_pre_stripe_color a, ppl_planning_feeder_dtls b where a.pre_cost_fabric_cost_dtls_id=b.pre_cost_id and a.color_number_id=b.color_id and a.stripe_color=b.stripe_color_id and b.dtls_id in($program_ids) and a.job_no in('".$sales_nos."') and b.no_of_feeder>0 and a.status_active=1 and a.is_deleted=0 GROUP BY  a.color_number_id, a.stripe_color, a.measurement, a.uom, b.dtls_id, b.no_of_feeder order by a.color_number_id, a.stripe_color asc";
    //order by b.dtls_id
$result_stripe = sql_select($sql_strip);
$stripeWiseColorArr = array();
$stripeDataArr = array();
foreach($result_stripe as $row)
{

    if($stripeDataArr_chk[$row[csf('dtls_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$row[csf('uom')]]['measurement']!=$row[csf('measurement')])
    {
        $stripeDataArr[$row[csf('dtls_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$row[csf('measurement')]][$row[csf('uom')]][$row[csf('no_of_feeder')]]['dtls_id']=$row[csf('dtls_id')];
        $stripeDataArr[$row[csf('dtls_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$row[csf('measurement')]][$row[csf('uom')]][$row[csf('no_of_feeder')]]['color_number_id']=$row[csf('color_number_id')];
        $stripeDataArr[$row[csf('dtls_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$row[csf('measurement')]][$row[csf('uom')]][$row[csf('no_of_feeder')]]['stripe_color']=$row[csf('stripe_color')];
        $stripeDataArr[$row[csf('dtls_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$row[csf('measurement')]][$row[csf('uom')]][$row[csf('no_of_feeder')]]['measurement']=$row[csf('measurement')];
        $stripeDataArr[$row[csf('dtls_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$row[csf('measurement')]][$row[csf('uom')]][$row[csf('no_of_feeder')]]['uom']=$row[csf('uom')];
        $stripeDataArr[$row[csf('dtls_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$row[csf('measurement')]][$row[csf('uom')]][$row[csf('no_of_feeder')]]['no_of_feeder']=$row[csf('no_of_feeder')];

        $stripeWiseColorArr[$row[csf('dtls_id')]]['color_number_id']=$row[csf('color_number_id')];

        $stripeDataArr_chk[$row[csf('dtls_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$row[csf('uom')]]['measurement']=$row[csf('measurement')];
    }
	
}

// echo "<pre>";
// print_r($stripeDataArr);
// echo "</pre>";
				...
				foreach($stripe_color_data as $m_id=>$m_data)
				{
					foreach($m_data as $u_id=>$u_data)
					{
						foreach($u_data as $row=>$val)
						{
                            ...
                            $noOfFeeders=$feedarDataArr[$prog_ids][$val['color_number_id']][$val['stripe_color']][$ii]['no_of_feeder'];
            
							//$tot_feeder += $val['no_of_feeder'];
                            $tot_feeder += $noOfFeeders;
							?>
							<tr valign="middle" bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>">
							<?
								  if(!in_array($prog_ids,$prog_chks))
								  {
									$prog_chks[]=$prog_ids;
									?>
									<td width="30" align="center" rowspan="<? echo $s_prog_span;?>" valign='middle' style="text-align:center"><? echo $i; ?></td>
									<td width="50" align="center" rowspan="<? echo $s_prog_span;?>" valign='middle' style="text-align:center"><? echo $prog_ids; ?></td>
									<td width="140" rowspan="<? echo $s_prog_span;?>" valign='middle' style="text-align:center"><p><? echo $color_library[$val['color_number_id']]; ?></p></td>
									<?

								  }
								  ?>
								<td width="130"><p><? echo $color_library[$val['stripe_color']]; ?></p></td>
								<td width="70" align="center"><? echo $val['measurement']; ?></td>
								<td width="50" align="center"><p><? echo $unit_of_measurement[$val['uom']]; ?></p></td>
								<td align="right" style="padding-right:10px"><? echo  $noOfFeeders;//$val['no_of_feeder']; ?>&nbsp;</td>
							</tr>
							<?
							$tot_masurement += $val['measurement'];
                            $ii++;
            ...
            <div style="width:655px;">
                ...
                <table width="100%" cellspacing="2" cellpadding="2" border="1" rules="all" class="rpt_table">
                    <tr>
                        ...
                        <td colspan="2" width="455" align="center" valign="middle" style="font-size:16px;border-left:hidden; border-top:hidden; border-right:hidden; border-bottom:hidden;"><b><? echo $company_arr[$program_data_arr[$progs]['company_id']]; ?></b></td>
                        <td><span  id="barcode_img_id_<? echo $inc_no;?>" ></span></td>
                    </tr>
                    <tr>
                        <td colspan="3" style="border-left:hidden; border-right:hidden;">&nbsp;</td>
                    ...
                    <tr>
                        <td class="font_size_define"><b>COUNT</b></td>
                        <td colspan="3" class="font_size_define"><? echo $program_data_arr[$progs]['yarn_dtls']; ?></td>
                    </tr>
                    <tr>
                        <td class="font_size_define"><b>LOT</b></td>
                ...
                <?
                $salesNo= $program_data_arr[$progs]['text_ref'];
                $pre_cost_ids=$withInGrpArr[$salesNo]['pre_cost_fabric_cost_dtls_id'];
                $colorIds=$withInGrpArr[$salesNo]['color_id'];

                if($withInGrpArr[$salesNo]['within_group']==2)
                {
                    $withInGrpCondition=" job_no in('".$salesNo."') and";
                }
                else
                {
                    $withInGrpCondition=" pre_cost_fabric_cost_dtls_id in($pre_cost_ids) and";
                }
                if($colorIds!="")
                {
                    $colorCondition = "and color_number_id in($colorIds)";
                    $colorCondition2 = "and b.color_id in($colorIds)";
                }



                $sql_strip = "SELECT a.pre_cost_fabric_cost_dtls_id,a.color_number_id, a.stripe_color, a.uom, b.dtls_id, b.no_of_feeder as no_of_feeder from wo_pre_stripe_color a, ppl_planning_feeder_dtls b where a.pre_cost_fabric_cost_dtls_id=b.pre_cost_id and a.color_number_id=b.color_id and a.stripe_color=b.stripe_color_id and b.dtls_id in($progs) and a.job_no in('".$salesNo."') and a.status_active=1 and a.is_deleted=0 GROUP BY a.pre_cost_fabric_cost_dtls_id,a.color_number_id, a.stripe_color, a.uom, b.dtls_id, b.no_of_feeder order by a.pre_cost_fabric_cost_dtls_id, a.color_number_id, a.stripe_color asc"; //and b.no_of_feeder>0
                $result_stripe = sql_select($sql_strip);
                if (count($result_stripe) > 0)
                {
                    foreach ($result_stripe as $row)
                    {
                        $precos_id=$row[csf('pre_cost_fabric_cost_dtls_id')].",";
                    }
                    $precos_id=chop($precos_id,",");
                }
                ...
                foreach ($result_stripe as $row)
                {
                    if($withInGrpArr[$salesNo]['within_group']==2)
                    {
                        $mesurmentArr[$row[csf('color_number_id')]][$row[csf('stripe_color')]][$sll]['measurement']=$row[csf('measurement')];
                    }
                    else
                    {
                        $mesurmentArr[$row[csf('pre_cost_fabric_cost_dtls_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$sll]['measurement']=$row[csf('measurement')];
                    }
                    $sll++;
                }

                /*echo "<pre>";
                print_r($mesurmentArr);
                echo "</pre>";*/
                ...
                $sql_measurement = "SELECT b.pre_cost_id,b.color_id, b.stripe_color_id, b.dtls_id, b.no_of_feeder as no_of_feeder from  ppl_planning_feeder_dtls b where b.dtls_id in($progs) $colorCondition2 and b.status_active=1 and b.is_deleted=0 GROUP BY  b.pre_cost_id,b.color_id, b.stripe_color_id, b.dtls_id, b.no_of_feeder order by b.pre_cost_id, b.color_id, b.stripe_color_id asc";

                $result_measurement = sql_select($sql_measurement);
                $slll=1;
                foreach ($result_measurement as $rows)
                {
                    if($withInGrpArr[$salesNo]['within_group']==2)
                    {
                        $messurmentNo=$mesurmentArr[$rows[csf('color_id')]][$rows[csf('stripe_color_id')]][$slll]['measurement'];

                        $programInfoArr[$rows[csf('color_id')]][$rows[csf('stripe_color_id')]]['program_no']=$rows[csf('dtls_id')];
                        $programInfoArr[$rows[csf('color_id')]][$rows[csf('stripe_color_id')]]['no_of_feeder']=$rows[csf('no_of_feeder')];
                    }
                    else
                    {
                        $messurmentNo=$mesurmentArr[$rows[csf('pre_cost_id')]][$rows[csf('color_id')]][$rows[csf('stripe_color_id')]][$slll]['measurement'];
                        $programInfoArr[$rows[csf('pre_cost_id')]][$rows[csf('color_id')]][$rows[csf('stripe_color_id')]][ $messurmentNo]['program_no']=$rows[csf('dtls_id')];
                        $programInfoArr[$rows[csf('pre_cost_id')]][$rows[csf('color_id')]][$rows[csf('stripe_color_id')]][ $messurmentNo]['no_of_feeder']=$rows[csf('no_of_feeder')]; 
                    }
                    $slll++;
                ...
                if (count($result_stripe) > 0)
                {
                    ...
                    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="100%" class="rpt_table">
                        ...
                        foreach ($result_stripe as $row)
                        {
                            ...
                            //$measurementInfo=$measurementArr[$row[csf('color_number_id')]][$row[csf('stripe_color')]][$row[csf('uom')]][$row[csf('dtls_id')]][$row[csf('no_of_feeder')]]['measurement']
                            ...
                            if($withInGrpArr[$salesNo]['within_group']==2)
                            {
                                $withInGrpPreCsot_or_salesDtlsId=$row[csf('sales_dtls_id')];
                                $progNoInfo=$programInfoArr[$row[csf('color_number_id')]][$row[csf('stripe_color')]]['program_no'];
                                $noOfFeederInfo=$programInfoArr[$row[csf('color_number_id')]][$row[csf('stripe_color')]]['no_of_feeder'];
                            }
                            else
                            {
                                $withInGrpPreCsot_or_salesDtlsId=$row[csf('pre_cost_fabric_cost_dtls_id')];
                                $progNoInfo=$programInfoArr[$withInGrpPreCsot_or_salesDtlsId][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$row[csf('measurement')]]['program_no'];
                                $noOfFeederInfo=$programInfoArr[$withInGrpPreCsot_or_salesDtlsId][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$row[csf('measurement')]]['no_of_feeder'];

                            }

                           

                             $tot_feeder += $noOfFeederInfo;


                            ?>
                            <tr valign="middle" bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>">
                                <td width="30" align="center"><? echo $i; ?></td>
                                <td width="100" align="center" ><? echo $progNoInfo; ?></td>
                                <td width="140" align="center"><p><? echo $color_library[$row[csf('color_number_id')]]; ?></p></td>
                                <td width="130" align="center"><p><? echo $color_library[$row[csf('stripe_color')]]; ?></p></td>
                                <td width="100" align="center"><? echo $row[csf('measurement')]; ?></td>
                                <td width="80" align="center"><p><? echo $unit_of_measurement[$row[csf('uom')]]; ?></p></td>
                                <td align="right" style="padding-right:10px"><? echo $noOfFeederInfo; ?>&nbsp;</td>
                            </tr>
                            <?
                            $tot_masurement += $row[csf('measurement')];
                            $i++;
