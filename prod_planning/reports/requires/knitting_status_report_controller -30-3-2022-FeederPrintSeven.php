	...
	if ($template == 1) 
	{
		...
		$party_type=array();
	 	if(!empty($nameArray))	
	 	{
			...
			if($r_id3 && $r_id4 && $r_id5 && $r_id6 && $r_id7 && $r_id8)
			{
			    oci_commit($con);
			}
		}
				...
				foreach ($nameArray as $row) 
				{
					...
					if (count($result_stripe) > 0) {
						...
						<table cellspacing="0" cellpadding="0" border="1" rules="all" width="600" class="rpt_table" >
							...
							$matchArr=array();
							foreach ($result_stripe as $row) {
								$measurementArr[$row[csf('pre_cost_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]]=$row[csf('measurement')];
								$uomArr[$row[csf('pre_cost_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]]=$row[csf('uom')];
							}
							//echo "============|This is Feedar part |=============";
							foreach ($noOfFeeder_arrayx as $pre_cost_id_key => $pre_cost_id_data) {
								foreach ($pre_cost_id_data as $color_number_id_key => $color_number_id_data) {
									foreach ($color_number_id_data as $stripe_color_key => $stripe_color_data) {
										foreach ($stripe_color_data as $sl_key => $row) {
											?>
											<tr valign="middle" bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>"> 
												<td width="30" align="center"><? echo $i; ?></td>
												<td width="50" align="center"><? echo $programIDS_arr[$pre_cost_id_key];//$row[csf('dtls_id')]; ?></td> 	
												<td width="140"><p><? echo $color_library[$color_number_id_key]; ?></p></td> 
												<td width="130"><p><? echo $color_library[$stripe_color_key]; ?></p></td>     
												<td width="70" align="center"><? echo $measurementArr[$pre_cost_id_key][$color_number_id_key][$stripe_color_key]; ?></td>               
												<td width="50" align="center"><p><? echo $unit_of_measurement[$uomArr[$pre_cost_id_key][$color_number_id_key][$stripe_color_key]]; ?></p></td>
												<td align="right" style="padding-right:10px"><? echo $row;//$row[csf('no_of_feeder')]; ?>&nbsp;</td>
											</tr>
											<?

											$matchArr[$pre_cost_id_key][$color_number_id_key][$stripe_color_key][$measurementArr[$pre_cost_id_key][$color_number_id_key][$stripe_color_key]][$uomArr[$pre_cost_id_key][$color_number_id_key][$stripe_color_key]]=1;
											$tot_masurement +=  $measurementArr[$pre_cost_id_key][$color_number_id_key][$stripe_color_key];
											$tot_feeder += $row;
							...
							//echo "============|This is Without Feedar part |=============";
							foreach ($result_stripe as $row) {
								$measurementArr[$row[csf('pre_cost_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]]=$row[csf('measurement')];
								$uomArr[$row[csf('pre_cost_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]]=$row[csf('uom')];
								//echo $row[csf('pre_cost_id')].'='.$row[csf('color_number_id')].'='.$row[csf('stripe_color')].'='.$kl.'<br/>';
								//$no_of_feeder=$noOfFeeder_arrayx[$row[csf('pre_cost_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$kl];
								...
								if($matchArr[$row[csf('pre_cost_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$measurementArr[$row[csf('pre_cost_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]]][$uomArr[$row[csf('pre_cost_id')]][$row[csf('color_number_id')]][$row[csf('stripe_color')]]]!=1)
