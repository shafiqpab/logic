	...
	if ($template == 1) 
	{
		...
		if(!empty($program_arr))
		{
			...
			foreach ($knitting_dataArray as $row) {
				...
				$receive_ids_arr[] = $row[csf('knit_id')];
			}
		}
		...
		if(!empty($receive_ids_arr))
		{
			...
			/*	$knitting_recv_qnty_array = return_library_array("select a.booking_id,a.recv_number, a.receive_date, a.receive_basis, d.booking_no, a.knitting_source, a.challan_no, a.knitting_company, b.prod_id, sum(b.grey_receive_qnty) as knitting_qnty 
						from inv_receive_master a, pro_grey_prod_entry_dtls b, inv_receive_master c, ppl_planning_info_entry_mst d, ppl_planning_info_entry_dtls e 
						where a.id = b.mst_id and a.entry_form =22 and a.receive_basis =9 and a.company_id = $company_name and  a.booking_id = c.id  and c.entry_form=2 and c.receive_basis = 2 and d.id = e.mst_id $program_cond4 and c.booking_id = e.id
						group by a.booking_id,a.recv_number, a.receive_date, a.receive_basis, d.booking_no, a.knitting_source, a.challan_no, a.knitting_company, b.prod_id", "booking_id", "knitting_qnty");*/

			/*$sql_for_recev =sql_select("select  a.booking_id, sum(c.qnty) as quantity 
				from inv_receive_master a, pro_grey_prod_entry_dtls b, pro_roll_details c, ppl_planning_info_entry_mst d, ppl_planning_info_entry_dtls e
				where a.id=b.mst_id and b.id=c.dtls_id  and a.entry_form = 58 and c.entry_form = 58 and a.status_active=1 and a.is_deleted=0 
				and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $program_cond4 and a.company_id = $company_name and cast(e.id as varchar2(4000)) = c.booking_no and d.id = e.mst_id
				group by a.booking_id
				union all
				select  a.booking_id,sum(b.grey_receive_qnty) as quantity 
				from inv_receive_master a, pro_grey_prod_entry_dtls b , ppl_planning_info_entry_mst c, ppl_planning_info_entry_dtls d 
				where a.id=b.mst_id  and b.program_no = d.id and c.id = d.mst_id and a.item_category=13 and a.entry_form=22 and a.receive_basis=11 and b.status_active=1 and b.is_deleted=0 
				$program_no_cond and a.company_id = $company_name
				group by a.booking_id
				union all
				select  a.booking_id, sum(b.grey_receive_qnty) as quantity 
				from inv_receive_master a, pro_grey_prod_entry_dtls b, inv_receive_master c, ppl_planning_info_entry_mst d, ppl_planning_info_entry_dtls e 
				where a.id = b.mst_id and a.entry_form =22 and a.receive_basis =9 and a.company_id = $company_name and  a.booking_id = c.id  and c.entry_form=2 and c.receive_basis = 2 and d.id = e.mst_id $program_cond4 and c.booking_id = e.id
				group by a.booking_id");
				foreach ($sql_for_recev as $row ) {
					$knitting_recv_qnty_array[$row[csf('booking_id')]]=$row[csf('quantity')];
				}
				*/
		...
		if(!empty($barcode_no_arr))
		{
			...
			//echo "select a.booking_no,b.current_delivery from pro_roll_details a,pro_grey_prod_delivery_dtls b where a.mst_id=b.grey_sys_id and a.barcode_no=b.barcode_num and a.entry_form=2 and a.receive_basis=2 and a.booking_without_order=0 $all_barcode_no_cond and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0  group by a.booking_no,a.barcode_no,b.current_delivery";
						...
						<div style="width:1820px; overflow-y:scroll; max-height:330px;" id="scroll_body">
							...
							<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1800px" class="rpt_table" id="tbl_list_search">	
		 					...
		 					$sql_booking_qry="select c.id,c.booking_no,c.buyer_id,b.job_no_mst as job_no,d.fabric_color_id,d.color_type,sum(d.grey_fab_qnty) as grey_fab_qnty,listagg(cast(b.po_number as varchar(4000)),',') within group (order by b.po_number) as po_number ,a.style_ref_no,c.po_break_down_id as multiple_booking_po from wo_po_details_master a,wo_po_break_down b,wo_booking_mst c,wo_booking_dtls d where  b.job_no_mst=a.job_no and c.booking_no=d.booking_no and d.po_break_down_id=b.id and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.company_id=$company_name $booking_search_cond_2 $buyer_id_cond_2 $year_cond $job_cond $file_no_cond $internal_cond group by c.id,c.booking_no,c.buyer_id,d.fabric_color_id,b.job_no_mst,d.color_type,a.style_ref_no,c.po_break_down_id";
								...
								$sql_color="select  a.construction, a.composition, c.booking_no,c.fabric_color_id ,d.count_id,d.copm_one_id,d.percent_one from wo_pre_cost_fabric_cost_dtls a,wo_booking_dtls c,wo_pre_cost_fab_yarn_cost_dtls d where a.job_no=c.job_no and d.fabric_cost_dtls_id=a.id  and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and a.id = c.pre_cost_fabric_cost_dtls_id and c.booking_type =1 and c.po_break_down_id in($po_ids) and c.booking_no in($booking_nos) and a.company_id=$company_name
								union all select  b.construction, b.composition, c.booking_no,c.fabric_color_id ,d.count_id,d.copm_one_id,d.percent_one from wo_pre_cost_fab_conv_cost_dtls a, wo_pre_cost_fabric_cost_dtls b, wo_booking_dtls c ,wo_pre_cost_fab_yarn_cost_dtls d where b.job_no=c.job_no and a.status_active=1 and b.status_active=1 and c.status_active=1  and a.id = c.pre_cost_fabric_cost_dtls_id  and d.fabric_cost_dtls_id=b.id and a.fabric_description = b.id and c.booking_type =4 and c.po_break_down_id in($po_ids) and c.booking_no in($booking_nos) and b.company_id=$company_name ";
								...
								$i=1;
									...
									<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>"> 
										...
										<td width="100"><? echo $color_type[$row[csf('color_type')]]; ?></td>
...
if ($action == "knitting_card_print_5") 
{
	...
	if($program_ids)
	{
		...
		foreach ($reqsQntyLotWise as $row) {
			...
			$reqNoArr[$row[csf('prod_id')]]['knit_id'] = $row[csf('knit_id')];
	...
	unset($sql_data);
		...
		foreach ($dataArray as $row)
		{
			...
			$yarn_desc=''; $lot_no=""; $brand_name="";$yarnReqQnty="";$reqnNo="";$countFeeding="";
			/*$ex_yarn_desc=array_unique(explode(",",$booking_item_array[$row[csf('booking_no')]]));
			foreach($ex_yarn_desc as $prodid)
			{
				if($yarn_desc=='') $yarn_desc=$product_details_array[$prodid]['desc']; else $yarn_desc.=','.$product_details_array[$prodid]['desc'];
