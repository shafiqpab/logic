...
$user_ids = $_SESSION['logic_erp']['user_id'];
...
if($action=="company_wise_report_button_setting")
{
	...
	echo "$('#requisition_print7').hide();\n";
	echo "$('#knitting_card9').hide();\n";
	echo "$('#requisition_print8').hide();\n";
	echo "$('#PriPrint16nt13').hide();\n";
	...
	if($print_report_format != "")
	{
		foreach($print_report_format_arr as $id)
		{
			...
			if($id==767){echo "$('#requisition_print7').show();\n";}
			if($id==227){echo "$('#requisition_print8').show();\n";}
			if($id==503){echo "$('#Print16').show();\n";}

			if($id==353){echo "$('#Print11').show();\n";}
			if($id==891){echo "$('#Print15').show();\n";}
			if($id==890){echo "$('#Print10').show();\n";}
			if($id==889){echo "$('#Print8').show();\n";}
			if($id==133){echo "$('#Print14').show();\n";}
		}
	}
	else
	{
		...
		echo "$('#requisition_print7').show();\n";
		echo "$('#knitting_card9').show();\n";
		echo "$('#requisition_print8').show();\n";
		echo "$('#Print16').show();\n";
		echo "$('#Print15').show();\n";
		echo "$('#Print14').show();\n";
	}
...
if ($action=="create_booking_search_list_view")
{
	...
	$sql = "select a.booking_no_prefix_num, a.booking_no, a.booking_date, a.company_id, a.buyer_id, b.job_no, a.po_break_down_id, a.item_category, a.fabric_source, a.supplier_id, a.is_approved, a.ready_to_approved,a.pay_mode from wo_booking_mst a,wo_booking_dtls b where a.booking_no=b.booking_no and $company $buyer $booking_no $booking_date $year_cond and a.booking_type in(1,4) and a.status_active=1 and a.is_deleted=0  group by a.booking_no_prefix_num, a.booking_no, a.booking_date, a.company_id, a.buyer_id, b.job_no, a.po_break_down_id, a.item_category, a.fabric_source, a.supplier_id, a.is_approved, a.ready_to_approved,a.pay_mode order by a.booking_no_prefix_num Desc";
	...
	<div id="report_container" align="left">
		<table class="rpt_table" border="1" rules="all" width="1000" cellpadding="0" cellspacing="0">
	       <thead>
	           	<tr>
	                <th width="30">SL</th>
	                <th width="80">Booking No</th>
		...
		<div style="width:1010px; overflow-y:scroll; max-height:305px" id="scroll_body" align="left" >
	        <table class="rpt_table" border="1" rules="all" width="1000" cellpadding="0" cellspacing="0" id="list_view">
	        	<tbody>
						<?
						$i=0;
						$sql_booking_info=sql_select($sql);
						foreach ($sql_booking_info as $rows) {
							$i++;
							if ($i % 2 == 0)
								$bgcolor = "#E9F3FF";
							else
							...
							<tr bgcolor="<? echo $bgcolor; ?>" onClick="js_set_value('<? echo $rows[csf('booking_no')]; ?>');" style="cursor:pointer">
								<td width="30" align="center"><? echo $i; ?></td>
								<td width="80" align="center"><? echo $rows[csf('booking_no')]; ?></td>
								<td width="80" align="center"><? echo change_date_format($rows[csf('booking_date')]); ?></td>
								<td width="70" align="center"><? echo $comp[$rows[csf('company_id')]]; ?></td>
								<td width="100" align="center"><? echo $buyer_arr[$rows[csf('buyer_id')]]; ?></td>
								<td width="90" align="center"><? echo $rows[csf('job_no')]; ?></td>
								<td width="200" align="center"><? echo $item_category[$rows[csf('item_category')]]; ?></td>
								<td width="80" align="center"><? echo $fabric_source[$rows[csf('fabric_source')]]; ?></td>
								<td width="80" align="center"><? if($rows[csf('pay_mode')]==1 || $rows[csf('pay_mode')]==2){ echo $suplier[$rows[csf('supplier_id')]];}else{ echo $comp[$rows[csf('supplier_id')]];} ?></td>
								<td width="50" align="center"><? echo $approved[$rows[csf('is_approved')]]; ?></td>
								<td align="center"><? echo $is_ready[$rows[csf('ready_to_approved')]]; ?></td>
							</tr>
							<?
...
if ($action == "report_generate")
{
	...
	$txt_day_upto_no = str_replace("'","", $txt_day_upto_no);
	...
	if ($template == 1)
	{
		...
		$auto_fabric_store_variable = return_field_value("auto_update", "variable_settings_production", "company_name=$company_name and variable_list=15 and item_category_id=13 and status_active=1 and is_deleted=0", "auto_update");

		if(($po_search_cond != "" || $job_cond!="" || $internal_cond!="" || $file_no_cond!="") || str_replace("'", "", trim($txt_booking_no)) != "")
		{
			$po_array= array();
			$po_id = "";
			$x = 0;
			...
			if(count($costing_sql)==0)
			{
				echo "<br><div align='center'><b  style='color:red;font-size:22px'> NO Data Found</b></div>";	die;
			}
		}
		...
		$con = connect();
		execute_query("delete from tmp_barcode_no where userid=$user_ids");
		execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_ids and entry_form=38");
		oci_commit($con);

		if ($presentationType == 1 || $presentationType == 6 || $presentationType == 7) // Show
		{
			if ($txt_machine_no != "")
			{
				if ($db_type == 0) {
				...
				where a.id=b.mst_id and b.id=c.dtls_id and b.id=d.dtls_id and b.mst_id=c.mst_id and a.id=c.mst_id and d.machine_id in($machine_id_ref)  and a.company_id=$company_name  and b.knitting_party like '$party_type' and b.machine_dia like '$machine_dia' and b.id like '$program_no' and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and c.is_sales =0 $buyer_id_cond $po_id_cond $date_cond $booking_search_cond $status_cond $knitting_source_cond order by b.knitting_source, b.machine_dia, b.machine_gg,b.id";
			}
			else
			{
				$sql = "SELECT a.company_id, a.buyer_id, a.body_part_id,  a.fabric_desc, a.gsm_weight, a.dia, a.width_dia_type, b.id, b.knitting_source, b.knitting_party, b.color_id, b.color_range, b.machine_dia, b.machine_gg,b.program_qnty as program_qnty, b.program_date, b.stitch_length, b.spandex_stitch_length, b.draft_ratio, b.machine_id, b.distribution_qnty, b.status, b.start_date, b.end_date, b.remarks, b.color_id, b.location_id, c.po_id
				from ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b, ppl_planning_entry_plan_dtls c
				where a.id=b.mst_id and b.id=c.dtls_id  and b.mst_id=c.mst_id and a.id=c.mst_id and a.company_id=$company_name   and b.knitting_party like '$party_type' and b.machine_dia like '$machine_dia' and b.id like '$program_no' and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and c.is_sales =0 $buyer_id_cond $po_id_cond $date_cond $booking_search_cond $status_cond $knitting_source_cond group by a.company_id, a.buyer_id, a.body_part_id,  a.fabric_desc, a.gsm_weight, a.dia, a.width_dia_type, b.id, b.knitting_source, b.knitting_party, b.color_id, b.color_range, b.machine_dia, b.machine_gg,b.program_qnty, b.program_date, b.stitch_length, b.spandex_stitch_length, b.draft_ratio, b.machine_id, b.distribution_qnty, b.status, b.start_date, b.end_date, b.remarks, b.color_id, b.location_id, c.po_id order by b.knitting_source, b.machine_dia, b.machine_gg,b.id";
			}
			// echo $sql;
			$nameArray = sql_select($sql);	$checkDupliateRow=array();
			foreach ($nameArray as $key => $row)
			{
				$machine_dia_gg = $row[csf('machine_dia')].'X'.$row[csf('machine_gg')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['company_id']=$row[csf('company_id')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['buyer_id']=$row[csf('buyer_id')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['location_id']=$row[csf('location_id')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['body_part_id']=$row[csf('body_part_id')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['color_type_id']=$row[csf('color_type_id')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['fabric_desc']=$row[csf('fabric_desc')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['gsm_weight']=$row[csf('gsm_weight')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['dia']=$row[csf('dia')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['width_dia_type']=$row[csf('width_dia_type')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['knitting_party']=$row[csf('knitting_party')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['color_id']=$row[csf('color_id')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['color_range']=$row[csf('color_range')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['program_date']=$row[csf('program_date')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['stitch_length']=$row[csf('stitch_length')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['spandex_stitch_length']=$row[csf('spandex_stitch_length')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['draft_ratio']=$row[csf('draft_ratio')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['machine_id']=$row[csf('machine_id')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['distribution_qnty']=$row[csf('distribution_qnty')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['status']=$row[csf('status')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['start_date']=$row[csf('start_date')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['end_date']=$row[csf('end_date')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['remarks']=$row[csf('remarks')];
				$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['po_id'].=$row[csf('po_id')].',';
				if($checkDupliateRow[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]][$row[csf('program_qnty')]]!=$row[csf('program_qnty')])
				{
					$checkDupliateRow[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]][$row[csf('program_qnty')]]=$row[csf('program_qnty')];
					$data_arr[$row[csf('knitting_source')]][$machine_dia_gg][$row[csf('id')]]['program_qnty']+=$row[csf('program_qnty')];
				}
		...
		else if ($presentationType == 2) // Job/Order Status
		{
			$sql = "SELECT a.company_id, a.buyer_id, a.body_part_id, a.fabric_desc, a.gsm_weight, a.dia, a.width_dia_type, b.id, b.knitting_source, b.knitting_party, b.color_id, b.color_range, b.machine_dia, b.machine_gg, b.program_qnty, b.program_date, b.stitch_length, b.spandex_stitch_length, b.draft_ratio, b.machine_id, b.distribution_qnty, b.status, b.start_date, b.end_date, b.remarks, c.po_id
			from ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b, ppl_planning_entry_plan_dtls c
		...
		else if ($presentationType == 3) // Summary
		{
			if ($txt_machine_no != "") {
				if ($db_type == 0) {
					$machine_id_ref = return_field_value("group_concat(id) as machine_id", "lib_machine_name", "machine_no='$txt_machine_no'", "machine_id");
				...
				$sql = "SELECT a.company_id, a.buyer_id, a.body_part_id,  a.fabric_desc, a.gsm_weight, a.dia, a.width_dia_type, b.id, b.knitting_source, b.knitting_party, b.color_id, b.color_range, b.machine_dia, b.machine_gg, b.program_qnty as program_qnty, b.program_date, b.stitch_length, b.spandex_stitch_length, b.draft_ratio, b.machine_id, b.distribution_qnty, b.status, b.start_date, b.end_date, b.remarks, b.color_id,c.po_id
				from ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b, ppl_planning_entry_plan_dtls c, ppl_entry_machine_datewise d
				where a.id=b.mst_id and b.id=c.dtls_id and b.id=d.dtls_id and d.machine_id in($machine_id_ref) and a.company_id=$company_name and b.knitting_party like '$party_type' and b.machine_dia like '$machine_dia' and b.id like '$program_no' and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and c.is_sales =0 $buyer_id_cond $po_id_cond $date_cond $booking_search_cond $status_cond $knitting_source_cond order by b.machine_dia,b.machine_gg, b.id";
			}
			else
			{
				$sql = "SELECT a.company_id, a.buyer_id, a.body_part_id,  a.fabric_desc, a.gsm_weight, a.dia, a.width_dia_type, b.id, b.knitting_source, b.knitting_party, b.color_id, b.color_range, b.machine_dia, b.machine_gg, b.program_qnty as program_qnty, b.program_date, b.stitch_length, b.spandex_stitch_length, b.draft_ratio, b.machine_id, b.distribution_qnty, b.status, b.start_date, b.end_date, b.remarks, b.color_id,c.po_id
			...
			{
				$machine_dia_gg = $row[csf('machine_dia')].'X'.$row[csf('machine_gg')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['company_id']=$row[csf('company_id')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['buyer_id']=$row[csf('buyer_id')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['location_id']=$row[csf('location_id')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['body_part_id']=$row[csf('body_part_id')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['color_type_id']=$row[csf('color_type_id')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['fabric_desc']=$row[csf('fabric_desc')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['gsm_weight']=$row[csf('gsm_weight')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['dia']=$row[csf('dia')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['width_dia_type']=$row[csf('width_dia_type')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['knitting_party']=$row[csf('knitting_party')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['color_id']=$row[csf('color_id')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['color_range']=$row[csf('color_range')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['program_date']=$row[csf('program_date')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['stitch_length']=$row[csf('stitch_length')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['spandex_stitch_length']=$row[csf('spandex_stitch_length')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['draft_ratio']=$row[csf('draft_ratio')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['machine_id']=$row[csf('machine_id')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['distribution_qnty']=$row[csf('distribution_qnty')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['status']=$row[csf('status')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['start_date']=$row[csf('start_date')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['end_date']=$row[csf('end_date')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['remarks']=$row[csf('remarks')];
				$data_arr[$machine_dia_gg][$row[csf('id')]]['po_id'].=$row[csf('po_id')].',';

				if($checkDupliateRow[$machine_dia_gg][$row[csf('id')]][$row[csf('program_qnty')]]!=$row[csf('program_qnty')])
				{
					$checkDupliateRow[$machine_dia_gg][$row[csf('id')]][$row[csf('program_qnty')]]=$row[csf('program_qnty')];
					$data_arr[$machine_dia_gg][$row[csf('id')]]['program_qnty']+=$row[csf('program_qnty')];
				}
		...
		/*$r_id3=execute_query("delete from tmp_prog_no where userid=$user_ids");
		$r_id4=execute_query("delete from tmp_poid where userid=$user_ids");
		$r_id5=execute_query("delete from tmp_reqs_no where userid=$user_ids");
		$r_id6=execute_query("delete from tmp_recv_dtls where userid=$user_ids");
		$r_id7=execute_query("delete from tmp_barcode_no where userid=$user_ids");
		$r_id8=execute_query("delete from tmp_prod_id where userid=$user_ids");*/

		if ($presentationType != 2 && $presentationType != 1 && $presentationType != 6 && $presentationType != 7 && $presentationType != 3)
		{
			foreach ($nameArray as $row)
			{
				$program_arr[$row[csf('id')]] = $row[csf('id')];
				$all_po_id_arr=explode(",",$row[csf('po_id')]);
				foreach($all_po_id_arr as $p_id)
				{
					if(!$po_id_arr[$p_id])
					{
						$po_id_arr[$p_id] = $p_id;
		...
		//if ($presentationType == 2) // Job/Order Status
		//{
			$con = connect();
			if (!empty($po_id_arr) && $user_ids>0)
			{
				fnc_tempengine("GBL_TEMP_ENGINE", $user_ids, 38, 1,$po_id_arr, $empty_arr);
			}
			if (!empty($program_arr) && $user_ids>0)
			{
				fnc_tempengine("GBL_TEMP_ENGINE", $user_ids, 38, 2,$program_arr, $empty_arr);
			}
			oci_commit($con);
			...
			$barcode_sql=sql_select("SELECT a.barcode_no from pro_roll_details a, GBL_TEMP_ENGINE b where a.entry_form =58 and a.po_breakdown_id=b.ref_val and b.user_id=$user_ids and b.entry_form=38 and b.ref_from=1 and a.status_active=1");
			...
			if(!empty($po_id_arr))
			{
				$costing_sql = sql_select("SELECT a.job_no,$year_field, a.style_ref_no, b.id, b.po_number,b.pub_shipment_date, b.grouping, b.file_no from wo_po_details_master a, wo_po_break_down b, GBL_TEMP_ENGINE c where a.job_no=b.job_no_mst and b.id=c.ref_val and c.user_id=$user_ids and c.entry_form=38 and c.ref_from=1 and a.company_name=$company_name and a.status_active=1 and a.is_deleted=0 and b.status_active in (1,3) and b.is_deleted=0 $job_cond $internal_cond $file_no_cond");//$all_po_cond
			...
			{
				//$plan_details_array = return_library_array("SELECT a.dtls_id, LISTAGG(a.po_id, ',') WITHIN GROUP (ORDER BY a.po_id) as po_id from ppl_planning_entry_plan_dtls a, GBL_TEMP_ENGINE b where a.dtls_id=b.ref_val and b.user_id=$user_ids and b.entry_form=38 and b.ref_from=2 and a.company_id=$company_name  group by a.dtls_id", "dtls_id", "po_id");// no need
				$plan_details_sql="SELECT a.dtls_id, a.po_id from ppl_planning_entry_plan_dtls a, GBL_TEMP_ENGINE b
				where a.dtls_id=b.ref_val and b.user_id=$user_ids and b.entry_form=38 and b.ref_from=2 and a.company_id=$company_name";
				// echo $plan_details_sql;die;
				$plan_details_sql_data=sql_select($plan_details_sql);
				foreach ($plan_details_sql_data as $key => $row)
				{
					$plan_details_array[$row[csf("dtls_id")]].=$row[csf("po_id")].',';
				}
				// echo "<pre>";print_r($plan_details_array);die;
			}

			if(!empty($program_arr)) // requisition_no insert into TMP_ENGINE
			{
				$reqsDataArr = array();
				//$reqsData = sql_select("SELECT a.knit_id, max(a.requisition_no) as reqs_no, LISTAGG(a.prod_id, ',') WITHIN GROUP (ORDER BY a.prod_id) as prod_id , sum(a.yarn_qnty) as yarn_req_qnty from ppl_yarn_requisition_entry a, GBL_TEMP_ENGINE b where a.knit_id=b.ref_val and b.user_id=$user_ids and b.entry_form=38 and b.ref_from=2 and a.status_active=1 and a.is_deleted=0 group by a.knit_id,a.requisition_no");
				$reqsData = sql_select("SELECT a.knit_id, a.requisition_no as reqs_no, a.prod_id, a.yarn_qnty as yarn_req_qnty from ppl_yarn_requisition_entry a, GBL_TEMP_ENGINE b where a.knit_id=b.ref_val and b.user_id=$user_ids and b.entry_form=38 and b.ref_from=2 and a.status_active=1 and a.is_deleted=0");
				...
				foreach ($reqsData as $row)
				{
					...
					//{
				    	...
				    	//$rID3=execute_query("insert into tmp_reqs_no (userid, reqs_no) values ($user_ids,".$row[csf('reqs_no')].")");
					//}
				}
				// echo "<pre>";print_r($prod_arr);die;
				if (!empty($requisition_no_arr) && $user_ids>0)
				{
					fnc_tempengine("GBL_TEMP_ENGINE", $user_ids, 38, 3,$requisition_no_arr, $empty_arr);
				}
				if (!empty($prod_arr) && $user_ids>0)
				{
					fnc_tempengine("GBL_TEMP_ENGINE", $user_ids, 38, 5,$prod_arr, $empty_arr);
				}
				oci_commit($con);
			}
			...
			{
				$yarn_iss_arr = array();
				$yarnIssueData = sql_select("SELECT a.requisition_no, a.prod_id, a.cons_quantity as qnty from inv_transaction a, GBL_TEMP_ENGINE b where a.item_category=1 and a.transaction_type=2 and a.receive_basis=3 and a.status_active=1 and a.is_deleted=0 and a.requisition_no=b.ref_val and b.user_id=$user_ids and b.entry_form=38 and b.ref_from=3");
			...
			if(!empty($requisition_no_arr))
			{
				...
				$yarnIssueRtnData = sql_select("SELECT a.booking_id as reqsn_no, b.prod_id, b.cons_quantity as qnty, b.cons_reject_qnty as reject_qnty from inv_receive_master a, inv_transaction b, GBL_TEMP_ENGINE c where a.id=b.mst_id and a.receive_basis=3 and a.entry_form=9 and b.item_category=1 and b.transaction_type=4 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.booking_id=c.ref_val and c.user_id=$user_ids and c.entry_form=38 and c.ref_from=3");
				$yarn_IssRtn_arr = array();
			...
			if(!empty($program_arr))
			{
				...
				$knitting_dataArray = sql_select("SELECT a.booking_id,a.id as knit_id, b.gsm as gsm, b.grey_receive_qnty as knitting_qnty,reject_fabric_receive as fabric_reject_qty,b.trans_id,b.no_of_roll  from GBL_TEMP_ENGINE c, inv_receive_master a, pro_grey_prod_entry_dtls b  where c.ref_val=a.booking_id and c.user_id=$user_ids and c.entry_form=38 and c.ref_from=2 and a.id=b.mst_id and a.item_category=13 and a.entry_form=2 and a.receive_basis=2 and b.status_active=1 and b.is_deleted=0");
				foreach ($knitting_dataArray as $row)
				{
					...
					/*if(!$receive_ids_arr[$row[csf('knit_id')]])
					{
				    	...
				    	$rID4=execute_query("insert into tmp_recv_dtls (userid, dtls_id) values ($user_ids,".$row[csf('knit_id')].")");
					}*/
					$receive_ids_arr[$row[csf('knit_id')]] = $row[csf('knit_id')];
				}
				if (!empty($receive_ids_arr) && $user_ids>0)
				{
					fnc_tempengine("GBL_TEMP_ENGINE", $user_ids, 38, 4,$receive_ids_arr, $empty_arr);
				}
				oci_commit($con);
			}
			...
			if(!empty($requisition_no_arr))
			{
				//$cons_qty_arr = return_library_array("SELECT a.booking_id,sum( b.cons_quantity) as cons_quantity  from  inv_receive_master a, inv_transaction b,  product_details_master c, GBL_TEMP_ENGINE d where b.prod_id=c.id and a.id=b.mst_id and b.item_category=1 and b.transaction_type=4 and a.booking_id=d.ref_val and d.user_id=$user_ids and d.entry_form=38 and d.ref_from=3 group by a.booking_id", 'booking_id', 'cons_quantity');
				//$cons_qty_arr[$reqsDataArr[$row[csf('id')]]['reqs_no']]

				$iss_rtn_sql="SELECT a.booking_id, b.cons_quantity from inv_receive_master a, inv_transaction b,  product_details_master c, GBL_TEMP_ENGINE d
				where b.prod_id=c.id and a.id=b.mst_id and b.item_category=1 and b.transaction_type=4 and a.booking_id=d.ref_val and d.user_id=$user_ids and d.entry_form=38 and d.ref_from=3";
				$iss_rtn_sql_data=sql_select($iss_rtn_sql);
				foreach ($iss_rtn_sql_data as $key => $row)
				{
					$cons_qty_arr[$row[csf("booking_id")]]+=$row[csf("cons_quantity")];
				}
			}
			 //echo "<pre>";print_r($receive_ids_arr);die;
			if(!empty($receive_ids_arr))
			{
				$gds_qnty_sql="SELECT b.program_no, b.current_delivery as delivery_qnty
				from pro_grey_prod_delivery_mst a, pro_grey_prod_delivery_dtls b, GBL_TEMP_ENGINE c
				where a.id=b.mst_id and a.entry_form=53 and b.entry_form=53 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.grey_sys_id=c.ref_val and c.user_id=$user_ids and c.entry_form=38 and c.ref_from=4";
				//echo $gds_qnty_sql;die;
				$gds_qnty_sql_data=sql_select($gds_qnty_sql);
				$gds_qnty_array = array();
				$deliveryStorQtyArr = array();
				foreach ($gds_qnty_sql_data as $key => $row)
				{
					$gds_qnty_array[$row[csf("program_no")]]+=$row[csf("delivery_qnty")];
					$deliveryStorQtyArr[$row[csf('program_no')]]+=$row[csf("delivery_qnty")];
				}
				unset($gds_qnty_sql_data);


				$knitting_recv_qnty_sql="SELECT a.booking_id, b.grey_receive_qnty as knitting_qnty
				from inv_receive_master a, pro_grey_prod_entry_dtls b, GBL_TEMP_ENGINE c
				where a.id=b.mst_id and a.item_category=13 and a.entry_form=22 and a.receive_basis=9 and b.status_active=1 and b.is_deleted=0 and a.booking_id=c.ref_val and c.user_id=$user_ids and c.entry_form=38 and c.ref_from=4";
				// echo $knitting_recv_qnty_sql;die;
				$knitting_recv_qnty_sql_data=sql_select($knitting_recv_qnty_sql);
				foreach ($knitting_recv_qnty_sql_data as $key => $row)
				{
			...
			if(!empty($program_arr))
			{
				/*$knit_recvProg_qty_arr = return_library_array("SELECT b.program_no, sum(b.grey_receive_qnty) as knitting_qnty from inv_receive_master a, pro_grey_prod_entry_dtls b, GBL_TEMP_ENGINE c
				where c.ref_val=b.program_no and c.user_id=$user_ids and c.entry_form=38 and c.ref_from=2 and a.id=b.mst_id and a.item_category=13 and a.entry_form=22 and a.receive_basis=11 and b.status_active=1 and b.is_deleted=0 group by b.program_no", "program_no", "knitting_qnty");*/

				$knit_recvProg_qty_sql = "SELECT b.program_no, b.grey_receive_qnty as knitting_qnty from inv_receive_master a, pro_grey_prod_entry_dtls b, GBL_TEMP_ENGINE c
				where c.ref_val=b.program_no and c.user_id=$user_ids and c.entry_form=38 and c.ref_from=2 and a.id=b.mst_id and a.item_category=13 and a.entry_form=22 and a.receive_basis=11 and b.status_active=1 and b.is_deleted=0";
				// echo $knit_recvProg_qty_sql;die;
				$knit_recvProg_qty_sql_data=sql_select($knit_recvProg_qty_sql);
				foreach ($knit_recvProg_qty_sql_data as $key => $row)
				{
					$knit_recvProg_qty_arr[$row[csf("program_no")]]+=$row[csf("knitting_qnty")];
				}
			}

			$receive_ids_arr=array_filter($receive_ids_arr);
			//echo "<pre>";print_r($receive_ids_arr);
			if(!empty($receive_ids_arr))
			{
				$barcode_arr = array();

				$barcodeData = sql_select("SELECT mst_id, barcode_no from inv_receive_master a, pro_roll_details b, GBL_TEMP_ENGINE c where
					a.id=c.ref_val and c.user_id=$user_ids and c.entry_form=38 and c.ref_from=4 and
				...
				foreach ($barcodeData as $row)
				{
					...
					if(!$barcode_no_arr[$row[csf('barcode_no')]])
					{
				    	...
				    	$rID5=execute_query("insert into tmp_barcode_no (userid, barcode_no) values ($user_ids,".$row[csf('barcode_no')].")");
					}
			...
			if(!empty($barcode_no_arr))
			{
				...
				// if
				$recv_delivery_sql = sql_select("SELECT a.barcode_no, a.qnty from pro_roll_details a, tmp_barcode_no b where a.entry_form=58 and a.status_active=1 and a.is_deleted=0 and a.barcode_no=b.barcode_no and b.userid=$user_ids");

				//echo "select a.barcode_no, a.qnty from pro_roll_details a, tmp_barcode_no b where a.entry_form=58 and a.status_active=1 and a.is_deleted=0 and a.barcode_no=b.barcode_no and b.userid=$user_ids";
				/*$r_id3=execute_query("delete from tmp_prog_no where userid=$user_ids");
				$r_id4=execute_query("delete from tmp_poid where userid=$user_ids");
				$r_id5=execute_query("delete from tmp_reqs_no where userid=$user_ids");
				$r_id6=execute_query("delete from tmp_recv_dtls where userid=$user_ids");oci_commit($con);die;*/
			...
			if(!empty($prod_arr))
			{
				...
				//echo "SELECT b.id, b.product_name_details, b.lot, b.supplier_id from GBL_TEMP_ENGINE a, product_details_master b where a.ref_val=b.id and a.user_id=$user_ids and a.entry_form=38 and a.ref_from=5 and b.company_id=$company_name and b.item_category_id=1 $procuct_cond";die;
				$pro_sql = sql_select("SELECT b.id, b.product_name_details, b.lot, b.supplier_id from GBL_TEMP_ENGINE a, product_details_master b where a.ref_val=b.id and a.user_id=$user_ids and a.entry_form=38 and a.ref_from=5 and b.company_id=$company_name and b.item_category_id=1 $procuct_cond");
			...
			// only for $presentationType == 1 || $presentationType == 5 || $presentationType == 6
			$sqlBookingInfo=sql_select("SELECT a.booking_no,a.job_no,a.PO_BREAK_DOWN_ID,a.GSM_WEIGHT,a.FABRIC_COLOR_ID,a.color_type from wo_booking_mst c,WO_BOOKING_DTLS
			a, GBL_TEMP_ENGINE b where a.booking_no=c.booking_no and a.PO_BREAK_DOWN_ID=b.ref_val and b.user_id=$user_ids and b.entry_form=38 and b.ref_from=1 and c.company_id=$company_name group by a.booking_no,a.job_no,a.PO_BREAK_DOWN_ID,a.GSM_WEIGHT,a.FABRIC_COLOR_ID,a.color_type");
			foreach ($sqlBookingInfo as $row)
			{
				$bookingInfoArr[$row[csf('job_no')]][$row[csf('PO_BREAK_DOWN_ID')]][$row[csf('GSM_WEIGHT')]][$row[csf('FABRIC_COLOR_ID')]]['color_type']=$row[csf('color_type')];
			}
		//}

		$colspan = 31;
		if($presentationType == 6){
			$tbl_width = 4070+300+400+100;
			//$colspan = 32;
		}else{
		...
		$r_id8=execute_query("delete from tmp_prod_id where userid=$user_ids");*/
		execute_query("delete from tmp_barcode_no where userid=$user_ids");
		execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_ids and entry_form in (38)");
		oci_commit($con);

		// Data show ========
		if ($presentationType == 1 || $presentationType == 5 || $presentationType == 6 )
		{
			ob_start();
			...
			<fieldset style="width:<? echo $tbl_width; ?>px;">
				...
				<table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width - 40 +200; ?>" class="rpt_table" >
					<thead>
						<th width="40"></th>
						<th width="40">SL</th>
						...
						<th width="100">
							<? if($presentationType == 6){
								echo "Knit Balance (Program Wise)";
							}else{ echo "Knit Balance Qnty";}?>
						</th>
						<th width="100">Delivery Store</th>
						<th width="100">No. Of Roll</th>
						<th width="100">Received Qnty</th>
						<? if($presentationType == 6){ ?>
						<th width="100">Knit Balance (Issue Wise)</th>
						<? }?>
						<th width="100">No. Of Roll</th>
						<th width="100">Recv. Bal. Qnty</th>
						<th width="100">No. Of Roll</th>
						<th width="80">Knitting Status</th>
						<th width="150">Program Age</th>
						<th>Remarks</th>
					</thead>
				</table>
				<div style="width:<? echo $tbl_width - 20 +200; ?>px; overflow-y:scroll; max-height:330px;" id="scroll_body">
					<table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width - 40 +200; ?>" class="rpt_table" id="tbl_list_search">
						<tbody>
							<?
							$i = 1;
							$k = 1;
							...
							$tot_knit_ballance_iw_qnty = 0;
							$machine_dia_gg_array = array();


							$party_array=array();
							foreach ($data_arr as $knitt_source => $knitt_sourcev)
							{
								?>
								<tr bgcolor="#C6D9C9">
									<td style="font-weight: 900" colspan="<? if($presentationType == 6){ echo$colspan + 21+1;}else{ echo$colspan + 21; } ?>"><b><?php echo $search_by_arr[$knitt_source]; ?></b></td>
								</tr>
								<?
								foreach ($knitt_sourcev as $machine_dia_gg => $machine_dia_ggv)
								{
									?>
									<tr bgcolor="#EFEFEF">
										<td colspan="<? if($presentationType == 6){ echo$colspan + 21+1;}else{ echo$colspan + 21; } ?>"><b>Machine Dia:- <?php echo $machine_dia_gg; ?></b></td>
									</tr>
									<?
									...
									$sub_knit_ballance_iw_qnty = 0;
									$sub_tot_knitting_recv_qnty = 0;
									foreach ($machine_dia_ggv as $program => $row)
									{
										$date1=date("d-m-Y");
										$date2=change_date_format($row['program_date']);
										$diff = abs(strtotime($date1) - strtotime($date2));
										$years = floor($diff / (365*60*60*24));
										$months = floor(($diff - $years * 365*60*60*24) / (30*60*60*24));
										//$days = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24));
										//printf("%d Years, %d Months, %d Days\n", $years, $months, $days);
										$program_age = floor(($diff)/ (60*60*24));
										...
										if($display==true)
										{
											...
											if($knitt_source==1)
											{
												...
												$knitting_party=$company_library[$row['knitting_party']];
											}
											else
											{
												$knitting_cond="Outside";
												$knitting_party=$supplier_details[$row['knitting_party']];
											}
											...
											$prod_id = array_unique(explode(",", $reqsDataArr[$program]['prod_id']));
											$yarn_desc = '';
											$lot = '';
											$supplier = '';
											$yarn_issue_qnty = 0;
											...
											foreach ($prod_id as $val)
											{
												...
												$yarn_issue_return_qnty += $yarn_IssRtn_arr[$reqsDataArr[$program]['reqs_no']][$val];

												$yarn_issue_reject_qnty += $yarn_IssRej_arr[$reqsDataArr[$program]['reqs_no']][$val];

												$yarn_issue_qnty += $yarn_iss_arr[$reqsDataArr[$program]['reqs_no']][$val]; //- $yarn_IssRtn_arr[$reqsDataArr[$program]['reqs_no']][$val];
											}
											$yarn_desc = explode(",", substr($yarn_desc, 0, -1));
											$lot = explode(",", substr($lot, 0, -1));
											$supplier = explode(",", substr($supplier, 0, -1));

											$po_id = array_unique(explode(",", $plan_details_array[$program]));
											$po_no = '';
											$style_ref = '';
											$job_no = '';
											$file_nos = '';
											...
											$knitting_qnty = $knitDataArr[$program]['qnty'] + $knit_recvProg_qty_arr[$program];

											$knit_id = $knitDataArr[$program]['knit_id'];
											$fabric_reject_qnty = $knitDataArr[$program]['reject_qnty'];
											...
											if($autoProductionRecv[$program]['qnty'] !="")
											{
												$knitting_recv_qnty = $autoProductionRecv[$program]['qnty'] ;
											}
											else
											{
												$knitting_recv_qnty = $knit_recv_qty + $knit_recvProg_qty_arr[$program];
											}
											//+ $autoProductionRecv[$program]['qnty']

											$no_of_roll_knit = $knitDataArr[$program]['no_of_roll'];
											$no_of_roll_delv = $deliveryStorNoOfRollArr[$program];
											$no_of_roll_recv=$autoProductionRecv[$program]['no_of_roll']+$no_of_rollRec;
											$no_of_roll_balance=$no_of_roll_knit-$no_of_roll_recv;


											$balance_qnty = $row['program_qnty'] - $knitting_qnty;
											$balance_recv_qnty = $knitting_qnty - $knitting_recv_qnty;
											$yarn_issue_bl_qnty = $reqsDataArr[$program]['yarn_req_qnty'] - $yarn_issue_qnty;

											$complete = '&nbsp;';
											if ($knitting_qnty >= $row['program_qnty'])
												$complete = 'Complete';
											$po_id_booking = array_unique(explode(",", $plan_details_array[$program]));
											$colorType=$bookingInfoArr[$job_no][$po_id_booking[0]][$row['gsm_weight']][$row['COLOR_ID']]['color_type'];
											?>
											<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
												<td width="40" align="center" valign="middle">
													<input type="checkbox" id="tbl_<? echo $i; ?>" name="check[]" onClick="selected_row(<? echo $i. "," .$presentationType; ?>);" />
													<input id="promram_id_<? echo $i; ?>" name="promram_id[]" type="hidden" value="<? echo $program; ?>" />
													<input id="job_no_<? echo $i; ?>" name="job_no[]" type="hidden" value="<? echo $job_no; ?>" />
													<input id="source_id_<? echo $i; ?>" name="source_id_[]" type="hidden" value="<? echo $knitt_source; ?>" />
													<input id="party_id_<? echo $i; ?>" name="party_id_[]" type="hidden" value="<? echo $row['knitting_party']; ?>" />

												</td>
												<td width="40"><? echo $i; ?></td>
												<td width='100'><p> <? if (($type == 3 || $type==0) && $knitt_source==3){ echo " <a href='##' onclick=\"generate_report(" . $row['company_id'] . "," . $program . ")\">" . $knitting_party . "</a>";} else { echo $knitting_party;} ?>

												</p></td>
												<td width="100">
													<p align="center"><? echo $location_arr[$row['location_id']];?></p>
												</td>


												<td width="60" align="center"><a href='##' onClick="generate_report2(<? echo $row['company_id'] . "," . $program . "," . "'knitting_plan_path'" . "," . "'knitting_plan_report'"; ?>)"><? echo $program; ?></a></td>
												<td width="80" align="center">&nbsp;<? echo change_date_format($row['program_date']); ?></td>
												<td width="80" align="center">&nbsp;<? if ($row['start_date'] != "0000-00-00") echo change_date_format($row['start_date']); ?></td>
												<td width="80" align="center">&nbsp;<? if ($row['end_date'] != "0000-00-00") echo change_date_format($row['end_date']); ?></td>
												...
												<td width="130">
													<div style="word-wrap:break-word; width:129px">
														...
														else echo "<a href='##' onclick=\"generate_report(" . $row['company_id'] . "," . $program . ")\">$po_no</a>"; ?>
													</div>
												</td>
												...
												<td width="70" align="center"><? echo $reqsDataArr[$program]['reqs_no']; ?>&nbsp;</td>
												<td width="80"><p><? echo $machine_dia_gg; ?></p></td>
												<td align="right" width="100"><? echo number_format($row['distribution_qnty'], 2); ?></td>
												<td width="80"><p><? echo $machine_no; ?>&nbsp;</p></td>
												<td width="70"><p><? echo $knitting_program_status[$row['status']]; ?>&nbsp;</p></td>
												<td width="140"><p><? echo $row['fabric_desc']; ?></p></td>
												...
												<td width="100"><p><? echo
												//$job_no.'='. $po_id_booking[0].'='.$row[csf('gsm_weight')].'='. $row[csf('COLOR_ID')];
												$color_type[$colorType];//$color_type[$row[csf('color_type_id')]]; ?>&nbsp;</p></td>
												<td width="100"><p><? echo $row['stitch_length']; ?>&nbsp;</p></td>
												<td width="100"><p><? echo $row['spandex_stitch_length']; ?>&nbsp;</p></td>
												<td align="right" width="100"><? echo number_format($row['draft_ratio'], 2); ?>&nbsp;</td>
												<td width="70"><p><? echo $row['gsm_weight']; ?>&nbsp;</p></td>
												<td width="70"><p><? echo $row['dia']; ?>&nbsp;</p></td>
												<td width="80"><? echo $fabric_typee[$row['width_dia_type']]; ?>&nbsp;</td>
												<td align="right" width="100"><? echo number_format($row['program_qnty'], 2,".",""); ?></td>
												<td align="right" width="100"><? echo number_format($reqsDataArr[$program]['yarn_req_qnty'],2,".","");?></td>
												<td align="right" width="100">
													<?
													$requi_balance = ($row['program_qnty'] - $reqsDataArr[$program]['yarn_req_qnty']);
												...
												<td align="right" width="100">
													<a  href='##' onClick="openmypage_popup('<? echo $program;?>','knitting_popup')">
												...
												<td align="right" width="100">
													<a href="##" onClick="openmypage_popup('<? echo $program;?>','grey_purchase_delivery')">
														<? echo number_format($deliveryStorQtyArr[$program],2);?>
													</a>
												</td>
												<td align="right" width="100">
												...
												<td align="right" width="100">
													<a href="##" onClick="openmypage_popup('<? echo $program;?>','grey_receive_popup')">
														<? echo number_format($knitting_recv_qnty,2);?>
													</a>
													<? //echo number_format($knitting_recv_qnty, 2); ?>
												</td>
												<? if($presentationType == 6){ ?>
												<td width="100" align="right" title="( Yarn Issue Qnty-Issue Return Qnty-Reject Qnty-Knitting Qnty-Reject Fabric Qnty )">
												<?
												$knit_ballance_iw_qnty = ($yarn_issue_qnty-$yarn_issue_return_qnty-$yarn_issue_reject_qnty-$knitting_qnty-$fabric_reject_qnty);
												echo number_format($knit_ballance_iw_qnty, 2); ?>
												</td>
												<? }?>
												<td align="right" width="100">
														<?
												...
												<td align="center" width="150"><p><?php echo $program_age;?></p></td>
												<td><p><? echo $row['remarks']; ?>&nbsp;</p></td>
											</tr>
											<?
											$sub_tot_program_qnty += $row['program_qnty'];
											$sub_tot_requ_qnty += $reqsDataArr[$program]['yarn_req_qnty'];
											...
											$sub_tot_delivery_qty += $deliveryStorQtyArr[$program];
											...
											$sub_knit_ballance_iw_qnty += $knit_ballance_iw_qnty;

											$tot_program_qnty += $row['program_qnty'];
											$tot_requ_qnty += $reqsDataArr[$program]['yarn_req_qnty'];
											...
											$total_delivery_qty += $deliveryStorQtyArr[$program];
											...
											$tot_knit_ballance_iw_qnty += $knit_ballance_iw_qnty;
									...
									<tr bgcolor="#CCCCCC">
										...
										<? if($presentationType == 6){ ?>
										<td width="100" align="right"><? echo number_format($sub_knit_ballance_iw_qnty, 2, '.', ''); ?></td>
										<? }?>
						...
						<tfoot>
							...
							<th align="right"><b><? echo number_format($total_delivery_qty, 2, '.', ''); ?></b></th>
							<th align="right"><b><? echo number_format($tot_no_of_roll_delv, 2, '.', ''); ?></b></th>
							<th align="right"><? echo number_format($tot_knitting_recv_qnty, 2, '.', ''); ?></th>

							<? if($presentationType == 6){ ?>
							<th width="100"><? echo number_format($tot_knit_ballance_iw_qnty, 2, '.', ''); ?></th>
							<? }?>
					...
					<table cellspacing="0" cellpadding="0" border="1" rules="all" width="3580" class="rpt_table" id="tbl_list_search">
						<tbody>
							...
							foreach ($data_arr as $knitt_source => $knitt_sourcev)
							{
								?>
								<tr bgcolor="#EFEFEF">
									<td colspan="37"><b>Source:- <? echo $search_by_arr[$knitt_source]; ?></b></td>
								</tr>
								<?
								foreach ($knitt_sourcev as $machine_dia_gg => $machine_dia_ggv)
								{
									?>
									<tr bgcolor="#EFEFEF">
										<td colspan="37"><b>Machine Dia:- <? echo $machine_dia_gg; ?></b></td>
									</tr>
									<?
									$sub_tot_program_qnty=0;
									$sub_yarn_issue_qnty=0;
									...
									foreach ($machine_dia_ggv as $program => $row)
									{
										...
										$prod_id = array_unique(explode(",", chop($reqsDataArr[$program]['prod_id'],",")));
										$yarn_desc = '';
										$lot = '';
										$supplier = '';
										foreach ($prod_id as $val) {
											...
											$yarnRtnQty = $yarn_IssRtn_arr[$reqsDataArr[$program]['reqs_no']][$val];
											$yarn_issue_qnty += $yarn_iss_arr[$reqsDataArr[$program]['reqs_no']][$val] - $yarnRtnQty;
										}
										...
										$po_id = array_unique(explode(",", chop($plan_details_array[$program],",")));
										$po_no = '';
										$style_ref = '';
										$job_no = '';
										...
										$knitting_qnty = $knitDataArr[$program]['qnty'];
										$knit_id = $knitDataArr[$program]['knit_id'];
										$knit_id = array_unique(explode(",", $knit_id));
										...
										$balance_qnty = $row['program_qnty'] - $knitting_qnty;
										$balance_recv_qnty = $knitting_qnty - $knitting_recv_qnty;
										$yarn_issue_bl_qnty = $row['program_qnty'] - $yarn_issue_qnty;
										...
										if ($knitt_source == 1)
											$knitting_source = $company_library[$row['knitting_party']];
										else if ($knitt_source == 3)
											$knitting_source = $supplier_details[$row['knitting_party']];
										else
											$knitting_source = "&nbsp;";
										?>
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
											...
											<td width="60" align="center"><? echo $program; ?>&nbsp;</td>
											<td width="80" align="center"><? echo change_date_format($row['program_date']); ?>&nbsp;</td>
											<td width="80" align="center"><? if ($row['start_date'] != "0000-00-00") echo change_date_format($row['start_date']); ?>&nbsp;</td>
											<td width="80" align="center"><? if ($row['end_date'] != "0000-00-00") echo change_date_format($row['end_date']); ?>&nbsp;</td>
											<td width="80"><p><? echo $buyer_arr[$row['buyer_id']]; ?></p></td>
											...
											<td width="130">
												<div style="word-wrap:break-word; width:129px">
													<? echo "<a href='##' onclick=\"generate_report(" . $row['company_id'] . "," . $program . ")\">$po_no</a>"; ?>
												</div>
											</td>
											<td width="110"><p><? echo $style_ref; ?></p></td>
											<td width="70" align="center"><? echo $reqsDataArr[$program]['reqs_no']; ?>&nbsp;</td>
											...
											echo $cons_qty_arr[$reqsDataArr[$program]['reqs_no']];
											$tot_issue_return_qnty += $cons_qty_arr[$reqsDataArr[$program]['reqs_no']];
											$sub_issue_ret_tot += $cons_qty_arr[$reqsDataArr[$program]['reqs_no']];
											?></b></td>
											<td align="right" width="100"><? echo number_format($balance_qnty, 2); ?></td>
										...
										$source_tot_program_qnty += $row['program_qnty'];
				...
				<div style="width:<? echo $tbl_width2 - 20; ?>px; overflow-y:scroll; max-height:330px;" id="scroll_body">
					<table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width2 - 40; ?>" class="rpt_table" id="tbl_list_search">
						<tbody>
							...
							foreach ($data_arr as $machine_dia_gg => $machine_dia_ggv)
							{
								?>
								<tr bgcolor="#EFEFEF">
								...
								foreach ($machine_dia_ggv as $program => $row)
								{
									...
									$knit_id = $knitDataArr[$program]['knit_id'];
									$knit_id = array_unique(explode(",", $knit_id));
									$knit_gsm = $knitDataArr[$program]['gsm'];
									...
									<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
										...
										<td width="40"><? echo $i; ?></td>
										<? if ($type == 3) echo "<td width='100'><p><a href='##' onclick=\"generate_report(" . $row['company_id'] . "," . $program . ")\">" . $supplier_details[$row['knitting_party']] . "</a></p></td>"; ?>
										<td width="60" align="center"><? echo $program; ?>&nbsp;</td>
										...
										<td width="110"><p><? echo $style_ref; ?></p></td>
										<td width="70" align="center"><? echo change_date_format($ship_date); //$reqsDataArr[$program]['reqs_no'];  ?>&nbsp;</td>
										...
										<td width="70"><p><? echo $row['dia']; ?>&nbsp;</p></td>
										<td width="80"><? echo $fabric_typee[$row['width_dia_type']]; ?>&nbsp;</td>
										<td align="right" title="Program Qty" width="100"><? echo number_format($row['program_qnty'],2);  ?></td>
										<td align="right" width="100"><? echo number_format($yarn_issue_qnty, 2); ?></td>
										<td align="right" width="100"><? echo number_format($yarn_issue_bl_qnty, 2); ?></td>
										<td align="right" width="100"><? echo number_format($knitting_qnty, 2); ?></td>
										<td align="right" width="100"><? echo number_format($balance_qnty, 2); ?></td>
										<td align="right" width="100"><? echo number_format($knitting_recv_qnty, 2); ?></td>
										<td align="right" width="100"><? echo number_format($balance_recv_qnty, 2); ?></td>
										<td align="center" width="80"><? echo $knitting_program_status[$row['status']]; ?></td>
										<td><p><? echo $row['remarks']; ?>&nbsp;</p></td>
									</tr>
									<?
									$sub_tot_program_qnty += $row['program_qnty'];
				...
				<div style="width:<? echo $tbl_width - 20 +200; ?>px; overflow-y:scroll; max-height:330px;" id="scroll_body">
					<table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width - 40 +200; ?>" class="rpt_table" id="tbl_list_search">
						<tbody>
							...
							{
								$date1=date("d-m-Y");
								$date2=change_date_format($row[csf('program_date')]);
								$diff = abs(strtotime($date1) - strtotime($date2));
								$years = floor($diff / (365*60*60*24));
								$months = floor(($diff - $years * 365*60*60*24) / (30*60*60*24));
								//$days = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24));
								...
								if($display==trude)
								{
									...
									if (!in_array($machine_dia_gg, $machine_dia_gg_array[$knitting_source]))
									{
										...
										if( $knitting_source_tmp[$knitting_source]=='')
										{
											...
											?>
											<tr bgcolor="#C6D9C9">
												<td style="font-weight: 900" colspan="<? if($presentationType == 6){ echo$colspan + 21+1;}else{ echo$colspan + 21; } ?>"><b><?php echo $knitting_cond; ?></b></td>
											</tr>
											<?
									...
									$po_id_booking = array_unique(explode(",", $plan_details_array[$row[csf('id')]]));
									$colorType=$bookingInfoArr[$job_no][$po_id_booking[0]][$row[csf('gsm_weight')]][$row[csf('COLOR_ID')]]['color_type'];
									?>
									<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
										<td width="40" align="center" valign="middle">
											<input type="checkbox" id="tbl_<? echo $i; ?>" name="check[]" onClick="selected_row(<? echo $i. "," .$presentationType; ?>);" />
										...
										<td width="60" align="center"><a href='##' onClick="generate_report2(<? echo $row[csf('company_id')] . "," . $row[csf('id')] . "," . "'knitting_plan_path'" . "," . "'knitting_plan_report'"; ?>)"><? echo $row[csf('id')]; ?></a></td>
										...
										<td align="right" width="100">
											<a href="##" onClick="openmypage_popup('<? echo $row[csf('id')];?>','grey_purchase_weight_delivery')">
												<?
												 echo number_format($gds_qnty_array[$row[csf('id')]],2);?>
											</a>
										</td>
										<td align="right" width="100">
												<? echo number_format($no_of_roll_delv,2);?>
									...
									$sub_tot_knitting_recv_qnty += $knitting_recv_qnty;
									$sub_tot_recv_balance += $balance_recv_qnty;
									$sub_tot_delivery_qty += $gds_qnty_array[$row[csf('id')]];
									$sub_yarn_issue_qnty += $yarn_issue_qnty;
									$sub_yarn_issue_return_qnty += $yarn_issue_return_qnty;
									$sub_yarn_issue_reject_qnty += $yarn_issue_reject_qnty;
									...
									$tot_knitting_recv_qnty += $knitting_recv_qnty;
									$tot_balance_recv_qnty += $balance_recv_qnty;
									$total_delivery_qty += $gds_qnty_array[$row[csf('id')]];
									$tot_yarn_issue_qnty += $yarn_issue_qnty;
									$tot_yarn_issue_return_qnty += $yarn_issue_return_qnty;
									$tot_yarn_issue_reject_qnty += $yarn_issue_reject_qnty;
									$tot_yarn_issue_bl_qnty += $yarn_issue_bl_qnty;
    ...
    foreach (glob("$user_ids*.xls") as $filename) {
    	if (@filemtime($filename) < (time() - $seconds_old))
    		@unlink($filename);
    }
    //---------end------------//
    $name = time();
    $filename = $user_ids . "_" . $name . ".xls";
    $create_new_doc = fopen($filename, 'w');
    $is_created = fwrite($create_new_doc, ob_get_contents());
    $filename = "requires/" . $user_ids . "_" . $name . ".xls";
    echo "$total_data####$filename####$presentationType";
    exit();
}
			...
			<table width="100%" cellpadding="2" cellspacing="5">
				...
				<tr>
					...
					<tr>
						<td><b>Buyer Name </b></td>
						<td>:</td>
						<td><? if($showBuyer==1) echo $buyer_name;else echo "WHS"; ?></td>
					</tr>
					<tr>
						<td><b>Style </b></td>
						<td>:</td>
...
if ($action == "requisition_print_three_excel")
{
	extract($_REQUEST);
	...
	?>
	<div style="width:1200px; margin-left:5px">
		<table width="100%" style="margin-top:10px; text-align:center;font-size:18px;" >
			<tr>
				<td width="100%" align="center" colspan="5" style="font-weight:bold"><b><? echo $company; ?></b></td>
			</tr>
			<tr>
			...
			<tr>
				<td width="100%" align="center" colspan="5"><b><u>Knitting Program</u></b></td>
			</tr>
		</table>
		<div style="margin-top:10px; width:950px">
			<table width="200px" cellpadding="2" cellspacing="5" style="text-align:center;font-size:18px">
				<tr >
					<td width="100" colspan="2"><b >Knitting Factory :</b></td>

					<td colspan="2"> <b><? echo substr($knitting_factory, 0, -1); ?>&nbsp; <span><? echo "($location_array[$location_id])";?></span></b></td>
				</tr>
                <? if($k_source==3){?>
                <tr>
                ...
                <? } ?>
				<tr>
					<td colspan="2"><b>Attention :</b></td>
					<?
					if ($typeForAttention == 1) {
						echo "<td style=\"font-size:18px; font-weight:bold;\" colspan=\"2\"> Knitting Manager </td>";
					} else {
						?>
						<td  colspan="2"><b><?
						if ($k_source == 3) {
					...
					<tr>
						<td colspan="2"><b>Sub-contract :</b></td>
						<td colspan="2"><b><? echo $sub_con; ?></b></td>
					</tr>
					<tr>
						<td colspan="2"><b>Buyer Name :</b></td>
						<td colspan="2"><? echo $buyer_name; ?></td>
					</tr>
					<tr>
						<td colspan="2"><b>Style :</b></td>
					...
					<tr>
						...
						<td colspan="2" align="left"><b><? echo implode(',', $order_no); ?></b></td>
					</tr>
					<tr>
						<td colspan="2"><b>Internal Ref. No </b></td>
						<td colspan="2">
							<?
							if($db_type==0)
							{
		...
		if(!empty($rqsn_array)){
		?>
		<table width="<? echo $tblWidth;?>" style="margin-top:10px; font-size:18px" border="1" rules="all" cellpadding="3"  >
			<thead>
				<th width="20">SL</th>
				<th width="50">Requisition No</th>
				<th width="50">Brand</th>
				<th width="50">Lot No</th>
				<th width="70">Yarn Description</th>
				<th width="50">Color</th>

                <? if($distribute_qnty_variable == 1){?>
					<th width="50">Distribution Qnty</th>
				<? } ?>

                <th width="50">Requisition Qty.</th>

				<? if($distribute_qnty_variable == 1){?>
					<th width="50">Returnable Qnty</th>
				<? } ?>
				<th>No Of Cone</th>
			</thead>
			<?
			...
			foreach ($rqsn_array as $prod_id => $data)
			{
				...
				?>
				<tr bgcolor="<? echo $bgcolor; ?>">
					<td width="20"><? echo $j; ?></td>
					<td width="50"><? echo substr($data['reqsn'], 0, -1); ?></td>
					<td width="50"><p><? echo $product_details_array[$prod_id]['brand']; ?>&nbsp;</p></td>
					<td width="50"><p><? echo $product_details_array[$prod_id]['lot']; ?></p></td>
					<td width="70"><p><? echo $product_details_array[$prod_id]['desc']; ?></p></th>
                    <td width="50"><p><? echo $product_details_array[$prod_id]['color']; ?>&nbsp;</p></td>
                    <?
					if($distribute_qnty_variable == 1){
						$existing_dist = return_field_value("sum(distribution_qnty) as exis_distribution_qnty","ppl_yarn_req_distribution","requisition_no in(".chop($data['reqsn'],",").") and prod_id=".$prod_id." and status_active=1 and is_deleted=0",'exis_distribution_qnty');
						?>
						<td align="right" title="<? echo $data['reqsn']; ?>" width="50"><? echo number_format($existing_dist, 2); ?></td>
						<?
					}
					?>
                    <td width="50" align="right"><p><? echo number_format($data['qnty'], 2, '.', ''); ?></p></td>
                    <?
					if($distribute_qnty_variable == 1){
						?>
						<td align="right" width="50"><? echo $returnable = (($data['qnty']-$existing_dist) > 0)?number_format($data['qnty']-$existing_dist, 2):"0.00"; ?></td>
						<?
					}
					?>
                    <td align="right"><? echo number_format($data['no_of_cone']); ?></td>
		...
		<? } ?>
		<br>
		<table style="margin-top:10px;font-size:18px" width="700px" border="1" rules="all" cellpadding="0" cellspacing="0" class="rpt_table" align="center">
				<thead align="center">
					<th width="10">SL</th>
					<th width="30">Prog/Req./Date</th>
					<th width="50">Fabrication</th>
					<th width="30">GSM</th>
					<th width="30">F. Dia</th>
					<th width="30">Dia Type</th>
					<th width="30">Floor</th>
					<th width="30">M/c. No</th>
					<th width="30">M/c. Dia & GG</th>
					<th width="30">Color</th>
					<th width="30">Color Range</th>
					<th width="30">S/L</th>
					<th width="30">Spandex S/L</th>
					<th width="30">Feeder</th>
					<th width="30">Count Feeding</th>
					<th width="30">Knit Start</th>
					<th width="30">Knit End</th>
					<th width="30">Program Qty.</th>
					<th width="50">Yarn Description</th>
					<th width="30">Lot</th>
					<th width="30">Yarn Qty.(KG)</th>
					<th>Remarks</th>
				</thead>
				<?
				$i = 1;
				...
				foreach ($nameArray as $row)
				{
						...
						foreach ($all_prod_id as $prod_id)
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>">
								...
								if ($z == 0) {
									...
									?>
									<td width="10" rowspan="<? echo $row_span; ?>"><? echo $i; ?></td>
									<td width="30" rowspan="<? echo $row_span; ?>" align="center" ><? echo $row[csf('program_id')]; ?><br><? echo $requisition_array[$row[csf('program_id')]]; ?><br><? echo change_date_format($row[csf("program_date")]);?></td>
									<td width="50" rowspan="<? echo $row_span; ?>"><? echo $row[csf('fabric_desc')]; ?></td>
									<td width="30" align="center" rowspan="<? echo $row_span; ?>"><? echo $row[csf('gsm_weight')]; ?></th>
										<td width="30" align="center" rowspan="<? echo $row_span; ?>"><? echo $row[csf('fabric_dia')]; ?></td>
										<td width="30" rowspan="<? echo $row_span; ?>"><? echo $fabric_typee[$row[csf('diatype')]]; ?></td>

										<td width="30" align="center" rowspan="<? echo $row_span; ?>"><? echo $floor_arr[$sql_floor[0][csf('floor_id')]]; ?></td>

										<td width="30" align="center" rowspan="<? echo $row_span; ?>"><? echo $machine_no; ?></td>
										<td width="30" rowspan="<? echo $row_span; ?>"><? echo $row[csf('machine_dia')] . "X" . $row[csf('machine_gg')]; ?></td>
										<td width="30" rowspan="<? echo $row_span; ?>"><? echo $color; ?></td>
										<td width="30" rowspan="<? echo $row_span; ?>"><? echo $color_range[$row[csf('color_range')]]; ?></td>
										<td width="30" rowspan="<? echo $row_span; ?>"><? echo $row[csf('stitch_length')]; ?></td>
										<td width="30" rowspan="<? echo $row_span; ?>"><? echo $row[csf('spandex_stitch_length')]; ?></td>
										<td width="30" rowspan="<? echo $row_span; ?>"><? echo $feeder[$row[csf('feeder')]]; ?></td>
										<td width="30" rowspan="<? echo $row_span; ?>"><? echo $count_feeding?> </td>
										<td width="30" rowspan="<? echo $row_span; ?>"><? echo change_date_format($row[csf('start_date')]); ?></td>
										<td width="30" rowspan="<? echo $row_span; ?>"><? echo change_date_format($row[csf('end_date')]); ?></td>
										<td width="30" align="right" rowspan="<? echo $row_span; ?>"><? echo number_format($row[csf('program_qnty')], 2, '.', ''); ?></td>
										<?
										$tot_program_qnty += $row[csf('program_qnty')];
										$i++;
									}
									?>
									<td width="50"><? echo $product_details_array[$prod_id]['desc']; ?></td>
									<td width="30" align="center"><? echo $product_details_array[$prod_id]['lot']; ?></td>
									<td width="30" align="right"><? echo number_format($prod_id_array[$row[csf('program_id')]][$prod_id], 2, '.', ''); ?></td>
									<?
									if ($z == 0) {
										?>
							...
							<tr bgcolor="<? echo $bgcolor; ?>">
								...
								<td width="30" rowspan="<? echo $row_span; ?>" align="center" ><? echo $row[csf('program_id')]; ?><br><? echo $requisition_array[$row[csf('program_id')]]; ?><br><? echo change_date_format($row[csf("program_date")]);?></td>
								<td width="50" rowspan="<? echo $row_span; ?>"><? echo $row[csf('fabric_desc')]; ?></td>
								<td width="30" rowspan="<? echo $row_span; ?>" align="center"><? echo $row[csf('gsm_weight')]; ?></th>
									<td width="30" rowspan="<? echo $row_span; ?>" align="center"><? echo $row[csf('fabric_dia')]; ?></td>
									<td width="30" rowspan="<? echo $row_span; ?>"><? echo $fabric_typee[$row[csf('diatype')]]; ?></td>

									<td width="30" rowspan="<? echo $row_span; ?>" align="center"><? echo $floor_arr[$sql_floor[0][csf('floor_id')]]; ?></td>

									<td width="30" rowspan="<? echo $row_span; ?>" align="center"><? echo $machine_no; ?></td>
									<td width="30" rowspan="<? echo $row_span; ?>"><? echo $row[csf('machine_dia')] . "X" . $row[csf('machine_gg')]; ?></td>
									<td width="30" rowspan="<? echo $row_span; ?>"><? echo $color; ?></td>
									<td width="30" rowspan="<? echo $row_span; ?>"><? echo $color_range[$row[csf('color_range')]]; ?></td>
									<td width="30" rowspan="<? echo $row_span; ?>"><? echo $row[csf('stitch_length')]; ?></td>
									...
									<td width="30" rowspan="<? echo $row_span; ?>"><? echo change_date_format($row[csf('start_date')]); ?></td>
									<td width="30" rowspan="<? echo $row_span; ?>"><? echo change_date_format($row[csf('end_date')]); ?></td>
									<td width="30" align="right"><? echo number_format($row[csf('program_qnty')], 2, '.', ''); ?>&nbsp;</td>
									<td width="50">&nbsp;</td>
									<td width="30">&nbsp;</td>
					...
					{
						?>
						<table style="margin-top:10px; font-size:18px" width="230" border="1" rules="all" cellpadding="0" cellspacing="0" class="rpt_table">
							<thead>
								<tr>
									<th width="30">SL</th>
									<th width="50">Body Part</th>
									<th width="50">Grey Size</th>
							...
							<tbody>
								...
								foreach($sql_collarCuff as $row)
								{
									...
									<tr>
										...
										<td align="right"><? echo number_format($row[csf('qty_pcs')],0); $total_qty_pcs+=$row[csf('qty_pcs')]; ?></td>
									</tr>
									<?
									$i++;
								}
				...
				if (count($result_stripe) > 0) {
					?>
					<table cellspacing="0" cellpadding="0" border="1" rules="all" width="190" class="rpt_table" style="font-size:18px">
						<thead>
							<tr>
								<th colspan="7">Stripe Measurement</th>
							</tr>
							<tr>
								<th width="10">SL</th>
								<th width="30">Prog. no</th>
								<th width="30">Color</th>
								<th width="30">Stripe Color</th>
						...
						foreach ($result_stripe as $row) {
							...
							<tr valign="middle" bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>">
								...
								<td width="30" align="center"><? echo $programIDS_arr[$row[csf('pre_cost_id')]];//$row[csf('dtls_id')]; ?></td>
								<td width="30"><p><? echo $color_library[$row[csf('color_number_id')]]; ?></p></td>
								<td width="30"><p><? echo $color_library[$row[csf('stripe_color')]]; ?></p></td>
								<td width="30" align="center"><? echo $row[csf('measurement')]; ?></td>
				...
				<table border="1" rules="all"  class="rpt_table" style="font-size:18px;font-weight:bold;">
					<tr>
				...
				<table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" >
					<tr>
						<td>
							...
							<table border="1" rules="all" class="rpt_table" width="100" style="font-size:18px;">
								<thead>
									<th colspan="4">Please Strictly Avoid The Following Faults.</th>
								</thead>
								<tbody >
										<tr >
											<td colspan="2"><b> 1.</b> Patta</td>
											<td colspan="2"><b> 8.</b> Sinker mark</td>
										</tr>
										<tr>
											<td colspan="2"><b> 2.</b> Loop </td>
											<td colspan="2"><b> 9.</b> Needle mark</td>
										</tr>
										<tr>
											<td colspan="2"><b> 3.</b> Hole </td>
											<td colspan="2"><b> 10.</b> Oil mark</td>
										</tr>
										<tr>
											<td colspan="2" ><b> 4.</b> Star marks</td>
											<td colspan="2" ><b> 11.</b> Dia mark/Crease Mark</td>
										</tr>
										<tr>
											<td colspan="2"><b> 5.</b> Barre</td>
											<td colspan="2"><b> 12.</b> Wheel Free</td>
										</tr>
										<tr>
											<td colspan="2"><b> 6.</b> Drop Stitch</td>
											<td colspan="2"><b> 13.</b> Slub</td>
										</tr>
										<tr>
											<td colspan="2"><b> 7.</b> Lot mixing</td>
											<td colspan="2"><b> 14.</b> Other contamination</td>
									</tr>
								</tbody>
							</table>
						</td>
						<td>
							...
							<table border="1" rules="all" class="rpt_table" width="90" style="font-size:18px;">
								<thead>
									<th colspan="3" width="90">Machine Wise Plan Distribution Qty</th>
								</thead>
								<tr>
									<th width="30"><p>MC No</p></th>
									<th width="30"><p> M/C. Dia && GG</p></th>
									<th width="30"> <p>Prog. Qty</p></th>
								...
								foreach($machine_array as $k=>$v)
								{
									?>
									<tr>
										<td width="30" align="center"> <? echo $v["no"]; ?></td>
										<td width="30" align="center"> <? echo  $lib_machine_arr[trim($k,",")]["dia_width"]."X".$lib_machine_arr[trim($k,",")]["gauge"];?></td>
										<td width="30" align="right"> <? echo number_format($v["qty"],2); ?></td>
						...
						<td>
							...
							<table border="1" rules="all" class="rpt_table" width="100" style="font-size:18px;">
								<thead>
									<th colspan="4">Please Mark The Role The Each Role as Follows</th>
								</thead>
								<tr>
									<td colspan="2"><b> 1.</b> Manufacturing Factory Name</td>
									<td colspan="2"><b> 6.</b> Fabrics Type</td>
								</tr>
								<tr>
									<td colspan="2"><b> 2.</b> Company Name.</td>
									<td colspan="2"><b> 7.</b> Finished Dia </td>
								</tr>
								<tr>
									<td colspan="2"><b> 3.</b> Buyer, Style,Order no.</td>
									<td colspan="2"><b> 8.</b> Finished Gsm & Color</td>
								</tr>
								<tr>
									<td colspan="2"><b> 4.</b> Yarn Count, Lot & Brand</td>
									<td colspan="2"><b> 9.</b> Yarn Composition</td>
								</tr>
								<tr>
									<td colspan="2"><b> 5.</b> M/C No., Dia, Stitch Length</td>
									<td colspan="2"><b> 10.</b> Knit Program No </td>
								</tr>
							</table>
						</td>
					</tr>
	...
	//---------end------------//
	$name=time();
	$filename="tb".$user_ids."_".$name.".xls";
	$create_new_doc = fopen($filename, 'w');
...
if ($action == "requisition_print_three_new")
{
	echo load_html_head_contents("Program Qnty Info", "../../", 1, 1, '', '', '');
	extract($_REQUEST);
	$data = explode('**', $data);
		...
		<div style="margin-top:10px; width:950px">
			<table width="100%" cellpadding="2" cellspacing="5">
				<tr >
					<td width="130"><b style="font-size:20px">Knitting Factory </b></td>
					<td>:</td>
					<td style="font-size:20px"> <b><?echo $company; ?></b></td>
				</tr>
				<tr>
					<td style="font-size:20px"><b>Attention </b></td>
					<td>:</td>
			...
			*/
                                            ...
                                            foreach($color_id_arr as $color_id)
                                            {
                                                ...
                                                <tr>
                                                    ...
                                                    if( !empty($color_prog_arr[$row[csf('program_id')]][$color_id]))
                                                    {
                                                        ...
                                                        echo number_format($color_prog_arr[$row[csf('program_id')]][$color_id]);
                                                    }
                                                    else
                                                    {
                                                        echo number_format($row[csf('program_qnty')]);
                                                    }
                                                    ?>
                                                    </td>
                                                </tr>
			...
			<br>
				...
				foreach($sql_collarCuff as $row)
				{
					...
					$finish_gty .= $row['FINISH_SIZE'];
					$finish_gty .= $row['QTY_PCS'];
				}
				

				/*echo "<pre>";
				print_r($prog_body_part_arr);
				echo "<pre>";*/

				//for stripe
				//$sql_strip_data = "select a.pre_cost_fabric_cost_dtls_id,a.color_number_id, a.stripe_color, a.measurement, a.uom, b.dtls_id, b.no_of_feeder as no_of_feeder from wo_pre_stripe_color a, ppl_planning_feeder_dtls b where a.pre_cost_fabric_cost_dtls_id=b.pre_cost_id and a.color_number_id=b.color_id and a.stripe_color=b.stripe_color_id and b.dtls_id in(".$program_ids.") and b.no_of_feeder>0 and a.status_active=1 and a.is_deleted=0  group by a.pre_cost_fabric_cost_dtls_id,a.color_number_id, a.stripe_color, a.measurement, a.uom, b.dtls_id, b.no_of_feeder ";

				$sql_strip_data = "select b.dtls_id, c.id, c.pre_cost_fabric_cost_dtls_id as pre_cost_id, c.color_number_id, c.stripe_color, c.measurement, c.uom from wo_pre_stripe_color a, ppl_planning_feeder_dtls b, wo_pre_stripe_color c where a.pre_cost_fabric_cost_dtls_id=b.pre_cost_id and a.color_number_id=b.color_id and a.stripe_color=b.stripe_color_id and a.pre_cost_fabric_cost_dtls_id=c.pre_cost_fabric_cost_dtls_id and b.pre_cost_id=c.pre_cost_fabric_cost_dtls_id and a.color_number_id=c.color_number_id and b.color_id=c.color_number_id and a.stripe_color=c.stripe_color and b.stripe_color_id=c.stripe_color and a.color_number_id=c.color_number_id and b.dtls_id in(".$program_ids.") and b.no_of_feeder>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.status_active=1 and c.is_deleted=0 group by b.dtls_id, c.id, c.pre_cost_fabric_cost_dtls_id, c.color_number_id, c.stripe_color, c.measurement, c.uom order by  c.color_number_id,c.id";
                ...
                foreach ($result_stripe_data as $row)
				{
					$pre_cost_fb_cost_dtls_id_arr[$row[csf('pre_cost_id')]] = $row[csf('pre_cost_id')];
					$pre_cost_prog_no = $row[csf('dtls_id')];
					$prog_body_part = $prog_body_part_arr[$pre_cost_prog_no]['BODY_PART_ID'];
					$stripe_data_arr[$prog_body_part][$pre_cost_prog_no][$row[csf('id')]]['PRE_COST_ID'] = $row[csf('pre_cost_id')];
					$stripe_data_arr[$prog_body_part][$pre_cost_prog_no][$row[csf('id')]]['COLOR_NUMBER_ID'] = $row[csf('color_number_id')];
					$stripe_data_arr[$prog_body_part][$pre_cost_prog_no][$row[csf('id')]]['STRIPE_COLOR'] = $row[csf('stripe_color')];
					$stripe_data_arr[$prog_body_part][$pre_cost_prog_no][$row[csf('id')]]['MEASUREMENT'] = $row[csf('measurement')];
					$stripe_data_arr[$prog_body_part][$pre_cost_prog_no][$row[csf('id')]]['UOM'] = $row[csf('uom')];
                }

				/*echo "<pre>";
				print_r($stripe_data_arr);
				echo "<pre>";*/
                ...
                <?
				if(!empty($collar_cuff_data) && $finish_gty!='')
				{
						...
						<table style="margin-top:10px;float:left;" width="600" border="1" rules="all" cellspacing="0" cellpadding="0" class="rpt_table">
							...
							foreach($bd_part_arr as $prg_no=>$prg_no_arr)
							{
								...
								foreach($no_of_feeder_data as $key=>$val)
								{
									...
									if($no_of_feeder!="")
									{
										$noOfFeeder_array[$pre_cost_dtls_id][$color_id][$stripe_color][$ii] = $no_of_feeder;
										$ii++;
									}
								}

								?>
								<tr height='3px' style="background-color: #E5E5E5;">
									<th colspan="7"> </th>
								</tr>
								<?php
								...
								{
									//echo $row['PRE_COST_ID'].'='.$row['COLOR_NUMBER_ID'].'='.$row['STRIPE_COLOR'].'='.$i."<br/>";
									$no_of_feeder = $noOfFeeder_array[$row['PRE_COST_ID']][$row['COLOR_NUMBER_ID']][$row['STRIPE_COLOR']][$i];
									$tot_feeder += $no_of_feeder;
									if ($i % 2 == 0)
										$bgcolor = "#E9F3FF";
									else
...
//requisition_print_seven
if ($action == "requisition_print_seven")
{
	echo load_html_head_contents("Program Qnty Info", "../../", 1, 1, '', '', '');
	extract($_REQUEST);
	$data = explode('**', $data);
	...
	$location_array=return_library_array( "select id, location_name from lib_location",'id','location_name');
	if($typePresentation==5)
	{
		$order_no = '';
		$buyer_name = '';
		$knitting_factory = '';
		$job_no = '';$jobs_no = '';
		$booking_no = '';$bookingNos= '';
		$company = '';
		if ($db_type == 0) {
			$dataArray = sql_select("select a.id, a.knitting_source, a.knitting_party, a.subcontract_party,a.location_id, a.attention, b.buyer_id, b.booking_no, b.company_id, group_concat(distinct(b.po_id)) as po_id from ppl_planning_info_entry_dtls a, ppl_planning_entry_plan_dtls b where a.id=b.dtls_id and a.id in ($program_ids) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id, a.knitting_source, a.knitting_party, a.subcontract_party,a.location_id, b.buyer_id, b.booking_no, b.company_id");
		} else {
		...
		foreach ($dataArray as $row)
		{
			...
			if ($buyerName!=$row[csf('buyer_id')])
			{
				$buyer_name.= $buyer_arr[$row[csf('buyer_id')]].",";
				$buyerName=$row[csf('buyer_id')];
			}
			if ($bookingNO!= $row[csf('booking_no')])
			{
				$booking_no.= $row[csf('booking_no')].",";
			...
			foreach ($po_id as $val) {
				$order_no .= $po_array[$val]['no'] . ",";
				if ($job_noss != $po_array[$val]['job_no'])
				{
					//$job_no = $po_array[$val]['job_no'];
					$job_no.="'".$po_array[$val]['job_no']."',";
					$jobs_no.=$po_array[$val]['job_no'].",";
					$job_noss=$po_array[$val]['job_no'];
				}
	...
	<div style="width:1200px; margin-left:5px; font-family: arial-narrow">
		...
		<div style="margin-top:10px; width:950px">
			<table width="100%" cellpadding="2" cellspacing="5">
				...
				<tr>
					<td><b>Buyer Name </b></td>
					<td>:</td>
					<td><b><? echo chop($buyer_name,','); ?></b></td>
				</tr>
				<tr>
					<td><b>Style </b></td>
					<td>:</td>
					...
					if ($job_no != '') {
						...
						{
							$style_val="";
							$sqlStyeRef=sql_select("select job_no,style_ref_no from wo_po_details_master where job_no in($job_no) and status_active=1 and is_deleted=0");
							foreach ($sqlStyeRef as $row) {
								$style_val.=$row[csf('style_ref_no')].",";
							}
							$style_val=chop($style_val,",");
						}
						else
						{
							$style_val = return_field_value("style_ref_no", "wo_po_details_master", "job_no='$job_no' and status_active=1 and is_deleted=0", "style_ref_no");
				...
				<tr>
					...
					<td>
						...
						if($typePresentation==5)
						{
							...
							if($db_type==0)
							{
								$sql_ref = sql_select("select group_concat(grouping) as grouping  from wo_po_break_down where job_no_mst in($job_no) and status_active=1 and is_deleted=0");
							}
							else
							{
								$sql_ref = sql_select("select listagg(cast(grouping as varchar2(4000)),',') within group (order by grouping) as grouping  from wo_po_break_down where job_no_mst in($job_no) and status_active=1 and is_deleted=0");
							}
						}
						else
						{
				...
				<tr>
					<td><b>Job No </b></td>
					<td>:</td>
					<td><? if($typePresentation==5){ echo $jobs_no;}else{echo $job_no;}  ?></td>
				</tr>
				<tr>
					<td><b>Booking No </b></td>
					<td>:</td>
					...
					$is_short_book=return_field_value("is_short","wo_booking_mst","booking_no in($bookingNos)","is_short");
					$book_sql=sql_select("select booking_type,is_short from wo_booking_mst where status_active=1 and booking_no='$booking_no'");
					$is_short_book=$book_sql[0][csf("is_short")];
					$booking_type=$book_sql[0][csf("booking_type")];
					if($booking_type==4)
					...
					echo chop($booking_no,",").' ('.$is_short_type.")";
					?></b></td>
				</tr>
                <tr>
                	<td>User Name</td>
                	<td>:</td>
                	<td><? echo $_SESSION['logic_erp']['user_name']; ?></td>
                </tr>
			</table>
		</div>
        <?
		...
		<table style="margin-top:10px; font-family: arial-narrow;" width="100%" border="1" rules="all" cellpadding="0" cellspacing="0" class="rpt_table" align="center">
				...
				$feedingResult =  sql_select("SELECT dtls_id, seq_no, count_id, feeding_id FROM ppl_planning_count_feed_dtls WHERE dtls_id in($program_ids) and status_active=1 and is_deleted=0 order by seq_no");
				...
				$sql = "select a.company_id, a.fabric_desc, a.gsm_weight, a.dia, a.width_dia_type, b.id as program_id, b.color_id, b.color_range, b.machine_dia, b.width_dia_type as diatype, b.machine_gg, b.fabric_dia, b.program_qnty, b.program_date, b.stitch_length,b.spandex_stitch_length,b.feeder, b.machine_id, b.start_date, b.end_date, b.remarks,b.advice from ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b where a.id=b.mst_id and b.id in($program_ids) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.fabric_desc, a.gsm_weight, b.stitch_length";
				$nameArray = sql_select($sql);
				$fab_gsm_sl_qty = array();
				foreach ($nameArray as $row)
				{
					$fgs_qty = 0;
					if ($knit_id_array[$row[csf('program_id')]] != "")
					{
						foreach($color_id_arr as $color_id)
						{
							if( !empty($color_prog_arr[$row[csf('program_id')]][$color_id]))
							{
								$fgs_qty = $color_prog_arr[$row[csf('program_id')]][$color_id];
							}
							else
							{
								$fgs_qty = $row[csf('program_qnty')];
							}
						}
					}
					else
					...
					$fab_gsm_sl_qty[$row[csf('fabric_desc')]][$row[csf('gsm_weight')]][$row[csf('stitch_length')]]['qty'] += $fgs_qty;
					$fab_gsm_sl_qty[$row[csf('fabric_desc')]][$row[csf('gsm_weight')]][$row[csf('stitch_length')]]['cnt'] += 1;
				}
				...
				foreach ($nameArray as $row)
				{
						...
						foreach ($all_prod_id as $prod_id)
						{
                            ...
                            if ($z == 0)
                            {
                                ...
                                <td width="50" align="center" rowspan="<? echo $row_span; ?>"><p><? echo $row[csf('gsm_weight')]; ?></p></th>
                                    ...
                                    <?php if( !empty($color_prog_arr[$row[csf('program_id')]][$color_id]))
                                    {
                                    	$sub_total_prog_qty += $color_prog_arr[$row[csf('program_id')]][$color_id];
										echo number_format($color_prog_arr[$row[csf('program_id')]][$color_id], 2, '.', '');
                                    }
                                    else
                                    {
                                    	$sub_total_prog_qty += $row[csf('program_qnty')];
					...
					if($fab_gsm_sl_qty[$row[csf('fabric_desc')]][$row[csf('gsm_weight')]][$row[csf('stitch_length')]]['cnt'] == $cnt && $cnt > 1)
					{
						?>
						<tr >
							<td colspan="15" align="right"><strong>Sub Total</strong></td>
                            <td align="right"><strong><?php echo number_format($sub_total_prog_qty, 2, '.', ',');?></strong></td>
                            <td colspan="4"></td>
						</tr>
						<?
						$cnt = 0;
						$sub_total_prog_qty = 0;
					}
					...
					if(count($sql_collarCuff)>0)
					{
						...
						foreach($sql_collarCuff as $row)
						{
							$prog_body_part_arr[$row[csf('dtls_id')]]['BODY_PART_ID'] = $row[csf('body_part_id')];
						}
						?>
						<table style="margin-top:10px;" width="850" border="1" rules="all" cellpadding="0" cellspacing="0" class="rpt_table">
							<thead>
					...
					//echo $sql_strip_data;die;

					/* $sql_strip_data = "SELECT a.pre_cost_fabric_cost_dtls_id,a.color_number_id, a.stripe_color, a.measurement, a.uom, b.dtls_id, b.no_of_feeder as no_of_feeder from wo_pre_stripe_color a, ppl_planning_feeder_dtls b where a.pre_cost_fabric_cost_dtls_id=b.pre_cost_id and a.color_number_id=b.color_id and a.stripe_color=b.stripe_color_id and b.dtls_id in($program_ids) and b.no_of_feeder>0 and a.status_active=1 and a.is_deleted=0 group by a.pre_cost_fabric_cost_dtls_id,a.color_number_id, a.stripe_color, a.measurement, a.uom, b.dtls_id, b.no_of_feeder "; */

					$result_stripe_data = sql_select($sql_strip_data);
					$pre_cost_fabric_cost_dtls_id="";$programIDS_arr=array();$stripeColors="";$colorsNumber="";
					$stripe_data_arr = array();
					foreach ($result_stripe_data as $row) {
						$pre_cost_fabric_cost_dtls_id.=$row[csf('pre_cost_fabric_cost_dtls_id')].",";
						$stripeColors.=$row[csf('stripe_color')].",";
						$colorsNumber.=$row[csf('color_number_id')].",";
						$programIDS_arr[$row[csf('pre_cost_fabric_cost_dtls_id')]]=$row[csf('dtls_id')];

						$pre_cost_prog_no = $row[csf('dtls_id')];
						$prog_body_part = $prog_body_part_arr[$pre_cost_prog_no]['BODY_PART_ID'];

						$stripe_data_arr[$prog_body_part][$pre_cost_prog_no][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$row[csf('id')]]['PRE_COST_ID'] = $row[csf('pre_cost_id')];
						$stripe_data_arr[$prog_body_part][$pre_cost_prog_no][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$row[csf('id')]]['MEASUREMENT'] = $row[csf('measurement')];
						$stripe_data_arr[$prog_body_part][$pre_cost_prog_no][$row[csf('color_number_id')]][$row[csf('stripe_color')]][$row[csf('id')]]['UOM'] = $row[csf('uom')];

					}

					//echo "<pre>";print_r($stripe_data_arr);

					$pre_cost_fabric_cost_dtls_id=chop($pre_cost_fabric_cost_dtls_id,",");
					$stripeColors=chop($stripeColors,",");
					$colorsNumber=chop($colorsNumber,",");
					...
					{
						$feeder_data_arr[$row[csf('id')]] = $row[csf('no_fo_feeder_data')];
						$program_qnty_arr[$row[csf('id')]] += $row[csf('program_qnty')];
					}
					...
					// {
					// 	$color_wise_data = explode("_", $no_of_feeder_data[$i]);
					// 	if($color_wise_data[3]!=""){
					// 		$no_of_feeder_datax.= $color_wise_data[0]."_".$color_wise_data[1]."_".$color_wise_data[2]."_".$color_wise_data[3].",";
					// 	}
					// 	$pcfc_dtls_id = $color_wise_data[0];
					// 	$color_id = $color_wise_data[1];
					// 	$stripe_color = $color_wise_data[2];
					// 	$no_of_feeder = $color_wise_data[3];

					// 	//$noOfFeeder_array[$pcfc_dtls_id][$color_id][$stripe_color]=$no_of_feeder;
					// 	$noOfFeeder_array[$pcfc_dtls_id][$color_id][$stripe_color][$i]=$no_of_feeder;
					// 	//$noOfFeeder_array[$i] = $no_of_feeder;
					// }
						/*$no_of_feeder_datax=chop($no_of_feeder_datax,",");
						$no_of_feeder_dataxx = explode(",", $no_of_feeder_datax);
						for ($ii = 0; $ii < count($no_of_feeder_dataxx); $ii++)
						{
							$color_wise_datax = explode("_", $no_of_feeder_dataxx[$ii]);

							$pcfc_dtls_idx = $color_wise_datax[0];
							$color_idx = $color_wise_datax[1];
							$stripe_colorx = $color_wise_datax[2];
							$no_of_feederx = $color_wise_datax[3];
							$noOfFeeder_arrayx[$pcfc_dtls_idx][$color_idx][$stripe_colorx][$ii]=$no_of_feederx;
						}*/

						/*echo "<pre>";
						print_r($noOfFeeder_arrayx);
						echo "</pre>";*/

					// $sql_strip= "SELECT pre_cost_fabric_cost_dtls_id as pre_cost_id, color_number_id, stripe_color, measurement, uom from wo_pre_stripe_color where pre_cost_fabric_cost_dtls_id in($pre_cost_fabric_cost_dtls_id) and stripe_color in($stripeColors) and color_number_id in($colorsNumber)  and status_active=1 and is_deleted=0 order by color_number_id,id";
					// $result_stripe = sql_select($sql_strip);

					$prog_wise_feeder_arr = array();
					$prg_no_count=array();
					$color_count=array();
					$stripe_color_count=array();
					foreach($stripe_data_arr as $bd_part=>$bd_part_arr)
					{
						foreach($bd_part_arr as $prg_no=>$prg_no_arr)
						{
							...
							foreach($no_of_feeder_data as $key=>$val)
							{
								...
								if($no_of_feeder!="")
								{
									$noOfFeeder_array[$pre_cost_dtls_id][$color_id][$stripe_color][$kk] = $no_of_feeder;
									$kk++;
								}
							}

							$l=1;
							foreach($prg_no_arr as $color_id=>$color_id_arr)
							{
								foreach($color_id_arr as $stripe_color_id=>$stripe_color_arr)
								{
									foreach($stripe_color_arr as $row)
									{
										$prog_wise_feeder_arr[$prg_no] += $noOfFeeder_array[$row['PRE_COST_ID']][$color_id][$stripe_color_id][$l];

										$prg_no_count[$prg_no]++;
										$color_count[$prg_no][$color_id]++;
										$stripe_color_count[$prg_no][$color_id][$stripe_color_id]++;
					...
					//echo "<pre>";print_r($prog_wise_feeder_arr);

					if (count($stripe_data_arr) > 0) {
						?>
						<table cellspacing="0" cellpadding="0" border="1" rules="all" width="750" class="rpt_table" >
							<thead>
								<tr>
									<th colspan="9">Stripe Measurement</th>
								</tr>
								<tr>
									<th width="30">SL</th>
									<th width="100">Body Part</th>
							...
							foreach($stripe_data_arr as $bd_part=>$bd_part_arr)
							{
								foreach($bd_part_arr as $prg_no=>$prg_no_arr)
								{
									...
									foreach($no_of_feeder_data as $key=>$val)
									{
										...
										$color_wise_data = explode("_", $val);
										$pre_cost_dtls_id = $color_wise_data[0];
										$colour_id = $color_wise_data[1];
										$stripe_color = $color_wise_data[2];
										$no_of_feeder = $color_wise_data[3];
										if($no_of_feeder!="")
										{
											$noOfFeeder_array[$pre_cost_dtls_id][$colour_id][$stripe_color][$ii] = $no_of_feeder;
											$ii++;
										}
									}
									...
									foreach($prg_no_arr as $color_id=>$color_id_arr)
									{
										foreach($color_id_arr as $stripe_color_id=>$stripe_color_arr)
										{
											foreach($stripe_color_arr as $row)
											{
												...
												$program_no_span = $prg_no_count[$prg_no];
												$color_no_span = $color_count[$prg_no][$color_id];
												$stripe_color_span = $stripe_color_count[$prg_no][$color_id][$stripe_color_id];

												$no_of_feeder = $noOfFeeder_array[$row['PRE_COST_ID']][$color_id][$stripe_color_id][$i];

												$program_qnty = $program_qnty_arr[$prg_no];
												$program_wise_tot_feeder = $prog_wise_feeder_arr[$prg_no];
												$req_qnty = ($program_qnty/$program_wise_tot_feeder)*$no_of_feeder;
												if ($i % 2 == 0)
													$bgcolor = "#E9F3FF";
												else
													$bgcolor = "#FFFFFF";
												...
												<tr valign="middle" bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>">
													<td width="30" align="center"><? echo $i; ?></td>
													 <?
													if(!in_array($prg_no,$prg_no_chk))
													{
														$prg_no_chk[]=$prg_no;
														?>
													<td width="100" align="center" valign="middle" rowspan="<? echo $program_no_span ;?>" ><? echo $body_part[$bd_part]; ?></td>
													<td width="60" align="center" valign="middle" rowspan="<? echo $program_no_span ;?>" ><? echo $prg_no; ?></td>
													<? }
													if(!in_array($prg_no."**".$color_id,$color_chk))
													{
														$color_chk[]=$prg_no."**".$color_id;
													 ?>
													<td width="140" align="center" valign="middle" rowspan="<? echo $color_no_span ;?>"><p><? echo $color_library[$color_id]; ?></p></td>
													<? }
													if(!in_array($prg_no."**".$color_id."**".$stripe_color_id,$stripe_color_chk))
													{
														$stripe_color_chk[]=$prg_no."**".$color_id."**".$stripe_color_id;
													?>
													<td width="130" align="center" valign="middle" rowspan="<? echo $stripe_color_span ;?>"><p><? echo $color_library[$stripe_color_id]; ?></p></td>
													<? }?>
													<td width="70" align="right"><? echo $row['MEASUREMENT']; ?></td>
													<td width="50" align="center"><p><? echo $unit_of_measurement[$row['UOM']]; ?></p></td>
													<td width="50" align="right" title="<? echo $row['PRE_COST_ID']."=".$kl;?>">
													<?
													echo $no_of_feeder; ?>&nbsp;
												</td>
													<td align="right" style="padding-right:10px" title="<? echo 'program_qnty='.$program_qnty.'Formula=(Program Qnty/program wise total feeder*color wise feeder)';?>"><? echo number_format($req_qnty,2,'.',''); ?>&nbsp;</td>
												</tr>
												<?
												$tot_masurement += $row[csf('measurement')];
												$tot_feeder += $no_of_feeder;
												...
												$g_tot_masurement+= $row[csf('measurement')];
												$g_tot_feeder += $no_of_feeder;
												$g_tot_req_qnty += $req_qnty;
												$i++;
											}
										}
									}
									?>
										<tr>
											...
											<td width="140">&nbsp;</td>
											<td width="130" align="right"><b>Total :</b></td>
											<td width="70" align="right"><b><? echo $tot_masurement; ?></b></td>
											<td width="50" align="center">&nbsp;</td>
											<td width="50" align="right"><b><? echo $tot_feeder; ?></b>&nbsp;</td>
											<td align="right"  style="padding-right:10px"><b><? echo number_format($tot_req_qnty,2,'.',''); ?></b>&nbsp;</td>
										</tr>
									<?
						...
						<tfoot>
							...
							<th align="right"> <? echo $g_tot_masurement; ?>&nbsp;</th>
							<th>&nbsp;</th>
							<th><? echo $g_tot_feeder; ?>&nbsp;</th>
							<th style="padding-right:10px"><? echo number_format($g_tot_req_qnty,2,'.',''); ?>&nbsp;</th>
						</tfoot>
			...
			<div style="float:left; border:1px solid #000; margin-top:60px;margin-left: 10px;">
				<table border="1" rules="all" class="rpt_table" width="300" height="200">
					<thead>
						<th colspan="3" style="font-size:20px; font-weight:bold;">Machine Wise Plan Distribution Qty</th>
					</thead>
					<tr>
		...
		<table  width="400" class="rpt_table" style="border:1px solid black;"   border="0" cellpadding="0" cellspacing="0">
			<?

			$data_array=sql_select("select id, terms from  wo_booking_terms_condition where booking_no='$booking_no' and entry_form=429");
			if ( count($data_array)>0)
			{
				...
				<thead>
					<tr style="border:1px solid black;">
						<th width="3%" style="border:1px solid black;">Sl</th><th width="97%" style="border:1px solid black;font-weight:bold;"><b>Terms and Condition/Notes </b></th>
					</tr>
				</thead>
				<tbody>
			...
			/* else
			{
				...
				<?
					$i=0;
				$data_array=sql_select("select id, terms from  lib_terms_condition where is_default=1 and page_id=429");// quotation_id='$data'
				foreach( $data_array as $row )
				{
					$i++;
					?>
...
if ($action == "requisition_print_eight")
{
	echo load_html_head_contents("Program Qnty Info", "../../", 1, 1, '', '', '');
	extract($_REQUEST);
	$data = explode('**', $data);
		...
		<table width="100%" style="margin-top:10px;">
			<tr>
				<td width="100%" align="center" style="font-size:22px;">
					...
					if ($job_no != '')
					{
						$style_owner = return_field_value("style_owner", "wo_po_details_master", "job_no='$job_no' and status_active=1 and is_deleted=0", "style_owner");
					}
					if(!empty($style_owner)){
					?>
				   <b>Style Owner :	<? echo $company_details[$style_owner];?></b>
				   <? } ?>
				   <br>
				<stronmg> Owner Company : <? echo $company; ?></stronmg></td>
			</tr>
			<tr>
				<td align="center" style="font-size:16px">
					<?
		...
		<table style="margin-top:10px; font-family: arial-narrow;" width="100%" border="1" rules="all" cellpadding="0" cellspacing="0" class="rpt_table" align="center">
			...
			*/
			...
			<br>
				...
				/*echo "<pre>";
				print_r($collar_cuff_data);
				echo "<pre>";*/

				//for stripe
				//$sql_strip_data = "select a.pre_cost_fabric_cost_dtls_id,a.color_number_id, a.stripe_color, a.measurement, a.uom, b.dtls_id, b.no_of_feeder as no_of_feeder from wo_pre_stripe_color a, ppl_planning_feeder_dtls b where a.pre_cost_fabric_cost_dtls_id=b.pre_cost_id and a.color_number_id=b.color_id and a.stripe_color=b.stripe_color_id and b.dtls_id in(".$program_ids.") and b.no_of_feeder>0 and a.status_active=1 and a.is_deleted=0  group by a.pre_cost_fabric_cost_dtls_id,a.color_number_id, a.stripe_color, a.measurement, a.uom, b.dtls_id, b.no_of_feeder ";

				$sql_strip_data = "select b.dtls_id, c.id, c.pre_cost_fabric_cost_dtls_id as pre_cost_id, c.color_number_id, c.stripe_color, c.measurement, c.uom from wo_pre_stripe_color a, ppl_planning_feeder_dtls b, wo_pre_stripe_color c where a.pre_cost_fabric_cost_dtls_id=b.pre_cost_id and a.color_number_id=b.color_id and a.stripe_color=b.stripe_color_id and a.pre_cost_fabric_cost_dtls_id=c.pre_cost_fabric_cost_dtls_id and b.pre_cost_id=c.pre_cost_fabric_cost_dtls_id and a.color_number_id=c.color_number_id and b.color_id=c.color_number_id and a.stripe_color=c.stripe_color and b.stripe_color_id=c.stripe_color and a.color_number_id=c.color_number_id and b.dtls_id in(".$program_ids.") and b.no_of_feeder>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.status_active=1 and c.is_deleted=0 group by b.dtls_id, c.id, c.pre_cost_fabric_cost_dtls_id, c.color_number_id, c.stripe_color, c.measurement, c.uom";
				//echo $sql_strip_data;
				$result_stripe_data = sql_select($sql_strip_data);
				$stripe_data_arr = array();
				$pre_cost_fabric_cost_dtls_id="";
...
if ($action == "requisition_print_seven_20022022")
{
	echo load_html_head_contents("Program Qnty Info", "../../", 1, 1, '', '', '');
	extract($_REQUEST);
	$data = explode('**', $data);
...
if ($action == "knitting_card_print_4")
{
	...
	if(!$program_ids)
	{
		...
		$company_arr 	= return_library_array("select id,company_name from lib_company", "id", "company_name");
		$imge_arr		=return_library_array( "select master_tble_id, image_location from common_photo_library where form_name='company_details' and file_type=1",'master_tble_id','image_location');
		$imge_job_arr=return_library_array( "select master_tble_id,image_location from common_photo_library where file_type=1 and form_name='knit_order_entry'",'master_tble_id','image_location');
	...
	unset($sql_data);
		...
		$job_data_sql=sql_select("SELECT a.id,a.grouping, a.job_no_mst,a.po_number, sum(a.po_quantity) as poQuantity,b.booking_no,c.style_ref_no,c.product_dept,d.gmts_item_id from wo_po_break_down a, ppl_planning_entry_plan_dtls b,wo_po_details_master c,wo_po_details_mas_set_details d where a.id=b.po_id and a.job_id=c.id and c.id=d.job_id and b.dtls_id in ($program_ids) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  group by a.id,a.grouping,a.job_no_mst,a.po_number,b.booking_no,c.style_ref_no,c.product_dept,d.gmts_item_id");
		...
		foreach($job_data_sql as $row)
		{
			...
			$product_depart = $row[csf('product_dept')];
		...
		$company= $dataArray[0]["COMPANY_ID"];

		$name_iso_Array=sql_select( "select iso_no from lib_iso where company_id=$company and status_active=1 and module_id=4 and menu_id=307");
			...
			<div style="width:1030px;">
				<table width="100%" cellpadding="0" cellspacing="0" >
					<tr>
						...
						<td width="200" style="vertical-align: middle;"><b> <?= "ISO Number :".$name_iso_Array[0]["ISO_NO"];?></b></td>
						<td><strong>Garments Image:</strong></td>
					 	<td width="200"><img  src='../../<? echo $imge_job_arr[$jobNo]; ?>' height='30%' width='30%' /></td>
					</tr>
				</table>
				<table width="50%" cellpadding="0" cellspacing="0" align="center" style="margin-left:130px;">
					<tr>
						<td><strong>Style Ref.:</strong></td>
					 	<td width="100"><? echo ($style)? $style : "" ; ?></td>
						 <td><strong>Prod. Dept:</strong></td>
					 	<td width="100"><? echo ($product_depart)? $product_dept[$product_depart] : "" ; ?></td>
						 <td><strong>Garments Item:</strong></td>
					 	<td width="100"><? echo ($gmts_item_id)? $garments_item[$gmts_item_id] : "" ; ?>  </td>
					</tr>
				</table>
				<div style="margin-top:5px; width:1030px">
				...
				<div style="margin-top:10px; width:1030px">
				...
				<? echo signature_table(119, $prog_data['company_id'], "1030px","","20"); ?>
				<div class="page_break">&nbsp;</div>
			</div>
			<div>
			    <script type="text/javascript" src="../../js/jquery.js"></script>
...
if ($action == "knitting_card_print_5")
{
	...
	unset($sql_data);
		...
		$jobNo="";
		$poQuantity="";
		$job_data_sql=sql_select("select a.id, a.job_no_mst, a.po_number, a.pub_shipment_date, sum(a.po_quantity) as poQuantity, c.style_ref_no from wo_po_break_down a, ppl_planning_entry_plan_dtls b,wo_po_details_master c where a.id=b.po_id and b.dtls_id in ($program_ids) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.job_no_mst=job_no group by a.id, a.job_no_mst, a.po_number, a.pub_shipment_date, c.style_ref_no");
		$po_details= array();
		foreach($job_data_sql as $row)
		{
			$jobNo= $row[csf('job_no_mst')];
			...
			$style = $row[csf('style_ref_no')];
			$po_details[$row[csf('id')]]['po_number']= $row[csf('po_number')];
			$po_details[$row[csf('id')]]['pub_shipment_date']= $row[csf('pub_shipment_date')];
		}
		...
		foreach ($dataArray as $row)
		{
			...
			$program_data_arr[$row[csf('id')]]['pub_shipment_date']=$po_details[$row[csf('po_id')]]['pub_shipment_date'];
		}
		unset($dataArray);
				...
				<div style="margin-top:5px; width:930px">
					<table cellspacing="2" cellpadding="2" border="1" rules="all" width="100%" class="rpt_table" style="font-size:13px; font-family:'Arial Narrow'">
						...
						<tr height="22">
							...
							<td>Remarks</td>
							<td><p><? echo $prog_data['remarks']; ?></p></td>
							<td>Shipment date</td>
							<td><? echo  change_date_format($prog_data['pub_shipment_date']); ?></td>
...
if($action == "knitting_popup")
{
	...
	<div style="width:1137px" align="center"><input type="button" value="Print Preview" onClick="print_window()"
		style="width:100px" class="formbutton"/></div>
		<fieldset style="width:1137px;">
			<div id="report_container">

				<table border="1" class="rpt_table" rules="all" width="1120" cellpadding="0" cellspacing="0">
					<thead>
						<th colspan="13"><b>Grey Receive Info</b></th>
					</thead>
					<thead>
						<th width="30">SL</th>
				...
				<div style="width:1138px; max-height:330px; overflow-y:scroll" id="scroll_body">
					<table border="1" class="rpt_table" rules="all" width="1120" cellpadding="0" cellspacing="0"
					id="tbl_list_search">
					<?
					$i = 1;
					$total_receive_qnty = 0;$total_receive_qnty_pcs = 0;
					...
					$sql = "select * from (
						select  a.receive_date, a.recv_number,c.booking_no,  b.prod_id,b.machine_no_id, a.knitting_company,a.knitting_source,  a.receive_basis, a.challan_no, sum(b.grey_receive_qnty) as knitting_qnty,sum(b.grey_receive_qnty_pcs) as grey_receive_qnty_pcs from inv_receive_master a, pro_grey_prod_entry_dtls b , ppl_planning_info_entry_mst c, ppl_planning_info_entry_dtls d where a.id=b.mst_id and a.booking_id = d.id and c.id = d.mst_id and a.item_category=13 and a.entry_form=2 and a.receive_basis=2 and a.booking_id = $program_id and a.company_id = $companyID and b.status_active=1 and b.is_deleted=0 group by a.receive_date,a.receive_basis,a.recv_number,c.booking_no,  b.prod_id,b.machine_no_id, a.knitting_source,  a.knitting_company, a.challan_no
						union all
						select a.receive_date, a.recv_number,c.booking_no, b.prod_id, b.machine_no_id,a.knitting_company, a.knitting_source,   a.receive_basis, a.challan_no,   sum(b.grey_receive_qnty) as knitting_qnty,sum(b.grey_receive_qnty_pcs) as grey_receive_qnty_pcs from inv_receive_master a, pro_grey_prod_entry_dtls b , ppl_planning_info_entry_mst c, ppl_planning_info_entry_dtls d where a.id=b.mst_id  and b.program_no = d.id and c.id = d.mst_id and a.item_category=13 and a.entry_form=22 and a.receive_basis=11 and b.status_active=1 and b.is_deleted=0 and b.program_no in($program_id) and a.company_id = $companyID group by a.receive_date,a.receive_basis,a.recv_number,c.booking_no,  b.prod_id,b.machine_no_id, a.knitting_source, a.knitting_company, a.challan_no
					...
					foreach ($result as $row) {
						...
						$total_receive_qnty_pcs += $row[csf('grey_receive_qnty_pcs')];
						?>
						<tr bgcolor="<? echo $bgcolor; ?>"
			...
			<table border="1" class="rpt_table" rules="all" width="1120" cellpadding="0" cellspacing="0">
				<tfoot>
					...
					<th width="80" align="right"
					id="value_receive_qnty_tot"><? echo number_format($total_receive_qnty_pcs, 2, '.', ''); ?></th>
					<th width="70">&nbsp;</th>
					<th>&nbsp;</th>
				</tfoot>
			</table>
		...
		<div id="report_container">
			...
			<div style="width:740px; max-height:330px; overflow-y:scroll" id="scroll_body">
				<table border="1" class="rpt_table" rules="all" width="720" cellpadding="0" cellspacing="0">
					...
					$roll_sql = "SELECT c.sys_number,c.knitting_company,c.knitting_source,c.delevery_date, a.booking_no, sum(a.qnty)  as quantity, b.product_id from pro_roll_details a,pro_grey_prod_delivery_dtls b, pro_grey_prod_delivery_mst c where a.mst_id=b.grey_sys_id and b.mst_id = c.id and a.barcode_no=b.barcode_num and a.entry_form=2 and a.receive_basis=2 and a.booking_without_order=0 and a.booking_no = '$program_id' and c.company_id = $companyID and b.entry_form = 56 and b.status_active=1 and b.is_deleted=0 and c.status_active =1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 group by c.sys_number,c.knitting_company,c.knitting_source,c.delevery_date, a.booking_no, b.product_id order by c.delevery_date";

					$gds_sql="SELECT a.sys_number,a.knitting_company,a.knitting_source,a.delevery_date, b.program_no as booking_no, sum(b.current_delivery)  as quantity, b.product_id,b.grey_sys_id
					from pro_grey_prod_delivery_mst a, pro_grey_prod_delivery_dtls b
					where a.id=b.mst_id and b.program_no = '$program_id' and a.entry_form=53 and b.entry_form=53 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.sys_number,a.knitting_company,a.knitting_source,a.delevery_date, b.program_no, b.product_id,b.grey_sys_id order by a.delevery_date";
					$gds_result = sql_select($gds_sql);

					foreach ($gds_result as $row)
					{
						$grey_sys_id = $row[csf('grey_sys_id')];

						$gdsknitting_com=return_library_array( "select id, knitting_company from inv_receive_master where id=$grey_sys_id and entry_form=2 and item_category=13 and status_active=1 and is_deleted=0",'id','knitting_company');
						$gdsknitting_source=return_library_array( "select id, knitting_source from inv_receive_master where id=$grey_sys_id and entry_form=2 and item_category=13 and status_active=1 and is_deleted=0",'id','knitting_source');
					}

					$roll_result = sql_select($roll_sql);

					$result = array_merge($roll_result,$gds_result);
					...
					foreach ($result as $row) {
						...
						$knit_comp="";
						if($gdsknitting_com[$row[csf('grey_sys_id')]] !="")
						{
							if ($gdsknitting_source[$row[csf('grey_sys_id')]] == 1)
							{
								$knit_comp = $company_library[$gdsknitting_com[$row[csf('grey_sys_id')]]];
							}
							else if($gdsknitting_source[$row[csf('grey_sys_id')]] == 3)
							{
								$knit_comp = $supplier_details[$gdsknitting_com[$row[csf('grey_sys_id')]]];
							}
						}
						else
						{
							...
							{
								$knit_comp = $supplier_details[$row[csf('knitting_company')]];
							}
						}
						?>
						<tr bgcolor="<? echo $bgcolor; ?>"
...
if ($action == "grey_purchase_weight_delivery")
{
	echo load_html_head_contents("Report Info", "../../../", 1, 1, '', '', '');
	extract($_REQUEST);
	?>
		...
		<div id="report_container">
			...
			<div style="width:740px; max-height:330px; overflow-y:scroll" id="scroll_body">
				<table border="1" class="rpt_table" rules="all" width="720" cellpadding="0" cellspacing="0">
					...
					$sql="SELECT a.sys_number, a.company_id, a.delevery_date, b.program_no, b.current_delivery as delivery_qnty, b.product_id
					from pro_grey_prod_delivery_mst a, pro_grey_prod_delivery_dtls b
					where a.id=b.mst_id and a.company_id = $companyID and b.program_no = '$program_id' and a.entry_form=53 and b.entry_form=53 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
					...
					foreach ($result as $row) {
						...
						<tr bgcolor="<? echo $bgcolor; ?>"
							...
							<td width="30"><? echo $i; ?></td>
							<td width="125"><p><? echo $row[csf('sys_number')]; ?></p></td>
							<td width="200"><p><? echo $product_arr[$row[csf('product_id')]]; ?></p></td>
							<td width="75" align="center"><? echo change_date_format($row[csf('delevery_date')]); ?></td>
							<td align="right" width="80">
								<?
								echo number_format($row[csf('delivery_qnty')], 2, '.', '');
								$total_delivery_qnty += $row[csf('delivery_qnty')];
								?>
							</td>
							<td>
								<?  echo $company_library[$row[csf('company_id')]];  ?>
							</td>
						</tr>
						<?
						$i++;
			...
			<div id="report_container">
				...
				<div style="width:1037px; max-height:330px; overflow-y:scroll" id="scroll_body">
					<table border="1" class="rpt_table" rules="all" width="1020" cellpadding="0" cellspacing="0">
						...
						$auto_fabric_store_variable = return_field_value("auto_update", "variable_settings_production", "company_name=$companyID and variable_list=15 and item_category_id=13 and status_active=1 and is_deleted=0", "auto_update");
						...
						$sql = " SELECT a.recv_number, a.receive_date, a.receive_basis, a.knitting_source, a.challan_no, a.knitting_company, b.prod_id, sum(c.quantity) as quantity,a.booking_no
						...
						from inv_receive_master a, pro_grey_prod_entry_dtls b,order_wise_pro_details c
						where a.id=b.mst_id and c.dtls_id=b.id and a.item_category=13 and a.entry_form=2 and c.entry_form=2 and a.receive_basis=2
						and b.status_active=1 and b.is_deleted=0 and a.booking_id = '$program_id' and (b.trans_id = 0 or b.trans_id > 0)  and a.company_id = $companyID"; //
...
if ($action == "knitting_card_print_10")
{
	...
	$sub_subcontract = return_library_array("select id,supplier_name from lib_supplier", "id", "supplier_name");
	$brand_arr = return_library_array("select id, brand_name from lib_brand", 'id', 'brand_name');
	$lib_buyerBrand_arr = return_library_array("select id, brand_name from lib_buyer_brand", 'id', 'brand_name');
	$company_arr = return_library_array("select id,company_name from lib_company", "id", "company_name");
	$imge_arr=return_library_array( "select master_tble_id, image_location from common_photo_library where form_name='company_details' and file_type=1",'master_tble_id','image_location');
	$count_arr = return_library_array("select id, yarn_count from lib_yarn_count", 'id', 'yarn_count');
	...
	unset($sql_data);
		...
		$jobNo="";
		$poQuantity="";
		$job_data_sql=sql_select("select a.id, a.job_no_mst, a.po_number, a.pub_shipment_date, sum(a.po_quantity) as poQuantity, c.style_ref_no,c.brand_id from wo_po_break_down a, ppl_planning_entry_plan_dtls b,wo_po_details_master c where a.id=b.po_id and b.dtls_id in ($program_ids) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.job_no_mst=job_no group by a.id, a.job_no_mst, a.po_number, a.pub_shipment_date, c.style_ref_no,c.brand_id");
		$po_details= array();
		foreach($job_data_sql as $row)
		{
			$jobNo= $row[csf('job_no_mst')];
			...
			$po_details[$row[csf('id')]]['po_number']= $row[csf('po_number')];
			$po_details[$row[csf('id')]]['pub_shipment_date']= $row[csf('pub_shipment_date')];
			$buyerBrandArr[$row[csf('job_no_mst')]]['brand_id']= $row[csf('brand_id')];
		}
		...
		$data_sql="select a.id, a. knitting_source, a.knitting_party, a.subcontract_party, a.machine_id, a.machine_gg, a.machine_dia, a.color_id, b.program_qnty, a.stitch_length, a.fabric_dia, a.program_date, a.draft_ratio, a.start_date, a.end_date, a.remarks, a.co_efficient, a.width_dia_type, b.buyer_id, b.booking_no, b.company_id, b.fabric_desc, b.yarn_desc, b.gsm_weight,b.po_id from ppl_planning_info_entry_dtls a, ppl_planning_entry_plan_dtls b where a.id=b.dtls_id and a.id in ($program_ids) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ";
		...
		foreach($ex_mc_id as $mc_id)
		{
			...
			<div style="width:930px;">
				...
				<!--Start --->
				<div style="margin-top:5px; width:930px">
					<table cellspacing="2" cellpadding="2" border="1" rules="all" width="100%" class="rpt_table" style="font-size:12px;font-weight: bolder; font-family:'Arial Narrow'">
						<tr>
							<td width="130">Prog. No:</td><td width="160"><? echo $prog_no; ?></td>
							<td width="130">RPM:</td><td width="160">&nbsp;</td>
							<td width="130">Knit.Card Date:</td><td><? echo date("d-m-Y",time()); ?></td>
						...
						<tr>
							<td width="130">MC No:</td><td width="160"><? echo $machine_arr[$mc_id];//$prog_data['mc_nmae']; ?></td>
							<td width="130">Booking Qty:</td><td width="160"><? echo number_format($prog_data['booking_qty'], 2, '.', ''); ?></td>
							<td width="130">Colour:</td><td><? echo $prog_data['color_id']; ?></td>
						</tr>
						<tr>
	                    ...
	                    <tr>
							<td width="130">PO Quantity:</td><td width="160"><? echo  $poQuantity; ?></td>
							<td width="130">Prog. Quantity:</td><td ><? echo $prog_data['prog_qty']; ?></td>
	                        <td width="130">Y.Brand:</td><td ><? echo $prog_data['brand_name']; ?></td>
						</tr>
	                    <tr>
	                    	<td width="130">B.Brand:</td><td width="160"><? echo $lib_buyerBrand_arr[$buyerBrandArr[$jobNo]['brand_id']]; ?></td>
	                    	<td width="130">Remarks:</td><td  colspan="3" ><? echo $prog_data['remarks']; ?></td>
	                    </tr>
...
if ($action == "knit_card_print")
{
	...
	unset($sql_data);
		...
		$job_data_sql=sql_select("SELECT a.id, a.grouping, a.job_no_mst, a.po_number, a.pub_shipment_date, sum(a.po_quantity) as poQuantity, a.file_no, c.style_ref_no  from wo_po_break_down a, ppl_planning_entry_plan_dtls b,wo_po_details_master c where a.id=b.po_id and b.dtls_id in ($program_ids) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.job_no_mst=job_no group by a.id, a.grouping, a.job_no_mst, a.po_number, a.pub_shipment_date, a.file_no, c.style_ref_no");
		$po_details= array();
		foreach($job_data_sql as $row)
		{
			$jobNo= $row[csf('job_no_mst')];
		...
		$data_sql="select a.id, a. knitting_source, a.knitting_party, a.subcontract_party, a.machine_id, a.machine_gg, a.machine_dia, a.color_id, a.program_qnty, a.stitch_length, a.fabric_dia, a.program_date, a.draft_ratio, a.start_date, a.end_date, a.remarks, a.co_efficient, a.width_dia_type, b.buyer_id, b.booking_no, b.company_id, b.fabric_desc, b.yarn_desc, b.gsm_weight,b.po_id, a.feeder from ppl_planning_info_entry_dtls a, ppl_planning_entry_plan_dtls b where a.id=b.dtls_id and a.id in ($program_ids) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ";
		$dataArray = sql_select($data_sql);
		$program_data_arr=array();
		//$program_ids_arr=array();
		$company_id = '';
		$buyer_name = '';
		$booking_no = '';
		$orderNo = "";
		foreach ($dataArray as $row)
		{
			...
			$program_data_arr[$row[csf('id')]]['pub_shipment_date']=$po_details[$row[csf('po_id')]]['pub_shipment_date'];
			$program_data_arr[$row[csf('id')]]['feeder']=$row[csf('feeder')];
			//array_push($program_ids_arr,$row[csf('id')]);
		}
		unset($dataArray);
		...
		foreach ($reqsn_dataArray as $row)
		{
			$rqsn_array[$row[csf('knit_id')]][$row[csf('prod_id')]]['reqsn'] .= $row[csf('requisition_no')] . ",";
			$rqsn_array[$row[csf('knit_id')]][$row[csf('prod_id')]]['qnty'] += $row[csf('yarn_qnty')];
			$rqsn_array[$row[csf('knit_id')]][$row[csf('prod_id')]]['no_of_cone'] += $row[csf('no_of_cone')];
		}
		unset($reqsn_dataArray);
		...
		foreach($ex_mc_id as $mc_id)
		{
			...
			foreach($program_data_arr as $prog_no=>$prog_data)
			{
					...
					<div style="width:930px;">
						...
						<div style="margin-top:5px; width:930px">
							<table cellspacing="2" cellpadding="2" border="1" rules="all" width="100%" class="rpt_table" id="dataTable">
								<thead height="25">
									<th colspan="2">Program Details</th>
									<th colspan="2" width="150">Job Details</th>
									<th colspan="2" width="150">M/C Details</th>
									<th colspan="2" width="110">Technical Details</th>
								</thead>
								<tr height="22">
									<td><strong> Program No</strong></td>
									<td ><strong><? echo $prog_no; ?></strong></td>
									<td>Job No</td>
									<td ><? echo $jobNo; ?></td>
									<td><strong>M/C No</strong></td>
									<td ><strong><? echo $machine_arr[$mc_id];?></strong></td>
									<td>Stitch Length</td>
									<td width="100" ><? echo $prog_data['s_length']; ?></td>
								</tr>
								<tr height="22">
									<td>Program Date</td>
									<td ><? echo change_date_format($prog_data['program_date']); ?></td>
								...
								<tr height="22">
									...
									<td>Finished Dia</td>
									<td ><? echo $prog_data['fabric_dia']; ?></td>
									<td rowspan="2"  valign="middle" >Lycra CM</td>
									<td  rowspan="2" ><span>Write&nbsp;</span></td>
								</tr>
								<tr height="22">
									<td>M/C Target QTY </td>
									<td a>
										<?
										$distribution_qnty = $machin_prog[$mc_id][$prog_no]['distribution_qnty'];
										echo $distribution_qnty;
										?>
									</td>
									<td>Order No </td>
									<td ><? echo $prog_data['po_number']; ?></td>
									<td> Dia Type </td>
									<td width="150" ><? echo $fabric_typee[$prog_data['width_dia_type']]; ?></td>

								</tr>
								<tr height="22">
									<td>Knit Start Date </td>
									<td ><? echo change_date_format($prog_data['start_date']); ?></td>
									<td>Buyer</td>
									<td ><? echo $buyer_arr[$prog_data['buyer_id']]; ?></td>

									<td>Fabric Type & GSM</td>
									<td colspan="3" ><? echo $prog_data['fabric_desc'].', '.$prog_data['gsm_weight']; ?></td>

								</tr>
								<tr height="22">
									<td rowspan='2'>Knit End Date</td>
									<td rowspan='2' ><? echo change_date_format($prog_data['end_date']); ?></td>
									<td>File NO.</td>
									<td><? echo ($file_no)? $file_no : "" ; ?></td>
									<td>Grey GSM</td>
									<td><span>Write&nbsp;</span></td>
									<td>Fabric Take-up</td>
									<td><span>Write&nbsp;</span></td>
								...
								<tr height="22">
									<td>Remarks</td>
									<td colspan='4'><? echo $prog_data['remarks']; ?></td>
									<td>Fabric Color</td>
									<td colspan='2'><? echo $prog_data['color_id']; ?></td>
								</tr>
							</table>
						</div>
				...
				<?
					...
					if (count($rqsn_array[$prog_no]) > 0)
					{

						?>
						<table width="930" style="margin-top:10px" border="1" rules="all" cellpadding="0" cellspacing="0" class="rpt_table">
							<thead>
								<tr ><th colspan='8'>Yarn Details</th></tr>
								<tr>
									<th width="30">SL</th>
									<th width="100">Requisition No</th>
									<th width="100">Lot No</th>
							...
							$j = 1;
							$tot_reqsn_qty = 0;
							foreach ($rqsn_array[$prog_no] as $prod_id => $data)
							{
								if ($j % 2 == 0)
									$bgcolor = "#E9F3FF";
								else
					...
					$sql_strip_data = "select a.pre_cost_fabric_cost_dtls_id,a.color_number_id, a.stripe_color, a.measurement, a.uom, b.dtls_id, b.no_of_feeder as no_of_feeder from wo_pre_stripe_color a, ppl_planning_feeder_dtls b where a.pre_cost_fabric_cost_dtls_id=b.pre_cost_id and a.color_number_id=b.color_id and a.stripe_color=b.stripe_color_id and b.dtls_id in($prog_no) and b.no_of_feeder>0 and a.status_active=1 and a.is_deleted=0  group by a.pre_cost_fabric_cost_dtls_id,a.color_number_id, a.stripe_color, a.measurement, a.uom, b.dtls_id, b.no_of_feeder ";
					$result_stripe_data = sql_select($sql_strip_data);
					$pre_cost_fabric_cost_dtls_id="";$programIDS_arr=array();
					foreach ($result_stripe_data as $row)
					{
					...
					$feeder_data_sql= sql_select("select id, knitting_source, knitting_party, subcontract_party, color_id, color_range, machine_dia, width_dia_type, machine_gg, fabric_dia, program_qnty, stitch_length, spandex_stitch_length, draft_ratio, machine_id, machine_capacity, distribution_qnty, status, start_date, end_date, program_date, feeder, remarks,attention, co_efficient, save_data, no_fo_feeder_data, location_id, advice, collar_cuff_data, grey_dia from ppl_planning_info_entry_dtls where  id in($prog_no)");
					foreach ($feeder_data_sql as $row )
					{
						$no_of_feeder_data =$row[csf('no_fo_feeder_data')];
					}
				...
				if (count($result_stripe) > 0)
				{
					...
					<table cellspacing="0" cellpadding="0" border="1" rules="all" width="930" class="rpt_table" >
						...
						foreach ($result_stripe as $row)
						{
							...
							<tr valign="middle" bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>">
								<td width="30" align="center"><? echo $i; ?></td>
								<td width="200"><p><? echo $color_library[$row[csf('color_number_id')]]; ?></p></td>
								<td width="130"><p><? echo $color_library[$row[csf('stripe_color')]]; ?></p></td>
								<td width="70" align="center"><? echo $row[csf('measurement')]; ?></td>
								<td width="50" align="center"><p><? echo $unit_of_measurement[$row[csf('uom')]]; ?></p></td>
								<td align="right" style="padding-right:10px" width="50"><? echo $no_of_feeder;?>&nbsp;</td>
								<td  align="center">&nbsp;</td>
							</tr>
							<?
				...
				<div style="margin-top:10px; width:930px;">
					<table  cellspacing="2" cellpadding="2" border="1" rules="all" width="100%" class="rpt_table" id="dataTable">
						...
						<thead height="25">
							<tr>
							   <td width="128" height="20" colspan='2'>Target Qnty/Shift</td>
								<td width="68"><span>Write&nbsp;</span></td>
								<td width="74"><span>Write&nbsp;</span></td>
								<td width="99"><span>Write&nbsp;</span></td>
								<td width="149">Target Counter/Shift</td>
								<td width="78"><span>Write&nbsp;</span></td>
								<td width="100"><span>Write&nbsp;</span></td>
								<td width="66"><span>Write&nbsp;</span></td>
								<td width="150" colspan='2'><span>Write&nbsp;</span></td>
							</tr>
				...
				<div style="margin-top:10px; width:920px">
					<table cellspacing="2" cellpadding="2" rules="all" width="100%" style="font-size:14px; font-family:'Arial Narrow'">
						<tbody>
							...
							<tr><td>* রোল মার্কিং সঠিকভাবে করতে হবে। </td></tr>
							<tr><td>* রোলে কোন প্রকার কাটা-কাটি বা ভুল লেখা চলবেনা।</td></tr>
							<tr><td style='color:red;'>* রোলের গায়ে তারিখ/ অপারেটরের নাম এবং শিফট অবশ্যই লিখতে হবে।</td></tr>
							<tr><td>&nbsp;</td></tr>
						</tbody>
					</table>
				</div>
...
if ($action == "knitting_card_print_11")
{
	...
	$jobNo=""; $poQuantity="";
	$job_data_sql=sql_select("select a.id,a.grouping, a.job_no_mst,a.po_number, sum(a.po_quantity) as poQuantity,c.style_ref_no from wo_po_break_down a, ppl_planning_entry_plan_dtls b,wo_po_details_master c where a.id=b.po_id and b.dtls_id in ($program_ids) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.job_no_mst=job_no group by a.id,a.grouping,a.job_no_mst,a.po_number,c.style_ref_no");
	$po_details= array();$job_detailsArr= array();
	foreach($job_data_sql as $row)
	{
		$jobNo		= $row[csf('job_no_mst')];
		$poQuantity	= $row[csf('poQuantity')];
		...
		$po_details[$row[csf('id')]]['po_number']= $row[csf('po_number')];
		$ref_no 	= $row[csf('grouping')];
		$job_detailsArr[$row[csf('id')]]['job_no_mst']= $row[csf('job_no_mst')];
		$job_detailsArr2[$row[csf('job_no_mst')]]['style_ref_no']= $row[csf('style_ref_no')];
	}
		...
		foreach ($dataArray as $row)
		{
			...
			if($orderNo=="")
			{
				$orderNo .= $row[csf('po_id')];
				$po_number .= $po_details[$row[csf('po_id')]]['po_number'];
				$jobNumber .= $job_detailsArr[$row[csf('po_id')]]['job_no_mst'];
				$styleRefName .= $job_detailsArr2[$job_detailsArr[$row[csf('po_id')]]['job_no_mst']]['style_ref_no'];
			}else {
				$orderNo .= ",".$row[csf('po_id')];
				$po_number .= ",".$po_details[$row[csf('po_id')]]['po_number'];
				$jobNumber .= ",".$job_detailsArr[$row[csf('po_id')]]['job_no_mst'];
			...
			$program_data_arr[$row[csf('id')]]['po_number']=$po_number;
			$program_data_arr[$row[csf('id')]]['job_number']=$jobNumber;
			$program_data_arr[$row[csf('id')]]['styleRefName']=$styleRefName;
			$program_data_arr[$row[csf('id')]]['co_efficient']=$row[csf('co_efficient')];
			$program_data_arr[$row[csf('id')]]['draft_ratio']=$row[csf('draft_ratio')];
			$program_data_arr[$row[csf('id')]]['start_date']=$row[csf('start_date')];
			$program_data_arr[$row[csf('id')]]['end_date']=$row[csf('end_date')];
		...
		foreach($ex_mc_id as $mc_id)
		{
			...
			<div style="width:930px;">
				...
				<div style="margin-top:5px; width:1270px">

					<table cellspacing="2" cellpadding="2" border="1" rules="all" width="100%" class="rpt_table" style="" id="dataTable">
					  	<thead height="25">
							<th colspan="2" width="230" id="font_size_define">Program Details</th>
							<th colspan="2" width="233" id="font_size_define">Job Details</th>
							<th colspan="2" width="233" id="font_size_define">Yarn/Fabric Details</th>
							<th colspan="2" width="233" id="font_size_define">M/C Details</th>
							<th colspan="2" width="233" id="font_size_define">Technical Details</th>
							<th></th>
						</thead>
						<tbody>
						  <tr>
						    ...
						    <td rowspan="4"  width="100" class="font_size_define" >Yarn Desc</td>
						    <td rowspan="4"  width="132" class="font_size_define" align="center"><? echo $prog_data['yarn_desc']; //echo $buyer_arr[$prog_data['buyer_id']]; ?></td>
						  ...
						  <tr>
						    <td class="font_size_define" >Program Date</td>
						    <td class="font_size_define" align="center"><? echo change_date_format($prog_data['program_date']); ?></td>
						    <td class="font_size_define">Order</td>
						    <td class="font_size_define" align="center" ><?  echo $prog_data['po_number']; //echo ($ref_no)? $ref_no : "" ; ?></td>



						    <td class="font_size_define" >Dia x Gauge</td>
						    <td class="font_size_define" align="center" ><? echo $prog_data['machine_dia']." x ".$prog_data['machine_gg']; ?></td>
						    <td class="font_size_define" >Draft Ration</td>
						    <td class="font_size_define" align="center"><? echo $prog_data['draft_ratio'];?></td>
						    <td>&nbsp;</td>
						  </tr>
						  ...
						  <tr>
						    <td class="font_size_define" >Program Qty</td>
						    <td class="font_size_define" align="center" ><? echo number_format($prog_data['prog_qty'],2);?></td>
						    <td class="font_size_define">Job</td>
						    <td class="font_size_define" ><? echo $prog_data['job_number']; ?></td>
						  ...
						  <tr>
						    ...
						    <td class="font_size_define" align="center"><? echo $prog_data['styleRefName'];//echo $reqsDataArr[$prog_no]['reqs_no']; ?></td>


						    <td class="font_size_define" >Fabric Type</td>
						    <td class="font_size_define"  align="center"><? echo $prog_data['fabric_desc']; ?></td>
						    <td class="font_size_define" >Feeder</td>
						    <td><? echo $feeder[$prog_data['feeder']]; ?></td>
						  ...
						  <tr>
						    <td class="font_size_define">Program TnA Start</td>
						    <td class="font_size_define" align="center"><? echo change_date_format($prog_data['start_date']); ?></td>
						    <td class="font_size_define">Knit TnA Start</td>
						    <td class="font_size_define" align="center"><? //echo $reqsDataArr[$prog_no]['reqs_no']; ?></td>

						    <td rowspan="2"  width="100" class="font_size_define" >Yarn Brand</td>
						    <td rowspan="2"  width="132" class="font_size_define" align="center"><? echo $prog_data['brand_name']; ?></td>
						  ...
						  <tr>
						    <td class="font_size_define">Program TnA End</td>
						    <td class="font_size_define" align="center"><? echo change_date_format($prog_data['end_date']); ?></td>
						    <td class="font_size_define">Knit TnA End</td>
						    <td class="font_size_define" align="center"><? //echo $reqsDataArr[$prog_no]['reqs_no']; ?></td>
						    <td class="font_size_define" >Color</td>
						   	<td class="font_size_define"  align="center"><? echo $prog_data['color_id']; ?></td>

						   	<td class="font_size_define" >Yarn Tension</td>
						   	<td class="font_size_define"></td>
						   	<td>&nbsp;</td>
						  ...
						  <tr>
						    <td class="font_size_define">Knitting Coefficient</td>
						    <td class="font_size_define" align="center"><? echo $prog_data['co_efficient']; ?></td>
						    <td class="font_size_define">Knit Party</td>
						    <td class="font_size_define" ><? echo $prog_data['knit_factory']; ?></td>
						    ...
						    <td class="font_size_define" align="center" ><? echo $prog_data['lot']; ?></td>
						    <td class="font_size_define" >Counter</td>
						   	<td class="font_size_define"></td>
						   	<td class="font_size_define" >Spreder Dia</td>
						   	<td class="font_size_define"></td>
						   	<td>&nbsp;</td>
						  </tr>
						  ...
						  <tr>
	                            ...
	                            <td class="font_size_define">Remarks</td>
	                            <td colspan="3" class="font_size_define" align="center"><? echo $prog_data['remarks']; ?></td>


	                           <td class="font_size_define" >M/C Target QTY</td>
	                           <td class="font_size_define"></td>
	                           <td class="font_size_define" >Fabric Take-up</td>
	                           <td class="font_size_define"></td>
	                           <td>&nbsp;</td>
                          </tr>
						  ...
						  <tr style="height:30px;">
						    <td style="font-size:16px;" width="100"  align="center"><b>Date</b></td>
						    <td style="font-size:16px;" width="132"  align="center" ><b>Shift</b></td>
						    <td style="font-size:16px;" width="100"  align="center"><b>Order Qty</b></td>
						    <td style="font-size:16px;" width="132"  align="center"><b>No. Or Roll</b></td>

						    <td style="font-size:16px;" width="100"  align="center"><b>Production qty</b></td>
						    <td style="font-size:16px;" width="132"  align="center"><b>Reject qty</b></td>


						    <td style="font-size:16px;" width="100"  align="center"><b>Balance Qty</b></td>
						    <td style="font-size:16px;" width="132"  align="center" ><b>Operator Id</b></td>
						    <td style="font-size:16px;" width="130"  align="center" ><b>Name</b></td>
						    <td style="font-size:16px;" width="100"  align="center"><b>Signature</b></td>
						    <td style="font-size:16px;" width="132"  align="center" ><b>Remarks</b></td>
						  </tr>
						  <? for ($i=1; $i <= 9; $i++)
						   {
						  	?>
...
if ($action == "knitting_card_print_12")
{
	echo load_html_head_contents("Knitting Card Info", "../../", 1, 1, '', '', '');
	extract($_REQUEST);
	$program_ids =  $data;
	...
	$job_data_sql=sql_select("SELECT a.id,a.grouping, a.job_no_mst,a.po_number, sum(a.po_quantity) as poQuantity, b.booking_no, c.style_ref_no,a.pub_shipment_date,b.dtls_id from wo_po_break_down a, ppl_planning_entry_plan_dtls b, wo_po_details_master c where a.id=b.po_id and b.dtls_id in ($program_ids) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.job_no_mst=job_no group by a.id,a.grouping,a.job_no_mst,a.po_number,b.booking_no,c.style_ref_no,a.pub_shipment_date,b.dtls_id");
	//echo "SELECT a.id,a.grouping, a.job_no_mst,a.po_number, sum(a.po_quantity) as poQuantity,b.booking_no,c.style_ref_no from wo_po_break_down a, ppl_planning_entry_plan_dtls b,wo_po_details_master c where a.id=b.po_id and b.dtls_id in ($program_ids) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.job_no_mst=job_no group by a.id,a.grouping,a.job_no_mst,a.po_number,b.booking_no,c.style_ref_no";
	$po_details= array();
	foreach($job_data_sql as $row)
	{
		...
		$program_stl_data_arr[$row[csf('dtls_id')]]['style_ref_no'] = $row[csf('style_ref_no')];
	}
	//echo $order_nature.'XXXXXXX';
	/* echo "<pre>";
	print_r($program_stl_data_arr);
	echo "</pre>"; */
 ...
 */
		...
		if($reqsDataArr[$row[csf('id')]]['prod_id'] != '')
		{
			...
			foreach ($prod_id as $val)
			{
				...
				//$yarn_dtls .= $product_details_arr[$val]['desc'] . ",".$product_details_arr[$val]['lot'] . ",".$brand_arr[$product_details_arr[$val]['brand_name']].'('.$product_details_arr[$val]['lot'].')' . "<br>";
				/* $yarn_dtls .= $yarn_details_arr[$val]['yarn_count'] . ", ".$yarn_details_arr[$val]['composition'] . ", ".$yarn_details_arr[$val]['yarn_type'].", ".$yarn_details_arr[$val]['color'].', '.$yarn_details_arr[$val]['brand'].", ".$yarn_details_arr[$val]['lot'] . "<br>"; */
				$yarn_dtls .= $yarn_details_arr[$val]['yarn_count'];
			}
		...
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['po_number'] .= $po_number.',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['prog_no'] .= $row[csf('id')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['co_efficient'] .= $row[csf('co_efficient')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['start_date'] .= $row[csf('start_date')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['end_date'] .= $row[csf('end_date')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['machine_dia'] .= $row[csf('machine_dia')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['machine_gg'] .= $row[csf('machine_gg')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['color_id'] .= $color_name.',';

		if ($check_program_no[$row[csf('id')]]=="")
		{
			$check_program_no[$row[csf('id')]]=$row[csf('id')];
			$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['prog_qty'] += $row[csf('program_qnty')];
		}

		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['s_length'] .= $row[csf('stitch_length')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['program_date'] .= $row[csf('program_date')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['booking_qty'] += $booking_qnty_arr[$row[csf('booking_no')]];
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['remarks'] .= $row[csf('remarks')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['yarn_dtls'] .= $yarn_dtls.',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['yarn_desc'] .= $yarn_desc.',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['lot'] .= $lot_no.',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['brand_name'] .= $brand_name.',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['machine_name'] .= $machine_name.',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['sub_party'] .= $supplier_details[$row[csf('subcontract_party')]].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['booking_no'] .= $row[csf('booking_no')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['company_id'] = $row[csf('company_id')];
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['spandex_stitch_length'] .= $row[csf('spandex_stitch_length')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['feeder'] .= $row[csf('feeder')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['advice'] .= $row[csf('advice')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['knit_id'] .= $row[csf('mst_id')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['width_dia_type'] .= $row[csf('width_dia_type')].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['int_ref'] .= $po_details[$row[csf('po_id')]]['int_ref'].',';
		$program_data_arr[$knitting_factory][$row[csf('buyer_id')]][$styles][$row[csf('fabric_dia')]][$row[csf('fabric_desc')]][$row[csf('gsm_weight')]]['pub_shipment_date'] .= change_date_format($po_details[$row[csf('po_id')]]['pub_shipment_date']).',';
	...
	{
		// program array loop
		foreach($program_data_arr as $party_id=>$party_data)
		{
			foreach($party_data as $buyer_id=>$buyer_data)
			{
				foreach($buyer_data as $style_id=>$style_data)
				{
					foreach($style_data as $f_dia_id=>$f_dia_data)
					{
						foreach($f_dia_data as $f_desc=>$f_desc_data)
						{
							foreach($f_desc_data as $f_gsm=>$prog_data)
							{
								//var_dump($prog_data);

								$prog_no = implode(",",array_filter(array_unique(explode(",", chop($prog_data['prog_no'])))));
								$prog_no_barcode = implode("-",array_filter(array_unique(explode(",", chop($prog_data['prog_no'])))));
								$booking_no = implode(",",array_filter(array_unique(explode(",", chop($prog_data['booking_no'])))));
								$program_date = implode(",",array_filter(array_unique(explode(",", chop($prog_data['program_date'])))));
								...
								$machine_dia = implode(",",array_filter(array_unique(explode(",", chop($prog_data['machine_dia'])))));
								$machine_gg = implode(",",array_filter(array_unique(explode(",", chop($prog_data['machine_gg'])))));
								$width_dia_type = implode(",",array_filter(array_unique(explode(",", chop($prog_data['width_dia_type'])))));
								$yarn_dtls = implode(",",array_filter(array_unique(explode(",", chop($prog_data['yarn_dtls'])))));
								$lot = implode(",",array_filter(array_unique(explode(",", chop($prog_data['lot'])))));
								$brand_name = implode(",",array_filter(array_unique(explode(",", chop($prog_data['brand_name'])))));
								$s_length = implode(",",array_filter(array_unique(explode(",", chop($prog_data['s_length'])))));
								$color_id = implode(",",array_filter(array_unique(explode(",", chop($prog_data['color_id'])))));
								?>
								<style type="text/css">
									.page_break	{
								...
								<div style="width:800px; margin-left: 10px;">
									...
									<table width="100%" cellspacing="2" cellpadding="2" border="1" rules="all" class="rpt_table">
										...
										<tr>
											...
											<td width="350" class="font_size_define"><b><? echo $prog_no; ?></b></td>
											<td width="105" class="font_size_define" style="border-left:hidden;"><b>Date : <? echo date('d-m-Y', strtotime($program_date)); ?></b></td>
										</tr>
										<tr>
											<td class="font_size_define"><b>KNIT PARTY</b></td>
											<td colspan="2" class="font_size_define"><? echo $party_id; ?></td>
										</tr>
										<tr>
											<td class="font_size_define"><b>BUYER</b></td>
											<td colspan="2" class="font_size_define"><? echo $buyer_arr[$buyer_id]; ?></td>
										</tr>
										<tr>
											<td class="font_size_define"><b>BOOKING NO</b></td>
											<td colspan="2" class="font_size_define"><? echo $booking_no; ?></td>
										</tr>
										<tr>
											<td class="font_size_define"><b>Style No</b></td>
											<td colspan="2" class="font_size_define"><? echo $style_id; ?></td>
										</tr>
										<tr>
											<td class="font_size_define"><b>Fire Date/<br>Shipment Date</b></td>
											<td colspan="2" class="font_size_define"><? echo $pub_shipment_date; ?></td>
										</tr>
										<tr>
											<td class="font_size_define"><b>Order No</b></td>
											<td colspan="2" class="font_size_define"><? echo $po_number; ?></td>
										</tr>
										<tr>
											<td colspan="3">&nbsp;</td>
										</tr>
										...
										<tr>
											<td class="font_size_define"><b>M/C DIA & GG</b></td>
											<td colspan="2" class="font_size_define"><? echo $machine_dia." x ".$machine_gg; ?></td>
										</tr>
										<tr>
											<td class="font_size_define"><b>F. DIA</b></td>
											<td colspan="2" class="font_size_define"><? echo $f_dia_id." "."[".$fabric_typee[$width_dia_type]."]"; ?></td>
										</tr>
										<tr>
											<td class="font_size_define"><b>F. TYPE</b></td>
											<td colspan="2" class="font_size_define"><? echo $f_desc; ?></td>
										</tr>
										<tr>
											<td class="font_size_define"><b>GSM</b></td>
											<td colspan="2" class="font_size_define"><? echo $f_gsm; ?></td>
										</tr>
										<tr>
											<td class="font_size_define"><b>COUNT</b></td>
											<td colspan="2" class="font_size_define"><? echo $yarn_dtls; ?></td>
										</tr>
										<tr>
											<td class="font_size_define"><b>LOT</b></td>
											<td colspan="2" class="font_size_define"><? echo $lot; ?></td>
										</tr>
										<tr>
											<td class="font_size_define"><b>BRAND</b></td>
											<td colspan="2" class="font_size_define"><? echo $brand_name; ?></td>
										</tr>
										<tr>
											<td colspan="3">&nbsp;</td>
										</tr>
										<tr>
											<td class="font_size_define"><b>SL</b></td>
											<td colspan="2" class="font_size_define"><? echo $s_length; ?></td>
										</tr>
										<tr>
											<td class="font_size_define"><b>COLOR</b></td>
											<td colspan="2" class="font_size_define"><? echo $color_id; ?></td>
										</tr>
										<tr>
											<td class="font_size_define"><b>P. QTY. (Kg)</b></td>
											<td colspan="2" class="font_size_define"><? echo number_format($prog_data['prog_qty'],2);?></td>
								...
								<div style="margin-top:10px; margin-left: 10px; width:800px;">
										...
										<div style="margin-top:10px; margin-left: 10px; width:800px">
											...
											<? echo signature_table(119, $prog_data['company_id'], "780px","","20"); ?>
										</div>
									...
									<div>
										...
										<script>
											...
											generateBarcode('<? echo $prog_no_barcode; ?>');
										</script>
									</div>
...
if ($action == "knitting_card_print_12_04122022")
{
	echo load_html_head_contents("Knitting Card Info", "../../", 1, 1, '', '', '');
	extract($_REQUEST);
	$program_ids =  $data;
	...
	$jobNo="";
	$poQuantity="";
	$job_data_sql=sql_select("SELECT a.id,a.grouping, a.job_no_mst,a.po_number, sum(a.po_quantity) as poQuantity, b.booking_no, c.style_ref_no,a.pub_shipment_date from wo_po_break_down a, ppl_planning_entry_plan_dtls b, wo_po_details_master c where a.id=b.po_id and b.dtls_id in ($program_ids) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.job_no_mst=job_no group by a.id,a.grouping,a.job_no_mst,a.po_number,b.booking_no,c.style_ref_no,a.pub_shipment_date");
	//echo "SELECT a.id,a.grouping, a.job_no_mst,a.po_number, sum(a.po_quantity) as poQuantity,b.booking_no,c.style_ref_no from wo_po_break_down a, ppl_planning_entry_plan_dtls b,wo_po_details_master c where a.id=b.po_id and b.dtls_id in ($program_ids) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.job_no_mst=job_no group by a.id,a.grouping,a.job_no_mst,a.po_number,b.booking_no,c.style_ref_no";
	$po_details= array();
	foreach($job_data_sql as $row)
	{
	...
	foreach($ex_mc_id as $mc_id)
	{
		...
		<div style="width:800px;">
            ...
            <table width="100%" cellspacing="2" cellpadding="2" border="1" rules="all" class="rpt_table">
                ...
                <tr>
                	<td class="font_size_define"><b>Style No</b></td>
                	<td colspan="2" class="font_size_define"><? echo $style; ?></td>
                </tr>
				<tr>
                	<td class="font_size_define"><b>Fire Date/<br>Shipment Date</b></td>
                	<td colspan="2" class="font_size_define"><? echo  change_date_format($prog_data['pub_shipment_date']); ?></td>
                </tr>
				<tr>
                	<td class="font_size_define"><b>Order No</b></td>
                	<td colspan="2" class="font_size_define"><? echo $prog_data['po_number']; ?></td>
                </tr>
                <tr>
                	<td colspan="3">&nbsp;</td>
                </tr>
