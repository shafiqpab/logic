						...
						<table cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
							...
							<tr>
								<td>
									...
									echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company where status_active=1  order by company_name","id,company_name",1, "-- Select Company --", $companyID, "",1);
									?>
								</td>
...
if ($action=="create_booking_search_list_view")
{
	...
	$sql= "SELECT booking_no_prefix_num, booking_no, booking_date, company_id, buyer_id, job_no, po_break_down_id, item_category, fabric_source, supplier_id, is_approved, ready_to_approved,pay_mode from wo_booking_mst where $company $buyer $booking_no $booking_date and booking_type in(1,4) and is_short in(1,2) and status_active=1 and is_deleted=0 order by id Desc";

	$sql_smn= "SELECT booking_no_prefix_num, booking_no, booking_date, company_id, buyer_id, job_no, po_break_down_id, item_category, fabric_source, supplier_id, is_approved, ready_to_approved,pay_mode from wo_non_ord_samp_booking_mst where $company $buyer $booking_no $booking_date and booking_type in(4) and is_short is null and status_active=1 and is_deleted=0 order by id Desc";
	...
	<div style="width:850px; max-height:270px;overflow-y:scroll;" >
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="845" class="rpt_table" id="tbl_list_search" style="float:left; ">
			...
			$sqlsmnQuery=sql_select($sql_smn);
			$mainDataArr = array_merge($sqlQuery,$sqlsmnQuery);

			$i=1;
			foreach( $mainDataArr as $row )
			{
				if ($i%2==0)
					$bgcolor="#E9F3FF";
				else
...
if ($action == "fso_no_search_popup")
{
	...
	function search_populate(str)
	{
		if (str == 1)
		{
			document.getElementById('based_on_th_up').innerHTML = "FSO No";
			document.getElementById('based_on_th_up1').innerHTML = "FSO Date";
			$('#txt_booking_fso_no').val("");
		}
		else if (str == 2)
		{
			document.getElementById('based_on_th_up').innerHTML = "Booking No";
			document.getElementById('based_on_th_up1').innerHTML = "Booking Date";
			$('#txt_booking_fso_no').val("");
		}
	...
	<div align="center" style="width:100%;" >
		...
		<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
			<table width="845" cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center" rules="all">
				<tr>
					<td align="center" width="100%">
						<table cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
							<thead>
								...
								<th width="80" id="based_on_th_up">FSO No</th>
								<th width="180" id="based_on_th_up1">FSO Date</th>
								<th>&nbsp;</th>
							</thead>
							<tr>
								<td>
									<input type="hidden" id="selected_fso">

									<?
									echo create_drop_down( "cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1  order by company_name","id,company_name",1, "-- Select Company --", $companyID, "",1);
									?>
								</td>
								<?
								if($buyerID !=""){ $buyer_cond = " buy.id='$buyerID' and";}
								?>
								<td id="buyer_td"><? echo create_drop_down( "cbo_buyer_name", 140, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where $buyer_cond buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$companyID' and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,90))  group by buy.id,buy.buyer_name order by buyer_name","id,buyer_name", 1, "-- All Buyer --",0,"",0 ); ?></td>
								<td>
									<?
									 $search_type_arr = array(1 => "FSO No", 2 => "Booking No");
									 $fnc_name = "search_populate(this.value)";
									 echo create_drop_down("search_type", 140, $search_type_arr, "", 0, "-Select-", 0, $fnc_name, 0, "", "", "", "", "");
									?>
								</td>
								<td>
									<input type="text" id="txt_booking_fso_no" name="txt_booking_fso_no" class="text_boxes_numeric" style="width:75px" />
								</td>
								<td>
									<input name="txt_date_from" id="txt_date_from" class="datepicker" style="width:70px">
									<input name="txt_date_to" id="txt_date_to" class="datepicker" style="width:70px">
								...
								<td align="center">
									<input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_booking_fso_no').value+'_'+document.getElementById('search_type').value, 'create_fso_search_list_view', 'search_div', 'plan_wise_yarn_issue_monitoring_report_controller','setFilterGrid(\'tbl_list_search\',-1)')" style="width:100px;" />
								</td>
							</tr>
						</table>
					</td>
...
{
	$data=explode('_',$data);
	if ($data[0]!=0) $company=" and b.company_id='$data[0]'"; else { echo "Please Select Company First."; die; }
	if ($data[1]!=0) $buyer=" and b.customer_buyer='$data[1]'"; else $buyer="";
	if($data[5]==1)
	{
		if ($data[4]!=0) $job_no=" and b.job_no_prefix_num='$data[4]'"; else $job_no='';
		if($db_type==0)
		{
			if ($data[2]!="" &&  $data[3]!="") $fso_date  = " and b.insert_date  between '".change_date_format($data[2], "yyyy-mm-dd", "-")."' and '".change_date_format($data[3], "yyyy-mm-dd", "-")."'"; else $fso_date ="";
		}
		if($db_type==2)
		{
			if ($data[2]!="" &&  $data[3]!="") $fso_date  = " and b.insert_date  between '".change_date_format($data[2], "yyyy-mm-dd", "-",1)."' and '".change_date_format($data[3], "yyyy-mm-dd", "-",1)."'"; else $fso_date ="";
		}
	}
	else
	{
		if ($data[4]!=0) $booking_no=" and a.booking_no_prefix_num='$data[4]'"; else $booking_no='';
		if($db_type==0)
		{
			if ($data[2]!="" &&  $data[3]!="") $fso_date  = " and a.insert_date  between '".change_date_format($data[2], "yyyy-mm-dd", "-")."' and '".change_date_format($data[3], "yyyy-mm-dd", "-")."'"; else $fso_date ="";
		}
		if($db_type==2)
		{
			if ($data[2]!="" &&  $data[3]!="") $fso_date  = " and a.insert_date  between '".change_date_format($data[2], "yyyy-mm-dd", "-",1)."' and '".change_date_format($data[3], "yyyy-mm-dd", "-",1)."'"; else $fso_date ="";
		}
	}



	$sql= "SELECT b.booking_date,b.company_id,a.buyer_id,a.job_no,b.id,b.job_no AS fso_no,b.insert_date AS fso_date, b.sales_booking_no from fabric_sales_order_mst b left join wo_booking_mst a on b.booking_id= a.id and a.status_active=1
	and a.is_deleted=0 AND a.booking_type IN (1, 4) AND a.is_short = 2  where b.status_active=1 and b.is_deleted=0  and (b.booking_without_order =0 or b.booking_without_order is null) $company $buyer $booking_no $job_no $fso_date  order by b.id Desc";
	//echo $sql;
	$smn_sql= "SELECT b.booking_date,b.company_id,a.buyer_id,a.job_no,b.id,b.job_no AS fso_no,b.insert_date AS fso_date, b.sales_booking_no from fabric_sales_order_mst b, wo_non_ord_samp_booking_mst a where b.booking_id= a.id and a.status_active=1
	and a.is_deleted=0 AND a.booking_type IN (4) AND a.is_short is null and b.status_active=1 and b.is_deleted=0 and b.booking_without_order =1 $company $buyer $booking_no $job_no $fso_date order by b.id Desc";
	//echo $smn_sql;
	...
	<div style="width:850px; max-height:270px;overflow-y:scroll;" >
		<table cellspacing="0" cellpadding="0" border="1" rules="all" width="845" class="rpt_table" id="table_body" style="float:left; ">
			<?

			$sqlQuery=sql_select($sql);
			$smnsqlQuery=sql_select($smn_sql);
			$mainDataArr = array_merge($sqlQuery,$smnsqlQuery);
			...
			foreach( $mainDataArr as $row )
			{
				...
				<tr bgcolor="<? echo $bgcolor; ?>" onclick="js_set_value(<? echo $i; ?>)" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>">
					<td width="30"><?php echo $i; ?>
						...
						<input type="hidden" name="txt_individual" id="txt_individual<?php echo $i ?>" value="<? echo $row[csf("fso_no")]; ?>"/>
					</td>
					<td width="110"><?php echo $row[csf("fso_no")]; ?></td>
					<td width="110"><?php echo $row[csf("sales_booking_no")]; ?></td>
					<td width="70"><?php echo change_date_format($row[csf("fso_date")]); ?></td>
					<td width="110"><?php echo $comp[$row[csf("company_id")]]; ?></td>
					<td width="110"><?php echo $buyer_arr[$row[csf("buyer_id")]]; ?></td>
					<td width="110"><?php echo $row[csf("job_no")]; ?></td>
				</tr>
...
if ($action == "report_generate")
{
	...
	$booking_date_cond = "";$booking_date_cond2 = "";
	if (str_replace("'", "", trim($txt_date_from)) != "" && str_replace("'", "", trim($txt_date_to)) != "") {
		$booking_date_cond = " and a.booking_date between " . trim($txt_date_from) . " and " . trim($txt_date_to) . "";
		$booking_date_cond2 = " and f.booking_date between " . trim($txt_date_from) . " and " . trim($txt_date_to) . "";
	}
	...
	if ($report_type == 1)
	{
		...
		$con = connect();
		$r_id6=execute_query("delete from GBL_TEMP_ENGINE where user_id=$user_name and entry_form in (35)");
		...
		fnc_tempengine("GBL_TEMP_ENGINE", $user_name, 35, 1,$all_po_id_arr, $empty_arr);
		oci_commit($con);
		//die;
       ...
       /* $progQnty_sql = "SELECT a.booking_no,a.buyer_id,
		...
		$progQnty_sql = "SELECT a.booking_no,a.buyer_id, b.program_qnty as program_qnty, b.id program_id from ppl_planning_info_entry_mst d,ppl_planning_info_entry_dtls b,ppl_planning_entry_plan_dtls a,wo_booking_dtls c , tmp_booking_id e where d.id=b.mst_id and a.dtls_id=b.id and a.mst_id=b.mst_id and a.booking_no = c.booking_no  and a.booking_no = e.booking_no  and e.userid=$user_name $company_cond $buyer_cond and a.status_active=1 and a.is_deleted=0 and a.is_sales!=1 and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0 ";
		...
		{
			if($programIdQntyChk[$row[csf("program_id")]] == "")
			{
				$programIdQntyChk[$row[csf("program_id")]] = $row[csf("program_id")];
		...
		/*$product_sql = "SELECT a.booking_id, c.quantity as knitting_qnty,c.returnable_qnty as return_qnty, c.id, d.booking_no , d.job_no,a.buyer_id
			...
			$product_sql = "SELECT a.booking_id, c.quantity as knitting_qnty,c.returnable_qnty as return_qnty, c.id, d.booking_no , d.job_no,a.buyer_id
			from inv_receive_master a, pro_grey_prod_entry_dtls b,order_wise_pro_details c , wo_booking_dtls d,wo_booking_mst f,tmp_booking_id e
			where a.id=b.mst_id and c.dtls_id=b.id $company_cond $buyer_cond $booking_date_cond2
			and c.po_breakdown_id = d.po_break_down_id and d.booking_mst_id=f.id and d.booking_no=e.booking_no and e.userid=$user_name
			and a.item_category=13 and a.entry_form=2 and c.entry_form=2
			and a.receive_basis=2 and b.status_active=1 and b.is_deleted=0 and c.status_active  = 1 and c.is_deleted = 0
			and a.status_active = 1 and a.is_deleted = 0 ";
		...
		$sql_issue = "SELECT (b.quantity) as issue_qnty, e.buyer_name, d.requisition_no
		from order_wise_pro_details b, inv_transaction d, wo_po_break_down c, wo_po_details_master e, GBL_TEMP_ENGINE f
		where d.transaction_type=2 and d.item_category=1 and d.id=b.trans_id and b.po_breakdown_id = c.id and c.job_id = e.id and b.po_breakdown_id=f.po_id and c.id=f.ref_val  and f.user_id=$user_name and f.entry_form=35 and b.trans_type=2 and b.entry_form=3 and b.status_active=1 and d.receive_basis = 3 and b.is_deleted=0 and d.status_active= 1  and d.is_deleted= 0 and b.issue_purpose not in (2,8) and d.company_id=$companyID $buyer_cond4";
		...
		$sql_return = "SELECT (b.quantity) as returned_qnty, b.reject_qty as cons_reject_qnty, e.buyer_name, a.booking_no as requ_no from inv_receive_master a, order_wise_pro_details b, inv_transaction d,wo_po_break_down c, wo_po_details_master e, GBL_TEMP_ENGINE f where a.id = d.mst_id  and d.id=b.trans_id and b.po_breakdown_id = c.id  and c.job_id = e.id and b.po_breakdown_id=f.ref_val  and c.id=f.ref_val and f.user_id=$user_name and f.entry_form=35 and a.entry_form=9 and a.item_category =1 and a.receive_basis = 3 and d.transaction_type=4 and d.item_category=1 and b.trans_type=4 and b.entry_form=9 and b.status_active=1 and b.is_deleted=0
		and a.status_active=1 and a.is_deleted=0
		and d.status_active=1 and d.is_deleted=0 and b.issue_purpose!=2 $company_cond $buyer_cond4";
...
if ($action == "report_generate_sales")
{
	...
	$hiden_fso_id =str_replace("'","",$hiden_fso_id);
	...
	if($companyID>0) $company_cond="and a.company_id=$companyID"; else $company_cond="";
	if($companyID>0) $company_cond_2="and a.company_name=$companyID"; else $company_cond_2="";
	if($companyID>0) $company_cond_3="and d.company_id=$companyID"; else $company_cond_3="";
	...
	if($buyerId>0)
	{
		...
		$buyer_cond3="and c.buyer_id = ".$buyerId."";
		$buyer_cond4="and e.buyer_name = ".$buyerId."";
		$buyer_cond5="and a.customer_buyer = ".$buyerId."";
		$buyer_cond6="and b.customer_buyer = ".$buyerId."";
		$buyer_cond7="and c.customer_buyer = ".$buyerId."";
	}
	else
	{
		$buyer_cond="";
	...
	if (str_replace("'", "", trim($txt_job_no)) != "")
	{
		...
		$job_nos = chop($job_nos,",");
		$job_cond .= " and b.job_no in ( $job_nos )" ;
		$po_job_cond .= " and a.po_job_no in ( $job_nos )" ;
	...
	if ($hiden_fso_id!="")
	{
		$fsoId_cond="and d.id in($hiden_fso_id)";
		$fsoId_cond1="and a.id in($hiden_fso_id)";
	}
	else
	{
		$fsoId_cond="";
	...
	{
		$booking_cond = " and a.booking_no = $txt_booking_no";
		$booking_cond1 = " and a.sales_booking_no = $txt_booking_no";
	}
	...
	{
		$booking_type_cond =" and a.booking_type=4 and a.is_short is null";
	}
	else
	{
		$booking_type_cond = '';
	...
	if($txt_date_from != "" && $txt_date_to != "")
	{
		if($search_type == 1)
		{
			if($db_type==0)
			{
				if($txt_date_from!="" && $txt_date_to!="") $date_cond="and d.insert_date between '".change_date_format($txt_date_from,"yyyy-mm-dd")."  00:00:01' and '".change_date_format($txt_date_to,"yyyy-mm-dd")." 23:59:59' "; else $date_cond="";

				if($txt_date_from!="" && $txt_date_to!="") $date_cond1="and a.insert_date between '".change_date_format($txt_date_from,"yyyy-mm-dd")."  00:00:01' and '".change_date_format($txt_date_to,"yyyy-mm-dd")." 23:59:59' "; else $date_cond1="";

			}else
			{
				if($txt_date_from!="" && $txt_date_to!="") $date_cond="and d.insert_date between '".date("j-M-Y",strtotime($txt_date_from))."  01:00:01 AM' and '".date("j-M-Y",strtotime($txt_date_to))."  11:59:59 PM' "; else $date_cond="";

				if($txt_date_from!="" && $txt_date_to!="") $date_cond1="and a.insert_date between '".date("j-M-Y",strtotime($txt_date_from))."  01:00:01 AM' and '".date("j-M-Y",strtotime($txt_date_to))."  11:59:59 PM' "; else $date_cond1="";
			}
		}
		else
		{
	...
	if($txt_program_no != '')
	{
		...
		$booking_cond = " and a.booking_no in('".implode("','",$bookingNoArr)."')";
		$booking_cond1 = " and a.sales_booking_no in('".implode("','",$bookingNoArr)."')";
	}
	...
	$print_report3 = return_field_value("format_id", "lib_report_template", "template_name=" . $companyID . "  and module_id=2 and report_id in(3) and is_deleted=0 and status_active=1");
	$format_ids3 = explode(",", $print_report3);
	$print_report4 = return_field_value("format_id", "lib_report_template", "template_name=" . $companyID . "  and module_id=2 and report_id in(4) and is_deleted=0 and status_active=1");
	$format_ids4 = explode(",", $print_report4);
	//end for Print Button Permission
		$con = connect();
		$r_id111=execute_query("DELETE FROM TMP_BOOKING_ID WHERE USERID=$user_name ");
		$r_id222=execute_query("DELETE FROM TMP_JOB_NO WHERE USERID=$user_name ");
		...
		{
			$main_sql_no = "SELECT a.id as sales_id, a.company_id, a.buyer_id as customer_buyer, a.job_no as sales_order_no,a.sales_booking_no as booking_no, null as job_no,a.within_group
			from fabric_sales_order_mst a
			where a.within_group=2 and a.status_active=1 and a.is_deleted=0 and a.booking_without_order is null $po_job_cond $date_cond1 $booking_cond1 $company_cond $buyer_cond5 $fsoId_cond1";
			//echo $main_sql_no;die;

			$result_no = sql_select($main_sql_no);
			$data_summury = array();
			$data_array = array();
			$fsoIdChk = array();
			foreach($result_no as $row)
			{
				$data_summury[$row[csf("customer_buyer")]]["booking_no"] .= "'".$row[csf("booking_no")]."',";
				$data_array[$row[csf("job_no")]][$row[csf("booking_no")]]['po_id'] .= $row[csf("sales_id")].",";
				$data_array[$row[csf("job_no")]][$row[csf("booking_no")]]['customer_buyer'] = $row[csf("customer_buyer")];
				$data_array[$row[csf("job_no")]][$row[csf("booking_no")]]['company_id'] = $row[csf("company_id")];
				...
				$data_array[$row[csf("job_no")]][$row[csf("booking_no")]]['within_group'] = $row[csf("within_group")];

				if($fsoIdChk[$row[csf('sales_id')]] == "")
				{
					$fsoIdChk[$row[csf('sales_id')]] = $row[csf('sales_id')];
		...
		if($txt_job_no == "")
		{
			$main_smn_sql = "SELECT a.id, a.company_id, a.booking_no, a.booking_type, a.is_approved, a.is_short, a.buyer_id as customer_buyer, null as job_no, a.po_break_down_id, null as grouping, null as po_number, b.grey_fabric as qnty, d.id as sales_id, d.job_no as sales_order_no, d.buyer_id, a.ENTRY_FORM_ID as entry_form
			from wo_non_ord_samp_booking_mst a, wo_non_ord_samp_booking_dtls b, fabric_sales_order_mst d
			where a.id = b.booking_mst_id and  a.id=d.booking_id and a.item_category=2
			and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.booking_without_order=1 and a.booking_type in(4) and a.is_short is null
			and b.grey_fabric>0  $booking_cond $date_cond $company_cond_3 $buyer_cond $fsoId_cond $booking_type_cond
			order by  a.job_no,a.buyer_id,a.booking_no";
			//echo $main_smn_sql;die;
			$smn_result = sql_select($main_smn_sql);
		}
		

		$main_sql = "SELECT a.id, a.company_id, a.booking_no, a.booking_type, a.is_approved, a.is_short, a.buyer_id as customer_buyer, b.job_no, b.po_break_down_id, c.grouping, c.po_number, b.grey_fab_qnty as qnty, d.id as sales_id, d.job_no as sales_order_no, d.buyer_id, a.entry_form
		from wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c, fabric_sales_order_mst d
		where a.id = b.booking_mst_id and b.po_break_down_id = c.id and a.id=d.booking_id and a.item_category=2 and a.fabric_source in(1,2)
		and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.booking_without_order=0 and a.booking_type in(1,4) and a.is_short in(1,2)
		and b.grey_fab_qnty>0 $job_cond $booking_cond $date_cond $company_cond_3 $internalRefNo_cond $buyer_cond $fsoId_cond $booking_type_cond
		order by  a.job_no,a.buyer_id,a.booking_no";// a.id=b.booking_mst_id
		//echo $main_sql;die;
		$result = sql_select($main_sql);
		if(!empty($smn_result))
		{
			$main_result = array_merge($result,$smn_result);
		}
		else
		{
			$main_result = $result;
		...
		foreach($main_result as $row)
		{
			...
			$data_summury[$row[csf("customer_buyer")]]["qnty"] += $row[csf("qnty")];
			$data_summury[$row[csf("customer_buyer")]]["job_no"] .= "'".$row[csf("job_no")]."',";
			$data_summury[$row[csf("customer_buyer")]]["booking_no"] .= "'".$row[csf("booking_no")]."',";
			$data_summury[$row[csf("customer_buyer")]]["sales_order_no"] .= "'".$row[csf("sales_order_no")]."',";
			...
			$data_array[$row[csf("job_no")]][$row[csf("booking_no")]]['sales_id'] = $row[csf("sales_id")];
			$data_array[$row[csf("job_no")]][$row[csf("booking_no")]]['sales_order_no'] = $row[csf("sales_order_no")];
			$data_array[$row[csf("job_no")]][$row[csf("booking_no")]]['booking_id'] = $row[csf("id")];
			if($row[csf("entry_form")] == 118) $data_array[$row[csf("job_no")]][$row[csf("booking_no")]]['REPORT_ID'] = 1;
			else if($row[csf("entry_form")] == 108) $data_array[$row[csf("job_no")]][$row[csf("booking_no")]]['REPORT_ID'] = 35;
			else if($row[csf("entry_form")] == 88) $data_array[$row[csf("job_no")]][$row[csf("booking_no")]]['REPORT_ID'] = 2;
			else if($row[csf("entry_form")] == 140) $data_array[$row[csf("job_no")]][$row[csf("booking_no")]]['REPORT_ID'] = 90;
			else if($row[csf("entry_form")] == '') $data_array[$row[csf("job_no")]][$row[csf("booking_no")]]['REPORT_ID'] = 3;
			$data_array[$row[csf("job_no")]][$row[csf("booking_no")]]['entry_form'] = $row[csf("entry_form")];
			...
			else if($row[csf("booking_type")] ==4  && $row[csf("is_short")] ==2)
			{
				$booking_type_arr[$row[csf("booking_no")]] = "Sample<br>[With Order]";
			}
			else if($row[csf("booking_type")] ==4  && $row[csf("is_short")] == null)
			{
				$booking_type_arr[$row[csf("booking_no")]] = "Sample<br>[With Out Order]";
			}
		}
		unset($main_result);
		...
		//die;
		$alloc_sql_no ="SELECT a.job_no, a.booking_no, a.qnty, a.po_break_down_id,b.buyer_id
		from inv_material_allocation_mst a,fabric_sales_order_mst b, tmp_po_id c
		where a.po_break_down_id=cast(b.id as varchar2(4000)) and  b.within_group=2 $buyer_cond6 and b.company_id=$companyID and b.id=c.po_id and c.user_id=$user_name and a.is_dyied_yarn in(0,1) and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and a.is_sales=1 and b.booking_without_order is null";
		//echo $alloc_sql_no; die;
		...
		// $allocation_sql ="SELECT a.job_no, a.booking_no, a.qnty, a.po_break_down_id,b.buyer_id
		// from inv_material_allocation_mst a,wo_booking_mst b, tmp_booking_id c, fabric_sales_order_mst e, tmp_job_no d
		// where a.booking_no=b.booking_no and a.booking_no=c.booking_no and a.po_break_down_id=e.id $buyer_cond2 and b.company_id=$companyID  and a.item_category=1 and c.userid=$user_name and a.job_no=d.job_no and d.userid=$user_name AND (a.is_dyied_yarn != 1 or a.is_dyied_yarn is null) $booking_cond and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and a.is_sales=1 and a.entry_form=475 and e.booking_without_order=0";

		$allocation_sql ="SELECT a.job_no, a.booking_no, a.qnty, a.po_break_down_id,b.buyer_id
		from inv_material_allocation_mst a,wo_booking_mst b, fabric_sales_order_mst e, tmp_po_id d
		where a.po_break_down_id = cast(e.id as varchar2(4000)) and e.booking_id = b.id $buyer_cond2 and e.company_id=$companyID  and e.id=d.po_id and d.user_id=$user_name and a.is_dyied_yarn in(0,1) $booking_cond and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and a.is_sales=1 and e.booking_without_order=0";

		//echo $allocation_sql; die; // and a.entry_form in(120,475)
		$allocation_sql_resutl = sql_select($allocation_sql);
		...
		// $allocation_smn_sql ="SELECT a.job_no, a.booking_no, a.qnty, b.buyer_id
		// from inv_material_allocation_mst a,wo_non_ord_samp_booking_mst b, fabric_sales_order_mst e, tmp_booking_id c, tmp_job_no d
		// where a.booking_no=b.booking_no and a.po_break_down_id=e.id $buyer_cond2 and b.company_id=$companyID  and a.item_category=1 and a.booking_no=c.booking_no and c.userid=$user_name and a.job_no=d.job_no and d.userid=$user_name AND (a.is_dyied_yarn != 1 or a.is_dyied_yarn is null) $booking_cond and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and a.is_sales=1 and a.entry_form=475 and e.booking_without_order=1";

		$allocation_smn_sql ="SELECT a.job_no, a.booking_no, a.qnty, b.buyer_id
		from inv_material_allocation_mst a,wo_non_ord_samp_booking_mst b, fabric_sales_order_mst e, tmp_po_id d
		where a.po_break_down_id = cast(e.id as varchar2(4000)) and e.booking_id = b.id $buyer_cond2 and e.company_id=$companyID  and e.id=d.po_id and d.user_id=$user_name and a.is_dyied_yarn in(0,1) $booking_cond and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and a.is_sales=1 and e.booking_without_order=1";

		//echo $allocation_smn_sql; die;//and a.entry_form in(120,475)
		$allocation_smn_sql_resutl = sql_select($allocation_smn_sql);


		foreach($allocation_smn_sql_resutl as $row)
		{
			$allocation_data[$row[csf("job_no")]][$row[csf("booking_no")]]['alloc_qnty'] += $row[csf("qnty")];
			$allocation_data[$row[csf("buyer_id")]]['alloc_buyer_qnty'] += $row[csf("qnty")];
		}
		unset($allocation_smn_sql_resutl);



		$progQnty_sql = "SELECT a.booking_no,a.buyer_id  as customer_buyer,e.buyer_id,b.program_qnty as program_qnty, b.id program_id
		from ppl_planning_entry_plan_dtls a left join
         wo_booking_dtls  c ON  a.booking_no = c.booking_no AND c.status_active = 1
         AND c.is_deleted = 0, ppl_planning_info_entry_dtls b, ppl_planning_info_entry_mst d, fabric_sales_order_mst e, tmp_booking_id f
		where d.id=a.mst_id and a.dtls_id = b.id and a.po_id=e.id and e.company_id=$companyID $buyer_cond and a.booking_no=f.booking_no and f.userid=$user_name
		and a.status_active=1 and a.is_deleted=0 and a.is_sales=1
		and b.status_active = 1 and b.is_deleted = 0 group by a.booking_no,a.buyer_id,e.buyer_id,b.program_qnty, b.id order by b.id";
		//echo $progQnty_sql;die;
		$progQntyResult = sql_select($progQnty_sql);
		...
		foreach($progQntyResult as $row)
		{
			if($programIdQntyChk[$row[csf("id")]] == "")
			{
				...
				$progQntyArr[$row[csf("booking_no")]]['book_prog_qnty'] += $row[csf("program_qnty")];
				$progNoArr[$row[csf("booking_no")]][] = $row[csf("program_id")];
				$progQntyArr[$row[csf("customer_buyer")]]['buyer_prog_qnty'] += $row[csf("program_qnty")];
			}
		}
		unset($progQntyResult);
		...
		$reqs_sql = "SELECT a.yarn_qnty as yarn_req_qnty, a.requisition_no,c.booking_no, b.id as program_no,a.id, c.buyer_id  as customer_buyer,d.buyer_id
			from ppl_yarn_requisition_entry a, ppl_planning_info_entry_dtls b, ppl_planning_info_entry_mst c, fabric_sales_order_mst d,tmp_booking_id e, tmp_po_id f
			where a.knit_id = b.id and b.mst_id = c.id and c.booking_no = d.sales_booking_no
			and b.is_sales = 1 $buyer_cond3 and d.company_id=$companyID and c.booking_no=e.booking_no and e.userid=$user_name and d.id=f.po_id and f.user_id=$user_name
			and a.yarn_qnty>0
			and a.status_active = 1 and a.is_deleted = 0
			and b.status_active = 1 and b.is_deleted = 0
			and c.status_active = 1 and c.is_deleted = 0
		...
		foreach ($reqs_sql_result as $row)
		{
			if($requIdCheckArr[$row[csf('id')]] == "")
			{
				...
				$reqs_array[$row[csf('booking_no')]]['requ_no'] .= $row[csf('requisition_no')].",";
				$reqs_array[$row[csf('booking_no')]]['program_no'] .= $row[csf('program_no')].",";
				$reqs_array[$row[csf('customer_buyer')]]['buyer_req_qnty'] += $row[csf('yarn_req_qnty')];
				$reqs_buyer_array[$row[csf('customer_buyer')]]['requ_no'] .= $row[csf('requisition_no')].",";
				$program_buyer_array[$row[csf('customer_buyer')]]['program_no'] .= $row[csf('program_no')].",";
		...
		$product_sql = "SELECT a.booking_id, c.quantity as knitting_qnty,c.returnable_qnty as return_qnty, c.id, d.sales_booking_no , d.job_no,a.buyer_id  as cust_buyer,d.buyer_id
			from inv_receive_master a, pro_grey_prod_entry_dtls b,order_wise_pro_details c, fabric_sales_order_mst d,tmp_booking_id e
			where a.id=b.mst_id and c.dtls_id=b.id and d.company_id=$companyID $buyer_cond
		...
		foreach($productionArr as $row)
		{
			if($row[csf("booking_id")])
			{
				if($prodQntyIdChk[$row[csf("id")]] == "")
				{
					...
					$productionQntyArr[$row[csf("booking_id")]]["return_qnty"] += $row[csf("return_qnty")];
					//$nprog[$row[csf("booking_id")]]=$row[csf("booking_id")];
					$productionQntyArr_buy[$row[csf("cust_buyer")]]["buyer_prod_qnty"] += $row[csf("knitting_qnty")];
					$productionQntyArr_buy[$row[csf("cust_buyer")]]["buyer_return_qnty"] += $row[csf("return_qnty")];
				}
			}
		}
		unset($productionArr);

		$sql_issue_no = "SELECT (b.quantity) AS issue_qnty, c.buyer_id as buyer_name,d.requisition_no,d.mst_id
   		FROM order_wise_pro_details  b, inv_transaction d, fabric_sales_order_mst c,tmp_po_id f
  		WHERE  d.transaction_type = 2 AND d.item_category = 1 AND d.id = b.trans_id AND b.po_breakdown_id = c.id AND b.po_breakdown_id = f.po_id AND f.user_id = $user_name AND b.trans_type = 2 AND b.entry_form = 3 AND b.status_active = 1
		AND d.receive_basis IN (3, 8) AND b.is_deleted = 0 AND d.status_active = 1 and c.within_group=2 AND b.issue_purpose NOT IN (2, 8) and c.booking_without_order is null
		AND c.company_id = $companyID $buyer_cond7";

		//echo $sql_issue_no;die;
		$sql_issue_no_data = sql_select( $sql_issue_no );

		$issue_qty_arr=array();
		$issue_info_arr=array();
		$issue_qty_summary_arr=array();
		foreach ($sql_issue_no_data as $row)
		{
			$issue_qty_arr[$row[csf('requisition_no')]]['issue_qnty'] += $row[csf('issue_qnty')];
			$issue_info_arr[$row[csf('requisition_no')]]['mst_id'] .= $row[csf('mst_id')].',';
			$issue_qty_summary_arr[$row[csf('buyer_name')]]['issue_qnty'] += $row[csf('issue_qnty')];
		}
		unset($sql_issue_no_data);
		...
		where d.transaction_type=2 and d.item_category=1 and d.id=b.trans_id and b.po_breakdown_id = c.id and c.po_job_no = e.job_no and b.po_breakdown_id=f.po_id and f.user_id=$user_name and b.trans_type=2 and c.within_group=1 and b.entry_form=3 and b.status_active=1 and d.receive_basis in(3,8) and b.is_deleted=0 and d.status_active= 1 and c.within_group=1 and b.issue_purpose not in (2,8) and c.booking_without_order = 0 and c.company_id=$companyID $buyer_cond7";
		...
		$sql_smn_issue = "SELECT (b.quantity) as issue_qnty, a.buyer_id as buyer_name, d.requisition_no, d.mst_id
		from order_wise_pro_details b, inv_transaction d, fabric_sales_order_mst c,  wo_non_ord_samp_booking_mst a, tmp_po_id f
		where d.transaction_type=2 and d.item_category=1 and d.id=b.trans_id and b.po_breakdown_id = c.id and c.booking_id=a.id and b.po_breakdown_id=f.po_id and f.user_id=$user_name and b.trans_type=2 and c.within_group=1 and b.entry_form=3 and b.status_active=1 and d.receive_basis in(3,8) and b.is_deleted=0 and d.status_active= 1 and c.within_group=1 and b.issue_purpose not in (2,8) and a.booking_type=4 and a.is_short is null and c.booking_without_order = 1 and c.company_id=$companyID $buyer_cond7";

		//echo $sql_smn_issue;die;
		$sql_smn_issue_data = sql_select( $sql_smn_issue );
		foreach ($sql_smn_issue_data as $row)
		{
			$issue_qty_arr[$row[csf('requisition_no')]]['issue_qnty'] += $row[csf('issue_qnty')];
			$issue_info_arr[$row[csf('requisition_no')]]['mst_id'] .= $row[csf('mst_id')].',';
			$issue_qty_summary_arr[$row[csf('buyer_name')]]['issue_qnty'] += $row[csf('issue_qnty')];
		}
		unset($sql_smn_issue_data);


		$sql_return = "SELECT (b.quantity) as returned_qnty, b.reject_qty as cons_reject_qnty, e.buyer_name, a.booking_no as requ_no, a.requisition_no from inv_receive_master a, order_wise_pro_details b, inv_transaction d,fabric_sales_order_mst c, wo_po_details_master e, tmp_po_id f where a.id = d.mst_id and a.entry_form=9 and a.item_category =1 and a.receive_basis in(3,8) and d.transaction_type=4 and d.item_category=1 and d.id=b.trans_id and b.po_breakdown_id = c.id and c.po_job_no = e.job_no and b.po_breakdown_id=f.po_id and f.user_id=$user_name and b.trans_type=4 and b.entry_form=9 and b.status_active=1 and b.is_deleted=0 and b.issue_purpose!=2 and (c.booking_without_order = 0 or c.booking_without_order is null) and c.company_id=$companyID $buyer_cond4";
		//echo $sql_return;die;
		$sql_return_data = sql_select($sql_return);
		...
		$sql_smn_return = "SELECT (b.quantity) as returned_qnty, b.reject_qty as cons_reject_qnty, e.buyer_id as buyer_name, a.booking_no as requ_no, a.requisition_no from inv_receive_master a, order_wise_pro_details b, inv_transaction d,fabric_sales_order_mst c,  wo_non_ord_samp_booking_mst e, tmp_po_id f where a.id = d.mst_id and a.entry_form=9 and a.item_category =1 and a.receive_basis in(3,8) and d.transaction_type=4 and d.item_category=1 and d.id=b.trans_id and b.po_breakdown_id = c.id and c.booking_id=e.id and b.po_breakdown_id=f.po_id and f.user_id=$user_name and b.trans_type=4 and b.entry_form=9 and b.status_active=1 and b.is_deleted=0 and e.booking_type=4 and e.is_short is null and b.issue_purpose!=2 and c.booking_without_order = 1 and c.company_id=$companyID $buyer_cond4";
		//echo $sql_smn_return;die;
		$sql_smn_return_data = sql_select($sql_smn_return);


		foreach ($sql_smn_return_data as $row)
		{
			if($row[csf('receive_basis')]==3)
			{
				$return_qty_arr[$row[csf('requ_no')]]['returned_qnty'] += $row[csf('returned_qnty')];
		...
		$sql_fso_no = "SELECT a.id, a.job_no, a.sales_booking_no, a.buyer_id as customer_buyer, b.grey_qty from fabric_sales_order_mst a,fabric_sales_order_dtls b, tmp_po_id c where a.id=b.mst_id and a.id=c.po_id and c.user_id=$user_name and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.within_group=2 and a.booking_without_order is null $company_cond";
		//echo $sql_fso;die;
		$sql_fso_no_data = sql_select($sql_fso_no);

		$sql_fso_order = "SELECT a.id, a.job_no, a.sales_booking_no, d.buyer_id as customer_buyer, b.grey_qty from fabric_sales_order_mst a,fabric_sales_order_dtls b,wo_booking_mst d, tmp_po_id c where a.id=b.mst_id and a.booking_id=d.id and a.id=c.po_id and c.user_id=$user_name and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.within_group=1 and a.booking_without_order=0 $company_cond";
		//echo $sql_fso_order;die;
		$sql_fso_order_data = sql_select($sql_fso_order);

		$sql_fso_non_order = "SELECT a.id, a.job_no, a.sales_booking_no, d.buyer_id as customer_buyer, b.grey_qty from fabric_sales_order_mst a,fabric_sales_order_dtls b,wo_non_ord_samp_booking_mst d, tmp_po_id c where a.id=b.mst_id and a.booking_id=d.id and a.id=c.po_id and c.user_id=$user_name and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.within_group=1 and a.booking_without_order=1 $company_cond";
		//echo $sql_fso_order;die;
		$sql_fso_non_order_data = sql_select($sql_fso_non_order);

		$main_fso_Qnty_arr = array_merge($sql_fso_no_data,$sql_fso_order_data,$sql_fso_non_order_data);

		$fso_data=array();
		$fso_qty_summary_arr=array();
		foreach ($main_fso_Qnty_arr as $row)
		{
			$fso_data[$row[csf('job_no')]][$row[csf('sales_booking_no')]]['grey_qnty']+= $row[csf('grey_qty')];
			$fso_qty_summary_arr[$row[csf('customer_buyer')]]['grey_qty'] += $row[csf('grey_qty')];
		}
		unset($main_fso_Qnty_arr);
		//echo "<pre></pre>";print_r($fso_data);die;
		...
		<fieldset style="width:1650px;">
			...
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1580" class="rpt_table" >
				<thead>
					...
					<th width="100">Allocation Qnty</th>
					<th width="130"><p>Yarn Allocation<br> Balance<br><small style="font-size: 8px;">(FSO - Allocation)</small></p></th>
					<th width="90">Knit Program</th>
					<th width="90">Prog. Balance<br><small style="font-size: 8px;">(FSO - Program)</small></th>
					<th width="90">Yarn Requisition</th>
					<th width="90">Yarn Req Balance<br><p><small style="font-size: 8px;">(Allocation - Requisition)</small></p></th>
					<th width="90">Yarn Issue</th>
					<th width="90">Yarn Issue Return</th>
			...
			<div style="width:1600px; overflow-y:scroll; max-height:330px;" id="scroll_body">
				<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1580" class="rpt_table" id="tbl_list_search">
					<tbody>
						<?
						$z = 1;
						$tot_booking = 0;
						$tot_alloc_buyer_qnty = 0;
						$tot_alloc_buyer_qnty_balance = 0;
						$tot_buyer_prog_qnty = 0;
						$tot_buyer_prog_qnty_balance = 0;
						$tot_buyer_req_qnty = 0;
						$tot_buyer_req_qty_blnc = 0;
						$tot_buyer_issue_qnty = 0;
						$tot_buyer_issue_qty_blnc = 0;
						$tot_buyer_issue_ret_qnty = 0;
						$tot_buyer_production_qnty = 0;
						$tot_buyer_return_qnty = 0;
						$tot_buyer_reject_qnty = 0;
						$tot_buyer_balance_qnty = 0;
						$tot_fso_buyer_balance_qnty = 0;
						...
						foreach($data_summury as $buyer_id =>$row)
						{
							if ($z % 2 == 0)
								...
								$bgcolor = "#FFFFFF";
							$fso_buyer_qnty = $fso_qty_summary_arr[$buyer_id]['grey_qty'];
							$alloc_buyer_qnty = $allocation_data[$buyer_id]["alloc_buyer_qnty"];
							//$alloc_buyer_qnty_balance = $row["qnty"] - $alloc_buyer_qnty;
							$alloc_buyer_qnty_balance = $fso_buyer_qnty - $alloc_buyer_qnty;
							$buyer_prog_qnty = $progQntyArr[$buyer_id]['buyer_prog_qnty'];
							//$buyer_prog_qnty_balance = $row["qnty"] - $buyer_prog_qnty;
							$buyer_prog_qnty_balance = $fso_buyer_qnty - $buyer_prog_qnty;
							//$buyer_prog_qnty_balance = number_format($buyer_prog_qnty_balance,2,".","");
							$buyer_req_qnty = $reqs_array[$buyer_id]['buyer_req_qnty'];
							$buyer_req_qnty_balance = $alloc_buyer_qnty-$buyer_req_qnty;
							...
							//$buyer_return_qnty = $productionQntyArr_buy[$buyer_id]["buyer_return_qnty"];
							$buyer_balance_qnty = $buyer_issue_qnty - $buyer_issue_ret_qnty -$buyer_production_qnty- $buyer_return_qnty-$buyer_reject_qnty;
							if((($cbo_allocation_type == 1) && (number_format($alloc_buyer_qnty_balance,2) > 0.00)) || ($cbo_allocation_type == 2 && (number_format($alloc_buyer_qnty_balance,2) <= 0.00)) || ($cbo_allocation_type == 3 && (number_format($alloc_buyer_qnty,2) <= 0.00)) || ($cbo_allocation_type == 0))
							{
								?>
								<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_s_<? echo $z; ?>', '<? echo $bgcolor; ?>')" id="tr_s_<? echo $z; ?>">
									<td width="40"><? echo $z;?></td>
									<td width="80" align="right" class="wrd_brk" title="<? echo "buyer_id=".$buyer_id;?>"><? echo $buyer_arr[$buyer_id];?></td>
									<td width="100" align="right" class="wrd_brk"><p><? echo  number_format($row["qnty"],2,".","");?></p></td>
									<td width="100" align="right" class="wrd_brk" title="<? echo $row["sales_order_no"];?>"><p><? echo number_format($fso_buyer_qnty,2,".","");?></p></td>
									<td width="100" align="right" class="wrd_brk"><p><? echo number_format($alloc_buyer_qnty,2,".","");?></p></td>
									<td width="130" align="right" title="FSO-Allocation Qnty" class="wrd_brk" title="(FSO - Allocation)"><p><? echo number_format($alloc_buyer_qnty_balance,2,".","");?></p></td>
									<td width="90" align="right" class="wrd_brk"><p><? echo number_format($buyer_prog_qnty,2,".","");?></p></td>
									<td width="90" align="right" title="FSO-Knit Program" class="wrd_brk"><p><? echo number_format($buyer_prog_qnty_balance,2,".","");?></p></td>
									<td width="90" align="right" class="wrd_brk"><p><? echo number_format($buyer_req_qnty,2,".","");?></p></td>
									<td width="90" align="right" class="wrd_brk" title="(Allocation - Requisition)"><p><? echo number_format($buyer_req_qnty_balance,2,".","");?></p></td>
									<td width="90" align="right" class="wrd_brk"><p><? echo number_format($buyer_issue_qnty,2,".","");?></p></td>
									<td width="90" align="right" class="wrd_brk"><p><? echo number_format($buyer_issue_ret_qnty,2,".","");?></p></td>
									<td width="120" align="right" class="wrd_brk" title="(Knit Program - Yarn Issue + issue return)"><p><? echo number_format($buyer_issue_qnty_balance,2,".","");?></p></td>
									<td width="90" align="right" class="wrd_brk"><p><? echo number_format($buyer_production_qnty,2,".","");?></p></td>
									<td width="90" align="right" class="wrd_brk"><p><? echo number_format($buyer_return_qnty,2,".","");?></p></td>
									<td width="90" align="right" class="wrd_brk"><p><? echo number_format($buyer_reject_qnty,2,".","");?></p></td>
									<td width="" align="right" class="wrd_brk" title="Yarn Issue-Yarn Issue Return-Knit Production-Fabric Rejec-Yarn Reject Qnty"><p><? echo number_format($buyer_balance_qnty,2,".","");?></p></td>
								</tr>
								<?
								$z++;
								$tot_booking += $row["qnty"];
								$tot_alloc_buyer_qnty += $alloc_buyer_qnty;
								$tot_alloc_buyer_qnty_balance += $alloc_buyer_qnty_balance;
								$tot_buyer_prog_qnty += $buyer_prog_qnty;
								$tot_buyer_prog_qnty_balance += $buyer_prog_qnty_balance;
								$tot_buyer_req_qnty += $buyer_req_qnty;
								$tot_buyer_req_qty_blnc += $buyer_req_qnty_balance;
								$tot_buyer_issue_qnty += $buyer_issue_qnty;
								$tot_buyer_issue_qty_blnc += $buyer_issue_qnty_balance;
								$tot_buyer_issue_ret_qnty += $buyer_issue_ret_qnty;
								$tot_buyer_production_qnty+=$buyer_production_qnty;
								$tot_buyer_return_qnty += $buyer_return_qnty;
								$tot_buyer_reject_qnty += $buyer_reject_qnty;
								$tot_buyer_balance_qnty += $buyer_balance_qnty;
								$tot_fso_buyer_balance_qnty += $fso_buyer_qnty;
							}
						}
						?>
						<tr style="font-weight:bold;background-color: #e0e0e0">
							<td colspan="2" align="left" width="120">Total:</td>
							<td align="right" width="100"><p><? echo number_format($tot_booking,2,".","");?></p></td>
							<td align="right" width="100"><p><? echo number_format($tot_fso_buyer_balance_qnty,2,".","");?></p></td>
							<td align="right" width="100"><p><? echo number_format($tot_alloc_buyer_qnty,2,".","");?> </p></td>
							<td align="right" width="130"><p><? echo number_format($tot_alloc_buyer_qnty_balance,2,".","");?></p></td>
							<td align="right" width="90"><p><? echo number_format($tot_buyer_prog_qnty,2,".","");?></p></td>
							<td align="right" width="90"><p><? echo number_format($tot_buyer_prog_qnty_balance,2,".","");?></p></td>
							<td align="right" width="90"><p><? echo number_format($tot_buyer_req_qnty,2,".","");?></p></td>
							<td align="right" width="90"><p><? echo number_format($tot_buyer_req_qty_blnc,2,".","");?></p></td>
							<td align="right" width="90"><p><? echo number_format($tot_buyer_issue_qnty,2,".","");?></p></td>
							<td align="right" width="90"><p><? echo number_format($tot_buyer_issue_ret_qnty,2,".","");?></p></td>
							<td align="right" width="120"><p><? echo number_format($tot_buyer_issue_qty_blnc,2,".","");?></p></td>
							<td align="right" width="90"><p><? echo number_format($tot_buyer_production_qnty,2,".","");?></p></td>
							<td align="right" width="90"><p><? echo number_format($tot_buyer_return_qnty,2,".","");?></p></td>
							<td align="right" width="90"><p><? echo number_format($tot_buyer_reject_qnty,2,".","");?></p></td>
							<td align="right"><p><? echo number_format($tot_buyer_balance_qnty,2,".","");?></p></td>
			...
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="2510" class="rpt_table" >
				<thead>
					<th width="40" >SL</th>
					<th width="100" class="wrd_brk">Job No.</th>
					<th width="80" class="wrd_brk">Buyer</th>
					<th width="100" class="wrd_brk">Fabric Booking No</th>
					<th width="120" class="wrd_brk">Sales Order No</th>
					<th width="100" class="wrd_brk">Booking Type</th>
					<th width="100" class="wrd_brk">Internal Ref. No</th>
					<th width="130" class="wrd_brk">PO No</th>
					<th width="100" class="wrd_brk">Booking Qnty</th>
					<th width="100" class="wrd_brk">FSO Qnty</th>
					<th width="100" class="wrd_brk">Allocation Qnty</th>
					<th width="130" class="wrd_brk">Yarn Allocation <br>Balance<br><small style="font-size: 8px;">(FSO - Allocation)</small></th>
					<th width="90" class="wrd_brk">Knit Program</th>
					<th width="90" class="wrd_brk">Prog. Balance<br><small style="font-size: 8px;">(FSO - Program)</small></th>
					<th width="90" class="wrd_brk">Yarn Requisition</th>
					<th width="90" class="wrd_brk">Yarn Req <br>Balance<br><p><small style="font-size: 8px;">(Allocation - Requisition)</small></p></th>
					<th width="100" class="wrd_brk">Yarn Demand</th>
					<th width="100" class="wrd_brk">Yarn Demand <br>Balance<br><p><small style="font-size: 8px;">(Yarn Requisition - Yarn Demand)</small></p></th>
					<th width="90" class="wrd_brk">Yarn Issue</th>
					<th width="90" class="wrd_brk">Yarn Issue Return</th>
					<th width="90" class="wrd_brk">FSO Yarn <br>Balance<br><p><small style="font-size: 8px;">(FSO Qty - Yarn Issue) + (Issue Return + Reject Qty)</small></p></th>
					<th width="120" class="wrd_brk">Issue Balance<br><p><small style="font-size: 8px;">(Knit Program - Yarn Issue) + Issue Return</small></p></th>
					<th width="70" class="wrd_brk">Knit Production</th>
					<th width="90" class="wrd_brk">Fabric Reject</th>
					<th width="90" class="wrd_brk">Yarn Reject Qnty</th>
					<th width="" class="wrd_brk">Balance</th>
				</thead>
			</table>
			<?
			//  echo "<pre>";
			//  print_r($data_array); 
			 $print_report_new = sql_select("select REPORT_ID, format_id from LIB_REPORT_TEMPLATE where TEMPLATE_NAME = $companyID AND MODULE_ID = 2 AND STATUS_ACTIVE = 1");
			 $format_ids_arr = array();
			 foreach($print_report_new as $row)
			 {
				$format_ids_arr[$row['REPORT_ID']] = explode(',', $row['FORMAT_ID'])[0];
			 }

			// $format_ids = explode(",", $print_report);
			?>
			<div style="width:2530px; overflow-y:scroll; max-height:330px;" id="scroll_body_dtls">
				<table cellspacing="0" cellpadding="0" border="1" rules="all" width="2510" class="rpt_table" id="tbl_list_search">
					<tbody>
						<?
						...
						foreach($data_array as $job_no => $job_data)
						{
							...
							{
								//var_dump($booking_no);
								$fso_qnty = $fso_data[$row['sales_order_no']][$booking_no]['grey_qnty'];
								$allocation_qnty = $allocation_data[$row['sales_order_no']][$booking_no]['alloc_qnty'];
								//$allocation_balance = $row["qnty"] - $allocation_qnty;
								$allocation_balance = $fso_qnty - $allocation_qnty;
								if((($cbo_allocation_type == 1) && (number_format($allocation_balance,2) > 0.00)) || ($cbo_allocation_type == 2 && (number_format($allocation_balance,2) <= 0.00 )) || ($cbo_allocation_type == 0))
								{
									$job_row_span++;
								}
						...
						$i=$m=1;
						$grand_job_book_qnty=$grand_job_alloc_qnty=$grand_alloc_balance=$grand_prog_Qnty=$grand_prog_balance=$grand_requ_qnty=$grand_requ_balance=$grand_issue_qnty=$grand_issue_ret_qnty=$grand_issue_balance=$grand_prod_qnty=$grand_return_qnty=$grand_reject_qnty=$grand_balance_qnty=$grand_fso_qnty=$grand_booking_yarn_balance=0;
						...
						foreach($data_array as $job_no => $job_data)
						{
							...
							$sub_job_book_qnty=$sub_job_alloc_qnty=$sub_alloc_balance=$sub_prog_Qnty=$sub_prog_balance=$sub_requ_qnty=$sub_requ_balance=$sub_issue_qnty=$sub_issue_ret_qnty=$sub_issue_balance=$sub_prod_qnty=$sub_return_qnty=$sub_reject_qnty=$sub_balance_qnty=$sub_demand_qty=$sub_demand_balance=$sub_fso_qnty=$sub_booking_yarn_balance=0;
							foreach($job_data as $booking_no => $row)
							{
								...
								if((($cbo_allocation_type == 1) && (number_format($allocation_balance,2) > 0.00)) || ($cbo_allocation_type == 2 && (number_format($allocation_balance,2) <= 0.00 )) || ($cbo_allocation_type == 2 && (number_format($allocation_qnty,2) <= 0.00 )) || ($cbo_allocation_type == 0))
								{
									$po_ids = implode(",",array_unique(explode(",",chop($row["po_id"],','))));
									$prog_Qnty = $progQntyArr[$booking_no]['book_prog_qnty'];
									$program_ids = implode(", ",array_filter(array_unique($progNoArr[$booking_no])));
									if ($prog_Qnty == 0) $program_bgcolor = "background-color:red;";else $program_bgcolor= "";
									//$prog_balance = $row["qnty"] - $prog_Qnty;
									$prog_balance = $fso_qnty - $prog_Qnty;
									//$prog_balance = number_format($prog_balance,2,".","");
									$requ_qnty = $reqs_array[$booking_no]['qnty'];
									if ($requ_qnty == 0) $requ_bgcolor = "background-color:red;";else $requ_bgcolor= "";
									$requ_balance = $allocation_qnty - $requ_qnty;
									...
									//for booking yarn balance
									$booking_yarn_balance = (number_format($fso_qnty,2,'.','')-(number_format($issue_qnty,2,'.',''))+(number_format($issue_ret_qnty,2,'.','')+number_format($reject_qnty,2,'.','')));

									//$po_ids = implode(",",array_unique(explode(",",chop($row['po_id'],','))));

									//===========================================
									if ($row['booking_type'] == 4 && $row['is_short'] == 2)
									{
										$booking_type = 3;
									}
									if ($row['booking_type'] == 4 && $row['is_short'] == null)
									{
										$booking_type = 4;
									}
									else
									...
									if($row['REPORT_ID'] == 1) //main fabric booking
									{
										
									}
									else if($row['REPORT_ID'] == 35) //partial fabric booking
									{
										if($format_ids_arr[$row['REPORT_ID']] == 723)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_17','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 719)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_16','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 143)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','show_fabric_booking_report_urmi','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 84)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','show_fabric_booking_report_urmi_per_job','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 85)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_3','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 151)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','show_fabric_booking_report_advance_attire_ltd','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 160)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_5','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 175)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_6','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 218)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_7','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 220)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_northern_new','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 235)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_northern_9','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 274)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_10','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 241)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_11','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 269)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_12','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 28)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_13','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 280)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_14','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 304)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_15','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 339)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_18','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 370)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_19','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
										if($format_ids_arr[$row['REPORT_ID']] == 768)
										{
											$print_btn = "<a href='#'  onClick=\"generate_worder_report('" . 2 . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row['REPORT_ID'] . "','print_booking_20','" . $i . "')\">" . $row[csf('booking_no')] . $pre . " <a/>";
										}
									}
									else if($row['REPORT_ID'] == 2) //short fabric booking
									{
										
									}
									else if($row['REPORT_ID'] == 3) //sample fabric booking with order
									{
										
									}
									else if($row['REPORT_ID'] == 90) //sample requisition fabric booking without order
									{	
									...
									// 	}
									// }
									// else if ($booking_type == 4) //SMN Fabric booking
									// {
									// 	foreach ($format_ids4 as $row_id)
									// 	{
									// 		$print_btn = "<a href='#'   onClick=\"generate_worder_report('" . $booking_type . "','" . $row[csf('booking_no')] . "','" . $row[csf('company_id')] . "','" . $row[csf('po_break_down_id')] . "','" . $row[csf('item_category')] . "','" . $row[csf('fabric_source')] . "','" . $row[csf('job_no')] . "','" . $row[csf('is_approved')] . "','" . $row_id . "','show_fabric_booking_report7','" . $i . "')\">" . $row[csf('booking_no')] . $pre . "<a/>";
									...
									<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $m; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $m; ?>">
										...
										<td width="100" class="wrd_brk"><? echo $job_no;?></td>
										<td width="80"  class="wrd_brk" title="<? echo $row['customer_buyer']; ?>"><? echo $buyer_arr[$row['customer_buyer']];?></td>
										<td width="100" class="wrd_brk"><?
										 //echo $format_ids_arr[$row['REPORT_ID']];
										 echo $print_btn;?></td>
										<td width="120" class="wrd_brk">
										<?
										 echo "<a href='#'   onClick=\"generate_sales_report('" . $row[csf('company_id')] . "','" . $row['booking_id'] . "','" . $row[csf('booking_no')] . "','" . $row[csf('job_no')] . "')\">" . $row['sales_order_no'] . "<a/>";
										 ?></td>
										<td width="100" class="wrd_brk" align="center"><? echo $booking_type_arr[$booking_no];?></td>
										<td width="100" title="<?=$po_ids;?>" class="wrd_brk"><?
											$internal_ref_no="";
											foreach(explode(",", $po_ids) as $po_id)
											{
												if($internal_ref_no =="") $internal_ref_no =  $internalRefArr[$po_id]["internalRefNumber"]; else $internal_ref_no .= ",".$internalRefArr[$po_id]["internalRefNumber"];
										...
										<td width="130" style="max-width:130px;" class="wrd_brk"><? echo $po_number;//chop($row['po'],',');?></td>
										<td width="100" align="right" class="wrd_brk"><? echo number_format($row['qnty'],2,'.','');?></td>
										<td width="100" align="right" class="wrd_brk"><? echo number_format($fso_qnty,2,'.','');?></td>
										<td width="100" align="right" style="<? echo $alloc_bgcolor;?>" class="wrd_brk"><a href='##' onClick="openmypage_allocation('<? echo $row['sales_order_no'] . "_" . $booking_no ?>', 'allocation_sales_popup')"><? echo number_format($allocation_qnty,2,'.','');?></a></td>
										<td width="130" align="right" title="FSO-Allocation" class="wrd_brk"><? echo number_format($allocation_balance,2,'.','');?></td>
										<td width="90" align="right" style="<? echo $program_bgcolor;?>" class="wrd_brk"><a href='##' onClick="openmypage_program('<? echo $program_ids; ?>', 'program_sales_popup')"><? echo number_format($prog_Qnty,2,'.','');?></a></td>

										<td width="90" align="right" title="FSO - Program" class="wrd_brk"><? echo number_format($prog_balance,2,'.','');?></td>

										<td width="90" class="wrd_brk" align="right" style="<? echo $requ_bgcolor;?>" title="Requisition no=<? echo $requ_nos;?>"><a href='##' onClick="openmypage_requisition('<? echo $program_ids; ?>', 'requisition_popup')"><? echo number_format($requ_qnty,2,'.','');?></a></td>
										<td class="wrd_brk" width="90" align="right" title="(Allocation - Requisition)"><? echo number_format($requ_balance,2,'.','');?></td>
										<td class="wrd_brk" width="100" align="right" title="(Allocation - Requisition)"><a href='##' onClick="openmypage_requisition('<? echo $program_ids; ?>', 'demand_popup')"><? echo number_format($demand_qty,2,'.','');?></a></td>
										<td class="wrd_brk" width="100" align="right" title="Yarn Requisition - Yarn Demand"><? echo number_format($demand_balance,2,'.','');?></td>
										<td class="wrd_brk" width="90" align="right"><a href='##' onClick="openmypage_issue('<? echo $requ_nos."_".$booking_no. "_" . $po_ids. "_" . $job_no; ?>', 'issue_sales_popup')"><? echo number_format($issue_qnty,2,'.','');?></a></td>
										<td class="wrd_brk" width="90" align="right"><a href='##' onClick="openmypage_issue('<? echo chop($requ_nos,',')."_".$booking_no. "_" . $po_ids. "_" . $job_no; ?>', 'issue_return_sales_popup')"><? echo number_format($issue_ret_qnty,2,'.','');?></a></td>
										<td class="wrd_brk" width="90" align="right" title="(FSO Qty - Yarn Issue) + (Issue Return + Reject Qty)"><? echo number_format($booking_yarn_balance,2,'.','');?></td>
										<td class="wrd_brk" width="120" align="right" title="(Knit Program - Yarn Issue) + Issue Return"><? echo number_format($issue_balance,2,'.','');?></td>
										<td class="wrd_brk" width="70" align="right" title="<? echo chop($reqs_array[$booking_no]['program_no'],",");?>"><? echo number_format($prod_qnty,2,'.','');?></td>
										<td class="wrd_brk" width="90" align="right"><? echo number_format($return_qnty,2,'.','');?></td>
										<td class="wrd_brk" width="90" align="right"><? echo number_format($reject_qnty,2,'.','');?></td>
										<td class="wrd_brk" width="" align="right" title="Yarn Issue-Yarn Issue Return-Knit Production-Fabric Rejec-Yarn Reject Qnty"><? echo number_format($balance_qnty,2,'.','');?></td>
									</tr>
									<?
									$m++;$y++;//$job_no
									$sub_job_book_qnty +=  $row['qnty'];
									...
									$sub_fso_qnty += $fso_qnty;
									$sub_booking_yarn_balance += $booking_yarn_balance;
									//$sub_booking_yarn_balance += number_format($booking_yarn_balance,2,'.','');
									$show = true;
							...
							if($show == true)
							{
								...
								<tr style="font-weight: bold;background-color: #e0e0e0">
									...
									<td colspan="2" align="right" width="230">Job Sub Total </td>
									<td align="right" width="100"><p><? echo number_format($sub_job_book_qnty,2,'.','');?></p></td>
									<td align="right" width="100"><p><? echo number_format($sub_fso_qnty,2,'.','');?></p></td>
									<td align="right" width="100"><p><? echo number_format($sub_job_alloc_qnty,2,'.','');?></p></td>
									<td align="right" width="130"><p><? echo number_format($sub_alloc_balance,2,'.','');?></p></td>
									<td align="right" width="90"><p><? echo number_format($sub_prog_Qnty,2,'.','');?></p></td>
									<td align="right" width="90"><p><? echo number_format($sub_prog_balance,2,'.','');?></p></td>
						...
						<tr style="font-weight: bold;background-color: #ccc;border-top: 2px solid">
							...
							<td colspan="2" align="right" width="230">Job Grand Total </td>
							<td align="right" width="100"><p><? echo number_format($grand_job_book_qnty,2,'.','');?></p></td>
							<td align="right" width="100"><p><? echo number_format($grand_fso_qnty,2,'.','');?></p></td>
							<td align="right" width="100"><p><? echo number_format($grand_job_alloc_qnty,2,'.','');?></p></td>
							<td align="right" width="130"><p><? echo number_format($grand_alloc_balance,2,'.','');?></p></td>
							<td align="right" width="90"><p><? echo number_format($grand_prog_Qnty,2,'.','');?></p></td>
							<td align="right" width="90"><p><? echo number_format($grand_prog_balance,2,'.','');?></p></td>
...
if ($action == "report_generate_sales_080623")
{
	$process = array(&$_POST);
	extract(check_magic_quote_gpc($process));
	$cbo_allocation_type = str_replace("'", "", $cbo_allocation_type);
	...
	if ($internalRefNo!="") {$internalRefNo_cond="and c.grouping='$internalRefNo'";}else{$internalRefNo_cond="";}
	if ($hiden_fso_id!="") {$fsoId_cond="and d.id in($hiden_fso_id)";}else{$fsoId_cond="";}
	$booking_cond = "";
	if (str_replace("'", "", trim($txt_booking_no)) != "")
	{
		$booking_cond = " and a.booking_no = $txt_booking_no";
	...
	//end for Print Button Permission
		...
		// Main SQL ====>
		$main_sql = "SELECT a.id, a.company_id, a.booking_no, a.booking_type, a.is_approved, a.is_short, a.buyer_id, b.job_no, b.po_break_down_id, c.grouping, c.po_number, b.grey_fab_qnty as qnty, d.id as sales_id, d.job_no as sales_order_no, d.customer_buyer
		from wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c, fabric_sales_order_mst d
		...
		and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.booking_type in(1,4)
		and b.grey_fab_qnty>0 $job_cond $booking_cond $date_cond $company_cond $internalRefNo_cond $buyer_cond $fsoId_cond
		order by  a.job_no,a.buyer_id,a.booking_no";// a.id=b.booking_mst_id
		//echo $main_sql;die;
		$result = sql_select($main_sql);
        ...
        //Program Qnty
        $progQnty_sql = "SELECT a.booking_no,a.buyer_id,e.customer_buyer,
			b.program_qnty as program_qnty, b.id program_id
			from ppl_planning_entry_plan_dtls a,ppl_planning_info_entry_dtls b,wo_booking_dtls c ,ppl_planning_info_entry_mst d, fabric_sales_order_mst e, tmp_booking_id f
			where d.id=a.mst_id and a.dtls_id = b.id and a.booking_no = c.booking_no and a.po_id=e.id $company_cond $buyer_cond and a.booking_no=f.booking_no and f.userid=$user_name
			and a.status_active=1 and a.is_deleted=0 and a.is_sales=1
			and b.status_active = 1 and b.is_deleted = 0 and c.status_active = 1 and c.is_deleted = 0 group by a.booking_no,a.buyer_id,e.customer_buyer,b.program_qnty, b.id order by b.id";
			//echo $progQnty_sql;die;
		$progQntyResult = sql_select($progQnty_sql);
		...
		$reqs_sql = "SELECT a.yarn_qnty as yarn_req_qnty, a.requisition_no,c.booking_no, b.id as program_no,a.id, c.buyer_id,d.customer_buyer
		...
		$sql_fso = "SELECT a.id, a.job_no, a.sales_booking_no, a.customer_buyer, b.grey_qty from fabric_sales_order_mst a,fabric_sales_order_dtls b, tmp_po_id c where a.id=b.mst_id and a.id=c.po_id and c.user_id=$user_name and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $company_cond";
		//echo $sql_fso;die;
		$sql_fso_data = sql_select($sql_fso);

		$fso_data=array();
		$fso_qty_summary_arr=array();
		foreach ($sql_fso_data as $row)
		{
			$fso_data[$row[csf('job_no')]][$row[csf('sales_booking_no')]]['grey_qnty']+= $row[csf('grey_qty')];
			$fso_qty_summary_arr[$row[csf('customer_buyer')]]['grey_qty'] += $row[csf('grey_qty')];
		}
					...
					<tbody>
						...
						foreach($data_summury as $buyer_id =>$row)
						{
							...
							if((($cbo_allocation_type == 1) && (number_format($alloc_buyer_qnty_balance,2) > 0.00)) || ($cbo_allocation_type == 2 && (number_format($alloc_buyer_qnty_balance,2) <= 0.00)) || ($cbo_allocation_type == 3 && (number_format($alloc_buyer_qnty,2) <= 0.00)) || ($cbo_allocation_type == 0))
							{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_s_<? echo $z; ?>', '<? echo $bgcolor; ?>')" id="tr_s_<? echo $z; ?>">
									<td width="40"><? echo $z;?></td>
									<td width="80" align="right" class="wrd_brk"><? echo $buyer_arr[$buyer_id];?></td>
									<td width="100" align="right" class="wrd_brk"><p><? echo $row["qnty"];?></p></td>
									<td width="100" align="right" class="wrd_brk"><p><? echo $fso_buyer_qnty;?></p></td>
									<td width="100" align="right" class="wrd_brk"><p><? echo $alloc_buyer_qnty;?></p></td>
									<td width="130" align="right" title="FSO-Allocation Qnty" class="wrd_brk" title="(FSO - Allocation)"><p><? echo $alloc_buyer_qnty_balance;?></p></td>
									<td width="90" align="right" class="wrd_brk"><p><? echo $buyer_prog_qnty;?></p></td>
									<td width="90" align="right" title="FSO-Knit Program" class="wrd_brk"><p><? echo $buyer_prog_qnty_balance;?></p></td>
									<td width="90" align="right" class="wrd_brk"><p><? echo $buyer_req_qnty;?></p></td>
									<td width="90" align="right" class="wrd_brk" title="(Allocation - Requisition)"><p><? echo $buyer_req_qnty_balance;?></p></td>
									<td width="90" align="right" class="wrd_brk"><p><? echo $buyer_issue_qnty;?></p></td>
									<td width="90" align="right" class="wrd_brk"><p><? echo $buyer_issue_ret_qnty;?></p></td>
									<td width="120" align="right" class="wrd_brk" title="(Knit Program - Yarn Issue + issue return)"><p><? echo $buyer_issue_qnty_balance;?></p></td>
									<td width="90" align="right" class="wrd_brk"><p><? echo $buyer_production_qnty;?></p></td>
									<td width="90" align="right" class="wrd_brk"><p><? echo $buyer_return_qnty;?></p></td>
									<td width="90" align="right" class="wrd_brk"><p><? echo $buyer_reject_qnty;?></p></td>
									<td width="" align="right" class="wrd_brk" title="Yarn Issue-Yarn Issue Return-Knit Production-Fabric Rejec-Yarn Reject Qnty"><p><? echo $buyer_balance_qnty;?></p></td>
								</tr>
								<?
								$z++;
								$tot_booking += $row["qnty"];
						...
						<tr style="font-weight:bold;background-color: #e0e0e0">
							<td colspan="2" align="left" width="120">Total:</td>
							<td align="right" width="100"><p><? echo $tot_booking;?></p></td>
							<td align="right" width="100"><p><? echo $tot_fso_buyer_balance_qnty;?></p></td>
							<td align="right" width="100"><p><? echo $tot_alloc_buyer_qnty;?></p></td>
							<td align="right" width="130"><p><? echo $tot_alloc_buyer_qnty_balance;?></p></td>
							<td align="right" width="90"><p><? echo $tot_buyer_prog_qnty;?></p></td>
							<td align="right" width="90"><p><? echo $tot_buyer_prog_qnty;?></p></td>
							<td align="right" width="90"><p><? echo $tot_buyer_req_qnty;?></p></td>
							<td align="right" width="90"><p><? echo $tot_buyer_req_qty_blnc;?></p></td>
							<td align="right" width="90"><p><? echo $tot_buyer_issue_qnty;?></p></td>
							<td align="right" width="90"><p><? echo $tot_buyer_issue_ret_qnty;?></p></td>
							<td align="right" width="120"><p><? echo $tot_buyer_issue_qty_blnc;?></p></td>
							<td align="right" width="90"><p><? echo $tot_buyer_production_qnty;?></p></td>
							<td align="right" width="90"><p><? echo $tot_buyer_return_qnty;?></p></td>
							<td align="right" width="90"><p><? echo $tot_buyer_reject_qnty;?></p></td>
							<td align="right"><p><? echo $tot_buyer_balance_qnty;?></p></td>
						</tr>
					</tbody>
				</table>
			</div>
				...
				<table cellspacing="0" cellpadding="0" border="1" rules="all" width="2510" class="rpt_table" id="tbl_list_search">
					<tbody>
						...
						foreach($data_array as $job_no => $job_data)
						{
							...
							foreach($job_data as $booking_no => $row)
							{
								...
								if((($cbo_allocation_type == 1) && (number_format($allocation_balance,2) > 0.00)) || ($cbo_allocation_type == 2 && (number_format($allocation_balance,2) <= 0.00 )) || ($cbo_allocation_type == 2 && (number_format($allocation_qnty,2) <= 0.00 )) || ($cbo_allocation_type == 0))
								{
									...
									<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $m; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $m; ?>">
										...
										{
											?>
											<td width="40" rowspan="<? echo $job_row_span;?>" class="wrd_brk"><? echo $i;?></td>
											<td width="100" rowspan="<? echo $job_row_span;?>" class="wrd_brk"><? echo $job_no;?></td>
											<td width="80" rowspan="<? echo $job_row_span;?>" class="wrd_brk"><? echo $buyer_arr[$row['customer_buyer']];?></td>
											<?
										}
										?>
										<td width="100" class="wrd_brk"><? echo $print_btn;?></td>
										<td width="120" class="wrd_brk">
										<?
										 echo "<a href='#'   onClick=\"generate_sales_report('" . $row[csf('company_id')] . "','" . $row['booking_id'] . "','" . $row[csf('booking_no')] . "','" . $row[csf('job_no')] . "')\">" . $row['sales_order_no'] . "<a/>";
										 ?></td>
										<td width="100" class="wrd_brk"><? echo $booking_type_arr[$booking_no];?></td>
										<td width="100" title="<?=$po_ids;?>" class="wrd_brk"><?
											$internal_ref_no="";
											foreach(explode(",", $po_ids) as $po_id)
											{
										...
										<td width="130" style="max-width:130px;" class="wrd_brk"><? echo $po_number;//chop($row['po'],',');?></td>
										<td width="100" align="right" class="wrd_brk"><? echo $row['qnty'];?></td>
										<td width="100" align="right" class="wrd_brk"><? echo $fso_qnty;?></td>
										<td width="100" align="right" style="<? echo $alloc_bgcolor;?>" class="wrd_brk"><a href='##' onClick="openmypage_allocation('<? echo $row['sales_order_no'] . "_" . $booking_no ?>', 'allocation_sales_popup')"><? echo $allocation_qnty;?></a></td>
										<td width="130" align="right" title="FSO-Allocation" class="wrd_brk"><? echo number_format($allocation_balance,2);?></td>
										<td width="90" align="right" style="<? echo $program_bgcolor;?>" class="wrd_brk"><a href='##' onClick="openmypage_program('<? echo $program_ids; ?>', 'program_sales_popup')"><? echo number_format($prog_Qnty,2);?></a></td>
										...
										<td class="wrd_brk" width="100" align="right" title="Yarn Requisition - Yarn Demand"><? echo number_format($demand_balance,2);?></td>
										<td class="wrd_brk" width="90" align="right"><a href='##' onClick="openmypage_issue('<? echo $requ_nos."_".$booking_no. "_" . $po_ids. "_" . $job_no; ?>', 'issue_sales_popup')"><? echo number_format($issue_qnty,2);?></a></td>
										<td class="wrd_brk" width="90" align="right"><a href='##' onClick="openmypage_issue('<? echo chop($issue_id,',')."_".$booking_no. "_" . $po_ids. "_" . $job_no; ?>', 'issue_return_sales_popup')"><? echo number_format($issue_ret_qnty,2);?></a></td>
										<td class="wrd_brk" width="90" align="right" title="(FSO Qty - Yarn Issue) + (Issue Return + Reject Qty)"><? echo number_format($booking_yarn_balance,2);?></td>
										<td class="wrd_brk" width="120" align="right" title="(Knit Program - Yarn Issue) + Issue Return"><? echo number_format($issue_balance,2);?></td>
										<td class="wrd_brk" width="70" align="right" title="<? echo chop($reqs_array[$booking_no]['program_no'],",");?>"><? echo number_format($prod_qnty,2);?></td>
										<td class="wrd_brk" width="90" align="right"><? echo number_format($return_qnty,2);?></td>
							...
							if($show == true)
							{
								...
								<tr style="font-weight: bold;background-color: #e0e0e0">
									...
									<td width="100">&nbsp;</td>
									<td colspan="2" align="right" width="230">Job Sub Total </td>
									<td align="right" width="100"><p><? echo $sub_job_book_qnty;?></p></td>
									<td align="right" width="100"><p><? echo $sub_fso_qnty;?></p></td>
									<td align="right" width="100"><p><? echo $sub_job_alloc_qnty?></p></td>
									<td align="right" width="130"><p><? echo $sub_alloc_balance;?></p></td>
									<td align="right" width="90"><p><? echo $sub_prog_Qnty;?></p></td>
									<td align="right" width="90"><p><? echo $sub_prog_balance;?></p></td>
									<td align="right" width="90"><p><? echo $sub_requ_qnty;?></p></td>
									<td align="right" width="90"><p><? echo $sub_requ_balance;?></p></td>
									<td align="right" width="100"><p><? echo $sub_demand_qty;?></p></td>
									<td align="right" width="100"><p><? echo $sub_demand_balance;?></p></td>
									<td align="right" width="90"><p><? echo $sub_issue_qnty;?></p></td>
									<td align="right" width="90"><p><? echo $sub_issue_ret_qnty;?></p></td>
									<td align="right" width="90"><p><? echo $sub_booking_yarn_balance;?></p></td>
									<td align="right" width="120"><p><? echo $sub_issue_balance;?></p></td>
									<td align="right" width="70"><p><? echo $sub_prod_qnty;?></p></td>
									<td align="right"  width="90"><p><? echo $sub_return_qnty;?></p></td>
									<td align="right"  width="90"><p><? echo $sub_reject_qnty;?></p></td>
									<td align="right"  width=""><p><? echo $sub_balance_qnty;?></p></td>
								</tr>
								<?
							}
							$grand_job_book_qnty +=  $sub_job_book_qnty;
						...
						<tr style="font-weight: bold;background-color: #ccc;border-top: 2px solid">
							...
							<td width="100">&nbsp;</td>
							<td colspan="2" align="right" width="230">Job Grand Total </td>
							<td align="right" width="100"><p><? echo $grand_job_book_qnty;?></p></td>
							<td align="right" width="100"><p><? echo $grand_fso_qnty;?></p></td>
							<td align="right" width="100"><p><? echo $grand_job_alloc_qnty?></p></td>
							<td align="right" width="130"><p><? echo $grand_alloc_balance;?></p></td>
							<td align="right" width="90"><p><? echo $grand_prog_Qnty;?></p></td>
							<td align="right" width="90"><p><? echo $grand_prog_balance;?></p></td>
							<td align="right" width="90"><p><? echo $grand_requ_qnty;?></p></td>
							<td align="right" width="90"><p><? echo $grand_requ_balance;?></p></td>
							<td align="right" width="100"><p><? echo $sub_demand_qty;?></p></td>
							<td align="right" width="100"><p><? echo $sub_demand_balance;?></p></td>
							<td align="right" width="90"><p><? echo $grand_issue_qnty;?></p></td>
							<td align="right" width="90"><p><? echo $grand_issue_ret_qnty;?></p></td>
							<td align="right" width="90"><p><? echo $grand_booking_yarn_balance;?></p></td>
							<td align="right" width="120"><p><? echo $grand_issue_balance;?></p></td>
							<td align="right" width="70"><p><? echo $grand_prod_qnty;?></p></td>
							<td align="right" width="90"><p><? echo $grand_return_qnty;?></p></td>
							<td align="right" width="90"><p><? echo $grand_reject_qnty;?></p></td>
							<td align="right" width=""><p><? echo $grand_balance_qnty;?></p></td>
						</tr>
					</tbody>
				</table>
			</div>
...
if ($action == "report_generate_2")
{
	...
	if ($report_type == 2)
	{
		...
		foreach($result as $row)
		{
			...
			if($bookingDtlsIdChk[$row[csf('dtls_id')]] == "")
			{
				$bookingDtlsIdChk[$row[csf('dtls_id')]] = $row[csf('dtls_id')];
				$all_booking_dtls_id_arr[$row[csf("dtls_id")]] = $row[csf("dtls_id")];
			}
		...
		$all_po_id_arr = array_filter($all_po_id_arr);
		$all_booking_dtls_id_arr = array_filter($all_booking_dtls_id_arr);
		fnc_tempengine("GBL_TEMP_ENGINE", $user_name, 35, 1,$all_po_id_arr, $empty_arr);
		fnc_tempengine("GBL_TEMP_ENGINE", $user_name, 35, 2,$all_booking_dtls_id_arr, $empty_arr);
		oci_commit($con);

		$all_booking_dtls_id_arr = array_filter($all_booking_dtls_id_arr);
		if(!empty($all_booking_dtls_id_arr))
		{
			$con = connect();
			foreach($all_booking_dtls_id_arr as $dtlsId)
			{
				execute_query("INSERT INTO TMP_BOOKING_ID(BOOKING_ID,USERID) VALUES(".$dtlsId.", ".$user_name.")");
			}
			oci_commit($con);
		}
		...
		$allocation_sql ="select a.job_no, a.booking_no, a.qnty, a.po_break_down_id,b.buyer_id
		from inv_material_allocation_mst a,wo_booking_mst b, tmp_booking_id c, tmp_job_no d
		where a.booking_no=b.booking_no $buyer_cond2 and b.company_id=$companyID and a.booking_no=c.booking_no and c.userid=$user_name and a.job_no=d.job_no and d.userid=$user_name
		and a.is_dyied_yarn=2 and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0";
		...
		$reqs_sql = "SELECT a.yarn_qnty as yarn_req_qnty, a.requisition_no,c.booking_no, b.id as program_no,a.id, c.buyer_id
		from ppl_yarn_requisition_entry a, ppl_planning_info_entry_dtls b, ppl_planning_info_entry_mst c, wo_booking_mst d,tmp_booking_id e, tmp_job_no f
		where a.knit_id = b.id and b.mst_id = c.id and c.booking_no = d.booking_no and c.booking_no=e.booking_no and d.job_no=f.job_no and f.userid=$user_name   and e.userid=$user_name
		and b.is_sales != 1 $buyer_cond3 and c.company_id=$companyID
		and a.yarn_qnty>0
		and a.status_active = 1 and a.is_deleted = 0
		and b.status_active = 1 and b.is_deleted = 0
		and c.status_active = 1 and c.is_deleted = 0
		...
		$product_sql = "SELECT a.booking_id, c.quantity as knitting_qnty,c.returnable_qnty as return_qnty, c.id, d.booking_no , d.job_no,a.buyer_id
		from inv_receive_master a, pro_grey_prod_entry_dtls b,order_wise_pro_details c , wo_booking_dtls d,wo_booking_mst f,tmp_booking_id e , tmp_job_no g
		where a.id=b.mst_id and c.dtls_id=b.id $company_cond $buyer_cond $booking_date_cond2
		and c.po_breakdown_id = d.po_break_down_id  d.booking_no=f.booking_no and d.booking_no=e.booking_no and e.userid=$user_name and d.job_no=g.job_no and g.userid=$user_name
		and a.item_category=13 and a.entry_form=2 and c.entry_form=2
		and a.receive_basis=2 and b.status_active=1 and b.is_deleted=0 and c.status_active  = 1 and c.is_deleted = 0
		and a.status_active = 1 and a.is_deleted = 0 ";
		//echo $product_sql ; die;
		$productionArr = sql_select($product_sql);
		...
		$sql_issue = "SELECT (b.quantity) as issue_qnty, e.buyer_name, d.requisition_no
		from order_wise_pro_details b, inv_transaction d, wo_po_break_down c, wo_po_details_master e, GBL_TEMP_ENGINE f
		where d.transaction_type=2 and d.item_category=1 and d.id=b.trans_id and b.po_breakdown_id = c.id and c.job_id = e.id and b.po_breakdown_id=f.po_id and c.id=f.ref_val  and f.user_id=$user_name and b.trans_type=2 and b.entry_form=3 and b.status_active=1 and d.receive_basis = 3 and b.is_deleted=0 and d.status_active= 1 and f.ref_from=1 and f.entry_form=35 and d.is_deleted= 0 and b.issue_purpose not in (2,8) and d.company_id=$companyID $buyer_cond4";
		...
		$sql_return = "SELECT (b.quantity) as returned_qnty, b.reject_qty as cons_reject_qnty, e.buyer_name, a.booking_no as requ_no from inv_receive_master a, order_wise_pro_details b, inv_transaction d,wo_po_break_down c, wo_po_details_master e, GBL_TEMP_ENGINE f where a.id = d.mst_id  and d.id=b.trans_id and b.po_breakdown_id = c.id  and c.job_id = e.id and b.po_breakdown_id=f.ref_val  and c.id=f.ref_val and f.user_id=$user_name and f.ref_from=1 and f.entry_form=35  and a.entry_form=9 and a.item_category =1 and a.receive_basis = 3 and d.transaction_type=4 and d.item_category=1 and b.trans_type=4 and b.entry_form=9 and b.status_active=1 and b.is_deleted=0
		and a.status_active=1 and a.is_deleted=0
		and d.status_active=1 and d.is_deleted=0 and b.issue_purpose!=2 $company_cond $buyer_cond4";
		...
		$sql_trnf = "SELECT b.id, b.transfer_qnty, c.job_no, c.booking_no, d.buyer_id
		FROM inv_item_transfer_mst a, inv_item_transfer_dtls b, wo_booking_dtls c, wo_booking_mst d,GBL_TEMP_ENGINE e, GBL_TEMP_ENGINE f
		WHERE a.id = b.mst_id and b.to_order_id = c.po_break_down_id and c.booking_no = d.booking_no and c.id=e.ref_val and e.user_id=$user_name and e.ref_from=2 and e.entry_form=35 and b.to_order_id=f.ref_val and f.ref_from=1 and f.entry_form=35 and f.user_id=$user_name and a.item_category = 13 and a.entry_form = 82 and a.transfer_criteria in(1, 4)";
		...
		$sql_trnf_out = "SELECT b.id, b.transfer_qnty, c.job_no, c.booking_no, d.buyer_id
		FROM inv_item_transfer_mst a, inv_item_transfer_dtls b, wo_booking_dtls c, wo_booking_mst d ,GBL_TEMP_ENGINE e, GBL_TEMP_ENGINE f
		WHERE a.id = b.mst_id and b.from_order_id = c.po_break_down_id and c.booking_no = d.booking_no and c.id=e.ref_val and e.user_id=$user_name and e.ref_from=2 and e.entry_form=35 and b.from_order_id=f.ref_val and f.ref_from=1 and f.entry_form=35 and f.user_id=$user_name and a.item_category = 13 and a.entry_form = 82 and a.transfer_criteria in(1, 4)";
		//echo $sql_trnf_out;
	...
	<div style="width:820px; overflow-y:scroll; max-height:400px;" id="scroll_body">
		<table cellspacing="0" cellpadding="0" border="1" width="800" rules="all" class="rpt_table" id="tbl_list_search">
			...
			foreach ($nameArray as $row)
			{
				...
				if ($knit_id_array[$row[csf('program_id')]] != "")
				{
					$all_prod_id = array_unique(explode(",", substr($knit_id_array[$row[csf('program_id')]], 0, -1)));
					//var_dump($all_prod_id);
					$row_span = count($all_prod_id);
					$z = 0;
					foreach ($all_prod_id as $prod_id)
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>">
							...
							if($reqChk[$requisition_no] == "")
							{
								$reqChk[$requisition_no] = $requisition_no;
...
if ($action == "issue_popup")
{
	...
	/*
	$job_sql="select id from wo_po_break_down where job_no_mst='$job_nos' and status_active=1 and is_deleted=0";
	...
	<fieldset style="width:1150px; margin-left:3px">
		<div style="width:100%" id="report_container">
			...
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1140" class="rpt_table" id="tbl_list_search">
				...
				if($requ_ids !='')
				{
					$sql = "SELECT a.issue_number, a.issue_date,a.issue_basis, a.challan_no, a.knit_dye_source, a.knit_dye_company, a.booking_no, a.store_id, sum(b.quantity) as issue_qnty, c.lot, c.yarn_type, c.id as prod_id, c.product_name_details, d.brand_id,d.requisition_no,c.brand
					from inv_issue_master a, order_wise_pro_details b, product_details_master c, inv_transaction d
					where a.id=d.mst_id and d.transaction_type=2 and d.item_category=1 and d.id=b.trans_id and b.trans_type=2 and b.entry_form=3 and d.requisition_no in ($requ_ids) and b.po_breakdown_id in($po_ids) and b.prod_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.issue_basis in(3,8) and  b.issue_purpose not in (2,8) and b.is_sales=1
					group by a.id,c.id,a.issue_number,a.issue_date, a.challan_no, a.knit_dye_source,a.knit_dye_company, a.booking_no, a.store_id, c.lot, c.yarn_type, c.product_name_details, d.brand_id,a.issue_basis,d.requisition_no,c.brand";
					//echo $sql;
					$result = sql_select($sql);
				}
	...
	<fieldset style="width:1050px; margin-left:3px">
		<div style="width:100%" id="report_container">
			...
			<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1040" class="rpt_table" id="tbl_list_search">
				...
				$sql = "SELECT a.recv_number, a.receive_date, a.challan_no, a.knitting_source, a.knitting_company, a.booking_no, a.store_id, sum(b.quantity) as returned_qnty, c.lot, c.yarn_type, c.id as prod_id, c.product_name_details, d.brand_id, c.brand from inv_receive_master a, order_wise_pro_details b, product_details_master c, inv_transaction d where a.id=d.mst_id and d.transaction_type=4 and d.item_category=1 and d.id=b.trans_id and b.trans_type=4 and b.entry_form=9 and a.requisition_no in ($req_nos) and b.po_breakdown_id IN($po_ids) and b.prod_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.issue_purpose not in (2,8) group by a.id, c.id, a.recv_number, a.receive_date, a.challan_no, a.knitting_source, a.knitting_company, a.booking_no, a.store_id, c.lot, c.yarn_type, c.product_name_details, d.brand_id, c.brand";
				//echo $sql;
				$result = sql_select($sql);
				foreach ($result as $row) {
					if ($i % 2 == 0)
