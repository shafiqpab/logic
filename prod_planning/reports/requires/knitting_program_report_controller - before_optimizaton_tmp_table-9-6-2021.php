...
if ($action == "report_generate") 
{
	...
	if ($template == 1)
	{
		...
		*/
		...
		$barcode_sql=sql_select("SELECT barcode_no FROM pro_roll_details WHERE entry_form=58 AND po_breakdown_id IN(".implode(",",$po_arr).") AND status_active=1");
		...
		if(!empty($requisition_no_arr))
		{
			...
			// for yarn_IssRtn_arr
			$yarnIssueRtnData = sql_select("SELECT a.booking_id as reqsn_no, b.prod_id, SUM(b.cons_quantity) AS qnty, SUM(b.cons_reject_qnty) AS reject_qnty FROM inv_receive_master a, inv_transaction b WHERE a.id=b.mst_id AND a.receive_basis=3 AND a.entry_form=9 AND b.item_category=1 AND b.transaction_type=4 AND a.status_active=1 AND a.is_deleted=0 $requisition_cond2 group by a.booking_id, b.prod_id");
		...
		if(!empty($receive_ids_arr))
		{
			...
			//barcodeData
			$barcodeData = sql_select("SELECT mst_id, barcode_no FROM inv_receive_master a, pro_roll_details b WHERE a.id=b.mst_id AND a.entry_form=2 AND b.entry_form=2 AND a.item_category=13 AND a.receive_basis=2 ".$all_receive_id_cond."");
		...
		foreach ($nameArray as $row)
		{
			...
			$rptPrint[$typeZS][$comZS][$locZS][$floorZS][$diaZS][$pogramZS]['shipDate'] = $row[csf('')];
...
if ($action == "report_generate--") 
{
	...
	if ($template == 1)
	{
		...
		*/
		...
		if ($presentationType == 1) 
		{
			$sqlField = "a.company_id, a.buyer_id, a.body_part_id, a.color_type_id, a.fabric_desc, a.gsm_weight, a.dia, a.width_dia_type, b.id, b.knitting_source, b.knitting_party, b.color_id, b.color_range, b.machine_dia, b.machine_gg, b.program_qnty AS program_qnty, b.program_date, b.stitch_length, b.spandex_stitch_length, b.draft_ratio, b.machine_id, b.distribution_qnty, b.status, b.start_date, b.end_date, b.remarks, b.color_id, b.location_id, c.booking_no, c.po_id";
			$sqlWhere = "a.id = b.mst_id AND b.id = c.dtls_id AND a.company_id = ".$company_name." AND b.knitting_party LIKE '".$party_type."' AND b.machine_dia LIKE '".$machine_dia."' AND b.id LIKE '".$program_no."' AND a.status_active = 1 AND a.is_deleted = 0 AND b.status_active = 1 AND b.is_deleted = 0 AND c.status_active = 1 AND c.is_deleted = 0 AND c.is_sales = 0 $location_cond $buyer_id_cond $po_id_cond $date_cond $booking_search_cond $status_cond $knitting_source_cond";
			
			if ($txt_machine_no != '' || $floor_id != 0)
			{
				$sqlTable = "ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b, ppl_planning_entry_plan_dtls c, ppl_entry_machine_datewise d";
				//$sqlWhere .= " AND b.id = d.dtls_id AND d.machine_id IN(SELECT id FROM lib_machine_name WHERE machine_no = '".$txt_machine_no."')";
				$sqlWhere .= " AND b.id = d.dtls_id AND d.machine_id IN(SELECT id FROM lib_machine_name WHERE category_id = 1 AND status_active = 1 AND is_deleted = 0 AND company_id IN(".$company_name.") AND floor_id IN(".$floor_id."))";
			}
			else
			{
				$sqlTable = "ppl_planning_info_entry_mst a, ppl_planning_info_entry_dtls b, ppl_planning_entry_plan_dtls c";
			}
		...
		foreach ($nameArray as $row)
		{
			...
			{
				$floorId = $floorDataArr[$row[csf('id')]];
			}
...
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['programNo'] = $row[csf('id')];
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['programDate'] = change_date_format($row[csf('program_date')]);
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['bookingType'] = $row[csf('')];
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['bookingNo'] = $row[csf('booking_no')];
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['jobNo'] = $job_no;
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['buyer'] = $buyer_arr[$row[csf('buyer_id')]];
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['orderNo'] = $po_no;
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['intRef'] = implode(",",array_unique($ref_nos_arr));
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['style'] = $style_ref;
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['shipDate'] = $row[csf('')];
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['fabricDescription'] = $row[csf('fabric_desc')];
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['fabricColor'] = $gmts_color;
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['fabricType'] = $color_type[$row[csf('color_type_id')]];
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['fabricGSM'] = $row[csf('gsm_weight')];
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['fabricDia'] = $row[csf('dia')];
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['widthDiaType'] = $row[csf('width_dia_type')];
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['requistionNo'] = $reqsDataArr[$row[csf('id')]]['reqs_no'];
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['yarnDescription'] = join(",", array_unique($yarn_desc));
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['supplier'] = implode(", ", array_unique($supplier));
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['lot'] = join(", ", array_unique($lot));
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['programQty'] = number_format($row[csf('program_qnty')], 2, '.', '');
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['knitProduction'] = number_format($knitting_qnty,2, '.', '');
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['balance'] = $balance_qnty;
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['requisitionQty'] = number_format($reqsDataArr[$row[csf('id')]]['yarn_req_qnty'],2, '.', '');
$requi_balance = ($row[csf('program_qnty')] - $reqsDataArr[$row[csf('id')]]['yarn_req_qnty']);
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['requisitionBalance'] = number_format($requi_balance, 2, '.', '');

$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['demandQty'] = $row[csf('')];
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['demandBalance'] = $row[csf('')];
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['yarnIssueQty'] = number_format($yarn_issue_qnty, 2);
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['yarnReturnQty'] = number_format($yarn_issue_return_qnty, 2);
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['fabricRejectQty'] = number_format($yarn_issue_reject_qnty, 2);
$rptPrint[$knitting_cond][$row[csf('company_id')]][$row[csf('location_id')]][$floorId][$machine_dia_gg]['issueBalanceQty'] = number_format($yarn_issue_bl_qnty, 2);
		}
			...
			<fieldset style="width:<? echo $tbl_width; ?>px;">
				...
				<div style="width:<? echo $tbl_width - 20 +100; ?>px; overflow-y:scroll; max-height:330px;" id="scroll_body">
					<table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width - 40 +100; ?>" class="rpt_table" id="tbl_list_search">
						<tbody>
							<?
							if($type>0) $knitting_source_cond="and b.knitting_source=$type";else $knitting_source_cond="";
							$status_cond = "";
