				...
				<table width="770" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
					...
					<tbody>
						<tr>
							...
							<td align="center">
								<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view('<? echo $companyID; ?>' + '**' + document.getElementById('cbo_buyer_name').value + '**' + document.getElementById('cbo_search_by').value + '**' + document.getElementById('txt_search_common').value + '**' + document.getElementById('txt_date_from').value + '**' + document.getElementById('txt_date_to').value + '**' +'<? echo $jobYear; ?>', 'create_order_no_search_list_view', 'search_div', 'knitting_program_report_controller', 'setFilterGrid(\'tbl_list_search\',-1)');" style="width:100px;" />
							</td>
						</tr>
						<tr>
...
if ($action=="create_booking_search_list_view")
{
	...
	*/
	...
	$sql = "select a.pay_mode,a.booking_no_prefix_num, a.booking_no, a.booking_date, a.company_id, a.buyer_id, a.job_no, a.po_break_down_id, a.item_category, a.fabric_source, a.supplier_id, a.is_approved, a.ready_to_approved from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and $company $buyer $booking_no $booking_date $booking_type $is_short and a.status_active=1 and a.is_deleted=0 and a.item_category=2 group by a.pay_mode,a.booking_no_prefix_num, a.booking_no, a.booking_date, a.company_id, a.buyer_id, a.job_no, a.po_break_down_id, a.item_category, a.fabric_source, a.supplier_id, a.is_approved, a.ready_to_approved order by a.booking_no_prefix_num DESC";
	$sql_arr=sql_select($sql);
	foreach ($sql_arr as $row)
	{
		if ($row[csf('pay_mode')]==3 || $row[csf('pay_mode')]==5) {
			$suplier=return_library_array( "select id, company_short_name from lib_company",'id','company_short_name');
		}
		else
		{
...
if ($action == "report_generate") 
{
	...
	if ($template == 1)
	{
		...
		if (str_replace("'", "", trim($txt_date_from)) != "" && str_replace("'", "", trim($txt_date_to)) != "") 
		{
			...
			{
				$po_received_date_cond = " AND b.pub_shipment_date between " . trim($txt_date_from) . " AND " . trim($txt_date_to) . "";
			}
			else
			{
		...
		*/
		...
		if($order_cond != "" || $job_cond!="" || $internal_cond!="" || $file_no_cond!="" || $po_received_date_cond!="")
		{
			...
			{
				echo "This PO not found in job year $cbo_year";die;
			}
		}
		//echo "<pre>";
		...
		if(!empty($program_arr))
		{
			...
			{
				$demandData = sql_select("SELECT a.knit_id, MAX(a.requisition_no) AS reqs_no, LISTAGG(a.prod_id, ',') WITHIN GROUP (ORDER BY a.prod_id) AS prod_id, SUM(b.yarn_demand_qnty) AS yarn_demand_qnty FROM ppl_yarn_requisition_entry a, ppl_yarn_demand_reqsn_dtls b ,tmp_reqs_no x WHERE a.requisition_no=b.requisition_no and a.prod_id=b.prod_id and a.requisition_no=x.reqs_no and x.userid=$user_name and a.status_active=1 AND a.is_deleted=0 and b.status_active=1 AND b.is_deleted=0 GROUP BY a.knit_id, a.requisition_no");

				foreach ($demandData as $row)
				{
					$demandDataArr[$row[csf('knit_id')]]['yarn_demand_qnty'] = $row[csf('yarn_demand_qnty')];
				}
		...
		if(!empty($requisition_no_arr))
		{
			...
			//for yarn_IssRtn_arr
			$yarnIssueRtnData = sql_select("SELECT a.requisition_no, a.prod_id, SUM(a.cons_quantity) AS qnty from inv_transaction a, tmp_reqs_no x WHERE a.requisition_no=x.reqs_no and x.userid=$user_name and a.item_category=1 AND a.transaction_type=2 AND a.receive_basis=3 AND a.status_active=1 AND a.is_deleted=0 and a.company_id=".$company_name." group by a.requisition_no, a.prod_id");
		...
		if(!empty($receive_ids_arr))
		{
			...
			$knitting_recv_qnty_array = return_library_array("SELECT a.booking_id, SUM(b.grey_receive_qnty) AS knitting_qnty FROM tmp_recv_mst_id x, inv_receive_master a, pro_grey_prod_entry_dtls b WHERE a.booking_id=x.mst_id and x.userid=$user_name and a.id=b.mst_id AND x.mst_id=b.mst_id AND a.item_category=13 AND a.entry_form=22 AND a.receive_basis=9 AND a.status_active=1 AND a.is_deleted=0 AND b.status_active=1 AND b.is_deleted=0 AND a.company_id=".$company_name." GROUP BY a.booking_id", "booking_id", "knitting_qnty");

			//barcodeData
			$barcodeData = sql_select("SELECT b.mst_id, b.barcode_no FROM tmp_recv_mst_id x, inv_receive_master a, pro_roll_details b WHERE a.id=x.mst_id and x.userid=$user_name and a.id=b.mst_id and x.mst_id=b.mst_id AND a.entry_form=2 AND b.entry_form=2 AND a.item_category=13 AND a.receive_basis=2 AND a.company_id=".$company_name."");
		...
		{
			$deliveryStorQtyArr = array();$deliveryStorQtyArrs = array();
			...
			foreach ($deliveryquantityArr as $row)
			{
				...
				$deliveryStorQtyArrs[$row[csf('booking_no')]]['delivery_qnty'] += $row[csf('current_delivery')];
			}
		}
		...
		$sqlFloor = "SELECT a.dtls_id, b.floor_id FROM ppl_planning_info_machine_dtls a, lib_machine_name b WHERE a.machine_id=b.id ".$floorCond." GROUP BY a.dtls_id, b.floor_id";
		...
		foreach ($nameArray as $row)
		{
			...
			$knitting_delivery_qnty = $deliveryStorQtyArrs[$row[csf('id')]]['delivery_qnty'];
			$knitting_delivery_balance_qnty = $knitting_qnty-$deliveryStorQtyArrs[$row[csf('id')]]['delivery_qnty'];
			...
			$rptPrint[$typeZS][$comZS][$locZS][$floorZS][$diaZS][$pogramZS]['delivery_qnty'] = $knitting_delivery_qnty;
			$rptPrint[$typeZS][$comZS][$locZS][$floorZS][$diaZS][$pogramZS]['delivery_balance_qnty'] = $knitting_delivery_balance_qnty;	
			...
			$rptPrint[$typeZS][$comZS][$locZS][$floorZS][$diaZS][$pogramZS]['demandQty'] = $demandDataArr[$row[csf('id')]]['yarn_demand_qnty'];
			$rptPrint[$typeZS][$comZS][$locZS][$floorZS][$diaZS][$pogramZS]['demandBalance'] = ($reqsDataArr[$row[csf('id')]]['yarn_req_qnty']-$demandDataArr[$row[csf('id')]]['yarn_demand_qnty']);
        ...
        <fieldset style="width:<? echo $tbl_width; ?>px;">
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width; ?>" class="rpt_table" >
                <thead>
					...
					<th width="100">Store Delivery Qty</th>
            ...
            <div style="width:<? echo $tbl_width+18; ?>px; overflow-y:scroll; max-height:450px;" id="scroll_body">
                <table cellspacing="0" cellpadding="0" border="1" rules="all" width="<? echo $tbl_width; ?>" class="rpt_table" id="tbl_list_search">
                    ...
                    foreach($rptPrint as $type=>$typeArr)
                    {
                        ...
                        foreach($typeArr as $companyId=>$companyArr)
                        {
                            ...
                            foreach($companyArr as $locationId=>$locationArr)
                            {
                                ...
                                foreach($locationArr as $floorId=>$floorArr)
                                {
                                    ...
                                    foreach($floorArr as $dia=>$diaArr)
                                    {
                                        ...
                                        foreach($diaArr as $prog=>$row)
                                        {
                                            ...
                                            <tr bgcolor="<?php echo $bgcolor; ?>" valign="middle">
                                                ...
                                                <td width="60" align="center" style="word-break:break-all;">
			                            			<?
			                            			echo $print_program_no = "<a href='##' onclick=\"generate_report(" . $company_name . "," . $row['programNo'] . ")\">" . $row['programNo'] . "</a>";
			                            			?>
			                            		</td>
												...
												<td width="100" align="right"><?php echo number_format($row['delivery_qnty'], 2); ?></td>
												<td width="100" align="right"><?php echo number_format($row['delivery_balance_qnty'], 2); ?></td>
											...
											$totalDiaQty['delivery_qnty'] += number_format($row['delivery_qnty'], 2, '.', '');
											$totalDiaQty['delivery_balance_qnty'] += number_format($row['delivery_balance_qnty'], 2, '.', '');
                                            ...
                                            $totalDiaQty['demandQty'] += number_format($row['demandQty'], 2, '.', '');
                                            $totalDiaQty['demandBalance'] += number_format($row['demandBalance'], 2, '.', '');
											...
											$totalFloorQty['delivery_qnty'] += number_format($row['delivery_qnty'], 2, '.', '');
											$totalFloorQty['delivery_balance_qnty'] += number_format($row['delivery_balance_qnty'], 2, '.', '');
                                            ...
                                            $totalFloorQty['demandQty'] += number_format($row['demandQty'], 2, '.', '');
                                            $totalFloorQty['demandBalance'] += number_format($row['demandBalance'], 2, '.', '');
											...
											$totalLocationQty['delivery_qnty'] += number_format($row['delivery_qnty'], 2, '.', '');
											$totalLocationQty['delivery_balance_qnty'] += number_format($row['delivery_balance_qnty'], 2, '.', '');
                                            ...
                                            $totalLocationQty['demandQty'] += number_format($row['demandQty'], 2, '.', '');
                                            $totalLocationQty['demandBalance'] += number_format($row['demandBalance'], 2, '.', '');
                                            ...
                                            $totalCompanyQty['balance'] += number_format($row['demandBalance'], 2, '.', '');

											$totalCompanyQty['delivery_qnty'] += number_format($row['delivery_qnty'], 2, '.', '');
											$totalCompanyQty['delivery_balance_qnty'] += number_format($row['delivery_balance_qnty'], 2, '.', '');
                                            ...
                                            $totalCompanyQty['demandQty'] += number_format($row['demandQty'], 2, '.', '');
                                            $totalCompanyQty['demandBalance'] += number_format($row['demandBalance'], 2, '.', '');
											...
											$totalQty['delivery_qnty'] += number_format($row['delivery_qnty'], 2, '.', '');
											$totalQty['delivery_balance_qnty'] += number_format($row['delivery_balance_qnty'], 2, '.', '');
                                            ...
                                            $totalQty['demandQty'] += number_format($row['demandQty'], 2, '.', '');
                                            $totalQty['demandBalance'] += number_format($row['demandBalance'], 2, '.', '');
                                        ...
                                        ?>
                                        <tr bgcolor="#C6D9C9" style="font-weight:bold;">
                                            <td align="right" colspan="<?php echo $colspan-13; ?>">Sub Total: </td>
                                            <?php
									...
									if($type == 'Inside')
									{
										?>
										<tr bgcolor="#C6D9C9" style="font-weight:bold;">
											<td align="right" colspan="<?php echo $colspan-13; ?>">Floor Total: </td>
											<?php
                                ...
                                if($type == 'Inside')
								{
									?>
									<tr bgcolor="#C6D9C9" style="font-weight:bold;">
										<td align="right" colspan="<?php echo $colspan-13; ?>">Location Total: </td>
										<?php
                            ...
                            ?>
                            <tr bgcolor="#C6D9C9" style="font-weight:bold;">
                                <td align="right" colspan="<?php echo $colspan-13; ?>">Company Total: </td>
                                <?php
                ...
                <tfoot>
                    <tr bgcolor="#C6D9C9" style="font-weight:bold;">
                        <th align="right" colspan="<?php echo $colspan-13; ?>">Grand Total: </th>
                        <?php
