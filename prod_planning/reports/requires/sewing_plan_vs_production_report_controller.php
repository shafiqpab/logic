...
if ($action=="load_drop_down_lc_buyer")
{
	echo create_drop_down( "cbo_buyer_name", 140, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,90)) $buyer_cond group by buy.id,buy.buyer_name order by buyer_name","id,buyer_name", 1, "- All Buyer -", $selected, "load_drop_down('requires/sewing_plan_vs_production_report_controller', this.value, 'load_drop_down_brand', 'brand_td' );load_drop_down('requires/sewing_plan_vs_production_report_controller', this.value, 'load_drop_down_season', 'season_td' );" );
...
{ 
	...
	//--------------------------------------------------------------------------------------------------------------------
		...
		if($cbo_lc_company_id){$where_con=" and a.COMPANY_NAME=$cbo_lc_company_id ";}
		if($cbo_brand_id){$where_con.=" and a.BRAND_ID=$cbo_brand_id ";}
		if($cbo_season_id){$where_con.=" and a.SEASON_BUYER_WISE=$cbo_season_id ";}
		if($cbo_season_year){$where_con.=" and a.SEASON_YEAR=$cbo_season_year ";}
		...
		foreach( $data_result as $row)
		{
			...
			$poline[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]=$row[csf("line_id")];
			
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['po']=$sewing_data;
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['plan_id']=$row[csf("plan_id")];
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['plan_qnty']=$row[csf("plan_qnty")];
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['start']=$row[csf("start_hour")];
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['end']=$row[csf("end_hour")];
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['duration']=$row[csf("duration")];
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['comp']=$row[csf("comp_level")];
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['first_day']=$row[csf("first_day_output")];
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['increment']=$row[csf("increment_qty")];
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['terget']=$row[csf("terget")];
			
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['off_day']=$row[csf("off_day_plan")];
			
			
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['lead_day']=$row[csf("date_diff")];
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['buyer']=$row[csf("buyer_name")];
			
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]][date("M-d",strtotime($row[csf("plan_date")]))]['day_wise']=$row[csf("pdplan_qnty")];
			
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['pdplan_qntyLine']=$row[csf("plan_qnty")];
			
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['po_id'][$row[csf("po_id")]]=$row[csf("po_id")];
			
			
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['po_number'][$row[csf("po_number")]]=$row[csf("po_number")];
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['style'][$row[csf("style")]]=$row[csf("style")];
			
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['job'][$row[csf("job_no_prefix_num")]]=$row[csf("job_no_prefix_num")];
			$sewing_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("plan_id")]][$row[csf("line_id")]]['pub_shipment']=$row[csf("pub_shipment_date")];


			$po_qty_data_arr[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("po_id")]][$row[csf("line_id")]]['po_qty']=$row[csf("po_quantity")];			
			
			$item_id_data[$row[csf("company_id")]][$row[csf("location_id")]][$row[csf("po_id")]][$row[csf("line_id")]]['item_id']=$row[csf("item_number_id")];
		...
		$end_date_db=date("Y-m-d",max($date_found));
			...
			$sewingProSql= "select serving_company as company_id,po_break_down_id,location,production_date,item_number_id,sewing_line,prod_reso_allo,production_type,production_quantity as prod_quantity from pro_garments_production_mst where serving_company in($company_id) and status_active=1 and is_deleted=0 and production_type in(5,4) ".where_con_using_array($poid,0,'po_break_down_id')." $location_id_cond_sewing";
			//echo $sewingProSql;die;
			$tot_sewing_prod=sql_select($sewingProSql);
			...
			$sewingSql="select company_id,po_break_down_id,location, item_number_id, sum(production_quantity) as production_quantity from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type=5 ".where_con_using_array($poid,0,'po_break_down_id')."   group by company_id,po_break_down_id, item_number_id,location";
			//echo $sewingSql;die;
			$sql_sewing_prod=sql_select( $sewingSql );//Output qty//Actual Qty
					...
					<div style="width:<? echo $width+20; ?>px; overflow-y:scroll; max-height:330px;" id="scroll_body">
							<table width="<? echo $width; ?>" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table">
								...
								foreach($sewing_data as $company_key=>$unit_arr)
								{
									foreach($unit_arr as $unit_key=>$sewing_data_arr) 
									{
										...
										foreach($sewing_data_arr as $plan_id=>$linedata)
										{
											$po=0;
											$prod_qty=0;
											...
											foreach($linedata as $lineid=>$pdata)
											{
												...
												$po++;
												if ($po==1){ $tdbgcolor=$bgcolor;}


												$k++;

												$po_qty=$item_id=$prod_qty=$production_days=0;
												foreach($sewing_data[$company_key][$unit_key][$plan_id][$lineid]['po_id'] as $po_id){
												
													
												
												$item_id=$item_id_data[$company_key][$unit_key][$po_id][$lineid]['item_id'];
												...
												$po_qty+=$po_qty_data_arr[$company_key][$unit_key][$po_id][$lineid]['po_qty'];
												...
												<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $j; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $j; ?>"> 
													...
													<td width="90" rowspan="3" align="center" style="background-color: <? if($po !=1){echo $tdbgcolor;} ?>; color:<? if($po !=1){echo $tdbgcolor;}  ?>" >
														<p>
															<? 
															echo $buyer_arr[$pdata['buyer']].'<br>'.implode(',',$pdata['style']).'<br>'.implode(',',$pdata['job']); 
															?> 
														</p>
													</td>
													<td width="90" rowspan="3" align="center" style="background-color: <? if($po !=1){echo $tdbgcolor;} ?>; color:<? if($po !=1){echo $tdbgcolor;}  ?>" >
														<p>
															<? 
															echo implode(',',$pdata['po_number']).'<br>'.change_date_format($pdata['pub_shipment']).'<br>'.$pdata['lead_day'].' Days';
															?> 
														</p>
													</td>
													...
													<td width="50" rowspan="3" bgcolor="<? echo $color; ?>" style="background-color: <? if($po !=1 && $color==""){echo $tdbgcolor;}else{echo $color;}  ?>; color:<? if($po ==1){echo '';} if($po !=1 && $color!=""){echo $color;}if($po !=1 && $color==""){echo $tdbgcolor;}  ?>" title="Early/Late By=(publish date - (((po qty - production qty)/daily production) + current date))-2" align="center" > 
														<? echo $late_early; ?> 
													</td>
													...
													<td width="80" rowspan="3"  title="Actaul Total"  align="right">
														...
														for($p=0;$p<$num_days;$p++)
														{
															...
															foreach($sewing_data[$company_key][$unit_key][$plan_id][$lineid]['po_id'] as $po_id){
															$item_id=$item_id_data[$company_key][$unit_key][$po_id][$lineid]['item_id'];
															$is_actual+=$total_sewing_data_arr[$company_key][$unit_key][$po_id][$lineid][$item_id][$pdate]['prod_qty'];
															
															
															
															if($sewing_data[$company_key][$unit_key][$plan_id][$lineid][$pdate]['day_wise']){
														...
														$tot_line_plan+=$sewing_data[$company_key][$unit_key][$plan_id][$lineid]['pdplan_qntyLine'];
														echo $sewing_data[$company_key][$unit_key][$plan_id][$lineid]['pdplan_qntyLine']."<br/>".number_format($actual_qty_tot); 
														?> 
													</td>
													...
													for($m=0;$m<$num_days;$m++)
													{
														...
														<td align="right" width="80">
															...
															$ddate=add_date($start_date_db,$m);
															$pdate=date("M-d",strtotime($ddate));
															$duration=$sewing_data[$company_key][$unit_key][$plan_id][$lineid][$pdate]['duration'];
															$plan_qty=$sewing_data[$company_key][$unit_key][$plan_id][$lineid][$pdate]['day_wise'];
															...
															else
															{
																...
																$date_plan_qty_arr[$pdate]+=$plan_qty;
															}
															?>
														</td>
														<?
												...
												<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tract_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="tract_<? echo $k; ?>">
													...
													for($m=0;$m<$num_days;$m++)
													{
														...
														$item_id=$actual_qty=0;$plan_qty=0;
														foreach($sewing_data[$company_key][$unit_key][$plan_id][$lineid]['po_id'] as $po_id){
														...
														$plan_qty+=$sewing_data[$company_key][$unit_key][$plan_id][$lineid][$pdate]['day_wise']*1;
														...
														<td align="right" width="80">
															...
															if($actual_qty==0 || $actual_qty=="" || $plan_qty==0)
															{
																echo "";
															}
															else
												...
												<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trvari_<? echo $k; ?>','<? echo $bgcolor; ?>')" id="trvari_<? echo $k; ?>">
													...
													for($m=0;$m<$num_days;$m++)
													{
														...
														$plan_qty=$sewing_data[$company_key][$unit_key][$plan_id][$lineid][$pdate]['day_wise']*1;
														$item_id=$item_id_data[$company_key][$unit_key][$po_id][$lineid]['item_id'];
														$actual_qty+=$total_sewing_data_arr[$company_key][$unit_key][$po_id][$lineid][$item_id][$pdate]['prod_qty']*1;
														}
														?>
														<td align="right" width="80">
															<? 
															$tot_variance=$actual_qty-$plan_qty;
															if($tot_variance==0 || $tot_variance=="" || $plan_qty==0)
															{
																echo "";	
															}
															else
											...
											<tr class="tbl_bottom">
												...
												<td width="80">
													<? 
													echo number_format($tot_line_plan)."<br>";
													$plan_qty_tot_unit +=$tot_line_plan;
													$plan_qty_tot_grnd +=$tot_line_plan; 
													$tot_line_plan=0;
										...
										<tr class="tbl_bottom">
											...
											<td width="80">
												<? 
												echo number_format($plan_qty_tot_unit,0)."<br>";
												$plan_qty_tot_unit=0;  
		                    ...
		                    {  
		                    	...
		                    	<tr class="tbl_bottom">
		                    		...
		                    		<td width="80">
										<? echo number_format($plan_qty_tot_grnd,0);?><br>
                                        <? echo number_format($actual_qty_tot_grnd,0);?>
                                    </td>
		                    		<td width="70"></td>
		                    		<td width="100"></td>
		                    		<?
		                    		for($m=0;$m<$num_days;$m++)
		                    		{
		                    			...
		                    			<td width="80" align="right"> 
		                    				<? echo number_format($date_plan_qty_arr2[$pdate]); ?><br>
                  ...
                  //ppl_sewing_plan_board
					...
					if($cbo_date_type==2)
					{
						$sql_unplan=("select $lead_day, b.id as po_id,b.pub_shipment_date,b.po_quantity,b.po_number,a.buyer_name,a.job_no_prefix_num,a.style_ref_no as style 
							from  wo_po_break_down b,wo_po_details_master a where  a.job_no=b.job_no_mst   and a.WORKING_COMPANY_ID=$company_id and b.id not in(select a.po_break_down_id  from ppl_sewing_plan_board a,ppl_sewing_plan_board_dtls d where  a.plan_id=d.plan_id and status_active=1 $date_cond ) and b.status_active=1 and b.is_deleted=0 $where_con  $buyer_id_cond $po_cond $job_no_cond $location_id_cond ");
					}
					else
					{
						$sql_unplan=("select $lead_day, b.id as po_id,b.pub_shipment_date,b.po_quantity,b.po_number,a.buyer_name,a.job_no_prefix_num,a.style_ref_no as style 
							from  wo_po_break_down b,wo_po_details_master a where  a.job_no=b.job_no_mst   and a.WORKING_COMPANY_ID=$company_id and b.id not in(select a.po_break_down_id  from ppl_sewing_plan_board a where status_active=1 ) and b.status_active=1 and b.is_deleted=0 $where_con  $buyer_id_cond $po_cond $date_cond $job_no_cond $location_id_cond ");
					}
