...
if($_SESSION['logic_erp']['graph_id']==4){
	$type=0;
}else{
	$type=1;
}
...
<?
	...
	if( $_SESSION['logic_erp']["data"]=="")
	{
		...
		$exFactory_arr=array();
		$data_arr=sql_select( "select po_break_down_id, country_id, sum(CASE WHEN entry_form!=85 THEN ex_factory_qnty ELSE 0 END) as ex_factory_qnty, sum(CASE WHEN entry_form=85 THEN ex_factory_qnty ELSE 0 END) as ex_factory_return_qnty from pro_ex_factory_mst where status_active=1 and is_deleted=0 group by po_break_down_id, country_id");
		foreach($data_arr as $row)
		{
			$exFactory_arr[$row[csf('po_break_down_id')]][$row[csf('country_id')]]=$row[csf('ex_factory_qnty')]-$row[csf('ex_factory_return_qnty')];
		}
		...
		foreach($yr_mon_part as $key=>$val)
		{
			if($type==1){
				if($db_type==0) 
				{
					...
					where b.job_no_mst=c.job_no and c.company_name in($manufacturing_company) and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and $dateField like '".$val."-%"."' group by b.id";
				}
				else
				{
					$sql="select b.id as po_id, c.total_set_qnty as ratio, b.unit_price,
					...
					where b.job_no_mst=c.job_no and c.company_name in($manufacturing_company) and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and to_char($dateField,'YYYY-MM-DD') like '".$val."-%"."' group by b.id, c.total_set_qnty,b.unit_price";	 
				}
			}
			else
			{
				if($db_type==0) 
				{
					$sql="select b.id as po_id, c.total_set_qnty as ratio, b.unit_price, a.country_id, 
					...
					where a.po_break_down_id=b.id and b.job_no_mst=c.job_no and c.company_name in($manufacturing_company) and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and $dateField like '".$val."-%"."' group by b.id, a.country_id";
				}
				else
				{
					$sql="select b.id as po_id, c.total_set_qnty as ratio, b.unit_price, a.country_id, 
...
// echo $sql;
		...
		{
			if($i!=$tot_month) $catg .="'".date("M",strtotime($yr_mon_part[$i]))."',"; else $catg .="'".date("M",strtotime($yr_mon_part[$i]))."']";
		}
		...
		foreach($sql_comp as $row_comp)
		{
			...
			for($i=0;$i<=$tot_month;$i++)
			{
				...
				if($type==1){
					$sql="select b.id as po_id, c.total_set_qnty as ratio, b.unit_price, sum(b.po_total_price) AS povalue, sum(b.po_quantity*c.total_set_qnty) AS poqty from wo_po_break_down b, wo_po_details_master c where b.job_no_mst=c.job_no and c.company_name='".$row_comp[csf('id')]."' and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and $year_field like '".$yr_mon_part[$i]."-%"."' group by b.id, c.total_set_qnty,b.unit_price";
				}
				else{
		...
		foreach($tot_for_graph as $key=>$value )
		{
			...
			if( $i!=$tot_month)  $data_qnt_stck .=number_format( $conf_tot_for_graph_stack[$key],0,'.','').","; else $data_qnt_stck .=number_format( $conf_tot_for_graph_stack[$key],0,'.','')."";
			if( $i!=$tot_month)  $data_val_stck .=number_format( $conf_tot_for_graph_stack_val[$key],0,'.','').","; else $data_val_stck .=number_format( $conf_tot_for_graph_stack_val[$key],0,'.','')."";
		}
		...
		foreach($tot_for_graph as $key=>$value )
		{
			if( $i!=$tot_month)  $data_qnt_stck .=number_format( $proj_tot_for_graph_stack[$key],0,'.','').","; else $data_qnt_stck .=number_format( $proj_tot_for_graph_stack[$key],0,'.','')."";
			if( $i!=$tot_month)  $data_val_stck .=number_format( $proj_tot_for_graph_stack_val[$key],0,'.','').","; else $data_val_stck .=number_format( $proj_tot_for_graph_stack_val[$key],0,'.','')."";
		}
		$data_qnt_stck .="], stack: 'conf'}] ";
