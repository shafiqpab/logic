...
class condition{
	 ...
	 protected $po_status_active_cond =' and b.status_active=1';
	 protected $po_is_deleted_cond =' and b.is_deleted=0';
	 ...
	 public function season($value){
		 $this->season_cond=' and a.season '.$value;
	 ...
	 public function po_id_in($value){
		//$poIds=chop($value,','); 
		$po_cond_in="";
		$po_ids=array_flip(array_flip(explode(",",rtrim($value,','))));
		//$po_ids=count(array_unique(explode(",",$value)));
		//echo $po_ids;
		if(count($po_ids)>1000)
		{
			$po_cond_in=" and (";
			$poIdsArr=array_chunk($po_ids,999);
	 ...
	 public function po_status_active($value){
		 $this->po_status_active_cond=' and b.status_active '.$value;
		 //$this->cond.=$this->po_shiping_status_cond;
		 return $this;
	 }
	  public function po_is_deleted($value){
		 $this->po_is_deleted_cond=' and b.is_deleted '.$value;
		 //$this->cond.=$this->po_shiping_status_cond;
		 return $this;
	 }
	...
	public function cutup_date($value){
	 ...
	 public function getCond(){
		  $this->cond.=$this->po_status_active_cond;
		  $this->cond.=$this->po_is_deleted_cond;
		  return $this->cond;
	 }
	...
	public  function _setGmtsitemRatioArr(){
		$gmtsitemRatioSql=$this->sql_select('select a.job_no AS "job_no",b.gmts_item_id AS "gmts_item_id" ,b.set_item_ratio AS "set_item_ratio" from wo_po_details_master a, wo_po_details_mas_set_details b where 1=1 '.$this->getJobTableCond().' and a.id=b.job_id');
		while($gmtsitemRatioSqlRow=oci_fetch_assoc($gmtsitemRatioSql)){
	...
	private function _setCostingPerArr(){
		$costingPerArr=$this->sql_select( 'select a.job_no AS "job_no",b.costing_per AS "costing_per" from  wo_po_details_master a, wo_pre_cost_mst b where 1=1 '.$this->getJobTableCond().' and a.id=b.job_id');
		//$costingPerArrRow=oci_fetch_assoc($costingPerArr)
		while($costingPerArrRow=oci_fetch_assoc($costingPerArr)){
