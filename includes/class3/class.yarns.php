...
class yarn extends report{
	...
	private function _setYarnQty($level){
		...
		while($row=oci_fetch_assoc($this->_dataArray))
		{
			...
			elseif($level=='order_and_construction_gsm_weight_Count_type_wise'){
				$yarnQty[$poId][$construction][$gsm_weight][$typeId][$countId]+=$reqyarnqnty;
			}
			else{
				return null;
			}
	...
	public function get_order_and_construction_gsm_weight_Count_type_wise_QtyArray(){
		$jobCountCompositionColorAndTypeWiseYarnQty=$this->_setYarnQty('order_and_construction_gsm_weight_Count_type_wise');
		return $jobCountCompositionColorAndTypeWiseYarnQty;
	}
