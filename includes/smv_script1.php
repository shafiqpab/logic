...
$sql=sql_select("select a.id,a.job_no, a.set_smv, b.sew_smv   FROM wo_po_details_master a, wo_pre_cost_mst b WHERE a.job_no=b.job_no  and a.order_uom=1   GROUP BY a.id,a.job_no,a.set_smv,b.sew_smv  order by a.id");
$field_array='set_smv';
$i=0;
foreach($sql as $row){
	if($row[csf('set_smv')] != $row[csf('sew_smv')]){
		$data_array ="".$row[csf('sew_smv')]."";
		$con = connect();
		 $rID=sql_update("wo_po_details_master",$field_array,$data_array,"job_no","'".$row[csf('job_no')]."'",0);
		if($rID ){
			oci_commit($con);   
