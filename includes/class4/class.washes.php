...
class wash extends report{
	...
	private $_By_JobEmbnameAndEmbtypeColorSize='By_JobEmbnameAndEmbtypeColorSize';
	...
	private function _calculateQty($plan_cut_qnty,$costingPerQty,$total_set_qnty,$cons_qnty){
	  //return $reqyarnqnty =def_number_format((($plan_cut_qnty/$total_set_qnty)/$costingPerQty)*$cons_qnty,5,"");
	...
	private function _setQty($level){
		...
		foreach($this->_dataArray as $row)
		{
			...
			elseif($level==$this->_By_JobEmbnameAndEmbtypeColorSize){
				if(isset($Qty[$jobNo][$embnameId][$embtypeId][$colorId][$sizeId])){
					$Qty[$jobNo][$embnameId][$embtypeId][$colorId][$sizeId]+=$reqqnty;
				}
				else{
					$Qty[$jobNo][$embnameId][$embtypeId][$colorId][$sizeId]=$reqqnty;
				}
	...
	private function _setAmount($level){
		...
		foreach($this->_dataArray as $row)
		{
			...
			elseif($level==$this->_By_JobEmbnameAndEmbtypeColorSize){
				if(isset($Amount[$jobNo][$embnameId][$embtypeId][$colorId][$sizeId])){
					$Amount[$jobNo][$embnameId][$embtypeId][$colorId][$sizeId]+=$amount;
				}
				else{
					$Amount[$jobNo][$embnameId][$embtypeId][$colorId][$sizeId]=$amount;
				}
	...
	//Job, Fabric and Process wise Color Size
	//Qty
	public function getQty_by_jobEmbnameAndEmbtypeColorSize($jobNo,$embname,$embtype,$colorId,$sizeId){
		$Qty=$this->_setQty($this->_By_JobEmbnameAndEmbtypeColorSize);
		return $Qty[$jobNo][$embname][$embtype][$colorId][$sizeId];
	}
	
	public function getQtyArray_by_jobEmbnameAndEmbtypeColorSize(){
		$Qty=$this->_setQty($this->_By_JobEmbnameAndEmbtypeColorSize);
		return $Qty;
		
	}
	//Amount
	public function getAmount_by_jobEmbnameAndEmbtypeColorSize($jobNo,$embname,$embtype,$colorId,$sizeId){
		$Amount=$this->_setAmount($this->_By_JobEmbnameAndEmbtypeColorSize);
		return $Amount[$jobNo][$embname][$embtype][$colorId][$sizeId];
	}
	
	public function getAmountArray_by_jobEmbnameAndEmbtypeColorSize(){
		$Amount=$this->_setAmount($this->_By_JobEmbnameAndEmbtypeColorSize);
		return $Amount;
	}
	...
	// Order,Gmts Item, Gmts Color and Emblishment name wise
