...
echo load_html_head_contents("Issue Info","../../", 1, 1, $unicode,1,1); 
?>	
<script>
...
function popup_description()
{
	...
	var cbo_store_name = $("#cbo_store_name").val();
 	var page_link="requires/emb_material_issue_controller.php?action=item_description_popup&company_id="+company_id+"&cbo_store_name="+cbo_store_name+'&item_cat=106';
	...
	emailwindow.onclose=function()
	{
		...
		load_drop_down( 'requires/emb_material_issue_controller', splitArr[4], 'load_drop_down_uom', 'uom_td' );
  	}
}
...
function popup_serial()
{
	...
	var page_link="requires/emb_material_issue_controller.php?action=serial_popup&serialStringNo="+serialStringNo+"&serialStringID="+serialStringID+"&current_prod_id="+current_prod_id+"&txt_received_id="+txt_received_id; 
...
{ 
	...
	var title = 'PO Info';	
	var page_link = 'requires/emb_material_issue_controller.php?company='+company+'&action=order_popup';
...
function generate_report_file(data,action,page)
		{
			window.open("requires/emb_material_issue_controller.php?data=" + data+'&action='+action, true );
		}
...
function fnc_general_item_issue_entry(operation)
{
	if(operation==4)
	 {
		 var report_title=$( "div.form_caption" ).html();
		  ...
		  generate_report_file( $('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+$('#txt_system_no').val()+'*'+report_title,'general_item_issue_print','requires/emb_material_issue_controller');
		// print_report( $('#cbo_company_name').val()+'*'+$('#txt_system_id').val()+'*'+report_title, "general_item_issue_print", "requires/emb_material_issue_controller" ) 
		 return;
	 }
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		if( form_validation('cbo_company_name*cbo_issue_purpose*txt_issue_date*txt_issue_req_no*txt_item_desc*cbo_item_category*txt_issue_qnty*cbo_store_name','Company Name*Issue Purpose*Issue Date*Issue Req No*Item Description*Item Category*Issue Quantity*Store Name')==false )
		{
			return;
		}
...
{	
	if(http.readyState == 4) 
	{
		...
		disable_enable_fields( 'cbo_company_id*cbo_issue_purpose*txt_issue_req_no*cbo_issue_source*cbo_issue_to', 1, "", "" );		 	
 		//$("#tbl_child").find('select,input').val('');	
		reset_form('','','txt_item_desc*txt_buyer_order*txt_order_id*cbo_room*cbo_item_category*cbo_rack*cbo_item_group*txt_serial_no*txt_serial_id*cbo_self*hidden_p_issue_qnty*txt_issue_qnty*txt_current_stock*cbo_binbox*cbo_machine_category*cbo_floor_id*cbo_machine_name*txt_return_qty','','','');   
		show_list_view(reponse[2],'show_dtls_list_view','list_container','requires/emb_material_issue_controller','');
		release_freezing(); 
...
function open_mrrpopup()
{
	...
	var company = $("#cbo_company_name").val();	
	var page_link='requires/emb_material_issue_controller.php?action=mrr_popup&company='+company+'&item_cat=106';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(sys_id, "populate_data_from_data", "requires/emb_material_issue_controller");
		...
		if(posted_in_account==1) 	$("#accounting_posting_td").text("Already Posted In Accounts.");
 		...
 		//list view call here
		show_list_view(sys_id,'show_dtls_list_view','list_container','requires/emb_material_issue_controller','');
		set_button_status(0, permission, 'fnc_general_item_issue_entry',1,1);
 	}
}
...
{
   var status = return_global_ajax_value(company, 'chk_issue_requisition_variabe', '', 'requires/emb_material_issue_controller').trim();
   status = status.split("**");
   if(status[0] == 1)
   {
...
function fnc_items_sys_popup()
{
    ...
    var page_link='requires/emb_material_issue_controller.php?cbo_company_id='+cbo_company_id+'&action=item_issue_requisition_popup_search&item_category_id=106';
    ...
    emailwindow.onclose=function()
    {
        ...
        if(trim(hidden_item_issue_id)!="")
        {
			...
			load_drop_down( 'requires/emb_material_issue_controller',data[4], 'load_drop_down_section', 'section_td' );
			$('#cbo_section').val(data[5]);
			//$('#hidden_indent_date').val(data[2]);
			$('#txt_item_desc').prop('disabled',true);
			show_list_view(data[0]+'__106','show_item_issue_listview','item_issue_listview','requires/emb_material_issue_controller','');
        }
        release_freezing();
    }
}
    ...
    <table width="80%" cellpadding="0" cellspacing="2">
     	<tr>
        	<td width="100%" align="center" valign="top">   
                ...
                <legend>Embroidery Item Issue</legend>
                <br />
                        ...
                        <table  width="950" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                            <tr>
                           	...
                           	<tr>
	                            ...
	                            <td width="170">
	                                <? 
	                                 echo create_drop_down( "cbo_company_name", 170, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond $company_credential_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/emb_material_issue_controller', this.value, 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/emb_material_issue_controller', this.value+'__'+106, 'load_drop_down_store', 'store_td' );load_drop_down( 'requires/emb_material_issue_controller', this.value, 'load_drop_down_department', 'department_td' );chk_issue_requisition_variabe(this.value);" );
	                                ?>
	                            </td>
	                            <td width="120" class="must_entry_caption">Issue Purpose</td>
	                            <td width="160"><? 
	                                 echo create_drop_down( "cbo_issue_purpose", 170, $general_issue_purpose,"", 1, "-- Select Purpose --", $selected, "","","","","","9,10,11,12,13,14,16,17,18,19,20");
	                                ?></td>
                            ...
                            <tr>                           
                                ...
                                <td width="170"><input name="txt_issue_req_no" id="txt_issue_req_no" class="text_boxes" style="width:160px" />
                                ...
                                <td width=""><input type="text" name="txt_remarks" id="txt_remarks" class="text_boxes" style="width:160px"  placeholder="Entry" /></td>
                            </tr>
                            <tr>
                        ...
                        <legend>New Issue Item</legend>                                     
                            <table  width="100%" cellspacing="2" cellpadding="0" border="0">
                                <tr>                               
                                    ...
                                    <td width="140">
                                     	<?
											echo create_drop_down( "cbo_room", 110, "select room from inv_transaction where status_active=1 and room!=0 group by room order by room","room,room", 1, "--Select--", "", "fn_room_rack_self_box()",0 );
										?>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                    	<? echo create_drop_down( "cbo_item_category", 150, $item_category,"", 1, "-- Select --", 1, "load_drop_down( 'requires/emb_material_issue_controller', this.value, 'load_drop_down_itemgroup', 'item_group_td' );", 1,"106" ); ?>
                                    </td>
                                    <td >UOM</td>
                                    <td id="uom_td"><?
                                    ...
                                    <td>
										...
										//echo "select rack from inv_transaction where status_active=1 and rack !=' ' group by rack order by rack";
											echo create_drop_down( "cbo_rack", 110, "select rack from inv_transaction where status_active=1 and rack !=' ' group by rack order by rack","rack,rack", 1, "--Select--", "", "fn_room_rack_self_box()",1 );
										?>
                                    </td>
                                </tr>
                                <tr>
                                    <td >Item Group</td>
                                    <td id="item_group_td">
                                    	<? echo create_drop_down( "cbo_item_group", 150, "select id,item_name from lib_item_group where item_category in(106) and status_active=1 and is_deleted=0 order by item_name", "id,item_name", 1, "-- Select --", 0, "", 1,"" );
                                         ?>
                                    </td>
                                    <td >Serial No</td>   
                                    ...
                                    <td >Self</td>
                                    <td>
                                    	<? echo create_drop_down( "cbo_self", 110, "select self from inv_transaction where status_active=1 and self !=0 group by self order by self","self,self", 1, "--Select--", "", "fn_room_rack_self_box()",1 );
										?>
                                    </td>
                                </tr>
                                <tr>
                                     ...
                                     <td >Bin/Box</td>
                                     <td>
                                     	<? echo create_drop_down( "cbo_binbox", 110, "select bin_box from inv_transaction where status_active=1 and bin_box !=0 group by bin_box order by bin_box","bin_box,bin_box", 1, "--Select--", "", "",1 );
										?>
                                     </td>
                              	</tr>
                                <tr>
                                    <td >Machine Categ.</td>
                                    <td><? 
                                    	echo create_drop_down( "cbo_machine_category", 150, $machine_category,"", 1, "-- Select --", "", "load_drop_down( 'requires/emb_material_issue_controller', document.getElementById('cbo_company_name').value+'_'+this.value+'_'+document.getElementById('cbo_floor_id').value, 'load_drop_machine', 'machine_td' );load_drop_down( 'requires/emb_material_issue_controller',document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_location').value+'_'+this.value, 'load_drop_down_floor', 'floor_td' );" );
                                    ?>
                                    </td>
                                    <td >Floor</td>
                                    <td id="floor_td">
                                ...
                                <tr>
                                    ...
                                    <td>Origin</td>
                                    <td><?
                                        echo create_drop_down( "cbo_origin", 120, "select country_name,id from lib_country comp where is_deleted=0  and status_active=1 order by country_name",'id,country_name', 1, '--- Select Country ---', 0 ,"",1);            
                                        ?>
                                    </td>
                                   	<td>Model</td>
                                    <td>
