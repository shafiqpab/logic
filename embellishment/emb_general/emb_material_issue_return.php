...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Return Info","../../", 1, 1, $unicode,1,1); 
...
function open_itemdesc()
{
	...
	//var page_link='requires/emb_material_issue_return_controller.php?action=return_number_popup&company='+company; 
 	var page_link='requires/emb_material_issue_return_controller.php?action=itemdesc_popup&company='+company; 
	...
	emailwindow.onclose=function()
	{
		...
		var issue_rtn_qty = return_global_ajax_value(mrrNumber[1], 'issue_rtn_qty', '', 'requires/emb_material_issue_return_controller');
...
function open_returnpopup()
{
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(returnNumber[0], "populate_master_from_data", "requires/emb_material_issue_return_controller");  		
		show_list_view(returnNumber[1]+'**'+returnNumber[0],'show_dtls_list_view','list_container_yarn','requires/emb_material_issue_return_controller','');
...
function fnc_gi_issue_return_entry(operation)
{
	if(operation==4)
	{
		...
		print_report( $('#cbo_company_id').val()+'*'+$('#txt_sys_id').val()+'*'+report_title,"general_item_issue_return_print", "requires/emb_material_issue_return_controller")
		 return;
	}
...
{	
	...
	{	  		
		...
		if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
		{
			...
			show_list_view(reponse[1]+'**'+reponse[3],'show_dtls_list_view','list_container_yarn','requires/emb_material_issue_return_controller','');		
			set_button_status(0, permission, 'fnc_gi_issue_return_entry',1,1);
			reset_form('','','txt_item_description*txt_serial_no*cbo_item_group*txt_return_qnty*cbo_machine_no*txt_remarks*txt_reject_qnty*cbo_uom*cbo_store_name*cbo_item_category','','','');
		}
...
function popup_serial()
{
	...
	var page_link="requires/emb_material_issue_return_controller.php?action=serial_popup&serialStringNo='"+serialStringNo+"'&serialStringID='"+serialStringID+"'+&current_prod_id="+current_prod_id; 
    ...
    <table width="100%" cellpadding="0" cellspacing="2" align="center" id="tbl_master">
     	<tr>
        	<td width="80%" align="center" valign="top">   
                ...
                <legend>Embroidery Item Issue Return</legend>
                <br />
                <fieldset style="width:90%;">                                       
                    <table  width="100%" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                      ...
                      <tr>
                            ...
                            <td width="145">
                                <? 
                                    echo create_drop_down( "cbo_company_id", 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond $company_credential_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/emb_material_issue_return_controller', this.value, 'load_drop_down_location', 'location_td' );load_drop_down( 'requires/emb_material_issue_return_controller', this.value, 'load_drop_down_store', 'store_td' );" );
