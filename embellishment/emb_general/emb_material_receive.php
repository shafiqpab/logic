...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Receive Info","../../", 1, 1, $unicode,1,1);
...
$sql = sql_select("select store_id,exchange_rate,currency_id,max(id) from inv_receive_master where item_category in (126)");
...
function openmypage(page_link,title,itme_category_id)
{
	...
	page_link='requires/emb_material_receive_controller.php?action=wopi_popup&company='+company+'&receive_basis='+receive_basis+'&itme_category_id='+itme_category_id;
	...
	emailwindow.onclose=function()
	{
		...
		if (rowID!="")
		{
			...
			//alert(rowID);
			get_php_form_data(receive_basis+"**"+rowID, "populate_data_from_wopi_popup", "requires/emb_material_receive_controller" );
			show_list_view(receive_basis+"**"+rowID+"**"+itme_category_id,'show_product_listview','list_product_container','requires/emb_material_receive_controller','');
			release_freezing();
...
function popuppage_lc()
{
	...
	var company = $("#cbo_company_name").val();
	var page_link='requires/emb_material_receive_controller.php?action=lc_popup&company='+company;
...
function fn_calile()
{
	...
	var responseHtml = return_ajax_request_value(company+'**'+source+'**'+rate+'**'+item_category+'**'+item_group, 'show_ile', 'requires/emb_material_receive_controller');
	var splitResponse="";
	if(responseHtml!="")
	{
...
function fnc_general_item_receive_entry(operation)
{
	...
	if(operation==4)
	 {
		 var report_title=$( "div.form_caption" ).html();
		 print_report( $('#cbo_company_name').val()+'*'+$('#hidden_mrr_id').val()+'*'+report_title, "general_item_receive_print", "requires/emb_material_receive_controller" )
		 return;
	 }
...
function fnc_general_item_receive_entry_reponse()
{
	if(http.readyState == 4)
	{
		...
		else if(reponse[0]==0)
		{
			...
			show_list_view(reponse[2],'show_dtls_list_view','list_container','requires/emb_material_receive_controller','');
			if(receive_basis==1 || receive_basis==2 || receive_basis==7)
			{
				show_list_view($("#cbo_receive_basis").val()+"**"+$("#txt_wo_pi_req_id").val()+"**"+$("#cbo_item_category").val(),'show_product_listview','list_product_container','requires/emb_material_receive_controller','');
			}
...
function open_mrrpopup()
{
	...
	var company = $("#cbo_company_name").val();
	var page_link='requires/emb_material_receive_controller.php?action=mrr_popup&company='+company+'&item_category=126';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(mrr_id, "populate_data_from_data", "requires/emb_material_receive_controller");
		...
		{
			show_list_view($("#cbo_receive_basis").val()+"**"+$("#txt_wo_pi_req_id").val()+"**126",'show_product_listview','list_product_container','requires/emb_material_receive_controller','');
		}
		set_button_status(0, permission, 'fnc_general_item_receive_entry',1,1);
 	}
}
...
function popup_serial()
{
	...
	var page_link="requires/emb_material_receive_controller.php?action=serial_popup&serialString='"+serialString+"'";
...
function popup_description()
{
	...
	var item_category = $("#cbo_item_category").val();
	var item_group = $("#cbo_item_group").val();
	var page_link="requires/emb_material_receive_controller.php?action=item_description_popup&company_id="+company_id+"&item_category="+item_category+"&item_group="+item_group;
	var title="Item Description Popup";
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=620px,height=320px,center=1,resize=1,scrolling=0','../../')
	emailwindow.onclose=function()
	{
		var theform=this.contentDoc.forms[0];
...
function check_exchange_rate()
{
	...
	var response=return_global_ajax_value( cbo_currercy+"**"+receive_date, 'check_conversion_rate', '', 'requires/emb_material_receive_controller');
...
function independence_basis_controll_function(data)
{
    ...
    $("#cbo_receive_basis option[value='126']").show();
    if(independent_control_arr[data]==1)
    {
        $("#cbo_receive_basis option[value='126']").hide();
    }
}

function open_additional_info_popup()
{
   var title = "Additional Info Details";
   var pre_addi_info = $('#txt_addi_info').val();
   page_link='requires/emb_material_receive_controller.php?action=addi_info_popup&pre_addi_info='+pre_addi_info;
...
<div style="width:100%;">
	    ...
	    <table width="980" cellpadding="0" cellspacing="2" align="left">
	     	<tr>
	        	<td width="80%" align="center" valign="top">
	                ...
	                <legend>Embroidery Material Receive</legend>
	                <br />
	                        ...
	                        <table  width="880" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
	                            <tr>
	                           	...
	                           	<tr>
	                                ...
	                                <td width="170">
	                                <?
	                                echo create_drop_down( "cbo_company_name", 170, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond $company_credential_cond order by company_name","id,company_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/emb_material_receive_controller', this.value, 'load_drop_down_supplier', 'supplier' );load_drop_down( 'requires/emb_material_receive_controller', this.value+'_'+106, 'load_drop_down_store', 'store_td' ); independence_basis_controll_function(this.value);" );
	                                ?>
	                                </td>
	                                ...
	                                <td width="170"><input type="text" name="txt_receive_date" id="txt_receive_date" class="datepicker" style="width:160px;" placeholder="Select Date" onChange="check_exchange_rate();" /></td>
	                            </tr>
	                            <tr>
	                            ...
	                            <tr>
	                                ...
	                                <td id="supplier" >
	                                <?
	                                //echo "select a.id,a.supplier_name from lib_supplier a, lib_supplier_party_type b where a.id=b.supplier_id and  b.party_type in(1,5,6,7,8,30,36,37,39) $supplier_credential_cond and a.status_active=1 and a.is_deleted=0  group by a.id,a.supplier_name order by a.supplier_name";
	                                ...
	                                <td  id="currency">
	                                <? //get_php_form_data(this.value,'load_exchange_rate','requires/emb_material_receive_controller');
	                             ...
	                             <tr>
	                                ...
	                                <td ><input class="text_boxes"  type="text" name="txt_wo_pi_req" id="txt_wo_pi_req" onDblClick="openmypage('xx','Order Search','106')"  placeholder="Double Click" style="width:160px;"  readonly disabled />
	                            ...
	                            <tr>
	                                ...
	                                <td>
	                                    <input type="text" id="txt_additional_info" name="txt_additional_info" class="text_boxes" style="width:160px;" onDblClick="open_additional_info_popup()" placeholder="Double Click" readonly >
			                        ...
			                        <legend>New Receive Item</legend>
				                        <table width="250" cellspacing="2" cellpadding="0" border="0" style="float:left">
				                            <tr>
				                                ...
				                                <td width="130">
				                                    <? echo create_drop_down( "cbo_item_category", 130, $item_category,"", 1, "-- Select --", 0, "load_drop_down( 'requires/emb_material_receive_controller', this.value, 'load_drop_down_itemgroup', 'item_group_td' );", 0,"106" ); ?>
				                                </td>
				                            </tr>
				                            <tr>
				                                <td class="must_entry_caption">Item Group</td>
				                                <td id="item_group_td">
				                                <? echo create_drop_down( "cbo_item_group", 130, "select id,item_name from lib_item_group where item_category in (106) and status_active and is_deleted=0 order by item_name", "id,item_name", 1, "-- Select --", 0, "load_drop_down( 'requires/emb_material_receive_controller', this.value, 'load_drop_down_uom', 'uom_td' );", $disabled,"" ); ?>
				                                </td>
				                            </tr>
				                            <tr>
				                                <td class="must_entry_caption">Item Description.</td>
				                                <td>
				                                    <input name="txt_item_desc" id="txt_item_desc" class="text_boxes" type="text" style="width:120px;" placeholder="Double Click" onDblClick="popup_description();" readonly />
				                                </td>
				                            </tr>
				                            <tr>
				                                <td>UOM</td>
				                        ...
				                        <table width="220" cellspacing="2" cellpadding="0" border="0" style="float:left">
				                            ...
				                            <tr>
				                                <td>Brand</td>
				                                <td><input class="text_boxes"  name="txt_brand" id="txt_brand" type="text" style="width:120px;" readonly disabled/></td>
				                            </tr>
				                        </table>
				                        ...
				                        <table width="190" cellspacing="2" cellpadding="0" border="0">
				                            <tr>
				                                <td width="80">Room</td>
				                                <td width="100"><input class="text_boxes_numeric"  name="txt_room" id="txt_room" type="text" style="width:100px;" /></td>
				                            </tr>
				                            <tr>
				                                <td>Rack</td>
				                                <td><input class="text_boxes"  name="txt_rack" id="txt_rack" type="text" style="width:100px;"  /></td>
				                            </tr>
				                            <tr>
							                  	<td>Self</td>
							                  	<td><input class="text_boxes_numeric"  name="txt_self" id="txt_self" type="text" style="width:100px;"  /></td>
				                            </tr>
				                            <tr>
				                                <td>Bin/Box</td>
				                                <td><input class="text_boxes_numeric"  name="txt_binbox" id="txt_binbox" type="text" style="width:100px;" /></td>
				                            </tr>
				                                <!-- new dev -->
				                            <tr>
				                                <td> Model </td>
				                                <td><input class="text_boxes"  name="txt_model" id="txt_model" type="text" style="width:100px;" readonly disabled/> </td>
				                            </tr>
				                        </table>
			                    	</fieldset>
			                    </td>
    ...
    <div id="list_product_container" style="max-height:500px; width:420px; overflow:auto; float:left; padding-top:5px; margin-top:5px; position:relative;"></div>
