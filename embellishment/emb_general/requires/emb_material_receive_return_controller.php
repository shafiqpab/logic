...
if ($action=="load_room_rack_self_bin")
	{
		load_room_rack_self_bin("requires/emb_material_receive_return_controller",$data);
	}
            ...
            <tbody>
                <tr>
                    ...
                    <td  align="center">
					 ...
					 echo create_drop_down( "cbo_item_category", 170, $item_category,"", 1, "-- Select --", 106, "", 1,"$item_cate_credential_cond","","","" );
...
if($action=="create_mrr_search_list_view")
{
	...
	where b.item_category  in(106) and a.id=b.mst_id and  a.status_active=1 and a.entry_form in(366,369) and b.balance_qnty>0 $sql_cond $suplier_cond $item_category_cond  $mrr_cond $credientian_cond 
...
if($action=="populate_data_from_data")
{
	$sql = "select a.id, a.recv_number, a.company_id, a.supplier_id, a.exchange_rate
			...
			where a.recv_number='$data' and a.entry_form in(366,369) ";
	//echo $sql;
	$res = sql_select($sql);
	foreach($res as $row)
	{
		...
		echo"show_list_view('".$row[csf("id")]."','show_product_listview','list_product_container','requires/emb_material_receive_return_controller','');\n";
   	}	
	exit();	
}
?>
...
{ 
	...
	where b.prod_id=c.id and b.mst_id='$data' and b.transaction_type in(1,4) and b.balance_qnty>0 and b.item_category not in (1,2,3,5,6,7,12,13,14)";
	//echo $sql;
  	$result = sql_select($sql);
	//print_r( $result) ;
        ...
        <caption>Display Received Items </caption>
            ...
            <tbody>
				...
				{ 
					...
					?>
					<tr bgcolor="<? echo $bgcolor; ?>" onClick='get_php_form_data("<? echo $row[csf("tr_id")];?>","item_details_form_input","requires/emb_material_receive_return_controller")' style="cursor:pointer" >
                        <td><? echo $i; ?></td>
...
if($action=="item_details_form_input")
{
	...
	foreach($result as $row)
	{
		//echo "load_room_rack_self_bin('requires/emb_material_receive_return_controller*106', 'store','store_td', '".$row[csf('company_id')]."','"."',this.value);\n";
		...
		echo "load_room_rack_self_bin('requires/emb_material_receive_return_controller', 'floor','floor_td', '".$row[csf('company_id')]."','"."','".$row[csf('store_id')]."',this.value);\n";
		echo "$('#cbo_floor').val('".$row[csf("floor_id")]."');\n";
		echo "load_room_rack_self_bin('requires/emb_material_receive_return_controller', 'room','room_td', '".$row[csf('company_id')]."','"."','".$row[csf('store_id')]."','".$row[csf('floor_id')]."',this.value);\n";
		echo "$('#cbo_room').val('".$row[csf("room")]."');\n";
		echo "load_room_rack_self_bin('requires/emb_material_receive_return_controller', 'rack','rack_td', '".$row[csf('company_id')]."','"."','".$row[csf('store_id')]."','".$row[csf('floor_id')]."','".$row[csf('room')]."',this.value);\n";
		echo "$('#txt_rack').val('".$row[csf("rack")]."');\n";
		echo "load_room_rack_self_bin('requires/emb_material_receive_return_controller', 'shelf','shelf_td', '".$row[csf('company_id')]."','"."','".$row[csf('store_id')]."','".$row[csf('floor_id')]."','".$row[csf('room')]."','".$row[csf('rack')]."',this.value);\n";	
		echo "$('#txt_shelf').val('".$row[csf("self")]."');\n";
		echo "load_room_rack_self_bin('requires/emb_material_receive_return_controller', 'bin','bin_td', '".$row[csf('company_id')]."','"."','".$row[csf('store_id')]."','".$row[csf('floor_id')]."','".$row[csf('room')]."','".$row[csf('rack')]."','".$row[csf('self')]."',this.value);\n";	
...
{	
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		//if( check_table_status( $_SESSION['menu_id'], 1 )==0 ) { echo "15**0**0"; disconnect($con); die;}
 		...
 		if(str_replace("'","",$txt_mrr_retrun_no)!="")
		{
			...
			$data_array_receive="368*".$cbo_company_id."*".$cbo_return_to."*".$txt_return_date."*".$txt_challan_no."*".$txt_received_id."*".$txt_mrr_no."*'".$user_id."'*'".$pc_date_time."'*".$txt_remark."";
		...
		{	 
			...
			//$new_return_number=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '', 'GRR', date("Y",time()), 5, "select issue_number_prefix,issue_number_prefix_num from inv_issue_master where company_id=$cbo_company_id and entry_form=26 $mrr_date_check order by id DESC ", "issue_number_prefix", "issue_number_prefix_num" ));
			
			$id = return_next_id_by_sequence("INV_ISSUE_MASTER_PK_SEQ", "inv_issue_master", $con);
			$new_return_number = explode("*", return_next_id_by_sequence("INV_ISSUE_MASTER_PK_SEQ", "inv_issue_master",$con,1,str_replace("'","",$cbo_company_id),'EBRR',368,date("Y",time())));
			...
			$data_array_receive="(".$id.",'".$new_return_number[1]."','".$new_return_number[2]."','".$new_return_number[0]."',368,".$cbo_company_id.",".$cbo_return_to.",".$txt_return_date.",".$txt_challan_no.",".$txt_received_id.",".$txt_mrr_no.",'".$user_id."','".$pc_date_time."',".$txt_remark.")";
		}
		...
		$data_array1 = "(".$mrrWiseIsID.",".$hidden_receive_trans_id.",".$transactionID.",368,".$txt_prod_id.",".$txt_return_qnty.",".$txt_return_rate.",".$txt_return_value.",'".$user_id."','".$pc_date_time."')";
		...
		{				
			...
			{					
				...
				$data_array1 .= "(".$mrrWiseIsID.",".$issue_trans_id.",".$transactionID.",26,".$txt_prod_id.",".$issueQnty.",".$cons_rate.",".$amount.",'".$user_id."','".$pc_date_time."')";
				//for update
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$data_array_mrr = "(".$mrrWiseIsID.",".$hidden_receive_trans_id.",".$update_id.",368,".$txt_prod_id.",".$txt_return_qnty.",".$txt_return_rate.",".$txt_return_value.",'".$user_id."','".$pc_date_time."')";
		...
		if($before_prod_id==$txt_prod_id)
		{
			...
			$query2 = execute_query("DELETE FROM inv_mrr_wise_issue_details WHERE issue_trans_id=$update_id and entry_form=368",1);
	...
	else if ($operation==2) // Delete Here----------------------------------------------------------
	{
		...
		else 
		{
			...
			{
				//echo "10**select id from inv_mrr_wise_issue_details where prod_id=$product_id and status_active=1 and is_deleted=0 and issue_trans_id=$update_id"; disconnect($con); die;
	...
	<table width="750" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
            ...
            <tbody>
                <tr class="general">                    
                    ...
                    <td align="center">
                        <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $company; ?>, 'create_return_search_list_view', 'search_div', 'emb_material_receive_return_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
                    </td>
            </tr>
        	<tr>                  
...
if($action=="create_return_search_list_view")
{
	...
	$sql = "select a.id, a.issue_number_prefix_num, a.issue_number, a.company_id, a.supplier_id, a.issue_date, a.item_category, a.received_id, a.received_mrr_no, a.is_posted_account   
			...
			where a.id=b.mst_id and b.transaction_type=3 and a.status_active=1 and a.entry_form=368 $sql_cond $credientian_cond 
...
{  
	$sql = "select id,issue_number,company_id,supplier_id,issue_date,challan_no,item_category,received_id,received_mrr_no   
			...
			where id='$data' and entry_form=368";
	//echo $sql;
	$res = sql_select($sql);
	foreach($res as $row)
	{
		...
		if($db_type==0)
		{
			...
			where a.id=b.mst_id  and a.recv_number='".$row[csf("received_mrr_no")]."' and b.transaction_type=1 and b.item_category in (106) group by a.id");
		}
		else
		{
		...
		foreach($sql as $row_t)
		{
			...
			echo"show_list_view('".$row_t[csf("rec_id")]."','show_product_listview','list_product_container','requires/emb_material_receive_return_controller','');\n";
		}
   	}	
	exit();	
}
     	...
     	<table class="rpt_table" border="1" cellpadding="2" cellspacing="0" style="width:850px" rules="all" >
            ...
            <tbody>
				...
				foreach($result as $row){					
					...
					$sqlTr = sql_select("select a.id, b.balance_qnty,b.cons_quantity from inv_receive_master a, inv_transaction b where a.id=b.mst_id and b.prod_id='$pro_id' and b.transaction_type=1 and b.item_category not in (1,2,3,5,6,7,12,13,14) and  a.recv_number='".$row[csf("received_mrr_no")]."'");
 				...
 				?>
                	<tr bgcolor="<? echo $bgcolor; ?>" onClick='get_php_form_data("<? echo $row[csf("id")];?>,<? echo $rcvQnty;?>,<? echo $total_cons_quantity;?>,<? echo $row[csf("company_id")];?>","child_form_input_data","requires/emb_material_receive_return_controller")' style="cursor:pointer" >
...
if($action=="child_form_input_data")
{
	...
	$company= $ex_data[3]; 
 	$sql = "select c.id as prod_id, c.item_description, c.item_group_id, c.current_stock,b.company_id, b.id as tr_id, b.item_category, b.store_id,b.floor_id,b.room,b.rack,b.self, b.cons_uom, b.rcv_rate as cons_rate, b.cons_quantity, b.rcv_amount as cons_amount, a.received_id, a.remarks
			from  inv_issue_master a, inv_transaction b, product_details_master c
 			where a.id=b.mst_id and b.prod_id=c.id and b.id=$data2 and b.status_active=1 and b.transaction_type=3 and b.item_category not in (1,2,3,5,6,7,12,13,14) ";
 	//echo $sql;die;
	$store_name=return_library_array("select id,store_name from lib_store_location", "id","store_name"); 
	$item_name_arr=return_library_array("select id,item_name from lib_item_group", "id","item_name");
	$result = sql_select($sql);
	foreach($result as $row)
	{
		...
		echo "load_drop_down('requires/emb_material_issue_return_controller', '".$company."','load_drop_down_store', 'store_td');\n";
		echo "$('#cbo_store_name').val('".$row[csf("store_id")]."');\n";
		echo "$('#store').val('".$row[csf("store_id")]."');\n";
...
<?
	...
	foreach($sql_result as $row)
	{
        ...
        <br>
		 <?
            echo signature_table(186, $data[0], "900px");
         ?>
      </div>
   </div> 
 <? 
