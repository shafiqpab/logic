...
if ($action=="load_drop_down_department")
{
	...
	echo create_drop_down( "cbo_department", 120, "select a.id,a.department_name from  lib_department a, lib_division b where b.id=a.division_id and a.status_active =1 and a.is_deleted=0 and b.company_id='$data' order by department_name","id,department_name", 1, "-- Select --", $selected, "load_drop_down( 'requires/emb_material_issue_controller', this.value, 'load_drop_down_section', 'section_td' );",0 );     	 
	exit();
}
...
{	   
	...
	echo create_drop_down( "cbo_store_name", 130, "select a.id,a.store_name from lib_store_location a, lib_store_location_category b where a.id=b.store_location_id and b.category_type in ($data[1]) $store_location_credential_cond and a.status_active=1 and a.is_deleted=0 and a.company_id=$data[0] group by a.id,a.store_name order by a.store_name","id,store_name", 1, "-- Select --", 0, "","" ); 	 
...
{
	echo create_drop_down( "cbo_item_group", 150, "select id,item_name from lib_item_group where item_category=$data and status_active=1 and is_deleted=0 order by item_name","id,item_name", 1, "-- Select --", 0, "load_drop_down( 'requires/emb_material_issue_controller', this.value, 'load_drop_down_uom', 'uom_td' );","" );  	 
	exit();
}   
...
if ($action=="item_issue_requisition_popup_search")
{
	...
	<body>
		...
		<? 
			$necessity_sql=sql_select("select b.approval_need as approval_need, a.id as max_id from approval_setup_mst a, approval_setup_dtls b 
			where a.id=b.mst_id and a.company_id = $cbo_company_id and b.page_id = 23 and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0 and a.id=(select max(id) as id from approval_setup_mst where company_id = $cbo_company_id and status_active=1)");
	                    ...
	                    <tr>
	                    	<td>
								<?
                                    ...
                                    echo create_drop_down("cbo_company_id",100,$company,"id,company_name",1,"--select--",$cbo_company_id,"load_drop_down( 'emb_material_issue_controller', this.value, 'load_drop_down_location_popup','location_td');",1);
                                 ?>
	                  		</td>
    ...
    load_drop_down( 'emb_material_issue_controller', cbo_company_id, 'load_drop_down_location_popup','location_td_popup');
    </script>
...
if($action=="items_search_list_view")
{
	...
	where a.id=b.mst_id and b.product_id=c.id and a.status_active=1 and a.is_deleted=0 and c.item_category_id in($item_category_id) $remarks $manual_requisition_no $company_id $indent_date $require_date $location_id $division_id $department_id $section_id $sub_section_id $delivery_id $ind_id
...
if ($action=="show_item_issue_listview")
{
	 ...
	 {
		$sql="select b.id, b.mst_id as rid, b.req_qty, b.item_group, b.item_description, b.product_id from inv_itemissue_requisition_dtls b, product_details_master c 
		where b.product_id=c.id and c.item_category_id in($item_cat) and b.mst_id='$req_id' and b.status_active=1 and b.is_deleted=0 ";
	 }
	 else
	 {
		 $sql="select a.id as rid a.itemissue_req_sys_id, department_id, b.mst_id, b.req_qty, b.item_group, b.item_description, b.product_id from inv_item_issue_requisition_mst a,inv_itemissue_requisition_dtls b where b.mst_id=a.id and a.itemissue_req_sys_id='$data' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	 }
	 //echo $sql;
			...
			$item_group=return_library_array("select id,item_name from lib_item_group",'id','item_name');
	         $i=1;
					foreach ($nameArray as $selectResult)
				   	{
	       	 		?>
                    <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="get_php_form_data('<? echo $selectResult[csf('product_id')];?>+**+<? echo $selectResult[csf('req_qty')];?>+**+<? echo $selectResult[csf('rid')];?>','populate_item_details_form_data_dtls','requires/emb_material_issue_controller');" >
...
<script> 
	...
	function open_itemCode_popup()
	{
		...
		var page_link="emb_material_issue_controller.php?action=item_code_popup&cbo_item_category="+cbo_item_category; 
                    ...
                    <td>
                        <?  
                           // $search_by = array(1=>'Return Number');
							...
							//echo create_drop_down( "cbo_item_category", 180, $item_category,"", 1, "-- Select --", 0, "load_drop_down( 'requires/emb_material_issue_controller', this.value, 'load_drop_down_itemgroupPop', 'item_group_td' );", 0,"","","",$item_cate);

						echo create_drop_down( "cbo_item_category", 180, $item_category,"", 1, "-- Select --", $item_cat, "load_drop_down( 'emb_material_issue_controller', this.value, 'load_drop_down_itemgroupPop', 'item_group_td' );load_drop_down( 'emb_material_issue_controller', $company_id+'**'+this.value, 'load_drop_down_store_up', 'store_td' );", 0,$item_cat);
                        ?>
                    </td>
...
if ($action=="create_item_search_list_view")
{
	...
	?>
    <table width="880" cellspacing="0" cellpadding="0" border="1" class="rpt_table" id="" rules="all" >
        <thead>
            <tr>                	 
    ...
    <div style="width:880px; overflow-y:scroll; max-height:250px;font-size:12px; overflow-x:hidden;" id="scroll_body">
    <table width="862" cellspacing="0" cellpadding="0" border="1" class="rpt_table" id="tbl_serial" rules="all">
        <tbody> 
            <?
			...
			$store_id = $ex_data[6];
			$entry_cond="";
			//if(str_replace("'","",$item_category_sql)==106) $entry_cond="and b.entry_form=366";
            if ($item_category_sql!=0) $item_category_sql=" and a.item_category in($item_category_sql) and b.item_category_id in($item_category_sql)"; else { echo "Please Select item category."; die; };
			//echo $item_category;
			...
			$sql="select  b.id as id, (sum(case when a.transaction_type in(1,4,5) then a.cons_quantity else 0 end) - sum(case when a.transaction_type in(2,3,6) then a.cons_quantity else 0 end)) as balance_stock, sum(case when a.transaction_type in(1,4,5) then a.balance_qnty else 0 end) as receive, sum(case when a.transaction_type in(2,3,6) then a.balance_qnty else 0 end) as issue, b.current_stock, b.item_category_id, b.item_group_id,b.sub_group_name,$concat(b.item_description $concat_coma ',' $concat_coma b.item_size) as des, a.store_id as store_id,b.item_code,b.brand_name,b.origin,b.model
            from  inv_transaction a, product_details_master b
            where a.prod_id=b.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.company_id=$company $item_category_sql $item_group $store_name $item_code_cond $entry_cond $prod_cond $store_cond
			group by a.store_id,b.id,b.item_category_id,b.item_group_id,b.sub_group_name,b.item_description, b.item_size,b.current_stock,b.item_code,b.brand_name,b.origin,b.model
			having  (sum(case when a.transaction_type in(1,4,5) then a.cons_quantity else 0 end) - sum(case when a.transaction_type in(2,3,6) then a.cons_quantity else 0 end)) > 0";
           	//echo $sql;
            $itemgroup_arr = return_library_array("select id,item_name from lib_item_group where item_category in (106) and status_active=1 and is_deleted=0",'id','item_name');
            ...
            <tr bgcolor="<? echo $bgcolor; ?>"  onClick='js_set_value("<? echo $row[csf('id')]; ?>*<? echo $row[csf('des')] ;?>*<? echo $row[csf('balance_stock')];  //echo ($row[csf('receive')]-$row[csf('issue')]) ; ?>*<? echo $row[csf('item_category_id')] ; ?>*<? echo $row[csf('item_group_id')] ; ?>*<? echo $row[csf('store_id')] ; ?>*<? echo $row[csf('brand_name')] ; ?>*<? echo $row[csf('origin')] ; ?>*<? echo $row[csf('model')] ; ?>")' id="" style="cursor:pointer">
                ...
                <td align="right" ><?  echo number_format($row[csf('balance_stock')],0); ?></td>
            </tr> 
            <? 
            $i++;
            }
...
if($action=="create_po_search_list_view")
{
	...
	if(trim($year)!=0)
	{
		if($db_type==0) $sql_cond.=" and year(b.shipment_date)='$year'";
		else $sql_cond.=" and to_char(b.shipment_date,'YYYY')='$year'";
	}
...
{	 
	...
	if($req_id!="")
	{
		...
		{
			echo "20**Company must be same of Requisition Company"; disconnect($con); die;
		}
		$trans_id=str_replace("'","",$update_id);
		$up_cond="";
		if($trans_id!="") $up_cond=" and b.id <> $trans_id";
		$prev_req_rcv=sql_select("select sum(b.cons_quantity) as rcv_qnty from inv_issue_master a, inv_transaction b where a.id=b.mst_id and a.entry_form=367 and b.transaction_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.req_id=$req_id and b.prod_id=$current_prod_id $up_cond");
		...
		{
			echo "20**Issue Quantity Not Allow Over Requisition Quantity \n Requisition Balance Quantity=$cu_req_qnty"; disconnect($con); die;
		}
	}
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		//table lock here 	 
		//if( check_table_status( $_SESSION['menu_id'], 1 )==0 ) { echo "15**0"; disconnect($con); disconnect($con); die;}
		...
		if( str_replace("'","",$txt_system_no) == "" ) //new insert
		{
			...
			$new_mrr_number = explode("*", return_next_id_by_sequence("INV_ISSUE_MASTER_PK_SEQ", "inv_issue_master",$con,1,str_replace("'","",$cbo_company_name),'EBMI',367,date("Y",time())));
			...
			$data_array_master="(".$id.",'".$new_mrr_number[1]."','".$new_mrr_number[2]."','".$new_mrr_number[0]."',".$cbo_issue_purpose.",367,".$cbo_company_name.",".$txt_issue_date.",".$txt_challan_no.",".$txt_issue_req_no.",".$txt_remarks.",".$txt_store_sl_no.",'".$user_id."','".$pc_date_time."')";
		...
		$field_array_trans_insert = "id,mst_id,company_id,prod_id,item_category,transaction_type,transaction_date,store_id,order_id,cons_uom,cons_quantity,cons_rate,cons_amount,floor_id,machine_id,item_return_qty,machine_category,room,rack,self,bin_box,location_id,department_id,section_id,use_for,inserted_by,insert_date";
 		$data_array_trans_insert = "(".$transactionID.",".$id.",".$cbo_company_name.",".$txt_prod_id.",".$cbo_item_category.",2,".$txt_issue_date.",".$cbo_store_name.",".$txt_order_id.",".$cbo_uom.",".$txt_issue_qnty.",".$avg_rate.",".$issue_stock_value.",".$cbo_floor_id.",".$cbo_machine_name.",".$txt_return_qty.",".$cbo_machine_category.",".$cbo_room.",".$cbo_rack.",".$cbo_self.",".$cbo_binbox.",".$cbo_location.",".$cbo_department.",".$cbo_section.",".$cbo_use_for.",'".$user_id."','".$pc_date_time."')"; 
		...
		{
			//echo "select (store_method || '_' || allocation) as store_data from variable_settings_inventory where company_name=$cbo_company_name and variable_list=17 and item_category_id=$cbo_item_category and status_active=1 and is_deleted=0";die;
		...
		foreach($sql as $result)
		{
			...
			{					
				...
				$data_array_lifu_fifu .= "(".$mrrWiseIsID.",".$recv_trans_id.",".$transactionID.",367,".$txt_prod_id.",".$issueQnty.",".$txt_return_qty.",".$cons_rate.",".$amount.",'".$user_id."','".$pc_date_time."')";
				//for update
			...
			else if($issueQntyBalance<0)
			{
				...
				//for insert
				if($data_array_lifu_fifu!="") $data_array_lifu_fifu .= ",";  
				$data_array_lifu_fifu .= "(".$mrrWiseIsID.",".$recv_trans_id.",".$transactionID.",367,".$txt_prod_id.",".$balance_qnty.",".$txt_return_qty.",".$cons_rate.",".$amount.",'".$user_id."','".$pc_date_time."')";
				//for update
				$updateID_array_lifu_fifu[]=$recv_trans_id; 
		...
		//echo "10**".$rID." && ".$transID." && ".$mrrWiseIssueID." && ".$upTrID." && ".$prodUpdate." && ".$serialUpdate;die;
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		//product master table information
		//before stock update
		$sql = sql_select( "select a.id,a.avg_rate_per_unit,a.current_stock,a.stock_value, b.cons_quantity,b.item_return_qty, b.cons_amount from product_details_master a, inv_transaction b where a.id=b.prod_id and b.id=$update_id and b.transaction_type=2");
		...
		if($before_prod_id==$txt_prod_id)
		{
			...
			$stock_qnty=return_field_value("sum(case when transaction_type in(1,4,5) then cons_quantity else 0 end) - sum(case when transaction_type in(2,3,6) then cons_quantity else 0 end) as balance_stock","inv_transaction","status_active=1 and prod_id=$before_prod_id and store_id=$cbo_store_name","balance_stock");
		...
		else
		{
			...
			$latest_current_stock=return_field_value("sum(case when transaction_type in(1,4,5) then cons_quantity else 0 end) - sum(case when transaction_type in(2,3,6) then cons_quantity else 0 end) as balance_stock","inv_transaction","status_active=1 and prod_id=$txt_prod_id and store_id=$cbo_store_name","balance_stock");
		...
		$sql = sql_select("select a.id,a.balance_qnty,a.balance_amount,b.issue_qnty,b.rate,b.amount from inv_transaction a, inv_mrr_wise_issue_details b where a.id=b.recv_trans_id and b.issue_trans_id=$update_id and b.entry_form=367");
		...
		$field_array_again = "prod_id*item_category*transaction_type*transaction_date*store_id*order_id*cons_uom*cons_quantity*cons_rate*cons_amount*floor_id*machine_id*item_return_qty*machine_category*room*rack*self*bin_box*location_id*department_id*section_id*use_for*updated_by*update_date";
 		$data_array_again = "".$txt_prod_id."*".$cbo_item_category."*2*".$txt_issue_date."*".$cbo_store_name."*".$txt_order_id."*".$cbo_uom."*".$txt_issue_qnty."*".$avg_rate."*".$issue_stock_value."*".$cbo_floor_id."*".$cbo_machine_name."*".$txt_return_qty."*".$cbo_machine_category."*".$cbo_room."*".$cbo_rack."*".$cbo_self."*".$cbo_binbox."*".$cbo_location."*".$cbo_department."*".$cbo_section."*".$cbo_use_for."*'".$user_id."'*'".$pc_date_time."'"; 
		//echo $field_array."<br>".$data_array;."-".;
		...
		*/$issueQnty = $txt_issue_qnty;
 		...
 		foreach($sql as $result)
		{
			...
			if($issueQntyBalance>=0)
			{
				...
				$update_data_mrr_insert .= "(".$mrrWiseIsID.",".$issue_trans_id.",".$update_id.",367,".$txt_prod_id.",".$issueQnty.",".$txt_return_qty.",".$cons_rate.",".$amount.",'".$user_id."','".$pc_date_time."')";
				//for update
		...
		{
			 $updateIDArray = implode(",",$update_data_trans);
			 $query3 = execute_query("DELETE FROM inv_mrr_wise_issue_details WHERE issue_trans_id=$update_id and entry_form=367",0);
		}
	...
	else if ($operation==2) // Delete Here----------------------------------------------------------
	{
		...
		else 
		{
			...
			{ 
				echo "18**Delete not allowed.This item is used in another transaction"; disconnect($con); die;
			}
			else
			{
...
{
	echo create_drop_down( "cbo_item_category", 150, $item_category,"", 1, "-- Select --", 0, "", 1,"106" );
}
	...
	<table width="780" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
            ...
            <tbody>
                <tr> 
                    ...
                    <td align="center">
                        <input type="button" name="btn_show" class="formbutton" value="Show" onClick="show_list_view (document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $company; ?>+'_'+'<? echo $item_cat; ?>'+'_'+document.getElementById('cbo_year_selection').value, 'create_mrr_search_list_view', 'search_div', 'emb_material_issue_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />				
                    </td>
            </tr>
        	<tr>                  
...
if($action=="create_mrr_search_list_view")
{
	...
	where a.id=b.mst_id and a.entry_form=367 and b.item_category in($item_cat) and a.status_active=1 and b.status_active=1 $sql_cond $year_cond $credientian_cond
...
{
	$sql = "select id, issue_number, issue_purpose, company_id, issue_date, loan_party, challan_no, req_no, remarks,store_sl_no,is_posted_account from inv_issue_master  where id='$data' and entry_form=367";
	//echo $sql;
	$res = sql_select($sql);
	foreach($res as $row)
	{
		...
		{
			echo"load_drop_down( 'requires/emb_material_issue_controller', ".$row[csf("company_id")].", 'load_drop_down_loan_party', 'loan_party_td' );\n";
		...
		//clear child form
		echo "fnc_loan_party('".$row[csf("loan_party")]."');\n";
...
if($action=="show_dtls_list_view")
{
	...
	$po_no_arr=return_library_array( "select id, po_number from wo_po_break_down",'id','po_number');
 	$group_arr=return_library_array( "select id,item_name from lib_item_group where item_category in (106) and status_active=1 and is_deleted=0",'id','item_name');
	...
	$sql = "select a.issue_number, b.id, b.store_id, b.cons_uom, b.cons_quantity, b.machine_category, b.machine_id, b.prod_id, b.location_id, b.department_id, b.section_id, c.item_category_id, c.item_description, c.item_group_id, b.order_id 
			...
			where a.id=b.mst_id and b.prod_id=c.id and b.transaction_type=2 and a.entry_form=367 and b.status_active=1 $cond";
			//echo $sql;
    	...
    	<table class="rpt_table" border="1" cellpadding="2" cellspacing="0" style="width:1000px" rules="all">
            ...
            <tbody>
            	<? foreach($result as $row){  
 				...
 				?>
                	<tr bgcolor="<? echo $bgcolor; ?>" onClick='get_php_form_data("<? echo $row[csf("id")];?>","child_form_input_data","requires/emb_material_issue_controller")' style="cursor:pointer" >
...
if($action=="child_form_input_data")
{
	...
	/*new dev	*/	
	$sql = "select b.id, b.location_id, b.company_id, c.id as prod_id, c.item_description, c.item_category_id, c.item_group_id,b.item_return_qty, b.cons_quantity, c.current_stock, b.store_id, b.cons_uom, b.order_id, b.floor_id, b.machine_id, b.machine_category, b.location_id, b.department_id, b.section_id, b.room, b.rack, b.self, b.bin_box, b.use_for ,c.brand_name,c.origin,c.model
			from inv_transaction b, product_details_master c
			where b.prod_id=c.id and b.id='$rcv_dtls_id' and b.transaction_type=2 and b.item_category in (106)";
	//echo $sql;die;
	$result = sql_select($sql);
	...
	foreach($result as $row)
	{
		...
		echo "load_drop_down( 'requires/emb_material_issue_controller',".$row[csf("company_id")]."+'_'+".$row[csf("location_id")].", 'load_drop_down_floor', 'floor_td' );\n";
		echo "$('#cbo_floor_id').val(".$row[csf("floor_id")].");\n";
		echo "$('#cbo_machine_category').val(".$row[csf("machine_category")].");\n";
		echo "load_drop_down( 'requires/emb_material_issue_controller',".$row[csf("company_id")]."+'_'+".$row[csf("machine_category")]."+'_'+".$row[csf("floor_id")].", 'load_drop_machine', 'machine_td' );\n";
		...
		//echo "load_drop_down( 'requires/emb_material_issue_controller',document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_department').value, 'load_drop_down_section', 'section_td');\n";
		echo "load_drop_down( 'requires/emb_material_issue_controller', ".$row[csf("department_id")].", 'load_drop_down_section', 'section_td' );\n";
...
if ($action=="general_item_issue_print")
{
	...
	$sql=" select a.id, a.issue_number, a.issue_purpose, a.issue_date, a.req_no, a.challan_no, a.remarks,a.store_sl_no, max(b.item_category) as item_category 
	from inv_issue_master a, inv_transaction b 
	where a.id=b.mst_id and a.id=$data[1] and b.transaction_type=2 
	group by a.id, a.issue_number, a.issue_purpose, a.issue_date, a.req_no, a.challan_no, a.remarks,a.store_sl_no";
...
<div style="width:1060px;">
    <table width="980" cellspacing="0" align="right">
        ...
        <tr>
         ...
         <br>
	<div style="width:100%;">
    <table align="right" cellspacing="0" width="1060"  border="1" rules="all" class="rpt_table" >
...
<?
	...
	$i=1;
	$sql_result = sql_select("select a.issue_number, b.id, b.store_id, b.cons_uom, b.cons_quantity, b.floor_id, b.machine_category, b.machine_id, b.prod_id, b.location_id, b.department_id, b.section_id, c.item_category_id, c.item_description, c.item_group_id, b.order_id, b.use_for from inv_issue_master a, inv_transaction b, product_details_master c where a.id=b.mst_id and b.prod_id=c.id and b.transaction_type=2 and b.status_active=1 and a.entry_form=367 $cond");
	...
	foreach($sql_result as $row)
	{
        ...
        <br>
		<?
        	echo signature_table(185, $data[0], "1000px");
        ?>
	</div>
	</div>
