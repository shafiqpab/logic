...
$userCredential = sql_select("SELECT store_location_id,unit_id as company_id,company_location_id,supplier_id,buyer_id FROM user_passwd where id=$user_id and status_active =1 and is_deleted=0");
...
if ($action=="load_drop_down_loan_party")
{
	$party_sql = "SELECT a.id, a.supplier_name from lib_supplier a, lib_supplier_tag_company b 
		where a.id=b.supplier_id and b.tag_company=$data and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.id in(select supplier_id from lib_supplier_party_type where party_type=91) order by supplier_name";
	...
	where a.id=b.supplier_id and b.tag_company=$data and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.id in(select supplier_id from lib_supplier_party_type where party_type=91) order by supplier_name","id,supplier_name", 1, "- Select Loan Party -", $selected, "","","" );
	exit();
}
...
if($action=="create_reqisition_search_list_view")
{
	...
	if($emb_job!="" || $emb_order!="")
	{
		...
		$emb_job_sql="SELECT b.id, a.job_no_prefix_num, b.job_no_mst, b.order_no from subcon_ord_mst a, subcon_ord_dtls b where a.id=b.mst_id and a.entry_form=204 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.company_id=$company $emb_job_cond";
	...
	{
		$emb_job_sql="SELECT b.id, a.job_no_prefix_num, b.job_no_mst, b.order_no from subcon_ord_mst a, subcon_ord_dtls b where a.id=b.mst_id and a.entry_form=204 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.company_id=$company";
		//echo $emb_job_sql;die;
		$emb_job_result=sql_select($emb_job_sql);
		$emb_data=array();
		foreach($emb_job_result as $row)
	...
	$recipe_arr=sql_select("SELECT a.id, a.recipe_no, a.store_id from pro_recipe_entry_mst a where a.company_id=$company and a.entry_form=220 and a.status_active =1 and a.is_deleted=0 and a.id in(".implode(",",$all_recipe_id).") ");
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="790" class="rpt_table" id="tbl_list_search" >
        <tbody>
            ...
            foreach($result_sql as $row)
            {
				...
				?>
				<tr bgcolor="<? echo $bgcolor; ?>" onClick="js_set_value('<? echo $row[csf('requ_no')]."_".$row[csf('id')]."_".$row[csf('recipe_id')]."_".$recipi_no."_".$emb_data[$row[csf("order_id")]]["job_no_mst"]."_".$emb_data[$row[csf("order_id")]]["order_no"]."_".$row[csf("order_id")]."_".$buyer_order_data[$row[csf("buyer_po_id")]]["style_ref_no"]."_".$buyer_order_data[$row[csf("buyer_po_id")]]["po_number"]."_".$row[csf("buyer_po_id")]."_".$store_ids; ?>')">
...
if($action=="item_details")
{
	...
	$store_arr=return_library_array( "SELECT a.id as id,a.store_name  as store_name from lib_store_location a, lib_store_location_category b where a.id=b.store_location_id and a.status_active=1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted =0 and a.company_id=$company_id and b.category_type in(5,6,7,23) group by a.id,a.store_name order by a.store_name", "id", "store_name"  );
	?>
	<div>
            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1120" class="rpt_table" id="tbl_list_search">
                ...
                if($is_update=="")
                {
					if($issue_basis==4)
					{
						...
						foreach ($nameArray as $selectResult)
						{
							...
							<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>">
                                ...
                                <input type="hidden" name="txt_lot[]" id="txt_lot_<? echo $i; ?>" class="text_boxes_numeric"  value="<? echo $selectResult[csf('lot')]; ?>">
                                </td>
					...
					if($issue_basis==7)
					{
						...
						$total_issue_sql=sql_select("SELECT a.issue_number, b.product_id, b.req_qny_edit as req_qny_edit, c.batch_lot 
						from inv_issue_master a, dyes_chem_issue_dtls b, inv_transaction c 
						where a.id=b.mst_id and b.trans_id=c.id and a.entry_form=250 and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0 and a.issue_basis=7 and a.req_id='".$req_id."' and b.sub_process=$sub_process_id");
						foreach($total_issue_sql as $row)
						{
							$total_issue_arr[$row[csf("product_id")]][$row[csf("batch_lot")]]+=$row[csf("req_qny_edit")];
						...
						where a.id=b.product_id and b.mst_id='".$req_id."' and b.multicolor_id=$sub_process_id and b.status_active=1 and b.is_deleted=0 and a.company_id='$company_id' and a.item_category_id in(5,6,7,23,22) and a.status_active=1 and a.is_deleted=0 
						order by a.item_category_id 
						";
						$nameArray=sql_select( $sql );

						$sql_req=sql_select("SELECT a.id, b.required_qnty as required_qnty, b.req_qny_edit, b.item_lot
						from product_details_master a, dyes_chem_issue_requ_dtls b 
						where a.id=b.product_id and b.mst_id='".$req_id."' and b.multicolor_id=$sub_process_id and b.status_active=1 and b.is_deleted=0 and a.company_id=$company_id and a.item_category_id in(5,6,7,23,22) and a.status_active=1 and a.is_deleted=0 ");	
						...
						foreach ($nameArray as $selectResult)
						{
							...
							{
								$stock_qty=$stock_qty_arr[$company_id][$cbo_store][$selectResult[csf('item_category_id')]][$selectResult[csf('id')]][$selectResult[csf('item_lot')]]['stock'];
							}
							
							if(def_number_format($req_qnty,5,"")>0)
							{
								$cu_req_qnty=(($req_data[$selectResult[csf('id')]][$selectResult[csf('item_lot')]]-$total_issue_arr[$selectResult[csf('id')]][$selectResult[csf('item_lot')]])*1);
								?>
                                <tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>"> 
                                    <td width="30" align="center" id="sl_<? echo $i; ?>"><? echo $i; ?></td>
                                    ...
                                    <input type="hidden" name="txt_lot[]" id="txt_lot_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo $selectResult[csf('item_lot')]; ?>">
                                    </td>
                                    <td  width="100"><p><? echo $item_category[$selectResult[csf('item_category_id')]]; ?></p>
                                    <input type="hidden" name="txt_item_cat[]" id="txt_item_cat_<? echo $i; ?>" class="text_boxes_numeric" style="width:38px"  value="<? echo $selectResult[csf('item_category_id')]; ?>">
                                    </td>
                ...
                else
                {
					...
					$sql_store="SELECT a.id, b.store_id, sum((case when transaction_type in(1,4,5) then cons_quantity else 0 end)-(case when transaction_type in(2,3,6) then cons_quantity else 0 end)) as store_stock
					from product_details_master a, inv_transaction b 
					where a.id=b.prod_id and a.company_id='$company_id' and b.store_id=$cbo_store and a.item_category_id in(22) and a.current_stock>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id, b.store_id";
					//echo $sql_store;
					$store_result=sql_select( $sql_store );
					$store_chemical_stock=array();
					foreach($store_result as $row)
					...
					foreach ($nameArray as $selectResult)
					{
						...
						{
							$stock_qty=$stock_qty_arr[$company_id][$cbo_store][$selectResult[csf('item_category_id')]][$selectResult[csf('id')]][$selectResult[csf('batch_lot')]]['stock']+$selectResult[csf('req_qny_edit')];
						}
						...
						if($issue_basis==4)
						{
							...
							<tr align="center" bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>"> 
                                ...
                                <input type="hidden" name="txt_lot[]" id="txt_lot_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo $selectResult[csf('batch_lot')]; ?>">
                                </td>
                                <td  width="100"><p><? echo $item_category[$selectResult[csf('item_category_id')]]; ?></p>
                                <input type="hidden" name="txt_item_cat[]" id="txt_item_cat_<? echo $i; ?>" class="text_boxes_numeric" style="width:38px"  value="<? echo $selectResult[csf('item_category_id')]; ?>">
                                </td>
...
if($action=="create_mrr_search_list_view")
{
	...
	$sql = sql_select("SELECT a.issue_number_prefix_num, a.issue_number, a.company_id, a.issue_date, a.issue_basis, a.issue_purpose, a.batch_no, a.issue_date, a.id, $year_id, a.is_posted_account from inv_issue_master a where a.company_id=$company and a.entry_form=250 $sql_cond  $batch_cond $year_cond and a.status_active =1 and a.is_deleted=0 order by a.id ");
	//echo $sql;
	?>
	<br/>
...
{	  
	...
	if(str_replace("'","",$cbo_issue_basis)==7)
	{
		...
		$sql_req=sql_select("SELECT a.id, b.required_qnty as required_qnty, b.req_qny_edit, b.item_lot 
		from product_details_master a, dyes_chem_issue_requ_dtls b 
		where a.id=b.product_id and b.mst_id='".str_replace("'","",$txt_req_id)."' and b.multicolor_id='".str_replace("'","",$cbo_sub_process)."' and b.status_active=1 and b.is_deleted=0 and a.company_id='".str_replace("'","",$cbo_company_name)."' and a.item_category_id in(5,6,7,23,22) and a.status_active=1 and a.is_deleted=0 $prod_cond");
		$req_data=array();
		foreach($sql_req as $row)
		{
			$req_data[$row[csf("id")]][$row[csf("item_lot")]]=$row[csf("req_qny_edit")];
		...
		for ($i=1;$i<$total_row;$i++)
		{
			...
			$transId="transId_".$i;
			$hidtxt_reqn_qnty_edit="hidtxt_reqn_qnty_edit_".$i;
			$cu_req_qnty=(($req_data[str_replace("'","",$$txt_prod_id)][str_replace("'","",$$txt_lot)]-$prev_issue[str_replace("'","",$$txt_prod_id)][str_replace("'","",$$txt_lot)])*1);
	...
	$product_arr=array();
	$dataArray = sql_select("SELECT id, avg_rate_per_unit, current_stock, stock_value from product_details_master where item_category_id in (5,6,7,23,22) and status_active =1 and is_deleted=0");
	...
	$sql_store = sql_select("SELECT id, prod_id, store_id, category_id as cat_id, rate as avg_rate, cons_qty as current_stock, amount as stock_value, last_purchased_qnty as last_qty, lot from inv_store_wise_qty_dtls where company_id=$cbo_company_name and store_id=$issue_store_id and status_active =1 and is_deleted=0 ");
	...
	foreach($sql_store as $row)
	{
		...
		$store_idarr[$row[csf("prod_id")]][$row[csf("lot")]][$row[csf("store_id")]][$row[csf("cat_id")]]['id']=$row[csf("id")];
		$store_wise_stock_qnty_arr[$row[csf("prod_id")]][$row[csf("lot")]] = $row[csf("current_stock")];
	}
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			$new_system_id = explode("*", return_next_id_by_sequence("INV_ISSUE_MASTER_PK_SEQ", "inv_issue_master",$con,1,$cbo_company_name,'EDCI',250,date("Y",time()) ));
		...
		for ($i=1;$i<$total_row;$i++)
		{
			...
			{
				$sql_store="SELECT a.id, b.store_id, sum((case when transaction_type in(1,4,5) then cons_quantity else 0 end)-(case when transaction_type in(2,3,6) then cons_quantity else 0 end)) as store_stock 	from product_details_master a, transaction b where a.id=b.prod_id and a.company_id=$cbo_company_name and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and b.store_id=$issue_store_id and a.id in(".str_replace("'",'',$$txt_prod_id).")
				group by a.id, b.store_id";
				//echo $sql_store;
				$store_result=sql_select( $sql_store );
			...
			{
				$store_prod_qnty=$store_wise_stock_qnty_arr[str_replace("'",'',$$txt_prod_id)][str_replace("'",'',$$txt_lot)]*1;
			}
			...
			if(str_replace("'",'',$$txt_reqn_qnty_edit)>0)
			{
				...
				$storeid=$store_idarr[$prod_id][$item_lot][$issue_store_id][$cat_id]['id'];
				$store_stock_arr[$storeid][$prod_id][$item_lot][$issue_store_id][$cat_id]['issue_qty']=$txt_reqn_qnty_edit;
		...
		for ($i=1;$i<$total_row;$i++)
		{
			...
			if(str_replace("'",'',$$reqn_qnty_edit)>0)
			{
				...
				{
					$reg_issue_qty=$store_stock_arr[$storeId][$prod_id][$item_lot][$issue_store_id][$cat_id]['issue_qty'];
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		for ($i=1;$i<$total_row;$i++)
		{
			...
			if(str_replace("'",'',$$txt_reqn_qnty_edit)>0)
			{
				...
				$storeid=$store_idarr[$prod_id][$item_lot][$issue_store_id][$cat_id]['id'];
				$store_stock_arr[$storeid][$prod_id][$item_lot][$issue_store_id][$cat_id]['issue_qty']=$txt_reqn_qnty_edit;
				$store_stock_arr[$storeid][$prod_id][$item_lot][$issue_store_id][$cat_id]['hidden_qty']=$hidtxt_reqn_qnty_edit;
		...
		for ($i=1;$i<$total_row;$i++)
		{
			...
			if(str_replace("'",'',$txt_reqn_qnty_e)>0)
			{
				...
				if(in_array($storeId,$storeArrId))
				{
					if(str_replace("'",'',$storeId)!="")
					{
						$req_hidden_qty=$store_stock_arr[$storeId][$prod_id][$item_lot][$issue_store_id][$cat_id]['hidden_qty'];
						$reg_issue_qty=$store_stock_arr[$storeId][$prod_id][$item_lot][$issue_store_id][$cat_id]['issue_qty'];
