...
/*if($action=="create_reqisition_search_list_view_copy")
{
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="790" class="rpt_table" id="tbl_list_search" >
        <tbody>
            ...
            foreach($result_sql as $row)
            {
				...
				?>
				<tr bgcolor="<? echo $bgcolor; ?>" onClick="js_set_value('<? echo $row[csf('requ_no')]."_".$row[csf('id')]."_".$row[csf('recipe_id')]."_".$recipi_no."_".$emb_data[$row[csf("order_id")]]["job_no_mst"]."_".$emb_data[$row[csf("order_id")]]["order_no"]."_".$row[csf("order_id")]."_".$buyer_order_data[$row[csf("buyer_po_id")]]["style_ref_no"]."_".$buyer_order_data[$row[csf("buyer_po_id")]]["po_number"]."_".$row[csf("buyer_po_id")]."_".$store_ids."_".$recipe_for; ?>')">
...
if($action=="create_reqisition_search_list_view")
{
	...
	$sql ="SELECT a.requ_no, a.requ_prefix_num, a.company_id, a.requisition_date, a.requisition_basis, a.recipe_id, a.order_ids, a.id, a.order_id, a.buyer_po_id
	from dyes_chem_issue_requ_mst a where a.company_id=$company and a.is_apply_last_update!=2 and a.entry_form=221 and a.status_active=1 and a.is_deleted=0 and a.recipe_id!=$null_cond $req_cond";
	//echo $sql;
	$result_sql =sql_select($sql);
	foreach($result_sql as $row)
    ...
    <table cellspacing="0" cellpadding="0" border="1" rules="all" width="790" class="rpt_table" id="tbl_list_search" >
        <tbody>
            ...
            foreach($result_sql as $row)
            {
				...
				$buyer_po=""; $buyer_style=""; $order_no=""; $job_no="";
				$order_id=explode(",",$row[csf('order_ids')]);
				foreach($order_id as $po_id)
				{
					...
					if($order_no=="") $order_no=$emb_data[$po_id]['order_no']; else $order_no.=','.$buyer_po_arr[$po_id]['order_no'];
					if($job_no=="") $job_no=$emb_data[$po_id]['job_no_mst']; else $job_no.=','.$buyer_po_arr[$po_id]['job_no_mst'];
				...
				?>
				<tr bgcolor="<? echo $bgcolor; ?>" onClick="js_set_value('<? echo $row[csf('requ_no')]."_".$row[csf('id')]."_".$row[csf('recipe_id')]."_".$recipi_no."_".$job_no."_".$order_no."_".$row[csf("order_id")]."_".$buyer_style."_".$buyer_po."_".$row[csf("buyer_po_id")]."_".$store_ids; ?>')">
                    <td width="30" align="center" title="<? echo $recipe_ids; ?>"><? echo $i; ?></td>
                    <td width="120" align="center"><? echo $row[csf("requ_no")]; ?></td>
                    <td width="70" align="center"><? echo change_date_format($row[csf("requisition_date")]); ?></td>
                    <td width="120" align="center" ><? echo $job_no; //$emb_data[$row[csf("order_id")]]["job_no_mst"]; ?></td>
                    <td  width="120" align="center"><? echo $order_no; //$emb_data[$row[csf("order_id")]]["order_no"]; ?></td>
                    <td width="120" align="center"><? echo $buyer_style; //$buyer_order_data[$row[csf("buyer_po_id")]]["style_ref_no"]; ?></td>
                    <td align=""><? echo $buyer_po; //$buyer_order_data[$row[csf("buyer_po_id")]]["po_number"]; ?></td>
				</tr>
				<? 
				$i++; 
            }
        ...
        <div style="width:1200px; overflow-y:scroll; max-height:230px;" id="buyer_list_view" align="center">
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1180" class="rpt_table" id="tbl_list_search">
                ...
                else
                {
					...
					$sql_store="SELECT a.id, b.store_id, sum((case when transaction_type in(1,4,5) then cons_quantity else 0 end)-(case when transaction_type in(2,3,6) then cons_quantity else 0 end)) as store_stock
					from product_details_master a, inv_transaction b where a.id=b.prod_id and a.company_id='$company_id' and b.store_id=$cbo_store and a.item_category_id in(22) and a.current_stock>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id, b.store_id";
					//echo $sql_store;
					$store_result=sql_select( $sql_store );
					$store_chemical_stock=array();
					foreach($store_result as $row)
...
{	  
	...
	if(str_replace("'","",$cbo_issue_basis)==7)
	{
		...
		$total_issue_sql=sql_select("SELECT a.issue_number, b.product_id, b.req_qny_edit as req_qny_edit from inv_issue_master a, dyes_chem_issue_dtls b where a.id=b.mst_id and a.entry_form=250 and a.issue_basis=7 and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0 and a.req_id='".str_replace("'","",$txt_req_id)."' and b.sub_process='".str_replace("'","",$cbo_sub_process)."' $dtls_cond");
		$prev_issue=array();
		foreach($total_issue_sql as $row)
		{
...
if($action=="chemical_dyes_issue_print")
{
    ...
    <table width="1100" cellspacing="0" align="">
        ...
        <tr>
            ...
            <td><strong>Color</strong></td><td><div style="word-wrap:break-word; width:175px">: <? echo $color_name; //$color_arr[$color_id]; ?></div></td>
        </tr>
         <tr>
