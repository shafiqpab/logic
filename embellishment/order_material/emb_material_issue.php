...
<script>
	...
	{ 
		...
		var page_link='requires/emb_material_issue_controller.php?action=issue_popup&data='+data;
		var title="Issue ID";
		...
		emailwindow.onclose=function()
		{
			...
			if (splt_val[0]!="")
			{
				...
				get_php_form_data( splt_val[0], "load_php_data_to_form", "requires/emb_material_issue_controller" );
				
				var list_view_orders = return_global_ajax_value( splt_val[0]+'**'+splt_val[1]+'**'+1, 'load_php_dtls_form', '', 'requires/emb_material_issue_controller');
				if(list_view_orders!='')
				{
	...
	function job_search_popup()
	{
		...
		{
			var data=document.getElementById('cbo_company_name').value+"_"+document.getElementById('cbo_location_name').value+"_"+document.getElementById('cbo_party_name').value+"_"+document.getElementById('cbo_within_group').value;
			var page_link='requires/emb_material_issue_controller.php?action=job_popup&data='+data;
			...
			emailwindow.onclose=function()
			{
				...
				var list_view_orders = return_global_ajax_value( 0+'**'+theemail+'**'+1, 'load_php_dtls_form', '', 'requires/emb_material_issue_controller');
				if(list_view_orders!='')
				{
					$("#rec_issue_table tr").remove();
					$("#rec_issue_table").append(list_view_orders);
	...
	function report_material_issue(report){

		if(report==1)
		{
				var report_title=$( "div.form_caption" ).html();
				 generate_report_file( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+$('#txt_issue_no').val()+'*'+$('#cbo_party_name').val()+'*'+$('#cbo_location_name').val()+'*'+$('#txt_issue_challan').val()+'*'+$('#cbo_within_group').val()+'*'+$('#txt_job_no').val()+'*'+report_title,'material_issue_print','requires/emb_material_issue_controller');
	...
	function fnc_material_issue_response()
	{
		if(http.readyState == 4) 
		{
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				var list_view_orders = return_global_ajax_value( response[2]+'**'+response[3]+'**'+2, 'load_php_dtls_form', '', 'requires/emb_material_issue_controller');
				if(list_view_orders!='')
				{
					$("#rec_issue_table tr").remove();
					$("#rec_issue_table").append(list_view_orders);
        ...
        <form name="materialissue_1" id="materialissue_1" autocomplete="off">  
            ...
            <table  width="800" cellspacing="2" cellpadding="0" border="0">
                ...
                <tr>
                    ...
                    <td width="150"> 
                        <? echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/emb_material_issue_controller', this.value, 'load_drop_down_location', 'location_td' ); location_select(); load_drop_down( 'requires/emb_material_issue_controller', this.value+'_'+document.getElementById('cbo_within_group').value, 'load_drop_down_buyer', 'buyer_td' ); ");//load_drop_down( 'requires/emb_material_issue_controller', this.value, 'load_drop_down_issueto', 'issue_to_td' ); force_pro_source(); ?>
                    </td>
					...
					<td>
						<?php echo create_drop_down( "cbo_within_group", 150, $yes_no,"", 0, "--  --", 0, "load_drop_down( 'requires/emb_material_issue_controller',document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?>
					</td>
        		</tr>
                <tr>
                ...
                <tr style="display:none">
	                ...
	                <td>
	                     <?
	                       echo create_drop_down( "cbo_source", 150, $knitting_source,"", 1, "-- Select Source --", 1, "load_drop_down( 'requires/emb_material_issue_controller', this.value+'**'+$('#cbo_company_name').val(), 'load_drop_down_company_supplier', 'issue_to_td' );",0,'1,3' );
	                     ?> 
	                </td>
        ...
        <legend>Metarial Details Entry</legend>
            <table cellpadding="0" cellspacing="2" border="1" width="1140" id="details_tbl" rules="all">
             	...
             	<tbody id="rec_issue_table">
                    <tr>
                        <td><input type="hidden" name="ordernoid_1" id="ordernoid_1">
                            ...
                            <input class="text_boxes" name="txtorderno_1" id="txtorderno_1" type="text" style="width:90px" readonly /> <!--onDblClick="job_search_popup('requires/emb_material_issue_controller.php?action=job_popup','Order Selection Form')" -->
                        </td>
             ...
             <table width="1020" cellspacing="2" cellpadding="0" border="0">
				 ...
				 <tr>
					<td align="center">
				       <input type="button"  id="show_button" class="formbutton" style="width:100px; text-align:center;" value="Print"  name="print"  onClick="report_material_issue(1)" />
					</td>
				 </tr>
          	</table>
            </fieldset>
