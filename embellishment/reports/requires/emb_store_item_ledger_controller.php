...
if($action=="create_lot_search_list_view")
{
	...
	if($txt_prod_id!="") $sql_cond.=" and  id=$txt_prod_id";
 	...
 	$sql = "select id,item_group_id,item_description as product_name_details,item_code,item_size from product_details_master where company_id=$company and item_category_id =$cbo_item_cat $sql_cond  and status_active=1 and is_deleted=0"; 
...
{ 
	...
	$sql_transfer = "select a.id as trid,a.transaction_type,b.transfer_system_id, b.company_id, b.to_company
			...
			where a.mst_id=b.id $item_desc_search_cond and a.transaction_type in (5,6) and a.item_category='$cbo_item_cat' and b.transfer_criteria in(1,2) and b.entry_form in()"; //and b.entry_form=57		 //echo $sql_transfer;die;
    ...
    <?
		...
		if($cbo_method==0) //average rate #########################################################################
		{
			...
			from inv_transaction a left join inv_issue_master c on a.mst_id=c.id and a.transaction_type in (2,3), product_details_master b 
			where a.status_active=1 and a.is_deleted=0 $item_desc_search_cond and a.prod_id=b.id  and a.item_category='$cbo_item_cat' and a.company_id=$cbo_company_name $search_cond $store_cond order by  a.prod_id, a.id ASC";		
		...
		else if($cbo_method==2) //LIFU #########################################################################
		{
			$sql = "select a.id, a.prod_id,a.store_id, a.transaction_date, a.transaction_type, a.cons_quantity, a.cons_rate, a.cons_amount,  a.department_id, b.product_name_details, b.unit_of_measure, b.item_group_id, b.lot, c.knit_dye_source, c.knit_dye_company, c.issue_purpose
			from inv_transaction a left join inv_issue_master c on a.mst_id=c.id and a.transaction_type in (2,3), product_details_master b 
			where a.status_active=1 and a.is_deleted=0 and  $item_desc_search_cond and a.prod_id=b.id  and a.item_category='$cbo_item_cat' and a.company_id=$cbo_company_name $search_cond $store_cond order by  a.prod_id, a.id DESC";		
					...
					if(!in_array($row[csf("prod_id")],$checkItemArr))
					{
						...
						if($i!=1) // product wise sum/total here------------
						{
							...
							<tr class="tbl_bottom">
								...
								<!-- product wise herder -->
                            <tr>
                            	<td colspan="16"><b>Product ID : <? echo $row[csf("prod_id")]." , ".$row[csf("product_name_details")].", Item Group# ".$item_group_arr[$row[csf("item_group_id")]].", Item Category#  ".$item_category[$cbo_item_cat].", UOM# ".$unit_of_measurement[$row[csf("unit_of_measure")]]; ?></b></td>
                            </tr>
