...
<script>
	...
	function fnc_load_party(type,within_group)
	{
		...
		{
			load_drop_down( 'requires/emb_bill_issue_controller', company+'_'+1, 'load_drop_down_buyer', 'buyer_td' );
		}
		else if(within_group==2 && type==1)
		{
			load_drop_down( 'requires/emb_bill_issue_controller', company+'_'+2, 'load_drop_down_buyer', 'buyer_td' );
		}
		if(within_group==1)
		{
			load_drop_down( 'requires/emb_bill_issue_controller', party_name, 'load_drop_down_party_location', 'partylocation_td' );
		}
	...
	function fnc_job_no()
	{
		...
		else
		{
			...
			var page_link='requires/emb_bill_issue_controller.php?action=job_popup&data='+data
			...
			emailwindow.onclose=function()
			{
				...
				//alert("job_wise_delivery_id=>"+job_wise_delivery_id);

				var variable_status = $('#txt_variable_status').val();

				var list_view_orders = return_global_ajax_value( 0+'**'+ex_data[0]+'**'+1+'**'+search_type+'**'+delivery_id+'**'+variable_status+'**'+0+'**'+job_wise_delivery_id, 'load_php_dtls_form', '', 'requires/emb_bill_issue_controller');

				// var list_view_orders = return_global_ajax_value( 0+'**'+ex_data[0]+'**'+1, 'load_php_dtls_form', '', 'requires/emb_bill_issue_controller');
				if(list_view_orders!='')
				{
	   ...
	   //alert(variable_status);
		...
		else
		{
			if(operation==4)
			{
				...
				print_report( $('#cbo_company_name').val()+'*'+$('#txt_update_id').val()+'*'+report_title+'*'+$('#cbo_template_id').val()+'*'+$('#cbo_location_name').val(), "embl_bill_issue_print", "requires/emb_bill_issue_controller") 
				//return;
				show_msg("3");
			}
			else
			{
				...
				var data_str=get_submitted_data_string('txt_bill_no*cbo_company_name*cbo_location_name*cbo_within_group*cbo_party_name*cbo_party_location*txt_bill_date*txt_remarks*txtJob_no*txt_update_id*is_posted_account*txt_variable_status*txt_delivery_id*txt_exchange_rate*cbo_currency_id',"../../");

				// var data_str=get_submitted_data_string('txt_bill_no*cbo_company_name*cbo_location_name*cbo_within_group*cbo_party_name*cbo_party_location*txt_bill_date*txt_remarks*txtJob_no*txt_update_id*is_posted_account*txt_exchange_rate*cbo_currency_id',"../../");
				var tot_row=$('#dtls_tbody tr').length;
				 var k=0;
				...
				http.open("POST","requires/emb_bill_issue_controller.php",true);
	...
	function fnc_embl_bill_issue_response()
	{
		if(http.readyState == 4) 
		{
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				var search_type=0;
				var delivery_id=0;
				var list_view_orders = return_global_ajax_value( response[1]+'**'+$('#txtJob_no').val()+'**'+2+'**'+search_type+'**'+delivery_id+'**'+variable_status+'**'+response[3], 'load_php_dtls_form', '', 'requires/emb_bill_issue_controller');
				// var list_view_orders = return_global_ajax_value( response[1]+'**'+$('#txtJob_no').val()+'**'+2, 'load_php_dtls_form', '', 'requires/emb_bill_issue_controller');
				if(list_view_orders!='')
				{
					$("#dtls_tbody tr").remove();
					$("#dtls_tbody").append(list_view_orders);
	...
	{ 
		...
		else
		{
			...
			//alert(data);
			var page_link='requires/emb_bill_issue_controller.php?action=bill_popup&data='+data;
			...
			emailwindow.onclose=function()
			{
				...
				{ 
					...
					//alert(bill_data[0]+'**'+bill_data[18]);
					var variable_status = $('#txt_variable_status').val();
					var search_type=0;
					var delivery_id=0;
					
					var list_view_orders = return_global_ajax_value( bill_data[0]+'**'+bill_data[8]+'**'+1+'**'+search_type+'**'+delivery_id+'**'+variable_status+'**'+bill_data[18], 'load_php_dtls_form', '', 'requires/emb_bill_issue_controller');
					
					// var list_view_orders = return_global_ajax_value( bill_data[0]+'**'+bill_data[8]+'**'+1, 'load_php_dtls_form', '', 'requires/emb_bill_issue_controller');
	...
	function exchange_rate(val)
	{
		...
		else
		{
			...
			var response=return_global_ajax_value( val+"**"+bill_date+"**"+company_name, 'check_conversion_rate', '', 'requires/emb_bill_issue_controller');
	...
	{
		var status = return_global_ajax_value(company, 'chk_qty_level_variable', '', 'requires/emb_bill_issue_controller').trim();
...
<body onLoad="set_hotkey()">
	<div style="width:100%;" align="center">
        ...
        <form name="embbillissue_1" id="embbillissue_1" autocomplete="off"> 
			...
			<legend>Emb Bill Issue</legend>
                <table width="990" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td width="160"><? echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/emb_bill_issue_controller', this.value, 'load_drop_down_location', 'location_td'); fnc_load_party(1,document.getElementById('cbo_within_group').value);chk_qty_level_variabe(this.value)"); ?>
                        </td>
                    ...
                    <tr>
                        ...
                        <td class="must_entry_caption">Exchange Rate</td>
               			 <td><input type="text" name="txt_exchange_rate" id="txt_exchange_rate" style="width:130px" class="text_boxes_numeric"  value=""  readonly/></td>
                    </tr>
            ...
            <legend>Embroidery Bill Details</legend>
                <table style="width:1210px;" cellpadding="0" cellspacing="2" border="1" class="rpt_table" rules="all" id="tbl_dtls" >
                    ...
                    <tbody id="dtls_tbody">
                    	<tr bgcolor="#FFFFFF">
                            ...
                            <td align="center"><input type="text" name="txtbillrate_1" id="txtbillrate_1" class="text_boxes_numeric" style="width:50px;" onBlur="fnc_amount_calculation(this.value,1); fnc_total_calculate();fnc_copy_final_rate(this.id,this.value);" /></td>
