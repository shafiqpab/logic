<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Embroidery QC Entry
Functionality	:	
JS Functions	:
Created by		:	Feroz
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Embroidery QC Entry Info", "../../", 1, 1,'','','');
...
<script>
	...
	function fnc_embel_entry(operation)
	{
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title, "embl_qc_entry_print", "requires/emb_qc_controller") 
			//return;
			show_msg("3");
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			$("#embellishment_details_container").find('tr').each(function()
			{
				...
				{
					j++;
					dataString += '&colorSizeId_' + j + '=' + colorSizeId + '&updateIdDtls_' + j + '=' + updateIdDtls + '&txtRejQty_' + j + '=' + txtRejQty + '&txtQcQty_' + j + '=' + txtQcQty + '&txtPhysicalQty_' + j + '=' + txtPhysicalQty + '&txtremarks_' + j + '=' + txtremarks;
				}
			});
			if(j<1)
			{
			...
			http.open("POST","requires/emb_qc_controller.php",true);
	...
	function openmypage_production()
	{
		...
		{ 	
			...
			var page_link = 'requires/emb_qc_controller.php?cbo_company_id='+cbo_company_id+'&action=production_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if(update_id!="")
				{
					...
					load_drop_down( 'requires/emb_qc_controller', cbo_company_id+'_'+estr_data[6]+'_'+estr_data[5], 'load_drop_down_buyer', 'party_td');
	...
	function fnc_dtls_data_load(production_id,uid)
	{
		...
		var cbo_company_id = $('#cbo_company_id').val();
		var list_view_orders = return_global_ajax_value( cbo_company_id+'***'+production_id+'***'+uid, 'order_details', '', 'requires/emb_qc_controller');
		if(list_view_orders!='')
		{
	...
	function fnc_embel_qc_id()
	{
		...
		{ 
			...
			var page_link = 'requires/emb_qc_controller.php?cbo_company_id='+company_id+'&action=embel_qc_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if(emb_data[0]!="")
				{
					...
					load_drop_down( 'requires/emb_qc_controller', company_id+'_'+emb_data[8]+'_'+emb_data[9], 'load_drop_down_buyer', 'party_td');
	...
	function fnc_calculate_qcqty(incid)
	{
		...
		var txtRejQty=$("#txtRejQty_"+incid).val()*1;
 		//$("#txtQcQty_"+incid).val(prod_qty-txtRejQty);
		//$("#txtQcQty_"+incid).val(prod_qty-txtRejQty);
		var qc_qty_blance=qc_qty+txtRejQty;
		
		var production_balance = $("#txtQcQty_"+incid).attr('placeholder')*1;
		
		if(((qc_qty_blance*1)+production_balance)>prod_qty)
		{
			//alert("Qnty Excceded");
	...
	function fnc_calculate_rejectqty(incid)
	{
		var qc_qty=$("#txtQcQty_"+incid).val()*1;
		var prod_qty=$("#txtProdQty_"+incid).val()*1;
		var txtRejQty=$("#txtRejQty_"+incid).val()*1;
	...
	function show_production()
	{
		...
		{
			show_list_view($('#cbo_company_id').val()+'__'+$('#txt_job_no').val(), 'show_production_no', 'list_production_nos', 'requires/emb_qc_controller', 'setFilterGrid(\'tbl_prod_list_search\',-1);');
		}
	}
	...
	function set_form_data(data)
	{
		...
		load_drop_down( 'requires/emb_qc_controller', cbo_company_id+'_'+data[6]+'_'+data[5], 'load_drop_down_buyer', 'party_td');
	...
	function openmypage_remarks(id)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/emb_qc_controller.php?data='+data+'&action=remarks_popup','Remarks Popup', 'width=420px,height=320px,center=1,resize=1,scrolling=0','../')
...
<body onLoad="set_hotkey(); btn_load_change_production();">
    ...
    <fieldset style="width:900px;">
    	<form name="embellishmentEntry_1" id="embellishmentEntry_1">
            ...
            <fieldset style="width:905px; margin-top:10px" >
            <legend>Emb QC Details Info</legend>
                <table cellpadding="0" cellspacing="0" width="905" class="rpt_table" border="1" rules="all" id="tbl_item_details" align="left">
                    <thead>
                        <tr>
                            <th>&nbsp;</th>
    ...
    <div id="list_production_nos" style="width:370px; overflow:auto; float:left; padding-top:5px; margin-top:5px; margin-left:10px; position:relative;">
