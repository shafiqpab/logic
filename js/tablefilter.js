...
{	
	...
	var rowVisibility, colWidth, bindScript,exFunction;
	...
	f!=undefined && f["ex_function"] ? exFunction=true : exFunction=false;
	...
	if(fltgrid)
	{
		...
		for(var i=0; i<n; i++)// this loop adds filters
		{
			...
			if(f==undefined || f["col_"+i]==undefined || f["col_"+i]=="none") 
			{
				...
				var inp = createElm( "input",["id","flt"+i+"_"+id],["type",inptype],["class",inpclass],["placeholder","Search"]);					
...
{	
	...
	for(var i=0; i<SearchFlt.length; i++)
		SearchArgs.push(trim((grabEBI(SearchFlt[i]).value).toLowerCase()));
			...
			for(var j=0; j<nchilds; j++)// this loop retrieves cell data
			{
				...
				if(SearchArgs[j]!="")
				{
					...
					{						
						...
						try{
							var st_html=document.getElementById('cbo_string_search_type').value;
						...
						var regexp;
						if(ematch || fprops["col_"+j]=="select")
						{
							regexp = new RegExp('(^)'+regexpEscape(SearchArgs[j])+'($)',"gi");
						}
						else 
						{
							regexp =rePattern(st,SearchArgs[j],st_html);
						}
						occurence[j] = regexp.test(trim(cell_data));
						//alert(occurence[j]);
					}
				}//if SearchArgs
			}//for j
	...
	//==============================
	var extFunction=t.tf_exFunction	;	  
	if(extFunction)
	{
		var t = grabEBI(id);
	    var f = t.tf_Obj, n = t.tf_ncells;	
		t.tf_exFunction = f["ex_function"] ;
		if(	t.tf_exFunction!=undefined &&
			t.tf_exFunction["fn_name"]!=undefined )
		{//calls a fn if defined  
			t.tf_exFunction["fn_name"].call(null,id);
		}
	}//if extFunction
	//=============================================
}
...
=====================================================*/
{
  	var t = grabEBI(id);
	var tr;
	if(nrow == undefined || nrow == 1) tr = grabTag(t,"tr")[0];
...
function getColValues(id,colindex,num,skip='none')
/*====================================================
	...
	for(var i=start_row; i<nrows; i++)//iterates rows
	{
		...
		if(nchilds == ncells)// checks if row has exact cell #
		{
			for(var j=0; j<nchilds; j++)// this loop retrieves cell data
			{
				...
				{  
					var cell_data = getCellText( cell[j] ).toLowerCase();  
					var cell_data = cell_data.replace(/\,/g,'');

					if(cell[j].className === skip && skip != 'none' ){continue;}
					//This line for input type text value...................................Developed by REZA
					try {
						for(var cel_i=0;cel_i<cell[j].childNodes.length;cel_i++){
							cell_data = (cell[j].childNodes[cel_i].value != undefined) ? cell[j].childNodes[cel_i].value : cell_data;
							if(cell_data>0){break;}
						}
					  }
					  catch(err) {
...
=====================================================*/
{
	if( hasGrid(id) )
	{
		...
		var skipSumArr =  t.tf_colOperation["skip_sum"];
			...
			&& (typeof operation).toLowerCase()=="object" )
		{
			...
			{
				if( (typeof skipSumArr).toLowerCase() == "object" )
				{
					colvalues.push( getColValues(id,colIndex[k],true,skipSumArr[k]) );
				}
				else{
			...
			for(var i=0; i<colvalues.length; i++)
			{
				...
				{
					$field_type=labelId[i].split('_');

					if($field_type[0]=='value')
					{
...
function rePattern(st,SearchArgs,st_html){
	
	if(st_html==undefined){
		st_html=st;
	}
	//alert(st_html);
	var regexp;
	if(st_html==1){
		regexp = new RegExp("^"+regexpEscape(SearchArgs)+"$");
	}
	else if(st_html==2){
		regexp = new RegExp("^"+regexpEscape(SearchArgs)+"","gi");
	}
	else if (st_html==3){
		regexp = new RegExp(""+regexpEscape(SearchArgs)+"$","gi");
	}
...
/*function rePattern(st,SearchArgs){
	var regexp;
	if(st==1){
	regexp = new RegExp('^'+regexpEscape(SearchArgs)+'',"gi");
	}
