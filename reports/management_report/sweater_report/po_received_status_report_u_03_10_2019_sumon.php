...
<script>
	...
	function generate_report(type)
	{
		...
		if ($("#cbo_company_name").val() > 0 && $("#cbo_year").val() == 0) {	
		...
		if ($("#cbo_working_company_name").val() > 0 && $("#cbo_year").val() == 0) {	
			//$("#curr_date_range").addClass('must_entry_caption');
			//alert("if block");
			if( form_validation('cbo_year','Year')==false )
			{
		...
		var dataString = "&cbo_company_name="+cbo_company_name+"&working_company_name="+cbo_working_company_name+"&cbo_buyer="+cbo_buyer+"&cbo_year="+cbo_year+"&rpt_type="+type;
		//alert(dataString);
        if(type==1)
        {
