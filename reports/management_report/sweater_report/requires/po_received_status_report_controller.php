...
if($action=="generate_report")
{
	...
	if($company_name>0){
		$company_condition = " and a.company_name = '$company_name'";
	}
	...
	if($db_type==0)
	{
		if($cbo_year!=0 || ($from_date!="" && $to_date!=""))
		{
			if($from_date !="" && $to_date !="" )
			{
				...
				if($cbo_shipment_type == 1)
				{
					$date_cond="and b.pub_shipment_date between '$from_date' and '$to_date'";
				}
				else
				{
					$date_cond="and b.po_received_date between '$from_date' and '$to_date'";
				}

				$year_cond=" and YEAR(b.insert_date)=$cbo_year";
				//$ex_factory_date_year_cond=" and c.ex_factory_date between '$from_date' and '$to_date'";
			}
			else
			{
				$year_cond=" and YEAR(b.insert_date)=$cbo_year";
				$date_cond="";
				//$ex_factory_date_year_cond=" and YEAR(c.ex_factory_date)=$cbo_year";
			}
		}
	}
	else if ($db_type==2)
	...
	if($rpt_type==1)
	{
		...
		<h2>Year:<?=date("Y");?></h2>
			<fieldset>
				<table width="1220" cellspacing="0" cellpadding="0" class="rpt_table" border="1" rules="all" id="po_received_status_table">
					<thead>
						<tr>
							...
							<th width="70" rowspan="2">Shipped %</th>
					...
					<tbody id="scroll_body">
						<?
							$i=1; $total_po=0; $total_price=0;
							 ...
							 $sql_data_array = "select a.company_name,a.buyer_name,a.dealing_marchant,a.avg_unit_price, b.unit_price, b.id as po_breakdown_id,b.po_quantity,b.po_total_price,b.pub_shipment_date, (a.total_set_qnty*b.po_quantity) as po_qnty_pcs, (a.total_set_qnty*b.po_total_price) as po_total_price_pcs
							from wo_po_details_master a, wo_po_break_down b
							where a.job_no=b.job_no_mst and b.po_quantity>0 $company_condition $style_owner_cond $buyer_id_cond $dealing_marchant_cond  $date_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.insert_date desc";
							//$year_cond
							...
							foreach ($data_array as $value) {
								...
								$po_receive_data_array[$value[csf('buyer_name')]]['unit_price'] = $value[csf('unit_price')];
								$po_receive_data_array[$value[csf('buyer_name')]]['po_quantity'] += $value[csf('po_quantity')];
								$po_receive_data_array[$value[csf('buyer_name')]]['po_qnty_pcs'] += $value[csf('po_qnty_pcs')];
								$po_receive_data_array[$value[csf('buyer_name')]]['po_total_price'] += $value[csf('po_total_price')];
								$po_receive_data_array[$value[csf('buyer_name')]]['po_total_price_pcs'] += $value[csf('po_total_price_pcs')];
								$po_receive_data_array[$value[csf('buyer_name')]]['po_breakdown_id'] .= $value[csf('po_breakdown_id')].",";

								$order_wise_rate_pcs[$value[csf('po_breakdown_id')]] = $value[csf('po_total_price_pcs')]/$value[csf('po_qnty_pcs')];
								$order_wise_po_arr[$value[csf('po_breakdown_id')]]['qty'] = $value[csf('po_qnty_pcs')];
								$order_wise_po_arr[$value[csf('po_breakdown_id')]]['value'] = $value[csf('po_total_price_pcs')]/$value[csf('po_qnty_pcs')];
								//$po_breakdown_ids[$value[csf('po_breakdown_id')]] = $value[csf('po_breakdown_id')];
								$po_breakdown_ids .= $value[csf('po_breakdown_id')].",";
								$po_wise_buyer[$value[csf('po_breakdown_id')]]=$value[csf('buyer_name')];

								$order_wise_poid_arr[$value[csf('po_breakdown_id')]] = $value[csf('po_breakdown_id')];
							}
							//echo '<pre>';print_r($order_wise_poid_arr);

							$po_breakdown_ids = chop($po_breakdown_ids,",");
	
							$all_po_breakdown_ids=array_chunk(explode(",",$po_breakdown_ids),999,true);
							$ex_factory_cond="";
							$ji=0;
							foreach($all_po_breakdown_ids as $key=> $value)
							{
								if($ji==0)
								{
							...
							$ex_factory_sql = "select c.po_break_down_id, sum(c.ex_factory_qnty) as ex_factory_qnty, max(c.shiping_status) as shiping_status
							from pro_ex_factory_delivery_mst b, pro_ex_factory_mst c 
							where b.id=c.delivery_mst_id and b.status_active=1 and b.is_deleted=0 $ex_factory_cond and c.status_active=1 and c.is_deleted=0 and c.entry_form<>85 group by c.po_break_down_id";  //$ex_factory_date_year_cond
							//echo $ex_factory_sql;//die;

							$ex_factory_details_resule=sql_select($ex_factory_sql);
							$ex_factory_po_arr=array();
							foreach ($ex_factory_details_resule as  $value)
							{
								$ex_factory_data_array[$po_wise_buyer[$value[csf('po_break_down_id')]]]["qnty"] += $value[csf("ex_factory_qnty")];
								$ex_factory_data_array[$po_wise_buyer[$value[csf('po_break_down_id')]]]["amt"] += $value[csf("ex_factory_qnty")]*$order_wise_rate_pcs[$value[csf('po_break_down_id')]];
								$ex_factory_po_arr[$value[csf('po_break_down_id')]]=$value[csf('po_break_down_id')];

								$po_buyer = $po_wise_buyer[$value[csf('po_break_down_id')]];

								$po_qty   = $order_wise_po_arr[$value[csf('po_break_down_id')]]['qty'];
								$po_value = $order_wise_po_arr[$value[csf('po_break_down_id')]]['value'];


								if($value[csf("shiping_status")]==3)//Fullshipment
								{
									$po_receive_data_array[$po_buyer]['shipQty'] += 0;
									$po_value = 0;
								}
								else
								{
									$po_receive_data_array[$po_buyer]['shipQty'] += $po_qty-$value[csf("ex_factory_qnty")];
									$po_receive_data_array[$po_buyer]['value'] = $po_receive_data_array[$po_buyer]['shipQty']*$po_value;
								}
							}

							//echo "<pre>";
							//print_r($arr_dif);
							//var_dump($po_receive_data_array);
							$po_diff = array_diff($order_wise_poid_arr,$ex_factory_po_arr);
							foreach ($po_diff as $po_id)
							{
								$po_buyer = $po_wise_buyer[$po_id];
								$po_qnty   = $order_wise_po_arr[$po_id]['qty'];
								$po_receive_data_array[$po_buyer]['shipQty'] += $po_qnty;
								$po_value = $order_wise_po_arr[$po_id]['value'];
								$po_receive_data_array[$po_buyer]['value'] += $po_qnty*$po_value;
							}
							...
							foreach ($po_receive_data_array as $row)
							{
								...
								<tr bgcolor="<? echo $bgcolor;?>">
									...
									<td align="right">
									<?
									echo number_format($ex_factory_data_array[$row['buyer_name']]["qnty"],0);
									//if ($ex_factory_data_array[$row['buyer_name']]["qnty"])
									$shipped_total_qnty+=$ex_factory_data_array[$row['buyer_name']]["qnty"];

									?>
									</td>
									<td align="right" id="value">
									<?
										//$unit_rate = ($row['po_total_price_pcs']*1)/($row['po_qnty_pcs']*1);
										$ex_factory_value = $ex_factory_data_array[$row['buyer_name']]["amt"];
										echo number_format($ex_factory_value,2); $total_ex_factory_value+=$ex_factory_value;

									?>
									</td>
									<td align="right">
										<? $ship_percentage=$ex_factory_value*100/$row['po_total_price'];
										echo number_format($ship_percentage,2); ?>
									</td>

									<td align="right">
									<?
									    $order_in_hand_quantity=0;
									    echo number_format( $row['shipQty'], 0);
									    $total_order_in_hand_quantity+=$row['shipQty'];
										/*if($row['shiping_status'] ==3){
											echo $row['po_quantity']-$ex_factory_data_array[$row['buyer_name']]["qnty"];
										}else{
											echo $row['po_quantity']-$ex_factory_data_array[$row['buyer_name']]["qnty"];

										}*/
									    //$total_order_in_hand_quantity+=$tot_qty;
									?>
									</td>
									<td align="right" id="value">
									<?
										$in_hand_order_value=0;
										echo number_format($row['value'], 2);
										$total_in_hand_order_value += $row['value'];
									?>
									</td>
					...
					<tfoot>
						...
						<th><? echo number_format($total_price/$total_po,2); ?></th>
						<th></th>
						<th><? echo number_format($shipped_total_qnty,0); ?></th>
						<th><? echo number_format($total_ex_factory_value,2); ?></th>
						<th><? //$total_ship_percentage=$total_ex_factory_value*100/$total_price; echo number_format($total_ship_percentage,2); ?></th>
...
if ($action == "inter_company_export_proceed")
{
		...
		if($db_type==0)
	{
		if($cbo_year!=0 || ($from_date!="" && $to_date!=""))
		{
			if($from_date !="" && $to_date !="" )
			{
				...
				if($cbo_shipment_type == 1)
				{
					$date_cond="and c.ex_factory_date between '$from_date' and '$to_date'";
				}
				else
				{
					$date_cond="and c.ex_factory_date between '$from_date' and '$to_date'";
	...
	else if ($db_type==2)
	{
		...
		where c.job_no_mst=d.job_no and d.job_no=a.job_no and a.job_no=b.job_no and a.status_active=1 and b.status_active=1 and c.status_active=1 and b.amount > 0  $budge_com_cond
		union all
				...
				<table width="1030" cellspacing="0" cellpadding="0" class="rpt_table" border="1" rules="all" id="po_received_status_table">
					...
					<tbody>
						<?
							...
							where a.job_no=b.job_no_mst and b.id=c.po_break_down_id $company_cond $buyer_id_cond $dealing_marchant_cond  $date_cond  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";//$year_cond
							...
							foreach ($data_array as $value) {
								...
								$order_in_hand_quantity=0;
								if(($po_receive_details_array[$value[csf('buyer_name')]]['po_quantity']*1) >= ($po_receive_details_array[$value[csf('buyer_name')]]['ex_factory_qnty']*1))
								{
									$order_in_hand_quantity=($po_receive_details_array[$value[csf('buyer_name')]]['po_quantity']*1)-($po_receive_details_array[$value[csf('buyer_name')]]['ex_factory_qnty']*1);
							...
							foreach ($po_receive_details_array as $row)
							{
								...
								<tr bgcolor="<? echo $bgcolor;?>">
									...
									<td align="right"  id="value">
										<?
											echo number_format($row['ex_factory_qnty'],0);
										?>
									</td>
									<td align="right">
										<?
								...
								<?
								$i++;
								$ex_factory_value=$order_in_hand_quantity=$in_hand_order_value=0;
							}
						?>
					</tbody>
					<tfoot>
