...
if($action=="load_drop_down_brands")
{
	...
	$buyer_id = $data_arr[1]; 
    echo create_drop_down("cbo_brand_name", 120, "select id, brand_name from lib_buyer_brand where buyer_id in ($buyer_id) and status_active =1 and is_deleted=0 order by brand_name ASC","id,brand_name", 1, "--Select--", "", "" );
	exit();
}
...
if( $action=="report_generate" )
{
	...
	if($cbo_brand_name!="") $brand_name_cond=" and a.brand_id in ($cbo_brand_name)"; else $brand_name_cond="";
	...
	else $buyer_id_cond=" and a.buyer_name in ($cbo_buyer_name)";//.str_replace("'","",$cbo_buyer_name)
    ...
    if($job_id=='' && str_replace("'","",$txt_job_no)==""){
        $job_id_sql=sql_select("SELECT a.id as job_id from wo_po_details_master a join wo_po_break_down b on a.id=b.job_id where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and   a.company_name=$company_name $buyer_id_cond $date_cond $yearCond $brand_name_cond group by a.id");
		$job_id_arr=array();
	...
	if(count($job_id_arr)>0){
		...
		$main_sql="SELECT a.job_no, a.id as job_id, a.buyer_name, a.style_ref_no, a.brand_id, a.job_quantity, a.total_set_qnty, a.order_uom, a.working_company_id, rtrim(xmlagg(xmlelement(po_data, b.po_number,', ').extract('//text()') order by b.id).getclobval(),', ') as po_data , min(b.pub_shipment_date) as shipment_data, min(b.shipment_date) as original_ship_date, rtrim(xmlagg(xmlelement(gmts_id, c.gmts_item_id,',').extract('//text()') order by c.id).getclobval(),',') as gmts_id, e.cm_cost, f.exchange_rate from wo_po_details_master a join wo_po_break_down b on a.id=b.job_id join wo_po_details_mas_set_details c on a.id=c.job_id join gbl_temp_engine d on a.id=d.ref_val join wo_pre_cost_dtls e on e.job_id=a.id join wo_pre_cost_mst f on a.id=f.job_id where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.entry_form=2004 and ref_from=2 group by a.id, a.job_no, a.buyer_name, a.style_ref_no, a.brand_id, a.job_quantity, a.total_set_qnty, a.order_uom, a.working_company_id, e.cm_cost, f.exchange_rate order by min(b.shipment_date)";
	}
	else{
		if($job_id_cond!=''){
			$main_sql="SELECT a.job_no, a.id as job_id, a.buyer_name, a.style_ref_no, a.brand_id, a.job_quantity, a.total_set_qnty, a.order_uom, a.working_company_id, rtrim(xmlagg(xmlelement(po_data, b.po_number,', ').extract('//text()') order by b.id).getclobval(),', ') as po_data , min(b.pub_shipment_date) as shipment_data, min(b.shipment_date) as original_ship_date, rtrim(xmlagg(xmlelement(gmts_id, c.gmts_item_id,',').extract('//text()') order by c.id).getclobval(),',') as gmts_id, e.cm_cost, f.exchange_rate from wo_po_details_master a join wo_po_break_down b on a.id=b.job_id join wo_po_details_mas_set_details c on a.id=c.job_id left join wo_pre_cost_dtls e on e.job_id=a.id left join wo_pre_cost_mst f on a.id=f.job_id  where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and  a.company_name=$company_name $buyer_id_cond $date_cond $yearCond $brand_name_cond $job_id_cond  group by a.id, a.job_no, a.buyer_name, a.style_ref_no, a.brand_id, a.job_quantity, a.total_set_qnty, e.cm_cost, f.exchange_rate, a.order_uom, a.working_company_id order by min(b.shipment_date)";
		}
		else{
			$main_sql="SELECT a.job_no, a.id as job_id, a.buyer_name, a.style_ref_no, a.brand_id, a.job_quantity, a.total_set_qnty, a.order_uom, a.working_company_id, rtrim(xmlagg(xmlelement(po_data, b.po_number,', ').extract('//text()') order by b.id).getclobval(),', ') as po_data , min(b.pub_shipment_date) as shipment_data, min(b.shipment_date) as original_ship_date, rtrim(xmlagg(xmlelement(gmts_id, c.gmts_item_id,',').extract('//text()') order by c.id).getclobval(),',') as gmts_id, e.cm_cost, f.exchange_rate from wo_po_details_master a join wo_po_break_down b on a.id=b.job_id join wo_po_details_mas_set_details c on a.id=c.job_id left join wo_pre_cost_dtls e on e.job_id=a.id left join wo_pre_cost_mst f on a.id=f.job_id  where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and  a.company_name=$company_name $buyer_id_cond $date_cond $yearCond $brand_name_cond $jobcond group by a.id, a.job_no, a.buyer_name, a.style_ref_no, a.brand_id, a.job_quantity, a.total_set_qnty, a.order_uom, a.working_company_id, e.cm_cost, f.exchange_rate order by min(b.shipment_date)";
		}
	...
	$sarvice_booking=sql_select("SELECT a.entry_form,a.booking_no, a.pay_mode, a.supplier_id, a.company_id, c.job_id from wo_booking_mst a join wo_booking_dtls b on a.id=b.booking_mst_id join wo_po_break_down c on c.id=b.po_break_down_id join gbl_temp_engine d on d.ref_val=c.job_id where a.status_active=1 and a.is_deleted=0 and a.booking_type=3 and d.entry_form=2004 and d.ref_from=1 and d.user_id=$user_id and a.entry_form in (228,229,162)");
	...
	$fabric_rcv=sql_select("SELECT b.receive_qnty, d.job_id, b.color_id from inv_receive_master a join pro_finish_fabric_rcv_dtls b on a.id=b.mst_id join order_wise_pro_details c on b.trans_id=c.trans_id and b.prod_id=c.prod_id and b.id=c.dtls_id join wo_po_break_down d on c.po_breakdown_id=d.id join gbl_temp_engine e on e.ref_val=d.job_id where e.entry_form=2004 and e.ref_from=1 and e.user_id=$user_id and c.entry_form =37 and c.trans_type=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0");
	$fabric_rcv_arr=array();
	foreach($fabric_rcv as $row){
		$fabric_rcv_arr[$row[csf('job_id')]]+=$row[csf('receive_qnty')];
		$color_wise_fabric_rcv_arr[$row[csf('job_id')]][$row[csf('color_id')]]+=$row[csf('receive_qnty')];
	}
	...
	*/
	...
	$export_invoice_qty=sql_select("SELECT c.job_id, sum(a.current_invoice_qnty) as current_invoice_qnty, sum(a.current_invoice_value) as current_invoice_value from com_export_invoice_ship_dtls a join com_export_invoice_ship_mst b on a.mst_id=b.id join wo_po_break_down c on a.po_breakdown_id=c.id join gbl_temp_engine d on d.ref_val=c.job_id where  d.entry_form=2004 and d.user_id=$user_id and d.ref_from=1 and a.status_active=1 and a.is_deleted=0 group by c.job_id");
	foreach($export_invoice_qty as $row){
		$exfactory_qty_arr[$row[csf('job_id')]]['qty']=$row[csf('current_invoice_qnty')];
	}
	...
	$sample_data=sql_select("SELECT a.id, a.sample_type_id, a.approval_status, b.color_number_id, b.job_id from wo_po_sample_approval_info a join wo_po_color_size_breakdown b on a.po_break_down_id=b.po_break_down_id and a.color_number_id=b.id join gbl_temp_engine c on c.ref_val=b.job_id where a.sample_type_id in (2,7) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.approval_status>0 and c.entry_form=2004 and c.user_id=$user_id and c.ref_from=1 group by a.id,a.sample_type_id, a.approval_status, b.color_number_id, b.job_id order by a.id asc");
	foreach($sample_data as $row){
		$sample_data_arr[$row[csf('job_id')]][$row[csf('sample_type_id')]][$row[csf('color_number_id')]]=$row[csf('approval_status')];
	}
	$check_sample_arr=array(); $job_wise_sample=array();
	foreach($sample_data_arr as $job_id=>$job_data){
		foreach($job_data as $sample_id=>$colordata){
			foreach($colordata as $colorid=>$data){
				if($check_sample_arr[$job_id.'*'.$sample_id]==''){
					$check_sample_arr[$job_id.'*'.$sample_id]=$job_id.'*'.$sample_id;					
					$job_wise_sample[$job_id][$sample_id].="[<b>".$sample_library[$sample_id].'::</b>'.$color_arr[$colorid].':'.$approval_status[$data];
				}
				else{
					$job_wise_sample[$job_id][$sample_id].=", ".$color_arr[$colorid].':'.$approval_status[$data]."]";
				}
				if(count($colordata)==1){
					$job_wise_sample[$job_id][$sample_id].="]";
				}				
			}
		}
	}
	//echo __LINE__; die;
	$fabric_status_dkl=sql_select("SELECT a.issue_number_prefix_num as issue_no, a.issue_date, d.color_id, sum(d.quantity) as issue_qty, e.job_id from inv_issue_master a join inv_finish_fabric_issue_dtls b on a.id=b.mst_id join inv_transaction c on b.trans_id=c.id join order_wise_pro_details d on c.id=d.trans_id join wo_po_break_down e on e.id=d.po_breakdown_id join gbl_temp_engine f on f.ref_val=e.job_id where a.knit_dye_company=2 and d.trans_type=2 and d.entry_form=18 and f.entry_form=2004 and f.user_id=$user_id and f.ref_from=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 group by a.issue_number_prefix_num, a.issue_date, d.color_id, e.job_id order by a.issue_number_prefix_num desc");
	$fabric_status_dk=array();
	foreach($fabric_status_dkl as $row){
		$fabric_status_dk[$row[csf('job_id')]]['issue_qty'][$row[csf('color_id')]]+=$row[csf('issue_qty')];
		$fabric_status_dk[$row[csf('job_id')]]['issue_no'][$row[csf('issue_no')]]=$row[csf('issue_no')];
		$fabric_status_dk[$row[csf('job_id')]]['issue_date']=$row[csf('issue_date')];
	}
	$acc_status_dkl=sql_select("SELECT a.issue_number_prefix_num as issue_no, a.issue_date, b.item_group_id, e.job_id from inv_issue_master a join inv_trims_issue_dtls b on a.id=b.mst_id join inv_transaction c on b.trans_id=c.id join order_wise_pro_details d on c.id=d.trans_id join wo_po_break_down e on e.id=d.po_breakdown_id join gbl_temp_engine f on f.ref_val=e.job_id where a.knit_dye_company=2 and d.trans_type=2 and d.entry_form=25 and f.entry_form=2004 and f.user_id=$user_id and f.ref_from=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 group by a.issue_number_prefix_num, a.issue_date, b.item_group_id, e.job_id order by a.issue_number_prefix_num desc");
	$acc_status_dk=array();
	foreach($acc_status_dkl as $row){
		$acc_status_dk[$row[csf('job_id')]]['item_group'][$row[csf('item_group_id')]]=$item_library[$row[csf('item_group_id')]];
		$acc_status_dk[$row[csf('job_id')]]['issue_no'][$row[csf('issue_no')]]=$row[csf('issue_no')];
		$acc_status_dk[$row[csf('job_id')]]['issue_date']=$row[csf('issue_date')];
	}
	/* echo '<pre>';
	print_r($fabric_status_dk); die; */
	...
	<div style="width:4850px; height:700px; overflow: auto;border: 1px solid;">
		<table width="4800" class="rpt_table freeze-table" cellpadding="0" cellspacing="0" border="1" rules="all">
			<thead>
				<tr>
					<th width="30">SL</th>
			...
			<tbody id="table_body">
				...
				foreach($main_data as $row){
					...
					if($fabric_rcv_arr[$row[csf('job_id')]]>=array_sum($fabric_booking_arr[$row[csf('job_id')]]['fin_fab_qty'])){
						...
						foreach($fabric_booking_arr[$row[csf('job_id')]]['fin_fab_qty'] as $colorid=>$fabqty){
							$colorcv_qty='';
							if($color_wise_fabric_rcv_arr[$row[csf('job_id')]][$colorid]>0){
								$colorcv_qty='-'.$color_wise_fabric_rcv_arr[$row[csf('job_id')]][$colorid];
							}
							$color_wise_fabric[]=$color_arr[$colorid].':'.number_format($fabqty).$colorcv_qty;
						}
					...
					$fabricstatusdk='';
					$k=1;
					foreach($fabric_status_dk[$row[csf('job_id')]]['issue_qty'] as $colr_id=>$data){
						if($k==1){
							$fabricstatusdk=$color_arr[$colr_id].'-'.$data;
						}
						else{
							$fabricstatusdk.=$color_arr[$colr_id].'-'.$data.',';
						}
				...
				<tr>
					...
					<td align="right" width="100"><?= $row[csf('job_quantity')] ?></td>
					<td align="center" width="100"><?= $unit_of_measurement[$row[csf('order_uom')]] ?></td>
					...
					<td width="100"><?= $company_arr[$row[csf('working_company_id')]] ?></td>
					<td width="100"><?= implode(", ",$production_unit_arr[$row[csf('job_id')]]) ?></td>
					<td width="170"><?= implode(", ",$job_wise_sample[$row[csf('job_id')]]) ?></td>
					...
					<td width="60"><?= $fabricstatusdk; ?></td>
					<td width="60"><?= implode(",",$fabric_status_dk[$row[csf('job_id')]]['issue_no']) ?></td>
					<td width="60"><?= change_date_format($fabric_status_dk[$row[csf('job_id')]]['issue_date']) ?></td>
					<td width="60"><?= implode(",",$acc_status_dk[$row[csf('job_id')]]['item_group']) ?></td>
					<td width="60"><?= implode(",",$acc_status_dk[$row[csf('job_id')]]['issue_no']) ?></td>
					<td width="60"><?= change_date_format($acc_status_dk[$row[csf('job_id')]]['issue_date']) ?></td>
