<?
/*-------------------------------------------- Comments
Purpose			: working company wise capacity vs allocation and plan report
Created by		: Al-Hasan
Creation date 	: 31-10-2023
Updated by 		:
Update date		:
Working  BY	    : Sajib Sutradhar
QC Performed BY	: Sajib Sutradhar
QC Date			:
Comments		:
*/
session_start();
...
echo load_html_head_contents("Working Company Wise Capacity & Order Status","../../../", 1, 1, $unicode,1,'');
?>	
<script>
	...
	function fn_report_generated(type)
	{
		...
		else
		{
			...
			if(type==1)
			{
				var catid = $("#cbo_date_cat_id").val();
				if(catid == 5){
					alert('Not allow this date category for show button');
					return false;
				}
			...
			if(type==2)
			{
				var catid = $("#cbo_date_cat_id").val();
				if(catid == 2 || catid == 3){
					alert('Not allow this date category for plan button');
					return false;
				}
			    var data="action=report_generate_plan"+get_submitted_data_string('cbo_company_id*cbo_location_id*cbo_lc_company_id*cbo_lc_location_id*cbo_date_cat_id*txt_date_from*txt_date_to',"../../../")+'&report_title='+report_title;
			}


			freeze_window(3);
			http.open("POST","requires/working_company_wise_capacity_vs_allocation_and_plan_report_controller.php",true);
                        ...
                        <thead>  
                         ...
                         <tbody>
                            <tr class="general">
                                ...
                                <td>
                                    <input type="button" name="search" id="search" value="Show" onClick="fn_report_generated(1)" style="width:60px" class="formbutton"/>
                                    <input type="button" name="search" id="search" value="Plan" onClick="fn_report_generated(2)" style="width:60px" class="formbutton"/>
                                </td>
                            </tr>
                            <tr>
...
<script>
	...
	$("#multiselect_dropdown_table_headercbo_company_id").click(function(){
		...
		load_drop_down( 'requires/working_company_wise_capacity_vs_allocation_and_plan_report_controller',wcCompany, 'load_drop_down_location', 'wc_location_td' );
		set_multiselect('cbo_location_id','0','0','','0');
	});
	
	$("#multiselect_dropdown_table_headercbo_lc_company_id").click(function(){
		var lcCompany=$("#cbo_lc_company_id").val();
		load_drop_down( 'requires/working_company_wise_capacity_vs_allocation_and_plan_report_controller', lcCompany, 'lc_load_drop_down_location', 'lc_location_td' );
		set_multiselect('cbo_lc_location_id','0','0','','0');
	});
