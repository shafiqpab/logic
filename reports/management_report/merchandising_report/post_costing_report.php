...
<script>
	...
	function fn_report_generated(report)
	{
		...
		{	
			if(report==1){
				var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_job_no*cbo_year*txt_order_no*txt_file_no*txt_ref_no*hide_order_id*txt_date_from*txt_date_to*shipping_status*cbo_order_status*cbo_pre_cost_class*cbo_date_type*cbo_season_name',"../../../");
			}else if(report==3){
				var data="action=report_generate2"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_job_no*cbo_year*txt_order_no*txt_file_no*txt_ref_no*hide_order_id*txt_date_from*txt_date_to*shipping_status*cbo_order_status*cbo_pre_cost_class*cbo_date_type*cbo_season_name',"../../../");
			}
			else
			{
				var data="action=report_generate_actual_cost"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_job_no*cbo_year*txt_order_no*txt_file_no*txt_ref_no*hide_order_id*txt_date_from*txt_date_to*shipping_status*cbo_order_status*cbo_pre_cost_class*cbo_date_type*cbo_season_name',"../../../");
			}
...
'<html><head><link rel="stylesheet" href="../../css/style_common.css" type="text/css"/><title></title></head><body>'+document.getElementById(container_div).innerHTML+'</body</html>');
	...
	function openmypage_mkt(mkt_data,type,tittle)
	{
		...
		{
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_report_controller2.php?mkt_data='+mkt_data+'&action='+type, tittle, 'width=660px, height=200px, center=1, resize=0, scrolling=0', '../../');
		}
	}
	...
	function openmypage_actual(po_id,type,tittle,popup_width)
	{
		...
		{
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_report_controller2.php?po_id='+po_id+'&action='+type, tittle, 'width='+popup_width+', height=400px, center=1, resize=0, scrolling=0', '../../');
		}
	}
	...
	function generate_ex_factory_popup(action,job,id,width)
	{
		...
		{
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_report_controller2.php?action='+action+'&job='+job+'&id='+id, 'Ex-Factory Details', 'width='+width+',height=400px,center=1,resize=0,scrolling=0','../../');
		}
	}	
	...
	function generate_po_report(company_name,po_id,job_no,action,type)
	{
		...
		{
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_report_controller2.php?action='+action+'&po_id='+po_id+'&job_no='+job_no+'&company_name='+company_name, 'PO Detail', 'width='+popup_width+',height=400px,center=1,resize=0,scrolling=0','../../');
		}
	}
</script>
</head>
            ...
            <fieldset style="width:1300px;">
                <table class="rpt_table" width="1300" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                	<thead>
                     ...
                     <tbody>
                        <tr class="general">
                            <td><? echo create_drop_down( "cbo_company_name", 140, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/post_costing_report_controller2',this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?></td>
                            ...
                            <td><input type="button" id="show_button" class="formbutton" style="width:50px" value="Show" onClick="fn_report_generated(1);" /><input type="button" id="show_button" class="formbutton" style="width:50px" value="Show 2" onClick="fn_report_generated(3);" /></td>
                        </tr>
                        <tr>
