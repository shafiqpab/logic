...
<script>
	...
	function fn_report_generated(action)
	{
		...
		{	
			...
			freeze_window(3);
			http.open("POST","requires/post_costing_report_v3_controller.php",true);
	...
	function openmypage_order()
	{
		...
		var page_link='requires/post_costing_report_v3_controller.php?action=order_no_search_popup&companyID='+companyID;
...
'<html><head><link rel="stylesheet" href="../../css/style_common.css" type="text/css"/><title></title></head><body>'+document.getElementById(container_div).innerHTML+'</body</html>');
	...
	function openmypage(po_id,type,tittle)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_report_v3_controller.php?po_id='+po_id+'&action='+type, tittle, 'width='+popup_width+', height=400px, center=1, resize=0, scrolling=0', '../../');
	}
	
	function openmypage_mkt(mkt_data,type,tittle)
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_report_v3_controller.php?mkt_data='+mkt_data+'&action='+type, tittle, 'width=660px, height=200px, center=1, resize=0, scrolling=0', '../../');
	}
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_report_v3_controller.php?po_id='+po_id+'&action='+type+'&popup_type='+popup_type, tittle, 'width='+popup_width+', height=400px, center=1, resize=0, scrolling=0', '../../');
	}
	function openmypage_actual3(po_id,type,tittle,popup_width)
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_report_v3_controller.php?po_id='+po_id+'&action='+type, tittle, 'width='+popup_width+', height=400px, center=1, resize=0, scrolling=0', '../../');
	}
	function openmypage_gmt_actual(po_id,embl_name,type,tittle,popup_width)
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_report_v3_controller.php?po_id='+po_id+'&action='+type+'&embl_name='+embl_name, tittle, 'width='+popup_width+', height=400px, center=1, resize=0, scrolling=0', '../../');
	}
	
	function generate_ex_factory_popup(action,job,id,width)
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_report_v3_controller.php?action='+action+'&job='+job+'&id='+id, 'Ex-Factory Details', 'width='+width+',height=400px,center=1,resize=0,scrolling=0','../../');
	}	
	...
	function generate_po_report(company_name,po_id,job_no,action,type)
	{
		...
		popup_width='940px';
		//alert(po_id);
		//emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_report_v3_controller.php?po_id='+po_id+'&company_name='+company_name+'&action='+type, report_title, 'width='+popup_width+', height=400px, center=1, resize=0, scrolling=0', '../../');
		
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_report_v3_controller.php?action='+action+'&po_id='+po_id+'&job_no='+job_no+'&company_name='+company_name, 'PO Detail', 'width='+popup_width+',height=400px,center=1,resize=0,scrolling=0','../../');
	}
...
// 	}
	...
	function generate_report(type)
	{
		...
		var hide_order_id=$('#hide_order_id').val();
			...
			var path = '../../';
            print_report(cbo_company_name+'**'+cbo_buyer_name+'**'+txt_job_no+'**'+txt_order_no+'**'+cbo_year+'**'+hide_order_id+'**'+ path , "bomRpt2", "requires/post_costing_report_v3_controller");
	}
</script>
            ...
            <fieldset style="width:1450px;">
                <table class="rpt_table" width="1430" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                    ...
                    <tr class="general">
                        <td> 
                            <?
                                echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/post_costing_report_v3_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
                        <td id="buyer_td">
                            <? 
					...
					<tr class="">
							<td class="" colspan="11" align="center">
								<input type="button" id="report_btn_4" class="formbutton" value=" Final Cost Report" onClick="generate_report('bomRpt2');"  />
								<input type="button" id="show_button1" class="formbutton" style="width:50px" value="Show" onClick="fn_report_generated('report_generate')" />
								<input type="button" id="show_button2" class="formbutton" style="width:50px" value="Show2" onClick="fn_report_generated('report_generate2')" />
								<input type="button" id="show_button3" class="formbutton" style="width:50px" value="Show3" onClick="fn_report_generated('report_generate3')" />
								<input type="button" id="show_button4" class="formbutton" style="width:50px" value="Show4" onClick="fn_report_generated('report_generate4')" />
								<input type="button" id="show_button5" class="formbutton" style="width:50px" value="Show5" onClick="fn_report_generated('report_generate5')" />
							</td>
					</tr>
                    </tbody>
                </table>
