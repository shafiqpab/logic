...
<script>
	...
	function fn_report_generated(type)
	{
		...
		var data="action=report_generate&reporttype="+type+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_date_from*txt_date_to*txt_job_no*txt_job_id*txt_file_no*txt_ref_no*txt_order_id*txt_order_no*cbo_year*cbo_order_status*cbo_search_date*txt_season*cbo_status*cbo_approved_status*cbo_search_type*txt_season_id',"../../../");
		freeze_window(3);
		if(pre_cost_class==1)
		{
			http.open("POST","requires/budget_breakdown_report_controller.php",true);
		}
		else
		{
			http.open("POST","requires/budget_breakdown_report_controller_v2.php",true);
		}
	...
	function openmypage_job2()
	{
		...
		var page_link='requires/budget_breakdown_report_controller.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
		...
		emailwindow.onclose=function()
		{
			...
			var job_no=this.contentDoc.getElementById("txt_selected_no").value; // product Description
			//alert(job_no);
	...
	function openmypage_job()
	{
		...
		var page_link='requires/budget_breakdown_report_controller_v2.php?action=style_refarence_search&companyID='+companyID+'&buyer_name='+buyer_name+'&txt_style_ref_no='+txt_style_ref_no+'&txt_style_ref_id='+txt_style_ref_id+'&txt_style_ref='+txt_style_ref+'&cbo_year_id='+cbo_year_id;
	...
	function openmypage_order()
	{
		...
		//alert (data);
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/budget_breakdown_report_controller_v2.php?action=order_no_popup&data='+data,'Order No Popup', 'width=630px,height=420px,center=1,resize=0','../../')
		emailwindow.onclose=function()
		{
	...
	function precost_bom_pop_old(po_id,job_no,company_id,buyer_id,style_ref,costing_date)
	{
		var pre_cost_class=document.getElementById('cbo_pre_cost_class').value;
		//alert(po_id);
		/*var popup_width='900px';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/budget_breakdown_report_controller.php?company_id='+company_id+'&po_id='+po_id+'&job_no='+job_no+'&buyer_id='+buyer_id+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../../');*/
...
*/
	...
	function precost_bom_pop(type,job_no,company_id,buyer_id,style_ref,txt_costing_date,entry_form,quotation_id)
	{
	...
	function openmypage_season()
	{
		...
		var page_link='requires/budget_breakdown_report_controller.php?action=season_popup&companyID='+companyID+'&buyerID='+buyerID+'&job_no='+job_no;
            ...
            <table class="rpt_table" cellpadding="1" cellspacing="2" align="center" border="1" rules="all">
                ...
                <tbody>
                    <tr class="general">
                        <td>
							<?
								$search_type_arr=array(1=>"Job/Style Wise",2=>"PO Wise");
                           		echo create_drop_down( "cbo_search_type", 100, $search_type_arr,"", 0, "-- Select --", 1, "fnc_field_disabled();" );
                            ?>
                        </td>
                         <td>
							<?
                           		echo create_drop_down( "cbo_company_name", 140, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- All Company --", $selected, "load_drop_down( 'requires/budget_breakdown_report_controller_v2',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
                        <td id="buyer_td">
							<?
                    ...
                    <tr >
                    	<td colspan="16" align="center">
                            ...
                            <input type="button" id="show_button" class="formbutton" style="width:120px;" value="Convert to Excel" onClick="fn_report_excel_generated(1)" /><A id="aa1" href="" style="text-decoration:none;"></A>
                            ...
                            <input type="button"  id="show_button2" class="formbutton" style="width:90px;" value="Report" onClick="fn_report_generated(3)" />
							<input type="button"  id="show_button2" class="formbutton" style="width:90px;" value="Report 2" onClick="fn_report_generated(4)" />
                        </td>
                    </tr>
                </table>
            </fieldset>
