...
<script>
	...
	var tableFilters = 
	{
		col_operation: 
		{
		  ...
		  //col: [16,19,20,22,23,24,25,26,27,28,29,30],
	...
	function generate_report(div,type)
	{
		...
		var data=cbo_company_name+"_"+cbo_buyer_name+"_"+txt_date_from+"_"+txt_date_to+"_"+cbo_team_name+"_"+cbo_team_member+"_"+cbo_category_by+"_"+zero_value+"_"+cbo_capacity_source+"_"+cbo_year+"_"+txt_style_ref+"_"+cbo_factory_merchant+"_"+cbo_agent;
		freeze_window(3);
		http.open("GET","requires/capacity_and_order_booking_status_ffl_controller.php?data="+data+"&type="+type,true);
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/capacity_and_order_booking_status_ffl_controller.php?action=country_work_progress_report_details&job_no='+job_no+'&po_id='+po_id+'&template_id='+template_id+'&tna_process_type='+tna_process_type+'&country_id='+country_id, 'Work Progress Report Details Country PO Wise', 'width=1150px,height=480px,center=1,resize=1,scrolling=0','../../')
		emailwindow.onclose=function()
		{
		}
	}
...
//for report summary
	function fn_report_generated_summary()
	{
		...
		//alert($('#cbo_year_selection').val())
			...
			//alert(data);
			freeze_window(3);
			http.open("POST","requires/capacity_and_order_booking_status_ffl_controller.php",true);
			http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
			http.send(data);
...
'<html><head><link rel="stylesheet" href="../../css/style_common.css" type="text/css" media="print" /><title></title></head><body>'+document.getElementById('report_container2').innerHTML+'</body</html>');
	...
	{
		var page_link='requires/capacity_and_order_booking_status_ffl_controller.php?action='+action+'&job_no='+job_no+'&type='+type
		var title="File View";
	...
	function set_defult_date(companyId){
		var defult_date=return_global_ajax_value(companyId, 'get_defult_date', '', 'requires/capacity_and_order_booking_status_ffl_controller');
	...
	{
		//alert(job_no); 
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/capacity_and_order_booking_status_ffl_controller.php?action='+action+'&job_no='+job_no+'&id='+id, 'Ex-Factory Details', 'width='+width+',height=400px,center=1,resize=0,scrolling=0','../../');
	}
</script>
</head>
<body onLoad="set_defult_date(document.getElementById('cbo_company_name').value)">
    <div style="width:100%;" align="center">
        ...
        <h3 style="width:1345px;" align="center" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3> 
			...
			<fieldset>
                <table class="rpt_table" width="1300" cellpadding="0" cellspacing="0" border="1" rules="all">
                    ...
                    <tr class="general">
                        <td>
                            <?
                                echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, " load_drop_down( 'requires/capacity_and_order_booking_status_ffl_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/capacity_and_order_booking_status_ffl_controller', this.value, 'load_drop_down_agent', 'agent_td'); set_defult_date(this.value)" );
                            ?> 
                        </td>
                        <td id="buyer_td">
                            <? 
                        ...
                        <td id="agent_td">
                            <? 
                                echo create_drop_down( "cbo_agent", 140, $blank_array,"", 1, "-- Select Agent --", $selected, "" );
                            ?>	
                        </td>
                        <td align="center">
                        ...
                        <td>
                            <? echo create_drop_down( "cbo_year", 60, create_year_array(),"", 1,"-- All --",0 , "",0,"" );//date("Y",time()) ?>	
                        </td>
                        <td >                
                            <?
                                echo create_drop_down( "cbo_team_name", 100, "select id,team_name from lib_marketing_team  where status_active =1 and is_deleted=0  order by team_name","id,team_name", 1, "-- Select Team --", $selected, " load_drop_down( '../merchandising_report/requires/capacity_and_order_booking_status_ffl_controller', this.value, 'load_drop_down_team_member', 'team_td' );load_drop_down( '../merchandising_report/requires/capacity_and_order_booking_status_ffl_controller', this.value, 'cbo_factory_merchant', 'div_marchant_factory' ) " );
                            ?>
                        </td>
                        <td id="team_td">
                        <div id="div_team">
                        ...
                        <td>
                        <? 
                                echo create_drop_down( "cbo_category_by", 100, $report_date_catagory,"", 0, "", $selected, "",'',"1,2,4" );
                            ?>	
