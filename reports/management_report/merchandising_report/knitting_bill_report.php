...
<script>
	...
	function fn_report_generated()
	{
		...
		{	
			var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_job_no*cbo_year*txt_inter_ref*txt_order_no*hide_order_id*txt_exchange_rate*txt_date_from*txt_date_to*shipping_status',"../../../");
			freeze_window(3);
	...
	function openmypage_order()
	{
		...
		var page_link='requires/knitting_bill_report_controller.php?action=order_no_search_popup&companyID='+companyID;
	...
	{
		//alert(po_id); return;
		var dd = po_id.split('_');
		var po_id = dd[0];
		var exchange_rate = dd[1];
		
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/knitting_bill_report_controller.php?po_id='+po_id+'&exchange_rate='+exchange_rate+'&action='+type, tittle, 'width=800px, height=350px, center=1, resize=0, scrolling=0', '../../');
	}
</script>
</head>
            ...
            <?php echo load_freeze_divs ("../../../"); ?>
             <h3 align="left" id="accordion_h1" style="width:1000px" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>
                ...
                <fieldset style="width:970px;">
                    <table class="rpt_table" width="970" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                        <thead>
                            <tr>                   
                        ...
                        <tbody>
                        <tr class="general">
                            <td><?php echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/knitting_bill_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?></td>
                            <td id="buyer_td"><?php echo create_drop_down( "cbo_buyer_name", 150, $blank_array,"", 1, "-- All Buyer --", $selected, "",0,"" ); ?></td>
                            <td><?php echo create_drop_down( "cbo_year", 60, create_year_array(),"", 1,"-- All --", date("Y",time()), "",0,"" ); ?>
                            </td>
                            ...
                            <td><?php echo create_drop_down( "shipping_status", 120, $shipment_status,"", 1, "-- All --", 0, "",0,'','','','','' ); ?></td>
