<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This file for Order wise Finish Fabrics Requirement.
Functionality	:	
JS Functions	:
...
<script> 
	...
	{		
		...
		{			
			...
			var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_booking_no*txt_order_no*txt_date_from*txt_date_to*txt_booking_id*txt_order_id*cbo_year*cbo_season',"../../../")+'&report_title='+report_title;
			
			//alert(data);
			freeze_window();
			http.open("POST","requires/order_wise_finish_fabrics_requirement_controller.php",true);
...
function openmypage_booking(type)
	{
		...
		else
		{
			...
			var action='order_no_popup';
			var widthVal='860px';
		}
		var companyID = $("#cbo_company_name").val();
		var buyer_name = $("#cbo_buyer_name").val();
		var cbo_year  = $("#cbo_year").val();
		var page_link='requires/order_wise_finish_fabrics_requirement_controller.php?action='+action+'&companyID='+companyID+'&buyer_name='+buyer_name+'&type='+type+'&cbo_year='+cbo_year;
		...
		emailwindow.onclose=function()
		{
			...
			{
				var no=this.contentDoc.getElementById("txt_order_no").value;
				var id=this.contentDoc.getElementById("txt_order_id").value;
				$('#txt_order_no').val(no);
				$('#txt_order_id').val(id);
			}	
...
<form id="Order_monitoring_report">
    <div style="width:100%;" align="center">
        ...
        <h3 style="width:1080px; margin-top:20px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>
            ...
            <fieldset style="width:1080px;">
                <table class="rpt_table" cellpadding="1" cellspacing="0" rules="all" border="1">
                    <thead>
                        ...
                        <th width="300" id="search_date_td">Pub. Ship Date</th>
                    ...
                    <tr class="general">
                        <td> 
							<?
                           	 // echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "" );
                           	  echo create_drop_down( "cbo_company_name", 150, "select id,company_name from lib_company comp where status_active=1 $company_cond order by company_name","id,company_name",1, "-- Select Company --", 0, "load_drop_down( 'requires/order_wise_finish_fabrics_requirement_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
                            ?>
                        </td>
                      
                        <td id="buyer_td">
							<? 
                            	echo create_drop_down( "cbo_buyer_name", 150, $blank_array,"", 1, "-- All --", $selected, "",1,"" );
                            ?>
                        </td>
                        <td id="season_td">
							<? 
                        ...
                        <td id="year_td">
							<? 
							$year_current=date("Y");
							echo create_drop_down( "cbo_year", 100, $year,"", 1, "--All Year--", 0, "" );
                        ?>
                        </td>
                        <td>
                        	<input type="text"  id="txt_booking_no" name="txt_booking_no" class="text_boxes"  onDblClick="openmypage_booking(1);" placeholder="Write / Browse" >
							<input type="hidden" id="txt_booking_id" name="txt_booking_id"/>
                        </td>
                        <td>
                        	<input type="text"  id="txt_order_no" name="txt_order_no" class="text_boxes" onDblClick="openmypage_booking(2);" placeholder="Write / Browse">
							<input type="hidden" id="txt_order_id" name="txt_order_id"/>
                        </td>
                       
                        <td>
                            <input name="txt_date_from" id="txt_date_from" class="datepicker" style="width:90px" placeholder="From Date" value="" >&nbsp; To
                            <input name="txt_date_to" id="txt_date_to" class="datepicker" style="width:90px"  placeholder="To Date" value=""  >
                        </td>
                        <td>
...
$("#multiselect_dropdown_table_headercbo_company_name").click(function(){
	...
	load_drop_down( 'requires/order_wise_finish_fabrics_requirement_controller',data, 'load_drop_down_buyer', 'buyer_td' );
