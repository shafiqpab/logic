...
echo load_html_head_contents("Fabric Wise Report","../../../", 1, 1, $unicode,1,1);
?>	
<script>
	...
	function fn_report_generated(type)
	{
		...
		if(type==2)
		{
		var data="action=report_generate&reporttype="+type+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_date_from*txt_date_to*txt_job_no*txt_job_id*txt_order_id*txt_order_no*cbo_year*cbo_search_date*cbo_order_status',"../../../")+'&report_title='+report_title;
		}
		else
		{
			var data="action=report_generate_color_range&reporttype="+type+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_date_from*txt_date_to*txt_job_no*txt_job_id*txt_order_id*txt_order_no*cbo_year*cbo_search_date*cbo_order_status',"../../../")+'&report_title='+report_title;
		}
		//alert(data);return;
		freeze_window(3);
		if(type==1 || type==2 ||  type==3 ||  type==4)
		{
			http.open("POST","requires/fabric_wise_fab_req_report_controller.php",true);
		}
	...
	function openmypage_job()
	{
		...
		var page_link='requires/fabric_wise_fab_req_report_controller.php?action=job_no_popup&companyID='+companyID+'&buyer_name='+buyer_name+'&cbo_year_id='+cbo_year_id;
	...
	function openmypage_order()
	{
		...
		//alert (data);
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/fabric_wise_fab_req_report_controller.php?action=order_no_popup&data='+data,'Order No Popup', 'width=630px,height=420px,center=1,resize=0','../../')
		emailwindow.onclose=function()
		{
	...
	function openmypage_actual_po()
	{
		...
		//alert (data);
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/fabric_wise_fab_req_report_controller.php?action=actual_popup&data='+data,'Actual PO Popup', 'width=630px,height=420px,center=1,resize=0','../../')
		emailwindow.onclose=function()
		{
			var theemail=this.contentDoc.getElementById("order_no_id");
			var theemailv=this.contentDoc.getElementById("order_no_val");
	...
	{  
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/fabric_wise_fab_req_report_controller.php?po_id='+po_id+'&country_date='+country_date+'&buyer_id='+buyer_id+'&job_no='+job_no+'&action='+action, 'Details View', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../../');
	}
	...
	{  
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/fabric_wise_fab_req_report_controller.php?po_id='+po_id+'&country_id='+country_id+'&buyer_id='+buyer_id+'&job_no='+job_no+'&action='+action, 'Details View', 'width='+popup_width+', height=450px,center=1,resize=0,scrolling=0','../../');
	}
	...
	{ 
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/fabric_wise_fab_req_report_controller.php?company_id='+company_id+'&po_id='+po_id+'&job_no='+job_no+'&buyer_id='+buyer_id+'&action='+action, 'Details View', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../../');*/
	...
	function openmypage_season()
	{
		...
		var page_link='requires/fabric_wise_fab_req_report_controller.php?action=search_popup&companyID='+companyID+'&buyerID='+buyerID+'&job_no='+job_no;
...
<form id="costSheetReport_1">
    <div style="width:100%;" align="center">
        <? echo load_freeze_divs ("../../../",$permission);  ?>
         ...
         <fieldset style="width:1080px;" id="content_search_panel">
            <table class="rpt_table" width="1080" cellpadding="1" cellspacing="2" align="center" border="1" rules="all">
                <thead>                    
                    ...
                    <th>Search By</th>
                    <th colspan="2" id="search_by_th_up" class="must_entry_caption">Country Ship Date</th>
                </thead>
                <tbody>
                    <tr class="general">
                        <td> 
							<?
                           		echo create_drop_down( "cbo_company_name", 140, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/fabric_wise_fab_req_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
                        <td id="buyer_td">
							<? 
                        ...
                        <td width="" align="center">
							<?  
								$search_by = array(1=>'Country Ship Date');
                    ...
                    <tr align="center"  class="general">
                        <td colspan="16">
                           ...
                           <input type="button" id="show_button" class="formbutton" style="width:100px; float:right" value="Color Range" onClick="fn_report_generated(3)" />
                            <input type="button" id="show_button" class="formbutton" style="width:100px; float:right" value="Fabric Wise" onClick="fn_report_generated(2)" />
