<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This file for Order Follo up Report.
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Order Follo up Report","../../../", 1, 1, $unicode);
?>	
<script> 
	...
	function fn_report_generated(str)
	{
		...
		else if($('#cbo_season').val()>0){
			var file="cbo_company_name*cbo_year";
			var message="Comapny*Year";
		}
		...
		else
		{
			...
			var data="action="+action+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_job_no*txt_style_ref*cbo_order_status*txt_order_no*cbo_date_type*txt_date_from*txt_date_to*cbo_agent_name*cbo_year*cbo_ship_status*cbo_fabric_nature*cbo_fabric_source*txt_file_no*txt_ref_no*tna_task_id*cbo_season',"../../../");
			
			
			//alert(data);
			freeze_window();
			http.open("POST","requires/order_follo_up_report_controller.php",true);
	...
	function fn_report_generated_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			var tableFilters = {
					/*col_operation: {
					id: ["td_po_quantity","td_gf_qnty","td_gp_qty","td_gp_to_qty","td_daying_qnty","td_ff_qnty","td_fabrics_avl_qnty","td_blance","td_cut_qnty","td_emb_issue_qnty","td_emb_rec_qnty","td_sewing_in_qnty","td_sewing_out_qnty","td_sewing_finish_qnty","td_ex_qnty"],
					col: [10,13,14,15,17,18,19,20,21,23,24,25,26,27,28],
					operation: ["sum","sum","sum","sum","sum","sum","sum","sum","sum","sum","sum","sum","sum","sum","sum"],
					write_method: ["innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML","innerHTML"]
					}*/
					col_operation: {
    ...
    <div style="width:100%;" align="center">
        ...
        <div id="content_search_panel" > 
            <fieldset style="width:1520px;">
                <table class="rpt_table" width="1520" cellpadding="1" cellspacing="0" rules="all" border="1">
                    <thead>
                    ...
                    <tr class="general">
                        <td> 
							<?
                           	 echo create_drop_down( "cbo_company_name", 120, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/order_follo_up_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );load_drop_down( 'requires/order_follo_up_report_controller', this.value, 'load_drop_down_agent', 'agent_td' );get_php_form_data(this.value,'print_button_variable_setting','requires/order_follo_up_report_controller' );" );
                            ?>
                        </td>
                        <td align="center">
                        ...
                        <td id="buyer_td">
							<? 
                            	echo create_drop_down( "cbo_buyer_name", 100, $blank_array,"", 1, "-- All --", $selected, "load_drop_down( 'requires/order_follo_up_report_controller', this.value, 'load_drop_down_season', 'season_td');",1,"" );
                            ?>
                        </td>
						<td id="season_td">
							<? 
								echo create_drop_down( "cbo_season", 70, "select id, season_name from lib_buyer_season where  status_active =1 and is_deleted=0  order by season_name ASC","id,season_name", 1, "-Season-",  $selected, "",1 ); 
							?>
						</td>
                        <td id="agent_td">
							<? 
                        ...
                        <td>
						   ...
						   ?>
                            ...
                            <input name="txt_date_to" id="txt_date_to" class="datepicker" style="width:45px"  placeholder="To Date" value=""  >
						</td>
                        <td>
