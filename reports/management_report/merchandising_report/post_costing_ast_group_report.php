...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Post Costing Ast Group Report ","../../../", 1, 1, $unicode,1,1);
?>	
...
<script>
	...
	function fn_report_generated(report)
	{
		...
		var cbo_date_type=document.getElementById('cbo_date_type').value;
		...
		{	
			if(report==1){
				...
				var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_job_no*cbo_year*txt_order_no*txt_file_no*txt_ref_no*hide_order_id*txt_date_from*txt_date_to*shipping_status*cbo_order_status*cbo_pre_cost_class*cbo_date_type',"../../../");
			}
			else
			{
				var data="action=report_generate_actual_cost"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_job_no*cbo_year*txt_order_no*txt_file_no*txt_ref_no*hide_order_id*txt_date_from*txt_date_to*shipping_status*cbo_order_status*cbo_pre_cost_class*cbo_date_type',"../../../");
			}
			
			freeze_window(3);
			if(pre_cost_version==1)
			{
				http.open("POST","requires/post_costing_ast_group_report_controller.php",true);
			}
			else
			{
				http.open("POST","requires/post_costing_ast_group_report_controller2.php",true);
			}
	...
	function openmypage_order()
	{
		...
		var page_link='requires/post_costing_ast_group_report_controller.php?action=order_no_search_popup&companyID='+companyID+'&cbo_pre_cost_class='+cbo_pre_cost_class;
...
'<html><head><link rel="stylesheet" href="../../css/style_common.css" type="text/css"/><title></title></head><body>'+document.getElementById(container_div).innerHTML+'</body</html>');
	...
	function openmypage(po_id,type,tittle)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_ast_group_report_controller.php?po_id='+po_id+'&action='+type, tittle, 'width='+popup_width+', height=400px, center=1, resize=0, scrolling=0', '../../');
	}
	...
	function openmypage_mkt(mkt_data,type,tittle)
	{
		...
		if(version_type==1)
		{
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_ast_group_report_controller.php?mkt_data='+mkt_data+'&action='+type, tittle, 'width=660px, height=200px, center=1, resize=0, scrolling=0', '../../');
		}
		else
		{
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_ast_group_report_controller2.php?mkt_data='+mkt_data+'&action='+type, tittle, 'width=660px, height=200px, center=1, resize=0, scrolling=0', '../../');
		}
	}
	...
	function openmypage_actual(po_id,type,tittle,popup_width)
	{
		...
		{
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_ast_group_report_controller2.php?po_id='+po_id+'&action='+type, tittle, 'width='+popup_width+', height=400px, center=1, resize=0, scrolling=0', '../../');
		}
	}
	...
	function generate_ex_factory_popup(action,job,id,width)
	{
		...
		if(version_type==1)
		{
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_ast_group_report_controller.php?action='+action+'&job='+job+'&id='+id, 'Ex-Factory Details', 'width='+width+',height=400px,center=1,resize=0,scrolling=0','../../');
		}
		else
		{
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_ast_group_report_controller2.php?action='+action+'&job='+job+'&id='+id, 'Ex-Factory Details', 'width='+width+',height=400px,center=1,resize=0,scrolling=0','../../');
		}
	}	
	...
	function generate_po_report(company_name,po_id,job_no,action,type)
	{
		...
		if(version_type==1)
		{
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_ast_group_report_controller.php?action='+action+'&po_id='+po_id+'&job_no='+job_no+'&company_name='+company_name, 'PO Detail', 'width='+popup_width+',height=400px,center=1,resize=0,scrolling=0','../../');
		}
		else
		{
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/post_costing_ast_group_report_controller2.php?action='+action+'&po_id='+po_id+'&job_no='+job_no+'&company_name='+company_name, 'PO Detail', 'width='+popup_width+',height=400px,center=1,resize=0,scrolling=0','../../');
		}
	}
</script>
</head>
            ...
            <fieldset style="width:1200px;">
                <table class="rpt_table" width="1200" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                	<thead>
                     ...
                     <tbody>
                        <tr class="general">
                            <td><? echo create_drop_down( "cbo_company_name", 140, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/post_costing_ast_group_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?></td>
                            ...
                            <td><? echo create_drop_down( "shipping_status", 80, $shipment_status,"", 1, "-- All --", 3, "",0,'','','','','' ); ?></td>
                            <td>
                                <?  
                            ...
                            <td>
                                <? $date_type_arr = array(1=>'Pub-Shipment Date',2=>'Ex-factory Date',3=>'Ref.Closing Date',4=>'Cancelled Date');
                        ...
                        <tr>
                            ...
                            <td colspan="2" align="center"><input type="button" id="show_button" class="formbutton" style="width:90px" value="Pre VS Actual" onClick="fn_report_generated(2);" /></td>
                        </tr>
                     </tbody>
                </table>
            </fieldset>
