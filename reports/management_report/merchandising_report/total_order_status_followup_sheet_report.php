<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create Total Order Status And FollowUp Sheet  Report.
Functionality	:	
JS Functions	:
Created by		:	Aziz
...
<script>
	...
	function fn_report_generated()
	{
		...
		//alert(data);return;
		freeze_window(3);
		http.open("POST","requires/total_order_status_followup_sheet_report_controller.php",true);
	...
	function openmypage_style()
	{
		...
		var txt_style_ref = $("#txt_style_ref").val();
		var page_link='requires/total_order_status_followup_sheet_report_controller.php?action=style_refarence_surch&companyID='+company+'&buyer_name='+buyer+'&txt_style_ref_no='+txt_style_ref_no+'&txt_style_ref_id='+txt_style_ref_id+'&txt_style_ref='+txt_style_ref+'&cbo_year_id='+cbo_year;
	...
	function openmypage_order()
	{
		...
		var txt_order = $("#txt_order").val();
		var page_link='requires/total_order_status_followup_sheet_report_controller.php?action=order_popup&company='+company+'&buyer_name='+buyer+'&txt_order_id_no='+txt_order_id_no+'&txt_order_id='+txt_order_id+'&txt_order='+txt_order+'&txt_style_ref='+txt_style_ref+'&cbo_year='+cbo_year; 
	...
	function open_trims_dtls(po_break_down_id,tot_po_qnty,ratio,page_title,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/total_order_status_followup_sheet_report_controller.php?po_break_down_id='+po_break_down_id+'&tot_po_qnty='+tot_po_qnty+'&ratio='+ratio+'&action='+action, page_title, 'width=670px,height=400px,center=1,resize=0,scrolling=0','../../');
	}
	function generate_ex_factory_popup(action,job,id,width)
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/total_order_status_followup_sheet_report_controller.php?action='+action+'&job='+job+'&id='+id, 'Ex-Factory Details', 'width='+width+',height=400px,center=1,resize=0,scrolling=0','../../');
	}
	
	function openmypage_rej(po_id,company,action,reportType)
	{
		//alert(country_id);
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/total_order_status_followup_sheet_report_controller.php?po_id='+po_id+'&company='+company+'&action='+action+'&reportType='+reportType, 'Reject Quantity', 'width=600px,height=350px,center=1,resize=0,scrolling=0','../../');
	}	
	function openmypage(req_id,job_no,action,type) {
				var popup_width = '1090px';
			
			//alert(action);
			emailwindow = dhtmlmodal.open('EmailBox', 'iframe', 'requires/total_order_status_followup_sheet_report_controller.php?req_id=' + req_id + '&action=' + action + '&job_no=' + job_no + '&type=' + type, 'Detail Veiw', 'width=' + popup_width + ', height=450px,center=1,resize=0,scrolling=0', '../../');
		}
            ...
            <fieldset style="width:1100px;">
                <table class="rpt_table" width="1100" cellpadding="0" cellspacing="0" align="center" rules="all">
                    ...
                    <tr class="general">                   
                        <td> 
							<?
                            echo create_drop_down( "cbo_company_name", 130, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/total_order_status_followup_sheet_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
