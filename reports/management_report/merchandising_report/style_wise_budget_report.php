...
var permission = '<? echo $permission; ?>';
 	 function open_order_no()
	 {
		...
		var page_link='requires/style_wise_budget_report_controller.php?action=job_no_search_popup&company='+company+'&style=1'; 
		...
		emailwindow.onclose=function()
			{
				...
				$("#txt_order_no").val(styleDescription);
	  ...
	  function open_job_no()
	 {
	    ...
	    var page_link='requires/style_wise_budget_report_controller.php?action=job_no_search_popup&company='+company+'&style=0'; 
...
function generate_report(type)
	{
		...
		if((txt_job_no.length==0 && hidden_job_id.length==0) && (txt_date_from.length==0 && txt_date_to.length==0) && (txt_order_no.length==0 && hidden_order_id.length==0))
		{
			alert('Job or Order or Date Mandatory');
			return;
		}
		var report_title=$( "div.form_caption" ).html();
		var data="action=generate_report"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_job_no*cbo_year_selection*txt_order_no*hidden_job_id*hidden_order_id*txt_date_from*cbo_date_type*txt_date_to*txt_int_ref*txt_file_no',"../../../")+'&report_title='+report_title+'&type='+type;
		freeze_window(3);
		http.open("POST","requires/style_wise_budget_report_controller.php",true);
...
function openmypage_embl(company_id,order_id,order_number,insert_date,type,action,width,height,embl_type,color_id)
	...
	function openmypage_buyer_ins(style,po_id,action)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/style_wise_budget_report_controller.php?data='+data+'&action='+action, 'Inspection View', 'width=550px,height=300px,center=1,resize=0,scrolling=0','../../../');
	}


	function change_date_title(type)
	{
		if(type==1)
		{
			$("#search_by_td_up").text("Publish Shipment Date Range");
		}
		else{
			$("#search_by_td_up").text("Original Shipment Date Range");
		}
	}
            ...
            <table class="rpt_table" width="1300px" cellpadding="0" cellspacing="0" align="center">
               <thead>                    
                       <tr>
                        ...
                        <th class="must_entry_caption" width="150" colspan="2" > <span id="search_by_td_up">Publish Shipment Date Range</span> </th>
                ...
                <tr class="general">
                    <td width="140"> 
                        <?
                            echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/style_wise_budget_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                        ?>
                    </td>
                    ...
                    <td width="110">
                    	 <input type="text" id="txt_int_ref"  name="txt_int_ref"  style="width:120px" class="text_boxes"  placeholder="Write" />
                    </td>
                    <td width="110">
                    	 <input type="text" id="txt_file_no"  name="txt_file_no"  style="width:120px" class="text_boxes"  placeholder="Write" />
                    </td>
                    <td width="">
                       <input type="text" id="txt_job_no"  name="txt_job_no"  style="width:120px" class="text_boxes" onDblClick="open_job_no();$('#hidden_job_id').val('');$('#txt_job_no').val('');" placeholder="Browse/Write" />
                       <input type="hidden" id="hidden_job_id"  name="hidden_job_id" />
                    </td>
                    <td width="" id="location_td">
                     <input type="text" id="txt_order_no"  name="txt_order_no"  style="width:120px" class="text_boxes" onDblClick="open_order_no();$('#hidden_order_id').val('');$('#txt_order_no').val('');" placeholder="Browse/Write" />
                     ...
                     <?
					     ...
					     $cbo_date_type=array(1=>"Publish Shipment",2=>"Original Shipment");
					     $dd = "change_date_title(this.value) ";
                         echo create_drop_down( "cbo_date_type", 110, $cbo_date_type,"", 0, "--Select--", 1, $dd,0,"" );
                     ?>
                     </td>
...
<script type="text/javascript">
	...
	{  
		var company=$("#cbo_company_name").val(); 		 
		load_drop_down( 'requires/style_wise_budget_report_controller',company, 'load_drop_down_buyer', 'buyer_td' );
	}
</script>
