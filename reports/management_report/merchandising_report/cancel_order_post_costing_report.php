...
<script>
	...
	function fn_report_generated()
	{
		...
		{	
			var data="action=report_generate"+get_submitted_data_string('cbo_company_name*cbo_buyer_name*txt_job_no*cbo_year*txt_order_no*txt_file_no*txt_ref_no*hide_order_id*txt_date_from*txt_date_to*shipping_status',"../../../");
			freeze_window(3);
			http.open("POST","requires/cancel_order_post_costing_report_controller.php",true);
	...
	function openmypage_order()
	{
		...
		var page_link='requires/cancel_order_post_costing_report_controller.php?action=order_no_search_popup&companyID='+companyID;
...
'<html><head><link rel="stylesheet" href="../../css/style_common.css" type="text/css"/><title></title></head><body>'+document.getElementById(container_div).innerHTML+'</body</html>');
	...
	function openmypage(po_id,type,tittle)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/cancel_order_post_costing_report_controller.php?po_id='+po_id+'&action='+type, tittle, 'width='+popup_width+', height=400px, center=1, resize=0, scrolling=0', '../../');
	}
	
	function openmypage_mkt(mkt_data,type,tittle)
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/cancel_order_post_costing_report_controller.php?mkt_data='+mkt_data+'&action='+type, tittle, 'width=660px, height=200px, center=1, resize=0, scrolling=0', '../../');
	}
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/cancel_order_post_costing_report_controller.php?action='+action+'&job='+job+'&id='+id, 'Ex-Factory Details', 'width='+width+',height=400px,center=1,resize=0,scrolling=0','../../');
	}	
</script>
        ...
        <? echo load_freeze_divs ("../../../"); ?>
         <h3 align="left" id="accordion_h1" style="width:1150px" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel', '')"> -Search Panel</h3>
            ...
            <fieldset style="width:1150px;">
                <table class="rpt_table" width="1140" cellpadding="0" cellspacing="0" border="1" rules="all" align="center">
                	<thead>
                    	<tr>                   
                    ...
                    <tr class="general">
                        <td> 
                            <?
                                echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active =1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/cancel_order_post_costing_report_controller',this.value, 'load_drop_down_buyer', 'buyer_td' );" );
                            ?>
                        </td>
                        <td id="buyer_td">
                            <? 
                        ...
                        <td>
                        	<?
								echo create_drop_down( "shipping_status", 120, $shipment_status,"", 0, "-- Select --", 0, "",0,'','','','','' );
							?>
                        </td>
