...
if($action=="report_generate_sheet_kal") //Yearly Top Sheet of KAL
{
	...
	//echo $cm_cost_method_based_on.'DD';die;
	...
	$po_id_array=array();
	$sql_fin_prod="SELECT a.location as LOCATION, a.embel_name as EMBEL_NAME, a.po_break_down_id as POID, a.item_number_id as ITEM_ID, b.color_size_break_down_id as COLOR_SIZE_BREAK_DOWN_ID";
	...
	$sql_fin_prod.=", (CASE WHEN a.production_type=5 THEN b.production_qnty END) AS SEW_OUT from pro_garments_production_mst a,pro_garments_production_dtls b where a.production_source in(1) and a.production_type in(5,3) and a.serving_company in($cbo_company_id) and a.production_date between '$startDate' and '$endDate' and a.status_active=1 and a.is_deleted=0 and a.location>0 and a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and b.production_type in(5,3) and b.production_qnty>0 ";//and a.location in(3,5) 
	 // //and NVL(a.location,0)<> 0
	...
	//echo count($po_id_array); die;

	// ========================= for kniting ======================
	$sql_kniting_dyeing="SELECT b.po_breakdown_id as POID, b.is_sales as IS_SALES, a.febric_description_id as DETER_ID,c.receive_date as RECEIVE_DATE";
	foreach($fiscal_year_arr as $fyear=>$ydata)
	{
		$exydata=explode("_",$ydata);
		$fyear=str_replace("-","_",$fyear);
		
		$sql_kniting_dyeing.=", (CASE WHEN c.receive_date between '$exydata[0]' and '$exydata[1]' THEN b.quantity END) AS M$fyear ";
	}
   ...
   //and c.knitting_location_id in(3,5
	...
	$dying_prod_sql.="  from pro_fab_subprocess c, pro_batch_create_mst a,pro_batch_create_dtls d,product_details_master f where c.batch_id=a.id and a.id=d.mst_id  and c.batch_id=d.mst_id and f.id=d.prod_id and c.load_unload_id=2 and a.batch_against=1  and c.entry_form=35 and c.service_company in($cbo_company_id)  and c.process_end_date between '$startDate' and '$endDate' and a.status_active=1 and d.status_active=1 and c.status_active=1  order by c.floor_id,c.process_end_date ";
	...
	$sql_sub_sewOut.=" from subcon_gmts_prod_dtls a where  a.company_id in($cbo_company_id) and a.production_date between '$startDate' and '$endDate' and a.production_type=2  and a.status_active=1 and a.location_id in(3,5)";
	...
	// =================================== subcon kniting =============================
	$sql_sub_knit="SELECT b.order_id as ORDER_ID,b.process as PROCESS,a.product_date as PRODUCT_DATE";
	foreach($fiscal_year_arr as $fyear=>$ydata)
	{
		$exydata=explode("_",$ydata);
		$fyear=str_replace("-","_",$fyear);
	...
	$sql_sub_knit.=" from subcon_production_mst a, subcon_production_dtls b where a.id=b.mst_id and a.entry_form in(292,159)  and a.product_date between '$startDate' and '$endDate' and a.product_type in(2,4)    and a.status_active=1 and b.status_active=1";
	...
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 3, 1, $po_id_array, $empty_arr);//PO ID Ref from=1
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 3, 2, $sales_id_array, $empty_arr);//Sales ID Ref from=2
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 3, 3, $sub_po_id_array, $empty_arr);//Subcon PO ID Ref from=3
	...
	$cm_sql = "SELECT c.costing_date as COSTING_DATE, c.costing_per as COSTING_PER, c.sew_smv as SEW_SMV, a.cm_cost as CM_COST, b.id as POID, d.smv_set as SMV_SET, d.gmts_item_id as GMTS_ID from wo_pre_cost_dtls a, wo_po_break_down b, wo_pre_cost_mst c, wo_po_details_mas_set_details d, gbl_temp_engine g where c.job_id=b.job_id and c.job_id=a.job_id and  a.job_id=b.job_id and d.job_id=b.job_id and d.job_id=c.job_id  and g.ref_val=b.id and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=3 and a.status_active=1 and b.status_active=1 and c.status_active=1";
	...
	$sql_sub_dye.=" from subcon_production_mst a, subcon_production_dtls b where a.id=b.mst_id and a.entry_form=292 and a.company_id in($cbo_company_id) and a.product_date between '$startDate' and '$endDate' and a.product_type=4 and a.location_id in(3,5) ";
	...
	{	
		foreach($fiscal_year_arr as $fyear=>$ydata)
		{
			...
			//if($sew_smv) echo $sew_smv.'='.$sew_out.'='.$myear.',';die;
			...
			if($val[$pmyear]>0)
			{
				...
				$print_amount=$val[$pmyear]*$print_avg_rate;
			...
			if($sew_smv>0)
			{
				...
				if($sew_out>0 && ($val['LOCATION']==3 || $val['LOCATION']==5))
				{
	...
	foreach ($sql_kniting_dyeing_res as $val) 
	{
		foreach($fiscal_year_arr as $fyear=>$ydata)
		{
			$nyear=str_replace("-","_",$fyear);
			$myear='M'.$nyear;
			// $main_array[$fyear]['qty']+=$val[csf($myear)]; IS_SALES
			$is_sales_id=$val['IS_SALES'];
			$knittingProdQty=$val[$myear];
			$kniting_cost=0;
			$kniting_qty=0;					
			if($val['POID']>0)
			{
				...
				else // For Sales
				{
					...
					//echo $knit_rate.'='.$val[csf($myear)].'<br>';
				}
				//echo $knit_rate.'='.$val[csf($myear)].'='.$val[csf('po_breakdown_id')].'<br>';
				if($knit_rate>0)
				{
					$knitingCost =$knit_rate*$knittingProdQty;	
					$main_array[$fyear]['kniting'] += $knitingCost;
				}
			}
			//$sales_data_array[$val[csf('id')]][$val[csf('deter_id')]][$val[csf('color_id')]][$p_key]['process_rate'] 	
		}
	}
				...
				if($po_id>0)
				{
					if(!in_array($key, $process_array ))
					{
						if($is_sales_id!=1)
						{
							...
							{
								//$po_color_fab_array2[$val[csf('id')]][$val[csf('deter_id')]][$val[csf('cons_process')]]['rate']
						...
						//echo array_sum($conversion_costing_arr[$val[csf('po_breakdown_id')]][$key]).', ';
						if($conv_amount>0)
						{
							$dying_prod_qty_array[$fyear][$val['FLOOR_ID']]['dyeing_prod']+=$conv_amount;
						}
					}
				}
			}
	...
	{	
		foreach($fiscal_year_arr as $fyear=>$ydata)
		{
			...
			if($subKnit_cost>0)
			{
				...
				$main_array[$fyear]['subKnit'] += $subKnit_costUSD;
			}
			if(!(in_array($fab_dyeing_chk_key, $processArr )))
			{
				if($subFinish_cost>0)
				{
					...
					$main_array[$fyear]['subFinish'] += $subFin_costUSD;
				}
			}
		}
	}
	...
	<!--=============Total Summary Start==================================================================-->
		...
		<!-- <caption></caption> -->
		        ...
		        <?
		        	...
		        	foreach ($fiscal_year_arr as $year => $val) 
		        	{
				        ...
				        <tr bgcolor="<? echo $bgcolor; ?>">
				            <td><a href="javascript:void()" onclick="report_generate_by_year('<? echo $year?>','1')"><? echo $year;?></a></td>
				            <?
				            $total_gmts = 0;
				           ...
				           // $year_total = $fiscal_total+$main_array[$year]['dyeing']+$main_array[$year]['kniting'];
				            ...
				            foreach ($floorArray as $floor_id => $floor_name) 
				            {
				            	$dying_prod_qty=$dying_prod_qty_array[$year][$floor_id]['dyeing_prod']+$dying_prod_qty_array[$year][$floor_id]['sub_dyeing_prod'];
								  ...
								  ?>
				            	<td align="right" title="Without Knitting+YarnDying+AOP Rate from PreCost*Dying Prod Qty+SubConDyeing Prod*SubCOn Order Rate(<? echo $dying_prod_qty_array[$year][$floor_id]['sub_dyeing_prod'];?>)"><?  echo number_format($dying_prod_qty,0);?></td>
				            	<?
				            ...
				            <td align="right" title="Knit Prod*Pre Cost Knit avg Rate+SubCon Knit(<? echo $main_array[$year]['subKnit'];?>)"><? echo number_format($main_array[$year]['kniting']+$main_array[$year]['subKnit'],0); ?></td>
				            ...
				            <td align="right"><? echo number_format($year_prod_cost_arr[$year]['wash_recv'],0); ?></td>
							 ...
							 echo number_format($tot_textile,2); ?></td>
	        ...
	        <tfoot>
	            ...
	            <th><? echo number_format($gr_kniting_total,0); ?></th>
...
if($action=="print_prod_kal") //
{
	...
	foreach ($sql_fin_prod_res as $val)   //Main Query
	{
		 ...
		 //echo $color_id.'='.$item_id.',';
			...
			// echo $print_avg_rate.'='.$dzn_qnty.'<br>';
			...
			// echo $print_amount.'='.$print_qty.'<br>';
