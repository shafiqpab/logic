...
function poWiseExmfgCost_back($poids,$type)
{
	//echo load_html_head_contents("Excess Mat. Cost", "../../../../", 1, 1,$unicode,'','');
	//echo $poid.'=';//die;
	//$expData=explode('__',$data);
	...
	if(!empty($poids))
	{
		...
		//echo "ff"; die;
		//Item Ratio Details
		$gmtsitemRatioSql="select a.job_id AS JOB_ID, a.gmts_item_id AS GMTS_ITEM_ID, a.set_item_ratio AS SET_ITEM_RATIO from wo_po_details_mas_set_details a, gbl_temp_engine b where 1=1 and a.id=b.ref_val and b.user_id = ".$user_id." and b.entry_form=880 and b.ref_from='$refFormJobCond'";
		//echo $gmtsitemRatioSql; die;
		$gmtsitemRatioSqlRes = sql_select($gmtsitemRatioSql);
		$jobItemRatioArr=array();
		foreach($gmtsitemRatioSqlRes as $row)
		...
		$sqlYIssue="SELECT a.id as issue_id, a.issue_number, a.booking_no, a.knit_dye_source, a.knit_dye_company, b.quantity as issue_qnty, b.po_breakdown_id, b.prod_id, d.cons_rate, a.issue_purpose from inv_issue_master a, order_wise_pro_details b, inv_transaction d, gbl_temp_engine e
			where a.id=d.mst_id and d.transaction_type=2 and d.item_category=1 and d.id=b.trans_id and b.trans_type=2 and b.entry_form=3 
			and b.po_breakdown_id=e.ref_val and e.user_id = ".$user_id." and e.entry_form=880 and e.ref_from='$refFormPoCond'
			 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.issue_purpose  in (1,2,15,50) order by a.id ASC";
		//echo $sqlYIssue; die;
		$sqlYarnIssue=sql_select($sqlYIssue);
		$yarnStolenArr=array(); $yarnratearr=array(); $yarnWoArr=array();
		foreach($sqlYarnIssue as $yirow)
		...
		$sqlGYIssue="SELECT a.id as issue_id, b.quantity as issue_qnty, b.po_breakdown_id, b.prod_id, c.cons_rate, d.lot, d.brand_supplier, d.yarn_count_id, d.yarn_comp_type1st from inv_issue_master a, order_wise_pro_details b, inv_transaction c, product_details_master d, gbl_temp_engine e
		...
		$sqlGray="select a.id,b.id as dtls_id, c.po_breakdown_id, b.prod_id, b.yarn_prod_id, b.febric_description_id, d.product_name_details, c.po_breakdown_id, c.quantity as quantity, b.kniting_charge, b.order_yarn_rate,a.knitting_source from inv_receive_master a, pro_grey_prod_entry_dtls b, order_wise_pro_details c, product_details_master d, gbl_temp_engine e
		...
		foreach($sqlGrayRec as $grrow)
		{
			$grayDataArr[$grrow[csf("po_breakdown_id")]][$grrow[csf("product_name_details")]]['yprodid'].=','.$grrow[csf("yarn_prod_id")];
			$grayDataArr[$grrow[csf("po_breakdown_id")]][$grrow[csf("product_name_details")]]['grecqty']+=$grrow[csf("quantity")];
			...
			if($grrow[csf("knitting_source")]==1)
			{
				$grayDataArr[$grrow[csf("po_breakdown_id")]][$grrow[csf("product_name_details")]]['grecamt']+=$grrow[csf("quantity")]*($grrow[csf("kniting_charge")]/82);
				$prodrateArr[$grrow[csf("product_name_details")]]=($grrow[csf("kniting_charge")]/82);
			}
			else
			{
				$grayDataArr[$grrow[csf("po_breakdown_id")]][$grrow[csf("product_name_details")]]['grecamt']+=$grrow[csf("quantity")]*($recv_wise_knitting_charge[$grrow[csf('id')]]/82);
				$prodrateArr[$grrow[csf("product_name_details")]]=($recv_wise_knitting_charge[$grrow[csf('id')]]/82);
			}
			...
			foreach($exyarnid as $ynid)
			{
				$greyYarnDtlsArr[$grrow[csf("po_breakdown_id")]][$grrow[csf("product_name_details")]][$ynid]['yrecqty']+=$yusedArr1[$ynid][$grrow[csf("id")]][$grrow[csf("dtls_id")]]['yqty'];
				$greyYarnDtlsArr[$grrow[csf("po_breakdown_id")]][$grrow[csf("product_name_details")]][$ynid]['yrecamt']+=($yusedArr1[$ynid][$grrow[csf("id")]][$grrow[csf("dtls_id")]]['amount']/82);
				$grayDataArr[$grrow[csf("po_breakdown_id")]][$grrow[csf("product_name_details")]]['yrntotamt']+=($yusedArr1[$ynid][$grrow[csf("id")]][$grrow[csf("dtls_id")]]['amount']/82);
		...
		foreach($sqlTransRes as $gtrrow)
		{
			if($gtrrow[csf("trans_type")]==5)
			{
				$greyTransinDtlsArr[$gtrrow[csf("po_breakdown_id")]][$gtrrow[csf("product_name_details")]]['trinpoid'].=$gtrrow[csf("from_order_id")].',';
				$greyTransinDtlsArr[$gtrrow[csf("po_breakdown_id")]][$gtrrow[csf("product_name_details")]]['trinqty']+=$gtrrow[csf("transfer_qnty")];
				//$greyTransinDtlsArr[$gtrrow[csf("product_name_details")]]['trinamt']+=($gtrrow[csf("transfer_qnty")]*$prodrateArr[$gtrrow[csf("product_name_details")]]);
				//change here
				$greyTransinDtlsArr[$gtrrow[csf("po_breakdown_id")]][$gtrrow[csf("product_name_details")]]['trinamt']+=($gtrrow[csf("transfer_value")]/82);
				//array_push($trnsPoIdArr,$gtrrow[csf('from_order_id')]);
				//echo $gtrrow[csf('from_order_id')].'i <br>';
			}
			else if($gtrrow[csf("trans_type")]==6)
			{
				$greyTransDtlsArr[$gtrrow[csf("po_breakdown_id")]][$gtrrow[csf("product_name_details")]]['troutpoid'].=$gtrrow[csf("to_order_id")].',';
				$greyTransDtlsArr[$gtrrow[csf("po_breakdown_id")]][$gtrrow[csf("product_name_details")]]['troutqty']+=$gtrrow[csf("transfer_qnty")];
				$greyTransDtlsArr[$gtrrow[csf("po_breakdown_id")]][$gtrrow[csf("product_name_details")]]['troutamt']+=$gtrrow[csf("transfer_qnty")]*$prodrateArr[$gtrrow[csf("product_name_details")]];
				//array_push($trnsPoIdArr,$gtrrow[csf('to_order_id')]);
				//echo $gtrrow[csf('to_order_id')].' <br>';
			}
			$trnsPoIdArr[$gtrrow[csf('to_order_id')]]=$gtrrow[csf('to_order_id')];
		...
		//Transfer In Status
		foreach($greyTransinDtlsArr as $poid=>$podata)
		{
			foreach($podata as $fabricdtls=>$fabricdtlsdata)
			{
				$bomTransinAvgRate=0;
				...
				$gTransinQty[$poid]+=$fabricdtlsdata['trinqty'];
				$gTransinAmt[$poid]+=$fabricdtlsdata['trinamt'];
			}
		}
		...
		$sql_outsite_service="SELECT a.febric_description_id, a.process_id, a.wo_qty, a.batch_issue_qty, a.rate, a.amount, a.order_id, a.fin_dia, a.fin_gsm, a.grey_used, c.fabric_color_id, c.po_break_down_id, a.id
			...
			WHERE a.mst_id = b.id AND a.booking_dtls_id = c.id and c.po_break_down_id=e.ref_val and e.user_id = ".$user_id." and e.entry_form=880 and e.ref_from='$refFormPoCond' AND b.entry_form = 92 AND c.is_deleted=0 AND a.is_deleted=0 AND b.is_deleted=0";
		//echo $sql_outsite_service; die;
		...
		{
			$comp_ids[$row[csf('copmposition_id')]]=$row[csf('copmposition_id')];
		}
		$comp_cond=where_con_using_array($comp_ids,0,"id");
		$composition = return_library_array("select id,composition_name from  lib_composition_array where status_active=1 and is_deleted=0 $comp_cond", "id", "composition_name");
		foreach($data_array as $row )
		{
			$fab_id=$row[csf('id')];
		...
		foreach($batchDataArr as $poid=>$podata)
		{
			foreach($podata as $fabcolorid=>$fabcolordata)
			{
				foreach($fabcolordata as $fabric=>$fabdata)
				{
					...
					$greyAvgRate=$productWiseRate[$fabric];
			...
			foreach($podata as $fabricdtls=>$fabricdtlsdata)
			{
				foreach($fabricdtlsdata as $fabriccolor=>$colordata)
				{
					if($batchEntryArr[$colordata['batchid']]!=37)
					{
						...
						$gfinishAmt[$poid]+=$colordata['finTin_amt'];
					}
				}
			}
		}
...
function poWiseExmfgCost($poids,$type)
{
	...
	if(!empty($poids))
	{
		...
		foreach($sqlYarnRes as $row)
		{
			...
			$consQnty=$row['CONS_QNTY'];//($row['REQUIRMENT']*$row['CONS_RATIO'])/100;
			
			$yarnReq=($planQty/$itemRatio)*($consQnty/$costingPer);
			
			$yarnAmt=$yarnReq*$row['RATE'];
			
			//echo $planQty.'='.$itemRatio.'='.$row['CONS_QNTY'].'='.$costingPer.'='.$yarnReq.'='.$yarnAmt.'<br>';
			$reqYarnArr[$row['POID']][$row['COPM_ONE_ID']][$row['COUNT_ID']]['yarn_qty']+=$yarnReq;
			$reqYarnArr[$row['POID']][$row['COPM_ONE_ID']][$row['COUNT_ID']]['yarn_amt']+=$yarnAmt;
		}
		unset($sqlYarnRes);
		
		//print_r($reqYarnArr); die;
	...
	//print_r($gExcessFinishFabCostArr);
	//return fn_number_format($gExcessFinishFabCost,8,".","");
        ...
        <div class="podtls" style="width:1400px; font-size:14px; font-weight:bold; color:Black; background-color:#008040" align="center"><?="PO Details of Buyer by Season wise :". $season_sql[0][csf('season_name')]; ?></div>
            <table class="rpt_table" border="1" cellpadding="2" cellspacing="2" style="width:1400px; margin-top:5px" rules="all">
				...
				foreach($po_dtls_arr as $poid=>$val)
				{
					...
					$excessFinishFabCost=$exMatCostP[$poid];
...
if($action=="exmfg_details_list_view")
{
	...
	where 1=1 and a.id=b.pre_cost_fabric_cost_dtls_id and b.cons!=0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.po_break_down_id='$poid' and a.fabric_source=1 ";
		...
		$sql_outsite_service="SELECT   a.febric_description_id,
								       a.process_id,
								       a.wo_qty,
								       a.batch_issue_qty,
								       a.rate,
								...
								WHERE     a.mst_id = b.id
								       AND a.booking_dtls_id = c.id
								       AND b.entry_form = 92
								       AND c.is_deleted=0
								       AND a.is_deleted=0
								       AND b.is_deleted=0
								       AND c.po_break_down_id = $poid";
		...
		$sql_heat_set=" SELECT
						...
						WHERE     a.id = b.mst_id
						         ...
						         AND b.po_id = $poid
						         AND a.is_deleted = 0
						GROUP BY c.product_name_details,
						        c.detarmination_id,
						         a.color_id";
		//echo $sql_heat_set;
            ...
            <table cellpadding="0" width="1340px" class="rpt_table" rules="all" border="1">
				...
				foreach($batchDataArr as $fabcolorid=>$fabcolordata)
				{
					foreach($fabcolordata as $fabric=>$fabdata)
					{
						...
						//$fabdata['batch_amt']/$fabdata['batch_qty'];
						...
						<tr bgcolor="<?=$bgcolor; ?>" >
							...
							<td align="right" title="InHouse=<?=$data_heat_set[$fabcolorid][$fabric]['qnty'];?>,OutSite=<?=$data_outsite_service[$fabcolorid][$fabric][33]['qnty'];?>"><?=number_format($heatSetQty,2); ?></td>
							<td align="right"><?=number_format($heatSetRate,2); ?></td>
							<td align="right" title="InHouse=<?=$data_heat_set[$fabcolorid][$fabric]['amount'];?>,OutSite=<?=$data_outsite_service[$fabcolorid][$fabric][33]['amount'];?>"><?=number_format($heatSetAmt,2); ?></td>
							...
							<td align="right" title="InHouse=<?=$specialDataArr[67][$fabcolorid][$fabric]['sp_qty'];?>,OutSite=<?=$data_outsite_service[$fabcolorid][$fabric][67]['qnty'];?>"><?=number_format($peachFinishQty,2); ?></td>
							<td align="right"><?=number_format($peachFinishRate,2); ?></td>
							<td align="right" title="InHouse=<?=$specialDataArr[67][$fabcolorid][$fabric]['sp_amt'];?>,OutSite=<?=$data_outsite_service[$fabcolorid][$fabric][67]['amount'];?>"><?=number_format($peachFinishAmt,2); ?></td>
							
							<td align="right" title="InHouse=<?=$specialDataArr[68][$fabcolorid][$fabric]['sp_qty'];?>,OutSite=<?=$data_outsite_service[$fabcolorid][$fabric][68]['qnty'];?>"><?=number_format($brushingQty,2); ?></td>
							<td align="right"><?=number_format($brushingRate,2); ?></td>
							<td align="right" title="InHouse=<?=$specialDataArr[68][$fabcolorid][$fabric]['sp_amt'];?>,OutSite=<?=$data_outsite_service[$fabcolorid][$fabric][68]['amount'];?>"><?=number_format($brushingAmt,2); ?></td>
							
							<td align="right" title="InHouse=<?=$specialDataArr[35][$fabcolorid][$fabric]['sp_qty'];?>,OutSite=<?=$data_outsite_service[$fabcolorid][$fabric][35]['qnty'];?>"><?=number_format($aopQty,2); ?></td>
							<td align="right"><?=number_format($aopRate,2); ?></td>
							<td align="right" title="InHouse=<?=$specialDataArr[35][$fabcolorid][$fabric]['sp_amt'];?>,OutSite=<?=$data_outsite_service[$fabcolorid][$fabric][35]['amount'];?>"><?=number_format($aopAmt,2); ?></td>
                ...
                <tfoot>
					<tr class="tbl_bottom">
                        ...
                        <td align="right"><?=number_format($gHeadSetQty,2); ?></td>
                        <td align="right">&nbsp;</td>
                        <td align="right"><?=number_format($gHeadSetAmt,2); ?></td>
