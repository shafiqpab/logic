...
if($action=="create_po_search_list_view")
{
	...
	if($budget_version==1){
		...
		$budget_version_cond="and f.entry_from=520";
	}
	else{
	...
	if ($data[2]==0)
	{
		if($cbo_search_by==1 || $cbo_search_by==2)
		{
			...
			if($db_type==2)
			{
				...
				$sql="SELECT a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity, b.id as po_id,b.po_number, sum(b.po_quantity) as po_quantity,b.shipment_date,a.garments_nature,b.grouping,b.file_no,(pub_shipment_date - po_received_date) as date_diff,to_char(a.insert_date,'YYYY') as year from wo_po_details_master a
				join wo_po_break_down b on a.id=b.job_id join wo_po_color_size_breakdown c on a.id = c.job_id and b.id=c.po_break_down_id join wo_pre_cost_mst f on	a.id=f.job_id where a.garments_nature=$data[5] and a.status_active=1 and b.status_active=1  $company $buyer ".set_user_lavel_filtering(' and a.buyer_name','buyer_id')." $job_cond $order_cond $style_cond $shiping_status $file_no_cond $internal_ref_cond $date_cond $budget_version_cond group by a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity,b.id, b.po_number,b.shipment_date,a.garments_nature,b.grouping,b.file_no,a.insert_date,b.pub_shipment_date,b.po_received_date order by a.job_no DESC";
				/*echo $sql;*/
			}
		}
		else
 ...
 if($action=="show_transfer_popup")
{
		...
		$po_id = implode(",",explode("_",$po_id));
		 ...
		 $sql_wopo="select b.id, b.po_number,b.job_no_mst, b.grouping from wo_po_break_down b where b.status_active=1 and b.is_deleted=0 and b.id in($po_id) ";
		$data_result=sql_select($sql_wopo);
		$po_dtls_arr=array();
		foreach($data_result as $row){
			$po_dtls_arr[$row[csf("id")]]['po_no']=$row[csf("po_number")];
			$job=$row[csf("job_no_mst")];
		...
		$per_kg_yarn=$jobWise_yarn_amt/$jobWise_yarn_qty;
                ...
                foreach ($result as $row) {
					...
					/*
						...
						$transfer_amount=($row[csf('transfer_qnty')]*$per_kg_yarn)+($row[csf('transfer_qnty')]*$gray_charge_unit);
                    ...
                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                        ...
                        <td align="right" title="Avg Rate: <? echo 'Transfer Amount: Transfer Qnty*per kg yarn+ Transfer Qnty*gray charge unit'; ?>"><? //echo number_format(($row[csf('transfer_qnty')]*$avg_rate)/$g_exchange_rate, 2);
                ...
                foreach ($result as $row) {
                    if ($i % 2 == 0)
						...
						$transfer_amount=($row[csf('transfer_qnty')]*$per_kg_yarn)+($row[csf('transfer_qnty')]*$gray_charge_unit);
						//$transfer_amount_out=$row[csf('transfer_qnty')]*();
                    ...
                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                        ...
                        <td width="170"  style="word-break: break-all;" title="Product Id: <?= $row[csf('prod_id')]; ?>"><p><? echo $row[csf('product_name_details')]; ?></p></td>
                        <td  width="100" align="right"><? echo number_format($row[csf('transfer_qnty')], 2); ?> </td>
                         <td align="right"  title="Avg Rate: <? echo 'Transfer Amount: Transfer Qnty*per kg yarn+ Transfer Qnty*gray charge unit'; ?>"><? 
						//  $transfer_amount=$row[csf('transfer_amount')]/$g_exchange_rate;
						  echo number_format($transfer_amount, 2);
            ...
            <table border="1" class="rpt_table" rules="all" width="850" cellpadding="0" cellspacing="0">
                ...
                foreach ($result as $row) {
					...
					// $knit_cost=$knit_qty=0;
						...
						$transfer_amount=($row[csf('transfer_qnty')]*$per_kg_yarn)+($row[csf('transfer_qnty')]*$gray_charge_unit)+($row[csf('transfer_qnty')]*$finish_charge_unit);
                ...
                foreach ($result as $row) {
                    ...
                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                        ...
                        <td width="170"  style="word-break: break-all;" title="Product Id: <?= $row[csf('prod_id')]; ?>"><p><? echo $row[csf('product_name_details')]; ?></p></td>
                        <td align="right"><? echo number_format($row[csf('transfer_qnty')], 2); ?> </td>
                        <td align="right"  title="Avg Rate: <? echo 'Transfer Amount: Transfer Qnty*per_kg_yarn+Qnty*gray charge unit+Qnty*Finish charge unit'; ?>">
                        	<?  
                        	//$transfer_amount=$row[csf('transfer_amount')]/$g_exchange_rate;
