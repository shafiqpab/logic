...
{ 
	...
	$all_po_id=""; $exchangepo_rateArr=array(); $exchangejob_rateArr=array();
    ...
    <fieldset>
    	...
    	<div style="width:3552px; max-height:400px; overflow-y:scroll" id="scroll_body">
            ...
            <?
				...
				{
					$exchange_rate=0;
					$exchange_rate=$exchangepo_rateArr[$finRow[csf("po_breakdown_id")]];
				...
				foreach($dataArrayFinPurchaseW as $finRow)
				{
					$exchange_rate=0;
					$exchange_rate=$exchangepo_rateArr[$finRow[csf("po_breakdown_id")]];
					$finish_purchase_amnt_arr[$finRow[csf('po_breakdown_id')]]+=$finRow[csf('woven_purchase_amnt')]/$exchange_rate;
				}
                    ...
                    <div id="summary_full"> <font color="#FF0000" style="display:none">*Yarn Dyeing Charge included with actual Yarn Cost</font>
                        ...
                        <table width="760" cellpadding="0" cellspacing="0" border="1" class="rpt_table" rules="all">
                            ...
                            <tr bgcolor="<? echo $bgcolor2; ?>" onClick="change_color('trtd_4','<? echo $bgcolor2; ?>')" id="trtd_4">
                                ...
                                <td align="right"><a href="#report_details" onClick="openmypage('<? echo $po_ids; ?>','yarn_cost','Yarn Cost Details')"><? echo fn_number_format($tot_yarn_cost_actual,2); ?></a></td>
                                ...
                                ?>
                                    ...
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trTD3_<? echo $j; ?>','<? echo $bgcolor; ?>')" id="trTD3_<? echo $j; ?>">
                                        ...
                                        <?
                                            ...
                                            echo fn_number_format($emb_var,2); 
                                        ?>
                                        </td>
                                        <td align="right">
                                        <?
    ...
    <fieldset>
    	...
    	<div style="width:3712px; max-height:400px; overflow-y:scroll" id="scroll_body">
            ...
            <?
				...
				$yarnTrimsDataArray=sql_select("select b.po_breakdown_id, b.prod_id, a.item_category, 
					sum(CASE WHEN a.transaction_type=2 and b.entry_form ='3' and b.trans_type=2 and b.issue_purpose!=2 THENa.cons_rate* b.quantity ELSE 0 END) AS yarn_iss_qty,
					sum(CASE WHEN a.transaction_type=4 and b.entry_form ='9' and b.trans_type=4 THEN a.cons_rate*b.quantity ELSE 0 END) AS yarn_iss_return_qty,
					sum(CASE WHEN a.transaction_type=5 and b.entry_form ='11' and b.trans_type=5 THEN a.cons_rate*b.quantity ELSE 0 END) AS trans_in_qty_yarn,
					sum(CASE WHEN a.transaction_type=6 and b.entry_form ='11' and b.trans_type=6 THEN a.cons_rate*b.quantity ELSE 0 END) AS trans_out_qty_yarn,
					sum(CASE WHEN a.transaction_type=2 and b.entry_form ='25' and b.trans_type=2 THEN a.cons_rate*b.quantity ELSE 0 END) AS trims_issue_amnt
...
if($action=="yarn_cost")
{
	...
	foreach($yarnIssDataArray as $invRow)
	{
		...
		$rate='';
		$rate=$avg_rate_array[$invRow[csf('prod_id')]];
            ...
            <div style="width:1030px; max-height:310px; overflow-y:scroll" id="scroll_body">
                <table class="rpt_table" width="1010" cellpadding="0" cellspacing="0" border="1" rules="all">
					...
					foreach($result as $row)
					{
						...
						$yarn_iss_cost=$dataArrayYarnIssue[$row[csf('id')]][2]/$exchange_rate;
...
if($action=="knitting_bill")
{
	...
	//$exchange_rate=80;
	$costing_per_arr=return_library_array( "select job_no, costing_per_id from wo_pre_cost_dtls where status_active=1 and is_deleted=0", "job_no", "costing_per_id");
	$exchange_rate=return_field_value("exchange_rate", "wo_pre_cost_mst a, wo_po_break_down b", "a.job_no=b.job_no_mst and a.status_active=1 and a.is_deleted=0 and a.id in ($po_id)");
	return_library_array( "select job_no, exchange_rate from wo_pre_cost_mst where status_active=1 and is_deleted=0", "job_no", "exchange_rate");
                    ...
                    <tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
                        ...
                        <td align="right" width="80">
							<? 
								...
								echo fn_number_format($yarn_trans_cost/$row[csf('transfer_qnty')],2); 
                            ?>&nbsp;
                        </td>
                        <td>
							<?
                    ...
                    <tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
                        ...
                        <td align="right" width="80">
							<? 
								...
								echo fn_number_format($yarn_trans_cost_out/$row[csf('transfer_qnty')],2); 
                            ?>&nbsp;
                        </td>
                        <td align="right">
							<?
	...
	<fieldset style="width:870px; margin-left:7px">
        ...
        <div style="width:865px; max-height:330px; overflow-y:scroll" id="scroll_body">
			<table border="1" class="rpt_table" rules="all" width="845" cellpadding="0" cellspacing="0">  
				...
				<?
                $i=1; //$avg_rate=76;
				$avg_rate=return_field_value("exchange_rate", "wo_pre_cost_mst a, wo_po_break_down b", "a.job_no=b.job_no_mst and a.status_active=1 and a.is_deleted=0 and a.id in ($po_id)");
...
if($action=="fabric_purchase_cost_actual")
{
	...
	extract($_REQUEST);
	//$ex_rate=76;
	$ex_rate=return_field_value("exchange_rate", "wo_pre_cost_mst a, wo_po_break_down b", "a.job_no=b.job_no_mst and a.status_active=1 and a.is_deleted=0 and a.id in ($po_id)");
?>
<script>
