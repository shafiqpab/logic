...
{	
	...
	else
	{
		...
		elseif(str_replace("'","",$cbo_date_type)==4)
		{
			$date_category='d.country_ship_date';
 ...
 $sql="select $date_diff a.buyer_name,a.total_set_qnty,a.job_no_prefix_num ,a.job_no,a.style_ref_no,b.po_number,a.set_smv,a.set_break_down,a.order_uom,a.gmts_item_id,b.id,a.buyer_name,(d.order_quantity) as po_quantity,d.order_quantity as  po_qty,d.plan_cut_qnty as plan_cut,d.order_rate as unit_price,$date_category as pub_shipment_date,b.po_received_date,b.insert_date,b.grouping, b.file_no,b.is_confirmed,b.inserted_by,d.order_total as po_total_price,b.details_remarks,a.id as job_id,d.country_ship_date  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown d where a.id=b.job_id and a.id=d.job_id  and b.id=d.po_break_down_id $company_name and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and d.is_deleted=0 and d.status_active=1 $search_text $buyer_id_cond $shipping_status_cond $po_cond_for_in $file_no_cond $ref_no_cond order by $date_category";  
        ...
        <div style="width:<?=$main_width+$new_column+20;?>px; max-height:350px; overflow-y:scroll" id="report_div" > 
            ...
            ?>	
                <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" style="font-size:12px; cursor:pointer;" valign="middle">
					...
					<td width="80" style="word-break: break-all;" align="right"><? echo number_format($row[csf('po_quantity')]*$row[csf('unit_price')],2);?></td>
                    ...
                    ?>
                    <td width="60" style="word-break: break-all;" align="right"><a href="javascript:void(0)" onclick="qty_details('load_cut_lay_data', '<?=implode(',', $lay_cut_data_dtls_id)?>', <?=$row[csf("id")]?>)"><? echo  omitZero(number_format($lay_cut_data_dtls[$row[csf("id")]]['lay_cut_qty']),0);//omitZero(number_format($lay_cut_data[$row[csf('job_no')]][$row[csf('buyer_name')]],0));?></a></td>
