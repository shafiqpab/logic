...
if($action=="report_generate")
{
	...
	//$sql_allocationl="select a.JOB_NO,a.COMPANY_ID,a.LOCATION_NAME,b.PO_NO as PO_ID,b.DATE_NAME,b.QTY,b.SMV from ppl_order_allocation_mst a,ppl_order_allocation_dtls b where a.id=b.mst_id AND a.STATUS_ACTIVE=1 and a.IS_DELETED=0 and b.STATUS_ACTIVE=1 and b.IS_DELETED=0 and b.DATE_NAME between '$s_date' and '$e_date'";
	...
	$sql_allocationlRes=sql_select($sql_allocationl); 
	$allocationArr=array();
            ...
            <table class="rpt_table" width="<?=$width; ?>" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body" align="left">
					...
					foreach($poMinuteArr as $company_location=>$dataRows)
					{
						...
						?>
						 <tr bgcolor="<?=$bgcolor;?>" onClick="change_color('tr_<?=$i; ?>','<?=$bgcolor;?>')" id="tr_<?=$i; ?>" style="font-size:12px">
							...
							<td width="60" rowspan="2" valign="middle" align="center"><?=$companyArr[$company_id]; ?></td>
							<td width="100" rowspan="2" valign="middle"><?=$locationArr[$location_id]; ?></td>
                ...
                <tfoot>
					<tr>
							...
							foreach($month_arr as $monthVal)
							{
								$capacity=$capacityMonthArr[$monthVal];
								$totalCapacity+=$capacity;
								?>
								<th width="90" align="right"><?=fn_number_format($capacity); ?>&nbsp;</th>
								<?
							}
							$i++;
							if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
							?>
							<th align="right"><?=fn_number_format($totalCapacity); ?>&nbsp;</th>
						 </tr>
	...
	echo "$html****$filename****$capacityStr****$allocatedStr****'$monthStr'****";
