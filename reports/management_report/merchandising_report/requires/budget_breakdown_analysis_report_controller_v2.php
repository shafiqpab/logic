					...
					<table width="570" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
						...
						<tbody>
							<tr>
								...
								<td align="center">
									<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<? echo $companyID; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+'<? echo $cbo_year_id; ?>'+'**'+'<? echo $cbo_month_id; ?>', 'create_job_no_search_list_view', 'search_div', 'budget_breakdown_analysis_report_controller', 'setFilterGrid(\'tbl_list_search\',-1)');" style="width:100px;" />
								</td>
							</tr>
						</tbody>
					</table>
...
if ($action == "create_job_no_search_list_view") {
	...
	if ($data[1] == 0) {
		if ($_SESSION['logic_erp']["data_level_secured"] == 1) {
			if ($_SESSION['logic_erp']["buyer_id"] != "") $buyer_id_cond = " and buyer_name in (" . $_SESSION['logic_erp']["buyer_id"] . ")";
			else $buyer_id_cond = "";
		} else {
			$buyer_id_cond = "";
		}
	...
	if ($search_by == 2) $search_field = "style_ref_no";
...
?>
	...
	else $buyer_name = " and b.buyer_name=$data[1]";
	if ($data[2] == "") $order_no = "";
	else $order_no = " and a.po_number=$data[2]";
	$job_no = str_replace("'", "", $txt_job_id);
	if ($db_type == 0) {
		if ($data[2] == "") $job_no_cond = "";
		else $job_no_cond = "  and FIND_IN_SET(b.job_no_prefix_num,'$data[2]')";
	} else if ($db_type == 2) {
		if ($data[2] == "") $job_no_cond = "";
		else $job_no_cond = " and job_no_prefix_num in($data[2]) ";
	}
	...
	echo  create_list_view("list_view", "Job No,Buyer,Style Ref.,Order No", "110,110,150,180", "610", "350", 0, $sql, "js_set_value", "id,po_number", "", 1, "0,buyer_name,0,0,0", $arr, "job_no_prefix_num,buyer_name,style_ref_no,po_number", "budget_breakdown_analysis_report_controller", 'setFilterGrid("list_view",-1);', '0,0,0,0,0', '');
	disconnect($con);
	exit();
}
...
if ($action == "report_generate") {
	...
	if ($db_type == 0) {
		...
		if (trim($cbo_year) != 0) $year_cond = " $year_field_con=$cbo_year";
		else $year_cond = "";
	}
	...
	if ($season == "") $season_cond = "";
	else $season_cond = " and a.season in('" . implode("','", explode(",", $season)) . "')";
	...
	else if (str_replace("'", "", $txt_order_id) != "" && str_replace("'", "", $txt_order_id) != 0) $order_id_cond_trans2 = " and b.wo_po_break_down_id in ($order_no)";
	else if ($order_num == "") $order_no_cond = "";
	else $order_no_cond = " and  b.po_number in ('$order_num') ";
	if ($db_type == 0) {
	...
	if ($cbo_search_type == 2) {
		...
		if ($report_type == 1) {
			if ($template == 1) {
				...
				$sql_budget = "select a.company_name,a.job_no_prefix_num, b.insert_date, c.insert_date as pre_insert_date, c.costing_date, b.update_date, a.job_no, a.buyer_name, a.style_ref_no, a.order_uom, b.is_confirmed, a.agent_name, a.avg_unit_price, a.dealing_marchant,a.team_leader, a.gmts_item_id, a.total_set_qnty as ratio, b.plan_cut, b.id as po_id, b.po_number, b.pub_shipment_date, b.po_received_date, b.po_quantity, b.unit_price, b.po_total_price, b.grouping, b.file_no,d.item_number_id,d.color_number_id,d.order_quantity,d.plan_cut_qnty,d.order_total from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c,wo_po_color_size_breakdown d where a.job_no=b.job_no_mst and a.job_no=c.job_no and a.job_no=d.job_no_mst and b.id=d.po_break_down_id  and c.entry_from=158 $company_con_name_a and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.status_active=$cbo_status and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0  $date_cond $buyer_id_cond $year_cond $job_no_cond $order_id_cond_trans $order_no_cond $order_status_cond $season_cond $approvalCon $file_con_b $ref_con_b $order_by";
				//echo $sql_budget; die; 
				...
				foreach ($result_sql_budget as $row) {
					...
					$po_item_qty_arr[$row[csf('po_id')]][$row[csf('item_number_id')]]['plancut'] += $row[csf('plan_cut_qnty')];
					$po_item_qty_arr[$row[csf('po_id')]][$row[csf('item_number_id')]]['order_quantity'] += $row[csf('order_quantity')];
					...
					$po_wise_arr[$row[csf('po_id')]]['po_id'] = $row[csf('po_id')];
					$po_wise_arr[$row[csf('po_id')]]['insert_date'] = $row[csf('insert_date')];
					$po_wise_arr[$row[csf('po_id')]]['po_quantity'] += $row[csf('order_quantity')];
					$po_wise_arr[$row[csf('po_id')]]['plan_cut_qnty'] += $row[csf('plan_cut_qnty')];
					$po_wise_arr[$row[csf('po_id')]]['insert_date'] = $row[csf('insert_date')];
					$po_wise_arr[$row[csf('po_id')]]['pre_insert_date'] = $row[csf('pre_insert_date')];
					$po_wise_arr[$row[csf('po_id')]]['update_date'] = $row[csf('update_date')];
					...
					$po_wise_arr[$row[csf('po_id')]]['po_total_price'] += $row[csf('order_total')];
					$po_wise_arr[$row[csf('po_id')]]['grouping'] = $row[csf('grouping')];
					$po_wise_arr[$row[csf('po_id')]]['file_no'] = $row[csf('file_no')];
					$po_wise_arr[$row[csf('po_id')]]['item_number_id'] = $row[csf('item_number_id')];
				}
	...
	?>
				...
				<div id="scroll_body" style="display: none;width: 1650px;"></div>
				<!-- <h3 align="left" id="accordion_h2" style="width:1650px" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel2', '')"> -Search Panel</h3> -->
				<!-- <fieldset style="width:1625px;" id="content_search_panel2"> -->
				<!-- <table width="1650">
					<tr class="form_caption">
						<td colspan="25" align="center"><strong>Cost Break Down Analysis Report<br>
                        <? if ($start_date != "") echo change_date_format($start_date) . ' To ' . change_date_format($end_date);
						else echo ""; ?>
                        </strong></td>
					</tr>
					<tr colspan="25" align="center" class="form_caption">
				...
				?>
				<h2 align="center"><strong>Cost Break Down Analysis Report<br>
						<? if ($start_date != "") echo change_date_format($start_date) . ' To ' . change_date_format($end_date);
						else echo ""; ?>
					</strong></h2>

				<table id="table_header_1" class="rpt_table" width="1525" cellpadding="0" cellspacing="0" border="1" rules="all" style="font-size: 18px;">
					<thead>
						<tr>
							<th width="30">SL</th>
							...
							<!-- <th width="80" rowspan="2">T. Leader<hr>Merchant</th> -->
							<th width="90">Style
								<hr>Order No
							</th>
							<th width="90">Item Name</th>
							<th width="70">Avg Unit Price</th>
							<th width="70">Order Date
								<hr>Shipment Date<? //echo $caption; 
													?>
							</th>
							<th width="70">Total Order Qty</th>


							<th width="90">Cons/Dzn
								<hr>Total Cons.
							</th>
							<th width="50">Fab
								<hr> Rate/Kgs
							</th>
							<th width="90">Fab Cost/Dz
								<hr>Total Fab. Cost
							</th>
							<th width="80">Trim/Dz
								<hr>Trim Cost
							</th>
							<th width="80">Print/Emb /Dz
								<hr>Total Print/Emb
							</th>

							<th width="80">WashOther/ Dz
								<hr>Total WashOther
							</th>

							<th width="80">Freight/Dz
								<hr>Total Freight
							</th>
							<th width="90">Comm/Dz
								<hr>Total Comm.
							...
							<th width="75">Total Price</th>
							<th width="75">Total Margin</th>
			...
			<!-- <div style="1625px;" id="scroll_body"> -->
					<!-- <table class="rpt_table" width="1650" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body" style="font-size: 15px;"> -->
					<?
					...
					$grand_tot_trims_cost = 0;
					$total_washing_cost = 0;
					$other_cost = 0;
					$total_trim_cost = 0;
					...
					$po_numbers = str_replace("'", "", $txt_order_no);
					$po_number = explode(",", $po_numbers);
					$p = 1;
					foreach ($po_number as $pid) {
						if ($p == 1) {
							$po_number_ids .= "'" . $pid . "'";
							$p++;
						} else {
					...
					if (str_replace("'", "", $txt_order_no) != '') {
						$condition->po_number("in ($po_number_ids)");
					}
					if (str_replace("'", "", $txt_file_no) != '') {
					...
					/*if(str_replace("'","",$txt_season)!='')
			 {
				...
				//$condition->season("in($txt_season)");
					...
					$po_conversion_cost_arr = $conversion->getAmountArray_by_order();
					//print_r($po_conversion_cost_arr);
					...
					$total_all_fab_cost = $total_margin = 0;
					foreach ($po_wise_arr as $po_id => $row) {
						...
						foreach ($gmts_item_id as $item_id) {
							...
							$po_item_plan_qty += $po_item_qty_arr[$po_id][$item_id]['plancut'];
							$po_item_qty += $po_item_qty_arr[$po_id][$item_id]['order_quantity'];
						}
						$fab_purchase_knit_qty = array_sum($fabric_qty_arr['knit']['grey'][$row[('po_id')]]);
						$fab_purchase_woven_qty = array_sum($fabric_qty_arr['woven']['grey'][$row[('po_id')]]);
						$fab_purchase_qty = $fab_purchase_knit_qty + $fab_purchase_woven_qty;
						$fab_cons_dzn_req = ($fab_purchase_qty / $po_item_plan_qty) * $dzn_qnty;
						...
						$print_amount = $emblishment_costing_arr_name[$row[('po_id')]][1];
						$embroidery_amount = $emblishment_costing_arr_name[$row[('po_id')]][2];
						$special_amount = $emblishment_costing_arr_name[$row[('po_id')]][4];
						$wash_cost = $emblishment_costing_arr_name_wash[$row[('po_id')]][3];
						$other_amount = $emblishment_costing_arr_name[$row[('po_id')]][5];
						$foreign = $commission_costing_arr[$row[('po_id')]][1];
						$local = $commission_costing_arr[$row[('po_id')]][2];
						$test_cost = $other_costing_arr[$row[('po_id')]]['lab_test'];
						$freight_cost = $other_costing_arr[$row[('po_id')]]['freight'];
						//$inspection=$other_costing_arr[$row[('po_id')]]['inspection'];
						//$certificate_cost=$other_costing_arr[$row[('po_id')]]['certificate_pre_cost'];
						//$common_oh=$other_costing_arr[$row[('po_id')]]['common_oh'];
						//$currier_cost=$other_costing_arr[$row[('po_id')]]['currier_pre_cost'];

						//$deffdlc_cost=$other_costing_arr[$row[('po_id')]]['deffdlc_cost'];
						//$design_cost=$other_costing_arr[$row[('po_id')]]['design_cost'];
						//$studio_cost=$other_costing_arr[$row[('po_id')]]['studio_cost'];
						$commercial_cost = $commercial_costing_arr[$row[('po_id')]];

						$cm_cost = $other_costing_arr[$row[('po_id')]]['cm_cost'];
						$yarn_costing = $yarn_costing_arr[$row[('po_id')]];
						$avg_rate = $yarn_costing / $yarn_req_qty_arr[$row[('po_id')]];
						$yarn_cost_percent = ($yarn_costing / $order_value) * 100;

						//po_conversion_cost_arr
						$conversion_cost = array_sum($po_conversion_cost_arr[$row[('po_id')]]);
						$trim_amount = $trims_costing_arr[$row[('po_id')]];

						$all_fab_cost = $yarn_costing + $fab_purchase + $conversion_cost;
						$all_fab_cost_dzn = ($all_fab_cost / $po_item_qty) * $dzn_qnty;
						$trim_amount_dzn = ($trim_amount / $po_item_qty) * $dzn_qnty;

						$print_emb_amount_dzn = (($print_amount + $embroidery_amount) / $po_item_qty) * $dzn_qnty;
						$wash_amount_dzn = (($wash_cost + $other_amount) / $po_item_qty) * $dzn_qnty;
						$test_cost_dzn = ($test_cost / $po_item_qty) * $dzn_qnty;
						$freight_cost_dzn = ($freight_cost / $po_item_qty) * $dzn_qnty;
						$commercial_cost_dzn = ($commercial_cost / $po_item_qty) * $dzn_qnty;
						$commission_cost_dzn = (($foreign + $local) / $po_item_qty) * $dzn_qnty;
						$cm_cost_dzn = ($cm_cost / $po_item_qty) * $dzn_qnty;
					...
					?>
						<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" style="font-size:18px">
							<td width="" height="50" align="right"><? echo $i; ?></td>

							<td width="" height="50" align="right">
								<div style="word-wrap:break-word; "><? echo $buyer_library[$row[('buyer_name')]]; ?></div>
							</td>
							<td width="" height="50" align="right">
								<p><? echo $row[('job_no_prefix_num')]; ?></p>
							</td>
							<td width="" height="50" align="right">
								<p><?php echo $row[('grouping')]; ?>
									<hr><? echo $row[('file_no')]; ?>
								</p>
							</td>
							<!--  <td width=""><p><? echo $team_leader_arr[$row[('team_leader')]]; ?><hr>
                     <? echo $dealing_merchant_array[$row[('dealing_marchant')]]; ?>
                     </p></td>  -->
							<td width="" height="50" align="right">
								<div style="word-wrap:break-word; "><? echo $row[('style_ref_no')]; ?>
									<hr>
									<a href="javascript:precost_bom_pop('<? echo $row[('po_id')]; ?>','<? echo $row[('job_no')]; ?>','<? echo $company_name; ?>','<? echo $row[('buyer_name')]; ?>','<? echo $row[('style_ref_no')]; ?>','<? echo $costing_date; ?>');"><? echo $row[('po_number')]; ?></a>
								</div>
							</td>
							<td width="" height="50" align="right">
							...
							<td align="right" width="" height="50" title="<? echo $row[('unit_price')]; ?>">
								<div style="word-wrap:break-word;"><? echo number_format($row['po_total_price'] / $row[('po_quantity')], 2); ?></div>
							</td>
							<td width="" align="right">
								<div style="word-wrap:break-word; "><? echo change_date_format($row[('po_received_date')]) . '<hr>' . change_date_format($row[('pub_shipment_date')]); ?></div>
							</td>

							<td width="" height="50" align="right">
								<p><? echo number_format($row[('po_quantity')], 2); ?></p>
							</td>
							<!--  <td width="70"><p><? echo $unit_of_measurement[$row[('order_uom')]]; ?></p></td> -->
							<td width="" align="right" height="50" title="Fab Req/Plan Qty(<? echo $po_item_qty; ?>)*<? echo $dzn_qnty; ?>" bgcolor="<? echo $td_color; ?>">
								<div style="word-wrap:break-word; "><? echo number_format($fab_cons_dzn_req, 2) . '<hr>' . number_format($fab_purchase_qty, 2); ?></div>
							</td>
							<!-- start_rating -->

							<td width="" height="50" align="right">
								<p> <? echo number_format($all_fab_cost / $fab_purchase_qty, 2); ?> </p>
							</td>
							<!-- end_rating -->
							<td width="" height="50" align="right" title="Yarn+Fab+Conversion(Fab Req/PO Qty*<? echo $dzn_qnty; ?>)">
								<p><? echo number_format($all_fab_cost_dzn, 2) . '<hr>' . number_format($all_fab_cost, 2); ?></p>
							</td>

							<td width="" height="50" align="right">
								<p><? echo number_format($trim_amount_dzn, 2) . '<hr>' . number_format($trim_amount, 2);; ?></p>
							</td>
							<td width="" height="50" align="right" title="Req Amt/PO Qty*<? echo $dzn_qnty; ?>">
								<p><? echo number_format($print_emb_amount_dzn, 2) . '<hr>' . number_format($print_amount + $embroidery_amount, 2);; ?></p>
							</td>
							<td width="" height="50" align="right" title="Req Amt/PO Qty*<? echo $dzn_qnty; ?>">
								<p><? echo number_format($wash_amount_dzn, 2) . '<hr>' . number_format($wash_cost + $other_amount, 2); ?></p>
							</td>
							<!--  <td width="80" align="right" title="Req Amt/PO Qty*<? echo $dzn_qnty; ?>"><p><? echo number_format($test_cost_dzn, 2) . '<hr>' . number_format($test_cost, 2); ?></p></td>  -->
							<td width="" height="50" align="right" title="Req Amt/PO Qty*<? echo $dzn_qnty; ?>">
								<p><? echo number_format($freight_cost_dzn, 2) . '<hr>' . number_format($freight_cost, 2); ?></p>
							</td>
							<td width="" height="50" align="right" title="Req Amt/PO Qty*<? echo $dzn_qnty; ?>">
								<p><? echo number_format($commercial_cost_dzn, 2) . '<hr>' . number_format($commercial_cost, 2); ?></p>
							</td>

							<td width="" height="50" align="right" title="Req Amt/PO Qty*<? echo $dzn_qnty; ?>">
								<p><? echo number_format($cm_cost_dzn, 2) . '<hr>' . number_format($cm_cost, 2); ?></p>
							</td>
							<?
							$total_cost = $all_fab_cost + $trim_amount + $test_cost + $print_amount + $embroidery_amount + $other_amount + $wash_cost + $commercial_cost + $foreign + $local + $freight_cost + $cm_cost;
							//$cm_value=$net_order_val-$others_cost_value;
							$tot_margin = $row['po_total_price'] - $total_cost;
							?>
							<td width="" height="50" align="right">
								<p><? echo number_format($total_cost, 2); ?></p>
							</td>
							<td width="" height="50" align="right">
								<p><? echo number_format($row['po_total_price'], 2); ?></p>
							</td>
							<td width="" height="50" align="right"><? echo number_format($tot_margin, 2); ?></td>

							<?
							//echo  $total_cost;
							$total_print_amount += $print_amount + $embroidery_amount;
							//$total_embroidery_amount+=$embroidery_amount;
							//	$total_special_amount+=$special_amount;
							$total_other_amount += $other_amount;
							$total_wash_cost += $wash_cost + $other_amount;
					...
					<?
						...
						$total_order_amount += $row[('po_total_price')];
						$total_all_fab_cost += $all_fab_cost;
						$total_fab_purchase_qty += $fab_purchase_qty;
					...
					<!-- </table> -->
					<!-- </div> -->
					<!-- <table class="tbl_bottom" width="1650" id="report_table_footer" cellpadding="0" cellspacing="0" border="1" rules="all" style="font-size: 15px;"> -->
					<tr>
						...
						<td height="50" width="">&nbsp;</td>
						<td height="50" width=""></td>
						<td height="50" width=""><strong>Total:</strong></td>
						<td height="50" width="" align="right" id="total_order_qnty"><? echo number_format($total_order_qty, 2); ?></td>
						<!--  <td width="70">&nbsp;</td> -->
						<td height="50" width="" align="right" id="total_fab_purchase_qty"><? echo number_format($total_fab_purchase_qty, 2); ?></td>
						<td height="50" width="" align="right" id=""></td>
						<td height="50" width="" id="total_all_fab_cost" align="right"><? echo number_format($total_all_fab_cost, 2); ?></td>
						<td height="50" width="" align="right" id="total_trim_cost"><? echo number_format($total_trim_cost, 2); ?></td>
						<td height="50" width="" align="right" id="total_print_amount"><? echo number_format($total_print_amount, 2); ?></td>
						<td height="50" width="" id="total_wash_cost" align="right"><? echo number_format($total_wash_cost, 2); ?></td>

						<!-- <td width="80" id="total_testing_cost"><? echo number_format($total_testing_cost, 2); ?></td> -->
						<td height="50" width="" align="right" id="total_freight_cost"><? echo number_format($total_freight_cost, 2); ?></td>
						<td height="50" width="" align="right" id="total_commercial_cost"><? echo number_format($total_commercial_cost, 2); ?></td>

						<td height="50" width="" align="right" id="total_cm_cost"><? echo number_format($total_cm_cost, 2); ?></td>
						<td height="50" width="" align="right" id="total_cm_cost"><? echo number_format($total_tot_cost, 2); ?></td>
						<td height="50" width="" align="right" id="total_order_amount"><? echo number_format($total_order_amount, 2); ?></td>
						<td height="50" width="" align="right" id="total_margin"><? echo number_format($total_margin, 2); ?></td>
					...
					<tr>
						...
						<td height="50" width="">&nbsp;</td>
						<td height="50" width=""></td>
						<td height="50" width=""><strong>%</strong></td>
						<td height="50" width="" align="right" id="total_order_qnty_per"><? //echo number_format($total_order_qty,2); 
																							?></td>
						<!--  <td width="70">&nbsp;</td> -->
						<td height="50" width="" align="right" id="total_fab_purchase_qty_per"><? //if($total_fab_purchase_qty>0) echo number_format($total_fab_purchase_qty/$total_tot_cost*100,2);else echo "0"; 
																								?></td>
						<td height="50" width="" align="right" id=""></td>
						<td height="50" width="" title="Fab Cost/Total Price*100" id="total_all_fab_cost_per" align="right"><? if ($total_all_fab_cost > 0) echo number_format($total_all_fab_cost / $total_order_amount * 100, 2) . '%';
																															else echo "0%"; //echo number_format($total_all_fab_cost,2); 
																															?></td>
						<td height="50" width="" title="Trims Cost/Total Price*100" align="right" id="total_trim_cost_per"><? if ($total_trim_cost > 0) echo number_format($total_trim_cost / $total_order_amount * 100, 2) . '%';
																															else echo "0%"; //echo number_format($total_trim_cost,2); 
																															?></td>
						<td height="50" width="" title="Print Cost/Total Price*100" align="right" id="total_print_amount_per"><? if ($total_print_amount > 0) echo number_format($total_print_amount / $total_order_amount * 100, 2) . '%';
																																else echo "0%"; //echo number_format($total_print_amount,2); 
																																?></td>
						<td height="50" width="" id="total_wash_cost_per" align="right"><? if ($total_wash_cost > 0) echo number_format($total_wash_cost / $total_order_amount * 100, 2) . '%';
																						else echo "0%"; //echo number_format($total_wash_cost,2); 
																						?></td>

						<!-- <td width="80" id="total_testing_cost"><? //echo number_format($total_testing_cost,2); 
																	?></td> -->
						<td height="50" width="" align="right" id="total_freight_cost_per"><? if ($total_freight_cost > 0) echo number_format($total_freight_cost / $total_order_amount * 100, 2) . '%';
																							else echo "0%"; //echo number_format($total_freight_cost,2); 
																							?></td>
						<td height="50" width="" align="right" id="total_commercial_cost_per"><? if ($total_commercial_cost > 0) echo number_format($total_commercial_cost / $total_order_amount * 100, 2) . '%';
																								else echo "0%"; // echo number_format($total_commercial_cost,2); 
																								?></td>

						<td height="50" width="" align="right" id="total_cm_cost_per"><? if ($total_cm_cost > 0) echo number_format($total_cm_cost / $total_order_amount * 100, 2) . '%';
																						else echo "0%"; //echo number_format($total_cm_cost,2); 
																						?></td>
						<td height="50" width="" align="right" id="total_cm_cost_per"><? if ($total_tot_cost > 0) echo number_format($total_tot_cost / $total_order_amount * 100, 2) . '%';
																						else echo "0%"; //echo number_format($total_tot_cost,2); 
																						?></td>
						<td height="50" width="" align="right" id="total_order_amount_per"><? if ($total_order_amount > 0) echo number_format($total_order_amount / $total_order_amount * 100, 2) . '%';
																							else echo "0%"; //echo number_format($total_order_amount,2); 
																							?></td>
						<td height="50" width="" align="right" id="total_margin_per"><? if ($total_margin > 0) echo number_format($total_margin / $total_order_amount * 100, 2) . '%';
																						else echo "0%"; //echo number_format($total_margin,2); 
																						?></td>
					...
					<div style="width:600px; display:none" id="yarn_summary">
						...
						<table class="rpt_table" width="590" cellpadding="0" cellspacing="0" border="1" rules="all" style="font-size: 18px;">
							<thead>
								<tr>
							...
							?>
											<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr3_<? echo $s; ?>','<? echo $bgcolor; ?>')" id="tr3_<? echo $s; ?>">
												<td><? echo $s; ?></td>
												<td>
													<div style="word-wrap:break-word; width:140px"><? echo $composition[$Composition] . " " . $percent . "%"; ?></div>
												</td>
												<td>
													<div style="word-wrap:break-word; width:60px"><? echo $yarn_count_library[$count]; ?></div>
												</td>
												<td>
													<div style="word-wrap:break-word; width:80px"><? echo $yarn_type[$type]; ?></div>
												</td>
			...
			if ($template == 1) {
				...
				$style1 = "#E9F3FF";
				$style = "#FFFFFF";
				$approval_arr = array(1 => "Approved", 2 => "Unapproved");
				...
				unset($sql_cpm);
				//============================================SC No===============================================
				$sales_contract_sql = sql_select("select a.company_name,a.job_no_prefix_num, b.insert_date, c.insert_date as pre_insert_date, c.costing_date, b.update_date, a.job_no, a.buyer_name, a.style_ref_no, a.order_uom, b.is_confirmed, a.agent_name, a.avg_unit_price, a.dealing_marchant,a.team_leader, a.gmts_item_id, a.total_set_qnty as ratio, b.plan_cut, b.id as po_id, b.po_number, b.pub_shipment_date, b.po_received_date, b.po_quantity, b.unit_price, b.po_total_price, b.grouping, b.file_no,d.item_number_id,d.color_number_id,d.order_quantity,d.plan_cut_qnty, c.approved,d.order_total,e.contract_no,f.wo_po_break_down_id from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c,wo_po_color_size_breakdown d, com_sales_contract e,com_sales_contract_order_info f where a.job_no=b.job_no_mst and a.job_no=c.job_no and a.job_no=d.job_no_mst and b.id=d.po_break_down_id  and c.entry_from=158 and  e.id=f.com_sales_contract_id and b.id=f.wo_po_break_down_id $company_con_name_a and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.status_active=$cbo_status and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0  and e.status_active=1 and f.status_active=1 $date_cond $buyer_id_cond $year_cond $job_no_cond $order_id_cond_trans $order_no_cond $order_status_cond $season_cond $approvalCon $file_con_b $ref_con_b $order_by");
				$budget_data_arr = array();
				foreach ($sales_contract_sql as $row) {
					$po_wise_arr[$row[csf('po_id')]]['contract_no'] .= $row[csf('contract_no')] . ",";
				}
				unset($sales_contract_sql);
				//============================================SC NO===============================================

				$asking_profit = array();

				$sql_budget = "select a.company_name,a.job_no_prefix_num, b.insert_date, c.insert_date as pre_insert_date, c.costing_date, b.update_date, a.job_no, a.buyer_name, a.style_ref_no, a.order_uom, b.is_confirmed, a.agent_name, a.avg_unit_price, a.dealing_marchant,a.team_leader, a.gmts_item_id, a.total_set_qnty as ratio, b.plan_cut, b.id as po_id, b.po_number, b.pub_shipment_date, b.po_received_date, b.po_quantity, b.unit_price, b.po_total_price, b.grouping, b.file_no,d.item_number_id,d.color_number_id,d.order_quantity,d.plan_cut_qnty, c.approved,d.order_total, a.quotation_id from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c,wo_po_color_size_breakdown d where a.job_no=b.job_no_mst and a.job_no=c.job_no and a.job_no=d.job_no_mst and b.id=d.po_break_down_id  and c.entry_from=158 $company_con_name_a and a.status_active=1 and a.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and b.status_active=$cbo_status and b.is_deleted=0 and d.status_active=1 and d.is_deleted=0  $date_cond $buyer_id_cond $year_cond $job_no_cond $order_id_cond_trans $order_no_cond $order_status_cond $season_cond $approvalCon $file_con_b $ref_con_b $order_by";
				...
				foreach ($result_sql_budget as $row) {
					...
					$po_wise_arr[$row[csf('po_id')]]['insert_date'] = $row[csf('insert_date')];
					$po_wise_arr[$row[csf('po_id')]]['po_quantity'] += $row[csf('order_quantity')];
					//$po_wise_arr[$row[csf('po_id')]]['order_total']+=$row[csf('order_total')];
					$po_wise_arr[$row[csf('po_id')]]['plan_cut_qnty'] += $row[csf('plan_cut_qnty')];
					$po_wise_arr[$row[csf('po_id')]]['insert_date'] = $row[csf('insert_date')];
					$po_wise_arr[$row[csf('po_id')]]['pre_insert_date'] = $row[csf('pre_insert_date')];
					$po_wise_arr[$row[csf('po_id')]]['update_date'] = $row[csf('update_date')];
					...
					$po_wise_arr[$row[csf('po_id')]]['file_no'] = $row[csf('file_no')];
					$po_wise_arr[$row[csf('po_id')]]['item_number_id'] = $row[csf('item_number_id')];
					$po_wise_arr[$row[csf('po_id')]]['approved'] = $row[csf('approved')];
					$po_wise_arr[$row[csf('po_id')]]['quotation_id'] = $row[csf('quotation_id')];
				}
				...
				?>
					...
					?>
					<h2 align="center">
						<strong>Cost Break Down Analysis Report<br>
							<? if ($start_date != "") echo change_date_format($start_date) . ' To ' . change_date_format($end_date);
							else echo ""; ?>
						</strong>
					</h2>

					<table id="table_header_1" class="rpt_table" width="2935" cellpadding="0" cellspacing="0" border="1" rules="all" style="font-size: 18px;">
						<thead>
							<!-- <tr>
						<th align="center" colspan="11">Order Status</th>
						<th align="center" colspan="10">Fabric Cost</th>
						<th align="center" >Trim Cost</th>
						<th align="center" colspan="3">Emblishment Cost</th>
						<th align="center" >Gmts.Wash</th>
						<th align="center" >Commercial Cost</th>
						<th align="center" >Lab Test Cost</th>
						<th align="center" >Freight Cost</th>
						<th align="center" >Others Cost</th>
						<th align="center" >Merchandising Cost</th>
						<th align="center" >Merchandising Cost%</th>
						<th align="center" >CM Cost</th>
						<th align="center" >Total Cost</th>
						<th align="center" >Total Price</th>
						<th align="center" >Total Margin</th>
							...
							<tr>
								...
								<th width="90">Item Name</th>
								<th width="100"> Sales Contact No</th>
								...
								<th width="80">Fabric Req Qty</th>
								<th width="90">Purchase Fabric-$</th>
								<th width="80">Yarn-$</th>
								<th width="80">Knitting-$</th>
								<th width="80">Dyeing-$</th>
								<th width="80">Yarn Dyeing-$</th>
								<th width="80">AOP-$</th>


								<th width="80">Wash/Other-$</th>
								<th width="80">Total Fabric Cost(Po Wise)</th>
								<th width="80">Trim-$</th>
								<th width="80">Print-$</th>
								<th width="80">Embroidary-$</th>
								<th width="80">Other-$</th>
								<th width="80">Gmts.Wash-$</th>
								<th width="90">Commercial-$</th>
								<th width="80">Lab Test-$</th>
								<th width="80">Freight-$</th>
								<th width="80">Comm,Inspec,<br>Curier,Certi Cost-$</th>
								<th width="80">Merchandising Cost-$</th>
								<th width="80">Merchandising Cost-%</th>
								<th width="90">CM-$</th>
					...
					<div style="width:2935px; overflow-y:scroll; max-height:350px" id="scroll_body">
						<table class="rpt_table" width="2935" cellpadding="0" cellspacing="0" border="1" rules="all" style="font-size: 18px;" id="table_body">
							<?

							$i = 1;
							$tot_order_qnty = $tot_yarn_cost = $tot_order_value = $tot_purchase_cost = $grand_tot_trims_cost = $tot_washing_cost = $other_cost = $tot_trim_cost = $tot_commercial_cost = $tot_print_amount = $tot_embroidery_amount = $tot_other_amount = $tot_others_amount = $other_amount = $tot_wash_cost = $tot_yarn_costing = $tot_foreign_amount = $tot_local_amount = $tot_test_cost_amount = $tot_freight_amount = $tot_knit_cost = $tot_gmt_wash_cost = $tot_fabric_dyeing_cost = $tot_yarn_dyeing_cost = $tot_all_over_print_cost = $tot_other_cost = $tot_merchandise_cost = $tot_margin = $total_cm_amount = $tot_fabric = $tot_fabric_cost = $grand_total_cost = $tot_order_amount = 0;
							...
							$gmts_wash_cost_arr = array(64, 123, 132, 138, 140, 142, 148, 175, 188, 201, 209, 221, 231, 232, 239, 276, 193);
							$trims = new trims($condition);
							$trims_costing_arr = $trims->getAmountArray_by_order();
							$emblishment = new emblishment($condition);
							$emblishment_costing_arr_name = $emblishment->getAmountArray_by_orderAndEmbname();
							...
							foreach ($po_wise_arr as $po_id => $row) {
								...
								foreach ($aop_cost_arr as $aop_process_id) {
									$all_over_cost += array_sum($conversion_costing_arr_process[$po_id][$aop_process_id]);
								}
								$all_over_print_cost += $all_over_cost;

								$gmts_wash_cost = 0;
								foreach ($gmts_wash_cost_arr as $gmts_wash_process_id) {
									$gmts_wash_cost += array_sum($conversion_costing_arr_process[$po_id][$gmts_wash_process_id]);
								}
								$total_gmts_wash_cost += $gmts_wash_cost;

								$knit_cost = 0;
								foreach ($knit_cost_arr as $process_id) {
									$knit_cost += array_sum($conversion_costing_arr_process[$po_id][$process_id]);
								}
								$total_knit_cost += $knit_cost;

								$fabric_dyeing_cost = 0;
								foreach ($fabric_dyeingCost_arr as $fab_process_id) {
									$fabric_dyeing_cost += array_sum($conversion_costing_arr_process[$po_id][$fab_process_id]);
								}
								$total_fabric_dyeing_cost += $fabric_dyeing_cost;

								$yarn_dyeing_cost = array_sum($conversion_costing_arr_process[$po_id][30]);
								$aop_cost = array_sum($conversion_costing_arr_process[$po_id][35]);
								...
								$special_amount = $emblishment_costing_arr_name[$row[('po_id')]][4];
								$wash_cost = $emblishment_costing_arr_name_wash[$row[('po_id')]][3];
								$gmts_dyeing_amount = $emblishment_costing_arr_name[$row[('po_id')]][5];
								$attachment_amount = $emblishment_costing_arr_name[$row[('po_id')]][6];
								$other_amount = $emblishment_costing_arr_name[$row[('po_id')]][99];
								$foreign = $commission_costing_arr[$row[('po_id')]][1];
								$local = $commission_costing_arr[$row[('po_id')]][2];
								$test_cost = $other_costing_arr[$row[('po_id')]]['lab_test'];
								$freight_cost = $other_costing_arr[$row[('po_id')]]['freight'];
								...
								$certificate_cost = $other_costing_arr[$row[('po_id')]]['certificate_pre_cost'];
								$currier_cost = $other_costing_arr[$row[('po_id')]]['currier_pre_cost'];
								$common_oh_cost = $other_costing_arr[$row[('po_id')]]['common_oh'];
								$other_cost = $foreign + $local + $inspection + $certificate_cost + $currier_cost;
								$others_amount = $gmts_dyeing_amount + $attachment_amount + $other_amount + $special_amount;
								$commercial_cost = $commercial_costing_arr[$row[('po_id')]];
								$cm_cost = $other_costing_arr[$row[('po_id')]]['cm_cost'];
								$yarn_costing = $yarn_costing_arr[$row[('po_id')]];
								$avg_rate = $yarn_costing / $yarn_req_qty_arr[$row[('po_id')]];
						...
						if($company_name!=0) $cbo_company_cond="and template_name=$cbo_company_name";else $cbo_company_cond="";

						$budget_print_report_format=return_field_value("format_id"," lib_report_template"," module_id=2 and report_id in (43) and is_deleted=0 and status_active=1 $cbo_company_cond");

						$wvnreportArr= explode(',',$budget_print_report_format);
						$wvnreport=$wvnreportArr[0];

						if($wvnreport==221){$reporAction="fabric_cost_detail";}
						else if($wvnreport==730){$reporAction="budgetsheet";}
						else if($wvnreport==50){$reporAction="preCostRpt";}
						else if($wvnreport==63){$reporAction="bomRpt2";}
						else if($wvnreport==156){$reporAction="accessories_details";}
						else if($wvnreport==157){$reporAction="accessories_details2";}
						else if($wvnreport==158){$reporAction="preCostRptWoven";}
						else if($wvnreport==159){$reporAction="bomRptWoven";}
						else if($wvnreport==170){$reporAction="preCostRpt3";}
						else if($wvnreport==171){$reporAction="preCostRpt4";}
						else if($wvnreport==173){$reporAction="preCostRpt5";}
						else if($wvnreport==238){$reporAction="summary";}
						else if($wvnreport==215){$reporAction="budget3_details";}
						else if($wvnreport==769){$reporAction="preCostRpt7";}
						else if($wvnreport==498){$reporAction="preCostRpt10";}
						else if($wvnreport==800){$reporAction="preCostRpt11";}
						//else if($wvnreport==129){$reporAction="budget5";}
						else if($wvnreport==268){$reporAction="budget_4";}
						else $reporAction="preCostRpt2";

							?>
								<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" style="font-size:18px">
									<td width="30" height="50" align="right"><? echo $i; ?></td>

									<td width="70" height="50" align="right">
										<div style="word-wrap:break-word; "><? echo $buyer_library[$row[('buyer_name')]]; ?></div>
									</td>
									...
									<td width="90" height="50" align="center">
										<div style="word-wrap:break-word; "><? echo $row[('style_ref_no')]; ?></div>
									</td>
									<td width="90" height="50" align="center">
										<div style="word-wrap:break-word; ">
											<a href="##" 
												onclick="<?	
														...
														{ 
															$reporAction="budget5";
															$function="generate_worder_report('".$reporAction."','".$row[('job_no')]."',".$company_name.",".$row[('buyer_name')].",'".$row[('style_ref_no')]."','".$row[('quotation_id')]."','".$row[('po_id')]."','".$row[('po_number')]."');"; 
															

														}else{
															$function="javascript:precost_bom_pop('".$reporAction."','".$row[('po_id')]."','". $row[('job_no')]."','".$company_name."','".$row[('buyer_name')]."','".$row[('style_ref_no')]."','".$costing_date."');";
														} echo $function;?>"><? echo $row[('po_number')]; ?></a>
										</div>
									</td>
									<td width="90" height="50" align="center">
										<div style="word-wrap:break-word;"><?
									...
									<td width="100" height="50" align="center">
										<div style="word-wrap:break-word;"><?
																			$sc_lc_no = rtrim($row[('contract_no')], ',');
									...
									<td align="right" width="70" height="50" title="<? echo $row[('unit_price')]; ?>">
										<div style="word-wrap:break-word;"><? echo number_format($row[('po_total_price')] / $row[('po_quantity')], 2); ?></div>
									</td>
									<td width="70" height="50" align="right">
										<p><? echo number_format($row[('po_total_price')], 2); ?></p>
									...
									<td width="70" height="50" align="center">
										<div style="word-wrap:break-word; "><? echo change_date_format($row[('pub_shipment_date')]); ?></div>
									</td>
									<td width="70" height="50" align="center">
										<div style="word-wrap:break-word; "><? echo $approval_arr[$row[('approved')]]; ?></div>
									</td>
									<td width="90" height="50" align="right" title="Fab Req/Plan Qty(<? echo $po_item_qty; ?>)*<? echo $dzn_qnty; ?>" bgcolor="<? echo $td_color; ?>">
										<div style="word-wrap:break-word; "><? echo number_format($fab_cons_dzn_req, 2);
																			$tot_cons_dzn += $fab_cons_dzn_req; ?></div>
									</td>

									<td width="80" align="right">
										<p> <? echo number_format($fab_purchase_qty, 2);
											$tot_all_fab_cost += $fab_purchase_qty; ?> </p>
									</td>
									<td width="90" height="50" align="right">
										<p> <? echo number_format($fab_purchase, 2); ?></p>
									</td>
									<td width="80" height="50" align="right">
										<p> <? echo number_format($yarn_costing, 2); ?></p>
									</td>
									<td width="80" height="50" align="right">
										<p> <? echo number_format($knit_cost, 2); ?> </p>
									</td>
									<td width="80" height="50" align="right">
										<p> <? echo number_format($fabric_dyeing_cost, 2); ?></p>
									</td>
									<td width="80" height="50" align="right">
										<p> <? echo number_format($yarn_dyeing_cost, 2); ?></p>
									</td>
									<td width="80" height="50" align="right">
										<p> <? echo number_format($all_over_cost, 2); ?></p>
									</td>

									<td width="80" height="50" align="right">
										<p><? echo number_format($gmts_wash_cost + $other_amount, 2); ?></p>
									</td>
									<?
									$total_fabric = $fab_purchase + $yarn_costing + $knit_cost + $fabric_dyeing_cost + $yarn_dyeing_cost + $all_over_cost + $gmts_wash_cost + $other_amount;
									?>
									<td width="80" height="50" align="right">
										<p><? echo number_format($total_fabric, 2); ?></p>
									</td>
									<td width="80" height="50" align="right">
										<p><? echo number_format($trim_amount, 2);; ?></p>
									</td>
									<td width="80" height="50" align="right">
										<p><? echo number_format($print_amount, 2);; ?></p>
									</td>
									<td width="80" height="50" align="right">
										<p><? echo number_format($embroidery_amount, 2); ?></p>
									</td>
									<td width="80" height="50" align="right">
										<p><? echo number_format($others_amount, 2); ?></p>
									</td>
									<td width="80" height="50" align="right">
										<p><? echo number_format($wash_cost, 2); ?></p>
									</td>
									<td width="90" height="50" align="right">
										<p><? echo number_format($commercial_cost, 2); ?></p>
									</td>
									<td width="80" align="right">
										<p><? echo number_format($test_cost, 2); ?></p>
									</td>
									<td width="80" height="50" align="right">
										<p><? echo number_format($freight_cost, 2); ?></p>
									</td>
									<td width="80" height="50" align="right">
										<p><? echo number_format($other_cost, 2); ?></p>
									</td>
									<?
									$merchandise_cost = $fab_purchase + $yarn_costing + $knit_cost + $fabric_dyeing_cost + $yarn_dyeing_cost + $all_over_cost + $wash_cost + $other_amount + $trim_amount + $print_amount + $embroidery_amount + $commercial_cost + $test_cost + $freight_cost + $other_cost + $common_oh_cost;
									?>
									<td width="80" height="50" align="right">
										<p><? echo number_format($merchandise_cost, 2); ?></p>
									</td>
									<td width="80" height="50" align="right">
										<p><? echo number_format($merchandise_cost / $row['po_total_price'] * 100, 2); ?></p>
									</td>

									<td width="90" height="50" align="right">
										<p><? echo number_format($cm_cost, 2); ?></p>
									</td>

									<?
									$total_cost = $merchandise_cost + $cm_cost;
									$tot_margin = $row['po_total_price'] - $total_cost;
									?>
									<td width="75" height="50" align="right">
										<p><? echo number_format($total_cost, 2); ?></p>
									</td>
									<td width="75" height="50" align="right">
										<p><? echo number_format($row['po_total_price'], 2); ?></p>
									</td>
									<td width="75" height="50" align="right"><? echo number_format($tot_margin, 2); ?></td>
							...
							<?
								$tot_order_qty_pcs += $order_qty_pcs;
								$tot_order_qnty += $row[('po_quantity')];
								$total_order_amount += $row[('po_total_price')];
								$tot_order_amount += $row[('po_total_price')];
								$grand_total_cost += $total_cost;
								$tot_order_price += $row[('po_total_price')];
								$total_all_fab_cost += $all_fab_cost;
								$total_fab_purchase_qty += $fab_purchase_qty;
								$total_yarn_cost += $yarn_costing;
								$total_purchase_cost += $fab_purchase;
								...
								$total_embroidery_amount += $embroidery_amount;
								$total_others_amount += $others_amount;
								$total_other_amount += $other_amount;
								$total_wash_cost += $wash_cost + $other_amount;
								$tot_yarn_costing += $yarn_costing;
								$total_foreign_amount += $foreign;
								$total_local_amount += $local;
								$total_test_cost_amount += $test_cost;
								$total_freight_amount += $freight_cost;
								$tot_knit_cost += $knit_cost;
								$tot_gmt_wash_cost += $gmts_wash_cost + $other_amount;
								$tot_fabric_dyeing_cost += $fabric_dyeing_cost;
								$tot_yarn_dyeing_cost += $yarn_dyeing_cost;
								$tot_all_over_print_cost += $all_over_cost;
								$tot_other_cost += $other_cost;
								$tot_fab_purchase += $fab_purchase;
							...
							$total_yarn_cost_percentage = $total_yarn_cost / $total_order_amount * 100;
							?>
							<table class="rpt_table" width="2935" cellpadding="0" cellspacing="0" border="1" rules="all" style="font-size: 18px;">
								<tfoot>
									<tr>
										...
										<td height="50" width="90">&nbsp;</td>
										<td height="50" width="90">&nbsp;</td>
										<td height="50" width="100">Total:</td>
										<td height="50" width="70" align="right" id="tot_order_qnty"><? echo number_format($tot_order_qnty, 2); ?></td>
										<td height="50" width="70"></td>
										<td height="50" width="70" align="right" id="tot_order_price"><? echo number_format($tot_order_price, 2); ?></td>
										<td height="50" width="70"><strong></strong></td>
										<td height="50" width="70"><strong></strong></td>
										<td height="50" width="90" align="right" id="tot_cons_dzn"><? echo number_format($tot_cons_dzn, 2); ?></td>
										<td height="50" width="80" align="right" id="tot_all_fab_cost"><? echo number_format($tot_all_fab_cost, 2); ?></td>
										<td height="50" width="90" id="tot_fab_purchase" align="right"><? echo number_format($tot_fab_purchase, 2); ?></td>
										<td height="50" width="80" id="tot_yarn_costing" align="right"><? echo number_format($tot_yarn_costing, 2); ?></td>
										<td height="50" width="80" id="tot_knit_cost" align="right"><? echo number_format($tot_knit_cost, 2); ?></td>
										<td height="50" width="80" id="tot_fabric_dyeing_cost" align="right"><? echo number_format($tot_fabric_dyeing_cost, 2); ?></td>
										<td height="50" width="80" id="tot_yarn_dyeing_cost" align="right"><? echo number_format($tot_yarn_dyeing_cost, 2); ?></td>
										<td height="50" width="80" id="tot_all_over_print_cost" align="right"><? echo number_format($tot_all_over_print_cost, 2); ?></td>
										<td height="50" width="80" align="right" id="tot_gmt_wash_cost"><? echo number_format($tot_gmt_wash_cost, 2); ?></td>
										<?
										$tot_fabric_cost = $tot_fab_purchase + $tot_yarn_costing + $tot_knit_cost + $tot_fabric_dyeing_cost + $tot_yarn_dyeing_cost + $tot_all_over_print_cost + $tot_gmt_wash_cost;
										?>
										<td height="50" width="80" align="right" id="tot_fabric_cost"><? echo number_format($tot_fabric_cost, 2); ?></td>
										<td height="50" width="80" align="right" id="total_trim_cost"><? echo number_format($total_trim_cost, 2); ?></td>
										<td height="50" width="80" align="right" id="total_print_amount"><? echo number_format($total_print_amount, 2); ?></td>
										<td height="50" width="80" align="right" id="total_embroidery_amount"><? echo number_format($total_embroidery_amount, 2); ?></td>
										<td height="50" width="80" id="total_others_amount" align="right"><? echo number_format($total_others_amount, 2); ?></td>
										<td height="50" width="80" id="total_wash_cost" align="right"><? echo number_format($total_wash_cost, 2); ?></td>
										<td height="50" width="90" align="right" id="total_commercial_cost"><? echo number_format($total_commercial_cost, 2); ?></td>
										<td width="80" align="right" id="total_testing_cost"><? echo number_format($total_testing_cost, 2); ?></td>
										<td height="50" width="80" align="right" id="total_freight_cost"><? echo number_format($total_freight_cost, 2); ?></td>
										<td height="50" width="80" align="right" id="tot_other_cost"><? echo number_format($tot_other_cost, 2); ?></td>
										<?
										$tot_merchandise_cost = $tot_fab_purchase + $tot_yarn_costing + $tot_knit_cost + $tot_fabric_dyeing_cost + $tot_yarn_dyeing_cost + $tot_all_over_print_cost + $tot_gmt_wash_cost + $total_trim_cost + $total_print_amount + $total_embroidery_amount + $total_others_amount + $total_wash_cost + $total_commercial_cost + $total_testing_cost + $total_freight_cost + $tot_other_cost;
										?>
										<td height="50" width="80" align="right" id="tot_merchandise_cost"><? echo number_format($tot_merchandise_cost, 2); ?></td>
										<td height="50" width="80" align="right"><? echo number_format($tot_merchandise_cost / $total_order_amount * 100, 2); ?></td>
										<td height="50" width="90" align="right" ><? echo number_format($tot_cm_cost, 2); ?></td>
										<td height="50" width="75" align="right" id="grand_total_cost"><? echo number_format($grand_total_cost, 2); ?></td>
										<td height="50" width="75" align="right" id="tot_order_amount"><? echo number_format($tot_order_amount, 2); ?></td>
										<td height="50" width="75" align="right" id="total_margin"><? echo number_format($total_margin, 2); ?></td>
									...
									<tr>
										...
										<td height="50" width="70" align="right" id="total_order_qnty_per"></td>
										<td height="50" width="70"></td>
										<td height="50" width="70"></td>
										<td height="50" width="70"></td>
										<td height="50" width="70"><strong></strong></td>
										<td height="50" width="90" align="right" id="total_fab_purchase_qty_per"></td>
										<td height="50" width="80" align="right" id=""></td>
										<td height="50" width="90" id="total_all_fab_cost_per" align="right"><? if ($tot_fab_purchase > 0) echo number_format($tot_fab_purchase / $total_order_amount * 100, 2) . '%';
																												else echo "0%"; //echo number_format($total_all_fab_cost,2); 
																												?></td>
										<td height="50" width="80" align="right" id="total_trim_cost_per"><? if ($tot_yarn_costing > 0) echo number_format($tot_yarn_costing / $total_order_amount * 100, 2) . '%';
																											else echo "0%"; //echo number_format($total_trim_cost,2); 
																											?></td>
										<td height="50" width="80" align="right" id="total_trim_cost_per"><? if ($tot_knit_cost > 0) echo number_format($tot_knit_cost / $total_order_amount * 100, 2) . '%';
																											else echo "0%"; //echo number_format($total_trim_cost,2); 
																											?></td>
										<td height="50" width="80" align="right" id="total_trim_cost_per"><? if ($tot_fabric_dyeing_cost > 0) echo number_format($tot_fabric_dyeing_cost / $total_order_amount * 100, 2) . '%';
																											else echo "0%"; //echo number_format($total_trim_cost,2); 
																											?></td>
										<td height="50" width="80" align="right" id="total_trim_cost_per"><? if ($tot_yarn_dyeing_cost > 0) echo number_format($tot_yarn_dyeing_cost / $total_order_amount * 100, 2) . '%';
																											else echo "0%"; //echo number_format($total_trim_cost,2); 
																											?></td>
										<td height="50" width="80" align="right" id="total_trim_cost_per"><? if ($tot_all_over_print_cost > 0) echo number_format($tot_all_over_print_cost / $total_order_amount * 100, 2) . '%';
																											else echo "0%"; //echo number_format($total_trim_cost,2); 
																											?></td>
										<td height="50" width="80" id="total_wash_cost_per" align="right"><? if ($tot_gmt_wash_cost > 0) echo number_format($tot_gmt_wash_cost / $total_order_amount * 100, 2) . '%';
																											else echo "0%"; //echo number_format($total_wash_cost,2); 
																											?></td>
										<td height="50" width="80" align="right" id="total_print_amount_per"><? if ($tot_fabric_cost > 0) echo number_format($tot_fabric_cost / $total_order_amount * 100, 2) . '%';
																												else echo "0%"; //echo number_format($total_print_amount,2); 
																												?></td>
										<td height="50" width="80" align="right" id="total_print_amount_per"><? if ($total_trim_cost > 0) echo number_format($total_trim_cost / $total_order_amount * 100, 2) . '%';
																												else echo "0%"; //echo number_format($total_print_amount,2); 
																												?></td>
										<td height="50" width="80" align="right" id="total_print_amount_per"><? if ($total_print_amount > 0) echo number_format($total_print_amount / $total_order_amount * 100, 2) . '%';
																												else echo "0%"; //echo number_format($total_print_amount,2); 
																												?></td>
										<td height="50" width="80" align="right" id="total_print_amount_per"><? if ($total_embroidery_amount > 0) echo number_format($total_embroidery_amount / $total_order_amount * 100, 2) . '%';
																												else echo "0%"; //echo number_format($total_print_amount,2); 
																												?></td>
										<td height="50" width="80" align="right" id="total_trim_cost_per"><? if ($total_wash_cost > 0) echo number_format($total_others_amount / $total_order_amount * 100, 2) . '%';
																											else echo "0%"; //echo number_format($total_trim_cost,2); 
																											?></td>
										<td height="50" width="80" align="right" id="total_trim_cost_per"><? if ($total_wash_cost > 0) echo number_format($total_wash_cost / $total_order_amount * 100, 2) . '%';
																											else echo "0%"; //echo number_format($total_trim_cost,2); 
																											?></td>
										<td height="50" width="90" align="right" id="total_commercial_cost_per"><? if ($total_commercial_cost > 0) echo number_format($total_commercial_cost / $total_order_amount * 100, 2) . '%';
																												else echo "0%"; // echo number_format($total_commercial_cost,2); 
																												?></td>
										<td height="50" width="80" align="right" id="total_commercial_cost_per"><? if ($total_testing_cost > 0) echo number_format($total_testing_cost / $total_order_amount * 100, 2) . '%';
																												else echo "0%"; // echo number_format($total_commercial_cost,2); 
																												?></td>
										<td height="50" width="80" align="right" id="total_freight_cost_per"><? if ($total_freight_cost > 0) echo number_format($total_freight_cost / $total_order_amount * 100, 2) . '%';
																												else echo "0%"; //echo number_format($total_freight_cost,2); 
																												?></td>
										<td height="50" width="80" align="right" id="total_cm_cost_per"><? if ($tot_other_cost > 0) echo number_format($tot_other_cost / $total_order_amount * 100, 2) . '%';
																										else echo "0%"; //echo number_format($total_tot_cost,2); 
																										?></td>
										<td height="50" width="80" align="right" id="total_cm_cost_per"><? if ($tot_merchandise_cost > 0) echo number_format($tot_merchandise_cost / $total_order_amount * 100, 2) . '%';
																										else echo "0%"; //echo number_format($total_tot_cost,2); 
																										?></td>
										<td height="50" width="80" align="right" id="total_cm_cost_per"><? // if($tot_merchandise_cost>0) echo number_format($tot_merchandise_cost/$total_order_amount*100,2).'%';else echo "0%";//echo number_format($total_tot_cost,2); 
																										?></td>
										<td height="50" width="90" align="right" id="total_cm_cost_per"><? if ($tot_cm_cost > 0) echo number_format($tot_cm_cost / $total_order_amount * 100, 2) . '%';
																										else echo "0%"; //echo number_format($total_cm_cost,2); 
																										?></td>
										<td height="50" width="75" align="right" id="total_cm_cost_per"><? if ($total_tot_cost > 0) echo number_format(($tot_merchandise_cost + $total_cm_cost) / $total_order_amount * 100, 2) . '%';
																										else echo "0%"; //echo number_format($total_tot_cost,2); 
																										?></td>
										<td height="50" width="75" align="right" id="total_order_amount_per"><? if ($total_order_amount > 0) echo number_format($total_order_amount / $total_order_amount * 100, 2) . '%';
																												else echo "0%"; //echo number_format($total_order_amount,2); 
																												?></td>
										<td height="50" width="75" align="right" id="total_margin_per"><? echo number_format(($total_order_amount - ($tot_merchandise_cost + $total_cm_cost)) / $total_order_amount * 100, 2) . '%'; ?></td>
		...
		//show 2 button end
			...
			if ($template == 1) {
				...
				$asking_profit = sql_select("select id, company_id, applying_period_date, applying_period_to_date, asking_profit, max_profit from lib_standard_cm_entry where status_active=1 and is_deleted=0 $company_con_id");
				...
				//var_dump($cpm_arr);die;

				$sql_budget = "select a.id as job_id,a.company_name,a.job_no_prefix_num,a.team_leader, b.insert_date, c.insert_date as pre_insert_date, c.costing_date, b.update_date, a.job_no, a.buyer_name, a.style_ref_no, a.order_uom,a.set_smv, b.is_confirmed, a.agent_name, a.avg_unit_price, a.dealing_marchant, a.gmts_item_id, a.total_set_qnty as ratio, b.plan_cut, b.id as po_id, b.po_number, b.pub_shipment_date, b.po_received_date, b.po_quantity, b.unit_price, b.po_total_price, b.grouping, b.file_no,b.excess_cut from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no and c.entry_from=158 $company_con_name_a and a.status_active=1 and a.is_deleted=0 and b.status_active=$cbo_status and b.is_deleted=0 $date_cond $buyer_id_cond $year_cond $job_no_cond $order_id_cond_trans $order_no_cond $order_status_cond $season_cond $approvalCon $file_con_b $ref_con_b $order_by";
				//echo $sql_budget; die;excess_cut
				...
				foreach ($result_sql_budget as $row) {
					...
					$fab_purchase_knit = array_sum($fabric_costing_arr['knit']['grey'][$poId]);
					$fab_purchase_woven = array_sum($fabric_costing_arr['woven']['grey'][$poId]);
					...
					$jobArr[$row[csf('job_id')]] = $row[csf('job_id')];
				...
				$job_ids = count($jobArr);
				if ($db_type == 2 && $job_ids > 1000) {
					$job_cond_for_in = " and (";
					$jobIdsArr = array_chunk($jobArr, 999);
					foreach ($jobIdsArr as $ids) {
						$job_cond_for_in .= " b.id in(" . implode(",", $ids) . ") or";
					}
					$job_cond_for_in = chop($job_cond_for_in, 'or ');
					$job_cond_for_in .= ")";
				} else {
					$job_cond_for_in = " and b.id in(" . implode(",", $jobArr) . ")";
				}


				$smv_eff_arr = array();
				$costing_library = array();
				...
				?>
					...
					<div style="width:<? echo $width; ?>px;">
									...
									<td width="350" align="left">
										<table width="350" border="1" rules="all" class="rpt_table" cellpadding="0" cellspacing="2">
											...
											<tr bgcolor="<? echo $style1; ?>">
												<td>19</td>
												<td>Net Profit</td>
												<td align="right"><? echo number_format($net_profit = ($total_order_amount - $total_tot_cost), 2); ?></td>
												<td align="right"><? echo number_format($net_profit / $total_order_amount * 100, 2); ?></td>
											</tr>
						...
						<h3 align="left" id="accordion_h2" style="width:<? echo $width + 18; ?>px" class="accordion_h" onClick="accordion_menu( this.id,'content_search_panel2', '')"> -Search Panel</h3>
						<fieldset style="width:100%;" id="content_search_panel2">
							<table width="<? echo $width; ?>">
								<tr class="form_caption">
									<td colspan="68" align="center">
										<h4>Cost Break Down Report</h4>
										<strong>Details Report </strong><br>
										<span style="height:15px; width:15px; background-color:#FF0000; float:left;  margin-left:10px; margin-right:10px;"></span>
										<span style="float:left;">1 Day delay from order entry date </span>
									</td>
							...
							<div style="width:<? echo $width + 18; ?>px; max-height:400px; overflow-y:scroll" id="scroll_body">
								<table class="rpt_table" width="<? echo $width; ?>" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
									...
									$i = 1;
									$total_order_qty = 0;
									$total_yarn_dyeing_cost = 0;
									$total_yarn_cost = 0;
									$total_order_amount = 0;
									$total_purchase_cost = 0;
									$grand_tot_trims_cost = 0;
									$total_fabric_dyeing_cost = 0;
									$total_knitting_cost = 0;
									$total_heat_setting_cost = 0;
									$total_finishing_cost = 0;
									$total_washing_cost = 0;
									$fabric_dyeing_cost_dzn = 0;
									$other_cost = 0;
									$all_over_print_cost = 0;
									$total_trim_cost = 0;
									$total_commercial_cost = 0;
									...
									$total_embroidery_amount = 0;
									$total_special_amount = 0;
									$total_plan_cut_qnty_pcs = 0;
									...
									?>
										<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" style="font-size:11px">
											...
											<td width="110">
												<p><? echo $team_member_arr[$row[csf('team_leader')]]; ?></p>
											</td>
											<td width="100">
												<p><? echo $company_library[$row[csf('company_name')]]; ?></p>
											</td>
											<td width="70">
											...
											<td width="110" align="right">
												<p><? echo $smv_eff_arr[$row[csf('job_no')]]['eff']; ?></p>
											</td>
											<td width="100">
												<p><? echo $row[csf('po_number')]; ?></p>
											</td>
											<td width="100">
												<p><? echo  $order_status[$row[csf('is_confirmed')]]; ?></p>
											</td>
											<td width="110" align="center">
												<p><? echo $costing_library[$row[csf('job_no')]]; ?></p>
											</td>
											<td width="70">
												<p><? echo  date("d-M-Y", strtotime($ship_po_recv_date)); ?></p>
											</td>
											<td width="90" align="right">
												<p><? echo number_format($order_qty, 2); ?></p>
											</td>
											<td width="60" align="center">
												<p><? echo $unit_of_measurement[$order_uom]; ?></p>
											</td>
											<td width="50" align="right">
												<p><? echo number_format($order_qty * $row[csf('set_smv')], 2); ?></p>
											</td>
											<td width="90" align="right">
												<p><? echo number_format($order_qty_pcs, 2); ?></p>
											</td>
											<td width="80" align="right">
												<p><? echo $row[csf('excess_cut')]; ?></p>
											</td>
											<td width="110" align="right">
												<p><? echo $row[csf('plan_cut')]; ?></p>
											</td>
											<td width="70" align="right">
												<p><? echo number_format($row[csf('unit_price')], 2); ?></p>
											</td>
											<td width="100" align="right">
												<p><? echo number_format($order_value, 2); ?></p>
											</td>
											<?
											...
											if ($fabric_dyeing_cost <= 0 && $yarn_dyeing_cost <= 0) $color_fab = "red";
											else $color_fab = "";
											if ($yarn_costing <= 0) $color_yarn = "red";
											else $color_yarn = "";
											if ($knit_cost <= 0) $color_knit = "red";
											else $color_knit = "";
											if ($fabric_finish <= 0) $color_finish = "red";
											else $color_finish = "";
											if ($commercial_cost <= 0) $color_com = "red";
											else $color_com = "";
											...
											if ($trim_amount <= 0) $color_trim = "red";
											else $color_trim = "";
											if ($cm_cost <= 0) $color = "red";
											else $color = "";
											$smv = 0;
											$eff = 0;
											$cpm = 0;
											...
											?>
											<td width="60" align="right">
												<p><? echo number_format($yarn_req_qty_arr[$row[csf('po_id')]], 2); ?></p>
											</td>
											<td width="60" align="right">
												<p><? echo number_format($knite_qty, 2); ?></p>
											</td>
											<td width="60" align="right">
												<p><? echo number_format($fabric_dyeing_qty, 2); ?></p>
											</td>
											<td width="60" align="right">
												<p><? echo number_format($washing_qty, 2); ?></p>
											</td>
											<td width="100" align="right">
												<p>
													<?
													$fab_cost = $tot_fabric_cost;
													echo number_format($fab_cost, 2);
													$tot_fab_cost += $fab_cost;
													?>
												</p>
											</td>
											<td width="60" align="right"><? echo number_format(($fab_cost / $order_value) * 100, 2); ?></td>
											<td width="100" align="right" bgcolor="<? echo $color_trim; ?>"><? echo number_format($trim_amount, 2); ?></td>
											<td width="60" align="right"><? echo number_format(($trim_amount / $order_value) * 100, 2); ?></td>
											<td width="80" align="right"><? echo number_format($print_amount, 2); ?></td>
											<td width="60" align="right"><? echo number_format(($print_amount / $order_value) * 100, 2); ?></td>
											<td width="85" align="right"><? echo number_format($embroidery_amount, 2); ?></td>
											<td width="60" align="right"><? echo number_format(($embroidery_amount / $order_value) * 100, 2); ?></td>
											<td width="80" align="right"><? echo number_format($special_amount, 2); ?></td>
											<td width="60" align="right"><? echo number_format(($special_amount / $order_value) * 100, 2); ?></td>
											<td width="80" align="right"><? echo number_format($wash_cost, 2); ?></td>
											<td width="60" align="right"><? echo number_format(($wash_cost / $order_value) * 100, 2); ?></td>
											<td width="80" align="right"><? echo number_format($other_amount, 2); ?></td>
											<td width="100" align="right">
												<?
												$tot_mat_svc_cost = $fab_cost + $trim_amount + $print_amount + $embroidery_amount + $special_amount + $wash_cost + $other_amount;
												echo number_format($tot_mat_svc_cost, 2);
												?>
											</td>
											<td width="60" align="right"><? echo number_format(($tot_mat_svc_cost / $order_value) * 100, 2); ?></td>
											<td width="120" align="right" bgcolor="<? echo $color_com; ?>"><? echo number_format($commercial_cost, 2); ?></td>
											<td width="60" align="right"><? echo number_format(($commercial_cost / $order_value) * 100, 2); ?></td>
											...
											<td width="60" align="right"><? echo number_format($dfc_cost, 2);
																			$tot_dfc_cost += $dfc_cost; ?></td>
											<td width="60" align="right"><? echo number_format($interest_cost, 2);
																			$tot_interest_cost += $interest_cost; ?></td>
											<td width="60" align="right"><? echo number_format($incometax_cost, 2);
																			$tot_incometax_cost += $incometax_cost; ?></td>
											<!--<td width="110" align="right"><? //echo number_format($total_material_cost,2); 
																				?></td>-->
											<td width="120" align="right"><? echo number_format($foreign, 2) ?></td>
											<td width="120" align="right"><? echo number_format($local, 2) ?></td>
											<td width="110" align="right"><? $tot_forg_local = $foreign + $local;
																			echo number_format($tot_forg_local, 2);  ?></td>
											<td width="110" align="right"><? $tot_netFOB = $order_value - $tot_forg_local;
																			echo number_format($tot_netFOB, 2); ?></td>
											<!--<td width="100" align="right"><? //$tot_cmValue=$tot_netFOB-$total_material_cost; echo number_format($tot_cmValue,2);
																				?></td>-->
											<td width="100" align="right" bgcolor="<? echo $color; ?>"><? echo number_format($cm_cost, 2); ?></td>
											<td width="60" align="right"><? echo number_format(($cm_cost / $order_value) * 100, 2); ?></td>
											<td width="100" align="right">
												<?

												$total_cost = $tot_mat_svc_cost + $commercial_cost + $test_cost + $freight_cost + $inspection + $certificate_cost + $finance_chrg + $currier_cost + $dfc_cost + $interest_cost + $incometax_cost + $foreign + $local + $cm_cost;

												echo number_format($total_cost, 2);
												?>
											</td>
											<td width="60" align="right"><? echo number_format(($total_cost / $order_value) * 100, 2); ?></td>

											<td width="110" align="right"><?
																			//$tot_margin=$tot_cmValue-$cm_cost;  
																			$tot_margin = $order_value - $total_cost;
																			echo  number_format($tot_margin, 2); ?></td>
											<td width="110" align="right">
												<p><? echo number_format($tot_margin / $order_qty_pcs, 2); ?></p>
											</td>

											<td width="60" align="right"><?php echo number_format(($tot_margin / $order_value) * 100, 2); ?></td>


											<td width="110" align="center">
												<p><? echo $lib_user_arr[$preCostApprove_arr[$row[csf('job_no')]]['apprv_by']]; ?></p>
											</td>
											<td width="110" align="center">
												<p><? echo $preCostApprove_arr[$row[csf('job_no')]]['apprv_date']; ?></p>
											</td>
							...
							<table class="tbl_bottom" width="<? echo $width; ?>" id="report_table_footer" cellpadding="0" cellspacing="0" border="1" rules="all">
								<tr>
									...
									<td width="90" align="right"><? echo number_format($total_order_qty_pcs, 2); ?></td>
									<td width="80">&nbsp;</td>
									<td width="110"><? echo number_format($total_plan_cut_qnty_pcs, 2); ?></td>
									<td width="70">&nbsp;</td>
									<td width="100" align="right"><? echo number_format($total_order_amount, 2); ?></td>
									...
									<td width="60" align="right"><? echo number_format(($tot_fab_cost / $total_order_amount) * 100, 2); ?></td>
									<td width="100"><? echo number_format($total_trim_cost, 2); ?></td>
									<td width="60" align="right"><? echo number_format(($total_trim_cost / $total_order_amount) * 100, 2); ?></td>
									<td width="80" align="right" id="total_print_amount"><? echo number_format($total_print_amount, 2); ?></td>
									<td width="60" align="right"><? echo number_format(($total_print_amount / $total_order_amount) * 100, 2); ?></td>
									<td width="85" align="right"><? echo number_format($total_embroidery_amount, 2); ?></td>
									<td width="60" align="right"><? echo number_format(($total_embroidery_amount / $total_order_amount) * 100, 2); ?></td>
									<td width="80" align="right"><? echo number_format($total_special_amount, 2); ?></td>
									<td width="60" align="right"><? echo number_format(($total_special_amount / $total_order_amount) * 100, 2); ?></td>
									<td width="80" align="right"><? echo number_format($total_wash_cost, 2); ?></td>
									<td width="60" align="right"><? echo number_format(($total_wash_cost / $total_order_amount) * 100, 2); ?></td>
									<td width="80" align="right"><? echo number_format($total_other_amount, 2); ?></td>
									<td width="100" align="right"><? echo number_format($total_mat_svc_cost, 2); ?></td>
									<td width="60" align="right"><? echo number_format(($total_mat_svc_cost / $total_order_amount) * 100, 2); ?></td>
									<td width="120" align="right"><? echo number_format($total_commercial_cost, 2); ?></td>
									<td width="60" align="right"><? echo number_format(($total_commercial_cost / $total_order_amount) * 100, 2); ?></td>
									<td width="100" align="right"><? echo number_format($total_test_cost_amount, 2); ?></td>
									<td width="100" align="right"><? echo number_format($total_freight_amount, 2); ?></td>
									<td width="120" align="right"><? echo number_format($total_inspection_amount, 2); ?></td>
									<td width="100" align="right"><? echo number_format($total_certificate_amount, 2); ?></td>
									<td width="100" align="right"><? echo number_format($total_finance_chrg_amount, 2); ?></td>

									<td width="100" align="right"><? echo number_format($total_currier_amount, 2); ?></td>
									...
									<!--<td width="110" align="right"><? //echo number_format($total_meterial_amount,2); 
																		?></td>-->

									<td width="120" align="right"><? echo number_format($total_foreign_amount, 2); ?></td>


									<td width="120" align="right"><? echo number_format($total_local_amount, 2); ?></td>
									...
									<!--<td width="100" align="right"><? //echo number_format($total_cm_new_value,2); 
																		?></td>-->

									<td width="100" align="right"><? echo number_format($total_cm_amount, 2); ?></td>
									<td width="60" align="right"><? echo number_format(($total_cm_amount / $total_order_amount) * 100, 2); ?></td>
									<td width="100" align="right"><? echo number_format($grand_total_cost, 2); ?></td>
									<td width="60" align="right"><? echo number_format(($grand_total_cost / $total_order_amount) * 100, 2); ?></td>
									<td width="110" align="right"><? echo number_format($total_margin_new_amount, 2); ?></td>
									<td width="110"></td>
									<td width="60"><? echo number_format($total_margin_new_amount / $total_order_amount * 100, 2); ?></td>
									<td width="110">&nbsp;</td>
									<td width="110">&nbsp;</td>
									<td>&nbsp;</td>
			...
			$fab_precost_arr = array();
			$commission_array = array();
			$knit_arr = array();
			$fabriccostArray = array();
			$fab_emb = array();
			$fabric_data_Array = array();
			$asking_profit_arr = array();
			$yarncostArray = array();
			...
			$trim_qty_arr = array();
			$trim_po_country_arr = array();
			$trim_poqty_country_arr = array();
			...
			*/
								...
								?>
									<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
										...
										<td width="100">
											<p><a href="#" onClick="precost_bom_pop('<? echo $row[csf('po_id')]; ?>','<? echo $row[csf('job_no')]; ?>','<? echo $company_name; ?>','<? echo $row[csf('buyer_name')]; ?>');"><? echo $row[csf('po_number')]; ?></a></p>
										</td>
										<td width="100">
											<p><? echo  $order_status[$row[csf('is_confirmed')]]; ?></p>
										</td>
										...
										<td width="110">
											<p><? echo $team_member_arr[$row[csf('dealing_marchant')]]; ?></p>
										</td>
										<td width="70">
											<p><? echo $ship_po_recv_date; ?></p>
										</td>
										<td width="90" align="right">
											<p><? echo number_format($row[csf('po_quantity')], 2); ?></p>
										</td>
										<!--<td width="90" align="right"><p><? //echo number_format($row[csf('avg_unit_price')],2); 
																			?></p></td>
					 <td width="100" align="right"><p><? //echo number_format($order_value,2); 
														?></p></td>-->
										<?
										...
										if ($kniting_cost <= 0) $color_knit = "red";
										else $color_knit = "";
										if ($fabric_finish <= 0) $color_finish = "red";
										else $color_finish = "";
										if ($commercial_cost <= 0) $color_com = "red";
...
if ($action == "report_generate_excel") {
	...
	if ($report_type == 1) {
		if ($template == 1) {
			...
			$cpm_arr = array();
			$cpm_date_arr = array(); //$yarn_desc_array=array();
			$finance_arr = array();
				...
				if($costing_per_id==1) $dzn_qnty=12;
								...
								?>
									<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" style="font-size:11px">
										<td width="40"><? echo $i; ?></td>
										<td width="110">
											<div style="word-wrap:break-word; width:110px"><? echo $team_member_arr[$row[csf('team_leader')]]; ?></div>
										</td>
										...
										<td width="110">
											<div style="word-wrap:break-word; width:110px"><? echo $row[csf('style_ref_no')]; ?></div>
										</td>
										<td width="70">
											<p><? echo $row[csf('job_no_prefix_num')]; ?></p>
										</td>
										<td width="110">
											<div style="word-wrap:break-word; width:110px"><? $gmts_item = '';
																							$gmts_item_id = explode(",", $row[csf('gmts_item_id')]);
																							foreach ($gmts_item_id as $item_id) {
																								if ($gmts_item == "") $gmts_item = $garments_item[$item_id];
																								else $gmts_item .= ", " . $garments_item[$item_id];
										...
										<td width="110" align="right">
											<div style="word-wrap:break-word; width:110px"><? echo $smv_eff_arr[$row[csf('job_no')]]['eff']; ?></div>
										</td>
										<td width="100">
											<div style="word-wrap:break-word; width:100px"><? echo $row[csf('po_number')]; ?></div>
										</td>
										<td width="100">
											<p><? echo  $order_status[$row[csf('is_confirmed')]]; ?></p>
										</td>
										<td width="110" align="center">
											<div style="word-wrap:break-word; width:110px"><? echo $costing_library[$row[csf('job_no')]]; ?></div>
										</td>
										<td width="70">
											<div style="word-wrap:break-word; width:70px"><? echo  date("d-M-Y", strtotime($ship_po_recv_date)); ?></div>
										</td>
										...
										<td width="110" align="right">
											<div style="word-wrap:break-word; width:110px"><? echo $row[csf('excess_cut')]; ?></div>
										</td>
										<td width="110" align="right">
											<div style="word-wrap:break-word; width:110px"><? echo $row[csf('plan_cut')]; ?></div>
										</td>
										<td width="70" align="right">
											<p><? echo number_format($row[csf('unit_price')], 2); ?></p>
										</td>
										...
										<td width="60" align="right">
											<p><? echo number_format($yarn_required[$row[csf('po_id')]], 2); ?></p>
										</td>
										<td width="60" align="right">
											<p><? echo number_format($knite_qty, 2); ?></p>
										</td>
										...
										<td width="100" align="right">
											<p><? /*$fab_cost=$fabric_cost_pcs_arr[$row[csf('po_id')]]*$plan_cut_qnty;*/
												$fab_cost = $tot_fabric_cost;
												echo number_format($fab_cost, 2);
												$tot_fab_cost += $fab_cost; ?></p>
										</td>
										...
										<td width="110" align="right"><? $tot_forg_local = $foreign + $local;
																		echo number_format($tot_forg_local, 2);  ?></td>
										<td width="110" align="right"><? $tot_netFOB = $order_value - $tot_forg_local;
																		echo number_format($tot_netFOB, 2); //.'_'.$row[csf('avg_unit_price')] 
																		?></td>



										<td width="100" align="right"><? $tot_cmValue = $tot_netFOB - $total_material_cost;
																		echo number_format($tot_cmValue, 2);       //echo number_format($order_value-$total_material_cost,2);
																		?></td>
										<td width="100" align="right" bgcolor="<? echo $color; ?>"><? echo number_format($cm_cost, 2); ?></td>
										<td width="110" align="right"><? $tot_margin = $tot_cmValue - $cm_cost;
																		echo  number_format($tot_cmValue - $cm_cost, 2); ?></td>
										<td width="110" align="right">
											<div style="word-wrap:break-word; width:110px"><? echo number_format($tot_margin / $order_qty_pcs, 2); ?></div>
										</td>

										<td width="60"><?php echo number_format(($tot_margin / $order_value) * 100, 2); ?></td>

										<td width="110" align="right">
											<div style="word-wrap:break-word; width:110px"><? $cpmVal = $smv_eff_arr[$row[csf('job_no')]]['cpm'] / $smv_eff_arr[$row[csf('job_no')]]['exc_rate'];
																							echo  number_format($cpmVal * 100 / $smv_eff_arr[$row[csf('job_no')]]['eff'], 3); ?></div>
										</td>

										<td width="110" align="right">
											<div style="word-wrap:break-word; width:110px"><? echo number_format(($tot_cmValue / $order_qty_pcs) / ($row[csf('set_smv')] / $row[csf('total_set_qnty')]), 3); ?></div>
										</td>

										<td width="110" align="center">
											<div style="word-wrap:break-word; width:110px"><? echo $lib_user_arr[$preCostApprove_arr[$row[csf('job_no')]]['apprv_by']]; ?></div>
										</td>
										<td width="110" align="center">
											<div style="word-wrap:break-word;"><? echo $preCostApprove_arr[$row[csf('job_no')]]['apprv_date']; ?></div>
										</td>
										<td><? echo $app_status_arr[$row[csf('approved')] . $row[csf('ready_to_approved')] . $row[csf('partial_approved')]]; ?></td>
										<?
										if ($total_profit_percentage2 <= 0) $color_pl = "red";
