...
{ 
	...
	$sql="select a.job_no_prefix_num, a.id as job_id, a.job_no, $year_field, a.company_name,a.season_buyer_wise as season,a.brand_id,a.season_year, a.buyer_name, a.style_ref_no, a.order_uom, a.gmts_item_id, a.total_set_qnty as ratio, a.set_smv, b.id, b.po_number,b.grouping,b.file_no, b.pub_shipment_date, b.po_quantity, b.plan_cut, b.unit_price, b.po_total_price, b.shiping_status from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no and c.job_no=b.job_no_mst and c.garments_nature=3 and a.company_name='$company_name' and a.job_no_prefix_num like '$job_no' and a.status_active=1 and a.is_deleted=0 and b.status_active=$cbo_order_status and b.is_deleted=0 $date_cond $buyer_id_cond $po_id_cond $ref_cond $file_cond $shipping_status_cond $year_cond $cbo_status_cond $brand_cond $season_cond $season_year_cond order by b.pub_shipment_date, a.job_no_prefix_num, b.id";
	//echo $sql; //die;
	$result=sql_select($sql);
	$all_po_id=array();  $job_id_arr=array();
	foreach($result as $row)
	{
	...
	$con = connect();
	execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ref_from in (1,2) and ENTRY_FORM=11");

	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 11, 1, $job_id_arr, $empty_arr);
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 11, 2, $all_po_id, $empty_arr);
	
	
		
	$ex_factory_arr=return_library_array( "SELECT b.po_break_down_id, sum(CASE WHEN b.entry_form!=85 THEN b.ex_factory_qnty ELSE 0 END)-sum(CASE WHEN b.entry_form=85 THEN b.ex_factory_qnty ELSE 0 END) as qnty from pro_ex_factory_mst b join gbl_temp_engine c on b.po_break_down_id=c.ref_val where b.status_active=1 and b.is_deleted=0 and c.ref_from=2 and c.entry_form=11 and c.user_id=$user_id group by b.po_break_down_id", "po_break_down_id", "qnty");
    ...
    <fieldset>
    	...
    	<div style="width:3862px; max-height:400px; overflow-y:scroll" id="scroll_body">
            ...
            <?
				$fabriccostArray=array(); $yarncostArray=array(); $trimsCostArray=array(); $prodcostArray=array(); $actualCostArray=array(); $actualTrimsCostArray=array(); 
				$subconCostArray=array(); $embellCostArray=array(); $washCostArray=array(); $aopCostArray=array(); $yarnTrimsCostArray=array(); 
				$yarncostDataArray=sql_select("SELECT a.job_no, sum(a.amount) as amnt, sum(a.rate*a.avg_cons_qnty) as amount from wo_pre_cost_fab_yarn_cost_dtls a join gbl_temp_engine b on a.job_id=b.ref_val where a.status_active=1 and a.is_deleted=0 and b.ref_from=1 and b.entry_form=11 and b.user_id=$user_id group by a.job_no");
				foreach($yarncostDataArray as $yarnRow)
				{
				   $yarncostArray[$yarnRow[csf('job_no')]]=$yarnRow[csf('amount')];
				}
				
				$fabriccostDataArray=sql_select("SELECT a.job_no, a.costing_per_id, a.embel_cost, a.wash_cost, a.cm_cost, a.commission, a.currier_pre_cost, a.lab_test, a.inspection, a.freight, a.comm_cost from wo_pre_cost_dtls a join gbl_temp_engine b on a.job_id=b.ref_val where a.status_active=1 and a.is_deleted=0 and b.ref_from=1 and b.entry_form=11 and b.user_id=$user_id");
				foreach($fabriccostDataArray as $fabRow)
				{
					 $fabriccostArray[$fabRow[csf('job_no')]]['costing_per_id']=$fabRow[csf('costing_per_id')];
					 $fabriccostArray[$fabRow[csf('job_no')]]['embel_cost']=$fabRow[csf('embel_cost')];
				...
				$trimscostDataArray=sql_select("SELECT b.po_break_down_id, sum(b.cons*a.rate) as total from wo_pre_cost_trim_cost_dtls a join wo_pre_cost_trim_co_cons_dtls b on a.id=b.wo_pre_cost_trim_cost_dtls_id join gbl_temp_engine c on a.job_id=c.ref_val where a.status_active=1 and a.is_deleted=0 and c.ref_from=1 and c.entry_form=11 and c.user_id=$user_id group by b.po_break_down_id");
				foreach($trimscostDataArray as $trimsRow)
				{
					 $trimsCostArray[$trimsRow[csf('po_break_down_id')]]=$trimsRow[csf('total')];
				}
				 
				$prodcostDataArray=sql_select("select a.job_no, 
									  sum(CASE WHEN a.cons_process=1 THEN a.amount END) AS knit_charge,
									  sum(CASE WHEN a.cons_process=30 THEN a.amount END) AS yarn_dye_charge,
									  sum(CASE WHEN a.cons_process=35 THEN a.amount END) AS aop_charge,
									  sum(CASE WHEN a.cons_process not in(1,2,30,35) THEN a.amount END) AS dye_finish_charge
									  from wo_pre_cost_fab_conv_cost_dtls a join gbl_temp_engine b on a.job_id=b.ref_val where a.status_active=1 and a.is_deleted=0 and b.ref_from=1 and b.entry_form=11 and b.user_id=$user_id group by job_no");
				foreach($prodcostDataArray as $prodRow)
				{
					$prodcostArray[$prodRow[csf('job_no')]]['knit_charge']=$prodRow[csf('knit_charge')];
					$prodcostArray[$prodRow[csf('job_no')]]['yarn_dye_charge']=$prodRow[csf('yarn_dye_charge')];
				...
				$actualCostDataArray=sql_select("SELECT a.cost_head,a.po_id,sum(a.amount_usd) as amount_usd from wo_actual_cost_entry a join gbl_temp_engine b on a.po_id=b.ref_val where a.status_active=1 and a.is_deleted=0 and b.ref_from=2 and b.entry_form=11 and b.user_id=$user_id group by a.cost_head,a.po_id");
				foreach($actualCostDataArray as $actualRow)
				{
				   $actualCostArray[$actualRow[csf('cost_head')]][$actualRow[csf('po_id')]]=$actualRow[csf('amount_usd')];
				}
				...
				$subconOutBillDataArray=sql_select("select b.order_id, 
									  sum(CASE WHEN a.process_id=2 THEN b.amount END) AS knit_bill,
									  sum(CASE WHEN a.process_id=4 THEN b.amount END) AS dye_finish_bill
									  from subcon_outbound_bill_mst a, subcon_outbound_bill_dtls b, gbl_temp_engine c where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.process_id in(2,4) and b.order_id=c.ref_val and c.ref_from=2 and c.entry_form=11 and c.user_id=$user_id  group by b.order_id");
				foreach($subconOutBillDataArray as $subRow)
				{
					$subconCostArray[$subRow[csf('order_id')]]['knit_bill']+=$subRow[csf('knit_bill')];
					$subconCostArray[$subRow[csf('order_id')]]['dye_finish_bill']+=$subRow[csf('dye_finish_bill')];
				...
				$embell_type_arr=return_library_array( "SELECT a.id, a.emb_name from wo_pre_cost_embe_cost_dtls a join gbl_temp_engine b on a.job_id=b.ref_val where b.ref_from=2 and b.entry_form=11 and b.user_id=$user_id", "id", "emb_name");	
				
				$bookingDataArray=sql_select("SELECT a.booking_type, a.item_category, a.currency_id, a.exchange_rate, b.po_break_down_id, b.process, b.amount, b.pre_cost_fabric_cost_dtls_id from wo_booking_mst a, wo_booking_dtls b, gbl_temp_engine c where a.booking_no=b.booking_no and a.item_category in(4,12,25) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.po_break_down_id=c.ref_val and c.ref_from=2 and c.entry_form=11 and c.user_id=$user_id");
				foreach($bookingDataArray as $woRow)
				{
					$amount=0; $trimsAmnt=0;
					if($woRow[csf('currency_id')]==1) { $amount=$woRow[csf('amount')]/$exchange_rate; } else { $amount=$woRow[csf('amount')]; }
				...
				$yarnTrimsDataArray=sql_select("SELECT b.po_breakdown_id, b.prod_id, a.item_category, 
					...
					sum(CASE WHEN a.transaction_type=6 and b.entry_form ='11' and b.trans_type=6 THEN b.quantity ELSE 0 END) AS trans_out_qty_yarn,
					sum(CASE WHEN a.transaction_type=2 and b.entry_form ='25' and b.trans_type=2 THEN a.cons_rate*b.quantity ELSE 0 END) AS trims_issue_amnt
					from inv_transaction a, order_wise_pro_details b, gbl_temp_engine c where a.id=b.trans_id and a.item_category in(1,4) and a.transaction_type in(2,4,5,6) and a.status_active=1 and a.is_deleted=0 and b.is_deleted=0 and b.status_active=1 and b.po_breakdown_id=c.ref_val and c.ref_from=2 and c.entry_form=11 and c.user_id=$user_id  group by b.po_breakdown_id, b.prod_id, a.item_category");
					$prod_id_arr=array();
				foreach($yarnTrimsDataArray as $row){
					$prod_id_arr[$row[csf('prod_id')]]=$row[csf('prod_id')];
				}
				fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 11, 3, $prod_id_arr, $empty_arr);
				
				$avg_rate_array=return_library_array( "SELECT a.id, a.avg_rate_per_unit from product_details_master a join gbl_temp_engine b on a.id=b.ref_val where a.item_category_id=1 and b.ref_from=3 and b.entry_form=11 and b.user_id=$user_id", "id", "avg_rate_per_unit"  );
				$receive_array=array();
				$sql_receive="SELECT a.currency_id,a.receive_purpose,b.prod_id, (b.order_qnty) as qty, (b.order_amount) as amnt,b.cons_quantity,b.cons_amount from inv_receive_master a, inv_transaction b, gbl_temp_engine c where  a.id=b.mst_id and b.transaction_type=1 and b.item_category=1 and b.status_active=1 and b.is_deleted=0 and b.prod_id=c.ref_val and c.ref_from=3 and c.entry_form=11 and c.user_id=$user_id";
				$resultReceive = sql_select($sql_receive);
				foreach($resultReceive as $invRow)
				{
					if($invRow[csf('currency_id')]==1)//Taka
				...
				$sqlPi=sql_select("SELECT c.po_break_down_id, sum(b.quantity) as quantity from  com_pi_master_details a, com_pi_item_details b,wo_booking_dtls c, gbl_temp_engine d where a.id=b.pi_id and b.work_order_no=c.booking_no  and a.item_category_id=2 and a.status_active=1 and c.status_active=1 and a.is_deleted=0 and b.status_active=1 and  b.is_deleted=0 and c.po_break_down_id=d.ref_val and d.ref_from=2 and d.entry_form=11 and d.user_id=$user_id  group by c.po_break_down_id");
				...
				$sql_fin_purchase="SELECT  c.currency_id,b.po_breakdown_id, sum(a.cons_rate*b.quantity) as finish_purchase_amnt,sum(a.order_amount) as order_amount from inv_receive_master c,inv_transaction a, order_wise_pro_details b, gbl_temp_engine d where a.id=b.trans_id and a.receive_basis in(1,2) and a.item_category=2 and a.transaction_type=1 and b.entry_form=37 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.po_breakdown_id=d.ref_val and d.ref_from=2 and d.entry_form=11 and d.user_id=$user_id group by b.po_breakdown_id, c.currency_id";
				$dataArrayFinPurchase=sql_select($sql_fin_purchase);
				foreach($dataArrayFinPurchase as $finRow)
				{
					if($pi_number_check[$finRow[csf('po_breakdown_id')]]==$finRow[csf('po_breakdown_id')])
				 ...
				 $sql_fin_purchase_wv="SELECT a.id, a.booking_no,a.booking_date,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.booking_type,b.job_no,b.po_break_down_id as po_id,sum(b.grey_fab_qnty) as grey_fab_qnty,sum(b.amount) as amount from wo_booking_mst a, wo_booking_dtls b,wo_pre_cost_fabric_cost_dtls c , gbl_temp_engine d where a.booking_no=b.booking_no and c.id=b.pre_cost_fabric_cost_dtls_id and a.booking_type=1 and b.fin_fab_qnty>0 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1  and c.is_deleted=0 and c.status_active=1 and d.ref_val=b.po_break_down_id and d.ref_from=2 and d.entry_form=11 and d.user_id=$user_id group by a.id, a.booking_no,a.booking_date,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.booking_type,b.job_no,b.po_break_down_id";
				...
				foreach($result as $row)
				{
					...
					$mkt_all_cost=$yarn_cost_mkt+$knit_cost_mkt+$dye_finish_cost_mkt+$yarn_dye_cost_mkt+$aop_cost_mkt+$trims_cost_mkt+$embell_cost_mkt+$wash_cost_mkt+$comm_cost_mkt+$freight_cost_mkt+$test_cost_mkt+$inspection_cost_mkt+$currier_cost_mkt+$fabric_purchase_cost_mkt;
					...
					//$mkt_margin=$po_value-$mkt_all_cost; 
					if($mkt_margin>0 && $CM_Value_Contribution_value_mkt>0) 	$mkt_margin_perc=($mkt_margin/$CM_Value_Contribution_value_mkt)*100;else $mkt_margin_perc="";
					...
					//$actual_margin=$ex_factory_value-$actual_all_cost;
					if($actual_margin>0 && $Net_PO_ex_Fact_Value_actual>0)  $actual_margin_perc=($actual_margin/$Net_PO_ex_Fact_Value_actual)*100;else  $actual_margin_perc="";
					...
					$mkt_cm_value_array[$row[csf('buyer_name')]]+=$CM_Value_Contribution_value_mkt;
					...
					$cm_value_cost_array[$row[csf('buyer_name')]]+=$CM_Value_Contribution_actual;
				...
				?>
                	<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
                        ...
                        <td width="98" style="padding-right:2px" title="CM Value-CM Cost" align="right"><? echo number_format($mkt_margin,2,'.',''); ?></td>
                        <td width="100" style="padding-right:2px" title="Mkt Margin/CM Value*100" align="right"><? echo number_format($mkt_margin_perc,2,'.',''); ?></td>
                    </tr>
                    <tr bgcolor="#F5F5F5" onClick="change_color('tr_a<? echo $i; ?>','#F5F5F5')" id="tr_a<? echo $i; ?>">
                    	<td width="100" align="left"><b>Actual</b></td>
                        <td width="98" align="right" style="padding-right:2px">
                        ...
                        <td width="98" style="padding-right:2px" title="CM Value-CM Cost" align="right"><? echo number_format($actual_margin,2,'.',''); ?></td>
                        <td width="100" style="padding-right:2px" align="right"><? echo number_format($actual_margin_perc,2,'.',''); ?></td>
                    </tr>
                    <tr bgcolor="#E9E9E9" onClick="change_color('tr_v<? echo $i; ?>','#E9E9E9')" id="tr_v<? echo $i; ?>">
                    	<td width="100" align="left"><b>Variance</b></td>
                        ...
                        <td width="98" style="padding-right:2px" title="Actual-Pre Cost(Depends on mkt & Actual Value)" align="right">
                        	<? 
								$actual_margin=$CM_Value_Contribution_actual-$cm_cost_actual;
                    ...
                    <div id="summary_full">
                        <table width="760" cellpadding="0" cellspacing="0" border="1" class="rpt_table" rules="all">
                            ...
                            <tr bgcolor="<? echo $bgcolor1; ?>" onClick="change_color('trtd_17','<? echo $bgcolor1; ?>')" id="trtd_17">
                                ...
                                <td align="right"><? echo number_format($tot_CM_Value_Contribution_value_actual,2); ?></td>
                                <td align="right" title="(Total At Actual CM Value(Contribution)/ Total Ex-factory value)*100"><? if($tot_ex_factory_val>0) echo number_format(($tot_CM_Value_Contribution_value_actual/$tot_ex_factory_val)*100,2);else echo ""; ?></td>
                                <td align="right"><? echo number_format($tot_CM_Value_Contribution_value_mkt-$tot_CM_Value_Contribution_value_actual,2); ?></td>
                            </tr>
                              <tr bgcolor="<? echo $bgcolor1; ?>" onClick="change_color('trtd_17','<? echo $bgcolor1; ?>')" id="trtd_17">
                                <td align="center">14</td>
                                <td>CM</td>
                    ...
                    <div id="summary_buyer">
                        <table width="100%" cellpadding="0" cellspacing="0" border="1" class="rpt_table" rules="all">
                            ...
                            <?
                                ...
                                ?>
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trTD_<? echo $j; ?>','<? echo $bgcolor; ?>')" id="trTD_<? echo $j; ?>">
                                        ...
                                        <td align="right"><? echo number_format($mkt_courier_array[$key],2); $tot_mkt_courier_cost+=$mkt_courier_array[$key]; ?></td>
                                          <td align="right"><? echo number_format($mkt_total_array[$key],2); $tot_mkt_total_cost+=$mkt_total_array[$key]; ?></td>
                                        <td align="right"><? echo number_format($mkt_cm_value_array[$key],2); $tot_mkt_cm_value_cost+=$mkt_cm_value_array[$key]; ?></td>
                                    ...
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trTD2_<? echo $j; ?>','<? echo $bgcolor; ?>')" id="trTD2_<? echo $j; ?>">
                                        ...
                                        <td align="right"><? echo number_format($courier_cost_array[$key],2); $tot_buyer_courier_cost+=$courier_cost_array[$key]; ?></td>
                                          ...
                                          <td align="right"><? echo number_format($total_cost_array[$key],2); $tot_buyer_total_cost+=$total_cost_array[$key]; ?></td>
                                         <td align="right"><? echo number_format($cm_value_cost_array[$key],2); $tot_actual_buyer_cm_value_cost+=$cm_value_cost_array[$key]; ?></td>
                                    ...
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trTD3_<? echo $j; ?>','<? echo $bgcolor; ?>')" id="trTD3_<? echo $j; ?>">
                                       ...
                                       <td align="right">
                                        ...
                                        <td align="right">
                                        <?
                                              $buyer_Net_PO_ex_Fact_Value_actual=$ex_factory_val_array[$key]-$commission_cost_array[$key];
										 $buyer_CM_Value_Contribution_actual=$buyer_Net_PO_ex_Fact_Value_actual-$total_cost_array[$key];
										// echo $mkt_cm_value_array[$key].'='.$buyer_CM_Value_Contribution_actual[$key].'=K';
										if($mkt_cm_value_array[$key]<0 && $buyer_CM_Value_Contribution_actual<0)
										{
											$buyer_variance_margin_cost=($tot_buyer_cm_value_cost)+($buyer_CM_Value_Contribution_actual);
											echo number_format($buyer_variance_margin_cost,2);
										}
										else if($mkt_cm_value_array[$key]>0) //Is greater than 0
										{
											$buyer_variance_margin_cost=($buyer_CM_Value_Contribution_actual)-($mkt_cm_value_array[$key]);
											echo number_format($buyer_variance_margin_cost,2);
											
										}
										else if($mkt_cm_value_array[$key]<0) //Is Less than 0
										{
											$buyer_variance_margin_cost=($buyer_CM_Value_Contribution_actual)+($mkt_cm_value_array[$key]);
											echo number_format($buyer_variance_margin_cost,2);
										}
										...
										{
											$buyer_variance_margin_cost=($mkt_cm_value_array[$key]-$buyer_CM_Value_Contribution_actual);
											echo number_format($buyer_variance_margin_cost,2);
										}
										if($mkt_cm_value_array[$key]>0) $buyer_variance_margin_cost_per=($buyer_variance_margin_cost/$tot_mkt_cm_value_cost)*100; else  $buyer_variance_margin_cost_per="";
										//echo number_format($buyer_variance_margin_cost,2);
                                        ?>
                                        </td>
                                    ...
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trTD4_<? echo $j; ?>','<? echo $bgcolor; ?>')" id="trTD4_<? echo $j; ?>">
                                        ...
                                        <td align="right"><? if($mkt_cm_value_array[$key]>0) echo number_format($buyer_variance_margin_cost_per,2);else echo ""; ?></td>
                            ...
                            <tr bgcolor="<? echo $bgcolor5; ?>" onClick="change_color('trTD5_<? echo $j; ?>','<? echo $bgcolor5; ?>')" id="trTD5_<? echo $j; ?>">
                                ...
                                <th align="right"><? echo number_format($tot_mkt_courier_cost,2); ?></th>
                                 <th align="right"><? echo number_format($tot_mkt_total_cost,2); ?></th>
                                <th align="right"><? echo number_format($tot_mkt_cm_value_cost,2); ?></th>
                            ...
                            <tr bgcolor="<? echo $bgcolor6; ?>" onClick="change_color('trTD6_<? echo $j; ?>','<? echo $bgcolor6; ?>')" id="trTD6_<? echo $j; ?>">
                                ...
                                <th align="right"><? echo number_format($tot_buyer_courier_cost,2); ?></th>
                                <th align="right"><? echo number_format($tot_buyer_total_cost,2); ?></th>
                                <th align="right"><? echo number_format($tot_actual_buyer_cm_value_cost,2); ?></th>
                            ...
                            <tr bgcolor="<? echo $bgcolor7; ?>" onClick="change_color('trTD7_<? echo $j; ?>','<? echo $bgcolor7; ?>')" id="trTD7_<? echo $j; ?>">
                                ...
                                <th align="right"><? $cm_valuevar=$tot_mkt_cm_value_cost-$tot_actual_buyer_cm_value_cost;  echo number_format($cm_valuevar,2); ?></th>
                               
                                <th align="right" title="Actual-Pre Cost"><? $mvar=$tot_buyer_margin_cost-$tot_mkt_margin_cost;  echo number_format($mvar,2); ?></th>
                                <th align="right"><? $mpvar=$mm-$pp;  echo number_format($mpvar,2); ?></th>   
                            </tr>
                            <tr bgcolor="<? echo $bgcolor6; ?>" onClick="change_color('trTD8_<? echo $j; ?>','<? echo $bgcolor6; ?>')" id="trTD8_<? echo $j; ?>">
                                <th colspan="3">Variance (%)</th>
                                ...
                                <th align="right"><?  if($tot_mkt_cm_value_cost>0) echo number_format(($cm_valuevar/$tot_mkt_cm_value_cost*100),2);else echo ""; ?></th>
 ...
 <?
	execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ref_from in (1,2,3) and ENTRY_FORM=11");
	oci_commit($con);
	disconnect($con);
	$html = ob_get_contents();
	ob_clean();
	...
	<fieldset style="width:760px; margin-left:7px">
        ...
        <div style="width:755px; max-height:330px; overflow-y:scroll" id="scroll_body">
			<table border="1" class="rpt_table" rules="all" width="735" cellpadding="0" cellspacing="0">    
                ...
                $sql="select a.booking_no, a.booking_date, a.currency_id, a.exchange_rate, sum(b.amount) as amount from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and b.po_break_down_id in ($po_id) and a.item_category=4 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.booking_no, a.booking_date, a.currency_id, a.exchange_rate";
                $result=sql_select($sql);
                foreach($result as $row)
                {
                    if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";	
	...
	<fieldset style="width:760px; margin-left:7px">
        ...
        <div style="width:755px; max-height:330px; overflow-y:scroll" id="scroll_body">
			<table border="1" class="rpt_table" rules="all" width="735" cellpadding="0" cellspacing="0">    
				<?
                $i=1; $total_aop_cost=0; $avg_rate=76;
                $sql="select a.booking_no, a.booking_date, a.currency_id, a.exchange_rate, sum(b.amount) as amount from wo_booking_mst a, wo_booking_dtls b, wo_pre_cost_embe_cost_dtls c where a.booking_no=b.booking_no and b.pre_cost_fabric_cost_dtls_id=c.id and c.emb_name=3 and b.po_break_down_id in ($po_id) and a.item_category=25 and a.booking_type in(6) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.booking_no, a.booking_date, a.currency_id, a.exchange_rate";
				//echo $sql; die;
                $result=sql_select($sql);
                foreach($result as $row)
                {
...
if($action=="ex_factory_popup")
{
	...
	extract($_REQUEST);
	//echo $id;//$job_no;
	$unit_price_data=sql_select("SELECT id, unit_price from wo_po_break_down where status_active=1 and is_deleted=0 and id in ($id)");
	//echo "SELECT id, unit_price from wo_po_break_down where status_active=1 and is_deleted=0 and id in ($id)"; die;
	foreach($unit_price_data as $row){
		$unit_price_arr[$row[csf('id')]]=$row[csf('unit_price')];
	}
	?>
    ...
    <input type="button" value="Print Preview" onClick="print_window()" style="width:100px"  class="formbutton"/>
        ...
        <div style="width:100%; max-height:400px;">
            <table cellpadding="0" width="100%" cellspacing="0" border="1" rules="all" class="rpt_table">
				...
				$exfac_sql=("select b.challan_no,b.ex_factory_date, b.po_break_down_id,
                ...
                { 
					...
					$unit_price=$unit_price_arr[$row_real[csf('po_break_down_id')]]; 
...
//disconnect($con);
if($action=="report_generate3") //Zakaria joy
{ 
	$process = array( &$_POST );
	extract(check_magic_quote_gpc( $process )); 
	...
	$exchange_date=change_date_format($pc_date_time,'','',1);
	$exchange_rate=set_conversion_rate( 2, $exchange_date,$company_name);	
	...
	$sql="SELECT a.id as job_id, a.job_no, a.company_name, a.season_buyer_wise as season, a.brand_id, a.season_year, a.buyer_name, a.style_ref_no, a.order_uom, a.gmts_item_id, a.total_set_qnty as ratio, a.set_smv, a.dealing_marchant, min(b.pack_handover_date) as pack_handover_date, a.style_description, a.product_dept, (a.job_quantity*a.total_set_qnty) as order_qty, a.avg_unit_price, d.cm_cost from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c, wo_pre_cost_dtls d where a.id=b.job_id and a.id=c.job_id and c.job_id=b.job_id and a.id=d.job_id and c.garments_nature=3 and a.company_name='$company_name' and a.job_no_prefix_num like '$job_no' and a.status_active=1 and a.is_deleted=0 and b.status_active=$cbo_order_status and b.is_deleted=0 $date_cond $buyer_id_cond $po_id_cond $ref_cond $file_cond $shipping_status_cond $year_cond $cbo_status_cond $brand_cond $season_cond $season_year_cond group by a.id, a.job_no, a.company_name,a.season_buyer_wise,a.brand_id, a.season_year, a.buyer_name, a.style_ref_no, a.order_uom, a.gmts_item_id, a.total_set_qnty, a.set_smv, a.style_description, a.product_dept, a.dealing_marchant, a.avg_unit_price, a.job_quantity*a.total_set_qnty, d.cm_cost";
	//echo $sql; die;
	$main_result=sql_select($sql);
	$job_id_arr=array();
	foreach($main_result as $row){
		$job_id_arr[$row[csf('job_id')]]=$row[csf('job_id')];
	}
	$con = connect();
	execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ref_from in (1) and entry_form=11");
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 11, 1, $job_id_arr, $empty_arr);

	$fabric_trims_booking_data=sql_select("SELECT a.id as job_id,c.amount as amount , e.emb_name from wo_po_details_master a, wo_po_break_down  d , wo_booking_dtls c ,wo_booking_mst b , wo_pre_cost_embe_cost_dtls e, gbl_temp_engine f where a.id=d.job_id and a.job_no=c.job_no and  d.id=c.po_break_down_id and c.booking_mst_id=b.id and b.entry_form=574  and c.pre_cost_fabric_cost_dtls_id=e.id and f.ref_val=a.id and c.booking_type=6 and a.garments_nature=3  and c.status_active=1 and c.is_deleted=0 and b.status_active=1 and e.status_active=1 and f.user_id=$user_id and f.ref_from in (1) and f.entry_form=11");
	$b2b_amount_arr=array();
	foreach ($fabric_trims_booking_data as $row) {
		if($row[csf('emb_name')]==3){
			$b2b_amount_arr[$row[csf('job_id')]]['wash']+=$row[csf('amount')];
			$b2b_amount_arr[$row[csf('job_id')]]['total']+=$row[csf('amount')];
		}else{
			$b2b_amount_arr[$row[csf('job_id')]]['emblishment']+=$row[csf('amount')];
			$b2b_amount_arr[$row[csf('job_id')]]['total']+=$row[csf('amount')];
		}
	}

	$fabric_booking_dtls=sql_select("SELECT a.id as booking_id, c.id as job_id, b.amount from wo_booking_mst a join wo_booking_dtls b on a.id=b.booking_mst_id join wo_po_details_master c on b.job_no=c.job_no join gbl_temp_engine d on d.ref_val=c.id where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and d.ref_from in (1) and d.entry_form=11 and d.user_id=$user_id and b.booking_type=1");
	$job_wise_wo_amount=array();
	foreach($fabric_booking_dtls as $row){
		$job_wise_wo_amount[$row[csf('job_id')]]+=$row[csf('amount')];
		$booking_mst_id[$row[csf('booking_id')]]=$row[csf('booking_id')];
	}
	$booking_id_str=implode(',',$booking_mst_id);
	$booking_amount=array();
	if(count($booking_mst_id)>0){
		$fabric_booking_amount=sql_select("SELECT a.id as booking_id, sum(b.amount) as wo_amount from wo_booking_mst a join wo_booking_dtls b on a.id=b.booking_mst_id where a.status_active=1 and b.is_deleted=0 and b.booking_mst_id in ($booking_id_str) and b.booking_type=1 group by a.id");
		
		foreach($fabric_booking_amount as $row){
			$booking_amount[$row[csf('booking_id')]]=$row[csf('wo_amount')];
		}
	}
	//echo "<pre>";
	//print_r($booking_amount); die;

	$pi_number_data=sql_select("SELECT b.booking_mst_id,f.id as job_id, d.total_amount as pi_amount from wo_booking_dtls b,com_pi_item_details c , com_pi_master_details d ,wo_po_details_master f , gbl_temp_engine g where b.booking_no=c.work_order_no and b.job_no = f.job_no and d.id=c.pi_id and g.ref_val=f.id and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.item_category_id in (3) and d.status_active=1 and d.is_deleted=0 and g.ref_from in (1) and g.entry_form=11 and g.user_id=$user_id group by b.booking_mst_id,f.id, d.total_amount");
	foreach ($pi_number_data as $row) {
		$job_wo_ratio=$job_wise_wo_amount[$row[csf('job_id')]]/$booking_amount[$row[csf('booking_mst_id')]]*100;
		$fabric_b2b_value=$job_wo_ratio*$row[csf('pi_amount')]/100;
		$b2b_amount_arr[$row[csf('job_id')]]['fabric']+=$fabric_b2b_value;	
		$b2b_amount_arr[$row[csf('job_id')]]['total']+=$fabric_b2b_value;	
	}
	$pi_number_data_acc=sql_select("SELECT f.id as job_id,d.item_category_id,c.amount from  wo_booking_dtls b,com_pi_item_details c , com_pi_master_details d ,wo_po_details_master f, gbl_temp_engine g where  b.booking_no=c.work_order_no and b.id=c.work_order_dtls_id   and b.job_no = f.job_no and  d.id=c.pi_id and  f.style_ref_no= c.buyer_style_ref and  g.ref_val=f.id and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.item_category_id in (4) and d.status_active=1 and d.is_deleted=0 and g.ref_from in (1) and g.entry_form=11 and g.user_id=$user_id");
	foreach ($pi_number_data_acc as $row) {
		$b2b_amount_arr[$row[csf('job_id')]]['accessories']+=$row[csf('amount')];
		$b2b_amount_arr[$row[csf('job_id')]]['total']+=$row[csf('amount')];
	}

	$labtest_data=sql_select("SELECT c.job_id,b.amount,a.currency from wo_labtest_mst a join wo_labtest_dtls b on a.id=b.mst_id join wo_po_break_down c on c.id=b.po_id join gbl_temp_engine d on d.ref_val=c.job_id where b.status_active=1 and b.is_deleted=0 and d.ref_from in (1) and d.entry_form=11 and d.user_id=$user_id");
	foreach ($labtest_data as $row) {
		if($row[csf('currency')]==1){
			$b2b_amount_arr[$row[csf('job_id')]]['labtest']+=$row[csf('amount')]/$exchange_rate;
			$b2b_amount_arr[$row[csf('job_id')]]['total']+=$row[csf('amount')]/$exchange_rate;
		}
		else{
			$b2b_amount_arr[$row[csf('job_id')]]['labtest']+=$row[csf('amount')];
			$b2b_amount_arr[$row[csf('job_id')]]['total']+=$row[csf('amount')];
		}
	...
	$act_cost_data=sql_select("SELECT a.cost_head, a.amount , b.job_id, a.amount_usd from wo_actual_cost_entry a join wo_po_break_down b on a.po_id=b.id join gbl_temp_engine c on c.ref_val=b.job_id where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.ref_from in (1) and c.entry_form=11 and c.user_id=$user_id and cost_head in (4,6,9)");
	foreach ($act_cost_data as $row) {
		if($row[csf('cost_head')]==4 || $row[csf('cost_head')]==6){
			$b2b_amount_arr[$row[csf('job_id')]]['courier_commercial']+=$row[csf('amount_usd')];
			$b2b_amount_arr[$row[csf('job_id')]]['total']+=$row[csf('amount_usd')];
		}
		if($row[csf('cost_head')]==9){
			$b2b_amount_arr[$row[csf('job_id')]]['others']+=$row[csf('amount_usd')];
			$b2b_amount_arr[$row[csf('job_id')]]['total']+=$row[csf('amount_usd')];
		}
		
	}
	$inspection_cost_data=sql_select("SELECT a.currency_id, b.amount, b.job_id from wo_inspection_mst a join wo_inspection_dtls b on a.id=b.booking_mst_id join gbl_temp_engine c on c.ref_val=b.job_id where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.ref_from in (1) and c.entry_form=11 and c.user_id=$user_id");
	foreach($inspection_cost_data as $row){
		if($row[csf('currency_id')]==1){
			$b2b_amount_arr[$row[csf('job_id')]]['inspection']+=$row[csf('amount')]/$exchange_rate;
			$b2b_amount_arr[$row[csf('job_id')]]['total']+=$row[csf('amount')]/$exchange_rate;
		}
		else{
			$b2b_amount_arr[$row[csf('job_id')]]['inspection']+=$row[csf('amount')];
			$b2b_amount_arr[$row[csf('job_id')]]['total']+=$row[csf('amount')];
		}
	}

	$exfactory_qty=sql_select("SELECT b.job_id, sum(a.ex_factory_qnty) as ex_factory_qnty, min(a.ex_factory_date) as ex_factory_date from pro_ex_factory_mst a join wo_po_break_down b on a.po_break_down_id=b.id join gbl_temp_engine c on c.ref_val=b.job_id where c.entry_form=11 and c.user_id=$user_id and c.ref_from=1 and a.status_active=1 and a.is_deleted=0 group by b.job_id");
	$exfactory_qty_arr=array();
	foreach($exfactory_qty as $row){
		$exfactory_qty_arr[$row[csf('job_id')]]['qty']=$row[csf('ex_factory_qnty')];
	}
	$cut_qty_data=sql_select("SELECT b.job_id, a.production_quantity, a.production_type from pro_garments_production_mst a join wo_po_break_down b on a.po_break_down_id=b.id join gbl_temp_engine c on b.job_id=c.ref_val where a.status_active=1 and a.is_deleted=0 and c.ref_from=1 and c.entry_form=11 and c.user_id=$user_id and a.production_type in (1,5)");
	$cut_qty_arr=array(); $sewing_qty_arr=array();
	foreach($cut_qty_data as $row){
		if($row[csf('production_type')]==1){
			$cut_qty_arr[$row[csf('job_id')]]['qty']+=$row[csf('production_quantity')];
		}else{
			$sewing_qty_arr[$row[csf('job_id')]]['qty']+=$row[csf('production_quantity')];
		}
		
	}

	$actual_cost_data=sql_select("SELECT sum(a.amount_usd) as amount , b.id as job_id from wo_actual_cost_entry a join wo_po_details_master b on a.job_no=b.job_no join gbl_temp_engine c on b.id=c.ref_val where a.status_active=1 and a.is_deleted=0 and c.ref_from=1 and c.entry_form=11 and c.user_id=$user_id and a.cost_head=5 group by b.id");
	$actual_cost_arr=array();
	foreach($actual_cost_data as $row){
		$actual_cost_arr[$row[csf('job_id')]]['amt']=$row[csf('amount')];
	}

	$budget_wast_cost=sql_select("SELECT a.job_id, a.amount from wo_pre_cost_embe_cost_dtls a join gbl_temp_engine b on a.job_id=b.ref_val where b.ref_from=1 and b.entry_form=11 and b.user_id=$user_id and a.status_active=1 and a.is_deleted=0 and a.emb_name=3");
	foreach($budget_wast_cost as $row){
		$budget_wash_arr[$row[csf('job_id')]]=$row[csf('amount')];
	}
    ...
    <fieldset>
    	<table width="2720">
            <tr class="form_caption">
                <td colspan="25" align="center"><strong>Post Costing Report</strong></td>
            </tr>
            <tr class="form_caption">
                <td colspan="25" align="center"><strong><? echo $company_arr[$company_name];?></strong>
                    <br>
                    <strong><? echo change_date_format(str_replace("'","",$txt_date_from)) .' To '. change_date_format(str_replace("'","",$txt_date_to)); ?></strong>
                </td>
            </tr>
			<tr>				
				<td colspan="25" align="left">**Current Exchange Rate:<?= $exchange_rate ?></td>
			</tr>
        </table>
        <table style="margin-top:10px" id="table_header_1" class="rpt_table" width="2720" cellpadding="0" cellspacing="0" border="1" rules="all">
            <thead>
                <th width="40">SL</th>
                ...
                <th width="80">Merchant</th>
                <th width="55">PCD</th>
                <th width="60">Order Qty</th>
                <th width="90">Cut Qnty</th>
                <th width="90">Ship Qty</th>
                <th width="40">FOB</th>
                <th width="90" title="FOB*Ship Qty">Total FOB</th>                    
                <th width="90">Fabric LC Value</th>
                <th width="90">Acc. LC Value</th>
                <th width="90">Wash LC Value</th>
                <th width="90">Print/Emb LC Value</th>
                <th width="90">Test cost</th>
                <th width="90">Commercial Cost</th>
                <th width="90">Inspection Cost</th>
                <th width="90">Others</th>
                <th width="90">Total B2B</th>
                <th width="90" title="Total FOB-Total B2B">Amount Balance</th>
                <th width="90" title="[Budget CM]">Costed CM/Pc</th>
                <th width="90" title="Total style expense/No. of produced(output) qty of that style">Produced CM/Pc</th>
                <th width="90" title="(Costed CM/Ps - Produced CM/Pc)*Order Qty">Profit/loss from CM</th>
                <th width="90" title="Order Qty*Produced CM/Pc">Total Produced CM</th>
                <th width="90" title="Amount Balance-Total Prudced CM">Net Buffer</th>
                <th width="90" title="(Net Buffer/Total FOB)">Buffer %</th>
                <th width="90">Cut to ship loss Value $</th>
                <th width="90">Tolerence</th>
                <th width="90" title="(Cut to ship loss Value - Tolerance)">Opportunity Loss</th>
                <th width="90" title="[{(Cut Qty-Ship Qty)/Cut Qty}*100]">Cut to ship loss %</th>
            ...
            <?
				$i=1;
				foreach($main_result as $row){ 
					$total_b2b=$b2b_amount_arr[$row[csf('job_id')]]['total'];
					$total_fob=$exfactory_qty_arr[$row[csf('job_id')]]['qty']*$row[csf('avg_unit_price')];
					$amount_balance=$total_fob-$total_b2b;				
					
					$produced_cm=0;
					if($actual_cost_arr[$row[csf('job_id')]]['amt']>0){
						$produced_cm=$actual_cost_arr[$row[csf('job_id')]]['amt']/$sewing_qty_arr[$row[csf('job_id')]]['qty'];
					}
					$total_cm=fn_number_format($produced_cm,2)*$row[csf('order_qty')];
					$net_buffer=$amount_balance-$total_cm;
					$profit_loss_cm=(fn_number_format($row[csf('cm_cost')],2)-fn_number_format($produced_cm,2))*$row[csf('order_qty')];
					if($budget_wash_arr[$row[csf('job_id')]]>0){
						$cut_to_ship_title="[{Cut Qty-(Cut Qty*1%)}- Ship Qty*FOB]";
						$tolerence_title="[(Cut Qty*1%)*FOB]";
						$cut_qty_per=($cut_qty_arr[$row[csf('job_id')]]['qty']*0.01);
						$cut_ship_loss_value=(($cut_qty_arr[$row[csf('job_id')]]['qty']-$cut_qty_per)-$exfactory_qty_arr[$row[csf('job_id')]]['qty'])*$row[csf('avg_unit_price')];
						$tolerence=$cut_qty_per*$row[csf('avg_unit_price')];
						$opportunity_loss=$cut_ship_loss_value-$tolerence;
					}
					else{
						$cut_to_ship_title="[{Cut Qty-(Cut Qty*.5%)}- Ship Qty*FOB]";
						$tolerence_title="[(Cut Qty*.5%)*FOB]";
						$cut_qty_per=($cut_qty_arr[$row[csf('job_id')]]['qty']*0.005);
						$cut_ship_loss_value=(($cut_qty_arr[$row[csf('job_id')]]['qty']-$cut_qty_per)-$exfactory_qty_arr[$row[csf('job_id')]]['qty'])*$row[csf('avg_unit_price')];
						$tolerence=$cut_qty_per*$row[csf('avg_unit_price')];
						$opportunity_loss=$cut_ship_loss_value-$tolerence;
					}
					
					$cut_ship_loss_percent=(($cut_qty_arr[$row[csf('job_id')]]['qty']-$exfactory_qty_arr[$row[csf('job_id')]]['qty'])/$cut_qty_arr[$row[csf('job_id')]]['qty'])*100;
					if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF"; 

				?>
				<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trTD_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="trTD_<? echo $i; ?>">
					<td><?= $i ?></td>
					<td><?= $buyer_arr[$row[csf('buyer_name')]] ?></td>
					<td title="<?= $row[csf('job_no')] ?>"><?= $row[csf('style_ref_no')] ?></td>
					<td><?= $row[csf('style_description')] ?></td>
					<td><?= $product_dept[$row[csf('product_dept')]] ?></td>
					<td><?= $dealing_merchant_array[$row[csf('dealing_marchant')]] ?></td>
					<td><?= change_date_format($row[csf('pack_handover_date')]) ?></td>
					<td align="right"><?= fn_number_format($row[csf('order_qty')],0) ?></td>
					<td align="right"><?= fn_number_format($cut_qty_arr[$row[csf('job_id')]]['qty'],0) ?></td>
					<td align="right"><?= fn_number_format($exfactory_qty_arr[$row[csf('job_id')]]['qty'],0) ?></td>
					<td align="right"><?= fn_number_format($row[csf('avg_unit_price')],2) ?></td>
					<td align="right"><?= fn_number_format($total_fob,2) ?></td>
					<td align="right"><?= fn_number_format($b2b_amount_arr[$row[csf('job_id')]]['fabric'],2,'.','') ?></td>
					<td align="right"><?= fn_number_format($b2b_amount_arr[$row[csf('job_id')]]['accessories'],2,'.','') ?></td>
					<td align="right"><?= fn_number_format($b2b_amount_arr[$row[csf('job_id')]]['wash'],2,'.','') ?></td>
					<td align="right"><?= fn_number_format($b2b_amount_arr[$row[csf('job_id')]]['emblishment'],2,'.','') ?></td>
					<td align="right"><?= fn_number_format($b2b_amount_arr[$row[csf('job_id')]]['labtest'],2,'.','') ?></td>
					<td align="right"><?= fn_number_format($b2b_amount_arr[$row[csf('job_id')]]['courier_commercial'],2,'.','') ?></td>
					<td align="right"><?= fn_number_format($b2b_amount_arr[$row[csf('job_id')]]['inspection'],2,'.','') ?></td>
					<td align="right"><?= fn_number_format($b2b_amount_arr[$row[csf('job_id')]]['others'],2,'.','') ?></td>
					<td align="right"><?= fn_number_format($total_b2b,2) ?></td>
					<td align="right"><?= fn_number_format($amount_balance,2) ?></td>
					<td align="right"><?= fn_number_format($row[csf('cm_cost')],2); ?></td>
					<td align="right"><?= fn_number_format($produced_cm,2) ?></td>
					<td align="right"><?= fn_number_format($profit_loss_cm,2) ?></td>
					<td align="right"><?= fn_number_format($total_cm,2) ?></td>
					<td align="right"><?= fn_number_format($net_buffer,2) ?></td>
					<td align="right"><?= fn_number_format($net_buffer/$total_fob,2) ?></td>
					<td align="right" title="<?=$cut_to_ship_title?>"><?= fn_number_format($cut_ship_loss_value,2) ?></td>
					<td align="right" title="<?=$tolerence_title?>"><?= fn_number_format($tolerence,2) ?></td>
					<td align="right"> <?= fn_number_format($opportunity_loss,2) ?></td>
					<td align="right"><?= fn_number_format($cut_ship_loss_percent,2) ?></td>
					<td align="right"></td>
					
					
				</tr>
				<? 
				$total_order+=$row[csf('order_qty')];
				$total_cut_qty+=$cut_qty_arr[$row[csf('job_id')]]['qty'];
				$total_ship_qty+=$exfactory_qty_arr[$row[csf('job_id')]]['qty'];
				$total_fob_value+=$total_fob;
				$total_lc_value+=$b2b_amount_arr[$row[csf('job_id')]]['fabric'];
				$total_accessories_value+=$b2b_amount_arr[$row[csf('job_id')]]['accessories'];
				$total_wash_value+=$b2b_amount_arr[$row[csf('job_id')]]['wash'];
				$total_emblishment_value+=$b2b_amount_arr[$row[csf('job_id')]]['emblishment'];
				$total_test_value+=$b2b_amount_arr[$row[csf('job_id')]]['labtest'];
				$total_courier_value+=$b2b_amount_arr[$row[csf('job_id')]]['courier_commercial'];
				$total_inspection_value+=$b2b_amount_arr[$row[csf('job_id')]]['inspection'];
				$total_b2b_value+=$total_b2b;
				$total_amount_balance+=$amount_balance;
				$total_profit_loss_cm+=$profit_loss_cm;
				$grand_total_cm+=$total_cm;
				$grand_net_buffer+=$net_buffer;
				$total_net_buffer_per+=$net_buffer/$total_fob;
				$total_cut_ship_loss_value+=$cut_ship_loss_value;
				$total_cut_ship_loss_per+=$cut_ship_loss_percent;
				$total_tolerence+=$tolerence;
				$total_opportunity_loss+=$opportunity_loss;
				$i++;
				}
			?>
			</tbody>
			<tfoot>
				<tr bgcolor="#E9E9E9">
					...
					<td align="right"><b><?= fn_number_format($total_order) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_cut_qty) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_ship_qty) ?></b></td>
					<td></td>
					<td align="right"><b><?= fn_number_format($total_fob_value,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_lc_value,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_accessories_value,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_wash_value,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_emblishment_value,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_test_value,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_courier_value,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_inspection_value,2) ?></b></td>
					<td> </td>
					<td align="right"><b><?= fn_number_format($total_b2b_value,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_amount_balance,2) ?></b></td>
					<td></td>
					<td></td>
					<td align="right"><b><?= fn_number_format($total_profit_loss_cm,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($grand_total_cm,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($grand_net_buffer,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_net_buffer_per,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_cut_ship_loss_value,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_tolerence,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_opportunity_loss,2) ?></b></td>
					<td align="right"><b><?= fn_number_format($total_cut_ship_loss_per,2) ?></b></td>				
					<td></td>
				</tr>
			</tfoot>
            </table>
...
if($action=="report_generate4") //Zakaria joy
{ 
	$process = array( &$_POST );
	extract(check_magic_quote_gpc( $process )); 
	...
	$sql="SELECT a.job_no_prefix_num, a.id as job_id, a.job_no, $year_field, a.company_name,a.season_buyer_wise as season,a.brand_id,a.season_year, a.buyer_name, a.style_ref_no, a.order_uom, a.gmts_item_id, a.total_set_qnty as ratio, a.set_smv, b.id, b.po_number,b.grouping,b.file_no, b.pub_shipment_date, b.po_quantity, b.plan_cut, b.unit_price, b.po_total_price, b.shiping_status, a.job_quantity, a.avg_unit_price, a.total_price from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no and c.job_no=b.job_no_mst and c.garments_nature=3 and a.company_name='$company_name' and a.job_no_prefix_num like '$job_no' and a.status_active=1 and a.is_deleted=0 and b.status_active=$cbo_order_status and b.is_deleted=0 $date_cond $buyer_id_cond $po_id_cond $ref_cond $file_cond $shipping_status_cond $year_cond $cbo_status_cond $brand_cond $season_cond $season_year_cond order by a.id";
	//echo $sql; die;
	$result=sql_select($sql);
	$all_po_id=array();  $job_id_arr=array();
	foreach($result as $row)
	{
		...
		$shiping_status_arr[$row[csf("job_id")]][]=$row[csf('shiping_status')];
		
	}
	$statusArray = [];
	foreach($shiping_status_arr as $job_id => $approvals){
		...
		elseif(!empty($aprovalCountArr[3]) && ($aprovalCountArr[3] == $appNum)){
			$status = 'Full Delivery/Closed'; 
		}
		elseif(!empty($aprovalCountArr[3]) && ($aprovalCountArr[3] < $appNum)){
			$status = 'Partial Delivery'; 
		}
		$statusArray[$job_id] = $status ;
	}
	...
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 11, 1, $job_id_arr, $empty_arr);
	$select_all_po=sql_select("SELECT a.id as po_id, a.job_id from wo_po_break_down a join gbl_temp_engine b on a.job_id=b.ref_val where b.ref_from=1 and b.entry_form=11 and b.user_id=$user_name");
	foreach($select_all_po as $row){
		$po_id_arr[$row[csf("po_id")]]=$row[csf("po_id")];
		$job_wise_po[$row[csf("job_id")]][$row[csf("po_id")]]=$row[csf("po_id")];
	}
	$po_ids=implode(",",$po_id_arr);
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 11, 2, $po_id_arr, $empty_arr);	

	$heading_arr=array('po_exfactory_qty'=>'PO/Ex-Factory Qnty','po_exfactory_value'=>'PO/Ex-Factory Value','fabric_purchase_cost'=>'Fabric Purchase Cost','trims_cost'=>'Trims Cost','embell_cost'=>'Embellishment Cost','wash_cost'=>'Wash Cost','commission_cost'=>'Commission Cost','commercial_cost'=>'Commercial Cost','freight_cost'=>'Freight Cost','test_cost'=>'Testing Cost','inspection_cost'=>'Inspection Cost','currier_cost'=>'Courier Cost','total_cost'=>'Tot Mat. Serv. Cost','CM_Contribution_value'=>'CM Value (Contribution)','cm_cost'=>'CM Cost','margin'=>'Margin','discount_allowed'=>'Discount Allowed','rmg'=>'RMG','source_tax'=>'Source Tax','short_realized'=>'Short Realized','actual_margin'=>'Actual Margin','per_exfactory_value'=>'% to Ex-Factory Value');

	$cost_heading_arr=array('po_exfactory_qty', 'po_exfactory_value', 'fabric_purchase_cost', 'trims_cost', 'embell_cost', 'wash_cost', 'commission_cost', 'commercial_cost' , 'freight_cost', 'test_cost', 'inspection_cost', 'currier_cost', 'total_cost', 'CM_Contribution_value', 'cm_cost', 'margin', 'discount_allowed', 'rmg', 'source_tax', 'short_realized', 'actual_margin', 'per_exfactory_value');

	$pre_cost_title_arr=array('CM_Contribution_value'=>'PO Value-Commission-All Cost','margin'=>'CM Value-CM Cost', 'actual_margin'=>'Margin-Discount Allowed-RMG-Source Tax-Short Realized','per_exfactory_value'=>'(MKT Margin/CM Value)*100');

	$actual_cost_title_arr=array('CM_Contribution_value'=>'(Ex-factory Value-Commission-All Cost', 'margin'=>'CM Value-CM Cost', 'actual_margin'=>'Margin-Discount Allowed-RMG-Source Tax-Short Realized');

	$variance_title_arr=array('po_exfactory_qty'=>'(Ex-factory - PO Qty)', 'margin'=>'Actual-Pre-cost(Depends on MKT & Actual Value)');
    ...
    <fieldset>
        ...
        <table style="margin-top:10px" id="table_header_1" class="rpt_table" width="3840" cellpadding="0" cellspacing="0" border="1" rules="all">
            ...
            <?
				$fabriccostArray=array(); $trimsCostArray=array(); $actualCostArray=array(); $actualTrimsCostArray=array(); 
				$subconCostArray=array(); $embellCostArray=array(); $washCostArray=array(); $aopCostArray=array(); $yarnTrimsCostArray=array(); 

				$ex_factory_data=sql_select("SELECT a.job_id, sum(CASE WHEN b.entry_form!=85 THEN b.ex_factory_qnty ELSE 0 END)-sum(CASE WHEN b.entry_form=85 THEN b.ex_factory_qnty ELSE 0 END) as qnty from pro_ex_factory_mst b join gbl_temp_engine c on b.po_break_down_id=c.ref_val join wo_po_break_down a on a.id=b.po_break_down_id where b.status_active=1 and b.is_deleted=0 and c.ref_from=2 and c.entry_form=11 and c.user_id=$user_id group by a.job_id");
				foreach($ex_factory_data as $row){
					$ex_factory_arr[$row[csf('job_id')]]=$row[csf('qnty')];
				}
				
				$fabriccostDataArray=sql_select("SELECT a.job_id, a.costing_per_id, a.embel_cost, a.wash_cost, a.cm_cost, a.commission, a.currier_pre_cost, a.lab_test, a.inspection, a.freight, a.comm_cost from wo_pre_cost_dtls a join gbl_temp_engine b on a.job_id=b.ref_val where a.status_active=1 and a.is_deleted=0 and b.ref_from=1 and b.entry_form=11 and b.user_id=$user_id");
				foreach($fabriccostDataArray as $fabRow)
				{
				...
				/* echo '<pre>';
				print_r($fabriccostArray); die; */
				
				$actualCostDataArray=sql_select("SELECT a.cost_head,c.job_id,sum(a.amount_usd) as amount_usd, sum(a.amount) as amt_tk from wo_actual_cost_entry a join gbl_temp_engine b on a.po_id=b.ref_val join wo_po_break_down c on c.id=a.po_id where a.status_active=1 and a.is_deleted=0 and b.ref_from=2 and b.entry_form=11 and b.user_id=$user_id group by a.cost_head,c.job_id");
				foreach($actualCostDataArray as $row)
				{
					if($row[csf('cost_head')]==17 || $row[csf('cost_head')]==18){
						$exchange_date=change_date_format($pc_date_time,'','',1);
						$exchange_rate=set_conversion_rate( 2, $exchange_date,$company_name);
						$actualCostArray[$row[csf('cost_head')]][$row[csf('job_id')]]=$row[csf('amt_tk')]/$exchange_rate;
					}
					else{
						$actualCostArray[$row[csf('cost_head')]][$row[csf('job_id')]]=$row[csf('amount_usd')];
					}
				   
				}
				/* echo '<pre>';
				print_r($actualCostArray); die;	 */		
				
				$embell_type_arr=return_library_array("SELECT a.id, a.emb_name from wo_pre_cost_embe_cost_dtls a join gbl_temp_engine b on a.job_id=b.ref_val where b.ref_from=1 and b.entry_form=11 and b.user_id=$user_id", "id", "emb_name");

				$bookingDataArray=sql_select("SELECT a.booking_type, a.item_category, a.currency_id, a.exchange_rate, b.po_break_down_id, b.process, b.amount, b.pre_cost_fabric_cost_dtls_id, d.job_id from wo_booking_mst a, wo_booking_dtls b, gbl_temp_engine c, wo_po_break_down d  where a.booking_no=b.booking_no and a.item_category in(4,12,25) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.po_break_down_id=c.ref_val and c.ref_from=2 and c.entry_form=11 and c.user_id=$user_id and b.po_break_down_id=d.id");
				foreach($bookingDataArray as $woRow)
				{
					$amount=0; $trimsAmnt=0;
					if($woRow[csf('currency_id')]==1) { $amount=$woRow[csf('amount')]/$exchange_rate; } else { $amount=$woRow[csf('amount')]; }
					...
					{ 
						if($embell_type_arr[$woRow[csf('pre_cost_fabric_cost_dtls_id')]]==3)
						{
							//$washCostArray[$woRow[csf('job_id')]]+=$amount; 
						}
						else
						{
							$embellCostArray[$woRow[csf('job_id')]]+=$amount; 
						}
					}
					else if($woRow[csf('item_category')]==12 && $woRow[csf('process')]==35 && ($woRow[csf('booking_type')]==3 || $woRow[csf('booking_type')]==6)) 
					{ 
						$aopCostArray[$woRow[csf('job_id')]]+=$amount; 
					}
					else if($woRow[csf('item_category')]==4)
					{
						if($woRow[csf('currency_id')]==1) { $trimsAmnt=$woRow[csf('amount')]/$woRow[csf('exchange_rate')]; } else { $trimsAmnt=$woRow[csf('amount')]; }
						$actualTrimsCostArray[$woRow[csf('job_id')]]+=$trimsAmnt; 
					}
				}				
				//$ex_rate=80;
				$pi_number_check=array();
				...
				$sql_fin_purchase="SELECT  c.currency_id,b.po_breakdown_id, e.job_id, sum(a.cons_rate*b.quantity) as finish_purchase_amnt,sum(a.order_amount) as order_amount from inv_receive_master c,inv_transaction a, order_wise_pro_details b, gbl_temp_engine d, wo_po_break_down e where a.id=b.trans_id and a.receive_basis in(1,2) and a.item_category=2 and a.transaction_type=1 and b.entry_form=37 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.po_breakdown_id=d.ref_val and d.ref_from=2 and d.entry_form=11 and d.user_id=$user_id and b.po_breakdown_id=e.id group by e.job_id,b.po_breakdown_id, c.currency_id";
				$dataArrayFinPurchase=sql_select($sql_fin_purchase);
				foreach($dataArrayFinPurchase as $finRow)
				{
					if($pi_number_check[$finRow[csf('po_breakdown_id')]]==$finRow[csf('po_breakdown_id')])
					{
						...
						if($currency_id==2)//USD
						{
							$finish_purchase_amnt_arr[$finRow[csf('job_id')]]=$finRow[csf('order_amount')];
						}
						else
						{
							$finish_purchase_amnt_arr[$finRow[csf('job_id')]]+=$finRow[csf('woven_purchase_amnt')]/$exchange_rate;
						}
					}
				}				

				$sql_fin_purchase_wv="SELECT a.id, a.booking_no,a.booking_date,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.booking_type,b.job_no,b.po_break_down_id as po_id,sum(b.grey_fab_qnty) as grey_fab_qnty,sum(b.amount) as amount, c.job_id from wo_booking_mst a, wo_booking_dtls b,wo_pre_cost_fabric_cost_dtls c , gbl_temp_engine d where a.booking_no=b.booking_no and c.id=b.pre_cost_fabric_cost_dtls_id and a.booking_type=1 and b.fin_fab_qnty>0 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1  and c.is_deleted=0 and c.status_active=1 and d.ref_val=b.po_break_down_id and d.ref_from=2 and d.entry_form=11 and d.user_id=$user_id group by a.id, a.booking_no,a.booking_date,a.booking_date,a.supplier_id,a.currency_id,a.exchange_rate,a.booking_type,b.job_no,b.po_break_down_id, c.job_id";
				...
				foreach($dataArrayFinPurchaseW as $finRow)
				{
					...
					if($currency_id==2)//USD
					{
						$finish_purchase_amnt_arr[$finRow[csf('job_id')]]+=$finRow[csf('amount')];
					}
					else
					{
						$finish_purchase_amnt_arr[$finRow[csf('job_id')]]+=$finRow[csf('amount')]/$exchange_rate;
					}
				}
				$current_pc_date=change_date_format($pc_date_time, "yyyy-mm-dd", "-",1)	;
				$financial_data_array=sql_select("select actual_cost_rmg_per,actual_cost_source_tax_per from lib_standard_cm_entry where company_id=$company_name   and status_active=1 and is_deleted=0 and '$current_pc_date' between applying_period_date and applying_period_to_date");
				foreach ($financial_data_array as $row)
				{
					$actual_cost_rmg_per=$row[csf('actual_cost_rmg_per')]/100;
					$actual_cost_source_tax_per=$row[csf('actual_cost_source_tax_per')]/100;
				}
				$comm_cost_from_invoice=sql_select("SELECT sum(b.current_invoice_value) as current_invoice_value, c.job_id, a.discount_in_percent, a.claim_in_percent, a.other_discount_percent from com_export_invoice_ship_mst a join com_export_invoice_ship_dtls b on a.id=b.mst_id join wo_po_break_down c on c.id=b.po_breakdown_id join gbl_temp_engine d on c.id=d.ref_val where d.ref_from=2 and d.entry_form=11 and d.user_id=$user_name and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by c.job_id, a.discount_in_percent, a.claim_in_percent, a.other_discount_percent");
				$discount_value=0; $claim_value=0; $other_discount_value=0;
				foreach($comm_cost_from_invoice as $row){
					if($row[csf('discount_in_percent')]>0){
						$discount_value=$row[csf('current_invoice_value')]*($row[csf('discount_in_percent')]/100);
					}
					if($row[csf('claim_in_percent')]>0){					
						$claim_value=$row[csf('current_invoice_value')]*($row[csf('claim_in_percent')]/100);
					}
					if($row[csf('other_discount_percent')]>0){
						$other_discount_value=$row[csf('current_invoice_value')]*($row[csf('other_discount_percent')]/100);
					}
					$net_invoice_value=	$row[csf('current_invoice_value')]-($discount_value+$claim_value+$other_discount_value);
					$rm_cost_arr[$row[csf('job_id')]]+=$net_invoice_value;
				}
				$labtest_data=sql_select("SELECT c.job_id,b.amount,a.currency from wo_labtest_mst a join wo_labtest_dtls b on a.id=b.mst_id join wo_po_break_down c on c.id=b.po_id join gbl_temp_engine d on d.ref_val=c.job_id where b.status_active=1 and b.is_deleted=0 and d.ref_from in (1) and d.entry_form=11 and d.user_id=$user_name");
				foreach ($labtest_data as $row) {
					if($row[csf('currency')]==1){
						$actualtestCostArray[$row[csf('job_id')]]+=$row[csf('amount')]/$exchange_rate;
					}
					else{
						$actualtestCostArray[$row[csf('job_id')]]+=$row[csf('amount')];
					}
					
				}
				$inspection_cost_data=sql_select("SELECT a.currency_id, b.amount, b.job_id from wo_inspection_mst a join wo_inspection_dtls b on a.id=b.booking_mst_id join gbl_temp_engine c on c.ref_val=b.job_id where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.ref_from in (1) and c.entry_form=11 and c.user_id=$user_name");
				foreach($inspection_cost_data as $row){
					if($row[csf('currency_id')]==1){
						$actualinspectionCostArray[$row[csf('job_id')]]+=$row[csf('amount')]/$exchange_rate;
					}
					else{
						$actualinspectionCostArray[$row[csf('job_id')]]+=$row[csf('amount')];
					}
				}

				$embellishment_booking_data=sql_select("SELECT a.id as job_id,c.amount as amount , b.currency_id from wo_po_details_master a, wo_po_break_down  d , wo_booking_dtls c ,wo_booking_mst b , wo_pre_cost_embe_cost_dtls e, gbl_temp_engine f where a.id=d.job_id and a.job_no=c.job_no and  d.id=c.po_break_down_id and c.booking_mst_id=b.id and c.pre_cost_fabric_cost_dtls_id=e.id and f.ref_val=a.id and c.booking_type=6 and a.garments_nature=3  and c.status_active=1 and c.is_deleted=0 and b.status_active=1 and e.status_active=1 and f.user_id=$user_name and f.ref_from in (1) and f.entry_form=11 and e.emb_name=3");
				foreach ($embellishment_booking_data as $row) {
					if($row[csf('currency_id')]==1){
						$washCostArray[$row[csf('job_id')]]+=$row[csf('amount')]/$exchange_rate;
					}
					else{
						$washCostArray[$row[csf('job_id')]]+=$row[csf('amount')];
					}
				}
				...
				$not_yarn_dyed_cost_arr=array(1,2,30,35);
				$dzn_qnty_arr=array(1=>12, 2=>1, 3=>24, 4=>36, 5=>48);
				$embell_cost_mkt_arr=array(1,2,4,5);
				$main_data_attributes=array('buyer_name', 'brand_id', 'year', 'job_no_prefix_num', 'file_no','style_ref_no', 'order_uom', 'set_smv','job_quantity', 'avg_unit_price', 'total_price', 'season', 'season_year');
				$main_data_arr=array();
				foreach($result as $row){
					...
					$main_data_arr[$jobid]['po_number'][$row[csf('po_number')]]= $row[csf('po_number')];
					$main_data_arr[$jobid]['ship_date']= $max_date[$row[csf('job_id')]];
					$main_data_arr[$jobid]['shiping_status']= $statusArray[$jobid];
					$gmts_item_id=explode(",",$row[csf('gmts_item_id')]);
					foreach($gmts_item_id as $item_id)
					{
					...
					$dzn_qnty=0;
					$costing_per_id=$fabriccostArray[$jobid]['costing_per_id'];
					$dzn_qnty=$dzn_qnty_arr[$costing_per_id];
					//echo $jobid.'--'.$costing_per_id.'--'.$dzn_qnty; die;
					$unit_price=$row[csf('avg_unit_price')];

					//Pre Cost Data
					$fabric_purchase_cost_mkt=array_sum($fabric_costing_arr['knit']['grey'][$jobno])+array_sum($fabric_costing_arr['woven']['grey'][$jobno]);
					...
					$special_amount=$emblishment_costing_arr_name[$jobno][4];
					$other_amount=$emblishment_costing_arr_name[$jobno][5];
					$embell_cost_mkt=$print_amount+$embroidery_amount+$special_amount+$other_amount;
					...
					$commission_cost_mkt=$local_cost+$foreign_cost;
					...
					$currier_cost=$other_costing_arr[$jobno]['currier_pre_cost'];
					$mkt_all_cost=$fabric_purchase_cost_mkt+$trims_cost_mkt+$embell_cost_mkt+$wash_cost+$commission_cost_mkt+$comm_cost_mkt+$freight_cost+$test_cost+$inspection_cost+$currier_cost;
					$Net_PO_ex_Fact_Value_mkt=$row[csf('total_price')]-$commission_cost_mkt;
					$CM_Value_Contribution_value_mkt=$Net_PO_ex_Fact_Value_mkt-$mkt_all_cost;
					$cm_cost=$other_costing_arr[$jobno]['cm_cost'];
					$mkt_margin=$CM_Value_Contribution_value_mkt-$cm_cost;

					if($mkt_margin>0 && $CM_Value_Contribution_value_mkt>0)     $mkt_margin_perc=($mkt_margin/$CM_Value_Contribution_value_mkt)*100;else $mkt_margin_perc="";
					

					$main_data_arr[$jobid]['pre']['po_exfactory_qty']=$row[csf('job_quantity')];
					$main_data_arr[$jobid]['pre']['po_exfactory_value']=$row[csf('total_price')];
					$main_data_arr[$jobid]['pre']['fabric_purchase_cost']=$fabric_purchase_cost_mkt;
					$main_data_arr[$jobid]['pre']['trims_cost']=$trims_cost_mkt;
					$main_data_arr[$jobid]['pre']['embell_cost']=$embell_cost_mkt;
					$main_data_arr[$jobid]['pre']['wash_cost']=$wash_cost;
					$main_data_arr[$jobid]['pre']['commission_cost']=$commission_cost_mkt;
					$main_data_arr[$jobid]['pre']['commercial_cost']=$comm_cost_mkt;
					$main_data_arr[$jobid]['pre']['freight_cost']=$freight_cost;
					$main_data_arr[$jobid]['pre']['test_cost']=$test_cost;
					$main_data_arr[$jobid]['pre']['inspection_cost']=$inspection_cost;
					$main_data_arr[$jobid]['pre']['currier_cost']=$currier_cost;
					$main_data_arr[$jobid]['pre']['total_cost']=$mkt_all_cost;
					$main_data_arr[$jobid]['pre']['CM_Contribution_value']=$CM_Value_Contribution_value_mkt;
					$main_data_arr[$jobid]['pre']['cm_cost']=$cm_cost;
					$main_data_arr[$jobid]['pre']['margin']=$mkt_margin;
					$main_data_arr[$jobid]['pre']['discount_allowed']=0;
					$main_data_arr[$jobid]['pre']['rmg']=0;
					$main_data_arr[$jobid]['pre']['source_tax']=0;
					$main_data_arr[$jobid]['pre']['short_realized']=0;
					$main_data_arr[$jobid]['pre']['actual_margin']=$mkt_margin;
					$main_data_arr[$jobid]['pre']['per_exfactory_value']=$mkt_margin_perc;

					//Actual Cost Data
					$ex_factory_qty=$ex_factory_arr[$jobid];
    				$ex_factory_value=$ex_factory_qty*$unit_price;
					$fabric_purchase_cost_actual=$finish_purchase_amnt_arr[$jobid];
					$trims_cost_actual=$actualTrimsCostArray[$jobid];
					$embell_cost_actual=$embellCostArray[$jobid];
					$wash_cost_actual=$washCostArray[$jobid];
					/* $commission_cost_actual=0;
					if($fabriccostArray[$jobid]['commission']>0){
						$commission_cost_actual=($ex_factory_qty/$dzn_qnty)*$fabriccostArray[$jobid]['commission'];
					} */
					$commission_cost_actual=$actualCostArray[10][$jobid];				

					$comm_cost_actual=$actualCostArray[6][$jobid];
					$freight_cost_actual=$actualCostArray[2][$jobid];

					$test_cost_actual=$actualtestCostArray[$jobid];
					$inspection_cost_actual=$actualinspectionCostArray[$jobid];

					$currier_cost_actual=$actualCostArray[4][$jobid];
					$cm_cost_actual=$actualCostArray[5][$jobid];
					$discount_allowed=$actualCostArray[17][$jobid];
					$short_realized=$actualCostArray[18][$jobid];

					$actual_all_cost=$fabric_purchase_cost_actual+$trims_cost_actual+$embell_cost_actual+$wash_cost_actual+$commission_cost_actual+$comm_cost_actual+$freight_cost_actual+$test_cost_actual+$inspection_cost_actual+$currier_cost_actual;

					$Net_PO_ex_Fact_Value_actual=$ex_factory_value-$commission_cost_actual;
    				$cm_value_contribution_actual=$Net_PO_ex_Fact_Value_actual-$actual_all_cost;
					$rmg_value_actual=$rm_cost_arr[$jobid]*$actual_cost_rmg_per;
					$source_tax_actual=$rm_cost_arr[$jobid]*$actual_cost_source_tax_per;

					$margin=$cm_value_contribution_actual-$cm_cost_actual;
					$actual_margin=$margin-($discount_allowed+$rmg_value_actual+$source_tax_actual+$short_realized);

					if($margin>0 && $Net_PO_ex_Fact_Value_actual>0)  $actual_margin_perc=($margin/$Net_PO_ex_Fact_Value_actual)*100;else  $actual_margin_perc=0;

					$main_data_arr[$jobid]['act']['po_exfactory_qty']=$ex_factory_qty;
					$main_data_arr[$jobid]['act']['po_exfactory_value']=$ex_factory_value;
					$main_data_arr[$jobid]['act']['fabric_purchase_cost']=$fabric_purchase_cost_actual;
					$main_data_arr[$jobid]['act']['trims_cost']=$trims_cost_actual;
					$main_data_arr[$jobid]['act']['embell_cost']=$embell_cost_actual;
					$main_data_arr[$jobid]['act']['wash_cost']=$wash_cost_actual;
					$main_data_arr[$jobid]['act']['commission_cost']=$commission_cost_actual;
					$main_data_arr[$jobid]['act']['commercial_cost']=$comm_cost_actual;
					$main_data_arr[$jobid]['act']['freight_cost']=$freight_cost_actual;
					$main_data_arr[$jobid]['act']['test_cost']=$test_cost_actual;
					$main_data_arr[$jobid]['act']['inspection_cost']=$inspection_cost_actual;
					$main_data_arr[$jobid]['act']['currier_cost']=$currier_cost_actual;
					$main_data_arr[$jobid]['act']['total_cost']=$actual_all_cost;
					$main_data_arr[$jobid]['act']['CM_Contribution_value']=$cm_value_contribution_actual;
					$main_data_arr[$jobid]['act']['cm_cost']=$cm_cost_actual;
					$main_data_arr[$jobid]['act']['margin']=$margin;
					$main_data_arr[$jobid]['act']['discount_allowed']=$discount_allowed;
					$main_data_arr[$jobid]['act']['rmg']=$rmg_value_actual;
					$main_data_arr[$jobid]['act']['source_tax']=$source_tax_actual;
					$main_data_arr[$jobid]['act']['short_realized']=$short_realized;
					$main_data_arr[$jobid]['act']['actual_margin']=$actual_margin;
					$main_data_arr[$jobid]['act']['per_exfactory_value']=$actual_margin_perc;
				}				
				/* echo '<pre>';
				...
				foreach($main_data_arr as $job_id=>$val){
					...
					$po_id_str=implode(",",$job_wise_po[$jobid]);
					?>
					<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
						...
						<td rowspan="4" valign="middle" align="center"><? echo $brand_library[$val['brand_id']]; ?></td>
						...
						<td rowspan="4" valign="middle" align="center"><? echo implode(", ",$val['po_number']); ?></td>
						...
						<td rowspan="4" valign="middle" align="center"><? echo $season_library[$val['season']]; ?></td>
						<td rowspan="4" valign="middle" align="center"><? echo $val['season_year']; ?></td>
						...
						<td rowspan="4" valign="middle" align="center"><? echo $val['shiping_status']; ?></td>
						<td align="left"><b>Pre Costing</b></td>
						<? foreach($cost_heading_arr as $cost_head){
							?>
								<td align="right" title="<?= $pre_cost_title_arr[$cost_head] ?>"><? echo fn_number_format($val['pre'][$cost_head],2,'.',''); ?></td>
								<? 							
								$buyer_wise_cost[$val['buyer_name']]['pre'][$cost_head]+=$val['pre'][$cost_head];
								$total_summary_cost['pre'][$cost_head]+=$val['pre'][$cost_head];
							} 
						?>
					</tr>
					<tr bgcolor="#F5F5F5" onClick="change_color('tr_a<? echo $i; ?>','#F5F5F5')" id="tr_a<? echo $i; ?>">
						...
						<? foreach($cost_heading_arr as $cost_head){
							if($cost_head=='po_exfactory_qty'){
								?>
								<td align="right" title="<?= $actual_cost_title_arr[$cost_head] ?>"><a href="##" onClick="generate_ex_factory_popup('ex_factory_popup','<? echo $row[csf('job_no_prefix_num')];?>','<? echo $po_id_str; ?>','650px')"><? echo fn_number_format($val['act'][$cost_head],2,'.',''); ?></a></td>
								<? 
							}
							else if($cost_head=='fabric_purchase_cost'){
								?>
								<td align="right" title="<?= $actual_cost_title_arr[$cost_head] ?>"><a href="#report_details" onClick="openmypage_actual('<? echo $po_id_str; ?>','fabric_purchase_cost_actual2','Fabric Purchase Cost Details','1100px','<?=$company_name ; ?>')"><? echo fn_number_format($val['act'][$cost_head],2,'.',''); ?></a></td>
								<? 
							}
							else if($cost_head=='trims_cost'){
								?>
								<td align="right" title="<?= $actual_cost_title_arr[$cost_head] ?>"><a href="#report_details" onClick="openmypage_actual('<? echo $po_id_str; ?>','trims_cost_actual','Trims Cost Details','800px',<?=$company_name; ?>)"><? echo fn_number_format($val['act'][$cost_head],2,'.',''); ?></a></td>
								<? 
							}
							else if($cost_head=='embell_cost'){
								?>
								<td align="right" title="<?= $actual_cost_title_arr[$cost_head] ?>"><a href="#report_details" onClick="openmypage_actual('<? echo $po_id_str; ?>','embell_cost_actual','Embellishment Cost Details','800px')"><? echo fn_number_format($val['act'][$cost_head],2,'.',''); ?></a></td>
								<? 
							}
							else if($cost_head=='wash_cost'){
								?>
								<td align="right" title="<?= $actual_cost_title_arr[$cost_head] ?>"><a href="#report_details" onClick="openmypage_actual('<? echo $po_id_str; ?>','wash_cost_actual','Wash Cost Details','800px')"><? echo fn_number_format($val['act'][$cost_head],2,'.',''); ?></a></td>
								<? 
							}
							else if($cost_head=='cm_cost'){
								?>
								<td align="right" title="<?= $val['act'][$cost_head] ?>"><? echo fn_number_format($val['act'][$cost_head],2,'.',''); ?></td>
								<? 
							}
							else{
								?>
								<td align="right" title="<?= $actual_cost_title_arr[$cost_head] ?>"><? echo fn_number_format($val['act'][$cost_head],2,'.',''); ?></td>
								<? 
							}														
								$buyer_wise_cost[$val['buyer_name']]['act'][$cost_head]+=$val['act'][$cost_head];
								$total_summary_cost['act'][$cost_head]+=$val['act'][$cost_head];
							} 
						?>
					</tr>
					<tr bgcolor="#E9E9E9" onClick="change_color('tr_v<? echo $i; ?>','#E9E9E9')" id="tr_v<? echo $i; ?>">
						...
						<? foreach($cost_heading_arr as $cost_head){
							$variance=$val['pre'][$cost_head]-$val['act'][$cost_head];
							?>
							<td align="right" title="<?= $variance_title_arr[$cost_head] ?>"><? echo fn_number_format($variance,2,'.',''); ?></td>
						<? } ?>
					</tr>
					<tr bgcolor="#DFDFDF" onClick="change_color('tr_vp<? echo $i; ?>','#DFDFDF')" id="tr_vp<? echo $i; ?>">
						<td align="left"><b>Variance (%)</b></td>
						<? foreach($cost_heading_arr as $cost_head){
							$variance=$val['pre'][$cost_head]-$val['act'][$cost_head];
							$variance_percnet=($variance/$val['pre'][$cost_head])*100;
							?>
				...
				<tr bgcolor="#CCDDEE" onClick="change_color('tr_pt','#CCDDEE')" id="tr_pt" style="font-weight:bold;">
					...
					<td align="center"></td>
					<td align="center"></td>
					<td align="left"><b>Pre Costing Total</b></td>
					<? foreach($cost_heading_arr as $cost_head){ ?>
						<td width="100" align="right" ><? echo fn_number_format($total_summary_cost['pre'][$cost_head],2,'.',''); ?></td>
					<? } ?>
				</tr>
				<tr bgcolor="#CCCCFF" onClick="change_color('tr_at','#CCCCFF')" id="tr_at" style="font-weight:bold;">
					<? for($k=1; $k<=19; $k++){ ?>
					...
					<? foreach($cost_heading_arr as $cost_head){ ?>
						<td width="100" align="right" ><? echo fn_number_format($total_summary_cost['act'][$cost_head],2,'.',''); ?></td>
					<? } ?>
				</tr>
				<tr bgcolor="#CCCCFF" onClick="change_color('tr_vt','#CCCCFF')" id="tr_vt" style="font-weight:bold;">
					...
					<? foreach($cost_heading_arr as $cost_head){ 
						$total_variance=$total_summary_cost['pre'][$cost_head]-$total_summary_cost['act'][$cost_head];
						?>
						<td width="100" align="right" ><? echo fn_number_format($total_variance,2,'.',''); ?></td>
					<? } ?>
				</tr>
				<tr bgcolor="#FFEEFF" onClick="change_color('tr_vt','#FFEEFF')" id="tr_vt" style="font-weight:bold;">
					<? for($k=1; $k<=19; $k++){ ?>
					...
					<? foreach($cost_heading_arr as $cost_head){ 
						$total_variance=$total_summary_cost['pre'][$cost_head]-$total_summary_cost['act'][$cost_head];
						$total_variance_per=($total_variance/$total_summary_cost['pre'][$cost_head])*100;
						?>
						<td width="100" align="right" ><? echo fn_number_format($total_variance_per,2,'.',''); ?></td>
					<? } ?>
				</tr>
            </table>
		</div>
        ...
        <table width="4500">
            <tr>
                <td width="770" valign="top">
                    ...
                    <br />
					<div id="summary_full">
                        <table width="770" cellpadding="0" cellspacing="0" border="1" class="rpt_table" rules="all">
                            <thead>
                            <tr>
                                <th colspan="7">Summary</th>
                            </tr>
                            <tr>
                                ...
                                <th width="150">Pre Costing</th>
                                <th width="80">%</th>
                                <th width="150">At Actual</th>
                                <th width="80">%</th>
							...
							<? 
							$k=1;
							foreach($heading_arr as $cost_head=>$heading){
								if($cost_head!='per_exfactory_value'){
								if($k%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
								$total_po_value=$total_summary_cost['pre']['po_exfactory_value'];
								$total_exfactory_value=$total_summary_cost['act']['po_exfactory_value'];
								?>
								<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trtd_<?=$k?>','<? echo $bgcolor; ?>')" id="trtd_<?=$k?>">
									<td align="center"><?= $k ?></td>
									<td><b><?= $heading ?></b></td>
									<td align="right"><?= number_format($total_summary_cost['pre'][$cost_head],2); ?></td>
									<? if($cost_head!='po_exfactory_qty' && $cost_head!='po_exfactory_value' && $total_po_value>0){ ?>
										<td align="right"><?= number_format(($total_summary_cost['pre'][$cost_head]/$total_po_value)*100,2) ?></td>
									<? } else{ ?>
										<td>&nbsp;</td>
									<? } ?>
									<? if($cost_head=='fabric_purchase_cost'){?>
										<td align="right"><a href="#report_details" title="Exchange rate=80" onClick="openmypage('<? echo $po_ids; ?>','fabric_purchase_cost','Fabric Purchase Cost Details')"><?= number_format($total_summary_cost['act'][$cost_head],2); ?></a></td>
									<? } else{?>
										<td align="right"><?= number_format($total_summary_cost['act'][$cost_head],2); ?></td>
									<? } ?>
									<? if($cost_head!='po_exfactory_qty' && $cost_head!='po_exfactory_value' && $total_exfactory_value>0){ ?>
										<td align="right"><?= number_format(($total_summary_cost['act'][$cost_head]/$total_exfactory_value)*100,2); ?></td>
									<? } else{ ?>
										<td>&nbsp;</td>
									<? } ?>
									<td align="right"><? echo number_format($total_summary_cost['pre'][$cost_head]-$total_summary_cost['act'][$cost_head],2); ?></td>
								</tr>
								<? 
								$k++;
								}
                ...
                <td width="" valign="top">
                    ...
                    <br />
                    <div id="summary_buyer">
                        <table width="3200" cellpadding="0" cellspacing="0" border="1" class="rpt_table" rules="all">
                            <thead>
                                 <tr>
                                    <th colspan="24">Buyer Level Summary</th>
                                </tr>
                                <tr>
                                    <th width="35">SL</th>
                                    <th width="100">Buyer name</th>
                                    <th width="150">Cost Source</th>
									<?
									foreach($heading_arr as $cost_head=>$heading){
										if($cost_head!='po_exfactory_qty'){
										?>
                            ...
                            <?
                                ...
                                foreach($buyer_wise_cost as $buyer_id=>$value)
                                {
                                    if ($j%2==0)  
                                        $bgcolor="#E9F3FF";
                                    else
                                ...
                                ?>
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trTD_<? echo $j; ?>','<? echo $bgcolor; ?>')" id="trTD_<? echo $j; ?>">
                                        <td rowspan="4"><? echo $j ;?></td>
                                        <td rowspan="4"><? echo $buyer_arr[$buyer_id] ;?></td>
                                        <td><b>Pre Costing</b></td>
										<? foreach($cost_heading_arr as $cost_head){ 
											if($cost_head!='po_exfactory_qty'){
											?>
                                        	<td align="right"><? echo number_format($value['pre'][$cost_head],2);?></td>
										<? }
										} ?>
                                    </tr>
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trTD2_<? echo $j; ?>','<? echo $bgcolor; ?>')" id="trTD2_<? echo $j; ?>">
										...
										<? foreach($cost_heading_arr as $cost_head){ 
											if($cost_head!='po_exfactory_qty'){
											?>
                                        	<td align="right"><? echo number_format($value['act'][$cost_head],2);?></td>
										<? }
										} ?>                                       
                                    </tr>
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trTD3_<? echo $j; ?>','<? echo $bgcolor; ?>')" id="trTD3_<? echo $j; ?>">
										...
										<? foreach($cost_heading_arr as $cost_head){ 
											...
											<td align="right">
											<?
											$ex_var= $value['pre'][$cost_head]-$value['act'][$cost_head];
											echo number_format($ex_var,2); 
											?>
											</td>
										<? }
                                    ...
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trTD4_<? echo $j; ?>','<? echo $bgcolor; ?>')" id="trTD4_<? echo $j; ?>">
                                        ...
                                        <? foreach($cost_heading_arr as $cost_head){ 
											if($cost_head!='po_exfactory_qty'){
											$total_variance=$value['pre'][$cost_head]-$value['act'][$cost_head];
											$total_variance_per=($total_variance/$value['pre'][$cost_head])*100;
											?>
											<td width="100" align="right" ><? echo fn_number_format($total_variance_per,2,'.',''); ?></td>
										<? }
										} ?>                                       
                            ...
                            ?>
                            <tr bgcolor="<? echo $bgcolor5; ?>" onClick="change_color('trTD5_<? echo $j; ?>','<? echo $bgcolor5; ?>')" id="trTD5_<? echo $j; ?>">
                                <td colspan="3" align="right"><b>Pre Costing</b></td>
								<? foreach($cost_heading_arr as $cost_head){ 
									if($cost_head!='po_exfactory_qty'){
									?>
									<td align="right"><b><? echo number_format($total_summary_cost['pre'][$cost_head],2);?></b></td>
								<? }
								} ?>   
                            </tr>
                            <tr bgcolor="<? echo $bgcolor6; ?>" onClick="change_color('trTD6_<? echo $j; ?>','<? echo $bgcolor6; ?>')" id="trTD6_<? echo $j; ?>">
                                <td colspan="3" align="right"><b>Actual</b></td>
                                <? foreach($cost_heading_arr as $cost_head){ 
									if($cost_head!='po_exfactory_qty'){
									?>
									<td align="right"><b><? echo number_format($total_summary_cost['act'][$cost_head],2);?></b></td>
								<? }
								} ?> 
                            </tr>
                            <tr bgcolor="<? echo $bgcolor7; ?>" onClick="change_color('trTD7_<? echo $j; ?>','<? echo $bgcolor7; ?>')" id="trTD7_<? echo $j; ?>">
                                <td colspan="3" align="right"><b>Variance</b></td>
								<? foreach($cost_heading_arr as $cost_head){ 
									if($cost_head!='po_exfactory_qty'){
										$variance=$total_summary_cost['pre'][$cost_head]-$total_summary_cost['act'][$cost_head];
									?>
									<td align="right"><b><? echo number_format($variance,2);?></b></td>
								<? }
								} ?> 
                            </tr>
                            <tr bgcolor="<? echo $bgcolor6; ?>" onClick="change_color('trTD8_<? echo $j; ?>','<? echo $bgcolor6; ?>')" id="trTD8_<? echo $j; ?>">
								...
								<? foreach($cost_heading_arr as $cost_head){ 
									...
									$total_variance_per=($total_variance/$total_summary_cost['pre'][$cost_head])*100;
									?>
									<td width="100" align="right" ><b><? echo fn_number_format($total_variance_per,2,'.',''); ?></b></td>
								<? }
								} ?>                                
                            </tr>
                        </table>
