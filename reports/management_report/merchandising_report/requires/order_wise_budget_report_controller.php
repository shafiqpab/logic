...
{ 
		...
		if($template==1)
		{
			...
			foreach($result as $row)
			{
				...
				foreach($month_array as $val)
				{
					...
					$knit_cost=$conversion_costing_arr_process[$row[csf('po_id')]][1]+$conversion_costing_arr_process[$row[csf('po_id')]][2]+$conversion_costing_arr_process[$row[csf('po_id')]][3];//($kniting_cost/$dzn_qnty)*$plan_cut_qnty;
	...
	else if($report_type==2)//Budget
	{
		if($template==1)
		{
			...
			a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $date_cond $buyer_id_cond $year_cond $job_no_cond $order_id_cond_trans $order_no_cond $team_cond $team_member_cond  $order_status_cond $season_cond $internal_ref_cond $file_no_cond";
				...
				foreach($result as $row )
                {
					...
					if(str_replace("'","",$cbo_search_date)==1)
					{
						...
						foreach($country_id_all as $id_country)
						{
							...
							$fab_purchase_knit=$fabric_costing_arr['knit']['grey'][$row[csf('po_id')]][$id_country];
						    $fab_purchase_woven=$fabric_costing_arr['woven']['grey'][$row[csf('po_id')]][$id_country];
						    $fab_purchase=$fab_purchase_knit+$fab_purchase_woven;
							
							$conversion_cost+=$conversion_costing_arr[$row[csf('po_id')]][$id_country];
							...
							$knit_cost+=$conversion_costing_arr_process[$row[csf('po_id')]][$id_country][1]+$conversion_costing_arr_process[$row[csf('po_id')]][$id_country][2]+$conversion_costing_arr_process[$row[csf('po_id')]][$id_country][3];
							foreach($fabric_dyeingCost_arr as $dye_id)
							{
								$fabric_dyeing_cost+=$conversion_costing_arr_process[$row[csf('po_id')]][$id_country][$dye_id];
							}
							
							$yarn_dyed_cost+=$conversion_costing_arr_process[$row[csf('po_id')]][$id_country][30];
							foreach ($aop_cost_arr as $aop_id)
							{
								$all_over_cost+=$conversion_costing_arr_process[$row[csf('po_id')]][$id_country][$aop_id];
							}
							
							$heat_setting_cost+=$conversion_costing_arr_process[$row[csf('po_id')]][$id_country][33]; 
							foreach($fab_finish_cost_arr as $fab_fin_id)
							{
								$fabric_finish+=$conversion_costing_arr_process[$row[csf('po_id')]][$id_country][$fab_fin_id];
							}
							foreach($washing_cost_arr as $washing_id)
							{
								$washing_cost+=$conversion_costing_arr_process[$row[csf('po_id')]][$id_country][$washing_id];
							}
					...
					else
					{
						...
						$conversion_cost=$conversion_costing_arr[$row[csf('po_id')]];
						...
						$knit_cost=$conversion_costing_arr_process[$row[csf('po_id')]][1]+$conversion_costing_arr_process[$row[csf('po_id')]][2]+$conversion_costing_arr_process[$row[csf('po_id')]][3];
						$fabric_dyeing_cost=0;
						foreach($fabric_dyeingCost_arr as $dye_id)
						{
	...
	else if($report_type==9) //PQ Vs Budget Wvn
	{
		if($template==1)
		{
			ob_start();
	...
	else if($report_type==7)//Budget 2
	{
		if($template==1)
		{
			...
			from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no and c.job_no=b.job_no_mst and a.company_name='$company_name' and c.entry_from=111 and a.status_active=1 and
		    ...
		    $order_status_cond $season_cond $internal_ref_cond $file_no_cond $team_cond $team_member_cond";
				...
				?>
				 <tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
                     ...
                     <? 
						...
						foreach($aop_cost_arr as $aop_process_id)
						{
							$all_over_cost+=$conversion_costing_arr_process[$row[csf('po_id')]][$aop_process_id];//[$row[csf('order_uom')]];	
						}
	...
	else if($report_type==8) //Spot Cost Vs Budget
	{
		...
		<script>
				...
				//var p_dyefin_val=(parseFloat(document.getElementById('p_fabric_dyeing_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('p_fabric_finishing_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('p_heat_setting_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('p_washing_td').innerHTML.split(',').join('')));
				var p_dyefin_val=(parseFloat(document.getElementById('p_fabric_dyeing_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('p_fabric_finishing_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('p_heat_setting_td').innerHTML.split(',').join('')));
				//var dyefin_val=(parseFloat(document.getElementById('fabric_dyeing_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('fabric_finishing_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('heat_setting_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('washing_td').innerHTML.split(',').join('')));
				var dyefin_val=(parseFloat(document.getElementById('fabric_dyeing_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('fabric_finishing_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('heat_setting_td').innerHTML.split(',').join('')));
				//var v_dyefin_val=(parseFloat(document.getElementById('v_fabric_dyeing_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('v_fabric_finishing_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('v_heat_setting_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('v_washing_td').innerHTML.split(',').join('')));
				var v_dyefin_val=(parseFloat(document.getElementById('v_fabric_dyeing_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('v_fabric_finishing_td').innerHTML.split(',').join(''))) + (parseFloat(document.getElementById('v_heat_setting_td').innerHTML.split(',').join('')));
				document.getElementById('dyeFin_qut_cost').innerHTML=number_format_common(p_dyefin_val,2);
				document.getElementById('dyeFin_qut_per').innerHTML=number_format_common((p_dyefin_val/order_value)*100,2);
				document.getElementById('dyeFin_bud_cost').innerHTML=number_format_common(dyefin_val,2);
				document.getElementById('dyeFin_bud_per').innerHTML=number_format_common((dyefin_val/order_value)*100,2);
