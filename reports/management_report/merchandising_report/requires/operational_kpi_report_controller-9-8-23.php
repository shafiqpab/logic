  ...
  $sql_po_plan_current="SELECT a.company_name as COMPANY_ID,b.id as POID, c.PLAN_QTY,c.PLAN_DATE, b.job_no_mst as JOB_NO from wo_po_details_master a,wo_po_break_down b, tna_plan_target c,gbl_temp_engine g where a.id=b.job_id and b.id=c.po_id and c.task_id=48   and c.task_type=1 and c.plan_qty>0 and b.id=g.ref_val and c.po_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=89 and  b.status_active=1 and b.is_deleted=0  and  c.status_active=1 and c.is_deleted=0 $company_conds and c.PLAN_DATE <= '$endDate' order by c.PLAN_DATE asc"; //and b.id in(20325,20326,20327,20328,20329,20330)
	...
	foreach ($sql_po_allocate_result as $val) 
	{
	 ...
	 fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 89, 2, $allocat_poIdArr, $empty_arr);//PO ID Ref from=2
	  ...
	  //and TRUNC(insert_date)<=TO_DATE('16-JAN-2023')
	 ...
	 //and b.id in(20325,20326,20327,20328,20329,20330)
	  ...
	  foreach ($sql_po_plan_result_ashulia as $val) 
	  {
		  ...
		  if($val['COMPANY_ID']==2) //Kal Ashulia
		  {
			...
			{
				$ashulia_plan_qty_array[3][$plandate][$val['POID']][$val['COMPANY_ID']]['qc_planQty']+=$val['QC_PLAN_QTY'];
			}
			if($val['INPUT_PLAN_QTY']>0 && $val['TASK_ID']==122 && $val['REF_FROM']==4) //Input
			{
				$ashulia_in_plan_qty_array[3][$plandate][$val['POID']][$val['COMPANY_ID']]['input_planQty']+=$val['INPUT_PLAN_QTY'];
			}
			if($val['OUT_PLAN_QTY']>0 && $val['TASK_ID']==86 && $val['REF_FROM']==5) //Output
			{
				$ashulia_out_plan_qty_array[3][$plandate][$val['POID']][$val['COMPANY_ID']]['out_planQty']+=$val['OUT_PLAN_QTY'];
			}
			if($val['FIN_PLAN_QTY']>0 && $val['TASK_ID']==88 && $val['REF_FROM']==6) //Fin
			{
				$ashulia_gFin_plan_qty_array[3][$plandate][$val['POID']][$val['COMPANY_ID']]['fin_planQty']+=$val['FIN_PLAN_QTY'];
			}
		 }
	   ...
	   //and b.id in(20325,20326,20327,20328,20329,20330)
		...
		foreach ($sql_po_result_prod_curr as $val) 
		{
			if($val['COMPANY_ID']==2) //Kal Ashulia 
			{
				...
				{
					$ashulia_plan_qty_array[3][$propddate][$val['POID']][$val['COMPANY_ID']]['prod_qc']+=$val['PROD_QC'];
				}
				if($val['PRODUCTION_TYPE']==4)
				{
					$ashulia_in_plan_qty_array[3][$propddate][$val['POID']][$val['COMPANY_ID']]['prod_in']+=$val['INPUT_PROD'];
				}
				if($val['PRODUCTION_TYPE']==5)
				{
					$ashulia_out_plan_qty_array[3][$propddate][$val['POID']][$val['COMPANY_ID']]['prod_out']+=$val['OUT_PROD'];
				}
				if($val['PRODUCTION_TYPE']==8)
				{
					$ashulia_gFin_plan_qty_array[3][$propddate][$val['POID']][$val['COMPANY_ID']]['prod_fin']+=$val['FIN_PROD'];
				}
		    }
				...
				//	$company_kip_cal_Arr=array();//$po_planQtyArr=array();
						...
						//{
							for($j=0;$j<$diff_days;$j++)
							{
								...
								//==========Qc Prod and Qc Plan==============
								$planQcQtycal=0;
								$planQcQtycal=$ashulia_plan_qty_array[3][$day_all][$poid][$comp_id]['qc_planQty'];//alloQty
								...
								//==========In Prod and In Plan==============
								$planInQtycal=0;
								$planInQtycal=$ashulia_in_plan_qty_array[3][$day_all][$poid][$comp_id]['input_planQty'];//alloQty
								//$ashulia_plan_qty_array[3][$plandate][$val['POID']][$val['COMPANY_ID']]['qc_planQty']
								...
								$planOutQtycal=0;
								$planOutQtycal=$ashulia_out_plan_qty_array[3][$day_all][$poid][$comp_id]['out_planQty'];//alloQty
								//$ashulia_plan_qty_array[3][$plandate][$val['POID']][$val['COMPANY_ID']]['qc_planQty']
								//po_prodOutQty_arr
								if($planOutQtycal>0)
								{
								...
								$planFinQtycal=$ashulia_gFin_plan_qty_array[3][$day_all][$poid][$comp_id]['fin_planQty'];
								//$ashulia_plan_qty_array[3][$plandate][$val['POID']][$val['COMPANY_ID']]['qc_planQty']
								//po_prodOutQty_arr
								if($planFinQtycal>0)
								{
			   ...
			   $qc_month_kpi_per=$qc_month_prod/$qc_month_plan*100; 
				...
				if($day_chk<=$today)// as on today
				{
					...
					if($qc_month_prod>0 && $qc_month_plan>0)
					{
						...
						// echo  $monthYr.'='.$month_allo.'='. $month_plan.'*100<br>';
					$ash_qc_month_wise_kpiArr[$comp_id][3][$yr_month]+=$qc_month_kpi_per;
					$ash_month_wise_kpiArr[$comp_id][3][$day_key]['qc']+=$qc_month_kpi_per;
					}
					if($qc_month_plan>0)
					{
						$mon_grand_qc_event_kpi_perArr[$comp_id][3][$day_key]=1;

						$ash_num_of_plan_days[$comp_id][3][$day_key]++;
						$gbl_num_of_plan_daysArr[$comp_id][3][$yr_month]++;
					}
				}
		   }
	   }
	   ...
	   //========****************===============
				...
				$monthYr=date("M-Y",strtotime($day_key));
				 ...
				 if($day_chk<=$today)// as on today
				 {
					 ...
					 if($in_month_prod>0 && $in_month_plan>0)
					 {
						 ...
						 // echo  $monthYr.'='.$month_allo.'='. $month_plan.'*100<br>';
					 $ash_in_month_wise_kpiArr[$comp_id][3][$yr_month]+=$in_month_kpi_per;
					 $ash_month_wise_kpiArr[$comp_id][3][$day_key]['in']+=$in_month_kpi_per;
					 }
					$qc_plan_day_chk= $qc_month_wise_kpiArr[$comp_id][$day_key]['qc_plan'];
					 if($in_month_plan>0)
					 {
						 $ash_in_num_of_plan_days[$comp_id][3][$yr_month]++;
						 $mon_grand_in_event_kpi_perArr[$comp_id][3][$day_key]=1;
					 }
		...
		foreach($ash_in_month_wise_kpiArr as $comp_id=>$comData) //Month Wise KPI Percentage summary part
		{
			 foreach($comData as $unit_id=>$monData)  
			 {
				 foreach($monData as $monYr=>$per)  
				 {
					 $ash_in_comp_avg_perArr[$comp_id][$unit_id]+=$per/$ash_in_num_of_plan_days[$comp_id][3][$monYr];
				 }
			 }
		}
				...
				$out_month_kpi_per=$out_month_prod/$out_month_plan*100; 
				 ...
				 if($day_chk<=$today)// as on today
				 {
					 ...
					 if($out_month_prod>0 && $out_month_plan>0)
					 {
						 ...
						 // echo  $monthYr.'='.$month_allo.'='. $month_plan.'*100<br>';
					 $ash_out_month_wise_kpiArr[$comp_id][3][$yr_month]+=$out_month_kpi_per;

					 $ash_month_wise_kpiArr[$comp_id][3][$day_key]['out']+=$out_month_kpi_per;
					 }
					 $qc_plan_day_chk= $qc_month_wise_kpiArr[$comp_id][$day_key]['qc_plan'];
					 ...
					 if($out_month_plan>0)
					 {
						 $ash_out_num_of_plan_days[$comp_id][3][$yr_month]++;
						 $mon_grand_out_event_kpi_perArr[$comp_id][3][$day_key]=1;
					 }
		...
		foreach($ash_out_month_wise_kpiArr as $comp_id=>$comData) //Month Wise KPI Percentage summary part
		{
			 foreach($comData as $unit_id=>$monData)  
			 {
				 foreach($monData as $monYr=>$per)  
				 {
					 $ash_out_comp_avg_perArr[$comp_id][$unit_id]+=$per/$ash_out_num_of_plan_days[$comp_id][3][$monYr];
				 }
			 }
		}
		...
		$fin_month_kpi_per=$fin_month_prod/$fin_month_plan*100; 
		 ...
		 if($day_chk<=$today)// as on today
		 {
			 ...
			 if($fin_month_prod>0 && $fin_month_plan>0)
			 {
				 ...
				 // echo  $monthYr.'='.$month_allo.'='. $month_plan.'*100<br>';
			 $ash_fin_month_wise_kpiArr[$comp_id][3][$yr_month]+=$fin_month_kpi_per;
			 $ash_month_wise_kpiArr[$comp_id][3][$day_key]['fin']+=$fin_month_kpi_per;
			 }
			 ...
			 if($fin_month_plan>0)
			 {
				 $ash_fin_num_of_plan_days[$comp_id][3][$yr_month]++;
				 $mon_grand_fin_event_kpi_perArr[$comp_id][3][$day_key]=1;
			 }
...
//gbl_num_of_plan_daysArr
			$event_qc_count=$mon_grand_qc_event_kpi_perArr[$comp_id][3][$day_key];//$mon_grand_qc_event_kpi_perArr
			$event_in_count=$mon_grand_in_event_kpi_perArr[$comp_id][3][$day_key];
			$event_out_count=$mon_grand_out_event_kpi_perArr[$comp_id][3][$day_key];
			$event_fin_count=$mon_grand_fin_event_kpi_perArr[$comp_id][3][$day_key];
						...
						<? }
						  ...
						  <? }
							...
							if($unit_id!=6)  //ash_comp_avg_perArr
							{
								...
								{
								$Ash_com_avg_per=$ash_comp_avg_perArr[$com_id][3]+$ash_in_comp_avg_perArr[$com_id][3]+$ash_out_comp_avg_perArr[$com_id][3]+$ash_fin_comp_avg_perArr[$com_id][3];
							...
							foreach ($fiscalMonth_arr as $year_mon => $val) 
							{
								...
								if($com_id==2 && $unit_id==3)
								{
									...
									$qc_month_wise_kpi_per =$ash_qc_month_wise_kpiArr[$com_id][3][$year_mon];
									$kpi_for_day=0;
									...
									$in_month_wise_kpi_per =$ash_in_month_wise_kpiArr[$com_id][3][$year_mon];
									...
									$out_month_wise_kpi_per =$ash_out_month_wise_kpiArr[$com_id][3][$year_mon];
									...
									$fin_month_wise_kpi_per =$ash_fin_month_wise_kpiArr[$com_id][3][$year_mon];
