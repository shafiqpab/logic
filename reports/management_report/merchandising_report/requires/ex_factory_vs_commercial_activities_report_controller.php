...
//echo load_html_head_contents("Ex-Factory vs Commercial Activities", "../../../../", "",1, $unicode,"", 1);
echo load_html_head_contents("Ex-Factory vs Commercial Activities", "../../../", "", '', '', '', $amchart);//this $amchart is not wark in this page


if ($action == "load_drop_down_location"){
	echo create_drop_down("cbo_location_name", 172, "select id,location_name from lib_location where company_id='$data' and status_active =1 and is_deleted=0 order by location_name", "id,location_name", 1, "-- Select Location --", 0, "load_drop_down( 'requires/ex_factory_vs_commercial_activities_report_controller',$('#cbo_company_name').val()+'_'+$('#cbo_knitting_company').val()+'_'+this.value, 'load_drop_down_floor', 'cbo_del_floor' );");
exit();
}
...
if ($action == "load_drop_down_floor") {
	...
	list($lc_company,$working_company,$location_id)= explode("_", $data);
	
	$company_id=($lc_company!=0)?$lc_company:$working_company;
...
{ 
	...
	$cbo_del_floor=str_replace("'","",$cbo_del_floor);
	$cbo_year=str_replace("'","",$cbo_year);
	
	
	$company_name=($cbo_company_name !=0)?$cbo_company_name:$cbo_knitting_company;
	...
	ob_start();
	?>
	<style> hr{ color:#666; } </style>
    <div align="center"> <h3 align="center"> Ex-Factory vs Commercial Activities </h3></div>
    <div align="center">
    ...
    <div align="center" style="width:1060px; height:auto; margin-left:40px;">
        ...
        foreach( $month_wise_sum_arr as $month_id=>$buyer_arr)
        {
            ...
            if($kh==1)
			{
				...
				<h3 class="accordion_h" align="center"> Month Wise Summary </h3>
				</div>
            	<?
			}
			?>
