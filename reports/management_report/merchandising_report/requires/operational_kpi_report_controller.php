...
////====================Plan Task ID=No plan Task ID==============
	...
	//======Actual PO for Ashulia N Ratabpur======================
	  ...
	  from wo_po_details_master a,wo_po_break_down b, pro_garments_production_mst c where a.id=b.job_id and b.id=c.po_break_down_id  and c.production_type in(1,4,5,8) and a.location_name in(3,5)  and c.PRODUCTION_QUANTITY>0 and  b.status_active=1 and b.is_deleted=0  and  c.status_active=1 and c.is_deleted=0 and b.id in(20328,20329,20330) and c.production_date between '$startDate' and '$endDate' $company_conds order by c.PRODUCTION_DATE asc"; //and a.location_name=3
		...
		//===============Plan Task ID=84,122,86,88========================
		//==============For Ratanpur Sample Plan Task Id=84,122,86,88=============
 ...
 ///================Plan Task ID=No Task ID for all actual PO =====================
	   ...
	   and d.quantity>0 and b.status_active=1 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0 
		 and b.id in(20328,20329,20330,20334,20335,20337) and c.transaction_type=1   and d.entry_form in(2,22) and c.transaction_date between '$startDate' and '$endDate' $company_conds order by c.transaction_date asc"; //and b.id in(20325,20326,20327,20328,20329,20330)
		  ...
		  and d.quantity>0 and b.status_active=1 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0 
			 ...
			 and c.BATCH_QNTY>0 and b.status_active=1 and d.load_unload_id=2 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0 
			   and b.id in(20328,20329,20330,20334,20335,20337) and d.batch_ext_no is null and d.entry_form in(35) and d.process_end_date between '$startDate' and '$endDate' $company_conds order by d.process_end_date asc"; //and b.id in(20325,20326,20327,20328,20329,20330)
				...
				fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 89, 31, $kal_fab_aop_poIdArr, $empty_arr);//PO ID Ref from=30
						//==========================Plan Task Id=267===========//
				$sql_fab_aop_kal_qty="SELECT a.company_name as COMPANY_ID,a.LOCATION_NAME,b.id as POID,b.GROUPING,d.BATCH_ISSUE_QTY,c.RECEIVE_DATE, b.job_no_mst as JOB_NO from wo_po_details_master a,wo_po_break_down b,
				...
				and d.BATCH_ISSUE_QTY>0 and b.status_active=1 and c.entry_form=92 and d.process_id=35 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0 
					...
					$sql_fab_aop_kal_result_qty = sql_select($sql_fab_aop_kal_qty); 
					foreach($sql_fab_aop_kal_result_qty  as $val)
					{
		   ...
		   //============Plan Task Id=73,Source Id=1 and Uom id=12
					...
					if($unit_id==5)
					{
						...
						for($j=0;$j<$diff_days;$j++)
						{
							...
							foreach($mar_plan_qty_array[5][$mar_newdate] as $poid_y=>$poData)
							{
								...
								//echo $mar_newdate.'='.$mar_alloQty.'<br>';
								if($mar_planQty>0)
								{
...
//=============**************MKT TL Wise************==================================
if($action=="report_generate_mkt_tl_wise")
{
	...
	// ========================= for Plan ======================
		 //============************************MKT(Marchandising) TL Wise *******************************************
		
	$con = connect();
 	execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id."  and ENTRY_FORM=89");
		 
	  $sql_po_plan="SELECT a.company_name as COMPANY_ID,a.TEAM_LEADER,a.DEALING_MARCHANT,a.LOCATION_NAME,b.id as POID,b.GROUPING,c.TASK_ID, c.PLAN_QTY,c.PLAN_DATE, c.SOURCE_ID,c.UOM_ID,b.job_no_mst as JOB_NO from wo_po_details_master a,wo_po_break_down b, tna_plan_target c where a.id=b.job_id and b.id=c.po_id and c.task_id in(48,73,70,71)   and c.task_type=1 and c.plan_qty>0 and  b.status_active=1 and b.is_deleted=0 and b.id in(20328,20329,20330,20334,20335,20337)   and c.PLAN_DATE between '$startDate' and '$endDate' $company_conds order by c.PLAN_DATE asc"; //and b.id in(20325,20326,20327,20328,20329,20330)
	$sql_po_plan_result = sql_select($sql_po_plan);
	 foreach ($sql_po_plan_result as $val) 
	 {
		  ////============***********Marchandising ********===============
		  $poWiseTLnMemberArr[$val['COMPANY_ID']][$val['POID']]['tl']=$val['TEAM_LEADER'];
		  $poWiseTLnMemberArr[$val['COMPANY_ID']][$val['POID']]['member']=$val['DEALING_MARCHANT'];
  ...
  $sql_po_plan_current="SELECT a.company_name as COMPANY_ID,b.id as POID, c.PLAN_QTY,c.PLAN_DATE, b.job_no_mst as JOB_NO from wo_po_details_master a,wo_po_break_down b, tna_plan_target c,gbl_temp_engine g where a.id=b.job_id and b.id=c.po_id and c.task_id=48   and c.task_type=1 and c.plan_qty>0 and b.id=g.ref_val and c.po_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=89 and  b.status_active=1 and b.is_deleted=0  and  c.status_active=1 and c.is_deleted=0 $company_conds and c.PLAN_DATE <= '$endDate' order by c.PLAN_DATE asc"; //and b.id in(20325,20326,20327,20328,20329,20330)
	...
	// fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 89, 2, $allocat_poIdArr, $empty_arr);//PO ID Ref from=2
     ...
     $curr_sql_po_allocate="SELECT a.company_name as COMPANY_ID,a.TEAM_LEADER,a.DEALING_MARCHANT,b.id as POID, c.QNTY,c.allocation_date as ALLOC_DATE, b.job_no_mst as JOB_NO,g.REF_FROM from wo_po_details_master a,wo_po_break_down b, inv_material_allocat_hystory c,gbl_temp_engine g  where a.id=b.job_id and b.id=c.po_break_down_id and b.id=g.ref_val and c.po_break_down_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in(44) and g.entry_form=89  and c.allocation_date <= '$endDate'  and  b.status_active=1 and b.is_deleted=0    $company_conds order by c.allocation_date,c.id asc";
...
fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 89, 36, $plan_trim_fin_mar_poIdArr, $empty_arr);//PO ID Ref from=26
 ...
 $sql_po_plan_trim="SELECT a.company_name as COMPANY_ID,a.TEAM_LEADER,a.DEALING_MARCHANT,a.LOCATION_NAME,b.id as POID,c.PLAN_DATE,c.TASK_ID,c.SOURCE_ID,c.UOM_ID,g.REF_FROM, b.job_no_mst as JOB_NO,
(case when  c.task_id=73 and c.source_id=2 and g.ref_from=34  then c.PLAN_QTY else 0 end) as MAR_FIN_PLAN_QTY,
(case when  c.task_id=70   and g.ref_from=35  then c.PLAN_QTY else 0 end) as MAR_TRIM_SEW_PLAN_QTY,
(case when  c.task_id=71  and g.ref_from=36  then c.PLAN_QTY else 0 end) as MAR_TRIM_FIN_PLAN_QTY

from wo_po_details_master a,wo_po_break_down b, tna_plan_target c,gbl_temp_engine g where a.id=b.job_id and b.id=c.po_id    and c.task_type=1 and c.plan_qty>0 and b.id=g.ref_val and c.po_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in(34,35,36) and c.task_id in(73,70,71) and g.entry_form=89  and  c.status_active=1 and c.is_deleted=0 and  b.status_active=1 and b.is_deleted=0 $company_conds and c.PLAN_DATE <= '$endDate' order by c.PLAN_DATE asc";
//and b.id in(20325,20326,20327,20328,20329,20330)
 $sql_po_plan_result_trim = sql_select($sql_po_plan_trim);
 foreach ($sql_po_plan_result_trim as $val) 
 {
	$poWiseTLnMemberArr[$val['COMPANY_ID']][$val['POID']]['tl']=$val['TEAM_LEADER'];
   $poWiseTLnMemberArr[$val['COMPANY_ID']][$val['POID']]['member']=$val['DEALING_MARCHANT'];
...
//	=======JM Marchandising================
 ...
 //echo "<pre>";
 // print_r($mar_fin_plan_recv_qty_array);
 // ======================Gmts Fin  Recv production=================
   $sql_mar_fin_gmts_kal="SELECT a.company_name as COMPANY_ID,a.TEAM_LEADER,a.DEALING_MARCHANT,a.LOCATION_NAME,b.id as POID,b.GROUPING,d.QUANTITY, b.job_no_mst as JOB_NO from wo_po_details_master a,wo_po_break_down b,
 inv_transaction c,order_wise_pro_details d  where a.id=b.job_id  and c.id=d.trans_id and b.id=d.po_breakdown_id
 and d.quantity>0 and b.status_active=1 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0 
   and b.id in(20328,20329,20330,20334,20335,20337) and c.transaction_type=1   and d.entry_form in(37) and c.transaction_date between '$startDate' and '$endDate' $company_conds order by c.transaction_date asc"; //and b.id in(20325,20326,20327,20328,20329,20330)
	...
	and d.quantity>0 and b.status_active=1 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0 
	  ...
	  //and b.id in(20325,20326,20327,20328,20329,20330)
	   ...
	   ////================Trims Sewing n Fin===================inv_trims_entry_dtls lib_item_group
	$sql_mar_trim_recv="SELECT a.company_name as COMPANY_ID,a.TEAM_LEADER,a.DEALING_MARCHANT,a.LOCATION_NAME,b.id as POID,b.GROUPING,d.QUANTITY, b.job_no_mst as JOB_NO,f.trim_type as TRIM_TYPE from wo_po_details_master a,wo_po_break_down b,
 inv_transaction c,order_wise_pro_details d,inv_trims_entry_dtls e,lib_item_group f  where a.id=b.job_id  and c.id=d.trans_id and b.id=d.po_breakdown_id and c.id=e.trans_id and e.id=d.dtls_id  and f.id=e.item_group_id
 and d.quantity>0 and f.status_active=1 and f.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0  and  e.status_active=1 and e.is_deleted=0  and  d.status_active=1 and d.is_deleted=0 
   and b.id in(20328,20329,20330,20334,20335,20337) and c.transaction_type=1   and d.entry_form in(24) and c.transaction_date between '$startDate' and '$endDate' $company_conds order by c.transaction_date asc";  //and b.id in(20325,20326,20327,20328,20329,20330)
 ...
 and d.quantity>0 and f.status_active=1 and f.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0  and  e.status_active=1 and e.is_deleted=0  and  d.status_active=1 and d.is_deleted=0 
   and b.id in(20328,20329,20330,20334,20335,20337) and c.transaction_type=1   and d.entry_form in(24) and c.transaction_date <= '$endDate' $company_conds order by c.transaction_date asc";
	 ...
	 //=============Actula Po========
	 $sql_actual_po_plan_jm="SELECT c.id as acul_poid,a.company_name as COMPANY_ID,a.TEAM_LEADER,a.DEALING_MARCHANT,a.LOCATION_NAME,b.id as POID,b.GROUPING, c.acc_po_qty as PLAN_QTY,c.ACC_SHIP_DATE as PLAN_DATE, b.job_no_mst as JOB_NO from wo_po_details_master a,wo_po_break_down b, wo_po_acc_po_info c where a.id=b.job_id and b.id=c.po_break_down_id    and c.acc_po_qty>0 and  b.status_active=1 and b.is_deleted=0   and b.id in(20328,20329,20330,20334,20335,20337) and c.acc_ship_date between '$startDate' and '$endDate' $company_conds  order by acul_poid, PLAN_DATE asc"; //and b.id in(20325,20326,20327,20328,20329,20330)
	$sql_actual_po_plan_result_jm = sql_select($sql_actual_po_plan_jm);
	foreach($sql_actual_po_plan_result_jm  as $val)
	{
	$poWiseTLnMemberArr[$val['COMPANY_ID']][$val['POID']]['tl']=$val['TEAM_LEADER'];
	...
	// for actual po ship qty
	$sql_actual_qty_po_plan_jm="SELECT c.id as ACUL_POID,a.company_name as COMPANY_ID,a.LOCATION_NAME,b.id as POID,b.GROUPING,c.ACC_PO_NO, c.acc_po_qty as PLAN_QTY,c.ACC_SHIP_DATE as PLAN_DATE, b.job_no_mst as JOB_NO,g.REF_FROM from wo_po_details_master a,wo_po_break_down b, wo_po_acc_po_info c, gbl_temp_engine g where a.id=b.job_id and b.id=c.po_break_down_id and c.acc_po_qty>0 and b.id=g.ref_val and c.po_break_down_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in(40) and g.entry_form=89 and  b.status_active=1 and b.is_deleted=0  and b.id in(20328,20329,20330,20334,20335,20337)  and c.ACC_SHIP_DATE <= '$endDate' $company_conds order by  PLAN_DATE asc"; //and b.id in(20325,20326,20327,20328,20329,20330)
	$sql_actual_po_qty_plan_result_jm = sql_select($sql_actual_qty_po_plan_jm);
	foreach($sql_actual_po_qty_plan_result_jm  as $val)
	{
		$plandate=date('d-M-Y',strtotime($val['PLAN_DATE']));
	...
	$sql_actual_po_delivery_jm="SELECT a.company_name as COMPANY_ID,a.TEAM_LEADER,a.DEALING_MARCHANT,a.LOCATION_NAME,b.id as POID,b.GROUPING, b.job_no_mst as JOB_NO from wo_po_details_master a,wo_po_break_down b,
 pro_ex_factory_mst c,pro_ex_factory_actual_po_details d  where a.id=b.job_id and b.id=c.po_break_down_id
  and c.id=d.mst_id   and d.ex_fact_qty>0 and b.status_active=1 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0 
  and b.id in(20328,20329,20330,20334,20335,20337) and c.ex_factory_date between '$startDate' and '$endDate' $company_conds  order by c.ex_factory_date asc"; //and b.id in(20325,20326,20327,20328,20329,20330) // and a.location_name=6  and a.company_name=1
	$sql_actual_po_delivery_result_jm = sql_select($sql_actual_po_delivery_jm);
	...
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 89, 41, $mar_plan_acl_poIdArr, $empty_arr);//PO ID Ref from=41

	 $sql_actual_po_qty_delivery_jm="SELECT d.actual_po_id as ACTUAL_PO_ID,a.company_name as COMPANY_ID,a.LOCATION_NAME,b.id as POID,b.GROUPING, d.ex_fact_qty as DEL_QTY,c.ex_factory_date as EX_DATE, b.job_no_mst as JOB_NO,g.REF_FROM from wo_po_details_master a,wo_po_break_down b, pro_ex_factory_mst c,pro_ex_factory_actual_po_details d,gbl_temp_engine g where a.id=b.job_id and b.id=c.po_break_down_id  and c.id=d.mst_id and b.id=g.ref_val and c.po_break_down_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in(41) and g.entry_form=89  and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0  and d.ex_fact_qty>0 and  b.status_active=1 and b.is_deleted=0   and b.id in(20328,20329,20330,20334,20335,20337) and c.ex_factory_date<= '$endDate' $company_conds  order by c.ex_factory_date asc"; //and b.id in(20325,20326,20327,20328,20329,20330) //and a.location_name=6 
	$sql_actual_po_delivery_qty_result_jm = sql_select($sql_actual_po_qty_delivery_jm);
	foreach ($sql_actual_po_delivery_qty_result_jm as $val) 
	{
		$ex_date=date('d-M-Y',strtotime($val['EX_DATE']));
 ...
 // =============================Calculation of KPI============= //plan asy  allocatio ni  day dorbo
	...
	$avg_company_kip_perArr=array();$tot_avg_kpiArr=array();
	foreach ($company_wise_arr as   $com_id) 
	{
			foreach ($fiscalMonth_arr as $year_mon => $val) 
			{
			 ...
			 foreach ($fiscalMonth_arr as $year_mon => $val) 
			{
				...
				$diff_days=datediff('d',$from_date,$to_date);
					...
					//{
						...
						$tl_name=$poWiseTLnMemberArr[$comp_id][$poid]['tl'];
						$member=$poWiseTLnMemberArr[$comp_id][$poid]['member'];
						//echo $tl_name.'='.$member.'<br>';
						...
						for($j=0;$j<$diff_days;$j++)
						{
							...
							{
								$yarn_mon_alloc_qty=$mar_po_alloQty_arr[$day_all][$poid];
								$mar_yarn_month_wise_kpiArr[$comp_id][$tl_name][$member][$day_all]['yarn_allo']+=$yarn_mon_alloc_qty;
							}
							$mar_yarn_month_wise_kpiArr[$comp_id][$tl_name][$member][$day_all]['plan']+=$mar_po_planQty_arr[$day_all][$poid];
							//===========================Fin Fabric Production  and Plan=============
							$fin_fab_planQtycal=0;
							$fin_fab_planQtycal=$mar_fin_plan_recv_qty_array[5][$day_all][$poid][$comp_id]['plan'];//alloQty
							if($fin_fab_planQtycal>0)
							{
								$fin_mon_prod_qty=$mar_fin_fab_po_Qty_arr[$day_all][$poid];
								//echo $day_all.'X'.$fin_mon_prod_qty.'<br>';
								$mar_fin_month_wise_kpiArr[$comp_id][$tl_name][$member][$day_all]['fin_recv']+=$fin_mon_prod_qty;
							}
							$mar_fin_month_wise_kpiArr[$comp_id][$tl_name][$member][$day_all]['plan']+=$mar_fin_po_planQty_arr[$day_all][$poid];
							...
							{
								$sew_mon_prod_qty=$mar_trim_sew_po_Qty_arr[$day_all][$poid];
								$mar_trim_sew_month_wise_kpiArr[$comp_id][$tl_name][$member][$day_all]['trim_recv']+=$sew_mon_prod_qty;
							}
							$mar_trim_sew_month_wise_kpiArr[$comp_id][$tl_name][$member][$day_all]['plan']+=$mar_trim_sew_po_planQty_arr[$day_all][$poid];
							...
							{
								$fin_mon_prod_qty=$mar_trim_fin_po_Qty_arr[$day_all][$poid];
								$mar_trim_fin_month_wise_kpiArr[$comp_id][$tl_name][$member][$day_all]['trim_recv']+=$fin_mon_prod_qty;
							}
							$mar_trim_fin_month_wise_kpiArr[$comp_id][$tl_name][$member][$day_all]['plan']+=$mar_trim_fin_po_planQty_arr[$day_all][$poid];
...
foreach($mar_yarn_month_wise_kpiArr as $comp_id=>$comData) //Month Wise KPI Percentage summary part
{
	foreach($comData as $team_id=>$TeamData)   
	{
	foreach($TeamData as $member_id=>$memberData)  
	{
	 foreach($memberData as $day_key=>$row)
	 {
		   $mar_yarn_month_prod=$row['yarn_allo']; 
		   $mar_yarn_month_plan=$row['plan'];
		   $yarn_month_kpi_per=0;
			$yarn_month_kpi_per=$mar_yarn_month_prod/$mar_yarn_month_plan*100; 
			 ...
			 if($day_chk<=$today)// as on today
			 {
				 ...
				 {
					 if($yarn_month_kpi_per>100)  $yarn_month_kpi_per=100;
				 $mar_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['yarn']+=$yarn_month_kpi_per;
				 }
			 
				 if($mar_yarn_month_plan>0)
				 {
					 $mar_mon_grand_yarn_event_kpi_perArr[$comp_id][$team_id][$member_id][$day_key]=1;
				 }

				 //if($knit_month_plan>0 && !$yarn_plan_day_chk)
				 if($mar_yarn_month_plan>0)
				 {
					 $mar_gbl_num_of_plan_daysArr[$comp_id][$team_id][$member_id][$yr_month]++;
				 }
			 }
		  }
		}
 ...
 //echo "<pre>";
 //print_r($mar_month_wise_kpiArr);die;
		//****************Month wise Marchandising Fin Fabric Prod and Plan======================================= */
 foreach($mar_fin_month_wise_kpiArr as $comp_id=>$comData) //Month Wise KPI Percentage summary part
 {
  foreach($comData as $team_id=>$TeamData)   
		   ...
		   $fin_fab_month_kpi_per=0;
			$fin_fab_month_kpi_per=$mar_fin_fab_month_prod/$mar_fin_fab_month_plan*100; 
			 ...
			 if($day_chk<=$today)// as on today
			 {
				 ...
				 {
					 if($fin_fab_month_kpi_per>100)  $fin_fab_month_kpi_per=100;
				 $mar_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['fin']+=$fin_fab_month_kpi_per;
				 }
				$mar_yarn_plan_day_chk= $mar_yarn_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['plan'];

				 if($mar_fin_fab_month_plan>0)
				 {
					 $mar_mon_grand_fin_fab_event_kpi_perArr[$comp_id][$team_id][$member_id][$day_key]=1;
				 }
			...
			foreach ($fiscalMonth_arr as $year_mon => $val) 
			{
				...
				$member=$poWiseTLnMemberArr[$comp_id][$poid]['member'];
						...
						for($j=0;$j<$diff_days;$j++)
						{
							...
							{
								$mon_del_qty=$mon_delivery_qty;
								$mar_delivery_month_wise_kpiArr[$comp_id][$tl_name][$member][$day_all]['del_prod']+=$mon_delivery_qty;
							}
							$mar_delivery_month_wise_kpiArr[$comp_id][$tl_name][$member][$day_all]['acl_plan']+=$planAclQtycal;
						} 
						 //***********=========== Marchandisiing End===================********//
			}
		}
...
foreach($mar_delivery_month_wise_kpiArr as $comp_id=>$comData) //Month Wise KPI Percentage summary part
{
	foreach($comData as $team_id=>$TeamData) //Month Wise KPI Percentage summary part
	{
	 foreach($TeamData as $member_id=>$memberData) //Month Wise KPI Percentage summary part
	 {
	 foreach($memberData as $day_key=>$row)
	 {
		   $mar_del_month_prod=$row['del_prod']; 
		   ...
		   $fin_fab_month_kpi_per=0;
			$del_month_kpi_per=$mar_del_month_prod/$mar_del_month_plan*100; 
			 ...
			 if($day_chk<=$today)// as on today
			 {
				 ...
				 {
					 if($del_month_kpi_per>100)  $del_month_kpi_per=100;
				 $mar_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['deli']+=$del_month_kpi_per;
				 }
				 $mar_yarn_plan_day_chk= $mar_yarn_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['plan'];
				$mar_fin_plan_day_chk= $mar_fin_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['plan'];
				 if($mar_del_month_plan>0)
				 {
					 $mar_mon_grand_deli_event_kpi_perArr[$comp_id][$team_id][$member_id][$day_key]=1;
				 }
				 //if($knit_month_plan>0 && !$yarn_plan_day_chk)
				 if($mar_del_month_plan>0 && !$mar_yarn_plan_day_chk && !$mar_fin_plan_day_chk)
				 {
		...
		// ==============Marchandising Trim Sew PO and Plan=========
	foreach($mar_trim_sew_month_wise_kpiArr as $comp_id=>$comData)  
	{
		foreach($comData as $team_id=>$teamData)  
		{
		foreach($teamData as $member_id=>$memberData)  
		   ...
		   $sew_month_kpi_per=0;
			...
			$monthYr=date("M-Y",strtotime($day_key));
			 ...
			 if($day_chk<=$today)// as on today
			 {
				 ...
				 {
					 if($sew_month_kpi_per>100)  $sew_month_kpi_per=100;
				 	$mar_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['trim_sew']+=$sew_month_kpi_per;
				 }
				 $mar_yarn_plan_day_chk= $mar_yarn_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['plan'];
				$mar_fin_plan_day_chk= $mar_fin_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['plan'];
				$mar_del_plan_day_chk= $mar_delivery_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['acl_plan'];
				 if($mar_trim_month_plan>0)
				 {
					 $mar_mon_grand_trim_sew_event_kpi_perArr[$comp_id][$team_id][$member_id][$day_key]=1;
				 }
				 if($mar_trim_month_plan>0 && !$mar_yarn_plan_day_chk && !$mar_fin_plan_day_chk  && !$mar_del_plan_day_chk)
				 {
					 $mar_gbl_num_of_plan_daysArr[$comp_id][$team_id][$member_id][$yr_month]++;
		   ...
		   $fin_month_kpi_per=0;
			...
			$monthYr=date("M-Y",strtotime($day_key));
			 ...
			 if($day_chk<=$today)// as on today
			 {
				 ...
				 {
					 if($fin_month_kpi_per>100)  $fin_month_kpi_per=100;
				 $mar_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['trim_fin']+=$fin_month_kpi_per;
				 }
				 $mar_yarn_plan_day_chk= $mar_yarn_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['plan'];
				$mar_fin_plan_day_chk= $mar_fin_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['plan'];
				$mar_del_plan_day_chk= $mar_delivery_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['acl_plan'];
				$mar_sew_plan_day_chk= $mar_trim_sew_month_wise_kpiArr[$comp_id][$team_id][$member_id][$day_key]['plan'];
				 if($mar_trim_fin_month_plan>0)
				 {
					 $mar_mon_grand_trim_fin_event_kpi_perArr[$comp_id][$team_id][$member_id][$day_key]=1;
				 }
				 if($mar_trim_fin_month_plan>0 && !$mar_yarn_plan_day_chk && !$mar_fin_plan_day_chk  && !$mar_del_plan_day_chk  && !$mar_sew_plan_day_chk)
				 {
					 $mar_gbl_num_of_plan_daysArr[$comp_id][$team_id][$member_id][$yr_month]++;
	...
	foreach($mar_month_wise_kpiArr as $comp_id=>$comData) //Month Wise KPI Percentage summary part
	 {
		...
		foreach($teamData as $member_id=>$memberData) //Month Wise KPI Percentage summary part
		{
			foreach($memberData as $day_key=>$pers)  
			{
				$event_yarn_count=0;$event_fin_fab_count=0;$event_deli_count=0;$event_trim_sew_count=0;$event_trim_fin_count=0;
				$event_yarn_count=$mar_mon_grand_yarn_event_kpi_perArr[$comp_id][$team_id][$member_id][$day_key];
				$event_fin_fab_count=$mar_mon_grand_fin_fab_event_kpi_perArr[$comp_id][$team_id][$member_id][$day_key];
				$event_deli_count=$mar_mon_grand_deli_event_kpi_perArr[$comp_id][$team_id][$member_id][$day_key];
				$event_trim_sew_count=$mar_mon_grand_trim_sew_event_kpi_perArr[$comp_id][$team_id][$member_id][$day_key];
				$event_trim_fin_count=$mar_mon_grand_trim_fin_event_kpi_perArr[$comp_id][$team_id][$member_id][$day_key];
				$mar_event_mon_count= $event_yarn_count+$event_fin_fab_count+$event_deli_count+$event_trim_sew_count+$event_trim_fin_count;
				$mar_all_kpi_per=$pers['yarn']+$pers['fin']+$pers['deli']+$pers['trim_sew']+$pers['trim_fin'];
				$today=strtotime($today_date);
				$day_chk=strtotime($day_key);
				if($day_chk<=$today)// as on today
				{
				$mar_gbl_comp_avg_perArr[$comp_id][$team_id][$member_id][$day_key]+=$mar_all_kpi_per/$mar_event_mon_count;
				}
				//$all_avg_perArr[$day_key]+=$shafi_all_kpi_per/$shafi_event_mon_count;
			}
		  }
...
foreach($mar_gbl_comp_avg_perArr as $comp_id=>$comData) //Month Wise KPI Percentage summary part
{
  foreach($comData as $team_id=>$teamData) //Month Wise KPI Percentage summary part
  {
	foreach($teamData as $member_id=>$memberData) //Month Wise KPI Percentage summary part
	{
		foreach($memberData as $day_key=>$val)  
		{
			$monthYr=date("M-Y",strtotime($day_key));
			$yr_month=strtoupper($monthYr);
			 // echo $day_key.'='.$val.'<br>';
			...
			$day_chk=strtotime($day_key);
			 if($day_chk<=$today)// as on today
			 {
			$mar_team_comp_mon_avg_perArr[$comp_id][$team_id][$member_id][$yr_month]+=$val;
			}
		}
	  }
  }
	   ...
	   //  echo "<pre>";
	   // print_r($mar_gbl_comp_avg_perArr);
	//print_r($mon_wise_allocateQty_tillCalculateArr);
	  //  echo "<pre>"; 	
	  //$mar_gbl_num_of_plan_daysArr[$comp_id][$team_id][$member_id][$yr_month]
	$team_name_library=return_library_array( "select id,team_leader_name from lib_marketing_team", "id", "team_leader_name"  );
	...
	$tbl_width = 400+(count($fiscalMonth_arr)*80);
	ob_start();	
	$tbl_width2 = 5000;
	?>
	<style>
	...
	<!--=============Total PO wise dtls Start============================= style="display:none" =====================================-->
        <?
		foreach($mar_team_comp_mon_avg_perArr as $com_id=>$comData)
		{
			$avg_mon_countArr=array(); $Avg_team_wise_kpi_perArr=array();
		foreach($comData as $team_id=>$TeamData)
		{
			$team_rowspan=0;$tot_avg_month_kpi_per=0; 
			foreach($TeamData as $member_id=>$MemData)
			{
				$team_rowspan++; 
				...
				foreach ($fiscalMonth_arr as $year_mon => $val) 
				{
				   ...
				   $yr_month=strtoupper($mon_yr);
			   // $date=$day.'-'.$mon_yr;
				   $today=strtotime($today_date);
				   $day_chk=strtotime($date);
				   
				 
				   $avg_event_day_count=$mar_gbl_num_of_plan_daysArr[$com_id][$team_id][$member_id][$yr_month];
				   $avg_mar_gbl_comp_avg_kip=$mar_team_comp_mon_avg_perArr[$com_id][$team_id][$member_id][$yr_month];
				   

				   if($avg_mar_gbl_comp_avg_kip/$avg_event_day_count>0)
				   {
				   $avg_mon_countArr[$team_id][$member_id]+=1;
				   $tot_avg_month_kpi_per=$avg_mar_gbl_comp_avg_kip/$avg_event_day_count;
				 // echo  $yr_month.'='.$avg_mar_gbl_comp_avg_kip.'='.$avg_event_day_count.'<br>';
					 $Avg_team_wise_kpi_perArr[$team_id][$member_id]+=$tot_avg_month_kpi_per;
				   }
				}
			}
			$teamSpanArr[$com_id][$team_id]=$team_rowspan;
		}
	 }
	  // print_r($Avg_team_wise_kpi_perArr);
	   $tbl_width_unit = 330+(count($fiscalMonth_arr)*80);
		...
		<table border="1" rules="all" class="rpt_table" width="<? echo $tbl_width_unit;?>" cellpadding="0" cellspacing="0">
	        <thead>
		        ...
		        <?
						...
						asort($mar_team_comp_mon_avg_perArr);
						foreach($mar_team_comp_mon_avg_perArr as $com_id=>$comData)
						{
							$teamMon_wise_KPIArr=array();$kk=1;$avg_team_kpiArr=array();$team_total_avg_team_kpiArr=array();//$team_total_avg_team_kpi=0;
						foreach($comData as $team_id=>$TeamData)
						{
							$t=1;
						foreach($TeamData as $member_id=>$MemData)
						{	
								$team_rowspan=$teamSpanArr[$com_id][$team_id]; 
							if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
				        ...
				        <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trskpiunit_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="trskpiunit_<? echo $i; ?>">
						 ...
						 {
							?>
						<td align="center" title="<?=$team_id;?>" rowspan="<?=$team_rowspan;?>"> <? echo $team_name_library[$team_id]; ?> </td>
							<?
						 }
						 $avg_team_kpi=0;
						 $avg_team_kpi=$Avg_team_wise_kpi_perArr[$team_id][$member_id]/$avg_mon_countArr[$team_id][$member_id];
						 $team_total_avg_team_kpi=0;
						 if($avg_team_kpi>0) 
						 { 
							 $avg_team_kpiArr[$team_id]+=1;
							 $team_total_avg_team_kpiArr[$team_id]+=$avg_team_kpi;
						 }
						 ?>
						<td align="center" title="<?=$member_id;?>"> <a href="javascript:void()" onclick="report_generate_by_mkt_tl_popup('<? echo $cbo_company_id.'_'.$team_id.'_'.$member_id;?>','2')"><? echo $team_member_name_library[$member_id]; ?> </a></td>
						<td align="center" title="Total Percent(<?=$Avg_team_wise_kpi_perArr[$team_id][$member_id];?>/<?=$avg_mon_countArr[$team_id][$member_id];?>)"> <? echo fn_number_format($Avg_team_wise_kpi_perArr[$team_id][$member_id]/$avg_mon_countArr[$team_id][$member_id],2); ?> </td>
						<?
						...
						// $date=$day.'-'.$mon_yr;
							...
							//echo $today.'='.$day_chk.'<br>';
							$month_kpi_per=0; 
							$event_day_count=$mar_gbl_num_of_plan_daysArr[$com_id][$team_id][$member_id][$yr_month];
							$mar_gbl_comp_avg_kip=$mar_team_comp_mon_avg_perArr[$com_id][$team_id][$member_id][$yr_month];
							$month_kpi_per=$mar_gbl_comp_avg_kip/$event_day_count;
							$tot_team_month_kpi_per+=$month_kpi_per;
						?>
						<td width="80" align="center" title="<?=$mar_gbl_comp_avg_kip.'/'.$event_day_count?>"><?  if($month_kpi_per>0) echo fn_number_format($month_kpi_per,2).'%';?></td>
						<?
							if($month_kpi_per>0)
							{
								$teamMon_wise_KPIArr[$team_id][$yr_month]+=$month_kpi_per;
								$teamMonCount_wise_KPIArr[$team_id][$yr_month]+=$kk;
							}
						 }
						?>
				        </tr> 
						...
						<td align="right" title="Total Avg Kpi(<?=$team_total_avg_team_kpiArr[$team_id];?>/<?=$avg_team_kpiArr[$team_id];?>"> <?  echo fn_number_format($team_total_avg_team_kpiArr[$team_id]/$avg_team_kpiArr[$team_id],2); ?> </td>
						<?
						foreach ($fiscalMonth_arr as $year_mon => $val) 
						{
							$mon_yr=date('M-Y',strtotime($year_mon));
						    $yr_month=strtoupper($mon_yr);
							$team_tot_avg_kpi_per=$teamMon_wise_KPIArr[$team_id][$yr_month];
							$teamMonCount=$teamMonCount_wise_KPIArr[$team_id][$yr_month];
							?>
						<td width="80" align="center" title="<?=$teamMonCount.'='.$team_tot_avg_kpi_per;?>"> <b><? if($team_tot_avg_kpi_per>0) echo fn_number_format(($team_tot_avg_kpi_per/$teamMonCount),2).'%';?></b></td>
						<?
						//unset($teamMon_wise_KPIArr);
						}
				?>
						</tr>
						   <?
...
//=============**************MKT TL Wise************==================================
if($action=="report_generate_by_year_team_member_mkt")
{
	
	$cbo_company_id = str_replace("'","",$cbo_company_id);
	$from_year 		= str_replace("'","",$cbo_from_year);
	$cbo_templete_id 	= str_replace("'","",$cbo_templete_id);
	$teamid 	= str_replace("'","",$team_id);
	$memberid 	= str_replace("'","",$member_id);
	...
	$firstYear 		= $exfirstYear[0];
	$lastYear 		= $exfirstYear[1];
	  // echo $cbo_company_id.'='.$team_id.'='.$member_id; 
	...
	$con = connect();
 	execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id."  and ENTRY_FORM=89");
	  ...
	  $sql_po_plan="SELECT a.company_name as COMPANY_ID,a.TEAM_LEADER,a.DEALING_MARCHANT,a.LOCATION_NAME,b.id as POID,b.GROUPING,c.TASK_ID, c.PLAN_QTY,c.PLAN_DATE, c.SOURCE_ID,c.UOM_ID,b.job_no_mst as JOB_NO from wo_po_details_master a,wo_po_break_down b, tna_plan_target c where a.id=b.job_id and b.id=c.po_id and c.task_id in(48,73,70,71)   and c.task_type=1 and c.plan_qty>0 and  b.status_active=1 and a.team_leader=$teamid and a.dealing_marchant=$memberid and b.is_deleted=0 and b.id in(20328,20329,20330,20334,20335,20337)   and c.PLAN_DATE between '$startDate' and '$endDate' $company_conds order by c.PLAN_DATE asc"; //and b.id in(20325,20326,20327,20328,20329,20330)
	$sql_po_plan_result = sql_select($sql_po_plan);
	 foreach ($sql_po_plan_result as $val) 
	 {
		  ////============***********Marchandising ********===============
	 ...
	 fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 89, 1, $plan_poIdArr, $empty_arr);//PO ID Ref from=1
  $sql_po_plan_current="SELECT a.company_name as COMPANY_ID,b.id as POID, c.PLAN_QTY,c.PLAN_DATE, b.job_no_mst as JOB_NO from wo_po_details_master a,wo_po_break_down b, tna_plan_target c,gbl_temp_engine g where a.id=b.job_id and b.id=c.po_id and c.task_id=48   and c.task_type=1 and c.plan_qty>0 and b.id=g.ref_val and c.po_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from=1 and g.entry_form=89 and a.team_leader=$teamid and a.dealing_marchant=$memberid and  b.status_active=1 and b.is_deleted=0  and  c.status_active=1 and c.is_deleted=0 $company_conds and c.PLAN_DATE <= '$endDate' order by c.PLAN_DATE asc"; //and b.id in(20325,20326,20327,20328,20329,20330)
	$sql_po_plan_result_current = sql_select($sql_po_plan_current);
	//$previ_plan_cumulative_cal=0;
	foreach ($sql_po_plan_result_current as $val) 
	{
	...
	$sql_po_allocate="SELECT a.company_name as COMPANY_ID,b.id as POID,b.GROUPING, c.QNTY,c.allocation_date as ALLOC_DATE, b.job_no_mst as JOB_NO from wo_po_details_master a,wo_po_break_down b, inv_material_allocat_hystory c where a.id=b.job_id and b.id=c.po_break_down_id and b.id in(20328,20329,20330,20334,20335,20337)  and  b.status_active=1 and b.is_deleted=0  and a.team_leader=$teamid and a.dealing_marchant=$memberid  and c.allocation_date between '$startDate' and '$endDate'   $company_conds order by c.allocation_date asc"; 
	$sql_po_allocate_result = sql_select($sql_po_allocate); //and b.id in(20325,20326,20327,20328,20329,20330)
	foreach ($sql_po_allocate_result as $val) 
	{
		$poWiseTLnMemberArr[$val['COMPANY_ID']][$val['POID']]['tl']=$val['TEAM_LEADER'];
	...
	// fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 89, 2, $allocat_poIdArr, $empty_arr);//PO ID Ref from=2
	 fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 89, 44, $mar_allocat_poIdArr, $empty_arr);//PO ID Ref from=44
	// ===============Plan Task ID=48==================
     $curr_sql_po_allocate="SELECT a.company_name as COMPANY_ID,a.TEAM_LEADER,a.DEALING_MARCHANT,b.id as POID, c.QNTY,c.allocation_date as ALLOC_DATE, b.job_no_mst as JOB_NO,g.REF_FROM from wo_po_details_master a,wo_po_break_down b, inv_material_allocat_hystory c,gbl_temp_engine g  where a.id=b.job_id and b.id=c.po_break_down_id and b.id=g.ref_val and c.po_break_down_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in(44) and g.entry_form=89  and a.team_leader=$teamid and a.dealing_marchant=$memberid and c.allocation_date <= '$endDate'  and  b.status_active=1 and b.is_deleted=0    $company_conds order by c.allocation_date,c.id asc";
...
from wo_po_details_master a,wo_po_break_down b, tna_plan_target c,gbl_temp_engine g where a.id=b.job_id and b.id=c.po_id    and c.task_type=1 and c.plan_qty>0 and b.id=g.ref_val and c.po_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in(34,35,36) and c.task_id in(73,70,71) and g.entry_form=89  and a.team_leader=$teamid and a.dealing_marchant=$memberid and  c.status_active=1 and c.is_deleted=0 and  b.status_active=1 and b.is_deleted=0 $company_conds and c.PLAN_DATE <= '$endDate' order by c.PLAN_DATE asc";
//and b.id in(20325,20326,20327,20328,20329,20330)
 $sql_po_plan_result_trim = sql_select($sql_po_plan_trim);
 foreach ($sql_po_plan_result_trim as $val) 
 {
...
//	=======JM Marchandising================
 ...
 inv_transaction c,order_wise_pro_details d  where a.id=b.job_id  and c.id=d.trans_id and b.id=d.po_breakdown_id
 and d.quantity>0 and b.status_active=1 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0 
   and b.id in(20328,20329,20330,20334,20335,20337) and a.team_leader=$teamid and a.dealing_marchant=$memberid and c.transaction_type=1   and d.entry_form in(37) and c.transaction_date between '$startDate' and '$endDate' $company_conds order by c.transaction_date asc"; //and b.id in(20325,20326,20327,20328,20329,20330)
 ...
 //====================Plan Task ID=73 and SOURCE_ID=2==============
	  $sql_mar_fin_gmts_qty="SELECT a.company_name as COMPANY_ID,a.LOCATION_NAME,b.id as POID,b.GROUPING,d.QUANTITY,c.TRANSACTION_DATE, b.job_no_mst as JOB_NO from wo_po_details_master a,wo_po_break_down b,
	inv_transaction c,order_wise_pro_details d,gbl_temp_engine g   where a.id=b.job_id  and c.id=d.trans_id and b.id=d.po_breakdown_id and b.id=g.ref_val and d.po_breakdown_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in(37) and g.entry_form=89
	and d.quantity>0 and b.status_active=1 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0 
	  and b.id in(20328,20329,20330,20334,20335,20337) and a.team_leader=$teamid and a.dealing_marchant=$memberid and c.transaction_type=1   and d.entry_form in(37)    and c.transaction_date <= '$endDate'  $company_conds order by c.transaction_date asc"; 
	  //and b.id in(20325,20326,20327,20328,20329,20330)
 ...
 inv_transaction c,order_wise_pro_details d,inv_trims_entry_dtls e,lib_item_group f  where a.id=b.job_id  and c.id=d.trans_id and b.id=d.po_breakdown_id and c.id=e.trans_id and e.id=d.dtls_id  and f.id=e.item_group_id
 and d.quantity>0 and f.status_active=1 and f.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0  and  e.status_active=1 and e.is_deleted=0  and  d.status_active=1 and d.is_deleted=0 
   and b.id in(20328,20329,20330,20334,20335,20337) and a.team_leader=$teamid and a.dealing_marchant=$memberid and c.transaction_type=1   and d.entry_form in(24) and c.transaction_date between '$startDate' and '$endDate' $company_conds order by c.transaction_date asc";  //and b.id in(20325,20326,20327,20328,20329,20330)
	...
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 89, 38, $mar_trim_recvpoIdArr, $empty_arr);//PO ID Ref from=38
	//====================Plan Task ID=71 and Sewing n Fin==============
	  $sql_mar_trim_recv_qty="SELECT a.company_name as COMPANY_ID,a.LOCATION_NAME,b.id as POID,b.GROUPING,d.QUANTITY, b.job_no_mst as JOB_NO,c.TRANSACTION_DATE,f.trim_type as TRIM_TYPE from wo_po_details_master a,wo_po_break_down b,
 inv_transaction c,order_wise_pro_details d,inv_trims_entry_dtls e,lib_item_group f,gbl_temp_engine g  where a.id=b.job_id  and c.id=d.trans_id and b.id=d.po_breakdown_id and c.id=e.trans_id and e.id=d.dtls_id  and f.id=e.item_group_id and b.id=g.ref_val and d.po_breakdown_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in(38) and g.entry_form=89
 and d.quantity>0 and f.status_active=1 and f.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0  and  e.status_active=1 and e.is_deleted=0  and  d.status_active=1 and d.is_deleted=0 
   and b.id in(20328,20329,20330,20334,20335,20337) and a.team_leader=$teamid and a.dealing_marchant=$memberid and c.transaction_type=1   and d.entry_form in(24) and c.transaction_date <= '$endDate' $company_conds order by c.transaction_date asc";
	 ...
	 //=============Actula Po========
	 $sql_actual_po_plan_jm="SELECT c.id as acul_poid,a.company_name as COMPANY_ID,a.TEAM_LEADER,a.DEALING_MARCHANT,a.LOCATION_NAME,b.id as POID,b.GROUPING, c.acc_po_qty as PLAN_QTY,c.ACC_SHIP_DATE as PLAN_DATE, b.job_no_mst as JOB_NO from wo_po_details_master a,wo_po_break_down b, wo_po_acc_po_info c where a.id=b.job_id and b.id=c.po_break_down_id    and c.acc_po_qty>0 and  b.status_active=1 and b.is_deleted=0   and a.team_leader=$teamid and a.dealing_marchant=$memberid and b.id in(20328,20329,20330,20334,20335,20337) and c.acc_ship_date between '$startDate' and '$endDate' $company_conds  order by acul_poid, PLAN_DATE asc"; //and b.id in(20325,20326,20327,20328,20329,20330)
	$sql_actual_po_plan_result_jm = sql_select($sql_actual_po_plan_jm);
	foreach($sql_actual_po_plan_result_jm  as $val)
	{
	$poWiseTLnMemberArr[$val['COMPANY_ID']][$val['POID']]['tl']=$val['TEAM_LEADER'];
	...
	// for actual po ship qty
	//====================Plan Task ID=no task id==============
	$sql_actual_qty_po_plan_jm="SELECT c.id as ACUL_POID,a.company_name as COMPANY_ID,a.LOCATION_NAME,b.id as POID,b.GROUPING,c.ACC_PO_NO, c.acc_po_qty as PLAN_QTY,c.ACC_SHIP_DATE as PLAN_DATE, b.job_no_mst as JOB_NO,g.REF_FROM from wo_po_details_master a,wo_po_break_down b, wo_po_acc_po_info c, gbl_temp_engine g where a.id=b.job_id and b.id=c.po_break_down_id and c.acc_po_qty>0 and b.id=g.ref_val and c.po_break_down_id=g.ref_val  and g.user_id = ".$user_id." and g.ref_from in(40) and g.entry_form=89 and  b.status_active=1 and b.is_deleted=0  and b.id in(20328,20329,20330,20334,20335,20337) and a.team_leader=$teamid and a.dealing_marchant=$memberid and c.ACC_SHIP_DATE <= '$endDate' $company_conds order by  PLAN_DATE asc"; //and b.id in(20325,20326,20327,20328,20329,20330)
	$sql_actual_po_qty_plan_result_jm = sql_select($sql_actual_qty_po_plan_jm);
	foreach($sql_actual_po_qty_plan_result_jm  as $val)
	{
		$plandate=date('d-M-Y',strtotime($val['PLAN_DATE']));
 ...
 pro_ex_factory_mst c,pro_ex_factory_actual_po_details d  where a.id=b.job_id and b.id=c.po_break_down_id
  and c.id=d.mst_id   and d.ex_fact_qty>0 and b.status_active=1 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  d.status_active=1 and d.is_deleted=0 
  and b.id in(20328,20329,20330,20334,20335,20337) and a.team_leader=$teamid and a.dealing_marchant=$memberid and c.ex_factory_date between '$startDate' and '$endDate' $company_conds  order by c.ex_factory_date asc"; //and b.id in(20325,20326,20327,20328,20329,20330) // and a.location_name=6  and a.company_name=1
	$sql_actual_po_delivery_result_jm = sql_select($sql_actual_po_delivery_jm);
	foreach($sql_actual_po_delivery_result_jm  as $val)
	{
		$poWiseTLnMemberArr[$val['COMPANY_ID']][$val['POID']]['tl']=$val['TEAM_LEADER'];
	...
	foreach($mar_month_wise_kpiArr as $comp_id=>$comData) //Month Wise KPI Percentage summary part
	 {
		...
		foreach($teamData as $member_id=>$memberData) //Month Wise KPI Percentage summary part
		{
			foreach($memberData as $day_key=>$pers)  
			{
				...
				if($day_key=='29-May-2023' )
				{
					   //echo $day_key.'='.$pers['yarn'].'='.$pers['fin'].'='.$pers['deli'].'='.$pers['trim_sew'].'='.$pers['trim_fin'].'/'.$mar_event_mon_count.'<br>';
				}
				// echo $day_key.'='.$event_del_count.'='.$per['deli'].'/'.$nay_event_mon_count.'<br>';
				$mar_gbl_comp_avg_perArr[$comp_id][$team_id][$member_id][$day_key]+=$mar_all_kpi_per/$mar_event_mon_count;
				//$all_avg_perArr[$day_key]+=$shafi_all_kpi_per/$shafi_event_mon_count;
...
$team_member_name_library=return_library_array( "select id,team_member_name from lib_mkt_team_member_info", "id", "team_member_name"  );
	...
	// print_r($teamSpanArr);
	  ...
	  // if($cbo_company_id==2) $com_ttl="Shafipur"; 
	  // else $com_ttl="Nayapara Fabric";  and a.team_leader=$teamid and a.dealing_marchant=$memberid
	  $com_ttl=$team_member_name_library[$memberid];  
		?>
          <table width="<? echo $tbl_width_unit;?>"  cellspacing="0">
	        <tr class="form_caption">
	            <td colspan="11" align="center" ><strong style="font-size:19px"><?php //echo $company_arr[$cbo_company_id]; ?></strong></td>
	        ...
	        <tr class="form_caption">
	            <td colspan="11" align="center"><strong style="font-size:14px">KPI For <?=$team_member_name_library[$memberid].'<br>'.$company_arr[$cbo_company_id];?> </strong></td>
	        </tr>
	    </table>
	    <h3 style="width:<? echo $tbl_width_unit;?>px;" align="left" id="accordion_h1" class="accordion_h" onClick="accordion_menu( this.id,'kpi_unit_mkt_report_popup', '')"> -<b>KPI MKT  <?=$com_ttl;?>[<? echo $from_year; ?>]</b></h3>
	    <div id="kpi_unit_mkt_report_popup">
		<table border="1" rules="all" class="rpt_table" width="<? echo $tbl_width_unit;?>" cellpadding="0" cellspacing="0">
	        <thead>
			<th width="80">Date</th>
				<?
		        ...
		        <?
						...
						$day=str_pad($j, 2, '0', STR_PAD_LEFT);       		
			        	?>     
				        <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trskpiunitmkt_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="trskpiunitmkt_<? echo $i; ?>">
						...
						<?
						 foreach ($fiscalMonth_arr as $year_mon => $val) 
						 {
						   $mon_yr=date('M-Y',strtotime($year_mon));
							...
							if($day_chk<=$today)// as on today
							{
								$mar_gbl_comp_avg_kip=$mar_gbl_comp_avg_perArr[$cbo_company_id][$teamid][$memberid][$date];//$ash_gbl_comp_avg_perArr[$cbo_company_id][$unitid][$date];
								$month_kpi_per=$mar_gbl_comp_avg_kip;
							}

						?>
						<td width="80" align="center" title="<??>"><?  if($month_kpi_per>0) echo fn_number_format($month_kpi_per,2).'%';?></td>
						<?
							if($month_kpi_per>0)
							{
							$date_wise_KPIArr[$memberid][$year_mon]+=$month_kpi_per;
							}
						 }
						?>
				        </tr> 
	        ...
	        <tfoot>
				...
				foreach ($fiscalMonth_arr as $year_mon => $val) 
	            {
					...
					$tot_mon_days=$mar_gbl_num_of_plan_daysArr[$cbo_company_id][$teamid][$memberid][$year_mon];
					
					$tot_kpi_per=0;$tot_avg_kpi_per=0;
					$tot_kpi_per=$date_wise_KPIArr[$memberid][$year_mon];
					if($tot_kpi_per>0)
					{
					...
					?>
                <th width="80" align="center" title="tot Days-"<? echo $tot_mon_days;?>><? if($tot_avg_kpi_per>0) echo fn_number_format($tot_avg_kpi_per,2).'%';?></th>
				<?
				}
				?>
		...
		<table border="1" rules="all" class="rpt_table" width="<? echo $tbl_width_unit;?>" cellpadding="0" cellspacing="0">
	        ...
	        <thead>
		        ...
		        <?
					 ...
					 // $yr_month=strtoupper($monthYr);
						...
						?>
						<td width="80" align="center" title="<?=$day_allocat_qty.'/'.$day_plan_qty.'*100';?>"><?   if($month_kpi_per>0) echo fn_number_format($month_kpi_per,2).'%';?></td>
						<?
							if($month_kpi_per>0)
							{
	        ...
	        <tfoot>
				...
				foreach ($fiscalMonth_arr as $year_mon => $val) 
	            {
					...
					?>
                <th width="80" align="center" title="tot Days="<?=$tot_mon_days;?>><? if($tot_avg_kpi_per>0) echo fn_number_format($tot_avg_kpi_per,2).'%';?></th>
				<?
				}
				?>
...
fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 89, 36, $plan_trim_fin_mar_poIdArr, $empty_arr);//PO ID Ref from=26
 ...
 $sql_po_plan_trim="SELECT a.company_name as COMPANY_ID,a.LOCATION_NAME,b.id as POID,c.PLAN_DATE,c.TASK_ID,c.SOURCE_ID,c.UOM_ID,g.REF_FROM, b.job_no_mst as JOB_NO,
(case when  c.task_id=73 and c.source_id=2 and g.ref_from=34  then c.PLAN_QTY else 0 end) as MAR_FIN_PLAN_QTY,
(case when  c.task_id=70   and g.ref_from=35  then c.PLAN_QTY else 0 end) as MAR_TRIM_SEW_PLAN_QTY,
(case when  c.task_id=71  and g.ref_from=36  then c.PLAN_QTY else 0 end) as MAR_TRIM_FIN_PLAN_QTY
			...
			foreach ($fiscalMonth_arr as $year_mon => $val) 
			{
					...
					$diff_days=datediff('d',$from_date,$to_date);
						 ...
						 foreach($mar_acl_delivery_qty_array[5][$mar_newdate] as $poid_del=>$poDataArr)
						 {
							...
							//===========Yarn Plan and Allocation for Marchandising============
		...
		<table border="1" rules="all" class="rpt_table" width="<? echo $tbl_width_unit;?>" cellpadding="0" cellspacing="0">
	        ...
	        <thead>
		        ...
		        <?
						...
						if($day_chk<=$today)// as on today
						{
							$mar_gbl_comp_avg_kip=$mar_gbl_comp_avg_perArr[$cbo_company_id][$date];//shafi_gbl_comp_avg_perArr
							$month_kpi_per=$mar_gbl_comp_avg_kip;
	        ...
	        <tfoot>
				...
				foreach ($fiscalMonth_arr as $year_mon => $val) 
	            {
					...
					$tot_mon_days=$mar_gbl_num_of_plan_daysArr[$cbo_company_id][$year_mon];
					if($tot_mon_days>0) $tt_day=$tot_mon_days;
				//	$mar_gbl_num_of_plan_daysArr[$comp_id][$yr_month]
					//echo $tot_mon_days.'f';
					$tot_kpi_per=0;$tot_avg_kpi_per=0;
					$tot_kpi_per=$date_wise_KPIArr[$unitid][$year_mon];
					...
					?>
                <th width="80" align="center" title="tot Days="<? echo $tt_day;?>><? echo fn_number_format($tot_avg_kpi_per,2).'%';?></th>
				<?
				}
				?>
