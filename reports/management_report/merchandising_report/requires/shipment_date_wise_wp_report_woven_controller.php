...
if ($action=="load_drop_down_brand")
{
	...
	if($data_arr[1] == 1) $width=70; else $width=100;
...
if ($db_type == 0) {
    ...
    $toDay = date("Y-M-d");
        ...
        if($db_type==0)
		{
			if ($cbo_season_year==0) $season_year_cond=""; else $season_year_cond=" and YEAR(a.insert_date)=$cbo_season_year";
		}
		elseif($db_type==2)
		{ 
			if ($cbo_season_year==0) $season_year_cond=""; else $season_year_cond=" and TO_CHAR(a.insert_date,'YYYY')=$cbo_season_year";
		}
...
if ($cbo_order_status== 0) $order_status_cond = ""; else $order_status_cond = " and b.is_confirmed='$cbo_order_status'";
...
<div align="center">
    ...
    if (str_replace("'", "", $cbo_search_by) == 3) {
    ?>
    <table width="4220" id="table_header_1" border="1" class="rpt_table" rules="all">
        <thead>
        <tr>
            <th width="40">SL</th>
            ...
            <th width="60">Balance Ship Qnty As Per Ex-factory</th>
            <th width="">Remarks</th>
        </tr>
        </thead>
    </table>
    <div style="max-height:400px; overflow-y:scroll; width:4240px" align="left" id="scroll_body">
        <table width="4220" border="1" class="rpt_table" rules="all" id="table_body">
            <?
            } else {
            ?>
            <table width="3780" id="table_header_1" border="1" class="rpt_table" rules="all">
                <thead>
                <tr>
                    <th width="40">SL</th>
                    <th width="60">Job No</th>
                    ...
                    <th width="90">Ship Qnty. (Pcs) As Per Invoice</th>
                    <th width="100">Ship Value (Gross)</th>
                    <th width="60">Balance Ship Qnty As Per Invoice</th>
                    <th width="">Remarks</th>
                </tr>
                </thead>
            </table>
            <div style="max-height:400px; overflow-y:scroll; width:3800px" align="left" id="scroll_body">
                <table width="3780" border="1" class="rpt_table" rules="all" id="table_body">
                    <?
                    }
                    ...
                    if (str_replace("'", "", $cbo_search_by) == 1 || str_replace("'", "", $cbo_search_by) == 2) {
                        ...
                        WHERE
                            a.job_no_mst=b.job_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $poIds_cond_emb group by a.id";
                        //echo $sqlOrder;die;
                        ...
                        $insp_qnty = "select po_break_down_id, sum(inspection_qnty) as inspection_qnty from pro_buyer_inspection where inspection_status=1 and is_deleted=0 and status_active=1 $poIds_cond_prod group by po_break_down_id";
                    ...
                    $sampleSQL = "select po_break_down_id, approval_status, $date_diff as delay_day from wo_po_sample_approval_info where approval_status<>4 and current_status=1 and is_deleted=0 and status_active=1 $poIds_cond_prod";
                    //echo $sampleSQL;die;
                    ...
                    //print_r($lapdipAPParr);
                    //echo "<br>".$total_po;die;
                    ...
                    if (str_replace("'", "", $cbo_search_by) == 1) {//Order wise report==================
                        ...
                        foreach ($jobSqlResult as $row) {
                            ...
                            if ($cbo_ls_sc == 1 || ($cbo_ls_sc == 2 && $lcsc_amnt > 0) || ($cbo_ls_sc == 3 && $lcsc_amnt <= 0)) {
                                ...
                                <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                                    ...
                                    <td width="60" align="right"><? echo $row[csf('unit_price')]; ?></td>
                                    ...
                                    <td width="80" align="right">
                                        ...
                                        if(is_infinite($prnt_qty) || is_nan($prnt_qty)){$prnt_qty=0;}
                                        $embel_qty = $prodArr[$row[csf('id')]]['embel'] / $poReqArr[$row[csf('id')]][2] * 100;
                                        if(is_infinite($embel_qty) || is_nan($embel_qty)){$embel_qty=0;}
                                        $wash_qty = $prodArr[$row[csf('id')]]['wash'] / $poReqArr[$row[csf('id')]][3] * 100;
                                        if(is_infinite($wash_qty) || is_nan($wash_qty)){$wash_qty=0;}
                                        $special_qty = $prodArr[$row[csf('id')]]['special'] / $poReqArr[$row[csf('id')]][4] * 100;
                                        if(is_infinite($special_qty) || is_nan($special_qty)){$special_qty=0;}
                                        $totalP = 0;
                                        $n = 0;
                                        if ($prnt_qty !== 0) {
                                            $totalP += $prnt_qty;
                                ...
                                <td width="80" align="right">
                                    <a href="##" onclick="show_progress_report_details('iron_output_popup', '<? echo $row[csf("job_no")]; ?>_<? echo $row[csf('id')]; ?>', '850px')">
                                        ...
                                        if ($ironoutput_qnty != '') {
                                            ...
                                            if(is_infinite($ironoutput_qnty_perc) || is_nan($ironoutput_qnty_perc)){$ironoutput_qnty_perc=0;}
                        ...
                        from
                            wo_po_details_master a, wo_po_break_down b ,wo_po_color_size_breakdown c,wo_pre_cost_mst d
                        where
                        ...
                        from
                            wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c,wo_pre_cost_mst d
                        where
                            a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id  and  d.job_no=c.job_no_mst and  a.job_no=d.job_no  and  b.job_no_mst=d.job_no and a.status_active=1 and b.status_active=1 and c.status_active=1 and b.po_number like '$order_no_cond' and a.style_ref_no like '$style_ref_cond' $date_cond $searchCond $job_no_cond $team_name_cond $team_member_cond $year_cond $file_no_cond $ref_no_cond
                            ...
                            <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                                ...
                                <td width="80" align="right">
                                    ...
                                    if ($prnt_qty != '') {
                                        ...
                                        if(is_infinite($prnt_qty_perc) || is_nan($prnt_qty_perc)){$prnt_qty_perc=0;}
                                    } else {
                                        $prnt_qty_perc = '0.00';
                                    }
                                ...
                                <td width="80" align="right">
                                    ...
                                    if ($embel_qty != '') {
                                        ...
                                        if(is_infinite($embel_qty_perc) || is_nan($embel_qty_perc)){$embel_qty_perc=0;}
                                    } else {
                                        $embel_qty_perc = '0.00';
                                    }
                                ...
                                <td width="80" align="right">
                                    ...
                                    if ($wash_qty != '') {
                                        ...
                                        if(is_infinite($wash_qty_perc) || is_nan($wash_qty_perc)){$wash_qty_perc=0;}
                                    } else {
                                        $wash_qty_perc = '0.00';
                                    }
                                ...
                                <td width="80" align="right">
                                    ...
                                    if ($special_qty != '') {
                                        ...
                                        if(is_infinite($special_qty_perc) || is_nan($special_qty_perc)){$special_qty_perc=0;}
                                    } else {
                ...
                if (str_replace("'", "", $cbo_search_by) == 3) {
                    ...
                    <table border="1" class="rpt_table" width="4220" rules="all" id="report_table_footer" >
                        ...
                        <th width="60" id="total_balance_ship_qnty_as_ex"></th>
                        <th></th>
                        </tfoot>
                    </table>
                    <?
                    ...
                    ?>
                    <table border="1" class="rpt_table"  width="3780" rules="all" id="report_table_footer" >
                        <tfoot>
                        <th width="40">&nbsp;</th>
                        <th width="60"></th>
                        <th width="60"></th>
                        ...
                        <th width="90" id="total_ship_qnty"></th>
                        <th width="100" id="value_total_ship_value"></th>
                        <th width="60" id="total_balance_ship_qnty"></th>
                        <th></th>
                        </tfoot>
                    </table>
                    <?
    ...
    if ($action == "woven_fabric_booking_popup") {
        ...
        <div style="width:100%" align="center">
            <fieldset style="width:800px">
                ...
                <div style="width:100%">
                    <table cellpadding="0" width="100%" class="rpt_table" rules="all" border="1">
                        ...
                        <tr>
                            ...
                            <th width="90">Fabric Required</th>
                ...
                <table cellpadding="0" width="230" class="rpt_table" rules="all" border="1">
                    <thead>
                    <tr>
                        <th width="30">SL</th>
                    ...
                    <?
                        $booking_arr = sql_select("SELECT a.booking_no, sum(b.grey_fab_qnty) as grey_fab_qnty from wo_booking_mst a join wo_booking_dtls b on a.booking_no = b.booking_no where a.item_category=3 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.po_break_down_id in ($po_id)  group by a.booking_no");
                        $i=1;
                        foreach ($booking_arr as $row) {
                           ?>
                                <tr>
                                   ...
                                   <td width="90"><? echo number_format($row[csf("grey_fab_qnty")], 2); ?></td>
                                </tr>
                           <?
                           $total_booking += $row[csf("grey_fab_qnty")];
                           $i++;
                        }
                    ?>
                    <tr>
                        ...
                        <th width="90"><? echo number_format($total_booking, 2); ?></th>
                    </tr>
                </table>
            </fieldset>
        </div>
    ...
    if ($action == "lcsc_rcv_popup") {
        ...
        where a.id=b.com_export_lc_id and b.wo_po_break_down_id in ($po_id) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.bank_file_no, a.internal_file_no, a.lc_year, a.last_shipment_date");
                    ...
                    $sql_btb = "select a.internal_file_no, a.id as lc_sc_id, a.contract_value as lc_sc_value, b.current_distribution, c.id as btb_id, c.lc_number as btb_lc_no, c.lc_value, 1 as type
        from com_sales_contract a
        left join com_btb_export_lc_attachment b on a.id=b.lc_sc_id and b.is_lc_sc=1 and b.status_active=1
        left join com_btb_lc_master_details c on b.import_mst_id=c.id and c.status_active=1
        where a.internal_file_no in($internal_file_nos) and a.sc_year='$sc_years' and a.status_active=1 and a.is_deleted=0 and a.beneficiary_name=$com_id
        UNION ALL
        select a.internal_file_no, a.id as lc_sc_id, a.lc_value as lc_sc_value, b.current_distribution, c.id as btb_id, c.lc_number as btb_lc_no, c.lc_value, 0 as type
        from com_export_lc a
        left join com_btb_export_lc_attachment b on a.id=b.lc_sc_id and b.is_lc_sc=0 and b.status_active=1
        left join com_btb_lc_master_details c on b.import_mst_id=c.id and c.status_active=1
        where a.internal_file_no in($internal_file_nos) and a.lc_year='$sc_years' and a.status_active=1 and a.is_deleted=0 and a.beneficiary_name=$com_id";
                    //echo $sql_btb;
                    $sql_btb_result = sql_select($sql_btb);
                    foreach($sql_btb_result as $row)
                    {
                        if($lc_sc_check[$row[csf("type")]][$row[csf("lc_sc_id")]]=="")
                        {
                            $lc_sc_check[$row[csf("type")]][$row[csf("lc_sc_id")]]=$row[csf("lc_sc_id")];
                            if($row[csf("type")]==1) $tot_sc_value+=$row[csf("lc_sc_value")]; else $tot_lc_value+=$row[csf("lc_sc_value")];
                            //$file_value+=$row[csf("lc_sc_value")];
                        }
                        if($btb_check[$row[csf("btb_id")]]=="")
                        {
                            $btb_check[$row[csf("btb_id")]]=$row[csf("btb_id")];
                            $btb_open_value+=$row[csf("lc_value")]*1;
                        }

                    }
                    $balance_value=$tot_sc_value-$tot_lc_value;
                    $file_value=$balance_value+$tot_lc_value;
    ...
    a.company_name,a.style_ref_no,a.insert_date");
                    ...
                    <tr>
                        <td align="right" title="from commercial"><strong>Year</strong> :</td>
                        <td><? echo $sc_years; ?></td>
                        <td align="right" title="from commercial"><strong>Int. Ref. No</strong> : </td>
                        <td><? echo $internal_file_nos; ?> </td>
                    </tr>
                    <tr>
                        <td align="right" title="from commercial"><strong>File No</strong> : </td>
                        <td><? echo $bank_file_no; ?> </td>
                        <td align="right" title="from commercial"><strong>Last Shipdate</strong> : </td>
                        <td><? echo $last_shipment_dates; ?> </td>
                    </tr>
                    <tr>
                        <td align="right" title="from commercial"><strong>BTB Percentage</strong> : </td>
                        <td title="<? echo "btb value=".$btb_open_value." file value=".$file_value ?>"><? echo number_format($btb_percent,2); ?> </td>
                        <td>&nbsp;</td>
                        <td>&nbsp;</td>
                    </tr>
                    <tr>
            ...
            <fieldset style="width:830px">
                ...
                <div style="width:600px; overflow-y:scroll; max-height:250px">
                    <table cellpadding="0" width="580" class="rpt_table" rules="all" border="1">
                        ...
                        $bookingData = sql_select("select b.fabric_color_id, sum(b.fin_fab_qnty) as fin_fab_qnty from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and b.po_break_down_id in ($po_id) and a.item_category=3 and b.status_active=1 and b.is_deleted=0 group by b.fabric_color_id order by b.fabric_color_id");
                        foreach ($bookingData as $row) {
                            $booking_qnty_arr[$row[csf('fabric_color_id')]] = $row[csf('fin_fab_qnty')];
                            $color_arr[$row[csf('fabric_color_id')]] = $row[csf('fabric_color_id')];
                        }
                ...
                <table cellpadding="0" width="350" class="rpt_table" rules="all" border="1">
                    <thead>
                    <tr>
                        <th width="80" align="center">System ID</th>
                        <th width="40" align="center">Color</th>
                        <th width="40" align="center">Fin. Rcv. Qty.</th>
                    </tr>
                    </thead>
                        <?
                        $bgcolor = "#EFEFEF";
                        ...
                        $fin_trans_arr = array();
                        $fin_trans_data = sql_select("SELECT a.recv_number, c.color_id, sum(c.quantity) AS woven_fin_receive from inv_receive_master a join inv_transaction b on a.id = b.mst_id join order_wise_pro_details c on b.id=c.trans_id where a.entry_form =17 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and c.entry_form=17 and c.po_breakdown_id in ($po_id) group by a.recv_number, c.color_id order by c.color_id");

                        foreach ($fin_trans_data as $row) {
                            $fin_trans_arr[$row[csf('color_id')]]['recv_number'][] = $row[csf('recv_number')];
                            $fin_trans_arr[$row[csf('color_id')]]['quantity'][] = $row[csf('woven_fin_receive')];
                        }
                        $tot_req_qnty = 0;
                        $tot_avl_qnty = 0;
                        $tot_bl_qnty = 0;
                        $row_span = '';
                        foreach ($fin_trans_arr as $key=>$color_id) {
                            if ($i % 2 == 0) $bgcolor = "#EFEFEF"; else $bgcolor = "#FFFFFF";
                            $row_span = count($color_id['recv_number']);
                            //echo $row_span; die;
                            $j=0;
                            for($k=0; $k<=$row_span; $k++){
                                if($j==0){
                            ?>
                            <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i.$j; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i.$j; ?>">
                                <td width="80" title="<? echo $j; ?>" align="center"><? echo $color_id['recv_number'][$k] ?></td>
                                <td width="40" align="center" rowspan="<? echo $row_span ?>"><? echo $colorArr[$key]; ?></td>
                                <td width="40" align="center"><? echo $color_id['quantity'][$k] ?></td>
                            </tr>
                            <?
                                    $tot_rec_qnty += $color_id['quantity'][$k];
                                }
                                else{ ?>
                                <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i.$j; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i.$j; ?>">
                                    <td width="60" title="<? echo $j; ?>" align="center"><? echo $color_id['recv_number'][$k] ?></td>
                                    <td width="40" align="center"><? echo $color_id['quantity'][$k] ?></td>
                                </tr>
                        ...
                        <tfoot>
                        <th colspan="2">&nbsp;</th>
                        <th align="center"><? echo number_format($tot_rec_qnty, 2); ?></th>
                        </tfoot>
                    </table>
            </fieldset>
        </div>
    ...
    if ($action == "cutting_finish_popup") {
        ...
        if ($type == 3) {
            $colorData = sql_select("select color_number_id, size_number_id, sum(plan_cut_qnty) as plan_cut_qnty, sum(order_quantity) as order_quantity from wo_po_color_size_breakdown where po_break_down_id in ($po_id) and country_id in($country_name_arr) group by color_number_id, size_number_id order by color_number_id, size_number_id");
        } else {
            $colorData = sql_select("select color_number_id, size_number_id, sum(plan_cut_qnty) as plan_cut_qnty,sum(order_quantity) as order_quantity from wo_po_color_size_breakdown where po_break_down_id in ($po_id) group by color_number_id, size_number_id order by color_number_id, size_number_id");
        ...
        <div style="width:100%" align="center">
            <fieldset style="width:<? echo $width; ?>px">
                <table width="600">
                    <?
                    if ($type == 3) {
                        $job_sql = sql_select("select a.job_no,a.buyer_name,a.company_name,a.style_ref_no,sum(c.order_quantity) as po_quantity,sum(c.plan_cut_qnty) as plan_cut from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and c.po_break_down_id=b.id and a.job_no='$job_number' and b.id in($po_id) and c.country_id in($country_name_arr) and b.po_quantity>0 and a.is_deleted=0 and a.status_active=1 and c.status_active=1 and c.status_active=1 group by a.job_no,a.buyer_name,a.company_name,a.style_ref_no");
                    } else {
                        $job_sql = sql_select("select a.job_no,a.buyer_name,a.company_name,a.style_ref_no,b.po_quantity,b.plan_cut from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.job_no='$job_number' and b.id in($po_id) and b.po_quantity>0  and a.is_deleted=0 and a.status_active=1 ");
						//echo "select a.job_no,a.buyer_name,a.company_name,a.style_ref_no,b.po_quantity,b.plan_cut from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.job_no='$job_number' and b.id in($po_id) and b.po_quantity>0 and a.is_deleted=0 and a.status_active=1 ";
                    }
					$plan_cut=$po_quantity=0;
                    foreach ($job_sql as $row_job)
					{
						$plan_cut+=$row_job[csf("plan_cut")];
						$po_quantity+=$row_job[csf("po_quantity")];
					}
                       // ;  // Master Job  table queery ends here
                    ?>
                    <tr class="form_caption">
                    ...
                    <table cellpadding="0" width="100%" class="rpt_table" rules="all" border="1">
                        ...
                        foreach ($color_id_array as $color_id) {
                            ...
                            <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_2<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_2<? echo $i; ?>">
                                <td width="130"><? echo $colorArr[$color_id]; ?></td>
                                <? foreach ($sizeArr_pend as $size_id) { ?>
                                    <td width="80" align="right" title="<? echo $qnty_array[$color_id][$size_id].'='.$ColorSizeArr[$color_id .$size_id];?>">&nbsp;<?
    ...
    if ($action == "print_emb_completed_popup") {
        ...
        <div id="details_reports">
            ...
            <table id="tbl_id" class="rpt_table" width="" border="1" rules="all" >
                ...
                <tr>
                    ...
                    <th width="100">Cutting Finished Qty</th>
                </tr>
                </thead>
                <?
        ...
        ?> 
                                ...
                                <td align="right"><? echo $balance = $row_total_color_qnty; ?></td> 
    ...
    if ($action == "sewing_finish_popup") {
        ...
        /* $sql = "select a.id,c.size_number_id,c.color_number_id,sum(b.production_qnty) as production_qnty
          ...
          where a.id=b.mst_id and b.color_size_break_down_id=c.id and a.po_break_down_id in ($po_id) and a.production_type=1 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 group by a.id,c.size_number_id,c.color_number_id"; */
        if ($type == 3) {
            $sql = "select c.size_number_id,c.color_number_id,sum(b.production_qnty) as production_qnty, a.production_source,a.serving_company as company_id
            from pro_garments_production_mst a, pro_garments_production_dtls b, wo_po_color_size_breakdown c
            where a.id=b.mst_id and b.color_size_break_down_id=c.id and a.po_break_down_id in ($po_id) and c.country_id in($country_name_arr) and a.production_type=5 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.status_active=1 and c.status_active=1 group by c.size_number_id,c.color_number_id order by c.color_number_id,c.size_number_id, a.production_source,a.serving_company";
        } else {
            $sql = "select c.size_number_id,c.color_number_id,sum(b.production_qnty) as production_qnty, a.production_source,a.serving_company as company_id
            from pro_garments_production_mst a, pro_garments_production_dtls b, wo_po_color_size_breakdown c
            where a.id=b.mst_id and b.color_size_break_down_id=c.id and a.po_break_down_id in ($po_id) and a.production_type=5 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 group by c.size_number_id,c.color_number_id, a.production_source,a.serving_company order by c.color_number_id,c.size_number_id";
        }
        ...
        foreach ($sqlResult as $row) {
            $index = $row[csf("color_number_id")] . $row[csf("size_number_id")];
            $ColorSizeArr[$index][$row[csf('production_source')]]['qnty'] += $row[csf("production_qnty")];
             if($row[csf('production_source')]==1)
            {
                 $color_source_wise_company[$row[csf("color_number_id")]][$row[csf('production_source')]]['company_id'].=$company_arr[$row[csf('company_id')]]."***";
            }
            else{
                 $color_source_wise_company[$row[csf("color_number_id")]][$row[csf('production_source')]]['company_id'].=$supplier_arr[$row[csf('company_id')]]."***";
            }
        ...
        <div style="width:100%" align="center">
            <fieldset style="width:98%">
                <table width="600">
                    <?
                    if ($type == 3) {
                        $job_sql = sql_select("select a.job_no,a.buyer_name,a.company_name,a.style_ref_no,sum(c.order_quantity) as po_quantity,sum(c.plan_cut_qnty) as plan_cut from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and c.po_break_down_id=b.id and a.job_no='$job_number' and b.id in($po_id) and c.country_id in($country_name_arr) and a.is_deleted=0  and b.po_quantity>0 and a.status_active=1 and c.status_active=1 and c.status_active=1 group by a.job_no,a.buyer_name,a.company_name,a.style_ref_no");
                    } else {
                        $job_sql = sql_select("select a.job_no,a.buyer_name,a.company_name,a.style_ref_no,b.po_quantity,b.plan_cut from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.job_no='$job_number' and b.id in($po_id)  and b.po_quantity>0 and a.is_deleted=0 and a.status_active=1 ");
                    }
                    foreach ($job_sql as $row_job)
                    ...
                    <tr>
                        ...
                        <td><? echo $po_quantity; ?> </td>
                        <td align="right"><strong>Cutting Qty.</strong> :</td>
                        <td>
                        <?
                         if ($type == 3) {
                            $sql = "select c.size_number_id,c.color_number_id,sum(b.production_qnty) as production_qnty
                        ...
                        foreach ($sqlResult as $row) {
                            $index = $row[csf("color_number_id")] . $row[csf("size_number_id")];
                            $ColorSizeArrr[$index] += $row[csf("production_qnty")];
                        }
                        ...
                        $qnty_arrray = array();
                        $sizeArrr_pend = array();
                        $color_id_arrray = array();
                        if ($type == 3) {
                            $coloorData = sql_select("select color_number_id, size_number_id, sum(plan_cut_qnty) as plan_cut_qnty from wo_po_color_size_breakdown where po_break_down_id in ($po_id) and country_id in($country_name_arr) group by color_number_id, size_number_id order by color_number_id, size_number_id");
                        } else {
                            $coloorData = sql_select("select color_number_id, size_number_id, sum(plan_cut_qnty) as plan_cut_qnty from wo_po_color_size_breakdown where po_break_down_id in ($po_id) group by color_number_id, size_number_id order by color_number_id, size_number_id");
                            //$colorData = sql_select("sum(plan_cut_qnty) as plan_cut_qnty","wo_po_color_size_breakdown","po_break_down_id in ($po_id) and color_number_id='".$poColorArr[$k]."'","plan_cut_qnty");	
                        }
                
                        foreach ($coloorData as $row) {
                            $qnty_arrray[$row[csf('color_number_id')]][$row[csf('size_number_id')]] += $row[csf('plan_cut_qnty')];
                            $sizeArrr_pend[$row[csf('size_number_id')]] = $row[csf('size_number_id')];
                            $color_id_arrray[$row[csf('color_number_id')]] = $row[csf('color_number_id')];
                        }
                       
                         foreach ($color_id_arrray as $color_id) {
                            ...
                            ?>
                                ...
                                <? foreach ($sizeArrr_pend as $size_id) { ?>
                                   
            <?
            $row_total_color_qnty += $ColorSizeArrr[$color_id . $size_id];
        }
        ?> 
                               
                                <? echo $balance =$row_total_color_qnty; ?>
                        ...
                        foreach ($color_id_array as $color_id => $totalcolorqnty) {
                            ...
                            /* if($type==3)
                              ...
                              $totalcolorqnty = return_field_value("sum(order_quantity) as order_quantity","wo_po_color_size_breakdown","po_break_down_id in ($po_id) and color_number_id='".$color_id."'","plan_cut_qnty");
                               ...
                               foreach ($sizeArr_pend as $size_id) 
                              {
                                 $row_total_color_qnty += $ColorSizeArr[$color_id . $size_id][1]['qnty'];
                                 $row_total_color_qnty += $ColorSizeArr[$color_id . $size_id][3]['qnty'];
                              }
                            ?>
                            <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                                <td width="70" >In-house</td>
                                <td width="120" style="word-break: break-all;"><? echo implode(", ", array_unique(explode("***", chop($color_source_wise_company[$color_id][1]['company_id'],"***"))));?></td>
                                <td width="100" ><? echo $colorArr[$color_id]; ?></td> 
                                <td width="80" align="right" rowspan="2"><? echo $totalcolorqnty; //$ColorvalueArr[$k];//[$poColorArr[$k].$poSizeArr[$j]]; ?></td> 
                                    <? foreach ($sizeArr_pend as $size_id) { ?>
                                    <td width="80" align="right" >&nbsp;<? echo $ColorSizeArr[$color_id . $size_id][1]['qnty']; ?></td> 
                                        <?$ColorSizeArr[$color_id . $size_id][3]['qnty'];
                                        $col_total_size_qnty[$size_id] += $ColorSizeArr[$color_id . $size_id][1]['qnty'];
                                    }
                                    ?> 
                                <td width="80" align="right" rowspan="2"><? echo $row_total_color_qnty; ?></td> 
                                <td  align="right" rowspan="2"><? echo $balance = $totalcolorqnty - $row_total_color_qnty; ?></td> 
                            </tr>
                            <?
                            ...
                            ?>
                             ...
                             <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                                <td width="70" >Out-bound</td>
                                <td width="120" style="word-break: break-all;"><? echo implode(", ", array_unique(explode("***", chop($color_source_wise_company[$color_id][3]['company_id'],"***"))));?></td>
                                <td width="100" ><? echo $colorArr[$color_id]; ?></td>  
                                    <? foreach ($sizeArr_pend as $size_id) { ?>
                                    <td width="80" align="right" >&nbsp;<? echo $ColorSizeArr[$color_id . $size_id][3]['qnty']; ?></td> 
                                        <?$ColorSizeArr[$color_id . $size_id][3]['qnty'];
                                       
                                        $col_total_size_qnty[$size_id] += $ColorSizeArr[$color_id . $size_id][3]['qnty'];
                                    }
                                    ?> 
                    ...
                    <table cellpadding="0" width="100%" class="rpt_table" rules="all" border="1">
                        ...
                        foreach ($color_id_array as $color_id => $totalcolorqnty) {
                            ...
                            <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_2<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_2<? echo $i; ?>">
                                ...
                                <? foreach ($sizeArr_pend as $size_id) { ?>
                                    <td width="80" align="right">&nbsp;<?
                                        $balance_qnty = $qnty_array[$color_id][$size_id] -($ColorSizeArr[$color_id . $size_id][1]['qnty']+$ColorSizeArr[$color_id . $size_id][3]['qnty']);
                                        echo $balance_qnty;
                                        ?></td>
                                    <?
                                    $row_total_color_qnty += $balance_qnty;
        ...
        <div style="width:100%" align="center">
            <fieldset style="width:<? echo $width; ?>px">
                <table width="600">
                    ...
                    {
                        ;  // Master Job  table queery ends here
                        $order_qty += $row_job[csf("po_quantity")];
                    }
                    ?>
                    <tr class="form_caption">
    ...
    if ($action == "finish_completed_popup") {
	 ...
	 if ($action == "wvn_finish_completed_popup") {
        echo load_html_head_contents("Finish Complete Details", "../../../../", 1, 1, $unicode, '', '');
        ...
        if ($type == 3) {
            $sql = "select c.size_number_id,c.color_number_id,sum(b.production_qnty) as production_qnty
            from pro_garments_production_mst a, pro_garments_production_dtls b, wo_po_color_size_breakdown c
            where a.id=b.mst_id and b.color_size_break_down_id=c.id and a.po_break_down_id in ($po_id) and c.country_id in($country_name_arr) and a.production_type=80 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.status_active=1 and c.status_active=1 group by c.size_number_id,c.color_number_id order by c.color_number_id,c.size_number_id";
        } else {
            $sql = "select c.size_number_id,c.color_number_id,sum(b.production_qnty) as production_qnty
            from pro_garments_production_mst a, pro_garments_production_dtls b, wo_po_color_size_breakdown c
            where a.id=b.mst_id and b.color_size_break_down_id=c.id and a.po_break_down_id in ($po_id) and a.production_type=80 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 group by c.size_number_id,c.color_number_id order by c.color_number_id,c.size_number_id";
        }
            ...
            <fieldset style="width:1170px">
                ...
                <div style="width:100%; max-height:300px; overflow-y:scroll">
                    <table cellpadding="0" width="100%" cellspacing="0" border="1" rules="all" class="rpt_table">
                        ...
                        /* $trims_recv_arr=return_library_array( "select a.item_group_id, sum(b.quantity) as recv_qnty from inv_trims_entry_dtls a, order_wise_pro_details b where a.id=b.dtls_id and b.po_breakdown_id in($po_id) and entry_form=24 and trans_type=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.item_group_id", "item_group_id", "recv_qnty");
                          $trims_iss_arr=return_library_array( "select a.item_group_id, sum(b.quantity) as iss_qnty from inv_trims_issue_dtls a, order_wise_pro_details b where a.id=b.dtls_id and b.po_breakdown_id in($po_id) and entry_form=25 and trans_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.item_group_id", "item_group_id", "iss_qnty"); */
                        ...
                        $sql = "select b.item_group_id, c.quantity as receive_qnty, b.amount, c.po_breakdown_id as order_id 
						from inv_receive_master a,inv_trims_entry_dtls b, order_wise_pro_details c 
						where a.id=b.mst_id and b.id=c.dtls_id and c.entry_form=24  and a.entry_form=24 and c.trans_type=1 and c.po_breakdown_id in($po_id) and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0";
                        $result = sql_select($sql);
                        foreach ($result as $row) 
						{
                        ...
                        $result = sql_select($sql);$tot_recv_qnty=$tot_req_value=$tot_recv_rec_amu=0;
                        foreach ($result as $row) {
                            if ($i % 2 == 0)
                                $bgcolor = "#E9F3FF";
                            else
                            ...
                            //$req_qnty=($poDataArr[$row[csf('po_break_down_id')]]['po_quantity']/$dzn_qnty)*$row[csf('cons')];
                            //$req_value=($poDataArr[$row[csf('po_break_down_id')]]['unit_price']/$dzn_qnty)*$req_qnty;
                            //$trims_qnty=($poDataArr[$row[csf("po_break_down_id")]][$row[csf('trim_group')]]['wo_qnty']/$dzn_qnty);
                            // $trims_value=($poDataArr[$row[csf("po_break_down_id")]][$row[csf('trim_group')]]['rate']/$dzn_qnty)*$trims_qnty;
                            ...
                            <tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
                                <td width="30"><? echo $i; ?></td>
                                <td><? echo $trim_group[$row[csf("trim_group")]]; ?></td>
                                <td width="60" align="center"><? echo $unit_of_measurement[$row[csf("cons_uom")]]; ?></td>
                                <td width="70" align="right"><? echo number_format($req_qnty, 2); ?></td>
                                <td width="70" align="right"><? echo number_format($req_value, 2); ?></td>
                                <td width="70" align="right"><? echo $booking_qty = $trimsDataArr[$row[csf("po_break_down_id")]][$row[csf('trim_group')]]['wo_qnty']; ?></td>
                                <td width="70" align="right"><? echo $booking_amu = $trimsDataArr[$row[csf("po_break_down_id")]][$row[csf('trim_group')]]['amount']; ?></td>
                                <td width="70"><? echo $trimsDataArr[$row[csf("po_break_down_id")]][$row[csf('trim_group')]]['booking_no']; ?></td>
                                <td width="70"><? echo $trimsDataArr[$row[csf("po_break_down_id")]][$row[csf('trim_group')]]['supplier_id']; ?></td>
                                <td width="70"><? echo $trimsDataArr[$row[csf("po_break_down_id")]][$row[csf('trim_group')]]['booking_date']; ?></td>
                                <td width="70"><? echo $trimsDataArr[$row[csf("po_break_down_id")]][$row[csf('trim_group')]]['delivery_date']; ?></td>
                                <td width="70" align="right"><?
                                    $rec_qty = $recDataArr[$row[csf('po_break_down_id')]][$row[csf('trim_group')]]['receive_qnty'];
                                    echo number_format($rec_qty, 2)
                                    ?></td>
                                <td width="70" align="right"><?
                                    $rec_amu = $recDataArr[$row[csf('po_break_down_id')]][$row[csf('trim_group')]]['amount'];
                                    echo number_format($rec_amu, 2)
                                    ?></td>
                                <td width="70" align="right"><?
                                    $rec_bal = $req_qnty - $recDataArr[$row[csf('po_break_down_id')]][$row[csf('trim_group')]]['receive_qnty'];
                                    echo number_format($rec_bal, 2);
                                    ?></td>
                                <td width="70" align="right"><?
                                    $issue_qty = $issueDataArr[$row[csf('po_break_down_id')]][$row[csf('trim_group')]]['issue_qnty'];
                                    echo number_format($issue_qty, 2);
                                    ?></td>
                                <td width="70" align="right"><?
                                    $issue_bal = $rec_qty  - $issue_qty;
                                    echo number_format($issue_bal, 2);
                                    ?></td>
                            </tr>
                            <?
                            $tot_booking_qnty += $booking_qty;
                            $tot_booking_amu += $booking_amu;
                            $tot_req_qnty += $req_qnty;
                            $tot_req_value += $req_value;

                            $tot_recv_qnty += $rec_qty;
                            $tot_recv_bl_qnty += $rec_bal;
                            $tot_recv_rec_amu += $rec_amu;

                            $tot_iss_qnty += $issue_qty;
                            $tot_iss_bl_qnty += $issue_bal;
    ...
    if ($action == "update_tna_progress_comment") {
        ...
        <fieldset style="width:1010px">
            ...
            <table style="margin-top:10px" width="1000" border="1" rules="all" class="rpt_table">
                <?php
                $date_diff_cond="(a.pub_shipment_date - a.po_received_date)";

                $sql = "select b.company_name,b.buyer_name,a.po_number,b.job_no,b.style_ref_no,b.gmts_item_id,a.po_received_date,a.shipment_date,set_smv,(po_quantity*total_set_qnty) as po_qty_pcs, $date_diff_cond as date_diff from wo_po_break_down a,wo_po_details_master b where a.id=$po_id and a.job_no_mst=b.job_no ";
                $result = sql_select($sql);
