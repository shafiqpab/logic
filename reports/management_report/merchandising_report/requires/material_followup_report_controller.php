...
if($action=="report_generate")
{
	...
	$lib_supplier_array=return_library_array( "select id, supplier_name from lib_supplier", "id", "supplier_name"  );
	...
	$int_ref   = str_replace( "'", "", $txt_internal_ref);
	...
	if(str_replace("'","",$txt_internal_ref)=="") $int_ref_cond=""; else $int_ref_cond=" and b.grouping=$txt_internal_ref";
	...
	//wo_pre_cost_fabric_cost_dtls c,wo_po_color_size_breakdown d, wo_pre_cos_fab_co_color_dtls e,wo_pre_cos_fab_co_avg_con_dtls

	// Main query =========pre_cost_fabric_cost_dtls_id,color_size_table_id
	...
	where a.id=b.job_id and c.po_break_down_id=b.id and  a.id=d.job_id and  d.job_id=b.job_id and  d.job_id=c.job_id and  c.po_break_down_id=b.id and  a.company_name=$company_name and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and d.is_deleted=0 and d.status_active=1   $date_cond $buyer_id_cond $jobcond $int_ref_cond $ordercond $ship_status_cond $style_ref_cond 
	...
	{		
		...
		if($row[csf("pay_mode")]==3 || $row[csf("pay_mode")]==5)//In house & within group
		{
			$com_supplier=$company_library[$row[csf("supplier_id")]];
			$booking_supp_arr[$row[csf("po_id")]][$row[csf("bpart_id")]][$row[csf("deter_id")]][$fb_fabric_color]['supp_com'].=$company_library[$row[csf("supplier_id")]].',';
		}
		else
		{
			$com_supplier=$lib_supplier_array[$row[csf("supplier_id")]];
			$booking_supp_arr[$row[csf("po_id")]][$row[csf("bpart_id")]][$row[csf("deter_id")]][$fb_fabric_color]['supp_com'].=$lib_supplier_array[$row[csf("supplier_id")]].',';
		}
	...
	$sql="select b.pi_id,a.id,a.item_category_id,a.pi_number,a.pi_date,a.importer_id, b.work_order_no,b.work_order_id,b.item_group,b.item_prod_id, b.determination_id from com_pi_master_details  a,  com_pi_item_details b where a.id=b.pi_id and a.importer_id=$cbo_company_name   and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $book_cond_for_in";
	//echo $sql ; // die;
						...
						foreach($body_data as $deter_id=>$deter_data)
						{
							...
							foreach($deter_data as $fabric_color=>$source_data)
							{
								foreach($source_data as $source_id=>$val)
								{
									...
									$exPoIds=array_filter(array_unique(explode("**",$val["po_id"])));
									foreach($exPoNos as $pono)
									{
										$jobPoArr[$job_no][$pono]=$pono;
									}
									foreach($exPoIds as $poId)
									{
										// $fabric_req_knitTT=$fabric_req_arr['knit']['grey'][$poId][$bprat_id][$deter_id][$fabric_color][27];
											//  echo $fabric_req_knitTT.'T ';
									}
								}
							}
					  	...
					  	foreach($body_data as $deter_id=>$deter_data)
					  	{
							...
							foreach($deter_data as $fabric_color=>$source_data)
							{
								foreach($source_data as $source_id=>$val)
								{
									...
									//{
										// echo $gmt_color.'<br>';
										$tot_fabric_req_knit=0;$tot_fabric_req_wov_cost=0;
										foreach($po_ids as $pId)
										{
											 
											$tot_fabric_req_knit+=$fabric_req_arr['woven']['grey'][$pId][$bprat_id][$deter_id][$fabric_color][$val["fab_uom"]]+$fabric_req_arr['knit']['grey'][$pId][$bprat_id][$deter_id][$fabric_color][$val["fab_uom"]];
											$tot_fabric_req_wov_cost+=$fabric_req_cost_arr['woven']['grey'][$pId][$bprat_id][$deter_id][$fabric_color][$val["fab_uom"]]+$fabric_req_cost_arr['knit']['grey'][$pId][$bprat_id][$deter_id][$fabric_color][$val["fab_uom"]];
									...
									$tot_fabric_req=$tot_fabric_req_knit;//$val["pre_fab_req_qty"];
									$tot_fabric_req_cost=$tot_fabric_req_wov_cost;//$val["pre_fab_req_amt"];
									...
									?>
							 		<tr bgcolor = "<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" style="font-size:13px">
										...
										echo implode(",",array_unique(explode(",",$supp_com)));//Issue Id-631
										?></div></td>
    ...
    if(str_replace("'","",$cbo_search_by)==1) //Order Wise
	{
		if($template==1)
		{
			...
			<table class="rpt_table" width="<?=$tbl_width;?>" cellpadding="0" cellspacing="0" border="1" rules="all">
             <caption> <b style="float:left">Accessories1</b></caption>
			 <thead>
			 <tr>
					<th width="100" colspan="38">&nbsp;</th>     
			...
			where
			a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and a.company_name=$company_name $buyer_id_cond ".set_user_lavel_filtering(' and a.buyer_name','buyer_id')." $date_cond $style_ref_cond $jobcond $int_ref_cond $ordercond $file_no_cond $internal_ref_cond $year_cond $season_con
		...
		if($template==1)
		{
		 ...
		 $sql_pos=("select a.buyer_name, a.job_no, a.job_no_prefix_num, a.style_ref_no, a.order_uom, b.file_no, b.grouping, b.id, b.po_number, sum(c.order_quantity) as order_quantity, sum(c.order_quantity/a.total_set_qnty) as order_quantity_set, b.pub_shipment_date , a.season_buyer_wise
			...
			foreach($sql_po as $row)
			{
				...
				$po_id_string.=$row[csf('id')].",";
				$int_ref_cond1=$row[csf('grouping')];
			...
			// else 
			// {
            //     $int_ref_cond2=" and e.grouping='$int_ref_cond1'";
			// }
