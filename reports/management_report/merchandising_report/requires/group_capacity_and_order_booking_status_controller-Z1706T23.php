...
$location_arr=return_library_array( "select id,location_name from lib_location where is_deleted=0",'id','location_name');
...
{
	echo create_drop_down( "cbo_location_name", 110, "select id,location_name from lib_location where company_id in($data) and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select --", $selected, "" );
	exit();	 
}
...
{
	echo create_drop_down( "cbo_factory_merchant", 130, "select id,team_member_name from lib_mkt_team_member_info where team_id='$data' and status_active =1 and is_deleted=0 order by team_member_name","id,team_member_name", 1, "-- Select --", $selected, "" );
}
...
if($type=="report_generate_gr")
{
	...
	if(trim($data[12])=="0") $agent="%%"; else $agent="$data[12]";
	if(trim($data[13])=="0") $location=""; else $location=' and a.location_name='.$data[13];
	...
	else if($cbo_category_by==5)
	{
		...
		if ($start_date!="" && $end_date!="")
		{
			if($db_type==0)
			{
				$date_cond=" and b.extended_ship_date between '".$start_date."' and '".$end_date."'";
			}
			else
			{
				$date_cond=" and b.extended_ship_date between '".$start_date."' and '".$end_date."'";
	...
	//echo $date_cond.'=='.$cbo_category_by;die;
	...
	if($db_type==2)
	{
    $sql_con="SELECT  a.company_id,b.buyer_id";
	}
	...
	c.id=d.mst_id AND 
	a.company_id in($company_name)   AND 
	c.comapny_id in($company_name) AND
	a.year_id=$sy and 
	...
	c.is_deleted=0  
	GROUP BY a.company_id,b.buyer_id";
...
{
	$exfactory_data=sql_select("select po_break_down_id,sum(ex_factory_qnty) as ex_factory_qnty, MAX(ex_factory_date) as ex_factory_date from pro_ex_factory_mst  where status_active=1 and is_deleted=0 group by po_break_down_id");
	foreach($exfactory_data as $exfatory_row)
	{
...
$data_array=sql_select("select a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.agent_name, a.style_ref_no, a.style_description, a.job_quantity, a.product_category, a.job_no, a.location_name, a.gmts_item_id, a.total_set_qnty, a.order_uom, a.team_leader, a.dealing_marchant,a.factory_marchant,a.product_code, b.id, b.is_confirmed, b.po_number, sum(c.order_quantity/a.total_set_qnty) as po_quantity, sum(c.order_quantity) as po_quantity_pcs,b.shipment_date, b.pub_shipment_date,b.extended_ship_date, b.po_received_date, DATEDIFF(c.country_ship_date, '$date') date_diff_1, DATEDIFF(c.country_ship_date, '$date') date_diff_2, b.unit_price, sum(c.order_total) as  order_total, b.details_remarks, b.delay_for, b.grouping,b.file_no,c.shiping_status,c.country_remarks,c.country_ship_date,c.country_id,  b.t_year, b.t_month  from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id   and a.company_name in($company_name) and a.buyer_name like '$buyer_name' and LOWER(a.style_ref_no) like LOWER('$style_ref_cond') and a.team_leader like '$team_leader'  and a.dealing_marchant like '$dealing_marchant' and a.factory_marchant like  '$factory_marchant' and a.agent_name like '$agent'  $date_cond  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 group by a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.agent_name, a.style_ref_no, a.style_description, a.job_quantity, a.product_category, a.job_no, a.location_name, a.gmts_item_id, a.total_set_qnty, a.order_uom, a.team_leader, a.dealing_marchant,a.factory_marchant,a.product_code,b.id,b.is_confirmed,b.po_number,b.extended_ship_date,b.shipment_date,b.pub_shipment_date,b.po_received_date,b.unit_price,b.details_remarks, b.delay_for,b.grouping,b.file_no,b.t_year, b.t_month,c.country_id,c.country_remarks,c.country_ship_date,c.shiping_status order by c.country_ship_date,a.job_no_prefix_num,b.id");
	}
	else if($cbo_category_by==2)
	{
	 $data_array=sql_select("select a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.agent_name, a.style_ref_no, a.style_description, a.job_quantity, a.product_category, a.job_no, a.location_name, a.gmts_item_id, a.total_set_qnty, a.order_uom, a.team_leader, a.dealing_marchant,a.factory_marchant,a.product_code, b.id, b.is_confirmed, b.po_number, sum(b.po_quantity) as po_quantity, sum(b.po_quantity*a.total_set_qnty) as po_quantity_pcs,b.shiping_status,b.shipment_date,b.extended_ship_date, b.pub_shipment_date, b.po_received_date, DATEDIFF(b.pub_shipment_date, '$date') date_diff_1, DATEDIFF(b.pub_shipment_date, '$date') date_diff_2, b.unit_price, b.po_total_price as order_total, b.details_remarks, b.delay_for, b.grouping,b.file_no, b.t_year, b.t_month  from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.company_name in($company_name) and a.buyer_name like '$buyer_name' and LOWER(a.style_ref_no) like LOWER('$style_ref_cond') and a.team_leader like '$team_leader'  and a.dealing_marchant like '$dealing_marchant' and a.factory_marchant like  '$factory_marchant' and a.agent_name like '$agent' $location $date_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.agent_name, a.style_ref_no, a.style_description, a.job_quantity, a.product_category, a.job_no, a.location_name, a.gmts_item_id, a.total_set_qnty, a.order_uom, a.team_leader, a.dealing_marchant,a.factory_marchant,a.product_code,b.id,b.is_confirmed,b.po_number,b.shipment_date,b.extended_ship_date,b.pub_shipment_date,b.po_received_date,b.unit_price,b.po_total_price,b.details_remarks, b.delay_for,b.grouping,b.file_no,b.t_year, b.t_month order by b.pub_shipment_date,a.job_no_prefix_num,b.id");
	}
	else
	{
	 $data_array=sql_select("select a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.agent_name, a.style_ref_no, a.style_description, a.job_quantity, a.product_category, a.job_no, a.location_name, a.gmts_item_id, a.total_set_qnty, a.order_uom, a.team_leader, a.dealing_marchant,a.factory_marchant,a.product_code, b.id, b.is_confirmed, b.po_number, sum(b.po_quantity) as po_quantity, sum(b.po_quantity*a.total_set_qnty) as po_quantity_pcs,b.shiping_status,b.shipment_date,b.extended_ship_date, b.pub_shipment_date, b.po_received_date, DATEDIFF(b.shipment_date, '$date') date_diff_1, DATEDIFF(b.extended_ship_date, '$date') date_diff_14, DATEDIFF(b.shipment_date, '$date') date_diff_2, b.unit_price, b.po_total_price as order_total, b.details_remarks, b.delay_for, b.grouping,b.file_no, b.t_year, b.t_month  from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.company_name in($company_name) and a.buyer_name like '$buyer_name' and LOWER(a.style_ref_no) like LOWER('$style_ref_cond') and a.team_leader like '$team_leader'  and a.dealing_marchant like '$dealing_marchant' and a.factory_marchant like  '$factory_marchant' and a.agent_name like '$agent' $date_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.agent_name, a.style_ref_no, a.style_description, a.job_quantity, a.product_category, a.job_no, a.location_name, a.gmts_item_id, a.total_set_qnty, a.order_uom, a.team_leader, a.dealing_marchant,a.factory_marchant,a.product_code,b.id,b.is_confirmed,b.po_number,b.shipment_date,b.extended_ship_date,b.pub_shipment_date,b.po_received_date,b.unit_price,b.po_total_price,b.details_remarks, b.delay_for,b.grouping,b.file_no,b.t_year, b.t_month  order by b.pub_shipment_date,a.job_no_prefix_num,b.id");
	}
...
if($cbo_category_by==1)
	...
	$data_array=sql_select("select a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.agent_name, a.style_ref_no, a.style_description, a.job_quantity, a.product_category, a.job_no, a.location_name, a.gmts_item_id, a.total_set_qnty, a.order_uom, a.team_leader, a.dealing_marchant,a.factory_marchant,a.product_code, b.id, b.is_confirmed, b.po_number, sum(c.order_quantity/a.total_set_qnty) as po_quantity, sum(c.order_quantity) as po_quantity_pcs,sum(c.order_total) as order_total,b.shipment_date,  b.pub_shipment_date,b.extended_ship_date, b.po_received_date, (c.country_ship_date - to_date('$date','dd-mm-yyyy')) date_diff_1, (c.country_ship_date - to_date('$date','dd-mm-yyyy')) date_diff_2, b.unit_price, b.po_total_price, b.details_remarks, b.delay_for,b.grouping,b.file_no, c.shiping_status,c.country_remarks,c.country_ship_date,c.country_id,  b.t_year, b.t_month  from wo_po_details_master a, wo_po_break_down b, wo_po_color_size_breakdown c where a.job_no=b.job_no_mst and a.job_no=c.job_no_mst and b.id=c.po_break_down_id   and a.company_name in($company_name) and a.buyer_name like '$buyer_name' and LOWER(a.style_ref_no) like LOWER('$style_ref_cond') and a.team_leader like '$team_leader'  and a.dealing_marchant like '$dealing_marchant' and a.factory_marchant like  '$factory_marchant' and a.agent_name like '$agent' $date_cond  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0   group by a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.agent_name, a.style_ref_no, a.style_description, a.job_quantity, a.product_category, a.job_no, a.location_name, a.gmts_item_id, a.total_set_qnty, a.order_uom, a.team_leader, a.dealing_marchant,a.factory_marchant,a.product_code,b.id,b.is_confirmed,b.po_number,b.extended_ship_date,b.shipment_date,b.pub_shipment_date,b.po_received_date,b.unit_price,b.po_total_price,b.details_remarks, b.delay_for,b.grouping,b.file_no,b.t_year, b.t_month,c.country_id,c.country_remarks,c.country_ship_date,c.shiping_status order by c.country_ship_date,a.job_no_prefix_num,b.id");
	}
	else if($cbo_category_by==2)
	{
		
		//echo "select a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.agent_name, a.style_ref_no, a.style_description, a.job_quantity, a.product_category, a.job_no, a.location_name, a.gmts_item_id, a.total_set_qnty, a.order_uom, a.team_leader, a.dealing_marchant,a.product_code, b.id, b.is_confirmed, b.po_number, sum(b.po_quantity) as po_quantity, sum(b.po_quantity*a.total_set_qnty) as po_quantity_pcs,b.shiping_status, b.pub_shipment_date, b.po_received_date, (b.pub_shipment_date - to_date('$date','dd-mm-yyyy')) date_diff_1, (b.pub_shipment_date - to_date('$date','dd-mm-yyyy')) date_diff_2, b.unit_price, b.po_total_price as order_total, b.details_remarks,b.grouping,b.file_no, b.shiping_status as shiping_status2,b.details_remarks as country_remarks, b.t_year, b.t_month  from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.company_name like '$company_name' and a.buyer_name like '$buyer_name' and a.team_leader like '$team_leader'  and a.dealing_marchant like '$dealing_marchant' $date_cond  and a.status_active=1 and a.is_deleted=0 and b.status_active=1  and b.is_deleted=0 group by a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.agent_name, a.style_ref_no, a.style_description, a.job_quantity, a.product_category, a.job_no, a.location_name, a.gmts_item_id, a.total_set_qnty, a.order_uom, a.team_leader, a.dealing_marchant,a.product_code,b.id,b.is_confirmed,b.po_number,b.shiping_status,b.pub_shipment_date,b.po_received_date,b.unit_price,b.po_total_price,b.details_remarks,b.grouping,b.file_no,b.t_year, b.t_month order by b.pub_shipment_date, b.id";
	$data_array=sql_select("select a.job_no_prefix_num, a.job_no, a.company_name,a.buyer_name, a.agent_name, a.style_ref_no, a.style_description, a.job_quantity, a.product_category, a.job_no, a.location_name, a.gmts_item_id, a.total_set_qnty, a.order_uom, a.team_leader, a.dealing_marchant,a.factory_marchant,a.product_code, b.id, b.is_confirmed, b.po_number, sum(b.po_quantity) as po_quantity, sum(b.po_quantity*a.total_set_qnty) as po_quantity_pcs,b.shiping_status,b.shipment_date,b.extended_ship_date, b.pub_shipment_date, b.po_received_date, (b.pub_shipment_date - to_date('$date','dd-mm-yyyy')) date_diff_1, (b.pub_shipment_date - to_date('$date','dd-mm-yyyy')) date_diff_2, b.unit_price, b.po_total_price as order_total, b.details_remarks, b.delay_for,b.grouping,b.file_no, b.shiping_status as shiping_status2, b.t_year, b.t_month  from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.company_name in($company_name) and a.buyer_name like '$buyer_name' and LOWER(a.style_ref_no) like LOWER('$style_ref_cond') and a.team_leader like '$team_leader'  and a.dealing_marchant like '$dealing_marchant' and a.factory_marchant like  '$factory_marchant' and a.agent_name like '$agent' $location $date_cond  and a.status_active=1 and a.is_deleted=0 and b.status_active=1  and b.is_deleted=0 group by a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.agent_name, a.style_ref_no, a.style_description, a.job_quantity, a.product_category, a.job_no, a.location_name, a.gmts_item_id, a.total_set_qnty, a.order_uom, a.team_leader, a.dealing_marchant,a.factory_marchant,a.product_code,b.id,b.is_confirmed,b.po_number,b.shiping_status,b.extended_ship_date,b.shipment_date,b.pub_shipment_date,b.po_received_date,b.unit_price,b.po_total_price,b.details_remarks, b.delay_for,b.grouping,b.file_no,b.t_year, b.t_month order by b.pub_shipment_date,a.job_no_prefix_num,b.id");
	...
	$data_array=sql_select("select a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.agent_name, a.style_ref_no, a.style_description, a.job_quantity, a.product_category, a.job_no, a.location_name, a.gmts_item_id, a.total_set_qnty, a.order_uom, a.team_leader, a.dealing_marchant,a.factory_marchant,a.product_code, b.id, b.is_confirmed, b.po_number, sum(b.po_quantity) as po_quantity, sum(b.po_quantity*a.total_set_qnty) as po_quantity_pcs,b.shiping_status,b.shipment_date,b.extended_ship_date, b.pub_shipment_date, b.po_received_date, (b.shipment_date - to_date('$date','dd-mm-yyyy')) date_diff_1,(b.extended_ship_date - to_date('$date','dd-mm-yyyy')) date_diff_14, (b.shipment_date - to_date('$date','dd-mm-yyyy')) date_diff_2, b.unit_price, b.po_total_price as order_total, b.details_remarks, b.delay_for,b.grouping,b.file_no, b.shiping_status as shiping_status2, b.t_year, b.t_month  from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.company_name in($company_name) and a.buyer_name like '$buyer_name' and LOWER(a.style_ref_no) like LOWER('$style_ref_cond') and a.team_leader like '$team_leader'  and a.dealing_marchant like '$dealing_marchant' and a.factory_marchant like  '$factory_marchant' and a.agent_name like '$agent' $date_cond  and a.status_active=1 and a.is_deleted=0 and b.status_active=1  and b.is_deleted=0 group by a.job_no_prefix_num, a.job_no, a.company_name, a.buyer_name, a.agent_name, a.style_ref_no, a.style_description, a.job_quantity, a.product_category, a.job_no, a.location_name, a.gmts_item_id, a.total_set_qnty, a.order_uom, a.team_leader, a.dealing_marchant,a.factory_marchant,a.product_code,b.id,b.is_confirmed,b.po_number,b.shiping_status,b.shipment_date,b.extended_ship_date,b.pub_shipment_date,b.po_received_date,b.unit_price,b.po_total_price,b.details_remarks, b.delay_for,b.grouping,b.file_no,b.t_year, b.t_month order by b.pub_shipment_date,a.job_no_prefix_num,b.id");
	//$data_array=sql_select($sql); //a.job_no_prefix_num,b.id 
	}
}
//========================================================================================
...
$job_smv_sql=sql_select("select job_no, smv_set, finsmv_set from wo_po_details_mas_set_details where 1=1 $job_cond_for_in");
$job_smv_arr=array();
foreach($job_smv_sql as $row)
{
	$job_smv_arr[$row[csf("job_no")]]["smv_pcs"]+=$row[csf("smv_set")];
	$job_smv_arr[$row[csf("job_no")]]["finsmv_pcs"]+=$row[csf("finsmv_set")];
}
unset($job_smv_sql);
//================================================================================
...
if($cbo_category_by==1)
{
	...
	MAX(ex_factory_date) as ex_factory_date from pro_ex_factory_mst  where 1=1 $po_cond_for_in and status_active=1 and is_deleted=0 group by po_break_down_id,country_id");
	foreach($exfactory_data as $exfatory_row)
	{
...
else
{
	...
	sum(CASE WHEN entry_form!=85 THEN ex_factory_qnty ELSE 0 END) as ex_factory_qnty,
	sum(CASE WHEN entry_form=85 THEN ex_factory_qnty ELSE 0 END) as return_qnty, 
	MAX(ex_factory_date) as ex_factory_date from pro_ex_factory_mst  where 1=1 $po_cond_for_in and status_active=1 and is_deleted=0 group by po_break_down_id");
	foreach($exfactory_data as $exfatory_row)
	{
...
foreach ($data_array as $row)
{
 ...
 $company_buyer_array[$row[csf("company_name")]][$row[csf("buyer_name")]][smv]+=($job_smv_arr[$row[csf("job_no")]]["smv_pcs"])*$row[csf('po_quantity')];
 if($precost_smv_arr[$row[csf("job_no")]] !="" || $job_smv_arr[$row[csf("job_no")]]["smv_pcs"] !=0)
 {
	$booked_basic_qnty=($row[csf("po_quantity")]*($job_smv_arr[$row[csf("job_no")]]["smv_pcs"]))/$basic_smv_arr[$row[csf("company_name")]];
...
$company_buyer_array_confirm[$row[csf("company_name")]][$row[csf("buyer_name")]][smv]+=($job_smv_arr[$row[csf("job_no")]]["smv_pcs"])*$row[csf('po_quantity')];
if($precost_smv_arr[$row[csf("job_no")]] !="" || $job_smv_arr[$row[csf("job_no")]]["smv_pcs"] !=0)
 {
	$booked_basic_qnty_c=($row[csf("po_quantity")]*($job_smv_arr[$row[csf("job_no")]]["smv_pcs"]))/$basic_smv_arr[$row[csf("company_name")]];
...
$company_buyer_array_projected[$row[csf("company_name")]][$row[csf("buyer_name")]][smv]+=($job_smv_arr[$row[csf("job_no")]]["smv_pcs"])*$row[csf('po_quantity')];
if($precost_smv_arr[$row[csf("job_no")]] !="" || $job_smv_arr[$row[csf("job_no")]]["smv_pcs"] !=0)
 {
	$booked_basic_qnty_p=($row[csf("po_quantity")]*($job_smv_arr[$row[csf("job_no")]]["smv_pcs"]))/$basic_smv_arr[$row[csf("company_name")]];
...
<th width="50">Buyer Share on Ord Value % </th>
...
foreach ($company_buyer_array as $key=>$value)
{
	...
	foreach($value as $buyer=> $value1)
	{
		if($zero_value==0)
		{
		if($value1[order_qty_pcs]>0 || $target_basic_qnty[$key][$buyer] > 0 || $value1[booked_basic_qnty] > 0 || $value1[ex_factory] > 0  || $value1[order_value] > 0)
		{
$i++;
if ($i%2==0)  
$bgcolor="#E9F3FF";
...
<? 
$exfactory_value=$value1[ex_factory]*($value1[order_value]/$value1[order_qty_pcs]);
$tot_exfactory_value+=$exfactory_value;
echo fn_number_format($exfactory_value,0);
?>
</td> 
<td width="100" align="right">
<? 
...
<?  echo fn_number_format(($target_basic_qnty[$key][$buyer]/$total_target_basic_qnty)*100,2); $percent_total+=($target_basic_qnty[$key][$buyer]/$total_target_basic_qnty)*100; ?>
</td>
<td width="100" align="right" bgcolor="#CCCCCC"><? echo number_format($value1[booked_basic_qnty],0);?></td>
<td width="50" align="right" bgcolor="#CCCCCC"><? echo fn_number_format(($value1[booked_basic_qnty]/$target_basic_qnty[$key][$buyer])*100,2); ?></td>
<td width="80" align="right" bgcolor="#CCCCCC"><? echo number_format($target_basic_qnty[$key][$buyer]-$value1[booked_basic_qnty],0);?></td>
<td width="50" align="right" bgcolor="#CCCCCC"><? echo fn_number_format((($target_basic_qnty[$key][$buyer]-$value1[booked_basic_qnty])/$target_basic_qnty[$key][$buyer])*100,2);?></td> 
...
<?
		...
		else
		{
			...
			<? 
			$exfactory_value=$value1[ex_factory]*($value1[order_value]/$value1[order_qty_pcs]);
			echo number_format($exfactory_value,0);
			?>
            </td> 
			<td width="100" align="right">
			<? 
            ...
            <td width="100" align="right" bgcolor="#CCCCCC"><? echo number_format($target_basic_qnty[$key][$buyer],0); $target_basic_qnty_tot+=$target_basic_qnty[$key][$buyer];?></td>
			<td width="50" align="right" bgcolor="#CCCCCC"><?  echo fn_number_format(($target_basic_qnty[$key][$buyer]/$total_target_basic_qnty)*100,2); $percent_total+=($target_basic_qnty[$key][$buyer]/$total_target_basic_qnty)*100; ?></td>
			<td width="100" align="right" bgcolor="#CCCCCC"><? echo number_format($value1[booked_basic_qnty],0);?></td>
			<td width="50" align="right" bgcolor="#CCCCCC"><? echo fn_number_format(($value1[booked_basic_qnty]/$target_basic_qnty[$key][$buyer])*100,2); ?></td>
			<td width="80" align="right" bgcolor="#CCCCCC"><? echo number_format($target_basic_qnty[$key][$buyer]-$value1[booked_basic_qnty],0);?></td>
			<td width="50" align="right" bgcolor="#CCCCCC"><? echo fn_number_format((($target_basic_qnty[$key][$buyer]-$value1[booked_basic_qnty])/$target_basic_qnty[$key][$buyer])*100,2);?></td>
...
<th width="100"><? echo number_format($tot_exfactory_value,0); ?></th> 
...
<th width="50"><? echo fn_number_format($percent_total,2); ?></th>
<th width="100"><? echo number_format($booked_basic_qnty_tot,0); ?></th>
<th width="50"><? echo fn_number_format($booked_basic_qnty_tot/$target_basic_qnty_tot*100,2); ?></th>
<th width="80"><? echo number_format($target_basic_qnty_tot-$booked_basic_qnty_tot,0); ?></th>
<th width="50"><? echo fn_number_format((($target_basic_qnty_tot-$booked_basic_qnty_tot)/$target_basic_qnty_tot)*100,2); ?></th>
...
<th width="50">&nbsp; </th>
<th width="100">&nbsp;</th> 
<th width="100">Short Shipment</th> 
...
<td width="" colspan="2" align="right"><?  echo fn_number_format($po_total_price_tot_p/$quantity_tot_p,2); ?> </td>
<td width="" colspan="2" align="right"><?  echo fn_number_format($po_total_price_tot_p/$booked_basic_qnty_tot_p,2); ?> </td>
...
<th colspan="9">Confirmed Order - <?  echo date('M,Y',strtotime($start_date));?></th>
</tr>
<tr>
<th width="50">SL</th>
<th width="70">Company Name</th>
...
<th colspan="9">Projected Order - <?  echo date('M,Y',strtotime($start_date));?></th>
</tr>
<tr>
<th width="50">SL</th>
<th width="70">Company Name</th>
...
<h3 align="left" id="accordion_h4" class="accordion_h" onClick="accordion_menu( this.id,'content_report_panel', '')"> -Report Panel </h3>
            <div id="content_report_panel"> 
                <table width="4320" id="table_header_1" border="1" class="rpt_table" rules="all">
                    <thead>
                        <tr>
                            <th width="50">SL</th>
                            ...
                            <th  width="100"><u>Pord Code</u><hr>TNA</th>
                            <th width="30">Img</th>
                            <th width="30">File</th>
                            ...
                            <th  width="100">Last Ex-Factory Date</th>
                            ...
                            <th  width="100">Sewing SMV</th>
                            <th  width="100">Iron SMV</th>
                            <th  width="100">Total Sewing SMV</th>
                            <th  width="100">Total Iron SMV</th>
                            
                            <th width="100">Eqv. Basic Qty. (Pcs)</th>
                            <th width="90">Ex-Fac Qnty (Pcs) </th>
                             <th width="90">Ex-Fac Value (USD) </th>
                ...
                <div style=" max-height:400px; overflow-y:scroll; width:4320px"  align="left" id="scroll_body">
                    <table width="4300" border="1" class="rpt_table" rules="all" id="table-body">
                    <?
					$buyer_wise_performance=array();
                    ...
                    { 
						...
						{
						$ex_factory_date=$exfactory_data_array[$row[csf('id')]][ex_factory_date];
						$date_diff_3=datediff( "d", $ex_factory_date , $row[csf('extended_ship_date')]);
						$date_diff_4=datediff( "d", $ex_factory_date , $row[csf('extended_ship_date')]);
						}
						else
						{
						$ex_factory_date=$exfactory_data_array[$row[csf('id')]][ex_factory_date];
						$date_diff_3=datediff( "d", $ex_factory_date , $row[csf('shipment_date')]);
						...
						//--Color Determination-------------
						//================================== date_diff_11
						if($cbo_category_by==5)
						{
							$date_diff_1=$row[csf('date_diff_14')];
						}
						else $date_diff_1=$row[csf('date_diff_1')];
						
						$shipment_performance=0;
						if($row[csf('shiping_status')]==1 && $date_diff_1>10 )
						{
						$color="";	
						$number_of_order['yet']+=1;
						$shipment_performance=0;
						}
						if($row[csf('shiping_status')]==1 && ($date_diff_1<=10 && $date_diff_1>=0))
						{
						$color="orange";
						$number_of_order['yet']+=1;
						$shipment_performance=0;
						}
						if($row[csf('shiping_status')]==1 &&  $date_diff_1<0)
						{
						$color="red";	
						$number_of_order['yet']+=1;
						$shipment_performance=0;
						...
						//=====================================
						if($row[csf('shiping_status')]==2 && $date_diff_1>10 )
						{
						$color="";	
						}
						if($row[csf('shiping_status')]==2 && ($date_diff_1<=10 && $date_diff_1>=0))
						{
						$color="orange";	
						}
						if($row[csf('shiping_status')]==2 &&  $date_diff_1<0)
						{
						$color="red";	
						}
						...
						$number_of_order['ontime']+=1;
						$shipment_performance=1;
						$buyer_wise_performance[$row[csf('buyer_name')]]['ontime']+=1;	
						}
						if($row[csf('shiping_status')]==3 && $date_diff_4<0)
						{
						$number_of_order['after']+=1;
						...
						if($cbo_category_by==1)
						{
							...
							$country_remarks="";
							if($row[csf('details_remarks')] !="" && $row[csf('country_remarks')] !="")
							{
							$country_remarks.=$row[csf('country_remarks')]." ,".$row[csf('details_remarks')];
							}
							else if($row[csf('details_remarks')] !="" && $row[csf('country_remarks')] =="")
							{
								$country_remarks.=$row[csf('details_remarks')];
							}
							else if($row[csf('details_remarks')] =="" && $row[csf('country_remarks')] !="")
							{
								$country_remarks.=$row[csf('country_remarks')];
							}
						}
						else if($cbo_category_by==2)
						{
							...
							$ship_date=$row[csf('pub_shipment_date')];
						 	$country_remarks=$row[csf('details_remarks')];
							//$remark=$row[csf('country_remarks')];
						}
						else if($cbo_category_by==5)
						{
							$country_name="";
							...
							$ship_date=$row[csf('extended_ship_date')];
						 	$country_remarks=$row[csf('details_remarks')];
							//$remark=$row[csf('country_remarks')];
						}
						else
						{
							...
							$ship_date=$row[csf('shipment_date')];
							
						 	$country_remarks=$row[csf('details_remarks')];
						}
						$buyer_wise_performance[$row[csf('buyer_name')]]['total_po']+=1;	
						?>
							<tr bgcolor="<? echo $bgcolor; ?>" style="vertical-align:middle" height="25" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>" >
								...
								<td width="100" align="center" style="word-wrap: break-word;word-break: break-all;"><? echo $location_arr[$row[csf('location_name')]];?></td>
                                ...
                                <td  width="100" align="center" style="word-wrap: break-word;word-break: break-all;"> <? echo $buyer_full_name_arr[$row[csf('agent_name')]];?></td>
                                <td  width="150" align="center" style="word-wrap: break-word;word-break: break-all;">
                                <font style="color:<? echo $color_font; ?>">
                                
                                	<a href='#report_details' onclick="order_dtls_popup('<? echo $row[csf('job_no')]; ?>','<? echo $row[csf('id')]; ?>','<? echo $template_id; ?>','<? echo $tna_process_type; ?>');"><? echo $row[csf('po_number')];  ?></a><br />
                                ...
                                <td width="30" onclick="openmypage_image('requires/group_capacity_and_order_booking_status_controller.php?action=show_image&job_no=<? echo $row[csf("job_no")] ?>','Image View')" style="word-wrap: break-word;word-break: break-all;"><img  src='../../../<? echo $imge_arr[$row[csf('job_no')]]; ?>' height='25' width='30' /></td>
                                <td width="30">
                                ...
                                <td width="30" align="right">
                                 ...
                                 <td width="100" align="center" style="word-wrap: break-word;word-break: break-all;">
								 <? echo change_date_format($ex_factory_date,'dd-mm-yyyy','-');?></td>
                                <td  width="50" align="center" title="<? echo $row[csf('shiping_status')];?>" bgcolor="<? echo $color; ?>" style="word-wrap: break-word;word-break: break-all;"> 
                                ...
                                <td width="100" align="right" style="word-wrap: break-word;word-break: break-all;"><? $net_order_value=$row[csf('order_total')]-$commission;$net_order_value_tot+=$net_order_value; echo number_format ($net_order_value,2); ?></td>
                                
                                <td  width="100" align="right" style="word-wrap: break-word;word-break: break-all;">
								<?   
								//echo number_format($job_smv_arr[$row['job_no']],2); 
								echo number_format($job_smv_arr[$row[csf('job_no')]]["smv_pcs"],2); 
								?>
                                </td>
                                <td  width="100" align="right" style="word-wrap: break-word;word-break: break-all;">
								<?   
								//echo number_format($job_smv_arr[$row['job_no']],2); 
								echo number_format($job_smv_arr[$row[csf('job_no')]]["finsmv_pcs"],2); 
								?>
                                </td>
								<td  width="100" align="right" style="word-wrap: break-word;word-break: break-all;"><?  $smv= ($job_smv_arr[$row[csf('job_no')]]["smv_pcs"])*$row[csf('po_quantity')]; $smv_tot+=$smv; echo number_format($smv,2); ?></td>
                                
								<td  width="100" align="right" style="word-wrap: break-word;word-break: break-all;"><?  $smv_iron= ($job_smv_arr[$row[csf('job_no')]]["finsmv_pcs"])*$row[csf('po_quantity')]; $smv_tot_iron+=$smv_iron; echo number_format($smv_iron,2); ?></td>
								...
								<? 
								$basic_qnty_pcs= (($job_smv_arr[$row[csf('job_no')]]["smv_pcs"])*$row[csf('po_quantity')])/$basic_smv_arr[$row[csf("company_name")]];
								$basic_qnty_pcs_tot+=$basic_qnty_pcs;
								echo fn_number_format($basic_qnty_pcs,0);
								...
								?>
                                 ...
                                 <a href="##" onClick="generate_ex_factory_popup('ex_factory_popup','<? echo $row[csf("country_id")];?>', '<? echo $row[csf('id')]; ?>','750px')"><? echo  number_format( $ex_factory_qnty,0); ?></a>
                                <?
                                ...
                                <td width="90" align="right" style="word-wrap: break-word;word-break: break-all;">
                                <?
								if($ex_factory_qnty>=0){
								$ex_factory_value=($ex_factory_qnty/$row[csf('total_set_qnty')])*$row[csf('unit_price')];
								echo fn_number_format($ex_factory_value,2);
								}
								?>
                                </td>
								<td  width="90" align="right" style="word-wrap: break-word;word-break: break-all;">
                                ...
                                <td width="60" align="center" style="word-wrap: break-word;word-break: break-all;"><? echo  $short_over_shipment;?></td>
								...
								<td width="200"style="word-wrap: break-word;word-break: break-all;"><? echo $country_remarks; ?></td>
                                <td  style="word-wrap: break-word;word-break: break-all;"><? echo $row[csf('id')]; ?></td>
							</tr>
                    <?
                    $i++;
					}
                ...
                <table width="4320" id="report_table_footer" border="1" class="rpt_table" rules="all">
                    <tfoot>
                        <tr>
                            <th width="50"></th>
                            <th width="65" ></th>
                            ...
                            <th  width="100" id="value_smv_tot_iron"><? echo number_format($smv_tot_iron,2); ?></th>
                            ...
                            <th width="90" id="value_total_ex_factory_value"><? //echo number_format($total_ex_factory_value,0); ?></th>
                ...
                <div id="shipment_performance">
                    <fieldset>
                        ...
                        <tr bgcolor="#E9F3FF">
                        <td> </td><td></td><td align="right"><? echo number_format($po_qnty['yet']+$po_qnty['ontime']+$po_qnty['after'],0); ?></td><td align="right"><? echo fn_number_format(((100*$po_qnty['yet'])/$order_qnty_pcs_tot)+((100*$po_qnty['after'])/$order_qnty_pcs_tot)+((100*$po_qnty['ontime'])/$order_qnty_pcs_tot),2); ?></td>
                        </tr>
                        </table>
                    </fieldset>
                </div>
                
                <div id="shipment_performance_buyer_wise">
                    <fieldset>
                        <table width="600" border="1" cellpadding="0" cellspacing="1" class="rpt_table" rules="all" >
                        <thead>
                        <tr>
                        <th colspan="5"> <font size="4">On Time Delivery (Buyer Wise)</font></th>
                        </tr>
                        <tr>
                        <th>Sl</th><th>Buyer Name</th><th>TOTAL P.O</th><th> OTD P.O</th><th>OTD %</th>
                        </tr>
                        </thead>
                        <?
						$buyer_total_po=0;
						$buyer_total_Ontime_po=0;
						$sl=1;
						foreach($buyer_wise_performance as $buyer_wise_performance_key => $buyer_wise_performance_value)
						{
						if ($sl%2==0)  
						$bgcolor="#E9F3FF";
						else
						$bgcolor="#FFFFFF";	
						?>
                        <tr bgcolor="<? echo $bgcolor; ?>"   onclick="change_color('tr_4nd<? echo $sl; ?>','<? echo $bgcolor; ?>')" id="tr_4nd<? echo $sl; ?>">
                        <td><? echo $sl; ?></td><td><? echo $buyer_short_name_arr[$buyer_wise_performance_key]; ?></td><td align="right"><? echo $buyer_wise_performance_value['total_po']; $buyer_total_po+=$buyer_wise_performance_value['total_po'];?></td><td align="right"><? echo $buyer_wise_performance_value['ontime']; $buyer_total_Ontime_po+=$buyer_wise_performance_value['ontime'];?></td><td align="right"><? echo fn_number_format(($buyer_wise_performance_value['ontime']/$buyer_wise_performance_value['total_po'])*100,2); ?></td>
                        </tr>
                        <?
						$sl++;
						}
                        ...
                        <tfoot>
                        <tr>
                        <th></th><th></th><th><? echo $buyer_total_po; ?></th><th><? echo $buyer_total_Ontime_po; ?></th><th></th>
                        </tr>
                        </tfoot>
                        </table>
                    </fieldset>
...
<fieldset style="width:99%;">
    ...
    <tr>
        ...
        <td><?php echo $product_dept[$mst_data_arr[0][csf('product_dept')]]; ?></td>
...
<div id="nevbar">
    <ul>
        ...
        <li><a href="group_capacity_and_order_booking_status_controller.php?action=gmts_image&job_no=<? echo $job_no;?>">Gmts.Image</a></li>
        <li><a href="group_capacity_and_order_booking_status_controller.php?action=size_color_dtls&job_no=<? echo $job_no;?>&po_id=<? echo $po_id;?>">Size and Color Details</a></li>
        <li><a href="group_capacity_and_order_booking_status_controller.php?action=cost_info&txt_job_no=<? echo $job_no;?>&po_id=<? echo $po_id;?>&cbo_company_name=<? echo $mst_data_arr[0][csf('company_name')];?>">Cost Info</a></li>
        <li><a href="group_capacity_and_order_booking_status_controller.php?action=fab_info&txt_job_no=<? echo $job_no;?>&po_id=<? echo $po_id;?>&cbo_company_name=<? echo $mst_data_arr[0][csf('company_name')];?>">Fabric Info</a></li>
        
        <li><a href="group_capacity_and_order_booking_status_controller.php?action=sample_approval&job_no=<? echo $job_no;?>&po_id=<? echo $po_id;?>">Sample App.</a></li>
        <li><a href="group_capacity_and_order_booking_status_controller.php?action=labdip_approval&job_no=<? echo $job_no;?>&po_id=<? echo $po_id;?>">Labdip App.</a></li>
        <li><a href="group_capacity_and_order_booking_status_controller.php?action=trims_approval&job_no=<? echo $job_no;?>&po_id=<? echo $po_id;?>">Trims App.</a></li>
...
b.job_no_mst, b.po_number,b.po_quantity, b.po_received_date, b.unit_price, b.shipment_date, b.factory_received_date,b.po_total_price as order_value
FROM wo_po_details_master a, wo_po_break_down b
WHERE b.id=$po_id and b.job_no_mst='$job_no' and a.job_no=b.job_no_mst
");
...
<table class="rpt_table" width="100%" cellpadding="0" cellspacing="0" border="1" rules="all">
    <tr bgcolor="#FFFFFF">
     ...
     <tr bgcolor="#E9F3FF">  
        <td>&nbsp; Shipment Date</td><td>&nbsp; <?php echo change_date_format($order_data_arr[0][csf('shipment_date')]);?></td>
     ...
     <tr bgcolor="#E9F3FF">  
        <td>&nbsp; Factory Recv.Date</td><td>&nbsp; <?php echo $order_data_arr[0][csf('factory_received_date')];?></td>
...
if($action=="size_color_dtls")
{
	...
	$color_size_arr=sql_select("SELECT order_quantity,color_number_id,size_number_id FROM wo_po_color_size_breakdown WHERE job_no_mst='$job_no' and po_break_down_id='$po_id'");
...
<?	
	...
	?>
     <button style="float:left; cursor:pointer; border:1px solid #999; border-radius:3px;" onclick="new_window();">Print</button> 
...
function open_febric_receive_status_order_wise_popup(order_id,type,color)
{
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'group_capacity_and_order_booking_status_controller.php?order_id='+order_id+'&action='+type+'&color='+color, 'Detail Veiw', 'width='+popup_width+', height=380px,center=1,resize=0,scrolling=0','../../../');
}
...
{ //alert(book_num);
	...
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'group_capacity_and_order_booking_status_controller.php?po_id='+po_id+'&item_name='+item_name+'&job_no='+job_no+'&book_num='+book_num+'&trim_dtla_id='+trim_dtla_id+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../../../');
}
	...
	{
		var popup_width='900px';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'group_capacity_and_order_booking_status_controller.php?po_id='+po_id+'&item_name='+item_name+'&action='+action, 'Details Veiw', 'width='+popup_width+', height=350px,center=1,resize=0,scrolling=0','../../../');
	}
	...
	<div style="width:100%" align="center">
        ...
        <div style="width:100%; max-height:400px;">
            <table cellpadding="0" width="100%" cellspacing="0" border="1" rules="all" class="rpt_table">
                <?
                $i=1;
				if($country_id!=0) $country_cond="and country_id in($country_id)";else  $country_cond="";
				...
				from  pro_ex_factory_delivery_mst a,  pro_ex_factory_mst b  where a.id=b.delivery_mst_id and a.status_active=1 and a.is_deleted=0 and b.po_break_down_id in($id) $country_cond");
