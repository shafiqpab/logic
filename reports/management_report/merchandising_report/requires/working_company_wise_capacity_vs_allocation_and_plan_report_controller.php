   ...
   // cbo_lc_location_id   ==>  LC location (a.location_name)
	...
	foreach( $sql_data as $row)
	{
		...
		//	$fff[$row['po_id']]['ratio'] = $set;
			...
			$order_wise_array[$row[csf('po_id')]]['total_set_smv'] = $row[csf('set_smv')];
				
			$order_wise_array[$row[csf('po_id')]]['set_item_ratio'] = $row[csf('set_item_ratio')];
			$order_wise_array[$row[csf('po_id')]]['set_item_ratio']+= $set['SET_ITEM_RATIO'];



			$job_wise_array[$row[csf('job_no')]]['job_no']=$row[csf('job_no')];
			$job_wise_array[$row[csf('job_no')]]['set_item_ratio'] = $row[csf('set_item_ratio')];
			$job_wise_array[$row[csf('job_no')]]['set_item_ratio']+= $set['SET_ITEM_RATIO'];


			// confirmed and protected
			if($row[csf('is_confirmed')] == 1){
				$order_wise_array[$row[csf('po_id')]]['po_qty_pcs'] = $row[csf('confirm_qty')];
			}
			else{
				$order_wise_array[$row[csf('po_id')]]['po_qty_pcs'] = $row[csf('projected_qty')];
			}
	...
	// echo "<pre>";
	// print_r($order_wise_array22);die;
	...
	foreach($data_array as $row)
	{
		...
		$ratio = $job_wise_array[$row[csf('job_no')]];

		$set_item_ratio_arr[$row[csf('working_company_id')]][$row[csf('year')]][$row[csf('month')]]['set_item_ratio'] += $row[csf('set_item_ratio')];


		//$data['working_company_id']  = $fff[$row[csf('job_id')]]['ratio'];
		...
		if($row[csf('is_confirmed')]==1)
		{
			...
			$type = $row[('order_uom')];
			if($type==58){
				$smv_ratio_set = $row[csf('set_smv')]/$ratio;
				$total_po_qty_pcs = $row[csf('po_quantity')]*$ratio;
				$total_mins = $total_po_qty_pcs*$smv_ratio_set;
				$confrim_data_arr[$row[csf('working_company_id')]][$row[csf('year')]][$row[csf('month')]]['confirmMinSet'] += $total_mins;
			}else{
				$confrim_data_arr[$row[csf('working_company_id')]][$row[csf('year')]][$row[csf('month')]]['confirmMinPcs'] += $row[csf('po_quantity')]*$row[csf('set_smv')];
			}
		}
		elseif($row[csf('is_confirmed')]==2)
		{
			...
			if($type==58){
				...
				$total_po_qty_pcs = $row[csf('po_quantity')]*$ratio;
				$total_mins = $total_po_qty_pcs*$smv_ratio_set;
				$project_data_arr[$row[csf('working_company_id')]][$row[csf('year')]][$row[csf('month')]]['projectMinSet'] += $total_mins;
			}else{
				$project_data_arr[$row[csf('working_company_id')]][$row[csf('year')]][$row[csf('month')]]['projectMinPcs'] += $row[csf('po_quantity')]*$row[csf('set_smv')];
			}
		}
	}
				...
				<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_2nd1','<? echo $bgcolor; ?>')" id="tr_2nd1">
                    ...
                    foreach($month_count_arr as $year_id=>$year_val)
                    {
						...
						?>
							...
							<!-- confirmMinSet+confirmMinPcs  -->
							<?php
							$total_con = $confrim_data_arr[$com_id][$year][$month]['confirmMinSet']+$confrim_data_arr[$com_id][$year][$month]['confirmMinPcs'];

							$total_pro =$project_data_arr[$com_id][$year][$month]['projectMinSet']+$project_data_arr[$com_id][$year][$month]['projectMinPcs'];
							?>

			<!-- projectMinSet+projectMinPcs -->


							<td width="80" align="right"><?= number_format($confrim_data_arr[$com_id][$year][$month]['confirmQty'],0);?></td>
							<td width="80" align="right"><?= number_format($total_con,0);?></td>
							<td width="80" align="right"><?= number_format($project_data_arr[$com_id][$year][$month]['projectQty'],0);?></td>
							<td width="80" align="right"><?= number_format($total_pro,0);?></td>
							...
							<td width="80" align="right"><?= number_format($total_con+$total_pro, 0) ?></td>
							...
							<td width="80" align="right"><?= number_format(($total_con+$total_pro)-$capacity_arr[$com_id][$year][$month]['capacity_min'],0) ?></td>
							...
							$confirm_qty_mins[$year][$month]['confirm_qty_mins'] +=$total_con; 
							$projected_qty_pcs[$year][$month]['projected_qty_pcs']  +=$project_data_arr[$com_id][$year][$month]['projectQty'];
							$projected_qty_mins[$year][$month]['projected_qty_mins'] +=$total_pro;
							...
							$total_qty_mins[$year][$month]['total_qty_mins'] += $total_con+$total_pro;
							...
							$variance_mins[$year][$month]['variance_mins'] += ($total_con+$total_pro)-$capacity_arr[$com_id][$year][$month]['capacity_min'];
				...
				<tr>
					...
					foreach($month_count_arr as $year_id=>$year_val)
					{
						foreach($year_val as $month_id=>$month_val)
						{
							$year = $month_val['year'];
							$month = $month_val['month'];
							$grand_total_percent = $total_qty_pcs[$year][$month]['total_qty_pcs']/$total_qnty[$year][$month]['total_qnty']*100;
						?>
						...
						<th width="80" align="right"><?=number_format($grand_total_percent, 0);?>%</th>
						<?
						}
					}
					?>
	...
	<div style="width:2820px;">
		<table width="2820" id="table_header_1" border="1" class="rpt_table" rules="all">
            ...
            <div style=" max-height:400px; overflow-y:scroll; width:2840px"  align="left" id="scroll_body">
                <table width="2820" border="1" class="rpt_table" rules="all" id="table-body">
					...
					<tr bgcolor="<?= $bgcolor; ?>" style="vertical-align:middle"  onclick="change_color('tr_<?= $k; ?>','<?= $bgcolor;?>')" id="tr_<?= $k; ?>">
						...
						$type = $row[('order_uom')];
						if($type==58){
							$total_po_qty_pcs = $row[('po_qty_pcs')]*$row[('set_item_ratio')];
						}
						else{
						...
						<td style="word-break: break-all; word-wrap: break-word;" width="100" align="right"><?= number_format($total_po_qty_pcs, 2);?></td>
						<?php
							$type = $row[('order_uom')];
							if($type==58){
								$smv_ratio_set = $row[("set_smv")]/$row[('set_item_ratio')];
								$total_po_qty_pcs = $row[('po_qty_pcs')]*$row[('set_item_ratio')];
								$total_mins = $total_po_qty_pcs*$smv_ratio_set;
							}
							else{
								$total_po_qty_pcs = $row[('po_qty_pcs')];
								$total_mins = $total_po_qty_pcs*$row[('set_smv')];
							} 
						?>
					    <td style="word-break: break-all; word-wrap: break-word;" width="100" align="right"><?= number_format($total_mins,2); ?></td>
						...
						<?php
							$type = $row[('order_uom')];
							if($type==58){
								$unit_set = $row[('unit_price')]/$row[('set_item_ratio')];
								$total_po_qty_pcs = $row[('po_qty_pcs')]*$row[('set_item_ratio')];
								$total_order_value = $total_po_qty_pcs*$unit_set;
							}
							else{
								$total_po_qty_pcs = $row[('po_qty_pcs')];
								$total_order_value = $total_po_qty_pcs*$row[('unit_price')];
							}
					...
					<?
					$k++;
					$total_po_qty+=$total_po_qty_pcs;
					$total_po_qty_mis+=$total_mins;
...
if($action = 'report_generate_plan')
{
   ...
   // echo $sql;die;
	...
	<div>
		...
		<div id="scroll_body">
            <table width="300" border="1" class="rpt_table" rules="all" id="table-body"></table>
		</div>
	</div>
	<?
	$user_id=$_SESSION['logic_erp']['user_id'];
