...
if ($action=="load_drop_down_buyer")
{
	...
	echo create_drop_down( "cbo_buyer_name", 100, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) group by buy.id,buy.buyer_name  order by buyer_name","id,buyer_name", 1, "-- All --", $selected, "load_drop_down( 'requires/order_follo_up_report_controller', this.value, 'load_drop_down_season', 'season_td');" );
	exit();
}
...
if ($action=="report_generate")
{
	...
	$season_lib=return_library_array( "select id, season_name from lib_buyer_season where  status_active =1 and is_deleted=0 order by season_name ASC", "id", "season_name"  );
	...
	(CASE WHEN b.booking_type=1 THEN b.fin_fab_qnty END) as fin_fab_qnty
	 from wo_booking_mst a, wo_booking_dtls b,wo_po_break_down c,wo_po_details_master d where a.booking_no=b.booking_no and  b.po_break_down_id=c.id and d.id=c.job_id and b.booking_type=1 and b.status_active=1 and b.is_deleted=0 $company_name2 $buyer_id_cond2 $fab_nature_cond $fab_source_cond $search_text2";
	$po_ids='';
...
$po_job_cond=where_con_using_array($order_id_arr,0,"b.order_id");
		$grey_and_dyeing_cost_sql="SELECT sum(b.amount) as amount, b.order_id as order_id,a.process_id as process_id
		 ...
		 else
		 {
		   ...
		   if(str_replace("'","",$cbo_buyer_name) !=0){
			...
			if(str_replace("'","",$cbo_year)!=0)
			{
				$condition->job_year(" and to_char(a.insert_date,'YYYY')=$cbo_year");//to_char(a.insert_date,'YYYY')='2023'
			}
			...
			{
				//$condition->po_id_in("$po_id_str");//season_buyer_wise job_year
			}
		   $condition->init();
		...
		if(count($all_prod_ids)==0)
		{
		 ...
		 $sql_receive_for_issue="select c.booking_id,a.job_no,a.prod_id,c.exchange_rate,max(a.transaction_date) as transaction_date,c.currency_id,c.receive_purpose,b.lot,b.color,
		...
		//echo $sql_receive_for_issue;die;
		...
		foreach($yarnTrimsDataArray as $invRow)
		{
			...
			//echo strtotime($conversion_date);die;
				...
				$exchange_rate=$receive_date_array[$invRow[csf('prod_id')]]['last_exchange_rate'];
		...
		//echo "CAA==";die;
		  ...
		  $yarnissue_retData="select d.booking_no,a.item_category,b.po_breakdown_id, b.prod_id, a.mst_id,a.receive_basis, b.issue_purpose,c.lot,c.color,
			...
			foreach($yarnissueretDataArray as $invRow)
			{
				...
				$currency_rate=$receive_date_array[$invRow[csf('prod_id')]]['last_exchange_rate'];
    ...
    $ex_factory_sql="SELECT a.po_break_down_id,
	(CASE WHEN a.entry_form!=85 THEN a.ex_factory_qnty ELSE 0 END) AS delivery_qnty,
	(CASE WHEN a.entry_form=85 THEN a.ex_factory_qnty ELSE 0 END) AS ex_factory_return_qnty
	from pro_ex_factory_mst a WHERE a.status_active=1 and a.is_deleted=0  $poCon";

	$ex_factory_qnty_sql=sql_select($ex_factory_sql);
	$ex_factory_qnty_arr=array();

	foreach($ex_factory_qnty_sql as $val)
	{
		$ex_factory_qnty_arr[$val[csf('po_break_down_id')]]['delivery_qnty']+=$val[csf('delivery_qnty')];
		$ex_factory_qnty_arr[$val[csf('po_break_down_id')]]['ex_factory_return_qnty']+=$val[csf('ex_factory_return_qnty')];
	}
//    echo '<pre>';print_r($ex_factory_qnty_arr); echo '</pre>';
	...
	foreach($order_data_arr as $rows)
	{
		...
		$buyer_summary_arr[$rows[csf('buyer_name')]]['delivery_qnty']+=$ex_factory_qnty_arr[$rows[csf('id')]]['delivery_qnty'];
		$buyer_summary_arr[$rows[csf('buyer_name')]]['ex_factory_return_qnty']+=$ex_factory_qnty_arr[$rows[csf('id')]]['ex_factory_return_qnty'];
		...
		$buyer_tot_arr['delivery_qnty']+=$ex_factory_qnty_arr[$rows[csf('id')]]['delivery_qnty'];
		$buyer_tot_arr['exfactory_qty']+=$ex_qnty_array[$rows[csf('id')]];
		$buyer_tot_arr['ex_factory_return_qnty']+=$ex_factory_qnty_arr[$rows[csf('id')]]['ex_factory_return_qnty'];
        ...
        <table width="100%" border="1" rules="all"  class="rpt_table">
            ...
            <tbody>
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('str_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="str_<? echo $i; ?>" style="font-size:12px; cursor:pointer;" valign="middle">
                    ...
                    <td align="right"><? $actual_ex_factory=$rows['delivery_qnty']-$rows['ex_factory_return_qnty']; echo omitZero(number_format($actual_ex_factory,0)); ?></td>
			   ...
			   $total_lay_cut_qty+=$rows['lay_cut_qty'];
			    ...
			    $tot_lay_cut_per=0;  $tot_cut_per=0;
			  	$tot_lay_cut_per= ($total_lay_cut_qty/$buyer_tot_arr['po_quantity'])*100;
			   	...
			   	$buyer_tot_arr['total_actual_ex_factory']+=$actual_ex_factory;
			   ?>
            </tbody>
            <tfoot>
                ...
                <th align="right"><?  echo omitZero(number_format($buyer_tot_arr['delivery_qnty']-$buyer_tot_arr['ex_factory_return_qnty']),0);?></th>
                <th align="right"><? echo omitZero(number_format(($buyer_tot_arr['delivery_qnty']/$buyer_tot_arr['po_quantity'])*100,0));?></th>
            </tfoot>
	...
	<div style="width:<?=$main_width+$new_column+20;?>px" align="left">
        ...
        <table width="<?=$main_width+$new_column+20;?>" border="1" rules="all"  class="rpt_table">
            <thead>
                <tr style="font-size:12px">
                    ...
                    <th width="80" style="word-break: break-all;">Ship Date</th>
                    ...
                    <th style="width:50px;word-break: break-all;">Ex- Factory return qty</th>
                    <th style="width:50px;word-break: break-all;">Actual Ex Factory Qty</th>
        ...
        <div style="width:<?=$main_width+$new_column+20;?>px; max-height:350px; overflow-y:scroll" id="report_div" >
            ...
            ?>
                <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" style="font-size:12px; cursor:pointer;" valign="middle">
					...
					<td width="100" style="word-break: break-all;"><p><? echo $season_lib[$row[csf('season_buyer_wise')]];?></p></td>
					...
					<td width="80" style="word-break: break-all;" align="right"><? echo number_format($row[csf('unit_price')],2);?></td>
                    ...
                    <td width="60" style="word-break: break-all;" align="right"><? echo  omitZero(number_format($lay_cut_data_dtls[$row[csf("id")]]['lay_cut_qty']),0);//omitZero(number_format($lay_cut_data[$row[csf('job_no')]][$row[csf('buyer_name')]],0));?></td>
                    <td width="60" style="word-break: break-all;" align="right"><? echo omitZero(number_format($lay_cut_per,2));?></td>

                    <td width="60" style="word-break: break-all;" align="right"><? echo omitZero(number_format($cut_qnty_array[$row[csf('id')]],0));?></td>
                    <td width="60" style="word-break: break-all;" align="right"><? echo omitZero(number_format($cut_per,2));?></td>
                    <td width="50" style="word-break: break-all;" align="right"><? echo omitZero(number_format($emb_issue_qnty_array[$row[csf('id')]],0));?></td>
                    <td width="60" style="word-break: break-all;" align="right"><? echo omitZero(number_format($emb_rec_qnty_array[$row[csf('id')]],0));?></td>
                    <td width="60" style="word-break: break-all;" align="right"><? echo omitZero(number_format($sewing_in_qnty_array[$row[csf('id')]],0));?></td>
                    <td width="60" style="word-break: break-all;" align="right"><? echo omitZero(number_format($sewing_out_qnty_array[$row[csf('id')]],0));?></td>
					...
					<td width="50" style="word-break: break-all;" align="right"><? echo omitZero(number_format($ex_factory_qnty_arr[$row[csf('id')]]['delivery_qnty'],0));?></td>
					<td width="50" style="word-break: break-all;" align="right"><? echo omitZero(number_format($ex_factory_qnty_arr[$row[csf('id')]]['ex_factory_return_qnty'],0));?></td>
					<td width="50" style="word-break: break-all;" align="right"><? $actual_ex_factory_qnty=$ex_factory_qnty_arr[$row[csf('id')]]['delivery_qnty']-$ex_factory_qnty_arr[$row[csf('id')]]['ex_factory_return_qnty']; echo omitZero(number_format($actual_ex_factory_qnty,0));?></td>
            ...
            <?
				...
				$tot_delivery_qnty+=$ex_factory_qnty_arr[$row[csf('id')]]['delivery_qnty'];
				$tot_ex_qnty+=$ex_factory_qnty_arr[$row[csf('id')]]['ex_factory_return_qnty'];
				$tot_actual_qnty+=$actual_ex_factory_qnty;
        ...
        <table width="<?=$main_width+$new_column;?>" border="1" cellpadding="0" cellspacing="0" class="tbl_bottom" rules="all">
			 ...
			 <td width="35">&nbsp;</td>
                    ...
                    <td width="80">&nbsp;</td>
                    <td width="60">&nbsp;</td>
                    <td width="60"><p id="td_gf_qnty_" style="width:60px; word-wrap:break-word;"><? echo number_format($tot_gf_qnty,0);?></p></td>
					<td width="60"><p id="td_yarn_alloc_qnty_" style="width:60px; word-wrap:break-word;"><? echo number_format($tot_yarn_alloc_qnty,0);?></p></td>
					<td width="60"><p id="td_yarn_issue_qnty_" style="width:60px; word-wrap:break-word;"><? echo number_format($tot_yarn_issue_qnty,0);?></p></td>
					<td width="60"><p id="td_yarn_value" style="width:60px; word-wrap:break-word;"><? echo number_format($tot_yarn_value,0);?></p></td>
					<td width="60"><p id="td_gp_qty_" style="width:60px; word-wrap:break-word;"><? echo number_format($tot_gp_qty,0);?></p></td>
					<td width="60"><p id="td_yarn_budget_value" style="width:60px; word-wrap:break-word;"><? echo number_format($yarn_budget_value,0);?></p></td>
					<td width="60"><p id="td_grey_cost_value" style="width:60px; word-wrap:break-word;"><? echo number_format($tot_grey_cost_value,0);?></p></td>
					<td width="60"><p id="td_gp_to_qty_" style="width:60px; word-wrap:break-word;"><? echo number_format($tot_grey_to_dye,0);?></p></td>
                    <td width="60">&nbsp;</td>
                    <td width="60"><p id="td_daying_qnty_" style="width:60px; word-wrap:break-word;"><? echo number_format($tot_daying_qnty,0);?></p></td>
					<td width="60"><p id="td_dying_cost" style="width:60px; word-wrap:break-word;"><? echo number_format($tot_dying_cost,0);?></p></td>
					<td width="60"><p id="td_total_fabric_cost" style="width:60px; word-wrap:break-word;"><? echo number_format($total_fabric_cost,0);?></p></td>
					...
					<td width="60"><p id="td_cut_qnty_" style="width:60px; word-wrap:break-word;"><? echo number_format($tot_lay_cut_qnty,0);?></p></td>
                    <td width="60">&nbsp;</td>
                    <td width="60">&nbsp;</td>
                    <td width="60">&nbsp;</td>
					...
					<td width="50"><p id="td_ex_qnty_" style="width:50px; word-wrap:break-word;"><? echo number_format($tot_delivery_qnty,0);?></p></td>
					<td width="50"><p id="td_ex_qnty_" style="width:50px; word-wrap:break-word;"><? echo number_format($tot_ex_qnty,0);?></p></td>
					<td width="50"><p id="td_ex_qnty_" style="width:50px; word-wrap:break-word;"><? echo number_format($tot_actual_qnty,0);?></p></td>
					<td width="" style="word-break: break-all;">&nbsp;</td>
      		</tfoot>
        </table>
	</div>
	</fieldset>
