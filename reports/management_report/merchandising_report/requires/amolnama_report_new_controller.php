...
if($action=="report_generate_by_year")
{
	...
	$sqlgreyfab = "SELECT d.BUYER_NAME, a.TRANSACTION_TYPE,c.RECEIVE_PURPOSE,a.ITEM_CATEGORY,to_char(e.shipment_date,'MON-YYYY') as MONTH_YEAR, b.QUANTITY,b.RETURNABLE_QNTY,g.fabric_source,g.booking_type  from inv_receive_master c, inv_transaction a, order_wise_pro_details b,wo_po_details_master d,wo_po_break_down e,pro_batch_create_mst f,wo_booking_mst g where c.id=a.mst_id and a.id=b.trans_id and a.prod_id=b.prod_id and d.id=e.job_id and e.id=b.po_breakdown_id and a.item_category in(2,13) and f.booking_no=g.booking_no and g.booking_type in(1,3,4) and a.transaction_type in(1,4) and f.id=a.pi_wo_batch_no and f.batch_against in(0,1,2) and f.status_active=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and e.shipment_date between '$startDate' and '$endDate' $greyfabPoCond";
	// echo $sqlgreyfab;die();
	...
	foreach ($greyfabRes as $val) 
	{
		...
		else
		{
			if($val['BOOKING_TYPE']==1)
			{
				...
				$buyer_fab_data_array[$val['BUYER_NAME']][$val['ITEM_CATEGORY']][$val['TRANSACTION_TYPE']]['main_receive_qnty'] += $val['QUANTITY'];
			}
	...
	/									transfer data											/
	/==========================================================================================*/
	$transPoCond = str_replace("b.id", "b.to_order_id", $poCond);
	$transPoCond2 = str_replace("b.id", "b.from_order_id", $poCond);
	$sqlTrnsfrIn = "SELECT c.BUYER_NAME,to_char(d.shipment_date,'MON-YYYY') as MONTH_YEAR,a.ITEM_CATEGORY,e.cons_quantity as QUANTITY from inv_item_transfer_mst a,inv_item_transfer_dtls b,wo_po_details_master c,wo_po_break_down d,inv_transaction e where a.id=b.mst_id and c.id=d.job_id and a.item_category in(2,13) and e.id=b.to_trans_id and e.transaction_type in(5) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.transfer_criteria in(2,4) and a.entry_form in(13,14) and d.id=b.to_order_id and d.shipment_date between '$startDate' and '$endDate' $transPoCond";
	...
	//======================================================================================
	$sqlTrnsfrOut = "SELECT c.BUYER_NAME,to_char(d.shipment_date,'MON-YYYY') as MONTH_YEAR,a.ITEM_CATEGORY,e.cons_quantity as QUANTITY from inv_item_transfer_mst a,inv_item_transfer_dtls b,wo_po_details_master c,wo_po_break_down d,inv_transaction e where a.id=b.mst_id and c.id=d.job_id and a.item_category in(2,13) and e.id=b.trans_id and d.id=b.from_order_id and e.transaction_type in(6) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.transfer_criteria in(2,4) and a.entry_form in(13,14) and b.active_dtls_id_in_transfer=1 and d.shipment_date between '$startDate' and '$endDate' $transPoCond2";
				...
				<table border="1" rules="all" class="rpt_table" width="<? echo $tbl_width;?>" cellpadding="0" cellspacing="0">			
			        ...
			        <tbody>   
			        	...
			        	foreach ($buyer_data_array as $buyer_name => $row) 
			        	{
							...
							// echo "(".$grey_fab_rcv_without_sample."+".$grey_trnsf_in."+".$grey_fab_iss_rtn_without_sample.") -(".$grey_fab_issue_without_sample."+".$grey_fab_rcv_rtn_without_sample." + ".$grey_lft_over ."+". $grey_trnsf_out.")<br>";
			        		...
			        		$main_receive_qnty 	= $buyer_fab_data_array[$buyer_id][2][1]['main_receive_qnty'];
			        		$fin_fab_rcv 	= $buyer_fab_data_array[$buyer_id][2][1]['all_receive_qnty'];
							...
							// echo "(".number_format($all_fin_fab_rcv,2)."+".number_format($fin_fab_iss_rtn,2)."+".number_format($fin_trnsf_in,2).") -(".number_format($fin_fab_issue,2)."+".number_format($fin_trnsf_out,2)."+".number_format($fin_fab_rcv_rtn,2).")<br>";
			        		...
			        		$dyeing_pro_los = ($batch_qnty>0) ? (($batch_qnty - ($main_receive_qnty - $fin_fab_purses_rcv))/$batch_qnty)*100 : 0;
			        		// echo "((".$batch_qnty ."- (".$fin_fab_rcv ."-". number_format($fin_fab_purses_rcv,3)."))/".$batch_qnty.")*100<br>";
...
if($action=="report_generate_by_buyer_month_year")// report generate when buyer click
{
	...
	foreach ($greyfabRes as $val) 
	{
		...
		else
		{
			if($val['BOOKING_TYPE']==1)
			{
				$fab_data_array[$val['SEASON_BUYER_WISE']][$val['ITEM_CATEGORY']][$val['TRANSACTION_TYPE']]['main_receive_qnty'] += $val['QUANTITY'];
			}
			// if($val['FABRIC_SOURCE']!=2)
			// {
	...
	/==========================================================================================*/
	$poIdsCondff = str_replace("b.id", "c.po_breakdown_id", $poCond);
    $sqlFinFab = "SELECT e.season_buyer_wise,e.BUYER_NAME,to_char(d.shipment_date,'MON-YYYY') as MONTH_YEAR,b.TRANSACTION_TYPE,c.quantity AS QTY
	...
	$transPoCond = str_replace("b.id", "b.to_order_id", $poCond);
	$transPoCond2 = str_replace("b.id", "b.from_order_id", $poCond);
	$sqlTrnsfrIn = "SELECT c.BUYER_NAME,c.season_buyer_wise,to_char(d.shipment_date,'MON-YYYY') as MONTH_YEAR,a.ITEM_CATEGORY,e.cons_quantity as QUANTITY from inv_item_transfer_mst a,inv_item_transfer_dtls b,wo_po_details_master c,wo_po_break_down d,inv_transaction e where a.id=b.mst_id and c.id=d.job_id and a.item_category in(2,13) and e.id=b.to_trans_id and e.transaction_type in(5) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.transfer_criteria in(2,4) and a.entry_form in(13,14) and d.id=b.to_order_id and c.buyer_name=$buyer_id and d.shipment_date between '$startDate' and '$endDate' $transPoCond";
	// echo $sqlTrnsfrIn;die();
	$trnsfrResIn = sql_select($sqlTrnsfrIn);
	$transfer_in_data_array = array();
	$buyer_transfer_in_data_array = array();
	...
	// print_r($transfer_in_data_array);die();
	//======================================================================================
	$sqlTrnsfrOut = "SELECT c.season_buyer_wise,to_char(d.shipment_date,'MON-YYYY') as MONTH_YEAR,a.ITEM_CATEGORY,e.cons_quantity as QUANTITY from inv_item_transfer_mst a,inv_item_transfer_dtls b,wo_po_details_master c,wo_po_break_down d,inv_transaction e where a.id=b.mst_id and c.id=d.job_id and a.item_category in(2,13) and e.id=b.trans_id and e.transaction_type in(6) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.transfer_criteria in(2,4) and a.entry_form in(13,14) and d.id=b.from_order_id and b.active_dtls_id_in_transfer=1  and c.buyer_name=$buyer_id and d.shipment_date between '$startDate' and '$endDate' $transPoCond2";
	// echo $sqlTrnsfrOut;die();
	$trnsfrResOut = sql_select($sqlTrnsfrOut);
	$transfer_out_data_array = array();
	$buyer_transfer_out_data_array = array();
	        ...
	        <thead>
		        ...
		        <?		
		        	...
		        	foreach ($buyer_season_data_array as $season => $val) 
		        	{
						...
						// echo "<br>(".$grey_fab_rcv_without_sample."+".$grey_trnsf_in."+".$grey_fab_iss_rtn_without_sample.") -(".$grey_fab_issue_without_sample."+".$grey_fab_rcv_rtn_without_sample." + ".$grey_lft_over ."+". $grey_trnsf_out.")<br>";


		        		$grey_lft_over 	= $scrap_data_array[$season][13]['grey']+$grey_fab_rej_without_sample;

		        		$main_receive_qnty 	= $fab_data_array[$season][2][1]['main_receive_qnty'];
		        		$fin_fab_rcv 	= $fab_data_array[$season][2][1]['all_receive_qnty'];
		        		...
		        		$dyeing_pro_los 	= ($batch_qnty>0) ? (($batch_qnty - ($main_receive_qnty - $fin_fab_purses_rcv))/$batch_qnty)*100 : 0;
						// echo "$season==((".$batch_qnty ."- (".$fin_fab_rcv ."-". number_format($fin_fab_purses_rcv,3)."))/".$batch_qnty.")*100<br>";
...
if($action=="report_generate_by_intref_wise")// report generate when month/buyer details click
{
	...
	$int_ref_wise_po_id_array = array();
	$po_id_array = array();
	$job_id_array = array();
	$gmt_array = array();
	foreach ($sql_gmt_res as $val) 
	{	
		...
		$po_id_array[$val['PO_ID']] = $val['PO_ID'];	
		$int_ref_wise_po_id_array[$val['INT_REF']] = $val['PO_ID'];	
		$job_id_array[$val['ID']] = $val['ID'];	
	}
	// print_r($buyer_data_array);die();
	unset($sql_gmt_res);
	...
	// $bookPoCond = str_replace("b.id", "b.id", $poCond);
	$sqlBook = "SELECT a.booking_no from wo_booking_dtls a,  wo_po_break_down b where a.po_break_down_id=b.id and a.booking_type in (1,4,3) and a.status_active=1 and b.status_active=1 $poCond";
	// echo $sqlBook;die;
	$res = sql_select($sqlBook);
	$search_book_arr = array();
	foreach ($res as $val)
	{
		$search_book_arr[$val[csf("booking_no")]] = $val[csf("booking_no")];
	}

	// echo "<pre>"; print_r($search_book_arr);die();
    /*==========================================================================================/
	/									grey and fin fabric	rcv									/
	/==========================================================================================*/
	$greyfabPoCond = str_replace("b.id", "b.po_breakdown_id", $poCond);
	$sqlgreyfab = "SELECT e.grouping as INT_REF, a.TRANSACTION_TYPE,c.RECEIVE_PURPOSE,a.ITEM_CATEGORY, b.QUANTITY,b.RETURNABLE_QNTY,g.FABRIC_SOURCE,g.booking_type  from inv_receive_master c, inv_transaction a, order_wise_pro_details b,wo_po_details_master d,wo_po_break_down e,pro_batch_create_mst f,wo_booking_mst g where c.id=a.mst_id and a.id=b.trans_id and a.prod_id=b.prod_id and d.id=e.job_id and e.id=b.po_breakdown_id and a.item_category in(2,13) and f.booking_no=g.booking_no and g.booking_type in(1,3,4) and a.transaction_type in(1,4) and f.id=a.pi_wo_batch_no and f.batch_against in(0,1,2) and f.status_active=1 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $greyfabPoCond ";//and f.batch_against=1  and g.booking_type=1
	// echo $sqlgreyfab;die();
	$greyfabRes = sql_select($sqlgreyfab);
	$fab_data_array = array();
	foreach ($greyfabRes as $val) 
	{
		...
		else
		{
			if($val['BOOKING_TYPE']==1)
			{
				$fab_data_array[$val['INT_REF']][$val['ITEM_CATEGORY']][$val['TRANSACTION_TYPE']]['main_receive_qnty'] += $val['QUANTITY'];
			}
			// if($val['FABRIC_SOURCE']!=2)
			// {
	...
	// echo "<pre>"; print_r($fab_data_array);die();
    /*==========================================================================================/
	/									grey and fin fabric	issue								/
	/==========================================================================================*/
	$greyfabPoCond = str_replace("b.id", "b.po_breakdown_id", $poCond);
	...
	$transPoCond = str_replace("b.id", "b.to_order_id", $poCond);
	$transPoCond2 = str_replace("b.id", "b.from_order_id", $poCond);
	$sqlTrnsfrIn = "SELECT  a.ITEM_CATEGORY,d.grouping as INT_REF,e.cons_quantity as QUANTITY from inv_item_transfer_mst a,inv_item_transfer_dtls b,wo_po_details_master c,wo_po_break_down d,inv_transaction e where a.id=b.mst_id and c.id=d.job_id and a.item_category in(2,13) and e.id=b.to_trans_id and e.transaction_type in(5) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.transfer_criteria in(2,4) and a.entry_form in(13,14) and d.id=b.to_order_id  $transPoCond";
	// echo $sqlTrnsfrIn;die();
	$trnsfrResIn = sql_select($sqlTrnsfrIn);
	$transfer_in_data_array = array();
	foreach ($trnsfrResIn as $val)
	...
	// print_r($transfer_in_data_array);die();
	//======================================================================================
	$sqlTrnsfrOut = "SELECT  a.ITEM_CATEGORY,d.grouping as INT_REF,e.cons_quantity as QUANTITY from inv_item_transfer_mst a,inv_item_transfer_dtls b,wo_po_details_master c,wo_po_break_down d,inv_transaction e where a.id=b.mst_id and c.id=d.job_id and e.id=b.trans_id and d.id=b.from_order_id and a.item_category in(2,13) and e.transaction_type in(6)  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.transfer_criteria in(2,4) and a.entry_form in(13,14) and b.active_dtls_id_in_transfer=1 $transPoCond2";
	// echo $sqlTrnsfrOut;die();
	$trnsfrResOut = sql_select($sqlTrnsfrOut);
	$transfer_out_data_array = array();
	foreach ($trnsfrResOut as $val) 
	        ...
	        <thead>
		        ...
		        <?			
		        	...
		        	foreach ($gmt_array as $intRef => $val) 
		        	{
		        		...
		        		$main_receive_qnty 	= $fab_data_array[$intRef][2][1]['main_receive_qnty'];
		        		$fin_fab_rcv 	= $fab_data_array[$intRef][2][1]['all_receive_qnty'];
		        		...
		        		// echo $intRef."==".$all_fin_fab_rcv."+". $fin_trnsf_in ."+".$fin_fab_iss_rtn .")-(".$fin_fab_issue ."+". $fin_trnsf_out ."+". $fin_fab_rcv_rtn .")<br>";
						$fin_fab_lost_ttip = "(Rcv=".$all_fin_fab_rcv.",Transfer In=". $fin_trnsf_in .", Issue Rtn=".$fin_fab_iss_rtn .")-(Issue=".$fin_fab_issue .",Trans Out=". $fin_trnsf_out .", Rcv Rtn=". $fin_fab_rcv_rtn .")";
				        ...
				        <tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>" style="font-size:11px">
				            ...
				            <td align="right"><? echo number_format($yarn_dyed_loss,2); ?></td>
				            <td align="right">
								<a href="javascript:void()" onclick="garments_popup('<?=$int_ref_wise_po_id_array[$intRef];?>','batch_popup')">
									<? echo number_format($dyeing_pro_los,2); ?>
								</a>
							</td>
				            <td title="<?="Grey Used=".$aop_grey_fab.",  Total Rev. Qty=".$aop_fin_fab;?>" align="right"><? echo number_format($aop_pro_loss,2); ?></td>
				            ...
				            <td align="right"><? echo number_format($yarn_lost_qty,0); ?></td>
				            <td align="right"><? echo number_format($grey_fab_lost,0); ?></td>
				            <td align="right" title="<?=$fin_fab_lost_ttip;?>"><? echo number_format($fin_fab_lost,0); ?></td>
				            <td align="right" title="At Cutting=<? echo $cutting_lost;?>&#13;At Printing=<? echo $print_lost_qty;?>&#13;At Embroidery=<? echo $emb_lost_qty;?>&#13;At Sewing=<? echo $sewing_lost;?>">
				            	<? echo number_format($cutpanel_lost,0); ?>
				            </td>
				            <td align="right" title="At Wash = <? echo $wash_lost_qty;?>"><? echo number_format($lost_gmts_qty,0); ?></td>
            ...
            <table valign="middle" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all" width="960">
                ...
                <tbody>
                    ...
                    foreach ($dataArray as $colorName => $colorData) 
                    {
                        ...
                        { 
                            ...
                            // echo $finFabArray[$batchId]['fin_qty'] ."+". $finFabArray[$redyeingBatchIdArray[$batchId]]['fin_qty']."<br>";
                            ...
                            <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('trs_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="trs_<? echo $i; ?>">
                                ...
                                <td align="right"><? echo number_format($dyeing_qty,0);?></td>
                                ...
                                <td align="right"><? echo number_format($grey_qty,0);?></td>
                                <td align="right"><? echo ($rcv_date !="") ? date('d M',strtotime($rcv_date)) : '';?></td>
                                <td align="right"><? echo number_format($fin_qty,0);?></td>
                                <td align="right"><? echo number_format($reject_qty,0);?></td>
                                <td align="right"><? echo number_format($pl,2);?></td>
                            </tr>
                            <?
                            $i++;
                            $batchQty   += $row['qty'];
                        ...
                        <tr class="tbl_footer" style="font-weight:bold;text-align:right;background: #dccdcd">
                            ...
                            <td></td>
                            <td>Total</td>
                            <td><? echo number_format($batchQty,0);?></td>
                            <td><? echo number_format($dyeingQty,0);?></td>
                            <td></td>
                            <td></td>
                            <td></td>
                            <td><? echo number_format($finQty,0);?></td>
                            <td></td>
                            <td><? echo number_format($delQty,0);?></td>
                            <td><? echo number_format($rejQty,0);?></td>
                            <td><? echo number_format($plQty,2);?></td>
                        </tr>
                        <?
                    }
                ...
                <tfoot>
                    <tr>
                        ...
                        <th></th>
                        <th>Grand Total</th>
                        <th><? echo number_format($gr_batchQty,0);?></th>
                        <th><? echo number_format($gr_dyeingQty,0);?></th>
                        <th></th>
                        <th></th>
                        <th></th>
                        <th><? echo number_format($gr_finQty,0);?></th>
                        <th></th>
                        <th><? echo number_format($gr_delQty,0);?></th>
                        <th><? echo number_format($gr_rejQty,0);?></th>
                        <th><? echo number_format($gr_plQty,2);?></th>
                    </tr>
                </tfoot>
            </table>
        </div>   
