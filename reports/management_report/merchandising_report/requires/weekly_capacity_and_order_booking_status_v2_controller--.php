...
if($action=="report_generate")
{
	...
	// echo $cbo_year;
	$weekarr=week_of_year($cbo_year,"Sunday");*/
	// print_r($weekarr);die();
	//$con = connect();
...
//	$po_cond_booking = str_replace("d.po_break_down_id", "a.po_break_down_id", $po_ids_cond);
	...
	$booking_sql=sql_select("SELECT a.po_break_down_id ,c.color_number_id,a.booking_no,a.fin_fab_qnty from wo_booking_dtls a,wo_po_color_size_breakdown c  where c.id=a.color_size_table_id  and  a.status_active=1 and a.booking_type=1 and a.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 $booking_no_cond $country_cond $date_cond  ");
	...
	$fab_sql = "SELECT sum(b.receive_qnty) as qnty,b.order_id,c.style_ref_no 
	FROM inv_receive_master a, pro_finish_fabric_rcv_dtls b, wo_po_details_master c, wo_po_break_down d
	WHERE a.id=b.mst_id and b.order_id=to_char(d.id) and c.job_no=d.job_no_mst and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and a.company_id=$company_name 
	GROUP BY b.receive_qnty,b.order_id,c.style_ref_no";
	$fab_sql_res = sql_select($fab_sql);
	$rcv_qty_arr = [];
	foreach ($fab_sql_res as $key => $val) 
	{
		$rcv_qty_arr[$val[(csf('style_ref_no'))]][$val[(csf('order_id'))]]['qnty'] += $val[(csf('qnty'))];
	}
	//============================================== FOR CM P/DOZN ====================================
/*	$po_cond_for_cm = str_replace("d.po_break_down_id", "b.id", $po_ids_cond);	
	$cm_sql="SELECT a.job_no_prefix_num as job_prefix,a.job_no, a.gmts_item_id,a.avg_unit_price,a.company_name, a.buyer_name, a.style_description as style_desc, a.style_ref_no, a.order_uom, a.total_set_qnty as ratio, b.id as po_id,b.file_no,b.po_quantity,b.plan_cut,b.po_total_price, b.po_number, b.pub_shipment_date 
	FROM wo_po_details_master a, wo_po_break_down b  
	WHERE a.job_no=b.job_no_mst  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.company_name in($company_name) $po_cond_for_cm
	ORDER BY b.id ";
	$cm_sql_res = sql_select($cm_sql);
	foreach ($cm_sql_res as $key => $row) 
	{
		$style_wise_arr[$row[csf("style_ref_no")]][$row[csf("po_id")]]['qty']+=$row[csf('po_quantity')];
		$style_wise_arr[$row[csf("style_ref_no")]][$row[csf("po_id")]]['qty_pcs']+=$row[csf('po_quantity')]*$row[csf('ratio')];
		$style_wise_arr[$row[csf("style_ref_no")]][$row[csf("po_id")]]['plan_cut']+=$row[csf('plan_cut')];
		$job_wise_arr[$row[csf("job_no")]]['po_amount']+=$row[csf('po_total_price')];
	}
	// =========================================== for fab details ====================================
	$sql_fab="SELECT a.job_no_prefix_num as job_prefix,a.job_no, a.company_name, a.buyer_name, a.style_description as style_desc, a.style_ref_no, a.order_uom, a.total_set_qnty as ratio, b.id as po_id,b.po_quantity,b.po_total_price, b.po_number, b.pub_shipment_date,c.id, c.job_no,c.item_number_id, c.body_part_id as body_id, c.fab_nature_id as nat_id, c.color_type_id as color_type, c.fabric_description as fab_desc, c.avg_cons,c.uom, c.fabric_source as fab_source, c.rate, c.amount, c.avg_finish_cons, c.status_active 
	FROM wo_po_details_master a, wo_po_break_down b,wo_pre_cost_fabric_cost_dtls c  
	WHERE a.job_no=b.job_no_mst and c.job_no=b.job_no_mst and c.job_no=a.job_no  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.company_name in($company_name) $po_cond_for_cm $style_cond $buyer_cond  
	ORDER BY b.id,c.fab_nature_id,c.fabric_source, c.fabric_description,c.uom";
*/
	// echo "<pre>";
				...
				foreach ($main_array as $style_id => $style_data) 
				{
					...
					foreach ($style_data as $po_id => $row) 
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_1nd<? echo $jj; ?>','<? echo $bgcolor; ?>')" id="tr_1nd<? echo $jj; ?>">
							...
							<td style="word-wrap: break-word;word-break: break-all;" width="80" align="right"><? $rcv_qnty = $rcv_qty_arr[$style_id][$po_id]['qnty']; echo number_format($rcv_qnty,0); ?></td>
					...
					<tr bgcolor="#CCCCCC">
                        ...
                        <td style="word-wrap: break-word;word-break: break-all;" width="80"></td>
						<td style="word-wrap: break-word;word-break: break-all;" width="80"></td>
						<td style="word-wrap: break-word;word-break: break-all;" width="80"><? echo number_format($style_wise_fob_qty,0); ?></td>
						<td style="word-wrap: break-word;word-break: break-all;" width="80"></td>
						<td style="word-wrap: break-word;word-break: break-all;" width="80"></td>
