...
if($action=="get_pre_cost_data")
{
	...
	list($cbo_company_name,$location_id,$cbo_year_name,$cbo_month,$cbo_end_year_name,$cbo_month_end)=explode('_',$data);
...
$sql_con_po="SELECT a.set_smv, a.total_set_qnty, b.id as po_id, b.pub_shipment_date as shipment_date, b.po_total_price
	...
	WHERE a.job_no=b.job_no_mst AND b.job_no_mst=c.job_no AND a.job_no=c.job_no AND c.entry_from=111 AND a.company_name=$cbo_company_name $locatin_cond AND b.pub_shipment_date between '$s_date' and '$e_date' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";	
	//
	//echo $sql_con_po;die;
...
*/		
		...
		$fab_purchase_knit=$fabric_costing_arr['knit']['grey'][$row_po[csf('po_id')]];
		$fab_purchase_woven=$fabric_costing_arr['woven']['grey'][$row_po[csf('po_id')]];
		$fab_purchase=$fab_purchase_knit+$fab_purchase_woven;
		$yarn_dyeing_cost=$conversion_costing_arr_process[$row_po[csf('po_id')]][30];
		$heat_setting_cost=$conversion_costing_arr_process[$row_po[csf('po_id')]][33];
...
*/		
		...
		{
			$knit_cost+=$conversion_costing_arr_process[$row_po[csf('po_id')]][$process_id];	
		}
		
		$washing_cost=0;
		foreach($washing_cost_arr as $w_process_id)
		{
			$washing_cost+=$conversion_costing_arr_process[$row_po[csf('po_id')]][$w_process_id];	
		}
		
		$all_over_cost=0;
		foreach($aop_cost_arr as $aop_process_id)
		{
			$all_over_cost+=$conversion_costing_arr_process[$row_po[csf('po_id')]][$aop_process_id];	
		}
		
		$fabric_dyeing_cost=0;
		foreach($fabric_dyeingCost_arr as $fab_process_id)
		{
			$fabric_dyeing_cost+=$conversion_costing_arr_process[$row_po[csf('po_id')]][$fab_process_id];	
		}
		
		$fabric_finish=0;
		foreach($fab_finish_cost_arr as $fin_process_id)
		{
			$fabric_finish+=$conversion_costing_arr_process[$row_po[csf('po_id')]][$fin_process_id];	
		}
