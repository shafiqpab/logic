...
if ($action == "load_drop_down_buyer") {
    echo create_drop_down("cbo_buyer_name", 110, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond  and buy.id in (select buyer_id from  lib_buyer_party_type where party_type in (1,3,21,90)) group by buy.id, buy.buyer_name order by buyer_name", "id,buyer_name", 1, "-- Select Buyer --", $selected, "load_drop_down( 'requires/shipment_date_wise_wp_report_controller', this.value, 'load_drop_down_season', 'season_td');");
    exit();
}
...
if ($action == "report_generate") {
    ...
    //echo $cbo_order_status.'system';die;
    ...
    if($cbo_group_by==2){
        $group_cond="order by a.job_no desc ";

    }else{
        $group_cond="order by b.pub_shipment_date,a.job_no_prefix_num,b.id";
    }
    ...
    <div align="center">
        ...
        if (str_replace("'", "", $cbo_search_by) == 3) {
            ...
            <div style="max-height:400px; overflow-y:scroll; width:4340px" align="left" id="scroll_body">
                <table width="4320" border="1" class="rpt_table" rules="all" id="table_body">
                    <?
                } else {
                    ?>
                    <table width="3880" id="table_header_1" border="1" class="rpt_table" rules="all">
                        <thead>
                            <tr>
                                <th width="40">SL</th>
                                <th width="60">Job No</th>
                    ...
                    <div style="max-height:400px; overflow-y:scroll; width:3900px" align="left" id="scroll_body">
                        <table width="3880" border="1" class="rpt_table" rules="all" id="table_body">
                            <?
                        }
                        ...
                        if (str_replace("'", "", $cbo_search_by) == 1 || str_replace("'", "", $cbo_search_by) == 2) {
							...
							from 
								wo_po_details_master a, wo_po_break_down b ,wo_pre_cost_mst c
							where  
								a.job_no=b.job_no_mst  and a.job_no=c.job_no and b.job_no_mst=c.job_no and c.entry_from=158 and a.status_active=1 and b.status_active=1 and c.status_active=1 and a.style_ref_no like '$style_ref_cond' $date_cond $searchCond $job_no_cond $year_cond $file_no_cond $ref_no_cond $team_name_cond $team_member_cond $season_id_cond $order_status_cond group by b.job_no_mst order by b.job_no_mst desc";
                                } else {
						...
						(CASE WHEN production_type ='1' THEN production_quantity END) AS cutting,
						(CASE WHEN production_type ='3' THEN production_quantity END) AS printing,
						(CASE WHEN production_type ='3' and embel_name=1 THEN production_quantity END) AS prnt,
						(CASE WHEN production_type ='3' and embel_name=2 THEN production_quantity END) AS embel,
						(CASE WHEN production_type ='3' and embel_name=3 THEN production_quantity END) AS wash,
						(CASE WHEN production_type ='3' and embel_name=4 THEN production_quantity END) AS special,
						(CASE WHEN production_type ='5' THEN production_quantity END) AS sewingout,
						(CASE WHEN production_type ='6' THEN production_quantity END) AS finishinput,
						(CASE WHEN production_type ='7' THEN production_quantity END) AS ironoutput,
						(CASE WHEN production_type ='8' THEN production_quantity END) AS finishcompleted,
                        (CASE WHEN production_type ='11' THEN production_quantity END) AS poly
						from pro_garments_production_mst
						where status_active=1 and is_deleted=0 $poIds_cond_prod ";
                            //echo $prod_sql;die;			
                            ...
                            foreach ($prodSQLresult as $key => $val) {
                                ...
                                $prodArr[$val[csf('po_break_down_id')]]['poly'] += $val[csf('poly')];
                            }
						...
						where status_active=1 and is_deleted=0 ";
                            ...
                            foreach ($prodSQLresult as $key => $val) {
                                ...
                                $prodArr[$val[csf('po_break_down_id')]][$val[csf('country_id')]]['poly'] += $val[csf('poly')];
                            } //var_dump($prodArr);
                            //buyer inspection query----------------------------------//					
                        ...
                        //for Fabric Booking percentage----------------------------------//
                           $fabricSQL = "select a.id as bookingId,a.booking_no, a.item_category, b.po_break_down_id, (b.grey_fab_qnty) as grey_fab_qnty, (b.fin_fab_qnty) as fin_fab_qnty
							 from wo_booking_mst a, wo_booking_dtls b ,wo_pre_cost_fabric_cost_dtls c
							 where a.booking_no=b.booking_no and b.pre_cost_fabric_cost_dtls_id=c.id and b.job_no=c.job_no and a.booking_type!=3 and a.item_category in (2,3,13) and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 $poIds_cond ";
                        ...
                        foreach ($fabricSQLresult as $key => $val) {
                            ...
                            $bookingIdArr[$val[csf('bookingId')]]=$val[csf('bookingId')];
                            $bookingPOWiseNoArr[$val[csf('po_break_down_id')]]=$val[csf('booking_no')];

                        }
                       $bookingId_cond=where_con_using_array($bookingIdArr,0,'booking_id');
                        //echo $fabricSQL;die;
                     //   echo "select id  as salesMstId from fabric_sales_order_mst where 1=1 and status_active=1 $bookingId_cond";
                        $sql_sales_for=sql_select("select sales_booking_no,booking_id, id  as salesMstId from fabric_sales_order_mst where 1=1 and status_active=1 $bookingId_cond");
                        foreach( $sql_sales_for as $row)
                        {
                            $salesMstIdArr[$row[csf("salesMstId")]]=$row[csf("salesMstId")]; 
                            $fso_bookingNoArr[$row[csf('sales_booking_no')]]=$row[csf('salesMstId')];
                            $bookingFsoNoArr[$row[csf('salesMstId')]]=$row[csf('sales_booking_no')];
                        }
                        //  $noOfPos = count(explode(",", chop($poIds, ',')));
                        $salesMstId_cond=where_con_using_array($salesMstIdArr,0,'c.po_breakdown_id');
                        ...
                        $knitSQL_sales = "select c.po_breakdown_id,
                        (CASE WHEN c.entry_form in(2,22,58) and c.trans_id<>0 THEN c.quantity ELSE 0 END) AS grey_receive,
                        (CASE WHEN c.entry_form =13 and c.trans_type=5 THEN c.quantity ELSE 0 END) AS transfer_in_qnty_grey,
                        (CASE WHEN c.entry_form =13 and c.trans_type=6 THEN c.quantity ELSE 0 END) AS transfer_out_qnty_grey,
                        (CASE WHEN c.entry_form in(7,37) and c.trans_id<>0 THEN c.quantity ELSE 0 END) AS finish_receive,
                        (CASE WHEN c.entry_form ='66' THEN c.quantity ELSE 0 END) AS finish_receive_roll_wise,
                        (CASE WHEN c.entry_form=17 THEN c.quantity ELSE 0 END) AS woven_fin_receive,
                        (CASE WHEN c.entry_form =15 and c.trans_type=5 THEN c.quantity ELSE 0 END) AS transfer_in_qnty_fin,
                        (CASE WHEN c.entry_form =15 and c.trans_type=6 THEN c.quantity ELSE 0 END) AS transfer_out_qnty_fin
                     from inv_transaction b,order_wise_pro_details c
                     where c.trans_id=b.id and  c.entry_form in(2,22,7,37,13,15,17,66,58) and b.is_deleted=0 and b.status_active=1  and c.is_deleted=0 and c.status_active=1 $salesMstId_cond ";
               
                 $knitSQLresult_sales = sql_select($knitSQL_sales);
                foreach ($knitSQLresult_sales as $key => $val) {
                    $bookingFso= $bookingFsoNoArr[$val[csf('po_breakdown_id')]];
                    $tot_qty=$val[csf('grey_receive')] + $val[csf('transfer_in_qnty_grey')] - $val[csf('transfer_out_qnty_grey')];
                    if($tot_qty && $bookingFso!="")
                    {
                        $knitSalesArr[$bookingFso]+= $val[csf('grey_receive')] + $val[csf('transfer_in_qnty_grey')] - $val[csf('transfer_out_qnty_grey')];
                    }
                    
                  //  $finishArr[$val[csf('po_breakdown_id')]] = $val[csf('finish_receive')] + $val[csf('transfer_in_qnty_fin')] + $val[csf('finish_receive_roll_wise')] - $val[csf('transfer_out_qnty_fin')];
                   // $wovenfinishArr[$val[csf('po_breakdown_id')]] = $val[csf('woven_fin_receive')];
                }
                // print_r($knitSalesArr);
                            ...
                            foreach ($jobSqlResult as $row) {
                                ...
                                if ($cbo_ls_sc == 1 || ($cbo_ls_sc == 2 && $lcsc_amnt > 0) || ($cbo_ls_sc == 3 && $lcsc_amnt <= 0)) {
                                    ...
                                    <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                                        ...
                                        <td width="50" align="center" onclick="openmypage_image('requires/shipment_date_wise_wp_report_controller.php?action=show_image&job_no=<? echo $row[csf('job_no')]; ?>', 'Image View')"><img src='../../../<? echo $imge_arr[$row[csf('job_no')]]; ?>' height='25' width='30' /></td>
                                        ...
                                        <td width="80" title="<?=$fabricArr[$row[csf('id')]].'='.$reqArr[$row[csf('id')]];?>" align="right">
                                            <?
                                        ...
                                        <td width="80" align="right" <? //echo $bg_color_wv_booking;?>> 
                                            ...
                                            <td width="80" align="right">
                                                <?
                                                $knitFin=0; $knit_perc=0; $fso_id=''; $bookingPO='';
                                                 $bookingPO=$bookingPOWiseNoArr[$row[csf('id')]];
                                                 $fso_id= $fso_bookingNoArr[$bookingPO];
                                                // $knitSalesArr[$bookingFso];
                                                 
                                                 if($fso_id!="" && $bookingPO!="")
                                                 {
                                                    if($knitSalesArr[$bookingPO])
                                                    {
                                                        $knitFin=$knitSalesArr[$bookingPO];
                                                        //echo $fso_id.'='.$knitFin.'='.$fabricArr[$row[csf('id')]].'<br>';
                                                    }
                                                   
                                                 }
                                                 else{
                                                    $knitFin=$knitArr[$row[csf('id')]];
                                                    
                                                 }
                                                // if($knitFin=='')  $knitFin=0;
                                               // $knit_perc = $knitArr[$row[csf('id')]] * 100 / $fabricArr[$row[csf('id')]];
                                              
                                               $knit_perc_cal=0;
                                               if($knitFin && $fabricArr[$row[csf('id')]])
                                               {
                                                $knit_perc_cal = $knitFin * 100 / $fabricArr[$row[csf('id')]];
                                                $knit_perc= fn_number_format($knit_perc_cal,4,'.','');
                                               }
                                               else $knit_perc=0;
                                               ...
                                               // echo $knit_perc.'='.$knitArr[$row[csf('id')]].'='.$fabricArr[$row[csf('id')]].'<br>';
                                                ?>
                ...
                <? } ?>
                                        <td width="80" align="right"> 
                                            ...
                                            <td width="80" align="right"> 
                                                ...
                                                $finish_perc= fn_number_format($finish_perc,4,'.','');
                                                if($finish_perc=='')  $finish_perc=0;
                ...
                <? } ?>
                                        <td width="80" align="center"> 
                                            <a href="##" onclick="show_trims_rec('trims_rec_popup', '<? echo $row[csf("po_number")]; ?>', '<? echo $row[csf('id')]; ?>', '1300px')">View</a>
                                        </td>
                                        ...
                                        <td width="80" align="right"> 
                                           ...
                                           elseif ($embel_qty) {
                                            ...
                                            else  $embPercentage =0;
                                           
                                           // echo $embPercentage.'='.$totalP.'='.$n.'=B<br>';
                                            $task_finish_date = '';
                                            $notice_date_end = '';
                                            $actual_finish_date = '';
                                            if ($tna_integrated == 1) {
                                            ...
                                            $embPercentage= fn_number_format($embPercentage,4,'.','');
                                            if($embPercentage=='')  $embPercentage=0;
                                        ...
                                        <td width="80" align="right" title="Calculation Based on Order Qnty."> 
                                            <a href="##" onclick="show_progress_report_details('poly_details_popup', '<? echo $row[csf("job_no")]; ?>_<? echo $row[csf('id')]; ?>', '850px')">
                                                <?
                                                $poly = $prodArr[$row[csf('id')]]['poly'];
                                                if ($poly != '') {
                                                    $poly_qty_perc = ($poly * 100) / ($row[csf('po_quantity')] * $row[csf('total_set_qnty')]);
                                                    if(is_infinite($poly_qty_perc) || is_nan($poly_qty_perc)){$poly_qty_perc=0;}

                                                    echo number_format($poly_qty_perc, 2) . ' ' . '%';
                                                } else
                                                    echo '0.00' . '%';
                                                ?>
                                            </a>
                            ...
                            foreach ($jobSqlResult as $row) {
                                ...
                                foreach ($po_id as $id) {
                                    ...
                                    $bookingPO=$bookingPOWiseNoArr[$id];
                                    $fso_id= $fso_bookingNoArr[$bookingPO];
                                  //  $knitSalesArr[$bookingFso];
                                   // echo $bookingPO.'='.$fso_id.'';
                                    if($fso_id!="")
                                    {
                                       $knitFin=$knitSalesArr[$bookingPO];
                                    }
            ...
            <? } ?>
                                    ...
                                    <td width="80" align="right"> 
                                        <a href="##" onclick="show_progress_report_details('poly_details_popup', '<? echo $row[csf("job_no")]; ?>_<? echo $row[csf('id')]; ?>', '850px')">
                                            ...
                                            // $ironoutput_qnty =  $prodArr[$row[csf('job_no')]]['ironoutput'];
                                            if ($poly != '') {
                                                $poly_qnty_perc = ($poly * 100) / $row[csf('po_quantity')];
                                                echo number_format($poly_qnty_perc, 2) . ' ' . '%';
                                            } else
                                                echo '0.00' . '%';
                                            ?>
                                        </a>
						...
						where  
                            ...
                            foreach ($jobSqlResult as $row) {
                                ...
                                foreach ($country_data as $country_id) {
                                    ...
                                    $poly += $prodArr[$row[csf('id')]][$country_id]['poly'];
            ...
            ?>
                                    ...
                                    <td width="80" align="right"> 
                                        <a href="##" onclick="show_progress_report_details('poly_details_popup', '<? echo $row[csf("job_no")]; ?>_<? echo $row[csf('id')]; ?>', '850px', '<? echo $type_search; ?>', '<? echo $row[csf('country_id')]; ?>')">
                                            <?
                                            if ($poly != '') {
                                                $poly_perc = ($poly * 100) / ($row[csf('po_quantity')]);
                                                if(is_infinite($poly_perc) || is_nan($poly_perc)){$poly_perc=0;}
                                                echo number_format($poly_perc, 2) . ' ' . '%';
                                            } else
                                                echo '0.00' . '%';
                                            ?>
                                        </a>
    ...
    if (str_replace("'", "", $cbo_search_by) == 3) {
        ?>
                        <table border="1" class="rpt_table" width="4320" rules="all" id="report_table_footer" >
                            <tfoot>
                            <th width="40">&nbsp;</th>
                            <th width="60"></th>
                            <th width="60"></th>
        ...
        ?>
                        <table border="1" class="rpt_table"  width="3880" rules="all" id="report_table_footer" >
                            <tfoot>
                            <th width="40">&nbsp;</th>
                            <th width="60"></th>
                            <th width="60"></th>
...
//#############################################################################################
    ...
    ?>
                        ...
                        <?
                        $bgcolor = "#EFEFEF";
                        $sql_book=sql_select("select a.id,b.grey_fab_qnty as grey_fab_qnty from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and b.po_break_down_id in ($po_id) and a.item_category in (2,13) and b.status_active=1 and b.is_deleted=0"); $booking_qnty=0;
                        foreach( $sql_book as $row)
                        {
                            $booking_qnty+=$row[csf("grey_fab_qnty")]; 
                            $bookingIdArr[$row[csf("id")]]=$row[csf("id")]; 
                        }
                        //echo $booking_qnty.'=A';
                        $bookingid=implode(",",$bookingIdArr);
                        $sql_sales_for=sql_select("select id  as salesMstId from fabric_sales_order_mst where booking_id in($bookingid)");
                        foreach( $sql_sales_for as $row)
                        {
                            $salesMstId=$row[csf("salesMstId")]; 
                        }
                        if($salesMstId)
                        {
                            $po_id=$salesMstId;
                     ...
                     //   $booking_qnty = return_field_value("sum(b.grey_fab_qnty) as grey_fab_qnty", "wo_booking_mst a, wo_booking_dtls b", "a.booking_no=b.booking_no and b.po_break_down_id in ($po_id) and a.item_category in (2,13) and b.status_active=1 and b.is_deleted=0", "grey_fab_qnty");
                        ...
                        $grey_receive_sql = sql_select("select sum(case when a.entry_form=2 and a.receive_basis in(2,1) then c.quantity else 0 end) as grey_auto_rcv, sum(case when a.entry_form in(22,58) and a.receive_basis in(9,10) then c.quantity else 0 end) as grey_prod_rcv 
    ...
    if ($action == "poly_details_popup") 
    {
       // echo "test poly";die;
        echo load_html_head_contents("Poly Output Details", "../../../../", 1, 1, $unicode, '', '');
        ...
        if ($type == 3) 
        {
            $sql = "select c.size_number_id,c.color_number_id,sum(b.production_qnty) as production_qnty
            from pro_garments_production_mst a, pro_garments_production_dtls b, wo_po_color_size_breakdown c 
            where a.id=b.mst_id and b.color_size_break_down_id=c.id and a.po_break_down_id in ($po_id) and c.country_id in($country_name_arr) and a.production_type=11 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.status_active=1 and c.status_active=1 group by c.size_number_id,c.color_number_id order by c.color_number_id,c.size_number_id";
        } 
        else 
        {
            $sql = "select c.size_number_id,c.color_number_id,sum(b.production_qnty) as production_qnty
            from pro_garments_production_mst a, pro_garments_production_dtls b, wo_po_color_size_breakdown c 
            where a.id=b.mst_id and b.color_size_break_down_id=c.id and a.po_break_down_id in ($po_id) and a.production_type=11 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 group by c.size_number_id,c.color_number_id order by c.color_number_id,c.size_number_id";
        }
        ...
        ?>
                    <tr class="form_caption">
                        <td align="center" colspan="4"><strong>Poly Details</strong></td>    
                    </tr>
                    <tr>
                        <td align="right" width="130"> <strong>Job Number</strong> :</td> 
                        <td width="200"><? echo $job_number; ?></td> 
...
if ($action == "actual_shipment_popup") {
    ...
    <?
     if ($type == 3) {
          $sql_ex="select b.invoice_no,b.po_break_down_id as po_id,b.ex_factory_date from  pro_ex_factory_mst b where  b.po_break_down_id in ($po_id) and b.country_id in($country_name_arr) and b.status_active=1";
     }
     else
     {
          $sql_ex="select b.invoice_no,b.po_break_down_id as po_id, b.ex_factory_date from  pro_ex_factory_mst b where  b.po_break_down_id in ($po_id) and b.status_active=1";
     }
     $ex_fact_sql = sql_select($sql_ex);
     foreach ($ex_fact_sql as $row) 
     {
       $ex_fact_date_arr[$row[csf("po_id")]][$row[csf("invoice_no")]]=$row[csf("ex_factory_date")];
     }

    $i = 0;
    if ($type == 3) {
        $sql = "select c.id as in_mstId,a.po_breakdown_id,b.pub_shipment_date,c.invoice_no,a.current_invoice_qnty,a.current_invoice_value, c.actual_shipment_date 
    ...
    foreach ($fabric_sql as $row) {
        ...
        ?>
            ...
            <?
            if ($type == 3) {
                $poSQL = sql_select("select b.id,b.po_number,c.order_quantity as order_qnty from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c where a.id=b.job_id and  a.id=c.job_id and b.id=c.po_break_down_id and c.country_id in($country_name_arr)  and b.id=" . $row[csf("po_breakdown_id")]);
            } else {
                $poSQL = sql_select("select  b.id,b.po_number,b.po_quantity as order_qnty from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and b.id=" . $row[csf("po_breakdown_id")]);
            }
           
            foreach ($poSQL as $res)
                ;
                $actual_shipment_date=strtotime($row[csf("actual_shipment_date")]);
           
                 if( $actual_shipment_date=='') $actl_shipdate=change_date_format($ex_fact_date_arr[$row[csf("po_breakdown_id")]][$row[csf("in_mstId")]]);
                 else $actl_shipdate=change_date_format($row[csf("actual_shipment_date")]);
            ...
            ?>
                                ...
                                <td width="100"><? echo $actl_shipdate; ?>&nbsp;</td>
    ...
    ?>
        ...
        <div style="width:1250px" align="center">
            <fieldset style="width:1250px"> 
                ...
                <table cellpadding="0" width="1238px" class="rpt_table"   rules="all"  border="1">
                    <thead>
                        <tr>
                            <th rowspan="2" width="30">SL</th>
                ...
                <div style="width:1238px; max-height:300px; overflow-y:scroll">
                    <table cellpadding="0" width="1220px" cellspacing="0" border="1" align="left" rules="all" class="rpt_table">
                       <tbody>
                       <?
                        ...
                        foreach ($trimsDataResult as $row) {
                            ...
                            $trimsDataArr[$row[csf('po_break_down_id')]][$row[csf('trim_group')]]['wo_qnty'] += $row[csf('wo_qnty')];
                       ...
                       $sql = "select b.item_group_id,c.quantity as receive_qnty, c.order_amount as amount,c.po_breakdown_id as order_id from inv_receive_master a,inv_trims_entry_dtls b,order_wise_pro_details c where a.id=b.mst_id and b.id=c.dtls_id and c.entry_form=24  and a.entry_form=24 and c.trans_type=1 and c.po_breakdown_id in($po_id) and a.status_active = 1 and a.is_deleted = 0 and b.status_active = 1 and b.is_deleted = 0";
                        $result = sql_select($sql);
                        foreach ($result as $row) {
                            $recDataArr[$row[csf('order_id')]][$row[csf('item_group_id')]]['receive_qnty'] += $row[csf('receive_qnty')];
                        ...
                        $result = sql_select($sql);
                        foreach ($result as $row) {
                            $issueDataArr[$row[csf('order_id')]][$row[csf('item_group_id')]]['issue_qnty'] += $row[csf('issue_qnty')];
                        ...
                        foreach ($result as $row) {
                           ...
                           // $req_value = ($row[csf('rate')]) * $req_qnty;
                            ...
                            <tr bgcolor="<? echo $bgcolor; ?>" id="tr_<? echo $i; ?>">
                                <td width="30"><? echo $i; ?></td> 
                                <td   width=""><? echo $trim_group[$row[csf("trim_group")]]; ?></td>
                                ...
                                <td width="70" align="right" ><? $booking_qty = $trimsDataArr[$row[csf("po_break_down_id")]][$row[csf('trim_group')]]['wo_qnty']; echo number_format($booking_qty,2); ?></td>
                                <td width="70" align="right" ><? $booking_amu = $trimsDataArr[$row[csf("po_break_down_id")]][$row[csf('trim_group')]]['amount']; echo number_format($booking_amu,2); ?></td>
                                <td width="100" ><p style="word-break: break-all"><? echo $trimsDataArr[$row[csf("po_break_down_id")]][$row[csf('trim_group')]]['booking_no']; ?></p></td>
                                <td width="100" ><p style="word-break: break-all"><? echo $trimsDataArr[$row[csf("po_break_down_id")]][$row[csf('trim_group')]]['supplier_id']; ?></p></td>
                                <td width="70" ><p style="word-break: break-all"><? echo $trimsDataArr[$row[csf("po_break_down_id")]][$row[csf('trim_group')]]['booking_date']; ?></p></td>
                                <td width="70" ><p style="word-break: break-all"><? echo $trimsDataArr[$row[csf("po_break_down_id")]][$row[csf('trim_group')]]['delivery_date']; ?></p></td>
                                <td width="70" align="right" ><?
            ...
            ?></td>
             <td width="70" align="right"><? $issue_bal = $rec_qty  - $issue_qty; echo number_format($issue_bal, 2); ?></td>
          </tr>
            <?
