...
{
	echo create_drop_down( "cbo_buyer_name", 130, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,90))  group by buy.id, buy.buyer_name  order by buy.buyer_name","id,buyer_name", 1, "-- All Buyer --", $selected, "load_drop_down( 'requires/post_costing_report_controller_jk', this.value, 'load_drop_down_season_buyer', 'season_td');" );     	 
	exit();
}
...
if($action=="style_no_search_popup")
{
	...
	<script>
		...
		function js_set_value(str)
		{
			...
			//alert (str);
			$("#hide_order_no").val(splitData[2]);
			$("#hide_order_id").val(splitData[1]);
            ...
            <table width="770" cellspacing="0" cellpadding="0" border="1" rules="all" align="center" class="rpt_table" id="tbl_list">
                ...
                <tbody>
                	<tr>
                        ...
                        <td align="center">
                        	<input type="button" name="button" class="formbutton" value="Show" onClick="show_list_view ('<? echo $companyID; ?>'+'**'+document.getElementById('cbo_buyer_name').value+'**'+document.getElementById('cbo_search_by').value+'**'+document.getElementById('txt_search_common').value+'**'+document.getElementById('txt_date_from').value+'**'+document.getElementById('txt_date_to').value+'**'+document.getElementById('hide_pre_cost_ver_id').value, 'create_style_no_search_list_view', 'search_div', 'post_costing_report_controller_jk', 'setFilterGrid(\'tbl_list_search\',-1)');" style="width:100px;" />
                    	</td>
                    </tr>
                    <tr>
                        <td colspan="5" valign="middle"><? echo load_month_buttons(1); ?></td>
...
$sql="select  a.id,a.job_no, $year_field, a.job_no_prefix_num, a.company_name, a.buyer_name, a.style_ref_no from wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c where a.job_no=b.job_no_mst and a.job_no=c.job_no and  b.job_no_mst=c.job_no and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.company_name=$company_id and $search_field like '$search_string' $buyer_id_cond $date_cond $entry_form_cond group by a.id, a.job_no,a.insert_date, a.job_no_prefix_num, a.company_name, a.buyer_name, a.style_ref_no order by a.id desc";
	...
	echo create_list_view("tbl_list_search", "Style Ref. No,Job No,Year", "120,120,70","460","210",0, $sql , "js_set_value", "id,style_ref_no","",1,"0,0,0",$arr,"style_ref_no,job_no_prefix_num,year","",'','0,0,0','',1) ;
   exit(); 
}
...
{ 
	...
	$style_id_cond="";
	if(trim(str_replace("'","",$txt_style_no))!="")
	{
		if(str_replace("'","",$hide_style_id)!="") $style_id_cond=" and a.id in(".str_replace("'","",$hide_style_id).")";
		else $style_id_cond=" and LOWER(a.style_ref_no) like LOWER('%".trim(str_replace("'","",$txt_style_no))."%')";
	}
    ...
    <fieldset>
        ...
        <table style="margin-top:10px" id="table_header_1" class="rpt_table" width="4190" cellpadding="0" cellspacing="0" border="1" rules="all">
            <thead>
                <th width="40">SL</th>
                ...
                <th width="110">Finish Fabric Cost</th>
    	...
    	<div style="width:4212px; max-height:400px; overflow-y:scroll" id="scroll_body">
            <table class="rpt_table" width="4190" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
            <?
				...
				$receive_rtn_qty_data=sql_select("select b.po_breakdown_id as po_id, c.item_group_id, b.quantity as quantity, c.order_rate as rate
			from product_details_master c,order_wise_pro_details b
			where b.prod_id=c.id and b.trans_type=3 and b.entry_form=49 and b.status_active=1 and b.is_deleted=0 $po_cond_for_in4");
			...
			foreach($receive_rtn_qty_data as $row)
			{
				...
				$actualTrimsCostArray[$row[csf('po_id')]]['receive_rtn_amount']+=$receive_rtn_amount;
			}
			//echo "<pre>";print_r($style_data_arr);
			unset($receive_rtn_qty_data);
			
				$sql_rec_data=sql_select("select b.po_breakdown_id as po_id, a.item_group_id, c.receive_basis, a.booking_id, b.quantity as quantity, b.order_rate as rate, c.exchange_rate, (b.quantity*b.order_rate) as amount
			from inv_receive_master c, product_details_master d, inv_trims_entry_dtls a, order_wise_pro_details b
			where a.mst_id=c.id and a.trans_id=b.trans_id and a.prod_id=d.id and b.trans_type=1 and b.entry_form=24 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  $po_cond_for_in4 order by a.item_group_id ");
			//echo $receive_qty_data; die;
			foreach($sql_rec_data as $row)
			{
				$receive_rtn_amount=$actualTrimsCostArray[$row[csf('po_id')]]['receive_rtn_amount'];
				$amount=0;  $amount=($row[csf('quantity')]*$row[csf('rate')]);
				$actualTrimsCostArray[$row[csf('po_id')]]['inhouse_value']+=$amount-$receive_rtn_amount;
			}
			unset($sql_rec_data);
				...
				$total_fin_fab_transfer_amt_actual_in=$total_fin_fab_transfer_amt_actual_out=$total_grey_fab_transfer_amt_actual_in=$total_grey_fab_transfer_amt_actual_out=0;
				$tot_fin_fab_cost_actual=$tot_fin_fab_cost_mkt=0;
				...
				$hide_po_id=trim(str_replace("'","",$hide_order_id));
				...
				if(trim(str_replace("'","",$txt_order_no))!=''  && $hide_po_id=='')
				 {
				...
				foreach($result as $row)
				{
					...
					//echo $trans_in_amt.'='.$trans_out_amt.'DDDX';
					...
					$fin_trans_in=$fin_fab_trans_array[$row[csf('id')]][5];
				    $fin_trans_out=$fin_fab_trans_array[$row[csf('id')]][6];
					//echo  $fin_trans_out.'='. $knit_charge_mkt_fin;
					$fin_fab_transfer_amt_actual_in=$fin_trans_in*$knit_charge_mkt_fin;
					...
					$grey_transfer_in_cost_array[$row[csf('buyer_name')]]+=$grey_fab_transfer_amt_actual_in;
					$grey_transfer_out_cost_array[$row[csf('buyer_name')]]+=$grey_fab_transfer_amt_actual_out;
					$fin_transfer_in_cost_array[$row[csf('buyer_name')]]+=$fin_fab_transfer_amt_actual_in;
					$fin_transfer_out_cost_array[$row[csf('buyer_name')]]+=$fin_fab_transfer_amt_actual_out;
					
					$grey_fab_cost_mkt_array[$row[csf('buyer_name')]]+=$grey_fab_cost_mkt;
				...
				?>
                	<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
                        ...
                        <td width="108" title="Grey Fab Cost+Dye Fin Cost+AOP Cost" style="padding-right:2px" align="right"><? $fin_fab_cost_mkt=$grey_fab_cost_mkt+$dye_finish_cost_mkt+$aop_cost_mkt;
															$buyer_fin_fab_cost_mkt_array[$row[csf('buyer_name')]]+=$fin_fab_cost_mkt;
														$tot_fin_fab_cost_mkt+=$fin_fab_cost_mkt;echo fn_number_format($fin_fab_cost_mkt,2,'.',''); ?></td>
                        ...
                        <td style="" align="right"><? echo fn_number_format($mkt_margin_perc,2,'.',''); ?></td>
                    </tr>
...
"  align="right"><a href="#report_details" onClick="openmypage_actual('<? echo $row[csf('id')]; ?>','knit_cost_actual','Actual Knitting Cost Details','900px')"><? //echo number_format($grey_fab_cost_actual,2,'.',''); ?></a><? //echo number_format($grey_fab_cost_actual,2,'.',''); ?></td>
                        ...
                        <td width="108" style="padding-right:2px" align="right"><a href="#report_details" onClick="openmypage_actual('<? echo $row[csf('id')]; ?>','dye_finish_cost_actual','Actual Dyeing & Finish Cost Details','1100px')"><? echo fn_number_format($dye_finish_cost_actual,2,'.',''); ?></a></td>
                          ...
                          <td width="108"   style="padding-right:2px" title="Fin Transfer In"  align="right"><a href="#report_details" onClick="openmypage_actual('<? echo $row[csf('id')]; ?>','fin_transfer_cost_actual','Actual Fin Transfer Cost Details','900px')"><? echo fn_number_format($fin_fab_transfer_amt_actual_in,2,'.',''); ?></a><? //echo number_format($fin_fab_transfer_amt_actual_in,2,'.',''); ?></td>
                        <td width="108"   style="padding-right:2px"  align="right"><a href="#report_details" onClick="openmypage_actual('<? echo $row[csf('id')]; ?>','fin_transfer_cost_actual_out','Actual Fin Transfer Cost Details','900px')"><? echo fn_number_format($fin_fab_transfer_amt_actual_out,2,'.',''); ?></a><? //echo number_format($fin_fab_transfer_amt_actual_out,2,'.',''); ?></td>
                        ...
                        <td width="108" style="padding-right:2px" align="right"><a href="#report_details" onClick="openmypage_actual('<? echo $row[csf('id')]; ?>','aop_cost_actual','AOP Cost Details','800px')"><? echo fn_number_format($aop_cost_actual,2,'.',''); ?></a></td>
                         <td width="108" title="Grey Fab Cost+Dye fin cost+Aop Cost+Fin Trans In-Fin Trans Out" style="padding-right:2px" align="right"><a href="#report_details" onClick="openmypage_actual('<? echo $row[csf('id')]; ?>','finish_fab_cost_actual','Finish Cost Details','800px')"><?  
						 $fin_fab_cost_actual=$dye_finish_cost_actual+$aop_cost_actual+($fin_fab_transfer_amt_actual_in-$fin_fab_transfer_amt_actual_out);
						 $buyer_fin_fab_cost_actual_array[$row[csf('buyer_name')]]+=$fin_fab_cost_actual;
					$tot_fin_fab_cost_actual+=$fin_fab_cost_actual;
					
						  ?></a><? echo fn_number_format($fin_fab_cost_actual,2,'.','');?></td>
                        ...
                        <td style="" align="right"><? echo fn_number_format($actual_margin_perc,2,'.',''); ?></td>
                    </tr>
                    <tr bgcolor="#E9E9E9" onClick="change_color('tr_v<? echo $i; ?>','#E9E9E9')" id="tr_v<? echo $i; ?>">
                        ...
                        <td width="108" style="padding-right:2px" align="right">
                         ...
                         <td width="108" style="padding-right:2px" align="right">
                        	<? 
								$fin_fab_cost_varinace=$fin_fab_cost_mkt-$fin_fab_cost_actual;
								//$variance_aop_cost=$aop_cost_mkt-$aop_cost_actual;
								$variance_fin_fab_cost_per=($fin_fab_cost_varinace/$fin_fab_cost_mkt)*100; 
								echo fn_number_format($fin_fab_cost_varinace,2,'.',''); 
							?>
                        </td>
                    ...
                    <tr bgcolor="#DFDFDF" onClick="change_color('tr_vp<? echo $i; ?>','#DFDFDF')" id="tr_vp<? echo $i; ?>">
                        ...
                        <td width="108" style="padding-right:2px" align="right"><? echo fn_number_format($variance_fin_fab_cost_per,2,'.',''); ?></td>
                        ...
                        <td style="" align="right"><? echo fn_number_format($variance_per_cost_per,2,'.',''); ?></td>
                    </tr>
				<?
					$i++;
				}
        ...
        <table class="rpt_table" width="4194" cellpadding="0" cellspacing="0" border="1" rules="all">
            <tr bgcolor="#CCDDEE" onClick="change_color('tr_pt','#CCDDEE')" id="tr_pt" style="font-weight:bold;">
                <td colspan="9" style="padding-right:6px" align="right">Total</td>
                ...
                <td width="108" style="padding-right:2px" align="right"><? echo fn_number_format($tot_aop_cost_mkt,2,'.',''); ?></td>
                 <td width="108" style="padding-right:2px" align="right"><? echo fn_number_format($tot_fin_fab_cost_mkt,2,'.',''); ?></td>
                ...
                <td width="100" style="" align="right"><? echo fn_number_format($tot_mkt_margin_perc,2,'.',''); ?></td>
            </tr>
            <tr bgcolor="#CCCCFF" onClick="change_color('tr_at','#CCCCFF')" id="tr_at" style="font-weight:bold;">
                <td colspan="9" style="padding-right618px" align="right"></td>
                ...
                <td width="108" style="padding-right:2px" align="right"><? echo fn_number_format($tot_aop_cost_actual,2,'.',''); ?></td>
                 <td width="108" style="padding-right:2px" align="right"><? echo fn_number_format($tot_fin_fab_cost_actual,2,'.',''); ?></td>
                ...
                <td width="" style="" align="right"><? echo fn_number_format($tot_actual_margin_perc,2,'.',''); ?></td>
            </tr>
            <tr bgcolor="#FFEEFF" onClick="change_color('tr_vt','#FFEEFF')" id="tr_vt" style="font-weight:bold;">
                <td colspan="9" style="padding-right:68px" align="right"></td>
                <td width="88" style="padding-right:2px" align="right">&nbsp;</td>
                <td width="50">&nbsp;</td>
                <td width="70">&nbsp;</td>
                <td width="108" style="padding-right:2px" align="right">&nbsp;</td>
                ...
                <td width="108" style="padding-right:2px" align="right">
                    <? 
                       $tot_variance_fin_fab_cost=$tot_fin_fab_cost_mkt-$tot_fin_fab_cost_actual;
                       $tot_variance_fin_fab_cost_per=($tot_variance_fin_fab_cost/$tot_fin_fab_cost_mkt)*100; 
            ...
            <tr bgcolor="#CCCCEE" onClick="change_color('tr_vpt','#CCCCEE')" id="tr_vpt" style="font-weight:bold;">
                <td colspan="9" style="padding-right:6px" align="right"></td>
                <td width="88" style="padding-right:2px" align="right">&nbsp;</td>
                <td width="50">&nbsp;</td>
                <td width="70">&nbsp;</td>
                <td width="108" style="padding-right:2px" align="right">&nbsp;</td>
                ...
                <td width="108" style="padding-right:2px" align="right"><? echo fn_number_format($tot_variance_fin_fab_cost_per,2,'.',''); ?></td>
                ...
                <td width="" style="" align="right"><? echo fn_number_format($tot_variance_per_cost_per,2,'.',''); ?></td>
            </tr>
        </table>
        <br />
        <table width="3370">
                        ...
                        <table width="760" cellpadding="0" cellspacing="0" border="1" class="rpt_table" rules="all">
                            ...
                            <tr bgcolor="<? echo $bgcolor1; ?>"  onclick="change_color('trtd_7','<? echo $bgcolor1; ?>')" id="trtd_7">
                             ...
                             <tr bgcolor="<? echo $bgcolor1; ?>"  onclick="change_color('trtd_7','<? echo $bgcolor1; ?>')" id="trtd_7">
                                ...
                                <td>Finish Fabric Cost</td>
                                <td align="right"><? echo fn_number_format($tot_fin_fab_cost_mkt,2); ?></td>
                                <td align="right"><? echo fn_number_format(($tot_fin_fab_cost_mkt/$tot_po_value)*100,2); ?></td>
                                <td align="right"><? echo fn_number_format($tot_fin_fab_cost_actual,2); ?></td>
                                <td align="right"><? echo fn_number_format(($tot_fin_fab_cost_actual/$tot_ex_factory_val)*100,2); ?></td>
                                <td align="right"><? echo fn_number_format($tot_fin_fab_cost_mkt-$tot_fin_fab_cost_actual,2); ?></td>
                            </tr>
                ...
                <td width="" valign="top">
                    ...
                    <div id="summary_buyer">
                        ...
                        <table width="100%" cellpadding="0" cellspacing="0" border="1" class="rpt_table" rules="all">
                            <thead>
                                 <tr>
                                    <th colspan="28">Buyer Level Summary</th>
                                </tr>
                                <tr>
                                    <th width="35">SL</th>
                                    ...
                                    <th width="110">Yarn cost</th>
                                    <th width="110">Knitting Cost</th>
                                     <th width="110">Grey Fabric Transfer In Cost</th>
                                     <th width="110">Grey Fabric Transfer Out Cost</th>
                                     <th width="110">Grey Fabric Cost</th>
                                       
                                    <th width="110">Dye & Fin Cost</th>
                                    <th width="110">Finish Fabric Transfer in Cost</th>
                                    <th width="110">Finish Fabric Transfer Out Cost</th>
                            ...
                            <?
                                ...
                                ?>
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trTD_<? echo $j; ?>','<? echo $bgcolor; ?>')" id="trTD_<? echo $j; ?>">
                                        ...
                                        <td align="right"><? //echo fn_number_format($grey_transfer_in_cost_array[$key],2); $tot_grey_transfer_in_cost+=$grey_transfer_in_cost_array[$key]; ?></td>
                                        <td align="right"><? //echo fn_number_format($mkt_knit_array[$key],2); $tot_mkt_knit_cost+=$mkt_knit_array[$key]; ?></td>
                                        <td align="right"><? echo fn_number_format($grey_fab_cost_mkt_array[$key],2); $tot_grey_fab_mkt_cost+=$grey_fab_cost_mkt_array[$key]; ?></td>
                                        ...
                                        <td align="right"><? echo fn_number_format($mkt_aop_array[$key],2); $tot_mkt_aop_cost+=$mkt_aop_array[$key]; ?></td>
                                         <td align="right"><? echo fn_number_format($buyer_fin_fab_cost_mkt_array[$key],2); $tot_mkt_fin_fab_cost+=$buyer_fin_fab_cost_mkt_array[$key]; ?></td>
                                    ...
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trTD2_<? echo $j; ?>','<? echo $bgcolor; ?>')" id="trTD2_<? echo $j; ?>">
                                        ...
                                        <td align="right"><? echo fn_number_format($knit_cost_array[$key],2); $tot_buyer_knit_cost+=$knit_cost_array[$key]; ?></td>
                                         <td align="right"><?  echo fn_number_format($grey_transfer_in_cost_array[$key],2); $tot_grey_transfer_in_cost+=$grey_transfer_in_cost_array[$key];; ?></td>
                                         <td align="right"><? echo fn_number_format($grey_transfer_out_cost_array[$key],2); $tot_grey_transfer_out_cost+=$grey_transfer_out_cost_array[$key]; ?></td>
                                         <td align="right"><? //echo fn_number_format($knit_cost_array[$key],2); $tot_buyer_knit_cost+=$knit_cost_array[$key]; ?></td>
                                          
                                        <td align="right"><? echo fn_number_format($dye_cost_array[$key],2); $tot_buyer_dye_cost+=$dye_cost_array[$key]; ?></td>
                                         <td align="right"><? echo fn_number_format($fin_transfer_in_cost_array[$key],2); $tot_buyer_fin_transfer_in_cost+=$fin_transfer_in_cost_array[$key]; ?></td>
                                         <td align="right"><?  echo fn_number_format($fin_transfer_out_cost_array[$key],2); $tot_buyer_fin_transfer_out_cost+=$fin_transfer_out_cost_array[$key]; ?></td>
                                        ...
                                        <td align="right"><? echo fn_number_format($aop_n_others_cost_array[$key],2); $tot_buyer_aop_n_others_cost+=$aop_n_others_cost_array[$key]; ?></td>
                                         <td align="right"><? echo fn_number_format($buyer_fin_fab_cost_actual_array[$key],2); $tot_actual_fin_fab_cost+=$buyer_fin_fab_cost_actual_array[$key]; ?></td>
                                    ...
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trTD3_<? echo $j; ?>','<? echo $bgcolor; ?>')" id="trTD3_<? echo $j; ?>">
                                        ...
                                        <td align="right"><? $aop_var= $mkt_aop_array[$key]-$aop_n_others_cost_array[$key]; echo fn_number_format($aop_var,2); ?></td>
                                          <td align="right"><? $fin_fab_var= $buyer_fin_fab_cost_mkt_array[$key]-$buyer_fin_fab_cost_actual_array[$key]; echo fn_number_format($fin_fab_var,2); ?></td>
                                    ...
                                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('trTD4_<? echo $j; ?>','<? echo $bgcolor; ?>')" id="trTD4_<? echo $j; ?>">
                                        ...
                                        <td align="right"><? echo fn_number_format(($aop_var/$mkt_aop_array[$key]*100),2); ?></td>
                                         <td align="right"><? echo fn_number_format(($fin_fab_var/$buyer_fin_fab_cost_mkt_array[$key]*100),2); ?></td>
                            ...
                            <tr bgcolor="<? echo $bgcolor5; ?>" onClick="change_color('trTD5_<? echo $j; ?>','<? echo $bgcolor5; ?>')" id="trTD5_<? echo $j; ?>">
                                ...
                                <th align="right"><? echo fn_number_format($tot_mkt_knit_cost,2); ?></th>
                                 <th align="right"><? //echo fn_number_format($tot_grey_transfer_in_cost,2); ?></th>
                                  <th align="right"><? //echo fn_number_format($tot_mkt_knit_cost,2); ?></th>
                                  <th align="right"><? echo fn_number_format($tot_grey_fab_mkt_cost,2); ?></th>
                                ...
                                <th align="right"><? echo fn_number_format($tot_mkt_aop_cost,2); ?></th>
                                 <th align="right"><? echo fn_number_format($tot_mkt_fin_fab_cost,2); ?></th>
                            ...
                            <tr bgcolor="<? echo $bgcolor6; ?>" onClick="change_color('trTD6_<? echo $j; ?>','<? echo $bgcolor6; ?>')" id="trTD6_<? echo $j; ?>">
                                ...
                                <th align="right"><? echo fn_number_format($tot_buyer_knit_cost,2); ?></th>
                                 <th align="right"><? echo fn_number_format($tot_grey_transfer_in_cost,2); ?></th>
                                  <th align="right"><? echo fn_number_format($tot_grey_transfer_out_cost,2); ?></th>
                                   <th align="right"><? //echo fn_number_format($tot_buyer_knit_cost,2); ?></th>
                                <th align="right"><? echo fn_number_format($tot_buyer_dye_cost,2); ?></th>
                                  <th align="right"><? echo fn_number_format($tot_buyer_fin_transfer_in_cost,2); ?></th>
                                    <th align="right"><? echo fn_number_format($tot_buyer_fin_transfer_out_cost,2); ?></th>
                                ...
                                <th align="right"><? echo fn_number_format($tot_buyer_aop_n_others_cost,2); ?></th>
                                 <th align="right"><? echo fn_number_format($tot_actual_fin_fab_cost,2); ?></th>
                            ...
                            <tr bgcolor="<? echo $bgcolor7; ?>" onClick="change_color('trTD7_<? echo $j; ?>','<? echo $bgcolor7; ?>')" id="trTD7_<? echo $j; ?>">
                                ...
                                <th align="right"><? $fin_fab_var=$tot_mkt_fin_fab_cost-$tot_actual_fin_fab_cost;  echo fn_number_format($fin_fab_var,2); ?></th>
                            ...
                            <tr bgcolor="<? echo $bgcolor6; ?>" onClick="change_color('trTD8_<? echo $j; ?>','<? echo $bgcolor6; ?>')" id="trTD8_<? echo $j; ?>">
                                ...
                                <th align="right"><? echo fn_number_format(($aopvar/$tot_mkt_aop_cost*100),2); ?></th>
                                 <th align="right"><? echo fn_number_format(($fin_fab_var/$tot_mkt_fin_fab_cost*100),2); ?></th>
...
{ 
    ...
    <fieldset>
        ...
        <table style="margin-top:10px" id="table_header_1" class="rpt_table" width="4310" cellpadding="0" cellspacing="0" border="1" rules="all">
            <thead>
                <th width="40">SL</th>
                <th width="70">Buyer</th>
    	...
    	<div style="width:4322px; max-height:400px; overflow-y:scroll" id="scroll_body">
            <table class="rpt_table" width="4310" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
            <?
				$fabriccostArray=array(); $yarncostArray=array(); $trimsCostArray=array(); $prodcostArray=array(); $actualCostArray=array(); $actualTrimsCostArray=array(); 
				$subconCostArray=array(); $embellCostArray=array(); $washCostArray=array(); $aopCostArray=array(); $yarnTrimsCostArray=array(); 
			...
			unset($sql_rec_data);
				...
				$i=1; $tot_po_qnty=0; $tot_po_value=0; $tot_ex_factory_qnty=0; $tot_ex_factory_val=0; $tot_yarn_cost_mkt=0; $tot_knit_cost_mkt=0; $tot_dye_finish_cost_mkt=0; $tot_yarn_dye_cost_mkt=0; $tot_aop_cost_mkt=0; $tot_trims_cost_mkt=0; $tot_embell_cost_mkt=0; $tot_wash_cost_mkt=0; $tot_commission_cost_mkt=0; $tot_comm_cost_mkt=0; $tot_freight_cost_mkt=0; $tot_test_cost_mkt=0; $tot_inspection_cost_mkt=0; $tot_currier_cost_mkt=0; $tot_cm_cost_mkt=0; $tot_mkt_all_cost=0; $tot_mkt_margin=0; $tot_yarn_cost_actual=0; $tot_knit_cost_actual=0; $tot_dye_finish_cost_actual=0; $tot_yarn_dye_cost_actual=0; $tot_aop_cost_actual=0; $tot_trims_cost_actual=0; $tot_embell_cost_actual=0; $tot_wash_cost_actual=0; $tot_commission_cost_actual=0; $tot_comm_cost_actual=0; $tot_freight_cost_actual=0; $tot_test_cost_actual=0; $tot_inspection_cost_actual=0; $tot_currier_cost_actual=0; $tot_cm_cost_actual=0; $tot_actual_all_cost=0; $tot_actual_margin=0; $tot_fabric_purchase_cost_mkt=0; $tot_fabric_purchase_cost_actual=0;
				$tot_dye_finish_cost_actual=$total_fin_fab_transfer_amt_actual_in=$total_fin_fab_transfer_amt_actual_out=0;
$total_grey_fab_transfer_amt_actual_in=$total_grey_fab_transfer_amt_actual_out=0;
				...
				foreach($result as $row)
				{
					...
					$fin_trans_in=$fin_fab_trans_array[$row[csf('id')]][5];
				    $fin_trans_out=$fin_fab_trans_array[$row[csf('id')]][6];
					//echo  $fin_trans_in.'='.$fin_trans_out.'='. $knit_charge_mkt_fin.'<br>';
					$fin_fab_transfer_amt_actual_in=$fin_trans_in*$knit_charge_mkt_fin;
					$fin_fab_transfer_amt_actual_out=$fin_trans_out*$knit_charge_mkt_fin;
...
"  align="right"><a href="#report_details" onClick="openmypage_actual('<? echo $row[csf('id')]; ?>','knit_cost_actual','Actual Knitting Cost Details','900px')"><? //echo number_format($grey_fab_cost_actual,2,'.',''); ?></a><? //echo number_format($grey_fab_cost_actual,2,'.',''); ?></td>
       ...
       // die;
		?>
        <table class="rpt_table" width="4310" cellpadding="0" cellspacing="0" border="1" rules="all">
            <tr bgcolor="#CCDDEE" onClick="change_color('tr_pt','#CCDDEE')" id="tr_pt" style="font-weight:bold;">
                ...
                <td width="108" style="padding-right:2px" align="right"><? echo fn_number_format($tot_dye_finish_cost_mkt,2); ?></td>
                 <td width="108"  style="padding-right:2px" title="Fin Fabric In Cost" align="right"><? //echo number_format($tot_knit_cost_mkt,2,'.',''); ?></td>
                 <td width="108" style="padding-right:2px" title="Fin Fabric Cost"  align="right"><? //echo number_format($tot_grey_fab_cost_mkt,2,'.',''); ?></td>
...
?>
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'post_costing_report_controller_jk.php?po_id='+po_id+'&action='+type, tittle, 'width=560px, height=350px, center=1, resize=0, scrolling=0', '../../../');
	}
    </script>
	...
	<fieldset style="width:1070px; margin-left:7px">
        <table class="rpt_table" width="1065" cellpadding="0" cellspacing="0" border="1" rules="all">
            <thead>
                ...
                <th width="80"><p>Additonal Rate</p></th>
			...
			<table border="1" class="rpt_table" rules="all" width="1045" cellpadding="0" cellspacing="0">  
                ...
                foreach($result as $row)
                {
                    ...
                    <tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
                        ...
                        <td align="right" width="80"><p><? echo  $row[csf('add_process_name')]; ?></p>&nbsp;</td>
	...
	<fieldset style="width:760px; margin-left:7px">
        ...
        <div style="width:755px; max-height:330px; overflow-y:scroll" id="scroll_body">
			...
			unset($receive_rtn_qty_data);
				...
				$sql_rec_data=sql_select("select c.company_id,b.po_breakdown_id as po_id, a.item_group_id, c.receive_basis, a.booking_id, b.quantity as quantity, b.order_rate as rate, c.exchange_rate, (b.quantity*b.order_rate) as amount
			from inv_receive_master c, product_details_master d, inv_trims_entry_dtls a, order_wise_pro_details b
			where a.mst_id=c.id and a.trans_id=b.trans_id and a.prod_id=d.id and b.trans_type=1 and b.entry_form=24 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  $po_cond_for_in4 order by a.item_group_id ");
			//echo $receive_qty_data; die;
			foreach($sql_rec_data as $row)
        ...
        <div style="width:100%; max-height:400px;">
            <table cellpadding="0" width="100%" cellspacing="0" border="1" rules="all" class="rpt_table">
                ...
                $i=1;
				 ...
				 $exfac_sql=("select a.sys_number,b.challan_no,b.ex_factory_date, 
				...
				from   pro_ex_factory_mst b,pro_ex_factory_delivery_mst a  where a.id=b.delivery_mst_id and  b.status_active=1 and b.is_deleted=0 and b.po_break_down_id in($id) ");
...
if($action=="show_po_detail_report")
{
	...
	{
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'post_costing_report_controller_jk.php?mkt_data='+mkt_data+'&action='+type, tittle, 'width=660px, height=200px, center=1, resize=0, scrolling=0', '../../../');
	}
	...
	function openmypage(po_id,type,tittle)
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'post_costing_report_controller_jk.php?po_id='+po_id+'&action='+type, tittle, 'width='+popup_width+', height=400px, center=1, resize=0, scrolling=0', '../../../');
	}
