...
//	if($season=="") $season_cond=""; else $season_cond=" and a.season in('".implode("','",explode(",",$season))."')";
	...
	<div>
		...
		{
			$all_po_id="LISTAGG(b.id, ',') WITHIN GROUP (ORDER BY b.id)";
			$all_po_number="LISTAGG(b.po_number, ',') WITHIN GROUP (ORDER BY b.po_number)";
			$all_group="LISTAGG(b.grouping, ',') WITHIN GROUP (ORDER BY b.grouping)";
			$all_file="LISTAGG(b.file_no, ',') WITHIN GROUP (ORDER BY b.file_no)";
			$sql="select a.job_no_prefix_num, a.job_no, a.buyer_name, a.style_ref_no, a.avg_unit_price, a.total_set_qnty as ratio, $all_po_id as po_id, $all_po_number as po_number, SUM(b.po_quantity) as po_quantity, SUM(b.plan_cut) as plan_cut,AVG(b.unit_price) as unit_price, $all_group as grouping, $all_file as file_no, SUM(b.po_total_price) as po_total_price from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.company_name='$company_name' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $date_cond $buyer_id_cond  $year_cond $job_no_cond $order_id_cond_trans $order_no_cond $order_status_cond $season_cond $internal_ref_cond group by a.job_no_prefix_num, a.job_no, a.buyer_name, a.style_ref_no, a.avg_unit_price, a.total_set_qnty ";// and b.shiping_status=3
			$sql_unit="select b.unit_price  from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.company_name='$company_name' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $date_cond $buyer_id_cond  $year_cond $job_no_cond $order_id_cond_trans $order_no_cond $order_status_cond $season_cond $internal_ref_cond ";
			$unit_result=sql_select($sql_unit);
		...
		$fab_finish_cost_arr=array(33,34,38,63,65,66,67,68,69,70,71,72,73,75,76,77,88,82,89,90,91,92,93,94,128,129,135,136,141,143,150,151,155,156,157,145,132,144);
		...
		foreach($result as $row)
		{
			...
			if($report_type==5)
			{
				...
				$tot_conversion_cost=array_sum($conversion_costing_arr[$row[csf('po_id')]]);
				...
				$tot_knit_cost=array_sum($conversion_costing_arr_process[$row[csf('po_id')]][1])+array_sum($conversion_costing_arr_process[$row[csf('po_id')]][2])+array_sum($conversion_costing_arr_process[$row[csf('po_id')]][3]);
				...
				{
					$all_over_cost+=array_sum($conversion_costing_arr_process[$row[csf('po_id')]][$aop_id]);
				}
				...
				{
					$fabric_finish+=array_sum($conversion_costing_arr_process[$row[csf('po_id')]][$fab_fin_id]);
				}

				$washing_cost=0;
				foreach($washing_cost_arr as $washing_id)
				{
					$washing_cost+=array_sum($conversion_costing_arr_process[$row[csf('po_id')]][$washing_id]);
				}

				$other_cost_conv=0;
				foreach($other_cost_arr as $other_id)
				{
					$other_cost_conv+=array_sum($conversion_costing_arr_process[$row[csf('po_id')]][$other_id]);
				}
			...
			else if($report_type==12)
			{
				...
				$ex_all_po_id=array_unique(explode(',',$row[csf('po_id')]));
				foreach($ex_all_po_id as $po_id)
				{
					$actual_shipout_qty+=$actual_shipout_arr[$po_id]['shipout_qty'];
					$actual_shipout_val+=$actual_shipout_qty*$unit_price;

					$foreign+=$commission_costing_arr[$po_id][1];
					$local+=$commission_costing_arr[$po_id][2];
					$commission_cost=$foreign+$local;
					$net_fob_value=$gross_fob_value-$commission_cost;
					...
					$tot_conversion_cost+=array_sum($conversion_costing_arr[$po_id]);
					if(is_infinite($tot_conversion_cost) || is_nan($tot_conversion_cost)){$tot_conversion_cost=0;}
					$tot_trim_cost+= $trims_costing_arr[$po_id];
					if(is_infinite($tot_trim_cost) || is_nan($tot_trim_cost)){$tot_trim_cost=0;}
					$tot_embell_cost+=$emblishment_costing_arr[$po_id];
					if(is_infinite($tot_embell_cost) || is_nan($tot_embell_cost)){$tot_embell_cost=0;}
					$freight_cost+=$other_costing_arr[$po_id]['freight'];
					if(is_infinite($freight_cost) || is_nan($freight_cost)){$freight_cost=0;}
					$inspection+=$other_costing_arr[$po_id]['inspection'];
					if(is_infinite($inspection) || is_nan($inspection)){$inspection=0;}
					...
					$tot_knit_cost+=array_sum($conversion_costing_arr_process[$po_id][1])+array_sum($conversion_costing_arr_process[$po_id][2])+array_sum($conversion_costing_arr_process[$po_id][3]);

					/* $fabric_dyeing_cost=0;
					foreach($fabric_dyeingCost_arr as $dye_id)
					{
						$fabric_dyeing_cost+=array_sum($conversion_costing_arr_process[$po_id][$dye_id]);
						$fabric_dyeingCost_arr=array(25,31,26,32,60,61,62,63,72,80,81,84,85,86,87,38,39,74,78,79,101,133,137,138,139,146,147,149); 
					} */
					$fabric_dyeing_cost+=array_sum($conversion_costing_arr_process[$po_id][25])+array_sum($conversion_costing_arr_process[$po_id][26])+array_sum($conversion_costing_arr_process[$po_id][31])+array_sum($conversion_costing_arr_process[$po_id][32])+array_sum($conversion_costing_arr_process[$po_id][60])+array_sum($conversion_costing_arr_process[$po_id][61])+array_sum($conversion_costing_arr_process[$po_id][62])+array_sum($conversion_costing_arr_process[$po_id][63])+array_sum($conversion_costing_arr_process[$po_id][72])+array_sum($conversion_costing_arr_process[$po_id][80])+array_sum($conversion_costing_arr_process[$po_id][81])+array_sum($conversion_costing_arr_process[$po_id][84])+array_sum($conversion_costing_arr_process[$po_id][85])+array_sum($conversion_costing_arr_process[$po_id][86])+array_sum($conversion_costing_arr_process[$po_id][87])+array_sum($conversion_costing_arr_process[$po_id][38])+array_sum($conversion_costing_arr_process[$po_id][39])+array_sum($conversion_costing_arr_process[$po_id][74])+array_sum($conversion_costing_arr_process[$po_id][78])+array_sum($conversion_costing_arr_process[$po_id][79])+array_sum($conversion_costing_arr_process[$po_id][101])+array_sum($conversion_costing_arr_process[$po_id][133])+array_sum($conversion_costing_arr_process[$po_id][137])+array_sum($conversion_costing_arr_process[$po_id][138])+array_sum($conversion_costing_arr_process[$po_id][139])+array_sum($conversion_costing_arr_process[$po_id][146])+array_sum($conversion_costing_arr_process[$po_id][147])+array_sum($conversion_costing_arr_process[$po_id][149]);

					$yarn_dyed_cost+=array_sum($conversion_costing_arr_process[$po_id][30]);

					/* $all_over_cost=0;
					foreach ($aop_cost_arr as $aop_id)
					{
						$all_over_cost+=array_sum($conversion_costing_arr_process[$po_id][$aop_id]);
					} */
					$all_over_cost+=array_sum($conversion_costing_arr_process[$po_id][35])+array_sum($conversion_costing_arr_process[$po_id][36])+array_sum($conversion_costing_arr_process[$po_id][37])+array_sum($conversion_costing_arr_process[$po_id][40]);

					$heat_setting_cost+=array_sum($conversion_costing_arr_process[$po_id][33]);

					/* $fabric_finish=0;
					foreach($fab_finish_cost_arr as $fab_fin_id)
					{
						$fabric_finish+=array_sum($conversion_costing_arr_process[$po_id][$fab_fin_id]);
					...
					$fabric_finish+=array_sum($conversion_costing_arr_process[$po_id][33])+array_sum($conversion_costing_arr_process[$po_id][34])+array_sum($conversion_costing_arr_process[$po_id][38])+array_sum($conversion_costing_arr_process[$po_id][63])+array_sum($conversion_costing_arr_process[$po_id][65])+array_sum($conversion_costing_arr_process[$po_id][66])+array_sum($conversion_costing_arr_process[$po_id][67])+array_sum($conversion_costing_arr_process[$po_id][68])+array_sum($conversion_costing_arr_process[$po_id][69])+array_sum($conversion_costing_arr_process[$po_id][70])+array_sum($conversion_costing_arr_process[$po_id][71])+array_sum($conversion_costing_arr_process[$po_id][72])+array_sum($conversion_costing_arr_process[$po_id][73])+array_sum($conversion_costing_arr_process[$po_id][75])+array_sum($conversion_costing_arr_process[$po_id][76])+array_sum($conversion_costing_arr_process[$po_id][77])+array_sum($conversion_costing_arr_process[$po_id][88])+array_sum($conversion_costing_arr_process[$po_id][82])+array_sum($conversion_costing_arr_process[$po_id][89])+array_sum($conversion_costing_arr_process[$po_id][90])+array_sum($conversion_costing_arr_process[$po_id][91])+array_sum($conversion_costing_arr_process[$po_id][92])+array_sum($conversion_costing_arr_process[$po_id][93])+array_sum($conversion_costing_arr_process[$po_id][94])+array_sum($conversion_costing_arr_process[$po_id][128])+array_sum($conversion_costing_arr_process[$po_id][129])+array_sum($conversion_costing_arr_process[$po_id][135])+array_sum($conversion_costing_arr_process[$po_id][136])+array_sum($conversion_costing_arr_process[$po_id][141])+array_sum($conversion_costing_arr_process[$po_id][143])+array_sum($conversion_costing_arr_process[$po_id][150])+array_sum($conversion_costing_arr_process[$po_id][151])+array_sum($conversion_costing_arr_process[$po_id][155])+array_sum($conversion_costing_arr_process[$po_id][156])+array_sum($conversion_costing_arr_process[$po_id][157])+array_sum($conversion_costing_arr_process[$po_id][145])+array_sum($conversion_costing_arr_process[$po_id][132])+array_sum($conversion_costing_arr_process[$po_id][144]);

					/* $washing_cost=0;
					foreach($washing_cost_arr as $washing_id)
					{
						$washing_cost+=array_sum($conversion_costing_arr_process[$po_id][$washing_id]);
					} */
					$washing_cost+=array_sum($conversion_costing_arr_process[$po_id][140])+array_sum($conversion_costing_arr_process[$po_id][142])+array_sum($conversion_costing_arr_process[$po_id][148])+array_sum($conversion_costing_arr_process[$po_id][64]);
					$other_cost_conv=0;
					foreach($other_cost_arr as $other_id)
					{
						$other_cost_conv+=array_sum($conversion_costing_arr_process[$po_id][$other_id]);
					}
					$print_cost+=$emblishment_costing_arr_name[$po_id][1];
					if(is_infinite($print_cost) || is_nan($print_cost)){$print_cost=0;}
					$embroidery_cost+=$emblishment_costing_arr_name[$po_id][2];
					if(is_infinite($embroidery_cost) || is_nan($embroidery_cost)){$embroidery_cost=0;}
					$special_cost+=$emblishment_costing_arr_name[$po_id][4];
					if(is_infinite($special_cost) || is_nan($special_cost)){$special_cost=0;}
					$contribution_value=$net_fob_value-$cost_of_material_service;
					...
					$commercial_cost=$fabriccostArray[$row[csf('job_no')]]['comm_cost'];
					if(is_infinite($commercial_cost) || is_nan($commercial_cost)){$commercial_cost=0;}
					$operating_expense+=$other_costing_arr[$po_id]['common_oh'];
					if(is_infinite($operating_expense) || is_nan($operating_expense)){$operating_expense=0;}
					$tot_commercial_cost+=$commercial_costing_arr[$po_id];
					if(is_infinite($tot_commercial_cost) || is_nan($tot_commercial_cost)){$tot_commercial_cost=0;}

					$operating_profit=$gross_profit-($tot_commercial_cost+$operating_expense);
					$depreciation_amortization+=$other_costing_arr[$po_id]['depr_amor_pre_cost'];
					$interest_expense=$net_fob_value*$financial_para[csf('interest_expense')]/100;
					if(is_infinite($interest_expense) || is_nan($interest_expense)){$interest_expense=0;}
					$income_tax=$net_fob_value*$financial_para[csf('income_tax')]/100;
					if(is_infinite($income_tax) || is_nan($income_tax)){$income_tax=0;}
			...
			else if($report_type==6)
			{
				...
				foreach($ex_all_country_id as $country_id)
				{
					...
					$fab_purchase_knit=array_sum($fabric_costing_arr['knit']['grey'][$row[csf('po_id')]][$country_id]);
					$fab_purchase_woven=array_sum($fabric_costing_arr['woven']['grey'][$row[csf('po_id')]][$country_id]);
					$fab_purchase=$fab_purchase_knit+$fab_purchase_woven;

					$tot_conversion_cost+=array_sum($conversion_costing_arr[$row[csf('po_id')]][$country_id]);
					...
					$tot_knit_cost+=array_sum($conversion_costing_arr_process[$row[csf('po_id')]][$country_id][1])+array_sum($conversion_costing_arr_process[$row[csf('po_id')]][$country_id][2])+array_sum($conversion_costing_arr_process[$row[csf('po_id')]][$country_id][3]);

					foreach($fabric_dyeingCost_arr as $dye_id)
					{
						$fabric_dyeing_cost+=array_sum($conversion_costing_arr_process[$row[csf('po_id')]][$country_id][$dye_id]);
					}
					$yarn_dyed_cost+=array_sum($conversion_costing_arr_process[$row[csf('po_id')]][$country_id][30]);

					foreach ($aop_cost_arr as $aop_id)
					{
						$all_over_cost+=array_sum($conversion_costing_arr_process[$row[csf('po_id')]][$country_id][$aop_id]);
					}
					$heat_setting_cost+=array_sum($conversion_costing_arr_process[$row[csf('po_id')]][$country_id][33]);

					foreach($fab_finish_cost_arr as $fab_fin_id)
					{
						$fabric_finish+=array_sum($conversion_costing_arr_process[$row[csf('po_id')]][$country_id][$fab_fin_id]);
					}

					foreach($washing_cost_arr as $washing_id)
					{
						$washing_cost+=array_sum($conversion_costing_arr_process[$row[csf('po_id')]][$country_id][$washing_id]);
					}

					foreach($other_cost_arr as $other_id)
					{
						$other_cost_conv+=array_sum($conversion_costing_arr_process[$row[csf('po_id')]][$country_id][$other_id]);
					}
 ...
 '<? echo $row[csf('po_id')]; ?>','750px')"><? echo  number_format($actual_shipout_qty,2); ?></a>
			...
			<tr>
				...
				<td width="100" align="right" title="(Yarn costing(<?=$yarn_costing?>)/order qty pcs(<?=$order_qty_pcs?>))*Actual shipout qty(<?=$actual_shipout_qty?>)"> <? echo number_format($actual_yarn_costing,2); ?></p></td>
                <td width="80" align="right"  title="(Fab purchase(<?=$actual_fab_purchase?>)/order qty pcs(<?=$order_qty_pcs?>))*Actual shipout qty(<?=$actual_shipout_qty?>)"><p><? echo number_format($actual_fab_purchase,2); ?></p></td>
				<td width="100" align="right" title="(Conversion cost(<?=$actual_conversion_cost?>)/order qty pcs(<?=$order_qty_pcs?>))*Actual shipout qty(<?=$actual_shipout_qty?>)"><p><? echo number_format($actual_conversion_cost,2); ?></p></td>
