...
{
	$print_report_format=return_field_value("format_id","lib_report_template","template_name ='".$data."' and module_id=11 and report_id=279 and is_deleted=0 and status_active=1");
...
{ 
	...
	if($company_name!=0) $company_con3=" and b.company_id='$company_name'"; else $company_con3="";
	$date_cond='';
	if(str_replace("'","",$cbo_year_from)!=0 && str_replace("'","",$cbo_month_from)!=0)
	{
		...
		if($cbo_date_category==1)
		{
			if($db_type==0) 
			{
				...
				$date_cond3=" and c.country_ship_date between '$start_date' and '$end_date'";
			...
			if($db_type==2) 
			{
				...
				$date_cond3=" and c.country_ship_date between '".date("j-M-Y",strtotime(str_replace("'","",$start_date)))."' and '".date("j-M-Y",strtotime(str_replace("'","",$end_date)))."'";
		...
		else if($cbo_date_category==2) //Cut-Off Date
		{
			if($db_type==0) 
			{
				...
				$date_cond3=" and c.cutup_date between '$start_date' and '$end_date'";
			...
			if($db_type==2) 
			{
				...
				$date_cond3=" and c.cutup_date between '".date("j-M-Y",strtotime(str_replace("'","",$start_date)))."' and '".date("j-M-Y",strtotime(str_replace("'","",$end_date)))."'";
	...
	unset($smv_sql);
	 ...
	 $data_array="SELECT a.id as job_id, a.job_no, a.style_ref_no,a.dealing_marchant, a.company_name,a.product_category,  a.buyer_name, a.ship_mode,a.total_set_qnty as ratio, a.gmts_item_id,	 a.set_smv, a.season_buyer_wise as season, a.insert_date,
	a.update_date, a.order_uom, b.id as color_break_id, b.po_break_down_id,b.order_quantity as po_quantity_pcs,c.po_quantity as po_qty, b.country_ship_date, b.cutup_date, b.order_total,b.item_number_id, c.details_remarks, c.unit_price,c.is_confirmed,c.po_number,c.po_quantity,c.po_received_date from wo_po_details_master a,wo_po_color_size_breakdown b,wo_po_break_down c	where a.job_no=b.job_no_mst	and po_break_down_id=c.id and b.status_active=1  and a.status_active=1 and c.status_active=1 and b.is_deleted=0 and c.is_deleted=0 	$company_con $date_cond $buyer_cond	$order_status_cond	$season_cond $shipment_status_cond $marchd_cond $product_category_con	order by  $order_by_cond,c.is_confirmed,a.job_no ASC";
	// echo "SELECT a.id as job_id, a.job_no, a.style_ref_no,a.dealing_marchant, a.company_name,a.product_category,  a.buyer_name, a.ship_mode,a.total_set_qnty as ratio, a.gmts_item_id,	 a.set_smv, a.season_buyer_wise as season, a.insert_date,
	// a.update_date, a.order_uom, b.id as color_break_id, b.po_break_down_id,b.order_quantity as po_quantity_pcs,c.po_quantity as po_qty, b.country_ship_date, b.cutup_date, b.order_total,b.item_number_id, c.details_remarks, c.unit_price,c.is_confirmed,c.po_number,c.po_quantity,c.po_received_date from wo_po_details_master a,wo_po_color_size_breakdown b,wo_po_break_down c	where a.job_no=b.job_no_mst	and po_break_down_id=c.id and b.status_active=1  and a.status_active=1 and c.status_active=1 and b.is_deleted=0 and c.is_deleted=0 	$company_con $date_cond $buyer_cond	$order_status_cond	$season_cond $shipment_status_cond $marchd_cond $product_category_con	order by  $order_by_cond,c.is_confirmed,a.job_no ASC";die;
	...
	$day_total_con=array(); $item_wise_qty_arr=array();
		foreach($result_po as $row)
		{
			...
			$tmp_arr[$coutry_ship_date][$buyerId][$is_confirmed][$poId]["product_category"] =$row[csf("product_category")];
	...
	// if(!$all_po_ids_cond)$all_po_ids_cond="";
		...
		foreach($all_po_array as $ord_id) {
			if($ord_id!=0) {
				$r_id2=execute_query("insert into tmp_poid (userid, poid, type) values ($user_id,$ord_id,3)");
	...
	// $total_pcs_sql="SELECT   c.id,b.order_quantity from wo_po_details_master a, wo_po_color_size_breakdown b,wo_po_break_down c where 	a.job_no=b.job_no_mst	and b.po_break_down_id=c.id and b.status_active=1 and c.status_active=1 and b.is_deleted=0 and c.is_deleted=0	$marchd_cond $all_po_ids_cond $date_cond $company_con ";

	$total_pcs_sql="SELECT   c.id,b.order_quantity from wo_po_details_master a, wo_po_color_size_breakdown b,wo_po_break_down c, tmp_poid tmp where 	a.job_no=b.job_no_mst	and b.po_break_down_id=c.id and b.status_active=1 and c.status_active=1 and b.is_deleted=0 and c.is_deleted=0 and tmp.userid=$user_id	and  c.id=tmp.poid and tmp.type=3 $marchd_cond ";

	 

	$total_pcs_data=sql_select($total_pcs_sql);

	$total_pcs_arr=array();
	foreach($total_pcs_data as $v)
	{
		$total_pcs_arr[$v[csf("id")]]+=$v[csf("order_quantity")];
	}
	unset($total_pcs_data);

	 $sample_sql="SELECT   c.id as po_id,d.sample_type,a.approval_status from wo_po_sample_approval_info a, wo_po_break_down c,lib_sample d, tmp_poid tmp where a.po_break_down_id=c.id and d.id=a.sample_type_id and a.status_active=1 and c.status_active=1 and c.is_deleted=0 and tmp.userid=$user_id and  c.id=tmp.poid and tmp.type=3 and d.status_active=1 and d.sample_type=13  ";
	...
	?>
		...
		foreach($tmp_arr as $date_key=>$buyer_id_arr)
		{
				...
				foreach($buyer_id_arr as $buyer_id=>$is_confirm_arr)
				{
					...
					foreach($is_confirm_arr as $is_confirm_id=>$order_id_arr)
					{
						if($is_confirm_id==1)
						{
							...
							<td>&nbsp;New Job</td>
                            <td bgcolor="#e67e22" height="15" width="30">&nbsp;</td>
                            <td>&nbsp;Updated Job</td>
                            </tr>
                            </table>
							<table  width="<? if($all_date_button==""){ echo 2840;}else { echo 3440;} ?>" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all" >
                                <thead> 
                                ...
                                foreach($order_id_arr as $order_id=>$row)
                                {
									...
									if($update_date!='' || $update_date!='01-01-1970') $date_diff_update=datediff( "d", $update_date , $today_date);
									 //echo 'sdsdsd';
									 if($date_diff_update<=3) $bg_color="#e67e22";	
									 else if($date_diff_insert<=3) $bg_color="#00CED1";	
									...
									//{
										...
										<tr style="font-family:'Arial Narrow'" bgcolor="<? echo $bg_color; ?>" onClick="change_color('tr_<? echo $x; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $x; ?>">
                                            ...
                                            <td><p><? //echo $fab_type_arr[$row["job_no"]]['composition'];
												...
												echo $color_id;
                                            ?></p></td>
											<td><p><? echo $product_category[$row[csf('product_category')]];?></p></td>
                                            <td><p><? 
									...
									?>
                                        <tr style="font-family:'Arial Narrow'" bgcolor="<? echo $bg_color; ?>" onClick="change_color('tr_<? echo $x; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $x; ?>">
                                            ...
                                            <td width="80"><p><? //echo $fab_type_arr[$row["job_no"]]['composition'];
												...
												echo $color_id;
												?></p></td>
												<td><p><? echo $product_category[$row["product_category"]]; ?></p></td>
                                            	<td width="80"><p><? 
												$gmts_item=''; $gmts_item_id=explode(",",$row["gmts_item_id"]);
												$smv_min=0;
												foreach($gmts_item_id as $item_id)
                                ...
                                <tr bgcolor="#CCCCCC" style="font-family:'Arial Narrow'">
                                    <td colspan="17"  align="right" style="font-weight:bold;">Confirm Total:</td>
            ...
            <table  width="<? if($all_date_button==""){ echo 2770;}else { echo 3370;} ?>" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all" >
	...
	$r_id3=execute_query("delete from tmp_poid where userid=$user_id and type in (3)");
    if($db_type==0) {
        if($r_id3) {
            mysql_query("COMMIT");  
        }
...
//report_generate_order 2
if($action=="report_generate_order2")
{ 
	$process = array( &$_POST );
	extract(check_magic_quote_gpc( $process )); 
	//echo __LINE__.'---'.$cbo_year_from;die;
	$company_name=str_replace("'","",$cbo_company_name);
	$cbo_buyer_name=str_replace("'","",$cbo_buyer_name);
	$cbo_date_category=str_replace("'","",$cbo_date_category);
	$season=str_replace("'","",$txt_season);
	...
	$data_array="SELECT a.id as job_id, a.job_no, a.style_ref_no,a.dealing_marchant, a.company_name,a.product_category,  a.buyer_name, a.ship_mode,a.total_set_qnty as ratio, a.gmts_item_id, a.set_smv, a.season_buyer_wise as season, a.insert_date, a.update_date,b.update_date as up_date, a.order_uom, b.id as color_break_id, b.po_break_down_id,b.order_quantity as po_quantity_pcs,c.po_quantity as po_qty, b.country_ship_date, b.cutup_date, b.order_total,b.item_number_id, c.details_remarks, c.unit_price,c.is_confirmed, c.id as po_id, c.po_number,c.po_quantity,c.po_received_date, c.grouping from wo_po_details_master a,wo_po_color_size_breakdown b,wo_po_break_down c	where a.job_no=b.job_no_mst	and po_break_down_id=c.id and b.status_active=1  and a.status_active=1 and c.status_active=1 and b.is_deleted=0 and c.is_deleted=0 	$company_con $date_cond $buyer_cond	$order_status_cond	$season_cond $shipment_status_cond $marchd_cond $product_category_con";
	//echo $data_array;die; 
	$result_po=sql_select($data_array);
	$job_id_array=array();
	$po_id_array=array();
	...
	$con = connect();
	execute_query("DELETE FROM GBL_TEMP_ENGINE WHERE USER_ID = ".$user_id." and ref_from in (1,2) and ENTRY_FORM=80");
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 85, 1, $job_id_array, $empty_arr);
	fnc_tempengine("GBL_TEMP_ENGINE", $user_id, 85, 2, $po_id_array, $empty_arr);
	
	$sql_color_no_arr=return_library_array("select a.po_break_down_id, count(distinct a.color_number_id) as color_id from wo_po_color_size_breakdown a join gbl_temp_engine b on a.po_break_down_id=b.ref_val where a.status_active=1 and a.is_deleted=0 and b.ref_from=2 and b.entry_form=85 and b.user_id=$user_id  group by po_break_down_id","po_break_down_id","color_id");
	
	if($db_type==2) $fab_full_name="a.fabric_description || ',' || a.gsm_weight";
	else if ($db_type==0) $fab_full_name="a.fabric_description,',',a.gsm_weight";
	$body_part_arr=return_library_array("select c.id, $fab_full_name as fabrication from wo_pre_cost_fabric_cost_dtls a ,wo_po_details_master b,  wo_po_break_down c, gbl_temp_engine d where a.job_no=b.job_no and b.id=c.job_id and a.body_part_id in(1,651,20) and b.id=d.ref_val and d.ref_from=1 and d.entry_form=85 and d.user_id=$user_id","id","fabrication");
	//var_dump($body_part_arr);die;
	$weak_of_year=return_library_array( "select week_date,week from  week_of_year",'week_date','week');
	
	$costing_per_arr=return_library_array( "select a.job_no, a.costing_per from wo_pre_cost_mst a join gbl_temp_engine b on a.job_id=b.ref_val and b.ref_from=1 and b.entry_form=85 and b.user_id=$user_id",'job_no','costing_per');
	$supplier_name_arr=return_library_array( "select id, supplier_name from lib_supplier",'id','supplier_name');
	
	if($db_type==2) $color_type_group="LISTAGG(CAST(b.color_type_id AS VARCHAR2(4000)),',') WITHIN GROUP ( ORDER BY color_type_id) as color_type_id";
	else if($db_type==0) $color_type_group="group_concat(b.color_type_id) as color_type_id";
	
	$sql_pre="SELECT  b.job_no, $color_type_group from wo_pre_cost_fabric_cost_dtls  b,wo_po_details_master a, gbl_temp_engine c where a.job_no=b.job_no and b.status_active=1 and b.is_deleted=0 and a.id=c.ref_val and  c.ref_from=1 and c.entry_form=85 and c.user_id=$user_id group by b.job_no"; 
	$pre_result=sql_select($sql_pre);
	$fab_color_type_arr=array();
	foreach($pre_result as $row)
	{
	...
	unset($pre_result);
	$sql_sum="SELECT  b.job_no, b.fab_knit_req_kg, b.fab_knit_fin_req_kg from wo_pre_cost_sum_dtls  b,wo_po_details_master a, gbl_temp_engine c where a.job_no=b.job_no and b.status_active=1 and b.is_deleted=0 and a.id=c.ref_val and  c.ref_from=1 and c.entry_form=85 and c.user_id=$user_id $marchd_cond";
	$pre_result_sum=sql_select($sql_sum);
	$fab_grey_cons_arr=array();
	foreach($pre_result_sum as $row)
	{
	...
	$sql_ebl="select  b.job_no,$embl_group from wo_pre_cost_embe_cost_dtls  b,wo_po_details_master a, gbl_temp_engine c where a.job_no=b.job_no and b.status_active=1 and b.is_deleted=0 and cons_dzn_gmts>0 and a.id=c.ref_val and  c.ref_from=1 and c.entry_form=85 and c.user_id=$user_id group by b.job_no";
	//echo $sql_ebl;
	$ebl_result=sql_select($sql_ebl);
	$embl_fab_type_arr=array();
	foreach($ebl_result as $row)
	...
	$embl_order_arr=array();
	$sql_embl="select a.job_no, a.gmts_item_id, a.embelishment, a.embro, a.wash, a.spworks, a.gmtsdying from wo_po_details_mas_set_details a join gbl_temp_engine b on a.job_id=b.ref_val where b.ref_from=1 and b.entry_form=85 and b.user_id=$user_id";
	$embl_result=sql_select($sql_embl);
	$embl_ord_type_arr=array();
	foreach($embl_result as $row)
	{
	...
	$sql_book="SELECT  a.entry_form,a.is_short, a.item_category,a.fabric_source,a.is_approved,b.po_break_down_id as po_id,b.job_no,a.booking_no,a.booking_no_prefix_num as booking_no_pre from wo_booking_dtls  b, wo_booking_mst a, wo_po_details_master c, gbl_temp_engine d where a.id=b.booking_mst_id and a.company_id=$company_name and b.booking_type=1 and b.status_active=1 and b.is_deleted=0 and c.job_no=b.job_no and c.id=d.ref_val and d.ref_from=1 and d.entry_form=85 and d.user_id=$user_id  group by  a.entry_form,a.is_short, a.item_category,a.fabric_source,a.is_approved,b.po_break_down_id  ,b.job_no,a.booking_no,a.booking_no_prefix_num order by a.is_short asc ";
	//echo $sql_book; die;
	$pre_book=sql_select($sql_book);
	$fab_booking_arr=array();
	$fab_booking_type_arr=array();
	...
	$sql_sewing="select  b.location,b.production_source as prod_source,b.serving_company,d.production_qnty as production_qnty,b.company_id,c.country_ship_date as country_ship_date,c.cutup_date,c.po_break_down_id as po_id,e.buyer_name from pro_garments_production_mst  b,pro_garments_production_dtls d,wo_po_color_size_breakdown c,wo_po_details_master e, gbl_temp_engine f where b.id=d.mst_id and c.id=d.color_size_break_down_id and e.job_no=c.job_no_mst and b.production_type=5 and d.production_type=5 and b.status_active=1 and b.is_deleted=0  and c.status_active=1 and c.is_deleted=0 and e.id=f.ref_val and f.ref_from=1 and f.entry_form=85 and f.user_id=$user_id $date_cond3 $company_con3";
	//echo $sql_sewing;die;
	$sew_result=sql_select($sql_sewing);
	$sewing_outCompany_arr=array();
	foreach($sew_result as $row)
	...
	{	
		 ...
		 $sql_exf_c_date="SELECT d.id as color_break_id,a.buyer_name,d.po_break_down_id as po_id,d.country_ship_date as country_ship_date,d.cutup_date, c.is_confirmed from wo_po_break_down c,wo_po_details_master a,wo_po_color_size_breakdown d, gbl_temp_engine e  where  c.job_no_mst=a.job_no and  d.status_active=1 and d.is_deleted=0 and c.id=d.po_break_down_id  and  a.job_no=d.job_no_mst  and c.id=e.ref_val and e.ref_from=2 and e.entry_form=85 and e.user_id=$user_id";
		$result_c=sql_select($sql_exf_c_date);
		foreach($result_c as $row)
		{
			if($cbo_date_category==1)
		...
		$sql_exf="SELECT a.buyer_name,b.po_break_down_id as po_id,d.color_size_break_down_id as color_break_id, c.is_confirmed, b.shiping_status, (CASE WHEN b.entry_form!=85 THEN d.production_qnty 	 ELSE 0 END) as ex_fact_qty, (CASE WHEN b.entry_form=85 THEN d.production_qnty  ELSE 0 END) as ex_fact_ret_qty from wo_po_break_down c,pro_ex_factory_mst b,wo_po_details_master a,pro_ex_factory_dtls d, gbl_temp_engine e  where c.id=b.po_break_down_id and c.job_no_mst=a.job_no and b.id=d.mst_id and b.po_break_down_id=b.po_break_down_id and  b.status_active=1 and b.is_deleted=0 and  d.status_active=1 and d.is_deleted=0 and c.id=e.ref_val and e.ref_from=2 and e.entry_form=85 and e.user_id=$user_id";
		//echo $sql_exf; die;
		$result=sql_select($sql_exf);
		$ex_fact_con_arr=array();$ex_fact_proj_arr=array(); $ship_status_arr=array();
		foreach($result as $row)
	...
	$smv_arr=array();
	$smv_sql=sql_select("SELECT a.job_no, a.set_item_ratio, a.smv_pcs, a.gmts_item_id from wo_po_details_mas_set_details a join gbl_temp_engine b on a.id=b.ref_val and e.ref_from=1 and e.entry_form=85 and e.user_id=$user_id ");
	foreach($smv_sql as $row)
	{
		$smv_arr[$row[csf("job_no")]][$row[csf("gmts_item_id")]]=$row[csf("smv_pcs")];
	}
	...
	$day_total_con=array(); $item_wise_qty_arr=array();
		foreach($result_po as $row)
		{
			...
			$tmp_arr[$coutry_ship_date][$buyerId][$is_confirmed][$poId]["update_date"] =$row[csf("update_date")];
			$tmp_arr[$coutry_ship_date][$buyerId][$is_confirmed][$poId]["up_date"] =$row[csf("up_date")];
			$tmp_arr[$coutry_ship_date][$buyerId][$is_confirmed][$poId]["insert_date"] =$row[csf("insert_date")];
			
			$tmp_arr[$coutry_ship_date][$buyerId][$is_confirmed][$poId]["po_received_date"] =$row[csf("po_received_date")];
			$tmp_arr[$coutry_ship_date][$buyerId][$is_confirmed][$poId]["grouping"] =$row[csf("grouping")];
			$item_wise_qty_arr[$poId][$row[csf("item_number_id")]]+=$row[csf("po_quantity_pcs")];
			if($is_confirmed==1)
			{
				$po_qty_arr[$poId]["po_qty"]=$row[csf("po_qty")];
	...
	$total_pcs_sql="SELECT   c.id,b.order_quantity from wo_po_details_master a, wo_po_color_size_breakdown b,wo_po_break_down c, gbl_temp_engine d where 	a.job_no=b.job_no_mst	and b.po_break_down_id=c.id and b.status_active=1 and c.status_active=1 and b.is_deleted=0 and c.is_deleted=0 and  c.id=d.ref_val and d.ref_from=2 and d.entry_form=85 and d.user_id=$user_id";

	// echo $total_pcs_sql; die;
	...
	unset($total_pcs_data);
	 ...
	 $sample_sql="SELECT   c.id as po_id,d.sample_type,a.approval_status from wo_po_sample_approval_info a, wo_po_break_down c,lib_sample d, gbl_temp_engine e where a.po_break_down_id=c.id and d.id=a.sample_type_id and a.status_active=1 and c.status_active=1 and c.is_deleted=0 and and c.id=e.ref_val and e.ref_from=2 and e.entry_form=85 and e.user_id=$user_id and d.status_active=1 and d.sample_type=13  ";
		...
		foreach($tmp_arr as $date_key=>$buyer_id_arr)
		{
			...
			$num_days = cal_days_in_month(CAL_GREGORIAN, $month_value[1], $month_value[0]);
				foreach($buyer_id_arr as $buyer_id=>$is_confirm_arr)
				{
					...
					foreach($is_confirm_arr as $is_confirm_id=>$order_id_arr)
					{
						//echo __LINE__.'--'.$is_confirm_id; die;
						if($is_confirm_id==1)
						{
							echo "Confirm Order :";
							?>
							...
							<table style="vertical-align: middle;"  width="<? if($all_date_button==""){ echo 2840;}else { echo 3440;} ?>" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all" >
                                <thead> 
                                    <tr style="font-family:'Arial Narrow'" >
                                        <th width="80">Job No</th>
                                        <th width="70" title="counter sample approval">C/S Approval</th>
                                        <th width="80">Order No</th>
                                        <th width="100">Int Reference</th>
                                        <th width="80">Style Ref.</th>
                                        <th width="80">Season</th>
                                        <th width="100">Dealing Marchant</th>
                                        <th width="150">Fabric (body)</th>
                                ...
                                <tbody id="data_panel_<?php echo $html_search;?>">
                                <?
                                //($m<=9)? '0'.$m:$m.$month_value[1].$month_value[0];
                                $k=1;$total_val="";$total_buyer_qty="";$confirm_total_qty=array();$total_grey_qty=0;$total_finish_qty=0;$total_conf_sewing_output=0;
								//echo __LINE__; die;
                                ...
                                foreach($order_id_arr as $order_id=>$row)
                                {
									...
									$order_qty_pcs=$row["po_qty"]*$row['ratio'];
									$update_date_arr=explode(" ",$row["update_date"]);
									$up_date_arr=explode(" ",$row["up_date"]);
									$insert_date_arr=explode(" ",$row["insert_date"]);
									$update_date=date("d-m-Y",strtotime($update_date_arr[0]));
									$up_date=date("d-m-Y",strtotime($up_date_arr[0]));
									$insert_date=date("d-m-Y",strtotime($insert_date_arr[0]));
									$today_date=date("d-m-Y");//date("d-m-Y");
									...
									if($update_date=='' || $update_date=='01-01-1970') $date_diff_insert=datediff( "d", $insert_date , $today_date);
									if($update_date!='' || $update_date!='01-01-1970') $date_diff_update=datediff( "d", $update_date , $today_date);
									if($up_date!='' || $up_date!='01-01-1970') $date_diff_up=datediff( "d", $up_date , $today_date);
									if($date_diff_update<=3) $bg_color="#e67e22";	
									else if($date_diff_up<=3) $bg_color="#e67e22";
									else if($date_diff_insert<=7) $bg_color="#00CED1";	
									else $bg_color=$bgcolor;
									
									$fab_grey_cons=$fab_grey_cons_arr[$row["job_no"]]['fab_knit_req_kg'];//$fab_grey_knit+$fab_grey_woven;
									$total_grey_cons+=$fab_grey_cons;
									$fab_grey_knit=array_sum($fabric_qty_arr['knit']['grey'][$row["po_break_down_id"]]);
									$fab_grey_woven=array_sum($fabric_qty_arr['woven']['grey'][$row["po_break_down_id"]]);
									//echo $fab_grey_knit.'--'.$fab_grey_woven.'<br>';
									$fab_grey_qty=$fab_grey_knit+$fab_grey_woven;
									$total_grey_qty+=$fab_grey_qty;
									//echo __LINE__.'10**'.$total_grey_qty; die;
									$fab_fin_cons=$fab_grey_cons_arr[$row["job_no"]]['fab_knit_fin_req_kg'];//$fab_grey_knit+$fab_grey_woven;
									$total_fin_qty+=$fab_fin_cons;
									
									$fab_finish_knit=array_sum($fabric_qty_arr['knit']['finish'][$row["po_break_down_id"]]);
									$fab_finish_woven=array_sum($fabric_qty_arr['woven']['finish'][$row["po_break_down_id"]]);
									$fab_finish_cons=$fab_finish_knit+$fab_finish_woven;
									$total_finish_qty+=$fab_finish_cons;
									$prod_source=$sewing_outCompany_arr[$row["po_break_down_id"]]['prod_source'];
									$serving_company=$sewing_outCompany_arr[$row["po_break_down_id"]]['serving_company'];
								...
								if($shipment_status_id==2)
								{
									...
									//{
										...
										<tr style="font-family:'Arial Narrow'" bgcolor="<? echo $bg_color; ?>" onClick="change_color('tr_<? echo $x; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $x; ?>">
                                            <td><p><? echo $row["job_no"];?></p></td>
                                            <td align="center"><p><? echo $sample_arr[$order_id];//$sewing_outCompany_arr[$row["po_break_down_id"]]['prod_qty']; ?></p></td>
                                            <td><p><? echo $row["po_number"];?></p></td>
                                            <td><p><? echo $row["grouping"];?></p></td>
                                            <td><p><? echo $row["style_ref_no"];?></p></td>
                                            <td><p><? echo $season_arr[$row["season"]];?></p></td>
                                            <td><p><? echo $dealing_marchant_arr[$row["dealing_marchant"]];?></p></td>
                                            <td><p><? echo $body_part_arr[$row["po_break_down_id"]]; ?></p></td>
								...
								else
								{
									?>
                                        <tr style="font-family:'Arial Narrow'" bgcolor="<? echo $bg_color; ?>" onClick="change_color('tr_<? echo $x; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $x; ?>">
                                            <td width="80"><p><? echo $row["job_no"];?></p></td>
                                            <td align="center" width="70"><p><? echo $sample_arr[$order_id]; ?></p></td>
                                            <td width="80"><p><? echo $row["po_number"];?></p></td>
                                            <td width="100"><p><? echo $row["grouping"];?></p></td>
                                            <td width="80"><p><? echo $row["style_ref_no"];?></p></td>
                                            <td align="center" width="80"><p><? echo $season_arr[$row["season"]];?></p></td>
                                            <td  width="100"><p><? echo $dealing_marchant_arr[$row["dealing_marchant"]];?></p></td>
                                            <td width="80"><p><? echo $body_part_arr[$row["po_break_down_id"]]; ?></p></td>
                                            <td align="center" width="80"><p><? //echo $fab_type_arr[$row["job_no"]]['composition'];
												$color_type_id=array_unique(explode(",",$fab_color_type_arr[$row["job_no"]]['color_type_id']));						
												$color_id='';
												foreach($color_type_id as $c_id)
												{
												...
												$avg_smv=$row['set_smv']/$row['ratio']; //new
											  	$smv_min=$avg_smv*$c_days_qty; //new
												echo $gmts_item;?></p></td>
                                            <td align="center" width="80"><a href="##" onclick="embellish_type_popup(<?php echo $company_name;?>,'<? echo $row["job_no"];?>');"> <p><? //if($embl_ord_type_arr[$row["job_no"]]['emb_name']!="") $embl_name_id=array_unique(explode(",",chop($embl_ord_type_arr[$row["job_no"]]['emb_name'],','))); else 
											$embl_name_id=array_unique(explode(",",$embl_fab_type_arr[$row["job_no"]]['emb_name']));								
												$emble_names='';
												foreach($embl_name_id as $emb_id)
												{
                                            ...
                                            <td width="35" align="center"><p><? echo $row["unit_price"];?></p></td>
                                            <td width="50" align="center"><p><? echo $sql_color_no_arr[$row["po_break_down_id"]]; ?></p></td>
                                            <td align="center" width="70"><p><? echo $variable;?></p></td>
                                            <td width="50" align="center"><p><? echo number_format($fab_grey_cons,2);?></p></td>
                                            <td width="70" align="right"><p><? echo number_format($fab_grey_qty,2);?></p></td>
                                            <td width="50" align="center"><p><? echo number_format($fab_fin_cons,2);?></p></td>
                                            <td width="70" align="right"><p><? echo number_format($fab_finish_cons,2);?></p></td>
                                            ...
                                            for($p=1;$p<=$num_days;$p++)
                                            {
												...
												if($all_date_button=="")
												{
													if($day_total_con[$date_key."-".$day][$buyer_id]>0){
														?>
														<td align="center" style="vertical-align: middle;" title="<? echo $exfact_qty_con;?>" style="height:60px;" width="45" align="center" valign="bottom"><div><? if($days_qty=="") echo ""; else echo  number_format($days_qty,0); ?></div></td>
														<?
														$confirm_total_qty[$p] +=$days_qty;
														$confirm_project_buyer_total[$buyer_id][$p] +=$days_qty;
													}
                                ...
                                ?>
                                <tr bgcolor="#CCCCCC" style="font-family:'Arial Narrow'">
                                    <td colspan="18"  align="right" style="font-weight:bold;">Confirm Total:</td>
                                    <td align="right" style="font-weight:bold;"><?  //$buyer_total_order_grey[$buyer_id] +=$total_grey_cons; $total_grey_cons=''; ?></td>
                                    <td align="right" style="font-weight:bold;"><?  echo number_format($total_grey_qty,2); $buyer_total_order_grey[$buyer_id]+=$total_grey_qty; $total_grey_qty=''; ?></td>
                                    <td align="right" style="font-weight:bold;"><?  //$buyer_total_order_grey[$buyer_id] +=$total_grey_qty; $total_grey_qty=''; ?></td>
                                    <td align="right" style="font-weight:bold;"><? echo number_format($total_finish_qty,2);$buyer_total_order_finish[$buyer_id] +=$total_finish_qty; $total_finish_qty='';?></td>
                                    ...
                                    for($p=1;$p<=$num_days;$p++)
                                    {
										...
										if($all_date_button=="")
										{
											if($day_total_con[$date_key."-".$day][$buyer_id]>0){
											?>
											<td width="45" align="center"  valign="bottom"><div><? if($confirm_total_qty[$p]=="") echo ""; else echo number_format($confirm_total_qty[$p],0); ?></div></td>
											<?
											}
										}
										else
                        ...
                        <script>setFilterGrid('data_panel_<?php echo $html_search;?>',-1);</script>
						<?
						$html_search++;
					}
					else if($is_confirm_id==2)//project start here
					{
						...
						<table style="vertical-align: middle;"  width="<? if($all_date_button==""){ echo 2760;}else { echo 3360;} ?>" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
                            <thead> 
                                <tr style="font-family:'Arial Narrow'">
                                    <th width="80">Job No</th>
                                    <th width="70" title="counter sample approval">C/S Approval</th>
                            ...
                            foreach($order_id_arr as $order_id=>$row)
                            {
								...
								<tr style="font-family:'Arial Narrow'" bgcolor="<? echo $bg_colors; ?>" onClick="change_color('tr_<? echo $x; ?>','<? echo $bgcolors;?>')" id="tr_<? echo $x; ?>">
									<td><p><? echo $row["job_no"];?></p></td>
									<td align="center"><p><? echo $sample_arr[$order_id]; ?></p></td>
									<td><p><? echo $row["po_number"];?></p></td>
									<td><p><? echo $row["grouping"];?></p></td>
									<td><p><? echo $row["style_ref_no"];?></p></td>
									<td align="center"><p><? echo $season_arr[$row["season"]];?></p></td>
									<td><p><? echo $dealing_marchant_arr[$row["dealing_marchant"]];?></p></td>
									<td><p><? echo $body_part_arr[$row["po_break_down_id"]]; ?></p></td>
									<td><p><?  $color_type_id=array_unique(explode(",",$fab_color_type_arr[$row["job_no"]]['color_type_id'])); $color_id='';
										foreach($color_type_id as $c_id)
									...
									$smv_min=0;
										...
										$smv_min=$avg_smv*$c_days_qty; //new
										echo $gmts_item; ?></p></td>
									<td align="center"><a href="##" onclick="embellish_type_popup(<?php echo $company_name;?>,'<? echo $row["job_no"];?>');"><p><?
										$embl_name_id=array_unique(explode(",",$embl_fab_type_arr[$row["job_no"]]['emb_name'])); $emble_names='';
										foreach($embl_name_id as $emb_id)
										{
											if($emble_names=="") $emble_names=$emblishment_name_array[$emb_id]; else $emble_names.=", ".$emblishment_name_array[$emb_id];
									...
									<td align="center"><p><? echo $row["unit_price"]?></p></td>
									<td align="center"><p><? echo $sql_color_no_arr[$row["po_break_down_id"]]; ?></p></td>
									<td  align="center"><p><?  echo $variable;?></p></td>
									...
									for($p=1;$p<=$num_days;$p++)
									{
										...
										if($all_date_button=="")
										{
											...
											{
												?>
												<td  align="center" style="height:60px; vertical-align: middle;" title="<? echo $exfact_qty_proj;?>"  align="center"  valign="bottom"><div><? if($proj_days_qty=="") echo ""; else echo number_format($proj_days_qty,0); ?></div></td>
												<?
												$confirm_total_qty_project[$p] +=$proj_days_qty;
												$confirm_project_buyer_total[$buyer_id][$p] +=$proj_days_qty;
											}
						...
						?>
						<tr style="font-family:'Arial Narrow'" bgcolor="#CCCCCC">
                            <td colspan="17"  align="right" style="font-weight:bold;">Projected Total:</td>
                            <td align="right" style="font-weight:bold;"><? //$buyer_total_order_gery[$buyer_id] +=$total_grey_qty_proj; $total_grey_qty_proj=''; ?></td>
                            <td align="right" style="font-weight:bold;"><? echo number_format($total_grey_qty_proj,2); $buyer_total_order_grey[$buyer_id]+=$total_grey_qty_proj; $total_grey_qty_proj=''; ?></td>
                            <td align="right" style="font-weight:bold;"><? //$buyer_total_order_gery[$buyer_id] +=$total_grey_qty_proj; $total_grey_qty_proj=''; ?></td>
                            <td align="right" style="font-weight:bold;"><? echo number_format($total_finish_qty_proj,2); $buyer_total_order_finish[$buyer_id] +=$total_finish_qty_proj; $total_finish_qty_proj='';  ?></td>
                            ...
                            for($p=1;$p<=$num_days;$p++)
                            {
								...
								if($all_date_button=="")
								{
									...
									{
										?>
											<td width="45" align="center"><div style="word-break:break-all"><? echo number_format($confirm_total_qty_project[$p],0); ?></div></td>
										<?
										$project_qty_total[$p]+=$confirm_total_qty_project[$p];
										//$total_val[]
									}
            ...
            for($m=1;$m<=$num_days;$m++)
            {
				...
				if($all_date_button=="")
				{
					if($ship_status_total>0)
					{
						...
						{
							?>
							<td align="center" width="45"><div style="word-break:break-all"><p><? //echo number_format($grand_day_qty_arr[$m],0); ?></p></div></td>
							<?
						}
						else
						{
...
{ 
	...
	//    if(!$all_po_ids_cond)$all_po_ids_cond="";
		...
		foreach($all_po_array as $ord_id) {
			if($ord_id!=0) {
				$r_id2=execute_query("insert into tmp_poid (userid, poid, type) values ($user_id,$ord_id,4)");
				// echo "insert into tmp_poid (userid, poid, type) values ($user_id,$ord_id,985)";
				// if($pi_id=="") $pi_id=$p_id[csf('pi_id')];else $pi_id.=",".$p_id[csf('pi_id')];
			}            
		}
		...
		$total_pcs_sql="SELECT   c.id,b.order_quantity from wo_po_details_master a, wo_po_color_size_breakdown b,wo_po_break_down c, tmp_poid tmp where 	a.job_no=b.job_no_mst	and b.po_break_down_id=c.id	and b.status_active=1 and c.status_active=1	and b.is_deleted=0 and c.is_deleted=0 and tmp.userid=$user_id	and  c.id=tmp.poid and tmp.type=4 $date_cond3 $company_con";// and  b.status_active=1 and b.is_deleted=0 and  d.status_active=1 and d.is_deleted=0
		$total_pcs_arr=array();
					...
					foreach($is_confirm_arr as $is_confirm_id=>$order_id_arr)
					{
							...
							<table  width="<? if($all_date_button==""){ echo 2150;}else { echo 2990;} ?>" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all" >
                                ...
                                foreach($order_id_arr as $order_id=>$row)
                                {
									...
									$insert_date_arr=explode(" ",$row["insert_date"]);
									 ...
									 if($date_diff_update<=3)
										{
											$bg_color="#e67e22";	
										}
										else if($date_diff_insert<=3)
										{
											$bg_color="#00CED1";	
										}
										else
						...
						else if($is_confirm_id==2)
						{
							...
							<table  width="<? if($all_date_button==""){ echo 2150;}else { echo 2990;} ?>" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
                                ...
                                <?
                                //($m<=9)? '0'.$m:$m.$month_value[1].$month_value[0];
                                $m=1;$total_val=array();$total_buyer_qty="";$confirm_total_qty_project=array();
								$total_grey_qty_proj=0;	$total_finish_qty_proj=0;$total_proj_sewing_output=0;
                                foreach($order_id_arr as $order_id=>$row)
                                {
									if ($m%2==0)
...
<?
	...
	$r_id3=execute_query("delete from tmp_poid where userid=$user_id and type in (4)");
    if($db_type==0) {
        if($r_id3) {
            mysql_query("COMMIT");  
        }
			...
			//var_dump($date_key);die;
               ...
               <table  width="1170" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
                   <thead> 
					...
					<?
                    //($m<=9)? '0'.$m:$m.$month_value[1].$month_value[0];
					$k=1;$total_val=array();$total_buyer_qty="";$total_month_qty=0;$row_span=0;$company_check=array();
...
if ($action=="embellish_type_popup")
{
	extract($_REQUEST);
	echo load_html_head_contents("Season No Info", "../../../../", 1, 1,'','','');
	?>
	...
	$data_array=sql_select("select id, job_no, emb_name, emb_type, body_part_id, country, supplier_id, is_apply_last_update,cons_dzn_gmts, rate, amount, status_active, budget_on from wo_pre_cost_embe_cost_dtls where emb_name!=3 and job_no='$job_no' and is_deleted=0 order by id");
	?>
	<div id="content_embellishment_cost" align="left">
    	<fieldset style="width:590px">
    		<table width="590" cellspacing="0" class="rpt_table" border="0" id="tbl_embellishment_cost" rules="all">
    			<thead>
    				<tr>
    					<th colspan="4">Garments Emblishment</th>
    				</tr>
                	<tr>
                ...
                <tbody>
                	<?php
                		...
                		{
                			$data_arr[$data[csf('emb_name')]]['emb_name'] = $data[csf('emb_name')];
                			$data_arr[$data[csf('emb_name')]]['emb_type'] = $data[csf('emb_type')];
                			$data_arr[$data[csf('emb_name')]]['body_part_id'] = $data[csf('body_part_id')];

                			$country_text="";
                			$countryarr=explode(",",$data[csf("country")]);
                			...
                			$data_arr[$data[csf('emb_name')]]['country_text'] = $country_text;
                		}
                	...
                	<tr>
                		...
                		<td><?php echo $type_array[$data_arr[$key]['emb_name']][$data_arr[$key]['emb_type']];?></td>
                		<td><?php echo $body_part_arr[$data_arr[$key]['body_part_id']];?></td>
                		<td><?php echo $data_arr[$key]['country_text'];?></td>
                	</tr>
                	<?php
                			}
                		}
			...
			$num_days = cal_days_in_month(CAL_GREGORIAN, $month_value[1], $month_value[0]);
               ...
               <table  width="1150" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
                   <thead> 
                    ...
                    <tbody>
					<?
					$k=1;$total_val=array();$total_buyer_qty="";$total_month_qty=0;
                    foreach($buyer_id_arr as $buyer_id=>$is_confirm)
                    {
						foreach($is_confirm as $row)
						{
							...
							if($row['is_confirmed']==1)
							{
								...
								{?>
								<tr bgcolor="<? echo $bgcolor ; ?>" style="font-family:'Arial Narrow'">
									<td title="fkjg"><? echo $buyer_library[$row['buyer']]; ?></td>
									<td align="right"><? echo number_format($row['po_qty'],0); $total_buyer_qty+=$row['po_qty']; ?>&nbsp;</td>
									<td align="right" title="<? echo  'Po Qty='.$tot_c_mon_qtys.', Ship qty '.$c_total_exfact_qty_con;?>" ><p><? echo number_format(($tot_c_mon_qty),0); $total_month_qty +=$tot_c_mon_qty; ?></p></td>
									<?
									$exfact_qty_con=0;$tot_bal_qty=0;
									for($p=1;$p<=$num_days;$p++)
									{
										...
										if($all_date_button==""){
											...
											if($day_total_con[$date_key."-".$day]>0 || $day_total_proj[$date_key."-".$day]>0){
												...
												?>
												<td  style="height:20px;" title="wq<? echo 'PO Qty='.$day_qtys.', Ship Qty'.$exfact_qty_con;?>" width="35" align="center"><div><? if($row_data_dtls[$date_key][$buyer_id][$date_key."-".$day]>0) {if($day_qty=="") echo ""; else echo number_format($day_qty,0);} else echo ""; ?></div></td>
												<?
												$total_val[$day] +=$day_qty;
											}
										}
...
{ 
    ...
    if($shipment_status_id==2) //Running Order Balance Qty
        {
            ...
            $sql_exf_c_date="SELECT d.id as color_break_id,a.buyer_name,a.dealing_marchant,d.po_break_down_id as po_id,d.country_ship_date as country_ship_date,d.cutup_date, c.is_confirmed, a.product_category
             from wo_po_break_down c,wo_po_details_master a,wo_po_color_size_breakdown d  where  c.job_no_mst=a.job_no and  d.status_active=1 and d.is_deleted=0 and c.id=d.po_break_down_id  and  a.job_no=d.job_no_mst  and c.status_active=1 and c.is_deleted=0   $company_con  $marchd_cond  $buyer_cond $season_cond $date_cond2";
            $result_c=sql_select($sql_exf_c_date);
            foreach($result_c as $row)
            {
                ...
                if($row[csf("is_confirmed")]==1)
                {
					$c_date_con_arr[$row[csf("is_confirmed")]][$row[csf("buyer_name")]][$row[csf("po_id")]][$row[csf("color_break_id")]]['country_date']=$row[csf("country_ship_date")];
					$c_date_con_arr[$row[csf("is_confirmed")]][$row[csf("buyer_name")]][$row[csf("po_id")]][$row[csf("color_break_id")]]['cut_date']=$row[csf("country_ship_date")];
                }
                else
                {
					$c_date_con_arr[$row[csf("is_confirmed")]][$row[csf("buyer_name")]][$row[csf("po_id")]][$row[csf("color_break_id")]]['country_date']=$row[csf("country_ship_date")];
		...
		/* echo "<pre>";
		print_r($c_date_con_arr); die; */
        
     $data_array="SELECT a.id as job_id, a.job_no,  a.company_name, a.buyer_name,a.dealing_marchant,a.total_set_qnty,b.id as color_break_id,b.po_break_down_id,b.order_quantity as po_quantity,b.order_quantity as po_quantity_pcs,c.po_quantity as po_qty,b.country_ship_date,b.cutup_date,b.order_total,c.is_confirmed, a.product_category from wo_po_details_master a, wo_po_color_size_breakdown b,wo_po_break_down c  where    a.job_no=b.job_no_mst and b.po_break_down_id=c.id  and b.status_active=1 and b.is_deleted=0  $company_con $date_cond $buyer_cond $order_status_cond $shipment_status_cond $season_cond $marchd_cond $product_category_con order by $month_order_by a.buyer_name ASC";
     //echo $data_array; die;
    $po_id_array=array();
    $result_po=sql_select($data_array);
    $row_data_dtls=array();
    $tmp_arr=array();
        ...
        {   
            ...
            /* $tmp_arr[date("Y-m",strtotime($row[csf("country_ship_date")]))][$row[csf("buyer_name")]][$row[csf('product_category')]][$row[csf("is_confirmed")]]["po_qty"]+=$row[csf("po_quantity_pcs")];
            $tmp_arr[date("Y-m",strtotime($row[csf("country_ship_date")]))][$row[csf("buyer_name")]][$row[csf('product_category')]][$row[csf("is_confirmed")]]["buyer"] =$row[csf("buyer_name")];
            $tmp_arr[date("Y-m",strtotime($row[csf("country_ship_date")]))][$row[csf("buyer_name")]][$row[csf('product_category')]][$row[csf("is_confirmed")]]["dealing_marchant"] =$row[csf("dealing_marchant")];
            $tmp_arr[date("Y-m",strtotime($row[csf("country_ship_date")]))][$row[csf("buyer_name")]][$row[csf('product_category')]][$row[csf("is_confirmed")]]["is_confirmed"] =$row[csf("is_confirmed")]; */

			$tmp_arr[date("Y-m",strtotime($row[csf("country_ship_date")]))][$row[csf("is_confirmed")]][$row[csf("buyer_name")]][$row[csf('product_category')]]["po_qty"]+=$row[csf("po_quantity_pcs")];
            $tmp_arr[date("Y-m",strtotime($row[csf("country_ship_date")]))][$row[csf("is_confirmed")]][$row[csf("buyer_name")]][$row[csf('product_category')]]["buyer"] =$row[csf("buyer_name")];
            $tmp_arr[date("Y-m",strtotime($row[csf("country_ship_date")]))][$row[csf("is_confirmed")]][$row[csf("buyer_name")]][$row[csf('product_category')]]["dealing_marchant"] =$row[csf("dealing_marchant")];
            $tmp_arr[date("Y-m",strtotime($row[csf("country_ship_date")]))][$row[csf("is_confirmed")]][$row[csf("buyer_name")]][$row[csf('product_category')]]["is_confirmed"] =$row[csf("is_confirmed")];


            $month_wise_total[date("Y-m",strtotime($row[csf("country_ship_date")]))] +=$row[csf("po_quantity_pcs")];
            if($row[csf("is_confirmed")]==1)
            {
                $row_data_dtls[date("Y-m",strtotime($row[csf("country_ship_date")]))][$row[csf("is_confirmed")]][$row[csf("buyer_name")]][$row[csf('product_category')]][date("Y-m-d",strtotime($row[csf("country_ship_date")]))]+=$row[csf("po_quantity_pcs")];

                $day_total_con[date("Y-m-d",strtotime($row[csf("country_ship_date")]))]+=$row[csf("po_quantity_pcs")];
            }
            else if($row[csf("is_confirmed")]==2)
            {
                $row_data_dtls_project[date("Y-m",strtotime($row[csf("country_ship_date")]))][$row[csf("is_confirmed")]][$row[csf("buyer_name")]][$row[csf('product_category')]][date("Y-m-d",strtotime($row[csf("country_ship_date")]))]+=$row[csf("po_quantity_pcs")];
			...
			$product_category_arr[date("Y-m",strtotime($row[csf("country_ship_date")]))][$row[csf('product_category')]]=$row[csf('product_category')];
        ...
        if($shipment_status_id==2)
        {
            $sql_exf="SELECT f.country_ship_date, a.buyer_name,b.po_break_down_id as po_id,d.color_size_break_down_id as color_break_id, c.is_confirmed, a.product_category,
            (CASE WHEN b.entry_form!=85 THEN d.production_qnty   ELSE 0 END) as ex_fact_qty,
            (CASE WHEN b.entry_form=85 THEN d.production_qnty  ELSE 0 END) as ex_fact_ret_qty
            from wo_po_break_down c,pro_ex_factory_mst b,wo_po_details_master a,pro_ex_factory_dtls d ,wo_po_color_size_breakdown f where c.id=b.po_break_down_id and c.job_no_mst=a.job_no and b.id=d.mst_id and b.po_break_down_id=b.po_break_down_id and f.id=d.color_size_break_down_id and f.po_break_down_id=c.id and  b.status_active=1 and b.is_deleted=0 $po_ids_cond  and  d.status_active=1 and d.is_deleted=0  $company_con  $buyer_cond $season_cond ";
			//echo $sql_exf; die;
            ...
            foreach($result as $row)
            {
                ...
                if($cbo_date_category==1)
                {
                    $row[csf("country_ship_date")]=$c_date_con_arr[$row[csf("is_confirmed")]][$row[csf("buyer_name")]][$row[csf("po_id")]][$row[csf("color_break_id")]]['country_date'];
                }
                else //Cut-Off
                {
                    $row[csf("country_ship_date")]=$c_date_con_arr[$row[csf("is_confirmed")]][$row[csf("buyer_name")]][$row[csf("po_id")]][$row[csf("color_break_id")]]['cut_date'];   
                }
                //echo 'zzz'.$row[csf("country_ship_date")].'<br>';
                if($row[csf("is_confirmed")]==1 && $row[csf("country_ship_date")]!='')
                {
                    $ex_fact_con_arr[date("Y-m",strtotime($row[csf("country_ship_date")]))][$row[csf('is_confirmed')]][$row[csf("buyer_name")]][$row[csf('product_category')]][date("Y-m-d",strtotime($row[csf("country_ship_date")]))]+=$row[csf("ex_fact_qty")]-$row[csf("ex_fact_ret_qty")];
                }
                else if($row[csf("is_confirmed")]==2 && $row[csf("country_ship_date")]!='')
                {
                    $ex_fact_proj_arr[date("Y-m",strtotime($row[csf("country_ship_date")]))][$row[csf('is_confirmed')]][$row[csf("buyer_name")]][$row[csf('product_category')]][date("Y-m-d",strtotime($row[csf("country_ship_date")]))]+=$row[csf("ex_fact_qty")]-$row[csf("ex_fact_ret_qty")];
                }
            }
			/* echo "<pre>";
            print_r($ex_fact_con_arr); die; */

        }
        ob_start();
        ?>
		<div  class="scroll_div_inner" style="margin-left:10px;">
        <table width="1150">
            <tr class="form_caption">
                <td colspan="10" width="200"></td>
                <td colspan="12" align="center" style="font-size:18px;" width="200">
		...
		$category_wise_row=array();
		foreach($tmp_arr as $date_key=>$is_confirm_arr)
        {
			foreach($is_confirm_arr as $confirm_id=>$buyer_id_arr)
			{
				foreach($buyer_id_arr as $buyer_id=>$categorywise_data)
				{   
					foreach($categorywise_data as $category_id=>$row)
					{
						
						$category_wise_row[$date_key][$confirm_id][$buyer_id]+=1;
		...
		/* echo '<pre>';
		print_r($category_wise_row); die; */


        foreach($tmp_arr as $date_key=>$confirm_arr)
        {
            $month_value=explode("-",$date_key);
            $num_days = cal_days_in_month(CAL_GREGORIAN, $month_value[1], $month_value[0]);
                ?>
               ...
               <table  width="1250" class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
                   <thead> 
                    ...
                    <?
                    //($m<=9)? '0'.$m:$m.$month_value[1].$month_value[0];
                    $total_val=array(); $total_category_val=array(); $total_buyer_cat_qty=array(); $total_buyer_qty=""; $total_month_cat_qty=array(); $total_month_qty=0;
					
					foreach($confirm_arr as $confirmid=>$buyer_id_arr)
					{	
						foreach($buyer_id_arr as $buyer_id=>$categorywise_data)
						{   $k=1;
							foreach($categorywise_data as $category_id=>$row)
							{	
								...
								if($row['is_confirmed']==1)
								{
									...
									for($p=1;$p<=$num_days;$p++)
									{
										...
										if($shipment_status_id==2) //Balance Qty
										{
											$c_total_exfact_qty_con=$ex_fact_con_arr[$date_key][$row['is_confirmed']][$buyer_id][$category_id][$date_key."-".$day]; 
											//echo $date_key.'--'.$row['is_confirmed'].'--'.$buyer_id.'--'.$category_id.'--'.$date_key."-".$day.'--'.$row_data_dtls[$date_key][$row['is_confirmed']][$buyer_id][$category_id][$date_key."-".$day].'<br>';
											$tot_c_bal_qty=$row_data_dtls[$date_key][$row['is_confirmed']][$buyer_id][$category_id][$date_key."-".$day];
											$total_cqty_po=$tot_c_bal_qty-$c_total_exfact_qty_con;
											if($total_cqty_po<0) $totc_day_qty=0;else $totc_day_qty=$total_cqty_po;
										}
										else
										{
											$totc_day_qty=$row_data_dtls[$date_key][$row['is_confirmed']][$buyer_id][$category_id][$date_key."-".$day];
										}
										$total_cqty+=$totc_day_qty;
									...
									{                                       
										...
										<tr bgcolor="<? echo $bgcolor ; ?>" style="font-family:'Arial Narrow'">
											<? if($k==1){ ?>
											<td style="vertical-align: middle; text-align:center" rowspan="<?= $category_wise_row[$date_key][$confirmid][$row['buyer']] ?>"><? echo $buyer_library[$row['buyer']]; ?></td>
											<? } ?>
											<td ><? echo $product_category[$category_id]; ?></td>
											<td align="right"><? echo number_format($row['po_qty'],0); $total_buyer_qty+=$row['po_qty'];  ?>&nbsp;</td>
											<td align="right" title="<? echo  'Po Qty='.$tot_c_mon_qtys.', Ship qty '.$c_total_exfact_qty_con;?>" ><p><? echo number_format(($tot_c_mon_qty),0); $total_month_qty +=$tot_c_mon_qty;  ?></p></td>
											<?
											$total_buyer_cat_qty[$date_key][$category_id]+=$row['po_qty'];
											$total_month_cat_qty[$date_key][$category_id]+=$tot_c_mon_qty;
											...
											for($p=1;$p<=$num_days;$p++)
											{
												...
												{
													//$c_total_exfact_qty_con=$ex_fact_con_arr[$date_key][$row['is_confirmed']][$buyer_id][$category_id][$date_key."-".$day]; 
													$exfact_qty_con=$ex_fact_con_arr[$date_key][$row['is_confirmed']][$buyer_id][$category_id][$date_key."-".$day]; 
													$tot_bal_qty=$row_data_dtls[$date_key][$row['is_confirmed']][$buyer_id][$category_id][$date_key."-".$day];
												}
												else
												{
													$tot_bal_qty=$row_data_dtls[$date_key][$row['is_confirmed']][$buyer_id][$category_id][$date_key."-".$day]-$exfact_qty_con;
												}
												if($all_date_button==0 || $all_date_button=='')
												{
													if($day_total_con[$date_key."-".$day]>0 || $day_total_proj[$date_key."-".$day]>0){
														...
														?>
														<td  style="height:20px;" title="<? echo 'PO Qty= '.$day_qtys.', Ship Qty= '.$exfact_qty_con;?>" width="35" align="center"><div><? if($row_data_dtls[$date_key][$row['is_confirmed']][$buyer_id][$category_id][$date_key."-".$day]>0) {if($day_qty=="") echo ""; else echo number_format($day_qty,0);} else echo ""; ?></div></td>
														<?
														$total_val[$day] +=$day_qty;
														$total_category_val[$date_key][$category_id][$day] +=$day_qty;
													}
												}
												else
												{
								...
								else if($row['is_confirmed']==2)
								{
									...
									for($p=1;$p<=$num_days;$p++)
									{
										...
										if($shipment_status_id==2) //Balance Qty
										{
											if($day_total_proj[$date_key."-".$day]>0 || $day_total_con[$date_key."-".$day]>0)
											{
												$c_total_exfact_qty_proj+=$ex_fact_proj_arr[$date_key][$buyer_id][$category_id][$date_key."-".$day];  
											}
										}                                       
									}
									?>
									<tr bgcolor="<? echo $bgcolor ; ?>" style="font-family:'Arial Narrow'">
										<? if($k==1){ ?>
										<td rowspan="<?= $category_wise_row[$date_key][$confirmid][$row['buyer']] ?>" style="vertical-align: middle; text-align:center"><? echo $buyer_library[$row['buyer']]."(bk)"; ?></td>
										<? } ?>
										<td ><? echo $product_category[$category_id]; ?></td>
										<td  align="right"><? echo number_format($row['po_qty'],0); $total_buyer_qty+=$row['po_qty']; ?>&nbsp;</td>
										<td align="right" title="<? echo $c_total_exfact_qty_proj;?>"><p><? echo number_format($row["po_qty"]-$c_total_exfact_qty_proj,0); $total_month_qty +=$row["po_qty"]-$c_total_exfact_qty_proj; ?></p></td>
										<?
										$total_buyer_cat_qty[$date_key][$category_id]+=$row['po_qty'];
										$total_month_cat_qty[$date_key][$category_id]+=$row["po_qty"]-$c_total_exfact_qty_proj;
										...
										for($p=1;$p<=$num_days;$p++)
										{
											...
											if($shipment_status_id==2) //Balance Qty
											{
												$exfact_qty_proj=$ex_fact_proj_arr[$date_key][$row['is_confirmed']][$buyer_id][$category_id][$date_key."-".$day];
												$tot_bal_qty=$row_data_dtls_project[$date_key][$row['is_confirmed']][$buyer_id][$category_id][$date_key."-".$day]-$exfact_qty_proj;
												if($tot_bal_qty<0) $days_qty=0;else $days_qty=$tot_bal_qty;
											}
											else
											{
												$days_qty=$row_data_dtls_project[$date_key][$row['is_confirmed']][$buyer_id][$category_id][$date_key."-".$day]-$exfact_qty_proj;
											}
												if($all_date_button=="")
												{
													if($day_total_proj[$date_key."-".$day]>0 || $day_total_con[$date_key."-".$day]>0){
														?>
														<td  style="height:20px;" title="<? echo $exfact_qty_proj;?>" width="35" align="center" ><div><?  if($row_data_dtls_project[$date_key][$row['is_confirmed']][$buyer_id][$category_id][$date_key."-".$day] >0) { if($days_qty=="") echo ""; else echo number_format($days_qty,0); } else echo "";  ?></div></td>
														<?
														$total_val[$day] +=$days_qty;
														$total_category_val[$date_key][$category_id][$day] +=$days_qty;
													}
												}
												else
												{
					...
					$pc=1;
					if(count($product_category_arr[$date_key])>1){
                    ?>
						<tr bgcolor="#BDD7EE" style="font-family:'Arial Narrow';">
                            <td  align="right" style="font-weight:bold; vertical-align: middle; text-align:center" rowspan="<?= count($product_category_arr[$date_key]) ?>" >Total</td>
							<? foreach($product_category_arr[$date_key] as $catid){
								
								if($pc!=1) { 
									if ($pc%2==0) $bgcolor="#FFFFFF"; else $bgcolor="#BDD7EE"; ?> 
									<tr id=<?= $pc?> bgcolor="<? echo $bgcolor; ?>" style="font-family:'Arial Narrow';"> <? }
								 ?>
								<td  align="right" style="font-weight:bold;"><? echo $product_category[$catid]; ?></td>
								<td  align="right" style="font-weight:bold;"><? echo number_format($total_buyer_cat_qty[$date_key][$catid],0); ?>&nbsp;</td>
								<td  align="right" style="font-weight:bold;"><?  echo number_format($total_month_cat_qty[$date_key][$catid],0); ?>&nbsp;</td>
								<?
								for($p=1;$p<=$num_days;$p++)
								{
									$day=($p<=9)? '0'.$p:$p;
									if($all_date_button==0)
									{
										if($day_total_cons_all[$date_key."-".$day]>0){
										?>
										<td  style="height:20px;" width="35"  align="center" title="<?= $catid.'--'.$day ?>" ><div><?  echo number_format($total_category_val[$date_key][$catid][$day],0); ?></div></td>
										<?
										}
									}
									else
									{
										?>
										<td  style="height:20px;" width="35"  align="center" title="<?= $catid.'--'.$day ?>"><div><?  echo number_format($total_category_val[$catid][$day],0); ?></div></td>
										<?
									}
								}
								$pc++;
						...
						<? } ?>
                        <tr bgcolor="#CCCCCC" style="font-family:'Arial Narrow';">
                            <td  align="right" style="font-weight:bold; text-align:center">G.Total</td>
                            <td  align="right" style="font-weight:bold;"></td>
                            <td  align="right" style="font-weight:bold;"><? echo number_format($total_buyer_qty,0); ?>&nbsp;</td>
                             <td  align="right" style="font-weight:bold;"><?  echo number_format($total_month_qty,0); ?>&nbsp;</td>
                            <?
