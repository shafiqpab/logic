...
{	
	...
	if($txt_date_from!="" && $txt_date_to!=""){
		...
		else
		{
			...
			elseif(str_replace("'","",$cbo_date_type)==1)
			{
				$search_text .=" and d.ex_factory_date between '".$start_date."' and '".$end_date."'";
				$search_text2 .=" and e.ex_factory_date between '".$start_date."' and '".$end_date."'";
			}
	...
	//pro_ex_factory_mst
	 $sql_ex_fac="select b.id as po_id,max(d.ex_factory_date) as  ex_factory_date,sum(d.ex_factory_qnty) as ex_factory_qnty from wo_po_details_master a, wo_po_break_down b,pro_ex_factory_mst d where a.id=b.job_id and b.id=d.po_break_down_id $company_name and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and d.is_deleted=0 and d.status_active=1 $search_text $buyer_id_cond $shipping_status_cond $po_cond_for_in $file_no_cond $ref_no_cond group by b.id  order by b.id";
	$ex_sql_result = sql_select($sql_ex_fac); 
	foreach($ex_sql_result as $row) 
	{
		$ex_fact_arr[$row[csf('po_id')]]['ex_factory_date']=$row[csf('ex_factory_date')];
		$ex_fact_arr[$row[csf('po_id')]]['qty']=$row[csf('ex_factory_qnty')];
		$ex_fact_poid_arr[$row[csf('po_id')]]=$row[csf('po_id')];
		
	}
		$ex_poCon=where_con_using_array($ex_fact_poid_arr,0,"b.id");
		$job_poCon=where_con_using_array($ex_fact_poid_arr,0,"c.id");
	...
	//echo $po_cond_for_in;die;ex_factory_date
   ...
   /* $sql="select $date_diff a.buyer_name,a.total_set_qnty,a.job_no_prefix_num ,a.job_no,a.style_ref_no,b.po_number,a.set_smv,a.set_break_down,a.order_uom,a.gmts_item_id,b.id,a.buyer_name,sum(d.order_quantity) as po_quantity,sum(d.order_quantity) as  po_qty,sum(d.plan_cut_qnty) as plan_cut,b.unit_price as unit_price,$date_category as pub_shipment_date,b.po_received_date,b.insert_date,b.grouping, b.file_no,b.is_confirmed,b.inserted_by,sum(d.order_total) as po_total_price,b.details_remarks,a.id as job_id  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown d where a.id=b.job_id and a.id=d.job_id  and b.id=d.po_break_down_id $company_name and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and d.is_deleted=0 and d.status_active=1 $search_text $buyer_id_cond $shipping_status_cond $po_cond_for_in $file_no_cond $ref_no_cond $groupby order by $date_category";*/
	     ...
	     $sql_po="select $date_diff a.buyer_name,a.total_set_qnty,a.job_no_prefix_num ,a.job_no,a.style_ref_no,b.po_number,a.set_smv,a.set_break_down,a.order_uom,a.gmts_item_id,b.id,a.buyer_name,(b.po_quantity*a.total_set_qnty) as po_quantity,(b.po_quantity) as  po_qty,(b.plan_cut) as plan_cut,b.unit_price as unit_price,$date_category as pub_shipment_date,b.po_received_date,b.insert_date,b.grouping, b.file_no,b.is_confirmed,b.inserted_by,(b.po_total_price) as po_total_price,b.details_remarks,a.id as job_id  from wo_po_details_master a, wo_po_break_down b where a.id=b.job_id  $company_name and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1   $buyer_id_cond $shipping_status_cond $po_cond_for_in $file_no_cond $ref_no_cond  $ex_poCon order by a.buyer_name,b.id";  
   ...
   $sql="select $date_diff a.buyer_name,a.total_set_qnty,a.job_no_prefix_num ,a.job_no,a.style_ref_no,b.po_number,a.set_smv,a.set_break_down,a.order_uom,a.gmts_item_id,b.id,a.buyer_name,sum(d.order_quantity) as po_quantity,sum(d.order_quantity) as  po_qty,sum(d.plan_cut_qnty) as plan_cut,b.unit_price as unit_price,$date_category as pub_shipment_date,b.po_received_date,b.insert_date,b.grouping, b.file_no,b.is_confirmed,b.inserted_by,sum(d.order_total) as po_total_price,b.details_remarks,a.id as job_id,d.country_id  from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown d where a.id=b.job_id and a.id=d.job_id  and b.id=d.po_break_down_id $company_name and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and d.is_deleted=0 and d.status_active=1 $search_text $buyer_id_cond $shipping_status_cond $po_cond_for_in $file_no_cond $ref_no_cond $groupby order by $date_category";  
	...
	$po_id_str=array();	 		
	$order_sql_result = sql_select($sql_po) or die(mysql_error());
	$job_no_arr=array();
	$all_job_IdArr=array();
	foreach($order_sql_result as $rows){
		...
		$po_id_str[$rows[csf('id')]]=$rows[csf('id')];
...
/*$poCon=where_con_using_array($order_id_arr,0,"a.po_break_down_id");
$poCon2=where_con_using_array($order_id_arr,0,"b.po_id");
$poCon3=where_con_using_array($order_id_arr,0,"po_break_down_id");
$poCon4=where_con_using_array($order_id_arr,0,"a.po_breakdown_id");
$poCon5=where_con_using_array($order_id_arr,0,"po_breakdown_id");
$poCon6=where_con_using_array($order_id_arr,0,"b.order_id");
$poCon7=where_con_using_array($order_id_arr,0,"b.po_breakdown_id");*/
		...
		$grey_and_dyeing_cost_sql="SELECT sum(b.amount) as amount, b.order_id as order_id,a.process_id as process_id
	    	...
	    	//echo $grey_and_dyeing_cost_sql;die;
			 ...
			 //echo $cbo_year.'DD'.$cbo_year_selection;die;
			$cbo_yearId=str_replace("'","",$cbo_year);
			//$cbo_year_selection=str_replace("'","",$cbo_year_selection);
			 if(str_replace("'","",$cbo_year) !=0){
				// $condition->job_year("and to_char(a.insert_date,'YYYY')=$cbo_yearId");
			}
			// if(str_replace("'","",$cbo_year)==0)
			// {
				  if(str_replace("'","",$cbo_year) !=0)
				  {
					 $condition->job_year("and to_char(a.insert_date,'YYYY')=$cbo_year");
					}
			...
			//print_r($po_id_str);die;
			$ppid=implode(",",$po_id_str);
			if($ppid!='')
			{
				$condition->po_id_in("$ppid"); 
			}
		   $condition->init();
	...
	foreach($order_data_arr as $rows)
	{
		...
		$ex_factory_qty=$ex_fact_arr[$rows[csf('id')]]['qty'];
						//$ex_factory_value=$ex_factory_qty*($rows[csf('po_total_price')]/$rows[csf('po_quantity')]);
		...
		//$lay_cut_data[$rows[csf('job_no')]][$rows[csf('buyer_name')]];
			...
			$yarn_cost_actual=$yarnTrimsCostArray[$rows[csf('id')]]['grey_yarn_amt']-$yarnissueRetCostArray[$rows[csf('id')]]['grey_yarn_ret_amt'];
		...
		$buyer_summary_arr[$rows[csf('buyer_name')]]['exfactory_qty']+=$ex_factory_qty;
		$buyer_summary_arr[$rows[csf('buyer_name')]]['yarn_budget_val']+=$po_wise_yarn_budget_data[$rows[csf('id')]];
		$buyer_summary_arr[$rows[csf('buyer_name')]]['exfactory_val']+=$ex_factory_qty*($rows[csf('po_total_price')]/$rows[csf('po_quantity')]);
		...
		$buyer_summary_arr[$rows[csf('buyer_name')]]['yarn_value']+=round($yarn_cost_actual); 
		...
		$buyer_tot_arr['yarn_budget_val']+=$po_wise_yarn_budget_data[$rows[csf('id')]];//new
		...
		$buyer_tot_arr['exfactory_qty']+=$ex_factory_qty;
		$buyer_tot_arr['exfactory_val']+=$ex_factory_qty*($rows[csf('po_total_price')]/$rows[csf('po_quantity')]);
		...
		$buyer_tot_arr['yarn_value']+=round($yarn_cost_actual);
        ...
        <table width="100%" border="1" rules="all"  class="rpt_table"> 
            <thead>
                <tr style="font-size:12px"> 
                    ...
                    <th width="80">Knititng Prod.</th>
                    <th width="80">Yarn Budget Value</th>
                    <th width="80">Grey to Dye</th>
                    <th width="80">G2D (%)</th>
                    <th width="80">Dyeing Unload Qty.</th>
                    <th width="80">Fini. Fabrics Req.</th> 
                    <th width="80">Finish Fab. Rcvd Qty.</th>
            ...
            <tbody>
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('str_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="str_<? echo $i; ?>" style="font-size:12px; cursor:pointer;" valign="middle">
                    ...
                    <td align="right"><? echo omitZero(number_format($rows['po_val'],0));?></td>
                     ...
                     <td align="right"><? echo omitZero(number_format($rows['exfactory_val'],0));?></td>
					...
					// echo gettype($po_qty);
                     ...
                     <td align="right"><? echo omitZero(number_format($rows['yarn_issue'],0));?></td>
                      <td align="right"><? echo omitZero(number_format($rows['yarn_value'],0));?></td>
                     <td align="right"><? echo number_format($rows['grey_prod'],0);?></td>
                      <td align="right"><? echo number_format($rows['yarn_budget_val'],0);?></td>
            ...
            <tfoot>
                ...
                <th align="right"><? echo omitZero(number_format($buyer_tot_arr['po_val']));?></th>
                 ...
                 <th align="right"><? echo omitZero(number_format($buyer_tot_arr['exfactory_val']));?></th>
                 <th align="right"><? //echo omitZero(number_format($buyer_tot_arr['po_val']));?></th>
                ...
                <th align="right"><? echo round($buyer_tot_arr['yarn_alloc']);?></th>	 
                <th align="right"><? echo omitZero(number_format($buyer_tot_arr['yarn_issue'],0));?></th>
                 <th align="right"><? echo omitZero(number_format($buyer_tot_arr['yarn_value'],0));?></th>	
                <th align="right"><? echo omitZero(number_format($buyer_tot_arr['grey_prod'],0));?></th>	
                <th align="right"><? echo omitZero(number_format($buyer_tot_arr['yarn_budget_val'],0));?></th>	
	...
	<div style="width:<?=$main_width+$new_column+20;?>px" align="left">	
        <table width="100%" cellpadding="0" cellspacing="0">
            <tr><td colspan="30" align="center" style="font-size:22px;">Ex Factory Wise Order Follow-Up Report</td></tr>
            <tr>
        ...
        <table width="<?=$main_width+$new_column+20;?>" border="1" rules="all"  class="rpt_table"> 
            <thead>
                <tr style="font-size:12px"> 
					...
					<th width="80" style="word-break: break-all;">Last Ex-Factotry Date</th>
                    
                    <th width="80" style="word-break: break-all;">Ex-Factory Qty.</th>
                    <th width="80" style="word-break: break-all;">Ex-Factory FOB Value</th>
                    <th width="60" style="word-break: break-all;">Ex-Fac %</th>
                    ...
                    <th width="" style="word-break: break-all;" title="95% GREEN 75% - 95% YELLOW">Finish</th>
            ...
            ?>	
                <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" style="font-size:12px; cursor:pointer;" valign="middle">
					...
					<?
						...
						$last_ex_factory_date=$ex_fact_arr[$row[csf('id')]]['ex_factory_date'];
						$ex_factory_qty=$ex_fact_arr[$row[csf('id')]]['qty'];
						$ex_factory_value=$ex_factory_qty*($row[csf('po_total_price')]/$row[csf('po_quantity')]);
					?>
                    </p></td>	
                    ...
                    <td width="60" style="width:60px; word-wrap:break-word;" onClick="openmypage_image('requires/ex_factory_date_wise_order_follow_up_report_controller.php?action=show_image&job_no=<? echo $row[csf("job_no")] ?>','Image View')" align="center"><img src="../../../<? echo $imge_arr[$row[csf('job_no')]];?>" height='25' width='25' /></td>	
					...
					<td width="80" style="word-break: break-all;" align="center"><?  echo change_date_format($last_ex_factory_date);?></td>
                    
                    <td width="80" style="word-break: break-all;" align="right"><a href="javascript:void(0)" onclick="qty_details('load_ex_factory_data', '', <?=$row[csf("id")]?>)"><? //echo omitZero(number_format($ex_qnty_array[$row[csf('id')]],0));?></a><? echo number_format($ex_factory_qty,2);?></td>
                    <td width="80" style="word-break: break-all;" align="right"><?   echo number_format($ex_factory_value,2);?></td>
                    <td width="60" style="word-break: break-all;" align="right"><?  echo number_format(($ex_factory_qty/$row[csf('po_quantity')]*100),2);?></td>
                    ...
                    <td width="" style="word-break: break-all;" align="right" <? echo $finfBgColor;?> title="<? echo $finFactoryPer;?>%">
        ...
        <table width="<?=$main_width+$new_column;?>" border="1" cellpadding="0" cellspacing="0" id="report_table_footer" class="rpt_table" rules="all">
         	 <tfoot>
	           <tr>
				...
				<th width="80"></th>
			    <th width="80" ></th>	
                <th width="80" id="td_ex_qty"><p  style="width:80px; word-wrap:break-word;"><? echo number_format($tot_po_value,0);?></p></th>
                <th width="80" id="td_ex_value"></th>
                <th width="60"></th>
                ...
                <th width="" id="td_sewing_finish_qnty" ><p style="width:60px; word-wrap:break-word;"><? echo number_format($tot_sewing_finish_qnty,0);?></p></th>
        ...
        <div style="width:<? echo $tbWith+20;?>px; max-height:350px; overflow-y:scroll" id="report_div" > 
            ...
            ?>	
                <tr bgcolor="<? echo $bgcolor; ?>" onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>" style="font-size:12px; cursor:pointer;"" valign="middle">
                    ...
                    <td width="60" onClick="openmypage_image('requires/ex_factory_date_wise_order_follow_up_report_controller.php?action=show_image&job_no=<? echo $row[csf("job_no")] ?>','Image View')" align="center"><img src="../../../<? echo $imge_arr[$row[csf('job_no')]];?>" height='25' width='30' /></td>	
