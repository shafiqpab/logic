...
//--------------------------------------------------------------------------------------------------------------------
		...
		<body>
			<div align="center" style="width:100%;" >
				<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
					<table width="1280" cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
						<tr>
								...
								<table  cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
									...
									<tr>
										<td> 
											...
											<? 
											echo create_drop_down( "cbo_company_mst", 120, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", $company_name,"load_drop_down( 'style_wise_cost_comparison_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" );
											?>
										</td>
										<td id="buyer_td">
											<? 
	...
	if(str_replace("'","",$data[3])!="" && str_replace("'","",$data[4])!="")
	{
	 if($db_type==0)
		...
		{
			$date_cond=" and d.ex_factory_date between '$start_date' and '$end_date'";
		}
	}
	...
	$style_cond="";
		...
		if ($data[2]==0)
		{
			...
			else
			{
				if($db_type==0)
				{
					$sql="select a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity, b.id as po_id,b.po_number,sum(b.po_quantity) as  po_quantity,b.shipment_date,a.garments_nature,b.grouping,b.file_no,DATEDIFF(pub_shipment_date,po_received_date) as  date_diff,SUBSTRING_INDEX(a.`insert_date`, '-', 1) as year from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c,pro_ex_factory_mst d where a.job_no=b.job_no_mst and c.job_no_mst=a.job_no and c.po_break_down_id=b.id and c.po_break_down_id=d.po_break_down_id and b.id=d.po_break_down_id and a.garments_nature=$data[5] and a.status_active=1 and b.status_active=1  $company $buyer $job_cond $order_cond $style_cond $shiping_status $file_no_cond$date_cond  $internal_ref_cond ".set_user_lavel_filtering(' and a.buyer_name','buyer_id')."  group by a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity, b.po_number,b.shipment_date,a.garments_nature,b.id,b.grouping,b.file_no,a.insert_date,b.pub_shipment_date,b.po_received_date order by a.job_no";
				}
				if($db_type==2)
				{
		...
		{
			//a.job_no_prefix_num, a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.job_quantity, b.id as po_id,b.po_number,sum(b.po_quantity) as  po_quantity,b.shipment_date,a.garments_nature,b.grouping,b.file_no,(pub_shipment_date - po_received_date) as  date_diff,to_char(a.insert_date,'YYYY') as year
			if($cbo_search_by==3 && $data[13]==3)
			{
        ...
        <div>
     	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1180" class="rpt_table">
            <thead>
                ...
                <th width="70"><?
                	if($cbo_search_by==3 && $data[13]==3) echo "Ex-Factory";else echo "Shipment Date";
				?></th>
        ...
        <div style="width:1180px; max-height:270px;overflow-y:scroll;" >	 
            ...
            foreach($style_po_arr as $po_id=>$row)
			{
				...
				$style_data=$row[("job_prefix")].'_'.$row[("year")].'_'.$row[('company_name')];
	...
	if($action=="create_po_search_list_view22") //Not used
	{
	//echo $data;die;
	$data=explode('_',$data);
	$cbo_search_by = str_replace("'","",$data[14]);
...
//	 $sql="select a.id,a.trans_id,a.trans_type,a.entry_form,a.po_breakdown_id,a.prod_id,a.quantity,a.issue_purpose,a.returnable_qnty,a.is_sales,b.yarn_count_id,b.yarn_comp_type1st,  b.yarn_comp_percent1st,b.yarn_type,c.cons_rate,c.cons_amount from order_wise_pro_details a, product_details_master b,inv_transaction c where a.prod_id=b.id and a.trans_id=c.id and  a.trans_type=2 and a.entry_form=3 and a.po_breakdown_id in(".implode(",",$poIdArr).") and a.is_deleted=0 and  a.status_active=1 and b.is_deleted=0 and  b.status_active=1";
		...
		//echo array_sum($grey_fab_trans_array['tt']).'dd';
		//print_r($grey_fab_trans_array);
		
	  	/*$subconInBillData_dyeing="select b.order_id, 
		sum(CASE WHEN a.process_id not in(2,4) THEN b.amount END) AS other_bill,
		sum(CASE WHEN a.process_id=4 THEN b.amount END) AS dye_finish_bill
		from subcon_outbound_bill_mst a, subcon_outbound_bill_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and a.process_id in(2,4,3) and b.order_id in(".implode(",",$poIdArr).")  group by b.order_id";
		$subconInBillDataDyingArray=sql_select($subconInBillData_dyeing);
		foreach($subconInBillDataDyingArray as $subRow)
		{
		//$subconCostArray[$subRow[csf('order_id')]]['knit_bill']+=$subRow[csf('other_bill')];
		$subconCostArray[$subRow[csf('order_id')]]['dye_finish_bill']+=$subRow[csf('dye_finish_bill')]+$subRow[csf('other_bill')];
		}	*/
		...
		foreach($subconOutBillDataArray as $subRow)
		{
			...
			//echo $grey_fab_cost.'='.$tot_knit_charge;;	
		...
		// Transfer Grey Act
				...
				if($fin_from_order_id!="") //Finish Transfer
				{
					...
					//echo $tot_dye_finish_cost_pre.'='.$tot_dye_finish_cost_pre_qty;	
		...
		/* $conversion= new conversion($condition);
		 ...
		 $conversion= new conversion($condition);
		 $conversion_costing_qty_arr_process=$conversion->getQtyArray_by_jobAndProcess();*/ 
...
// Fabric Purch ============================
	...
	//$aopDesArr=array();
		...
		//body_part_id febric_description_id
		 $sql = "select  b.body_part_id,b.febric_description_id,b.receive_qty,b.rate,b.amount from subcon_outbound_bill_mst a, subcon_outbound_bill_dtls b where a.id=b.mst_id and a.process_id=4 and b.sub_process_id in(31,35) and b.order_id in(".implode(",",$poIdArr).") and  a.is_deleted=0 and a.status_active=1 and  b.is_deleted=0 and b.status_active=1";
		$data_aop=sql_select($sql);
		foreach($data_aop as $row_aop){
		...
		<div style="width:1202px; margin:0 auto">
			...
			<table class="rpt_table" border="1" cellpadding="2" cellspacing="2" style="width:1200px; margin-top:5px" rules="all">
				...
				foreach($YarnData as $index=>$row ){
                 ...
                 // $yarn_bgcolor="#E9F3FF"; $fab_bgcolor="#FFFFFF";
			...
			//echo "(".$commiData['pre']['rate']."/(".$costPerQty*$totalSetQnty."))*".$exfactoryQty;
				...
				<tr  bgcolor="<? echo $other_bgcolor;?>" onClick="change_color('commer_<? echo $embl; ?>','<? echo $other_bgcolor;?>')" id="commer_<? echo $embl; ?>">
					...
					<td width="100" align="right"><? echo $exfactoryQty ?></td>
					<td width="100" align="right"></td>
					<td width="100" align="right"  bgcolor="<? if(number_format($commaData['acl']['amount'],4,".","")>number_format($commaData['pre']['amount'],4,".","")){echo "red";}else{ echo "";} ?>"><? echo number_format($commaData['acl']['amount'],4); $totalOtherAclValue+=number_format($commaData['acl']['amount'],4,".","");?></td>
				</tr>
			<tr  bgcolor="<? echo $other_bgcolor;?>" onClick="change_color('trfc_<? echo $embl; ?>','<? echo $other_bgcolor;?>')" id="trfc_<? echo $embl; ?>">
				...
				<tr style="font-weight:bold;background-color:#CCC" bgcolor="<? echo $other_bgcolor;?>" onClick="change_color('trot_<? echo $embl; ?>','<? echo $other_bgcolor;?>')" id="trot_<? echo $embl; ?>">
            ...
            <table border="1" class="rpt_table" rules="all" width="750" cellpadding="0" cellspacing="0">
                ...
                foreach ($result as $row) {
                    if ($i % 2 == 0)
						...
						$knit_cost=$conversion_costing_arr_process[$from_order_id][1];
						$knit_qty=$conversion_costing_arr_process_qty[$from_order_id][1];
						$knit_charge=$knit_cost/$knit_qty;
                    ...
                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                        ...
                        <td align="right" title="<? echo number_format($knit_charge, 4); ?>"><? echo number_format($row[csf('transfer_qnty')]*$knit_charge, 2); ?> </td>
                    </tr>
                    <?
                    $total_trans_in_qnty += $row[csf('transfer_qnty')];
					$total_trans_in_amt += $row[csf('transfer_qnty')]*$knit_charge;
                    $i++;
                }
                ?>
                ...
                foreach ($result as $row) {
                    if ($i % 2 == 0)
						...
						$knit_cost_out=$conversion_costing_arr_process[$out_from_order_id][1];
						$knit_qty_out=$conversion_costing_arr_process_qty[$out_from_order_id][1];
                    ...
                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                        ...
                        <td width="170"><p><? echo $row[csf('product_name_details')]; ?></p></td>
                        <td  width="100" align="right"><? echo number_format($row[csf('transfer_qnty')], 2); ?> </td>
                         <td align="right" title="<? echo $knit_charge_out; ?>"><? echo number_format($row[csf('transfer_qnty')]*$knit_charge_out, 2); ?> </td>
                    </tr>
                    <?
                    $total_trans_out_qnty += $row[csf('transfer_qnty')];
					 $total_trans_out_amt += $row[csf('transfer_qnty')]*$knit_charge_out;
                    $i++;
                }
                ?>
                <tr style="font-weight:bold">
...
?>
	<fieldset style="width:950px; margin-left:7px">
        ...
        <caption> <b>Marking Cost (CM) Actaul</b></caption>
            <thead>
                <th width="30">SL</th>
        ...
        <div style="width:970px; max-height:330px; overflow-y:scroll" id="scroll_body">
			<table border="1" class="rpt_table" rules="all" width="950px" cellpadding="0" cellspacing="0">  
				...
				/*$sql_job="select  a.job_no,a.buyer_name,a.style_ref_no,b.production_date,b.produce_min,b.available_min from wo_po_details_master a,production_logicsoft b where a.job_no=b.jobNo and a.job_no='$job_no' and a.status_active=1 and a.is_deleted=0 ";*/
				 $sql_job="SELECT  a.job_no,a.buyer_name,a.style_ref_no,c.production_date,sum(c.efficency_min) as available_min ,sum(c.produced_min) as produce_min from wo_po_details_master a,wo_po_break_down b,pro_resource_ava_min_dtls c where a.job_no=b.job_no_mst and b.id=c.order_ids and a.status_active=1 and b.status_active=1 and c.status_active=1 and a.job_no='$job_no' group by a.job_no,a.buyer_name,a.style_ref_no,c.production_date ";
                ...
                foreach($result_job as $row)
                {
                    ...
                    <tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
                        ...
                        <td align="right" width="60"><? echo  number_format(($row[csf('produce_min')]/$row[csf('available_min')])*100,2); ?>&nbsp;</td>
