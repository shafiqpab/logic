                    ...
                    <div style="max-height:400px; overflow-y:scroll; width:3800px" align="left" id="scroll_body">
                        ...
                        if (str_replace("'", "", $cbo_search_by) == 1 || str_replace("'", "", $cbo_search_by) == 2) {
							...
							from 
								wo_po_details_master a, wo_po_break_down b,wo_pre_cost_mst c 
							where  
								a.job_no=b.job_no_mst  and a.job_no=c.job_no and b.job_no_mst=c.job_no and c.entry_from=111 and a.status_active=1 and b.status_active=1 and c.status_active=1 and b.po_number like '$order_no_cond' and a.style_ref_no like '$style_ref_cond' $date_cond $searchCond $job_no_cond $team_name_cond $team_member_cond $year_cond $file_no_cond $ref_no_cond $season_id_cond $order_status_cond order by b.pub_shipment_date,a.job_no_prefix_num, b.id";
                                }//$year_cond
                                //echo $jobSql;die;
...
//#############################################################################################
    ...
    if ($action == "sewing_finish_popup") {
       ...
       // echo $sql;//die;
        ...
        $poColorArr = $poSizeArr = $ColorSizeArr = array();
        //$ColorvalueArr=array();
        $color_source_wise_company=array();
        foreach ($sqlResult as $row) {
            $index = $row[csf("color_number_id")] . $row[csf("size_number_id")];
    ...
    ?>   
                        <tfoot>
                            <tr>
                                <th colspan="4" style="word-break: break-all;">Total</th>
                                <?
                                foreach ($sizeArr_pend as $val) {
                                    $grandTotal += $col_total_size_qnty[$val];
                                    ?>                            
