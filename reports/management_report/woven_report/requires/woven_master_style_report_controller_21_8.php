...
if($action=="report_generate2")
{
	...
	<div>
		...
		$sql_inq="select c.id as inquery_id,c.inquery_date,c.mail_send_date,c.team_leader,c.buyer_id,c.color_id,c.color,c.insert_by,c.company_id as company_name,c.system_number_prefix_num,c.dealing_marchant,c.concern_marchant,c.style_description,c.style_refernce,c.season_buyer_wise, c.season_year,c.brand_id,c.actual_req_quot_date,c.actual_sam_send_date,c.actual_sam_send_date,c.con_rec_target_date,c.system_number
		...
		//{
			$inquery_id_Cond=".where_con_using_array($inquery_id_arr,0,'c.id').";
		//}
			  $sql_po_chk="select c.id as inquery_id,a.id as job_id,a.job_no_prefix_num,a.job_quantity,a.inserted_by as po_insert_by,a.insert_date as po_insert_date,a.dealing_marchant as po_dealing_marchant,a.team_leader, a.job_no,a.company_name,a.buyer_name, a.style_ref_no, a.avg_unit_price, a.total_set_qnty as ratio,b.po_quantity, b.id as po_id, b.po_number,b.grouping, b.unit_price, b.shiping_status,b.is_confirmed, b.pub_shipment_date,b.grouping as ref_no,c.inquery_date,c.insert_by,c.system_number_prefix_num,c.dealing_marchant,c.concern_marchant,c.style_description,c.style_refernce,c.season_buyer_wise, c.season_year,c.brand_id,c.actual_req_quot_date,c.actual_sam_send_date,c.actual_sam_send_date,c.con_rec_target_date,c.system_number,d.order_quantity,d.plan_cut_qnty,d.proj_qty,c.buyer_submit_price,c.offer_qty from wo_po_details_master a, wo_po_break_down b, wo_quotation_inquery c,wo_po_color_size_breakdown d where a.job_no=b.job_no_mst and d.po_break_down_id=b.id and  a.job_no=d.job_no_mst   and c.id=a.inquiry_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0   $comp_cond $style_owner_cond  $team_leader_cond $dealing_cond $buyer_id_cond $season_year_cond $job_no_cond   $job_id_cond $order_id_cond_trans $order_no_cond $brand_name_cond $date_cond_target $date_cond $season_name_cond  $int_ref_cond2  $style_ref_cond  ".where_con_using_array($inquery_id_arr,0,'c.id')." order by c.id ";
...
if($action=="report_generate3")
{
	...
	<div>
		...
		$sql_inq="select c.id as inquery_id,c.inquery_date,c.mail_send_date,c.team_leader,c.buyer_id,c.color_id,c.color,c.insert_by,c.company_id as company_name,c.system_number_prefix_num,c.dealing_marchant,c.concern_marchant,c.style_description,c.style_refernce,c.season_buyer_wise, c.season_year,c.brand_id,c.actual_req_quot_date,c.actual_sam_send_date,c.actual_sam_send_date,c.con_rec_target_date,c.system_number,c.offer_qty  from  wo_quotation_inquery c where  c.status_active=1 and c.is_deleted=0   $comp_cond2   $team_leader_cond3 $dealing_cond2  $buyer_id_cond2 $season_year_cond2  $brand_name_cond2 $date_cond_target $date_cond $season_name_cond2  $int_ref_cond2  $inq_style_ref_cond  $inqu_ref_cond $team_leader_chk $date_cond_bulk order by c.id desc"; // ".where_con_using_array($inquery_id_arr,0,'c.id')."
		...
		foreach($result_sql_inq as $row)
		{
			...
			$master_stlye_arr[$row[csf('company_name')]][$row[csf('buyer_id')]]['qnty']+=$row[csf('offer_qty')];
		...
		?>
		 <table class="rpt_table" width="<? echo $width_td;?>" cellpadding="0" cellspacing="0" border="1" rules="all">
            <body>
                    <tr style="font-size:13px">
						<th colspan="4"><h1>Experience Group</h1></th>                   
                    </tr>
					<tr style="font-size:13px">
						<th colspan="4"><h3>From To</h3></th>                 
                    </tr>
					<tr style="font-size:13px">
						<th colspan="4"><h1>Company & Buyer Wise Summary</h1></th>                
                    </tr>
            </body>
        </table>
		...
		$com_wise_tot=array();
		foreach($master_stlye_arr as $com_id=>$buyer_data)
		{
		  ...
		  <table class="rpt_table" width="<? echo $width_td;?>" cellpadding="0" cellspacing="0" border="1" rules="all">
            <thead>
					<tr style="font-size:13px">
                       	<th colspan="4"><?=$company_library[$com_id];?></th>
		...
		<?
			...
			foreach($buyer_data as $buyer_id=>$row){?>	
				...
				<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
					...
					<td width="120" title="<?=$com_id;?>"><p><? echo $company_library[$com_id]; ?></p></td>			
					<td width="100" title="<?=$buyer_id;?>"><p><? echo $buyer_library[$buyer_id]; ?></p></td>
					<td width="100"><p><? echo  $row[('qnty')]; ?></p></td>			
				</tr>
				<?
				$com_wise_tot[$com_id]+=$row[('qnty')];
				$grand_total+=$row[('qnty')];
				$i++;
			  } ?>
		...
		<table class="tbl_bottom" width="<? echo $width_td;?>" id="report_table_footer" cellpadding="0" cellspacing="0" border="1" rules="all">
        	<tr>
					...
					<td width="100" colspan="3"><?=$company_library[$com_id];?> Total Pcs</td>
					<td width="100">&nbsp;<?=$com_wise_tot[$com_id];?></td>         
            </tr>
        ...
        <table class="tbl_bottom" width="<? echo $width_td;?>" id="report_table_footer" cellpadding="0" cellspacing="0" border="1" rules="all">
        	<tr>
                ...
                <td width="100" colspan="3">Group Total Pcs</td>
                <td width="100">&nbsp;<?=$grand_total;?></td>             
            </tr>
