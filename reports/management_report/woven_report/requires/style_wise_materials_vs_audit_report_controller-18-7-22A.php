	...
	//$shiping_status=1;$shiping_status_id="";
	 ...
	 $sql_pi = "select a.id as pi_mst_id,a.item_category_id as cat_id,a.pi_number,b.work_order_no,b.color_id,b.work_order_id,b.determination_id,(b.quantity) as qty,b.rate,b.amount,b.item_group,b.item_description from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id  and a.pi_basis_id=1  and b.item_category_id in(2,3) and b.status_active=1 and b.is_deleted=0 $booking_cond_in ";//wo_id_arr
		...
		foreach ($TrimiSQLresult as $row) {
			...
			//echo $trim_group.'DD';
			$pi_description=$row[csf('item_description')];
	...
	//print_r($PiArr3);
		unset($EmblSQLresult);
			 ...
			 foreach( $pre_other_result as $row )
			{
				...
				$commercial_cost=($row[csf('comm_cost')]/$order_price_per_dzn)*$order_job_qnty;
		 ...
		 foreach( $result_commi as $row )
			{
				...
				$style_desc_arr[$job_no][$commision_type]['req_amount']+=($row[csf('commission_amount')]/$order_price_per_dzn)*$order_job_qnty;
				$style_desc_arr[$job_no][$commision_type]['sourcing_amount']+=$row[csf('commission_amount')];
				$style_desc_arr[$job_no][$commision_type]['sourcing_req_amount']+=($row[csf('commission_amount')]/$order_price_per_dzn)*$order_job_qnty;
			} 
			unset($result_commi);
             ...
             <tr>
               ...
               <tr style="font-size:13px">
                        ...
                        <th width="100" >Job No</th>
                       	...
                       	<th width="100" >Style Order<br> Qty (Pcs)</th>
		...
		?> 
                ...
                <td width="100" rowspan="<? echo $job_row_span;?>"><p><? echo  $job_wiseArr[$job_key]['order_quantity']; ?></p></td>
                <?
					$tot_po_qty+=$job_wiseArr[$job_key]['order_quantity'];
				}
				?>
                ...
                <td width="80" align="right"><p><? echo number_format($row[('amount')],6); ?></p></td>
				<td width="80" align="right"><p><? echo number_format($row[('req_amount')],6); ?></p></td>
				<td width="80"  title="<? ?>" align="right"><p> <? echo  number_format($row[('sourcing_amount')],6); ?> 	</p></td>
				
				<td width="80" align="right" title="Sourcing Amount"><p><? echo number_format($row[('sourcing_req_amount')],6); ?></p></td>
				<td width="80" align="right"><p><? echo number_format($row[('cons')],6); ?></p></td>
				
                <td width="80" align="right"><p><? echo number_format($row[('req_qty')],6); ?></p></td>
                <td width="80" align="right"><p><? echo number_format($row[('fin_fab_qnty')],6); ?></p></td>
				<td width="80" align="right" title="Woven Fin "><p><? if($row[('booking_amount')]) echo number_format($row[('booking_amount')]/$row[('fin_fab_qnty')],6);else echo ""; ?></p></td>
				 
                <td width="80" align="right"><p><a href="##" onClick="report_generate_popup('<? echo $company_name; ?>','<? echo $job_key; ?>','<? echo $typeId; ?>','<? echo $pre_dtls_ids; ?>','<? echo $fab_deter_min_id; ?>','<? echo $all_pre_dtls_ids; ?>','','report_generate_woven',1)"><? echo number_format($row[('booking_amount')],6); ?></a><? //echo number_format($row[('booking_amount')],4); ?></p></td>
				<td width="80" align="right" title="Fab Req-Booking Req Qty"><p><? $fab_bal_qty=$row[('req_qty')]-$row[('fin_fab_qnty')];echo number_format($fab_bal_qty,6); 
				 $fab_value_balance=$row[('sourcing_req_amount')]-$row[('booking_amount')];
				?></p></td>
				<td width="80" align="right" title="Sourcing Value-Booking Value"><p><? 
				echo number_format($fab_value_balance,6); ?></p></td>
				<td width="80" align="right" title=" Pi Qty"><p><a href="##" onClick="report_generate_popup('<? echo $company_name; ?>','<? echo $job_key; ?>','<? echo $typeId; ?>','<? echo $pi_mst_ids; ?>','<? echo $fab_deter_min_id; ?>','<? echo $all_pre_dtls_ids; ?>','<? echo $all_pi_dtls_ids; ?>','report_generate_woven_pi',2)"><? echo number_format($row[('pi_qty')],6); ?></a><? //$fabric_bal_after_cut_qty=$wv_recv_fin-$fabric_used_qty;
				//echo number_format($row[('pi_qty')],4); ?></p></td>
				<td width="80" align="right" title="PI Amount/PI Qty"><p><? if($row[('pi_amt')]) echo number_format($row[('pi_amt')]/$row[('pi_qty')],6); else echo "";?></p></td>
				<td width="80" align="right" title="PI Amount"><p><?  echo number_format($row[('pi_amt')],6);
				 ?></p></td>
				...
				$pi_balance_amount=$row[('booking_amount')]-$row[('pi_amt')];
				 echo number_format($pi_balance_qty,6); ?>   </p></td>
                 
				<td width="80" align="right" title="Booking Amount-PI Amount"><p><? echo number_format($pi_balance_amount,6); ?></p></td>
         ...
         <table class="tbl_bottom" width="<? echo $width_td;?>" id="report_table_footer" cellpadding="0" cellspacing="0" border="1" rules="all">
        	<tr>
                ...
                <td width="150">Total Cost(Without CM)</td>
                 <td width="60"></td>
                <td width="80"><? echo number_format($tot_budget_amount_dzn,6);?></td>
                <td width="80"><? echo number_format($tot_budget_req_amount,6);?></td>
                <td width="80"><? echo number_format($tot_sourcing_amount_dzn,6);?></td>
                <td width="80"><? echo number_format($tot_sourcing_req_amount,6);?></td>
                <td width="80"><? //echo number_format($tot_budget_cons_dzn,6);?></td>
                <td width="80"><? echo number_format($tot_budget_req_qty,6);?></td>
                <td width="80"><? echo number_format($tot_booking_qty,6);?></td>
                <td width="80"><? //echo number_format($tot_booking_qty,6);?></td>
                <td width="80"><? echo number_format($tot_booking_amount,6);?></td>
                <td width="80"><? echo number_format($tot_fab_qty_balance,6);?></td>
                <td width="80"><? echo number_format($tot_fab_value_balance,6);?></td>
                <td width="80"><? echo number_format($tot_pi_qty,6);?></td>
                <td width="80"><? //echo number_format($tot_pi_qty,6);?></td>
                <td width="80"><? echo number_format($tot_pi_value,6);?></td>
                <td width="80"><? echo number_format($tot_pi_balance_qty,6);?></td>
                <td width="80"><? echo number_format($tot_pi_balance_value,6);?></td>
                <td width="100"><? //echo number_format($total_cut_to_ship_ratio,6);?></td>
                <td width=""><? //echo number_format($total_other_cost_conv,6);?></td>
            ...
            <tr>
                ...
                <td width="150">CM Amount/Dzn(USD)</td>
                 <td width="60"></td>
                <td width="80"><? echo number_format($tot_style_cm_dzn,6);?></td>
                <td width="80" title="CM Amount/Dzn*PoQty(<? echo $tot_po_qty;?>)/12"><? echo number_format($tot_cm_dzn,6);?></td>
				...
				echo number_format($cm_amt_sourcing_amt_dzn,6);?></td>
                <td width="80" title="Grand Total Com Budget amt-Sourcing Amt"><? 
				//cm_amt_sourcing_amt
				echo number_format($cm_amt_sourcing_amt,6);?></td>
                <td width="80"><? //echo number_format($tot_budget_cons_dzn,6);?></td>
                <td width="80"><? //echo number_format($tot_budget_req_qty,6);?></td>
                <td width="80"><? //echo number_format($tot_booking_qty,6);?></td>
                <td width="80"><? //echo number_format($tot_booking_qty,6);?></td>
			...
			?>
             <tr>
                ...
                <td width="150">Grand Total Cost</td>
                 <td width="60"></td>
                <td width="80"><? //echo number_format($tot_style_cm_dzn,6);?></td>
                <td width="80" title="Total Value -Tot CM Value"><? echo number_format($grnd_tot_budget_amt,6);?></td>
                <td width="80"><? //echo number_format($tot_sourcing_amount_dzn,6);?></td>
                <td width="80"><? echo number_format($grnd_tot_sourcing_amt,6);?></td>
                <td width="80"><? //echo number_format($tot_budget_cons_dzn,6);?></td>
                <td width="80"><? //echo number_format($tot_budget_req_qty,6);?></td>
                <td width="80"><? //echo number_format($tot_booking_qty,6);?></td>
                <td width="80"><? //echo number_format($tot_booking_qty,6);?></td>
			...
			?>
             <tr>
                ...
                <td width="100"></td>
                <td width="100"></td>
                <td width="150">FOB/Pc(USD)</td>
                 <td width="60"></td>
               
                <td width="80" title="CM Amount/Dzn+Total Cost(Without CM)/12"><? echo number_format($fob_pcs_usd_dzn,6);?></td>
                  <td width="80"><? //echo number_format($tot_sourcing_amount_dzn,6);?></td>
                 <td width="80" title="Grand Tot Cost(<? echo $grnd_tot_sourcing_amt;?>)/Po Qty(<? echo $tot_po_qty;?>)"><? echo number_format($grnd_tot_sourcing_amt/$tot_po_qty,6);?></td>
             ...
             <tr>
                ...
                <td width="100"></td>
                <td width="100"></td>
                <td width="150">SMV/Pc</td>
                 <td width="60"></td>
                <td width="80"><? //echo number_format($tot_style_cm_dzn,6);?></td>
                <td width="80" ><? echo number_format($tot_job_smv,6);?></td>
                <td width="80"><? //echo number_format($tot_sourcing_amount_dzn,6);?></td>
                <td width="80"><? //echo number_format($tot_sourcing_req_amount,6);?></td>
                <td width="80"><? //echo number_format($tot_budget_cons_dzn,6);?></td>
                <td width="80"><? //echo number_format($tot_budget_req_qty,6);?></td>
              ...
              <tr>
                ...
                <td width="150">EPM(USD)</td>
                 <td width="60"></td>
                <td width="80"><? //echo number_format($tot_style_cm_dzn,6);?></td>
                <td width="80" title="CM Amount Dzn/12/SMV"><? echo number_format(($tot_style_cm_dzn/12)/$tot_job_smv,6);?></td>
                <td width="80"><? //echo number_format($tot_sourcing_amount_dzn,6);?></td>
                <td width="80" title="CM Amount Sourcing /12/SMV"><? echo number_format(($cm_amt_sourcing_amt_dzn/12)/$tot_job_smv,6);?></td>
                <td width="80"><? //echo number_format($tot_budget_cons_dzn,6);?></td>
                <td width="80"><? //echo number_format($tot_budget_req_qty,6);?></td>
                <td width="80"><? //echo number_format($tot_booking_qty,6);?></td>
                <td width="80"><? //echo number_format($tot_booking_qty,6);?></td>
...
if($action=="report_generate_woven")
{
	...
	<table class="rpt_table" border="1" cellpadding="5" cellspacing="2" width="<? echo $width;?>" rules="all">
			...
			foreach ($bookingData as $fab_des => $row) 
			{
				...
				?>
				 <tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $m; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $m; ?>" style="font-size:11px">
                    ...
                    <td align="right" ><? echo number_format($row['wo_qnty'],6);?></td>
                    <td align="right" ><? echo number_format($row['amount']/$row['wo_qnty'],6);?></td>
                    <td align="right" ><? echo number_format($row['amount'],6);?></td>
		...
		<tr>
			...
			<th align="right"><? echo number_format($tot_fin_fab_qnty,6,'.','');?></th>
            <th align="right"><? //echo number_format($tot_amount,6,'.','');?></th>
            <th align="right"><? echo number_format($tot_amount,6,'.','');?></th>
 ...
 if($action=="report_generate_woven_pi")
{
	...
	<table class="rpt_table" border="1" cellpadding="5" cellspacing="2" width="<? echo $width;?>" rules="all">
			...
			foreach ($bookingData as $fab_des => $row) 
			{
				...
				?>
				 <tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $m; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $m; ?>" style="font-size:11px">
                    ...
                    <td align="right" ><? echo number_format($booking_qnty,6);?></td>
                    <td align="right" ><? echo number_format($booking_amount/$booking_qnty,6);?></td>
                    <td align="right" ><? echo number_format($booking_amount,6);?></td>
                    
                    <td align="right" ><? echo number_format($row['pi_qty'],6);?></td>
                    <td align="right" ><? echo number_format($row['pi_amount']/$row['pi_qty'],6);?></td>
                    <td align="right" ><? echo number_format($row['pi_amount'],6);?></td>
                    
                    <td align="right" title="BookingQty-PI Qty" ><? echo number_format($booking_qnty-$row['pi_qty'],6);?></td>
                    <td align="right" title="BookingAmount-PI Amount" ><? echo number_format($booking_amount-$row['pi_amount'],6);?></td>
                    <td align="center" ><? echo $row['supplier'];?></td>
                    <td align="center" ><? echo $row['insert_date'];?></td>
                    <td align="center" ><? echo $lib_user_arr[$row['inserted_by']];?></td>
		...
		<tr>
            ...
            <th align="right"><? echo number_format($tot_booking_qnty,6,'.','');?></th>
             <th align="right"><? //echo number_format($tot_booking_amount,2,'.','');?></th>
            <th align="right"><? echo number_format($tot_booking_amount,6,'.','');?></th>
            <th align="right"><? echo number_format($tot_pi_qty,6,'.','');?></th>
            <th align="right"><? //echo number_format($tot_amount,2,'.','');?></th>
			<th align="right"><? echo number_format($tot_pi_amount,6,'.','');?></th>
          
            <th align="right"><? echo number_format($tot_ex_balanceQty,6,'.','');?></th>
            <th align="right"><? echo number_format($tot_ex_balanceAmt,6,'.','');?></th>
            <th align="right"><? //echo number_format($tot_amount,2,'.','');?></th>
            <th align="right"><? //echo number_format($issue_total,2,'.','');?></th>
            <th align="right"><? //echo number_format($issue_total,2,'.','');?></th>
		</tr>
		  ...
		  from inv_transaction a, order_wise_pro_details b,inv_item_transfer_mst c where a.id=b.trans_id and c.id=a.mst_id and a.status_active=1 and a.is_deleted=0 and c.transfer_criteria!=2 and b.status_active=1 and b.is_deleted=0 and b.entry_form in(258) and b.po_breakdown_id in ($po_id) and a.item_category=3 and a.transaction_type in(5) and b.trans_type in(5) ";
			...
			foreach ($result_in as $row) {
				...
				<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>', '<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
					...
					<td align="right" title="Avg  Rate: <? //echo $avg_rate; ?>"><? echo number_format($row[csf('cons_amount')]/$g_exchange_rate, 6); ?> </td>
				</tr>
				<?
			...
			<tr style="font-weight:bold">
				...
				<td align="right"><? echo number_format($total_trans_in_amt, 6); ?></td>
			</tr>
			<thead>
				<tr>
		  ...
		  from inv_transaction a, order_wise_pro_details b,inv_item_transfer_mst c where a.id=b.trans_id and c.id=a.mst_id and a.status_active=1 and a.is_deleted=0 and c.transfer_criteria!=2 and b.status_active=1 and b.is_deleted=0 and b.entry_form in(258) and b.po_breakdown_id in ($po_id) and a.item_category=3 and a.transaction_type in(6) and b.trans_type in(6) ";
			...
			<tr style="font-weight:bold">
				...
				<td align="right"><? echo number_format($total_trans_out_qnty, 2); ?></td>
				 <td align="right"><? echo number_format($total_trans_out_amt, 6); ?></td>
			</tr>
			<tfoot>
			<th></th>
			<th></th>
			...
			<th align="right"><? echo number_format($total_trans_in_qnty - $total_trans_out_qnty, 2); ?></th>
			 <th align="right"><? echo number_format($total_trans_in_amt - $total_trans_out_amt, 6); ?></th>
			</tfoot>
		</table>
	</div>
	</fieldset>
			...
			foreach($sql_std_para as $row )
			{
                ...
                foreach($result_job as $row)
                {
                    ...
                    <tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
                        ...
                        <td align="right" width="80"><? echo number_format($row[csf('available_min')],6,".",""); ?></td>
                        <td align="right" width="80"><? echo $row[csf('produce_min')]; ?>&nbsp;</td>
                        <td align="right" width="60"  title="Produced Min/Used  Min*100"><? echo number_format(($row[csf('produce_min')]/$row[csf('available_min')])*100,6);//$job_effi_array[$row[csf('job_no')]]; ?>&nbsp;</td>
                        <td align="right" width="60"><? echo $cost_per_minute; ?>&nbsp;</td>
                        <td align="right" title="Used Min*CPM/Exchange Rate">
                            <?
                                echo number_format($cm_cost,6);
                            ?>
                        </td>
                    </tr>
                <?
                ...
                ?>
                   <tr bgcolor="#CCCCCC">
                    ...
                    <th width="180">&nbsp;</th>
                    <th width="110">&nbsp;</th>
					<th width="80"><?  echo number_format($tot_used_min,6); ?></th>
                    <th width="80"><?  echo number_format($tot_produce_min,6); ?></th>
                   
                   <th width="60"><?  echo number_format((($tot_produce_min/$tot_used_min)*100),6); ?></th>
                    <th width="60">&nbsp;</th>
                    <th align="right" style="color:#FFFFFF"> <?  echo number_format($total_cm,6); ?></th>
                </tr>
