...
{ 
	...
	//if(!empty($txt_style_ref)) $style_ref_cond=" and a.style_ref_no like '%$txt_style_ref%'";else $style_ref_cond="";
	...
	if(str_replace("'","",$txt_date_from)!="" && str_replace("'","",$txt_date_to)!="")
	{
		...
		if($cbo_date_type==2){
			...
			$ex_cond3=" and b.id=d.po_break_down_id";
	...
	/*$sql_style=sql_select("SELECT a.id from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.status_active=1 and a.is_deleted=0 and b.is_deleted=0 $date_cond $buyer_id_cond $style_ref_cond  $po_id_cond ");
	$job_id=array();
	unset($po_id_cond);
	foreach ($sql_style as $v) 
	{
		array_push($job_id, $v[csf('id')]);
	}
	unset($sql_style);

	$job_id_cond= where_con_using_array($job_id,0,"a.id");
	unset($job_id);*/
    	...
    	<div style="width:2740px; max-height:400px; overflow-y:scroll" id="scroll_body">
            ...
            <?
				...
				foreach($result as $row)
				{
					...
					$style_wise_data[$row[csf('style_ref_no')]]['styleQtyPcs']+=$row[csf('poqtypcs')];
				...
				foreach ($style_wise_data as $style_ref_no => $val) 
				{
					...
					$marginVal=$val['styleVal']-$val['actual_total_cost_per_cost'];
                	...
                	<tr bgcolor="<?=$bgcolor;?>" onClick="change_color('tr_<?=$i; ?>','<?=$bgcolor; ?>');" id="tr_<?=$i; ?>" style="font-size:12px">
                        ...
                        <td width="180" style="word-break:break-all;" align="center"><p><?=chop($val['pono'],","); ?>&nbsp;</p><!--<a href="##" onClick="generate_po_popup('po_popup','<?//=$po_id; ?>','<?//=chop($val['pono'],","); ?>','650px')"></a>--></td>
                        ...
                        <td width="100" style="word-break:break-all;" align="right"><a href="##" onClick="generate_ex_factory_popup('ex_factory_popup','<?=$job_no_prefix_num;?>','<?=$po_id; ?>','650px')"><?=number_format($val['ex_factory_qty'],0,'.',''); ?></a></td>
                        ...
                        <td width="80" title="<? echo $val['budget_oparational_cost_per_pcs'];?>" style="word-break:break-all;" align="right"><a href="##" onClick="generate_budget_op_cost_popup('budget_oparational_cost_popup','<?=$val['styleQtyPcs'];?>','<?=$val['id']; ?>','650px')"><?=number_format($val['budget_oparational_cost_per_pcs'],2,'.','');
						//$tot_budget_cost=$val['budget_fabrics_cost_per_pcs']+$val['budget_accessories_cost_per_pcs']+$val['budget_embellishment_cost']+$val['budget_washing_cost']+$val['budget_cm_per_pcs']+$val['budget_oparational_cost_per_pcs'];
                        ...
                        <td width="80" title="Opt.Cost=<? echo $val['actual_oparational_cost_per_pcs'];?>" style="word-break:break-all;" align="right"><a href="##" onClick="generate_actual_op_cost_popup('actual_oparational_cost_popup','<?=$val['styleQtyPcs'];?>','<?= $ac_id; ?>','650px')"><?=number_format($val['actual_oparational_cost_per_pcs'],2,'.',''); ?></a></td>
                        ...
                        <td width="80" style="word-break:break-all;" align="right" title="<?=($val['ex_factory_qty']*$val['unit_price']).'-'.$val['actual_total_cost_per_cost']; ?>"><?=number_format(($val['ex_factory_qty']*$val['unit_price'])-$val['actual_total_cost_per_cost'],2,'.','');?></td>
                ...
                <thead>
					...
					<tr>
					   ...
					   $total=$all_other_data[0][csf("commercial_amount")]+$all_other_data[0][csf("lab_test")]+$all_other_data[0][csf("inspection")]+$all_other_data[0][csf("freight")]+$all_other_data[0][csf("currier_pre_cost")]+$all_other_data[0][csf("design_cost")];
					    echo $all_other_data[0][csf("commercial_amount")]*$job_qty; ?></td>
					</tr>
					<tr>
					   <td width="120">Lab Test</td>
					   <td width="90"><? echo $all_other_data[0][csf("lab_test")]*$job_qty; ?></td>
					</tr>
					<tr>
					   <td width="120">Inspection Cost</td>
					   <td width="90"><? echo $all_other_data[0][csf("inspection")]*$job_qty; ?></td>
					</tr>
					<tr>
					   <td width="120">Freight Cost</td>
					   <td width="90"><? echo $all_other_data[0][csf("freight")]*$job_qty; ?></td>
					</tr>
					<tr>
					   <td width="120">Courier Cost</td>
					   <td width="90"><? echo $all_other_data[0][csf("currier_pre_cost")]*$job_qty; ?></td>
					</tr>
					<tr>
					   <td width="120">Design Cost</td>
					   <td width="90"><? echo $all_other_data[0][csf("design_cost")]*$job_qty; ?></td>
					</tr>
					<tr>
					   <td width="120"><b>Total</b></td>
					   <td width="90"><b><? echo $total*$job_qty; ?></b></td>
					</tr>
            </table>
        </div>
        ...
        <div class="form_caption" align="center"><strong>Oparational Cost</strong></div><br />
            ...
            <table cellpadding="0" width="100%" class="rpt_table" rules="all" border="1">
				...
				<tbody>
					<tr>
					   <td width="120">Commercial Cost</td>
					   <td width="90"><?  echo $comm_cost; ?></td>
					</tr>
					<tr>
					   <td width="120">Lab Test</td>
					   <td width="90"><? echo $labtest_other_data[0][csf("amnt")]; ?></td>
					</tr>
					<tr>
					   <td width="120">Inspection Cost</td>
					...
					<tr>
					   <td width="120">Freight Cost</td>
					   <td width="90"><? echo $freight_cost; ?></td>
					</tr>
					<tr>
					   <td width="120">Courier Cost</td>
					...
					<tr>
					   <td width="120"><b>Total</b></td>
					   <td width="90"><b><? echo $comm_cost+$design_cost+$currier_pre_cost+$freight_cost+$inspection+$labtest_other_data[0][csf("amnt")]; ?></b></td>
					</tr>
					</tbody>
            </table>
        </div>
