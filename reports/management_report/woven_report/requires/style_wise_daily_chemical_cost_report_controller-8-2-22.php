...
if($action=="report_generate")
{
	...
	if($reporttype==1) //Budget Button
	{
			...
			//echo $sql; die;
				...
				$sql_wash_batch="select a.id,a.extention_no,a.batch_no,a.color_id,b.roll_no as batch_qnty,b.po_id,c.issue_date,c.req_no,b.buyer_style_ref from pro_batch_create_mst a,pro_batch_create_dtls b,inv_issue_master c,pro_recipe_entry_mst d where  a.id=b.mst_id and c.sub_order_id=b.po_id and d.batch_id=a.id and b.mst_id=d.batch_id and d.entry_form=300 and to_char(d.id)=c.lap_dip_no  and a.entry_form=316  and c.entry_form=298 and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 ".where_con_using_array($wash_po_id_arr,0,'b.po_id')."   order  by a.id";
