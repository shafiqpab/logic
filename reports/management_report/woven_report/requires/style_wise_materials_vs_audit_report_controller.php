...
if($action=="report_generate")
{
	...
	//$shiping_status=1;$shiping_status_id="";
		...
		$job_no_cond_in=where_con_using_array($JobNoArr,0,'job_no');
			...
			//for test
				...
				/*$sql = "select a.id as mst_id, a.booking_type, a.is_short, a.booking_no_prefix_num, a.booking_no, a.booking_date, a.buyer_id, a.supplier_id,rtrim(xmlagg(xmlelement(e,c.id,',').extract('//text()') order by c.id).GetClobVal(),',') AS dtls_id , 1 as type
					...
					foreach($grp_data_array as $row)
					{
						...
						if($piQtyArr[$job_no][$alls_data]['qty']!=""){
	...
	$sql_pi_lc = "select b.work_order_no,b.determination_id,a.lc_number from com_btb_lc_master_details a, com_pi_item_details b, com_btb_lc_pi c where a.id=c.com_btb_lc_master_details_id  and b.pi_id=c.pi_id  and b.item_category_id in(2,3) and b.status_active=1 and b.is_deleted=0 $booking_cond_in ";//wo_id_arr
	$PiLcSQLresult = sql_select($sql_pi_lc);

	foreach ($PiLcSQLresult as $row) 
	{
		$job_no=$booking_NoArr[$row[csf('work_order_no')]];
		$PiArr[$job_no][$row[csf('determination_id')]]['lc_number'] .= $row[csf('lc_number')].',';
	}
	unset($PiLcSQLresult);
			...
			foreach($pre_fab_result as $row)
			{
				...
				$style_desc_arr[$job_no][$fab_desc]['lc_no'].=$PiArr[$job_no][$row[csf('deter_min_id')]]['lc_number'].',';
			...
			$pre_fab_total_arr="select  b.costing_per,b.id, b.job_no,b.body_part_id,b.lib_yarn_count_deter_id as deter_min_id, b.fab_nature_id, b.color_type_id, b.fabric_description as fab_desc,b.uom,b.avg_cons,b.avg_cons_yarn, b.avg_process_loss,b.construction,b.composition,b.fabric_source,b.gsm_weight, b.rate,b.amount,b.avg_finish_cons,b.sourcing_rate,b.sourcing_amount,b.sourcing_nominated_supp from wo_pre_cost_fabric_cost_dtls  b where  b.status_active=1 and b.is_deleted=0 $job_id_cond_in order by b.id";
			$pre_fab_total_result=sql_select($pre_fab_total_arr);
			foreach($pre_fab_total_result as $row)
			{
				...
				$fab_req_amount=$fabric_amount_arr['knit']['grey'][$row[csf("id")]][$row[csf("uom")]]+$fabric_amount_arr['woven']['grey'][$row[csf("id")]][$row[csf("uom")]];
				$fab_req_sourcing_amount=$fab_req_qty*$row[csf('sourcing_rate')];
				$fabric_total="Fabric Total";
				$style_desc_arr[$job_no][$fabric_total]['type']='fabric_total';
				$style_desc_arr[$job_no][$fabric_total]['req_qty']+=$fab_req_qty;
				$style_desc_arr[$job_no][$fabric_total]['req_amount']+=$fab_req_amount;
				$style_desc_arr[$job_no][$fabric_total]['cons']+=$row[csf('avg_cons')];
				$style_desc_arr[$job_no][$fabric_total]['amount']+=$row[csf('amount')];
				$style_desc_arr[$job_no][$fabric_total]['sourcing_req_amount']+=$fab_req_sourcing_amount;
				$style_desc_arr[$job_no][$fabric_total]['sourcing_amount']+=$row[csf('sourcing_amount')];
				$style_desc_arr[$job_no][$fabric_total]['sourcing_rate']=$row[csf('sourcing_rate')];
				$style_desc_arr[$job_no][$fabric_total]['pi_qty']=$pi_style_desc_arr[$job_no][$fab_desc]['fab_pi_qty'];
				$style_desc_arr[$job_no][$fabric_total]['pi_amt']=$pi_style_desc_arr[$job_no][$fab_desc]['fab_pi_amt'];
				$style_desc_arr[$job_no][$fabric_total]['minus_req_qty']+=$fab_req_qty;
				$style_desc_arr[$job_no][$fabric_total]['minus_req_amount']+=$fab_req_amount;
				$style_desc_arr[$job_no][$fabric_total]['minus_cons']+=$row[csf('avg_cons')];
				$style_desc_arr[$job_no][$fabric_total]['minus_amount']+=$row[csf('amount')];
				$style_desc_arr[$job_no][$fabric_total]['minus_sourcing_req_amount']+=$fab_req_sourcing_amount;
				$style_desc_arr[$job_no][$fabric_total]['minus_sourcing_amount']+=$row[csf('sourcing_amount')];
				$style_desc_arr[$job_no][$fabric_total]['minus_sourcing_rate']=$row[csf('sourcing_rate')];
				$style_desc_arr[$job_no][$fabric_total]['minus_pi_qty']=$pi_style_desc_arr[$job_no][$fab_desc]['fab_pi_qty'];
				$style_desc_arr[$job_no][$fabric_total]['minus_pi_amt']=$pi_style_desc_arr[$job_no][$fab_desc]['fab_pi_amt'];
			}
			unset($pre_fab_total_result);
			$wvn_booking_partial_total="select  c.id as booking_id,c.booking_no,c.supplier_id,c.pay_mode,a.fin_fab_qnty,a.grey_fab_qnty,a.rate,a.amount,b.id, b.job_no,b.body_part_id,b.lib_yarn_count_deter_id as deter_min_id,  b.fabric_description as fab_desc,b.uom,b.avg_finish_cons,b.sourcing_rate,b.sourcing_amount,b.sourcing_nominated_supp,b.id as  pre_dtls_id from wo_booking_mst c,wo_booking_dtls a,wo_pre_cost_fabric_cost_dtls  b where b.id=a.pre_cost_fabric_cost_dtls_id and c.booking_no=a.booking_no and a.job_no=b.job_no  and a.booking_type=1 and c.status_active=1 and c.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 $job_id_cond_in order by b.id";
		  $wvn_booking_partial_total_result=sql_select($wvn_booking_partial_total);
		   foreach($wvn_booking_partial_total_result as $row)
			{
				$job_no=$row[csf('job_no')];
				$fabric_total="Fabric Total";
				$style_desc_arr[$job_no][$fabric_total]['fin_fab_qnty']+=$row[csf('grey_fab_qnty')];
				$style_desc_arr[$job_no][$fabric_total]['booking_amount']+=$row[csf('grey_fab_qnty')]*$row[csf('rate')];
				$style_desc_arr[$job_no][$fabric_total]['minus_fin_fab_qnty']+=$row[csf('grey_fab_qnty')];
				$style_desc_arr[$job_no][$fabric_total]['minus_booking_amount']+=$row[csf('grey_fab_qnty')]*$row[csf('rate')];
			}
			unset($wvn_booking_partial_total_result);
		...
		$trim_po_cond_in=where_con_using_array($poIdArr,0,'b.order_id');

		$trim_sql_pi = "select a.id as pi_mst_id,b.id as pi_dtls_id,a.item_category_id as cat_id,a.pi_number,b.work_order_no,b.determination_id,(b.quantity) as qty,b.rate,b.amount,b.item_group,b.item_description,b.work_order_dtls_id from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id  and a.pi_basis_id=1  and b.item_category_id in(4) and b.status_active=1 and b.is_deleted=0  $trim_booking_cond_in $trim_po_cond_in";//wo_id_arr
		...
		foreach ($TrimiSQLresult as $row) {
			...
			$PiArr2[$job_no][$item_id]['pi_trim_dtls_id'] .= $row[csf('pi_dtls_id')].',';
		...
		$trim_sql_pi_lc = "select b.work_order_no,a.lc_number,b.item_group,b.work_order_dtls_id from com_btb_lc_master_details a, com_pi_item_details b, com_btb_lc_pi c where a.id=c.com_btb_lc_master_details_id  and b.pi_id=c.pi_id  and b.item_category_id in(4) and b.status_active=1 and b.is_deleted=0 $trim_booking_cond_in $trim_po_cond_in group by b.work_order_no,a.lc_number,b.item_group,b.work_order_dtls_id";//wo_id_arr
		$TrimPiLcSQLresult = sql_select($trim_sql_pi_lc);

		foreach ($TrimPiLcSQLresult as $row) 
		{
			$job_no=$trim_booking_NoArr[$row[csf('work_order_no')]];
			$trim_name=$trim_groupArr[$row[csf('item_group')]];
			$pi_description=$trim_desc_booking_idArr[$job_no][$row[csf('work_order_dtls_id')]];//$row[csf('item_description')];
			...
			if($pi_description!="") $descriptionCond=", ".$pi_description; else $descriptionCond="";
			$item_id=$trim_name.$descriptionCond;
			$PiArr2[$job_no][$item_id]['lc_number'] .= $row[csf('lc_number')].',';
		}
		/* echo "<pre>";
		print_r($PiArr22);die; */
		unset($TrimPiLcSQLresult); 
		...
		foreach($pre_trim_result as $row)
		{
			...
			$pimstid=rtrim($PiArr2[$job_no][$item_id]['pi_mst_id'],',');
			$pi_trim_dtls_id=rtrim($PiArr2[$job_no][$item_id]['pi_trim_dtls_id'],',');
			...
			$style_desc_arr[$job_no][$item_id]['lc_no'].=$PiArr2[$job_no][$item_id]['lc_number'].',';
			$style_desc_arr[$job_no][$item_id]['pi_mst_id'].=$pimstid.',';
			$style_desc_arr[$job_no][$item_id]['pi_trim_dtls_id'].=$pi_trim_dtls_id.',';//$PiArr2[$job_no][$item_id]['pi_trim_dtls_id']
		...
		unset($pre_trim_result);
		
			$pre_trim_sql_total="select b.seq,b.id,c.trim_type,c.item_name,b.description,b.job_no,b.trim_group,b.tot_cons,b.ex_per,b.cons_dzn_gmts,b.cons_uom as uom, b.rate,b.amount,b.sourcing_rate,b.sourcing_amount,b.sourcing_nominated_supp from wo_pre_cost_trim_cost_dtls b,lib_item_group c where  c.id=b.trim_group  and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0  and c.trim_type in(1,2) $job_id_cond_in order by b.seq";
			//and c.trim_type in(1,2)
			$pre_trim_total_result=sql_select($pre_trim_sql_total);
			
			$p_sew_trim_precost_arr=$p_fin_trim_precost_arr=array();
			foreach($pre_trim_total_result as $row)
			{
				$job_no=$row[csf('job_no')];
				$description=$row[csf('description')];
				if($description=='0' || $description=='') 
				...
				$trim_req_sourcing_amount=$trim_req_qty*$row[csf('sourcing_rate')];
				$trim_req_amount=$trim_amount_arr[$row[csf('id')]];
				$trims_total="Trims Total";
				$style_desc_arr[$job_no][$trims_total]['type']='trim_total';
				$style_desc_arr[$job_no][$trims_total]['req_qty']+=$trim_req_qty;
				$style_desc_arr[$job_no][$trims_total]['minus_req_qty']+=$trim_req_qty;
				$style_desc_arr[$job_no][$trims_total]['req_amount']+=$trim_req_amount;
				$style_desc_arr[$job_no][$trims_total]['minus_req_amount']+=$trim_req_amount;
				$style_desc_arr[$job_no][$trims_total]['sourcing_req_amount']+=$trim_req_sourcing_amount;
				$style_desc_arr[$job_no][$trims_total]['minus_sourcing_req_amount']+=$trim_req_sourcing_amount;
				$style_desc_arr[$job_no][$trims_total]['cons']+=$row[csf('cons_dzn_gmts')];
				$style_desc_arr[$job_no][$trims_total]['minus_cons']+=$row[csf('cons_dzn_gmts')];
				$style_desc_arr[$job_no][$trims_total]['trims_tot_cons']+=$row[csf('cons_dzn_gmts')];
				$style_desc_arr[$job_no][$trims_total]['minus_trims_tot_cons']+=$row[csf('cons_dzn_gmts')];
				$style_desc_arr[$job_no][$trims_total]['amount']+=$row[csf('amount')];
				$style_desc_arr[$job_no][$trims_total]['minus_amount']+=$row[csf('amount')];
				$style_desc_arr[$job_no][$trims_total]['sourcing_amount']+=$row[csf('sourcing_amount')];
				$style_desc_arr[$job_no][$trims_total]['minus_sourcing_amount']+=$row[csf('sourcing_amount')];
				$style_desc_arr[$job_no][$trims_total]['pi_qty']=$PiArr2[$job_no][$item_id]['pi_qty'];
				$style_desc_arr[$job_no][$trims_total]['minus_pi_qty']=$PiArr2[$job_no][$item_id]['pi_qty'];
				$style_desc_arr[$job_no][$trims_total]['minus_pi_amt']=$PiArr2[$job_no][$item_id]['pi_amt'];
			}
			unset($pre_trim_total_result);
			$booking_trim_total_sql="select b.job_no,SUM(a.amount) as amount,SUM(a.wo_qnty) as wo_qnty from wo_booking_mst d,wo_booking_dtls a,wo_pre_cost_trim_cost_dtls b,lib_item_group c where  b.id=a.pre_cost_fabric_cost_dtls_id and d.booking_no=a.booking_no and c.id=b.trim_group  and b.job_no=a.job_no  and a.booking_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and c.trim_type in(1,2) $job_id_cond_in group by b.job_no ,b.seq order by b.seq";
			//and c.trim_type in(1,2)
		   $booking_trim_total_result=sql_select($booking_trim_total_sql);
		   foreach($booking_trim_total_result as $row)
		   {
			   $job_no=$row[csf('job_no')];
			   $trims_total="Trims Total";
			   $style_desc_arr[$job_no][$trims_total]['fin_fab_qnty']+=$row[csf('wo_qnty')];
			   $style_desc_arr[$job_no][$trims_total]['booking_amount']+=$row[csf('amount')];
			   $style_desc_arr[$job_no][$trims_total]['minus_fin_fab_qnty']+=$row[csf('wo_qnty')];
			   $style_desc_arr[$job_no][$trims_total]['minus_booking_amount']+=$row[csf('amount')];
		   }
		   	unset($booking_trim_total_result); 
			//////////=========Emblishment====//////////////
	...
	//print_r($PiArr3);
		...
		$emb_sql_pi_lc = "select b.work_order_no,b.determination_id,a.lc_number,b.embell_name,b.embell_type from com_btb_lc_master_details a, com_pi_item_details b, com_btb_lc_pi c where a.id=c.com_btb_lc_master_details_id  and b.pi_id=c.pi_id  and b.item_category_id in(25) and b.status_active=1 and b.is_deleted=0 $embl_booking_cond_in ";//wo_id_arr
		$EmbPiLcSQLresult = sql_select($emb_sql_pi_lc);

		foreach ($EmbPiLcSQLresult as $row) 
		{
			$job_no=$embl_trim_booking_NoArr[$row[csf('work_order_no')]];
			$emb_name_id=$row[csf('embell_name')];
			$emb_type=$row[csf('embell_type')];
			//$job_no=$row[csf('job_no')];
			 ...
			 if($emb_name_id==1) //Print type
				...
				$PiArr2[$job_no][$emb_name]['lc_number'] .= $row[csf('lc_number')].',';
		}
		unset($EmbPiLcSQLresult);
			...
			foreach($pre_wash_result as $row)
			{
				...
				$style_desc_arr[$job_no][$emb_name]['lc_no'].=$PiArr2[$job_no][$emb_name]['lc_number'].',';
			...
			$sql_labtest_booking = "select a.id as booking_dtls_id,a.job_no,a.wo_with_vat_value,a.qty_breakdown,b.labtest_no,b.supplier_id,b.pay_mode from wo_labtest_dtls a, wo_labtest_mst b  where a.mst_id=b.id and a.status_active=1 and b.status_active=1 and a.entry_form=274 and a.job_no in('".$jobNumber."')";
			$result_labtest_booking=sql_select($sql_labtest_booking);
			$prev_amt_breakdown=$prev_qty_breakdown=0;
						
			foreach( $result_labtest_booking as $row )
			{
				$job_no=$row[csf('job_no')];
				$lab_desc_booking='Test Charge';
				$supplier_id=$row[csf('supplier_id')];
				$pay_modeId=$row[csf('pay_mode')];
				if($pay_modeId==3 || $pay_modeId==5)
				{
				...
				foreach ($test_item_qty_break as $value)
				{
					$value_ref=explode("_",$value);
					$prev_qty_breakdown+=$value_ref[1];
					$prev_amt_breakdown+=$value_ref[2];
				}
					$style_desc_arr[$job_no][$lab_desc_booking]['type']='test_charge';
					$style_desc_arr[$job_no][$lab_desc_booking]['booking_amount']+=$row[csf('wo_with_vat_value')];
					$style_desc_arr[$job_no][$lab_desc_booking]['fin_fab_qnty']+=$prev_qty_breakdown;
					$style_desc_arr[$job_no][$lab_desc_booking]['booking_supplier'].=$booking_supplier.',';
					$style_desc_arr[$job_no][$lab_desc_booking]['prev_amount']+=$prev_amt_breakdown;
					$style_desc_arr[$job_no][$lab_desc_booking]['trim_booking_no']=$row[csf('labtest_no')];
					$test_booking_idArr[$row[csf('booking_dtls_id')]]=$row[csf('booking_dtls_id')];
					$test_trim_booking_NoArr[$row[csf('labtest_no')]]=$row[csf('job_no')];
				
			}
			unset($result_labtest_booking);

		$test_booking_cond_in=where_con_using_array($test_booking_idArr,0,'b.work_order_dtls_id');

		$test_sql_pi = "select a.id as pi_mst_id,a.pi_number,b.work_order_no,b.work_order_dtls_id,(b.quantity) as qty,b.rate,b.amount from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id  and a.pi_basis_id=1  and b.item_category_id in(31) and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 $test_booking_cond_in ";//wo_id_arr
		
		$TestSQLresult = sql_select($test_sql_pi);
		 
		foreach ($TestSQLresult as $row) {
			$job_no=$test_trim_booking_NoArr[$row[csf('work_order_no')]];
			$lab_desc_pi='Test Charge';

			$PiArr2[$job_no][$lab_desc_pi]['pi_number'] .= $row[csf('pi_number')].',';
			$PiArr2[$job_no][$lab_desc_pi]['pi_mst_id'] .= $row[csf('pi_mst_id')].',';
			$PiArr2[$job_no][$lab_desc_pi]['pi_qty'] += $row[csf('qty')];
			$PiArr2[$job_no][$lab_desc_pi]['pi_amt'] += $row[csf('amount')];
			
		}
		unset($TestSQLresult);

		$test_sql_pi_lc = "select b.work_order_no,b.determination_id,a.lc_number,b.embell_name,b.embell_type from com_btb_lc_master_details a, com_pi_item_details b, com_btb_lc_pi c where a.id=c.com_btb_lc_master_details_id  and b.pi_id=c.pi_id  and b.item_category_id in(31) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $test_booking_cond_in ";//wo_id_arr
		$TestPiLcSQLresult = sql_select($test_sql_pi_lc);

		foreach ($TestPiLcSQLresult as $row) 
		{
			$job_no=$test_trim_booking_NoArr[$row[csf('work_order_no')]];
			$lab_desc_pi='Test Charge';
			$PiArr2[$job_no][$lab_desc_pi]['lc_number'] .= $row[csf('lc_number')].',';
		}
		unset($TestPiLcSQLresult);

		$sql_inspection_booking = "select id as booking_dtls_id,job_no,wo_qnty,net_amount,booking_no from wo_inspection_dtls  where status_active=1 and entry_form=605 $job_id_cond_in2";
		$result_inspection_booking=sql_select($sql_inspection_booking);
		$prev_amt_breakdown=$prev_qty_breakdown=0;
					
		foreach( $result_inspection_booking as $row )
		{
			$job_no=$row[csf('job_no')];
			$inspection_desc_booking='Inspection Charge';

			$style_desc_arr[$job_no][$inspection_desc_booking]['type']='inspection_charge';
			$style_desc_arr[$job_no][$inspection_desc_booking]['booking_amount']+=$row[csf('net_amount')];
			$style_desc_arr[$job_no][$inspection_desc_booking]['fin_fab_qnty']+=$row[csf('wo_qnty')];
			$style_desc_arr[$job_no][$inspection_desc_booking]['trim_booking_no']=$row[csf('booking_no')];
			$inspection_booking_idArr[$row[csf('booking_dtls_id')]]=$row[csf('booking_dtls_id')];
			$inspection_booking_NoArr[$row[csf('booking_no')]]=$row[csf('job_no')];
		}
		unset($result_inspection_booking);

		$sql_inspection_supplier = "select a.supplier_id,a.pay_mode,b.job_no from wo_inspection_mst a,wo_inspection_dtls b where a.id=b.booking_mst_id and a.status_active=1 and a.status_active=1  and a.entry_form=605 $job_id_cond_in";
		$result_inspection_supplier=sql_select($sql_inspection_supplier);
		foreach($result_inspection_supplier as $row )
		{
			$job_no=$row[csf('job_no')];
			$supplier_id=$row[csf('supplier_id')];
			$pay_modeId=$row[csf('pay_mode')];
			...
			$style_desc_arr[$job_no][$inspection_desc_booking]['booking_supplier'].=$booking_supplier.',';
		}
		unset($result_inspection_supplier);

		$ins_booking_cond_in=where_con_using_array($inspection_booking_idArr,0,'b.work_order_dtls_id');

		$inspection_sql_pi = "select a.id as pi_mst_id,a.pi_number,b.work_order_no,b.work_order_dtls_id,(b.quantity) as qty,b.rate,b.amount from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id  and a.pi_basis_id=1  and b.item_category_id in(115) and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0 $ins_booking_cond_in ";//wo_id_arr
		$InspectionSQLresult = sql_select($inspection_sql_pi);
		 
		foreach ($InspectionSQLresult as $row) {
			$job_no=$inspection_booking_NoArr[$row[csf('work_order_no')]];
			$inspection_desc_pi='Inspection Charge';

			$PiArr2[$job_no][$inspection_desc_pi]['pi_number'] .= $row[csf('pi_number')].',';
			$PiArr2[$job_no][$inspection_desc_pi]['pi_mst_id'] .= $row[csf('pi_mst_id')].',';
			$PiArr2[$job_no][$inspection_desc_pi]['pi_qty'] += $row[csf('qty')];
			$PiArr2[$job_no][$inspection_desc_pi]['pi_amt'] += $row[csf('amount')];			
		}
		unset($InspectionSQLresult);

		$test_sql_pi_lc = "select b.work_order_no,a.lc_number from com_btb_lc_master_details a, com_pi_item_details b, com_btb_lc_pi c where a.id=c.com_btb_lc_master_details_id  and b.pi_id=c.pi_id  and b.item_category_id in(115) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $ins_booking_cond_in ";//wo_id_arr
		$TestPiLcSQLresult = sql_select($test_sql_pi_lc);
		...
		foreach ($TestPiLcSQLresult as $row) 
		{
			$job_no=$inspection_booking_NoArr[$row[csf('work_order_no')]];
			$inspection_desc_pi='Inspection Charge';
			$PiArr2[$job_no][$inspection_desc_pi]['lc_number'] .= $row[csf('lc_number')].',';
		}
		...
		unset($TestPiLcSQLresult);
			 ...
			 foreach( $pre_other_result as $row )
			{
				...
				$style_desc_arr[$job_no][$lab_desc]['type']='test_charge';
				...
				$style_desc_arr[$job_no][$lab_desc]['pi_qty']=$PiArr2[$job_no][$lab_desc]['pi_qty'];
				$style_desc_arr[$job_no][$lab_desc]['pi_amt']=$PiArr2[$job_no][$lab_desc]['pi_amt'];
				$style_desc_arr[$job_no][$lab_desc]['pi_no'].=$PiArr2[$job_no][$lab_desc]['pi_number'].',';
				$style_desc_arr[$job_no][$lab_desc]['lc_no'].=$PiArr2[$job_no][$lab_desc]['lc_number'].',';
				$style_desc_arr[$job_no][$lab_desc]['pi_mst_id'].=$PiArr2[$job_no][$lab_desc]['pi_mst_id'].',';

				$style_desc_arr[$job_no][$inspection_desc]['type']='inspection_charge';
				...
				$style_desc_arr[$job_no][$inspection_desc]['pi_qty']=$PiArr2[$job_no][$inspection_desc]['pi_qty'];
				$style_desc_arr[$job_no][$inspection_desc]['pi_amt']=$PiArr2[$job_no][$inspection_desc]['pi_amt'];
				$style_desc_arr[$job_no][$inspection_desc]['pi_no'].=$PiArr2[$job_no][$inspection_desc]['pi_number'].',';
				$style_desc_arr[$job_no][$inspection_desc]['lc_no'].=$PiArr2[$job_no][$inspection_desc]['lc_number'].',';
				$style_desc_arr[$job_no][$inspection_desc]['pi_mst_id'].=$PiArr2[$job_no][$inspection_desc]['pi_mst_id'].',';
		...
		foreach($style_desc_arr as $job_key=>$job_data)
		{
			 ...
			 foreach($job_data as $desc_key=>$row)
			{
					...
					else if($typeId=='fabric_total') $color_msg="#D3D3D3";
					else if($typeId=='trim') $color_msg="#FF9933";
					else if($typeId=='trim_total') $color_msg="#D3D3D3";
					else if($typeId=='embl') $color_msg="#6699CC";
					//else if($typeId=='Other') $color_msg="#FFFFFF";
					else if($typeId=='test_charge') $color_msg="#F5F5DC";
					else if($typeId=='inspection_charge') $color_msg="#DEB887";
				...
				else if($typeId=='trim')
				{
					...
					//$pi_mst_id=rtrim($row[('pi_mst_id')],',');//pi_trim_dtls_id
					//$pi_mst_ids=implode(",",array_unique(explode(",",$pi_mst_id)));
					$pi_trim_dtls_id=rtrim($row[('pi_trim_dtls_id')],',');//pi_trim_dtls_id
					$pi_mst_ids=implode(",",array_unique(explode(",",$pi_trim_dtls_id)));
					//echo $pi_trim_dtls_ids;
				}
				else if($typeId=='embl')
				{
					$trim_booking_no=$style_desc_arr[$job_key][$desc_key]['trim_booking_no'];
				...
				else if($typeId=='inspection_charge')
				{
					$trim_booking_no=$style_desc_arr[$job_key][$desc_key]['trim_booking_no'];
					$pre_dtls_id=rtrim($style_desc_arr[$job_no][$desc_key]['pre_dtls_id'],',');
					$booking_dtls_id=rtrim($style_desc_arr[$job_no][$desc_key]['booking_dtls_id'],',');
					...
					$lc_numbers=implode(", ",array_unique(explode(",",$lc_number)));
		...
		?> 
                ...
                <td width="80" align="right"><p><? echo fn_number_format($row[('amount')],4);; ?></p></td>
				<td width="80" align="right"><p><? echo fn_number_format($row[('req_amount')],4); ?></p></td>
				<td width="80"  title="<? ?>" align="right"><p> <? echo  fn_number_format($row[('sourcing_amount')],4);; ?> 	</p></td>
				
				<td width="80" align="right" title="Sourcing Amount"><p><? echo fn_number_format($row[('sourcing_req_amount')],4); ?></p></td>
				<td width="80" align="right"><p><? echo fn_number_format($row[('cons')],4); ?></p></td>
				
                <td width="80" align="right"><p><? echo fn_number_format($row[('req_qty')],4); ?></p></td>
                <td width="80" align="right"><p><? echo fn_number_format($row[('fin_fab_qnty')],4); ?></p></td>
				<td width="80" align="right" title="Booking amount/FinQty "><p><? if($row[('booking_amount')]) echo fn_number_format($row[('booking_amount')]/$row[('fin_fab_qnty')],4);else echo ""; ?></p></td>
				 
                <td width="80" align="right"><p><a href="##" onClick="report_generate_popup('<? echo $company_name; ?>','<? echo $job_key; ?>','<? echo $typeId; ?>','<? echo $pre_dtls_ids; ?>','<? echo $fab_deter_min_id; ?>','<? echo $all_pre_dtls_ids; ?>','','report_generate_woven',1)"><? echo fn_number_format($row[('booking_amount')],4); ?></a><? //echo number_format($row[('booking_amount')],4); ?></p></td>
				<td width="80" align="right" title="Fab Req-Booking Req Qty"><p><? $fab_bal_qty=$row[('req_qty')]-$row[('fin_fab_qnty')];echo fn_number_format($fab_bal_qty,4); 
				 $fab_value_balance=$row[('sourcing_req_amount')]-$row[('booking_amount')];
				?></p></td>
				<td width="80" align="right" title="Sourcing Value-Booking Value"><p><? 
				echo fn_number_format($fab_value_balance,4); ?></p></td>
				<td width="80" align="right" title=" Pi Qty"><p><a href="##" onClick="report_generate_popup('<? echo $company_name; ?>','<? echo $job_key; ?>','<? echo $typeId; ?>','<? echo $pi_mst_ids; ?>','<? echo $fab_deter_min_id; ?>','<? echo $all_pre_dtls_ids; ?>','<? echo $all_pi_dtls_ids; ?>','report_generate_woven_pi',2)"><? echo fn_number_format($row[('pi_qty')],4); ?></a><? //$fabric_bal_after_cut_qty=$wv_recv_fin-$fabric_used_qty;
				//echo number_format($row[('pi_qty')],4); ?></p></td>
				<td width="80" align="right" title="PI Amount/PI Qty"><p><? if($row[('pi_amt')]) echo fn_number_format($row[('pi_amt')]/$row[('pi_qty')],4); else echo "";?></p></td>
				<td width="80" align="right" title="PI Amount"><p><?  echo fn_number_format($row[('pi_amt')],4);
				 ?></p></td>
				...
				$pi_balance_amount=$row[('booking_amount')]-$row[('pi_amt')];
				 echo fn_number_format($pi_balance_qty,4); ?>   </p></td>
                 
				<td width="80" align="right" title="Booking Amount-PI Amount"><p><? echo fn_number_format($pi_balance_amount,4); ?></p></td>
                <td width="100" align="center"><p style="word-break:break-all"><? echo $pi_numbers; ?></p></td>
				<td width="100" align="center"  title="Booking supplier"><p style="word-break:break-all"><? echo $booking_suppliers; ?></p></td>
				<td width="" align="center"  title="Booking supplier"><p style="word-break:break-all"><? echo $lc_numbers; ?></p></td>
			...
			$tot_pi_value_sum+=$row[('pi_amt')];
			$tot_pi_qty_sum+=$row[('pi_qty')];
			$tot_booking_amount_sum+=$row[('booking_amount')];
			$tot_booking_qty_sum+=$row[('fin_fab_qnty')];
			$tot_budget_cons_dzn_sum+=$row[('cons')];
			$tot_budget_req_qty_sum+=$row[('req_qty')];
			$tot_sourcing_amount_dzn_sum+=$row[('sourcing_amount')];
			$tot_sourcing_req_amount_sum+=$row[('sourcing_req_amount')];
			$tot_budget_amount_dzn_sum+=$row[('amount')];
			$tot_budget_req_amount_sum+=$row[('req_amount')];

			$tot_minus_pi_value+=$row[('minus_pi_amt')];
			$tot_minus_pi_qty+=$row[('minus_pi_qty')];
			$tot_minus_booking_amount+=$row[('minus_booking_amount')];
			$tot_minus_booking_qty+=$row[('minus_fin_fab_qnty')];
			$tot_minus_budget_cons_dzn+=$row[('minus_cons')];
			$tot_minus_budget_req_qty+=$row[('minus_req_qty')];
			$tot_minus_sourcing_amount_dzn+=$row[('minus_sourcing_amount')];
			$tot_minus_sourcing_req_amount+=$row[('minus_sourcing_req_amount')];
			$tot_minus_budget_amount_dzn+=$row[('minus_amount')];
			$tot_minus_budget_req_amount+=$row[('minus_req_amount')];
         ...
         <table class="tbl_bottom" width="<? echo $width_td;?>" id="report_table_footer" cellpadding="0" cellspacing="0" border="1" rules="all">
        	<tr>
                ...
                <td width="150">Total Cost(Without CM)</td>
                 <td width="60"></td>
                <td width="80"><? $tot_budget_amount_dzn=$tot_budget_amount_dzn_sum-$tot_minus_budget_amount_dzn;echo fn_number_format($tot_budget_amount_dzn,4);?></td>
                <td width="80"><? $tot_budget_req_amount=$tot_budget_req_amount_sum-$tot_minus_budget_req_amount;echo fn_number_format($tot_budget_req_amount,4);?></td>
                <td width="80" ><? $tot_sourcing_amount_dzn=$tot_sourcing_amount_dzn_sum-$tot_minus_sourcing_amount_dzn;echo fn_number_format($tot_sourcing_amount_dzn,4);?></td>
                <td width="80" ><? $tot_sourcing_req_amount=$tot_sourcing_req_amount_sum-$tot_minus_sourcing_req_amount;echo fn_number_format($tot_sourcing_req_amount,4);?></td>
                <td width="80"><? //echo number_format($tot_budget_cons_dzn,4);?></td>
                <td width="80"><? $tot_budget_req_qty=$tot_budget_req_qty_sum-$tot_minus_budget_req_qty;echo fn_number_format($tot_budget_req_qty,4);?></td>
                <td width="80"><? $tot_booking_qty=$tot_booking_qty_sum-$tot_minus_booking_qty;echo fn_number_format($tot_booking_qty,4);?></td>
                <td width="80"><? //echo number_format($tot_booking_qty,2);?></td>
                <td width="80"><? $tot_booking_amount=$tot_booking_amount_sum-$tot_minus_booking_amount;echo fn_number_format($tot_booking_amount,4);?></td>
                <td width="80"><? echo fn_number_format($tot_fab_qty_balance,4);?></td>
                <td width="80"><? echo fn_number_format($tot_fab_value_balance,4);?></td>
                <td width="80" ><? $tot_pi_qty=$tot_pi_qty_sum-$tot_minus_pi_qty;echo fn_number_format($tot_pi_qty_sum,4);?></td>
                <td width="80"><? //echo number_format($tot_pi_qty,4);?></td>
                <td width="80"><? $tot_pi_value=$tot_pi_value_sum-$tot_minus_pi_value;echo fn_number_format($tot_pi_value,4);?></td>
                <td width="80"><? echo fn_number_format($tot_pi_balance_qty,4);?></td>
                <td width="80"><? echo fn_number_format($tot_pi_balance_value,4);?></td>
...
if($action=="report_generate_woven")
{
	...
	if($wo_type=='fabric') $caption="Fabric";
	else if($wo_type=='trim') $caption="Trims";
	else if($wo_type=='embl') $caption="Embellishment";
	else if($wo_type=='test_charge') $caption="Lab Test";
	else if($wo_type=='inspection_charge') $caption="Inspection";
	else if($wo_type=='commission') $caption="Commission";
	...
	if($wo_type=='test_charge')
	{
     
	    $sql_booking="select d.labtest_no as booking_no,d.inserted_by, d.insert_date,d.wo_date,d.supplier_id,d.pay_mode,a.amount,a.wo_with_vat_value from wo_labtest_mst d,wo_labtest_dtls a where  d.id=a.mst_id and a.entry_form=274 and a.status_active=1 and a.is_deleted=0  and d.status_active=1 and d.is_deleted=0  and a.job_no ='".$job_no."' ";
		...
		foreach ($result_booking as $row)
		 {
			...
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['wo_qnty']+=$row[csf("wo_with_vat_value")];
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['amount']+=$row[csf("amount")];
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['job_no']=$row[csf("job_no")];
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['item_category']='31';
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['supplier']=$supplier_array[$row[csf("supplier_id")]];
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['booking_type']='';
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['booking_date']=$row[csf("wo_date")];
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['uom']='';
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['inserted_by']=$row[csf("inserted_by")];
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['insert_date']=$row[csf("insert_date")];
		}
	}
	if($wo_type=='inspection_charge')
	{
     
	    $sql_booking="select d.booking_no,d.inserted_by, d.insert_date,d.booking_date,d.supplier_id,d.pay_mode,a.amount,a.wo_qnty from wo_inspection_mst d,wo_inspection_dtls a where  d.id=a.booking_mst_id and a.entry_form=605 and a.status_active=1 and a.is_deleted=0  and d.status_active=1 and d.is_deleted=0  and a.job_no ='".$job_no."' ";
		...
		foreach ($result_booking as $row)
		 {
			...
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['amount']+=$row[csf("amount")];
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['job_no']=$row[csf("job_no")];
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['item_category']='115';
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['supplier']=$supplier_array[$row[csf("supplier_id")]];
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['booking_type']='';
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['booking_date']=$row[csf("booking_date")];
			$wo_booking_arr[$row[csf("booking_no")]][$item_id]['uom']='';
	...
	<table class="rpt_table" border="1" cellpadding="5" cellspacing="2" width="<? echo $width;?>" rules="all">
		<caption> <b><? echo $caption;?>  Summary </b> </caption>
		<thead>
			<th width="20">SL#</th>
