...
if($action=="create_order_no_search_list_view")
{
	...
	$entry_form_cond="and c.entry_from in(111,158,425)";
	if($type==1)
	{
		...
		<table id="table_header_1" class="rpt_table" width="2720" cellpadding="0" cellspacing="0" border="1" rules="all">
			<thead>
				<tr style="font-size:12px">
					...
					<th rowspan="2" title="(Margin Val/FOB Value)*100">Margin %</th>
				</tr>
				<tr style="font-size:12px">
		...
		<div style="width:2740px; max-height:400px; overflow-y:scroll" id="scroll_body">
			<table class="rpt_table" width="2720" cellpadding="0" cellspacing="0" border="1" rules="all" id="table_body">
				...
				foreach($result_sql_exf as $row)
				{
					...
					//echo $commercial.'D';
					$month_year='';
					$month_year=date('m-Y',strtotime($row[csf("ex_factory_date")]));
					//echo "<pre>".$month_year."</pre>";
					if(!in_array('6*'.$month_year."*".$row[csf('po_break_down_id')], $array_actual_cost_po))
					{
						$actualCostArray2[6][$row[csf('po_break_down_id')]]+=$commercial;
						array_push($array_actual_cost_po, '6*'.$month_year."*".$row[csf('po_break_down_id')]);
					}

					if(!in_array('5*'.$month_year."*".$row[csf('po_break_down_id')], $array_actual_cost_po))
					{
						$actualCostArray2[5][$row[csf('po_break_down_id')]]+=$cm_cost;
						array_push($array_actual_cost_po, '5*'.$month_year."*".$row[csf('po_break_down_id')]);
					}
					
					if(!in_array('2*'.$month_year."*".$row[csf('po_break_down_id')], $array_actual_cost_po))
					{
						$actualCostArray2[2][$row[csf('po_break_down_id')]]+=$freight_cost;
						array_push($array_actual_cost_po, '2*'.$month_year."*".$row[csf('po_break_down_id')]);
					}

					if(!in_array('3*'.$month_year."*".$row[csf('po_break_down_id')], $array_actual_cost_po))
					{
						$actualCostArray2[3][$row[csf('po_break_down_id')]]+=$inspection;
						array_push($array_actual_cost_po, '3*'.$month_year."*".$row[csf('po_break_down_id')]);
					}
					
					if(!in_array('4*'.$month_year."*".$row[csf('po_break_down_id')], $array_actual_cost_po))
					{
						$actualCostArray2[4][$row[csf('po_break_down_id')]]+=$currier_pre_cost;
						array_push($array_actual_cost_po, '4*'.$month_year."*".$row[csf('po_break_down_id')]);
					}
					
					if(!in_array('7*'.$month_year."*".$row[csf('po_break_down_id')], $array_actual_cost_po))
					{
						$actualCostArray2[7][$row[csf('po_break_down_id')]]+=$design_cost;
						array_push($array_actual_cost_po, '7*'.$month_year."*".$row[csf('po_break_down_id')]);
					}
					//
				}
			...
			//print_r($style_wise_data);
				foreach ($style_wise_data as $style_ref_no => $val) 
				{
					...
					<td style="word-break:break-all;" align="right" title="<?='('.$marginVal .'/'.$fob.')*100'; ?>"><?=number_format($marginPer,2,'.',''); ?></td>
				</tr>
				<?
				...
				$avgMarginPer=($gmarginVal/$gFob)*100;
				?>
			</table>
		</div>
...
$sql="SELECT a.job_no_prefix_num, a.job_no, $year_field, a.company_name, a.season_buyer_wise as season, a.brand_id, a.season_year, a.buyer_name, a.style_ref_no, a.order_uom, a.gmts_item_id, a.total_set_qnty as ratio, a.set_smv,b.job_id, b.id, b.po_number, b.grouping, b.file_no, b.pub_shipment_date, b.po_quantity, (b.po_quantity*a.total_set_qnty) as poqtypcs, b.plan_cut, b.unit_price, b.po_total_price,b.is_confirmed, b.shiping_status,d.ex_factory_date from wo_po_details_master a, wo_po_break_down b, pro_ex_factory_mst d where a.id=b.job_id and b.id=d.po_break_down_id and d.status_active=1  and a.company_name='$company_name' and b.is_confirmed in(1,2) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  $date_cond3  $buyer_id_cond $date_cond $style_ref_cond $po_id_cond   order by d.ex_factory_date desc";
	...
	foreach($result_2 as $row)
	{
		...
		$style_job_wise_qty[$row[csf("job_no")]]+=$row[csf("poqtypcs")];
		$style_job_wise_val[$row[csf("job_no")]]+=$row[csf("po_total_price")];
		$style_job_po_wise_arr[$row[csf("job_no")]]['poQty']+=$row[csf("poqtypcs")];
		$style_job_po_wise_arr[$row[csf("job_no")]]['poVal']+=$row[csf("po_total_price")];
	...
	{	
		...
		else
		{
			...
			po3 - ex full 
			*/
			//echo $shipId.'='.$row[csf("id")].'<br>';
			if($from_date!="" && $to_date !="")
			{
				if($shipId==3 && ($last_ex_factory_date>=$from_date && $last_ex_factory_date<=$to_date) && $is_partialship_job_count[$row[csf("job_no")]]!=1)
				{
	...
	<fieldset>
		...
		<table id="table_header_1" class="rpt_table" width="<? echo $width;?>" cellpadding="0" cellspacing="0" border="1" rules="all">
			<caption> <b style="color:#FF6600; font-size:20px; float:left">Ex-Factory Date Against Style All PO Close: </b> </caption>
			<thead>
				<tr style="font-size:12px">
					<th rowspan="3" width="30">SL</th>
					<th rowspan="3" width="120">Buyer</th>
					...
					<th rowspan="2" width="80" title="Total Budget Cost-Total Actual Cost">Variance [BOM Vs Actual]</th>
					<th rowspan="2" width="80" title="Ex-Factory Value ($)-Total Actual Cost">Margin Value</th>
					<th rowspan="2" title="(Margin Val/FOB Val)*100">Margin %</th>
				</tr>
				<tr style="font-size:12px">
					<th width="80">Fabrics Cost</th>
					<th width="80">Acce. Cost</th>
		...
		<div style="width:<? echo $width+20;?>px; max-height:400px; overflow-y:scroll" id="scroll_body">
			...
			//	print_r($emblishment_costing_arr_name_wash);
				...
				foreach($result_2 as $row)
				{
					$wash_cost_arr[$row[csf('job_no')]]+=$emblishment_costing_arr_name_wash[$row[csf('id')]][3];
					$print_cost_arr[$row[csf('job_no')]]+=$emblishment_costing_arr_name[$row[csf('id')]][1];
					$embr_cost_arr[$row[csf('job_no')]]+=$emblishment_costing_arr_name[$row[csf('id')]][2];
					$spcial_cost_arr[$row[csf('job_no')]]+=$emblishment_costing_arr_name[$row[csf('id')]][4];
					$other_cost_arr[$row[csf('job_no')]]+=$emblishment_costing_arr_name[$row[csf('id')]][5];
					$trims_amt_arr[$row[csf('job_no')]]+=$trims_costing_arr[$row[csf('id')]];
					$fab_amt=array_sum($fabric_costing_arr['knit']['grey'][$row[csf('id')]])+array_sum($fabric_costing_arr['woven']['grey'][$row[csf('id')]]);
					$fabric_amt_arr[$row[csf('job_no')]]+=$fab_amt;
					$cm_amt_arr[$row[csf('job_no')]]+=$other_costing_arr[$row[csf('id')]]['cm_cost'];
					$commi_local_amt_arr[$row[csf('job_no')]]+=$commission_costing_arr[$row[csf('id')]][1];
					$commi_foreign_amt_arr[$row[csf('job_no')]]+=$commission_costing_arr[$row[csf('id')]][2];
					$commercial_amt_arr[$row[csf('job_no')]]+=$commercial_costing_arr[$row[csf('id')]];
					$lab_test_amt_arr[$row[csf('job_no')]]+=$other_costing_arr[$row[csf('id')]]['lab_test'];
					$freight_amt_arr[$row[csf('job_no')]]+=$other_costing_arr[$row[csf('id')]]['freight'];
					$inspection_amt_arr[$row[csf('job_no')]]+=$other_costing_arr[$row[csf('id')]]['inspection'];
					$certificate_amt_arr[$row[csf('job_no')]]+=$other_costing_arr[$row[csf('id')]]['certificate_pre_cost'];
					$currier_amt_arr[$row[csf('job_no')]]+=$other_costing_arr[$row[csf('id')]]['currier_pre_cost'];
					$design_amt_arr[$row[csf('job_no')]]+=$other_costing_arr[$row[csf('id')]]['design_cost'];
				}
				//emblishment_costing_arr_name[$row[csf('id')]][1]
				//print_r($wash_cost_arr);
				...
				foreach($result_2 as $row)
				{
					...
					if($fullship_job_po_chk_arr[$row[csf("job_no")]] && $shipId==3 && $ex_factoryQty>0)
					{
						...
						$dzn_qnty=$dzn_qnty*$row[csf('ratio')];
						$trims_cost_mkt=$trims_amt_arr[$row[csf('job_no')]];//$trims_costing_arr[$row[csf('id')]];

						$print_amount=$print_cost_arr[$row[csf('job_no')]];//$emblishment_costing_arr_name[$row[csf('id')]][1];
						$embroidery_amount=$embr_cost_arr[$row[csf('job_no')]];//$emblishment_costing_arr_name[$row[csf('id')]][2];
						$special_amount=$spcial_cost_arr[$row[csf('job_no')]];//$emblishment_costing_arr_name[$row[csf('id')]][4];
						$wash_cost=$wash_cost_arr[$row[csf('job_no')]];//$emblishment_costing_arr_name_wash[$row[csf('id')]][3];
					//	echo $wash_cost.'T';
						$other_amount=$other_cost_arr[$row[csf('job_no')]];//$emblishment_costing_arr_name[$row[csf('id')]][5];
						$foreign_cost=$commi_foreign_amt_arr[$row[csf('job_no')]];//$commission_costing_arr[$row[csf('id')]][1];
						$local_cost=$commi_local_amt_arr[$row[csf('job_no')]];//$commission_costing_arr[$row[csf('id')]][2];
						//commi_foreign_amt_arr
	
						$comm_cost_mkt=$commercial_amt_arr[$row[csf('job_no')]];//$commercial_costing_arr[$row[csf('id')]];
	
						$test_cost=$lab_test_amt_arr[$row[csf('job_no')]];//$other_costing_arr[$row[csf('id')]]['lab_test'];
						$freight_cost=$freight_amt_arr[$row[csf('job_no')]];//$other_costing_arr[$row[csf('id')]]['freight'];
						$inspection_cost=$inspection_amt_arr[$row[csf('job_no')]];//$other_costing_arr[$row[csf('id')]]['inspection'];
						$certificate_cost=$certificate_amt_arr[$row[csf('job_no')]];//$other_costing_arr[$row[csf('id')]]['certificate_pre_cost'];
						$currier_cost=$currier_amt_arr[$row[csf('job_no')]];//$other_costing_arr[$row[csf('id')]]['currier_pre_cost'];
						$design_cost=$design_amt_arr[$row[csf('job_no')]];//$other_costing_arr[$row[csf('id')]]['design_cost'];
					...
					//$ship_chk_arr[$row[csf("style_ref_no")]]
						...
						$cm_cost=$cm_amt_arr[$row[csf('job_no')]];//$other_costing_arr[$row[csf('id')]]['cm_cost'];
						$comm_cost_actual=$actualCostArray[6][$row[csf('id')]];
					...
					//echo $comm_cost_actual.'A'.$freight_cost_actual.'='.$inspection_actual.'='.$currier_pre_cost_actual.'='.$design_cost_actua.'='.$lab_test."<br>";
						...
						$cm_cost_mkt=$cm_cost;
						$fabric_purchase_cost_mkt=0;
						$fabric_purchase_cost_mkt=$fabric_amt_arr[$row[csf('job_no')]];//array_sum($fabric_costing_arr['knit']['grey'][$row[csf('id')]])+array_sum($fabric_costing_arr['woven']['grey'][$row[csf('id')]]);
					//$fabric_purchase_cost_mkt=array_sum($fabric_costing_arr['woven']['grey'][$row[csf('id')]]);
					...
					$poQty=$style_job_po_wise_arr[$row[csf("job_no")]]['poQty'];
					$poVal=$style_job_po_wise_arr[$row[csf("job_no")]]['poVal'];
			...
			<div style="width:100%">
				<table cellpadding="0" width="100%" class="rpt_table" rules="all" border="1">
					...
					<tbody>
						...
						<tr>
							<td width="120">Freight Cost</td>
							<td width="90" title="<? echo number_format($freight,2,'.',''); ?>"><? echo $freight; ?></td>
						</tr>
						<tr>
							<td width="120">Courier Cost</td>
							<td width="90"><? echo $currier_pre_cost; ?></td>
