...
if($action=="report_generate")
{
	...
	if(str_replace("'","",$cbo_costcontrol_source)==1)
	{
		...
		//echo "select a.job_no from wo_po_details_master a,wo_pre_cost_mst f where  a.job_no=f.job_no  and a.company_name=$company_name and f.entry_from=158 and a.job_no_prefix_num like '$job_no' $year_cond group by a.job_no";
	...
	/*
		...
		$sql_trim_trans="select c.from_order_id,a.from_prod_id,
		 sum(CASE WHEN b.entry_form in(78) and b.trans_type in(5) THEN  b.quantity ELSE 0 END) AS grey_in,
		 sum(CASE WHEN b.entry_form in(78) and b.trans_type in(6) THEN  b.quantity ELSE 0 END) AS grey_out,
		  sum(CASE WHEN b.entry_form in(78) and b.trans_type in(5) THEN  b.quantity*a.rate ELSE 0 END) AS grey_in_amt,
		  sum(CASE WHEN b.entry_form in(78) and b.trans_type in(6) THEN  b.quantity*a.rate ELSE 0 END) AS grey_out_amt
		  from  inv_item_transfer_mst c, inv_item_transfer_dtls a, order_wise_pro_details b where c.id=a.mst_id and a.id=b.dtls_id and a.status_active=1 and a.is_deleted=0 and c.transfer_criteria!=2 and b.status_active=1 and b.is_deleted=0 and b.entry_form in(78) and b.trans_type in(5,6) and b.po_breakdown_id in(".implode(",",$poIdArr).") group by  a.from_prod_id,c.from_order_id";//
		...
		c.from_order_id
		from inv_item_transfer_dtls a, order_wise_pro_details b,inv_item_transfer_mst c where a.id=b.dtls_id and c.id=a.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and c.transfer_criteria!=2 and b.is_deleted=0 and b.entry_form in(78) and b.trans_type in(5,6) and b.po_breakdown_id in(".implode(",",$poIdArr).") ";
		$result_trim_trans=sql_select( $sql_trim_trans );
		...
		//echo $sql = "select a.item_category, a.exchange_rate, b.grey_fab_qnty, b.wo_qnty, b.fin_fab_qnty, b.po_break_down_id, b.rate, b.amount from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and a.booking_type=6 and b.emblishment_name!=3 and b.po_break_down_id in(".implode(",",$poIdArr).") and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1";
		 $sql_emb = "select a.item_category,a.exchange_rate,b.grey_fab_qnty,b.wo_qnty,b.fin_fab_qnty,b.po_break_down_id,b.rate,b.amount,c.emb_name, c.emb_type from wo_booking_mst a,wo_booking_dtls b,wo_pre_cost_embe_cost_dtls c where a.booking_no=b.booking_no and c.job_no=b.job_no and c.id=b.pre_cost_fabric_cost_dtls_id and a.booking_type=6 and c.emb_name!=3 and b.po_break_down_id in(".implode(",",$poIdArr).") and a.is_deleted=0 and a.status_active=1   and c.is_deleted=0 and c.status_active=1 and b.is_deleted=0 and b.status_active=1";
		...
		<div style="width:1302px; margin:0 auto">
			...
			<table class="rpt_table" border="1" cellpadding="2" cellspacing="2" style="width:1300px; margin-top:10px" rules="all">
				...
				?>
				 <tr class="fbpur" bgcolor="<? echo $fab_bgcolor2;?>" onClick="change_color('trfab_<? echo $f1; ?>','<? echo $fab_bgcolor2;?>')" id="trfab_<? echo $f1; ?>">
                    ...
                    <td width="100" align="right"><? echo number_format(($fabPurArr['pre']['woven']['amount']*100/$jobValue),2).'%'; ?></td>
					...
					<td width="100" align="right" title="SampleBookingQty=<?=$sample_booking_all_qnty?>">
							...
							<a href="#" onClick="openmypage_sample_fab('<? echo implode("_",array_keys($poNumberArr)); ?>','<?=$jobNumber?>')">
								<? echo fn_number_format($fabPurArr['acl']['woven']['qty']+$sample_booking_all_qnty,4) ?>
							</a>
						</td>
                    ...
                    <td width="100" align="right"><? echo number_format(($fabPurArr['acl']['woven']['amount']*100/$jobValue),2).'%'; ?></td>
				</tr>
				<tr style="font-weight:bold;background-color:#CCC">
					...
					<td width="100" align="right">
						<?
						$fabPurMkt= number_format($fabPurArr['mkt']['woven']['amount'],4,".","");
						echo number_format($fabPurMkt,4);
						$GrandTotalMktValue+=number_format($fabPurMkt,4,".","");
						$fabPurPre= number_format($fabPurArr['pre']['woven']['amount'],4,".","");
						$fabPurAcl= number_format($fabPurArr['acl']['woven']['amount'],4,".","");
						?>
					</td>
					<td width="100" align="right">&nbsp;</td>
					<td width="100" align="right">&nbsp;</td>
			  ...
			  <?
				...
				?>
				 <tr class="transc" bgcolor="<? echo $trans_bgcolor1;?>" onClick="change_color('trtrans_<? echo $tt; ?>','<? echo $trans_bgcolor1;?>')" id="trtrans_<? echo $tt; ?>">
                       ...
                       <td width="100" align="right"><? echo number_format(($tot_grey_fab_cost_acl*100/$jobValue),2).'%'; ?></td>
					</tr>
				<?
				 $tt2=1;
				?>
				<tr class="transc" bgcolor="<? echo $trans_bgcolor2;?>" onClick="change_color('trtrans2_<? echo $tt2; ?>','<? echo $trans_bgcolor2;?>')" id="trtrans2_<? echo $tt2; ?>">
						...
						<td width="100" align="right"><? echo number_format($tot_wvn_fin_fab_transfer_cost/$tot_fin_fab_transfer_qty,4); ?></td>
				...
				<tr style="font-weight:bold;background-color:#CCC">
					...
					<td width="100" align="right"><? echo number_format(($tot_wvn_fin_fab_transfer_cost+$tot_grey_fab_cost_acl)/($tot_fin_fab_transfer_qty+$grey_fab_trans_qty_acl),4);?></td>
					<td width="100" align="right">
						<?
						echo number_format($tot_wvn_fin_fab_transfer_cost+$tot_grey_fab_cost_acl,4);
						$total_grey_fin_fab_transfer_actl_cost=$tot_wvn_fin_fab_transfer_cost+$tot_grey_fab_cost_acl;
				...
				?>
                 <tr style="font-weight:bold;background-color:#CCC">
					...
					<td width="100" align="right"><? echo number_format($total_mkt_amt/$total_mkt_qty,4); ?></td>
					...
					<td width="100" align="right"><? echo number_format($total_pre_amt/$total_pre_qty,4); ?></td>
					...
					<td width="100" align="right"><? echo number_format($total_acl_amt/$total_acl_qty,4); ?></td>
				...
				<tr  bgcolor="<? echo $other_bgcolor;?>" onClick="change_color('commer_<? echo $embl; ?>','<? echo $other_bgcolor;?>')" id="commer_<? echo $embl; ?>">
					...
					<td width="100" align="right"><? //echo  number_format($commer_act_cost/$exfactoryQty,4); ?></td>

					<td width="100" align="right" title="Unit Price*Ex Fact Qty*(0.5/100)" bgcolor="<? //if(number_format($exfactoryQty*$unitPrice)>number_format($commaData['pre']['amount'],4,".","")){echo "red";}else{ echo "";} ?>"><? //echo number_format($commer_act_cost,4); $totalOtherAclValue+=number_format($commer_act_cost,4,".",""); ?></td>
				...
				<tr  bgcolor="<? echo $other_bgcolor;?>" onClick="change_color('trtc_<? echo $embl; ?>','<? echo $other_bgcolor;?>')" id="trtc_<? echo $embl; ?>">
					...
					//echo number_format($test_act_cost/$exfactoryQty,4); ?></td>
					<td width="100" align="right" title="Unit Price*Ex Fact Qty*(0.05/100)" bgcolor="<? //if(number_format($test_act_cost,4,".","")>number_format($otherData['pre']['lab_test']['amount'],4,".","")){echo "red";}else{ echo "";} ?>"><? //echo number_format($test_act_cost,4); $totalOtherAclValue+=number_format($test_act_cost,4,".","");?></td>	
			...
			<table class="rpt_table" border="1" cellpadding="2" cellspacing="2" style="width:1200px; margin-top:5px" rules="all">
				...
				<tr>
					...
					<td width="100" align="right"  title="TotalMktValue/OfferValue*100"><? echo number_format(($GrandTotalMktValue/$quaOfferValue)*100,4); ?></td>
					<td width="150" align="right"><? echo number_format($yarnTrimMktValue,4) ?></td>
					<td width="100" align="right" title="Total Expand/OfferValue*100"><? echo number_format(($yarnTrimMktValue/$quaOfferValue)*100,4); ?></td>
                    <td width="150" align="right"><? echo number_format($total_mkt_amt,4) ?></td>
					<td width="100" align="right" title="Total Mkt Wash/OfferValue*100"><? echo number_format(($total_mkt_amt/$quaOfferValue)*100,4); ?></td>
                    
					<td width="200" align="right" title="Offer Value-GrandTotal Mkt Value"><? echo number_format($quaOfferValue-$GrandTotalMktValue,4); ?></td>
					<td width="100" align="right" title="Offer Value-GrandTotal Mkt Value/OfferValue*100"><? echo number_format((($quaOfferValue-$GrandTotalMktValue)/$quaOfferValue)*100,4); ?></td>
				</tr>
				<tr>
					...
					<td width="100" align="right" title="TotalPreValue/JobValue*100"><? echo number_format(($GrandTotalPreValue/$jobValue)*100,4); ?></td>
					<td width="150" align="right"><? echo number_format($yarnTrimPreValue,4) ?></td>
					<td width="100" align="right" title="Total Trims/Job Value*100"><? echo number_format(($yarnTrimPreValue/$jobValue)*100,4); ?></td>
                     <td width="150" align="right"><? echo number_format($total_pre_amt,4) ?></td>
					<td width="100" align="right" title="Total Pre Wash/JobValue*100"><? echo number_format(($total_pre_amt/$jobValue)*100,4); ?></td>
                    
					<td width="200" align="right" title="Job Value-GrandTotal Pre Value"><? echo number_format($jobValue-$GrandTotalPreValue,4);?></td>
					<td width="100" align="right" title="Job Value-GrandTotal Pre Value/PreValue*100"><? echo number_format((($jobValue-$GrandTotalPreValue)/$jobValue)*100,4);?></td>
				</tr>
				<tr>
					...
					<td width="100" align="right" title="TotalActualValue/Ex-FactValue*100"><? echo number_format(($GrandTotalAclValue/$exfactoryValue)*100,4); ?></td>
					<td width="150" align="right"><? echo number_format($yarnTrimAclValue,4) ?></td>
					<td width="100" align="right" title="Total TrimsAcl/Ex-fact Value*100"><? echo number_format(($yarnTrimAclValue/$exfactoryValue)*100,4); ?></td>
                     <td width="150" align="right"><? echo number_format($total_acl_amt,4) ?></td>
					<td width="100" align="right" title="Total Wash Acl/Ex-FactValue*100"><? echo number_format(($total_acl_amt/$exfactoryValue)*100,4); ?></td>
                    
					<td width="200" align="right" title="Ex-Fact Value-GrandTotal Acl Value"><? echo number_format($exfactoryValue-$GrandTotalAclValue,4);?></td>
					<td width="100" align="right" title="Ex-fact Value-GrandTotal Ex Value/AclValue*100"><? echo number_format((($exfactoryValue-$GrandTotalAclValue)/$exfactoryValue)*100,4);?></td>
				</tr>
			</table>
            <br>
            <table class="rpt_table" border="1" cellpadding="2" cellspacing="2" style="width:1200px; margin-top:5px" rules="all">
				...
				<tr>
					<td width="200" >Marketing Cost</td>
					<td width="160" align="right" title="Mkt Cost/JobQty"><? $tot_offer_val_pcs=$unitPrice;//$quaOfferValue/$jobQty;
					echo number_format($tot_offer_val_pcs,4) ?></td>
					<td width="150" align="right"><? $tot_mkt_val_pcs=$GrandTotalMktValue/$jobQty;echo number_format($tot_mkt_val_pcs,4); ?></td>
					<td width="100" align="right"  title="TotalMktValue/OfferValue*100"><? echo number_format(($tot_mkt_val_pcs/$tot_offer_val_pcs)*100,4); ?></td>
					<td width="150" align="right"><? $tot_trim_val_pcs=$yarnTrimMktValue/$jobQty;echo number_format($tot_trim_val_pcs,4) ?></td>
					<td width="100" align="right" title="Total Trim/OfferValue*100"><? echo number_format(($tot_trim_val_pcs/$tot_offer_val_pcs)*100,4); ?></td>
                    <td width="150" align="right"><? $tot_wash_val_pcs=$total_mkt_amt/$jobQty; echo number_format($tot_wash_val_pcs,4) ?></td>
					<td width="100" align="right" title="Total Mkt Wash/OfferValue*100"><? echo number_format(($tot_wash_val_pcs/$tot_offer_val_pcs)*100,4); ?></td>
                     <td width="150" align="right"><? $tot_mkt_cm_pcs=$otherData['mkt']['cm_cost']['amount']/$jobQty; echo number_format($tot_mkt_cm_pcs,4); ?></td>
                    
					<td width="200" align="right" title="Offer Value-GrandTotal Mkt Value"><? $profit_mkt_margin_pcs=($quaOfferValue-$GrandTotalMktValue)/$jobQty;echo number_format($profit_mkt_margin_pcs,4); ?></td>
					<td width="100" align="right" title="Offer Value-(GrandTotal Mkt Value/OfferValue*100)"><? echo number_format(($profit_mkt_margin_pcs/$tot_offer_val_pcs)*100,4); ?></td>
				</tr>
				<tr>
					<td width="200" >Pre Costing</td>
					<td width="160" align="right" title="Pre Cost/JobQty"><? $tot_preJob_value=$unitPrice;//$jobValue/$jobQty;
					...
					<td width="100" align="right" title="TotalPreValue/JobValue*100"><? $tot_pre_costing_expnd_per=$tot_pre_costing_expnd/$tot_preJob_value*100;echo number_format($tot_pre_costing_expnd_per,4); ?></td>
					<td width="150" align="right"><? $tot_preTrim_costing=$yarnTrimPreValue/$jobQty;echo number_format($tot_preTrim_costing,4) ?></td>
					<td width="100" align="right" title="Total Trims/Job Value*100"><? $tot_preTrim_costing_per=($tot_preTrim_costing/$tot_preJob_value)*100;echo number_format($tot_preTrim_costing_per,4); ?></td>
                     <td width="150" align="right"><?  $tot_preWash_value=$total_pre_amt/$jobQty;echo number_format($tot_preWash_value,4) ?></td>
					<td width="100" align="right" title="Total Pre Wash/JobValue*100"><?  $tot_preWash_value_pre=$tot_preWash_value/$tot_preJob_value*100;echo number_format($tot_preWash_value_pre,4); ?></td>
                     <td width="150" align="right"><? $tot_pre_cm_pcs=$otherData['pre']['cm_cost']['amount']/$jobQty; echo number_format($tot_pre_cm_pcs,4); ?></td>
                    
					<td width="200" align="right" title="Job Value-GrandTotal Pre Value"><? $profit_pre_margin_pcs=($jobValue-$GrandTotalPreValue)/$jobQty;echo number_format($profit_pre_margin_pcs,4);?></td>
					<td width="100" align="right" title="Job Value-(GrandTotal Pre Value/PreValue*100)"><? echo number_format(($profit_pre_margin_pcs/$tot_preJob_value)*100,4);?></td>
				</tr>
				<tr>
					<td width="200" >Actual Costing</td>
					<td width="160" align="right" title="Acl JobValue/JobQty"><? $tot_AclJob_valuePcs=$unitPrice;//$exfactoryValue/$jobQty;
					echo number_format($tot_AclJob_valuePcs,4) ?></td>
					<td width="150" align="right"><? $tot_AclJob_ExpndPcs=$GrandTotalAclValue/$jobQty; echo number_format($tot_AclJob_ExpndPcs,4);?></td>
					<td width="100" align="right" title="TotalActualValue/Ex-FactValue*100"><? echo number_format(($tot_AclJob_ExpndPcs/$tot_AclJob_valuePcs)*100,4); ?></td>
					<td width="150" align="right"><? $tot_AcltrimValue=$yarnTrimAclValue/$jobQty;echo number_format($tot_AcltrimValue,4) ?></td>
					<td width="100" align="right" title="Total TrimsAcl/Ex-fact Value*100"><? echo number_format(($tot_AcltrimValue/$tot_AclJob_valuePcs)*100,4); ?></td>
                     <td width="150" align="right"><? $tot_AclJobWashPcs=$total_acl_amt/$jobQty; echo number_format($tot_AclJobWashPcs,4) ?></td>
					<td width="100" align="right" title="Total Wash Acl/Ex-FactValue*100"><? echo number_format(($tot_AclJobWashPcs/$tot_AclJob_valuePcs)*100,4); ?></td>
                     <td width="150" align="right"><?  $tot_cmAcl_pcs=$making_cost_act/$jobQty;; echo number_format($tot_cmAcl_pcs,4) ?></td>
                    
					<td width="200" align="right" title="Ex-Fact Value-GrandTotal Acl Value"><?  $profit_acl_margin_pcs=($exfactoryValue-$GrandTotalAclValue)/$jobQty; echo number_format($profit_acl_margin_pcs,4);?></td>

					<td width="100" align="right" title="Ex-fact Value-(GrandTotal Ex Value/AclValue*100)"><? echo number_format(($profit_acl_margin_pcs/$tot_AclJob_valuePcs)*100,4);?></td>
				</tr>
			</table>
             <br>
              <br>
	  ...
	  //  echo "<pre>$sql_sample</pre>";
		...
		//echo $quaOfferQnty; die;
			...
			//$mktcons=($fabPur_row[csf('avg_cons')]/$costPerQty)*($jobPcutQty/$totalSetQnty);
			 ...
			 $sql_trim = "select id,  trim_group, cons_uom, cons_dzn_gmts, rate, amount, apvl_req, nominated_supp,status_active from wo_pri_quo_trim_cost_dtls  where quotation_id='".$quotationId."' and status_active=1";
		...
		<div style="width:1302px; margin:0 auto">
			...
			<table class="rpt_table" border="1" cellpadding="2" cellspacing="2" style="width:1300px; margin-top:10px" rules="all">
			  ...
			  <?
				...
				<tr  bgcolor="<? echo $other_bgcolor;?>" onClick="change_color('trmc_<? echo $embl; ?>','<? echo $other_bgcolor;?>')" id="trmc_<? echo $embl; ?>">
					...
					?>
					<td width="100" align="right"><? echo $exfactoryQty ?></td>
					<td width="100" align="right"><? echo number_format($making_cost_act/$exfactoryQty,4); ?></td>
					<!--<td width="100" align="right"  bgcolor="<? //if(number_format($otherData['acl']['cm_cost']['amount'],4,".","")>number_format($otherData['pre']['cm_cost']['amount'],4,".","")){echo "red";}else{ echo "";} ?>"><?  
			...
			<table class="rpt_table" border="1" cellpadding="2" cellspacing="2" style="width:1200px; margin-top:5px" rules="all">
				...
				<tr>
					...
					<td width="160" align="right"><? echo number_format($quaOfferValue,4) ?></td>
					<td width="150" align="right"><? echo number_format($GrandTotalMktValue,4); ?></td>
					<td width="100" align="right"  title="Total Mkt Value/Offer Value*100"><? echo number_format(($GrandTotalMktValue/$quaOfferValue)*100,4); ?></td>
					<td width="150" align="right"><? echo number_format($yarnTrimMktValue,4) ?></td>
					<td width="100" align="right" title="Total Expand/Offer Value*100"><? echo number_format(($yarnTrimMktValue/$quaOfferValue)*100,4); ?></td>
                    <td width="150" align="right"><? echo number_format($total_mkt_amt,4) ?></td>
					<td width="100" align="right" title="Total Mkt Wash/Offer Value*100"><? echo number_format(($total_mkt_amt/$quaOfferValue)*100,4); ?></td>
					...
					<td width="100" align="right" title="Offer Value(<? echo $quaOfferValue;?>)-Grand Total Mkt Value(<? echo $GrandTotalMktValue;?>)/Offer Value(<? echo $quaOfferValue;?>)*100" ><? echo number_format((($quaOfferValue-$GrandTotalMktValue)/$quaOfferValue)*100,4); ?></td>
				</tr>
				<tr>
					<td width="200" >Pre Costing</td>
					<td width="160" align="right"><? echo number_format($jobValue,4) ?></td>
					<td width="150" align="right"><? echo number_format($GrandTotalPreValue,4);?></td>
					<td width="100" align="right" title="Total Pre Value/Job Value*100"><? echo number_format(($GrandTotalPreValue/$jobValue)*100,4); ?></td>
					<td width="150" align="right"><? echo number_format($yarnTrimPreValue,4) ?></td>
					<td width="100" align="right" title="Total Trims/Job Value*100"><? echo number_format(($yarnTrimPreValue/$jobValue)*100,4); ?></td>
                     <td width="150" align="right"><? echo number_format($total_pre_amt,4) ?></td>
					<td width="100" align="right" title="Total Pre Wash/Job Value*100"><? echo number_format(($total_pre_amt/$jobValue)*100,4); ?></td>
                    
					<td width="200" align="right" title="Job Value-Grand Total Pre Value"><? echo number_format($jobValue-$GrandTotalPreValue,4);?></td>
					<td width="100" align="right" title="Job Value(<? echo $jobValue;?>)-Grand Total Pre Value(<? echo $GrandTotalPreValue;?>)/Ex-Fact Value(<? echo $jobValue;?>)*100"><? echo number_format((($jobValue-$GrandTotalPreValue)/$jobValue)*100,4);?></td>
				</tr>
				<tr>
					<td width="200" >Actual Costing</td>
					<td width="160" align="right"><? echo number_format($exfactoryValue,4) ?></td>
					<td width="150" align="right"><? echo number_format($GrandTotalAclValue,4);?></td>
					<td width="100" align="right" title="Total Actual Value/Ex-Fact Value*100"><? echo number_format(($GrandTotalAclValue/$exfactoryValue)*100,4); ?></td>
					<td width="150" align="right"><? echo number_format($yarnTrimAclValue,4) ?></td>
					<td width="100" align="right" title="Total Trims Acl/Ex-fact Value*100"><? echo number_format(($yarnTrimAclValue/$exfactoryValue)*100,4); ?></td>
                     <td width="150" align="right"><? echo number_format($total_acl_amt,4) ?></td>
					<td width="100" align="right" title="Total Wash Acl/Ex-Fact Value*100"><? echo number_format(($total_acl_amt/$exfactoryValue)*100,4); ?></td>
					...
					<td width="100" align="right" title="Ex-fact Value(<? echo $exfactoryValue;?>)-GrandTotal Ex Value(<? echo $GrandTotalAclValue;?>)/Ex-Fact Value(<? echo $exfactoryValue;?>)*100"><? echo number_format((($exfactoryValue-$GrandTotalAclValue)/$exfactoryValue)*100,4);?></td>
				</tr>
			</table>
            <br>
            <table class="rpt_table" border="1" cellpadding="2" cellspacing="2" style="width:1200px; margin-top:5px" rules="all">
				...
				<tr>
					...
					<td width="100" align="right"  title="Total Expenditure(<? echo $tot_mkt_val_pcs;?>)/FOB Pcs(<? echo $tot_offer_val_pcs;?>)*100"><? echo number_format(($tot_mkt_val_pcs/$tot_offer_val_pcs)*100,4); ?></td>
					...
					<td width="100" align="right" title="Total Mkt Trims(<? echo $tot_trim_val_pcs?>)/FOB Pcs(<? echo $tot_offer_val_pcs?>)*100"><? echo number_format(($tot_trim_val_pcs/$tot_offer_val_pcs)*100,4); ?></td>
					...
					<td width="100" align="right" title="Total Mkt Wash(<? echo $tot_wash_val_pcs?>)/Offer Value(<? echo $tot_offer_val_pcs?>)*100"><? echo number_format(($tot_wash_val_pcs/$tot_offer_val_pcs)*100,4); ?></td>
				...
				<tr>
					...
					<td width="100" align="right" title="Total Expenditure/Job Value*100"><? echo number_format(($tot_AclJob_ExpndPcs/$tot_AclJob_valuePcs)*100,4); ?></td>
					...
					<td width="100" align="right" title="Total Trims Acl/Job Value*100"><? echo number_format(($tot_AcltrimValue/$tot_AclJob_valuePcs)*100,4); ?></td>
                     <td width="150" align="right"><? $tot_AclJobWashPcs=$total_acl_amt/$jobQty; echo number_format($tot_AclJobWashPcs,4) ?></td>
					<td width="100" align="right" title="Total Wash Acl/Job Value*100"><? echo number_format(($tot_AclJobWashPcs/$tot_AclJob_valuePcs)*100,4); ?></td>
					...
					<td width="100" align="right" title="Profit margin(<? echo $profit_acl_margin_pcs;?>)/Actual FOB/Pcs (<? echo $tot_AclJob_valuePcs;?>)*100)"><? echo number_format(($profit_acl_margin_pcs/$tot_AclJob_valuePcs)*100,4);?></td>
				</tr>
			</table>
             <br>
              <br>
...
if($action=="show_transfer_popup")
{
	...
	$sql_trim_trans="select c.transfer_system_id, c.transfer_date, c.challan_no, c.from_order_id,c.to_order_id, b.prod_id,c.from_order_id,b.quantity as transfer_qnty_in, (b.quantity*a.rate) as transfer_amount
		  from  inv_item_transfer_mst c, inv_item_transfer_dtls a, order_wise_pro_details b where c.id=a.mst_id and a.id=b.dtls_id and a.status_active=1 and a.is_deleted=0 and c.transfer_criteria!=2 and b.status_active=1 and b.is_deleted=0 and b.entry_form in(78) and b.trans_type in(5) and b.po_breakdown_id in(".$po_ids.")";//
		//$result_trim_trans=sql_select( $sql_trim_trans );
			...
			$sql_trim_trans="select c.transfer_system_id, c.transfer_date, c.challan_no, c.from_order_id,c.to_order_id, b.prod_id,c.from_order_id,b.quantity as transfer_qnty_in, (b.quantity*a.rate) as transfer_amount
		  from  inv_item_transfer_mst c, inv_item_transfer_dtls a, order_wise_pro_details b where c.id=a.mst_id and a.id=b.dtls_id and a.status_active=1 and a.is_deleted=0 and c.transfer_criteria!=2 and b.status_active=1 and b.is_deleted=0 and b.entry_form in(78) and b.trans_type in(6) and b.po_breakdown_id in(".$po_ids.")";
			$result = sql_select($sql);
			foreach ($result as $row) {
				if ($i % 2 == 0)
					$bgcolor = "#E9F3FF";
