...
if ($action=="load_drop_down_buyer"){
	echo create_drop_down( "cbo_buyer_name", 100, "select buy.id, buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data' $buyer_cond and buy.id in (select buyer_id from lib_buyer_party_type where party_type in (1,3,21,90)) order by buy.buyer_name","id,buyer_name", 1, "-- All Buyer --", $selected, "get_buyer_config(this.value);" );
	exit();
}
...
if ($action=="load_drop_down_brand")
{
	...
	if($data_arr[1] == 1) $width=85; else $width=85;
...
{
	echo create_drop_down( "cbo_season_name", 85, "SELECT id, season_name from lib_buyer_season where buyer_id='$data' and status_active =1 and is_deleted=0 group by id, season_name order by season_name ASC","id,season_name", 1, "-- Select Season--", "", "" );
	exit();
}
if ($action=="order_popup")
{
	...
	<script>
		...
		function get_buyer_config(buyer_id)
		{
			...
			load_drop_down( 'style_wise_cost_comparison_report_controller', buyer_id, 'load_drop_down_season_buyer', 'season_td');
			load_drop_down( 'style_wise_cost_comparison_report_controller', buyer_id+'*'+1, 'load_drop_down_brand', 'brand_td');
		}
	</script>
</head>
<body>
	<div align="center" style="width:100%;" >
		<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            <table width="1110" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
                ...
                <tr class="general">
                    <td><? echo create_drop_down( "cbo_company_mst", 100, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name",1, "-- Select Company --", $company_name,"load_drop_down( 'style_wise_cost_comparison_report_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?></td>
                    <td id="buyer_td"><? echo create_drop_down( "cbo_buyer_name", 100, $blank_array,'', 1, "-- Select Buyer --","","get_buyer_config(this.value);" ); ?></td>
                    <td id="brand_td"><? echo create_drop_down( "cbo_brand_id", 85, $blank_array,'', 1, "--Brand--",$selected, "" ); ?>
                     <td id="season_td"><? echo create_drop_down( "cbo_season_name", 85, $blank_array,"", 1, "-- Select Season --", $selected, "" ); ?>
                     	
                     </td>
                     <td>
                     	<? echo create_drop_down( "cbo_season_year", 45, create_year_array(),"", 1,"-Year-", 1, "",0,"" ); ?>
                 	 </td>
                    ...
                    <td>
                        <?
						echo create_drop_down( "shipping_status", 100, $shipment_status,"", 1, "ALL", 0, "",0,'','','','','' );
...
if($action=="create_po_search_list_view")
{
	...
	else
	{
		...
		if($db_type==0)
		{
			$sql="select a.job_no_prefix_num,a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.garments_nature,SUBSTRING_INDEX(a.`insert_date`, '-', 1) as year,a.brand_id,a.season_buyer_wise,a.season_year from wo_po_details_master a,wo_pre_cost_mst f where  a.job_no=f.job_no  and a.job_no not in( select distinct job_no_mst from wo_po_break_down where status_active=1 and is_deleted=0 ) and a.garments_nature=$data[5] and a.status_active=1 ".set_user_lavel_filtering(' and a.buyer_name','buyer_id')."   and a.is_deleted=0 $company $buyer $job_cond $budget_version_cond $style_cond $brand_seasion_seasion_year_cond order by a.job_no DESC";
		}
		if($db_type==2)
		{
			$sql="select a.job_no_prefix_num,a.job_no,a.company_name,a.buyer_name,a.style_ref_no,a.garments_nature,to_char(a.insert_date,'YYYY') as year,a.brand_id,a.season_buyer_wise,a.season_year from wo_po_details_master a,wo_pre_cost_mst f where   a.job_no=f.job_no and a.job_no not in( select distinct job_no_mst from wo_po_break_down where status_active=1 and is_deleted=0 ) and a.garments_nature=$data[5] and a.status_active=1 ".set_user_lavel_filtering(' and a.buyer_name','buyer_id')."  and a.is_deleted=0 $company $buyer $budget_version_cond $job_cond $style_cond $brand_seasion_seasion_year_cond order by a.job_no DESC";
		}
	...
	foreach ($result as $row)
	{
		if($cbo_search_by==3 && $data[13]==3)
		{
			if(strtotime($maxDateChkArr[$row[csf("job_no")]]["ex_date"]) <= strtotime($to_date))
			{
				if($po_chk[$row[("po_id")]] == "")
				{
					...
					$style_po_arr[$row[csf("job_no")]]["brand_id"]= $row[csf("brand_id")];
					$style_po_arr[$row[csf("job_no")]]["season_year"]= $row[csf("season_year")];
					$style_po_arr[$row[csf("job_no")]]["season_buyer_wise"]= $row[csf("season_buyer_wise")];
		...
		else
		{
			...
			$style_po_arr[$row[csf("po_id")]]["brand_id"]= $row[csf("brand_id")];
			$style_po_arr[$row[csf("po_id")]]["season_year"]= $row[csf("season_year")];
			$style_po_arr[$row[csf("po_id")]]["season_buyer_wise"]= $row[csf("season_buyer_wise")];
	...
	<table cellspacing="0" cellpadding="0" border="1" rules="all" width="1180" class="rpt_table">
		<thead>
			...
			<th width="70">Seasion</th>
			<th width="45">S.Year</th>
	...
	<div style="width:1180px; max-height:270px;overflow-y:scroll;" >
		...
		?>
			<tr bgcolor="<? echo $bgcolor;?>"   style="cursor:pointer;" id="tr_<? echo $k; ?>" onClick="js_set_value('<? echo $style_data; ?>');change_color('tr_<? echo $k; ?>','<? echo $bgcolor;?>')">
                ...
                <td width="45"><? echo $row[('job_prefix')]; ?></td>
                <td width="45"><? echo $row[('year')];; ?></td>
                <td width="50"><? echo $comp[$row[('company_name')]];//$garments_item[$row[csf('company_name')]]; ?></td>
                <td width="50"><p><? echo $buyer_arr[$row[('buyer_name')]]; ?></p></td>
                <td width="70"><p><? echo $brand_arr[$row[('brand_id')]]; ?></p></td>
                <td width="70"><p><? echo $seasion_arr[$row[('season_buyer_wise')]]; ?></p></td>
                <td width="45"><p><? echo $row[('season_year')]; ?></p></td>
...
if($action=="load_exchange_rate")
{
	...
	//echo "select b.job_no,b.exchange_rate from wo_po_details_master a,wo_pre_cost_mst b,wo_pre_cost_mst f where  a.job_no=b.job_no and a.job_no=f.job_no and a.company_name=$company_id and a.job_no_prefix_num=$job_no $year_cond $budget_version_cond";
...
if($action=="report_generate")
{
	...
	if($db_type==2) $year_cond=" and to_char(a.insert_date,'YYYY')=$cbo_year";
	$check_data=sql_select("select a.job_no from wo_po_details_master a,wo_pre_cost_mst f where a.job_no=f.job_no   and a.company_name=$company_name and a.job_no_prefix_num like '$job_no' $year_cond group by a.job_no");
	$chk_job_nos='';
		...
		<div style="width:1202px; margin:0 auto">
			...
			<table class="rpt_table" border="1" cellpadding="2" cellspacing="2" style="width:1200px; margin-top:5px" rules="all">
                ...
                <tr>
                	<td> </td>
                    <td> </td>
                    <td colspan="3"> Buyer Cost</td>
                     <td colspan="4"> Booking Cost</td>
                    <td colspan="4"> Post Cost</td>
                </tr>
