...
$company_arr=return_library_array( "select id, short_name from lib_company",'id','short_name');
...
if($action=="report_generate")
{
	...
	//$shiping_status=1;$shiping_status_id="";
		   ...
		   foreach($wvn_booking_partial_result as $row)
			{
				...
				$style_desc_arr[$job_no][$fab_desc]['fin_fab_qnty']+=$row[csf('fin_fab_qnty')];
				$style_desc_arr[$job_no][$fab_desc]['booking_amount']+=$row[csf('fin_fab_qnty')]*$row[csf('rate')];
		...
		$sql_pi = "select a.id as pi_mst_id,a.item_category_id as cat_id,a.pi_number,b.work_order_no,b.determination_id,(b.quantity) as qty,b.rate,b.amount,b.item_group,b.item_description from com_pi_master_details a, com_pi_item_details b where a.id=b.pi_id  and a.pi_basis_id=1  and b.item_category_id in(3) and b.status_active=1 and b.is_deleted=0 $booking_cond_in ";//wo_id_arr
		...
		unset($PiSQLresult);
			...
			foreach($pre_fab_result as $row)
			{
				...
				$style_desc_arr[$job_no][$fab_desc]['pi_qty']+=$PiArr[$job_no][$row[csf('deter_min_id')]]['pi_qty'];
				$style_desc_arr[$job_no][$fab_desc]['pi_amt']+=$PiArr[$job_no][$row[csf('deter_min_id')]]['pi_amt'];
				$style_desc_arr[$job_no][$fab_desc]['pi_no'].=$PiArr[$job_no][$row[csf('deter_min_id')]]['pi_number'].',';
				$style_desc_arr[$job_no][$fab_desc]['pi_mst_id'].=$PiArr[$job_no][$row[csf('deter_min_id')]]['pi_mst_id'].',';
			...
			foreach($booking_trim_result as $row)
			{
				...
				$trim_booking_idArr[$row[csf('booking_id')]]=$row[csf('booking_id')];
				$booking_dtls_idArr[$row[csf('booking_dtls_id')]]=$row[csf('booking_dtls_id')];
				$trim_booking_NoArr[$row[csf('booking_no')]]=$row[csf('job_no')];
			}
			//print_r($trim_booking_NoArr);
			$trim_booking_cond_in=where_con_using_array($booking_dtls_idArr,0,'b.work_order_dtls_id');
	 ...
	 //print_r($PiArr2);
		unset($TrimiSQLresult);
			...
			foreach($pre_trim_result as $row)
			{
				...
				$style_desc_arr[$job_no][$item_id]['cons']+=$row[csf('tot_cons')];
	...
	//print_r($PiArr3);
		...
		?> 
				...
				<td width="80" align="right" title="Woven Fin "><p><? echo number_format($row[('booking_amount')]/$row[('fin_fab_qnty')],4); ?></p></td>
				...
				<td width="80" align="right" title="Woven Fabric Pi Qty"><p><a href="##" onClick="report_generate_popup('<? echo $company_name; ?>','<? echo $job_key; ?>','<? echo $typeId; ?>','<? echo $pi_mst_ids; ?>','<? echo $fab_deter_min_id; ?>','<? echo $all_pre_dtls_ids; ?>','report_generate_woven_pi',2)"><? echo number_format($row[('pi_qty')],4); ?></a><? //$fabric_bal_after_cut_qty=$wv_recv_fin-$fabric_used_qty;
				//echo number_format($row[('pi_qty')],4); ?></p></td>
				<td width="80" align="right" title="PI Amount/PI Qty"><p><?  echo number_format($row[('pi_amt')]/$row[('pi_qty')],4); ?></p></td>
...
if($action=="report_generate_woven")
{
	...
	//echo $sql_booking="select b.pre_cost_fabric_cost_dtls_id, b.po_break_down_id, b.job_no, b.booking_no, b.booking_type, b.is_short, b.description,a.supplier_id, b.uom, b.wo_qnty, b.rate, b.amount,b.inserted_by, b.insert_date  from wo_booking_mst a, wo_booking_dtls b where a.booking_no=b.booking_no and b.job_no ='".$job_no."'   and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ";
		...
		foreach ($result_booking as $row) 
		{
			...
			$wo_booking_arr[$row[csf("booking_no")]][$fab_dess]['wo_qnty']+=$row[csf("fin_fab_qnty")];
			$wo_booking_arr[$row[csf("booking_no")]][$fab_dess]['amount']+=$row[csf("fin_fab_qnty")]*$row[csf("rate")];
	...
	<table class="rpt_table" border="1" cellpadding="5" cellspacing="2" width="<? echo $width;?>" rules="all">
		...
		<tr>
			...
			<th align="right"><? echo number_format($tot_fin_fab_qnty,2,'.','');?></th>
 ...
 if($action=="report_generate_woven_pi")
{
	...
	if($wo_type=='fabric')
	{
	   ...
	   $sql_pi = "select a.id as pi_mst_id,a.inserted_by, a.insert_date,a.supplier_id,a.pi_number,a.pi_date,a.item_category_id as cat_id,a.pi_number,b.work_order_no as booking_no,b.work_order_id,b.determination_id,b.fabric_composition,b.uom,(b.quantity) as qty,b.net_pi_amount,b.amount,b.rate,b.amount,b.item_group,b.item_description,c.booking_date,c.booking_type,c.is_short from com_pi_master_details a, com_pi_item_details b,wo_booking_mst c where a.id=b.pi_id and c.booking_no=b.work_order_no and a.pi_basis_id=1  and b.item_category_id in(3) and b.status_active=1 and b.is_deleted=0  and a.id in($descrip)  and b.determination_id in($deter_min)  ";//wo_id_arr
		...
		foreach ($result_pi as $row) 
		{
			...
			$supplier_id=$row[csf('supplier_id')];
			$pay_modeId=$row[csf('pay_mode')];
			$fab_dess=$row[csf("fabric_composition")];
			
				$booking_supplier=$supplier_arr[$supplier_id];
			
			//echo $supplier_id.'sds';
			
			$wo_booking_arr[$row[csf("booking_no")]][$fab_dess]['pi_qty']+=$row[csf("qty")];
			$wo_booking_arr[$row[csf("booking_no")]][$fab_dess]['pi_amount']+=$row[csf("amount")];
	...
	<table class="rpt_table" border="1" cellpadding="5" cellspacing="2" width="<? echo $width;?>" rules="all">
		...
		<tr>
            ...
            <th align="right"><? echo number_format($tot_booking_qnty,2,'.','');?></th>
             <th align="right"><? //echo number_format($tot_booking_amount,2,'.','');?></th>
            <th align="right"><? echo number_format($tot_booking_amount,2,'.','');?></th>
            <th align="right"><? echo number_format($tot_booking_qnty,2,'.','');?></th>
            <th align="right"><? //echo number_format($tot_amount,2,'.','');?></th>
			<th align="right"><? echo number_format($tot_pi_amount,2,'.','');?></th>
          
            <th align="right"><? echo number_format($tot_ex_balanceQty,2,'.','');?></th>
            <th align="right"><? echo number_format($tot_ex_balanceAmt,2,'.','');?></th>
            <th align="right"><? //echo number_format($tot_amount,2,'.','');?></th>
            <th align="right"><? //echo number_format($issue_total,2,'.','');?></th>
            <th align="right"><? //echo number_format($issue_total,2,'.','');?></th>
		</tr>
