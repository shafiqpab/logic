...
if ($action=="load_drop_down_season")
{
	...
	echo create_drop_down( "cbo_season_id", 100, "select id, season_name from lib_buyer_season where buyer_id in($data) and status_active =1 and is_deleted=0 order by season_name ASC","id,season_name", 0, "-- Select Season--", $selected, "" );
	exit();
}
...
if($action=="print_button_variable_setting")
{
    ...
    $print_report_format=return_field_value("format_id","lib_report_template","template_name in (".$data.") and module_id=11 and report_id=212 and is_deleted=0 and status_active=1");
...
if($action=="report_generate")
{
	...
	if($txt_style_ref=="") $inq_style_ref_cond=""; else $inq_style_ref_cond=" and c.style_refernce='$txt_style_ref' ";
	...
	//	echo "select a.inquiry_id,a.system_no as system_no,a.inserted_by,a.la_costing_date,a.pattern_master,a.bom_no from consumption_la_costing_mst a  where  a.status_active=1 and a.is_deleted=0  ".where_con_using_array($inquery_id_arr,0,'a.inquiry_id')."  order by a.inquiry_id asc";
		...
		//	echo "<br>";
			...
			//echo $inquery_id.'='.$samp_prefix_no.'<br>';
...
if($action=="report_generate2")
{
	...
	if(str_replace("'","",$txt_date_from_rec)!="" && str_replace("'","",$txt_date_to_rec)!="")
		...
		//echo $date_cond.'d';
		if(str_replace("'","",$txt_date_from_bulk)!="" && str_replace("'","",$txt_date_to_bulk)!="")
		{
			if($db_type==0)
			{
				$start_date_bulk=change_date_format(str_replace("'","",$txt_date_from_bulk),"yyyy-mm-dd","");
				$end_date_bulk=change_date_format(str_replace("'","",$txt_date_to_bulk),"yyyy-mm-dd","");
			}
			else if($db_type==2)
			{
				$start_date_bulk=change_date_format(str_replace("'","",$txt_date_from_bulk),"","",1);
				$end_date_bulk=change_date_format(str_replace("'","",$txt_date_to_bulk),"","",1);
			}
			$date_cond_bulk=" and c.est_ship_date between '$start_date_bulk' and '$end_date_bulk'";
		}
	...
	<div>
        <table width="<? echo $width_td;?>">
            <tr class="form_caption">
                <td colspan="13" align="center"><strong><? echo $hader_caption; ?></strong></td>
            </tr>
            <tr class="form_caption">
                <td colspan="13" align="center"><strong><? echo $company_library[$company_name]; ?></strong></td>
            </tr>
        </table>
        <table class="rpt_table" width="<? echo $width_td;?>" cellpadding="0" cellspacing="0" border="1" rules="all">
            <thead>
              ...
              <tr style="font-size:13px">
                	<th colspan="11">Confirm Order</th>
                    <th>Sample Requisition with Booking</th>
                    <th>Consumption Entry [CAD]</th>
                    <th colspan="2">Buyer Costing</th>
                    <th>Color & Size </br>Break Down</th>
                    <th colspan="2">Budget</th>
                    <th colspan="3">Work Order</th> 
                    <th colspan="3">Pro Forma Invoice V2</th>
              <tr/>
               <tr style="font-size:13px">
                       	<th width="20">SL</th>
                       	<th width="110">Com. Short Name</th>                      
                        <th width="100">Team Leader</th>
                       	...
                       	<th width="80">Buyer</th>                   
                       	<th width="80">Master Style</th>   
						<th width="80">Bulk Offer Qty</th>   
						<th width="80">Buyer Submit Price</th>   
						<th width="80">Style Value</th>  
						<th width="80">Esst. Ship. Date</th>  
		...
		$sql_inq="select c.id as inquery_id,c.inquery_date,c.mail_send_date,c.team_leader,c.buyer_id,c.color_id,c.color,c.insert_by,c.company_id as company_name,c.system_number_prefix_num,c.dealing_marchant,c.est_ship_date,c.concern_marchant,c.style_description,c.style_refernce,c.season_buyer_wise, c.season_year,c.brand_id,c.actual_req_quot_date,c.actual_sam_send_date,c.actual_sam_send_date,c.con_rec_target_date,c.system_number
		,c.buyer_submit_price,c.offer_qty from  wo_quotation_inquery c where  c.status_active=1 and c.is_deleted=0   $comp_cond2   $team_leader_cond3 $dealing_cond2  $buyer_id_cond2 $season_year_cond2  $brand_name_cond2 $date_cond_target $date_cond $date_cond_bulk $season_name_cond2  $int_ref_cond2  $inq_style_ref_cond  $inqu_ref_cond $team_leader_chk order by c.id "; // ".where_con_using_array($inquery_id_arr,0,'c.id')."
		...
		foreach($result_sql_inq as $row)
		{
			...
			$master_stlye_arr[$row[csf('inquery_id')]]['sys_prefixNo']=$row[csf('system_number_prefix_num')];
			$master_stlye_arr[$row[csf('inquery_id')]]['inquery_date']=$row[csf('inquery_date')];
			$master_stlye_arr[$row[csf('inquery_id')]]['est_ship_date']=$row[csf('est_ship_date')];
			$master_stlye_arr[$row[csf('inquery_id')]]['con_rec_target_date']=$row[csf('con_rec_target_date')];
			$master_stlye_arr[$row[csf('inquery_id')]]['actual_req_quot_date']=$row[csf('actual_req_quot_date')];
			$master_stlye_arr[$row[csf('inquery_id')]]['actual_sam_send_date']=$row[csf('actual_sam_send_date')];
			$master_stlye_arr[$row[csf('inquery_id')]]['buyer_submit_price']=$row[csf('buyer_submit_price')];
			$master_stlye_arr[$row[csf('inquery_id')]]['offer_qty']=$row[csf('offer_qty')];
			$master_stlye_arr[$row[csf('inquery_id')]]['style_value']=$row[csf('offer_qty')]*$row[csf('buyer_submit_price')];
			$master_stlye_arr[$row[csf('inquery_id')]]['mail_send_date']=$row[csf('mail_send_date')];
			$master_stlye_arr[$row[csf('inquery_id')]]['concern_marchant']=$dealing_library[$row[csf('concern_marchant')]];
			$master_stlye_arr[$row[csf('inquery_id')]]['buyer_name']=$row[csf('buyer_id')];
			$master_stlye_arr[$row[csf('inquery_id')]]['company_name']=$row[csf('company_name')];
		...
		//{
			$inquery_id_Cond=".where_con_using_array($inquery_id_arr,0,'c.id').";
		//}
			  $sql_po_chk="select c.id as inquery_id,a.id as job_id,a.job_no_prefix_num,a.job_quantity,a.inserted_by as po_insert_by,a.insert_date as po_insert_date,a.dealing_marchant as po_dealing_marchant,a.team_leader, a.job_no,a.company_name,a.buyer_name, a.style_ref_no, a.avg_unit_price, a.total_set_qnty as ratio,b.po_quantity, b.id as po_id, b.po_number,b.grouping, b.unit_price, b.shiping_status,b.is_confirmed, b.pub_shipment_date,b.grouping as ref_no,c.inquery_date,c.insert_by,c.system_number_prefix_num,c.dealing_marchant,c.concern_marchant,c.style_description,c.style_refernce,c.season_buyer_wise, c.season_year,c.brand_id,c.actual_req_quot_date,c.actual_sam_send_date,c.actual_sam_send_date,c.con_rec_target_date,c.system_number,c.est_ship_date,d.order_quantity,d.plan_cut_qnty,d.proj_qty,c.buyer_submit_price,c.offer_qty from wo_po_details_master a, wo_po_break_down b, wo_quotation_inquery c,wo_po_color_size_breakdown d where a.job_no=b.job_no_mst and d.po_break_down_id=b.id and  a.job_no=d.job_no_mst   and c.id=a.inquiry_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0   $comp_cond $style_owner_cond  $team_leader_cond $dealing_cond $buyer_id_cond $season_year_cond $job_no_cond   $job_id_cond $order_id_cond_trans $order_no_cond $brand_name_cond $date_cond_target $date_cond $season_name_cond  $int_ref_cond2  $style_ref_cond $date_cond_bulk  ".where_con_using_array($inquery_id_arr,0,'c.id')." order by c.id ";
	...
	//	$tot_rows=count($result_po);
     ...
     $sql_po="select c.id as inquery_id,a.id as job_id,a.job_no_prefix_num,a.job_quantity,a.inserted_by as po_insert_by,a.insert_date as po_insert_date,a.dealing_marchant as po_dealing_marchant,a.team_leader, a.job_no,a.company_name,a.buyer_name, a.style_ref_no, a.avg_unit_price, a.total_set_qnty as ratio,b.po_quantity, b.id as po_id, b.po_number,b.grouping, b.unit_price, b.shiping_status,b.is_confirmed, b.pub_shipment_date,b.grouping as ref_no,c.inquery_date,c.insert_by,c.system_number_prefix_num,c.dealing_marchant,c.concern_marchant,c.style_description,c.style_refernce,c.season_buyer_wise, c.season_year,c.brand_id,c.actual_req_quot_date,c.actual_sam_send_date,c.actual_sam_send_date,c.con_rec_target_date,c.system_number,c.est_ship_date,d.order_quantity,d.plan_cut_qnty,d.proj_qty,c.buyer_submit_price,c.offer_qty from wo_po_details_master a, wo_po_break_down b, wo_quotation_inquery c,wo_po_color_size_breakdown d where a.job_no=b.job_no_mst and d.po_break_down_id=b.id and  a.job_no=d.job_no_mst and b.grouping=c.style_refernce and c.id=a.inquiry_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0   $comp_cond $style_owner_cond  $team_leader_cond $dealing_cond $buyer_id_cond $season_year_cond $job_no_cond   $job_id_cond $order_id_cond_trans $order_no_cond $brand_name_cond $date_cond_target $date_cond $season_name_cond  $int_ref_cond  $style_ref_cond $date_cond_bulk ".where_con_using_array($inquery_id_arr,0,'c.id')." order by c.id ";
	...
	//	echo "select a.inquiry_id,a.system_no as system_no,a.inserted_by,a.la_costing_date,a.pattern_master,a.bom_no from consumption_la_costing_mst a  where  a.status_active=1 and a.is_deleted=0  ".where_con_using_array($inquery_id_arr,0,'a.inquiry_id')."  order by a.inquiry_id asc";
		...
		$total_conf_qty_pcs=$total_proj_po_qty_pcs=$total_balance_qty=0;
		$total_bulk_qty=0;$total_style_value_qty=0;
		foreach($master_stlye_arr as $inquery_id=>$row)
		{
			if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
		 ...
		 //	echo "<br>";
			...
			$po_team_leader=$master_stlye_arr[$inquery_id]['team_leader'];
			$po_insert_date=$master_stlye_arr[$inquery_id]['po_insert_date'];
			$est_ship_date=$master_stlye_arr[$inquery_id]['est_ship_date'];
			//$po_insert_date=$master_stlye_arr[$inquery_id]['po_insert_date'];
			//qc_show_file
		 ...
		 //else $print_button_job.=",".'<a href="##" onclick="report_generate_pop('.$company_name.",".$style_owner.",'".$job_prefix."','".$style_ref_nos."',".$buyer_name.",'".$po_id_all."','generate_style_report_with_graph',1".')">'.$job_prefix.'</a>';
			...
			<tr bgcolor="<? echo $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
				...
				<td width="80"><p><? echo $buyer_library[$row[('buyer_name')]]; ?></p></td>			
                <td width="80" ><p style="word-break:break-all"><? echo $row[('style_refernce')]; ?></p></td>
				<td width="80" align="right"><p style="word-break:break-all"><? echo number_format($row[('offer_qty')],2); ?></p></td>
				<td width="80" align="right"><p style="word-break:break-all"><? echo number_format($row[('buyer_submit_price')],4); ?></p></td>
				<td width="80" align="right"><p style="word-break:break-all"><? echo  number_format($row[('style_value')],2); ?></p></td>
				<td width="80"><p><? echo change_date_format($est_ship_date); ?></p></td>	
			...
			<?
				...
				$total_proj_po_qty_pcs+=$row[('proj_quantity')];
				$total_balance_qty+=$balance_qty;
				$total_bulk_qty+=$row[('offer_qty')];
				$total_style_value_qty+=$row[('style_value')];
        ...
        <table class="tbl_bottom" width="<? echo $width_td;?>" id="report_table_footer" cellpadding="0" cellspacing="0" border="1" rules="all">
        	<tr>
                ...
                <td width="80">&nbsp;<?=number_format($total_bulk_qty,2);?></td>
				<td width="80">&nbsp;</td>
				<td width="80">&nbsp;<?=number_format($total_style_value_qty,2);?></td>
                <td width="80">&nbsp;</td>
				<td width="80">&nbsp;</td>
...
if($action=="report_generate3")
{
	...
	if(str_replace("'","",$txt_date_from_rec)!="" && str_replace("'","",$txt_date_to_rec)!="")
	{
		...
		$date_cond=" and a.inquery_date between '$start_date_rec' and '$end_date_rec'"; 
	}

    // Cons. Rec.Tgt. Date Range
	$txt_date_from_target=trim(str_replace("'","",$txt_date_from_target));
	$txt_date_to_target=trim(str_replace("'","",$txt_date_to_target));
	...
	if(str_replace("'","",$txt_date_from_target)!="" && str_replace("'","",$txt_date_to_target)!="")
	{
		...
		$date_cond_target=" and a.con_rec_target_date between '$start_date_rec' and '$end_date_rec'";
	} 

	// company 
	if($company_name>0) $comp_cond="and a.company_id in($company_name)";else $comp_cond="";
	if($company_name>0) $comp_cond2="and b.company_name in($company_name)";else $comp_cond2="";
	...
	if(str_replace("'","",$cbo_buyer_name)==0)
	{
		if ($_SESSION['logic_erp']["data_level_secured"]==1)
		{
			if($_SESSION['logic_erp']["buyer_id"]!="") {
				...
				$buyer_id_cond2=" and c.buyer_name in (".$_SESSION['logic_erp']["buyer_id"].")";
			} 
			else{
				$buyer_id_cond="";
				$buyer_id_cond2="";
	...
	else
	{
		...
		$buyer_id_cond2=" and c.buyer_name in($cbo_buyer_name)";
	}

	//season
	if($cbo_season_id>0) $season_name_cond2="and a.season_buyer_wise in($cbo_season_id)";else $season_name_cond2="";
	...
	$dealing_merchant=str_replace("'","",$cbo_dealing_merchant);
	if($dealing_merchant>0) $dealing_cond="and a.dealing_marchant in($dealing_merchant)";else $dealing_cond="";
	//if($dealing_merchant>0) $dealing_cond2="and a.dealing_marchant in($dealing_merchant)";else $dealing_cond2="";

	// order 
	$hide_order_id=trim(str_replace("'","",$hide_order_id));
	// style id
	$txt_style_id=trim(str_replace("'","",$txt_style_id));
	if($txt_style_id=="") $inqu_ref_cond=""; else $inqu_ref_cond=" and a.id=$txt_style_id ";

	$txt_date_from_bulk=str_replace("'","",trim($txt_date_from_bulk));
	$txt_date_to_bulk=str_replace("'","",trim($txt_date_to_bulk));
	if($txt_date_from_bulk!="" && $txt_date_to_bulk!="")
	{
		if($db_type==0)
		{
			$start_date_bulk=change_date_format(str_replace("'","",$txt_date_from_bulk),"yyyy-mm-dd","");
		...
		$date_cond_bulk=" and a.est_ship_date between '$start_date_bulk' and '$end_date_bulk'";
	}
	...
	$sql = "SELECT a.ID as INQUIRY_ID,a.COMPANY_ID, a.BUYER_ID, a.STYLE_REFERNCE, a.GMTS_ITEM, a.EST_SHIP_DATE, a.OFFER_QTY, a.BUYER_SUBMIT_PRICE FROM wo_quotation_inquery a WHERE a.status_active=1 and a.IS_DELETED=0 $where_cond $date_cond $date_cond_target $comp_cond $buyer_id_cond $season_name_cond2 $season_year_cond $dealing_cond $inqu_ref_cond $date_cond_bulk ORDER BY a.id";
	// echo $sql ;die;
	...
	$company_sql ="SELECT id,company_name FROM lib_company WHERE id in ($company_name)";
	$setup_company_res=sql_select($company_sql);
	$setup_company_array=array();
	foreach($setup_company_res as $row)
	{  
		$setup_company_array[$row['ID']] = $row['COMPANY_NAME'];
	}
	?>
	<!-- hasan --> 
	...
	<thead>
		<tr>
			<td colspan="3" align="center">
				...
				<?= change_date_format($start_date_rec);?> <b>To</b>
				<?= change_date_format($end_date_rec);?>
			</td>
		</tr>
	...
	<tbody>
		...
		foreach($dataArr[$company_id] as $buyer_id => $buyer_rows){
			...
			?>
			<tr bgcolor="<?= $bgcolor;?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i; ?>">
				<td><?=$company_name;?></td>
			...
			<tr bgcolor="<?= $bgcolor;?>">
				...
				<th align="right"><?= $compWiseQty[$company_id];?></th>
			</tr>
			<? $i++;
			}
		?>
			<tr bgcolor="<?= $bgcolor;?>">
				...
				<th align="right"><?= array_sum($compWiseQty);?></th>
			</tr>
	</tbody>
</table>
...
if($action=="report_generate4")
{
	...
	<div align="center">
		...
		$sql = "select a.SYSTEM_NUMBER,a.ID as INQUIRY_ID,a.COMPANY_ID, a.BUYER_ID, a.STYLE_REFERNCE,a.GMTS_ITEM, a.EST_SHIP_DATE,a.OFFER_QTY,a.BUYER_SUBMIT_PRICE from wo_quotation_inquery a where a.status_active=1 and a.IS_DELETED=0 $where_cond $date_cond $date_cond_target $comp_cond $buyer_id_cond $season_name_cond2 $season_year_cond $dealing_cond $inqu_ref_cond $date_cond_bulk order by a.id";
		//echo $sql ;die;
		$sql_result=sql_select($sql);
		$inquiry_id_arr=array();
		$dataArr = array();
		...
		<table width="800">
			<tr>
				...
				<td colspan="5" align="right">
					...
					<strong><?=change_date_format($txt_date_from_rec);?> To <?= change_date_format($end_date_rec);?></strong>
				</td>
			</tr>
		</table>
        ...
        <?
         // echo $inquire_id.'ddd';;
				...
				// $po_sql="select a.job_no,b.po_number,a.style_ref_no as master_style from wo_po_break_down b,wo_po_details_master a,wo_po_color_size_breakdown c where  a.id=b.job_id and a.id=c.job_id  and b.id=c.po_break_down_id and b.grouping in('$master_style_ref') and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.status_active=1 and a.is_deleted=0 and a.inquiry_id=$inquire_id";//and a.inquiry_id=$inquire_id
				 $po_sql="select a.job_no,b.po_number,a.style_ref_no as master_style from wo_po_break_down b,wo_po_details_master a where  a.id=b.job_id  and a.style_ref_no in('$master_style_ref') and b.status_active=1 and b.is_deleted=0  and a.status_active=1 and a.is_deleted=0 and a.inquiry_id=$inquire_id";
				$po_sql_result=sql_select($po_sql);
				$tot_conf=$tot_proj=0;
				foreach($po_sql_result as $row)
				{
        ...
        <?
         // echo $inquire_id.'ddd';;
				...
				foreach($po_sql_result as $row)
				{
					...
					$product_dept=$product_dept[$row[csf('product_dept')]];
					$code_id=$row[csf('code_id')];
					if($is_confirmed==1)
					{
					$conf_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]]['color_qty']+=$row[csf('order_quantity')];
					$conf_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]]['pub_shipment_date']=$row[csf('pub_shipment_date')];
					$conf_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]]['c_code']=$code_arr[$row[csf('code_id')]];
					$conf_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]]['po_number']=$row[csf('po_number')];
					$conf_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]]['master_style']=$row[csf('master_style')];
					$conf_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]]['job_prefix']=$row[csf('job_no_prefix_num')];
					$conf_size_arr[$row[csf('size_id')]]=$row[csf('size_id')];
					 $conf_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]][$row[csf('size_id')]]['size_qty']+=$row[csf('order_quantity')];
					 $conf_size_color_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]][$row[csf('size_id')]]['size_qty']+=$row[csf('order_quantity')];
					$tot_conf+=$row[csf('order_quantity')];
					}
					else
					{
					$proj_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]]['color_qty']+=$row[csf('proj_qty')];
					$proj_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]]['pub_shipment_date']=$row[csf('pub_shipment_date')];
					$proj_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]]['c_code']=$code_arr[$row[csf('code_id')]];
					$proj_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]]['po_number']=$row[csf('po_number')];
					$proj_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]]['master_style']=$row[csf('master_style')];
					$proj_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]]['job_prefix']=$row[csf('job_no_prefix_num')];
					$proj_size_arr[$row[csf('size_id')]]=$row[csf('size_id')];
					 $proj_size_color_po_arr[$row[csf('job_no')]][$row[csf('po_id')]][$row[csf('country_id')]][$code_id][$row[csf('color_id')]][$row[csf('size_id')]]['size_qty']+=$row[csf('proj_qty')];
        ...
        <div id="report_div" align="center" style=" margin-left:10px;">
           ...
           <table border="1" class="rpt_table" rules="all" width="<? echo $td_width+100;?>" cellpadding="0" cellspacing="0" align="left" >
            ...
            <caption><b style="float:left"> Projected Order (SMEE)</b> </caption>
				...
				{
				 foreach($countryData as $code_id=>$code_Data)
				 {
					foreach($code_Data as $color_id=>$row)
				 	{
					...
					<tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
                        ...
                        <td width="80" align="center"><p><? echo $code_arr[$code_id]; ?></p></td>
						...
						foreach($proj_size_arr as $size_id )
						{
							  $proj_sizeQty=$proj_size_color_po_arr[$job_no][$po_id][$country_id][$code_id][$color_id][$size_id]['size_qty'];
				...
				foreach($countryData as $code_id=>$code_Data)
				 ...
				 foreach($code_Data as $color_id=>$row)
				 {
					...
					<tr bgcolor="<? echo  $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor;?>')" id="tr_<? echo $i;?>">
						...
						foreach($conf_size_arr as $size_id )
						{
							  $conf_sizeQty=$conf_size_color_po_arr[$job_no][$po_id][$country_id][$code_id][$color_id][$size_id]['size_qty'];
