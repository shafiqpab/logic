...
if($action=="set_field_name")
{
	...
	// echo $data_ref[0]."____".$data_ref[1]; die;
...
if($action=='save_update_delete')
{
	...
	if($operation==1)
	{
        ...
        $user_id = str_replace("'","",$text_user_id);// '132,133,134' 
        $duplicate_sql = "select a.id from field_level_access a, GBL_TEMP_ENGINE b where a.company_id=$cbo_company_name and a.user_id=b.ref_val and b.entry_form=2000 and b.ref_from=100 and b.user_id=$userId and a.page_id=$cbo_page_id and a.status_active=1"; 
  
        $duplicate_result = sql_select($duplicate_sql); 
          
        if(count($duplicate_result)>0)
        {			
            $rID = execute_query("delete  from field_level_access a where exists( select * from GBL_TEMP_ENGINE b where a.company_id=$cbo_company_name and a.user_id=b.ref_val and b.entry_form=2000 and b.ref_from=100 and b.user_id=$userId and a.page_id=$cbo_page_id )",1);	
        }
...
if($action=='action_user_data')
{
	...
	$array=sql_select("SELECT a.id, a.mst_id, a.field_id, a.field_name, a.is_disable, a.defalt_value, a.user_id from field_level_access a, GBL_TEMP_ENGINE b where a.company_id=$com_id and a.user_id=b.ref_val and b.entry_form=2000 and b.ref_from=100 and b.user_id=$userId and a.page_id=$page_id and a.status_active=1 and a.is_deleted=0");
