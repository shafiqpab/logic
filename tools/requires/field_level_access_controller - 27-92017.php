...
if($action=="set_field_name")
{
	...
	if($data_ref[1]==1)
	{
		echo "$('#setDefaultVal2_".$data_ref[2]."').show();\n";
	}
	else
	{
		echo "$('#setDefaultVal_".$data_ref[2]."').show();\n";
	}
...
if($action=='action_user_data')
{
	...
	else
	{
        ...
        <tr>
            <td align="center" id="fieldtd">
              <? echo create_drop_down("cboFieldId_".$i,200,$field_arr,"",1,"----Select----",$key,"set_hide_data(this.value+'**'+1);",1,"","","","","",""); ?>
            </td>
            <td align="center">
