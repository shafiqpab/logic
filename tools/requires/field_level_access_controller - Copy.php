...
if($action=='save_update_delete')
{
	...
	if($operation==0)
	{
		...
		$mst_id=$id;
		$duplicate_sql="select id from field_level_access where company_id=$cbo_company_name and user_id = $cbo_user_id and page_id=$cbo_page_id and status_active=1";
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			for($i=1;$i<=$total_row;$i++)
			{
				...
				if ($i!=1) $data_array .=",";
				$data_array	.="(".$id.",".$mst_id.",".$cbo_company_name.",".$cbo_user_id.",".$cbo_page_id.",'".$$cboFieldId."','".$$txtFieldName."','".$$cboIsDisable."','".$$setDefaultVal."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
				$id++;
			}
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");  
				echo "0**".$mst_id."**".str_replace("'","",$cbo_company_name)."**".str_replace("'","",$cbo_user_id)."**".str_replace("'","",$cbo_page_id);
			}
			else
			{
	...
	if($operation==1)
	{
		...
		{
			$cboFieldId="cboFieldId_".$i;
			$hideDtlsId="hideDtlsId_".$i;
			$sql_cond="";
			if($$hideDtlsId>0)
			{
				$sql_cond=" and id <>".$$hideDtlsId;
				if($i!=1)
				{
					$duplicate_sql.="  or ( field_id= ".$$cboFieldId." $sql_cond )";
				}
				else
				{
					$duplicate_sql.=" and (( field_id=".$$cboFieldId ." $sql_cond )";
				}
			}
			else
			{
		...
		if(str_replace("'","",$update_id)!="")
		{
			$txtDeleteRow=str_replace("'","",$txtDeleteRow);
			$delete_field_arr="status_active*is_deleted*updated_by*update_date";
			if($txtDeleteRow!="")
			{
				$delete_id_arr=array_unique(explode(",",$txtDeleteRow));
				foreach($delete_id_arr as $del_id)
				{
					$delete_id_array[]=$del_id;
					$delete_data_array[$del_id]=explode("*",("0*1*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
				}
			}
			$id=return_next_id( "id", "field_level_access", 1 ) ;
			$field_array_insert="id,mst_id,company_id,user_id,page_id,field_id,field_name,is_disable,defalt_value,inserted_by,insert_date";
			$field_array_update="mst_id*company_id*user_id*page_id*field_id*field_name*is_disable*defalt_value*updated_by*update_date";
			$data_array_insert="";
			for($i=1;$i<=$total_row;$i++)
			{
				$cboFieldId="cboFieldId_".$i;
				...
				if($$hideDtlsId>0)
				{
					$id_array_update[]=$$hideDtlsId;
					$data_array_update[$$hideDtlsId]=explode("*",($update_id."*".$cbo_company_name."*".$cbo_user_id."*".$cbo_page_id."*'".$$cboFieldId."'*'".$$txtFieldName."'*'".$$cboIsDisable."'*'".$$setDefaultVal."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
					
				}
				else
				{
					if ($data_array_insert!="") $data_array_insert .=",";
					$data_array_insert.="(".$id.",".$update_id.",".$cbo_company_name.",".$cbo_user_id.",".$cbo_page_id.",'".$$cboFieldId."','".$$txtFieldName."','".$$cboIsDisable."','".$$setDefaultVal."',".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
					$id++;
				}
		...
		{
			$rID = sql_insert("field_level_access",$field_array_insert,$data_array_insert,1);
		}
		if(count($delete_id_array)>0)
		{
			$rID2=execute_query(bulk_update_sql_statement( "field_level_access", "id", $delete_field_arr, $delete_data_array, $delete_id_array ));
		}
		if(count($id_array_update)>0)
		{
			$rID3=execute_query(bulk_update_sql_statement( "field_level_access", "id", $field_array_update, $data_array_update, $id_array_update ));
		}
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");  
				echo "1**".str_replace("'","",$update_id)."**".str_replace("'","",$cbo_company_name)."**".str_replace("'","",$cbo_user_id)."**".str_replace("'","",$cbo_page_id);
			}
			else
			{
				mysql_query("ROLLBACK"); 
