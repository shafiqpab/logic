...
//*************************************************************************************		
		...
		if($db_type==0) 
		{
			$group_con =" group by b.id, b.pub_shipment_date";
		}
		else
		{
			$group_con ="  group by b.id, c.total_set_qnty,b.unit_price,b.pub_shipment_date";
		}
		...
		if($db_type==0) 
		{
			...
			$sql="select to_char(b.pub_shipment_date,'YYYY-MM') as yr_mon,b.id as po_id, c.total_set_qnty as ratio, b.unit_price, sum(CASE WHEN b.is_confirmed=1 THEN b.po_total_price ELSE 0 END) AS confpoval, sum(CASE WHEN b.is_confirmed=2 THEN b.po_total_price ELSE 0 END) AS projpoval ,sum(CASE WHEN b.is_confirmed=1 THEN (b.po_quantity*c.total_set_qnty) ELSE 0 END) AS confpoqty, sum(CASE WHEN b.is_confirmed=2 THEN (b.po_quantity*c.total_set_qnty) ELSE 0 END) AS projpoqty from wo_po_break_down b, wo_po_details_master c where b.job_no_mst=c.job_no and c.company_name in($manufacturing_company) and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 $where_con $group_con";
		}
		else
		{
			$sql="select to_char(b.pub_shipment_date,'YYYY-MM') as yr_mon,b.id as po_id, c.total_set_qnty as ratio, b.unit_price, sum(CASE WHEN b.is_confirmed=1 THEN b.po_total_price ELSE 0 END) AS confpoval, sum(CASE WHEN b.is_confirmed=2 THEN b.po_total_price ELSE 0 END) AS projpoval ,sum(CASE WHEN b.is_confirmed=1 THEN (b.po_quantity*c.total_set_qnty) ELSE 0 END) AS confpoqty, sum(CASE WHEN b.is_confirmed=2 THEN (b.po_quantity*c.total_set_qnty) ELSE 0 END) AS projpoqty from wo_po_break_down b, wo_po_details_master c where b.job_no_mst=c.job_no and c.company_name in($manufacturing_company) and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 $where_con $group_con";	 
