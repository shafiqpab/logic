...
<?
	...
	$no_of_company='';
	if($_SESSION['erp_dashboard']["dashboard_active"]==1)
	{
		$_SESSION['erp_dashboard']["dashboard_active"]=1;
		if($db_type==0) 
		{
...
unset($result);
		...
		if($db_type==0) 
		{
			...
			$sql="select DATE_FORMAT(a.country_ship_date,'%Y-%m') as yr_mon,c.company_name,sum(a.order_total) AS povalue, sum(a.order_quantity) AS poqty from wo_po_color_size_breakdown a, wo_po_break_down b, wo_po_details_master c where a.po_break_down_id=b.id and b.job_no_mst=c.job_no and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 $where_con group by c.company_name,a.country_ship_date";
		}
		else
		{
			$sql="select to_char(a.country_ship_date,'YYYY-MM') as yr_mon,c.company_name,sum(a.order_total) AS povalue, sum(a.order_quantity) AS poqty from wo_po_color_size_breakdown a, wo_po_break_down b, wo_po_details_master c where a.po_break_down_id=b.id and b.job_no_mst=c.job_no and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 $where_con group by c.company_name,a.country_ship_date";
...
//*********************************************************************		
		...
		$_SESSION['erp_dashboard']["data"]=$data;
		$_SESSION['erp_dashboard']["data_qnt"]=$data_qnt;
		$_SESSION['erp_dashboard']["capacity"]=$capacity;
		$_SESSION['erp_dashboard']["catg"]=$catg;
		$_SESSION['erp_dashboard']["data_qnt_stck"]=$data_qnt_stck;
		$_SESSION['erp_dashboard']["data_val_stck"]=$data_val_stck;
		
		$_SESSION['erp_dashboard']["data_summ_qty"]=$html;
		$_SESSION['erp_dashboard']["data_summ_val"]=$html2;
		$_SESSION['erp_dashboard']["no_of_company"]=$no_of_company;
	}
	else
	{
		$data=$_SESSION['erp_dashboard']["data"];
		$data_qnt=$_SESSION['erp_dashboard']["data_qnt"];
		$capacity=$_SESSION['erp_dashboard']["capacity"];
		$catg=$_SESSION['erp_dashboard']["catg"];
		$data_qnt_stck=$_SESSION['erp_dashboard']["data_qnt_stck"];
		$data_val_stck=$_SESSION['erp_dashboard']["data_val_stck"];
		
		$html=$_SESSION['erp_dashboard']["data_summ_qty"];
		$html2=$_SESSION['erp_dashboard']["data_summ_val"];
		$no_of_company=$_SESSION['erp_dashboard']["no_of_company"];
	}
