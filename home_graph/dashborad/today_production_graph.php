...
if($location!=0){
	...
	$location_name_con=" and location_name = $location";	
	$location_id_con=" and location_id = $location";	
}

if($floor!=0){
	$floor_name_con=" and floor_name = $floor";
	$floor_id_con=" and floor_id = $floor";
}
...
$floor_name_arr=return_library_array("select id,floor_name from lib_prod_floor where is_deleted=0 and status_active=1 order by floor_serial_no", "id","floor_name");
	...
	?>	
		...
		<div class="wrap" style="margin-left:15px; margin-top:10px">
			...
			$lineDataArr=sql_select( "select id, line_name, company_name, location_name,floor_name from lib_sewing_line where company_name in ($manufacturing_company) and status_active=1 and is_deleted=0 $location_name_con $floor_name_con order by sewing_line_serial");
			foreach($lineDataArr as $row)
			{
				...
				$new_array[$row[csf('company_name')]][$row[csf('location_name')]][$row[csf('floor_name')]][$row[csf('id')]]=$row[csf('line_name')];
			}
			...
			if($prod_reso_allocation==1)
			{
				if($db_type==0)
				{
					$line_sql="select b.id, b.line_number, c.sewing_line_serial, c.location_name, sum(a.target_per_hour) tph from prod_resource_dtls a, prod_resource_mst b, lib_sewing_line c where b.id=a.mst_id and b.line_number=c.id and b.company_id in($manufacturing_company) and a.pr_date='$today' $location_name_con $floor_name_con and a.is_deleted=0 and b.is_deleted=0 and b.line_marge=2 group by b.id, b.line_number, a.pr_date, c.sewing_line_serial order by c.location_name,c.floor_name,c.sewing_line_serial order by c.sewing_line_serial"; // $str_location
				}
				else
				{
					$line_sql="select  b.id,c.location_name,b.floor_id, b.line_number,   sum(a.target_per_hour) tph ,d.floor_serial_no,c.sewing_line_serial
					from prod_resource_dtls a, prod_resource_mst b, lib_sewing_line c ,lib_prod_floor d
					where d.id=b.floor_id and b.id=a.mst_id and b.line_number=to_char(c.id) and b.company_id in($manufacturing_company) and a.pr_date='$today' $location_name_con $floor_id_con and a.is_deleted=0 and b.is_deleted=0 and b.line_marge=2 
					group by c.location_name ,b.floor_id,d.floor_serial_no,c.sewing_line_serial,b.id, b.line_number, a.pr_date
					order by d.floor_serial_no,c.sewing_line_serial";					 
				...
				//Line Merge Yes data............
				$tph_sql=sql_select("select b.id, b.line_number, sum(a.target_per_hour) tph from prod_resource_dtls a, prod_resource_mst b where b.id=a.mst_id and b.company_id in($manufacturing_company) $location_id_con $floor_id_con and a.pr_date='$today' and a.is_deleted=0 and b.is_deleted=0 and b.line_marge=1 group by b.id, b.line_number, a.pr_date");
			...
			$sql.=" from pro_garments_production_mst where production_type in(5,11) and company_id in($manufacturing_company) $location_con $floor_id_con and production_date='$today' and is_deleted=0 and status_active=1 group by sewing_line";
			
			$sql_subconProd.=" from subcon_gmts_prod_dtls WHERE company_id in($manufacturing_company) $location_id_con $floor_id_con and production_date='$today'  and production_type=2 and status_active=1 and is_deleted=0 group by line_id";
