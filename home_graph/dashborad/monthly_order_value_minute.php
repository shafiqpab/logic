...
<?
	...
	if($_SESSION['logic_erp']["month_arr__"]=="")
	{
        ...
        else
        {
		   ...
		   // a.cutup_date >='$startDate'
           $sql="select a.country_id,c.set_smv,c.buyer_name,b.id as po_id,a.country_ship_date,sum(a.order_quantity) AS qnty,sum(a.order_total) AS order_value from wo_po_color_size_breakdown a, wo_po_break_down b, wo_po_details_master c where a.po_break_down_id=b.id and b.job_no_mst=c.job_no and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and a.country_ship_date >='$startDate' $company_cond group by a.country_id,c.set_smv,c.buyer_name,b.id,a.country_ship_date";
          $order_result=sql_select($sql);
              //echo $sql;die;
			
			$orderQtyArr=array();$orderValArr=array();$orderMinArr=array();$orderIdArr=array();
            foreach($order_result as $row)
            { 
				$monthKey=date("M y",strtotime($row[csf('country_ship_date')]));
				$orderQtyArr[$monthKey]+=$row[csf('qnty')];
				$orderValArr[$monthKey]+=$row[csf('order_value')];
				$orderMinArr[$monthKey]+=($row[csf('qnty')]*$row[csf('set_smv')]);
				...
				if(strtotime($maxDate) <= strtotime($row[csf('country_ship_date')])){
					$maxDate=$row[csf('country_ship_date')];
				}
            }
		...
		foreach($month_arr as $key=>$monthYear)
        {
            ...
            $order_min_array[]=number_format($orderMinArr[$monthYear],0,'.','');
        }
		...
		$_SESSION['logic_erp']["order_min"]=$order_min_array;
	...
	else
	{
		...
		$order_min_array=$_SESSION['logic_erp']["order_min"];
	...
	$order_min_array= json_encode($order_min_array); 
	...
	<div style="margin:10px;width:100%;" >
        ...
        <div style="width:<? echo $width;?>%;float:left; overflow:hidden; margin-left:10px; margin-bottom:10px; border:solid 1px;">
            ...
            <table id="canvas_info" class="orderExportData" border="1" rules="all">
                ...
                <tr>
                    <td>Mins</td>
                    <? foreach($month_arr as $month){echo "<td align='right'>" . number_format($orderMinArr[$month],2) . "&nbsp;</td>";} ?>
                </tr>
            </table>
        ...
        <? if($print!=1){ ?>
        <a href="home_graph/dashborad/monthly_order_value_minute.php?print=1" target="_blank"><img src="img/print.jpg" height="20" alt="" /></a>
        <? } ?>
        <? if($print==1){?>
