...
// $type=0;//0 for country ship date wise;
// $type=1;//1 for public ship date wise;
		...
		foreach($yr_mon_part as $key=>$val)
		{
			if($type==1){
				if($db_type==0) 
				{
					...
					where b.job_no_mst=c.job_no and c.company_name in($manufacturing_company) and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and $dateField like '".$val."-%"."' group by b.id c.total_set_qnty,b.unit_price";
				}
				else
				{
					$sql="select b.id as po_id, c.total_set_qnty as ratio, b.unit_price,
