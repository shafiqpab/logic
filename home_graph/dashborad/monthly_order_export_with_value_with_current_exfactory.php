...
<script src="<? echo $printLink;?>Chart.js-master/Chart.min.js"></script>
	...
	if($_SESSION['logic_erp']["month_arr"]=="")
	{
		...
		//print_r($orderQtyComProjected[1]);die;
		...
		// pro_ex_factory_mst delivery_mst_id
		// pro_ex_factory_delivery_mst id

		$currExfactory_sql ="SELECT a.EX_FACTORY_DATE, b.COMPANY_ID, SUM(CASE WHEN a.entry_form!=85 THEN a.ex_factory_qnty ELSE 0 END) as EX_FACTORY_QNTY, sum(CASE WHEN a.entry_form=85 THEN a.ex_factory_qnty ELSE 0 END) as EX_FACTORY_RETURN_QNTY from pro_ex_factory_mst a, pro_ex_factory_delivery_mst b where a.delivery_mst_id=b.id and a.status_active=1 and a.is_deleted=0 and a.EX_FACTORY_DATE >='$startDate' and a.EX_FACTORY_DATE <='$endDate' group by a.EX_FACTORY_DATE,b.COMPANY_ID";
		//echo $currExfactory_sql;die;

		$currExfactoryRes=sql_select($currExfactory_sql);
		$curr_ex_qty_arr = array(); $company_curr_ex_qty_arr_val =array();
		foreach($currExfactoryRes as $cxRow)
        {
            $key = date("M y",strtotime($cxRow['EX_FACTORY_DATE']));
			$exfactory_qty = $cxRow['EX_FACTORY_QNTY']-$cxRow['EX_FACTORY_RETURN_QNTY'];
			$curr_ex_qty_arr[$key] += $exfactory_qty;
			$company_curr_ex_qty_arr[$key] += $exfactory_qty;
			$company_curr_ex_qty_arr_val[$cxRow['COMPANY_ID']][$key] += $exfactory_qty;
        }
        
		//echo "<pre>";
		//print_r($company_curr_ex_qty_arr);die;
		...
		foreach($month_arr as $key=>$monthYear)
        {
			...
			$curr_exfactory_qty_arr[]=number_format(($curr_ex_qty_arr[$monthYear]*1),0,'.','');
		...
		foreach($company_arr as $com=>$com_nam)
		{
			...
			{ 
				...
				foreach($monthWisePoComArr[$com_nam][$monthYear] as  $po)
				{
					...
					// $company_curr_ex_qty_arr_val[$com_nam][$monthYear]=number_format($company_curr_ex_qty_arr,0,'.','');
		...
		$_SESSION['logic_erp']["company_curr_ex_qty_arr"]=$company_curr_ex_qty_arr;
		$_SESSION['logic_erp']["company_curr_ex_qty_arr_val"]=$company_curr_ex_qty_arr_val;
		...
		$_SESSION['logic_erp']["curr_exfactory_qty_arr"]=$curr_exfactory_qty_arr;
	}
	else
	{
		##################### Delivery Status as Running Full Pending and Partial Deliverd ##################################
		...
		$company_curr_ex_qty_arr=$_SESSION['logic_erp']["company_curr_ex_qty_arr"];
		$company_curr_ex_qty_arr_val=$_SESSION['logic_erp']["company_curr_ex_qty_arr_val"];
		##################### Company Wise Monthly Projected AND Confirmed ##################################
		...
		$curr_exfactory_qty_arr=$_SESSION['logic_erp']["curr_exfactory_qty_arr"];
	}
	...
	$curr_exfactory_qty_arr= json_encode($curr_exfactory_qty_arr); 
	...
	<div style="margin:10px;width:100%;" >
        <div style="width:<? echo $width;?>%;float:left; overflow:hidden; margin-left:10px; margin-bottom:10px; border:solid 1px;">
			<table class="orderExportData canvas_info" border="1" rules="all">
				<thead>
					<tr>
						<th colspan="13" bgcolor="#D5D9D8">Shipment In Hand Summery</th>
					</tr>
					<tr>
            ...
            <table class="orderExportData canvas_info" border="1" rules="all">
				<tbody>
					...
					<tr>
						<td><b>Current Export Qty (Pcs)</b></td>
						<? foreach($month_arr as $month){echo "<td align='right' style='color:black;'>" ;if(!empty($company_curr_ex_qty_arr[$month])){echo number_format($company_curr_ex_qty_arr[$month],2);}echo "&nbsp;</td>";} ?>
					</tr>
				</tbody>
            </table>
			<?
			...
			foreach($company_arr as $k=>$v)
			{
				...
				<table class="orderExportData canvas_info" border="1" rules="all">
					<tbody>
						...
						<tr>
							<td><b>Current Export Qty (Pcs)</b></td>
							<? foreach($month_arr as $month){echo "<td align='right' style='color:black;'>" ;if(!empty($company_curr_ex_qty_arr_val[$v][$month])){echo number_format($company_curr_ex_qty_arr_val[$v][$month],2);}echo "&nbsp;</td>";} ?>
						</tr>
		...
		var line_bar_data= {
			...
			data: {
			  ...
			  datasets: [
				...
				{
				  label: "Current Ex-Factory QTY(PCS)",
				  type: "bar",
				  backgroundColor: "#9DDE58",
				  data:<?= $curr_exfactory_qty_arr;?>,
				  fill: false
				}
			  ]
			},
