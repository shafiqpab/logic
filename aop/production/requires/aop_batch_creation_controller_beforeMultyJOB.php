...
if ($action=="load_drop_down_machine")
{
	...
	else
	{
		if($db_type==2)
		{
			echo create_drop_down( "txt_machine_no", 172, "select id,machine_no || '-' || brand as machine_name from lib_machine_name where category_id in(2,4,6) and company_id='$data[0]' and location_id='$data[1]' and status_active=1 and is_deleted=0 and is_locked=0 order by seq_no","id,machine_name", 1,"-- Select Machine --", $selected, "","" );
		}
		else if($db_type==0)
		{
			echo create_drop_down( "txt_machine_no", 172, "select id,concat(machine_no,'-',brand) as machine_name from lib_machine_name where category_id in(2,4,6) and company_id='$data[0]' and location_id='$data[1]' and status_active=1 and is_deleted=0 and is_locked=0 order by  seq_no","id,machine_name", 1, "-- Select Machine --", $selected, "","" );
		}
	}	
}
	...
	*/		 
	...
	{ 
		...
		if(str_replace("'","",$update_id)=="")
		{
			...
			$field_array="id, batch_against, batch_no,machine_no,location_id, batch_date, entry_form, company_id, extention_no, color_id, batch_weight, total_trims_weight, color_range_id, process_id, dur_req_hr, dur_req_min,remarks, aop_type, floor_id,buyer_po_id, inserted_by, insert_date";
			
			$data_array="(".$id.",".$cbo_batch_against.",".$txt_batch_number.",".$txt_machine_no.",".$cbo_location_name.",".$txt_batch_date.",281,".$cbo_company_id.",".$txt_ext_no.",".$color_id.",".$txt_batch_weight.",".$txt_tot_trims_weight.",".$cbo_color_range.",".$txt_process_id.",".$txt_du_req_hr.",".$txt_du_req_min.",".$txt_remarks.",".$txt_aop_type.",".$cbo_floor_name.",".$txt_po_id.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		else
		{
			...
			$field_array_update="batch_against*batch_no*machine_no*location_id*batch_date*company_id*extention_no*color_id*batch_weight*total_trims_weight*color_range_id*process_id*dur_req_hr*dur_req_min*remarks*aop_type*floor_id*buyer_po_id*updated_by*update_date";
		...
		$field_array_dtls="id, mst_id, po_id, item_description, prod_id, width_dia_type, roll_no, batch_qnty, inserted_by, insert_date";
		...
		for($i=1;$i<=$total_row;$i++)
		{
			...
			$data_array_dtls.="(".$id_dtls.",".$batch_update_id.",".$$po_id.",".$$prod_desc.",".$$prod_id.",".$$cboDiaWidthType.",".$$txtRollNo.",".$$txtBatchQnty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')"; 
	...
	{ 
		...
		else
		{
			...
			$field_array_update="batch_against*batch_no*machine_no*location_id*batch_date*extention_no*color_id*batch_weight*total_trims_weight*color_range_id*process_id*dur_req_hr*dur_req_min*remarks*buyer_po_id*updated_by*update_date";
			
			$data_array_update=$cbo_batch_against."*".$txt_batch_number."*".$txt_machine_no."*".$cbo_location_name."*".$txt_batch_date."*".$txt_ext_no."*".$color_id."*".$txt_batch_weight."*".$txt_tot_trims_weight."*".$cbo_color_range."*".$txt_process_id."*".$txt_du_req_hr."*".$txt_du_req_min."*".$txt_remarks."*".$txt_po_id."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
			$field_array_dtls="id, mst_id, po_id, item_description, prod_id, width_dia_type, roll_no, batch_qnty, inserted_by, insert_date";
			$field_array_dtls_update="po_id*item_description*prod_id*width_dia_type*roll_no*batch_qnty*updated_by*update_date";
			...
			for($i=1;$i<=$total_row;$i++)
			{
				...
				if(str_replace("'","",$$updateIdDtls)!="")
				{
					...
					$data_array_dtls_update[str_replace("'",'',$$updateIdDtls)] = explode("*",($$po_id."*".$$prod_desc."*".$$prod_id."*".$$cboDiaWidthType."*".$$txtRollNo."*".$$txtBatchQnty."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'"));
				...
				else
				{
					...
					$data_array_dtls.="(".$id_dtls_batch.",".$batch_update_id.",".$$po_id.",".$$prod_desc.",".$$prod_id.",".$$cboDiaWidthType.",".$$txtRollNo.",".$$txtBatchQnty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')"; 	
        ...
        <form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
            <table cellpadding="0" cellspacing="0" border="1" rules="all" width="500" class="rpt_table">
                ...
                <tr class="general">
                    ...
                    <td align="center">
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_search_batch_sl').value+'_'+<? echo $cbo_company_id; ?>+'_'+document.getElementById('cbo_string_search_type').value, 'create_batch_search_list_view', 'search_div', 'aop_batch_creation_controller', 'setFilterGrid(\'list_view\',-1);')" style="width:100px;" />
                    </td>
                </tr>
            </table>
...
if ($action=="populate_data_from_search_popup")
{
	...
	$data_array=sql_select("select id, company_id,location_id,machine_no, batch_against, batch_no, extention_no, batch_weight, total_trims_weight, batch_date, re_dyeing_from, color_id, color_range_id, process_id, dur_req_hr, dur_req_min, remarks, aop_type, floor_id, buyer_po_id, $year_cond from pro_batch_create_mst where id='$batch_id' and entry_form=281");
	//print_r($data_array);
	foreach ($data_array as $row)
	{
		...
		echo "load_drop_down( 'requires/aop_batch_creation_controller', document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_location_name').value, 'load_drop_down_machine', 'machine_td' );";
...
if( $action == 'batch_details' ) 
{
	...
	foreach($data_array as $row)
	{
		...
		<tr class="general" id="tr_<? echo $tblRow; ?>">
			...
			<td id="field_po_id">	
            	<?
				echo create_drop_down( "poId_".$tblRow, 90, "select b.id as po_id, b.order_no as po_number from subcon_ord_mst a, subcon_ord_dtls b where a.subcon_job=b.job_no_mst and a.company_id='".$row[csf('company_id')]."' and b.id='".$row[csf('po_id')]."' and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by b.id, b.order_no order by b.order_no ASC","po_id,po_number", 1, "-- Select PO --", $po_no, "hidden_data_load($tblRow);","", "", "", "", "", "","","poId[]" );
            	?>
            </td> 
            <td>
            	<? 
            	echo create_drop_down( "cboBodyPart_".$tblRow, 130, $body_part,"", 1, "--Select--",$body_part_id, "","","","", "", "", "", "","cboBodyPart[]" );
            	?>
            </td>  
            ...
            <td>
                <input type="text" name="txtRollNo[]" id="txtRollNo_<? echo $tblRow; ?>" class="text_boxes_numeric" value="<? if($row[csf('roll_no')]!=0) echo $row[csf('roll_no')]; ?>" style="width:80px" />
            ...
            <td>
                <input type="text" name="txtBatchQnty[]"  id="txtBatchQnty_<? echo $tblRow; ?>" class="text_boxes_numeric" value="<? echo $row[csf('batch_qnty')]; ?>" onKeyUp="calculate_batch_qnty();" onChange="check_balance_qnty(this.id)" style="width:90px" />
...
?>
	...
	<table width="930" cellspacing="0" align="center" border="0">
        ...
        <tr>
        	...
        	<td width="250">:<? 
			echo $dataArray[0][csf('cust_style_ref')]; 
			?></td>
	...
	//echo $sql_dtls;
			...
			foreach($sql_result as $row)
			{
				...
				<tr bgcolor="<? echo $bgcolor; ?>">
					...
					<td width="150" ><? echo $row[csf('item_description')];//$desc[0]; ?></td>
					<td width="50" align="center"><? echo $row[csf('gsm')];//$desc[1]; ?></td>
					<td width="60" align="center"><? echo $row[csf('grey_dia')];//$desc[2]; ?></td>
					<td width="60"><? echo $row[csf('fin_dia')]; ?></td>
					<td width="80" align="center"><? if($row[csf('fabric_from')]==1 || $row[csf('fabric_from')]==3) { echo $mc_dia_gauge_in; }else{if($machine_gauge_up!='') echo $machine_dia_up." X ".$machine_gauge_up; else echo $machine_dia_up;} ?></td>
                    ...
                    <td width="80" align="center"><? echo $row[csf('rec_challan')]; ?></td>
        ...
        <tr>
         ...
         <tr>
			...
			<? 
             ...
             ?>
            <tr>
                <th colspan="13" align="left" ><strong>Process Required</strong></th>
            </tr>
            <tr>
                <td colspan="13" title="<? echo $process_value; ?>"> <strong><? echo $process_value; ?> </strong></td>
            </tr>
            <tr>
                <td colspan="5" align="left">Heat Setting:</td>
...
if( $action == "batch_color_popup"){
    ...
    ?>
        <script>
            function js_set_value(po_dtls_id,body_part,desc,qty,color_id,color,po,style,buyer_po_id){
            	document.getElementById('hidden_po_dtls_id').value=po_dtls_id;
            	document.getElementById('hidden_body_part').value=body_part;
            	document.getElementById('hidden_desc').value=desc;
            	...
            	document.getElementById('hidden_po').value=po;
            	document.getElementById('hidden_style').value=style;
            	document.getElementById('hidden_buyer_po_id').value=buyer_po_id;
            ...
            function change_caption(type)
			{
				if(type==1)
				{
					$('#td_search').html('Enter Order NO');
				}
				else if(type==2)
				{
					$('#td_search').html('Enter JOB NO.');
				}
				else if(type==3)
				{
					$('#td_search').html('Enter Buyer PO');
				}
				else if(type==4)
				{
					$('#td_search').html('Enter Buyer Style Ref.');
				}
			}
        </script>        
    <body>
            ...
            <table cellpadding="0" cellspacing="0" border="1" rules="all" width="720" class="rpt_table">
                <thead>
                    ...
                    <tr>
						...
						<th>Party Name</th>
						<th>Search By</th> 
                        <th id="td_search">Order NO</th>
                        <th colspan="2">Date ranges</th>
                        <th>
                            <input type="reset" name="reset" id="reset" value="Reset" style="width:70px" class="formbutton" />
                           	<input type="hidden" name="hidden_po_dtls_id" id="hidden_po_dtls_id" value="">
                           	<input type="hidden" name="hidden_body_part" id="hidden_body_part" value="">
                           	<input type="hidden" name="hidden_desc" id="hidden_desc" value="">
                           	<input type="hidden" name="hidden_qty" id="hidden_qty" value="">
                           	<input type="hidden" name="hidden_color" id="hidden_color" value="">
                           	<input type="hidden" name="hidden_color_id" id="hidden_color_id" value="">
                           	<input type="hidden" name="hidden_po" id="hidden_po" value="">
                           	<input type="hidden" name="hidden_style" id="hidden_style" value="">
                           	<input type="hidden" name="hidden_buyer_po_id" id="hidden_buyer_po_id" value="">
                ...
                <tr class="general">
                    ...
                    <td>
						<?php echo create_drop_down( "cbo_within_group", 90, $yes_no,"", 0, "--  --", 0, "load_drop_down( 'requires/aop_material_receive_controller',document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_buyer', 'buyer_td' );" ); ?>
					</td> 
					<td id="">
					 	<? 
					...
					<td> 
                        <?
							$search_by=array(1=>'Order NO.',2=>'JOB NO.',3=>'Buyer PO',4=>'Buyer Style Ref.');
                    ...
                    <td align="center">				
                        <input type="text" style="width:70px" class="text_boxes datepicker "  name="txt_start" id="txt_start" readonly />	
                    </td> 
                    <td align="center">		
                        <input type="text" style="width:70px" class="text_boxes datepicker"  name="txt_end" id="txt_end" readonly />	
                    </td> 
                    <td align="center">
                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_search_comm').value+'_'+<? echo $company_id; ?>+'_'+document.getElementById('txt_start').value+'_'+document.getElementById('txt_end').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('cbo_within_group').value, 'create_batch_color_search_list_view', 'search_div', 'aop_batch_creation_controller', 'setFilterGrid(\'tbl_list_search\',-1);')" style="width:70px;" />
                    </td>
                </tr>
            </table>
            <div id="search_div" style="margin-top:10px"></div>   
...
if( $action== "create_batch_color_search_list_view")
{
	...
	if($party_id!=0) $party_id_cond="and a.party_id=$party_id";else $party_id_cond="";
        ...
        $sql_cond = ""; $job_cond=""; $style_cond=""; $po_cond=""; $search_com_cond="";
        if($search_type==3 || $search_type==0)
		{
			if($search_str!="")
			{
				//if($search_by==1) $search_com_cond="and a.job_no_prefix_num like '%$search_str%'";  
				if($search_by==1) $search_com_cond="and a.order_no like '%$search_str%'";  
				
				if ($search_by==2) $job_cond=" and a.job_no_prefix_num like '%$search_str%'";  
				else if ($search_by==3) $po_cond=" and b.po_number like '%$search_str%'";
		...
		$sql= "select b.id as po_dtls_id, b.order_no,  b.buyer_po_id, b.aop_color_id, b.body_part,b.construction ,b.composition,b.order_quantity from subcon_ord_mst a, subcon_ord_dtls b where a.subcon_job=b.job_no_mst and a.entry_form=278 and a.company_id=$company_id and a.within_group=$within_group and a.status_active=1 $order_rcv_date  $search_com_cond  $party_id_cond $po_idsCond order by b.id DESC";
		$nameArray=sql_select( $sql ); $data_arr=array();
		foreach ($nameArray as $row)
		{
			...
			$data_arr[$row[csf("po_dtls_id")]][$row[csf("buyer_po_id")]][$row[csf("aop_color_id")]]['desc']=$row[csf("construction")].",".$row[csf("composition")];
		}
                ...
                foreach ($data_arr as $po_dtls_id=>$po_dtls_id_val)
                {
                	foreach ($po_dtls_id_val as $buyer_po_id=>$buyer_po_id_val)
                	{
                		foreach ($buyer_po_id_val as $aop_color_id=>$aop_color_id_val)
                		{
                			if($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
		                    ?>
		                    <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer" id="search<? echo $i;?>" onClick="js_set_value('<? echo $po_dtls_id ; ?>','<? echo $aop_color_id_val['body_part'] ; ?>','<? echo $aop_color_id_val['desc'] ; ?>','<? echo $aop_color_id_val['order_quantity'] ; ?>','<? echo $aop_color_id ; ?>','<? echo $color_arr[$aop_color_id] ; ?>','<? echo $buyer_po_arr[$buyer_po_id]['po'] ; ?>','<? echo $buyer_po_arr[$buyer_po_id]['style'] ; ?>','<? echo $buyer_po_id ; ?>')">
		                        <td width="30" align="center"><? echo $i; ?></td>
		                        <td width="130" align="center"><p><? echo $aop_color_id_val['order_no']; ?></p></td>
		                        <td width="130" style="word-break:break-all"><? echo $buyer_po_arr[$buyer_po_id]['job']; ?></td>
		                        <td width="130" style="word-break:break-all"><?php echo $buyer_po_arr[$buyer_po_id]['po']; ?></td>
		                        <td width="130" style="word-break:break-all"><?php echo $buyer_po_arr[$buyer_po_id]['style']; ?></td>
