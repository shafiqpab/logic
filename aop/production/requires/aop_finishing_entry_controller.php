...
{
	echo create_drop_down( "cbo_location_name", 140, "select id,location_name from lib_location where company_id='$data' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_drop_down( 'requires/aop_finishing_entry_controller', document.getElementById('cbo_company_id').value+'_'+this.value, 'load_drop_down_floor', 'floor_td' );load_drop_down( 'requires/aop_finishing_entry_controller', document.getElementById('cbo_company_id').value+'_'+this.value+'_'+0, 'load_drop_down_machine', 'machine_td' );" );	
}

if ($action=="load_drop_down_floor")
{
	$ex_data=explode('_',$data);
	echo create_drop_down( "cbo_floor_name", 140, "select id,floor_name from lib_prod_floor where company_id=$ex_data[0] and location_id=$ex_data[1] and status_active =1 and is_deleted=0 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/aop_finishing_entry_controller', document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_location_name').value+'_'+this.value, 'load_drop_down_machine', 'machine_td' );" );
}
        ...
        <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        <input type="text" name="txt_search_common" id="txt_search_common" class="text_boxes" style="width:77px" />
                    </td>
                    <td>
                        <input type="text" name="txt_batch_no" id="txt_batch_no" class="text_boxes" style="width:77px" />
                    </td>
                    <td>
...
if ($action=="fabric_finishing_id_search_list_view")
{
	...
	//echo "<pr>";
	//print_r($ref_arr);
	$sql= "select a.id, product_no, a.prefix_no_num, $year_cond, a.party_id, a.product_date, a.prod_chalan_no, $batch_cond, $order_cond, sum(b.product_qnty) as product_qnty from subcon_production_mst a, subcon_production_dtls b where a.id=b.mst_id and a.entry_form=453 and a.status_active=1 $company_name $buyer_cond $production_date_cond $product_id_cond $po_id_cond $batchIdsCond group by a.id, product_no, a.prefix_no_num, a.insert_date, a.party_id, a.product_date, a.prod_chalan_no order by a.id DESC";
	
	//echo  create_list_view("list_view", "Prod. ID,Year,Basis,Party,Prod. Date,Product Challan", "80,80,120,120,70,120","750","250",0, $sql , "js_set_value", "id", "", 1, "0,0,basis,party_id,0,0", $arr , "prefix_no_num,year,basis,party_id,product_date,prod_chalan_no", "aop_finishing_entry_controller","",'0,0,0,0,3,0');
    ?>
    <div>
    <script>
...
{
	$data=explode("_",$data);
	$nameArray=sql_select( "select a.id, a.product_no, a.basis, a.company_id, a.location_id, a.party_id, a.product_date, a.prod_chalan_no, a.remarks from subcon_production_mst a where a.entry_form=453 and a.id='$data[0]'" ); 
	...
	{	
		...
		echo "load_drop_down( 'requires/aop_finishing_entry_controller', $data[2]+'_'+$data[1], 'load_drop_down_buyer', 'buyer_td' );";
            ...
            <table width="750" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                ...
                <tbody>
                    <tr>
                        ...
                        <td align="center">
                            <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_id').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+document.getElementById('txt_aop_ref').value, 'batch_search_list_view', 'search_div', 'aop_finishing_entry_controller', 'setFilterGrid(\'list_view\',-1)')" style="width:100px;" />
                        </td>
                    </tr>
                    <tr>
...
if ($action=="batch_search_list_view")
{
	...
	else
	{
		...
		$po_id_cond='';
			...
			$buyer_po_arr=array();
			$order_buyer_po='';
			$order_sql ="select b.id,b.buyer_po_no,b.buyer_style_ref,buyer_po_id from subcon_ord_mst a, subcon_ord_dtls b where a.company_id =$data[0] and a.entry_form=278 and  a.subcon_job=b.job_no_mst"; 
    ...
    <div>
     ...
     <div style="width:850px; max-height:270px;overflow-y:scroll;" >	 
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="830" class="rpt_table" id="list_view">
			<?
            ...
            foreach( $result as $row )
            {
				...
				$order_no=''; $order_id='';  $order_ids='';  $aop_jobs=''; $aop_job=''; $buyer_job=''; $buyer_po=''; $buyer_style=''; $all_ref_arr=array();
				$order_no=''; $ref_no='';  $buyer_po=''; $buyerpoid='';
				...
				?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value('<? echo $row[csf("id")]."_".$ref_no."_".$row[csf("recipe_id")]."_".$buyer_po."_".$buyer_style."_".$buyerpoid;?>');" >
						...
						<td width="120" align="center"><p><? echo $row[csf("recipe_no")]; ?></p></td>
						<td width="120" align="center"><p><? echo $row[csf("batch_no")]; ?></p></td>
...
if($action=="load_php_data_to_form_batch")
{
	...
	if($db_type==0)
	{
		$sql="select a.id,a.entry_form,a.sales_order_no,a.booking_no ,a.sales_order_id ,a.is_sales, a.batch_no, a.extention_no, a.color_id, a.process_id,a.within_group,a.party_id,a.company_id,a.floor_id,a.machine_no,a.location_id ,a.shift_id, b.po_id $group_concat1  from pro_batch_create_mst a,pro_batch_create_dtls b where a.id=b.mst_id and a.id='$data[0]'  group by a.batch_no, a.extention_no, b.po_id " ;
	}
	elseif($db_type==2)
	{
		$sql="select a.id,a.entry_form ,a.is_sales,a.sales_order_no,a.booking_no ,a.sales_order_id, a.batch_no, a.extention_no, a.color_id, a.process_id,a.within_group,a.party_id,a.company_id,a.floor_id,a.machine_no,a.location_id ,a.shift_id, b.po_id  $group_concat1  from pro_batch_create_mst a,pro_batch_create_dtls b where a.id=b.mst_id and a.id='$data[0]' group by a.id, a.entry_form, a.is_sales,a.sales_order_no,a.booking_no ,a.sales_order_id, a.batch_no, a.extention_no, a.color_id, a.process_id,a.within_group,a.party_id,a.company_id,a.floor_id,a.machine_no,a.location_id ,a.shift_id, b.po_id ";
	}
	//echo $sql; die;
	...
	{	
		...
		echo "load_drop_down( 'requires/aop_finishing_entry_controller','$datas' , 'load_drop_down_buyer', 'buyer_td' );";
		echo "document.getElementById('cbo_party_name').value			= '".$buyer."';\n";

		echo "load_drop_down( 'requires/aop_finishing_entry_controller', document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_location_name').value, 'load_drop_down_floor', 'floor_td' );";
		echo "document.getElementById('cbo_floor_name').value 			= '".$row[csf("floor_id")]."';\n";
		
		echo "load_drop_down( 'requires/aop_finishing_entry_controller', document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_location_name').value+'_'+document.getElementById('cbo_floor_name').value, 'load_drop_down_machine', 'machine_td' );";
...
if($action=="show_fabric_desc_listview")
{
	...
	*/
	...
	foreach($prod_data_sql as $row)
	{
	  ...
	  $prev_prod_sql="Select a.batch_id, a.cons_comp_id,a.process, sum(a.product_qnty) as product_qnty from subcon_production_dtls a,subcon_production_mst b where b.id=a.mst_id and b.entry_form in (453) and  a.batch_id='$data[2]' and a.status_active=1 and a.is_deleted=0 and to_char(a.process)=$process_id group by  a.batch_id, a.cons_comp_id,a.process";
	$prev_prod_data_sql=sql_select($prev_prod_sql);
	foreach($prev_prod_data_sql as $row)
	{
		$prev_production_qty_array[$row[csf('batch_id')]][$row[csf('cons_comp_id')]][$row[csf('process')]] +=$row[csf('product_qnty')];
	}
	...
	$sql = "select  a.id as batch_id,a.entry_form, a.batch_no, a.extention_no, a.color_id,a.style_ref_no,a.is_sales,a.sales_order_no,a.booking_no ,a.sales_order_id, b.id, b.po_id, b.buyer_po_id, b.item_description, b.body_part_id,roll_no, b.batch_qnty as qnty,b.prod_id from  pro_batch_create_mst a, pro_batch_create_dtls b where a.id='$data[2]' and a.id=b.mst_id $entry_form_cond and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 ";
        ...
        <tbody>
            ...
            {  
                ...
                $desc=explode(",",$row[csf('item_description')]); 
				  ...
				  $prev_production_qty=$prev_production_qty_array[$row[csf('batch_id')]][$row[csf('id')]][$process_id];
				 $balance= $production_qty_array[$row[csf('batch_id')]][$row[csf('id')]]-$prev_production_qty;  
               ...
               // $balance=$row[csf('qnty')]-$production_qty_array[$row[csf('batch_id')]][$row[csf('id')]];
				...
				$style_ref_no=implode(", ",array_unique(explode(", ",$style_ref_no)));
             	...
             	?>
                <tr bgcolor="<? echo $bgcolor; ?>" onClick='set_form_data("<? echo $row[csf('id')]."**".$desc[0].",".$desc[1]."**".$desc[2]."**".$desc[3]."**".$row[csf('color_id')]."**".$color_arr[$row[csf('color_id')]]."**".$row[csf('po_id')]."**".$row[csf('body_part_id')]."**".$po."**".$style_ref_no."**".$row[csf('buyer_po_id')]."**".$row[csf('roll_no')]."**".$uom_id."**".$balance."**".$row[csf('is_sales')]; ?>")' style="cursor:pointer" >
                    <td><? echo $i; ?></td>
                    ...
                    <td align="right" title="<? echo number_format( $prev_production_qty,2,'.',''); ?>"><? echo number_format( $prev_production_qty,2,'.','');//number_format( $balance,2,'.',''); ?></td>
                </tr>
	            <? 
	            $i++; 
            } 
    ...
    <div style="width:100%;max-height:180px; overflow:y-scroll" id="sewing_production_list_view" align="left">
        ...
        <?php  
			...
			foreach($sql_result as $row)
			{
				...
				?>
                <tr bgcolor="<? echo $bgcolor; ?>" onClick="get_php_form_data('<? echo $click_data ?>','load_php_data_to_form_dtls','requires/aop_finishing_entry_controller');" style="text-decoration:none; cursor:pointer" > 
                    <td width="30" align="center"><? echo $i; ?></td>
...
if ($action=="load_php_data_to_form_dtls")
{
	...
	foreach ($nameArray as $row)
	{
		...
		echo "load_drop_down( 'requires/aop_finishing_entry_controller', document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_location_name').value+'_'+document.getElementById('cbo_floor_name').value, 'load_drop_down_machine', 'machine_td' );";
		echo "document.getElementById('cbo_machine_id').value 			= '".$row[csf("machine_no")]."';\n";
		echo "show_list_view(document.getElementById('order_no_id').value+'_'+document.getElementById('txt_process_id').value+'_'+document.getElementById('txt_batch_id').value+'_'+document.getElementById('cbo_company_id').value,'show_fabric_desc_listview','list_fabric_desc_container','requires/aop_finishing_entry_controller','');\n";
		...
		/*echo "load_drop_down( 'requires/aop_finishing_entry_controller', document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_location_name').value, 'load_drop_down_floor', 'floor_td' );\n";
		echo "document.getElementById('cbo_floor_name').value		 			= '".$row[csf("floor_id")]."';\n"; 
		echo "load_drop_down( 'requires/aop_finishing_entry_controller', document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_floor_name').value, 'load_drop_machine', 'machine_td');\n";*/
		//echo "document.getElementById('txt_order_numbers').value		 		= '".$order_no."';\n";
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   // Insert Here===================================================================================================================
	{
		...
		$prod_sql="Select a.batch_id, a.cons_comp_id, sum(a.product_qnty) as product_qnty from subcon_production_dtls a , subcon_production_mst b where b.id=a.mst_id and b.entry_form in (291) and  a.batch_id=$txt_batch_id and a.status_active=1 and a.is_deleted=0 group by  a.batch_id, a.cons_comp_id";
		$prod_data_sql=sql_select($prod_sql);
		foreach($prod_data_sql as $row)
		{
			$production_qty_array[$row[csf('batch_id')]][$row[csf('cons_comp_id')]] +=$row[csf('product_qnty')];
		 ...
		 $prev_prod_sql="Select a.batch_id, a.cons_comp_id,a.process, sum(a.product_qnty) as product_qnty from subcon_production_dtls a,subcon_production_mst b where b.id=a.mst_id and b.entry_form in (453) and  a.batch_id=$txt_batch_id and a.status_active=1 and a.is_deleted=0 and to_char(a.process)=$txt_process_id group by  a.batch_id, a.cons_comp_id,a.process"; 
		$prev_prod_data_sql=sql_select($prev_prod_sql);
		foreach($prev_prod_data_sql as $row)
		{
			$prev_production_qty_array[$row[csf('batch_id')]][$row[csf('cons_comp_id')]][$row[csf('process')]] +=$row[csf('product_qnty')];
		...
		$production_qty=$production_qty_array[$row[csf('batch_id')]][$row[csf('cons_comp_id')]];
		$prev_production_qty=$prev_production_qty_array[$row[csf('batch_id')]][$row[csf('cons_comp_id')]][$row[csf('process')]];
		$balance_production_qty=$production_qty-$prev_production_qty;
		
		//echo "10**";
		//echo str_replace("'","",$txt_product_qnty) ."===".$balance_production_qty; die;
		
		if(str_replace("'","",$txt_product_qnty)>$balance_production_qty)
		{
				echo "20**AOP Finishing Qty can not exceed Production Qty";
				disconnect($con); die;
		}
			
		//txt_production_id
		$new_return_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id), '','AOPF', date("Y",time()), 5, "select id,prefix_no,prefix_no_num from  subcon_production_mst where entry_form=453 and company_id=$cbo_company_id  $year_cond=".date('Y',time())." order by id desc ", "prefix_no", "prefix_no_num" ));
		...
		{			
			...
			$data_array="(".$id.",453,'".$new_return_no[1]."','".$new_return_no[2]."','".$new_return_no[0]."','".$process_finishing."',".$cbo_receive_basis.",".$cbo_company_id.",".$cbo_location_name.",".$cbo_party_name.",".$txt_finishing_date.",".$txt_chal_no.",".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
		...
		$field_array2="id, mst_id, batch_id, width_dia_type, order_id, product_type, process, fabric_description, cons_comp_id, color_id, gsm, dia_width, product_qnty, reject_qnty, no_of_roll, floor_id, machine_id,shift,uom_id,buyer_po_id,body_part_id,remarks, inserted_by, insert_date";
		$data_array2="(".$id1.",".$id.",".$txt_batch_id.",".$hidden_dia_type.",".$order_no_id.",'".$process_finishing."','".$txt_process_id."',".$txt_description.",".$comp_id.",".$hidden_color_id.",".$txt_gsm.",".$txt_dia_width.",".$txt_product_qnty.",".$txt_reject_qty.",".$txt_roll_no.",".$cbo_floor_name.",".$cbo_machine_id.",".$cboShift.",".$cbo_uom.",".$txt_buyer_po_id.",".$cbo_body_part.",".$txt_remarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')"; 
		...
		//===========================================================================================================================================
	 	//echo "10**";
		//echo "select b.id from subcon_production_mst a, subcon_production_dtls b where a.id=b.mst_id  and a.entry_form = 453 and a.product_no=$txt_finishing_id and b.process=to_char($txt_process_id) and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0"; die;
		...
		$duplicate = return_field_value("a.product_no","subcon_production_mst a, subcon_production_dtls b","a.id=b.mst_id  and a.entry_form = 453 and a.product_no=$txt_finishing_id and b.process=to_char($txt_process_id) and  a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0", "product_no");
		
 		if(str_replace("'","",$duplicate) !="")
		{
			echo "20**Duplicate Process Name and  Batch No  is Not Allow in Same Finishing ID.\nFinishing ID = $duplicate";
			if($db_type==0)
			{
	...
	else if ($operation==1)   // Update Here==============================================================================================================
	{
		...
		//echo "10**";
		$prev_prod_sql="Select a.batch_id, a.cons_comp_id,a.process, sum(a.product_qnty) as product_qnty from subcon_production_dtls a,subcon_production_mst b where b.id=a.mst_id and b.entry_form in (453) and  a.batch_id=$txt_batch_id and a.status_active=1 and a.is_deleted=0 and to_char(a.process)=$txt_process_id and a.id!=$update_id_dtl group by  a.batch_id, a.cons_comp_id,a.process"; 
		$prev_prod_data_sql=sql_select($prev_prod_sql);
		foreach($prev_prod_data_sql as $row)
		{
			$prev_production_qty_array[$row[csf('batch_id')]][$row[csf('cons_comp_id')]][$row[csf('process')]] +=$row[csf('product_qnty')];
		...
		asort($txt_process_id);
		$txt_process_id=implode(",",$txt_process_id);
		$field_array2="batch_id*width_dia_type*order_id*process*fabric_description*cons_comp_id*color_id*gsm*dia_width*product_qnty*reject_qnty*no_of_roll*floor_id*machine_id*shift*uom_id*buyer_po_id*body_part_id*remarks*updated_by*update_date";
...
<div style="width:930px;">
    <table width="930" cellspacing="0" align="right" border="0">
        ...
        <tr>
            ...
            <td width="125"><strong>Party Name:</strong></td><td width="175px"><? if ($aop_order_within_group_val==1) echo $company_library[$dataArray[0][csf('party_id')]]; else echo $buyer_arr[$dataArray[0][csf('party_id')]]; //echo  $buyer_arr[$dataArray[0][csf('party_id')]]; ?></td>
        </tr>
        <tr>
