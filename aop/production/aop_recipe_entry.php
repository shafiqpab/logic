<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Recipe Entry For AOP
Functionality	:
JS Functions	:
Created by		:	Kausar
...
<script>
	...
	/*function fnc_openmypage_po()
	{
		...
		else
		{
			...
			var title = 'Order Selection Form';

			var page_link = 'requires/aop_recipe_entry_controller.php?action=emblorder_popup';

			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link+'&cbo_company_id='+cbo_company_id+'&cbo_within_group='+cbo_within_group, title, 'width=900px,height=450px,center=1,resize=1,scrolling=0','../');
			...
			emailwindow.onclose=function()
			{
				...
				if(estr_data[9]!="")
				{
					...
					load_drop_down( 'requires/aop_recipe_entry_controller', estr_data[6], 'load_drop_down_embl_type', 'embl_type_td');
	...
	{
		load_drop_down( 'requires/aop_recipe_entry_controller', company+'_'+val, 'load_drop_down_buyer', 'buyer_td_id' );
	}
	...
	function show_details()
	{
		...
		var list_view_orders = return_global_ajax_value( cbo_company_id+'**'+cbo_store_id, 'item_details', '', 'requires/aop_recipe_entry_controller');
		if(list_view_orders!='')
		{
	...
	function openmypage_itemLot(id)
	{
		...
		else
		{
			...
			var page_link = 'requires/aop_recipe_entry_controller.php?cbo_company_id='+cbo_company_id+'&txt_prod_id='+txt_prod_id+'&txt_item_lot='+txt_item_lot+'&action=itemLot_popup';
	...
	function fnc_recipe_entry(operation)
	{
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title, "recipe_entry_print", "requires/aop_recipe_entry_controller")
			//return;
			show_msg("3");
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			var water_ratio=$('#txt_water_ratio').val()*1;
			...
			if(water_ratio>100 || water_ratio <0)
			{
				alert("Water Ratio Should be between 1 and 100");
				return;
			}
			...
			if(recipe_for!=3)
			{
				if($('#copy_id').val()==2)
				{
					if( form_validation('cbo_recipe_for*cbo_company_id*txt_recipe_date*txt_batch_color*txt_multi_color*cbo_store_id*cbo_location','Recipe For*Company*Recipe Date*Color*Color*Store*Location')==false )
					{
						return;
					}
				}
				...
				{
					if( form_validation('cbo_recipe_for*cbo_company_id*txt_recipe_date*txt_batch_color*cbo_location','Recipe For*Company*Recipe Date*Color*Location')==false )
					{
						return;
					}
				}
			...
			else
			{
				if($('#copy_id').val()==2)
				{
					if( form_validation('cbo_recipe_for*cbo_company_id*txt_recipe_date*txt_multi_color*cbo_store_id*cbo_item_category*txt_item_group*cbo_location','Recipe For*Company*Recipe Date*Color*Store*Item Category*Item Group*Location')==false )
					{
						return;
					}
				}
				...
				{
					if( form_validation('cbo_recipe_for*cbo_company_id*txt_recipe_date*cbo_item_category*txt_item_group*cbo_location','Recipe For*Company*Recipe Date*Item Category*Item Group*Location')==false )
					{
						return;
					}
				}
			...
			var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_sys_id*cbo_recipe_for*txt_recipe_des*txt_recipe_date*cbo_company_id*txt_batch_id*cbo_location*txt_order_id*txt_order*cbo_buyer_name*txt_pocolor_id*txt_po_color*hidden_batch_color_id*hid_job_no*hid_item_id*hid_bodypart_id*txt_remarks*update_id*txt_multi_color*cbo_store_id*cbo_item_category*item_group_id*hidd_group_code*hidd_cons_uom*hidd_order_uom*txt_subgroup_name*txt_description*txt_item_size*hidd_newprod_id*txtbuyerPoId*copy_id*update_id_check*cbo_within_group*txt_water_ratio',"../../")+data_all+'&total_row='+i;
			//alert(data);

			//alert (data);//return;
			freeze_window(operation);
			http.open("POST","requires/aop_recipe_entry_controller.php",true);
	...
	function fnc_recipe_entry_Reply_info()
	{
		if(http.readyState == 4)
		{
			...
			if((reponse[0]==0 || reponse[0]==1))
			{
				...
				show_list_view(reponse[1], 'recipe_item_details', 'recipe_items_list_view', 'requires/aop_recipe_entry_controller', '' ) ;
				setFilterGrid('tbl_list_search',-1);
				$('#txt_multi_color').val('');

				$('#hidd_newprod_id').val('');
				//alert(reponse[3]);
				$('#cbo_store_id').val(reponse[3]);
				...
				$('#hidd_order_uom').val('');
				$('#hidd_group_code').val('');
				$('#txt_days_ratio').val(0);
				$('#txt_water_ratio').val(100);
			...
			release_freezing();
			$('#txt_batch_number').attr('disabled',true);
		}
	}
	...
	{
		var cbo_company_id = $('#cbo_company_id').val();
		var cbo_recipe_for = $('#cbo_recipe_for').val();

		if (form_validation('cbo_company_id*cbo_recipe_for','Company*Recipe For')==false)
		{
			return;
		}
		else
		{
			...
			//var page_link = 'requires/aop_recipe_entry_controller.php?cbo_company_id='+cbo_company_id+'&action=systemid_popup';
			var page_link = 'requires/aop_recipe_entry_controller.php?cbo_company_id='+cbo_company_id+'&cbo_recipe_for='+cbo_recipe_for+'&action=systemid_popup';

			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=985px,height=390px,center=1,resize=1,scrolling=0','../');

			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
				var update_id_info=this.contentDoc.getElementById("hidden_update_id").value;	 //Access form field with id="emailfield"
				var update_id_info=update_id_info.split('_');
				if(update_id_info[0]!="")
				{
					freeze_window(5);
					$('#list_container_recipe_items').html('');
					get_php_form_data(update_id_info[0], "populate_data_from_search_popup", "requires/aop_recipe_entry_controller" );
					$('#txtAopRef').val(update_id_info[1]);
					show_list_view(update_id_info[0], 'recipe_item_details', 'recipe_items_list_view', 'requires/aop_recipe_entry_controller', '' ) ;
	...
	function fnc_item_details(color_data)
	{
		...
		show_list_view(cbo_company_id+"**"+cbo_store_id+"**"+color_id+"**"+update_id, 'item_details', 'list_container_recipe_items', 'requires/aop_recipe_entry_controller', '');
		setFilterGrid('tbl_list_search',-1);

		var response=return_global_ajax_value(cbo_company_id+"**"+cbo_store_id+"**"+color_id+"**"+update_id, 'check_water_ratio', '', 'requires/aop_recipe_entry_controller');
		//alert(response);
		$('#txt_water_ratio').val(trim(response));

		$("#txt_multi_color").attr("disabled",false);
		$("#txt_water_ratio").attr("disabled",false);
		$("#show").attr("disabled",false);
		$('#txt_multi_color').val(color_name);
		$('#hidd_newprod_id').val(new_prod_id);
		$('#cbo_store_id').val(cbo_store_id);
	...
	function fnResetForm()
	{
		...
		reset_form('recipeEntry_1','list_container_recipe_items*recipe_items_list_view','',"$('#cbo_recipe_for').val(),2");
	}
	...
	function openmypage_item_group()
	{
		...
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/aop_recipe_entry_controller.php?category='+category+'&action=itemgroup_popup','Search Group Name', 'width=580px,height=400px,center=1,resize=0,scrolling=0','../')
			emailwindow.onclose=function()
			{
	...
	function fnc_tot_ratio_calculate()
	{
		//alert();
		var row_num=$('#tbl_list_search tbody tr').length-1;
		// var total_ratio=0; var final_total_ratio=0;
		// for(var j=1; j<=row_num; j++)
		// {
		// 	total_ratio=(total_ratio*1)+($('#txt_ratio_'+j).val()*1);
		// }
		// $('#txt_days_ratio').val( total_ratio );
		// var water_ratio= $('#txt_water_ratio').val()*1;
		// final_total_ratio=total_ratio+water_ratio;
		// $('#txt_total_ratio').val( final_total_ratio );
		//.toFixed(4)
		var total_ratio=0; 
		for(var j=1; j<=row_num; j++)
		{
		...
		$('#txt_days_ratio').val( total_ratio );
		var water_ratio=100-total_ratio;
		$('#txt_water_ratio').val( water_ratio );
	...
	function copy_check(type)
	{
		...
		{
			$("#list_container_recipe_items").html('');
			show_list_view($('#update_id').val(), 'recipe_item_details', 'recipe_items_list_view', 'requires/aop_recipe_entry_controller', '' ) ;
			$('#txt_sys_id').val('');
			$('#update_id').val('');
			$('#txt_max_seq').val('');
			$('#txt_recipe_date').val('');
			...
			$('#txt_batch_id').val('');
			$('#cbo_within_group').val('');
			$('#txt_batch_color').val('');
			$('#hidden_batch_color_id').val('');
			$('#txt_batch_weight').val('');
			$('#txt_total_ratio').val(100);
			$('#txt_water_ratio').val(100);
			$('#txt_days_ratio').val(0);
			$('#txt_multi_color').val('');
			$("#txt_batch_number").attr("disabled",false);
			$("#txt_multi_color").attr("disabled",true);
			$("#txt_water_ratio").attr("disabled",true);
	...
	function openmypage_batchNo()
	{
		...
		else
		{
			...
			var page_link = 'requires/aop_recipe_entry_controller.php?cbo_company_id='+cbo_company_id+'&action=batch_popup';
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=1060px,height=420px,center=1,resize=1,scrolling=0','');

			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
				var batch_info=this.contentDoc.getElementById("hidden_batch_info").value;
				//alert(batch_info);
				var estr_data=batch_info.split("___");
				if(estr_data[0]!="")
				{
					freeze_window(5);
					$('#txt_batch_id').val(estr_data[0]);
					$('#txt_batch_number').val(estr_data[1]);
					$('#txt_order_id').val(estr_data[2]);
					$('#txt_order').val(estr_data[3]);
					//$('#cbo_location').val(estr_data[4]);
					$('#hid_job_no').val(estr_data[5]);
					$('#txtbuyerPo').val(estr_data[6]);
					$('#txtstyleRef').val(estr_data[7]);
					$('#cbo_buyer_name').val( estr_data[8] );
					$('#txtbuyerPoId').val(estr_data[9]);
					$('#hidden_batch_color_id').val(estr_data[10]);
					$('#txt_batch_color').val(estr_data[11]);
					$('#txt_batch_weight').val(estr_data[12]);
					$('#txt_batch_entry_form').val(estr_data[13]);
					$('#txtAopRef').val(estr_data[14]);
					$('#cbo_within_group').val(estr_data[15]);
					release_freezing();
				}
			}
		}
	...
	$('#txt_batch_number').live('keydown', function(e)
	{
		if (e.keyCode === 13)
		{
			e.preventDefault();

			var batch_no=trim($('#txt_batch_number').val());
			var cbo_company_id = $('#cbo_company_id').val();
			//alert(scanned_barcode+"**"+batch_no);

			var pi_id_prev=$('#pi_id').val();
			var batch_id_curr=trim(return_global_ajax_value( cbo_company_id+'_'+batch_no, 'populate_batch_id', '', 'requires/aop_recipe_entry_controller'));

			if(batch_id_curr=="")
			{
				alert('Batch No. is Not Valid');
				$('#messagebox_main', window.parent.document).fadeTo(100,1,function() //start fading the messagebox
				{
					$('#messagebox_main', window.parent.document).html('Batch No. Not Valid.').removeClass('messagebox').addClass('messagebox_error').fadeOut(5500);
				});
				$('#txt_batch_number').val('');
				return;
			}
			...
			{
				freeze_window(5);
				get_php_form_data(cbo_company_id+'_'+batch_id_curr, "load_data_to_form_batch", "requires/aop_recipe_entry_controller" );
				release_freezing();
			}
		}
	});
		...
		<form name="recipeEntry_1" id="recipeEntry_1">
			<fieldset style="width:920px;">
                <table width="910" align="center" border="0">
                    <tr>
                        <td colspan="3" align="right"><strong>Recipe  ID</strong></td>
                    ...
                    <tr>
                        ...
                        <td><input type="text" name="txt_recipe_date" id="txt_recipe_date" class="datepicker" style="width:140px;" readonly/></td>
                    </tr>
                    <tr>
                        <td class="must_entry_caption">Company Name</td>
                        <td><? echo create_drop_down( "cbo_company_id", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-Select Company-", 0, "load_drop_down('requires/aop_recipe_entry_controller', this.value, 'load_drop_down_location', 'location_td'); load_drop_down('requires/aop_recipe_entry_controller', this.value+'__'+document.getElementById('cbo_location').value+'__'+document.getElementById('cbo_recipe_for').value, 'load_drop_down_store', 'store_td'); fnc_load_party(this.value,document.getElementById('cbo_within_group').value);"); ?></td>
                        ...
                        <td>Batch Number</td>
                        <td>
                            <input type="text" name="txt_batch_number" id="txt_batch_number" class="text_boxes" style="width:140px;" placeholder="Browse" onDblClick="openmypage_batchNo()" readonly/>
                            <input type="hidden" name="variable_check" id="variable_check" style="width:60px"/>
                            <input type="hidden" name="txt_batch_id" id="txt_batch_id" style="width:60px"/>
                            <input type="hidden" name="txt_batch_entry_form" id="txt_batch_entry_form" style="width:60px"/>
                            <input type="hidden" name="cbo_within_group" id="cbo_within_group" style="width:60px"/>
                        </td>
                        <td style="display: none;">
                            <?php echo create_drop_down( "cbo_within_groupd", 150, $yes_no,"", 0, "--  --", 0, "fnc_load_party( document.getElementById('cbo_company_id').value, this.value);" ); ?>
                        </td>
                    </tr>
                    <tr>
                    	<td>Batch Color</td>
                        <td>
                            <input type="text" name="txt_batch_color" id="txt_batch_color" class="text_boxes" value="" style="width:140px;" readonly/>
                            <input type="hidden" value="" id="hidden_batch_color_id">
                        </td>
                        <td >Batch Weight </td>
                        <td>
                            <input type="text" name="txt_batch_weight" id="txt_batch_weight" class="text_boxes" style="width:140px;" />
                        </td>
                        <td>Work Order</td>
                        <td>
                            <input type="text" name="txt_order" id="txt_order" class="text_boxes" value="" style="width:140px;" readonly />
                    ...
                    <tr>
                    	...
                    	<td>AOP Ref.</td>
                    	<td><input name="txtAopRef" id="txtAopRef" type="text" class="text_boxes" style="width:140px" readonly /></td>
                    </tr>
                    <tr>
                    	<td>Remarks</td>
                        <td colspan="5"><input type="text" name="txt_remarks" id="txt_remarks" class="text_boxes" value="" style="width:735px;" /></td>
                        <input type="hidden" name="cbo_buyer_name" id="cbo_buyer_name" class="text_boxes" value="" style="width:735px;" />
                    </tr>
                    <tr style="display: none;">
                    	<td id="buyer_td_id"><? echo create_drop_down( "cbo_buyer_named", 150, $blank_array,"", 1, "--Select Party--", $selected, "",1 ); ?></td>
            ...
            <legend>Item Details</legend>
 				<table cellpadding="1" cellspacing="1" border="0" width="100%">
                	<tr>
                    	<td width="100" align="center" class="must_entry_caption" id="rec_color_td"><strong>Color</strong></td>
                        <td width="80" align="center" class="must_entry_caption" id="rec_store_td"><strong>Store</strong></td>
                        <td width="140" align="center"><strong>Item Category</strong></td>
                        <td width="130" align="center"><strong>Item Group</strong></td>
                        <td width="120" align="center"><strong>Sub Group Name</strong></td>
 					...
 					<tr>
                        ...
                        <td align="center" id="store_td"><? echo create_drop_down( "cbo_store_id", 150, $blank_array,"", "1", "-Store-", 0, "","1","","","","","","" );  ?></td>
                        ...
                        <td align="center">
                            ...
                            <Input name="txt_item_group" ID="txt_item_group" style="width:110px" value="" class="text_boxes" placeholder="Double Click to Search" onDblClick="openmypage_item_group();" readonly disabled />
                        </td>
                		<td align="center"><Input name="txt_subgroup_name" ID="txt_subgroup_name"  style="width:100px" class="text_boxes" placeholder="Write" disabled ></td>
                        <td align="center"><Input name="txt_description" ID="txt_description"  style="width:150px" class="text_boxes" placeholder="Write" disabled ></td>
                        <td align="center" style="display: none;"><Input name="txt_item_size" ID="txt_item_size"  style="width:60px" class="text_boxes" placeholder="Write" disabled ></td>

                        <td id="button_th" align="left">
                    ...
                    <tr>
                    	<td colspan="2" align="left"><strong>Dyes & Chemical Ratio :</strong> <Input name="txt_days_ratio" ID="txt_days_ratio"  style="width:97px" class="text_boxes_numeric" placeholder="Dispaly" disabled ></td>
                        <td align="right"><strong>Water Ratio :</strong></td>
                        <td >&nbsp;<strong><Input name="txt_water_ratio" ID="txt_water_ratio"  style="width:110px" class="text_boxes_numeric" value="100" placeholder="Write" onKeyUp="fnc_tot_ratio_calculate()"; readonly  ></strong></td>
                    	<td align="right"><strong>Total Ratio :</strong></td>
                        <td >&nbsp;<strong><Input name="txt_total_ratio" ID="txt_total_ratio"  style="width:150px" class="text_boxes_numeric" placeholder="Dispaly" value="100" disabled ></strong></td>
                    </tr>
                </table>
