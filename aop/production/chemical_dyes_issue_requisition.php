<?
/*********************Comments******************
* Purpose			: 	This form will create AOP Dyes And Chemical Issue Requisition  
* Functionality	:                                                                           
* JS Functions	:                                                                           
* Created by		:	Kausar                                                               	
...
echo load_html_head_contents("AOP Dyes And Chemical Issue Requisition ","../../", 1, 1, $unicode,1,1); 
//--------------------------------------------------------------------------------------------------------------------
...
<script>
	...
	function openmypage_recipe()
	{
		...
		{ 	
			...
			var page_link = 'requires/chemical_dyes_issue_requisition_controller.php?cbo_company_id='+cbo_company_id+'&recipe_id='+txt_recipe_id+'&action=recipe_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if(erecipedata[1]!="")
				{
					...
					$('#txtAopRef').val(erecipedata[8]);
					$('#cbo_store_name').val(erecipedata[9]);
					
					show_list_view(cbo_company_id+"**"+erecipedata[0]+"**"+0, 'item_details', 'list_container_recipe_items', 'requires/chemical_dyes_issue_requisition_controller', '');
	...
	function apply_last_update()
	{
		...
		if(recipe_id!="")
		{
			...
			show_list_view(cbo_company_id+"**"+recipe_id,'item_details','list_container_recipe_items','requires/chemical_dyes_issue_requisition_controller','../');
			$('#is_apply_last_update').val(1);
			release_freezing();
		} 
	}
	...
	function fnc_chemical_dyes_issue_requisition(operation)
	{
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title, "chemical_dyes_issue_requisition_print", "requires/chemical_dyes_issue_requisition_controller" );
			return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			...
			var mst_data=get_submitted_data_string('txt_req_no*update_id*cbo_company_name*cbo_location_name*txt_requisition_date*cbo_receive_basis*txt_recipe_id*txt_order_id*txt_job_no*is_apply_last_update*txtbuyerPoId*cbo_store_name',"../../");
	...
	{	
		...
		{   
			...
			if((reponse[0]==0 || reponse[0]==1))
			{
				...
				show_list_view(cbo_company_id+"**"+recipe_id+"**"+reponse[2], 'item_details', 'list_container_recipe_items', 'requires/chemical_dyes_issue_requisition_controller', '');
	...
	function open_requisitionpopup()
	{
		...
		var page_link='requires/chemical_dyes_issue_requisition_controller.php?action=requisition_popup&company='+company; 
		var title="Requisition Popup Info";
		...
		emailwindow.onclose=function()
		{
			...
			if(reqnId!="")
			{
				...
				show_list_view(company+"**"+recipe_id+"**"+reqnId, 'item_details', 'list_container_recipe_items', 'requires/chemical_dyes_issue_requisition_controller', '');
				$('#txtAopRef').val(ereqsn_data[2]);
				fnc_req_calculate(2,0);
			}
		}
	}
	...
	function fnc_print_item_wise_summary()
	{
		var update_id=$('#update_id').val();
		if(update_id!="")
		{
		 	var report_title=$( "div.form_caption" ).html();
			print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title, "print_requisition_item_wise_summary", "requires/chemical_dyes_issue_requisition_controller" );
			return;
		}
		else
		{
            ...
            <legend>Dyes And Chemical Issue Requisition</legend>
                <table width="810" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td width="155"><? echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-Select Company-", $selected, "rcv_basis_reset(); load_drop_down( 'requires/chemical_dyes_issue_requisition_controller', this.value, 'load_drop_down_location', 'location_td')",""); ?>
                        </td>
                        ...
                        <td ><input type="text" name="txt_requisition_date" id="txt_requisition_date" class="datepicker" style="width:140px;" placeholder="Select Date" /></td>
                    </tr>
                    <tr>
                        <td class="must_entry_caption">Requisition Basis</td>
                        ...
                        <td>
                            ...
                            <input class="text_boxes" type="hidden" name="txt_order_id" id="txt_order_id" readonly/>
                            <input class="text_boxes" type="hidden" name="txt_job_no" id="txt_job_no" readonly /> 
                        </td>
                    </tr>
                    <tr>
                    	 <td class="must_entry_caption">Store Name</td>
                         <td><? echo create_drop_down( "cbo_store_name", 150, "select lib_store_location.id,lib_store_location.store_name from lib_store_location,lib_store_location_category where lib_store_location.id=lib_store_location_category.store_location_id and lib_store_location.status_active=1 and lib_store_location.is_deleted=0  and lib_store_location_category.category_type in(5,6,7,22,23) group by lib_store_location.id,lib_store_location.store_name order by lib_store_location.store_name","id,store_name", 1, "-- Select Store --", $storeName, "",1 ); ?></td>
                    	<td>Buyer Po</td>
                    ...
                    <tr>
                        ...
                        <input type="hidden" name="update_dtls_id" id="update_dtls_id" readonly>
							<? echo load_submit_buttons($permission, "fnc_chemical_dyes_issue_requisition", 0,1,"fnResetForm();",1); ?> 
                             <input type="button" value="Print2" id="btn_consignment" name="btn_consignment" class="formbutton" style="width:100px;" onClick="fnc_print_item_wise_summary()" />
