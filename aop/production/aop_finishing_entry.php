<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create AOP Finishing Entry
Functionality	:	
JS Functions	:
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("AOP Finishing Entry", "../../",1, 1,$unicode,1,'');
?>
<script>
	...
	{ 
		...
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox','iframe', 'requires/aop_finishing_entry_controller.php?action=finishing_id_popup&data='+data,'AOP Finishing Popup Info', 'width=800px, height=400px, center=1, resize=0, scrolling=0','../')
			emailwindow.onclose=function()
			{
				...
				if (theemail.value!="")
				{
					freeze_window(5);
					//var data=data.split("**");
					get_php_form_data(theemail.value, "load_php_data_to_form_mst", "requires/aop_finishing_entry_controller" );
					show_list_view(theemail.value,'fabric_finishing_list_view','fabric_finishing_list_view','requires/aop_finishing_entry_controller','setFilterGrid("list_view",-1)');

					//show_list_view(document.getElementById('order_no_id').value+"_"+document.getElementById('process_id').value+"_"+document.getElementById('txt_batch_id').value+"_"+document.getElementById('cbo_company_id').value,'show_fabric_desc_listview','list_fabric_desc_container','requires/aop_finishing_entry_controller','');
	...
	{ 
  		if ( form_validation('cbo_company_id*txt_process_id','Company Name*Process Name')==false )
		{
			return;
		}
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox','iframe','requires/aop_finishing_entry_controller.php?action=batch_numbers_popup&data='+data,'Batch Numbers Popup', 'width=900px, height=420px, center=1, resize=0, scrolling=0','../')
			emailwindow.onclose=function()
			{
				...
				{
					freeze_window(5); 
					reset_form('','','txt_description*txt_gsm*txt_dia_width*txt_product_qnty*txt_reject_qty*txt_roll_no*cbo_machine_id');
					get_php_form_data( data[0]+"_"+data[3]+"_"+data[4]+"_"+data[5], "load_php_data_to_form_batch", "requires/aop_finishing_entry_controller" );
					$('#txt_aop_ref').val(data[1]);
					
					show_list_view(document.getElementById('order_no_id').value+"_"+document.getElementById('txt_process_id').value+"_"+document.getElementById('txt_batch_id').value+"_"+document.getElementById('cbo_company_id').value+"_"+data[3],'show_fabric_desc_listview','list_fabric_desc_container','requires/aop_finishing_entry_controller','');
	...
	function openmypage_qnty(order_no_id)
	{
		...
		var page_link = 'requires/aop_finishing_entry_controller.php?action=order_qnty_popup&data='+data;
	...
	function subcon_fabric_finishing(operation)
	{
		...
		if(operation==4)
		{
			 ...
			 print_report( $('#cbo_company_id').val()+'*'+$('#txt_finishing_id').val()+'*'+report_title, "subcon_fabric_finishing_print", "requires/aop_finishing_entry_controller" ) 
			 return;
		}
		else if(operation==2)
		{
		...
		{
			if( form_validation('cbo_company_id*cbo_party_name*txt_finishing_date*txt_process_id*txt_dia_width*cbo_uom','Company Name*Party Name*Finishing Date*Process Name*Dia/Width*UOM')==false )
			{
				return;
			}
			...
			else
			{
				...
				var data="action=save_update_delete&operation="+operation+get_submitted_data_string('txt_finishing_id*cbo_receive_basis*cbo_company_id*cbo_location_name*cbo_party_name*txt_finishing_date*txt_chal_no*txt_remarks*update_id*txt_batch_id*txt_batch_no*txt_batch_ext_no*hidden_dia_type*txt_process_id*order_no_id*item_order_id*txt_description*comp_id*txt_color*hidden_color_id*txt_gsm*txt_dia_width*txt_product_qnty*txt_receive_qnty*txt_reject_qty*txt_roll_no*update_id_qnty*cbo_floor_name*cbo_machine_id*cboShift*cbo_uom*update_id_dtl*txt_buyer_po_id*cbo_body_part',"../../");
				//alert (data); return;
				freeze_window(operation);
				http.open("POST","requires/aop_finishing_entry_controller.php",true);
	...
	function subcon_fabric_finishing_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			if((reponse[0]==0 || reponse[0]==1))
			{
				...
				var cbo_company_id = $('#cbo_company_id').val();
				show_list_view(reponse[1]+'_'+cbo_company_id,'fabric_finishing_list_view','fabric_finishing_list_view','requires/aop_finishing_entry_controller','setFilterGrid("list_view",-1)');
				reset_form('fabricfinishing_1','','','txt_finishing_date,<? echo date("d-m-Y"); ?>','','txt_finishing_id*cbo_receive_basis*cbo_company_id*cbo_location_name*cbo_party_name*txt_chal_no*txt_remarks*update_id*txt_batch_no*txt_batch_ext_no*txt_batch_id*order_no_id*txt_process_id');
				//$('#txt_process_id').val('');
				show_list_view(document.getElementById('order_no_id').value+'_'+document.getElementById('txt_process_id').value+'_'+document.getElementById('txt_batch_id').value+"_"+document.getElementById('cbo_company_id').value,'show_fabric_desc_listview','list_fabric_desc_container','requires/aop_finishing_entry_controller','');
	...
	function openmypage_process()
	{
		...
		var page_link = 'requires/aop_finishing_entry_controller.php?txt_process_id='+txt_process_id+'&action=process_name_popup';
...
function clearChallan()
{
	...
	$("#fabric_finishing_list_view").html("");
	$("#txt_finishing_id").val("");
	$("#update_id").val("");
	$("#update_id_dtl").val("");
	show_list_view(document.getElementById('order_no_id').value+"_"+document.getElementById('txt_process_id').value+"_"+document.getElementById('txt_batch_id').value+"_"+document.getElementById('cbo_company_id').value,'show_fabric_desc_listview','list_fabric_desc_container','requires/aop_finishing_entry_controller','');
}
		            ...
		            <tr>
		                ...
		                <fieldset>
		                    <table cellpadding="0" cellspacing="2" width="100%">
		                        <tr>
		                            <td align="right" colspan="3"><strong>Finishing ID </strong></td>
		                        ...
		                        <tr>
		                            ...
		                            <td width="140">
										<?php 
											echo create_drop_down( "cbo_company_id",140,"select id,company_name from lib_company comp where is_deleted=0 and status_active=1 $company_cond order by company_name","id,company_name", 1, "--Select Company--", $selected, "load_drop_down( 'requires/aop_finishing_entry_controller', this.value, 'load_drop_down_location', 'location_td');","","","","","",3);	
		                                ?>
		                            </td>
		                ...
		                <legend>New Entry</legend>
		                    <table  cellpadding="0" cellspacing="2" width="100%" align="center">
		                        <tr>
			                        ...
			                        <td>
			                        	<? echo create_drop_down( "txt_process_id", 140, $conversion_cost_head_array,"", 1, "-- Select --", $selected, "clearChallan()" ,"","35,133,148,150,207,84,156,209,93,220,221,230,231,232,233,234,235,236,237"); ?>
