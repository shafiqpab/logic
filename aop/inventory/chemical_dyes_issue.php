...
<script>
	...
	function fnc_chemical_dyes_issue_entry(operation)
	{
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#txt_mrr_no').val()+'*'+$('#txt_buyer_po').val()+'*'+$('#txt_buyer_style').val()+'*'+$('#txt_apo_ref').val(),'chemical_dyes_issue_print','requires/chemical_dyes_issue_controller');
			return;
		}
		else if(operation==0 || operation==1 || operation==2)
		{
			if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][308]);?>'){
				if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][308]);?>','<? echo implode('*',$_SESSION['logic_erp']['field_message'][308]);?>')==false)
				{
			...
			var dataString = "txt_mrr_no*update_id*cbo_company_name*cbo_location_name*cbo_issue_basis*cbo_issue_purpose*cbo_loan_party*txt_req_no*txt_req_id*txt_issue_date*txt_recipe_no*txt_recipe_id*txt_job_no*txt_order_no*hidden_order_id*cbo_store_name*txt_buyer_po*hidden_buyer_po_id*cbo_sub_process*txt_buyer_style*txt_batch_no*hidden_batch_id*txt_apo_ref*txt_remarks*variable_lot";
	...
	{	
		if(http.readyState == 4) 
		{
			...
			{
				disable_enable_fields( 'cbo_company_name*cbo_issue_basis*txt_req_no', 1, '', '' ); //*cbo_sub_process
	...
	function open_reqpopup()
	{
		...
		emailwindow.onclose=function()
		{
			...
			$("#cbo_location_name").val(requ_data_ref[11]);
			$("#hidden_batch_id").val(requ_data_ref[12]);
			$("#txt_batch_no").val(requ_data_ref[13]);
			$("#txt_apo_ref").val(requ_data_ref[14]);
	...
	function fnc_item_details(sub_process_id,is_update,store_id)
	{
		...
		show_list_view(cbo_company_id+'**'+sub_process_id+"**"+txt_recipe_id+"**"+cbo_issue_basis+"**"+is_update+"**"+txt_req_id+"**"+cbo_store_name+"**"+hidden_posted_account+"**"+variable_lot, 'item_details', 'list_container_recipe_items', 'requires/chemical_dyes_issue_controller', '');
		if(is_update !="")
		{
	...
	function open_mrrpopup()
	{
		...
		var title="Issue Number Pop-up Info";
		...
		emailwindow.onclose=function()
		{
			...
			disable_enable_fields( 'cbo_company_name*cbo_issue_basis*txt_req_no', 1, '', '' );
		}
	}
	...
	function fn_active_inactive(sid)
	{
		//alert(sid);
		if(sid==15 || sid==5)
                    ...
                    <table  width="950" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                        ...
                        <tr>
                         ...
                         <tr>
                            ...
                            <td>
                            <? 
                            echo create_drop_down( "cbo_issue_purpose", 170, $general_issue_purpose,"", 1, "-- Select Purpose --",77, "fn_active_inactive(this.value)", "","1,5,15,61,63,65,69,77");
                            ?>
                            </td>
                        ...
                        <tr>
                            ...
                            <td id="store_td" >
							<? 
                                echo create_drop_down( "cbo_store_name", 170, "select a.id,a.store_name from lib_store_location a,lib_store_location_category b where a.id=b.store_location_id and a.status_active=1 and a.is_deleted=0 and b.category_type in(5,6,7,22,23) $store_location_credential_cond group by a.id,a.store_name order by a.store_name","id,store_name", 1, "-- Select Store --", "", "fn_sub_process_enable(this.value);", 1,"");
                            ?>
                            </td>
                        	<td align="right">Batch No</td>
                            <td>
                            <input class="text_boxes"  type="text" name="txt_batch_no" id="txt_batch_no" placeholder="Display" style="width:160px;" readonly />
                        ...
                        <tr>
                            ...
                            <td align="right">AOP Ref</td>
                            <td>
                            <input class="text_boxes"  type="text" name="txt_apo_ref" id="txt_apo_ref" placeholder="Display" style="width:160px;" readonly /> 
                            </td>
                        </tr>
                        <tr>
                           ...
                           // echo create_drop_down( "cbo_sub_process", 170, "select id, color_name from lib_color where status_active=1 and is_deleted=0","id,color_name", 1, "-- Select--", "", "fnc_item_details(this.value,'','')",1);
                            ...
                            <td colspan="3">
                            <input class="text_boxes"  type="text" name="txt_remarks" id="txt_remarks" placeholder="Write" style="width:480px;" /> 
                            </td>
                        </tr>
                    </table>
                </fieldset>
