            ...
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="1120" class="rpt_table" id="tbl_list_search">
                ...
                else
                {
					...
					foreach ($nameArray as $selectResult)
					{
						...
						{
							//$stock_qty=$stock_qty_arr[$company_id][$cbo_store][$selectResult[csf('item_category_id')]][$selectResult[csf('id')]]['stock']+$selectResult[csf('req_qny_edit')];
 	...
 	<table align="center" cellspacing="0" width="920"  border="1" rules="all" class="rpt_table"  id="list_view" >
	    ...
	    foreach($sql_result as $inf)
		{
			...
			<tr bgcolor="<? echo $bgcolor; ?>" onClick='js_set_value("<? echo $inf[csf("issue_number")]."_".$inf[csf("id")]."_".$inf[csf("issue_basis")]."_".$inf[csf("issue_purpose")]."_".$inf[csf("is_posted_account")];?>")' style="cursor:pointer">
                ...
                <td width="100"><? echo $yarn_issue_purpose[$inf[csf("issue_purpose")]]; ?></td>
	...
	if( $operation==0 ) // Insert Here----------------------------------------------------------
	{
		...
		for ($i=1;$i<$total_row;$i++)
		{
			...
			if(str_replace("'",'',$$txt_item_cat)==22)
			{
				$sql_store="SELECT a.id, b.store_id, sum((case when transaction_type in(1,4,5) then cons_quantity else 0 end)-(case when transaction_type in(2,3,6) then cons_quantity else 0 end)) as store_stock
				from product_details_master a, inv_transaction b 
				where a.id=b.prod_id and a.company_id=$cbo_company_name and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0 and b.store_id=$issue_store_id and a.id in(".str_replace("'",'',$$txt_prod_id).")
				group by a.id, b.store_id";
				//echo $sql_store;
				$store_result=sql_select( $sql_store );
		...
		if($all_prod_id_ac!="")
		{
			$auxChemData = sql_select("SELECT prod_id, id, cons_rate, balance_qnty, balance_amount from inv_transaction where prod_id in ($all_prod_id_ac) and balance_qnty>0 and transaction_type in (1,4,5) and item_category in(7,22,23) order by transaction_date,id $cond_lifofifoA");
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		for ($i=1;$i<$total_row;$i++)
		{
			...
			if(str_replace("'",'',$$txt_item_cat)==22)
			{
				$sql_store="SELECT a.id, b.store_id, sum((case when transaction_type in(1,4,5) then cons_quantity else 0 end)-(case when transaction_type in(2,3,6) then cons_quantity else 0 end)) as store_stock
				from product_details_master a, inv_transaction b 
				where a.id=b.prod_id and a.company_id=$cbo_company_name and b.store_id=$issue_store_id and a.id in(".str_replace("'",'',$$txt_prod_id).")
				group by a.id, b.store_id";
				//echo $sql_store;
				$store_result=sql_select( $sql_store );
				$store_prod_qnty=$store_result[0][csf("store_stock")]*1;
...
?>
>>>>>>> deacb37e2fcf001d90dc62ab7e41adfdba703628
