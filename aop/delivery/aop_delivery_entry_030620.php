...
<script>
	...
	{ 
		...
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox','iframe', 'requires/aop_delivery_entry_controller.php?action=delv_popup&data='+data,'Delivery Popup', 'width=800px, height=400px, center=1, resize=0, scrolling=0','')
			emailwindow.onclose=function()
			{
				...
				if (theemail.value!="")
				{
					...
					show_list_view(datas[3],'show_fabric_desc_listview','list_fabric_desc_container','requires/aop_delivery_entry_controller','setFilterGrid("list_view",-1)');
	...
	function fnc_print_Challan()
	{
			 ...
			 print_report( $('#cbo_company_id').val()+'*'+$('#txt_delevery_id').val()+'*'+report_title+'*'+$('#cbo_within_group').val(), "aop_delevery_entry_print_Challan", "requires/aop_delivery_entry_controller" ) 
			 return;
	...
	function aop_delevery_entry(operation)
	{
		...
		{
			 var report_title=$( "div.form_caption" ).html();
			 print_report( $('#cbo_company_id').val()+'*'+$('#txt_delevery_id').val()+'*'+report_title, "aop_delevery_entry_print", "requires/aop_delivery_entry_controller" ) 
			 return;
		}
		else if(operation==2)
		{
	...
	function aop_delevery_entry_reponse()
	{
		if(http.readyState == 4) 
		{
			...
			if((reponse[0]==0 || reponse[0]==1))
			{
				...
				//alert(reponse[4]);
				show_list_view(reponse[4],'show_fabric_desc_listview','list_fabric_desc_container','requires/aop_delivery_entry_controller','setFilterGrid("list_view",-1)');
				show_list_view(reponse[1],'fabric_finishing_list_view','aop_delevery_list_view','requires/aop_delivery_entry_controller','setFilterGrid("list_view",-1)');
				fnResetForm();
				set_button_status(0, permission, 'aop_delevery_entry',1,1);
			}
			release_freezing();
	...
	function openmypage_order()
	{
		...
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link,'Order Number Form', 'width=1100px,height=400px,center=1,resize=0,scrolling=0','../')
			emailwindow.onclose=function()
			{
		        ...
		        <table cellpadding="0" cellspacing="1" width="100%">
		                ...
		                <legend>New Entry</legend>
		                    <table  cellpadding="0" cellspacing="2" width="100%" align="center">
                                ...
                                <tr>
		                            ...
		                            <td>
		                                <input type="text" name="txt_dyeing_batch" id="txt_dyeing_batch" class="text_boxes" style="width:130px" tabindex="11"/>
		                            </td>
		                        </tr>
		                        <tr>
		                        	<td >Remarks</td>
		            ...
		            <tr>
		                <td colspan="6" align="center" class="button_container">
							<? 
								$date=date('d-m-Y');
								echo load_submit_buttons($permission, "aop_delevery_entry", 0,1,"",1);
		                    ?> 
                            <input type="button" value="Print2" id="btn_consignment" name="btn_consignment" class="formbutton" style="width:100px;" onClick="fnc_print_Challan()" />
