...
<script>
	...
	function openmypage_job()
	{
		...
		emailwindow.onclose=function()
		{
			...
			{//alert(theemail.value);
				...
				show_list_view(2+'_'+ex_data[1]+'_'+within_group+'_'+$("#update_id").val(),'order_dtls_list_view','aop_details_container','requires/aop_order_entry_controller','setFilterGrid(\'list_view\',-1)');
	...
	function fnc_job_order_entry( operation )
	{
		...
		if(operation==0)
		{
			...
			if(!empty($_SESSION['logic_erp']['mandatory_field'][278][1])) echo " isFileMandatory = ". $_SESSION['logic_erp']['mandatory_field'][278][1] . ";\n";
			?>
			if($("#multiple_file_field")[0].files.length==0 && isFileMandatory!="" && $('#txt_job_no').val()=='')
			{
		...
		if ('<?php echo implode('*', arrayExclude($_SESSION['logic_erp']['mandatory_field'][198],array(1))); ?>') 
		{
	...
	function fnc_job_order_entry_response()
	{
		if(http.readyState == 4) 
		{
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				show_list_view(2+'_'+response[1]+'_'+within_group+'_'+response[2],'order_dtls_list_view','aop_details_container','requires/aop_order_entry_controller','setFilterGrid(\'list_view\',-1)');
				set_button_status(1, permission, 'fnc_job_order_entry',1);
	...
	<div style="width:100%;" align="center">
			...
			<legend>AOP Order Entry</legend>
                <table width="830" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td width="160"><? echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/aop_order_entry_controller', this.value+'_'+1, 'load_drop_down_location', 'location_td'); location_select(); fnc_load_party(1,document.getElementById('cbo_within_group').value);exchange_rate(document.getElementById('cbo_currency').value);"); ?>
                        </td>
        ...
        <fieldset style="width:1870px;">
           <legend>AOP Order Details Entry
                ...
                <table width="1800px" cellpadding="0" cellspacing="2" border="1" class="rpt_table" rules="all" id="tbl_dtls_aop">
