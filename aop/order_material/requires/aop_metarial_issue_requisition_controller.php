...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   
	{
		...
		if($db_type==0)
		{
			$new_requisition_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name),'', 'AOPMIR' , date("Y",time()), 5, "select id,prefix_no,prefix_no_num from sub_issue_requisition_mst where company_id=$cbo_company_name and requisition_type='$requisition_type' and entry_form=474 and YEAR(insert_date)=".date('Y',time())." order by id desc ", "prefix_no", "prefix_no_num" ));
		}
		else if($db_type==2)
		{
			$new_requisition_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name),'', 'AOPMIR' , date("Y",time()), 5, "select id,prefix_no,prefix_no_num from sub_issue_requisition_mst where company_id=$cbo_company_name and requisition_type='$requisition_type' and entry_form=474 and TO_CHAR(insert_date,'YYYY')=".date('Y',time())." order by id desc ", "prefix_no", "prefix_no_num" ));
		}

		if(is_duplicate_field( "a.remarks", "sub_issue_requisition_mst a, sub_issue_requisition_dtls b", "a.sys_no='$new_requisition_no[0]' and a.remarks=$txt_requisition_remarks and b.order_id=$order_no_id and b.material_description=$txt_material_description and b.color_id=$color_id" )==1)
		{
		...
		$data_array="(".$id.",'474','".$new_requisition_no[1]."','".$new_requisition_no[2]."','".$new_requisition_no[0]."','".$requisition_type."',".$cbo_company_name.",".$cbo_location_name.",".$cbo_party_name.",".$txt_requisition_remarks.",".$txt_requesition_date.",".$cbo_within_group.",".$txt_job_no.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";  
		...
		$field_array2="id, mst_id, receive_qty, job_dtls_id, buyer_po_id,fabric_details_id,entry_form,receive_dtls_id,receive_id,job_id,quantity, inserted_by, insert_date, status_active, is_deleted";
		...
		for($i=1; $i<=$total_row; $i++)
		{
			...
			$data_array2.="(".$id1.",'".$id."',".$$txtreceiveqty.",".$$ordernoid.",".$$txtbuyerPoId.",".$$fabricdtlsid.",'474',".$$receivedtlsid.",".$$receiveid.",".$$jobid.",".$$txtrequisitionqty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			 
			$id1=$id1+1; $add_commaa++;
		}
		//echo "10**INSERT INTO sub_issue_requisition_mst (".$field_array.") VALUES ".$data_array; disconnect($con); die;
		$flag=1;
		//.echo "10**INSERT INTO sub_issue_requisition_dtls (".$field_array2.") VALUES ".$data_array2; disconnect($con); die;
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");  
				echo "0**".str_replace("'",'',$txt_requesition_no)."**".str_replace("'",'',$id)."**".str_replace("'",'',$txt_job_no)."**".str_replace("'",'',$cbo_within_group);
			}
			else
			{
				mysql_query("ROLLBACK"); 
				echo "10**".str_replace("'",'',$txt_requesition_no)."**".str_replace("'",'',$id)."**".str_replace("'",'',$txt_job_no)."**".str_replace("'",'',$cbo_within_group);
			}
		}
		else if($db_type==2)
		{
	...
	else if ($operation==1)   // Update Here
	{
		...
		/*$iss_number=return_field_value( "sys_no", "sub_issue_requisition_mst"," embl_job_no=$txt_job_no and status_active=1 and is_deleted=0 and requisition_type=2");
		if($iss_number){
			echo "emblIssue**".str_replace("'","",$txt_job_no)."**".$iss_number;
			disconnect($con); die;
		}*/
		
		$rec_sql_dtls="SELECT b.id from sub_issue_requisition_dtls b, sub_issue_requisition_mst a where a.id=b.mst_id and a.id=$update_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.requisition_type=2";//
		...
		$field_arr_up="quantity*job_dtls_id*buyer_po_id*fabric_details_id*updated_by*update_date";
		...
		for($i=1; $i<=$total_row; $i++)
		{
			...
			if(str_replace("'","",$$updatedtlsid)=="")
			{
				...
				$data_array2.="(".$id1.",".$update_id.",".$$txtreceiveqty.",".$$ordernoid.",".$$txtbuyerPoId.",".$$fabricdtlsid.",'474',".$$receivedtlsid.",".$$receiveid.",".$$jobid.",".$$txtrequisitionqty.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
				$id_arr_rec[]=$id1;
				$id1=$id1+1; $add_commaa++;
			}
			else if(str_replace("'","",$$updatedtlsid)!="")
			{
				$data_arr_up[str_replace("'","",$$updatedtlsid)]=explode("*",("".$$txtrequisitionqty."*".$$ordernoid."*".$$txtbuyerPoId."*".$$fabricdtlsid."*".$user_id."*'".$pc_date_time."'"));
		...
		if(str_replace("'",'',$distance_delete_id)!="")
		{
			...
			$rID4=execute_query(bulk_update_sql_statement( "sub_issue_requisition_dtls", "id", $field_array_del,$data_array_del,$ex_delete_id),1);
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");  
				echo "1**".str_replace("'",'',$txt_requesition_no)."**".str_replace("'",'',$update_id)."**".str_replace("'",'',$txt_job_no)."**".str_replace("'",'',$cbo_within_group);	
			}
			else
			{
				mysql_query("ROLLBACK"); 
				echo "10**".str_replace("'",'',$txt_requesition_no)."**".str_replace("'",'',$update_id)."**".str_replace("'",'',$txt_job_no)."**".str_replace("'",'',$cbo_within_group);
			}
		}
		else if($db_type==2)
		{
	...
	else if ($operation==2)   // delete
	{
		...
		{*/
			...
			{
				$rID=sql_update("sub_issue_requisition_dtls",$field_array,$data_array_dtls,"id",$update_id2,1); //disconnect($con); die;
			}
			else
			{*/
			$flag=1;
...
if ($action=="Requisition_popup")
{
	...
	<script>
		...
		{   //alert(company+'_'+party_name+'_'+within_group);	
			load_drop_down( 'aop_metarial_issue_requisition_controller', company+'_'+within_group+'_'+party_name, 'load_drop_down_buyer_pop', 'buyer_td' );
		}
                ...
                <table width="870" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    <thead>
                    	...
                    	<tr>                	 
                            ...
                            <th width="80" style="display:none">Remarks</th>
                    ...
                    <tbody>
                        <tr class="general">
							...
							<? 
								echo create_drop_down( "cbo_company_name", 140, "SELECT id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $company, "load_drop_down( 'aop_metarial_issue_requisition_controller', this.value+'_'+".$within_group.", 'load_drop_down_buyer_pop', 'buyer_td' );"); ?>
                            </td>
                            <td>
							<?
								echo create_drop_down( "cbo_within_group", 50, $yes_no,"", 0, "--  --",$within_group, "load_drop_down( 'aop_metarial_issue_requisition_controller',document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_buyer_pop', 'buyer_td' );" ); ?>
							</td>
                            <td id="buyer_td">
								<? 
...
if($action=="create_receive_search_list_view")
{
	...
	if($search_type==1)
	{
		...
		if ($data[5]!='') $challan_no_cond=" and a.remarks='$data[5]'"; else $challan_no_cond="";
	}
	else if($search_type==4 || $search_type==0)
	{
		...
		if ($data[5]!='') $challan_no_cond=" and a.remarks like '%$data[5]%'"; else $challan_no_cond="";
	}
...
{
	$nameArray=sql_select( "SELECT id, sys_no, company_id, location_id, party_id, requisition_date, remarks,within_group, embl_job_no from sub_issue_requisition_mst where id='$data' and status_active=1 and is_deleted=0" ); 
	foreach ($nameArray as $row)
	{	
		...
		echo "load_drop_down( 'requires/aop_metarial_issue_requisition_controller', document.getElementById('cbo_company_name').value, 'load_drop_down_location', 'location_td' );\n";
		
		echo "load_drop_down( 'requires/aop_metarial_issue_requisition_controller', document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_within_group').value, 'load_drop_down_buyer', 'buyer_td' );\n"; 		
		...
		//echo "set_button_status(0, '".$_SESSION['page_permission']."', 'fnc_aop_material_requisition',1);\n";
...
if ($action=="fabric_finish_popup")
{
	...
	function toggle( x, origColor ){
		...
		{
			//alert();
			load_drop_down( 'aop_metarial_issue_requisition_controller', company+'_'+within_group+'_'+party, 'load_drop_down_buyer', 'buyer_td' );
                ...
                <table width="850" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+<? echo $data[3];?>+'_'+document.getElementById('txt_aop_ref').value, 'create_fabric_finish_search_list_view', 'search_div', 'aop_metarial_issue_requisition_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:70px;" />
                            </td>
                        </tr>
                        <tr>
...
{	
	...
	else if($db_type==2)
	{
		...
		*/
		...
		$pre_qty_arr=array();
		$sql_rec="SELECT a.id, a.mst_id, a.quantity, a.uom, a.job_dtls_id, a.buyer_po_id, a.remarks,a.fabric_details_id from sub_issue_requisition_dtls a, sub_issue_requisition_mst b where b.id=a.mst_id and b.requisition_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	...
	order by a.id DESC";*/
	 ...
	 $sql= "SELECT a.id,c.mst_id,c.id as fab_process_dtlsid, a.subcon_job, a.job_no_prefix_num, $ins_year_cond as year, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date,c.batch_issue_qty, $color_id_str as color_id, $buyer_po_id_cond as buyer_po_id,a.aop_reference 
	from subcon_ord_mst a, subcon_ord_dtls b ,pro_grey_batch_dtls c,inv_receive_mas_batchroll d ,sub_material_mst e, sub_material_dtls f
	where a.entry_form=278 and a.subcon_job=b.job_no_mst and a.id=b.mst_id  and a.order_no=c.booking_no and b.booking_dtls_id=c.booking_dtls_id  and c.mst_id=d.id and d.entry_form=91   and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $order_rcv_date $company $buyer $withinGroup $search_com_cond $po_idsCond $aop_cond and e.id=f.mst_id  and  b.id=f.job_dtls_id  and e.trans_type=1 and e.entry_form='279' and  c.id=f.fabric_details_id and  e.status_active=1 and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 
	...
	from subcon_ord_mst a, subcon_ord_dtls b ,sub_material_mst d, sub_material_dtls e
	where a.entry_form=278 and a.subcon_job=b.job_no_mst and a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $order_rcv_date $company $buyer $withinGroup $search_com_cond $po_idsCond $aop_cond  and d.id=e.mst_id  and  b.id=e.job_dtls_id  and d.trans_type=1 and d.entry_form='279' and   d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0
                ...
                <table width="850" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+<? echo $data[3];?>+'_'+document.getElementById('txt_aop_ref').value, 'create_job_search_list_view', 'search_div', 'aop_metarial_issue_requisition_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:70px;" />
                            </td>
                        </tr>
                        <tr>
                            <td colspan="6" align="center" valign="middle">
...
if($action=="load_php_dtls_form")
{
	...
	if($within_group==1)
	{
		...
		$sql_rec="SELECT a.id, a.mst_id, a.quantity, a.uom, a.job_dtls_id, a.buyer_po_id, a.remarks,a.fabric_details_id,a.receive_dtls_id from sub_issue_requisition_dtls a, sub_issue_requisition_mst b where b.id=a.mst_id and b.embl_job_no='$jobno' and b.requisition_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
		$sql_rec_res =sql_select($sql_rec);
		foreach ($sql_rec_res as $row)
		{
			
				$pre_qty_arr[$row[csf("fabric_details_id")]][$row[csf("receive_dtls_id")]]['qty']+=$row[csf("quantity")];
		}
	}
	else if($within_group==2)
	{
		/*$sql_rec="SELECT a.id, a.mst_id, a.quantity, a.uom, a.job_dtls_id, a.buyer_po_id, a.remarks,b.fabric_details_id from sub_issue_requisition_dtls a, sub_issue_requisition_mst b where b.id=a.mst_id and b.embl_job_no='$jobno' and b.requisition_type=2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";*/
	...
	if($within_group==1)
	{
	 		if($update_id!=0)
			{
				   ...
				   $sql_job="SELECT a.id, c.id as fabric_dtlsid,a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id as po_id, b.buyer_po_id, b.gmts_item_id, b.body_part, c.batch_issue_qty as order_quantity, b.order_uom , b.body_part, b.construction, b.composition, b.gsm, b.grey_dia, b.gmts_color_id, b.item_color_id, b.fin_dia, b.aop_color_id, b.lib_yarn_deter,b.booking_dtls_id,f.receive_qty as receive_qty,f.receive_dtls_id as receive_dtls_id,f.receive_id as receive_id,f.id as update_dtls_id,f.quantity as requisition_qty
		from subcon_ord_mst a, subcon_ord_dtls b,pro_grey_batch_dtls c,inv_receive_mas_batchroll d ,sub_issue_requisition_mst e, sub_issue_requisition_dtls f  
		where a.entry_form=278 and a.subcon_job=b.job_no_mst and a.id=b.mst_id  and a.order_no=c.booking_no and b.booking_dtls_id=c.booking_dtls_id   and c.mst_id=d.id and d.entry_form=91 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.subcon_job='$jobno' and e.id=$update_id and e.id=f.mst_id  and  b.id=f.job_dtls_id  and e.requisition_type=2 and e.entry_form='474' and   e.status_active=1 and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and c.id=f.fabric_details_id  order by b.id ASC";
			}
			else
			{
		
		 $sql_job="SELECT a.id, c.id as fabric_dtlsid,a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id as po_id, b.buyer_po_id, b.gmts_item_id, b.body_part, c.batch_issue_qty as order_quantity, b.order_uom , b.body_part, b.construction, b.composition, b.gsm, b.grey_dia, b.gmts_color_id, b.item_color_id, b.fin_dia, b.aop_color_id, b.lib_yarn_deter,b.booking_dtls_id,f.quantity as receive_qty,f.id as receive_dtls_id,e.id as receive_id
		from subcon_ord_mst a, subcon_ord_dtls b,pro_grey_batch_dtls c,inv_receive_mas_batchroll d ,sub_material_mst e, sub_material_dtls f  
		where a.entry_form=278 and a.subcon_job=b.job_no_mst and a.id=b.mst_id  and a.order_no=c.booking_no and b.booking_dtls_id=c.booking_dtls_id   and c.mst_id=d.id and d.entry_form=91 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.subcon_job='$jobno' and c.id in ($fabric_dtls_id) and e.id=f.mst_id  and  b.id=f.job_dtls_id  and e.trans_type=1 and e.entry_form='279' and   e.status_active=1 and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and c.id=f.fabric_details_id  order by b.id ASC";
			}
	}
	else if($within_group==2)
	{
		...
		$sql_job="SELECT a.id, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id as po_id, b.buyer_po_id, b.gmts_item_id, b.body_part, b.order_quantity, b.order_uom , b.body_part, b.construction, b.composition, b.gsm, b.grey_dia, b.gmts_color_id, b.item_color_id, b.fin_dia, b.aop_color_id, b.lib_yarn_deter,b.booking_dtls_id,b.buyer_po_no, b.buyer_style_ref, b.buyer_buyer,e.receive_qty as receive_qty,e.receive_dtls_id as receive_dtls_id,e.receive_id as receive_id,e.id as update_dtls_id,e.quantity as requisition_qty,e.remarks
		from subcon_ord_mst a, subcon_ord_dtls b ,sub_issue_requisition_mst d, sub_issue_requisition_dtls e   where a.entry_form=278 and a.subcon_job=b.job_no_mst and a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.subcon_job='$jobno' 
		  and d.id=e.mst_id  and  b.id=e.job_dtls_id  and d.requisition_type=2 and d.entry_form='474' and d.id=$update_id and   d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 order by b.id ASC";
		
		}
		else
		{
			
			 $sql_job="SELECT a.id, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id as po_id, b.buyer_po_id, b.gmts_item_id, b.body_part, b.order_quantity, b.order_uom , b.body_part, b.construction, b.composition, b.gsm, b.grey_dia, b.gmts_color_id, b.item_color_id, b.fin_dia, b.aop_color_id, b.lib_yarn_deter,b.booking_dtls_id,b.buyer_po_no, b.buyer_style_ref, b.buyer_buyer,e.quantity as receive_qty,e.id as receive_dtls_id,d.id as receive_id
		from subcon_ord_mst a, subcon_ord_dtls b ,sub_material_mst d, sub_material_dtls e  where a.entry_form=278 and a.subcon_job=b.job_no_mst and a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.subcon_job='$jobno'  and d.id=e.mst_id  and  b.id=e.job_dtls_id  and d.trans_type=1 and d.entry_form='279' and   d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 order by b.id ASC";
	...
	foreach ($sql_result as $row)
	{
		...
		if($within_group==1)
		{
			...
			$prerec_qty=$pre_qty_arr[$row[csf("fabric_dtlsid")]][$row[csf("receive_dtls_id")]]['qty']-$current_requisition_qty;
		...
		?>
		 <tr>
            ...
            <td><input type="text" id="txtordqty_<? echo $k; ?>" name="txtordqty_<? echo $k; ?>" class="text_boxes_numeric" style="width:57px"  value="<? echo $orderQty; ?>" readonly/></td>
            <td><input type="text" id="txtreceiveqty_<? echo $k; ?>" name="txtreceiveqty_<? echo $k; ?>" class="text_boxes_numeric" style="width:57px" o value="<? echo $row[csf("receive_qty")]; ?>" readonly /></td>
            <td><input type="text" id="txtpreqty_<? echo $k; ?>" name="txtpreqty_<? echo $k; ?>" class="text_boxes_numeric" style="width:57px"  value="<? echo $prerec_qty; ?>"  readonly /></td>
            <td><input type="text" id="txtrequisitionqty_<? echo $k; ?>" name="txtrequisitionqty_<? echo $k; ?>" class="text_boxes_numeric" style="width:57px" onKeyUp="check_receive_qty_ability(this.value,<? echo $k; ?>); fnc_total_calculate();" value="<? echo $quantity; ?>" placeholder="<? echo $balanceQty; ?>" pre_rec_qty="<? echo $prerec_qty; ?>" order_qty="<? echo $receiveQty; ?>"  /></td>
            <td><input type="text" id="txtbalanceqty_<? echo $k; ?>" name="txtbalanceqty_<? echo $k; ?>" class="text_boxes_numeric" style="width:57px"  value="<? echo $balanceQty; ?>"  readonly /></td>
        </tr>
	<?	
	}
	exit();
...
if ($action=="aop_issue_print")
{
    ...
    <table width="1360" cellspacing="0" align="right" border="0">
        <tr>
	...
	<div style="width:100%;">
		<table align="right" cellspacing="0" width="1360"  border="1" rules="all" class="rpt_table" >
            <thead bgcolor="#dddddd" align="center">
                    ...
                    <th width="80" >AOP Color</th>
                    <th width="70" >Order Qty (KG)</th>
                    <th width="70" >Received Qty (KG) </th>
                    <th width="60">Prev. Requ. Qty</th>
                    <th width="70" >Requ. Qty</th>
                    <th>Balance Qty (KG)</th>
            </thead>
   <?
	...
	if($within_group==1)
	{
		$sql_rec="SELECT a.id, a.mst_id, a.quantity, a.uom, a.job_dtls_id, a.buyer_po_id, a.remarks,a.fabric_details_id,a.receive_dtls_id from sub_issue_requisition_dtls a, sub_issue_requisition_mst b where b.id=a.mst_id and b.embl_job_no='$jobno' and b.requisition_type=2 and b.id!=$update_id   and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
		$sql_rec_res =sql_select($sql_rec);
		foreach ($sql_rec_res as $row)
		{
	...
	if($within_group==1)
	{
				   $sql_job="SELECT a.id, c.id as fabric_dtlsid,a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id as po_id, b.buyer_po_id, b.gmts_item_id, b.body_part, c.batch_issue_qty as order_quantity, b.order_uom , b.body_part, b.construction, b.composition, b.gsm, b.grey_dia, b.gmts_color_id, b.item_color_id, b.fin_dia, b.aop_color_id, b.lib_yarn_deter,b.booking_dtls_id,f.receive_qty as receive_qty,f.receive_dtls_id as receive_dtls_id,f.receive_id as receive_id,f.id as update_dtls_id,f.quantity as requisition_qty
		from subcon_ord_mst a, subcon_ord_dtls b,pro_grey_batch_dtls c,inv_receive_mas_batchroll d ,sub_issue_requisition_mst e, sub_issue_requisition_dtls f  
		where a.entry_form=278 and a.subcon_job=b.job_no_mst and a.id=b.mst_id  and a.order_no=c.booking_no and b.booking_dtls_id=c.booking_dtls_id   and c.mst_id=d.id and d.entry_form=91 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and a.subcon_job='$jobno' and  f.quantity>0 and e.id=$update_id and e.id=f.mst_id  and  b.id=f.job_dtls_id  and e.requisition_type=2 and e.entry_form='474' and   e.status_active=1 and e.is_deleted=0 and f.status_active=1 and f.is_deleted=0 and c.id=f.fabric_details_id  order by b.id ASC";
			
	}
	else if($within_group==2)
	{
		$sql_job="SELECT a.id, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id as po_id, b.buyer_po_id, b.gmts_item_id, b.body_part, b.order_quantity, b.order_uom , b.body_part, b.construction, b.composition, b.gsm, b.grey_dia, b.gmts_color_id, b.item_color_id, b.fin_dia, b.aop_color_id, b.lib_yarn_deter,b.booking_dtls_id,b.buyer_po_no, b.buyer_style_ref, b.buyer_buyer,e.receive_qty as receive_qty,e.receive_dtls_id as receive_dtls_id,e.receive_id as receive_id,e.id as update_dtls_id,e.quantity as requisition_qty,e.remarks
		from subcon_ord_mst a, subcon_ord_dtls b ,sub_issue_requisition_mst d, sub_issue_requisition_dtls e   where a.entry_form=278 and a.subcon_job=b.job_no_mst and a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.subcon_job='$jobno'  and e.quantity>0
		  and d.id=e.mst_id  and  b.id=e.job_dtls_id  and d.requisition_type=2 and d.entry_form='474' and d.id=$update_id and   d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 order by b.id ASC";
	...
	foreach($sql_result as $row)
	{
		...
		{
			$quantity=0; $dtlsup_id=""; $balanceQty=0; $prerec_qty=0; $orderQty=0; $remarks='';
			$prerec_qty=$pre_qty_arr[$row[csf("fabric_dtlsid")]][$row[csf("receive_dtls_id")]]['qty'];
			$receiveQty=  number_format($row[csf("receive_qty")],0,'.','');
			$orderQty=number_format($row[csf("order_quantity")],4,'.','');
			$balanceQty=number_format($receiveQty-$prerec_qty,4,'.','');
			$quantity= $row[csf("requisition_qty")];
		...
		<tr bgcolor="<? echo $bgcolor; ?>">
				   ...
				   <td width="30"><? echo $k; ?></td>
                    ...
                    <td width="90" ><? echo $body_part[$row[csf('body_part')]] ?></td>
                    <td width="90" ><? echo $row[csf("construction")]; ?></td>
                    <td width="90" ><? echo $row[csf("composition")]; ?></td>
                    <td width="60" ><? echo $row[csf("gsm")]; ?></td>
                    <td width="60" ><? echo $row[csf("grey_dia")]; ?></td>
                    <td width="80" ><? echo $color_arrey[$row[csf("gmts_color_id")]]; ?></td>
                    <td width="80" ><? echo $color_arrey[$row[csf("item_color_id")]]; ?></td>
                    <td width="60" ><? echo $row[csf("fin_dia")]; ?></td>
                    <td width="80" ><? echo $color_arrey[$row[csf("aop_color_id")]]; ?></td>
                    <td width="70"align="right" ><? echo $orderQty; ?></td>
                    <td width="70"  align="right"><? echo $row[csf("receive_qty")]; ?></td>
                    <td width="60" align="right"><? echo $prerec_qty; ?></td>
                    <td width="70" align="right"><? echo $quantity; ?></td>
    ...
    <br>
	 <?
        echo signature_table(234, $data[0], "1340px");
     ?>
	</div>
	</div>
