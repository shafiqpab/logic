...
if ($action=="load_drop_down_search_by")
{
	$data=explode('_',$data);
	if($data[0]==1)
	{
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   
	{
		...
		if($db_type==0)
		{
			$new_issue_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name),'', 'AOPMI' , date("Y",time()), 5, "select id,prefix_no,prefix_no_num from sub_material_mst where company_id=$cbo_company_name and trans_Type='$trans_Type' and entry_form=280 and YEAR(insert_date)=".date('Y',time())." order by id desc ", "prefix_no", "prefix_no_num" ));
		}
		else if($db_type==2)
		{
			$new_issue_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name),'', 'AOPMI' , date("Y",time()), 5, "select id,prefix_no,prefix_no_num from sub_material_mst where company_id=$cbo_company_name and trans_Type='$trans_Type' and entry_form=280 and TO_CHAR(insert_date,'YYYY')=".date('Y',time())." order by id desc ", "prefix_no", "prefix_no_num" ));
		}

		if(is_duplicate_field( "a.chalan_no", "sub_material_mst a, sub_material_dtls b", "a.sys_no='$new_issue_no[0]' and a.trans_Type='$trans_Type' and a.entry_form=280 and a.chalan_no=$txt_issue_challan and b.order_id=$order_no_id and b.material_description=$txt_material_description and b.color_id=$color_id" )==1)
		{
		...
		$field_array="id, entry_form, prefix_no, prefix_no_num, sys_no, trans_type, company_id, location_id, party_id, chalan_no, prod_source, issue_to, subcon_date, within_group, embl_job_no,requisition_no,requisition_id,issue_basis, inserted_by, insert_date, status_active, is_deleted";
		$data_array="(".$id.",'280','".$new_issue_no[1]."','".$new_issue_no[2]."','".$new_issue_no[0]."','".$trans_Type."',".$cbo_company_name.",".$cbo_location_name.",".$cbo_party_name.",".$txt_issue_challan.",1,".$cbo_company_name.",".$txt_issue_date.",".$cbo_within_group.",".$txt_job_no.",".$txt_requisition_no.",".$txt_requisition_id.",".$cbo_basis.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";  
		
		$txt_issue_no=$new_issue_no[0];//change_date_format($data[2], "dd-mm-yyyy", "-",1)
		
		$id1=return_next_id("id","sub_material_dtls",1) ;
		$field_array2="id, mst_id, quantity, job_dtls_id, buyer_po_id, rec_challan,requisition_dtls_id, inserted_by, insert_date, status_active, is_deleted";
		...
		for($i=1; $i<=$total_row; $i++)
		{
			...
			$data_array2.="(".$id1.",'".$id."',".$$txtissueqty.",".$$ordernoid.",".$$txtbuyerPoId.",".$$hidrecsyschallan.",".$$requsitionDtlsId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
			 //.",".$$internalRef
	...
	else if ($operation==1)   // Update Here
	{
		...
		$field_array2="id, mst_id, quantity, job_dtls_id, buyer_po_id, rec_challan, inserted_by, insert_date, status_active, is_deleted";
		...
		for($i=1; $i<=$total_row; $i++)
		{
			...
			if(str_replace("'","",$$updatedtlsid)=="")
			{
				...
				$data_array2.="(".$id1.",".$update_id.",".$$txtissueqty.",".$$ordernoid.",".$$txtbuyerPoId.",".$$hidrecsyschallan.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
				//.",".$$internalRef
				$id_arr_iss[]=$id1;
				$id1=$id1+1; $add_commaa++;
			}
			...
			{
				$data_arr_up[str_replace("'","",$$updatedtlsid)]=explode("*",("".$$txtissueqty."*".$$cbouom."*".$$ordernoid."*".$$txtbuyerPoId."*".$$hidrecsyschallan."*".$user_id."*'".$pc_date_time."'"));
...
if ($action=="issue_popup")
{
	...
	<script>
		...
		{   //alert(company+'_'+party_name+'_'+within_group);	
			load_drop_down( 'aop_material_issue_controller', company+'_'+within_group+'_'+party_name, 'load_drop_down_buyer_pop', 'buyer_td' );
		}
                ...
                <table width="870" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
							...
							<? 
								echo create_drop_down( "cbo_company_name", 140, "SELECT id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $company, "load_drop_down( 'aop_material_issue_controller', this.value+'_'+".$within_group.", 'load_drop_down_buyer_pop', 'buyer_td' );"); ?>
                            </td>
                            <td>
							<?
								echo create_drop_down( "cbo_within_group", 50, $yes_no,"", 0, "--  --",$within_group, "load_drop_down( 'aop_material_issue_controller',document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_buyer_pop', 'buyer_td' );" ); ?>
							</td>
                            <td id="buyer_td">
								<? 
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('txt_search_common').value+'_'+document.getElementById('txt_search_challan').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+document.getElementById('cbo_within_group').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('txt_aop_ref').value, 'create_issue_search_list_view', 'search_div', 'aop_material_issue_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:70px;" /></td>
                        </tr>
                        <tr>
...
if($action=="create_issue_search_list_view")
{
	...
	//$aop_reference_arr=return_library_array( "select subcon_job,aop_reference from subcon_ord_mst",'subcon_job','aop_reference');
	...
	if($db_type==0)
	{
		...
		//$buyer_po_id_cond="group_concat(distinct(b.job_dtls_id))";
	...
	$sql= "SELECT a.id, a.sys_no, a.prefix_no_num, $insert_date_cond as year, a.location_id, a.party_id, a.subcon_date, a.within_group, a.chalan_no, a.remarks, a.embl_job_no, $wo_cond as order_id, $buyer_po_id_cond as buyer_po_id $aop_reference from sub_material_mst a, sub_material_dtls b $ref_tab where a.id=b.mst_id and a.trans_type=2 and a.entry_form=280 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $issue_date $company  $aop_cond $buyer_cond $withinGroup $issue_id_cond $challan_no_cond $spo_idsCond  $subconjob $po_idsCond $ref_tab_rel group by a.id, a.sys_no, a.prefix_no_num, a.insert_date, a.location_id, a.party_id, a.subcon_date, a.within_group, a.chalan_no, a.remarks, a.embl_job_no $aop_reference order by a.id DESC ";
	//echo $sql; 
     ...
     <div style="width:920px; max-height:270px;overflow-y:scroll;" >	 
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="900" class="rpt_table" id="tbl_po_list">
            ...
            foreach( $result as $row )
            {
				...
				{
					if($order_no=="") $order_no=$order_arr[$val]['order_no']; else $order_no.=",".$order_arr[$val]['order_no'];
				}
				...
				?>
					<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value('<? echo $row[csf("id")]."_".$row[csf("embl_job_no")];?>');" > 
                        ...
                        <td width="100" align="center" style="word-break:break-all"><? echo $buyer_job; ?></td>
...
if ($action=="load_php_data_to_form")
{
	...
	$nameArray=sql_select( "SELECT id, sys_no, company_id, location_id, party_id, subcon_date, chalan_no,within_group, embl_job_no,requisition_no,issue_basis,requisition_id from sub_material_mst where id='$data' and status_active=1 and is_deleted=0" ); 
	foreach ($nameArray as $row)
	{	
		...
		echo "load_drop_down( 'requires/aop_material_issue_controller', document.getElementById('cbo_company_name').value, 'load_drop_down_location', 'location_td' );\n";
		
		echo "load_drop_down( 'requires/aop_material_issue_controller', document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_within_group').value, 'load_drop_down_buyer', 'buyer_td' );\n"; 		
...
if ($action=="job_popup")
{
	...
	<script>
		...
		{
			//alert();
			load_drop_down( 'aop_material_issue_controller', company+'_'+within_group+'_'+party, 'load_drop_down_buyer', 'buyer_td' );
			load_drop_down( 'aop_material_issue_controller', within_group, 'load_drop_down_search_by', 'search_td' );
                ...
                <table width="850" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <td align="center">
                                <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_party_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_type').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('cbo_string_search_type').value+'_'+<? echo $data[3];?>+'_'+document.getElementById('txt_aop_ref').value+'_'+<? echo $data[4];?>, 'create_job_search_list_view', 'search_div', 'aop_material_issue_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:70px;" />
                            </td>
                        </tr>
                        <tr>
...
{	
	...
	if ($within_group!=0) $withinGroup=" and a.within_group='$within_group'"; else $withinGroup="";
	$job_cond=""; $style_cond=""; $po_cond=""; $search_com_cond="";$within_group="";
	if($search_type==1)
	{
		...
		if($search_str!="")
		{
			...
			else if ($within_group==1 && $search_by==6) $internal_ref=" and b.grouping = '$search_str' ";
		}
	}
	else if($search_type==4 || $search_type==0)
	{
		...
		if($search_str!="")
		{
			...
			else if ($within_group==1 && $search_by==6) $internal_ref=" and b.grouping like '%$search_str%'";  
		}
	}
	else if($search_type==2)
	{
	...
	$po_ids='';
	//echo $job_cond."**".$style_cond."**".$po_cond; die;
	if($within_group==1)
	{
		if($db_type==0) $id_cond="group_concat(b.id) as id";
		else if($db_type==2) $id_cond="rtrim(xmlagg(xmlelement(e,b.id,',').extract('//text()') order by b.id).GetClobVal(),',') as id";
		...
		//echo "select $id_cond as id from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst $job_cond $style_cond $po_cond";
		if(($job_cond!="" && $search_by==3) || ($po_cond!="" && $search_by==4) || ($style_cond!="" && $search_by==5)|| ($internal_ref!="" && $search_by==6))
		{
			$po_ids = return_field_value("$id_cond", "wo_po_details_master a, wo_po_break_down b", "a.job_no=b.job_no_mst $job_cond $style_cond $po_cond $internal_ref", "id");
		}
		//echo $po_ids;
		if($db_type==2 && $po_ids!="") $po_ids = $po_ids->load();
		if ($po_ids!="")
	...
	$buyer_po_arr=array();
	$po_sql ="SELECT a.style_ref_no,a.job_no_prefix_num, b.id, b.po_number from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0";
	$po_sql_res=sql_select($po_sql);
	foreach ($po_sql_res as $row)
	{
		$buyer_po_arr[$row[csf("id")]]['style']=$row[csf("style_ref_no")];
	...
	if($db_type==0)
	{
		...
		//$buyer_po_no_cond="year(b.buyer_po_no)";
		//$buyer_style_cond="year(b.buyer_style_ref)";
	}
	else if($db_type==2)
	{
	...
	{
		$ins_year_cond="year(a.insert_date)";
		$color_id_str=",group_concat(b.aop_color_id) as color_id";
		if($within_group==1)
		{
	...
	{
		$ins_year_cond="TO_CHAR(a.insert_date,'YYYY')";
		$color_id_str=" ,rtrim(xmlagg(xmlelement(e,b.aop_color_id,',').extract('//text()') order by b.aop_color_id).GetClobVal(),',') as color_id";
	 ...
	 if ($basis==1)
	 {
		...
		$sql= "SELECT a.id, a.subcon_job, a.job_no_prefix_num, $ins_year_cond as year, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date $buyer_po_no_str $buyer_po_style_str $color_id_str $buyer_po_id_str ,a.aop_reference ,d.sys_no ,d.id as requsition_id 
	from subcon_ord_mst a, subcon_ord_dtls b ,sub_issue_requisition_mst d, sub_issue_requisition_dtls e  
	where a.entry_form=278 and a.subcon_job=b.job_no_mst and a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $order_rcv_date $company $buyer $withinGroup $search_com_cond $po_idsCond  $aop_cond and d.id=e.mst_id  and  b.id=e.job_dtls_id  and d.requisition_type=2 and d.entry_form='474' and  d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0
	group by a.id, a.subcon_job, a.job_no_prefix_num, a.insert_date, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date,a.aop_reference,d.sys_no ,d.id
	order by a.id DESC";
		 
	 }
	 else
	 {
		 $sql= "SELECT a.id, a.subcon_job, a.job_no_prefix_num, $ins_year_cond as year, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date $buyer_po_no_str $buyer_po_style_str $color_id_str $buyer_po_id_str ,a.aop_reference  
	...
	?>
     <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="985" >
        <thead>
            ...
            <? if ($basis==1) { ?> <th width="110">Requisition No</th> <? } ?>
            <th>AOP Reference</th>
        </thead>
        </table>
        ...
        <tbody>
            ...
            {  
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" onClick='js_set_value("<? echo $js_data; ?>")' style="cursor:pointer" >
                    ...
                    <td width="100" style="word-break:break-all;text-align:center;"><? echo $buyer_job; ?></td>
                    <td width="100" style="word-break:break-all"><? echo $buyer_po; ?></td>
                    <td width="100" style="word-break:break-all"><? echo $buyer_style; ?></td>
                    ...
                    <td width="80" style="word-break:break-all"><? echo $color_name; ?></td>
                     <? if ($basis==1) { ?>  <td width="110"><? echo $row[csf('sys_no')]; ?></td> <? } ?>	
                    <td style="text-align:center;"><? echo $row[csf('aop_reference')]; ?></td>	
                </tr>
				<? 
                $i++; 
            } 
...
if($action=="load_php_dtls_form")
{
	...
	$po_sql ="SELECT a.style_ref_no, b.grouping, b.id, b.po_number from wo_po_details_master a, wo_po_break_down b where a.job_no=b.job_no_mst and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0";
	$po_sql_res=sql_select($po_sql);
	foreach ($po_sql_res as $row)
	{
		$buyer_po_arr[$row[csf("id")]]['style']=$row[csf("style_ref_no")];
	...
	$rec_data_arr=array();
	$sql_rec="SELECT a.sys_no, b.quantity, b.job_dtls_id, b.job_break_id, b.grouping, b.buyer_po_id from sub_material_mst a, sub_material_dtls b where a.id=b.mst_id and a.embl_job_no='$jobno' and a.trans_type=1 and a.entry_form=279 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	$sql_rec_res =sql_select($sql_rec);
	foreach ($sql_rec_res as $row)
	{
		$rec_data_arr[$row[csf("job_dtls_id")]]['sys_challan']=$row[csf("sys_no")];
		$rec_data_arr[$row[csf("job_dtls_id")]]['int_ref']=$row[csf("grouping")];
...
/*	
	...
	if($basis==1)
	{
	 	...
	 	$sql_job="SELECT a.id, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id as po_id, b.buyer_po_id, b.gmts_item_id, b.body_part, e.quantity as order_quantity, b.order_uom , b.body_part, b.construction, b.composition, b.gsm, b.grey_dia, b.gmts_color_id, b.item_color_id, b.fin_dia, b.aop_color_id, b.lib_yarn_deter, b.buyer_po_no, b.buyer_style_ref, a.within_group,e.id as requsition_dtls_id,e.mst_id as requsition_id
		from subcon_ord_mst a, subcon_ord_dtls b ,sub_issue_requisition_mst d, sub_issue_requisition_dtls e
		where a.entry_form=278 and a.subcon_job=b.job_no_mst and a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.subcon_job='$jobno' and d.id=e.mst_id  and  b.id=e.job_dtls_id  and d.requisition_type=2 and d.entry_form='474' and d.id=$requsition_id and  d.status_active=1 and d.is_deleted=0 and e.status_active=1 and e.is_deleted=0 order by b.id ASC";
	...
	foreach ($sql_result as $row)
	{
		...
		if($basis==1)
		{
			$rec_qty=$requsition_data_arr[$row[csf("requsition_dtls_id")]]['qty'];
		...
		if($row[csf("within_group")]==1)
		{
			...
			$int_ref= $rec_data_arr[$row[csf("po_id")]]['int_ref'];
		}
		?>
		 <tr>
            <td><input type="hidden" name="ordernoid_<? echo $k; ?>" id="ordernoid_<? echo $k; ?>" value="<? echo $row[csf("po_id")]; ?>">
               <input type="hidden" name="requsitionDtlsId_<? echo $k; ?>" id="requsitionDtlsId_<? echo $k; ?>" value="<? echo $row[csf("requsition_dtls_id")]; ?>">
                ...
                <input type="text" name="txtorderno_<? echo $k; ?>" id="txtorderno_<? echo $k; ?>" class="text_boxes" style="width:90px" value="<? echo $row[csf("order_no")]; ?>" readonly /> <!--onDblClick="job_search_popup('requires/aop_material_issue_controller.php?action=job_popup','Order Selection Form')" -->
            </td>
            ...
            <td><input name="txtbuyerPo_<? echo $k; ?>" id="txtbuyerPo_<? echo $k; ?>" type="text" class="text_boxes" style="width:90px" value="<? echo $buyer_po ; ?>" readonly />
            ...
            <td><input type="text" name="txtstyleRef_<? echo $k; ?>" id="txtstyleRef_<? echo $k; ?>" type="text" class="text_boxes" style="width:90px" value="<? echo $buyer_style ; ?>" readonly /></td>
            ...
            <td title="<? echo  "Prev. Issue. Qty"."=>".$pre_issue_qty." "."Curent qty"."=>".$balance_qty." "."balance qty"."=>".$balance_qty; ?>"><input name="txtissueqty_<? echo $k; ?>" id="txtissueqty_<? echo $k; ?>" class="text_boxes_numeric" type="text" onKeyUp="check_iss_qty_ability(this.value,<? echo $k; ?>); fnc_total_calculate();" value="<? echo $qty; ?>" placeholder="<? echo $balance_qty; ?>" pre_issue_qty="<? echo $pre_issue_qty; ?>" rec_qty="<? echo $rec_qty; ?>" style="width:62px" /></td>
        </tr>
	<?	
	}
	exit();
...
if ($action=="aop_material_issue_print")
{
    extract($_REQUEST);
	$data=explode('*',$data);
	//print_r ($data); die;
	...
	$sql="SELECT id, sys_no, company_id, location_id, party_id, subcon_date, chalan_no,within_group, embl_job_no from sub_material_mst where sys_no='$data[1]' and status_active=1 and is_deleted=0  and trans_Type=2  and entry_form=280  ";

	//$nameArray=sql_select( "SELECT id, sys_no, company_id, location_id, party_id, subcon_date, chalan_no,within_group, embl_job_no from sub_material_mst where sys_no='$data[1]' and status_active=1 and is_deleted=0" );
	...
	<div style="width:100%;">
		<table align="right" cellspacing="0" width="1160"  border="1" rules="all" class="rpt_table" >
            <thead bgcolor="#dddddd" align="center">
                ...
                <th width="100" align="center">Composition</th>
                ...
                <th width="80" align="center">Fin. Dia</th>
                <th width="80" align="center">AOP Color</th>
                <th width="" align="center">Issue Qty (KG)</th>
            </thead>
   <?
	//$mst_id=$dataArray[0][csf('id')];
	//$job_no=$dataArray[0][csf('embl_job_no')];
    $i=1;
	...
	/*$updtls_data_arr=array(); 
	 ...
	 $sql_iss="SELECT a.id, a.mst_id, a.quantity, a.uom, a.job_dtls_id, a.buyer_po_id from sub_material_dtls a, sub_material_mst b where b.id=a.mst_id and b.embl_job_no='$jobno' and b.trans_type=2 and b.entry_form=280 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	$sql_iss_res =sql_select($sql_iss);  
	...
	$sql_job="SELECT a.id, a.subcon_job, a.job_no_prefix_num, a.order_id, a.order_no, a.delivery_date, b.id as po_id, b.buyer_po_id, b.gmts_item_id, b.body_part, b.order_quantity, b.order_uom , b.body_part, b.construction, b.composition, b.gsm, b.grey_dia, b.gmts_color_id, b.item_color_id, b.fin_dia, b.aop_color_id, b.lib_yarn_deter, b.buyer_po_no, b.buyer_style_ref, a.within_group,c.quantity
		...
		where a.entry_form=278 and a.subcon_job=b.job_no_mst and a.id=b.mst_id and b.id=c.job_dtls_id and d.id=c.mst_id  and d.embl_job_no=a.subcon_job and   d.trans_type=2 and d.entry_form=280 and   a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and  c.status_active=1 and c.is_deleted=0 and  a.subcon_job='$jobno' and d.sys_no='$data[1]'  order by b.id ASC";
	//echo $sql_job; die;
	...
	foreach($sql_result as $row)
	{
		...
		<tr bgcolor="<? echo $bgcolor; ?>">
            ...
            <td width="100"><p><? echo $body_part[$row[csf("body_part")]]; ?></p></td>
            ...
            <td width="60"><p><? echo $row[csf("grey_dia")]; ?></p></td>
            <td width="60"><p><? echo $color_arrey[$row[csf("gmts_color_id")]]; ?></p></td>
            <td width="60"><p><? echo $color_arrey[$row[csf("item_color_id")]]; ?></p></td>
            <td width="80"><p><? echo $row[csf("fin_dia")]; ?></p></td>
            <td width="80"><p><? echo $color_arrey[$row[csf("aop_color_id")]]; ?></p></td>
            <td width="" align="right"><p><? $qty=$row[csf("quantity")]; //$qty=$updtls_data_arr[$row[csf("po_id")]]['qty']; 
            	$total_qty+=$qty;
            	echo number_format($qty,2,'.',''); ?></p></td>
		</tr>
		<?php
    ...
    <br>
	 <?
        echo signature_table(231, $data[0], "1140px");
     ?>
