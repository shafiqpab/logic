...
if ($action=="save_update_delete")
{
	...
	if ($operation==0)   
	{
		...
		$field_array2="id, mst_id, quantity, job_dtls_id, buyer_po_id, inserted_by, insert_date, status_active, is_deleted";
		...
		for($i=1; $i<=$total_row; $i++)
		{
			...
			$data_array2.="(".$id1.",'".$id."',".$$txtreceiveqty.",".$$ordernoid.",".$$txtbuyerPoId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
	...
	else if ($operation==1)   // Update Here
	{
		...
		$field_arr_up="quantity*job_dtls_id*buyer_po_id*updated_by*update_date";
		...
		for($i=1; $i<=$total_row; $i++)
		{
			...
			if(str_replace("'","",$$updatedtlsid)=="")
			{
				...
				$data_array2.="(".$id1.",".$update_id.",".$$txtreceiveqty.",".$$ordernoid.",".$$txtbuyerPoId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
				$id_arr_rec[]=$id1;
				$id1=$id1+1; $add_commaa++;
			}
			else if(str_replace("'","",$$updatedtlsid)!="")
			{
				$data_arr_up[str_replace("'","",$$updatedtlsid)]=explode("*",("".$$txtreceiveqty."*".$$ordernoid."*".$$txtbuyerPoId."*".$user_id."*'".$pc_date_time."'"));
...
{	
	...
	$sql= "SELECT a.id, a.subcon_job, a.job_no_prefix_num, $ins_year_cond as year, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date, $color_id_str as color_id, $buyer_po_id_cond as buyer_po_id,a.aop_reference 
	...
	foreach ($sql_result as $row)
	{
		...
		?>
		 <tr>
            ...
            <td><input type="text" name="txtbuyerPo_<? echo $k; ?>" id="txtbuyerPo_<? echo $k; ?>"  class="text_boxes" style="width:90px" value="<? echo $buyer_po_arr[$row[csf("buyer_po_id")]]['po']; ?>" readonly />
            ...
            <td><input type="text" name="txtstyleRef_<? echo $k; ?>" id="txtstyleRef_<? echo $k; ?>" type="text" class="text_boxes" style="width:90px" value="<? echo $buyer_po_arr[$row[csf("buyer_po_id")]]['style']; ?>" readonly /></td>
