...
<script>
	...
	{ 
		...
		var title="Receive Id Pop-up Info";	
		...
		emailwindow.onclose=function()
		{
			...
			if (splt_val[0]!="")
			{
				...
				var list_view_orders = return_global_ajax_value( splt_val[0]+'**'+splt_val[1]+'**'+1+'**'+splt_val[2], 'load_php_dtls_form', '', 'requires/aop_material_receive_controller');
				
				
				//var list_view_orders = return_global_ajax_value( 0+'**'+theemail+'**'+1+'**'+within_group+'**'+dtls_id, 'load_php_dtls_form', '', 'requires/aop_material_receive_controller');
	...
	function job_search_popup(page_link,title)     
	{
		...
		else
		{
			...
			{
				var data=document.getElementById('cbo_company_name').value+"_"+document.getElementById('cbo_location_name').value+"_"+document.getElementById('cbo_party_name').value+"_"+document.getElementById('cbo_within_group').value;
			page_link='requires/aop_material_receive_controller.php?action=fabric_finish_popup&data='+data;
	...
	function fnc_material_receive( operation )
	{
		...
		else
		{
			...
			for (var i=1; i<=tot_row; i++)
			{
				...
				{
					k++;
					data_str+="&ordernoid_" + k + "='" + $('#ordernoid_'+i).val()+"'"+"&txtbuyerPoId_" + k + "='" + $('#txtbuyerPoId_'+i).val()+"'"+"&txtreceiveqty_" + k + "='" + $('#txtreceiveqty_'+i).val()+"'"+"&txtremarks_" + k + "='" + $('#txtremarks_'+i).val()+"'"+"&updatedtlsid_" + k + "='" + $('#updatedtlsid_'+i).val()+"'"+"&fabricdtlsid_" + k + "='" + $('#fabricdtlsid_'+i).val()+"'";
					//+"&internalRef_" + k + "='" + $('#internalRef_'+i).val()+"'";
				}
			}
			if(k==0){
				alert("At least one receive quantity value should be greater than zero!!!");
				return;
			}
	...
	function fnc_material_receive_response()
	{
		if(http.readyState == 4) 
		{
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				var list_view_orders = return_global_ajax_value( response[2]+'**'+response[3]+'**'+2+'**'+response[4], 'load_php_dtls_form', '', 'requires/aop_material_receive_controller');
				if(list_view_orders!='')
				{
					$("#rec_issue_table tr").remove();
					$("#rec_issue_table").append(list_view_orders);
    	...
    	<legend>AOP Material Receive</legend>
            <table  width="800" cellspacing="2" cellpadding="0" border="0">
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="txt_receive_date" id="txt_receive_date"  class="datepicker" style="width:140px"  readonly/>             
                    </td>
                </tr>
                <tr>
            ...
            <table cellpadding="0" cellspacing="2" border="1" width="1175" id="details_tbl" rules="all">
