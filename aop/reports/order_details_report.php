<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Form Will Create AOP Order Details Report.
Functionality	:	
JS Functions	:
Created by		:	Kausar
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("AOP Order Details Report", "../../", 1, 1,$unicode,1,1);
?>	
...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../../logout.php";  
	...
	{
		col_operation: {
			id: ["value_tdinqty","value_tdindeliqty","value_tdinbalqty",],
			col: [14,15,16],
	...
	function fn_report_generated(type)
	{
		...
		var style_ref_no = $("#txt_buyer_style").val();
		var order_no = $("#txt_order_no").val();
		var reference_no = $("#txt_reference_no").val();
		var buyer_po = $("#txt_buyer_po").val();
		var company_id = $("#cbo_company_id").val();
		
		if(job_no=="" && style_ref_no=="" && order_no=="" && reference_no=="" && buyer_po=="" && company_id==0)
		{
		...
		else
		{
			...
			var data="action=report_generate"+get_submitted_data_string('cbo_company_id*cbo_within_group*cbo_party_name*txt_job_no*txt_order_no*txt_order_id*txt_reference_no*txt_buyer_buyer_no*txt_buyer_po*txt_buyer_style*cbo_delivery_status*cbo_basedon*txt_date_from*txt_date_to',"../../")+'&report_title='+report_title+'&type='+type;
			//alert(data)
			freeze_window(3);
			http.open("POST","requires/order_details_report_controller.php",true);
	...
	{ 
		 emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/order_details_report_controller.php?action='+action+'&order_id='+order_id, 'AOP Production Status Report', 'width='+width+',height=400px,center=1,resize=0,scrolling=0','../');
	} 
	...
	function fnc_load_party(type)
	{
		...
		load_drop_down( 'requires/order_details_report_controller', company+'_'+cbo_within_group, 'load_drop_down_buyer', 'buyer_td' );
		load_drop_down( 'requires/order_details_report_controller', company+'_'+cbo_within_group, 'load_drop_down_buyer_buyer', 'buyer_buye_td' );
	}
	...
	{ 
		...
		var page_link="requires/order_details_report_controller.php?action=job_no_popup&company_id="+company_name+"&cbo_buyer_name="+cbo_buyer_name+"&within_group="+within_group;
	...
	{ 
		...
		var page_link="requires/order_details_report_controller.php?action=order_no_popup&company_id="+company_name+"&cbo_buyer_name="+cbo_buyer_name+"&job_no="+job_no+"&within_group="+within_group;
                ...
                <tbody>
                    <tr class="general">
                        ...
                        <td><? 
							...
							$deliveryStatus_arr=array(1=>'Full Delivery',2=>'Partial Delivery',3=>'Pending');
							$basedOn_arr=array(1=>'Order Rec. Date',3=>'Delivery Date');
							$dd="search_populate(this.value)";
							echo create_drop_down( "cbo_delivery_status", 80, $deliveryStatus_arr,"", 1, "--All--", 0, "",0 );
							//echo create_drop_down( "cbo_report_type", 80, $reportType_arr,"", 1, "-- All --", 0, "","","1,3" ); ?></td>
                        <td style="display:none"><? //echo create_drop_down( "cbo_delivery_status", 80, $deliveryStatus_arr,"", 1, "--All--", 0, "",0 ); ?></td>
                        <td><? echo create_drop_down( "cbo_basedon", 80, $basedOn_arr,"", 0, "--All--", 1, $dd,0 ); ?></td>
