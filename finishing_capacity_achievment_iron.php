<?
/*-------------------------------------------- Comments -----------------------
Purpose			: 	This Graph Will Create Finishing Capacity and Achivment(Iron)
Functionality	:	
JS Functions	:
Created by		:	Jahid 
...
$tval=$cps[2];
	...
	if($m=="finishing_capacity_achievment_iron")
	{
		$caption="Finishing Capacity and Achievment(Iron) In HOURS";
	}
	else
	{
		$caption="Finishing Capacity and Achievment(Iron) In HOURS";
	...
	$allocationData=sql_select("select a.year, b.month_id, sum(b.capacity_month_mint) as capa_min from lib_fin_gmts_capacity_cal_mst a,  lib_fin_gmts_capa_year_dtls b where a.id=b.mst_id and a.company_id in($company) and a.status_active=1 and a.is_deleted=0 $location_cond group by a.year, b.month_id");
	...
	$sew_qty_arr=array();
	$sewing_qnty=sql_select("SELECT po_break_down_id, country_id, item_number_id, sum(production_quantity) AS production_quantity  from pro_garments_production_mst where production_type ='7' and is_deleted=0 and status_active=1 and country_id>0 group by po_break_down_id, country_id, item_number_id");
	...
	foreach($yr_mon_part as $key=>$val)
	{
		...
		$dataSew_arr=sql_select("select production_date, po_break_down_id, item_number_id, sum(production_quantity) as production_quantity from pro_garments_production_mst where company_id in($company) and status_active=1 and is_deleted=0 and production_type=7 and $production_date_con like '".$val."-%"."'  $location_product_cond group by production_date,po_break_down_id,item_number_id");
		$producedQnty=0;
		foreach($dataSew_arr as $row)
		{
			...
			$producedQnty+=(($row[csf("production_quantity")]*$item_smv)/60);
		}
		...
		foreach($result as $row)
		{
			...
			$bookedQty+=(($row[csf('qnty')]*$item_smv)/60);
		}
		...
		{
			$capacity=(($allocation_lib_arr[$year][(int) $month])/60);
		}
		
		if($i!=12) $capacity_data .="".number_format($capacity,2,'.','').","; else $capacity_data .="".number_format($capacity,2,'.','')."]";
		if($i!=12) $booked_data .="".number_format($bookedQty,2,'.','').","; else $booked_data .="".number_format($bookedQty,2,'.','')."]";
		if($i!=12) $produced_data .="".number_format($producedQnty,2,'.','').","; else $produced_data .="".number_format($producedQnty,2,'.','')."]";
