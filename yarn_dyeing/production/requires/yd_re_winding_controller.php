...
if($action == "populate_batch_from_list") {
   ...
   $count_arr = return_library_array("Select id, yarn_count from  lib_yarn_count where  status_active=1", 'id', 'yarn_count');
    ...
    echo "document.getElementById('txtDryerProdQty').value = '" . $data_array[0][csf("no_bag")] . "';\n";
    // echo "document.getElementById('txtProdDryerQtyPcs').value = '" . $data_array[0][csf("no_bag")] . "';\n";
    ...
    echo "document.getElementById('txtBalanceQty').value = '" . $data_array[0][csf("receive_qty")] . "';\n";
    echo "document.getElementById('txtProdDryerQtyPcs').value = '" . $data_array[0][csf("avg_wgt")] . "';\n";
    echo "document.getElementById('txtBalanceQtyPcs').value = '" . $available_balance_production . "';\n";
...
if($action == "populate_saved_data_from_list") {
    ...
    /*$data_array = sql_select("select b.id,b.quantity,b.winding_qty, b.remarks, a.company_id, a.party_id,a.yarn_type_id,a.Winding_Cone_Type
    ...
    /*echo "select b.id,b.quantity,b.winding_qty, b.remarks, a.company_id, a.party_id,a.yarn_type_id,a.Winding_Cone_Type
        from yd_production_mst a, yd_production_dtls b
        where a.is_deleted=0 AND a.status_active=1 AND a.id=b.mst_id AND b.id='$data'";die;*/
   
    //echo "load_drop_down( 'requires/yd_re_winding_controller', document.getElementById('cbo_yarn_type').value, '', '' );\n";
   ...
   $count_arr = return_library_array("Select id, yarn_count from  lib_yarn_count where  status_active=1", 'id', 'yarn_count');
    ...
    //echo "document.getElementById('txtWindingConeType').value = '" . $data_array[0][csf("Winding_Cone_Type")] . "';\n";
...
if ($action=="save_update_delete")
{
    ...
    {   
        ...
        {
            $new_product_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_name),'', 'YDRW' , date("Y",time()), 5, "select id,prod_no_prefix,prod_no_prefix_num from yd_production_mst where company_id=$cbo_company_name and entry_form=442 $insert_date_con order by id desc ", "prod_no_prefix", "prod_no_prefix_num" ));

            $mst_id=return_next_id("id","yd_production_mst",1);
            // echo $mst_id;die;
            $field_array="id,booking_without_order,booking_type,entry_form,prod_no_prefix,prod_no_prefix_num, yd_prod_no,company_id, party_id, prod_date, machine_rpm, tension, yarn_type_id, winding_cone_type, remarks,batch_id, inserted_by,insert_date, status_active,is_deleted";
            $data_array="(".$mst_id.",".$booking_without_order.",".$booking_type.",'442','".$new_product_no[1]."','".$new_product_no[2]."','".$new_product_no[0]."',".$cbo_company_name.",".$cbo_party_name.",".$txtProductionDate.",".$txtMachineRpm.",".$txtTension.",".$cbo_yarn_type.",".$txtWindingConeType.",".$txtRemarks.",".$txtBatchId.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
        ...
        $field_array2="id, mst_id,entry_form, sales_order_id, sales_order_no, product_id, job_id, job_dtls_id, order_id,winding_qty, quantity,winding_cone_type, remarks,thread_length,package_avg_weight,inserted_by, insert_date,status_active,is_deleted";  

        // echo $mst_id;die;
        
        $data_array2.="(".$id1.",".$mst_id.",'442',".$sales_order_id.",".$txtSalesOrder.",".$product_id.",".$job_id.",".$job_dtls_id.",".$order_id.",".$txtWindingCone.",".$txtPrdctnQty.",".$txtWindingConeType.",".$txtRemarks.",".$txtThreadLength.",".$txtPackAvgRate.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
        //echo $id1;
    ...
    else if ($operation==1)   // Update Here
    {
        ...
        $field_array_mst="prod_date*machine_rpm*tension*yarn_type_id*Winding_Cone_Type*updated_by*update_date";
        $data_array_mst="".$txtProductionDate."*".$txtMachineRpm."*".$txtTension."*".$cbo_yarn_type."*".$txtWindingConeType."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";

        $field_array_dtls = "quantity*Winding_Cone_Type*remarks*updated_by*update_date";

        $data_array_dtls="".$txtPrdctnQty."*".$txtWindingConeType."*".$txtRemarks."*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
...
if($action == "create_production_order_list") {
    ...
    $data=explode('_',$data);
    $mst_id = $data[0];
    $sql = "select a.company_id,a.party_id, a.machine_rpm,a.yd_prod_no, a.prod_date, a.tension, a.yarn_type_id, a.Winding_Cone_Type, b.id as dtls_id, b.mst_id, b.job_dtls_id, b.quantity
    from yd_production_mst a, yd_production_dtls b
    where b.mst_id='$mst_id' and a.id=b.mst_id and a.entry_form=442";
    /*echo $sql; die;*/
    $count_arr = return_library_array("Select id, yarn_count from  lib_yarn_count where  status_active=1", 'id', 'yarn_count');
    $comp_arr = return_library_array("select id,composition_name from lib_composition_array where is_deleted=0 and status_active=1", 'id', 'composition_name');
    $arr=array(2=>$count_arr, 3=>$comp_arr);

    echo create_list_view('list_view', 'YD Product No,Start Date,Production Date,End Date,Quantity', '150,50,80,150,200', '620', '770', 0, $sql, 'put_data_into_form', "dtls_id", "", 1, '0,0,count_id,yarn_composition_id,0', $arr, 'yd_prod_no,machine_rpm,prod_date,tension,quantity', '', '', '0,0,0,0,1');
...
if ($action=="production_popup")
{
    ...
    <script>
        ...
        {   //alert(company+'_'+party_name+'_'+within_group);   
            load_drop_down( 'yd_re_winding_controller', company+'_'+party_name, 'load_drop_down_buyer_pop', 'buyer_td' );
        }
                ...
                <table width="870" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                    ...
                    <tbody>
                        <tr class="general">
                            ...
                            <? 
                                echo create_drop_down( "cbo_company_name", 140, "select id,company_name from lib_company comp where status_active =1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $company, "load_drop_down( 'yd_re_winding_controller', this.value+'_'+".$within_group.", 'load_drop_down_buyer_pop', 'buyer_td' );"); ?>
                            </td>
                    ...
                    <table cellpadding="0" cellspacing="0" border="1" rules="all" width="500" class="rpt_table">
                        ...
                        <tr class="general">
                            ...
                            <td align="center">
                                ...
                                onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+'<? echo $cbo_company_id; ?>'+'_'+document.getElementById('cbo_string_search_type').value, 'create_batch_search_list_view', 'search_div', 'yd_re_winding_controller', 'setFilterGrid(\'list_view\',-1);')"
                                style="width:100px;"/>
                            </td>
                        </tr>
                    </table>
...
if ($action == "create_batch_search_list_view")
 {
    ...
    $sql = "select a.id,a.yd_job_id,a.order_id,a.order_no,a.booking_without_order,a.booking_type,a.yd_batch_id,a.company_id,a.location_id,a.batch_color_id,a.batch_color_range,a.batch_number,a.batch_against,a.batch_weight,a.extention_no,a.duration_req,a.process_id,a.machine_id,a.batch_date,a.remarks,b.id as batch_dtls_id, c.lot, c.count_id, c.yarn_type_id, c.yarn_composition_id,c.mst_id
            from yd_batch_mst a ,yd_batch_dtls b, yd_ord_dtls c where a.id=b.mst_id and b.yd_job_dtls_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $condition order by a.id DESC"; 
            /*select a.id, a.order_id, a.Winding_Cone_Type, a.winding_pckg_qty, a.quantity, b.sales_order_no, b.style_ref, b.lot, b.count_id, b.yarn_type_id, b.yarn_composition_id
        ...
        <div style="width:618px; overflow-y:scroll; max-height:240px;" align="center">
            <table cellspacing="0" cellpadding="0" border="1" rules="all" width="590" class="rpt_table" id="list_view">
                ...
                foreach ($nameArray as $selectResult) {
                   ...
                   // $batch_data=$selectResult[csf('id')].'_'.$search_by.'_'.$selectResult[csf('batch_dtls_id')].'_'.$selectResult[csf('mst_id')];
...
if($action=="create_issue_search_list_view")
{
   ...
   /* echo $search_str;die;*/
    ...
    $sql= "select a.id,b.job_id, a.yd_prod_no,a.prod_no_prefix,a.prod_no_prefix_num, $insert_date_cond as year, a.location_id, a.party_id, a.prod_date,a.machine_rpm,a.tension from yd_production_mst a, yd_production_dtls b where a.id=b.mst_id and a.entry_form=442 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $production_date $company $buyer_cond $issue_id_cond group by a.id,b.job_id, a.yd_prod_no, a.prod_no_prefix_num,a.prod_no_prefix, a.insert_date, a.location_id, a.party_id, a.prod_date,a.machine_rpm,a.tension order by a.id DESC "; // within group add here if need
    // echo $sql;
            ...
            {   
                ...
                ?>  
                    <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"  onclick="js_set_value('<? echo $row[csf("id")].'_'.$row[csf("job_id")];?>');"> 
                        ...
                        <td width="100" align="center"><? echo $row[csf("machine_rpm")]; ?></td>
                        <td width="80"><? echo change_date_format($row[csf("prod_date")]);  ?></td>  
                        <td width="120" style="word-break:break-all"><p><? echo $row[csf("tension")]; ?></p></td>   
...
{  
    ...
    $nameArray=sql_select( "select a.id,a.booking_without_order,a.booking_type,a.entry_form,a.prod_no_prefix,a.prod_no_prefix_num,a. yd_prod_no,a.company_id,a.party_id,a.prod_date,a.machine_rpm,a.tension,a.yarn_type_id,a.Winding_Cone_Type,a.remarks,a.batch_id  from yd_production_mst a where a.id='$data'");
    ...
    {   
		...
		echo "load_drop_down('requires/yd_re_winding_controller', document.getElementById('cbo_company_name').value, 'load_drop_down_buyer', 'party_td' );\n";
		...
		echo "document.getElementById('txtMachineRpm').value         = '".$row[csf("machine_rpm")]."';\n"; 
		echo "document.getElementById('txtTension').value           = '".$row[csf("tension")]."';\n"; 
		echo "document.getElementById('cbo_party_name').value       = '".$row[csf("party_id")]."';\n";     
		echo "document.getElementById('update_id').value            = '".$row[csf("id")]."';\n";
		echo "$('#cbo_party_name').attr('disabled','true')".";\n";
		echo "$('#txtProductionDate').attr('disabled','true')".";\n";
		echo "$('#txtMachineRpm').attr('disabled','true')".";\n"; 
		echo "$('#txtTension').attr('disabled','true')".";\n";  
