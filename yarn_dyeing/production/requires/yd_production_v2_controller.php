...
if ($action == "load_drop_floor") {
	...
	echo create_drop_down("cbo_floor", 135, "select id,floor_name from lib_prod_floor where status_active=1 and is_deleted=0 and company_id='$data[0]'  and production_process=3  order by floor_name", "id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/yd_production_v2_controller', document.getElementById('cbo_service_company').value+'**'+this.value, 'load_drop_machine', 'machine_td' );");
	exit();
}
...
if ($action == "load_batch_machine") 
{
	//$floor_arr = return_library_array("select id,floor_name from  lib_prod_floor", 'id', 'floor_name');
	$ex_data = explode('**', $data);
	if($ex_data[2]>0) $flood_cond="and a.floor_id=$ex_data[2]";else $flood_cond="";
	$sql_res = "select a.machine_id,max(a.id) as  mst_id from yd_production_dtls a,lib_machine_name b where a.machine_id=b.id and b.id=$ex_data[1] and a.load_unload_id=1 and a.company_id=$ex_data[0] and a.machine_id=$ex_data[1] and a.multi_batch_load_id=1 and a.status_active=1 and a.is_deleted=0 $flood_cond group by a.machine_id";
	//echo $sql_res;die;
	$nameArray = sql_select($sql_res);
	$mst_id=$nameArray[0][csf('mst_id')];
	
	$sql_res_unload = "select a.machine_id,a.batch_id,a.process_end_date,a.floor_id,a.end_hours,a.end_minutes from yd_production_dtls a,lib_machine_name b where a.machine_id=b.id   and a.load_unload_id=2 and a.company_id=$ex_data[0]  and a.machine_id=$ex_data[1] and a.status_active=1 and a.is_deleted=0";
	$nameArray_unload = sql_select($sql_res_unload);
	foreach ($nameArray_unload as $row) {
		
		$unload_chk_arr[$row[csf("batch_id")]]=$row[csf("batch_id")];
	}
	
	$sql_res_mst = "select a.machine_id,a.batch_id,a.process_end_date,a.floor_id,a.end_hours,a.end_minutes from yd_production_dtls a,lib_machine_name b where a.machine_id=b.id and a.id=$mst_id and a.load_unload_id=1 and a.company_id=$ex_data[0]  and a.machine_id=$ex_data[1] and a.multi_batch_load_id=1 and a.status_active=1 and a.is_deleted=0";
	$nameArray_mst = sql_select($sql_res_mst);
	if(count($nameArray_mst)>0)
	{
		foreach ($nameArray_mst as $row) {
			if($unload_chk_arr[$row[csf("batch_id")]]=='')
			{
			echo "document.getElementById('txt_process_start_date').value 			= '" . change_date_format($row[csf("process_end_date")]) . "';\n";
			echo "document.getElementById('txt_start_hours').value 			= '" . $row[csf("end_hours")] . "';\n";
			echo "document.getElementById('txt_start_minutes').value 			= '" . $row[csf("end_minutes")] . "';\n";
        ...
        <form name="searchbatchnofrm" id="searchbatchnofrm">
            <fieldset style="width:790px;">
                ...
                <table cellpadding="0" cellspacing="0" width="770" border="1" rules="all" class="rpt_table">
                    ...
                    <tr>
                        <th width="150px" style="display:none">Batch Type</th>
                    ...
                    <tr>
                        <td align="center" style="display:none">
							<?
							echo create_drop_down("cbo_batch_type", 150, $order_source, "", 0, "--Select--", 1, 0, 0,"1,3,4");
							?>
                        </td>
                        <td align="center">
...
if ($action == "create_batch_search_list_view") {
	...
	if ($search_type == 1) {
		if ($batch_no != '') $batch_cond = " and a.batch_number='$batch_no'"; else $batch_cond = "";
	} else if ($search_type == 4 || $search_type == 0) {
		if ($batch_no != '') $batch_cond = " and a.batch_number like '%$batch_no%'"; else $batch_cond = "";
	...
	{
		$user_working_comp_cond = " and a.working_company_id in (".$_SESSION['logic_erp']["company_id"].")";
	}
 
	if ($load_unload_id == 1) 
	{
		//$sql = "SELECT a.id, a.double_dyeing,a.entry_form,a.batch_no, a.batch_date, a.batch_weight, a.booking_no, a.extention_no, a.color_id, a.batch_against, a.re_dyeing_from, a.is_sales,a.sales_order_no,a.sales_order_id 
		//from pro_batch_create_mst a 
		//where a.batch_for in(0,1,3) and a.batch_against<>4 and a.status_active=1 and a.is_deleted=0  $search_field_cond_batch $batch_date_con $batch_cond $user_working_comp_cond 
		//group by  a.id, a.double_dyeing,a.entry_form,a.batch_no, a.batch_date, a.batch_weight, a.booking_no, a.extention_no, a.color_id, a.batch_against, a.re_dyeing_from, a.is_sales,a.sales_order_no,a.sales_order_id order by a.id desc";
		
		/*select 
   rowid, id, yd_job_id, order_id, 
   entry_form, yd_batch_id_prefix, yd_batch_id_prefix_num, 
   yd_batch_id, company_id, location_id, 
   batch_color_id, batch_color_range, batch_number, 
   batch_against, batch_weight, extention_no, 
   duration_req, process_id, machine_id, 
   batch_date, remarks, inserted_by, 
   insert_date, updated_by, update_date, 
   status_active, is_deleted, booking_without_order, 
   booking_type, order_no, sales_order_id, 
   sales_order_no, load_unload_id
from platformerpv3.yd_batch_mst
where
batch_number = '1111'
order by id desc
*/
...
/*select 
   rowid, id, mst_id, yd_job_id, 
   yd_job_dtls_id, order_id, winding_pckg_qty, 
   bobbin_type, quantity, remarks, 
   inserted_by, insert_date, updated_by, 
   update_date, status_active, is_deleted, 
   sales_order_id, product_id, order_no, 
   sales_order_no, entry_form
from platformerpv3.yd_batch_dtls
where
order_no = 'RpC-YDW-22-00043'
order by id desc*/
		
		  $sql = "select a.id,a.entry_form, a.yd_batch_id, a.batch_number as batch_no, a.batch_date,a.batch_against, a.batch_weight,a.extention_no,a.batch_color_id as color_id, b.yd_job,c.order_no, 
   c.sales_order_no
            from yd_batch_mst a,yd_batch_dtls c, yd_ord_mst b
            where a.id=c.mst_id and  a.is_deleted=0 and a.status_active=1  and  b.is_deleted=0 and b.status_active=1  and c.is_deleted=0 and c.status_active=1 $batch_cond $batch_date_con and a.yd_job_id=b.id and c.yd_job_id=b.id group by  a.id,a.entry_form, a.yd_batch_id, a.batch_number, a.batch_date,a.batch_against, a.batch_weight,a.extention_no,a.batch_color_id, b.yd_job,c.order_no, 
   c.sales_order_no "; //and a.load_unload_id!=1
	} 
	else 
	{
		//$sql = "SELECT a.id,a.double_dyeing,a.entry_form, a.batch_no, a.batch_date, a.batch_weight, a.booking_no, a.extention_no, a.color_id, a.batch_against, a.re_dyeing_from,b.system_no,a.is_sales 
		//from pro_batch_create_mst a,yd_production_dtls b 
		//where a.id=b.batch_id and a.batch_for in(0,1,3) and a.batch_against<>4 and a.status_active=1 and a.is_deleted=0 and b.load_unload_id=1  $search_field_cond_batch $batch_date_con $batch_cond $user_working_comp_cond 
		//group by a.id,a.double_dyeing,a.entry_form, a.batch_no, a.batch_date, a.batch_weight, a.booking_no, a.extention_no, a.color_id, a.batch_against, a.re_dyeing_from,b.system_no,a.is_sales order by a.id desc";
		
		
		 $sql = "select a.id,a.entry_form, a.yd_batch_id, a.batch_number as batch_no, a.batch_date,a.batch_against, a.batch_weight,a.extention_no,a.batch_color_id as color_id, b.yd_job,c.order_no, 
   c.sales_order_no
            from yd_batch_mst a,yd_batch_dtls c, yd_ord_mst b,yd_production_dtls d
            where a.id=c.mst_id and  a.is_deleted=0 and a.status_active=1  and  b.is_deleted=0 and b.status_active=1  and c.is_deleted=0 and c.status_active=1 $batch_cond $batch_date_con and a.yd_job_id=b.id and c.yd_job_id=b.id  and a.batch_number=d.batch_no and d.entry_form=414 and d.load_unload_id=1   group by  a.id,a.entry_form, a.yd_batch_id, a.batch_number, a.batch_date,a.batch_against, a.batch_weight,a.extention_no,a.batch_color_id, b.yd_job,c.order_no, 
   c.sales_order_no "; 
		
		/* $sql = "select a.id, a.yd_batch_id, a.batch_number, b.yd_job
            from yd_batch_mst a, yd_ord_mst b,yd_production_dtls c
            where a.is_deleted=0 and a.status_active=1 and a.yd_batch_id=c.batch_no and c.entry_form=414 and c.load_unload_id=1  $condition and a.yd_job_id=b.id";*/
	}
	...
	/*foreach ($nameArray as $row)
	{
		...
		{
			$all_sales_booking[$row[csf('booking_no')]]=$row[csf('booking_no')];
		}
		else
		{
			$all_batch_no_arr[$row[csf('id')]]=$row[csf('id')];
		}
	}

	$all_sales_booking = array_filter($all_sales_booking);
	
	if(!empty($all_sales_booking))
	{
		$all_sales_booking_nos = "'".implode("','",explode(",", $all_sales_booking))."'";

		$all_sales_booking_cond="";
		$bookCond="";
        if($db_type==2 && count($all_sales_booking)>999)
        {
        	$all_sales_booking_chunk=array_chunk($all_sales_booking,999) ;
        	foreach($all_sales_booking_chunk as $chunk_arr)
        	{
        		$chunk_arr_value=implode(",",$chunk_arr);
        		$bookCond.=" b.booking_no in($chunk_arr_value) or ";
        	}

        	$all_sales_booking_cond.=" and (".chop($bookCond,'or ').")";
        }
        else
        {
        	$all_sales_booking_cond=" and b.booking_no in($all_sales_booking_nos)";
        }

        $sql_sales_job=array();
		$sql_sales_job=sql_select("select b.job_no as job_no_mst,b.booking_no,f.job_no as sales_order_no,f.within_group from wo_booking_mst a, wo_booking_dtls b, wo_po_break_down c, FABRIC_SALES_ORDER_MST f where a.booking_no=b.booking_no and b.booking_no=f.SALES_BOOKING_NO and b.po_break_down_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $all_sales_booking_cond group by b.job_no,b.booking_no,f.job_no,f.within_group");
	...
	$all_batch_no_arr = array_filter($all_batch_no_arr);
	if(!empty($all_batch_no_arr))
	{
		$all_batch_no_cond="";
		$batchCond="";
        ...
        {
        	$all_batch_no_cond=" and b.mst_id in(". implode(",",explode(",", $all_sales_booking)). ")";
        }
        ...
        if ($db_type == 2) 
        {
			if ($batch_type == 0 || $batch_type == 2) {
				...
				$sql_po = sql_select("select b.mst_id, a.job_no_mst,listagg(cast(a.order_no as varchar2(4000)),',') within group (order by a.order_no) as po_no from  subcon_ord_dtls a, pro_batch_create_dtls b where a.id=b.po_id and  b.status_active=1 and b.is_deleted=0 $all_batch_no_cond group by b.mst_id,a.job_no_mst");
			} else {
				$sql_po = sql_select("select b.mst_id, a.job_no_mst,listagg(cast(a.po_number as varchar2(4000)),',') within group (order by a.po_number) as po_no from wo_po_break_down a, pro_batch_create_dtls b where a.id=b.po_id and  b.status_active=1 and b.is_deleted=0 $all_batch_no_cond group by b.mst_id,a.job_no_mst");
			}
		}
		if ($db_type == 0) 
		{
			if ($batch_type == 0 || $batch_type == 2) 
			{
				$sql_po = sql_select("select b.mst_id, a.job_no_mst,group_concat( distinct a.order_no )  as po_no from  subcon_ord_dtls a, pro_batch_create_dtls b where a.id=b.po_id and  b.status_active=1 and b.is_deleted=0 $all_batch_no_cond group by b.mst_id,a.job_no_mst");
			} else {
				$sql_po = sql_select("select b.mst_id, a.job_no_mst,group_concat(distinct a.po_number)  as po_no from wo_po_break_down a, pro_batch_create_dtls b where a.id=b.po_id and  b.status_active=1 and b.is_deleted=0 $all_batch_no_cond group by b.mst_id,a.job_no_mst");
			}
		}
	...
	// $sql;
	$color_arr = return_library_array("select id, color_name from lib_color", 'id', 'color_name');
 	/*$sql_load="select b.batch_id,b.result,b.load_unload_id from pro_batch_create_mst a,yd_production_dtls b where a.id=b.batch_id and b.load_unload_id in(1,2) and b.entry_form=414 and b.status_active=1  $search_field_cond_batch $batch_date_con $batch_cond  order by b.id desc";
	...
	/*$control_based_on_chemical_issue = return_field_value("chemical_issue", "variable_settings_production", "company_name ='$company_id' and variable_list=63 and is_deleted=0 and status_active=1");
	// echo $control_based_on_chemical_issue;die;
	$chemical_issue_arr=array();
	if($control_based_on_chemical_issue==1) // Yes
	{
	 	$chemical_issue="SELECT a.id as batch_id from pro_batch_create_mst a,inv_issue_master b 
	 	where to_char(a.id)=b.batch_no and b.issue_basis=7 and b.entry_form=5 and b.status_active=1 $search_field_cond_batch $batch_date_con $batch_cond order by b.id desc";
		$result_chemical_issue = sql_select($chemical_issue);
		$chemical_issue_arr=array();
		foreach ($result_chemical_issue as $row)
		{
			$chemical_issue_arr[$row[csf('batch_id')]]= $row[csf('batch_id')];
		}
	}
	// echo "<pre>";print_r($chemical_issue_arr);*/
	?>
    <div>
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="850" class="rpt_table">
            ...
            <th width="115">YD Job No</th>
            <th width="80">Color</th>
            <th>Job/Sales order no</th>
            </thead>
        </table>
				...
				if (!empty($nameArray)) 
				{
					foreach ($nameArray as $selectResult) 
					{
						...
						if ($selectResult[csf('re_dyeing_from')] == 0 || $selectResult[csf('re_dyeing_from')]>0) 
						{
							if($load_unload_arr[$selectResult[csf('id')]]=="")
							{
	                            ...
	                            <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
	                                ...
	                                <td width="110"><p><? echo $selectResult[csf('batch_no')]; ?></p></td>
	                                ...
	                                <td width="115"><p><? echo $selectResult[csf('yd_job')];//$job_no; ?></p></td>
	                                <td width="80"><p><? echo $color_arr[$selectResult[csf('color_id')]]; ?></p></td>
	                                <td><? echo $selectResult[csf('sales_order_no')];//$po_no; ?></td>
	                            </tr>
								<?
								$i++;
							}
						...
						else 
						{
							if ($load_unload_id == 1) 
							{
								$sql_re = "SELECT a.id,a.entry_form, a.batch_no, a.batch_date, a.batch_weight, a.booking_no, MAX(a.extention_no) as extention_no, a.color_id, a.batch_against,a.is_sales, a.re_dyeing_from 
								from pro_batch_create_mst a 
								where  a.batch_for in(0,1) and a.entry_form in(0,136) and a.batch_against<>4 and a.status_active=1 and a.is_deleted=0 and a.id='" . $selectResult[csf('re_dyeing_from')] . "'  
								group by a.id,a.entry_form, a.batch_no, a.batch_date, a.batch_weight, a.booking_no,a.color_id, a.batch_against, a.re_dyeing_from,a.is_sales  ";
							} 
							else 
							{
								$sql_re = "SELECT a.id,a.entry_form, a.batch_no, a.batch_date, a.batch_weight, a.booking_no, MAX(a.extention_no) as extention_no, a.color_id, a.batch_against,b.system_no,a.is_sales, a.re_dyeing_from 
								from pro_batch_create_mst a,yd_production_dtls b 
								where a.id=b.batch_id and  a.batch_for in(0,1) and a.entry_form in(0,136) and a.batch_against<>4 and a.status_active=1 and a.is_deleted=0 and a.id='" . $selectResult[csf('re_dyeing_from')] . "'  
								group by a.id,a.entry_form,b.system_no,a.batch_no, a.batch_date, a.batch_weight, a.booking_no,a.color_id,b.system_no,a.batch_against,b.re_dyeing_from,a.is_sales  ";
							}
...
if ($action == 'populate_data_from_batch') 
{
	...
	//echo $entry_form.'entry_form'; 
	//echo $load_unload.'load_unload'; die;
	//$is_sales = $ex_data[3];
	
		/* $sql = "select a.id,a.entry_form, a.yd_batch_id, a.batch_number as batch_no, a.batch_date,a.batch_against, a.batch_weight,a.extention_no,a.batch_color_id as color_id, b.yd_job,c.order_no, 
   c.sales_order_no
            from yd_batch_mst a,yd_batch_dtls c, yd_ord_mst b,yd_production_dtls d
            where a.id=c.mst_id and  a.is_deleted=0 and a.status_active=1  and  b.is_deleted=0 and b.status_active=1  and c.is_deleted=0 and c.status_active=1 $batch_cond $batch_date_con and a.yd_job_id=b.id and c.yd_job_id=b.id  and a.yd_batch_id=d.batch_no and d.entry_form=414 and d.load_unload_id=1   group by  a.id,a.entry_form, a.yd_batch_id, a.batch_number, a.batch_date,a.batch_against, a.batch_weight,a.extention_no,a.batch_color_id, b.yd_job,c.order_no, 
   c.sales_order_no "; 
	*/
	
	//echo "select company_id,entry_form,double_dyeing from yd_batch_mst where id ='$batch_id' and is_deleted=0 and status_active=1"; die;
	$double_dyeing=0;
	$batch_data =sql_select("select company_id,entry_form,double_dyeing,batch_number, yd_batch_id,location_id, extention_no, batch_color_id, order_id, booking_without_order, booking_type from yd_batch_mst where id ='$batch_id' and is_deleted=0 and status_active=1");
	...
	echo "document.getElementById('hdnBookingWithoutOrder').value = '".$booking_without_order."';\n";
    echo "document.getElementById('hdnBookingType').value = '".$booking_type."';\n";
	...
	$sales_job_arr=array();$load_arr=array();
	 $last_load="select batch_id,result,load_unload_id from yd_production_dtls where load_unload_id in(1,2) and entry_form=414 and status_active=1 and batch_id=$batch_id  order by id desc";
	...
	//echo "select batch_id,result from yd_production_dtls where load_unload_id in(1,2) and entry_form=414 and status_active=1 and batch_id=$batch_id  order by id desc";
	$sql_load=sql_select("select batch_id,result,load_unload_id from yd_production_dtls where load_unload_id in(1,2) and entry_form=414 and status_active=1 and batch_id=$batch_id  order by id desc");
	if($multi_dyeing==2) //Multi is no
	{
	...
	else if ($db_type == 2)  $select_field1 = " group by a.id,b.sales_order_no,a.double_dyeing,a.batch_number,a.batch_weight,a.batch_color_id,a.company_id,a.process_id, a.booking_without_order,a.entry_form,b.order_no,a.yd_job_id  order by a.id";
	...
	if ($db_type == 0) $select_list = "group_concat(distinct(b.yd_job_dtls_id)) as po_id";
	else if ($db_type == 2) $select_list = " listagg(b.yd_job_dtls_id,',') within group (order by b.yd_job_dtls_id) as po_id";
	//echo $entry_form.'XXX';
	//$booking_id=implode(",",array_unique(explode(",",$booking_id)));
	if ($load_unload == 1) 
	{
	//and a.id not in(select batch_id from yd_production_dtls where load_unload_id in(1) and entry_form=414 and status_active=1)
		if ($batch_no != '') 
		{
			 
				$sql_re = "select a.id as id,a.double_dyeing,a.batch_number as batch_no,0 as is_sales, a.entry_form,a.company_id,0 as working_company_id, a.batch_weight,Max(a.extention_no) as extention_no,a.process_id as process_id_batch, a.batch_color_id as color_id, a.booking_without_order,b.order_no as booking_no, b.sales_order_no,a.yd_job_id,
				sum(b.quantity) as batch_qnty,  $select_list from yd_batch_mst a,yd_batch_dtls b where
				a.id='$batch_id'  and a.entry_form in(398) and a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and a.status_active=1  and a.is_deleted=0  $select_field1";
	...
	if($load_unload_arr[$row[csf("id")]]=="")
	{
		...
		echo "load_drop_down( 'requires/yd_production_v2_controller', " . $inhouse . "+'**'+" . $row[csf("working_company_id")] . ", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
		...
		echo "load_drop_down( 'requires/yd_production_v2_controller', '" . $row[csf("working_company_id")] . "', 'load_drop_floor', 'floor_td' );\n";
		echo "fnc_mc_load();\n";
...
//echo $salesOrder."wewe"; die;
		...
		{
			$job_id=$row[csf('yd_job_id')];
			$buyer_id = return_field_value("party_id", "yd_ord_mst", "id ='$job_id' and is_deleted=0 and status_active=1 and  entry_form = 374");
			
			//echo $buyer_id.'"fssf"'; die;
			echo "document.getElementById('txt_buyer').value 			= '" . $buyer_arr[$buyer_id] . "';\n";
			echo "document.getElementById('txt_job_no').value 			= '" . $row[csf('sales_order_no')] . "';\n";
		...
		$sql_batch_d = sql_select("select id,service_source,system_no,service_company,received_chalan,issue_chalan,issue_challan_mst_id,company_id,batch_id,batch_no,process_end_date,end_hours,end_minutes,machine_id,floor_id,process_id,ltb_btb_id,remarks,dyeing_type_id,hour_load_meter from
			yd_production_dtls where batch_id='$batch_id' and entry_form=414 and load_unload_id=1 and status_active=1 and is_deleted=0 ");
		...
		{//$minute=str_pad($r_batch[csf("end_minutes")],2,'0',STR_PAD_LEFT);
			...
			echo "load_drop_down( 'requires/yd_production_v2_controller', " . $dyeing_d[csf("service_source")] . "+'**'+" . $dyeing_d[csf("company_id")] . ", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
			echo "$('#cbo_service_company').val(" . $dyeing_d[csf("service_company")] . ");\n";
			echo "load_drop_down( 'requires/yd_production_v2_controller', '" . $dyeing_d[csf("service_company")] . "', 'load_drop_floor', 'floor_td' );\n";
	...
	foreach ($sql_batch as $r_batch) 
	{
		if ($load_unload == 1) //Load
		{
			...
			echo "load_drop_down( 'requires/yd_production_v2_controller', '" . $r_batch[csf("service_company")] . "', 'load_drop_floor', 'floor_td' );\n";
			...
			echo "load_drop_down( 'requires/yd_production_v2_controller', " . $r_batch[csf("service_source")] . "+'**'+" . $r_batch[csf("company_id")] . ", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
			...
			echo "load_drop_down( 'requires/yd_production_v2_controller', document.getElementById('cbo_service_company').value+'**'+" . $r_batch[csf("floor_id")] . ", 'load_drop_machine', 'machine_td' );\n";

			echo "document.getElementById('cbo_machine_name').value = '" . $r_batch[csf("machine_id")] . "';\n";
			//echo "fnc_load_data(".$r_batch[csf("multi_batch_load_id")].");\n";
			...
			if ($r_batch[csf("id")] != 0) {
				...
				echo "set_button_status(0, '" . $_SESSION['page_permission'] . "', 'fnc_yd_production_dtls',0);\n";
			} else {
		...
		else if ($load_unload == 2) //Unload
		{
			if ($r_batch[csf("load_unload_id")] == 2) 
			{
				...
				echo "document.getElementById('cbo_responsibility').value	= '" . $r_batch[csf("responsibility_id")] . "';\n";
			...
			//echo "load_drop_down( 'requires/yd_production_v2_controller', '".$r_batch[csf("process_id")]."', 'load_drop_sub_process', 'sub_process_td' );\n";
...
if ($action == 'populate_data_from_batch2') 
{
	...
	$batch_no = $ex_data[2];
	$company = $ex_data[3];
	$update_mst_id = $ex_data[5];
	//echo $update_mst_id.'DS';;
	$entry_form = $ex_data[4];
	...
	//$batch_data =sql_select("select company_id,double_dyeing,process_id from yd_batch_mst where id ='$batch_id' and entry_form in(398) and is_deleted=0 and status_active=1");
	
	
	//echo "select company_id,entry_form,double_dyeing,batch_number, yd_batch_id,location_id, extention_no, batch_color_id, order_id, booking_without_order, booking_type,process_id from yd_batch_mst where id ='$batch_id' and entry_form in(398) and is_deleted=0 and status_active=1"; die;
	
	$batch_data =sql_select("select company_id,entry_form,double_dyeing,batch_number, yd_batch_id,location_id, extention_no, batch_color_id, order_id, booking_without_order, booking_type,process_id from yd_batch_mst where id ='$batch_id' and entry_form in(398) and is_deleted=0 and status_active=1");
	foreach ($batch_data as $row)
	{
		$company_id = $row[csf('company_id')];
		//$entry_form_id= $row[csf('entry_form')];
	...
	if ($db_type == 0) $select_field1 = "order by a.id";
	else if ($db_type == 2) $select_field1 = " group by a.id,a.sales_order_no,a.batch_number,a.batch_weight,a.batch_color_id,a.company_id,a.process_id, a.booking_without_order,a.entry_form,b.order_no,b.sales_order_no,a.yd_job_id order by a.id";
	if ($db_type == 0) $select_list = "group_concat(distinct(b.yd_job_dtls_id)) as po_id";
	else if ($db_type == 2) $select_list = " listagg(b.yd_job_dtls_id,',') within group (order by b.yd_job_dtls_id) as po_id";
	//$booking_id=implode(",",array_unique(explode(",",$booking_id)));
	//echo $entry_form.'AA';
	if ($load_unload == 1) 
	{
		if ($batch_no != '') 
		{
 			$data_array = sql_select("select a.id as id,a.batch_number as batch_no,a.entry_form,a.company_id,0 as working_company_id, a.batch_weight,Max(a.extention_no) as extention_no,a.process_id as process_id_batch,a.batch_color_id as color_id, a.booking_without_order,b.order_no as booking_no,b.sales_order_no,a.yd_job_id,
						sum(b.quantity) as batch_qnty,  $select_list from yd_batch_mst a,yd_batch_dtls b where
						a.id='$batch_id'  and a.entry_form in(398) and a.id=b.mst_id and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0  $select_field1");
	...
	foreach ($data_array as $row) {
		...
		//echo "document.getElementById('roll_maintained').value 				= '" . $roll_maintained . "';\n";
		echo "document.getElementById('page_upto').value 				= '" . $page_upto_id . "';\n";
		echo "document.getElementById('hidden_double_dyeing').value 				= '" . $multi_dyeing . "';\n";
		...
		$sql_batch_d = sql_select("select id,service_source,system_no,service_company,received_chalan,issue_chalan,issue_challan_mst_id,company_id,batch_id,batch_no,process_end_date,end_hours,end_minutes,machine_id,floor_id,process_id,ltb_btb_id,remarks,hour_load_meter from
					yd_production_dtls where batch_id='$batch_id' and id=$update_mst_id and entry_form=414 and load_unload_id=1 and status_active=1 and is_deleted=0 ");
	...
	if ($load_unload == 1) {
		...
		$sql_batch = sql_select(
			"select id,batch_no,company_id,system_no,batch_id,service_source,service_company,received_chalan,issue_chalan,issue_challan_mst_id,process_end_date,load_unload_id,end_hours,end_minutes,machine_id,floor_id,process_id,
			ltb_btb_id,water_flow_meter,result,remarks,dyeing_type_id,hour_load_meter,multi_batch_load_id	from yd_production_dtls where batch_id='$batch_id' and id=$update_mst_id and entry_form=414 and load_unload_id in(1) and status_active=1 and is_deleted=0  ");
	} else if ($load_unload == 2) {
		$sql_batch = sql_select("select id,batch_no,company_id,system_no,service_source,service_company,received_chalan,issue_chalan,issue_challan_mst_id,batch_id,process_end_date,load_unload_id,end_hours,end_minutes,machine_id,floor_id,process_id,ltb_btb_id,water_flow_meter,result,remarks,dyeing_type_id,hour_unload_meter,shift_name,responsibility_id,fabric_type,production_date from yd_production_dtls where batch_id='$batch_id' and entry_form=414 and load_unload_id in(2,1)  and status_active=1 and is_deleted=0 $select_group_row1");
	...
	foreach ($sql_batch as $r_batch) {
		if ($load_unload == 1) //Load
		{
			...
			if ($r_batch[csf("id")] != 0) {
				...
				echo "set_button_status(1, '" . $_SESSION['page_permission'] . "', 'fnc_yd_production_dtls',1);\n";
			} else {
				//echo "document.getElementById('txt_system_no').value 			= '';\n";
		...
		{
			if ($r_batch[csf("load_unload_id")] == 2) {
				echo "document.getElementById('txt_update_id').value 		= '" . $update_mst_id . "';\n";
				echo "document.getElementById('txt_batch_no').value 			= '" . $r_batch[csf("batch_no")] . "';\n";
				echo "document.getElementById('txt_process_end_date').value = '" . ($r_batch[csf("load_unload_id")] == 1 ? "" : change_date_format($r_batch[csf("process_end_date")])) . "';\n";
				echo "document.getElementById('txt_process_date').value = '" . ($r_batch[csf("load_unload_id")] == 1 ? "" : change_date_format($r_batch[csf("production_date")])) . "';\n";
				echo "document.getElementById('cbo_shift_name').value	= '" . $r_batch[csf("shift_name")] . "';\n";
...
if ($action == 'show_fabric_desc_listview')
 {
	...
	from product_details_master c,yd_batch_dtls b where c.id=b.product_id and b.mst_id ='$batch_id' and b.status_active=1");
...
//echo $company_id.'=='.$double_dyeing.'='.$page_upto_id.'='.$roll_maintained_id;
		...
		$max_load_id = return_field_value("max(id) as maxload_id", "yd_production_dtls", "batch_id =$batch_id and entry_form=414  and load_unload_id in(1) and is_deleted=0 and status_active=1","maxload_id");
		$max_unload_id = return_field_value("max(id)  as maxload_id", "yd_production_dtls", "batch_id =$batch_id and entry_form=414  and load_unload_id in(2) and is_deleted=0 and status_active=1","maxload_id");

		if($max_load_id!="")
		{
		 $load_insert = ("select a.load_unload_id,b.production_qty,b.roll_id,b.no_of_roll,b.prod_id,b.gsm,b.dia_width,b.barcode_no,b.width_dia_type from  yd_production_dtls a, pro_yd_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$batch_id and a.entry_form=414  and a.load_unload_id in(1) and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.is_deleted=0 and b.mst_id in($max_load_id) ");
		...
		foreach ($load_result as $row) {
					...
					$load_qty_arr2[$row[csf('prod_id')]][$row[csf('gsm')]][$row[csf('width_dia_type')]][$row[csf('no_of_roll')]]= $row[csf('production_qty')];
		...
		if($max_unload_id!="")
		{
		 $unload_insert = ("select a.load_unload_id,b.production_qty,b.roll_id,b.prod_id,b.gsm,b.dia_width,b.barcode_no,b.width_dia_type from  yd_production_dtls a, pro_yd_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$batch_id and a.entry_form=414  and a.load_unload_id in(2) and a.status_active=1 and b.status_active=1 and b.is_deleted=0 and a.is_deleted=0  ");//and b.mst_id in($max_unload_id)
  ...
  //echo $entry_form.'='.$page_upto_id.'='.$roll_maintained;
			...
			$batch_id_search = sql_select("select a.batch_id from yd_production_dtls a,pro_yd_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$batch_id and company_id=$company_id and entry_form=414 and a.status_active=1 and b.status_active=1 $load_unload_cond");
			//echo "select a.batch_id from yd_production_dtls a,pro_yd_subprocess_dtls b where a.id=b.mst_id and a.batch_id=$batch_id and company_id=$company_id and entry_form=414 and a.status_active=1 and b.status_active=1 $load_unload_cond";
		...
		$sql_result ="select a.entry_form,0 as width_dia_type,b.product_id as prod_id,b.yd_job_dtls_id as po_id, sum(b.quantity) as batch_qnty,b.winding_pckg_qty,b.order_id,b.yd_job_id,b.sales_order_id, b.bobbin_type,c.order_no, c.style_ref,c.lot, c.count_id, c.yarn_type_id, c.yarn_composition_id from yd_batch_dtls b,yd_batch_mst a,yd_ord_dtls c where b.mst_id=$batch_id and a.id=b.mst_id and  a.entry_form in(398)  and b.yd_job_dtls_id=c.id  and b.status_active=1 and b.is_deleted=0 $batch_insert_cond  group by a.entry_form,b.product_id,b.yd_job_dtls_id,b.winding_pckg_qty,b.order_id,b.yd_job_id,b.sales_order_id,b.bobbin_type,c.order_no,c.style_ref, c.lot, c.count_id, c.yarn_type_id, c.yarn_composition_id";
				$result=sql_select($sql_result);
			}
			...
			if (count($result) > 0)
			 {
				...
				foreach ($result as $row) {
					...
					if($dyeing_type_id==2)//CBP dyeing
					{
						if($load_unload == 2)
						{
							$load_prod_qnty=$load_qty_arr2[$row[csf('prod_id')]][$gsm][$row[csf('width_dia_type')]][$row[csf('no_of_roll')]];
					...
					<tr class="general" id="row_<? echo $i; ?>">
                        ...
                        <td><input type="text" name="txtlot_<? echo $i; ?>" id="txtlot_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px;" value="<? echo $row[csf('lot')]; ?>" readonly  disabled/></td>
						<td><input type="text" name="txtyarncount_<? echo $i; ?>" id="txtyarncount_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo  $count_arr[$row[csf('count_id')]]; ?>"  readonly disabled/>
                        <input type="hidden" name="txtyarncountid_<? echo $i; ?>" id="txtyarncountid_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo  $row[csf('count_id')]; ?>"  readonly disabled/> 
                        <input type="hidden" name="txtjobid_<? echo $i; ?>" id="txtjobid_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo  $row[csf('yd_job_id')]; ?>"  readonly disabled/>
                        <input type="hidden" name="txtorderid_<? echo $i; ?>" id="txtorderid_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo  $row[csf('order_id')]; ?>"  readonly disabled/>
                          <input type="hidden" name="txtsalesorderid_<? echo $i; ?>" id="txtsalesorderid_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo  $row[csf('sales_order_id')]; ?>"  readonly disabled/>
                        </td>
						<td>
                        <input type="text" name="txtyarntype_<? echo $i; ?>" id="txtyarntype_<? echo $i; ?>"  class="text_boxes_numeric" style="width:60px;" value="<? echo $yarn_type[$row[csf('yarn_type_id')]]; ?>" readonly  disabled/>
                         <input type="hidden" name="txtyarntypeid_<? echo $i; ?>" id="txtyarntypeid_<? echo $i; ?>"  class="text_boxes_numeric" style="width:60px;" value="<? echo $row[csf('yarn_type_id')]; ?>" readonly  disabled/>
                        
                        <input type="hidden" name="txtbrand_<? echo $i; ?>" id="txtbrand_<? echo $i; ?>"  class="text_boxes_numeric" style="width:60px;" value="<? echo $brand_value; ?>" readonly  disabled/>
							<input type="hidden" name="txtprodid_<? echo $i; ?>" id="txtprodid_<? echo $i; ?>" value="<? echo $row[csf('prod_id')]; ?>"/>
                            <input type="hidden" name="txtjobdtlsid_<? echo $i; ?>" id="txtjobdtlsid_<? echo $i; ?>" value="<? echo $row[csf('po_id')]; ?>"/>
						...
						<td><input type="text" name="txtconscomp_<? echo $i; ?>" id="txtconscomp_<? echo $i; ?>" class="text_boxes" style="width:170px;" value="<?  echo  $comp_arr[$row[csf('yarn_composition_id')]];; ?>" disabled/></td>
						<td>
                         <input type="text" name="txtbobintype_<? echo $i; ?>" id="txtbobintype_<? echo $i; ?>" class="text_boxes" style="width:40px;" value="<? echo  $row[csf('bobbin_type')];; ?>"  readonly disabled />
                        
                        <input type="hidden" name="txtgsm_<? echo $i; ?>" id="txtgsm_<? echo $i; ?>" class="text_boxes" style="width:40px;" value="<? echo trim($gsm); ?>"/>
                        
                        
                        </td>
						<td>
                         
                         <input type="text" name="txtWindingPackageQty_<? echo $i; ?>" id="txtWindingPackageQty_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px;" value="<? echo number_format($row[csf('winding_pckg_qty')], 2, ".", ""); ?>" disabled/>
                         
                         
                        <input type="hidden" name="txtdiawidth_<? echo $i; ?>" id="txtdiawidth_<? echo $i; ?>" class="text_boxes" style="width:40px;" value="<? echo $dia_width; ?>" disabled/></td>
						<td style="display:none"><input type="text" name="txtdiatype_<? echo $i; ?>" id="txtdiatype_<? echo $i; ?>" class="text_boxes" style="width:70px;" value="<? echo $fabric_typee[$row[csf('width_dia_type')]]; ?>" disabled/>
						...
						<td style="display:none"><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes"   style="width:40px;" value="<? echo $roll_no; ?>"/> <input type="hidden" name="rollid_<? echo $i; ?>" id="rollid_<? echo $i; ?>" style="width:50px;" value="<? echo $row[csf('roll_id')]; ?>" class="text_boxes_numeric"/>
						</td>
						...
						<td><input type="text" name="txtprodqnty_<? echo $i; ?>" id="txtprodqnty_<? echo $i; ?>"class="text_boxes_numeric" style="width:60px;" onKeyUp="calculate_production_qnty();" value="<? echo number_format($prod_qnty, 2, ".", ""); ?>" <? echo $readdata ?>  readonly/>
                        <input type="hidden" name="hiddenprodqnty_<? echo $i; ?>" id="hiddenprodqnty_<? echo $i; ?>"class="text_boxes_numeric" style="width:60px;"  value="<? echo number_format($prod_qnty, 2, ".", ""); ?>" <? echo $readdata ?> />
                        </td>
						<td style="display:none"><input type="text" name="txtrate_<? echo $i; ?>" id="txtrate_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo $row[csf('rate')]; ?>" readonly/></td>
						<td style="display:none"><input type="text" name="txtamount_<? echo $i; ?>" id="txtamount_<? echo $i; ?>" class="text_boxes_numeric" style="width:70px;" value="<? echo $row[csf('amount')]; ?>" readonly/></td>
			...
			else
			 {
				...
				<tr class="general" id="row_1">
                        <td> 1 <input type="hidden" id="checkRow_1" name="checkRow_1" > </td>
                        <td><input type="text" name="txtlot_1" id="txtlot_1" class="text_boxes_numeric" style="width:40px;" readonly disabled />  </td>
                        <td><input type="text" name="txtyarncount_1" id="txtyarncount_1" class="text_boxes_numeric" style="width:60px;" readonly disabled /> 
                        <input type="hidden" name="txtyarncountid_1" id="txtyarncountid_1" class="text_boxes_numeric" style="width:60px;" readonly disabled /> 
                        </td>
                        <td> 
                         <input type="text" name="txtyarntype_1" id="txtyarntype_1" class="text_boxes_numeric" style="width:60px;" readonly disabled /> 
                         <input type="hidden" name="txtyarntypeid_1" id="txtyarntypeid_1" class="text_boxes_numeric" style="width:60px;" readonly disabled /> 
                        <input type="hidden" name="txtbrand_1" id="txtbrand_1" class="text_boxes_numeric" style="width:60px;" readonly disabled /> </td> 
                        <td><input type="text" name="txtconscomp_1" id="txtconscomp_1" class="text_boxes" style="width:170px;" readonly disabled /></td>
                        <td> 
                        <input type="text" name="txtbobintype_1" id="txtbobintype_1" class="text_boxes" style="width:40px;"  readonly disabled /> 
                        <input type="hidden" name="txtgsm_1" id="txtgsm_1" class="text_boxes" style="width:40px;"readonly disabled /></td>
                        <td>
                        <input type="text" name="txtWindingPackageQty_1" id="txtWindingPackageQty_1" class="text_boxes" style="width:40px;" readonly  disabled/>
                        <input type="hidden" name="txtdiawidth_1" id="txtdiawidth_1" class="text_boxes" style="width:40px;" readonly  disabled/>
                        </td>
                        <td style="display:none"><input type="text" name="txtdiatype_1" id="txtdiatype_1" class="text_boxes" style="width:70px;" readonly disabled/></td>
                        <td style="display:none"><input type="text" name="txtroll_1" id="txtroll_1" class="text_boxes" style="width:40px;" readonly  disabled/>  
                        ...
                        <td style="display:none"><input type="text" name="txtrate_1" id="txtrate_1" class="text_boxes_numeric" style="width:60px;" readonly/></td>
                        <td style="display:none"><input type="text" name="txtamount_1" id="txtamount_1" class="text_boxes_numeric" style="width:70px;"  readonly/> 
                        ...
                        <input type="hidden" name="txtjobdtlsid_1" id="txtjobdtlsid_1"/>
                        <input type="hidden" name="txtjobid_1" id="txtjobid_1" class="text_boxes_numeric" style="width:60px;" value=""  readonly disabled/>
                        <input type="hidden" name="txtorderid_1" id="txtorderid_1" class="text_boxes_numeric" style="width:60px;" value=""  readonly disabled/>
                        <input type="hidden" name="txtsalesorderid_1" id="txtsalesorderid_1" class="text_boxes_numeric" style="width:60px;" value=""  readonly disabled/>
                        </td> 
				</tr>
				<?
			...
			<tr>
				...
				<td align="right" style="display:none"><input type="text" name="total_amount" id="total_amount" class="text_boxes_numeric" style="width:70px" value="<? echo $tot_amount; ?>" readonly/></td>
			</tr>
			<?
			exit();
...
if ($action == 'issue_show_fabric_desc_listview') {
	...
	$result = sql_select("select a.entry_form,b.width_dia_type,b.prod_id,b.po_id,b.roll_no,b.roll_id ,b.item_description,b.width_dia_type, sum(b.batch_qnty) as batch_qnty from pro_batch_create_dtls b,pro_batch_create_mst a  where b.mst_id=$batch_id and a.id=b.mst_id and  a.entry_form in(0,136) and b.status_active=1 and b.is_deleted=0 group by a.entry_form,b.width_dia_type,b.prod_id,b.po_id,b.roll_no,b.roll_id ,b.item_description,b.width_dia_type");
	foreach ($result as $row) {
		if ($row[csf('entry_form')] == 36) {
			$desc = explode(",", $row[csf('item_description')]);
			//print_r($desc);
        ...
        <tr class="general" id="row_<? echo $i; ?>">
            ...
            <td><input type="text" name="txtbrand_<? echo $i; ?>" id="txtbrand_<? echo $i; ?>"
                ...
                <input type="hidden" name="txtprodid_<? echo $i; ?>" id="txtprodid_<? echo $i; ?>"
                       ...
                       <input type="hidden" name="txtjobdtlsid_<? echo $i; ?>" id="txtjobdtlsid_<? echo $i; ?>" value="<? echo $row[csf('yd_job_dtls_id')]; ?>"/>
...
if($action=="check_process_start_date")
{
	...
	$sql="select min(a.process_end_date) as process_end_date,a.batch_no from yd_production_dtls a,pro_yd_subprocess_dtls b where a.id=b.mst_id and a.load_unload_id=1 and a.entry_form=414 and a.status_active=1 and a.system_no=$system_no group by a.batch_no order by process_end_date";
	//echo $sql;die;
...
if($action=="check_process_production_date")
{
	...
	$fnc_bach_arr=array();
	$sql="select min(a.id) as id,min(a.process_end_date) as production_date,min(end_hours) as end_hours,min(end_minutes) as end_minutes, listagg(cast(a.batch_no AS VARCHAR2(4000)),',') within group (order by a.batch_no) as batch_no from yd_production_dtls a,pro_yd_subprocess_dtls b where a.id=b.mst_id and a.load_unload_id=2 and a.entry_form=414 and a.status_active=1 and a.system_no=$system_no order by id asc";
//	echo $sql;die;
	$check_data_array=sql_select($sql,1);
	$data_array=sql_select($sql);
	foreach($data_array as $row)
	{
	 $batch_noArr=array_unique(explode(",",$row[csf('batch_no')]));
	 foreach( $batch_noArr as $bid)
	 {
		$fnc_bach_arr[$bid]=$bid;
	 }
	$fnc_bach_hr_arr[$row[csf('end_hours')]]=$row[csf('end_hours')];
	$fnc_bach_min_arr[$row[csf('end_minutes')]]=$row[csf('end_minutes')];
	}
	$tot_batch_no=count($fnc_bach_arr);
	$end_hours=$check_data_array[0][csf('end_hours')];
	$end_minutes=$check_data_array[0][csf('end_minutes')];
	//echo count($fnc_bach_arr).'='.$check_data_array[0][csf('production_date')].'='.$new_process_end_date;
	
	if(count($fnc_bach_arr)!=1)
	{
		$production_date=date("Y-m-d",strtotime($check_data_array[0][csf('production_date')])).'#'.$tot_batch_no.'#'.$end_hours.'#'.$end_minutes;
		...
		{
			echo date("Y-m-d",strtotime($check_data_array[0][csf('production_date')])).'#'.$tot_batch_no.'#'.$end_hours.'#'.$end_minutes;
		}
	}
...
/*if($action=="machine_load_status")
{
	...
	$sql="select batch_no  from yd_production_dtls where load_unload_id=1 and entry_form=414 and process_end_date ='$process_date' and  floor_id=$data[1] and machine_id=$data[2] $update_id_cond";
...
if ($action == "save_update_delete")
 {
	...
	$load_process_start_date = str_replace("'", "", $txt_process_start_date);
	$load_process_start_date_chk = strtotime($load_process_start_date);
	
	$unload_process_end_date = str_replace("'", "", $txt_process_end_date);
	...
	$sql_batch=sql_select("select batch_no,batch_date from yd_batch_mst where id =$batch_id and is_deleted=0 and status_active=1");
	foreach($sql_batch as $row)
	{
		$batch_no_saved=$row[csf('batch_no')];
		$batch_date=strtotime($row[csf('batch_date')]);
	}
	
	if (str_replace("'", '', $cbo_load_unload) == 1) //Load
	{
	...
	if (str_replace("'", '', $cbo_load_unload) == 2) //unLoad
	{
...
*/
	/*$control_based_on_chemical_issue = return_field_value("chemical_issue", "variable_settings_production", "company_name =$cbo_company_id and variable_list=63 and is_deleted=0 and status_active=1");
	...
	if($control_based_on_chemical_issue==1) // Yes
	{
		...
		{
			echo "23**Batch Not Found In Dyes and Chemical Issue";
			disconnect($con);
			 die;
		}
	}*/
	// echo "23**===$cbo_company_id".$control_based_on_chemical_issue;die;
 	//echo "10**=A";die;
	...
	if ($operation == 0)  // Insert Here
	{
		...
		$cbo_machine_id = str_replace("'", "", $cbo_machine_name);
		...
		//$id = return_next_id("id", "yd_production_dtls", 1);   
		$id = return_next_id("id", "yd_production_dtls", 1); 
		...
		/*if (str_replace("'", '', $cbo_load_unload) == 2) 
		{
			//if (str_replace("'", '', $txt_process_id) == 31) {
				...
				$sql_data = "select id, batch_id from yd_production_dtls where  company_id=" . $cbo_company_id . " and  batch_id=" . $txt_batch_ID . " and load_unload_id=1 and entry_form=414 and is_deleted=0 and status_active=1";
			...
			//}
					...
					$sql_loadunload="select id,load_unload_id, batch_id from yd_production_dtls where  company_id=".$cbo_company_id." and  batch_id=".$txt_batch_ID." and entry_form=414 and is_deleted=0 and status_active=1   order by id desc";
			...
			if($multi_dyeing==2) // is Multi no
			{
				...
				{
					$sql_unload="select id, batch_id from yd_production_dtls where  company_id=".$cbo_company_id." and  batch_id=".$txt_batch_ID." and load_unload_id=2 and entry_form=414 and is_deleted=0 and status_active=1";
		...
		if($multi_dyeing==2 && $cbo_dyeing_type==1) // is Multi no //CBP Dyeing
			{
				$sql_load="select id, batch_id from yd_production_dtls where  company_id=".$cbo_company_id." and  batch_id=".$txt_batch_ID." and load_unload_id=1 and entry_form=414 and is_deleted=0 and status_active=1";
				...
				if(count($load_data_array)>0)
				{
					...
					$sql_loadunload="select id,load_unload_id, batch_id,machine_id from yd_production_dtls where  company_id=".$cbo_company_id." and  batch_id=".$txt_batch_ID."  and entry_form=414 and is_deleted=0 and status_active=1   order by id desc";//and load_unload_id=1 
					$loadunload_data_array=sql_select($sql_loadunload);
		...
		*/
		...
		//$page_upto_id = return_field_value("page_upto_id", "variable_settings_production", "company_name =$cbo_company_id and item_category_id=50 and variable_list=3 and is_deleted=0 and status_active=1");
		 $entry_form = 414;
		 $production_id = return_next_id("id", "yd_production_mst", 1); 
		...
		// echo "select prod_no_prefix,prod_no_prefix_num from yd_production_mst where entry_form=$entry_form and company_id=$cbo_company_id order by id desc";
		  //die;
        ...
        if($updateId == '') 
		{
            ...
            $field_array_mst = 'id, entry_form, company_id,yd_prod_no, prod_no_prefix, prod_no_prefix_num, booking_without_order, booking_type, batch_no, inserted_by, insert_date';
            $data_array_mst = "(".$production_id.",".$entry_form.",".$cbo_company_id.",'".$new_prod_id[0]."', '".$new_prod_id[1]."', ".$new_prod_id[2].", ".$hdnBookingWithoutOrder.", ".$hdnBookingType.", ".$txt_batch_no.", ".$_SESSION['logic_erp']['user_id'].", '".$pc_date_time."')";
	     ...
	     //$field_array_dtls = 'id, entry_form, mst_id, batch_no, job_id, job_dtls_id, order_id, quantity, load_unload_id, company_id, service_source_id, service_company_id, process_id, start_date, start_hours, start_minutes, floor_id, machine_id, machine_group_id, remarks, loading_date, loading_hours, loading_minutes, party_id, location_id, sales_order_id, sales_order_no, product_id,batch_id, inserted_by, insert_date';

			$field_array = "id,company_id,system_no,service_source,service_company,received_chalan,issue_chalan,issue_challan_mst_id,batch_no,batch_id,batch_ext_no,process_id,ltb_btb_id,water_flow_meter,process_end_date,end_hours,end_minutes,machine_id,floor_id,load_unload_id,entry_form,multi_dyeing_id,remarks,dyeing_type_id,hour_load_meter,multi_batch_load_id,mst_id,inserted_by,insert_date";
			$data_array = "(" . $id . "," . $cbo_company_id . "," . $system_no . "," . $cbo_service_source . "," . $cbo_service_company . "," . $txt_recevied_chalan . "," . $txt_issue_chalan . "," . $txt_issue_mst_id . "," . $txt_batch_no . "," . $txt_batch_ID . "," . $txt_ext_id . "," . $txt_process_id . "," . $cbo_ltb_btb . "," . $txt_water_flow . "," . $txt_process_start_date . "," . $txt_start_hours . "," . $txt_start_minutes . "," . $cbo_machine_name . "," . $cbo_floor . "," . $cbo_load_unload . ",414," . $hidden_double_dyeing . "," . $txt_remarks . "," . $cbo_dyeing_type . "," . $txt_load_meter . "," . $cbo_yesno . "," . $production_id . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";

			//cbo_dyeing_type
			$id_dtls = return_next_id("id", "pro_yd_subprocess_dtls", 1);
			
			
			 
				$field_array_dtls = "id, mst_id,entry_page,prod_id,yd_job_dtls_id,const_composition,gsm,dia_width,width_dia_type,batch_qty,no_of_roll,load_unload_id,production_qty,remarks,yarn_type, bobin_type, winding_packageqty,yarn_count_id, order_id,job_id, sales_order_id,lot,inserted_by,insert_date";
				for ($i = 1; $i <= $total_row; $i++) 
				{
					...
					$txtbobintype="txtbobintype_".$i;
					$txtWindingPackageQty="txtWindingPackageQty_".$i;
					$txtproductionqty = "txtprodqnty_" . $i;
 					$txtyarncount="txtyarncount_".$i;   
					//$txtbrand="txtbrand_".$i;
					$txtyarntypeid = "txtyarntypeid_" . $i;
					$txtyarncountid = "txtyarncountid_" . $i;
					$txtjobid = "txtjobid_" . $i;
					$txtorderid = "txtorderid_" . $i;
					$txtsalesorderid = "txtsalesorderid_" . $i;
					$txtlot = "txtlot_" . $i; 
					$hiddendiatypeid_ = "hiddendiatypeid_" . $i;
					$Itemprod_id = str_replace("'", "", $$prod_id);
					if ($data_array_dtls != "") $data_array_dtls .= ",";
					$data_array_dtls .= "(" . $id_dtls . "," . $mst_update_id . ",414,'" .$Itemprod_id ."'," . $$txtjobdtlsid . "," . $$txtconscomp . "," . $$txtgsm . "," . $$txtdiawidth . "," . $$hiddendiatypeid_ . "," . $$txtbatchqnty . "," . $$txtroll . ",1," . $$txtproductionqty . "," . $$txtremark . "," . $$txtyarntypeid . "," . $$txtbobintype . "," . $$txtWindingPackageQty . "," . $$txtyarncountid . "," . $$txtorderid . "," . $$txtjobid . "," . $$txtsalesorderid . "," . $$txtlot . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
					$id_dtls = $id_dtls + 1;
		...
		if ($cbo_load_unload == "'2'") 
		{
			...
			$field_array = "id,company_id,system_no,service_source,service_company,received_chalan,issue_chalan,issue_challan_mst_id,batch_no,batch_id,batch_ext_no,process_id,ltb_btb_id,water_flow_meter,process_end_date,end_hours,end_minutes,machine_id,floor_id,load_unload_id,result,entry_form,multi_dyeing_id,remarks,dyeing_type_id,hour_unload_meter,shift_name,fabric_type,production_date,booking_no,responsibility_id,mst_id,inserted_by,insert_date $field_arr";
			$data_array = "(" . $id . "," . $cbo_company_id . "," . $system_no . "," . $cbo_service_source . "," . $cbo_service_company . "," . $txt_recevied_chalan . "," . $txt_issue_chalan . "," . $txt_issue_mst_id . "," . $txt_batch_no . "," . $txt_batch_ID . "," . $txt_ext_id . "," . $txt_process_id . "," . $cbo_ltb_btb . "," . $txt_water_flow . "," . $txt_process_end_date . "," . $txt_end_hours . "," . $txt_end_minutes . "," . $cbo_machine_name . "," . $cbo_floor . "," . $cbo_load_unload . "," . $cbo_result_name . ",414," . $hidden_double_dyeing . "," . $txt_remarks . "," . $cbo_dyeing_type . "," . $txt_unload_meter . "," . $cbo_shift_name . "," . $cbo_fabric_type . "," . $txt_process_date . "," . $txt_booking_no . "," . $cbo_responsibility . "," . $production_id . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "' ".$field_data_arr.")";
			$mst_update_id = str_replace("'", "", $id);//


			$id_dtls = return_next_id("id", "pro_yd_subprocess_dtls", 1);
			 
				$field_array_dtls = "id, mst_id,entry_page,prod_id,yd_job_dtls_id,const_composition,gsm,dia_width,width_dia_type,batch_qty,no_of_roll, load_unload_id, production_qty, rate, amount, currency_id,exchange_rate,remarks,yarn_type, bobin_type, winding_packageqty,yarn_count_id,order_id,job_id, sales_order_id,lot,inserted_by,insert_date";
				for ($i = 1; $i <= $total_row; $i++) {
					$prod_id = "txtprodid_" . $i;
					$txtjobdtlsid = "txtjobdtlsid_" . $i;
					$txtconscomp = "txtconscomp_" . $i;
					...
					//echo "10**".$txtproductionqty; die;
					
					
					$Itemprod_id = str_replace("'", "", $$prod_id);
					if ($data_array_dtls != "") $data_array_dtls .= ",";
					$data_array_dtls .= "(" . $id_dtls . "," . $mst_update_id . ",414,'" . $Itemprod_id . "'," . $$txtjobdtlsid . "," . $$txtconscomp . "," . $$txtgsm . "," . $$txtdiawidth . "," . $$hiddendiatypeid . "," . $$txtbatchqnty . "," . $$txtroll . ",2," . $$txtproductionqty . "," . $$txtrate . "," . $$txtamount . "," . $hidden_currency . "," . $hidden_exchange_rate . "," . $$txtremark . "," . $$txtyarntypeid . "," . $$txtbobintype . "," . $$txtWindingPackageQty . "," . $$txtyarncountid . "," . $$txtorderid . "," . $$txtjobid . "," . $$txtsalesorderid . "," . $$txtlot . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
					$id_dtls = $id_dtls + 1;
					//print_r($data_array_dtls);die;
 			}
		...
		$rID = $rID2 = $rID3=true;
			...
			$rID3= sql_insert("yd_production_mst", $field_array_mst, $data_array_mst, 0);
			if ($flag == 1) 
			{
				if ($rID3) $flag = 1; else $flag = 0;
			 }
		
		$rID = sql_insert("yd_production_dtls", $field_array, $data_array, 0);
		if ($rID) $flag = 1; else $flag = 0;
		//echo "10**insert into pro_yd_subprocess_dtls($field_array_dtls)values".$data_array_dtls;die;
		//echo "insert into pro_fab_dyeing_focus (id,batch_no,production_date) values (1,'abc','2020-Jan-01')";
			$rID2 = sql_insert("pro_yd_subprocess_dtls", $field_array_dtls, $data_array_dtls, 0);
			if ($flag == 1) 
			{
	 ...
	 //	echo "10**".$rID3."**".$rID."**".$rID2."**".$flag.'**'.$load_unload_id."**".$msg;die;
		if ($db_type == 0) {
			if ($flag == 1) {
				mysql_query("COMMIT");
	...
	else if ($operation == 1)   // Update Here
	{
		...
		$cbo_machine_id=str_replace("'","",$cbo_machine_name);
		//if($updateId) $up_id_cond="and a.id!=$updateId";else $up_id_cond="";
		$batch_prod=sql_select("select a.batch_id,max(a.id) as mst_id from yd_production_dtls a where  a.batch_id=$txt_batch_ID and a.entry_form=414 and a.company_id=$cbo_company_id and a.status_active=1  group by a.batch_id ");
		$last_mst_id=0;
		foreach($batch_prod as $row)
		{
			$last_mst_id= $row[csf('mst_id')];
		}
		$batch_last_process=sql_select("select a.batch_id,a.load_unload_id,a.result,a.end_minutes,a.process_end_date,a.production_date from yd_production_dtls a where  a.batch_id=$txt_batch_ID and a.company_id=$cbo_company_id and a.id in($last_mst_id) and a.status_active=1  ");
		//echo "10**select a.batch_id,a.end_hours,a.end_minutes,a.process_end_date,a.production_date from yd_production_dtls a where  a.batch_id=$txt_batch_ID and a.company_id=$cbo_company_id and a.id in($last_mst_id) and a.status_active=1  ";die;
		foreach($batch_last_process as $row)
		{
			$last_load_unload_id= $row[csf('load_unload_id')];
			$previ_end_hours= $row[csf('end_hours')];
			$previ_end_minutes= $row[csf('end_minutes')];
		}
		if($double_dyeing_id==1 && $load_unloadId==2 && $last_load_unload_id==1)
		{
			echo "14**Already Load,Unlod First.";
			disconnect($con);
			die;
		}
		...
		/*if(count($loadunload_data_array)>0  && $machine_id>0)
		{
			if($cbo_machine_id!=$machine_id)
			{
				$msg='Please load correct machine which already unloaded';
				echo "101**".$msg;
				disconnect($con);
				die;
			}
		...
		/*if($double_dyeing_id==1 && $load_unloadId==1 && $last_load_unload_id==2)
		{
			echo "14**Already UnLoad";
			disconnect($con);
			die;
		}*/
	//	echo "10**=".$double_dyeing_id.'='.$load_unloadId.'='.$last_load_unload_id;die;
		...
		if ($cbo_load_unload == "'1'") {
			...
			$data_array_update = "" . $cbo_service_source . "*" . $cbo_service_company . "*" . $txt_recevied_chalan . "*" . $txt_issue_chalan . "*" . $txt_issue_mst_id . "*" . $txt_batch_no . "*" . $txt_batch_ID . "*" . $txt_ext_id . "*" . $txt_process_id . "*" . $cbo_ltb_btb . "*" . $txt_water_flow . "*" . $txt_process_start_date . "*" . $txt_start_hours . "*" . $txt_start_minutes . "*" . $cbo_machine_name . "*" . $cbo_floor . "*" . $cbo_load_unload . "*414*" . $txt_remarks . "*" . $cbo_dyeing_type . "*" . $txt_load_meter . "*" . $cbo_yesno . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'";//

			$id_dtls = return_next_id("id", "pro_yd_subprocess_dtls", 1);
			$field_array_dtls_insert = "id, mst_id,entry_page,prod_id,yd_job_dtls_id,const_composition,gsm,dia_width,width_dia_type,batch_qty,roll_no,barcode_no,load_unload_id,roll_id,production_qty,remarks,yarn_type, bobin_type, winding_packageqty,yarn_count_id,order_id,job_id, sales_order_id,lot,inserted_by,insert_date";
				...
				$field_array_up = "mst_id*yd_job_dtls_id*gsm*batch_qty*production_qty*remarks*no_of_roll*roll_id*yarn_type*bobin_type*winding_packageqty*yarn_count_id*order_id*job_id* sales_order_id*lot*updated_by*update_date";
				for ($i = 1; $i <= $total_row; $i++) {
					...
					if (str_replace("'", '', $$updateiddtls) != "")
					 {
						...
						$data_array_up[str_replace("'", '', $$updateiddtls)] = explode("*", ("" . $update_id . "*" . $$txtjobdtlsid . "*" . $$txtgsm . "*" . $$txtbatchqnty . "*" . $$txtprodqnty . "*" . $$txtremark . "*" . $$txtroll . "*" . $$rollid . "*" . $$txtyarntypeid . "*" . $$txtbobintype . "*" . $$txtWindingPackageQty . "*" . $$txtyarncountid . "*" . $$txtorderid . "*" . $$txtjobid . "*" . $$txtsalesorderid . "*" . $$txtlot . "*'" . $_SESSION['logic_erp']['user_id'] . "'*'" . $pc_date_time . "'"));
					}
					 else 
					{
						if ($data_array_dtls_insert != "") $data_array_dtls_insert .= ",";
						$data_array_dtls_insert .= "(" . $id_dtls . "," . $update_id . ",414,'" . $Itemprod_id . "'," . $$txtjobdtlsid . "," . $$txtconscomp . "," . $$txtgsm . "," . $$txtdiawidth . "," . $$hiddendiatypeid . "," . $$txtbatchqnty . "," . $$txtroll . ",1," . $$rollid . "," . $$txtprodqnty . "," . $$txtremark . "," . $$txtyarntype . "," . $$txtbobintype . "," . $$txtWindingPackageQty . "," . $$txtyarncountid . "," . $$txtorderid . "," . $$txtjobid . "," . $$txtsalesorderid . "," . $$txtlot . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
						$id_dtls = $id_dtls + 1;
 				}
			}
			//echo "10**insert into pro_yd_subprocess_dtls($field_array_dtls_insert)values".$data_array_dtls_insert;
			//echo bulk_update_sql_statement("pro_yd_subprocess_dtls", "id",$field_array_up,$data_array_up,$id_arr );die;
			//print_r($data_array_up);die;
			/*if(count($data_array_up)>0)
			{
			$rID2=execute_query(bulk_update_sql_statement("pro_yd_subprocess_dtls", "id",$field_array_up,$data_array_up,$id_arr ));
			if($rID2) $flag=1; else $flag=20;
			//echo $flag;die;
			}
			if(count($data_array_delete)>0)
			{
			$rID3=execute_query(bulk_update_sql_statement("pro_yd_subprocess_dtls", "id",$field_array_delete,$data_array_delete,$id_arr_delete ));
		...
		if ($cbo_load_unload == "'2'") {
			...
			$field_array_update = "service_source*service_company*received_chalan*issue_chalan*issue_challan_mst_id*batch_no*batch_id*batch_ext_no*process_id*ltb_btb_id*water_flow_meter*process_end_date*end_hours*end_minutes*machine_id*floor_id*load_unload_id*result*entry_form*remarks*dyeing_type_id*hour_unload_meter*shift_name*fabric_type*production_date*booking_no*responsibility_id*updated_by*update_date $field_arr";
	$data_array_update = "" . $cbo_service_source . "*" . $cbo_service_company . "*" . $txt_recevied_chalan . "*" . $txt_issue_chalan . "*" . $txt_issue_mst_id . "*" . $txt_batch_no . "*" . $txt_batch_ID . "*" . $txt_ext_id . "*" . $txt_process_id . "*" . $cbo_ltb_btb . "*" . $txt_water_flow . "*" . $txt_process_end_date . "*" . $txt_end_hours . "*" . $txt_end_minutes . "*" . $cbo_machine_name . "*" . $cbo_floor . "*" . $cbo_load_unload . "*" . $cbo_result_name . "*414*" . $txt_remarks . "*" . $cbo_dyeing_type . "*" . $txt_unload_meter . "*" . $cbo_shift_name . "*" . $cbo_fabric_type . "*" . $txt_process_date . "*" . $txt_booking_no . "*" . $cbo_responsibility . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "' ".$field_data_arr."";


			$id_dtls = return_next_id("id", "pro_yd_subprocess_dtls", 1);
			$field_array_dtls_insert = "id, mst_id,entry_page,prod_id,const_composition,gsm,dia_width,width_dia_type,batch_qty,roll_no,barcode_no,load_unload_id,roll_id,rate,amount,production_qty,remarks,yarn_type,bobin_type,winding_packageqty,inserted_by,insert_date";
			 
				//$field_array_dtls="id, mst_id, entry_page, prod_id, const_composition, gsm, dia_width, width_dia_type, batch_qty, inserted_by, insert_date";
				$field_array_up = "mst_id*gsm*batch_qty*production_qty*no_of_roll*roll_id*rate*amount*currency_id*exchange_rate*remarks*yarn_type*bobin_type*winding_packageqty*updated_by*update_date";
				for ($i = 1; $i <= $total_row; $i++) {
					$checkRowTd = "checkRow_" . $i;
					$prod_id = "txtprodid_" . $i;
					$txtconscomp = "txtconscomp_" . $i;
					...
					if (str_replace("'", "", $$updateiddtls) != "") {
						$id_arr[] = str_replace("'", '', $$updateiddtls);
						$data_array_up[str_replace("'", '', $$updateiddtls)] = explode("*", ("" . $update_id . "*" . $$txtgsm . "*" . $$txtbatchqnty . "*" . $$txtprodqnty . "*" . $$txtroll . "*" . $$rollid . "*" . $$txtrate . "*" . $$txtamount . "*" . $hidden_currency . "*" . $hidden_exchange_rate . "*" . $$txtremark . "*" . $$txtyarntype . "*" . $$txtbobintype . "*" . $$txtWindingPackageQty . "*'" . $_SESSION['logic_erp']['user_id'] . "'*'" . $pc_date_time . "'"));
					}

					if (str_replace("'", '', $$updateiddtls) == "") {
						if ($data_array_dtls_insert != "") $data_array_dtls_insert .= ",";
						$data_array_dtls_insert .= "(" . $id_dtls . "," . $update_id . ",414,'" . $Itemprod_id . "'," . $$txtconscomp . "," . $$txtgsm . "," . $$txtdiawidth . "," . $$hiddendiatypeid . "," . $$txtbatchqnty . "," . $$txtroll . ",2," . $$rollid . "," . $$txtrate . "," . $$txtamount . "," . $$txtprodqnty . "," . $$txtremark . "," . $$txtyarntype . "," . $$txtbobintype . "," . $$txtWindingPackageQty . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";
						$id_dtls = $id_dtls + 1;
		...
		//echo "10**insert into yd_production_dtls($field_array_update)values".$data_array_update;
		$flag = 0;
		$rID = sql_update("yd_production_dtls", $field_array_update, $data_array_update, "id", $update_id, 1);
		if ($rID) $flag = 1; else $flag = 0;

		if (count($data_array_up) > 0) 
		{
			$rID2 = execute_query(bulk_update_sql_statement("pro_yd_subprocess_dtls", "id", $field_array_up, $data_array_up, $id_arr));
			//echo "10**".bulk_update_sql_statement("pro_yd_subprocess_dtls", "id", $field_array_up, $data_array_up, $id_arr);die;
			if ($rID2) $flag = 1; else $flag = 0;
		}
		if (count($data_array_delete) > 0) 
		{
		...
		if ($data_array_dtls_insert != "") 
		{
			...
			if ($flag == 1) {
				...
				$rID4 = sql_insert("pro_yd_subprocess_dtls", $field_array_dtls_insert, $data_array_dtls_insert, 0);
				if ($rID4) $flag = 1; else $flag = 0;
			}
		}
		//echo $rID.'='.$rID2.'='.$rID3.'='.$rID4;die;
		//$rID=sql_update("yd_production_dtls",$field_array_update,$data_array_update,"id",$update_id,1);
		//echo "insert into yd_production_dtls values $field_array_update,$field_array_update,'id',$update_id,0)";die;
		//check_table_status( $_SESSION['menu_id'],0);
		if ($db_type == 0) {
			if ($flag == 1) {
				mysql_query("COMMIT");
...
if ($action == "check_batch_no") {  
	...
	if($_SESSION['logic_erp']["company_id"]) 
	{
		$user_working_comp_cond = " and working_company_id in (".$_SESSION['logic_erp']["company_id"].")";
	}
	$sql = "select id, entry_form,batch_no,company_id from pro_batch_create_mst where batch_no='" . trim($data[1]) . "' and entry_form in(0,136) and is_deleted=0 and status_active=1 $user_working_comp_cond order by id desc";
	$data_array = sql_select($sql, 1);
	if (count($data_array) > 0) {
...
if ($action == "check_batch_no_scan") {
	$data = explode("**", $data);
	$batch_id = (int)$data[1];
	$sql = "select id, batch_no,company_id from pro_batch_create_mst where id='" . $batch_id . "' and entry_form in(0,136) and is_deleted=0 and status_active=1 order by id desc";
	$data_array = sql_select($sql, 1);
...
if ($action == "check_batch_no_load") {
	...
	$process = $data[3];
	//if ($process == 31) {
		$sql = "select id, batch_id from yd_production_dtls where  company_id='" . trim($data[0]) . "' and  batch_id='" . trim($data[2]) . "' and load_unload_id=1 and entry_form=414 and is_deleted=0 and status_active=1";
		$data_array = sql_select($sql);
		if (count($data_array) > 0) {
...
if ($action == "check_batch_no_for_machine") {
	...
	$sql = "select  a.batch_no as batch_no,a.batch_id from yd_production_dtls a,pro_yd_subprocess_dtls b  where  a.id=b.mst_id and a.service_company='" . trim($data[0]) . "'  and a.machine_id='$data[3]' and a.service_source in(1) and a.load_unload_id=1 and a.entry_form=414 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1   group by  a.batch_no,a.batch_id";
	...
	$sql_batch = sql_select("select a.batch_id,a.batch_no from yd_production_dtls a,pro_yd_subprocess_dtls b    where  a.id=b.mst_id and a.service_company='" . trim($data[0]) . "'  and a.machine_id='$data[3]'  and a.service_source in(1) and a.load_unload_id=2 and a.entry_form=414 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1   $bat_cond_for_in  group by  a.batch_no,a.batch_id");// and a.batch_id in(".implode(",",$loaded_batch_idarr).")
	}
...
if ($action == "check_for_shade_matched") {
	...
	$sql_unload = "select  batch_no from yd_production_dtls where  company_id='" . trim($data[0]) . "' and  batch_id=" . trim($data[2]) . " and load_unload_id=2 and entry_form=414 and is_deleted=0 and result=1 and status_active=1";
...
if ($action == "check_date_time")//Un used it
{
	...
	$sql = "select id, batch_no,company_id from pro_batch_create_mst where batch_no='" . trim($data[1]) . "' and entry_form in(0,136) and is_deleted=0 and status_active=1 order by id desc";
	$data_array = sql_select($sql, 1);
	if (count($data_array) > 0) {
		echo "1" . "_" . $data_array[0][csf('id')] . "_" . $data_array[0][csf('company_id')];
	} else {
...
if($action == "process_floor"){
	$explode_data = explode("**", $data);
	$company_id = $explode_data[0];
	$process = $explode_data[1];
	$sql = "select id, floor_name from lib_prod_floor where company_id=$company_id and production_process=$process and status_active=1 and is_deleted=0";
	echo create_drop_down("cbo_floor", 135, $sql, "id,floor_name", 1, "-- Select Floor --", "", "load_drop_down( 'requires/yd_production_v2_controller',$('#cbo_company_id').val()+'**'+this.value,'load_drop_down_machines','machine_td' )");	
}

if($action == "load_drop_down_machines"){
	$explode_data = explode("**", $data);
	$company_id = $explode_data[0];
	$floor_id = $explode_data[1];
	$sql = "select b.id, b.machine_no from lib_prod_floor a, lib_machine_name b where a.id=b.floor_id and  a.company_id=$company_id and a.id=$floor_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
	//echo $sql; exit();
	echo create_drop_down("cbo_machine_name", 135, $sql, "id,machine_no", 1, "-- Select Machine --");	
}
            ...
            <fieldset>
                <table cellpadding="0" cellspacing="2" width="100%" id="main_tbl">
                    ...
                    <tr>
                        ...
                        <td>
							<?
							echo create_drop_down("cbo_company_id", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 and comp.core_business not in(3) $company_cond order by comp.company_name", "id,company_name", 1, "--Select Company--", $selected, "load_drop_down('requires/yd_production_v2_controller', this.value, 'load_drop_floor', 'floor_td' );", "", "", "", "", "");
							?>
                    ...
                    <tr>
                        ...
                        <td>
							<?
							//fnc_company_val();
							echo create_drop_down("cbo_service_source", 135, $knitting_source, "", 1, "-- Select --", $selected, "load_drop_down( 'requires/yd_production_v2_controller',this.value+'**'+$('#cbo_company_id').val(),'load_drop_down_knitting_com','dyeing_company_td' );search_populate(this.value);", "", "1,3");
							?>
                        </td>
                    ...
                    <tr>
                        ...
                        <td>
							<?
							echo create_drop_down("txt_process_id", 135, $production_process, "", 1, "--Select---", 31, "load_drop_down( 'requires/yd_production_v2_controller', document.getElementById('cbo_company_id').value+'**'+this.value, 'process_floor', 'floor_td' );", "", "", "", "", "1,2,3,4,101,120,121,122,123,124");
							?>
                    ...
                    <tr>
                        ...
                        <td id="floor_td">
							<?
							echo create_drop_down("cbo_floor", 135, $blank_array, "id,floor_name", 1, "-- Select Floor --", $selected, "", 0, "", "", "", "", 4);
							?>
                        </td>
                    <tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>
							<?
							echo create_drop_down("cbo_yesno", 80, $yes_no, "", 0, "-- Select--", 2, "fnc_load_data(this.value);", 0, "", "", "", "");
							?>
                        </td>
                        </tr>
        ...
        <fieldset onLoad="process_check(document.getElementById('txt_process_id').value);">
            <table cellpadding="0" cellspacing="2" width="100%" id="main_tbl">
                ...
                <tr>
                    ...
                    <td>
						<?
						echo create_drop_down("cbo_company_id", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "--Select Company--", $selected, "load_drop_down('requires/yd_production_v2_controller', this.value, 'load_drop_floor', 'floor_td' );");
						?>
                        <input type="hidden" name="txt_update_id" id="txt_update_id" style="width:100px;"
                               class="text_boxes" readonly/>
                        <input type="hidden" name="roll_maintained" id="roll_maintained" style="width:100px;"
                ...
                <tr>
                    ...
                    <td>
						<?
						echo create_drop_down("cbo_result_name", 135, $dyeing_result, "", 1, "-- Select Result --", 0, "", 0, "1,2,3,4,5,6,20", "", "", "", "");
						?>
                    </td>
                </tr>
                ...
                <tr style="display:none">
                  ...
                  <tr style="display:none">
                    <td>Responsibility Dept</td>
                    <td>
						<?
						echo create_drop_down("cbo_responsibility", 135, $responsibility_dept_arr, "", 1, "-- Select --", 0, "", 1, "", "", "", "", "");
						?>
                    </td>
                 </tr>
...
if ($action == "load_drop_down_knitting_com") {
	...
	if ($data[0] == 1) {
		...
		echo create_drop_down("cbo_service_company", 135, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name", "id,company_name", 1, "-- Select --", "", "load_drop_down('requires/yd_production_v2_controller', this.value, 'load_drop_floor', 'floor_td' );fnc_mc_load()", "");//load_batch_working_company(this.value);
	} else if ($data[0] == 3) {
...
if ($action == "populate_data_from_data") {
	...
	foreach ($res as $row) {
		...
		echo "load_drop_down( 'requires/yd_production_v2_controller', " . $row[csf("dyeing_source")] . "+'**'+" . $row[csf("company_id")] . ", 'load_drop_down_knitting_com', 'dyeing_company_td' );\n";
            ...
            <fieldset style="width:860px; margin-left:2px">
                <table cellpadding="0" cellspacing="0" width="850" border="1" rules="all" class="rpt_table">
                    ...
                    <tr class="general">
                        ...
                        <td align="center">
							<?
							echo create_drop_down("cbo_service_source", 135, $knitting_source, "", 1, "-- Select --", $selected, "load_drop_down( 'yd_production_v2_controller',this.value+'**'+$cbo_company_id,'load_drop_down_knitting_com','dyeing_company_td' );", "", "1,3");
							?>
                        </td>
                        <td id="dyeing_company_td">
							<?
                        ...
                        <td align="center">
                            <input type="button" name="button2" class="formbutton" value="Show"
                                   onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+<? echo $cbo_company_id; ?>, 'create_challan_search_list_view', 'search_div', 'yd_production_v2_controller', 'setFilterGrid(\'tbl_list_search\',-1);')"
                                   style="width:100px;"/>
                        </td>
                    </tr>
                    <tr>
...
*/
            ...
            <tr bgcolor="<? echo $bgcolor; ?>"
                onClick='show_list_view("<? echo $batch_id . '_' . $load_unload . '_' . $po_id_arr . '_' . $row[csf("id")]; ?>","child_form_input_data","list_fabric_desc_container","requires/yd_production_v2_controller");get_php_form_data("<? echo $row[csf("id")]; ?>","mst_id_child_form_input_data","requires/yd_production_v2_controller")'
...
if ($action == "child_form_input_data") {
	...
	//echo "select company_id from yd_production_dtls where batch_id='$batch_id' and entry_form=414 and is_deleted=0 and status_active=1";
	$company_id = return_field_value("company_id", "yd_production_dtls", "batch_id='$batch_id' and entry_form=414 and is_deleted=0 and status_active=1");
	$count_arr = return_library_array("select id, yarn_count from lib_yarn_count", 'id', 'yarn_count');
	...
	$sql_result = sql_select("select a.id,b.id as dtls_id,b.prod_id,b.const_composition,b.remarks,b.gsm,b.dia_width, b.width_dia_type, b.batch_qty, b.production_qty,b.barcode_no, b.roll_no, b.no_of_roll,b.roll_id,b.rate,b.amount,b.yd_job_dtls_id,b.yarn_count_id, b.order_id,b.yarn_type, 
   b.job_id, b.sales_order_id, b.lot from yd_production_dtls a,pro_yd_subprocess_dtls b where a.id=b.mst_id and a.id=$mst_id and a.entry_form=414 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $load_unload_cond");
 	//echo $company_id.'='.$roll_maintained;
	//$subprocess_dtls=return_field_value("id ", "pro_yd_subprocess_dtls", "id=$hidden_buyer_id  and status_active=1 and is_deleted=0","job_no");
	$i = 1;
	foreach ($sql_result as $row) {
        ...
        <tr class="general" id="row_<? echo $i; ?>">
            ...
            <td><input type="text" name="txtlot_<? echo $i; ?>" id="txtlot_<? echo $i; ?>" class="text_boxes_numeric" style="width:40px;" value="<? echo $row[csf('lot')]; ?>" disabled readonly/></td>  
            <td><input type="text" name="txtyarncount_<? echo $i; ?>" id="txtyarncount_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo $count_arr[$row[csf('yarn_count_id')]]; ?>" disabled readonly/>
             <input type="hidden" name="txtyarncountid_<? echo $i; ?>" id="txtyarncountid_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo  $row[csf('yarn_count_id')]; ?>"  readonly disabled/> 
             <input type="hidden" name="txtjobid_<? echo $i; ?>" id="txtjobid_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo  $row[csf('job_id')]; ?>"  readonly disabled/>
                        <input type="hidden" name="txtorderid_<? echo $i; ?>" id="txtorderid_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo  $row[csf('order_id')]; ?>"  readonly disabled/>
                          <input type="hidden" name="txtsalesorderid_<? echo $i; ?>" id="txtsalesorderid_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo  $row[csf('sales_order_id')]; ?>"  readonly disabled/>
            
            <input type="hidden" name="txtbrand_<? echo $i; ?>" id="txtbrand_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo $brand_value; ?>" disabled   readonly/>
			<input type="hidden" name="txtremark_<? echo $i; ?>" id="txtremark_<? echo $i; ?>" class="text_boxes_numeric" style="width:60px;" value="<? echo $row[csf('remarks')]; ?>" disabled   readonly/> <input type="hidden" name="txtjobdtlsid_<? echo $i; ?>" id="txtjobdtlsid_<? echo $i; ?>" value="<? echo $row[csf('yd_job_dtls_id')]; ?>"/>
           
            
            </td>
            
            <td>
                        <input type="text" name="txtyarntype_<? echo $i; ?>" id="txtyarntype_<? echo $i; ?>"  class="text_boxes_numeric" style="width:60px;" value="<? echo $yarn_type[$row[csf('yarn_type')]]; ?>" readonly  disabled/> 
                         <input type="hidden" name="txtyarntypeid_<? echo $i; ?>" id="txtyarntypeid_<? echo $i; ?>"  class="text_boxes_numeric" style="width:60px;" value="<? echo $row[csf('yarn_type')]; ?>" readonly  disabled/>
            ...
            <td>
                        ...
                        <input type="hidden" name="txtgsm_<? echo $i; ?>" id="txtgsm_<? echo $i; ?>" class="text_boxes_numeric"  style="width:40px;" value="<? echo trim($gsm); ?>"/>
          </td>
          ...
          <td>
                 ...
                 <input type="hidden" name="txtdiawidth_<? echo $i; ?>" id="txtdiawidth_<? echo $i; ?>" class="text_boxes"  style="width:50px;" value="<? echo $dia_width; //$row[csf('width_dia_type')];
				?>" disabled/></td> 
           
            <td style="display:none"><input type="text" name="txtdiatype_<? echo $i; ?>" id="txtdiatype_<? echo $i; ?>" class="text_boxes" style="width:70px;" value="<? echo $fabric_typee[$width_dia_type]; ?>" disabled/>
                <input type="hidden" name="hiddendiatypeid_<? echo $i; ?>" id="hiddendiatypeid_<? echo $i; ?>"  value="<? echo $row[csf('width_dia_type')]; ?>" readonly/>
            </td>
            <td style="display:none"><input type="text" name="txtroll_<? echo $i; ?>" id="txtroll_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo $roll_no; ?>" style="width:40px;" <? echo $readonly; ?> />
            ...
            <td><input type="text" name="txtprodqnty_<? echo $i; ?>" id="txtprodqnty_<? echo $i; ?>" class="text_boxes_numeric" value="<? echo $production_qty; ?>"
                       onKeyUp="calculate_production_qnty();" style="width:60px;" readonly/></td>
            <td style="display:none"><input type="text" name="txtrate_<? echo $i; ?>" id="txtrate_<? echo $i; ?>" class="text_boxes_numeric"  style="width:60px;" value="<? echo $row[csf('rate')]; ?>" readonly/></td>
            <td style="display:none"><input type="text" name="txtamount_<? echo $i; ?>" id="txtamount_<? echo $i; ?>" class="text_boxes_numeric" style="width:70px;" value="<? echo $row[csf('amount')]; ?>" readonly/>
            </td> 
    ...
    <tr>
        <td colspan="7" align="right"><b>Sum:</b> <? //echo $b_qty; ?> </td>
        <td align="right"><b><input type="text" name="total_batch_qnty" id="total_batch_qnty" class="text_boxes_numeric"  style="width:60px" 
        value="<? if($tot_batch_qty>100) echo number_format($tot_batch_qty,0);else echo $tot_batch_qty;//below of fraction 50 ?>" readonly/> </b>
        </td>
        ...
        <td align="right"  style="display:none"><input type="text" name="total_amount" id="total_amount" class="text_boxes_numeric"
                                 style="width:70px" value="<? echo $tot_amount; ?>" readonly/></td>
...
if ($action == "mst_id_child_form_input_data") {
	$sql_result = sql_select("select a.id, a.company_id, a.service_source, a.service_company, a.length_shrinkage, a.width_shrinkage, a.spirality, a.received_chalan, a.issue_chalan, a.issue_challan_mst_id, a.process_end_date, a.production_date, a.process_start_date, a.process_id, a.end_hours, a.end_minutes, a.start_hours, a.start_minutes, a.temparature, a.stretch, a.over_feed, a.feed_in, a.pinning, a.speed_min, a.floor_id, a.machine_id, a.shift_name, a.remarks
				from yd_production_dtls a
				where a.id=$data and a.entry_form=414  and a.status_active=1 and a.is_deleted=0");
	...
	echo "set_button_status(1, '" . $_SESSION['page_permission'] . "', 'fnc_yd_production_dtls',1,1);\n";
//echo "set_button_status(1, permission, 'fnc_yd_production_dtls',1,1);\n";
	exit();
}
...
//echo $data."<br>";
	...
	if ($load_unload == 1) {
		...
		$sql = "select a.id,a.company_id,a.batch_id,a.batch_no,sum(b.batch_qty) as batch_qty,sum(b.production_qty) as production_qty, $dyeing_date_con  from yd_production_dtls a,pro_yd_subprocess_dtls b where a.id=b.mst_id and a.system_no=$system_no and a.entry_form=414 and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $load_unload_cond  group by a.id,a.batch_id,a.batch_no,a.company_id,$dyeing_date_con order by a.id";
		//echo $sql;
	} else {
    ...
    <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" width="800" rules="all">
		...
		<? foreach ($result as $row) {
            ...
            <tr bgcolor="<? echo $bgcolor; ?>"
                onClick='show_list_view("<? echo $batch_id . '_' . $load_unload . '_' . $po_id_arr . '_' . $row[csf("id")]. '_' . $entry_form_no; ?>","child_form_input_data","list_fabric_desc_container","requires/yd_production_v2_controller");get_php_form_data("<? echo $load_unload . '_' . $row[csf("batch_id")] . '_' . $row[csf("batch_no")] . '_' . $row[csf("company_id")]. '_' . $entry_form_no. '_' . $row[csf("id")]; ?>","populate_data_from_batch2","requires/yd_production_v2_controller")'
                style="cursor:pointer">
                <td width="30"><? echo $i; ?></td>
            ...
            <table cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all" width="900">
                ...
                <tr>
                    <td align="center"><input type="hidden" id="selected_booking">
						<?
						echo create_drop_down("cbo_company_mst", 150, "select id,company_name from lib_company comp where status_active=1 and id=" . $cbo_company_id . " order by company_name", "id,company_name", 1, "-- Select Company --", '', "load_drop_down( 'yd_production_v2_controller', this.value, 'load_drop_down_buyer', 'buyer_td' );");
						?>
                    </td>
                    ...
                    <td align="center">
                        <input type="button" name="button2" class="formbutton" value="Show"
                               onClick="show_list_view ( document.getElementById('cbo_company_mst').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+document.getElementById('txt_date_from').value+'_'+document.getElementById('txt_date_to').value+'_'+document.getElementById('cbo_search_category').value+'_'+document.getElementById('txt_booking_no').value+'_'+'<?php echo $supplier_id . "_" . $process_id; ?>', 'create_booking_search_list_view', 'search_div', 'yd_production_v2_controller', 'setFilterGrid(\'table_body\',-1)')"
                               style="width:100px;"/>
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td align="center">
                        <input type="button" name="btn_show" class="formbutton" value="Show"
                               onClick="show_list_view ( document.getElementById('cbo_company_id').value+'_'+document.getElementById('txt_system_no').value+'_'+document.getElementById('txt_batch_no').value+'_'+'<? echo $load_unload; ?>', 'create_sys_search_list_view', 'search_div', 'yd_production_v2_controller', 'setFilterGrid(\'list_view\',-1)')"
                               style="width:100px;"/>
                    </td>
                </tr>
                <tr>
...
if ($action == "create_sys_search_list_view") {
	...
	$sql = "select a.entry_form,a.batch_for,a.batch_against,a.extention_no,b.service_source,b.service_company,b.id,b.batch_no,b.company_id,b.load_unload_id,b.batch_id,b.system_no,b.process_end_date,b.end_hours,b.end_minutes,b.production_date
			from  pro_batch_create_mst a,yd_production_dtls b where a.id=b.batch_id and b.entry_form=414 and b.status_active=1 and b.is_deleted=0 $com_cond $sys_cond $batch_no_cond  $load_unload_cond $user_working_comp_cond order by b.id desc";
	//echo $sql;
	$sql2=sql_select($sql);
	foreach ($sql2 as $row) {
		...
		$prod_arr[$row[csf('id')]]['prod_date']= $row[csf('process_end_date')];
		$prod_arr[$row[csf('id')]]['end_date']= $row[csf('production_date')];
		$prod_arr[$row[csf('id')]]['end_hours']= $row[csf('end_hours')];
		$prod_arr[$row[csf('id')]]['end_minutes']= $row[csf('end_minutes')];
	}
	$sql_last_prod="select max(b.id) as mst_id,b.batch_id
			from  pro_batch_create_mst a,yd_production_dtls b where a.id=b.batch_id and b.entry_form=414 and b.status_active=1 and b.is_deleted=0 $com_cond $sys_cond $batch_no_cond  $load_unload_cond $user_working_comp_cond group by b.batch_id order by mst_id desc";
			
	$result_last_prod=sql_select($sql_last_prod);
	foreach ($result_last_prod as $row) 
	{
		$last_prod_arrr[$row[csf('batch_id')]]=$row[csf('mst_id')];
	}
	//print_r($last_prod_arrr);
    ...
    <div>
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="820" class="rpt_table">
            ...
            <th width="80">Process Start/Prod Date</th>
            <th width="80">Extention No</th>
            <th width="80">Batch For</th>
            <th width="80">Batch Against</th>
            <th width="">Service Company</th>
            
            </thead>
        </table>
        <div style="width:820px; overflow-y:scroll; max-height:230px;" id="buyer_list_view" align="center">
				...
				if (!empty($nameArray)) 
				{
					foreach ($nameArray as $row) 
					{
						if ($i % 2 == 0)
							...
							$bgcolor = "#FFFFFF";
					$last_mst_id=$last_prod_arrr[$row[csf('batch_id')]];
					$prod_date=change_date_format($prod_arr[$last_mst_id]['prod_date']);
					$end_date=change_date_format($prod_arr[$last_mst_id]['end_date']);
					$end_hours=$prod_arr[$last_mst_id]['end_hours'];
					$end_minutes=$prod_arr[$last_mst_id]['end_minutes'];
					
					//echo $prod_date.'='.$end_date.'<br>';
						//id,system_no,batch_id,batch_no,load_unload_id,entry_form
								 
								?>
	                            <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
	                                onClick="js_set_value('<? echo $row[csf('id')] . '_' . $row[csf('system_no')] . '_' . $row[csf('batch_id')]. '_' . $row[csf('batch_no')]. '_' . $row[csf('load_unload_id')]. '_' . $row[csf('entry_form')]. '_' . $end_date. '_' . $prod_date. '_' . $end_hours. '_' . $end_minutes; ?>')">
	                                <td width="40" align="center"><? echo $i; ?></td>
	                                <td width="120"><p><? echo $company_name_arr[$row[csf('company_id')]]; ?></p></td>
	                                <td width="100"><p><? echo $row[csf('system_no')]; ?></p></td>
	                                <td width="80"><p><? /*if($selectResult[csf('extention_no')]!=0)*/
											echo $row[csf('batch_no')]; ?></p></td>
                                   <td width="80"><? echo change_date_format($row[csf('process_end_date')]); ?></td>
	                                <td width="80"><? echo $row[csf('extention_no')]; ?></td>
                                     
	                                <td width="80" align="right"><? echo $batch_for[$row[csf('batch_for')]]; ?></td>
	                                <td width="80"><p><? echo $batch_against[$row[csf('batch_against')]];; ?></p></td>
	                                <td width=""><p><? echo $batch_comp_supp_arr[$row[csf('batch_id')]]; ?></p></td>
...
if ($action == "dyeing_pro_print") {
	...
	$qry_pro_fab_subpr = "select a.system_no,$select_list,sum(b.batch_qty) as batch_qty from yd_production_dtls a,pro_yd_subprocess_dtls b where a.entry_form=414 and a.id=b.mst_id and b.entry_page=414 and a.company_id=$data[0] and a.is_deleted=0 and a.status_active=1 and a.load_unload_id=$data[2] and b.status_active=1 and b.is_deleted=0 and a.system_no=$data[1] group by a.system_no";
...
if ($action == 'get_load_end_hour_minute'){
	$sql_result_arr=sql_select("select end_hours,end_minutes from yd_production_dtls where batch_id=$data and entry_form=414 and load_unload_id =1 and status_active=1 and is_deleted=0");
