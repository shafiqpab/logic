...
if ($action == "check_batch_no")
 {  
	...
	{
		$user_working_comp_cond = " and company_id in (".$_SESSION['logic_erp']["company_id"].")";
	}
	  $sql = "select id, entry_form,batch_number,company_id from yd_batch_mst where batch_number='" . trim($data[1]) . "' and entry_form in(398) and is_deleted=0 and status_active=1 $user_working_comp_cond order by id desc";  
...
if ($action=='load_drop_down_location') {
    echo create_drop_down('cbo_location_name', 110, "select id,location_name from lib_location where company_id='$data' and status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "--Select Location--", $selected, "load_drop_down('requires/yd_production_controller', document.getElementById('cbo_company_id').value+'_'+this.value, 'load_drop_down_floor', 'floor_td' );load_drop_down( 'requires/yd_production_controller', document.getElementById('cbo_company_id').value+'_'+this.value, 'load_drop_down_machine', 'machine_td');" );
}

if ($action=='load_drop_down_floor') {
    $ex_data=explode('_',$data);
    echo create_drop_down( "cbo_floor_name", 110, "select id,floor_name from lib_prod_floor where company_id=$ex_data[0] and location_id=$ex_data[1] and status_active =1 and is_deleted=0 order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/yd_production_controller', document.getElementById('cbo_company_id').value+'_'+document.getElementById('cbo_location_name').value+'_'+this.value, 'load_drop_down_machine', 'machine_td' );" );
}
...
if ($action=='load_drop_down_machine') {
    ...
    if($data[1]==0 || $data[2]==0) {
        echo create_drop_down('cbo_machine_id', 135, $blank_array, '', 1, '-- Select Machine --', $selected, '');
    } else {
        if($db_type==2) {
            echo create_drop_down( "cbo_machine_id", 135, "select id,machine_no || '-' || brand as machine_name from lib_machine_name where company_id='$data[0]' and location_id='$data[1]' and floor_id='$data[2]' and status_active=1 and is_deleted=0 and is_locked=0 order by seq_no","id,machine_name", 1,"-- Select Machine --", $selected, "","" );
        } else if($db_type==0) {
            echo create_drop_down( "cbo_machine_id", 135, "select id,concat(machine_no,'-',brand) as machine_name from lib_machine_name where company_id='$data[0]' and location_id='$data[1]' and floor_id='$data[2]' and status_active=1 and is_deleted=0 and is_locked=0 order by  seq_no","id,machine_name", 1, "-- Select Machine --", $selected, "","" );
        }
    }   
}
            ...
            <table width="580" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
                ...
                <tbody>
                    <tr class="general">
                        <td>
                            <?php
                            echo create_drop_down('cbo_company_id', 140, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name", 'id,company_name', 1, '-- Select Company --',$cbo_company_id, '', 1); ?>
                        </td>
                        <td>
                            <input class="text_boxes" type="text" name="txt_batch_no" id="txt_batch_no" style="width: 140px;">
                        </td>
                        <td>
                            <input class="text_boxes" type="text" name="txt_serial_batch_no" id="txt_serial_batch_no" style="width: 140px;">
                        </td>
                        <td>
                        ...
                        <td align="center">
                            ...
                            <input type="button" name="showBtn" class="formbutton" value="Show" onClick="show_list_view(document.getElementById('cbo_search_type').value+'_'+document.getElementById('cbo_company_id').value+'_'+document.getElementById('txt_serial_batch_no').value+'_'+document.getElementById('txt_job_no').value+'_'+document.getElementById('txt_batch_no').value+'_'+'<? echo $load_unload; ?>', 'create_batch_search_list_view', 'search_div', 'yd_production_controller', 'setFilterGrid(\'list_view\',-1)')" style="width: 80px;" />
                        </td>
                    </tr>
                </tbody>
            </table>
...
if ($action == 'create_batch_search_list_view')
 {
	...
	if ($load_unload_id == 1) 
	{
		   $sql = "select a.id, a.yd_batch_id, a.batch_number, b.yd_job
            ...
            where a.is_deleted=0 and a.status_active=1 $condition and a.yd_job_id=b.id and a.load_unload_id!=1";
	} 
	else 
	{
		 $sql = "select a.id, a.yd_batch_id, a.batch_number, b.yd_job
...
if($action == 'save_update_delete') 
{
    ...
    if ($operation==0) 
	{
        ...
        $data_array_dtls = "(".$dtlsId.",".$entry_form.",".$mstId.",".$txtBatchId.", ".$hdnYdOrdId.",".$tempDtls.",".$hdnYdOrdId.",".$tmpQty.",".$cbo_load_unload.", ".$cbo_company_id.", ".$cbo_service_source.", ".$cbo_service_company.", ".$cbo_process.", ".$txtProcessStartDate.", ".$txtProcessStartHour.", ".$txtProcessStartMinute.", ".$cbo_floor_name.", ".$cbo_machine_id.", ".$machine_group_id.", ".$txtRemarks.", ".$txtLoadingDate.", ".$txtLoadingHour.", ".$txtLoadingMinute.", ".$cbo_party.", ".$cbo_location_name.", ".$hdnSalesOrderId.", ".$txtSalesOrderNo.", ".$hdnProductId.",".$hdnBatchMstId.", ".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."')";
        ...
        // echo "10**insert into yd_production_dtls(".$field_array_dtls.") values ".$data_array_dtls;die;
         $rID2 = sql_insert('yd_production_dtls', $field_array_dtls, $data_array_dtls, 0);
		    ...
		    $hdnBatchMstId = str_replace("'", '', $hdnBatchMstId);
 			$upPiId=true;
 			$field_array_batch="load_unload_id";
			$data_array_batch="".$cbo_load_unload."";
			$upPiId= sql_update("yd_batch_mst",$field_array_batch,$data_array_batch,"id",$hdnBatchMstId,0);
			 

        $flag = ($flag && $rID2 && $upPiId);   // return true if $flag is true and dtls table insert is successful
        ...
        $field_array_dtls = 'quantity*load_unload_id*service_source_id*service_company_id*process_id*start_date*start_hours*start_minutes*floor_id*machine_id*machine_group_id*remarks*loading_date*loading_hours*loading_minutes*party_id*location_id*batch_id*updated_by*update_date';
        $data_array_dtls = ''.$tmpQty.'*'.$cbo_load_unload.'*'.$cbo_service_source.'*'.$cbo_service_company.'*'.$cbo_process.'*'.$txtProcessStartDate.'*'.$txtProcessStartHour.'*'.$txtProcessStartMinute.'*'.$cbo_floor_name.'*'.$cbo_machine_id.'*'.$machine_group_id.'*'.$txtRemarks.'*'.$txtLoadingDate.'*'.$txtLoadingHour.'*'.$txtLoadingMinute.'*'.$cbo_party.'*'.$cbo_location_name.'*'.$hdnBatchMstId.'*'.$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
        ...
        $flag = ($flag && $rID && $upPiId);    // return true if $flag is true and dtls table update is successful
