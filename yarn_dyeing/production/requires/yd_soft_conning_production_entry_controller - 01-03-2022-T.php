...
if ($action=="load_drop_down_buyer")
{
    ...
    echo create_drop_down( "cbo_party_name", 117, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Party --", $data, "");

    /*echo create_drop_down( "cbo_company_name", 117, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/yd_soft_conning_production_entry_controller', this.value, 'load_drop_down_buyer', 'party_td' );");*/
    exit();
...
if($action == "create_sales_order_list") {
    ...
    echo create_list_view('list_view', 'Sales Order No,Lot,Count,Composition,Order Qty', '100,50,100,150,100', '560', '700', 0, $sql, 'put_data_into_dtls', 'dtls_id', '', 1, '0,0,count_id,yarn_composition_id,0', $arr, 'sales_order_no,lot,count_id,yarn_composition_id,order_quantity', '', '', '0,0,0,0,1');
...
if($action == "populate_saved_data_from_list") {
    ...
    echo "load_drop_down( 'requires/yd_soft_conning_production_entry_controller', document.getElementById('cbo_yarn_type').value, '', '' );\n";
...
if ($action=="save_update_delete")
{
    ...
    {   
        ...
        $field_array2="id, mst_id,entry_form, sales_order_id, sales_order_no, product_id, job_id, job_dtls_id, order_id,winding_qty, quantity,bobbin_type, remarks,inserted_by, insert_date,status_active,is_deleted";  

        // echo $mst_id;die;
        
        $data_array2.="(".$id1.",".$mst_id.",'397',".$sales_order_id.",".$txtSalesOrder.",".$product_id.",".$job_id.",".$job_dtls_id.",".$order_id.",".$txtWindingCone.",".$txtPrdctnQty.",".$txtBobbinType.",".$txtRemarks.",".$_SESSION['logic_erp']['user_id'].",'".$pc_date_time."',1,0)";
        //echo $id1;
...
{  
    ...
    $nameArray=sql_select( "select id, yd_prod_no,prod_no_prefix,prod_no_prefix_num,prod_date, start_date,company_id, party_id, end_date from yd_production_mst where id='$data'");
   /* echo "select id, yd_prod_no,prod_no_prefix,prod_no_prefix_num,prod_date, start_date, party_id, end_date from yd_production_mst where id='$data'"; die;*/
    ...
    {   
        ...
        echo "load_drop_down('requires/yd_soft_conning_production_entry_controller', document.getElementById('cbo_company_name').value, 'load_drop_down_buyer', 'party_td' );\n";
