...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents('Re-Winding', '../../../', 1, 1, $unicode, 0, '');
?>
<script>
	...
	function put_data_into_dtls(id, action, controller_path) { // function copied from gray_production_entry put_data_dtls_part
		...
		get_php_form_data(id, 'populate_batch_from_list', 'requires/yd_re_winding_controller');
		set_button_status(0, permission, 'fnc_yd_re_winding',1);
		...
		/*if(production_id==''){
			...
			set_button_status(1, permission, 'fnc_yd_re_winding',1);
	...
	function put_data_into_form(id, action, controller_path) { // function copied from gray_production_entry put_data_dtls_part
		...
		$('#txtMachineRpm').removeAttr('disabled');
		$('#txtTension').removeAttr('disabled');
		$('#cbo_yarn_type').removeAttr('disabled');
		$('#txtWindingConeType').removeAttr('disabled');
		...
		reset_form('','','txtWindingCone*txtPrdctnQty*txtRemarks','','');
		get_php_form_data(id, 'populate_saved_data_from_list', 'requires/yd_re_winding_controller');
    ...
    { 
        ...
        var page_link='requires/yd_re_winding_controller.php?action=batch_popup&data='+data;
        var title="Issue ID";
        ...
        emailwindow.onclose=function()
        {
            ...
            if (splt_val[0]!='') {
				...
				/*reset_form('','','txtSalesOrder*cbo_company_name*cbo_location_name*cbo_party_name*txtStyle*txtProductionDate*txtMachineRpm*txtTension','','');*/

				show_list_view(theemail, 'create_sales_order_list', 'sales_order_list', 'requires/yd_re_winding_controller', '');
				//get_php_form_data( splt_val[0], "load_php_data_to_form", "requires/yd_re_winding_controller" );
				//show_list_view(splt_val[0], 'create_production_order_list', 'production_list', 'requires/yd_re_winding_controller', '');
    ...
    { 
        ...
        var data=document.getElementById('cbo_company_name').value+"_"+document.getElementById('cbo_party_name').value;
        var page_link='requires/yd_re_winding_controller.php?action=production_popup&data='+data;
        ...
        emailwindow.onclose=function()
        {
            ...
            if (splt_val[0]!='') {
				...
				show_list_view(splt_val[1], 'create_sales_order_list', 'sales_order_list', 'requires/yd_re_winding_controller', '');
				get_php_form_data( splt_val[0], "load_php_data_to_form", "requires/yd_re_winding_controller" );
				show_list_view(splt_val[0], 'create_production_order_list', 'production_list', 'requires/yd_re_winding_controller', '');
				$('#cbo_company_name').attr('disabled','disabled');
				set_button_status(0, permission, 'fnc_yd_re_winding',1);
	...
	function openSalesNumPopup() 
	{
		...
		page_link='requires/yd_re_winding_controller.php?action=job_popup&data='+data;
		...
		popupWindow.onclose=function() {
			...
			if (job_id_mst!='') {
				...
				show_list_view(job_id_mst, 'create_sales_order_list', 'sales_order_list', 'requires/yd_re_winding_controller', '');
				// show_list_view( data, action, div, path, extra_func, is_append )
				release_freezing();
				/*reset_form('softconning_1', '', 'txtBalanceQty', '', '', '');*/
	...
	function fnc_yd_re_winding( operation )
    {
        if ( form_validation('cbo_company_name*txtProductionDate*txtPrdctnQty', 'Company Name*Production Date*Production Quantity')==false )
        {
           return;
        }
        ...
        var data_str="";
	    var data_str=get_submitted_data_string('txtBatchId*txtStyle*txtSalesOrder*cbo_company_name*cbo_party_name*txtProductionDate*txtMachineRpm*txtTension*cbo_yarn_type*txtWindingConeType*txtWindingCone*txtPrdctnQty*txtRemarks*txtLot*txtCount*txtComposition*txtYdColor*txtDryerProdQty*txtProdDryerQtyPcs*txtBalanceQty*txtBalanceQtyPcs*update_id*sales_order_serial*update_dtls_id*job_dtls_id*order_id*job_id*within_group*booking_without_order*booking_type*sales_order_id*product_id*previous_production_qty*txtThreadLength*txtPackAvgRate*txtBatchId*txtProductionNo',"../");
	        //alert(data_str);
	        
	    var data="action=save_update_delete&operation="+operation+data_str;//+'&zero_val='+zero_val
	        /*alert(data);return;*/
	    freeze_window(operation);
	    http.open("POST","requires/yd_re_winding_controller.php",true);
        ...
        {
        	var data_str="";
		    var data_str=get_submitted_data_string('txtBatchId*txtStyle*txtSalesOrder*cbo_company_name*cbo_party_name*txtProductionDate*txtMachineRpm*txtTension*cbo_yarn_type*txtWindingConeType*txtWindingCone*txtPrdctnQty*txtRemarks*txtLot*txtCount*txtComposition*txtYdColor*txtDryerProdQty*txtProdDryerQtyPcs*txtBalanceQty*txtBalanceQtyPcs*update_id*sales_order_serial*update_dtls_id*job_dtls_id*order_id*job_id*within_group*booking_without_order*booking_type*sales_order_id*product_id*previous_production_qty*txtThreadLength*txtPackAvgRate*txtBatchId',"../");
		        //alert(data_str);
            ...
            if(response[0]==0 || response[0]==1)
            {
               ...
               /*alert(response);return;*/
                ...
                if(response[0]==0 ){
                	...
                	document.getElementById('sales_order_serial').value="";
                	document.getElementById('txtBalanceQtyPcs').value=parseInt(document.getElementById('txtBalanceQtyPcs').value)-parseInt(document.getElementById('txtPrdctnQty').value);
                }
                else
                {
                	document.getElementById('txtBalanceQtyPcs').value=(parseInt(document.getElementById('txtBalanceQtyPcs').value)+parseInt(document.getElementById('previous_production_qty').value))-parseInt(document.getElementById('txtPrdctnQty').value);
                }
                ...
                document.getElementById('txtMachineRpm').setAttribute('disabled', 'disabled');
                document.getElementById('txtTension').setAttribute('disabled', 'disabled');
                document.getElementById('cbo_yarn_type').setAttribute('disabled', 'disabled');
                document.getElementById('txtWindingConeType').setAttribute('disabled', 'disabled');
                ...
                $("#txtYdColor").val('');
                $("#txtDryerProdQty").val('');
                $("#txtProdDryerQtyPcs").val('');
                $("#txtBalanceQty").val('');
                $("#txtBalanceQtyPcs").val('');
                $("#txtThreadLength").val('');
                $("#txtPackAvgRate").val('');
                $("#txtPrdctnQty").val('');
                $("#txtRemarks").val('');
                ...
                show_list_view(job_id_mst, 'create_production_order_list', 'production_list', 'requires/yd_re_winding_controller', '');
               /* if(list_view_orders!='')
                {
    ...
    function check_production_balance(operation)
    {
    		...
    		if(operation==0)
    		{//for save
        	var available_production=parseInt(document.getElementById('txtBalanceQtyPcs').value);
        	...
        	var available_production=parseInt(document.getElementById('txtBalanceQtyPcs').value)+parseInt(document.getElementById('previous_production_qty').value);
        	var current_production=parseInt(document.getElementById('txtPrdctnQty').value);
        	if(current_production>available_production){
			...
			<table width="100%">
				...
				<tr>
					<td>
						...
						<table border="0">
							<tr>
								<td class="must_entry_caption" align="right">Batch No.</td>
								<td>
							...
							<tr>
								...
								<td id="company_td">
									<? echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/yd_re_winding_controller', '', 'load_drop_down_buyer', 'party_td' );"); ?>
								</td>
							</tr>
							<tr>
							...
							<tr>
								<td align="right" >Machine Rpm</td>
								<td>
									<input class="text_boxes_numeric" type="text"  name="txtMachineRpm" id="txtMachineRpm"  style="width:137px;"   />
								</td>
							</tr>
							<tr>
								<td align="right" >Tension</td>
								<td>
									<input class="text_boxes_numeric" type="text"  name="txtTension" id="txtTension"  style="width:137px;" />
								</td>
							</tr>
							<tr>
							...
							<tr>
								<td align="right">Winding Cone Type</td>
								<td>
									<?php
										// $bobbin_arr = array("Bobbin Type 1", "Bobbin Type 2", "Bobbin Type 3");
									 	// echo create_drop_down( "txtWindingConeType", 150, $bobbin_arr, 1, "-- Bobbin Type --", $selected, "");
									?>
									<input class="text_boxes_numeric" style="width:137px;" type="text" name="txtWindingConeType" id="txtWindingConeType" />
								</td>
							</tr>
							<tr>
								<td align="right">Thread Length</td>
								<td>
									<input class="text_boxes_numeric" type="text" style="width:137px;"   placeholder="Write" name="txtThreadLength" id="txtThreadLength" />
								</td>
							</tr>
							<tr>
								<td align="right">Package Avg. Weight</td>
								<td>
									<input class="text_boxes_numeric" type="text" style="width:137px;"   placeholder="Write" name="txtPackAvgRate" id="txtPackAvgRate" />
								</td>
							</tr>
							<tr>
								<td align="right">Winding Cone Qty.</td>
								<td>
					...
					<td valign="top" align="right">
						...
						<table border="0" style="margin-left:15px;">
							...
							<tr>
								<td align="right">Yarn Dryer Production Qty</td>
								<td>
									<input class="text_boxes_numeric" style="width:137px;" type="text" placeholder="Display"  name="txtDryerProdQty" id="txtDryerProdQty" readonly />
								</td>
							</tr>
							<tr>
								<td align="right">Production Dryer Package Qty/Pcs</td>
								<td>
									<input class="text_boxes_numeric" style="width:137px;" type="text" placeholder="Display"  name="txtProdDryerQtyPcs" id="txtProdDryerQtyPcs" readonly />
								</td>
							</tr>
							<tr>
								<td align="right">Balance Re-Winding Production Qty</td>
								<td>
									<input class="text_boxes_numeric must_entry_caption" style="width:137px;" type="text" placeholder="Display"  name="txtBalanceQty" id="txtBalanceQty" readonly/>
								</td>
							</tr>
							<tr>
								<td align="right">Balance Re-Winding Package QTY/Pcs</td>
								<td>
									<input class="text_boxes_numeric" style="width:137px;" type="text" placeholder="Display"  name="txtBalanceQtyPcs" id="txtBalanceQtyPcs" readonly/>
								</td>
							</tr>
						</table>
						<!-- display table end -->
				...
				<tr style="margin-top:50px;">
					<td colspan="2" align="center">
						<? echo load_submit_buttons($permission, "fnc_yd_re_winding", 0,0,"",1); ?>
					</td>
				</tr>
			</table>		
		</fieldset>
