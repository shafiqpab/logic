...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../../logout.php";
	...
	function openmypage_servicebook()
	{
		...
		var page_link='requires/yd_production_v2_controller.php?cbo_company_id='+cbo_company_id+'&supplier_id='+supplier_id+'&process_id='+process_id+'&action=service_booking_popup';
	...
	function openmypage_batchnum()
	{
		...
		{
			page_link='requires/yd_production_v2_controller.php?action=batch_number_popup&cbo_company_id='+cbo_company_id+'&batch_no='+batch_no+'&load_unload='+load_unload;

			//var page_link='requires/yd_production_v2_controller.php?cbo_company_id='+cbo_company_id+'&action=batch_number_popup'+'&load_unload=load_unload';
			...
			emailwindow.onclose=function()
			{
				...
				if(batch_datas!="")
				{
					...
					freeze_window(5);
					//get_php_form_data(document.getElementById('cbo_load_unload').value+'_'+response[1]+'_'+batch_no+'_'+response[2], "populate_data_from_batch", "requires/yd_production_v2_controller" );
					...
					get_php_form_data(document.getElementById('cbo_load_unload').value+'_'+batch_id+'_'+batch_no+'_'+is_sales+'_'+entry_form_no, "populate_data_from_batch", "requires/yd_production_v2_controller" );
					...
					show_list_view(batch_id+'_'+roll_maintained+'_'+document.getElementById('cbo_load_unload').value+'_'+document.getElementById('txt_process_id').value+'_'+entry_form_no+'_'+hidden_double_dyeing+'_'+hidden_result_id+'_'+cbo_dyeing_type,'show_fabric_desc_listview','list_fabric_desc_container','requires/yd_production_v2_controller','');
					$('#cbo_company_id').attr('disabled','disabled');

					//show_list_view(batch_id+'_'+document.getElementById('cbo_load_unload').value,'show_dtls_batch_list_view','list_container','requires/yd_production_v2_controller','');
					release_freezing();
				}
				get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/yd_production_v2_controller' );
	...
	function check_batch()
	{
		...
		if(batch_no!="")
		{
			...
			var response=return_global_ajax_value( cbo_company_id+"**"+batch_no, 'check_batch_no', '', 'requires/yd_production_v2_controller');
			...
			else
			{
				...
				//alert(response[1]);
				//get_php_form_data(response[1], "populate_data_from_batch2", "requires/yd_production_v2_controller" );
				get_php_form_data(document.getElementById('cbo_load_unload').value+'_'+response[1]+'_'+batch_no+'_'+response[2]+'_'+response[3], "populate_data_from_batch", "requires/yd_production_v2_controller" );
				...
				//show_list_view(response[1]+'_'+document.getElementById('cbo_load_unload').value+'_'+document.getElementById('txt_system_no').value,'show_dtls_list_view','list_container','requires/yd_production_v2_controller','');
				//show_list_view(response[1]+'_'+document.getElementById('cbo_load_unload').value+'_'+document.getElementById('txt_system_no').value,'show_dtls_batch_list_view','list_container','requires/yd_production_v2_controller','');
				get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/yd_production_v2_controller' );
	...
	function fnc_yd_production_dtls( operation ) //Save Update Here...
	{
		if(operation==4)// print
		{
			//function print_report( data, action, path )
			var report_title=$( "div.form_caption") .html();
			print_report( $('#cbo_company_id').val()+'*'+$('#txt_system_no').val()+'*'+$('#cbo_load_unload').val()+'*'+$('#txt_batch_ID').val()+'*'+report_title, "dyeing_pro_print", "requires/yd_production_v2_controller")
			//alert(print_report);
			return;
		}
		
		
		if( form_validation('cbo_load_unload*total_batch_qnty','Load Unload*Total Batch Qty')==false )
		{
			return;
		}
		...
		if (document.getElementById('cbo_load_unload').value==1) //Loading Here
		{
			...
			{
			var response2=return_global_ajax_value(cbo_company_id+"**"+batch_no+"**"+batch_id+"**"+machine_no,'check_for_shade_matched','','requires/yd_production_v2_controller');
			var response2=response2.split("_");
			}
			//alert(1);
			var response=return_global_ajax_value( service_company+"**"+batch_no+"**"+batch_id+"**"+machine_no, 'check_batch_no_for_machine', '', 'requires/yd_production_v2_controller');
			var response=response.split("_");
			if(service_source!=3)
			{
			...
			if(operation==0)
			{
				...
				else
				{
					if(yesno==2)
					{
						...
						/*var response3=return_global_ajax_value( process_start_date+"**"+cbo_floor+"**"+machine_no+"**"+update_id, 'machine_load_status', '', 'requires/yd_production_v2_controller');
					...
					else if(yesno==1)
					{
						...
						{
							var date_response=trim(return_global_ajax_value( functional_batch+"**"+process_start_date, 'check_process_start_date', '', 'requires/yd_production_v2_controller'));
		...
		if (document.getElementById('cbo_load_unload').value==2) //unloading here
		{
			...
			//var date_time_response=return_global_ajax_value( cbo_company_id+"**"+batch_no+"**"+batch_id+"**"+dyeing_end_date+"**"+dyeing_started_date+"**"+unload_hr_min+"**"+load_hr_min, 'check_date_time', '', 'requires/yd_production_v2_controller');
			...
			var load_end_hour_minute_data=return_global_ajax_value( $('#hidden_batch_id').val(), 'get_load_end_hour_minute', '', 'requires/yd_production_v2_controller');
			...
			//var end_minute=$('#txt_end_minutes').val()*1;
				...
				{
					var date_response=trim(return_global_ajax_value( functional_batch+"**"+process_start_date, 'check_process_production_date', '', 'requires/yd_production_v2_controller'));
			...
			for (var i=1; i<=row_num; i++)
			{
				...
				else
				{
				 ...
				 data_all+="&txtconscomp_" + z + "='" + $('#txtconscomp_'+i).val()+"'"+"&txtgsm_" + z + "='" + $('#txtgsm_'+i).val()+"'"+"&txtdiawidth_" + z + "='" + $('#txtdiawidth_'+i).val()+"'"+"&txtdiatype_" + z + "='" + $('#txtdiatype_'+i).val()+"'"+"&txtroll_" + z + "='" + $('#txtroll_'+i).val()+"'"+"&txtbatchqnty_" + z + "='" + $('#txtbatchqnty_'+i).val()+"'"+"&txtprodqnty_" + z + "='" + $('#txtprodqnty_'+i).val()+"'"+"&hiddendiatypeid_" + z + "='" + $('#hiddendiatypeid_'+i).val()+"'"+"&txtlot_" + z + "='" + $('#txtlot_'+i).val()+"'"+"&rollid_" + z + "='" + $('#rollid_'+i).val()+"'"+"&txtyarncount_" + z + "='" + $('#txtyarncount_'+i).val()+"'"+"&updateiddtls_" + z + "='" + $('#updateiddtls_'+i).val()+"'"+"&txtbrand_" + z + "='" + $('#txtbrand_'+i).val()+"'"+"&txtprodid_" + z + "='" + $('#txtprodid_'+i).val()+"'"+"&txtremark_" + z + "='" + $('#txtremark_'+i).val()+"'"+"&txtrate_" + z + "='" + $('#txtrate_'+i).val()+"'"+"&txtamount_" + z + "='" + $('#txtamount_'+i).val()+"'"+"&txtyarntype_" + z + "='" + $('#txtyarntype_'+i).val()+"'"+"&txtbobintype_" + z + "='" + $('#txtbobintype_'+i).val()+"'"+"&txtWindingPackageQty_" + z + "='" + $('#txtWindingPackageQty_'+i).val()+"'"+"&txtjobdtlsid_" + z + "='" + $('#txtjobdtlsid_'+i).val()+"'"+"&txtyarntypeid_" + z + "='" + $('#txtyarntypeid_'+i).val()+"'"+"&txtyarncountid_" + z + "='" + $('#txtyarncountid_'+i).val()+"'"+"&txtjobid_" + z + "='" + $('#txtjobid_'+i).val()+"'"+"&txtorderid_" + z + "='" + $('#txtorderid_'+i).val()+"'"+"&txtsalesorderid_" + z + "='" + $('#txtsalesorderid_'+i).val()+"'";
					z++;
			}
			...
			var cbo_company_id = $('#cbo_company_id').val();
			var batch_id = $('#hidden_batch_id').val();
			var response=return_global_ajax_value( cbo_company_id+"**"+batch_no+"**"+batch_id+"**"+process_id, 'check_batch_no_load', '', 'requires/yd_production_v2_controller');
			var response=response.split("_");
			if(response[0]==0)
				{
					//alert(process_id);
	...
	function fnc_yd_production_dtls_response()
	{
		if(http.readyState == 4)
		{
			...
			{
				show_msg(response[0]);
				//set_button_status(1, permission, 'fnc_yd_production_dtls',1,1);

				document.getElementById('txt_update_id').value = '';
				//alert(response[3]);
				document.getElementById('txt_system_no').value = response[3];
				set_button_status(0, permission, 'fnc_yd_production_dtls',0,1);
				release_freezing();
			}
			else if(response[0]==1)
			{
				show_msg(response[0]);
				set_button_status(0, permission, 'fnc_yd_production_dtls',1,1);
				release_freezing();
				document.getElementById('txt_update_id').value = '';
			}
			...
			//show_list_view(document.getElementById('txt_batch_ID').value+'_'+document.getElementById('roll_maintained').value+'_'+document.getElementById('cbo_load_unload').value,'show_fabric_desc_listview','list_fabric_desc_container','requires/yd_production_v2_controller','');
			//alert(response[3]);
			show_list_view( response[2]+'_'+response[4]+'_'+response[3]+'_'+document.getElementById('txt_entry_form_no').value,'show_dtls_batch_list_view','list_container','requires/yd_production_v2_controller','');
	...
	function load_list_view (str)
	{
		...
		show_list_view(str,'on_change_data','load_unload_container','requires/yd_production_v2_controller','');
		...
		set_button_status(0, permission, 'fnc_yd_production_dtls',1);
	}
	...
	var batch_no=$('#txt_batch_no').val();
	var cbo_company_id = $('#cbo_company_id').val();
	//var response=return_global_ajax_value( cbo_company_id+"**"+str, 'check_batch_no_scan', '', 'requires/yd_production_v2_controller');
	//var response=response.split("_");
	check_batch();
	...
	/*if(response[0]==0)
	...
	else
		{
			$('#hidden_batch_id').val(response[1]);
			get_php_form_data(document.getElementById('cbo_load_unload').value+'_'+response[1]+'_'+batch_no+'_'+response[2], "populate_data_from_batch", "requires/yd_production_v2_controller" );
			show_list_view(response[1],'show_fabric_desc_listview','list_fabric_desc_container','requires/yd_production_v2_controller','');
			get_php_form_data(document.getElementById('cbo_company_id').value+'**'+document.getElementById('cbo_floor').value+'**'+document.getElementById('cbo_machine_name').value, 'populate_data_from_machine', 'requires/yd_production_v2_controller' );
			$('#cbo_company_id').focus();
		}*/
	}
...
function openmypage_process()
	{
		...
		var page_link = 'requires/yd_production_v2_controller.php?txt_process_id='+txt_process_id+'&action=process_name_popup';
	...
	function openmypage_process_unload()
	{
		var txt_process_id = $('#txt_process_id').val();
		var title = 'Process Name Selection Form';
		var page_link = 'requires/yd_production_v2_controller.php?txt_process_id='+txt_process_id+'&action=process_name_popup_unload';
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=400px,height=370px,center=1,resize=1,scrolling=0','../');
		emailwindow.onclose=function()
		{
		var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
	...
	function openmypage_issue_challan()
	{
		...
		emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/yd_production_v2_controller.php?cbo_company_id='+cbo_company_id+'&action=issue_challan_popup','Issue Popup', 'width=880px,height=350px,center=1,resize=1,scrolling=0','../')
		emailwindow.onclose=function()
		{
			var theform=this.contentDoc.forms[0]//("search_order_frm"); //Access the form inside the modal window
			...
			if(issue_id!="")
			{
				get_php_form_data(issue_id, "populate_data_from_data", "requires/yd_production_v2_controller");
	...
	function check_issue_challan_scan(str) //Issue Challan
	{
		...
		if(issue_chalan!="")
		{
			var response=return_global_ajax_value( cbo_company_id+"**"+issue_chalan, 'check_issue_challan_no', '', 'requires/yd_production_v2_controller');
			var response=response.split("_");
			...
			else
			{
				...
				get_php_form_data(response[1], "populate_data_from_data", "requires/yd_production_v2_controller" );
				var hidden_roll_id = $('#txt_roll_id').val();
				show_list_view(document.getElementById('txt_batch_ID').value+'_'+document.getElementById('roll_maintained').value+'_'+document.getElementById('cbo_load_unload').value,'issue_show_fabric_desc_listview','list_fabric_desc_container','requires/yd_production_v2_controller','');
	 ...
	 function check_issue_challan_scan(str) //Issue Challan Scan
	{
		...
		if(issue_chalan!="")
		{
			var response=return_global_ajax_value( cbo_company_id+"**"+issue_chalan, 'check_issue_challan_no_scan', '', 'requires/yd_production_v2_controller');
			var response=response.split("_");
			if(response[0]==0)
			{
				alert('Issue Challan  not found.');
			...
			else
			{
				...
				get_php_form_data(response[1], "populate_data_from_data", "requires/yd_production_v2_controller" );
				//var hidden_roll_id = $('#txt_roll_id').val();
			//	show_list_view(batch_id+'_'+hidden_roll_id+'_'+cbo_company_id,'issue_show_fabric_desc_listview','list_fabric_desc_container','requires/yd_production_v2_controller','');
				$('#cbo_service_source').focus();
			}
		...
		//
			...
			if(service_source==1)
			{
					if(hidden_service_company!=service_company)
					{
						...
						load_drop_down( 'requires/yd_production_v2_controller', hidden_service_company, 'load_drop_floor', 'floor_td' );
					}
					else
					{
	...
	function calculate_production_qnty()
	{
		...
		math_operation( "total_production_qnty", "txtprodqnty_", "+",numRow,ddd );
			...
			for(var i=1;i<=numRow; i++)
			{
				...
				if(batch_qty<p_qty)
				{
					alert('Prod Qty is greater than batch Qty');
...
function open_syspopup(page_link,title)
{
	...
	var load_unload = $("#cbo_load_unload").val();
	var batch_no = $("#txt_batch_no").val();
	page_link='requires/yd_production_v2_controller.php?action=sys_popup&company='+company+'&batch_no='+batch_no+'&load_unload='+load_unload+'&system_no='+system_no;
	...
	emailwindow.onclose=function()
	{
		...
		if (sysNumber!="")
		{
			...
			//get_php_form_data(sysNumber[0], "populate_master_from_data", "requires/yd_production_v2_controller" );

			show_list_view( sysNumber[2]+'_'+sysNumber[4]+'_'+sysNumber[1]+'_'+sysNumber[5],'show_dtls_batch_list_view','list_container','requires/yd_production_v2_controller','');			//function disable_enable_fields( flds, operation, loop_flds, loop_leng )
...
function fnc_load_data(type_id)
{
		...
		if(type_id==1)
		{
		get_php_form_data($('#cbo_company_id').val()+'**'+$('#cbo_machine_name').val()+'**'+$('#cbo_floor').val(),'load_batch_machine','requires/yd_production_v2_controller' );
		}
		/*else
		{
...
function fnc_mc_load()//
{
	...
	if(floor_length==1)
	{
		load_drop_down( 'requires/yd_production_v2_controller', document.getElementById('cbo_service_company').value+'**'+document.getElementById('cbo_floor').value, 'load_drop_machine', 'machine_td' );
	}
}
</script>
</head>
        ...
        <table cellpadding="0" cellspacing="1" width="1250" border="0" align="left" height="auto" id="master_tbl">
                                    ...
                                    <td colspan="4" align="center"><strong>Functional Batch No :</strong> &nbsp;<input type="text" name="txt_system_no" id="txt_system_no" class="text_boxes_numeric" onDblClick="open_syspopup()" style="width:100px;" placeholder="Double Click To Browse" readonly />  </td>
                                        ...
                                        <tr>
                                         ...
                                         <tr>
                                            ...
                                            <td colspan="2"> <div id="batch_type" style="color:#F00"> </div> </td>
                                        </tr>
                                       <!-- <tr>
                                ...
                                <fieldset>
                                     <table width="660" align="right" class="rpt_table" rules="all" id="tbl_item_details">
                                        <thead>
                                            ...
                                            <th style="display:none">D/W Type</th>
                                            <th style="display:none">Roll No</th>
                                            <th>Batch Qty</th>
                                            <th>Prod. Qty</th>
                                            <th width="" style="display:none" >Rate</th>
                                            <th width="" style="display:none" >Amount</th>     
                                        ...
                                        <tbody id="list_fabric_desc_container">
                                            ...
                                            <td><input type="text" name="txtbrand_1" id="txtbrand_1" class="text_boxes_numeric" style="width:60px;" readonly disabled /> </td>
											...
											<td align="right" style="display:none"><input type="text" name="total_amount" id="total_amount" class="text_boxes_numeric" style="width:70px"  readonly/></td> 
											</tr>
                                        </tbody>
                                  </table>
                               </fieldset>
            ...
            <tr>
                <td align="center" colspan="4" class="button_container">
                    <?
                        echo load_submit_buttons($permission, "fnc_yd_production_dtls", 0,1,"fnResetForm()",1);
                    ?>
                </td>
            </tr>
