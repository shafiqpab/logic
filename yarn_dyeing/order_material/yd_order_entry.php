...
<script>
	...
	function fnc_wo_check_advance(type)
	{
        ...
        $("#tbl_dtls_yarn_dyeing tbody tr").each(function()
        {
            ...
            var hdnPiDtlsId       	= $(this).find('input[name="hdnPiDtlsId[]"]').val();
    ...
    function fnc_yarn_dyeing_response()
    {
        ...
        if(http.readyState == 4) 
        {
            ...
            if(response[0]==0 || response[0]==1){
                ...
                var within_group = $('#cbo_within_group').val();
                var advance_job = $('#txt_advance_job').val();
                var tag_pi_no = $('#txt_tag_pi_no').val();
                /*if(within_group==2){
               ...
               // $('#txt_delivery_date').attr('disabled',true);
                ...
                show_list_view(2+'_'+update_id+'_'+job_no+'_'+within_group+'_'+order_no+'_'+advance_job+'_'+tag_pi_no,'order_dtls_list_view','dyeing_details_container','requires/yd_order_entry_controller','setFilterGrid(\'list_view\',-1)');
    ...
    function openmypage_job(){
        ...
        emailwindow.onclose=function()
        {
            ...
            show_list_view(2+'_'+theemaildata+'_'+advance_job+'_'+tag_pi_no,'order_dtls_list_view','dyeing_details_container','requires/yd_order_entry_controller','setFilterGrid(\'list_view\',-1)');
            set_button_status(1, permission, 'fnc_yarn_dyeing',1);
    ...
    {
        if (form_validation('cbo_company_name*cbo_within_group*cbo_party_name', 'Company Name*Within Group*Party') == false) {
            return;
        }
        var company = $("#cbo_company_name").val();
        var cbo_within_group = $("#cbo_within_group").val();
        var cbo_party_name = $("#cbo_party_name").val();
        page_link = 'requires/yd_order_entry_controller.php?action=order_popup&company=' + company+'&cbo_within_group=' + cbo_within_group+'&cbo_party_name=' + cbo_party_name;
        ...
        emailwindow.onclose = function () 
        {
            ...
            if (sys_number[0] != "") {
                ...
                show_list_view(1+'_'+order_id+'_'+0+'_'+within_group+'_'+0+'_'+advance_job+'_'+tag_pi_no,'order_dtls_list_view','dyeing_details_container','requires/yd_order_entry_controller','setFilterGrid(\'list_view\',-1)');
    ...
    function sum_total_qnty(id)
    {
       ...
       // var totalQty=orderQty/100+processLoss+orderQty;
        ...
        $('#txtAmount_'+id).val(total_amount.toFixed(4));
...
//alert(type);
	...
	function fnc_amount_cal(type)
	{
		...
		var total_amount=qty*rate;
        $('#txtAmount_'+type).val(total_amount.toFixed(4));
        ...
        $('#txtTotalqty_'+type).val(total_qty.toFixed(2));
    ...
    function check_total_amount()
    {
        ...
        var txt_tag_pi_no         = $('#txt_tag_pi_no').val();
        var balance_qty           = $('#txt_tag_pi_balance_qty').val()*1;
        var tot_blance_amount     = $('#txt_tag_pi_amount_qty').val()*1;
        ...
        if(txt_check_box_confirm!='' && txt_advance_job!='' && txt_tag_pi_no!='')
        {
            ...
            var tot_qty = 0; var tot_amount = 0;

            for(var j=1; j<=row_num; j++)
            {
                tot_qty += $('#txtOrderqty_'+j).val()*1;
                //tot_blance_amount += (document.getElementById("txtAmount_"+j).placeholder)*1;
                tot_amount += $('#txtAmount_'+j).val()*1;

                var order_qty = $('#txtOrderqty_'+j).val()*1;
                var order_balance = $('#hdnPiBalance_'+j).val()*1;

                if(order_qty>order_balance)
                {
                	//alert("Order Quantity Can Not Be Greater Than Balance Quantity!!! Balance Quantity ="+order_balance);
                	//return;
                }

            }

            if(tot_qty>balance_qty)
            {
                alert("Total Order Quantity Can Not Be Greater Than Balance Quantity!!! Total Balance Quantity ="+balance_qty);
                return true;
            }

            if(tot_amount>tot_blance_amount)
            {
                alert("Total Order Amount Can Not Be Greater Than Balance Amount!!! Total Balance Amount ="+tot_blance_amount);
                return true;
            }
        }
    ...
    function fnc_addRow( i, table_id, tr_id )
    {
        ...
        else
        {
            ...
            var txtcountType= $("#"+'txtcountType_'+i).val();
            var cboCount= $("#"+'cboCount_'+i).val();
            var cboComposition= $("#"+'cboComposition_'+i).val();
			var cboYarnType= $("#"+'cboYarnType_'+i).val();
            var cboUom= $("#"+'cboUom_'+i).val();
            var txtadjType= $("#txtadjType_1").val();
            ...
            $("#tbl_dtls_yarn_dyeing").append(clone);
            //$('#rate_'+row_num).removeAttr("onKeyUp").attr("onKeyUp","calculate_amount("+row_num+")");
             $('#cboYarnType_'+row_num).removeAttr("value").attr("value",cboYarnType);
            $('#txtprocess_'+row_num).removeAttr("value").attr("value",txtprocess);
            $('#txtcountType_'+row_num).removeAttr("value").attr("value",txtcountType);
            ...
            $('#txtadjType_'+row_num).removeAttr("value").attr("value",txtadjType);
            ...
            $('#txtRemarks_'+row_num).removeAttr("onclick").attr("onclick","openmypage_remarks("+row_num+");");
    ...
    function openmypage_process(row_no)
    {
        ...
        var page_link = 'requires/yd_order_entry_controller.php?action=process_popup&txtprocess='+txtprocess+'&txtprocessname='+txtprocessname+'&yd_process='+cbo_yd_process+'&cbo_yd_type='+cbo_yd_type;
    ...
    {
        if( form_validation('cbo_company_name*txt_order_receive_date*cbo_within_group*cbo_party_name','Company Name*Order Receive Date*Within Group*Party Name')==false )
        {
            return;
        }
        var data=document.getElementById('cbo_company_name').value+'_'+document.getElementById('txt_advance_job').value+'_'+document.getElementById('cbo_within_group').value+'_'+document.getElementById('cbo_party_name').value;
        emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/yd_order_entry_controller.php?action=job_no_popup&data='+data,'YD Job No Popup', 'width=940px,height=380px,center=1,resize=0','../')
        emailwindow.onclose=function()
        {
            ...
            if (theemail.value!="")
            {
                ...
                var pi_id= response[4];
    			var item_category_id= response[5];
    			var importer_id= document.getElementById('cbo_company_name').value;
    			var within_group = document.getElementById('cbo_within_group').value;
    			var pi_no = response[2];

				show_list_view(pi_id+'_'+item_category_id+'_'+importer_id+'_'+within_group+'_'+pi_no, 'pi_details', 'dyeing_details_container', 'requires/yd_order_entry_controller', '' ) ;
                ...
                document.getElementById("txt_tag_pi_amount_qty").value=response[6];
                document.getElementById("cbo_pro_type").value=response[7];
                document.getElementById("cbo_order_type").value=response[8];
                document.getElementById("cbo_yd_type").value=response[9];
                document.getElementById("cbo_yd_process").value=response[10];
                document.getElementById("cbo_currency").value=response[11];
                disable_enable_fields('txt_tag_pi_no*cbo_party_name*cbo_pro_type*cbo_order_type*cbo_yd_type*cbo_yd_process*cbo_currency',1);
                exchange_rate(response[11]);
                release_freezing();
            }
        }
    }
    ...
    function openmypage_remarks(id) {
        ...
        emailwindow=dhtmlmodal.open('EmailBox', 'iframe','requires/yd_order_entry_controller.php?data='+data+'&action=remarks_popup','Remarks Popup', 'width=420px,height=320px,center=1,resize=1,scrolling=0','')
...
<body onLoad="set_hotkey()">
	<div style="width:100%;" align="center">
            ...
            <legend>Yarn Dyeing Order Entry</legend>
                <table width="1330" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td width="350"><?php echo create_drop_down( "cbo_within_group", 150, $yes_no,"", 1, "-- Select --", 0, "fnc_load_party(1,this.value);fnc_load_wo(this.value);" ); ?> &nbsp;
                    ...
                    <tr>
                        ...
                        <td><input type="text" name="txt_tag_pi_no" id="txt_tag_pi_no" style="width:140px" class="text_boxes" value="" placeholder="Write" />
                            ...
                            <input type="hidden" name="txt_tag_pi_amount_qty" id="txt_tag_pi_amount_qty" style="width:140px" class="text_boxes" value="" placeholder="Write" />
                        </td>
        ...
        <fieldset style="width:2400px;">
           <legend>YD Order Entry Details</legend>
                <table width="2380px" cellpadding="0" cellspacing="2" border="0" class="rpt_table" rules="all" id="tbl_dtls_yarn_dyeing">
                    <thead class="form_table_header">
                        ...
                        <th width="90" class="must_entry_caption">Raw Yarn Lot</th>
                        ...
                        <th width="60" title="Approval Ref./Lab Dip">App.Ref./Lot</th>
                        ...
                        <tr id="row_1">
                           ...
                           <input type="hidden" name="txtprocess[]" id="txtprocess_1" class="text_boxes" value="" style="width:80px;"  readonly />
                            ...
                            <td><input name="txtAVG[]" id="txtAVG_1" type="text"  class="text_boxes_numeric" style="width:50px" placeholder="Write" /></td>
                             ...
                             <td><input name="txtLabDipNo[]" id="txtLabDipNo_1" type="text"  class="text_boxes" style="width:50px" placeholder="Write" /></td>
                            ...
                            <td>
                                ...
                                ?>
                            <td><input readonly type="text" name="txtTotalqty[]" id="txtTotalqty_1" class="text_boxes_numeric" style="width:50px"  placeholder="" /><input type="hidden" name="hdnDtlsUpdateId[]" id="hdnDtlsUpdateId_1" class="text_boxes_numeric" style="width:50px"  readonly /><input type="hidden" name="hdnPiDtlsId[]" id="hdnPiDtlsId_1" class="text_boxes_numeric" style="width:50px"  readonly /></td>
                ...
                <table width="2380px" cellspacing="2" cellpadding="0" border="0">
                    <tr>
