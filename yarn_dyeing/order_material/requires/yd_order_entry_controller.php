...
if ($action == "create_sys_search_list_view")
{
    ...
    if ($supplier != 0) $supplier = "and a.supplier_id='$ex_data[3]'"; else  $supplier = "";
    if ($company != 0) $company = " and a.company_id='$ex_data[0]'"; else  $company = "";
    ...
    <div style="width:930px;">
        <table cellspacing="0" cellpadding="0" border="1" rules="all" width="890" class="rpt_table">
            <thead>
                ...
                <th width="160">Job No/Sales Order No</th>
        ...
        <div style="width:930px; overflow-y:scroll; max-height:300px;" id="buyer_list_view">
            ...
            foreach ($nameArray as $selectResult)
            {
                ...
                <tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
                    ...
                    <td width="160" style="word-break:break-all"> <? echo $selectResult[csf('sales_job')]; ?></td>
...
if ($action=="populate_job_master_from_data")
{
    ...
    $sql1=" select a.id, a.job_no_prefix_num, a.yd_job, a.order_no, a.pro_type, a.order_type, a.yd_type, a.yd_process, c.pi_number, sum(d.quantity) as  order_quantity, sum(d.amount) as  amount from yd_ord_mst a, yd_ord_dtls b, com_export_pi_mst c, com_export_pi_dtls d where a.id=b.mst_id and a.company_id=$company_id and a.yd_job='$advance_job' and c.pi_number='$tag_pi_no' and a.status_active=1 and a.is_deleted=0 and a.entry_form=374 and a.order_type=2 and a.check_box_advance=1 and a.yd_job=c.YD_JOB and c.id=d.pi_id and b.id=d.work_order_dtls_id group by a.id, a.job_no_prefix_num, a.yd_job, a.order_no, a.pro_type, a.order_type, a.yd_type, a.yd_process, c.pi_number order by id desc ";//$job_no_cond
    ...
    {   
       ...
       // echo "$('#txt_order_no').attr('disabled','true')".";\n";
        ...
        if($row[csf("advance_job")]!='')
        {
            ...
            echo "$('#cbo_yd_process').attr('disabled','true')".";\n";
        }

        $balance =$advance_qty-$pre_qty+$row[csf("order_quantity")];
        $balance_amount =$advance_amount-$pre_amount+$row[csf("amount")];

        echo "document.getElementById('txt_tag_pi_balance_qty').value     = '".number_format($balance,2,".","")."';\n";
        echo "document.getElementById('txt_tag_pi_amount_qty').value     = '".number_format($balance_amount,4,".","")."';\n";
    }
    exit(); 
}
...
if( $action=='order_dtls_list_view' ) 
{
    ...
    else
    {
        ...
        $sql1="select a.advance_job, a.tag_pi_no, b.pi_dtls_id, sum(b.order_quantity) as order_quantity, sum(b.amount) as amount from yd_ord_mst a, yd_ord_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and a.entry_form=374 and a.check_box_confirm=1 and a.tag_pi_no is not null and a.tag_pi_no='$tag_pi_no' and a.advance_job is not null and a.advance_job='$advance_job' group by a.advance_job, a.tag_pi_no, b.pi_dtls_id ";//$job_no_cond
        ...
        foreach($sql_pre_order_res as $data)
        {
            $pre_qty_arr[$data[csf('pi_dtls_id')]]['order_quantity'] += $data[csf('order_quantity')];

            $pre_qty_arr[$data[csf('pi_dtls_id')]]['amount'] += $data[csf('amount')];        }

        $pi_data_sql = "select b.id as pi_dtls_id, b.quantity, b.amount from com_export_pi_mst a, com_export_pi_dtls b where a.id=b.pi_id and a.pi_number='$tag_pi_no' and a.yd_job='$advance_job' and b.status_active=1 and b.is_deleted=0 and a.status_active=1 and a.is_deleted=0";

        $pi_data_res=sql_select($pi_data_sql);

        $pi_qty_arr = array();

        foreach($pi_data_res as $data)
        {
            $pi_qty_arr[$data[csf('pi_dtls_id')]]['order_quantity'] += $data[csf('quantity')];
            $pi_qty_arr[$data[csf('pi_dtls_id')]]['amount'] += $data[csf('amount')];
        }
    }
    //echo $sql;
    ...
    if(count($data_array) > 0)
    {
        foreach($data_array as $row)
        {
            ...
            else
            {
                ...
                $hdnPiDtlsId = $row[csf("pi_dtls_id")];

                $txtRemarks = $row[csf("remarks")];
				 $lab_dip_no = $row[csf("lab_dip_no")];

                $balance = $pi_qty_arr[$row[csf("pi_dtls_id")]]['order_quantity']-$pre_qty_arr[$row[csf("pi_dtls_id")]]['order_quantity']+$orderQty;

                $blance_amount = $pi_qty_arr[$row[csf("pi_dtls_id")]]['amount']-$pre_qty_arr[$row[csf("pi_dtls_id")]]['amount']+$amount;
            }
            $tblRow++;
            ...
            <tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
               ...
               <input type="hidden" name="txtprocess[]" id="txtprocess_<? echo $tblRow; ?>" class="text_boxes" style="width:80px;" value="<?php echo $process;?>" readonly />
                ...
                <td><input name="txtLabDipNo[]" id="txtLabDipNo_<? echo $tblRow; ?>" type="text"   class="text_boxes" style="width:50px" value="<? echo $lab_dip_no; ?>" /></td>
                <td><? echo create_drop_down( "cboUom_".$tblRow, 60, $unit_of_measurement,"", 1, "-- Select --",$uomId,"", 1,'','','','','','',"cboUom[]"); ?></td>
                <td><input name="txtOrderqty[]" id="txtOrderqty_<? echo $tblRow; ?>" type="text" style="width:50px"  class="text_boxes_numeric" value="<? echo  number_format($orderQty,2,".","");  ?>" onKeyUp="fnc_amount_cal(<?php echo $tblRow;?>);sum_total_qnty(<? echo $tblRow;?>);" <? echo $disabled; ?> /></td> 
                ...
                <td><input name="txtAmount[]" id="txtAmount_<? echo $tblRow; ?>" type="text" style="width:50px"  class="text_boxes_numeric" placeholder="<? echo  number_format($blance_amount,4,".","");    ?>"  value="<? echo  number_format($amount,4,".","");  ?>" readonly /></td> 
                <td><input name="txtProcessLoss[]" id="txtProcessLoss_<? echo $tblRow; ?>" type="text"  class="text_boxes_numeric" style="width:50px" placeholder="%" value="<? echo $processLoss; ?>" onKeyUp="fnc_amount_cal(<? echo $tblRow;?>);"  /></td>
                ...
                <td><input type="text" name="txtTotalqty[]" id="txtTotalqty_<? echo $tblRow; ?>" class="text_boxes_numeric" style="width:50px" value="<? echo number_format($totalQty,2,".","");   ?>" readonly />
                    ...
                    <input type="hidden" name="hdnPiDtlsId[]" id="hdnPiDtlsId_<? echo $tblRow; ?>" class="text_boxes_numeric" style="width:50px" value="<? echo $hdnPiDtlsId; ?>"  readonly />
                    <input type="hidden" name="hdnPiBalance[]" id="hdnPiBalance_<? echo $tblRow; ?>" class="text_boxes_numeric" style="width:50px"  value="<? echo $balance; ?>"  readonly />
                </td>
                <td>
                    <input type="text"  name="txtRemarks[]" id="txtRemarks_<? echo $tblRow; ?>" class="text_boxes"  placeholder="Browse" value="<?php echo $txtRemarks;?>" onClick="openmypage_remarks(<? echo $tblRow; ?>);" readonly/>
                </td>
                <td width="80">
    ...
    else
    {
        ...
        <tr id="row_1">
            ...
            <td><? echo create_drop_down( "cboUom_1", 60, $unit_of_measurement,"", 1, "-- Select --",12,"", "","2,1,12,23,15,52",'','','','','',"cboUom[]" ); ?></td>
            ...
            <td><input readonly type="text" name="txtTotalqty[]" id="txtTotalqty_1" class="text_boxes_numeric" style="width:50px"  placeholder="" /><input type="hidden" name="hdnDtlsUpdateId[]" id="hdnDtlsUpdateId_1" class="text_boxes_numeric" style="width:50px"  readonly /><input type="hidden" name="hdnPiDtlsId[]" id="hdnPiDtlsId_1" class="text_boxes_numeric" style="width:50px"  readonly />
            <input type="hidden" name="hdnPiBalance[]" id="hdnPiBalance_<? echo $tblRow; ?>" class="text_boxes_numeric" style="width:50px" value="<? echo $balance; ?>"  readonly /></td>
            ...
            <td>
                <input  type="text"  name="txtRemarks[]" id="txtRemarks_1" class="text_boxes"  placeholder="Browse" onClick="openmypage_remarks(1);" readonly/>
            </td>
            <td width="80">
...
if ($action=="save_update_delete")
{
        ...
        else
        {
            ...
            {  
               ...
               /* if (str_replace("'", "", trim($$txtItemColor)) != "") {
                ...
                $data_array2 .="(".$id1.",".$id.",'".$new_job_no[0]."','".$hid_order_id."','".$txt_order_no."',".$$txtstyleRef.",".$$txtsaleOrder.",".$$txtsaleOrderID.",".$$txtProductID.",".$$txtprocess.",".$$txtprocessname.",".$$txtlot.",".$$cboCount.",".$$cboYarnType.",".$$cboComposition.",".$$txtItemColor.",".$yd_color_id.",".$$txtCSP.",".$$txtnoBag.",".$$txtConeBag.",".$$txtNoCone.",".$$txtAVG.",".$$cboUom.",".str_replace(",",'',$$txtOrderqty).",".$$txtRate.",".str_replace(",",'',$$txtAmount).",".str_replace(",",'',$$txtProcessLoss).",".str_replace(",",'',$$txtTotalqty).",".str_replace(",",'',$$txtcounttype).",".str_replace(",",'',$$txtbuyerbuyer).",".str_replace(",",'',$$txtadjtype).",".str_replace(",",'',$$useFor).",".str_replace(",",'',$$appRef).",".str_replace(",",'',$$txtShade).",".str_replace(",",'',$$txtShadeId).",".str_replace(",",'',$$txtShadeMstId).",".str_replace(",",'',$$hdnPiDtlsId).",".str_replace(",",'',$$txtRemarks).",".str_replace(",",'',$$txtLabDipNo).",'".$user_id."','".$pc_date_time."')";
    ...
    else if ($operation==1)   // Update Here
    {
       ...
       $field_array2="order_id*order_no*style_ref*sales_order_no*sales_order_id*product_id*process_id*process_name*lot*count_id*yarn_type_id*yarn_composition_id*item_color_id*yd_color_id*csp*no_bag*cone_per_bag*no_cone*avg_wgt*uom*order_quantity*rate*amount*process_loss*total_order_quantity*count_type*buyer_buyer*adj_type*use_for*app_ref*shade*shade_dtls_id*shade_mst_id*pi_dtls_id*remarks*lab_dip_no*updated_by*update_date";
       ...
       $id1=return_next_id( "id", "yd_ord_dtls",1);
		...
		//	echo "10**".$detailsOrderqty;
            ...
            else
            {
                ...
                $data_array3 .="(".$id1.",".$update_id.",'".$txt_job_no."','".$hid_order_id."','".$txt_order_no."',".$$txtstyleRef.",".$$txtsaleOrder.",".$$txtsaleOrderID.",".$$txtProductID.",".$$txtprocess.",".$$txtprocessname.",".$$txtlot.",".$$cboCount.",".$$cboYarnType.",".$$cboComposition.",".$$txtItemColor.",".$yd_color_id.",".$$txtCSP.",".$$txtnoBag.",".$$txtConeBag.",".$$txtNoCone.",".$$txtAVG.",".$$cboUom.",".str_replace(",",'',$$txtOrderqty).",".$$txtRate.",".str_replace(",",'',$$txtAmount).",".str_replace(",",'',$$txtProcessLoss).",".str_replace(",",'',$$txtTotalqty).",".str_replace(",",'',$$txtcounttype).",".str_replace(",",'',$$txtbuyerbuyer).",".str_replace(",",'',$$txtadjtype).",".str_replace(",",'',$$useFor).",".str_replace(",",'',$$appRef).",".str_replace(",",'',$$txtShade).",".str_replace(",",'',$$txtShadeMstId).",".str_replace(",",'',$$txtShadeId).",".str_replace(",",'',$$hdnPiDtlsId).",".str_replace(",",'',$$txtRemarks).",".str_replace(",",'',$$txtLabDipNo).",'".$user_id."','".$pc_date_time."')";
...
if($action=="yarn_dyeing_order_entry_print")
{
    ...
    foreach ($qry_result_details as  $row) 
    {
		...
		$wo_arr[$row[csf("id")]]["lab_dip_no"] =$row[csf("lab_dip_no")];
		$wo_arr[$row[csf("id")]]["rate"] =$row[csf("rate")];
		$wo_arr[$row[csf("id")]]["amount"] =$row[csf("amount")];
		$wo_arr[$row[csf("id")]]["remarks"] =$row[csf("remarks")];
    }
    ...
    <div  style="width:100%">
        ...
        <div style="width:100%;">
            <table align="left" cellspacing="0" width="1400"  border="1" rules="all" class="rpt_table"  >
                ...
                $total_order=$total_yd_qty=$total_cone_qty=$total_bag_qty=$total_amount=0;
                ...
                foreach($wo_arr as $yd_details_id=> $yd_data)
                {
                    ...
                    <tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
                        ...
                        <td width="100" style="word-break:break-all"><?  echo $color_range[$wo_arr[$yd_details_id]["item_color_id"]] ; ?></td>
                         <td width="100" style="word-break:break-all"><?  echo $wo_arr[$yd_details_id]["lab_dip_no"] ; ?></td>  
                        <td width="70" style="word-break:break-all"><?  echo number_format($wo_arr[$yd_details_id]["order_quantity"],2,".",""); ?></td>
                          <td width="70" style="word-break:break-all"><?  echo number_format($wo_arr[$yd_details_id]["rate"],2,".",""); ?></td>
                            <td width="70" style="word-break:break-all"><?  echo number_format($wo_arr[$yd_details_id]["amount"],4,".",""); ?></td>
                        ...
                        <td width="30"  align="right"><?  echo $unit_of_measurement[$wo_arr[$yd_details_id]["uom"]] ; ?></td>
                        <td  align="right"><?  echo number_format($wo_arr[$yd_details_id]["no_cone"],0,".","");   ?></td>
                        <td   align="right"><?  echo number_format($wo_arr[$yd_details_id]["no_bag"],0,".","");   ?></td>
                        <td   align="right"><?  echo $wo_arr[$yd_details_id]["remarks"];   ?></td>
                    </tr>
                    <?
                    ...
                    $total_bag_qty += $wo_arr[$yd_details_id]["no_bag"];
					 $total_amount += $wo_arr[$yd_details_id]["amount"];
                    $tblRow++; 
                    }
                    ?>
                    <tr>
                        <td colspan="11" align="right">Total:</td>
                        <td align="right"><?php  echo number_format($total_order,2,".",""); //echo $total_order;?></td>
                        <td></td>
                       <td align="right"><?php  echo number_format($total_amount,4,".",""); //echo $total_order;?></td>
                        <td></td>
                        <td align="right"><?php  echo number_format($total_yd_qty,2,".",""); //echo $total_yd_qty;?></td>
                        <td></td>
                        <td align="right"><?php  echo number_format($total_cone_qty,0,".",""); //echo $total_cone_qty;?></td>
                        <td align="right"><?php  echo number_format($total_bag_qty,0,".","");?></td>
                    </tr>
                </table>
            </div>
        <br>
    ...
    echo signature_table(317, $data[0], "1100px",$cbo_template_id,50,$inserted_by);
...
if($action=="populate_shade_rate_from_data")
{
    ...
    else
    {
        ...
        echo "$('#txtItemColor_".$row_num."').attr('disabled',false);\n";
        die;
    }
    ...
    if(count($result)>0)
    {
        foreach($result as $data)
        {
            ...
            echo "document.getElementById('txtItemColor_".$row_num."').value         = '".$data[csf("color_range_id")]."';\n";
            echo "$('#txtRate_".$row_num."').attr('readonly','readonly');\n";
            echo "$('#txtItemColor_".$row_num."').attr('disabled',true);\n";
        }
    }
    else
    {
...
if ($action=="job_no_popup")
{
   ...
   // $sql="select a.id, a.job_no_prefix_num, a.yd_job, a.order_no, a.pro_type, a.order_type, a.yd_type, a.yd_process, c.pi_number, c.id as pi_id, sum(d.quantity) as order_quantity, sum(d.amount) as amount, c.item_category_id, c.currency_id from yd_ord_mst a, yd_ord_dtls b, com_export_pi_mst c, com_export_pi_dtls d where a.id=b.mst_id and a.company_id=$data[0] and a.status_active=1 and a.is_deleted=0 and a.entry_form=374 and a.order_type=2 and a.check_box_advance=1 and a.yd_job=c.YD_JOB and c.id=d.pi_id and b.id=d.work_order_dtls_id and c.within_group=$data[2] and c.buyer_id=$data[3] group by a.id, a.job_no_prefix_num, a.yd_job, a.order_no, a.pro_type, a.order_type, a.yd_type, a.yd_process, c.pi_number, c.id, c.item_category_id, c.currency_id order by id desc";//$job_no_cond

   // $sql1="select a.advance_job, a.tag_pi_no, sum(b.order_quantity) as order_quantity, sum(b.amount) as amount from yd_ord_mst a, yd_ord_dtls b where a.id=b.mst_id and a.company_id=$data[0] and a.within_group=$data[2] and a.party_id=$data[3] and a.status_active=1 and a.is_deleted=0 and a.entry_form=374 and a.check_box_confirm=1 and a.tag_pi_no is not null and a.advance_job is not null group by a.advance_job, a.tag_pi_no ";//$job_no_cond
	
	 $sql="select a.id, a.job_no_prefix_num, a.yd_job, a.order_no, a.pro_type, a.order_type, a.yd_type, a.yd_process, c.pi_number, c.id as pi_id, sum(d.quantity) as order_quantity, sum(d.amount) as amount, c.item_category_id, c.currency_id from yd_ord_mst a, yd_ord_dtls b, com_export_pi_mst c, com_export_pi_dtls d where a.id=b.mst_id and a.company_id=$data[0] and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0  and a.entry_form=374 and a.order_type=2 and a.check_box_advance=1 and a.yd_job=c.YD_JOB and c.id=d.pi_id and b.id=d.work_order_dtls_id and c.within_group=$data[2] and c.buyer_id=$data[3] group by a.id, a.job_no_prefix_num, a.yd_job, a.order_no, a.pro_type, a.order_type, a.yd_type, a.yd_process, c.pi_number, c.id, c.item_category_id, c.currency_id order by id desc";//$job_no_cond

    $sql1="select a.advance_job, a.tag_pi_no, sum(b.order_quantity) as order_quantity, sum(b.amount) as amount from yd_ord_mst a, yd_ord_dtls b where a.id=b.mst_id and a.company_id=$data[0] and a.within_group=$data[2] and a.party_id=$data[3] and a.status_active=1 and a.is_deleted=0  and  b.status_active=1 and b.is_deleted=0 and a.entry_form=374 and a.check_box_confirm=1 and a.tag_pi_no is not null and a.advance_job is not null group by a.advance_job, a.tag_pi_no ";//$job_no_cond
    ...
    foreach($sql_pre_order_res as $data)
    {
        $pre_qty_arr[$data[csf('advance_job')]][$data[csf('tag_pi_no')]]['order_quantity'] += $data[csf('order_quantity')];
        $pre_qty_arr[$data[csf('advance_job')]][$data[csf('tag_pi_no')]]['amount'] += $data[csf('amount')];
    }
    ...
    ?>
    <table cellpadding="0" cellspacing="0" border="1" rules="all" width="925" class="rpt_table">
        <thead>
            <tr>
                 <th width="30">SL</th>
    ...
    <div style="width:940px; max-height:350; overflow-y:scroll;">
        <table cellpadding="0" cellspacing="0" border="1" rules="all" width="925" class="rpt_table" id="list_view">         
            <?
            $i=1;
            foreach($sql_res as $row)
            {
                ...
                $balance = $row[csf('order_quantity')]-$pre_qty_arr[$row[csf('yd_job')]][$row[csf('pi_number')]]['order_quantity'];
                $amount = $row[csf('amount')]-$pre_qty_arr[$row[csf('yd_job')]][$row[csf('pi_number')]]['amount'];

                if($balance>0)
                {
                    ?>
                    <tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $i; ?>" onClick="js_set_value('<? echo $row[csf('id')].'_'.$row[csf('yd_job')].'_'.$row[csf('pi_number')].'_'.$balance.'_'.$row[csf('pi_id')].'_'.$row[csf('item_category_id')].'_'.$amount.'_'.$row[csf('pro_type')].'_'.$row[csf('order_type')].'_'.$row[csf('yd_type')].'_'.$row[csf('yd_process')].'_'.$row[csf('currency_id')];?>')" style="cursor:pointer" align="center">
                        ...
                        <td width="100" align="center"><? echo $yd_process_arr[$row[csf('yd_process')]]; ?></td>
                        <td width="100" align="right"><? echo number_format($row[csf('order_quantity')],4,'.',''); ?></td>
...
if( $action == 'pi_details' ) 
{
    ...
    $sql = "select b.id, b.work_order_id, b.work_order_no, b.cust_buyer, b.count_type_id, b.count_id, b.yarn_type_id, b.yarn_composition_id, b.color_range_id, b.color_id, b.app_ref, b.uom, b.total_order_qty, b.rate, b.amount, b.adj_type_id, b.quantity, d.style_ref, d.process_id, d.process_name, d.lot, d.use_for, d.csp, d.no_bag, d.cone_per_bag, d.no_cone, d.avg_wgt, d.process_loss, d.adj_type, d.total_order_quantity, d.shade, d.shade_dtls_id, d.shade_mst_id, d.product_id, b.remarks,d.lab_dip_no from com_export_pi_mst a, com_export_pi_dtls b, yd_ord_mst c, yd_ord_dtls d where a.id=b.pi_id and b.pi_id='$pi_id' and b.status_active=1 and b.is_deleted=0 and c.id=d.mst_id and a.yd_job=c.yd_job and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0 and b.work_order_dtls_id=d.id";

    $sql1="select a.advance_job, a.tag_pi_no, b.pi_dtls_id, sum(b.order_quantity) as order_quantity , sum(b.amount) as amount from yd_ord_mst a, yd_ord_dtls b where a.id=b.mst_id and a.company_id=$importer_id and a.status_active=1 and a.is_deleted=0 and a.entry_form=374 and a.check_box_confirm=1 and a.tag_pi_no is not null and a.tag_pi_no='$pi_no' and a.advance_job is not null group by a.advance_job, a.tag_pi_no, b.pi_dtls_id ";//$job_no_cond
    ...
    if($pi_data_arr!='')
    {
        ...
        {
            $styleRef=$row[csf("style_ref")];
            $saleOrderNo=$row[csf("work_order_no")];
            $saleOrderID=$row[csf("work_order_id")];
            $prodID=$row[csf("product_id")];
            $buyerBuyer = $row[csf("cust_buyer")];
            $processName = $row[csf("process_name")];
            $process=$row[csf("process_id")];
            $lot=$row[csf("lot")];
            $countType = $row[csf("count_type_id")];
            $count=$row[csf("count_id")];
            $yarnType_id = $row[csf("yarn_type_id")];
            $yarn_composition=$row[csf("yarn_composition_id")];
            $color_range_id = $row[csf("color_range_id")];
            $yarn_color=$row[csf("color_id")];
            $shade=$row[csf("shade")];
            $shadeId = $row[csf("shade_dtls_id")];
            $shadeMstId = $row[csf("shade_mst_id")];
            $appRef=$row[csf("app_ref")];
            ...
            $totalQty=$row[csf("total_order_quantity")];
            $hdnDtlsUpdateId='';
            $hdnPiDtlsId=$row[csf("id")]; 
			$lab_dip_no=$row[csf("lab_dip_no")];

            $balance_qty = $row[csf('quantity')]-$pre_qty_arr[$row[csf('id')]]['order_quantity'];
            $balance = $row[csf('quantity')];
            $blance_amount = $amount-$pre_qty_arr[$row[csf('id')]]['amount'];
            $balance_adj_total_qty = ($balance*$processLoss)/100;
            $balance_total_qty = $balance-$balance_adj_total_qty;
            $txtRemarks = $row[csf('remarks')];

        ?>
            <tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
                <td>
                    <input id="txtstyleRef_<? echo $tblRow; ?>" name="txtstyleRef[]" type="text"  class="text_boxes" style="width:80px" value="<? echo $styleRef; ?>"/>
                </td>

                <td>
                    <input id="txtsaleOrder_<? echo $tblRow; ?>" name="txtsaleOrder[]" type="text" value="<? echo $saleOrderNo; ?>" class="text_boxes" style="width:80px" placeholder=""/>
                    <input id="txtsaleOrderID_<? echo $tblRow; ?>" name="txtsaleOrderID[]" type="hidden" value="<? echo $saleOrderID; ?>" class="text_boxes_numeric" style="width:80px" placeholder=""/>
                    <input id="txtProductID_<? echo $tblRow; ?>" name="txtProductID[]" type="hidden" value="<? echo $prodID; ?>" class="text_boxes_numeric" style="width:80px" placeholder=""/>
                </td>
                ...
                <td>
                    <input id="txtlot_<? echo $tblRow; ?>" name="txtlot[]" type="text" class="text_boxes" style="width:80px" value="<?php echo $lot; ?>" />
                </td>
                ...
                <td id="count_td">
                   <?
                        $sql="select distinct(b.id) as id,b.yarn_count from lib_yarn_count b where b.status_active=1 and b.is_deleted=0";
                        echo create_drop_down( "cboCount_".$tblRow, 80, $sql,"id,yarn_count", 1, "-- Select --",$count,"",0,'','','','','','',"cboCount[]");
                    ?>
                </td>
                
                <td id="yarn_type_td">
                   <? echo create_drop_down( "cboYarnType_".$tblRow, 80, $yarn_type,"", 1, "-- Select --",$yarnType_id,"",0,'','','','','','',"cboYarnType[]"); ?>
                </td>

                <td align="center" id="composition_td">
                    <? echo create_drop_down( "cboComposition_".$tblRow, 80, $composition,"", 1, "-- Select --",$yarn_composition,"",0,'','','','','','',"cboComposition[]"); ?></td> 
                <td>
                
                    <? echo create_drop_down( "txtItemColor_".$tblRow, 90, $color_range,"", 1, "-- Select --",$color_range_id,"check_shade_rate(this.id);",0,'','','','','','',"txtItemColor[]"); ?>
                    
                    <input id="txtItemColorID_<? echo $tblRow; ?>" type="hidden"  name="txtItemColorID[]" class="text_boxes" style="width:70px" value="<? echo $color_range_id; ?>"/>
                </td>
                ...
                <td>
                    <input id="txtShade_<? echo $tblRow; ?>" type="text"  name="txtShade[]" class="text_boxes_numeric" placeholder="write Or Browse" onChange="set_shade_rate(this.id);" onDblClick="fnc_shade_popup(this.id);" style="width:70px" value="<? echo $shade; ?>"/>
                    <input name="txtShadeId[]" id="txtShadeId_<? echo $tblRow; ?>" type="hidden" class="text_boxes_numeric" style="width:50px" placeholder="write Or Browse" value="<? echo $shadeId; ?>"/>
                    <input name="txtShadeMstId[]" id="txtShadeMstId_<? echo $tblRow; ?>" type="hidden" class="text_boxes_numeric" style="width:50px" placeholder="write Or Browse" value="<? echo $shadeMstId; ?>"/>
                </td>

                <td>
                    <input id="txtAppRef_<? echo $tblRow; ?>" name="txtAppRef[]" type="text" class="text_boxes" style="width:70px" value="<? echo $appRef; ?>" placeholder=""/>
                </td>

                <td>
                    <input onFocus="add_auto_complete(this.id)" onBlur="fn_filed_check(this.id);use_for_copy_value(this.value,'txtUseFor_',this.id)" id="txtUseFor_<? echo $tblRow; ?>" name="txtUseFor[]" type="text" class="text_boxes" style="width:70px" value="<? echo $useFor; ?>" placeholder=""/>
                </td>

                <td>
                    <input id="txtCSP_<? echo $tblRow; ?>" name="txtCSP[]" type="text" class="text_boxes_numeric" style="width:70px" value="<? echo $csp; ?>" />
                </td>

                <td>
                    <input name="txtnoBag[]" id="txtnoBag_<? echo $tblRow; ?>" type="text" class="text_boxes_numeric" style="width:70px" value="<? echo $no_of_bag; ?>"/>
                </td>

                <td>
                    <input name="txtConeBag[]" id="txtConeBag_<? echo $tblRow; ?>" type="text" class="text_boxes_numeric" style="width:50px" value="<? echo $coneBag; ?>" />
                </td>

                <td>
                    <input name="txtNoCone[]" id="txtNoCone_<? echo $tblRow; ?>" type="text" class="text_boxes_numeric" style="width:50px" value="<? echo $no_of_cone; ?>"  />
                </td>

                <td>
                    <input name="txtAVG[]" id="txtAVG_<? echo $tblRow; ?>" type="text"   class="text_boxes_numeric" style="width:50px" value="<? echo $avg; ?>" />
                </td>
				 <td>
                    <input name="txtLabDipNo[]" id="txtLabDipNo_<? echo $tblRow; ?>" type="text"   class="text_boxes" style="width:50px" value="<? echo $lab_dip_no; ?>" />
                </td>
                <td>
                    <? echo create_drop_down( "cboUom_".$tblRow, 60, $unit_of_measurement,"", 1, "-- Select --",$uomId,"", 1,'','','','','','',"cboUom[]"); ?>
                        
                </td>

                <td>
                    <input name="txtOrderqty[]"  id="txtOrderqty_<? echo $tblRow; ?>" type="text" style="width:50px"  class="text_boxes_numeric" value="<? echo number_format($balance,2,".","");  ?>" onKeyUp="fnc_amount_cal(<?php echo $tblRow;?>);sum_total_qnty(<? echo $tblRow;?>);" />
                </td> 

                <td>
                    <input name="txtRate[]" id="txtRate_<? echo $tblRow; ?>" type="text" style="width:50px"  class="text_boxes_numeric" value="<? echo number_format($rate,4,".",""); ?>" onKeyUp="fnc_amount_cal(<?php echo $tblRow;?>)" />
                </td> 

                <td>
                    <input name="txtAmount[]" id="txtAmount_<? echo $tblRow; ?>" type="text" style="width:50px"  class="text_boxes_numeric" readonly placeholder="<? echo $blance_amount; ?>"  value="<? echo number_format($amount,4,".",""); ?>"  />
                </td> 

                <td>
                    <input name="txtProcessLoss[]"   id="txtProcessLoss_<? echo $tblRow; ?>" type="text"  class="text_boxes_numeric" style="width:50px" placeholder="%" value="<? echo $processLoss; ?>" onKeyUp="fnc_amount_cal(<? echo $tblRow;?>);"  /></td>
                ...
                <td>
                    <input  type="text" name="txtTotalqty[]" id="txtTotalqty_<? echo $tblRow; ?>" class="text_boxes_numeric" style="width:50px" value="<? echo number_format($balance_total_qty,2,".",""); ?>" readonly />
                    <input type="hidden" name="hdnDtlsUpdateId[]" id="hdnDtlsUpdateId_<? echo $tblRow; ?>" class="text_boxes_numeric" style="width:50px" value="<? echo $hdnDtlsUpdateId; ?>" readonly />
                    <input type="hidden" name="hdnPiDtlsId[]" id="hdnPiDtlsId_<? echo $tblRow; ?>" class="text_boxes_numeric" style="width:50px" value="<? echo $hdnPiDtlsId; ?>"  readonly />
                    <input type="hidden" name="hdnPiBalance[]" id="hdnPiBalance_<? echo $tblRow; ?>" class="text_boxes_numeric" style="width:50px" value="<? echo number_format($balance_qty,2,".","");  ?>"  readonly />
                </td>
...
if($action=="remarks_popup")
{
    ...
    <body>
    <div align="center">
        <fieldset style="width:340px;margin-left:4px;">
