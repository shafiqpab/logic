...
<script>
    ...
    function get_company_config(company_id) {
        load_drop_down('requires/yd_packing_and_delivery_controller', company_id, 'load_drop_down_location', 'location_td');
    }
    ...
    function openSalesOrderPopup() {
        ...
        // console.log(data);
        var pageLink = 'requires/yd_packing_and_delivery_controller.php?action=sales_no_popup&data='+data;
        ...
        salesPopup.onclose = function() {
            ...
            //get_php_form_data('1**'+prod_id_mst, 'populate_mst_data_from_search_popup', 'requires/yd_packing_and_delivery_controller');
            //alert (prod_id_mst);
            show_list_view('1**'+prod_id_mst, 'populate_dtls_data_from_search_popup', 'delivery_details_container', 'requires/yd_packing_and_delivery_controller', '');
        }
    }
    ...
    function salesPopupCloseHandler() {
        ...
        get_php_form_data(job_id_mst, 'populate_data_from_search_popup', 'requires/yd_packing_and_delivery_controller');
        // release_freezing();
    }
    ...
    function fnc_inspection(operation) {
        ...
        $("#tbl_dtls_yarn_dyeing tbody tr").each(function()
        {
            ...
            var txtPackConeQty      = $(this).find('input[name="txtPackConeQty[]"]').val();
        ...
        if(check_field==0){
            ...
            //alert (data); //return;
            freeze_window(operation);
            http.open("POST","requires/yd_packing_and_delivery_controller.php",true);
    ...
    function fnc_inspection_response() {
        if(http.readyState == 4) {
            ...
            if(response[0]==0 || response[0]==1) {
                ...
                show_list_view('2**'+response[2], 'populate_dtls_data_from_search_popup', 'delivery_details_container', 'requires/yd_packing_and_delivery_controller', '');
    ...
    function openmypage_system_id() {
        ...
        var data = document.getElementById('cbo_company_name').value;
        // console.log(data);
        var pageLink = 'requires/yd_packing_and_delivery_controller.php?action=system_id_popup&data='+data;
        ...
        deliveryPopup.onclose = function() {
            ...
            get_php_form_data('2**'+mst_id, 'populate_mst_data_from_search_popup', 'requires/yd_packing_and_delivery_controller');
            show_list_view('2**'+mst_id, 'populate_dtls_data_from_search_popup', 'delivery_details_container', 'requires/yd_packing_and_delivery_controller', '');
            set_button_status(1, permission, 'fnc_inspection', 1);
        }
    }

    function checkPackgQty(rowNum) {
        // console.log('check:' + rowNum);
    }
    ...
    function fnc_load_party(type, within_group) {
        ...
        if(within_group==1 && type==1) {
            load_drop_down( 'requires/yd_packing_and_delivery_controller', company+'_'+1, 'load_drop_down_buyer', 'buyer_td' );
        }
        else if(within_group==2 && type==1) {
            load_drop_down( 'requires/yd_packing_and_delivery_controller', company+'_'+2, 'load_drop_down_buyer', 'buyer_td' );
        }
        else if(within_group==1 && type==2) {
            load_drop_down( 'requires/yd_packing_and_delivery_controller', party_name+'_'+2, 'load_drop_down_location', 'party_location_td' );
        }
    }
...
<body onLoad="set_hotkey()">
  <div style="width:100%;" align="center">
    <? echo load_freeze_divs ("../../",$permission); ?>
            ...
            <legend>YD Inspection</legend>
                <table width="830" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td width="160"><? echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/yd_packing_and_delivery_controller', this.value+'_'+1, 'load_drop_down_location', 'location_td'); fnc_load_party(1,document.getElementById('cbo_within_group').value);"); ?>
                        </td>
        ...
        <fieldset style="width:850px;">
           <legend>YD Packing And Delivery Details</legend>
                <table width="850px" cellpadding="0" cellspacing="2" border="0" class="rpt_table" rules="all" id="tbl_dtls_yarn_dyeing">
                  <thead class="form_table_header">
                    ...
                    <th width="80" >Packing Cone Qty</th>
                    <th >Delivery Production QTY</th>
                ...
                <tbody id="delivery_details_container">
                    <tr>
                        ...
                        <td>
                            <input name="txtPackConeQty_1" id="txtPackConeQty_1" class="text_boxes" type="text"  placeholder="Display" readonly style="width:67px" />
                        </td>
                        <td>
