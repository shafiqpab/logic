...
if ($action=="load_drop_down_buyer")
{
	...
	if($data[1]==1)
	{
		...
		echo create_drop_down( "cbo_buyer_name", 152, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Party --", $data[2], "", 1);
		exit();
	}
	else
	{
...
if ($action=="load_room_rack_self_bin")
{
	...
	load_room_rack_self_bin("requires/yd_recipe_entry_controller",$data);
}
	...
	<div align="center" style="width:100%;">
		<form name="searchlabdipfrm" id="searchlabdipfrm">
			<fieldset style="width:885px;">
					...
					<thead>
						...
						<tr>
							...
							<th>
								...
								<input type="hidden" name="working_company_id" id="working_company_id" class="text_boxes" value="<? echo $cbo_company_id; ?>">
				...
				<div style="width:100%; margin-top:10px; margin-left:3px;" id="search_div" align="center"></div>
			</fieldset>
		</form>
	</div>
</body>
...
if ($action == "create_recipe_search_list_view") 
{
	...
	$batch_no_arr = return_library_array("select id, batch_number from yd_batch_mst where  status_active=1 and is_deleted=0", 'id', 'batch_number');
	$data = explode("_", $data);
	$labdip = $data[0];
	$sysid = $data[1];
	$start_date = $data[2];
	...
	if ($batch_no != "") 
	{
		$sql_batch=sql_select(" select id from yd_batch_mst where company_id=$company_id and batch_no='$batch_no' and status_active=1 and is_deleted=0");
	...
	$sql = "select id, labdip_no,batch_id,recipe_no, recipe_date, style_or_order, buyer_id, color_id, color_range from yd_recipe_entry_mst where entry_form=435 and status_active=1 and is_deleted=0 $lc_working_company_cond $labdip_cond $sysid_cond $rec_des_cond $date_cond $batch_cond order by id DESC";

	$sql_result=sql_select($sql);
  	$arr = array(2 => $batch_no_arr, 4 => $buyer_arr, 5 => $color_arr, 6 => $color_range);
 	echo create_list_view("tbl_list_search", "Recipe No,Labdip No,Batch No,Recipe Date,Buyer,Color,Color Range", "110,80,110,70,140,140,90", "800", "200", 0, $sql, "js_set_value", "id", "", 1, "0,0,batch_id,0,buyer_id,color_id,color_range", $arr, "recipe_no,labdip_no,batch_id,recipe_date,buyer_id,color_id,color_range", "", "", '0,0,0,3,0,0,0', '');
...
{
	//echo "select id, labdip_no, company_id, location_id, recipe_date, order_source, style_or_order, booking_id, total_liquor, batch_ratio, liquor_ratio, remarks, buyer_id, color_id, color_range from yd_recipe_entry_mst where id='$data'";
	$data_array = sql_select("select id, entry_form, labdip_no, company_id,recipe_no, location_id, batch_id,recipe_date, color_id, buyer_id, color_range, total_liquor, batch_ratio, liquor_ratio,batch_qty, composition,lot,count,yarn_type_id ,remarks from yd_recipe_entry_mst where id='$data'");

	foreach ($data_array as $row) 
	{
		echo "document.getElementById('txt_sys_id').value 					= '" . $row[csf("id")] . "';\n";
		echo "document.getElementById('txt_sys_number').value 					= '" . $row[csf("recipe_no")] . "';\n";
		...
		//echo "load_drop_down('requires/yd_recipe_entry_controller', ".$row[csf('company_id')].", 'load_drop_down_location', 'location_td' );\n";
		//echo "load_drop_down('requires/yd_recipe_entry_controller', ".$row[csf('company_id')].", 'load_drop_down_buyer', 'buyer_td_id' );\n";
		echo "load_drop_down('requires/yd_recipe_entry_controller', '" . $row[csf("company_id")] . "', 'load_drop_down_location', 'location_td' );\n";
		...
		echo "document.getElementById('txt_comosition').value 				= '" . $row[csf("txt_comosition")] . "';\n";
		echo "document.getElementById('txt_yarn_lot').value 				= '" . $row[csf("lot")] . "';\n";
		echo "document.getElementById('txt_count').value 				= '" . $row[csf("count")] . "';\n";
		...
		echo "document.getElementById('print2').style.display= '';\n";
		//$order_source=$row[csf("order_source")];
		echo "get_php_form_data(" . $row[csf("company_id")] . "+'**'+" . $row[csf("batch_id")] . "+'**'+5, 'load_data_from_batch', 'requires/yd_recipe_entry_controller');\n";
		if ($row[csf("batch_qty")] != 0) 
		{
			echo "document.getElementById('txt_batch_weight').value 				= '" . $row[csf("batch_qty")] . "';\n";
		}
         echo "document.getElementById('txt_trims_weight').value 				= '" . $row[csf("batch_qty")] . "';\n";
		//echo "select id,sub_process_id,process_remark from yd_recipe_entry_dtls where mst_id='".$row[csf("id")]."' ";
		$sql_rec_dtls = sql_select("select id,sub_process_id,process_remark,liquor_ratio,total_liquor from yd_recipe_entry_dtls where mst_id=" . $row[csf("id")] . " ");
			...
			<fieldset style="width:100%;">
				...
				<table cellpadding="0" cellspacing="0" width="750" class="rpt_table" border="1" rules="all">
					...
					<tr>
						...
						<td align="center">
							<input type="button" name="button2" class="formbutton" value="Show"
							onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+document.getElementById('txt_company_id').value+'_'+document.getElementById('cbo_buyer_name').value+'_'+'<? echo $batch_against; ?>', 'create_booking_search_list_view', 'search_div', 'yd_recipe_entry_controller', 'setFilterGrid(\'tbl_list_search\',-1);')"
							style="width:100px;"/>
						</td>
					</tr>
				</table>
...
if ($action == "create_booking_search_list_view") {
    ...
    if ($action == "recipe_item_details") { 
    	...
    	$sql = "select id, sub_process_id as sub_process_id,process_remark, store_id from yd_recipe_entry_dtls where mst_id='$data' and status_active=1 and is_deleted=0 order by id";
				...
				<form name="searchorderfrm_1" id="searchorderfrm_1" autocomplete="off">
					<table cellpadding="0" cellspacing="0" border="1" rules="all" width="500" class="rpt_table">
						...
						<tr class="general">
							...
							<td align="center">
								<input type="button" name="button2" class="formbutton" value="Show"
								onClick="show_list_view ( document.getElementById('txt_search_common').value+'_'+document.getElementById('cbo_search_by').value+'_'+'<? echo $cbo_company_id; ?>'+'_'+document.getElementById('cbo_string_search_type').value, 'create_batch_search_list_view', 'search_div', 'yd_recipe_entry_controller', 'setFilterGrid(\'list_view\',-1);')"
								style="width:100px;"/>
							</td>
						</tr>
					</table>
...
if ($action == "create_batch_search_list_view") 
{
	...
	if ($search_type == 1) 
	{
		if ($search_common != '') $batch_cond = " and a.batch_number='$search_common'"; else $batch_cond = "";
	} 
	else if ($search_type == 4 || $search_type == 0) 
	{
		if ($search_common != '') $batch_cond = " and a.batch_number like '%$search_common%'"; else $batch_cond = "";
	} 
	else if ($search_type == 2)
	 {
		if ($search_common != '') $batch_cond = " and a.batch_number like '$search_common%'"; else $batch_cond = "";
	...
	/*$sql = "select a.id,a.yd_job_id,a.order_id,a.order_no,a.booking_without_order,a.booking_type,a.yd_batch_id,a.company_id,a.location_id,a.batch_color_id,a.batch_color_range,a.batch_number,a.batch_against,a.batch_weight,a.extention_no,a.duration_req,a.process_id,a.machine_id,a.batch_date,a.remarks,b.id as batch_dtls_id,listagg(c.lot,',') within group (order by c.lot) as lot, c.count_id, c.yarn_type_id, c.yarn_composition_id
            from yd_batch_mst a ,yd_batch_dtls b, yd_ord_dtls c where a.id=b.mst_id and b.yd_job_dtls_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $batch_cond group by a.id,a.yd_job_id,a.order_id,a.order_no,a.booking_without_order,a.booking_type,a.yd_batch_id,a.company_id,a.location_id,a.batch_color_id,a.batch_color_range,a.batch_number,a.batch_against,a.batch_weight,a.extention_no,a.duration_req,a.process_id,a.machine_id,a.batch_date,a.remarks,b.id, c.count_id, c.yarn_type_id, c.yarn_composition_id order by a.id DESC"; */
			
			
			  $sql = "select a.id,a.yd_job_id,a.order_id,a.yd_batch_id,a.batch_color_id,a.batch_color_range,a.batch_number,sum(b.quantity) as batch_quantity ,a.batch_date,listagg(b.id,',') within group (order by b.id) as batch_dtls_id,listagg(c.lot,',') within group (order by c.lot) as lot,listagg(c.count_id,',') within group (order by c.count_id) as count_id,listagg(c.yarn_type_id,',') within group (order by c.yarn_type_id) as yarn_type_id,listagg(c.yarn_composition_id,',') within group (order by c.yarn_composition_id) as yarn_composition_id,a.batch_weight 
            from yd_batch_mst a ,yd_batch_dtls b, yd_ord_dtls c where a.id=b.mst_id and b.yd_job_dtls_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $batch_cond group by a.id,a.yd_job_id,a.order_id,a.yd_batch_id,a.batch_color_id,a.batch_color_range,a.batch_number,a.batch_date,a.batch_weight order by a.id DESC"; 
				...
				foreach ($nameArray as $selectResult)
				 {
					if ($i % 2 == 0) $bgcolor = "#E9F3FF"; else $bgcolor = "#FFFFFF";
 					...
 					$batch_data=$selectResult[csf('id')].'_'.$search_by.'_'.$selectResult[csf('batch_dtls_id')].'_'.$selectResult[csf('batch_quantity')].'_'.$selectResult[csf('batch_weight')];
					?>
						<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none; cursor:pointer"
							id="search<? echo $i; ?>" onClick="js_set_value('<? echo $batch_data; ?>')">
							<td width="30"><? echo $i; ?></td>
							<td width="120"><p><? echo $selectResult[csf('batch_number')]; ?></p></td>
							<td width="100"><p><? echo $color_arr[$selectResult[csf('batch_color_id')]]; ?></p></td>
							<td width="100" align="center"><p><? echo change_date_format($selectResult[csf('batch_date')]); ?></p></td>
							<td width="100"><p><? echo $selectResult[csf('lot')]; ?></p></td>
							<td><p><? echo $selectResult[csf('batch_quantity')]; ?></p></td> 
						</tr>
						<?
						$i++;
 					}
	...
	if ($action == "ratio_data_from_dtls")
	 {
		...
		$update_id = $ex_data[2];
		if ($sub_process == 93 || $sub_process == 94 || $sub_process == 95 || $sub_process == 96 || $sub_process == 97 || $sub_process == 98) {
			$sql_rec_dtls = "select id,sub_process_id,process_remark,liquor_ratio,total_liquor from yd_recipe_entry_dtls where mst_id=" . $update_id . " and sub_process_id=$sub_process ";
		} else {
			$sql_rec_dtls = "select id,sub_process_id,process_remark,liquor_ratio,total_liquor from yd_recipe_entry_dtls where mst_id=" . $update_id . " and sub_process_id=$sub_process and ratio>0";
		}
		//echo $sql_rec_dtls;
		...
		if($update_id!='')
		{
		$sub_seq_no =return_field_value("max(a.sub_seq) as sub_seq","yd_recipe_entry_dtls a"," a.mst_id=" . $update_id . " and a.ratio>0 and a.status_active=1 and a.is_deleted=0", "sub_seq");
		}
		$sub_seq=$sub_seq_no+1;
		echo "document.getElementById('txt_subprocess_seq').value 			= '" . $sub_seq . "';\n";
		
		$sub_seq_no =return_field_value("a.sub_seq as sub_seq","yd_recipe_entry_dtls a"," a.mst_id=" . $update_id . " and a.ratio>0 and a.sub_process_id=$sub_process and a.status_active=1 and a.is_deleted=0", "sub_seq");
		if($sub_seq_no!="")
		{
...
$sql_recipe="select recipe_date from yd_recipe_entry_mst where  entry_form=435 and  recipe_date>='$from_date' and status_active=1";
	//echo $sql_recipe;die;
	...
	if ($action == "load_data_from_batch") 
	{
		...
		$batch_cat=$ex_data[2];
		  ...
		  $sql = "select a.id,a.yd_job_id,a.order_id,a.order_no,a.booking_without_order,a.booking_type,a.yd_batch_id,a.company_id,a.location_id,a.batch_color_id,a.batch_color_range,a.batch_number,a.batch_against,a.batch_weight,a.extention_no,a.duration_req,a.process_id,a.machine_id,a.batch_date,a.remarks,b.id as batch_dtls_id, c.lot, c.count_id, c.yarn_type_id, c.yarn_composition_id,d.party_id,d.within_group,b.quantity
            from yd_batch_mst a ,yd_batch_dtls b, yd_ord_dtls c ,yd_ord_mst d  where a.id=b.mst_id and b.yd_job_dtls_id=c.id and  d.id=c.mst_id    and a.id=$batch_id  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0   order by a.id DESC";
		...
		foreach ($result_sql as $row) 
		{
			...
			$tot_batch_wgtTrim=$row[csf("batch_weight")];
			//echo "document.getElementById('cbo_order_source').value 		= '" . $batch_cat . "';\n";
			echo "document.getElementById('txt_batch_no').value 			= '" . $row[csf("batch_number")] . "';\n";
			...
			echo "load_drop_down( 'requires/yd_recipe_entry_controller', '".$row[csf("company_id")]."'+'_'+'".$row[csf("within_group")]."', 'load_drop_down_buyer', 'buyer_td' );\n"; 

			echo "document.getElementById('cbo_buyer_name').value 			= '" . $row[csf("party_id")]. "';\n";
			echo "document.getElementById('txt_color').value 				= '" . $color_arr[$row[csf("batch_color_id")]] . "';\n";
			echo "document.getElementById('txt_color_id').value 			= '" . $row[csf("batch_color_id")] . "';\n";
			echo "document.getElementById('cbo_color_range').value 			= '" . $row[csf("batch_color_range")] . "';\n";
			echo "document.getElementById('txt_yarn_lot').value 			= '" . $row[csf("lot")] . "';\n";
			echo "document.getElementById('txt_count').value 				= '" . $count_arr[$row[csf("count_id")]] . "';\n";
			echo "document.getElementById('cbo_yarn_type').value 			= '" . $row[csf("yarn_type_id")] . "';\n";
			echo "document.getElementById('txt_comosition').value 			= '" . $comp_arr[$row[csf("yarn_composition_id")]] . "';\n";
			//echo "document.getElementById('txt_trims_weight').value 		= '" . $row[csf("quantity")] . "';\n";
			
			/*if($batch_cat!=4)
			{
				echo "get_php_form_data('" . $po_id . "'+'**'+'" . $prod_id . "'+'**'+'" . $booking_no . "'+'**'+'" . $booking_without_order . "'+'**'+'" . $ex_data[1] . "'+'**'+'" . $row[csf("company_id")] . "', 'lode_data_from_grey_production', 'requires/yd_recipe_entry_controller');\n";
			}
			echo "caculate_tot_liquor();\n";*/
		}
		exit();
...
if($action=="item_details")
{
	...
	{	//sum(b.req_qny_edit) as qnty
		...
		$recipeData=sql_select("select prod_id, id, item_lot, comments, dose_base, ratio, seq_no, store_id, item_lot from yd_recipe_entry_dtls where mst_id=$update_id and sub_process_id=$sub_process_id and status_active=1 and is_deleted=0 $ration_cond order by seq_no");
	...
	where a.id=b.prod_id and a.company_id='$company_id' and b.store_id=$store_id and a.item_category_id in(5,6,7) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $stock_check";
	
	/*if(!empty($recepi_prod_id))
	{
		$sql.=" and a.id in(".implode(",",$recepi_prod_id).")";
	}*/
	...
	?>
	<div>
		<table cellspacing="0" cellpadding="0" rules="all" width="1050" class="rpt_table" >
			<tbody>
				<tr>
					<td align="center">
						<? echo create_drop_down( "cbo_string_search_type", 150, $string_search_type,'', 1, "-- Searching Type --",0 );
						?>
					</td>
				</tr>
			</tbody>
						...
						foreach($recipe_prod_id_arr as $prodId)
						{
							...
							//{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>">
									...
									<td  width="100" id="sub_process_<? echo $i; ?>"><p><? echo $dyeing_sub_process[$sub_process_id]; ?></p><input type="hidden" name="updateIdDtls[]" id="updateIdDtls_<? echo $i; ?>" value="<? echo $dtls_id; ?>"></td>
									<td width="50" align="center" id="product_id_<? echo $i; ?>"><? echo $prodId_ref[0]; ?><input type="hidden" name="txt_prod_id[]" id="txt_prod_id_<? echo $i; ?>" value="<? echo $prodId_ref[0]; ?>"></td>
									<td align="right" width="70" title="<? echo 'Stock Qty Allowed 12 Digit after decimal='.$store_stock; ?>" id="stock_qty_<? echo $i; ?>"><? echo number_format($store_stock,3,'.',''); ?></td>
									<td width="" align="center" id="comments_<? echo $i; ?>"><input type="text" name="txt_comments[]" id="txt_comments_<? echo $i; ?>" class="text_boxes" style="width:80px" value="<? echo $comments; ?>"></td>
								</tr>
								<?
							//$max_seq_no[]=$selectResult[csf('seq_no')];
						...
						if(!in_array($prodId,$recipe_prod_id_arr))
						{
							...
							//{
								...
								<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>">
									...
									<td  width="100" id="sub_process_<? echo $i; ?>"><p><? echo $dyeing_sub_process[$sub_process_id]; ?></p><input type="hidden" name="updateIdDtls[]" id="updateIdDtls_<? echo $i; ?>" value="<? //echo $dtls_id; ?>"></td>
									<td width="50" align="center" id="product_id_<? echo $i; ?>"><? echo $prodId_ref[0]; ?><input type="hidden" name="txt_prod_id[]" id="txt_prod_id_<? echo $i; ?>" value="<? echo $prodId_ref[0]; ?>"></td>
									<td align="right" width="70" title="<? echo "current stock=".$prodData[6]."store stock=".$store_stock  ?>" id="stock_qty_<? echo $i; ?>"><? echo number_format($store_stock,3,'.',''); ?></td>
									<td width="" align="center" id="comments_<? echo $i; ?>"><input type="text" name="txt_comments[]" id="txt_comments_<? echo $i; ?>" class="text_boxes" style="width:80px" value="<? echo $comments; ?>"></td>
								</tr>
								<?
							//$max_seq_no[]=$selectResult[csf('seq_no')];
...
if($action=="populate_stock_data")
{
	...
	//echo "select is_control, id from variable_settings_production where company_name = $data and variable_list = 54 and is_deleted = 0 and status_active = 1";
...
if ($action == "item_details_old") {
	...
	if ($update_id != "") {    //sum(b.req_qny_edit) as qnty
		...
		//echo "select prod_id, id, item_lot,comments,dose_base, ratio, seq_no from yd_recipe_entry_dtls where mst_id=$update_id and sub_process_id=$sub_process_id and status_active=1 and is_deleted=0 and ratio>0 order by seq_no";


		//echo "select prod_id, id, item_lot,comments,dose_base, ratio, seq_no from yd_recipe_entry_dtls where mst_id=$update_id and sub_process_id=$sub_process_id and status_active=1 and is_deleted=0 $ration_cond order by seq_no";
		$recipeData = sql_select("select prod_id, id, item_lot,comments,dose_base, ratio, seq_no from yd_recipe_entry_dtls where mst_id=$update_id and sub_process_id=$sub_process_id and status_active=1 and is_deleted=0 $ration_cond order by seq_no");
				...
				} else                //Wash End....
				{
					...
					if (count($recipe_prod_id_arr) > 0) {
						...
						<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>">
							...
							<td width="70" id="sub_group_name_<? echo $i; ?>"><p><? echo $sub_group_name; ?>&nbsp;</p></td>
							<td width="130" id="item_description_<? echo $i; ?>"><p><? echo $item_description . " " . $item_size; ?></p></td>
							<td width="80" id="item_lot_<? echo $i; ?>"><input type="text" name="txt_item_lot[]" id="txt_item_lot_<? echo $i; ?>" class="text_boxes" style="width:68px" onDblClick="openmypage_itemLot(<? echo $i; ?>)" placeholder="Browse" value="<? echo $selectResult[csf('item_lot')]; ?>"> </td>
							<td width="40" align="center" id="uom_<? echo $i; ?>"><? echo $unit_of_measurement[$unit_of_measure]; ?> &nbsp;</td>
							<td width="70" align="center"
							id="dose_base_<? echo $i; ?>"><? echo create_drop_down("cbo_dose_base_$i", 68, $dose_base, "", 1, "-Select Dose Base-", $selected_dose); ?></td>
							...
							<td width="40" align="center" id="seqno_<? echo $i; ?>"><input type="text" name="txt_seqno[]" id="txt_seqno_<? echo $i; ?>" class="text_boxes_numeric" style="width:30px" value="<? echo $seq_no; ?>" onBlur="row_sequence(<? echo $i; ?>);">
							</td>
							<td width="100" id="sub_process_<? echo $i; ?>"> <p><? echo $dyeing_sub_process[$sub_process_id]; ?></p><input type="hidden" name="updateIdDtls[]" id="updateIdDtls_<? echo $i; ?>"  value="<? //echo $dtls_id; ?>"></td>
                            ...
                            <td align="right" width="70" title="<? echo $copy_val; ?>" id="stock_qty_<? echo $i; ?>"><? echo number_format($current_stock, 2, '.', ''); ?></td>
                            <td width="" align="center" id="comments_<? echo $i; ?>"><input type="text" name="txt_comments[]" id="txt_comments_<? echo $i; ?>" class="text_boxes" style="width:80px" value="<? echo $comments; ?>"></td>
                      	</tr>
                      	<?
							//$max_seq_no[]=$selectResult[csf('seq_no')];
...
if ($action == "itemLot_popup")
{
	...
	echo create_list_view("list_view", "Item Lot", "200", "330", "250", 0, $sql, "js_set_value", "batch_lot", "", 1, "", 0, "batch_lot", "yd_recipe_entry_controller", 'setFilterGrid("list_view",-1);', '0', '', 1);
	die;
}
...
if ($action == "recipe_entry_print_2") 
{
	...
	foreach ($sales_order_result as $sales_row) {
		...
		$sql = "select a.id, a.batch_number as batch_no,a.batch_against,a.entry_form, a.batch_weight as total_trims_weight, listagg(b.yd_job_dtls_id,',') within group (order by b.yd_job_dtls_id) as po_id, listagg(b.product_id,',') within group (order by b.product_id) as prod_id,  sum(b.quantity) as batch_qnty,d.within_group  from yd_batch_mst a, yd_batch_dtls b, yd_ord_dtls c ,yd_ord_mst d where a.id=b.mst_id and a.company_id='$data[0]' and b.yd_job_dtls_id=c.id and  d.id=c.mst_id  and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id,a.batch_against, a.batch_number,a.entry_form,a.batch_weight,d.within_group order by a.id DESC";
	...
	$sql_recipe_mst = "select id, entry_form, labdip_no, company_id, location_id, batch_id,recipe_date, color_id, buyer_id, color_range, total_liquor, batch_ratio, liquor_ratio,batch_qty, composition,lot,count,yarn_type_id ,remarks from yd_recipe_entry_mst where id='$data[1]'";
	...
	$within_group=$batch_array[$dataArray[0][csf('batch_id')]]['within_group'];
	$order_array = return_library_array("select b.id, b.order_no from subcon_ord_dtls b,pro_batch_create_dtls c where c.po_id=b.id and c.mst_id='$batch_id' ", "id", "order_no");
	...
	// echo $dataArray[0][csf('batch_id')];die;
	?>
	<div style="width:930px; font-size:9px">
		<table width="930" cellspacing="0" align="right" border="0">
			<tr>
				<td colspan="6" align="center" style="font-size:x-large">
					<strong><? echo 'Working Company : '.$company_library[$company_id]; ?></strong>
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td width="175"><? echo $data[3]; //$dataArray[0][csf('id')]; ?></td>
            ...
            <tr>
            	...
            	<td><? if ($batch_array[$dataArray[0][csf('batch_id')]]['batch_against'] == 3) echo "Sample Without Order"; else echo $batch_array[$dataArray[0][csf('batch_id')]]['order']; ?>
				</td>
        ...
        <table width="930" cellspacing="0"  class="rpt_table" border="1" style="font-size:20px;" rules="all">
			<thead bgcolor="#dddddd" align="center">
					...
					<tr bgcolor="#CCCCFF">
						<th colspan="8" align="center"><strong>Yarn Details</strong></th>
					</tr>
					<tr>
						<th width="30" height="40">SL</th>
						<th width="80" height="40">Style</th>
						<th width="100" height="40">Job no/ Sales Order No</th>
						<th width="70" height="40">Lot</th>
						<th width="70" height="40">Count</th>
						<th width="70" height="40">Yarn Type</th>
						<th width="70" height="40">Yarn Composition</th>
						<th width="70" height="40">Yarn Color</th>
					</tr>
			</thead>
			<tbody>
				<?
				...
				{
					$batch_sql = "SELECT a.id,b.id as job_dtls_id, a.order_id, a.bobbin_type, a.winding_pckg_qty, a.quantity, b.sales_order_no, b.style_ref, b.lot,b.count_id, b.yarn_type_id, b.yarn_composition_id,b.yd_color_id
					FROM yd_batch_dtls a, yd_ord_dtls b
					WHERE a.mst_id=$b_id and a.status_active=1 and a.yd_job_dtls_id=b.id and a.is_deleted=0";
					
					$result_batch_query = sql_select($batch_sql);
					foreach ($result_batch_query as $row)
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>">
							...
							<td align="center" height="40"><b><?php echo $row[csf('style_ref')]; ?></b></td>
							<td align="center" height="40"><b><?php echo $row[csf('sales_order_no')]; ?></b></td>
							<td align="center" height="40"><?php echo $row[csf('lot')]; ?></td>
							<td align="center" height="40"><?php echo $count_arr[$row[csf('count_id')]]; ?></td>
							<td align="center" height="40"><?php echo $yarn_type[$row[csf('yarn_type_id')]]; ?></td>
							<td align="center" height="40"><?php echo $comp_arr[$row[csf('yarn_composition_id')]]; ?></td>
							<td align="center" height="40"><?php echo $color_arr[$row[csf('yd_color_id')]]; ?></td>
						</tr>
						<?
						$j++;
					}
        ...
        <div style="width:100%;">
        	<table  cellspacing="0" width="1300" border="1" rules="all" class="rpt_table" style="font-size:20px;">
        		...
        		$com_id = $data[0];
        			...
        			$sql_ratio = "select id, sub_process_id as sub_process_id,process_remark,total_liquor,liquor_ratio from yd_recipe_entry_dtls where mst_id=$mst_id and status_active=1 and is_deleted=0 order by id";
...
if ($action == "save_update_delete") {
	...
	if ($operation == 0)  // Insert Here
	{
		...
		if(str_replace("'","",$txt_color)!="")
		{
			...
			{
				$color_id = return_id( str_replace("'","",$txt_color), $color_arr, "lib_color", "id,color_name","435");
		...
		{
			$recipe_serial_no =return_field_value("max(a.recipe_serial_no) as recipe_serial_no","yd_recipe_entry_mst a"," a.batch_id=" . $batch_id . " and a.entry_form=435 and a.status_active=1 and a.is_deleted=0", "recipe_serial_no");
		...
		else
		{
 		...
 		if (str_replace("'", "", $update_id) == "") {
			/*if(is_duplicate_field( "labdip_no", "yd_recipe_entry_mst", "labdip_no=$txt_labdip_no" )==1)
			{
				echo "11**0";
				die;
			}*/
			...
			if($db_type==0)
			{
				$new_issue_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id),'', 'YDR' , date("Y",time()), 5, "select id,recipe_no_prefix,recipe_no_prefix_num from yd_recipe_entry_mst where company_id=$cbo_company_id and entry_form=435 and YEAR(insert_date)=".date('Y',time())." order by id desc ", "recipe_no_prefix", "recipe_no_prefix_num" ));
			}
			else if($db_type==2)
			{
				$new_issue_no=explode("*",return_mrr_number( str_replace("'","",$cbo_company_id),'', 'YDR' , date("Y",time()), 5, "select id,recipe_no_prefix,recipe_no_prefix_num from yd_recipe_entry_mst where company_id=$cbo_company_id and entry_form=435 and TO_CHAR(insert_date,'YYYY')=".date('Y',time())." order by id desc ", "recipe_no_prefix", "recipe_no_prefix_num" ));
			}
			$id = return_next_id("id", "yd_recipe_entry_mst", 1);
			...
			$field_array = "id, entry_form, recipe_no_prefix ,recipe_no_prefix_num , recipe_no, labdip_no, company_id, location_id, batch_id,recipe_date, color_id, buyer_id, color_range, total_liquor, batch_ratio, liquor_ratio,batch_qty, composition,lot,count,yarn_type_id ,remarks, inserted_by, insert_date";
			//echo $txt_liquor;  txt_comosition txt_yarn_lot txt_count cbo_yarn_type
			$data_array = "(".$id.",'435','".$new_issue_no[1]."','".$new_issue_no[2]."','".$new_issue_no[0]."'," . $txt_labdip_no . "," . $cbo_company_id . "," . $cbo_location . "," . $txt_batch_id . "," . $txt_recipe_date . ",'" . $color_id . "'," . $cbo_buyer_name . "," . $cbo_color_range . "," . $txt_liquor . "," . $txt_batch_ratio . "," . $txt_liquor_ratio . "," . $txt_batch_weight . "," . $txt_comosition . "," . $txt_yarn_lot . "," . $txt_count . "," . $cbo_yarn_type . "," . $txt_remarks . "," . $_SESSION['logic_erp']['user_id'] . ",'" . $pc_date_time . "')";

			//$rID=sql_insert("yd_recipe_entry_mst",$field_array,$data_array,0);
			//if($rID) $flag=1; else $flag=0;
			$recipe_update_id = $id;
		} else {
  
			if (is_duplicate_field("sub_process_id", "yd_recipe_entry_dtls", "mst_id=$update_id and sub_process_id=$cbo_sub_process and status_active=1 and is_deleted=0") == 1) {
				echo "11**0";
				die;
			}

			$field_array_update = "labdip_no*company_id*location_id*batch_id*recipe_date*color_id*buyer_id*color_range*total_liquor*batch_ratio*liquor_ratio*batch_qty*composition*lot*count*yarn_type_id *remarks*updated_by*update_date";

			$data_array_update = $txt_labdip_no . "*" . $cbo_company_id . "*" . $cbo_location . "*" . $txt_batch_id . "*" . $txt_recipe_date . "*" . $color_id . "*" . $cbo_buyer_name . "*" . $cbo_color_range . "*" . $txt_liquor . "*" . $txt_batch_ratio . "*" . $txt_liquor_ratio . "*" . $txt_batch_weight . "*" . $txt_comosition . "*" . $txt_yarn_lot . "*" . $txt_count . "*" . $cbo_yarn_type . "*" . $txt_remarks . "*" . $_SESSION['logic_erp']['user_id'] . "*'" . $pc_date_time . "'";

			//$rID=sql_update("yd_recipe_entry_mst",$field_array_update,$data_array_update,"id",$update_id,1);
			//if($rID) $flag=1; else $flag=0;
		...
		$dtls_id = return_next_id("id", "yd_recipe_entry_dtls", 1);
		...
		$rID2 = sql_insert("yd_recipe_entry_dtls", $field_array_dtls, $data_array_dtls, 1);
		if ($flag == 1) {
		...
		if ($db_type == 0) {
			if ($flag == 1) {
				mysql_query("COMMIT");
				echo "0**" . $recipe_update_id . "**" . $subprocess. "**" . $recipe_serial_no. "**" . str_replace("'", '', $new_issue_no[0]); 
			} else {
	...
	else if ($operation == 1)   // Update Here
	{
		...
		/*if(is_duplicate_field( "labdip_no", "yd_recipe_entry_mst", "labdip_no=$txt_labdip_no and id<>$update_id" )==1)
		{
			echo "11**0";
			die;
		}*/
		...
		if (str_replace("'", "", $copy_id) == 1) {
			//$recipe_no = return_field_value("id", "yd_recipe_entry_mst", "batch_id=" . $batch_id . " and entry_form=435", "id");

			$batch_process_ids = return_field_value("process_id","yd_batch_mst","id = $batch_id");
			if($batch_process_ids!= "")
			{
				$batch_process_id_arr = explode(",", $batch_process_ids);
				if(!in_array("137", $batch_process_id_arr))
				{
					$recipe_no = return_field_value("a.recipe_no", "yd_recipe_entry_mst a,yd_recipe_entry_dtls b", "a.id=b.mst_id and b.sub_process_id=$subprocess and a.batch_id=" . $batch_id . " and a.entry_form=435 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0", "recipe_no");
					if ($recipe_no != '') {
						echo "14**0**$recipe_no";
						die;
					}
		...
		{
			$rID2=execute_query(bulk_update_sql_statement( "yd_recipe_entry_dtls", "id", $field_array_dtls_update, $data_array_dtls_update, $id_arr ),1);
			//echo bulk_update_sql_statement( "yd_recipe_entry_dtls", "id", $field_array_dtls_update, $data_array_dtls_update, $id_arr );die;
			if($flag==1)
			{
				if($rID2) $flag=1; else $flag=0;
			}
		...
		if($data_array_dtls!="")
		{
			//echo "insert into yd_recipe_entry_dtls (".$field_array_dtls.") Values ".$data_array_dtls."";die;
			$rID2=sql_insert("yd_recipe_entry_dtls",$field_array_dtls,$data_array_dtls,1);
			if($flag==1)
			{
				if($rID2) $flag=1; else $flag=0;
		...
		if ($data_array_dtls_update2 != "") {
			$rID = sql_update("yd_recipe_entry_dtls", $field_array_dtls_update2, $data_array_dtls_update2, "id", $update_dtls_id, 1);
			if ($rID) $flag = 1; else $flag = 0;
		}
		if ($data_array_dtls_update != "") {
			$rID2 = execute_query(bulk_update_sql_statement("yd_recipe_entry_dtls", "id", $field_array_dtls_update, $data_array_dtls_update, $id_arr), 1);
		...
		if ($db_type == 0) {
			if ($flag == 1) {
				mysql_query("COMMIT");
				echo "1**" . str_replace("'", '', $update_id) . "**" . $subprocess. "**" . $recipe_serial_no. "**" . str_replace("'", '', $txt_sys_number);
	...
	else if ($operation==2)  // Delete Here----------------------------------------------------------
	{
		...
		$recipe_no = return_field_value("a.id as id", "yd_recipe_entry_mst a,yd_recipe_entry_dtls b", "a.id=b.mst_id and a.id=" . $update_id . " and a.entry_form!=435 and  b.status_active=1 and b.is_deleted=0", "id");
		if ($recipe_no != '')
		{
		...
		$delete_cause=str_replace(')','',$delete_cause);
			...
			/*$rID1=execute_query( "update yd_recipe_entry_mst set status_active=0,is_deleted=1,updated_by=".$_SESSION['logic_erp']['user_id'].", update_date='".$pc_date_time."'   where  id in (".str_replace("'","",$update_id).")",0);
			if($rID1) $flag = 1; else $flag = 0;
			$rID2=execute_query( "update yd_recipe_entry_dtls set status_active=0,is_deleted=1 where  mst_id in(".str_replace("'","",$update_id).") ",0);
			if( $flag==1)
			{
				if($rID2) $flag = 1; else $flag = 0;
			}*/
			$rID2=execute_query( "update yd_recipe_entry_dtls set status_active=0,is_deleted=1,delete_cause='$delete_cause' where  mst_id in(".str_replace("'","",$update_id).") and sub_process_id=".str_replace("'","",$cbo_sub_process)." ",0);
			if($rID2) $flag = 1; else $flag = 0;
...
{
	//echo "10**".$arrUpdateFields.'**'.$arrUpdateValues; die;
	...
	if ($action == "item_stock_details") {
		...
		$sql = "select $grp_con from yd_recipe_entry_dtls where   mst_id=" . $data[1] . " and is_deleted=0 and status_active=1";
...
from platformerpv3.yd_batch_dtls*/
		...
		//echo $sql_recipe_mst = "select id, labdip_no, company_id,working_company_id as w_com_id,location_id, recipe_description, batch_id, method, recipe_date, order_source, style_or_order, booking_id, total_liquor, batch_ratio, liquor_ratio, remarks, buyer_id, color_id, color_range, remarks from yd_recipe_entry_mst where id='$data[1]'"; die;
		
		
		
		
		$sql_recipe_mst = "select id, entry_form, labdip_no, company_id, location_id, batch_id,recipe_date, color_id, buyer_id, color_range, total_liquor, batch_ratio, liquor_ratio,batch_qty, composition,lot,count,yarn_type_id ,remarks,recipe_no from yd_recipe_entry_mst where id='$data[1]'";
		...
		<div style="width:930px; font-size:6px">
			<table width="930" cellspacing="0" align="right" border="0">
				...
				<tr>
					...
					if($within_group==1){ 	echo $company_library[$dataArray[0][csf('buyer_id')]]; }else{ echo $buyer_library[$dataArray[0][csf('buyer_id')]];}?></td>
					<td><strong>Booking:</strong></td>
					<td><? echo $dataArray[0][csf('style_or_order')]; ?></td>
					<td><strong>Color:</strong></td>
					<td> <? echo $color_arr[$dataArray[0][csf('color_id')]]; ?></td>
...
if($action == "recipe_entry_print_2")
{
	...
	$batch_array = array();
	if ($db_type == 0) {
		$sql = "select a.id, a.batch_no,a.batch_against, a.entry_form, a.batch_weight, a.total_trims_weight, group_concat(b.po_id) as po_id, group_concat(b.prod_id) as prod_id, sum(b.batch_qnty) as batch_qnty from yd_batch_mst a,  pro_batch_create_dtls b where a.id=b.mst_id and a.company_id='$company_id'  and a.id=$batch_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id order by a.id DESC";
	} else if ($db_type == 2) {
		 $sql = "select a.id, a.batch_no,a.batch_against,a.entry_form, a.batch_weight, a.total_trims_weight, listagg(b.po_id,',') within group (order by b.po_id) as po_id, listagg(b.prod_id,',') within group (order by b.prod_id) as prod_id,  sum(b.batch_qnty) as batch_qnty  from yd_batch_mst a, pro_batch_create_dtls b where a.id=b.mst_id and a.company_id='$company_id' and a.id=$batch_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by a.id,a.batch_against, a.batch_no,a.entry_form, a.batch_weight, a.total_trims_weight order by a.id DESC";
	}
	//echo $sql;
	...
	$sql_recipe_mst = "select id, labdip_no, company_id,working_company_id as w_com_id,location_id, recipe_description, batch_id, method, recipe_date, order_source, style_or_order, booking_id, total_liquor, batch_ratio, liquor_ratio, remarks, buyer_id, color_id, color_range, remarks from yd_recipe_entry_mst where id='$update_id'";
	...
	<div style="width:1000px; font-size:6px">
		<table width="1000" cellspacing="0" align="center" border="0" role="all">
			...
			<tr>
				<td colspan="6" style="font-size:x-large; text-align:center;"><u><strong><? echo $company_library[$data[0]]; //.data[3]; ?></u></strong></td>
			</tr>
			<tr>
				<td colspan="6" align="center">
					<?
		...
		<table cellspacing="0" align="center" border="1" width="1000" style="margin-top:20px;" rules="all" class="rpt_table">
			...
			<div style="width: 600px; float: left; margin-top:15px; margin-right:10px;">
				<table cellpadding="0" cellspacing="0" border="1" rules="all" width="600" class="rpt_table">
					...
					<tr>
						...
						<td colspan="3" align="center"><? echo $solution_amount;?></td>
					...
					$recipeData=sql_select("SELECT
						...
						b.item_description,
						b.unit_of_measure
						FROM yd_recipe_entry_dtls a, product_details_master b
						WHERE     a.prod_id = b.id
			...
			<div style="width: 370px; float: left; margin-top:15px; margin-left:3px;">
				<table cellpadding="0" cellspacing="0" border="1" rules="all" width="360" class="rpt_table">
					...
					<tr>
						<td width="110"></td>
						<td width="110" align="center"><? echo $alkali_solution_amount; ?></td>
