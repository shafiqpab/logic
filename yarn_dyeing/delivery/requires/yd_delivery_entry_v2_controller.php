...
{	
	...
	else if($yarn_dyeing_process==2 && $service_process_id==2)
	{
		if($cbo_company_name!=0)
		{
			$condition .= " and a.company_id=$cbo_company_name";
		...
		if($txt_receive_no!=0)
		{
			$condition .= " and a.trans_no_prefix_num=$txt_receive_no";
		}
		...
		if($search_type==1)
		{
			if($search_str!="")
			{
				...
				else if ($search_by==4) $condition=" and d.sales_order_no = '$search_str' ";
			}
		...
		else if($search_type==2)
		{
			if($search_str!="")
			{
				...
				else if ($search_by==4) $condition=" and d.sales_order_no like  '$search_str%' ";
			}
   ...
   a.yd_process, a.yd_type,a.pro_type,b.style_ref,b.count_type,b.sales_order_no,b.id order by a.id DESC";
		...
		{
			$sql= "select a.YD_TRANS_NO as yd_receive, a.id, b.id as receive_dtls_id, d.style_ref, a.party_id, c.pro_type, a.within_group, c.yd_job as job_no, d.order_no, d.order_id, c.order_type,a.receive_date,  d.count_type, b.sales_order_no 
			from yd_material_mst a,yd_material_dtls b, yd_ord_mst c,yd_ord_dtls d
			where a.id=b.mst_id and a.entry_form=387 and a.status_active =1 and a.is_deleted =0  and a.yd_job_id=c.id  and c.id=d.mst_id and d.id=b.job_dtls_id $date_con $condition group by a.YD_TRANS_NO, a.id, b.id, d.style_ref, a.party_id, c.pro_type, a.within_group, c.yd_job, d.order_no, d.order_id, c.order_type, a.receive_date,  d.count_type, b.sales_order_no  order by a.id DESC";
		}
		else
		{     
	...
	<div style="width:996px; max-height:320px;overflow-y:scroll;" >
	...
	<table width="100%" cellspacing="0" cellpadding="0" style="border:none;margin-top:10px" align="center">
		    <tr>
...
if($action=="load_php_yd_job_data_to_form")
{
	...
	$sql = "select a.id, a.company_id, a.location_id, a.within_group, a.party_id, a.party_location, a.order_no, a.pro_type, a.order_type from yd_ord_mst a where a.yd_job='$data[0]' and a.status_active=1 and a.is_deleted=0 and a.check_box_confirm=1";

	$data_array = sql_select($sql);
	unset($sql);
	
	$nameArray= sql_select("select id, company_name, variable_list, service_process_id,yarn_dyeing_process from variable_setting_yarn_dyeing where company_name=".$data[2]." and variable_list=2 order by id");
	
	$variable_list=$nameArray[0][csf('variable_list')];//2
	$yarn_dyeing_process=$nameArray[0][csf('yarn_dyeing_process')];
	$service_process_id=$nameArray[0][csf('service_process_id')];  
    if($yarn_dyeing_process==13 && $service_process_id==3)
	{
		echo "document.getElementById('qty_no_td').innerHTML = 'Inspect. Qty';\n";
	}
	else if($yarn_dyeing_process==2 && $service_process_id==2)
	{
		echo "document.getElementById('qty_no_td').innerHTML = 'Metrial Rcvd Qty';\n";
	}
	else{
		echo "document.getElementById('qty_no_td').innerHTML = 'Order Qty';\n";
	}
...
if($action=="load_php_yd_delivery_data_to_form")
{
	...
	$sql = "select a.id, a.company_id, a.location_id, a.within_group, a.party_id, a.party_location, a.receive_date, a.driver_name, a.vehicle_no, a.remarks from yd_store_receive_mst a where a.yd_receive='$data[1]' and a.status_active=1 and a.is_deleted=0 and a.entry_form=640";
    ...
    foreach($data_array as $data)
    {
    	...
    	echo "document.getElementById('txt_driver_name').value = '".$data[csf('driver_name')]."';\n";
    	echo "document.getElementById('txt_vehicle_no').value = '".$data[csf('vehicle_no')]."';\n";
    	echo "document.getElementById('txt_remarks').value = '".$data[csf('remarks')]."';\n";
    }
	if($yarn_dyeing_process==13 && $service_process_id==3)
	{
		echo "document.getElementById('qty_no_td').innerHTML = 'Insp. Qty';\n";
	}
	else if($yarn_dyeing_process==2 && $service_process_id==2)
	{
		echo "document.getElementById('qty_no_td').innerHTML = 'Metrial Rcvd Qty';\n";
...
if($action=="receive_dtls_list_view")
{
	...
	{
		// need to work for inspection
		$pre_del_sql = "select b.id, a.yd_receive, b.dtls_id, b.receive_qty,b.rcv_grey_qty , b.job_no, c.id as rcv_dtls_id from yd_store_receive_mst a, yd_store_receive_dtls b , yd_material_dtls c where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.job_no='$job_no' and a.entry_form=640 and b.dtls_id = c.job_dtls_id and c.status_active=1 and c.is_deleted=0 ";
		// echo $pre_del_sql;
		$pre_del_data = sql_select($pre_del_sql);
		...
		{
			// $pre_del_arr[$data[csf('dtls_id')]]['receive_qty'] += $data[csf('receive_qty')]; 
			// $pre_del_arr[$data[csf('dtls_id')]]['rcv_grey_qty'] += $data[csf('rcv_grey_qty')]; 
			$pre_del_arr[$data[csf('rcv_dtls_id')]]['receive_qty'] += $data[csf('receive_qty')]; 
			$pre_del_arr[$data[csf('rcv_dtls_id')]]['rcv_grey_qty'] += $data[csf('rcv_grey_qty')]; 
		}
		//  echo "<pre>";print_r($pre_del_arr);die;

		$inspected_qty_sql="select b.id, a.SYS_NUMBER, b.JOB_DTLS_ID, b.INSPECTION_STATUS, b.PRODUCTION_QTY from yd_inspection_mst a, yd_inspection_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.SYS_NUMBER='$receive_no' and a.entry_form=443";
		$inspected_qty = sql_select($inspected_qty_sql);
		foreach ($inspected_qty as $key => $val) {
			$ins_arr[$val['JOB_DTLS_ID']]+=$val['INSPECTION_STATUS'];
		}
		// print_r($ins_arr);die;
		// $inspected_qty=return_field_value("INSPECTION_STATUS","yd_inspection_mst a, yd_inspection_dtls b","a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.SYS_NUMBER='$receive_no' and a.entry_form=443");
		// $batch_qty=return_field_value("QUANTITY","yd_batch_mst a, yd_batch_dtls b, yd_ord_mst c","c.yd_job='$job_no' and c.id=b.YD_JOB_ID and a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0");


	}
	else if($yarn_dyeing_process==2 && $service_process_id==2)
	{
		$pre_del_sql = "select b.id, a.yd_receive, b.dtls_id, b.receive_qty,b.rcv_grey_qty , b.job_no, c.id as rcv_dtls_id from yd_store_receive_mst a, yd_store_receive_dtls b , yd_material_dtls c where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.job_no='$job_no' and a.entry_form=640 and b.dtls_id = c.job_dtls_id and c.id=b.rcv_dtls_id and c.status_active=1 and c.is_deleted=0 ";
		// echo $pre_del_sql;
		$pre_del_data = sql_select($pre_del_sql);
		...
		//  echo "<pre>";print_r($pre_del_arr);die;
		$mtrl_rcv_qty_sql="select b.id, a.yd_trans_no, b.JOB_DTLS_ID, b.receive_qty from yd_material_mst a, yd_material_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.yd_trans_no='$receive_no' and a.entry_form=387";
		$mtrl_rcv_qty = sql_select($mtrl_rcv_qty_sql);
		foreach ($mtrl_rcv_qty as $key => $val) {
			// $ins_arr[$val['JOB_DTLS_ID']]+=$val['RECEIVE_QTY'];
			$ins_arr[$val['ID']]+=$val['RECEIVE_QTY'];
		}
	}
	else
	{ 
	...
	$batch_qty_sql="select a.BATCH_NUMBER, b.YD_JOB_DTLS_ID, b.QUANTITY, d.id as rcv_dtls_id from yd_batch_mst a, yd_batch_dtls b, yd_ord_mst c ,yd_material_dtls d where c.yd_job='$job_no' and c.id=b.YD_JOB_ID and b.YD_JOB_DTLS_ID = d.job_dtls_id and a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 and d.status_active=1 and d.is_deleted=0";
	// echo $batch_qty_sql;
	$batch_qty=sql_select($batch_qty_sql);
	foreach ($batch_qty as $key => $val) {
		$batch_arr[$val['RCV_DTLS_ID']]['QUANTITY']+=$val['QUANTITY'];
		$batch_arr[$val['RCV_DTLS_ID']]['BATCH_NUMBER']=$val['BATCH_NUMBER'];
		// $batch_arr[$val['YD_JOB_DTLS_ID']]['QUANTITY']+=$val['QUANTITY'];
		// $batch_arr[$val['YD_JOB_DTLS_ID']]['BATCH_NUMBER']=$val['BATCH_NUMBER'];
	}
	...
	if($yarn_dyeing_process==1 && $service_process_id==1)
	{
			$sql= "select a.id as job_id, b.id as receive_dtls_id, a.yd_job as job_no, a.job_no_prefix_num,a.within_group, $ins_year_cond as year, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date ,a.booking_without_order, a.booking_type,a.order_type,a.yd_process,b.lot,b.count_id, b.yarn_type_id, b.yarn_composition_id,b.uom,b.no_bag,b.process_loss, b.cone_per_bag, b.no_cone,b.order_quantity,b.order_quantity as receive_qty,b.adj_type, b.total_order_quantity, item_color_id, a.yd_type,a.pro_type,b.style_ref,b.count_type,b.sales_order_no,b.buyer_buyer,b.yd_color_id from yd_ord_mst a,yd_ord_dtls b where a.id=b.mst_id and a.entry_form=374 and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0  and a.yd_job='$receive_no'";
		   $result = sql_select($sql);
	}
	else if($yarn_dyeing_process==13 && $service_process_id==3)
	{
	...
	{
		$sql= "select a.id as job_id, b.id as job_dtls_id, d.id as receive_dtls_id, a.yd_job as job_no, a.job_no_prefix_num,a.within_group, $ins_year_cond as year, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date ,a.booking_without_order, a.booking_type,a.order_type,a.yd_process,d.lot,b.count_id, b.yarn_type_id, b.yarn_composition_id,b.uom,b.no_bag,b.process_loss, b.cone_per_bag, b.no_cone,b.order_quantity,b.order_quantity as receive_qty,b.adj_type, b.total_order_quantity, item_color_id, a.yd_type,a.pro_type,b.style_ref,b.count_type,b.sales_order_no,b.buyer_buyer,b.yd_color_id, c.id as rcv_id, YD_TRANS_NO as rcv_no
		from yd_ord_mst a, yd_ord_dtls b , yd_material_mst c, yd_material_dtls d 
		where a.id=b.mst_id and  d.mst_id = c.id AND b.id=d.job_dtls_id and c.yd_job_id=a.id AND c.entry_form=387 AND a.entry_form=374 and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0  and c.yd_trans_no='$receive_no' and c.status_active =1 and c.is_deleted =0 and d.status_active =1 and d.is_deleted =0";
		//    echo $sql; 
		$result = sql_select($sql);
	}
	else
	{ 
		...
		$sql = "select a.id as job_id, a.order_type, b.id as receive_dtls_id, b.style_ref, b.sales_order_no, b.sales_order_id, b.buyer_buyer, b.lot, b.gray_lot, b.count_type, b.count_id, b.yarn_type_id, b.yarn_composition_id, b.yd_color_id, b.no_bag, b.cone_per_bag, b.uom, b.order_quantity, b.process_loss, b.adj_type, b.total_order_quantity, b.receive_qty,b.DTLS_ID
	...
	foreach($result as $data)
	{
		...
		$prev_receive_grey_qty = $pre_del_arr[$data[csf('receive_dtls_id')]]['rcv_grey_qty'];
 
 
		if($data[csf('order_type')]==1)
		{
			$current_delivery =number_format($ins_arr[$data['RECEIVE_DTLS_ID']]-($ins_arr[$data['RECEIVE_DTLS_ID']]*($data[csf('process_loss')]/100)),2,'.','')-number_format($prev_receive_qty,2,'.','');
			// $current_delivery =number_format($data[csf('total_order_quantity')],2,'.','')-number_format($prev_receive_qty,2,'.','');
			//$order_qty = $data[csf('total_order_quantity')];
			$order_qty = number_format($data[csf('total_order_quantity')],2,'.',''); 
			$total_finish_qty=$current_delivery-$ins_arr[$data['RECEIVE_DTLS_ID']];
		}
		elseif($data[csf('order_type')]==2)
		{
			$current_delivery =number_format($ins_arr[$data['RECEIVE_DTLS_ID']]-($ins_arr[$data['RECEIVE_DTLS_ID']]*($data[csf('process_loss')]/100)),2,'.','')-number_format($prev_receive_qty,2,'.','');
			// $current_delivery =number_format($data[csf('order_quantity')],2,'.','')-number_format($prev_receive_qty,2,'.','');
			$order_qty = number_format($data[csf('order_quantity')],2,'.',''); 
		}
		//$receive_qty = $receive_array[$data[csf('receive_dtls_id')]];
		
		$balance =number_format( $data[csf('receive_qty')],2,'.','')-number_format($prev_receive_qty,2,'.','');
	?>
        <tr id="row_<?php echo $tblRow;?>">
			...
			<td width="80">
            	<input style="width:80px" readonly class="text_boxes" type="text" name="txtlot[]" id="txtlot_<?php echo $tblRow;?>" value="<?php echo $data['LOT'];?>">
            </td>
            ...
            <td width="120">
            	...
            	<? echo create_drop_down( "txtYarnColor_".$tblRow, 120, $color_arr,"", 1, "-- Select --",$data[csf('yd_color_id')],"",1,'','','','','','',"txtYarnColor[]"); ?>
            </td>
            <td width="80">
            	<input style="width:80px" <?php echo $readonly; ?> class="text_boxes" type="text" name="txtGrayLot[]" id="txtGrayLot_<?php echo $tblRow;?>" value="<?php echo $batch_arr[$data['RECEIVE_DTLS_ID']]['BATCH_NUMBER'];?>">
            ...
            <td width="30">
            	<input style="width:30px" readonly class="text_boxes_numeric" type="text" name="txtProcessLoss[]" id="txtProcessLoss_<?php echo $tblRow;?>" value="<?php echo $data[csf('process_loss')];?>">
            ...
            <td width="80">
             ...
             <td width="80">
            	<input style="width:80px" readonly class="text_boxes_numeric" type="text" name="txtTotalqty[]" id="txtTotalqty_<?php echo $tblRow;?>" value="<?php echo number_format( $ins_arr[$data['RECEIVE_DTLS_ID']],2,'.',''); ?>">
            ...
            <td width="50">
            	<input style="width:50px" readonly class="text_boxes_numeric" type="text" name="txtTotalqty[]" id="txtTotalqty_<?php echo $tblRow;?>" value="<?php echo number_format( $batch_arr[$data['RECEIVE_DTLS_ID']]['QUANTITY'],2,'.',''); ?>">
			...
			<td width="50">
				<input style="width:50px" readonly class="text_boxes_numeric" type="text" name="txtTotalReceiveqty[]" id="txtTotalReceiveqty_<?php echo $tblRow;?>" value="<?php echo number_format($prev_receive_qty,2,'.','');?>">
			</td>
			<td width="50">
				<input style="width:50px" readonly class="text_boxes_numeric" type="text" name="txtTotalReceiveGreyqty[]" id="txtTotalReceiveGreyqty_<?php echo $tblRow;?>" value="<?php echo number_format($prev_receive_grey_qty ,2,'.','');?>">
			</td>
            ...
            <td width="50" >
            	<input style="width:50px" class="text_boxes_numeric" type="text" onKeyup="calculate_receive_qty(this.id,this.value);" name="txtReceiveQty[]" id="txtReceiveQty_<?php echo $tblRow;?>"  placeholder="<?php echo number_format($current_delivery,2,'.','');?>" value="">	
            </td>
			<td width="50">
				<input style="width:50px" class="text_boxes_numeric" type="text" onKeyup="cal_pro_loss(this.id,this.value);" name="txtProLoss[]" id="txtProLoss_<?php echo $tblRow;?>"  placeholder="" value="" disabled>
			</td>
			<td width="50">
				<input style="width:50px" class="text_boxes_numeric" type="text" onBlur="cal_current_delv_grey_qty(this.id,this.value);" name="txtGreyReceiveQty[]" id="txtGreyReceiveQty_<?php echo $tblRow;?>"  placeholder="" value="" disabled>
			</td>
			
			<td width="50">
				<input style="width:50px" readonly class="text_boxes_numeric" type="text" name="txtBncGreyqty[]" id="txtBncGreyqty_<?php echo $tblRow;?>" value="" placeholder="<?php echo number_format($ins_arr[$data['RECEIVE_DTLS_ID']]-$prev_receive_grey_qty,2,'.','');?>" disabled>
				<input class="text_boxes_numeric" type="hidden" name="hdnTotGreyQty[]" id="hdnTotGreyQty_<?php echo $tblRow;?>" value="<?php echo $ins_arr[$data['RECEIVE_DTLS_ID']]-$prev_receive_grey_qty;?>">
			</td>
			<td width="50">
				<input style="width:50px" readonly class="text_boxes_numeric" type="text" name="txtTotalFinqty[]" id="txtTotalFinqty_<?php echo $tblRow;?>" value="" disabled>
			</td>
			<td width="60">
            ...
            <td width="60">
				...
				<input class="text_boxes_numeric" type="hidden" name="hiddenOrderQty[]" id="hiddenOrderQty_<?php echo $tblRow;?>" value="<?php echo number_format($order_qty,2,'.','') ?>">
            	<input class="text_boxes_numeric" type="hidden" name="txtHiddenDeliveryId[]" id="txtHiddenDeliveryId_<?php echo $tblRow;?>" value="">
            	<input class="text_boxes_numeric" type="hidden" name="txtHiddenDtlsId[]" id="txtHiddenDtlsId_<?php echo $tblRow;?>" value="<?php echo $data[csf('job_dtls_id')];?>">
            	<input class="text_boxes_numeric" type="hidden" name="txtHiddenRcvDtlsId[]" id="txtHiddenRcvDtlsId_<?php echo $tblRow;?>" value="<?php echo $data[csf('receive_dtls_id')];?>">
            </td>
...
if($action=="delivery_update_dtls_list_view")    
{
	...
	$service_process_id=$nameArray[0][csf('service_process_id')]; //1 
	  ...
	  if($yarn_dyeing_process==1 && $service_process_id==1)
		...
		{
			$inspected_qty_sql="select b.id, a.SYS_NUMBER, b.JOB_DTLS_ID, b.INSPECTION_STATUS, b.PRODUCTION_QTY from yd_inspection_mst a, yd_inspection_dtls b, yd_ord_dtls c  where a.id=b.mst_id and c.JOB_NO_MST='$job_no' and  c.id=b.JOB_DTLS_ID and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and a.entry_form=443";
		...
		{
			$mtrl_rcv_qty_sql="select b.id, a.yd_trans_no, b.JOB_DTLS_ID, b.receive_qty from yd_material_mst a, yd_material_dtls b, yd_ord_dtls c  where a.id=b.mst_id and c.JOB_NO_MST='$job_no' and  c.id=b.JOB_DTLS_ID and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  and a.entry_form=387";
		...
		$batch_qty_sql="select a.BATCH_NUMBER, b.YD_JOB_DTLS_ID, b.QUANTITY from yd_batch_mst a, yd_batch_dtls b, yd_ord_mst c where c.yd_job='$job_no' and c.id=b.YD_JOB_ID and a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0";
			// echo $batch_qty_sql;
			$batch_qty=sql_select($batch_qty_sql);
			foreach ($batch_qty as $key => $val) {
				$batch_arr[$val['YD_JOB_DTLS_ID']]['QUANTITY']+=$val['QUANTITY'];
			...
			$pre_del_sql = "select b.id, a.yd_receive, b.dtls_id, b.rcv_dtls_id, b.receive_qty,b.rcv_grey_qty , b.job_no from yd_store_receive_mst a, yd_store_receive_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.job_no='$job_no' and b.mst_id!=$update_id and a.entry_form=640";
			// echo $pre_del_sql;
			$pre_del_data = sql_select($pre_del_sql);
			...
			// $pre_del_sql = "select b.id, a.yd_receive, b.dtls_id, b.receive_qty, b.job_no from yd_store_receive_mst a, yd_store_receive_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.job_no='$job_no' and b.mst_id!=$update_id and a.entry_form=640";// inspection
			// $pre_del_data = sql_select($pre_del_sql);
			// foreach($pre_del_data as $data)
			// {
			// 	$pre_del_arr[$data[csf('dtls_id')]]['receive_qty'] += $data[csf('receive_qty')]; 
	...
	{ 
		$current_delivery_arr[$data[csf('rcv_dtls_id')]]['receive_qty']+= $data[csf('receive_qty')];
		$current_delivery_arr[$data[csf('rcv_dtls_id')]]['receive_dtls_id']= $data[csf('receive_dtls_id')];
		$current_delivery_arr[$data[csf('rcv_dtls_id')]]['gray_lot']= $data[csf('gray_lot')];
		$current_delivery_arr[$data[csf('rcv_dtls_id')]]['no_bag']+= $data[csf('no_bag')];
		$current_delivery_arr[$data[csf('rcv_dtls_id')]]['cone_per_bag']+= $data[csf('cone_per_bag')];
	}
	$result = sql_select($sql);
	...
	foreach($result as $data)
	{
		...
		// $prev_receive_qty = $pre_del_arr[$data[csf('receive_dtls_id')]]['receive_qty'];
 		if($data[csf('order_type')]==1)
		{ 
			$current_delivery =number_format($ins_arr[$data['RECEIVE_DTLS_ID']]-($ins_arr[$data['RECEIVE_DTLS_ID']]*($data[csf('process_loss')]/100))-$prev_receive_qty,2,'.','');
			
			// $current_delivery =number_format($data[csf('total_order_quantity')],2,'.','')-number_format($prev_receive_qty,2,'.','');
			$order_qty =number_format($data[csf('total_order_quantity')],2,'.','');
			// $total_finish_qty=$current_delivery-$ins_arr[$data['RECEIVE_DTLS_ID']];
			
			$total_finish_qty=number_format($data['RCV_QTY'],2,'.','')+number_format($prev_receive_qty,2,'.','');
		}
		elseif($data[csf('order_type')]==2)
		{
			$current_delivery =number_format($data[csf('order_quantity')],2,'.','')-number_format($prev_receive_qty,2,'.','');
		...
		// $balance =number_format($data[csf('receive_qty')],2,'.','')-number_format($prev_receive_qty,2,'.','');
		$balance_grey_qty =number_format($ins_arr[$data['RECEIVE_DTLS_ID']]-$prev_receive_grey_qty-$data[csf('rcv_grey_qty')],2,'.','');
	...
	?>
        <tr id="row_<?php echo $tblRow;?>">
            ...
            <td width="80">
            	<input style="width:80px" readonly class="text_boxes" type="text" name="txtlot[]" id="txtlot_<?php echo $tblRow;?>" value="<?php echo $data[csf('raw_yarn_lot')];?>">
            </td>
            <td width="60">
            	<input class="text_boxes" type="hidden" name="txtcountId[]" id="txtcountId_<?php echo $tblRow;?>" value="<?php echo $data[csf('count_id')];?>">
            	<?
			...
			<td width="50">
            	<input style="width:50px" readonly class="text_boxes_numeric" type="text" name="txtTotalqty[]" id="txtTotalqty_<?php echo $tblRow;?>" value="<?php echo number_format( $ins_arr[$data['RECEIVE_DTLS_ID']],2,'.',''); ?>">
            	<input readonly class="text_boxes_numeric" type="hidden" name="txtHiddenTotalqty[]" id="txtHiddenTotalqty_<?php echo $tblRow;?>" value="<?php echo number_format( $order_qty,2,'.','');?>">
            </td>
			<td width="50">
				<input style="width:50px" readonly class="text_boxes_numeric" type="text" name="txtTotalqty[]" id="txtTotalqty_<?php echo $tblRow;?>" value="<?php echo number_format( $batch_arr[$data['DTLS_ID']]['QUANTITY'],2,'.',''); ?>">
			</td>
			<td width="50">
				<input style="width:50px" readonly class="text_boxes_numeric" type="text" name="txtTotalReceiveqty[]" id="txtTotalReceiveqty_<?php echo $tblRow;?>" value="<?php echo number_format($prev_receive_qty,2,'.','');?>">
			</td>
			...
			<td width="50" >
				<input style="width:50px" class="text_boxes_numeric" type="text" onKeyup="cal_pro_loss(this.id,this.value);" name="txtProLoss[]" id="txtProLoss_<?php echo $tblRow;?>"  placeholder="" value="<?php echo number_format($data[csf('pro_loss')],2,'.','');?>" disabled>
			</td>
			<td width="50">
				<input style="width:50px" class="text_boxes_numeric" type="text" onBlur="cal_current_delv_grey_qty(this.id,this.value);" name="txtGreyReceiveQty[]" id="txtGreyReceiveQty_<?php echo $tblRow;?>"  placeholder="" value="<?php echo number_format($data[csf('rcv_grey_qty')],2,'.','');?>" disabled>
			</td>
			<td width="50">
				<input style="width:50px" readonly class="text_boxes_numeric" type="text" name="txtBncGreyqty[]" id="txtBncGreyqty_<?php echo $tblRow;?>" value="" placeholder="<?php echo $balance_grey_qty;?>" disabled>
				<input class="text_boxes_numeric" type="hidden" name="hdnTotGreyQty[]" id="hdnTotGreyQty_<?php echo $tblRow;?>" value="<?php echo $data[csf('order_quantity')];?>">
			</td>
			<td width="50">
				<input style="width:50px" readonly class="text_boxes_numeric" type="text" name="txtTotalFinqty[]" id="txtTotalFinqty_<?php echo $tblRow;?>" value="<?php echo number_format($total_finish_qty,2,'.','');?>" disabled>
			</td>
			<td width="60">
            	<input class="text_boxes" type="hidden" name="txtcountTypeId[]" id="txtcountTypeId_<?php echo $tblRow;?>" value="<?php echo $data[csf('count_type')];?>">
            	<?
...
if($action=="dtls_list_view")
{
	...
	?>
        <tr style="cursor:pointer" id="dtls_row_<?php echo $tblRow;?>" bgcolor="<? echo $bgcolor; ?>" onClick="load_receive_data('<?php echo $data[csf('job_no')];?>',<?php echo $data[csf('job_id')];?>,'<?php echo $data[csf('yd_receive')];?>',<?php echo $data[csf('rcv_id')];?>,<?php echo $tblRow;?>)">
...
if ($action=="save_update_delete")
{
    ...
    if ($operation==0) // Insert Start Here
    {
	    ...
	    $hdn_update_id    			= str_replace("'",'',$hdn_update_id);
	   	...
	   	$job_id    					= str_replace("'",'',$hdn_job_id);
	   	$rcv_id    					= str_replace("'",'',$hdn_rcv_id);
		...
		$prev_rcv_grey_qty= array();

		$check_prev_qty =sql_select("Select b.rcv_grey_qty, rcv_dtls_id from yd_store_receive_mst a, yd_store_receive_dtls b
		where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form=640 and  b.rcv_id=$rcv_id");

		foreach ($check_prev_qty as $key => $row) {
			$prev_rcv_grey_qty[$row['RCV_DTLS_ID']]+=$row['RCV_GREY_QTY'];
		}
	    ...
	    if(str_replace("'","",$hdn_update_id)=="")
		{
			...
			$nameArray= sql_select("select id, company_name, variable_list, service_process_id,yarn_dyeing_process from variable_setting_yarn_dyeing where company_name='$cbo_company_name' and variable_list=2 order by id");
			$variable_list=$nameArray[0][csf('variable_list')];
			$yarn_dyeing_process=$nameArray[0][csf('yarn_dyeing_process')];
			$service_process_id=$nameArray[0][csf('service_process_id')];
		
	    	$field_array="id, entry_form, yd_receive, receive_no_prefix, receive_no_prefix_num, receive_date, company_id, location_id, within_group, party_id, party_location, DRIVER_NAME, vehicle_no, remarks, service_process_id, yarn_dyeing_process, inserted_by, insert_date";

	    	$data_array="(".$id.", 640, '".$txt_delivery_no[0]."', '".$txt_delivery_no[1]."', '".$txt_delivery_no[2]."', '".$txt_delivery_date."', '".$cbo_company_name."', '".$cbo_location_name."', '".$cbo_within_group."', '".$cbo_party_name."', '".$cbo_party_location."', '".$txt_driver_name."','".$txt_vehicle_no."','".$txt_remarks."',".$service_process_id.",".$yarn_dyeing_process.",".$_SESSION['logic_erp']['user_id'].", '".$pc_date_time."')";
		...
		else
		{
			...
			$pre_del_sql = "select b.id, a.yd_receive, b.dtls_id, b.receive_qty,b.rcv_grey_qty, b.job_no from yd_store_receive_mst a, yd_store_receive_dtls b where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.yd_receive='$txt_delivery_no' and a.id=$id and a.entry_form=640";

			$pre_del_data = sql_select($pre_del_sql);
			// echo "<pre>";print_r($pre_del_data);die;
			foreach($pre_del_data as $data)
			{
				// $pre_del_arr[$data[csf('yd_receive')]][$data[csf('job_no')]][$data[csf('dtls_id')]]['receive_qty'] += $data[csf('receive_qty')];
				$pre_del_arr[$data[csf('yd_receive')]][$data[csf('job_no')]][$data[csf('dtls_id')]]['receive_qty'] += $data[csf('rcv_grey_qty')];
	    ...
	    $field_array2="id, mst_id, receive_no_mst, style_ref, sales_order_no, sales_order_id, buyer_buyer, lot, gray_lot, count_type, count_id, yarn_type_id, yarn_composition_id, yd_color_id, no_bag, cone_per_bag, uom, order_quantity, process_loss, adj_type, total_order_quantity, receive_qty, rcv_grey_qty, pro_loss, dtls_id, rcv_dtls_id, job_no, order_no, pro_type, order_type, rcv_no, rcv_id, job_id, inserted_by, insert_date";

	    $field_array3="style_ref*sales_order_no*sales_order_id*buyer_buyer*lot*gray_lot*count_type*count_id* yarn_type_id*yarn_composition_id*yd_color_id*no_bag*cone_per_bag*uom*order_quantity*process_loss*adj_type* total_order_quantity*receive_qty*rcv_grey_qty*pro_loss*dtls_id*updated_by*update_date";
	    ...
	    for($i=1; $i<=$total_row; $i++)
	    {
	        ...
	        $txtHiddenRcvDtlsId     = "txtHiddenRcvDtlsId_".$i;
	        $txtTotalReceiveqty    	= "txtTotalReceiveqty_".$i;
	        $txtGreyReceiveQty    	= "txtGreyReceiveQty_".$i;
	        $txtTotalqty    		= "txtTotalqty_".$i;
	        $txtProLoss    			= "txtProLoss_".$i;
			
			$Totalqty = str_replace("'",'',$$txtTotalqty);
			$txtHdnRcvDtlsId = str_replace("'",'',$$txtHiddenRcvDtlsId);
			$GreyReceiveQty = str_replace("'",'',$$txtGreyReceiveQty);
			$available_grey_qty = $Totalqty-$prev_rcv_grey_qty[$txtHdnRcvDtlsId];
			if($GreyReceiveQty > $available_grey_qty)
			{
				echo "11** Delivery Grey Qty Must not greater than Available Qty";
				disconnect($con);die; 
			}
			
	        	if($pre_del_arr[$txt_delivery_no][$txt_job_no][$hidden_dtls_id]['receive_qty']!='')
	        	{
	        		$cu_receive_qty = str_replace("'","",$$txtGreyReceiveQty);
	        		...
	        		$data_array3[$dtlsUpdateId]=explode("*",("".$$txtstyleRef."*".$$txtsaleOrder."*".$$txtsaleOrderID."*".$$buyerBuyer."*".$$txtlot."*".$$txtGrayLot."*".$$txtcountTypeId."*".$$txtcountId."*".$$cboYarnTypeId."*".$$txtydCompositionId."*".$$txtYarnColorId."*".$$txtnoBag."*".$$txtConeBag."*".$$cboUomId."*".$$txtOrderqty."*".$$txtProcessLoss."*".$$txtadjTypeId."*".$$txtHiddenTotalqty."*".$receive_qty."*".$$txtGreyReceiveQty."*".$$txtProLoss."*".$$txtHiddenDtlsId."*".$user_id."*'".$pc_date_time."'"));
	        	...
	        	else
	        	{
			        ...
			        $data_array2 .="(".$id1.",".$id.",'".$txt_delivery_no."',".$$txtstyleRef.",".$$txtsaleOrder.",".$$txtsaleOrderID.",".$$buyerBuyer.",".$$txtlot.",".$$txtGrayLot.",".$$txtcountTypeId.",".$$txtcountId.",".$$cboYarnTypeId.",".$$txtydCompositionId.",".$$txtYarnColorId.",".$$txtnoBag.",".$$txtConeBag.",".$$cboUomId.",".$$txtOrderqty.",".$$txtProcessLoss.",".$$txtadjTypeId.",".$$txtHiddenTotalqty.",".$$txtReceiveQty.",".$$txtGreyReceiveQty.",".$$txtProLoss.",".$$txtHiddenDtlsId.",".$$txtHiddenRcvDtlsId.",'".$txt_job_no."','".$txt_wo_no."',".$cbo_pro_type.",".$cbo_order_type.",'".$txt_rcv_no."',".$hdn_rcv_id.",".$hdn_job_id.",'".$user_id."','".$pc_date_time."')";
    ...
    elseif ($operation==1) // Update Start Here
    {
		...
		$check_prev_qty =sql_select("Select b.rcv_grey_qty, rcv_dtls_id from yd_store_receive_mst a, yd_store_receive_dtls b
		where a.id=b.mst_id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and a.entry_form=640 and  b.rcv_id=$rcv_id and a.id!=$hdn_update_id");
        ...
        $field_array="job_no*receive_date*company_id*location_id*within_group*party_id*party_location*order_no*pro_type*order_type*DRIVER_NAME*vehicle_no*remarks*updated_by*update_date";

        $data_array="'".$txt_job_no."'*'".$txt_delivery_date."'*'".$cbo_company_name."'*'".$cbo_location_name."'*'".$cbo_within_group."'*'".$cbo_party_name."'*'".$cbo_party_location."'*'".$txt_wo_no."'*'".$cbo_order_type."'*'".$cbo_pro_type."'*'".$txt_driver_name."'*'".$txt_vehicle_no."'*'".$txt_remarks."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";

        $field_array2="style_ref*sales_order_no*sales_order_id*buyer_buyer*lot*gray_lot*count_type*count_id* yarn_type_id*yarn_composition_id*yd_color_id*no_bag*cone_per_bag*uom*order_quantity*process_loss*adj_type* total_order_quantity*receive_qty*rcv_grey_qty*pro_loss*updated_by*update_date";
        ...
        for($i=1; $i<=$total_row; $i++)
	    {
			...
			$Totalqty = str_replace("'",'',$$txtTotalqty);
			$HiddenRcvDtlsId = str_replace("'",'',$$txtHiddenRcvDtlsId);
			$GreyReceiveQty = str_replace("'",'',$$txtGreyReceiveQty);
			$available_grey_qty = $Totalqty-$prev_rcv_grey_qty[$HiddenRcvDtlsId];
			if($GreyReceiveQty > $available_grey_qty)
			{
				echo "11** Delivery Grey Qty Must not greater than Available Qty";
				disconnect($con);die; 
			...
			if($cu_receive_qty>$txtTotalqty)
			{
				echo "11** Delivery Qty Must not greater than Inspact/Rcv Qty";disconnect($con);die; 
			}

			
	        if(str_replace("'",'',$$txtHiddenDeliveryId)!="")
            {
            	$data_array2[$dtlsUpdateId]=explode("*",("".$$txtstyleRef."*".$$txtsaleOrder."*".$$txtsaleOrderID."*".$$buyerBuyer."*".$$txtlot."*".$$txtGrayLot."*".$$txtcountTypeId."*".$$txtcountId."*".$$cboYarnTypeId."*".$$txtydCompositionId."*".$$txtYarnColorId."*".$$txtnoBag."*".$$txtConeBag."*".$$cboUomId."*".$$txtOrderqty."*".$$txtProcessLoss."*".$$txtadjTypeId."*".$$txtHiddenTotalqty."*".$$txtReceiveQty."*".$$txtGreyReceiveQty."*".$$txtProLoss."*".$user_id."*'".$pc_date_time."'"));

				

                $hdn_dtls_id_arr[]=str_replace("'",'',$$txtHiddenDeliveryId);
            }
            else
        	{
        		if ($add_commaa!=0) $data_array2 .=","; $add_comma=0;

		        $data_array3 .="(".$id1.",".$hdn_update_id.",'".$txt_delivery_no."',".$$txtstyleRef.",".$$txtsaleOrder.",".$$txtsaleOrderID.",".$$buyerBuyer.",".$$txtlot.",".$$txtGrayLot.",".$$txtcountTypeId.",".$$txtcountId.",".$$cboYarnTypeId.",".$$txtydCompositionId.",".$$txtYarnColorId.",".$$txtnoBag.",".$$txtConeBag.",".$$cboUomId.",".$$txtOrderqty.",".$$txtProcessLoss.",".$$txtadjTypeId.",".$$txtHiddenTotalqty.",".$$txtReceiveQty.",".$$txtGreyReceiveQty.",".$$txtProLoss.",".$$txtHiddenDtlsId.",'".$txt_job_no."','".$txt_wo_no."',".$cbo_pro_type.",".$cbo_order_type.",'".$user_id."','".$pc_date_time."')";
...
if($action=="delivery_dtls_list_view")
{
	...
	//die;
		...
		if($yarn_dyeing_process!=0 && $service_process_id!=0)
		{
 			//   $sql= "select a.id, a.party_id, a.yd_job as job_no, c.yd_receive, a.pro_type, a.order_type, a.order_no, a.yd_job ,d.rcv_no as store_receive
			//   from yd_ord_mst a,yd_ord_dtls b,yd_store_receive_mst c, yd_store_receive_dtls d  
			//   where a.id=b.mst_id and c.id=d.mst_id and a.entry_form=374 and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0  and c.status_active =1 and c.is_deleted =0 and d.status_active =1 and d.is_deleted =0 and c.id=$update_id and c.entry_form=640  and d.dtls_id=b.id group by a.id, a.party_id, a.yd_job, c.yd_receive, a.pro_type, a.order_type, a.order_no, a.yd_job, c.rcv_no";
			$sql= "select a.id, a.party_id, b.job_no, a.yd_receive, b.pro_type, b.order_type, b.order_no, b.job_no , b.rcv_no as store_receive, b.rcv_id, b.job_id from yd_store_receive_mst a, yd_store_receive_dtls b where  a.id=b.mst_id and a.status_active =1 and a.is_deleted =0 and b.status_active =1 and b.is_deleted =0 and a.id=$update_id and a.entry_form=640 group by a.id, a.party_id, b.job_no, a.yd_receive, b.pro_type, b.order_type, b.order_no, b.job_no , b.rcv_no, b.rcv_id, b.job_id";
		}
		else
		{ 
	...
	?>
        <tr style="cursor:pointer" id="row_<?php echo $tblRow;?>" bgcolor="<? echo $bgcolor; ?>" onClick="load_delivery_data('<?php echo $data[csf('job_no')];?>','<?php echo $data[csf('yd_receive')];?>',<?php echo $data[csf('pro_type')];?>,<?php echo $data[csf('order_type')];?>,'<?php echo $data[csf('order_no')];?>','<?php echo $data[csf('store_receive')];?>',<?php echo $data[csf('rcv_id')];?>,<?php echo $data[csf('job_id')];?>)">
        	<td width="35" align="center"><?php echo $tblRow;?></td>
            <td align="center" width="150">
            	<?php echo $party_name;?>
            </td>
			...
			<form name="searchorderfrm_<?php echo $tblRow;?>" id="searchorderfrm_<?php echo $tblRow;?>" autocomplete="off">
				<table cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center" style="width: 100%;">
		            ...
		            <tbody>
                		<tr class="general">
                			<td>
		                        <?php echo create_drop_down('cbo_company_name', 120, "select id, company_name from lib_company where status_active=1 and is_deleted=0 order by company_name", 'id,company_name', 1, '-- Select Company --', $company, "load_drop_down( 'yd_delivery_entry_v2_controller', this.value+'_'+document.getElementById('cbo_within_group').value, 'load_drop_down_party', 'party_td' );"); ?>
		                    </td>
		                    <td> 
	                            <?php echo create_drop_down('cbo_within_group', 80, $yes_no, '', 1, '-- Select Within Group --', $within_group, "load_drop_down( 'yd_delivery_entry_v2_controller', document.getElementById('cbo_company_name').value+'_'+this.value, 'load_drop_down_party', 'party_td' );",0); ?>
	                        </td>
	                        <td id="party_td"> 
	                            <?php
								
									if($within_group==1 && $company!=0)
                                    {

                                        echo create_drop_down( "cbo_party_name", 120, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 order by comp.company_name","id,company_name", 1, "-- Select Party --", $party_name, "",1);
                                    }
                                    elseif($within_group==2 && $company!=0)
                                    {
                                        echo create_drop_down( "cbo_party_name", 120, "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data[0]' $buyer_cond  and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (2,3)) order by buyer_name","id,buyer_name", 1, "-- Select Party --",$party_name, "",1 );
                                    }
                                    else
                                    {
...
if ($action=="embl_delivery_print")
{
	...
	$sql_mst = "select a.id, a.company_id, a.location_id, a.within_group, a.party_id, a.party_location, a.order_no, a.pro_type, a.order_type, a.receive_date, a.yd_receive, a.inserted_by, a.remarks, a.driver_name,a.vehicle_no from yd_store_receive_mst a where a.id='$data[1]' and a.status_active=1 and a.is_deleted=0 and a.entry_form=640";
    ...
    <div style="width:1220px;">
        ...
        <table width="800" cellpadding="5" cellspacing="0" border="1" rules="all" >  
			...
			<tr>
				...
				<td width="250"><?php echo $dataArray[0][csf('driver_name')]; ?></td>
				<td width="150"><strong> Vahicle No:</strong></td>
				<td><?php echo $dataArray[0][csf('vehicle_no')]; ?></td>
			</tr>
			<tr>
            	...
            	<?php
            		...
            		foreach ($sql_res as $row) 
					{
						...
						<tr>
							...
							<td align="center" width="100"><?php echo $row[csf("gray_lot")]; ?></td>
			                ...
			                <td align="right" width="100">
			                	<?php 
									echo number_format($row[csf("RCV_GREY_QTY")],2) ;
						...
						$grand_tot_total_order_quantity_qty += $row[csf("RCV_GREY_QTY")];
