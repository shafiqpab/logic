...
foreach($sql_company as $row)
{
	foreach($entry_form_wise_mrr_tble as $entry_form=>$table_name)
	{
		if($entry_form==502)
		{
			if($db_type==0) $insert_year_cond=" and year(insert_date)=$year_global"; else $insert_year_cond=" and to_char(insert_date,'YYYY')=$year_global";
			
			$mrr_prefix_sql=sql_select("select max(barcode_suffix_no) as max_prefix_num from pro_roll_details where status_active=1  and entry_form=2  $insert_year_cond");
			
			if($mrr_prefix_sql[0][csf("max_prefix_num")]<1) $max_mrr_prefix_num=0;
			else  $max_mrr_prefix_num=$mrr_prefix_sql[0][csf("max_prefix_num")];
			...
			if($roll_check[$table_name]=="")
			{
				...
				if($db_type==0)
				{
					...
					$data_arr.="('".strtoupper($table_name)."','".$max_mrr_prefix_num."',0,2,$year_global,0,0,0,0,0)";
				}
				else
				{
					$data_arr[]="('".strtoupper($table_name)."','".$max_mrr_prefix_num."',0,2,$year_global,0,0,0,0,0)";
		...
		else
		{
			...
			if($db_type==0) $insert_year_cond=" and year(insert_date)=$year_global"; else $insert_year_cond=" and to_char(insert_date,'YYYY')=$year_global";
			$mrr_prefix_sql=sql_select("select max(".$mrr_table_wise_prefix_num[$table_name].") as max_prefix_num from $table_name where status_active=1 and company_id=".$row[csf("id")]." $entry_form_cond $insert_year_cond");
			...
			{
				if($data_arr!="") $data_arr.=",";
				$data_arr.="('".strtoupper($table_name)."','".$max_mrr_prefix_num."','".$row[csf("id")]."','".$entry_form."',$year_global,0,0,0,0,0)";
			}
			else
			{
				$data_arr[]="('".strtoupper($table_name)."','".$max_mrr_prefix_num."','".$row[csf("id")]."','".$entry_form."',$year_global,0,0,0,0,0)";
...
else
{
	...
	$mrrWiseSeq= sql_insert2("platform_sequence_pk", $field_arr, $data_arr, 1,0);
	if ($mrrWiseSeq) 
	{
		oci_commit($con);
		echo "Data Save Successfully";
