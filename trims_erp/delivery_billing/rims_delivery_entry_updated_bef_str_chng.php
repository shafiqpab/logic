...
<script>
		...
		else
		{
			...
			else
			{
				...
				$("#tbl_dtls_emb tbody tr").each(function()
				{
					...
					{
						data_all += "&txtCurQty_" + j + "='" + txtCurQty +"'&txtOrderQuantity_" + j + "='" + txtOrderQuantity +"'&txtRemarksDtls_" + j + "='" + txtRemarksDtls +"'&hdnDtlsUpdateId_" + j + "='" + hdnDtlsUpdateId +"'&cboStatus_" + j + "='" + cboStatus+"'&cboshipingStatus_" + j + "='" + cboshipingStatus +"'&hdn_break_down_id_" + j + "='" + hdn_break_down_id+ "'";
						j++; i++;
					}
				});	
			}
	...
	function cal_values(rowNo)
	{
		...
		//document.getElementById("demo").innerHTML = x.toFixed(0)
		...
		}if(txtDelvBalance>0 && txtDelvBalance!=totaldoqnty)
		{
			$('#cboshipingStatus_'+rowNo).val(2);
		}if(txtDelvBalance>0 && txtDelvBalance==txtWorkOrderQuantity)
		{
