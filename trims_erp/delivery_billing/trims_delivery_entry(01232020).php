...
<script>
	...
	function openmypage_devivery_workorder()
	{
		...
		emailwindow.onclose=function()
		{
			...
			{//alert(theemail.value);
				...
				{
					show_list_view(1+'_'+ex_data[1]+'_'+within_group+'_'+received_id,'po_wise_dalivery_order_dtls_list_view','emb_details_container','requires/trims_delivery_entry_controller','setFilterGrid(\'list_view\',-1)');	
	...
	function openmypage_delevery_qnty(row)
	{
		var within_group    = $("#cbo_within_group").val();
		...
		var subSection 			= $('#cboSection_'+row).val();
		var recv_id 		= $('#received_id').val();
		var order_id 		= $('#hid_order_id').val();
		var JobDtlsId 		= $('#hdnJobDtlsId_'+row).val();
		var ReceiveDtlsId 	= $('#hdnReceiveDtlsId_'+row).val();
		var cbo_company_name 	= $('#cbo_company_name').val();
		var cbo_party_name 		= $('#cbo_party_name').val();
		var buyerPoId 		= $('#txtbuyerPoId_'+row).val();
		var buyerPo 			= $('#txtbuyerPo_'+row).val();
		var ProductionDtlsId = $('#hdnProductionDtlsId_'+row).val();
		var ItemGroup 		= $('#cboItemGroup_'+row).val();
		var WorkOrder 		= $('#txtWorkOrder_'+row).val();
		var WorkOrderID 		= $('#txtWorkOrderID_'+row).val();
		var prodQuantity 	= $('#txtOrderQuantity_'+row).val();
		...
		var txt_gate_pass_no 	= $('#txt_gate_pass_no').val();
		var update_id 			= $('#update_id').val();
		var cboshipingStatus 	= $('#cboshipingStatus_'+row).val();
		var txt_remarks 		= $('#txt_remarks_'+row).val();
		var txtstyleRef 		= $('#txtstyleRef_'+row).val();
		var txtbuyer 			= $('#txtbuyer_'+row).val();
		
		var cboUom 				= $('#cboUom_'+row).val();
		var txtPrevQty 			= $('#txtPrevQty_'+row).val();
		var txtCurQty 			= $('#txtCurQty_'+row).val();
		var txtClaimQty 		= $('#txtClaimQty_'+row).val();
		var txtRemarksDtls 		= $('#txtRemarksDtls_'+row).val();
		var txtcolor 				= $('#txtcolor_'+row).val();
		var txtsize 				= $('#txtsize_'+row).val();
		var txtWorkOrderQuantity 	= $('#txtWorkOrderQuantity_'+row).val();
		var txtDelvBalance 			= $('#txtDelvBalance_'+row).val();
		var cboStatus 				= $('#cboStatus_'+row).val();
		var cboshipingStatus 		= $('#cboshipingStatus_'+row).val();
		var hdnbookingDtlsId 	= $('#hdnbookingDtlsId_'+row).val();
		var hdn_break_down_rate 	= $('#hdn_break_down_rate_'+row).val();
		var hdn_break_down_id 		= $('#hdn_break_down_id_'+row).val();
		//..............................................................................//

		var page_link = 'requires/trims_delivery_entry_controller.php?within_group='+within_group+'&order_no='+order_no+'&data_break='+data_break+'&hdnDtlsUpdateId='+hdnDtlsUpdateId+'&colorID='+colorID+'&sizeID='+sizeID+'&txtItem='+txtItem+'&subSection='+subSection+'&recv_id='+recv_id+'&order_id='+order_id+'&JobDtlsId='+JobDtlsId+'&ReceiveDtlsId='+ReceiveDtlsId+'&buyerPoId='+buyerPoId+'&buyerPo='+buyerPo+'&ProductionDtlsId='+ProductionDtlsId+'&ItemGroup='+ItemGroup+'&WorkOrder='+WorkOrder+'&WorkOrderID='+WorkOrderID+'&prodQuantity='+prodQuantity+'&action=delevery_qty_popup';
		//alert(page_link);

		emailwindow=dhtmlmodal.open('EmailBox','iframe',page_link,'Quantity Details Pop-up', 'width=870px, height=400px, center=1, resize=0, scrolling=0','../')
		emailwindow.onclose=function()
		{
	...
	{
	   var status = return_global_ajax_value(company, 'chk_po_level_variable', '', 'requires/trims_delivery_entry_controller').trim();
	   status = status.split("**");
