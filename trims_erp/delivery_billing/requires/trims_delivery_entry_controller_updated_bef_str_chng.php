...
if( $action=='dalivery_order_dtls_list_view' ) 
{
	...
	if($data[0]==1)
	{
		foreach($rcv_result as  $row)
		{
			...
			//$orderQuantity= number_format($row[csf('order_quantity')],0,'.','');
			...
			if($del_bal*1==0)
			{$shipStatus=3;}
        	//echo $orderQuantity."ma".$row[csf('order_quantity')]."==".$del_bal."==".$shipStatus;
			?>
			<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
				<td title="<? echo $row[csf('buyer_po_no')]; ?>"><? echo $row[csf('buyer_po_no')]; ?></td>
				<td><? echo $trims_section[$row[csf('section')]]; ?></td>
				<td><? echo $trims_groups_arr[chop($row[csf('id')],",")]['item_group']; ?></td>		
				<td title="<? echo $row[csf('description')]; ?>"><? echo $row[csf('description')]; ?></td>
				...
				<td><input id="txtWorkOrderQuantity_<? echo $tblRow;?>" name="txtWorkOrderQuantity[]" value="<? echo number_format($orderQuantity,4,'.',''); ?>" class="text_boxes_numeric" type="text"  style="width:60px" readonly /></td>
				<td title=""><input id="txtOrderQuantity_<? echo $tblRow; ?>" name="txtOrderQuantity[]" value="<? echo number_format($convertedBQty,4,'.',''); ?>" class="text_boxes_numeric" type="text"  style="width:60px" 
					 placeholder=" " readonly /></td>
				<td><input id="txtPrevQty_<? echo $tblRow; ?>" name="txtPrevQty[]" value="<? if($delevery_qty_trims_arr[$row[csf("break_id")]]['delevery_qty']!=''){echo number_format($delevery_qty_trims_arr[$row[csf("break_id")]]['delevery_qty'],4);}else{echo "0";} ?>" type="text"  class="text_boxes_numeric" style="width:60px" readonly /></td>
				...
				<td><input id="txtDelvBalance_<? echo $tblRow; ?>" name="txtDelvBalance[]"  value="<? echo number_format($del_bal,4,'.','');// $del_bal ; ?>"   type="text" style="width:70px"  class="text_boxes_numeric" readonly /></td> 
				<td title="<? echo $shipStatus;?>"><? echo create_drop_down( "cboshipingStatus_$tblRow", 80, $delivery_status,"", 0, "--  --", $shipStatus,"", 1,'','','','','','',"cboshipingStatus[]"); ?>
				</td>
				...
				<td><? echo create_drop_down( "cboStatus_".$tblRow, 60, $row_status,"", 1, "-- Select --",$selected,"", 1,'','','','','','',"cboStatus[]"); ?>	
				</td>
			</tr>
			<?
		}
	...
	else
	{
		foreach($rcv_result as $row)
		{
			...
			//$shipStatus=$row[csf('delevery_status')];
			//if($row[csf('order_quantity')]==$del_bal) $shipStatus=1; 
	        //else if($row[csf('order_quantity')]>$del_bal) $shipStatus=2; 
	        //else if($del_bal==0) $shipStatus=3;
			...
			<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
				...
				<td title="<? echo $row[csf('buyer_po_no')]; ?>"><input id="txtbuyerPo_<? echo $tblRow; ?>" name="txtbuyerPo[]" value="<? echo $row[csf('buyer_po_no')]; ?>" class="text_boxes" type="text"  style="width:100px" disabled />
				...
				<td title="<? echo $row[csf('description')]; ?>"><input id="txtItem_<? echo $tblRow; ?>" name="txtItem[]" type="text" class="text_boxes" style="width:87px" value="<? echo $row[csf('description')]; ?>" placeholder="Display" readonly disabled/>
				...
				<td><input id="txtWorkOrderQuantity_<? echo $tblRow; ?>" name="txtWorkOrderQuantity[]" class="text_boxes_numeric" type="text"  value="<? echo   number_format($orderQuantity,4,'.',''); ?>" style="width:60px" placeholder="" readonly /></td>
				<td title=""><input id="txtOrderQuantity_<? echo $tblRow; ?>" name="txtOrderQuantity[]" value="<? echo number_format($convertedBQty,4,'.',''); ?>" class="text_boxes_numeric" type="text"  style="width:60px" 
					 placeholder=" " readonly /></td>
				<td><input id="txtPrevQty_<? echo $tblRow; ?>" name="txtPrevQty[]"  value="<? echo number_format($CumDelvQty,4); ?>"  type="text"  class="text_boxes_numeric" style="width:60px" disabled /></td>
				...
				<td><input id="txtDelvBalance_<? echo $tblRow; ?>" name="txtDelvBalance[]" value="<? echo number_format($del_bal,4,'.','');//$del_bal;; ?>"  type="text" style="width:70px"  class="text_boxes_numeric" readonly /></td>
				<td>
					<? echo create_drop_down( "cboshipingStatus_$tblRow", 80, $delivery_status,"", 0, "--  --", $shipStatus,"", 1,'','','','','','',"cboshipingStatus[]");?>	
				</td> 
				<td><input id="txtRemarksDtls_<? echo $tblRow; ?>" name="txtRemarksDtls[]" value="<? echo $row[csf('remarks')]; ?>" type="text"  class="text_boxes" style="width:77px" />
					...
					<input id="hdnDtlsUpdateId_<? echo $tblRow; ?>" name="hdnDtlsUpdateId[]" type="hidden" value="<? echo  $row[csf('delDtlsId')];//$updateDtlsId; ?>" class="text_boxes_numeric" style="width:40px" />
					...
					<input type="hidden" id="hdnReceiveDtlsId_<? echo $tblRow; ?>" name="hdnReceiveDtlsId[]" value="<? echo $row[csf('id')]//$production_arr[$row[csf("break_id")]]['receive_dtls_id']; //$row[csf('id')] ; ?>">
					<input type="hidden" id="hdnJobDtlsId_<? echo $tblRow; ?>" name="hdnJobDtlsId[]" value="<? echo $production_arr[$row[csf("break_id")]]['job_dtls_id'];//$production_arr[$row[csf("break_id")]]['job_dtls_id']; ?>">
					<input type="hidden" id="hdnProductionDtlsId_<? echo $tblRow; ?>" name="hdnProductionDtlsId[]" value="<? echo chop($production_arr[$row[csf("break_id")]]['id'],","); ?>">
					<input type="hidden" id="hdn_break_down_rate_<? echo $tblRow; ?>" name="hdn_break_down_rate[]" value="<? echo $row[csf('rate')];  ?>">
					<input type="hidden" id="hdn_break_down_id_<? echo $tblRow; ?>" name="hdn_break_down_id[]" value="<? echo $row[csf('break_id')]; ?>">
				</td>
				</td>
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Start Here
	{
		...
		foreach ($order_result as $rows)
		{
			...
			$order_dtls_arr[$rows[csf("break_id")]]['item_group']		=$rows[csf("item_group")];
		...
		{	
			...
			//$txtDelvBalance 		= "txtDelvBalance_".$i;
	...
	else if ($operation==1)   // Update Here
	{
		...
		$field_array2="received_id*booking_dtls_id*receive_dtls_id*job_dtls_id*production_dtls_id*order_id*order_no*buyer_po_id*buyer_po_no*buyer_style_ref*buyer_buyer*section*item_group*order_uom*order_quantity*delevery_qty*claim_qty*remarks*description*color_id*size_id*color_name*size_name*delevery_status*workoder_qty*order_receive_rate* break_down_details_id*status_active*updated_by*update_date";

		$field_array3="delivery_status*updated_by*update_date";
		$field_array4="id, mst_id, received_id, booking_dtls_id, receive_dtls_id, job_dtls_id, production_dtls_id,  order_id, order_no, buyer_po_id, buyer_po_no,  buyer_style_ref, buyer_buyer, section,   item_group, order_uom, order_quantity, delevery_qty, claim_qty, remarks,description, color_id, size_id,color_name,size_name,delevery_status,workoder_qty,order_receive_rate, break_down_details_id, inserted_by, insert_date";
		$field_array5="delivery_status";
		$id1=return_next_id( "id", "trims_delivery_dtls",1) ;
		$add_comma=0;	$flag=1;
		for($i=1; $i<=$total_row; $i++)
		{	
			...
			$hdn_break_down_rate	= "hdn_break_down_rate_".$i;
			$hdn_break_down_id	    = "hdn_break_down_id_".$i;
		...
		if($data_array2!="")
		{
			//echo "10**".bulk_update_sql_statement( "trims_delivery_dtls", "id",$field_array2,$data_array2,$hdn_dtls_id_arr);ie;
...
if($action=="challan_print") 
{
    ...
    <div style="width:1200px" class="page-break">
		...
		<table class="rpt_table" width="100%" cellspacing="1" >
            <tr>
                <td valign="top" width="100"><strong> Delevery To</strong></td>
      	...
      	<table  class="rpt_table" width="100%" cellspacing="1" rules="all" border="1">
      		<thead>
	            <tr>
	                ...
	                <th width="70">Curr. Delv Qty</th>
	                <th width="70">Delv Balance Qty</th>
	                <th>Remarks</th>
	            </tr>
            </thead>
            <tbody>
	...
	{  echo $buyer_arr[$row[csf('buyer_buyer')]]; } else { echo $row[csf('buyer_buyer')];  } ?></p></td>
                ...
                <td align="right"><?php echo $row[csf('workoder_qty')]-($row[csf('delevery_qty')]+$CumDelvQty); $delevery_Balance_quantity += $row[csf('workoder_qty')]-($row[csf('delevery_qty')]+$CumDelvQty);  ?></td>
