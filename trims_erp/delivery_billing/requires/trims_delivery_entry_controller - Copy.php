        ...
        <table width="940" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tbody>
                <tr class="general">
                    ...
                    <td>
						<?
                           $search_by_arr=array(1=>"Receive System ID",2=>"W/O No",4=>"Buyer Po",5=>"Buyer Style",6=>"F.Trims Receive ID");
...
{	
	...
	if($chk_fTrims==1){
		$sql= "select  a.id, a.subcon_job,a.job_no_prefix_num, a.company_id, a.wo_id, a.receive_date, a.challan_date, a.challan_no, a.store_id, a.receive_basis, $ins_year_cond as year from trims_receive_mst a , trims_receive_dtls b where a.id=b.mst_id and a.entry_form=451 and a.status_active=1 and b.status_active=1 $order_rcv_date $company order by a.id DESC";
		...
		if($variable_status==2)
	 	{
			...
			where a.entry_form=255 and a.subcon_job=b.job_no_mst and a.id=b.mst_id and a.status_active=1 and b.status_active=1 and c.status_active=1 $order_rcv_date $company $party_id_cond $search_com_cond  $withinGroup $year_cond and b.id=c.mst_id and b.job_no_mst=c.job_no_mst
			...
			where a.entry_form=255 and a.subcon_job=b.job_no_mst and a.id=b.mst_id and a.status_active=1 and b.status_active=1 and c.status_active=1 and a.id=d.received_id and d.status_active=1 $order_rcv_date $company $party_id_cond $search_com_cond  $withinGroup $year_cond and b.id=c.mst_id  
			group by a.id, a.subcon_job, a.job_no_prefix_num, a.insert_date, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date
			order by a.id DESC";
	 	}
	}
        ...
        <tbody>
            ...
            {  
               ...
               /* if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
                ...
                ?>
                <tr bgcolor="<? echo $bgcolor; ?>" onClick='js_set_value("<? echo $row[csf('id')].'_'.$row[csf('subcon_job')].'_'.$chk_fTrims.'_'.$row[csf("receive_basis")]; ?>")' style="cursor:pointer" >
...
if ($action=="load_php_rcv_data_to_form")
{
	...
	foreach ($nameArray as $row)
	{
		...
		if($receive_basis ==7)
		{
			...
			//echo $subcon_rcv_order_Array[$row[csf("receive_basis")]][$row[csf("wo_id")]]['wo_no'].'=='.$row[csf("receive_basis")].'=='.$row[csf("wo_id")];
			$order_no=$subcon_rcv_order_Array[$row[csf("receive_basis")]][$row[csf("wo_id")]]['wo_no'];
		}
		...
		echo "document.getElementById('txt_receive_basis').value 		= '".$rcvBasis."';\n";
		echo "document.getElementById('cbo_company_name').value 		= '".$row[csf("company_id")]."';\n";
...
if( $action=='dalivery_order_dtls_list_view' ) 
{
	...
	if($data[5]==1){
		...
		if($data[6]==7){
			...
			where b.id in ($wo_dtls_id) and b.status_active=1";
	...
	if($data[5]==1) // F.Trims Received
	{
		foreach($wo_arr as $rcv_id=> $rcv_id_data)
		{
			...
			{ 
				...
				<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
					...
					<td align="right" id="txtOrderQuantity_<? echo $tblRow; ?>" title="<?= number_format($rcv_qty,4,'.','');?>"><? 
					
					echo number_format($rcv_qty,4,'.',''); ?></td>
		...
		else
		{
			...
			foreach($rcv_result as $row)
			{
				...
				$del_bal=($overConvertedBQty-($cumDelvQty+$row[csf('delevery_qty')]));
...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Start Here
	{
		...
		if($is_fabric_trims==1){ // Fabric Trims
			if($txt_receive_basis!=7){ // NOT Requisition basis
				$sql="select a.id as break_id, a.job_no_mst, a.description, a.color_id, a.size_id, a.qnty, a.rate, a.amount, a.order_rcv_break_id, a.order_rcv_id ,b.id as dtls_id ,b.order_quantity as wo_qnty, b.order_uom, b.rate, b.amount, b.section, b.sub_section, b.item_group  as trim_group , b.order_rcv_dtls_id, b.order_rcv_id, a.mst_id, c.rcv_qty, c.remarks, c.wo_id, c.wo_dtls_id, c.wo_break_id, c.order_rcv_id  from trims_subcon_ord_breakdown a , trims_subcon_ord_dtls b, trims_receive_dtls c where a.job_no_mst=b.job_no_mst and b.id=a.mst_id and a.id=c.wo_break_id and b.mst_id=c.wo_id and b.order_rcv_id in ( $order_received_id ) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 order by c.id";
