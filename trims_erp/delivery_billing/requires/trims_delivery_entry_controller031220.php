        ...
        <table width="940" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
            ...
            <tbody>
                <tr class="general">
                    ...
                    <td>
                        <?php echo create_drop_down( "cbo_within_group", 100, $yes_no,"", 0, "--  --", $data[3], "fnc_load_party_popup(1,this.value);",$data[5] ); ?>
                    </td>
                    <td id="buyer_td">
...
{	
	...
	//echo $variable_status ; die;
  	if($variable_status==2)
 	{
		...
		where a.entry_form=255 and a.subcon_job=b.job_no_mst and a.id=b.mst_id and a.status_active=1 and b.status_active=1 $order_rcv_date $company $party_id_cond $search_com_cond  $withinGroup $year_cond and b.id=c.mst_id  
...
if( $action=='dalivery_order_dtls_list_view' ) 
{
	...
	if($data[0]==1)
	{
		$sql = "select a.id, a.mst_id, a.job_no_mst, a.order_id, a.order_no, a.buyer_po_id, a.booking_dtls_id,b.id as receive_dtls_id, b.qnty as order_quantity , a.order_uom,a.booked_uom, a.booked_conv_fac, b.rate, a.amount, a.delivery_date, a.buyer_po_no, a.buyer_style_ref, a.buyer_buyer, a.section, a.item_group as trim_group, a.rate_domestic,  a.amount_domestic,b.item_id, b.color_id, b.size_id,b.description,b.id as break_id,b.book_con_dtls_id,a.source_for_order from subcon_ord_dtls a,subcon_ord_breakdown b where   a.id=b.mst_id and a.job_no_mst=b.job_no_mst and a.job_no_mst='$data[1]' and a.mst_id='$data[3]'  and a.order_quantity<>0 and a.booked_qty<>0 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0  order by a.id ASC";
	}
	else
	{
		$sql = "select a.id,a.mst_id,a.job_no_mst,a.order_id,a.order_no,a.buyer_po_id,a.booking_dtls_id,b.id AS receive_dtls_id,b.qnty AS order_quantity,a.order_uom,a.booked_uom,a.booked_conv_fac,b.rate,a.amount,a.delivery_date,a.buyer_po_no,a.buyer_style_ref,a.buyer_buyer,a.section,a.item_group AS trim_group,a.rate_domestic,a.amount_domestic,b.item_id,b.color_id,b.size_id,b.description,b.id AS break_id,b.book_con_dtls_id,a.source_for_order ,c.delevery_qty,c.claim_qty,c.id AS delDtlsId,c.size_name,c.color_name,c.remarks,c.status_active,c.no_of_roll_bag FROM subcon_ord_dtls a ,subcon_ord_breakdown b LEFT JOIN trims_delivery_dtls c ON c.break_down_details_id = b.id AND c.mst_id ='$data[1]' and c.status_active<>0 WHERE a.id = b.mst_id and a.job_no_mst=b.job_no_mst and a.mst_id='$data[3]'  and b.status_active=1 and b.is_deleted=0  ";
	}
	//echo $sql;
	$rcv_result =sql_select($sql); 
	$break_qty_arr=array();
	...
	foreach ($trim_sql_res as $row){
		$trims_groups_arr[$row[csf("id")]]['item_group']=$item_group_arr[$row[csf("item_group")]];
		$trims_groups_arr[$row[csf("id")]]['item_group_id']=$row[csf("item_group")];
		$trims_groups_arr[$row[csf("id")]]['buyer_buyer']=$row[csf("buyer_buyer")];
		$trims_groups_arr[$row[csf("id")]]['booked_conv_fac']=$row[csf("booked_conv_fac")];
	}
	unset($trim_sql_res);
	...
	if($data[0]==1)
	{
		foreach($rcv_result as  $row)
		{
			...
			if($row[csf('source_for_order')]==2){
				...
				//.'='.$row[csf('section')].'='.$trims_groups_arr[chop($row[csf('id')],",").'='.'item_group_id'].'='.$row[csf('description')].'='.$row[csf('color_id')].'='.$row[csf('size_id')].'='.$row[csf('order_uom')]].'+';
				//$rec_qty_arr[$row[csf("mst_id")]][$row[csf("break_id")]]["cum_qty"] ;
			}
			...
			//echo $orderQuantity.'=='.$delevery_qty_trims_arr[$row[csf("break_id")]]['delevery_qty'].'++';
			
			//$del_bal=($OverorderQuantity-($delevery_qty_trims_arr[$row[csf("break_id")]]['delevery_qty']));
			
			
			//echo $del_bal.'=='.$delevery_qty_trims_arr[$row[csf("break_id")]]['delevery_qty'].'++';
			//echo $del_bal.'=='.$OverorderQuantity.'++';
			/*if(($OverorderQuantity*1==$del_bal*1)){
				$shipStatus=1;
			} else if($OverorderQuantity*1>$del_bal*1){
				$shipStatus=2; 
			} else if($del_bal*1==0 || ( $del_bal <= $overQty )){
				$shipStatus=3;
			} */
			//echo $shipStatus;
			//echo $overQty.'==';0
			//echo $del_bal."==".$del_bal*1.'++';  

			//if($del_bal==0 || ( $del_bal <= $overQty ))
			...
			<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
				...
				<td><? echo $trims_section[$row[csf('section')]]; ?></td>
				<td><? echo $trims_groups_arr[chop($row[csf('id')],",")]['item_group']; ?></td>		
				<td><? echo trim($row[csf('description')]); ?></td>
				<td><? echo $color_library[$row[csf('color_id')]] ?></td>
				<td><? echo $size_arr[$row[csf('size_id')]] ?></td>
				<td><? echo $unit_of_measurement[$row[csf('order_uom')]] ?></td>
				<td align="right" id="txtWorkOrderQuantity_<? echo $tblRow; ?>"  title="<?= $orderQuantity; ?>"><? echo number_format($orderQuantity,4,'.',''); ?></td>
				<td align="right" id="txtOrderQuantity_<? echo $tblRow; ?>" title="<?= number_format($overConvertedBQty,4,'.','');?>"><? 
				...
				<td align="right"><input id="txtCurQty_<? echo $tblRow; ?>" name="txtCurQty[]" onKeyUp="cal_values(<? echo $tblRow; ?>);" value="<? echo number_format($overConvertedBQty-$prev_qty,4,'.','') ?>" type="text" style="width:70px"  class="text_boxes_numeric"  /></td>
                <td align="right"><input id="noOfRollBag_<? echo $tblRow; ?>" name="noOfRollBag[]" value="" type="text" style="width:70px"  class="text_boxes"  /></td>
				<td align="right" id="txtDelvBalance_<? echo $tblRow; ?>" title="<?= number_format($del_bal,4,'.','');?>"> <? echo number_format($del_bal,4,'.','');// $del_bal ; ?></td> 
				<td id="cboshipingStatus_<? echo $tblRow; ?>" title="<?= $shipStatus;?>"><? echo $delivery_status[$shipStatus]; ?></td>
				<td><input id="txtRemarksDtls_<? echo $tblRow; ?>" name="txtRemarksDtls[]" value="<? echo $remarks; ?>" type="text" class="text_boxes" style="width:77px" />
	...
	else
	{
		foreach($rcv_result as $row)
		{
			...
			$CumDelvQty=($delevery_qty_trims_arr[$row[csf("break_id")]]['delevery_qty']-$row[csf('delevery_qty')]);
			//$del_bal=($OverorderQuantity-($delevery_qty_trims_arr[$row[csf("break_id")]]['delevery_qty']));
			//$del_bal=($OverorderQuantity-($CumDelvQty+$row[csf('delevery_qty')]));
			//19.908333=120.90833=1=100 
			//echo $del_bal."=".$OverorderQuantity."=".$CumDelvQty."=".$row[csf('delevery_qty')];
			//echo $OverorderQuantity.'=='.$delevery_qty_trims_arr[$row[csf("break_id")]]['delevery_qty'].'++';
			$overQty=$OverorderQuantity-$orderQuantity;
			/*if(($OverorderQuantity*1==$del_bal*1)) $shipStatus=1; 
			else if($OverorderQuantity*1>$del_bal*1) $shipStatus=2; 
			else if(($del_bal*1==0 && $overConvertedBQty !='' && $overConvertedBQty!=0) || ( $del_bal <= $overQty )) $shipStatus=3;*/
        	...
        	if($del_bal*1==0 || ( $del_bal <= $overQty )) $shipStatus=3;*/
			...
			<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
				...
				<td><input id="txtCurQty_<? echo $tblRow; ?>" name="txtCurQty[]" onKeyUp="cal_values(<? echo $tblRow; ?>);" value="<? echo number_format($row[csf('delevery_qty')],4,'.',''); ?> " type="text" style="width:70px"  class="text_boxes_numeric"  /></td>
                <td align="right"><input id="noOfRollBag_<? echo $tblRow; ?>" name="noOfRollBag[]" value="<? echo $row[csf('no_of_roll_bag')]; ?>" type="text" style="width:70px"  class="text_boxes"  /></td>
				<td align="right" id="txtDelvBalance_<? echo $tblRow; ?>" title="<?= number_format($del_bal,4,'.','');?>"> <? echo  $del_bal ; ?></td> 
	    ...
	    <div style="width:1500px" class="page-break">
	        ...
	        <br>
	      	<table  class="rpt_table" width="100%" cellspacing="1" rules="all" border="1">
				...
				$sql = "select a.id, a.mst_id, a.booking_dtls_id, a.receive_dtls_id, a.job_dtls_id, a.production_dtls_id, a.order_id, a.order_no, b.buyer_po_id, b.buyer_po_no,  a.buyer_style_ref, a.buyer_buyer, a.section, a.item_group as trim_group, a.order_uom, a.order_quantity,a.delevery_qty,a.claim_qty,a.remarks,a.color_id,a.size_id, a.description, a.delivery_status,a.color_name,a.size_name,a.workoder_qty,a.break_down_details_id,a.no_of_roll_bag,c.trims_del, c.delivery_date from trims_delivery_dtls a,subcon_ord_dtls b,trims_delivery_mst c  where a.mst_id in($data[1]) and  a.receive_dtls_id=b.id and a.mst_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 order by a.id ASC";
