...
<script>
	...
	function fnc_job_order_entry( operation )
	{
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+$('#cbo_within_group').val()+'*'+report_title+'*'+$('#txt_order_no').val()+'*'+$('#txt_dalivery_no').val(), "challan_print", "requires/trims_delivery_entry_controller") 
			//return;
			show_msg("3");
		}
		else
	...
	function openmypage_order_qnty11(type,receive_dtls_id,row)
	{
		var company 		= $('#cbo_company_name').val();
		var party_name 		= $('#cbo_party_name').val();
		var within_group 	= $('#cbo_within_group').val();
	...
	function cal_values(rowNo)
	{
		...
		$('#txtDelvBalance_'+rowNo).val(txtWorkOrderQuantity-(txtCurQty+txtPrevQty));
                    ...
                    <tbody id="emb_details_container">
                        <tr id="row_1">
                            ...
                            <td><input id="txtRemarksDtls_1" name="txtRemarksDtls[]" type="text"  class="text_boxes_numeric" style="width:77px" readonly="readonly" />
                              ...
                              <td><?   echo create_drop_down( "cboStatus_1", 90, $row_status,"",0, $selected,0,'','','','','','','','',"cboStatus[]");?>	</td>
