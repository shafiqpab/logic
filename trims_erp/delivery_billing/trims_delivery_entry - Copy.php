	...
	function openmypage_devivery_workorder()
	{
		...
		emailwindow.onclose=function()
		{
			...
			{//alert(theemail.value);
				...
				document.getElementById('txt_wo_type').value = (ex_data[2]);
				var within_group = $('#cbo_within_group').val();
				var received_id =$("#received_id").val();
				var company_name =$("#cbo_company_name").val();
				//alert(received_id);
				show_list_view(1+'_'+ex_data[1]+'_'+within_group+'_'+received_id+'_'+company_name+'_'+ex_data[2]+'_'+ex_data[3],'dalivery_order_dtls_list_view','emb_details_container','requires/trims_delivery_entry_controller','setFilterGrid(\'list_view\',-1)');				
...
function show_print_report(type)
{
	...
	else
	{
		...
		print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+$('#cbo_within_group').val()+'*'+report_title+'*'+$('#txt_order_no').val()+'*'+$('#txt_dalivery_no').val()+'*'+$('#cbo_template_id').val(), action, "requires/trims_delivery_entry_controller") 
		...
		else
		{
			...
			$("#tbl_dtls_emb").find('tbody tr').each(function() 
			{
				...
				if(txt_wo_type==1){
					...
					{
						data_all += "&txtCurQty_" + j + "='" + txtCurQty+"'&noOfRollBag_" + j + "='" + noOfRollBag+"'&txtRemarksDtls_" + j + "='" + txtRemarksDtls +"'&hdnDtlsUpdateId_" + j + "='" + hdnDtlsUpdateId +"'&cboStatus_" + j + "='" + cboStatus+"'&hdn_break_down_id_" + j + "='" + hdn_break_down_id +"'&cboshipingStatus_" + j + "='" + cboshipingStatus  +"'&txtWorkOrderQuantity_" + j + "='" + txtWorkOrderQuantity  +"'&txtOrderQuantity_" + j + "='" + txtOrderQuantity  +"'&txtPrevQty_" + j + "='" + txtPrevQty  +"'&txtDelvBalance_" + j + "='" + txtDelvBalance  +"'&txtBuyerPO_" + j + "='" + txtBuyerPO  +"'&txtBuyerStyle_" + j + "='" + txtBuyerStyle  +"'&txtSection_" + j + "='" + txtSection  +"'&txtTrimGroup_" + j + "='" + txtTrimGroup  +"'&txtStyle_" + j + "='" + txtStyle  +"'&txtDesc_" + j + "='" + txtDesc+"'&txtUom_" + j + "='" + txtUom  +"'&txtColor_" + j + "='" + txtColor  +"'&txtSize_" + j + "='" + txtSize  +"'&hdn_rcv_basis_" + j + "='" + hdn_rcv_basis+ "'";
						j++; total_row++;
					}
				}
				else{
			...
			<legend>Trims Delivery</legend>
                <table width="900" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td width="160"><? echo create_drop_down( "cbo_company_name", 150, "select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "-- Select Company --", $selected, "fnc_load_party(1,document.getElementById('cbo_within_group').value);"); ?>
                        </td>
                        <td width="110" class="must_entry_caption">Location Name</td>
                        <td width="160" id="location_td"><? echo create_drop_down( "cbo_location_name", 150, "select id, location_name from lib_location where status_active =1 and is_deleted=0 order by location_name","id,location_name", 1, "-- Select Location --", $selected, "" ); ?></td>
                ...
                <table width="1410" cellspacing="2" cellpadding="0" border="0">
                    <tr>
                        <td align="center" colspan="15" valign="middle" class="button_container">
                            ...
                            <input type="button" id="btn_print" value="Print2" class="formbutton" style="width:100px;" onClick="show_print_report(1);" >
                            <input type="button" id="btn_print2" value="Print3" class="formbutton" style="width:100px;" onClick="show_print_report(2);" >
