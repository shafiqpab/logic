...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Start Here
	{
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");  
				echo "0**".str_replace("'",'',$txt_job_no)."**".str_replace("'",'',$id)."**".str_replace("'",'',$hid_recv_id);
			}
			else
			{
				mysql_query("ROLLBACK"); 
				echo "10**".str_replace("'",'',$txt_job_no)."**".str_replace("'",'',$id)."**".str_replace("'",'',$hid_recv_id);
			}
		}
		else if($db_type==2)
		{
	...
	else if ($operation==1)   // Update Here
	{
		...
		if($db_type==0)
		{
			...
			{
				mysql_query("COMMIT");  
				echo "1**".str_replace("'",'',$txt_production_no)."**".str_replace("'",'',$update_id)."**".str_replace("'",'',$hid_recv_id);
			}
			else
			{
				mysql_query("ROLLBACK"); 
				echo "10**".str_replace("'",'',$txt_production_no)."**".str_replace("'",'',$update_id)."**".str_replace("'",'',$hid_recv_id);
			}
		}
		else if($db_type==2)
		{  
...
if( $action=='order_dtls_list_view' ) 
{
	...
	{
		$sql= "select a.id, a.mst_id, a.machine_id, a.item_description, a.color_id, a.size_id, a.uom,  a.impression, a.material_color_id, a.qty_reel,  a.total_head, a.qc_qty, a.production_qty, a.comp_prod, a.reject_qty, a.prod_time, a.remarks, a.booking_dtls_id, a.receive_dtls_id, a.job_dtls_id, a.book_con_dtls_id, a.break_id from trims_production_dtls a where a.mst_id=$data[1]";
	}
	...
	foreach($data_array as $row)
	{
		...
		else
		{
			...
			$prod_time=$row[csf('prod_time')];
			$jobDtls_id=$row[csf('job_dtls_id')];
			$prodDtls_id=$row[csf('id')];
			$qty=$row[csf('production_qty')];
			$qty_reel=$row[csf('qty_reel')];
			$total_head=$row[csf('total_head')];
			$comp_prod=$row[csf('comp_prod')];
			$reject_qty=$row[csf('reject_qty')];
			$remarks=$row[csf('remarks')];
			//$material_color = return_field_value("material_color", "trims_job_card_dtls", "status_active=1 and id=$jobDtls_id", "material_color");
			$material_color_id=$row[csf('material_color_id')];
			$item=$row[csf('item_description')];
			$sub_section=$jobDtls_arr[$jobDtls_id]['sub_section'];
			$material_color=$jobDtls_arr[$jobDtls_id]['material_color'];
        ...
        <tr id="row_<? echo $tblRow; ?>">
			...
			<td><input id="txtRejectQty_<? echo $tblRow; ?>" name="txtRejectQty[]" class="text_boxes_numeric" type="text"  style="width:47px" placeholder="Display" value="<? echo $reject_qty; ?>"  onKeyUp="cal_values(<? echo $tblRow; ?>);" /></td>
...
if($action=="row_metarial_popup")
{
	...
	{	
		...
		$sql= "select a.id, a.trims_job, a.job_no_prefix_num, $ins_year_cond as year, a.company_id, a.location_id, a.party_id,  a.order_no, a.received_id,a.received_no ,a.section_id
		 ...
		 where a.entry_form=257 and a.trims_job=b.job_no_mst and a.id=b.mst_id and a.status_active=1 and b.status_active=1 $delivery_date $company $buyer $withinGroup $search_com_cond $po_idsCond $withinGroup 
	...
	{	
		...
		if($search_type==1)
		{
			if($search_str!="")
			{
				if($search_by==1) $search_com_cond="and a.prod_no_prefix='$search_str'";
				else if($search_by==2) $search_com_cond="and a.order_no='$search_str'";
				
				if ($search_by==3) $job_cond=" and a.prod_no_prefix = '$search_str' ";
				else if ($search_by==4) $po_cond=" and b.po_number = '$search_str' ";
				else if ($search_by==5) $style_cond=" and a.style_ref_no = '$search_str' ";
			}
		}
		else if($search_type==4 || $search_type==0)
		{
			if($search_str!="")
			{
				if($search_by==1) $search_com_cond="and a.prod_no_prefix like '%$search_str%'";  
				else if($search_by==2) $search_com_cond="and a.order_no like '%$search_str%'";  
				
				if ($search_by==3) $job_cond=" and a.prod_no_prefix like '%$search_str%'";  
				else if ($search_by==4) $po_cond=" and b.po_number like '%$search_str%'"; 
				else if ($search_by==5) $style_cond=" and a.style_ref_no like '%$search_str%'";   
			}
		}
		...
		if($search_str!="")
		{
			$search_com_cond="and a.prod_no_prefix_num like '%$search_str'";
		}
		...
		/*$sql= "select a.id, a.trims_job, a.job_no_prefix_num, $ins_year_cond as year, a.company_id, a.location_id, a.party_id,  a.order_no,a.received_no ,a.section_id, b.item_description
		 from trims_job_card_mst a, trims_job_card_dtls b
		 where a.entry_form=257 and a.trims_job=b.job_no_mst and a.id=b.mst_id and a.status_active=1 and b.status_active=1 $production_date $company $buyer $withinGroup $search_com_cond $po_idsCond $withinGroup 
		 group by a.id, a.trims_job, a.job_no_prefix_num, a.insert_date, a.company_id, a.location_id, a.party_id, a.order_no ,a.received_no ,a.section_id, b.item_description
		 order by a.id DESC";*/
		 ...
		 where a.entry_form=269 and a.id=b.mst_id and a.status_active=1 and b.status_active=1 $production_date $company $buyer $withinGroup $search_com_cond $po_idsCond $withinGroup 
	     ...
	     <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="685" >
	        ...
	        <tbody>
	            ...
	            {  
	                if ($i%2==0) $bgcolor="#E9F3FF"; else $bgcolor="#FFFFFF";
	                ?>
	                <tr bgcolor="<? echo $bgcolor; ?>" onClick='js_set_value("<? echo $row[csf('id')].'_'.$row[csf('trims_production')].'_'.$row[csf('received_id')]; ?>")' style="cursor:pointer" >
	                    <td width="30"><? echo $i; ?></td>
