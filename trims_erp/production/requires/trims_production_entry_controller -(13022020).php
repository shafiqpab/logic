...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Start Here
	{
		...
		$field_array2="id, mst_id, machine_id, item_description,color_id, size_id, uom,  impression, material_color_id, qty_reel,  total_head, qc_qty, production_qty, comp_prod, reject_qty, prod_time, remarks, booking_dtls_id, receive_dtls_id, job_dtls_id, break_id, production_level, inserted_by, insert_date";
		...
		{	
			...
			$data_array2 .="(".$id1.",".$id.",".$$cboMachineName.",".$$txtItem.",".$$txtcolor.",".$$txtsize.",".$$cboUom.",".$$txtImpression.",".$$cborawColor.",".$$txtQtyReel.",".$$txtTotalHead.",".str_replace(",",'',$$txtQcPassQty).",".str_replace(",",'',$$txtProdQty).",".$$txtCompProd.",".$$txtRejectQty.",".$$cboProdTime.",".$$txtRemarks.",".$$hdnbookingDtlsId.",".$$hdnRcvDtlsId.",".$$hdnjobDtlsId.",".$$hdnBreakIDs.",'".$variable_status."','".$user_id."','".$pc_date_time."')";
	...
	else if ($operation==1)   // Update Here
	{
		...
		$field_array2="machine_id*color_id*size_id*uom*impression*material_color_id*qty_reel*total_head*qc_qty*production_qty*comp_prod*reject_qty*prod_time*remarks*production_level*updated_by*update_date";
		$field_array3="id, mst_id, machine_id, item_description,color_id, size_id, uom,  impression, material_color_id, qty_reel,  total_head, qc_qty, production_qty, comp_prod, reject_qty, prod_time, remarks, booking_dtls_id, receive_dtls_id, job_dtls_id, break_id, production_level, inserted_by, insert_date";
		...
		{			
			...
			if(str_replace("'",'',$$hdnDtlsUpdateId)!=""){
				if(str_replace(",",'',$$txtCompProd)<str_replace(",",'',$$hdnCumDelQty)){
					echo "555**".$$hdnCumDelQty; die;
				} else {
					$aa	=str_replace("'",'',$$hdnDtlsUpdateId);
					$data_array2[$aa]=explode("*",("".$$cboMachineName."*".$$txtcolor."*".$$txtsize."*".$$cboUom."*".$$txtImpression."*".$$cborawColor."*".str_replace(",",'',$$txtQtyReel)."*".str_replace(",",'',$$txtTotalHead)."*".str_replace(",",'',$$txtQcPassQty)."*".str_replace(",",'',$$txtProdQty)."*".str_replace(",",'',$$txtCompProd)."*".str_replace(",",'',$$txtRejectQty)."*".$$cboProdTime."*".$$txtRemarks."*".$variable_status."*".$user_id."*'".$pc_date_time."'"));
				...
				$data_array3 .="(".$id1.",".$update_id.",".$$cboMachineName.",".$$txtItem.",".$$txtcolor.",".$$txtsize.",".$$cboUom.",".$$txtImpression.",".$$cborawColor.",".$$txtQtyReel.",".$$txtTotalHead.",".str_replace(",",'',$$txtQcPassQty).",".str_replace(",",'',$$txtProdQty).",".$$txtCompProd.",".$$txtRejectQty.",".$$cboProdTime.",".$$txtRemarks.",".$$hdnbookingDtlsId.",".$$hdnRcvDtlsId.",".$$hdnjobDtlsId.",".$$hdnBreakIDs.",'".$variable_status."','".$user_id."','".$pc_date_time."')";
				$add_commaa++;
				//$hdn_dtls_id_arr[]=str_replace("'",'',$$hdnDtlsUpdateId);
...
if($action=="qc_qty_popup")
{
	...
	function sum_total_qnty(status,rowId,sumId,rowNum)
	{
		...
		if (status==1) 
		{
			...
			var ordQty=0; var jobQty=0; var prodQty=0; var balQty=0;var CumProdQty=0;
			for(var i=1; i<=tot_row; i++)
			{
			...
			$('#txtTotOrdQty').html( ordQty );
			$('#txtjobOrdQty').html( jobQty );
			$('#txt_total_Prod_qnty').html( prodQty );
			$('#txt_total_Comp_qnty').html( CumProdQty );
			$('#txt_total_Bal_qnty').html( balQty );
		}
		else
		{
			var prodQty=($("#txtQcPassQty_"+rowNum).val()*1)+($("#txtRejectQty_"+rowNum).val()*1);
			...
			if(balQty<0){
				...
				$('#txtProdQty_'+rowNum).html( prodQty );
				$('#txtBalQty_'+rowNum).html( balQty );
			}
			...
			$('#txt_total_Prod_qnty').html( totalProdQty );
			$('#txt_total_Comp_qnty').html( totalCumProdQty );
			$('#txt_total_Bal_qnty').html( totalBalQty );
		}
	}
		...
		<form name="qntypopup_1"  id="qntypopup_1" autocomplete="off">
			<table class="rpt_table" width="1100px" cellspacing="0" cellpadding="0" rules="all" id="tbl_share_details_entry">
				...
				<tbody>
					...
					$cumelative_sql ="select id, mst_id, order_id, buyer_po_no, buyer_po_id, order_no,job_id, curr_prod_qty, job_dtls_id, break_dtls_id, booking_dtls_id, receive_id,receive_details_id, qc_pass_qty, reject_qty, remarks  from trims_prod_order_breakdown  where job_id=$job_id";
					...
					foreach ($dataArray as  $row) {
						//echo "nnnn"; die;
						$dtls_arr[$row[csf("buyer_po_no")]]["delivery_date"]		=change_date_format($row[csf("delivery_date")]);
						$dtls_arr[$row[csf("buyer_po_no")]]["item_group"]			.=$item_group_arr[$row[csf("item_group")]].',';
						$dtls_arr[$row[csf("buyer_po_no")]]["order_uom"]			.=$row[csf("order_uom")].',';
						$dtls_arr[$row[csf("buyer_po_no")]]["booked_uom"]			.=$row[csf("booked_uom")].',';
						$dtls_arr[$row[csf("buyer_po_no")]]["qnty"]					+=$row[csf("qnty")];
						$dtls_arr[$row[csf("buyer_po_no")]]["booked_qty"]			+=$row[csf("booked_qty")];
						$dtls_arr[$row[csf("buyer_po_no")]]["subconDtlsId"]			.=$row[csf("subconDtlsId")].',';
						$dtls_arr[$row[csf("buyer_po_no")]]["subconBrkId"]			.=$row[csf("subconBrkId")].',';
						$dtls_arr[$row[csf("buyer_po_no")]]["buyer_po_id"]			=$row[csf("buyer_po_id")];
						$dtls_arr[$row[csf("buyer_po_no")]]["recv_id"]				=$row[csf("receive_id")];
						
						if($hdnDtlsUpdateId!=""){
							$dtls_arr[$row[csf("buyer_po_no")]]["prodBrkID"]			=$row[csf("prodBrkID")];
							$dtls_arr[$row[csf("buyer_po_no")]]["qc_pass_qty"]			=$row[csf("qc_pass_qty")];
							$dtls_arr[$row[csf("buyer_po_no")]]["reject_qty"]			=$row[csf("reject_qty")];
							$dtls_arr[$row[csf("buyer_po_no")]]["remarks"]				=$row[csf("remarks")];
						}
					}
					...
					{ 
						$k=1; $subconBrkId=0;
						foreach($dtls_arr as $buyerPoNo=> $row)
						{
...
if( $action=='po_order_dtls_list_view' ) 
{
	...
	foreach($data_array as $row){
		...
		if(!in_array($row[csf('id')], $prodn_jobDtls_arr)){
			if($prodDtls_id!=""){
				$qc_bal=$job_qty-$total_qc_qty;
				$bal=$job_qty-($cumelative_qty)-$curent_qty;
				$hideBal=$bal;
		...
		<tr id="row_<? echo $tblRow; ?>">
			...
			<td><input id="txtcolor_<? echo $tblRow; ?>" name="txtcolor[]" type="text" value="<? echo $color_library[$row[csf('color_id')]] ?>" class="text_boxes" style="width:57px" placeholder="Display" disabled/>
				<input id="txtcolorID_<? echo $tblRow; ?>" name="txtcolorID[]" type="hidden" value="<? echo $row[csf('color_id')] ?>" class="text_boxes" style="width:57px" placeholder="Display" disabled/></td>
			<td><input id="txtsize_<? echo $tblRow; ?>" name="txtsize[]" type="text" value="<? echo $size_arr[$row[csf('size_id')]] ?>"  class="text_boxes" style="width:57px" placeholder="Display" disabled/>
				<input id="txtsizeID_<? echo $tblRow; ?>" name="txtsizeID[]" type="hidden" value="<? echo $row[csf('size_id')] ?>"  class="text_boxes" style="width:57px" placeholder="Display" disabled/></td>
