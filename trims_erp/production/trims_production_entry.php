...
$data_arr= json_encode( $_SESSION['logic_erp']['data_arr'][269] );
?>
<script>
	...
	function fnc_production_entry( operation )
	{
		...
		if ( form_validation('txt_prod_date*cbo_mc_group*cbo_location_name','Production Date*M/C Group*location name')==false )
			{
				return;
			}
		...
		$("#tbl_dtls_emb tbody tr").each(function()
		{
			...
			var txtItem 			= encodeURIComponent($(this).find('input[name="txtItem[]"]').val());
			...
			var hidBalQty 			= $(this).find('input[name="hidBalQty[]"]').val();
			...
			if(hdnDtlsUpdateId!='' || (hdnDtlsUpdateId=='' && txtProdQty*1>0))
			{
				...
				j++;  i++;
				//alert(j);
				data_all += "&cboMachineName_" + j + "='" + cboMachineName + "'&txtMcNo_" + j + "='" + txtMcNo+ "'&txtMcNoID_" + j + "='" + txtMcNoID + "'&txtItem_" + j + "='" + txtItem  + "'&cbosubProcess_" + j + "='" + cbosubProcess + "'&txtgmtscolorId_" + j + "='" + txtgmtscolorId + "'&txtgmtssizeId_" + j + "='" + txtgmtssizeId + "'&txtcolor_" + j + "='" + txtcolor + "'&txtsize_" + j + "='" + txtsize + "'&cboUom_" + j + "='" + cboUom + "'&txtImpression_" + j + "='" + txtImpression + "'&cborawColor_" + j + "='" + cborawColor + "'&txtQtyReel_" + j + "='" + txtQtyReel + "'&txtTotalHead_" + j + "='" + txtTotalHead+ "'&txtProdQty_" + j + "='" + txtProdQty+ "'&txtCompProd_" + j + "='" + txtCompProd+ "'&hdnCumDelQty_" + j + "='" + hdnCumDelQty+ "'&txtRejectQty_" + j + "='" + txtRejectQty+ "'&txtQcPassQty_" + j + "='" + txtQcPassQty+ "'&cboProdTime_" + j + "='" + cboProdTime + "'&txtRemarks_" + j + "='" + txtRemarks +"'&hdnjobDtlsId_" + j + "='" + hdnjobDtlsId +"'&hdnbookingDtlsId_" + j + "='" + hdnbookingDtlsId+"'&hdnDtlsUpdateId_" + j + "='" + hdnDtlsUpdateId+"'&hdnRcvDtlsId_" + j + "='" + hdnRcvDtlsId+"'&hdnBreakIDs_" + j + "='" + hdnBreakIDs +"'&hdnDtlsdata_" + j + "='" + hdnDtlsdata +"'&txtDeletedId_" + j + "='" + txtDeletedId+"'&txtBalQty_" + j + "='" + txtBalQty +"'&hidBalQty_" + j + "='" + hidBalQty+"'&txtcolorID_" + j + "='" + txtcolorID+"'&txtsizeID_" + j + "='" + txtsizeID  + "'";
			}
		});
		//alert(j+'=');
		if(j==0)
	...
	function fnc_job_entry_response()
	{
		...
		if(http.readyState == 4) 
		{
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				var over_ctrl_var_sts 	=$('#txt_over_ctrl_var_sts').val()*1;
				if(variable_status==2){
					show_list_view(2+'**'+response[4]+'**'+within_group+'**'+company_name+'**'+response[3]+'**'+response[2]+'**'+location_name+'**'+mc_group+'**'+floor_id+'**'+over_ctrl_var_sts,'po_order_dtls_list_view','emb_details_container','requires/trims_production_entry_controller','setFilterGrid(\'list_view\',-1)');
				}else{
					
					// $('#item_color_th').css('display','none');
	  				// $('#item_size_th').css('display','none');
	...
	function openmypage_job()
	{
		...
		emailwindow.onclose=function()
		{
			...
			{//alert(theemail.value);
				freeze_window(5);
				var company_name = $('#cbo_company_name').val();
				get_php_form_data( ex_data[0]+'_'+company_name, "load_mst_php_data_to_form", "requires/trims_production_entry_controller" );
				var within_group = $('#cbo_within_group').val();
				...
				var variable_status = $('#txt_variable_status').val()*1;
				var over_ctrl_var_sts = $('#txt_over_ctrl_var_sts').val()*1;
				//alert(over_ctrl_var_sts);
				if(variable_status==2)
				{
					show_list_view(1+'**'+ex_data[0]+'**'+within_group+'**'+company_name+'**'+ex_data[2]+'**'+over_ctrl_var_sts,'po_order_dtls_list_view','emb_details_container','requires/trims_production_entry_controller','setFilterGrid(\'list_view\',-1)');
				}
				else
				{
	...
	function openmypage_production()
	{
		...
		emailwindow.onclose=function()
		{
			...
			{//alert(ex_data[2]);
				...
				freeze_window(5);
				var company_name = $('#cbo_company_name').val();
				get_php_form_data( ex_data[0]+'_'+company_name, "load_production_data_to_form", "requires/trims_production_entry_controller" );
				...
				if(variable_status==2)
				{
					show_list_view(2+'**'+ex_data[3]+'**'+within_group+'**'+company_name+'**'+ex_data[2]+'**'+ex_data[0]+'**'+location_name+'**'+mc_group+'**'+floor_id+'**'+over_ctrl_var_sts,'po_order_dtls_list_view','emb_details_container','requires/trims_production_entry_controller','setFilterGrid(\'list_view\',-1)');	
				}
				else
				{ 
...
function chk_po_level_variabe(company)
{
   ...
   //status = status.split("**");
   //alert(status);
   $('#txt_variable_status').val(status);
   if (status==2) $('#copyvalues').css('display','none');
   else $('#copyvalues').css('display','block');   
}
			...
			<legend>Trims Order Receive</legend>
                <table width="900" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    <tr>
                        ...
                        <td colspan="3">
                            <input class="text_boxes"  type="text" name="txt_production_no" id="txt_production_no" placeholder="Double Click" style="width:140px;" onDblClick="openmypage_production();"  readonly />
                             ...
                             <input type="hidden" id="txt_over_ctrl_var_sts" name="txt_over_ctrl_var_sts" value="" />
                            </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                       ...
                       <td><input type="text" name="txt_prod_date" id="txt_prod_date"  style="width:140px"  class="datepicker" value="<? echo date("d-m-Y"); ?>" /></td>
                    </tr> 
                    <tr>
           	...
           	<legend>Trims Order Receive Details Entry</legend>
                <table cellpadding="0" cellspacing="2" border="0" class="rpt_table" rules="all" id="tbl_dtls_emb">
                    <thead class="form_table_header">
                    	...
                    	<th width="70" id="item_color_th">Item Color</th>
                    	<th width="80" id="item_size_th">Item Size</th>
                    ...
                    <tbody id="emb_details_container">
                        <tr id="row_1">
							...
							<td>
								<input id="txtgmtscolor_1" name="txtgmtscolor[]" type="text" class="text_boxes" style="width:57px" placeholder="Display"/>
                        		<input id="txtgmtscolorId_1" name="txtgmtscolorId[]" type="hidden" value="" class="text_boxes" style="width:57px" placeholder="Display"/>
							</td>
                            <td>
								<input id="txtgmtssize_1" name="txtgmtssize[]" type="text" class="text_boxes" style="width:57px" placeholder="Display"/>
                            	<input id="txtgmtssizeId_1" name="txtgmtssizeId[]" type="hidden" value=""  class="text_boxes" style="width:57px" placeholder="Display"/>
							</td>
                        	<td>
								<input id="txtcolor_1" name="txtcolor[]" type="text" class="text_boxes" style="width:57px" placeholder="Display"/>
                        		<input id="txtcolorID_1" name="txtcolorID[]" type="hidden" value="" class="text_boxes" style="width:57px" placeholder="Display"/>
							</td>
                            <td>
								<input id="txtsize_1" name="txtsize[]" type="text" class="text_boxes" style="width:77px" placeholder="Display"/>
                            	<input id="txtsizeID_1" name="txtsizeID[]" type="hidden" value=""  class="text_boxes" style="width:77px" placeholder="Display"/>
							</td>
                            <td id="subSectionTd_1"><? echo create_drop_down( "cboSubSection_1", 70, $trims_sub_section,"", 1, "-- Select Sub-Section --","",'',0,'','','','','','',"cboSubSection[]"); ?></td>
                            <td><? echo create_drop_down( "cboUom_1", 40, $unit_of_measurement,"", 1, "-- Select --",2,1, 1,'','','','','','',"cboUom[]"); ?>	</td>	
                            <td id="machinetd_1">
                ...
                <table width="1310" cellspacing="2" cellpadding="0" border="0">
                    <tr>
