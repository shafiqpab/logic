...
<script>
	...
	function fnc_job_entry_response()
	{
		...
		if(http.readyState == 4) 
		{
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				{
					show_list_view(2+'**'+response[4]+'**'+within_group+'**'+company_name+'**'+response[3]+'**'+response[2],'po_order_dtls_list_view','emb_details_container','requires/trims_production_entry_controller','setFilterGrid(\'list_view\',-1)');
				}
				else
				{
	...
	function openmypage_qc_qnty(row)
	{
		...
		//var page_link = 'requires/trims_production_entry_controller.php?within_group='+within_group+'&order_no='+order_no+'&data_break='+data_break+'&hdnDtlsUpdateId='+hdnDtlsUpdateId+'&job_no='+job_no+'&action=qc_qty_popup';
	...
	function openmypage_production()
	{
		...
		emailwindow.onclose=function()
		{
			...
			{//alert(ex_data[2]);
				...
				if(variable_status==2)
				{
					show_list_view(2+'**'+ex_data[3]+'**'+within_group+'**'+company_name+'**'+ex_data[2]+'**'+ex_data[0],'po_order_dtls_list_view','emb_details_container','requires/trims_production_entry_controller','setFilterGrid(\'list_view\',-1)');	
				}
				else
				{
			...
			<legend>Trims Order Receive</legend>
                <table width="900" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td>Floor</td>
                        <td id="floor_td"><? echo create_drop_down( "cbo_floor_id", 150, $blank_array,"", 1, "-- Select Floor --", $selected, "load_drop_down( 'requires/trims_production_entry_controller', this.value, 'load_drop_machine', 'machine_td'); ",1 ); ?></td>
           	...
           	<legend>Trims Order Receive Details Entry</legend>
                <table cellpadding="0" cellspacing="2" border="0" class="rpt_table" rules="all" id="tbl_dtls_emb">
                    <thead class="form_table_header">
                        ...
                        <th width="60">QC Pass<input id="copy_basis" name="copy_basis" type="checkbox" onChange="copy_values()" /></th>
