...
function openmypage_systemId()
{
	...
	var page_link = 'requires/raw_material_transfer_entry_controller.php?cbo_company_id='+cbo_company_id+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=itemTransfer_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(transfer_id, "populate_data_from_transfer_master", "requires/raw_material_transfer_entry_controller" );
		show_list_view(transfer_id,'show_transfer_listview','div_transfer_item_list','requires/raw_material_transfer_entry_controller','');
...
{
	get_php_form_data(data, "populate_transfer_details_form_data", "requires/raw_material_transfer_entry_controller" );
}
...
function openmypage_itemDescription()
{
	...
	else
	{
		if(cbo_transfer_criteria==1)
		{
			...
			var page_link = 'requires/raw_material_transfer_entry_controller.php?cbo_company_id='+cbo_company_id+'&cbo_store_name='+cbo_store_name+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=itemDescription_com_popup';
			...
			emailwindow.onclose=function()
			{
				...
				get_php_form_data(product_id+"**"+job_id+"**"+order_no+"**"+cbo_store_name+"**"+cbo_transfer_criteria, "populate_data_from_product_master", "requires/raw_material_transfer_entry_controller" );
			}
		}
		else
		{
			...
			var title = 'Item Description Info';	
			var page_link = 'requires/raw_material_transfer_entry_controller.php?cbo_company_id='+cbo_company_id+'&cbo_store_name='+cbo_store_name+'&cbo_transfer_criteria='+cbo_transfer_criteria+'&action=itemDescription_popup';
			...
			emailwindow.onclose=function()
			{
				...
				//reset_form('','','txt_style_no*txt_order_no*job_id*hidden_product_id*txt_item_desc*txt_transfer_qnty*txt_current_stock*hidden_current_stock*txt_rate*cbo_supplier*cbo_uom','','','');
				//reset_form('','','hidden_product_id*txt_item_desc*txt_transfer_qnty*txt_current_stock*hidden_current_stock*txt_rate*cbo_uom','','','');
				...
				$('#cbo_item_category').attr('disabled',true);
				//alert(10111);
				get_php_form_data(product_id+"**"+cbo_store_name+"**"+cbo_transfer_criteria+"**"+floor_id+"**"+room+"**"+rack+"**"+shelf+"**"+bin, "populate_data_from_product_master", "requires/raw_material_transfer_entry_controller" );
...
function fnc_yarn_transfer_entry(operation)
{
	if(operation==4)
	{
		...
		print_report( $('#cbo_company_id').val()+'*'+$('#update_id').val()+'*'+report_title, "trims_transfer_print", "requires/raw_material_transfer_entry_controller" ) 
		return;
	}
	else if(operation==0 || operation==1 || operation==2)
	{
		...
		http.open("POST","requires/raw_material_transfer_entry_controller.php",true);
...
{	
	if(http.readyState == 4) 
	{
		...
		if(reponse[0]==0 || reponse[0]==1 || reponse[0]==2)
		{
			...
			reset_form('transferEntry_1','','','','','update_id*txt_system_id*cbo_transfer_criteria*cbo_company_id*cbo_company_id_to*txt_transfer_date*txt_challan_no*cbo_item_category*cbo_currency*txt_remarks*store_update_upto*cbo_location*cbo_location_to*cbo_store_name*cbo_store_name_to');
			...
			show_list_view(reponse[1],'show_transfer_listview','div_transfer_item_list','requires/raw_material_transfer_entry_controller','');
			set_button_status(0, permission, 'fnc_yarn_transfer_entry',1,1);
...
function fn_to_store(company_id)
{
	...
	{
		load_drop_down( 'requires/raw_material_transfer_entry_controller', company_id, 'load_drop_down_store_to', 'store_td' );
	}*/
}
function fn_store_wise_location_from(store_id)
{
	var from_company=$('#cbo_company_id').val()*1;
	get_php_form_data(store_id + "**" + from_company, "populate_data_location_from", "requires/raw_material_transfer_entry_controller");	
}
function fn_store_wise_location_to(store_id)
{
	var from_company=$('#cbo_company_id').val()*1;
	get_php_form_data(store_id + "**" + from_company, "populate_data_location_to", "requires/raw_material_transfer_entry_controller");	
}
...
function fnc_get_from_com(fromComp){
	...
	if ($transfer_criteria==2) {
		...
		load_drop_down( 'requires/raw_material_transfer_entry_controller',fromComp, 'load_drop_down_location_to', 'to_location_td' );
	}
    ...
    xmlhttp.open("POST", "requires/raw_material_transfer_entry_controller.php", true);
...
{	
	var varible_string=return_global_ajax_value( data, 'varible_inventory', '', 'requires/raw_material_transfer_entry_controller');
	
	var varible_string_ref=varible_string.split("**");
	//alert(varible_string_ref[3]);
...
<div style="width:100%;" align="center">
    ...
    <div style="width:100%;">   
        ...
        <legend>Raw Material Transfer Entry</legend>
        <br>
            ...
            <br>
            <table width="810" cellspacing="2" cellpadding="2" border="0" id="tbl_dtls">
                <tr>
                    <td >
	                ...
	                <td>
	                    <div  style="float: none; width: 250px;">
	                    	<fieldset>
	                        ...
	                        <legend>Item Info</legend>
	                            <table id="tbl_item_info"  cellpadding="0" cellspacing="1" width="100%" style="float: left;">										
	                                <tr>
				                        ...
				                        <td>
											...
											//function create_drop_down( $field_id, $field_width, $query, $field_list, $show_select, $select_text_msg, $selected_index, $onchange_func, $is_disabled, $array_index, $fixed_options, $fixed_values, $not_show_array_index, $tab_index, $new_conn, $field_name )
				                            	...
				                            	echo create_drop_down( "cbo_item_category", 160, $item_category,'', 1, '--Select Category--', 0, '','0','101');
				                            ?>
				                        </td>
			                        ...
			                        <tr>
			                            ...
			                            <td>Remarks</td>
			                            <td colspan="3">
			                                <input type="text" name="txt_remarks" id="txt_remarks" class="text_boxes" style="width:390px;" />
			                            </td>
			                        </tr>
			                        <tr style="display:none;">
			                            <td>Supplier</td>						
...
<script> 
	if($("#cbo_company_id").val() >0 ){ $("#cbo_company_id").change();}
</script>
</html>
