...
function fnc_transfer_acknowledgement( operation )
{
	...
	{  
		...
		$("#pay_dtls_table tbody tr").each(function()
		{
			...
			var fromOrderID 			= $(this).find('input[name="fromOrderID[]"]').val();
			...
			j++;
			data_all += "&txtOrderNo_" + j + "='" + txtOrderNo + "'&txtOrderID_" + j + "='" + txtOrderID + "'&fromOrderID_" + j + "='" + fromOrderID+ "'&txtDtlsID_" + j + "='" + txtDtlsID  + "'&txtDtlsAcID_" + j + "='" + txtDtlsAcID  + "'&txtTransID_" + j + "='" + txtTransID  + "'&txtItemDesc_" + j + "='" + txtItemDesc + "'&productID_" + j + "='" + productID+ "'&fromProductID_" + j + "='" + fromProductID+ "'&colorID_" + j + "='" + colorID + "'&cboFloorTo_" + j + "='" + cboFloorTo + "'&cboRoomTo_" + j + "='" + cboRoomTo + "'&txtRackTo_" + j + "='" + txtRackTo + "'&txtShelfTo_" + j + "='" + txtShelfTo + "'&txtBinTo_" + j + "='" + txtBinTo + "'&hiddenTranQnty_" + j + "='" + hiddenTranQnty + "'&hiddenTranRate_" + j + "='" + hiddenTranRate + "'&hiddenTranValue_" + j + "='" + hiddenTranValue + "'&txtBatchId_" + j + "='" + txtBatchId +"'&cboUom_" + j + "='" + cboUom + "'";
		});
		...
		http.open("POST","requires/raw_material_transfer_acknowledgement_controller.php",true);
...
{	 
	...
	{	  		
		...
		if(response[0]==0 || response[0]==1)
		{
			...
			show_list_view(response[1],'show_dtls_list_view_update','payDtlsTableTbody','requires/raw_material_transfer_acknowledgement_controller','');
			
			set_button_status(1, permission, 'fnc_transfer_acknowledgement',1,1);
		} 
		else if(response[0]==20)
		{  
			show_msg(show_msg(11));
			release_freezing();
			//return;
		}
		release_freezing();
...
function openmypage_challan()
{
	...
	var page_link='requires/raw_material_transfer_acknowledgement_controller.php?action=itemTransfer_popup&company='+company+'&location='+location+'&store='+store;
	var title="Search Lc Popup";
	emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=800px,height=300px,center=1,resize=0,scrolling=0','../')
	...
	emailwindow.onclose=function()
	{
		...
		if(transfer_id!='')
		{
			...
			get_php_form_data(transfer_data_str, "populate_data_from_transfer_master", "requires/raw_material_transfer_acknowledgement_controller" );
			show_list_view(transfer_id,'show_dtls_list_view','payDtlsTableTbody','requires/raw_material_transfer_acknowledgement_controller','');	
		}
	}
}
...
function openmypage_systemId()
{
	...
	var page_link = 'requires/raw_material_transfer_acknowledgement_controller.php?cbo_company_id_to='+cbo_company_id_to+'&action=itemAcknowle_popup';
	...
	emailwindow.onclose=function()
	{
		...
		get_php_form_data(transfer_id, "populate_data_from_transfer_master_update", "requires/raw_material_transfer_acknowledgement_controller" );
		show_list_view(transfer_id,'show_dtls_list_view_update','payDtlsTableTbody','requires/raw_material_transfer_acknowledgement_controller','');
		set_button_status(1, permission, 'fnc_transfer_acknowledgement',1,1);
	}
}
...
function fnc_floor_load(datas){
	...
	for (var i = 1;numRow>=i; i++) {
		...
		load_drop_down( 'requires/raw_material_transfer_acknowledgement_controller',storeId+'_'+company+'_'+location+'_'+i, 'load_drop_down_floor_to', floor_tds );
	}
}
	...
	<div style="width:100%;" align="center">																	
        ...
        <fieldset style="width:1020px; margin-bottom:5px;">
           	<form name="transAcknowledgement" id="transAcknowledgement" autocomplete="off" method="POST"  >
           		<legend>Raw Material Transfer Acknowledgement</legend>
        		<br>
                ...
                <table width="880" cellspacing="2" cellpadding="2" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td>
                            <?
                                echo create_drop_down("cbo_transfer_criteria", 160,$item_transfer_criteria,"", 0,"-- Select --",'2',"",1,'1,2,4');
                            ?>
                        </td>
                    ...
                    <tr>
                        ...
                        <td>
							<?
                            	echo create_drop_down( "cbo_item_category", 160, $item_category,'', 0, '', '', '','1',101 );
                            ?>
                        </td>                                   
                        <td>Remarks</td>
                        ...
                        <tbody id="payDtlsTableTbody">
                            <tr id="dtlsTbodyTr_1">
                                ...
                                <td style="display: none">
                                	...
                                	<input type="hidden" name="fromOrderID_1" id="fromOrderID[]" class="text_boxes" />
                                </td>
                                <td>
                                	<input type="text" name="txtItemDesc_1" id="txtItemDesc[]" class="text_boxes" style="width:150px;" readonly />
                                ...
                                <td id="bin_td">
                                	<? echo create_drop_down( "cbo_bin_to", 152,$blank_array,"", 1, "--Select--", 0, "",0,"","","","","","","txtBinTo[]" ); ?>
                                </td>
                                <td>
