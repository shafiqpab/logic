...
{
	echo create_drop_down( "cbo_location", 140, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$data' $company_location_credential_cond order by location_name","id,location_name", 1, "-- Select Location --", $selected, "load_room_rack_self_bin('requires/raw_material_item_issue_return_controller*4_8_9_10_11_15_16_17_18_19_20_21_22_32_33_34_35_36_37_38_39_40_41_44_45_46_47_48_49_50_51_52_53_54_55_56_57_58_59_60_61_62_63_64_65_66_67_68_69_70_89_90_91_92_93_94_101_22', 'store','store_td', $('#cbo_company_id').val(), this.value);",0 );     	 
	exit();
}
if ($action=="load_room_rack_self_bin")
	{
		load_room_rack_self_bin("requires/raw_material_item_issue_return_controller",$data);
	}
	...
	<table width="1080" cellspacing="0" cellpadding="0" border="1" class="rpt_table" align="center" rules="all">
            ...
            <tbody>
                <tr>                    
                    <td align="center">
                        <?  
							echo create_drop_down( "cbo_item_category", 180, $item_category,"", 1, "-- Select --", 0, "", 0,"101,22","","","1,2,3,4,5,6,7,12,13,14" );
                        ?>
                    </td>
                    <td align="center">
                        <input name="txt_issue_challan" id="txt_issue_challan" type="text" class="text_boxes" style="width:100px" placeholder="Write" />
                    </td>
                    <td align="center">
                        <input name="txt_item_description" id="txt_item_description" type="text" class="text_boxes" style="width:100px" placeholder="Write" />
                    </td>
...
if($action=="create_item_search_list_view")
{
	...
	$toDate = $ex_data[3];
	$company = $ex_data[4];
	$issue_challan = trim($ex_data[5]);
	$item_description = trim($ex_data[6]);

	
	//if ($issue_challan!="") $str_cond .=" and a.issue_number='$issue_challan'";
	if ($issue_challan!="") $str_cond .=" and a.issue_number_prefix_num='$issue_challan'";
	if ($item_description!="") $str_cond .=" and c.item_description='$item_description'";

	if($db_type==0) { $year_cond=" and YEAR(a.insert_date)=$ex_data[7]";   }
	if($db_type==2) {$year_cond=" and to_char(a.insert_date,'YYYY')=$ex_data[7]";}
	...
	$sql ="select a.id as issue_id,a.issue_number,sum(case when b.transaction_type=2 then (b.cons_quantity+b.cons_reject_qnty) else 0 end) as to_isue,c.id as prod_id,b.item_category,c.product_name_details,c.lot,c.current_stock,c.item_description,c.item_group_id,c.sub_group_name,c.item_size,c.avg_rate_per_unit ,c.unit_of_measure, b.batch_lot
		from inv_issue_master a, inv_transaction b left join product_details_master c on b.prod_id=c.id and c.status_active=1 and c.item_category_id in(101,22)
		where a.id=b.mst_id and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 and c.status_active=1 and b.item_category in(101,22) and b.transaction_type=2 and a.entry_form=265  $sql_cond $year_cond $item_category $item_group $str_cond  
		group by a.id,a.issue_number,c.id,c.product_name_details,c.lot,c.current_stock,c.item_description,c.item_group_id,c.sub_group_name,c.item_size,c.avg_rate_per_unit,c.unit_of_measure,b.item_category,b.batch_lot";
	
	// echo $sql;die;
	
	$item_group_arr=return_library_array( "select id, item_name from lib_item_group",'id','item_name');
	$arr=array(4=>$item_group_arr);
	?>
	...
	<?
 	echo create_list_view("list_view", "Product Id, Lot, Issue Id,Issue Challan No, Item Description,Item Group, Item SubGroup,  Item Size, Total Issue","60,60,60,120,120,130,100,80,80","900","260",0, $sql , "js_set_value", "item_description,prod_id,unit_of_measure,to_isue,item_category,item_group_id,avg_rate_per_unit,issue_id,issue_number,batch_lot", "", 1, "0,0,0,0,0,item_group_id,0,0,0", $arr, "prod_id,batch_lot,issue_id,issue_number,item_description,item_group_id,sub_group_name,item_size,to_isue", "","",'0,0,0,0,0') ;	
	exit();
...
if($action=="issue_rtn_qty")
{
	...
	//print_r($data);
	//echo $data;
	//echo "select sum(case when transaction_type=4 then (cons_quantity+cons_reject_qnty) else 0 end) as to_isue_return from inv_transaction where prod_id=$data and item_category in(101) and transaction_type=4"; die;
	
	echo return_field_value("sum(case when transaction_type=4 then (cons_quantity+nvl(cons_reject_qnty,0)) else 0 end) as to_isue_return","inv_transaction","prod_id=$prodID and  issue_id=$issue_id and item_category in(101,22) and transaction_type=4","to_isue_return");
...
if($action=="populate_data_from_data")
{
	...
	$sql = "select sum(case when b.transaction_type=4 then (b.cons_quantity+b.cons_reject_qnty) else 0 end) as to_isue_return, c.id as prod_id,concat(c.sub_group_name,',',c.item_description,',',c.item_size) as pro_description,c.unit_of_measure,c.item_group_id,b.item_category
			...
			where a.id=b.mst_id and b.prod_id=c.id and c.id=$prodID and b.item_category in(101,22) and b.transaction_type in(2,4) and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 group by c.id";		
...
//		}
		...
		if(str_replace("'","",$txt_return_no)=="")
		{
			...
			$new_recv_number = explode("*", return_next_id_by_sequence("INV_RECEIVE_MASTER_PK_SEQ", "inv_receive_master",$con,1,str_replace("'","",$cbo_company_id),'RMIIR',266,date("Y",time()))); 
			...
			$data_array_receive="(".$id.",'".$new_recv_number[1]."','".$new_recv_number[2]."','".$new_recv_number[0]."',266,".$cbo_item_category.",".$cbo_company_id.",".$txt_return_date.",".$txt_return_challan_no.",".$cbo_store_name.",".$cbo_floor.",".$cbo_room.",".$txt_rack.",".$txt_shelf.",".$cbo_bin.",".$cbo_location.",1,1,".$txt_remarks.",'".$user_id."','".$pc_date_time."')";
		...
		else
		{
			...
			$data_array_receive="266*".$cbo_item_category."*".$cbo_company_id."*".$txt_return_date."*".$txt_return_challan_no."*".$cbo_store_name."*".$cbo_floor."*".$cbo_room."*".$txt_rack."*".$txt_shelf."*".$cbo_bin."*".$cbo_location."*1*1*".$txt_remarks."*'".$user_id."'*'".$pc_date_time."'";
			//echo $field_array."<br>".$data_array;die;
		...
		$field_array_trans_insert = "id,mst_id,company_id,prod_id,item_category,transaction_type,transaction_date,store_id,floor_id,room,rack,self,bin_box,machine_id,cons_uom,cons_quantity,cons_reject_qnty,cons_rate,cons_amount,balance_qnty,balance_amount,issue_challan_no,remarks,raw_issue_challan, 
   issue_id,inserted_by,insert_date,batch_lot";
 		$data_array_trans_insert = "(".$transactionID.",".$id.",".$cbo_company_id.",".$txt_prod_id.",".$cbo_item_category.",4,".$txt_return_date.",".$cbo_store_name.",".$cbo_floor.",".$cbo_room.",".$txt_rack.",".$txt_shelf.",".$cbo_bin.",".$cbo_machine_no.",".$cbo_uom.",".$txt_return_qnty.",".$txt_reject_qnty.",".$txt_avrage_rate.",".$txt_amount.",".$txt_return_qnty.",".$txt_amount.",".$txt_return_challan_no.",".$txt_remarks.",".$txt_issue_no.",".$txt_issue_id.",'".$user_id."','".$pc_date_time."',".$txt_lot_no.")";
		//echo $field_array."<br>".$data_array;die;
		...
		//echo "INSERT INTO inv_transaction (".$field_array_trans_insert.") VALUES ".$data_array_trans_insert.""; die;
	...
	else if ($operation==1) // Update Here----------------------------------------------------------
	{
		...
		$sql = sql_select("select a.cons_quantity,a.cons_amount,b.current_stock,b.stock_value from inv_transaction a, product_details_master b where a.id=$update_id and a.prod_id=b.id and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0");
		...
		$field_array_trans = "company_id*prod_id*item_category*transaction_type*transaction_date*store_id*floor_id*room*rack*self*bin_box*machine_id*cons_uom*cons_quantity*cons_reject_qnty*cons_rate*cons_amount*balance_qnty*balance_amount*issue_challan_no*remarks*raw_issue_challan* 
   issue_id*updated_by*update_date*batch_lot";
...
<form name="searchorderfrm_1"  id="searchorderfrm_1" autocomplete="off">
	<table width="780" cellspacing="0" cellpadding="0" border="0" class="rpt_table" align="center">
            ...
            <tbody>
                <tr> 
				   <td align="center"><? echo create_drop_down( "cbo_year_selection", 100, $year,"", 1, "-- Select --", date('Y'), "",0 ); ?>
				    </td>
                    <td>
                       <input type="text" id="src_return_no" name="src_return_no" style="width:130px;" class="text_boxes" />
                    </td>
                    <td align="center">
                    	<input type="text" name="txt_return_date" id="txt_return_date" class="datepicker" style="width:125px;" placeholder="Select Date" />
                    </td>
                    <td align="center" >
                    	<input type="text" id="src_challan_no" name="src_challan_no" style="width:125px;" class="text_boxes" />				
                    </td>    
                    <td align="center">
						<? 
...
if($action=="create_return_search_list_view")
{
	...
	if($ex_data[5]!=0) $year_cond=" and to_char(a.insert_date,'YYYY')=$ex_data[5]";
	...
	$sql = "SELECT a.id,a.recv_number,a.company_id,a.receive_date,a.challan_no,a.location_id from inv_receive_master a, inv_transaction b
			where a.id=b.mst_id and b.transaction_type=4 and a.entry_form=266 and b.item_category in(101,22) and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 $company $location $challan_no $return_date $return_no  $credientian_cond $year_cond
			group by a.id ,a.recv_number,a.company_id,a.receive_date,a.challan_no,a.location_id order by a.id desc"; 
 	// echo $sql; 
	...
	<div style="text-align:center;" class="search_type"><? echo create_drop_down( "cbo_string_search_type", 150, $string_search_type,'', 1, "-- Searching Type --",4,"","","1,2,3,4" ); ?></div>
	<?
  	echo create_list_view("list_view", "Return No,Return Date,Return Challan No,Location","150,150,150,200","700","260",0, $sql , "js_set_value", "id,recv_number", "", 1, "0,0,0,location_id", $arr, "recv_number,receive_date,challan_no,location_id","","",'0,0,0,0') ;	
 	exit();
}
...
if($action=="show_dtls_list_view")	
{
	...
	$sql = "SELECT a.recv_number,a.company_id,a.receive_date,a.item_category,a.recv_number,b.id,b.cons_quantity,b.cons_reject_qnty,b.cons_uom,b.cons_rate,b.cons_amount,c.product_name_details,c.id as prod_id, b.batch_lot 
	from  inv_receive_master a, inv_transaction b left join product_details_master c on b.prod_id=c.id 
	where a.id=b.mst_id and b.item_category in(101,22) and b.transaction_type=4 $cond and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0";		
	//echo $sql; //die;
	$result = sql_select($sql);
	$i=1;
	$rettotalQnty=0;
     	...
     	<table class="rpt_table" border="1" cellpadding="2" cellspacing="0" style="width:980px" rules="all" >
            ...
            <tbody>
				...
				foreach($result as $row){					
 				...
 				?>
                	<tr bgcolor="<? echo $bgcolor; ?>" onClick='get_php_form_data("<? echo $row[csf("id")];?>","child_form_input_data","requires/raw_material_item_issue_return_controller")' style="cursor:pointer" >
...
{
	//$data // transaction id
  	$sql = "SELECT b.id as prod_id, b.product_name_details, b.lot, a.id as tr_id,a.company_id,a.location_id, a.store_id,a.floor_id,a.room,a.rack,a.self,a.bin_box, a.cons_uom, a.cons_rate, a.cons_quantity,nvl(a.cons_reject_qnty,0), a.cons_amount, a.issue_challan_no,a.remarks,a.item_category,a.machine_id,b.item_group_id, a.batch_lot,a.raw_issue_challan,a.issue_id 	
			from inv_transaction a, product_details_master b
 			where a.id=$data and a.status_active=1 and a.item_category in(101,22) and transaction_type=4 and a.prod_id=b.id and b.status_active=1";
 	// echo $sql;die; 
	$result = sql_select($sql);
	foreach($result as $row)
	{
		...
		echo "$('#txt_issue_no').val('".$row[csf("raw_issue_challan")]."');\n";
		echo "$('#txt_issue_id').val('".$row[csf("issue_id")]."');\n";
		echo "load_room_rack_self_bin('requires/raw_material_item_issue_return_controller*4_8_9_10_11_15_16_17_18_19_20_21_22_32_33_34_35_36_37_38_39_40_41_44_45_46_47_48_49_50_51_52_53_54_55_56_57_58_59_60_61_62_63_64_65_66_67_68_69_70_89_90_91_92_93_94_101', 'store','store_td', '".$row[csf('company_id')]."','".$row[csf('location_id')]."',this.value);\n";
		echo "$('#cbo_store_name').val(".$row[csf("store_id")].");\n";

		echo "load_room_rack_self_bin('requires/raw_material_item_issue_return_controller', 'floor','floor_td', '".$row[csf('company_id')]."','".$row[csf('location_id')]."','".$row[csf('store_id')]."',this.value);\n";
		echo "$('#cbo_floor').val('".$row[csf("floor_id")]."');\n";
		echo "load_room_rack_self_bin('requires/raw_material_item_issue_return_controller', 'room','room_td', '".$row[csf('company_id')]."','".$row[csf('location_id')]."','".$row[csf('store_id')]."','".$row[csf('floor_id')]."',this.value);\n";
		echo "$('#cbo_room').val('".$row[csf("room")]."');\n";
		echo "load_room_rack_self_bin('requires/raw_material_item_issue_return_controller', 'rack','rack_td', '".$row[csf('company_id')]."','".$row[csf('location_id')]."','".$row[csf('store_id')]."','".$row[csf('floor_id')]."','".$row[csf('room')]."',this.value);\n";
		echo "$('#txt_rack').val('".$row[csf("rack")]."');\n";
		echo "load_room_rack_self_bin('requires/raw_material_item_issue_return_controller', 'shelf','shelf_td', '".$row[csf('company_id')]."','".$row[csf('location_id')]."','".$row[csf('store_id')]."','".$row[csf('floor_id')]."','".$row[csf('room')]."','".$row[csf('rack')]."',this.value);\n";	
		echo "$('#txt_shelf').val('".$row[csf("self")]."');\n";
		echo "load_room_rack_self_bin('requires/raw_material_item_issue_return_controller', 'bin','bin_td', '".$row[csf('company_id')]."','".$row[csf('location_id')]."','".$row[csf('store_id')]."','".$row[csf('floor_id')]."','".$row[csf('room')]."','".$row[csf('rack')]."','".$row[csf('self')]."',this.value);\n";	
		...
		// $totalIssued = return_field_value("sum(b.cons_quantity+b.cons_reject_qnty) as to_issue","inv_issue_master a, inv_transaction b"," a.id=b.mst_id and b.prod_id='".$row[csf("prod_id")]."' and b.item_category in(101,22) and b.transaction_type=2 group by prod_id","to_issue");
		// echo "SELECT sum(b.cons_quantity+b.cons_reject_qnty) as to_issue FROM inv_issue_master a, inv_transaction b WHERE a.id=b.mst_id and b.prod_id='".$row[csf("prod_id")]."' and a.id='".$row[csf("issue_id")]."' and b.item_category in(101,22) and b.transaction_type=2 group by prod_id";
		$totalIssued = return_field_value("sum(b.cons_quantity+b.cons_reject_qnty) as to_issue","inv_issue_master a, inv_transaction b"," a.id=b.mst_id and b.prod_id='".$row[csf("prod_id")]."' and a.id='".$row[csf("issue_id")]."' and b.item_category in(101,22) and b.transaction_type=2","to_issue");
		// $totalIssuedReturn = return_field_value("sum(b.cons_quantity+nvl(b.cons_reject_qnty,0)) as to_issue_return","inv_receive_master a, inv_transaction b"," a.id=b.mst_id and b.prod_id='".$row[csf("prod_id")]."' and b.item_category in(101,22) and b.transaction_type=4 group by prod_id","to_issue_return");
		// if($totalIssued=="") $totalIssued=0;
        ...
        // $total_new_qty=$totalIssued-$totalIssuedReturn;
...
if ($action=="general_item_issue_return_print")
{
	...
	$sql=" select id, recv_number, location_id, receive_date, challan_no, inserted_by from  inv_receive_master where id='$data[1]' and company_id='$data[0]' and entry_form=266";
	//echo $sql;die;
	...
	<div style="width:100%;">
		<table align="right" cellspacing="0" width="900"  border="1" rules="all" class="rpt_table" >
            <thead bgcolor="#dddddd" align="center">
                ...
                <th width="50" align="center">Issue Challan</th> 
                <th width="50" align="center">Issued Qty</th> 
...
<?
	...
	$issue_qty=sql_select("SELECT a.issue_number,b.cons_quantity,c.item_description from inv_issue_master a,inv_transaction b,product_details_master c where a.id=b.issue_id and b.prod_id=c.id and  b.transaction_type=4 and a.company_id='$data[0]' and a.entry_form=265");

	$issue_qty_arr=array();
		foreach ($issue_qty as $row) {
			$issue_qty_arr[$row[csf("issue_number")]][$row[csf("item_description")]]["cons_quantity_issue"]=$row[csf("cons_quantity")];	
		}

	$sql_dtls= "select a.id, a.item_category,
	b.cons_uom, b.cons_quantity, b.cons_rate, b.cons_amount, b.store_id,
	c.item_group_id, $concat(c.sub_group_name $concat_coma ' ' $concat_coma c.item_description $concat_coma ' ' $concat_coma c.item_size ) as product_name_details, b.batch_lot,b.raw_issue_challan 
	from inv_receive_master a, inv_transaction b,  product_details_master c
	where a.id='$data[1]' and a.company_id='$data[0]' and a.id=b.mst_id and b.prod_id=c.id and b.transaction_type=4 and b.item_category in (101,22) and a.entry_form=266 and a.status_active =1 and a.is_deleted=0 and b.status_active =1 and b.is_deleted=0 ";
	//echo $sql_dtls;die;
	...
	foreach($sql_result as $row)
	{
		...
		?>
			<tr bgcolor="<? echo $bgcolor; ?>">
                ...
                <td align="center"><? echo $row[csf('raw_issue_challan')]; ?></td>
                <td align="center"><? echo $row[csf('last_issued_qnty')]; ?></td>
                <td align="right"><? echo $issue_qty_arr[$row[csf("raw_issue_challan")]][$row[csf("item_description")]]["cons_quantity_issue"]; ?></td>
        ...
        <br>
		 <?
            echo signature_table(271, $data[0], "900px","","",$inserted_by);
         ?>
      </div>
   </div> 
<?
