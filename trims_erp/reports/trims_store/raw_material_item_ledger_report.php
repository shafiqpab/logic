<?
/*-------------------------------------------- Comments
Purpose			: 	This form will create Raw Material Item Ledger
...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Raw Material Item Ledger","../../../", 1, 1, $unicode,1,1); 
?>	
<script>
...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../logout.php";
	...
	function openmypage_item()
	{
		...
		var page_link='requires/raw_material_item_ledger_controller.php?action=item_description_search&company='+company+'&txt_produc_no='+txt_produc_no+'&txt_produc_id='+txt_produc_id+'&txt_product='+txt_product; */
		var page_link='requires/raw_material_item_ledger_controller.php?action=item_description_search&company='+company+'&cbo_item_cat='+cbo_item_cat; 
	...
	function generate_report(operation)
	{
		...
		http.open("POST","requires/raw_material_item_ledger_controller.php",true);
        ...
        <fieldset style="width:900px;">
			<table class="rpt_table" width="900" cellpadding="0" cellspacing="0">
                ...
                <tr class="general" align="center">
                    <td>
                            <? 
                               echo create_drop_down( "cbo_company_name", 130, "select id,company_name from lib_company comp where status_active=1 and is_deleted=0 $company_cond order by company_name","id,company_name", 1, "-- Select Company --", $selected, "load_drop_down( 'requires/raw_material_item_ledger_controller', this.value, 'load_drop_down_store', 'store_td' );" );
                            ?>                            
                    </td>
                    <td align="center">
                            <?
								echo create_drop_down( "cbo_item_cat", 120, $item_category,"", 1, "-- Select Item --", $selected, "",1,101 );
							?>                           
                    </td>
                    <td align="center">
