...
if($action=="generate_report")
{
	...
	$itemgroupArr = return_library_array("select id,item_name from lib_item_group where status_active=1 and is_deleted=0 and item_category=101","id","item_name");
	...
	*/
	...
	where a.prod_id=b.id and a.status_active=1 and b.item_category_id=101 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $company_id $group_id $prod_cond $store_id $item_account $search_cond $item_category_id $item_category_cond";
	...
	$returnRes_date="select prod_id as PROD_ID, min(transaction_date) as MIN_DATE, max(transaction_date) as MAX_DATE from inv_transaction 
	where is_deleted=0 and status_active=1 and COMPANY_ID=$cbo_company_name and $item_category_cond group by prod_id";
	...
	$wo_qty_arr=return_library_array("select b.item_id, sum(b.supplier_order_quantity) as qty from wo_non_order_info_mst a, wo_non_order_info_dtls b where a.id=b.mst_id and a.COMPANY_NAME=$cbo_company_name and a.item_category in (101) and a.pay_mode<>2 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by item_id","item_id","qty");

	$pi_qty_arr=return_library_array("select b.item_prod_id, sum(b.quantity) as qty from com_pi_master_details a, com_pi_item_details b 
	where a.id=b.pi_id and a.IMPORTER_ID=$cbo_company_name and a.item_category_id in (101) and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 group by item_prod_id","item_prod_id","qty");
	$i=1;
	//echo "test6";die;
	ob_start();
	?>
	<div align="center" style="height:auto; width:auto; margin:0 auto; padding:0; width:3168px;">
		<table width="3168" cellpadding="0" cellspacing="0" id="caption" align="center">
			<thead>
				...
				<tr style="border:none;">
					<td colspan="20" class="form_caption" align="center" style="border:none; font-size:14px;">
					   <b>Store Name: <? if($cbo_store_name==0){ echo "ALL Store";}else{ echo $storeArr[$cbo_store_name];} ?></b>
					</td>
				</tr>
				<tr style="border:none;">
		...
		<?
			...
			where b.status_active=1 and b.is_deleted=0 and b.item_category_id=101 and b.company_id='$cbo_company_name' $item_category_id $group_id $prod_cond $item_account_cond $group_id $search_cond";
			//echo $sql;die;
				...
				if($value_with==1)
				{
					if(number_format($closingStock,2)>0.00 || number_format($openingBalance,2)>0.00 )
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>" style=" word-wrap:break-word; <? echo $stylecolor; ?> "  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
							...
							<td width="100" style="word-break: break-all;"><? echo $itemgroupArr[$row[csf("item_group_id")]]; ?></td>
							<td width="100" style="word-break: break-all;" ><? echo $row[csf("sub_group_name")]; ?></td>
							<td width="180"  style="word-break: break-all;"><p><? echo $row[csf("item_description")]; ?>&nbsp;</p></td>
                            <td width="100"  style="word-break: break-all;" ><p><? echo $trims_section[$row[csf("section_id")]]; ?>&nbsp;</p></td>
                            <td width="100" style="word-break: break-all;"><p><? echo $row[csf("brand_name")]; ?>&nbsp;</p></td>
							<td width="70"><p><? echo $row[csf("item_size")]; ?>&nbsp;</p></td>
							<td width="80" align="center"  style="word-break: break-all;"><p><? echo $country_nameArr[$row[csf("origin")]]; ?>&nbsp;</p></td>
                            <td width="60" align="center"  style="word-break: break-all;"><p><? echo $unit_of_measurement[$row[csf("unit_of_measure")]]; ?>&nbsp;</p></td>

                            <td width="100" align="right"  style="word-break: break-all;"><p><? echo number_format($openingBalance,3); $tot_open_bl+=$openingBalance; ?></p></td>
							<td width="100" align="right"  style="word-break: break-all;"><p><? echo number_format($openingRate,3); ?></p></td>
                            <td width="110" align="right"  style="word-break: break-all;"><p><? //echo number_format($openingBalanceValue,3); $tot_open_bl_amt+=$openingBalanceValue; 
							...
							?></p></td>
							<td width="80" align="right"  style="word-break: break-all;"><a href="##" onclick="fnc_purchase_details('<? echo $row[csf("id")]."_".$from_date."_".$to_date;?>','purchase_popup')"><p><? echo number_format($data_array[$row[csf("id")]]['purchase'],3); $tot_purchase+=$data_array[$row[csf("id")]]['purchase']; ?></p></a></td>
							<td width="80" align="right"  style="word-break: break-all;"><p><? echo number_format($data_array[$row[csf("id")]]['purchase_amt']/$currency_rate,3); $tot_purchase_amount+=$data_array[$row[csf("id")]]['purchase_amt']/$currency_rate; ?></p></td>
							<td width="80" align="right"  style="word-break: break-all;" ><p><? echo number_format($data_array[$row[csf("id")]]['issue_return'],3); $tot_issue_return+=$data_array[$row[csf("id")]]['issue_return'];//$row[csf("issue_return")]; ?></p></td>
							<td width="80" align="right" style="word-break: break-all;"><p><? echo number_format($data_array[$row[csf("id")]]['issue_return_amt']/$currency_rate,3); $tot_issue_return_amount+=$data_array[$row[csf("id")]]['issue_return_amt']/$currency_rate;//$row[csf("issue_return")]; ?></p></td>
							<td width="100" align="right" style="word-break: break-all;"><p><? echo number_format($totalReceive,3); $tot_total_receive+=$totalReceive; ?></p></td>
							<td width="100" align="right" style="word-break: break-all;"><p><? echo number_format($totalReceive_amt,3); $tot_total_receive_amount+=$totalReceive_amt; ?></p></td>
							<td width="80" align="right" style="word-break: break-all;"><p><? echo number_format($issue_qty,3); $tot_issue+=$issue_qty; ?></p></td>

							<td width="80" align="right" style="word-break: break-all;"><p><? echo number_format($issue_value/$currency_rate,3); $tot_total_issue_amount+=$issue_value/$currency_rate; ?></p></td>
							<td width="80" align="right" style="word-break: break-all;"><p><? echo number_format($data_array[$row[csf("id")]]['receive_return'],3); $tot_rec_return+=$data_array[$row[csf("id")]]['receive_return'];//$issue_result[0][csf('receive_return')]; ?></p></td>
							<td width="80" align="right" style="word-break: break-all;"><p><? echo number_format($data_array[$row[csf("id")]]['receive_return_amt']/$currency_rate,3); $tot_rec_return_amount+=$data_array[$row[csf("id")]]['receive_return_amt']/$currency_rate;//$issue_result[0][csf('receive_return')]; ?></p></td>
							<td width="100" align="right"  style="word-break: break-all;"><p><? echo number_format($totalIssue,3); $tot_total_issue+=$totalIssue; ?></p></td>
							<td width="100" align="right" style="word-break: break-all;"><p><? echo number_format($totalIssue_amt,3); $tot_total_issue+=$totalIssue_amt; ?></p></td>
                            <td width="100" align="right"  style="word-break: break-all;"><p><? echo number_format($closingStock,3); $tot_closing_stock+=$closingStock; ?></p></td>
                            <td width="60" align="center"  style="word-break: break-all;"><p><? echo $unit_of_measurement[$row[csf("order_uom")]]; ?></p></td>
                            <td width="100" align="right" style="word-break: break-all;"><p><? echo number_format($closing_stock_ord,3); $tot_closing_stock_ord+=$closing_stock_ord; ?></p></td>
							<td width="80" align="right" style="word-break: break-all;"><p><? echo number_format($avg_rate,3); //number_format($row[csf("avg_rate")],2); $tot_avg_rate+=$row[csf("avg_rate")]; ?></p></td>
                            <td width="120" align="right"  style="word-break: break-all;"><p><?  //echo number_format($stockValue,3); $tot_stock_value+=$stockValue; 
                            ...
                            <td width="120" align="right" style="word-break: break-all;"><p><?
                             echo number_format($closing_amount,3); $value_tot_stock_value_tka+=$closing_amount; ?></p></td>
							<td width="60" align="center" style="word-break: break-all;"><p><? echo $daysOnHand;//$daysOnHand; ?>&nbsp;</p></td>
							<td width="100" align="right"  style="word-break: break-all;" title="<? echo $wo_qty_arr[$row[csf("id")]]."==".$pi_qty_arr[$row[csf("id")]]; ?>"><a href="##" onclick="fnc_pipeLine_details('<? echo $row[csf("id")];?>','pipe_line_popup')"><p><? echo number_format($pipeLine_qty,2); ?></p></a></td>
							<td  style="word-break: break-all;"><p><? echo "";//chop($data_array[$row[csf("id")]]['lot_no'],","); ?>&nbsp;</p></td>
						</tr>
						<?
						$i++;
				...
				else
				{
					if(number_format($closingStock,3)>0.000 || number_format($openingBalance,3)>0.000 || number_format($totalReceive,3)>0.000 || number_format($totalIssue,3)>0.000)
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>" style=" word-wrap:break-word; <? echo $stylecolor; ?> "  onclick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
							...
							<td width="80"  style="word-break: break-all;" align="center"><p><? echo $country_nameArr[$row[csf("origin")]];  ?>&nbsp;</p></td>
                            <td width="60"  style="word-break: break-all;" align="center"><p><? echo $unit_of_measurement[$row[csf("unit_of_measure")]]; ?>&nbsp;</p></td>

                            <td width="100"  style="word-break: break-all;" align="right"><p><? echo number_format($openingBalance,3); $tot_open_bl+=$openingBalance; ?></p></td>
							<td width="100"  style="word-break: break-all;" align="right"><p><? echo number_format($openingRate,3); ?></p></td>
                            <td width="110"  style="word-break: break-all;" align="right"><p><? //echo number_format($openingBalanceValue,3); $tot_open_bl_amt+=$openingBalanceValue; 
							if(number_format($openingBalance,3)>0.000){
							 echo number_format($openingBalanceValue,3); $tot_open_bl_amt+=$openingBalanceValue;}else { echo "0.000"; $tot_open_bl_amt+="0.000";} 
							?></p></td>
							<td width="80"  style="word-break: break-all;" align="right"><a href="##" onclick="fnc_purchase_details('<? echo $row[csf("id")]."_".$from_date."_".$to_date; ?>','purchase_popup')"><p><? echo number_format($data_array[$row[csf("id")]]['purchase'],3); $tot_purchase+=$data_array[$row[csf("id")]]['purchase']; ?></p></a></td>
							<td width="80"  style="word-break: break-all;" align="right"><p><? echo number_format($data_array[$row[csf("id")]]['purchase_amt']/$currency_rate,3); $tot_purchase_amount+=$data_array[$row[csf("id")]]['purchase_amt']/$currency_rate; ?></p></td>
							<td width="80"  style="word-break: break-all;" align="right"><p><? echo number_format($data_array[$row[csf("id")]]['issue_return'],3); $tot_issue_return+=$data_array[$row[csf("id")]]['issue_return'];//$row[csf("issue_return")]; ?></p></td>
							<td width="80"  style="word-break: break-all;" align="right"><p><? echo number_format($data_array[$row[csf("id")]]['issue_return_amt']/$currency_rate,3); $tot_issue_return_amount+=$data_array[$row[csf("id")]]['issue_return_amt']/$currency_rate;//$row[csf("issue_return")]; ?></p></td>
							<td width="100"  style="word-break: break-all;" align="right"><p><? echo number_format($totalReceive,3); $tot_total_receive+=$totalReceive; ?></p></td>
							<td width="100" align="right"><p><? echo number_format($totalReceive_amt,3); $tot_total_receive_amount+=$totalReceive_amt; ?></p></td>
							<td width="80"  style="word-break: break-all;" align="right"><p><? echo number_format($issue_qty,3); $tot_issue+=$issue_qty; ?></p></td>

							<td width="80"  style="word-break: break-all;" align="right"><p><? echo number_format($issue_value/$currency_rate,3); $tot_issue_value+=$issue_value/$currency_rate; ?></p></td>
							...
							<td width="80"  style="word-break: break-all;" align="right"><p><? echo number_format($data_array[$row[csf("id")]]['receive_return_amt']/$currency_rate,3); $tot_rec_return_amount+=$data_array[$row[csf("id")]]['receive_return_amt']/$currency_rate;//$issue_result[0][csf('receive_return')]; ?></p></td>
							<td width="100"  style="word-break: break-all;" align="right"><p><? echo number_format($totalIssue,3); $tot_total_issue+=$totalIssue; ?></p></td>
							<td width="100" align="right"><p><? echo number_format($totalIssue_amt,3); $tot_total_issue_amount+=$totalIssue_amt; ?></p></td>
							<td width="100"  style="word-break: break-all;" align="right"><p><? echo number_format($closingStock,3); $tot_closing_stock+=$closingStock; ?></p></td>
                            <td width="60"  style="word-break: break-all;" align="center"><p><? echo $unit_of_measurement[$row[csf("order_uom")]]; ?></p></td>
                            <td width="100"  style="word-break: break-all;" align="right"><p><? echo number_format($closing_stock_ord,3); $tot_closing_stock_ord+=$closing_stock_ord; ?></p></td>
							<td width="80"  style="word-break: break-all;" align="right"><p><? echo number_format($avg_rate,3); //number_format($row[csf("avg_rate")],2); $tot_avg_rate+=$row[csf("avg_rate")]; ?></p></td>
                            <td width="120"  style="word-break: break-all;" align="right"><p><?
							if(number_format($closingStock,3)>0.000){
							 echo number_format($stockValue,3); $tot_stock_value+=$stockValue;}else { echo "0.000"; $tot_stock_value+="0.000";} ?></p></td>

                            <td width="120"  style="word-break: break-all;" align="right"><p><? 
                            echo number_format($closing_amount,3); $value_tot_stock_value_tka+=$closing_amount; ?></p></td>
							<td width="60"  style="word-break: break-all;" align="center"><p><? echo $daysOnHand;//$daysOnHand; ?>&nbsp;</p></td>
							<td width="100"  style="word-break: break-all;" align="right" title="<? echo $wo_qty_arr[$row[csf("id")]]."==".$pi_qty_arr[$row[csf("id")]]; ?>"><a href="##" onclick="fnc_pipeLine_details('<? echo $row[csf("id")];?>','pipe_line_popup')"><p><? echo number_format($pipeLine_qty,2); ?></p></a></td>
							<td  style="word-break: break-all;"><p><? echo "";//chop($data_array[$row[csf("id")]]['lot_no'],","); ?>&nbsp;</p></td>
						</tr>
						<?
						$i++;
		...
		<table width="3168" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" id="table_body_footer">
			<tfoot>
				<tr>
					...
					<th width="110"  style="word-break: break-all;" align="right" id="value_tot_purchase_amount"><? echo number_format($tot_open_bl_amt,3); ?></th>
					<th width="80" align="right" id="value_tot_purchase"><? //echo number_format($tot_purchase,3); ?></th>
					<th width="80"  style="word-break: break-all;" align="right" id="value_tot_issue_return_amount"><? echo number_format($tot_purchase_amount,3); ?></th>
					<th width="80" align="right" id="value_tot_transfer_in_qty"><? //echo number_format($tot_issue_return,3); ?></th>
					<th width="80"  style="word-break: break-all;" align="right" id="value_tot_total_receive_amount"><? echo number_format($tot_issue_return_amount,3); ?></th>
					...
					<th width="80"  style="word-break: break-all;" align="right" id="value_tot_rec_return"><? //echo number_format($tot_rec_return,3); ?></th>
					<th width="80"  style="word-break: break-all;" align="right" id="value_tot_total_issue_amount"><? echo number_format($tot_rec_return_amount,3); ?></th>
					
					<th width="100"  style="word-break: break-all;" align="right" id="value_tot_stock_valu"><p><?// echo number_format($tot_stock_value,2); ?></p></th>
					<th width="100" style="word-break: break-all;" align="right" id="value_tot_total_issue"><? echo number_format($tot_total_issue_amount,3); ?></th>
                    <th width="100"  style="word-break: break-all;" align="right" id="value_tot_closing_stock"><? //echo number_format($tot_closing_stock,3); ?></th>
                    <th width="60" style="word-break: break-all;" align="right"></th>
                    <th width="100"  style="word-break: break-all;" align="right" id="value_tot_closing_stock_ord"><? //echo number_format($tot_closing_stock_ord,3); ?></th>
					<th width="80">&nbsp;</th>
					<th width="120"  style="word-break: break-all;" align="right" id="value_tot_stock_value"><p><? echo number_format($tot_stock_value,2); ?></p></th>
					
                    <th width="120"  style="word-break: break-all;" align="right" id="value_tot_stock_value_tk"><p><? echo number_format($value_tot_stock_value_tka,2); ?></p></th>
					<th width="60">&nbsp;</th>
					<th width="100" align="right"  style="word-break: break-all;" id="value_totalpipeLine_qty"><? //echo number_format($totalpipeLine_qty,2); ?></th>
					<th>&nbsp;</th>
				</tr>
			</tfoot>
		</table>
...
if($action=="generate_report2")
{
	...
	if ($cbo_item_category_id==0) $item_category_cond=" and item_category in(101)"; else $item_category_cond=" and item_category='$cbo_item_category_id'";
	$returnRes_date="select prod_id as PROD_ID, min(transaction_date) as MIN_DATE, max(transaction_date) as MAX_DATE,
	sum(case when a.transaction_type in (1,4,5) then a.cons_quantity else 0 end) as TOTAL_RECEIVE,
	sum(case when a.transaction_type in (2,3,6) then a.cons_quantity else 0 end) as TOTAL_ISSUE, 
	sum(case when a.transaction_type in (1,4,5) then a.cons_amount else 0 end) as TOTAL_RECEIVE_AMMOUNT,
