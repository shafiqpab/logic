...
if ($action=="load_drop_down_store")
{
	...
	echo create_drop_down( "cbo_store_name", 100, "select a.id,a.store_name from lib_store_location a, lib_store_location_category b where a.id=b.store_location_id and b.category_type in (101) $store_location_credential_cond and a.status_active=1 and a.is_deleted=0 and a.company_id in($data)  group by a.id ,a.store_name order by a.store_name","id,store_name", 1, "-- Select Store --", "", "","" ); 
        exit();
}
...
if($action=="create_lot_search_list_view")
{
	...
	if(trim($txt_search_common)!="")
	{
		...
		{
			$sql_cond= " and upper(product_name_details) LIKE upper('%$txt_search_common%')";
 		}
	...
	if($txt_prod_id!="") $sql_cond.=" and  id=$txt_prod_id";
 	...
 	$sql = "select id,item_group_id,item_description as product_name_details,item_code,item_size from product_details_master where company_id=$company and item_category_id =$cbo_item_cat and status_active=1 and is_deleted=0 $sql_cond "; 
...
{ 
	...
	where a.mst_id=b.id $item_desc_search_cond and a.transaction_type in (5,6) and a.item_category='$cbo_item_cat' and b.transfer_criteria in(1,2) and b.entry_form in(57,487)"; //and b.entry_form =57		 
    ...
    <?
		...
		if( $from_date!="" && $to_date!="" ) 
		{
			...
			if($cbo_store_name>0) $opening_store_cond=" and a.store_id=$cbo_store_name";
			//for opening balance
			...
			where a.transaction_date < '".$from_date."' $opening_store_cond $item_desc_search_cond and a.status_active=1 and a.is_deleted=0 group by a.prod_id";
			// echo $sqlTR ;
		...
		if($cbo_method==0) //average rate #########################################################################
		{
			...
			from inv_transaction a left join inv_issue_master c on a.mst_id=c.id and a.transaction_type in (2,3), product_details_master b 
			where a.status_active=1 and a.is_deleted=0  and b.status_active=1 and b.is_deleted=0 $item_desc_search_cond and a.prod_id=b.id  and a.item_category='$cbo_item_cat' and a.company_id=$cbo_company_name $search_cond $store_cond order by  a.prod_id, a.id ASC";		
		...
		else if($cbo_method==1) //FIFU #########################################################################
		{
			...
			from inv_transaction a left join inv_issue_master c on a.mst_id=c.id and a.transaction_type in (2,3), product_details_master b 
			where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $item_desc_search_cond and a.prod_id=b.id  and a.item_category='$cbo_item_cat' and a.company_id=$cbo_company_name $search_cond $store_cond order by  a.prod_id, a.transaction_date ASC";		
		...
		else if($cbo_method==2) //LIFU #########################################################################
		{
			$sql = "select a.id, a.prod_id,a.store_id, a.transaction_date, a.transaction_type, a.cons_quantity, a.cons_rate, a.cons_amount,  a.department_id, b.product_name_details, b.unit_of_measure, b.item_group_id, b.lot, c.knit_dye_source, c.knit_dye_company, c.issue_purpose
			from inv_transaction a left join inv_issue_master c on a.mst_id=c.id and a.transaction_type in (2,3), product_details_master b 
			where a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 $item_desc_search_cond and a.prod_id=b.id  and a.item_category='$cbo_item_cat' and a.company_id=$cbo_company_name $search_cond $store_cond order by  a.prod_id, a.transaction_date DESC";		
		...
		?>
        	...
        	<table width="1500" border="0">
                ...
                <tr>
                    <td colspan="17" align="center"><b>Weighted Average Method</b></td>
                </tr>
            </table>
            <table width="1500" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" id="table_header_1" > 
                <thead>
                    <tr>
        ...
        <div style="width:1500px; overflow-y:scroll; max-height:250px" id="scroll_body" > 
            <table width="1480" border="1" cellpadding="2" cellspacing="0" class="rpt_table" rules="all" id="table_body"> 
                <tbody>
                <?		
					...
					if(!in_array($row[csf("prod_id")],$product_id_arr))
					{
						...
						<tr bgcolor="<? echo $bgcolor; ?>" <? echo $stylecolor; ?> onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
                            ...
                            if($row[csf("transaction_type")]==2)
                            {
								...
								{  	
									if($row[csf("issue_purpose")]==15)
									{
								     $transactionWith =  $buyer_name_arr[$row[csf("knit_dye_company")]];
									 }
								     else 
									 {
                            ...
                            <td width="60" align="right"><? echo number_format($bal_rate,4,'.',''); ?></td>
