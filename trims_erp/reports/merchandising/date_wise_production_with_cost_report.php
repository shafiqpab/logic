...
<script>
	...
	var tableFilters = 
	{
		col_30: "none",
		col_operation: {
		id: ["tot_amount","tot_today_qc_val","tot_today_issue_cost","tot_cum_qc_val","tot_cum_issue_val"],
		col: [7,9,10,13,14],
	...
	function load_sub_section(rowNo)
	{
		...
		load_drop_down( 'requires/date_wise_production_with_cost_report_controller',section, 'load_drop_down_subsection', 'subSectionTd');
	}
	...
	function openmypage_order()
	{
		...
		else
		{
			...
			var page_link = 'requires/date_wise_production_with_cost_report_controller.php?company='+company+'&party_name='+party_name+'&customer_source='+customer_source+'&action=order_popup';
	...
	function generate_report(report_type)
	{
		...
		//alert(data);
		freeze_window(3);
		http.open("POST","requires/date_wise_production_with_cost_report_controller.php",true);
    ...
    function fnc_amount_details(wo_id, section, jobcard_id, order_uom, rate, exchange_rate,form_date,to_date,pop_order_uom,item_group, action, popupTitle,issue_ids,currency_id='',company='') 
	{
		
		//alert(issue_ids);
        emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/date_wise_production_with_cost_report_controller.php?wo_id='+wo_id+'&section='+section+'&jobcard_id='+jobcard_id+'&issue_ids='+issue_ids+'&order_uom='+order_uom+'&rate='+rate+'&exchange_rate='+exchange_rate+'&form_date='+form_date+'&to_date='+to_date+'&pop_order_uom='+pop_order_uom+'&item_group='+item_group+'&action='+action+'&currency_id='+currency_id+'&company='+company, popupTitle, 'width=1200px,height=320px,center=1,resize=0', '../../');
        emailwindow.onclose=function() {}
    }
                        ...
                        <table class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
                            ...
                            <tbody>
                            	<tr>
                                    <td>
										 <? 
                                       		 echo create_drop_down( "cbo_company_id",120,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", $selected, "load_drop_down( 'requires/date_wise_production_with_cost_report_controller', this.value, 'load_drop_down_location', 'location_td');","","","","","",2);  ?>
                                    </td>
                                    <td id="location_td">
		                                <?php 
                                    ...
                                    <td>
										<? 
                                       		 echo create_drop_down( "cbo_customer_source",100,$yes_no,"", 1, "--All--",0, "load_drop_down( 'requires/date_wise_production_with_cost_report_controller', (document.getElementById('cbo_company_id').value+'_'+this.value), 'load_drop_down_buyer', 'buyer_td' )","","","","","",2);
                                        ?>
                                    </td>
                                    <td id="buyer_td">
										<? 
                                    ...
                                    <td >
                                        <input name="txt_order_no" id="txt_order_no" type="text"  class="text_boxes" style="width:127px" placeholder="Write/Browse" onDblClick="openmypage_order();"/><input type="hidden" name="hid_order_id" id="hid_order_id"></td>
                                     <td width="90">
                                    	<input type="text" name="txt_date_from" id="txt_date_from" value="<? echo date("d-m-Y");?>" class="datepicker" style="width:80px;"/>                    				</td>
                                    <td width="90">
                                    	<input type="text" name="txt_date_to" id="txt_date_to" value="<? echo date("d-m-Y");?>" class="datepicker" style="width:80px;"/>                        		    </td>
                                    <td>
