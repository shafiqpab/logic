...
if($action=="generate_report")
{
	...
	$delivery_sql = "select a.id,a.order_no,a.currency_id, d.delevery_qty as delivery_qty , d.break_down_details_id,b.section, a.receive_date, b.sub_section, b.item_group, b.source_for_order, b.order_quantity as order_qty, b.order_uom, e.rate, e.qnty, e.amount, b.booked_conv_fac as conv_factor from subcon_ord_mst a, trims_delivery_mst c, trims_delivery_dtls d ,subcon_ord_dtls b, subcon_ord_breakdown e where c.id = d.mst_id and a.order_no=d.order_no and a.id = b.mst_id and a.subcon_job=e.job_no_mst and b.id=e.mst_id and d.break_down_details_id=e.id  and a.is_deleted=0 and d.is_deleted=0 and c.entry_form=208 $sql_con_date $where_con and a.company_id = $cbo_company_id ";
	...
	foreach ($delivery_ord_result as $rows) {
		...
		$booked_array[$rows[csf('id')].'*'.$key]['order_quantity']+=$rows[csf('qnty')];
		$booked_array[$rows[csf('id')].'*'.$key]['amount']+=$rows[csf('amount')];
		$booked_array[$rows[csf('id')].'*'.$key]['delivery_qty']+=$rows[csf('delivery_qty')];
	}
	//echo "<pre>";
	...
	foreach ($delivery_ord_result as $row) {
		...
		$orderquantity=$booked_array[$row[csf('id')].'*'.$key]['order_quantity'];
		$orderamount=$booked_array[$row[csf('id')].'*'.$key]['amount'];
	...
	$trims_ord_sql = "select a.currency_id, b.section, a.receive_date, a.order_no, b.sub_section, b.item_group, b.source_for_order, sum(b.order_quantity) as order_qty , b.order_uom from subcon_ord_mst a, subcon_ord_dtls b where a.id = b.mst_id and a.is_deleted=0 and b.is_deleted=0 and a.entry_form=255 $sql_con $where_con and a.company_id = $cbo_company_id $sql_con_id group by a.currency_id, b.section, a.order_no, a.receive_date, b.sub_section, b.item_group, b.source_for_order, b.order_uom";
	...
	<div align="center" style="height:auto; width:<?php echo $width+20;?>px; margin:0 auto; padding:0;">
        ...
        <table border="1" cellpadding="2" cellspacing="0" class="rpt_table" width="<?php echo $width;?>" rules="all" id="scroll_body" align="left">
        	...
        	<tfoot>
        		<th colspan="6"></th>
        		<th><?php echo number_format($total_delivery_qty, 2); ?></th>
