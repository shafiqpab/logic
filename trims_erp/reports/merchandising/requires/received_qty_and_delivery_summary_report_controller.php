...
if($action=="generate_report")
{
	...
	$delivery_sql = "select a.id,a.order_no,a.currency_id, d.delevery_qty as delivery_qty , d.break_down_details_id,b.section, a.receive_date, b.sub_section, b.item_group, b.source_for_order, b.order_quantity, b.order_uom, e.rate, e.qnty  as order_qty, e.amount, b.booked_conv_fac as conv_factor,e.description from subcon_ord_mst a, trims_delivery_mst c, trims_delivery_dtls d ,subcon_ord_dtls b, subcon_ord_breakdown e where c.id = d.mst_id and a.order_no=d.order_no and a.id = b.mst_id and a.subcon_job=e.job_no_mst and b.id=e.mst_id and d.break_down_details_id=e.id  and a.is_deleted=0 and d.is_deleted=0 and c.entry_form=208  and e.status_active=1 $sql_con_date $where_con and a.company_id = $cbo_company_id  and c.company_id = $cbo_company_id order by  b.section ";
	...
	/*$ord_sql = "select a.id,a.currency_id, b.section, a.receive_date, a.order_no, b.sub_section, b.item_group, b.source_for_order, sum(e.qnty) as order_qty, sum(e.amount) as amount , b.order_uom,e.description from subcon_ord_mst a, subcon_ord_dtls b, subcon_ord_breakdown e where a.id = b.mst_id and a.subcon_job=e.job_no_mst and b.id=e.mst_id  and a.is_deleted=0 and b.is_deleted=0  and e.status_active=1 and a.entry_form=255 $where_con and a.company_id = $cbo_company_id $sql_ord_con_id   group by a.currency_id, b.section, a.order_no, a.receive_date, b.sub_section, b.item_group, b.source_for_order, b.order_uom,a.id,e.description order by  b.section";*/
	$ord_sql="select a.id,a.subcon_job,a.receive_date,a.exchange_rate,a.currency_id,a.delivery_date,a.order_no as cust_order_no,a.party_id,a.within_group,a.subcon_job,b.buyer_buyer,b.section,b.sub_section, b.item_group, b.source_for_order,b.job_no_mst,b.order_no,b.booked_uom,b.order_uom,e.qnty,e.order_id ,e.qnty as order_qty,e.rate,e.amount,e.booked_qty,e.description,e.id as breakdown_id,b.delivery_date as delivery_target_date, a.trims_ref
	from subcon_ord_mst a, subcon_ord_dtls b, subcon_ord_breakdown e
	where a.subcon_job=b.job_no_mst and b.job_no_mst=e.job_no_mst and b.id=e.mst_id and a.entry_form=255 and a.company_id =$cbo_company_id $where_con_ord and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and e.is_deleted=0 and e.status_active=1 $where_con and a.company_id = $cbo_company_id $sql_ord_con_id ";
	...
	foreach ($ord_sql_result as $rows) {
		...
		$qty_key= $rows[csf('section')].'*'.$rows[csf('sub_section')].'*'.$rows[csf('item_group')].'*'.$rows[csf('source_for_order')].'*'.$rows[csf('order_uom')];
		...
		$paymentDateEnd=date('Y-m-d', strtotime($txt_date_to));
		$contractDateBegin = date('Y-m-d', strtotime($rows[csf('receive_date')]));
		$contractDateEnd = date('Y-m-d', strtotime($rows[csf('receive_date')]));
		//echo $paymentDate.'=='.$contractDateBegin.'=='.$contractDateEnd."<br>";
		if (($paymentDate <= $contractDateBegin) && ($paymentDateEnd >= $contractDateEnd)){
			//echo $rows[csf('order_qty')].'++';
		   $trims_ord_arr[$qty_key]['order_qty'] += $rows[csf('order_qty')];
		   $trims_ord_arr[$qty_key]['amount'] += $rows[csf('amount')];
		}
	}
	...
	foreach ($delivery_ord_result as $row) {
		...
		if (!in_array($new_key, $array_chk))
		{
			...
			if($currency_id==1){
				//echo $rate.'=='.$currency_rate; 
				$usdrate=number_format($rate/$currency_rate,4);
			}else{
				$usdrate=$rate;
			}

			//echo $orderquantity.'=='.$orderamount.'=='.$currency_id.'=='.$rate.'=='.$usdrate.'=='.$currency_rate.'=='.$dalivery_order_qty_usd[$new_key]['delivery_qty']*$usdrate.'=='.$dalivery_order_qty[$key]['delivery_qty']."<br>";
			//42840==80.3250000000000028421709430404007434844970703125==1==0.0019==0.0000==0
			...
			$trims_ord_arr[$key]['delivery_amt_usd'] += number_format($dalivery_order_qty_usd[$new_key]['delivery_qty']*$usdrate,4,".","");
	...
	//, sum(b.order_quantity) as order_qty
	$trims_ord_sql = "select a.currency_id, b.section, a.receive_date, a.order_no, b.sub_section, b.item_group, b.source_for_order, sum(b.amount) as amount ,sum( e.qnty) as order_qty , b.order_uom from subcon_ord_mst a, subcon_ord_dtls b , subcon_ord_breakdown e where a.id = b.mst_id and a.subcon_job=e.job_no_mst and b.id=e.mst_id and a.is_deleted=0 and b.is_deleted=0  and e.status_active=1 and a.entry_form=255 $sql_con $where_con and a.company_id = $cbo_company_id $sql_con_id  group by a.currency_id, b.section, a.order_no, a.receive_date, b.sub_section, b.item_group, b.source_for_order, b.order_uom order by  b.section";

	

	//echo $trims_ord_arr[$key]['order_qty'].'==<br>';
        		...
        		<?php
        			...
        			foreach ($trims_ord_arr as $key=> $row) {
        				...
        				<tr id='tr_2nd<? echo $sl; ?>' bgcolor="<?php echo $bgcolor; ?>" onClick="change_color('tr_2nd<? echo $sl; ?>','<? echo $bgcolor; ?>')" style="cursor:pointer">
        					...
        					<td title="<?php echo $row['item_group']; ?>"><?php echo $item_group_arr[$row['item_group']]; ?></td>
        					...
        					<td align="right"><?php echo number_format($row['delivery_amt_usd'],4,".","");  ?></td>
        				...
        				$total_delivery_amt_usd +=number_format($row['delivery_amt_usd'],4,".","");
        			}
        		?>
        	</tbody>
        	<tfoot >
        		<th colspan="7"></th>
    			<th><?php echo number_format($total_delivery_amt_usd,4); ?></th>
    			<th>&nbsp;</th>
        	</tfoot>
		</table>
    </div>
