...
if($action=="generate_report")
{
	...
	if ($report_type==1)
	{
		...
		if(count($result)>0)
		{
			...
			foreach($order_sql_result as $row)
			{
				...
				$order_array[$row[csf('id')]][$row[csf('subcon_job')]][$row[csf('section')]][$row[csf('order_uom')]]['jobcard_id'] =$row[csf('jobcard_id')];
		...
		$issue_sql = "SELECT g.received_id,e.issue_date,f.cons_quantity as issue_qty, f.cons_rate,f.prod_id,e.id as issue_id 
		from trims_raw_mat_requisition_mst a,trims_raw_mat_requisition_dtls b,inv_issue_master e,inv_transaction f,trims_job_card_mst g
		where a.entry_form in (427,501) and a.job_id=g.id and a.id = b.mst_id and a.job_no = g.trims_job and a.id = e.req_id and e.id=f.mst_id and b.product_id = f.prod_id and b.mst_id = e.req_id and b.job_no = g.trims_job and e.req_no=a.requisition_no and f.transaction_type = 2 and e.issue_basis = 7 and e.entry_form = 265 and a.status_active = 1 and b.status_active = 1 and e.status_active = 1 and f.status_active = 1 and g.status_active = 1  and g.received_id in (select poid from tmp_poid where userid=$user_id and type=555)
		union all
		SELECT c.received_id,a.issue_date, b.cons_quantity as issue_qty, b.cons_rate,b.prod_id,a.id as issue_id 
		from inv_issue_master a, inv_transaction b, trims_job_card_mst c 
		where a.id=b.mst_id and a.req_id=c.id and b.transaction_type=2 and a.issue_basis=15 and a.entry_form=265  and c.received_id in (select poid from tmp_poid where userid=$user_id and type=555)";  

		$iss_data_array=sql_select($issue_sql);
		$issue_qty_arr=array(); $date_wise_issue_qty_arr=array();
		foreach($iss_data_array as $row){
			$issue_qty_arr[$row[csf("received_id")]]['qty']+=$row[csf("issue_qty")];
			$issue_qty_arr[$row[csf("received_id")]]['amount']+=$row[csf("issue_qty")]*$row[csf("cons_rate")];
			$issue_qty_arr[$row[csf("received_id")]]['issue_id'].=$row[csf("issue_id")].',';
			$issue_qty_arr[$row[csf("received_id")]]['prod_id'].=$row[csf("prod_id")].',';
			if($from_date!="" and $to_date!="")
			{
				
				if( (strtotime($row[csf('issue_date')])>=strtotime($from_date)) && (strtotime($row[csf('issue_date')])<=strtotime($to_date)) )
				{
			    	$date_wise_issue_qty_arr[$row[csf("received_id")]]['today_qty']+=$row[csf("issue_qty")];
					$date_wise_issue_qty_arr[$row[csf("received_id")]]['today_amount']+=$row[csf("issue_qty")]*$row[csf("cons_rate")];
					$date_wise_issue_qty_arr[$row[csf("received_id")]]['issue_id'].=$row[csf("issue_id")].',';
					$date_wise_issue_qty_arr[$row[csf("received_id")]]['prod_id'].=$row[csf("prod_id")].',';
					//echo "dsfds";
			    }
			}
			else
						...
						foreach($wo_id_arr as $subcon_job=>$subcon_job_data)
						{
							foreach($subcon_job_data as $section_id=>$section_id_data)
							{
								foreach($section_id_data as $order_uom=>$row)
								{
									...
									$issue_id=$issue_qty_arr[$wo_id]['issue_id'];
									...
									$cum_issue_qty=$issue_qty_arr[$wo_id]['qty']-$rtn_qty;
									$cum_issue_val=($issue_qty_arr[$wo_id]['amount']-$rtn_val)/$exchange_rate;
									//$cum_issue_val=($cum_issue_qty*$rate)/$exchange_rate;
									//$cum_issue_val=($cum_issue_qty*$rate);
									$today_issue_id=$date_wise_issue_qty_arr[$wo_id]['issue_id'];
									...
									//echo $today_rtn_qty;
									$today_issue_qty=$date_wise_issue_qty_arr[$wo_id]['today_qty']-$today_rtn_qty;
									$today_issue_cost=($date_wise_issue_qty_arr[$wo_id]['today_amount']-$today_rtn_val)/$exchange_rate;
				                ...
				                <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
				                    ...
				                    <td width="100" style="word-break: break-all;" align="right"><a href="##" onClick="fnc_amount_details(<?php echo "$wo_id, $section_id, $jobcard_id, $order_uom, $rate, $exchange_rate, '$txt_date_from', '$txt_date_to','$pop_order_uom','$item_group'"; ?>, 'issue_qty_popup', 'Material Issued Cost',<?php echo "$issue_id"; ?>)"><? echo number_format($cum_issue_val,4) ;?></a></td>  
	...
	else
	{
        ...
        $issue_sql = "SELECT g.received_id,e.issue_date,f.cons_quantity as issue_qty, f.cons_rate,f.prod_id,e.id as issue_id 
		from trims_raw_mat_requisition_mst a,trims_raw_mat_requisition_dtls b,inv_issue_master e,inv_transaction f,trims_job_card_mst g,gbl_temp_engine h
		where a.entry_form in (427,501) and a.id = b.mst_id and a.job_id=g.id and a.id = e.req_id and e.id=f.mst_id and b.product_id = f.prod_id and b.mst_id = e.req_id and b.job_no = g.trims_job and e.req_no=a.requisition_no and f.transaction_type = 2 and e.issue_basis = 7 and e.entry_form = 265 and a.status_active = 1 and b.status_active = 1 and e.status_active = 1 and f.status_active = 1 and g.status_active=1 and g.received_id=h.ref_val and h.user_id=$user_id and h.entry_form=555
		union all
		SELECT c.received_id,a.issue_date, b.cons_quantity as issue_qty, b.cons_rate,b.prod_id,a.id as issue_id 
		from inv_issue_master a, inv_transaction b, trims_job_card_mst c,gbl_temp_engine h 
		where a.issue_basis=15 and a.entry_form=265  and  b.transaction_type=2 and  a.id=b.mst_id and a.req_id=c.id and  a.status_active=1 and b.status_active=1 and c.status_active=1 and c.received_id=h.ref_val and h.user_id=$user_id and h.entry_form=555"; 
						...
						foreach($section_id_data as $order_uom=>$order_uom_data)
						{
							...
							foreach($order_uom_data as $wo_id=>$wo_id_arr)
							{
								foreach($wo_id_arr as $subcon_job=>$row)
								{
									...
									//$cum_issue_val=($cum_issue_qty*$rate);
									if($currency_id_ac == 1){
                                        $cum_issue_val=($issue_qty_arr[$wo_id]['amount']-$rtn_val)/$currency_rate;
                                    }
									$today_issue_id=$date_wise_issue_qty_arr[$wo_id]['issue_id'];
									$today_issue_ids=array_unique(explode(",",chop($today_issue_id,',')));
									foreach ($today_issue_ids as $key => $value) {
									...
									$today_issue_cost=($date_wise_issue_qty_arr[$wo_id]['today_amount']-$today_rtn_val)/$exchange_rate;
									if($currency_id_ac == 1){
                                        $today_issue_cost=($date_wise_issue_qty_arr[$wo_id]['today_amount']-$today_rtn_val)/$currency_rate;
                                    }
                                    $today_margin_percent =(($today_qc_val-$today_issue_cost)*100)/$today_qc_val;
									$gross_margin_percent =(($cum_qc_val-$cum_issue_val)*100)/$cum_qc_val;
									$balance_qty=$qnty-$cum_qc_qty;
								...
								if(number_format($today_qc_qty,2)>0)
								{
									...
									<tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
										...
										<td width="100" style="word-break: break-all;" align="right"><a href="##" onClick="fnc_amount_details(<?php echo "$wo_id, $section_id, $jobcard_id, $order_uom, $rate, $exchange_rate, '$txt_date_from', '$txt_date_to','$pop_order_uom','$item_group'"; ?>, 'today_issue_qty_popup', 'Issue Quantity',<?php echo trim($issue_id, ","); ?>, '<?=$currency_id_ac?>','<?=$cbo_company_id?>')"><?php echo number_format($today_issue_cost,4); ?></a></td>
										<td width="80" style="word-break: break-all;" align="right" style="word-break:break-all;"><? echo number_format($today_margin_percent,2); ?></td>

										<td width="100" style="word-break: break-all;" align="right"><a href="##" onClick="fnc_amount_details(<?php echo "$wo_id, $section_id, $jobcard_id, $order_uom, $rate, $exchange_rate, '$txt_date_from', '$txt_date_to','$pop_order_uom','$item_group'"; ?>, 'cum_production_qty_popup', 'Production Quantity')"><?php echo number_format($cum_qc_qty,2); ?></a></td>
										<td width="100" style="word-break: break-all;" align="right"><? echo number_format($cum_qc_val,4);?></td>
										<td width="100" style="word-break: break-all;" align="right"><a href="##" onClick="fnc_amount_details(<?php echo "$wo_id, $section_id, $jobcard_id, $order_uom, $rate, $exchange_rate, '$txt_date_from', '$txt_date_to','$pop_order_uom','$item_group'"; ?>, 'issue_qty_popup', 'Material Issued Cost',<?php echo trim($issue_id, ","); ?>, '<?=$currency_id_ac?>','<?=$cbo_company_id?>')"><? echo number_format($cum_issue_val,4) ;?></a></td>
										<td width="80" style="word-break: break-all;" align="right"><? echo number_format($gross_margin_percent,2);?></td>
										<td style="word-break: break-all;" align="right"><? echo number_format($balance_qty,2);?></td>
									</tr>
									<? 
...
if($action=='today_issue_qty_popup') {
    ...
    <table width="1195" cellpadding="0" cellspacing="0" class="rpt_table" border="1" rules="all">
		...
		where a.entry_form in (427,501) and a.job_id=g.id and a.id = b.mst_id and a.job_no = g.trims_job and a.id = e.req_id and e.id=f.mst_id and b.product_id = f.prod_id and b.mst_id = e.req_id and b.job_no = g.trims_job and e.req_no=a.requisition_no and f.transaction_type = 2 and e.issue_basis = 7 and e.entry_form = 265 and a.status_active = 1 and b.status_active = 1 and e.status_active = 1 and f.status_active = 1 $job_card_cond and g.received_id in ($wo_id) $where_con_req
		union all
		SELECT c.received_id ,a.id as issue_id,a.issue_date,a.issue_number,a.req_no,b.cons_quantity as issue_qty, b.cons_rate,b.store_id,b.prod_id,b.cons_uom from inv_issue_master a, inv_transaction b, trims_job_card_mst c where a.id=b.mst_id and a.req_id=c.id and b.transaction_type=2 and a.issue_basis=15 and a.entry_form=265 $job_card_condd and c.received_id in ($wo_id) $where_con 
