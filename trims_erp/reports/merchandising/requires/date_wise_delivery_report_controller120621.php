...
if($action=="generate_report")
{
	...
	where a.subcon_job=b.job_no_mst and b.job_no_mst=c.job_no_mst and b.id=c.mst_id and a.entry_form=255 and a.company_id =$cbo_company_id $where_con_ord and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 $sql_order_no ";
	...
	$trims_order_sql= "select a.id,a.trims_del,a.delivery_date,b.received_id, b.job_dtls_id,b.production_dtls_id,b.break_down_details_id,b.order_no,b.buyer_po_no,b.section as section_id,b.delevery_qty,b.remarks,b.description as item_description, b.buyer_buyer,c.sub_section,c.job_no_mst,c.booked_conv_fac as conv_factor ,d.qnty,d.order_id ,d.qnty,d.rate,d.amount,d.booked_qty,d.description as item_description,d.id as breakdown_id from trims_delivery_mst a, trims_delivery_dtls b,subcon_ord_dtls c, subcon_ord_breakdown d where c.id=d.mst_id and d.id=b.break_down_details_id and  a.id=b.mst_id and c.id=b.receive_dtls_id $trimsreceiveid_cond  and a.entry_form=208 and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and c.status_active=1 and c.is_deleted=0 $sql_cond $where_con  $company $buyer_po_id_cond  order by a.id DESC"; 
							...
							foreach($item_description_data as $order_no_id=>$order_no_data)
							{
								foreach($order_no_data as $job_no_mst_id=>$deleverydate_data)
								{
					                ...
					                <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
					                    ...
					                    <td width="100" align="right"><? echo  number_format($delevery_quantity);  //$usdrate ?></td>
