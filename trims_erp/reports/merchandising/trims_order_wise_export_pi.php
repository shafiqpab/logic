...
//--------------------------------------------------------------------------------------------------------------------
echo load_html_head_contents("Trims Order Wise Export PI","../../../", 1, 1, $unicode,1,1); 
?>	
<script>
...
if( $('#index_page', window.parent.document).val()!=1) window.location.href = "../logout.php";
	...
	function load_sub_section(rowNo)
	{
		...
		load_drop_down( 'requires/trims_order_wise_export_pi_controller',section, 'load_drop_down_subsection', 'subSectionTd');
	}
	...
	function openmypage_order()
	{
		...
		else
		{
			...
			var page_link = 'requires/trims_order_wise_export_pi_controller.php?company='+company+'&party_name='+party_name+'&customer_source='+customer_source+'&action=order_popup';
	...
	function generate_report(report_type)
	{
		...
		var data="action=generate_report&report_type="+report_type+"&report_title="+$( "div.form_caption" ).html()+get_submitted_data_string('cbo_company_id*cbo_location_name*cbo_section_id*cbo_customer_source*cbo_customer_name*txt_order_no*txt_date_from*txt_date_to*hid_order_id*report_type_all*date_range_all',"../../../");
		
		//alert(data);
		freeze_window(3);
		http.open("POST","requires/trims_order_wise_export_pi_controller.php",true);
                        ...
                        <table class="rpt_table" cellpadding="0" cellspacing="0" border="1" rules="all">
                            ...
                            <tbody>
                            	<tr>
                                    <td>
										 <? 
                                       		 echo create_drop_down( "cbo_company_id",120,"select comp.id, comp.company_name from lib_company comp where comp.status_active=1 and comp.is_deleted=0 $company_cond order by comp.company_name","id,company_name", 1, "--Select Company--", $selected, "load_drop_down( 'requires/trims_order_wise_export_pi_controller', this.value, 'load_drop_down_location', 'location_td');","","","","","",2);  ?>
                                    </td>
                                    <td id="location_td">
		                                <?php 
                                    ...
                                    <td>
										<? 
                                       		 echo create_drop_down( "cbo_customer_source",100,$yes_no,"", 1, "--All--",0, "load_drop_down( 'requires/trims_order_wise_export_pi_controller', (document.getElementById('cbo_company_id').value+'_'+this.value), 'load_drop_down_buyer', 'buyer_td' )","","","","","",2);
                                        ?>
                                    </td>
                                    <td id="buyer_td">
										<? 
