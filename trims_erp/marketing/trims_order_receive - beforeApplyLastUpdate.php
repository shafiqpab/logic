...
<script>
	...
	function fnc_load_party(type,within_group)
	{
		...
		//load_drop_down( 'requires/trims_order_receive_controller', company+'_'+1, 'load_drop_down_group', 'group_td' );
	...
	function openmypage_order()
	{
		...
		else
		{
			...
			emailwindow=dhtmlmodal.open('EmailBox', 'iframe', page_link, title, 'width=700px,height=420px,center=1,resize=1,scrolling=0','../')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0];
	...
	function cal_booked_qty(rowNo)
	{
		...
		if(within_group==1)
		{
			...
			for(i=rowNo;i<=row_num;i++)
			{
				...
				{
					$('#txtConvFactor_'+i).val(1);
					$('#txtBookQty_'+i).val(txtOrderQuantity.toFixed(0));
				}
				else
				{
					$('#txtConvFactor_'+i).val(txtConvFactor);
					$('#txtBookQty_'+i).val((txtOrderQuantity*txtConvFactor).toFixed(0));
				}
			}
		}
		else
		{
			...
			{
				$('#txtConvFactor_'+rowNo).val(1);
				$('#txtBookQty_'+rowNo).val(txtOrderQuantity.toFixed(0));
			}
			else
			{
				var txtConvFactor=$('#txtConvFactor_'+rowNo).val()*1;
				$('#txtBookQty_'+rowNo).val((txtOrderQuantity*txtConvFactor).toFixed(0));
			}
		}
	}
