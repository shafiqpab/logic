...
$data_arr= json_encode( $_SESSION['logic_erp']['data_arr'][255] );
$mandatory_field_data = $_SESSION['logic_erp']['mandatory_field'][255];
...
<script>
    ...
    echo "var mandatory_field = '". implode('*',$mandatory_field_data) . "';\n";
	...
	function openmypage_job()
	{
		...
		emailwindow.onclose=function()
		{
			...
			{//alert(theemaildata);
				...
				show_list_view(2+'_'+order_no+'_'+within_group+'_'+ex_data[1]+'_'+update_id+'_'+cbo_round_type+'_'+company_name,'order_dtls_list_view','emb_details_container','requires/trims_order_receive_controller','setFilterGrid(\'list_view\',-1)');
	...
	function fnc_job_order_entry( operation )
	{
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#cbo_within_group').val(), "trims_order_receive_print", "requires/trims_order_receive_controller") 
			//return;
			show_msg("3");*/
		}
		else
		{
			...
			{	
				if ( form_validation('cbo_company_name*cbo_location_name*cbo_within_group*cbo_party_name*cbo_currency*txt_order_receive_date*txt_order_no*cbo_party_location','Company*location name*Within Group*Party*Currency*Order Receive Date*Order No.*Party Location')==false )
				{
					return;
				}
			}
			...
			{
				if ( form_validation('cbo_company_name*cbo_location_name*cbo_within_group*cbo_party_name*cbo_currency*txt_order_receive_date*txt_delivery_date','Company*location name*Within Group*Party*Currency*Order Receive Date*Order Delivery Date')==false )
				{
					return;
				}
                if(mandatory_field)
                {
                    if (form_validation(mandatory_field,"File*Work Order")==false)
                    {
                        return;
                    }
                }
			...
			if(document.getElementById('is_approved_id').value==1)
			{
				alert("This Order Receive is already Approved. So You can't change it.")
				return;
			}
			if(document.getElementById('is_approved_id').value==3)
			{
				alert("This Order Receive is Partial Approved. So You can't change it.")
				return;
			}
			...
			var section_variable_id = $('#section_variable_id').val();
			...
			var txt_fac_merchan 	= $('#txt_fac_merchan').val();
			var txt_buying_merchant = $('#txt_buying_merchant').val();
			var cbo_payterm_id      = $('#cbo_payterm_id').val();
			var cbo_wo_type_id      = $('#cbo_wo_type_id').val();
			var cbo_party_wise_grade      = $('#cbo_party_wise_grade').val();
			...
			var txt_trims_ref 		= $('#txt_trims_ref').val();
			var txt_delivery_point 	= $('#txt_delivery_point').val();
			var cbo_ready_to_approved = $('#cbo_ready_to_approved').val();
			var cbo_Status_type = $('#cbo_Status_type').val();
			var txt_buyer_tb = $('#txt_buyer_tb').val();
			...
			$("#tbl_dtls_emb tbody tr").each(function()
			{
				...
				var txtbuyerPo 			= encodeURIComponent($(this).find('input[name="txtbuyerPo[]"]').val());
				var txtstyleRef 		= encodeURIComponent($(this).find('input[name="txtstyleRef[]"]').val());
				...
				var txtSamSubDate 		= $(this).find('input[name="txtSamSubDate[]"]').val();
				var txtSamApprDate 		= $(this).find('input[name="txtSamApprDate[]"]').val();
				var txtOrderDeliveryDate 	= $(this).find('input[name="txtOrderDeliveryDate[]"]').val();
				var hdnDtlsdata 		= encodeURIComponent($(this).find('input[name="hdnDtlsdata[]"]').val());
				...
				var txtIsDuplicate 		= $(this).find('input[name="txtIsDuplicate[]"]').val();
				var txtIsRevised 		= $(this).find('input[name="txtIsRevised[]"]').val();
				var cboSource 			= $(this).find('select[name="cboSource[]"]').val();
				var txtPlaneCut 		= $(this).find('input[name="txtPlaneCut[]"]').val();
                var sizedtlsdata        = $(this).find('input[name="sizedtlsdata[]"]').val();

				
				if( txtIsRevised !=2 && (cboSection==0 || cboItemGroup==0 || cboUom==0 || txtOrderQuantity==''|| cboBookUom==0 || txtConvFactor==''|| txtConvFactor==0 || txtBookQty==''|| txtRate==''|| txtRate==0 || txtAmount==''|| txtAmount==0 || txtDomRate=='' || txtDomRate==0||  txtDomamount==''  || txtDomamount==0 || txtOrderDeliveryDate=='' ||  txtBookQty==''  || txtBookQty==0))
				{	 				
					if(cboSection==0){
					...
					else if(txtBookQty==''  || txtBookQty==0){
						alert('Please Fill up Booked Qty');
						check_field=1 ; return;
					}
					else{
		...
		if(check_field==0)
		{
			if('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][255]);?>'){
				if (form_validation('<? echo implode('*',$_SESSION['logic_erp']['mandatory_field'][255]);?>','<? echo implode('*',$_SESSION['logic_erp']['field_message'][255]);?>')==false)
				{
					release_freezing();
					return;
				}
	...
	function fnc_job_order_entry_response()
	{
		...
		if(http.readyState == 4) 
		{
			...
			{
				alert ("Delivery Done Update Not Allowe");
				release_freezing();
				return;
			}
			...
			else if(trim(response[0])=='28')
			{
				alert ("Order Quantity Can't Less Than Bill Quantity");
				release_freezing();
				return;
			}
			else if(trim(response[0])=='24')
			{
				alert ("Order Quantity Can't Less Than Production Quantity");
				release_freezing();
				return;
			}
			else if(trim(response[0])=='37')
			{
				alert ("Duplicate Work Order Not Allow");
				release_freezing();
				return;
			}
			/* else if(trim(response[0])=='31')
			{
				let text = response[2]+"\nPress a button!\nEither OK or Cancel.";
				if (confirm(text) == true) {
					text = "You pressed OK!";
					$("#txtIsDuplicate_"+response[1]).val(1);
				} else {
					$("#txtIsDuplicate_"+response[1]).val(0);
				}
				release_freezing();
				return;
			} */
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				show_list_view(2+'_'+order_no+'_'+within_group+'_'+response[1]+'_'+response[2]+'_'+cbo_round_type+'_'+company_name,'order_dtls_list_view','emb_details_container','requires/trims_order_receive_controller','setFilterGrid(\'list_view\',-1)');
				set_button_status(1, permission, 'fnc_job_order_entry',1);
				btn_load_change_bookings();
				set_all_onclick();
				fileUpload('order_file',$("#update_id").val(),'trims_order_receive','../../',1);
	...
	function show_print_report(type)
	{
		if (type==2) {
			...
			else
			{
				...
				var report_title=$( "div.form_caption" ).html();
				print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#cbo_within_group').val()+'*'+allow_rate, "trims_order_receive_print_2", "requires/trims_order_receive_controller") 
				return;
			}
		}
		else if (type==3) 
		{
			...
			//{
				...
				else
				{
					...
					if (rate_cond==true) allow_rate="1"; else allow_rate="0";
					var report_title=$( "div.form_caption" ).html();
					print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#cbo_within_group').val()+'*'+allow_rate, "trims_order_receive_print_3", "requires/trims_order_receive_controller") 
					return;
				}

			//}else{
				//alert("This Button Only For Within Group Yes");
				//return;
			//}
		}else if(type==4){
			if($('#update_id').val()=="")
			...
			else
			{
				...
				if (rate_cond==true) allow_rate="1"; else allow_rate="0";
				var report_title=$( "div.form_caption" ).html();
				print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#cbo_within_group').val()+'*'+allow_rate, "trims_order_receive_print_4", "requires/trims_order_receive_controller") 
				return;
			}
	...
	function fnc_load_party(type,within_group)
	{
		...
		if(within_group==1 && (type==1 || type==3 ))
		{
			...
			$("#txt_trims_ref").val(''); 
			$('#txt_trims_ref').attr('disabled',true);
		}
		else if(within_group==2 && type==1)
		{
			...
			$("#txt_trims_ref").val(''); 
			$('#txt_trims_ref').attr('disabled',false);
		}
	...
	function openmypage_order()
	{
		...
		else
		{
			...
			var page_link = 'requires/trims_order_receive_controller.php?company='+company+'&party_name='+party_name+'&cbo_within_group='+cbo_within_group+'&action=order_popup';
			...
			emailwindow.onclose=function()
			{
				...
				if (theemaildata!="")
				{
					...
					show_list_view(1+'_'+ex_data[1]+'_'+within_group+'_'+exchange_rate+'_'+ex_data[3]+'_'+cbo_round_type+'_'+company_name,'order_dtls_list_view','emb_details_container','requires/trims_order_receive_controller','setFilterGrid(\'list_view\',-1)');
	...
	function fnc_exchange_rate()
	{
		...
		var response=return_global_ajax_value(data, 'check_conversion_rate', '', 'requires/trims_order_receive_controller');
	...
	function section_variable_status()
	{
		//var rcv_date=$('#txt_rcv_date').val();
		var currency_id=$('#cbo_currency').val();
		var within_group = $('#cbo_within_group').val();
		var cbo_company_name = $('#cbo_company_name').val();
		var data =within_group+'**'+cbo_company_name;
		var response=return_global_ajax_value(data, 'section_variable', '', 'requires/trims_order_receive_controller');
		//alert(response);
		if(response)
		{
			$('#section_variable_id').val(response);
		}
		else
		{
			$('#section_variable_id').val('');
		}
	...
	function openmypage_order_qnty(row)
	{
		...
		var data_break=encodeURIComponent($('#hdnDtlsdata_'+row).val());
        var sizedtlsdata = $('#sizedtlsdata_'+row).val();
		var txtConvFactor=$('#txtConvFactor_'+row).val();
		//alert(data_break);
		var hdnDtlsUpdateId=$('#hdnDtlsUpdateId_'+row).val();
		var txtstyleRef=$('#txtstyleRef_'+row).val();
		var nextProcessChk=$('#nextProcessChk_'+row).val();
		//alert(hdnDtlsUpdateId+'='+txtConvFactor)
		var page_link = 'requires/trims_order_receive_controller.php?within_group='+within_group+'&order_no='+order_no+'&sizedtlsdata='+sizedtlsdata+'&data_break='+data_break+'&hdnDtlsUpdateId='+hdnDtlsUpdateId+'&booking_po_id='+booking_po_id+'&job_no='+job_no+'&txtConvFactor='+txtConvFactor+'&txtstyleRef='+txtstyleRef+'&nextProcessChk='+nextProcessChk+'&cbo_round_type='+cbo_round_type+'&company='+company+'&action=order_qty_popup';
		//alert(page_link);
		emailwindow=dhtmlmodal.open('EmailBox','iframe',page_link,'Quantity Details Pop-up', 'width=1250px, height=400px, center=1, resize=0, scrolling=0','../')
		emailwindow.onclose=function()
		{
			...
			var sizeinfodtls=this.contentDoc.getElementById("sizeinfodtls");
			...
			var receive_txtplane_total_amount=this.contentDoc.getElementById("txtplane_total"); 
			
			$('#hdnDtlsdata_'+row).val(break_data.value);
			$('#txtDeletedId_'+row).val(break_delete_id.value);
			$('#sizedtlsdata_'+row).val(sizeinfodtls.value);
			...
			$('#txtPlaneCut_'+row).val(receive_txtplane_total_amount.value);
			if(within_group==2)
			{
	...
	{ 
	   ...
	   var section_variable = $('#section_variable_id').val();
 		var within_group = $('#cbo_within_group').val();
		if(within_group==1)
		{
			return;
		...
		else
		{
			...
			clone.find("input,select").each(function(){
                if($(this).hasClass('rowCheck')){
                    ...
                    $(this).val(row_num);
                    $(this).attr('id', 'rowCheck_'+row_num);
                }else{
                    $(this).attr({
            ...
            $('#cboBookUom_'+row_num).prop("disabled", false);
			
			$('table #row_'+row_num+' #subSectionTd_'+i).removeAttr("id").attr('id','subSectionTd_'+row_num);
			$('table #row_'+row_num+' #itemGroupTd_'+i).removeAttr("id").attr('id','itemGroupTd_'+row_num);
			...
			//{
				//var section=$('#cboSection_'+rowNo).val();
 				//load_drop_down( 'requires/trims_order_receive_controller',section+'_'+rowNo , 'load_drop_down_itemgroup', 'itemGroupTd_'+rowNo );
				//$('#cboSection_'+row_num).removeAttr("onChange").attr("onChange","load_sub_section("+row_num+")");
			//}
			...
			$('#txtSamSubDate_'+row_num).removeAttr("onChange").attr("value","");
			$('#txtSamApprDate_'+row_num).removeAttr("onChange").attr("value","");
			$('#txtOrderDeliveryDate_'+row_num).removeAttr("onChange").attr("onChange","chk_min_del_date("+row_num+"); datecopy("+row_num+")");
			...
			$('#txtSamSubDate_'+row_num).removeAttr("class").attr("class","datepicker");
			$('#txtSamApprDate_'+row_num).removeAttr("class").attr("class","datepicker");
	...
	function dateCopy(rowNum){
        var row_num = $('#tbl_dtls_emb tbody tr').length;
        for(var i=rowNum;i<=row_num;i++)
        {
            $('#txtOrderDeliveryDate_'+i).val($('#txtOrderDeliveryDate_'+rowNum).val());
        }
    }
	...
	function cal_booked_qty(rowNo)
	{
		...
		if(within_group==1)
		{
			var order_uom=0; var cboBookUom=0; var txtplanecut=0;
			var row_num = $('#tbl_dtls_emb tbody tr').length;
			...
			for(i=rowNo;i<=row_num;i++)
			{
				...
				txtplanecut=$('#txtPlaneCut_'+i).val()*1;
				if(order_uom==cboBookUom)
				{
					$('#txtConvFactor_'+i).val(1);
					$('#txtBookQty_'+i).val(txtplanecut.toFixed(4));
				}
				else
				{
					$('#txtConvFactor_'+i).val(txtConvFactor);
					$('#txtBookQty_'+i).val((txtplanecut*txtConvFactor).toFixed(4));
				}
			}
		}
		else
		{
			...
			var txtplanecut=$('#txtPlaneCut_'+rowNo).val()*1;
			if(order_uom==cboBookUom)
			{
				$('#txtConvFactor_'+rowNo).val(1);
				$('#txtBookQty_'+rowNo).val(txtplanecut.toFixed(4));
			}
			else
			{
				var txtConvFactor=$('#txtConvFactor_'+rowNo).val()*1;
				$('#txtBookQty_'+rowNo).val((txtplanecut*txtConvFactor).toFixed(4));
			}
		}
	}
	...
	function set_form_data(data) 
	{
		...
		var  company_name 	= $('#cbo_company_name').val();
		var cbo_round_type=$('#cbo_round_type').val();
		show_list_view(2+'_'+order_no+'_'+within_group+'_'+id+'_'+update_id+'_'+cbo_round_type+'_'+company_name,'order_dtls_list_view','emb_details_container','requires/trims_order_receive_controller','setFilterGrid(\'list_view\',-1)');				
		set_button_status(1, permission, 'fnc_job_order_entry',1);
	...
	function copy_order_rcv(){
		...
		//alert (date);
		$("#update_id").val();
		$("#txt_order_receive_date").val(date);
		var within_group = $('#cbo_within_group').val();
		if(within_group==2){
			$("#txt_order_no").attr('disabled',false);
			$("#cbo_party_name").attr('disabled',false).val(0);
		}
		...
		for (var i=0;i<numRow; i++)
		{
			...
			/*hdnDtlsdata = $("#hdnDtlsdata_"+i).val();
			var dtls_data = hdnDtlsdata.split("**");*/
		} 
		set_button_status(0, permission, 'fnc_job_order_entry',1);
		
	}

	function copy_order_rcv_old(){
		...
		if(cbo_within_group==1)
		{	
			if ( form_validation('cbo_company_name*cbo_location_name*cbo_within_group*cbo_party_name*cbo_currency*txt_order_receive_date*txt_delivery_date*txt_order_no*cbo_party_location*cbo_team_leader*cbo_team_member','Company*location name*Within Group*Party*Currency*Order No*Order Receive Date*Order Delivery Date*Order No.*Party Location*Team Leader*Team Member')==false )
			{
				return;
			}
		}
		...
		{
			if ( form_validation('cbo_company_name*cbo_location_name*cbo_within_group*cbo_party_name*cbo_currency*txt_order_receive_date*txt_delivery_date*cbo_team_leader*cbo_team_member','Company*location name*Within Group*Party*Currency*Order Receive Date*Order Delivery Date*Team Leader*Team Member')==false )
			{
				return;
			}
		}
		...
		$("#tbl_dtls_emb tbody tr").each(function()
		{
			...
			j++;
			data_all += "&txtbuyerPoId_" + j + "='" + txtbuyerPoId + "'&txtbuyerPo_" + j + "='" + txtbuyerPo + "'&txtstyleRef_" + j + "='" + txtstyleRef + "'&txtbuyer_" + j + "='" + txtbuyer + "'&cboSection_" + j + "='" + cboSection  + "'&cboSubSection_" + j + "='" + cboSubSection + "'&cboItemGroup_" + j + "='" + cboItemGroup + "'&cboItemGroup_" + j + "='" + cboItemGroup + "'&txtOrderQuantity_" + j + "='" + txtOrderQuantity + "'&cboUom_" + j + "='" + cboUom + "'&cboBookUom_" + j + "='" + cboBookUom + "'&txtConvFactor_" + j + "='" + txtConvFactor + "'&txtBookQty_" + j + "='" + txtBookQty + "'&txtRate_" + j + "='" + txtRate + "'&txtAmount_" + j + "='" + txtAmount +"'&txtDomRate_" + j + "='" + txtDomRate +"'&txtDomamount_" + j + "='" + txtDomamount +"'&txtSamSubDate_" + j + "='" + txtSamSubDate +"'&txtSamApprDate_" + j + "='" + txtSamApprDate +"'&txtOrderDeliveryDate_" + j + "='" + txtOrderDeliveryDate +"'&hdnDtlsdata_" + j + "='" + hdnDtlsdata +"'&hdnDtlsUpdateId_" + j + "='" + hdnDtlsUpdateId +"'&hdnbookingDtlsId_" + j + "='" + hdnbookingDtlsId +"'&txtDeletedId_" + j + "='" + txtDeletedId +"'&txtIsWithOrder_" + j + "='" + txtIsWithOrder+"'&cboSource_" + j + "='" + cboSource + "'";
			i++;
		});
		...
		var data="action=copy_order_rcv&"+'total_row='+i+'&txt_job_no='+txt_job_no+'&cbo_company_name='+cbo_company_name+'&cbo_location_name='+cbo_location_name+'&cbo_within_group='+cbo_within_group+'&cbo_party_name='+cbo_party_name+'&cbo_party_location='+cbo_party_location+'&cbo_currency='+cbo_currency+'&txt_order_receive_date='+txt_order_receive_date+'&txt_delivery_date='+txt_delivery_date+'&txt_rec_start_date='+txt_rec_start_date+'&txt_rec_end_date='+txt_rec_end_date+'&txt_order_no='+txt_order_no+'&hid_order_id='+hid_order_id+'&txt_exchange_rate='+txt_exchange_rate+'&cbo_team_leader='+cbo_team_leader+'&cbo_team_member='+cbo_team_member+'&txt_fac_merchan='+txt_fac_merchan+'&txt_remarks='+txt_remarks+'&update_id='+update_id+'&txt_deleted_id='+txt_deleted_id+'&txt_is_sample='+txt_is_sample+'&is_apply_last_update='+is_apply_last_update+'&txt_revise_no='+txt_revise_no+'&txt_trims_ref='+txt_trims_ref+'&txt_delivery_point='+txt_delivery_point+data_all; 
		//alert (data); //return; 
		freeze_window(5);
		http.open("POST","requires/trims_order_receive_controller.php",true);
		http.setRequestHeader("Content-type","application/x-www-form-urlencoded");
		http.send(data);
		http.onreadystatechange = copy_order_rcv_reponse;
	}
	
	

	function copy_order_rcv_reponse()
	{
		if(http.readyState == 4) 
		{
			//alert (http.responseText);//return;
			...
			{
				alert ("Delivery Date not allowed before Today");
				release_freezing();
				return;
			}
			else if(trim(response[0])=='25')
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				var update_id 	= $('#update_id').val();
				 ...
				 var cbo_round_type=$('#cbo_round_type').val();
				//show_list_view(2+'_'+response[1]+'_'+within_group+'_'+response[2],'order_dtls_list_view','emb_details_container','requires/trims_order_receive_controller','setFilterGrid(\'list_view\',-1)');
				show_list_view(2+'_'+response[3]+'_'+within_group+'_'+response[1]+'_'+update_id[1]+'_'+cbo_round_type+'_'+company_name,'order_dtls_list_view','emb_details_container','requires/trims_order_receive_controller','setFilterGrid(\'list_view\',-1)');
				document.getElementById('txt_order_receive_date').value = response[4];
				set_button_status(1, permission, 'fnc_job_order_entry',1);
				btn_load_change_bookings();
				set_all_onclick();
	...
	function fnc_refpointup(type)
	{
		if( ($('#update_id').val()*1)!=0 )
		{
			...
			if(type==1)
			{
				var refpointdata=$('#txt_trims_ref').val()+'***'+type+'***'+$('#update_id').val();
			}
			else if(type==2)
			{
				var refpointdata=$('#txt_delivery_point').val()+'***'+type+'***'+$('#update_id').val();
			}
			var response=return_global_ajax_value(refpointdata, 'trimsref_deliverypointupdate', '', 'requires/trims_order_receive_controller');
		}
	}

    function rowSingleCheck(){
        var uncheckArr = [];
        $('.rowCheck').each(function (index) {
            if($(this).is(':checked') == false){
                uncheckArr.push('uncheck');
            }
        });
	...
	function copyExccessCut(){
        ...
        var excessCut = $('#common_excees_cut').val();
        if(excessCut == ''){
            alert("Excess % is required!");
            return;
        }
        if(uncheckArr.length == 0){
            alert("You did not select any row!");
        }else{
            $.each(uncheckArr, function (index, val) {
                if($('#hdnDtlsdata_'+val).val().length == 0){
                    var getArr = [];
                    for( var i = 0; i < 14; i++){
                        if(i == 11){
                            getArr[i] = excessCut;
                    ...
                    $('#hdnDtlsdata_'+val).val(getArr.join('_'));
                }else{
                    // console.log($('#hdnDtlsdata_'+val).val().split('***'));
                    var numberofRow = $('#hdnDtlsdata_'+val).val().split('***');
                    var tempArr = [];
                    var totalPlanQty = 0;
                    $.each(numberofRow, function (index, val) {
                        var getArr = [];
                        getArr = val.split('_');
                        var qty = getArr[3] == "" ? 0 : getArr[3];
                        var rate = getArr[4] == "" ? 0 : getArr[4];
                        var planQty = (parseFloat(qty)/100)*(100+parseFloat(excessCut));
                        var planAmount = planQty*parseFloat(rate);
                        getArr.splice(11, 3);
                        getArr.splice(11, 0, parseFloat(excessCut));
                        getArr.splice(12, 0, planQty);
                        getArr.splice(13, 0, planAmount);
                        tempArr.push(getArr.join('_'));
                        totalPlanQty += planQty;
                    });
                    $('#hdnDtlsdata_'+val).val(tempArr.join('***'));
                    var roundVal = Math.round((totalPlanQty + Number.EPSILON) * 10000) / 10000;
                    $('#txtPlaneCut_'+val).val(roundVal.toFixed(4));
                }
            });
...
<body onLoad="set_hotkey();btn_load_change_bookings();"> <!--//btn_load_change_bookings();-->
	<div style="width:100%;" align="center">
                ...
                <table width="800" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    ...
                    <tr>
                        ...
                        <td ><input name="txt_order_no" id="txt_order_no" type="text"  class="text_boxes" style="width:140px" placeholder="Browse" onDblClick="openmypage_order();" readonly />
                            ...
                            <input type="hidden" name="section_variable_id" id="section_variable_id">
                        </td>
                    </tr> 
                    <tr>
                    	<td >Team Leader</td>
                        ...
                        <td >Team Member</td>
                        <td id="member_td"><? echo create_drop_down( "cbo_team_member", 150,  $blank_array,"", 1, "-- Select Member --", $selected, ""); ?></td>
                        <td class="">Factory Merchant.</td>
                        <td id=""><input name="txt_fac_merchan" id="txt_fac_merchan" type="text"  class="text_boxes" style="width:140px" /></td>
                        
                    </tr>
                    <tr>
                    	<td>File</td>
                    	<td>
                    		<input type="file" multiple id="order_file" class="image_uploader" style="width:150px" accept=".text,.pdf,.xls,.xlsx" onChange="document.getElementById('txt_is_file_uploaded').value=1" >

							<input type="button" id="image_button" class="image_uploader" style="width:152px;" value="CLICK TO ADD FILE" onClick="file_uploader( '../../', document.getElementById('update_id').value,'', 'trims_order_receive',2,1)" />
                        ...
                        <td>
	                        ...
	                        terms_condition(255,'update_id','../../');
	                        ?>
                        </td>
                        <td>Trims Ref.</td>
                    	<td><input name="txt_trims_ref" id="txt_trims_ref" type="text"  class="text_boxes" style="width:140px" disabled="disabled" onBlur="fnc_refpointup(1);" /></td>
                    </tr>
                    <tr>
						<td>Ready To Approved</td>
						<td><? echo create_drop_down( "cbo_ready_to_approved", 150, $yes_no,"", 1, "-- Select--", 2, "","","" ); ?></td>
                        <td>Delivery Point</td>
                    	<td><input name="txt_delivery_point" id="txt_delivery_point" type="text" class="text_boxes" style="width:140px" onBlur="fnc_refpointup(2);"/></td>
                    	<td>Remarks</td>
                    	<td><input name="txt_remarks" id="txt_remarks" type="text"  class="text_boxes" style="width:140px" /></td>
                    </tr>
					<tr>
						<td>Round Type</td>
								<td><? $round_type=array(1=>"Round Up",2=>"Round Down"); echo create_drop_down( "cbo_round_type", 150, $round_type, "", 1,"-Select-", 0,"", 0 ); ?></td>
						<td id="color">Status</td>
						<td><? $Status=array(1=>"Active",2=>"InActive",3=>"Cancelled"); echo create_drop_down( "cbo_Status_type", 150, $Status, "", 1,"-Select-", 0,"", 0 ); ?></td>
						<td>Buyers TB</td>
                    	<td><input name="txt_buyer_tb" id="txt_buyer_tb" type="text"  class="text_boxes" style="width:140px" /></td>
					</tr>
		
					<tr>
						<td class="label">Buying Merchant</td>
						<td>
							<input name="txt_buying_merchant" id="txt_buying_merchant" type="text" class="text_boxes" style="width: 140px;">
						</td>
						<td class="label">Pay Term</td>
                        <td><?php echo create_drop_down("cbo_payterm_id", 150, $pay_term, '', 1, '-Select-', 0, "", 0, '');?></td>
                        <td class="label">WO Type</td>
                        <td><?php echo create_drop_down("cbo_wo_type_id", 150, $wo_type_arr, '', 1, '-Select-', 2, "", 0, '');?></td>
					</tr>
                    <tr>
						 
						<td class="label">Party Wise Grade</td>
                        <td><?php 
						$party_wise_grade_arr=array(1=>"A",2=>"B",3=>"C",4=>"D",5=>"E",6=>"F",7=>"G");
						
						 echo create_drop_down( "cbo_party_wise_grade", 150,$party_wise_grade_arr,"", 1, "-- Select Party Wise Grade --", "", "", 0, '')  ?></td>
                       
                         
					</tr>

					<tr>
					    <td colspan="5" id="refusing_cause" style="color:#F00; font-size:16px; font-weight:bold;"></td>
					</tr>
					...
					<tr>
                		...
                		<td align="center" valign="top">
							<div id="approved_msg" style="font-size:18px; color:#F00"></div>
						</td>
                	</tr>
                </table>
            </div>
        ...
        <fieldset style="width:1760px;">
            <legend style="height: 22px;"><div style="float:left;margin-top: 2px;padding-right: 8px;border-right: 2px solid #626262;">Trims Order Receive Details Entry</div><div style="float: left;padding-left: 8px;color: #000;">Excess % <input name="common_excees_cut" id="common_excees_cut" type="text"  class="text_boxes_numeric" style="width:75px" /><input type="checkbox" class="rowCheckall" onClick="chekcAll()" style="cursor: pointer; margin-left: 10px;" id="rowCheckAll"><label style="cursor: pointer" for="rowCheckAll">Check / Uncheck All</label><input type="button" value="Copy"  onclick="copyExccessCut()" style="width:80px;margin-left: 8px;border: 1px solid #464646;" id="copyexcess" class="formbutton"></div></legend>
                <table cellpadding="0" cellspacing="2" border="0" class="rpt_table" rules="all" id="tbl_dtls_emb">
                    <thead class="form_table_header">
                        ...
                        <th width="70" class="must_entry_caption">Plan Qty</th>
                        ...
                        <th width="60" class="must_entry_caption">Source</th>
                        <th></th>
                    </thead>
                    <tbody id="emb_details_container">
                        <tr id="row_1">
                            <td align="center"><input type="checkbox" value="1" class="rowCheck" id="rowCheck_1" onClick="rowSingleCheck()"></td>
                            ...
                            <td id="itemGroupTd_1"><? echo create_drop_down( "cboItemGroup_1", 90, "select id, item_name from lib_item_group where item_category=4 and status_active=1 order by item_name","id,item_name", 1, "-- Select --",$selected, "load_uom(1)",0,'','','','','','',"cboItemGroup[]"); ?>	</td>
                            ...
                            <td><input id="txtOrderQuantity_1" name="txtOrderQuantity[]" class="text_boxes_numeric" type="text"  style="width:60px" onClick="openmypage_order_qnty(1,'0',1)" placeholder="Click To Search" readonly /></td>
							 <td><input id="txtPlaneCut_1" title="new Colame" name="txtPlaneCut[]" class="text_boxes_numeric" type="text"  style="width:60px" readonly /></td>
                            ...
                            <td><input id="txtBookQty_1" title="here I have to work" name="txtBookQty[]" type="text"  class="text_boxes_numeric" style="width:57px" readonly /></td>
                            ...
                            <td><input type="text" name="txtSamSubDate[]" id="txtSamSubDate_1" class="datepicker"  style="width:67px" />
                            </td>
                            <td><input type="text" name="txtSamApprDate[]" id="txtSamApprDate_1" class="datepicker" style="width:67px" />
                            </td>
                            ...
                            <td><input type="text" name="txtOrderDeliveryDate[]" id="txtOrderDeliveryDate_1" class="datepicker" onChange="chk_min_del_date(1); dateCopy(1);" style="width:67px" />
                                ...
                                <input type="hidden" name="txtIsDuplicate[]" id="txtIsDuplicate_1" value="0">
                            </td>
                ...
                <table width="1480" cellspacing="2" cellpadding="0" border="0">
                    <tr>
						...
						style="visibility: hidden; width:120px"/>-->
                            ...
                            <input type="button" id="btn_print2" value="Print2" class="formbutton" style="width:100px;" onClick="show_print_report(2);" >
                            <input type="button" id="btn_print3" value="In-House" class="formbutton" style="width:100px;" onClick="show_print_report(3);" >
                            <input type="button" id="copy_btn" class="formbutton" value="Copy" onClick="copy_order_rcv();" style="width: 55px;" />
                            <input type="button" id="btn_print4" value="Print 4" class="formbutton" style="width:100px;" onClick="show_print_report(4);" >
