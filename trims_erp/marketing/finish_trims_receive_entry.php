...
<script>
	...
	function set_form_data(data) 
	{
		...
		var within_group = $('#cbo_within_group').val();
		show_list_view(2+'_'+order_no+'_'+within_group+'_'+id,'order_dtls_list_view','emb_details_container','requires/trims_order_receive_controller','setFilterGrid(\'list_view\',-1)');				
	...
	{
		if ( form_validation('cbo_company_name*cbo_receive_basis','Company*Receive Basis ')==false )
		{
			return;
		}
		var rcv_basis=$('#cbo_receive_basis').val();
		if(rcv_basis==2){
			var data=document.getElementById('cbo_company_name').value;
			page_link='requires/finish_trims_receive_entry_controller.php?action=wo_popup&data='+data;
			title='WO Pop-up';
			//alert(rcv_basis);
			emailwindow=dhtmlmodal.open('EmailBox','iframe', page_link, title, 'width=730px, height=420px, center=1, resize=0, scrolling=0','../')
			emailwindow.onclose=function()
			{
				var theform=this.contentDoc.forms[0]
				...
				{
					freeze_window(5);
					get_php_form_data( ex_data[0], "load_php_data_to_form", "requires/finish_trims_receive_entry_controller" );
					//var subcon_jobs=return_global_ajax_value(ex_data[0], 'return_subcon_jobs', '', 'requires/finish_trims_receive_entry_controller');
					//$('#txt_ord_rec_no').val(subcon_jobs);
					//$('#txt_ord_rec_id').val(ex_data[0]);
					
					show_list_view(2+'_'+ex_data[0],'order_dtls_list_view','tbl_dtls_emb','requires/finish_trims_receive_entry_controller','setFilterGrid(\'list_view\',-1)');				
					//set_button_status(1, permission, 'fnc_job_order_entry',1);
					set_all_onclick();
					//tr_disabled();
					release_freezing();
			...
			var cbo_company_name = $("#cbo_company_name").val();
	  		emailwindow=dhtmlmodal.open('EmailBox', 'iframe', 'requires/finish_trims_receive_entry_controller.php?cbo_company_name='+cbo_company_name+'&action=purchase_requisition_popup', 'Purchase Requisition Search', 'width=920px,height=400px,center=1,resize=0,scrolling=0','../')
	  		emailwindow.onclose=function()
	  		{
	  			...
	  			{
	  				freeze_window(5);
	  				get_php_form_data( theemail.value, "load_php_req_data_to_form", "requires/finish_trims_receive_entry_controller" );
	  				show_list_view(3+'_'+theemail.value,'order_dtls_req_list_view','tbl_dtls_emb','requires/finish_trims_receive_entry_controller','setFilterGrid(\'list_view\',-1)');	
	  				...
	  				disable_enable_fields('cbo_company_name*cbo_location_name*cbo_receive_basis',1);
	  				//set_button_status(0, permission, 'fnc_purchase_requisition',1,0);
	  				release_freezing();
	  			}
	  		}
		}
	...
	function fnc_job_order_entry( operation )
	{
		...
		if ( form_validation('cbo_company_name*cbo_receive_basis*txt_wo_no*cbo_store_name','Company*Receive Basis*Work Order*cbo_store_name')==false )
		{
			return;
		}
		...
		if(operation==4)
		{
			...
			print_report( $('#cbo_company_name').val()+'*'+$('#update_id').val()+'*'+report_title+'*'+$('#cbo_receive_basis').val(), "trims_finish_rcv_print", "requires/finish_trims_receive_entry_controller") 
			//return;
			show_msg("3");
		}
		else
		{
			...
			var txt_challan_date 	= $('#txt_challan_date').val();
			var cbo_store_name 		= $('#cbo_store_name').val();
			var cbo_receive_basis 	= $('#cbo_receive_basis').val();

			var update_id 			= $('#update_id').val();
			var j=0; var check_field=0; var qty_chk=0; var check_req_field=0; data_all="";
			...
			$("#tbl_dtls_emb tbody tr").each(function()
			{
				...
				var txtItemDesc 		= encodeURIComponent($(this).find('input[name="txtItemDesc[]"]').val());
				...
				if(check_field !=1 )
				{
					...
					{
						j++;
						data_all += "'&txtRcvQty_" + j + "='" + txtRcvQty  + "'&txtRemark_" + j + "='" + txtRemark + "'&hdnRcvId_" + j + "='" + hdnRcvId + "'&hdnWoDtlsId_" + j + "='" + hdnWoDtlsId + "'&hdnWoBrkId_" + j + "='" + hdnWoBrkId + "'&hdnDtlsUpdateId_" + j + "='" + hdnDtlsUpdateId + "'";
						i++;
					}
				}

				/*
				if(cbo_receive_basis !=7){
					if(txtRcvQty=='' || txtRcvQty==0){
						alert('Please Fill up Receive Qnty ');
		...
		if( qty_chk==1 )
		{
			var data="action=save_update_delete&operation="+operation+'&total_row='+i+'&txt_wo_no='+txt_wo_no+'&cbo_company_name='+cbo_company_name+'&txt_system_id='+txt_system_id+'&hid_wo_id='+hid_wo_id+'&txt_order_receive_date='+txt_order_receive_date+'&txt_challan_no='+txt_challan_no+'&txt_challan_date='+txt_challan_date+'&cbo_store_name='+cbo_store_name+'&cbo_receive_basis='+cbo_receive_basis+'&update_id='+update_id+data_all;
			//alert (data); return; 
			//freeze_window(operation);
			http.open("POST","requires/finish_trims_receive_entry_controller.php",true);
	...
	function fnc_job_order_entry_response()
	{
		...
		if(http.readyState == 4) 
		{
			...
			if(response[0]==0 || response[0]==1)
			{
				...
				$('#cbo_within_group').attr('disabled',true);
				*/
				//show_list_view(2+'_'+response[1]+'_'+within_group+'_'+response[2],'order_dtls_list_view','emb_details_container','requires/finish_trims_receive_entry_controller','setFilterGrid(\'list_view\',-1)');
				show_list_view(2+'_'+response[2]+'_'+wo_id+'_'+receive_basis,'order_dtls_list_view_update','tbl_dtls_emb','requires/finish_trims_receive_entry_controller','setFilterGrid(\'list_view\',-1)');
				set_button_status(1, permission, 'fnc_job_order_entry',1);
	...
	function openmypage_job()
	{
		...
		var data=document.getElementById('cbo_company_name').value;
		page_link='requires/finish_trims_receive_entry_controller.php?action=job_popup&data='+data;
		title='System ID Pop-up';
		...
		emailwindow.onclose=function()
		{
			...
			{
				freeze_window(5);
				get_php_form_data( ex_data[0]+'_'+ex_data[3], "load_php_rcv_data_to_form", "requires/finish_trims_receive_entry_controller" );
				//var subcon_jobs=return_global_ajax_value(ex_data[0], 'return_subcon_jobs', '', 'requires/finish_trims_receive_entry_controller');
				//$('#txt_ord_rec_no').val(subcon_jobs);
				//$('#txt_ord_rec_id').val(ex_data[0]);
				var receive_basis = $('#cbo_receive_basis').val();
				
				show_list_view(2+'_'+ex_data[0]+'_'+ex_data[2]+'_'+receive_basis,'order_dtls_list_view_update','tbl_dtls_emb','requires/finish_trims_receive_entry_controller','setFilterGrid(\'list_view\',-1)');				
				set_button_status(1, permission, 'fnc_job_order_entry',1);
				set_all_onclick();
				//tr_disabled();
				release_freezing();
        ...
        <form name="trims_order_receive" id="trims_order_receive" autocomplete="off"> 
			...
			<div style="width:850px; float:left;" align="center">
                <table width="800" cellspacing="2" cellpadding="0" border="0" id="tbl_master">
                    ...
                    <td>
                        <? 
                        	echo create_drop_down("cbo_receive_basis",150,$receive_basis_arr,"",1,"-- Select --",2,"set_receive_basis();","",'2,4,7,14');
                        ?>
                    </td>
                    </tr>
                    <tr>
                        ...
                        echo create_drop_down( "cbo_source_id", 151, $source_for_order,'', 0, '',0,0); ?></td>
                        <td class="must_entry_caption" ><strong>WO / Requisition</strong></td>
                        <td ><input name="txt_wo_no" id="txt_wo_no" type="text"  class="text_boxes" style="width:140px" placeholder="Browse" onDblClick="openmypage_wo();" readonly />
                            <input type="hidden" name="hid_wo_id" id="hid_wo_id">
                        </td>
                        <td >Receive Date</td>
                    ...
                    <tr>
                    	<td>Challan No</td>
                    	<td ><input name="txt_challan_no" id="txt_challan_no" type="text"  class="text_boxes" style="width:140px" /></td>

                    	<td >Challan Date</td>
                        <td><input type="text" name="txt_challan_date" id="txt_challan_date"  style="width:140px"  class="datepicker" value=""/></td>
                        <td >Supplier</td>
                    ...
                    <tr>
	                    ...
	                    <td id="store_td">
							<?
	                            echo create_drop_down( "cbo_store_name", 150, $blank_array,"",1, "--Select store--", 1, "" );
	                        ?>
	                    </td>
                    </tr> 
                </table>
           ...
           <legend>Trims Order Receive Details Entry</legend>
                <table cellpadding="0" cellspacing="2" border="0" class="rpt_table" rules="all" id="tbl_dtls_emb">
                    <thead class="form_table_header">
                    	<tr><th colspan="12">Order No :</th></tr>
                    	<tr>
                    ...
                    <tbody id="emb_details_container">
                        <tr id="row_1">
                            ...
                            <td><? echo create_drop_down( "cboSection_1", 110, $trims_section,"", 1, "-- Select Section --","","load_sub_section(1)",0,'','','','','','',"cboSection[]"); ?></td>
                            <td id="subSectionTd_1"><? echo create_drop_down( "cboSubSection_1", 110, $trims_sub_section,"", 1, "-- Select Sub-Section --","",'',0,'','','','','','',"cboSubSection[]"); ?></td>
                            <td><? echo create_drop_down( "cboItemGroup_1", 110, "select id, item_name from lib_item_group where item_category=4 and status_active=1","id,item_name", 1, "-- Select --",$selected, "load_uom(1)",0,'','','','','','',"cboItemGroup[]"); ?>	</td>
                            <td><input id="txtItemdescription_1" name="txtItemdescription[]" type="text"  class="text_boxes" style="width:157px"  /></td>
                            <td><input id="txtColor_1" name="txtColor[]" type="text"  class="text_boxes" style="width:87px"  />
                            	<input id="txtColorId_1" name="txtColorId[]" type="hidden"  class="text_boxes" style="width:47px"  /></td>
                            <td><input id="txtSize_1" name="txtSize[]" type="text"  class="text_boxes" style="width:77px"  />
                            	<input id="txtSizeId_1" name="txtSizeId[]" type="hidden"  class="text_boxes" style="width:47px"  /></td>
                            <td><? echo create_drop_down( "cboUom_1", 60, $unit_of_measurement,"", 1, "-- Select --",2,1, 1,'','','','','','',"cboUom[]"); ?>	</td>
                            <td><input id="txtOrderQuantity_1" name="txtOrderQuantity[]" class="text_boxes_numeric" type="text"  style="width:47px" onClick="openmypage_order_qnty(1,'0',1)" readonly /></td>
                            <td><input id="txtCumRcvQty_1" name="txtCumRcvQty[]" type="text"  class="text_boxes_numeric" style="width:47px" readonly /></td>
                            <td><input id="txtRcvQty_1" name="txtRcvQty[]" type="text" style="width:70px"  class="text_boxes_numeric" readonly /></td> 
                            <td><input id="txtRcvQtyBal_1" name="txtRcvQtyBal[]" type="text"  class="text_boxes_numeric" style="width:57px" readonly /></td> 
                            <td><input id="txtRemarks_1" name="txtRemarks[]" type="text"  class="text_boxes_numeric" style="width:97px" readonly />
