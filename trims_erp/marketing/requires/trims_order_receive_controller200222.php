...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Start Here
	{
		...
		else
		{
			...
			$field_array3="id, mst_id, order_id, job_no_mst, book_con_dtls_id, description, color_id, size_id, qnty, rate, amount, booked_qty, style, inserted_by, insert_date, excess_cut, plan_cut, plan_cut_amount";
			...
			{			
				...
				for($j=0; $j<count($dtls_data); $j++)
				{
					...
					$data_array3.="(".$id3.",".$id1.",'".$hid_order_id."','".$new_job_no[0]."',".$book_con_dtls_id.",'".trim($description)."','".$color_id."','".$size_id."',".$qty.",".$rate.",".$amount.",".$booked_qty.",".$style.",'".$user_id."','".$pc_date_time."',".$excess_cut.",".$plan_cut.",".$plan_cut_amount.")";
	...
	else if ($operation==1)   // Update Here
	{
		...
		if($sub_order_no!="")
		{
			echo "20**".str_replace("'",'',$txt_job_no)."**".str_replace("'",'',$update_id)."**".str_replace("'",'',$txt_order_no)."**".$sub_order_no;
			disconnect($con);
			die;
		}*/
		...
		else
		{
			...
			//echo "10**".$next_process_bill.'**'.$next_process.'**'.$chk_next_process; die;
			if($chk_next_process==0)
			{
				$field_array="location_id*within_group*party_id*party_location*currency_id*receive_date*delivery_date*rec_start_date*rec_end_date*order_id*order_no*exchange_rate*team_leader*team_member*team_marchant*delivery_point*ready_to_approved*remarks*trims_ref*is_apply_last_update*revise_no*updated_by*update_date";
				...
				$field_array3="id, mst_id, order_id, job_no_mst, book_con_dtls_id, description, color_id, size_id, qnty, rate, amount,booked_qty,style, inserted_by, insert_date, excess_cut, plan_cut,  plan_cut_amount";
				$field_array4="order_id*book_con_dtls_id*description*color_id*size_id*qnty*rate*amount*booked_qty*style*updated_by*update_date*excess_cut*plan_cut*plan_cut_amount";
				...
				$data_array="'".$cbo_location_name."'*'".$cbo_within_group."'*'".$cbo_party_name."'*'".$cbo_party_location."'*'".$cbo_currency."'*'".$txt_order_receive_date."'*'".$txt_delivery_date."'*'".$txt_rec_start_date."'*'".$txt_rec_end_date."'*'".$hid_order_id."'*'".$txt_order_no."'*'".$txt_exchange_rate."'*'".$cbo_team_leader."'*'".$cbo_team_member."'*'".$txt_fac_merchan."'*'".$txt_delivery_point."'*'".$cbo_ready_to_approved."'*'".$txt_remarks."'*'".$txt_trims_ref."'*'".$lastUpdate."'*'".$revise_no."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
				...
				{			
					...
					for($j=0; $j<count($dtls_data); $j++)
					{
						...
						{ 
							...
							{
								//echo "10**".$bb."**".$cbo_within_group; die;
								if(str_replace("'",'',$$hdnDtlsUpdateId)!='') $dtlsIdForBreak=str_replace("'",'',$$hdnDtlsUpdateId); else $dtlsIdForBreak=$id1;
								if ($add_commadtls!=0) $data_array3 .=",";
								$data_array3.="(".$id3.",".$dtlsIdForBreak.",'".$hid_order_id."','".$txt_job_no."',".$book_con_dtls_id.",'".trim($description)."','".$color_id."','".$size_id."',".$qty.",".$rate.",".$amount.",".$booked_qty.",'".$style."','".$user_id."','".$pc_date_time."',".$excess_cut.",".$plan_cut.",".$plan_cut_amount.")";
								$id3=$id3+1; $add_commadtls++;
							}
							//else if($bb!=0 && (!in_array($dtlsIdForBreak, $brk_item_missmatch_chk_arr)))\
							else if($bb!=0 && (!in_array($dtlsIdForBreak, $brk_item_missmatch_chk_arr)))
							{
								$data_array4[$bb]=explode("*",("".$hid_order_id."*".$book_con_dtls_id."*'".trim($description)."'*".$color_id."*".$size_id."*".$qty."*".$rate."*".$amount."*".$booked_qty."*'".$style."'*".$user_id."*'".$pc_date_time."',".$excess_cut.",".$plan_cut.",".$plan_cut_amount.""));
				...
				//echo "10**".bulk_update_sql_statement( "subcon_ord_breakdown", "id",$field_brk_array_re,$data_brk_array_re,$revBrk_id_arr); die;
				//die;
				...
				//echo "10**".$breakDelIds.'='.$txt_deleted_id; die;
			...
			{
				$field_array="ready_to_approved*updated_by*update_date";
				...
				$data_array="'".$cbo_ready_to_approved."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
				...
				for($i=1; $i<=$total_row; $i++)
				{
					...
					for($j=0; $j<count($dtls_data); $j++)
					{
						...
						$rate="'".str_replace(",",'',$exdata[4])."'";
						$amount="'".str_replace(",",'',$exdata[5])."'";
						$excess_cut="'".str_replace(",",'',$exdata[11])."'";
						$plan_cut="'".str_replace(",",'',$exdata[12])."'";
						$plan_cut_amount="'".str_replace(",",'',$exdata[13])."'";
						$dtlsup_id="'".$exdata[7]."'";
						$bb=$exdata[7];
						$data_array4[$bb]=explode("*",("".$rate."*".$amount."*".$user_id."*'".$pc_date_time."'*".$excess_cut."*".$plan_cut."*".$plan_cut_amount.""));
						$hdn_break_id_arr[]		=$bb;
					}
				}
			}
			...
			//echo "10**".$id_break; die;
			//echo "10**".$rID.'='.$rID2.'='.$rID3.'='.$rID4.'='.$rID6.'='.$rID7.'='.$rID8.'='.$rID_re.'='.$rIDBrk_re.'='.$rIDBooking.'='.$flag.'='.str_replace("'",'',$is_apply_last_update); die;
			//1===0=======0=0
			//10**1=1=1=1===1====1=0
			//if($rID4) $flag=1; else $flag=0;
...
{	
	...
	$sql= "select a.id, a.subcon_job, a.job_no_prefix_num, $ins_year_cond as year, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date, b.item_group $buyer_po_no_str $buyer_po_style_str $color_id_str $buyer_po_id_str
	from subcon_ord_mst a, subcon_ord_dtls b, subcon_ord_breakdown c  
	...
	group by a.id, a.subcon_job, a.job_no_prefix_num, a.insert_date, a.company_id, a.location_id, a.party_id, a.receive_date, a.order_no, a.delivery_date, b.item_group
	order by a.id DESC";
	//echo $sql;
...
if( $action=='order_dtls_list_view' ) 
{
	...
	$bookConsIds_arr=array();
	if($data[2]==1 && $data[0]==2 )
	{
		//ho "select id, book_con_dtls_id,mst_id from subcon_ord_breakdown where job_no_mst='$data[3]'"; die;
		$sql_cons=sql_select("select id, book_con_dtls_id,mst_id,style, excess_cut, plan_cut, plan_cut_amount from subcon_ord_breakdown where status_active=1 and is_deleted=0 and  job_no_mst='$data[3]'");
		foreach($sql_cons as $row)
		{
			...
			//$bookConsIds_arr[$row[csf('book_con_dtls_id')]]['mst_id']=$row[csf('mst_id')];
		}
		unset($sql_cons);
	}
	...
	if($data[2]==1)
	{
		...
		$qry_result = sql_select( "select  id as book_con_dtls_id, wo_trim_booking_dtls_id, job_no,  po_break_down_id,  item_color as color_id, item_size as size_id, requirment as qnty, description, brand_supplier, rate, amount from wo_trim_book_con_dtls where booking_no='$data[1]' and status_active=1 and is_deleted=0 and requirment!=0 order by id ASC");
		...
		foreach ($qry_result as $rows)
		{
			...
			//.'_'.$rows[csf('excess_cut')].'_'.$rows[csf('plan_cut')].'_'.$rows[csf('plan_cut_amount')]

			$data_dreak_arr[$rows[csf('wo_trim_booking_dtls_id')]].=$rows[csf('description')].'_'.$color_library[$rows[csf('color_id')]].'_'.$rows[csf('size_id')].'_'.$rows[csf('qnty')].'_'.$rows[csf('rate')].'_'.$rows[csf('amount')].'_'.$rows[csf('book_con_dtls_id')].'_'.$breakDownUpId.'_'.$rows[csf('is_revised')].'_0_'.$style.'_'.$excess_cut.'_'.$plan_cut.'_'.$plan_cut_amount.'#';

			/*$data_dreak_arr[$rows[csf('mst_id')]].=$rows[csf('description')].'_'.$color_library[$rows[csf('color_id')]].'_'.$size_arr[$rows[csf('size_id')]].'_'.$rows[csf('qnty')].'_'.$rows[csf('rate')].'_'.$rows[csf('amount')].'_'.$rows[csf('book_con_dtls_id')].'_'.$rows[csf('id')].'_'.$rows[csf('is_revised')].'_0_'.$rows[csf('style')].'_'.$rows[csf('excess_cut')].'_'.$rows[csf('plan_cut')].'_'.$rows[csf('plan_cut_amount')].'#';*/
		}
	}
	else
	{
		$qry_result=sql_select( "select id, mst_id, order_id, job_no_mst, book_con_dtls_id, description, color_id, size_id, qnty, rate, amount,is_revised, style, excess_cut, plan_cut, plan_cut_amount from subcon_ord_breakdown where status_active=1 and is_deleted=0 and job_no_mst='$data[3]' order by id");	
	...
	if($data[2]==1 && $data[0]==1 )
	{
		...
		if(trim($data[4])==1)
		{
			$sql = "SELECT  a.id, a.booking_type, a.booking_no, a.company_id, a.buyer_id, a.job_no, a.booking_date, a.currency_id, b.id as booking_dtls_id, b.po_break_down_id, b.trim_group,b.delivery_date,b.fabric_description, b.uom, b.wo_qnty, b.rate, b.amount,1 as source_for_order
		...
		{
			$sql = "SELECT  a.id, a.booking_type, a.booking_no, a.company_id, a.buyer_id, a.job_no, a.booking_date, a.currency_id, b.id as booking_dtls_id, a.po_break_down_id, b.trim_group,b.delivery_date,b.fabric_description, b.gmts_color, b.fabric_color, b.gmts_size, b.item_size, b.uom, b.trim_qty as wo_qnty, b.rate, b.amount,1 as source_for_order
		...
		foreach($data_array as $row)
		{
			...
			if($data[2]==1)  //within group yes 
			{
				...
				else if($data[0]==2)
				{
					...
					$disabled='disabled';
					$data_break="";
					//echo $row[csf('booking_dtls_id')];
					//print_r($data_dreak_arr[$row[csf('booking_dtls_id')]]);
					$data_break=implode("***",array_filter(explode('#',$data_dreak_arr[$row[csf('booking_dtls_id')]])));
				}
			}
			else if($data[2]==2)
			...
			<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
				...
				<td>
					...
					if($row[csf('section')]==1) $subID='1,2,3,23';
					...
					else if($row[csf('section')]==7) $subID='19,20,21';
				...
				<td><? echo create_drop_down( "cboItemGroup_".$tblRow, 90, "select id, item_name from lib_item_group where item_category=4 and status_active=1","id,item_name", 1, "-- Select --",$row[csf('trim_group')], "",$next_proc_disabled_dropdown,'','','','','','',"cboItemGroup[]"); ?></td>
				<td><? echo create_drop_down( "cboUom_".$tblRow, 60, $unit_of_measurement,"", 1, "-- Select --",$order_uom,"", 1,'','','','','','',"cboUom[]"); ?>	</td>
				<td><input id="txtOrderQuantity_<? echo $tblRow; ?>" name="txtOrderQuantity[]" value="<?if($rnd_type==1){ echo round($wo_qnty); }else{  echo number_format($wo_qnty,4,'.','');} ?>" class="text_boxes_numeric" type="text"  style="width:60px" onClick="openmypage_order_qnty(<? echo $tblRow; ?>)" placeholder="Click To Search" readonly /></td>

				<td><input id="txtPlaneCut_<? echo $tblRow; ?>" name="txtPlaneCut[]" value="<?if($rnd_type==1){ echo round($plan_cut); }else{  echo number_format($plan_cut,4,'.','');} ?>" class="text_boxes_numeric" type="text"  style="width:60px" readonly /></td>
				...
				<td><input id="txtBookQty_<? echo $tblRow; ?>" name="txtBookQty[]" type="text"  class="text_boxes_numeric" style="width:57px"  value="<? if($rnd_type==1){ echo round($row[csf('booked_qty')]); } else{ echo number_format($row[csf('booked_qty')],4,'.',''); }  ?>" readonly /></td>
		...
		function fnc_close()
		{
			...
			$("#tbl_share_details_entry tbody tr").each(function()
			{
				...
				if(check_field==0)
				{
					if(data_break_down=="")
					{
						data_break_down+=txtdescription+'_'+txtcolor+'_'+txtsize+'_'+txtorderquantity+'_'+txtorderrate+'_'+txtorderamount+'_'+hidbookingconsid+'_'+hiddenid+'_'+txtcolorId+'_'+txtsizeID+'_'+txtStyle+'_'+txtexcesscut+'_'+txtplanecut+'_'+txtplanecutamount;
					}
					else
					{
						data_break_down+="***"+txtdescription+'_'+txtcolor+'_'+txtsize+'_'+txtorderquantity+'_'+txtorderrate+'_'+txtorderamount+'_'+hidbookingconsid+'_'+hiddenid+'_'+txtcolorId+'_'+txtsizeID+'_'+txtStyle+'_'+txtexcesscut+'_'+txtplanecut+'_'+txtplanecutamount;
		...
		<form name="qntypopup_1"  id="qntypopup_1" autocomplete="off">
			<table class="rpt_table" width="1080px" cellspacing="0" cellpadding="0" rules="all" id="tbl_share_details_entry">
				<thead>
					...
					<th width="60">Rate</th>
					<th width="80">Amount</th>
					<th width="90">Plan Cut Amount</th>
					<th>&nbsp;</th>
				</thead>
				<tbody>
					<input type="hidden" name="txtDeletedId" id="txtDeletedId" class="text_boxes_numeric" style="width:90px" readonly />
					...
					if($is_available_datas>0)
					{
						foreach($data_array as $row)
						{
							...
							?>
							<tr id="row_<? echo $k;?>">
								<td><input type="text" name="txtSl[]" id="txtRow_<? echo $k;?>" class="text_boxes" style="width:30px" value="<? echo $k; ?>" disabled /></td>
								<td><input type="text" id="txtStyle_<? echo $k;?>" name="txtStyle[]" class="text_boxes" style="width:120px" value="<? echo $styleRef; ?>" <? echo $disabled; ?> <? echo $disabled_next; ?> /></td>
								<td><input type="text" id="txtdescription_<? echo $k;?>" name="txtdescription[]" class="text_boxes" style="width:120px" value="<? echo $data[0]; ?>" <? echo $disabled_next; ?> />
								</td>
								<td>
									<input type="text" id="txtcolor_<? echo $k;?>" name="txtcolor[]" class="text_boxes txt_color" style="width:90px" value="<? echo $data[1]; ?>" <? echo $disabled_next; ?> >
									<input type="hidden" id="txtcolorId_<? echo $k;?>" name="txtcolorId[]" class="text_boxes_numeric" style="width:90px" value="<? echo $data[8]; ?>"  /></td>
								<td><input type="text" id="txtsize_<? echo $k;?>" name="txtsize[]" class="text_boxes txt_size" style="width:70px" value="<? echo $data[2]; ?>" <? echo $disabled_next; ?> >
									<input type="hidden" id="txtsizeID_<? echo $k;?>" name="txtsizeID[]" class="text_boxes_numeric" style="width:70px" value="<? echo $data[9]; ?>"></td>
								<td>
									<input type="text" id="txtorderquantity_<? echo $k;?>" name="txtorderquantity[]" class="text_boxes_numeric" style="width:70px" onKeyUp="sum_total_qnty(<? echo $k;?>);" value="<? echo number_format($data[3],4,'.',''); ?>" <? echo $disabled; ?> />
								...
								<td><input type="text" title="new" id="txtexcesscut_<? echo $k;?>" name="txtexcesscut[]"  class="text_boxes_numeric" style="width:60px" onKeyUp="sum_total_qnty(<? echo $k;?>)" value="<? echo number_format($data[11],0,'.',''); ?>" />
								</td>
								<td><input type="text" title="new" id="txtPlaneCut_<? echo $k;?>" name="txtPlaneCut[]"  class="text_boxes_numeric" style="width:60px" onKeyUp="sum_total_qnty(<? echo $k;?>)" value="<? echo number_format($data[12],4,'.',''); ?>" <? echo $disabled; ?> />
								</td>
								<td><input type="text" id="txtorderrate_<? echo $k;?>" name="txtorderrate[]"  class="text_boxes_numeric" style="width:60px" onKeyUp="sum_total_qnty(<? echo $k;?>)" value="<? echo number_format($data[4],4,'.',''); ?>" <? echo $disabled; ?> />
								</td>
								...
								<td><input type="text" title="new" id="txtPlaneCutAmount_<? echo $k;?>" name="txtPlaneCutAmount[]" class="text_boxes_numeric" style="width:90px" value="<? echo number_format($data[13],4,'.',''); ?>" disabled/></td>
								<td>
					...
					else
					{
						?>
                        <tr id="row_1">
							<td><input type="text"  name="txtSl[]" id="txtRow_1" class="text_boxes" style="width:30px" value="1" disabled /></td>
                        	<td><input type="text" id="txtStyle_1" name="txtStyle[]" class="text_boxes" style="width:120px" value="<? echo $txtstyleRef; ?>" /></td>
                        	<td><input type="text" id="txtdescription_1" name="txtdescription[]" class="text_boxes" style="width:120px" value="" /></td>
							<td>
								<input type="text" id="txtcolor_1" name="txtcolor[]" class="text_boxes txt_color" style="width:90px" value="" >
								<input type="hidden" id="txtcolorId_1" name="txtcolorId[]" class="text_boxes_numeric" style="width:90px" value=""  /></td>
							<td><input type="text" id="txtsize_1" name="txtsize[]" class="text_boxes txt_size" style="width:70px" value=""  >
								<input type="hidden" id="txtsizeID_1" name="txtsizeID[]" class="text_boxes_numeric" style="width:70px" value=""></td>
							<td>
							...
							<td><input type="text" id="txtexcesscut_1" title="new" name="txtexcesscut[]"  class="text_boxes_numeric" style="width:60px" onKeyUp="sum_total_qnty(1)" value="" />
							</td>
							<td><input type="text" id="txtPlaneCut_1" title="new" name="txtPlaneCut[]"  class="text_boxes_numeric" style="width:60px" onKeyUp="sum_total_qnty(1)" value="" <? echo $disabled; ?> />
							</td>
							<td><input type="text" id="txtorderrate_1" name="txtorderrate[]"  class="text_boxes_numeric" style="width:60px" onKeyUp="sum_total_qnty(1)" value="" <? echo $disabled; ?> />
							</td>
				...
				<tfoot>
					...
					<th><input type="text" id="txtplane_total" value="<? echo $break_avg_rate; ?>";  name="txtplane_total" class="text_boxes_numeric" readonly style="width:61px" value="" /></th>
					<th><input type="text" id="txt_average_rate" name="txt_average_rate" class="text_boxes_numeric" readonly style="width:61px" value="<? echo $break_avg_rate; ?>"; /></th>
					<th><input type="text" id="txt_total_order_amount" name="txt_total_order_amount" class="text_boxes_numeric" readonly style="width:70px" value="<? echo $break_total_value; ?>"; /></th>
...
if($action=="trims_order_receive_print")
{
	...
	<?
    	echo signature_table(212, $data[0], "1100px");
    ?>
</div>
<?
exit();
...
if($action=="trims_order_receive_print_2")
{
	...
	$sql_mst="select id, subcon_job, company_id, location_id, party_id, currency_id, party_location, delivery_date, rec_start_date, rec_end_date, receive_date, within_group, party_location, order_id, order_no,exchange_rate,team_leader,team_member,team_marchant,trims_ref,remarks,is_apply_last_update,revise_no, delivery_point,update_date
		from subcon_ord_mst
