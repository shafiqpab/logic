...
if($action=="load_drop_down_subsection")
{
	...
	else if($data[0]==7) $subID='19,20,21,25,26,27,28';
		...
		// echo "10**".$variable_bill_prod_delv."eeee"; die;
			...
			else
			{
				...
				if($variable_bill_prod_delv==1 || $variable_bill_prod_delv==2 || $variable_bill_prod_delv==3)
				{
					...
					$field_array="ready_to_approved*buyer_tb*remarks*updated_by*update_date";
					...
					$field_array4="order_id*book_con_dtls_id*description*color_id*size_id*qnty*rate*amount*booked_qty*style*updated_by*update_date*excess_cut*plan_cut*plan_cut_amount*ply*size_name*gmts_color_id*gmts_size_id";
					$data_array="'".$cbo_ready_to_approved."'*'".$txt_buyer_tb."'*'".$txt_remarks."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
					$field_array7="length*width*height*flap*gusset*thickness*measurementid";
		...
		foreach($data_array as $row)
		{
			...
			<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
				...
				<td>
					...
					else if($row[csf('section')]==7) $subID='19,20,21,25,26,27,28';
...
//                    print_r($data_array_size);
					...
					if($is_available_datas>0)
					{
						foreach($data_array as $row)
						{
							...
							<tr id="row_<? echo $k;?>">
								...
								<td><input type="text" id="txtStyle_<? echo $k;?>" name="txtStyle[]" class="text_boxes" style="width:80px" value="<? echo $styleRef; ?>" <? echo $disabled; ?> <? echo $disabled_next; ?> <? echo $disabled_next_delivery; ?>/></td>
								<td>
                                    <input type="text" id="txtdescription_<? echo $k;?>" name="txtdescription[]" class="text_boxes" style="width:80px" value="<? echo $data[0]; ?>" <? echo $disabled_next; ?> <? echo $disabled_next_delivery; ?> />
								</td>
								<td>
                                    <input type="text" id="txtply_<? echo $k;?>" name="txtply[]" class="text_boxes txt_ply" style="width:70px" value="<? echo $data[14]; ?>" <? echo $disabled_next_delivery; ?> />
                                </td>
								<td>
									<input type="text" id="txtgmtscolor_<? echo $k;?>" name="txtgmtscolor[]" class="text_boxes" style="width:70px" value="<? echo $data[15]; ?>" <? echo $disabled; ?> <? echo $disabled_next_delivery; ?> />
								...
								<td>
									<input type="text" id="txtgmtssize_<? echo $k;?>" name="txtgmtssize[]" class="text_boxes" style="width:70px" value="<? echo $data[16]; ?>" <? echo $disabled; ?> <? echo $disabled_next_delivery; ?>/>
								...
								<td>
                                    <input type="text" title="new" id="txtexcesscut_<? echo $k;?>" name="txtexcesscut[]"  class="text_boxes_numeric" style="width:60px" onKeyUp="sum_total_qnty(<? echo $k;?>)" value="<? echo number_format($data[11],0,'.',''); ?>" <? echo $disabled_next_delivery; ?>/>
								</td>
								<td>
                                    <input type="text" title="new" id="txtPlaneCut_<? echo $k;?>" name="txtPlaneCut[]"  class="text_boxes_numeric" style="width:60px" onKeyUp="sum_total_qnty(<? echo $k;?>)" value="<? echo number_format($data[12],4,'.',''); ?>" <? echo $disabled; ?>  <? echo $disabled_next_delivery; ?>/>
								</td>
								<td>
				...
				<tfoot>
					...
					<th><input type="text" id="txtplane_total" value="<? echo number_format($break_avg_rate, 6, '.', ''); ?>"  name="txtplane_total" class="text_boxes_numeric" readonly style="width:61px" value="" /></th>
					<th><input type="text" id="txt_average_rate" name="txt_average_rate" class="text_boxes_numeric" readonly style="width:61px" value="<? echo $break_avg_rate ?>"/></th>
					<th><input type="text" id="txt_total_order_amount" name="txt_total_order_amount" class="text_boxes_numeric" readonly style="width:70px" value="<? echo $break_total_value; ?>" /></th>
		...
		foreach($data_array as $row)
		{
			...
			<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
				...
				<td>
					...
					else if($row[csf('section')]==7) $subID='19,20,21,25,26,27';
					else if($row[csf('section')]==9) $subID='22';
					else $subID='0';
					?>
				</td>
