...
if ($action=="save_update_delete")
{
	...
	if ($operation==0) // Insert Start Here
	{
		...
		$field_array2="id, mst_id, job_no_mst, receive_dtls_id, booking_dtls_id, book_con_dtls_id, break_id, buyer_po_id,buyer_po_no, buyer_style_ref, item_description, color_id, size_id, sub_section, uom, job_quantity,  impression, material_color, conv_factor, is_copy_material, inserted_by, insert_date";
		$field_array3="id, mst_id, order_id, job_no_mst, book_con_dtls_id, product_id, description, specification, unit, pcs_unit, cons_qty, process_loss, process_loss_qty, req_qty, remarks, inserted_by, insert_date";
		...
		for($i=1; $i<=$total_row; $i++)
		{
			...
			if(str_replace("'","",$$txtcolorID)=="")
			{
				...
				{ 
					...
					{
						$color_id = return_id( str_replace("'","",$$txtcolor), $color_library_arr, "lib_color", "id,color_name","257");  
			...
			$data_array2 .="(".$id1.",".$id.",'".$new_job_no[0]."',".$$hdnDtlsIDs.",".$$hdnbookingDtlsId.",".$$bookConDtlsId.",".$$hdnBreakIDs.",'".$txtbuyerPoId."',".$$txtbuyerPo.",".$$txtstyleRef.",".$$txtdescription.",'".$color_id."','".$size_id."',".$$cboSubSection.",".$$cboUom.",".str_replace(",",'',$$txtJobQuantity).",".$$txtImpression.",'".$rawcolor."',".$$txtConvFactor.",".$copyChk.",'".$user_id."','".$pc_date_time."')";
			...
			for($j=0; $j<count($dtls_data); $j++)
			{
				...
				$data_array3.="(".$id3.",".$id1.",'".$hid_order_id."','".$new_job_no[0]."',".$$bookConDtlsId.",".$product_id.",".$description.",".$specification.",".$unit.",".$unit_pcs.",".$cons_qty.",".$process_loss.",".$process_loss_qty.",".$req_qty.",".$remarks.", ".$_SESSION['logic_erp']['user_id'].", '".$pc_date_time."')";
	...
	else if ($operation==1)   // Update Here
	{
		...
		$field_array2="buyer_po_id*buyer_po_no*buyer_style_ref*item_description*color_id*size_id*sub_section*uom*job_quantity*impression*material_color*conv_factor*is_copy_material*updated_by*update_date";
		...
		$field_array6="id, mst_id, job_no_mst, receive_dtls_id, booking_dtls_id, book_con_dtls_id, break_id, buyer_po_id,buyer_po_no, buyer_style_ref, item_description, color_id, size_id, sub_section, uom, job_quantity,  impression, material_color, conv_factor, is_copy_material, inserted_by, insert_date";
		...
		{			
			...
			{
				$data_array2[$aa]=explode("*",("'".$$txtbuyerPoId."'*".$$txtbuyerPo."*".$$txtstyleRef."*".$$txtdescription."*".$$txtcolorID."*".$$txtsizeID."*".$$cboSubSection."*".$$cboUom."*".str_replace(",",'',$$txtJobQuantity)."*".$$txtImpression."*'".$rawcolor."'*".$$txtConvFactor."*".$copyChk."*".$user_id."*'".$pc_date_time."'"));
				$hdn_dtls_id_arr[]=str_replace("'",'',$$hdnDtlsUpdateId);
			}
			else
			{
				if ($add_commaa_dtls!=0) $data_array6 .=","; $add_commaa_dtls=0;
				$data_array6 .="(".$id1.",".$update_id.",'".$txt_job_no."',".$$hdnDtlsIDs.",".$$hdnbookingDtlsId.",".$$bookConDtlsId.",".$$hdnBreakIDs.",'".$txtbuyerPoId."',".$$txtbuyerPo.",".$$txtstyleRef.",".$$txtdescription.",".$$txtcolorID.",".$$txtsizeID.",".$$cboSubSection.",".$$cboUom.",".str_replace(",",'',$$txtJobQuantity).",".$$txtImpression.",'".$rawcolor."',".$$txtConvFactor.",".$copyChk.",'".$user_id."','".$pc_date_time."')";
				$add_commaa_dtls++;
			}
			//echo "10**".$$hdnDtlsdata."#";
			$dtls_data=explode("**",str_replace("'",'',$$hdnDtlsdata));
			for($j=0; $j<=count($dtls_data); $j++)
			{
				...
				else
				{
					...
					if($product_id!='')
					{
						...
						$data_array5.="(".$id3.",".$dtlsId.",'".$hid_order_id."','".$txt_job_no."',".$product_id.",".$description.",".$specification.",".$unit.",".$unit_pcs.",".$cons_qty.",".$process_loss.",".$process_loss_qty.",".$req_qty.",".$remarks.",'".$user_id."','".$pc_date_time."')";
						$id3=$id3+1; $add_commadtls++;
					}
	...
	else if ($operation==2)   // delete here
	{
		...
		if($next_process!=''){
			echo "20**".str_replace("'",'',$txt_job_no)."**".str_replace("'",'',$update_id);oci_rollback($con); disconnect($con); die;
		...
		if($next_process_issue!=''){
			...
			echo "18**Delete restricted, This Information is used in Raw Material Issue.".$next_process_issue;oci_rollback($con); disconnect($con); die;
...
{	
	...
	*/
	...
	/*$job_sql= "select  a.subcon_job, a.section_id as section, sum(b.job_quantity) as qty 
	...
	group by   a.subcon_job, a.section_id";
	 //echo $sql;
	...
	?>
     <table class="rpt_table" border="1" cellpadding="0" cellspacing="0" rules="all" width="885" >
        ...
        <div style="width:885px; max-height:260px;overflow-y:scroll;" >	 
...
if($action=="create_order_search_list_view15022020")
{	
	$data=explode('_',$data);
	$party_id=str_replace("'","",$data[1]);
	$search_by=str_replace("'","",$data[4]);
	            ...
	            foreach($data_array as $row)
	            {
	            	...
	            	{*/
		            	...
		            	<tr bgcolor="<? echo $bgcolor; ?>" id="search<? echo $i; ?>" style="text-decoration:none; cursor:pointer" onClick="js_set_value(<? echo $i;?>)" > 
		            		...
		            		<td width="60"><? echo $trims_section[$row[csf('section')]]; ?></td>
		            		<td width="80"><? echo $group_arr[$row[csf('item_group')]]; ?></td>
		            		<td width="120" style="word-break:break-all" ><pre><? echo $row[csf('description')]; ?></pre></td>
		            		<td width="80"><? echo $row[csf('qnty')]; ?></td>
		            		<td width="60" style="text-align:center;"><? echo change_date_format($row[csf('receive_date')]); ?></td>
		            		<td width="60" style="text-align:center;"><? echo change_date_format($row[csf('delivery_date')]); ?></td>
		            		<td style="word-break:break-all" ><? echo $buyerBuyer; ?>
...
if( $action=='order_dtls_list_view' ) 
{
	...
	foreach($break_ids as $key=> $value)
	{
		if($ji==0)
		{
			$break_cond=" a.id in(".implode(",",$value).")"; 
		}
		else
		{
	...
	if($data[0]==1)
	{
		...
		$brk_sql= "select a.description,a.color_id,a.size_id $id_cond from subcon_ord_breakdown a where $break_cond group by a.description,a.color_id,a.size_id "; 
		...
		/*$dtls_po_sql= "select b.id ,b.booking_dtls_id,b.buyer_po_id from subcon_ord_dtls b where b.order_quantity<>0 and b.mst_id in ($data[3]) and b.section=$data[4] and b.status_active=1 and b.is_deleted=0";
		$dtls_po_res=sql_select($dtls_po_sql);  $buyer_poid_arr=array();
		foreach ($dtls_po_res as  $row) 
		{
			$buyer_poid_arr[$row[csf("id")]]["booking_dtls_id"] =$row[csf("booking_dtls_id")];
			$buyer_poid_arr[$row[csf("id")]]["buyer_po_id"] =$row[csf("buyer_po_id")];
		}*/
		
		$dtls_sql= "select a.description,a.color_id,a.size_id,b.sub_section,b.booked_uom,b.booked_conv_fac, sum(a.booked_qty) as qnty  $dtlsid_cond from subcon_ord_dtls b ,subcon_ord_breakdown a where a.mst_id=b.id and b.order_quantity<>0 and a.qnty<>0 and $break_cond and b.mst_id in ($data[3]) and section=$data[4] and b.job_no_mst=a.job_no_mst group by b.sub_section,b.booked_uom,b.booked_conv_fac,a.description,a.color_id,a.size_id"; //die;
		//die;
		//echo $brk_sql; die; //die; 
		...
		foreach ($data_array as  $rows) 
		{
			...
			/*$booking_dtls_ids=''; $buyer_po_id=''; 
			$mstIDS 			=$row[csf("mst_id")]->load(); 
			$mstIDS=array_unique(explode(",",$mstIDS));
			foreach ($mstIDS as  $rowId) 
			{
				$booking_dtls_ids .=$buyer_poid_arr[$rowId]["booking_dtls_id"].',';
				$buyer_po_id .=$buyer_poid_arr[$rowId]["buyer_po_id"].',';
			}*/

			$subconDtls_arr[$rows[csf("sub_section")]][$rows[csf("booked_uom")]][$rows[csf("booked_conv_fac")]][$rows[csf("description")]][$rows[csf("color_id")]][$rows[csf("size_id")]]["qnty"] +=$rows[csf("qnty")];
			//$subconDtls_arr[$rows[csf("sub_section")]][$rows[csf("booked_uom")]][$rows[csf("booked_conv_fac")]][$rows[csf("description")]][$rows[csf("color_id")]][$rows[csf("size_id")]]["booking_dtls_ids"] =chop($booking_dtls_ids,',');
			//$subconDtls_arr[$rows[csf("sub_section")]][$rows[csf("booked_uom")]][$rows[csf("booked_conv_fac")]][$rows[csf("description")]][$rows[csf("color_id")]][$rows[csf("size_id")]]["buyer_po_id"] ==chop($buyer_po_id,',');
		}
		//echo "<pre>";
		...
		{
			foreach($subSection_data as $booked_uom=> $booked_uom_data)
			{ 
	...
	else
	{
		...
		foreach ($data_array as  $row) 
		{
			...
			$dtls_arr[$row[csf("mst_id")]][$row[csf("id")]]["job_quantity"]		=$row[csf("job_quantity")];//$jobQnty;
		...
		foreach($dtls_arr as $dtls_data)
		{
			...
			foreach($dtls_data as $row)
			{
				...
				<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
					<td title="<? echo $row['buyer_po_no']; ?>"><input id="txtbuyerPo_<? echo $tblRow; ?>" name="txtbuyerPo[]"   class="text_boxes" type="text" value="<? echo $row['buyer_po_no']; ?>" style="width:100px" disabled />
	                ...
	                <td title="<? echo $row['buyer_style_ref']; ?>"><input id="txtstyleRef_<? echo $tblRow; ?>" name="txtstyleRef[]" type="text" class="text_boxes" value="<? echo $row['buyer_style_ref']; ?>"  style="width:100px" placeholder="Display" disabled/></td>
	                <td title="<? echo $row['item_description']; ?>"><input id="txtdescription_<? echo $tblRow; ?>" name="txtdescription[]" type="text" class="text_boxes" value="<? echo $row['item_description']; ?>"  style="width:100px" title="<? echo $row['item_description']; ?>" placeholder="Display" disabled/></td>
					...
					<td><input id="txtRawMat_<? echo $tblRow; ?>" name="txtRawMat[]" type="text" class="text_boxes" value="<? echo chop($row['description'],','); ?>" style="width:100px"  onClick="openmypage_row_metarial(2,'0',<? echo $tblRow; ?>)" placeholder="Double Click"/></td>
					<td><input id="txtImpression_<? echo $tblRow; ?>" name="txtImpression[]" type="text" class="text_boxes_numeric" value="<? echo $row['impression']; ?>" style="width:100px" placeholder="Write"/></td>
		...
		$brk_sql= "select a.description,a.color_id,a.size_id $id_cond from subcon_ord_breakdown a where job_no_mst='$data[5]' group by a.description,a.color_id,a.size_id "; 
		...
		$revised_dtls_sql= "select a.description,a.color_id,a.size_id,b.sub_section,b.booked_uom,b.booked_conv_fac, sum(a.booked_qty) as qnty  $dtlsid_cond from subcon_ord_dtls b ,subcon_ord_breakdown a where a.mst_id=b.id and b.order_quantity<>0 and a.qnty<>0 and b.mst_id in ($data[3]) and section=$data[4] and b.job_no_mst=a.job_no_mst group by b.sub_section,b.booked_uom,b.booked_conv_fac,a.description,a.color_id,a.size_id";
		...
		foreach ($revised_array as  $rows) 
		{
			...
			$buyer_po_id=$rows[csf("buyer_po_id")]->load();
			$subconDtls_arr[$rows[csf("sub_section")]][$rows[csf("booked_uom")]][$rows[csf("booked_conv_fac")]][$rows[csf("description")]][$rows[csf("color_id")]][$rows[csf("size_id")]]["qnty"] +=$rows[csf("qnty")];
			$subconDtls_arr[$rows[csf("sub_section")]][$rows[csf("booked_uom")]][$rows[csf("booked_conv_fac")]][$rows[csf("description")]][$rows[csf("color_id")]][$rows[csf("size_id")]]["booking_dtls_ids"] =$booking_dtls_ids;
			$subconDtls_arr[$rows[csf("sub_section")]][$rows[csf("booked_uom")]][$rows[csf("booked_conv_fac")]][$rows[csf("description")]][$rows[csf("color_id")]][$rows[csf("size_id")]]["buyer_po_id"] =$buyer_po_id;
		}
		//echo "<pre>";
		//print_r($rcvItemChk_arr); //die; 
		$a=1;
						...
						foreach($description_data as $color_id=> $color_data)
						{
							...
							foreach($color_data as $size_id => $row)
							{
								...
								if (!in_array($revisedItem, $rcvItemChk_arr))
								{
									...
									?>
									<tr bgcolor="<? echo $bgcolor; ?>" id="row_<? echo $tblRow; ?>" align="center">
										<td title="<? echo chop($buyer_po); ?>"><input id="txtbuyerPo_<? echo $tblRow; ?>" name="txtbuyerPo[]"   class="text_boxes" type="text" value="<? echo chop($buyer_po); ?>" style="width:100px" disabled />
											<input id="txtbuyerPoId_<? echo $tblRow; ?>" name="txtbuyerPoId[]" value="<? echo chop($buyer_po_id,","); ?>" class="text_boxes" type="hidden" style="width:70px" disabled />
										</td>
						                <td title="<? echo $row['buyer_style_ref']; ?>"><input id="txtstyleRef_<? echo $tblRow; ?>" name="txtstyleRef[]" type="text" class="text_boxes" value="<? echo chop($buyer_style,","); ?>"  style="width:100px" placeholder="Display" disabled/></td>
    ...
    <script>
		...
		function create_description_row(prod_ids,id,row)
	    {
	    	if(id!=0 && id!='')
	    	{
				//$("#row_"+row).remove();
				var row_num =  $('#tbl_share_details_entry tbody tr').length;
				var response_data = return_global_ajax_value(prod_ids + "**" + row_num + "**" + id , 'populate_prod_data', '', 'Job_card_preparation_controller');
	    	...
	    	else
	    	{
		        ...
		        var response_data = return_global_ajax_value(prod_ids + "**" + row_num , 'populate_prod_data', '', 'Job_card_preparation_controller');
	        ...
	        <table width="700" cellspacing="0" cellpadding="0" border="1" rules="all" class="rpt_table" align="center">
	            ...
	            <tbody>
	                <tr class="general">
	                    ...
	                    <td align="center">
	                        <input type="button" name="button2" class="formbutton" value="Show" onClick="show_list_view ( document.getElementById('cbo_company_name').value+'_'+document.getElementById('cbo_item_group').value+'_'+document.getElementById('txt_search_string').value+'_'+document.getElementById('txt_product_id').value+'_'+document.getElementById('txt_brand_id').value+'_'+document.getElementById('cbo_section').value, 'create_description_search_list_view', 'search_div', 'Job_card_preparation_controller', 'setFilterGrid(\'tbl_po_list\',-1)')" style="width:70px;" />
	                        <input type="hidden" name="hidden_mst_id" id="hidden_mst_id" class="datepicker" style="width:70px">
	                    </td>
	                    </tr>
	                    <tr>
...
if($action=="job_card_print")
{
	...
	$job_qnty_arr=array();
	//$qty_sql ="SELECT a.id, a.booked_qty from subcon_ord_breakdown a where a.booked_qty is not null and a.booked_qty!=0";
	$qty_sql= "select b.id as receive_details_id,a.id,a.description,a.color_id,a.size_id,b.sub_section,b.booked_uom,b.booked_conv_fac, a.booked_qty  from subcon_ord_dtls b ,subcon_ord_breakdown a where a.mst_id=b.id  and b.job_no_mst=a.job_no_mst and a.booked_qty is not null and a.booked_qty!=0";
	$qty_sql_res=sql_select($qty_sql);
	foreach ($qty_sql_res as $row)
	{
	...
	<div style="width:1060px;">
		<table width="1060" cellspacing="0" align="center" border="0">
			...
			<tr>
				<td colspan="6">&nbsp;</td>
				<!--<td>Order Qty </td><td colspan="3"  id="td_order_qty"></td>-->
...
if($action=="job_card_print(backup)")
{
	extract($_REQUEST);
	$data=explode('*',$data);
	//echo "select buy.id,buy.buyer_name from lib_buyer buy, lib_buyer_tag_company b where buy.status_active =1 and buy.is_deleted=0 and b.buyer_id=buy.id and b.tag_company='$data[0]'  and buy.id in (select  buyer_id from  lib_buyer_party_type where party_type in (2, 3)) order by buyer_name"; die;
	...
	<div style="width:1060px;">
		...
		<div style="width:100%;">
			...
			$mst_id=$data[1];
			$trims_job=$dataArray[0][csf('trims_job')]; //die;
			$sqlDtls_result =sql_select("select  id, mst_id, job_no_mst, receive_dtls_id, booking_dtls_id, book_con_dtls_id, break_id as subcon_break_ids, buyer_po_id,buyer_po_no, buyer_style_ref, item_description, color_id, size_id, uom, job_quantity,  impression, material_color from trims_job_card_dtls where mst_id=$data[1] and status_active=1 and is_deleted=0");
...
if($action=="job_card_print2")
{
	...
	<table width="1060" cellspacing="0" align="center" border="0">
		...
		<div style="width:100%;" id="details_id_tbl">
			...
			$mst_id=$data[1];
			$trims_job=$dataArray[0][csf('trims_job')]; //die;
			//echo "select  id, mst_id, job_no_mst, receive_dtls_id, booking_dtls_id, book_con_dtls_id, break_id as subcon_break_ids, buyer_po_id,buyer_po_no, buyer_style_ref, item_description, color_id, size_id, uom, job_quantity,  impression, material_color,conv_factor from trims_job_card_dtls where mst_id=$data[1] and status_active=1 and is_deleted=0";
			$sqlDtls_result =sql_select("select  id, mst_id, job_no_mst, receive_dtls_id, booking_dtls_id, book_con_dtls_id, break_id as subcon_break_ids, buyer_po_id,buyer_po_no, buyer_style_ref, item_description, color_id, size_id, uom, job_quantity,  impression, material_color,conv_factor from trims_job_card_dtls where mst_id=$data[1] and status_active=1 and is_deleted=0");
...
if($action=="job_card_print2(backup)")
{
	extract($_REQUEST);
	$data=explode('*',$data);
	$company_library=return_library_array( "select id, company_name from lib_company", "id", "company_name"  );
