...
if ($action=="save_update_delete")
{
	...
	else if ($operation==1)   // Update Here
	{
		...
		$field_array="location_id*within_group*party_id*party_location*currency_id*receive_date*delivery_date*rec_start_date*rec_end_date*order_id*order_no*exchange_rate*team_leader*team_member*remarks*updated_by*update_date";		
		$data_array="'".$cbo_location_name."'*'".$cbo_within_group."'*'".$cbo_party_name."'*'".$cbo_party_location."'*'".$cbo_currency."'*'".$txt_order_receive_date."'*'".$txt_delivery_date."'*'".$txt_rec_start_date."'*'".$txt_rec_end_date."'*'".$hid_order_id."'*'".$txt_order_no."'*'".$txt_exchange_rate."'*'".$cbo_team_leader."'*'".$cbo_team_member."'*'".$txt_remarks."'*".$_SESSION['logic_erp']['user_id']."*'".$pc_date_time."'";
		...
		//echo "10**".$rID.'='.$rID2.'='.$rID3.'='.$rID4.'='.$rID6.'='.$rID7.'='.$rID8; die;
		//if($rID4) $flag=1; else $flag=0;
...
{
	$nameArray=sql_select( "select id, subcon_job, company_id, location_id, party_id, currency_id, party_location, delivery_date, rec_start_date, rec_end_date, receive_date, within_group, party_location, order_id, order_no,exchange_rate,team_leader,team_member,remarks from subcon_ord_mst where subcon_job='$data' and entry_form=255 and status_active=1" );
		...
		<form name="qntypopup_1"  id="qntypopup_1" autocomplete="off">
			<table class="rpt_table" width="630px" cellspacing="0" cellpadding="0" rules="all" id="tbl_share_details_entry">
				...
				<tbody>
					...
					/*if($type==2)
					{
						$sql_break_down="select id, mst_id, order_id, job_no_mst, book_con_dtls_id, item_id, body_part, embellishment_type, description, color_id, size_id, qnty, rate, amount from subcon_ord_breakdown where mst_id='$hdnDtlsUpdateId' and job_no_mst='$job_no' order by id ASC"; //die;
						...
						foreach($data_break_down as $row)
						{
							...
							if($data_break=="") $data_break.=$row[csf('description')].'_'.$color_arr[$row[csf('color_id')]].'_'.$size_arr[$row[csf('size_id')]].'_'.$row[csf('qnty')].'_'.$row[csf('rate')].'_'.$row[csf('amount')].'_'.$row[csf('id')].'_'.$row[csf('id')].'_'.$row[csf('color_id')].'_'.$row[csf('size_id')];
							else $data_break.='***'.$row[csf('description')].'_'.$color_arr[$row[csf('color_id')]].'_'.$size_arr[$row[csf('size_id')]].'_'.$row[csf('qnty')].'_'.$row[csf('rate')].'_'.$row[csf('amount')].'_'.$row[csf('id')].'_'.$row[csf('id')].'_'.$row[csf('color_id')].'_'.$row[csf('size_id')];
						}
					}
					else
					{
						//echo "nazim"; die;
						//echo $within_group."==".$data_break; die;
						//1**RED***HFF***4.3333***2.4***10.3999***85702***2917,fqfqewfqwef***OFF WHITE 11-102 [HB651]***HFF***4***2.4***9.6***85703***2918,
						if($within_group==1 && $data_break=="")
						{	
							$sql = "select  id, wo_trim_booking_dtls_id, job_no,  po_break_down_id,  item_color, item_size, requirment,description, brand_supplier, rate, amount from wo_trim_book_con_dtls where wo_trim_booking_dtls_id='$booking_dtls_id' and po_break_down_id='$booking_po_id' and status_active=1 and is_deleted=0 and requirment!=0 order by id ASC"; //die;
							...
							{//echo "1--";
								...
								if($break_down_arr[$row[csf('id')]]=="") $break_down_arr[$row[csf('id')]]=0;
								//echo $data_break."++";
								if($data_break=="") $data_break.=$row[csf('description')].'_'.$color_arr[$row[csf('item_color')]].'_'.$size_arr[$row[csf('item_size')]].'_'.$row[csf('requirment')].'_'.$row[csf('rate')].'_'.$row[csf('amount')].'_'.$row[csf('id')].'_'.$break_down_arr[$row[csf('id')]].'_'.$row[csf('item_color')].'_'.$row[csf('item_size')];
								else $data_break.='***'.$row[csf('description')].'_'.$color_arr[$row[csf('item_color')]].'_'.$size_arr[$row[csf('item_size')]].'_'.$row[csf('requirment')].'_'.$row[csf('rate')].'_'.$row[csf('amount')].'_'.$row[csf('id')].'_'.$break_down_arr[$row[csf('id')]].'_'.$row[csf('item_color')].'_'.$row[csf('item_size')];
							}
						}
						//echo $data_break;
					}*/
...
if ($action == 'btn_load_change_bookings') {
	$sql = "select id, company_id, buyer_id, job_no, booking_no from wo_booking_mst where status_active=1 and is_deleted=0  and  is_apply_last_update=2";
	$data_array = sql_select($sql);
	echo count($data_array);
	exit();
}
